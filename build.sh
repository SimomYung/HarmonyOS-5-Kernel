...
#kernel-headers
			...
			cd ${TOPDIR}/${mod_path}/build/kernel
			cp ${TOPDIR}/scripts/${mod_path}/kernel/.config_headers ${TOPDIR}/${mod_path}/kernel/
			cp -f ${TOPDIR}/scripts/${mod_path}/kernel/HMbuild ${TOPDIR}/${mod_path}/kernel/
			cp -f ${TOPDIR}/scripts/${mod_path}/modules/HMbuild ${TOPDIR}/${mod_path}/modules/
			cp -f ${TOPDIR}/scripts/${mod_path}/kernel/arch/aarch64/HMbuild ${TOPDIR}/${mod_path}/kernel/arch/aarch64/
			ln -sf ${TOPDIR}/${mod_path}/arch/aarch64/include ${TOPDIR}/${mod_path}/kernel/arch/aarch64/include
			cp ../../kernel/.config_headers .config
			cmake -DCMAKE_MODULE_PATH:PATH=$TOPDIR/output/aarch64/usr/share/cmake/Modules \
				-DHMNATIVESDKPATH:PATH=$TOPDIR/output/tools \
				-DCMAKE_SYSROOT:PATH=$TOPDIR/output/aarch64 \
				-DCMAKE_TOOLCHAIN_FILE=$TOPDIR/output/aarch64/usr/share/cmake/Modules/$TOOLCHAIN_FILE \
				-DBUILD_USE_HMBUILD=TRUE \
				${TOPDIR}/${mod_path}/kernel
			export LD_LIBRARY_PATH=${LD_LIBRARY_PATH}:$(${TOOLCHAIN_CC} -print-file-name=plugin)
			cmake --build ${TOPDIR}/${mod_path}/build/kernel --verbose -- -j 32
			install -d ${TOPDIR}/output/aarch64/usr/share/info
			install .config ${TOPDIR}/output/aarch64/usr/share/info/config-hm-kernel-headers
			DESTDIR="${TOPDIR}/output/aarch64" cmake --install ${TOPDIR}/${mod_path}/build/kernel
#ulibs-headers
