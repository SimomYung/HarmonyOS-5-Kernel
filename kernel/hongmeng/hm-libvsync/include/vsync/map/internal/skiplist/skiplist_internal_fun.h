 ...
 */
static inline void _vskiplist_init_node(vskiplist_node_t *node, vsize_t height)
{
 ...
 */
static inline vskiplist_node_t *_vskiplist_lookup(vskiplist_t *lst,
						  vskiplist_key_t key,
						  vbool_t accept_next_gt_key)
{
	...
	ASSERT(curr != head);
	if (curr != tail &&
	    (lst->fun_cmp(curr, key) == 0 || accept_next_gt_key)) {
		return curr;
	}
	return NULL;
}
 ...
 */
static inline vbool_t _vskiplist_logically_remove_node(vskiplist_node_t *node)
{
	vsize_t level = 0;
	vskiplist_node_t *succ = NULL;
	...
	for (level = top_level; level <= top_level; level--) {
		...
		do {
             ...
             * node on this level */
			i_marked_it = vatomicptr_markable_cmpxchg(
				&node->next[level], succ, false, succ, true);
		} while (!i_marked_it);
	} /* for all levels */
 ...
 */
static inline vbool_t _vskiplist_detach_node(vskiplist_t *lst,
					     vskiplist_key_t key,
					     vskiplist_node_t *node)
{
 ...
 */
static inline void _vskiplist_visit_level(vskiplist_t *lst, vsize_t level,
					  void (*visit)(vskiplist_node_t *,
							vbool_t, void *),
					  void *args)
{
	vskiplist_node_t *head = &lst->head;
 ...
 */
static inline vbool_t _vskiplist_disconnect_node(vskiplist_node_t *node)
{
	vsize_t non_snipped = vatomicsz_dec_get_rlx(&node->non_snipped_level);
	ASSERT(non_snipped <= SKIPLIST_MAX_HEIGHT);
 ...
 */
static inline void _vskiplist_destroy_node(vskiplist_node_t *node,
