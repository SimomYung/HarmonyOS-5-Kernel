 ...
 *`-DSKIPLIST_MAX_HEIGHT=H`. `H=32` by default.
 *
 * @example
 ...
 */
static inline void vskiplist_init(vskiplist_t *lst,
				  vskiplist_cmp_node_t fun_cmp,
				  vskiplist_handle_node_t fun_retire,
				  void *fun_retire_arg, vuint32_t rand_seed)
{
	vsize_t i = 0;
	vskiplist_node_t *head = &lst->head;
	vskiplist_node_t *tail = &lst->tail;

	_vskiplist_seed_init(&lst->seed, rand_seed, SKIPLIST_MAX_HEIGHT);
	...
	_vskiplist_init_node(head, SKIPLIST_MAX_HEIGHT);
	_vskiplist_init_node(tail, SKIPLIST_MAX_HEIGHT);
 ...
 * @param lst address of vskiplist_t object.
 */
static inline void vskiplist_destroy(vskiplist_t *lst)
{
	vsize_t top_level = SKIPLIST_MAX_HEIGHT - 1;
	/**
 ...
 */
static inline vskiplist_node_t *vskiplist_lookup(vskiplist_t *lst,
						 vskiplist_key_t key)
{
 ...
 */
static inline vbool_t vskiplist_add(vskiplist_t *lst, vskiplist_key_t key,
				    vskiplist_node_t *node,
				    vskiplist_node_t **out_node)

{
	vskiplist_node_t *preds[SKIPLIST_MAX_HEIGHT];
	vskiplist_node_t *succs[SKIPLIST_MAX_HEIGHT];
...
#if defined(VSYNC_VERIFICATION) && defined(SKIP_LIST_ONE_HEIGHT)
	vsize_t height = SKIPLIST_MAX_HEIGHT;
#else
	vsize_t height = _vskiplist_get_rand_height(&lst->seed);
	...
	while (true) {
		...
		for (level = 0; level < height; level++) {
			succ = succs[level];
			vatomicptr_markable_set(&node->next[level], succ,
						false);
		}
		...
		if (!vatomicptr_markable_cmpxchg(&pred->next[0], succ, false,
						 node, false)) {
			continue;
		}
		...
		for (level = 1; level < height; level++) {
			while (true) {
				...
				do {
                     ...
                     * */
					cur_succ = (vskiplist_node_t *)
						vatomicptr_markable_get(
							&node->next[level],
							&mark);
					...
					&node->next[level], cur_succ, mark,
					succ, mark));

				// we connect the node to this level
				if (vatomicptr_markable_cmpxchg(
					    &pred->next[level], succ, false,
					    node, false)) {
				...
				_vskiplist_find(lst, key, preds, succs, false,
						NULL);
			} // while(true)
		} // for
		return true;
 ...
 * @note call within SMR critical section.
 */
static inline vbool_t vskiplist_remove(vskiplist_t *lst, vskiplist_key_t key)
{
	vskiplist_node_t *preds[SKIPLIST_MAX_HEIGHT];
	vskiplist_node_t *succs[SKIPLIST_MAX_HEIGHT];
 ...
 * @note call within SMR critical section.
 */
static inline vskiplist_node_t *vskiplist_lookup_next(vskiplist_t *lst,
						      vskiplist_key_t key)
{
 ...
 */
static inline vskiplist_node_t *vskiplist_get_succ(vskiplist_t *lst,
						   vskiplist_node_t *curr)
{
	...
	succ_next = (vskiplist_node_t *)vatomicptr_markable_get_pointer(
		&curr->next[level]);
 ...
 * @note call within  SMR critical section.
 */
static inline vskiplist_node_t *vskiplist_get_first(vskiplist_t *lst)
{
	vbool_t marked = false;
	vskiplist_node_t *node = NULL;

	node = (vskiplist_node_t *)vatomicptr_markable_get_pointer(
		&lst->head.next[0]);
 ...
 */
static inline vbool_t vskiplist_remove_node(vskiplist_t *lst,
					    vskiplist_key_t key,
					    vskiplist_node_t *node)
{
	vskiplist_node_t *succs[SKIPLIST_MAX_HEIGHT];
 ...
 * @note call within  SMR critical section.
 */
static inline void vskiplist_cleanup(vskiplist_t *lst)
{
		...
		while (curr != tail) {
			...
			while (marked) {
				...
				snip = vatomicptr_markable_cmpxchg(
					&pred->next[level], curr, false, succ,
					false);
				if (!snip) {
				...
				o_non_snip_lvl = vatomicsz_get_dec(
					&curr->non_snipped_level);
				...
				curr = (vskiplist_node_t *)
					vatomicptr_markable_get_pointer(
						&pred->next[level]);
				succ = (vskiplist_node_t *)
					vatomicptr_markable_get(
						&curr->next[level], &marked);
			} // while(marked)
