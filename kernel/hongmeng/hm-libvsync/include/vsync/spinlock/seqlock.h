 ...
 * Create: Mon Jul 7 14:27:00 CEST 2022
 */
 ...
 */
static inline seqvalue_t _seqlock_await_even(seqlock_t *seq)
{
 ...
 */
static inline void seqlock_init(seqlock_t *seq)
{
 ...
 * critical section.
 */
static inline void seqlock_acquire(seqlock_t *seq)
{
	seqvalue_t count = 0;
	...
	do {
		...
		o_count =
			vatomic32_cmpxchg_acq(&seq->seqcount, count, count + 1);
 ...
 */
static inline void seqlock_release(seqlock_t *seq)
{
 ...
 * critical section.
 */
static inline seqvalue_t seqlock_rbegin(seqlock_t *seq)
{
 ...
 * data.
 */
static inline vbool_t seqlock_rend(seqlock_t *seq, seqvalue_t sv)
{
	ASSERT(VIS_EVEN(sv));
	vatomic_fence_acq();
