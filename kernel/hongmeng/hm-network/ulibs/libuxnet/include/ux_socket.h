...
typedef struct ux_socket {
	...
	/* events in client side, for socket cache */
	uint32_t events;
...
static inline void ux_epoll_ctx_set_socket(const ux_socket_t *s, ux_epoll_context_t *ctx)
{
	BUG_ON(s == NULL || ctx == NULL);

	ctx->socket = (void *)s;
}

static inline ux_socket_t *ux_epoll_ctx_get_socket(ux_epoll_context_t *ctx)
{
	return (ux_socket_t *)ctx->socket;
}
...
typedef struct ux_proto_ops {
	...
	int (*getpeername)(ux_socket_t *sock, struct sockaddr *addr,
			   socklen_t *addrlen);
	int (*getsockevent)(ux_socket_t *sock, unsigned int *rcvev, unsigned int *sendev,
