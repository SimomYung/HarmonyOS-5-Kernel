...
static int lwip_inet_getsockevent(ux_socket_t *sock, unsigned int *rcvev,
				  unsigned int *sendev, unsigned int *errev,
				  int *error)
{
	ux_lwip_sock_t *lwip_sock = lwip_sock_from_socket(sock);
	int ret;

	ret = lwip_getsockevent(lwip_sock->fd, rcvev, sendev, errev, error);
	return ret;
}
...
static ux_proto_ops_t lwip_inet_ops = {
	...
	.getsockevent = lwip_inet_getsockevent,
