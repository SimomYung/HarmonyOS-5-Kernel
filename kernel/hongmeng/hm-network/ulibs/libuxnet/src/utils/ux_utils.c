...
static void __ux_wq_wakeup(struct ux_wq_head *waitqueue, bool all)
{
	...
	dlist_for_each_entry_safe(waiter, next, &waitqueue->wq, struct ux_wq_entry, node) {
		...
		(void)hm_futex_wake(&(waiter->wait), 1, FUTEX_TYPE_PRIVATE);
		if (!all) {
			break;
		}
	}
...
void ux_wq_wakeup(struct ux_wq_head *waitqueue)
{
	__ux_wq_wakeup(waitqueue, false);
	return;
}

void ux_wq_wakeup_all(struct ux_wq_head *waitqueue)
{
	__ux_wq_wakeup(waitqueue, true);
	return;
}
