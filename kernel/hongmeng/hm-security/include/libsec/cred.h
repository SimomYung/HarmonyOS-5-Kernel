...
#define MAC_ADR_OF_SH_CRED(sh_cred) ({ \
	hm_pacdb_shcred_check((sh_cred), SEC_PAC_SHARED_MAC, (sh_cred)->idx); \
	(&(sh_cred)->cred_mac); \
})

#define BASIC_ADR_OF_SH_CRED(sh_cred) ({ \
	hm_pacdb_shcred_check((sh_cred), SEC_PAC_SHARED_BASIC, (sh_cred)->idx); \
	(&(sh_cred)->cred_basic); \
})

#define PROP_ADR_OF_SH_CRED(sh_cred) ({ \
	hm_pacdb_shcred_check((sh_cred), SEC_PAC_SHARED_PROPERTY, (sh_cred)->idx); \
	(&(sh_cred)->prop); \
})

#define AUDIT_ADR_OF_SH_CRED(sh_cred) ({ \
	hm_pacdb_shcred_check((sh_cred), SEC_PAC_SHARED_AUDIT, (sh_cred)->idx); \
	(&(sh_cred)->cred_audit); \
})
...
#define DAC_ADR_OF_CRED(b_cred) ({ \
	hm_pacga_bcred_check((b_cred), SEC_PAC_BASE_IDX); \
	hm_pacdb_bcred_check((b_cred), SEC_PAC_BASE_DAC, (b_cred)->idx); \
	(&(b_cred)->cred_dac); \
})

#define SEL_ADR_OF_CRED(b_cred) ({ \
	hm_pacga_bcred_check((b_cred), SEC_PAC_BASE_IDX); \
	hm_pacdb_bcred_check((b_cred), SEC_PAC_BASE_SEHARMONY, (b_cred)->idx); \
	(&(b_cred)->cred_sel); \
})

#define CAPS_ADR_OF_CRED(b_cred) ({ \
	hm_pacga_bcred_check((b_cred), SEC_PAC_BASE_IDX); \
	hm_pacdb_bcred_check((b_cred), SEC_PAC_BASE_CAPS, (b_cred)->idx); \
	(&(b_cred)->cred_caps); \
})

#define KEYRING_ADR_OF_CRED(b_cred) ({ \
	hm_pacga_bcred_check((b_cred), SEC_PAC_BASE_IDX); \
	hm_pacdb_bcred_check((b_cred), SEC_PAC_BASE_KEYRING, (b_cred)->idx); \
	(&(b_cred)->cred_key); \
})

#define NSID_GROUP_ADR_OF_CRED(b_cred) ({ \
	hm_pacdb_bcred_check((b_cred), SEC_PAC_BASE_NSID_GROUP, (b_cred)->idx); \
	(&(b_cred)->nsid_group); \
})
