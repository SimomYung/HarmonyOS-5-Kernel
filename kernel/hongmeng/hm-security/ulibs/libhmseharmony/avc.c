...
static hmsel_av_t
hmsel_avc_audit_required(struct hmsel_audit_data *audit_data, struct hmsel_av_decision *avd,
...
static inline bool try_check_entry_pac(struct ac_entry *entry, uint64_t pac,
	uint64_t pac_expected, hmsel_seq_t seq_pre, hmsel_seq_t seq_post)
{
	if (pac != pac_expected) {
		if (!entry_seq_match(seq_pre, seq_post)) {
			return false;
		}
		hm_error("[PAC AC DFI] ac entry check fail, pac %llx %llx\n",
			(unsigned long long)pac, (unsigned long long)pac_expected);
		_hm_pac_record_fault(entry->ssid, (uint64_t)(uintptr_t)entry, pac, pac_expected);
	}
	return true;
}

static inline void hm_pacga_ac_entry_check(struct ac_entry *entry, bool fast)
{
	for (;;) {
		hmsel_seq_t seq_pre = entry_seq_read_pre(entry);
		uint64_t pac = hm_pacga_ac_entry(entry, fast);
		uint64_t pac_expected = (fast ? entry->pac_fast : entry->pac);
		hmsel_seq_t seq_post = entry_seq_read_post(entry);
		if (try_check_entry_pac(entry, pac, pac_expected, seq_pre, seq_post)) {
			return;
		}
	}
}
...
{
	hm_pacga_ac_entry_check(entry, true);
	return (ssid == entry->ssid) && (tsid == entry->tsid);
}

static bool entry_cmp(hmsel_sid_t ssid, hmsel_sid_t tsid, hmsel_class_t tclass,
		      struct ac_entry *entry)
{
	hm_pacga_ac_entry_check(entry, false);
	return (ssid == entry->ssid) && (tsid == entry->tsid) && (tclass == entry->tclass);
}
			     ...
			     bool is_read)
{
	...
	if (entry_cmp_fast(ssid, tsid, entry)) {
		if (dest != NULL) {
			...
			bool is_read)
{
	...
	if (entry_cmp(ssid, tsid, tclass, entry)) {
		if (dest != NULL) {
			/* xp_node might be freed during ac_add */
			if (is_read) {
				(void)raw_rwlock_rdlock(&entry->xpn_lock);
...
static unsigned int
__xperms_has_perm(struct hmsel_xperms_decision *xpd, uint8_t xperms, uint8_t type)
{
	unsigned int rc = 0;
	switch (type) {
