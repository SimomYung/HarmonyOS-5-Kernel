...
static inline void
arch_actv_conf_init(struct arch_actv_conf_s *arch_actv_conf)
{
...
{
	UNUSED(caller_uctx);
	REG_RPCINFO(&uctx->regs) = rpc_info;
}
...
void arch_actv_switch_contextidr(struct actv_s *actv);
static inline void actv_switch_perf_hook(struct actv_s *actv)
{
	if (static_branch_unlikely(&actv_perf_knob)) {
		if (static_branch_unlikely(&actv_spe_knob)) {
			arch_actv_switch_contextidr(actv);
		}
		if (static_branch_unlikely(&actv_brbe_knob)) {
			actv_switch_pause_brbe(actv);
		}
	}
}
#else
