...
#ifndef __HOST_LLT__
#define read_sysreg(name)			__aarch64_read_sysreg(name)
#else
#define read_sysreg(name)			llt_read_sysreg(name)
#endif

/*
 * Suppress pclint warning about 'v unused'.
 * Directly add UNUSED(v) to write_sysreg not work because
 * pclint would think the variable __v is useless and the last
 * value assigned to __v is not used.
 */
static inline void __aarch64_write_sysreg_use_v(u64 v)
{
	UNUSED(v);
}
...
do { \
	__u64 __v = (__u64)(v); \
	__aarch64_write_sysreg(name, __v); \
	__aarch64_write_sysreg_use_v(__v); \
} while(0)
