			    ...
			    unsigned long bitset2_mask)
{
	 ...
	 * For example, the rwlock only uses FUTEX_BITSET2_WR and
	 * FUTEX_BITSET2_RD bit on bitset2 to identify that it is
	 * a reader lock or a writer lock.
	 * If we want to determine if there are any usage scenarios
	 * for rwlock, we need to & FUTEX_BITSET2_RWLOCK_RW_MASK
	 * to distinguish it.
	 *
