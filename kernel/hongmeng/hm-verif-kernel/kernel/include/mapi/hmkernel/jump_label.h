...
int static_key_enable(struct static_key *key);
int static_key_disable(struct static_key *key);
int static_key_inc(struct static_key *key);
int static_key_dec(struct static_key *key);
#else
static inline int static_key_enable(struct static_key *key)
{
	if (vatomic32_read(&key->enabled) != 1) {
		vatomic32_write((vatomic32_t *)&key->enabled, 1);
	}
	return E_HM_OK;
}

static inline int static_key_disable(struct static_key *key)
{
	if (vatomic32_read(&key->enabled) != 0) {
		vatomic32_write((vatomic32_t *)&key->enabled, 0);
	}
	return E_HM_OK;
}
