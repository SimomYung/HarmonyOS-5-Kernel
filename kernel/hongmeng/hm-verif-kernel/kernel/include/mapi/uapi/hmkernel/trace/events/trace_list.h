...
TRACE_TCB_EVENT(sched, hmlt_update_tcb_load,
	...
	TP_ARGS(tcb, cpu, capacity, runtime_us, runtime, curr_load, prev_load, hists, top_load_hists),
	TP_FIELD(
		__array(char, name, 16)
		__field(__s32, tid)
		__field(__u32, cpu)
		...
		__array(__u16, top_load_hists, 3)
	),
	TP_ASSIGN(
		str_cpy_p_to_a(__entry->name, tcb_name_of(tcb));
	...
	TP_PRINT("tid=%d comm=%s runtime_us=%"__PRIu64" runtime=%"__PRIu64" "
		 ...
		 __entry->top_load_hists[1], __entry->top_load_hists[2]
	)
)
...
TRACE_TCB_EVENT(sched, sched_choose_cpu,
	TP_PROTO(struct tcb_s *, tcb, bool, prefer_idle, unsigned int, load, int, boost),
	TP_ARGS(tcb, prefer_idle, load, boost),
	TP_FIELD(
		__field(__s32, tid)
		__field(__s32, prio)
...
TRACE_TCB_EVENT(sched, sched_lt_boost,
	TP_PROTO(unsigned int, id, int, boost),
	TP_ARGS(id, boost),
	TP_FIELD(
		__field(__u32, id)
		__field(__u32, boost)
	...
	TP_PRINT("group id = %d, boost = %d ", __entry->id, __entry->boost
	)
)

TRACE_TCB_EVENT(sched, sched_lt_overutil,
	TP_PROTO(unsigned int, id, bool, overutil_enable),
	TP_ARGS(id, overutil_enable),
	TP_FIELD(
		__field(__u32, id)
		__field(bool, overutil_enable)
	),
	TP_ASSIGN(
		__entry->id = id;
		__entry->overutil_enable = overutil_enable;
	),
	TP_PRINT("group id = %d, overutil_enable = %d ", __entry->id, __entry->overutil_enable
	)
)
...
TRACE_TCB_EVENT(power_kernel, eer_prefer_cluster_each,
	TP_PROTO(const struct tcb_s *, tcb,
		 unsigned int, cluster_id,
...
TRACE_TCB_EVENT(power_kernel, sched_ind_get_util,
	...
	TP_ARGS(cpu, chg_event, util, cluster_load, top_load, ddl_load,
		ddl_freq, min_util, iowait, cluster_min_load, max_tcb_load, raw_load),
	TP_FIELD(
		__field(__u32, cpu)
	...
	TP_PRINT("cpu=%u util=%u cluster_load=%u "
		 ...
		 "ddl_freq=%u min_util=%u event=%u iowait=%u "
		 "cluster_min_load=%u max_load=%u raw_load =%u",
		 __entry->cpu, __entry->util, __entry->cluster_load,
		 __entry->top_load, __entry->ddl_load, __entry->ddl_freq,
...
TRACE_TCB_EVENT(power_kernel, mipslt_sched_ind_data,
	...
	TP_ARGS(cpu, chg_event, avg_ipc, cpu_demand, task_demand, inst, cycle, ddl_load),
	TP_FIELD(
		__field(unsigned int, cpu)
	...
	TP_PRINT("cpu=%u chg_event=%u avg_ipc=%u cpu_demand=%u task_demand=%u inst=%u cycle=%u ddl_load=%u",
		  __entry->cpu, __entry->chg_event, __entry->avg_ipc, __entry->cpu_demand, __entry->task_demand, __entry->inst,
		  __entry->cycle, __entry->ddl_load
	)
)
...
TRACE_TCB_EVENT(power_kernel, phase_task_delta,
	...
	TP_ARGS(tcb, delta_exec, delta, len),
	TP_FIELD(
		__array(char, name, 16)
		__field(int, tid)
