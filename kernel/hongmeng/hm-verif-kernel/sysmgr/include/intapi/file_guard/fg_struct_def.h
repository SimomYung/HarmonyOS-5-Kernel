...
enum fg_command_type {
	...
	FG_CMD_IPC_UID_WHITE_LIST_RULE = 121
};
...
#define get_network_segment_boundary_size(segments) get_rules_size(fg_network_segment_boundary_t, \
																   fg_network_segment_addr_t, segments)

typedef struct fg_network_segment_other_policy {
	fg_network_segment_policy_t flag; // trusted, untrusted, unset (relative trusted)
} fg_network_segment_other_policy_t;
...
// utils
enum {
	BOLCK_CACHE_KEYWORD = 0,
	BOLCK_CACHE_KIA_FILE,
	BOLCK_CACHE_MAX
};

typedef struct block_rule_profile {
	fg_process_data_t process;
	fg_path_data_t file_path[BOLCK_CACHE_MAX];
	uint64_t time_stamp[BOLCK_CACHE_MAX];
	uint32_t new_idx;
	bool valid;
...
static inline const char *get_command_name(uint32_t cmd)
{
	...
	case FG_CMD_COMMON_RULE:
		return "common rule";
	case FG_CMD_COMMON_STOP_DRIVER:
		return "stop driver";
	...
	case FG_CMD_IPC_UID_WHITE_LIST_RULE:
		return "uid white list";
	default:
		return "undefined";
	}
}
