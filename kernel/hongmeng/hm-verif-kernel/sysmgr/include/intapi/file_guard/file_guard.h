...
void fg_report_kia_variant(const char *origin_path, const char *variant_path);
void fg_report_rename(const char *old_path, const char *new_path);

void do_new_file_notify(const char *fpath);
void do_usb_notify(block_rule_profile_t *block_rule_data, const char *usb_fname);
...
int lookup_block_rule(pid_t pid, block_rule_profile_t *block_rule);
