...
/* prefer alloc from kern zone */
#define PAF_PREFER_KZ		(uint32_t)0x00400000

/* alloc from compact */
#define PAF_COMPACT		(uint32_t)0x00800000
...
static inline bool paf_is_core(uint32_t paf)
{
	return ((paf & __PAF_CORE) != 0);
}

static inline bool paf_is_emerg(uint32_t paf)
{
	return ((paf & __PAF_EMRG) != 0);
}
