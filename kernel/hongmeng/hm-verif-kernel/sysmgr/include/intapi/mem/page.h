...
__attribute__((noinline)) void dump_pgstr_maybe(struct vspace_s *vs, uintptr_t abort_addr);
...
#define PAGE_FLAG_MASK		PAGE_FLAG_CAST(0x0210FFF0)
...
{											\
	BUG_ON_D(pg == NULL);								\
	return ((vatomic64_read((vatomic64_t *)&((pg)->flags)) &			\
...
{											\
	...
	(void)(vatomic64_or((vatomic64_t *)&((pg)->flags),				\
...
{											\
	...
	(void)(vatomic64_and((vatomic64_t *)&((pg)->flags),				\
...
{											\
	BUG_ON_D(pg == NULL);								\
	page_set_flag_check(pg, PAGE_F_##uppercase_flag);				\
	return ((vatomic64_get_or((vatomic64_t *)&((pg)->flags),			\
...
{											\
	BUG_ON_D(pg == NULL);								\
	page_clear_flag_check(pg, PAGE_F_##uppercase_flag);				\
	return ((vatomic64_get_and((vatomic64_t *)&(pg->flags),				\
		~(PAGE_F_##uppercase_flag)) & (PAGE_F_##uppercase_flag)) != 0);		\
}											\
...
#define SYSMGR_PROTECT_LRU_LEVEL_MASK	0x3UL
...
static inline int fscache_page_protect_level_from_sysmgr(struct page_s *page)
{
...
static inline void page_set_order(struct page_s *pg, uint32_t order)
{
	...
	do {
		old_val = raw_atomic_long_read((raw_atomic_long_t *)&pg->flags);
		new_val = (long)(((uint64_t)old_val & ~PAGE_ORDER_MASK) | order_to_flags(order));
		ret_val = raw_atomic_long_cas((raw_atomic_long_t *)&pg->flags, old_val, new_val);
...
static inline void page_set_order(struct page_s *pg, uint32_t order)
{
	...
	do {
		old_val = raw_atomic_int_read((raw_atomic_int_t *)&pg->flags);
		new_val = (int)(((uint32_t)old_val & ~PAGE_ORDER_MASK) | order_to_flags(order));
		ret_val = raw_atomic_int_cas((raw_atomic_int_t *)&pg->flags, old_val, new_val);
	} while (ret_val != old_val);
}
#endif
...
bool cma_reuse_enable(void);
void cma_reuse_set_enable(bool enable);
#else
static inline bool cma_reuse_enable(void)
{
	return false;
}
static inline void cma_reuse_set_enable(bool enable) {}
#endif
