...
#else
static inline void vspace_set_tsan_mmap(struct vspace_s *vspace)
{
	return;
}
#endif
...
struct vspace_s *vspace_create_remote(const struct process_s *process, cref_t owner,
				      cref_t to_grant, bool is_vm);

struct vspace_s *vspace_create_at_current(const struct process_s *process);
...
void vspace_set_may_invalid(struct vspace_s *vspace);

void vspace_clear_may_invalid(struct vspace_s *vspace);

int vspace_release_resource(struct vspace_s *vs);

bool vspace_may_invalid(struct vspace_s *vspace);
...
void vspace_main_proc_name_of(struct vspace_s *v, char *buf, size_t size);
