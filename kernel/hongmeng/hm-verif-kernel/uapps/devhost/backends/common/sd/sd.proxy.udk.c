...
static int udk_dma_buf(unsigned int cmd, void *rd, size_t rd_size,
		       void *wr, size_t wr_size)
{
	int ret = 0;
	int ret1, ret2;
	...
	switch (PAL_PROXY_MINOR_CMD_OF(cmd)) {
		case UDK_DMA_BUF_MINOR_CMD_COPY:
			if (sizeof(int) > wr_size) {
				ret = -EFBIG;
			} else {
				ret1 = udk_copy_to_user(wr, (void *)&w_test, sizeof(int));
			}

			if (sizeof(int) > rd_size) {
				ret = -EFBIG;
			} else {
				ret2 = udk_copy_from_user((void *)&r_test, rd, sizeof(int));
			}
			break;
		default:
			ret = -ENOSYS;
...
static int sd_export_dev_create(void)
{
	...
	(void)udk_pal_proxy_register(PAL_PROXY_MAJOR(UDK_DMA_BUF_MAJOR_CMD), udk_dma_buf);
