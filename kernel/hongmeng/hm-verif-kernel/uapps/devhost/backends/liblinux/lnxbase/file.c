...
{
	int ret = ksys_rename(oldpath, newpath);
	return (ret >= 0) ? 0 : -hmerrno2posix(ret);
}

int lnxbase_ksys_readlinkat(int dfd, const char *path, char *buf, size_t bufsize)
{
	ssize_t ret = ksys_readlinkat(dfd, path, buf, bufsize);
	if (ret > INT_MAX) {
...
int lnxbase_ksys_unlink(const char *pathname)
{
	...
	if (ret < 0) {
		dh_error("vfs unlink failed, pathname:%s, ret:%s\n",
			 pathname, hmstrerror(ret));
		return -hmerrno2posix(ret);
	}
	return 0;
...
{
	long ret = ksys_symlink(oldname, newname);
	return (ret >= 0) ? 0 : -hmerrno2posix(ret);
}
...
{
	int ret = E_HM_OK;
	ret = ksys_fchown(fd, uid, gid);
	if (ret < 0) {
		dh_error("vfs fchown failed, fd:%d, uid:%u, gid:%u, ret:%s\n",
...
{
	int fd = 0;
	fd = ksys_open(filename, flag, mode);
	if (fd < 0) {
...
{
	int fd = 0;
	fd = ksys_openat(dfd, filename, flag, mode);
	if (fd < 0) {
...
{
	int ret = ksys_ioctl(fd, cmd, arg, arg_sz);
	return (ret >= 0) ? 0 : -hmerrno2posix(ret);
}
...
{
	int ret = E_HM_OK;
	ret = ksys_lseek(fd, offset, whence, pos);
...
{
	ssize_t ret = ksys_write(fd, buf, nbyte);
	return (ret >= 0) ? ret : -hmerrno2posix((int)ret);
}
...
{
	ssize_t ret = ksys_read(fd, buf, nbyte);
	return (ret >= 0) ? ret : -hmerrno2posix((int)ret);
}

ssize_t lnxbase_ksys_readdir(int fd, void *buf, size_t nbyte)
{
	ssize_t ret = ksys_readdir(fd, buf, nbyte);
	return (ret >= 0) ? ret : -hmerrno2posix((int)ret);
}

int lnxbase_ksys_fsync(int fd)
{
	int ret = ksys_fsync(fd);
	return (ret >= 0) ? 0 : -hmerrno2posix(ret);
}
