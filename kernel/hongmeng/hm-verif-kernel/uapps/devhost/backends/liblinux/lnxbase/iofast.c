...
static unsigned int lnxbase_iofast_flags_from_gfp(unsigned int gfp_flags)
{
	...
	if (____IS(gfp_flags & LIBLINUX_PAL_GFP_CONTINUOUS)) {
		flags |= IOFAST_CONTINUOUS;
	}
 ...
 *	LIBLINUX_PAL_GFP_CONTINUOUS <--> IOFAST_CONTINUOUS:
 ...
 *   2. If IOFAST_CONTINUOUS is set, pat_array[1..num-1] must meet the
 *      order size, otherwise the allocation will be terminated and the
 *      number of pa actually allocated will be returned.
 *   3. If IOFAST_CONTINUOUS is not set, pa_array[1..num-1] can be smaller
 *      than the order size.
 */
