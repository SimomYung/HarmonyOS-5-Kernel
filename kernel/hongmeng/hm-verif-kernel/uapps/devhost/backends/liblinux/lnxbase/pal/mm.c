				       ...
				       unsigned long size, unsigned int flags)
{
	...
	ret = actvcall_hmcall_mem_change_secmem_linmap_attr(vaddr, phys_addr, size, hint);
	if (ret < 0) {
...
int liblinux_pal_vm_zap_sp(unsigned long addr, unsigned long len, int pid)
{
	int ret;

	ret = actvxactcall_hmcall_mem_munmap_other(false, true, (const void*)addr, len, pid, 1U);
	if (ret < 0) {
		return -hmerrno2posix(ret);
	}
...
int liblinux_pal_usermap_munmap_sp(unsigned long addr, unsigned long len, int pid)
{
	int ret;
	ret = actvxactcall_hmcall_mem_munmap_other(false, true, (const void*)addr, len, pid, 0);
	if (ret < 0) {
		return -hmerrno2posix(ret);
	}
	return 0;
...
int liblinux_pal_copy_msghdr_to_user(void *dst, void *src, void *uaddr, void *ucmsg)
{
	int ret;

	ret = devhost_pal_copy_msghdr_to_user(dst, src, uaddr, ucmsg);
	if (unlikely(ret < 0)) {
		return -hmerrno2posix(ret);
	}
