...
void nvme_tagset_free_tags(struct udk_blk_mq_tags *tags)
{
	udk_blk_mq_free_tags(tags);
}

struct udk_blk_mq_tags *nvme_tagset_init_tags(struct udk_blk_mq_tag_set *set)
{
...
void mas_tag_nvme_tags_init(bool nvme_flag, struct udk_blk_mq_tag_set *tag_set)
{
	...
	blk_mq_tagset_nvme_mq_iosched_enable(tag_set, true);
