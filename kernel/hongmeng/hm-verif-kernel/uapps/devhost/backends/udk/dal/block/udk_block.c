...
static int udk_blk_end_io(struct udk_bio *bio)
{
	...
	if (udk_bio_has_crypt_ctx(bio)) {
		if (udk_bio_data_dir(bio) == UDK_REQ_WRITE) {
			udk_blk_crypto_mempool_mem_free(bio);
...
#define BIO_QUEUE_LIMIT_SIZE		(1024 * 1024)
static bool should_queue_end_io(const struct udk_bio *bio)
{
	return (udk_bio_data_dir(bio) == UDK_REQ_WRITE) &&
		(bio->bi_size >= BIO_QUEUE_LIMIT_SIZE);
}
...
static bool udk_bio_param_vaild(struct udk_bio *bio)
{
	...
	if (bio->bi_ctx.direct_map_paddr == 0) {
		return false;
	}
...
/**
 * Split from the end of the bio. Bio will not change after call this fuction,
 * you can call udk_bio_reduce to cut it down after split.
 * Incoming bio      |-----------|
 * Return split bio         |----|
 *
...
struct udk_bio *udk_bio_split(struct udk_bio *bio, unsigned int sectors)
{
	...
	vec_offset = (udk_bio_sectors(bio) - sectors) >> UDK_PFN_PER_SECTOR_SHIFT;
	vec_cnt = sectors >> UDK_PFN_PER_SECTOR_SHIFT;

	return udk_bio_clone(bio, vec_offset, vec_cnt);
}
...
void udk_bio_reduce(struct udk_bio *bio, unsigned int sectors)
{
	if (bio == NULL || sectors >= udk_bio_sectors(bio) ||
	    (sectors & UDK_PFN_PER_SECTOR_MASK) != 0) {
		udk_warn("reduce bio %u invalid\n", sectors);
		return;
	}

	bio->bi_size -= sectors << UDK_BLKSHIFT_DEFAULT;
	bio->bi_vec_num -= sectors >> UDK_PFN_PER_SECTOR_SHIFT;
	bio->bi_vec_size -= sectors >> UDK_PFN_PER_SECTOR_SHIFT;
}
...
int udk_bio_end(struct udk_bio *bio)
{
	...
	if ((bio->bi_ctx.async) && (bio->bi_ctx.direct_map_paddr)) {
		is_should_queue = should_queue_end_io(bio);
		if(is_should_queue) {
			  ...
			  struct udk_bio_pvec *pvec, struct udk_simple_bio_set *bio_set)
{
	...
	bio = udk_bio_alloc(pvec_num, true);
	if (bio == NULL) {
		return -EINVAL;
	}
	...
	bio->bi_vec_num = pvec_num;
	bio->bi_size = pvec->len;
	set_udk_bio_context(bio, bio_set->cb_private);
	bio->bi_ctx.devhost_end_bio_fn = bio_set->cb;
#ifdef UDK_FUSION
