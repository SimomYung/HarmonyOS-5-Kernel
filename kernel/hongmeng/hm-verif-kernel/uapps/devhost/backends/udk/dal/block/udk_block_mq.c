...
#include <udk/block/mas_blk/mas_blk_iosched_mq.h>
#endif
...
retry:
	...
	tags = udk_blk_mq_hctx_tags(ctx->hctx, req_flags);
#ifdef CONFIG_MAS_BLK
...
static int udk_blk_mq_sched_request(struct udk_request *req)
{
	struct udk_request_queue *queue = req->queue;
	struct udk_io_scheduler_queue *sched = queue->scheduler;
	struct udk_blk_mq_hw_ctx *hctx = req->mq_hctx;
...
{
	int ret = 0;
	struct raw_rwlock_attr rwlock_attr;
	...
	/* freeze rwlock init */
	raw_rwlock_attr_set_wrhold(&rwlock_attr, 1);
	raw_rwlock_init(&queue->freeze_lock, &rwlock_attr);
...
{
	(void)raw_rwlock_wrlock(&queue->freeze_lock);
}

void udk_blk_mq_unfreeze_queue(struct udk_request_queue *queue)
{
	(void)raw_rwlock_unlock(&queue->freeze_lock);
}

int udk_blk_mq_queue_enter(struct udk_request_queue *queue)
{
	int ret = raw_rwlock_rdlock(&queue->freeze_lock);
