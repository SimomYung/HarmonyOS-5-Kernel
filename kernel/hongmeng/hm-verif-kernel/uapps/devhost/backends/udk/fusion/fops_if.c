...
int libdh_dev_close(struct hmsrv_io_ctx *ctx)
{
	 ...
	 * all fusion devices should support fast open for performace
	 */
	if (IS_REF_VALID(ctx->dh_ap_rref)) {
		(void)hm_ucap_reject(0, ctx->dh_ap_rref);
		ctx->dh_ap_rref = ERR_TO_REF(E_HM_INVAL);
	}

	return udk_fusion_fops_close(u64_to_ptr(ctx->filp, struct filp_node));
}
