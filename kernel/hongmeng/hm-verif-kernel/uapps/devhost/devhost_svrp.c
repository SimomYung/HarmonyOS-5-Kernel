...
typedef struct {
	uint8_t buf[DEVHOST_ACTV_BUF_SIZE];
} dh_actv_rcvbuf_t;

static cref_t g_devhost_ap;
static struct actv g_init_actvs[DEVHOST_ACTV_INIT_NUM];
static dh_actv_rcvbuf_t g_init_actv_rcvdata[DEVHOST_ACTV_INIT_NUM];
...
void devhost_init_actvs_create(void)
{
	...
	while (index < DEVHOST_ACTV_INIT_NUM) {
		mem_zero_s(attr);
		attr.actv_pool		= g_devhost_ap;
		attr.recv_buf		= g_init_actv_rcvdata[index].buf;
		attr.recv_buf_sz	= sizeof(g_init_actv_rcvdata[index].buf),
		attr.tsd_size		= DEVHOST_ACTV_TSD_SIZE,
		attr.stack_size		= DEVHOST_ACTV_STACK_SIZE,
		attr.actvret_size	= sizeof(union __actvret_drvcall_devhost),
		attr.action_table	= sysif_devhost_actvcapcallhdlr_table,
		attr.action_table_sz	= sizeof(void *) * (size_t)__devhost_method_MAX,
		attr.actv_init		= devhost_actv_init;

		err = hm_actv_create(&g_init_actvs[index], &attr);
		if (err != E_HM_OK) {
			dh_panic("dh init actv create failed: i: %d, err: %s\n",
					index, hmstrerror(err));
		}

		err = syscap_ActivationReady(g_init_actvs[index].obj.cref);
		if (err != E_HM_OK) {
			dh_panic("dh init actv set ready failed: i: %d, err: %s\n",
					index, hmstrerror(err));
		}
		index++;
	}
