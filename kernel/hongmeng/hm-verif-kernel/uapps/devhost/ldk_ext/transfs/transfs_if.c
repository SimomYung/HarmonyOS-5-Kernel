...
void devhost_transfs_put_dentry(void *sb, void *dentry)
{
	...
	ret = get_sb_index_by_sb(sb, &sb_index);
	if (ret < 0) {
		hm_error("get sb index error, ret:%s\n", hmstrerror(ret));
		return;
	}

	ret = get_d_index_by_dentry(sb_index, dentry, &d_index);
	if (ret < 0) {
		hm_error("get dentry index error, ret:%s\n", hmstrerror(ret));
		return;
	}
