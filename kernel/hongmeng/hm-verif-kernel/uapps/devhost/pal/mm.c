...
static inline unsigned int curr_nsupervs(void)
{
	...
	if (likely(!xact_owner_is_myself())) {
		...
		nsupervs = __RPC_INFO_DECODE_XACT_SUPERVSIDX(local->rpcinfo);
	}
	return nsupervs;
}
...
static struct raw_rwlock g_vma_idr_lock = RAW_RDLOCK_INITIALIZER;
...
void devhost_pal_usermap_release(const struct devhost_vma_info *vma_info)
{
	...
	if (vma != vma_info) {
		dh_error("invalid vma info [%lx-%lx] of [%u-%u], id [%u-%u]\n",
			 ptr_to_ulong(vma_info), ptr_to_ulong(vma), vma_info->vma_id, vma->vma_id,
				...
				struct devhost_vma_info **out)
{
	...
	if (!iomap_prepared_ahead) {
		...
		if (ret != E_HM_OK) {
			...
			dh_error("mem_iomap_prepare failed, addr=%#lx, len=%#lx, prot=%#lx, ret=%d\n",
				addr, len, prot, ret);
			return ret;
		}
				...
				const bool iomap_prepared_ahead, int success, int update)
{
	...
	if (!iomap_prepared_ahead) {
		if (update != 0) {
			flag |= MFLAG_IOMAP_UPDATE;
		}
		ret = actvxactcall_hmcall_mem_iomap_finish(true, true, (const void *)user_vaddr, flag, vma_info->vma_id);
	}
	if (ret != E_HM_OK) {
...
int devhost_pal_copy_msghdr_to_user(void *dst, void *src, void *uaddr, void *ucmsg)
{
