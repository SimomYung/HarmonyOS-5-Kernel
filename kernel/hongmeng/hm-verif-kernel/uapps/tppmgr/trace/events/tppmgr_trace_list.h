...
TRACE_EVENT(power, mips_cpu_freq,
	...
	TP_ARGS(cpu, reporting_policy, cpum_freq, taskm_freq, pid_freq, top_freq, maxm_freq, freq),
	TP_FIELD(
		__field(unsigned int, cpu)
	...
	TP_PRINT("cpu=%u reporting_policy=%u cpum_freq=%u taskm_freq=%u pid_freq=%u "
		"top_freq=%u maxm_freq=%u freq=%u",
		__entry->cpu, __entry->reporting_policy, __entry->cpum_freq, __entry->taskm_freq,
		__entry->pid_freq, __entry->top_freq, __entry->maxm_freq, __entry->freq
	)
)
...
TRACE_EVENT(power, update_mips_cluster_freq,
	...
	TP_ARGS(cluster_id, ddl_freq, next_freq),
	TP_FIELD(
	...
	TP_PRINT("cluster_id=%u ddl_freq=%u next_freq=%u",
		__entry->cluster_id, __entry->ddl_freq, __entry->next_freq
	)
)
