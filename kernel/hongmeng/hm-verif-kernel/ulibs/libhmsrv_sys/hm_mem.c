...
#define MMAP_SLEEP_TIME 10000U
#define MMAP_RETRY_NUMS 512
...
static inline bool need_retry(int rc, uint32_t flags)
{
	return ((rc == E_HM_NOMEM) &&
		(flags & MFLAG_MAP_RETRY) != 0U);
}

/*
 * Sleep and loop try if map failed with no memory.
 * Because there may be some free memory in the way
 * of memory reclaim.
 */
static int try_mmap(const struct local_map_arg_s *args, uintptr_t *rvaddr, bool *unmapped)
{
	int tries = MMAP_RETRY_NUMS;
	int rc;

	rc = do_map_call(args, rvaddr, unmapped);
	if (need_retry(rc, args->flags)) {
		while (tries-- > 0) {
			(void)hm_usleep(MMAP_SLEEP_TIME);
			rc = do_map_call(args, rvaddr, unmapped);
			if (rc != E_HM_NOMEM) {
				break;
			}
		   ...
		   bool *unmapped)
{
	...
	args.srv_rref = srv;
	args.guard_size = 0;
	rc = try_mmap(&args, rvaddr, unmapped);
	return rc;
}
...
int hm_mem_cma_setup(uint64_t *vaddr, unsigned long len)
{
	...
	hva = (uint64_t)(uintptr_t)hm_mem_mmap(0, len,
