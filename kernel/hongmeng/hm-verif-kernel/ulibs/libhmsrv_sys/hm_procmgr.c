...
pid_t hm_mysid(void)
{
	return actvxactcall_hmcall_proc_mysid(true, false);
}
...
pid_t hm_fork(void)
{
	...
	mem_zero_s(req);
	req.clone_flags = SIGCHLD;

	/* realise fork through clone process */
	return hm_clone_ng(&req);
}
...
int hm_clone_ng(const struct hm_clone_req *req)
{
	...
	is_clone_process = ((req->clone_flags & __U(CLONE_THREAD)) == 0U) ? __true : __false;
	mem_zero_s(ret);
	...
	if (err < 0) {
		...
		if (!is_clone_process) {
			err = hm_thread_resume(ret.cref);
			if ((err < 0) && (err != E_HM_DEAD)) {
				hm_panic("hm_thread_resume failed: %s\n", hmstrerror(err));
			}
		}
	} else {
		/*
		 * reinit epoll is similar to epoll_ctl ADD, hence other lock and server
		 * should be ready before reinit epoll
		 */
		if (req->entry != 0U) {
			rc = (*(int(*)(void *))(uintptr_t)req->entry)((void *)(uintptr_t)req->args);
			hm_thread_exit(rc);
