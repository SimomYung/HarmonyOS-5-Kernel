...
#include <hmkernel/sched/sched_attr.h>
...
static inline unsigned int ufutex_hash(int *uaddr)
{
...
static void init_waiter_info(struct waiter_info_s * waiter, volatile int *uaddr, int val, uint32_t bitset)
{
	waiter->uaddr = uaddr;
	waiter->val = val;
...
static void waiter_insert(struct waiter_info_s *waiter)
{
	...
	while (p != NULL) {
		...
		BUG_ON(curr == waiter, "-----error-----already in tree, %p,l:%p, r:%p, p:%p, root:%p\n",
				p, p->rbt_left, p->rbt_right, rbt_parent(p), tree->rbt_root.root);
...
static struct waiter_info_s *waiter_next(struct user_futex_s *futex_tree, struct waiter_info_s *curr)
{
	...
	if (curr != NULL) {
		while (i < total_num) {
			...
			if (i < deq_num) {
				...
				actual_deq_num = i;
				rbt_remove(&ufutex_tree(curr->uaddr)->rbt_root, (struct rbt_node *)&curr->tree_node);
				dlist_delete(&curr->list_node);
				BUG_ON(i > iov_size);
			} else {
...
static unsigned int futex_get_wake_bitset(unsigned int waiter_bitset, unsigned int bitset1, unsigned int bitset2)
{
	unsigned int wake_bitset;

	/* If bitset is matched, means block_at->futex's bitset is same with bitset which transfered by userspace */
	if (bitset2 != 0U && (waiter_bitset & bitset2 & FUTEX_BITSET2_RWLOCK_RW_MASK)) {
		wake_bitset = bitset2;
	} else {
		wake_bitset = bitset1;
	}
...
static void do_enqueue_waiters(struct waiter_info_s * waiter, unsigned int idx, struct get_waiters_args_s *args)
{
	...
	if (idx < __UFUTEX_MAX_UNBLOCK_TARGET) {
		...
		struct user_futex_s *tree = ufutex_tree(waiter->uaddr);
		rbt_remove(&tree->rbt_root, (struct rbt_node *)&waiter->tree_node);
		dlist_delete(&waiter->list_node);
	} else {
		/*
				...
				unsigned long flags)
{
	...
	unsigned long true_bitset1 = (flags & __FUTEX_FLAG_BITLOCK) ? FUTEX_BITSET_MATCH_ANY : bitset;
	unsigned long true_bitset2 = (flags & __FUTEX_FLAG_BITLOCK) ? 0 : bitset2;

	while ((num_to_wake > 0) && args.waiters_left) {
		int cpu = spinlock_acquire_preempt(&tree->slock);
		unsigned int num_added = get_waiters_to_wake_ex(uaddr, num_to_wake, true_bitset1, true_bitset2,  &args);
		if ((flags & __FUTEX_FLAG_BITLOCK) && (args.next_waiter == NULL)) {
			clear_bitlock_wait_bit(uaddr, bitset, bitset2);
		}
		spinlock_release_preempt(&tree->slock, cpu);
