...
static void thread_lock(void)
{
	...
	__tl_lock();
#elif !defined __HOST_LLT__ && !defined CONFIG_RAW_THREAD_COMPATIBLE_WITH_PTHREAD
...
{
	int err;
	struct __actvret_hmcall_proc_clone ret;
	...
	clone_flags = raw_attr->flags | CLONE_CHILD_CLEARTID | CLONE_THREAD |
		      CLONE_VM | CLONE_SIGHAND | CLONE_SETTLS;
	hm_flags = THREAD_CONFIG_FLAGS_MASK_CALLER_VSPACE | THREAD_CONFIG_FLAGS_MASK_SERVICES_CREATE;
	if (thread_config_flags_create_pre_actv(clone_flags)) {
		hm_flags |= THREAD_CONFIG_FLAGS_MASK_CREATE_PRE_ACTV;
	}
	...
	if (CMPT_THREAD_REG(new, DETACH_STAT) == DT_DETACHED) {
		thread_setup_config(&req, CMPT_THREAD_REG(new, THREAD_STACK), THREAD_TO_TLS(new), entry,
				    arg, clone_flags, NULL, (void *)&raw_tl_lock, hm_flags);
	} else {
		thread_setup_config(&req, CMPT_THREAD_REG(new, THREAD_STACK), THREAD_TO_TLS(new), entry,
				    arg, clone_flags, NULL, (void *)CMPT_THREAD_REF(new, tid), hm_flags);
	}

	err = actvcall_hmcall_proc_clone(req, &ret);
	if (err < 0) {
		hm_warn("actvcall_hmcall_proc_clone failed, err=%s\n", hmstrerror(err));
	}
	if (err >= 0) {
		new->cref.tcb_cref = ret.cref;
		CMPT_THREAD_REG(new, tid) = ret.tid;
		err = E_HM_OK;
	}
		      ...
		      void *args)
{
	...
	if (ret == E_HM_OK) {
		ret = do_raw_thread_create(&attr, new, raw_thread_entry, new);
		if (ret != E_HM_OK) {
