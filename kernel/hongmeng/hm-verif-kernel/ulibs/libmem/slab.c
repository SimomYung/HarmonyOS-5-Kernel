					...
					const struct slab_desc_range *s_range)
{
	...
	s_desc->s_range.real_start = s_range->real_start;
	s_desc->s_range.real_size = s_range->real_size;
	dlist_init(&s_desc->node);
	s_desc->ator = NULL;
	s_desc->magic = SLAB_INIT_MAGIC;
...
static void slab_desc_insert(struct slab_ator *s_ator, struct slab_desc *s_desc)
{
	...
	if (unlikely(s_ator->slab_page_hold == (unsigned int)INT_MAX)) {
		hm_debug("allocator[%s] slab page hold number reach to maximum, stop counting\n",
			 s_ator->name);
	} else {
		/* here only insert into empty so just one */
		s_ator->slab_page_hold++;
	}
}
...
static void slab_desc_remove(struct slab_ator *s_ator, struct slab_desc *s_desc)
{
	...
	if (unlikely(s_ator->slab_page_hold == 0U)) {
		hm_debug("allocator[%s] slab page hold number reach to zero, stop counting\n",
			 s_ator->name);
	} else {
		s_ator->slab_page_hold--;
	}
		      ...
		      struct slab_pool *s_pool, uint32_t s_flags)
{
	...
	s_ator->slab_page_hold = 0;
	s_ator->used_slot_num = 0;
#ifdef CONFIG_SLAB_DEBUG
...
{
	RAW_MUTEX_GUARD(_, &s_ator->lock);
	return (s_ator->slab_page_hold > SLAB_PAGE_HOLD_WATERMARK_NUM);
}
			   ...
			   struct slab_desc *s_desc)
{
	...
	if (unlikely(s_desc->magic != SLAB_DESC_MAGIC || idx >= s_ator->max_slot_count)) {
...
int slab_pool_push_memory(struct slab_pool *s_pool, uintptr_t start, size_t size)
{
	...
	if (ret == E_HM_OK) {
		ret = slab_check_and_convert_range(start, size, (size_t)SLAB_PAGE_SIZE, &real_start, &real_size);
		if (unlikely(ret != E_HM_OK)) {
			start_ptr = ulong_to_ptr(start, void);
			hm_debug("push memory size is not enough start:(0x%lx), size:(0x%zx)\n",
				 vaddr_of(start_ptr), size);
		}
	}
	...
	if (ret == E_HM_OK) {
		...
		s_range.real_start = real_start;
		s_range.real_size = real_size;
		s_desc = slab_desc_init(real_start, &s_range);
		...
		/* update total_size free_size grown_size and real_size */
		...
		(void)raw_atomic_ullong_add_relaxed(&s_pool->real_total_size, (uint64_t)s_range.real_size);
	}
...
uintptr_t slab_pool_pop_memory(struct slab_pool *s_pool)
{
	...
	if (likely(s_pool != NULL)) {
		...
		if (!dlist_empty(&(s_pool->mem_list))) {
			...
			if (used) {
				start_ptr = ulong_to_ptr(s_desc->s_range.real_start, void);
				memm_slab_error_cb((uintptr_t)s_desc);
				hm_panic("slab page 0x%lx in mem list is in use, real start is 0x%lx,"
					 "size is %zu, ator is %s\n", vaddr_of(s_desc),
					 vaddr_of(start_ptr),
					 s_desc->s_range.real_size, s_desc->ator->name);
			}
			...
			(void)raw_atomic_ullong_sub_relaxed(&s_pool->real_total_size, (uint64_t)s_desc->s_range.real_size);
...
size_t slab_mem_size(struct slab_pool *s_pool, enum slab_size_type types)
{
	...
	if (likely(s_pool != NULL)) {
		...
		case SLAB_GROWN_SIZE:
			...
			break;
		case SLAB_REAL_SIZE:
			ret_size = raw_atomic_ullong_read_relaxed(&s_pool->real_total_size);
			break;
		case SLAB_USED_SIZE:
			ret_size = raw_atomic_ullong_read_relaxed(&s_pool->total_size);
			ret_size -= raw_atomic_ullong_read_relaxed(&s_pool->free_size);
...
void slab_dump_nolock(struct slab_info_s *info, struct slab_pool *s_pool)
{
	...
	info->real_total_slab_size = slab_mem_size(s_pool, SLAB_REAL_SIZE);
	hmmem_debug("======== SLAB_POOL INFOMATION ========\n");
	hmmem_debug("[SLAB_POOL]: slab page size total:[%zu b], free:[%zu b], grown:[%zu b], real:[%zu b]\n",
		    info->total_slab_size, info->free_slab_size,
		    info->grown_slab_size, info->real_total_slab_size);
}

static void dump_slab_desc(const struct slab_desc *desc, uint32_t *used_num)
{
	hmmem_debug("|--------- [slab page] vaddr: 0x%lx, used_num: %"PRIu32", ator: 0x%lx\n",
		    ptr_to_ulong(desc), desc->used_num, ptr_to_ulong(desc->ator));
	hmmem_debug("|--------- [slab page] start = 0x%"PRIxPTR", o_size = 0x%zx, r_start = 0x%"PRIxPTR", r_size = 0x%zx\n\n",
		    desc->s_range.ori_start, desc->s_range.ori_size, desc->s_range.real_start, desc->s_range.real_size);
	if (used_num != NULL) {
		*used_num = desc->used_num;
	}
}

/*
 * This function will dump info stored in slab_desc of every slab page
 * in the slab pool. It uses hmmem_debug to dump the info so the msg is
 * printed only when the mem_debug_switch is on.
 */
void slab_dump_pool_desc(const struct slab_pool *pool)
{
	struct slab_desc *slab_desc_unused = NULL;
	struct slab_desc *tmp = NULL;

	/* dump slab page in pool */
	hmmem_debug("======== SLAB_POOL FREE MEM NODE INFOMATION ========\n");
	dlist_for_each_entry_safe(slab_desc_unused, tmp, &(pool->mem_list),
				  struct slab_desc, node) {
		dump_slab_desc(slab_desc_unused, NULL);
	}
}

static void check_slab_page_content(struct slab_desc *curr)
{
	if (unlikely(IS_INVALID_PTR(curr->node.next) || (curr->magic != SLAB_DESC_MAGIC))) {
		memm_slab_error_cb((uintptr_t)curr);
	}
}
			   ...
			   struct slab_slot_info_s *slot_info)
{
	...
	struct slab_desc *tmp = NULL;
	uint32_t total_used_num = 0;
	uint32_t total_slot = 0;
	uint32_t used_num;
	int rc;

	hmmem_debug("|-- [allocator %s]: SLAB_PAGE SIZE: %u, obj_size: %"PRIu32"\n",
		    ator->name, SLAB_PAGE_SIZE, ator->obj_size);
	hmmem_debug("|-- [allocator %s]: max_slot_count: %"PRIu32", slab_page_hold: %u\n",
		    ator->name, ator->max_slot_count, ator->slab_page_hold);

	ator_info->in_use = 1;
	ator_info->slab_page_size = SLAB_PAGE_SIZE;
	ator_info->obj_size = ator->obj_size;
	ator_info->max_slot_count = ator->max_slot_count;
	ator_info->slab_page_hold = ator->slab_page_hold;
	ator_info->flags = ator->flags;
	rc = strncpy_s(ator_info->name, SLAB_NAME_LEN, ator->name, SLAB_NAME_LEN - 1U);
	BUG_ON(rc != 0);

	hmmem_debug("|---- [empty slab descript]:\n");
	dlist_for_each_entry_safe(desc, tmp, &(ator->empty), struct slab_desc, node) {
		check_slab_page_content(desc);
		dump_slab_desc(desc, &used_num);
		total_used_num += used_num;
		total_slot += ator->max_slot_count;
		ator_info->empty_num++;
	}
	hmmem_debug("|---- [partial slab descript]:\n");
	dlist_for_each_entry_safe(desc, tmp, &(ator->partial), struct slab_desc, node) {
		check_slab_page_content(desc);
		dump_slab_desc(desc, &used_num);
		total_used_num += used_num;
		total_slot += ator->max_slot_count;
		ator_info->partial_num++;
	}
	hmmem_debug("|---- [full slab descript]:\n");
	...
	ator_info->slot_used = total_used_num;
	ator_info->slot_free = total_slot - total_used_num;
	hmmem_debug("|-- [allocator %s]: total slot: %"PRIu32", total used_num: %"PRIu32"\n",
		    ator->name, total_slot, total_used_num);
	hmmem_debug("|-- [allocator %s]: empty: %u, partial: %u, full: %u\n\n",
		    ator->name, ator_info->empty_num, ator_info->partial_num, ator_info->full_num);
...
size_t slab_get_ator_free_size(struct slab_ator *ator)
{
	...
	if (ator != NULL) {
		raw_mutex_lock(&ator->lock);
		free_size = (size_t)ator->obj_size * (ator->max_slot_count * ator->slab_page_hold - ator->used_slot_num);
		raw_mutex_unlock(&ator->lock);
	}
