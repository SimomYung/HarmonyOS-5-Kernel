...
#define SLAB_INIT_MAGIC	0
#define SLAB_DESC_MAGIC	0x44455343
...
struct slab_desc_range {
	uintptr_t ori_start; /* alloc start vaddr this may not be align */
	size_t ori_size; /* alloc origin size */
	uintptr_t real_start; /* align to SLAB PAGE vaddr */
	size_t real_size; /* really size */
};
...
struct slab_ator {
	...
	unsigned int slab_page_hold; /* number of slab_page have */
	uint32_t used_slot_num; /* the number of all used slots */
	uint32_t obj_size;
...
struct slab_pool {
	...
	raw_atomic_ullong_t real_total_size;
};
...
static inline uintptr_t slot_idx_to_ptr(const struct slab_ator *s_ator,
...
static inline uint32_t slot_ptr_to_idx(const struct slab_ator *s_ator,
				       const struct slab_desc *s_desc,
				       const void *ptr)
{
