				     ...
				     vfs_fscache_stat_t *fscache_stat)
{
	...
	fscache_stat->lru_slab_size = (unsigned int)raw_atomic_long_read_relaxed(&stat->fscache[PAGES_FREE]) *
				      (unsigned int)raw_atomic_long_read_relaxed(&stat->fscache[PAGE_METADATA_SIZE]);
	...
	fscache_stat->pages_scanned_async = (unsigned long)raw_atomic_long_read_relaxed(&stat->fscache[PAGES_SCANNED_ASYNC]);
	fscache_stat->pages_scanned_sync = (unsigned long)raw_atomic_long_read_relaxed(&stat->fscache[PAGES_SCANNED_SYNC]);
	fscache_stat->pages_unmap_failed = (unsigned long)raw_atomic_long_read_relaxed(&stat->fscache[PAGES_UNMAP_FAILED]);
	fscache_stat->pages_drop_mmap_failed =
		(unsigned long)raw_atomic_long_read_relaxed(&stat->fscache[PAGES_DROP_MMAP_FAILED]);
	fscache_stat->pages_free_page_failed =
		(unsigned long)raw_atomic_long_read_relaxed(&stat->fscache[PAGES_FREE_PAGE_FAILED]);
	...
	fscache_stat->nr_shmem_total = (unsigned long)raw_atomic_long_read_relaxed(&stat->fscache[NR_SHMEM_TOTAL]);
	fscache_stat->nr_shmem_ashmem = (unsigned long)raw_atomic_long_read_relaxed(&stat->fscache[NR_SHMEM_ASHMEM]);
	fscache_stat->nr_shmem_tmpfs = (unsigned long)raw_atomic_long_read_relaxed(&stat->fscache[NR_SHMEM_TMPFS]);
	fscache_stat->nr_shmem_unevict = (unsigned long)raw_atomic_long_read_relaxed(&stat->fscache[NR_SHMEM_UNEVICT]);
	fscache_stat->nr_shmem_protect = (unsigned long)raw_atomic_long_read_relaxed(&stat->fscache[NR_SHMEM_PROTECT]);
	fscache_stat->nr_shmem_swapped = (unsigned long)raw_atomic_long_read_relaxed(&stat->fscache[NR_SHMEM_SWAPPED]);
	fscache_stat->nr_shmem_swapout_succ =
		(unsigned long)raw_atomic_long_read_relaxed(&stat->fscache[NR_SHMEM_SWAPOUT_SUCC]);
	fscache_stat->nr_shmem_swapout_fail =
		(unsigned long)raw_atomic_long_read_relaxed(&stat->fscache[NR_SHMEM_SWAPOUT_FAIL]);
	fscache_stat->nr_shmem_swapin_succ =
		(unsigned long)raw_atomic_long_read_relaxed(&stat->fscache[NR_SHMEM_SWAPIN_SUCC]);
	fscache_stat->nr_shmem_swapin_fail =
		(unsigned long)raw_atomic_long_read_relaxed(&stat->fscache[NR_SHMEM_SWAPIN_FAIL]);
...
uint64_t vfs_stat_pools_event(vfs_stat_common_t *stat, enum vfs_pools_event event)
{
	...
	if (likely(stat != NULL)) {
		switch (event) {
			case POOL_PAGES_TOTAL:
				val = (unsigned long)(stat->mpools.pages_total - stat->mpools.pages_reclaimed);
				break;
			case POOL_PAGES_RECLAIMED:
				val = (unsigned long)stat->mpools.pages_reclaimed;
				break;
			case POOL_PAGES_FREE:
				val = (unsigned long)(stat->mpools.pages_free - stat->mpools.pages_reclaimed);
				break;
			case POOL_PAGES_USED:
				val = (unsigned long)stat->mpools.pages_used;
				break;
			default:
				break;
		}
...
uint64_t vfs_stat_slab_event(vfs_stat_common_t *stat, enum vfs_slab_event event)
{
	...
	if (likely(stat != NULL)) {
		switch (event) {
			case BYTES_VNODE:
				val = (unsigned long)raw_atomic_long_read_relaxed(&stat->slab.bytes_vnode);
				break;
			case BYTES_DENTRY:
				val = (unsigned long)raw_atomic_long_read_relaxed(&stat->slab.bytes_dentry);
				break;
			case MMAP_CACHE_LEN:
				val = (unsigned long)stat->slab.mmap_cached_len >> PAGE_SHIFT;
				break;
			default:
				break;
		}
...
uint64_t vfs_stat_fscache_event(vfs_stat_common_t *stat, enum vfs_cache_event event)
{
	...
	if (unlikely(stat == NULL || event < 0 || event >= FSCACHE_EVENT_MAX)) {
		val = 0U;
	} else {
		val = (unsigned long)raw_atomic_long_read_relaxed(&stat->fscache[event]);
	}
...
{
	return actvxcap_fscall(vfs_binderfs_read_debugfs_info, true, true, shm_key, len, type, pid);
}
