 ...
 * Description: ARM32 native support
...
#define V_DEFINE_write_64(P, mo)                                             \
	...
	{                                                                    \
		...
		V_ASM(V_FENCE_PRE_ST(mo) "1:\n" V_LDX_64(mo, oldv, a)        \
			      V_STX_64(mo, tmp, newv, a) V_CMPI(tmp, "#0")   \
				      V_BNE(1b) V_FENCE_POST_ST(mo)          \
...
#define V_DEFINE_xchg(P, mo)                                               \
	...
	{                                                                  \
		...
		vuint32_t tmp;                                             \
		V_ASM(V_FENCE_PRE_ST(mo) "1:\n" V_LDX(P, mo, oldv, a)      \
			      V_STX(P, mo, tmp, newv, a) V_CMPI(tmp, "#0") \
				      V_BNE(1b) V_FENCE_POST_LD(mo)        \
		      : [oldv] "=&r"(old), [tmp] "=&r"(tmp)                \
		      : [newv] "r"(v), [a] "Q"(a->_v)                      \
		      : "memory", "cc");                                   \
		return old;                                                \
...
#define V_DEFINE_cmpxchg(P, mo)                                              \
	...
	{                                                                    \
		...
		vuint32_t tmp;                                               \
		V_ASM(V_FENCE_PRE_ST(mo) "1:\n" V_LDX(P, mo, oldv, a)        \
			      V_CMPEQ(P, oldv, expv) V_BNE(2f) V_STX(        \
				      P, mo, tmp, newv, a) V_CMPI(tmp, "#0") \
				      V_BNE(1b) "2:\n" V_FENCE_POST_LD(mo)   \
		      : [oldv] "=&r"(old), [tmp] "=&r"(tmp)                  \
