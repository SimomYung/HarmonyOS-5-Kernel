 ...
 * Description: ARM64 native support
...
#define V_DEFINE_xchg(P, mo)                                                  \
	static inline V_MAP_T(P) V_FUNC(P, xchg, mo)(V_MAP_A(P) *a,           \
						     V_MAP_T(P) v)            \
	{                                                                     \
		V_MAP_T(P) oldv;                                              \
		vuint32_t tmp;                                                \
		V_ASM(V_WITH_LLSC_LXE(                                        \
			      V_PRFM(a) "1:\n" V_LDX(mo, V_MAP_R(P), oldv, a) \
				      V_STX(mo, V_MAP_R(P), tmp, newv, a),    \
			      V_CBNZ("w", tmp, 1b), V_CBZ("w", tmp, 2f))      \
			      V_WITH_LSE(V_SWP(mo, V_MAP_R(P), newv, oldv,    \
					       a) "2:\n")                     \
		...
		V_ASM(V_WITH_LLSC_LXE(                                         \
			      V_PRFM(a) "1:\n" V_LDX(mo, V_MAP_R(P), oldv, a)  \
				      V_CMP(V_MAP_R(P), oldv, exp) V_BNE(2f)   \
					      V_STX(mo, V_MAP_R(P), tmp, newv, \
						    a),                        \
			      V_CBNZ("w", tmp, 1b), V_CBZ("w", tmp, 2f))       \
			      V_WITH_LSE(V_MOV(V_MAP_R(P), oldv, exp) V_CAS(   \
				      mo, V_MAP_R(P), oldv, newv, a)) "2:\n"   \
		      : [oldv] "=&r"(oldv), [tmp] "=&r"(tmp)                   \
...
#define V_DEFINE_get_max(P, mo)                                          \
	...
	{                                                                \
		V_MAP_T(P) oldv;                                         \
		V_OP_RMW(V_MAX_CMP, 2f, V_LDLSE, "umax", mo, V_MAP_R(P), \
			 V_MAP_T(P));                                    \
		return oldv;                                             \
	}
...
#define V_DEFINE_max(P, mo)                                                \
	...
	{                                                                  \
		V_MAP_T(P) oldv;                                           \
		V_OP_RMW(V_MAX_CMP, 2f, V_STLSE, "umax", mo, V_MAP_R(P),   \
			 V_MAP_T(P));                                      \
	}
...
#define V_DEFINE_get_add(P, mo)                                        \
	...
	{                                                              \
		V_MAP_T(P) oldv;                                       \
		V_OP_RMW(V_OP3, "add", V_LDLSE, "add", mo, V_MAP_R(P), \
			 V_MAP_T(P));                                  \
		return oldv;                                           \
	}
...
#define V_DEFINE_add(P, mo)                                                \
	...
	{                                                                  \
		V_MAP_T(P) oldv;                                           \
		V_OP_RMW(V_OP3, "add", V_STLSE, "add", mo, V_MAP_R(P),     \
			 V_MAP_T(P));                                      \
	}
...
#define V_DEFINE_get_or(P, mo)                                         \
	...
	{                                                              \
		V_MAP_T(P) oldv;                                       \
		V_OP_RMW(V_OP3, "orr", V_LDLSE, "set", mo, V_MAP_R(P), \
			 V_MAP_T(P));                                  \
		return oldv;                                           \
	}
...
#define V_DEFINE_or(P, mo)                                                \
	...
	{                                                                 \
		V_MAP_T(P) oldv;                                          \
		V_OP_RMW(V_OP3, "orr", V_STLSE, "set", mo, V_MAP_R(P),    \
			 V_MAP_T(P));                                     \
	}
...
#define V_DEFINE_get_xor(P, mo)                                        \
	...
	{                                                              \
		V_MAP_T(P) oldv;                                       \
		V_OP_RMW(V_OP3, "eor", V_LDLSE, "eor", mo, V_MAP_R(P), \
			 V_MAP_T(P));                                  \
		return oldv;                                           \
	}
...
#define V_DEFINE_xor(P, mo)                                                \
	...
	{                                                                  \
		V_MAP_T(P) oldv;                                           \
		V_OP_RMW(V_OP3, "eor", V_STLSE, "eor", mo, V_MAP_R(P),     \
			 V_MAP_T(P));                                      \
	}
...
#define V_DEFINE_get_and(P, mo)                                           \
	...
	{                                                                 \
		V_MAP_T(P) oldv;                                          \
		V_OP_RMW(V_OP3, "and", V_MVN_LDSE, "clr", mo, V_MAP_R(P), \
			 V_MAP_T(P));                                     \
		return oldv;                                              \
	}
...
#define V_DEFINE_await_COND_OP(P, mo, cond, op)                                  \
	...
	{                                                                        \
		...
		V_MAP_T(P) newv;                                                 \
		vuint32_t tmp;                                                   \
		V_ASM(V_PRFM(a) V_LDX(mo, V_MAP_R(P), oldv, a)                   \
			      V_CMP(V_MAP_R(P), oldv, cmpv)                      \
				      V_AWAIT_B(cond, 3f) V_CODE_ALIGN           \
		      "1:\n" V_WFE                                               \
		      "2:\n" V_LDX(mo, V_MAP_R(P), oldv, a) V_CMP(V_MAP_R(P),    \
								  oldv, cmpv)    \
			      V_AWAIT_BNOT(cond, 1b) "3:\n" V_AWAIT_OP(          \
				      op, V_MAP_R(P), newv, oldv, v)             \
				      V_STX(mo, V_MAP_R(P), tmp, newv, a)        \
					      V_CBNZ("w", tmp, 2b)               \
