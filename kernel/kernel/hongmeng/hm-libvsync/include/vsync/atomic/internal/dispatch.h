 ...
 * Description: vatomic function dispatchers
...
#define V_DISPATCH(S, F, mo, a, ...)                           \
	((sizeof((a)->_v) == sizeof(vatomic64_t)) ?            \
		 V_CONCAT(V_DISPATCH_, S)(vatomic64, F, mo, a, \
					  ##__VA_ARGS__) :     \
	 (sizeof((a)->_v) == sizeof(vatomic32_t)) ?            \
		 V_CONCAT(V_DISPATCH_, S)(vatomic32, F, mo, a, \
					  ##__VA_ARGS__) :     \
	 (sizeof((a)->_v) == sizeof(vatomic16_t)) ?            \
		 V_CONCAT(V_DISPATCH_, S)(vatomic16, F, mo, a, \
					  ##__VA_ARGS__) :     \
...
#define V_DISPATCH_ALL(P, F, mo, a, ...)                                       \
	({                                                                     \
		V_DISPATCH_N(V_MAP_A(P), V_MAP_T(P), V_FUNC(P, F, mo), V_CAST, \
			     a, ##__VA_ARGS__);                                \
	})
...
#define V_DISPATCH_INT(P, F, mo, a, ...)                               \
	({                                                             \
		V_DISPATCH_N(V_MAP_A(P), V_MAP_T(P), V_FUNC(P, F, mo), \
			     V_CAST_INT, a, ##__VA_ARGS__);            \
	})
