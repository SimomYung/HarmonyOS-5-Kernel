 ...
 */
static inline vuint64_t _vbitmap_lsb_mask(vsize_t idx)
{
	vuint64_t mask = 1;
 ...
 * @return vuint64_t mask.
 */
static inline vuint64_t _vbitmap_msb_mask(vsize_t idx)
{
	vuint64_t mask = 1;
	mask = mask << ((vuint64_t)idx);
	mask = mask + (mask - 1);
 ...
 * @return vuint64_t mask.
 */
static inline vuint64_t _vbitmap_range_mask(vsize_t from, vsize_t to)
{
 ...
 * bits.
 */
static inline vsize_t vbitmap_size(vsize_t bit_count)
{
	vsize_t arr_sz =
		VCEIL_DIV(bit_count, VUINT64_WIDTH) * sizeof(vatomic64_t);
 ...
 */
static inline void vbitmap_init(vbitmap_t *bitmap, vsize_t bit_count,
				vbool_t set)
{
 ...
 * @param bitmap address of vbitmap_t object.
 */
static inline void vbitmap_clear(vbitmap_t *bitmap)
{
	for (vsize_t i = 0; i < bitmap->maps_cnt; i++) {
 ...
 */
static inline vbool_t vbitmap_get(vbitmap_t *bitmap, vsize_t bit_idx)
{
	ASSERT(bitmap);
 ...
 * @param bit_idx index of the bit to set.
 */
static inline void vbitmap_set_bit(vbitmap_t *bitmap, vsize_t bit_idx)
{
	ASSERT(bitmap);
	ASSERT(bit_idx < bitmap->bit_cnt);
	vsize_t idx = VBITMAP_CALC_SLOT_IDX(bit_idx);
 ...
 */
static inline void _vbitbmap_set_slot(vbitmap_t *bitmap, vsize_t idx,
				      vuint64_t mask)
{
	ASSERT(idx < bitmap->maps_cnt);
 ...
 */
static inline void _vbitbmap_clr_slot(vbitmap_t *bitmap, vsize_t idx,
				      vuint64_t mask)
{
	ASSERT(idx < bitmap->maps_cnt);
 ...
 */
static inline void vbitmap_set_range(vbitmap_t *bitmap, vsize_t from,
				     vsize_t to)
{
	vsize_t lsb_slot = 0;
	vsize_t msb_slot = 0;
 ...
 */
static inline void vbitmap_clr_range(vbitmap_t *bitmap, vsize_t from,
				     vsize_t to)
{
	vsize_t lsb_slot = 0;
	vsize_t msb_slot = 0;
 ...
 */
static inline void vbitmap_clr_bit(vbitmap_t *bitmap, vsize_t bit_idx)
{
	ASSERT(bitmap);
	ASSERT(bit_idx < bitmap->bit_cnt);
	vsize_t idx = VBITMAP_CALC_SLOT_IDX(bit_idx);
 ...
 */
static inline void vbitmap_iter_init(vbitmap_t *bitmap, vbitmap_iter_t *iter)
{
	ASSERT(bitmap);
	ASSERT(iter);
 ...
 */
static inline vbool_t vbitmap_iter_next(vbitmap_iter_t *iter,
					vsize_t *out_bit_idx)
{
