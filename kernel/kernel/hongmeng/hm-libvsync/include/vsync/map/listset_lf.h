 ...
 */
static inline void vlistset_init(vlistset_t *lst,
				 vlistset_handle_node_t retire_fun,
				 void *retire_fun_arg,
				 vlistset_cmp_key_t cmp_fun)
{
	...
	vatomicptr_markable_set_rlx(&lst->head_sentinel.next,
				    &lst->tail_sentinel, false);
 ...
 * @param lst address of vlistset_t object.
 */
static inline void vlistset_destroy(vlistset_t *lst)
{
	ASSERT(lst);
 ...
 */
static inline vbool_t vlistset_add(vlistset_t *lst, vlistset_key_t key,
				   vlistset_node_t *node)
{
	...
	while (true) {
		...
		if (tail != curr && VLISTSET_CMP(lst, curr, key) == 0) {
			...
			// pred->next = node
			if (vatomicptr_markable_cmpxchg(&pred->next, curr,
							false, node, false)) {
				return true;
			} else {
 ...
 * @note must be called inside SMR critical section.
 */
static inline vbool_t vlistset_remove(vlistset_t *lst, vlistset_key_t key)
{
	vlistset_node_t *pred = NULL;
	vlistset_node_t *curr = NULL;
	...
	while (true) {
		...
		if (tail == curr || VLISTSET_CMP(lst, curr, key) != 0) {
             ...
             */
			snip = vatomicptr_markable_cmpxchg(&curr->next, succ,
							   false, succ, true);
			...
			// pred->next = succ
			detached = vatomicptr_markable_cmpxchg(
 ...
 * @note must be called inside SMR critical section.
 */
static inline vlistset_node_t *vlistset_get(vlistset_t *lst, vlistset_key_t key)
{
	vlistset_node_t *curr = NULL;
	vlistset_node_t *tail = NULL;
 ...
 */
static inline void _vlistset_visit(vlistset_t *lst,
 ...
 * changed.
 */
static inline vbool_t _vlistset_find_loop(vlistset_t *lst, vlistset_key_t key,
					  vlistset_node_t **out_pred,
					  vlistset_node_t **out_curr,
					  vbool_t *out_snipped)
{
				     ...
				     vlistset_node_t **out_curr)
{
	...
	while (_vlistset_find_loop(lst, key, out_pred, out_curr,
				   &ever_snipped)) {
	}
	return ever_snipped;
}
