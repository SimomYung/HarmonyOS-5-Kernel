 ...
 */
static inline vsize_t vsimpleht_buff_size(vsize_t capacity)
{
	ASSERT(capacity > 0);
 ...
 * hashtable stored objects.
 */
static inline void vsimpleht_init(vsimpleht_t *tbl, void *buff,
				  vsize_t capacity, vsimpleht_cmp_key_t cmp_fun,
				  vsimpleht_hash_key_t hash_fun,
 ...
 * @param tbl address of vsimpleht_t object.
 */
static inline void vsimpleht_destroy(vsimpleht_t *tbl)
{
 ...
 */
static inline void vsimpleht_thread_register(vsimpleht_t *tbl)
{
#if defined(VSIMPLEHT_DISABLE_REMOVE)
	V_UNUSED(tbl);
#else
 ...
 */
static inline void vsimpleht_thread_deregister(vsimpleht_t *tbl)
{
#if defined(VSIMPLEHT_DISABLE_REMOVE)
	V_UNUSED(tbl);
#else
 ...
 */
static inline vsimpleht_ret_t vsimpleht_add(vsimpleht_t *tbl, vuintptr_t key,
					    void *value)
{
	ASSERT(key != 0);
	ASSERT(value != NULL);
 ...
 */
static inline void *vsimpleht_get(vsimpleht_t *tbl, vuintptr_t key)
{
	vsize_t index = 0;
	...
	for (index = tbl->hash_key(key);; index++) {
		...
		probed_key =
			(vuintptr_t)vatomicptr_read(&tbl->entries[index].key);
		if (probed_key == 0) {
			return NULL;
 ...
 */
static inline void vsimpleht_iter_init(vsimpleht_t *tbl, vsimpleht_iter_t *iter)
{
	ASSERT(tbl);
	ASSERT(iter);
	iter->tbl = tbl;
 ...
 * ```
 */
static inline vbool_t vsimpleht_iter_next(vsimpleht_iter_t *iter,
					  vuintptr_t *key, void **val)
{
	vuintptr_t k = 0;
	void *v = NULL;
 ...
 * can be super-slow.
 */
static inline vsimpleht_ret_t vsimpleht_remove(vsimpleht_t *tbl, vuintptr_t key)
{
#if defined(VSIMPLEHT_DISABLE_REMOVE)
	ASSERT(0 &&
	...
	do {
		...
		if (probed_key == 0) {
			...
			probed_value =
				vatomicptr_read_acq(&tbl->entries[index].value);
			// we found the key
			...
			if (vatomicptr_cmpxchg_rel(&tbl->entries[index].value,
						   probed_value,
						   NULL) == probed_value) {
					     ...
					     void *value)
{
	...
	for (index = tbl->hash_key(key); cnt < tbl->capacity; cnt++, index++) {
        ...
        */
		val = vatomicptr_cmpxchg(&tbl->entries[index].value, NULL,
					 value);
		return (val == NULL) ? VSIMPLEHT_RET_OK :
				       VSIMPLEHT_RET_KEY_EXISTS;
	}
...
static inline void _vsimpleht_give_cleanup_a_chance(vsimpleht_t *tbl)
{
	...
	V_UNUSED(tbl);
#else
	if (rwlock_acquired_by_writer(&tbl->lock) > 0) {
