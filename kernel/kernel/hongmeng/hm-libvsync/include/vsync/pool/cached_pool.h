...
static inline void _cached_pool_entry_set_next(cached_pool_entry_t *entry,
...
static inline void *_cached_pool_entry_to_addr(cached_pool_entry_t *entry)
{
	return (cached_pool_entry_t *)(((vuintptr_t)entry) + sizeof(void *));
}

static inline cached_pool_entry_t *_cached_pool_entry_from_addr(void *p)
{
	return (cached_pool_entry_t *)(((vuintptr_t)p) - sizeof(void *));
}

/******************************* buffer ***************************************/
static void _cached_pool_buffer_init(cached_pool_buffer_t *buf)
{
...
static inline void _cached_pool_buffer_free(cached_pool_t *a,
					    cached_pool_buffer_t *buf,
...
/********************************* vunit **************************************/
static void _cached_pool_vunit_init(cached_pool_t *a, cached_pool_vunit_t *u,
				    cached_pool_buffer_t *b,
				    cached_pool_entry_t *es)
{
...
static inline void *_cached_pool_vunit_alloc(cached_pool_t *a,
					     cached_pool_vunit_t *u)
{
...
static inline void _cached_pool_vunit_free(cached_pool_t *a,
					   cached_pool_vunit_t *u, void *p)
{
...
static inline cached_pool_vunit_t *_cached_pool_vunit_find(cached_pool_t *a,
							   vuint32_t id)
{
 ...
 */
static cached_pool_t *cached_pool_init(void *buf, vuint32_t thread_num,
				       vuint32_t entry_num,
				       vuint32_t entry_size)
{
	vuint32_t cached_pool_head = sizeof(cached_pool_t);
	vuint32_t buffer_head = sizeof(cached_pool_buffer_t);
	vuint32_t buffer_size = buffer_head + sizeof(void *) * thread_num * 2;
	...
	a->entries =
		(cached_pool_entry_t *)(((vuintptr_t)a) + entry_addr_offset);
	...
	for (vuint32_t id = 0; id < thread_num; id++) {
		...
		_cached_pool_vunit_init(
			a, &a->vunits[id], a->buf,
			_cached_pool_entry_find(a->entries, a->conf.entry_space,
						start));
		for (vuint32_t idx = start; idx < middle - 1; idx++) {
			cached_pool_entry_t *curr = _cached_pool_entry_find(
				a->entries, a->conf.entry_space, idx);
 ...
 */
static inline void *cached_pool_alloc(cached_pool_t *a, vuint32_t id)
{
 ...
 *
 */
static inline void cached_pool_free(cached_pool_t *a, vuint32_t id, void *p)
{
