 ...
 * Description: Block-based Bounded Queue MPMC
 ...
 */
static inline vuint32_t bbq_mpmc_enqueue(bbq_mpmc_t *q, vuintptr_t *buf,
					 vuint32_t count, vbool_t wait)
{
 ...
 */
static inline vuint32_t bbq_mpmc_dequeue(bbq_mpmc_t *q, vuintptr_t *buf,
					 vuint32_t count, vbool_t wait)
{
	vuint32_t rest = count;
	vuintptr_t *rest_buf = buf;
 ...
 */
static inline vsize_t bbq_mpmc_memsize(vsize_t capacity)
{
 ...
 */
static inline vbool_t bbq_mpmc_init(bbq_mpmc_t *rb, vsize_t size)
{
	if (unlikely(rb == NULL) ||
	    unlikely(BBQ_ENTRY_SIZE < BBQ_MIN_ENTRY_SIZE) ||
	...
	vsize_t blk_size =
		v_pow2_round_down(blks_total_size >> BBQ_BLOCK_NUM_LOG);
	...
	for (vsize_t i = 0; i < (1UL << BBQ_BLOCK_NUM_LOG); i++) {
		_bbq_mpmc_block_init(
			(bbq_mpmc_block_t *)((rb)->blk + (i << blk_size_log)),
			i, blk_size);
	}
	return true;
}
...
/* return means retry */
static inline vbool_t _bbq_mpmc_enqueue(bbq_mpmc_t *q, vuintptr_t **buf,
					vuint32_t *count)
{
	if (*count == 0) {
		return false;
	...
	if (likely(allocated_space < block_size)) {
		...
		if (likely(old_local_space < block_size)) {
			vuint16_t space = VMIN(entry_total_size,
					       block_size - old_local_space);
			void *entry = BBQ_GET_ENTRY(blk, old_local_space);
#ifdef __KERNEL__
			int r = 0;
			if (v_memcpy_s_params_ok(entry, space, *buf, space)) {
				(void)memcpy(entry, *buf, space);
			} else {
				r = 1;
			}
#else
	...
	if (unlikely(!BBQ_BLOCK_FULLY_CONSUMED_WITH_VSN(nblk, block_size,
							global_vsn))) {
		return false;
	}
					...
					vuint32_t *count)
{
	...
	if (likely(reserved_space < block_size)) {
		...
		if (unlikely(reserved_space >= committed_space)) {
			...
			ASSERT(reserved <= committed &&
			       "reserved must be <= committed");
			return false;
		}
		vuint16_t entry_total_size =
			VMIN((*count) << BBQ_ENTRY_SIZE_LOG,
...
#ifdef __KERNEL__
		int r = 0;
		if (v_memcpy_s_params_ok(*buf, entry_total_size, entry,
					 entry_total_size)) {
			(void)memcpy(*buf, entry, entry_total_size);
		} else {
			r = 1;
		}
#else
		int r = memcpy_s(*buf, entry_total_size, entry,
				 entry_total_size);
#endif
		BUG_ON(r != 0);
	...
	vuint64_t next_consumer_vsn =
		BBQ_LOCAL_VSN(reserved) - (block_idx != 0);
