 ...
 * Description: Block-based Bounded Queue SPSC
 ...
 */
static inline vuint32_t bbq_spsc_enqueue(bbq_spsc_t *q, vuintptr_t *buf,
					 vuint32_t count, vbool_t wait)
{
     ...
     * while(bbq_spsc_enqueue_internal(q, &rest_buf, &rest) || (wait && rest));
     */
	do {
 ...
 */
static inline vuint32_t bbq_spsc_dequeue(bbq_spsc_t *q, vuintptr_t *buf,
					 vuint32_t count, vbool_t wait)
{
	vuint32_t rest = count;
	vuintptr_t *rest_buf = buf;
	...
	/* the following is equivalent to
     * while(bbq_spsc_dequeue_internal(q, &rest_buf, &rest) || (wait && rest));
     */
	do {
 ...
 */
static inline vsize_t bbq_spsc_memsize(vsize_t capacity)
{
 ...
 */
static inline vbool_t bbq_spsc_init(bbq_spsc_t *rb, vsize_t size)
{
	// we shift vuint16_t by BBQ_ENTRY_SIZE_LOG, we need to make sure the
	// behavior is defined
	ASSERT(BBQ_ENTRY_SIZE_LOG < 16U &&
	...
	if (unlikely(blk_size <=
		     BBQ_BLOCK_INIT_VALUE(struct bbq_spsc_block_s))) {
		return false;
	}
	...
	for (vsize_t i = 0; i < (1UL << BBQ_BLOCK_NUM_LOG); i++) {
		_bbq_spsc_block_init(
			(bbq_spsc_block_t *)((rb)->blk + (i << blk_size_log)),
			i, blk_size);
	}
	return true;
}
					...
					vuint32_t *count)
{
	...
	if (likely(committed_space < block_size)) {
		vuint16_t space =
			VMIN(entry_total_size, block_size - committed_space);
...
#endif
		BUG_ON(r != 0);
		vuint64_t new_committed = BBQ_LOCAL_COMPOSE(
			BBQ_LOCAL_VSN(committed), committed_space + space);
...
#ifdef __KERNEL__
		int r = 0;
		if (v_memcpy_s_params_ok(*buf, space, entry, space)) {
			(void)memcpy(*buf, entry, space);
		} else {
			r = 1;
		}
#else
