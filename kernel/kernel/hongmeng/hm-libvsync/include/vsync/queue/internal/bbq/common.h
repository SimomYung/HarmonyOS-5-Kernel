 ...
 * Description: Common macros and functions for BBQ variants
...
#define BBQ_BLK_SZ_VERIFICATION(name)                                          \
	do {                                                                   \
		...
		vuint32_t count_per_blk = (g_verify_bbq_count) >>              \
					  BBQ_BLOCK_NUM_LOG;                   \
...
static inline void bbq_reset_block_cursor_heavy(vatomic64_t *v,
						vuint64_t new_vsn,
						vuintptr_t init_v)
{
...
#define BBQ_COUNT(rb, name, name_uc)                                         \
	({                                                                   \
		...
		((((widx - ridx) *                                           \
		   (block_size -                                             \
		    BBQ_BLOCK_INIT_VALUE(struct bbq_##name##_block_s))) +    \
