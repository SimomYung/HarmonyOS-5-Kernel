 ...
 */
static inline void vqueue_ub_destroy(vqueue_ub_t *q,
				     vqueue_ub_node_handler_t retire, void *arg)
{
	...
	while (true) {
		...
		if (last == vatomicptr_stamped_get(&q->tail, &t_stamp) &&
			...
			if (next == NULL) {
				// append the new node
				// tail->next = n
				if (vatomicptr_stamped_cmpxchg(
					    &last->next, next, next_stamp,
					    qnode, (next_stamp + 1))) {
				...
				verification_ignore();
				vatomicptr_stamped_cmpxchg(&q->tail, last,
							   l_stamp, next,
							   (l_stamp + 1));
			}
		}
 ...
 */
static inline vbool_t vqueue_ub_empty(vqueue_ub_t *q)
{
	...
	while (true) {
		...
		if (first == vatomicptr_stamped_get(&q->head, &t_stamp) &&
			...
			if (first == last) {
				...
				data = next->data;
				/* LP */
				if (vatomicptr_stamped_cmpxchg(&q->head, first,
							       f_stamp, next,
							       (f_stamp + 1))) {
					if (first != &q->sentinel) {
						retire(first, arg);
 ...
 */
static inline void _vqueue_ub_visit_nodes(vqueue_ub_t *q,
					  vqueue_ub_node_handler_t visitor,
					  void *arg)
{
	vqueue_ub_node_t *next = NULL;
	vqueue_ub_node_t *curr = NULL;
