...
static inline vbool_t smr_rwlock_lib_is_set(smr_rwlock_lib_t *lock_lib)
{
	if (!lock_lib) {
		return false;
	}
...
static inline vbool_t smr_lock_lib_is_set(smr_lock_lib_t *lock_lib)
{
	if (!lock_lib) {
		return false;
	}
 ...
 */
static inline void smr_rwlock_lib_copy(smr_rwlock_lib_t *des,
				       smr_rwlock_lib_t *src)
{
	ASSERT(des);
	ASSERT(src);
