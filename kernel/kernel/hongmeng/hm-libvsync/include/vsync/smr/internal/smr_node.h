...
static inline void smr_node_buff_init(smr_node_buff_t *buff)
{
	ASSERT(buff);
	buff->idx = 0;
}

static inline vbool_t smr_node_buff_insert(smr_node_buff_t *buff,
					   smr_node_t *node,
					   smr_node_destroy_fun destroy_fun,
					   void *destroy_fun_arg)
{
	...
	if (buff->idx < V_SMR_NODE_BUFF_SIZE) {
		...
		node->core.next = buff->idx == 0 ?
					  NULL :
					  &buff->buffer[buff->idx - 1]->core;
		buff->idx++;
		return true;
	}
	return false;
...
static inline vsize_t smr_node_buff_popall(smr_node_buff_t *buff,
					   smr_node_t **head, smr_node_t **tail)
{
	vsize_t count = 0;
	ASSERT(buff);
