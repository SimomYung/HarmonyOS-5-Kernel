 ...
 */
static inline void smr_nodes_list_init(smr_nodes_list_t *lst)
{
	ASSERT(lst);
 ...
 */
static inline vbool_t smr_nodes_list_is_empty(smr_nodes_list_t *lst)
{
	ASSERT(lst);
 ...
 */
static inline vuint64_t smr_nodes_list_get_length(smr_nodes_list_t *lst)
{
 ...
 */
static inline void _smr_nodes_list_connect(smr_nodes_list_t *lst,
					   smr_node_core_t *head,
					   smr_node_core_t *tail)
{
	...
	/* KCLEANUP triggers a false positive await termination if the annotation is
     * not used. The annotation is not accurate for this loop. It is a
     * workaround! */
	await_do
	{
		cmp = old;
 ...
 */
static inline void smr_nodes_list_add(smr_nodes_list_t *lst, smr_node_t *node,
 ...
 */
static inline void smr_nodes_list_merge(smr_nodes_list_t *lst, smr_node_t *head,
					smr_node_t *tail, vuint64_t count)
{
	ASSERT(lst);
	ASSERT(head);
	ASSERT(tail);
 ...
 */
static inline smr_node_t *smr_nodes_list_get_and_empty(smr_nodes_list_t *lst)
{
 ...
 */
static inline vsize_t smr_nodes_list_destroy(smr_node_t *head)
{
	vsize_t count = 0;
	...
	while (curr) {
		...
		freed_node->destroy_fun(freed_node,
					freed_node->destroy_fun_arg);
		count++;
	}
	return count;
}
