 ...
 */
static inline vbool_t mcslock_tryacquire(mcslock_t *l, mcs_node_t *node)
{
	mcs_node_t *pred;
...
static inline void mcslock_release(mcslock_t *l, mcs_node_t *node)
{
	...
	if (vatomicptr_read_rlx(&node->next) == NULL) {
		next = (mcs_node_t *)vatomicptr_cmpxchg_rel(&l->tail, node,
							    NULL);
		if (next == node) {
			return;
		}
 ...
 */
static inline vbool_t mcslock_has_waiters(mcslock_t *l, mcs_node_t *node)
{
	V_UNUSED(l);
