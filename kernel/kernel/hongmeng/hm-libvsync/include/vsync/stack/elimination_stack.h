 ...
 * number.
 */
static inline void vstack_init(vstack_t *stack, backoff_rand_fun_t rand_fun)
{
	vsize_t i = 0;
	ASSERT(stack);
	ASSERT(rand_fun);
...
static inline void vstack_push(vstack_t *stack, vstack_node_t *node)
{
	...
	while (true) {
		...
		if (vstack_core_try_push(&stack->core, node)) {
			...
			exchanged_node =
				_vstack_rendezvous(stack, node, max, &success);
			if (success) {
						...
						vbool_t *out_success)
{
	...
	exchanged_node = exchanger_xchg(exchanger, node, VSTACK_XCHG_MAX_TRIALS,
					out_success);
