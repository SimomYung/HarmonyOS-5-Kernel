...
typedef struct vstack_node_s {
	struct vstack_node_s *next; /* next does not need to be atomic, because it
 ...
 */
static inline void vstack_core_init(vstack_core_t *stack)
{
	ASSERT(stack);
 ...
 */
static inline vbool_t vstack_core_try_push(vstack_core_t *stack,
					   vstack_node_t *node)
{
 ...
 */
static inline vstack_node_t *vstack_core_try_pop(vstack_core_t *stack,
						 vbool_t *out_success)
{
 ...
 */
static inline void vstack_core_visit(vstack_core_t *stack,
				     vstack_node_handler_t visitor, void *arg)
{
 ...
 */
static inline void vstack_core_destroy(vstack_core_t *stack,
				       vstack_node_handler_t destroy,
				       void *destroy_arg)
{
