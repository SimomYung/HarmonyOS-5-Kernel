 ...
 */
static inline void vstack_init(vstack_t *stack,
			       backoff_usleep_fun_t vstack_usleep,
			       backoff_rand_fun_t rand_fun,
			       vuint32_t min_backoff, vuint32_t max_backoff)
{
	ASSERT(stack);
	...
	xbobackoff_init(&stack->backoff, min_backoff, max_backoff,
			vstack_usleep, rand_fun);
}
/**
 ...
 */
static inline void vstack_push(vstack_t *stack, vstack_node_t *node)
{
	ASSERT(stack);
	ASSERT(node);
 ...
 */
static inline vstack_node_t *vstack_pop(vstack_t *stack)
{
 ...
 */
static inline void vstack_destroy(vstack_t *stack,
				  vstack_node_handler_t destroy, void *arg)
{
 ...
 */
static inline void _vstack_visit(vstack_t *stack, vstack_node_handler_t visitor,
				 void *arg)
{
