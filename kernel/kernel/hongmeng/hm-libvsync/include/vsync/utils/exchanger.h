...
#else
static inline void ghost_set_check_point(vsize_t ck)
{
	V_UNUSED(ck);
}
#endif
 ...
 */
static inline void exchanger_reset(exchanger_t *exchanger)
{
	vatomicptr_write(&exchanger->slot,
			 (void *)(vuintptr_t)SLOT_STATE_EMPTY);
}
/**
 ...
 */
static inline void *exchanger_xchg(exchanger_t *exchanger, void *my_item,
				   vuint32_t max_trials, vbool_t *success)
{
		...
		case SLOT_STATE_EMPTY:
			...
			if (_exchanger_slot_cmpxchg(exchanger, your_item,
				...
				while (trial < max_trials) {
					...
					your_item = _exchanger_slot_get(
						exchanger, &slot_state);
			...
			exchanger_slot_state_t new_state)
{
	...
	void *exp_val = V_ATOMICPTR_MARKABLE_COMBINE_ADDRESS_MARK(
		expected_addr, expected_state);
	void *new_val =
