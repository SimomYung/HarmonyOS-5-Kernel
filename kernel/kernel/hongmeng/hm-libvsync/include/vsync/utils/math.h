...
static inline vuint32_t v_log2(vuint32_t v)
{
	ASSERT(v != 0);
	return ((V_NUM_8 * sizeof(unsigned long long) -
		 (vuint32_t)__builtin_clzll(v)) -
		1U);
}

static inline vuint32_t v_pow2_round_up(vuint32_t v)
{
	ASSERT(v != 0);
	return v == 1U ? 1U :
			 1U << (V_NUM_32 - (vuint32_t)__builtin_clz(v - 1U));
}

static inline vuint32_t v_pow2_round_down(vuint32_t v)
{
	ASSERT(v != 0);
 ...
 * @return vsize_t N.
 */
static inline vsize_t v_least_containing_multiple(vsize_t divisor,
						  vsize_t dividend)
{
	ASSERT(dividend);
