...
int hm_net_dev_ioctl(int request, void *args)
{
	...
	ret = actvxactcapcall_netcall_socket_dev_ioctl(false, false, rref, request, args);
#else
...
int hm_net_sys_socket_attach(struct ux_socket *sock, unsigned int cnode, int fd)
{
	...
	ret = actvcapcall_netcall_socket_attach(rref, sock, cnode, fd);
...
void *hm_net_sys_socket_get(unsigned int cnode, int fd)
{
	...
	ret = actvcapcall_netcall_socket_get(rref, cnode, fd, &sock_ret);
	if (ret < 0) {
		return NULL;
	}
...
int hm_net_sys_socket_put(struct ux_socket *sock)
{
	...
	ret = actvcapcall_netcall_socket_put(rref, sock);
