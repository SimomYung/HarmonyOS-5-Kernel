...
int socket_sendmmsg(ux_socket_t *sock, const void *msgvec, size_t vlen, int flags)
{
	...
	if (!ux_is_compat_client() && (flags & MSG_CMSG_COMPAT)) {
		return E_HM_INVAL;
	}
...
static ssize_t msg_iov_totlen(const struct msghdr *msg)
{
	size_t totlen = 0;
	int i;
	size_t len;
	...
	for (i = 0; i < msg->msg_iovlen; i++) {
		...
		if (len > MAX_RW_COUNT - totlen) {
			len = MAX_RW_COUNT - totlen;
...
static int alloc_server_msg_iov(struct msghdr *srv_msg, const struct msghdr *cli_msg)
{
	 ...
	 * so an extra byte needs to be allocated.
	 */
	len = acquire_malloc_length((size_t)buflen);
			     ...
			     struct msghdr *cli_msg, void *msg, socket_recvmsg_handler handler)
{
	...
	int ret;
	ssize_t unused_buf_len;

	unused_buf_len = msg_iov_totlen(cli_msg);
	if (unused_buf_len < 0) {
		return E_HM_INVAL;
	}

	do {
		size = unused_buf_len > MAX_MSG_SIZE ? MAX_MSG_SIZE : (size_t)unused_buf_len;
