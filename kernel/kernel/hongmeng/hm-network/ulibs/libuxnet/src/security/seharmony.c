					  ...
					  ux_sock_t *other, ux_sock_t *newsk)
{
	...
	if (rc == E_HM_OK) {
		...
		rc = hmsel_sid_mls_copy(sksec_other->sid, sksec_sock->sid, &sid, false);
	}
...
static int sec_net_sel_audit(const void *c, const void *args, struct hmsel_audit_info *ai)
{
	UNUSED(c);
...
static int sel_socket_perms(const struct sec_chk_cred *cred,
			     ux_socket_t *sock)
{
...
SEC_DEFINE_COMPONENT(net, seharmony,
			 ...
			 sec_define_hook_method(socket_perms, sel_socket_perms),
