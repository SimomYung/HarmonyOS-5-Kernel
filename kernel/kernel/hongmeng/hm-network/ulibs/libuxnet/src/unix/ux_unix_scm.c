...
static bool check_expand_ssocks_cap(void)
{
	...
	if (sec_capable(&cred, POSIX_CAP_SYS_RESOURCE) ||
	    sec_capable(&cred, POSIX_CAP_SYS_ADMIN)) {
		return true;
	}
...
static int scm_check_creds(struct ucred *creds, struct sec_chk_cred *cred)
{
	struct sec_cred_basic *basic_cred = basic_cred_of_sec_cred(cred);
	struct sec_cred_dac *dac_cred = dac_cred_of_sec_cred(cred);

	if (!uid_valid(creds->uid) || !gid_valid(creds->gid)) {
		return E_HM_INVAL;
	}

	if (!(creds->pid == basic_cred->pid ||
	    sec_capable(cred, POSIX_CAP_SYS_ADMIN))) {
		return E_HM_PERM;
	}

	if (!((uid_eq(creds->uid, dac_cred->ruid) || uid_eq(creds->uid, dac_cred->euid) ||
	      uid_eq(creds->uid, dac_cred->suid)) || sec_capable(cred, POSIX_CAP_SETUID))) {
	       return E_HM_PERM;
	}

	if (!((gid_eq(creds->gid, dac_cred->rgid) || gid_eq(creds->gid, dac_cred->egid) ||
	      gid_eq(creds->gid, dac_cred->sgid)) || sec_capable(cred, POSIX_CAP_SETGID))) {
	       return E_HM_PERM;
	}
...
static int unix_scm_copy_cred(struct cmsghdr *cmsg, ux_scm_creds_t *scm_creds,
		    struct sec_chk_cred *cred)
{
	...
	err = scm_check_creds(ucred, cred);
	if (err != E_HM_OK) {
		net_warn(HMNET_UNIX, "failed to check scm creds, err=%d\n", err);
		return err;
	}

	scm_creds->passcd = 1;
	scm_creds->pid = ucred->pid;
	scm_creds->uid = ucred->uid;
	scm_creds->gid = ucred->gid;

	uev_trace_scm_send_cred(scm_creds);
...
static int unix_scm_copy_fd(uint32_t sender_cidx, ux_sock_owner_t *owner,
			     struct cmsghdr *cmsg, ux_scm_fd_t *fdp)
{
		    ...
		    struct sec_chk_cred *cred)
{
	...
	for (cmsg = CMSG_FIRSTHDR(msg); cmsg != NULL; cmsg = CMSG_NXTHDR(msg, cmsg)) {
		...
		case SCM_RIGHTS:
			if (owner == NULL) {
				owner = ux_sock_owner_get(sender_cidx, 0);
				BUG_ON(owner == NULL);
			}
			err = unix_scm_copy_fd(sender_cidx, owner, cmsg, &scm->fds);
			if (err != E_HM_OK) {
				goto error;
			}
			break;
		case SCM_CREDENTIALS:
			err = unix_scm_copy_cred(cmsg, &scm->creds, cred);
			if (err != E_HM_OK) {
				goto error;
			}
			break;
		   ...
		   struct sec_chk_cred *cred)
{
	...
	return __unix_scm_send(msg, scm, cred);
}
