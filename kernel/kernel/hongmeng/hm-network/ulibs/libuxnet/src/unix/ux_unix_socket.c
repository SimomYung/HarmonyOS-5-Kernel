...
{
	ux_unix_sock_t *usk = unix_sock_from_socket(sock);
	unsigned int tmp_backlog = (unsigned int)backlog;
	int ret = E_HM_OK;
	...
	if (ret == E_HM_OK) {
		if (tmp_backlog > usk->backlog) {
			ux_wq_wakeup(&usk->connwq);
		}
		usk->state = SS_LISTENING;
		usk->backlog = tmp_backlog;
			    ...
			    struct sockaddr *uaddr, int flags)
{
	...
	if (msg->msg_flags != 0) {
		if (ux_is_compat_client()) {
			err = osal_copy_to_user(&umsg_compat->msg_flags,
						&msg->msg_flags, sizeof(int));
		} else {
