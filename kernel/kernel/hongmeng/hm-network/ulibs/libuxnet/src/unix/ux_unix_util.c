 ...
 */
STATIC_DEFINE_RAW_RWLOCK(ux_unix_bindlock);
...
static int osal_get_cred(struct sec_chk_cred *cred)
{
	return sec_chk_src_cred_init_requester_simple(cred);
}

static void osal_get_ucred_by_cred(struct ucred *ucred, struct sec_chk_cred *cred)
{
...
int osal_get_ucred(struct ucred *ucred)
{
	...
	rc = osal_get_cred(&cred);
	if (rc != E_HM_OK) {
		net_debug(HMNET_UNIX, "%s: fail to init cred\n", __func__);
		return rc;
	}

	osal_get_ucred_by_cred(ucred, &cred);

	return E_HM_OK;
}

static int construct_scm_aggr(struct ux_scm_aggr *scm_aggr, struct msghdr *msg,
		    struct ucred *ucred, ux_scm_cookie_t *scm)
{
	struct sec_chk_cred cred = SEC_CHK_CRED_RESET;
	int ret;

	ret = osal_get_cred(&cred);
	if (ret != E_HM_OK) {
		return ret;
	}

	osal_get_ucred_by_cred(ucred, &cred);
	ret = unix_scm_send(msg, scm, &cred);
	if (ret != E_HM_OK) {
...
static unsigned int __get_disconnect_events(const ux_unix_sock_t *usk)
{
	if (usk->type == SOCK_DGRAM) {
		return 0;
	}

	if (usk->state == SS_UNCONNECTED) {
		return EPOLLHUP;
	}
	if (usk->state == SS_CONNECTED && usk->peer != NULL &&
	    usk->peer->state == SS_UNCONNECTED) {
		return EPOLLIN | EPOLLHUP;
	}

	if (usk->shutdown == RCV_SHUTDOWN) {
		return EPOLLIN;
	}
	if (usk->shutdown == (RCV_SHUTDOWN | SEND_SHUTDOWN)) {
		return EPOLLIN | EPOLLHUP;
	}
...
static unsigned int __get_sendev_dgram_nolock(ux_unix_sock_t *usk)
{
	unsigned int event = EPOLLOUT;

	if (ux_unix_send_buffer_used(usk) < usk->sk.sndbuf) {
		ux_unix_sock_t *peer = usk_peer(usk);
		if (peer != NULL && (ux_buf_queue_len(&peer->recv_queue) >= peer->backlog)) {
			event = 0;
		}
	} else {
		event = 0;
...
static unsigned int __get_sendev_stream_nolock(ux_unix_sock_t *usk)
{
	if (usk->state == SS_LISTENING) {
		return 0;
	}

	return  (ux_unix_send_buffer_used(usk) < usk->sk.sndbuf) ? EPOLLOUT : 0;
}

static int af_unix_getsockevent(ux_sock_t *sk, unsigned int *rcvev, unsigned int *sendev, unsigned int *errev)
{
	ux_unix_sock_t *usk = unix_sk(sk);
	...
	if (rcvev != NULL) {
		if (usk->type == SOCK_DGRAM) {
			*rcvev = (ux_buf_len(&usk->recv_queue) > 0) ? EPOLLIN : 0;
		} else {
			if (usk->state == SS_LISTENING) {
				*rcvev = (usk->queued > 0) ? EPOLLIN : 0;
			}
	...
	if (sendev != NULL) {
		if (usk->type == SOCK_DGRAM) {
			*sendev = __get_sendev_dgram_nolock(usk);
		} else {
			*sendev = __get_sendev_stream_nolock(usk);
		}
	}
	if (errev != NULL) {
		*errev = __get_disconnect_events(usk);
		if (usk->sk.sk_err != 0) {
			*errev |= EPOLLERR;
		}
	}
				   ...
				   struct ucred *cred, ux_namespace_t *ns)
{
	...
	usk->sk.getsockevent = af_unix_getsockevent;
	dlist_init(&usk->head);
				       ...
				       struct ux_scm_aggr *aggr)
{
	...
	ux_wq_wakeup_all_sync(&peer->recvwq);
	return ret;
}

static ssize_t __unix_sendmsg_stream(ux_unix_sock_t *usk, ux_unix_sock_t *pusk,
				     struct netd_msghdr *msg, int flags, struct ux_scm_aggr *scm_aggr)
{
	ssize_t sent = 0;
	ssize_t copied = 0;
...
static ssize_t unix_sendmsg_stream(ux_unix_sock_t *usk, struct netd_msghdr *msg, int flags)
{
	...
	ret = construct_scm_aggr(&scm_aggr, (struct msghdr *)msg, &ucred, &scm);
	if (ret != E_HM_OK) {
		return ret;
	}
	...
	if (ret == E_HM_OK) {
		if (ux_is_wr_shutdown(usk)) {
			...
			sent = __unix_sendmsg_stream(usk, peer, msg, flags, &scm_aggr);
			if (sent < 0) {
				ret = (int)sent;
			}
		}
				      ...
				      bool nonblock)
{
	...
	while ((ret == E_HM_OK) && need_check_backlog) {
		struct ux_wq_entry waiter;
				    ...
				    struct netd_msghdr *msg, int flags, struct ux_scm_aggr *scm_aggr)
{
	...
	if (usk->type != SOCK_SEQPACKET) {
		...
		if (ret == E_HM_OK) {
			ret = sec_chk_multi_user_iso(usk->cred.uid, pusk->cred.uid);
		}
...
static ssize_t unix_sendmsg_dgram(ux_unix_sock_t *usk, struct netd_msghdr *msg, int flags)
{
	...
	if (ret == E_HM_OK) {
		sent = __unix_sendmsg_dgram(usk, peer, msg, flags, &scm_aggr);
		if (sent < 0) {
			ret = (int)sent;
		}
	}
