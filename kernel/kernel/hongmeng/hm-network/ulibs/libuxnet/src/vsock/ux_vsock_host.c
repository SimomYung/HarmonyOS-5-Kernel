...
static int __do_host_vsock_unregister_vm_cid(ux_host_vsock_sock_t *vsock)
{
	uint32_t cnode_idx = osal_hm_actv_src_cnode();
	int rc;
...
static int host_vsock_close(ux_socket_t *sock)
{
	...
	(void)__do_host_vsock_unregister_vm_cid(vsock);
	ux_sock_put(&vsock->sk);
...
static int host_vsock_getsockevent(ux_sock_t *sk, unsigned int *rcvev, unsigned int *sendev, unsigned int *errev)
{
	ux_host_vsock_sock_t *vsock = host_vsock_sk(sk);
	if (vsock == NULL) {
		return E_HM_INVAL;
	}
...
static void __clear_host_vsock_rcv_queue(ux_host_vsock_sock_t *host_vsock)
{
	struct vm_msg *curr = NULL;
	struct vm_msg *next = NULL;

	dlist_for_each_entry_safe(curr, next, &host_vsock->sk.rcv_queue, struct vm_msg, node) {
...
{
	ux_host_vsock_sock_t *host_vsock = host_vsock_sk(sk);
	lock_host_vsock(host_vsock);
	__clear_host_vsock_rcv_queue(host_vsock);
	unlock_host_vsock(host_vsock);
	osal_free(host_vsock);
}
...
static ux_host_vsock_sock_t *alloc_and_init_host_vsock(ux_socket_t *socket, int domain, int type, int protocol)
{
	...
	host_vsock->sk.getsockevent = host_vsock_getsockevent;

	return host_vsock;
}

int ux_host_vsock_create(ux_socket_t *socket, int domain, int type, int protocol, unsigned int flags)
{
