...
static int vsock_getsockevent(ux_sock_t *sk, unsigned int *rcvev, unsigned int *sendev, unsigned int *errev)
{
	ux_vsock_sock_t *vsock = container_of(sk, ux_vsock_sock_t, sk);
	int ret = E_HM_OK;
...
ux_vsock_sock_t *vsock_alloc_and_init(ux_socket_t *sock, int domain, int type, int protocol)
{
	...
	if (vsock != NULL) {
		...
		vsock->max_buffer_size = VSOCK_MAX_BUFFER_SIZE;
		vsock->min_buffer_size = VSOCK_MIN_BUFFER_SIZE;

		ux_trans_alloc(vsock);
		vsock->sk.release = vsock_release;
		vsock->sk.set_write_space = vsock_set_write_space;
		vsock->sk.getsockevent = vsock_getsockevent;
	}
