...
{
	struct ux_wq_head *wq = is_send ? &vsock->sk.send_wq : &vsock->sk.recv_wq;
	int ret = E_HM_OK;
	struct ux_wq_entry waiter;

	ux_wq_prepare_wait(wq, &waiter);
	unlock_vsock(vsock);

	ret = ux_wq_do_wait(&waiter, sndtimeo);
	ux_wq_finish_wait(wq, &waiter);
...
{
	ux_wq_wakeup_all(&vsock->sk.recv_wq);
	ux_wq_wakeup_all(&vsock->sk.send_wq);
			   ...
			   unsigned int flags)
{
	...
	if (ret == E_HM_OK) {
		...
		if (child != NULL) {
			...
			ux_sock_set_socket(&child->sk, nsock);
		} else {
...
int ux_vsock_close(ux_vsock_sock_t *vsock)
{
	...
	if (vsock == NULL) {
		...
		lock_vsock(vsock);
		ux_set_sock_flags(&vsock->sk, SOCK_DEAD);
		if (vsock_state(vsock) == SS_CONNECTED ||
		    vsock_state(vsock) == SS_DISCONNECTING) {
...
static int copy_buffer_size_to_usr(uint64_t *optval, socklen_t *optlen, uint64_t value)
{
	socklen_t len;
	int ret;
	...
	if (ret == E_HM_OK) {
		if ((int)len < 0 || len < sizeof(uint64_t)) {
			ret = E_HM_INVAL;
		}
	}

	if (ret == E_HM_OK) {
		ret = hm_actv_write_caller_vm(optval, &value, sizeof(uint64_t));
	}
			 ...
			 void *optval, socklen_t *optlen)
{
	...
	case VSOCK_OPT_BUFFER_SIZE: {
		uint64_t buffer_size = ux_trans_get_buffer_size(vsock);

		ret = copy_buffer_size_to_usr(optval, optlen, buffer_size);
		break;
	}
	case VSOCK_OPT_MIN_BUFFER_SIZE: {
		uint64_t min_buffer_size = ux_vsock_get_min_buffer_size(vsock);

		ret = copy_buffer_size_to_usr(optval, optlen, min_buffer_size);
		break;
	}
	case VSOCK_OPT_MAX_BUFFER_SIZE: {
		uint64_t max_buffer_size = ux_vsock_get_max_buffer_size(vsock);

		ret = copy_buffer_size_to_usr(optval, optlen, max_buffer_size);
		break;
	}
	default:
		ret = E_HM_NOPROTOOPT;
...
static void ux_vsock_update_buffer_size(ux_vsock_sock_t *vsock, uint64_t buffer_size)
{
	uint64_t max_buffer_size = ux_vsock_get_max_buffer_size(vsock);
	uint64_t min_buffer_size = ux_vsock_get_min_buffer_size(vsock);
	uint64_t update_size = buffer_size;

	if (update_size > max_buffer_size) {
		update_size = max_buffer_size;
	}

	if (update_size < min_buffer_size) {
		update_size = min_buffer_size;
	}

	if (update_size != ux_trans_get_buffer_size(vsock)) {
		ux_trans_set_buffer_size(vsock, update_size);
		if (vsock_state(vsock) == SS_CONNECTED || vsock_state(vsock) == SS_DISCONNECTING) {
			ux_trans_update_credit(vsock);
		}
	}
}

static void ux_vsock_set_max_buffer_size(ux_vsock_sock_t *vsock, uint64_t max_buffer_size)
{
	vsock->max_buffer_size = max_buffer_size;
	ux_vsock_update_buffer_size(vsock, ux_trans_get_buffer_size(vsock));
}

static void ux_vsock_set_min_buffer_size(ux_vsock_sock_t *vsock, uint64_t min_buffer_size)
{
	vsock->min_buffer_size = min_buffer_size;
	ux_vsock_update_buffer_size(vsock, ux_trans_get_buffer_size(vsock));
}

static int set_buffer_size_wrapper(ux_vsock_sock_t *vsock, const void *optval, socklen_t optlen,
				   void (*handler)(ux_vsock_sock_t *vsock, uint64_t buffer_size))
{
	...
	if (ret == E_HM_OK) {
		ret = osal_copy_from_user(&buffer_size, optval, sizeof(uint64_t));
	}

	if (ret == E_HM_OK) {
		handler(vsock, buffer_size);
	}
			 ...
			 const void *optval, socklen_t optlen)
{
	...
	switch (optname) {
	case VSOCK_OPT_BUFFER_SIZE: {
		ret = set_buffer_size_wrapper(vsock, optval, optlen, ux_vsock_update_buffer_size);
		break;
	}
	case VSOCK_OPT_MAX_BUFFER_SIZE: {
		ret = set_buffer_size_wrapper(vsock, optval, optlen, ux_vsock_set_max_buffer_size);
		break;
	}
	case VSOCK_OPT_MIN_BUFFER_SIZE: {
		ret = set_buffer_size_wrapper(vsock, optval, optlen, ux_vsock_set_min_buffer_size);
		break;
	}
	default:
		ret = E_HM_NOPROTOOPT;
...
static int vsock_recv_data_listening(ux_vsock_sock_t *vsock, struct vm_msg *msg)
{
	...
	if (ret == E_HM_OK) {
		...
		/**
		 * wakeup waiting parent, we reuse the sk.recv_wq
