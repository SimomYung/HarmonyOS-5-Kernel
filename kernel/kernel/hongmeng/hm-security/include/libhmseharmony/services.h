...
struct hmsel_av_decision {
	__sec_data(seharmony, cntx) uint32_t allowed;
	uint32_t auditallow;
	uint32_t auditdeny;
	__sec_data(seharmony, cntx) uint32_t flags;
	uint32_t audited;
...
struct hmsel_xperms_decision {
	...
	struct hmsel_xperms_data *auditallow;
	struct hmsel_xperms_data *dontaudit;
};
...
void *hmsel_prmem_malloc_shared(size_t size);
void *hmsel_prmem_calloc_shared(size_t num, size_t size);
char *hmsel_prmem_strdup_shared(const char *s);
void hmsel_prmem_free_shared(void *p);
