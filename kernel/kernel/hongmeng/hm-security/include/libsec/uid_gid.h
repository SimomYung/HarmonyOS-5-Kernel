...
#define NORMAL_USER_ID_BASE __U(100)
#define MAX_APPID_PER_USER __U(200000)
...
/*
 * uid = MAX_APPID_PER_USER * user_id + appid
 */
static inline uint32_t convert_to_userid(uid_t uid)
{
	return (uint32_t)(uid / MAX_APPID_PER_USER);
}

static inline bool is_normal_userid(uid_t uid)
{
	return convert_to_userid(uid) >= NORMAL_USER_ID_BASE;
}

extern int sec_chk_multi_user_iso(uid_t uid, uid_t peer_uid);
