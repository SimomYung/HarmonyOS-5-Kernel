...
{
#ifdef PAC_FWCFI_SYSSERVICE
	BSL_SAL_MemCallback callback = {0};
	__asm__ __volatile__("\nmov %0, %1": "=r"(callback.pfMalloc): "r"(hitls_malloc));
	__asm__ __volatile__("\nmov %0, %1": "=r"(callback.pfFree): "r"(free));
	PAC_FECFI_SIGN_ZERO(callback.pfMalloc);
	PAC_FECFI_SIGN_ZERO(callback.pfFree);
#else
