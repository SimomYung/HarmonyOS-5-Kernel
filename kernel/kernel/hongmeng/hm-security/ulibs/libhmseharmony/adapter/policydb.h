...
int copy_mls_from_defcon(struct hmsel_context *dst_ctx, struct hmsel_context *ctx);
...
static inline int adapter_policydb_malloc(struct adapter_policydb *db)
{
#ifdef CONFIG_HKIP_SEHARMONY_PROT
#ifdef CONFIG_SEHARMONY_POLICY_SHARE
	db->p = (struct policydb *)hmsel_prmem_malloc_shared(sizeof(struct policydb));
#else
	db->p = (struct policydb *)hmsel_prmem_malloc(sizeof(struct policydb));
#endif
#else
	db->p = (struct policydb *)malloc(sizeof(struct policydb));
#endif
	if (db->p == NULL) {
		return E_HM_NOMEM;
	}
...
static inline int adapter_policydb_memset(const struct adapter_policydb *db)
{
	return memset_s(db->p, sizeof(struct policydb), 0, sizeof(struct policydb));
}

static inline void adapter_policydb_free(struct adapter_policydb *db)
{
#ifdef CONFIG_HKIP_SEHARMONY_PROT
#ifdef CONFIG_SEHARMONY_POLICY_SHARE
	hmsel_prmem_free_shared(db->p);
#else
	hmsel_prmem_free(db->p);
#endif
#else
	free(db->p);
#endif
...
#ifdef CONFIG_SEHARMONY_POLICY_SHARE
static inline void adapter_policydb_set_vaddr(struct adapter_policydb *db, void *p)
{
	db->p = p;
}
#endif

static inline void* adapter_policydb_get_vaddr(struct adapter_policydb *db)
{
	return db->p;
}
