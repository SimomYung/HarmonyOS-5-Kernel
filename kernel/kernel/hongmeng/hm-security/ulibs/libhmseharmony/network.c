...
static hmsel_class_t vsock_protocol_to_sclass(int protocol)
{
	hmsel_class_t rc = HM_SECCLASS_SOCKET;
	switch (protocol) {
		case PF_VSOCK_HOST:
			rc = HM_SECCLASS_VSOCK_HOST_SOCKET;
			break;
		case PF_VSOCK_DEFAULT:
		case PF_VSOCK:
			rc = HM_SECCLASS_VSOCK_SOCKET;
			break;
		default:
			break;
	}
...
hmsel_class_t socket_type_to_sclass(int domain, int type, int protocol)
{
	...
	case AF_XDP:
		...
		break;
	case AF_VSOCK:
		rc = vsock_protocol_to_sclass(protocol);
		break;
	default:
		break;
	}
