...
static inline int hmsel_fsuse_ocontext_sid_init(const struct hmsel_policydb *db,
						struct hmsel_sidtab *sidtab,
						int (*func)(struct hmsel_sidtab *,
							    struct hmsel_ocontext *,
							    unsigned int, hmsel_sid_t *))
{
	return adapter_fsuse_ocontext_sid_init(&db->adapter_policydb, sidtab, func);
}

static inline int hmsel_genfs_ocontext_sid_init(const struct hmsel_policydb *db,
						struct hmsel_sidtab *sidtab,
						int (*func)(struct hmsel_sidtab *,
							    struct hmsel_ocontext *,
							    unsigned int, hmsel_sid_t *))
{
	return adapter_genfs_ocontext_sid_init(&db->adapter_policydb, sidtab, func);
}
#endif
#endif
