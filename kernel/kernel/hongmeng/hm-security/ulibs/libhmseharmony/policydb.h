...
static inline int hmsel_policydb_malloc(struct hmsel_policydb *db)
{
	return adapter_policydb_malloc(&db->adapter_policydb);
}

static inline int hmsel_policydb_memset(const struct hmsel_policydb *db)
{
	return adapter_policydb_memset(&db->adapter_policydb);
}

static inline void hmsel_policydb_free(struct hmsel_policydb *db)
{
	adapter_policydb_free(&db->adapter_policydb);
}

#ifdef CONFIG_SEHARMONY_POLICY_SHARE
static inline void hmsel_policydb_set_vaddr(struct hmsel_policydb *db, void *p)
{
	adapter_policydb_set_vaddr(&db->adapter_policydb, p);
}
#endif

static inline void* hmsel_policydb_get_vaddr(struct hmsel_policydb *db)
{
	return adapter_policydb_get_vaddr(&db->adapter_policydb);
}
