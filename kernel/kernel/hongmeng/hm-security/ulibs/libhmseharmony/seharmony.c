...
int sel_load_policy(const void *data, size_t size, unsigned int service, unsigned long *policydb_vaddr)
{
	int ret;
	...
	if (ret == E_HM_OK) {
		*policydb_vaddr = ptr_to_ulong(hmsel_get_policydb());
	}
#endif
...
#if CONFIG_SEHARMONY_POLICY_SHARE
		hmsel_set_policydb_shared_vaddr(desc->policydb_vaddr);
#endif
