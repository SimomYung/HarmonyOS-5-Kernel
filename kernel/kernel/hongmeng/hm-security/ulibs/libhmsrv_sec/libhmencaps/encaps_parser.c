...
static int encap_vl_int(const char *encap_str, uint32_t vl_start_index, uint32_t vl_len, uint32_t *vl_int)
{
	...
	if (end_ptr == vl_start_ptr || end_ptr != vl_end_ptr) {
		hm_warn("Error: Invalid value, the value string cannot be safely parse as number, \
		at %d\n", vl_start_index);
		return ENCAP_FMT_ERROR;
	}
...
static inline int encaps_sort(struct sec_encaps *const encap_list, uint32_t encap_count, uint32_t *mrg_len)
{
	int ret = encaps_merge_sort(encap_list, 0, encap_count - 1, mrg_len);
	switch (ret) {
		case MALLOC_ERROR:
			return ENCAP_INTL_ERROR;
		case VALUE_ERROR:
...
static inline int process_parser_error_state(int encap_state, struct sec_encaps **encap_list, uint32_t *count)
{
...
static inline int process_get_encaps_value_error_state(int encap_state)
{
	encaps_warn(encap_state);
...
int encaps_parser(const char *encaps_str, struct sec_encaps **const encap_list, uint32_t *count)
{
	...
	if (encap_state != JSON_BEGIN) {
		return process_parser_error_state(encap_state, encap_list, count);
	}
...
int get_encaps_value_from_str(const char *encaps_str, char *key, uint32_t *value, enum encaps_type *type)
{
	...
	if (encap_state != JSON_BEGIN) {
		return process_get_encaps_value_error_state(encap_state);
	}
