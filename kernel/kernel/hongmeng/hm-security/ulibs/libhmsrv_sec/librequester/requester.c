...
static inline int __requester_assign_cred_simple(struct requester *req)
{
	...
	if (unlikely(((unsigned long)req->fwd_base_cred & 0xf) != 0U)) {
		hm_error("base_cred is invalid, fwd_idx=0x%x, req->fwd_base_cred=%px\n",
				fwd_idx, req->fwd_base_cred);
	}
#ifdef ENABLE_PAC_HARD_CODE
	req->fwd_cred_idx = fwd_idx;
#endif
...
__thread struct requester_frame *t_requester_current_frame;
