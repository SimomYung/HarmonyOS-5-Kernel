...
static void free_posix_acl_callback(struct ebr_entry *entry)
{
	struct sec_posix_acl *acl = container_of(entry, struct sec_posix_acl, ebr_entry);
...
void sec_posix_acl_free_ebr(struct sec_posix_acl *acl, struct epoch_set *epoch)
{
	...
	call_ebr(epoch, &old->ebr_entry, free_posix_acl_callback);
}
