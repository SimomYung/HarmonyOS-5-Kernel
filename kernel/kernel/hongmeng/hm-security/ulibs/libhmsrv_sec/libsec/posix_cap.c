	...
	, unsigned int opts)
{
	...
	bool needopen = sec_close_mod_pan();
#endif
	bool ret = sec_check_posix_cap(cred, eff, cap) && (g_sel_cap_check(cred, sel_cred->sid, cap, opts, true) == E_HM_OK);
#if defined(CONFIG_SEHARMONY_POLICY_SHARE) && defined(CONFIG_AARCH64_PAN)
	sec_open_mod_pan(needopen);
#endif
	return ret;
#endif
}
