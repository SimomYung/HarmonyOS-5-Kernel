...
int sec_chk_multi_user_iso(uid_t uid, uid_t peer_uid)
{
	int ret = E_HM_OK;

#ifdef CONFIG_SEC_MULTI_USER_ISO
	if (convert_to_userid(uid) != convert_to_userid(peer_uid) &&
	    is_normal_userid(uid) && is_normal_userid(peer_uid)) {
		ret = E_HM_ACCES;
	}
#else
	UNUSED(uid, peer_uid);
