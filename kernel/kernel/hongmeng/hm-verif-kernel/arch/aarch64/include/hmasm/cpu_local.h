...
struct arch_cpu_local_s {
	...
	u64 idmap;
	/*
	 * See comment in ./kernel/arch/aarch64/include/hmasm/registers.h
	 * It is init as 0.
	 */
	const struct arch_kstackhead_s *curr_kstackhead;
...
{
	unsigned long sp = ptr_to_ulong(__builtin_frame_address(0));
	return (sp >= VA_KSTACK_START) && (sp < VA_KSTACK_END);
}
