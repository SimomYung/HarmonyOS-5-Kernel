...
static inline unsigned int arch_ctrlmem_vspace_supervsidx_from_busyidx(unsigned int busyidx)
{
	return busyidx + 1u;
}

static inline unsigned int arch_ctrlmem_vspace_supervsidx_to_busyidx(unsigned int supervsidx)
{
	return supervsidx - 1u;
}
...
/* Deprecated */
static inline void arch_ctrlmem_vspace_flush_tlb_prepare(void)
{
	dsb(ishst);
}
