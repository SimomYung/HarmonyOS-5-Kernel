...
#define KDP_CLOBBER
#define KDP_CLOBBER_STRICT_CHECK "x0"
#else
#define KDP_CLOBBER "x18"
#define KDP_CLOBBER_STRICT_CHECK "x0", "x18"
#endif

/* DBGWCR0_EL1 */
/* SSC   HMC     BAS               LSC  PAC  E */
/* 01    0       11111111          10   01   1 */
#ifdef CONFIG_KDP_STRICT_CHECK
#define DEBUG_REG_WRITE_CASE_WCR_KDP_0				\
	case 0:							\
	asm volatile(						\
		"mrs	x18, mdscr_el1\n"			\
		"and	x18, x18, #1 << 13\n"			\
		...
		/* set dbgwcr */				\
		"mov	x18, #0x5ff3\n" 			\
		"movk	x18, #0x1f00, lsl #16\n"		\
		"msr	dbgwcr0_el1, x18\n"			\
		/* check write content */			\
		"mov	x0, #0x5ff3\n"				\
		"movk	x0, #0x1f00, lsl #16\n"			\
		"sub	x0, x0, x18\n"				\
		"cbz	x0, 9f\n"				\
		"cbnz	x0, 8f\n"				\
								\
...
/* DBGWCR1_EL1 */
/*    MASK    RES0-WT-LBN    SSC   HMC     BAS               LSC  PAC  E */
/*    01100   000000000      01    0       11111111          11   01   1 */
#ifdef CONFIG_KDP_DATA_INVISIBLE
# ifdef CONFIG_KDP_STRICT_CHECK
#define DEBUG_REG_WRITE_CASE_WCR_KDP_1				\
	case 1:							\
	asm volatile(						\
		"mrs	x18, mdscr_el1\n"			\
		"and	x18, x18, #1 << 13\n"			\
		...
		/* set dbgwcr */				\
		"mov	x18, #0x5ffb\n"				\
		"movk	x18, #0xc00, lsl #16\n"			\
		"msr	dbgwcr1_el1, x18\n"			\
		/* check write content */			\
		"mov	x0, #0x5ffb\n"				\
		"movk	x0, #0xc00, lsl #16\n"			\
		"sub	x0, x0, x18\n"				\
		"cbz	x0, 9f\n"				\
		"cbnz	x0, 8f\n"				\
								\
...
#else /* CONFIG_KDP_DATA_INVISIBLE */
/* NOP */
#define DEBUG_REG_WRITE_CASE_WCR_KDP_1	\
	case 1:				\
	break
#endif /* CONFIG_KDP_DATA_INVISIBLE */
...
#define DEBUG_REG_WRITE_CASE_WCR(DBGREG, idx, val)	DEBUG_REG_WRITE_CASE_WCR_KDP_##idx
