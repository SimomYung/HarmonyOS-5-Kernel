...
#if !defined(CONFIG_KDP_DATA_INVISIBLE) && !defined(CONFIG_PACXO)
	/* IB key is per cpu */
...
.macro __switch_stack_key_normal
	ldp x2, x3, [sp, (PAC_CTX_OFFSET + 16 * __ARCH_REGS_PAUTH_IB)]
#ifdef CONFIG_DICP
	add x8, sp, #(PAC_CTX_OFFSET + 16 * __ARCH_REGS_PAUTH_IB)
	dpacga x2, x8
	add x8, x8, #8
	dpacga x3, x8
#endif
...
#ifdef CONFIG_DICP
	add x8, sp, #(PAC_CTX_OFFSET + 16 * __ARCH_REGS_PAUTH_IA)
	dpacga x2, x8
	add x8, x8, #8
	dpacga x3, x8
#endif
...
#ifdef CONFIG_DICP
	add x8, sp, #(PAC_CTX_OFFSET + 16 * __ARCH_REGS_PAUTH_DA)
	dpacga x2, x8
	add x8, x8, #8
	dpacga x3, x8
#endif
...
#ifdef CONFIG_DICP
	add x8, sp, #(PAC_CTX_OFFSET + 16 * __ARCH_REGS_PAUTH_GA)
	dpacga x2, x8
	add x8, x8, #8
	dpacga x3, x8
#endif
...
#ifdef CONFIG_PACXO
/* set ia, ib, da, ga */
void pacxo_set_kernel_keys(void);
/* set db */
void pacxo_set_db_kernel_key(void);
/* set ia, ib, da, db, ga */
void pacxo_set_all_kernel_keys(void);
#endif
