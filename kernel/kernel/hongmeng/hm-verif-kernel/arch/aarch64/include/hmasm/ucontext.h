...
static inline const struct arch_uctx *
arch_uctx_of_const_regs(const struct arch_regs *regs)
{
	return container_of(regs, struct arch_uctx, regs);
}
...
static inline bool
arch_uctx_scno_is_valid(const struct arch_uctx *ctx)
{
	return (arch_uctx_scno_of(ctx) != A64_UCONTEXT_FLAG_SCNO_INVALID);
}
...
static inline bool arch_regs_is_user_mode(const struct arch_regs *regs)
{
	return __IS((REG_SPSR(regs) & PSR_M_MASK) == PSR_M_EL0T);
}
...
void arch_uctx_copy_with_orig_r0(struct arch_uctx *dst, const struct arch_uctx *src, bool orig_r0);

struct aarch32_uctx;
void arch_uctx32_copy_with_orig_r0(struct aarch32_uctx *dst, const struct arch_uctx *src, bool orig_r0);
