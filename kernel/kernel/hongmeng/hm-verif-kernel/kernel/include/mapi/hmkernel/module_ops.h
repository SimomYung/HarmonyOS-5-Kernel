...
static inline __always_inline int
module_copy_vs_from_user(__maybe_unused struct ctrlmem_kobj_vspace_s *kobjvs, void *dst,
			 size_t dst_size, const void __user *src, size_t src_size)
{
	int err;
	if ((src == NULL) || (src_size != dst_size)) {
		err = E_HM_INVAL;
	} else {
		err = (int)copy_vs_from_user_faultin(kobjvs, dst, src, src_size);
	}
	return err;
}

static inline __always_inline int
module_copy_vs_to_user(__maybe_unused struct ctrlmem_kobj_vspace_s *kobjvs, void __user *dst,
		       size_t dst_size, const void *src, size_t src_size)
{
	int err;
	if ((dst == NULL) || (src_size != dst_size)) {
		err = E_HM_INVAL;
	} else {
		err = (int)copy_vs_to_user_faultin(kobjvs, dst, src, src_size);
	}
	return err;
}
...
extern int tcb_switch_rq_hook(struct tcb_s *tcb, unsigned int rq, const struct sched_param_s *param);
...
static inline int
tcb_switch_rq_hook(struct tcb_s *tcb, unsigned int rq, const struct sched_param_s *param)
{
	UNUSED(tcb, rq, param);
	return E_HM_OK;
}
