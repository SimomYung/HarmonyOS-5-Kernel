...
#define EM_NR_CAPACITY_LEVEL	(__EM_SYSTEM_MAX_CAPACITY / 100 + 1)
...
struct em_cluster_s {
	...
	unsigned int nr_cpu;
	cpuset_t cpumask;
	struct em_cluster_energy_model_s cem;
	struct em_cluster_dynamic_energy_model_s cdem;
	struct em_cluster_custom_energy_model_s *ccem;
};

struct em_cpu_s {
	unsigned int cpu_id;
	struct em_cluster_s *cluster;
};
...
unsigned int em_cpu_frequency_of_capacity(unsigned int cpu, unsigned int cap);
...
unsigned int em_cluster_min_freq_of(unsigned int cluster);
...
unsigned int em_cluster_cap_of(unsigned int cluster, unsigned int freq);

unsigned int em_cpu_avail_capacity_of(unsigned int cpu, unsigned int cap);
...
static inline unsigned int em_first_cpu_of(unsigned int cluster)
{
	UNUSED(cluster);
	return 0U;
}
...
static inline unsigned int em_cluster_min_freq_of(unsigned int cluster)
{
	UNUSED(cluster);
	return 0U;
}
...
static inline unsigned int em_cluster_cap_of(unsigned int cluster, unsigned int freq)
{
	UNUSED(cluster, freq);
	return 0U;
}

static inline unsigned int em_cpu_avail_capacity_of(unsigned int cpu, unsigned int cap)
{
	UNUSED(cpu, cap);
	return 0U;
}
