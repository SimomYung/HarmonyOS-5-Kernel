 ...
 */
#define SCHED_FLAGS_BALANCE			BIT_RANGE(8, 14)
#define SCHED_FLAG_BALAN_PIN			BIT(8)
#define SCHED_FLAG_BALAN_CAPACITY_LE		BIT(9)
#define SCHED_FLAG_BALAN_CAPACITY_GE		BIT(10)
#define SCHED_FLAG_BALAN_SKIP			BIT(11)

/*
 * Mark tcb is upgrade to higher capacity level.
 * This flag make sense only when tcb is on queue.
 * Set while enqueue tcb and it's choosen to higher cap level then fit level,
 * clr with other flags of SCHED_FLAGS_BALANCE.
 */
#define SCHED_FLAG_UPGRADE			BIT(14)

/*
 * Mark tcb is degrade to lower capacity level.
 * This flag make sense when tcb is on queue or running.
 * Set while enqueue tcb and it's choosen to lower cap level then fit level,
 * clr before next time enqueue this tcb.
 */
#define SCHED_FLAG_DEGRADE			BIT(15)
#define SCHED_FLAGS_MIGRATE			(SCHED_FLAG_UPGRADE | SCHED_FLAG_DEGRADE)
...
/* tcb is going to be killed */
#define SCHED_FLAG_KILLED			BIT(29)
