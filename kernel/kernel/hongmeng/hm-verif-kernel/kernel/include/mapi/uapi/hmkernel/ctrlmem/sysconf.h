...
struct __ctrlmem_sysconf_spinhdlr_dfrwfe_s {
	void *fallback_sp;
	void *fallback_entry;
};
...
struct __ctrlmem_sysconf_spincalls_s {
	...
	__u64 spinhdlr_ptlwfe_lsyscall_close;
	__u64 spinhdlr_ptlwfe_lsyscall_dup;
...
struct __ctrlmem_sysconf_hkip_s {
	...
	__u32 ctrlmem_hkip_rsvmem_range_update;
};
...
enum __ctrlmem_sysconf_reserv_mem_type {
	...
	__CTRLMEM_SYSCONF_RESERV_MEM_EKBOX,
...
struct __ctrlmem_sysconf_track_info_s {
	__bool pause;
};
...
struct __ctrlmem_sysconf_exception_info_s {
	__u32 cpu;
	__u32 reason;
	__u32 padding;
	__u32 exception_cnode_idx;
	__u64 exception_pc;
};

#define __CTRLMEM_SYSCONF_CPUINFO_MAX_CPU_CNT		32
#define __CTRLMEM_SYSCONF_CPUINFO_MAX_REG_CNT		11
...
#define __CTRLMEM_FIXUP_PC_RET_PF   (__U(0))
#define __CTRLMEM_FIXUP_PC_RET_NOPF (__U(1))

enum __ctrlmem_exp_table_write_type {
	__EXP_TABLE_INIT,
	__EXP_TABLE_SFCP_NOPF,
	__EXP_TABLE_SFCP_PF,
};

#define __CTRLMEM_SYSCONF_EXP_SFCP 0
#define __CTRLMEM_SYSCONF_EXP_SFCP_NOPF 1
#define __CTRLMEM_SYSCONF_EXP_NR 2

struct __ctrlmem_sysconf_exp_entry_s {
	unsigned long start;
	unsigned long end;
	unsigned long fixup;
};

struct __ctrlmem_sysconf_exp_table_s {
	struct __ctrlmem_sysconf_exp_entry_s entries[__CTRLMEM_SYSCONF_EXP_NR];
};
...
struct __ctrlmem_sysconf_page_s {
	...
	struct __ctrlmem_sysconf_exp_table_s    exp_table;
	struct __ctrlmem_sysconf_exception_info_s	exception_info;
	...
	/* u8 */
	struct __ctrlmem_sysconf_track_info_s	track_info;
	...
	__u8 padding[__CTRLMEM_SYSCONF_MAX_SIZE - sizeof(struct __ctrlmem_sysconf_version_s)
			     ...
			     - sizeof(struct __ctrlmem_sysconf_exp_table_s)
			     - sizeof(struct __ctrlmem_sysconf_exception_info_s)
			     ...
			     - sizeof(struct __ctrlmem_sysconf_track_info_s)
