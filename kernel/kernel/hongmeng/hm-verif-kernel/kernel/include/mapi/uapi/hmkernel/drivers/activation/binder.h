...
/* The info about the data rply from server to client */
struct __actv_binder_rply_info {
	__u32 bc_cmd;
	/* The cref of the client thread */
	cref_t client_cref;
};
...
union __actv_binder_write_read {
	...
	struct __actv_binder_rply_info rply_info;
};
...
struct __actv_binder_wake_args {
	cref_t tcb_cref;
};
...
/* Wake up the thread joined in an actvpool */
#define __ACTV_BINDER_THREAD_WAKEUP	__ACTV_BINDER_IOW('b', 102, struct __actv_binder_wake_args)
