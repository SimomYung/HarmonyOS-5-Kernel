...
/* Futex flags */
#define FUTEX_TYPE_PRIVATE			(0x0UL)
#define FUTEX_TYPE_SHARED			(0x1UL)
#define FUTEX_TYPE_MASK				(0xfUL)

#ifdef CONFIG_MODULE_TIMER_RBTREE
#define FUTEX_REALTIME_CLOCK			(0x80UL)
#define FUTEX_BOOTTIME_CLOCK			(0x100UL)
#define FUTEX_ALARM_CLOCK			(0x200UL)
#define FUTEX_CLOCK_MASK			(FUTEX_REALTIME_CLOCK | FUTEX_BOOTTIME_CLOCK | FUTEX_ALARM_CLOCK)
#endif

#define FUTEX_FLAG_WAIT_ENQUEUE			(0x400UL)

/*
 * If __FUTEX_BLOCKED_HAS_LOCK_OWNER is set,
 * snapshot will print lock owner,
 * when the thread is blocked for lock
 */
#define __FUTEX_BLOCKED_HAS_LOCK_OWNER		(0x800UL)

/*
 * If FUTEX_BLOCKED_TIMEDOUT_DETECT is set, hguard will check if
 * the waiter is waiting for a long time.
 */
#define FUTEX_BLOCKED_TIMEDOUT_DETECT		(0x1000UL)
#define __FUTEX_FLAG_USER_NOT			(0x2000UL)

/*
 * __FUTEX_FLAG_BITLOCK modifies how the uaddr value check is implemented.
 * Instead of (*uaddr == val) it's ((*uaddr & val) == val)
 */
#define __FUTEX_FLAG_BITLOCK			(0x4000UL)

/* Indicates the futex is lock_wait in ldk */
#define __FUTEX_FLAG_LOCK_IN_LDK		(0x8000UL)

#define __FUTEX_FLAG_IOWAIT			(0x10000UL)
/* Indicates the futex is xdep lock */
#define __FUTEX_BLOCKED_LOCK_XDEP		(0x20000UL)
#define __FUTEX_FLAG_PAGELOCK			(0x40000UL)
