...
#define __SCHED_XDEP_INFER_CTRL 109UL
/* Preempt operation */
...
#define CPU_MAX_PERIOD_MAX         1000000
#define CPU_MAX_PERIOD_MIN         1000
#define CPU_QUOTA_PERIOD_BASE      100000
...
#define SCHED_LT_FLAG_CGROUP_ID		(1U << 20)
...
enum sched_info_type {
	...
	SCHED_INFO_EXPEL,
	SCHED_INFO_CGROUP_ID,
	NR_SCHED_INFO
};

#define SCHED_LT_CGROUP_ID_BIT_FIELD_WIDTH	5
#define SCHED_LT_CGROUP_ID_MAX	(1U << SCHED_LT_CGROUP_ID_BIT_FIELD_WIDTH)
...
typedef struct {
	...
	__u32 restricted : 1;
	__u32 expel : 3;
	__u32 lt_cgroup_id : SCHED_LT_CGROUP_ID_BIT_FIELD_WIDTH;
} sched_info_struct;
...
#define XDEP_MAX_WAKE_LEVEL 3U
#define XDEP_MAX_REPORT_NUM 20U

enum sched_xdep_infer_cmd {
	XDEP_MARK = 0,
	XDEP_STATE,
	XDEP_FREE,
	XDEP_DECAY,
...
struct sched_xdep_thread_info_s {
	char name[16];
	__s32 pid;
	__s32 tid;
	__u32 wake_point;
...
struct xdep_infer_info_s {
	__u8 wake_num;
	struct sched_xdep_thread_info_s thread_info[XDEP_MAX_REPORT_NUM];
} __attribute__((aligned(8)));

struct sched_xdep_infer_ctrl_s {
	cref_t cref;
	__u32 cmd;
	__u32 value;
	__u32 wake_point_filter;
	_Bool multi_proc;
} __attribute__((aligned(8)));

#endif /* UAPI_MODULE_SCHED_MODULE_H */
