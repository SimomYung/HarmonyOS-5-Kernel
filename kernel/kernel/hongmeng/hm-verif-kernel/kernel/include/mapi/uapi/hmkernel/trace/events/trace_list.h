...
TRACE_TCB_EVENT(sched, hmlt_update_tcb_load,
	...
	TP_ASSIGN(
		...
		mem_cpy_p_to_p(__entry->hists, sizeof(__entry->hists), hists, sizeof(__entry->hists));
		mem_cpy_p_to_p(__entry->top_load_hists, sizeof(__entry->top_load_hists),
				top_load_hists, sizeof(__entry->top_load_hists));
	),
...
TRACE_TCB_EVENT(sched, hmlt_update_tcb_load_in_cpu,
	...
	TP_ASSIGN(
		...
		mem_cpy_p_to_p(__entry->curr_in_cpu, sizeof(__entry->curr_in_cpu), curr_in_cpu, sizeof(__entry->curr_in_cpu));
		mem_cpy_p_to_p(__entry->prev_in_cpu, sizeof(__entry->prev_in_cpu), prev_in_cpu, sizeof(__entry->prev_in_cpu));
	),
...
TRACE_TCB_EVENT(sched, hmlt_update_delta_load_in_cpu,
	TP_PROTO(const struct tcb_s *, tcb,
		unsigned int, cpu,
		unsigned int, nr_window,
		unsigned int, capacity,
		unsigned int, max_util,
		unsigned int, delta_load),
	TP_ARGS(tcb, cpu, nr_window, capacity, max_util, delta_load),
	TP_FIELD(
		__array(char, name, 16)
		__field(__s32, tid)
		__field(__u32, cpu)
		__field(__u32, nr_window)
		__field(__u32, curr_cap)
		__field(__u32, max_util)
		__field(__u32, delta_load)
	),
	TP_ASSIGN(
		str_cpy_p_to_a(__entry->name, tcb_name_of(tcb));
		__entry->tid = (__s32)(*tcb_miscdata_of((struct tcb_s *)tcb, MISCDATA_TCB_TID));
		__entry->cpu = cpu;
		__entry->nr_window = nr_window;
		__entry->curr_cap = capacity;
		__entry->max_util = max_util;
		__entry->delta_load = delta_load;),
	TP_PRINT("tid=%d comm=%s cpu=%u nr_window=%u curr_capacity=%u  max_util=%u delta_load=%u\n",
		 __entry->tid, __entry->name, __entry->cpu, __entry->nr_window, __entry->curr_cap,
		 __entry->max_util, __entry->delta_load
	)
)
...
TRACE_TCB_EVENT(sched, sched_boost_task,
	...
	TP_PRINT("comm=%s tid=%d load=%u boost=%d min_load=%u max_load=%u\n",
...
TRACE_TCB_EVENT(sched, hmlt_update_pcpu_load,
	...
	TP_ASSIGN(
		...
		__entry->delta_load = delta_load;
		__entry->nr_window = nr_window;
		__entry->contribute_load = contribute_load;),
	TP_PRINT("cpu=%u core=%u curr_capacity=%u nr_running_cpu=%u curr_time_us=%"__PRIu64" curr_load=%u prev_load=%u "
		 "delta_load=%u nr_window=%u contribute_load=%u\n",
		 __entry->cpu, __entry->core, __entry->curr_cap, __entry->nr_running_cpu,
		 __entry->curr_time_us, __entry->curr_load, __entry->prev_load, __entry->delta_load,
		 __entry->nr_window, __entry->contribute_load))
...
TRACE_EVENT(sched, cpu_irq_load,
	TP_PROTO(__u32, cpu, __u64, load),
	TP_ARGS(cpu, load),
	TP_FIELD(
		__field(__u32, cpu)
		__field(__u64, load)
	...
	TP_PRINT("cpu_irq_load_%d state=%llu cpu_id=0",
...
TRACE_TCB_EVENT(sched, sched_choose_cpu,
	TP_PROTO(struct tcb_s *, tcb,
		 ...
		 unsigned int, load,
		 int, boost,
		 unsigned int, boost_flag,
		 unsigned int, rt_non_task_cpuset,
		 unsigned int, non_task_cpuset),
	TP_ARGS(tcb, prefer_idle, load, boost, boost_flag, rt_non_task_cpuset,
		non_task_cpuset),
	TP_FIELD(
		__field(__s32, tid)
		__field(__s32, prio)
		...
		__field(__u32, rt_non_task_cpuset)
		__field(__u32, non_task_cpuset)
		__array(char, name, 16)
	),
	TP_ASSIGN(str_cpy_p_to_a(__entry->name, tcb_name_of(tcb));
		...
		__entry->rt_non_task_cpuset = rt_non_task_cpuset;
		__entry->non_task_cpuset = non_task_cpuset;
	),
	TP_PRINT("tcb=%s load=%lu boost=%d cref=%llx tid=%d prio=%d sched_flags=0x%x "
		"prefer_idle=%u boost_flag=%u select cpu:%u rt_non_task:0x%x non_task:0x%x",
		...
		__entry->rt_non_task_cpuset, __entry->non_task_cpuset
	)
)
...
TRACE_EVENT(sched, xdep_wake_info,
	TP_PROTO(unsigned int, waker_index,
		 unsigned int, wakee_index,
		 unsigned int, failed_reason),
	TP_ARGS(waker_index, wakee_index, failed_reason),
	TP_FIELD(
		__field(__u32, waker_index)
		__field(__u32, wakee_index)
		__field(__u32, failed_reason)
	),
	TP_ASSIGN(
		__entry->waker_index = waker_index;
		__entry->wakee_index = wakee_index;
		__entry->failed_reason = failed_reason;
	),
	TP_PRINT("waker[%u], wakee[%u], failed_reason = %u",
		__entry->waker_index, __entry->wakee_index,
		__entry->failed_reason
	)
)

TRACE_EVENT(sched, xdep_report_info,
	TP_PROTO(int, wake_free_num,
		 int, index_num,
		 int, key_num),
	TP_ARGS(wake_free_num, index_num, key_num),
	TP_FIELD(
		__field(__s32, wake_free_num)
		__field(__s32, index_num)
		__field(__s32, key_num)
	),
	TP_ASSIGN(
		__entry->wake_free_num = wake_free_num;
		__entry->index_num = index_num;
		__entry->key_num = key_num;
	),
	TP_PRINT("wake_free_num[%d], index_num[%d], key_num[%d]",
		__entry->wake_free_num, __entry->index_num,
		__entry->key_num
	)
)
...
TRACE_EVENT(sched, sched_check_cpu,
	TP_PROTO(const struct tcb_s *, tcb,
		...
		bool, high_irq_load,
		bool, allowed),
	TP_ARGS(tcb, curr_cpu, tcb_time, curr_time, high_irq_load, allowed),
	TP_FIELD(
	...
	TP_ASSIGN(
		...
		__entry->high_irq_load = (u32)high_irq_load;
	...
	TP_PRINT("tcb=%s tid=%d sched_flags=0x%x tcb_cpu:%u level:%u sc_time:%llu "
		 ...
		 "high_irq_load:%u check_allowed:%u sched_delay:%lluus\n",
		 ...
		 __entry->cpu_epoch, __entry->tcb_epoch, __entry->high_irq_load, __entry->allowed,
		 __entry->cur_time - __entry->tcb_time
	)
)
...
TRACE_EVENT(sched, sched_lb_mark_upgrade,
	TP_PROTO(struct tcb_s *, tcb,
		 unsigned int, fit_lv),
	TP_ARGS(tcb, fit_lv),
	TP_FIELD(
		__array(char, name, 16)
		__field(__s32, tid)
		__field(__u32, fit_lv)
	),
	TP_ASSIGN(
		str_cpy_p_to_a(__entry->name, tcb_name_of(tcb));
		__entry->tid = *(ptr_cast(tcb_miscdata_of(tcb, MISCDATA_TCB_TID), __s32));
		__entry->fit_lv = fit_lv;
	),
	TP_PRINT("tcb=%s tid=%d fit_lv=%u",
		 __entry->name, __entry->tid, __entry->fit_lv
	)
)

TRACE_EVENT(sched, sched_lb_mark_degrade,
	TP_PROTO(struct tcb_s *, tcb,
		 unsigned int, fit_lv,
		 unsigned int, max_lv),
	TP_ARGS(tcb, fit_lv, max_lv),
	TP_FIELD(
		__array(char, name, 16)
		__field(__s32, tid)
		__field(__u32, fit_lv)
		__field(__u32, max_lv)
	),
	TP_ASSIGN(
		str_cpy_p_to_a(__entry->name, tcb_name_of(tcb));
		__entry->tid = *(ptr_cast(tcb_miscdata_of(tcb, MISCDATA_TCB_TID), __s32));
		__entry->fit_lv = fit_lv;
		__entry->max_lv = max_lv;
	),
	TP_PRINT("tcb=%s tid=%d fit_lv=%u max_lv=%u",
		 __entry->name, __entry->tid, __entry->fit_lv, __entry->max_lv
	)
)

TRACE_EVENT(sched, sched_lb_migrate_notify,
	TP_PROTO(unsigned int, cpu,
		 unsigned int, mask),
	TP_ARGS(cpu, mask),
	TP_FIELD(
	...
	TP_PRINT("cpu %u notify mask 0x%x",
		 __entry->cpu, __entry->mask
	)
)
...
TRACE_TCB_EVENT(sched, xdep_sync,
	TP_PROTO(struct tcb_s *, waiter,
		struct tcb_s *, holder,
		unsigned int, holder_prev_prio,
		unsigned int, holder_curr_prio,
		int, sync_type,
		int, boost_type),
	...
	TP_FIELD(
		...
		__field(__u32, _waiter_priority)
		__field(__u32, _holder_prev_prio)
		__field(__u32, _holder_curr_prio)
	...
	TP_PRINT("waiter:%s(%d), holder:%s(%d), waiter_prio:%u, holder_prev_prio:%u, " \
		"holder_curr_prio:%u, sync_type:%d, boost_type:%d\n",
...
TRACE_TCB_EVENT(power_kernel, idle_irq_info,
	TP_PROTO(unsigned int, irq, int, need_update, unsigned int, wakeup_flag),
	TP_ARGS(irq, need_update, wakeup_flag),
	TP_FIELD(
		__field(unsigned int, irq)
...
TRACE_TCB_EVENT(power_kernel, sched_ind_get_util,
	...
	TP_ARGS(cpu, chg_event, util, cluster_load, top_load, ddl_load,
		max_util, min_util, iowait, cluster_min_load, max_tcb_load, raw_load),
	TP_FIELD(
		__field(__u32, cpu)
	...
	TP_PRINT("cpu=%u util=%u cluster_load=%u "
		 ...
		 "max_util=%u min_util=%u event=%u iowait=%u "
		 "cluster_min_load=%u max_load=%u raw_load=%u",
		 __entry->cpu, __entry->util, __entry->cluster_load,
		 __entry->top_load, __entry->ddl_load, __entry->max_util,
...
TRACE_TCB_EVENT(power_kernel, mipslt_cpu_pcount_hist,
	...
	TP_ASSIGN(
		...
		mem_cpy_p_to_p(__entry->pcounts, sizeof(__entry->pcounts), pcounts, sizeof(__entry->pcounts));
	),
...
do { \
	...
	int offset = 0; \
	for (i = 0; i < (num); i++) { \
		ret = snprintf_s((str) + offset, (len) - offset, (len) - offset - 1, "%llu,", (array)[i]); \
		if (ret < 0) { \
			break; \
		} \
		offset += ret; \
...
TRACE_TCB_EVENT(power_kernel, heca_print_freq_stat,
	TP_PROTO(unsigned int, cpu,
		 unsigned int, class,
		 unsigned int, cal_cap,
		 unsigned int, real_cap,
		 unsigned int, avg_cap,
		 unsigned int, cpu_freq,
		 unsigned int, hc_freq,
	...
	TP_ARGS(cpu, class, cal_cap, real_cap, avg_cap, cpu_freq, hc_freq,
		ddr_freq, irq_in, irq_out, min_freq, max_freq),
	TP_FIELD(
		__field(__u32, cpu)
		__field(__u32, class)
		__field(__u32, cal_cap)
		__field(__u32, real_cap)
		__field(__u32, avg_cap)
		__field(__u32, cpu_freq)
		__field(__u32, hc_freq)
		__field(__u32, ddr_freq)
		__field(__u32, irq_in)
		__field(__u32, irq_out)
		__field(__u32, min_freq)
		__field(__u32, max_freq)
	),
	TP_ASSIGN(
		__entry->cpu = cpu;
		__entry->class = class;
		__entry->cal_cap = cal_cap;
		__entry->real_cap = real_cap;
		__entry->avg_cap = avg_cap;
		__entry->cpu_freq = cpu_freq;
		__entry->hc_freq = hc_freq;
		__entry->ddr_freq = ddr_freq;
		__entry->irq_in = irq_in;
		__entry->irq_out = irq_out;
		__entry->min_freq = min_freq;
		__entry->max_freq = max_freq;
	),
 
	TP_PRINT("heca_trace cpu %u class %u cal cap %u real cap %u avg cap %u freq %u %u %u irqnum %u %u minfreq %u maxfreq %u",
		 __entry->cpu, __entry->class, __entry->cal_cap,
		 __entry->real_cap, __entry->avg_cap, __entry->cpu_freq, __entry->hc_freq,
		 __entry->ddr_freq, __entry->irq_in, __entry->irq_out, __entry->min_freq,
		 __entry->max_freq)
)
 
TRACE_TCB_EVENT(power_kernel, cpu_frequency,
	TP_PROTO(unsigned int, state, unsigned int, cpu_id),
	TP_ARGS(state, cpu_id),
	TP_FIELD(
		__field(unsigned int, state)
...
TRACE_EVENT(power_kernel, cpu_frequency_limits,
...
TRACE_TCB_EVENT(power_kernel, update_cpu_usage,
	TP_PROTO(const struct tcb_s *, tcb,
		unsigned int, cpu,
		unsigned long long, last_window,
		unsigned long long, runtime_us,
		unsigned int *, hists,
		unsigned int, curr_hist_idx),
	TP_ARGS(tcb, cpu, last_window, runtime_us, hists, curr_hist_idx),
	TP_FIELD(
		__array(char, name, 16)
		__field(__s32, tid)
		__field(__u32, cpu)
		__field(__u64, last_window)
		__field(__u64, runtime_us)
		__array(__u32, hists, 6)
		__field(__u32, curr_hist_idx)
	),
	TP_ASSIGN(
		str_cpy_p_to_a(__entry->name, tcb_name_of(tcb));
		__entry->tid = *(ptr_cast(tcb_miscdata_of((struct tcb_s *)tcb, MISCDATA_TCB_TID), __s32));
		__entry->cpu = cpu;
		__entry->last_window = last_window;
		__entry->runtime_us = runtime_us;
		mem_cpy_p_to_p(__entry->hists, sizeof(__entry->hists), hists, sizeof(__entry->hists));
		__entry->curr_hist_idx = curr_hist_idx;
	),
	TP_PRINT("cpu=%u tid=%d comm=%s last_window=%"__PRIu64" runtime_us=%"__PRIu64" "
		 "hist0=%u hist1=%u hist2=%u hist3=%u hist4=%u hist5=%u idx=%u",
		 __entry->cpu, __entry->tid, __entry->name, __entry->last_window, __entry->runtime_us, __entry->hists[0],
		 __entry->hists[1], __entry->hists[2], __entry->hists[3], __entry->hists[4], __entry->hists[5],
		 __entry->curr_hist_idx
	)
)
 
TRACE_TCB_EVENT(power_kernel, count_target_usage,
	TP_PROTO(unsigned int, cpu,
		int, speed,
		int, speed_err_sum,
		int, last_speed,
	...
	TP_ARGS(cpu, speed, speed_err_sum, last_speed, p_speed, i_speed, d_speed, speed_target),
	TP_FIELD(
		__field(unsigned int, cpu)
		__field(int, speed)
		__field(int, speed_err_sum)
		__field(int, speed_err)
		__field(int, p_speed)
		__field(int, i_speed)
		__field(int, d_speed)
	...
	TP_ASSIGN(
		__entry->cpu = cpu;
		__entry->speed = speed;
		__entry->speed_err_sum = speed_err_sum;
		__entry->speed_err = speed - last_speed;
		__entry->p_speed = p_speed;
		__entry->i_speed = i_speed;
		__entry->d_speed = d_speed;
		__entry->speed_target = speed_target;
	),
	TP_PRINT("cpu=%u speed=%d speed_err_sum=%d speed_err=%d "
		"p_speed=%d i_speed=%d d_speed=%d speed_target=%d",
		__entry->cpu, __entry->speed, __entry->speed_err_sum, __entry->speed_err,
		__entry->p_speed, __entry->i_speed, __entry->d_speed, __entry->speed_target
	)
)
 
TRACE_TCB_EVENT(power_kernel, calculate_util_err,
	TP_PROTO(unsigned int, cpu,
		unsigned int, curr_util,
		unsigned int, origin_usage,
		unsigned int, curr_usage,
		unsigned int, target_load_out,
		unsigned int, target_util
	),
	TP_ARGS(cpu, curr_util, origin_usage, curr_usage, target_load_out, target_util),
	TP_FIELD(
		__field(unsigned int, cpu)
		__field(unsigned int, curr_util)
		__field(unsigned int, origin_usage)
		__field(unsigned int, curr_usage)
		__field(unsigned int, target_load_out)
		__field(unsigned int, target_util)
	),
	TP_ASSIGN(
		__entry->cpu = cpu;
		__entry->curr_util = curr_util;
		__entry->origin_usage = origin_usage;
		__entry->curr_usage = curr_usage;
		__entry->target_load_out = target_load_out;
		__entry->target_util = target_util;
	),
	TP_PRINT("cpu=%u curr_util=%u origin_usage=%u curr_usage=%u target_load_out=%u target_util=%u",
		__entry->cpu, __entry->curr_util, __entry->origin_usage, __entry->curr_usage,
		__entry->target_load_out, __entry->target_util
	)
)
 
TRACE_TCB_EVENT(power_kernel, util_pid_control,
	TP_PROTO(unsigned int, cpu,
		int, util_err,
		int, util_err_sum,
		int, last_util_err,
	...
	TP_ARGS(cpu, util_err, util_err_sum, last_util_err, p_util, i_util, d_util),
	TP_FIELD(
		__field(unsigned int, cpu)
		__field(int, util_err)
		__field(int, util_err_sum)
		__field(int, util_err_diff)
		__field(int, p_util)
		__field(int, i_util)
		__field(int, d_util)
	),
	TP_ASSIGN(
		__entry->cpu = cpu;
		__entry->util_err = util_err;
		__entry->util_err_sum = util_err_sum;
		__entry->util_err_diff = util_err - last_util_err;
		__entry->p_util = p_util;
		__entry->i_util = i_util;
		__entry->d_util = d_util;
	),
	TP_PRINT("cpu=%u util_err=%d util_err_sum=%d util_err_diff=%d p_util=%d i_util=%d d_util=%d",
		__entry->cpu, __entry->util_err, __entry->util_err_sum, __entry->util_err_diff,
		__entry->p_util, __entry->i_util, __entry->d_util
	)
)
 
TRACE_TCB_EVENT(power_kernel, calculate_pid_util,
	TP_PROTO(unsigned int, cpu,
		int, last_max_pid_util,
		int, min_capacity,
		int, max_capacity,
		int, util_target
	),
	TP_ARGS(cpu, last_max_pid_util, min_capacity, max_capacity, util_target),
	TP_FIELD(
		__field(unsigned int, cpu)
		__field(int, last_max_pid_util)
		__field(int, min_capacity)
		__field(int, max_capacity)
		__field(int, util_target)
	),
	TP_ASSIGN(
		__entry->cpu = cpu;
		__entry->last_max_pid_util = last_max_pid_util;
		__entry->min_capacity = min_capacity;
		__entry->max_capacity = max_capacity;
		__entry->util_target = util_target;
	),
	TP_PRINT("cpu=%u last_max_pid_util=%d min_capacity=%d max_capacity=%d util_target=%d",
		__entry->cpu, __entry->last_max_pid_util,
		__entry->min_capacity, __entry->max_capacity, __entry->util_target
	)
)
 
TRACE_TCB_EVENT(power_kernel, pid_next_freq,
	TP_PROTO(unsigned int, cpu,
		unsigned int, event,
		unsigned int, pid_util,
		unsigned int, ddl_load,
		unsigned int, top_load,
		unsigned int, final_util,
		unsigned int, freq
	),
	TP_ARGS(cpu, event, pid_util, ddl_load, top_load, final_util, freq),
	TP_FIELD(
		__field(unsigned int, cpu)
		__field(unsigned int, event)
		__field(unsigned int, pid_util)
		__field(unsigned int, ddl_load)
		__field(unsigned int, top_load)
		__field(unsigned int, final_util)
		__field(unsigned int, freq)
	),
	TP_ASSIGN(
		__entry->cpu = cpu;
		__entry->event = event;
		__entry->pid_util = pid_util;
		__entry->ddl_load = ddl_load;
		__entry->top_load = top_load;
		__entry->final_util = final_util;
		__entry->freq = freq;
	),
	TP_PRINT("cpu=%u event=%u pid_util=%u ddl_load=%u top_load=%u final_util=%u next_freq=%u",
		__entry->cpu, __entry->event, __entry->pid_util, __entry->ddl_load, __entry->top_load,
		__entry->final_util, __entry->freq
	)
)
 
TRACE_TCB_EVENT(power_kernel, pid_timer,
	TP_PROTO(unsigned int, cpu,
		unsigned int, type
	),
	TP_ARGS(cpu, type),
	...
	TP_PRINT("cpu=%u type=%u", __entry->cpu, __entry->type)
)
 
TRACE_TCB_EVENT(power_kernel, pid_timer_invalid,
	TP_PROTO(unsigned int, cpu,
		unsigned int, timer_cpu,
		bool, pid_control_enable,
		struct tcb_s *, tcb),
	TP_ARGS(cpu, timer_cpu, pid_control_enable, tcb),
	TP_FIELD(
		__field(unsigned int, cpu)
		__field(unsigned int, timer_cpu)
		__field(int, pid_control_enable)
		__field(int, tcb_is_null)
		__field(int, tcb_is_idle)
		__field(int, tcb_is_prev_idle)
	),
	TP_ASSIGN(
		__entry->cpu = cpu;
		__entry->timer_cpu = timer_cpu;
		__entry->pid_control_enable = pid_control_enable? 1: 0;
		if (tcb == NULL) {
			__entry->tcb_is_null = 1;
			__entry->tcb_is_idle = 0;
			__entry->tcb_is_prev_idle = 0;
		} else {
			__entry->tcb_is_null = 0;
			__entry->tcb_is_idle = tcb_is_idle(tcb);
			__entry->tcb_is_prev_idle = tcb_is_prev_idle(tcb);
		}
	),
	TP_PRINT("pid_mips_timer_invalid cpu=%u timer_cpu=%u pid_control_enable=%d tcb_is_null=%d tcb_is_idle=%d tcb_is_prev_idle=%d\n",
		__entry->cpu, __entry->timer_cpu, __entry->pid_control_enable, __entry->tcb_is_null, __entry->tcb_is_idle, __entry->tcb_is_prev_idle
	)
)
...
TRACE_EVENT(tracing_mark_write, tracing_mark_write,
	...
	TP_ASSIGN(
		...
		if (err < 0) {
			 ...
			 * length of err_msg might less than content_size.
			 */
			const char *err_msg = "<faulted>";
			str_cpy_p_to_p(dest, content_size + 1, err_msg, content_size + 1);
		} else {
