	...
	struct ____coobj_radix1var_free_list *list)
{
	...
	*target_idx = ____coobj_radix1var_size_to_list_idx(size);
	...
	/* can not split across different allocator types, e.g., 16B-256B and 512B-4KB */
	__relpos_t max_size = (*target_idx < NUM_RADIX1VAR_OBJ_SIZE) ? NUM_RADIX1VAR_OBJ_SIZE : L_NUM_RADIX1VAR_OBJ_SIZE;
	for (__relpos_t i = *target_idx; i < max_size; i++) {
...
static inline
__relpos_t ____coobj_buddy_offset(__relpos_t idx, __relpos_t len_order)
{
	return (idx ^ (1 << len_order));
}
static inline
__relpos_t ____coobj_buddy_meta_offset(void *page, __relpos_t meta_size, __relpos_t idx)
{
	__relpos_t buddy_meta_idx = ____COOBJ_INVALID_POS;
	struct ____coobj_radix1var_meta_data_s *meta =
		(struct ____coobj_radix1var_meta_data_s *)____coobj_relpos_raw(page, meta_size, idx);
	__relpos_t len_order = meta->len - 1;
	__relpos_t max_obj_meta_len =
		((meta->type == 0) ? RADIX1VAR_MAX_OBJ_SIZE : L_RADIX1VAR_MAX_OBJ_SIZE) / RADIX1VAR_MIN_OBJ_SIZE;
	if ((1 << len_order) < max_obj_meta_len) {
		buddy_meta_idx = idx / max_obj_meta_len * max_obj_meta_len +
			____coobj_buddy_offset(idx % max_obj_meta_len, len_order);
	}
	return buddy_meta_idx;
}
static inline
__bool ____coobj_buddy_is_mergeable(void *page, __relpos_t meta_size, __relpos_t meta_idx, __relpos_t buddy_meta_idx)
{
	__bool ret = __false;
	if (buddy_meta_idx != ____COOBJ_INVALID_POS) {
		struct ____coobj_radix1var_meta_data_s *meta =
			(struct ____coobj_radix1var_meta_data_s *)____coobj_relpos_raw(page, meta_size, meta_idx);
		struct ____coobj_radix1var_meta_data_s *buddy_meta =
			(struct ____coobj_radix1var_meta_data_s *)____coobj_relpos_raw(page, meta_size, buddy_meta_idx);
		if (buddy_meta->free == 0 && buddy_meta->len == meta->len) {
			ret = __true;
		}
	}
	return ret;
	...
	void *page, __relpos_t meta_size, __relpos_t idx, __u8 len, __u8 free, unsigned int flags)
{
	...
	if ((flags & HM_COOBJ_FLAGS_HKIP_ENABLE) != 0) {
		struct ____coobj_radix1var_meta_data_s meta_tmp = *meta;
		meta_tmp.len = len;
		meta_tmp.type = len > NUM_RADIX1VAR_OBJ_SIZE ? 1 : 0;
		meta_tmp.free = free;
		hm_hkip_rowm_cpy_nofail(meta, &meta_tmp, sizeof(meta_tmp));
	} else {
		meta->len = len;
...
#else
	meta->len = len;
	meta->type = len > NUM_RADIX1VAR_OBJ_SIZE ? 1 : 0;
	meta->free = free;
#endif
}
...
static inline
void ____coobj_radix1var_free_list_delete_pos(
	struct ____coobj_radix1var_uholder_lnode_s *lnode,
	struct coobj_dlist_s *dlist, void *page, __relpos_t relpos,
	__relpos_t tepar_type_size, __relpos_t tepar_chunk_order, unsigned int flags)
{
	__relpos_t prev_pos;
	__relpos_t next_pos;
	struct coobj_dlist_node_s *cur_node;
	struct coobj_dlist_node_s *prev_node;
	struct coobj_dlist_node_s *next_node;
	if (dlist->head == relpos) {
		struct coobj_dlist_node_s *dhead = ____coobj_relpos_dnode(
			lnode->child_dnode_list[relpos >> tepar_chunk_order].page_uva, tepar_type_size,
			relpos & ____coobj_order_mask(tepar_chunk_order));
		dlist->head = dhead->next;
		if (dlist->head != ____COOBJ_INVALID_POS) {
			dhead = ____coobj_relpos_dnode(
				lnode->child_dnode_list[dlist->head >> tepar_chunk_order].page_uva, tepar_type_size,
				dlist->head & ____coobj_order_mask(tepar_chunk_order));
#ifdef CONFIG_HKIP_SECCOMP_PROT
			if ((flags & HM_COOBJ_FLAGS_HKIP_ENABLE) != 0) {
				__relpos_t invalid_pos = ____COOBJ_INVALID_POS;
				hm_hkip_rowm_cpy_nofail(&dhead->prev, &invalid_pos, sizeof(invalid_pos));
			} else
#endif
			dhead->prev = ____COOBJ_INVALID_POS;
		}
	} else {
		cur_node = ____coobj_relpos_dnode(
			lnode->child_dnode_list[relpos >> tepar_chunk_order].page_uva, tepar_type_size,
			relpos & ____coobj_order_mask(tepar_chunk_order));
		prev_pos = cur_node->prev;
		next_pos = cur_node->next;
		prev_node = ____coobj_relpos_dnode(
			lnode->child_dnode_list[prev_pos >> tepar_chunk_order].page_uva, tepar_type_size,
			prev_pos & ____coobj_order_mask(tepar_chunk_order));
		if (next_pos != ____COOBJ_INVALID_POS) {
			next_node = ____coobj_relpos_dnode(
				lnode->child_dnode_list[next_pos >> tepar_chunk_order].page_uva, tepar_type_size,
				next_pos & ____coobj_order_mask(tepar_chunk_order));
#ifdef CONFIG_HKIP_SECCOMP_PROT
			if ((flags & HM_COOBJ_FLAGS_HKIP_ENABLE) != 0) {
				hm_hkip_rowm_cpy_nofail(&next_node->prev, &prev_pos, sizeof(prev_pos));
			} else
#endif
			next_node->prev = prev_pos;
		}
#ifdef CONFIG_HKIP_SECCOMP_PROT
		if ((flags & HM_COOBJ_FLAGS_HKIP_ENABLE) != 0) {
			hm_hkip_rowm_cpy_nofail(&prev_node->next, &next_pos, sizeof(next_pos));
		} else
#endif
		prev_node->next = next_pos;
	}
	dlist->size -= 1;
}
	...
	__relpos_t level1_relpos, __relpos_t tepar_type_size, __relpos_t tepar_chunk_order, unsigned int flags)
{
	...
	__relpos_t chunk_pos = meta_pos_offset;
	for (int i = (int)L_RADIX1VAR_OBJ_SIZE_NUM - 1; i >= 0; i--) {
		__relpos_t node_pos = baseof_chunk + chunk_pos;
		struct coobj_dlist_node_s *node = ____coobj_relpos_dnode(
			page, tepar_type_size, chunk_pos);
	...
	unsigned int flags)
{
	...
	while (obj_idx > target_idx && target_idx < L_NUM_RADIX1VAR_OBJ_SIZE) {
	...
	__relpos_t relpos, __relpos_t tepar_type_size, __relpos_t tepar_chunk_order, unsigned int flags)
{
	...
	__relpos_t meta_offset = chunk_relpos - RADIX1VAR_REAL_DATA_OFFSET / tepar_type_size;
	__u8 len = ____coobj_radix1var_meta_data_get_len(dnode->page_uva, RADIX1VAR_META_DATA_SIZE, meta_offset);
	__relpos_t buddy_meta_offset = ____coobj_buddy_meta_offset(dnode->page_uva, RADIX1VAR_META_DATA_SIZE, meta_offset);
	if (____coobj_buddy_is_mergeable(dnode->page_uva, RADIX1VAR_META_DATA_SIZE, meta_offset, buddy_meta_offset)) {
		/* update the metadata of obj and its buddy */
		__relpos_t buddy_obj_relpos = buddy_meta_offset + RADIX1VAR_REAL_DATA_OFFSET / tepar_type_size +
			(level1_relpos << tepar_chunk_order);
		if (buddy_meta_offset < meta_offset) {
			____coobj_radix1var_meta_data_update(dnode->page_uva, RADIX1VAR_META_DATA_SIZE,
				buddy_meta_offset, len + 1, 0, flags);
			____coobj_radix1var_meta_data_update(dnode->page_uva, RADIX1VAR_META_DATA_SIZE, meta_offset, 0, 0, flags);
			____coobj_radix1var_free_list_delete_pos(lnode, &free_list->free_list[len - 1], dnode->page_uva,
				buddy_obj_relpos, tepar_type_size, tepar_chunk_order, flags);
			____coobj_radix1var_free_list_add_head(lnode, &free_list->free_list[len], dnode->page_uva,
				buddy_obj_relpos, tepar_type_size, tepar_chunk_order, flags);
		} else {
			____coobj_radix1var_meta_data_update(dnode->page_uva, RADIX1VAR_META_DATA_SIZE, meta_offset, len + 1, 0, flags);
			____coobj_radix1var_meta_data_update(dnode->page_uva, RADIX1VAR_META_DATA_SIZE, buddy_meta_offset, 0, 0, flags);
			____coobj_radix1var_free_list_delete_pos(lnode, &free_list->free_list[len - 1], dnode->page_uva,
				buddy_obj_relpos, tepar_type_size, tepar_chunk_order, flags);
			____coobj_radix1var_free_list_add_head(lnode, &free_list->free_list[len], dnode->page_uva,
		...
		____coobj_radix1var_meta_data_update_free(dnode->page_uva, RADIX1VAR_META_DATA_SIZE, meta_offset, 0, flags);
