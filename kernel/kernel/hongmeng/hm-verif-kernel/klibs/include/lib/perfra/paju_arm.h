...
/* arm nop */
#define __paju_arm_asm_nop(point_succ) 		"nop"
...
#define __PAJU_ARM_NOP_OPCODE			0x00f020e3
#else
#define __PAJU_ARM_NOP_OPCODE			0xe320f000
#endif
...
#define __paju_arm_asm_group_data(group_name) \
	...
	"\n\t.4byte " __paju_pointsec_start_str(group_name) \
	"\n\t.4byte " __paju_pointsec_stop_str(group_name) \
	__paju_asm_size_weak_type(__paju_group_data_name_str(group_name), "%object") \
	"\n\t.popsection"
#define __paju_arm_asm_group(group_name) \
	"\n\t.ifndef " __paju_group_data_name_str(group_name) \
	__paju_arm_asm_group_data(group_name) \
	"\n\t.size " __paju_group_datasec_stop_str ", 32" \
	"\n\t.endif"

#define __paju_arm_asm_point_declare(group_name, point_name) \
	"\n\t.pushsection " __paju_pointsec_name_str(group_name) ",\"awM\",%progbits,1" \
	"\n\t.p2align 3" \
	"\n" __paju_name_str(group_name, point_name) ":" \
	__paju_asm_size_global_type(__paju_name_str(group_name, point_name), "%object", "8") \
	"\n\t.previous"
#define __paju_arm_asm_point_section(dst) \
	"\n5:" \
	"\n\tnop" \
	"\n\t.previous" \
	"\n6:" \
	"\n\t.4byte 5b, %l[" __paju_str(dst) "]" \
	"\n\t.popsection"
...
{
	__paju_asm_goto(__paju_arm_asm_point_section(dst) :::: dst);
...
#define __paju_arm_true(group_name, point_name) ({ \
		__paju_asm_volatile( \
			__paju_arm_asm_group(group_name) \
			__paju_arm_asm_point_declare(group_name, point_name)); \
		__paju_asm_volatile("" ::: "memory"); \
		__paju_arm_value_true(); \
	})
#else
#define __paju_arm_true(group_name, point_name) 	__PAJU_VALUE_TRUE
#endif

#define __paju_arch_true(group_name, point_name)	__paju_arm_true(group_name, point_name)
