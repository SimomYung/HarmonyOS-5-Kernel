...
__rmq_producer_advance(rmq_t *q, rmq_producer_t *p, __u8 cid, __u64 ratio_and_ppos)
{
	for (__u64 i = 0; i < RMQ_RETRY_NUM; i++) {
		...
		BUG_ON(RMQ_VARIABLE_POS(ratio_and_ppos) >= nppos,
		       "nppos:%lx\n", (long)RMQ_VARIABLE_POS(ratio_and_ppos));
		...
		__u64 updated_ratio_and_gpos = vatomic64_read(&q->global.ratio_and_gpos);
		if (RMQ_VARIABLE_RATIO(updated_ratio_and_gpos) < ratio) {
		...
		if (vatomic64_get_add(&mc->committed, RMQ_CAPSULE_INIT_SIZE) != next_committed) {
			continue;
		}
...
__rmq_producer_get_entry_by_cache(rmq_t *q, __u8 cid, rmq_producer_cache_t *cache, rmq_producer_entry_t *pe, __u64 size)
{
	...
	for (__u64 i = 0; i < RMQ_RETRY_NUM; i++) {
		...
		if (allocated_idx + entry_total_size <= RMQ_CAPSULE_BOUND && time_stamp_offset <= RMQ_UINT32_MAX) {
			...
			BUG_ON(dc->head.core_id != cid, "core_id:%u %u\n", dc->head.core_id, cid);
			...
			BUG_ON(dummy_size < RMQ_ENTRY_METADATA_SIZE, "dummy_size:%lx\n", (long)dummy_size);
