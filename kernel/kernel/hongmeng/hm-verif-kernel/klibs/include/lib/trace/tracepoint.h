...
#define tracepoint_enabled_of(a) 0
#endif
...
#define DECLARE_TRACE_FUNC(group, _name, tcb_event, args, proto, fields, assign) \
static inline void __do_trace_##_name(struct trace_event_entry_##_name *__entry,	\
...
{	\
	...
	if (q && unlikely(hm_rmq_is_record_enabled(q))) {			\
		...
		ret = hm_rmq_enqueue_get_entry(q, (__u8)curr_cpu, &pe, len);		\
		if (ret <= 0 || (__u64)ret != len) {				\
		} else {							\
			__entry = (struct trace_event_entry_##_name *)pe.addr;\
			__entry->th.common_flags = (unsigned char)0;			\
			__entry->th.common_preempt_count = (unsigned char)0;		\
			__entry->th.common_pid = (int)hm_trace_tid_of(tcb_event);	\
			__entry->th.common_type = (unsigned short)tracepoint_##_name.type;	\
			fields							\
			{assign} 					\
...
#define DEFINE_TRACE(group, _name, etsize, tcb_event, proto, args, fields, assign)		\
	...
	DECLARE_TRACE_FUNC(group, _name, tcb_event, PARAMS(args), PARAMS(proto), PARAMS(fields), PARAMS(assign)) \
	DECLARE_HMPROBE_CLASS(group, _name, tcb_event, PARAMS(args), PARAMS(proto), PARAMS(fields), PARAMS(assign)) \
	void __trace_probe_##_name(TP_FUNC_PROTO(proto))				\
	{										\
		struct trace_event_entry_##_name *__entry = NULL;			\
		struct trace_##group##_##_name##_offset_s data_offsets; 		\
		unsigned int array_len = (unsigned int)trace_cal_##_name##_offsets(&data_offsets, \
										   TP_FUNC_ARGS(proto));	\
		__s32 ____enabled = tracepoint_enabled_of(&tracepoint_##_name);		\
											\
		if (tracepoint_is_trace_enabled(____enabled)) {	\
			__do_trace_##_name(__entry, data_offsets, array_len, TP_FUNC_ARGS(proto));		\
			if (__read_once(tracepoint_##_name.attached_events) > 0) {			\
				perf_trace_##_name(etable_##_name, TP_FUNC_ARGS(proto));	\
			}								\
		}	\
		if (tracepoint_is_probe_enabled(____enabled)) {	\
			hmprobe_update_##_name##_tp_func(__entry, data_offsets, args);		\
		}	\
	}
