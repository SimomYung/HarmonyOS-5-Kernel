 ...
 * Description: The strncmp function
 * Author: Huawei OS Kernel Lab
 * Create: Fri Jul 6 13:22:05 2018
 */
#ifdef __KERNEL__
	...
	if(n >= sizeof(n)) {
		...
		do {
			...
			if ( ((((uintptr_t)s1|(uintptr_t)s2) & (sizeof(n)-1)) == 0) && /* we're dbl aligned */
				(!hasnull(*(size_t*)s1) && !hasnull(*(size_t*)s2)) && /* no nulls */
					(*(size_t*)s1 == *(size_t*)s2) ) { /* they match in full words */
						s1+=sizeof(size_t);
						s2+=sizeof(size_t);
