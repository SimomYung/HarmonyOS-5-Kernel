...
void aug_rbt_insert_with_callback(struct aug_rbt_tree *aug_tree, struct rbt_node *node,
		struct rbt_node *parent, bool is_rchild, summary_update_callback upd_cb)
{
	__rbt_insert(&aug_tree->tree, node, parent, is_rchild, upd_cb);
}
...
void aug_rbt_remove_with_callback(struct aug_rbt_tree *aug_tree, struct rbt_node *node, summary_update_callback upd_cb)
{
	__rbt_remove(&aug_tree->tree, node, upd_cb);
}
