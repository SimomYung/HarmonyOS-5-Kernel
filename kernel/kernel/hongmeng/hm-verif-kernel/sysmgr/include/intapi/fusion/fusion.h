...
unsigned long fusion_flush_cache_range_by_pa(unsigned long pstart, unsigned long pend,
					     unsigned int cmd);
...
int fusion_sysmgr_workqueue_flush(void);
int fusion_sysmgr_workqueue_flush_until(struct raw_work *work);
...
static inline int fusion_sysmgr_workqueue_flush(void)
{
	return E_HM_NOSYS;
}

static inline int fusion_sysmgr_workqueue_flush_until(struct raw_work *work)
{
	UNUSED(work);
	return E_HM_NOSYS;
}
