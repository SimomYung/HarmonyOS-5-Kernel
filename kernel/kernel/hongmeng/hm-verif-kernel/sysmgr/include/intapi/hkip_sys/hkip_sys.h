...
int hkip_rsvmem_range_update(void);
bool hkip_pfn_savable(uint64_t pfn);

#ifdef CONFIG_HKIP_SEC_REGION_DISABLE_SUPPORT
int sec_region_sys_disable(void);
#endif
...
static inline int hkip_rsvmem_range_update(void)
{
	return 0;
}

static inline bool hkip_pfn_savable(uint64_t pfn)
{
	UNUSED(pfn);
	return false;
}

static inline int sec_region_sys_disable(void)
{
	return 0;
}
#endif
