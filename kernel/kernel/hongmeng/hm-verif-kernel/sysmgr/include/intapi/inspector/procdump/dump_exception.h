...
static inline bool
is_dump_process(const struct process_s *process)
{
	if (!process_is_system_service(process) && (user_process_dump_read() == 0)) {
		return false;
	} else {
		return true;
	}
...
#else
static inline bool
is_dump_process(__maybe_unused const struct process_s *process)
{
	return true;
}
#endif
