...
struct lru_cache_s {
	...
	raw_atomic_ullong_t nr_shmem[__NR_LRU_LISTS]; /* ashmem/memfd/tmpfs */
#ifdef CONFIG_USE_PREEMPT_PROCESS
...
struct scan_control {
	...
	union {
		struct {
			...
			unsigned int force:1; /* force will skip abit check */
			unsigned int hibernate:1; /* reclaim for hibernate mode */
		};
		unsigned int flags;
	};
	...
	unsigned long nr_no_compressed; /* total nr_pages which do not need compress */
	bool force_shrink_all_reclaim;
#endif
...
void lru_cache_init(struct lru_s *lru);
...
size_t lru_anon_shrink_para(struct para_anon_entry *entry);
#endif
#ifdef CONFIG_MEM_PURGEABLE
