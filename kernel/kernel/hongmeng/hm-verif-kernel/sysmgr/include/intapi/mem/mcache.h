...
/*
 * mcache_fetch_ator_by_name: fetch the matched slab_ator by comparing the mcache name,
 * used in mcache_for_each
 *
 * @param mcache	[I] The mcache whose name to be queried.
 *
 * @param args		[I/O] store names and return matched slab_ator in args
 */
void mcache_fetch_ator_by_name(struct mcache_s *mcache, void *args);
...
{										\
	...
	BUG_ON(g_##mcachename##_index_table == 0); /* not allow exception rollback here */ \
