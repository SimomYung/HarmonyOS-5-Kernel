...
#define MAX_VR_VERSION_CHECK_TIMES 10000U
/* 
...
int mem_proc_sysview(char *data, size_t data_len);
void mem_proc_sysview_dump(void);
#endif
...
#ifdef CONFIG_HIGH_ORDER_ACCT
int mem_proc_high_order_acct(char *data, size_t data_len);
#endif
...
inline uint64_t get_process_dmaheap_size_inbyte(struct process_s *process)
{
	UNUSED(process);
	return 0;
}
...
int mem_single_proc_dmabuf_process(char *data, size_t data_len, struct process_s *process);
...
int mem_proc_debug_meminfo(char *buf, size_t len, size_t *size);
