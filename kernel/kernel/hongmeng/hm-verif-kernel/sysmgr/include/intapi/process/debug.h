...
struct pdebug {
	...
	struct base_cred *debugger_cred;
	__u32 debugger_cnode_idx;
...
bool ptracer_capable(const struct thread_s *thread);
...
static inline bool ptracer_capable(__maybe_unused const struct thread_s *thread)
{
	UNUSED(thread);
	return true;
}
