...
#include <intapi/process/kill_acct.h>
...
enum process_flags_type {
	/* RLIMIT_NPROC type exceeded in setuid */
	TYPE_PROCESS_NPROC_EXCEEDED,
	TYPE_PROCESS_MEM_REAPING,
	TYPE_PROCESS_VFORK_USE_PARENT_VS,
	TYPE_PROCESS_FORK_NOTICE_FS,
	TYPE_PROCESS_MEM_OOM_KILLING = 5,

#ifdef CONFIG_HUNGTASK_EXIT
	/* flags type for process exit */
	/* indicates that the process has entered the function:thread_handle_hmsignal_kill */
	TYPE_PROCESS_EXIT_IN_HANDLE_SIGNAL = 20,
	/* indicates that the process has entered the function:process_remove_thread */
	TYPE_PROCESS_EXIT_BEFORE_PROCESS_EXIT,
	/* indicates that the process has entered the function:process_resource_delete_from_ref */
	TYPE_PROCESS_EXIT_IN_RESOURCE_DELETE,
	/* indicates that the process has entered the function:process_cleanup */
	TYPE_PROCESS_EXIT_IN_PROCESS_CLEANUP,
	/* indicates that the process has entered the function:process_delete_from_ref */
	TYPE_PROCESS_EXIT_IN_PROCESS_DELETE,
#endif

#ifdef CONFIG_RESMGR_PIDS_FORK_CONTROL
	TYPE_PROCESS_FORK_FROM_DFX = 30,
	TYPE_PROCESS_FORK_FROM_WEBDFX,
#endif
};

#define INVALID_CNODE_RREF			0xFFFFFFFFFFFFFFFF
/* RLIMIT_NPROC exceeded in setuid */
#define PROCESS_NPROC_EXCEEDED			(1U << TYPE_PROCESS_NPROC_EXCEEDED)
#define PROCESS_MEM_REAPING			(1U << TYPE_PROCESS_MEM_REAPING)
#define PROCESS_VFORK_USE_PARENT_VS		(1U << TYPE_PROCESS_VFORK_USE_PARENT_VS)
#define PROCESS_FORK_NOTICE_FS			(1U << TYPE_PROCESS_FORK_NOTICE_FS)
#define PROCESS_MEM_OOM_KILLING			(1U << TYPE_PROCESS_MEM_OOM_KILLING)
#ifdef CONFIG_HUNGTASK_EXIT
/* flags for process exit */
#define PROCESS_EXIT_IN_HANDLE_SIGNAL		(1U << TYPE_PROCESS_EXIT_IN_HANDLE_SIGNAL)
#define PROCESS_EXIT_BEFORE_PROCESS_EXIT 	(1U << TYPE_PROCESS_EXIT_BEFORE_PROCESS_EXIT)
#define PROCESS_EXIT_IN_RESOURCE_DELETE 	(1U << TYPE_PROCESS_EXIT_IN_RESOURCE_DELETE)
#define PROCESS_EXIT_IN_PROCESS_CLEANUP		(1U << TYPE_PROCESS_EXIT_IN_PROCESS_CLEANUP)
#define PROCESS_EXIT_IN_PROCESS_DELETE		(1U << TYPE_PROCESS_EXIT_IN_PROCESS_DELETE)
#endif
#ifdef CONFIG_RESMGR_PIDS_FORK_CONTROL
#define	PROCESS_FORK_FROM_DFX			(1U << TYPE_PROCESS_FORK_FROM_DFX)
#define	PROCESS_FORK_FROM_WEBDFX		(1U << TYPE_PROCESS_FORK_FROM_WEBDFX)
#endif

#ifdef CONFIG_FREEZER_USER_HANDLER
enum process_ext_flags_type {
	TYPE_PROCESS_IN_USERHANDLER,
	TYPE_PROCESS_IN_USERHANDLER_FLAG,
	TYPE_PROCESS_REIN_USERHANDLER_FLAG,
};
/* the process has a thread which is in user handler */
#define PROCESS_IN_USERHANDLER			(1U << TYPE_PROCESS_IN_USERHANDLER)
/* user handler is being or has been executed */
#define PROCESS_IN_USERHANDLER_FLAG		(1U << TYPE_PROCESS_IN_USERHANDLER_FLAG)
/* the process has a thread which has reined user handler */
#define PROCESS_REIN_USERHANDLER_FLAG		(1U << TYPE_PROCESS_REIN_USERHANDLER_FLAG)
#endif
...
#ifdef CONFIG_HISI_PROCESS_PERF_CTRL
struct process_load_stat_s {
	__u64 last_load_sum;
	__u64 window_start;
};
#endif
...
#ifdef __HOST_UT__
/*
 * Because some ugly bug in sysmgr unittest framework, different
 * .c can see different header, even they are going to be merged
 * into one ELF. This happends in UT_shmm_open_grantee, where
 * sysmgr/procmgr/process/procpool.c is compiled with CONFIG_VFS_FUSION
 * off, but sysmgr/memmgr/mem/shmem/shm_node.c is compiled with
 * CONFIG_VFS_FUSION on, which makes offset of process refcnt difference,
 * so UT_shmm_open_grantee has to wait for refcnt rollback to finish.
 *
 * This __padding_for_fixing_ut macro adds a useless fields in affected
 * data structiue. With proper alignment setting, the offset of key data
 * structure can be adjusted.
 */
#define __padding_for_fixing_ut(n, a) \
	struct { \
		unsigned char __padding; \
	} __padding_for_fixing_ut_##n __attribute__((aligned(a)));
#else
#define __padding_for_fixing_ut(n, a)
#endif
struct process_s {
	 ...
	 * Please use process_signal_lock and process_signal_unlock
	 * instead of raw_mutex_lock and raw_mutex_unlock to acquire and release the signal_lock
	 *
...
#ifdef CONFIG_HISI_MM_AMA_POLICY
	uint32_t il_prev;
#endif
...
#ifdef CONFIG_HISI_PROCESS_PERF_CTRL
	struct process_load_stat_s load_stat;
#endif
#ifdef CONFIG_YAMA
...
void process_privilege_num_init(struct process_s *process, bool is_sys_srv, uint32_t privilege_num);
...
struct pid_ns *caller_ns_of(unsigned long credential, bool is_uspinhdlr);
