...
process_test_signal_handler(struct process_s *process, unsigned int signum)
{
	...
	process_signal_lock(process);
	ret = __process_test_signal_handler(process, signum);
	process_signal_unlock(process);
...
static inline bool
__process_test_signal_default(const struct process_s *process, unsigned int signum)
{
	return (signum > 0 && signum < _NSIG &&
		process->signal->actions[__SIGIDX_OF(signum)].handler == HM_SIG_DFL);
}

static inline bool
process_test_signal_default(struct process_s *process, unsigned int signum)
{
	bool ret = false;

	process_signal_lock(process);
	ret = __process_test_signal_default(process, signum);
	process_signal_unlock(process);
