...
#ifdef CONFIG_ENCAPS
static inline int copy_encaps_list_from_security(struct base_cred *cred, struct security_s *security, bool sign)
{
	int ret = E_HM_OK;
	...
	if (sign) {
		hm_pacdb_bcred(cred, SEC_PAC_BASE_ENCAPS);
	}
	raw_rwlock_unlock(&cred->cred_encaps->rwlock);
	return ret;
}

static inline int set_encaps_for_kernel_proc(struct base_cred *cred, struct security_s *security, bool sign)
{
	int ret = E_HM_OK;
...
static inline int set_encaps_common(struct base_cred *cred, struct security_s *security, bool sign)
{
	if (cred->cred_encaps == NULL && cred->encaps_idx == 0) {
		cred->cred_encaps = sec_cred_encaps_alloc(&cred->encaps_idx);
	}
	if (cred->cred_encaps != NULL) {
		return copy_encaps_list_from_security(cred, security, sign);
	} else {
		return E_HM_NOMEM;
	}
}

static inline int copy_encaps_from_security(struct base_cred *cred, struct security_s *security, bool sign)
{
	int ret = E_HM_OK;

	if (security->props.system_service == 1U) {
		ret = set_encaps_for_kernel_proc(cred, security, sign);
	} else if (security->cred.encaps.count != 0) {
		ret = set_encaps_common(cred, security, sign);
	}
	if (ret != E_HM_OK) {
...
#endif
#ifdef CONFIG_ENCAPS
	copy_encaps_from_security(cred, &proc->security, false);
#endif
...
#endif
#ifdef CONFIG_ENCAPS
	copy_encaps_from_security(b_cred, security, false);
#endif
#ifdef CONFIG_KEYMGR
