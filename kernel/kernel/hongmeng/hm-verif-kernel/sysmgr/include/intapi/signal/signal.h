...
int signalmgr_kill_thread(struct thread_s *target, unsigned int signum,
			  struct process_s *source, const struct __siginfo *siginfo);
int signalmgr_kill_thread_nolock(struct thread_s *target, unsigned int signum,
				 struct process_s *source, const struct __siginfo *siginfo);
...
void signalmgr_clone_finish(const struct thread_s *parent, struct thread_s *child);
...
int __signalmgr_check_signal_handler(struct process_s *target,
				     const struct process_s *source, bool same_origin,
				     unsigned int signum);
int __signalmgr_dequeue_specific(struct thread_s *t, unsigned int signum, struct __siginfo *info);

/*
 * Caller should hold process->signal_lock and mode should be one of CLD_PROC_CONT CLD_PROC_STOP
 */
void __set_process_notify_parent_mode(struct process_s *p, unsigned int mode);
void process_signal_lock(struct process_s *p);
void process_signal_unlock(struct process_s *p);
...
int sigalmgr_do_rt_sigtimedwait(struct thread_s *t, void *set, void *info,
			      void *timeout, unsigned long size);
int signalmgr_do_rt_sigsuspend(unsigned long credential, void *set, unsigned long size);
int signalmgr_signal_tkill_lsyscall(unsigned long credential,
				    int tid, int signum);
#ifdef CONFIG_SIGNALFD_SUPPORT
...
void __signalmgr_suspend_thread_async(struct thread_s *thread);
