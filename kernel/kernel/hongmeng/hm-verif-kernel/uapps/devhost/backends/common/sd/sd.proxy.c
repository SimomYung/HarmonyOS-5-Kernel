			...
			void *wr, size_t wr_size)
{
	...
	switch (PAL_PROXY_MINOR_CMD_OF(cmd)) {
		case DMA_BUF_MINOR_CMD_COPY:
			if (wr_size != sizeof(int) || rd_size != sizeof(int)) {
				return -EINVAL;
			}
			ret = copy_to_user(wr, (void *)&w_test, sizeof(int));
			if (ret != 0) {
				pr_err("sd proxy copy to user failed, ret=%d\n", ret);
				return ret;
			}
			ret = copy_from_user((void *)&r_test, rd, sizeof(int));
			break;
		default:
			ret = -ENOSYS;
			break;
