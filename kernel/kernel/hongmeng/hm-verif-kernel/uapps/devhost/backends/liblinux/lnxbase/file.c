...
static lnxbase_ipermission_fn_t lnxbase_ipermission_fn;
...
int lnxbase_register_inode(lnxbase_ipermission_fn_t ipmfn)
{
	if (lnxbase_ipermission_fn != NULL) {
		return E_HM_EXIST;
	}

	lnxbase_ipermission_fn = ipmfn;
	return 0;
}
...
void *lnxbase_pidfd_anon_fget(int pid, int fd, void (*fget)(void *))
{
	struct __actvret_hmcall_proc_fd_query_file actv_ret;
	struct filp_node *node = NULL;
	void *filp = NULL;
	unsigned long long file_idx;
	unsigned int fastpath;
	...
	mem_zero_s(actv_ret);
	ret = actvxactcall_hmcall_proc_fd_query_file(false, true, pid, fd, &actv_ret);
	if (ret != E_HM_OK) {
		dh_error("query_file failed, pid=%d, fd=%d, err=%s\n", pid, fd, hmstrerror(ret));
		return NULL;
	}

	file_idx = actv_ret.file_idx;
	fastpath = actv_ret.fastpath;

	if (!fastpath) {
		dh_error("proc_fd_query_file error ret, not fastpath file\n");
		return NULL;
	}

	acquire_devhost_filp_node(node, 0, fastpath, file_idx);
	if (node == NULL) {
		dh_error("acquire filp_node failed, fastpath=%u, file_idx=0x%llx\n",
			 fastpath, file_idx);
		return NULL;
	}
	...
	fget(filp);
	/*
	 * Corresponding to devhost_get_file in proc_fd_query_file.
	 * It uses IO_DEV_FASTPATH_GET to inc refcnt of fastpath filp_node.
	 * The fastpath and file_idx are same as those in devhost_fops_file_get.
	 * Only acquire_devhost_filp_node success, it will call devhost_get_file
	 * and devhost_put_file.
	 */
	devhost_put_file(node);
...
lnxbase_ipermission_fn_t lnxbase_inode_permission_fn(void)
{
	return lnxbase_ipermission_fn;
}
