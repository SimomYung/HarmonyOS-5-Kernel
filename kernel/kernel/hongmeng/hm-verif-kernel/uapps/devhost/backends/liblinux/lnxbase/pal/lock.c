...
#ifndef LIBLINUX_PAL_FUTEX_KILLABLE
#define LIBLINUX_PAL_FUTEX_KILLABLE		16UL
#endif
				...
				const struct liblinux_wchan_info *wchan_info)
{
	...
	if (____IS(flags & LIBLINUX_PAL_FUTEX_UNINTERRUPTIBLE)) {
		mask = &maskall;
	} else if (____IS(flags & LIBLINUX_PAL_FUTEX_KILLABLE)) {
		mask = &maskall;
		maskall.__mask &= ~(1ULL << HMSIGNAL_KILL);
	}
	...
	if (____IS(flags & LIBLINUX_PAL_FUTEX_LOCK_IN_LDK)) {
		__flags |= (__FUTEX_FLAG_LOCK_IN_LDK | __FUTEX_BLOCKED_HAS_LOCK_OWNER);
	}
