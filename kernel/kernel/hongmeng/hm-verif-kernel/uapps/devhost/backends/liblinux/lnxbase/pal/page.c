...
void *liblinux_pal_reserve_size_name(unsigned long size, int prot, int flags, const char *name)
{
	void *rvaddr = NULL;
	...
	rvaddr = hm_mem_mmap(0, (size_t)size, (uint32_t)prot,
			     (uint32_t)flags, 0, 0);
	if (rvaddr == MAP_FAILED) {
	...
	/* set optional anon memory name */
	if (name != NULL) {
		(void)hm_mem_set_anon_name(rvaddr, (size_t)size, name);
	}
	return rvaddr;
}
