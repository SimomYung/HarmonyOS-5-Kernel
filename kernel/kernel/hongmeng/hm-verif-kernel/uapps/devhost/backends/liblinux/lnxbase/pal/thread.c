			       ...
			       int preempt_flag)
{
	...
	ret = hm_sysctrl_read_conf(__SYSCONF_REGISTER_CPU, &cpu_nr, sizeof(cpu_nr));
	if (ret < 0) {
...
void liblinux_pal_thread_dump(pid_t tid)
{
	...
	dumpstack_set_flag(flags, DUMPSTACK_FLAGS_SYMBOL_RELATIVE_ADDR);

	/* will dump current thread if tid is 0 */
	ret = hm_dump_thread_to_klog(tid, flags);
	if (ret < 0) {
		dh_error("dump thread %u failed, ret=%s\n", tid, hmstrerror(ret));
	}
}
...
int liblinux_pal_get_control_terminal_devno(unsigned int *devno)
{
	...
	if (ret < 0) {
		dh_error("failed to get ttyinfo, ret=%s\n", hmstrerror(ret));
		return -hmerrno2posix(ret);
	}
...
int liblinux_pal_get_sid_pgid(int *sid, int *pgid)
{
	...
	ret = devhost_current_sid_pgid(sid, pgid);
	if (ret < 0) {
		dh_error("get sid & pgid failed, err=%d\n", ret);
	}
...
{
	int ret = hm_thread_freezer_count(0, count, fwd);
	if (ret != E_HM_OK) {
		dh_error("hm thread freezer %scount, ret=%s\n", count?"":"do not ", hmstrerror(ret));
	}
}

int liblinux_pal_process_dormancy_check(void)
{
	return actvcall_hmcall_proc_pm_dormancy_check();
}
...
{
	int ret = hm_thread_get_name_by_root_ns_tid(tid, comm, len);
	if (ret != E_HM_OK) {
		return -hmerrno2posix(ret);
	}
	return 0;
...
/* set vip prio by thread id if the input parameter prio is equal to 1 (or true) */
int liblinux_pal_thread_set_vip_prio_by_tid(int tid, int prio)
{
	int ret = actvcall_hmcall_thread_set_vip_prio(tid, prio);
	if (ret != E_HM_OK) {
		dh_error("actvcall_hmcall_thread_set_vip_prio failed, err=%s\n", hmstrerror(ret));
		return -hmerrno2posix(ret);
	}
	return 0;
}
