...
STATIC_DEFINE_RAW_RWLOCK(_hook_lock);
...
int liblinux_pal_remap_vmalloc_range(unsigned long long vs_key, unsigned long uaddr, unsigned long kaddr,
	unsigned long size, unsigned long prot, unsigned int flags)
{
	return libdh_vmalloc_mmap_page(vs_key, uaddr, kaddr, size, prot, flags);
}
