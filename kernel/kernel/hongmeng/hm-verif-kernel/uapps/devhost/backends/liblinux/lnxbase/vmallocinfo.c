...
static int __procfs_ctx_init(struct procfs_ctx *ctx)
{
	*(ctx->r_size_out) = 0;
	...
	ctx->show_kaddress = liblinux_pal_kptr_show_value();
			      ...
			      const char *fmt, ...)
{
	...
	va_list args;
	va_start(args, fmt);
	ret = lnxbase_liblinux_vformat_string(ctx->tmp, ctx->tmp_sz, fmt, args);
	if (ret < 0) {
		ret = E_HM_OVERFLOW;
	}
	va_end(args);
...
{
	int ret = 0;
	const void *vaddr_start = NULL;
	const void *vaddr_end = NULL;
	...
	if (ctx->show_kaddress) {
		vaddr_start = vnode->vaddr;
		vaddr_end = ulong_to_ptr(ptr_to_ulong(vnode->vaddr) + vnode->size, const void);
	}

	ret = __procfs_ctx_write(ctx, "0x%px-0x%px %7lu",
				 vaddr_start,
				 vaddr_end,
				 pmem_size);
	if (ret < 0) {
	...
	if (vnode->caller != NULL) {
		ret = __procfs_ctx_write(ctx, " %pS", vnode->caller);
		if (ret < 0) {
			return ret;
		}
	}

	return __procfs_ctx_write(ctx, " pages=%d", nr_pages);
}
