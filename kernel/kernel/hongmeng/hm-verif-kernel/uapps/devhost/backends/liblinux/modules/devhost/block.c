...
static void end_bio(struct bio *bio)
{
	...
	if (bio_status(bio) != 0) {
#if LINUX_VERSION_CODE > KERNEL_VERSION(4,14,0) && LINUX_VERSION_CODE < KERNEL_VERSION(6, 6, 0)
		pr_warn("bio callback called, err=%d, driver name=%s\n", (int)bio_status(bio), (bio->bi_disk ? bio->bi_disk->disk_name : "NULL"));
#else
