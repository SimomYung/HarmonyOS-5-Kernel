...
void *udk_dlsym(void *handle, const char *symbol)
{
#ifdef CONFIG_UDK_FUSION
	UNUSED(handle, symbol);
	return NULL;
#else
...
void *udk_dlopen(const char *filename, int flag)
{
#ifdef CONFIG_UDK_FUSION
	UNUSED(filename, flag);
	return NULL;
#else
...
void *udk_dlopen_from_mem(const void *mem, const char *name, int flag)
{
#ifdef CONFIG_UDK_FUSION
	UNUSED(mem, name, flag);
	return NULL;
#else
	return dlopen_from_mem(mem, name, flag);
#endif
}

char *udk_dlerror(void)
{
#ifdef CONFIG_UDK_FUSION
	return NULL;
#else
...
static int udk_load_module_deps(const char *libname, const void *data)
{
	...
	handle = udk_dlopen_from_mem(data, libname, RTLD_LAZY | RTLD_LOCAL);
	if (handle == NULL) {
	...
	module_deps = (char (*)[UDK_MODULE_STR_MAX_LEN])udk_dlsym(handle, "udk_module_deps");
	module_len = (int *)udk_dlsym(handle, "udk_module_len");
...
static int init_module(const struct udk_module *module, const char *libname, void *handle)
{
	...
	set_id_func = (init_id_type)(uintptr_t)udk_dlsym(handle, "udk_set_this_module");
	/* We allow a module to not have set_id_func */
	if (set_id_func != NULL) {
		set_id_func(module);
	}

	func = (init_type)(uintptr_t)udk_dlsym(handle, "udk_init_module");
...
static int udk_init_params(struct udk_module *module, void *handle, char *args)
{
	...
	func = (param_type)(uintptr_t)udk_dlsym(handle, "udk_get_param_list");
	if (func == NULL) {
		return 0;
	}
	param_list = func();
			   ...
			   const char *libname, const void *data, size_t size, char *args)
{
	...
	handle = udk_dlopen_from_mem(data, libname, RTLD_NOW | RTLD_GLOBAL);
	if (handle == NULL) {
...
static int exit_module(const char *libname, void *handle)
{
	...
	func = (init_type)(uintptr_t)udk_dlsym(handle, "udk_exit_module");
