...
udk_blk_mq_alloc_request(struct udk_request_queue *queue, struct udk_bio *bio)
{
	...
	if (bio == NULL || queue == NULL || queue->mq_ops == NULL) {
		return NULL;
	}
	cmd_flags = bio->bi_rw;
#ifdef CONFIG_HISI_UDK_MAS_BLK
...
void udk_blk_mq_free_request(struct udk_request *req)
{
	...
	if (req == NULL || req->queue == NULL) {
		udk_error("invalid req or req->queue\n");
		return;
	}
	queue = req->queue;
	hctx = req->mq_hctx;
	tag = (unsigned int)req->tag;
		...
		busy_tag_iter_fn *fn, void *priv)
{
	...
	if (tagset == NULL) {
		udk_error("invalid tagset\n");
		return;
	}
	flags = tagset->flags;
	for (i = 0; i < (int)tagset->nr_hw_queues; i++) {
			   ...
			   struct udk_io_scheduler_queue *sched)
{
	...
	if (queue == NULL || sched == NULL || sched->type == NULL) {
		udk_error("invalid queue, sched or sched->type\n");
		return;
	}
	for (unsigned int idx = 0; idx < queue->nr_hw_queues; idx++) {
		hctx = queue->hctx_table[idx];
			  ...
			  struct udk_io_scheduler_type *sched_type)
{
	...
	if (queue == NULL || sched_type == NULL || queue->tag_set == NULL) {
		return -EINVAL;
	}
	flags = queue->tag_set->flags;
...
{
	uint32_t pending = 0;
	struct udk_blk_mq_tag_set *set = NULL;
	struct udk_blk_mq_tags **tags = NULL;

	if (queue == NULL || queue->tag_set == NULL) {
		return -EINVAL;
	}
	set = queue->tag_set;
	tags = set->tags;
	for (int i = 0; i < (int)set->nr_hw_queues; i++)
