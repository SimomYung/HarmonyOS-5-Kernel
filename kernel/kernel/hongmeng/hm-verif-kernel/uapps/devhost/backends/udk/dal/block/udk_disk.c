...
static void disk_sysfs_block_create(struct udk_disk *disk)
{
	struct udk_partition *part0 = NULL;
	struct udk_partition *part = NULL;
	int i;

	/* create the global sysfs block directory */
	udk_sysfs_block_dir_create(disk);

	/* add each partition sysfs node */
	for (i = 1; i < MAX_PARTITIONS; i++) {
		part = disk->parts[i];
		if (part != NULL) {
			udk_sysfs_block_create(&(part->udkdev),
								MKDEV(disk->major, disk->first_minor + i));
		}
	}

	/* add part0 sysfs node */
	part0 = &(disk->part0);
	udk_sysfs_block_create(&(part0->udkdev),
						MKDEV(disk->major, disk->first_minor + part0->partno));
}
