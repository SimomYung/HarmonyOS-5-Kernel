...
static void partition_parse_efi_part(struct udk_parsed_partitions *p,
			       ...
			       struct udk_disk *disk, unsigned int logical_size)
{
	...
	if ((ret == E_HM_OK) && (gpt != NULL) && (pte != NULL)) {
		partition_parse_efi_part(parsed_parts, pte, gpt->last_usable_lba, logical_size);
	}
					       ...
					       uint64_t len, char *part_name)
{
	...
	ret = udk_disk_device_add(name,
			      MKDEV(disk->major, disk->first_minor + partno),
			      udkdev);
	if (ret != E_HM_OK) {
		free(name);
		free(part);
