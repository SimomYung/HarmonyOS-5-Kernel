...
int udk_chrctx_init(struct udk_char_device *udk_chrdev,
		    struct udk_char_context *udk_chrctx,
		    void *ctx, unsigned int io_flags)
{
	int err;

	if (udk_chrdev == NULL || udk_chrctx == NULL) {
		return -EINVAL;
	}

	udk_chrctx->ctx		= ctx;
	udk_chrctx->io_flags	= io_flags;
	udk_chrctx->chrdev	= udk_chrdev;

	if (udk_chrdev->ops != NULL && udk_chrdev->ops->open != NULL) {
		err = udk_chrdev->ops->open(udk_chrctx);
		if (err != 0) {
			return err;
		}
	}

	raw_refcnt_init(&udk_chrctx->refcnt, 1, native_char_context_release);
	return err;
}
...
static int native_chrdev_open(struct device_info *info, void **ctx, unsigned int flags)
{
	...
	err = udk_chrctx_init(udk_chrdev, fctx, NULL, flags);
	if (err != 0) {
		udk_device_opener_dec(&udk_chrdev->udkdev);
		udk_module_close(owner);
		udk_free(fctx);
