...
#include <udk/log.h>
#ifdef CONFIG_HISI_UDK_IOMMU
#include <see/devhost/backends/udk/include/udk/udk_iommu.h>
#endif
...
STATIC_DEFINE_RAW_RWLOCK(g_fusion_dev_lock);

/* Define the probe wait queue */
static struct udk_thread_cond probe_waitqueue;
static struct raw_mutex probe_wait_mutex = RAW_MUTEX_INITIALIZER;
static struct udk_atomic_int *probe_count;
...
int udk_fusion_hdr_device_bind(int devid, struct hm_actv_buf dev_compat)
{
	...
	/* decrement device probe count after successful bind */
	udk_device_probe_dec();
	udk_fusion_devinfo_put(info);
...
/* Initialize probe-related resources. */
static int probe_resources_init(void)
{
	int ret = udk_thread_cond_init(&probe_waitqueue, NULL);
	if (ret < 0) {
		return ret;
	}

	probe_count = udk_atomic_int_alloc(0);
	if (probe_count == NULL) {
		return -ENOMEM;
	}
...
udk_init_call(probe_resources_init);

/* Increment the device probe count. */
void udk_device_probe_inc(void)
{
	udk_atomic_int_inc(probe_count);
}

/* Decrement the device probe count. */
void udk_device_probe_dec(void)
{
	udk_atomic_int_dec(probe_count);

	/* If probe count reaches zero, broadcast to waiting threads. */
	if (udk_atomic_int_read(probe_count) == 0) {
		(void)udk_thread_cond_broadcast(&probe_waitqueue);
	}
}

/**
 * @brief Waits for all device probing processes to finish.
 *
 * This function waits until all device probing is complete by checking that
 * `probe_count` reaches zero. The `probe_count` is incremented when a device
 * is instantiated (via `udk_of_device_instantiate`), and decremented only after
 * the driver binding (`drv_ops->bind`) completes, regardless of whether the binding
 * succeeds or fails. If the probing process does not reach the binding stage, the
 * count is not decremented.
 *
 * The function locks a mutex to safely check and wait for the probe count to
 * reach zero, ensuring thread safety during the process.
 */
void wait_for_device_probe(void)
{
	/* lock mutex to protect probe count check and wait operation */
	raw_mutex_lock(&probe_wait_mutex);

	/* wait until the device count is zero */
	while (udk_atomic_int_read(probe_count) != 0) {
		udk_info("waiting for device probe to complete. Current probe_count: %d\n", udk_atomic_int_read(probe_count));
		(void)udk_thread_cond_wait(&probe_waitqueue, &probe_wait_mutex);
	}

	/* unlock the mutex after the wait is over */
	raw_mutex_unlock(&probe_wait_mutex);
}