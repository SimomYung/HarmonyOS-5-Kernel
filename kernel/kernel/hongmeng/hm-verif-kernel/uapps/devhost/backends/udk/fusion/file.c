...
static void udk_fusion_filp_node_release(struct raw_compounded_auto_refcnt *ref)
{
	int ret;
...
struct filp_node *udk_fusion_filp_node_alloc(__u32 cnode_idx, struct device_info *info)
{
	...
	raw_compounded_auto_refcnt_init(&node->refcnt, 1U, udk_fusion_filp_node_release);
