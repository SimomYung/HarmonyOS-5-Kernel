...
{
	if (info == NULL || fops == NULL || info->p == NULL) {
		udk_error("invalid info or fops\n");
		return;
	}
...
{
	if (info == NULL || ex_fops == NULL || info->p == NULL) {
		udk_error("invalid info or ex_fops\n");
		return;
	}
...
{
	if (info == NULL || pmops == NULL || info->p == NULL) {
		udk_error("invalid info or pmops\n");
		return;
	}
	info->p->pmops = pmops;
}
