...
STATIC_DEFINE_RAW_RWLOCK(g_fusion_pm_lock);
...
static int __udk_pm_wait_for_device_probe(void)
{
...
static int __udk_pm_action(unsigned int action)
{
	...
	case DH_PM_WAIT_FOR_DEVICE_PROBE:
		return __udk_pm_wait_for_device_probe();
	default:
