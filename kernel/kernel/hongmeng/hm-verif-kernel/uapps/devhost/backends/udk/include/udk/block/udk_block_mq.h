...
#ifdef CONFIG_HISI_UDK_MAS_DEBUG_FS
	/*
...
struct udk_blk_mq_tag_set {
	...
	bool ufs_flag;
#ifdef CONFIG_HISI_UDK_MAS_BLK_NVME
	bool nvme_flag;
#endif
#endif
};
...
struct udk_blk_mq_ops {
	...
	void (*map_queues)(struct udk_blk_mq_tag_set *set, unsigned int cpu_num);
	struct udk_blk_mq_hw_ctx* (*update_hctx)(struct udk_request_queue *queue, struct udk_blk_mq_hw_ctx *hctx,
		int cpu, unsigned int flags);
};
...
struct udk_request_queue {
	...
	/* we use freeze lock to protect q's freeze/active depth and q's state */
	struct udk_mutex freeze_lock;
	vatomic32_t freeze_depth;
	vatomic32_t active_depth;
	struct udk_wait_cond_s wait_cond;
...
extern void *sysmem_palloc(unsigned long size);
...
struct udk_request *udk_blk_mq_alloc_request(struct udk_request_queue *queue,
					     struct udk_bio *bio);
