...
{
	uint64_t hint = libdh_get_pbha_hint(flags);
	int ret = udkmgr_vmap(pfn_array, array_len, prot, hint, rvaddr);
	return -hmerrno2posix(ret);
}

int udk_vunmap(unsigned long va, unsigned long size)
{
	int ret = udkmgr_vunmap(va, size);
	return -hmerrno2posix(ret);
}
		   ...
		   unsigned int prot, unsigned int flags, uintptr_t *ret_vaddr)
{
	...
	uint64_t hint = libdh_iomem_map_hint(prot, flags);
	int ret = udkmgr_iomem_vmap_to_devhost(pfn_array, cnt, prot, hint, cnode_idx, ret_vaddr);
	return -hmerrno2posix(ret);
}
