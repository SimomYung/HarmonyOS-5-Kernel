...
static struct devhost_irq_info *devhost_irq_idr_remove(unsigned int linux_irq)
{
	...
	ret = idr_begin(&linux_irqs, false);
	if (ret < 0) {
		return irq_info;
	}
	uspinlock_lock(&g_idr_spinlock);
...
static int devhost_irq_handler_set_affinity(raw_thread_t thread, unsigned int idx)
{
	 ...
	 * if there exist isolate cpus, and devhost have no privilege to set affinity, and
	 * devhost will not run in isolate cpus, so it is ok to fail for set_affinity.
	 */
	err = hm_thread_sched_setaffinity(raw_thread_tid_of(thread), sizeof(affinity), &affinity);
	if (err != 0) {
		hm_warn("set affinity failed for dh-irq%u, err is %s\n", target_cpu, hmstrerror(err));
	}
...
int devhost_irq_setup(void)
{
	...
	if (err == E_HM_OK) {
		/* idr do not need rollback when other process fails */
		err = idr_init_nolock(&linux_irqs, config_cpu_nr);
	}
					      ...
					      raw_thread_t *server, unsigned long server_len)
{
	...
	unsigned int idx = __cpuset_first_cpu(info->thread_affinity);
	if (idx >= server_len) {
...
static int irq_actv_pool_init(void)
{
	int err = E_HM_OK;
...
int devhost_irq_setaffinity(unsigned int linux_irq, unsigned long affinity)
{
	...
	struct hm_irq_attribute irq_attr = {0};
	if (info == NULL) {
		dh_debug("no irqinfo, linux_irq=%u\n", linux_irq);
		return -hmerrno2posix(E_HM_INVAL);
	}
...
static int devhost_fetch_sgi_ref(int irq, unsigned int cpu, rref_t *rref_ptr,
				...
				unsigned int cpu)
{
	...
	/* rref is useless now, ignore ret value */
	(void)hm_ucap_reject(0, irq_rref);
	irq_sgi_rref[linux_irq + IRQ_PERCPU_NR * cpu] = ERR_TO_REF(E_HM_INVAL);
}
				...
				unsigned int cpu)
{
	...
	if (err == E_HM_OK) {
		err = devhost_fetch_sgi_ref(irq, cpu, &rref, &arg, &ret);
	}
	...
	if (err == E_HM_OK) {
		...
		err = devhost_register_sgi(rref, logic_irq,
					   ret.rlt.hwconf, ret.rlt.hwconf_len, cpu);
		if (err < 0) {
			devhost_sgi_irq_return(irq, rref, cpu);
		}
	}
...
int devhost_unregister_ipi(int irq, unsigned int cpu)
{
	...
	if (err == E_HM_OK) {
		logic_irq = irq + IRQ_PERCPU_NR * cpu;
		if (IS_REF_VALID(irq_sgi_rref[logic_irq])) {
			devhost_sgi_irq_return(irq, irq_sgi_rref[logic_irq], cpu);
		}
	}
...
{
	unsigned int logic_irq = irq + IRQ_PERCPU_NR * cpu;
	__cpuset_t target_set = __cpuset_empty;
	rref_t rref;
	int ret;

	if (irq < 0 || irq >= (int)IRQ_SGI_NR ||
	    logic_irq >= IRQ_PERCPU_NR * IRQ_MAX_CPU_NR) {
		return -hmerrno2posix(E_HM_NXIO);
	}

	rref = irq_sgi_rref[logic_irq];
	if (IS_REF_ERR(rref)) {
		dh_debug("the ipi irq has not been registered yet, irq=%u\n", logic_irq);
		return -hmerrno2posix(E_HM_INVAL);
	}

	target_set = __cpuset_add(target_set, cpu);
	ret = hm_irq_generate_sgi(rref, logic_irq, target_set, NULL, 0);
	if (ret < 0) {
		dh_warn("failed to smp call cpu, irq=%d, cpu=%u, err = %s\n",
			irq, cpu, hmstrerror(ret));
...
static unsigned long long g_its_base = 0ULL;
