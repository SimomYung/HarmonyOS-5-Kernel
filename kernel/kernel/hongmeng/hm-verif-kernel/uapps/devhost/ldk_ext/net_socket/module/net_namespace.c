...
static int devhost_net_alloc_new_ns(int nsid)
{
	...
	/* Use net_cookie to store the nsid,
	 * and ensure that the net_cookie of each net namespace is unique.
	 */
	atomic64_set(&net.net_cookie, (u64)nsid);
	new_ns = copy_net_ns(CLONE_NEWNET, user_ns, &net);
	if (IS_ERR(new_ns)) {
		return -EINVAL;
	}
