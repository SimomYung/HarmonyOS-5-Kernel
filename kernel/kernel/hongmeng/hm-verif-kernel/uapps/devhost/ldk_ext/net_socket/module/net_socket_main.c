...
static int devhost_net_socket_listen(void *devhost_socket, int backlog)
{
	...
	if ((unsigned int)kconn > (unsigned int)somaxconn) {
		kconn = somaxconn;
	}
			       ...
			       struct iovec **iov, int *p_flags)
{
	 ...
	 */
	*p_flags = *p_flags & ~MSG_CMSG_COMPAT;
				...
				struct msghdr *msghdr_sys, int flags)
{
	...
	msghdr_sys->msg_flags = (unsigned int)flags;
...
static inline int copy_msghdr_to_user(struct msghdr *msghdr_sys, struct user_msghdr *umsg, unsigned long cmsg_ptr,
					struct sockaddr_storage __user *uaddr, void *ucmsg, int ulen)
{
	...
	return liblinux_pal_copy_msghdr_to_user(umsg, &smsg, uaddr, ucmsg, ulen);
}
			    ...
			    struct msghdr *msghdr_sys, int flags)
{
	...
	if ((flags & MSG_CMSG_COMPAT)) {
		...
		rc = copy_msghdr_to_user(msghdr_sys, umsg, cmsg_ptr, uaddr, ucmsg, ulen);
	}
			    ...
			    size_t vlen, int flags, struct timespec64 *timeout)
{
	...
	while ((unsigned int)datagrams < vlen) {
