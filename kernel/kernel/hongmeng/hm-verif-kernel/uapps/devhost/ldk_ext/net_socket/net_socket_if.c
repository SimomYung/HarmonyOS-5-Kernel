...
static int do_unregister_file(unsigned int cnode_idx, int sockfd)
{
	struct socket_device_info *sock_dev = NULL;
	struct filp_node *node = NULL;
	...
	return devhost_unregister_file((uint64_t)cnode_idx, (unsigned int)sockfd);
}
...
int devhost_net_socket_directcall_close(unsigned int cnode_idx, int sockfd)
{
	...
	ret = do_unregister_file(cnode_idx, sockfd);
	backend_syscall_exit();
