...
#ifdef PAC_FWCFI_SYSSERVICE
extern void (*const __transfs_ro_after_init_hdlr_start []) (void);
extern void (*const __transfs_ro_after_init_hdlr_end []) (void);
#endif
...
int dh_extension_init(struct lnxbase_ctx *ctx)
{
...
#ifdef PAC_FWCFI_SYSSERVICE
	ext_hdlr_init((uintptr_t)&__transfs_ro_after_init_hdlr_start, (uintptr_t)(&__transfs_ro_after_init_hdlr_end));
#endif
	...
	ret = lnxbase_register_inode(devhost_transfs_inode_permission);
	if (ret < 0) {
		hm_error("register transfs inode permission ops failed, %d\n", ret);
		return ret;
	}
