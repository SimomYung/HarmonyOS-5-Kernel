...
static int get_d_index_by_dentry(unsigned int sb_index, struct dentry *dentry,
				...
				void *dentry, unsigned int *d_index)
{
	...
	for (i = 0; i < g_trans_sb_info.sb_tables_size; i++) {
		if (g_trans_sb_info.sb_tables[i].sb == sb &&
		    g_trans_sb_info.sb_tables[i].s_reverse_query) {
			ret = get_d_index_by_dentry(i, dentry, d_index);
			if (ret == E_HM_OK) {
				*sb_index = i;
				break;
			}
