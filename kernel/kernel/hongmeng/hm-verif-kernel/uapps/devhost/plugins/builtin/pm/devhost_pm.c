...
int devhost_pm_add_device(struct device_info *dev)
{
	...
	hm_info("device[%d:%s] register to powermgr\n", dev->devid,
		(dev->devname != NULL) ? dev->devname : "");
...
int devhost_pm_remove_device(const struct device_info *dev)
{
	...
	dlist_for_each_entry_safe(pos, n, &dev_pm_list,
		...
		if ((pos != NULL) && (pos->dev == dev)) {
			hm_info("device[%d:%s] unregister from powermgr\n", dev->devid,
				(dev->devname != NULL) ? dev->devname : "");
...
static int devices_shutdown(void)
{
	...
	while (&(pos->node) != &dev_pm_list) {
		...
		if (dev != NULL) {
			hm_info("device[%d:%s] will shutdown\n", dev->devid,
				(dev->devname != NULL) ? dev->devname : "");
		}
		/*
...
static void platform_proc_freeze(void)
{
	if ((devhost_plat_pm_ops != NULL) &&
	    (devhost_plat_pm_ops->proc_freeze != NULL)) {
		devhost_plat_pm_ops->proc_freeze();
	}
}

static void platform_proc_thaw(void)
{
	if ((devhost_plat_pm_ops != NULL) &&
	    (devhost_plat_pm_ops->proc_thaw != NULL)) {
		devhost_plat_pm_ops->proc_thaw();
	}
}
...
int devhost_pm_action(unsigned int action)
{
	...
	case DH_PM_IDLE_EXIT:
		...
		break;
#if defined(CONFIG_PM_SUSPEND) || defined(CONFIG_PM_HIBERNATE)
	case DH_PM_PROC_FREEZE:
		platform_proc_freeze();
		break;
	case DH_PM_PROC_THAW:
		platform_proc_thaw();
		break;
