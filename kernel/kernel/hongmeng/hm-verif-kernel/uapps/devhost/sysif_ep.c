...
#define DECLARE_DEVHOST_SYSIF_EP_ALS_HDLR(method, ...)	\
	DECLARE_SYSIF_EP_CUSTOM_HDLR(DEFINE_SYSCALL_BASIC_NO_EXIT_HDLR, drvhandler_devhost,	\
				     method, devhost, int, ##__VA_ARGS__)
...
/* if use no_enter sysif definition, it need custom implementation to do same initialization work as liblinux_enter */
...
#define DECLARE_DEVHOST_SYSIF_EP_SYSCALL_BASIC_HDLR(method, ...)	\
	DECLARE_SYSIF_EP_CUSTOM_HDLR(DEFINE_SYSCALL_BASIC_HDLR, drvhandler_devhost, method, devhost, int, ##__VA_ARGS__)
...
DECLARE_DEVHOST_SYSIF_EP_SYSCALL_BASIC_HDLR(process_revoke,
				  unsigned long long, sender, unsigned long, credential,
...
DECLARE_DEVHOST_SYSIF_EP_SYSCALL_HDLR(file_info_query_gpu,
				      unsigned long long, sender, unsigned long, credential,
				      unsigned long, pid);
...
DECLARE_DEVHOST_SYSIF_EP_SYSCALL_HDLR(test_pac_fwcfi_fault_inject,
				      unsigned long long, sender, unsigned long, credential);
