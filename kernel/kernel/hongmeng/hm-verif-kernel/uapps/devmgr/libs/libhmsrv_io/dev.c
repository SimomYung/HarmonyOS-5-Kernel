...
int hmsrv_io_dev_file_get(const struct hmsrv_io_ctx *ctx)
{
	...
	if (ctx->dh_cnode_idx == hm_ucap_self_cnode_idx()) {
		return libdh_io_dev_file_get(ctx);
	}
...
int hmsrv_io_shrinker_query(rref_t rref, int flag, bool try_drvcall)
{
	int ret = 0;

	ret = libdh_shrinker_query(rref, flag);
	/*
	 * if return E_HM_NODEV, it should use original path
	 */
	if (ret != E_HM_NODEV) {
		return ret;
	}
	...
	if (try_drvcall) {
		return actvcapcall_drvcall_devhost_shrinker_query(rref, flag);
	} else {
		return 0;
	}
}

int hmsrv_io_shrinker_shrink(rref_t rref, int max_nr, int *ret_nr)
{
	int err = E_HM_OK;
	int ret = 0;
	struct __actvret_drvcall_devhost_shrinker_shrink pret;

	err = libdh_shrinker_shrink(rref, max_nr, &ret);
	/*
	 * if return E_HM_NODEV, it should use original path
	 */
	if (err != E_HM_NODEV) {
	...
	err = actvcapcall_drvcall_devhost_shrinker_shrink(rref, max_nr, &pret);
	if (err == E_HM_OK) {
		if (ret_nr != NULL) {
			*ret_nr += pret.cnt;
		}
