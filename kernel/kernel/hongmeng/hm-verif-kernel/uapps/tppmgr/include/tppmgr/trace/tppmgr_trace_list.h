...
TRACE_EVENT(pid_freq, thermal_pid_modify_target_load,
	TP_PROTO(
		unsigned int, cpu,
		int, shell_back_thermal,
		int, target_load_origin,
		int, offset,
		int, target_load_final
	),
	TP_ARGS(cpu, shell_back_thermal, target_load_origin, offset, target_load_final),
	TP_FIELD(
		__field(unsigned int, cpu)
		__field(int, shell_back_thermal)
		__field(int, target_load_origin)
		__field(int, offset)
		__field(int, target_load_final)
	),
	TP_ASSIGN(
		__entry->cpu = cpu;
		__entry->shell_back_thermal = shell_back_thermal;
		__entry->target_load_origin = target_load_origin;
		__entry->offset = offset;
		__entry->target_load_final = target_load_final;
	),
	TP_PRINT("cpu=%u shell_back_thermal=%d target_load_origin=%d offset=%d final=%d",
		__entry->cpu, __entry->shell_back_thermal,
		__entry->target_load_origin, __entry->offset, __entry->target_load_final
	)
)
