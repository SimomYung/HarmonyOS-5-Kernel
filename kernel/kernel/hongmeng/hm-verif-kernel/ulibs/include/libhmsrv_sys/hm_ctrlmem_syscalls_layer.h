...
static inline int sysctrlmem_kobjctnr_spawn_copgtable_vspace(struct __ctrlmem_kobjctnr_page_freelist_s *fl,
						   void *pages_linear_va,
						   struct __ctrlmem_kobj_pcache_s *pcache,
						   struct __ctrlmem_kobj_vspace_s *main_kobjvs)
{
	return sysctrlmem_v32(__CTRLMEM_SYSCALL_UNKNOWN, &fl->head, __CTRLMEM_KOBJCTNR_META_KOBJ_VSPACE,
			      __sysctrlmem_arg_ptr(pages_linear_va),
			      __sysctrlmem_arg_ulong(__CTRLMEM_KOBJ_VSPACE_CTOR_FLAG_COPGTABLEVS),
			      __sysctrlmem_arg_ptr(pcache),
				  __sysctrlmem_arg_ptr(main_kobjvs));
}
...
static inline int sysctrlmem_kobjctnr_spawn_actv_binder_context(struct __ctrlmem_kobjctnr_page_freelist_s *fl,
								unsigned int devno, unsigned int flags, cref_t actv_pool_cref)
{
	return sysctrlmem_v32(__CTRLMEM_SYSCALL_UNKNOWN, &fl->head, __CTRLMEM_KOBJCTNR_META_KOBJ_ACTV_BINDER_CONTEXT,
			      __sysctrlmem_arg_u32(devno), __sysctrlmem_arg_u32(flags), __sysctrlmem_arg_u64(actv_pool_cref));
}
...
#define UNMAP_RANGEOP_REAPING		8
static inline __always_inline
						   ...
						   unsigned int nrptes)
{
	...
	if (nrptes > __KOBJVS_RANGEOP_VAL_NRPTES_MASK) {
		...
		} else if (option & UNMAP_RANGEOP_REAPING) {
			opval = __kobjvs_rangeop_mkval(REAPING, __KOBJVS_RANGEOP_VAL_FLAGS_CHANNELPG, 0, nrptes);
		} else {
...
static inline __always_inline
int sysctrlmem_vspace_slot_substitute(__u64 *slot_cref, __u64 vs_cref)
{
	/* usage: slot->cref = XXX
	 * syscall implementation: kernel/ctrlmem/vspace/dispatch.c
	 */
	return sysctrlmem_v64(__CTRLMEM_SYSCALL_UNKNOWN, slot_cref, __sysctrlmem_arg_u64(vs_cref));
}

static inline __always_inline
int sysctrlmem_policydb_ctrl(void *ptr, __u32 op)
{
	return sysctrlmem_v64(__CTRLMEM_SYSCALL_UNKNOWN, ptr, __sysctrlmem_arg_u32(op));
}
