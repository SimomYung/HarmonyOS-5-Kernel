...
| NOPOPULATE     | IO_URING    | PA_4G          | RETRY      | ACTV_CACHE | VMUSER    | STATIC_SHM_IO| IOMM            |
+----------------+-------------+----------------+------------+------------+-----------+--------------+-----------------+
| 23             | 22          | 21             | 20         | 19         | 18        | 17           | 16              |
| FILE           | SHMM        | NOCOW          | NOREPLACE  | SYNC       | HUGETLB   | STACK        | NONBLOCK/RLIMIT |
+----------------+-------------+----------------+------------+------------+-----------+--------------+-----------------+
| 15             | 14          | 13             | 12         | 11         | 10        | 9            | 8               |
| POPULATE       | NORESERVE   | LOCKED         | JIT        | SAFE       | VM        | GAP          | GROWSDOWN       |
+----------------+-------------+----------------+------------+------------+-----------+--------------+-----------------+
| 7              | 6           | 5              | 4          | 3          | 2         | 1            | 0               |
| ACTV           | XPM         | ANON/ANONYMOUS | FIXED      | USEREXPTE  | PURGEABLE | PRIVATE      | SHARED          |
+----------------+-------------+----------------+------------+------------+-----------+--------------+-----------------+
...
The flags of bits [26, 31] in the above table are valid only when hugetlb is off.
*/
...
#define MFLAG_MAP_GROWSDOWN		(unsigned int)MAP_GROWSDOWN 		/* 0x100 mapping can grows down in specific situation */
 ...
 * Bits [26:31] encode the hugetlb page size.
 */
#define MFLAG_MAP_PURGEABLE		(unsigned int)0x4			/* 0x04 */
#define MFLAG_MAP_USEREXPTE		(unsigned int)0x8			/* 0x08 */
#define MFLAG_MAP_XPM			(unsigned int)0x40			/* 0x40 */
#define MAP_XPM				(unsigned int)MFLAG_MAP_XPM
#define MFLAG_MAP_ACTV			(unsigned int)0x80			/* 0x80 actv stack mmap */
#define MFLAG_MAP_GAP			(unsigned int)0x200			/* 0x200 insert random gap between unfixed mmap vregions */
#define MFLAG_MAP_VM			(unsigned int)0x400			/* 0x400 mmap for virtual machine */
...
/* BEGIN: flags conflicted with hugetlb reserved bits, used only when hugetlb is off */
...
/* END: flags conflicted with hugetlb reserved bits, used only when hugetlb is off */
 ...
 */
#define IOFAST_NODOWNGRADE               0x1U /* pages must be continuity and not downgrade.
						 callers must have fallback when alloc big order memory failed. */
...
#define IOFAST_NO_RSVCTG                 0x200U /* do not use reserve contiguous memory */
...
/*
 * flag for iomap type (0-15bit) & method (16-31bit)
 */
#define IOMAP_TYPE_DEFAULT	(1U << 0)
#define IOMAP_TYPE_DMA_BUF	(1U << 1)
#define IOMAP_TYPE_HVGR		(1U << 2)
#define IOMAP_ASYNC_UNMAP	(1U << 16)

#define IOMAP_DMA_BUF_ASYNC_UNMAP \
	(IOMAP_TYPE_DMA_BUF | IOMAP_ASYNC_UNMAP)

#define IOMAP_MASK \
	(IOMAP_TYPE_DEFAULT | IOMAP_TYPE_DMA_BUF | IOMAP_TYPE_HVGR | IOMAP_ASYNC_UNMAP)
