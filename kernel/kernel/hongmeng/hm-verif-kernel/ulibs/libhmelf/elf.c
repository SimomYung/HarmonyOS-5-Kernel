...
static int set_elf_header(struct elf_context *ctx)
{
	...
	len = elf_fop(ctx, read, ident, 0, sizeof(ident));
	if (len < 0) {
		err = (int)len;
		 ...
		 void *data, off_t offset, size_t length)
{
	...
	if (err == E_HM_OK) {
		...
		len = elf_fop(ctx, read, data, offset + (off_t)phdr->offset, size);
		if (len < 0) {
			err = (int)len;
			hm_warn("read elf segments failed: %s\n", hmstrerror(err));
		}
	}
		 ...
		 void *data, off_t offset, size_t length)
{
	...
	if (err == E_HM_OK) {
		...
		size = length > diff ? diff : length;
		len = elf_fop(ctx, read, data, offset + (off_t)shdr->offset, size);
		if (len < 0) {
			err = (int)len;
			hm_warn("read elf sections failed, err=%s\n", hmstrerror(err));
