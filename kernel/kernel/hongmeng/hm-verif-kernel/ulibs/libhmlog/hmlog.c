...
unsigned int linux_log_level_of_name(const char *name)
{
	...
	/*
	 * In linux situation, console will output level less than you input.
	 * So we should add 1 for each level in order to adapt with linux.
	 */
	if (strcmp(name, "DEBUG") == 0) {
		level = (unsigned int)LIBLINUX_LOGLEVEL_MAX;
...
unsigned int hmlog_level_of_name(const char *name)
{
	...
	if (strcmp(name, "DEBUG") == 0) {
		level = (unsigned int)HMLOG_DEBUG;
	} else if (strcmp(name, "INFO") == 0) {
		level = (unsigned int)HMLOG_INFO;
	} else if (strcmp(name, "WARN") == 0) {
		level = (unsigned int)HMLOG_WARN;
	} else if (strcmp(name, "ERROR") == 0) {
		level = (unsigned int)HMLOG_ERROR;
	} else if (strcmp(name, "PANIC") == 0) {
		level = (unsigned int)HMLOG_PANIC;
	} else {
		level = UINT_MAX;
	}
	return level;
