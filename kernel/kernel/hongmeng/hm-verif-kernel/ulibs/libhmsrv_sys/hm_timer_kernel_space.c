...
static raw_thread_t g_alarm_utimermgr_thread = NULL;
...
{
	int cnt = raw_refcnt_put(&(utimer->refcnt));
	BUG_ON(cnt < 0, "timer.utimermgr: put failed cnt=%d clk_id=%d clk_ori_id=%d type=%d "
		"owner_pid=%d owner_tid=%d owner_name=%s expire=%llu",
		cnt, utimer->clk_id, utimer->clk_ori_id, utimer->type, utimer->owner_pid, utimer->owner_tid,
		utimer->owner_name, utimer->expire);
}
...
static int utimer_thread_create(void)
{
	...
	g_alarm_utimermgr_thread = alarm_thread;
...
/*
 * Utimermgr-a should ensure that operation of settime is synchronous.
 * Otherwise cpu may enter S3 suspend before the last expired alarm utimer
 * futex_wait, which leads to missing this resume.
 */
static void utimer_alarm_sync_settime(void)
{
	int err = E_HM_OK;
	int cnt_stat_retry = UTIMER_STAT_RETRY;

	struct sysarg_thread_stat utimermgr_alarm_state = {0};

	while (err == E_HM_OK && cnt_stat_retry > 0) {
		err = hm_thread_stat(raw_thread_cref_of(g_alarm_utimermgr_thread),
				     &utimermgr_alarm_state,
				     THREAD_STAT_STATE | THREAD_STAT_BLOCK);
		if ((err == E_HM_OK) &&
		    (utimermgr_alarm_state.tcb_state == (__u32)__TCB_STATE_BLOCKED) &&
		    (utimermgr_alarm_state.tcb_block == (__u32)__TCB_BLOCK_FUTEX)) {
			break;
		}
	...
	if (err != E_HM_OK) {
		hm_error("timer.utimermgr: failed to get utimermgr-a thread state err=%s\n", hmstrerror(err));
	}
	if (cnt_stat_retry == 0) {
		hm_warn("timer.utimermgr: utimermgr-a is not blocked in futex, tcb_state %u, tcb_block %u\n",
				utimermgr_alarm_state.tcb_state, utimermgr_alarm_state.tcb_block);
	}
}
...
int utimer_settime_kernel(int timerid, int flags, const struct itimerspec *value, struct itimerspec *old)
{
	...
	if (err == E_HM_OK) {
		if (utimer_pool == g_alarm_utimer_pool) {
			(void)utimer_alarm_sync_settime();
		}
	}
