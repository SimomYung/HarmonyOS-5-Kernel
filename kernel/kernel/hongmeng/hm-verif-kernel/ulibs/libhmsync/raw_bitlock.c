...
static inline int do_bitlock_wait(int *addr, unsigned int flags, int lock_bit, int wait_bit,
				  unsigned long long *timeout)
{
	...
	*/
	...
	flags |= (__FUTEX_FLAG_BITLOCK | FUTEX_BLOCKED_TIMEDOUT_DETECT);
...
/*
 * raw_bitlock_lock_timeout - use only two bits to realize a lock, return err if timeout
 * @addr: address of lock
 * @flags:  define locking behaviour (0 is default)
 * @lock_bit: which bit of addr means lock (1: someone takes lock, 0: nobody uses this lock)
 * @wait_bit: which bit of addr means wait, used to determine if need to wake up waiter in unlock
 * @timeout: longest waiting tocks, return E_HM_TIMEDOUT if lock wait timeout
 *
 * Return 0 when getting lock successfully
 * Return E_HM_xx when an error occurred
 *
 */
int raw_bitlock_lock_timeout(int *addr, unsigned int flags, int lock_bit, int wait_bit, unsigned long long *timeout)
{
	unsigned int old;
	int rc;
	...
	if (likely(!raw_bitlock_disabled())) {
		...
		while (old & (1U << lock_bit)) {
			...
			rc = do_bitlock_wait(addr, flags, lock_bit, wait_bit, timeout);
			if (rc == E_HM_TIMEDOUT) {
				return rc;
			}
...
int raw_bitlock_lock(int *addr, int lock_bit, int wait_bit)
{
	return raw_bitlock_lock_timeout(addr, 0U, lock_bit, wait_bit, NULL);
}
...
int raw_bitlock_spin_trylock(int *addr, int lock_bit)
{
	int old;
	int ret = E_HM_OK;
	/* disable preempt */
...
int raw_onebit_lock(int *addr, int lock_bit)
{
	...
	if (likely(!raw_bitlock_disabled())) {
		...
		/* origin bit is 1, means someone takes this bitlock. current thread needs to wait */
		while (old & (1U << lock_bit)) {
			(void)do_bitlock_wait(addr, 0U, lock_bit, INVALID_BITS_NUM, NULL);
			/* wakeup by previous bitlock owner, retry to acquire the bitlock. */
			old = vatomic32_get_or((vatomic32_t *)(uintptr_t)addr, 1U << lock_bit);
		}
	}
