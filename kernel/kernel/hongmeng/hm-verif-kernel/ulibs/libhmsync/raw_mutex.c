...
static inline __always_inline uint32_t owner_with_flags(struct raw_mutex *mutex)
{
	return (owner_of(mutex) | MUTEX_DIRECT_OWNER);
}
...
try_lock_recursive(struct raw_mutex *mutex)
{
	unsigned int old;
	int ret;
	unsigned int new = owner_with_flags(mutex);

	/*
	 * old owner: old & ~MUTEX_FLAGS
	 * new owner: new & ~MUTEX_FLAGS
	 * owner == 0:
	 ...
	 * owner == others: good
	 */
	old = vatomic32_cmpxchg_acq(&mutex->lock, 0, new);
	if (old == 0 || ((old ^ new) & ~MUTEX_FLAGS) == 0) {
...
static int raw_mutex_wait_lock(struct raw_mutex *mutex, unsigned int flags,
			       unsigned long pc, unsigned long xdep_bitset)
{
	uint32_t r, v, old;
	int ret;
	...
	while ((ret = try_lock(mutex)) == E_HM_BUSY) {
		...
		ret = raw_futex_wait_ex(ptr_to_type(&mutex->lock, int *), (int)v,
					flags, FUTEX_BITSET_MATCH_ANY, xdep_bitset, 0UL,
					&block_param);
		raw_mutex_dec_waiters(mutex);
				      ...
				      unsigned long pc)
{
...
#ifdef CONFIG_XDEP_RAW_MUTEX
	if (shared_flag_of(mutex) == 0) {
		flags |= __FUTEX_BLOCKED_LOCK_XDEP;
		xdep_bitset |= __FUTEX_BITSET2_XDEP_MUTEX;
	}
#endif
	...
	return raw_mutex_wait_lock(mutex, flags, pc, xdep_bitset);
}
...
static inline __always_inline bool raw_mutex_lock_fast(struct raw_mutex *mutex)
{
	...
	if ((type_of(mutex) & RAW_MUTEX_TYPE_MASK) == 0) {
		ret = (bool)(vatomic32_cmpxchg_acq(&mutex->lock, 0, new) == 0);
	}
	return ret;
}
...
void raw_mutex_lock_yield(struct raw_mutex *mutex, int waiters)
{
	if ((type_of(mutex) & RAW_MUTEX_TYPE_MASK) == 0) {
		...
		if (!(vatomic32_cmpxchg_acq(&mutex->lock, 0, owner_with_flags(mutex)) == 0)) {
			raw_mutex_lock_slowpath(mutex);
		}
	} else {
...
{
	unsigned int value = owner_with_flags(mutex);
	bool ret = false;

	if ((type_of(mutex) & RAW_MUTEX_TYPE_MASK) == 0 && waiters_of(mutex) == 0) {
		ret = (bool)(vatomic32_cmpxchg_rel(&mutex->lock, value, 0) == value);
	}
...
static void __noinline raw_mutex_unlock_slowpath(struct raw_mutex *mutex)
{
	...
	if (!ret) {
		...
		if (cnt == 0) {
			...
			if (waiters != 0 || (val & MUTEX_HAS_WAITERS) != 0) {
