...
#ifdef CONFIG_RAW_RWLOCK_RDLOCKOWNER
	/* the first avaiable index in rdlock array, and the range is [0, MAXNUM_OF_RWLOCK] */
	unsigned int rdlock_free_idx;
	/*
	 * when thread rdlock A, the first available node in rdlock array
	 * will be allocated and be inserted in A.rd_owner_list;
	 *
	 * when thread unlock A, the corresponding node in rdlocks
	 * will be deleted from A.rd_owner_list and be free to rdlock array.
	 */
	struct dlist_node rdlock[MAXNUM_OF_RWLOCK];
#endif
...
#ifdef CONFIG_RAW_RWLOCK_RDLOCKOWNER
static inline void raw_thread_rdlock_lockowner_init(struct raw_thread *thread)
{
	thread->rdlock_free_idx = 0;
	for (unsigned int i = 0; i < MAXNUM_OF_RWLOCK; i++) {
		dlist_init(&thread->rdlock[i]);
	}
}

static inline bool raw_thread_rdlock_has_free_owner_node(struct raw_thread *thread)
{
	BUG_ON_D(thread->rdlock_free_idx > MAXNUM_OF_RWLOCK);
	return thread->rdlock_free_idx < MAXNUM_OF_RWLOCK;
}

static inline struct dlist_node *raw_thread_rdlock_alloc_free_owner_node(struct raw_thread *thread)
{
	return &thread->rdlock[thread->rdlock_free_idx++];
}

static inline struct dlist_node *raw_thread_rdlock_current_using_owner_node(struct raw_thread *thread)
{
	return (thread->rdlock_free_idx == 0U ? NULL : &thread->rdlock[thread->rdlock_free_idx - 1U]);
}

static inline void raw_thread_rdlock_free_current_owner_node(struct raw_thread *thread)
{
	while (thread->rdlock_free_idx > 0 && dlist_empty(&thread->rdlock[thread->rdlock_free_idx - 1U])) {
		thread->rdlock_free_idx--;
	}
}

#define RAW_THREAD_RDLOCK_ALL(thread, cur_node)							\
	for(unsigned int _idx = 0U;								\
	    (bool)((cur_node) = (_idx < MAXNUM_OF_RWLOCK ? &(thread)->rdlock[_idx] : NULL));	\
	    ++_idx)

#define RAW_THREAD_RDLOCK_EACH_PREV(thread, cur_node)					\
	for(unsigned int _idx = (thread)->rdlock_free_idx;				\
	    (bool)((cur_node) = (_idx > 0U ? &(thread)->rdlock[_idx - 1U] : NULL));	\
	    --_idx)

static inline void raw_thread_rdlock_print_node(struct raw_thread *thread, const char info[])
{
	struct dlist_node *next_node = NULL;

	hm_warn("%s, thread:%p, idx:%u\n", info, thread, thread->rdlock_free_idx);
	RAW_THREAD_RDLOCK_ALL(thread, next_node) {
		hm_warn("thread record: node[%p] prev[%p] next[%p]\n",
			next_node, next_node->prev, next_node->next);
	}
}

static inline uintptr_t raw_thread_rdlock_cal_tls(struct dlist_node *node, unsigned int index)
{
	unsigned int i = (index < MAXNUM_OF_RWLOCK ? index : MAXNUM_OF_RWLOCK -1);
	struct raw_thread *thread = container_of(node, struct raw_thread, rdlock[i]);
	return (uintptr_t)THREAD_TO_TLS(thread);
}
#else
static inline void raw_thread_rdlock_lockowner_init(const struct raw_thread *thread)
{
	UNUSED(thread);
}
#endif
...
static inline unsigned long raw_get_and_check_tls(void)
{
	unsigned long val = raw_get_tls();
#if !defined(__HOST_LLT__) && defined(CONFIG_DEBUG_BUILD)
	if (unlikely((val != 0UL) && (plibc != NULL) &&
		     (!IS_ALIGNED((uintptr_t)val, plibc->tls_align)))) {
		__builtin_trap();
	}
#endif
	return val;
