...
static int g_memm_slab_initialized __ro_after_init = 0;
...
uint32_t memm_slab_get_slab_size_with_idx(uint32_t idx)
{
	if (idx >= MEMM_ROOT_SLAB_ATOR_NUM) {
		hm_error("invalid idx=0x%x\n", idx);
		return 0;
	}
 
	return g_sized_array[idx];
}
...
struct slab_ator *memm_slab_ator_fetch(size_t size)
{
	struct slab_ator *ator = NULL;

	int index = check_and_find_index(size);
	if (index >= 0) {
		ator = &(g_sized_slab_ator[index]);
	}
...
size_t memm_slab_size_type(enum slab_size_type type)
{
	...
	slab_pool_lock(&g_memm_slab_pool);
	size_t ret = memm_slab_size_type_nolock(type);
	slab_pool_unlock(&g_memm_slab_pool);
	return ret;
}
...
uint32_t memm_slab_get_ator_active_obj_count(void)
{
	int i;
	uint32_t active_obj_count = 0;

	for (i = 0; i < MEMM_ROOT_SLAB_ATOR_NUM; i++) {
		active_obj_count += slab_get_ator_used_slot_num(&g_sized_slab_ator[i], SLAB_STORE_USER);
	}

	return active_obj_count;
}
...
bool memm_slab_is_inited(void)
{
	return g_memm_slab_initialized;
}
