...
#define CONFIG_SLAB_EMPTY_REMAIN_PAGE 3U
#endif

#define SLAB_EMPTY_REMAIN_PAGE CONFIG_SLAB_EMPTY_REMAIN_PAGE /* slab empty list will store number of page */
					...
					const struct slab_desc_range *s_range)
{
	...
	s_desc = slab_obj_to_slab_desc(ulong_to_ptr(slab_page_vaddr, const void), SLAB_PAGE_SIZE);
...
static void slab_desc_insert(struct slab_ator *s_ator, struct slab_desc *s_desc)
{
	...
	if (unlikely(vatomic32_read_rlx(&s_ator->slab_page_hold) == (unsigned int)INT_MAX)) {
		hm_warn("allocator[%s] slab page hold number reach to maximum, stop counting\n",
			 s_ator->name);
	} else {
		      ...
		      struct slab_pool *s_pool, uint32_t s_flags)
{
	...
	BUG_ON(ret != 0);
	s_ator->pool = s_pool;
	s_ator->max_slot_count = slab_page_usable_size(s_ator) / (s_ator->obj_size);
...
uint32_t slab_ator_flags_of(const struct slab_ator *ator)
{
	return (ator != NULL) ? ator->flags : 0;
}
