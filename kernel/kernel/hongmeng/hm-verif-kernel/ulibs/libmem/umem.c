...
/*
 * This function supports safe_copy only.
 * So it can be used only in the EL1 scenario.
 */
int hm_copy_from_user_nopf(void *dst_ori, const void *src_ori, size_t bytes)
{
	...
	if (enable_safe_copy()) {
		if ((ptr_to_ulong(src) < ((size_t)ULONG_MAX - bytes)) &&
		    (ptr_to_ulong(src) + bytes) < g_spin_priv_start) {
			/* safe_copy will return -EFAULT only if failed */
			ret = safe_copy_nopf_with_pan(dst, src, bytes);
			if (ret < 0) {
				ret = posix2hmerrno(-ret);
			}
		}
...
int hm_strcpy_from_user(void *dst_ori, const void *src_ori, size_t bytes)
{
	...
	if (!is_legal_copy_addr(dst_ori, bytes)) {
		hm_panic("Copying from user with illegal address!\n");
	}

	if (enable_safe_copy()) {
		if ((ptr_to_ulong(src_ori) < ((size_t)ULONG_MAX - bytes)) &&
			(ptr_to_ulong(src_ori) + bytes) < g_spin_priv_start) {
			/* safe_copy will return -EFAULT only if failed */
			ret = safe_strncpy_with_pan(dst_ori, src_ori, bytes);
			if (ret < 0) {
				ret = posix2hmerrno(-ret);
			}
		}
	...
	if (ret != E_HM_OK) {
		ret = hm_actv_strcpy_xact_vm(dst_ori, src_ori, bytes);
	}
	return ret;
}
