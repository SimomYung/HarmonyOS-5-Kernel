...
#include <hmkernel/track_info/track_info_list.h>
#undef DEFINE_TRACK_INFO
#undef TRACK_PARSE_GEN
#undef TRACK_INFO_PARSE
...
track_info_slot_header_of(struct track_info_header_s *info_header, bool dump_active)
{
	unsigned int slot_id = info_header->curr_slot;

	if (!dump_active) {
		slot_id = slot_id == SLOT_ID_A ? SLOT_ID_B : SLOT_ID_A;
	}
...
hm_track_info_slot_header_of(void *track_info_ptr, size_t size, bool dump_active)
{
	...
	if (track_info_ptr != NULL) {
		...
		if (!is_track_info_header_invalid(info_header, track_info_size)) {
			slot_header = track_info_slot_header_of(info_header, dump_active);
...
int hm_track_info_set(bool pause)
{
	...
	err = actvcall_hmcall_sysctrl_track_info_set(pause);
...
hm_track_info_slot_header_of(void *track_info_ptr, size_t size, bool dump_active)
{
	UNUSED(track_info_ptr, size, dump_active);
	return NULL;
}
