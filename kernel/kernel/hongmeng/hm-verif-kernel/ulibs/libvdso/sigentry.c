...
static void vdso_save_fpsimd(ucontext_t *ctx, size_t *used_size_ptr)
{
	...
	*used_size_ptr += sizeof(struct fpsimd_context);
}
...
static void vdso_save_esr(const ucontext_t *ctx, __u64 fault_code, size_t *used_size_ptr)
{
	// The reserved field signals the end of context data by setting the magic and size fields to 0.
	size_t reserved_len = sizeof(struct _aarch64_ctx);
	if (*used_size_ptr > sizeof(ctx->uc_mcontext.__reserved) - sizeof(struct esr_context) - reserved_len) {
		// no enough size for esr_context
		return;
	}
	struct esr_context *esr_ctx = (struct esr_context *)((uint8_t *)(&ctx->uc_mcontext.__reserved) + *used_size_ptr);
	esr_ctx->head.magic = ESR_MAGIC;
	esr_ctx->head.size = (unsigned int)sizeof(struct esr_context);
	esr_ctx->esr = fault_code;
	*used_size_ptr += sizeof(struct esr_context);
}
...
static void vdso_save_fpsimd(ucontext_t *ctx, size_t *used_size_ptr)
{
	...
	*used_size_ptr += sizeof(struct vfp_sigframe);
}
...
static void vdso_convert_to_siginfo_t(siginfo_t *dst, const struct __siginfo *src)
{
	...
	if (src->si_type == __SI_RT) {
		dst->si_value.sival_ptr = (void *)(uintptr_t)src->__sigrt.sig_value.sival_ptr;
		dst->si_pid = src->__sigrt.pid;
		dst->si_uid = src->__sigrt.uid;
...
void vdso_sigentry(struct sigframe *frame)
{
	...
	if ((frame->action.flags & UNI_SA_NOFPSIMD) == 0) {
		vdso_save_fpsimd(&p.ctx, &used_size);
	}

// Only ARM 64 have ESR Context
#if defined(__aarch64__)
	if (((frame->action.flags & SA_SIGINFO) != 0) && (frame->siginfo.si_type == __SI_FAULT) &&
	(frame->siginfo.si_fault_type_indicator == 0)) {
		vdso_save_esr(&p.ctx, frame->siginfo.__fault.fault_code, &used_size);
	}
#endif
