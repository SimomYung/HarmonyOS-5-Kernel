...
struct vhost_client_s *vhost_new_client(uint32_t pid, uint32_t virtio_id,
					uint32_t device_id,
		   ...
		   uint64_t mmio_base, uint32_t irq, uint32_t mmio_size)
{
	...
	if (c == NULL) {
		...
		ret = vhost_grant_vm_request(c, c->self_pid, vm_cref, &vm_rref);
		if (ret == E_HM_OK) {
