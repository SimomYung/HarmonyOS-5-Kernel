				  ...
				  struct vhost_backend_s *hook, uint32_t cnode_idx)
{
	if (hook != NULL) {
		...
		for (j = 0; j < VHOST_SERVER_MAX_DEVICE_NUM; ++j) {
			...
			if (cnode_idx != 0 &&
				(uint32_t)hm_pid_of_cnode_idx(cnode_idx) != device->pid) {
				continue;
			}
				     ...
				     struct vhost_user_msg_s *rsp)
{
	...
	if (ret == E_HM_OK) {
		ret = hm_get_cnode_idx(req->u.grant_vm_request.vm_pid, &uvmm_cnode_idx);
	}

	if (ret == E_HM_OK) {
		vm_rref = hm_ucap_require(0, uvmm_cnode_idx, 0);
...
int vhost_server_handle_msg(struct vhost_server_s *server, uint32_t pid,
			    const struct vhost_user_msg_s *req, struct vhost_user_msg_s *rsp)
{
	...
	if (ret == E_HM_OK) {
		device = get_or_alloc_vhost_device(hook, pid,
						   req->device_id);
		if (device == NULL) {
			ret = E_HM_NOMEM;
		}
