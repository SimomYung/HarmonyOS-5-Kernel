 ...
 * <asm/pointer_auth_context.h> and <asm/ptrace.h>.  Deferring this #include
 * ensures that contents of ptrace.h are visible to pointer_auth_context.h.
 */
#ifdef CONFIG_ARM64_PTR_AUTH_DATA_FIELD
				     ...
				     unsigned long val)
{
	if (r != 31) {
#ifdef CONFIG_ARM64_PTR_AUTH_DATA_FIELD
		set_exception_context_register_index(regs, r, val);
#else
		regs->regs[r] = val;
#endif
	}
...
{
#ifdef CONFIG_ARM64_PTR_AUTH_DATA_FIELD
	set_exception_context_register(regs, REGS_PC, val);
#else
	regs->pc = val;
#endif
}
...
{
#ifdef CONFIG_ARM64_PTR_AUTH_DATA_FIELD
	set_exception_context_register_index(regs, 30, val);
#else
