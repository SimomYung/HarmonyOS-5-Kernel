...
struct cache_info_s {
	...
	int order;
	/*
	 * true: support shrink, use get_free_pages to allocate memory for buf;
	 * false: not support shrink, use vmalloc to allocate memory for buf;
	 */
	bool is_support_shrink;
};
...
struct {
	...
	{"/data/vendor/cust_conn/ini_cfg", false, false},
	{"/vendor/etc/cfg_*", true, false},
};
...
static void *__file_cache_buf_alloc(struct cache_info_s *cache_info, long size)
{
	int order = 0;
	void *buf = NULL;

	if (cache_info->is_support_shrink) {
...
static void __file_cache_buf_free(struct cache_info_s *cache_info)
{
	if (cache_info->buf == NULL)
		return;
			      ...
			      const char *name, long size, long ctime_sec)
{
	...
	cache_info->buf = __file_cache_buf_alloc(cache_info, size);
	if (!cache_info->buf) {
	...
	if (ret < 0) {
		...
		__file_cache_buf_free(cache_info);
		return ret;
	}
...
static struct cache_info_s *__file_cache_find(const char *name, bool is_support_shrink)
{
	...
	cache_info->order = -1;
	cache_info->is_support_shrink = is_support_shrink;
...
static bool __file_cache_is_allowed(const char *name, bool *is_support_shrink)
{
	bool allow = false;
	size_t len;
	...
	for (int i = 0; i < sizeof(file_cache_whitelist) / sizeof(file_cache_whitelist[0]); i++) {
		...
		if (strncmp(file_cache_whitelist[i].name, name, len) == 0) {
			allow = true;
			if (is_support_shrink != NULL) {
				*is_support_shrink = file_cache_whitelist[i].is_support_shrink;
			}
			break;
		}
	}
...
static void __file_cache_check(struct plat_file *fp, const char *name, struct stat *stat)
{
	int ret = 0;
	struct cache_info_s *cache_info;
	bool is_support_shrink = false;

	if (!__file_cache_is_allowed(name, &is_support_shrink))
		return;

	cache_info = __file_cache_find(name, is_support_shrink);
	if (!cache_info)
		return;
...
struct file __override *filp_open(const char *filename, int flags, umode_t mode)
{
	...
	if (filename == NULL) {
		pr_err("%s, filename is NULL\n", __func__);
		return ERR_PTR(-ENOENT);
	}

	fd = ksys_open(filename, flags, mode);
	if (fd < 0) {
		pr_debug("%s, open %s failed, flags:%d, mode:0x%x, fd=%d\n", __func__,
	...
	if (ret != 0) {
		pr_err("%s, stat %s failed, flags:%d, mode:0x%x\n", __func__,
		       filename, flags, mode);
		ksys_close(fd);
		return ERR_PTR(ret);
	}
	...
	if (IS_ERR_OR_NULL(filp)) {
		pr_err("%s, %s open_with_noaccount failed\n", __func__, filename);
		kfree(fp);
		ksys_close(fd);
		return ERR_PTR(-ENOENT);
	}
	...
	if (tmp && *(tmp + 1) != '\0') {
		if (strncpy_s(fp->name, FILE_NAME_LEN, tmp + 1, FILE_NAME_LEN - 1)) {
			pr_err("%s, copy(%s) name failed\n", __func__, (tmp + 1));
			__filp_priv_close(filp);
			__fput_sync((filp));
			return ERR_PTR(-ENOENT);
		}
					      ...
					      struct shrink_control *sc)
{
	...
	list_for_each_entry(cache_info, &file_cache.info_list, list) {
		...
		if ((cache_info->is_support_shrink) && (cache_info->buf != NULL)) {
			order = cache_info->order;
			__file_cache_buf_free(cache_info);
			pages_freed += 1 << order;
			cache_info->size = 0;
					       ...
					       struct shrink_control *sc)
{
	...
	mutex_lock(&file_cache.list_lock);
	list_for_each_entry(cache_info, &file_cache.info_list, list) {
		if ((cache_info->is_support_shrink) && (cache_info->order >= 0))
