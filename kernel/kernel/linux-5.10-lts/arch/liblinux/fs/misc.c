...
EXPORT_SYMBOL(liblinux_ext_alloc_unused_fd);
...
/**
 * liblinux_ext_pidfd_fget - get a file by pid and fd,
 * and increase the refcnt. This function only support
 * to get fastpath file.
 */
struct file *liblinux_ext_pidfd_fget(int pid, unsigned int fd)
{
	if (pid < 0 || fd > (unsigned int)INT_MAX) {
		return NULL;
	}

	return (struct file *)liblinux_pal_pidfd_anon_fget(pid, (int)fd, __fget);
}
EXPORT_SYMBOL(liblinux_ext_pidfd_fget);
