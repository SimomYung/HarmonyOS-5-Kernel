...
static void release_chunk_data(struct rcu_head *head)
{
	struct chunk_data *cdata = container_of(head, struct chunk_data, rcu);
	struct page **pages = cdata->pages;
	const int nr_pages = pcpu_group_sizes[0] >> PAGE_SHIFT;
	int i;
...
static void pcpu_destroy_chunk(struct pcpu_chunk *chunk)
{
	...
	if (chunk->data) {
		...
		call_rcu(&cdata->rcu, release_chunk_data);
	}
