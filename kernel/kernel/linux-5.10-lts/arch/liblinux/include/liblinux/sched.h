...
static inline void __liblinux_ctx_enter(void *ret __attribute__((unused)))
{
	liblinux_ctx_enter();
}

#define liblinux_ctx_guard(type, ret) \
	typedef type data_t; \
	data_t (ret) __attribute__ ((cleanup(__liblinux_ctx_enter))) = {0}; \
	liblinux_ctx_leave();
