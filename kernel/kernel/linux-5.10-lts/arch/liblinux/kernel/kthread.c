...
static DEFINE_SPINLOCK(thread_join_lock);
...
static int thread_join_fn(void *_unused)
{
	...
	while (!kthread_should_stop()) {
		spin_lock(&thread_join_lock);
		list_replace_init(&thread_join_task_list, &del_list);
		spin_unlock(&thread_join_lock);
...
static void kthread_task_init(struct task_struct *p, liblinux_pal_thread_handle_t *handle)
{
	...
	/* Clear switch info before call kthread() */
	p->last_switch_time = TASK_STATE_INITIAL;
#endif
	raw_spin_lock_init(&p->pi_lock);
}
...
void __set_task_comm(struct task_struct *tsk, const char *buf, bool exec)
{
	...
	/* Set name before acquire spinlock to avoid possible block within spin critical region */
...
static void try_cache_worker(void *data, struct kthread_worker *worker)
{
	...
	if (cached_worker_nr >= kthread_worker_caching_thresh) {
		...
		cached_worker_nr += 1;
		c_worker->cached = true;
		spin_unlock(&cached_worker_lock);
	}
}
INIT_VENDOR_HOOK(ldk_rvh_destroy_worker, try_cache_worker);

static void hook_free_kthread_worker(void *data, struct kthread_worker *worker)
{
	struct cached_worker *c_worker;
...
INIT_VENDOR_HOOK(ldk_rvh_free_kthread_worker, hook_free_kthread_worker);
...
static void hook_get_thread_name(void *args, pid_t tid, char *comm, int len)
{
	liblinux_pal_get_thread_name(tid, comm, len);
}
INIT_VENDOR_HOOK(ldk_rvh_get_thread_name, hook_get_thread_name);
