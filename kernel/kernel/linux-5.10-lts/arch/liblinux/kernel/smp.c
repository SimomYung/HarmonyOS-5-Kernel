...
struct smp_call_handler {
	...
	unsigned long call_start; /* Jiffies of the last call cpu started. */
	unsigned long call_end; /* Jiffies of the last call cpu ended. */
	int cpu;
};

DEFINE_PER_CPU_SHARED_ALIGNED(struct smp_call_handler, g_hdlr);

static inline void trace_call_cpu_begin(struct smp_call_handler *hdlr)
{
	hdlr->call_start = jiffies;
}

static inline void trace_call_cpu_end(struct smp_call_handler *hdlr)
{
	hdlr->call_end = jiffies;
}

static inline void trace_call_cpu_init(struct smp_call_handler *hdlr)
{
	hdlr->call_start = jiffies;
	hdlr->call_end = hdlr->call_start;
}

static int __send_call_function_single_ipi(int cpu)
{
	struct smp_call_handler *hdlr = per_cpu_ptr(&g_hdlr, cpu);
	trace_call_cpu_begin(hdlr);
	return liblinux_pal_smp_call_cpu(IPI_USER_CALL_FUNC, cpu);
}

void send_call_function_single_ipi(int cpu)
{
	__send_call_function_single_ipi(cpu);
}

/*
 * Call cpu retry when a CPU fails to respond or the time since the last call
 * exceeds 1 jiffies.
 */
#define CALL_CPU_TIMEOUT (1L)
static bool call_retry(struct smp_call_handler *hdlr)
{
	unsigned long call_start = hdlr->call_start;
	unsigned long call_end = hdlr->call_end;
	unsigned long spond_timeout = call_end + CALL_CPU_TIMEOUT;
	unsigned long send_timeout = call_start + CALL_CPU_TIMEOUT;
	unsigned long j = jiffies;

	if (time_before(call_start, spond_timeout) &&
	    time_before(j, send_timeout))
		return false;

	pr_warn_ratelimited("Re-sending IPI from CPU#%d to CPU#%d, respond/resend is %ld/%ld\n",
		raw_smp_processor_id(), hdlr->cpu, call_start - call_end, j - call_start);
	return true;
}
...
/* 5s */
#define SMPCALL_WAIT_TIMEOUT_US		(5 * USEC_PER_SEC)
...
static int csd_lock_wait_timeout(struct __call_single_data *csd, unsigned long timeout)
{
	unsigned long time_start = liblinux_pal_get_time_us();
	unsigned int flags;
	int ret = 0;

	for (;;) {
		flags = smp_load_acquire(&csd->flags);
		if (flags != CALL_SYNC) {
			break;
		}
		/* no mater if overflow: liblinux_get_time_us() < time_start */
		if ((liblinux_pal_get_time_us() - time_start) >= timeout) {
			ret = -ETIMEDOUT;
			break;
		}
		cpu_relax();
...
static void wait_csd(int cpu, struct __call_single_data *csd)
{
	...
	while (smp_load_acquire(&csd->flags) == CALL_SYNC) {
		ret = csd_lock_wait_timeout(csd, SMPCALL_WAIT_TIMEOUT_US);
		if (ret == -ETIMEDOUT) {
...
static inline int __call_cpu(int cpu, struct __call_single_data *csd, bool force)
{
	...
	if (llist_add(&csd->llist, &hdlr->list) || call_retry(hdlr) || force) {
		trace_call_cpu_begin(hdlr);
		ret = __send_call_function_single_ipi(cpu);
	}
...
static int call_cpu_async(int cpu, smp_call_func_t func, void *info, bool force)
{
	...
	csd->func = func;
	csd->info = info;
	return __call_cpu(cpu, csd, force);
}
...
static void __smp_call_function_many_async(const struct cpumask *cpu_mask, smp_call_func_t func, void *info, smp_cond_func_t cond_func)
{
	...
	for_each_cpu(cpu, cpu_mask) {
		if (cond_func && !cond_func(cpu, info))
			continue;
		call_cpu_async(cpu, func, info, false);
	}
}
...
int __smp_call_function_single(int cpu, smp_call_func_t func, void *info, int wait, bool force)
{
	int ret = 0;
	int this_cpu;
	...
	if (wait) {
		...
		call_sync_lock();
		smpcall_stat_syncreq_inc(cpu);
		ret = __call_cpu(cpu, &csd, force);
		wait_csd(cpu, &csd);
		call_sync_unlock();
	} else {
		ret = call_cpu_async(cpu, func, info, force);
	}
...
{
	return __smp_call_function_single(cpu, func, info, wait, false);
}

int smp_call_function_single_force(int cpu, smp_call_func_t func, void *info, int wait)
{
	return __smp_call_function_single(cpu, func, info, wait, true);
}
EXPORT_SYMBOL(smp_call_function_single_force);
...
static void hook_smp_call_cpu_dump(void *__data, int unuse)
{
	unsigned long j = jiffies;
	int cpu;
	...
	for_each_cpu(cpu, cpu_possible_mask) {
		struct smp_call_handler *hdlr = per_cpu_ptr(&g_hdlr, cpu);
		pr_warn("call CPU track: CPU-%d before snd-%lu rcv-%lu jiffies list_empty-%d\n",
			 cpu, j - hdlr->call_start, j - hdlr->call_end,
			 llist_empty(&hdlr->list));
	}
}
INIT_VENDOR_HOOK(ldk_rvh_smp_call_cpu_dump, hook_smp_call_cpu_dump);
