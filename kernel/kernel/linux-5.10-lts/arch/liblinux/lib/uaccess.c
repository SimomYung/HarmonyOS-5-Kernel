...
long strncpy_from_kernel_nofault(char *dst, const void *src, long count)
{
	long ret;
	/* check null-page */
	if ((dst < (void *)PAGE_SIZE) || (src < (void *)PAGE_SIZE)) {
	...
	ret = strncpy_s(dst, count, src, count);
	if (ret == 0) {
		dst[count - 1] = '\0';
	}
			       ...
			       long count)
{
	...
	if (get_fs() == USER_DS) {
		ret = liblinux_pal_strcpy_from_user(dst, unsafe_addr, count);
	} else {
		ret = strncpy_s(dst, count, unsafe_addr, count);
	}
