...
static void unmap_mapping_range_tree(struct rb_root_cached *root,
				     struct zap_details *details)
{
	struct vm_area_struct *vma;
	pgoff_t vma_ba, vma_ea, zap_ba, zap_ea;
	unsigned long start_addr, end_addr;

	vma_interval_tree_foreach(vma, root,
			details->first_index, details->last_index) {
		vma_ba = vma->vm_pgoff;
		vma_ea = vma_ba + vma_pages(vma) - 1;
		zap_ba = details->first_index;
		if (zap_ba < vma_ba)
			zap_ba = vma_ba;
		zap_ea = details->last_index;
		if (zap_ea > vma_ea)
			zap_ea = vma_ea;

		start_addr = ((zap_ba - vma_ba) << PAGE_SHIFT) + vma->vm_start;
		end_addr = ((zap_ea - vma_ba + 1) << PAGE_SHIFT) + vma->vm_start;
		liblinux_zap_vma_ptes(vma, start_addr, end_addr - start_addr);
	}
}
...
{
	struct zap_details detail = { };

	detail.first_index = start;
	detail.check_mapping = even_cows ? NULL : mapping;
	detail.last_index = start + nr - 1;
	if (detail.last_index < detail.first_index)
		detail.last_index = ULONG_MAX;

	i_mmap_lock_write(mapping);
	if (unlikely(!RB_EMPTY_ROOT(&mapping->i_mmap.rb_root)))
		unmap_mapping_range_tree(&mapping->i_mmap, &detail);
...
static void hook_charge_memlock(void *args, u32 pages, int errno, int *ret)
{
	*ret = liblinux_pal_charge_mlock(pages << PAGE_SHIFT);
	if (*ret)
		*ret = errno;
}
INIT_VENDOR_HOOK(ldk_vh_charge_memlock, hook_charge_memlock);

static void hook_uncharge_memlock(void *args, u32 pages)
{
	(void)liblinux_pal_uncharge_mlock(pages << PAGE_SHIFT);
}
INIT_VENDOR_HOOK(ldk_vh_uncharge_memlock, hook_uncharge_memlock);
