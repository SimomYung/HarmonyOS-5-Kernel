...
void liblinux_mm_prepare_unmap(void *va, unsigned long size)
{
	...
	liblinux_iofast_mm_unmap_time_stat(ktime_e - ktime_b);
	return;
}
...
static int page_alloc_seq_show(struct seq_file *s, void *d)
{
	page_time_stat_get(PAGE_ALLOC, s);
	return 0;
}

static ssize_t
page_alloc_write(struct file *file,
				const char __user * buffer,
				size_t count, loff_t * ppos)
{
	pr_warn("do not support write\n");
	return -ENOSYS;
}

static int page_alloc_open(struct inode *inode, struct file *file)
{
	return single_open(file, page_alloc_seq_show,
			   PDE_DATA(inode));
}

static const struct proc_ops page_alloc_proc_ops = {
	.proc_open	= page_alloc_open,
	.proc_read	= seq_read,
	.proc_write	= page_alloc_write,
...
{
	page_time_stat_init(PAGE_ALLOC);
	proc_create("page_alloc", S_IFREG | S_IRUGO | S_IWUSR,
			NULL, &page_alloc_proc_ops);

	return 0;
}
late_initcall(page_alloc_proc_init);
#endif

static void hook_alloc_pages_time_stat(void *args, unsigned int order, gfp_t gfp_mask, s64 used_us, struct page *page)
{
#ifdef CONFIG_LIBLINUX_PAGE_TIME_STAT
	page_order_time_stat_account(PAGE_ALLOC, order, used_us);
	page_total_time_stat_account(PAGE_ALLOC, used_us);
	if (page)
		page_succ_time_stat_account(PAGE_ALLOC, used_us);
#endif

	if ((gfp_mask & __GFP_DIRECT_RECLAIM) == 0) {
		if (used_us > 1000) {
			pr_warn_ratelimited("alloc page order:%u, gfp_mask:%#x, used_us:%lld, page is %s, caller: %ps take more than 1ms\n",
				order, gfp_mask, used_us, page ? "not null" : "null", __builtin_return_address(2));
		}
	} else {
		if (used_us > 2000) {
			pr_warn_ratelimited("alloc page order:%u, gfp_mask:%#x, used_us:%lld, page is %s, caller: %ps take more than 2ms\n",
				order, gfp_mask, used_us, page ? "not null" : "null", __builtin_return_address(2));
		}
	}
}
INIT_VENDOR_HOOK(ldk_rvh_alloc_pages_time_stat, hook_alloc_pages_time_stat);
