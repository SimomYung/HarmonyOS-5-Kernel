...
static bool is_kmap_addr(const void *x)
{
	...
	unsigned long k_end = k_start + LIBLINUX_KMAP_SIZE;
...
static void __vmalloc_init(void)
{
	int cpu;

	for_each_possible_cpu(cpu) {
		struct vfree_deferred *p;
		p = &per_cpu(vfree_deferred, cpu);
...
void vfree(const void *addr)
{
	...
	/*
	 * __vfree will actv call witch might sleep in HM.
	 * It is not allowed when preemption is disabled.
	 * So there's an extra check for in_atomic.
	 */
    ...
    unsigned long uaddr, void *kaddr, unsigned long pgoff, unsigned long size)
{
	...
	kaddr += off;
	return liblinux_pal_remap_vmalloc_range(VSPACE_KEY(vma->vm_mm), uaddr, (unsigned long)kaddr, size,
