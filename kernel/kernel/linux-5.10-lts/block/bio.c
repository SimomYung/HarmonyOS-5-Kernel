...
static void bio_free(struct bio *bio)
{
	...
	if (bio->ci_metadata) {
		kfree(bio->ci_metadata);
		bio->ci_metadata = NULL;
	}

	if (bio->bi_crypt_context) {
		if (bio->bi_crypt_context->bc_key) {
			kfree(bio->bi_crypt_context->bc_key);
			bio->bi_crypt_context->bc_key = NULL;
		}
...
#ifdef CONFIG_SCSI_UFS_INLINE_CRYPTO
#ifdef CONFIG_HISI_INLINE_ENCRYPTION
	mas_blk_inline_crypto_bio_split_post(bio, split->bi_iter.bi_size);
#else
