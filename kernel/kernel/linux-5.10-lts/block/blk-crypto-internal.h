...
#ifdef CONFIG_EXT4_INLINE_ENCRYPTION
static inline bool bio_crypt_ctx_back_mergeable(struct request *req,
						struct bio *bio)
{
	if (!bio_crypt_rq_ctx_compatible(req, bio))
		return false;
 
	return (req->bio->index + req->bio->bi_vcnt) == bio->index;
}
						 ...
						 struct bio *bio)
{
	...
	return (bio->index + bio->bi_vcnt) == req->bio->index;
}

static inline bool bio_crypt_ctx_merge_rq(struct request *req,
					  struct request *next)
{
	if (!bio_crypt_rq_ctx_compatible(req, next->bio))
		return false;
 
	return (req->bio->index + req->bio->bi_vcnt) == next->bio->index;
}
