...
#else
	if (platform_notify && action == KOBJ_ADD)
		ret = platform_notify(dev);
	else if (platform_notify_remove && action == KOBJ_REMOVE)
		ret = platform_notify_remove(dev);
	return ret;
#endif
}
			      ...
			      const char *buf, size_t count)
{
	...
	if (dev_attr->store) {
		...
		dev_attr->store = pauth_pacia(pauth_xpaci(dev_attr->store), __builtin_get_modifier_bytype( \
			ssize_t (*)(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)));
#endif
...
int device_add(struct device *dev)
{
	...
	trace_ldk_rvh_device_notify(dev, LIBLINUX_VH_DEVICE_ADD, &error);
	if (error) {
		if (MAJOR(dev->devt)) {
	...
	if (IS_ENABLED(CONFIG_LIBLINUX))
		trace_ldk_vh_kobject_add_uevent(dev, &dev->kobj);
	else
...
void device_del(struct device *dev)
{
	...
	trace_ldk_rvh_device_notify(dev, LIBLINUX_VH_DEVICE_DEL, NULL);
