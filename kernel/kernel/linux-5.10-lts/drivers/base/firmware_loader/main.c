					  ...
					  u32 opt_flags)
{
	...
	INIT_LIST_HEAD(&fw_priv->white_list);
#endif
...
static const char * const fw_path[] = {
	...
	"/lib/firmware",
	"/odm/etc/firmware",
	"/chip_prod/etc/firmware",
	"/chip_prod/etc/firmware/ts",
	"/vendor/firmware/ivp",
	"/vendor/firmware",
	"/system/vendor/firmware",
	"/vendor/hisi/dpa/firmware",
	"/vendor/communication/dpa",
	"/tmp",
};
...
#ifdef CONFIG_LIBLINUX
	trace_ldk_vh_fw_get_filesystem_firmware(device, fw_priv, suffix, decompress,
						fw_path, ARRAY_SIZE(fw_path), &rc);
	if (trace_ldk_vh_fw_get_filesystem_firmware_enabled())
		return rc;
...
#ifdef CONFIG_LIBLINUX
	if (fw)
		trace_ldk_rvh_request_firmware(fw->priv, free_fw_priv);
#endif /* CONFIG_LIBLINUX */
