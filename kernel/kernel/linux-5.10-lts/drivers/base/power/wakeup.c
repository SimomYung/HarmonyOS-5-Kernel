...
static void wakeup_source_activate(struct wakeup_source *ws)
{
	...
	trace_ldk_rvh_wakeup_source_activate(true, split_counters);
...
static void wakeup_source_deactivate(struct wakeup_source *ws)
{
	...
	trace_ldk_rvh_wakeup_source_activate(false, split_counters);
...
#ifdef CONFIG_POWER_DUBAI
void pm_get_active_wakeup_sources()
{
	size_t max = DUBAI_MAX_SUSPEND_ABORT_LEN;
	char pending_wakeup_source[DUBAI_MAX_SUSPEND_ABORT_LEN];
	...
	srcu_read_unlock(&wakeup_srcu, srcuidx);
	log_suspend_abort_reason(pending_wakeup_source);
	pr_info("PM: %s\n", pending_wakeup_source);
}
#endif
...
{
	unsigned long flags;
	char non_irq_wake_reason[DUBAI_MAX_SUSPEND_ABORT_LEN];
	va_list args;
	va_start(args, fmt);
	vsnprintf(non_irq_wake_reason, DUBAI_MAX_SUSPEND_ABORT_LEN, fmt, args);
...
#ifdef CONFIG_POWER_DUBAI
static const struct proc_ops wakeup_sources_stats_proc_ops = {
...
static int __init wakeup_sources_debugfs_init(void)
{
	...
	/*
	 * liblinux: Currently, the debugfs file creation is skipped to avoid
	 * conflict when creating the same file elsewhere. This will be removed
	 * once we develop a new method to keep track of ws stats.
	 */
	debugfs_create_file("wakeup_sources", S_IRUGO, NULL, NULL,
			    &wakeup_sources_stats_fops);
#endif /* !CONFIG_LIBLINUX */
	trace_ldk_vh_ws_debugfs_init(&deleted_ws);
	return 0;
}
...
static int __init wakeup_sources_proc_init(void)
{
	proc_create("wakeup_sources", S_IRUGO,
			(struct proc_dir_entry *)NULL, &wakeup_sources_stats_proc_ops);
	return 0;
}
/*lint -e528 -esym(528,*)*/
