 ...
 */
int32 plat_dump_rotate_cmd_read(struct file *file, uint64 arg)
{
    ...
    if (skb_queue_len(&g_dump_cmd_queue.dump_type_queue) == 0) {
        if (file->f_flags & O_NONBLOCK) {  /* if O_NONBLOCK read and return */
...
STATIC int64 hw_debug_ioctl(struct file *file, uint32 cmd, uint64 arg)
{
    ...
    switch (cmd) {
        ...
        case PLAT_DUMP_FILE_READ_CMD:
            plat_dump_rotate_cmd_read(file, arg);
            break;
...
#else
int32 plat_excp_dump_rotate_cmd_read(struct file *filp, uint64 arg, memdump_info_t *memdump_info)
{
...
int32 plat_bfgx_dump_rotate_cmd_read(struct file *filp, uint64 arg)
{
    return plat_excp_dump_rotate_cmd_read(filp, arg, &g_bcpu_memdump_cfg);
}

int32 plat_wifi_dump_rotate_cmd_read(struct file *filp, uint64 arg)
{
    return plat_excp_dump_rotate_cmd_read(filp, arg, &g_wcpu_memdump_cfg);
}
...
/*
 * Prototype    : hw_wifiexcp_poll
 * Description  : called by oam_hisi func from hal;
 *                check whether or not allow read and write
 */
STATIC uint32_t hw_bfgexcp_poll(struct file *filp, poll_table *wait)
{
    uint32_t mask = 0;
    ...
    memdump_info = &g_bcpu_memdump_cfg;
    /* push curr wait event to wait queue */
...
STATIC int64 hw_bfgexcp_ioctl(struct file *file, uint32 cmd, uint64 arg)
{
    ...
    switch (cmd) {
        case PLAT_BFGX_DUMP_FILE_READ_CMD:
            ret = plat_bfgx_dump_rotate_cmd_read(file, arg);
            break;
 ...
 *                check whether or not allow read and write
 */
STATIC uint32_t hw_wifiexcp_poll(struct file *filp, poll_table *wait)
{
    uint32_t mask = 0;
    memdump_info_t *memdump_info;
    ...
    memdump_info = &g_wcpu_memdump_cfg;
    /* push curr wait event to wait queue */
    poll_wait(filp, &memdump_info->dump_type_wait, wait);
...
STATIC int64 hw_wifiexcp_ioctl(struct file *file, uint32 cmd, uint64 arg)
{
    ...
    switch (cmd) {
        case PLAT_WIFI_DUMP_FILE_READ_CMD:
            ret = plat_wifi_dump_rotate_cmd_read(file, arg);
            break;
        default:
            ps_print_warning("hw_debug_ioctl cmd = %d not find\n", cmd);
            return -EINVAL;
...
STATIC uint32_t hw_debug_poll(struct file *filp, poll_table *wait)
{
    ...
    if (ps_core_d->rx_dbg_seq.qlen > 0) { /* have data to read */
        mask |= POLLIN | POLLRDNORM;
    }
...
STATIC ssize_t hw_debug_do_write(struct file *filp, const int8 __user *buf,
                              size_t count, loff_t *f_pos)
{
    struct ps_core_s *ps_core_d = NULL;
    struct sk_buff *skb = NULL;
