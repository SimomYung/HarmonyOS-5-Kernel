...
int32_t hw_ir_ctrl_set(int32_t power_on)
{
    ...
    if (g_board_info.have_ir == OAL_FALSE) {
        ps_print_info("board have no ir module");
        return OAL_SUCC;
    }

    ps_print_info("set ir ctrl mode as %d!\n", power_on);
    if ((power_on != OAL_TRUE) && (power_on != OAL_FALSE)) {
        ps_print_err("power ctrl param should be 0 or 1, cur value is [%d]\n", power_on);
        return -OAL_FAIL;
    }
    ...
    if (pm_data->board->irled_power_type == IR_GPIO_CTRL) {
        if (!is_asic()) {
            ps_print_info("HI1102 FPGA VERSION, ignore ir contral gpio\n");
            return OAL_SUCC;
        }
        gpio_level = (power_on == OAL_TRUE) ? GPIO_HIGHLEVEL : GPIO_LOWLEVEL;
        oal_gpio_direction_output(pm_data->board->bfgx_ir_ctrl_gpio, gpio_level);
