 ...
 */
static int32 chr_bfg_dev_tx(uint32 ul_errno)
{
    ...
    if (ret != CHR_SUCC) {
        chr_err("chr_bfg_dev_tx::prepare work fail, bring to reset work\n");
    ...
    if (skb == NULL) {
        chr_err("chr_bfg_dev_tx::alloc skbuff failed! len=%d, errno=0x%x\n", sk_len, ul_errno);
    ...
    *(uint32 *)(skb->data + sizeof(struct ps_packet_head)) = ul_errno;
    ...
    chr_warning("chr_bfg_dev_tx::tx is succ,errno %u\n", ul_errno);

    return CHR_SUCC;
}

static int32 chr_bfg_dev_tx_handler(uint32 errno)
{
