...
#ifdef _PRE_WLAN_EXPORT
    if ((!ini_file_exist(g_ini_file_name)) && (!ini_file_exist(g_ini_conn_file_name))) {
        ini_error("Cannot find ini_file:g_ini_file_name[%s] or g_ini_conn_file_name[%s]!!! Please Check!!!\n",
                  g_ini_file_name, g_ini_conn_file_name);
        return INI_FAILED;
    }
#endif
