...
#endif
#ifdef _PRE_WLAN_EXPORT
OAL_STATIC oal_void host_export_params_init_first(oal_void)
{
    g_host_init_params[WLAN_CFG_INIT_EFUSE_MAC_ADDR] = 0;
    g_host_init_params[WLAN_CFG_INIT_EFUSE_MAC_ADDR_2BYTE] = 0;
    g_host_init_params[WLAN_CFG_INIT_NB_POWER_ENABLE] = 1;
}
#endif
/*
...
static int32_t hwifi_config_get_cust_string(uint32_t cfg_id, uint8_t *ini_pa_params)
{
    uint32_t cfg_id_tmp;
    if (get_cust_conf_string(INI_MODU_WIFI, g_nvram_pro_line_config_ini[cfg_id].name,
        ini_pa_params, DY_CALI_PARAMS_LEN) == INI_FAILED) {
        if (oal_value_eq_any2(cfg_id, WLAN_CFG_DTS_NVRAM_RATIO_PA5GA0_BAND1_LOW, WLAN_CFG_DTS_NVRAM_RATIO_PA5GA0_LOW)) {
            cfg_id_tmp = ((cfg_id == WLAN_CFG_DTS_NVRAM_RATIO_PA5GA0_BAND1_LOW) ?
                            ...
                            WLAN_CFG_DTS_NVRAM_RATIO_PA5GA0 : cfg_id);
            ...
            OAM_ERROR_LOG1(0, OAM_SF_CUSTOM, "hwifi_config_init_dy_cali_custom read, check id[%d] exists!", cfg_id);
            return OAL_FAIL;
        }
    }
    return OAL_SUCC;
...
OAL_STATIC oal_uint32 hwifi_config_init_dy_cali_custom(oal_void)
{
    ...
    for (ul_cfg_id = WLAN_CFG_DTS_NVRAM_RATIO_PA2GCCKA0; ul_cfg_id < WLAN_CFG_DTS_NVRAM_PARAMS_BUTT; ul_cfg_id++) {
        /* 二次拟合系数 */
        if (hwifi_config_get_cust_string(ul_cfg_id, auc_ini_pa_params) != OAL_SUCC) {
            ul_ret = OAL_FAIL;
            break;
        }
