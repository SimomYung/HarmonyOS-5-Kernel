...
void hmac_config_connect_update_app_ie_to_vap(mac_vap_stru *mac_vap, mac_cfg80211_connect_param_stru *connect_param)
{
    ...
    if (hmac_config_set_app_ie_to_vap(mac_vap, &app_ie, app_ie.en_app_ie_type) != OAL_SUCC) {
        oam_warning_log0(mac_vap->uc_vap_id, 0, "{hmac_config_connect_update_app_ie_to_vap::set_app_ie_to_vap fail.}");
    }
}
...
oal_uint32 hmac_config_edca_param_set(mac_vap_stru *mac_vap, oal_uint16 len, oal_uint8 *param)
{
    oal_uint32 ret;
    ...
    /***************************************************************************
        抛事件到DMAC层, 同步DMAC数据
    ***************************************************************************/
    ret = hmac_config_send_event(mac_vap, WLAN_CFGID_EDCA_PARAM, len, param);
    if (oal_unlikely(ret != OAL_SUCC)) {
        OAM_WARNING_LOG1(mac_vap->uc_vap_id, OAM_SF_CFG, "{hmac_config_hitalk_key_set::send_event failed[%d].}",
                         ret);
    }
...
oal_uint32 hmac_config_cca_bypass_set(mac_vap_stru *mac_vap, oal_uint16 len, oal_uint8 *param)
{
    oal_uint32 ret;
    ...
    /***************************************************************************
        抛事件到DMAC层, 同步DMAC数据
    ***************************************************************************/
    ret = hmac_config_send_event(mac_vap, WLAN_CFGID_CCA_BYPASS, len, param);
    if (oal_unlikely(ret != OAL_SUCC)) {
        OAM_WARNING_LOG1(mac_vap->uc_vap_id, OAM_SF_CFG, "{hmac_config_cca_bypass_set::send_event failed[%d].}",
                         ret);
    }
