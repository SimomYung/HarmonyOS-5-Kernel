...
STATIC int32_t bfgx_comcpu_exception_test(struct ps_core_s *ps_core_d);
...
excp_dbg_cmd_t g_bfgx_excp_dbg[] = {
    {1, bfgx_comcpu_exception_test},      /* cmd: 1, trigger comcpu exception [bt:panic gnss:bus_error] */
...
STATIC int32_t bfgx_comcpu_exception_test(struct ps_core_s *ps_core_d)
{
    ...
    ps_print_info("[dfr_test]bfgx comcpu exception cause\n");
    ps_tx_sys_cmd(ps_core_d, SYS_MSG, SYS_CFG_COMCPU_EXCP);
...
STATIC int32_t dfr_sys_rst_uart_suspend_test(struct ps_core_s *ps_core_d)
{
    ...
    set_excp_test_en(UART_SUSPEND_FAULT, (uintptr_t)ps_core_d);
    return OAL_SUCC;
}
...
STATIC int32_t dfr_sys_rst_uart_suspend_clean(struct ps_core_s *ps_core_d)
{
    ...
    cancel_excp_test_en(UART_SUSPEND_FAULT, (uintptr_t)ps_core_d);
...
ssize_t bfgx_baudrate_show_comm(struct device *dev, char *buf)
{
    struct miscdevice *miscdev = dev_get_drvdata(dev);
    struct ps_core_s *ps_core_d = NULL;
    struct ps_plat_s *ps_plat_d = NULL;
    ...
    ps_print_info("core name is %s, use uart baud_rate %lu\n", ps_plat_d->uart_name, ps_plat_d->baud_rate);
    return snprintf_s(buf, PAGE_SIZE, PAGE_SIZE - 1, "%lu\n", ps_plat_d->baud_rate);
}

ssize_t bfgx_baudrate_store_comm(struct device *dev, const char *buf, size_t count)
{
    struct miscdevice *miscdev = dev_get_drvdata(dev);
    struct ps_core_s *ps_core_d = NULL;
    struct ps_plat_s *ps_plat_d = NULL;
    ...
    if (ps_core_chk_bfgx_active(ps_core_d) == true) {
        ps_print_err("must close current subsys\n");
        bfgx_print_subsys_state(ps_core_d->uart_index);
        return -EBUSY;
    }

    if (sscanf_s(buf, "%lu", &baud_rate) != 1) {
        ps_print_err("input subsys baud_rate error\n");
        return -EINVAL;
    }
    ps_print_info("baud_rate set to %lu\n", baud_rate);

    if ((baud_rate != UART_BAUD_RATE_DBG) && (baud_rate != UART_BAUD_RATE_4M) &&
        (baud_rate != UART_BAUD_RATE_6M) && (baud_rate != UART_BAUD_RATE_8M) &&
        (baud_rate != UART_BAUD_RATE_10M) && (baud_rate != UART_BAUD_RATE_20M)) {
        ps_print_err("baud_rate:%lu is illegal\n", baud_rate);
        return -EINVAL;
    }

    ps_plat_d->baud_rate = baud_rate;
    ps_print_info("dev %s baud_rate set to %lu succ\n", dev_name(miscdev->this_device), ps_plat_d->baud_rate);
