...
STATIC int32_t hw_bt_init(struct bfgx_dev_node *dev_node, struct ps_core_s *ps_core_d)
{
    ...
    if (dev_node->dev_data == NULL) {
        ...
        private_data = (struct bfgx_bt_private_data *)(dev_node->dev_data);
    }
...
ssize_t hw_bt_read(struct file *filp, char __user *buf, size_t count, loff_t *f_pos)
{
    ...
    ps_print_dbg("read want=%d\n", (int32_t)count);
    /* 关机找回状态下，防止BT进程未关闭，从BT队列读取数据 */
    if (off_find_get_status() == OAL_TRUE) {
        if (strcmp(dev_node->c.name, HW_OFF_FIND_DEV_NAME) != 0) {
            ps_print_warning("in off find status, not allow %s read\n", dev_node->c.name);
            return -EINVAL;
        }
    }
                                 ...
                                 struct ps_core_s *ps_core_d, struct bt_data_combination *data_comb)
{
    ...
    bfgx_pkt_dump(ps_core_d, BFGX_BT, PKG_DUMP_SEND, skb->data, total_len);
                             ...
                             struct bt_data_combination *data_comb)
{
    ...
    if (unlikely(hw_bfgx_write_check(buf, ps_core_d, BFGX_BT) < 0)) {
        return -EINVAL;
    }
...
ssize_t hw_bt_write(struct file *filp, const char __user *buf, size_t count, loff_t *f_pos)
{
    ...
    dev_node = oal_container_of((struct miscdevice *)filp->private_data, struct bfgx_dev_node, c);
    if (dev_node == NULL || dev_node->dev_data == NULL) {
        ps_print_err("dev_node private data null\n");
        return -EINVAL;
    }
    /* 关机找回状态下，防止BT进程未关闭，向BT队列写数据 */
    if (off_find_get_status() == OAL_TRUE) {
        if (strcmp(dev_node->c.name, HW_OFF_FIND_DEV_NAME) != 0) {
            ps_print_warning("in off find status, not allow %s write\n", dev_node->c.name);
            return -EINVAL;
        }
    }
...
uint32_t hw_bt_poll(struct file *filp, poll_table *wait)
{
    ...
    ps_print_dbg("bt rx qlen=%u\n", ps_core_d->bfgx_info[BFGX_BT].rx_queue.qlen);
...
STATIC void bt_fake_close_cmd(struct bfgx_dev_node *dev_node, unsigned long arg)
{
    struct ps_core_s *ps_core_d = dev_node->core;

    if (unlikely(ps_core_d == NULL)) {
        ps_print_err("ps_core_d is NULL\n");
        return;
    }
    ps_print_info("arg is %lu", arg);
...
long hw_bt_ioctl(struct file *filp, uint32_t cmd, unsigned long arg)
{
    ...
    if (unlikely((filp == NULL) || (filp->private_data == NULL))) {
        ps_print_err("%s filp is null", __func__);
        return -EINVAL;
    }
    ...
    ps_print_info("cmd is %d", cmd);
    /* hal下发cmd */
    switch (cmd) {
        case BT_IOCTL_OFF_FIND_CMD:
            off_find_cmd_parse(dev_node, (uint8_t OAL_USER *)arg);
            break;
        case BT_IOCTL_FAKE_CLOSE_CMD:
            bt_fake_close_cmd(dev_node, arg);
            break;
        default:
...
STATIC ssize_t baudrate_show(struct device *dev, struct device_attribute *attr, char *buf)
{
    return bfgx_baudrate_show_comm(dev, buf);
}

STATIC ssize_t baudrate_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
    return bfgx_baudrate_store_comm(dev, buf, count);
}
