...
/* 从ini文件中读取低功耗使能配置，如果没有配置项，则用default */
static uint32_t get_bfgx_pm_switch_ini(const char *ini_cfg, uint32_t default_value)
{
    int32_t pm_switch_ini = 0;
    int32_t ret;

    if (ini_cfg != NULL) {
        ret = get_cust_conf_int32(INI_MODU_PLAT, ini_cfg, &pm_switch_ini);
        if (ret == INI_FAILED) {
            ps_print_info("ini %s get fail, use default %d", ini_cfg, default_value);
            return default_value;
        }

        ps_print_info("ini defined %s = %d", ini_cfg, pm_switch_ini);
        return (uint32_t)pm_switch_ini;
    } else {
        ps_print_info("no bfgx pm_switch ini defined, use default %d", default_value);
        return default_value;
    }
}
...
static int32_t ps_plat_dts_init(struct ps_private_data *private_data, struct ps_plat_s *plat_d)
{
    ...
    plat_d->pm_switch = get_ps_u32_by_dts_label(private_data->of_node, dts_label->pm_switch,
                                                get_bfgx_pm_switch_ini(private_data->v.pm_switch_ini, 1));
...
#else
static int32_t ps_plat_config_by_private_data(struct ps_private_data *private_data, struct ps_plat_s *plat_d)
{
    if (private_data == NULL) {
        ps_print_info("private_data is NULL\n");
        return OAL_FAIL;
    }

    plat_d->uart_name = private_data->v.uart_name;
    plat_d->uart_index = private_data->v.uart_index;
    plat_d->baud_rate = get_legancy_baudrate(private_data->v.baud_rate);
    plat_d->host_wkup_io_type = BFGX_WKUP_HOST;
    plat_d->pm_switch = get_bfgx_pm_switch_ini(private_data->v.pm_switch_ini, 1);
    plat_d->flow_cntrl = private_data->v.flow_cntrl;
    plat_d->paren = get_ini_parity_en(private_data->v.paren);
...
#else
    private_data = (struct ps_private_data *)(pdev->dev.platform_data);
#endif
...
#else
    ret = ps_plat_config_by_private_data(private_data, ps_plat_d);
#endif
    if (ret < 0) {
...
static oal_bool_enum heart_timeout_need_retry(struct ps_core_s *ps_core_d)
{
    ...
    if (is_dfr_test_en(HEARTBEATER_TIMEOUT_FAULT, (uintptr_t)ps_core_d)) {
        ps_print_info("[%s]heartbeat dfr test, don't trigger panic by ssi\n", index2name(ps_core_d->uart_index));
        return OAL_FALSE;
    }
#endif
...
static struct ps_private_data bfgx_ps_private_data = {
    .v = {
        ...
        .pm_switch_ini = HW_BFGX_PM_SWITCH_0
    },
    .dts = {
...
static struct ps_private_data sle_ps_private_data = {
    .v = {
        ...
        .pm_switch_ini = HW_BFGX_PM_SWITCH_1
    },
    .dts = {
...
static int32_t bfg_dev_pm_prepare(struct device *dev)
{
    ...
    if (oal_wakelock_active(&(ps_plat_d->core_data->pm_data->bus_wake_lock)) == OAL_TRUE) {
        ps_print_info("[%s]- already wake up, break", pdev->name);
