...
int32_t g_pkt_dump_switch = 0;
oal_debug_module_param(g_pkt_dump_switch, int, S_IRUGO | S_IWUSR);

#define PKE_HEADER_ERR_THRESH   100
...
/*
 * Prototype    : bfgx_pkt_dump
 * Description  : dump the pkt content for debug
 * input        : @dir: 0:rcv; 1:send
 *                @subsys: service id
 *                @buf: will be dump pkt buff
 *                @len: will be dump pkt len
 *                @g_pkt_dump_switch:  dump pkt by mask switch, can be control by manual :
 *                  echo xxxx > /sys/module/plat_1105/parameters/g_pkt_dump_switch
 *                  mask value：
 *                      BT_DUMP_MSK = (1 << BFGX_BT)      // 0x1
 *                      FM_DUMP_MSK = (1 << BFGX_FM)      // 0x2
 *                      GNSS_DUMP_MSK = (1 << BFGX_GNSS)  // 0x4
 *                      IR_DUMP_MSK = (1 << BFGX_GNSS)    // 0x8
 *                      NFC_DUMP_MSK = (1 << BFGX_GNSS)   // 0x10
 *                      SLE_DUMP_MSK = (1 << BFGX_GNSS)   // 0x20
 *                      PLAT_DUMP_MSK = (1 << PLAT_SUBSYS) // 0x80
 */
void bfgx_pkt_dump(struct ps_core_s *ps_core_d, int32_t subsys, int32_t dir, const uint8_t *buf, uint16_t len)
{
#ifdef CONFIG_PRINTK
    uint32_t dump_msk = (1 << subsys);

    if (subsys > PLAT_SUBSYS) {
        return;
    }

    if ((g_pkt_dump_switch & dump_msk) != 0) {
        ps_print_info("[%s][%s]pkt_dump subsys[%d], len[%u]\n", index2name(ps_core_d->pm_data->index),
                      ((dir == PKG_DUMP_RCV) ? "rcv" : "send"), subsys, len);
        /* print to kenrel msg. rowsize = 32,groupsize = 1 */
        print_hex_dump(KERN_INFO, "pkt_data: ", DUMP_PREFIX_OFFSET, 32, 1,
                       buf, len, false); /* 内核函数固定传参 */
    }
#endif
}
...
int32_t ps_push_skb_queue(struct ps_core_s *ps_core_d, uint8_t *buf_ptr, uint16_t pkt_len, uint8_t type)
{
    ...
    if (pkt_len == 0) {
        ps_print_info("pkt_len is 0, skip it\n");
        return 0;
    }
...
STATIC int32_t ps_push_device_log_to_queue(struct ps_core_s *ps_core_d, const uint8_t *buf_ptr, uint16_t len)
{
    ...
    bfgx_pkt_dump(ps_core_d, PLAT_SUBSYS, PKG_DUMP_RCV, buf_ptr, len);
...
STATIC void report_last_word_to_dsm(struct ps_core_s *ps_core_d, om_exception_info_stru *oml_exce_info)
{
    ...
    ret = snprintf_s(buf, sizeof(buf), sizeof(buf) - 1,
                     "[%s]panic: type=0x%x reason=0x%x PC=0x%x LR=0x%x msp=0x%x bserpri=0x%x control=0x%x",
                     index2name(ps_core_d->uart_index), (uint32_t)oml_exce_info->fault_type,
                     (uint32_t)oml_exce_info->fault_reason, (uint32_t)oml_exce_info->pc_value,
                     (uint32_t)oml_exce_info->lr_value, (uint32_t)oml_exce_info->msp_value,
                     (uint32_t)oml_exce_info->bser_pri_value, (uint32_t)oml_exce_info->control_value);
    if (ret > 0) {
...
STATIC void ps_device_last_word_check_process(struct ps_core_s *ps_core_d, uint8_t *buf_ptr)
{
    ...
    if ((oml_exce_info->msg_header.frame_start != PACKET_START_SIGNAL) ||
        ...
        (oml_exce_info->msg_header.func_type != PACKET_RX_FUNC_SYSTEM_RESET)) ||
    ...
    if (oml_exce_info->msg_header.frame_len == rx_pkt_total_len) {
        ...
        /* 清除memdump_is_workink标记，防止上次memdump异常退出，导致本次memdump信息没有上报到oam_hisi */
        bfgx_memdump_finish(ps_core_d);

        /* 上报chr */
        report_last_word_to_dsm(ps_core_d, oml_exce_info);
        if (oml_exce_info->msg_header.func_type == PACKET_RX_FUNC_SYSTEM_RESET) {
            chr_exception_report(CHR_PLATFORM_EXCEPTION_EVENTID, CHR_SYSTEM_PLAT, CHR_LAYER_DRV,
                                 CHR_PLT_DRV_EVENT_DEV, CHR_PLAT_DRV_ERROR_BFG_DEV_SYSTEM_PANIC);
            plat_exception_handler(SUBSYS_BFGX, THREAD_IDLE, get_system_reset_excp_by_uart_idx(ps_core_d->uart_index));
        } else if (oml_exce_info->msg_header.func_type == PACKET_RX_FUNC_LAST_WORDS) {
            chr_exception_report(CHR_PLATFORM_EXCEPTION_EVENTID, CHR_SYSTEM_PLAT, CHR_LAYER_DRV,
...
STATIC int32_t ps_push_sepreated_data_to_queue(struct ps_core_s *ps_core_d, uint8_t subsys)
{
    ...
    bfgx_pkt_dump(ps_core_d, subsys, PKG_DUMP_RCV,
                  pst_sepreted_data->rx_buf_org_ptr, pst_sepreted_data->rx_buf_all_len);
    ...
    if (pst_bfgx_data->rx_queue.qlen >= g_bfgx_rx_queue_max_num[subsys]) {
        ps_print_warning("[%s]%s rx queue too large! qlen=%d\n", index2name(ps_core_d->pm_data->index),
                         pst_bfgx_data->name, pst_bfgx_data->rx_queue.qlen);
    ...
    if (ret < 0) {
        ps_print_err("[%s]push %s rx data to skb failed!\n",
                     index2name(ps_core_d->pm_data->index), pst_bfgx_data->name);
        return -EINVAL;
    }
...
STATIC int32_t ps_sepreated_data_recv(struct ps_core_s *ps_core_d, uint8_t *buf_ptr, uint8_t subsys)
{
    ...
    if (seq_correct != SEPRETED_RX_PKT_SEQ_CORRECT) {
        ps_print_err("[%s]%s rx seq is err! sepreted_type=%d, rx_prev_seq=%d\n",
                     index2name(ps_core_d->pm_data->index),
                     pst_bfgx_data->name, sepreted_type, pst_sepreted_data->rx_prev_seq);
        return -EINVAL;
    }
...
STATIC int32_t ps_recv_sepreated_data(struct ps_core_s *ps_core_d, uint8_t *buf_ptr)
{
    ...
    if (is_uart_loop_test_enable() == OAL_TRUE) {
...
STATIC int32_t ps_recv_no_sepreated_data(struct ps_core_s *ps_core_d, uint8_t *buf_ptr)
{
    ...
    bfgx_pkt_dump(ps_core_d, subsys, PKG_DUMP_RCV, buf_ptr, rx_pkt_total_len);
    ...
    if (atomic_read(&pst_bfgx_data->subsys_state) == POWER_STATE_OPEN) {
        ...
        ps_print_dbg("%s rx done! qlen=%d,pkt_len=%u\n", pst_bfgx_data->name,
                     pst_bfgx_data->rx_queue.qlen, rx_pkt_total_len);
        oal_wait_queue_wake_up_interrupt(&pst_bfgx_data->rx_wait);
    }
...
STATIC int32_t ps_check_packet_first_byte(struct ps_core_s *ps_core_d, uint8_t first_byte, int32_t count)
{
    if (unlikely(first_byte != PACKET_START_SIGNAL)) {
        ps_core_d->err_header_recv++;
        // 如果包头一直有错，大概率是时钟有异常无法恢复，频繁打印会导致刷屏丢日志
        if (ps_core_d->err_header_recv <= PKE_HEADER_ERR_THRESH) {
            ps_print_warning("pkt head is ERR:%x,count=[%d]\n", first_byte, count);
            print_uart_decode_param(ps_core_d);
        }
        return -OAL_EINVAL;
    }

    if (ps_core_d->err_header_recv != 0) {
        ps_print_warning("receive uart header, detect errcnt %llu\n", ps_core_d->err_header_recv);
        ps_core_d->err_header_recv = 0;
    }
...
STATIC int32_t ps_check_packet_head(struct ps_core_s *ps_core_d, uint8_t *buf_ptr, int32_t count)
{
    ...
    if (ps_check_packet_first_byte(ps_core_d, *ptr, count) < 0) {
        return RX_PACKET_ERR;
    }
...
STATIC uint32_t ps_core_recv_handle(struct ps_core_s *ps_core_d, const uint8_t *data, int32_t count)
{
    uint8_t *ptr = NULL;
    if (count <= 0) {
        ps_print_err("uart packet len was error!\n");
...
STATIC void ps_core_recv_error_packet_report(struct ps_core_s *ps_core_d, int32_t err_len)
{
    if (err_len != 0) {
        declare_dft_trace_key_info("rx_pkt_error", OAL_DFT_TRACE_FAIL);

        if (ps_core_d->pm_data->index == BUART) {
            if (oal_print_rate_limit(30 * PRINT_RATE_MINUTE)) { /* 30分钟上报一次 */
                chr_exception_report(CHR_PLATFORM_EXCEPTION_EVENTID, CHR_SYSTEM_PLAT, CHR_LAYER_DRV,
                                     CHR_PLT_DRV_EVENT_UART, CHR_PLAT_DRV_ERROR_B_UART_RX_PKT_ERROR);
            }
        }

        if (ps_core_d->pm_data->index == GUART) {
            if (oal_print_rate_limit(30 * PRINT_RATE_MINUTE)) { /* 30分钟上报一次 */
                chr_exception_report(CHR_PLATFORM_EXCEPTION_EVENTID, CHR_SYSTEM_PLAT, CHR_LAYER_DRV,
                                     CHR_PLT_DRV_EVENT_UART, CHR_PLAT_DRV_ERROR_G_UART_RX_PKT_ERROR);
            }
        }
    }
}
...
{
    struct ps_core_s *ps_core_d = (struct ps_core_s *)disc_data;
    int32_t data_len, ret;
    int32_t err_len = 0;

    if (ps_core_recv_handle(ps_core_d, data, count) != OAL_TRUE) {
        return 0;
    }
    ...
    ps_core_recv_error_packet_report(ps_core_d, err_len);
...
int32_t ps_h2d_tx_data(struct ps_core_s *ps_core_d, uint8_t subsys, const char __user *buf, size_t count)
{
    ...
    while (count > 0) {
        ...
        bfgx_pkt_dump(ps_core_d, subsys, PKG_DUMP_SEND, skb->data, tx_skb_len);
...
STATIC int32_t bfgx_subsys_data_init(struct ps_core_s *ps_core_d)
{
    ...
    for (i = 0; i < BFGX_BUTT; i++) {
        ...
        ps_core_d->bfgx_info[i].name = g_bfgx_name[i];
        /* 初始化开关业务完成量 */
...
STATIC void bfgx_subsys_data_remove(struct ps_core_s *ps_core_d)
{
    ...
    ps_core_d->rx_public_buf_org_ptr = NULL;
}
...
STATIC int32_t bfgx_trx_init(struct ps_core_s *ps_core_d)
{
    ...
    if (snprintf_s(ps_core_d->tx_work_name, sizeof(ps_core_d->tx_work_name), sizeof(ps_core_d->tx_work_name) - 1,
                   "ps_tx_queue%u", ps_core_d->uart_index) < 0) {
