...
STATIC ssize_t hw_debug_do_write(struct file *filp, const char __user *buf, size_t count, loff_t *f_pos)
{
    struct ps_core_s *ps_core_d = NULL;
    struct sk_buff *skb;
    uint16_t total_len = (uint16_t)(count + PS_PKG_SIZE);
    ...
    if (ps_add_packet_head(skb->data, OML_MSG, total_len) != OAL_SUCC) {
        kfree_skb(skb);
        post_to_visit_node(ps_core_d, prepare_result);
        return -EFAULT;
    }
                             ...
                             size_t count, loff_t *f_pos)
{
    ...
    if (ps_skb_queue_len(ps_core_d, RX_DBG_QUEUE) == 0) { /* if no data, and wait timeout function */
        if (filp->f_flags & O_NONBLOCK) {  /* if O_NONBLOCK read and return */
            ps_print_dbg("O_NONBLOCK mode, return EAGAIN!\n");
            return -EAGAIN;
        }
        ...
        if (!timeout) {
            ps_print_dbg("debug read return ETIMEDOUT!\n");
            return -ETIMEDOUT;
        }
    }
    ...
    if ((skb = ps_skb_dequeue(ps_core_d, RX_DBG_QUEUE)) == NULL) {
        ps_print_dbg("ps_skb_dequeue return ETIMEDOUT\n");
        return -ETIMEDOUT;
    }
...
/*
 * Prototype    : hw_debug_poll
 * Description  : called by oam_hisi func from hal;
 *                check whether or not allow read and write
 */
STATIC uint32_t hw_debug_poll(struct file *filp, poll_table *wait)
{
    uint32_t mask = 0;
    struct ps_core_s *ps_core_d = NULL;
    ...
    poll_wait(filp, &ps_core_d->rx_dbg_wait, wait);

    if (oal_netbuf_list_len(&ps_core_d->rx_dbg_seq) > 0) { /* have data to read */
...
STATIC long hw_compat_debug_ioctl(struct file *filp, uint32_t cmd, unsigned long arg)
{
    ps_print_info("compat ioctl version\n");
    return hw_debug_ioctl(filp, cmd, arg);
}
#endif
...
STATIC const struct file_operations g_hw_default_debug_fops = {
    ...
    .read = hw_debug_read,
#if defined(CONFIG_COMPAT)
    .compat_ioctl = hw_compat_debug_ioctl,
#endif
...
STATIC int32_t plat_excp_dump_rotate_cmd_read(struct file *filp, unsigned long arg, memdump_info_t *memdump_info)
{
    ...
    /* 非阻塞模式下返回，阻塞模式下在此阻塞等待异常memdump */
    if (skb_queue_len(&memdump_info->dump_type_queue) == 0) {
        if (filp->f_flags & O_NONBLOCK) {  /* if O_NONBLOCK read and return */
            ps_print_info("dump_type_queue empty\n");
            return -EAGAIN;
        }
        ps_print_warning("waiting memdump trigger.....\n");
        if (wait_event_interruptible(memdump_info->dump_type_wait,
                                     (skb_queue_len(&memdump_info->dump_type_queue)) > 0)) {
            return -EINVAL;
        }
    }
...
/*
 * Prototype    : hw_excp_poll
 * Description  : called by oam_hisi func from hal;
 *                check whether or not allow read and write
 */
STATIC uint32_t hw_excp_poll(struct file *filp, poll_table *wait)
{
    uint32_t mask = 0;
    struct bfgx_dev_node *dev_node = NULL;
    struct miscdevice *miscdev = NULL;
    memdump_info_t *memdump_info;

    if (filp == NULL || filp->private_data == NULL) {
        ps_print_err("filp null\n");
        return -EINVAL;
    }
    ...
    memdump_info = (memdump_info_t *)(dev_node->dev_data);
    /* push curr wait event to wait queue */
    poll_wait(filp, &memdump_info->dump_type_wait, wait);

    if (skb_queue_len(&memdump_info->dump_type_queue) > 0) { /* have data to read */
        mask |= POLLIN | POLLRDNORM;
    }
...
STATIC long hw_excp_ioctl(struct file *filp, uint32_t cmd, unsigned long arg)
{
    ...
    ps_print_warning("hwexcep[%s] hw_debug_ioctl cmd = %d\n", miscdev->name, cmd);
    switch (cmd) {
        ...
        case PLAT_GT_DUMP_FILE_READ_CMD:
            ret = plat_excp_dump_rotate_cmd_read(filp, arg, dev_node->dev_data);;
            break;
...
#if defined(CONFIG_COMPAT)
STATIC long hw_compat_excp_ioctl(struct file *filp, uint32_t cmd, unsigned long arg)
{
    ps_print_info("compat ioctl version\n");
    return hw_excp_ioctl(filp, cmd, arg);
}
#endif
...
STATIC const struct file_operations g_hw_excp_fops = {
    ...
    .read = hw_excp_read,
#if defined(CONFIG_COMPAT)
    .compat_ioctl = hw_compat_excp_ioctl,
#endif
