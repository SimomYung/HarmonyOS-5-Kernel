...
STATIC struct service_attr g_srv_desc[BFGX_BUTT] = {
    {SYS_CFG_OPEN_BT, SYS_CFG_CLOSE_BT, WAIT_BT_OPEN_TIME, WAIT_BT_CLOSE_TIME},
    {SYS_CFG_OPEN_FM, SYS_CFG_CLOSE_FM, WAIT_FM_OPEN_TIME,  WAIT_FM_CLOSE_TIME},
    {SYS_CFG_OPEN_GNSS, SYS_CFG_CLOSE_GNSS, WAIT_GNSS_OPEN_TIME, WAIT_GNSS_CLOSE_TIME},
    {SYS_CFG_OPEN_IR, SYS_CFG_CLOSE_IR, WAIT_IR_OPEN_TIME, WAIT_IR_CLOSE_TIME},
    {SYS_CFG_OPEN_NFC, SYS_CFG_CLOSE_NFC, WAIT_NFC_OPEN_TIME, WAIT_NFC_CLOSE_TIME},
    {SYS_CFG_OPEN_SLE, SYS_CFG_CLOSE_SLE, WAIT_SLE_OPEN_TIME, WAIT_SLE_CLOSE_TIME}
};
...
bool ps_chk_rx_queue_empty(struct ps_core_s *ps_core_d)
{
    int32_t i;
    if (ps_core_d == NULL) {
        ps_print_err("ps_core_d is NULL");
    ...
    for (i = 0; i < BFGX_BUTT; i++) {
        if (ps_core_d->bfgx_info[i].rx_queue.qlen != 0) {
            ps_print_warning("[%s]subsys %d rx_queue len %d", index2name(ps_core_d->pm_data->index), i,
                             ps_core_d->bfgx_info[i].rx_queue.qlen);
            return false;
        }
    }
    return true;
...
#define WAIT_CLOSE_TIMES 100
    ...
    unsigned long long module_set;

    ps_plat_data = (struct ps_plat_s *)ps_core_d->ps_plat;
    if (unlikely(ps_plat_data == NULL)) {
        ps_print_err("ps_plat_data is NULL\n");
        return ret;
    }
    ...
    if (ret == FAILURE) {
        ...
        module_set = SSI_MODULE_MASK_AON | SSI_MODULE_MASK_ARM_REG | SSI_MODULE_MASK_PM_TRACE;
        if (ps_core_d->uart_index == BUART) {
            module_set |= SSI_MODULE_MASK_BTRACE;
        } else if (ps_core_d->uart_index == GUART) {
            module_set |= SSI_MODULE_MASK_GTRACE;
        }
        ssi_dump_device_regs(module_set); // bfg关闭失败, 新增导出pm_trace
    }
...
static void print_uart_status_info(struct st_bfgx_data *pst_bfgx_data)
{
...
int32_t bfgx_open_cmd_send(struct ps_core_s *ps_core_d, uint32_t subsys)
{
    ...
    if (subsys == BFGX_IR) {
        ps_tx_sys_cmd(ps_core_d, SYS_MSG, g_srv_desc[subsys].open_cmd);
        msleep(20); // sleep 20ms
        return SUCCESS;
    }

    if (subsys == BFGX_GNSS) {
        gnss_thread_status_clear();
    }
    ...
    timeleft = oal_wait_for_completion_timeout(&pst_bfgx_data->wait_opened,
                                               msecs_to_jiffies(g_srv_desc[subsys].open_cmd_timeout));
    if (!timeleft) {
        unsigned long long module_set;
        print_uart_status_info(pst_bfgx_data);
        ps_uart_state_dump(ps_core_d);
        ps_print_err("wait %s open ack timeout\n", pst_bfgx_data->name);
        if (ps_core_d->uart_index == BUART) {
            module_set = SSI_MODULE_MASK_AON | SSI_MODULE_MASK_ARM_REG | SSI_MODULE_MASK_BTRACE |
                         SSI_MODULE_MASK_BCTRL;
        } else {
            module_set = SSI_MODULE_MASK_AON | SSI_MODULE_MASK_ARM_REG | SSI_MODULE_MASK_GTRACE |
...
int32_t bfgx_close_cmd_send(struct ps_core_s *ps_core_d, uint32_t subsys)
{
    ...
    if (subsys == BFGX_IR) {
        ps_tx_sys_cmd(ps_core_d, SYS_MSG, g_srv_desc[subsys].close_cmd);
        msleep(20); // sleep 20ms
        return SUCCESS;
    }
    ...
    timeleft = oal_wait_for_completion_timeout(&pst_bfgx_data->wait_closed,
                                               msecs_to_jiffies(g_srv_desc[subsys].close_cmd_timeout));
    if (!timeleft) {
        print_uart_status_info(pst_bfgx_data);
        ps_uart_state_dump(ps_core_d);
...
STATIC int32_t hw_bfgx_start(struct ps_core_s *ps_core_d, uint32_t subsys)
{
    ...
    if (error != BFGX_POWER_SUCCESS) {
        ps_print_err("[%s]set %s power on error=%d\n", index2name(pm_data->index), pst_bfgx_data->name, error);
    ...
    if (oal_atomic_read(&ps_core_d->ir_only) == 0) {
        ps_print_info("open %s mod_timer %d\n", pst_bfgx_data->name,
                      BT_SLEEP_TIME + g_srv_desc[subsys].open_cmd_timeout);
        mod_timer(&pm_data->bfg_timer, jiffies + msecs_to_jiffies(BT_SLEEP_TIME + g_srv_desc[subsys].open_cmd_timeout));
...
int32_t hw_bfgx_open(struct ps_core_s *ps_core_d, uint32_t subsys)
{
    ...
    ps_print_info("[%s]open %s\n", index2name(ps_core_d->uart_index), pst_bfgx_data->name);

    if (off_find_get_status() == OAL_TRUE) {
        ps_print_warning("[%s]%s in off find status,not allow open\n",
                         index2name(ps_core_d->uart_index), pst_bfgx_data->name);
        return BFGX_POWER_FAILED;
    }
...
int32_t hw_bfgx_close(struct ps_core_s *ps_core_d, uint32_t subsys)
{
    ...
    if (ret) {
        ps_print_err("set %s power off err!ret = %d", pst_bfgx_data->name, ret);
    }
...
/*
 * Prototype    : hw_bfgx_close_all_service
 * Description  : 关闭ps_core下所有的业务，清理相关资源
 *                pwr_off：是否对芯片下电
 */
void hw_bfgx_close_all_service(struct pm_drv_data *pm_data, uint32_t pwr_off)
{
    uint32_t i;
    ...
    if (pm_data == NULL) {
        ps_print_info("pm_data is null"NEWLINE);
        return;
    }
...
/*
 * 函 数 名  : hw_plat_open
 * 功能描述  : 不依赖应用层调用，平台利用该接口单独上电，用于平台功能调试
               uart:指定命令通过哪个uart下发。subsys:下发哪个子系统的open cmd
 */
int32_t hw_plat_open(uint8_t uart, uint32_t subsys)
{
    int32_t ret;
    struct ps_core_s *ps_core_d = NULL;
    struct pm_top* pm_top_data = pm_get_top();

    if (uart >= UART_BUTT || subsys >= BFGX_BUTT) {
        return -EINVAL;
    }
    ...
    ret = hw_bfgx_open(ps_core_d, subsys);
    mutex_unlock(&(pm_top_data->host_mutex));
    return ret;
}
...
/*
 * 函 数 名  : hw_plat_close
 * 功能描述  : 不依赖应用层调用，平台利用该接口单独下电，用于平台功能调试
               uart:指定命令通过哪个uart下发。subsys:下发哪个子系统的close cmd
 */
int32_t hw_plat_close(uint8_t uart, uint32_t subsys)
{
    int32_t ret;
    struct ps_core_s *ps_core_d = NULL;
    struct pm_top* pm_top_data = pm_get_top();
    ...
    ret = hw_bfgx_close(ps_core_d, subsys);
    mutex_unlock(&(pm_top_data->host_mutex));
    return ret;
}

STATIC uart_loop_cfg g_uart_loop_test_cfg = { 256, 60000, 0, 0, 0 };
STATIC uart_loop_test_struct *g_uart_loop_test_info = NULL;
STATIC int g_uart_loop_mode = 0;  // default: mode 0
oal_debug_module_param(g_uart_loop_mode, int, S_IRUGO | S_IWUSR);

uint32_t is_uart_loop_test_enable(void)
{
    return g_uart_loop_test_cfg.uart_loop_enable;
}
...
STATIC int32_t alloc_uart_loop_test(void)
{
    ...
    if (g_uart_loop_test_info == NULL) {
        ...
        g_uart_loop_test_cfg.uart_loop_enable = OAL_TRUE;
        g_uart_loop_test_cfg.uart_loop_tx_random_enable = 1;

        if (g_uart_loop_mode != 0) {
            g_uart_loop_test_cfg.loop_count = UART_LOOP_MAX_PKT_LEN; // 从1字节到2048字节轮询发送
        } else {
            g_uart_loop_test_cfg.loop_count = 0x100; // 恢复默认的初始值256次
        }
...
STATIC void free_uart_loop_test(void)
{
    ...
    g_uart_loop_test_cfg.uart_loop_enable = OAL_FALSE;
...
void uart_loop_set_complete(struct ps_core_s *ps_core_d, uint8_t *buf_ptr)
{
    if (g_uart_loop_test_info != NULL) {
...
int32_t uart_loop_test(struct ps_core_s *ps_core_d)
{
    ...
    for (i = 0; i < count; i++) {
        ...
        if (g_uart_loop_mode != 0) {
            g_uart_loop_test_info->test_cfg->pkt_len = i + 1;
        }
        ps_print_info("uart loop test start, i=[%u]\n", i);
