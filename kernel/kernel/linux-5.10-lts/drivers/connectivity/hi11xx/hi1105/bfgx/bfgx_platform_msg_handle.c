...
STATIC void bfgx_hello_msg_handle(struct ps_core_s *ps_core_d, uint8_t *buf_ptr);
...
STATIC platform_msg_handler g_sys_msg_handle[SYS_INF_BUTT] = {
    bfgx_hello_msg_handle,       // SYS_INF_PF_HELLO = 0x00,               平台软件hello消息
    ...
    uart_loop_set_complete,      // SYS_INF_UART_LOOP_SET_DONE = 0x1a,     device 设置uart环回ok
    ...
    bfgx_thread_msg_handle,      // SYS_OFF_FIND_INIT = 0x27,              OFF_FIND SLE软件初始化完成
    bfgx_thread_msg_handle,      // SYS_OFF_FIND_EXIT = 0x28,              OFF_FIND SLE软件退出完成
};
...
STATIC uint32_t g_gnss_me_thread_status = DEV_THREAD_EXIT;
STATIC uint32_t g_gnss_lppe_thread_status = DEV_THREAD_EXIT;

void gnss_thread_status_clear(void)
{
...
STATIC void bfgx_hello_msg_handle(struct ps_core_s *ps_core_d, uint8_t *buf_ptr)
{
    ps_print_info("[%s]SYS_INF_PF_HELLO msg received\n", index2name(ps_core_d->uart_index));
}
...
STATIC void bfgx_thread_msg_handle(struct ps_core_s *ps_core_d, uint8_t *buf_ptr)
{
    ...
    switch (msg) {
        ...
        case SYS_INF_SLE_EXIT:
            ...
            break;
        case SYS_OFF_FIND_INIT:
        case SYS_OFF_FIND_EXIT:
            off_find_thread_completion_handle(msg);
            break;
        default:
            break;
    };
...
#endif
    if (((tty->ops->get_icount) != NULL) && (tty->ops->get_icount(tty, &uart_icount) < 0)) {
        memset_s(&uart_icount, sizeof(struct serial_icounter_struct),
                 0, sizeof(struct serial_icounter_struct));
    }

    ps_print_info("[%s]%ds,tty_tx=%d,tty_rx=%d,uart_tx=%d,uart_rx=%d,cts=%d,stopped=%d,hw_stopped=%d,chars_in_tty=%d\n",
                  index2name(pm_data->index), syschar - SYS_INF_HEARTBEAT_CMD_BASE - 1,
