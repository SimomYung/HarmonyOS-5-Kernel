...
ssize_t hw_sle_read(struct file *filp, char __user *buf, size_t count, loff_t *f_pos)
{
    ...
    if ((skb = ps_skb_dequeue(ps_core_d, RX_SLE_QUEUE)) == NULL) {
        ps_print_warning("sle read skb queue is null!\n");
        return 0;
    }
...
/*
 * Prototype    : hw_sle_ioctl
 * Description  : called by sle func from hal; default not use
 */
                                       ...
                                       size_t count)
{
    ...
    bfgx_pkt_dump(ps_core_d, BFGX_SLE, PKG_DUMP_SEND, skb->data, total_len);
                              ...
                              struct bt_data_combination *data_comb)
{
    ...
    if (unlikely(hw_bfgx_write_check(buf, ps_core_d, BFGX_SLE) < 0)) {
        return -EINVAL;
    }
