...
typedef struct {
    int32_t cmd;
    int32_t (*exception_dbg_func)(void);
} dbg_cmd_t;
...
STATIC ssize_t store_wifi_pmdbg(struct kobject *kobj, struct kobj_attribute *attr, const char *buf, size_t count)
{
    ...
    switch (input) {
        ...
        case 3: /* cmd: 3 open wifi */
            wlan_pm_open_dev(HCC_EP_WIFI_DEV, OAL_TRUE);
            break;
...
STATIC ssize_t show_exception_count(struct kobject *kobj, struct kobj_attribute *attr, char *buf)
{
    ...
    ret = plat_get_excp_total_cnt(&excp_cnt);
    if (ret != EXCEPTION_SUCCESS) {
        return -FAILURE;
    }

    ps_print_dbg("exception_dbg: chip_cnt = %u, subsys_cnt = %u, core_cnt = %u\n",
                 excp_cnt.subsys_cnt, excp_cnt.subsys_cnt, excp_cnt.core_cnt);
    ret = snprintf_s(buf, PAGE_SIZE, PAGE_SIZE - 1, "exception_dbg: chip_cnt = %u, subsys_cnt = %u, core_cnt = %u\n",
                     excp_cnt.subsys_cnt, excp_cnt.subsys_cnt, excp_cnt.core_cnt);
    return ret;
}
...
#if defined(CONFIG_HISI_PMU_RTC_READCOUNT) || defined(CONFIG_PMU_RTC_READCOUNT) || defined(CONFIG_RTC_DRV_PMIC_SPMI)
...
STATIC int32_t dfr_info_clear(void)
{
...
STATIC int32_t dfr_subsystem_rst_enable(void)
{
    struct st_exception_info *pst_exception_data = NULL;
...
STATIC int32_t dfr_subsystem_rst_disable(void)
{
    struct st_exception_info *pst_exception_data = NULL;
...
STATIC int32_t wifi_wkup_fail_test(void)
{
    struct wlan_pm_s *pst_wlan_pm = wlan_pm_get_drv();
...
STATIC int32_t wifi_trans_fail_test(void)
{
...
STATIC int32_t wifi_power_on_fail_test(void)
{
...
STATIC int32_t dfr_sys_rst_download_fail_test(void)
{
    struct st_exception_info *pst_exception_data = NULL;
    ...
    set_excp_test_en(FIRMWARE_DOWNLOAD_FAULT, (uintptr_t)NULL);
...
STATIC int32_t exception_dbg_proc(int32_t cmd)
{
    ...
    if (g_exception_dbg[cmd - 1].exception_dbg_func == NULL) {
        ps_print_err("[dfr_test] old method with cmd %d is discard\n", cmd);
        return OAL_FAIL;
    }

    return g_exception_dbg[cmd - 1].exception_dbg_func();
}
...
STATIC ssize_t store_exception_dbg(struct kobject *kobj, struct kobj_attribute *attr, const char *buf, size_t count)
{
    ...
    if (exception_dbg_proc(cmd) != OAL_SUCC) {
        return -FAILURE;
    }
...
uint64_t get_plat_boottime(void);
STATIC ssize_t show_plat_boottime(struct kobject *kobj, struct kobj_attribute *attr, char *buf)
{
    if (buf == NULL) {
        ps_print_err("buf is NULL\n");
        return -FAILURE;
    ...
    return snprintf_s(buf, PAGE_SIZE, INI_FILE_PATH_LEN, "%lu ms\n", get_plat_boottime());
}
...
STATIC struct kobj_attribute g_plat_boottime =
    __ATTR(plat_boottime, 0440, (void *)show_plat_boottime, NULL);

STATIC struct attribute *g_mpxx_debug_attrs[] = {
    &g_plat_exception_dbg.attr,
    &g_bfgx_dev_test.attr,
    &g_factory_test.attr,
    &g_plat_boottime.attr,
    NULL,
};
...
#ifdef _PRE_CONFIG_USE_OCTTY
struct kobject *bfgx_get_hi110x_bfgx_kobject(void)
{
    return g_sysfs_hi110x_bfgx;
}
#endif
