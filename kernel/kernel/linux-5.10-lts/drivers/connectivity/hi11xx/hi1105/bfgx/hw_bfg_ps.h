...
/* gf61没有bfgx_wakeup_ap管脚，用睡眠延迟保证上电握手
 * gf61下载firmware后片内上电耗时 < 50ms（去掉拉EN和FW下载耗时）,
 *  此延迟可以压缩到100ms量级, 座舱休眠唤醒卡顿暂配成800ms */
#define WAIT_BFGX_BOOTOK_TIME2 800
#define WAIT_BFGX_CLOSEOK_TIME 2000
#define WAIT_BT_OPEN_TIME      3000
#define WAIT_BT_CLOSE_TIME     4000  /* 心跳超时触发panic到last word要3120ms，延长500ms避免ssi dump和memdump撞上导致错包 */
#define WAIT_FM_OPEN_TIME      3000
#define WAIT_FM_CLOSE_TIME     4000
#define WAIT_GNSS_OPEN_TIME    3000
#define WAIT_GNSS_CLOSE_TIME   4000
#define WAIT_IR_OPEN_TIME      3000
#define WAIT_IR_CLOSE_TIME     4000
#define WAIT_NFC_OPEN_TIME     1000
#define WAIT_NFC_CLOSE_TIME    2000
#define WAIT_SLE_OPEN_TIME     3000
#define WAIT_SLE_CLOSE_TIME    4000
#define BFGX_CLOSE_DELAY       1000
#define WAIT_OPEN_SLE_HOST_TIME     3000
#define WAIT_CLOSE_SLE_HOST_TIME    4000
...
enum platform_event_enum {
    ...
    SYS_CFG_OFF_FIND = 0x2a,         /* 关机找回，通知device */
    SYS_CFG_COMCPU_EXCP = 0x2b,      /* 触发小核异常 */
    SYS_CFG_OPEN_OFF_FIND_THREAD = 0x2c,    /* HOST通过uart打开off_find线程 */
    SYS_CFG_CLOSE_OFF_FIND_THREAD = 0x2d,   /* HOST通过uart关闭off_find线程 */
    PL_EVT_BUTT
};

/* below is rx sys content define from device */
enum SYS_INF_MSG_VALUE_ENUM {
    SYS_INF_PF_HELLO = 0x00,               /* 平台软件hello消息 */
    ...
    SYS_OFF_FIND_INIT = 0x27,               /* OFF_FIND软件初始化完成 */
    SYS_OFF_FIND_EXIT = 0x28,               /* OFF_FIND软件退出完成 */
...
struct ps_core_s {
    ...
    uint8_t rx_pkt_type;                // 本次接收的uart数据的类型
    uint16_t rx_pkt_total_len;          // 本次接收的uart数据包头指示的长度，不含包头包尾
    uint16_t rx_have_recv_pkt_len;      // 已接收的数据长度
    uint16_t rx_have_del_public_len;    // 已经解析处理的公共buffer长度
    uint8_t *rx_decode_tty_ptr;         // 当前tty传递的rx buff指针
    uint8_t *rx_public_buf_org_ptr;     // 公共接收buffer的初始指针
    uint8_t *rx_decode_public_buf_ptr;  // 待处理的报文的公共接收buffer
