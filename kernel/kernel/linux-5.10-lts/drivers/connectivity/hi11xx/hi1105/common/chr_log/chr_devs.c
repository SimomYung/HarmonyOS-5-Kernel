...
static void chr_softtimer_timeline_debug(chr_inner_report_info* report, int16_t len)
{
    uint32_t* data = NULL;

    if (len - sizeof(chr_inner_report_info) - 16 * sizeof(uint8_t) < 0) { /* 16: 携带16字节数据 */
        ps_print_err("%s mem overlap\n", __FUNCTION__);
        return;
    }

    data = (uint32_t*)((uint8_t*)report + sizeof(chr_inner_report_info));
    chr_warning("softtimer_debug, stop timer[0x%x], cur timer[0x%x], pre_timeline[0x%x], cur_timeline[0x%x]",
                data[0], data[1], data[2], data[3]);  // 0:stop 1:cur 2:pre_timeline 3:cur_timeline
    declare_dft_trace_key_info("softimer timeline error", OAL_DFT_TRACE_FAIL);
}
...
static int32_t chr_cust_id_message_proc(uint32_t errno, uint8_t *ptr_data, uint16_t len)
{
    ...
    switch (report->errid) {
        ...
        case CHR_PLAT_INNER_ERROR_WIFI_PC_LR:
            ...
            break;
        case CHR_PLAT_INNER_ERROR_SOFTTIMER_DEBUG:
            chr_softtimer_timeline_debug(report, len);
            break;
        default:
            break;
    }
...
static int32_t chr_error_log_message_proc(uint32_t errno, uint16_t us_flag, uint8_t *ptr_data, uint16_t len)
{
...
static int32_t chr_message_report_to_kmsg(uint32_t errno, uint16_t us_flag, uint8_t *ptr_data, uint16_t len)
{
    ...
    if (errno == CHR_ERROR_LOG_EVENTID) {
        return chr_error_log_message_proc(errno, us_flag, ptr_data, len);
    }
    return CHR_SUCC;
}
...
static int32_t chr_write_errno_to_queue(uint32_t errno, uint16_t us_flag, uint8_t *ptr_data, uint16_t len)
{
    ...
    if ((errno == CHR_CHIP_INNER_REPORT_EVENTID) || (errno == CHR_ERROR_LOG_EVENTID)) {
        ret = chr_message_report_to_kmsg(errno, us_flag, ptr_data, len);
        return ret;
    }
