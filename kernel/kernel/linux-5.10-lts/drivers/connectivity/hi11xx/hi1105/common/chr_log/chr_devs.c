...
#define CHR_ERRLOG_MAX_NUM   4

typedef struct {
    uint32_t file_line;
    uint16_t chip_id;
    uint16_t param_num;
    int32_t  param[CHR_ERRLOG_MAX_NUM];
} chr_errlog_stru;
...
static void chr_rx_errno_to_dispatch(uint32_t error_no);
static int32_t chr_wifi_tx_handler(uint32_t error_no);
static int32_t chr_bfg_dev_tx_handler(uint32_t error_no);
 ...
 */
static int32_t chr_report_frequency_limit(uint32_t error_no)
{
#define CHR_REPORT_LIMIT_TIME 5 /* second */
    ...
    for (index = 0; index < len; index++) {
        if (error_no == g_chr_unlimit_errno[index]) {
            return CHR_SUCC;
        }
    }
    ...
    if ((chr_report_interval_time > CHR_REPORT_LIMIT_TIME) ||
        (chr_report_last_time == 0) || (old_errno != error_no)) {
 ...
 */
static int32_t chr_errno_convert_to_bfg_id(uint32_t error_no)
{
    uint32_t chr_id;

    chr_id = error_no / CHR_ID_MSK;
    ...
    if (chr_id == CHR_BT) {
        ...
        chr_err("unsupport error_no [%d]\n", error_no);
        return -EINVAL;
    }
}

static void chr_bfg_disallow_sleep_info(chr_inner_report_info* report, int16_t len)
{
    uint8_t* data = (uint8_t*)report + sizeof(chr_inner_report_info);
    if (len - sizeof(chr_inner_report_info) - 2 * sizeof(uint8_t) < 0) { /* 2: 携带2个判据 */
...
static void chr_pmu_event_err_reg_info(chr_inner_report_info* report, int16_t len)
{
    uint8_t* data = (uint8_t*)report + sizeof(chr_inner_report_info);
    if (len - sizeof(chr_inner_report_info) - 10 * sizeof(uint8_t) < 0) { /* 10: 携带10个数据据 */
        ps_print_err("%s mem overlap\n", __FUNCTION__);
        return;
    }
    ...
    declare_dft_trace_key_info("pmu_error_report", OAL_DFT_TRACE_FAIL);
}

static void chr_wifi_pc_lr_reg_info(chr_inner_report_info* report, int16_t len)
{
    ...
    if (len - sizeof(chr_inner_report_info) - sizeof(chr_inner_wifi_panic_info) < 0) {
        ps_print_err("%s mem overlap\n", __FUNCTION__);
        return;
    }
...
board_chr_cust_cb g_board_chr_cust_cb[CHR_PLAT_INNER_BUF] = {NULL};

void chr_cust_register(chr_plat_inner_error_enum error, board_chr_cust_cb cb)
{
    if (error >= CHR_PLAT_INNER_BUF) {
        return;
    }

    if (g_board_chr_cust_cb[error] != NULL) {
        chr_err("board_chr_cust[%d] cb already registed\n", (int32_t)error);
        return;
    }

    if (cb == NULL) {
        chr_err("board_chr_cust register cb is NULL\n");
        return;
    }

    g_board_chr_cust_cb[error] = cb;
    chr_warning("board_chr_cust[%d] cb register succ\n", error);
    return;
}

static int32_t chr_cust_id_message_proc(uint32_t error_no, uint8_t *ptr_data, uint16_t len)
{
    chr_inner_report_info* report = (chr_inner_report_info*)ptr_data;

    if (len < sizeof(chr_inner_report_info) || len != report->errlen) {
        chr_err("chr error_no len is wrong, error_no len = %d!\n", len);
        return -EINVAL;
    }

    chr_warning("chr inner report[%d] info: errid=%d, len=%d, module=0x%x\n",
                error_no, report->errid, report->errlen, report->module);
    switch (report->errid) {
        ...
        case CHR_PLAT_INNER_ERROR_BFG_DISALLOW_SLEEP:
            chr_bfg_disallow_sleep_info(report, len);
            break;
        case CHR_PLAT_INNER_ERROR_PMU_EVENT_REG_ERR:
            chr_pmu_event_err_reg_info(report, len);
            break;
        case CHR_PLAT_INNER_ERROR_WIFI_PC_LR:
            if (g_board_chr_cust_cb[CHR_PLAT_INNER_ERROR_WIFI_PC_LR] != NULL) {
                g_board_chr_cust_cb[CHR_PLAT_INNER_ERROR_WIFI_PC_LR]((uint8_t *)report, len);
            } else {
...
static int32_t chr_message_report_to_server(uint32_t error_no, uint16_t us_flag, uint8_t *ptr_data, uint16_t len)
{
    struct sk_buff *skb = NULL;
    uint16_t sk_len;
    int32_t ret;
    ...
    if (unlikely(len > (USHRT_MAX - sizeof(chr_dev_exception_stru_para)))) {
        chr_warning("chr error_no %x len %d over limit\n", error_no, len);
        return -EINVAL;
    }

    if (chr_report_frequency_limit(error_no)) {
        chr_warning("chr report limited, dispose error_no=%d\n", error_no);
        return CHR_SUCC;
    }

    if (skb_queue_len(&g_chr_event.errno_queue) > CHR_ERRNO_QUEUE_MAX_LEN) {
        chr_warning("chr error_no queue is full, dispose error_no=%d\n", error_no);
        return CHR_SUCC;
    }
    ...
    if (skb == NULL) {
        chr_err("chr error_no alloc skbuff failed! len=%d, error_no=%d\n", sk_len, error_no);
        return -ENOMEM;
    }

    skb_put(skb, sk_len);
    *(uint32_t *)skb->data = error_no;
...
static int32_t chr_error_log_message_proc(uint32_t error_no, uint16_t us_flag, uint8_t *ptr_data, uint16_t len)
{
    ...
    chr_warning("chr error log report[%d] info: file_id=%u, file_line=%u, chip_id=%u\n",
                error_no, (report->file_line >> CHR_FILEID_OFFSET), (report->file_line & 0xFFFF), report->chip_id);

    if (!oal_print_rate_limit(30 * PRINT_RATE_MINUTE)) { /* 30分钟上报一次 */
        chr_warning("chr errlog report limit\n");
        return CHR_SUCC;
    }

    return chr_message_report_to_server(CHR_ERROR_LOG_EVENTID, us_flag, ptr_data, len);
}

static int32_t chr_message_report_to_kmsg(uint32_t error_no, uint16_t us_flag, uint8_t *ptr_data, uint16_t len)
{
    if (error_no == CHR_CHIP_INNER_REPORT_EVENTID) {
        return chr_cust_id_message_proc(error_no, ptr_data, len);
    }

    if (error_no == CHR_ERROR_LOG_EVENTID) {
        return chr_error_log_message_proc(error_no, us_flag, ptr_data, len);
    }
    return CHR_SUCC;
}
 ...
 * 功能描述  : 将异常码写入队列
 */
static int32_t chr_write_errno_to_queue(uint32_t error_no, uint16_t us_flag, uint8_t *ptr_data, uint16_t len)
{
    int32_t ret;
    ...
    if ((error_no == CHR_CHIP_INNER_REPORT_EVENTID) || (error_no == CHR_ERROR_LOG_EVENTID)) {
        ret = chr_message_report_to_kmsg(error_no, us_flag, ptr_data, len);
        return ret;
    }

    ret = chr_message_report_to_server(error_no, us_flag, ptr_data, len);
    if (ret != CHR_SUCC) {
        return ret;
    }
...
{
    uint32_t chr_num;
    chr_num = error_no / CHR_ID_MSK;
    switch (chr_num) {
        case CHR_WIFI:

            if (chr_wifi_tx_handler(error_no) != CHR_SUCC) {
                chr_info("wifi tx failed,0x%x", error_no);
            }
            break;
        case CHR_BT:
        case CHR_GNSS:
            if (chr_bfg_dev_tx_handler(error_no) != CHR_SUCC) {
                chr_info("bt/gnss tx failed,0x%x", error_no);
            }
            break;
        default:
            chr_err("rcv error num 0x%x", error_no);
    }
}

/*
 * 函 数 名  : chr_wifi_dev_tx_handler
 * 功能描述  : 通过hcc通道将error_no下发到wifi device
 */
static int32_t chr_wifi_dev_tx_handler(uint32_t error_no)
{
    ...
    l_ret = memcpy_s(oal_netbuf_put(pst_netbuf, sizeof(uint32_t)), sizeof(uint32_t),
                     &error_no, sizeof(uint32_t));
    if (l_ret != EOK) {
        oal_io_print("chr_wifi error_no copy failed\n");
 ...
 * 功能描述  : 调用回调接口将error_no传给hmac
 */
static int32_t chr_host_tx_handler(uint32_t error_no)
{
    if (g_chr_get_wifi_info_callback.chr_get_wifi_info == NULL) {
        oal_io_print("{chr_host_tx_handler:: callback is null!}");
        return CHR_EFAIL;
    }
    if (g_chr_get_wifi_info_callback.chr_get_wifi_info(error_no) != CHR_SUCC) {
        oal_io_print("{chr_host_tx_handler:: tx faild, error_no = %u !}", error_no);
        return CHR_EFAIL;
    }
...
static int32_t chr_wifi_tx_handler(uint32_t error_no)
{
    ...
    ret1 = chr_host_tx_handler(error_no);
    ret2 = chr_wifi_dev_tx_handler(error_no);
    if (ret1 != CHR_SUCC || ret2 != CHR_SUCC) {
        chr_info("wifi tx failed,error_no[%u],host tx ret1[%d],device tx ret2[%d]", error_no, ret1, ret2);
        return -CHR_EFAIL;
    }

    chr_info("tx is succ,error_no %u\n", error_no);
...
static struct ps_core_s *get_valid_chr_ps_core(uint32_t error_no)
{
    int32_t bfgx_subsys;
    struct ps_core_s *ps_core_d = NULL;
    const char *dev_name = NULL;

    bfgx_subsys = chr_errno_convert_to_bfg_id(error_no);
    if (bfgx_subsys < 0) {
        return NULL;
    }
 ...
 * 功能描述  : 利用uart通道将error_no传给bfg
 */
static int32_t chr_bfg_dev_tx(uint32_t error_no)
{
    struct sk_buff *skb = NULL;
    uint16_t sk_len;
    int32_t ret;
    ...
    ps_core_d = get_valid_chr_ps_core(error_no);
    if (ps_core_d == NULL) {
        chr_info("chr dev tx abort, error_no[%u]\n", error_no);
        return -CHR_EFAIL;
    }
    ...
    skb = alloc_skb(sk_len, (oal_in_interrupt() || oal_in_atomic()) ? GFP_ATOMIC : GFP_KERNEL);
    if (skb == NULL) {
        chr_err("alloc skbuff failed! len=%d, error_no=0x%x\n", sk_len, error_no);
    ...
    /* put error_no into skb_data */
    *(uint32_t *)(skb->data + sizeof(struct ps_packet_head)) = error_no;
...
static int32_t chr_bfg_dev_tx_handler(uint32_t error_no)
{
    ...
    if (pm_top_data == NULL) {
        chr_err("get pm_top_data failed!");
        return -CHR_EFAIL;
    }
    mutex_lock(&(pm_top_data->host_mutex));
    ret = chr_bfg_dev_tx(error_no);
