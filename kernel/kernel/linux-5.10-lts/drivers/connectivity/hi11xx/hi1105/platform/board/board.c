...
/* 全局变量定义 */
mpxx_board_func g_mpxx_board_func[BOARD_VERSION_MPXX_BUTT] = {
    [BOARD_VERSION_MP13] = {board_info_init_mp13, NULL},
    [BOARD_VERSION_MP15] = {board_info_init_mp15, NULL},
    [BOARD_VERSION_MP16] = {board_info_init_mp16, NULL},
    [BOARD_VERSION_MP16C] = {board_info_init_mp16c, NULL},
    [BOARD_VERSION_GF61] = {board_info_init_gf61, NULL},
    [BOARD_VERSION_MP17C] = {board_info_init_mp17c, NULL},
    [BOARD_VERSION_MP12] = {board_info_init_mp12, NULL},
};
...
uint8_t g_wlan_device_mac_addr_2byte_flg = 0;
EXPORT_SYMBOL(g_wlan_device_mac_addr);
EXPORT_SYMBOL(g_wlan_device_mac_addr_2byte_flg);
#endif
...
OAL_STATIC device_board_version g_mpxx_board_version_list[] = {
    ...
    { .index = BOARD_VERSION_MP17C, .name = BOARD_VERSION_NAME_MP17C },
    { .index = BOARD_VERSION_MP12,  .name = BOARD_VERSION_NAME_MP12 },
};
...
oal_debug_module_param(g_mpxx_os_build_variant, int, S_IRUGO | S_IWUSR);
#endif
...
int32_t get_mpxx_subchip_version(void)
{
    ...
    return bd_info->subchip_version;
}
EXPORT_SYMBOL_GPL(get_mpxx_subchip_version);
...
/*
 * 函 数 名  : enable_board_pmu_regulator
 * 功能描述  : 开关connectivity依赖的SOC PMU电源
 */
int32_t enable_board_pmu_regulator(uint8_t en)
{
    int32_t ret = BOARD_SUCC;
    static uint8_t fem_switch1_status = OAL_FALSE;

    if (g_st_board_info.fem_switch1_regulator != NULL) {
        if ((en == OAL_TRUE) && (fem_switch1_status == OAL_FALSE)) {
            ret = regulator_enable(g_st_board_info.fem_switch1_regulator);
            fem_switch1_status = OAL_TRUE;
            ps_print_info("enable fem_switch1 ret %d, voltage %d, status %d\n", ret,
                          regulator_get_voltage(g_st_board_info.fem_switch1_regulator),
                          regulator_is_enabled(g_st_board_info.fem_switch1_regulator));
        } else if ((en == OAL_FALSE) && (fem_switch1_status == OAL_TRUE)) {
            ret = regulator_disable(g_st_board_info.fem_switch1_regulator);
            fem_switch1_status = OAL_FALSE;
            ps_print_info("disable fem_switch1 ret %d, status %d\n", ret,
                          regulator_is_enabled(g_st_board_info.fem_switch1_regulator));
        } else {
            ps_print_warning("en %d curr %d\n", en, fem_switch1_status);
        }
    }
...
#ifdef _PRE_DUAL_CHIP
int32_t board_host_wakeup_dev_set_by_id(uint32_t dev_id, int value)
{
    enum gpio_type type = conn_get_gpio_type_by_id(dev_id, HOST_WKUP_W);
    /*
     * host wakeup dev gpio pinmux to dft jtag when w boot,
     * must gpio low when bootup
     */
    if (conn_gpio_valid(type) == OAL_FALSE) {
        ps_print_info("host_wakeup_wlan [%d] gpio is 0\n", dev_id);
        return 0;
    }
    ps_print_info("host_wakeup_wlan [%d] set %s\n", dev_id, value ? "high" : "low");
    if (value) {
        return conn_set_gpio_level(type, GPIO_LEVEL_HIGH);
    } else {
        return conn_set_gpio_level(type, GPIO_LEVEL_LOW);
    }
}
#endif
...
{
    return g_st_board_info.bd_ops.susbsys_is_shutdown(subsystem);
}
...
void board_chip_power_on(void)
{
    ...
    if (ret != BOARD_SUCC) {
        ps_print_err("mpxx enable 32k clk fail.\n");
    }
    enable_board_pmu_regulator(OAL_TRUE);
    board_power_notify(PLAT_SUBSYS, OAL_TRUE);

    board_host_wakeup_dev_set(0);
    ret = main_clk_prepare_with_power_on(STAGE_POWER_PRE);
    if (ret != BOARD_SUCC) {
        ps_print_err("clk pre prepare fail.\n");
    }
    power_state_change(CHIP_POWR, BOARD_POWER_ON);
    ret = main_clk_prepare_with_power_on(STAGE_POWER_POST);
    if (ret != BOARD_SUCC) {
        ps_print_err("clk post prepare fail.\n");
    }
}
...
void board_chip_power_off(void)
{
    ...
    enable_board_pmu_regulator(OAL_FALSE);
...
static int32_t find_device_subchip_version(void)
{
#ifdef _PRE_CONFIG_USE_DTS
    int32_t ret;
    const char *mpxx_version = NULL;

    ret = get_board_custmize(DTS_NODE_HISI_MPXX, DTS_PROP_MPXX_SUBCHIP_VERSION, &mpxx_version);
    if (ret != BOARD_SUCC) {
        ps_print_info("not find in dts %s mpxx_subchip_version is %d\n",
            DTS_PROP_MPXX_SUBCHIP_VERSION, CHIP_VERSION_PILOT);
        return CHIP_VERSION_PILOT;
    }
    if (strcmp(mpxx_version, "mpw") == 0) {
        ps_print_info("from dts mpxx_subchip_version is %d\n", CHIP_VERSION_MPW);
        return CHIP_VERSION_MPW;
    } else if (strcmp(mpxx_version, "pilot") == 0) {
        ps_print_info("from dts mpxx_subchip_version is %d\n", CHIP_VERSION_PILOT);
        return CHIP_VERSION_PILOT;
    }
    return CHIP_VERSION_PILOT;
#else
    ps_print_info("mpxx_subchip_version is %d\n", CHIP_VERSION_PILOT);
    return CHIP_VERSION_PILOT;
#endif
}
...
int32_t board_chiptype_init(void)
{
    ...
    // 获取subchip 类型
    g_st_board_info.subchip_version = find_device_subchip_version();
...
static int32_t board_func_init(struct platform_device *dev)
{
    ...
    if ((chip_nr < 0) || (chip_nr >= BOARD_VERSION_MPXX_BUTT)) {
        ps_print_err("invalid chip_nr=[%d]\n", chip_nr);
        return BOARD_FAIL;
    }

    if (g_mpxx_board_func[chip_nr].init_func != NULL) {
        ret = g_mpxx_board_func[chip_nr].init_func();
        ps_print_info("%s load chip_nr[%d] board ops\n", __func__, chip_nr);
    } else {
        ps_print_err("chip_nr[%d] init_func is NULL\n", chip_nr);
        ret = BOARD_FAIL;
    }
...
OAL_STATIC int32_t get_download_mode(void)
{
    int32_t ret;
...
OAL_STATIC int32_t download_mode_check(void)
{
    if ((g_st_board_info.wlan_download_channel == CHANNEL_SDIO) && (g_st_board_info.download_mode == MODE_SEPARATE)) {
        ps_print_err("sdio do not support MODE_SEPARATE!!");
        return BOARD_FAIL;
    }
...
OAL_STATIC int32_t download_cust_init(void)
{
    int32_t ret;
...
STATIC int32_t mpxx_board_probe(struct platform_device *pdev)
{
    ...
    if (download_cust_init() != BOARD_SUCC) {
        goto err_init;
    }
    ...
    bt_cali_data_ops_init();
    /* 关机找回初始化，因为会用到GPIO的接口，需在GPIO初始化之后才能调用 */
    off_find_init(pdev);
...
mpxx_release_vtype hi110x_get_release_type(void)
{
    ...
    vtype = (mpxx_release_vtype)extern_get_logusertype_flag();
