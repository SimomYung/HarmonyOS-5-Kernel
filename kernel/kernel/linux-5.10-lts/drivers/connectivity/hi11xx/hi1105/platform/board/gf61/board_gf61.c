...
/* gf61 cfg files */
STATIC cfg_map_info g_gf61_asic_cfg_patch_in_vendor[] = {
    {
        ...
        .idx = GT_CFG,
        .cfg_path = GT_CFG_FILE
    }, {
        .idx = G_CFG,
        .cfg_path = SLE_CFG_FILE
    }
};

chip_firmware_cfg_str g_gf61_cfg_str[] = {
    {
        .chip_nr = BOARD_VERSION_GF61,
        .is_asic = VERSION_ASIC,
        .cfg_map_size = oal_array_size(g_gf61_asic_cfg_patch_in_vendor),
        .cfg_map = g_gf61_asic_cfg_patch_in_vendor,
        .pre_directory = FIRMWARE_GF61_CFG_DIR
    }
};

OAL_STATIC void gf61_set_cfg_path_null(uint32_t cfg_type)
{
    uint32_t i;

    for (i = 0; i < oal_array_size(g_gf61_asic_cfg_patch_in_vendor); i++) {
        if (cfg_type == g_gf61_asic_cfg_patch_in_vendor[i].idx) {
            g_gf61_asic_cfg_patch_in_vendor[i].cfg_path = NULL;
            break;
        }
    }
}

int32_t gf61_board_gpio_init(struct platform_device *pdev)
{
    oal_reference(pdev);
    if (is_wifi_support() == OAL_FALSE) {
        gf61_set_cfg_path_null(WIFI_CFG);
    }

    if (is_bfgx_support() == OAL_FALSE) {
        gf61_set_cfg_path_null(BFGX_CFG);
        gf61_set_cfg_path_null(G_CFG);
    }

    if (is_gt_support() == OAL_FALSE) {
        gf61_set_cfg_path_null(GT_CFG);
    }

    if ((is_wifi_support() == OAL_FALSE) && (is_bfgx_support() == OAL_FALSE)) {
        gf61_set_cfg_path_null(BFGX_AND_WIFI_CFG);
    }
    return 0;
}
...
{
    int32_t ret;
    uint32_t which_cfg = (sys == B_SYS) ? BFGX_CFG : G_CFG;
...
static int32_t hwifi_panic_handler(struct notifier_block *this, unsigned long event, void *unused)
{
    ps_print_info("%s Triggering a panic, The gf61 is to be powered off\n", __FUNCTION__);

    conn_set_gpio_level(BFGX_POWER, GPIO_LEVEL_LOW);
    ps_print_info("%s GPIO BFGX_EN down\n", __FUNCTION__);

    conn_set_gpio_level(GT_POWER, GPIO_LEVEL_LOW);
    ps_print_info("%s GPIO SLB_EN down\n", __FUNCTION__);

    conn_set_gpio_level(W_POWER, GPIO_LEVEL_LOW);
    ps_print_info("%s GPIO WIFI_EN down\n", __FUNCTION__);

    conn_set_gpio_level(CHIP_POWR, GPIO_LEVEL_LOW);
    ps_print_info("%s GPIO PWR_EN down\n", __FUNCTION__);
    ps_print_info("%s gf61 power off finish\n", __FUNCTION__);
...
{
    g_st_board_info.bd_ops.board_gpio_init = gf61_board_gpio_init;
...
#ifdef CONFIG_ARCH_KIRIN_PCIE
    atomic_notifier_chain_register(&panic_notifier_list, &g_hwifi_panic_notifier);
#endif
...
int32_t board_info_init_gf61(void)
{
    board_callback_init_dts();
    board_callback_init_gf61_power();
    plat_cust_init();
    chip_firmware_cfg_register(g_gf61_cfg_str, oal_array_size(g_gf61_cfg_str));
    return BOARD_SUCC;
}
...
#if defined(_PRE_PRODUCT_SYLINCOM) && defined(MODULE_IMPORT_NS)
