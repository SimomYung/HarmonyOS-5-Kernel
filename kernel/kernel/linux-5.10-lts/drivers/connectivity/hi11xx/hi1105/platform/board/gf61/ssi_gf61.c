...
static int gf61_ssi_device_regs_dump(unsigned long long module_set)
{
    ...
    if (ret != OAL_SUCC) {
        ...
        module_set_real = SSI_MODULE_MASK_COMM;
    } else {
        ssi_check_buck_scp_ocp_status();
    }

    ret = regs_dump(module_set_real);
    if (ret != OAL_SUCC) {
    ...
    if (SSI_DUMP_LIMIT_TIMEOUT - (time_cost_var_sub(ssi_timestamp)) > USEC_PER_SEC) {
        ...
        ssi_dump_device_key_mem(module_set_real, (int32_t)dump_trace_cnt);
    }
    oal_get_time_cost_end(ssi_timestamp);
    oal_calc_time_cost_sub(ssi_timestamp);
...
int gf61_ssi_config(unsigned long long module_set)
{
    ...
    return gf61_ssi_device_regs_dump(module_set);
}
