...
STATIC cfg_map_info g_mp13_pilot_cfg_patch_in_vendor[] = {
    {
        .idx = BFGX_AND_WIFI_CFG,
        .cfg_path = BFGX_AND_WIFI_CFG_MP13_PILOT_PATH
    }, {
        .idx = WIFI_CFG,
        .cfg_path = WIFI_CFG_MP13_PILOT_PATH
    }, {
        .idx = BFGX_CFG,
        .cfg_path = BFGX_CFG_MP13_PILOT_PATH
    },
};

chip_firmware_cfg_str g_mp13_cfg_str[] = {
    {
        .chip_nr = BOARD_VERSION_MP13,
        .is_asic = VERSION_ASIC,
        .cfg_map_size = oal_array_size(g_mp13_pilot_cfg_patch_in_vendor),
        .cfg_map = g_mp13_pilot_cfg_patch_in_vendor,
        .pre_directory = NULL
    }
};
...
STATIC int32_t mp13_wlan_firmware_download(void)
{
    ...
    if (bfgx_is_shutdown() && (mpxx_firmware_download_mode() == MODE_COMBO)) {
        ...
        error = firmware_download_function(WIFI_COMB_CFG, hcc_get_bus(HCC_EP_WIFI_DEV));
#else
...
void board_plat_cust_init_mp13(void)
{
    int32_t ret;
...
int32_t board_info_init_mp13(void)
{
...
#endif
    board_clk_init(DET_NONE, CLK_INIT_NONE);
    chip_firmware_cfg_register(g_mp13_cfg_str, oal_array_size(g_mp13_cfg_str));
    return BOARD_SUCC;
}
