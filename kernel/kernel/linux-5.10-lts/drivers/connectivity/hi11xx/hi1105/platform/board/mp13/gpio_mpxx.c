...
STATIC int32_t hi_drv_get_gpio_level(struct conn_gpio_node *node)
{
    return (int32_t)hitv_gpio_get_value(node->pin.number);
}
...
STATIC int32_t conn_get_gpio_nr_from_gpiolib(struct conn_gpio_node *gpio_node)
{
    ...
    struct platform_device *pdev = (struct platform_device *)conn_gpio_dev_get();
    if (pdev == NULL) {
        return  -OAL_FAIL;
    }
...
/* kungpeng的GPIO号采用GPIOLIB的接口获取，request的接口还和110x其它项目一样的内核接口 */
STATIC int32_t kungpeng_gpio_init(struct conn_gpio_node *node)
{
    int32_t ret;
#ifdef CONFIG_GPIOLIB
    if (gpio_node->attr.mode == MODE_GPIOLIB) {
...
STATIC void kungpeng_gpio_uninit(struct conn_gpio_node *node)
{
#ifdef CONFIG_GPIOLIB
    struct platform_device *pdev = (struct platform_device *)conn_gpio_dev_get();
#endif
...
#ifdef CONFIG_GPIOLIB
    if (node->attr.mode == MODE_GPIOLIB && pdev != NULL) {
        devm_gpiod_put(&pdev->dev, node->attr.u.gpiod.desc);
    }
#endif
}


STATIC struct conn_gpio_ops g_kunpeng_tv_ops = {
    .gpio_init = kungpeng_gpio_init,
    .gpio_uninit = kungpeng_gpio_uninit,
};
...
STATIC struct conn_gpio_chip g_kunpeng_drv_gpio_chip = {
    ...
    .pub_ops = &g_kunpeng_tv_ops
};

#elif defined(_PRE_WLAN_EXPORT)
#ifndef _PRE_CONFIG_USE_DTS
#define POWER_ON_ENABLE_GPIO            (4 * 8 + 4)    /* POWER_ON          GPIO_36 */
#define WLAN_POWER_ON_ENABLE_GPIO       (5 * 8 + 6)    /* WLAN_POWER_ON     GPIO_46 */
#define BFGX_POWER_ON_ENABLE_GPIO       (5 * 8 + 2)    /* BFGX_POWER_ON     GPIO_42 */
#define WLAN_WAKEUP_HOST_GPIO           (6 * 8 + 4)    /* WLAN_WAKEUP_HOST  GPIO_52 */
#define HOST_WAKEUP_WLAN_GPIO           (4 * 8 + 6)    /* HOST_WAKEUP_WLAN  GPIO_38 */
#define BFGX_WAKEUP_HOST_GPIO           (4 * 8 + 7)    /* BFGX_WAKEUP_HOST  GPIO_39 */
#endif

STATIC struct conn_gpio_node g_export_drv_gpio_node[] = {
    {
        .pin = {
            .name = PROC_NAME_GPIO_POWEN_ON,
            .is_key_gpio = OAL_TRUE,
...
#ifndef _PRE_CONFIG_USE_DTS
            .number = BFGX_POWER_ON_ENABLE_GPIO,
#endif
            .init = {
                .req_sts = REQ_ONE,
                .flags = GPIOF_OUT_INIT_LOW
...
#ifdef _PRE_CONFIG_I2C_SUPPORT
    {
        .pin = {
            .name = PROC_NAME_GPIO_I2C_WAKEUP_HOST,
            .is_key_gpio = OAL_TRUE,
            .type = I2C_WKUP_HOST,
            .init = {
                .req_sts = REQ_ONE,
                .flags = GPIOF_DIR_IN | GPIOF_INIT_LOW
            },
        },
        .attr = {
            .mode = MODE_DTS,
            .u = {
                .dts = {
                    .compatibale = DTS_NODE_MPXX_BFGX,
                    .label_name = DTS_PROP_MPXX_GPIO_I2C_WAKEUP_HOST
                }
            },
        },
        .irq = {
            .name = "i2c_wake_host",
            .flags = IRQF_NO_SUSPEND | IRQF_TRIGGER_HIGH | IRQF_DISABLED,
            .init_set_enable = OAL_FALSE,
        },
    },
#endif
...
STATIC struct conn_gpio_chip g_export_drv_gpio_chip = {
    .node = g_export_drv_gpio_node,
    .node_num = oal_array_size(g_export_drv_gpio_node),
};
...
#elif defined(_PRE_WLAN_EXPORT)
    gpio_chip = &g_export_drv_gpio_chip;
#else
