...
#define MP13_GLB_CTL_SYS_TICK_VALUE_W_1_REG  (MP13_GLB_CTL_BASE + 0xD4)
...
#define MP13_GLB_CTL_GLB_CTL_BCPU_LOAD_OK_MSK 0x2
#define MP13_GLB_CTL_GLB_CTL_WCPU_LOAD_OK_MSK 0x2
...
static ssi_reg_info g_mp15_b_except_mem  =    {0x80217574, 0x0, 0x64, SSI_RW_DWORD_MOD};
...
static uint32_t ssi_read_systick(void)
{
    ...
    v1 = (uint16_t)ssi_read32(MP13_GLB_CTL_SYS_TICK_VALUE_W_0_REG);
    v2 = (uint16_t)ssi_read32(MP13_GLB_CTL_SYS_TICK_VALUE_W_1_REG);
...
static int ssi_detect_tcxo_is_normal(void)
{
    ...
    if (sys_tick_new == sys_tick_old) {
        ...
        clock_32k = ((sys_tick_new - sys_tick_old) * 1000) / (uint32_t)us_to_s; /* 1000 is mean hz to KHz */
        ps_print_err("32k runtime:%llu us , sys_tick:%u\n", us_to_s, (sys_tick_new - sys_tick_old));
...
static int ssi_read_wpcu_pc_lr_sp(int trace_en)
{
    int i;
    uint32_t pc, lr, sp;
    uint64_t wait_times = 10; // 锁存cpu状态超过10ms，不再等待
    ktime_t start_time, end_time, load_time;
    ...
    for (i = 0; i < SSI_CPU_ARM_REG_DUMP_CNT; i++) {
        ...
        for (;;) {
            if ((ssi_read32(MP13_GLB_CTL_WCPU_LOAD_REG) & MP13_GLB_CTL_GLB_CTL_WCPU_LOAD_OK_MSK) ==
                MP13_GLB_CTL_GLB_CTL_WCPU_LOAD_OK_MSK) {
                break;
            }

            end_time = ktime_get();
            load_time = ktime_sub(end_time, start_time);
            if ((uint64_t)ktime_to_ms(load_time) > wait_times) {
                ps_print_err("cpu load not ok\n");
                break;
            }
            oal_udelay(10);  // 10us读一次锁存状态
        }
...
static int ssi_read_bpcu_pc_lr_sp(int trace_en)
{
    ...
    for (i = 0; i < SSI_CPU_ARM_REG_DUMP_CNT; i++) {
        ...
        start_time = ktime_get();
        for (;;) {
            if ((ssi_read32(MP13_GLB_CTL_BCPU_LOAD_REG) & MP13_GLB_CTL_GLB_CTL_BCPU_LOAD_OK_MSK) ==
                MP13_GLB_CTL_GLB_CTL_BCPU_LOAD_OK_MSK) {
                break;
            }
...
static int ssi_dump_device_bctl_and_exception_regs(unsigned long long module_set)
{
    ...
    if (module_set & SSI_MODULE_MASK_BCPU_EXCEPT_MEM) {
        if (ssi_check_bcpu_is_working()) {
            ret = ssi_read_reg_info(&g_mp15_b_except_mem, NULL, 0, g_ssi_is_logfile);
            if (ret) {
...
STATIC int ssi_device_regs_dump(unsigned long long module_set)
{
    int ret;
...
int ssi_default_config(unsigned long long module_set)
{
    ...
    return ssi_device_regs_dump(module_set);
}

/*
 * SSI寄存器打印，根据发生错误的业务场景，
 * 区分SSI打印的寄存器大小和打印频率，尽量减少对业务的影响
 * 区分user/eng ，区分user beta 和 user release版本
...
OAL_STATIC unsigned long long ssi_get_module_bfg_wkup_fail(ssi_err_type etype)
{
    ...
    full_flag = (SSI_MODULE_MASK_ARM_REG | SSI_MODULE_MASK_AON | SSI_MODULE_MASK_COEX_CTL | SSI_MODULE_MASK_BFGXCTRL |
                 SSI_MODULE_MASK_BTRACE | SSI_MODULE_MASK_GTRACE | SSI_MODULE_MASK_PMU | SSI_MODULE_MASK_PM_TRACE);
...
OAL_STATIC unsigned long long ssi_proc_fw_down_fail(ssi_err_type etype, int32_t is_sdio)
{
    ...
    if (vtype == MPXX_VTYPE_DEBUG) {
        ...
        if (is_sdio == OAL_TRUE) {
            ...
            flag |= SSI_MODULE_MASK_PCIE_FULL;
        }
    } else if (vtype == MPXX_VTYPE_RELEASE_DEBUG) {
        flag = SSI_MODULE_MASK_AON | SSI_MODULE_MASK_ARM_REG | SSI_MODULE_MASK_WCTRL | SSI_MODULE_MASK_WTRACE;
        if (is_sdio == OAL_TRUE) {
            flag |= SSI_MODULE_MASK_SDIO;
        } else {
            flag |= SSI_MODULE_MASK_PCIE_FULL;
...
OAL_STATIC unsigned long long ssi_get_module_wlan_custom_cali_fail(ssi_err_type etype)
{
    ...
    err_level = ssi_get_err_dump_level();
    if (err_level != SSI_ERR_LOG_LEVEL_CLOSE) {
        flag = SSI_MODULE_MASK_ARM_REG | SSI_MODULE_MASK_WTRACE | SSI_MODULE_MASK_GPIO;
    }
...
OAL_STATIC unsigned long long ssi_get_module_bfgx_dump_fail(ssi_err_type etype, unsigned long long flag_t)
{
    ...
    if (auto_switch == OAL_TRUE) {
        if (oal_print_rate_limit(30 * PRINT_RATE_SECOND) && (vtype != MPXX_VTYPE_RELEASE)) { /* 30s */
            ps_print_info("bfgx exception ssi dump starts\n");
            if ((etype != SSI_ERR_B_HEART_TIMEOUT) && (etype != SSI_ERR_G_HEART_TIMEOUT)) {
                flag = flag_t | SSI_MODULE_MASK_BTRACE | SSI_MODULE_MASK_GTRACE | SSI_MODULE_MASK_PM_TRACE;
            } else {
                flag = flag_t | SSI_MODULE_MASK_PM_TRACE;
            }
        }
    } else {
...
OAL_STATIC unsigned long long ssi_get_module_bfgx_open_fail(ssi_err_type etype)
{
    return ssi_get_module_bfgx_dump_fail(etype, SSI_MODULE_MASK_AON | SSI_MODULE_MASK_ARM_REG | \
                                         SSI_MODULE_MASK_BFGXCTRL | SSI_MODULE_MASK_PMU | SSI_MODULE_MASK_GPIO);
}

OAL_STATIC unsigned long long ssi_get_module_b_hb_timeout_fail(ssi_err_type etype)
{
    return ssi_get_module_bfgx_dump_fail(etype,
        (SSI_MODULE_MASK_AON | SSI_MODULE_MASK_ARM_REG| SSI_MODULE_MASK_PMU | SSI_MODULE_MASK_BTRACE |
         SSI_MODULE_MASK_BFGXCTRL | SSI_MODULE_MASK_BCPU_EXCEPT_MEM));
}

OAL_STATIC unsigned long long ssi_get_module_g_hb_timeout_fail(ssi_err_type etype)
{
    return ssi_get_module_bfgx_dump_fail(etype,
        (SSI_MODULE_MASK_AON | SSI_MODULE_MASK_ARM_REG| SSI_MODULE_MASK_PMU | SSI_MODULE_MASK_GTRACE |
         SSI_MODULE_MASK_GFCTRL | SSI_MODULE_MASK_SLECTRL));
}

OAL_STATIC unsigned long long ssi_get_module_bg_panic(ssi_err_type etype)
{
    mpxx_release_vtype vtype = hi110x_get_release_type();
    if (vtype == MPXX_VTYPE_RELEASE) {
        return 0;
    ...
    if (etype == SSI_ERR_B_TRIGER_PANIC) {
        return SSI_MODULE_MASK_TRG_B_PANIC;
    } else if (etype == SSI_ERR_G_TRIGER_PANIC) {
        return SSI_MODULE_MASK_TRG_G_PANIC;
    }

    return 0;
}

OAL_STATIC uint32_t ssi_get_control_bg_panic_flg(ssi_err_type etype)
{
    oal_reference(etype);
    return OAL_FALSE;
}
...
OAL_STATIC ssi_err_stru g_ssi_err_infos[] = {
    declare_ssi_err_info(SSI_ERR_COMM, ssi_get_module_comm, NULL), /* 1st fixed */
    ...
    declare_ssi_err_info(SSI_ERR_B_HEART_TIMEOUT, ssi_get_module_b_hb_timeout_fail, NULL),
    declare_ssi_err_info(SSI_ERR_G_HEART_TIMEOUT, ssi_get_module_g_hb_timeout_fail, NULL),
    declare_ssi_err_info(SSI_ERR_B_TRIGER_PANIC, ssi_get_module_bg_panic, ssi_get_control_bg_panic_flg),
    declare_ssi_err_info(SSI_ERR_G_TRIGER_PANIC, ssi_get_module_bg_panic, ssi_get_control_bg_panic_flg),
};
...
int ssi_dump_err_reg(ssi_err_type etype)
{
    ...
    if (pst_err_info->ssi_get_control_flag != NULL) {
        flags = pst_err_info->ssi_get_control_flag(pst_err_info->etype);
    }

    ret = ssi_dump_device_regs_handler(module_set, flags);
