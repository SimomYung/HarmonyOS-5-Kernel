...
#define GLB_CTL_SYS_TICK_VALUE_W_1_REG  (GLB_CTL_BASE + 0xD4)
...
#define GLB_CTL_SYS_TICK_VALUE_B_1_REG  (GLB_CTL_BASE + 0xF4)
...
static ssi_reg_info g_wcpu_ram_exception_mesg = { 0x1abd00, 0x0, 0x200,  SSI_RW_DWORD_MOD };
...
static void ssi_dump_wcpu_ram_exception_mem(void)
{
    ...
    if (ssi_check_wcpu_is_working()) {
        ret = ssi_read_reg_info(&g_wcpu_ram_exception_mesg, NULL, 0, g_ssi_is_logfile);
        if (ret != 0) {
...
static int mp16_ssi_device_regs_dump(unsigned long long module_set)
{
    int ret;
    g_halt_det_cnt = 0;
...
int mp16_ssi_config(unsigned long long module_set)
{
    ...
    return mp16_ssi_device_regs_dump(module_set);
}
