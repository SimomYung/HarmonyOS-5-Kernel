...
#define WLAN_WAKE_DEV_MIN_TIME_MP16C   8000            /* 设置唤醒dev最小等待时间 */
...
/* mp16c */
STATIC cfg_map_info g_mp16c_cfg_patch_in_vendor[] = {
    {
...
chip_firmware_cfg_str g_mp16c_cfg_str[] = {
    {
        .chip_nr = BOARD_VERSION_MP16C,
        .is_asic = VERSION_FPGA,
        .cfg_map_size = oal_array_size(g_mp16c_cfg_patch_in_vendor),
        .cfg_map = g_mp16c_cfg_patch_in_vendor,
        .pre_directory = FIRMWARE_MP16C_FPGA_CFG_DIR
    }, {
        .chip_nr = BOARD_VERSION_MP16C,
        .is_asic = VERSION_ASIC,
        .cfg_map_size = oal_array_size(g_mp16c_cfg_patch_in_vendor),
        .cfg_map = g_mp16c_cfg_patch_in_vendor,
        .pre_directory = FIRMWARE_MP16C_PILOT_CFG_DIR
    }
};
...
#ifndef BFGX_UART_DOWNLOAD_SUPPORT
STATIC int32_t mp16c_bfgx_download_handler(struct ps_core_s *ps_core_d, uint32_t cfg, uint32_t sys)
{
    int32_t ret;
    int32_t try_cnt = 1;
...
STATIC int32_t mp16c_bfgx_download_by_wifi(struct ps_core_s *ps_core_d, uint32_t sys)
{
    ...
    if (mp16c_bfgx_download_handler(ps_core_d, whitch_cfg, sys) != OAL_SUCC) {
...
#endif
    if (mpxx_firmware_download_mode() == MODE_COMBO) {
        (void)mp16c_board_power_off(B_SYS);
        (void)mp16c_board_power_off(G_SYS);
    } else {
...
STATIC void mp16c_power_state_change(int32_t type, int32_t flag)
{
    if (flag == BOARD_POWER_ON) {
        ...
        if (get_mpxx_subchip_version() == CHIP_VERSION_MPW) {
            mdelay(50); /* mpw 延时50ms 等待rfldo上电稳定 */
        }
...
STATIC void board_callback_init_mp16c_power(void)
{
    ...
    g_st_board_info.bd_ops.power_state_change = mp16c_power_state_change;
}
...
STATIC void board_plat_parameter_init_mp16c(void)
{
    g_wake_dev_min_time = WLAN_WAKE_DEV_MIN_TIME_MP16C;
    ps_print_info("g_wake_dev_min_time: %u\n",  g_wake_dev_min_time);
}

int32_t board_info_init_mp16c(void)
{
    board_callback_init_dts();
    board_callback_init_mp16c_power();
    board_plat_parameter_init_mp16c();
    board_plat_cust_init_mp16c();
    if (get_mpxx_subchip_version() == CHIP_VERSION_MPW) {
        board_clk_init(DET_BY_UART, CLK_INIT_DTS);
    } else {
        board_clk_init(DET_BY_UART, (CLK_INIT_INI | CLK_INIT_DTS));
    }
    chip_firmware_cfg_register(g_mp16c_cfg_str, oal_array_size(g_mp16c_cfg_str));
    return BOARD_SUCC;
}
