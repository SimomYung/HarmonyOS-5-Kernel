...
STATIC struct gpio_gugong g_gpios_tas[GPS_TAS_GPIO_NUM] = {
    {
        .dts_type = "gps_tas1",
...
uint32_t g_gugong_bbst_vout_inited = OAL_FALSE;

STATIC void tas_gpio_init(struct platform_device *pdev)
{
    ...
    for (index_gpio = 0, index_dts = 0; index_gpio < GPS_TAS_GPIO_NUM; index_gpio++) {
...
STATIC void gugong_voltage_init(struct device_node *of_node)
{
    ...
    g_gugong_bbst_vout_inited = OAL_TRUE;
}
...
STATIC void gugong_pa_gpio_off(void)
{
    ...
    if (g_gpios_gugong[BBST_EN].inited == OAL_TRUE &&
        ...
        gpio_direction_output(g_gpios_gugong[BBST_VOUT].number,
            (g_gpios_gugong[BBST_VOUT].init_mode == 0) ? GPIO_LEVEL_LOW : GPIO_LEVEL_HIGH);
    } else if (g_gugong_bbst_vout_inited == OAL_TRUE) {
#ifdef CONFIG_HUAWEI_LOW_POWER
...
/* gpio350   gpio351    status      场景
      ...
      1         1         2        仙人掌
 */
void tas_gpio_conf(uint8_t status)
{
...
STATIC void tas_gps_gpio_deinit(void)
{
    int32_t index_gpio;

    for (index_gpio = 0; index_gpio < GPS_TAS_GPIO_NUM; index_gpio++) {
