...
#define GLB_CTL_SYS_TICK_VALUE_W_1_REG            (GLB_CTL_BASE + 0x108)
...
#define GLB_CTL_SYS_TICK_VALUE_B_1_REG            (GLB_CTL_BASE + 0x11c)
...
#define GLB_CTL_SYS_TICK_VALUE_GF_1_REG           (GLB_CTL_BASE + 0x188)
...
#define GLB_CTL_SYS_TICK_VALUE_SLE_1_REG          (GLB_CTL_BASE + 0x19c)
...
#define MP16C_GLB_CTL_RB_B_SOFT_INT_SET_REG             (GLB_CTL_BASE + 0x144)
#define MP16C_GLB_CTL_RB_B_SOFT_INT_STS_REG             (GLB_CTL_BASE + 0x14C)
#define MP16C_B_SOFT_INT1_SET_AON_MASK                  0x2     // 使用软中断1
#define MP16C_GLB_CTL_RB_GLE_SOFT_INT_SET_REG           (GLB_CTL_BASE + 0x174)
#define MP16C_GLB_CTL_RB_GLE_SOFT_INT_STS_REG           (GLB_CTL_BASE + 0x17C)
#define MP16C_GLE_SOFT_INT1_SET_AON_MASK                0x2     // 使用软中断1
#define MP16C_GLB_CTL_RB_GLB_CTL_GP_REG17_REG           (GLB_CTL_BASE + 0x1C14)  /* 触发GLE核异常类型 */
#define GLE_EXCEPT_SSI_TRIGGER                           (3)
...
static ssi_reg_info g_coex_ctl_part1 = { 0x40011000, 0x0, 0x60,  SSI_RW_WORD_MOD }; // 仅GP REG
static ssi_reg_info g_ipc_ctrl_reg = { 0x4000e000, 0x0, 0x200,  SSI_RW_WORD_MOD };  // 到邮箱状态为止
static ssi_reg_info g_gf_ctrl_part1 = { 0x43000000, 0x0, 0xa4,  SSI_RW_WORD_MOD };  // 到时钟分频配置为止
static ssi_reg_info g_gf_diag_ctrl_part1 = { 0x43005490, 0x0, 0x10,  SSI_RW_WORD_MOD };
static ssi_reg_info g_gf_rf_abb_ctrl_full = { 0x43016000, 0x0, 0x20,  SSI_RW_WORD_MOD };
static ssi_reg_info g_b_ctrl_full = { 0x41000000, 0x0, 0xb4,  SSI_RW_WORD_MOD };
static ssi_reg_info g_bf_diag_ctrl_part1 = { 0x41006490, 0x0, 0x10,  SSI_RW_WORD_MOD };
static ssi_reg_info g_bf_rf_abb_ctrl_part1 = { 0x4101a000, 0x30, 0x30,  SSI_RW_WORD_MOD };
...
static ssi_reg_info g_b_pll_ctrl_full = { 0x41013000, 0x74, 0x24,  SSI_RW_WORD_MOD }; // 仅mdll寄存器
static ssi_reg_info g_w_ctrl_part1 = { 0x40105000, 0x108, 0x10,  SSI_RW_WORD_MOD };   // GP REG
static ssi_reg_info g_w_ctrl_part2 = { 0x40105000, 0x200, 0x60,  SSI_RW_WORD_MOD };
...
static ssi_reg_info g_sle_ctrl_part1 = { 0x42000000, 0x0, 0x20,  SSI_RW_WORD_MOD }; // gp reg
static ssi_reg_info g_sle_ctrl_part2 = { 0x42000000, 0x400, 0x3c,  SSI_RW_WORD_MOD }; // 仅crg相关
static ssi_reg_info g_sle_diag_ctrl_part1 = { 0x42005420, 0x0, 0x30,  SSI_RW_WORD_MOD };
static ssi_reg_info g_sle_rf_abb_ctrl_full = { 0x42012000, 0x0, 0x20,  SSI_RW_WORD_MOD };
static ssi_reg_info g_sle_pll_ctrl_full = { 0x42011000, 0x74, 0x24,  SSI_RW_WORD_MOD }; // 仅mdll寄存器
...
static ssi_reg_info g_b_uart_dma_status = { 0x447027d4, 0x0, 0x4, SSI_RW_DWORD_MOD};
static ssi_reg_info g_sle_int_dis_cnt  = { 0x44464748, 0x0, 0x4, SSI_RW_DWORD_MOD};
static ssi_reg_info g_sle_uart_dma_status = { 0x44464a2c, 0x0, 0x4, SSI_RW_DWORD_MOD};

static ssi_reg_info g_host_allow_sleep = { 0x44707694, 0x0, 0x4, SSI_RW_DWORD_MOD};
static ssi_reg_info g_uart_isr_count   = { 0x44707d28, 0x0, 0x4, SSI_RW_DWORD_MOD};
static ssi_reg_info g_b_uart_header_mem = { 0x447037e8, 0x0, 0x10, SSI_RW_DWORD_MOD};
static ssi_reg_info g_b_uart_payload_mem = { 0x44702571, 0x0, 0x10, SSI_RW_DWORD_MOD};
...
static ssi_reg_info g_sle_ram_log_mesg = { 0x44460000, 0x6280, 0x23c0,  SSI_RW_DWORD_MOD };
...
static ssi_reg_info *g_aon_reg_cut[] = {
    &g_glb_ctrl_full,
    &g_glb_ctrl_extend1,
    &g_glb_ctrl_extend2,
    &g_glb_ctrl_extend3,
...
static uint32_t g_normal_dump = OAL_FALSE;
...
static void dsm_cpu_info_dump(void)
{
    ...
    /* dsm cpu信息上报 */
    if (g_halt_det_cnt || g_mpxx_kernel_crash || g_normal_dump) {
        ps_print_info("g_halt_det_cnt=%u g_mpxx_kernel_crash=%d, g_normal_dump=%d dsm_cpu_info_dump return\n",
                      g_halt_det_cnt, g_mpxx_kernel_crash, g_normal_dump);
        return;
    }
...
{
    int ret;
    uint32_t wifi_gpio_wake_debug = 0x400008c0;  // wifi唤醒失败debug寄存器

    ps_print_info("wifi wkup host gpio level: %d, debug value:0x%x\n",
                  conn_get_gpio_level(W_WKUP_HOST), ssi_read32(wifi_gpio_wake_debug));
...
static int ssi_dump_device_aon_regs(unsigned long long module_set)
{
    ...
    if (module_set & SSI_MODULE_MASK_AON_CUT) {
        ret = ssi_read_reg_info_arry(g_aon_reg_cut, sizeof(g_aon_reg_cut) / sizeof(ssi_reg_info *),
                                     g_ssi_is_logfile);
        if (ret) {
            return -OAL_EFAIL;
        }
    ...
    if (module_set & SSI_MODULE_MASK_GPIO) {
        ssi_dump_gpio_regs();
    }
...
static int32_t ssi_dump_power_status_regs(void)
{
    int32_t ret, i;
    uint32_t data;
    uint32_t pmu_error_report = OAL_FALSE;

    /* config spmi pinmux */
    ssi_write_dword(GLB_CTL_RB_PINMUX_CFG_3_REG, 0x1);
    ps_print_err("6506 NP_EVENT IRQ_STS DUMP\n");
    ...
    for (i = 0; i < oal_array_size(pmu_regs_arr); i++) {
        data = mp16c_ssi_spmi_read(pmu_regs_arr[i][0]);
        if (data == 0xFFFFFFFF) {
#if defined(CONFIG_HUAWEI_OHOS_DSM) || defined(CONFIG_HUAWEI_DSM)
            ...
            /* pmu_regs_arr中仅读NP EVENT和PMU中断,任何非0代表可能有异常，DSM上报大数据 */
            if (data != 0) {
                pmu_error_report = OAL_TRUE;
            }
        }
    }
    if (pmu_error_report == OAL_TRUE) {
...
static uint32_t ssi_read_systick(void)
{
    uint16_t v1 = 0;
    uint16_t v2 = 0;
    if (ssi_check_cpu_is_working(WCPU)) {
        v1 = (uint16_t)ssi_read32(GLB_CTL_SYS_TICK_VALUE_W_0_REG);
        v2 = (uint16_t)ssi_read32(GLB_CTL_SYS_TICK_VALUE_W_1_REG);
    } else if (ssi_check_cpu_is_working(BCPU)) {
        v1 = (uint16_t)ssi_read32(GLB_CTL_SYS_TICK_VALUE_B_0_REG);
        v2 = (uint16_t)ssi_read32(GLB_CTL_SYS_TICK_VALUE_B_1_REG);
    } else if (ssi_check_cpu_is_working(GCPU)) {
        v1 = (uint16_t)ssi_read32(GLB_CTL_SYS_TICK_VALUE_GF_0_REG);
        v2 = (uint16_t)ssi_read32(GLB_CTL_SYS_TICK_VALUE_GF_1_REG);
    } else {
        v1 = (uint16_t)ssi_read32(GLB_CTL_SYS_TICK_VALUE_SLE_0_REG);
        v2 = (uint16_t)ssi_read32(GLB_CTL_SYS_TICK_VALUE_SLE_1_REG);
    }
    return (((uint32_t)v2 << 16) | (uint32_t)v1); // v2高16位
}
...
static int ssi_detect_32k_handle(uint32_t sys_tick_old, uint64_t cost_time_s, uint32_t *clock_32k)
{
    ...
    if (sys_tick_new == sys_tick_old) {
        ...
        cost_time_s += 4000; /* 经验值,误差4000us */
        (*clock_32k) = ((sys_tick_new - sys_tick_old) * 1000) / (uint32_t)cost_time_s; /* 1000 is mean hz to KHz */
...
void ssi_dump_sle_ram_log_mem(void)
{
    int ret;
    uint32_t base_addr = 0x44460000;
    uint32_t head_offset = 0x6278;
    uint32_t tail_offset = 0x627c;
    uint32_t allow_sleep_addr = 0x40002234;
    uint32_t soft_wakeup_addr = 0x40000174;
    ...
    if (ssi_check_cpu_is_working(SLE)) {
        ...
        ps_print_info("sle ram log dump start\n");
        ret = ssi_read_reg_info(&g_sle_ram_log_mesg, NULL, 0, 0);
        if (ret != 0) {
            ps_print_info("sle ram log read failed\n");
            return;
        }
        ps_print_info("sle ram log dump end\n");
    } else {
        ps_print_info("sle ram log can't dump, sle down\n");
    }
}

static void ssi_dump_off_find_log(unsigned long long module_set)
{
    if (module_set & SSI_MODULE_MASK_OFF_FIND_LOG) {
        /* off_find开机dump维测信息,不属于异常,不上报bbox */
        g_normal_dump = OAL_TRUE;
        ssi_dump_sle_ram_log_mem();
    }
}
...
static void ssi_dump_ram_exception_msg(unsigned long long module_set)
{
    ...
    ssi_dump_off_find_log(module_set);
}
...
static int32_t mp16c_ssi_check_valid(unsigned long long *reg_set)
{
    int32_t ret;
    ...
    if (ret) {
        ...
        (void)ssi_write16(gpio_ssi_reg(MP16C_SSI_SEL_CTRL), 0x180);
        (void)ssi_write16(gpio_ssi_reg(MP16C_SSI_SSI_CTRL), 0x180);
        ssi_switch_clk(SSI_AON_CLKSEL_SSI);
...
static int mp16c_ssi_device_regs_dump(unsigned long long module_set)
{
    int ret;
    ...
    ret = mp16c_ssi_check_valid(&reg_set);
    if (ret != OAL_SUCC) {
        goto ssi_fail;
    }
...
ssi_fail:
    ssi_switch_clk(SSI_AON_CLKSEL_TCXO);
    dsm_cpu_info_dump();
    g_normal_dump = OAL_FALSE;
    return ret;
}

STATIC int32_t mp16c_ssi_trigger_panic(unsigned long long module_set)
{
    uint32_t ret;
    ...
    if ((module_set & SSI_MODULE_MASK_TRG_B_PANIC) == SSI_MODULE_MASK_TRG_B_PANIC) {
        ret = ssi_write_word(MP16C_GLB_CTL_RB_B_SOFT_INT_SET_REG, MP16C_B_SOFT_INT1_SET_AON_MASK);
        if (ret == 0xffffffff) {
        ...
        ps_print_info("read b soft intr sts 0x%x\n", ssi_read_word(MP16C_GLB_CTL_RB_B_SOFT_INT_STS_REG));
        return OAL_SUCC;
    } else if ((module_set & SSI_MODULE_MASK_TRG_G_PANIC) == SSI_MODULE_MASK_TRG_G_PANIC) {
        // 软中断1复用，GP_REG17中写入软中断1触发类型
        ret = ssi_write_word(MP16C_GLB_CTL_RB_GLB_CTL_GP_REG17_REG, GLE_EXCEPT_SSI_TRIGGER);
        if (ret == 0xffffffff) {
            ps_print_err("ssi set g soft intr type fail\n");
            return -OAL_EFAIL;
        }

        ret = ssi_write_word(MP16C_GLB_CTL_RB_GLE_SOFT_INT_SET_REG, MP16C_GLE_SOFT_INT1_SET_AON_MASK);
        if (ret == 0xffffffff) {
        ...
        oal_udelay(50); // 50us 等待device 中断响应完成，后面可以check 是否响应完中断
        ps_print_info("read g soft intr sts 0x%x\n", ssi_read_word(MP16C_GLB_CTL_RB_GLE_SOFT_INT_STS_REG));
        return OAL_SUCC;
    }
...
int mp16c_ssi_config(unsigned long long module_set)
{
    ...
    if (temp_module_set != 0) {
        ps_print_info("trigger device panic\n");
        return mp16c_ssi_trigger_panic(temp_module_set);
    }

    return mp16c_ssi_device_regs_dump(module_set);
}
