...
#define INI_MPXX_PMU_VER "mpxx_pmu_ver"
#define PMU_VERSION_LEN    8
#define MPXX_PMU_6506 "6506"
#define MPXX_PMU_6516 "6516"

STATIC uint32_t g_mpxx_pmu_ver = SPMI_SLAVEID_HI6506;

/* gnss不通过上下电进入rsmc，guart使用tcxo倍频,tcxo38.4M场景下使用8倍采样率 */
uint32_t g_bg_tcxo_dll = BIT1; // bit0置1 开启（bcpu&buart）倍频  bit1置1 开启guart倍频（gcpu无倍频配置）
oal_debug_module_param(g_bg_tcxo_dll, uint, S_IRUGO | S_IWUSR);
...
{
    return g_mpxx_pmu_ver;
}

STATIC void mpxx_pmu_init(void)
{
    int32_t ret;

    char pmu_ver[PMU_VERSION_LEN] = {0};

    ret = get_cust_conf_string(INI_MODU_PLAT, INI_MPXX_PMU_VER, pmu_ver, sizeof(pmu_ver));
    if (ret != INI_SUCC) {
        ps_print_warning("%s can not find pmu_ver"NEWLINE,  __func__);
        g_mpxx_pmu_ver = SPMI_SLAVEID_HI6506;
        return;
    }

    if (oal_strncmp(MPXX_PMU_6506, pmu_ver, strlen(MPXX_PMU_6506)) == 0) {
        g_mpxx_pmu_ver = SPMI_SLAVEID_HI6506;
    } else if (oal_strncmp(MPXX_PMU_6516, pmu_ver, strlen(MPXX_PMU_6516)) == 0) {
        g_mpxx_pmu_ver = SPMI_SLAVEID_HI6516;
    } else {
        ps_print_err("invalid para : %s", pmu_ver);
    }

    ps_print_info("pmu version is %d!", g_mpxx_pmu_ver);
}
...
STATIC void board_bfgx_plat_cust_set_mp17c(struct ps_core_s *ps_core_d)
{
    plat_cust_data_mp17c *plat_cust = NULL;
    ...
    plat_cust->cfg.bg_tcxo_dll = (uint8_t)g_bg_tcxo_dll;
    ps_print_info("bg_tcxo_dll set = 0x%x\n", g_bg_tcxo_dll);
}
...
STATIC int32_t mp17c_bfgx_dev_power_on(struct ps_core_s *ps_core_d, uint32_t sys, uint32_t subsys)
{
    ...
    board_bfgx_plat_cust_set_mp17c(ps_core_d);
...
STATIC void mp17c_chip_poweroff_delay(void)
{
    ...
    if (g_mpxx_pmu_ver == SPMI_SLAVEID_HI6506) {
        mdelay(10); /* delay 10ms */
    } else if (g_mpxx_pmu_ver == SPMI_SLAVEID_HI6516) {
        /* PMU(6516)芯片的PMU-HOLD默认为复位模式，默认需要500ms才能再次操作PMU-HOLD；
...
STATIC void board_get_sensorhub_enable(plat_cust_data_mp17c *cust)
{
    int32_t cfg_value;
    int32_t ret;
