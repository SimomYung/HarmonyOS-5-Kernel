...
/*
 * 函 数 名  : mp17c_bfgx_get_nv_data_buf
 * 功能描述  : 返回保存bfgx nv数据的内存地址
 * 输出参数  : nv buf的长度
 * 返 回 值  : bfgx nv数据buf的地址，也可能是NULL
 */
STATIC void *mp17c_bfgx_get_nv_data_buf(uint32_t *pul_len)
{
    mp17c_bfgx_cali_data_stru *pst_bfgx_cali_data_buf = NULL;

    if (g_bfgx_cali_data_buf == NULL) {
        return NULL;
    }

    pst_bfgx_cali_data_buf = (mp17c_bfgx_cali_data_stru *)g_bfgx_cali_data_buf;
...
/*
 * 函 数 名  : mp17c_bfgx_get_ext_nv_data_buf
 * 功能描述  : 返回保存bfgx extended nv数据的内存地址
 * 输出参数  : nv extended buf的长度
 * 返 回 值  : bfgx extended nv数据buf的地址，也可能是NULL
 */
STATIC void *mp17c_bfgx_get_ext_nv_data_buf(uint32_t *pul_len)
{
    mp17c_bfgx_cali_data_stru *pst_bfgx_cali_data_buf = NULL;
...
STATIC void mp17c_plat_bfgx_cali_data_test(void)
{
    mp17c_bfgx_cali_data_stru *pst_cali_data = NULL;
    uint32_t *p_test = NULL;
    uint32_t count, i;

    pst_cali_data = (mp17c_bfgx_cali_data_stru *)g_bfgx_cali_data_buf;
...
/*
 * 函 数 名  : mp17c_get_bfgx_cali_data
 ...
 * 返 回 值  : 0表示成功，-1表示失败
 */
STATIC int32_t mp17c_get_bfgx_cali_data(uint8_t *buf, uint32_t buf_len, uint8_t uart_index)
{
    int32_t ret;
    struct ps_core_s *ps_core_d = NULL;
    struct st_bfgx_data *bfgx_data = NULL;
    mp17c_bfgx_cali_data_stru *bfgx_cali_data_buf = NULL;
    ...
    if (g_cali_buffer_debug != 0) {
        mp17c_plat_bfgx_cali_data_test();
    } else {
        bfgx_cali_data_buf = (mp17c_bfgx_cali_data_stru *)g_bfgx_cali_data_buf;
        if (memcpy_s(bfgx_cali_data_buf->auc_bfgx_data, MP17C_BFGX_BT_CALI_DATA_SIZE,
                     bfgx_data->cali_buf, MP17C_BFGX_BT_CALI_DATA_SIZE) != EOK) {
            ps_print_err("mp17c copy buffer fail\n");
            return -EFAIL;
        }
    }
...
/*
 * 函 数 名  : mp17c_bfgx_get_cust_ini_data_buf
 * 功能描述  : 返回保存bfgx ini定制化数据的内存地址
 * 输出参数  : bfgx ini定制化数据buf的长度
 * 返 回 值  : bfgx ini数据buf的地址，也可能是NULL
 */
STATIC void *mp17c_bfgx_get_cust_ini_data_buf(uint32_t *pul_len)
{
    mp17c_bfgx_cali_data_stru *pst_bfgx_cali_data_buf = NULL;
 ...
 */
STATIC void *mp17c_wifi_get_bt_cali_data_buf(uint32_t *pul_len)
{
    mp17c_bfgx_cali_data_stru *pst_bfgx_cali_data_buf = NULL;
...
bfgx_cali_data_ops_stru g_cali_data_ops_mp17c = {
    .bfgx_cali_data_len = MP17C_BFGX_CALI_DATA_BUF_LEN,
    .bt_cali_data_len = MP17C_BFGX_BT_CALI_DATA_SIZE,
    .nv_data_len = MP17C_BFGX_NV_DATA_SIZE,
    .cust_ini_data_len = MP17C_BFGX_BT_CUST_INI_SIZE,
    .get_bfgx_cali_data_ops = mp17c_get_bfgx_cali_data,
    .bfgx_get_nv_data_buf_ops = mp17c_bfgx_get_nv_data_buf,
    .bfgx_get_ext_nv_data_buf_ops = mp17c_bfgx_get_ext_nv_data_buf,
    .plat_bfgx_cali_data_test_ops = mp17c_plat_bfgx_cali_data_test,
    .wifi_get_bfgx_rc_data_buf_addr_ops = NULL,
    .wifi_get_bt_cali_data_buf_ops = mp17c_wifi_get_bt_cali_data_buf,
    .bfgx_get_cust_ini_data_buf_ops = mp17c_bfgx_get_cust_ini_data_buf
};
