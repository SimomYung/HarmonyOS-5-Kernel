...
/* 故宫状态跳转图
 * GNSS -> RDY2RSMC ->RSMC ->GNSS
 *      |-> FAC -> RDY2RSMC ->RSMC -> WIFI ->GNSS
 */
enum mp17c_gugong_stage_emum {
    STAGE_GNSS,
    STAGE_RDY2RSMC,
    STAGE_RSMC,
    STAGE_BUF
...
/* 故宫产测状态跳转图
 * FAC_NONE -> FAC_SET -> FAC_ENTER -> FAC_EXIT -> FAC_NONE
 */
enum mp17c_gugong_factory_stage_emum {
    FAC_NONE,
    FAC_SET,
    FAC_ENTER,
    FAC_EXIT,
...
enum mp17c_gugong_stage_emum g_mp17c_gugong_stage = STAGE_GNSS;
enum mp17c_gugong_factory_stage_emum g_mp17c_enter_gugong_fac_status = FAC_NONE;
...
STATIC void mp17c_tas_gpio_init(struct platform_device *pdev)
{
    ...
    for (index_gpio = 0, index_dts = 0; index_gpio < MP17C_GPS_TAS_GPIO_NUMM; index_gpio++) {
        ...
        ret = gpio_request_one(g_gpios_tas[index_gpio].number,
                               (g_gpios_tas[index_gpio].init_mode == 0) ? GPIOF_OUT_INIT_LOW : GPIOF_OUT_INIT_HIGH,
...
STATIC int32_t gugong_rsmc_config(struct conn_gpio_userdata *data)
{
    ...
    plat_cust->cfg.enter_rsmc = (g_mp17c_gugong_stage == STAGE_GNSS) ? OAL_FALSE : OAL_TRUE;
    return OAL_SUCC;
}
...
STATIC void get_gugong_status_to_user(struct conn_gpio_userdata *data, uint8_t OAL_USER *userdata)
{
    ...
    data->sub_type = (g_mp17c_gugong_stage == STAGE_RSMC) ? OAL_TRUE : OAL_FALSE;
...
STATIC void gugong_factory_config(uint32_t sub_type)
{
    ...
    if (sub_type == SW_ON) {
        g_mp17c_enter_gugong_fac_status = FAC_SET;
        plat_cust->cfg.gugong_fac_status = OAL_TRUE;
        ps_print_info("it is factory status\n");
    } else {
        g_mp17c_enter_gugong_fac_status = FAC_EXIT;
...
STATIC void gugong_status_reset(void)
{
    ...
    g_mp17c_gugong_stage = STAGE_GNSS;
    g_mp17c_enter_gugong_fac_status = FAC_NONE;
}
...
{
    if (g_mp17c_gugong_stage == STAGE_RSMC) {
        // 产测模式, 在GNSS打开的情况下，重新开wifi
        if (g_mp17c_enter_gugong_fac_status == FAC_ENTER && (board_subsys_is_shutdown(G_SYS) == OAL_FALSE) &&
            subsys == W_SYS) {
    ...
    if (g_mp17c_gugong_stage == STAGE_RDY2RSMC) {
        if (subsys == G_SYS) {
            if ((board_subsys_is_shutdown(W_SYS) == OAL_FALSE) || (board_subsys_is_shutdown(B_SYS) == OAL_FALSE)) {
                ps_print_err("subsys open before enter gugong!\n");
                return -OAL_FAIL;
            }

            if (g_mp17c_enter_gugong_fac_status == FAC_SET) {
                g_mp17c_enter_gugong_fac_status = FAC_ENTER;
            }

            // 成功进入故宫状态
            g_mp17c_gugong_stage = STAGE_RSMC;
            return OAL_SUCC;
        }
...
STATIC int32_t mp17c_gugong_check_subsys_power_off(uint32_t subsys)
{
    ...
    if (g_mp17c_gugong_stage == STAGE_RDY2RSMC) {
        if (subsys == G_SYS) {
            if (board_subsys_is_shutdown(W_SYS) == OAL_FALSE && g_mp17c_enter_gugong_fac_status == FAC_SET) {
                return OAL_SUCC;
            }

            if ((board_subsys_is_shutdown(W_SYS) == OAL_FALSE) || (board_subsys_is_shutdown(B_SYS) == OAL_FALSE)) {
                ps_print_info("gugong power off subsys not closed\n");
                return -OAL_FAIL;
            }
        ...
        ps_print_err("subsys[%u] should closed before gugong\n", subsys);
        return -OAL_FAIL;
    }

    if (g_mp17c_enter_gugong_fac_status == FAC_EXIT) {
        g_mp17c_enter_gugong_fac_status = FAC_NONE;
        if (board_subsys_is_shutdown(W_SYS) == OAL_FALSE) {
            return OAL_SUCC;
        }
