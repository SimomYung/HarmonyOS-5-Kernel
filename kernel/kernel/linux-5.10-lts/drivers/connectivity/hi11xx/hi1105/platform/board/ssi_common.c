...
static int check_subsys_is_work(void)
{
    ...
    if (get_mpxx_subchip_type() == BOARD_VERSION_MP16) {
        ...
        return mp17c_ssi_check_subsys_is_work();
    } else if (get_mpxx_subchip_type() == BOARD_VERSION_MP12) {
        return mp12_ssi_check_subsys_is_work();
    } else {
...
void show_subsys_mdpll_lock_status(void)
{
    if (get_mpxx_subchip_type() == BOARD_VERSION_MP16) {
        ...
        return mp17c_ssi_show_pll_lock_status();
    }
}

/* 32bits address, 16bits value */
uint32_t ssi_read_word(uint32_t addr)
{
    int32_t ret;
    uint16_t value = 0xffff;
    ssi_ahb_mode_set(SSI_RW_WORD_MOD, SSI_AHB_MODE_SET_START);
    ret = ssi_read_value16(addr, &value, 0);
    if (ret) {
        ps_print_err("read addr 0x%x low 16 bit failed, ret=%d\n", addr, ret);
        ssi_ahb_mode_set(SSI_RW_WORD_MOD, SSI_AHB_MODE_SET_END);
        return 0xffffffff;
    }
    ssi_ahb_mode_set(SSI_RW_WORD_MOD, SSI_AHB_MODE_SET_END);
...
/* 32bits address, 16bits value */
int32_t ssi_write_value16(uint32_t addr, uint16_t value)
{
    uint16_t addr_half_word_high;
    uint16_t addr_half_word_low;
...
/* 32bits address, 16bits value */
uint32_t ssi_write_word(uint32_t addr, uint16_t value)
{
    int32_t ret;

    ssi_ahb_mode_set(SSI_RW_WORD_MOD, SSI_AHB_MODE_SET_START);
    ret = ssi_write_value16(addr, value);
    if (ret != BOARD_SUCC) {
        ps_print_err("ssi write word: 0x%x=0x%x\n", addr, value);
        ssi_ahb_mode_set(SSI_RW_WORD_MOD, SSI_AHB_MODE_SET_END);
        return 0xffffffff;
    }
...
int ssi_check_device_isalive(void)
{
    ...
    for (i = 0; i < check_times; i++) {
        if (get_mpxx_subchip_type() == BOARD_VERSION_MP16 || get_mpxx_subchip_type() == BOARD_VERSION_MP16C ||
            ...
            reg = (uint32_t)ssi_read32(0x4000000);
        } else if (get_mpxx_subchip_type() == BOARD_VERSION_MP12) {
            reg = (uint32_t)ssi_read32(0x4000000);
        } else {
...
OAL_STATIC int ssi_dump_device_regs_check_condition(unsigned long long module_set, uint32_t flag)
{
    ...
    if ((vtype == MPXX_VTYPE_RELEASE) && (g_mpxx_kernel_crash == 0)) {
        ...
        if (!oal_print_rate_limit(30 * PRINT_RATE_SECOND)) { /* 30s打印一次 */
            /* print limit */
            ps_print_err("ssi dump print limit in release mode, ssi dump regs bypass\n");
            return OAL_FALSE;
        }
    }
    ...
    if ((!g_ssi_dfr_bypass) && (flag == OAL_TRUE)) {
        pst_exception_data = get_exception_info_reference();
        if (pst_exception_data == NULL) {
            ps_print_info("get exception info reference is error\n");
            return OAL_TRUE;
        } else if (oal_work_is_busy(&pst_exception_data->wifi_excp_worker) ||
                   oal_work_is_busy(&pst_exception_data->wifi_excp_recovery_worker) ||
                   (in_plat_exception_reset() == OAL_TRUE)) {
            ps_print_err("dfr is doing ,not do ssi read\n");
            return OAL_FALSE;
...
static unsigned long long ssi_module_set_parm_config(unsigned long long module_set)
{
    unsigned long long module_set_real = module_set;
    if (module_set_real & SSI_MODULE_MASK_AON_CUT) {
        module_set_real &= ~SSI_MODULE_MASK_AON;
    }

    if (module_set_real & SSI_MODULE_MASK_PCIE_CUT) {
        module_set_real &= ~(SSI_MODULE_MASK_PCIE_CFG | SSI_MODULE_MASK_PCIE_DBI);
    }
    return module_set_real;
}
...
void ssi_dump_gpio(void)
{
    ...
    } else if (chiptype == BOARD_VERSION_MP17C) {
        mp17c_ssi_dump_gpio();
    } else if (chiptype == BOARD_VERSION_MP12) {
        mp12_ssi_dump_gpio();
...
{
    if (ssi_dump_device_regs_check_condition(module_set, flag) != OAL_TRUE) {
        return -OAL_EBUSY;
    }

    /* ASIC版本，防止频繁dump */
    if ((flag == OAL_TRUE) && (!oal_print_rate_limit(g_ssi_limit_minute_cnts * PRINT_RATE_MINUTE)) &&
       (mpxx_is_asic() == VERSION_ASIC)) {
        ps_print_info("ssi dump limit\n");
        return -OAL_EBUSY;
    }
...
{
    int ret;
    unsigned long long module_set_real;
    // 打印device版本
    plat_changid_print();
    ...
    module_set_real = ssi_module_set_parm_config(module_set);

    declare_dft_trace_key_info("ssi_dump_device_regs", OAL_DFT_TRACE_FAIL);

    ps_print_info("module_set=0x%llx, module_set_real=0x%llx\n", module_set, module_set_real);

    if (get_mpxx_subchip_type() == BOARD_VERSION_MP16) {
        ret = mp16_ssi_config(module_set_real);
    } else if (get_mpxx_subchip_type() == BOARD_VERSION_MP16C) {
        ret = mp16c_ssi_config(module_set_real);
    } else if (get_mpxx_subchip_type() == BOARD_VERSION_GF61) {
        ret = gf61_ssi_config(module_set_real);
    } else if (get_mpxx_subchip_type() == BOARD_VERSION_MP17C) {
        ret = mp17c_ssi_config(module_set_real);
    } else if (get_mpxx_subchip_type() == BOARD_VERSION_MP12) {
        ret = mp12_ssi_config(module_set_real);
    } else {
        ret = ssi_default_config(module_set_real);
    }
