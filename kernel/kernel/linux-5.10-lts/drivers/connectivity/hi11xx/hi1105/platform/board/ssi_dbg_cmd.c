...
void oal_ssi_print_device_mem(uint32_t addr, uint32_t length)
{
    ...
    len_align = padding_m(length, 4); /* 计算4字节对齐后的长度，默认进位 */
    print_buf = vmalloc(len_align);
    if (print_buf == NULL) {
        return;
    }
    for (index = 0; index < len_align; index += (uint32_t)sizeof(uint32_t)) { /* 每次偏移4字节 */
        *(uint32_t *)(print_buf + index) = ssi_read_dword(addr + index);
    }
    oal_io_print("readmem addr:0x%8x len:%u done\n", addr, index);
    /* 打印一行16字节，4字节为一组 */
    print_hex_dump(KERN_INFO, "readmem: ", DUMP_PREFIX_OFFSET, 16, 4, print_buf, len_align, true);
    vfree(print_buf);
    return;
}
...
OAL_STATIC int32_t oal_ssi_loadfile(const char *file_name, uint32_t addr)
{
    ...
    forever_loop() {
        ...
        fp->f_pos += rlen;
        for (i = 0; i < rlen; i += (int)sizeof(uint32_t)) {
...
{
    struct file *fp = NULL;
    uint32_t index, len_align;
    int32_t ret;
    mm_segment_t fs;
    void *pst_buf = NULL;

    len_align = padding_m(length, 4); /* 计算4字节对齐后的长度，默认进位 */
    pst_buf = vmalloc(len_align);
    if (pst_buf == NULL) {
        oal_io_print("savemem pst_buf is null, vmalloc size %u failed!", len_align);
        return -OAL_ENOMEM;
    }
    ...
    if (index) {
        ...
        ret = oal_kernel_file_write(fp, pst_buf, len_align);
        if (ret < 0) {
            oal_io_print("vfs write failed!");
        }
    } else {
        oal_io_print("savemem cpu:0x%8x len:%u save_path:%s failed!", addr, len_align, file_name);
        ret = -OAL_EINVAL;
    }
                                          ...
                                          size_t count)
{
    ...
    for (i = 0; i < oal_array_size(g_ssi_debug); i++) {
        if (g_ssi_debug[i].name) {
            size_t len = OAL_STRLEN(g_ssi_debug[i].name);
            char last_c = *(buf + len);
            if ((count >= len) && !oal_memcmp(g_ssi_debug[i].name, buf, len) &&
                (last_c == '\n' || last_c == ' ' || last_c == '\0')) {
                break;
            }
        }
