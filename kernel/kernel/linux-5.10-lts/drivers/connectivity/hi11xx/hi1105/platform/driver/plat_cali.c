...
STATIC int32_t nvram_post_process(uint32_t nv_number, const char *nv_name, uint8_t offset, uint8_t length);
STATIC int32_t ext_nvram_post_process(uint32_t nv_number, const char *nv_name, uint8_t offset, uint8_t length);
#endif
...
STATIC bfgx_nv_table_stru g_bfgx_nv_table[] = {
    { OAL_BT_NVRAM_NAME, 0, OAL_BT_NVRAM_DATA_LENGTH, OAL_BT_NVRAM_NUMBER, nvram_post_process },
    // 扩展nv ram给业务使用，可以根据offset填充
    { OAL_BT_EXT_NVRAM_NAME, 0, BT_EXT_NVRAM_LENGTH, OAL_BT_EXT_NVRAM_NUMBER, ext_nvram_post_process },
};
#endif
#endif
...
#ifdef HISI_NVRAM_SUPPORT
#ifndef _PRE_WINDOWS_SUPPORT
STATIC int32_t nvram_gen_post_process(uint8_t *nv_buffer, uint8_t offset, uint8_t length,
...
void bt_get_wl_cali_data_mp17c(void)
{
    ...
    bt_get_wl_fem_abnormal_flag_mp17c(bt_cali_data, wl_cali_data); // TX POW
}
