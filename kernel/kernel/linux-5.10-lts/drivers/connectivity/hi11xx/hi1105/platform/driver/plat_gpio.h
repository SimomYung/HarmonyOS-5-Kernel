...
/* I2C_WKUP_HOST */
#define DTS_PROP_MPXX_GPIO_I2C_WAKEUP_HOST   "hi110x,gpio_i2c_wakeup_host"
#define PROC_NAME_GPIO_I2C_WAKEUP_HOST         "i2c_wake_host"
enum gpio_type {
    CHIP_POWR,
    W_POWER,
    GT_POWER,
...
struct conn_gpio_pin_info {
    char *name;              // 名字
    uint8_t main_type;       // soc pin or virtual gpio(pmu pin)
...
struct conn_gpio_node {
    ...
    struct conn_gpio_ops *ops;   // 如果无指定, 继承pub_ops
};
...
struct conn_gpio_ops {
    ...
    int32_t (*get_gpio_level)(struct conn_gpio_node *node);
};
