...
int32_t bfgx_reset_pre_proc(struct pm_drv_data *pm_data, excp_record_t *excp_record)
{
    ...
    if (oal_unlikely(pm_data == NULL || pm_data->ps_core_data == NULL)) {
        ps_print_err("%s: pm_data is NULL or ps_core_data is NULL\n", __func__);
    ...
    bfgx_gpio_intr_enable(ps_core_d->pm_data, OAL_FALSE);
...
int32_t get_bfgx_subsys_state(excp_comm_t *node_ptr)
{
    ...
    if (pm_drv_d == NULL || pm_drv_d->ps_core_data == NULL) {
        ps_print_err("pm_drv_d is NULL"NEWLINE);
        return POWER_STATE_SHUTDOWN;
    }
...
STATIC void bfgx_excp_close_for_all_cores(excp_subsys_res_t *subsys_res)
{
    ...
    if (subsys_res->core_attr == NULL) {
        hw_bfgx_close_all_service((struct pm_drv_data*)subsys_res->subsys_pm, OAL_TRUE);
        return;
    }
    ...
    for (i = 0; i < core_attr->sla_core_cnt; i++) {
        ...
        if (sla_core->sla_core_pm != NULL) {
            hw_bfgx_close_all_service((struct pm_drv_data*)sla_core->sla_core_pm, OAL_FALSE);
        }
    }
    hw_bfgx_close_all_service((struct pm_drv_data*)core_attr->master_pm, OAL_TRUE);
}
...
OAL_STATIC int32_t bfgx_firmware_download_post_proc(struct pm_drv_data *pm_data)
{
    ...
    if (oal_unlikely(pm_data->ps_plat_data == NULL)) {
        ps_print_err("%s: ps_plat_data is NULL\n", __func__);
        return -EXCEPTION_FAIL;
    }
...
void bfgx_dump_stack(struct ps_core_s *ps_core_d)
{
    ...
    if (exception_type != get_panic_excp_by_uart_idx(ps_core_d->pm_data->index) &&
        exception_type != get_system_reset_excp_by_uart_idx(ps_core_d->pm_data->index)) {
        return;
    }

    /* 等待读栈操作完成 */
    if (mpxx_is_asic() == VERSION_ASIC) {
        time_out = WAIT_BFGX_READ_STACK_TIME;
    } else {
        time_out = WAIT_BFGX_READ_STACK_TIME_FPGA;
    }
...
{
    uint32_t index = ps_core_d->uart_index;
    ...
    if (index == BUART) {
        ...
        ps_print_err("unknown dev index:%u\n", index);
    }
}
