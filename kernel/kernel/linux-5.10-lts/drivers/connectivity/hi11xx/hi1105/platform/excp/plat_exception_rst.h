...
#define WAIT_BFGX_READ_STACK_TIME_FPGA 20000 /* 等待bfgx读栈操作完成的时间，10000毫秒 */
...
enum exception_type_enum {
    ...
    BUART_SYSTEM_RESET = 30,
    GUART_SYSTEM_RESET = 31,
    WIFI_SYSTEM_RESET = 32,
    EXCEPTION_TYPE_BOTTOM = 33
};
...
enum excp_test_cfg_em {
    ...
    FIRMWARE_DOWNLOAD_FAULT = 6,
    UART_SUSPEND_FAULT = 7,
    EXCP_TEST_CFG_BUTT
};
...
typedef struct {
    bool (*is_ir_mode_need_dfr)(uint32_t subsys_type);
    bool (*is_chip_force_reset)(void);
    bool (*is_single_wifi)(void);
    ...
    int32_t (*firmware_download_mode_separate)(excp_comm_t *node_ptr);
...
typedef enum {
    ...
    EXCP_B_SYS = 0x1, /* b_cpu, MP16: b_cpu & g_cpu, MP12: bgi_cpu */
    EXCP_GLF_SYS,     /* MP16C: sle & gf, mp17c:gnss */
...
struct st_exception_info {
    ...
    struct st_wifi_dfr_callback *gt_callback;
    ...
    oal_work_stru gt_excp_worker;
    oal_work_stru gt_excp_recovery_worker;
    oal_workqueue_stru *gt_exception_workqueue;
    uint32_t gt_excp_type;
};
...
oal_workqueue_stru *wifi_get_exception_workqueue(hcc_bus *hi_bus);
