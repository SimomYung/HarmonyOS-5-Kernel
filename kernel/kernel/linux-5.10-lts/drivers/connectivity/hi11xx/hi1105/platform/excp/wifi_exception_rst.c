...
int32_t get_wifi_subsys_state(excp_comm_t *node_ptr)
{
    ...
    if (wlan_pm == NULL) {
        ps_print_err("wlan_pm is NULL"NEWLINE);
        return POWER_STATE_SHUTDOWN;
    }
...
int32_t wifi_exception_mem_dump(hcc_bus *hi_bus)
{
    ...
    if (excep_type != HCC_BUS_PANIC && excep_type != HCC_BUS_TRANS_FAIL && excep_type != WIFI_SYSTEM_RESET) {
...
oal_workqueue_stru *wifi_get_exception_workqueue(hcc_bus *hi_bus)
{
    ...
    if (hi_bus->dev_id == HCC_EP_GT_DEV) {
        return pst_exception_data->gt_exception_workqueue;
    } else {
