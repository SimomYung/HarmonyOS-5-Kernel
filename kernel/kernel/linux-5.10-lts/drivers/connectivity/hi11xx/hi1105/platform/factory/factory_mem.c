...
// mp17c
#define GET_MEM_CHECK_FLAG_MP17C    "0x04001C40,4"
#define RESULT_DETAIL_REG_MP17C     "0x04001C44,4"
#define RESULT_TIME_LOW_REG_MP17C   "0x04001C48,4"
#define RESULT_TIME_HIGH_REG_MP17C  "0x04001C4C,4"
#define RESULT_TSENSOR_C0_REG_MP17C "0x0401A48C,4"
#define RESULT_TSENSOR_C1_REG_MP17C "0x0401A4CC,4"
...
#define RAM_TEST_RUN_VOLTAGE_REG_MP17C      0x04007004
#define RAM_TEST_RUN_PROCESS_SEL_REG_MP17C  0x04007008
...
static uint8_t *g_mp17c_product_test_result[RESULT_TOTAL] = {
    RESULT_DETAIL_REG_MP17C,
    RESULT_TIME_LOW_REG_MP17C,
    RESULT_TIME_HIGH_REG_MP17C,
    RESULT_TSENSOR_C0_REG_MP17C,
    RESULT_TSENSOR_C1_REG_MP17C,
    GET_MEM_CHECK_FLAG_MP17C,
};
...
#define RAM_WIFI_CHECK_CFG_MP17C_ASIC_PATH  "/vendor/firmware/mp17c/pilot/reg_wifi_mem_test_cfg"
#define RAM_BGCPU_CHECK_CFG_MP17C_ASIC_PATH "/vendor/firmware/mp17c/pilot/reg_bgcpu_mem_test_cfg"
...
factory_cfg_str factory_cfg[] = {
    ...
    {BOARD_VERSION_MP17C, OAL_TRUE,  RAM_WIFI_CHECK_CFG_MP17C_ASIC_PATH},
    {BOARD_VERSION_MP17C, OAL_FALSE, RAM_BGCPU_CHECK_CFG_MP17C_ASIC_PATH},
...
int32_t g_pre_fwdownload_way = 0;
uint32_t g_pre_pcie_dma_trans = 0;
...
static factory_wlan_memdump_t g_factory_wlan_memdump_cfg_mp17c = { 0x08000000, 0x1000 };
...
static uint8_t **get_product_test_result(void)
{
    if (get_mpxx_subchip_type() == BOARD_VERSION_MP16) {
        ...
        return g_mp17c_product_test_result;
...
static int32_t number_cmd_send_and_read(const char *key, const char *value, uint32_t *result)
{
    ...
    buf_result = os_kzalloc_gfp(sizeof(uint32_t));
    if (buf_result == NULL) {
        ps_print_err("buf_result is null, vmalloc size 128 failed!");
        return -OAL_ENOMEM;
    }

    ret = read_msg(buf_result, sizeof(uint32_t), FILES_BIN_READ);
    if (ret < 0) {
        ps_print_warning("send cmd %s:%s read result failed,ret = %d\n", key, value, ret);
        os_mem_kfree(buf_result);
        return -OAL_EFAIL;
    }
    ps_print_warning("buf_result = %d, reg = %s\n", *(uint32_t *)(buf_result), value);
    *result = *(uint32_t *)(buf_result);
    os_mem_kfree(buf_result);
    return OAL_SUCC;
}
...
static int32_t memcheck_result_flag_check(uint8_t **result_reg)
{
    ...
    if (number_cmd_send_and_read(RMEM_CMD_KEYWORD, result_reg[MEM_CHECK_FLAG], &test_flag)) {
        return -OAL_EFAIL;
    }
...
{
    int32_t ret;
    uint8_t **result_reg = get_product_test_result();
...
static void tsensor_check(int32_t sensor_num, uint32_t reg_tsensor)
{
...
static factory_wlan_memdump_t *get_wlan_memdump_cfg(void)
{
    if (get_mpxx_subchip_type() == BOARD_VERSION_MP16C) {
        ...
        return &g_factory_wlan_memdump_cfg_mp17c;
    } else {
...
static int32_t get_device_test_mem(bool is_wifi)
{
    ...
    if (rmem_cmd_handler(RMEM_CMD_KEYWORD, buff, is_wifi) >= 0) {
...
int32_t wlan_device_mem_check(int32_t l_runing_test_mode)
{
    ...
    } else if (l_chip_type == BOARD_VERSION_MP17C) {
        ret = set_wlan_mem_check_mdelay(6000);  // wifi 6000ms
...
static int32_t get_device_soc_test_result(void)
{
    ...
    ret = read_msg((uint8_t *)&test_flag, sizeof(test_flag), FILES_BIN_READ);
    if (ret < 0) {
...
static int32_t device_mem_check_priv_init(void)
{
    ...
    if ((get_mpxx_subchip_type() == BOARD_VERSION_MP16) ||
        ...
        ram_test_voltage_addr = RAM_TEST_RUN_VOLTAGE_REG_MP17C;
        process_sel_addr = RAM_TEST_RUN_PROCESS_SEL_REG_MP17C;
    } else {
...
{
    int32_t ret;
    int32_t gpio = conn_get_gpio_number_by_type(HOST_WKUP_W);
    ...
    if (ret == SUCCESS) {
        ...
        if (hcc_is_pcie() && ((get_mpxx_subchip_type() == BOARD_VERSION_MP16C) ||
            (get_mpxx_subchip_type() == BOARD_VERSION_MP17C))) {
            hcc_bus_power_ctrl_register(hcc_get_bus(HCC_EP_WIFI_DEV), HCC_BUS_CTRL_POWER_DOWN,
                                        device_mem_check_host_wakeup_dev_set_low,  (void *)(uintptr_t)gpio);
            ...
            hcc_bus_power_ctrl_register(hcc_get_bus(HCC_EP_WIFI_DEV), HCC_BUS_CTRL_POWER_UP,
                                        device_mem_check_host_wakeup_dev_set_high, (void *)(uintptr_t)gpio);
            hcc_bus_power_action(hcc_get_bus(HCC_EP_WIFI_DEV), HCC_BUS_POWER_UP);
            hcc_bus_reinit(hcc_get_bus(HCC_EP_WIFI_DEV));
            ret = memcheck_result_flag_check(get_product_test_result());
        } else {
...
static void _device_mem_check_host_set(hcc_bus *pst_bus)
{
#ifdef _PRE_PLAT_FEATURE_HI110X_PCIE
    oal_pcie_linux_res *pst_pci_lres = (oal_pcie_linux_res *)pst_bus->data;
    if (get_mpxx_subchip_type() == BOARD_VERSION_MP17C) {
        g_pre_pcie_dma_trans = pst_pci_lres->pst_pci_res->chip_info.boot_cap.bits.dma_trans;
        pst_pci_lres->pst_pci_res->chip_info.boot_cap.bits.dma_trans = OAL_FALSE;
    }
#endif
    g_pre_fwdownload_way = g_st_board_info.download_mode;
    g_st_board_info.download_mode = MODE_COMBO;
}

static void _device_mem_check_host_restore(hcc_bus *pst_bus)
{
#ifdef _PRE_PLAT_FEATURE_HI110X_PCIE
    oal_pcie_linux_res *pst_pci_lres = (oal_pcie_linux_res *)pst_bus->data;
    if (get_mpxx_subchip_type() == BOARD_VERSION_MP17C) {
        pst_pci_lres->pst_pci_res->chip_info.boot_cap.bits.dma_trans = g_pre_pcie_dma_trans;
    }
#endif
    g_st_board_info.download_mode = g_pre_fwdownload_way;
}
...
static int32_t _device_mem_check(uint32_t *wcost, uint32_t *bcost, uint32_t *gcost)
{
    ...
    hcc_bus *pst_bus = hcc_get_bus(HCC_EP_WIFI_DEV);
    if (pst_bus == NULL) {
        ps_print_info("pst_bus is null!\n");
        return -OAL_EINVAL;
    }
    _device_mem_check_host_set(pst_bus);
...
device_mem_check_end:
    _device_mem_check_host_restore(pst_bus);
    return ret;
}
