...
#define GPIO_BASE_ADDR_MP16           0x40005000
#define GPIO_BASE_ADDR_MP16C          0x40005000
#define CHECK_DEVICE_RDY_ADDR_MP16    0x40000000
#define CHECK_DEVICE_RDY_ADDR_MP17C   0x04000000
#define GPIO_BASE_ADDR_MP17C          0x0400E000
...
int32_t mp13_check_device_ready(void)
{
    ...
    if (get_mpxx_subchip_type() == BOARD_VERSION_MP16 ||
        ...
        check_addr = CHECK_DEVICE_RDY_ADDR_MP16;
    } else if (get_mpxx_subchip_type() == BOARD_VERSION_MP17C) {
        check_addr = CHECK_DEVICE_RDY_ADDR_MP17C;
    } else {
        check_addr = CHECK_DEVICE_RDY_ADDR;
    }

    ret = read_device_reg16(check_addr, &value);
    if (ret) {
        oal_print_mpxx_log(MPXX_LOG_ERR, "read 0x%x reg failed, ret=%d", check_addr, ret);
        return -OAL_EFAIL;
    }
...
OAL_STATIC int32_t mp13_ssi_set_gpio_direction(uint32_t address, uint16_t gpio_bit, uint32_t direction)
{
    ...
    if (direction == GPIO_DIRECTION_OUTPUT || get_mpxx_subchip_type() == BOARD_VERSION_MP17C) {
        value |= gpio_bit;
    } else {
...
OAL_STATIC uint32_t dev_io_test_get_gpio_baseaddr(void)
{
    int32_t board_ver;

    board_ver = get_mpxx_subchip_type();
    if (board_ver == BOARD_VERSION_MP17C) {
        return GPIO_BASE_ADDR_MP17C;
    } else if (board_ver >= BOARD_VERSION_MP16 && board_ver <= BOARD_VERSION_GF61) {
        return (board_ver == BOARD_VERSION_MP16 ? GPIO_BASE_ADDR_MP16 : GPIO_BASE_ADDR_MP16C);
    } else {
...
OAL_STATIC int32_t mp13_check_wlan_wakeup_host(void)
{
    ...
    uint32_t gpio_base_addr, gpio_set_addr, gpio_clr_addr, gpio_dir_addr;
    uint32_t wlan_dev2host_gpio;
    int32_t board_ver;

    gpio_num = conn_get_gpio_number_by_type(W_WKUP_HOST);
    board_ver = get_mpxx_subchip_type();
    gpio_base_addr = dev_io_test_get_gpio_baseaddr();
    if (board_ver == BOARD_VERSION_MP17C) {
        gpio_set_addr = gpio_base_addr + 0x30;
        gpio_clr_addr = gpio_base_addr + 0x34;
        gpio_dir_addr = gpio_base_addr + 0x40;
    } else {
        gpio_set_addr = gpio_base_addr + GPIO_LEVEL_CONFIG_REGADDR;
        gpio_clr_addr = gpio_base_addr + GPIO_LEVEL_CONFIG_REGADDR;
        gpio_dir_addr = gpio_base_addr + GPIO_INOUT_CONFIG_REGADDR;
    }
    if (gpio_num == 0) {
        oal_print_mpxx_log(MPXX_LOG_ERR, "wlan_wakeup_host gpio is zero!");
        return -OAL_EIO;
    }
    if (board_ver >= BOARD_VERSION_MP16 && board_ver <= BOARD_VERSION_MP17C) {
        wlan_dev2host_gpio = WLAN_DEV2HOST_GPIO_MP16;
    } else {
        wlan_dev2host_gpio = WLAN_DEV2HOST_GPIO;
    }

    ret = mp13_ssi_set_gpio_direction(gpio_dir_addr, wlan_dev2host_gpio, GPIO_DIRECTION_OUTPUT);
    if (ret < 0) {
        return ret;
    }

    for (i = 0; i < ul_test_times; i++) {
        ret = mp13_ssi_set_gpio_value(gpio_num, GPIO_HIGHLEVEL, gpio_set_addr, wlan_dev2host_gpio);
        if (ret < 0) {
            return ret;
        }

        ret = mp13_ssi_set_gpio_value(gpio_num, GPIO_LOWLEVEL, gpio_clr_addr, board_ver != BOARD_VERSION_MP17C ? 0 :
            wlan_dev2host_gpio);
        if (ret < 0) {
            return ret;
        }
...
OAL_STATIC int32_t mp13_check_host_wakeup_wlan(void)
{
    ...
    if (board_ver != BOARD_VERSION_MP17C) { // 07C 默认输入
    ...
    ssi_address = gpio_base_addr + ((board_ver != BOARD_VERSION_MP17C) ? GPIO_LEVEL_GET_REGADDR : 0);
    for (i = 0; i < ul_test_times; i++) {
