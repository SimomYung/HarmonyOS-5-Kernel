...
OAL_STATIC int32_t pcie_ip_test_power_on(hcc_bus *pst_bus)
{
    ...
    hcc_bus_frw_download_pre(pst_bus);
...
int32_t pcie_slt_chip_poweron(void *data)
{
    int32_t ret;
...
int32_t pcie_slt_chip_poweroff(void *data)
{
    int32_t ret;
    hcc_bus *pst_bus;
...
int32_t pcie_slt_chip_transfer(void *ddr_address, uint32_t data_size, uint32_t direction)
{
    hcc_bus *pst_bus;
...
EXPORT_SYMBOL(pcie_slt_chip_poweron);
EXPORT_SYMBOL(pcie_slt_chip_transfer);
EXPORT_SYMBOL(pcie_slt_chip_poweroff);
#endif

int32_t pcie_slt_chip_init(void *data)
{
    oal_print_mpxx_log(MPXX_LOG_INFO, "slt pcie init");
    return 0;
}

#if ((defined(CONFIG_PCIE_KIRIN_SLT_HI110X)|| defined(CONFIG_PCIE_KPORT_SLT_DEVICE) || defined(CONFIG_HIPCIE_SLT))\
    && (defined(CONFIG_HISI_DEBUG_FS) || defined(CONFIG_DFX_DEBUG_FS)))
...
int32_t pcie_chip_rc_slt_register(void)
{
#if (defined(_PRE_PLAT_FEATURE_HI110X_PCIE) || defined(_PRE_PLAT_FEATURE_GF6X_PCIE))
#if ((defined(CONFIG_PCIE_KIRIN_SLT_HI110X)|| defined(CONFIG_PCIE_KPORT_SLT_DEVICE) || defined(CONFIG_HIPCIE_SLT))\
    && (defined(CONFIG_HISI_DEBUG_FS) || defined(CONFIG_DFX_DEBUG_FS)))
#ifdef _PRE_CONFIG_ARCH_HI1620S_KUNPENG_PCIE
    return pcie_slt_hook_register(g_pcie_rc_idx, 0x2,
                                  pcie_slt_chip_init, pcie_slt_chip_poweron_kunpeng,
                                  pcie_slt_chip_poweroff_kunpeng, pcie_slt_chip_init,
                                  pcie_slt_chip_transfer_kunpeng);
#else
    return pcie_slt_hook_register(g_pcie_rc_idx, 0x2,
                                  pcie_slt_chip_init, pcie_slt_chip_poweron,
                                  pcie_slt_chip_poweroff, pcie_slt_chip_init,
                                  pcie_slt_chip_transfer);
#endif
#else
    return 0;
#endif
...
int32_t pcie_chip_rc_slt_unregister(void)
{
#if (defined(_PRE_PLAT_FEATURE_HI110X_PCIE) || defined(_PRE_PLAT_FEATURE_GF6X_PCIE))
#if ((defined(CONFIG_PCIE_KIRIN_SLT_HI110X)|| defined(CONFIG_PCIE_KPORT_SLT_DEVICE) || defined(CONFIG_HIPCIE_SLT))\
    && (defined(CONFIG_HISI_DEBUG_FS) || defined(CONFIG_DFX_DEBUG_FS)))
...
EXPORT_SYMBOL(pcie_chip_rc_slt_register);
EXPORT_SYMBOL(pcie_chip_rc_slt_unregister);
