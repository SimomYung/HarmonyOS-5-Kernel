...
/* 全局变量定义 */
STATIC firmware_cfg_t g_chip_firmware_cfg;

/* 存储cfg文件信息，解析cfg文件时赋值，加载的时候使用该变量 */
STATIC firmware_globals g_cfg_info;
...
/* 每个board 下单独的cust 配置 */
firmware_cust_cb g_frw_cust_cb = NULL;
...
STATIC int32_t exec_board_cust_cmd(uint8_t *key, uint8_t *value);
STATIC int32_t exec_sec_quit_cmd(uint8_t *key, uint8_t *value);
...
STATIC firmware_number g_number_type[] = {
    ...
    {BOARD_CUST_CMD_KEYWORD, exec_board_cust_cmd},
    {SEC_QUIT_CMD_KEYWORD, exec_sec_quit_cmd}
};

firmware_globals* get_firmare_cfg_info(void)
{
    return &g_cfg_info;
}

void set_firmware_cust(firmware_cust_cb cb)
{
    g_frw_cust_cb = cb;
}
...
int32_t mem_read_proc(uint8_t *data_buf, int32_t len, int32_t *rd_len)
{
    int32_t l_ret = -EFAIL;
    uint8_t retry = 3;
    int32_t lenbuf = *rd_len;

    while (len > lenbuf) {
        l_ret = read_msg(data_buf + lenbuf, len - lenbuf, FILES_BIN_READ);
        if (l_ret > 0) {
            lenbuf += l_ret;
        } else {
            retry--;
...
STATIC int32_t recv_device_mem(os_kernel_file *fp, uint8_t *data_buf, int32_t len)
{
    ...
    l_ret = mem_read_proc(data_buf, len, &rd_len);
...
STATIC int32_t check_version(void)
{
    ...
    uint8_t download_try = HOST_DEV_TIMEOUT;
    ...
    if (mpxx_is_asic() == VERSION_FPGA) {
        download_try = 1; // fpga版本不做尝试
    }

    for (i = 0; i < download_try; i++) {
        ...
        l_ret = send_msg(rec_buf, l_len, FILES_CMD_SEND);
        if (l_ret < 0) {
        ...
        l_ret = read_msg(rec_buf, VERSION_LEN, FILES_CMD_SEND);
        if (l_ret < 0) {
...
int32_t number_type_cmd_send(const char *key, const char *value)
{
    ...
    l_ret = send_msg(buff_tx, data_len, FILES_CMD_SEND);
...
STATIC int32_t get_cali_data(uint8_t *cmd_name, uint32_t fw_addr)
{
    ...
    if (!os_str_cmp(cmd_name, CALI_BFGX_DATA_CMD_KEYWORD)) {
        ...
        len = (uint32_t)g_cali_data_ops->bfgx_cali_data_len;
...
STATIC int32_t download_cali_data(uint8_t *key, uint8_t *value, uint8_t *cmd_name)
{
    ...
    /* 发送地址 */
    l_ret = msg_send_and_recv_except(buff_tx, os_str_len(buff_tx), MSG_FROM_DEV_READY_OK, FILES_CMD_SEND);
    if (l_ret < 0) {
    ...
    /* 发送bfgx或者dcxo校准数据 */
    l_ret = msg_send_and_recv_except(g_firmware_down_buf, len, MSG_FROM_DEV_FILES_OK, FILES_BIN_SEND);
    if (l_ret < 0) {
...
int32_t alloc_dump_rx_buf(uint8_t **rx_buf, uint32_t *buf_len)
{
    uint8_t *data_buf = NULL;
    /* 导内存先考虑成功率, 页大小对齐的内存容易申请成功。 */
    uint32_t transfer_limit = hcc_get_max_trans_size(hcc_get_handler(HCC_EP_WIFI_DEV));
    transfer_limit = oal_min(PAGE_SIZE, transfer_limit);
    if (transfer_limit == 0) {
        ps_print_err("transfer_limit is zero\n");
        return -EFAIL;
    }

    do {
        ps_print_info("try to malloc mem read buf len is [%d]\n", transfer_limit);
        data_buf = (uint8_t *)os_kmalloc_gfp(transfer_limit);
        if (data_buf == NULL) {
            ps_print_warning("malloc mem len [%d] fail, continue to try in a smaller size\n", transfer_limit);
            transfer_limit = transfer_limit >> 1;
        }
    } while ((data_buf == NULL) && (transfer_limit >= MIN_FIRMWARE_FILE_TX_BUF_LEN));
    ...
    *rx_buf = data_buf;
    *buf_len = transfer_limit;

    return SUCC;
}

int32_t rmem_cmd_handler(const char *key, const char *value, bool is_wifi)
{
    int32_t l_ret;
    int32_t size;
    uint32_t readlen;
    ...
    if (alloc_dump_rx_buf(&data_buf, &transfer_limit) != SUCC) {
        return -EFAIL;
    }
    ...
    while (size > 0) {
        readlen = min((uint32_t)size, transfer_limit);
...
STATIC int32_t exec_number_rmem_cmd(uint8_t *key, uint8_t *value)
{
    return rmem_cmd_handler(key, value, true);
}
...
STATIC int32_t exec_board_cust_cmd(uint8_t *key, uint8_t *value)
{
    if (g_frw_cust_cb == NULL) {
        return -EFAIL;
    }

    return g_frw_cust_cb(key, value);
}
...
STATIC int32_t exec_sec_quit_cmd(uint8_t *key, uint8_t *value)
{
    ...
    ret = recv_expect_result(MSG_FROM_DEV_SEC_QUIT_OK);
    if (ret < 0) {
        ps_print_err("recv expect result fail!\n");
        return ret;
    }
...
STATIC int32_t exec_quit_type_cmd(void)
{
    ...
    ret = msg_send_and_recv_except(buf, l_len, MSG_FROM_DEV_QUIT_OK, FILES_CMD_SEND);
...
STATIC int32_t build_file_cmd(uint8_t *buff, uint32_t len, unsigned long addr_send, uint32_t data_len)
{
    ...
    // 命令发送,sdio会进行4字节对齐,存在多搬的case,需要进行初始化,避免命令解析失败
...
STATIC int32_t read_file_context_and_send(os_kernel_file *fp, uint32_t file_len, unsigned long addr)
{
    unsigned long addr_send;
    uint32_t i, per_send_len, send_count;
    int32_t ret, rdlen;
    uint32_t offset = 0;
    ...
    for (i = 0; i < send_count; i++) {
        ...
        ret = msg_send_and_recv_except(buff_tx, os_str_len(buff_tx), MSG_FROM_DEV_READY_OK, FILES_CMD_SEND);
        if (ret < 0) {
            ps_print_err("SEND [%s] error\n", buff_tx);
            return -EFAIL;
        }

        ps_print_dbg("FILES_BIN_SEND\n");
        /* Wait at least 5 ms */
        oal_usleep_range(FILE_CMD_WAIT_TIME_MIN, FILE_CMD_WAIT_TIME_MAX);
        /* 发送文件内容 */
        ret = msg_send_and_recv_except(g_firmware_down_buf, rdlen, MSG_FROM_DEV_FILES_OK, FILES_BIN_SEND);
        if (ret < 0) {
...
#ifndef  _PRE_WINDOWS_SUPPORT
    if (ps_core_ir_only_mode() == OAL_TRUE) {
        if (get_mpxx_subchip_type() == BOARD_VERSION_MP17C ||
            get_mpxx_subchip_type() == BOARD_VERSION_MP12) {
            ps_print_info("ir only need download file cmd\n");
        } else {
...
#define BUCK_PARA_LEN   16
    ...
    if (os_str_str((char *)puc_cfg_buffer, QUIT_CMD_KEYWORD) != NULL) {
        ...
        char buck_para[BUCK_PARA_LEN] = { 0 };
...
STATIC int32_t print_firmware_download_cmd(uint32_t ul_index)
{
    ...
    ps_print_info("[%s] download cmd, total count is [%d]\n",
                  g_cfg_info.file_info.cfg_map->cfg_path, count);
...
{
    int32_t i;
    cfg_map_info *cfg_map = NULL;
    uint32_t cfg_map_size = g_cfg_info.file_info.cfg_map_size;

    for (i = 0; i < (int32_t)cfg_map_size; i++) {
        cfg_map = &g_cfg_info.file_info.cfg_map[i];
        if (cfg_map == NULL) {
            continue;
        }
        print_firmware_download_cmd(cfg_map->idx);
    }
...
STATIC void free_cfg_file_memory(void)
{
    ...
    for (i = 0; i < cfg_map_size; i++) {
        cfg_map = &g_cfg_info.file_info.cfg_map[i];
        if (g_cfg_info.file_info.from_heap_mem[cfg_map->idx] == OAL_TRUE) {
            os_mem_kfree(cfg_map->cfg_path);
        }
    }
}
...
STATIC int32_t cfg_file_add_pre_direcotry(void)
{
    ...
    for (i = 0; i < cfg_map_size; i++) {
        cfg_map = &g_cfg_info.file_info.cfg_map[i];
        if (cfg_map->cfg_path == NULL) {
            continue;
        }
        cfg_buf_size = (uint32_t)((OAL_STRLEN(g_cfg_info.file_info.pre_directory) +
                       sizeof(char) +
                       OAL_STRLEN(cfg_map->cfg_path) + 1));
        path = (uint8_t *)os_kzalloc_gfp(cfg_buf_size);
        if (path == NULL) {
            free_cfg_file_memory();
            return -ENOMEM;
        }
        g_cfg_info.file_info.from_heap_mem[cfg_map->idx] = OAL_TRUE;
        ret = sprintf_s(path, cfg_buf_size, "%s%c%s", g_cfg_info.file_info.pre_directory, '/', cfg_map->cfg_path);
        cfg_map->cfg_path = path;
        ps_print_dbg("cfg absolute path is %d, idx[%d], %s, ret %d", i, cfg_map->idx, cfg_map->cfg_path, ret);
    }

    return SUCC;
}

int32_t chip_firmware_cfg_register(chip_firmware_cfg_str *chip_cfg, uint32_t chip_cfg_size)
{
    if (chip_cfg == NULL) {
        ps_print_err("cfg is NULL");
        return -EFAIL;
    }

    g_chip_firmware_cfg.cfg_str = chip_cfg;
    g_chip_firmware_cfg.cfg_str_size = chip_cfg_size;
    ps_print_info("chip_nr=%d, cfg_str_size=%d", chip_cfg[0].chip_nr, chip_cfg_size);
    return SUCC;
}
...
{
    uint32_t i;
    chip_firmware_cfg_str *cfg_str = NULL;

    for (i = 0; i < g_chip_firmware_cfg.cfg_str_size; i++) {
        if ((get_mpxx_subchip_type() != g_chip_firmware_cfg.cfg_str[i].chip_nr) ||
            (g_chip_firmware_cfg.cfg_str[i].is_asic != mpxx_is_asic())) {
            continue;
        }

        cfg_str = &g_chip_firmware_cfg.cfg_str[i];
        break;
    }

    if (i == g_chip_firmware_cfg.cfg_str_size) {
        ps_print_err("chip [%d], is_asic [%d] need config", get_mpxx_subchip_type(), mpxx_is_asic());
        return -EFAIL;
    }

    g_cfg_info.file_info.cfg_map = cfg_str->cfg_map;
    g_cfg_info.file_info.cfg_map_size = cfg_str->cfg_map_size;
    g_cfg_info.file_info.pre_directory = cfg_str->pre_directory;
    ps_print_info("chip [%i], cfg_path=%s, pre_dir=%s", i, g_cfg_info.file_info.cfg_map->cfg_path, \
                  g_cfg_info.file_info.pre_directory);

    if ((g_cfg_info.file_info.pre_directory != NULL) && (cfg_file_add_pre_direcotry() != SUCC)) {
        ps_print_err("cfg firmware add pre path %s fail", g_cfg_info.file_info.pre_directory);
        return -EFAIL;
    }

    ps_print_info("firmware info chip [%d], type [%d]", get_mpxx_subchip_type(), mpxx_is_asic());
    return SUCC;
}
...
int32_t firmware_cfg_init(void)
{
    ...
    cfg_map = g_cfg_info.file_info.cfg_map;
    cfg_map_size = g_cfg_info.file_info.cfg_map_size;
    /* 解析cfg文件 */
    for (i = 0; i < cfg_map_size; i++) {
        if (cfg_map[i].cfg_path == NULL) {
            continue;
        }
        ret = firmware_get_cfg(cfg_map[i].cfg_path, cfg_map[i].idx);
        if (ret < 0) {
            if (cfg_map[i].idx == RAM_REG_TEST_CFG) {
