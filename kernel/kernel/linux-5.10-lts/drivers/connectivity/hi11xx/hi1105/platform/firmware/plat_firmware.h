...
#define GNSS_CFG_FILE            "gnss_cfg"
#define BSLE_CFG_FILE            "bsle_cfg"
#define BI_CFG_FILE              "bi_cfg"
#define WIFI_CFG_FILE            "wifi_cfg"
#define IR_CFG_FILE              "ir_cfg"
#define BFGX_CFG_FILE            "bfgx_cfg"
#define BSLE_COMB_CFG_FILE       "bsle_comb_cfg"
#define GNSS_COMB_CFG_FILE       "gnss_comb_cfg"
#define COMC_CFG_FILE            "comc_cfg"
#define WIFI_COMB_CFG_FILE       "wifi_comb_cfg"
#define IR_COMB_CFG_FILE         "ir_comb_cfg"
#define BGI_CFG_FILE             "bgi_cfg"
#define BGI_COMB_CFG_FILE        "bgi_comb_cfg"
...
/* gt panic结构有扩充 */
#define GT_CPU_PANIC_MEMDUMP_SIZE 512
/* 枚举定义 */
...
enum firmware_files_enum {
    FILES_CMD_SEND = 1,    // cmd发送/读取ack
    FILES_BIN_SEND = 2,    // 加载bin
    FILES_BIN_READ = 3,    // 读取memory
    FILES_CMD_BUTT,
};
...
enum firmware_cfg_file_enum {
    ...
    B_COMB_CFG,            /* B核组合加载使用 */
    G_COMB_CFG,            /* G核组合加载使用 */
    COMC_CFG,              /* 小核独立加载使用, mp17c新增 */
    WIFI_COMB_CFG,         /* 组合加载共存wifi上电只加载wifi firmware, mp17c新增 */
    IR_COMB_CFG,           /* 组合加载单红外上电只加载IR firmware, mp17c新增 */
    BGI_COMB_CFG,          /* BGI核组合加载使用 */
    CFG_FILE_TOTAL
};
...
typedef struct {
    int32_t chip_nr;
    int32_t is_asic;
    uint32_t cfg_map_size;             /* 有效cfg个数 */
    cfg_map_info *cfg_map;
    uint8_t *pre_directory;
} chip_firmware_cfg_str;

typedef struct {
    uint32_t cfg_str_size;
    chip_firmware_cfg_str *cfg_str;
} firmware_cfg_t;
...
typedef int32_t (*firmware_cust_cb)(uint8_t *key, uint8_t *value);
/* 函数声明 */
...
int32_t chip_firmware_cfg_register(chip_firmware_cfg_str *cfg, uint32_t cfg_size);
