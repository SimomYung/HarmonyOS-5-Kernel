...
static void firmware_download_fail_proc(struct pm_top *pm_top_data, int which_cfg)
{
    ...
    if ((pm_top_data->wlan_pm_info != NULL) && (pm_top_data->wlan_pm_info->pst_bus != NULL)) {
...
OAL_STATIC void post_download_process(hcc_bus *bus)
{
    hcc_bus_frw_download_post(bus);
}

/*
 * Description  : 加载前处理动作
 */
static int pre_download_process(struct pm_top *pm_top_data, firmware_downlaod_privfunc priv_func, hcc_bus *bus)
{
    int32_t ret;
    ...
    if (hcc_bus_frw_download_pre(bus) != OAL_SUCC) {
        ps_print_err("firmware msg download pre fail\n");
        return -EFAIL;
    }
...
static int firmware_download_bus_prepare(hcc_bus *bus)
{
    if (bus->bus_dev->dev_id == HCC_EP_WIFI_DEV
        ...
        || bus->bus_dev->dev_id == HCC_EP_WIFI_DEV1
#endif
    ) {
...
static int firmware_download_process(uint32_t which_cfg, firmware_downlaod_privfunc priv_func, hcc_bus *bus)
{
    ...
    ret = pre_download_process(pm_top_data, priv_func, bus);
    if (ret != SUCCESS) {
        hcc_bus_and_wake_unlock(bus);
        return ret;
    }
