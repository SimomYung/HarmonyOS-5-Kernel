 ...
 */
int32_t send_msg(uint8_t *data, int32_t len, uint32_t file_flag)
{
    int32_t l_ret;
    hcc_bus *pst_bus = hcc_get_current_mpxx_bus();

    if (unlikely((pst_bus == NULL) || (file_flag >= FILES_CMD_BUTT))) {
        ps_print_err("pst_bus is NULL or file_flag is invalid, file_flag=%d\n", file_flag);
        return -EFAIL;
    }
...
#endif
    l_ret = hcc_bus_patch_write(pst_bus, data, len, file_flag);
 ...
 */
int32_t read_msg(uint8_t *data, int32_t data_len, uint32_t file_flag)
{
    int32_t len;
    hcc_bus *pst_bus = hcc_get_current_mpxx_bus();
    ...
    len = hcc_bus_patch_read(pst_bus, data, data_len, READ_MEG_TIMEOUT, file_flag);
...
int32_t read_device_reg16(uint32_t address, uint16_t *value)
{
    ...
    ret = send_msg(buf_tx, buf_len, FILES_CMD_SEND);
    if (ret < 0) {
        ps_print_err("send msg [%s] failed, ret=%d", buf_tx, ret);
        return ret;
    }

    buf_result = os_kzalloc_gfp(READ_DEVICE_MAX_BUF_SIZE);
    if (buf_result == NULL) {
        ps_print_err("buf_result is null, vmalloc size 128 failed!");
        return -OAL_ENOMEM;
    }

    ret = read_msg(buf_result, ul_read_msg_len, FILES_BIN_READ);
    if (ret > 0) {
         /* 解析回读的内存,都是小端直接转换 */
        *value = (uint16_t)oal_readl((uintptr_t)buf_result);
...
int32_t recv_expect_result(const uint8_t *expect)
{
    ...
    for (i = 0; i < HOST_DEV_TIMEOUT; i++) {
        len = read_msg(buf, RECV_BUF_LEN, FILES_CMD_SEND);
        if (len < 0) {
...
STATIC int32_t read_msg_timeout(uint8_t *data, int32_t data_len, uint32_t timeout)
{
    ...
    len = hcc_bus_patch_read(pst_bus, data, data_len, timeout, FILES_CMD_SEND);
 ...
 * 返 回 值  : -1表示失败，0表示成功
 */
int32_t msg_send_and_recv_except(uint8_t *data, int32_t len, const uint8_t *expect, uint32_t file_flag)
{
    int32_t i;
    int32_t ret;

    for (i = 0; i < HOST_DEV_TIMEOUT; i++) {
        ret = send_msg(data, len, file_flag);
        if (ret < 0) {
            continue;
        }
