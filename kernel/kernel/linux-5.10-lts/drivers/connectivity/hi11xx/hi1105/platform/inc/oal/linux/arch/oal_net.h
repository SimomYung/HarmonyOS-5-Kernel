...
typedef struct {
    uint32_t is_ml_netdev     : 1;
    uint32_t is_link_netdev   : 1;
    uint32_t bind_with_ml_vap : 1;
    uint32_t link_id          : 8;
    uint32_t ap_link_id       : 8;
    uint32_t rsv              : 13;
    oal_net_device_stru *ml_netdev;
    oal_net_device_stru *link_netdev[MAX_ML_LINK_NUM];
} mlo_cfg_info_stru;
...
typedef struct {
    ...
    mlo_cfg_info_stru mlo_cfg_info;
...
OAL_STATIC OAL_INLINE void oal_net_free_netdev(oal_net_device_stru *pst_netdev)
{
    ...
    if (pst_netdev->reg_state == NETREG_RELEASED) {
        oal_io_print("oal_net_free_netdev released\r\n");
        return;
    }
...
void bsp_wifi_hac_free_skb(struct sk_buff *skb);
#endif
/*
...
OAL_STATIC OAL_INLINE uint32_t _oal_netbuf_free(oal_netbuf_stru *pst_netbuf)
{
    ...
    // 适配PCIE中断上下文释放内存
#ifdef _PRE_WLAN_FEATURE_ESPE
    bsp_wifi_hac_free_skb(pst_netbuf);
#else
...
#define oal_netbuf_orphan(netbuf) skb_orphan(netbuf)
...
OAL_STATIC OAL_INLINE void oal_netbuf_append(oal_netbuf_stru *old_netbuf, oal_netbuf_stru *netbuf,
                                                  oal_netbuf_head_stru *head)
{
    skb_append(old_netbuf, netbuf, head);
}

OAL_STATIC OAL_INLINE void oal_netbuf_queue_after_nolock(oal_netbuf_head_stru *head,
    oal_netbuf_stru *old_netbuf, oal_netbuf_stru *netbuf)
{
    __skb_queue_after(head, old_netbuf, netbuf);
}
...
OAL_STATIC OAL_INLINE void oal_netbuf_delete_nolock(oal_netbuf_stru *pst_buf, oal_netbuf_head_stru *pst_list_head)
{
    __skb_unlink(pst_buf, pst_list_head);
}
...
OAL_STATIC OAL_INLINE void netbuf_unlink(struct sk_buff *skb, struct sk_buff_head *list)
{
    ...
    if (oal_unlikely(skb == NULL || skb->next == NULL || skb->prev == NULL)) {
        return;
    }
    next = skb->next;
    prev = skb->prev;
...
OAL_STATIC OAL_INLINE bool oal_netlink_capable(const oal_netbuf_stru *pst_netbuf, int32_t cap)
{
#if (_PRE_OS_VERSION_LINUX == _PRE_OS_VERSION)
    return netlink_capable(pst_netbuf, cap);
#else
    return true;
#endif
}
