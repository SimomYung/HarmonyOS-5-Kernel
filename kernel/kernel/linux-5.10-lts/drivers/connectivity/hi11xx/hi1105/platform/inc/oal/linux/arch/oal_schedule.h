...
typedef struct rtc_time oal_time_stru;
...
/*
 * 函 数 名  : oal_time_before
 * 功能描述  : 判断时间戳ul_time_a是否在ul_time_b之前:
 * 返 回 值  : Return: 1 ul_time_a在ul_time_b之前; 否则 Return: 0
 */
OAL_STATIC OAL_INLINE int32_t oal_time_before(unsigned long ul_time_a, unsigned long ul_time_b)
{
    return time_before(ul_time_a, ul_time_b);
}
