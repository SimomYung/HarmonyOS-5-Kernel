...
/* 宏定义 */
#define OAL_BUS_RX_THREAD_POLICY        SCHED_FIFO
...
/* 全局变量声明 */
extern uint32_t g_hcc_exception_enable;
...
#define HCC_EP_WIFI_DEV1 (HCC_EP_WIFI_DEV + 1)
#endif
...
typedef enum _hcc_bus_power_action_type_ {
    ...
    HCC_BUS_POWER_SLEEP,              /* The action when wlan sleep */
    HCC_BUS_POWER_BUTT
...
typedef struct {
    oal_mutex_stru dev_switch_lock;
    uint8_t dev_num;
...
typedef struct _hcc_bus_opt_ops {
    ...
    int32_t (*patch_read)(hcc_bus *pst_bus, uint8_t *buff, int32_t len, uint32_t timeout, uint32_t file_flag);
    int32_t (*patch_write)(hcc_bus *pst_bus, uint8_t *buff, int32_t len, uint32_t file_flag);
    ...
    int32_t (*frw_download_pre)(hcc_bus *bus);
    void (*frw_download_post)(hcc_bus *bus);
} hcc_bus_opt_ops;
...
/**
 * @brief 获取WiFi DFR使能状态
 * @return uint32_t 异常状态值
 */
uint32_t hcc_get_exception_status(void);

/**
 * @brief 设置WiFi DFR使能状态
 * @param val 异常状态值
 */
                                                 ...
                                                 uint32_t file_flag)
{
    ...
    return hi_bus->opt_ops->patch_read(hi_bus, buff, len, timeout, file_flag);
}

OAL_STATIC OAL_INLINE int32_t hcc_bus_patch_write(hcc_bus *hi_bus, uint8_t *buff, int32_t len, uint32_t file_flag)
{
    if (oal_warn_on((hi_bus == NULL) || (buff == NULL))) {
        return -OAL_EINVAL;
    }
    ...
    return hi_bus->opt_ops->patch_write(hi_bus, buff, len, file_flag);
}
...
OAL_STATIC OAL_INLINE int32_t hcc_bus_frw_download_pre(hcc_bus *hi_bus)
{
    if (oal_unlikely(hi_bus == NULL)) {
        return -OAL_EINVAL;
    }

    if (hi_bus->opt_ops->frw_download_pre != NULL) {
        return hi_bus->opt_ops->frw_download_pre(hi_bus);
    }

    return OAL_SUCC;
}

OAL_STATIC OAL_INLINE int32_t hcc_bus_frw_download_post(hcc_bus *hi_bus)
{
    if (oal_unlikely(hi_bus == NULL)) {
        return -OAL_EINVAL;
    }

    if (hi_bus->opt_ops->frw_download_post != NULL) {
        hi_bus->opt_ops->frw_download_post(hi_bus);
    }
