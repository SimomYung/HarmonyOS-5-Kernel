...
OAL_STATIC OAL_INLINE void oal_wake_unlock_force(oal_wakelock_stru *pst_wakelock)
{
    ...
    pst_wakelock->lock_count = 0;
    // check status
    if (oal_wakelock_active(pst_wakelock) == OAL_TRUE) {
        printk(KERN_WARNING "wakelock[%s] still active, relax_count %lu, active_count %lu",
               pst_wakelock->st_wakelock.name, pst_wakelock->st_wakelock.relax_count,
               pst_wakelock->st_wakelock.active_count);
    }
