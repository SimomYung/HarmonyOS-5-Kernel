...
#define oal_any_zero_value3(_val0, _val1, _val2)             (((_val0) == 0) || ((_val1) == 0) || ((_val2) == 0))
...
OAL_STATIC OAL_INLINE uint8_t oal_strtohex(const char *c_string)
{
    ...
    str = *c_string;
    if (str >= '0' && str <= '9') {
        uc_ret = (uint8_t)(str - '0');
    } else if (str >= 'A' && str <= 'F') {
        uc_ret = (uint8_t)(str - 'A') + 10; /* 加10为了保证取出的'A'~'F'等于10~15 */
    } else if (str >= 'a' && str <= 'f') {
        uc_ret = (uint8_t)(str - 'a') + 10; /* 加10为了保证取出的'a'~'f'等于10~15 */
    }
...
{
    return (char *)OAL_STRSTR(pc_s1, pc_s2);
}
