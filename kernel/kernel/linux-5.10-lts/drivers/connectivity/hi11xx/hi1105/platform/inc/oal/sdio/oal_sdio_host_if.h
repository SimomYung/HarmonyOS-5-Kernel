...
typedef union sdio_func0_wakeup_reg_t {
    uint32_t dword;
    struct {
        uint32_t wakeup_dev          :   1;
        uint32_t version             :   1;
    } bits;
} sdio_func0_wakeup_reg;

/* offset address of sdio function0's registers(used by host driver) */
struct msdio_func0_addr {
    uint32_t wakeupdev;
    uint32_t ip_version;
};

/* offset address of sdio function1's registers(used by host driver) */
struct msdio_func1_addr {
    uint32_t fifo; /* Read Write FIFO, rw */
    uint32_t int_status; /* interrupt status and clear reg, rw */
    uint32_t int_en;  /* interrupt enable/disable reg, rw */
    uint32_t xfercount; /* notify number of bytes to be read */
    uint32_t host_msg; /* write msg to device */
    uint32_t slave_msg; /* notify Host that device has got the msg */
    uint32_t slave_msg_with_ack; /* notify Host that device has got the msg, receive the msg ack */
    uint32_t extend_sdio_func; /* extend sdio regster:int,msg,xfer,credit etc.(12B) */
    uint32_t extend_sdio_comm; /* extend comm registers, 68B */
};

/* sdio function 0、1 register's address */
struct msdio_reg_addr {
    struct msdio_func0_addr func0_addr;
    struct msdio_func1_addr func1_addr;
};
...
struct oal_sdio {
    ...
    struct msdio_reg_addr reg_addr;
};
