...
/* OAM 最大支持的VAP个数 */
#define OAM_VAP_SUPPORT_MAX_NUM_LIMIT 6
...
typedef enum {
    ...
    OAM_OTA_TYPE_MP17C_DEV = 29,
    OAM_OTA_TYPE_MP17C_HOST = 30,
    OAM_OTA_TYPE_MP17C_DEV_MPW  = 31,
    OAM_OTA_TYPE_MP17C_HOST_MPW = 32,
    OAM_OTA_TYPE_MP12_DEV = 33,
    OAM_OTA_TYPE_MP12_HOST = 34,
...
uint32_t oam_log_set_feature_level_ctx(uint8_t uc_vap_id, uint8_t en_feature_id,
                                       uint8_t en_log_level, oam_mng_ctx_stru *ctx);
/* puc_feature_id->oam_feature_enum */
uint32_t oam_get_feature_id(uint8_t *puc_feature_name, uint8_t *puc_feature_id);
...
uint32_t oam_report_80211_probe_get_switch_ctx(uint8_t en_probereq_direction,
                                               oam_80211_frame_ctx_union *oam_frame_report_un,
                                               oam_mng_ctx_stru *ctx);
...
uint8_t oam_report_dhcp_arp_get_switch_ctx(oam_mng_ctx_stru *ctx);
uint8_t oam_report_data_get_global_switch_ctx(uint8_t en_direction, oam_mng_ctx_stru *ctx);
...
/* STATISTICS模块对外接口 */
uint32_t oam_stats_report_timer_info_to_sdt_ctx(uint8_t *puc_timer_addr,
                                                uint8_t uc_timer_len,
                                                oam_mng_ctx_stru *ctx);
uint32_t oam_stats_report_event_queue_info_to_sdt_ctx(uint8_t *puc_event_queue_addr,
                                                      uint16_t us_event_queue_info_len,
                                                      oam_mng_ctx_stru *ctx);
void oam_stats_clear_stat_info(void);
uint32_t oam_stats_report_usr_info(uint16_t us_usr_id);
uint32_t oam_stats_report_mempool_info_to_sdt_ctx(uint8_t uc_pool_id,
                                                  ...
                                                  uint16_t us_subpool_free_cnt,
                                                  oam_mng_ctx_stru *ctx);
uint32_t oam_stats_report_memblock_info_to_sdt_ctx(uint8_t *puc_origin_data,
                                                   uint8_t uc_user_cnt,
                                                   uint8_t uc_pool_id,
                                                   uint8_t uc_subpool_id,
                                                   uint16_t us_len,
                                                   ...
                                                   oam_mng_ctx_stru *ctx);
void oam_stats_report_irq_info_to_sdt_ctx(uint8_t *puc_irq_info_addr, uint16_t us_irq_info_len, oam_mng_ctx_stru *ctx);
