...
#if (_PRE_OS_VERSION_LINUX == _PRE_OS_VERSION)
STATIC ktime_t g_plat_boot_time;
#endif
...
int32_t plat_init(void)
{
    ...
    l_return = mpxx_i2c_init();
    if (l_return != OAL_SUCC) {
        oal_io_print("plat_init: mpxx_i2c_init fail\r\n");
        goto i2c_init_fail;
    }
#endif
...
#if (_PRE_OS_VERSION_LINUX == _PRE_OS_VERSION)
    g_plat_boot_time = ktime_sub(ktime_get(), start_time);
#endif
    /* 启动完成后，输出打印 */
...
{
#if (_PRE_OS_VERSION_LINUX == _PRE_OS_VERSION)
    return (uint64_t)ktime_to_ms(g_plat_boot_time);
#else
    return 0;
#endif
}
