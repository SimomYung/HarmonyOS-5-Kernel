...
sdio-objs := sdio_master.o sdio_master_v120.o sdio_master_v151.o
sdio-objs := $(addprefix sdio/,$(sdio-objs))
oal-objs += oal_sdio_host.o $(sdio-objs)
oal-objs += oal_hcc_host.o oal_hcc_bus.o oal_hcc_test.o oal_hcc_test_enhance.o
ifeq ($(CONFIG_DUAL_CHIP_SUPPORT), y)
oal-objs += oal_hcc_bus_cfg.o oal_hcc_test_gt.o
endif
oal-objs += oal_softwdt.o oal_dft.o oal_workqueue.o oal_fsm.o ftrace.o

pcie-objs := pcie_dbg.o pcie_firmware.o pcie_host.o pcie_linux.o pcie_firmware_msg.o
...
pcie-pciev100-objs := pcie_iatu_v100.o
pcie-pciev100-objs := $(addprefix pcie/pciev100/,$(pcie-pciev100-objs))

pcie-pciev500-objs := pcie_iatu_v500.o
pcie-pciev500-objs := $(addprefix pcie/pciev500/,$(pcie-pciev500-objs))

pcie-pciev151-objs := pcie_iatu_v151.o
pcie-pciev151-objs := $(addprefix pcie/pciev151/,$(pcie-pciev151-objs))

pcie-chip-comm-objs := pcie_chip.o pcie_chiptest.o pcie_iatu.o pcie_pm.o pcie_changspeed.o
...
pcie-chip-mp17c-objs := pcie_chip_mp17c.o pcie_chiptest_mp17c.o pcie_iatu_mp17c.o pcie_pm_mp17c.o
pcie-chip-mp17c-objs := $(addprefix pcie/chip/mp17c/,$(pcie-chip-mp17c-objs))

pcie-chip-mp12-objs := pcie_chip_mp12.o pcie_chiptest_mp12.o pcie_iatu_mp12.o pcie_pm_mp12.o
pcie-chip-mp12-objs := $(addprefix pcie/chip/mp12/,$(pcie-chip-mp12-objs))

pcie-rc-porting-objs := socv100/pcie_kirin_porting.o socv200/pcie_kunpeng_porting.o socv300/pcie_hertz_porting.o pcie_stub_porting.o
pcie-rc-porting-objs := $(addprefix pcie/pcie_rc_porting/,$(pcie-rc-porting-objs))
...
pcie-pcs-objs += hiphy/hiphyv01/pcie_pcs_deye_patternv01.o hiphy/hiphyv02/pcie_pcs_deye_patternv02.o
pcie-pcs-objs := $(addprefix pcie/phy/,$(pcie-pcs-objs))

pcie-objs += $(pcie-edma-objs)
pcie-objs += $(pcie-ete-objs)
pcie-objs += $(pcie-pciev100-objs)
pcie-objs += $(pcie-pciev500-objs)
pcie-objs += $(pcie-pciev151-objs)
...
pcie-objs += $(pcie-chip-mp17c-objs)
pcie-objs += $(pcie-chip-mp12-objs)
pcie-objs += $(pcie-rc-porting-objs)
...
oam-objs := oam_main.o oam_event.o oam_log.o oam_statistics.o oam_config.o oam_rdr.o oam_dsm.o oam_mlog.o
...
driver-objs := plat_gpio.o plat_cali.o plat_common_clk.o plat_i2c.o cali_data_ops_default.o
ifeq ($(CONFIG_HI110X_OCTTY),yes)
driver-objs += plat_octty_uart.o
else
driver-objs += plat_uart.o
endif
...
bfgx-objs := bfgx_data_parse.o bfgx_platform_msg_handle.o bfgx_dev.o bfgx_core.o bfgx_gnss.o bfgx_sle.o bfgx_user_ctrl.o bfgx_attr_comm.o\
             bfgx_cust.o bfgx_bt.o bfgx_fm.o bfgx_ir.o bfgx_debug.o bfgx_off_find.o
ifeq ($(CONFIG_EXPORT_110X_BLUEZ), y)
...
board-mp15-objs := board_mp15.o bfgx_mp15.o mp15_exception_rst.o
ifeq ($(CONFIG_DUAL_CHIP_SUPPORT), y)
board-mp15-objs += board_dual_mp15.o gpio_dual_mp15.o
endif
...
board-mp16-objs := board_mp16.o gpio_mp16.o ssi_mp16.o bfgx_mp16.o mp16_exception_rst.o
board-mp17c-objs := gpio_mp17c.o bfgx_mp17c.o board_mp17c.o ssi_mp17c.o mp17c_exception_rst.o cali_data_ops_mp17c.o
board-mp12-objs := gpio_mp12.o bfgx_mp12.o board_mp12.o ssi_mp12.o mp12_exception_rst.o
...
board-objs := $(addprefix board/,$(board-common-objs)) $(addprefix board/mp13/,$(board-mp13-objs)) $(addprefix board/mp15/,$(board-mp15-objs)) \
              ...
              $(addprefix board/mp17c/,$(board-mp17c-objs)) $(addprefix board/mp12/,$(board-mp12-objs))
...
HI110X_INCLUDES += -I$(DRIVER_PATH)/oal/sdio
HI110X_INCLUDES += -I$(DRIVER_PATH)/oal/pcie
HI110X_INCLUDES += -I$(DRIVER_PATH)/oal/pcie/pciev100
HI110X_INCLUDES += -I$(DRIVER_PATH)/oal/pcie/pciev500
HI110X_INCLUDES += -I$(DRIVER_PATH)/oal/pcie/pciev151
...
HI110X_INCLUDES += -I$(DRIVER_PATH)/board/mp17c
HI110X_INCLUDES += -I$(DRIVER_PATH)/board/mp12
...
ifeq ($(PRODUCT_CONFIG), E5)
#hertz_udp平台适配linux路径
HI110X_INCLUDES += $(ATP_INCS_PATH)
HI110X_INCLUDES += $(E5_UDP)
HI110X_INCLUDES += -I$(CODE_TOPDIR)/app/package/atp/libraries/private/ssp/mlog/mlog_print_ker/src
endif
