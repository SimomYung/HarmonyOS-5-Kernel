...
unsigned long g_hcc_ready_msg_time;
EXPORT_SYMBOL_GPL(g_hcc_ready_msg_time);
...
oal_atomic g_bus_powerup_dev_wait_ack_mpxx_dev1;
#endif
#endif
...
uint32_t hcc_get_exception_status(void)
{
    return g_hcc_exception_enable;
}

void hcc_set_exception_status(uint32_t val)
{
    g_hcc_exception_enable = val;
}

#if (_PRE_OS_VERSION_LINUX == _PRE_OS_VERSION)
void hcc_bus_set_powerup_dev_wait_ack(hcc_bus *hi_bus, int32_t val)
{
#ifdef _PRE_DUAL_CHIP
    if (hi_bus->dev_id == HCC_EP_WIFI_DEV1) {
        oal_atomic_set(&g_bus_powerup_dev_wait_ack_mpxx_dev1, val);
    } else {
        oal_atomic_set(&g_bus_powerup_dev_wait_ack, val);
    }
#else
    oal_atomic_set(&g_bus_powerup_dev_wait_ack, val);
#endif
...
int32_t hcc_bus_read_powerup_dev_wait_ack(hcc_bus *hi_bus)
{
#ifdef _PRE_DUAL_CHIP
    if (hi_bus->dev_id == HCC_EP_WIFI_DEV1) {
        return oal_atomic_read(&g_bus_powerup_dev_wait_ack_mpxx_dev1);
    } else {
        return oal_atomic_read(&g_bus_powerup_dev_wait_ack);
    }
#else
    return oal_atomic_read(&g_bus_powerup_dev_wait_ack);
#endif
...
uint32_t hcc_get_dev_id(hcc_bus *pst_bus)
{
#ifdef _PRE_DUAL_CHIP
    return pst_bus->dev_id;
#else
    return HCC_EP_WIFI_DEV;
#endif
}
...
int32_t hcc_get_bus_dev_num(void)
{
    return g_bus_devs.dev_num;
}

void hcc_set_main_dev(int32_t dev_id)
{
    oal_print_mpxx_log(MPXX_LOG_INFO, "set main dev_id:%d", dev_id);
    g_bus_devs.main_dev = dev_id;
}
oal_module_symbol(hcc_set_main_dev);

int32_t hcc_get_main_dev(void)
{
    return g_bus_devs.main_dev;
}
oal_module_symbol(hcc_get_main_dev);

void hcc_main_dev_id_lock(void)
{
#if (_PRE_OS_VERSION_LINUX == _PRE_OS_VERSION)
#ifdef _PRE_PLAT_FEATURE_MULTI_HCC
    mutex_lock(&g_bus_devs.dev_switch_lock);
#endif
#endif
}
oal_module_symbol(hcc_main_dev_id_lock);

void hcc_main_dev_id_unlock(void)
{
#if (_PRE_OS_VERSION_LINUX == _PRE_OS_VERSION)
#ifdef _PRE_PLAT_FEATURE_MULTI_HCC
    mutex_unlock(&g_bus_devs.dev_switch_lock);
#endif
#endif
}
oal_module_symbol(hcc_main_dev_id_unlock);
...
void hcc_bus_exception_submit(hcc_bus *hi_bus, int32_t excep_type)
{
    ...
    pst_excp_workqueue = wifi_get_exception_workqueue(hi_bus);
...
OAL_STATIC void hcc_bus_recv_last_word_process(hcc_bus *hi_bus)
{
    int32_t ret;
    oal_pcie_linux_res *pst_pci_lres = (oal_pcie_linux_res *)hi_bus->data;
    if (hi_bus->bus_type == HCC_BUS_PCIE) {
        ret = oal_pcie_slave_soc_config(pst_pci_lres, PCI_CPU_DUMP);
        if (ret != OAL_SUCC) {
            ps_print_warning("cpu dump last word not config\n");
        }
    }
}
...
OAL_STATIC int32_t hcc_bus_device_panic_callback(void *data)
{
    ...
    hcc_bus_recv_last_word_process(hi_bus);
    hcc_bus_exception_submit(hi_bus, HCC_BUS_PANIC);
    oal_print_mpxx_log(MPXX_LOG_INFO, "receive dev panic msg");
    return OAL_SUCC;
}

OAL_STATIC int32_t hcc_bus_system_reset_callback(void *data)
{
    hcc_bus *hi_bus = (hcc_bus *)data;
    hcc_bus_disable_state(hi_bus, OAL_BUS_STATE_ALL);
    hcc_bus_recv_last_word_process(hi_bus);
    hcc_bus_exception_submit(hi_bus, WIFI_SYSTEM_RESET);
    oal_print_mpxx_log(MPXX_LOG_INFO, "receive system reset msg");
    return OAL_SUCC;
}
...
OAL_STATIC int32_t hcc_bus_device_init_ready(void *data)
{
    hcc_bus *hi_bus = (hcc_bus *)data;
#ifdef _PRE_DUAL_CHIP
    oal_print_mpxx_log(MPXX_LOG_INFO, "device ready, bus[%d]:%s", hi_bus->dev_id,
                       hcc_bus_get_bus_type_str(hi_bus->bus_type));
#else
    ...
    g_hcc_ready_msg_time = oal_read_once(OAL_TIME_JIFFY);
    return OAL_SUCC;
}
...
OAL_STATIC int32_t wait_wlan_wkup_gpio_level(int32_t gpio_level, unsigned long timeout, hcc_bus *hi_bus)
{
    int32_t gpio_value;
    gpio_level = (gpio_level == GPIO_LOWLEVEL) ? GPIO_LOWLEVEL : GPIO_HIGHLEVEL;

    forever_loop() {
        gpio_value = conn_get_gpio_level(hi_bus->bus_dev->wkup_host_io_type);
        ...
        if (time_after(OAL_TIME_JIFFY, timeout)) {
            ...
            if (((wlan_is_shutdown() == OAL_TRUE) && (hi_bus->dev_id == HCC_EP_WIFI_DEV)) ||
                ((gt_is_shutdown() == OAL_TRUE) && (hi_bus->dev_id == HCC_EP_GT_DEV))) {
...
OAL_STATIC int32_t oal_dev2host_gpio_hold_time_check(uint32_t switch_timeout, uint32_t hold_time, hcc_bus *hi_bus)
{
    ...
    // wait gpio high
    if (wait_wlan_wkup_gpio_level(GPIO_HIGHLEVEL, timeout, hi_bus) != SUCCESS) {
    ...
    if (wait_wlan_wkup_gpio_level(GPIO_LOWLEVEL, timeout, hi_bus) == SUCCESS) {
...
OAL_STATIC int32_t panic_status_check_by_gpio(hcc_bus *hi_bus)
{
    int32_t ret = OAL_FALSE;
    uint32_t timeout, hold_time;

    /* 如果GPIO没有分配, 则不检查GPIO状态来确定是否device panic，保持原异常类型 */
    if (conn_gpio_valid(hi_bus->bus_dev->wkup_host_io_type) == OAL_FALSE) {
        ret = OAL_FALSE;
    } else {
        hcc_bus_panic_get_checktime(&timeout, &hold_time);

        /* gpio mode,两参数均代表设置的超时 */
        if (oal_dev2host_gpio_hold_time_check(timeout, hold_time, hi_bus) == OAL_TRUE) {
            ret = OAL_TRUE;
        } else {
            ret = OAL_FALSE;
        }
...
OAL_STATIC int32_t hcc_bus_panic_status_check(hcc_bus *hi_bus)
{
    ...
    if ((get_mpxx_subchip_type() != BOARD_VERSION_MP16C) && (get_mpxx_subchip_type() != BOARD_VERSION_MP17C) &&
        (get_mpxx_subchip_type() != BOARD_VERSION_MP12)) {
        ret = panic_status_check_by_gpio(hi_bus);
        return ret;
    }

    if (hi_bus->bus_excp_type == HCC_BUS_PANIC || hi_bus->bus_excp_type == WIFI_SYSTEM_RESET) {
        return OAL_TRUE;
    }
...
OAL_STATIC uint32_t hcc_bus_get_exception_mem_info_mp12(hcc_bus *hi_bus)
{
    oal_print_mpxx_log(MPXX_LOG_INFO, "mp12 memdump");

    /* mp12 Memdump */
    hi_bus->mem_info = g_mp12_meminfo;
    hi_bus->mem_size = (uint32_t)(sizeof(g_mp12_meminfo) / sizeof(struct st_wifi_dump_mem_info));
    return OAL_SUCC;
}
...
int32_t hcc_bus_get_exception_mem_info(hcc_bus *hi_bus)
{
    ...
    if (get_mpxx_subchip_type() == BOARD_VERSION_MP15) {
        ...
        if (get_mpxx_subchip_version() == CHIP_VERSION_MPW) {
            /* mp17c mpw */
            mem_info = g_mp17c_mpw_meminfo;
            size = (uint32_t)(sizeof(g_mp17c_mpw_meminfo) / sizeof(struct st_wifi_dump_mem_info));
        } else {
            /* mp16c Memdump */
    ...
    } else if (get_mpxx_subchip_type() == BOARD_VERSION_MP17C) {
        oal_print_mpxx_log(MPXX_LOG_INFO, "mp17c memdump");

        /* mp17c Memdump */
        mem_info = g_mp17c_meminfo;
        size = (uint32_t)(sizeof(g_mp17c_meminfo) / sizeof(struct st_wifi_dump_mem_info));
    } else if (get_mpxx_subchip_type() == BOARD_VERSION_MP12) {
        return hcc_bus_get_exception_mem_info_mp12(hi_bus);
    } else {
...
OAL_STATIC void hcc_bus_print_excp_type(hcc_bus *hi_bus)
{
    ...
    } else if (hi_bus->bus_excp_type == HCC_BUS_TRANS_FAIL) {
        declare_dft_trace_key_info("wifi_trans_fail", OAL_DFT_TRACE_EXCEP);
        oam_error_log0(0, OAM_SF_ANY, "[E]wifi_trans_fail");
    } else if (hi_bus->bus_excp_type == WIFI_SYSTEM_RESET) {
        declare_dft_trace_key_info("wifi_system_reset", OAL_DFT_TRACE_EXCEP);
        oam_error_log0(0, OAM_SF_ANY, "[E]WiFi System Reset");
    }
}
...
OAL_STATIC void hcc_bus_exception_handler(oal_work_stru *work)
{
    ...
    hcc_bus_print_excp_type(hi_bus);
    ...
    if (oal_likely(hi_bus->pst_pm_callback != NULL && hi_bus->pst_pm_callback->pm_disable != NULL)) {
    ...
    if (((wlan_is_shutdown() == OAL_TRUE) && (hi_bus->dev_id == HCC_EP_WIFI_DEV)) ||
#ifdef _PRE_DUAL_CHIP
        ((wlan_is_shutdown() == OAL_TRUE) && (hi_bus->dev_id == HCC_EP_WIFI_DEV1)) ||
#endif
        ((gt_is_shutdown() == OAL_TRUE) && (hi_bus->dev_id == HCC_EP_GT_DEV))) {
    ...
    wlan_exception_work_submit(hi_bus, hi_bus->bus_excp_type);
...
OAL_STATIC int32_t oal_register_gpio_intr(hcc_bus_dev *pst_bus_dev)
{
    ...
    pst_bus_dev->wkup_host_io_type = type;
...
OAL_STATIC void oal_unregister_gpio_intr(hcc_bus_dev *pst_bus_dev)
{
    ...
    type = pst_bus_dev->wkup_host_io_type;
...
OAL_STATIC void oal_flowctrl_gpio_intr_enable(hcc_bus_dev *pst_bus_dev, oal_bool_enum_uint8 en_enable)
{
    if (en_enable) {
        ...
        if (!pst_bus_dev->en_flowctrl_irq_stat) {
            /* 使能GPIO流控中断 */
            conn_gpio_intr_enable(SDIO_FLOW_CTRL, en_enable);
            oal_print_mpxx_log(MPXX_LOG_WARN, "gpio flowctrl irq enable");
            /* 同步当前流控状态为使能 */
        ...
        if (pst_bus_dev->en_flowctrl_irq_stat) {
            /* 关闭GPIO流控中断 */
            conn_gpio_intr_enable(SDIO_FLOW_CTRL, en_enable);
            oal_print_mpxx_log(MPXX_LOG_WARN, "gpio flowctrl irq disable nosync");
            /* 同步当前流控状态为关闭 */
...
OAL_STATIC int32_t oal_register_gpio_flowctrl_intr(hcc_bus_dev *pst_bus_dev)
{
    ...
    ret = conn_irq_register(SDIO_FLOW_CTRL, hcc_bus_flowctrl_gpio_irq, pst_bus_dev);
    if (ret < 0) {
...
{
    /* disable wlan irq */
    conn_gpio_intr_enable(SDIO_FLOW_CTRL, OAL_FALSE);

    /* free irq when sdio driver deinit */
    conn_irq_unregister(SDIO_FLOW_CTRL, pst_bus_dev);
...
OAL_STATIC int32_t hcc_bus_resource_alloc(hcc_bus *pst_bus)
{
    ...
    oal_atomic_set(&pst_bus->wakeup_dev_wait_ack, 0);
    oal_atomic_set(&g_bus_powerup_dev_wait_ack, 0);
#ifdef _PRE_DUAL_CHIP
    oal_atomic_set(&g_bus_powerup_dev_wait_ack_mpxx_dev1, 0);
#endif
    ...
    hcc_bus_message_register(pst_bus, D2H_MSG_SYSTEM_RESET, hcc_bus_system_reset_callback, pst_bus);
    return OAL_SUCC;
}
...
int32_t hcc_dev_init(void)
{
    ...
    if ((g_st_board_info.wlan_download_channel == CHANNEL_PCIE) &&
        (g_bus_devs.dev_num != oal_pcie_get_probed_dev_num())) {
        oal_print_mpxx_log(MPXX_LOG_WARN, "g_bus_devs.dev_num[%d] != g_pcie_probed_dev_num[%d]!!! please check!!!\n\n",
                           g_bus_devs.dev_num, oal_pcie_get_probed_dev_num());
        g_bus_devs.dev_num = oal_pcie_get_probed_dev_num();
        dev_num = g_bus_devs.dev_num;
    }
#endif
...
void resume_wlan_wakeup_host_gpio(void)
{
    ...
    gpio_type = pst_bus_dev->wkup_host_io_type;
    conn_irq_unregister(gpio_type, pst_bus_dev);
    phy_gpio = conn_get_gpio_number_by_type(gpio_type);
    ...
    conn_irq_register(gpio_type, hcc_bus_wlan_gpio_irq, pst_bus_dev);
}
#endif
...
int32_t oal_wifi_platform_load_dev(void)
{
    ...
    if (sdio_cap == OAL_TRUE) {
        ret = oal_wifi_platform_load_sdio();
        if (ret != OAL_SUCC) {
            oal_print_mpxx_log(MPXX_LOG_ERR, "load sdio failed ret=%d", ret);
            return ret;
        }
    }
#endif
