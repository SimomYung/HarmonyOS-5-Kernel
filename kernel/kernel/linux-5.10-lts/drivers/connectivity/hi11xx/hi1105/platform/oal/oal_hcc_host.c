...
void hcc_tx_net_queue_map_init(struct hcc_handler *hcc)
{
    hcc_tx_wlan_queue_map_set(hcc, DATA_HI_QUEUE, WLAN_HI_QUEUE);
    hcc_tx_wlan_queue_map_set(hcc, DATA_LO_QUEUE, WLAN_NORMAL_QUEUE);
    hcc_tx_wlan_queue_map_set(hcc, DATA_TCP_DATA_QUEUE, WLAN_TCP_DATA_QUEUE);
    hcc_tx_wlan_queue_map_set(hcc, DATA_TCP_ACK_QUEUE, WLAN_TCP_ACK_QUEUE);
    hcc_tx_wlan_queue_map_set(hcc, DATA_UDP_BK_QUEUE, WLAN_UDP_BK_QUEUE);
    hcc_tx_wlan_queue_map_set(hcc, DATA_UDP_BE_QUEUE, WLAN_UDP_BE_QUEUE);
    hcc_tx_wlan_queue_map_set(hcc, DATA_UDP_VI_QUEUE, WLAN_UDP_VI_QUEUE);
    hcc_tx_wlan_queue_map_set(hcc, DATA_UDP_VO_QUEUE, WLAN_UDP_VO_QUEUE);
}
                                       ...
                                       struct hcc_transfer_param *param)
{
    ...
    if (param->fc_flag) {
        ...
        if (HCC_FC_WAIT & param->fc_flag) {
            /* condition为true时等待退出, 队列低于水线或通道非normal状态退出 */
            ret = oal_wait_event_interruptible_timeout_m(hcc->hcc_transer_info.tx_flow_ctrl.wait_queue,
                ...
                (hcc_tx_cond_check(hcc) != OAL_SUCC), OAL_SEC_PER_MIN * OAL_TIME_HZ);
            if (ret == 0) {
            ...
            ret1 = hcc_tx_cond_check(hcc);
            if (ret1 != OAL_SUCC) {
                /* hcc abnormal or powerdown, drop pkts */
                return ret1;
            }
        }
...
OAL_STATIC int32_t hcc_thread_wait_event_cond_check(struct hcc_handler *hcc)
{
    ...
    if (hcc->bus_dev->cur_bus->bus_type == HCC_BUS_PCIE) {
        ret = ((_queues_len_check(hcc, HCC_RX)) ||
               ...
               (hcc->p_hmac_tcp_ack_need_schedule_func &&
#ifdef _PRE_DUAL_CHIP
                hcc->p_hmac_tcp_ack_need_schedule_func(hcc->bus_dev->dev_id) == OAL_TRUE));
#else
...
#ifdef _PRE_DUAL_CHIP
                hcc->p_hmac_tcp_ack_process_func(hcc->bus_dev->dev_id);
#else
