...
struct hcc_test_event {
    ...
    /* 流量控制,interval间隔控制,毫毛级别，有效范围0~999ms,表示1秒内有interval_ms主动空闲 */
...
OAL_STATIC int32_t hcc_test_set_case(hcc_test_data *data);
...
static int32_t hcc_queues_info_print_flowctrl(struct hcc_handler *hcc, char *buf, int32_t buf_len)
{
    ...
    ret = snprintf_s(buf + count, buf_len - count, buf_len - count - 1,
                     "rx queue:%4d,low wl:%u,high wl:%u, enabled:%u\n",
                     DATA_LO_QUEUE,
                     pst_hcc_queue->flow_ctrl.low_waterline,
...
int32_t hcc_print_current_trans_info(uint32_t print_device_info)
{
    ...
    hcc = hcc_get_handler(hcc_get_dev_id(wlan_pm_info->pst_bus));
    if (hcc == NULL) {
        oal_io_print("get hcc handler failed!%s\n", __FUNCTION__);
        return -OAL_EFAIL;
    }

    pst_bus = hcc_get_bus(hcc_get_dev_id(wlan_pm_info->pst_bus));
...
OAL_STATIC ssize_t hcc_test_pcie_loopback_get_para(struct kobject *dev, struct kobj_attribute *attr, char *buf)
{
    ...
    if (dev == NULL || attr == NULL || buf == NULL) {
        oal_print_mpxx_log(MPXX_LOG_ERR, "dev or attr or buf is null");
        return 0;
    }
...
OAL_STATIC unsigned long hcc_test_get_interval_param(void)
{
    /* TEST_LOOP_CALIC_INTERVAL:最大不超过TEST_LOOP_CALIC_INTERVAL-1 ms */
    uint32_t interval_ms = g_hcc_test_event->interval_ms > (TEST_LOOP_CALIC_INTERVAL - 1) ?
                          (TEST_LOOP_CALIC_INTERVAL - 1) : g_hcc_test_event->interval_ms;
    uint32_t run_ms = TEST_LOOP_CALIC_INTERVAL - interval_ms;
    oal_print_mpxx_log(MPXX_LOG_INFO, "interval_ms = %u, run_ms = %u\n", interval_ms, run_ms);
    g_hcc_test_event->interval_ms = interval_ms;
    return oal_msecs_to_jiffies(run_ms);
}

#define HCC_TEST_INTERVAL_SLEEP_LIMIT 20 // ms
/* 混用高精度低精度睡眠定时器,线程上下文中调用,返回实际的休眠时间 */
OAL_STATIC uint32_t hcc_test_interval_sleep(uint32_t ms)
{
    if (ms == 0) {
        return 0;
    } else if (ms <= HCC_TEST_INTERVAL_SLEEP_LIMIT) {
        oal_usleep_range(ms * 1000, ms * 1000); // 1000:us to ms
        return ms;
    } else {
        unsigned long ret = msleep_interruptible(ms);
        if (ret != 0) {
            /* 被异常终止 */
            oal_print_mpxx_log(MPXX_LOG_WARN, "msleep_interruptible ret = %lu, request time=%u\n", ret, ms);
        }
        return (uint32_t)ret;
    }
}
...
OAL_STATIC int32_t hcc_test_normal_start(uint16_t start_cmd)
{
    ...
    int32_t i;
    uint32_t interval_ms;
    unsigned long run_tick, last_tick;
    hcc_test_cmd_stru cmd = {0};
    ...
    /* 间隔最大不能超过1秒 */
    run_tick = hcc_test_get_interval_param();
    interval_ms = g_hcc_test_event->interval_ms;
    /* 更新第一次统计窗口时间戳 */
    last_tick = jiffies;
    ...
    for (i = 0; i < g_hcc_test_event->test_data.pkt_gen; i++) {
        ...
        /* 流量控制, 每TEST_LOOP_CALIC_INTERVAL ms内空闲interval ms时间，
           可以保证xx ms窗口内还是满负荷,同时芯片不至于过温 */
        if (interval_ms > 0) {
            if (time_after(jiffies, (last_tick + run_tick))) {
                /* 已运行run_tick数，强制休眠 */
                hcc_test_interval_sleep(interval_ms);
                /* 更新下一次统计窗口时间戳 */
                last_tick = jiffies;
            }
        }
...
OAL_STATIC ssize_t hcc_test_print_thoughput(char *buf, uint32_t buf_len)
{
    ...
    ret = snprintf_s(buf + count, buf_len - count, buf_len - count - 1, "Throughput %llu Mbps\n",
...
int32_t hcc_test_set_case(hcc_test_data *data)
{
    int32_t ret;
    int32_t errorno;
    ...
    ret = (int32_t)wait_for_completion_interruptible(&g_hcc_test_event->test_done);
    if (ret < 0) {
        oal_io_print("Test Event  terminated ret=%d\n", ret);
                                             ...
                                             int32_t retry_cnt)
{
    ...
    ret = wlan_pm_open_dev(HCC_EP_WIFI_DEV, OAL_TRUE);
    if (ret != OAL_SUCC) {
        return ret;
    }
...
retry:
    ...
    if (pst_buff != NULL) {
        memset_s(pst_buff, PAGE_SIZE, 0, PAGE_SIZE);
    ...
    if (wlan_pm_close_dev(HCC_EP_WIFI_DEV) != OAL_SUCC) {
...
/* 测试高吞吐流量PCIE负载，用于产线等场景,需要控制执行时间和流量, 由外部来上电WiFi芯片 */
int32_t hcc_test_loop_pcie_chan(uint32_t interval_ms, unsigned long trans_timeout, uint64_t *throught)
{
    ...
    data.pkt_len = 1500; // 1500: 长包测试
    data.pkt_gen = 0x7fffffff; // 按照运行时间执行，包个数不限制
    data.trans_timeout = trans_timeout; // 用例执行时间，外部传入
    data.mode_idx = HCC_TEST_CASE_LOOP; // 环回测试，先发再收
    ...
    *throught = 0;
    ...
    if (pst_bus->bus_type != HCC_BUS_PCIE) {
        oal_print_mpxx_log(MPXX_LOG_ERR, "bus type is not PCIe");
        return -OAL_EFAIL;
    }
    ...
    ret = hcc_test_fix_wcpu_freq();
    if (ret) {
        oal_print_mpxx_log(MPXX_LOG_ERR, "set  wcpu fix freq failed! ret=%d, maybe hcc closed", ret);
        return ret;
    }
    ...
    save_interval = g_hcc_test_event->interval_ms;
    g_hcc_test_event->interval_ms = interval_ms;
    ...
    /* 返回实际的吞吐率 */
    *throught = g_hcc_test_event->test_data.throughput;
    /* 恢复interval设置 */
    g_hcc_test_event->interval_ms = save_interval;
    return OAL_SUCC;
}

/* 正常停止hcc回环，只针对指定运行时间的模式 */
void hcc_test_loop_pcie_chan_abort(void)
{
    g_hcc_test_event->tick_timeout = oal_msecs_to_jiffies(0);
}
#endif
...
int32_t hcc_test_init_module(struct hcc_handler *hcc)
{
    ...
    if (hcc->bus_dev->dev_id == HCC_EP_WIFI_DEV1) {
