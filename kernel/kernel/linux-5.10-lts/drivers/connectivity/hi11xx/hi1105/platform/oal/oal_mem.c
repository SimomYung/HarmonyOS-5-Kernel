...
OAL_STATIC uint8_t g_gt_mem_pool_local[TOTAL_GT_MEM_LOCAL_SIZE];
...
OAL_STATIC uint8_t g_gt_mem_pool_local[TOTAL_GT_MEM_LOCAL_SIZE] __attribute__((aligned(8)));
...
#define OAL_MEM_BLK_TOTAL_CNT_GT (WLAN_MEM_SHARED_RX_DSCR_CNT + WLAN_MEM_SHARED_TX_DSCR_CNT1 +  \
                               WLAN_MEM_SHARED_TX_DSCR_CNT2 + WLAN_MEM_SHARED_MGMT_PKT_CNT1 +   \
                               WLAN_MEM_SHARED_DATA_PKT_CNT + GT_MEM_LOCAL_CNT1 +               \
                               GT_MEM_LOCAL_CNT2 + GT_MEM_LOCAL_CNT3 +                          \
                               GT_MEM_LOCAL_CNT4 + GT_MEM_LOCAL_CNT5 +                          \
                               GT_MEM_LOCAL_CNT6 + WLAN_MEM_EVENT_CNT1 +                        \
                               WLAN_MEM_EVENT_CNT2 + WLAN_MEM_TX_RING_CNT1)
...
oal_mem_pool_context_stru g_gt_mem_pool_ctx = {
    .ctrl_blk = {NULL, OAL_MEM_BLK_TOTAL_CNT_GT * OAL_MEM_CTRL_BLK_SIZE, 0},
...
OAL_STATIC void oal_mem_reset_alloc_info(oal_mem_stru *pst_mem)
{
                               ...
                               oal_mem_pool_context_stru *context)
{
    ...
    oal_mem_reset_alloc_info(pst_mem);
                                                           ...
                                                           oal_mem_pool_context_stru *context)
{
    ...
    for (uc_index = 0; uc_index < uc_mem_num; uc_index++) {
        ...
        /* 该内存块上是否还有其他共享用户，直接返回 */
        if (oal_unlikely((pst_mem->uc_user_cnt != 0) || (pst_mem_subpool == NULL))) {
            continue;
        }
...
OAL_STATIC void oal_print_memory_leak_info(oal_mem_pool_stat *past_stat_start, oal_mem_pool_stat *past_stat_end,
                                           oal_mem_pool_context_stru *context, uint8_t bitmap)
{
                                  ...
                                  oal_mem_pool_context_stru *context)
{
    ...
    oal_print_memory_leak_info(past_stat_start, past_stat_end, context, uc_bitmap);
    return OAL_TRUE;
}
