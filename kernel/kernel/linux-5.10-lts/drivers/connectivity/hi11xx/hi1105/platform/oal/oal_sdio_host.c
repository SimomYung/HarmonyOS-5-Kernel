...
/* 函数定义 */
OAL_STATIC int32_t oal_sdio_transfer_scatt_t(struct oal_sdio *hi_sdio, int32_t rw, uint32_t addr,
                                             struct scatterlist *sg, uint32_t sg_len, uint32_t rw_sz);
...
int32_t oal_sdio_send_msg(hcc_bus *pst_bus, uint32_t val)
{
    ...
    oal_sdio_writel(func, (1 << val),
                    hi_sdio->reg_addr.func1_addr.host_msg, &ret);
    if (ret) {
...
OAL_STATIC int32_t sdio_dev_init(struct oal_sdio *hi_sdio)
{
     ...
     *
     */
    oal_sdio_writeb(func, HISDIO_FUNC1_INT_MASK, hi_sdio->reg_addr.func1_addr.int_en, &ret);
    if (ret < 0) {
...
OAL_STATIC OAL_INLINE int32_t oal_sdio_xfercount_get(struct oal_sdio *hi_sdio, uint32_t *xfercount)
{
    ...
    /* read from 0x0c */
    *xfercount = oal_sdio_readl(hi_sdio->func, hisdio->reg_addr.func1_addr.xfercount, &ret);
    if (oal_unlikely(ret)) {
    ...
    /* read from 0x0c */
    *xfercount = oal_sdio_readl(hi_sdio->func, hi_sdio->reg_addr.func1_addr.xfercount, &ret);
    if (oal_unlikely(ret)) {
...
OAL_STATIC OAL_INLINE int32_t oal_sdio_msg_stat(struct oal_sdio *hi_sdio, uint32_t *msg)
{
    ...
    *msg = oal_sdio_readl(hi_sdio->func, hisdio->reg_addr.func1_addr.slave_msg, &ret);
#else
    *msg = oal_sdio_readb(hi_sdio->func, hisdio->reg_addr.func1_addr.slave_msg, &ret);
#endif
    ...
    (void)oal_sdio_readb(hi_sdio->func, hi_sdio->reg_addr.func1_addr.slave_msg_with_ack, &ret);
    if (ret) {
...
OAL_STATIC int32_t oal_sdio_extend_buf_get(struct oal_sdio *hi_sdio)
{
    ...
    if (g_sdio_extend_func) {
        ret = oal_sdio_memcpy_fromio(hi_sdio->func, (void *)hi_sdio->sdio_extend,
                                     hi_sdio->reg_addr.func1_addr.extend_sdio_func, sizeof(struct hisdio_extend_func));
...
OAL_STATIC void oal_sdio_transfer_rx_reserved_buff(struct oal_sdio *hi_sdio)
{
    ...
    ret = oal_sdio_transfer_scatt_t(hi_sdio, SDIO_READ, hi_sdio->reg_addr.func1_addr.fifo, sg, seg_nums, ul_extend_len);
    if (oal_unlikely(ret)) {
...
OAL_STATIC OAL_INLINE int32_t oal_sdio_get_func1_int_status(struct oal_sdio *hi_sdio, uint8_t *int_stat)
{
    ...
    *int_stat = oal_sdio_readb(hi_sdio->func, hisdio->reg_addr.func1_addr.int_status, &ret);
    if (oal_unlikely(ret)) {
    ...
    if (g_sdio_extend_func) {
        ...
        *int_stat = (hi_sdio->sdio_extend->int_stat & 0xF);
        ...
        /* read interrupt indicator register */
        *int_stat = oal_sdio_readb(hi_sdio->func, hi_sdio->reg_addr.func1_addr.int_status, &ret);
        if (oal_unlikely(ret)) {
...
OAL_STATIC OAL_INLINE int32_t oal_sdio_clear_int_status(struct oal_sdio *hi_sdio, uint8_t int_stat)
{
    ...
    oal_sdio_writeb(hi_sdio->func, int_stat, hisdio->reg_addr.func1_addr.int_status, &ret);
    if (oal_unlikely(ret)) {
    ...
    /* clear interrupt mask */
    oal_sdio_writeb(hi_sdio->func, int_stat, hi_sdio->reg_addr.func1_addr.int_status, &ret);
    if (oal_unlikely(ret)) {
        oal_print_mpxx_log(MPXX_LOG_ERR, "faild to clear sdio func1 interrupt!ret=%d", ret);
        return ret;
    }
...
OAL_STATIC int32_t oal_sdio_get_sleep_reg(struct oal_sdio *hi_sdio)
{
    ...
    struct msdio_func0_addr *f0_addr = &hi_sdio->reg_addr.func0_addr;

    oal_sdio_claim_host(hi_sdio);
    wakeup_data.dword = (uint32_t)sdio_f0_readb(hi_sdio->func, f0_addr->wakeupdev, &ret);
    oal_sdio_release_host(hi_sdio);

    if (oal_unlikely(ret != 0)) {
        oal_print_mpxx_log(MPXX_LOG_ERR, "read func0 reg 0x%x failed, ret=%d", f0_addr->wakeupdev, ret);
    }

    return (int32_t)wakeup_data.bits.wakeup_dev;
}
...
OAL_STATIC int32_t oal_sdio_wakeup_dev(struct oal_sdio *hi_sdio)
{
    ...
    wakeup_data.dword = 0;
    wakeup_data.bits.wakeup_dev = DISALLOW_TO_SLEEP_VALUE;
    oal_sdio_claim_host(hi_sdio);
    sdio_f0_writeb(hi_sdio->func, (uint8_t)wakeup_data.dword, f0_addr->wakeupdev, &ret);
    oal_sdio_release_host(hi_sdio);
...
OAL_STATIC int32_t oal_sdio_sleep_dev(struct oal_sdio *hi_sdio)
{
    ...
    wakeup_data.dword = 0;
    wakeup_data.bits.wakeup_dev = ALLOW_TO_SLEEP_VALUE;
    oal_sdio_claim_host(hi_sdio);
    sdio_f0_writeb(hi_sdio->func, (uint8_t)wakeup_data.dword, f0_addr->wakeupdev, &ret);
    oal_sdio_release_host(hi_sdio);
...
OAL_STATIC int32_t oal_sdio_writesb_by_heap(struct oal_sdio *hi_sdio, uint8_t *buf, uint32_t len)
{
    ...
    if (virt_addr_valid(buf) && !object_is_on_stack(buf)) {
        ret = oal_sdio_writesb(func, hi_sdio->reg_addr.func1_addr.fifo, buf, len);
        if (ret < 0) {
        ...
        if ((len < HISDIO_SEND_SIZE) && (hi_sdio->sdio_send_buff != NULL)) {
            ...
            ret = oal_sdio_writesb(func, hi_sdio->reg_addr.func1_addr.fifo, send_buf, len);
            if (ret < 0) {
                oal_print_mpxx_log(MPXX_LOG_ERR, "write error:%d\n", ret);
            }
                                         ...
                                         uint32_t len, uint32_t timeout)
{
    ...
    forever_loop() {
        int_mask = oal_sdio_readb(func, pst_sdio->reg_addr.func1_addr.int_status, &ret);
        if (ret) {
    ...
    oal_sdio_writeb(func, int_mask, pst_sdio->reg_addr.func1_addr.int_status, &ret);
    if (ret < 0) {
    ...
    xfer_count = oal_sdio_readl(func, pst_sdio->reg_addr.func1_addr.xfercount, &ret);
    if (ret < 0) {
    ...
    /* kzalloc had memset */
    ret = oal_sdio_readsb(func, ver_info, pst_sdio->reg_addr.func1_addr.fifo, xfer_count);
    if (ret < 0) {
...
OAL_STATIC int32_t oal_sdio_patch_write(hcc_bus *pst_bus, uint8_t *buff, int32_t len, uint32_t file_flag)
{
    struct oal_sdio *hi_sdio = (struct oal_sdio *)pst_bus->data;
    if (oal_warn_on(hi_sdio == NULL)) {
        oal_print_mpxx_log(MPXX_LOG_ERR, "hi_sdio is null");
...
OAL_STATIC int32_t oal_sdio_dev_init(struct oal_sdio *hi_sdio)
{
    ...
    oal_sdio_writeb(func, HISDIO_FUNC1_INT_MASK, hi_sdio->reg_addr.func1_addr.int_status, &ret);
    if (ret) {
...
OAL_STATIC void oal_sdio_dev_deinit(struct oal_sdio *hi_sdio)
{
    ...
    oal_sdio_claim_host(hi_sdio);
    oal_sdio_writeb(func, 0, hi_sdio->reg_addr.func1_addr.int_en, &ret);
...
OAL_STATIC int32_t oal_sdio_probe(struct sdio_func *func, const struct sdio_device_id *ids)
{
    ...
    oal_print_mpxx_log(MPXX_LOG_DBG, "sdio vendor id:%x", ids->vendor);
    oal_print_mpxx_log(MPXX_LOG_DBG, "sdio device id:%x", ids->device);
    ...
    ret = sdio_master_ip_init(hi_sdio);
    if (ret != 0) {
        goto failed_sdio_ip_init;
    }
                                            ...
                                            void *buf, uint32_t size)
{
    ...
    return oal_sdio_rw_buf(hi_sdio, rw, hi_sdio->reg_addr.func1_addr.fifo, buf, size);
}
...
OAL_STATIC int32_t oal_sdio_transfer_scatt_t(struct oal_sdio *hi_sdio, int32_t rw,
                                           ...
                                           uint32_t rw_sz)
{
    ...
    ret = oal_sdio_transfer_scatt_t(hi_sdio, rw, addr, sg, sg_len, rw_sz);
                                      ...
                                      int32_t rw)
{
    ...
    ret = oal_sdio_transfer_scatt(hi_sdio, rw,
                                  hi_sdio->reg_addr.func1_addr.fifo, sg,
...
OAL_STATIC struct sdio_device_id const g_oal_sdio_ids[] = {
    ...
    { SDIO_DEVICE(MSDIO_VENDOR_ID_MPXX,    MSDIO_PRODUCT_ID_MP17C) },
    { SDIO_DEVICE(MSDIO_VENDOR_ID_MPXX,    MSDIO_PRODUCT_ID_MP12) },
    {},
};
...
void oal_sdio_exit_module(struct oal_sdio *hi_sdio)
{
    ...
    oal_sdio_free_buffer(hi_sdio);
    kfree(hi_sdio->scatt_info[SDIO_WRITE].sglist);
    hi_sdio->scatt_info[SDIO_WRITE].sglist = NULL;
    kfree(hi_sdio->scatt_info[SDIO_READ].sglist);
    hi_sdio->scatt_info[SDIO_READ].sglist = NULL;
    kfree(hi_sdio->sdio_extend);
    hi_sdio->sdio_extend = NULL;
    kfree(hi_sdio->rx_reserved_buff);
    hi_sdio->rx_reserved_buff = NULL;
    kfree(hi_sdio);
    g_mpxx_sdio = NULL;
#ifdef CONFIG_SDIO_DEBUG
...
void oal_sdio_tc_extend_001(void)
{
    ...
    ret = oal_sdio_memcpy_fromio(g_hi_sdio_debug->func, buf,
                                 g_hi_sdio_debug->reg_addr.func1_addr.extend_sdio_comm, HISDIO_FUNC1_EXTEND_REG_LEN);
...
OAL_STATIC int32_t oal_sdio_gpio_flowctrl_irq(hcc_bus *pst_hi_bus, int32_t l_irq)
{
    ...
    /* 获取流控GPIO管脚当前电平 */
    l_gpio_val = conn_get_gpio_level(SDIO_FLOW_CTRL);
    if (l_gpio_val == 1) {
        /* 高电平, 流控开启, 无法发送 */
