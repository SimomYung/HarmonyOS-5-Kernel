/**
 * Copyright (c) @CompanyNameMagicTag 2021-2023. All rights reserved.
 *
 * Description:Test code of the device pcie driver chip
 * Author: @CompanyNameTag
 */

#ifdef _PRE_PLAT_FEATURE_HI110X_PCIE
#define MPXX_LOG_MODULE_NAME "[PCIE_H]"
#define HISI_LOG_TAG           "[PCIE]"
#include <linux/module.h>
#include <linux/kernel.h>
#include <linux/device.h>
#include <linux/slab.h>
#include <linux/types.h>
#include <linux/err.h>
#include <linux/workqueue.h>
#include <linux/sched.h>
#include <linux/delay.h>
#include <linux/kthread.h>
#include "pcie_rc_port.h"
#include "pcie_host.h"
#include "oal_thread.h"
#include "oam_ext_if.h"
#include "pcie_linux.h"
#include "pcie_reg.h"
#include "oal_hcc_host_if.h"
#include "oal_kernel_file.h"
#include "plat_firmware.h"
#include "plat_pm_wlan.h"
#include "board.h"
#include "securec.h"
#include "plat_pm.h"

int32_t oal_pcie_chiptest_init_mp13(oal_pcie_res *pst_pci_res, int32_t device_id);
int32_t oal_pcie_chiptest_init_mp16(oal_pcie_res *pst_pci_res, int32_t device_id);
int32_t oal_pcie_chiptest_init_mp16c(oal_pcie_res *pst_pci_res, int32_t device_id);
int32_t oal_pcie_chiptest_init_mp17c(oal_pcie_res *pst_pci_res, int32_t device_id);
int32_t oal_pcie_chiptest_init_mp12(oal_pcie_res *pst_pci_res, int32_t device_id);

int32_t oal_pcie_ip_factory_test(hcc_bus *pst_bus, int32_t test_count)
{
    oal_pcie_linux_res *pst_pci_lres;

    pst_pci_lres = (oal_pcie_linux_res *)pst_bus->data;
    if (oal_warn_on(pst_pci_lres == NULL)) {
        return -OAL_ENODEV;
    }

    if (oal_warn_on(pst_pci_lres->pst_pci_res == NULL)) {
        return -OAL_ENODEV;
    }

    if (oal_warn_on(pst_pci_lres->pst_pci_res->chip_info.cb.pcie_ip_factory_test == NULL)) {
        return -OAL_EIO;
    }

    return pst_pci_lres->pst_pci_res->chip_info.cb.pcie_ip_factory_test(pst_bus, test_count);
}

/**
 * @brief 检查PCIE链路质量状态
 * 该函数用于检查PCIE链路的质量状态，确保链路正常工作(误码率统计等)。
 * 调用之前WiFi芯片必须已经正常打开, refer to pcie_chiptest_xx.c
 * @param pst_bus bus resource
 * @param h2d_ber ep rx误码率计数
 * @param d2h_ber rc rx误码率计数
 * @return int32_t
 *         - OAL_SUCC: 成功
 *         - 其他: 失败（具体错误码请参考错误码定义）
 * @note 该函数目前仅在特定平台上实现，其他平台默认返回失败。
 */
int32_t oal_pcie_get_link_quality_info(hcc_bus *pst_bus, uint32_t *h2d_ber, uint32_t *d2h_ber)
{
    int32_t ret;
    oal_pcie_linux_res *pst_pci_lres = NULL;

    if (pst_bus == NULL) {
        return -OAL_EINVAL;
    }

    pst_pci_lres = (oal_pcie_linux_res *)pst_bus->data;
    if (oal_warn_on(pst_pci_lres == NULL)) {
        return -OAL_ENODEV;
    }

    if (oal_warn_on(h2d_ber == NULL)) {
        return -OAL_ENODEV;
    }

    if (oal_warn_on(d2h_ber == NULL)) {
        return -OAL_ENODEV;
    }

    if (oal_warn_on(pst_pci_lres->pst_pci_res == NULL)) {
        return -OAL_ENODEV;
    }

    if (oal_warn_on(pst_pci_lres->pst_pci_res->chip_info.cb.pcie_get_link_quality_info == NULL)) {
        return -OAL_EIO;
    }
    *h2d_ber = 0;
    *d2h_ber = 0;
    /* 获取rc rx误码率计数, 先读取RC，RC直接总线读取不易失败 */
    ret = oal_pcie_get_rc_link_quality_info(pst_pci_lres, d2h_ber);
    if (ret != OAL_SUCC) {
        return ret;
    }
    /* 获取ep rx误码率计数 */
    return pst_pci_lres->pst_pci_res->chip_info.cb.pcie_get_link_quality_info(pst_pci_lres->pst_pci_res, h2d_ber);
}

int32_t oal_pcie_rc_slt_chip_transfer(hcc_bus *pst_bus, void *ddr_address,
                                      uint32_t data_size, int32_t direction)
{
    oal_pcie_linux_res *pst_pci_lres = NULL;
    if (pst_bus == NULL) {
        oal_print_mpxx_log(MPXX_LOG_ERR, "pst_bus is null");
        return -OAL_ENODEV;
    }

    pst_pci_lres = (oal_pcie_linux_res *)pst_bus->data;
    if (oal_warn_on(pst_pci_lres == NULL)) {
        oal_print_mpxx_log(MPXX_LOG_ERR, "pst_pci_lres is null");
        return -OAL_ENODEV;
    }

    if (direction == 1) { /* 1表示 Host to Device */
        return oal_pcie_copy_to_device_by_dword(pst_pci_lres->pst_pci_res, ddr_address,
                                                oal_pcie_slt_get_deivce_ram_cpuaddr(pst_pci_lres->pst_pci_res),
                                                data_size);
    } else if (direction == 2) { /* 2表示 Device to Host */
        return oal_pcie_copy_from_device_by_dword(pst_pci_lres->pst_pci_res, ddr_address,
                                                  oal_pcie_slt_get_deivce_ram_cpuaddr(pst_pci_lres->pst_pci_res),
                                                  data_size);
    } else {
        oal_print_mpxx_log(MPXX_LOG_ERR, "invaild direction:%d", direction);
        return -OAL_EINVAL;
    }
}

static pcie_chip_id_stru g_pcie_chiptest_id[] = {
    {PCIE_HISI_DEVICE_ID_MP13,     oal_pcie_chiptest_init_mp13},
    {PCIE_HISI_DEVICE_ID_MP15,     oal_pcie_chiptest_init_mp13},
    {PCIE_HISI_DEVICE_ID_MP16,     oal_pcie_chiptest_init_mp16},
    {PCIE_HISI_DEVICE_ID_MP16FPGA, oal_pcie_chiptest_init_mp16},
    {PCIE_HISI_DEVICE_ID_MP16C,     oal_pcie_chiptest_init_mp16c},
    {PCIE_HISI_DEVICE_ID_MP17CMPW,  oal_pcie_chiptest_init_mp16c},
    {PCIE_HISI_DEVICE_ID_MP17C,     oal_pcie_chiptest_init_mp17c},
    {PCIE_HISI_DEVICE_ID_MP12,      oal_pcie_chiptest_init_mp12}
};

int32_t oal_pcie_chiptest_init(oal_pcie_res *pst_pci_res, int32_t device_id)
{
    int32_t i;
    int32_t chip_num = (int32_t)oal_array_size(g_pcie_chiptest_id);
    for (i = 0; i < chip_num; i++) {
        if (g_pcie_chiptest_id[i].device_id == device_id) {
            if (g_pcie_chiptest_id[i].func != NULL) {
                return g_pcie_chiptest_id[i].func(pst_pci_res, device_id);
            } else {
                oal_print_mpxx_log(MPXX_LOG_INFO, "chiptest init 0x%x init null func", device_id);
                return OAL_SUCC;
            }
        }
    }
    oal_print_mpxx_log(MPXX_LOG_ERR, "not implement chiptest device_id=0x%x", device_id);
    return -OAL_ENODEV;
}

#endif
