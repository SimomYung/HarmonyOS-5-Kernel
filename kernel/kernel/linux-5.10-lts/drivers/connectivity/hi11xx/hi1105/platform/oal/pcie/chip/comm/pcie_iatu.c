...
{
    if (pst_pci_res->chip_info.cb.pcie_set_inbound_membar == NULL) {
        oal_print_mpxx_log(MPXX_LOG_INFO, "pcie_set_inbound_membar is null");
        return -OAL_ENODEV;
    }
    return pst_pci_res->chip_info.cb.pcie_set_inbound_membar(pst_pci_res);
}
