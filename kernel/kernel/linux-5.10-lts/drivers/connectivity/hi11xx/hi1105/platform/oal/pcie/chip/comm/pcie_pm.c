...
static pcie_chip_id_stru g_pcie_pm_chip_id[] = {
    ...
    {PCIE_HISI_DEVICE_ID_MP17CMPW, pcie_pm_chip_init_mp16c},
    {PCIE_HISI_DEVICE_ID_MP17C,    pcie_pm_chip_init_mp17c},
    {PCIE_HISI_DEVICE_ID_MP12,     pcie_pm_chip_init_mp12}
};
...
{
    g_pcie_auxclk_switch = value;
}

int32_t oal_pcie_device_auxclk_switch_get(void)
{
    return g_pcie_auxclk_switch;
}
...
{
    if (pst_pci_res->chip_info.cb.pm_cb.pcie_enable_aspm != NULL) {
        return pst_pci_res->chip_info.cb.pm_cb.pcie_enable_aspm(pst_pci_res);
    }
    return -OAL_ENODEV;
}
