...
#define DEV_VERSION_CPU_ADDR        0x00200174
#define DEV_BOOT_SHAREMEM_CPU_ADDR  0x0020019C /* 存放bootloader pcie share memory地址 */
...
int32_t oal_pcie_chip_info_init_mp12(oal_pcie_res *pst_pci_res, int32_t device_id)
{
    ...
    pst_pci_res->chip_info.addr_info.boot_sharemem = 0xFFFFFFFF; // 不使能boot share_mem
    /*
     * 不支持安全启动，不支持ete加载firmware
     */
    ...
    if (oal_pcie_get_device_mac_config(pst_pci_res) == 0) {
        /* 重构pcie mac atu配置 */
        oal_pcie_set_outbound_iatu_by_membar_v151(&pst_pci_res->st_iatu_bar);
    } else if (oal_pcie_get_device_mac_config(pst_pci_res) == 1) {
        /* 回退pcie mac atu配置 */
