...
// erroror：mp12 切高频暂未配置
int32_t oal_pcie_device_changeto_high_cpufreq_mp12(oal_pcie_res *pst_pci_res)
{
    int32_t ret;
    if (mpxx_is_asic()) {
        oal_udelay(30); /* 30us */
...
static int32_t oal_pcie_ip_factory_test_init(hcc_bus *pst_bus, oal_pcie_linux_res *pst_pci_lres)
{
    ...
    ret = oal_pcie_device_changeto_high_cpufreq_mp12(pst_pci_lres->pst_pci_res);
    if (ret) {
        return ret;
    }
    return OAL_SUCC;
...
int32_t oal_pcie_ip_factory_test_mp12(hcc_bus *pst_bus, int32_t test_count)
{
    int32_t i;
    int32_t ret;
...
{
    pst_pci_res->chip_info.cb.pcie_ip_factory_test = oal_pcie_ip_factory_test_mp12;
