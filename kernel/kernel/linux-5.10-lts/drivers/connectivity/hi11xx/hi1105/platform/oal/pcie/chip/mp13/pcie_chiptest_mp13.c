...
/* mp17c */
OAL_STATIC uintptr_t g_mp17c_asic_wmem_scan_array[][PCIE_MEM_MSG_SIZE] = {
    // 调试需注意itcm 最后一段为boot ram；
    { 0x002B8000, 0x002C7FFF }, /* itcm_ram */
    { 0x00403000, 0x0048FFFF }, /* dtcm, 前12k stack */
    { 0x08000000, 0x0808FFFF }, /* pkt mem */
    { 0x04040000, 0x04043FFF }, /* AON_share_mem */
};

OAL_STATIC uintptr_t g_mp17c_asic_bmem_scan_array[][PCIE_MEM_MSG_SIZE] = {
    { 0x01268000, 0x0132FFFF }, /* itcm */
    { 0x01400000, 0x01451FFF }, /* dtcm，前8k stack */
};

OAL_STATIC uintptr_t g_mp17c_asic_gmem_scan_array[][PCIE_MEM_MSG_SIZE] = {
    { 0x02270000, 0x022BFFFF }, /* sle_ram，前12k stack */
    { 0x02400000, 0x024C7FFF }, /* gf_itcm */
};
                                                      ...
                                                      uint32_t *pst_scan_nums)
{
    ...
    *pst_scan_nums = 0;
    if (get_mpxx_subchip_type() == BOARD_VERSION_MP15) {
        ...
        *pst_scan_nums = (uint32_t)sizeof(g_mp17c_asic_wmem_scan_array) /
                         (PCIE_MEM_MSG_SIZE * (uint32_t)sizeof(g_mp17c_asic_wmem_scan_array[0][0]));
        *pst_scan_base = g_mp17c_asic_wmem_scan_array;
    } else {
        if (pst_pci_res->revision == PCIE_REVISION_4_70A) {
...
OAL_STATIC int32_t oal_pcie_device_dereset_bcpu(oal_pcie_res *pst_pci_res)
{
    ...
    if (get_mpxx_subchip_type() == BOARD_VERSION_MP17C) {
        oal_print_mpxx_log(MPXX_LOG_INFO, "mp17c bcpu not need dereset");
        return OAL_SUCC;
    }
...
OAL_STATIC int32_t oal_pcie_device_dereset_gcpu(oal_pcie_res *pst_pci_res)
{
    ...
    void *pst_glb_ctrl = NULL; /* 0x50000000 */
    if (get_mpxx_subchip_type() == BOARD_VERSION_MP17C) {
        oal_print_mpxx_log(MPXX_LOG_INFO, "mp17c gcpu not need dereset");
        return OAL_SUCC;
    }
                                                      ...
                                                      uint32_t *pst_scan_nums)
{
    ...
    *pst_scan_nums = 0;
    if (get_mpxx_subchip_type() == BOARD_VERSION_MP15) {
        ...
        *pst_scan_nums = (uint32_t)sizeof(g_mp17c_asic_bmem_scan_array) /
                         (PCIE_MEM_MSG_SIZE * (uint32_t)sizeof(g_mp17c_asic_bmem_scan_array[0][0]));
        *pst_scan_base = g_mp17c_asic_bmem_scan_array;
    } else {
        if (pst_pci_res->revision == PCIE_REVISION_4_70A) {
                                                      ...
                                                      uint32_t *pst_scan_nums)
{
    if (get_mpxx_subchip_type() == BOARD_VERSION_MP16) {
        ...
        *pst_scan_nums = (uint32_t)sizeof(g_mp17c_asic_gmem_scan_array) /
                         (PCIE_MEM_MSG_SIZE * (uint32_t)sizeof(g_mp17c_asic_gmem_scan_array[0][0]));
        *pst_scan_base = g_mp17c_asic_gmem_scan_array;
    } else {
        if (pst_pci_res->revision == PCIE_REVISION_4_70A) {
            *pst_scan_nums = (uint32_t)sizeof(g_mpw2_bmem_scan_array) /
                             (PCIE_MEM_MSG_SIZE * (uint32_t)sizeof(g_mpw2_bmem_scan_array[0][0]));
...
int32_t oal_pcie_ip_l1pm_check(oal_pcie_linux_res *pst_pci_lres)
{
    ...
    if (g_ft_pcie_aspm_check_bypass) {
        ...
        return -OAL_ENODEV;
#else
        uint32_t value = show_link_state(g_pcie_rc_idx);
        if (value == 0xC000) {
            return OAL_SUCC;
        } else {
...
int32_t oal_pcie_chiptest_init_mp13(oal_pcie_res *pst_pci_res, int32_t device_id)
{
    ...
    pst_pci_res->chip_info.cb.pcie_get_link_quality_info = NULL;
    return OAL_SUCC;
}
