...
#ifdef _PRE_DUAL_CHIP
OAL_STATIC oal_pcie_region g_pcie_asic_chip2_regions[] = {
    {   .pci_start = 0x00000000,
        .pci_end = 0x000BFFFF,
        .cpu_start = 0x00000000,
        .cpu_end = 0x000BFFFF,
                                          ...
                                          oal_pcie_region **region_base, uint32_t *region_num)
{
    ...
    if (mpxx_is_asic()) {
        /* mp15 Asic */
#ifdef _PRE_DUAL_CHIP
        *region_num = (pst_pci_lres->pst_bus->dev_id == 0) ? (uint32_t)oal_array_size(g_pcie_asic_regions) \
                                                           : (uint32_t)oal_array_size(g_pcie_asic_chip2_regions);
        *region_base = (pst_pci_lres->pst_bus->dev_id == 0) ? (&g_pcie_asic_regions[0]) \
                                                            : (&g_pcie_asic_chip2_regions[0]);
#else
