...
OAL_STATIC int32_t oal_pcie_edma_power_action(hcc_bus *pst_bus, hcc_bus_power_action_type action)
{
    int32_t ret;
    oal_pcie_linux_res *pst_pci_lres = (oal_pcie_linux_res *)pst_bus->data;
    int32_t rc_idx = g_pcie_rc_idx;

    if (oal_warn_on(pst_pci_lres == NULL)) {
        return -OAL_ENODEV;
    }
#ifdef _PRE_DUAL_CHIP
    rc_idx = oal_pcie_get_pcie_rc_idx(pst_pci_lres->pst_pcie_dev);
#endif
    ...
    if (action == HCC_BUS_POWER_UP) {
        ...
        ret = oal_pcie_pm_control(pst_pci_lres->pst_pcie_dev, rc_idx, PCIE_POWER_ON);
        ...
        oal_pcie_power_notifiy_register(pst_pci_lres->pst_pcie_dev, rc_idx, NULL, NULL, NULL);
        if (ret) {
...
OAL_STATIC int32_t oal_pcie_edma_suspend_handler(void *data)
{
    ...
    oal_pcie_power_notifiy_register(pst_pci_lres->pst_pcie_dev, rc_idx,
                                    oal_pcie_edma_wake_powerup_handler, NULL, NULL);
    return 0;
}
...
int32_t oal_pcie_edma_suspend(oal_pci_dev_stru *pst_pci_dev, oal_pm_message_t state)
{
...
#ifdef _PRE_DUAL_CHIP
    rc_idx = oal_pcie_get_pcie_rc_idx(pst_pci_dev);
#endif
    ...
    oal_pcie_power_notifiy_register(pst_pci_lres->pst_pcie_dev, rc_idx, NULL,
                                    oal_pcie_edma_suspend_handler, (void *)pst_pci_lres);
    return 0;
}
