...
/* ete intx statistic */
OAL_STATIC uint32_t g_pcie_ete_intx_total_count = 0;
OAL_STATIC unsigned long g_pcie_ete_intx_last_jiff = 0;
...
/*
 * 由复现log分析，中断风暴时每处理1000次中断，耗时均在30ms内
 * 假设最极端场景：按照GEN3速率，折算为987.165MB/s,每次中断处理64Byte数据，每秒产生15387个中断
 * 即每百毫秒处理中断约1538个，故此处设置门限为 1000次/200ms
 */
#define INTX_LIMIT_TIME 200
OAL_STATIC void oal_pcie_ete_intx_count_cal(void)
{
    int32_t i;

    g_pcie_ete_intx_total_count++;

    if (g_pcie_ete_intx_last_jiff == 0) {
        g_pcie_ete_intx_last_jiff = jiffies + msecs_to_jiffies(INTX_LIMIT_TIME);
        pci_print_log(PCI_LOG_INFO, "ete_intx_total_count = %u, ete_intx_last_jiffies = %lu, curr_jiff = %lu",
                      g_pcie_ete_intx_total_count, g_pcie_ete_intx_last_jiff, jiffies);
        return;
    }

    if ((g_pcie_ete_intx_total_count % 1000) == 0) {            // 每1000次中断计算一次时间
        // 此处调用内核接口，可有效规避jiffies整数回绕问题
        if (oal_time_before(jiffies, g_pcie_ete_intx_last_jiff)) {
            if (oal_print_rate_limit(PRINT_RATE_SECOND * 5) == 0) {     // 防止刷屏，每5s打印一次
                g_pcie_ete_intx_last_jiff = jiffies + msecs_to_jiffies(INTX_LIMIT_TIME);
                return;
            }
            pci_print_log(PCI_LOG_INFO, "ete_intx_total_count = %u, last_jiffies = %lu, curr_jiff = %lu",
                          g_pcie_ete_intx_total_count, g_pcie_ete_intx_last_jiff, jiffies);
            for (i = 0; i < PCIE_HOST_CTL_INTR_BUTT; i++) {
                pci_print_log(PCI_LOG_INFO, "host_intx_count[%u] =  %u", i, g_pcie_ete_host_intx_count[i]);
            }
            for (i = 0; i < ETE_TX_CHANN_MAX_NUMS; i++) {
                pci_print_log(PCI_LOG_INFO, "host_tx[%u]_done_count = %u", i, g_pcie_ete_trans_tx_done_intx_count[i]);
                pci_print_log(PCI_LOG_INFO, "host_tx[%u]_err_count = %u", i, g_pcie_ete_trans_tx_err_intx_count[i]);
                pci_print_log(PCI_LOG_INFO, "host_tx[%u]_dre_count = %u", i, g_pcie_ete_dre_tx_intx_count[i]);
            }
            for (i = 0; i < ETE_RX_CHANN_MAX_NUMS; i++) {
                pci_print_log(PCI_LOG_INFO, "host_rx[%u]_done_count =  %u", i, g_pcie_ete_trans_rx_done_intx_count[i]);
                pci_print_log(PCI_LOG_INFO, "host_rx[%u]_err_count =  %u", i, g_pcie_ete_trans_rx_err_intx_count[i]);
                pci_print_log(PCI_LOG_INFO, "host_rx[%u]_dre_count =  %u", i, g_pcie_ete_dre_rx_intx_count[i]);
            }
        }
        g_pcie_ete_intx_last_jiff = jiffies + msecs_to_jiffies(INTX_LIMIT_TIME);
    }
...
int32_t oal_pcie_transfer_ete_done(oal_pcie_res *pst_pci_res)
{
    ...
    hreg_get_val(intr_status, pst_pci_res->ete_info.reg.host_intr_sts_addr);
    hreg_get_val(pcie_ctl_ete_intr_sts, pst_pci_res->ete_info.reg.ete_intr_sts_addr);
    hreg_get_val(dr_empty_sts, pst_pci_res->ete_info.reg.ete_dr_empty_sts_addr);

    /* 先清中断 */
    /* host2device_tx_intr host不会单独上报host，其他中断响应时不清楚，等待device响应 */
    intr_status.as_dword = intr_status.as_dword & (~(pst_pci_res->ete_info.int_event_ignore_mask));

    if (intr_status.as_dword != 0) {
        hreg_set_val(intr_status, pst_pci_res->ete_info.reg.host_intr_clr_addr);
    }
    if (pcie_ctl_ete_intr_sts.as_dword != 0) {
        hreg_set_val(pcie_ctl_ete_intr_sts, pst_pci_res->ete_info.reg.ete_intr_clr_addr);
    }
    if (dr_empty_sts.as_dword != 0) {
        hreg_set_val(dr_empty_sts, pst_pci_res->ete_info.reg.ete_dr_empty_clr_addr);
    }

    oal_pcie_ete_intx_count_cal();
    ...
    // 判断最后一个PCIE MEM读操作
    if (oal_unlikely(dr_empty_sts.as_dword == 0xFFFFFFFF) && (oal_pcie_check_link_state(pst_pci_res) == OAL_FALSE)) {
        return -OAL_ENODEV;
    }
...
void oal_ete_rx_res_clean(oal_pcie_res *pst_pci_res)
{
    ...
    for (i = 0; i < ETE_RX_CHANN_MAX_NUMS; i++) {
        ...
        forever_loop() {
            /* 释放RX补充队列 */
            oal_print_mpxx_log(MPXX_LOG_DBG, "prepare free rxq[%d] len=%d",
                               i,
...
int32_t pcie_ete_txbuf_freecount(pcie_ete_trans_txbuf *tx_buff)
{
    ...
    if (tx_buff->buff_wr_flag == tx_buff->buff_rd_flag) {
        free_cnt = (int32_t)(tx_buff->buff_items - (tx_buff->buff_wr - tx_buff->buff_rd));
    } else {
        free_cnt = (int32_t)(tx_buff->buff_rd - tx_buff->buff_wr);
    }
...
int32_t oal_ete_tx_transfer_process(oal_pcie_res *pst_pci_res, uint32_t chan_idx)
{
    ...
    trans_cnt = pcie_ete_update_ring_tx_donecnt(&pst_h2d_res->ring, (uintptr_t)pst_pci_res->pst_ete_base, chan_idx);
    if (oal_likely(trans_cnt != PCIE_ETE_INVALID_RD_VALUE)) {
        ...
        forever_loop() {
            ...
            if (oal_unlikely(pst_netbuf == NULL)) {
                ...
                oal_print_mpxx_log(MPXX_LOG_ERR, "ete_tx_netbuf_underflow[%u] trans_cnt=%u, trans_cnt_t=%u, \
                    netbuf_cnt=%u", chan_idx, trans_cnt + 1, trans_cnt_t, netbuf_cnt);
...
#ifdef _PRE_PLAT_FEATURE_MULTI_HCC
        hcc_tx_netbuf_free(pst_netbuf, hbus_to_hcc(pst_pci_lres->pst_bus));
#else
...
#ifdef _PRE_PLAT_FEATURE_MULTI_HCC
OAL_STATIC OAL_INLINE int32_t oal_ete_write_txbuff_by_send_buff(oal_pcie_linux_res *pst_pci_lres,
                                                             ...
                                                             oal_pci_dev_stru *pst_pci_dev)
{
    ...
    oal_netbuf_stru *pst_netbuf = NULL;
#ifdef _PRE_PLAT_FEATURE_MULTI_HCC
    oal_pcie_linux_res *pst_lres = (oal_pcie_linux_res *)oal_pci_get_drvdata(pst_pci_dev);
#endif
    uint32_t step = oal_ete_get_netbuf_step();
    ...
    for (i = 0; i < (int32_t)send_cnt; i++) {
        ...
        if (oal_unlikely(pst_pci_res->link_state < PCI_WLAN_LINK_WORK_UP)) {
            oal_print_mpxx_log(MPXX_LOG_ERR, "oal_pcie_send_netbuf_list loop invaild, link_state:%s",
                               oal_pcie_get_link_state_str(pst_pci_res->link_state));
#ifdef _PRE_PLAT_FEATURE_MULTI_HCC
            hcc_tx_netbuf_free(pst_netbuf, hbus_to_hcc(pst_lres->pst_bus));
#else
            hcc_tx_netbuf_free(pst_netbuf);
#endif
            return total_cnt;
...
#ifdef _PRE_PLAT_FEATURE_MULTI_HCC
            ret = oal_ete_write_txbuff_by_send_buff(pst_lres, pst_h2d_res, pst_netbuf, &pci_dma_addr, &dst_addr, len);
#else
...
int32_t pcie_wlan_func_register(pcie_wlan_callback_group_stru *pst_func)
{
    ...
    g_pcie_ete_intx_callback[MAC_COMMON_INTR_MASK] = pst_func->pcie_mac_common_isr_callback;
...
OAL_STATIC void oal_ete_int_mask_init(oal_pcie_res *pst_pci_res)
{
    ...
    /* dma_trans，须通过device2host_rx_int完成消息交互，不能mask; 非dma_trans，可mask */
    if (pst_pci_res->chip_info.boot_cap.bits.dma_trans == OAL_FALSE) {
                                ...
                                void **cpu_addr, uint64_t *dst_addr, dma_addr_t *iova)
{
    ...
    if (cpu_addr == NULL || pst_pci_res == NULL) {
        oal_print_mpxx_log(MPXX_LOG_ERR, "cpu_addr or pci_res is NULL");
        return -OAL_EINVAL;
    }
    pst_pci_dev = pcie_res_to_dev(pst_pci_res);
    if (pst_pci_dev == NULL) {
...
int32_t oal_ete_chan_init(oal_pcie_res *pst_pci_res)
{
    ...
    oal_ete_intr_register(HOST2DEVICE_TX_INTR_MASK, oal_ete_host2device_tx_intr_cb);
    ...
    if (oal_unlikely(pst_pci_res->link_state >= PCI_WLAN_LINK_RES_UP)) {
        if (conn_get_gpio_level(HOST_WKUP_W) == GPIO_LEVEL_HIGH) {
            ...
            if (print_flag & (HCC_PRINT_TRANS_FLAG_DEVICE_REGS | HCC_PRINT_TRANS_FLAG_DEVICE_STAT)) {
                ...
                oal_pcie_send_message_to_dev(pst_pci_res, PCIE_H2D_MESSAGE_PRINT_TRANS_REG);
            }
        }
    } else {
