...
#define WIFI_WAKEUP_COMPLETE_TIMEOUT 6000 // 6ms
...
OAL_STATIC uint32_t oal_pcie_ete_is_master_ip(oal_pci_dev_stru *pst_pci_dev)
{
    uint32_t is_master;
...
int32_t oal_pcie_ete_wake_powerup_handler(void *data)
{
    int32_t ret = 0;
    uint32_t is_master;
    ...
    is_master = oal_pcie_ete_is_master_ip(pst_pci_dev);
...
retry:
        ...
        ret = oal_pcie_wait_for_completion(&pst_wlan_pm->st_wifi_powerup_done, (uint32_t)(WIFI_POWERUP_DONE_TIMEOUT));
        if (oal_unlikely(ret == -OAL_ETIMEDOUT)) { // timeout
...
OAL_STATIC int32_t oal_pcie_ete_suspend_handler(void *data)
{
    ...
    oal_pcie_power_notifiy_register(pst_pci_dev, oal_pcie_get_pcie_rc_idx(pst_pci_dev),
...
OAL_STATIC int32_t oal_pcie_ete_wakeup_complete(hcc_bus *pst_bus)
{
    ...
    if (total_time > WIFI_WAKEUP_COMPLETE_TIMEOUT) {
        ...
        // device侧异常未切换auxclk时钟, 在此补救
        oal_pcie_device_auxclk_switch(pst_pci_res, AUXCLK_TCXO);
    }
...
int32_t oal_pcie_ete_suspend(oal_pci_dev_stru *pst_pci_dev, oal_pm_message_t state)
{
    ...
    if (g_pcie_pm_linkdown_switch == OAL_TRUE) { // 睡眠断链开关打开后，pcie此时已经断链
        return OAL_SUCC;
    }
                                                    ...
                                                    oal_pci_dev_stru *pst_pci_dev)
{
    ...
    if (g_pcie_pm_linkdown_switch == OAL_TRUE) {
        return OAL_SUCC; // 睡眠断链开关打开后, pcie此时未重新建链
    }
...
OAL_STATIC OAL_INLINE int32_t oal_ete_dual_pci_power_up(oal_pcie_linux_res *pst_pci_lres)
{
    ...
    oal_pcie_power_notifiy_register(pst_pci_lres->pst_pcie_dev_dual, g_pcie_rc_idx_dual,
                                    oal_pcie_dual_power_on_callback, NULL, NULL);
    ret = oal_pcie_pm_control(pst_pci_lres->pst_pcie_dev_dual, g_pcie_rc_idx_dual, PCIE_POWER_ON);
    if (ret == OAL_SUCC) {
...
OAL_STATIC OAL_INLINE int32_t oal_ete_bus_power_up(oal_pcie_linux_res *pst_pci_lres, hcc_bus *pst_bus)
{
    ...
    ret = oal_pcie_pm_control(pst_pci_lres->pst_pcie_dev,
                              oal_pcie_get_pcie_rc_idx(pst_pci_lres->pst_pcie_dev), PCIE_POWER_ON);
    ...
    oal_pcie_register_event(pst_pci_lres->pst_pcie_dev, pst_pci_lres);
...
OAL_STATIC OAL_INLINE void oal_ete_bus_power_sleep(oal_pcie_linux_res *pst_pci_lres, hcc_bus *pst_bus)
{
    /* should stop ete when wifi sleep */
    oal_pcie_host_ip_ete_exit(pst_pci_lres);
    pcie_bus_power_sleep_action(pst_bus);
}
...
OAL_STATIC int32_t oal_pcie_ete_power_action(hcc_bus *pst_bus, hcc_bus_power_action_type action)
{
    ...
    if (action == HCC_BUS_POWER_SLEEP) {
        oal_ete_bus_power_sleep(pst_pci_lres, pst_bus);
    }
...
OAL_STATIC int32_t oal_pcie_change_linkspeed(hcc_bus *pst_bus, uint32_t request_speed)
{
    ...
    if ((pst_pci_lres != NULL) && (pst_pci_lres->pst_pci_res != NULL) &&
        (pst_pci_lres->pst_pci_res->chip_info.cb.pcie_dyn_change_linkspeed != NULL)) {
...
OAL_STATIC hcc_bus_opt_ops g_pcie_ete_opt_ops = {
    ...
    .frw_download_pre = oal_firmware_msg_download_pre,
    .frw_download_post = oal_firmware_msg_download_post,
};
