...
{
    uint64_t total_time;
    static uint64_t max_trans_time = 0;
...
OAL_STATIC int32_t oal_pcie_debug_read_bits32(struct kobject *dev, struct kobj_attribute *attr,
                                              const char *buf, oal_pcie_res *pst_pcie_res)
{
    /* echo read_bits32 address pos bits dump_type > debug */
    int32_t ret;
    pci_addr_map addr_map;
    uint32_t cpu_address = 0xFFFFFFFF;
    uint32_t pos, bits, dump_type;
    uint32_t value;
    uintptr_t reg_addr = 0;
    pcs_reg_ops reg_ops;
    ktime_t  start_time;

    if (sscanf_s(buf, "0x%x %u %u %u", &cpu_address, &pos, &bits, &dump_type) != 0x4) {
        pci_print_log(PCI_LOG_ERR, "read_bits32 argument invalid, [%s]", buf);
        return -OAL_EINVAL;
    }

    if ((pst_pcie_res == NULL) || (pcie_res_to_dev(pst_pcie_res) == NULL)) {
        pci_print_log(PCI_LOG_ERR, "pst_pcie_res or pcie_dev is null");
        return -OAL_ENODEV;
    }

    ret = pcie_pcs_set_regs_ops(&reg_ops, dump_type);
    if (ret != OAL_SUCC) {
        oal_io_print("pcie_read_bits32 invalid dump_type:%u!\n", dump_type);
        return -OAL_EINVAL;
    }

    if (dump_type == PCIE_PCS_DUMP_BY_SSI) {
        reg_addr = (uintptr_t)cpu_address;
    } else if (dump_type == PCIE_PCS_DUMP_BY_MEMPORT || dump_type == PCIE_PCS_DUMP_BY_SOC) {
        ret = oal_pcie_inbound_ca_to_va(pst_pcie_res, cpu_address, &addr_map);
        if (ret != OAL_SUCC) {
            oal_io_print("pcie_read_bits32 0x%8x unmap, read failed!\n", cpu_address);
            return -OAL_EBUSY;
        }
        reg_addr = addr_map.va;
    }

    start_time = ktime_get();
    value = reg_ops.get_bits32(reg_addr, pos, bits);
    oal_pcie_trans_timedebug(start_time);
    oal_io_print("pcie_read_bits32 0x%8x read success!\n", cpu_address);
    oal_pcie_print_bits(&value, sizeof(uint32_t));
    return OAL_SUCC;
}

OAL_STATIC int32_t oal_pcie_debug_write_bits32(struct kobject *dev, struct kobj_attribute *attr,
                                               const char *buf, oal_pcie_res *pst_pcie_res)
{
    /* echo write_bits32 address pos bits dump_type > debug */
    int32_t ret;
    pci_addr_map addr_map;
    uint32_t cpu_address = 0xFFFFFFFF;
    uint32_t pos, bits, dump_type;
    ...
    if (sscanf_s(buf, "0x%x %u %u %u 0x%x", &cpu_address, &pos, &bits, &dump_type, &value) != 0x5) {
        pci_print_log(PCI_LOG_ERR, "write_bits32 argument invalid [%s]", buf);
        return -OAL_EINVAL;
    }
    ...
    if (dump_type == PCIE_PCS_DUMP_BY_SSI) {
        ...
        ret = oal_pcie_inbound_ca_to_va(pst_pcie_res, cpu_address, &addr_map);
        if (ret != OAL_SUCC) {
            oal_io_print("pcie_read_bits32 0x%08x unmap, read failed!\n", cpu_address);
            return -OAL_EBUSY;
        }
        reg_addr = addr_map.va;
    }
    ...
    start_time = ktime_get();
    old_value = reg_ops.get_bits32(reg_addr, pos, bits);
    reg_ops.set_bits32(reg_addr, pos, bits, value);
    oal_pcie_trans_timedebug(start_time);

    oal_io_print("pcie_write_bits32 change 0x%08x from 0x%08x to 0x%08x, callback_read = 0x%08x!\n",
                 cpu_address, old_value, value, reg_ops.get_bits32(reg_addr, pos, bits));
    oal_pcie_print_bits(&value, sizeof(uint32_t));
    return OAL_SUCC;
}
                                                ...
                                                const char *buf, oal_pcie_res *pst_pcie_res)
{
    ...
    uint32_t dev_id = 0;
    if ((sscanf_s(buf, "0x%x", &dev_id) != 1)) {
        pci_print_log(PCI_LOG_ERR, "input dev_id is null");
        return -OAL_EINVAL;
    }
    pci_print_log(PCI_LOG_INFO, "input dev_id[%d]", dev_id);
#endif
                                                     ...
                                                     const char *buf)
{
    ...
    oal_io_print("oal_pcie_testcase_turnoff_message::rc_idx is [%d]\n", rc_idx);
    pci_print_log(PCI_LOG_INFO, "turnoff_message host");
    kirin_pcie_power_notifiy_register(rc_idx, NULL, NULL, NULL);
    kirin_pcie_pm_control(0, rc_idx);
    return OAL_SUCC;
#endif
    /* 走到这里说明wakelock已经释放，WIFI已经深睡,通知RC/EP下电，发送TurnOff Message */
    /* 下电之前关闭 PCIE HOST 控制器 */
    pci_print_log(PCI_LOG_INFO, "turnoff_message host");
    kirin_pcie_power_notifiy_register(g_pcie_rc_idx, NULL, NULL, NULL);
    kirin_pcie_pm_control(0, g_pcie_rc_idx);
#endif
    return OAL_SUCC;
}
                                                  ...
                                                  const char *buf)
{
    ...
    kirin_pcie_power_notifiy_register(rc_idx, NULL, NULL, NULL);
    ret__ = kirin_pcie_pm_control(0, rc_idx);
    ...
    kirin_pcie_power_notifiy_register(g_pcie_rc_idx, NULL, NULL, NULL);
    ret__ = kirin_pcie_pm_control(0, g_pcie_rc_idx);
    board_host_wakeup_dev_set(0);
    if (ret__) {
        pci_print_log(PCI_LOG_ERR, "suspend test power on failed, ret=%d", ret__);
        declare_dft_trace_key_info("suspend test power on failed", OAL_DFT_TRACE_OTHER);
                                                 ...
                                                 const char *buf)
{
    ...
    kirin_pcie_power_notifiy_register(rc_idx, oal_pcie_resume_handler, NULL, NULL);
    ret__ = kirin_pcie_pm_control(1, rc_idx);
    ...
    /* 下电之前关闭 PCIE HOST 控制器 */
    pci_print_log(PCI_LOG_INFO, "resume_test host");
    kirin_pcie_power_notifiy_register(g_pcie_rc_idx, oal_pcie_resume_handler, NULL, NULL);
    ret__ = kirin_pcie_pm_control(1, g_pcie_rc_idx);
    oal_pcie_change_link_state(pst_pcie_res, PCI_WLAN_LINK_WORK_UP);
    if (ret__) {
        pci_print_log(PCI_LOG_ERR, "resume test power on failed, ret=%d", ret__);
        declare_dft_trace_key_info("resume test power on failed", OAL_DFT_TRACE_OTHER);
                                                     ...
                                                     const char *buf)
{
    ...
    ret = wlan_pm_open_dev(HCC_EP_WIFI_DEV, OAL_FALSE);
    if (ret != OAL_SUCC) {
                                             ...
                                             uint32_t length, uint32_t times, uint32_t burst_size)
{
    ...
    trans_us = oal_ktime_get_us(start_time);
    if (trans_us == 0) {
        trans_us = 1;
    }
...
OAL_STATIC pcie_sysfs_debug_info g_pci_debug[] = {
    ...
    { "read_bits32",              "address(hex)",   oal_pcie_debug_read_bits32 },
    { "write_bits32",             "address(hex) value(hex)", oal_pcie_debug_write_bits32 },
...
#ifdef _PRE_DUAL_CHIP
    { "wlan_power_on",            "dev_id(decimal)",        oal_pcie_debug_wlan_power_on },
#else
