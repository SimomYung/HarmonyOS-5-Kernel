...
OAL_STATIC int32_t oal_pcie_firmware_cmd_send_quit(oal_pcie_res *pst_pcie_res, uint8_t *buff, int32_t len)
{
    ...
    new = oal_readl((void *)addr_map.va);
    if (new == old) {
        oal_io_print("ch sel maybe not reset, read old 0x%4x new= 0x%4x\n", old, new);
        return -OAL_EFAIL;
    }
    pci_print_log(PCI_LOG_INFO, "pcie quit cmd,  change 0x%8x from 0x%4x to 0x%4x callback-read= 0x%4x\n",
                  DEV_PCIE_SDIO_SEL_REG, old, value, new);
...
#define PCIE_FIRMWARE_FILE_BURST_SIZE (4096)
OAL_STATIC void oal_pcie_firmware_io_trans_split(uintptr_t dst, uintptr_t src, uint32_t size)
{
    /* 每下载4KB回读一次，让出PCIE总线，防止写把其他CPU 的PCIE读操作饿死 */
    uint32_t copied = 0;
    uint32_t to_copy = 0;
    uint32_t read_t = 0;
    uint32_t len = size;
    while (copied < size) {
        to_copy = ((len - copied) < PCIE_FIRMWARE_FILE_BURST_SIZE) ? (len - copied) : PCIE_FIRMWARE_FILE_BURST_SIZE;
        oal_pcie_io_trans(dst + copied, src + copied, to_copy);
        /* 回读, 让出总线 */
        oal_pcie_io_trans((uintptr_t)&read_t, dst, sizeof(read_t));
...
OAL_STATIC int32_t oal_pcie_firmware_cmd_send_files(oal_pcie_res *pst_pcie_res, uint8_t *buff, int32_t len)
{
    ...
    if (g_files_flag == FILES_CMD_SEND) {
        ...
        oal_pcie_firmware_io_trans_split((uintptr_t)addr_map.va, (uintptr_t)buff, len);
                                                      ...
                                                      int32_t len, uint32_t timeout)
{
    ...
    pci_addr_map addr_map;
    int32_t ret;
    uint64_t version_address = (uint64_t)pst_pcie_res->chip_info.addr_info.boot_version;
    ...
    if (pci_dbg_condtion() || (mpxx_is_asic() == VERSION_FPGA)) {
        oal_io_print("pcie read version len %u, 0x%x!\n", len, (uint32_t)version_address);
