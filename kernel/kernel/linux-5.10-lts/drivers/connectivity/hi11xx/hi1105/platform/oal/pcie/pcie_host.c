...
OAL_STATIC oal_pcie_res *g_pci_res_tab[WLAN_CHIP_MAX_NUM_PER_BOARD] = {NULL, NULL};
OAL_STATIC oal_kobject *g_conn_syfs_pci_object_tab[WLAN_CHIP_MAX_NUM_PER_BOARD] = {NULL, NULL};
#endif
...
int32_t g_pcie_mac_cfg_bypass = 0;
oal_debug_module_param(g_pcie_mac_cfg_bypass, int, S_IRUGO | S_IWUSR);
...
{
    return oal_pcie_set_inbound_membar(pst_pci_res);
}
...
{
    uint8_t *buff = oal_netbuf_data(netbuf);
    if (g_pcie_dma_data_rx_check != 0) {
        // 规避方案打开
        return OAL_SUCC;
    }

    if ((*(volatile uint32_t*)buff) != PCIE_RX_TRANS_FLAG) {
    ...
    oal_print_hex_dump(buff, HCC_HDR_TOTAL_LEN, HEX_DUMP_GROUP_SIZE, "err_hdr ");
...
#define PCIE_RX_BUF_PENDING_WAIT_CNT 5000
                                              ...
                                              dma_addr_t dma_addr, int32_t len)
{
    ...
    * pcie burst写一般比cpu single读要快很多 */
    ...
    oam_warning_log0(0, OAM_SF_ANY, "pcie rx netbuf pending in ddr, try to fix succ!");
#endif
...
void oal_pcie_tx_netbuf_free(oal_pcie_res *pci_res, oal_netbuf_stru *netbuf)
{
    ...
    oal_pcie_linux_res *pst_pci_lres = (oal_pcie_linux_res *)oal_pci_get_drvdata(pcie_res_to_dev(pci_res));
#endif

    oal_pcie_unmap_tx_netbuf(pci_res, netbuf);

#ifdef _PRE_PLAT_FEATURE_MULTI_HCC
    hcc_tx_netbuf_free(netbuf, hbus_to_hcc(pst_pci_lres->pst_bus));
#else
...
int32_t oal_pcie_check_link_state_err_adapter(pci_addr_map addr_map)
{
    ...
    oam_error_log1(0, OAM_SF_ANY, "pcie_maybe_excpeption, auxclk_switch_en[%d]", oal_pcie_device_auxclk_switch_get());
...
int32_t oal_pcie_check_link_state(oal_pcie_res *pst_pci_res)
{
    ...
    if (share_mem_address == 0xFFFFFFFF) {
        ...
        do {
            ...
            if (conn_get_gpio_level(pci_data->pst_bus->bus_dev->wkup_host_io_type) == 0) {
...
int32_t oal_pcie_share_mem_res_map(oal_pcie_res *pst_pci_res)
{
    ...
    forever_loop() {
        ...
        /* share_mem_address 地址为0 或 0xFFFFFFFF 不做映射 继续重试 */
        if (share_mem_address != 0 && share_mem_address != 0xFFFFFFFF) {
...
int32_t oal_pcie_edma_ringbuf_init(oal_pcie_res *pst_pci_res)
{
    int32_t ret;
    int32_t i;
    pci_addr_map st_map;
...
int32_t oal_pcie_ringbuf_res_map(oal_pcie_res *pst_pci_res)
{
    ...
    pst_pci_res->st_rx_res.ringbuf_ctrl_dma_addr.va = pst_pci_res->st_ringbuf_map.va +
                                                      OAL_OFFSET_OF(pcie_ringbuf_res, st_d2h_buf);
    /* 初始化消息TX RX BUFF */
    ret = oal_pcie_edma_ringbuf_init(pst_pci_res);
    if (ret != OAL_SUCC) {
        pci_print_log(PCI_LOG_ERR, "oal_pcie_edma_ringbuf_init failed");
        return -OAL_ENOMEM;
    }
...
#ifdef _PRE_DUAL_CHIP
OAL_STATIC int32_t oal_pcie_sysfs_init_dual_mpxx(oal_pcie_res *pst_pcie_res)
{
    int32_t ret;
    oal_kobject *pst_root_object = NULL;
    int32_t dev_id = (int32_t)oal_pcie_get_probed_dev_num();
    char pci_obj_name[NUM_6_BYTES];
    pci_print_log(PCI_LOG_INFO, "dev_id is [%d]\n", dev_id);
    ...
    ret = snprintf_s(pci_obj_name, sizeof(pci_obj_name), sizeof(pci_obj_name) - 1, "pci%d", dev_id);
    if (ret < 0) {
        pci_print_log(PCI_LOG_ERR, "oal_pcie_sysfs_init mem buff overrun.\n");
        return -OAL_ENOMEM;
    }
    g_conn_syfs_pci_object_tab[dev_id] = kobject_create_and_add(pci_obj_name, pst_root_object);
    if (g_conn_syfs_pci_object_tab[dev_id] == NULL) {
        ret = -OAL_ENODEV;
        pci_print_log(PCI_LOG_ERR, "sysfs create kobject_create_and_add pci fail\n");
        return ret;
    }

    ret = oal_pcie_sysfs_group_create(g_conn_syfs_pci_object_tab[dev_id]);
    if (ret) {
        ret = -OAL_ENOMEM;
        pci_print_log(PCI_LOG_ERR, "sysfs create g_hpci_attribute_group group fail.ret=%d\n", ret);
        kobject_put(g_conn_syfs_pci_object_tab[dev_id]);
        g_conn_syfs_pci_object_tab[dev_id] = NULL;
        return ret;
    }
...
#ifdef _PRE_DUAL_CHIP
    return oal_pcie_sysfs_init_dual_mpxx(pst_pcie_res);
#endif
...
#ifdef _PRE_DUAL_CHIP
    uint32_t dev_id = oal_pcie_get_probed_dev_num();
    if (g_pci_res_tab[dev_id]) {
#else
    if (g_pci_res) {
#endif
...
{
#ifdef _PRE_DUAL_CHIP
    uint8_t id = oal_pcie_get_probed_dev_num();
    g_pci_res_tab[id] = NULL;
#else
    g_pci_res = NULL;
#endif
...
int32_t oal_pcie_config_l1ss_poweron_time(oal_pci_dev_stru *pst_pci_dev, uint32_t val)
{
    ...
    reg_new = (reg_old & 0xFFFFFF00) | val;
