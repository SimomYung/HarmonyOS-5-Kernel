/**
 * Copyright (c) @CompanyNameMagicTag 2021-2023. All rights reserved.
 *
 * Description:MPxx & 118x pcie pm interface file, used to unload the chip
 * Author: @CompanyNameTag
 */

#define MPXX_LOG_MODULE_NAME     "[PCIEL]"
#define MPXX_LOG_MODULE_NAME_VAR pciel_loglevel
#define HISI_LOG_TAG               "[PCIEL]"
#include "pcie_linux.h"
#include "pcie_pcs_host.h"
#ifdef _PRE_SMART_HOME_PANEL
#define INI_PCIE_DEBUG_MODE "pcie_debug_mode"
#endif

#if (_PRE_OS_VERSION_LINUX == _PRE_OS_VERSION)
#include <linux/module.h>
#include <linux/kernel.h>
#include <linux/device.h>
#include <linux/slab.h>
#include <linux/types.h>
#include <linux/err.h>
#include <linux/workqueue.h>
#include <linux/sched.h>
#include <linux/delay.h>
#include <linux/kthread.h>
#include <linux/pci.h>
#include "board.h"
#include "plat_pm.h"
#endif
#include "oal_util.h"
#include "oal_net.h"
#include "oal_ext_if.h"
#include "oal_ktime.h"
#include "hisi_ini.h"
#include "oam_ext_if.h"
#include "pcie_firmware.h"
#include "pcie_firmware_msg.h"
#include "oal_hcc_host_if.h"
#include "plat_exception_rst.h"
#include "ete/ete_host.h"
#include "external/lpcpu_feature.h"
#include "pcie_pcs_trace.h"
#include "pcie_chip.h"
#include "pcie_rc_port.h"
#include "factory_pcie.h"
#ifdef _PRE_DUAL_CHIP
#include "board_dual_mp15.h"
#endif
#ifdef _PRE_PLAT_FEATURE_HI110X_PCIE

#undef THIS_FILE_ID
#define THIS_FILE_ID OAM_FILE_ID_OAL_PCIE_LINUX_C

#define WIFI_POWERUP_DONE_TIMEOUT 2000
#define HOST_WAIT_DEV_DELAY_TIMEOUT 5000
#define WIFI_SLEEP_REQUEST_TIMEOUT 2000

#define INI_PCIE_DEBUG_MODE "pcie_debug_mode"

OAL_STATIC oal_pcie_linux_res *g_pcie_linux_res = NULL;

oal_completion g_probe_complete;    /* 初始化信号量 */
OAL_VOLATILE int32_t g_probe_ret; /* probe 返回值 */

int32_t g_pcie_enum_fail_reg_dump_flag = 0;

int32_t g_hipci_msi_enable = 0; /* 0 -intx 1-pci */
int32_t g_hipci_gen_select = PCIE_GEN2;
int32_t g_hipci_sync_flush_cache_enable = 0; // for device
int32_t g_hipci_sync_inv_cache_enable = 0;   // for cpu
int32_t g_ft_pcie_aspm_check_bypass = 0;
int32_t g_ft_pcie_gen_check_bypass = 0;
#if (_PRE_OS_VERSION_LINUX == _PRE_OS_VERSION)
oal_debug_module_param(g_hipci_msi_enable, int, S_IRUGO | S_IWUSR);
oal_debug_module_param(g_hipci_gen_select, int, S_IRUGO | S_IWUSR);
oal_debug_module_param(g_hipci_sync_flush_cache_enable, int, S_IRUGO | S_IWUSR);
oal_debug_module_param(g_hipci_sync_inv_cache_enable, int, S_IRUGO | S_IWUSR);
oal_debug_module_param(g_ft_pcie_aspm_check_bypass, int, S_IRUGO | S_IWUSR);
oal_debug_module_param(g_ft_pcie_gen_check_bypass, int, S_IRUGO | S_IWUSR);
#endif

int32_t g_pcie_rc_idx = 0;
oal_debug_module_param(g_pcie_rc_idx, int, S_IRUGO | S_IWUSR);
#ifdef _PRE_PLAT_FEATURE_HI110X_DUAL_PCIE
/* 双PCIE，EP PCIE1对应的KIRIN PCIE */
int32_t g_pcie_rc_idx_dual = 1;
oal_debug_module_param(g_pcie_rc_idx_dual, int, S_IRUGO | S_IWUSR);
#endif

int32_t g_pcie_print_once = 0;
oal_debug_module_param(g_pcie_print_once, int, S_IRUGO | S_IWUSR);

OAL_STATIC uint32_t g_pcie_intx_idle_bind_cpu = 0;
oal_debug_module_param(g_pcie_intx_idle_bind_cpu, uint, S_IRUGO | S_IWUSR);

OAL_STATIC uint32_t pcie_scatt_list_assemble = 0;
/* WCPU芯片唤醒阶段是否自动关闭L1降低单包延迟 */
int32_t g_pcie_auto_disable_aspm = 0;
oal_debug_module_param(g_pcie_auto_disable_aspm, int, S_IRUGO | S_IWUSR);

int32_t g_pcie_forbid_dev_access_host = 0;
oal_debug_module_param(g_pcie_forbid_dev_access_host, int, S_IRUGO | S_IWUSR);

int32_t g_pcie_pm_linkdown_switch = 0;
oal_debug_module_param(g_pcie_pm_linkdown_switch, int, S_IRUGO | S_IWUSR);

int32_t g_wlan_wakeup_delay_enable = 1;
oal_debug_module_param(g_wlan_wakeup_delay_enable, int, S_IRUGO | S_IWUSR);

OAL_STATIC int32_t g_pcie_shutdown_panic = 0;
#ifdef PLATFORM_DEBUG_ENABLE
module_param(g_pcie_shutdown_panic, int, S_IRUGO | S_IWUSR);
#endif
OAL_STATIC int32_t g_pcie_intx_time_cal_enable = OAL_FALSE;
oal_debug_module_param(g_pcie_intx_time_cal_enable, int, S_IRUGO | S_IWUSR);
OAL_STATIC uint64_t g_pcie_intx_max_time = 0;
oal_debug_module_param(g_pcie_intx_max_time, ullong, S_IRUGO | S_IWUSR);
OAL_STATIC uint64_t g_pcie_intx_time_print_limit = 10000;        // 默认门限为10000us
oal_debug_module_param(g_pcie_intx_time_print_limit, ullong, S_IRUGO | S_IWUSR);

#ifdef _PRE_DUAL_CHIP
OAL_STATIC oal_pcie_linux_res *g_pcie_linux_res_tab[WLAN_CHIP_MAX_NUM_PER_BOARD] = {NULL, NULL};
OAL_STATIC int32_t g_pcie_rc_idx_dev1 = 1;  /* 第二颗芯片的rc_idx */
OAL_STATIC uint32_t g_pcie_probed_dev_num = 0;  /* probe成功的pcie设备数量 */

OAL_STATIC void oal_pcie_bus_init_dual_mpxx(hcc_bus *pst_bus);
OAL_STATIC int32_t oal_pci_resource_init_dual_mpxx(void);
OAL_STATIC int32_t oal_pcie_enum_dual_mpxx(void);
OAL_STATIC int32_t oal_pcie_probe_dual_mpxx(oal_pci_dev_stru *pst_pci_dev, const oal_pci_device_id_stru *pst_id);
OAL_STATIC void oal_pcie_dual_mpxx_init(void);
#endif

/* Function Declare */
OAL_STATIC int32_t oal_pcie_probe(oal_pci_dev_stru *pst_pci_dev, const oal_pci_device_id_stru *pst_id);
OAL_STATIC hcc_bus *oal_pcie_bus_init(oal_pcie_linux_res *pst_pci_lres);
OAL_STATIC void oal_pcie_bus_exit(hcc_bus *pst_bus);
OAL_STATIC void oal_pcie_intx_bind_cpu(oal_pci_dev_stru *pci_dev, uint32_t cpu_num);
#ifdef _PRE_PLAT_FEATURE_HI110X_DUAL_PCIE
OAL_STATIC int32_t oal_pcie_dual_power_off_fail_callback(void *data);
OAL_STATIC int32_t wlan_dual_first_power_off_callback(void *data);
#endif

OAL_STATIC const oal_pci_device_id_stru g_pcie_id_tab[] = {
    { PCIE_HISI_VENDOR_ID,   PCIE_HISI_DEVICE_ID_MP13, PCI_ANY_ID, PCI_ANY_ID }, /* mp13 PCIE */
    { PCIE_HISI_VENDOR_ID,   PCIE_HISI_DEVICE_ID_MP15, PCI_ANY_ID, PCI_ANY_ID }, /* mp15 PCIE */
    { PCIE_HISI_VENDOR_ID,   PCIE_HISI_DEVICE_ID_MP16FPGA, PCI_ANY_ID, PCI_ANY_ID }, /* mp16 FPGA PCIE */
    { PCIE_HISI_VENDOR_ID_0, PCIE_HISI_DEVICE_ID_MP16, PCI_ANY_ID, PCI_ANY_ID }, /* mp16 PCIE */
    { PCIE_HISI_VENDOR_ID,   PCIE_HISI_DEVICE_ID_MP16, PCI_ANY_ID, PCI_ANY_ID }, /* mp16 tmp PCIE */
    { PCIE_HISI_VENDOR_ID_0, PCIE_HISI_DEVICE_ID_MP16C, PCI_ANY_ID, PCI_ANY_ID }, /* mp16c tmp PCIE */
    { PCIE_HISI_VENDOR_ID_0, PCIE_HISI_DEVICE_ID_GF61, PCI_ANY_ID, PCI_ANY_ID }, /* gf61 tmp PCIE */
    { PCIE_HISI_VENDOR_ID_0, PCIE_HISI_DEVICE_ID_MP17CMPW, PCI_ANY_ID, PCI_ANY_ID }, /* mp17c mpw tmp PCIE */
    { PCIE_HISI_VENDOR_ID_0, PCIE_HISI_DEVICE_ID_MP17C, PCI_ANY_ID, PCI_ANY_ID }, /* mp17c tmp PCIE */
    { PCIE_HISI_VENDOR_ID_MPXX, PCIE_HISI_DEVICE_ID_MP12, PCI_ANY_ID, PCI_ANY_ID }, /* mp12 tmp PCIE */
    { 0, }
};

uint32_t oal_get_pcie_scatt_list_assemble_flag(void)
{
    return pcie_scatt_list_assemble;
}

int32_t oal_get_pcie_forbid_dev_access_host_flag(void)
{
    return g_pcie_forbid_dev_access_host;
}

void oal_set_pcie_forbid_dev_access_host_flag(int32_t value)
{
    g_pcie_forbid_dev_access_host = value;
}

int32_t oal_pcie_set_loglevel(int32_t loglevel)
{
    int32_t ret = g_hipcie_loglevel;
    g_hipcie_loglevel = loglevel;
    return ret;
}

int32_t oal_pcie_set_mpxx_loglevel(int32_t loglevel)
{
    int32_t ret = MPXX_LOG_MODULE_NAME_VAR;
    MPXX_LOG_MODULE_NAME_VAR = loglevel;
    return ret;
}

void oal_pcie_rx_netbuf_hdr_init(oal_pci_dev_stru *hwdev, oal_netbuf_stru *pst_netbuf)
{
#if (_PRE_OS_VERSION_LINUX == _PRE_OS_VERSION)
    uint8_t *netbuf = oal_netbuf_data(pst_netbuf);

    if (g_pcie_dma_data_rx_hdr_init != 0) {
        memset_s((void *)netbuf, oal_netbuf_len(pst_netbuf), 0xff, oal_netbuf_len(pst_netbuf));
    }

    /* 申请rx_buf时：利用cpu，赋初值 */
    *(volatile uint32_t*)netbuf = PCIE_RX_TRANS_FLAG;

#endif
}

OAL_STATIC int32_t oal_pcie_enable_single_device(oal_pci_dev_stru *pst_pcie_dev, struct pci_saved_state **default_state)
{
    u16 old_cmd = 0;
    int32_t ret;

#if (LINUX_VERSION_CODE >= KERNEL_VERSION(3, 0, 0))

    if (oal_warn_on(*default_state == NULL)) {
        oal_io_print("pcie had't any saved state!\n");
        return -OAL_ENODEV;
    }

#if (LINUX_VERSION_CODE >= KERNEL_VERSION(3, 14, 0))
    /*
     * Updated with pci_load_and_free_saved_state to compatible
     * with kernel 3.14 or higher
     */
    pci_load_and_free_saved_state(pst_pcie_dev, default_state);

    /* Update default state */
    *default_state = pci_store_saved_state(pst_pcie_dev);
#else
    pci_load_saved_state(pst_pcie_dev, *default_state);
#endif /* (LINUX_VERSION_CODE >= KERNEL_VERSION(3, 14, 0)) */
#endif /* (LINUX_VERSION_CODE >= KERNEL_VERSION(3, 0, 0)) */

    pci_restore_state(pst_pcie_dev);

    ret = oal_pci_enable_device(pst_pcie_dev);
    if (ret) {
        oal_io_print("enable device failed!ret=%d\n", ret);
        pci_disable_device(pst_pcie_dev);
    } else {
        pci_set_master(pst_pcie_dev);
    }

    pci_read_config_word(pst_pcie_dev, PCI_COMMAND, &old_cmd);
    oal_io_print("PCI_COMMAND:0x%x\n", old_cmd);

    return ret;
}

OAL_STATIC int32_t oal_pcie_save_resource(oal_pci_dev_stru *pst_pcie_dev, struct pci_saved_state **default_state)
{
    int32_t ret = OAL_SUCC;

    pci_save_state(pst_pcie_dev);
    oal_io_print("save resource\n");
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(3, 0, 0))
    if (*default_state != NULL) {
        /* There is already exist pcie state */
        struct pci_saved_state *pst_state = *default_state;
        *default_state = NULL;
        kfree(pst_state);
        oal_io_print("default state already exist, free first\n");
    }

    *default_state = pci_store_saved_state(pst_pcie_dev);
    if (oal_unlikely(*default_state == NULL)) {
        oal_pci_disable_device(pst_pcie_dev);
        return -OAL_ENOMEM;
    }
#endif
    return ret;
}

OAL_STATIC int32_t oal_pcie_enable_device(oal_pcie_linux_res *pst_pci_lres)
{
    if (oal_warn_on(pst_pci_lres == NULL)) {
        return -OAL_ENOMEM;
    }

    if (oal_warn_on(pst_pci_lres->pst_pcie_dev == NULL)) {
        return -OAL_ENODEV;
    }
    return oal_pcie_enable_single_device(pst_pci_lres->pst_pcie_dev, &pst_pci_lres->default_state);
}

OAL_STATIC void oal_pcie_disable_device(oal_pcie_linux_res *pst_pci_lres)
{
    if (oal_warn_on(pst_pci_lres == NULL)) {
        return;
    }

    if (oal_warn_on(pst_pci_lres->pst_pcie_dev == NULL)) {
        return;
    }

    oal_pci_disable_device(pst_pci_lres->pst_pcie_dev);
}

int32_t oal_pcie_save_default_resource(oal_pcie_linux_res *pst_pci_lres)
{
    if (oal_warn_on(pst_pci_lres == NULL)) {
        return -OAL_EINVAL;
    }

    if (oal_warn_on(pst_pci_lres->pst_pcie_dev == NULL)) {
        return -OAL_ENODEV;
    }

    return oal_pcie_save_resource(pst_pci_lres->pst_pcie_dev, &pst_pci_lres->default_state);
}

#ifdef _PRE_PLAT_FEATURE_HI110X_DUAL_PCIE
int32_t oal_pcie_enable_dual_device(oal_pcie_linux_res *pst_pci_lres)
{
    if (oal_warn_on(pst_pci_lres == NULL)) {
        return -OAL_ENOMEM;
    }

    if (oal_warn_on(pst_pci_lres->pst_pcie_dev_dual == NULL)) {
        return -OAL_ENODEV;
    }

    return oal_pcie_enable_single_device(pst_pci_lres->pst_pcie_dev_dual, &pst_pci_lres->default_state_dual);
}

void oal_pcie_disable_dual_device(oal_pcie_linux_res *pst_pci_lres)
{
    if (oal_warn_on(pst_pci_lres == NULL)) {
        return;
    }

    if (oal_warn_on(pst_pci_lres->pst_pcie_dev_dual == NULL)) {
        return;
    }

    oal_pci_disable_device(pst_pci_lres->pst_pcie_dev_dual);
}

int32_t oal_pcie_save_default_dual_resource(oal_pcie_linux_res *pst_pci_lres)
{
    if (oal_warn_on(pst_pci_lres == NULL)) {
        return -OAL_EINVAL;
    }

    if (oal_warn_on(pst_pci_lres->pst_pcie_dev_dual == NULL)) {
        return -OAL_ENODEV;
    }

    return oal_pcie_save_resource(pst_pci_lres->pst_pcie_dev_dual, &pst_pci_lres->default_state_dual);
}
#endif

OAL_STATIC irqreturn_t oal_pcie_intx_edma_isr(int irq, void *dev_id)
{
    /*
     * 中断处理内容太多，目前无法下移，因为中断需要每次读空 ，而不在中断读的话，
     * 要先锁住中断，否则中断会狂报
     */
    oal_pcie_linux_res *pst_pci_lres = (oal_pcie_linux_res *)dev_id;
    if (oal_unlikely(oal_pcie_transfer_done(pst_pci_lres->pst_pci_res) < 0)) {
        pci_print_log(PCI_LOG_ERR, "froce to disable pcie intx");
        declare_dft_trace_key_info("transfer_done_fail", OAL_DFT_TRACE_EXCEP);
        oal_disable_pcie_irq(pst_pci_lres);

        /* DFR trigger */
        oal_pcie_change_link_state(pst_pci_lres->pst_pci_res, PCI_WLAN_LINK_DOWN);
        hcc_bus_exception_submit(pst_pci_lres->pst_bus, HCC_BUS_TRANS_FAIL);

        chr_exception_report(CHR_PLATFORM_EXCEPTION_EVENTID, CHR_SYSTEM_PLAT, CHR_LAYER_DRV,
                             CHR_PLT_DRV_EVENT_PCIE, CHR_PLAT_DRV_ERROR_INTX_ISR_PCIE_LINK_DOWN);
        /* 关闭低功耗 */
    }
    return IRQ_HANDLED;
}

irqreturn_t oal_pcie_intx_ete_isr(int irq, void *dev_id)
{
    /*
     * 中断处理内容太多，目前无法下移，因为中断需要每次读空 ，而不在中断读的话，
     * 要先锁住中断，否则中断会狂报
     */
    oal_pcie_linux_res *pst_pci_lres = (oal_pcie_linux_res *)dev_id;
    ktime_t start_time;
    unsigned long long total_time;

    if (g_pcie_intx_time_cal_enable == OAL_TRUE) {
        start_time = ktime_get();
    }

    if (oal_unlikely(oal_pcie_transfer_ete_done(pst_pci_lres->pst_pci_res) < 0)) {
        pci_print_log(PCI_LOG_ERR, "froce to disable pcie intx");
        declare_dft_trace_key_info("transfer_done_fail", OAL_DFT_TRACE_EXCEP);
        oal_disable_pcie_irq(pst_pci_lres);

        /* DFR trigger */
        oal_pcie_change_link_state(pst_pci_lres->pst_pci_res, PCI_WLAN_LINK_DOWN);
        hcc_bus_exception_submit(pst_pci_lres->pst_bus, HCC_BUS_TRANS_FAIL);

        chr_exception_report(CHR_PLATFORM_EXCEPTION_EVENTID, CHR_SYSTEM_PLAT, CHR_LAYER_DRV,
                             CHR_PLT_DRV_EVENT_PCIE, CHR_PLAT_DRV_ERROR_INTX_ISR_PCIE_LINK_DOWN);
        /* 关闭低功耗 */
    }
    /* 统计当前中断执行时长 */
    if (g_pcie_intx_time_cal_enable == OAL_TRUE) {
        total_time = oal_ktime_get_us(start_time);
        if (total_time > g_pcie_intx_max_time) {
            g_pcie_intx_max_time = total_time;
        }
        if (total_time > g_pcie_intx_time_print_limit) {
            pci_print_log(PCI_LOG_INFO, "intx_time_cali, cur_time=[%llu]us, max_time=[%llu]us",
                          total_time, g_pcie_intx_max_time);
        }
    }
    return IRQ_HANDLED;
}

OAL_STATIC void oal_pcie_intx_task(uintptr_t data)
{
    oal_pcie_linux_res *pst_pci_lres = (oal_pcie_linux_res *)data;
    if (oal_pcie_transfer_done(pst_pci_lres->pst_pci_res)) {
        /* 非0说明还需要调度 */
        oal_task_sched(&pst_pci_lres->st_rx_task);
    }
}

OAL_STATIC void oal_pcie_unregister_msi_by_index(oal_pcie_linux_res *pst_pci_lres, int32_t range)
{
    int32_t i, j;
    oal_pci_dev_stru *pst_pci_dev;
    pst_pci_dev = pst_pci_lres->pst_pcie_dev;

    j = (range > pst_pci_lres->st_msi.msi_num) ? pst_pci_lres->st_msi.msi_num : range;

    if (pst_pci_lres->st_msi.func == NULL) {
        return;
    }

    for (i = 0; i < j; i++) {
        oal_irq_handler_t msi_handler = pst_pci_lres->st_msi.func[i];
        if (msi_handler != NULL) {
            free_irq(pst_pci_dev->irq + i, (void *)pst_pci_lres->pst_pci_res);
        }
    }
}

OAL_STATIC void oal_pcie_unregister_msi(oal_pcie_linux_res *pst_pci_lres)
{
    oal_pcie_unregister_msi_by_index(pst_pci_lres, pst_pci_lres->st_msi.msi_num);
}

OAL_STATIC int32_t oal_pcie_register_msi(oal_pcie_linux_res *pst_pci_lres)
{
    int32_t i;
    int32_t ret;
    oal_pci_dev_stru *pst_pci_dev = NULL;

    if (pst_pci_lres->st_msi.msi_num <= 0) {
        return -OAL_ENODEV;
    }

    if (pst_pci_lres->st_msi.func == NULL) {
        return -OAL_ENODEV;
    }

    pst_pci_dev = pst_pci_lres->pst_pcie_dev;

    for (i = 0; i < pst_pci_lres->st_msi.msi_num; i++) {
        oal_irq_handler_t msi_handler = pst_pci_lres->st_msi.func[i];
        if (msi_handler != NULL) {
            pci_print_log(PCI_LOG_DBG, "try to request msi irq:%d", pst_pci_dev->irq + i);
            ret = request_irq(pst_pci_dev->irq + i, msi_handler, IRQF_SHARED,
                              "hisi_pci_msi", (void *)pst_pci_lres);
            if (ret) {
                oal_io_print("msi request irq failed, base irq:%u, msi index:%d, ret=%d\n", pst_pci_dev->irq, i, ret);
                goto failed_request_msi;
            }
        }
    }

    return OAL_SUCC;
failed_request_msi:
    oal_pcie_unregister_msi_by_index(pst_pci_lres, i);
    return ret;
}

OAL_STATIC int32_t oal_pcie_register_int(oal_pci_dev_stru *pst_pci_dev, oal_pcie_linux_res *pst_pci_lres)
{
    int32_t ret = -OAL_EFAIL;

    /* interrupt register */
#if (_PRE_OS_VERSION_LINUX == _PRE_OS_VERSION)
    oal_io_print("raw irq: %d\n", pst_pci_dev->irq);

    /* Read from DTS later */
    pst_pci_lres->st_msi.is_msi_support = g_hipci_msi_enable;

    if (pst_pci_lres->st_msi.is_msi_support) {
        /* Try to enable msi */
        ret = pci_enable_msi(pst_pci_dev);
        if (ret > 0) {
            /* ep support msi */
            oal_io_print("msi irq:%d, msi int nums:%d\n", pst_pci_dev->irq, ret);
            ret = oal_pcie_register_msi(pst_pci_lres);
            if (ret != OAL_SUCC) {
                goto failed_register_msi;
            } else {
                pst_pci_lres->irq_stat = 0;
            }
        } else {
            /* non msi drivers */
            oal_io_print("msi request failed, disable msi... ret=%d\n", ret);
            pst_pci_lres->st_msi.is_msi_support = 0;
        }
    }

    /* Try to register intx */
    if (!pst_pci_lres->st_msi.is_msi_support) {
        if (pst_pci_lres->pst_pci_res->chip_info.edma_support == OAL_TRUE) {
            ret = request_irq(pst_pci_dev->irq, oal_pcie_intx_edma_isr, IRQF_SHARED,
                              "hisi_edma_intx", (void *)pst_pci_lres);
        }
        if (pst_pci_lres->pst_pci_res->chip_info.ete_support == OAL_TRUE) {
            ret = request_irq(pst_pci_dev->irq, oal_pcie_intx_ete_isr, IRQF_SHARED,
                              "hisi_ete_intx", (void *)pst_pci_lres);
        }

        if (ret < 0) {
            oal_io_print("request intx failed, ret=%d\n", ret);
            goto failed_pci_intx_request;
        }

        pst_pci_lres->irq_stat = 0;
        /* 注册成功后立刻绑定至设置CPU */
        pci_print_log(PCI_LOG_INFO, "request bind_intx_to_cpu%u", g_pcie_intx_idle_bind_cpu);
        oal_pcie_intx_bind_cpu(pst_pci_dev, g_pcie_intx_idle_bind_cpu);
    }

#endif

    oal_io_print("request pcie intx irq %d succ\n", pst_pci_dev->irq);
    return OAL_SUCC;

    /* fail process */
failed_pci_intx_request:
    if (pst_pci_lres->st_msi.is_msi_support) {
        oal_pcie_unregister_msi(pst_pci_lres);
    } else {
        free_irq(pst_pci_dev->irq, pst_pci_lres);
    }
failed_register_msi:
    return ret;
}

#ifdef _PRE_PLAT_FEATURE_HI110X_DUAL_PCIE
OAL_STATIC int32_t oal_pcie_dual_pci_init(oal_pci_dev_stru *pst_pci_dev, oal_pcie_linux_res *pst_pci_lres)
{
    int32_t ret;
    if (pst_pci_lres->pst_pci_res == NULL) {
        oal_print_mpxx_log(MPXX_LOG_ERR, "pst_pci_res is null");
        return -OAL_ENODEV;
    }

    ret = oal_pci_enable_device(pst_pci_dev);
    if (ret) {
        oal_print_mpxx_log(MPXX_LOG_ERR, "enable dual pcie failed, ret=%d", ret);
        return ret;
    }

    ret = oal_pcie_dual_bar_init(pst_pci_lres->pst_pci_res, pst_pci_dev);
    if (ret != OAL_SUCC) {
        return ret;
    }
    ret = oal_pcie_bar1_init(&pst_pci_lres->pst_pci_res->st_iatu_dual_bar);
    if (ret != OAL_SUCC) {
        return ret;
    }
    ret = oal_pcie_save_default_dual_resource(pst_pci_lres);
    if (ret != OAL_SUCC) {
        oal_print_mpxx_log(MPXX_LOG_ERR, "save dual resource failed, ret=%d", ret);
        oal_pcie_bar1_exit(&pst_pci_lres->pst_pci_res->st_iatu_dual_bar);
        return ret;
    }
    return ret;
}

OAL_STATIC void oal_pcie_dual_pci_exit(oal_pcie_linux_res *pst_pci_lres)
{
    if (pst_pci_lres->pst_pci_res == NULL) {
        oal_print_mpxx_log(MPXX_LOG_ERR, "pst_pci_res is null");
        return;
    }
    oal_pcie_bar1_exit(&pst_pci_lres->pst_pci_res->st_iatu_dual_bar);
}

OAL_STATIC int32_t oal_pcie_dual_pci_probe(oal_pci_dev_stru *pst_pci_dev, const oal_pci_device_id_stru *pst_id)
{
    int32_t ret;
    uint32_t dword = 0;
    if (g_dual_pci_support == OAL_FALSE) {
        if (g_pcie_linux_res) {
            oal_print_mpxx_log(MPXX_LOG_ERR, "pcie device init failed, already init!\n");
            return -OAL_EBUSY;
        }
    } else {
        /* dual pcie, pcie0&pcie1 init flow */
        ret = oal_pci_read_config_dword(pst_pci_dev, PCI_EXP_DEVICE2_DUAL_FLAG, &dword);
        if (ret) {
            oal_print_mpxx_log(MPXX_LOG_ERR, "read dual flag failed, ret=%d", ret);
            return ret;
        }

        oal_print_mpxx_log(MPXX_LOG_INFO, "dual pcie flag: 0x%x", dword);

        if ((uint16_t)dword == (uint16_t)PCIE_DUAL_PCI_MASTER_FLAG) {
            /* pcie0 */
            if (g_pcie_linux_res != NULL) {
                oal_print_mpxx_log(MPXX_LOG_ERR, "dual pcie must probe master ip first!");
                return -OAL_EBUSY;
            }
        }

        if ((uint16_t)dword == (uint16_t)PCIE_DUAL_PCI_SLAVE_FLAG) {
            /* pcie1 */
            if (g_pcie_linux_res == NULL) {
                oal_print_mpxx_log(MPXX_LOG_ERR, "dual pcie must probe master ip first!");
                return -OAL_EBUSY;
            } else {
                g_pcie_linux_res->pst_pcie_dev_dual = pst_pci_dev;
                pci_set_drvdata(pst_pci_dev, g_pcie_linux_res);
                /* dual  pcie probe init */
                ret = oal_pcie_dual_pci_init(pst_pci_dev, g_pcie_linux_res);
                if (ret != OAL_SUCC) {
                    return ret;
                }
            }
        }
    }

    return OAL_SUCC;
}
#else
OAL_STATIC int32_t oal_pcie_dual_pci_probe(oal_pci_dev_stru *pst_pci_dev, const oal_pci_device_id_stru *pst_id)
{
    if (g_pcie_linux_res) {
        oal_io_print("pcie device init failed, already init!\n");
        return -OAL_EBUSY;
    }
    return OAL_SUCC;
}
#endif

OAL_STATIC void oal_pcie_init_dma_ops(oal_pci_dev_stru *pst_pci_dev)
{
    const struct dma_map_ops *ops = get_dma_ops(&pst_pci_dev->dev);
    if (ops == NULL) {
        g_hipci_sync_flush_cache_enable = 1;
        g_hipci_sync_inv_cache_enable   = 1;
        oal_print_mpxx_log(MPXX_LOG_INFO, "pcie dma is direct");
        return;
    }

    if (ops->sync_single_for_cpu == NULL) {
        // smmu old driver
        g_hipci_sync_inv_cache_enable = 0;
        oal_pcie_set_dma_data_rx_check_flag(0); // inv func null
    } else {
        g_hipci_sync_inv_cache_enable = 1;
    }

    if (ops->sync_single_for_device == NULL) {
        // smmu old driver
        g_hipci_sync_flush_cache_enable = 0;
    } else {
        g_hipci_sync_flush_cache_enable = 1;
    }

    oal_print_mpxx_log(MPXX_LOG_INFO, "pci dma sync flush cache is %s, inv cache is %s",
                       g_hipci_sync_flush_cache_enable == 0 ? "disable" : "enable",
                       g_hipci_sync_inv_cache_enable == 0 ? "disable" : "enable");
}

OAL_STATIC int32_t oal_pcie_dma_cap_init(oal_pci_dev_stru *pst_pci_dev)
{
    int32_t ret;
    oal_print_mpxx_log(MPXX_LOG_INFO, "before set, dma mask:0x%llx coherent mask:0x%llx",
                       dma_get_mask(&pst_pci_dev->dev), pst_pci_dev->dev.coherent_dma_mask);
    // extern pcie dma 64 bits address limit set.
    ret = dma_set_mask_and_coherent(&pst_pci_dev->dev, DMA_BIT_MASK(64));
    if (ret != 0) {
        oal_print_mpxx_log(MPXX_LOG_ERR, "dma set mask failed=%d", ret);
        return ret;
    }
    oal_print_mpxx_log(MPXX_LOG_INFO, "after set, dma mask:0x%llx coherent mask:0x%llx",
                       dma_get_mask(&pst_pci_dev->dev), pst_pci_dev->dev.coherent_dma_mask);
    return OAL_SUCC;
}

OAL_STATIC int32_t oal_pcie_init_dma_attr(oal_pci_dev_stru *pst_pci_dev)
{
    /* cache ops init */
    oal_pcie_init_dma_ops(pst_pci_dev);
    /* dma nocache cap init */
    return oal_pcie_dma_cap_init(pst_pci_dev);
}

/* 探测到一个PCIE设备, probe 函数可能会触发多次 */
OAL_STATIC int32_t oal_pcie_probe(oal_pci_dev_stru *pst_pci_dev, const oal_pci_device_id_stru *pst_id)
{
    uint8_t reg8 = 0;
    int32_t ret = -OAL_EFAIL;
    unsigned short device_id;
    hcc_bus *pst_bus = NULL;
    oal_pcie_linux_res *pst_pci_lres = NULL;

    if (oal_any_null_ptr2(pst_pci_dev, pst_id)) {
        /* never touch here */
        oal_io_print("oal_pcie_probe failed, pst_pci_dev:%p, pst_id:%p\n", pst_pci_dev, pst_id);
        g_probe_ret = -OAL_EIO;
        oal_complete(&g_probe_complete);
        return -OAL_EIO;
    }

    device_id = oal_pci_get_dev_id(pst_pci_dev);

    ret = oal_pcie_init_dma_attr(pst_pci_dev);
    if (ret != OAL_SUCC) {
        g_probe_ret = -OAL_EINVAL;
        oal_complete(&g_probe_complete);
        return -OAL_EINVAL;
    }

    /* 设备ID 和 产品ID */
    oal_print_mpxx_log(MPXX_LOG_DBG,
        "[PCIe][%s]devfn:0x%x , vendor:0x%x , device:0x%x , subsystem_vendor:0x%x , subsystem_device:0x%x , class:0x%x "
        "\n",
        dev_name(&pst_pci_dev->dev), pst_pci_dev->devfn, pst_pci_dev->vendor, pst_pci_dev->device,
        pst_pci_dev->subsystem_vendor, pst_pci_dev->subsystem_device, pst_pci_dev->class);

    ret = oal_pcie_dual_pci_probe(pst_pci_dev, pst_id);
    if (ret != OAL_SUCC) {
        g_probe_ret = ret;
        oal_complete(&g_probe_complete);
        return ret;
    }

    ret = oal_pci_enable_device(pst_pci_dev);
    if (ret) {
        oal_io_print("pci: pci_enable_device error ret=%d\n", ret);
        g_probe_ret = ret;
        oal_complete(&g_probe_complete);
        return ret;
    }

    /* If slave pcie, end the probe */
    if (g_pcie_linux_res != NULL) {
        g_probe_ret = OAL_SUCC;
        oal_complete(&g_probe_complete);
        return OAL_SUCC;
    }

    /* alloc pcie resources */
    pst_pci_lres = (oal_pcie_linux_res *)oal_memalloc(sizeof(oal_pcie_linux_res));
    if (pst_pci_lres == NULL) {
        ret = -OAL_ENOMEM;
        oal_io_print("%s alloc res failed,size:%lu\n", __FUNCTION__, (unsigned long)sizeof(oal_pcie_linux_res));
        goto failed_res_alloc;
    }

    memset_s((void *)pst_pci_lres, sizeof(oal_pcie_linux_res), 0, sizeof(oal_pcie_linux_res));

    pst_pci_lres->pst_pcie_dev = pst_pci_dev;

    pci_set_drvdata(pst_pci_dev, pst_pci_lres);

    oal_init_completion(&pst_pci_lres->st_pcie_ready);
    oal_atomic_set(&pst_pci_lres->st_pcie_wakeup_flag, 0);

    oal_task_init(&pst_pci_lres->st_rx_task, oal_pcie_intx_task, (uintptr_t)pst_pci_lres);

    oal_spin_lock_init(&pst_pci_lres->st_lock);
    mutex_init(&pst_pci_lres->sr_lock);

    oal_wake_lock_init(&pst_pci_lres->st_sr_bugfix, "pcie_sr_bugfix");

    pst_pci_lres->version = (pst_pci_dev->vendor) | (pst_pci_dev->device << 16); /* device为版本号的高16位 */

    ret = oal_pci_read_config_byte(pst_pci_dev, PCI_REVISION_ID, &reg8);
    if (ret) {
        oal_io_print("pci: read revision id  failed, ret=%d\n", ret);
        ret = -OAL_ENODEV;
        goto failed_pci_host_init;
    }

    pst_pci_lres->revision = (uint32_t)reg8;

    pci_print_log(PCI_LOG_INFO, "wifi pcie version:0x%8x, revision:%d", pst_pci_lres->version, pst_pci_lres->revision);

    /* soft resource init */
    pst_pci_lres->pst_pci_res = oal_pcie_host_init((void *)pst_pci_dev, &pst_pci_lres->st_msi, pst_pci_lres->revision);
    if (pst_pci_lres->pst_pci_res == NULL) {
        ret = -OAL_ENODEV;
        oal_io_print("pci: oal_pcie_host_init failed\n");
        goto failed_pci_host_init;
    }

    pst_bus = oal_pcie_bus_init(pst_pci_lres);
    if (pst_bus == NULL) {
        ret = -OAL_EFAIL;
        goto failed_pci_bus_init;
    }

    /* 硬件设备资源初始化, 5610+MP13 FPGA 没有上下电接口 */
    ret = oal_pcie_dev_init(pst_pci_lres->pst_pci_res);
    if (ret != OAL_SUCC) {
        goto failed_pci_dev_init;
    }

    ret = oal_pcie_register_int(pst_pci_dev, pst_pci_lres);
    if (ret != OAL_SUCC) {
        goto failed_register_msi;
    }

    ret = oal_pcie_firmware_msg_init(pst_pci_lres->pst_pci_res);
    if (ret != OAL_SUCC) {
        goto failed_register_msi;
    }

    /* KIRIN_PCIE_LINKDOWN_RECOVERY */
    oal_pcie_register_event(pst_pci_dev, pst_pci_lres);

    ret = oal_pcie_chiptest_init(pst_pci_lres->pst_pci_res, device_id);
    if (ret != OAL_SUCC) {
        oal_io_print("pci: oal_pcie_chiptest_init failed, device_id = %u\n", (uint32_t)device_id);
    }

    g_pcie_linux_res = pst_pci_lres;
    g_probe_ret = ret;
    oal_complete(&g_probe_complete);

    return ret;

failed_register_msi:
    oal_pcie_dev_deinit(pst_pci_lres->pst_pci_res);

failed_pci_dev_init:
    oal_pcie_bus_exit(pst_bus);
failed_pci_bus_init:
    oal_pcie_host_exit(pst_pci_lres->pst_pci_res);
failed_pci_host_init:
    pci_set_drvdata(pst_pci_dev, NULL);
    oal_wake_lock_exit(&pst_pci_lres->st_sr_bugfix);
    oal_free(pst_pci_lres);
failed_res_alloc:
    oal_pci_disable_device(pst_pci_dev);
    g_probe_ret = ret;
    oal_complete(&g_probe_complete);
    return ret;
}

OAL_STATIC void oal_pcie_remove(oal_pci_dev_stru *pst_pci_dev)
{
    unsigned short device_id;
    oal_pcie_linux_res *pst_pci_lres = NULL;

    device_id = oal_pci_get_dev_id(pst_pci_dev);

    oal_io_print("pcie driver remove 0x%x, name:%s\n", device_id, dev_name(&pst_pci_dev->dev));

    pst_pci_lres = (oal_pcie_linux_res *)oal_pci_get_drvdata(pst_pci_dev);
    if (pst_pci_lres == NULL) {
        oal_io_print("oal_pcie_remove lres is null\n");
        return;
    }

#ifdef _PRE_DUAL_CHIP
    g_pcie_linux_res_tab[pst_pci_lres->pst_bus->dev_id] = NULL;
#else
    g_pcie_linux_res = NULL;
#endif

    oal_pcie_deregister_event(pst_pci_lres);

#if (_PRE_OS_VERSION_LINUX == _PRE_OS_VERSION)
    if (pst_pci_lres->st_msi.is_msi_support) {
        oal_pcie_unregister_msi(pst_pci_lres);
    } else {
        free_irq(pst_pci_dev->irq, pst_pci_lres);
    }

#endif

    mutex_destroy(&pst_pci_lres->sr_lock);

#ifdef _PRE_PLAT_FEATURE_HI110X_DUAL_PCIE
    oal_pcie_dual_pci_exit(pst_pci_lres);
#endif
    oal_wake_lock_exit(&pst_pci_lres->st_sr_bugfix);

    oal_task_kill(&pst_pci_lres->st_rx_task);

    oal_pcie_dev_deinit(pst_pci_lres->pst_pci_res);

    oal_pcie_bus_exit(pst_pci_lres->pst_bus);

    oal_pcie_host_exit(pst_pci_lres->pst_pci_res);

    pst_pci_lres->pst_pcie_dev = NULL;

    oal_free(pst_pci_lres);

    oal_pci_disable_device(pst_pci_dev);
}

int32_t oal_pcie_device_wakeup_handler(const void *data)
{
#ifdef _PRE_DUAL_CHIP
    board_host_wakeup_dev_set_by_id((uint32_t)(uintptr_t)(data), 1);
    pci_print_log(PCI_LOG_INFO, "pcie wakeup device [%d] control, pull up gpio", (uint32_t)(uintptr_t)(data));
#else
    oal_reference(data);
    /* 这里保证解复位EP控制器时efuse已经稳定 */
    board_host_wakeup_dev_set(1);
    pci_print_log(PCI_LOG_INFO, "pcie wakeup device control, pull up gpio");
#endif
    return 0;
}

uint32_t oal_pcie_is_master_ip(oal_pci_dev_stru *pst_pci_dev)
{
    oal_pcie_linux_res *pst_pci_lres = (oal_pcie_linux_res *)oal_pci_get_drvdata(pst_pci_dev);
    if (pst_pci_lres == NULL) {
        return OAL_EINVAL;
    }
    return ((pst_pci_dev == pst_pci_lres->pst_pcie_dev) ? OAL_TRUE : OAL_FALSE);
}

int32_t oal_pcie_get_pcie_rc_idx(oal_pci_dev_stru *pst_pci_dev)
{
#ifdef _PRE_DUAL_CHIP
    oal_pcie_linux_res *pst_pci_lres = (oal_pcie_linux_res *)oal_pci_get_drvdata(pst_pci_dev);
    if (pst_pci_lres == NULL) {
        return OAL_EINVAL;
    }
    return (pst_pci_lres->pst_bus->dev_id == HCC_EP_WIFI_DEV ? g_pcie_rc_idx : g_pcie_rc_idx_dev1);
#endif

#ifdef _PRE_PLAT_FEATURE_HI110X_DUAL_PCIE
    return (oal_pcie_is_master_ip(pst_pci_dev) == OAL_TRUE ? g_pcie_rc_idx : g_pcie_rc_idx_dual);
#else
    return g_pcie_rc_idx;
#endif
}


int32_t oal_pcie_sr_para_check(oal_pcie_linux_res *pst_pci_lres,
                               hcc_bus **pst_bus, struct hcc_handler **pst_hcc)
{
    if (pst_pci_lres == NULL) {
        oal_io_print("lres is null\n");
        return -OAL_ENODEV;
    }

    *pst_bus = pst_pci_lres->pst_bus;
    if (*pst_bus == NULL) {
        oal_io_print("sr pst_bus is null\n");
        return -OAL_EFAIL;
    }

    if (hbus_to_dev(*pst_bus) == NULL) {
        oal_io_print("sr,pcie is not work...\n");
        return -OAL_ENODEV;
    }

    *pst_hcc = hbus_to_hcc(*pst_bus);
    if (*pst_hcc == NULL) {
        pci_print_log(PCI_LOG_WARN, "pcie dev's hcc handler is null!");
        return -OAL_ENODEV;
    }

    if (*pst_bus != hdev_to_hbus(hbus_to_dev(*pst_bus))) {
        /* pcie非当前接口 */
        oal_print_mpxx_log(MPXX_LOG_INFO, "pcie is not current bus, return");
        return -OAL_ENODEV;
    }

    if (oal_atomic_read(&(*pst_hcc)->state) != HCC_ON) {
        pci_print_log(PCI_LOG_INFO, "wifi is closed");
        return -OAL_ENODEV;
    }

    return OAL_SUCC;
}

// 判断pcie suspend之前是否持锁
int32_t oal_pcie_wakelock_active(hcc_bus *pst_bus)
{
    struct wlan_pm_s *pst_wlan_pm = wlan_pm_get_drv();
    if (pst_wlan_pm != NULL) {
        if (oal_wakelock_active(&pst_wlan_pm->st_pm_wakelock)) {
            pci_print_log(PCI_LOG_INFO, "st_pm_wakelock active");
            return OAL_TRUE;
        }
    }

    if (hcc_bus_wakelock_active(pst_bus)) {
        pci_print_log(PCI_LOG_INFO, "st_bus_wakelock active");
        return OAL_TRUE;
    }

    return OAL_FALSE;
}

OAL_STATIC int32_t oal_pcie_suspend(oal_pci_dev_stru *pst_pci_dev, oal_pm_message_t state)
{
    int32_t ret;
    oal_pcie_linux_res *pst_pci_lres = (oal_pcie_linux_res *)oal_pci_get_drvdata(pst_pci_dev);
    if (pst_pci_lres == NULL) {
        oal_io_print("pcie_suspend lres is null,pci_dev_name=%s\n", dev_name(&pst_pci_dev->dev));
        return 0;
    }
    oal_io_print("oal_pcie_suspend, event[%d]\n", state.event);

    if (pst_pci_lres->pst_pci_res->chip_info.edma_support == OAL_TRUE) {
        ret = oal_pcie_edma_suspend(pst_pci_dev, state);
    } else if (pst_pci_lres->pst_pci_res->chip_info.ete_support == OAL_TRUE) {
        ret = oal_pcie_ete_suspend(pst_pci_dev, state);
    } else {
        ret = -OAL_ENODEV;
    }

    return ret;
}

OAL_STATIC int32_t oal_pcie_resume(oal_pci_dev_stru *pst_pci_dev)
{
    int32_t ret;
    oal_pcie_linux_res *pst_pci_lres = (oal_pcie_linux_res *)oal_pci_get_drvdata(pst_pci_dev);
    if (pst_pci_lres == NULL) {
        oal_io_print("pcie_suspend lres is null,pci_dev_name=%s\n", dev_name(&pst_pci_dev->dev));
        return 0;
    }
    oal_io_print("oal_pcie_resume\n");

    if (pst_pci_lres->pst_pci_res->chip_info.edma_support == OAL_TRUE) {
        ret = oal_pcie_edma_resume(pst_pci_dev);
    } else if (pst_pci_lres->pst_pci_res->chip_info.ete_support == OAL_TRUE) {
        ret = oal_pcie_ete_resume(pst_pci_dev);
    } else {
        ret = -OAL_ENODEV;
    }

    return ret;
}

#if (_PRE_OS_VERSION_LINUX == _PRE_OS_VERSION)
OAL_STATIC void oal_pcie_shutdown(oal_pci_dev_stru *pst_pcie_dev)
{
    declare_time_cost_stru(cost);

    oal_pcie_linux_res *pst_pci_lres = (oal_pcie_linux_res *)oal_pci_get_drvdata(pst_pcie_dev);

    pci_print_log(PCI_LOG_INFO, "oal_pcie_shutdown");

    g_chip_err_block = 0;
    g_hcc_exception_enable = 0;
    hcc_bus_set_shutdown_flag(OAL_TRUE);
    if (pst_pci_lres == NULL) {
        pci_print_log(PCI_LOG_INFO, "pst_pci_lres is null");
        return;
    }

    ssi_excetpion_dump_disable();
    /* system shutdown, should't write sdt file */
    oam_set_output_type_all_sys(OAM_OUTPUT_TYPE_CONSOLE);

    if (pst_pci_lres->pst_bus == NULL) {
        pci_print_log(PCI_LOG_INFO, "pcie bus is null");
        return;
    }
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(4, 14, 0))
    time_cost_var_start(cost) = 0;
#else
    time_cost_var_start(cost) = (ktime_t) { .tv64 = 0 };
#endif

    oal_wlan_gpio_intr_enable(hbus_to_dev(pst_pci_lres->pst_bus), OAL_FALSE);
    oal_disable_pcie_irq(pst_pci_lres);

    /* power off wifi */
    if (g_pcie_shutdown_panic) {
        oal_get_time_cost_start(cost);
    }

    if (in_interrupt() || irqs_disabled() || in_atomic()) {
        pci_print_log(PCI_LOG_INFO, "no call wlan_pm_close_by_shutdown interrupt");
    } else {
        wlan_pm_close_dev_by_shutdown(pst_pci_lres->pst_bus->dev_id);
    }

    if (g_pcie_shutdown_panic) {
        oal_get_time_cost_end(cost);
        oal_calc_time_cost_sub(cost);
        oal_print_mpxx_log(MPXX_LOG_INFO, "pcie shutdown cost %llu us", time_cost_var_sub(cost));
#ifdef PLATFORM_DEBUG_ENABLE
        /* debug shutdown process when after wifi shutdown */
        if (time_cost_var_sub(cost) > (uint64_t)(uint32_t)g_pcie_shutdown_panic) {
            oal_print_mpxx_log(MPXX_LOG_INFO, "pcie shutdown panic");
            BUG_ON(1);
        }
#endif
    }
}

OAL_STATIC oal_pci_driver_stru g_pcie_drv = {
    .name = "hi110x_pci",
    .id_table = g_pcie_id_tab,
    .probe = oal_pcie_probe,
    .remove = oal_pcie_remove,
    .suspend = oal_pcie_suspend,
    .resume = oal_pcie_resume,
    .shutdown = oal_pcie_shutdown,
};
#endif

int32_t oal_pcie_get_state(hcc_bus *pst_bus, uint32_t mask)
{
    oal_pcie_linux_res *pst_pci_lres;

    pst_pci_lres = (oal_pcie_linux_res *)pst_bus->data;
    oal_reference(pst_pci_lres);

    return OAL_TRUE;
}

void oal_enable_pcie_state(hcc_bus *pst_bus, uint32_t mask)
{
    oal_pcie_linux_res *pst_pci_lres;

    pst_pci_lres = (oal_pcie_linux_res *)pst_bus->data;
    oal_reference(pst_pci_lres);
}

void oal_disable_pcie_state(hcc_bus *pst_bus, uint32_t mask)
{
    oal_pcie_linux_res *pst_pci_lres;

    pst_pci_lres = (oal_pcie_linux_res *)pst_bus->data;
    oal_reference(pst_pci_lres);
}

int32_t oal_pcie_rx_netbuf(hcc_bus *pst_bus, oal_netbuf_head_stru *pst_head)
{
    oal_pcie_linux_res *pst_pci_lres;

    pst_pci_lres = (oal_pcie_linux_res *)pst_bus->data;
    oal_reference(pst_pci_lres);
    return OAL_SUCC;
}

int32_t oal_pcie_check_tx_param(hcc_bus *pst_bus, oal_netbuf_head_stru *pst_head,
                                hcc_netbuf_queue_type qtype)
{
    oal_pcie_linux_res *pst_pci_lres = NULL;
    struct hcc_handler *pst_hcc = NULL;

    pst_pci_lres = (oal_pcie_linux_res *)pst_bus->data;
    if (oal_warn_on(pst_pci_lres == NULL)) {
        oal_io_print("pcie tx netbuf failed, lres is null\n");
        hcc_tx_netbuf_list_free(pst_head, hbus_to_hcc(pst_bus));
        return -OAL_ENODEV;
    }
    pst_hcc = hbus_to_hcc(pst_bus);
    if (pst_hcc == NULL) {
        pci_print_log(PCI_LOG_WARN, "pcie dev's hcc handler is null!");
        hcc_tx_netbuf_list_free(pst_head, pst_hcc);
        return -OAL_ENODEV;
    }

    if (oal_unlikely(oal_atomic_read(&pst_hcc->state) != HCC_ON)) {
        /* drop netbuf list, wlan close or exception */
        oal_print_mpxx_log(MPXX_LOG_INFO, "drop pcie netbuflist %u", oal_netbuf_list_len(pst_head));
        hcc_tx_netbuf_list_free(pst_head, pst_hcc);
        return -OAL_EFAIL;
    }

    return OAL_SUCC;
}

OAL_STATIC int32_t oal_pcie_send_message2dev(oal_pcie_linux_res *pst_pci_lres, uint32_t message)
{
    hcc_bus *pst_bus;
    int32_t ret;
    pst_bus = pst_pci_lres->pst_bus;

    if (oal_warn_on(pst_bus == NULL)) {
        return -OAL_ENODEV;
    }

    pci_print_log(PCI_LOG_DBG, "send msg to device [0x%8x]\n", (uint32_t)message);

    hcc_bus_wake_lock(pst_bus);
    ret = oal_pcie_send_message_to_dev(pst_pci_lres->pst_pci_res, message);
    hcc_bus_wake_unlock(pst_bus);

    return ret;
}

int32_t oal_pcie_send_msg(hcc_bus *pst_bus, uint32_t val)
{
    oal_pcie_linux_res *pst_pci_lres;

    pst_pci_lres = (oal_pcie_linux_res *)pst_bus->data;

    if (oal_warn_on(pst_pci_lres == NULL)) {
        return -OAL_ENODEV;
    }

    if (val >= H2D_MSG_COUNT) {
        oal_io_print("[Error]invaild param[%u]!\n", val);
        return -OAL_EINVAL;
    }

    return oal_pcie_send_message2dev(pst_pci_lres, (uint32_t)val);
}

OAL_STATIC void oal_pcie_intx_bind_cpu(oal_pci_dev_stru *pci_dev, uint32_t cpu_num)
{
#ifdef _PRE_HISI_BINDCPU
    int32_t ret;
    /* 绑核时需再次校验 */
    if (cpu_num < OAL_BUS_MAXCPU_NUM) {
        ret = irq_set_affinity_hint(pci_dev->irq, cpumask_of(cpu_num));
        if (ret != OAL_SUCC) {
            pci_print_log(PCI_LOG_ERR, "intx_irq bind fail, ret = %d", ret);
        }
    } else {
        pci_print_log(PCI_LOG_ERR, "bind cpu[%u] invalid", cpu_num);
    }
#else
    pci_print_log(PCI_LOG_ERR, "not allow bind...");
#endif
}

#ifndef _PRE_PRODUCT_HI1620S_KUNPENG
/*
 * 功能描述  : 根据吞吐量与PPS门限自动指定CPU绑定
 * 1.日    期  : 2020年07月02日
 */
OAL_STATIC void oal_pcie_bindcpu_autoctl(oal_pcie_linux_res *pci_lres,
                                         oal_pci_dev_stru *pci_dev,
                                         int32_t is_bind)
{
#if (_PRE_OS_VERSION_LINUX == _PRE_OS_VERSION)
#ifdef _PRE_HISI_BINDCPU
    oal_pcie_bindcpu_cfg *bindcpu_cfg = (oal_pcie_bindcpu_cfg *)&is_bind;
    uint32_t first_big_cpu;
    struct cpumask all_cpus, fast_cpus;
    cpumask_setall(&all_cpus);
    external_get_fast_cpus(&fast_cpus);
    first_big_cpu = cpumask_first(&fast_cpus);
    /* 中断允许绑核, 判定PCIE传输中断绑核 */
#ifdef CONFIG_NR_CPUS
#if (KERNEL_VERSION(2, 6, 35) <= LINUX_VERSION_CODE)
    if (bindcpu_cfg->irq_cpu && pci_dev->irq != 0) {
        if (bindcpu_cfg->is_bind) {
            oal_pcie_intx_bind_cpu(pci_dev, first_big_cpu);
        } else {
            /* 2.6.35-rc1 */
            oal_pcie_intx_bind_cpu(pci_dev, g_pcie_intx_idle_bind_cpu);
        }
        oal_print_mpxx_log(MPXX_LOG_INFO, "pcie bind irq[%d] to cpu %d",
                           pci_dev->irq, (bindcpu_cfg->is_bind ? first_big_cpu : 0));
    }
#endif
#endif

    /* 线程允许绑核, 判定PCIE补内存线程绑核 */
    if (bindcpu_cfg->thread_cmd && pci_lres->pst_pci_res->pst_rx_hi_task != NULL) {
        if (bindcpu_cfg->is_bind) {
            /* 中断所在核会影响线程迁移 */
            oal_cpumask_clear_cpu(first_big_cpu, &fast_cpus);
            set_cpus_allowed_ptr(pci_lres->pst_pci_res->pst_rx_hi_task, &fast_cpus);
        } else {
            /* CPU0上中断多, WIFI线程不放在CPU0 */
            oal_cpumask_clear_cpu(cpumask_first(&all_cpus), &all_cpus);
            set_cpus_allowed_ptr(pci_lres->pst_pci_res->pst_rx_hi_task, &all_cpus);
        }
        oal_print_mpxx_log(MPXX_LOG_INFO, "pcie rx task thread to 0x%lx",
            (bindcpu_cfg->is_bind ? cpumask_bits(&fast_cpus)[0] : cpumask_bits(&all_cpus)[0]));
    }
#endif
#endif
}

/*
 * 功能描述  : 根据用户命令指定CPU绑定
 * 1.日    期  : 2020年07月02日
 *   修改内容  : 新生成函数
 */
OAL_STATIC void oal_pcie_bindcpu_userctl(oal_pcie_linux_res *pci_lres,
                                         oal_pci_dev_stru *pci_dev,
                                         uint8_t irq_cpu,
                                         uint8_t thread_cmd)
{
#if (_PRE_OS_VERSION_LINUX == _PRE_OS_VERSION)
#ifdef _PRE_HISI_BINDCPU
    struct cpumask thread_cpu_mask;
#endif

#if defined(CONFIG_NR_CPUS) &&  defined(_PRE_HISI_BINDCPU)
    oal_print_mpxx_log(MPXX_LOG_DBG, "pcie bindcpu irq =%u, bind to cpu %d", pci_dev->irq, irq_cpu);
    oal_pcie_intx_bind_cpu(pci_dev, (irq_cpu == 0 ? g_pcie_intx_idle_bind_cpu : (uint32_t)irq_cpu));
#endif

#ifdef _PRE_HISI_BINDCPU
    if (pci_lres->pst_pci_res->pst_rx_hi_task) {
        thread_cpu_mask = *((struct cpumask *)&thread_cmd);
        if (irq_cpu >= OAL_BUS_HPCPU_NUM) {
            /* 中断所在核会影响线程迁移 */
            oal_cpumask_clear_cpu(irq_cpu, &thread_cpu_mask);
        }
        oal_print_mpxx_log(MPXX_LOG_DBG, "pst_rx_hi_task bind to cpu[0x%x]", thread_cmd);
        set_cpus_allowed_ptr(pci_lres->pst_pci_res->pst_rx_hi_task, &thread_cpu_mask);
    }
#endif
#endif
}

int32_t oal_pcie_bindcpu(hcc_bus *pst_bus, uint32_t chan, int32_t is_bind)
{
#ifndef _PRE_PRODUCT_HI1620S_KUNPENG
#if (_PRE_OS_VERSION_LINUX == _PRE_OS_VERSION)
    oal_pcie_bindcpu_cfg bindcpu_cfd = *(oal_pcie_bindcpu_cfg *)&is_bind;
    oal_pcie_linux_res *pst_pci_lres = NULL;
    oal_pci_dev_stru *pst_pci_dev = NULL;

    pst_pci_lres = (oal_pcie_linux_res *)pst_bus->data;
    if (pst_pci_lres == NULL) {
        return OAL_SUCC;
    }

    pst_pci_dev = pst_pci_lres->pst_pcie_dev;
    if (pst_pci_dev == NULL) {
        return OAL_SUCC;
    }

    if (bindcpu_cfd.is_userctl == OAL_TRUE) {
        /* 根据用户命令绑定CPU */
        oal_pcie_bindcpu_userctl(pst_pci_lres, pst_pci_dev, bindcpu_cfd.irq_cpu, bindcpu_cfd.thread_cmd);
    } else {
        /* 根据吞吐量与PPS门限自动绑定CPU */
        oal_pcie_bindcpu_autoctl(pst_pci_lres, pst_pci_dev, is_bind);
    }
#endif
#endif
    return OAL_SUCC;
}
#endif

int32_t oal_pcie_get_trans_count(hcc_bus *hi_bus, uint64_t *tx, uint64_t *rx)
{
    oal_pcie_linux_res *pst_pci_lres = (oal_pcie_linux_res *)hi_bus->data;
    return oal_pcie_get_host_trans_count(pst_pci_lres->pst_pci_res, tx, rx);
}

#ifdef _PRE_PLAT_FEATURE_HI110X_DUAL_PCIE
OAL_STATIC int32_t oal_pcie_dual_reinit(oal_pcie_linux_res *pst_pci_lres)
{
    int32_t ret;
    uint32_t version = 0x0;
    declare_time_cost_stru(reinit);

    if (pst_pci_lres->pst_pcie_dev_dual == NULL) {
        return OAL_SUCC; /* dual device not exist */
    }

    oal_get_time_cost_start(reinit);

    oal_print_mpxx_log(MPXX_LOG_INFO, "oal_pcie_dual_reinit");

    ret = oal_pcie_enable_dual_device(pst_pci_lres);
    if (ret != OAL_SUCC) {
        return ret;
    }

    /* check link status */
    ret = oal_pci_read_config_dword(pst_pci_lres->pst_pcie_dev_dual, 0x0, &version);
    if (ret) {
        pci_print_log(PCI_LOG_ERR, "read pci version failed, enable device failed, ret=%d, version=0x%x",
                      ret, version);
        declare_dft_trace_key_info("pcie_enable_dual_device_reinit_fail", OAL_DFT_TRACE_FAIL);
        return -OAL_ENODEV;
    }

    if (version != pst_pci_lres->version) {
        pci_print_log(PCI_LOG_WARN, "version:0x%x is not match with:0x%x", version, pst_pci_lres->version);
        return -OAL_ENODEV;
    } else {
        pci_print_log(PCI_LOG_INFO, "pcie enable device check succ");
    }

    /* 初始化PCIE资源 */
    ret = oal_pcie_set_outbound_membar(pst_pci_lres->pst_pci_res,
                                       &pst_pci_lres->pst_pci_res->st_iatu_dual_bar);
    if (ret) {
        oal_io_print("enable regions failed, ret=%d\n", ret);
        return ret;
    }

    oal_get_time_cost_end(reinit);
    oal_calc_time_cost_sub(reinit);
    pci_print_log(PCI_LOG_INFO, "pcie dual reinit cost %llu us", time_cost_var_sub(reinit));
    return ret;
}
#endif

int32_t oal_pcie_deinit(hcc_bus *pst_bus)
{
    return OAL_SUCC;
}

/* mp16 cs2 to be deleted: disable group2 cputrace */
#define DIAG_CTL_GROUP2_SAMPLE_ENABLE_REG 0x40124C80
OAL_STATIC int32_t oal_pcie_disable_cputrace(oal_pcie_linux_res *pst_pci_lres)
{
    int32_t ret;
    pci_addr_map addr_map;
    uint32_t cpu_address = DIAG_CTL_GROUP2_SAMPLE_ENABLE_REG;
    uint32_t value = 0x0;
    oal_pci_dev_stru *pst_pci_dev = pcie_res_to_dev(pst_pci_lres->pst_pci_res);
    if ((pst_pci_dev->device != PCIE_HISI_DEVICE_ID_MP16)
        && (pst_pci_dev->device != PCIE_HISI_DEVICE_ID_MP16FPGA)) {
        return OAL_SUCC;
    }

    ret = oal_pcie_inbound_ca_to_va(pst_pci_lres->pst_pci_res, cpu_address, &addr_map);
    if (oal_unlikely(ret != OAL_SUCC)) {
        oal_io_print("oal_pcie_disable_cputrace 0x%8x unmap, write failed!\n", cpu_address);
        return -OAL_EFAIL;
    }

    oal_writel(value, (void *)addr_map.va);
    oal_io_print("oal_pcie_disable_cputrace 0x%x = %x, write success!\n", cpu_address, value);
    return ret;
}

/* reinit pcie ep control */
int32_t oal_pcie_reinit(hcc_bus *pst_bus)
{
    int32_t ret;
    uint32_t version = 0x0;
    oal_pcie_linux_res *pst_pci_lres = (oal_pcie_linux_res *)pst_bus->data;
    declare_time_cost_stru(reinit);

    oal_get_time_cost_start(reinit);

    oal_print_mpxx_log(MPXX_LOG_INFO, "wake_sema_count=%d", pst_bus->sr_wake_sema.count);
    sema_init(&pst_bus->sr_wake_sema, 1); /* S/R信号量 */

    hcc_bus_disable_state(pst_bus, OAL_BUS_STATE_ALL);
    ret = oal_pcie_enable_device(pst_pci_lres);
    if (ret == OAL_SUCC) {
#ifdef _PRE_COMMENT_CODE_
        /* 需要在初始化完成后打开 */
        hcc_bus_enable_state(pst_bus, OAL_BUS_STATE_ALL);
#endif
    }

    /* check link status */
    if (pst_pci_lres->pst_pcie_dev != NULL) {
        ret = oal_pci_read_config_dword(pst_pci_lres->pst_pcie_dev, 0x0, &version);
        if (ret) {
            pci_print_log(PCI_LOG_ERR, "read pci version failed, enable device failed, ret=%d, version=0x%x",
                          ret, version);
            declare_dft_trace_key_info("pcie_enable_device_reinit_fail", OAL_DFT_TRACE_FAIL);
            return -OAL_ENODEV;
        }

        if (version != pst_pci_lres->version) {
            pci_print_log(PCI_LOG_WARN, "version:0x%x is not match with:0x%x", version, pst_pci_lres->version);
        } else {
            pci_print_log(PCI_LOG_INFO, "pcie enable device check succ");
        }
    }

    /* 初始化PCIE资源 */
    ret = oal_pcie_enable_regions(pst_pci_lres->pst_pci_res);
    if (ret) {
        oal_io_print("enable regions failed, ret=%d\n", ret);
        return ret;
    }

    ret = oal_pcie_iatu_init(pst_pci_lres->pst_pci_res);
    if (ret) {
        oal_io_print("iatu init failed, ret=%d\n", ret);
        return ret;
    }

    // pcie 中断重新绑核
    oal_pcie_intx_bind_cpu(pst_pci_lres->pst_pcie_dev, g_pcie_intx_idle_bind_cpu);

    oal_get_time_cost_end(reinit);
    oal_calc_time_cost_sub(reinit);
    pci_print_log(PCI_LOG_INFO, "pcie reinit cost %llu us", time_cost_var_sub(reinit));
#ifdef _PRE_PLAT_FEATURE_HI110X_DUAL_PCIE
    ret = oal_pcie_dual_reinit(pst_pci_lres);
#endif

    /* mp16 cs2 to be deleted: disable group2 cputrace */
    ret = oal_pcie_disable_cputrace(pst_pci_lres);

    return ret;
}

int32_t oal_pcie_host_lock(hcc_bus *pst_bus)
{
    oal_pcie_linux_res *pst_pci_lres;
    pst_pci_lres = (oal_pcie_linux_res *)pst_bus->data;
    return OAL_SUCC;
}

int32_t oal_pcie_host_unlock(hcc_bus *pst_bus)
{
    oal_pcie_linux_res *pst_pci_lres;
    pst_pci_lres = (oal_pcie_linux_res *)pst_bus->data;
    return OAL_SUCC;
}

OAL_STATIC int32_t oal_pcie_sleep_wait_rx_trans_done(oal_pcie_res *pst_pci_res)
{
    if (pst_pci_res->link_state <= PCI_WLAN_LINK_UP) {
        return OAL_SUCC;
    }

    if (pst_pci_res->chip_info.ete_support != OAL_TRUE) {
        return OAL_SUCC;
    }

    return oal_ete_sleep_wait_rx_trans_done(pst_pci_res);
}

void oal_pcie_sleep_linkdown(hcc_bus *pst_bus)
{
    oal_pcie_linux_res *pst_pci_lres;
    oal_pcie_res *pst_pci_res;
    pst_pci_lres = (oal_pcie_linux_res *)pst_bus->data;
    pst_pci_res = pst_pci_lres->pst_pci_res;
    oal_pcie_save_default_resource(pst_pci_lres);
    hcc_bus_power_ctrl_register(pst_bus, HCC_BUS_CTRL_POWER_DOWN, NULL, NULL);
    hcc_bus_power_action(pst_bus, HCC_BUS_POWER_SLEEP);
}

/* MP13 PCIE 通过 host_wakeup_dev gpio 来唤醒和通知WCPU睡眠 */
int32_t oal_pcie_sleep_request(hcc_bus *pst_bus)
{
    int32_t ret;
    ktime_t start_time;
    uint64_t total_time;
    oal_pcie_linux_res *pst_pci_lres;
    oal_pcie_res *pst_pci_res;
    pst_pci_lres = (oal_pcie_linux_res *)pst_bus->data;
    pst_pci_res = pst_pci_lres->pst_pci_res;

    if (oal_pcie_sleep_wait_rx_trans_done(pst_pci_res) != OAL_SUCC) {
        declare_dft_trace_key_info("wlan_sleep_wait_rx_done_timeout", OAL_DFT_TRACE_FAIL);
    }

    oal_disable_pcie_irq(pst_pci_lres);

    if (g_pcie_auto_disable_aspm != 0) {
        /* enable L1 after wakeup */
        oal_pcie_set_device_aspm_dync_disable(pst_pci_res, OAL_FALSE);
        oal_pcie_device_xfer_pending_sig_clr(pst_pci_res, OAL_TRUE);
    }
    start_time = ktime_get();

    /* 拉低GPIO，PCIE只有在system suspend的时候才会下电 GPIO 拉低之后 DEV 随时可能进深睡，不允许再通过PCIE 访问 */
    mutex_lock(&pst_pci_res->st_rx_mem_lock);
    oal_pcie_device_sr_debug(pst_pci_res, OAL_TRUE);
    oal_pcie_device_auxclk_switch(pst_pci_res, AUXCLK_RTC);
    oal_pcie_change_link_state(pst_pci_res, PCI_WLAN_LINK_UP);
    mutex_unlock(&pst_pci_res->st_rx_mem_lock);

    // pci_clear_master
    oal_pcie_enable_master(pst_pci_res, pst_pci_lres->pst_pcie_dev, OAL_FALSE);
    if (g_pcie_pm_linkdown_switch != 0) {
        oal_pcie_sleep_linkdown(pst_bus); // 睡眠断链开关打开后，pcie此时主动断链
    }

    total_time = oal_ktime_get_us(start_time);
    pci_print_log(PCI_LOG_WARN, "consume %llu us", total_time);
    if (total_time > WIFI_SLEEP_REQUEST_TIMEOUT) {
        pci_print_log(PCI_LOG_INFO, "oal_pcie_sleep_request long_time");
        declare_dft_trace_key_info("oal_pcie_sleep_request long_time", OAL_DFT_TRACE_OTHER);
    }
    ret = board_host_wakeup_dev_set(0);
    oal_usleep(WLAN_WAKEUP_DEV_EVENT_DELAY_US);
    return ret;
}

OAL_STATIC int32_t oal_pcie_sr_wakeup_request_enable(oal_pcie_linux_res *pst_pci_lres, hcc_bus *pst_bus)
{
    int32_t ret;

    if (oal_pcie_check_link_state(pst_pci_lres->pst_pci_res) == OAL_FALSE) {
        uint32_t version = 0x0;
        ret = oal_pci_read_config_dword(pst_pci_lres->pst_pcie_dev, 0x0, &version);
        pci_print_log(PCI_LOG_ERR, "mem access error, maybe linkdown! config read version :0x%x, ret=%d",
                      version, ret);
        return -OAL_ENODEV;
    }

    /* 唤醒流程，RES已经初始化 */
    oal_pcie_change_link_state(pst_pci_lres->pst_pci_res, PCI_WLAN_LINK_RES_UP);
    oal_pcie_device_xfer_pending_sig_clr(pst_pci_lres->pst_pci_res, OAL_FALSE);
    oal_atomic_set(&pst_pci_lres->st_pcie_wakeup_flag, 1);
    ret = oal_pcie_enable_device_func(pst_pci_lres);
    oal_atomic_set(&pst_pci_lres->st_pcie_wakeup_flag, 0);
    oal_pcie_device_xfer_pending_sig_clr(pst_pci_lres->pst_pci_res, OAL_TRUE);
    if (ret != OAL_SUCC) {
        oal_pcie_disable_regions(pst_pci_lres->pst_pci_res);
        return ret;
    }

    mutex_lock(&pst_pci_lres->pst_pci_res->st_rx_mem_lock);
    oal_pcie_change_link_state(pst_pci_lres->pst_pci_res, PCI_WLAN_LINK_WORK_UP);
    mutex_unlock(&pst_pci_lres->pst_pci_res->st_rx_mem_lock);

    hcc_bus_enable_state(pst_bus, OAL_BUS_STATE_ALL);
    pst_pci_lres->power_status = PCIE_EP_IP_POWER_UP;

    return OAL_SUCC;
}

/* 唤醒device, 增加延时防止gpio毛刺 */
int32_t oal_pcie_wake_linkup_handler(void *data)
{
    int32_t ret;
    ktime_t time_start;
    uint64_t trans_devack_us;
    uint64_t wakeup_delay_us;

    time_start = ktime_get();
    ret = oal_pcie_ete_wake_powerup_handler(data);
    if (g_wlan_wakeup_delay_enable == OAL_TRUE) {
        trans_devack_us = oal_ktime_get_us(time_start);
        if (trans_devack_us < g_wake_dev_min_time) {
            wakeup_delay_us = g_wake_dev_min_time - trans_devack_us;
            usleep_range(wakeup_delay_us, wakeup_delay_us); // 不足6ms时补齐
            oal_print_mpxx_log(MPXX_LOG_INFO, "get dev_ack cost %llu us, need delay %llu us",
                               trans_devack_us, wakeup_delay_us);
        }
    }
    return ret;
}

OAL_STATIC int32_t oal_pcie_linkdown_wakeup(oal_pcie_linux_res *pst_pci_lres, hcc_bus *pst_bus)
{
    int32_t ret;
    uint32_t ret_timeout;
    struct wlan_pm_s *pst_wlan_pm = wlan_pm_get_drv();
    if (g_pcie_pm_linkdown_switch == OAL_TRUE) {
        oal_pcie_power_notifiy_register(pst_pci_lres->pst_pcie_dev,
                                        oal_pcie_get_pcie_rc_idx(pst_pci_lres->pst_pcie_dev),
                                        oal_pcie_wake_linkup_handler, NULL, (void *)pst_pci_lres->pst_pcie_dev);
        ret = hcc_bus_power_action(pst_bus, HCC_BUS_POWER_UP);
        if (ret != OAL_SUCC) {
            declare_dft_trace_key_info("pcie_wakeup_relink fail", OAL_DFT_TRACE_FAIL);
            pci_print_log(PCI_LOG_ERR, "hcc_bus_power_action HCC_BUS_POWER_UP failed!ret=%d", ret);
            return -OAL_EFAIL;
        }
    } else {
        oal_pcie_device_wakeup_handler(NULL);
        if (pst_wlan_pm != NULL) {
            ret_timeout = oal_wait_for_completion_timeout(&pst_wlan_pm->st_wifi_powerup_done,
                (uint32_t)oal_msecs_to_jiffies(WIFI_POWERUP_DONE_TIMEOUT));
            if (oal_unlikely(ret_timeout == 0)) {
                /* 超时不做处理继续尝试建链 */
                declare_dft_trace_key_info("pcie_powerup_wakeup ack timeout", OAL_DFT_TRACE_FAIL);
            }
        } else {
            oal_msleep(100); /* delay 100 ms 让出系统调度 */
        }
    }
    return OAL_SUCC;
}

OAL_STATIC int32_t oal_pcie_sr_wakeup_request(oal_pcie_linux_res *pst_pci_lres, hcc_bus *pst_bus)
{
    int32_t ret;
    declare_time_cost_stru(cost);
    struct wlan_pm_s *pst_wlan_pm = wlan_pm_get_drv();
    oal_get_time_cost_start(cost);

    if (pst_wlan_pm != NULL) {
        oal_init_completion(&pst_wlan_pm->st_wifi_powerup_done);
    }

    oal_atomic_set(&g_bus_powerup_dev_wait_ack, 1);
    ret = oal_pcie_linkdown_wakeup(pst_pci_lres, pst_bus);
    oal_atomic_set(&g_bus_powerup_dev_wait_ack, 0);
    if (ret != OAL_SUCC) {
        return ret;
    }

    /* iatu */
    ret = oal_pcie_reinit(pst_bus);
    if (ret != OAL_SUCC) {
        pci_print_log(PCI_LOG_ERR, "oal_pcie_reinit failed!ret=%d", ret);
        return ret;
    }

    oal_pcie_host_ip_init(pst_pci_lres);

    if (pst_pci_lres->pst_pci_res->chip_info.ete_support == OAL_TRUE) {
        ret = oal_ete_transfer_deepwkup_restore(pst_pci_lres->pst_pci_res);
        if (ret != OAL_SUCC) {
            pci_print_log(PCI_LOG_ERR, " ete deep wakeup failed, ret=%d", ret);
            return ret;
        }
    }

    ret = oal_pcie_sr_wakeup_request_enable(pst_pci_lres, pst_bus);
    if (ret != OAL_SUCC) {
        return ret;
    }

    oal_get_time_cost_end(cost);
    oal_calc_time_cost_sub(cost);
    pci_print_log(PCI_LOG_INFO, "pcie power up init succ, cost %llu us\n", time_cost_var_sub(cost));

    return OAL_SUCC;
}

int32_t oal_pcie_wakeup_request(hcc_bus *pst_bus)
{
    int32_t ret;
    oal_pcie_linux_res *pst_pci_lres;
    pst_pci_lres = (oal_pcie_linux_res *)pst_bus->data;

    // 1.拉高 Host WakeUp Device gpio
    // 2.调用kirin_pcie_pm_control 上电RC 检查建链
    // 3.restore state, load iatu config
#ifdef _PRE_CONFIG_ARCH_HI1620S_KUNPENG_PCIE
    /* 正常单芯片唤醒拉高GPIO即可 */
    oal_pcie_device_wakeup_handler(NULL);
    // pci_set_master
    oal_pcie_enable_master(pst_pci_lres->pst_pci_res, pst_pci_lres->pst_pcie_dev, OAL_TRUE);

    return OAL_SUCC;
#endif

    if (oal_unlikely(pst_pci_lres->pst_pci_res->link_state <= PCI_WLAN_LINK_DOWN)) {
        pci_print_log(PCI_LOG_WARN, "link invaild, wakeup failed, link_state:%s",
                      oal_pcie_get_link_state_str(pst_pci_lres->pst_pci_res->link_state));
        return -OAL_ENODEV;
    }

    if (pst_pci_lres->power_status == PCIE_EP_IP_POWER_DOWN) {
        ret = oal_pcie_sr_wakeup_request(pst_pci_lres, pst_bus);
        if (ret != OAL_SUCC) {
            return ret;
        }
    } else {
        /* 正常单芯片唤醒拉高GPIO即可 */
        oal_pcie_device_wakeup_handler(NULL);
        // pci_set_master
        oal_pcie_enable_master(pst_pci_lres->pst_pci_res, pst_pci_lres->pst_pcie_dev, OAL_TRUE);
    }

    return OAL_SUCC;
}

int32_t oal_pcie_get_sleep_state(hcc_bus *pst_bus)
{
    oal_pcie_linux_res *pst_pci_lres;
    pst_pci_lres = (oal_pcie_linux_res *)pst_bus->data;
    if (oal_warn_on(pst_pci_lres == NULL)) {
        oal_io_print("pst_pci_lres is null\n");
        return -OAL_EINVAL;
    }

    return (conn_get_gpio_level(HOST_WKUP_W) == GPIO_LEVEL_HIGH) ?
                DISALLOW_TO_SLEEP_VALUE : ALLOW_TO_SLEEP_VALUE;
}

int32_t oal_pcie_rx_int_mask(hcc_bus *pst_bus, int32_t is_mask)
{
    oal_pcie_linux_res *pst_pci_lres;
    pst_pci_lres = (oal_pcie_linux_res *)pst_bus->data;

    if (oal_warn_on(pst_pci_lres == NULL)) {
        oal_io_print("pst_pci_lres is null\n");
        return -OAL_EINVAL;
    }
    return OAL_SUCC;
}

int32_t oal_pcie_power_ctrl(hcc_bus *hi_bus, hcc_bus_power_ctrl_type ctrl,
                            int (*func)(void *data), void *data)
{
    int32_t ret = -OAL_EFAIL;
    oal_pcie_linux_res *pst_pci_lres = NULL;
    oal_pci_dev_stru *pst_pci_dev;
    if (hi_bus == NULL) {
        return ret;
    }
    pst_pci_lres = (oal_pcie_linux_res *)hi_bus->data;
    pst_pci_dev = pst_pci_lres->pst_pcie_dev;
    if (ctrl == HCC_BUS_CTRL_POWER_UP) {
        ret = oal_pcie_power_notifiy_register(pst_pci_dev, oal_pcie_get_pcie_rc_idx(pst_pci_dev), func, NULL, data);
    }

    if (ctrl == HCC_BUS_CTRL_POWER_DOWN) {
        ret = oal_pcie_power_notifiy_register(pst_pci_dev, oal_pcie_get_pcie_rc_idx(pst_pci_dev), NULL, func, data);
    }

    return ret;
}

int32_t oal_pcie_wait_for_completion(oal_completion *completion, uint32_t timeout_ms)
{
    int32_t i;
    uint32_t ret;

    if (completion == NULL || timeout_ms == 0) {
        return -OAL_EINVAL;
    }

    for (i = 0; i < OAL_ROUND_UP(timeout_ms, MSEC_PER_SEC) / MSEC_PER_SEC; i++) {
        if (hcc_bus_get_shutdown_flag() == OAL_TRUE) {
            ps_print_info("hcc is shutdown, exit");
            return -OAL_EFAIL;
        }

        ret = oal_wait_for_completion_timeout(completion, (uint32_t)oal_msecs_to_jiffies(MSEC_PER_SEC));
        if (ret != 0) {
            return OAL_SUCC;
        }
    }
    return -OAL_ETIMEDOUT;
}

int32_t oal_pcie_enable_device_func(oal_pcie_linux_res *pst_pci_lres)
{
    int32_t i;
    int32_t ret;
    unsigned int timeout = hi110x_get_emu_timeout(HOST_WAIT_BOTTOM_INIT_TIMEOUT);
    const int32_t retry = 2;
    hcc_bus *pst_bus = pst_pci_lres->pst_bus;
    char *dname = NULL;

    if (pst_bus == NULL) {
        return -OAL_ENODEV;
    }

    for (i = 0; i < retry; i++) {
        oal_init_completion(&pst_pci_lres->st_pcie_ready);

        ret = oal_pcie_send_message2dev(pst_pci_lres, PCIE_H2D_MESSAGE_HOST_READY);
        if (ret != OAL_SUCC) {
            oal_io_print("oal_pcie_send_message2dev failed, ret=%d\n", ret);
            continue;
        }

        /* 第一个中断有可能在中断使能之前上报，强制调度一次RX Thread */
        up(&pst_bus->rx_sema);

        ret = oal_pcie_wait_for_completion(&pst_pci_lres->st_pcie_ready, timeout);
        if (ret == -OAL_ETIMEDOUT) {
            oal_io_print("wait pcie ready timeout... %u ms \n", timeout);
            up(&pst_bus->rx_sema);
            ret = oal_pcie_wait_for_completion(&pst_pci_lres->st_pcie_ready, timeout);
            if (ret == -OAL_ETIMEDOUT) {
                oal_io_print("pcie retry 5 second hold, still timeout");
                dname = (i == 0 ? "wait pcie ready when power on, retry" :
                    "wait pcie ready when power on, retry still failed");
                declare_dft_trace_key_info(dname, OAL_DFT_TRACE_FAIL);
                continue;
            } else if (ret == -OAL_EFAIL || ret == -OAL_EINVAL) {
                return ret;
            } else {
                /* 强制调度成功，说明有可能是GPIO中断未响应 */
                oal_io_print("[E]retry succ, maybe gpio interrupt issue");
                declare_dft_trace_key_info("pcie gpio int issue", OAL_DFT_TRACE_FAIL);
                break;
            }
        } else if (ret == -OAL_EFAIL || ret == -OAL_EINVAL) {
            return ret;
        } else {
            break;
        }
    }

    if (i >= retry) {
        return ret;
    }

    return OAL_SUCC;
}

#ifdef _PRE_CONFIG_HERTZ_PCIE
#define PCIE_INTX_ENABLE 1
#define PCIE_INTX_DISABLE 0
/*
 * Function: set pci_config_reg-COMMAND，enable/disable intx interrupts
 * @enable: whether to enable or disable PCI INTx
 */
void oal_pcie_intx_enable(oal_pcie_linux_res *pst_pci_lres, int32_t enable)
{
    if (oal_unlikely(pst_pci_lres->pst_pci_res->link_state < PCI_WLAN_LINK_UP)) {
        pci_print_log(PCI_LOG_ERR, "can't set config_intx_enable, link_state:%s",
                      oal_pcie_get_link_state_str((pst_pci_lres->pst_pci_res->link_state)));
        return;
    }
    return pci_intx(pst_pci_lres->pst_pcie_dev, enable);
}
#endif

int32_t oal_enable_pcie_irq(oal_pcie_linux_res *pst_pci_lres)
{
    oal_uint flag;
    if (oal_warn_on(pst_pci_lres == NULL)) {
        return -OAL_ENODEV;
    }

    oal_spin_lock_irq_save(&pst_pci_lres->st_lock, &flag);
    if (pst_pci_lres->irq_stat == 1) {
        if (!pst_pci_lres->st_msi.is_msi_support) {
            /* intx */
            pci_print_log(PCI_LOG_DBG, "enable_pcie_irq");
            enable_irq(pst_pci_lres->pst_pcie_dev->irq);
#ifdef _PRE_CONFIG_HERTZ_PCIE
            // hertz pcie为一对多设计，故disable_intx时需从dev屏蔽
            // 需注意，部分平台存在 config 口与 mem 口抢占问题
            oal_pcie_intx_enable(pst_pci_lres, PCIE_INTX_ENABLE);
#endif
        } else {
            /* msi */
        }
        pst_pci_lres->irq_stat = 0;
    }
    oal_spin_unlock_irq_restore(&pst_pci_lres->st_lock, &flag);

    return OAL_SUCC;
}

int32_t oal_disable_pcie_irq(oal_pcie_linux_res *pst_pci_lres)
{
    oal_uint flag;
    oal_uint nosync;

    if (oal_warn_on(pst_pci_lres == NULL)) {
        return -OAL_ENODEV;
    }

    nosync = in_atomic() || in_softirq() || in_interrupt() || irqs_disabled();
    oal_spin_lock_irq_save(&pst_pci_lres->st_lock, &flag);
    if (pst_pci_lres->irq_stat == 0) {
        pst_pci_lres->irq_stat = 1;
        if (!pst_pci_lres->st_msi.is_msi_support) {
            /* intx */
            pci_print_log(PCI_LOG_DBG, "disable_pcie_irq");
#ifdef _PRE_CONFIG_HERTZ_PCIE
            // hertz pcie为一对多设计，故disable_intx时需从dev屏蔽
            // 需注意，部分平台存在 config 口与 mem 口抢占问题
            oal_pcie_intx_enable(pst_pci_lres, PCIE_INTX_DISABLE);
#endif
            if (nosync) {
                disable_irq_nosync(pst_pci_lres->pst_pcie_dev->irq);
            } else {
                /* process context */
                disable_irq_nosync(pst_pci_lres->pst_pcie_dev->irq);
                oal_spin_unlock_irq_restore(&pst_pci_lres->st_lock, &flag);
                synchronize_irq(pst_pci_lres->pst_pcie_dev->irq);
                oal_spin_lock_irq_save(&pst_pci_lres->st_lock, &flag);
            }
        } else {
            /* msi */
        }
    }
    oal_spin_unlock_irq_restore(&pst_pci_lres->st_lock, &flag);

    return OAL_SUCC;
}

OAL_STATIC void oal_pcie_host_ip_edma_init(oal_pcie_linux_res *pst_pci_lres)
{
    if (pst_pci_lres->pst_pci_res->chip_info.edma_support != OAL_TRUE) {
        return;
    }
    oal_pcie_set_device_soft_fifo_enable(pst_pci_lres->pst_pci_res);
    oal_pcie_set_device_ringbuf_bugfix_enable(pst_pci_lres->pst_pci_res);
    oal_pcie_set_device_dma_check_enable(pst_pci_lres->pst_pci_res);
}

OAL_STATIC void oal_pcie_host_ip_ete_init(oal_pcie_linux_res *pst_pci_lres)
{
    if (pst_pci_lres->pst_pci_res->chip_info.ete_support != OAL_TRUE) {
        return;
    }
    oal_print_mpxx_log(MPXX_LOG_INFO, "oal_pcie_host_ip_ete_init");
    (void)oal_ete_ip_init(pst_pci_lres->pst_pci_res);
}

int32_t oal_pcie_host_ip_init(oal_pcie_linux_res *pst_pci_lres)
{
    oal_pcie_mps_init(pst_pci_lres);
    oal_pcie_host_aspm_init(pst_pci_lres);
    oal_pcie_host_ip_edma_init(pst_pci_lres);
    oal_pcie_host_ip_ete_init(pst_pci_lres);
    return OAL_SUCC;
}

void oal_pcie_print_chip_info(oal_pcie_linux_res *pst_pci_lres, uint32_t is_full_log)
{
    /* L1 recovery count */
    uint32_t l1_err_cnt = oal_readl(pst_pci_lres->pst_pci_res->pst_pci_ctrl_base + PCIE_STAT_CNT_LTSSM_ENTER_RCVRY_OFF);
    uint32_t l1_recvy_cnt = oal_readl(pst_pci_lres->pst_pci_res->pst_pci_ctrl_base + PCIE_STAT_CNT_L1_ENTER_RCVRY_OFF);
    const uint32_t l1_max_err_cnt = 3;

    if (pst_pci_lres->l1_err_cnt == 0) {
        pst_pci_lres->l1_err_cnt = l1_err_cnt;
    }

    if (is_full_log) {
        if (l1_err_cnt > l1_max_err_cnt - 1) {
            oal_print_mpxx_log(MPXX_LOG_ERR, "not l1 recovry error count %u, link_state unstable", l1_err_cnt);
        } else {
            /* 建链过程中会进入2次 */
            oal_print_mpxx_log(MPXX_LOG_INFO, "link_state stable, l1 excp count:%u", l1_err_cnt);
        }

        if (l1_recvy_cnt) {
            oal_print_mpxx_log(MPXX_LOG_INFO, "l1 enter count is %u", l1_recvy_cnt);
        } else {
            oal_print_mpxx_log(MPXX_LOG_INFO, "l1 maybe not enable");
        }
    } else {
        if (l1_err_cnt > l1_max_err_cnt - 1) {
            if ((pst_pci_lres->l1_err_cnt == 0) || (l1_err_cnt > pst_pci_lres->l1_err_cnt)) {
                /* link state error */
                oal_print_mpxx_log(MPXX_LOG_ERR, "not l1 recovry error count %u, link_state unstable", l1_err_cnt);
                declare_dft_trace_key_info("pcie_link_state_unstable", OAL_DFT_TRACE_FAIL);
            } else {
                oal_print_mpxx_log(MPXX_LOG_DBG, "link_state stable, l1 excp count:%u", l1_err_cnt);
            }
        }
    }

    pst_pci_lres->l1_err_cnt = l1_err_cnt;
}

void oal_pcie_chip_info(hcc_bus *pst_bus, uint32_t is_need_wakeup, uint32_t is_full_log)
{
    oal_pcie_linux_res *pst_pci_lres;
    pst_pci_lres = (oal_pcie_linux_res *)pst_bus->data;
    if (oal_warn_on(pst_pci_lres == NULL)) {
        return;
    }

    if (oal_unlikely(pst_pci_lres->pst_pci_res->link_state <= PCI_WLAN_LINK_DOWN)) {
        oal_print_mpxx_log(MPXX_LOG_ERR, "link_state:%s",
                           oal_pcie_get_link_state_str(pst_pci_lres->pst_pci_res->link_state));
        return;
    }

    if (is_need_wakeup == OAL_TRUE) {
        if (pst_bus->bus_dev == NULL) {
            return;
        }
    }

    if (is_need_wakeup == OAL_TRUE) {
        hcc_tx_transfer_lock(pst_bus->bus_dev->hcc);
        if (oal_unlikely(hcc_bus_pm_wakeup_device(pst_pci_lres->pst_bus) != OAL_SUCC)) {
            oal_print_mpxx_log(MPXX_LOG_ERR, "wakeup device failed");
            hcc_tx_transfer_unlock(pst_bus->bus_dev->hcc);
            return;
        }
    }

    oal_pcie_print_chip_info(pst_pci_lres, is_full_log);

    if (is_need_wakeup == OAL_TRUE) {
        hcc_tx_transfer_unlock(pst_bus->bus_dev->hcc);
    }
}

int32_t oal_pcie_ip_init(hcc_bus *pst_bus)
{
    int32_t ret;
    oal_pcie_linux_res *pst_pci_lres = NULL;
    if (pst_bus == NULL) {
        oal_print_mpxx_log(MPXX_LOG_ERR, "pst_bus is null");
        return -OAL_ENODEV;
    }

    pst_pci_lres = (oal_pcie_linux_res *)pst_bus->data;
    if (oal_warn_on(pst_pci_lres == NULL)) {
        oal_print_mpxx_log(MPXX_LOG_ERR, "pst_pci_lres is null");
        return -OAL_ENODEV;
    }

    ret = oal_pcie_device_auxclk_init(pst_pci_lres->pst_pci_res);
    if (ret) {
        return ret;
    }

    ret = oal_pcie_device_aspm_init(pst_pci_lres->pst_pci_res);
    if (ret) {
        return ret;
    }

    /* 使能低功耗 */
    ret = oal_pcie_host_aspm_init(pst_pci_lres);
    if (ret) {
        return ret;
    }

    return OAL_SUCC;
}

int32_t oal_pcie_ip_voltage_bias_init(hcc_bus *pst_bus)
{
    oal_pcie_linux_res *pst_pci_lres;
    pst_pci_lres = (oal_pcie_linux_res *)pst_bus->data;
    if (oal_warn_on(pst_pci_lres == NULL)) {
        return -OAL_ENODEV;
    }
    return oal_pcie_voltage_bias_init(pst_pci_lres->pst_pci_res);
}

#ifdef _PRE_PLAT_FEATURE_HI110X_DUAL_PCIE
OAL_STATIC OAL_INLINE int32_t oal_ete_dual_pci_power_down(oal_pcie_linux_res *pst_pci_lres)
{
    if (pst_pci_lres->pst_pcie_dev_dual == NULL) {
        /* no dual pci */
        return OAL_SUCC;
    }
    oal_pcie_power_notifiy_register(g_pcie_linux_res->pst_pcie_dev_dual, g_pcie_rc_idx_dual, NULL,
                                    wlan_dual_first_power_off_callback, NULL);
    return oal_pcie_pm_control(g_pcie_linux_res->pst_pcie_dev_dual, g_pcie_rc_idx_dual, PCIE_POWER_OFF_WITHOUT_L3MSG);
}
#endif

#ifdef _PRE_CONFIG_PCIE_SHARED_INTX_IRQ
/* request 会有申请内存操作，这里不能加自旋锁保护;  该函数只限于上下电调用，不会引起竞争 */
int32_t oal_enable_pcie_irq_with_request(oal_pcie_linux_res *pst_pci_lres)
{
    int32_t ret = OAL_SUCC;
    if (oal_warn_on(pst_pci_lres == NULL)) {
        return -OAL_ENODEV;
    }

    if (pst_pci_lres->irq_stat == 1) {
        if (!pst_pci_lres->st_msi.is_msi_support) {
            /* intx */
            pci_print_log(PCI_LOG_INFO, "enable_pcie_irq with request_irq");
            ret = request_irq(pst_pci_lres->pst_pcie_dev->irq, oal_pcie_intx_edma_isr, IRQF_SHARED,
                "hisi_edma_intx", (void *)pst_pci_lres);
            if (ret < 0) {
                oal_io_print("enable_pcie_irq with request_irq error\n");
                oal_pcie_change_link_state(pst_pci_lres->pst_pci_res, PCI_WLAN_LINK_DOWN);
                return -OAL_EINTR;
            }
        }
        pst_pci_lres->irq_stat = 0;
    }

    return OAL_SUCC;
}

int32_t oal_disable_pcie_irq_with_free(oal_pcie_linux_res *pst_pci_lres)
{
    if (oal_warn_on(pst_pci_lres == NULL)) {
        return -OAL_ENODEV;
    }

    if (pst_pci_lres->irq_stat == 0) {
        if (!pst_pci_lres->st_msi.is_msi_support) {
            /* intx */
            pci_print_log(PCI_LOG_INFO, "disable_pcie_irq with free_irq");
            free_irq(pst_pci_lres->pst_pcie_dev->irq, pst_pci_lres);
            pst_pci_lres->irq_stat = 1;
        }
    }

    return OAL_SUCC;
}
#endif

OAL_STATIC void oal_pcie_power_down(hcc_bus *pst_bus)
{
    oal_pcie_linux_res *pst_pci_lres = (oal_pcie_linux_res *)pst_bus->data;
    pcie_ep_ip_status old_power_status = pst_pci_lres->power_status;

    int32_t ret;
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(4, 4, 0))
    int32_t power_option = PCIE_POWER_OFF_WITHOUT_L3MSG;
#else
    int32_t power_option = PCIE_POWER_OFF_WITH_L3MSG;
#endif

    /* disable intx gpio... 等待中断处理完 */
#ifdef _PRE_CONFIG_PCIE_SHARED_INTX_IRQ
    ret = oal_disable_pcie_irq_with_free(pst_pci_lres);
#else
    ret = oal_disable_pcie_irq(pst_pci_lres);
#endif
    if (ret != OAL_SUCC) {
        return;
    }

    pst_pci_lres->power_status = PCIE_EP_IP_POWER_DOWN;
    if (pst_pci_lres->pst_pci_res == NULL) {
        oal_print_mpxx_log(MPXX_LOG_ERR, "pst_pci_res is null");
        return;
    }

    pst_pci_lres->l1_err_cnt = 0;

    mutex_lock(&pst_pci_lres->pst_pci_res->st_rx_mem_lock);
    oal_pcie_change_link_state(pst_pci_lres->pst_pci_res, PCI_WLAN_LINK_DOWN);
    mutex_unlock(&pst_pci_lres->pst_pci_res->st_rx_mem_lock);

    oal_task_kill(&pst_pci_lres->st_rx_task);

    oal_pcie_disable_regions(pst_pci_lres->pst_pci_res);

    oal_pcie_deregister_event(pst_pci_lres);

    /* 下电之前关闭 PCIE HOST 控制器 */
    ret = oal_pcie_pm_control(pst_pci_lres->pst_pcie_dev,
                              oal_pcie_get_pcie_rc_idx(pst_pci_lres->pst_pcie_dev), power_option);

    /* 下电后可确保无硬件访问行为，再释放软件相关资源 */
    oal_pcie_transfer_res_exit(pst_pci_lres->pst_pci_res);
    if (old_power_status != PCIE_EP_IP_POWER_DOWN) {
        if (ret) {
            declare_dft_trace_key_info("pcie_power_down_fail", OAL_DFT_TRACE_EXCEP);
        }
    }

    oal_pcie_power_notifiy_register(pst_pci_lres->pst_pcie_dev,
                                    oal_pcie_get_pcie_rc_idx(pst_pci_lres->pst_pcie_dev), NULL, NULL, NULL);
#ifdef _PRE_PLAT_FEATURE_HI110X_DUAL_PCIE
    oal_ete_dual_pci_power_down(pst_pci_lres);
#endif
}

void pcie_bus_power_down_action(hcc_bus *pst_bus)
{
    /* 关掉LINKDOWN注册回调函数 */
    hcc_disable(hbus_to_hcc(pst_bus), OAL_TRUE);
    oal_wlan_gpio_intr_enable(hbus_to_dev(pst_bus), OAL_FALSE);
    hcc_transfer_lock(hbus_to_hcc(pst_bus));
    oal_pcie_power_down(pst_bus);
    hcc_transfer_unlock(hbus_to_hcc(pst_bus));
    board_host_wakeup_dev_set(0);
}

void pcie_bus_power_sleep_action(hcc_bus *pst_bus)
{
    oal_pcie_linux_res *pst_pci_lres = (oal_pcie_linux_res *)pst_bus->data;
    int32_t power_option = PCIE_POWER_OFF_WITH_L3MSG;
    int32_t ret;

    pst_pci_lres->power_status = PCIE_EP_IP_POWER_DOWN;
    pst_pci_lres->l1_err_cnt = 0;
    if (pst_pci_lres->pst_pci_res == NULL) {
        oal_print_mpxx_log(MPXX_LOG_ERR, "pst_pci_res is null");
        return;
    }

    mutex_lock(&pst_pci_lres->pst_pci_res->st_rx_mem_lock);
    oal_pcie_change_link_state(pst_pci_lres->pst_pci_res, PCI_WLAN_LINK_DEEPSLEEP);
    mutex_unlock(&pst_pci_lres->pst_pci_res->st_rx_mem_lock);

    oal_ete_transfer_deepsleep_clean(pst_pci_lres->pst_pci_res);
    oal_pcie_disable_regions(pst_pci_lres->pst_pci_res);
    oal_pcie_deregister_event(pst_pci_lres);
    /* 下电之前关闭 PCIE HOST 控制器 */
    ret = oal_pcie_pm_control(pst_pci_lres->pst_pcie_dev,
                              oal_pcie_get_pcie_rc_idx(pst_pci_lres->pst_pcie_dev), power_option);
    if (ret) {
        oal_print_mpxx_log(MPXX_LOG_ERR, "oal_pcie_pm_control fail %d", ret);
    }
}

int32_t oal_pcie_patch_read(hcc_bus *pst_bus, uint8_t *buff, int32_t len, uint32_t timeout, uint32_t file_flag)
{
    int32_t ret;
    oal_pcie_linux_res *pst_pci_lres = NULL;
    oal_pcie_res *pci_res = NULL;

    pst_pci_lres = (oal_pcie_linux_res *)pst_bus->data;
    if (oal_warn_on(pst_pci_lres == NULL)) {
        return -OAL_ENODEV;
    }

    if (oal_warn_on(pst_pci_lres->pst_pci_res == NULL)) {
        return -OAL_ENODEV;
    }

    pci_res = pst_pci_lres->pst_pci_res;
    mutex_lock(&pci_res->st_rx_mem_lock);
    /* 检查链路状态 */
    if (oal_pcie_check_link_state(pci_res) != OAL_TRUE) {
        mutex_unlock(&pci_res->st_rx_mem_lock);
        return -OAL_EFAUL;
    }

    /* 选择dma传输，走msg交互流程 */
    if (pci_res->chip_info.boot_cap.bits.dma_trans == OAL_TRUE) {
        ret = oal_pcie_firmware_msg_read(pst_pci_lres, buff, len, timeout, file_flag);
    } else {
        ret = oal_pcie_firmware_read(pst_pci_lres, buff, len, timeout);
    }
    /* 命令执行完成后再次检查，防止通道异常造成命令执行失败 */
    if (oal_pcie_check_link_state(pci_res) != OAL_TRUE) {
        mutex_unlock(&pci_res->st_rx_mem_lock);
        return -OAL_EFAUL;
    }
    mutex_unlock(&pci_res->st_rx_mem_lock);

    return ret;
}

int32_t oal_pcie_patch_write(hcc_bus *pst_bus, uint8_t *buff, int32_t len, uint32_t file_flag)
{
    int32_t ret;
    oal_pcie_linux_res *pst_pci_lres = NULL;
    oal_pcie_res *pci_res = NULL;

    pst_pci_lres = (oal_pcie_linux_res *)pst_bus->data;
    if (oal_warn_on(pst_pci_lres == NULL)) {
        oal_io_print("oal_pcie_patch_write failed, lres is null\n");
        return -OAL_ENODEV;
    }

    if (oal_warn_on(pst_pci_lres->pst_pci_res == NULL)) {
        return -OAL_ENODEV;
    }

    pci_res = pst_pci_lres->pst_pci_res;
    mutex_lock(&pci_res->st_rx_mem_lock);
    /* 检查链路状态 */
    if (oal_pcie_check_link_state(pci_res) != OAL_TRUE) {
        mutex_unlock(&pci_res->st_rx_mem_lock);
        return -OAL_EFAUL;
    }

    /* 选择dma传输，走msg交互流程 */
    if (pci_res->chip_info.boot_cap.bits.dma_trans == OAL_TRUE) {
        ret = oal_pcie_firmware_msg_write(pst_pci_lres, buff, len, file_flag);
    } else {
        ret = oal_pcie_firmware_write(pst_pci_lres, buff, len);
    }

    /* 命令执行完成后再次检查，防止通道异常造成命令执行失败 */
    if (oal_pcie_check_link_state(pci_res) != OAL_TRUE) {
        mutex_unlock(&pci_res->st_rx_mem_lock);
        return -OAL_EFAUL;
    }
    mutex_unlock(&pci_res->st_rx_mem_lock);

    return ret;
}

int32_t oal_pcie_gpio_irq(hcc_bus *hi_bus, int32_t irq)
{
    oal_uint ul_state;
    oal_pcie_linux_res *pst_pci_lres = NULL;
#ifdef _PRE_DUAL_CHIP
    /* 双芯片不允许在中断里通过hcc_get_main_dev获取当前dev id */
    struct wlan_pm_s *pst_wlan_pm = wlan_pm_get_drv_by_id(hi_bus->dev_id);
#else
    struct wlan_pm_s *pst_wlan_pm = wlan_pm_get_drv();
#endif

    if (oal_unlikely(hi_bus == NULL)) {
        oal_io_print("pcie bus is null, irq:%d\n", irq);
        return -OAL_EINVAL;
    }

    pst_pci_lres = (oal_pcie_linux_res *)hi_bus->data;

    if (!hi_bus->pst_pm_callback || !hi_bus->pst_pm_callback->pm_state_get) {
        oal_io_print("GPIO interrupt function param is NULL\r\n");
        return -OAL_EINVAL;
    }

    hi_bus->gpio_int_count++;

    if (oal_atomic_read(&hi_bus->wakeup_dev_wait_ack)) {
        pci_print_log(PCI_LOG_INFO, "pcie_gpio_irq wakeup dev ack");
        hi_bus->pst_pm_callback->pm_wakeup_dev_ack();
    }

    if (hcc_bus_read_powerup_dev_wait_ack(hi_bus)) {
        pci_print_log(PCI_LOG_INFO, "pcie_gpio_irq powerup dev ack");
        if (oal_likely(pst_wlan_pm != NULL)) {
            oal_complete(&pst_wlan_pm->st_wifi_powerup_done);
        } else {
            pci_print_log(PCI_LOG_INFO, "pst_wlan_pm is null");
        }
    }

    if (oal_unlikely(pst_pci_lres != NULL)) {
        if (oal_atomic_read(&pst_pci_lres->st_pcie_wakeup_flag)) {
            pci_print_log(PCI_LOG_INFO, "pcie_gpio_irq dev ready ack");
            if (oal_likely(pst_wlan_pm != NULL)) {
                oal_complete(&pst_wlan_pm->st_wifi_powerup_done);
                up(&hi_bus->rx_sema);
            } else {
                pci_print_log(PCI_LOG_INFO, "pst_wlan_pm is null");
            }
        }
    }

    ul_state = hi_bus->pst_pm_callback->pm_state_get();
    if (ul_state == 0) {
        /* 0==HOST_DISALLOW_TO_SLEEP表示不允许休眠 */
        hi_bus->data_int_count++;
        /* PCIE message use gpio interrupt */
        pci_print_log(PCI_LOG_DBG, "pcie message come..");
        up(&hi_bus->rx_sema);
    } else {
        /* 1==HOST_ALLOW_TO_SLEEP表示当前是休眠，唤醒host */
        if (oal_warn_on(!hi_bus->pst_pm_callback->pm_wakeup_host)) {
            pci_print_log(PCI_LOG_ERR, "%s error:hi_bus->pst_pm_callback->pm_wakeup_host is null", __FUNCTION__);
            return -OAL_FAIL;
        }

        hi_bus->wakeup_int_count++;
        pci_print_log(PCI_LOG_INFO, "trigger wakeup work...");
        hi_bus->pst_pm_callback->pm_wakeup_host();
    }
    return OAL_SUCC;
}

int32_t oal_pcie_gpio_rx_data(hcc_bus *hi_bus)
{
    int32_t ret;
    uint32_t message = 0;
    oal_pcie_linux_res *pst_pci_lres = (oal_pcie_linux_res *)hi_bus->data;
    if (oal_warn_on(pst_pci_lres == NULL)) {
        return -OAL_EINVAL;
    }

    /* read message from device, read until ringbuf empty */
    forever_loop() {
        if (!pst_pci_lres->pst_pci_res->regions.inited) {
            oal_io_print("pcie rx data region is disabled!\n");
            return -OAL_ENODEV;
        }

        /* read one message */
        hcc_bus_wake_lock(hi_bus);
        mutex_lock(&pst_pci_lres->pst_pci_res->st_rx_mem_lock);
        ret = oal_pcie_read_d2h_message(pst_pci_lres->pst_pci_res, &message);
        mutex_unlock(&pst_pci_lres->pst_pci_res->st_rx_mem_lock);
        hcc_bus_wake_unlock(hi_bus);
        if (ret != OAL_SUCC) {
            break;
        }

        pci_print_log(PCI_LOG_DBG, "pci  message : %u", message);
        /* get message succ. */
        if (message < D2H_MSG_COUNT) {
            /* standard message */
            hi_bus->msg[message].count++;
            hi_bus->last_msg = message;
            hi_bus->msg[message].cpu_time = cpu_clock(UINT_MAX);
            if (hi_bus->msg[message].msg_rx) {
                hi_bus->msg[message].msg_rx(hi_bus->msg[message].data);
            } else {
                oal_io_print("pcie mssage :%u no callback functions\n", message);
            }
        } else {
            if (message == (uint32_t)PCIE_D2H_MESSAGE_HOST_READY_ACK) {
                pci_print_log(PCI_LOG_INFO, "d2h host read ack");
                oal_complete(&pst_pci_lres->st_pcie_ready);
            } else if (message == (uint32_t)PCIE_D2H_MESSAGE_ETE_INTR_ERR) {
                pci_print_log(PCI_LOG_ERR, "dev maybe occur err ete int");
                hcc_bus_print_trans_info(hi_bus, HCC_PRINT_TRANS_FLAG_DEVICE_REGS |
                                                 HCC_PRINT_TRANS_FLAG_DEVICE_STAT);
                hcc_bus_exception_submit(pst_pci_lres->pst_bus, HCC_BUS_TRANS_FAIL);
            }
        }
    }

    return OAL_SUCC;
}

void oal_pcie_print_trans_info(hcc_bus *hi_bus, uint64_t print_flag)
{
    oal_pcie_linux_res *pst_pci_lres = (oal_pcie_linux_res *)hi_bus->data;
    if (oal_warn_on(pst_pci_lres == NULL)) {
        return;
    }

    oal_pcie_print_transfer_info(pst_pci_lres->pst_pci_res, print_flag);
}

void oal_pcie_reset_trans_info(hcc_bus *hi_bus)
{
    oal_pcie_linux_res *pst_pci_lres = (oal_pcie_linux_res *)hi_bus->data;
    if (oal_warn_on(pst_pci_lres == NULL)) {
        return;
    }

    oal_pcie_reset_transfer_info(pst_pci_lres->pst_pci_res);
}

OAL_STATIC hcc_bus *oal_pcie_bus_init(oal_pcie_linux_res *pst_pci_lres)
{
    int32_t ret;
    hcc_bus *pst_bus = NULL;

    pst_bus = hcc_alloc_bus();
    if (pst_bus == NULL) {
        oal_io_print("alloc pcie hcc bus failed, size:%u\n", (uint32_t)sizeof(hcc_bus));
        return NULL;
    }

    pst_bus->bus_type = HCC_BUS_PCIE;
    pst_bus->bus_id = 0x0;
    pst_bus->dev_id = HCC_EP_WIFI_DEV; /* 这里可以根据 vendor id 区分110X 和118X */

    /* PCIE 只需要4字节对齐, burst大小对性能的影响有限 */
    pst_bus->cap.align_size[HCC_TX] = (uint32_t)sizeof(uint32_t);
    pst_bus->cap.align_size[HCC_RX] = (uint32_t)sizeof(uint32_t);
    pst_bus->cap.max_trans_size = 0x7fffffff;

    if (pst_pci_lres->pst_pci_res->chip_info.edma_support == OAL_TRUE) {
        oal_pcie_edma_bus_ops_init(pst_bus);
    } else if (pst_pci_lres->pst_pci_res->chip_info.ete_support == OAL_TRUE) {
        oal_pcie_ete_bus_ops_init(pst_bus);
    } else {
        pst_bus->opt_ops = NULL;
        oal_print_mpxx_log(MPXX_LOG_ERR, "unsupport chip, bus init failed!\n");
        hcc_free_bus(pst_bus);
        return NULL;
    }

#ifdef _PRE_DUAL_CHIP
    oal_pcie_bus_init_dual_mpxx(pst_bus);
#endif

    pst_bus->data = (void *)pst_pci_lres;

    ret = hcc_add_bus(pst_bus, "pcie");
    if (ret) {
        oal_io_print("add pcie bus failed, ret=%d\n", ret);
        hcc_free_bus(pst_bus);
        return NULL;
    }

    pst_pci_lres->pst_bus = pst_bus;
    return pst_bus;
}

OAL_STATIC void oal_pcie_bus_exit(hcc_bus *pst_bus)
{
    hcc_remove_bus(pst_bus);
    hcc_free_bus(pst_bus);
}

OAL_STATIC int32_t wlan_first_power_on_callback(void *data)
{
    oal_reference(data);
    /* before wlan chip power up, rc clock must on. */
    /* bootloader had delay 20us before reset pcie, soc requet 10us delay, need't delay here */
    hi_wlan_power_set(1);
    /* do something before pcie probe */
    pcie_linkup_prepare_fixup(0);
    return 0;
}

OAL_STATIC int32_t wlan_first_power_off_fail_callback(void *data)
{
    oal_reference(data);
#if defined(CONFIG_HUAWEI_OHOS_DSM) || defined(CONFIG_HUAWEI_DSM)
    hw_mpxx_dsm_client_notify(SYSTEM_TYPE_PLATFORM, DSM_PCIE_ENMU_FAIL, "pcie enmu fail\n");
#endif
    oal_io_print("wlan_first_power_off_fail_callback\n");
    pcie_linkup_fail_fixup(0);
    (void)ssi_dump_err_regs(SSI_ERR_PCIE_FST_POWER_OFF_FAIL);

    return -1;
}

OAL_STATIC int32_t wlan_try_power_off_fail_callback(void *data)
{
    oal_reference(data);
    g_pcie_enum_fail_reg_dump_flag = 1;
    pcie_linkup_fail_fixup(0);
    /* 阻止麒麟枚举失败后下电关时钟 */
    oal_chip_error_task_block();
    return -1;
}

OAL_STATIC int32_t wlan_first_power_off_callback(void *data)
{
    oal_reference(data);
    hi_wlan_power_set(0);
    return 0;
}

#ifdef _PRE_PLAT_FEATURE_HI110X_DUAL_PCIE
int32_t oal_pcie_dual_power_on_callback(void *data)
{
    oal_reference(data);
    oal_io_print("board_set_pcie_reset\n");
    board_set_pcie_reset(OAL_FALSE, OAL_TRUE);
    mdelay(10); /* 等待10ms，待拉gpio后状态稳定后使能phy tracer */
    pcie_linkup_prepare_fixup(1);
    return 0;
}

OAL_STATIC int32_t oal_pcie_dual_power_off_fail_callback(void *data)
{
    oal_reference(data);
    /* 阻止麒麟枚举失败后下电关时钟 */
    oal_io_print("oal_pcie_dual_power_off_fail_callback\n");
    g_pcie_enum_fail_reg_dump_flag = 1;
    pcie_linkup_fail_fixup(1);
    (void)ssi_dump_err_regs(SSI_ERR_PCIE_FST_POWER_OFF_FAIL);
    oal_chip_error_task_block();
    return -1;
}

OAL_STATIC int32_t wlan_dual_first_power_off_callback(void *data)
{
    oal_reference(data);
    board_set_pcie_reset(OAL_FALSE, OAL_FALSE);
    return 0;
}
#endif

OAL_STATIC void oal_pcie_device_auxclk_switch_init(void)
{
    int32_t cfg_value = 0;
    int32_t ret;

    /* 获取ini的配置值 */
    ret = get_cust_conf_int32(INI_MODU_PLAT, "pcie_device_auxclk_switch", &cfg_value);
    if (ret == INI_FAILED) {
        return;
    }

    if (cfg_value != 0) {
        oal_pcie_device_auxclk_switch_set(OAL_TRUE);
    } else {
        oal_pcie_device_auxclk_switch_set(OAL_FALSE);
    }

    pci_print_log(PCI_LOG_INFO, "pcie_device_auxclk_switch = %d", oal_pcie_device_auxclk_switch_get());
}

OAL_STATIC void oal_pcie_scatt_list_assemble_init(void)
{
    int32_t cfg_value = 0;
    int32_t ret;

    /* 获取ini的配置值 */
    ret = get_cust_conf_int32(INI_MODU_PLAT, "pcie_scatt_list_assemble", &cfg_value);
    if (ret == INI_FAILED) {
        return;
    }

    if (cfg_value != 0) {
        pcie_scatt_list_assemble = OAL_TRUE;
    } else {
        pcie_scatt_list_assemble = OAL_FALSE;
    }

    pci_print_log(PCI_LOG_INFO, "pcie_scatt_list_assemble = %u", pcie_scatt_list_assemble);
}

OAL_STATIC void oal_pcie_idle_intx_bind_init(void)
{
    int32_t cfg_value = 0;
    int32_t ret;

    /* 获取ini的配置值 */
    ret = get_cust_conf_int32(INI_MODU_PLAT, "pcie_intx_idle_bindcpu", &cfg_value);
    if (ret == INI_FAILED) {
        pci_print_log(PCI_LOG_INFO, "pcie_intx_idle_bindcpu not set");
        return;
    }

    if ((cfg_value < 0) || (cfg_value >= (int32_t)OAL_BUS_MAXCPU_NUM)) {  /* 0:cpu0 */
        pci_print_log(PCI_LOG_ERR, "invalid cpu[%d]\n", cfg_value);
        return;
    }
    g_pcie_intx_idle_bind_cpu = (uint32_t)cfg_value;
    pci_print_log(PCI_LOG_INFO, " pcie_intx_idle_bindcpu[%u]", g_pcie_intx_idle_bind_cpu);
}

OAL_STATIC void oal_pcie_speed_change_init(void)
{
    int32_t l_cfg_value = 0;
    int32_t l_ret;

    /* 获取ini的配置值 */
    l_ret = get_cust_conf_int32(INI_MODU_PLAT, "pcie_speed_change", &l_cfg_value);
    if (l_ret == INI_FAILED) {
        pci_print_log(PCI_LOG_INFO, "pcie_speed_change not set, no link speed change\n");
        return;
    }

    if ((g_pcie_speed_change < PCIE_GEN1) || (g_pcie_speed_change >= PCIE_GEN_MAX)) {
        pci_print_log(PCI_LOG_ERR, "pcie_speed_change target:%d invalid, no link speed change\n", g_pcie_speed_change);
        g_pcie_speed_change = 0;
        return;
    }

    g_pcie_speed_change = (uint16_t)l_cfg_value;
    pci_print_log(PCI_LOG_INFO, " pcie_speed_change target:%d[0:GEN1 1:GEN2 2:GEN3]", g_pcie_speed_change);
    return;
}

OAL_STATIC void oal_pcie_mac_bypass_init(void)
{
    int32_t l_cfg_value = 0;
    int32_t l_ret;

    /* 获取ini的配置值 */
    l_ret = get_cust_conf_int32(INI_MODU_PLAT, "pcie_mac_cfg_bypass", &l_cfg_value);
    if (l_ret == INI_FAILED) {
        pci_print_log(PCI_LOG_INFO, "pcie_mac_cfg_bypass not set.\n");
        return;
    }

    g_pcie_mac_cfg_bypass = (uint16_t)l_cfg_value;
    pci_print_log(PCI_LOG_INFO, " pcie_mac_cfg = 0x%x", g_pcie_mac_cfg_bypass);
    return;
}

OAL_STATIC void oal_pcie_ringbuf_bugfix_init(void)
{
    char buff[100]; /* 100字节足够存放要读取的参数 */

    memset_s(buff, sizeof(buff), 0, sizeof(buff));

    if (get_cust_conf_string(INI_MODU_PLAT, "pcie_ringbuf_bugfix", buff, sizeof(buff) - 1) == INI_FAILED) {
        pci_print_log(PCI_LOG_INFO, "can't found ini:pcie_ringbuf_bugfix, bugfix stat:%d",
                      g_pcie_ringbuf_bugfix_enable);
        return;
    }

    if (!oal_strncmp("enable", buff, OAL_STRLEN("enable"))) {
        g_pcie_ringbuf_bugfix_enable = 1;
    } else if (!strncmp("disable", buff, OAL_STRLEN("disable"))) {
        g_pcie_ringbuf_bugfix_enable = 0;
    } else {
        pci_print_log(PCI_LOG_WARN, "unkdown ringbuf bugfix ini:%s", buff);
    }

    pci_print_log(PCI_LOG_INFO, "ringbuf bugfix %s", g_pcie_ringbuf_bugfix_enable ? "enable" : "disable");
}

OAL_STATIC void oal_pcie_device_sleep_linkdown_init(void)
{
    int32_t cfg_value = 0;
    int32_t ret;

    /* 获取ini的配置值 */
    ret = get_cust_conf_int32(INI_MODU_PLAT, "pcie_pm_linkdown_switch", &cfg_value);
    if (ret == INI_FAILED) {
        return;
    }

    if (cfg_value != 0) {
        g_pcie_pm_linkdown_switch = OAL_TRUE;
    } else {
        g_pcie_pm_linkdown_switch = OAL_FALSE;
    }

    pci_print_log(PCI_LOG_INFO, "g_pcie_pm_linkdown_switch = %u", g_pcie_pm_linkdown_switch);
}

OAL_STATIC void oal_pcie_device_wakeup_delay_init(void)
{
    int32_t cfg_value = 0;
    int32_t ret;

    /* 获取ini的配置值 */
    ret = get_cust_conf_int32(INI_MODU_PLAT, "wlan_wakeup_delay_enable", &cfg_value);
    if (ret == INI_FAILED) {
        return;
    }

    if (cfg_value != 0) {
        g_wlan_wakeup_delay_enable = OAL_TRUE;
    } else {
        g_wlan_wakeup_delay_enable = OAL_FALSE;
    }

    pci_print_log(PCI_LOG_INFO, "g_wlan_wakeup_delay_enable = %u", g_wlan_wakeup_delay_enable);
}

#ifdef _PRE_PRODUCT_HI1620S_KUNPENG
OAL_STATIC int32_t oal_pcie_dts_init(void)
{
    return OAL_SUCC;
}
#else

OAL_STATIC int32_t oal_pcie_dts_init(void)
{
#ifndef _PRE_HI375X_PCIE
#ifdef _PRE_CONFIG_USE_DTS
    int ret;
    u32 pcie_rx_idx = 0;
    struct device_node *np = NULL;
    np = of_find_compatible_node(NULL, NULL, DTS_NODE_MPXX_WIFI);
    if (np == NULL) {
        oal_print_mpxx_log(MPXX_LOG_ERR, "can't find node [%s]", DTS_NODE_MPXX_WIFI);
        return -OAL_ENODEV;
    }

    ret = of_property_read_u32(np, DTS_PROP_MPXX_PCIE_RC_IDX, &pcie_rx_idx);
    if (ret) {
        oal_print_mpxx_log(MPXX_LOG_INFO, "read prop [%s] fail, ret=%d", DTS_PROP_MPXX_PCIE_RC_IDX, ret);
        return ret;
    }

    oal_print_mpxx_log(MPXX_LOG_INFO, "%s=%d", DTS_PROP_MPXX_PCIE_RC_IDX, pcie_rx_idx);

    g_pcie_rc_idx = (int32_t)pcie_rx_idx;
#endif
#endif
    return OAL_SUCC;
}
#endif


OAL_STATIC int32_t oal_pci_resource_init(void)
{
    int32_t ret;
    oal_pcie_linux_res *pst_pci_lres = NULL;

#ifdef _PRE_DUAL_CHIP
    return oal_pci_resource_init_dual_mpxx();
#endif
    /* Power off Wlan Chip */
    if (g_pcie_linux_res) {
        hcc_bus_disable_state(g_pcie_linux_res->pst_bus, OAL_BUS_STATE_ALL);
        pst_pci_lres = (oal_pcie_linux_res *)g_pcie_linux_res->pst_bus->data;

        /* 保存PCIE 配置寄存器 */
        ret = oal_pcie_save_default_resource(g_pcie_linux_res);
        if (ret != OAL_SUCC) {
            oal_pcie_disable_device(g_pcie_linux_res);
#ifdef _PRE_CONFIG_PCIE_SHARED_INTX_IRQ
            oal_disable_pcie_irq_with_free(g_pcie_linux_res);
#else
            oal_disable_pcie_irq(g_pcie_linux_res);
#endif
            oal_pci_unregister_driver(&g_pcie_drv);
            hi_wlan_power_set(0);
            oal_pcie_power_notifiy_register(pst_pci_lres->pst_pcie_dev,
                                            oal_pcie_get_pcie_rc_idx(pst_pci_lres->pst_pcie_dev), NULL, NULL, NULL);
            return ret;
        }

        oal_pcie_power_notifiy_register(pst_pci_lres->pst_pcie_dev,
                                        oal_pcie_get_pcie_rc_idx(pst_pci_lres->pst_pcie_dev),
                                        NULL, wlan_first_power_off_callback, NULL);
#ifdef _PRE_CONFIG_PCIE_SHARED_INTX_IRQ
        oal_disable_pcie_irq_with_free(g_pcie_linux_res);
#else
        oal_disable_pcie_irq(g_pcie_linux_res);
#endif
        hcc_bus_power_action(g_pcie_linux_res->pst_bus, HCC_BUS_POWER_DOWN);
        hi_wlan_power_set(0);
    } else {
        oal_io_print("g_pcie_linux_res is null\n");
        oal_pcie_power_notifiy_register(NULL, g_pcie_rc_idx, NULL, NULL, NULL);
        return -OAL_ENODEV;
    }
    return OAL_SUCC;
}

OAL_STATIC void oal_pcie_mpxx_enum_fail_proc(void)
{
    if (!g_pcie_enum_fail_reg_dump_flag) {
        (void)ssi_dump_err_regs(SSI_ERR_PCIE_ENUM_FAIL);
    }
}

OAL_STATIC int32_t oal_wait_pcie_probe_complete(void)
{
    int32_t ret = OAL_SUCC;
    unsigned int timeout = hi110x_get_emu_timeout(10 * HZ); // 10 sec
    if (oal_wait_for_completion_timeout(&g_probe_complete, (uint32_t)timeout)) {
        if (g_probe_ret != OAL_SUCC) {
            oal_io_print("pcie driver probe failed ret=%d, driname:%s\n", g_probe_ret, g_pcie_drv.name);
#ifdef CONFIG_HWCONNECTIVITY_PC
            ssi_dump_device_regs(SSI_MODULE_MASK_COMM | SSI_MODULE_MASK_PCIE_CFG | SSI_MODULE_MASK_PCIE_DBI |
                SSI_MODULE_MASK_WCTRL | SSI_MODULE_MASK_BCTRL);
            chr_exception_report(CHR_PLATFORM_EXCEPTION_EVENTID, CHR_SYSTEM_PLAT, CHR_LAYER_DRV,
                CHR_PLT_DRV_EVENT_INIT, CHR_PLAT_DRV_ERROR_PCIE_PROBE_FAIL);
#endif
            oal_pcie_power_notifiy_register(NULL, g_pcie_rc_idx, NULL, NULL, NULL);
            return g_probe_ret;
        }
    } else {
        oal_io_print("pcie driver probe timeout  driname:%s\n", g_pcie_drv.name);
        oal_pcie_mpxx_enum_fail_proc();
#ifdef CONFIG_HWCONNECTIVITY_PC
        chr_exception_report(CHR_PLATFORM_EXCEPTION_EVENTID, CHR_SYSTEM_PLAT, CHR_LAYER_DRV,
            CHR_PLT_DRV_EVENT_INIT, CHR_PLAT_DRV_ERROR_PCIE_PROBE_TIMEOUT);
#endif
        oal_pcie_power_notifiy_register(NULL, g_pcie_rc_idx, NULL, NULL, NULL);
        return -OAL_ETIMEDOUT;
    }
    return ret;
}

#ifdef _PRE_PLAT_FEATURE_HI110X_DUAL_PCIE
int32_t oal_pcie_mpxx_dual_init(void)
{
    int32_t ret;

    if (g_dual_pci_support == OAL_FALSE) {
        oal_io_print("dual pcie not support\n");
        return OAL_SUCC;
    }

    g_probe_ret = -OAL_EFAUL;
    g_pcie_enum_fail_reg_dump_flag = 0;
    oal_init_completion(&g_probe_complete);
    oal_io_print("%s PCIe driver dual device start\n", g_pcie_drv.name);

    if (g_pcie_linux_res == NULL || g_pcie_linux_res->pst_pcie_dev_dual == NULL) {
        oal_io_print("no dual pcie device\n");
        return -OAL_ENODEV;
    }
    /* 打开参考时钟 */
    ret = oal_pcie_power_notifiy_register(g_pcie_linux_res->pst_pcie_dev_dual, g_pcie_rc_idx_dual,
                                          oal_pcie_dual_power_on_callback,
                                          oal_pcie_dual_power_off_fail_callback, NULL);
    if (ret != OAL_SUCC) {
        oal_io_print("enumerate_prepare failed ret=%d\n", ret);
        return ret;
    }

    ret = oal_pcie_enumerate(g_pcie_rc_idx_dual);
    if (ret != OAL_SUCC) {
        oal_io_print("enumerate failed ret=%d\n", ret);
        kirin_pcie_power_notifiy_register(g_pcie_rc_idx_dual, NULL, NULL, NULL);
        return ret;
    }

    /* poweroff */
    oal_pcie_power_notifiy_register(g_pcie_linux_res->pst_pcie_dev_dual, g_pcie_rc_idx_dual, NULL,
                                    wlan_dual_first_power_off_callback, NULL);
    oal_pcie_pm_control(g_pcie_linux_res->pst_pcie_dev_dual, g_pcie_rc_idx_dual, PCIE_POWER_OFF_WITHOUT_L3MSG);

    return OAL_SUCC;
}
#endif

static void oal_pcie_ini_parameters_init(void)
{
    oal_pcie_voltage_bias_param_init();
    oal_pcie_ringbuf_bugfix_init();
    oal_pcie_speed_change_init();
    oal_pcie_mac_bypass_init();
    oal_pcie_idle_intx_bind_init();
    oal_pcie_phy_ini_config_init();
    oal_pcie_scatt_list_assemble_init();
    oal_pcie_device_auxclk_switch_init();
    oal_pcie_device_sleep_linkdown_init();
    oal_pcie_device_wakeup_delay_init();
}

static void oal_pcie_mpxx_pre_init(void)
{
    /* init host pcie */
    oal_io_print("%s PCIe driver register start\n", g_pcie_drv.name); /* Debug */
    oal_init_completion(&g_probe_complete);

    g_probe_ret = -OAL_EFAUL;

    g_pcie_enum_fail_reg_dump_flag = 0;

    oal_pcie_dts_init();

    oal_pcie_ini_parameters_init();

    oal_pcie_board_init();
#ifdef _PRE_DUAL_CHIP
    oal_pcie_dual_mpxx_init();
#endif
}

int32_t oal_pcie_mpxx_enum(void)
{
    int32_t ret;
#ifdef _PRE_DUAL_CHIP
    if (hcc_get_bus_dev_num() > NUM_1_BYTES) {
        return oal_pcie_enum_dual_mpxx();
    }
#endif
    /* 打开参考时钟 */
    ret = oal_pcie_power_notifiy_register(NULL, g_pcie_rc_idx, wlan_first_power_on_callback,
                                          wlan_first_power_off_fail_callback, NULL);
    if (ret != OAL_SUCC) {
        oal_io_print("enumerate first power register failed ret=%d\n", ret);
        return ret;
    }

    ret = oal_pcie_enumerate(g_pcie_rc_idx);
    if (ret == OAL_SUCC) {
        oal_io_print("pcie enumerate succ\n");
        return ret;
    }

    chr_exception_report(CHR_PLATFORM_EXCEPTION_EVENTID, CHR_SYSTEM_PLAT, CHR_LAYER_DRV,
                         CHR_PLT_DRV_EVENT_PCIE, CHR_PLAT_DRV_ERROR_PCIE_ENUM_FAIL);
    hi_wlan_power_set(0);
    msleep(1000); // 此时硬件状态可能有异常，预留1000ms 给硬件电源恢复再尝试
    // 尝试第二次枚举
    ret = oal_pcie_power_notifiy_register(NULL, g_pcie_rc_idx, wlan_first_power_on_callback,
                                          wlan_try_power_off_fail_callback, NULL);
    if (ret != OAL_SUCC) {
        oal_io_print("enumerate try power register failed ret=%d\n", ret);
        return ret;
    }

    ret = oal_pcie_enumerate(g_pcie_rc_idx);
    if (ret == OAL_SUCC) {
        chr_exception_report(CHR_PLATFORM_EXCEPTION_EVENTID, CHR_SYSTEM_PLAT, CHR_LAYER_DRV,
                             CHR_PLT_DRV_EVENT_PCIE, CHR_PLAT_DRV_ERROR_PCIE_ENUM_TRY_SUCC);
        oal_io_print("pcie try enum succ\n");
        return ret;
    }

    oal_pcie_mpxx_enum_fail_proc();
    oal_io_print("enumerate try failed ret=%d\n", ret);
    oal_pcie_power_notifiy_register(NULL, g_pcie_rc_idx, NULL, NULL, NULL);
    return ret;
}

/* trigger pcie probe, alloc pcie resource */
int32_t oal_pcie_mpxx_init(void)
{
    int32_t ret;

    oal_pcie_mpxx_pre_init();

    ret = oal_pci_register_driver(&g_pcie_drv);
    if (ret) {
        oal_io_print("pcie driver register failed ret=%d, driname:%s\n", ret, g_pcie_drv.name);
        return ret;
    }

    ret = oal_pcie_mpxx_enum();
    if (ret != OAL_SUCC) {
        oal_io_print("kirin pcie enum fail\n");
        oal_pci_unregister_driver(&g_pcie_drv);
        return ret;
    }

    ret = oal_wait_pcie_probe_complete();
    if (ret != OAL_SUCC) {
        return ret;
    }
#ifdef _PRE_PLAT_FEATURE_HI110X_DUAL_PCIE
    ret = oal_pcie_mpxx_dual_init();
    if (ret != OAL_SUCC) {
        return ret;
    }
#endif

    ret = oal_pci_resource_init();
    if (ret != OAL_SUCC) {
        oal_io_print("%s PCIe driver register not succ\n", g_pcie_drv.name);
#ifdef CONFIG_HWCONNECTIVITY_PC
        chr_exception_report(CHR_PLATFORM_EXCEPTION_EVENTID, CHR_SYSTEM_PLAT, CHR_LAYER_DRV,
            CHR_PLT_DRV_EVENT_INIT, CHR_PLAT_DRV_ERROR_PCIE_DRV_REG_FAIL);
#endif
        return ret;
    }

    oal_io_print("%s PCIe driver register succ\n", g_pcie_drv.name);
    return OAL_SUCC;
}

void oal_pcie_mpxx_exit(void)
{
    oal_pcie_power_notifiy_register(NULL, g_pcie_rc_idx, NULL, NULL, NULL);
    oal_pci_unregister_driver(&g_pcie_drv);
}
#endif

int32_t oal_pcie_mpxx_working_check(void)
{
    hcc_bus_dev *pst_bus_dev;
    pst_bus_dev = hcc_get_bus_dev(HCC_EP_WIFI_DEV);
    if (pst_bus_dev == NULL) {
        return OAL_FALSE;
    }

    if (pst_bus_dev->bus_cap & HCC_BUS_PCIE_CAP) {
        return OAL_TRUE;
    } else {
        return OAL_FALSE;
    }
}

int32_t oal_wifi_platform_load_pcie(void)
{
    int32_t ret = OAL_SUCC;
    int32_t debug_value = OAL_FALSE;

    if (oal_pcie_mpxx_working_check() != OAL_TRUE) {
        /* pcie driver don't support */
        oal_io_print("pcie driver don't support\n");
        return OAL_SUCC;
    }

    /* WiFi 芯片上电 + PCIE 枚举 */
#ifdef _PRE_PLAT_FEATURE_HI110X_PCIE
    ret = oal_pcie_mpxx_init();
#endif
    if (ret == OAL_SUCC) {
        pcie_chip_rc_slt_register();
    } else {
        if (get_cust_conf_int32(INI_MODU_PLAT, INI_PCIE_DEBUG_MODE, &debug_value) != INI_SUCC) {
            oal_print_mpxx_log(MPXX_LOG_INFO, "ini is not config pcie_debug_mode\n");
        }
        if (debug_value != OAL_TRUE) {
            hi_wlan_power_set(0);
        }
    }
    return ret;
}

void oal_wifi_platform_unload_pcie(void)
{
    if (oal_pcie_mpxx_working_check() != OAL_TRUE) {
        return;
    }

    pcie_chip_rc_slt_unregister();

#ifdef _PRE_PLAT_FEATURE_HI110X_PCIE
    oal_pcie_mpxx_exit();
#endif
}

#ifdef _PRE_PLAT_FEATURE_HI110X_PCIE
oal_pcie_linux_res *oal_get_pcie_linux_res(void)
{
    return g_pcie_linux_res;
}
#else
oal_pcie_linux_res *oal_get_pcie_linux_res(void)
{
    return NULL;
}
#endif
oal_module_symbol(oal_get_pcie_linux_res);

#ifdef _PRE_PLAT_FEATURE_HI110X_PCIE
oal_pcie_linux_res *oal_get_pcie_linux_res_by_id(uint32_t dev_id)
{
#ifdef _PRE_DUAL_CHIP
    return g_pcie_linux_res_tab[dev_id];
#else
    return g_pcie_linux_res;
#endif
}
#else
oal_pcie_linux_res *oal_get_pcie_linux_res_by_id(uint32_t dev_id)
{
    return NULL;
}
#endif
oal_module_symbol(oal_get_pcie_linux_res_by_id);

#ifdef _PRE_PLAT_FEATURE_HI110X_PCIE
oal_pci_dev_stru *oal_get_wifi_pcie_dev(void)
{
    oal_pcie_linux_res *pcie_linux_res = oal_get_pcie_linux_res();
#ifdef _PRE_DUAL_CHIP
    pcie_linux_res = oal_get_pcie_linux_res_by_id(HCC_EP_WIFI_DEV);
#endif
    if (pcie_linux_res == NULL) {
        return NULL;
    }
    return pcie_linux_res->pst_pcie_dev;
}
#else
oal_pci_dev_stru *oal_get_wifi_pcie_dev(void)
{
    return NULL;
}
#endif
oal_module_symbol(oal_get_wifi_pcie_dev);

#ifdef _PRE_PLAT_FEATURE_HI110X_PCIE
oal_pci_dev_stru *oal_get_wifi_pcie_dev_by_id(uint32_t dev_id)
{
    oal_pcie_linux_res *pcie_linux_res = oal_get_pcie_linux_res_by_id(dev_id);
    if (pcie_linux_res == NULL) {
        return NULL;
    }
    return pcie_linux_res->pst_pcie_dev;
}
#else
oal_pci_dev_stru *oal_get_wifi_pcie_dev_by_id(uint32_t dev_id)
{
    return NULL;
}
#endif
oal_module_symbol(oal_get_wifi_pcie_dev_by_id);

// for dual chip
#ifdef _PRE_DUAL_CHIP
OAL_STATIC int32_t wlan_first_power_on_callback_dual_mpxx(void *data)
{
    /* before wlan chip power up, rc clock must on. */
    /* bootloader had delay 20us before reset pcie, soc requet 10us delay, need't delay here */
    uint32_t dev_id = (uint32_t)(uintptr_t)(data);
    oal_io_print("wlan_first_power_on_callback_dual_mpxx::[%d]\n", dev_id);
    hi_wlan_power_set_by_id(dev_id, 1);
    /* do something before pcie probe */
    pcie_linkup_prepare_fixup(dev_id);
    return 0;
}

OAL_STATIC int32_t wlan_first_power_off_fail_callback_dual_mpxx(void *data)
{
    uint32_t dev_id = (uint32_t)(uintptr_t)(data);
#if defined(CONFIG_HUAWEI_OHOS_DSM) || defined(CONFIG_HUAWEI_DSM)
    hw_mpxx_dsm_client_notify(SYSTEM_TYPE_PLATFORM, DSM_PCIE_ENMU_FAIL, "pcie enmu fail\n");
#endif
    oal_io_print("wlan_first_power_off_fail_callback_dual_mpxx\n");
    pcie_linkup_fail_fixup(dev_id);
    (void)ssi_dump_err_regs(SSI_ERR_PCIE_FST_POWER_OFF_FAIL);

    return -1;
}

OAL_STATIC int32_t wlan_try_power_off_fail_callback_dual_mpxx(void *data)
{
    uint32_t dev_id = (uint32_t)(uintptr_t)(data);
    g_pcie_enum_fail_reg_dump_flag = 1;
    pcie_linkup_fail_fixup(dev_id);
    /* 阻止麒麟枚举失败后下电关时钟 */
    oal_chip_error_task_block();
    return -1;
}

OAL_STATIC void oal_pcie_sr_wakeup_pre_request(struct wlan_pm_s *pst_wlan_pm,
                                               oal_pcie_linux_res *pst_pci_lres,
                                               hcc_bus *pst_bus)
{
    uint32_t ret_timeout;

    hcc_bus_set_powerup_dev_wait_ack(pst_bus, 1);

    if (g_pcie_pm_linkdown_switch == OAL_TRUE) {
        oal_pcie_power_notifiy_register(pst_pci_lres->pst_pcie_dev,
            oal_pcie_get_pcie_rc_idx(pst_pci_lres->pst_pcie_dev), oal_pcie_wake_linkup_handler, NULL,
            (void *)pst_pci_lres->pst_pcie_dev);
        hcc_bus_power_action(pst_bus, HCC_BUS_POWER_UP);
    } else {
        oal_pcie_device_wakeup_handler((void *)(uintptr_t)(pst_bus->dev_id));
        if (pst_wlan_pm != NULL) {
            ret_timeout = oal_wait_for_completion_timeout(&pst_wlan_pm->st_wifi_powerup_done,
                (uint32_t)oal_msecs_to_jiffies(WIFI_POWERUP_DONE_TIMEOUT));
            if (oal_unlikely(ret_timeout == 0)) {
                /* 超时不做处理继续尝试建链 */
                declare_dft_trace_key_info("pcie_powerup_wakeup ack timeout", OAL_DFT_TRACE_FAIL);
            }
        } else {
            oal_msleep(100); /* delay 100 ms 让出系统调度 */
        }
    }

    hcc_bus_set_powerup_dev_wait_ack(pst_bus, 0);
}

OAL_STATIC int32_t oal_pcie_sr_wakeup_request_dual_mpxx(oal_pcie_linux_res *pst_pci_lres, hcc_bus *pst_bus)
{
    int32_t ret;
    declare_time_cost_stru(cost);
    struct wlan_pm_s *pst_wlan_pm = wlan_pm_get_drv_by_id(pst_bus->dev_id);
    oal_get_time_cost_start(cost);

    pci_print_log(PCI_LOG_INFO, "pst_bus->dev_id is [%d]", pst_bus->dev_id);
    if (pst_wlan_pm != NULL) {
        oal_init_completion(&pst_wlan_pm->st_wifi_powerup_done);
    }

    oal_pcie_sr_wakeup_pre_request(pst_wlan_pm, pst_pci_lres, pst_bus);

    /* iatu */
    ret = oal_pcie_reinit(pst_bus);
    if (ret != OAL_SUCC) {
        pci_print_log(PCI_LOG_ERR, "oal_pcie_reinit failed!ret=%d", ret);
        return ret;
    }

    oal_pcie_host_ip_init(pst_pci_lres);

    if (pst_pci_lres->pst_pci_res->chip_info.ete_support == OAL_TRUE) {
        ret = oal_ete_transfer_deepwkup_restore(pst_pci_lres->pst_pci_res);
        if (ret != OAL_SUCC) {
            pci_print_log(PCI_LOG_ERR, " ete deep wakeup failed, ret=%d", ret);
            return ret;
        }
    }

    ret = oal_pcie_sr_wakeup_request_enable(pst_pci_lres, pst_bus);
    if (ret != OAL_SUCC) {
        return ret;
    }

    oal_get_time_cost_end(cost);
    oal_calc_time_cost_sub(cost);
    pci_print_log(PCI_LOG_INFO, "pcie power up init succ, cost %llu us\n", time_cost_var_sub(cost));

    return OAL_SUCC;
}

OAL_STATIC int32_t oal_pcie_wakeup_request_dual_mpxx(hcc_bus *pst_bus)
{
    int32_t ret;
    oal_pcie_linux_res *pst_pci_lres;
    pst_pci_lres = (oal_pcie_linux_res *)pst_bus->data;

    // 1.拉高 Host WakeUp Device gpio
    // 2.调用kirin_pcie_pm_control 上电RC 检查建链
    // 3.restore state, load iatu config
    if (oal_unlikely(pst_pci_lres->pst_pci_res->link_state <= PCI_WLAN_LINK_DOWN)) {
        pci_print_log(PCI_LOG_WARN, "link invaild, wakeup failed, link_state:%s",
                      oal_pcie_get_link_state_str(pst_pci_lres->pst_pci_res->link_state));
        return -OAL_ENODEV;
    }

    if (pst_pci_lres->power_status == PCIE_EP_IP_POWER_DOWN) {
        ret = oal_pcie_sr_wakeup_request_dual_mpxx(pst_pci_lres, pst_bus);
        if (ret != OAL_SUCC) {
            return ret;
        }
    } else {
        /* 正常单芯片唤醒拉高GPIO即可 */
        oal_pcie_device_wakeup_handler((void *)(uintptr_t)(pst_bus->dev_id));
        // pci_set_master
        oal_pcie_enable_master(pst_pci_lres->pst_pci_res, pst_pci_lres->pst_pcie_dev, OAL_TRUE);
    }

    return OAL_SUCC;
}

OAL_STATIC void oal_pcie_bus_init_dual_mpxx(hcc_bus *pst_bus)
{
    /* 设置bus_id\dev_id */
    pst_bus->bus_id = (oal_pcie_get_probed_dev_num() == 0) ? HCC_EP_WIFI_DEV : HCC_EP_WIFI_DEV1;
    pst_bus->dev_id = (oal_pcie_get_probed_dev_num() == 0) ? HCC_EP_WIFI_DEV : HCC_EP_WIFI_DEV1;
    /* 双芯片对于g_pcie_edma_opt_ops结构体所注册函数的修改 */
    pst_bus->opt_ops->wakeup_request = oal_pcie_wakeup_request_dual_mpxx;

    oal_print_mpxx_log(MPXX_LOG_INFO, "add pcie bus [%d] enter\n", pst_bus->dev_id);
}

uint8_t oal_pcie_get_probed_dev_num(void)
{
    return g_pcie_probed_dev_num;
}

OAL_STATIC int32_t oal_pci_resource_init_by_id(uint32_t dev_id)
{
    int32_t ret;
    oal_pcie_linux_res *pst_pci_lres = NULL;

    /* Power off Wlan Chip */
    if (g_pcie_linux_res_tab[dev_id]) {
        hcc_bus_disable_state(g_pcie_linux_res_tab[dev_id]->pst_bus, OAL_BUS_STATE_ALL);
        pst_pci_lres = (oal_pcie_linux_res *)g_pcie_linux_res_tab[dev_id]->pst_bus->data;

        /* 保存PCIE 配置寄存器 */
        ret = oal_pcie_save_default_resource(g_pcie_linux_res_tab[dev_id]);
        if (ret != OAL_SUCC) {
            oal_pcie_disable_device(g_pcie_linux_res_tab[dev_id]);
#ifdef _PRE_CONFIG_PCIE_SHARED_INTX_IRQ
            oal_disable_pcie_irq_with_free(g_pcie_linux_res_tab[dev_id]);
#else
            oal_disable_pcie_irq(g_pcie_linux_res_tab[dev_id]);
#endif
            oal_pci_unregister_driver(&g_pcie_drv);
            hi_wlan_power_set_by_id(dev_id, 0);
            oal_pcie_power_notifiy_register(pst_pci_lres->pst_pcie_dev,
                                            oal_pcie_get_pcie_rc_idx(pst_pci_lres->pst_pcie_dev), NULL, NULL, NULL);
            return ret;
        }

        oal_pcie_power_notifiy_register(pst_pci_lres->pst_pcie_dev,
                                        oal_pcie_get_pcie_rc_idx(pst_pci_lres->pst_pcie_dev),
                                        NULL, wlan_first_power_off_callback, NULL);
#ifdef _PRE_CONFIG_PCIE_SHARED_INTX_IRQ
        oal_disable_pcie_irq_with_free(g_pcie_linux_res_tab[dev_id]);
#else
        oal_disable_pcie_irq(g_pcie_linux_res_tab[dev_id]);
#endif
        hcc_bus_power_action(g_pcie_linux_res_tab[dev_id]->pst_bus, HCC_BUS_POWER_DOWN);
        hi_wlan_power_set_by_id(dev_id, 0);
    } else {
        oal_io_print("g_pcie_linux_res[%d] is null\n", dev_id);
        oal_pcie_power_notifiy_register(NULL, dev_id, NULL, NULL, NULL);
        return -OAL_ENODEV;
    }
    return OAL_SUCC;
}

OAL_STATIC int32_t oal_pci_resource_init_dual_mpxx(void)
{
    uint32_t id = 0;
    int32_t ret = 0;
    for (id = 0; id < oal_pcie_get_probed_dev_num(); id++) {
        hcc_set_main_dev(id);
        ret = oal_pci_resource_init_by_id(id);
        if (ret != OAL_SUCC) {
            oal_io_print("%s[%d] PCIe driver register not succ\n", g_pcie_drv.name, id);
            return ret;
        }
    }
    hcc_set_main_dev(HCC_EP_WIFI_DEV);
    return OAL_SUCC;
}

OAL_STATIC int32_t oal_pcie_mpxx_enum_by_idx(int32_t rc_idx)
{
    int32_t ret;
    oal_io_print("oal_pcie_devs_enum_by_idx rc_idx=%d\n", rc_idx);

    /* 打开参考时钟 */
    ret = oal_pcie_power_notifiy_register(NULL, rc_idx, wlan_first_power_on_callback_dual_mpxx,
                                          wlan_first_power_off_fail_callback_dual_mpxx, NULL);
    if (ret != OAL_SUCC) {
        oal_io_print("enumerate first power register failed ret=%d\n", ret);
        return ret;
    }

    ret = oal_pcie_enumerate(rc_idx);
    if (ret == OAL_SUCC) {
        oal_io_print("pcie enumerate succ\n");
        return ret;
    }

    chr_exception_report(CHR_PLATFORM_EXCEPTION_EVENTID, CHR_SYSTEM_PLAT, CHR_LAYER_DRV,
                         CHR_PLT_DRV_EVENT_PCIE, CHR_PLAT_DRV_ERROR_PCIE_ENUM_FAIL);
    hi_wlan_power_set_by_id(rc_idx, 0);
    msleep(1000); // 此时硬件状态可能有异常，预留1000ms 给硬件电源恢复再尝试
    // 尝试第二次枚举
    ret = oal_pcie_power_notifiy_register(NULL, rc_idx, wlan_first_power_on_callback_dual_mpxx,
                                          wlan_try_power_off_fail_callback_dual_mpxx, NULL);
    if (ret != OAL_SUCC) {
        oal_io_print("enumerate try power register failed ret=%d\n", ret);
        return ret;
    }

    ret = oal_pcie_enumerate(rc_idx);
    if (ret == OAL_SUCC) {
        chr_exception_report(CHR_PLATFORM_EXCEPTION_EVENTID, CHR_SYSTEM_PLAT, CHR_LAYER_DRV,
                             CHR_PLT_DRV_EVENT_PCIE, CHR_PLAT_DRV_ERROR_PCIE_ENUM_TRY_SUCC);
        oal_io_print("pcie try enum succ\n");
        return ret;
    }

    oal_pcie_mpxx_enum_fail_proc();
    oal_io_print("enumerate try failed ret=%d\n", ret);
    oal_pcie_power_notifiy_register(NULL, rc_idx, NULL, NULL, NULL);
    return ret;
}

OAL_STATIC int32_t oal_pcie_enum_dual_mpxx(void)
{
    uint8_t enum_succ_cnt = 0;
    uint8_t rc_idx = 0;

    for (rc_idx = 0; rc_idx < WLAN_CHIP_MAX_NUM_PER_BOARD; rc_idx++) {
        if (oal_pcie_mpxx_enum_by_idx(rc_idx) == OAL_SUCC) {
            oal_io_print("kirin pcie enum [%d] success\n", rc_idx);
            enum_succ_cnt++;
        }
    }

    if (enum_succ_cnt == 0) {
        oal_io_print("kirin pcie enum fail\n");
        return OAL_FAIL;
    }
    return OAL_SUCC;
}

OAL_STATIC void oal_pcie_res_pre_process(oal_pcie_linux_res *pst_pci_lres, oal_pci_dev_stru *pst_pci_dev)
{
    memset_s((void *)pst_pci_lres, sizeof(oal_pcie_linux_res), 0, sizeof(oal_pcie_linux_res));

    pst_pci_lres->pst_pcie_dev = pst_pci_dev;
    pci_set_drvdata(pst_pci_dev, pst_pci_lres);
    oal_init_completion(&pst_pci_lres->st_pcie_ready);
    oal_atomic_set(&pst_pci_lres->st_pcie_wakeup_flag, 0);
    oal_task_init(&pst_pci_lres->st_rx_task, oal_pcie_intx_task, (uintptr_t)pst_pci_lres);

    oal_spin_lock_init(&pst_pci_lres->st_lock);
    mutex_init(&pst_pci_lres->sr_lock);
    oal_wake_lock_init(&pst_pci_lres->st_sr_bugfix, "pcie_sr_bugfix");

    pst_pci_lres->version = (pst_pci_dev->vendor) | (pst_pci_dev->device << 16); /* device为版本号的高16位 */
}

OAL_STATIC void oal_pcie_failed_res_alloc(int32_t ret, oal_pci_dev_stru *pst_pci_dev)
{
    oal_pci_disable_device(pst_pci_dev);
    g_probe_ret = ret;
    oal_complete(&g_probe_complete);
}

/* alloc pcie resources */
OAL_STATIC oal_pcie_linux_res *oal_pcie_res_alloc(oal_pci_dev_stru *pst_pci_dev)
{
    uint8_t reg8 = 0;
    int32_t ret = -OAL_EFAIL;
    oal_pcie_linux_res *pst_pci_lres = NULL;
    pst_pci_lres = (oal_pcie_linux_res *)oal_memalloc(sizeof(oal_pcie_linux_res));
    if (pst_pci_lres == NULL) {
        oal_io_print("oal_pcie_res_alloc:: alloc res failed,size:%lu\n",  (unsigned long)sizeof(oal_pcie_linux_res));
        oal_pci_disable_device(pst_pci_dev);
        g_probe_ret = ret;
        oal_complete(&g_probe_complete);
        return NULL;
    }

    oal_pcie_res_pre_process(pst_pci_lres, pst_pci_dev);

    ret = oal_pci_read_config_byte(pst_pci_dev, PCI_REVISION_ID, &reg8);
    if (ret) {
        oal_io_print("pci: read revision id  failed, ret=%d\n", ret);
        goto failed_pcie_res_alloc_host_init;
    }
    pst_pci_lres->revision = (uint32_t)reg8;
    pci_print_log(PCI_LOG_INFO, "wifi pcie version:0x%8x, revision:%d", pst_pci_lres->version, pst_pci_lres->revision);

    /* soft resource init */
    pst_pci_lres->pst_pci_res = oal_pcie_host_init((void *)pst_pci_dev, &pst_pci_lres->st_msi, pst_pci_lres->revision);
    if (pst_pci_lres->pst_pci_res == NULL) {
        oal_io_print("pci: oal_pcie_host_init failed\n");
        goto failed_pcie_res_alloc_host_init;
    }

    return pst_pci_lres;

failed_pcie_res_alloc_host_init:
    pci_set_drvdata(pst_pci_dev, NULL);
    oal_wake_lock_exit(&pst_pci_lres->st_sr_bugfix);
    oal_free(pst_pci_lres);
    oal_pcie_failed_res_alloc(ret, pst_pci_dev);
    return NULL;
}

OAL_STATIC void oal_pcie_failed_pci_bus_init(int32_t ret, oal_pci_dev_stru *pst_pci_dev,
                                             oal_pcie_linux_res *pst_pci_lres)
{
    oal_pcie_host_exit(pst_pci_lres->pst_pci_res);
    pci_set_drvdata(pst_pci_dev, NULL);
    oal_wake_lock_exit(&pst_pci_lres->st_sr_bugfix);
    oal_free(pst_pci_lres);
    oal_pcie_failed_res_alloc(ret, pst_pci_dev);
}

OAL_STATIC void oal_pcie_failed_pci_dev_init(int32_t ret, hcc_bus *pst_bus, oal_pci_dev_stru *pst_pci_dev,
                                             oal_pcie_linux_res *pst_pci_lres)
{
    oal_pcie_bus_exit(pst_bus);
    oal_pcie_failed_pci_bus_init(ret, pst_pci_dev, pst_pci_lres);
}

OAL_STATIC void oal_pcie_failed_register_msi(int32_t ret, hcc_bus *pst_bus, oal_pci_dev_stru *pst_pci_dev,
                                             oal_pcie_linux_res *pst_pci_lres)
{
    oal_pcie_dev_deinit(pst_pci_lres->pst_pci_res);
    oal_pcie_failed_pci_dev_init(ret, pst_bus, pst_pci_dev, pst_pci_lres);
}

/* pcie dev hw res init */
OAL_STATIC int32_t oal_pcie_dev_hw_res_init(hcc_bus *pst_bus, oal_pci_dev_stru *pst_pci_dev,
                                            oal_pcie_linux_res *pst_pci_lres)
{
    int32_t ret = -OAL_EFAIL;
    unsigned short device_id = oal_pci_get_dev_id(pst_pci_dev);

    /* 硬件设备资源初始化 */
    ret = oal_pcie_dev_init(pst_pci_lres->pst_pci_res);
    if (ret != OAL_SUCC) {
        oal_pcie_failed_pci_dev_init(ret, pst_bus, pst_pci_dev, pst_pci_lres);
        return ret;
    }

    ret = oal_pcie_register_int(pst_pci_dev, pst_pci_lres);
    if (ret != OAL_SUCC) {
        oal_pcie_failed_register_msi(ret, pst_bus, pst_pci_dev, pst_pci_lres);
        return ret;
    }

    ret = oal_pcie_firmware_msg_init(pst_pci_lres->pst_pci_res);
    if (ret != OAL_SUCC) {
        oal_pcie_failed_register_msi(ret, pst_bus, pst_pci_dev, pst_pci_lres);
        return ret;
    }
    /* KIRIN_PCIE_LINKDOWN_RECOVERY */
    oal_pcie_register_event(pst_pci_dev, pst_pci_lres);

    ret = oal_pcie_chiptest_init(pst_pci_lres->pst_pci_res, device_id);
    if (ret != OAL_SUCC) {
        oal_io_print("pci: oal_pcie_chiptest_init failed, device_id = %u\n", (uint32_t)device_id);
    }

    return OAL_SUCC;
}

/* pcie dev res init */
OAL_STATIC int32_t oal_pcie_dev_probe(oal_pci_dev_stru *pst_pci_dev)
{
    hcc_bus *pst_bus = NULL;
    oal_pcie_linux_res *pst_pci_lres = NULL;
    int32_t ret = -OAL_EFAIL;
    uint8_t chip_idx = oal_pcie_get_probed_dev_num();

    ret = oal_pcie_init_dma_attr(pst_pci_dev);
    if (ret != OAL_SUCC) {
        g_probe_ret = -OAL_EINVAL;
        oal_complete(&g_probe_complete);
        return -OAL_EINVAL;
    }
    /* 设备ID 和 产品ID */
    oal_print_mpxx_log(MPXX_LOG_INFO,
        "[PCIe][%s]devfn:0x%x , vendor:0x%x , device:0x%x , subsystem_vendor:0x%x , subsystem_device:0x%x , class:0x%x "
        "\n",
        dev_name(&pst_pci_dev->dev), pst_pci_dev->devfn, pst_pci_dev->vendor, pst_pci_dev->device,
        pst_pci_dev->subsystem_vendor, pst_pci_dev->subsystem_device, pst_pci_dev->class);

    ret = oal_pci_enable_device(pst_pci_dev);
    if (ret) {
        oal_io_print("pci: pci_enable_device error ret=%d\n", ret);
        g_probe_ret = ret;
        oal_complete(&g_probe_complete);
        return ret;
    }

    /* alloc pcie resources */
    pst_pci_lres = oal_pcie_res_alloc(pst_pci_dev);
    if (pst_pci_lres == NULL) {
        oal_io_print("pci: oal_pcie_res_alloc error \n");
        return -OAL_EFAIL;
    }

    /* pcie bus init */
    pst_bus = oal_pcie_bus_init(pst_pci_lres);
    if (pst_bus == NULL) {
        oal_pcie_failed_pci_bus_init(ret, pst_pci_dev, pst_pci_lres);
        return -OAL_EFAIL;
    }

    ret = oal_pcie_dev_hw_res_init(pst_bus, pst_pci_dev, pst_pci_lres);
    if (ret) {
        oal_io_print("pci: oal_pcie_dev_hw_res_init error ret=%d\n", ret);
        return ret;
    }

    g_pcie_linux_res_tab[chip_idx] = pst_pci_lres;
    return OAL_SUCC;
}

/* 探测到一个PCIE设备, 双芯片硬件环境正常情况下 probe 函数至少会触发2次 */
OAL_STATIC int32_t oal_pcie_probe_dual_mpxx(oal_pci_dev_stru *pst_pci_dev, const oal_pci_device_id_stru *pst_id)
{
    int32_t ret = -OAL_EFAIL;

    if (oal_any_null_ptr2(pst_pci_dev, pst_id)) {
        /* never touch here */
        oal_io_print("oal_pcie_probe failed, pst_pci_dev:%p, pst_id:%p\n", pst_pci_dev, pst_id);
        g_probe_ret = -OAL_EIO;
        oal_complete(&g_probe_complete);
        return -OAL_EIO;
    }

    /* ini中dev num只配置了1，无需执行第二次probe */
    if ((hcc_get_bus_dev_num() == NUM_1_BYTES) && (g_pcie_probed_dev_num != 0)) {
        oal_io_print("oal_pcie_probe:: config only one dev, no need probe twice\n");
        g_probe_ret = OAL_SUCC;
        oal_complete(&g_probe_complete);
        return OAL_SUCC;
    }

    ret = oal_pcie_dev_probe(pst_pci_dev);
    if (ret != OAL_SUCC) {
        oal_io_print("oal_pcie_dev_probe failed, ret = %d\n", ret);
        return ret;
    }

    g_pcie_probed_dev_num++;
    oal_io_print("oal_pcie_probe end, pst_pci_dev:%p, dev_num:%d\n", pst_pci_dev, oal_pcie_get_probed_dev_num());
    g_probe_ret = ret;
    oal_complete(&g_probe_complete);

    return ret;
}

OAL_STATIC void oal_pcie_dual_mpxx_init(void)
{
    g_pcie_drv.probe = oal_pcie_probe_dual_mpxx;
}

#endif
