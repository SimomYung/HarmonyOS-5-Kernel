...
OAL_STATIC int32_t g_pcie_rc_ber_check_bypass = 0;
oal_debug_module_param(g_pcie_rc_ber_check_bypass, int, S_IRUGO | S_IWUSR);
...
void oal_pcie_linkdown_callback(struct kirin_pcie_notify *noti)
{
    ...
    if (noti == NULL) {
        oal_io_print("pcie linkdown, noti is null!!\n");
        return;
    }

    pst_pci_dev = (oal_pci_dev_stru *)noti->user;
    if (pst_pci_dev == NULL) {
...
int32_t oal_pcie_host_aspm_init(oal_pcie_linux_res *pst_pci_lres)
{
    ...
    if (g_pcie_aspm_enable) {
        ...
        if ((pst_pci_lres->pst_pci_res != NULL) && (pst_pci_lres->pst_pci_res->pst_pci_dbi_base != NULL)) {
            oal_pcie_enable_aspm(pst_pci_lres->pst_pci_res);
        } else {
...
#define KPORT_CHS_MAC_PCS_RX_ERR_CNT_REG_OFF 0x2BC // RC RX 误码率统计
static int32_t oal_pcie_get_rc_link_quality_chs(oal_pci_dev_stru *pst_rc_dev, uint32_t *d2h_ber)
{
    int32_t ret;
    uint32_t val;
    ret = oal_pci_read_config_dword(pst_rc_dev, KPORT_CHS_MAC_PCS_RX_ERR_CNT_REG_OFF, &val);
    if (ret != 0) {
        pci_print_log(PCI_LOG_ERR, "pci read rx err cnt failed ret=%d", ret);
        return ret;
    }
    *d2h_ber = val;
    return OAL_SUCC;
...
int32_t oal_pcie_get_rc_link_quality_info(oal_pcie_linux_res *pst_pci_lres, uint32_t *d2h_ber)
{
    ...
    *d2h_ber = 0;
    ...
    ret = oal_pci_read_config_word(pst_rc_dev, PCI_VENDOR_ID, &vid);
    if (ret != 0) {
        pci_print_log(PCI_LOG_ERR, "pci read vendor id failed ret=%d", ret);
        return ret;
    }
    ret = oal_pci_read_config_word(pst_rc_dev, PCI_DEVICE_ID, &pid);
    if (ret != 0) {
        pci_print_log(PCI_LOG_ERR, "pci read product id failed ret=%d", ret);
        return ret;
    }
    pci_print_log(PCI_LOG_INFO, "rc vendor id:0x%x, product id:0x%x", vid, pid);
    id_val = vid | ((uint32_t)pid << 16); // 16: 高位存放pid
    switch (id_val) {
        // VID/PID RC侧是可以被修改的,需和产品约定当前用例下是否会被修改
        case 0x36a019e5: // chs soc
            return oal_pcie_get_rc_link_quality_chs(pst_rc_dev, d2h_ber);
        default:
            pci_print_log(PCI_LOG_WARN, "rc id:0x%x not support", id_val);
            if (g_pcie_rc_ber_check_bypass == 0) {
                pci_print_log(PCI_LOG_ERR, "force abort rc link quality check");
                return -OAL_ENODEV;
            }
    }
    return OAL_SUCC;
