    ...
    switch (chip_type) {
        ...
        case BOARD_VERSION_MP16C:
            if (get_mpxx_subchip_version() == CHIP_VERSION_PILOT) {
                ...
                ota_data->st_ota_hdr.auc_resv[0] = OAM_OTA_TYPE_MP17C_HOST_MPW;
            }
            break;
        ...
        case BOARD_VERSION_MP17C:
            ota_data->st_ota_hdr.auc_resv[0] = OAM_OTA_TYPE_MP17C_HOST;
            break;
        case BOARD_VERSION_MP12:
            ota_data->st_ota_hdr.auc_resv[0] = OAM_OTA_TYPE_MP12_HOST;
            break;
        default:
                                           ...
                                           oam_mng_ctx_stru *ctx)
{
    ...
    /* 复制数据,填写ota数据 */
    if (memcpy_s((void *)pst_ota_data->auc_ota_data, (uint32_t)us_dscr_len,
        (const void *)puc_dscr_addr, (uint32_t)us_dscr_len) != EOK) {
        oal_mem_sdt_netbuf_free(pst_netbuf, OAL_TRUE);
                                                ...
                                                oam_mng_ctx_stru *ctx)
{
    ...
    /* 复制数据,填写ota数据 */
    if (memcpy_s((void *)pst_ota_data->auc_ota_data, (uint32_t)us_eth_frame_len,
                 (const void *)puc_eth_frame_hdr_addr, (uint32_t)us_eth_frame_len) != EOK) {
        oal_mem_sdt_netbuf_free(pst_netbuf, OAL_TRUE);
                                                ...
                                                oam_mng_ctx_stru *ctx)
{
    ...
    /* 复制数据,填写ota数据 */
    if (memcpy_s((void *)pst_ota_data->auc_ota_data, cb_len, (const void *)netbuf_cb, cb_len) != EOK) {
        oal_mem_sdt_netbuf_free(pst_netbuf, OAL_TRUE);
