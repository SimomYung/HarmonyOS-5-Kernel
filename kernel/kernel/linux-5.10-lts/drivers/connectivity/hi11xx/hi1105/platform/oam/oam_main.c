...
void oam_dump_buff_by_hex(uint8_t *puc_buff, int32_t l_len, int32_t l_num)
{
    ...
    oal_print_hex_dump(puc_buff, l_len, HEX_DUMP_GROUP_SIZE, "buf: ");
}
...
int32_t oam_main_init(void)
{
    ...
    if (is_wifi_support()) {
        ...
        if (oal_bus_get_chip_num() == WLAN_CHIP_MAX_NUM_PER_BOARD) {
            hcc_rx_register(hcc_get_handler(HCC_EP_WIFI_DEV1), HCC_ACTION_TYPE_OAM, oam_rx_post_action_function, NULL);
            hcc_rx_register(hcc_get_handler(HCC_EP_WIFI_DEV1), HCC_ACTION_TYPE_CHR, chr_rx_post_action_function, NULL);
        }
#endif
    }
    if (is_gt_support()) {
