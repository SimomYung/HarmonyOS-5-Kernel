...
#if ((defined CONFIG_HISI_BB) || (defined CONFIG_DFX_BB)) && (defined CONFIG_HI110X_PLAT_BB) && \
...
void plat_bbox_msg_hander(int32_t subsys_type, int32_t exception_type)
{
    ...
    if (ps_is_device_log_enable() == false &&
        ...
        exception_type != BUART_SYSTEM_RESET &&
        exception_type != GUART_SYSTEM_RESET &&
        exception_type != HCC_BUS_PANIC &&
        exception_type != WIFI_SYSTEM_RESET) {
        return;
    }
    ...
    if (subsys_type == SUBSYS_BFGX) {
        switch (exception_type) {
            ...
            case BUART_SYSTEM_RESET:
            case GUART_SYSTEM_RESET:
