...
{
    uint32_t force_sleep = board_pm_force_sleep_get();
    unsigned long flags;
    ...
    if (force_sleep == OAL_TRUE) {
        oal_spin_lock_irq_save(&pm_data->uart_state_spinlock, &flags);
        if (bfgx_dev_state_get(pm_data) == BFGX_SLEEP) {
            oal_spin_unlock_irq_restore(&pm_data->uart_state_spinlock, &flags);
            ps_print_info("[%s]bfgx already sleep or force_sleep[%d]!\n", index2name(pm_data->index), force_sleep);
            return -FAILURE;
        }
        oal_spin_unlock_irq_restore(&pm_data->uart_state_spinlock, &flags);
    }
