...
OAL_STATIC int s3s4_poweroff_notify_func(unsigned long event, pm_notify_param* param, void *data)
{
    switch (event) {
        case PM_SUSPEND_PREPARE:
            ps_print_info("[BUART] S3S4 PM_SUSPEND_PREPARE suspend now!\n");
            suspend_mpxx();
            break;
        case PM_HIBERNATION_PREPARE:
            ps_print_info("[BUART] S3S4 PM_HIBERNATION_PREPARE suspend now!\n");
            suspend_mpxx();
            break;
        case PM_POST_SUSPEND:
            ps_print_info("[BUART] S3S4 PM_POST_SUSPEND resume now!\n");
            resume_mpxx();
            break;
        case PM_POST_HIBERNATION:
            ps_print_info("[BUART] S3S4 PM_POST_HIBERNATION resume now!\n");
            resume_mpxx();
            break;
        default:
            break;
...
/* 用于收编_PRE_CONFIG_HISI_S3S4_POWER_STATE特性下, S3也和S4一样处理，GPIO硬件下电，需要保存回复处理 */
pm_notify_node g_s3s4_poweroff_nb = {
    .name = "s3s4_poweroff_nb",
    .priority = PLAT_NOITFY_PRIO + 1,  // 涉及保存回复业务状态, 优先平台处理
    .cb_func = s3s4_poweroff_notify_func,
};
#endif

pm_notify_node g_bfg_pm_nb[UART_BUTT];

#ifdef _PRE_HOST_SUSPEND_UART_POWERDOWN
#define WAIT_TTY_CLOSE_TIME 200
#define WAIT_POST_SUSPEND_SLEEP_TIME 100

static int send_disallow_msg_via_tty(struct pm_drv_data *pm_data)
{
    int ret;
...
static void release_tty_after_send_msg(struct pm_drv_data *pm_data)
{
    release_tty_drv(pm_data->ps_core_data);
    msleep(WAIT_TTY_CLOSE_TIME);
}
#endif

/*
 * Function: bfg_pm_suspend_proc
 * Description: if force_sleep, wait till bfgx sleep
 * Ruturn: 0 -- success
 */
OAL_STATIC int bfg_pm_suspend_proc(pm_notify_param* param, void* data)
{
    unsigned long flags;
    ...
    for (; loop > 0; loop--) {
        ...
        if (oal_wakelock_active(&pm_data->bus_wake_lock) == 0) {
            // wakelock锁已释放
            oal_spin_unlock_irq_restore(&pm_data->wakelock_protect_spinlock, &flags);
            ps_print_info("[%s]- already sleep\n", index2name(pm_data->index));
#ifdef _PRE_HOST_SUSPEND_UART_POWERDOWN
            release_tty_after_send_msg(pm_data);
#endif
            if (ps_chk_rx_queue_empty(pm_data->ps_core_data) == false) {
                ps_print_warning("[%s]rx queue not empty when suspend", index2name(pm_data->index));
            }
            return OAL_SUCC;
        } else {
            if (param->force_sleep == OAL_FALSE) {
                oal_spin_unlock_irq_restore(&pm_data->wakelock_protect_spinlock, &flags);
                ps_print_info("[%s]- not support force sleep\n", index2name(pm_data->index));
                return -OAL_FAIL;
            }
            // 强制睡眠, 提交sleep work
            oal_spin_unlock_irq_restore(&pm_data->wakelock_protect_spinlock, &flags);
    ...
    ps_print_info("[%s]- sleep timeout\n", index2name(pm_data->index));
    return -OAL_FAIL;
}

OAL_STATIC int bfg_pm_resume_proc(pm_notify_param* param, void* data)
{
    struct pm_drv_data *pm_data = (struct pm_drv_data *)data;
    if (pm_data == NULL || param == NULL) {
        return -OAL_FAIL;
    }
    /* 收编原大屏项目处理，低功耗唤醒后主动发起一次disallow消息 */
#ifdef _PRE_HOST_SUSPEND_UART_POWERDOWN
    if (send_disallow_msg_via_tty(pm_data) != 0) {
        return -OAL_FAIL;
    }
#endif
    return OAL_SUCC;
...
OAL_STATIC int bfg_pm_board_notify_func(unsigned long event, pm_notify_param* param, void *data)
{
    int32_t ret = OAL_SUCC;
    switch (event) {
        case PM_SUSPEND_PREPARE:
            ret = bfg_pm_suspend_proc(param, data);
            break;
        case PM_HIBERNATION_PREPARE:
            break;
        case PM_POST_SUSPEND:
            ret = bfg_pm_resume_proc(param, data);
            break;
        case PM_POST_HIBERNATION:
            break;
        default:
...
OAL_STATIC void bfgx_pm_notifier_register(struct pm_drv_data *pm_data)
{
    uint32_t index = pm_data->index;

    if (snprintf_s(g_bfg_pm_nb[index].name, sizeof(g_bfg_pm_nb[index].name), sizeof(g_bfg_pm_nb[index].name) - 1,
                   "bfg_pm_nb_%u", index) < 0) {
        ps_print_err("name too long\n");
        return;
    }
    g_bfg_pm_nb[index].priority = PLAT_NOITFY_PRIO;
    g_bfg_pm_nb[index].cb_func = bfg_pm_board_notify_func;
    g_bfg_pm_nb[index].data = (void*)pm_data;
    board_pm_register_notify_cb(&g_bfg_pm_nb[index]);

#ifdef _PRE_CONFIG_HISI_S3S4_POWER_STATE
    if (index == BUART) {
        board_pm_register_notify_cb(&g_s3s4_poweroff_nb);
    }
#endif
    return;
}
...
static void host_wkup_operate(struct ps_core_s *ps_core_d, struct pm_drv_data *pm_data)
{
    ...
    /* 睡眠时间在3000到3100us之间, 避免device唤醒host的情况下，马上切换波特率异常 */
    oal_usleep_range(3000, 3100);
#endif
}
...
{
    int32_t ret, gpio_type;
    const uint32_t wkup_retry = 2;
    unsigned long timeleft, baud_rate;
    int bwkup_gpio_val = 0;
    uint32_t i;
    ...
    for (i = 0; i < wkup_retry; i++) {
        bwkup_gpio_val = conn_get_gpio_level(gpio_type);
        if (bwkup_gpio_val == GPIO_LEVEL_HIGH) {
            declare_dft_trace_key_info("uart_wakeup_conflict", OAL_DFT_TRACE_SUCC);
            ps_print_info("[%s]bfg wakeup dev,ignore %u,gpio level:1\n", index2name(pm_data->index), i + 1);
        } else {
            ps_print_info("[%s]bfg wakeup dev,try %u,gpio level:0\n", index2name(pm_data->index), i + 1);
            host_wkup_operate(ps_core_d, pm_data);
        }
...
#ifdef PLATFORM_DEBUG_ENABLE
    if (is_dfr_test_en(UART_SUSPEND_FAULT, (uintptr_t)ps_core_d) == OAL_TRUE) {
...
static void bfg_wakeup_time_debug(struct pm_drv_data *pm_data)
{
    ...
    if (total_time > report_thresh_hold) {
        ps_print_warning("host_send_disallow_msg work scheule slow");
...
STATIC int32_t agree_allow_bfg_sleep(struct ps_core_s *ps_core_d, struct pm_drv_data *pm_data)
{
    ...
    /* 发送数据的线程和睡眠线程异步的情况下，睡眠线程会清理掉资源 */
    if (time_after(pm_data->bfg_timer.expires, jiffies) == true) {
        ps_print_info("[%s] sleep timer is update, skip sleep stage\n", index2name(pm_data->index));
        return OAL_FALSE;
    }

    if ((gnss_get_lowpower_state(pm_data) == GNSS_NOT_AGREE_SLEEP) ||
    ...
    /* 仅检查rx queque状态做告警，不阻止睡眠，防止业务进程异常崩溃不取走数据，系统无法休眠 */
    if (ps_chk_rx_queue_empty(ps_core_d) == false) {
        ps_print_warning("[%s]rx queue not empty when allow sleep", index2name(pm_data->index));
    }
...
STATIC int32_t bfg_enter_sleep_check(struct pm_drv_data *pm_data)
{
    ...
    if (bfgx_dev_state_get(pm_data) == BFGX_SLEEP) {
        oal_spin_unlock_irq_restore(&pm_data->uart_state_spinlock, &flags);
        ps_print_info("[%s]bfgx already sleep!\n", index2name(pm_data->index));
        return -FAILURE;
    }
    oal_spin_unlock_irq_restore(&pm_data->uart_state_spinlock, &flags);
#ifdef PLATFORM_DEBUG_ENABLE
...
void wifi_notify_bfgx_status(uint8_t status)
{
    int32_t prepare_result;
...
int32_t wlan_power_on(void)
{
    ...
    if (hcc_bus_exception_is_busy(hcc_get_bus(hcc_get_dev_id(wlan_pm_info->pst_bus))) == OAL_TRUE) {
...
#ifdef PLATFORM_DEBUG_ENABLE
    if (is_dfr_test_en(WIFI_POWER_ON_FAULT, (uintptr_t)hcc_get_handler(hcc_get_dev_id(wlan_pm_info->pst_bus)))) {
        cancel_excp_test_en(WIFI_POWER_ON_FAULT, (uintptr_t)hcc_get_handler(hcc_get_dev_id(wlan_pm_info->pst_bus)));
...
int32_t wlan_power_off_complete(void)
{
    ...
    int gpio_level;
    struct wlan_pm_s *wlan_pm_info = wlan_pm_get_drv();
    int32_t type_gpio = wlan_pm_info->pst_bus->bus_dev->wkup_host_io_type;

    timeout_cnt = hi110x_get_emu_timeout(ACK_CHECK_MAX_CNT);
    gpio_level = 0;
    for (i = 0; i < timeout_cnt; i++) {
        msleep(ACK_CHECK_WAIT_TIME);
        gpio_level = conn_get_gpio_level(type_gpio);
        if (gpio_level == 1) {
            ret = OAL_SUCC;
            break;
        }
...
STATIC int32_t bfgx_power_off(struct pm_drv_data *pm_data, uint8_t subsys)
{
    ...
    ps_print_info("[%s]%s power off!\n", index2name(pm_data->index), pst_bfgx_data->name);
...
{
    if (queue_work(pm_data->wkup_dev_workqueue, &pm_data->send_disallow_msg_work) == true) {
        pm_data->disallow_work_add_time = ktime_get();
        ps_print_info("add work succ\n");
    } else {
        ps_print_info("work already exist\n");
    }
}
/*
...
irqreturn_t bfg_wake_host_isr(int irq, void *dev_id)
{
    ...
    if (conn_get_gpio_level(gpio_type) == GPIO_LOWLEVEL) {
        if (atomic_read(&pm_data->bfg_needwait_devboot_flag) == NEED_SET_FLAG) {
            ps_print_info("irq before power up, ignore\n");
            return IRQ_HANDLED;
        }
...
STATIC void devack_disagree_sleep_by_gpio(struct pm_drv_data *pm_data, struct ps_core_s *ps_core_d)
{
    ...
    if (pm_data->uc_dev_ack_wait_cnt < WAIT_DEVACK_TOTAL_CNT) {
        /* 定时器超时时间递增，避免太多timer中断影响系统调度 */
        timeout_ms = WAIT_DEVACK_MSEC * pm_data->uc_dev_ack_wait_cnt;
        timeout_ms = (timeout_ms <= WAIT_DEVACK_MSEC_MAX) ? timeout_ms : WAIT_DEVACK_MSEC_MAX;
        mod_timer(&pm_data->dev_ack_timer, jiffies +  msecs_to_jiffies(timeout_ms));
...
static int low_power_workq_init(struct pm_drv_data *pm_data)
{
    ...
    /* init bfg wake lock */
    ret = sprintf_s(name_buff, sizeof(name_buff), "%s_%u", BFG_LOCK_NAME, pm_data->index);
    ps_print_info("wakelock_name %s, ret = %d", name_buff, ret);
    oal_wake_lock_init(&pm_data->bus_wake_lock, name_buff);
    ret = sprintf_s(name_buff, sizeof(name_buff), "%s_%u", BT_LOCK_NAME, pm_data->index);
    ps_print_info("wakelock_name %s, ret = %d", name_buff, ret);
    oal_wake_lock_init(&pm_data->bt_wake_lock, name_buff);
    ret = sprintf_s(name_buff, sizeof(name_buff), "%s_%u", GNSS_LOCK_NAME, pm_data->index);
    ps_print_info("wakelock_name %s, ret = %d", name_buff, ret);
...
STATIC int low_power_probe(struct pm_drv_data **pp_data, struct ps_core_s *ps_core_d)
{
    ...
    /* register host pm */
    bfgx_pm_notifier_register(pm_data);
...
int low_power_init(void)
{
    ...
    if (is_wifi_support() && hcc_get_bus(HCC_EP_WIFI_DEV1) != NULL) {
        pm_top_data->wlan_pm_info_dev1 = mpxx_dev1_wlan_pm_init();
        if (pm_top_data->wlan_pm_info_dev1 == NULL) {
            ps_print_err("no mem to allocate wlan_pm_info\n");
            goto WLAN_INIT_FAIL;
        }
    }
