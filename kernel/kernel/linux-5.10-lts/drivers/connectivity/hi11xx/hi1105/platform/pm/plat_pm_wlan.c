...
#include <platform_include/basicplatform/linux/gpio_dump.h>
#endif
...
#ifdef _PRE_DUAL_CHIP
OAL_STATIC oal_uint mpxx_dev1_wlan_pm_wakeup_dev(void);
OAL_STATIC oal_uint mpxx_dev1_wlan_pm_state_get(void);
OAL_STATIC oal_uint mpxx_dev1_wlan_pm_wakeup_host(void);
OAL_STATIC void mpxx_dev1_wlan_pm_feed_wdg(void);
OAL_STATIC void mpxx_dev1_wlan_pm_wakeup_dev_ack(void);
OAL_STATIC uint32_t mpxx_dev1_wlan_pm_disable_check_wakeup(int32_t flag);
#endif
...
#ifdef _PRE_DUAL_CHIP
static pm_callback_stru g_wlan_pm_callback_mpxx_dev1 = {
    .pm_wakeup_dev = mpxx_dev1_wlan_pm_wakeup_dev,
    .pm_state_get = mpxx_dev1_wlan_pm_state_get,
    .pm_wakeup_host = mpxx_dev1_wlan_pm_wakeup_host,
    .pm_feed_wdg = mpxx_dev1_wlan_pm_feed_wdg,
    .pm_wakeup_dev_ack = mpxx_dev1_wlan_pm_wakeup_dev_ack,
    .pm_disable = mpxx_dev1_wlan_pm_disable_check_wakeup,
};

static struct wlan_pm_s *g_wlan_pm_info_mpxx_dev1 = NULL;
uint8_t g_custom_cali_done_mpxx_dev1 = OAL_FALSE;
EXPORT_SYMBOL_GPL(g_custom_cali_done_mpxx_dev1);
#endif
#endif
...
uint32_t g_wake_dev_min_time = WLAN_WAKE_DEV_MIN_TIME;

uint32_t g_delaysleep_time = WLAN_WAKELOCK_HOLD_TIME;
oal_debug_module_param(g_delaysleep_time, int, S_IRUGO | S_IWUSR);
...
struct wlan_pm_s *wlan_pm_get_drv(void)
{
    ...
    if (dev_id > HCC_EP_WIFI_DEV1) {
        oal_print_mpxx_log(MPXX_LOG_ERR, "get dev_id [%d] is out of range", dev_id);
        return NULL;
    }
    ...
    if (dev_id == HCC_EP_WIFI_DEV) {
        ...
        return g_wlan_pm_info_mpxx_dev1;
    }
#endif
    return g_wlan_pm_info;
}
...
/*
 * 函 数 名  : wlan_pm_get_drv_by_id
 * 功能描述  : 获取全局wlan结构
 * 返 回 值  : 初始化返回值，成功或失败原因
 */
struct wlan_pm_s *wlan_pm_get_drv_by_id(uint32_t dev_id)
{
#ifdef _PRE_DUAL_CHIP
    if (dev_id > HCC_EP_WIFI_DEV1) {
        oal_print_mpxx_log(MPXX_LOG_ERR, "get dev_id [%d] is out of range", dev_id);
...
EXPORT_SYMBOL_GPL(wlan_pm_get_drv_by_id);
...
STATIC void wlan_pm_host_pm_switch_init(void)
{
    ...
    if (l_cfg_value == WLAN_PM_ALL_ENABLE || l_cfg_value == WLAN_PM_LIGHT_SLEEP_SWITCH_EN ||
        l_cfg_value == WLAN_PM_ALL_ENABLE_HW_RX_BCN_EN) {
...
void wlan_pm_set_host_pm_switch(uint8_t switch_pm)
{
    if ((switch_pm == WLAN_PM_ALL_ENABLE) || (switch_pm == WLAN_PM_LIGHT_SLEEP_SWITCH_EN) ||
        (switch_pm == WLAN_PM_ALL_ENABLE_HW_RX_BCN_EN)) {
        g_wlan_host_pm_switch = OAL_TRUE;
        wlan_pm_enable();
    } else {
        wlan_pm_disable();
...
OAL_STATIC uint32_t wlan_pm_sleep_request(struct wlan_pm_s *pst_wlan_pm)
{
    ...
    enum gpio_type type_gpio = HOST_WKUP_W;
#ifdef _PRE_DUAL_CHIP
    type_gpio = conn_get_gpio_type_by_id(pst_wlan_pm->pst_bus->dev_id, type_gpio);
#endif
    ...
    oal_print_mpxx_log(MPXX_LOG_INFO, "sleep request send, gpio_%d = %d",
                       conn_get_gpio_number_by_type(type_gpio), conn_get_gpio_level(type_gpio));
...
static int wlan_poweroff_notify_sys(struct notifier_block *this, unsigned long code, void *unused)
{
    ...
    if (conn_get_gpio_level(W_POWER) == GPIO_LEVEL_HIGH) {
        err = wlan_pm_close_dev(HCC_EP_WIFI_DEV);
        if (err != OAL_SUCC) {
...
#ifdef _PRE_DUAL_CHIP
    ps_print_info("wlan_poweroff_notify_sys enter chip2\n");
    if (conn_get_gpio_level(W_POWER_CHIP2) == GPIO_LEVEL_HIGH) {
        err = wlan_pm_close_dev(HCC_EP_WIFI_DEV1);
        if (err != OAL_SUCC) {
            ps_print_warning("wlan_poweroff_notify_sys call chip2 wlan_pm_close failed (err=%d)\n", err);
        }
    } else {
        ps_print_info("chip2 wlan already poweroff no call wlan_pm_close\n");
    }
#endif
...
/*
 * Function: wlan_pm_suspend_proc
 * Description: if force_sleep, wait till wlan sleep
 * Ruturn: 0 -- success
 */
OAL_STATIC int32_t wlan_pm_suspend_proc(pm_notify_param* param, void* data)
{
    ktime_t time_start, time_cur;
    uint64_t trans_ms;
    int loop = PLAT_SUSPEND_TIMEOUT / PLATFORM_SLEEP_TIME;
    struct wlan_pm_s *pm_data = (struct wlan_pm_s *)data;
    struct hcc_handler *hcc;

    if (pm_data == NULL || param == NULL) {
        return -OAL_FAIL;
    }
    hcc = hcc_get_handler(hcc_get_dev_id(pm_data->pst_bus));
    ...
    for (; loop > 0; loop--) {
        ...
        // wakelock锁已释放
        if (oal_wakelock_active(&pm_data->st_pm_wakelock) == OAL_FALSE) {
            if (param->force_sleep == OAL_TRUE) {
                oal_wake_unlock_force(&pm_data->st_delaysleep_wakelock);
                ps_print_info("- support force sleep, delaysleep_wakelock status:%d",
                              oal_wakelock_active(&pm_data->st_delaysleep_wakelock));
            }
            hcc_tx_transfer_unlock(hcc);
            ps_print_info("- already sleep\n");
            return OAL_SUCC;
        } else {
            if (param->force_sleep == OAL_FALSE) {
                hcc_tx_transfer_unlock(hcc);
                ps_print_info("- not support force sleep\n");
                return -OAL_FAIL;
            }
            // 强制睡眠, 提交sleep work
            hcc_tx_transfer_unlock(hcc);
            time_cur = ktime_get();
            trans_ms = (uint64_t)ktime_to_ms(ktime_sub(time_cur, time_start));
            if (trans_ms > PLAT_SUSPEND_TIMEOUT) {
                break;
            }
            wlan_pm_work_submit(pm_data, &pm_data->st_sleep_work);
            msleep(PLATFORM_SLEEP_TIME);
        }
    }

    ps_print_info("- sleep timeout\n");
    return -OAL_FAIL;
}

OAL_STATIC int32_t wlan_pm_resume_proc(pm_notify_param* param, void* data)
{
    struct wlan_pm_s *pm_data = (struct wlan_pm_s *)data;
    if (pm_data == NULL || param == NULL) {
        return -OAL_FAIL;
...
OAL_STATIC int wlan_pm_board_notify_func(unsigned long event, pm_notify_param* param, void *data)
{
    int32_t ret = OAL_SUCC;
    switch (event) {
        case PM_SUSPEND_PREPARE:
            ret = wlan_pm_suspend_proc(param, data);
            break;
        case PM_HIBERNATION_PREPARE:
            break;
        case PM_POST_SUSPEND:
            ret = wlan_pm_resume_proc(param, data);
            break;
...
pm_notify_node g_wlan_pm_nb = {
    .name = "wlan_pm_nb",
    .priority = PLAT_NOITFY_PRIO,
    .cb_func = wlan_pm_board_notify_func,
};

#ifndef BFGX_UART_DOWNLOAD_SUPPORT
OAL_STATIC void hcc_message_register_all(struct wlan_pm_s *pst_wlan_pm)
{
    struct hcc_handler *hcc = hcc_get_handler(hcc_get_dev_id(pst_wlan_pm->pst_bus));
    hcc_message_register(hcc,
                         D2H_MSG_WAKEUP_SUCC,
...
STATIC void wlan_pm_wake_dev_time_init(void)
{
    int32_t cfg_value = 0;
    int32_t ret;

    /* 获取ini的配置值 */
    ret = get_cust_conf_int32(INI_MODU_WIFI, WLAN_WAKE_DEV_MIN_TIME_INI, &cfg_value);
    if (ret == INI_FAILED) {
        ps_print_info("no %s, keep default %u\n", WLAN_WAKE_DEV_MIN_TIME_INI, g_wake_dev_min_time);
        return;
    }

    g_wake_dev_min_time = (uint32_t)cfg_value;
    ps_print_info("g_wake_dev_min_time: %u\n",  g_wake_dev_min_time);
...
#ifndef BFGX_UART_DOWNLOAD_SUPPORT
    hcc_message_register_all(pst_wlan_pm);
...
#endif
    g_wlan_pm_nb.data = (void*)pst_wlan_pm;
    board_pm_register_notify_cb(&g_wlan_pm_nb);
...
#ifdef _PRE_DUAL_CHIP
pm_notify_node g_wlan_pm_nb_mpxx_dev1 = {
    .name = "wlan_pm_nb_dev1",
    .priority = PLAT_NOITFY_PRIO,
    .cb_func = wlan_pm_board_notify_func,
};

/*
 * 函 数 名  : mpxx_dev1_wlan_pm_init
 * 功能描述  : WLAN PM初始化接口
 * 返 回 值  : 初始化返回值，成功或失败原因
 */
struct wlan_pm_s *mpxx_dev1_wlan_pm_init(void)
{
    struct wlan_pm_s *pst_wlan_pm = NULL;
    int32_t ret;
    uint32_t dev_id = HCC_EP_WIFI_DEV1;

    pst_wlan_pm = kzalloc(sizeof(struct wlan_pm_s), GFP_KERNEL);
    if (pst_wlan_pm == NULL) {
        oal_print_mpxx_log(MPXX_LOG_INFO, "no mem to allocate wlan_pm_data[%d]", dev_id);
        return NULL;
    }

    memset_s(pst_wlan_pm, sizeof(struct wlan_pm_s), 0, sizeof(struct wlan_pm_s));
#ifndef BFGX_UART_DOWNLOAD_SUPPORT
    pst_wlan_pm->pst_bus = hcc_get_bus(dev_id);
    if (pst_wlan_pm->pst_bus == NULL) {
        oal_print_mpxx_log(MPXX_LOG_INFO, "hcc[%d] bus is NULL, failed to create wlan_pm_wq!", dev_id);
        kfree(pst_wlan_pm);
        return NULL;
    }

    pst_wlan_pm->pst_bus->pst_pm_callback = &g_wlan_pm_callback_mpxx_dev1;
#endif
    ...
    g_wlan_pm_nb_mpxx_dev1.data = (void*)pst_wlan_pm;
    board_pm_register_notify_cb(&g_wlan_pm_nb_mpxx_dev1);

    g_wlan_pm_info_mpxx_dev1 = pst_wlan_pm;

    oal_print_mpxx_log(MPXX_LOG_INFO, "wlan_pm_init[%d] ok!", dev_id);
    return pst_wlan_pm;
}
#endif

/*
 * 函 数 名  : wlan_pm_delete
 * 功能描述  : 释放wlan_pm_s
 * 返 回 值  : 无
 */
OAL_STATIC void wlan_pm_delete(void)
{
    ...
    oal_wake_lock_exit(&pst_wlan_pm->st_pm_wakelock);
    oal_wake_lock_exit(&pst_wlan_pm->st_delaysleep_wakelock);
...
OAL_STATIC int32_t wlan_pm_custom_cali_waitretry(struct wlan_pm_s *pst_wlan_pm)
{
    ...
    hcc_sched_transfer(hcc_get_handler(hcc_get_dev_id(pst_wlan_pm->pst_bus)));
...
OAL_STATIC int32_t wlan_pm_custom_cali(struct wlan_pm_s *pst_wlan_pm)
{
    ...
    if (g_custom_process_func.p_custom_cali_func() != OAL_SUCC) {
        oam_warning_log0(0, OAM_SF_CFG, "wlan_pm_open::custom_cali_func fail");
        return OAL_SUCC;
    }
    wait_ret = oal_wait_for_completion_killable_timeout(
        &pst_wlan_pm->pst_bus->st_device_ready, oal_msecs_to_jiffies(HOST_WAIT_BOTTOM_INIT_TIMEOUT));
    oam_warning_log1(0, OAM_SF_CFG, "wlan_pm_open:: Total time = %d",
        oal_jiffies_to_msecs(oal_read_once(OAL_TIME_JIFFY) - g_hcc_ready_msg_time));
...
OAL_STATIC int32_t wlan_pm_open_check(struct wlan_pm_s *pst_wlan_pm)
{
    ...
    if (off_find_get_status() == OAL_TRUE) {
        oam_warning_log0(0, OAM_SF_PWR, "in off find status, not allow open\n");
        return OAL_FAIL;
    }
...
int32_t wlan_wait_device_reg_init(struct wlan_pm_s *wlan_pm_info)
{
    uint64_t host_va = 0;
    uint32_t cnt = 0;
    if (oal_pcie_devca_to_hostva(hcc_get_dev_id(wlan_pm_info->pst_bus), 0x401052B4, &host_va) != OAL_SUCC) {
 ...
 * 返 回 值  : 初始化返回值，成功或失败原因
 */
OAL_STATIC int32_t wlan_pm_open(oal_bool_enum with_cali)
{
    int32_t ret;
    struct wlan_pm_s *pst_wlan_pm = wlan_pm_get_drv();
    ...
    if (hi110x_is_emu() == OAL_TRUE) {
        if (wlan_wait_device_reg_init(pst_wlan_pm) != OAL_SUCC) {
            return OAL_FAIL;
        }
    }
...
int32_t wlan_pm_open_dev(uint32_t dev_id, oal_bool_enum with_cali)
{
    int32_t ret;
    struct pm_top* pm_top_data = pm_get_top();
    mutex_lock(&(pm_top_data->host_mutex));
#ifdef _PRE_DUAL_CHIP
    hcc_main_dev_id_lock();
    hcc_set_main_dev(dev_id);
#endif
    ret = wlan_pm_open(with_cali);
#ifdef _PRE_DUAL_CHIP
    hcc_set_main_dev(HCC_EP_WIFI_DEV);
    hcc_main_dev_id_unlock();
#endif
    mutex_unlock(&(pm_top_data->host_mutex));
    return ret;
}
EXPORT_SYMBOL_GPL(wlan_pm_open_dev);
...
OAL_STATIC uint32_t wlan_pm_close_pre(struct wlan_pm_s *pst_wlan_pm)
{
    ...
    if ((!pst_wlan_pm->apmode_allow_pm_flag) && (off_find_get_status() == OAL_FALSE)) {
    ...
    hcc_bus_rx_int_mask(hcc_get_bus(hcc_get_dev_id(pst_wlan_pm->pst_bus)));
...
OAL_STATIC uint32_t wlan_pm_close_after(struct wlan_pm_s *pst_wlan_pm)
{
    ...
    hcc_bus_rx_int_unmask(hcc_get_bus(hcc_get_dev_id(pst_wlan_pm->pst_bus)));
 ...
 * 返 回 值  : 初始化返回值，成功或失败原因
 */
OAL_STATIC uint32_t wlan_pm_close(void)
{
    struct wlan_pm_s *pst_wlan_pm = wlan_pm_get_drv();
    uint32_t ret;
    ...
    hcc_dev_flowctrl_on(hcc_get_handler(hcc_get_dev_id(pst_wlan_pm->pst_bus)), 0);
...
uint32_t wlan_pm_close_dev(uint32_t dev_id)
{
    uint32_t ret;
    struct pm_top* pm_top_data = pm_get_top();
    mutex_lock(&(pm_top_data->host_mutex));
...
EXPORT_SYMBOL_GPL(wlan_pm_close_dev);
...
void wlan_pm_close_for_excp(void)
{
    ...
    pst_wlan_pm->pst_bus->pst_pm_callback->pm_disable(OAL_FALSE);
...
uint32_t wlan_pm_close_dev_by_shutdown(uint32_t dev_id)
{
    uint32_t ret;
    hcc_main_dev_id_lock();
    hcc_set_main_dev(dev_id);
    ret = wlan_pm_close_by_shutdown();
    hcc_set_main_dev(HCC_EP_WIFI_DEV);
    hcc_main_dev_id_unlock();
    return ret;
}
...
uint32_t wlan_pm_enable(void)
{
    ...
    hcc_tx_transfer_lock(hcc_get_handler(hcc_get_dev_id(pst_wlan_pm->pst_bus)));

    if (pst_wlan_pm->wlan_pm_enable == OAL_TRUE) {
        oam_warning_log0(0, OAM_SF_PWR, "wlan_pm_enable already enabled!");
        hcc_tx_transfer_unlock(hcc_get_handler(hcc_get_dev_id(pst_wlan_pm->pst_bus)));
        return OAL_SUCC;
    }

    pst_wlan_pm->wlan_pm_enable = OAL_TRUE;

    pst_wlan_pm->pst_bus->pst_pm_callback->pm_feed_wdg();
...
uint32_t wlan_pm_disable(void)
{
    ...
    if (pst_wlan_pm->pst_bus->pst_pm_callback->pm_disable != NULL) {
        return pst_wlan_pm->pst_bus->pst_pm_callback->pm_disable(OAL_TRUE);
    }
...
oal_uint wlan_pm_init_dev(void)
{
    ...
    pst_bus = hcc_get_bus(hcc_get_dev_id(pst_wlan_pm->pst_bus));
...
OAL_STATIC oal_uint wlan_pm_pcie_wakeup_cmd(struct wlan_pm_s *pst_wlan_pm)
{
    ...
    oal_atomic_set(&pst_wlan_pm->pst_bus->wakeup_dev_wait_ack, 1);
...
OAL_STATIC oal_uint wlan_pm_gpio_wakeup_dev(struct wlan_pm_s *pst_wlan_pm)
{
    ...
    for (retry = 0; retry < WLAN_SDIO_MSG_RETRY_NUM; retry++) {
        ...
        oal_wlan_gpio_intr_enable(hbus_to_dev(pst_wlan_pm->pst_bus), OAL_TRUE);
#ifdef _PRE_DUAL_CHIP
        board_host_wakeup_dev_set_by_id(pst_wlan_pm->pst_bus->dev_id, 0); /* wakeup dev */
        oal_udelay(100);  // delay 100us
        board_host_wakeup_dev_set_by_id(pst_wlan_pm->pst_bus->dev_id, 1); /* wakeup dev */
#else
...
OAL_STATIC oal_uint wlan_pm_wakeup_cmd_send(struct wlan_pm_s *pst_wlan_pm)
{
    ...
    if (is_dfr_test_en(WIFI_WKUP_FAULT, (uintptr_t)hcc_get_handler(hcc_get_dev_id(pst_wlan_pm->pst_bus)))) {
        cancel_excp_test_en(WIFI_WKUP_FAULT, (uintptr_t)hcc_get_handler(hcc_get_dev_id(pst_wlan_pm->pst_bus)));
        oam_warning_log0(0, OAM_SF_PWR, "[dfr_test] trigger wkup fail!\n");
...
OAL_STATIC uint32_t wlan_pm_wakeup_ack_timeout(struct wlan_pm_s *pst_wlan_pm)
{
 ...
 */
OAL_STATIC uint32_t wlan_pm_wakeup_wait_ack(struct wlan_pm_s *pst_wlan_pm)
{
    uint32_t ret;
    int32_t type_gpio = pst_wlan_pm->pst_bus->bus_dev->wkup_host_io_type;
    ret = oal_wait_for_completion_timeout(&pst_wlan_pm->st_wakeup_done,
                                          (uint32_t)oal_msecs_to_jiffies(WLAN_WAKUP_MSG_WAIT_TIMEOUT));
    if (ret == 0) {
        if ((conn_gpio_valid(type_gpio) == OAL_TRUE) && (conn_get_gpio_level(type_gpio) == GPIO_LEVEL_HIGH)) {
            declare_dft_trace_key_info("wlan_wakeup_ack irq loss", OAL_DFT_TRACE_EXCEP);
        } else {
            return wlan_pm_wakeup_ack_timeout(pst_wlan_pm);
        }
    }
...
OAL_STATIC void wlan_pm_wakeup_dev_ack(void)
{
    ...
    if (oal_atomic_read(&pst_wlan_pm->pst_bus->wakeup_dev_wait_ack)) {
        pst_wlan_pm->wakeup_dev_ack++;

        oal_complete(&pst_wlan_pm->st_wakeup_done);

        oal_atomic_set(&pst_wlan_pm->pst_bus->wakeup_dev_wait_ack, 0);
    }
...
#ifdef _PRE_DUAL_CHIP
/*
 * 函 数 名  : mpxx_dev1_wlan_pm_disable_check_wakeup
 * 功能描述  : 去使能wlan平台低功耗
 * 返 回 值  : 初始化返回值，成功或失败原因
 */
OAL_STATIC uint32_t mpxx_dev1_wlan_pm_disable_check_wakeup(int32_t flag)
{
    struct wlan_pm_s *pst_wlan_pm = wlan_pm_get_drv_by_id(HCC_EP_WIFI_DEV1);
    if (pst_wlan_pm == NULL) {
        oam_error_log0(0, OAM_SF_PWR, "mpxx_dev1_wlan_pm_disable_check_wakeup::pst_wlan_pm is NULL");
        return OAL_FAIL;
    }

    hcc_tx_transfer_lock(hcc_get_handler(HCC_EP_WIFI_DEV1));

    if (pst_wlan_pm->wlan_pm_enable == OAL_FALSE) {
        oam_warning_log0(0, OAM_SF_PWR, "mpxx_dev1_wlan_pm_disable_check_wakeup already disabled!");
        hcc_tx_transfer_unlock(hcc_get_handler(HCC_EP_WIFI_DEV1));
        return OAL_SUCC;
    }

    if (flag == OAL_TRUE) {
        if (mpxx_dev1_wlan_pm_wakeup_dev() != OAL_SUCC) {
            oam_warning_log1(0, OAM_SF_PWR, "pm wake up dev[%d] fail!", HCC_EP_WIFI_DEV1);
        }
    }
    ...
    oam_warning_log0(0, OAM_SF_PWR, "mpxx_dev1_wlan_pm_disable_check_wakeup hold deepsleep_wakelock SUCC!");

    return OAL_SUCC;
}

/*
 * 函 数 名  : mpxx_dev1_wlan_pm_wakeup_dev
 * 功能描述  : 唤醒device
 * 返 回 值  : 初始化返回值，成功或失败原因
 */
OAL_STATIC oal_uint mpxx_dev1_wlan_pm_wakeup_dev(void)
{
    ...
    uint64_t trans_us;
    int i;
    enum gpio_type con_gpio_type = HOST_WKUP_W;
    struct wlan_pm_s *pst_wlan_pm = wlan_pm_get_drv_by_id(HCC_EP_WIFI_DEV1);

    if (pst_wlan_pm == NULL) {
        oal_print_mpxx_log(MPXX_LOG_ERR, "mpxx_dev1_wlan_pm_wakeup_dev::pst_wlan_pm is NULL");
        return OAL_FAIL;
    }
    ...
    trans_us = (uint64_t)ktime_to_us(ktime_sub(time_stop, time_start));
#ifdef _PRE_DUAL_CHIP
    con_gpio_type = conn_get_gpio_type_by_id(pst_wlan_pm->pst_bus->dev_id, HOST_WKUP_W);
#endif
    oal_print_mpxx_log(MPXX_LOG_INFO, "wakeup dev[%d] succ, gpio_%d = %d, cost %llu us", pst_wlan_pm->pst_bus->dev_id,
                       conn_get_gpio_number_by_type(con_gpio_type), conn_get_gpio_level(con_gpio_type), trans_us);
    return OAL_SUCC;
}
...
OAL_STATIC void mpxx_dev1_wlan_pm_wakeup_dev_ack(void)
{
    ...
    if (oal_warn_on(pst_wlan_pm == NULL)) {
        oam_error_log0(0, OAM_SF_PWR, "mpxx_dev1_wlan_pm_wakeup_dev_ack pst_wlan_pm is null \n");
        return;
    }
...
/*
 * 函 数 名  : mpxx_dev1_wlan_pm_wakeup_host
 * 功能描述  : device唤醒host
 * 返 回 值  : 初始化返回值，成功或失败原因
 */
OAL_STATIC oal_uint mpxx_dev1_wlan_pm_wakeup_host(void)
{
    struct wlan_pm_s *pst_wlan_pm = wlan_pm_get_drv_by_id(HCC_EP_WIFI_DEV1);
    if (oal_warn_on(pst_wlan_pm == NULL)) {
        oam_error_log0(0, OAM_SF_PWR, "mpxx_dev1_wlan_pm_wakeup_host: st_wlan_pm is null \n");
        return OAL_FAIL;
    }
    ...
    wlan_pm_idle_sleep_vote(DISALLOW_IDLESLEEP);
    oam_info_log0(0, OAM_SF_PWR, "mpxx_dev1_wlan_pm_wakeup_host get wakelock\n");
...
/*
 * 函 数 名  : mpxx_dev1_wlan_pm_state_get
 * 功能描述  : 获取pm的sleep状态
...
OAL_STATIC oal_uint mpxx_dev1_wlan_pm_state_get(void)
{
    struct wlan_pm_s *pst_wlan_pm = wlan_pm_get_drv_by_id(HCC_EP_WIFI_DEV1);
    if (pst_wlan_pm == NULL) {
        oam_warning_log0(0, OAM_SF_PWR, "mpxx_dev1_wlan_pm_state_get pst_wlan_pm is null");
        return OAL_FAIL;
    }
...
/*
 * 函 数 名  : mpxx_dev1_wlan_pm_feed_wdg
 * 功能描述  : 启动睡眠定时器
 * 返 回 值  : 初始化返回值，成功或失败原因
 */
OAL_STATIC void mpxx_dev1_wlan_pm_feed_wdg(void)
{
    struct wlan_pm_s *pst_wlan_pm = wlan_pm_get_drv_by_id(HCC_EP_WIFI_DEV1);
    if (pst_wlan_pm == NULL) {
        oam_warning_log0(0, OAM_SF_PWR, "mpxx_dev1_wlan_pm_feed_wdg:: pst_wlan_pm is null");
        return;
    }
...
/*
 * 函 数 名  : mpxx_dual_wlan_pm_set_timeout
 * 功能描述  : 睡眠定时器超时时间设置
 * 返 回 值  : 初始化返回值，成功或失败原因
 */
void mpxx_dual_wlan_pm_set_timeout(uint32_t dev_id, uint32_t timeout)
{
    struct wlan_pm_s *pst_wlan_pm = wlan_pm_get_drv_by_id(dev_id);
    if (pst_wlan_pm == NULL) {
        oam_warning_log1(0, OAM_SF_PWR, "mpxx_dual_wlan_pm_set_timeout [%d] pst_wlan_pm is null", dev_id);
        return;
    }

    if (dev_id == HCC_EP_WIFI_DEV1) {
        oam_info_log2(0, OAM_SF_PWR, "mpxx_dual_wlan_pm_set[%d] timeout[%d]", pst_wlan_pm->pst_bus->dev_id, timeout);
    ...
    wlan_pm_set_timeout(timeout);
}
EXPORT_SYMBOL_GPL(mpxx_dual_wlan_pm_set_timeout);
#endif
...
void wlan_pm_wakeup_dev_pcie_switch(void)
{
    ...
    if (oal_unlikely(ret != OAL_SUCC)) {
        declare_dft_trace_key_info("wlan_pm_wakeup_dev_pcie_switch_fail", OAL_DFT_TRACE_FAIL);
    }
}
...
uint32_t wlan_pm_wakeup_dev_lock(void)
{
    ...
    wlan_pm->pst_bus->pst_pm_callback->pm_feed_wdg();
    hcc_tx_transfer_lock(hcc_get_handler(hcc_get_dev_id(wlan_pm->pst_bus)));
    ret = wlan_pm_wakeup_dev();
    if (oal_unlikely(ret != OAL_SUCC)) {
        oam_error_log0(0, 0, "{wlan_pm_wakeup_dev_lock::wakeup dev failed!}");
    }

    hcc_tx_transfer_unlock(hcc_get_handler(hcc_get_dev_id(wlan_pm->pst_bus)));
...
{
    oal_uint ret;
    struct wlan_pm_s *pst_wlan_pm = container_of(pst_worker, struct wlan_pm_s, st_wakeup_work);
    struct hcc_handler *hcc = hcc_get_handler(hcc_get_dev_id(pst_wlan_pm->pst_bus));
    ...
    ret = pst_wlan_pm->pst_bus->pst_pm_callback->pm_wakeup_dev();
    if (oal_unlikely(ret != OAL_SUCC)) {
        declare_dft_trace_key_info("wlan_pm_wakeup_work_fail", OAL_DFT_TRACE_FAIL);
    }
...
int32_t wlan_pm_wakeup_done_callback(void *data)
{
    ...
    pst_wlan_pm->pst_bus->pst_pm_callback->pm_wakeup_dev_ack();
...
int32_t wlan_pm_close_done_callback(void *data)
{
    ...
    hcc_bus_disable_state(hcc_get_bus(hcc_get_dev_id(pst_wlan_pm->pst_bus)), OAL_BUS_STATE_RX);
 ...
 */
OAL_STATIC uint32_t wlan_pm_sleep_cmd_send(struct wlan_pm_s *pst_wlan_pm)
{
    int32_t ret;
    uint8_t retry;
...
#ifdef _PRE_DUAL_CHIP
    hcc_bus *hi_bus = hcc_get_bus(pst_wlan_pm->pst_bus->dev_id);
#else
...
OAL_STATIC uint32_t  wlan_pm_sleep_cmd_proc(struct wlan_pm_s *pst_wlan_pm)
{
    ...
    if (pst_wlan_pm->packet_total_cnt != 0) {
        oal_wake_lock_timeout(&pst_wlan_pm->st_delaysleep_wakelock, g_delaysleep_time);
        oal_print_mpxx_log(MPXX_LOG_INFO, "wifi have traffic, hold deepsleep_wakelock %d ms, gpio_%d = %d",
                           g_delaysleep_time, conn_get_gpio_number_by_type(type_gpio),
                           conn_get_gpio_level(type_gpio));
    } else {
...
#define SLEEP_DELAY_THRESH      0
    uint32_t ret;
    struct wlan_pm_s *pst_wlan_pm = container_of(pst_worker, struct wlan_pm_s, st_sleep_work);
    struct hcc_handler *hcc = hcc_get_handler(hcc_get_dev_id(pst_wlan_pm->pst_bus));
    ...
    /* 总线无数据，可降至低速；确保业务执行正常，切速成功后延后一次进入深睡 */
...
static void wlan_pm_wdg_timeout_print_debug_info(struct wlan_pm_s *pm_data)
{
    ...
    oal_print_mpxx_log(MPXX_LOG_DBG, "tid not empty[%d],ring host[%d]+mpdu num[%d], m ddr[%d], s ddr[%d], forbid[%d]",
        pm_data->tid_not_empty_cnt, pm_data->tid_ring_host_cnt, pm_data->ring_has_mpdu_cnt,
        pm_data->master_ddr_rx_cnt, pm_data->slave_ddr_rx_cnt,
        pm_data->host_forbid_sleep_cnt);
#else
    oal_print_mpxx_log(MPXX_LOG_WARN, "pkt_num[%d],pm_en[%ld],timeout_cnt[%d],have_pkt[%d],sleep_work_submit[%d]",
        pm_data->packet_total_cnt, pm_data->wlan_pm_enable, pm_data->wdg_timeout_cnt,
        pm_data->have_packet, pm_data->sleep_work_submit);
    oal_print_mpxx_log(MPXX_LOG_WARN, "tid not empty[%d],ring host[%d]+mpdu num[%d], m ddr[%d], s ddr[%d], forbid[%d]",
        pm_data->tid_not_empty_cnt, pm_data->tid_ring_host_cnt, pm_data->ring_has_mpdu_cnt,
        pm_data->master_ddr_rx_cnt, pm_data->slave_ddr_rx_cnt,
        pm_data->host_forbid_sleep_cnt);
#endif
...
#endif
{
    ...
    hcc_bus_performance_core_schedule(hcc_get_dev_id(pm_data->pst_bus));
    ...
    pm_data->pst_bus->pst_pm_callback->pm_feed_wdg();
...
void wlan_pm_dump_host_info(void)
{
    ...
    oal_io_print("wakeup_dev_wait_ack:%d\n", oal_atomic_read(&wlan_pm_info->pst_bus->wakeup_dev_wait_ack));
    ...
    oal_io_print("tid_ring_host_cnt:%d\n", wlan_pm_info->tid_ring_host_cnt);
...
void wlan_pm_info_clean(void)
{
    ...
    wlan_pm_info->tid_ring_host_cnt = 0;
...
/*
 * 函 数 名  : wlan_pm_log_test
 * 功能描述  : debug, 测试wifi日志接口
 */
void wlan_pm_log_test(int32_t number)
{
    int32_t i;
    for (i = 0; i < number; i++) {
        oam_warning_log1(0, OAM_SF_PWR, "wlan_pm_log_test [%d]", i);
    }
}

void wlan_pm_set_host_timeout(uint32_t dev_id, uint32_t timeout)
{
#ifdef _PRE_DUAL_CHIP
    mpxx_dual_wlan_pm_set_timeout(dev_id, timeout);
#else
    wlan_pm_set_timeout(timeout);
#endif
}
EXPORT_SYMBOL_GPL(wlan_pm_set_host_timeout);
