...
OAL_STATIC void sdt_drv_netlink_recv(oal_netbuf_stru *pst_netbuf)
{
...
#ifdef SDT_CHECK_APP_NET_ADMIN
    if (oal_netlink_capable(pst_netbuf, CAP_NET_ADMIN) != true) {
        oal_io_print("sdt_drv_netlink_recv::app permissions error!\n");
        return;
    }
#endif
    ...
    ret = memcpy_s((void *)&st_msg_hdr, sizeof(st_msg_hdr),
                   (const void *)g_sdt_drv_mng_entry.puc_data, (uint32_t)sizeof(st_msg_hdr));
    if (ret != EOK) {
