...
#define dpd_size(n)                (sizeof(mp1x_wlan_cali_dpd_para_stru) * (n))
...
#define POW_DATA_PREFIX             g_conn_nve_struct.conn_nve_ramdisk->pow_cal_data
    // DPD数据单元
#define conn_nve_dpd_unit(pre, array, info) \
...
// TAS nv项信息
#define CONN_NVE_TAS_NUM   368
#define CONN_NVE_TAS_NAME  "TASDPN"
#define WICAL_TAS_BW_NUM   4
#endif
/* 返回值定义 */
typedef enum {
...
#ifdef _PRE_WLAN_FEATURE_TAS_ANT_SWITCH
typedef struct {
    wlan_cali_pow_para_stru delta_gain_tas_5g_20[WICAL_5G_RF_BAND_NUM];
    wlan_cali_pow_para_stru delta_gain_tas_5g_40[WICAL_5G_RF_BAND_NUM];
    wlan_cali_pow_para_stru delta_gain_tas_5g_80[WICAL_5G_RF_BAND_NUM];
    wlan_cali_pow_para_stru delta_gain_tas_5g_160[WICAL_5G_RF_BAND_NUM];
} nv_tas_pow_stru;
#endif
typedef struct {
...
typedef struct {
    // 2G校准数据
    mp1x_wlan_cali_dpd_para_stru cali_data_2g[WICAL_STREAM_NUM][WLAN_2G_CALI_BAND_NUM];
    // 5G校准数据
    mp1x_wlan_cali_dpd_para_stru cali_data_5g_20[WICAL_STREAM_NUM][WLAN_5G_20M_CALI_BAND_NUM];
    mp1x_wlan_cali_dpd_para_stru cali_data_5g_80[WICAL_STREAM_NUM][WLAN_5G_20M_CALI_BAND_NUM];
    mp1x_wlan_cali_dpd_para_stru cali_data_5g_160[WICAL_STREAM_NUM][WLAN_5G_160M_CALI_BAND_NUM];
    // 6G校准数据
#ifdef _PRE_WLAN_FEATURE_6G_EXTEND
    mp1x_wlan_cali_dpd_para_stru cali_data_6g_20[WICAL_STREAM_NUM][WLAN_6G_20M_CALI_BAND_NUM];
    mp1x_wlan_cali_dpd_para_stru cali_data_6g_80[WICAL_STREAM_NUM][WLAN_6G_20M_CALI_BAND_NUM];
    mp1x_wlan_cali_dpd_para_stru cali_data_6g_160[WICAL_STREAM_NUM][WLAN_6G_160M_CALI_BAND_NUM];
#endif
} nv_dpd_stru;

typedef struct {
    int16_t fb_gain[HAL_MAX_BAND_NUM];
    uint32_t crc;
} nv_fbgain_stru;
...
#ifdef _PRE_WLAN_FEATURE_TAS_ANT_SWITCH
typedef struct {
    nv_tas_pow_stru tas_pow_cal_data;
} conn_nve_tas_particion_stru;
#endif
...
typedef enum {
    CONN_DPD_CALI_SET_CHN,
    CONN_DPD_CALI_SAVE_RAM,
    CONN_DPD_CALI_GET_CHN,
    CONN_DPD_CALI_SAVE_NV,
    CONN_DPD_CALI_CLEAR_RAM,
    CONN_DPD_CALI_BUT,
} hisi_conn_dpd_cali_step;
