...
int32_t hwifi_get_wifi_config_priv_val(int32_t l_cfg_id, int32_t *value)
{
    return get_cust_conf_int32(INI_MODU_WIFI, g_wifi_config_priv[l_cfg_id].name, value);
}
#endif
...
int32_t hwifi_get_init_priv_value(int32_t l_cfg_id, int32_t *pl_priv_value)
{
    ...
    if (hcc_get_main_dev() == HCC_EP_WIFI_DEV1) {
        return hwifi_get_init_priv_value_dev1(l_cfg_id, pl_priv_value);
    }
#endif
...
#ifdef _PRE_DUAL_CHIP
    oal_io_print("hwifi_custom_adapt_priv_ini_param::g_wlan_open_cnt_dev1[%d]", g_wlan_open_cnt_dev1);
#endif
}
...
#ifdef _PRE_DUAL_CHIP
    if (hcc_get_main_dev() == HCC_EP_WIFI_DEV1) {
        *priv_cfg_value = (g_custom_cali_done_mpxx_dev1 == OAL_FALSE) ? OAL_FALSE : *priv_cfg_value;
        oal_io_print("hwifi_custom_adapt_priv_ini_param::g_custom_cali_done_mpxx_dev1[%d]\
                     auto_cali_mask[0x%x].\r\n", g_custom_cali_done_mpxx_dev1, *priv_cfg_value);
        return;
    }
#endif
