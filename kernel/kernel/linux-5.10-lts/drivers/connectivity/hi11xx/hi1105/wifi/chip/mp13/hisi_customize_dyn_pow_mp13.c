...
int32_t hwifi_get_nvram_pro_line_config_ini_name(uint32_t cfg_id, uint8_t *nv_pa_params)
{
    return get_cust_conf_string(INI_MODU_WIFI, g_nvram_pro_line_config_ini[cfg_id].name,
...
OAL_STATIC void hwifi_clear_cust_nvram_info(void)
{
#ifdef _PRE_DUAL_CHIP
    if (hcc_get_main_dev() == HCC_EP_WIFI_DEV1) {
        hwifi_clear_cust_nvram_info_dev1();
        return;
    }
#endif
...
#ifdef _PRE_DUAL_CHIP
    if (hcc_get_main_dev() == HCC_EP_WIFI_DEV1) {
        return hwifi_custom_set_nvram_data_dev1(cust_nvram_info);
    }
#endif
...
OAL_STATIC void hwifi_set_g_fact_cali_completed(oal_bool_enum_uint8 val)
{
#ifdef _PRE_DUAL_CHIP
    if (hcc_get_main_dev() == HCC_EP_WIFI_DEV1) {
        hwifi_set_g_fact_cali_completed_dev1(val);
    }
#endif
    g_en_fact_cali_completed = val;
}
...
#ifdef _PRE_DUAL_CHIP
    if (hcc_get_main_dev() == HCC_EP_WIFI_DEV1) {
        return hwifi_get_g_fact_cali_completed_dev1();
    }
#endif
...
int32_t hwifi_custom_host_read_dyn_cali_nvram(void)
{
    ...
    hwifi_set_g_fact_cali_completed(tmp_en_fact_cali_completed);
...
#ifdef _PRE_DUAL_CHIP
    if (hcc_get_main_dev() == HCC_EP_WIFI_DEV1) {
        return hwifi_get_nvram_param_dev1(nvram_param_idx);
    }
#endif
...
OAL_STATIC void hwifi_config_get_5g_curv_switch_point(uint8_t *puc_ini_pa_params, uint32_t cfg_id)
{
    ...
    /* 计算5g曲线switch point */
    for (param_idx = 0; param_idx < us_ini_param_num; param_idx++) {
        *(ps_extre_point_val + param_idx) = (int16_t)hwifi_dyn_cali_get_extre_point(l_ini_params +
                                               uc_secon_ratio_idx);
        oal_io_print("hwifi_config_get_5g_curv_switch_point::extre power[%d] param_idx[%d] cfg_id[%d]!\r\n",
                     *(ps_extre_point_val + param_idx), param_idx, cfg_id);
                                                                ...
                                                                int16_t *ps_5g_delt_power)
{
    ...
    us_nv_param_num /= DY_CALI_PARAMS_TIMES;
    /* 检查nv和ini中二次系数是否匹配 */
    for (param_idx = 0; param_idx < us_nv_param_num; param_idx++) {
        ...
        if (l_ini_params[uc_secon_ratio_idx] != l_nv_params[uc_secon_ratio_idx]) {
            oam_warning_log4(0, OAM_SF_CUSTOM, "hwifi_config_nvram_second_coefficient_check::nvram get mismatch value \
                idx[%d %d] val are [%d] and [%d]!", param_idx, uc_secon_ratio_idx, l_ini_params[uc_secon_ratio_idx],
        ...
        if ((cfg_id == WLAN_CFG_DTS_NVRAM_RATIO_PA5GA0) || (cfg_id == WLAN_CFG_DTS_NVRAM_RATIO_PA5GA1)) {
            /* 计算产线上的delt power */
            *(ps_5g_delt_power + param_idx) = hwifi_get_5g_pro_line_delt_pow_per_band(
            ...
            oal_io_print("hwifi_config_nvram_second_coefficient_check::delt power[%d] param_idx[%d] cfg_id[%d]!\r\n",
                         *(ps_5g_delt_power + param_idx), param_idx, cfg_id);
        }
        uc_secon_ratio_idx += DY_CALI_PARAMS_TIMES;
    }
...
static int32_t hwifi_config_get_cust_string(uint32_t cfg_id, uint8_t *nv_pa_params)
{
    ...
    if (get_cust_conf_string(INI_MODU_WIFI, g_nvram_pro_line_config_ini[cfg_id].name,
        ...
        if (oal_value_eq_any4(cfg_id, WLAN_CFG_DTS_NVRAM_RATIO_PA5GA0_LOW,
            ...
            if (get_cust_conf_string(INI_MODU_WIFI, g_nvram_pro_line_config_ini[cfg_id_tmp].name,
                nv_pa_params, CUS_PARAMS_LEN_MAX - 1) == INI_FAILED) {
...
#if (defined(HISI_CONN_NVE_SUPPORT) && (!defined(_PRE_CONFIG_READ_DYNCALI_E2PROM)) && (defined(CONFIG_NVE_AP_KERNEL) \
    || defined(CONFIG_HISILICON_PLATFORM))) || (defined(_PRE_CONFIG_READ_DYNCALI_E2PROM) && \
    defined(_PRE_CONFIG_READ_E2PROM_MAC))
/*
 * 函 数 名  : char2byte
...
int32_t hwifi_get_mac_addr(uint8_t *puc_buf)
{
    ...
    if (g_wifi_mac[0] != 0 || g_wifi_mac[1]  != 0 || g_wifi_mac[BYTE_OFFSET_2]  != 0 ||
        g_wifi_mac[BYTE_OFFSET_3] != 0 || g_wifi_mac[BYTE_OFFSET_4] != 0 || g_wifi_mac[BYTE_OFFSET_5] != 0) {
        if (memcpy_s(puc_buf, WLAN_MAC_ADDR_LEN, g_wifi_mac, WLAN_MAC_ADDR_LEN) != EOK) {
            return INI_FAILED;
        }
...
#ifdef _PRE_DUAL_CHIP
    hwifi_print_cfg_pro_line_params_dev1();
#endif
}
...
OAL_STATIC uint32_t hwifi_cfg_init_cus_dyn_cali(mac_cus_dy_cali_param_stru *puc_dyn_cali_param, int num)
{
    int32_t  val, ret;
    uint8_t  uc_idx = 0;
    uint8_t  rf_idx, dy_cal_param_idx;
    uint8_t  uc_cfg_id = WLAN_CFG_DTS_2G_CORE0_DPN_CH1;
    int8_t  *pc_ctx = NULL;

    for (rf_idx = 0; rf_idx < num; rf_idx++) {
        puc_dyn_cali_param->uc_rf_id = rf_idx;

        /* 动态校准二次项系数入参检查 */
        for (dy_cal_param_idx = 0; dy_cal_param_idx < DY_CALI_PARAMS_NUM; dy_cal_param_idx++) {
            if (!g_pro_line_params[rf_idx][dy_cal_param_idx].l_pow_par2) {
        ...
        ret = memcpy_s(puc_dyn_cali_param->al_dy_cali_base_ratio_params,
                       ...
                       g_pro_line_params[rf_idx], sizeof(puc_dyn_cali_param->al_dy_cali_base_ratio_params));
        ...
        ret += memcpy_s(puc_dyn_cali_param->al_dy_cali_base_ratio_ppa_params,
                        ...
                        &g_pro_line_params[rf_idx][CUS_DY_CALI_PARAMS_NUM],
                        sizeof(puc_dyn_cali_param->al_dy_cali_base_ratio_ppa_params));

        ret += memcpy_s(puc_dyn_cali_param->as_extre_point_val, sizeof(puc_dyn_cali_param->as_extre_point_val),
                        g_gs_extre_point_vals[rf_idx], sizeof(puc_dyn_cali_param->as_extre_point_val));
        ...
        hwifi_cfg_init_cus_dpn_cali(puc_dyn_cali_param, pc_ctx, rf_idx);
        ...
        /* 5G 160M DPN */
        if (hwifi_cfg_init_cus_5g_160m_dpn_cali(puc_dyn_cali_param, pc_ctx, rf_idx) != OAL_SUCC) {
            continue;
        }
        puc_dyn_cali_param++;
    }
...
#ifdef _PRE_DUAL_CHIP
    if (hcc_get_main_dev() == HCC_EP_WIFI_DEV1) {
        return hwifi_cfg_init_cus_dyn_cali_dev1(dy_cus_cali, WLAN_RF_CHANNEL_NUMS);
    }
#endif
