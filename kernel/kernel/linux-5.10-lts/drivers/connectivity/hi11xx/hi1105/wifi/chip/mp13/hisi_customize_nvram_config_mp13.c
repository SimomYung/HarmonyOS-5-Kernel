...
#define CFG_80M_160M_RC_CODE_DEFAULT 100
...
OAL_STATIC wlan_init_cust_nvram_params g_cust_nv_params_dev1 = {{{0}}}; /* chip2的最大发送功率定制化数组 */
#endif
...
OAL_STATIC wlan_cfg_cmd g_nvram_config_ini[NVRAM_PARAMS_PWR_INDEX_BUTT] = {
    ...
    { "cfg_tpc_ru_max_pow_5g_band7",      NVRAM_CFG_TPC_RU_MAX_POWER_5G_BAND7 },
    /* CN */
    ...
    { "cfg_80m_rc_code_percent", NVRAM_PARAMS_80M_RCCODE_PERCENT },
};
...
#ifdef _PRE_DUAL_CHIP
    if (hcc_get_main_dev() == HCC_EP_WIFI_DEV1) {
        return &g_cust_nv_params_dev1.st_pow_ctrl_custom_param;
    }
#endif
...
OAL_STATIC void hwifi_get_max_txpwr_base(const char *tag, uint8_t base_idx, uint8_t *base_params, uint8_t param_num)
{
    ...
    if ((hwifi_config_sepa_coefficient_from_param(base_pwr_params, l_nv_params, &per_param_num,
        param_num) == OAL_SUCC) && (per_param_num == param_num)) {
        /* 参数合理性检查 */
        for (param_idx = 0; param_idx < param_num; param_idx++) {
                                              ...
                                              uint8_t uc_param_num)
{
    ...
    if (get_cust_conf_int32(INI_MODU_WIFI, g_nvram_config_ini[uc_cus_id].name, &nvram_params) == INI_SUCC) {
...
OAL_STATIC void hwifi_get_delt_ru_pow_val_5g(void)
{
    ...
    if (get_cust_conf_int32(INI_MODU_WIFI, g_nvram_config_ini[NVRAM_CFG_TPC_RU_POWER_80M_L_5G].name,
        &val) != INI_SUCC) {
    ...
    if (get_cust_conf_int32(INI_MODU_WIFI, g_nvram_config_ini[NVRAM_CFG_TPC_RU_POWER_80M_H_5G].name,
        &val) != INI_SUCC) {
    ...
    if (get_cust_conf_int32(INI_MODU_WIFI, g_nvram_config_ini[NVRAM_CFG_TPC_RU_POWER_160M_L_5G].name,
        &val) != INI_SUCC) {
    ...
    if (get_cust_conf_int32(INI_MODU_WIFI, g_nvram_config_ini[NVRAM_CFG_TPC_RU_POWER_160M_H_5G].name,
        &val) != INI_SUCC) {
...
static void hwifi_get_delt_ru_pow_val_2g(void)
{
    ...
    /* 20m 26~242tone ru_power */
    if (get_cust_conf_int32(INI_MODU_WIFI, g_nvram_config_ini[NVRAM_CFG_TPC_RU_POWER_20M_2G].name,
        &l_nvram_params) != INI_SUCC) {
    ...
    /* 40m 26~242tone ru_power */
    if (get_cust_conf_int32(INI_MODU_WIFI, g_nvram_config_ini[NVRAM_CFG_TPC_RU_POWER_40M_0_2G].name,
        &l_nvram_params) != INI_SUCC) {
    ...
    if (get_cust_conf_int32(INI_MODU_WIFI, g_nvram_config_ini[NVRAM_CFG_TPC_RU_POWER_40M_1_2G].name,
        &l_nvram_params) != INI_SUCC) {
 ...
 *     修改内容   : 新生成函数
 */
static int32_t hwifi_get_5g_ru_max_ru_pow_val(wlan_cust_nvram_params *cust_nv_params)
{
    int32_t nvram_params = 0;
    int32_t ret = 0;
    uint32_t idx;
    ...
    for (band_idx = 0; band_idx < MAC_NUM_5G_BAND; band_idx++) {
        for (idx = CUS_POW_TX_CHAIN_MIMO; idx < CUS_POW_TX_CHAIN_BUTT; idx++) {
            ...
            ret += memcpy_s(cust_nv_params->auc_tpc_tb_ru_5g_max_power[band_idx].auc_tb_ru_5g_max_power[idx],
                WLAN_HE_RU_SIZE_BUTT, &nvram_params, sizeof(nvram_params));
            nvram_idx++;
            if (get_cust_conf_int32(INI_MODU_WIFI, g_nvram_config_ini[nvram_idx].name, &nvram_params) != INI_SUCC) {
                nvram_params = 0xFFFFFF;
            }
            ret += memcpy_s(&cust_nv_params->auc_tpc_tb_ru_5g_max_power[band_idx].auc_tb_ru_5g_max_power\
...
static void hwifi_get_total_max_ru_pow_val(wlan_cust_nvram_params *pst_g_cust_nv_params)
{
    ...
    if (get_cust_conf_int32(INI_MODU_WIFI, g_nvram_config_ini[nvram_idx].name, &l_nvram_params) != INI_SUCC) {
        l_nvram_params = TB_RU_MAX_POWER_DEFAULT;
    }
    pst_g_cust_nv_params->tpc_tb_ru_band7_max_power = l_nvram_params;
}
...
static void hwifi_clear_cust_nv_params(void)
{
#ifdef _PRE_DUAL_CHIP
    if (hcc_get_main_dev() == HCC_EP_WIFI_DEV1) {
        memset_s(&g_cust_nv_params_dev1, sizeof(g_cust_nv_params_dev1), 0, sizeof(g_cust_nv_params_dev1));
        return;
    }
#endif
...
static void hwifi_config_init_rc_code_nvram(void)
{
    wlan_cust_nvram_params *cust_nv_params = hwifi_get_nvram_params();
    int32_t nvram_params = 0;

    if (get_cust_conf_int32(INI_MODU_WIFI,
        g_nvram_config_ini[NVRAM_PARAMS_80M_RCCODE_PERCENT].name, &nvram_params) != INI_SUCC) {
        nvram_params = CFG_80M_160M_RC_CODE_DEFAULT;
    }
    cust_nv_params->cfg_80m_rc_code_percent = nvram_params;
}
...
int32_t hwifi_config_init_nvram(void)
{
   ...
   /* FCC/CE/SAR */
    ...
    hwifi_config_init_rc_code_nvram();
