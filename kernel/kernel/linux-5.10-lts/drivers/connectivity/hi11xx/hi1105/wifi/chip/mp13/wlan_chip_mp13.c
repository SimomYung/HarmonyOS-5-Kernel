...
OAL_STATIC void hwifi_cfg_host_global_switch_init(void)
{
    ...
    g_wlan_min_fast_ps_idle_p2p = (uint8_t)hwifi_get_init_value(CUS_TAG_INI, WLAN_CFG_INIT_MIN_FAST_PS_IDLE_P2P);
    g_wlan_max_fast_ps_idle_p2p = (uint8_t)hwifi_get_init_value(CUS_TAG_INI, WLAN_CFG_INIT_MAX_FAST_PS_IDLE_P2P);
    g_wlan_auto_ps_screen_on_p2p =
        (uint8_t)hwifi_get_init_value(CUS_TAG_INI, WLAN_CFG_INIT_AUTO_FAST_PS_THRESH_SCREENON_P2P);
    g_wlan_auto_ps_screen_off_p2p =
        (uint8_t)hwifi_get_init_value(CUS_TAG_INI, WLAN_CFG_INIT_AUTO_FAST_PS_THRESH_SCREENOFF_P2P);
...
uint8_t g_nb_power_enable = 1;
OAL_STATIC void hwifi_config_export_ini_mac_addr(void)
{
    int32_t l_val;
    uint32_t ini_mac_addr;

    g_nb_power_enable = hwifi_get_init_value(CUS_TAG_INI, WLAN_CFG_INIT_NB_POWER_ENABLE);
    /* ini中参数对应mac地址前二或三字节 */
    l_val = hwifi_get_init_value(CUS_TAG_INI, WLAN_CFG_INIT_EFUSE_MAC_ADDR);
    if (l_val == -1) {
        oam_warning_log0(0, OAM_SF_ANY, "{hwifi_cfg_host_global_init_export::no ini efuse_mac_addr!}");
        return;
    }
    ini_mac_addr = (uint32_t)l_val;

    l_val = hwifi_get_init_value(CUS_TAG_INI, WLAN_CFG_INIT_EFUSE_MAC_ADDR_2BYTE);
    g_wlan_device_mac_addr_2byte_flg = (l_val == 1) ? l_val : 0;
    if (g_wlan_device_mac_addr_2byte_flg == 1) {
        g_wlan_device_mac_addr[BYTE_OFFSET_0] = (ini_mac_addr >> BYTE_OFFSET_8) & 0xff;
        g_wlan_device_mac_addr[BYTE_OFFSET_1] = ini_mac_addr & 0xff;
    } else {
        g_wlan_device_mac_addr[BYTE_OFFSET_0] = (ini_mac_addr >> BYTE_OFFSET_16) & 0xff;
        g_wlan_device_mac_addr[BYTE_OFFSET_1] = (ini_mac_addr >> BYTE_OFFSET_8) & 0xff;
        g_wlan_device_mac_addr[BYTE_OFFSET_2] = ini_mac_addr & 0xff;
    }
}

OAL_STATIC void hwifi_cfg_host_global_init_export(void)
{
    hwifi_config_export_ini_mac_addr();
}
#endif
...
#ifdef _PRE_WLAN_EXPORT
    hwifi_cfg_host_global_init_export();
#endif
...
{
    /* 对mp13/mp15产品，mp16/mp17c独有事件不需要下发device */
    return (oal_value_in_valid_range(cmd_id, WLAN_CFGID_MP16_PRIV_START, WLAN_CFGID_MP16_PRIV_END)) ||
            (oal_value_in_valid_range(cmd_id, WLAN_CFGID_MP17C_PRIV_START, WLAN_CFGID_MP17C_PRIV_END));
}
    ...
    mac_cfg_add_vap_param_stru *add_vap_param)
{
    ...
    if (mac_device->uc_device_id == HCC_EP_WIFI_DEV1) {
        oam_warning_log0(0, OAM_SF_SCAN, "wlan_chip_ap_p2p_device_coex_check_valid_mp15::dev1 do not check coex");
        return OAL_TRUE;
    }
#endif
...
static uint32_t wlan_chip_set_tas_ant_switch_mp15(oal_net_device_stru *net_dev, oal_bool_enum_uint8 tas_ant_switch)
{
    uint32_t ret = OAL_FAIL;
    if (oal_any_true_value2(g_tas_switch_en[WLAN_RF_CHANNEL_ZERO], g_tas_switch_en[WLAN_RF_CHANNEL_ONE])) {
        /* 0:默认态 1:tas态 */
        oam_warning_log1(0, OAM_SF_ANY, "{wlan_chip_set_tas_ant_switch_mp15::tas switch[%d].}", tas_ant_switch);
        ret = (uint32_t)hal_wifi_tas_set(tas_ant_switch);
    }
    return ret;
}
#endif
...
static uint32_t wlan_chip_get_reduce_pwr_channel_2g_bitmap_mp15(void)
{
    ...
    return (ret != INI_FAILED) ? (uint32_t)ret : 0x0;
}
...
const struct wlan_chip_ops g_wlan_chip_ops_mp13 = {
    ...
    .set_chn_cali_data = NULL,
    .get_cali_data_send_addr = NULL,
    .get_cali_data_send_len = NULL,
    .get_cali_data_size = NULL,
    .save_cali_event_get_cali_times = NULL,
    // 特殊dbdc相关
    ...
    .hcc_rx_update_rx_ctrl = hmac_hcc_rx_update_rx_ctrl,
#ifdef _PRE_WLAN_FEATURE_TAS_ANT_SWITCH
    .set_tas_ant_switch = wlan_chip_set_tas_ant_switch_mp15,
    .get_tas_ant_status = NULL,
#endif
...
#ifdef _PRE_DUAL_CHIP
    .hcc_customize_h2d_data_cfg = hwifi_hcc_customize_h2d_data_cfg_dual_mp15,
#else
    .hcc_customize_h2d_data_cfg = hwifi_hcc_customize_h2d_data_cfg_mp13,
#endif
    .show_customize_info = hwifi_get_cfg_params,
...
#endif
#ifdef _PRE_DUAL_CHIP
    .custom_host_read_cfg_init = hwifi_custom_host_read_cfg_init_dual_mp15,
#else
    .custom_host_read_cfg_init = hwifi_custom_host_read_cfg_init_mp13,
#endif
    .h2d_cmd_need_filter = wlan_chip_h2d_cmd_need_filter_mp13,
...
const oal_wlan_cfg_stru g_wlan_spec_cfg_mp13 = {
    ...
    .max_user_limit = WLAN_ASSOC_USER_MAX_NUM_MP13 + WLAN_MULTI_USER_MAX_NUM_LIMIT_MP13,
    .invalid_user_id = WLAN_ASSOC_USER_MAX_NUM_MP13 + WLAN_MULTI_USER_MAX_NUM_LIMIT_MP13,
    ...
    .bfee_ntx_supports = 4,    /* mp13最多支持 4*2 MU Sounding */
    ...
    .vap_support_max_num_limit = WLAN_VAP_SUPPORT_MAX_NUM_LIMIT_MP13,
    .hal_max_vap_num = HAL_MAX_VAP_NUM_MP13,
    .need_check_netbuf_pre_num = OAL_TRUE,
    .feature_same_chan_remain_on_chan_support = OAL_FALSE,
    .hal_dev_num = WLAN_DEVICE_MAX_NUM_PER_CHIP_MP13,
    .power_off_ready_support = OAL_FALSE,
    .rx_ring_reduce_load_is_open = OAL_FALSE,
    .rx_support_hw_reorder = OAL_FALSE,
...
const oal_wlan_cfg_stru g_wlan_spec_cfg_mp15 = {
    ...
    .max_user_limit = WLAN_ASSOC_USER_MAX_NUM_MP15 + WLAN_MULTI_USER_MAX_NUM_LIMIT_MP15,
    .invalid_user_id = WLAN_ASSOC_USER_MAX_NUM_MP15 + WLAN_MULTI_USER_MAX_NUM_LIMIT_MP15,
    ...
    .bfee_ntx_supports = 8,    /* mp13最多支持 4*2 MU Sounding */
    .vht_max_mpdu_lenth = WLAN_MIB_VHT_MPDU_7991, /* VHT协议mp15最大mpdu_len */
    .pcie_speed = 0, /* 不使用 */
    .trigger_su_beamforming_feedback = OAL_FALSE,
    .trigger_mu_partialbw_feedback = OAL_FALSE,
    ...
    .emlsr_str_link = OAL_FALSE,
    .tx_ba_info = OAL_FALSE,
    .vap_support_max_num_limit = WLAN_VAP_SUPPORT_MAX_NUM_LIMIT_MP15,
#ifdef _PRE_DUAL_CHIP
    .hal_max_vap_num = HAL_MAX_VAP_NUM_DUAL_MP15,
#else
    .hal_max_vap_num = HAL_MAX_VAP_NUM_MP13,
#endif
    .need_check_netbuf_pre_num = OAL_TRUE,
    .feature_same_chan_remain_on_chan_support = OAL_TRUE,
    .hal_dev_num = WLAN_DEVICE_MAX_NUM_PER_CHIP_MP15,
    .power_off_ready_support = OAL_FALSE,
    .rx_ring_reduce_load_is_open = OAL_FALSE,
    .rx_support_hw_reorder = OAL_FALSE,
    .high_pps_fastsleep_close = OAL_FALSE,
