...
void hwifi_cfg_host_global_switch_init_mp16(void)
{
    ...
    g_wlan_fast_ps_dyn_ctl = ((g_wlan_ps_mode == MAX_FAST_PS) ? 1 : 0);
    ...
    g_wlan_min_fast_ps_idle_p2p = g_cust_cap.p2p_fast_ps.wlan_min_fast_ps_idle_p2p;
    g_wlan_max_fast_ps_idle_p2p = g_cust_cap.p2p_fast_ps.wlan_max_fast_ps_idle_p2p;
    g_wlan_auto_ps_screen_on_p2p = g_cust_cap.p2p_fast_ps.wlan_auto_ps_thresh_screen_on_p2p;
    g_wlan_auto_ps_screen_off_p2p = g_cust_cap.p2p_fast_ps.wlan_auto_ps_thresh_screen_off_p2p;
...
{
    uint8_t *tas_gpio_ctrl = mp17c_get_tas_gpio_ctrl();
    ...
    if (hwifi_get_cust_read_status(CUS_TAG_HOST, WLAN_CFG_HOST_EXPAND_FEATURE_SWITCH_BITMAP)) {
        g_expand_feature_switch_bitmap = g_cust_cap.expand_feature_switch_bitmap;
    }
    if (hwifi_get_cust_read_status(CUS_TAG_CAP, WLAN_CFG_TAS_GPIO)) {
        *tas_gpio_ctrl = g_cust_cap.tas_gpio_ctrl;
    }

    if ((hwifi_get_cust_read_status(CUS_TAG_CAP, WLAN_CFG_CAP_PHY_CAP_SWITCH) == OAL_TRUE) &&
        (hwifi_get_cust_read_status(CUS_TAG_CAP, WLAN_CFG_CAP_LP_CAP_SWITCH) == OAL_TRUE)) {
        hmac_set_elp_scan_switch(g_cust_cap.phy_cap_mask, g_cust_cap.lp_cap_mask);
    }
}
...
void hwifi_cfg_host_global_init_mlo_mp16(void)
{
    g_pst_mac_device_capability[0].switch_mlo =
        (g_cust_cap.wifi_11be_switch_mask & BIT16) ? OAL_TRUE : OAL_FALSE;
    /* 如果mlo定制化开关不使能，则相关mlo特性都不使能 */
    if (g_pst_mac_device_capability[0].switch_mlo == OAL_FALSE) {
        oam_warning_log0(0, OAM_SF_CFG, "hwifi_cfg_host_global_init_11be_mp16::mlo is closed!");
        return;
    }

    g_st_mac_device_custom_cfg.custom_cfg_11be.mlo_switch =
        (g_cust_cap.wifi_11be_switch_mask & BIT16) ? OAL_TRUE : OAL_FALSE;
    g_st_mac_device_custom_cfg.custom_cfg_11be.single_radio_switch =
        (g_cust_cap.wifi_11be_switch_mask & BIT17) ? OAL_TRUE : OAL_FALSE;
    g_st_mac_device_custom_cfg.custom_cfg_11be.emlsr_switch =
        (g_cust_cap.wifi_11be_switch_mask & BIT18) ? OAL_TRUE : OAL_FALSE;

    g_st_mac_device_custom_cfg.custom_cfg_11be.bpcc_switch =
        (g_cust_cap.wifi_11be_switch_mask & BIT20) ? OAL_TRUE : OAL_FALSE;
    g_st_mac_device_custom_cfg.custom_cfg_11be.aput_mlo_switch =
        (g_cust_cap.wifi_11be_switch_mask & BIT21) ? OAL_TRUE : OAL_FALSE;
    g_st_mac_device_custom_cfg.custom_cfg_11be.ttlm_switch =
        (g_cust_cap.wifi_11be_switch_mask & BIT22) ? OAL_TRUE : OAL_FALSE;
    g_st_mac_device_custom_cfg.custom_cfg_11be.reconfig_switch =
        (g_cust_cap.wifi_11be_switch_mask & BIT23) ? OAL_TRUE : OAL_FALSE;

    g_pst_mac_device_capability[0].switch_emlsr =
        g_st_mac_device_custom_cfg.custom_cfg_11be.emlsr_switch;
    if (g_pst_mac_device_capability[0].switch_emlsr == OAL_FALSE) {
        oam_warning_log0(0, OAM_SF_CFG, "hwifi_cfg_host_global_init_11be_mp16::emlsr is closed!");
        return;
    }
    g_st_mac_device_custom_cfg.custom_cfg_11be.emlsr_enter_switch =
        (g_cust_cap.wifi_11be_switch_mask & BIT24) ? OAL_TRUE : OAL_FALSE;
    g_st_mac_device_custom_cfg.custom_cfg_11be.emlsr_hisi_switch =
        (g_cust_cap.wifi_11be_switch_mask & BIT25) ? OAL_TRUE : OAL_FALSE;
}
void hwifi_cfg_host_global_init_11be_mp16(void)
{
    g_pst_mac_device_capability[0].switch_11be =
        (g_cust_cap.wifi_11be_switch_mask & BIT0) ? OAL_TRUE : OAL_FALSE;
    /* 如果11be定制化开关不使能，则相关11be mlo特性都不使能 */
    if (g_pst_mac_device_capability[0].switch_11be == OAL_FALSE) {
        oam_warning_log0(0, OAM_SF_CFG, "hwifi_cfg_host_global_init_11be_mp16::11be is closed!");
        return;
    }

    g_st_mac_device_custom_cfg.custom_cfg_11be.sta_11be_switch =
        (g_cust_cap.wifi_11be_switch_mask & BIT0) ? OAL_TRUE : OAL_FALSE;
    g_st_mac_device_custom_cfg.custom_cfg_11be.aput_11be_switch =
        (g_cust_cap.wifi_11be_switch_mask & BIT1) ? OAL_TRUE : OAL_FALSE;
    g_st_mac_device_custom_cfg.custom_cfg_11be.aput_11be_hisi_switch =
        (g_cust_cap.wifi_11be_switch_mask & BIT2) ? OAL_TRUE : OAL_FALSE;

    g_st_mac_device_custom_cfg.custom_cfg_11be.eht_1024qam_support =
        (g_cust_cap.wifi_11be_switch_mask & BIT4) ? OAL_TRUE : OAL_FALSE;
    g_st_mac_device_custom_cfg.custom_cfg_11be.eht_4096qam_support =
        (g_cust_cap.wifi_11be_switch_mask & BIT5) ? OAL_TRUE : OAL_FALSE;
    g_st_mac_device_custom_cfg.custom_cfg_11be.chba_11be_switch =
        (g_cust_cap.wifi_11be_switch_mask & BIT6) ? OAL_TRUE : OAL_FALSE;

    hwifi_cfg_host_global_init_mlo_mp16();
}
#endif
...
void hwifi_cfg_host_global_init_param_mp16(void)
{
    ...
    if (g_wlan_spec_cfg->feature_11be_is_open) {
        hwifi_cfg_host_global_init_11be_mp16();
    }
#endif

    if (hwifi_get_cust_read_status(CUS_TAG_HOST, WLAN_CFG_HOST_WLAN_GO_ASSOC_USER_MAX_NUM)) {
        g_p2p_go_max_user_num = g_cust_host.wlan_go_assoc_user_max_num;
    }
...
uint32_t hwifi_force_update_rf_params_mp16(void)
{
    static regdomain_enum regdomain_last = REGDOMAIN_COUNT;

    regdomain_enum regdomain_cur = hwifi_get_regdomain_from_country_code((uint8_t *)g_wifi_country_code);
    if (regdomain_last == regdomain_cur) {
        oam_warning_log0(0, OAM_SF_CFG, "hwifi_force_update_rf_params_mp16::regdomain is not updated");
        return INI_SUCC;
    }
    regdomain_last = regdomain_cur;
    oam_warning_log1(0, OAM_SF_CFG, "hwifi_force_update_rf_params_mp16::regdomain is updated to %d", regdomain_last);
...
uint32_t wlan_chip_get_11be_switch_mask_mp16(void)
{
    return g_cust_cap.wifi_11be_switch_mask;
}

uint8_t wlan_chip_get_wur_switch_mp16(void)
{
    return g_cust_cap.support_wur_switch;
}
...
{
    uint32_t idx;
    if (oal_value_in_valid_range(cmd_id, WLAN_CFGID_MP17C_PRIV_START, WLAN_CFGID_MP17C_PRIV_END)) {
        return OAL_TRUE;
    }
...
uint32_t wlan_chip_update_cfg80211_mgmt_tx_wait_time_mp16(uint32_t wait_time_temp)
{
    uint32_t wait_time = wait_time_temp;
    ...
    dmac_query_station_info_response_event *response_event)
{
    ...
    vap = dpe_wlan_vap_get(hmac_vap->st_vap_base_info.uc_vap_id);
    if (vap == NULL) {
        return;
    }
    vap_stats = dpe_wlan_vap_stat_get(vap);
    /* mp16 tx/rx帧统计从host获取 */
    hmac_vap->station_info.rx_packets = vap_stats->rx_packets;
    hmac_vap->station_info.rx_bytes = vap_stats->rx_bytes;
    hmac_vap->station_info.rx_dropped_misc = response_event->rx_dropped_misc + vap_stats->rx_dropped_misc;

    hmac_vap->station_info.tx_packets = vap_stats->tx_packets;
    hmac_vap->station_info.tx_bytes = vap_stats->tx_bytes;
    hmac_vap->station_info.tx_retries = response_event->tx_retries + vap_stats->tx_retries;
    hmac_vap->station_info.tx_failed = response_event->tx_failed + vap_stats->tx_failed;
}
...
#ifdef _PRE_WLAN_CHBA_MGMT
/* mp16是否支持chba dbdc+dbac共存
 * 返回值：OAL_TRUE:支持CHBA DBDC+DBAC; OAL_FALSE:不支持CHBA DBDC+DBAC
 */
oal_bool_enum_uint8 wlan_chip_is_support_chba_dbdc_dbac_mp16(void)
{
    return ((g_expand_feature_switch_bitmap & CUSTOM_CHBA_DBDC_DBAC) != 0) ? OAL_TRUE : OAL_FALSE;
}
#endif
...
void wal_hipriv_always_tx_write_msg_mp16(wal_msg_write_stru *write_msg, uint8_t en_tx_flag,
...
const struct wlan_chip_ops g_wlan_chip_ops_mp16 = {
    ...
    .get_11be_switch_mask = wlan_chip_get_11be_switch_mask_mp16,
    .get_wur_switch = NULL,
    ...
    .set_chn_cali_data = hmac_set_chn_cali_data,
    .get_cali_data_send_addr = hmac_get_cali_data_send_addr,
    .get_cali_data_send_len = hmac_get_cali_data_send_len,
    .get_cali_data_size = hmac_get_cali_data_size,
    .save_cali_event_get_cali_times = hmac_save_cali_event_get_cali_times,
    .get_slave_cali_data_save_addr = hmac_get_slave_cali_data_save_addr_mp16,
    // 特殊dbdc相关
    ...
    .hcc_rx_update_rx_ctrl = NULL,
#ifdef _PRE_WLAN_FEATURE_TAS_ANT_SWITCH
    .set_tas_ant_switch = wlan_chip_set_tas_ant_switch_mp16c,
    .get_tas_ant_status = wlan_chip_get_tas_ant_status_mp16c,
#ifdef _PRE_PLAT_FEATURE_CUSTOMIZE
    .init_nvram_main_tas = hwifi_cust_tas_nv_fcc_ce_pow_data_send,
#endif
#endif
    .get_reduce_pwr_channel_2g_bitmap = wlan_chip_get_reduce_pwr_channel_2g_bitmap_mp16,
#ifdef WIFI_DEBUG_ENABLE
    .always_tx_write_msg = wal_hipriv_always_tx_write_msg_mp16,
#endif
    .mvap_send_trans_tbl = NULL,
    .get_lowpower_stat_enable = NULL,
};
...
const oal_wlan_cfg_stru g_wlan_spec_cfg_mp16 = {
    ...
    .feature_11be_is_open = OAL_FALSE,
    .feature_ml_is_supported = OAL_FALSE,
    ...
    .max_rf_num = 4,
    .max_slave_rf_num = 2,          /* 辅路通道总数  */
    .max_slave_scan_rf_num = 2,     /* 并发扫描时用到的辅路通道数目 */
    .max_band_num = WLAN_CALI_BAND_6G,
    .max_5g_bw = WLAN_BANDWIDTH_160,
    .slave_scan_rf_start_idx = 2,   /* 并发扫描时用到的辅路起始通道号 */
    .feature_txq_ns_support = OAL_FALSE,
    .feature_txbf_mib_study = OAL_TRUE,
    .bfer_sounding_dimensions = 4, /* 发送最多支持4流 */
    .bfee_ntx_supports = 8,    /* mp16 最多支持 8*4 MU Sounding */
    ...
    .wur_support_5g = OAL_FALSE,
    .wide_bw_support = OAL_FALSE,
    .multi_tid_aggr_tx_support = OAL_FALSE,
    .emlsr_str_link = OAL_FALSE,
    .tx_ba_info = OAL_TRUE,
    .vap_support_max_num_limit = WLAN_VAP_SUPPORT_MAX_NUM_LIMIT_MP16C,
    .hal_max_vap_num = HAL_MAX_VAP_NUM_MP16,
    .need_check_netbuf_pre_num = OAL_TRUE,
    .feature_same_chan_remain_on_chan_support = OAL_TRUE,
    .hal_dev_num = WLAN_DEVICE_MAX_NUM_PER_CHIP_MP16,
    .power_off_ready_support = OAL_FALSE,
    .rx_ring_reduce_load_is_open = OAL_FALSE,
    .vsp_timer_id = 16, // VSP特性使用common timer 16
    .rx_support_hw_reorder = OAL_TRUE,
    .high_pps_fastsleep_close = OAL_FALSE,
};
