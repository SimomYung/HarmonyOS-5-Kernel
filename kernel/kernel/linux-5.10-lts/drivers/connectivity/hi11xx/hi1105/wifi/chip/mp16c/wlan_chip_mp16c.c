...
oal_bool_enum_uint8 g_tas_switch_status = OAL_FALSE;
uint32_t wlan_chip_check_tas_param(oal_net_device_stru *net_dev, oal_bool_enum_uint8 tas_ant_switch)
{
    uint8_t up_vap_num;
    mac_device_stru *mac_device = mac_res_get_mac_dev();
    mac_vap_stru *up_vap_array[WLAN_SERVICE_VAP_MAX_NUM_PER_DEVICE] = {NULL};

    if (g_tas_switch_en[WLAN_RF_TAS_ANT_IDX] == OAL_FALSE) {
        /* 仅C0支持TAS切换方案 */
        oam_error_log0(0, OAM_SF_ANY, "wlan_chip_set_tas_ant_switch_mp16c: tas_not support!");
        return OAL_FAIL;
    }
    /* vap未创建时，不处理supplicant命令 */
    if (oal_net_dev_priv(net_dev) == NULL) {
        oam_error_log0(0, OAM_SF_CFG, "wlan_chip_set_tas_ant_switch_mp16c::vap not created yet, ignore the cmd!");
        return OAL_FAIL;
    }
    /* 多VAP场景不切换TAS */
    up_vap_num = mac_device_find_up_vaps(mac_device, up_vap_array, WLAN_SERVICE_VAP_MAX_NUM_PER_DEVICE);
    if (up_vap_num > 1 && tas_ant_switch == OAL_TRUE) {
        /* 仅C0支持TAS切换方案 */
        oam_error_log1(0, OAM_SF_ANY, "wlan_chip_set_tas_ant_switch_mp16c: up vap num[%d] > 1, tas not switch!",
            up_vap_num);
        return OAL_FAIL;
    }
    return OAL_SUCC;
}
uint32_t wlan_chip_set_tas_ant_switch_mp16c(oal_net_device_stru *net_dev, oal_bool_enum_uint8 tas_ant_switch)
{
    int32_t ret = OAL_SUCC;
    wal_msg_write_stru st_write_msg;
    /* TAS当前状态 */
    oal_bool_enum_uint8 temp_cur_tas_status = g_tas_switch_status;

    if (wlan_chip_check_tas_param(net_dev, tas_ant_switch) != OAL_SUCC) {
        return OAL_FAIL;
    }

    if (g_tas_switch_status == tas_ant_switch) {
        /* TAS切状态未改变 */
        oam_warning_log1(0, OAM_SF_ANY, "wlan_chip_set_tas_ant_switch_mp16c: tas status[%d] not change!",
            g_tas_switch_status);
        return OAL_SUCC;
    }

    g_tas_switch_status = tas_ant_switch;
    /* 刷新定制化功率限制参数 */
    if (tas_ant_switch == OAL_TRUE) {
        wlan_chip_init_nvram_main_tas(net_dev);
    } else {
        wlan_chip_init_nvram_main(net_dev);
    }
    /* 抛事件到wal层处理 */
    wal_write_msg_hdr_init(&st_write_msg, WLAN_CFGID_TAS_ANT_SWITCH, sizeof(uint8_t));
    st_write_msg.auc_value[0] = tas_ant_switch;
    ...
    if (oal_unlikely(ret != OAL_SUCC)) {
        /* 切天线失败，恢复tas状态 */
        oam_error_log1(0, OAM_SF_ANY, "wlan_chip_set_tas_ant_switch_mp16c::send cfg event fail[%d]!", ret);
        g_tas_switch_status = temp_cur_tas_status;
        return ret;
    }
    /* 刷新SAR档位定制化 */
#if (_PRE_OS_VERSION_LINUX == _PRE_OS_VERSION)
    ret = wal_ioctl_reduce_sar(net_dev, g_cur_sar_txpwr_lvl);
    if (oal_unlikely(ret != OAL_SUCC)) {
        /* 降SAR失败，tas状态为新 */
        oam_error_log2(0, OAM_SF_ANY, "wlan_chip_set_tas_ant_switch_mp16c::reduce sar lvl[%d] fail[%d]!",
            g_cur_sar_txpwr_lvl, ret);
        return ret;
    }
#endif
    return OAL_SUCC;
...
int32_t wlan_chip_get_tas_ant_status_mp16c(oal_net_device_stru *net_dev)
{
    oam_warning_log1(0, OAM_SF_ANY, "wlan_chip_get_tas_ant_status_mp16c::TAS_GET_ANT[%d]", g_tas_switch_status);
    return g_tas_switch_status;
}
#endif
...
const oal_wlan_cfg_stru g_wlan_spec_cfg_mp16c = {
    ...
    .hal_dev_num = WLAN_DEVICE_MAX_NUM_PER_CHIP_MP16C,
