...
oal_bool_enum_uint8 wlan_chip_is_support_hw_wapi_mp17c(void)
{
    return g_wlan_spec_cfg->feature_hw_wapi;
}
#ifdef _PRE_WLAN_FEATURE_EMLSR
uint8_t wlan_chip_get_emlsr_transation_delay_mp17c(void)
{
    return g_cust_cap.emlsr_transation_delay;
}
uint8_t wlan_chip_get_emlsr_padding_delay_mp17c(void)
{
    return g_cust_cap.emlsr_padding_delay;
}
#endif
static inline uint8_t wlan_chip_soft_irq_sch_support_mp17c(void)
{
    return OAL_TRUE;
}

static uint8_t wlan_chip_get_pcie_probe_state_mp17c(mac_user_stru *mac_user)
{
...
static void wlan_chip_custom_default_init_mp17c(void)
{
    g_cust_cap.phy_cap_mask = 0xBF;
    g_cust_cap.lp_cap_mask = 0x0;
}

uint8_t wlan_chip_get_lowpower_stat_enable_mp17c(void)
{
    return OAL_TRUE;
}

static oal_bool_enum_uint8 wlan_chip_is_support_dual_5g_dbdc_by_radio_cap_mp17c(void)
{
    return OAL_TRUE;
}
#ifdef _PRE_WLAN_FEATURE_LP_MIRACAST
void wlan_chip_lp_miracast_stat_mbps_mp17c(uint32_t tx_throughput_mbps, uint32_t rx_throughput_mbps)
{
    hmac_config_lp_miracast_stat_mbps(tx_throughput_mbps, rx_throughput_mbps);
}

static uint32_t wlan_chip_set_lp_miracast_mp17c(mac_vap_stru *mac_vap, uint16_t len, uint8_t *param)
{
    return hmac_config_set_lp_miracast(mac_vap, len, param);
}
#endif
#ifdef WIFI_DEBUG_ENABLE
void wal_hipriv_always_tx_write_msg_mp17c(wal_msg_write_stru *write_msg, uint8_t en_tx_flag,
    mac_rf_payload_enum_uint8 en_payload_flag, uint32_t len)
{
    mac_cfg_altx_msdu_info_stru *al_tx_param = (mac_cfg_altx_msdu_info_stru *)(write_msg->auc_value);

    /* 抛事件到wal层处理 */
    wal_write_msg_hdr_init(write_msg, WLAN_CFGID_SET_ALWAYS_TX, sizeof(mac_cfg_altx_msdu_info_stru));
    al_tx_param->en_payload_flag = en_payload_flag;
    al_tx_param->payload_len = len;
    al_tx_param->write_index = 0;
    al_tx_param->al_tx_flag = en_tx_flag;
    al_tx_param->al_tx_netbuf_num = len;
    al_tx_param->msdu_info_addr_lsb = 0;

    oam_warning_log2(0, OAM_SF_ANY, "{wal_hipriv_always_tx_write_msg::tx_flag[%d],len[%d]!}", en_tx_flag, len);
}
#endif
const uint32_t g_cmd_need_filter_mp17c[] = {
...
static oal_bool_enum_uint8 wlan_chip_h2d_cmd_need_filter_mp17c(uint32_t cmd_id)
{
    uint32_t idx;

    for (idx = 0; idx < oal_array_size(g_cmd_need_filter_mp17c); idx++) {
        if (cmd_id == g_cmd_need_filter_mp17c[idx]) {
            return OAL_TRUE;
        }
    }
...
const struct wlan_chip_ops g_wlan_chip_ops_mp17c = {
#ifdef _PRE_PLAT_FEATURE_CUSTOMIZE
    ...
    .send_cali_data = wal_send_cali_data_mp17c,
    .send_20m_all_chn_cali_data = wal_send_cali_data_mp17c,
    ...
    .get_wur_switch = wlan_chip_get_wur_switch_mp16,
...
#endif
    .h2d_cmd_need_filter = wlan_chip_h2d_cmd_need_filter_mp17c,
    ...
    .get_slave_cali_data_save_addr = hmac_get_slave_cali_data_save_addr_mp17c,
    // 特殊dbdc相关
    ...
    .is_support_dual_5g_dbdc_by_radio_cap = wlan_chip_is_support_dual_5g_dbdc_by_radio_cap_mp17c,
    ...
    .is_support_hw_wapi = wlan_chip_is_support_hw_wapi_mp17c,
    ...
    .soft_irq_sch_support = wlan_chip_soft_irq_sch_support_mp17c,
    .get_pcie_probe_state = wlan_chip_get_pcie_probe_state_mp17c,
    .custom_default_init = wlan_chip_custom_default_init_mp17c,
#ifdef _PRE_WLAN_CHBA_MGMT
    .is_support_chba_dbdc_dbac = wlan_chip_is_support_chba_dbdc_dbac_mp16,
#endif
#ifdef _PRE_WLAN_FEATURE_LP_MIRACAST
    .lp_miracast_stat_func = wlan_chip_lp_miracast_stat_mbps_mp17c,
    .set_lp_miracast_func = wlan_chip_set_lp_miracast_mp17c,
#endif
#ifdef _PRE_WLAN_FEATURE_11AX
...
#ifdef _PRE_WLAN_FEATURE_EMLSR
    .get_emlsr_transation_delay = wlan_chip_get_emlsr_transation_delay_mp17c,
    .get_emlsr_padding_delay = wlan_chip_get_emlsr_padding_delay_mp17c,
#endif
#ifdef WIFI_DEBUG_ENABLE
    .always_tx_write_msg = wal_hipriv_always_tx_write_msg_mp17c,
#endif
    .mvap_send_trans_tbl = NULL,
    .get_lowpower_stat_enable = wlan_chip_get_lowpower_stat_enable_mp17c,
};

const oal_wlan_cfg_stru g_wlan_spec_cfg_mp17c = {
    ...
    .feature_11be_is_open = OAL_TRUE,
    .feature_ml_is_supported = OAL_TRUE,
    .p2p_device_gc_use_one_vap = OAL_FALSE,
    .max_sta_num = 5,
    .max_p2p_group_num = 2,
    .p2p_go_max_user_num = WLAN_P2P_GO_ASSOC_USER_MAX_NUM_MP17C,  /* MP17C P2P最大接入用户数为8 */
    ...
    .max_tx_ba = WLAN_MAX_TX_BA_MP17C,
    .max_rx_ba = WLAN_MAX_RX_BA_MP17C,
    .other_bss_id = WLAN_HAL_OHTER_BSS_ID_MP17C,
    .max_tx_ampdu_num = WLAN_AMPDU_TX_MAX_NUM_MP17C,
    .max_rf_num = 2,
    .max_slave_rf_num = 2,          /* 辅路通道总数  */
    .max_slave_scan_rf_num = 1,     /* 并发扫描时用到的辅路通道数目 */
    .max_band_num = WLAN_CALI_BAND_6G,
    .max_5g_bw = WLAN_BANDWIDTH_160,
    .slave_scan_rf_start_idx = 1,   /* 并发扫描时用到的辅路起始通道号 */
    .feature_txq_ns_support = OAL_FALSE,
    .feature_txbf_mib_study = OAL_TRUE,
    .bfer_sounding_dimensions = 2, /* 发送最多支持双流 */
    .bfee_ntx_supports = 8,    /* biseng最多支持 8*2 MU Sounding */
    .vht_max_mpdu_lenth = WLAN_MIB_VHT_MPDU_11454, /* VHT协议mp17c最大mpdu_len */
    .pcie_speed = 1, /* 默认GEN2 */
    ...
    .wide_bw_support = OAL_TRUE,
    .multi_tid_aggr_tx_support = OAL_TRUE,
    .emlsr_str_link = OAL_TRUE,
    .tx_ba_info = OAL_FALSE,
    .vap_support_max_num_limit = WLAN_VAP_SUPPORT_MAX_NUM_LIMIT_MP17C,
    .hal_max_vap_num = HAL_MAX_VAP_NUM_MP17C,
    .need_check_netbuf_pre_num = OAL_FALSE,
    .feature_same_chan_remain_on_chan_support = OAL_TRUE,
    .hal_dev_num = WLAN_DEVICE_MAX_NUM_PER_CHIP_MP17C,
    .power_off_ready_support = OAL_TRUE,
    .rx_ring_reduce_load_is_open = OAL_FALSE,
    .vsp_timer_id = 7, // VSP特性使用common timer 7
    .rx_support_hw_reorder = OAL_TRUE,
    .high_pps_fastsleep_close = OAL_TRUE,
};

const oal_wlan_cfg_stru g_wlan_spec_cfg_mp17c_mpw = {
    .feature_m2s_modem_is_open = OAL_TRUE,
    .feature_priv_closed_is_open = OAL_FALSE,
    .feature_hiex_is_open = OAL_TRUE,
    .rx_listen_ps_is_open = OAL_TRUE,
