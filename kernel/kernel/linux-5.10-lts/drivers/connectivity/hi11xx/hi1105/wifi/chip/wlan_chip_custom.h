...
OAL_STATIC OAL_INLINE uint8_t *wlan_chip_get_slave_cali_data_save_addr(uint8_t *cali_data,
    wlan_d2h_cali_data_type_enum_uint8 data_type)
{
    if (g_wlan_chip_ops->get_slave_cali_data_save_addr != NULL) {
        return g_wlan_chip_ops->get_slave_cali_data_save_addr(cali_data, data_type);
    }
    return 0;
}
