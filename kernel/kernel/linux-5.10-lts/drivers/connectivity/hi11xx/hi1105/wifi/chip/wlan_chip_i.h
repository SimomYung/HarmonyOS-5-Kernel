...
OAL_STATIC OAL_INLINE uint32_t wlan_chip_get_11be_switch_mask(void)
{
    if (g_wlan_chip_ops->get_11be_switch_mask != NULL) {
        return g_wlan_chip_ops->get_11be_switch_mask();
    }
    return 0;
}

OAL_STATIC OAL_INLINE uint8_t wlan_chip_get_wur_switch(void)
{
    if (g_wlan_chip_ops->get_wur_switch != NULL) {
        return g_wlan_chip_ops->get_wur_switch();
    }
    return 0;
}
...
OAL_STATIC OAL_INLINE oal_bool_enum_uint8 wlan_chip_mac_get_6g_flag(mac_rx_ctl_stru *rx_ctrl)
{
...
OAL_STATIC OAL_INLINE oal_bool_enum_uint8 wlan_chip_get_lowpower_stat_enable(void)
{
    if (g_wlan_chip_ops->get_lowpower_stat_enable != NULL) {
        return g_wlan_chip_ops->get_lowpower_stat_enable();
    }
    return OAL_FALSE;
}
...
#endif /* _PRE_WLAN_FEATURE_LP_MIRACAST */

#ifdef _PRE_WLAN_FEATURE_EMLSR
OAL_STATIC OAL_INLINE uint8_t wlan_chip_get_emlsr_transation_delay(void)
{
    if (g_wlan_chip_ops->get_emlsr_transation_delay != NULL) {
        return g_wlan_chip_ops->get_emlsr_transation_delay();
    }
    return 0;
}

OAL_STATIC OAL_INLINE uint8_t wlan_chip_get_emlsr_padding_delay(void)
{
    if (g_wlan_chip_ops->get_emlsr_padding_delay != NULL) {
        return g_wlan_chip_ops->get_emlsr_padding_delay();
    }
    return 0;
}
...
OAL_STATIC OAL_INLINE void wlan_chip_always_tx_write_msg(wal_msg_write_stru *write_msg, uint8_t en_tx_flag,
    mac_rf_payload_enum_uint8 en_payload_flag, uint32_t len)
{
    if (g_wlan_chip_ops->always_tx_write_msg != NULL) {
        return g_wlan_chip_ops->always_tx_write_msg(write_msg, en_tx_flag, en_payload_flag, len);
    }
    return;
}
#endif /* WIFI_DEBUG_ENABLE */

OAL_STATIC OAL_INLINE uint32_t wlan_chip_mvap_send_trans_tbl(hmac_vap_stru *hmac_vap)
{
    if (g_wlan_chip_ops->mvap_send_trans_tbl != NULL) {
        return g_wlan_chip_ops->mvap_send_trans_tbl(hmac_vap);
    }
    return OAL_FAIL;
}

OAL_STATIC OAL_INLINE void wlan_chip_update_action_filter(mac_vap_stru *mac_vap)
{
    if (g_wlan_chip_ops->update_action_filter != NULL) {
        g_wlan_chip_ops->update_action_filter(mac_vap);
    }
}
