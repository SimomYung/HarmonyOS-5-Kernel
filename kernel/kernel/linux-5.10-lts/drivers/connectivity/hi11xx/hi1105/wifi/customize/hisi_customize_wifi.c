...
mac_cust_feature_cap_stru g_cust_cap = {
    ...
    .wifi_11be_switch_mask = 0,
#ifdef _PRE_WLAN_FEATURE_TAS_ANT_SWITCH
    .rf_tas_loss_diff = 0,
    .rf_tas_lp_loss_diff = 0,
    .rf_tas_rssi_amend = {0, 0, 0, 0},
    .tas_tpc_ru_pow_diff = 0,
    .tas_max_txpwr_base_diff = 0,
#endif
    .support_wur_switch = 0,
    .dpd_support_11b = 0,
    .fem_type = {0, 0, 0},
    .mlsr_switch_alg_en = 0,
    .mlsr_switch_rssi_low_th = -70,
    .mlsr_switch_rssi_high_th = -65,
    .mlsr_switch_prefer_factor = 120,
    .mlsr_switch_pps_low_th = 600,
    .mlsr_switch_pps_high_th = 3000,
    .mlsr_switch_goodput_th = 50000,
    .enhanced_phy_cap_mask = 0x0,
#ifdef _PRE_WLAN_FEATURE_EMLSR
    .emlsr_transation_delay = 0,
    .emlsr_padding_delay = 0,
#endif
    .triangle_slope = 0,
    .fast_ps = { 3, 10, 5, 5 },
    .p2p_fast_ps = { 3, 10, 5, 5 },
    .tas_gpio_ctrl = 0,
    .sar_slide_pow_offset = { 10, 10, 10 },
    .support_omi_bw_switch = 0,
#ifdef _PRE_WLAN_FEATURE_TAS_ANT_SWITCH
    .tas_abf_en = OAL_FALSE,
#endif
    .coex_feature_bitmap = 0,
};
...
uint8_t g_wlan_open_cnt_dev1 = 0;
uint8_t hwifi_custom_cali_ini_param(uint8_t priv_cfg_value_input)
{
    uint8_t cali_interval;
    uint8_t priv_cfg_value = priv_cfg_value_input;

    /* 开机默认打开校准数据上传下发 */
    if (hcc_get_main_dev() == HCC_EP_WIFI_DEV) {
        g_wlan_open_cnt++;
    } else {
        g_wlan_open_cnt_dev1++;
    }

    if (((hcc_get_main_dev() == HCC_EP_WIFI_DEV) && (g_custom_cali_done == OAL_FALSE)) ||
        ((hcc_get_main_dev() == HCC_EP_WIFI_DEV1) && (g_custom_cali_done_mpxx_dev1 == OAL_FALSE))) {
        priv_cfg_value |= (CALI_DATA_REFRESH_MASK | CALI_DATA_REUPLOAD_MASK);
    } else if (g_wlan_cal_disable_switch == OAL_TRUE) {
        priv_cfg_value &= (~CALI_DATA_REFRESH_MASK);
    } else {
        if (g_wlan_cal_intvl_enable && (priv_cfg_value & CALI_DATA_REFRESH_MASK)) {
            cali_interval = (priv_cfg_value >> CALI_INTVL_OFFSET) + 1;
            if (((hcc_get_main_dev() == HCC_EP_WIFI_DEV) && (g_wlan_open_cnt % cali_interval)) ||
                ((hcc_get_main_dev() == HCC_EP_WIFI_DEV1) && (g_wlan_open_cnt_dev1 % cali_interval))) {
                priv_cfg_value &= (~CALI_DATA_REFRESH_MASK);
            }
        }
    }

    return priv_cfg_value;
}
#else
uint8_t hwifi_custom_cali_ini_param(uint8_t priv_cfg_value_input)
{
    ...
    if (g_custom_cali_done == OAL_FALSE) {
        ...
        if (g_wlan_cal_intvl_enable && (priv_cfg_value & CALI_DATA_REFRESH_MASK)) {
            cali_interval = (priv_cfg_value >> CALI_INTVL_OFFSET) + 1;
            if (g_wlan_open_cnt % cali_interval) {
                priv_cfg_value &= (~CALI_DATA_REFRESH_MASK);
            }
        }
    }
