...
/* 入参有效性由调用者保证 */
static void hmac_tx_stats_log_proc(hmac_tid_info_stru *tid_info, uint8_t vap_id, uint8_t user_id, uint8_t tid_id)
{
    uint32_t id;
...
void hmac_tx_stats_log(hmac_tid_info_stru *tid_info, uint32_t last_period_tx_msdu)
{
    ...
    dpe_user = dpe_wlan_user_get(tid_info->user_index);
    if (dpe_user == NULL) {
        return;
    }
    user_id = dpe_user->lut_index;
    vap_id = dpe_user->uc_vap_id;
    ...
    hmac_tx_stats_log_proc(tid_info, vap_id, user_id, tid_id);
}
