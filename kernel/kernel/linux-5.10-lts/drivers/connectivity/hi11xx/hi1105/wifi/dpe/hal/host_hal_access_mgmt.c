...
#define FORBIDE_SLEEP_RECORD_CNT 100
uint8_t g_forbid_sleep_record = 0;
oal_bool_enum_uint8 g_check_forbid_sleep_enable = OAL_FALSE;

void hal_pm_set_check_forbid_sleep_enable(oal_bool_enum_uint8 enable)
{
    g_check_forbid_sleep_enable = enable;
}
uint32_t g_forbid_sleep_bitmap = 0x0;
// 在定时器中检查forbid_sleep的值，如果连续10s都是非0，输出error
void hal_pm_check_forbid_sleep_in_timer(void)
{
    ...
    if (g_check_forbid_sleep_enable == OAL_FALSE || oal_atomic_read(&wlan_pm_info->forbid_sleep) == 0) {
        g_forbid_sleep_record = 0;
        return;
    }
    g_forbid_sleep_record++;
    if (g_forbid_sleep_record >= FORBIDE_SLEEP_RECORD_CNT) {
        oam_error_log2(0, 0, "hal_pm_check_forbid_sleep_in_timer: forbid_sleep[%d], bitmap is [%x] \n",
            oal_atomic_read(&wlan_pm_info->forbid_sleep), g_forbid_sleep_bitmap);
        g_forbid_sleep_record = 0;
    }
}
...
void hal_host_writew(uint16_t val, uintptr_t addr)
{
    rcu_read_lock();
    if (hal_host_pcpu_access_device_allowed()) {
        oal_writew(val, (void*)addr);
    } else {
        oam_warning_log0(0, 0, "{hal_host_writew::illegal writel}");
    }
    rcu_read_unlock();
}

OAL_STATIC uint32_t hal_host_readl_irq_proc(uintptr_t addr)
{
    uint32_t val = HAL_HOST_READL_INVALID_VAL; /* invalid val */
...
{
    if (in_irq()) {
        return hal_host_readl_irq_proc(addr);
    } else {
        return hal_host_readl(addr);
    }
}

OAL_STATIC void hal_host_writel_irq_proc(uint32_t val, uintptr_t addr)
{
    rcu_read_lock();
    if (!hal_host_pcpu_access_device_allowed()) {
...
{
    if (in_irq()) {
        hal_host_writel_irq_proc(val, addr);
    } else {
...
void hal_pm_try_wakeup_forbid_sleep(hal_pm_forbid_sleep_bitmap_type_enum event)
{
    ...
    g_forbid_sleep_bitmap |= BIT(event);
}
