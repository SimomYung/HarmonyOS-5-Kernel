...
{
    uint8_t vap_id = hw_vap_id % HAL_MAX_HW_VAP_NUM_MP12;
    const uint8_t hw_vap_id_2_hal_vap_id[HAL_MAX_HW_VAP_NUM_MP12] = {4, 5, 6, 7, 0, 1, 2, 3};
    return hw_vap_id_2_hal_vap_id[vap_id];
}
/* 功能：hal层vap id转换为hardware vap id */
uint8_t mp12_hal_vap_id_to_hw_vap_id(uint8_t hal_vap_id)
{
    uint8_t id = hal_vap_id % HAL_MAX_VAP_NUM_MP12;
    const uint8_t hal_vap_id_2_hw_vap_id[HAL_MAX_HW_VAP_NUM_MP12] = {4, 5, 6, 7, 0, 1, 2, 3};
    return hal_vap_id_2_hw_vap_id[id];
}
/* 功能: 根据hal vap id返回对应寄存器相对hw vap0的地址偏移,此资源有16份，和vap一一对应 */
uint32_t mp12_get_vap_reg_offset(uint8_t hal_vap_id)
{
    uint8_t hw_vap_id = mp12_hal_vap_id_to_hw_vap_id(hal_vap_id);
...
void mp12_clear_host_phy_int_status(hal_host_device_stru *hal_device, hal_intr_state_stru *status)
{
    ...
    hal_host_writel_irq(status->value1, hal_device->phy_regs.irq_clr_reg);
    hal_host_writel_irq(status->value2, hal_device->phy_regs.irq_clr_reg2);
}
...
static uint32_t mp12_regs_addr_transfer(hal_host_device_stru *hal_device, uint32_t reg_addr)
{
    ...
    switch (reg_addr) {
        ...
        case MAC_GLB_REG_HOST_INTR_STATUS_REG:
            ...
            break;
        case MAC_TMU_REG_VAP_TSFTIMER_RDVALL_STATUS_4_REG:
...
uint32_t mp12_host_regs_addr_init(hal_host_device_stru *hal_device)
{
    ...
    uint32_t hal_regs[] = {
        ...
        MAC_TMU_REG_VAP_TSFTIMER_RDVALL_STATUS_4_REG,
