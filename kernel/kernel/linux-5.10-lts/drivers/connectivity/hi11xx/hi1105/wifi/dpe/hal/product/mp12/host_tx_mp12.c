    ...
    hal_tx_ring_table_addr_stru *tx_ring_addr)
{
    if (tx_ring_addr == NULL) {
        oam_error_log0(0, 0, "{mp12_host_tx_get_msdu_info_devva::ring_devva is NULL");
...
{
    return g_mp12_host_tx_ring_table_mgmt.base_addr + MP12_TX_RING_TABLE_LEN * (lut_idx * WLAN_TIDNO_BUTT + tid);
}
...
static inline uint32_t mp12_host_tx_get_tid_ring_table_hostva(uint8_t lut_idx, uint8_t tid,
    hal_tx_ring_table_addr_val_stru *ring_tb_addr)
{
    return oal_pcie_devca_to_hostva(HCC_EP_WIFI_DEV,
        mp12_host_tx_get_tid_ring_table_addr(lut_idx, tid), &ring_tb_addr->addr_val);
}
    ...
    hal_tx_ring_table_addr_val_stru *ring_tb_addr)
{
    ...
    if (mp12_host_tx_get_tid_ring_table_hostva(lut_idx, tid, ring_tb_addr) != OAL_SUCC) {
        return OAL_FAIL;
    }

    memset_s(tx_ring_mgmt, sizeof(mp12_tx_msdu_ring_table_stru), 0, sizeof(mp12_tx_msdu_ring_table_stru));
    for (i = 0; i < MP12_TX_RING_TABLE_WORD_NUM; i++) {
        mp12_host_tx_update_ring_table(tx_ring_mgmt, ring_tb_addr->addr_val, i);
    }
...
uint32_t mp12_host_tx_ring_get_ops(hal_host_tx_ring_ops_param_stru *param, uint8_t type)
{
    ...
    tx_ring_mgmt = mp12_host_tx_get_tid_ring_table_mgmt(param->lut_idx, param->tid);
    if (g_mp12_host_tx_ring_get_ops_table[type] == NULL) {
        return OAL_FAIL;
    }
