    ...
    hal_tx_ring_table_addr_stru *tx_ring_addr)
{
    if (tx_ring_addr == NULL) {
        oam_error_log0(0, 0, "{mp16_host_tx_get_msdu_info_devva::ring_devva is NULL");
...
static inline uint32_t mp16_host_tx_get_tid_ring_table_hostva(uint8_t lut_idx, uint8_t tid,
    hal_tx_ring_table_addr_val_stru *ring_tb_addr)
{
    return oal_pcie_devca_to_hostva(HCC_EP_WIFI_DEV,
        mp16_host_tx_get_tid_ring_table_addr(lut_idx, tid), &ring_tb_addr->addr_val);
}

uint32_t mp16_host_tx_init_tid_ring_table(uint8_t lut_idx, uint8_t tid,
    hal_tx_ring_table_addr_val_stru *ring_tb_addr)
{
    mp16_tx_msdu_ring_table_stru *tx_ring_mgmt = mp16_host_tx_get_tid_ring_table_mgmt(lut_idx, tid);

    if (mp16_host_tx_get_tid_ring_table_hostva(lut_idx, tid, ring_tb_addr) != OAL_SUCC) {
        return OAL_FAIL;
    }
...
uint32_t mp16_host_tx_ring_get_ops(hal_host_tx_ring_ops_param_stru *param, uint8_t type)
{
    ...
    tx_ring_mgmt = mp16_host_tx_get_tid_ring_table_mgmt(param->lut_idx, param->tid);
    if (g_mp16_host_tx_ring_get_ops_table[type] == NULL) {
        return OAL_FAIL;
    }
