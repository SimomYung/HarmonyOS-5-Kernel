...
uint32_t g_mac_reg_offset_mp16c[WLAN_DEVICE_MAX_NUM_PER_CHIP_MP16C] = {0, MP16C_MAC_BANK_REG_OFFSET};
uint32_t mp16c_get_mac_reg_offset(uint8_t device_id, uint32_t reg_addr)
{
    return g_mac_reg_offset_mp16c[device_id];
}
hal_ring_mac_regs_info* mp16c_get_ring_mac_regs(void)
{
...
static void mp16c_host_rx_cb_set(mac_rx_ctl_stru *rx_ctl, mp16c_rx_mpdu_desc_stru *rx_hw_dscr, uint8_t first_msdu)
{
    if (first_msdu) {
        ...
        if (rx_hw_dscr->bit_vap_id != MP16C_OTHER_BSS_ID) {
            rx_ctl->bit_vap_id = mp16c_hw_vap_id_to_hal_vap_id(rx_ctl->bit_vap_id);
        }
 ...
 *   修改内容  : 新生成函数
 */
static uint32_t mp16c_host_rx_adjust_netbuf_len(oal_netbuf_stru *netbuf, mac_rx_ctl_stru *rx_ctl,
    mp16c_rx_mpdu_desc_stru *rx_hw_dscr)
{
    ...
    if ((rx_hw_dscr->bit_rx_status == HAL_RX_SUCCESS) &&
        (rx_hw_dscr->bit_packet_len == 0 || (rx_hw_dscr->bit_packet_len > (cb_res->len - MP16C_RX_MSDU_DSCR_LEN)))) {
        ...
        oam_report_dscr(BROADCAST_MACADDR, (uint8_t *)rx_hw_dscr, MP16C_RX_MSDU_DSCR_LEN, OAM_OTA_TYPE_RX_MSDU_DSCR);
        return OAL_FAIL;
    }

    oal_netbuf_pull(netbuf, MP16C_RX_MSDU_DSCR_LEN);
    ...
    oal_netbuf_stru *netbuf, mac_rx_ctl_stru *rx_ctl, uint8_t first_msdu)
{
    ...
    mp16c_host_rx_cb_set(rx_ctl, rx_msdu_hw_dscr, first_msdu);
    ret = mp16c_host_rx_adjust_netbuf_len(netbuf, rx_ctl, rx_msdu_hw_dscr);
    if (ret != OAL_SUCC) {
        return ret;
    }
...
#define MP16C_TX_MSDU_DSCR_LEN 24
...
uint8_t mp16c_host_tx_get_msdu_dscr_len(void)
{
    return MP16C_TX_MSDU_DSCR_LEN;
}

uint8_t mp16c_host_get_rx_msdu_dscr_len(void)
{
    return MP16C_RX_MSDU_DSCR_LEN;
}

void mp16c_host_h2d_rx_ring_reset(hal_host_device_stru *hal_device)
{
    hal_host_writel(MP16C_D2H_RX_RING_RESET, hal_device->st_host_rx_normal_free_ring.rx_ring_reset);
}
