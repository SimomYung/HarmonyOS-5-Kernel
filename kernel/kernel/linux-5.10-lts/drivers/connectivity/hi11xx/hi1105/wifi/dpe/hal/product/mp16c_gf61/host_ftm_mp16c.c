 ...
 *   修改内容   : 创建函数
 */
uint8_t mp16c_host_vap_get_by_hw_vap_id(uint8_t hal_device_id, uint8_t hw_vap_id)
{
    ...
    if ((hw_vap_id == MP16C_OTHER_BSS_ID) || (hw_vap_id >= HAL_MAX_HW_VAP_NUM_MP16C)) {
        return WLAN_VAP_MAX_NUM_LIMIT;
    }

    hal_vap_id = mp16c_hw_vap_id_to_hal_vap_id(hw_vap_id);
    hal_vap_id = hal_vap_id % HAL_MAX_VAP_NUM_MP16C;
