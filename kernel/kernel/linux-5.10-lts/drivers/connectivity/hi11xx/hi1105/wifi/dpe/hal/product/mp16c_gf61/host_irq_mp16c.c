...
{
    hal_mac_common_timer *timer = mp16c_get_host_mac_common_timer_ptr(MP16C_MAC_COMMON_TIMER_7);
...
{
    uint32_t soc_mask;
    mp16c_get_host_soc_int_mask(hal_device, &soc_mask);
    if ((soc_mask & MP16C_PCIE_CTRL_RB_MAC_N2_INTR_STATUS_MASK) != 0) {
        oam_error_log1(0, OAM_SF_RX, "{host_unmask_soc_host_intr:: device[1] unmask mac_n2 soc_mask[0x%x]}", soc_mask);
        soc_mask &= ~((uint32_t)MP16C_PCIE_CTRL_RB_MAC_N2_INTR_STATUS_MASK);
        mp16c_set_host_soc_irq_mask(hal_device, soc_mask);
    }
}
...
#define MP16_PCIE_MAC_N2_ERROR_THRESHOLD 100
static uint8_t mp16c_check_irq_host_mac_isr(hal_host_device_stru *hal_dev, uint32_t state, uint32_t mask)
{
    uint32_t               soc_raw_status = 0;
    uint32_t               soc_mask = 0;
    uint32_t               soc_status = 0;
    mac_device_stru        *mac_device = mac_res_get_dev(0);
    if (oal_unlikely(mac_device == NULL)) {
        return OAL_FALSE;
    }
    if (hal_dev->device_id == HAL_DEVICE_ID_SLAVE && mac_device->en_dbdc_running == 0 &&
        ++hal_dev->mac_intr_error_cnt >= MP16_PCIE_MAC_N2_ERROR_THRESHOLD) {
        /* 读取SOC中断状态 */
        mp16c_get_host_soc_int_raw_status(hal_dev, &soc_raw_status);
        mp16c_get_host_soc_int_mask(hal_dev, &soc_mask);
        mp16c_get_host_soc_int_status(hal_dev, &soc_status);
        oam_error_log2(0, OAM_SF_RX, "{irq_host_mac_isr:: device[1] error state[0x%x] mask[0x%x]}", state, mask);
        oam_error_log4(0, OAM_SF_RX,
            "{irq_host_mac_isr:: pcie_raw[0x%x] pcie_mask[0x%x] pcie_status[0x%x] cnt[%d]}",
            soc_raw_status, soc_mask, soc_status, hal_dev->mac_intr_error_cnt);
        mp16c_host_soc_int_mask(hal_dev, MP16C_PCIE_CTRL_RB_MAC_N2_INTR_STATUS_OFFSET);
        hal_dev->mac_intr_error_cnt = 0;
        return OAL_FALSE;
    }
    return OAL_TRUE;
}
void mp16c_irq_host_mac_isr(hal_host_device_stru *hal_dev)
{
    ...
    if (mp16c_check_irq_host_mac_isr(hal_dev, state, mask) == OAL_FALSE) {
        return;
    }
