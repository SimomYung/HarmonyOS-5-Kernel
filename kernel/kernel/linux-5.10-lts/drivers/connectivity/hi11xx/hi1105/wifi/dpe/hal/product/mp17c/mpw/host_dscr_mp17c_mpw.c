...
static void mp17c_mpw_host_rx_cb_set(mac_rx_ctl_stru *rx_ctl, mp17c_mpw_rx_msdu_dscr_stru *rx_hw_dscr,
    uint8_t first_msdu)
{
    ...
    if (first_msdu) {
        ...
        if (rx_hw_dscr->vap_id != WLAN_HAL_OHTER_BSS_ID_MP17C) {
            rx_ctl->bit_vap_id = mp17c_mpw_hw_vap_id_to_hal_vap_id(rx_ctl->bit_vap_id);
        }
 ...
 *   修改内容  : 新生成函数
 */
static uint32_t mp17c_mpw_host_rx_adjust_netbuf_len(oal_netbuf_stru *netbuf, mac_rx_ctl_stru *rx_ctl,
    mp17c_mpw_rx_msdu_dscr_stru *rx_hw_dscr)
{
    ...
    oal_netbuf_stru *netbuf, mac_rx_ctl_stru *rx_ctl, uint8_t first_msdu)
{
    ...
    mp17c_mpw_host_rx_cb_set(rx_ctl, rx_msdu_hw_dscr, first_msdu);
    ret = mp17c_mpw_host_rx_adjust_netbuf_len(netbuf, rx_ctl, rx_msdu_hw_dscr);
    if (ret != OAL_SUCC) {
        return ret;
    }

    if (rx_ctl->bit_frame_format == MAC_FRAME_TYPE_80211) {
        mac_get_rx_cb_mac_header_addr(rx_ctl) = (uint64_t)(uintptr_t)oal_netbuf_data(netbuf);
    }
    return OAL_SUCC;
}
