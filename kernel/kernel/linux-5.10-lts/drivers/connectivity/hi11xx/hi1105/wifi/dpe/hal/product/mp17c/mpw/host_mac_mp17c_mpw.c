...
#undef THIS_FILE_ID
#define THIS_FILE_ID OAM_FILE_ID_HOST_HAL_MAC_MP17C_MPW_C

/* 功能：hardware vap id转换为hal层vap id */
uint8_t mp17c_mpw_hw_vap_id_to_hal_vap_id(uint8_t hw_vap_id)
{
...
uint8_t mp17c_mpw_hal_vap_id_to_hw_vap_id(uint8_t hal_vap_id)
{
    /* 入参由函数调用者保证有效性 */
...
uint32_t mp17c_mpw_get_vap_reg_offset(uint8_t hal_vap_id)
{
    uint8_t hw_vap_id = mp17c_mpw_hal_vap_id_to_hw_vap_id(hal_vap_id);
...
{
    *status = hal_host_readl_irq(hal_device->mac_regs.irq_common_status_reg);
}
...
void mp17c_mpw_clear_host_mac_common_int_status(hal_host_device_stru *hal_dev, uint32_t status)
{
    ...
    hal_host_writel_irq(status, hal_dev->mac_regs.irq_common_clr_reg);
}
...
static uint32_t mp17c_mpw_common_regs_addr_get_offset(uint32_t reg_addr, uint32_t *offset)
{
...
{
    uint32_t offset = 0;
    uint32_t mac_offset[WLAN_DEVICE_MAX_NUM_PER_CHIP_MP17C] = {MP17C_MAC_BANK_REG_OFFSET, MP17C_MAC_BANK_REG_OFFSET};
    ...
    if (mp17c_mpw_common_regs_addr_get_offset(reg_addr, &offset) == OAL_SUCC) {
        return offset;
    }
#ifndef _PRE_LINUX_TEST
    switch (reg_addr) {
...
static uint32_t mp17c_mpw_regs_addr_transfer(hal_host_device_stru *hal_device, uint32_t reg_addr)
{
    uint64_t host_va = 0;
...
uint32_t mp17c_mpw_host_regs_addr_init(hal_host_device_stru *hal_device)
{
    ...
    if (hal_device->device_id >= dev_num) {
        oam_warning_log1(0, OAM_SF_ANY, "{host_regs_addr_init::device_id[%x]!}", hal_device->device_id);
        return OAL_FAIL;
    }
    /* 唤醒低功耗内存 */
    ...
    for (idx = 0; idx < sizeof(hal_regs) / sizeof(uint32_t); idx++) {
        if (mp17c_mpw_regs_addr_transfer(hal_device, hal_regs[idx]) != OAL_SUCC) {
            hal_pm_try_wakeup_allow_sleep(HAL_PM_FORBIDE_SLEEP_HOST_REG_ADDR_INIT);
...
void mp17c_mpw_host_intr_regs_init(uint8_t hal_dev_id)
{
    ...
    if (mp17c_mpw_host_regs_addr_init(hal_device) != OAL_SUCC) {
        return;
    }
...
void mp17c_mpw_host_set_hal_mac_common_timer(hal_mac_common_timer *mac_common_timer)
{
...
/*
 * 函 数 名  : bismp17c_mpwst_set_mac_common_timer
 * 功能描述  : 配置通用MAC定时器
 * 1.日    期  : 2020年12月22日
 *   作    者  : wifi
 *   修改内容  : 新生成函数
...
void mp17c_mpw_host_set_mac_common_timer(hal_mac_common_timer *mac_common_timer)
{
    ...
    mp17c_mpw_host_set_hal_mac_common_timer(mac_common_timer);
    hal_pm_try_wakeup_allow_sleep(HAL_PM_FORBIDE_SLEEP_SET_MAC_COMMON_TIMER);
}
