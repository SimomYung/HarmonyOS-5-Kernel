...
#undef THIS_FILE_ID
#define THIS_FILE_ID OAM_FILE_ID_HOST_SNIFFER_MP17C_C

/*
 * 函数描述：rx ppdu描述符 free ring 初始化
 */
uint32_t mp17c_rx_ppdu_dscr_free_ring_init(hal_host_device_stru *hal_dev)
{
    uint16_t entries;
    ...
    rx_ring_ctl->ring_subtype = HAL_RING_TYPE_P_F;
...
static uint32_t mp17c_dma_map_rx_ppdu_dscr(
    uint8_t dma_dir, hal_host_rx_ppdu_dscr_stru *ppdu_dscr, dma_addr_t *dma_addr)
{
    ...
    *dma_addr = dma_map_single(&pcie_dev->dev, (uint8_t *)&ppdu_dscr->hw_ppdu_data,
        sizeof(mp17c_rx_ppdu_dscr_stru), dma_dir);
 ...
 */
uint32_t mp17c_add_rx_ppdu_dscr(hal_host_device_stru *hal_device)
{
    uint64_t devva = 0;
    ...
    for (loop = 0; loop < need_add; loop++) {
        ppdu_dscr = oal_memalloc(MEM_RX_PPDU_DSCR_SIZE);
        if (ppdu_dscr == NULL) {
            oam_error_log0(0, OAM_SF_RX, "host_rx_add_buff_alloc alloc fail");
            break;
        }
        memset_s(ppdu_dscr, MEM_RX_PPDU_DSCR_SIZE, 0, MEM_RX_PPDU_DSCR_SIZE);

        if (mp17c_dma_map_rx_ppdu_dscr(DMA_FROM_DEVICE, ppdu_dscr, &pci_dma_addr) != OAL_SUCC) {
            oal_free(ppdu_dscr);
 ...
 */
uint32_t mp17c_sniffer_rx_ppdu_free_ring_init(hal_host_device_stru *hal_device)
{
    /* ring 初始化 */
    if (mp17c_rx_ppdu_dscr_free_ring_init(hal_device) != OAL_SUCC) {
    ...
    /* 向 free ring添加内存 */
    if (mp17c_add_rx_ppdu_dscr(hal_device) != OAL_SUCC) {
 ...
 * 功能描述: 释放rx ppdu ring
 */
void mp17c_rx_ppdu_from_list_free(hal_host_rx_ppdu_dscr_list_head_stru *ppdu_list)
{
    hal_host_rx_ppdu_dscr_stru *ppdu_dscr = NULL;
 ...
 */
void mp17c_sniffer_rx_ppdu_free_ring_deinit(hal_host_device_stru *hal_device)
{
    ...
    /* 释放alloc list */
    mp17c_rx_ppdu_from_list_free(&hal_device->host_rx_ppdu_alloc_list);
    /* 释放ring本身 */
 ...
 */
void mp17c_sniffer_set_rx_info(hal_sniffer_extend_info *sniffer_rx_info, mac_rx_ctl_stru *cb,
    mp17c_rx_ppdu_dscr_stru *ppdu_dscr)
{
 ...
 * 功能描述：rx速率信息填充
 */
void mp17c_sniffer_set_rx_rate_info(hal_sniffer_extend_info *sniffer_rx_info,
    mp17c_rx_ppdu_dscr_stru *ppdu_dscr)
{
...
static uintptr_t mp17c_get_rx_ppdu_dscr_addr(oal_netbuf_stru *netbuf)
{
    uint64_t rx_ppdu_dscr_addr = SNIFFER_INVALIAD_ADDR;
    mp17c_rx_msdu_dscr_stru *rx_hw_dscr = NULL;

    /* 之前函数中修改data指针指向帧体 为获取帧头部分mpdu描述符中的lsb和msb 修改data指针指向帧头 */
    oal_netbuf_push(netbuf, MP17C_RX_MSDU_DSCR_LEN);

    rx_hw_dscr = (mp17c_rx_msdu_dscr_stru *)oal_netbuf_data(netbuf);
    if (rx_hw_dscr == NULL) {
    ...
    oal_netbuf_pull(netbuf, MP17C_RX_MSDU_DSCR_LEN);

    return rx_ppdu_dscr_addr;
}

hal_host_rx_ppdu_dscr_stru *mp17c_sniffer_get_rx_ppdu_dscr(hal_host_device_stru *hal_dev, oal_netbuf_stru *netbuf)
{
    ...
    rx_ppdu_dscr_addr = mp17c_get_rx_ppdu_dscr_addr(netbuf);
...
void mp17c_sniffer_rx_info_fill_host_rx(hal_host_device_stru *hal_device, oal_netbuf_stru *netbuf,
    hal_sniffer_extend_info *sniffer_rx_info, mac_rx_ctl_stru *rx_ctl)
{
    ...
    rx_ppdu = mp17c_sniffer_get_rx_ppdu_dscr(hal_device, netbuf);
    if (rx_ppdu == NULL) {
        return;
    }

    dma_unmap_single(&pcie_dev->dev, rx_ppdu->paddr.addr,
        sizeof(mp17c_rx_ppdu_dscr_stru), DMA_FROM_DEVICE);

    mp17c_sniffer_set_rx_info(sniffer_rx_info, rx_ctl, &rx_ppdu->hw_ppdu_data);
    mp17c_sniffer_set_rx_rate_info(sniffer_rx_info, &rx_ppdu->hw_ppdu_data);

    oal_dlist_delete_entry(&rx_ppdu->entry);
    oal_free(rx_ppdu);
}

void mp17c_sniffer_rx_info_fill_device_rx(hal_host_device_stru *hal_device, oal_netbuf_stru *netbuf,
    hal_sniffer_extend_info *sniffer_rx_info, mac_rx_ctl_stru *rx_ctl)
{
...
void mp17c_sniffer_rx_info_fill(hal_host_device_stru *hal_device, oal_netbuf_stru *netbuf,
    hal_sniffer_extend_info *sniffer_rx_info, mac_rx_ctl_stru *rx_ctl)
{
    ...
    if (oal_atomic_read(&hal_device->rx_mode) == HAL_DDR_RX) {
        mp17c_sniffer_rx_info_fill_host_rx(hal_device, netbuf, sniffer_rx_info, rx_ctl);
    } else if (oal_atomic_read(&hal_device->rx_mode) == HAL_RAM_RX) {
        mp17c_sniffer_rx_info_fill_device_rx(hal_device, netbuf, sniffer_rx_info, rx_ctl);
    } else {
