...
void hmac_tid_ring_switch(uint16_t user_idx, hmac_tid_info_stru *tid_info, uint32_t mode)
{
    ...
    dpe_vap = (dpe_wlan_vap_stru *)dpe_wlan_vap_get(dpe_user->uc_vap_id);
    if (dpe_vap == NULL) {
        oam_warning_log1(0, 0, "{hmac_tx_ring_switch::NULL vap id[%d]!}", dpe_user->uc_vap_id);
        return;
    }

    if (dpe_vap->en_vap_state != MAC_VAP_STATE_UP ||
        dpe_user->en_user_asoc_state != MAC_USER_STATE_ASSOC) {
        oam_warning_log2(dpe_vap->uc_vap_id, 0,
            "{hmac_tid_ring_switch::user_state[%d],vap_state[%d] can't switch}",
            dpe_user->en_user_asoc_state, dpe_vap->en_vap_state);
        return;
    }
...
static inline uint8_t hmac_tx_ring_switch_allowed(uint16_t user_id, uint8_t tidno)
{
#ifdef _PRE_WLAN_FEATURE_VSP
    hmac_user_stru *hmac_user = mac_res_get_hmac_user(user_id);
    if (hmac_user == NULL) {
        oam_warning_log1(0, 0, "{hmac_tx_ring_switch_allowed::NULL user id[%d]!}", user_id);
        return OAL_FALSE;
    }
...
功能：程序动态切换入口
 */
uint8_t hmac_tx_ring_switch(hmac_tid_info_stru *tid_info, void *param) /* 是否会有并行问题？ */
{
    uint16_t user_id = tid_info->user_index;

    if (hmac_tx_ring_switch_allowed(user_id, tid_info->tid_no) == OAL_FALSE) {
        return OAL_CONTINUE;
    }
    ...
    hmac_tid_ring_switch(user_id, tid_info, *(uint32_t*)param);

    return OAL_CONTINUE;
}

OAL_STATIC OAL_INLINE void hmac_tx_ring_switch_pm_check(void)
{
    if (g_tid_switch_list.pm_quick_switch_mode == OAL_TRUE) {
...
{
    uint32_t mode;
    if (!dpe_tx_ring_switch_enabled()) {
        return;
    }
    hmac_tx_ring_switch_pm_check();
    /* pps状态机状态切换 */
...
void hmac_tid_ring_switch_disable(uint16_t user_idx)
{
    ...
    for (tid_no = WLAN_TIDNO_BEST_EFFORT; tid_no < WLAN_TIDNO_BCAST; tid_no++) {
        ...
        hmac_tid_ring_switch_list_delete(tid_info);
    }
}

static uint8_t hmac_tid_switch_list_tid_info_dump(hmac_tid_info_stru *tid_info, void *param)
{
    if (oal_unlikely(tid_info == NULL)) {
        oam_error_log0(0, 0, "{hmac_tid_switch_list_tid_info_dump::tid_info NULL}");
        return OAL_RETURN;
    }

    oam_warning_log2(0, 0, "{hmac_tid_switch_list_tid_info_dump::user[%d] tid[%d]}",
        tid_info->user_index, tid_info->tid_no);
...
{
    oam_warning_log1(0, 0, "{hmac_tid_switch_list_dump::type[%d]}", type);
    hmac_tid_list_for_each(&g_tid_switch_list.tid_list, hmac_tid_switch_list_tid_info_dump, NULL);
}