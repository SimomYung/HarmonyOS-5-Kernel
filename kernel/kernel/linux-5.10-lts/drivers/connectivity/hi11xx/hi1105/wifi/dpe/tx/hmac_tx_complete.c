...
static void hmac_tx_complete_netbuf_process(hmac_tid_info_stru *tid_info, hmac_msdu_info_ring_stru *tx_ring,
    un_rw_ptr release_ptr, oal_netbuf_head_stru *need_unmap)
{
    ...
    if (oal_unlikely(tx_ctl == NULL)) {
        oam_error_log0(0, 0, "oal_netbuf_cb fail");
        return;
    }
    ...
    if (hmac_tid_ring_tx_opt(tid_info) != 0 || mac_get_cb_is_needretry(tx_ctl) == OAL_FALSE) {
        hmac_tx_ring_release_netbuf_info(tx_ring, release_ptr.st_rw_ptr.bit_rw_ptr);
        oal_netbuf_add_to_list_tail(netbuf, need_unmap);
        return;
    }
...
static void hmac_tx_complete_msdu_info_process(hmac_tid_info_stru *tid_info, hmac_msdu_info_ring_stru *tx_ring)
{
    ...
    while (hal_tx_rw_ptr_compare(target_ptr, release_ptr) == RING_PTR_GREATER) {
        hmac_tx_complete_netbuf_process(tid_info, tx_ring, release_ptr, &before);
    ...
    hal_host_tx_dma_unmap_proc(&before, &after);
    hal_host_clear_netbuf_list(&after);
}
 ...
 *   修改内容 : 重构函数
 */
void hmac_tx_complete_tid_ring_process(dpe_wlan_user_stru *dpe_user, hmac_tid_info_stru *tid_info)
{
    hmac_msdu_info_ring_stru *tx_ring = &tid_info->tx_ring;
    dpe_wlan_vap_stru *dpe_vap = dpe_wlan_vap_get(dpe_user->uc_vap_id);
    ...
    hmac_host_ring_tx_resume(dpe_vap, dpe_user, tid_info);
...
uint32_t hmac_tx_reg_read_rptr(hmac_tid_info_stru *tid_info, uint32_t *rptr)
{
    ...
    if (hal_host_tx_ring_get_ops(&param, HAL_HOST_TX_GET_HW_RPTR) != OAL_SUCC) {
        return OAL_FAIL;
    }
...
OAL_STATIC uint32_t hmac_tx_reg_read_ml1_rptr(hmac_tid_info_stru *tid_info, uint32_t *rptr)
{
    hal_host_tx_ring_ops_param_stru param = {
        .tx_ring_table_addr = tid_info->tx_ring.tx_ring_table_addr.addr_val,
        .lut_idx = tid_info->lut_index,
    ...
    if (hal_host_tx_ring_get_ops(&param, HAL_HOST_TX_GET_HW_ML1_RPTR) != OAL_SUCC) {
        return OAL_FAIL;
    }
...
OAL_STATIC uint32_t hmac_tx_reg_read_ml_bitmap(hmac_tid_info_stru *tid_info, uint32_t *ml_bitmap)
{
    hal_host_tx_ring_ops_param_stru param = {
        .tx_ring_table_addr = tid_info->tx_ring.tx_ring_table_addr.addr_val,
        .lut_idx = tid_info->lut_index,
    ...
    if (hal_host_tx_ring_get_ops(&param, HAL_HOST_TX_GET_HW_ML_BITMAP) != OAL_SUCC) {
        return OAL_FAIL;
    }
...
{
    uint32_t ml_bitmap;
    msdu_info_ring_ml_stru *ml_ring_info = &(tid_info->tx_ring.ml_ring_info);
    /* MLD模式变化时，会重新设置bitmap，软件刷新需要等ring table中的设置完成后才能更新 */
    if (ml_ring_info->multi_link_bitmap_switch == OAL_TRUE) {
        hmac_tx_reg_read_ml_bitmap(tid_info, &ml_bitmap);
        if (ml_ring_info->multi_link_bitmap_next != ml_bitmap) {
            /* 此处只会是暂态，稳态时multi_link_bitmap_next和ml_bitmap相等 */
            oam_warning_log2(0, 0, "hmac_tx_comp_update_ml_bitmap next[0x%x] ring_table_bitmap[0x%x]",
                ml_ring_info->multi_link_bitmap_next, ml_bitmap);
            return;
        } else {
            ml_ring_info->multi_link_bitmap = ml_ring_info->multi_link_bitmap_next;
            ml_ring_info->multi_link_bitmap_switch = OAL_FALSE;
        }
    }
}
...
uint32_t hmac_tx_complete_update_mlink_ring_rptr(hmac_tid_info_stru *tid_info)
{
    ...
    hal_pm_try_wakeup_forbid_sleep(HAL_PM_FORBIDE_SLEEP_UPDATE_MLINK_RING_RPTR);
    if (hal_pm_try_wakeup_dev_lock() != OAL_SUCC) {
        hal_pm_try_wakeup_allow_sleep(HAL_PM_FORBIDE_SLEEP_UPDATE_MLINK_RING_RPTR);
        return OAL_FAIL;
    }
    ...
    if (tid_info->tx_ring.ml_ring_info.multi_link_bitmap == HAL_MULTILINK_TX_BITMAP_LINK0) {
        ...
        tid_info->tx_ring.base_ring_info.read_index = hal_host_tx_rw_smaller_ptr((uint16_t)rptr, (uint16_t)ml1_rptr,
...
uint32_t hmac_tx_complete_update_tid_ring_rptr(hmac_tid_info_stru *tid_info)
{
    ...
    hal_pm_try_wakeup_forbid_sleep(HAL_PM_FORBIDE_SLEEP_UPDATE_TID_RING_RPTR);
    if (hal_pm_try_wakeup_dev_lock() != OAL_SUCC) {
        hal_pm_try_wakeup_allow_sleep(HAL_PM_FORBIDE_SLEEP_UPDATE_TID_RING_RPTR);
        return OAL_FAIL;
    }
...
{
    if (g_wlan_spec_cfg->emlsr_str_link == 0) {
...
static void hmac_tx_complete_tid_info_process_nolock(uint16_t user_idx, hmac_tid_info_stru *tid_info)
{
...
#endif
    hmac_tx_complete_tid_ring_process(dpe_user, tid_info);
...
static void hmac_tx_complete_ba_info_process(hmac_tx_comp_info_stru *tx_comp_info, uint8_t tx_comp_info_cnt)
{
    ...
    for (idx = 0; idx < tx_comp_info_cnt; idx++) {
        tid_info = hmac_get_tx_tid_info(tx_comp_info[idx].dpe_user->us_assoc_id,
            tx_comp_info[idx].tid_no);
        if (tid_info == NULL) {
            continue;
        }
        oal_mutex_lock(&tid_info->tx_ring.tx_comp_lock);
        hmac_tx_complete_tid_info_process_nolock(tx_comp_info[idx].dpe_user->us_assoc_id, tid_info);
...
static void hmac_tx_complete_record_ba_info(dpe_wlan_user_stru *dpe_user, hal_tx_ba_info_stru *ba_info,
    hmac_tx_comp_info_stru *tx_comp_info, uint8_t *info_cnt)
{
    uint8_t idx;

    for (idx = 0; idx < *info_cnt; idx++) {
        if (dpe_user->us_assoc_id == tx_comp_info[idx].dpe_user->us_assoc_id &&
            ba_info->tid_no == tx_comp_info[idx].tid_no) {
            return;
        }
    }

    tx_comp_info[*info_cnt].dpe_user = dpe_user;
    ...
    hal_tx_ba_info_stru *ba_info, hmac_tx_comp_info_stru *tx_comp_info, uint8_t *tx_comp_info_cnt)
{
    ...
    dpe_user = dpe_wlan_get_user_by_lut_index(ba_info->user_id);
    if (oal_unlikely(dpe_user == NULL)) {
        oam_error_log1(0, 0, "{hmac_tx_complete_process_ba_info::dpe_user is NULL, lut_idx[%d]}", ba_info->user_id);
        return;
    }

    /* 记录待处理user tid, 筛选掉重复的user tid ba info, 避免反复处理 */
    hmac_tx_complete_record_ba_info(dpe_user, ba_info, tx_comp_info, tx_comp_info_cnt);
}
...
void hmac_tx_complete_process_by_ba_info(hal_host_device_stru *hal_device)
{
    ...
    hal_pm_try_wakeup_forbid_sleep(HAL_PM_FORBIDE_SLEEP_PROCESS_BY_BA_INFO);
    if (hal_pm_try_wakeup_dev_lock() != OAL_SUCC) {
        hal_pm_try_wakeup_allow_sleep(HAL_PM_FORBIDE_SLEEP_PROCESS_BY_BA_INFO);
        return;
    }
...
{
    dpe_wlan_user_stru *dpe_user = dpe_wlan_user_get(tid_info->user_index); /* user id 被修改后，确保切之前的发送完成 能处理完 */

    if (dpe_user == NULL) {
        return;
    }
    hmac_tx_complete_tid_info_process_nolock(dpe_user->us_assoc_id, tid_info);
}
