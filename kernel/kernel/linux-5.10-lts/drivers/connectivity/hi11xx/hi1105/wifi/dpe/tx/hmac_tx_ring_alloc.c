...
{
    return dpe_host_ring_tx_enabled() ? HOST_RING_TX_MODE : DEVICE_RING_TX_MODE;
}

static void hmac_tid_info_init(hmac_tid_info_stru *tid_info, uint16_t user_idx, uint8_t tid)
{
    ...
    tid_info->tid_no = tid;
    tid_info->user_index = dpe_user->us_assoc_id;
    tid_info->lut_index = hal_host_get_tx_ring_lut_idx(dpe_user->lut_index, dpe_user->ml_user_lut_idx);
    ...
    OAL_MUTEX_INIT(&tid_info->tx_ring.tx_lock);
    OAL_MUTEX_INIT(&tid_info->tx_ring.tx_comp_lock);
...
OAL_STATIC uint8_t hmac_tx_ring_get_mld_bitmap(uint16_t user_idx)
{
    ...
    if (dpe_vap->mld_mode != WLAN_LEGACY_MLD_MODE && dpe_vap->mld_assoc_mode == MLD_ASSOC_MODE_STR) {
...
void hmac_tx_ring_set_mld_bitmap_next(uint16_t user_idx, uint8_t bitmap)
{
    ...
    if (user_idx >= WLAN_USER_MAX_USER_LIMIT) {
        oam_error_log1(0, OAM_SF_TX,
            "hmac_tx_ring_set_mld_bitmap::user is null. bitmap 0x%x", bitmap);
        return;
    }
    ...
    for (tid = 0; tid < WLAN_TID_MAX_NUM; tid++) {
        tid_info = hmac_get_tx_tid_info(user_idx, tid);
        if (tid_info == NULL) {
            continue;
        }
        ...
        ml_info->multi_link_bitmap_next = bitmap;
        ml_info->multi_link_bitmap_switch = OAL_TRUE;
    }
}

static void hmac_host_tx_ring_ml_init(uint16_t user_idx, hmac_msdu_info_ring_stru *tx_ring)
{
...
#endif
    ml_info->multi_link_bitmap = hmac_tx_ring_get_mld_bitmap(user_idx);
}

#define HMAC_TX_RING_MAX_AMSDU_NUM 7
static uint32_t hmac_host_tx_ring_init(uint16_t user_idx, hmac_tid_info_stru *tid_info, uint8_t tid)
{
    hmac_msdu_info_ring_stru *tx_ring = &tid_info->tx_ring;

    if (!dpe_host_ring_tx_enabled()) {
        return OAL_SUCC;
    }
    ...
    hmac_host_tx_ring_ml_init(user_idx, tx_ring);
...
OAL_STATIC uint32_t hmac_alloc_tx_ring(hmac_msdu_info_ring_stru *tx_ring)
{
...
{
    hmac_tid_info_stru *tid_info = hmac_get_tx_tid_info(user_idx, tid);
    if (tid_info == NULL) {
        return OAL_FAIL;
    }
    if ((dpe_ring_tx_enabled() != OAL_TRUE) || tid == WLAN_TIDNO_BCAST) {
        return OAL_SUCC;
    }

    hmac_tid_info_init(tid_info, user_idx, tid);

    if (hmac_host_tx_ring_init(user_idx, tid_info, tid) != OAL_SUCC) {
...
uint32_t hmac_alloc_tx_ring_after_inited(hmac_tid_info_stru *tid_info, hmac_msdu_info_ring_stru *tx_ring)
{
    ...
    /* ring初始化时，当前user的mld模式改变过，next直接设置到当前bitmap即可 */
    if (tid_info->tx_ring.ml_ring_info.multi_link_bitmap_switch == OAL_TRUE) {
        tid_info->tx_ring.ml_ring_info.multi_link_bitmap = tid_info->tx_ring.ml_ring_info.multi_link_bitmap_next;
        tid_info->tx_ring.ml_ring_info.multi_link_bitmap_switch = OAL_FALSE;
    }

    hmac_tx_reg_write_ring_info(tid_info, TID_CMDID_CREATE, OAL_FALSE);
    oal_mutex_unlock(&tid_info->tx_ring.tx_comp_lock);
...
{
    hal_pm_try_wakeup_forbid_sleep(HAL_PM_FORBIDE_SLEEP_WAIT_RING_TX_DONE);
    if (hal_pm_try_wakeup_dev_lock() != OAL_SUCC) {
        hal_pm_try_wakeup_allow_sleep(HAL_PM_FORBIDE_SLEEP_WAIT_RING_TX_DONE);
        return OAL_FAIL;
    }

    hmac_tx_reg_write_ring_info(tid_info, TID_CMDID_STOP_TX, OAL_FALSE);
    hal_pm_try_wakeup_allow_sleep(HAL_PM_FORBIDE_SLEEP_WAIT_RING_TX_DONE);

    if (hmac_tx_ring_alloced(&tid_info->tx_ring)) {
        oal_msleep(20); /* wait 20ms for mac to complete host ring tx */
    }

    return OAL_SUCC;
}

static void hmac_host_tx_ring_deinit(dpe_wlan_user_stru *dpe_user, hmac_tid_info_stru *tid_info)
{
    dpe_wlan_vap_stru *dpe_vap = NULL;

    if (!dpe_host_ring_tx_enabled() || !oal_atomic_read(&tid_info->tx_ring.inited)) {
        return;
    }
    ...
    hmac_tid_ring_switch_disable(dpe_user->us_assoc_id);
    hal_pm_try_wakeup_forbid_sleep(HAL_PM_FORBIDE_SLEEP_HOST_TX_RING_DEINIT);
    if (hal_pm_try_wakeup_dev_lock() == OAL_SUCC) {
        hmac_tx_reg_write_ring_info(tid_info, TID_CMDID_DEL, OAL_FALSE);
    }
    hal_pm_try_wakeup_allow_sleep(HAL_PM_FORBIDE_SLEEP_HOST_TX_RING_DEINIT);
...
void hmac_ring_tx_deinit(uint16_t user_idx, uint8_t tid)
{
    ...
    if (tid_info == NULL || dpe_user == NULL) {
        return;
    }
    ...
    if (dpe_user->tid_res != NULL) {
        ...
        if (tid_res->work_flag == 0) {
            oam_error_log2(0, 0, "hmac_ring_tx_deinit::user[%d] tid[%d] work flag error = 0",
                dpe_user->us_assoc_id, tid);
        }
        /* =1说明只有当前单个user使用tid资源，不存在共用，可直接释放tx ring */
        if (tid_res->work_flag != 1) {
            oam_warning_log1(0, 0, "hmac_ring_tx_deinit::another link using(work_flag[%d])", tid_res->work_flag);
            return;
        }
    }

    hmac_tid_info_deinit(tid_info);
    hmac_host_tx_ring_deinit(dpe_user, tid_info);
    /* 检查tid_info */
...
void hmac_user_tx_ring_deinit(uint16_t user_idx)
{
    ...
    for (tid = 0; tid < WLAN_TID_MAX_NUM; tid++) {
        hmac_ring_tx_deinit(user_idx, tid);
    }
...
#ifndef _PRE_MULTI_CORE_DPE_OFFLOAD
    hmac_config_send_event_adapt_dpe(0, WLAN_CFGID_HOST_BA_INFO_RING_SYN,
...
uint32_t hmac_set_tx_ring_device_base_addr(frw_event_mem_stru *frw_mem)
{
    ...
    for (dev_id = 0; dev_id < dev_num; dev_id++) {
        hal_host_ba_ring_regs_init(dev_id);
    }
