...
#undef THIS_FILE_ID
#define THIS_FILE_ID OAM_FILE_ID_HOST_HAL_CSI_MP12_C
 ...
 *   修改内容  : 新生成函数
 */
static uint32_t mp12_regs_csi_transfer(hal_host_device_stru *hal_device, uint32_t reg_addr)
{
    uint64_t host_va = 0;
 ...
 *   修改内容  : 新生成函数
 */
void mp12_csi_base_addr_init(hal_host_device_stru *hal_device)
{
 ...
 *   修改内容  : 新生成函数
 */
uint32_t mp12_csi_reg_init(hal_host_device_stru *hal_device)
{
    uint32_t idx;
    uint32_t hal_regs[] = {
    ...
    if (hal_device->csi_regs.inited == OAL_FALSE) {
        for (idx = 0; idx < sizeof(hal_regs) / sizeof(uint32_t); idx++) {
            if (mp12_regs_csi_transfer(hal_device, hal_regs[idx]) != OAL_SUCC) {
                hal_device->csi_regs.inited = OAL_FALSE;
                return OAL_FAIL;
            }
        }
...
/*
 * 函 数 名  : mp12_host_set_csi_buf_base_addr
 * 功能描述  : 设置CSI采样上报片外循环BUF基地址
 * 1.日    期  : 2020年11月17日
 *   作    者  : wifi
 *   修改内容  : 新生成函数
 */
void mp12_host_set_csi_buf_base_addr(hal_host_device_stru *hal_device, uintptr_t devva)
{
    if (devva == 0) {
...
/*
 * 函 数 名  : mp12_host_set_csi_buf_size
 * 功能描述  : 设置CSI采样上报片外循环BUF大小
 * 1.日    期  : 2020年11月17日
 *   作    者  : wifi
 *   修改内容  : 新生成函数
 */
void mp12_host_set_csi_buf_size(hal_host_device_stru *hal_device, uint16_t cfg_csi_buf_size)
{
    hal_host_writel(cfg_csi_buf_size, (uintptr_t)hal_device->csi_regs.size);
}

/*
 * 函 数 名  : mp12_host_set_csi_en
 * 功能描述  : 开启和关闭CSI
 * 1.日    期  : 2020年11月17日
 *   作    者  : wifi
 *   修改内容  : 新生成函数
 */
void mp12_host_set_csi_en(hal_host_device_stru *hal_device, uint32_t reg_value)
{
...
/*
 * 函 数 名  : mp12_host_set_phy_info_en
 * 功能描述  : 设置phy info上报使能
 * 1.日    期  : 2020年11月17日
 *   作    者  : wifi
 *   修改内容  : 新生成函数
 */
void mp12_host_set_phy_info_en(hal_host_device_stru *hal_device, uint32_t reg_value)
{
...
/*
 * 函 数 名  : mp12_host_set_csi_bw
 * 功能描述  : 设置CSI采集的带宽
 * 1.日    期  : 2020年11月17日
 *   作    者  : wifi
 *   修改内容  : 新生成函数
 */
void mp12_host_set_csi_bw(hal_host_device_stru *hal_device, uint32_t reg_value)
{
    oal_setl_bits((void *)hal_device->csi_regs.csi_pro_reg,
...
/*
 * 函 数 名  : mp12_host_set_csi_frame_type
 * 功能描述  : 针对以下帧类型进行CSI采集上报
 * 1.日    期  : 2020年11月17日
 *   作    者  : wifi
 *   修改内容  : 新生成函数
 */
void mp12_host_set_csi_frame_type(hal_host_device_stru *hal_device, uint32_t reg_value)
{
    oal_setl_bits((void *)hal_device->csi_regs.csi_pro_reg,
...
/*
 * 函 数 名  : mp12_host_set_csi_extrat_en
 * 功能描述  : 设置CSI抽取,extra_en置一时,只有160M会抽取
 * 1.日    期  : 2020年11月17日
 *   作    者  : wifi
 *   修改内容  : 新生成函数
 */
void mp12_host_set_csi_extrat_en(hal_host_device_stru *hal_device, uint32_t reg_value)
{
    oal_setl_bits((void *)hal_device->csi_regs.csi_pro_reg,
...
/*
 * 函 数 名  : mp12_host_set_csi_sample_period
 * 功能描述  : 设置CSI采样周期
 * 1.日    期  : 2020年11月17日
 *   作    者  : wifi
 *   修改内容  : 新生成函数
 */
void mp12_host_set_csi_sample_period(hal_host_device_stru *hal_device, uint32_t reg_value)
{
    oal_setl_bits((void *)hal_device->csi_regs.csi_pro_reg,
                  MAC_PPU_REG_CFG_ML0_CSI_SAMPLE_PERIOD_OFFSET, MAC_PPU_REG_CFG_ML0_CSI_SAMPLE_PERIOD_LEN,
                  reg_value);
...
void mp12_host_set_csi_ta(hal_host_device_stru *hal_device, uint8_t *mac_addr)
{
 ...
 *   修改内容  : 新生成函数
 */
static uint32_t mp12_config_csi_buffer_addr(hal_host_device_stru *hal_device, uint32_t cfg_csi_on)
{
    int32_t ret;
    ...
    if (cfg_csi_on == OAL_TRUE) {
        if (hal_device->csi_regs.rbase_vaddr == NULL) {
            ...
            mp12_host_set_csi_buf_base_addr(hal_device, (uintptr_t)hal_device->csi_regs.devva);
            mp12_host_set_csi_buf_size(hal_device, HMAC_CSI_BUFF_SIZE);
        }
    } else {
        if (hal_device->csi_regs.rbase_vaddr != NULL) {
            dma_free_coherent(&pcie_dev->dev,
 ...
 *   修改内容  : 新生成函数
 */
uint32_t mp12_get_host_ftm_csi_locationinfo(hal_host_device_stru *hal_device)
{
...
uint32_t mp12_csi_config(mac_vap_stru *mac_vap, uint16_t len, uint8_t *param)
{
    ...
    if (mp12_csi_reg_init(hal_device) != OAL_SUCC) {
    ...
    mp12_host_mac_irq_unmask(hal_device, MAC_COMMON_REG_RPT_HOST_INTR_LOCATION_COMPLETE_OFFSET);
    mp12_host_set_csi_en(hal_device, OAL_FALSE);
    mp12_host_set_phy_info_en(hal_device, OAL_FALSE);

    /* 设置带宽 */
    mp12_host_set_csi_bw(hal_device, (uint32_t)cfg_csi_param->uc_csi_bw);

    /* 设置CSI采集的帧类型 */
    mp12_host_set_csi_frame_type(hal_device, (uint32_t)cfg_csi_param->uc_csi_frame_type);

    /* 设置CSI采样周期 */
    mp12_host_set_csi_sample_period(hal_device, (uint32_t)cfg_csi_param->uc_csi_sample_period);

    if (!mac_addr_is_zero(cfg_csi_param->auc_mac_addr)) {
        /* 设置上报csi帧的TA mac地址 */
        mp12_host_set_csi_ta(hal_device, cfg_csi_param->auc_mac_addr);
    }

    if (mp12_config_csi_buffer_addr(hal_device, !!cfg_csi_on) != OAL_SUCC) {
        hal_pm_try_wakeup_allow_sleep();
        return OAL_FAIL;
    }

    mp12_host_set_csi_en(hal_device, !!cfg_csi_on); /* !! 非零传入1, 0传入0 */
    mp12_host_set_phy_info_en(hal_device, !!cfg_csi_on);
    hal_pm_try_wakeup_allow_sleep();
...
uint32_t mp12_get_csi_info(hmac_csi_info_stru *hmac_csi_info, uint8_t *addr)
{
    mp12_csi_ppu_stru *csi_ppu = NULL;
    uint32_t timestamp;
    csi_ppu = (mp12_csi_ppu_stru *)addr;
...
uint32_t mp12_host_ftm_csi_init(hal_host_device_stru *hal_device)
{
    mp12_csi_base_addr_init(hal_device);
