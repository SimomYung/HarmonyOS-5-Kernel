...
#undef THIS_FILE_ID
#define THIS_FILE_ID OAM_FILE_ID_HOST_HAL_FTM_MP12_C
 ...
 *   修改内容  : 新生成函数
 */
static uint32_t mp12_regs_ftm_transfer(hal_host_device_stru *hal_device, uint32_t reg_addr)
{
    uint64_t host_va = 0;
 ...
 *   修改内容  : 新生成函数
 */
uint32_t mp12_host_ftm_reg_init(uint8_t hal_dev_id)
{
    uint32_t idx;
    uint32_t hal_regs[] = {
    ...
    if (!hal_device->ftm_regs.inited) {
        for (idx = 0; idx < sizeof(hal_regs) / sizeof(uint32_t); idx++) {
            if (mp12_regs_ftm_transfer(hal_device, hal_regs[idx]) != OAL_SUCC) {
                hal_pm_try_wakeup_allow_sleep();
                return OAL_FAIL;
            }
        }
 ...
 *   修改内容  : 新生成函数
 */
static void mp12_ftm_get_dscr_addr(uint64_t value, uint8_t *addr)
{
 ...
 *   修改内容   : 创建函数
 */
uint8_t mp12_host_vap_get_by_hw_vap_id(uint8_t hal_device_id, uint8_t hw_vap_id)
{
    ...
    if ((hw_vap_id == WLAN_HAL_OHTER_BSS_ID_MP12) || (hw_vap_id >= HAL_MAX_HW_VAP_NUM)) {
        return WLAN_VAP_MAX_NUM_LIMIT;
    }

    hal_vap_id = hal_hw_vap_id_to_hal_vap_id(hw_vap_id);
    hal_vap_id = hal_vap_id % HAL_MAX_VAP_NUM_MP12;
    return hal_device->hal_vap_sts_info[hal_vap_id].mac_vap_id;
}

uint32_t mp12_host_ftm_get_info(hal_ftm_info_stru *hal_ftm_info, uint8_t *addr)
{
    uint8_t idx;
    uint32_t *ftm_power_output = NULL;
    volatile uint32_t *ftm_power_input = NULL;
    mp12_ftm_dscr_stru *mp12_ftm_dscr = NULL;

    /* 获取hal_ftm_info的地址 */
    mp12_ftm_dscr = (mp12_ftm_dscr_stru *)addr;
    if (mp12_ftm_dscr == NULL) {
        return OAL_FAIL;
    }
    /* 唤醒低功耗内存 */
    hal_pm_try_wakeup_forbid_sleep();
    ...
    /* 2. 存储部分结果 */
    hal_ftm_info->ftm_vap_index = mp12_ftm_dscr->word4.bit_vap_index;
    hal_ftm_info->ftm_init_resp_flag = mp12_ftm_dscr->word10.bit_init_resp_flag;
    hal_ftm_info->ftm_intp_time = mp12_ftm_dscr->word10.bit_intp_time;
    hal_ftm_info->ftm_rx_time = mp12_ftm_dscr->word9.rx_time;
    hal_ftm_info->ftm_tx_time = mp12_ftm_dscr->word8.tx_time;
    hal_ftm_info->ftm_dialog_token = mp12_ftm_dscr->word4.bit_dialog_token;
    hal_ftm_info->ftm_timestamp = mp12_ftm_dscr->word7.timestamp;

    mp12_ftm_get_dscr_addr(mp12_ftm_dscr->word5.ra, &hal_ftm_info->ftm_ra[MAC_ADDR_0]);
    mp12_ftm_get_dscr_addr(mp12_ftm_dscr->word6.ta, &hal_ftm_info->ftm_ta[MAC_ADDR_0]);

    for (idx = 0; idx < HAL_HOST_MAX_RF_NUM; idx++) {
        hal_ftm_info->ftm_snr_ant[idx] = mp12_ftm_dscr->word1.snr_ant[idx];
        hal_ftm_info->ftm_rssi_ant[idx] = mp12_ftm_dscr->word1.rssi_hltf[idx];
    }
    hal_ftm_info->ftm_phase_incr_ant = mp12_ftm_dscr->word4.bit_phase_incr;
    ftm_power_input = (uint32_t *)((uint8_t *)mp12_ftm_dscr + sizeof(mp12_ftm_dscr_stru));
 ...
 *   修改内容  : 新生成函数
 */
void mp12_host_ftm_get_divider(hal_host_device_stru *hal_device, uint8_t *divider)
{
    uint32_t reg_value;
 ...
 *   修改内容  : 新生成函数
 */
void mp12_host_ftm_set_sample(hal_host_device_stru *hal_device, oal_bool_enum_uint8 ftm_status)
{
    /* 唤醒低功耗内存 */
    hal_pm_try_wakeup_forbid_sleep();
    if (hal_pm_try_wakeup_dev_lock() != OAL_SUCC) {
 ...
 *   修改内容  : 新生成函数
 */
void mp12_host_ftm_set_enable(hal_host_device_stru *hal_device, oal_bool_enum_uint8 ftm_status)
{
    /* 唤醒低功耗内存 */
    hal_pm_try_wakeup_forbid_sleep();
    if (hal_pm_try_wakeup_dev_lock() != OAL_SUCC) {
    ...
    /* 置位 PHY FTM cali enable位 mp12/61 设置为0，见问题单号 */
 ...
 *   修改内容  : 新生成函数
 */
void mp12_host_ftm_set_m2s_phy(hal_host_device_stru *hal_device, uint8_t tx_chain_selection,
 ...
 *   修改内容  : 新生成函数
 */
void mp12_host_ftm_set_buf_base_addr(hal_host_device_stru *hal_device, uintptr_t devva)
{
    if (devva == 0) {
 ...
 *   修改内容  : 新生成函数
 */
void mp12_host_ftm_set_buf_size(hal_host_device_stru *hal_device, uint16_t cfg_ftm_buf_size)
{
    /* 唤醒低功耗内存 */
    hal_pm_try_wakeup_forbid_sleep();
    if (hal_pm_try_wakeup_dev_lock() != OAL_SUCC) {
 ...
 *   修改内容  : 新生成函数
 */
uint32_t mp12_host_ftm_set_white_list(hal_host_device_stru *hal_device, uint8_t idx, uint8_t *mac_addr)
{
    uint32_t reg_value;
    uint32_t addr_lsb = 0;
    uint32_t addr_msb = 0;
...
static void mp12_host_ftm_copy_device_stru_to_host_stru(hal_ftm_info_device_stru *hal_ftm_info_device,
    hal_ftm_info_stru *hal_ftm_info)
{
    ...
    for (i = 0; i < HAL_HOST_MP12_RF_NUM; i++) {
 ...
 *   修改内容  : 新生成函数
 */
void mp12_host_ftm_transfer_device_stru_to_host_stru(uint8_t *device_param, uint8_t *host_param)
{
    ...
    mp12_host_ftm_copy_device_stru_to_host_stru(hal_ftm_info_device, hal_ftm_info);
}
 ...
 *   修改内容  : 新生成函数
 */
void mp12_host_ftm_get_rssi_selection(uint8_t *hmac_ftm, uint8_t *device_param)
{
    uint8_t chan_idx;
    ...
    for (chan_idx = 0; chan_idx < HAL_HOST_MP12_RF_NUM; ++chan_idx) {
