...
{
    hal_mac_common_timer *timer = mp12_get_host_mac_common_timer_ptr(16);
 ...
 *   修改内容   : 新生成函数
 */
void mp12_irq_host_common_timer_isr(hal_host_device_stru *hal_dev)
{
#ifdef _PRE_WLAN_FEATURE_VSP
 ...
 *   修改内容   : 新生成函数
 */
void mp12_irq_host_ftm_csi_isr(uint32_t status)
{
#ifdef _PRE_WLAN_FEATURE_CSI
...
void mp12_host_mac_phy_irq_mask(void)
{
    ...
    for (dev_id = 0; dev_id < hal_dev_num; dev_id++) {
        ...
        mp12_clear_host_phy_int_status(hal_dev, &status_mask);
    }
}

static void mp12_irq_host_mac_comm_isr_process(hal_host_device_stru *hal_dev,
    host_comm_intr_status_union intr_state)
{
    if (intr_state.bits.rpt_host_intr_location_complete != 0) {
        mp12_irq_host_ftm_csi_isr(intr_state.u32);
    } else if (intr_state.bits.rpt_host_intr_rx_ppdu_desc != 0) {
        /* RX PPDU描述符Complete Ring上报中断 */
        mp12_clear_host_mac_common_int_status(hal_dev, intr_state.u32);
        ...
        oam_warning_log1(0, OAM_SF_IRQ,
            "{mp12_irq_host_mac_comm_isr_process error, state.u32:0x%x.}", intr_state.u32);
        mp12_clear_host_mac_common_int_status(hal_dev, intr_state.u32);
    }
}
static void mp12_irq_host_mac_isr_process(hal_host_device_stru *hal_dev, host_intr_status_union intr_state)
{
    if (intr_state.bits.rpt_host_intr_tx_complete != 0) {
        /* 发送完成上报中断 */
        hal_irq_host_tx_complete_isr(hal_dev, intr_state.u32);
        mp12_clear_host_mac_int_status(hal_dev, intr_state.u32);
    } else if (intr_state.bits.rpt_host_intr_rx_self_frame != 0) {
        /* MAC地址冲突异常中断,芯片会丢帧 */
        oam_error_log1(0, OAM_SF_IRQ, "{mp12_irq_host_mac_isr_process error(rx self frame), state.u32:0x%x.}",
            intr_state.u32);
        mp12_clear_host_mac_int_status(hal_dev, intr_state.u32);
        ...
        /* 通用计时器中断 */
        mp12_irq_host_common_timer_isr(hal_dev);
        mp12_clear_host_mac_int_status(hal_dev, intr_state.u32);
    } else {
        oam_warning_log1(0, OAM_SF_IRQ, "{mp12_irq_host_mac_isr_process error, state.u32:0x%x.}", intr_state.u32);
        mp12_clear_host_mac_int_status(hal_dev, intr_state.u32);
    }
}

static void mp12_irq_host_mac_isr_process_all(hal_host_device_stru *hal_dev, uint32_t *state)
{
    ...
    while (state_tmp) {
        ...
        mp12_irq_host_mac_isr_process(hal_dev, intr_state);
    }
    *state = intr_state.u32;
}

static void mp12_irq_host_mac_common_isr_process_all(hal_host_device_stru *hal_dev, uint32_t *state)
{
    host_comm_intr_status_union intr_state;
    uint32_t state_tmp = *state;
    ...
    while (state_tmp) {
        ...
        intr_state.u32 = (uint32_t)((uint32_t)(-state_tmp) & state_tmp);
        state_tmp &= ~((uint32_t)intr_state.u32);
        mp12_irq_host_mac_comm_isr_process(hal_dev, intr_state);
    }
    *state = intr_state.u32;
}
...
void mp12_irq_host_mac_isr(hal_host_device_stru *hal_dev)
{
    ...
    mp12_irq_host_mac_isr_process_all(hal_dev, &state);
...
void mp12_mac_comm_irq_host_all(void *dev)
{
    ...
    state &= ~mask;
    mp12_irq_host_mac_common_isr_process_all(hal_dev, &state);
    mp12_clear_host_mac_common_int_status(hal_dev, mask);
}
