...
#undef THIS_FILE_ID
#define THIS_FILE_ID OAM_FILE_ID_HOST_SNIFFER_MP12_C

/*
 * 函数描述：rx ppdu描述符 free ring 初始化
 */
uint32_t mp12_rx_ppdu_dscr_free_ring_init(hal_host_device_stru *hal_dev)
{
    uint16_t entries;
...
static uint32_t mp12_dma_map_rx_ppdu_dscr(
    uint8_t dma_dir, hal_host_rx_ppdu_dscr_stru *ppdu_dscr, dma_addr_t *dma_addr)
{
    ...
    *dma_addr = dma_map_single(&pcie_dev->dev, (uint8_t *)&ppdu_dscr->hw_ppdu_data,
        sizeof(mp12_rx_ppdu_dscr_stru), dma_dir);
...
uint32_t mp12_add_rx_ppdu_dscr(hal_host_device_stru *hal_device)
{
    ...
    for (loop = 0; loop < need_add; loop++) {
        ...
        if (mp12_dma_map_rx_ppdu_dscr(DMA_FROM_DEVICE, ppdu_dscr, &pci_dma_addr) != OAL_SUCC) {
            oal_free(ppdu_dscr);
...
{
    /* ring 初始化 */
    if (mp12_rx_ppdu_dscr_free_ring_init(hal_device) != OAL_SUCC) {
    ...
    /* 向 free ring添加内存 */
    if (mp12_add_rx_ppdu_dscr(hal_device) != OAL_SUCC) {
 ...
 * 功能描述: 释放rx ppdu ring
 */
void mp12_rx_ppdu_from_list_free(hal_host_rx_ppdu_dscr_list_head_stru *ppdu_list)
{
    hal_host_rx_ppdu_dscr_stru *ppdu_dscr = NULL;
...
void mp12_sniffer_rx_ppdu_free_ring_deinit(hal_host_device_stru *hal_device)
{
    ...
    /* 释放alloc list */
    mp12_rx_ppdu_from_list_free(&hal_device->host_rx_ppdu_alloc_list);
    /* 释放ring本身 */
 ...
 */
void mp12_sniffer_set_rx_info(hal_sniffer_extend_info *sniffer_rx_info, mac_rx_ctl_stru *cb,
    mp12_rx_ppdu_dscr_stru *ppdu_dscr)
{
 ...
 * 功能描述：rx速率信息填充
 */
void mp12_sniffer_set_rx_rate_info(hal_sniffer_extend_info *sniffer_rx_info,
    mp12_rx_ppdu_dscr_stru *ppdu_dscr)
{
...
static uintptr_t mp12_get_rx_ppdu_dscr_addr(oal_netbuf_stru *netbuf)
{
    ...
    oal_netbuf_push(netbuf, MP12_RX_MSDU_DSCR_LEN);
...
hal_host_rx_ppdu_dscr_stru *mp12_sniffer_get_rx_ppdu_dscr(hal_host_device_stru *hal_dev, oal_netbuf_stru *netbuf)
{
    ...
    rx_ppdu_dscr_addr = mp12_get_rx_ppdu_dscr_addr(netbuf);
...
void mp12_sniffer_rx_info_fill_host_rx(hal_host_device_stru *hal_device, oal_netbuf_stru *netbuf,
    hal_sniffer_extend_info *sniffer_rx_info, mac_rx_ctl_stru *rx_ctl)
{
    ...
    rx_ppdu = mp12_sniffer_get_rx_ppdu_dscr(hal_device, netbuf);
    if (rx_ppdu == NULL) {
        return;
    }

    dma_unmap_single(&pcie_dev->dev, rx_ppdu->paddr.addr,
        sizeof(mp12_rx_ppdu_dscr_stru), DMA_FROM_DEVICE);

    mp12_sniffer_set_rx_info(sniffer_rx_info, rx_ctl, &rx_ppdu->hw_ppdu_data);
    mp12_sniffer_set_rx_rate_info(sniffer_rx_info, &rx_ppdu->hw_ppdu_data);

    oal_dlist_delete_entry(&rx_ppdu->entry);
    oal_free(rx_ppdu);
}

void mp12_sniffer_rx_info_fill_device_rx(hal_host_device_stru *hal_device, oal_netbuf_stru *netbuf,
    hal_sniffer_extend_info *sniffer_rx_info, mac_rx_ctl_stru *rx_ctl)
{
    ...
    hal_sniffer_extend_info *sniffer_rx_info, mac_rx_ctl_stru *rx_ctl)
{
    ...
    if (oal_atomic_read(&hal_device->rx_mode) == HAL_DDR_RX) {
        mp12_sniffer_rx_info_fill_host_rx(hal_device, netbuf, sniffer_rx_info, rx_ctl);
    } else if (oal_atomic_read(&hal_device->rx_mode) == HAL_RAM_RX) {
        mp12_sniffer_rx_info_fill_device_rx(hal_device, netbuf, sniffer_rx_info, rx_ctl);
    } else {
