    ...
    mac_rx_ctl_stru *rx_ctl, mp16c_rx_mpdu_desc_stru *rx_hw_dscr, uint8_t first_msdu)
{
    ...
    if (first_msdu) {
        ...
        if (rx_ctl->bit_vap_id < HAL_MAX_VAP_NUM_MP16C) {
...
hal_host_device_stru *mp16c_host_rx_get_mpdu_link(hal_host_device_stru *origin_dev, oal_netbuf_stru *mpdu)
{
    return origin_dev;
}
...
oal_bool_enum_uint8 mp16c_host_rx_ip_checksum_is_pass(oal_netbuf_stru *netbuf)
{
...
uint32_t mp16c_host_rx_buff_recycle(hal_host_device_stru *hal_device, oal_netbuf_head_stru *netbuf_head)
{
    ...
    while ((netbuf = oal_netbuf_delist_nolock(netbuf_head)) != NULL) {
        ...
        if (netbuf_total_len < LARGE_NETBUF_SIZE + MP16C_RX_MSDU_DSCR_LEN) {
        ...
        oal_netbuf_set_len(netbuf, LARGE_NETBUF_SIZE + MP16C_RX_MSDU_DSCR_LEN);
