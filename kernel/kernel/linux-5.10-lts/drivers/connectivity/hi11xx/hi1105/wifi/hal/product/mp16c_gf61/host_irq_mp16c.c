...
static void mp16c_slave_host_mac_isr_abnormal_dump(hal_host_device_stru *hal_dev, uint32_t state, uint32_t mask)
{
    ...
    oam_error_log3(0, OAM_SF_RX, "{irq_host_mac_isr:: device[%d] error state[0x%x] mask[0x%x]}",
        hal_dev->device_id, state, mask);
    oam_error_log3(0, OAM_SF_RX,
        "{irq_host_mac_isr:: pcie_raw[0x%x] pcie_mask[0x%x] pcie_status[0x%x]}",
        soc_raw_status, soc_mask, soc_status);
}
static uint8_t mp16c_check_slave_host_mac_isr_abnormal(hal_host_device_stru *hal_dev)
{
    ...
    if (hal_dev->device_id == HAL_DEVICE_ID_SLAVE && mac_device->en_dbdc_running == 0 &&
        ++hal_dev->slave_mac_int_err_cnt >= MP16_PCIE_MAC_N2_ERROR_THRESHOLD) {
        hal_dev->slave_mac_int_err_cnt = 0;
        return OAL_TRUE;
    }
    return OAL_FALSE;
}
void mp16c_irq_host_mac_isr(hal_host_device_stru *hal_dev)
{
    ...
    /* 若辅路下电后仍频繁上报中断可能会导致狗超时异常，需要检测 */
    if (mp16c_check_slave_host_mac_isr_abnormal(hal_dev) == OAL_TRUE) {
        mp16c_slave_host_mac_isr_abnormal_dump(hal_dev, state, mask);
