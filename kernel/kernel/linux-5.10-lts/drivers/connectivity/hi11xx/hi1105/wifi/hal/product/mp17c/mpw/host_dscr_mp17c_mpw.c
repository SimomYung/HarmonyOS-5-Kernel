...
#undef THIS_FILE_ID
#define THIS_FILE_ID OAM_FILE_ID_HOST_HAL_DSCR_MP17C_MPW_C
...
uint32_t g_mac_reg_offset_mp17c_mpw[WLAN_DEVICE_MAX_NUM_PER_CHIP_MP17C] = {
    MP17C_MAC_BANK_REG_OFFSET, MP17C_MAC_BANK_REG_OFFSET
};
uint32_t mp17c_mpw_get_mac_reg_offset(uint8_t device_id, uint32_t reg_addr)
{
    return g_mac_reg_offset_mp17c_mpw[device_id];
}
...
oal_netbuf_stru *mp17c_mpw_rx_get_next_sub_msdu(hal_host_device_stru *hal_device,
...
void mp17c_mpw_host_rx_amsdu_list_build(hal_host_device_stru *hal_device, oal_netbuf_stru *netbuf)
{
    ...
    while (netbuf != NULL) {
        next_netbuf = mp17c_mpw_rx_get_next_sub_msdu(hal_device, netbuf);
...
uint8_t mp17c_mpw_rx_is_frag_frm(mac_header_frame_control_stru *fc_ctl_hdr,
    mp17c_mpw_rx_msdu_dscr_stru *rx_msdu_hw_dscr)
{
    return (uint8_t)((rx_msdu_hw_dscr->frame_format == MAC_FRAME_TYPE_80211) &&
        (fc_ctl_hdr->bit_more_frag || rx_msdu_hw_dscr->fragment_num));
}

OAL_STATIC void mp17c_mpw_host_rx_cb_set_firts_msdu(mac_rx_ctl_stru *rx_ctl, mp17c_mpw_rx_msdu_dscr_stru *rx_hw_dscr)
{
...
static void mp17c_mpw_host_rx_cb_set(hal_host_device_stru *hal_dev,
    mac_rx_ctl_stru *rx_ctl, mp17c_mpw_rx_msdu_dscr_stru *rx_hw_dscr, uint8_t first_msdu)
{
    ...
    if (first_msdu) {
        ...
        if (mp17c_mpw_rx_is_frag_frm(fc_ctl_hdr, rx_hw_dscr)) {
 ...
 *   修改内容  : 新生成函数
 */
static uint32_t mp17c_mpw_host_rx_adjust_netbuf_len(hal_host_device_stru *hal_dev,
    oal_netbuf_stru *netbuf, mac_rx_ctl_stru *rx_ctl, mp17c_mpw_rx_msdu_dscr_stru *rx_hw_dscr, uint8_t first_msdu)
{
    ...
    if ((rx_hw_dscr->rx_status == HAL_RX_SUCCESS) &&
        (rx_hw_dscr->packet_len == 0 || (rx_hw_dscr->packet_len > (cb_res->len - MP17C_MPW_RX_MSDU_DSCR_LEN)))) {
        ...
        oam_report_dscr(BROADCAST_MACADDR, (uint8_t *)rx_hw_dscr,
            MP17C_MPW_RX_MSDU_DSCR_LEN, OAM_OTA_TYPE_RX_MSDU_DSCR);
        return OAL_FAIL;
    }
...
{
    mp17c_mpw_rx_msdu_dscr_stru      *rx_msdu_hw_dscr = NULL;
    uint32_t                       ret;

    memset_s(rx_ctl, sizeof(mac_rx_ctl_stru), 0, sizeof(mac_rx_ctl_stru));
    rx_msdu_hw_dscr = (mp17c_mpw_rx_msdu_dscr_stru *)oal_netbuf_data(netbuf);
    mp17c_mpw_host_rx_cb_set(hal_dev, rx_ctl, rx_msdu_hw_dscr, first_msdu);
    ret = mp17c_mpw_host_rx_adjust_netbuf_len(hal_dev, netbuf, rx_ctl, rx_msdu_hw_dscr, first_msdu);
    if (ret != OAL_SUCC) {
        return ret;
    }
...
#define MP17C_MPW_TX_MSDU_DSCR_LEN 20
void mp17c_mpw_tx_msdu_dscr_info_get(oal_netbuf_stru *netbuf, hal_tx_msdu_dscr_info_stru *tx_msdu_info)
{
    mp17c_mpw_tx_msdu_dscr_stru *tx_msdu_dscr = (mp17c_mpw_tx_msdu_dscr_stru *)((uintptr_t)oal_netbuf_data(netbuf) -
                                            MP17C_MPW_TX_MSDU_DSCR_LEN);
...
uint8_t mp17c_mpw_host_get_tx_tid_ring_size(uint8_t ring_size_grade, uint8_t tid_no)
{
    ...
    /*
    |   tid_no  |   ring_depth   |                mp17c_mpw_ring_size             |         备注          |
    ...
    注:1.mp16最小ring_depth为128,mp16_ring_depth = 128 << mp16_ring_size;
       2.mp17c_mpw最小ring_depth为1,mp17c_mpw_ring_depth = 1 << mp17c_mpw_ring_size;
    */
...
{
    mp17c_mpw_tx_msdu_dscr_stru *tx_msdu_dscr = (mp17c_mpw_tx_msdu_dscr_stru *)buffer;
...
uint32_t mp17c_mpw_host_rx_buff_recycle(hal_host_device_stru *hal_device, oal_netbuf_head_stru *netbuf_head)
{
    ...
    while ((netbuf = oal_netbuf_delist_nolock(netbuf_head)) != NULL) {
        ...
        if (netbuf_total_len < LARGE_NETBUF_SIZE + MP17C_MPW_RX_MSDU_DSCR_LEN) {
        ...
        oal_netbuf_set_len(netbuf, LARGE_NETBUF_SIZE + MP17C_MPW_RX_MSDU_DSCR_LEN);
...
{
    return MP17C_MPW_TX_MSDU_DSCR_LEN;
}

uint8_t mp17c_mpw_host_get_rx_msdu_dscr_len(void)
{
    return MP17C_MPW_RX_MSDU_DSCR_LEN;
}

void mp17c_mpw_host_h2d_rx_ring_reset(hal_host_device_stru *hal_device)
{
    hal_host_writel(MP17C_MPW_D2H_RX_RING_RESET, hal_device->st_host_rx_normal_free_ring.rx_ring_reset);
}
