...
static inline uint32_t mp17c_host_tx_get_tid_ring_table_hostva(uint8_t lut_idx, uint8_t tid, uint64_t *hostva)
{
    return oal_pcie_devca_to_hostva(HCC_EP_WIFI_DEV, mp17c_host_tx_get_tid_ring_table_addr(lut_idx, tid), hostva);
}

uint32_t mp17c_host_tx_init_tid_ring_table(uint8_t lut_idx, uint8_t tid, uint64_t *tx_ring_table_addr)
{
    mp17c_tx_msdu_ring_table_stru *tx_ring_mgmt = mp17c_host_tx_get_tid_ring_table_mgmt(lut_idx, tid);

    if (mp17c_host_tx_get_tid_ring_table_hostva(lut_idx, tid, tx_ring_table_addr) != OAL_SUCC) {
        return OAL_FAIL;
    }
    ...
    mp17c_tx_msdu_ring_table_stru *tx_ring_mgmt, uint64_t tx_ring_table_addr, uint32_t val)
{
    ...
    mp17c_host_tx_update_ring_table(tx_ring_mgmt, tx_ring_table_addr, MP17C_RING_TABLE_WPTR_OFFSET);
}
