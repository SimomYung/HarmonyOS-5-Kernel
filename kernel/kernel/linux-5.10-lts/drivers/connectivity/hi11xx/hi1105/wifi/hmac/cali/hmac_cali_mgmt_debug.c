...
void hmac_dump_cali_result(void)
{
    ...
    *puc_bfgx_rc_code = cali_data->st_rc_r_c_cali_data.uc_rc_cmp_code;
    ...
    for (uc_chan_idx = 0; uc_chan_idx < WLAN_RF_CHANNEL_NUMS; uc_chan_idx++) {
        ...
        /* 5g */
        if (mac_device_check_5g_enable_per_chip() == OAL_FALSE) {
            continue;
        }
...
OAL_STATIC void hmac_print_mp16_base_cali_result(mp1x_wlan_cali_data_para_stru *cali_param)
{
    mp1x_wlan_cali_ctl_para_stru *cali_data = &cali_param->common_cali_data.cali_ctl_data;
...
OAL_STATIC void hmac_print_mp12_base_cali_result(mp12_wlan_cali_data_para_stru *cali_param)
{
    mp12_wlan_cali_ctl_para_stru *cali_data = &cali_param->common_cali_data.cali_ctl_data;
    oam_warning_log2(0, OAM_SF_CALIBRATE, "check_hw_status:%d %d", cali_data->check_hw_status[0],
        cali_data->check_hw_status[1]); // 0/1: rf 0-1
    oam_warning_log4(0, OAM_SF_CALIBRATE, "update info tmp:%d, chan_idx_1_2[%d %d],cali_time:%d",
        cali_data->cali_update_info.bit_temperature,
        cali_data->cali_update_info.uc_5g_chan_idx1,
        cali_data->cali_update_info.uc_5g_chan_idx2,
...
int32_t hmac_print_mp16_cali_result(uint8_t rf_idx, uint8_t chn_idx,
    int8_t *pc_print_buff, int32_t remain_len, mp1x_wlan_cali_basic_para_stru *basic_cali_data)
{
    int8_t *pc_string;
...
int32_t hmac_print_mp12_cali_result(uint8_t rf_idx, uint8_t chn_idx,
    int8_t *pc_print_buff, int32_t remain_len, mp12_wlan_cali_basic_para_stru *basic_cali_data)
{
    int8_t *pc_string;
    wlan_cali_rxdc_para_stru *rxdc_cali_data = &basic_cali_data->rxdc_cali_data;
    wlan_cali_txdc_para_stru *txdc_cali_data = &basic_cali_data->txdc_cali_data;
...
OAL_STATIC int32_t hmac_print_mp16_2g_common_cali_result(uint8_t rf_idx, uint8_t chn_idx,
    int8_t *buff, int32_t remain_len, mp1x_wlan_cali_data_para_stru *cali_data)
{
    int8_t *pc_string = NULL;
...
OAL_STATIC int32_t hmac_print_mp12_2g_common_cali_result(uint8_t rf_idx, uint8_t chn_idx,
    int8_t *buff, int32_t remain_len, mp12_wlan_cali_data_para_stru *cali_data)
{
    int8_t *pc_string = NULL;
    wlan_cali_logen_para_stru *logen_cali_data = &cali_data->common_cali_data.logen_cali_data_2g[rf_idx][chn_idx];
    wlan_cali_lodiv_para_stru *lodiv_cali_data = &cali_data->common_cali_data.lodiv_cali_data[rf_idx];
...
OAL_STATIC int32_t hmac_print_mp12_5g_common_cali_result(uint8_t rf_idx, uint8_t chn_idx,
    int8_t *buff, int32_t remain_len, mp12_wlan_cali_data_para_stru *cali_data)
{
    int8_t *pc_string = NULL;
    wlan_cali_logen_para_stru *logen_cali_data = &cali_data->common_cali_data.logen_cali_data_5g[rf_idx][chn_idx];
...
uint32_t hmac_dump_mp16_2g_cali(int8_t *buff, mp1x_wlan_cali_data_para_stru *cali_data, uint8_t rf_idx)
{
    uint8_t   chn_idx;
    int32_t   str_tmp_len;
    int32_t   remain_len;
    uint32_t  str_len     = 0;
    int8_t *pc_string = "2G:";
    mp1x_wlan_cali_basic_para_stru *basic_cali_data = NULL;
...
uint32_t hmac_dump_mp12_2g_cali(int8_t *buff, mp12_wlan_cali_data_para_stru *cali_data, uint8_t rf_idx)
{
    uint8_t   chn_idx;
    int32_t   str_tmp_len;
    int32_t   remain_len;
    uint32_t  str_len     = 0;
    int8_t *pc_string = "2G:";
    mp12_wlan_cali_basic_para_stru *basic_cali_data = NULL;
    for (chn_idx = 0; chn_idx < OAL_2G_CHANNEL_NUM; chn_idx++) {
        remain_len = (int32_t)(OAM_REPORT_MAX_STRING_LEN - str_len - 1);
        if (remain_len <= 0) {
            hmac_print_error(buff);
        ...
        basic_cali_data = &cali_data->rf_cali_data[rf_idx].cali_data_2g_20m.cali_data[chn_idx];
        str_tmp_len = hmac_print_mp12_cali_result(rf_idx, chn_idx, buff + str_len, remain_len, basic_cali_data);
        if (str_tmp_len < 0) {
            return OAL_FAIL;
        }
        str_len += (uint32_t)str_tmp_len;

        str_tmp_len = hmac_print_mp12_2g_common_cali_result(rf_idx, chn_idx,
            buff + str_len, remain_len, cali_data);
        if (str_tmp_len < 0) {
            return OAL_FAIL;
        }
...
uint32_t hmac_dump_mp16_5g_cali(int8_t *buff, mp1x_wlan_cali_data_para_stru *cali_data, uint8_t rf_idx)
{
...
uint32_t hmac_dump_mp12_5g_cali(int8_t *buff, mp12_wlan_cali_data_para_stru *cali_data, uint8_t rf_idx)
{
    uint8_t chn_idx, band_idx;
    int32_t str_tmp_len, remain_len;
    uint32_t str_len = 0;
    mp12_wlan_cali_basic_para_stru *basic_cali_data = NULL;
    uint8_t print_cnt = 0;
    for (chn_idx = 0; chn_idx < (OAL_5G_20M_CHANNEL_NUM + OAL_5G_80M_CHANNEL_NUM); chn_idx++) {
        remain_len = (int32_t)(OAM_REPORT_MAX_STRING_LEN - str_len - 1);
        if (remain_len <= 0) {
            hmac_print_error(buff);
            return OAL_FAIL;
        ...
        print_cnt++;
        if (chn_idx < (OAL_5G_20M_CHANNEL_NUM + OAL_5G_80M_CHANNEL_NUM) && chn_idx >= OAL_5G_20M_CHANNEL_NUM) {
            band_idx = chn_idx - OAL_5G_20M_CHANNEL_NUM;
            basic_cali_data = &cali_data->rf_cali_data[rf_idx].cali_data_5g_80m.cali_data[band_idx];
        } else {
            band_idx = chn_idx;
        ...
        str_tmp_len = hmac_print_mp12_cali_result(rf_idx, band_idx, buff + str_len, remain_len, basic_cali_data);
        if (str_tmp_len < 0) {
            return OAL_FAIL;
        }
        str_len += (uint32_t)str_tmp_len;
        ...
        if (chn_idx < OAL_5G_20M_CHANNEL_NUM) {
            str_tmp_len = hmac_print_mp12_5g_common_cali_result(rf_idx, chn_idx,
                buff + str_len, remain_len, cali_data);
            if (str_tmp_len < 0) {
                return OAL_FAIL;
            }
...
void hmac_dump_cali_result_mp12(void)
{
    ...
    hmac_print_mp12_base_cali_result(cali_data);

    for (rf_idx = 0; rf_idx < rf_num; rf_idx++) {
        /* 2.4g 不超过ota上报最大字节分3次输出 */
        ret = hmac_dump_mp12_2g_cali(buff, cali_data, rf_idx);
        if (ret != OAL_SUCC) {
            oal_mem_free_m(buff, OAL_TRUE);
            return;
        }
        ...
        /* 5g 不超过ota上报最大字节分3次输出 */
        ret = hmac_dump_mp12_5g_cali(buff, cali_data, rf_idx);
        if (ret != OAL_SUCC) {
            oal_mem_free_m(buff, OAL_TRUE);
            return;
        }
