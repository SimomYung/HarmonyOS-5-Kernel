    ...
    hmac_chba_chan_switch_notify_stru *chan_switch_ntf)
{
    ...
    if (oal_any_null_ptr3(peer_addr, chan_switch_ntf, hmac_vap)) {
        return OAL_FAIL;
    }
...
static void hmac_chba_clear_csa_lost_device_flag(hmac_chba_chan_switch_info_stru *chan_switch_info)
{
    ...
    memset_s(&(chan_switch_info->csa_lost_device), sizeof(chan_switch_info->csa_lost_device), 0,
        sizeof(chan_switch_info->csa_lost_device));
}
...
/* 如果配置信道带宽超出定制化能力，降低带宽 */
void hmac_chba_reconfig_chan_cfg(mac_vap_stru *mac_vap, mac_channel_stru *target_channel)
{
    mac_cfg_channel_param_stru chan_param = {0};
    chan_param.uc_channel = target_channel->uc_chan_number;
    chan_param.en_band = target_channel->en_band;
    chan_param.en_bandwidth = target_channel->en_bandwidth;
    /* 检查定制化 */
    if (hmac_config_set_channel_check_param(mac_vap, &chan_param) != OAL_SUCC) {
        hmac_config_chan_bw_exceed(mac_vap, &chan_param);
        oam_warning_log4(0, OAM_SF_CHBA, "hmac_chba_reconfig_chan_cfg::chan[%d] bw[%d] -> chan[%d] bw[%d]",
            target_channel->uc_chan_number, target_channel->en_bandwidth,
            chan_param.uc_channel, chan_param.en_bandwidth);
        target_channel->en_bandwidth = chan_param.en_bandwidth;
    }
}
/*
    ...
    hmac_chba_mgmt_info_stru *chba_mgmt_info, hmac_chba_chan_switch_notify_stru *chan_switch_ntf, uint8_t *peer_addr)
{
    ...
    /* 已经在新信道上了，不回confirm */
    if (chba_vap_info->work_channel.uc_chan_number ==  target_channel.uc_chan_number &&
        chba_vap_info->work_channel.en_bandwidth == target_channel.en_bandwidth) {
        oam_warning_log2(0, OAM_SF_CHBA, "hmac_chba_rx_chan_switch_indication::already on new \
            channel[%d] bw=[%d], not confirm!", target_channel.uc_chan_number, target_channel.en_bandwidth);
        return;
    }
    /* 连续请求超时次数清0 */
    chan_switch_info->req_timeout_cnt = 0;
    ...
    /* 抛事件给DMAC，配置自己的信道切换 */
    hmac_chba_chan_work_channel_switch(hmac_vap, target_channel.uc_chan_number, target_channel.en_bandwidth,
    ...
    uint8_t chan_number, wlan_channel_bandwidth_enum_uint8 en_bandwidth)
{
    ...
    hmac_chba_reconfig_chan_cfg(&(hmac_vap->st_vap_base_info), target_channel);
...
{
    mac_channel_stru target_channel = { 0 };
    uint8_t pedding_data = 0;
    ...
    /* chba未建链时, 只有极速连接模式与检测到雷达时支持切信道 */
    if (hmac_vap->st_vap_base_info.us_user_nums == 0) {
        if ((chan_info->switch_type == MAC_CHBA_CHAN_SWITCH_FAST_CONNECT) ||
            (chan_info->switch_type == MAC_CHBA_CHAN_SWITCH_FOR_RADAR)) {
    ...
    switch (chan_info->switch_type) {
        ...
        case MAC_CHBA_CHAN_SWITCH_FOR_RADAR:
        ...
        case MAC_CHBA_CHAN_SWITCH_FOR_AVOID_DBAC:
            /* 因CHBA切信道导致DBAC上层下发切信道开始信道切换流程，只需要切换自身信道 */
            hmac_chba_chan_work_channel_switch(hmac_vap, target_channel.uc_chan_number,
                target_channel.en_bandwidth, MAC_CHBA_CHAN_SWITCH_IMMEDIATE);
            break;
    ...
    mac_channel_stru *social_channel)
{
    ...
    if (bandwidth >= WLAN_BAND_WIDTH_160PLUSPLUSPLUS) {
        bw_cap = WLAN_BW_CAP_160M;
    ...
    social_channel->en_bandwidth = mac_regdomain_get_bw_by_channel_bw_cap(channel_number, bw_cap);
...
/* 功能描述: chba工作信道检测到雷达波, 触发信道切换 */
void hmac_chba_adjust_channel_for_radar(mac_vap_stru *mac_vap, uint8_t channel,
    wlan_channel_bandwidth_enum_uint8 bandwidth)
{
    hmac_vap_stru *hmac_vap = NULL;
    mac_chba_adjust_chan_info chan_info = { 0 };

    if (mac_vap == NULL) {
        oam_error_log0(0, OAM_SF_DFS, "{hmac_chba_adjust_channel_for_radar:vap is NULL!}");
        return;
    }
    /* chba vap还未up, 不需要更新信道 */
    hmac_vap = (hmac_vap_stru *)mac_res_get_hmac_vap(mac_vap->uc_vap_id);
    if (hmac_chba_vap_start_check(hmac_vap) == OAL_FALSE) {
        return;
    }

    chan_info.chan_number = channel;
    chan_info.bandwidth = bandwidth;
    chan_info.switch_type = MAC_CHBA_CHAN_SWITCH_FOR_RADAR;
    oam_warning_log2(mac_vap->uc_vap_id, OAM_SF_DFS, "{hmac_chba_adjust_channel_for_radar:switch to [%d][%d]}",
        channel, bandwidth);
    hmac_chba_adjust_channel_proc(hmac_vap, &chan_info);
}

/* 功能描述: 检测到雷达波, 通知上层 */
void hmac_chba_dfs_detect_radar_report(mac_vap_stru *mac_vap)
{
#if (_PRE_OS_VERSION_LINUX == _PRE_OS_VERSION)
    hmac_vap_stru *hmac_vap = NULL;
    mac_detect_radar_report_stru report_info = { 0 };
    ...
    report_info.report_type = HMAC_CHBA_DETECT_RADAR_REPORT;
    oal_cfg80211_chba_report((uint8_t *)(&report_info), sizeof(report_info));
    oam_warning_log0(mac_vap->uc_vap_id, OAM_SF_DFS, "{hmac_chba_dfs_detect_radar_report}");
#endif
}
#endif
