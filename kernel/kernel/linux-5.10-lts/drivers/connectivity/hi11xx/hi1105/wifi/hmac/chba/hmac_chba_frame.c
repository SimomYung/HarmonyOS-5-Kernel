...
#define CHBA_NEGOACTION_PRIORITY_FLAG_MASK 0x3 /* bit0-1 为chba nego action优先级标记 */
...
oal_bool_enum_uint8 hmac_vap_rx_mgmt_is_chba_negotiation_action(oal_netbuf_stru *netbuf, mac_rx_ctl_stru *rx_ctrl)
{
    ...
    if ((netbuf == NULL) || (rx_ctrl == NULL)) {
        return OAL_FALSE;
    }
    mac_hdr = (uint8_t *)(uintptr_t)mac_get_rx_cb_mac_header_addr(rx_ctrl);
    mac_hdr_len = rx_ctrl->uc_mac_header_len; /* MAC头长度 */
    payload = (uint8_t *)(mac_hdr) + mac_hdr_len;
    payload_len = hmac_get_frame_body_len(netbuf);
    frame_type = mac_get_frame_type_and_subtype(mac_hdr);
    /* 不是action帧, 不处理 */
    if (frame_type != (WLAN_FC0_SUBTYPE_ACTION | WLAN_FC0_TYPE_MGT)) {
        return OAL_FALSE;
    }

    return hmac_is_chba_negotiation_action(payload, payload_len);
}

/* 功能描述: 获取chba negotiation帧优先级标记 */
uint8_t hmac_chba_frame_get_negotiation_priority_flag(uint8_t *chba_nego_action_buf,
    uint16_t chba_nego_action_len)
{
    uint8_t priority_flag;
    if (chba_nego_action_len < MAC_CHBA_NEGOTIATION_HIGH_PRIO_ACTION_MIN_LEN) { /* 帧长短于最短门限, 不视为高优先级帧 */
        return CHBA_NEGOACTION_LOW_PRIORITY;
    }

    priority_flag = (chba_nego_action_buf[BYTE_OFFSET_11] & CHBA_NEGOACTION_PRIORITY_FLAG_MASK);
 ...
 * 作    者  : wifi
 */
uint32_t hmac_send_chba_sync_request_action(mac_vap_stru *mac_vap, hmac_chba_vap_stru *chba_vap,
 ...
 * 日    期  : 2021年02月24日
 */
uint8_t *hmac_find_chba_attr(uint8_t attr_id, uint8_t *payload, int32_t length)
{
    int32_t len = length;
    if (payload == NULL) {
        return NULL;
...
uint32_t hmac_chba_set_master_election_attr(uint8_t *attr_buf)
{
    ...
    /* 填写period */
    schedule_period = device_info->config_para.schedule_period * device_info->config_para.slot_duration;
...
/* 组关联帧中的CHBA IE */
uint32_t hmac_chba_encap_assoc_attribute(uint8_t *buffer, uint32_t buffer_len)
{
    ...
    if (chba_assoc_ie_len < MAC_IE_HDR_LEN) {
        oam_error_log1(0, OAM_SF_CHBA, "hmac_chba_encap_assoc_attribute::chba ie len [%d] is too short",
            chba_assoc_ie_len);
        return 0;
    }
...
/* 功能描述: 在assoc_rsp帧中增加chba私有app ie */
uint32_t hmac_chba_encap_assoc_rsp_assoc_priv_ie(uint8_t *buffer, const unsigned char *sta_addr)
{
    int32_t ret;
    uint32_t assoc_priv_ie_len = 0;
    uint8_t *assoc_priv_ie = NULL;

    assoc_priv_ie = hmac_chba_find_assoc_priv_ie(sta_addr, &assoc_priv_ie_len);
    if (assoc_priv_ie == NULL) {
        return 0;
    }

    if (assoc_priv_ie_len == 0) {
        oam_error_log0(0, OAM_SF_ANY, "{hmac_chba_encap_assoc_assoc_priv_ie:assoc_priv_ie_len is 0!}");
        return 0;
    }
    ret = memcpy_s(buffer, assoc_priv_ie_len, assoc_priv_ie, assoc_priv_ie_len);
    if (ret != EOK) {
        oam_error_log0(0, OAM_SF_ANY, "hmac_chba_encap_assoc_assoc_priv_ie:memcpy fail!");
        return 0;
    }
    return assoc_priv_ie_len;
}
    ...
    uint16_t payload_len)
{
    ...
    // 跳过固定字段和私有IE头
    ies_info.frame_ies = payload + MAC_CHBA_BEACON_ELEMENT_OFFSET;
    ies_info.frame_len = payload_len - MAC_CHBA_BEACON_ELEMENT_OFFSET;
    ...
    if (((uint16_t)(chba_ie[1]) + MAC_IE_HDR_LEN) < sizeof(mac_ieee80211_vendor_ie_stru)) {
        oam_error_log1(0, OAM_SF_CHBA, "hmac_chba_rx_beacon_proc: find chba_ie len[%d]", chba_ie[1]);
        return;
    }
    ...
    uint8_t *payload, uint16_t payload_len)
{
    ...
    hmac_chba_get_master_info_from_frame(payload + 1, payload_len - 1, sa_addr, &another_master);
    ...
    mac_ieee80211_frame_stru *mac_hdr, uint8_t *payload, uint16_t payload_len)
{
    ...
    switch (payload[MAC_CHBA_ACTION_TYPE_LEN]) {
        ...
        case MAC_CHBA_DOMAIN_MERGE:
            ...
            break;
#ifdef _PRE_WLAN_FEATURE_NBW
        case MAC_NBW_TIMESLOT_NEG_ACTION:
            hmac_nbw_rx_timeslot_neg_handle(hmac_vap, sa_addr, payload + MAC_CHBA_ACTION_HDR_LEN,
                payload_len - MAC_CHBA_ACTION_HDR_LEN);
            break;
        case MAC_NBW_TIMESLOT_ADJUST_ACTION:
            hmac_nbw_rx_timeslot_adjust_handle(hmac_vap, sa_addr, payload + MAC_CHBA_ACTION_HDR_LEN,
                payload_len - MAC_CHBA_ACTION_HDR_LEN);
            break;
#endif
        default:
    ...
    uint8_t *payload, uint16_t payload_len)
{
    ...
    uint16_t bcn_int;
        ...
        if (mac_vap->nbw_mode == NBW_MODE) {
            return hmac_nbw_rx_beacon(hmac_vap, mac_hdr, payload, payload_len);
        }
#endif
    if (payload_len <= MAC_CHBA_BEACON_ELEMENT_OFFSET) {
        oam_error_log1(0, OAM_SF_CHBA, "{hmac_chba_rx_mgmt::len is too short[%d].}", payload_len);
        return OAL_FAIL;
    }
 ...
 * 作    者  : wifi
 */
uint32_t hmac_chba_rx_mgmt(hmac_vap_stru *hmac_vap, dmac_wlan_crx_event_stru *rx_mgmt_event,
    oal_netbuf_stru *netbuf)
{
    ...
    if (oal_any_null_ptr3(hmac_vap, rx_mgmt_event, netbuf)) {
        oam_error_log0(0, OAM_SF_CHBA, "{hmac_chba_rx_mgmt::null pointer.}");
        return OAL_ERR_CODE_PTR_NULL;
    }
    ...
    payload = (uint8_t *)mac_hdr + mac_hdr_len;
    payload_len = hmac_get_frame_body_len(netbuf);
    frame_type = mac_get_frame_type_and_subtype((uint8_t *)mac_hdr);
    /* 判断是否是Beacon帧 */
    if (frame_type == (WLAN_FC0_SUBTYPE_BEACON | WLAN_FC0_TYPE_MGT)) {
    ...
    if ((frame_type == (WLAN_FC0_SUBTYPE_ACTION | WLAN_FC0_TYPE_MGT)) &&
        hmac_is_chba_negotiation_action(payload, payload_len) == OAL_TRUE) {
        oam_warning_log0(hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_RX,
            "{hmac_chba_rx_mgmt:send chba negotiation to host}");
        hmac_rx_mgmt_send_to_host(hmac_vap, netbuf);
        return OAL_SUCC;
    }

    if ((frame_type == (WLAN_FC0_SUBTYPE_ACTION | WLAN_FC0_TYPE_MGT)) &&
        hmac_chba_is_chba_action(payload, payload_len)) {
        ...
        if (hmac_fsm_call_func_ap(hmac_vap, HMAC_FSM_INPUT_RX_MGMT, (void *)rx_mgmt_event) != OAL_SUCC) {
            oam_warning_log0(hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_CHBA,
                "{hmac_chba_rx_mgmt::hmac_fsm_call_func_ap fail.}");
        }
    }
...
uint32_t mac_set_vendor_chba_ie(mac_vap_stru *mac_vap, hmac_user_stru *hmac_user,
    uint8_t *buffer, const unsigned char *sta_addr)
{
    uint32_t ie_len;
    uint32_t len_ret;
    hmac_vap_stru *hmac_vap = NULL;
    ...
    /* 组驱动chba assoc ie */
    ie_len = hmac_chba_encap_assoc_attribute(buffer, 0);
    buffer = buffer + ie_len;

    /* 组上层下发assoc ie; chba极速连接如果根据mac地址查找不到对应私有ie，再通过广播地址查询一次 */
    if (hmac_chba_check_fast_connect_flag(hmac_user->chba_user.connect_info.conn_flag) == OAL_TRUE) {
        len_ret = hmac_chba_encap_assoc_rsp_assoc_priv_ie(buffer, sta_addr);
        if (len_ret == 0) {
            len_ret = hmac_chba_encap_assoc_rsp_assoc_priv_ie(buffer, BROADCAST_MACADDR);
        }
        ie_len += len_ret;
    }
    return ie_len;
