...
#define MIN_AUTH_BODY_LEN 6
    ...
    mac_hdr = (mac_ieee80211_frame_stru *)(uintptr_t)mac_get_rx_cb_mac_header_addr(rx_ctrl);
    ...
    if (mac_get_frame_type_and_subtype((uint8_t *)mac_hdr) != (WLAN_FC0_SUBTYPE_AUTH | WLAN_FC0_TYPE_MGT)) {
        return OAL_FAIL;
    }
    if (hmac_chba_whitelist_check(mac_hdr->auc_address2, WLAN_MAC_ADDR_LEN) != OAL_SUCC) {
        return OAL_FAIL;
    }
