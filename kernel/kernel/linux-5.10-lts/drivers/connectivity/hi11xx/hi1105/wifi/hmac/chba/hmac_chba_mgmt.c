...
#ifdef _PRE_WLAN_FEATURE_NBW
    .config_para = { MAC_CHBA_VERSION_2, MAC_CHBA_SLOT_DURATION, MAC_CHBA_SCHEDULE_PERIOD, MAC_NBW_USLOT_DURATION,
        MAC_NBW_USLOT_NUM, MAC_CHBA_SYNC_SLOT_CNT,
...
void hmac_chba_update_island_info(hmac_vap_stru *hmac_vap, hmac_chba_island_list_stru *island_info)
{
    ...
    if (chba_vap_info == NULL) {
        oam_warning_log0(0, OAM_SF_CHBA, "{hmac_chba_update_island_info:fail to find chba vap!}");
        return;
    }
    /* 初始化岛信息链表 */
    hi_list_init(&island_info->island_list);
...
{
    uint8_t max_rssi_device_id = UINT8_INVALID_VALUE, island_owner_device_id, island_owner_change;
...
#ifdef _PRE_WLAN_FEATURE_NBW
    hmac_nbw_init_device_info(device_info);
#endif
    /* 下发CHBA初始化参数给driver */
