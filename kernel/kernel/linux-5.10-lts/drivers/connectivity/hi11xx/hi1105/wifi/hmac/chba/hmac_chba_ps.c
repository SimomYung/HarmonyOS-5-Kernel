...
oal_bool_enum_uint8 hmac_chba_latest_data_is_unuseful(hmac_chba_user_throughput_stru *throughput_info,
    hmac_chba_user_stru *user_info)
{
    /* 刚从host休眠恢复时，因为统计窗口会被清空，且powerup,powerdown计数也会清空，这种情况不需要经过流量模型 */
    if (throughput_info->trx_tsf_interval[throughput_info->peer_cnt] >= LARGE_TIME_INTERVAL &&
        user_info->power_up_count == 0 && user_info->power_down_count == 0) {
        return OAL_TRUE;
    }
    return OAL_FALSE;
}
...
/*
 * 功能描述  : 判断最新的数据，如果是超大包，直接进入全醒档位, 解决传输1G文件性能比拼场景（总时间10s左右）
 *   日    期  : 2024年5月10日
 *   作    者  : wifi
 */
static uint32_t hmac_chba_latest_data_is_huge_packet(hmac_chba_user_throughput_stru *info)
{
    if (info->rx_bytes_windows[info->peer_cnt] >= HUGE_PACKET_BYTES_THREASHOLD ||
        info->tx_bytes_windows[info->peer_cnt] >= HUGE_PACKET_BYTES_THREASHOLD) {
        return OAL_TRUE;
    }
    return OAL_FALSE;
}
...
/* 功能描述: 清空流量模型统计计算 */
OAL_STATIC void hmac_chba_reset_throught_info_stat_cnt(hmac_chba_user_stru *user_info)
{
    user_info->power_down_count = 0;
    user_info->power_up_count = 0;
    memset_s(user_info->throughput_info.trx_tsf_interval, sizeof(user_info->throughput_info.trx_tsf_interval), 0,
        sizeof(user_info->throughput_info.trx_tsf_interval));
}
...
uint32_t hmac_chba_update_user_bitmap_level(mac_vap_stru *mac_vap, uint8_t len, uint8_t *param)
{
    ...
    hmac_chba_reset_throught_info_stat_cnt(chba_user_info);
...
{
    uint8_t peer_cnt;
    uint32_t cur_stamp = (uint32_t)oal_time_get_stamp_ms();
    hmac_chba_user_throughput_stru *chba_user_throughput = NULL;
    dpe_wlan_user_stat_stru user_stats = {0};

    if (dpe_wlan_user_stat_sync(hmac_user->st_user_base_info.us_assoc_id,
    ...
    if (chba_user_throughput->last_rx_bytes == 0 && chba_user_throughput->last_tx_bytes == 0) {
        ...
        chba_user_throughput->rx_bytes_windows[peer_cnt] = chba_get_trx_bytes(chba_user_throughput->last_rx_bytes,
            user_stats.rx_bytes);
        chba_user_throughput->trx_tsf_interval[peer_cnt] =
            (uint32_t)oal_time_get_runtime(chba_user_throughput->last_tsf, cur_stamp);
        if (chba_vap_info->chba_log_level >= CHBA_DEBUG_STAT_INFO_LOG) {
            oam_warning_log4(0, OAM_SF_CHBA,
                "{hmac_chba_throughput_windows_update:user_idx[%d], tx_bytes[%d], rx_bytes[%d], interval[%d]}",
                hmac_user->st_user_base_info.us_assoc_id, chba_user_throughput->tx_bytes_windows[peer_cnt],
                chba_user_throughput->rx_bytes_windows[peer_cnt], chba_user_throughput->trx_tsf_interval[peer_cnt]);
        }
    }
    chba_user_throughput->last_tx_bytes = user_stats.tx_bytes;
    chba_user_throughput->last_rx_bytes = user_stats.rx_bytes;
    chba_user_throughput->last_tsf = cur_stamp;
    ...
    chba_user_throughput->large_tsf_interval_cnt = 0;
    for (peer_cnt = 0; peer_cnt < CHBA_THROUGHPUT_WINDOW_LEN; peer_cnt++) {
        ...
        if (chba_user_throughput->trx_tsf_interval[peer_cnt] >= LARGE_TIME_INTERVAL) {
            chba_user_throughput->large_tsf_interval_cnt++;
        }
    }
}

static uint8_t hmac_chba_calc_bitmap_by_cnt(hmac_chba_user_stru *chba_user_info, uint8_t cur_bitmap_level,
    hmac_chba_ps_throught_thres_stru *chba_thres)
{
...
/* 功能描述: host休眠期间流量模型不工作, 刚推出host休眠的吞吐数据不再反应每100ms的实际吞吐值, 此时需要特殊处理 */
static uint8_t hmac_chba_calc_bitmap_by_unuseful_data(hmac_chba_user_throughput_stru *throughput_info,
    hmac_chba_user_stru *chba_user_info, uint8_t cur_bitmap_level)
{
    /* 如果在历史5次采样周期中至少三次出现了大采样周期，说明trx不频繁,
       统计模型不能生效，如果此时bitmap为全f，直接置为33333333 */
    if (cur_bitmap_level == CHBA_BITMAP_ALL_AWAKE_LEVEL &&
        throughput_info->large_tsf_interval_cnt >= TIME_INTERVAL_THRESHOLD) {
        return CHBA_BITMAP_HALF_AWAKE_LEVEL;
    }

    /* 休眠期间突然来了大量数据, chba需立刻退出保活状态 */
    if ((cur_bitmap_level == CHBA_BITMAP_KEEP_ALIVE_LEVEL) && hmac_chba_latest_data_is_large_packet(throughput_info)) {
        oam_warning_log2(0, OAM_SF_ANY,
            "{hmac_chba_calc_bitmap_by_throughput:high throught wakeup chba!, tx_data[%d], rx_data[%d]}",
            throughput_info->tx_bytes_windows[throughput_info->peer_cnt],
            throughput_info->rx_bytes_windows[throughput_info->peer_cnt]);
        return CHBA_BITMAP_HALF_AWAKE_LEVEL;
    }
    return cur_bitmap_level;
}

OAL_STATIC uint8_t hmac_chba_calc_bitmap_service_force_wakeup(hmac_user_stru *hmac_user,
    hmac_chba_user_throughput_stru *throughput_info, hmac_chba_user_stru *chba_user_info)
{
    hmac_chba_reset_throught_info_stat_cnt(chba_user_info); /* 清理当前统计结果,流量模型重新计数 */
    if (chba_user_info->user_bitmap_level != CHBA_BITMAP_ALL_AWAKE_LEVEL) {
        oam_warning_log2(0, OAM_SF_ANY,
            "{hmac_chba_calc_bitmap_service_force_wakeup:set user[%d] all awake, cmd_type[%d]}",
            hmac_user->st_user_base_info.us_assoc_id, chba_user_info->user_ps_service_type_mode);
    }
    return CHBA_BITMAP_ALL_AWAKE_LEVEL; /* 特殊业务调整至全醒 */
}
    ...
    hmac_chba_user_stru *chba_user_info)
{
    ...
    uint8_t goto_awake_direct_flag;
    ...
    if (chba_user_info->user_ps_service_type_mode != CHBA_PS_DEFALUT_TYPE) { /* 特殊业务类型强制调整为全醒 */
        chba_user_info->user_ps_service_type_mode = CHBA_PS_DEFALUT_TYPE;
        return hmac_chba_calc_bitmap_service_force_wakeup(hmac_user, tmp_throughput_info, chba_user_info);
    }
    chba_user_info->user_ps_service_type_mode = CHBA_PS_DEFALUT_TYPE; /* 无论是否调整bitmap, 都需恢复为默认业务类型 */

    /* 如果在历史5次采样周期中至少三次出现了大采样周期，说明trx不频繁,
       统计模型不能生效，如果此时bitmap为全f，直接置为33333333 */
    if (hmac_chba_latest_data_is_unuseful(tmp_throughput_info, chba_user_info) == OAL_TRUE) {
        return hmac_chba_calc_bitmap_by_unuseful_data(tmp_throughput_info, chba_user_info, cur_bitmap_level);
    }
    ...
    if (!hmac_chba_latest_data_is_zero(tmp_throughput_info)) {
        ...
        /* 如果该统计周期的流量是极大，则无论当前是什么档位，直接到全醒态 */
        goto_awake_direct_flag = hmac_chba_latest_data_is_huge_packet(tmp_throughput_info);
        if (goto_awake_direct_flag == OAL_TRUE) {
            return CHBA_BITMAP_ALL_AWAKE_LEVEL;
        }
    ...
    return hmac_chba_calc_bitmap_by_cnt(chba_user_info, cur_bitmap_level, chba_thres);
}
 ...
 */
uint32_t hmac_chba_set_user_bitmap_event(mac_vap_stru *mac_vap, hmac_user_stru *hmac_user)
{
...
uint32_t hmac_chba_set_user_bitmap_debug(mac_vap_stru *mac_vap, uint16_t len, uint8_t *params)
{
    ...
    /***************************************************************************
        抛事件到DMAC层, 同步DMAC数据
    ***************************************************************************/
    ret = hmac_chba_set_user_bitmap_event(mac_vap, hmac_user);
    if (ret != OAL_SUCC) {
        return ret;
    }
 ...
 */
uint32_t hmac_chba_set_auto_bitmap_switch_event(mac_vap_stru *mac_vap, uint8_t new_auto_bitmap_switch)
{
    uint32_t ret;
    hmac_chba_vap_stru *chba_vap_info = NULL;
    hmac_vap_stru *hmac_vap = (hmac_vap_stru *)mac_res_get_hmac_vap(mac_vap->uc_vap_id);
    uint8_t tmp_auto_bitmap_switch = new_auto_bitmap_switch;
    ...
    chba_vap_info->auto_bitmap_switch = tmp_auto_bitmap_switch;

    /***************************************************************************
        抛事件到DMAC层, 同步DMAC数据
    ***************************************************************************/
    ret = hmac_config_send_event(mac_vap, WLAN_CFGID_CHBA_AUTO_BITMAP_CMD, sizeof(uint8_t), &tmp_auto_bitmap_switch);
    if (ret != OAL_SUCC) {
...
{
    uint32_t ret;
    ret = hmac_chba_set_auto_bitmap_switch_event(mac_vap, (uint8_t)params[0]);
    return ret;
}
 ...
 */
uint32_t hmac_chba_set_vap_bitmap_event(mac_vap_stru *mac_vap, uint8_t vap_bitmap)
{
    hmac_chba_vap_stru *chba_vap_info = NULL;
    chba_vap_info = hmac_get_up_chba_vap_info();
    if (chba_vap_info == NULL) {
...
uint32_t hmac_chba_set_vap_bitmap_debug(mac_vap_stru *mac_vap, uint16_t len, uint8_t *params)
{
    ...
    vap_bitmap = *params;
    ret = hmac_chba_set_vap_bitmap_event(mac_vap, vap_bitmap);
    return ret;
}
uint32_t hmac_chba_service_ask_user_all_awake(mac_vap_stru *mac_vap, uint16_t len, uint8_t *params)
{
    hmac_vap_stru *hmac_vap = NULL;
    hmac_chba_vap_stru *chba_vap_info = NULL;
    hmac_user_stru *hmac_user = NULL;
    hmac_chba_user_stru *chba_user_info = NULL;
    wlan_chba_service_ask_awake_stru *service_info = NULL;

    if (mac_is_chba_mode(mac_vap) == OAL_FALSE) {
        oam_error_log0(0, OAM_SF_ANY, "{hmac_chba_service_ask_user_all_awake:not chba vap}");
        return OAL_FAIL;
    }
    hmac_vap = (hmac_vap_stru *)mac_res_get_hmac_vap(mac_vap->uc_vap_id);
    if (hmac_vap == NULL) {
        oam_error_log0(mac_vap->uc_vap_id, OAM_SF_ANY, "{hmac_chba_service_ask_user_all_awake:fail to find hmac_vap}");
        return OAL_FAIL;
    }
    chba_vap_info = hmac_chba_get_chba_vap_info(hmac_vap);
    if (chba_vap_info == NULL) {
        oam_error_log0(mac_vap->uc_vap_id, OAM_SF_ANY, "{hmac_chba_service_ask_user_all_awake:fail to get chba_info}");
        return OAL_FAIL;
    }

    service_info = (wlan_chba_service_ask_awake_stru *)params;
    hmac_user = mac_vap_get_hmac_user_by_addr(mac_vap, service_info->user_mac_addr, WLAN_MAC_ADDR_LEN);
    if (hmac_user == NULL) {
        oam_error_log3(mac_vap->uc_vap_id, OAM_SF_ANY,
            "{hmac_chba_service_ask_user_all_awake:%02x:xx:xx:xx:%02x:%02x is not my user!}",
            service_info->user_mac_addr[MAC_ADDR_0], service_info->user_mac_addr[MAC_ADDR_4],
            service_info->user_mac_addr[MAC_ADDR_5]);
        return OAL_FAIL;
    }
    chba_user_info = &(hmac_user->chba_user);
    chba_user_info->user_ps_service_type_mode = service_info->ps_service_type;
    oam_warning_log2(mac_vap->uc_vap_id, OAM_SF_ANY,
        "{hmac_chba_service_ask_user_all_awake:set user[%d] to all_awake, service_type[%d]}",
        hmac_user->st_user_base_info.us_assoc_id, chba_user_info->user_ps_service_type_mode);
    /* 避免host入睡, 立刻发起一次bitmap等级调整 */
...
void hmac_chba_stat_user_rx_netbuf(oal_netbuf_stru *netbuf)
{
    ...
    hmac_user = (hmac_user_stru *)mac_res_get_hmac_user(rx_cb->bit_rx_user_id);
