...
/* 4 函数实现 */
/* 功能描述: chba同步状态更新时通知上层 */
OAL_STATIC void hmac_chba_update_sync_status_report(uint8_t last_sync_status, uint8_t cur_sync_status)
{
#if (_PRE_OS_VERSION_LINUX == _PRE_OS_VERSION)
    mac_chba_sync_status_report_stru report_info = { 0 };

    if (last_sync_status == cur_sync_status) {
        return;
    }
    report_info.report_type = HMAC_CHBA_UPDATE_SYNC_STATUS;
    report_info.chba_sync_status = cur_sync_status;
    oal_cfg80211_chba_report((uint8_t *)(&report_info), sizeof(report_info));
    oam_warning_log2(0, OAM_SF_ANY, "{hmac_chba_update_sync_status_report:[%d]->[%d]}",
        last_sync_status, cur_sync_status);
#endif
}
...
void hmac_chba_set_sync_request_flag(sync_info_flags *sync_flags, uint8_t new_flag)
{
    ...
    uint8_t new_sync_status;
    uint8_t last_sync_status = hmac_chba_get_sync_status();
    ...
    oam_warning_log2(0, OAM_SF_CFG, "{hmac_chba_set_sync_request_flag::old[%d]new[%d].",
        old_flag, new_flag);
    new_sync_status = hmac_chba_get_sync_status();
    hmac_chba_update_sync_status_report(last_sync_status, new_sync_status);
}
...
/* 功能描述: 获取chba vap同步状态 */
oal_bool_enum_uint8 hmac_chba_get_sync_status(void)
{
    ...
    /* 时钟同步流程和域合并流程, 视为chba设备正在更新同步状态 */
    return ((hmac_chba_get_sync_request_flag(sync_flags) == OAL_TRUE) ||
        (hmac_chba_get_domain_merge_flag(domain_merge) == OAL_TRUE));
}
...
uint32_t hmac_chba_domain_merge_sync_wait_timeout(void *p)
{
    ...
    uint8_t max_retry_cnt = ((domain_merge->ask_for_sync_mgmt_type == MAC_CHBA_SYNC_REQUEST) ?
        MAC_CHBA_SYNC_REQUEST_CNT : MAC_MERGE_REQ_MAX_CNT);
    ...
    /* 连续请求同步次数超过门限, 放弃域合并 */
    if (domain_merge->ask_for_sync_cnt > max_retry_cnt) {
...
uint32_t hmac_chba_sync_multiple_domain_info(mac_vap_stru *mac_vap, uint8_t len, uint8_t *param)
{
    ...
    mac_chba_multiple_domain_stru  *multi_domain = (mac_chba_multiple_domain_stru  *)(param);
    ...
    /* 当前master比device上报的新master rp值大或相等, 不启动域合并 */
    if (hmac_chba_sync_dev_rp_compare(cur_master, &multi_domain->another_master) == OAL_TRUE) {
        ...
        /* device上报的新master就是当前master, 重复上报的域合并事件不处理;否则给对方发一个beacon帧，增加域合并触发概率 */
        if (oal_memcmp(cur_master->master_addr, &multi_domain->another_master.master_addr, OAL_MAC_ADDR_LEN) != 0) {
