...
void hmac_chba_user_set_connect_role(hmac_user_stru *hmac_user,
    chba_connect_role_enum connect_role, uint8_t conn_flag)
{
    if (hmac_user == NULL) {
    ...
    hmac_user->chba_user.connect_info.conn_flag = conn_flag;
    oam_warning_log3(0, OAM_SF_CHBA,
        "CHBA:set user [%d] connect_role to [%d]. 0: INITIATOR 1: RESPONDER, conn_flag[%d]",
        hmac_user->st_user_base_info.us_assoc_id, connect_role, conn_flag);
}
    ...
    uint16_t user_index)
{
    ...
    if (mac_vap->nbw_mode == NBW_MODE) {
        /* TDO: nbw 重配置user bitmap, bitmap取值待重配 */
        // 1. 调用hmac_nbw_sync_user_bitmap进行nbw user bitmap配置
    }
#endif
    /* 同步到dmac 初始化user低功耗配置 */
    user_ps_cfg.user_idx = user_index;
    user_ps_cfg.bitmap_level = chba_user_info->user_bitmap_level;
    if (hmac_config_send_event(mac_vap, WLAN_CFGID_CHBA_SET_USER_BITMAP_LEVEL, sizeof(chba_user_ps_cfg_stru),
        (uint8_t *)&user_ps_cfg) != OAL_SUCC) {
        oam_warning_log0(mac_vap->uc_vap_id, OAM_SF_CFG, "{hmac_chba_user_bitmap_level_init::send_event failed.}");
    }
}
...
uint32_t hmac_chba_add_user(mac_vap_stru *mac_vap, uint8_t *peer_addr, uint16_t *user_idx)
{
    ...
    memcpy_s(user_info.mac_addr, WLAN_MAC_ADDR_LEN, peer_addr, WLAN_MAC_ADDR_LEN);
    /* 添加用户 */
    ...
    *user_idx = user_index;
    ...
    chba_user->connect_info.last_asoc_state = MAC_USER_STATE_BUTT;
...
void hmac_chba_del_user_proc(hmac_vap_stru *hmac_vap, hmac_user_stru *hmac_user)
{
    ...
    /* 从chba私有app ie链表中删除对应设备ie */
    hmac_chba_del_assoc_priv_ie(peer_mac_addr, OAL_MAC_ADDR_LEN);
    /* 建链完成后白名单会自动删除; 但如果是建链过程中del user, 需要主动清理一次白名单 */
    hmac_chba_whitelist_del_user(peer_mac_addr, OAL_MAC_ADDR_LEN);
...
uint32_t hmac_chba_user_mgmt_conn_prepare(mac_vap_stru *mac_vap, hmac_chba_conn_param_stru *chba_conn_params,
    uint16_t *user_idx, uint8_t connect_role, uint8_t conn_flag)
{
    hmac_user_stru *hmac_user = NULL;
    hmac_chba_user_stru *chba_user = NULL;
    uint16_t user_index;
    ...
    *user_idx = user_index;
    ...
    chba_user = &(hmac_user->chba_user);
    hmac_chba_user_set_connect_role(hmac_user, connect_role, conn_flag);
    chba_user->connect_info.assoc_channel = chba_conn_params->assoc_channel;
    /* 关联发起方需要修改uc_assoc_vap_id,以便组帧 */
    mac_vap_set_assoc_id(mac_vap, user_index);
    return OAL_SUCC;
}

void hmac_chba_save_cur_user_asoc_state(hmac_vap_stru *hmac_vap, hmac_user_stru *hmac_user,
    mac_user_asoc_state_enum_uint8 asoc_state)
{
    ...
    if (hmac_user == NULL) {
        oam_error_log0(0, OAM_SF_CHBA,
            "{hmac_chba_save_cur_user_asoc_state:user is NULL}");
        return;
    }
    hmac_user->chba_user.connect_info.last_asoc_state = asoc_state;
}

mac_user_asoc_state_enum_uint8 hmac_chba_get_last_user_asoc_state(hmac_user_stru *hmac_user)
{
    if (hmac_user == NULL) {
        oam_error_log0(0, OAM_SF_CHBA,
            "{hmac_chba_get_last_user_asoc_state:user is NULL}");
        return MAC_USER_STATE_BUTT;
    }

    return hmac_user->chba_user.connect_info.last_asoc_state;
}
#endif
