                                ...
                                uint16_t us_len, uint8_t *puc_param)
{
    ...
    ret = hmac_config_alloc_event(mac_vap, HMAC_TO_DMAC_SYN_CFG, &pst_syn_msg, &pst_event_mem, us_len);
    ...
    if ((puc_param != NULL) && (us_len)) {
        if (memcpy_s(pst_syn_msg->auc_msg_body, (uint32_t)us_len, puc_param, (uint32_t)us_len) != EOK) {
    ...
    if (oal_unlikely(ret != OAL_SUCC)) {
        oam_warning_log1(mac_vap->uc_vap_id, OAM_SF_CFG, "{hmac_config_send_event::dispatch failed[%d].}", ret);
        frw_event_free_m(pst_event_mem);
        return ret;
    }
...
uint32_t hmac_config_send_event_adapt_dpe(uint8_t vap_idx, wlan_cfgid_enum_uint16 en_cfg_id,
    uint16_t us_len, uint8_t *puc_param)
{
    mac_vap_stru *mac_vap = mac_res_get_mac_vap(vap_idx);
    if (mac_vap == NULL) {
        return OAL_FAIL;
    }

    return hmac_config_send_event(mac_vap, en_cfg_id, us_len, puc_param);
}
...
uint32_t hmac_config_protocol_debug_switch(mac_vap_stru *pst_mac_vap, uint16_t us_len, uint8_t *puc_param)
{
    ...
    if (pst_protocol_debug->cmd_bit_map & BIT0) {
        /* 切到40M后使能mib能力，其余能力从device同步 */
        if (wlan_band_width_is_40m(pst_protocol_debug->en_band_force_switch_bit0) == OAL_TRUE) {
...
uint32_t hmac_config_tas_ant_switch(mac_vap_stru *pst_mac_vap, uint16_t us_len, uint8_t *puc_param)
{
    ...
    /***************************************************************************
        抛事件到DMAC层, 同步DMAC数据
    ***************************************************************************/
    ret = hmac_config_send_event(pst_mac_vap, WLAN_CFGID_TAS_ANT_SWITCH, us_len, puc_param);
    if (oal_unlikely(ret != OAL_SUCC)) {
        oam_warning_log1(pst_mac_vap->uc_vap_id, OAM_SF_CFG,
                         "hmac_config_tas_pwr_ctrl::hmac_config_send_event failed, error no[%d]!", ret);
        return ret;
...
uint32_t hmac_config_report_mac_addr(mac_vap_stru *mac_vap, uint8_t len, uint8_t *param)
{
    ...
    efuse_mac_addr_len = (g_wlan_device_mac_addr_2byte_flg == 1) ? NUM_4_BYTES : NUM_3_BYTES;
    for (i = 0; i < efuse_mac_addr_len; i++) {
...
uint32_t hmac_config_compete_5g_process(mac_vap_stru *mac_vap, uint16_t len, uint8_t *param)
{
    uint32_t ret = OAL_SUCC;
    mac_ioctl_alg_param_stru pst_alg_param = {0};
    int32_t online_cali[CMD_PARAMS_MAX_CNT] = {0};
    host_cmd_stru cmd_info[] = {{"set_eqmode", 1, NULL, NULL, hmac_package_uint8_params, OAL_TRUE,
        WLAN_CFGID_EQUIPMENT_MODE}};

    pst_alg_param.en_alg_cfg = MAC_ALG_CFG_AUTORATE_RTS_MODE;
    pst_alg_param.value = 0; /* 关RTS */
    online_cali[0] = 1;

    /* 关RTS */
    hmac_config_alg_param(mac_vap, sizeof(mac_ioctl_alg_param_stru), (uint8_t *)&pst_alg_param);
    oam_warning_log0(mac_vap->uc_vap_id, OAM_SF_CFG, "hmac_config_compete_pow_cfg::rts has benn closed");

    if (mac_vap->bit_ap_chip_oui == WLAN_AP_CHIP_OUI_BCM) {
        /* BCM关闭校准 */
        ret = wal_send_cmd_params_to_device(mac_vap, &cmd_info[0], online_cali, 0);
        if (ret != OAL_SUCC) {
            oam_error_log0(mac_vap->uc_vap_id, OAM_SF_ANY, "wal_process_cmd:cmd_id0,send event fail!");
            return ret;
        }
        oam_warning_log0(mac_vap->uc_vap_id, OAM_SF_CFG, "hmac_config_compete_pow_cfg::online clai has benn closed");
    }
    return ret;
}
...
{
    int32_t rx_mode[CMD_PARAMS_MAX_CNT] = {0}; // host
    host_cmd_stru cmd_info[] = {{ "rx_mode_switch", 1, NULL, NULL,
    ...
    /* 调整host rx */
    wal_send_cmd_params_to_device(mac_vap, &cmd_info[0], rx_mode, 0);
}

uint32_t hmac_config_compete_pow_cfg(mac_vap_stru *mac_vap, uint16_t len, uint8_t *param)
{
    uint32_t ret = OAL_SUCC;
    int32_t tx_mode[CMD_PARAMS_MAX_CNT] = {0};
    mac_ioctl_alg_param_stru pst_alg_param = {0};
    alg_cfg_param_txmode_all_user_stru pst_alg_param_txmode = {{0}};

    tx_mode[0] = 0;
    pst_alg_param.en_alg_cfg = MAC_ALG_CFG_TPC_MODE;
    pst_alg_param.value = 0; /* 关TPC */
    pst_alg_param_txmode.st_alg_cfg_hdr.en_cfg_id = MAC_ALG_CFG_TXMODE_ALL_USER_FIX_MODE;
    pst_alg_param_txmode.en_fix_txmode = ALG_CFG_TX_MODE_CSD2_FIX;

    hmac_tx_mode_switch(mac_vap, tx_mode); /* 调整host tx */
    ...
    if (mac_vap->st_channel.en_band == WLAN_BAND_5G) {
        ret = hmac_config_compete_5g_process(mac_vap, len, param);
    }

    hmac_config_set_rx_mode(mac_vap, OAL_TRUE);
    /* 关TPC */
    hmac_config_alg_param(mac_vap, sizeof(mac_ioctl_alg_param_stru), (uint8_t *)&pst_alg_param);
    oam_warning_log0(mac_vap->uc_vap_id, OAM_SF_CFG, "hmac_config_compete_pow_cfg::tpc has benn closed");

    /* 关TXmode, 固定至CSD模式 */
    hmac_config_alg_send_event(mac_vap, WLAN_CFGID_ALG_ENTRY, sizeof(alg_cfg_param_txmode_all_user_stru),
        (uint8_t *)&pst_alg_param_txmode);
    return ret;
}
...
uint32_t hmac_config_ch_status_sync(mac_device_stru *pst_mac_dev)
{
    ...
    pst_mac_vap = mac_res_get_mac_vap(pst_mac_dev->uc_cfg_vap_id);
    if (pst_mac_vap == NULL) {
...
uint32_t hmac_config_set_all_log_level(mac_vap_stru *pst_mac_vap, uint16_t us_len, uint8_t *puc_param)
{
    ...
    for (vap_idx = 0; vap_idx < mac_chip_get_vap_support_max_num_limit(); vap_idx++) {
...
uint32_t hmac_config_set_cus_dyn_cali(mac_vap_stru *mac_vap, uint16_t len, uint8_t *param)
{
    ...
    /***************************************************************************
        抛事件到DMAC层, 同步DMAC数据
    ***************************************************************************/
    ret = hmac_config_send_event(mac_vap, WLAN_CFGID_SET_CUS_DYN_CALI_PARAM, len, param);
    if (oal_unlikely(ret != OAL_SUCC)) {
        oam_warning_log1(mac_vap->uc_vap_id, OAM_SF_CFG,
...
{
    uint32_t ret;
    uint8_t param = rx_mode;

    /* 调整rx_mode */
    ret = hmac_config_send_event(mac_vap, WLAN_CFGID_RX_MODE_SWITCH_PM, sizeof(uint8_t), (uint8_t *)&param);
    if (ret != OAL_SUCC) {
        oam_warning_log1(0, OAM_SF_TX, "{hmac_config_pm_rx_mode::switch to [%d] send event fail!}", rx_mode);
    }
...
uint32_t hmac_send_custom_data(mac_vap_stru *mac_vap, uint16_t len, uint8_t *param, uint8_t syn_id)
{
    ...
    if (netbuf == NULL) {
        oam_error_log0(mac_vap->uc_vap_id, OAM_SF_ANY, "{hmac_send_custom_data::netbuf alloc failed");
        return OAL_FAIL;
    }
    ...
    if (event_mem == NULL) {
        oam_error_log0(mac_vap->uc_vap_id, OAM_SF_ANY, "{hmac_send_custom_data::pst_event_mem null.}");
        oal_netbuf_free(netbuf);
        return OAL_ERR_CODE_PTR_NULL;
    }
...
#endif
#endif
uint32_t hmac_config_ai_csma_edca_cfg_result_proc(mac_vap_stru *mac_vap, uint8_t len, uint8_t *param)
{
#if (_PRE_OS_VERSION_LINUX == _PRE_OS_VERSION)
    hmac_ai_csma_edca_cfg_result_stru cfg_result = {0};

    cfg_result.type = HMAC_AI_WIFI_DATA_REPORT;
    cfg_result.sub_type = AI_CSMA_EDCA_CFG_RESULT_REPORT;
    cfg_result.ac_type = param[0];
    cfg_result.result = param[1];
    /* 复用chba上报接口通知上层 */
    oal_cfg80211_chba_report((uint8_t *)(&cfg_result), sizeof(hmac_ai_csma_edca_cfg_result_stru));
#endif
    return OAL_SUCC;
}
uint32_t hmac_config_ai_csma_edca_interrupt_report_proc(mac_vap_stru *mac_vap, uint8_t len, uint8_t *param)
{
#if (_PRE_OS_VERSION_LINUX == _PRE_OS_VERSION)
    hmac_ai_csma_edca_interrupt_report_stru csma_report = {0};

    csma_report.type = HMAC_AI_WIFI_DATA_REPORT;
    csma_report.sub_type = AI_CSMA_EDCA_INTERRUPT_REPORT;
    /* 复用chba上报接口通知上层 */
    oal_cfg80211_chba_report((uint8_t *)(&csma_report), sizeof(hmac_ai_csma_edca_interrupt_report_stru));
#endif
    return OAL_SUCC;
}
/*
...
OAL_STATIC const hmac_config_syn_stru g_ast_hmac_config_syn[] = {
...
#endif
#ifdef _PRE_WLAN_FEATURE_EMLSR
    { WLAN_CFGID_EMLSR_INFO_SYNC,     { 0, 0 }, hmac_config_d2h_emlsr_info_syn },
#endif
#ifdef _PRE_WLAN_FEATURE_LP_MIRACAST
    ...
    { WLAN_CFGID_TAS_ANT_STATUS_RPT, { 0, 0 }, hmac_tas_switch_result_notify_event_status },
#endif
    ...
    { WLAN_CFGID_CHBA_TX_COMPLETE_ASSOC_RSP, { 0, 0 }, hmac_chba_tx_complete_assoc_rsp },
...
#endif
    {WLAN_CFGID_AI_CSMA_EDCA_CFG_RESULT, { 0, 0 }, hmac_config_ai_csma_edca_cfg_result_proc},
    {WLAN_CFGID_AI_CSMA_EDCA_INTERRUPT_REPORT, { 0, 0 }, hmac_config_ai_csma_edca_interrupt_report_proc},
#ifdef _PRE_WLAN_FEATURE_11AX
    { WLAN_CFGID_GET_OMI_BW_SWITCH, { 0, 0 }, hmac_config_d2h_notify_omi_bw_switch},
#endif
    { WLAN_CFGID_GET_LOWPOWER_STATISTICS, {0, 0}, hmac_config_query_lowpower_stat_rsp },
