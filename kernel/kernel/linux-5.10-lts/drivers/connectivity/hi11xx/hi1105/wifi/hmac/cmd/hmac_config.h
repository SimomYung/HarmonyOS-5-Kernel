...
typedef struct {
    uint8_t type; /* 内核用来区分消息类型 */
    uint8_t sub_type;
    uint8_t resv[2]; /* 4字节对齐 */
} hmac_ai_csma_edca_interrupt_report_stru;
typedef struct {
    uint8_t type; /* 内核用来区分消息类型 */
    uint8_t sub_type;
    uint8_t ac_type;
    uint8_t result;
} hmac_ai_csma_edca_cfg_result_stru;
typedef enum {
    AI_CSMA_EDCA_INTERRUPT_REPORT = 1,
    AI_CSMA_EDCA_CFG_RESULT_REPORT = 2,
} hmac_ai_wifi_data_report_enum;
enum {
...
extern oal_wait_queue_head_stru g_power_off_ready_wait_queue;
...
uint32_t hmac_config_sta_update_rates(mac_vap_stru *pst_mac_vap, mac_cfg_mode_param_stru *pst_cfg_mode,
...
void hmac_config_set_rx_mode(mac_vap_stru *mac_vap, uint8_t host_rx);
...
uint32_t hmac_config_send_event(mac_vap_stru *mac_vap, wlan_cfgid_enum_uint16 en_cfg_id,
    uint16_t us_len, uint8_t *puc_param);
uint32_t hmac_config_send_event_adapt_dpe(uint8_t vap_idx, wlan_cfgid_enum_uint16 en_cfg_id,
    uint16_t us_len, uint8_t *puc_param);
...
uint32_t hmac_config_set_freq(mac_vap_stru *mac_vap, uint16_t len, uint8_t *param);
uint32_t hmac_config_get_freq(mac_vap_stru *pst_mac_vap, uint16_t *pus_len, uint8_t *puc_param);
uint32_t hmac_config_set_wmm_params(mac_vap_stru *mac_vap, uint16_t len, uint8_t *param);
...
uint32_t hmac_config_kick_user(mac_vap_stru *mac_vap, uint16_t len, uint8_t *param);
...
uint32_t hmac_config_tas_ant_switch(mac_vap_stru *pst_mac_vap, uint16_t us_len, uint8_t *puc_param);
...
uint8_t hmac_check_single_link_enable(const mac_bss_dscr_stru *bss_dscr);
...
uint32_t hmac_config_query_station_info(mac_vap_stru *mac_vap, uint16_t len, uint8_t *param);
...
uint32_t hmac_config_query_lowpower_stat(mac_vap_stru *mac_vap, uint16_t len, uint8_t *param);
...
uint32_t hmac_init_user_security_port(mac_vap_stru *pst_mac_vap, hmac_user_stru *hmac_user);
...
uint32_t hmac_config_set_freq(mac_vap_stru *mac_vap, uint16_t len, uint8_t *param);
uint32_t hmac_config_set_mode(mac_vap_stru *mac_vap, uint16_t len, uint8_t *param);
void hmac_set_device_freq_mode(uint8_t device_enable);
...
uint32_t hmac_config_dpd_cali_factory(mac_vap_stru *mac_vap, uint16_t len, uint8_t *param);
...
uint32_t hmac_config_pm_rx_mode(mac_vap_stru *mac_vap, uint8_t rx_mode);
...
uint32_t hmac_config_user_ml_info_syn(mac_vap_stru *mac_vap, mac_user_stru *mac_user);
...
uint32_t hmac_config_user_mld_cap_syn(mac_vap_stru *mac_vap, mac_user_stru *mac_user);
...
uint32_t hmac_config_query_lowpower_stat_rsp(mac_vap_stru *mac_vap, uint8_t len, uint8_t *param);
