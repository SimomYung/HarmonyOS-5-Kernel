...
uint32_t hmac_atcmdsrv_report_efuse_reg(mac_vap_stru *pst_mac_vap, uint8_t uc_len, uint8_t *puc_param)
{
    ...
    if (get_mpxx_subchip_type() == BOARD_VERSION_MP17C) {
        conn_nve_write_efuse(g_us_efuse_buffer);
    }
#endif
    /* 唤醒wal_sdt_recv_reg_cmd等待的进程 */
    ...
    mp1x_wlan_cali_data_para_stru *cali_data)
{
    ...
    mp1x_wlan_cali_basic_para_stru *basic_cali_data_5g[] = {
    ...
    mp1x_wlan_cali_basic_para_stru *basic_cali_data_6g[] = {
    ...
    for (dpd_lvl_idx = 0; dpd_lvl_idx < WLAN_DPD_TPC_LEVEL_NUM; dpd_lvl_idx++) {
        info->buf = (uint8_t *)(&basic_cali_data->dpd_cali_data.dpd_cali_para[dpd_lvl_idx]);
...
OAL_STATIC int32_t hmac_save_dpd_by_band_mp12(nv_dpd_user_info_stru *info,
    mp12_wlan_cali_data_para_stru *cali_data)
{
    uint8_t dpd_lvl_idx;
    int32_t ret;
    mp12_wlan_cali_basic_para_stru *basic_cali_data = NULL;
    mp12_wlan_cali_basic_para_stru *basic_cali_data_5g[] = {
        cali_data->rf_cali_data[info->stream].cali_data_5g_20m.cali_data,
        cali_data->rf_cali_data[info->stream].cali_data_5g_20m.cali_data,
        cali_data->rf_cali_data[info->stream].cali_data_5g_80m.cali_data
    };
...
OAL_STATIC int32_t hmac_atcmdsrv_save_2g_dpd_data(uint8_t *mem_addr, uint8_t rf_num)
{
    nv_dpd_user_info_stru info = { WLAN_SINGLE_STREAM_0, WLAN_CALI_BAND_2G, WLAN_BANDWIDTH_20, 0, 0, 0, 0, NULL };
    int32_t ret;
    ...
    for (info.stream = WLAN_SINGLE_STREAM_0; info.stream < rf_num; info.stream++) {
        ...
        for (info.band_idx = 0; info.band_idx < WLAN_2G_CALI_BAND_NUM; info.band_idx++) {
            if (get_mpxx_subchip_type() == BOARD_VERSION_MP12) {
                info.dpd_lut_size = sizeof(mp12_wlan_cali_dpd_lut_stru);
                ret = hmac_save_dpd_by_band_mp12(&info, (mp12_wlan_cali_data_para_stru *)mem_addr);
            } else {
                info.dpd_lut_size = sizeof(mp1x_wlan_cali_dpd_lut_stru);
                ret = hmac_save_dpd_by_band(&info, (mp1x_wlan_cali_data_para_stru *)mem_addr);
            }
            if (ret != CONN_NVE_RET_OK) {
                return ret;
            }
...
OAL_STATIC int32_t hmac_atcmdsrv_save_dpd_band_data(uint8_t *mem_addr,
    nv_dpd_user_info_stru *info, uint8_t *band_num_info, uint8_t info_len)
{
    int32_t ret = CONN_NVE_RET_OK;
    int32_t type = get_mpxx_subchip_type();
    uint8_t bw_butt = (type == BOARD_VERSION_MP12) ? WLAN_BANDWIDTH_80 : WLAN_BANDWIDTH_160;
    for (info->bw = WLAN_BANDWIDTH_20; info->bw <= bw_butt; info->bw++) {
...
OAL_STATIC int32_t hmac_atcmdsrv_save_5g_dpd_data(uint8_t *mem_addr, uint8_t rf_num)
{
    nv_dpd_user_info_stru info = { WLAN_SINGLE_STREAM_0, WLAN_CALI_BAND_5G, 0, 0, 0, 0, 0, NULL };
    uint8_t band_num_info[NUM_4_BYTES] = { WLAN_5G_20M_CALI_BAND_NUM, WLAN_5G_20M_CALI_BAND_NUM,
        WLAN_5G_20M_CALI_BAND_NUM, WLAN_5G_160M_CALI_BAND_NUM };
    int32_t ret = CONN_NVE_RET_OK;
    ...
    for (info.stream = WLAN_SINGLE_STREAM_0; info.stream < rf_num; info.stream++) {
        ...
        ret += hmac_atcmdsrv_save_dpd_band_data(mem_addr, &info, band_num_info, NUM_4_BYTES);
    }
    return ret;
}

#ifdef _PRE_WLAN_FEATURE_6G_EXTEND
OAL_STATIC int32_t hmac_atcmdsrv_save_6g_dpd_data(uint8_t *mem_addr, uint8_t rf_num)
{
    nv_dpd_user_info_stru info = { WLAN_SINGLE_STREAM_0, WLAN_CALI_BAND_6G, 0, 0, 0, 0, 0, NULL };
    uint8_t band_num_info[NUM_4_BYTES] = { WLAN_6G_20M_CALI_BAND_NUM, WLAN_6G_20M_CALI_BAND_NUM,
        WLAN_6G_20M_CALI_BAND_NUM, WLAN_6G_160M_CALI_BAND_NUM };
    int32_t ret = CONN_NVE_RET_OK;
...
OAL_STATIC uint32_t hmac_atcmdsrv_save_dpd_data(mac_vap_stru *mac_vap, uint8_t uc_len, uint8_t *puc_param)
{
    ...
    ret = hmac_atcmdsrv_save_2g_dpd_data(mem_addr, rf_num);
    if (ret != CONN_NVE_RET_OK) {
    ...
    ret = hmac_atcmdsrv_save_5g_dpd_data(mem_addr, rf_num);
    if (ret != CONN_NVE_RET_OK) {
        pst_hmac_vap->st_atcmdsrv_get_status.dbb_num = (uint32_t)ret;
        oam_error_log1(mac_vap->uc_vap_id, OAM_SF_CFG,
...
#ifdef _PRE_WLAN_FEATURE_6G_EXTEND
    ret = hmac_atcmdsrv_save_6g_dpd_data(mem_addr, rf_num);
    if (ret != CONN_NVE_RET_OK) {
        pst_hmac_vap->st_atcmdsrv_get_status.dbb_num = (uint32_t)ret;
        oam_error_log1(mac_vap->uc_vap_id, OAM_SF_CFG,
...
uint32_t hmac_config_dpd_cali_factory(mac_vap_stru *mac_vap, uint16_t len, uint8_t *param)
{
    uint32_t ret;
#ifdef HISI_CONN_NVE_SUPPORT
    ret = hmac_atcmdsrv_save_dpd_data(mac_vap, len, param);
...
uint32_t hmac_config_ssi_test(mac_vap_stru *pst_mac_vap, uint8_t uc_len, uint8_t *puc_param)
{
    ...
    if (memcpy_s(&st_ssi_stru, sizeof(hmac_atcmdsrv_ssi_stru),
        puc_param, sizeof(hmac_atcmdsrv_ssi_stru)) != EOK) {
