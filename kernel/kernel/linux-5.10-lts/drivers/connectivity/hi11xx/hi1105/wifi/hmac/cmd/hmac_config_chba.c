...
uint32_t hmac_config_chba_set_ps_thres(mac_vap_stru *mac_vap, uint16_t len, uint8_t *params)
{
    ...
    oam_warning_log2(mac_vap->uc_vap_id, OAM_SF_CFG, "{hmac_config_chba_set_ps_thres: thres_type[%d], \
        new_ps_throught_thres %d(100ms)}", ps_thres_cfg->thres_type, ps_thres_cfg->thres_val);
