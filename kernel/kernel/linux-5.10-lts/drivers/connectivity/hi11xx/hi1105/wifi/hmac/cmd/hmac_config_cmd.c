 ...
 *   修改内容  : 新生成函数
 */
uint32_t hmac_config_set_pmksa(mac_vap_stru *mac_vap, uint16_t len, uint8_t *param)
{
    ...
    cfg_pmksa = (mac_cfg_pmksa_param_stru *)param;
...
uint32_t hmac_config_scan_abort(mac_vap_stru *pst_mac_vap, uint16_t us_len, uint8_t *puc_param)
{
    ...
    scan_mgmt = hmac_get_scam_mgmt(pst_hmac_vap);
    if (scan_mgmt == NULL) {
        oam_warning_log0(pst_mac_vap->uc_vap_id, OAM_SF_SCAN,
                         "{hmac_config_scan_abort::scan_mgmt is null.}");
        return OAL_ERR_CODE_MAC_DEVICE_NULL;
    }

    oam_warning_log2(pst_mac_vap->uc_vap_id, OAM_SF_SCAN,
                     "{hmac_config_scan_abort::scan abort,curr_scan_vap_id:%d vap state: %d.}",
                     scan_mgmt->st_scan_record_mgmt.uc_vap_id,
    ...
    if ((pst_mac_vap->en_vap_mode == WLAN_VAP_MODE_BSS_AP) &&
        ...
        (scan_mgmt->st_scan_record_mgmt.uc_vap_id == pst_mac_vap->uc_vap_id)) {
        oam_warning_log1(pst_hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_SCAN,
                         "{hmac_config_scan_abort::en_vap_last_state:%d}",
                         scan_mgmt->st_scan_record_mgmt.en_vap_last_state);
        hmac_fsm_change_state(pst_hmac_vap, scan_mgmt->st_scan_record_mgmt.en_vap_last_state);
        scan_mgmt->st_scan_record_mgmt.en_vap_last_state = MAC_VAP_STATE_BUTT;
    }
    /* END: mp12 作为ap ，40M 带宽下执行扫描，扫描完成后VAP 状态修改为扫描前的状态 */

    /* ml扫描到指定bssid信息，先抛事件到device停止扫描，再执行关联/漫游。
     * 避免漫游流程配置mac地址和扫描流程恢复mac地址冲突，使得两个vap mac 地址配置冲突。
     */
    ...
    /* 删除HMAC 扫描超时定时器 */
    if (scan_mgmt->st_scan_timeout.en_is_registerd == OAL_TRUE) {
        frw_timer_immediate_destroy_timer_m(&(scan_mgmt->st_scan_timeout));
        ret = hmac_scan_proc_scan_timeout_fn((void *)scan_mgmt);
    }
...
uint32_t hmac_config_get_dbdc_info(mac_vap_stru *pst_mac_vap, uint8_t uc_len, uint8_t *puc_param)
{
    ...
    hal_host_device_stru *hal_device = (hal_host_device_stru *)hal_get_host_device(HAL_DEVICE_ID_SLAVE);
    ...
    if (pst_mac_device->en_dbdc_running == OAL_TRUE) {
        hal_host_unmask_soc_host_intr(hal_device);
    }
    hal_device->mac_intr_error_cnt = 0;
    return OAL_SUCC;
}
...
uint32_t hmac_tas_switch_result_notify_event_status(mac_vap_stru *pst_mac_vap, uint8_t uc_len, uint8_t *puc_param)
{
    dmac_tas_switch_status_notify_stru *tas_switch_ret;

    if ((pst_mac_vap->uc_init_flag == MAC_VAP_INVAILD) || (puc_param == NULL)) {
        oam_warning_log1(0, OAM_SF_ANY,
            "{hmac_tas_switch_result_notify_event_status::pst_mac_vap->uc_init_flag[%d], or puc_param null!}",
            pst_mac_vap->uc_init_flag);
        return OAL_ERR_CODE_PTR_NULL;
    }
    tas_switch_ret = (dmac_tas_switch_status_notify_stru *)puc_param;
    if (tas_switch_ret->ret_val == OAL_SUCC) {
        return OAL_SUCC;
    } else {
        oam_warning_log2(0, OAM_SF_ANY,
            "{hmac_tas_switch_result_notify_event_status::device tas switch fail, revert tas flag from[%d]to[%d]!}",
            g_tas_switch_status, tas_switch_ret->cur_tas_status);
        g_tas_switch_status = tas_switch_ret->cur_tas_status;
        return tas_switch_ret->ret_val;
    }
}
#endif
 ...
 *   修改内容  : 新生成函数
 */
uint32_t hmac_config_set_mlme(mac_vap_stru *mac_vap, uint16_t len, uint8_t *param)
{
    ...
    oal_w2h_mlme_ie_stru *mlme_ie = (oal_w2h_mlme_ie_stru *)param;
    if ((mlme_ie->en_mlme_type != OAL_IEEE80211_MLME_ASSOC_RSP) &&
    ...
    if (ret != OAL_SUCC) {
        ...
        hmac_mgmt_send_deauth_frame(mac_vap->uc_vap_id, mlme_ie->auc_macaddr, MAC_ASOC_NOT_AUTH, OAL_FALSE);
        return ret;
    }
...
/*
 * 函 数 名  : hmac_print_pkt_filter_program
 * 功能描述  : 打印pkt_filter program到sdt
 * 1.日    期  : 2018年3月17日
 *   作    者  : wifi
 *   修改内容  : 新生成函数
 */
void hmac_print_pkt_filter_program(uint8_t *puc_program, uint32_t program_len)
{
    ...
    if (pc_print_buff == NULL) {
        oam_error_log0(0, OAM_SF_CFG, "{hmac_print_pkt_filter_program::pc_print_buff null.}");
        return;
    }
    ...
    if (l_string_tmp_len < 0) {
        oam_error_log1(0, OAM_SF_CFG, "hmac_print_pkt_filter_program:sprintf return error[%d]", l_string_tmp_len);
...
OAL_STATIC uint32_t hmac_send_pkt_filter_cmd_event(mac_vap_stru *mac_vap, oal_netbuf_stru *cmd_netbuf)
{
    uint32_t ret;
    frw_event_mem_stru *event_mem = NULL;
    ...
    ***************************************************************************/
    ...
    if (event_mem == NULL) {
        oam_error_log0(mac_vap->uc_vap_id, OAM_SF_CFG, "{hmac_send_pkt_filter_cmd_event::event_mem null.}");
        return OAL_ERR_CODE_PTR_NULL;
    }
    ...
    frw_event_hdr_init(&(hmac_to_dmac_ctx_event->st_event_hdr), FRW_EVENT_TYPE_WLAN_CTX,
        DMAC_WLAN_CTX_EVENT_SUB_TYPE_PKT_FILTER_CMD, sizeof(dmac_tx_event_stru), FRW_EVENT_PIPELINE_STAGE_1,
    ...
    if (ret != OAL_SUCC) {
        oam_error_log1(mac_vap->uc_vap_id, OAM_SF_CFG,
            "hmac_send_pkt_filter_cmd_event:dispatch_event fail[%d]!", ret);
    }
    frw_event_free_m(event_mem);
    return ret;
}
/*
 * 函 数 名  : hmac_config_pkt_filter_cmd
 * 功能描述  : 处理pkt_filter下发或上报program的命令
 * 1.日    期  : 2018年3月17日
 *   作    者  : wifi
 *   修改内容  : 新生成函数
 */
uint32_t hmac_config_pkt_filter_cmd(mac_vap_stru *mac_vap, uint16_t len, uint8_t *param)
{
    uint32_t ret;
    oal_netbuf_stru *cmd_netbuf = NULL;
    mac_pkt_filter_cmd_stru *pkt_filter_cmd = NULL;
    int32_t l_ret;

    if (oal_unlikely(oal_any_null_ptr2(mac_vap, param))) {
        oam_error_log0(0, OAM_SF_CFG, "{hmac_config_set_pkt_filter_program::mac_vap or param is null.}");
        return OAL_ERR_CODE_PTR_NULL;
    }

    pkt_filter_cmd = (mac_pkt_filter_cmd_stru *)param;

    /* 申请netbuf */
    cmd_netbuf = oal_mem_netbuf_alloc(OAL_NORMAL_NETBUF,
        sizeof(mac_pkt_filter_cmd_type_uint8) + pkt_filter_cmd->us_program_len, OAL_NETBUF_PRIORITY_MID);
    if (cmd_netbuf == NULL) {
        oam_error_log0(mac_vap->uc_vap_id, OAM_SF_ANY, "{hmac_config_set_pkt_filter_program::netbuf alloc null!}");
        return OAL_ERR_CODE_ALLOC_MEM_FAIL;
    }

    /* 拷贝命令结构体到netbuf */
    l_ret = memcpy_s(oal_netbuf_data(cmd_netbuf),
        sizeof(mac_pkt_filter_cmd_type_uint8) + pkt_filter_cmd->us_program_len,
        &pkt_filter_cmd->en_cmd_type, sizeof(mac_pkt_filter_cmd_type_uint8));
    oal_netbuf_put(cmd_netbuf, sizeof(mac_pkt_filter_cmd_type_uint8));

    if (pkt_filter_cmd->en_cmd_type == PKT_FILTER_SET_FILTER_CMD) {
        /* program内容拷贝到netbuf */
        l_ret += memcpy_s(oal_netbuf_data(cmd_netbuf) + sizeof(mac_pkt_filter_cmd_type_uint8),
            pkt_filter_cmd->us_program_len, pkt_filter_cmd->puc_program,
            pkt_filter_cmd->us_program_len);
        oal_netbuf_put(cmd_netbuf, pkt_filter_cmd->us_program_len);
        /* 打印到sdt */
        hmac_print_pkt_filter_program(pkt_filter_cmd->puc_program, pkt_filter_cmd->us_program_len);
    }
    if (l_ret != EOK) {
        oam_error_log0(0, OAM_SF_CFG, "hmac_config_pkt_filter_cmd::memcpy fail!");
        oal_netbuf_free(cmd_netbuf);
        return OAL_FAIL;
    }
    ret = hmac_send_pkt_filter_cmd_event(mac_vap, cmd_netbuf);
    oal_netbuf_free(cmd_netbuf);
    return ret;
}

/*
 * 函 数 名  : hmac_pkt_filter_program_report_event
 * 功能描述  : pkt_filter program上报事件处理
 * 1.日    期  : 2018年3月17日
 *   作    者  : wifi
 *   修改内容  : 新生成函数
 */
uint32_t hmac_pkt_filter_program_report_event(frw_event_mem_stru *pst_event_mem)
{
    frw_event_stru *pst_event;
    frw_event_hdr_stru *pst_event_hdr;
    dmac_pkt_filter_report_event_stru *pst_report_event;
    ...
    pst_report_event = (dmac_pkt_filter_report_event_stru *)pst_event->auc_event_data;

    pst_netbuf = (oal_netbuf_stru *)pst_report_event->p_program;
    if (!pst_netbuf) {
        oam_warning_log0(0, OAM_SF_CFG, "hmac_pkt_filter_program_report_event:netbuf is null");
        return OAL_FAIL;
    }
    puc_program = oal_netbuf_data(pst_netbuf);
    hmac_print_pkt_filter_program(oal_netbuf_data(pst_netbuf), oal_netbuf_len(pst_netbuf));
    ...
    oal_netbuf_stru *action_neighbor_req)
{
    ...
    if (data == NULL) {
        oam_error_log0(0, OAM_SF_ANY, "{hmac_config_encap_action_header: data null}");
        return;
    }
    /*************************************************************************/
 ...
 *   修改内容  : 新生成函数
 */
uint32_t hmac_config_send_neighbor_req(mac_vap_stru *mac_vap, uint16_t len, uint8_t *param)
{
    uint32_t ret = OAL_SUCC;
    oal_netbuf_stru *action_neighbor_req;
    ...
    uint16_t idx;
    uint8_t *data = NULL;
    mac_cfg_ssid_param_stru *ssid = (mac_cfg_ssid_param_stru *)param;
...
uint32_t hmac_config_set_dc_status(mac_vap_stru *pst_mac_vap, uint16_t us_len, uint8_t *puc_param)
{
    ...
    mac_device_stru *mac_device = mac_res_get_dev(pst_mac_vap->uc_device_id);
...
void hmac_config_del_p2p_ie(uint8_t *puc_ie, uint32_t *pul_ie_len)
{
    ...
    ies_info.frame_len = *pul_ie_len;
    puc_p2p_ie = mac_find_vendor_ie(MAC_WLAN_OUI_WFA, MAC_WLAN_OUI_TYPE_WFA_P2P, &ies_info);
...
#ifdef _PRE_WLAN_FEATURE_ML
uint32_t hmnac_config_tx_action_ml_reconfig_request(mac_vap_stru *mac_vap, uint16_t len, uint8_t *param)
{
    uint32_t ret;
    hmac_vap_stru *hmac_vap = NULL;
    mac_cfg_tx_ml_reconfig_param_stru *reconfig_para = (mac_cfg_tx_ml_reconfig_param_stru *)param;

    if (oal_unlikely(oal_any_null_ptr3(mac_vap, reconfig_para, mac_vap->pst_mib_info))) {
        oam_error_log0(0, OAM_SF_ANY, "{hmnac_config_tx_action_ml_reconfig_request::mac_vap or param null.}");
        return OAL_ERR_CODE_PTR_NULL;
    }
    oam_warning_log2(0, OAM_SF_ANY, "{hmnac_config_tx_action_ml_reconfig_request::link_id = %u, op_type = %u",
        reconfig_para->ap_link_id[0], reconfig_para->op_type[0]);

    hmac_vap = (hmac_vap_stru *)mac_res_get_hmac_vap(mac_vap->uc_vap_id);
    if (hmac_vap == NULL) {
        oam_error_log0(0, OAM_SF_ANY, "{hmnac_config_tx_action_ml_reconfig_request::hmac_vap is null.}");
        return OAL_ERR_CODE_PTR_NULL;
    }

    ret = hmac_tx_action_ml_reconfig_request_proc(mac_vap, reconfig_para);
    if (oal_unlikely(ret != OAL_SUCC)) {
        oam_error_log1(mac_vap->uc_vap_id, OAM_SF_CFG,
            "{hmnac_config_tx_action_ml_reconfig_request::send event fail[%u]}", ret);
        return ret;
    }
    return OAL_SUCC;
}

uint32_t hmac_config_ml_reconfig_remove_link(mac_vap_stru *mac_vap, uint16_t len, uint8_t *param)
{
    hmac_vap_stru *hmac_vap = NULL;
    hmac_vap_stru *delete_hmac_vap = NULL;
    hmac_mld_manage_stru *mld_vap_manage = NULL;
    mac_cfg_remove_ml_link_ap_param_stru *remove_link_param = NULL;
    uint16_t tbtt_num;
    ...
    if (hmac_vap == NULL || hmac_vap->mld_vap_link_info.mld_vap_manage == NULL) {
        oam_error_log0(0, OAM_SF_ASSOC, "{hmac_config_ml_reconfig_remove_link::hamc_vap or mld_vap_manage is null");
        return OAL_ERR_CODE_PTR_NULL;
    }
    mld_vap_manage = hmac_vap->mld_vap_link_info.mld_vap_manage;

    remove_link_param = (mac_cfg_remove_ml_link_ap_param_stru *)param;
    tbtt_num = remove_link_param->tbtt_num;
    delete_link_id = remove_link_param->delete_link_id;

    for (link_id = 0; link_id < WLAN_MAX_ML_LINK_NUM; link_id++) {
        delete_hmac_vap = mld_vap_manage->link_vap[link_id];
        if (delete_hmac_vap == NULL ||
            delete_hmac_vap->mld_vap_link_info.link_id != delete_link_id) {
            continue;
        }
        break;
    }
    ...
    return hmac_ml_reconfig_remove_link(mac_vap, tbtt_num, delete_hmac_vap);
}

uint32_t hmac_config_mld_assoc_mode(mac_vap_stru *mac_vap, uint16_t len, uint8_t *param)
{
    if (param[0] >= MLD_ASSOC_MODE_BUTT) {
        return OAL_FAIL;
    }

    g_mld_assoc_mode = param[0];

    return OAL_SUCC;
}
uint32_t hmac_config_str_ml_tx_mode_switch(mac_vap_stru *mac_vap, uint16_t len, uint8_t *param)
{
    uint32_t ret;

    if (param[0] >= WLAN_USER_MAX_USER_LIMIT || param[1] >= WLAN_TID_MAX_NUM) {
        return OAL_FAIL;
    }

    ret = hmac_config_send_event(mac_vap, WLAN_CFGID_SET_TX_RING_TABLE_DUP_EN, len, param);
    if (oal_unlikely(ret != OAL_SUCC)) {
        oam_error_log1(mac_vap->uc_vap_id, OAM_SF_ANY,
                       "{hmac_config_str_ml_tx_mode_switch::hmac_config_send_event failed[%d].}", ret);
        return ret;
    }
...
uint32_t hmac_config_mlsr_work_link_switch(mac_vap_stru *mac_vap, uint16_t len, uint8_t *param)
{
    hmac_vap_stru *hmac_vap = NULL;
    uint32_t ret = OAL_SUCC;
    ...
    if (hmac_vap == NULL) {
        oam_warning_log0(mac_vap->uc_vap_id, OAM_SF_SCAN,
                         "{hmac_config_mlsr_work_link_switch::pst_hmac_vap is null.}");
        return OAL_ERR_CODE_MAC_DEVICE_NULL;
    }
    if (hmac_vap_is_legacy_mld_mode(hmac_vap) == OAL_TRUE) {
        oam_warning_log0(mac_vap->uc_vap_id, OAM_SF_SCAN,
                         "{hmac_config_mlsr_work_link_switch::hmac_is_not_mld_vap.}");
        return OAL_FAIL;
    }
    if (hmac_vap->st_vap_base_info.en_vap_mode != WLAN_VAP_MODE_BSS_STA) {
        oam_warning_log0(mac_vap->uc_vap_id, OAM_SF_SCAN,
                         "{hmac_config_mlsr_work_link_switch::en_vap_mode is not STA.}");
        return OAL_FAIL;
    }

    ret = hmac_config_mlsr_link_switch(hmac_vap, param);
    if (oal_unlikely(ret != OAL_SUCC)) {
        oam_error_log1(mac_vap->uc_vap_id, OAM_SF_OPMODE,
                       "{hmac_config_mlsr_work_link_switch::hmac_config_send_event failed[%d].}", ret);
    }

    return ret;
}

uint32_t hmac_config_mld_tx_ttlm_request(mac_vap_stru *mac_vap, uint16_t len, uint8_t *param)
{
    uint32_t ret;
    mac_cfg_ttlm_frame_param *ttlm_req_param = NULL;
    hmac_user_stru            *hmac_user = NULL;

    if (oal_unlikely(oal_any_null_ptr3(mac_vap, param, mac_vap->pst_mib_info))) {
        oam_error_log0(0, OAM_SF_ANY, "{hmac_config_mld_ttlm_request::mac_vap or param null.}");
        return OAL_ERR_CODE_PTR_NULL;
    }
    if (len < sizeof(mac_cfg_ttlm_frame_param)) {
        oam_error_log1(mac_vap->uc_vap_id, OAM_SF_ANY, "{hmac_config_mld_ttlm_request::len[%u] not valid.}", len);
        return OAL_FAIL;
    }
    hmac_user = mac_res_get_hmac_user(mac_vap->us_assoc_vap_id);
    if (hmac_user == NULL) {
        oam_error_log0(mac_vap->uc_vap_id, OAM_SF_ANY, "{hmac_config_mld_ttlm_request::hmac_user is null.}");
        return OAL_ERR_CODE_PTR_NULL;
    }
    if (hmac_vap_check_ttlm_is_support(mac_vap) != OAL_TRUE) {
        oam_warning_log0(mac_vap->uc_vap_id, OAM_SF_ANY, "{hmac_config_mld_ttlm_request::vap is not surpported ttlm}");
        return OAL_SUCC;
    }
    if (hmac_user_check_ttlm_is_support(hmac_user) != OAL_TRUE) {
        oam_warning_log0(mac_vap->uc_vap_id, OAM_SF_ANY, "{hmac_config_mld_ttlm_request::user is not surpported ttlm}");
        return OAL_SUCC;
    }
    ttlm_req_param = (mac_cfg_ttlm_frame_param *)param;
    if (hmac_ttlm_req_param_check(hmac_user, ttlm_req_param) != OAL_SUCC) {
        oam_warning_log0(mac_vap->uc_vap_id, OAM_SF_ANY, "{hmac_config_mld_ttlm_request::ttlm request check failed}");
        return OAL_SUCC;
    }
    ret = hmac_mld_ttlm_request_process(mac_vap, hmac_user, ttlm_req_param);
    oam_warning_log1(mac_vap->uc_vap_id, OAM_SF_ANY, "{hmac_config_mld_ttlm_request::send ttlm request.ret=%u}", ret);

    return ret;
}

uint32_t hmac_config_mld_tx_ttlm_teardown(mac_vap_stru *mac_vap, uint16_t len, uint8_t *param)
{
    uint32_t ret;
    uint8_t vap_id;
    hmac_user_stru *hmac_user = NULL;
    if (oal_unlikely(oal_any_null_ptr3(mac_vap, param, mac_vap->pst_mib_info))) {
        oam_error_log0(0, OAM_SF_ANY, "{hmac_config_mld_tx_ttlm_teardown::mac_vap or param null.}");
        return OAL_ERR_CODE_PTR_NULL;
    }
    vap_id = mac_vap->uc_vap_id;
    if (len < sizeof(mac_cfg_ttlm_frame_param)) {
        oam_error_log1(mac_vap->uc_vap_id, OAM_SF_ANY, "{hmac_config_mld_tx_ttlm_teardown::len[%u] not valid.}", len);
        return OAL_FAIL;
    }
    hmac_user = mac_res_get_hmac_user(mac_vap->us_assoc_vap_id);
    if (hmac_user == NULL) {
        oam_error_log0(vap_id, OAM_SF_ANY, "{hmac_config_mld_tx_ttlm_teardown::hmac_user is null.}");
        return OAL_ERR_CODE_PTR_NULL;
    }
    if (hmac_vap_check_ttlm_is_support(mac_vap) != OAL_TRUE) {
        oam_warning_log0(vap_id, OAM_SF_ANY, "{hmac_config_mld_tx_ttlm_teardown::vap is not surpported ttlm}");
        return OAL_SUCC;
    }
    if (hmac_user_check_ttlm_is_support(hmac_user) != OAL_TRUE) {
        oam_warning_log0(vap_id, OAM_SF_ANY, "{hmac_config_mld_tx_ttlm_teardown::user is not surpported ttlm}");
        return OAL_SUCC;
    }
    ret = hmac_send_ttlm_teardown(mac_vap, hmac_user, param);
    if (ret == OAL_SUCC) {
        hmac_set_mld_user_default_ttlm_info(hmac_user);
        hmac_set_ttlm_status(hmac_user->mld_user_link_info.mld_user_manage, WLAN_TTLM_COMPLETE);
        if (hmac_ttlm_update_status(mac_vap, hmac_user) != OAL_SUCC) {
            oam_error_log0(0, OAM_SF_CFG, "{hmac_config_mld_tx_ttlm_teardown::update ttlm status failed.}");
            return OAL_FAIL;
        }
    }
    oam_warning_log1(vap_id, OAM_SF_ANY, "{hmac_config_mld_tx_ttlm_teardown::tx ttlm teardown. ret=%u.}", ret);
    return OAL_SUCC;
}
#endif

uint32_t hmac_config_set_lp_long_dtim(mac_vap_stru *mac_vap, uint16_t len, uint8_t *param)
{
    uint32_t ret;
    hmac_vap_stru *hmac_vap = NULL;

    hmac_vap = (hmac_vap_stru *)mac_res_get_hmac_vap(mac_vap->uc_vap_id);
    if (mac_is_secondary_sta(hmac_vap) == OAL_FALSE) {
        oam_warning_log0(mac_vap->uc_vap_id, OAM_SF_ANY, "{hmac_config_set_lp_long_dtim::not secondary sta}");
        return OAL_FAIL;
    }
    if (mac_vap->en_vap_state != MAC_VAP_STATE_UP) {
        return OAL_FAIL;
    }
    ret = hmac_config_send_event(mac_vap, WLAN_CFGID_SET_LP_LONG_DTIM, len, param);
    if (oal_unlikely(ret != OAL_SUCC)) {
        oam_error_log1(mac_vap->uc_vap_id, OAM_SF_ANY,
                       "{hmac_config_set_lp_long_dtim::hmac_config_send_event failed[%d].}", ret);
        return ret;
    }
...
#ifdef _PRE_WLAN_FEATURE_11AX
#define OMI_BW_SWITCH_2G_MASK 0x1
#define OMI_BW_SWITCH_5G_MASK 0x2
uint32_t hmac_config_set_omi_bw_switch(mac_vap_stru *mac_vap, uint16_t len, uint8_t *param)
{
    uint32_t ret;
    mac_user_stru *mac_user = NULL;
    uint32_t is_omi_open;
    wlan_bw_cap_enum_uint8 new_bw_cap;
    /* 检查输入参数， 异常则退出 */
    if (len < 2 * sizeof(uint8_t)) { /* 至少有2个入参 */
        return OAL_FAIL;
    }
    is_omi_open = param[0];    /* 第0个入参为开关 */
    new_bw_cap = param[1];     /* 第1个入参为带宽 */
    if (is_omi_open >= OAL_BUTT || new_bw_cap >= WLAN_BW_CAP_BUTT) {
        return OAL_FAIL;
    }

    /* 判断定制化能力是否支持切换带宽 */
    if (((mac_vap->st_channel.en_band == WLAN_BAND_2G) &&
        ((g_cust_cap.support_omi_bw_switch & OMI_BW_SWITCH_2G_MASK) == OAL_FALSE)) ||
        ((mac_vap->st_channel.en_band == WLAN_BAND_5G) &&
        ((g_cust_cap.support_omi_bw_switch & OMI_BW_SWITCH_5G_MASK) == OAL_FALSE))) {
        oam_warning_log2(0, OAM_SF_CFG, "{hmac_config_set_omi_bw_switch::en_band[%d],omi_bw_switch[%x]}",
            mac_vap->st_channel.en_band, g_cust_cap.support_omi_bw_switch);
        return OAL_FAIL;
    }
    ...
    /* 11ax omi支持才能配置下命令 */
    if (!is_legacy_sta(mac_vap) || (mac_vap->bit_htc_include_custom_switch == OAL_FALSE) ||
        (mac_vap->bit_om_include_custom_switch == OAL_FALSE) || (mac_vap_is_he_eht_protocol(mac_vap) == OAL_FALSE)) {
        oam_warning_log3(mac_vap->uc_vap_id, OAM_SF_ANY,
            "{hmac_config_set_omi_bw_switch::is_legacy_sta=[%d], vap_is_he_eht=[%d], vap_tx_om=[%d]}",
            is_legacy_sta(mac_vap), mac_vap_is_he_eht_protocol(mac_vap), mac_vap->bit_om_include_custom_switch);
        return OAL_FAIL;
    }

    mac_user = (mac_user_stru *)mac_res_get_mac_user(mac_vap->us_assoc_vap_id);
    if (mac_user == NULL) {
        oam_warning_log1(0, OAM_SF_TX, "{hmac_config_set_omi_bw_switch::mac_user[%d] null.", mac_vap->us_assoc_vap_id);
        return OAL_ERR_CODE_PTR_NULL;
    }
    /* 非hiex设备不进行命令 */
    if (mac_user_hiex_enabled(mac_user) == OAL_FALSE || mac_user->bit_tx_data_include_om == OAL_FALSE) {
        oam_warning_log2(mac_vap->uc_vap_id, OAM_SF_ANY,
            "{hmac_config_set_omi_bw_switch:: user_hiex_enable=[%d], user_tx_om=[%d]}",
            mac_user_hiex_enabled(mac_user), mac_user->bit_tx_data_include_om);
        return OAL_FAIL;
    }

    ret = hmac_config_send_event(mac_vap, WLAN_CFGID_SET_OMI_BW_SWITCH, len, param);
    if (oal_unlikely(ret != OAL_SUCC)) {
        oam_error_log1(mac_vap->uc_vap_id, OAM_SF_ANY,
                       "{hmac_config_set_omi_bw_switch::hmac_config_send_event failed[%d].}", ret);
        return ret;
    }
...
uint32_t hmac_config_set_station_ext_info_cdf_th(mac_vap_stru *mac_vap, uint16_t len, uint8_t *param)
{
    uint32_t ret;
    ret = hmac_config_send_event(mac_vap, WLAN_CFGID_STATION_INFO_EXT_CDF_TH, len, param);
    if (oal_unlikely(ret != OAL_SUCC)) {
        oam_error_log1(mac_vap->uc_vap_id, OAM_SF_ANY,
                       "{hmac_config_set_station_ext_info_cdf_th::hmac_config_send_event failed[%d].}", ret);
        return ret;
    }
    return OAL_SUCC;
}
