    ...
    ***************************************************************************/
    ...
    if (oal_unlikely(ret != OAL_SUCC)) {
        oam_warning_log1(pst_mac_vap->uc_vap_id, OAM_SF_M2S,
                         "{hmac_config_vap_m2s_info_syn::hmac_config_send_event failed[%d]].}", ret);
    }
...
OAL_INLINE void hmac_config_d2h_vap_m2s_info_syn_set_cap(mac_vap_stru *pst_mac_vap, mac_vap_m2s_stru *pst_syn_info)
{
    ...
    if (g_wlan_spec_cfg->feature_11be_is_open == OAL_TRUE) {
        mac_mib_set_EHTSUBeamformeeImplemented(pst_mac_vap->pst_mib_info,  pst_syn_info->en_su_bfee);
        mac_mib_set_EHTSUBeamformerImplemented(pst_mac_vap->pst_mib_info, pst_syn_info->en_he_su_bfer);
        mac_mib_set_EHTNumberSoundingDimensionsLessThanOrEqualTo80(pst_mac_vap->pst_mib_info,
            pst_syn_info->uc_he_num_dim_below_80m);
        mac_mib_set_EHTNumberSoundingDimensionsEqualTo160(pst_mac_vap->pst_mib_info,
            pst_syn_info->uc_he_num_dim_over_80m);
    }
#endif
}
...
uint32_t hmac_config_d2h_vap_m2s_info_syn(mac_vap_stru *pst_mac_vap, uint8_t uc_len, uint8_t *puc_param)
{
    ...
    if (pst_syn_info->en_m2s_type == WLAN_M2S_TYPE_HW) {
        /* 硬切换需要踢除所有主路上的用户 */
    ...
    /* 关键信息同步提示 */
    oam_warning_log3(pst_mac_vap->uc_vap_id, 0,
...
uint32_t hmac_config_mimo_compatibility(mac_vap_stru *pst_mac_vap, uint8_t uc_len, uint8_t *puc_param)
{
    ...
    if (pst_hmac_user->en_user_ap_type & MAC_AP_TYPE_MIMO_BLACKLIST) {
        /* 发起reassoc req */
        hmac_roam_start_fill_info(&roam_start_params, ROAM_SCAN_CHANNEL_ORG_0, OAL_FALSE, NULL, ROAM_TRIGGER_M2S);
        hmac_roam_start(pst_hmac_vap, &roam_start_params);
    }
...
uint32_t hmac_config_d2h_usr_lut_syn(mac_vap_stru *mac_vap, uint8_t len, uint8_t *p_param)
{
    ...
    if (!p_usr_ldx_info->valid) {
        hal_chip->user_sts_info[p_usr_ldx_info->lut_idx].user_valid = 0;
    } else {
        hal_chip->user_sts_info[p_usr_ldx_info->lut_idx].user_valid = 1;
        hal_chip->user_sts_info[p_usr_ldx_info->lut_idx].user_id = p_usr_ldx_info->usr_idx;
    }

    oam_warning_log3(mac_vap->uc_vap_id, 0, "hmac_config_d2h_usr_lut_syn:lut_idx[%d],usr_idx[%d],valid[%d]",
        p_usr_ldx_info->lut_idx, p_usr_ldx_info->usr_idx, p_usr_ldx_info->valid);

    return OAL_SUCC;
}

static void hmac_config_txbf_mib_update(mac_vap_stru *mac_vap, mac_d2h_mib_update_info_stru *mib_update_info)
{
...
#ifdef _PRE_WLAN_FEATURE_11BE
    if (g_wlan_spec_cfg->feature_11be_is_open == OAL_TRUE) {
        mac_mib_set_EHTSUBeamformeeImplemented(mac_vap->pst_mib_info, mib_update_info->en_su_bfee);
        mac_mib_set_EHTNumberSoundingDimensionsLessThanOrEqualTo80(mac_vap->pst_mib_info,
            mib_update_info->uc_he_num_dim_below_80m);
        mac_mib_set_EHTNumberSoundingDimensionsEqualTo160(mac_vap->pst_mib_info,
            mib_update_info->uc_he_num_dim_over_80m);
    }
#endif
}
...
static void hmac_config_vap_mib_update(mac_vap_stru *mac_vap, mac_d2h_mib_update_info_stru *mib_update_info)
{
    ...
    hmac_config_txbf_mib_update(mac_vap, mib_update_info);
...
uint32_t hmac_config_d2h_rx_mode_syn(mac_vap_stru *mac_vap, uint8_t len, uint8_t *p_param)
{
    ...
    if (hal_device->rx_alloc_list_inited == OAL_FALSE && rx_switch->new_rx_mode == HAL_RAM_RX) {
    ...
    hal_pm_try_wakeup_forbid_sleep(HAL_PM_FORBIDE_SLEEP_D2H_RX_MODE_SYN);
    if (rx_switch->new_rx_mode == HAL_DDR_RX) {
        ...
        if (hal_rx_host_start_dscr_queue(rx_switch->hal_device_id) != OAL_SUCC) {
            hal_pm_try_wakeup_allow_sleep(HAL_PM_FORBIDE_SLEEP_D2H_RX_MODE_SYN);
            return OAL_FAIL;
        }
...
uint32_t hmac_config_d2h_trigger_reassoc(mac_vap_stru *pst_mac_vap, uint8_t uc_len, uint8_t *puc_param)
{
    ...
    /* 发起reassoc req */
    hmac_roam_start_fill_info(&roam_start_params, ROAM_SCAN_CHANNEL_ORG_0, OAL_FALSE, NULL, ROAM_TRIGGER_D2H_REASSOC);
