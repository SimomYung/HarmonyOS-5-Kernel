...
void hmac_set_user_ampdu_tx_on(hmac_vap_stru *hmac_vap)
{
    ...
    oal_dlist_search_for_each_safe(entry, next_entry, &(hmac_vap->st_vap_base_info.st_mac_user_list_head)) {
        ...
        for (tid_idx = 0; tid_idx < WLAN_TID_MAX_NUM; tid_idx++) {
            tid_ba = hmac_get_tid_ba_info(hmac_user, tid_idx);
            if (tid_ba == NULL) {
                continue;
            }
            if (tid_ba->st_ba_tx_info.en_ba_status == DMAC_BA_COMPLETE) {
    ...
    mac_cfg_channel_param_stru *pst_prot_param)
{
    ...
    if ((pst_prot_param->en_band == WLAN_BAND_2G) && (pst_prot_param->en_bandwidth > WLAN_BAND_WIDTH_20M) &&
        (mac_mib_get_2GFortyMHzOperationImplemented(pst_mac_vap) == OAL_FALSE)) {
...
{
    if (pst_channel_param->en_bandwidth >= WLAN_BAND_WIDTH_160PLUSPLUSPLUS) {
...
#ifdef _PRE_WLAN_FEATURE_EMLSR
uint32_t hmac_config_ml_emlsr_enable(mac_vap_stru *mac_vap, uint16_t len, uint8_t *param)
{
    hmac_vap_stru *hmac_vap = NULL;
    ...
    if (hmac_vap == NULL) {
        oam_warning_log0(mac_vap->uc_vap_id, OAM_SF_SCAN,
                         "{hmac_config_ml_emlsr_enable::pst_hmac_vap is null.}");
        return OAL_ERR_CODE_MAC_DEVICE_NULL;
    }
    if (hmac_vap_is_legacy_mld_mode(hmac_vap) == OAL_TRUE) {
        oam_warning_log0(mac_vap->uc_vap_id, OAM_SF_SCAN,
                         "{hmac_config_ml_emlsr_enable::hmac_is_not_mld_vap.}");
        return OAL_FAIL;
    }
    if (hmac_vap->st_vap_base_info.en_vap_mode != WLAN_VAP_MODE_BSS_STA) {
        oam_warning_log0(mac_vap->uc_vap_id, OAM_SF_SCAN,
                         "{hmac_config_ml_emlsr_enable::en_vap_mode is not STA.}");
        return OAL_FAIL;
    }
    ...
    if (hmac_user == NULL) {
        oam_error_log0(hmac_vap->st_vap_base_info.uc_vap_id,
            OAM_SF_EMLSR, "{hmac_emlsr_tx_omn_action::dmac_user null.}");
        return OAL_ERR_CODE_PTR_NULL;
    }
    mld_user_manage = hmac_user->mld_user_link_info.mld_user_manage;

    emlsr_param = (mac_cfg_emlsr_action_param_stru *)param;
    /* 如果hipriv下发的emlsr参数与当前保存的不一致，则omn帧中需更新emlsr参数 */
    if ((mld_user_manage->eml_cap.emlsr_padding_delay != emlsr_param->emlsr_padding_delay) ||
        (mld_user_manage->eml_cap.emlsr_transition_delay != emlsr_param->emlsr_transition_delay)) {
        emlsr_param->emlsr_param_update_control = 1;
        mld_user_manage->eml_cap.emlsr_padding_delay = emlsr_param->emlsr_padding_delay;
        mld_user_manage->eml_cap.emlsr_transition_delay = emlsr_param->emlsr_transition_delay;
        oam_warning_log0(0, OAM_SF_EMLSR, "{hmac_emlsr_tx_omn_action::emlsr param update}");
    }
...
uint32_t hmac_config_ml_emlsr_cap(mac_vap_stru *mac_vap, uint16_t len, uint8_t *param)
{
    hmac_vap_stru *hmac_vap = NULL;
    hmac_mld_emlsr_cap_stru *emlsr_cap = NULL;

    emlsr_cap = (hmac_mld_emlsr_cap_stru *)param;
    hmac_vap = mac_res_get_hmac_vap(mac_vap->uc_vap_id);
    if (hmac_vap == NULL) {
        oam_warning_log0(mac_vap->uc_vap_id, OAM_SF_SCAN,
                         "{hmac_config_ml_emlsr_cap::hmac_vap is null.}");
        return OAL_ERR_CODE_MAC_DEVICE_NULL;
    }

    hmac_vap->mld_vap_link_info.emlsr_cap.emlsr_support = emlsr_cap->emlsr_support;
    hmac_vap->mld_vap_link_info.emlsr_cap.emlsr_padding_delay = emlsr_cap->emlsr_padding_delay;
    hmac_vap->mld_vap_link_info.emlsr_cap.emlsr_transition_delay = emlsr_cap->emlsr_transition_delay;
