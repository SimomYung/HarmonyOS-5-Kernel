...
uint32_t hmac_config_set_mcsbe(mac_vap_stru *mac_vap, uint16_t len, uint8_t *param)
{
    uint32_t ret;
    ...
    /***************************************************************************
        抛事件到DMAC层, 同步DMAC数据
    ***************************************************************************/
    ret = hmac_config_send_event(mac_vap, WLAN_CFGID_SET_MCSBE, len, param);
    if (oal_unlikely(ret != OAL_SUCC)) {
        oam_warning_log1(mac_vap->uc_vap_id, OAM_SF_CFG,
                         "{hmac_config_set_mcsbe::hmac_config_send_event failed[%d].}", ret);
    }
...
/* 功能：常发trx ring定制化开关check */
void hmac_altx_trx_switch_check(hal_device_always_tx_enum_uint8 al_tx_flag)
{
    if ((al_tx_flag == HAL_ALWAYS_TX_MPDU && g_trx_switch.host_ring_tx_switch != OAL_TRUE) ||
        ((al_tx_flag == HAL_ALWAYS_TX_RF || al_tx_flag == HAL_ALWAYS_TX_AMPDU_ENABLE) &&
        g_trx_switch.device_ring_tx_switch != OAL_TRUE)) {
        oam_error_log3(0, OAM_SF_CFG, "{hmac_altx_trx_switch_check::al_tx[%d] host ring[%d] dev ring[%d]}",
            al_tx_flag, g_trx_switch.host_ring_tx_switch, g_trx_switch.device_ring_tx_switch);
    }
}
/*
 ...
 *   修改内容  : 新生成函数
 */
uint32_t hmac_config_always_tx(mac_vap_stru *mac_vap, uint16_t len, uint8_t *param)
{
    uint8_t dev_id;
    uint32_t ret;
    hal_device_always_tx_enum_uint8 al_tx_flag = ((mac_cfg_tx_comp_stru *)param)->uc_param;
    hmac_vap_stru *hmac_vap = mac_res_get_hmac_vap(mac_vap->uc_vap_id);
    if (hmac_vap == NULL) {
        return OAL_ERR_CODE_PTR_NULL;
    }
    dev_id = hmac_vap->hal_dev_id;
#ifndef _PRE_LINUX_TEST
    /* 设置host常发状态 */
    hal_set_altx_status(dev_id, (al_tx_flag == HAL_ALWAYS_TX_DISABLE) ? OAL_FALSE : OAL_TRUE);
    dpe_wlan_device_al_trx_flag_sync((al_tx_flag == HAL_ALWAYS_TX_DISABLE) ? OAL_FALSE : OAL_TRUE);
#endif
    hmac_altx_trx_switch_check(al_tx_flag);
    /* 常发host ring tx处理，未使能host ring不处理 */
    if ((hmac_ring_tx_enabled() == OAL_TRUE) && al_tx_flag == HAL_ALWAYS_TX_MPDU) {
        ret = hmac_always_tx_proc(mac_vap, len, param, dev_id);
        if (ret != OAL_SUCC) {
            oam_warning_log1(mac_vap->uc_vap_id, OAM_SF_CFG,
                "{hmac_config_always_tx::hmac_always_tx_proc failed[%d].}", ret);
            return ret;
        }
    }
    /* 去使能且有包，执行释放动作 */
    if ((hmac_ring_tx_enabled() == OAL_TRUE) && (al_tx_flag == HAL_ALWAYS_TX_DISABLE) &&
        (oal_atomic_read(&g_always_tx_ring[dev_id].msdu_cnt) != 0)) {
        oam_warning_log2(mac_vap->uc_vap_id, OAM_SF_CFG,
            "{hmac_config_always_tx::al tx ring release before, msdu_cnt[%d] al tx mode[%d].}",
            oal_atomic_read(&g_always_tx_ring[dev_id].msdu_cnt), al_tx_flag);
        hmac_al_tx_ring_release(&g_always_tx_ring[dev_id]);
    }
    /***************************************************************************
        抛事件到DMAC层, 同步DMAC数据
    ***************************************************************************/
    ret = hmac_config_send_event(mac_vap, WLAN_CFGID_SET_ALWAYS_TX, len, param);
    if (oal_unlikely(ret != OAL_SUCC)) {
        oam_warning_log1(mac_vap->uc_vap_id, OAM_SF_CFG,
...
uint32_t hmac_config_phase_incr(mac_vap_stru *mac_vap, uint16_t len, uint8_t *param)
{
    uint32_t ret;
    ...
    /***************************************************************************
        抛事件到DMAC层, 同步DMAC数据
    ***************************************************************************/
    ret = hmac_config_send_event(mac_vap, WLAN_CFGID_SET_PHASE_INCR, len, param);
    if (oal_unlikely(ret != OAL_SUCC)) {
        oam_warning_log1(mac_vap->uc_vap_id, OAM_SF_CFG,
                         "{hmac_config_phase_incr::hmac_config_send_event failed[%d].}", ret);
    }
...
void hmac_config_show_fcs_info(mac_vap_stru *mac_vap)
{
    ...
    if ((hmac_rx_ring_switch_enabled() == OAL_TRUE)) {
        hal_host_get_rx_pckt_info(hmac_vap->hal_dev_id, &atcmdsrv_get_rx_pkcg_event);
        hal_host_al_rx_fcs_info(hmac_vap->hal_dev_id);
...
int32_t nb_netlink_send(uint8_t *data, oal_uint data_len)
{
    oal_netbuf_stru *skb;
...
#endif
    if (g_nb_handler_data.nlsk == NULL) {
...
uint32_t hmac_config_nb_info_report(mac_vap_stru *pst_mac_vap, uint8_t uc_len, uint8_t *puc_param)
{
    ...
    if (g_nb_handler_data.usepid == 0) {
        ...
        oam_warning_log2(pst_mac_vap->uc_vap_id, 0, "{hmac_config_autorate_level_report:rssi[%d],fail_num[%d].}",
 ...
 *   修改内容  : 新生成函数
 */
uint32_t hmac_config_always_rx(mac_vap_stru *mac_vap, uint16_t len, uint8_t *param)
{
    hmac_vap_stru *hmac_vap = mac_res_get_hmac_vap(mac_vap->uc_vap_id);
    uint8_t al_rx_flag = *(oal_bool_enum_uint8 *)param;
    uint32_t ret;
    ...
    /* 查找link vap并配置bssid为非0值 */
    ret = hmac_config_al_rx_set_link_vap_param(hmac_vap->pst_net_device, len, param);
    if (oal_unlikely(ret != OAL_SUCC)) {
        return ret;
    }

    /* 抛事件到DMAC层, 同步DMAC数据 */
    ret = hmac_config_send_event(mac_vap, WLAN_CFGID_SET_ALWAYS_RX, len, param);
    if (oal_unlikely(ret != OAL_SUCC)) {
    ...
    dpe_wlan_device_al_trx_flag_sync(al_rx_flag);
#endif
    return ret;
}
...
uint32_t hmac_config_roam_start(mac_vap_stru *pst_mac_vap, uint16_t us_len, uint8_t *puc_param)
{
    ...
    roam_start = (mac_cfg_set_roam_start_stru *)(puc_param);
    roam_info = (hmac_roam_info_stru *)pst_hmac_vap->pul_roam_info;
    if (roam_info == NULL) {
        return OAL_ERR_CODE_PTR_NULL;
    }
    roam_info->is_roaming_trigged_by_cmd = roam_start->is_roaming_trigged_by_cmd;
    if (ether_is_all_zero(roam_start->auc_bssid)) {
        /* reassociation or roaming */
        hmac_roam_start_fill_info(&roam_start_params, (roam_channel_org_enum)roam_start->uc_scan_type,
            roam_start->en_current_bss_ignore, NULL, ROAM_TRIGGER_APP);
    } else if (!oal_memcmp(pst_mac_vap->auc_bssid, roam_start->auc_bssid, OAL_MAC_ADDR_LEN)) {
        /* reassociation */
        hmac_roam_start_fill_info(&roam_start_params, ROAM_SCAN_CHANNEL_ORG_0, OAL_FALSE, NULL, ROAM_TRIGGER_APP);
    } else {
        /* roaming for specified BSSID */
        hmac_roam_start_fill_info(&roam_start_params, ROAM_SCAN_CHANNEL_ORG_DBDC,
            OAL_TRUE, roam_start->auc_bssid, ROAM_TRIGGER_BSSID);
    }
    roam_start_params.cur_mld_link_bss_policy = roam_start->cur_mld_link_bss_policy;
    return hmac_roam_start(pst_hmac_vap, &roam_start_params);
}
...
void hmac_set_user_ampdu_tx_on(hmac_vap_stru *hmac_vap)
{
    ...
    oal_dlist_search_for_each_safe(entry, next_entry, &(hmac_vap->st_vap_base_info.st_mac_user_list_head)) {
        ...
        for (tid_idx = 0; tid_idx < WLAN_TID_MAX_NUM; tid_idx++) {
            hmac_tid = hmac_get_ba_info(hmac_user, tid_idx);
            if (hmac_tid->st_ba_tx_info.en_ba_status == DMAC_BA_COMPLETE) {
 ...
 *   修改内容  : 新生成函数
 */
uint32_t hmac_config_set_ampdu_tx_on(mac_vap_stru *mac_vap, uint16_t len, uint8_t *param)
{
    ...
    p_ampdu_tx_on = (mac_cfg_ampdu_tx_on_param_stru *)param;
    ...
    if ((uint8_t)(p_ampdu_tx_on->uc_aggr_tx_on & (~(BIT1 | BIT0))) == 0) {
        ...
        if (p_ampdu_tx_on->uc_aggr_tx_on & BIT1) {
            ...
            hmac_config_send_event(mac_vap, WLAN_CFGID_AMPDU_TX_ON, len, param);
        }
#endif
        return OAL_SUCC;
    }
...
uint32_t g_ps_disable_cmd_vap_bitmap = 0x0;
...
void hmac_set_ps_disable_cmd_vap_bitmap(mac_vap_stru *mac_vap, uint8_t ps_enable)
{
    /* 打开powersaving 则清零该vap bitmap,关闭powersaving则该vap bitmap置1 */
    if (ps_enable == OAL_TRUE) {
        g_ps_disable_cmd_vap_bitmap &= ~BIT(mac_vap->uc_vap_id);
    } else {
        g_ps_disable_cmd_vap_bitmap |= BIT(mac_vap->uc_vap_id);
    }
}

void hmac_down_vap_update_ps_cmd_bitmap(mac_vap_stru *mac_vap)
{
    mac_vap_stru *up_mac_vap = NULL;
    uint8_t rx_listen_enable = OAL_TRUE;
    uint32_t ret;
    mac_device_stru *mac_device = mac_res_get_mac_dev();
    /* 所有vap都开ps cmd, 无操作 */
    if (g_ps_disable_cmd_vap_bitmap == 0) {
        return;
    }
    hmac_set_ps_disable_cmd_vap_bitmap(mac_vap, OAL_TRUE);
    /* 清完该vap后ps cmd为0说明只有该vap关powersaving cmd,需要切状态机 */
    if (g_ps_disable_cmd_vap_bitmap == 0) {
        /* 如果有up的vap用其进行状态机切换，否则等vap up时会自动配置 */
        ret = mac_device_find_up_vap(mac_device, &up_mac_vap);
        if ((ret != OAL_SUCC) || (up_mac_vap == NULL)) {
            oam_warning_log0(0, 0, "hmac_down_vap_update_ps_cmd_bitmap mac_device_find_up_vap::failed");
            return;
        }
        oam_warning_log2(0, 0, "hmac_down_vap_update_ps_cmd_bitmap::en[%d] bitmap[%d]",
            rx_listen_enable, g_ps_disable_cmd_vap_bitmap);
        hmac_config_sync_cmd_common(up_mac_vap, WLAN_CFGID_RX_LISTEN_PS_SWITCH, sizeof(uint8_t),
    ...
    uint8_t *puc_param)
{
    ...
    if (g_wlan_spec_cfg->rx_listen_ps_is_open) {
        ...
        hmac_set_ps_disable_cmd_vap_bitmap(pst_mac_vap, rx_listen_ps_switch->rx_listen_enable);
 ...
 *   修改内容  : 新生成函数
 */
uint32_t hmac_config_sync_set_sta_pm_on(mac_vap_stru *mac_vap, mac_cfg_ps_mode_param_stru *ps_mode_param,
    uint16_t len, uint8_t *param)
{
    uint32_t ret;
    mac_cfg_ps_open_stru *pst_sta_pm_open = (mac_cfg_ps_open_stru *)param;
    oam_warning_log2(mac_vap->uc_vap_id, OAM_SF_PWR, "hmac_config_set_sta_pm_on,enable[%d], ps_mode[%d]",
                     pst_sta_pm_open->uc_pm_enable, ps_mode_param->uc_vap_ps_mode);
    /* 先下发设置低功耗模式 */
    ret = hmac_config_sync_cmd_common(mac_vap, WLAN_CFGID_SET_PS_MODE, sizeof(mac_cfg_ps_mode_param_stru),
        (uint8_t *)ps_mode_param);
    if (ret != OAL_SUCC) {
        return ret;
    }
    /* 再下发打开低功耗 */
    return hmac_config_sync_cmd_common(mac_vap, WLAN_CFGID_SET_STA_PM_ON, len, param);
}
/*
 ...
 *   修改内容  : 新生成函数
 */
uint32_t hmac_config_set_sta_pm_on(mac_vap_stru *mac_vap, uint16_t len, uint8_t *param)
{
    ...
    if (pst_sta_pm_open->uc_pm_enable == MAC_STA_PM_MANUAL_MODE_ON) {
        hmac_vap->uc_cfg_sta_pm_manual = OAL_TRUE;
    ...
    st_ps_mode_param.uc_vap_ps_mode = pst_sta_pm_open->uc_pm_enable ?
        ((hmac_vap->uc_cfg_sta_pm_manual != OAL_FALSE) ? MAX_PSPOLL_PS : hmac_vap->uc_ps_mode) : NO_POWERSAVE;

    /* 低功耗退出host侧处理 */
    if (pst_sta_pm_open->uc_pm_enable == MAC_STA_PM_SWITCH_OFF) {
        hmac_fastsleep_stop(mac_vap);
        hmac_vap_state_change_notify_feature(hmac_vap, NULL, COFEATURE_PM_ENABLE_EVENT, COFEATURE_EXIT);
    } else {
        hmac_vap_state_change_notify_feature(hmac_vap, NULL, COFEATURE_PM_ENABLE_EVENT, COFEATURE_ENTER);
    }

    if (hmac_vap_is_legacy_mld_mode(hmac_vap) == OAL_TRUE) {
        return hmac_config_sync_set_sta_pm_on(mac_vap, &st_ps_mode_param, len, param);
    }
#ifdef _PRE_WLAN_FEATURE_ML
    return hmac_config_set_mld_sta_pm_on(hmac_vap, &st_ps_mode_param, pst_sta_pm_open->uc_pm_enable, len, param);
#else
    return OAL_SUCC;
#endif
}
...
{
    if ((en_band_config == WLAN_BAND_5G) && (en_band_device == WLAN_BAND_CAP_2G)) {
        /* 设置5G频带，但device不支持5G */
    ...
    } else if ((en_band_config == WLAN_BAND_2G) && (en_band_device == WLAN_BAND_CAP_5G)) {
        /* 设置2G频带，但device不支持2G */
        oam_warning_log2(0, OAM_SF_CFG,
    ...
    switch (protocol) {
        ...
        case WLAN_HE_MODE:
            if (g_wlan_spec_cfg->feature_11ax_is_open == OAL_FALSE) {
                oam_error_log0(0, OAM_SF_CFG, "{hmac_config_check_dev_protocol_cap_para:: \
                    protocol does not in the list.}");
                break;
            }
...
#ifdef _PRE_WLAN_FEATURE_11BE
        case WLAN_EHT_MODE:
            if (g_wlan_spec_cfg->feature_11be_is_open == OAL_FALSE) {
                oam_error_log0(0, OAM_SF_CFG, "{hmac_config_check_dev_protocol_cap_para:: \
                    protocol does not in the list.}");
                break;
            }

            if (pst_mac_device->en_protocol_cap < WLAN_PROTOCOL_CAP_EHT) {
                /* 设置11be协议，但device不支持EHT模式 */
                oam_warning_log2(0, OAM_SF_CFG,
                    "{hmac_config_check_dev_protocol_cap_para::not support EHT mode,protocol=%d protocol_cap=%d.}",
                    protocol, pst_mac_device->en_protocol_cap);
                return OAL_ERR_CODE_CONFIG_EXCEED_SPEC;
            }
            break;
                                                        ...
                                                        wlan_channel_bandwidth_enum_uint8 en_bw_config)
{
    ...
    switch (en_bw_device) {
        ...
        case WLAN_BAND_WIDTH_40PLUS:
            if ((en_bw_config == WLAN_BAND_WIDTH_80PLUSPLUS) || (en_bw_config == WLAN_BAND_WIDTH_80PLUSMINUS)) {
                return OAL_SUCC;
            }
            break;
        case WLAN_BAND_WIDTH_40MINUS:
            if ((en_bw_config == WLAN_BAND_WIDTH_80MINUSPLUS) || (en_bw_config == WLAN_BAND_WIDTH_80MINUSMINUS)) {
                return OAL_SUCC;
            }
            break;
 ...
 *   修改内容  : 新生成函数
 */
uint32_t hmac_config_set_mode(mac_vap_stru *mac_vap, uint16_t len, uint8_t *param)
{
...
uint32_t hmac_config_set_channel_check_param(mac_vap_stru *pst_mac_vap, mac_cfg_channel_param_stru *pst_prot_param)
{
    ...
    /* 检查40M能力 */
    if ((pst_prot_param->en_bandwidth >= WLAN_BAND_WIDTH_40PLUS) &&
        (mac_mib_get_dot11VapMaxBandWidth(pst_mac_vap) < WLAN_BW_CAP_40M)) {
        /* 设置40M带宽，但device能力不支持40M，返回错误码 */
        oam_warning_log2(pst_mac_vap->uc_vap_id, OAM_SF_CFG, "{hmac_config_set_channel_check_param::\
            .not support 40MHz bandwidth, en_protocol=%d en_dot11VapMaxBandWidth=%d.}",
            pst_prot_param->en_bandwidth, mac_mib_get_dot11VapMaxBandWidth(pst_mac_vap));
        return OAL_ERR_CODE_CONFIG_BW_EXCEED;
    }
    ...
    if ((pst_prot_param->en_band == WLAN_BAND_2G) && (pst_prot_param->en_bandwidth > WLAN_BAND_WIDTH_20M) &&
        (mac_mib_get_dot11VapMaxBandWidth(pst_mac_vap) == WLAN_BW_CAP_20M)) {
        return OAL_ERR_CODE_CONFIG_UNSUPPORT;
    }

    /* 不支持160M，但是上层下发160M带宽 */
    if (hmac_set_channel_160m_invalid(pst_mac_vap, pst_prot_param) == OAL_TRUE) {
        return OAL_ERR_CODE_CONFIG_BW_EXCEED;
    }
...
#ifdef _PRE_WLAN_FEATURE_DFS
    /* 检测到雷达波的信道会标记成block状态，如果覆盖到直接返回 */
    if (hmac_check_channel_block_status(mac_vap, &set_mac_channel) == OAL_TRUE) {
        return OAL_FAIL;
    }
#endif
    ...
    mac_cfg_channel_param_stru *channel_param, oal_bool_enum_uint8 *en_set_reg)
{
    ...
    oam_warning_log3(mac_vap->uc_vap_id, OAM_SF_MLOG, "hmac_config_set_channel: channel_num:%d, bw:%d, band:%d",
    ...
    uint8_t go_channel, wlan_channel_bandwidth_enum_uint8 *p_bandwidth)
{
    ...
    /* 只有GO 且 160M 带宽时才处理 */
    if (mac_vap_go_can_not_in_160m_check(p_go_mac_vap, go_channel) == OAL_TRUE) {
 ...
 *   修改内容  : 新生成函数
 */
uint32_t hmac_config_set_channel(mac_vap_stru *mac_vap, uint16_t len, uint8_t *param)
{
    uint32_t ret, up_vap_cnt;
    oal_bool_enum_uint8 en_set_reg = OAL_FALSE;
    oal_bool_enum_uint8 en_override = OAL_FALSE;

    mac_cfg_channel_param_stru *channel_param = (mac_cfg_channel_param_stru *)param;
    ...
    ***************************************************************************/
    if (en_set_reg == OAL_TRUE) {
        ...
        ret = hmac_config_send_event(mac_vap, WLAN_CFGID_CFG80211_SET_CHANNEL, len, param);
        if (oal_unlikely(ret != OAL_SUCC)) {
...
#ifdef _PRE_WLAN_FEATURE_ML
    if (mac_mib_get_MultiLinkActivated(mac_vap->pst_mib_info) == OAL_TRUE) {
        /* 如果支持MULTI-LINK，APUT修改VAP模式为ML-LINK模式，并配置multi-link参数更新到device */
        hmac_vap_change_to_mld_process(hmac_vap, NULL);
    }
#endif
 ...
 *   修改内容  : 新生成函数
 */
uint32_t hmac_config_get_txpower(mac_vap_stru *mac_vap, uint16_t *len, uint8_t *param)
{
    *((int32_t *)param) = mac_vap->uc_tx_power;
    *len = sizeof(int32_t);
...
#ifdef _PRE_WLAN_FEATURE_EMLSR
uint32_t hmac_config_ml_emlsr_enable(mac_vap_stru *mac_vap, uint8_t *param)
{
    hmac_vap_stru *hmac_vap = NULL;
    ...
    mac_cfg_emlsr_action_param_stru *cfg_emlsr_param = NULL;

    emlsr_param = (mac_cfg_emlsr_action_param_stru *)param;
    hmac_vap = mac_res_get_hmac_vap(mac_vap->uc_vap_id);
    if (hmac_vap == NULL || hmac_vap->mld_vap_link_info.mld_vap_manage == NULL) {
        oam_error_log0(mac_vap->uc_vap_id, OAM_SF_SCAN, "{hmac_config_ml_emlsr_enable::hmac_vap is null.}");
        return OAL_ERR_CODE_MAC_DEVICE_NULL;
    }
    mld_vap_manage = hmac_vap->mld_vap_link_info.mld_vap_manage;

    if (is_sta(mac_vap) == OAL_FALSE || hmac_vap_is_legacy_mld_mode(hmac_vap) == OAL_TRUE) {
        return OAL_FAIL;
    }
    ...
    if (hmac_user == NULL) {
        oam_error_log0(mac_vap->uc_vap_id, OAM_SF_EMLSR, "{hmac_emlsr_tx_omn_action::dmac_user null.}");
        return OAL_ERR_CODE_PTR_NULL;
    }

    emlsr_param = (mac_cfg_emlsr_action_param_stru *)param;
    /* 如果下发的emlsr参数全为0，则使用默认参数 */
    if (emlsr_param->emlsr_padding_delay == 0 && emlsr_param->emlsr_transition_delay == 0) {
    ...
    mld_vap_manage = hmac_vap->mld_vap_link_info.mld_vap_manage;
    cfg_emlsr_param = &mld_vap_manage->emlsr_record_info.emlsr_param;
    cfg_emlsr_param->emlsr_mode = emlsr_param->emlsr_mode;
    cfg_emlsr_param->emlsr_pm = emlsr_param->emlsr_pm;

    /* 如果下发的emlsr参数与当前保存的不一致，则omn帧中需更新emlsr参数 */
    if ((mld_vap_manage->emlsr_cap.emlsr_padding_delay != emlsr_param->emlsr_padding_delay) ||
        (mld_vap_manage->emlsr_cap.emlsr_transition_delay != emlsr_param->emlsr_transition_delay)) {
        emlsr_param->emlsr_param_update_control = 1;
        /* 自身的emlsr参数应该存在mld vap manage下面，且参数更新的omn帧需要发送成功才更新参数？
           参数更新device会配置ram寄存器，如果发送失败怎么处理 */
        ...
        oam_warning_log4(mac_vap->uc_vap_id, OAM_SF_EMLSR,
            "{hmac_emlsr_tx_omn_action::emlsr param update, padding_delay[%d]->[%d] transiton_delay[%d]->[%d]}",
            mld_vap_manage->emlsr_cap.emlsr_padding_delay, emlsr_param->emlsr_padding_delay,
            mld_vap_manage->emlsr_cap.emlsr_transition_delay, emlsr_param->emlsr_transition_delay);
    }

    /* 为避免退出EMLSR时信号跳变，选择RSSI强的VAP作为继续工作的VAP */
    next_work_vap = (emlsr_param->emlsr_mode == OAL_FALSE) ?
        hmac_emlsr_exit_choose_strong_rssi_link(hmac_vap) : hmac_vap;

    oam_warning_log1(next_work_vap->st_vap_base_info.uc_vap_id, OAM_SF_EMLSR,
        "{hmac_emlsr_tx_omn_action::emlsr_mode[%d]}", emlsr_param->emlsr_mode);

    hmac_vap_state_change_notify_emlsr(next_work_vap, hmac_user, EMLSR_CFGVENDOR_EVENT, emlsr_param->emlsr_mode);
    return OAL_SUCC;
}

uint32_t hmac_config_ml_emlsr_cfg_link(mac_vap_stru *mac_vap, uint16_t len, uint8_t *param)
{
    uint32_t ret;
    /***************************************************************************
        抛事件到DMAC层, 同步DMAC数据
    ***************************************************************************/
    ret = hmac_config_send_event(mac_vap, WLAN_CFGID_EMLSR_CFG_LINK, len, param);
    if (oal_unlikely(ret != OAL_SUCC)) {
        oam_error_log1(mac_vap->uc_vap_id, OAM_SF_OPMODE,
                       "{hmac_config_ml_emlsr_cfg_link::hmac_config_send_event failed[%d].}", ret);
    }
    return ret;
}

uint32_t hmac_config_al_ml_emlsr_enable(mac_vap_stru *mac_vap, uint16_t len, uint8_t *param)
{
    hmac_vap_stru *hmac_vap = NULL;
    uint32_t ret;
    emlsr_al_ctl *emlsr_cfg = NULL;

    emlsr_cfg = (emlsr_al_ctl *)param;
    hmac_vap = mac_res_get_hmac_vap(mac_vap->uc_vap_id);
    if (hmac_vap == NULL) {
        oam_warning_log0(mac_vap->uc_vap_id, OAM_SF_SCAN,
                         "{hmac_config_al_ml_emlsr_enable::pst_hmac_vap is null.}");
        return OAL_ERR_CODE_MAC_DEVICE_NULL;
    }

    if (emlsr_cfg->emlsr_al_mode == OAL_TRUE) {
        /* emlsr初始化 */
        hmac_emlsr_ram_init(mac_vap);
    }

    /***************************************************************************
        抛事件到DMAC层, 同步DMAC数据
    ***************************************************************************/
    ret = hmac_config_send_event(mac_vap, WLAN_CFGID_AL_EMLSR_ENABLE, len, param);
    if (oal_unlikely(ret != OAL_SUCC)) {
        oam_error_log1(mac_vap->uc_vap_id, OAM_SF_OPMODE,
                       "{hmac_config_ml_emlsr_enable::hmac_config_send_event failed[%d].}", ret);
    }
    return ret;
}
uint32_t hmac_config_ml_set_emlsr_dscr(mac_vap_stru *mac_vap, uint16_t len, uint8_t *param)
{
    uint32_t ret;
    /***************************************************************************
        抛事件到DMAC层, 同步DMAC数据
    ***************************************************************************/
    ret = hmac_config_send_event(mac_vap, WLAN_CFGID_EMLSR_CFG_SET_DSCR, len, param);
    if (oal_unlikely(ret != OAL_SUCC)) {
        oam_error_log1(mac_vap->uc_vap_id, OAM_SF_OPMODE,
                       "{hmac_config_ml_set_emlsr_dscr::hmac_config_send_event failed[%d].}", ret);
    }
    return ret;
}
#endif
