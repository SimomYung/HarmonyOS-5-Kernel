...
void hmac_set_device_freq_mode(uint8_t device_enable)
{
    ...
    st_device_freq_type.uc_device_freq_enable = device_enable;
...
OAL_STATIC void hmac_config_set_dfs_debug_mode(mac_vap_stru *mac_vap, uint16_t len, uint8_t *param)
{
    ...
    dfsmode = (uint8_t)((mac_cfg_set_tlv_stru *)param)->value;
    if (dfsmode >= DFSMODE_BUTT) {
        oam_error_log1(mac_vap->uc_vap_id, OAM_SF_CFG, "hmac_config_set_dfs_debug_mode:invalid_mode[%d]", dfsmode);
        return;
    }
    /* 记录dfs_debug下发参数，如果开启debug，信道不会禁用30min */
    mac_device->st_dfs.st_dfs_info.dfsmode = dfsmode;
}
/*
 ...
 *   修改内容  : 新生成函数
 */
OAL_STATIC void hmac_config_set_tlv_cmd_2(mac_vap_stru *mac_vap, uint16_t len, uint8_t *param)
{
    mac_cfg_set_tlv_stru *pst_config_para = (mac_cfg_set_tlv_stru *)param;

    /* HOST需要处理的事件 */
    switch (pst_config_para->us_cfg_id) {
#ifdef _PRE_WLAN_FEATURE_M2S
        case WLAN_CFGID_MIMO_BLACKLIST:
            g_en_mimo_blacklist = (uint8_t)((mac_cfg_set_tlv_stru *)param)->value;
            break;
#endif
#if defined(_PRE_WLAN_FEATURE_HID2D) && defined(_PRE_WLAN_FEATURE_HID2D_PRESENTATION)
        case WLAN_CFGID_HID2D_PRESENTATION_MODE:
            hmac_config_set_hid2d_presentation_mode(mac_vap, len, param);
            break;
#endif
        case WLAN_CFGID_DATA_COLLECT:
            hmac_config_set_dada_collect_switch(mac_vap, len, param);
            break;
        ...
        case WLAN_CFGID_SET_DFS_MODE:
            hmac_config_set_dfs_debug_mode(mac_vap, len, param);
            break;
#endif
        default:
            break;
...
uint32_t hmac_config_amsdu_ampdu_switch(mac_vap_stru *pst_mac_vap, uint16_t us_len, uint8_t *puc_param)
{
    ...
    for (uc_index = 0; uc_index < mac_chip_get_vap_support_max_num_limit(); uc_index++) {
...
OAL_STATIC void hmac_config_set_trx_ring_mode(mac_cfg_set_str_stru *pst_set_cmd)
{
    ...
    oam_warning_log2(0, OAM_SF_CFG, "{hmac_config_set_trx_ring_mode::ring switch[%d] independent[%d]}",
