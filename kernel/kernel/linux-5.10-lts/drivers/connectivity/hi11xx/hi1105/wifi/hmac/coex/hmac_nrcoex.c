    ...
    oal_net_device_stru *pst_net_dev, wal_wifi_priv_cmd_stru *priv_cmd, int8_t *data, uint32_t len)
{
    ...
    if (oal_copy_to_user(priv_cmd->puc_buf, data, sizeof(mac_nrcoex_stat_stru)) != EOK) {
        oam_error_log0(0, OAM_SF_ANY,
            "wal_vendor_get_nrcoex_info:CMD_GET_NRCOEX_INFO Failed to copy ioctl_data to user !");
        return -OAL_EFAIL;
    }
    priv_cmd->used_len = sizeof(mac_nrcoex_stat_stru);
    return OAL_SUCC;
}
#endif
