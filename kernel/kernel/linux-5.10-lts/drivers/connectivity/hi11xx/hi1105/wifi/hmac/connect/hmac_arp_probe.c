...
OAL_STATIC oal_bool_enum_uint8  hmac_arp_probe_fail_process(hmac_vap_stru *pst_hmac_vap, hmac_user_stru *pst_hmac_user)
{
    ...
    if (pst_hmac_vap->st_vap_base_info.pwl_enable == OAL_FALSE) {
        pst_hmac_arp_probe->en_arp_probe_on = OAL_FALSE; /* PWL未使能，保持和原有逻辑一致 */
...
#ifdef _PRE_WLAN_FEATURE_PWL
    if ((pst_hmac_vap->st_vap_base_info.pwl_enable == OAL_TRUE) && (uc_arp_probe_type & HMAC_VAP_ARP_PROBE_TYPE_PWL)) {
        need_reassoc = OAL_TRUE;
    }
#endif
    ...
    if ((need_reassoc == OAL_TRUE) && (pst_hmac_user->c_rssi > WLAN_FAR_DISTANCE_RSSI)) {
        ...
        hmac_roam_start_fill_info(&roam_start_params, ROAM_SCAN_CHANNEL_ORG_0, OAL_FALSE,
...
OAL_STATIC uint32_t hmac_vap_unicast_pkt_get(hmac_vap_stru *hmac_vap)
{
#ifdef _PRE_WLAN_FEATURE_ML
    uint32_t cnt;
    hmac_vap_stru *link_vap = hmac_mld_get_another_vap(hmac_vap);
    if (link_vap != NULL) {
        cnt = dpe_wlan_vap_unicast_pkt_get(hmac_vap->st_vap_base_info.uc_vap_id) +
            dpe_wlan_vap_unicast_pkt_get(link_vap->st_vap_base_info.uc_vap_id);
    } else {
        cnt = dpe_wlan_vap_unicast_pkt_get(hmac_vap->st_vap_base_info.uc_vap_id);
    }
    return cnt;
#else
    return dpe_wlan_vap_unicast_pkt_get(hmac_vap->st_vap_base_info.uc_vap_id);
#endif
}

OAL_STATIC void hmac_vap_unicast_pkt_reset(hmac_vap_stru *hmac_vap)
{
#ifdef _PRE_WLAN_FEATURE_ML
    hmac_vap_stru *link_vap = hmac_mld_get_another_vap(hmac_vap);
    dpe_wlan_vap_unicast_pkt_reset(hmac_vap->st_vap_base_info.uc_vap_id);
    if (link_vap != NULL) {
        dpe_wlan_vap_unicast_pkt_reset(link_vap->st_vap_base_info.uc_vap_id);
    }
#else
    dpe_wlan_vap_unicast_pkt_reset(hmac_vap->st_vap_base_info.uc_vap_id);
#endif
...
OAL_STATIC uint32_t hmac_arp_probe_timeout(void *p_arg)
{
    ...
    pst_hmac_arp_probe = &(pst_hmac_vap->st_hmac_arp_probe);
    ui_val = hmac_vap_unicast_pkt_get(pst_hmac_vap);
    if (ui_val == 0) {
    ...
    hmac_vap_unicast_pkt_reset(pst_hmac_vap);
...
void hmac_arp_probe_destory(void *p_hmac_vap, void *p_hmac_user)
{
    ...
    pst_hmac_arp_probe->en_arp_probe_on          = OAL_FALSE;
    pst_hmac_arp_probe->uc_arp_probe_type        = 0;
    dpe_wlan_vap_unicast_pkt_reset(pst_hmac_vap->st_vap_base_info.uc_vap_id);
    pst_hmac_arp_probe->uc_rx_no_pkt_count       = 0;
