...
mac_blacklist_info_stru g_ast_blacklist[WLAN_VAP_MAX_NUM_LIMIT];
/*
 * 功能描述  : 判断mac地址是否为0
 * 1.日    期  : 2014年3月28日
 *   作    者  : wifi
...
OAL_STATIC void hmac_blacklist_update_delete_user(hmac_vap_stru *hmac_vap, hmac_user_stru *hmac_user)
{
    ...
    if (is_legacy_ap(mac_vap) == OAL_TRUE || mac_is_chba_mode(mac_vap) == OAL_TRUE) {
        hmac_user->assoc_err_code = MAC_DISAS_LV_SS;
        hmac_start_kick_user_timer(mac_vap, hmac_user);
        return;
    }
 ...
 *   修改内容  : 新生成函数
 */
OAL_STATIC void hmac_mac_filter_del_user(mac_vap_stru *mac_vap)
{
    uint32_t ret;
    hmac_vap_stru *hmac_vap = NULL;
    ...
    uint8_t mode = CS_BLACKLIST_MODE_NONE;

    hmac_vap = (hmac_vap_stru *)mac_res_get_hmac_vap(mac_vap->uc_vap_id);
    if (hmac_vap == NULL) {
        oam_error_log0(mac_vap->uc_vap_id, OAM_SF_ANY, "hmac_mac_filter_del_user::hmac_vap is null");
        return;
    }
    ...
    oal_dlist_search_for_each_safe(entry, dlist_tmp, &(mac_vap->st_mac_user_list_head))
    {
        ...
        hmac_blacklist_get_mode(mac_vap, &mode);
        /* 2014.6.30  在如上的hmac_blacklist_filter()中会 drop_counter++ 它不是实际的过滤数，所以--恢复 */
        if (mode == CS_BLACKLIST_MODE_BLACK) {  /* 只在黑名单做增减，白名单不做 */
...
void hmac_whitelist_check_device_user(uint8_t device_id)
{
    ...
    for (vap_index = 0; vap_index < mac_device->uc_vap_num; vap_index++) {
        ...
        hmac_mac_filter_del_user(mac_vap);
    }
}
 ...
 *   修改内容  : 新生成函数
 */
void hmac_mac_filter_check_all_user(mac_vap_stru *mac_vap)
{
    if ((mac_vap->en_vap_mode != WLAN_VAP_MODE_BSS_AP) && (mac_vap->en_vap_mode != WLAN_VAP_MODE_BSS_STA)) {
        oam_warning_log0(0, OAM_SF_ANY, "{hmac_mac_filter_check_all_user::not AP or STA mode.");
        return;
    }
    hmac_mac_filter_del_user(mac_vap);
}
...
uint32_t hmac_blacklist_add(mac_vap_stru *mac_vap, uint8_t *mac_addr, uint32_t aging_time)
{
    ...
    /* 黑白名单添加成功后刷新一把用户 */
    hmac_mac_filter_check_all_user(mac_vap);
...
uint32_t hmac_blacklist_get_mode(mac_vap_stru *mac_vap, uint8_t *mode)
{
    ...
    *mode = hmac_vap->pst_blacklist_info->uc_mode;
    return OAL_SUCC;
}
...
OAL_STATIC void hmac_show_autoblacklist_info(mac_autoblacklist_info_stru *autoblacklist_info)
{
    ...
    for (blacklist_index = 0; blacklist_index < WLAN_BLACKLIST_MAX; blacklist_index++) {
        ...
        oam_info_log3(0, OAM_SF_ANY,
            "{src mac=%02X:XX:XX:XX:%02X:%02X}", sa[MAC_ADDR_0], sa[MAC_ADDR_4], sa[MAC_ADDR_5]);
...
void hmac_blacklist_print_form(mac_vap_stru *mac_vap,  mac_blacklist_info_stru *blacklist_info, int8_t *print_buff,
    uint8_t buff_index_input)
{
    uint8_t blacklist_index;
    uint8_t *sa = NULL;
    mac_blacklist_stru *blacklist = NULL;
    uint8_t buff_index = buff_index_input;
...
void mac_blacklist_get_pointer(wlan_vap_mode_enum_uint8 vap_mode,
                               uint8_t chip_id, uint8_t dev_id,
                               uint8_t vap_id, mac_blacklist_info_stru **blacklist_info)
{
    if (vap_mode == WLAN_VAP_MODE_BSS_AP || vap_mode == WLAN_VAP_MODE_BSS_STA) {
        if (vap_id >= WLAN_VAP_MAX_NUM_LIMIT) {
            oam_error_log2(vap_id, OAM_SF_ANY,
                "{mac_blacklist_get_pointer::vap_mode=%d, vap index=%d is wrong.}", vap_mode, vap_id);
            return;
        }
    } else {
        oam_error_log1(vap_id, OAM_SF_ANY, "{mac_blacklist_get_pointer::vap_mode=%d is wrong.}", vap_mode);
        return;
    }

    *blacklist_info = &g_ast_blacklist[vap_id];
    g_ast_blacklist[vap_id].uc_blacklist_vap_index = vap_id;
    g_ast_blacklist[vap_id].uc_blacklist_device_index = dev_id;
}
