...
***********************************************************************************************************/
/* 切信道状态切换表 双mac mac_mvap_state_trans_enum */
static uint8_t g_chan_switch_state_trans_tbl_double_mac[MAC_MVAP_STATE_BUTT][MAC_MVAP_STATE_BUTT] = {
    [MAC_MVAP_STATE_DCHN] = {
...
/* 切信道状态切换表 单mac mac_mvap_state_trans_enum */
static uint8_t g_chan_switch_state_trans_tbl_single_mac[MAC_MVAP_STATE_BUTT][MAC_MVAP_STATE_BUTT] = {
    [MAC_MVAP_STATE_DCHN] = {
        MAC_MVAP_STATE_TRANS_DCHN_TO_DCHN, MAC_MVAP_STATE_TRANS_UNSUPPORTED,
        MAC_MVAP_STATE_TRANS_DCHN_TO_DBAC, MAC_MVAP_STATE_TRANS_UNSUPPORTED,
        MAC_MVAP_STATE_TRANS_DCHN_TO_SINGLE_VAP, MAC_MVAP_STATE_TRANS_UNSUPPORTED
    }, /* 同频同信道 -> 其他状态 */
    [MAC_MVAP_STATE_DBDC] = {
        MAC_MVAP_STATE_TRANS_UNSUPPORTED, MAC_MVAP_STATE_TRANS_UNSUPPORTED,
    ...
    [MAC_MVAP_STATE_DBAC] = {
        MAC_MVAP_STATE_TRANS_DBAC_TO_DCHN, MAC_MVAP_STATE_TRANS_UNSUPPORTED,
        MAC_MVAP_STATE_TRANS_DBAC_TO_DBAC, MAC_MVAP_STATE_TRANS_UNSUPPORTED,
        MAC_MVAP_STATE_TRANS_DBAC_TO_SINGLE_VAP, MAC_MVAP_STATE_TRANS_UNSUPPORTED
    }, /* dbac -> 其他状态 */
    [MAC_MVAP_STATE_DBDC_DBAC] = {
        MAC_MVAP_STATE_TRANS_UNSUPPORTED, MAC_MVAP_STATE_TRANS_UNSUPPORTED,
    ...
    [MAC_MVAP_STATE_SINGLE_VAP] = {
        MAC_MVAP_STATE_TRANS_SINGLE_VAP_TO_DCHN, MAC_MVAP_STATE_TRANS_UNSUPPORTED,
        MAC_MVAP_STATE_TRANS_SINGLE_VAP_TO_DBAC, MAC_MVAP_STATE_TRANS_UNSUPPORTED,
        MAC_MVAP_STATE_TRANS_SINGLE_VAP_TO_SINGLE_VAP, MAC_MVAP_STATE_TRANS_UNSUPPORTED
    }, /* 单vap状态 -> 单vap状态 */
    [MAC_MVAP_STATE_UNSUPPORTED] = {
        MAC_MVAP_STATE_TRANS_UNSUPPORTED, MAC_MVAP_STATE_TRANS_UNSUPPORTED,
 ...
 * 功能描述 : 计算2vap状态
 */
static uint8_t hmac_calc_2vap_new_state(mac_vap_stru **up_mac_vap, mac_vap_stru *mac_vap,
    mac_channel_stru *new_channel, uint32_t up_vap_num, hmac_channel_nums_stat_stru *channel_nums)
{
    ...
    if (channel_nums->channel_nums_2g == 1 && channel_nums->channel_nums_5g == 1) {
        return mac_chip_is_support_slave() == OAL_TRUE ? MAC_MVAP_STATE_DBDC : MAC_MVAP_STATE_DBAC; // [1][1]
    }
 ...
 * 功能描述 : 计算3vap状态
 */
static uint8_t hmac_calc_3vap_new_state(mac_vap_stru **up_mac_vap, mac_vap_stru *mac_vap,
    mac_channel_stru *new_channel, uint32_t up_vap_num, hmac_channel_nums_stat_stru *channel_nums)
{
    uint8_t channel_nums_total = channel_nums->channel_nums_2g + channel_nums->channel_nums_5g;
    if (channel_nums_total == 1) {
 ...
 * 功能描述 : 计算新的多vap状态
 */
static uint8_t hmac_calc_new_state(mac_vap_stru **up_mac_vap, mac_vap_stru *mac_vap,
    mac_channel_stru *new_channel, uint32_t up_vap_num, hmac_channel_nums_stat_stru *channel_nums)
{
    if (up_vap_num == 1) {
 ...
 * 功能描述 : 获取当前多vap状态
 */
static uint8_t hmac_get_mvap_current_state(mac_device_stru *mac_device, uint32_t up_vap_num)
{
 ...
 */
static uint8_t hmac_mvap_state_special_filter(uint8_t mvap_state, mac_vap_stru **up_mac_vap,
    mac_vap_stru *mac_vap, mac_channel_stru *new_channel, uint32_t up_vap_num)
{
    /* 如果切换信道后形成DBDC但是定制化不支持DBDC，则返回不支持 */
    if (g_wlan_priv_dbdc_radio_cap == OAL_FALSE) {
        if (mvap_state == MAC_MVAP_STATE_DBDC) {
            oam_warning_log0(0, OAM_SF_ANY, "{hmac_chan_switch_get_state_trans:not support dbdc, start dbac!}");
            return MAC_MVAP_STATE_DBAC;
        } else if (mvap_state == MAC_MVAP_STATE_DBDC_DBAC) {
            oam_warning_log0(0, OAM_SF_ANY, "{hmac_chan_switch_get_state_trans:not support dbdc+dbac!}");
            return MAC_MVAP_STATE_UNSUPPORTED;
        }
    }
#ifdef _PRE_WLAN_CHBA_MGMT
 ...
 * 功能描述 : 获取新的多vap状态
 */
static uint8_t hmac_get_mvap_new_state(mac_device_stru *mac_device, mac_vap_stru **up_mac_vap,
    mac_vap_stru *mac_vap, mac_channel_stru *new_channel, uint32_t up_vap_num)
{
    uint32_t idx;
    uint8_t new_state;
...
/* 功能描述:过滤不允许的状态切换 */
static uint8_t hmac_chan_mgmt_filter_invalid_state_trans(
    mac_mvap_chan_switch_type_enum chan_switch_type, uint8_t state_trans)
{
    /* 已被拦截的信道切换 */
...
static uint8_t hmac_mvap_channel_switch_state_trans(mac_vap_stru *mac_vap,
    uint8_t cur_mvap_state, uint8_t new_mvap_state,
    mac_mvap_chan_switch_type_enum chan_switch_type)
{
    uint8_t state_trans;
    if (mac_chip_is_support_slave() == OAL_TRUE) {
        state_trans = g_chan_switch_state_trans_tbl_double_mac[cur_mvap_state][new_mvap_state];
    } else {
        state_trans = g_chan_switch_state_trans_tbl_single_mac[cur_mvap_state][new_mvap_state];
    }
    ...
    oam_warning_log2(mac_vap->uc_vap_id, OAM_SF_ANY,
        "{hmac_mvap_channel_switch_state_trans:current_state[%d], new_state[%d]\
        [0:DCHN 1:DBDC 2:DBAC 3:DBDC_DBAC 4:SINGLE_VAP 5:UNSUPPORT]}",
        cur_mvap_state, new_mvap_state);

    oam_warning_log2(mac_vap->uc_vap_id, OAM_SF_ANY,
        "{switch[%d][0:roam 1:go/ap csa 2:chba csa 3:link_switch], state_trans[%d][0-24:TRANS_TYPE 25:TRANS_UNSUPPORT]",
 ...
 */
uint8_t hmac_chan_switch_get_state_trans(mac_vap_stru **up_mac_vap, mac_vap_stru *mac_vap,
    mac_channel_stru *new_channel, uint32_t up_vap_num, mac_mvap_chan_switch_type_enum chan_switch_type)
{
    uint8_t cur_vap_state;
    uint8_t new_vap_state;
    uint8_t state_trans;
    ...
    state_trans = hmac_mvap_channel_switch_state_trans(mac_vap, cur_vap_state, new_vap_state, chan_switch_type);

    return state_trans;
}

static uint32_t hmac_mvap_copy_trans_tbl(oal_netbuf_stru *netbuf)
{
    uint8_t *trans_tbl = NULL;
    uint8_t *buf = (uint8_t *)oal_netbuf_data(netbuf);
    uint8_t i;

    /* 拷贝二维数组 */
    for (i = 0; i < MAC_MVAP_STATE_BUTT; i++) {
        if (mac_chip_is_support_slave() == OAL_TRUE) {
            trans_tbl = g_chan_switch_state_trans_tbl_double_mac[i];
        } else {
            trans_tbl = g_chan_switch_state_trans_tbl_single_mac[i];
        }
        if (memcpy_s(buf + i * MAC_MVAP_STATE_BUTT, WLAN_SHORT_NETBUF_SIZE - i * MAC_MVAP_STATE_BUTT,
            trans_tbl, MAC_MVAP_STATE_BUTT) != EOK) {
            return OAL_FAIL;
        }
    }
    oal_netbuf_put(netbuf, MAC_MVAP_STATE_BUTT * MAC_MVAP_STATE_BUTT);
    return OAL_SUCC;
}

uint32_t hmac_mvap_send_trans_tbl(hmac_vap_stru *hmac_vap)
{
    ...
    uint8_t uc_vap_num;
    uint16_t event_len = (uint16_t)sizeof(dmac_tx_event_stru);
    uint32_t ret;
    ...
    uc_vap_num = hmac_get_chip_vap_num(mac_chip);
    if (uc_vap_num > 1) {
        return OAL_SUCC;
    }
    netbuf = oal_mem_netbuf_alloc(OAL_NORMAL_NETBUF, WLAN_SHORT_NETBUF_SIZE, OAL_NETBUF_PRIORITY_MID);
    if (netbuf == NULL) {
        oam_error_log0(mac_vap->uc_vap_id, OAM_SF_MULTI_VAP, "hmac_mvap_send_trans_tbl:netbuf_alloc_fail!");
        return OAL_FAIL;
    }
    if (hmac_mvap_copy_trans_tbl(netbuf) != OAL_SUCC) {
        oal_netbuf_free(netbuf);
        return OAL_FAIL;
    }

    event_mem = frw_event_alloc_m(event_len);
    if (event_mem == NULL) {
        oam_error_log0(mac_vap->uc_vap_id, OAM_SF_MULTI_VAP, "hmac_mvap_send_trans_tbl:event_alloc_fail!");
        oal_netbuf_free(netbuf);
    ...
    frw_event_hdr_init(&(hmac_to_dmac_ctx_event->st_event_hdr), FRW_EVENT_TYPE_WLAN_CTX,
        DMAC_WLAN_CTX_EVENT_SUB_TYPE_MVAP_TRANS_TBL, event_len, FRW_EVENT_PIPELINE_STAGE_1,
    ...
    if (ret != OAL_SUCC) {
        oam_warning_log1(mac_vap->uc_vap_id, OAM_SF_MULTI_VAP, "hmac_mvap_send_trans_tbl:dispatch_event fail[%d]", ret);
    }
 ...
 * 功能描述 : 入网，获取新的多vap状态
 */
static uint8_t hmac_connect_get_state_trans(mac_device_stru *mac_device, mac_vap_stru **up_mac_vap,
    mac_vap_stru *mac_vap, mac_channel_stru *new_channel, uint32_t up_vap_num)
{
    int32_t ret;
    uint8_t new_vap_state;
...
static oal_bool_enum_uint8 hmac_multi_vap_is_same_band(const mac_vap_stru *mac_vap, const mac_channel_stru *new_channel,
    mac_vap_stru * const up_mac_vap[WLAN_SERVICE_VAP_MAX_NUM_PER_DEVICE], uint32_t up_vap_num)
{
    uint32_t i;
    for (i = 0; i < up_vap_num; i++) {
        if (up_mac_vap[i]->st_channel.en_band == new_channel->en_band) {
            return OAL_TRUE;
        }
    }
    return OAL_FALSE;
...
oal_bool_enum_uint8 hmac_check_coex_channel_is_valid(mac_vap_stru *mac_vap, mac_channel_stru *set_mac_channel)
{
    ...
    /* 前两个vap组成dbac，不允许第三个vap与前面vap是同band;允许不同band，组成DBDC+DBAC; */
    /* 2代表找到2个up vap */
    if (mac_is_dbac_running(mac_device) && up_vap_num == 2 &&
        hmac_multi_vap_is_same_band(mac_vap, set_mac_channel, up_mac_vap, up_vap_num) == OAL_TRUE) {
        hmac_print_vap_info(up_mac_vap, up_vap_num, 2); /* 2:函数调用点 */
        oam_warning_log1(mac_vap->uc_vap_id, OAM_SF_CHAN,
            "{hmac_check_coex_channel_is_valid::not allow multi(>2) vap dbac. channel %d}",
...
static oal_bool_enum_uint8 hmac_check_ap_follow_channel_block(mac_vap_stru *mac_vap, mac_channel_stru *channel_param,
    uint8_t follow_channel, wlan_channel_bandwidth_enum_uint8 follow_bandwidth)
{
    mac_channel_stru mac_channel = {0};

    mac_channel.en_band = channel_param->en_band;
    mac_channel.ext6g_band = channel_param->ext6g_band;
    mac_channel.uc_chan_number = follow_channel;
    mac_channel.en_bandwidth = follow_bandwidth;

    if (mac_get_channel_idx_from_num(mac_channel.en_band, mac_channel.uc_chan_number,
        mac_channel.ext6g_band, &(mac_channel.uc_chan_idx)) != OAL_SUCC) {
        return OAL_FALSE;
    }

    if (hmac_check_channel_block_status(mac_vap, &mac_channel) == OAL_TRUE) {
        return OAL_TRUE;
    }
    ...
    mac_cfg_channel_param_stru *channel_param, mac_channel_stru *set_mac_channel)
{
    ...
    for (vap_idx = 0; vap_idx < same_band_vap_num; vap_idx++) {
        ...
        if (((is_legacy_ap(check_mac_vap) == OAL_TRUE) ||
            ...
            if (hmac_check_ap_follow_channel_block(check_mac_vap, set_mac_channel, follow_channel,
                follow_bandwidth) != OAL_TRUE) {
                /* 同步修改信道和带宽 */
...
void hmac_chan_do_sync(mac_vap_stru *pst_mac_vap, dmac_set_chan_stru *pst_set_chan)
{
    ...
    if (ret != OAL_SUCC) {
        oam_warning_log3(pst_mac_vap->uc_vap_id, OAM_SF_ANY, "{hmac_chan_do_sync:failed:band[%d]ch[%d]6g[%d]}",
            pst_mac_vap->st_channel.en_band, pst_set_chan->st_channel.uc_chan_number,
            pst_set_chan->st_channel.ext6g_band);
        return;
    }
    ...
    if (memcpy_s(frw_get_event_payload(pst_event_mem), sizeof(dmac_set_chan_stru),
        (uint8_t *)pst_set_chan, sizeof(dmac_set_chan_stru)) != EOK) {
                    ...
                    oal_bool_enum_uint8 en_switch_immediately)
{
    ...
    hmac_cali_send_work_channel_cali_data(mac_vap, &(st_set_chan.st_channel), CALI_DATA_NORMAL_JOIN_TYPE);
...
uint32_t hmac_report_channel_switch(hmac_vap_stru *hmac_vap, mac_channel_stru *channel)
{
    ...
    if (memcpy_s((uint8_t *)frw_get_event_payload(event_mem), sizeof(chan_switch),
        (uint8_t *)&chan_switch, sizeof(chan_switch)) != EOK) {
...
uint32_t hmac_chan_switch_to_new_chan_complete(frw_event_mem_stru *event_mem)
{
    ...
    if (oal_unlikely(hmac_vap == NULL)) {
        oam_error_log0(event->st_event_hdr.uc_vap_id, OAM_SF_2040, "{hmac_chan_switch_to_new_chan_complete:vap null!}");
        return OAL_ERR_CODE_PTR_NULL;
    }

    oam_warning_log0(event->st_event_hdr.uc_vap_id, OAM_SF_2040, "hmac_chan_switch_to_new_chan_complete");
    hmac_dump_chan(&(hmac_vap->st_vap_base_info), (uint8_t *)set_chan);
    ...
    if (ret != OAL_SUCC) {
        oam_warning_log1(hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_2040,
...
#ifdef _PRE_WLAN_FEATURE_HID2D
    /* 通知HiD2D ACS切换成功 */
    hmac_hid2d_acs_switch_completed(&(hmac_vap->st_vap_base_info));
#endif
    /* 如果正在进行长期同信道监听, csa至新信道后检查能否继续同信道监听, 如果不行则abor监听 */
    hmac_abort_long_same_chan_remain_on_chan(&hmac_vap->st_vap_base_info, OAL_TRUE,
        MAC_ABOTR_SAME_CHAN_REMAIN_ON_CHAN_CSA);
...
{
    /* 双mac band不同，不会是DBAC; 单mac band不同, 可以是DBAC */
...
/*
 * 查找link_down_vap是否在up_vap_array中，如果在up_vap_array中，则将link_down_vap从up_vap_array删除
 */
uint32_t hmac_sub_vap_from_up_vap_array(const mac_vap_stru *link_down_vap,
    mac_vap_stru *up_vap_array[WLAN_SERVICE_VAP_MAX_NUM_PER_DEVICE], uint32_t up_vap_num)
{
    uint32_t k = 0;
    uint32_t idx;
    for (idx = 0; idx < up_vap_num; idx++) {
        if (up_vap_array[idx]->uc_vap_id != link_down_vap->uc_vap_id) {
            /* vap在vap_array中,要从vap_array扣掉 */
            up_vap_array[k++] = up_vap_array[idx];
        }
    }
    for (idx = k; idx < WLAN_SERVICE_VAP_MAX_NUM_PER_DEVICE; idx++) {
        up_vap_array[idx] = NULL;
    }
    return k;
}

/*
 * 查找vap是否在up_vap_array中，如果不在up_vap中，则将mac_vap添加到up_vap_array中
 */
static uint32_t hmac_add_vap_to_up_vap_array(mac_vap_stru *mac_vap,
    mac_vap_stru *up_vap_array[WLAN_SERVICE_VAP_MAX_NUM_PER_DEVICE], uint32_t up_vap_num)
{
    uint32_t idx;
    for (idx = 0; idx < up_vap_num; idx++) {
    ...
    if (idx >= WLAN_SERVICE_VAP_MAX_NUM_PER_DEVICE) {
        oam_error_log1(0, OAM_SF_DBDC,
            "hmac_add_vap_to_up_vap_array::up_vap_array is full, num = %d. could not add vap to up_vap_array",
            idx);
        return up_vap_num;
    }

    /* vap不在vap_array中，将vap添加到vap_array中，并增加vap_num */
    up_vap_array[up_vap_num] = mac_vap;
    return up_vap_num + 1;
}

/* 功能描述：判断multi-link 场景是否支持link 切换
 * 入参描述：hmac_link_down_vap:需要link down 的vap；hmac_link_up_vap:需要link up 的vap
 * 返回值： OAL_TRUE:支持link 切换； OAL_FALSE:不支持link 切换
 */
oal_bool_enum_uint8 hmac_mld_link_switch_channel_valid(mac_vap_stru *link_down_vap, mac_vap_stru *link_up_vap)
{
    uint32_t up_vap_num;
    mac_vap_stru *up_vap_array[WLAN_SERVICE_VAP_MAX_NUM_PER_DEVICE] = {NULL};
    mac_device_stru *mac_device = mac_res_get_mac_dev();

    uint8_t cur_mvap_state;
    uint8_t new_mvap_state;
    uint8_t state_trans;
    ...
    if (up_vap_num >= WLAN_SERVICE_VAP_MAX_NUM_PER_DEVICE) {
        oam_error_log4(link_down_vap->uc_vap_id, OAM_SF_CHAN,
            "hmac_mld_link_switch_channel_valid:link_down_vap %d, channel %d;link_up_vap %d channel %d",
            link_down_vap->uc_vap_id, link_down_vap->st_channel.uc_chan_number,
            link_up_vap->uc_vap_id, link_up_vap->st_channel.uc_chan_number);
        hmac_print_vap_info(up_vap_array, WLAN_SERVICE_VAP_MAX_NUM_PER_DEVICE, 1);
        return OAL_FALSE;
    }
    /* 添加link_up_vap 到up_vap 数组中 */
    up_vap_num = hmac_add_vap_to_up_vap_array(link_up_vap, up_vap_array, up_vap_num);

    cur_mvap_state = hmac_get_mvap_current_state(mac_device, up_vap_num);
    new_mvap_state = hmac_get_mvap_new_state(mac_device, up_vap_array,
        link_up_vap, &(link_up_vap->st_channel), up_vap_num);
    state_trans = hmac_mvap_channel_switch_state_trans(link_down_vap, cur_mvap_state, new_mvap_state,
        MAC_MVAP_CHAN_SWITCH_LINK_SWITCH);

    return (state_trans != MAC_MVAP_STATE_TRANS_UNSUPPORTED) ? OAL_TRUE : OAL_FALSE;
}
