...
OAL_STATIC void hmac_dbac_ap_csa_handle(mac_vap_stru *csa_vap, mac_channel_stru *csa_channel)
{
    ...
    /* band不同且支持双mac，优先DBDC，否则不过滤 */
    if ((csa_channel->en_band != csa_vap->st_channel.en_band) && (mac_chip_is_support_slave() == OAL_TRUE)) {
    ...
    mac_vap_stru **csa_vap, mac_channel_stru *csa_channel)
{
    ...
    /* 异频时，AP不允许跟随, GO根据定制化决定是否跟随 */
    if (mac_vap0->st_channel.en_band != mac_vap1->st_channel.en_band) {
        if (is_legacy_ap(*csa_vap) == OAL_TRUE) {
            support_channel_follow = OAL_FALSE;
        } else if ((is_p2p_go(*csa_vap) == OAL_TRUE) &&
            ((g_optimized_feature_switch_bitmap & BIT(CUSTOM_OPTIMIZE_CSA_EXT)) == 0)) {
            return OAL_FALSE;
        }
    }
    ...
    oam_warning_log2((*csa_vap)->uc_vap_id, OAM_SF_DBAC, "hmac_dbac_get_channel_flow_info:vap_mode[%d] follow[%d]",
        hmac_parse_vap_mode_for_dbac(*csa_vap), support_channel_follow);
