                                 ...
                                 mac_vap_state_enum_uint8 en_new_vap_state)
{
    if (pst_hmac_vap->st_vap_base_info.en_vap_mode == WLAN_VAP_MODE_BSS_STA &&
        ...
        hmac_fsm_change_state_for_single_link(pst_hmac_vap, en_new_vap_state);
    }
}
 ...
 * 1.日    期  : 2013年6月19日
  *   修改内容  : 新生成函数
 */
void hmac_fsm_change_state_for_single_link(hmac_vap_stru *pst_hmac_vap, mac_vap_state_enum_uint8 en_vap_state)
{
    ...
    if ((en_vap_state == MAC_VAP_STATE_UP) &&
        ...
        oam_error_log0(pst_hmac_vap->st_vap_base_info.uc_vap_id, 0,
            "{hmac_fsm_change_state_for_single_link::coex up vaps not supp!.}");
    }
    ...
    if (ret != OAL_SUCC) {
        oam_error_log3(pst_hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_ASSOC,
            "{hmac_fsm_change_state_for_single_link::hmac_syn_vap_state failed[%d], old_state=%d, new_state=%d.}",
            ret, en_old_state, en_vap_state);
    }

    if (en_vap_state == MAC_VAP_STATE_UP ||
        (en_old_state == MAC_VAP_STATE_UP &&
        (en_vap_state == MAC_VAP_STATE_STA_FAKE_UP || en_vap_state == MAC_VAP_STATE_INIT))) {
        /* 更新link 切换标志：
         * （1）ap 创建/删除
         * （2）sta 关联/断联
         */
...
/*
 * 函 数 名  : hmac_sta_fsm_change_state
 * 功能描述  : STA入网流程，改变全部link状态机状态
 * 1.日    期  : 2023年11月23日
  *   修改内容  : 新生成函数
 */
void hmac_fsm_change_state(hmac_vap_stru *hmac_vap, uint8_t vap_state)
{
    ...
    if (hmac_vap == NULL) {
        oam_error_log0(0, OAM_SF_AUTH, "{hmac_fsm_change_state::hmac_vap is null!}");
        return;
    }

    /* AP 或 非mld模式的STA，走单一vap切换流程 */
    if (hmac_vap->st_vap_base_info.en_vap_mode == WLAN_VAP_MODE_BSS_AP ||
        hmac_vap_is_ml_mode(hmac_vap) == OAL_FALSE) {
        hmac_fsm_change_state_for_single_link(hmac_vap, vap_state);
        return;
    }
    ...
    if (mld_vap_manage == NULL) {
        oam_error_log0(0, OAM_SF_CFG, "{hmac_sta_fsm_change_state::get mld_vap_mgmt fail.");
        return;
    }
    ...
    for (link_id = 0; link_id < WLAN_MAX_ML_LINK_NUM; link_id++) {
        ...
        /* vap状态切换为FAKE_UP，当前vap去关联/不工作，mld mode需要清除，刷新为legacy */
        if (vap_state == MAC_VAP_STATE_STA_FAKE_UP) {
            hmac_vap_mld_mode_change(hmac_vap, MLD_CHANGE_EVENT_CHANGE_TO_LEGACY, 0);
        }

        /* 切换slave link的vap状态机 */
        hmac_fsm_change_state_for_single_link(link_hmac_vap, vap_state);
    }

    /* 切换master link的vap状态机 */
    hmac_fsm_change_state_for_single_link(hmac_vap, vap_state);
#endif
}
...
#endif
    if (is_not_p2p_listen_printf(mac_vap) == OAL_FALSE) {
        return OAL_SUCC;
    }
    if (mac_vap->en_vap_state == MAC_VAP_STATE_STA_LINK_DOWN) {
        return OAL_SUCC;
    }
    /* 避免ap up scan时刷大量栈信息 */
    if ((mac_vap->en_vap_mode == WLAN_VAP_MODE_BSS_AP) &&
        ((mac_vap->en_vap_state == MAC_VAP_STATE_INIT) || (mac_vap->en_vap_state == MAC_VAP_STATE_AP_WAIT_START))) {
        return OAL_SUCC;
    }
...
OAL_STATIC void hmac_fsm_init_input_scan_req(void)
{
    ...
    */
    ...
    g_pa_hmac_sta_fsm_func[MAC_VAP_STATE_STA_WAIT_MLD_SCAN][HMAC_FSM_INPUT_SCAN_REQ] = hmac_scan_proc_scan_req_event;
}
...
OAL_STATIC void hmac_fsm_init_sta(void)
{
    ...
    */
    ...
    g_pa_hmac_sta_fsm_func[MAC_VAP_STATE_STA_LINK_DOWN][HMAC_FSM_INPUT_ROAMING_START] =
        hmac_roam_pause_user_in_link_down;
