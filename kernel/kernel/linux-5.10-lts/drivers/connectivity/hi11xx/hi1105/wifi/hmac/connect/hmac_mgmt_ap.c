    ...
    hmac_auth_req_info_stru *auth_req_info)
{
    ...
    uint16_t auth_alg = auth_req_info->auth_type;
...
static void hmac_ap_up_rx_auth_req_to_host(hmac_vap_stru *hmac_vap, oal_netbuf_stru *auth_req, uint16_t us_auth_alg_num)
{
    ...
    if (hmac_parse_auth_req(mac_vap, auth_req, payload, payload_len, &auth_req_info) != OAL_SUCC) {
        return;
    }
    ...
    if (hmac_ap_up_rx_filter_sae_auth_by_len(payload, payload_len, &auth_req_info) == OAL_TRUE) {
        oam_warning_log4(mac_vap->uc_vap_id, OAM_SF_AUTH,
            "{auth recieve invalid frame, auth alg %d, seq %d, status %d, payload_len %d}",
            auth_req_info.auth_type, auth_req_info.auth_seq, mac_get_auth_status(payload, payload_len), payload_len);
        return;
    }

    ret = hmac_encap_auth_rsp_get_user_idx(mac_vap, &auth_req_info, &auth_resend, &user_idx);
    if (ret != OAL_SUCC) {
    ...
    if (hmac_ap_rx_filter_dup_auth_req(hmac_user, &auth_req_info, auth_resend) == OAL_TRUE) {
        return;
    }
    hmac_ap_set_user_auth_alg_num(user_idx, auth_req_info.auth_type);
...
uint32_t hmac_chba_ap_check_whitelist(hmac_vap_stru *hmac_vap, oal_netbuf_stru *auth_req)
{
    ...
    rx_info = (mac_rx_ctl_stru *)oal_netbuf_cb(auth_req);
    if (rx_info == NULL) {
        return OAL_FAIL;
    }
    ieee80211_hdr = (mac_ieee80211_frame_stru *)(uintptr_t)mac_get_rx_cb_mac_header_addr(rx_info);
    if (hmac_chba_whitelist_check(ieee80211_hdr->auc_address2, WLAN_MAC_ADDR_LEN) != OAL_SUCC) {
        return OAL_FAIL;
    }
    return OAL_SUCC;
}
...
OAL_STATIC void hmac_ap_rx_auth_req(hmac_vap_stru *hmac_vap, oal_netbuf_stru *pst_auth_req)
{
    ...
    oam_warning_log1(uc_vap_id, OAM_SF_MLOG, "{hmac_ap_rx_auth_req::rx_auth_alg_num=[%d]}", auth_alg_num);
    ...
    uint8_t *payload, uint32_t msg_len)
{
    ...
    /* 获取RSN和WPA IE信息 */
    puc_rsn_ie = mac_find_ie(MAC_EID_RSN, &ies_info);
    puc_wpa_ie = mac_find_vendor_ie(MAC_WLAN_OUI_MICROSOFT, MAC_OUITYPE_WPA, &ies_info);
 ...
 * 1.日    期  : 2013年7月9日
  *   修改内容  : 新生成函数
 */
OAL_STATIC void hmac_ap_up_update_asoc_entry_prot(mac_vap_stru *mac_vap, hmac_user_stru *hmac_user,
    uint8_t *payload, uint32_t msg_len)
{
    /* WMM */
...
#ifdef _PRE_WLAN_FEATURE_WUR
    hmac_process_wur_ie(mac_vap, hmac_user, payload, msg_len);
#endif
}
                            ...
                            mac_status_code_enum_uint16 *status_code)
{
    ...
    /*                                                                       */
    /*************************************************************************/
    puc_owe_dh = mac_find_ie_ext_ie(MAC_EID_EXTENSION, MAC_EID_EXT_OWE_DH_PARAM, &ies_info);
                                                     ...
                                                     hmac_user_stru *hmac_user)
{
    ...
    if ((puc_vendor_vht_ie != NULL) && (puc_vendor_vht_ie[1] >= MAC_WLAN_OUI_VENDOR_VHT_HEADER)) {
        ...
        ies_info.frame_ies = puc_vendor_vht_ie + vendor_vht_ie_offset;
        ies_info.frame_len = puc_vendor_vht_ie[1] - MAC_WLAN_OUI_VENDOR_VHT_HEADER;
        puc_ie_tmp = mac_find_ie(MAC_EID_VHT_CAP, &ies_info);
        if (puc_ie_tmp != NULL) {
...
OAL_STATIC oal_bool_enum hmac_ap_frame_len_check(uint8_t *mac_hdr, uint32_t msg_len)
{
    ...
    len_min = (mac_get_frame_type_and_subtype(mac_hdr) == (WLAN_FC0_SUBTYPE_REASSOC_REQ | WLAN_FC0_TYPE_MGT)) ?
    ...
    uint8_t *payload, uint32_t msg_len)
{
    ...
    ie_tmp = mac_find_ie(MAC_EID_VHT_CAP, &ies_info);
    if (ie_tmp != NULL) {
        hmac_proc_vht_cap_ie(mac_vap, hmac_user, ie_tmp);
#ifdef _PRE_WLAN_FEATURE_1024QAM
        ie_tmp = mac_find_vendor_ie(MAC_HUAWEI_VENDER_IE, MAC_HISI_1024QAM_IE, &ies_info);
        if (ie_tmp != NULL) {
    ...
    uint8_t *payload, uint32_t msg_len)
{
    ...
    ie_tmp = mac_find_ie_ext_ie(MAC_EID_HE, MAC_EID_EXT_HE_CAP, &ies_info);
    hmac_proc_he_cap_ie(mac_vap, hmac_user, ie_tmp);

#ifdef _PRE_WLAN_FEATURE_6G_EXTEND
    /* 适配HE 6 GHz cap IE解析 */
    if (mac_vap->st_channel.ext6g_band == OAL_TRUE) {
        ie_tmp = mac_find_ie_ext_ie(MAC_EID_HE, MAC_EID_EXT_HE_6GHZ_BAND_CAP, &ies_info);
        if (ie_tmp != NULL) {
...
OAL_STATIC uint16_t hmac_ap_up_update_protocol_cap(hmac_vap_stru *hmac_vap, hmac_user_stru *hmac_user,
    uint16_t cap_info, uint8_t *mac_hdr, uint8_t *payload, uint32_t msg_len)
{
    uint16_t ret;
    mac_vap_stru *mac_vap = &(hmac_vap->st_vap_base_info);
    mac_user_stru *mac_user = &(hmac_user->st_user_base_info);
    ...
    if (hmac_vap_check_ht_capabilities_ap(hmac_vap, payload, msg_len, hmac_user) != MAC_SUCCESSFUL_STATUSCODE) {
    ...
    /* 更新对应STA的协议能力 */
    hmac_ap_up_update_asoc_entry_prot(mac_vap, hmac_user, payload, msg_len);
    /* 更新QoS能力 */
    ...
    if (hmac_mgmt_update_vht_cap(mac_vap, hmac_user, hmac_vap, payload, msg_len) != OAL_SUCC) {
    ...
    if (g_wlan_spec_cfg->feature_11ax_is_open) {
        ...
        hmac_ap_up_update_he_cap(hmac_user, mac_vap, payload, msg_len);
    }
#endif
#ifdef _PRE_WLAN_FEATURE_11BE
    /* 更新 11be EHT Capabilities ie */
    hmac_process_eht_ie(mac_vap, hmac_user, &ies_info);
#endif

#ifdef _PRE_WLAN_FEATURE_ML
    /* 处理ML元素，并申请对应的资源 */
    hmac_ap_parse_assoc_req_basic_ml_ie(hmac_vap, hmac_user, payload, (uint16_t)msg_len);
#endif

    /* 检查接收到的ASOC REQ消息中的SECURITY参数.如出错,则返回对应的错误码 */
    mac_user_init_key(mac_user);
    ret = hmac_check_rsn_ap(mac_vap, mac_user, payload, msg_len);
    ...
    uint8_t *payload, uint32_t msg_len)
{
    ...
    ie = mac_find_ie(MAC_EID_SUPPCHAN, &ies_info);
    if (ie == NULL) {
        return;
    }
...
OAL_STATIC uint32_t hmac_ap_up_update_sta_user(hmac_vap_stru *hmac_vap, uint8_t *mac_hdr, uint8_t *payload,
    uint32_t msg_len, hmac_user_stru *hmac_user, mac_status_code_enum_uint16 *pstatus_code)
{
    ...
    ***************************************************************************/
    ...
    puc_ie_tmp = mac_find_ie(MAC_EID_WMM, &ies_info);
    hmac_user->en_wmm_switch = (puc_ie_tmp != NULL) ? OAL_FALSE : OAL_TRUE;
    /* 当前用户已关联 */
    ret = hmac_ie_proc_assoc_user_legacy_rate(&ies_info, hmac_user);
    if (ret != OAL_SUCC) {
    ...
    /* 处理空间流 */
    ret = hmac_mgmt_update_spatial_stream_cap(hmac_vap, hmac_user, mac_hdr, &ies_info);
    if (ret != OAL_SUCC) {
        *pstatus_code = MAC_UNSPEC_FAIL;
    }
                                ...
                                mac_status_code_enum_uint16 *pstatus_code)
{
    ...
    asoc_rsp_len = hmac_mgmt_encap_asoc_rsp_ap(hmac_vap, puc_sta_addr,
                                                            ...
                                                            uint16_t *user_idx)
{
    ...
    if (rslt != OAL_SUCC) {
        ...
        hmac_mgmt_send_deauth_frame(hmac_vap->st_vap_base_info.uc_vap_id, sta_addr, MAC_ASOC_NOT_AUTH, OAL_FALSE);
        return NULL;
    }
                              ...
                              hmac_user_stru *hmac_user, uint8_t *payload, uint32_t payload_len)
{
    ...
    len = ((mgmt_frm_type == WLAN_FC0_SUBTYPE_REASSOC_REQ) ?
           (MAC_CAP_INFO_LEN + MAC_LISTEN_INT_LEN + WLAN_MAC_ADDR_LEN) : (MAC_CAP_INFO_LEN + MAC_LISTEN_INT_LEN));
    ies_info.frame_ies = payload + len;
    ies_info.frame_len = payload_len - len;
    if (is_p2p_go(&hmac_vap->st_vap_base_info) &&
        (mac_find_vendor_ie(MAC_WLAN_OUI_WFA, MAC_WLAN_OUI_TYPE_WFA_P2P, &ies_info) == NULL)) {
    ...
    if (status_code != MAC_REJECT_TEMP) {
        if (hmac_vap->st_vap_base_info.pst_mib_info->st_wlan_mib_privacy.en_dot11RSNAMFPC == OAL_FALSE) {
            if (status_code == MAC_INVALID_AKMP_CIPHER) {
                hmac_mgmt_send_deauth_frame(hmac_vap->st_vap_base_info.uc_vap_id,
...
{
#ifdef _PRE_WLAN_FEATURE_PWL
    // PWL模式下需要改变user状态，关闭pwl使能恢复原逻辑
    if (((mac_vap->pwl_enable == OAL_TRUE) || (mac_mib_get_dot11RSNAMFPC(mac_vap) != OAL_TRUE)) &&
        (hmac_user->assoc_ap_up_tx_auth_req)) {
#else
    ...
    hmac_user_stru *hmac_user, mac_status_code_enum_uint16 *status_code)
{
    ...
    mac_user = &(hmac_user->st_user_base_info);
    if ((mac_user->en_user_asoc_state == MAC_USER_STATE_ASSOC) &&
        (mac_user->st_cap_info.bit_pmf_active == OAL_TRUE) &&
        (mac_user->st_user_tx_info.st_security.en_cipher_protocol_type != WLAN_80211_CIPHER_SUITE_NO_ENCRYP)) {
    ...
    mac_rx_ctl_stru *rx_info, mac_status_code_enum_uint16 *status_code)
{
    ...
    oam_warning_log4(mac_vap->uc_vap_id, OAM_SF_MLOG,
        "{hmac_ap_up_rx_asoc_req:: ASSOC_REQ rx : user[%d] mac:%02X:XX:XX:XX:%02X:%02X.}",
        hmac_user->st_user_base_info.us_assoc_id, sta_addr[0], sta_addr[MAC_ADDR_4], sta_addr[MAC_ADDR_5]);
    ...
    if (cur_status == MAC_SUCCESSFUL_STATUSCODE) {
        ...
        rslt = hmac_init_user_security_port(mac_vap, hmac_user);
        if (rslt != OAL_SUCC) {
            oam_error_log1(mac_vap->uc_vap_id, OAM_SF_ASSOC,
...
void hmac_ap_up_process_asoc_succ_report(hmac_vap_stru *hmac_vap, hmac_user_stru *hmac_user)
{
    if (hmac_user->puc_assoc_req_ie_buff == NULL) {
        oam_error_log1(hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_ANY,
            "{hmac_ap_up_process_asoc_succ_report:assoc req ie is NULL, len[%d]}", hmac_user->assoc_req_ie_len);
        return;
    }
...
#ifdef _PRE_WLAN_CHBA_MGMT
    hmac_chba_connect_succ_handler(hmac_vap, hmac_user, hmac_user->puc_assoc_req_ie_buff, hmac_user->assoc_req_ie_len);
#endif
...
#ifdef _PRE_WLAN_CHBA_MGMT
    /* chba等待assoc rsp发送完成后再上报连接成功, 避免上报连接成功后assoc rsp发送失败导致的异常  */
    ...
    mac_rx_ctl_stru *rx_info, mac_status_code_enum_uint16 status_code, uint16_t user_idx)
{
    ...
    if (status_code == MAC_SUCCESSFUL_STATUSCODE) {
#ifdef _PRE_WLAN_CHBA_MGMT
        hmac_chba_save_cur_user_asoc_state(hmac_vap, hmac_user, hmac_user->st_user_base_info.en_user_asoc_state);
#endif
    ...
    if (hmac_user->st_hmac_cap_info.bit_owe == OAL_TRUE) {
        ...
        if (status_code == MAC_SUCCESSFUL_STATUSCODE) {
            hmac_ap_up_process_asoc_succ(hmac_vap, hmac_user, payload, payload_len, mgmt_frm_type);
            oam_warning_log4(hmac_user->st_user_base_info.uc_vap_id, OAM_SF_MLOG,
                "{ASSOC_RSP tx,STA assoc AP SUCC! user_idx=%d.user mac:%02X:XX:XX:XX:%02X:%02X}",
                user_idx, sta_addr[MAC_ADDR_0], sta_addr[MAC_ADDR_4], sta_addr[MAC_ADDR_5]);
        } else {
            /* AP检测STA失败，将其删除 */
...
OAL_STATIC uint32_t hmac_ap_up_rx_asoc_req(hmac_vap_stru *hmac_vap, mac_rx_ctl_stru *rx_info)
{
    ...
    mac_get_address2(mac_hdr, sta_addr, WLAN_MAC_ADDR_LEN);
#ifdef _PRE_WLAN_CHBA_MGMT
    rslt = hmac_chba_conn_response_rx_assoc_proc(hmac_vap, rx_info, mac_hdr);
    if (rslt != OAL_SUCC) {
        return rslt;
    }
#endif
                                          ...
                                          uint32_t payload_len, oal_bool_enum_uint8 is_protected)
{
    ...
    oam_warning_log1(hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_MLOG, "{hmac_ap_up_rx_disasoc user_id[%d]}",
...
#ifdef _PRE_WLAN_FEATURE_WUR
static void hmac_action_category_wur(hmac_vap_stru *hmac_vap, hmac_user_stru *hmac_user, uint8_t *payload,
    uint16_t payload_len)
{
    switch (payload[MAC_ACTION_OFFSET_ACTION]) {
...
static void hmac_action_category_vendor(hmac_vap_stru *hmac_vap, oal_netbuf_stru *netbuf,
    ...
    oal_netbuf_stru *netbuf, oal_bool_enum_uint8 is_protected)
{
    ...
    switch (payload[MAC_ACTION_OFFSET_CATEGORY]) {
        ...
        case MAC_ACTION_CATEGORY_VENDOR:
            hmac_action_category_vendor(hmac_vap, netbuf, payload, payload_len);
            break;
        ...
        case MAC_ACTION_CATEGORY_WUR:
            hmac_action_category_wur(hmac_vap, hmac_user, payload, payload_len);
            break;
#endif
                                     ...
                                     oal_bool_enum_uint8 en_is_protected)
{
    ...
    if (hmac_get_frame_body_len(netbuf) < MAC_ACTION_CATEGORY_AND_CODE_LEN) {
        oam_warning_log1(0, OAM_SF_RX, "{ap_up::frame len too short[%d].}", rx_ctrl->us_frame_len);
        return;
    }
    /* 获取帧头信息 */
 ...
 *   修改内容  : 新生成函数
 */
uint32_t hmac_ap_up_rx_mgmt(hmac_vap_stru *hmac_vap, void *param)
{
    ...
    pst_mgmt_rx_event = (dmac_wlan_crx_event_stru *)param;
    pst_rx_info = (mac_rx_ctl_stru *)oal_netbuf_cb(pst_mgmt_rx_event->pst_netbuf);
    if (pst_rx_info == NULL) {
        return OAL_ERR_CODE_PTR_NULL;
    }
    mac_hdr = (uint8_t *)(uintptr_t)mac_get_rx_cb_mac_header_addr(pst_rx_info);
    ...
    if (mac_get_frame_type(mac_hdr) == WLAN_FC0_TYPE_CTL) {
        ...
        if (mgmt_frm_type == WLAN_FC0_SUBTYPE_BAR) {
            hmac_up_rx_bar(hmac_vap, pst_rx_info, pst_mgmt_rx_event->pst_netbuf);
        }
...
uint32_t hmac_mgmt_timeout_ap(void *p_param)
{
    ...
    oam_warning_log2(hmac_user->st_user_base_info.uc_vap_id, OAM_SF_MLOG,
        "{hmac_mgmt_timeout_ap::Wait AUTH timeout!! user_id [%d], After %d ms.}",
        hmac_user->st_user_base_info.us_assoc_id, WLAN_AUTH_AP_TIMEOUT);
    chr_exception_report(CHR_PLATFORM_EXCEPTION_EVENTID, CHR_SYSTEM_WIFI,
 ...
 * 1.日    期  : 2014年10月16日
  *   修改内容  : 新生成函数
 */
uint32_t hmac_ap_wait_start_misc(hmac_vap_stru *hmac_vap, void *param)
{
    hmac_misc_input_stru *pst_misc_input = NULL;

    if (oal_unlikely(oal_any_null_ptr2(hmac_vap, param))) {
        oam_error_log0(0, OAM_SF_RX, "{hmac_ap_wait_start_misc::hmac_vap or param is null.}");
        return OAL_ERR_CODE_PTR_NULL;
    }

    pst_misc_input = (hmac_misc_input_stru *)param;
 ...
 * 1.日    期  : 2014年10月20日
  *   修改内容  : 新生成函数
 */
uint32_t hmac_ap_up_misc(hmac_vap_stru *hmac_vap, void *param)
{
    hmac_misc_input_stru *pst_misc_input = NULL;
