...
oal_netbuf_stru *hmac_mgmt_ba_session_alloc(uint8_t vap_id)
{
...
uint32_t hmac_tx_action_event(mac_vap_stru *mac_vap, oal_netbuf_stru *netbuf, uint16_t frame_len)
{
 ...
 *   修改内容  : 新生成函数
 */
void hmac_mgmt_send_deauth_frame(uint8_t vap_idx, const unsigned char *da_addr,
...
uint32_t hmac_config_send_deauth(mac_vap_stru *pst_mac_vap, uint8_t *puc_da)
{
    ...
    /* 发去认证帧 */
    hmac_mgmt_send_deauth_frame(pst_mac_vap->uc_vap_id, puc_da, MAC_AUTH_NOT_VALID, OAL_FALSE);
...
void hmac_send_mgmt_to_host(hmac_vap_stru *hmac_vap, oal_netbuf_stru *buf, uint16_t len, int32_t freq)
{
    ...
    if (memcpy_s(mgmt_data, len, (uint8_t *)(uintptr_t)mac_get_rx_cb_mac_header_addr(rx_info), len) != EOK) {
