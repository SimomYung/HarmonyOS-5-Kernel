    ...
    mac_rx_ctl_stru *rx_ctrl)
{
    ...
    if ((data[MAC_ACTION_OFFSET_CATEGORY] == MAC_ACTION_CATEGORY_PUBLIC) &&
        ...
        hmac_ftm_initiator_rx_rsp(hmac_vap, netbuf, rx_ctrl);
        return TRUE;
    }
    ...
    frw_event_hdr_stru *event_hdr, mac_rx_ctl_stru *rx_ctrl)
{
    ...
    if ((!g_wlan_spec_cfg->feature_ftm_is_open) || (rx_ctrl == NULL)) {
        return FALSE;
    }
    ...
    mac_rx_ctl_stru *rx_ctrl)
{
    ...
    if (hmac_chba_vap_start_check(hmac_vap)) {
        hmac_chba_rx_mgmt(hmac_vap, crx_event, netbuf);
        return;
    }

    /* p2p_dev与p2p_gc共用mac vap时, 需针对chba nego action收帧做特殊处理, 避免错误走入gc处理流程 */
    if (g_wlan_spec_cfg->p2p_device_gc_use_one_vap == OAL_TRUE) {
        if (((is_p2p_cl(&hmac_vap->st_vap_base_info) == OAL_TRUE) ||
            (is_p2p_dev(&hmac_vap->st_vap_base_info) == OAL_TRUE)) &&
            (hmac_vap_rx_mgmt_is_chba_negotiation_action(netbuf, rx_ctrl) == OAL_TRUE)) {
...
static uint32_t hmac_mgmt_send_deauth_before_assoc(mac_vap_stru *mac_vap, uint8_t *da, uint16_t err_code)
{
    ...
    /* 给当前vap发送去认证帧 */
    hmac_mgmt_send_deauth_frame(mac_vap->uc_vap_id, da, err_code, OAL_FALSE);  // 非PMF
    ...
    hmac_mgmt_send_deauth_frame(up_vap2->uc_vap_id, da, err_code | MAC_SEND_TWO_DEAUTH_FLAG, OAL_FALSE);
    return OAL_SUCC;
}
/*
 * 功能描述 : 去关联/去认证事件处理
...
void hmac_proc_disasoc_inform(hmac_vap_stru *hmac_vap, mac_reason_code_enum_uint16 reason_code, uint8_t is_roam)
{
    hmac_disconnect disconnect;

    disconnect.reason_code = reason_code;
    disconnect.type = WLAN_FC0_SUBTYPE_DISASSOC;
    disconnect.is_roam = is_roam;
                                               ...
                                               dmac_disasoc_misc_stru *pdmac_disasoc_misc_stru)
{
    ...
    /* 删除对应用户 */
    hmac_user_del(&hmac_vap->st_vap_base_info, hmac_user);
    hmac_proc_disasoc_inform(hmac_vap, reason_code, OAL_FALSE);
}
...
uint32_t hmac_proc_disasoc_misc_event(frw_event_mem_stru *event_mem)
{
    ...
    if (hmac_vap->st_vap_base_info.en_vap_state == MAC_VAP_STATE_STA_LINK_DOWN) {
        oam_error_log0(hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_ASSOC,
            "{hmac_proc_disasoc_misc_event::vap is linkdown.}");
        return OAL_SUCC;
    }
...
uint32_t hmac_proc_roam_trigger_event(frw_event_mem_stru *event_mem)
{
    ...
    hmac_roam_trigger_handle(hmac_vap, c_rssi, OAL_TRUE, ROAM_SAME_MLD_LINK_SWITCH);
