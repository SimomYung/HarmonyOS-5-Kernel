...
void hmac_update_join_req_params_2040(mac_vap_stru *mac_vap, mac_bss_dscr_stru *bss_dscr)
{
    ...
    /* (2) AP在40MHz运行 */
    if (mac_mib_get_FortyMHzOperationImplemented(mac_vap) == OAL_TRUE) {
    ...
    /* 不超过mac device最大带宽能力，如果ap 2g不携带vht ie，可能导致vap带宽配置超过最大值 */
    mac_vap->st_channel.en_bandwidth = mac_vap_get_bandwith(mac_mib_get_dot11VapMaxBandWidth(mac_vap),
...
{
    if (mac_mib_get_DesiredBSSType(&hmac_vap->st_vap_base_info) == WLAN_MIB_DESIRED_BSSTYPE_INFRA) {
...
oal_bool_enum_uint8 hmac_is_support_11grate(uint8_t *rates, uint8_t rate_num)
{
    ...
    if ((hmac_is_rate_support(rates, rate_num, 0x0C) == OAL_TRUE) ||
        (hmac_is_rate_support(rates, rate_num, 0x12) == OAL_TRUE) ||
        (hmac_is_rate_support(rates, rate_num, 0x18) == OAL_TRUE) ||
        (hmac_is_rate_support(rates, rate_num, 0x24) == OAL_TRUE) ||
        (hmac_is_rate_support(rates, rate_num, 0x30) == OAL_TRUE) ||
        (hmac_is_rate_support(rates, rate_num, 0x48) == OAL_TRUE) ||
        (hmac_is_rate_support(rates, rate_num, 0x60) == OAL_TRUE) ||
        (hmac_is_rate_support(rates, rate_num, 0x6C) == OAL_TRUE)) {
        return OAL_TRUE;
    }
    ...
    wlan_protocol_enum_uint8 *protocol_mode)
{
    ...
    if (g_wlan_spec_cfg->feature_11be_is_open && (bss_dscr->eht_capable == OAL_TRUE)) {
        *protocol_mode = WLAN_EHT_MODE;
        return;
    }
#endif
...
OAL_STATIC uint32_t hmac_sta_send_auth_seq3(hmac_vap_stru *sta, uint8_t *mac_hdr, mac_rx_ctl_stru *rx_ctl)
{
    ...
    uint8_t vap_id = sta->st_vap_base_info.uc_vap_id;
    ...
    if (auth_frame == NULL) {
        oam_error_log0(vap_id, OAM_SF_AUTH, "{hmac_wait_auth_sta::pst_auth_frame null.}");
        return OAL_ERR_CODE_PTR_NULL;
    }
    ...
    if (hmac_user_ap == NULL || auth_frame_len == 0) {
        oal_netbuf_free(auth_frame);
        oam_error_log2(vap_id, OAM_SF_AUTH, "{hmac_wait_auth_sta::hmac_user[%d] null or invalid auth_rsp_len %d.}",
            sta->st_vap_base_info.us_assoc_vap_id, auth_frame_len);
        return OAL_ERR_CODE_PTR_NULL;
    }
    ...
    if (ret != OAL_SUCC) {
        oal_netbuf_free(auth_frame);
        oam_warning_log1(vap_id, OAM_SF_AUTH, "{hmac_wait_auth_sta::hmac_tx_mgmt_send_event failed[%d].}", ret);
        return ret;
    }
...
static oal_bool_enum_uint8 hmac_connect_is_p2p_gc_connect_noa_go(hmac_vap_stru *hmac_vap, hmac_join_req_stru *join_req)
{
    ...
    while (ie_len != 0) {
        ...
        p2p_ie = mac_find_vendor_ie(MAC_WLAN_OUI_WFA, MAC_WLAN_OUI_TYPE_WFA_P2P, &ies_info);
        if (p2p_ie == NULL) {
            return OAL_FALSE;
        }
        ...
        p2p_noa_attribute = mac_find_p2p_attribute(MAC_P2P_ATTRIBUTE_NOA,
                                                   p2p_ie + BYTE_OFFSET_6, p2p_ie[1] - BYTE_OFFSET_4);
...
uint32_t hmac_sta_wait_join(hmac_vap_stru *hmac_sta, void *msg)
{
    ...
    if (hmac_connect_is_p2p_gc_connect_noa_go(hmac_sta, join_req) == OAL_TRUE) {
        ...
        frw_timer_create_timer_m(&hmac_sta->st_mgmt_timer, hmac_mgmt_timeout_sta, join_req->timeout,
        ...
        memcpy_s(user_info.mac_addr, WLAN_MAC_ADDR_LEN, join_req->st_bss_dscr.auc_bssid, WLAN_MAC_ADDR_LEN);
        /* 由于需要noa控制用户收发，提前添加用户 */
        ret = hmac_user_add(&(hmac_sta->st_vap_base_info), &user_info, &user_index);
        if (ret != OAL_SUCC) {
            oam_error_log1(hmac_sta->st_vap_base_info.uc_vap_id, OAM_SF_ASSOC,
                "{hmac_sta_wait_join:fail to add use[%d]!}", ret);
            return ret;
        }
    } else {
...
void hmac_auth_alg_check(hmac_vap_stru *pst_sta, uint16_t us_auth_alg)
{
    if ((mac_mib_get_AuthenticationMode(&pst_sta->st_vap_base_info) != us_auth_alg) &&
        (mac_mib_get_AuthenticationMode(&pst_sta->st_vap_base_info) != WLAN_WITP_AUTH_AUTOMATIC)) {
...
uint32_t hmac_sta_wait_auth_seq2_rx(hmac_vap_stru *pst_sta, void *pst_msg)
{
    ...
    puc_mac_hdr = (uint8_t *)(uintptr_t)mac_get_rx_cb_mac_header_addr(rx_ctl);
...
uint32_t hmac_sta_wait_auth_seq4_rx(hmac_vap_stru *pst_sta, void *p_msg)
{
    ...
    mac_hdr = (uint8_t *)(uintptr_t)mac_get_rx_cb_mac_header_addr(rx_ctl);
    ...
    hmac_remain_on_channel_param_stru *pst_remain_on_channel)
{
    ...
    if ((pst_remain_on_channel->en_roc_type == IEEE80211_ROC_TYPE_MGMT_TX) ||
        (pst_remain_on_channel->en_roc_type == IEEE80211_ROC_TYPE_CHBA_LOW_PRIORITY_MGMT_TX) ||
        (pst_remain_on_channel->en_roc_type == IEEE80211_ROC_TYPE_CHBA_HIGH_PRIORITY_MGMT_TX)) {
        pst_scan_params->p_fn_cb = hmac_mgmt_tx_roc_comp_cb;
        pst_scan_params->bit_is_special_roc_type = OAL_TRUE;
    } else if (pst_remain_on_channel->en_roc_type == IEEE80211_ROC_TYPE_CHBA_MGMT_RX) {
    ...
    pst_scan_params->en_scan_mode = WLAN_SCAN_MODE_P2P_LISTEN;
...
uint32_t hmac_p2p_remain_on_channel(hmac_vap_stru *pst_hmac_vap_sta, void *p_param)
{
    ...
    if (oal_unlikely(pst_mac_device == NULL)) {
        oam_error_log1(0, OAM_SF_ANY,
            "{hmac_p2p_listen_timeout::pst_mac_device[%d] null!}", pst_mac_vap->uc_device_id);
        return OAL_ERR_CODE_PTR_NULL;
    }
