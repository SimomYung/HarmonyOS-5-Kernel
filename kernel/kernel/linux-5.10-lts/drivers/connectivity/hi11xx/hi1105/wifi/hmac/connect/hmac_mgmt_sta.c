...
static oal_bool_enum_uint8 hmac_is_invalid_ht_rate_hp(uint8_t *ie)
{
    ...
    return ((ie[BYTE_OFFSET_5] == 0x02) && (ie[BYTE_OFFSET_6] == 0x00) &&
        (ie[BYTE_OFFSET_7] == 0x05) && (ie[BYTE_OFFSET_8] == 0x00));
}
    ...
    hmac_vap_stru *pst_hmac_vap_sta, oal_netbuf_stru *pst_netbuf)
{
    ...
    switch (en_public_action_field) {
        ...
        default:
            oam_warning_log1(0, OAM_SF_RX, "{hmac_sta_not_up_rx_public_action::rx other public action frame %d }",
                en_public_action_field);
            hmac_rx_mgmt_send_to_host(pst_hmac_vap_sta, pst_netbuf);
            break;
    }
...
OAL_STATIC void hmac_rx_mgmt_action(hmac_vap_stru *hmac_vap, dmac_wlan_crx_event_stru *mgmt_rx_event)
{
    ...
    oam_warning_log2(hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_RX,
        "{hmac_sta_not_up_rx_mgmt::category=[%d], action=[%d].}",
        payload[MAC_ACTION_OFFSET_CATEGORY], payload[MAC_ACTION_OFFSET_ACTION]);
...
uint8_t *hmac_sta_find_ie_in_probe_rsp(mac_vap_stru *pst_mac_vap, uint8_t uc_eid, uint16_t *pus_index)
{
    ...
    if (pst_bss_dscr == NULL) {
        oam_warning_log3(pst_mac_vap->uc_vap_id, OAM_SF_CFG,
                         "hmac_sta_find_ie_in_probe_rsp::find the bss failed by bssid:%02X:XX:XX:XX:%02X:%02X",
    ...
    ies_info.frame_ies = puc_payload;
    ies_info.frame_len = pst_bss_dscr->mgmt_len - us_offset;

    puc_ie = mac_find_ie(uc_eid, &ies_info);
    if (puc_ie == NULL) {
        return NULL;
    }
 ...
 *   修改内容  : 新生成函数
 */
void hmac_sta_check_ht_cap_ie(mac_vap_stru *pst_mac_sta, mac_ies_info *ies_info, mac_user_stru *pst_mac_user_ap,
    uint16_t *pus_amsdu_maxsize)
{
    uint8_t *puc_ie = NULL;
    uint8_t *ht_cap_payload = NULL;
    uint16_t us_ht_cap_index;

    if (oal_any_null_ptr3(pst_mac_sta, ies_info, pst_mac_user_ap)) {
        return;
    }

    puc_ie = mac_find_ie(MAC_EID_HT_CAP, ies_info);
    if (puc_ie == NULL || puc_ie[1] < MAC_HT_CAP_LEN) {
        ht_cap_payload = hmac_sta_find_ie_in_probe_rsp(pst_mac_sta, MAC_EID_HT_CAP, &us_ht_cap_index);
        if (ht_cap_payload == NULL) {
            oam_warning_log0(0, OAM_SF_ANY, "{hmac_sta_check_ht_cap_ie::ie_payload_for_ht_cap_chk is null.}");
            return;
        }

        if (ht_cap_payload[us_ht_cap_index + 1] < MAC_HT_CAP_LEN) {
            oam_warning_log1(0, OAM_SF_ANY, "{hmac_sta_check_ht_cap_ie::invalid ht cap len[%d].}",
                             ht_cap_payload[us_ht_cap_index + 1]);
            return;
        }
        puc_ie = ht_cap_payload + us_ht_cap_index; /* 赋值HT CAP IE */
    }

    /* 仅针对惠普打印机M202dw降协议 */
    if ((pst_mac_sta->st_channel.en_band == WLAN_BAND_2G) && hmac_is_invalid_ht_rate_hp(puc_ie)) {
    ...
    mac_ie_proc_ht_sta(pst_mac_sta, puc_ie, pst_mac_user_ap, pus_amsdu_maxsize);
}
 ...
 *   修改内容  : 新生成函数
 */
void hmac_sta_check_ext_cap_ie(mac_vap_stru *pst_mac_sta, mac_user_stru *pst_mac_user_ap, mac_ies_info *ies_info)
{
    uint8_t *puc_ie;
    uint8_t *ext_cap_payload = NULL;
    uint16_t us_index;

    puc_ie = mac_find_ie(MAC_EID_EXT_CAPS, ies_info);
    if (puc_ie == NULL || puc_ie[1] < MAC_MIN_XCAPS_LEN) {
        ext_cap_payload = hmac_sta_find_ie_in_probe_rsp(pst_mac_sta, MAC_EID_EXT_CAPS, &us_index);
        if (ext_cap_payload == NULL) {
            return;
        }

        if (ext_cap_payload[us_index + 1] < MAC_MIN_XCAPS_LEN) {
            oam_warning_log1(0, OAM_SF_ANY, "{hmac_sta_check_ext_cap_ie::invalid ext cap len[%d].}",
                             ext_cap_payload[us_index + 1]);
            return;
        }
        puc_ie = ext_cap_payload + us_index;
    }

    /* 处理 Extended Capabilities IE */
    mac_ie_proc_ext_cap_ie(pst_mac_user_ap, puc_ie);
}
 ...
 *   修改内容  : 新生成函数
 */
uint32_t hmac_sta_check_ht_opern_ie(mac_vap_stru *pst_mac_sta, mac_user_stru *pst_mac_user_ap, mac_ies_info *ies_info)
{
    uint8_t *puc_ie;
    uint8_t *ht_opern_payload = NULL;
    uint16_t us_index;
    uint32_t change = MAC_NO_CHANGE;

    puc_ie = mac_find_ie(MAC_EID_HT_OPERATION, ies_info);
    if (puc_ie == NULL || puc_ie[1] < MAC_HT_OPERN_LEN) {
        ht_opern_payload = hmac_sta_find_ie_in_probe_rsp(pst_mac_sta, MAC_EID_HT_OPERATION, &us_index);
        if (ht_opern_payload == NULL) {
            return change;
        }

        if (ht_opern_payload[us_index + 1] < MAC_HT_OPERN_LEN) {
            oam_warning_log1(0, OAM_SF_ANY, "{hmac_sta_check_ht_opern_ie::invalid ht cap len[%d].}",
                             ht_opern_payload[us_index + 1]);
            return change;
        }
        puc_ie = ht_opern_payload + us_index;
    }

    change |= mac_proc_ht_opern_ie(pst_mac_sta, puc_ie, pst_mac_user_ap);
 ...
 *   修改内容  : 新生成函数
 */
uint32_t hmac_mgmt_assoc_rsp_check_ht_sta(mac_vap_stru *pst_mac_sta, mac_ies_info *ies_info,
    hmac_user_stru *p_hmac_user_ap, uint16_t *pus_amsdu_maxsize)
{
    uint32_t change = MAC_NO_CHANGE;

    if (oal_any_null_ptr3(pst_mac_sta, ies_info, p_hmac_user_ap)) {
        return change;
    }

    hmac_ht_self_cure_event_set(pst_mac_sta->uc_vap_id, p_hmac_user_ap->st_user_base_info.auc_user_mac_addr,
    ...
    hmac_sta_check_ht_cap_ie(pst_mac_sta, ies_info, &p_hmac_user_ap->st_user_base_info,
        pus_amsdu_maxsize);
    /* sta处理AP的 Extended Capability */
    hmac_sta_check_ext_cap_ie(pst_mac_sta, &p_hmac_user_ap->st_user_base_info, ies_info);

    change = hmac_sta_check_ht_opern_ie(pst_mac_sta, &p_hmac_user_ap->st_user_base_info, ies_info);
 ...
 *   修改内容  : 新生成函数
 */
OAL_STATIC void hmac_1024qam_judge(hmac_user_stru *pst_hmac_user, mac_ies_info *ies_info)
{
    uint8_t *puc_tmp_ie = NULL;

    puc_tmp_ie = mac_find_vendor_ie(MAC_HUAWEI_VENDER_IE, MAC_HISI_1024QAM_IE, ies_info);
    if (puc_tmp_ie != NULL) {
    ...
    hmac_user_stru *pst_hmac_user)
{
    ...
    if (pst_mac_vap->st_channel.en_band == WLAN_BAND_2G) {
        ...
        if (oal_value_eq_any2(pst_hmac_user->st_user_base_info.en_protocol_mode, WLAN_HE_MODE, WLAN_EHT_MODE)) {
...
void hmac_ie_check_priv_mcs_judge(hmac_user_stru *pst_hmac_user, mac_ies_info *ies_info)
{
#ifdef _PRE_WLAN_FEATURE_1024QAM
    hmac_1024qam_judge(pst_hmac_user, ies_info);
#endif
}

static void hmac_process_assoc_rsp_update_vht_cap(mac_ies_info *ies_info,
    hmac_user_stru *hmac_user, hmac_vap_stru *hmac_sta, uint32_t *change)
{
    mac_vap_stru *mac_vap = &(hmac_sta->st_vap_base_info);
    uint8_t *tmp_ie = NULL;
    mac_ies_info ies_info_temp = {0};
    ...
    if (hmac_user->st_user_base_info.st_vht_hdl.en_vht_capable == OAL_FALSE) {
        ...
        puc_vendor_vht_ie = mac_find_vendor_ie(MAC_WLAN_OUI_BROADCOM_EPIGRAM, MAC_WLAN_OUI_VENDOR_VHT_TYPE, ies_info);
        if ((puc_vendor_vht_ie != NULL) && (puc_vendor_vht_ie[1] >= vendor_vht_ie_offset)) {
            ...
            ies_info_temp.frame_ies = puc_vendor_vht_ie + vendor_vht_ie_offset;
            ies_info_temp.frame_len = ies_info->frame_len;
            /* 进入此函数代表user支持2G 11ac */
            tmp_ie = mac_find_ie(MAC_EID_VHT_CAP, &ies_info_temp);
            if (tmp_ie != NULL) {
                hmac_proc_vht_cap_ie(mac_vap, hmac_user, tmp_ie);
            }
            /* 更新11ac VHT operation ie */
            tmp_ie = mac_find_ie(MAC_EID_VHT_OPERN, &ies_info_temp);
            if (tmp_ie != NULL) {
...
static void hmac_process_assoc_rsp_update_cap_ie(mac_ies_info *ies_info,
    hmac_user_stru *hmac_user, hmac_vap_stru *hmac_sta, uint32_t *change)
{
    mac_vap_stru *mac_vap = &(hmac_sta->st_vap_base_info);
    uint8_t *tmp_ie = NULL;
    ...
    hmac_mgmt_update_assoc_user_qos_table(ies_info, hmac_user);

    /* 更新关联用户的legacy速率集合 */
    hmac_user_init_rates(hmac_user);
    hmac_ie_proc_assoc_user_legacy_rate(ies_info, hmac_user);

    hmac_user_cap_update_by_hisi_cap_ie(hmac_user, ies_info);
    ...
    *change |= hmac_mgmt_assoc_rsp_check_ht_sta(&hmac_sta->st_vap_base_info, ies_info,
    ...
    tmp_ie = mac_find_ie(MAC_EID_VHT_CAP, ies_info);
    if (tmp_ie != NULL) {
        hmac_proc_vht_cap_ie(mac_vap, hmac_user, tmp_ie);
        hmac_ie_check_priv_mcs_judge(hmac_user, ies_info);
    }
    /* 更新11ac VHT operation ie */
    tmp_ie = mac_find_ie(MAC_EID_VHT_OPERN, ies_info);
    if (tmp_ie != NULL) {
        *change |= hmac_update_vht_opern_ie_sta(mac_vap, hmac_user, tmp_ie);
    }
    ...
    if (hmac_user_ht_support(hmac_user) == OAL_TRUE) {
        hmac_process_assoc_rsp_update_vht_cap(ies_info, hmac_user, hmac_sta, change);
    }
#ifdef _PRE_WLAN_FEATURE_11AX
    /* 更新 11ax HE Capabilities ie */
    if (g_wlan_spec_cfg->feature_11ax_is_open) {
        *change |= hmac_ie_check_he_sta(hmac_sta, hmac_user, ies_info);
    }
#endif

#ifdef _PRE_WLAN_FEATURE_11BE
    /* 更新 11be EHT Capabilities ie */
    if (g_wlan_spec_cfg->feature_11be_is_open) {
        *change |= hmac_process_eht_ie(mac_vap, hmac_user, ies_info);
    }
#endif

#ifdef _PRE_WLAN_FEATURE_HIEX
    if (g_wlan_spec_cfg->feature_hiex_is_open) {
        hmac_rx_assoc_rsp_parse_tb_frame_gain(ies_info);
        *change |= hmac_hiex_rx_assoc_rsp(hmac_sta, hmac_user, ies_info);
    }
#endif

#ifdef _PRE_WLAN_FEATURE_WUR
    hmac_process_wur_ie(mac_vap, hmac_user, ies_info->frame_ies, ies_info->frame_len);
#endif
}
/*
 * 函 数 名  : hmac_process_assoc_rsp_widerbw_ap_bw_syn
 * 功能描述  : wide bw 场景，同步ap带宽信息到dmac_vap
 * 1.日    期  : 2024年3月13日
 */
OAL_STATIC void hmac_process_assoc_rsp_widerbw_ap_bw_syn(hmac_vap_stru *hmac_sta,
    wlan_channel_bandwidth_enum_uint8 en_bandwidth_ap)
{
    uint32_t ret;
    ret = hmac_config_vap_widebw_ap_bw_syn(&(hmac_sta->st_vap_base_info),
                                           sizeof(en_bandwidth_ap),
                                           (uint8_t *)(&en_bandwidth_ap));
    if (ret != OAL_SUCC) {
        oam_error_log2(hmac_sta->st_vap_base_info.uc_vap_id, OAM_SF_ASSOC,
            "{hmac_process_assoc_rsp_widerbw_ap_bw_syn::syn failed[%d] apbw[%d].}",
            ret, en_bandwidth_ap);
    }
}
    ...
    hmac_vap_stru *hmac_sta, uint32_t *change)
{
...
#endif
    oam_warning_log3(mac_vap->uc_vap_id, OAM_SF_ASSOC, "{hmac_process_assoc_rsp:ap max nss[%d]vap nss[%d]usermode[%d]}",
                     hmac_user->st_user_base_info.en_user_max_cap_nss, mac_vap->en_vap_rx_nss, avail_mode);
    if (ret != OAL_SUCC) {
    ...
    sta_new_bandwidth = mac_vap_get_ap_usr_opern_bandwidth(mac_vap, &(hmac_user->st_user_base_info), &en_bandwidth_ap);
    *change |= mac_vap_set_bw_check(mac_vap, sta_new_bandwidth);
    ...
    if (g_wlan_spec_cfg->wide_bw_support) {
        hmac_process_assoc_rsp_widerbw_ap_bw_syn(hmac_sta, en_bandwidth_ap);
    }
    /* 获取用户160M带宽下的空间流 */
    ...
    hmac_user_stru *hmac_user, uint8_t *mac_hdr, mac_ies_info *ies_info)
{
...
#ifdef _PRE_WLAN_FEATURE_ML
    ret = hmac_config_user_ml_info_syn(&(hmac_sta->st_vap_base_info), &hmac_user->st_user_base_info);
#endif
    ...
    hmac_sta_up_update_edca_params(ies_info, hmac_sta, mac_get_frame_sub_type(mac_hdr),
        hmac_user);

    hmac_arp_probe_type_set(mac_vap, OAL_TRUE, HMAC_VAP_ARP_PROBE_TYPE_HTC);
#ifdef _PRE_WLAN_FEATURE_11AX
    if (g_wlan_spec_cfg->feature_11ax_is_open) {
        hmac_sta_up_update_he_edca_params(ies_info, hmac_sta,
...
{
    if (g_wlan_spec_cfg->feature_11ax_is_open) {
        hmac_sta_up_update_assoc_rsp_sr_params(ies_info, hmac_sta,
                                               mac_get_frame_sub_type(mac_hdr), hmac_user);
        hmac_sta_up_update_he_nfrp_params(ies_info, hmac_sta,
            mac_get_frame_sub_type(mac_hdr), hmac_user);
    }
}
#endif
...
void hmac_process_assoc_rsp_set_aid(mac_vap_stru *mac_vap, uint8_t *payload)
{
    ...
    if ((aid > 0) && (aid <= MAX_AID_NUM)) {
        ...
        oam_warning_log1(mac_vap->uc_vap_id, OAM_SF_ASSOC, "{hmac_process_assoc_rsp::invalid us_sta_aid[%d].}", aid);
    }
}
    ...
    uint8_t *mac_hdr, mac_ies_info *ies_info)
{
    ...
    /* 初始化安全端口过滤参数 */
    rslt = hmac_init_user_security_port(&(hmac_vap->st_vap_base_info), hmac_user);
    if (rslt != OAL_SUCC) {
...
#ifdef _PRE_WLAN_FEATURE_11AX
    hmac_process_assoc_rsp_update_sr_nfrp_para(hmac_vap, hmac_user, mac_hdr, ies_info);
#endif
}
    ...
    uint8_t *mac_hdr, mac_ies_info *ies_info)
{
    ...
    hmac_process_assoc_rsp_update_cap_ie(ies_info, hmac_user, hmac_vap, &change);
    /* 获取用户的协议模式 */
    ...
    ret = hmac_check_opmode_notify(hmac_vap, mac_hdr, ies_info, hmac_user);
    if (ret != OAL_SUCC) {
        oam_warning_log1(mac_vap->uc_vap_id, OAM_SF_ASSOC,
    ...
    hmac_process_assoc_rsp_user_sync_cap(hmac_vap, hmac_user, mac_hdr, ies_info);
    ...
    uint8_t *mac_hdr, uint16_t hdr_len, uint8_t *payload, uint16_t msg_len_input)
{
    ...
    uint32_t change = MAC_NO_CHANGE;
    mac_ies_info ies_info = {0};
    uint16_t msg_len = msg_len_input;
    if (hmac_vap == NULL) {
    ...
    /* 更新关联ID */
    hmac_process_assoc_rsp_set_aid(mac_vap, payload);
    ...
    /* 初始化安全端口过滤参数 */
    ret = hmac_init_user_security_port(&(hmac_vap->st_vap_base_info), hmac_user);
    if (ret != OAL_SUCC) {
        oam_error_log1(hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_ASSOC,
                       "{hmac_process_assoc_rsp::hmac_init_user_security_port failed[%d].}", ret);
    }
    ...
    if (hmac_vap->st_vap_base_info.en_vap_state == MAC_VAP_STATE_ROAMING) {
        hmac_sta_wait_asoc_rx_rpt_user_chan_check(&hmac_vap->st_vap_base_info,
            &ret, hmac_vap, hmac_user->st_user_base_info.us_assoc_id);
    }
    return OAL_SUCC;
}
...
{
    mac_rx_ctl_stru *rx_ctrl = (mac_rx_ctl_stru *)oal_netbuf_cb(mgmt_rx_event->pst_netbuf);
    ...
    if (frame_sub_type != (WLAN_FC0_SUBTYPE_ASSOC_RSP | WLAN_FC0_TYPE_MGT) &&
        ...
        /* 打印此netbuf相关信息 */
        mac_rx_report_80211_frame(hmac_sta->st_vap_base_info.uc_vap_id, (uint8_t *)rx_ctrl,
    ...
    mac_status_code_enum_uint16 asoc_status)
{
    ...
    if (asoc_status == MAC_MISMATCH_HTCAP || asoc_status == MAC_UNSUP_RATE) {
        ...
        hmac_ht_self_cure_event_set(hmac_sta->st_vap_base_info.uc_vap_id, addr_sa,
    ...
    hmac_vap_stru *hmac_sta, uint16_t user_idx)
{
    ...
    if (oal_notice_sta_join_result(hmac_sta->st_vap_base_info.uc_chip_id, OAL_TRUE) != OAL_SUCC) {
        oam_warning_log0(hmac_sta->st_vap_base_info.uc_vap_id, OAM_SF_COEX,
    ...
    hmac_vap_stru *hmac_sta, mac_vap_state_enum_uint8 en_new_vap_state, uint8_t *addr_sa, uint16_t user_idx)
{
    ...
    /* STA切换到UP状态 */
    hmac_fsm_change_state_for_single_link(hmac_sta, en_new_vap_state);
    ...
    hmac_sta_wait_asoc_rx_rpt_user_chan_check(&(hmac_sta->st_vap_base_info), &ret, hmac_sta, user_idx);
}
...
{
    if (hmac_vap_is_legacy_mld_mode(hmac_sta) == OAL_TRUE) {
        hmac_sta_wait_asoc_rx_succ_handle_single_link(hmac_user_ap, hmac_sta, MAC_VAP_STATE_UP, addr_sa, user_idx);
        return;
    }
#ifdef _PRE_WLAN_FEATURE_ML
    hmac_link_sta_wait_asoc_rx_succ_handle(hmac_sta);
    hmac_sta_wait_asoc_rx_succ_handle_single_link(hmac_user_ap, hmac_sta, MAC_VAP_STATE_UP, addr_sa, user_idx);
#endif
}
/*
...
uint32_t hmac_sta_wait_asoc_rx(hmac_vap_stru *hmac_sta, void *msg)
{
    ...
    pst_mac_vap = &(hmac_sta->st_vap_base_info);
    pst_mgmt_rx_event = (dmac_wlan_crx_event_stru *)msg;
    pst_rx_info = (mac_rx_ctl_stru *)oal_netbuf_cb(pst_mgmt_rx_event->pst_netbuf);
    if (pst_rx_info == NULL) {
        oam_error_log0(0, OAM_SF_ASSOC, "{hmac_sta_wait_asoc_rx::pst_rx_info null.}");
        return OAL_ERR_CODE_PTR_NULL;
    }
    puc_mac_hdr = (uint8_t *)(uintptr_t)mac_get_rx_cb_mac_header_addr(pst_rx_info);
    ...
    if (hmac_sta_wait_asoc_rx_judge_frame_subtype(hmac_sta, puc_mac_hdr, pst_mgmt_rx_event) != OAL_SUCC) {
        return OAL_FAIL;
    }
...
#endif
    if (hmac_sta_wait_asoc_rx_frame_para_check(hmac_sta, puc_payload, us_msg_len, puc_mac_hdr) != OAL_SUCC) {
        return OAL_FAIL;
    }
    rslt = hmac_sta_wait_asoc_rx_get_user(hmac_sta, puc_mac_hdr, auc_addr_sa, &us_user_idx, &pst_hmac_user_ap);
    if (rslt != OAL_SUCC) {
        return rslt;
    }
...
#ifdef _PRE_WLAN_FEATURE_ML
    ret = hmac_mld_process_asoc_rsp(hmac_sta, pst_hmac_user_ap, puc_mac_hdr, puc_payload, us_msg_len);
    if (ret != OAL_SUCC) {
        oam_warning_log1(hmac_sta->st_vap_base_info.uc_vap_id, OAM_SF_ASSOC,
                         "{hmac_sta_wait_asoc_rx::hmac_process_assoc_rsp mld failed[%d].}", ret);
        return ret;
    }
#endif

    ret = hmac_process_assoc_rsp(hmac_sta, pst_hmac_user_ap, puc_mac_hdr, us_hdr_len, puc_payload, us_msg_len);
    if (ret != OAL_SUCC) {
        oam_warning_log1(hmac_sta->st_vap_base_info.uc_vap_id, OAM_SF_ASSOC,
                         "{hmac_sta_wait_asoc_rx::hmac_process_assoc_rsp failed[%u].}", ret);
        return ret;
    }

    hmac_sta_wait_asoc_rx_succ_handle(pst_hmac_user_ap, hmac_sta, pst_mac_vap, auc_addr_sa, us_user_idx);

    /* 将用户(AP)在本地的状态信息设置为已关联状态 */
    /* 准备消息，上报给APP */
    hmac_sta_wait_asoc_rx_cfg_rsp_para(&st_asoc_rsp, puc_mac_hdr, us_msg_len, pst_hmac_user_ap);
    hmac_send_rsp_to_sme_sta(hmac_sta, HMAC_SME_ASOC_RSP, (uint8_t *)(&st_asoc_rsp));

#ifdef _PRE_WLAN_FEATURE_ML
    hmac_cofeture_state_change_notify_sta(hmac_sta, COFEATURE_TYPE_MLO, hmac_get_mlo_work_state(hmac_sta), 0);
#endif
...
uint32_t hmac_sta_auth_timeout(hmac_vap_stru *pst_hmac_sta, void *p_param)
{
    ...
    if (hmac_sta_get_mngpkt_sendstat(&pst_hmac_sta->st_vap_base_info, &st_mngpkt_sendstat_info) == OAL_SUCC) {
 ...
 *   修改内容  : 新生成函数
 */
static void hmac_sta_get_min_rate(dmac_set_rate_stru *pst_rate_params, mac_bss_dscr_stru *join_bss_dscr)
{
    uint32_t uc_idx;
    ...
    if (oal_any_null_ptr2(pst_rate_params, join_bss_dscr)) {
    ...
    // get min rate
    for (uc_idx = 0; uc_idx < join_bss_dscr->uc_num_supp_rates; uc_idx++) {
        if (hmac_get_phy_rate_and_protocol(join_bss_dscr->auc_supp_rates[uc_idx], &uc_protocol,
    ...
    pst_rate_params->un_capable_flag.st_capable.bit_ht_capable &= join_bss_dscr->en_ht_capable;
    pst_rate_params->un_capable_flag.st_capable.bit_vht_capable &= join_bss_dscr->en_vht_capable;
...
static oal_bool_enum_uint8 hmac_sta_support_opmode_rules(hmac_join_req_stru *pst_join_req)
{
    /* STA不支持OPMODE 场景：
     * 1.AP不支持vht
     * 2.AP不支持opmode
     */
    return ((pst_join_req->st_bss_dscr.en_vht_capable == OAL_FALSE) ||
...
OAL_STATIC uint32_t hmac_dual_sta_join_check(hmac_vap_stru *hmac_vap, hmac_join_req_stru *join_req)
{
    ...
    hmac_other_vap = (hmac_vap_stru *)mac_res_get_hmac_vap(other_vap->uc_vap_id);
    if (hmac_other_vap == NULL) {
        return OAL_ERR_CODE_PTR_NULL;
    }
    if (!mac_is_secondary_sta(hmac_other_vap)) {
        oam_warning_log2(other_vap->uc_vap_id, OAM_SF_ASSOC,
            "{hmac_dual_sta_join_check::unexpect vap, vap mode[%d], p2p mode[%d]}",
            other_vap->en_vap_mode, other_vap->en_p2p_mode);
        return OAL_SUCC;
    }
...
#ifdef _PRE_WLAN_FEATURE_11AX
    if (g_wlan_spec_cfg->feature_11ax_is_open) {
        hmac_sta_join_update_protocol_mib(pst_hmac_vap, &pst_join_req->st_bss_dscr);
    }
#endif
    ...
    hmac_join_req_stru *pst_join_req, mac_cfg_mode_param_stru *pst_cfg_mode)
{
    ...
    uint8_t emlsr_support = OAL_FALSE;
#endif
...
#ifdef _PRE_WLAN_FEATURE_WUR
    /* sta学习ap的wur能力 */
    mac_mib_set_WUROptionImplemented(pst_mac_vap, pst_join_req->st_bss_dscr.wur_capable);
    mac_mib_set_WURBeaconPeriod(pst_mac_vap, mac_mib_get_BeaconPeriod(pst_mac_vap));
#endif
#ifdef _PRE_WLAN_FEATURE_EMLSR
    /* 确认emlsr私有能力是否使能，若使能则只在对端ap支持emlsr私有对通时才可进入 */
    if (mac_is_primary_legacy_sta(&pst_hmac_vap->st_vap_base_info) == OAL_TRUE) {
        emlsr_support = hmac_get_emlsr_hisi_switch() == OAL_TRUE ?
            pst_join_req->st_bss_dscr.hisi_emlsr_capable : pst_join_req->st_bss_dscr.ml_info.eml_cap.emlsr_support;
        mac_mib_set_EHTEMLSROptionActivated(pst_mac_vap, emlsr_support);
    }
#endif
...
static uint32_t hmac_sta_update_join_req_params_bw_chan(hmac_vap_stru *hmac_vap,
    hmac_join_req_stru *pst_join_req)
{
    mac_vap_stru *mac_vap = &(hmac_vap->st_vap_base_info);
    uint32_t ret;
    ...
    hmac_update_join_req_params_prot_sta(hmac_vap, pst_join_req);
    /* 入网优化，不同频段下的能力不一样 */
...
#ifdef _PRE_WLAN_FEATURE_ML
    /* mlsr入网，join流程 辅link vap不下发校准数据；link切换时会重新下发 */
    if (hmac_vap_is_link_mode(hmac_vap) == OAL_TRUE &&
        hmac_get_mld_assoc_mode(hmac_vap) == MLD_ASSOC_MODE_MLSR) {
        oam_warning_log0(mac_vap->uc_vap_id, OAM_SF_SCAN,
            "{hmac_sta_update_join_req_params_bw_chan::link_mode vap not send cali_data}");
        return OAL_SUCC;
    }
#endif
    ...
    dmac_ctx_join_req_set_reg_stru *reg_params, mac_bss_dscr_stru *join_bss_dscr)
{
    ...
    /* 设置需要写入寄存器的BSSID信息 */
    oal_set_mac_addr(reg_params->auc_bssid, join_bss_dscr->auc_bssid);

#ifdef _PRE_WLAN_FEATURE_ML
    if (hmac_vap_is_legacy_mld_mode(hmac_vap)) {
        oal_set_mac_addr(reg_params->mld_mac_addr, join_bss_dscr->auc_bssid);
    } else {
        oal_set_mac_addr(reg_params->mld_mac_addr, join_bss_dscr->ml_info.ml_mac_addr);
    }
#else
    oal_set_mac_addr(reg_params->mld_mac_addr, join_bss_dscr->auc_bssid);
#endif
    ...
    /* 填写速率相关信息 */
    hmac_sta_get_min_rate(&reg_params->st_min_rate, join_bss_dscr);

    /* 设置beaocn period信息 */
    reg_params->us_beacon_period = (join_bss_dscr->us_beacon_period);
    ...
    /* 保存AP支持的最大空间流数 */
    reg_params->ap_support_max_nss = join_bss_dscr->en_support_max_nss;

    reg_params->sap_mode = join_bss_dscr->sap_mode;
#ifdef _PRE_WLAN_FEATURE_WUR
    reg_params->wur_cap = join_bss_dscr->wur_capable;
#endif
#ifdef _PRE_WLAN_FEATURE_EMLSR
    reg_params->emlsr_cap = join_bss_dscr->hisi_emlsr_capable;
#endif
    reg_params->set_vht_cap = mac_mib_get_VHTOptionImplemented(mac_vap);
    /* 下发ssid */
    memcpy_s(reg_params->auc_ssid, WLAN_SSID_MAX_LEN, join_bss_dscr->ac_ssid, WLAN_SSID_MAX_LEN);
    ...
    hmac_join_req_stru *join_req, mac_ap_type_enum_uint16 ap_type)
{
    ...
    hmac_sta_update_h2d_join_req_params(hmac_vap, reg_params, &(join_req->st_bss_dscr));
...
uint32_t hmac_sta_update_join_req_params(hmac_vap_stru *hmac_vap, hmac_join_req_stru *join_req)
{
    ...
    ret = hmac_sta_update_join_req_params_update_mib_cap(hmac_vap, join_req, &st_cfg_mode);
    if (ret != OAL_SUCC) {
        return ret;
    }

    /* 有些加密协议只能工作在legacy */
    hmac_sta_protocol_down_by_chipher(pst_mac_vap, &join_req->st_bss_dscr);
    hmac_sta_set_mode_params(&st_cfg_mode, pst_mac_vap->en_protocol, pst_mac_vap->st_channel.en_band,
        join_req->st_bss_dscr.st_channel.uc_chan_number, pst_mac_vap->st_channel.en_bandwidth);

    hmac_config_sta_update_rates(pst_mac_vap, &st_cfg_mode, (void *)&join_req->st_bss_dscr);
    ret = hmac_sta_update_join_req_params_bw_chan(hmac_vap, join_req);
    if (ret != OAL_SUCC) {
        return ret;
    }
    ...
    hmac_sta_update_join_req_params_for_roam(pst_mac_vap, join_req, &en_ap_type);
    ret = hmac_sta_update_join_req_params_post_event(hmac_vap, join_req, en_ap_type);
    if (ret != OAL_SUCC) {
        return ret;
    }
#ifdef _PRE_WLAN_FEATURE_M2S
    oam_warning_log4(pst_mac_vap->uc_vap_id, OAM_SF_ASSOC,
                     ...
                     join_req->st_bss_dscr.en_support_max_nss, join_req->st_bss_dscr.st_channel.en_band,
                     join_req->st_bss_dscr.en_ht_capable, join_req->st_bss_dscr.en_vht_capable);

    pst_mac_vap->st_cap_flag.bit_opmode = !hmac_sta_support_opmode_rules(join_req);
...
#ifdef _PRE_WLAN_FEATURE_PMF
    hmac_sta_roam_update_pmf(hmac_vap, &join_req->st_bss_dscr);
#endif
    ...
    oal_bool_enum_uint8 is_protected)
{
    ...
    p_mac_hdr = (mac_ieee80211_frame_stru *)(uintptr_t)mac_get_rx_cb_mac_header_addr(rx_ctrl);
    data = mac_get_rx_payload_addr(rx_ctrl, netbuf);

    if (hmac_get_frame_body_len(netbuf) < MAC_ACTION_CATEGORY_AND_CODE_LEN) {
        oam_warning_log1(0, OAM_SF_RX, "{sta_roam::frame len too short[%d].}", rx_ctrl->us_frame_len);
        return;
    }
