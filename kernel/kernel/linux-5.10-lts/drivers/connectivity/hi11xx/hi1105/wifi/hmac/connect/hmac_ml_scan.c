...
uint32_t hmac_set_hipriv_ml_scan(mac_vap_stru *mac_vap, uint32_t *params)
{
    ...
    oam_warning_log1(0, 0, "{hmac_set_hipriv_ml_scan::ml_scan [%d]}", g_hipriv_ml_scan);
    return OAL_SUCC;
}
...
void hmac_prepare_ml_scan_req(hmac_vap_stru *hmac_vap, mac_scan_req_stru *scan_params)
{
    ...
    */
    ...
    oam_warning_log3(hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_SCAN,
                     "hmac_prepare_ml_scan_req::set ml scan params bssid[%02X:xx:xx:xx:%02X:%02X],",
                     scan_params->auc_bssid[0][0],
    ...
    hmac_conn_param_stru *connect_param)
{
    ...
    // 缓存单播扫描的参数、关联参数等
    hmac_vap->st_vap_base_info.en_vap_state = vap_state;
...
uint32_t hmac_sta_defrag_ml_ie(hmac_vap_stru *hmac_vap, uint8_t *frame_body, uint16_t frame_len)
{
    ...
    if (hmac_sta_parse_basic_ml_common_info_process(ml_ie, frame_len, &ap_mld_info, &common_info_len) != OAL_SUCC) {
        ...
        oam_error_log0(0, OAM_SF_ASSOC, "{hmac_sta_defrag_ml_ie::parse ml_common_info fail.}");
        return OAL_FAIL;
    }
    ml_ie += MAC_IE_EXT_HDR_LEN + ML_CONTROL_FIELD_LEN + common_info_len;
    ...
    oal_netbuf_stru *new_netbuf, uint16_t new_buf_len, uint8_t link_id)
{
    ...
    add_ie_len += hmac_per_sta_profile_add_basic_ml_ie(new_frame_body, left_netbuf_len, link_id);
...
uint32_t hmac_sta_add_ml_scan_per_sta_profile_bss(hmac_vap_stru *hmac_vap, oal_netbuf_stru *netbuf, uint16_t buf_len)
{
    uint8_t link_id = 0;
    oal_netbuf_stru *new_netbuf = NULL;
    ...
    for (link_id = 0; link_id < WLAN_AP_MAX_ML_LINK_INFO_NUM; link_id++) {
        ...
        oam_warning_log1(0, OAM_SF_SCAN, "hmac_sta_add_ml_scan_per_sta_profile_bss::link_id[%d] netbuf goto add bss",
            link_id);
