...
OAL_STATIC void hmac_scan_print_scan_params(mac_scan_req_stru *scan_params, mac_vap_stru *mac_vap)
{
    if (!((scan_params->uc_scan_func == MAC_SCAN_FUNC_P2P_LISTEN) &&
        ...
        oam_warning_log4(scan_params->uc_vap_id, OAM_SF_MLOG,
...
oal_bool_enum_uint8 hmac_scan_is_ap_up_connect_user(mac_device_stru *mac_device, hmac_vap_stru *hmac_vap)
{
    uint32_t i;
    uint32_t up_vap_num;
...
{
    if (hmac_scan_is_ap_up_connect_user(mac_device, hmac_vap) == OAL_FALSE) {
        return;
    }
