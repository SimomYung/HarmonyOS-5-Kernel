...
OAL_STATIC void hmac_scan_print_scan_params(mac_scan_req_stru *scan_params, mac_vap_stru *mac_vap)
{
    if (!((scan_params->uc_scan_func == MAC_SCAN_FUNC_P2P_LISTEN) &&
        ...
        oam_warning_log1(scan_params->uc_vap_id, OAM_SF_SCAN,
            "hmac_scan_print_scan_params::SCAN_MOEE:%d",
    ...
    mac_scan_rsp_stru *d2h_scan_rsp_info)
{
    ...
    if ((hmac_vap->st_vap_base_info.en_vap_mode == WLAN_VAP_MODE_BSS_AP) &&
        ...
        oam_warning_log1(hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_SCAN,
            "hmac_scan_change_vap_state:scan_vap_id[%d]!", scan_mgmt->st_scan_record_mgmt.uc_vap_id);
        if (hmac_vap->st_vap_base_info.uc_vap_id == scan_mgmt->st_scan_record_mgmt.uc_vap_id) {
...
uint32_t hmac_scan_proc_scan_comp_event(frw_event_mem_stru *event_mem)
{
    ...
    if (oal_unlikely(scan_mgmt == NULL)) {
        return OAL_ERR_CODE_PTR_NULL;
    }
...
uint32_t hmac_scan_proc_scan_req_event_exception(hmac_vap_stru *hmac_vap, void *params)
{
    ...
    if ((hmac_vap->st_vap_base_info.en_vap_mode == WLAN_VAP_MODE_BSS_STA) &&
        (hmac_vap->st_vap_base_info.en_vap_state == MAC_VAP_STATE_STA_WAIT_SCAN)) {
        /* 改变vap状态到SCAN_COMP */
        mac_vap_state_change(&hmac_vap->st_vap_base_info, MAC_VAP_STATE_STA_SCAN_COMP);
    }
    ...
    wlan_vap_mode_enum_uint8 en_vap_mode)
{
    if (en_vap_mode == WLAN_VAP_MODE_BSS_STA) {
        if (scan_params->en_scan_mode != WLAN_SCAN_MODE_ROAM_SCAN &&
            scan_params->en_scan_mode != WLAN_SCAN_MODE_MLD_SCAN) {
            /* 修改扫描参数为sta的背景扫描 */
        ...
        oam_warning_log1(0, OAM_SF_SCAN, "{hmac_scan_update_scan_params::vap mode[%d], not use bg scan}", en_vap_mode);
        return OAL_FAIL;
    }
    return OAL_SUCC;
}
oal_bool_enum_uint8 hmac_scan_is_ap_up_connect_user(mac_device_stru *mac_device)
{
    uint32_t i;
    uint32_t up_vap_num;
 ...
 */
void hmac_scan_set_scan_params_when_ap_up(mac_device_stru *mac_device,
    mac_scan_req_stru *scan_params)
{
    if (hmac_scan_is_ap_up_connect_user(mac_device) == OAL_FALSE) {
        return;
    }
    scan_params->uc_scan_channel_interval = 1;
    scan_params->uc_max_scan_count_per_channel = 1;
    scan_params->bit_is_ap_up_has_user = OAL_TRUE;
}
...
OAL_STATIC uint32_t hmac_scan_update_up_vap_scan_params(hmac_vap_stru *hmac_vap, mac_scan_req_stru *scan_params)
{
    ...
    if ((ret == OAL_SUCC) && (mac_vap_temp != NULL)) {
        ...
        hmac_scan_set_scan_params_when_ap_up(mac_device, scan_params);
    }
    return OAL_SUCC;
}
 ...
 *   修改内容  : 新生成函数
 */
uint32_t hmac_scan_update_scan_params(hmac_vap_stru *hmac_vap, mac_scan_req_stru *scan_params,
                                      oal_bool_enum_uint8 en_is_random_mac_addr_scan)
{
    ...
    if (hmac_vap->st_vap_base_info.en_vap_state == MAC_VAP_STATE_STA_WAIT_MLD_SCAN) {
        scan_params->en_scan_mode = WLAN_SCAN_MODE_MLD_SCAN;
        oam_warning_log0(0, OAM_SF_SCAN, "{hmac_scan_update_scan_params::WLAN_SCAN_MODE_MLD_SCAN}");
    }
#endif
    ...
    oal_bool_enum_uint8 is_high_prio_scan)
{
    ...
    /* 同信道定时监听也视为一种扫描, 在其完成前不可发起新的扫描 */
    if (mac_normal_same_remain_on_chan_is_running(remain_on_chan_info) == OAL_TRUE) {
        oam_warning_log0(hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_SCAN,
            "{hmac_scan_check_is_dispatch_scan_req:same_chan_remain_on_chan is running!}");
        return OAL_FAIL;
    }
    ...
    if (hmac_mlsr_is_link_switching() == OAL_TRUE) {
        oam_warning_log0(hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_SCAN,
            "{hmac_scan_check_is_dispatch_scan_req::mlsr link is switching,reject scan.}");
        return OAL_FAIL;
    }
#endif
 ...
 *   修改内容  : 新生成函数
 */
OAL_STATIC void hmac_scan_proc_last_scan_record(hmac_vap_stru *hmac_vap, hmac_scan_stru *scan_mgmt)
{
    /* 本次扫描请求发起时，清除上一次扫描结果中过期的bss信息 */
    hmac_scan_clean_expire_scanned_bss(hmac_vap, &(scan_mgmt->st_scan_record_mgmt));
 ...
 *   修改内容  : 新生成函数
 */
uint32_t hmac_scan_proc_scan_timeout_fn(void *arg)
{
    hmac_scan_stru *scan_mgmt = (hmac_scan_stru *)arg;
    hmac_vap_stru *hmac_vap = NULL;
    hmac_scan_record_stru *scan_record = NULL;
    uint32_t pedding_data = 0;

    /* 获取扫描记录信息 */
    scan_record = &(scan_mgmt->st_scan_record_mgmt);
    ...
    if (scan_record->p_fn_cb != NULL) {
        ...
        /* 避免扫描完成中断上报至host 同时 timeout，重复调用回调 */
...
OAL_STATIC void hmac_scan_proc_clear_last_scan_record(mac_scan_req_stru *scan_params,
    hmac_vap_stru *hmac_vap, hmac_scan_stru *scan_mgmt)
{
    if (oal_any_null_ptr3(scan_params, hmac_vap, scan_mgmt)) {
        return;
    }
    ...
    if (hmac_get_feature_switch(HMAC_MIRACAST_SINK_SWITCH)) {
        if (scan_params->uc_scan_func & MAC_SCAN_FUNC_P2P_LISTEN) {
            ...
            hmac_scan_proc_last_scan_record(hmac_vap, scan_mgmt);
        }
    } else {
        /* 处理上一次扫描记录，目前直接清楚上一次结果，后续可能需要老化时间处理 */
        hmac_scan_proc_last_scan_record(hmac_vap, scan_mgmt);
...
void hmac_scan_param_convert_ex(mac_scan_req_ex_stru *scan_req_ex, mac_scan_req_stru *scan_params)
{
        ...
        scan_req_ex->mld_id = scan_params->mld_id;
...
void hmac_scan_update_scan_record(hmac_vap_stru *hmac_vap, mac_scan_req_stru *scan_params,
                                  hmac_scan_stru *scan_mgmt)
{
    hmac_scan_record_stru *scan_record = NULL;
   /* 设置扫描模块处于扫描状态，其它扫描请求将丢弃 */
    scan_mgmt->en_is_scanning = OAL_TRUE;

    hmac_scan_proc_clear_last_scan_record(scan_params, hmac_vap, scan_mgmt);

    /* 记录扫描发起者的信息，某些模块回调函数使用 */
    scan_record = &(scan_mgmt->st_scan_record_mgmt);
    scan_record->uc_chip_id = hmac_vap->st_vap_base_info.uc_chip_id;
    scan_record->uc_device_id = hmac_vap->st_vap_base_info.uc_device_id;
...
static void hmac_scan_req_fsm_change_state(hmac_vap_stru *hmac_vap, mac_scan_req_stru *scan_params)
{
    ...
    temp_flag = ((hmac_vap->st_vap_base_info.en_vap_mode == WLAN_VAP_MODE_BSS_STA) &&
                 ...
                 (hmac_vap->st_vap_base_info.en_vap_state != MAC_VAP_STATE_UP) &&
                 (hmac_vap->st_vap_base_info.en_vap_state != MAC_VAP_STATE_STA_WAIT_MLD_SCAN));
    if (temp_flag) {
...
void hmac_scan_req_event_scan_timeout_mechanism_set(mac_scan_req_stru *scan_params, hmac_vap_stru *hmac_vap,
    hmac_scan_stru *scan_mgmt)
{
    ...
    if ((scan_params->bit_is_ap_up_has_user == OAL_TRUE) && (scan_params->uc_scan_func != MAC_SCAN_FUNC_P2P_LISTEN)) {
        scan_timeout += WLAN_AP_USER_CONNECT_EXTRA_WAIT_TIME;
    }
#ifdef _PRE_WLAN_FEATURE_ML
    // 如果是ml scan扫描，扫描超时设置成 3 * 20ms，避免超时时间过长，否则会认为一直是connecting，打断其他操作。
    if (scan_params->en_scan_mode == WLAN_SCAN_MODE_MLD_SCAN) {
        scan_timeout = scan_params->us_scan_time * 3; // 扫描超时设置成 3 * 20ms
    }
#endif
    ...
    frw_timer_create_timer_m(&(scan_mgmt->st_scan_timeout), hmac_scan_proc_scan_timeout_fn,
        scan_timeout, scan_mgmt, OAL_FALSE, OAM_MODULE_ID_HMAC, hmac_vap->st_vap_base_info.core_id);
}
...
uint32_t hmac_scan_proc_scan_req_event(hmac_vap_stru *hmac_vap, void *params)
{
    ...
    hmac_scan_update_scan_record(hmac_vap, scan_params, &(hmac_device->st_scan_mgmt));
    ...
    hmac_scan_req_event_scan_timeout_mechanism_set(scan_params, hmac_vap, &(hmac_device->st_scan_mgmt));
...
uint32_t hmac_scan_proc_sched_scan_req_event(hmac_vap_stru *hmac_vap, void *params)
{
    ...
    /* 清空上一次的扫描结果 */
    hmac_scan_proc_last_scan_record(hmac_vap,  &(hmac_device->st_scan_mgmt));
 ...
 *   修改内容  : 新生成函数
 */
void hmac_scan_init(hmac_scan_stru *scan_mgmt)
{
    hmac_bss_mgmt_stru *bss_mgmt = NULL;
    ...
    /* 初始化扫描生成随机MAC 地址 */
    oal_random_ether_addr(scan_mgmt->auc_random_mac, WLAN_MAC_ADDR_LEN);
    return;
}
 ...
 *   修改内容  : 新生成函数
 */
void hmac_scan_exit(hmac_scan_stru *scan_mgmt)
{
    /* 清空扫描记录信息 */
    hmac_scan_clean_scan(scan_mgmt);

    if (scan_mgmt->st_init_scan_timeout.en_is_registerd) {
        frw_timer_immediate_destroy_timer_m(&scan_mgmt->st_init_scan_timeout);
    }
...
/*
 * 函 数 名  : hmac_get_scam_mgmt
 * 功能描述  : 通过hmac vap获取对应的扫描管理结构体
 */
hmac_scan_stru *hmac_get_scam_mgmt(hmac_vap_stru *hmac_vap)
{
    ...
    if (hmac_vap->st_vap_base_info.uc_vap_id == hmac_device->pst_device_base_info->uc_cfg_vap_id) {
        return hmac_get_wfsc_scan_mgmt();
    }
#endif
    return &(hmac_device->st_scan_mgmt);
}
