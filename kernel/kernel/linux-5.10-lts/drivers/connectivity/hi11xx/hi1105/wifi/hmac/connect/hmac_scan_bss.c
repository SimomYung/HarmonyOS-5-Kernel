...
#include "hmac_wur.h"
#endif
#ifdef _PRE_WLAN_FEATURE_11B_PLUS
 ...
 *   修改内容  : 新生成函数
 */
OAL_STATIC uint32_t hmac_scan_add_bss_to_list(hmac_scanned_bss_info *scanned_bss, hmac_bss_mgmt_stru *bss_mgmt)
{
 ...
 *   修改内容  : 新生成函数
 */
static uint32_t hmac_scan_del_bss_from_list_nolock(hmac_scanned_bss_info *scanned_bss, hmac_bss_mgmt_stru *bss_mgmt)
{
...
OAL_STATIC int32_t hmac_is_connected_ap_bssid(uint8_t device_id, uint8_t bssid[WLAN_MAC_ADDR_LEN])
{
    ...
    for (vap_idx = 0; vap_idx < mac_device->uc_vap_num; vap_idx++) {
        ...
        /* 识别当前关联的AP的bssid以及MLD的另外一路link的bssid */
        if (is_legacy_vap(mac_vap) &&
            ((mac_vap->en_vap_state == MAC_VAP_STATE_UP) || (mac_vap->en_vap_state == MAC_VAP_STATE_STA_LINK_DOWN))) {
    ...
    uint8_t *frame_body, uint16_t frame_len)
{
    ...
    if (ie != NULL) {
        ...
        ie = mac_find_ie(MAC_EID_HT_CAP, &ies_info);
        if (ie != NULL) {
...
OAL_STATIC void hmac_scan_update_bss_list_country(mac_bss_dscr_stru *bss_dscr, uint8_t *frame_body, uint16_t frame_len)
{
    ...
    ie = mac_find_ie(MAC_EID_COUNTRY, &ies_info);
    /* 国家码不存在, 全部标记为0 */
    if (ie == NULL) {
...
OAL_STATIC void hmac_scan_update_bss_list_1024qam(mac_bss_dscr_stru *bss_dscr, uint8_t *frame_body, uint16_t frame_len)
{
    ...
    ies_info.frame_ies = frame_body;
    ies_info.frame_len = frame_len;
    ie = mac_find_vendor_ie(MAC_HUAWEI_VENDER_IE, MAC_HISI_1024QAM_IE, &ies_info);
    if (ie == NULL) {
...
OAL_STATIC void hmac_scan_update_11i(mac_bss_dscr_stru *bss_dscr, uint8_t *frame_body, uint16_t frame_len)
{
    ...
    bss_dscr->puc_rsn_ie = mac_find_ie(MAC_EID_RSN, &ies_info);
    bss_dscr->puc_wpa_ie = mac_find_vendor_ie(MAC_WLAN_OUI_MICROSOFT, MAC_OUITYPE_WPA, &ies_info);
}
...
OAL_STATIC void hmac_scan_update_bss_list_11n(mac_bss_dscr_stru *bss_dscr, uint8_t *frame_body, uint16_t frame_len)
{
    ...
    ie = mac_find_ie(MAC_EID_HT_OPERATION, &ies_info);
    ...
    ie = mac_find_ie(MAC_EID_EXT_CAPS, &ies_info);
    if ((ie != NULL) && (ie[1] >= 1)) {
    ...
    uint8_t *frame_body, uint16_t frame_len, uint32_t en_is_vendor_ie, uint8_t vendor_subtype)
{
    ...
    ie = mac_find_ie(MAC_EID_VHT_CAP, &ies_info);
    ...
    uint32_t en_is_vendor_ie, uint8_t vendor_subtype)
{
    ...
    ie = mac_find_ie(MAC_EID_VHT_OPERN, &ies_info);
    if ((ie != NULL) && (ie[1] >= MAC_VHT_OPERN_LEN)) {
        ...
        } else if (vht_chan_width == WLAN_MIB_VHT_OP_WIDTH_80_160) { /* 80MHz */
            hmac_scan_update_bss_list_11ac_80m(bss_dscr, chan_center_freq, chan_center_freq_1);
#ifdef _PRE_WLAN_FEATURE_160M
        } else if (vht_chan_width == WLAN_MIB_VHT_OP_WIDTH_160_DEPRECATED) { /* 2: 160MHz */
            hmac_scan_update_bss_list_11ac_160m(bss_dscr, chan_center_freq);
#endif
        } else if (vht_chan_width == WLAN_MIB_VHT_OP_WIDTH_80PLUS80_DEPRECATED) { /* 3: 80+80MHz */
            hmac_scan_update_bss_list_11ac_80m_ext(bss_dscr, chan_center_freq);
        }
    }
}
...
void hmac_scan_update_sap_mode(mac_bss_dscr_stru *bss_dscr, uint8_t *payload, uint16_t frame_len)
{
    ...
    /*******************************************************************************************
     ...
     ------------------------------------------------------------------------------------
    *********************************************************************************************/
    sap_ie = mac_find_vendor_ie(MAC_HUAWEI_VENDER_IE, MAC_HISI_SAP_IE, &ies_info);
    if (sap_ie == NULL) {
...
#ifdef _PRE_WLAN_FEATURE_11BE
        if (hisi_vap_ie.bit_11be_support != OAL_FALSE && is_custom_open_11be_switch(&hmac_vap->st_vap_base_info)) {
            bss_dscr->eht_capable = hisi_vap_ie.bit_11be_support;
        }
#endif
#ifdef _PRE_WLAN_FEATURE_WUR
        if (hisi_vap_ie.bit_wur_support != OAL_FALSE && wlan_chip_get_wur_switch() == OAL_TRUE) {
            bss_dscr->wur_capable = hisi_vap_ie.bit_wur_support;
        }
#endif
#ifdef _PRE_WLAN_FEATURE_EMLSR
        if (hisi_vap_ie.bit_emlsr_support != OAL_FALSE && hmac_get_emlsr_switch() == OAL_TRUE) {
            bss_dscr->hisi_emlsr_capable = hisi_vap_ie.bit_emlsr_support;
        }
#endif
#ifdef _PRE_WLAN_FEATURE_11B_PLUS
        if (hisi_vap_ie.bit_11b_plus_cap != 0 && hisi_priv_cap->bit_11b_plus_cap != 0) {
            bss_dscr->hisi_11bplus_cap = oal_min(hisi_vap_ie.bit_11b_plus_cap, hisi_priv_cap->bit_11b_plus_cap);
        }
#endif
    ...
    mac_bss_dscr_stru *bss_dscr, uint8_t *frame_body, uint16_t frame_len)
{
    ...
    if (g_wlan_spec_cfg->feature_11be_is_open) {
        hmac_scan_update_11be_ie(hmac_vap, bss_dscr, frame_body, frame_len);
    }
#endif

#ifdef _PRE_WLAN_FEATURE_ML
    hmac_scan_update_multi_link_cap(hmac_vap, bss_dscr, frame_body, frame_len);
#endif
...
void hmac_scan_btcoex_backlist_check_by_oui(mac_bss_dscr_stru *bss_dscr, uint8_t *frame_body, uint16_t frame_len)
{
    ...
    if (mac_find_vendor_ie(MAC_WLAN_CHIP_OUI_RALINK, MAC_WLAN_CHIP_OUI_TYPE_RALINK, &ies_info) != NULL ||
        mac_find_vendor_ie(MAC_WLAN_CHIP_OUI_SHENZHEN, MAC_WLAN_CHIP_OUI_TYPE_SHENZHEN, &ies_info) != NULL ||
        mac_find_vendor_ie(MAC_WLAN_CHIP_OUI_BROADCOM, MAC_WLAN_CHIP_OUI_TYPE_BROADCOM, &ies_info) != NULL ||
        mac_find_vendor_ie(MAC_WLAN_CHIP_OUI_ATHEROSC, MAC_WLAN_CHIP_OUI_TYPE_ATHEROSC, &ies_info) != NULL ||
        mac_find_vendor_ie(MAC_WLAN_CHIP_OUI_REALTEK, MAC_WLAN_CHIP_OUI_TYPE_REALTEK, &ies_info) != NULL ||
        mac_find_vendor_ie(MAC_WLAN_CHIP_OUI_METALINK, MAC_WLAN_CHIP_OUI_TYPE_METALINK, &ies_info) != NULL) {
        bss_dscr->en_btcoex_blacklist_chip_oui |= MAC_BTCOEX_BLACKLIST_LEV0;
    }

    /*  判断是否为苹果AP */
    if (((mac_find_vendor_ie(MAC_WLAN_CHIP_OUI_APPLE1, MAC_WLAN_CHIP_OUI_TYPE_APPLE_1_1, &ies_info) != NULL) ||
        (mac_find_vendor_ie(MAC_WLAN_CHIP_OUI_APPLE1, MAC_WLAN_CHIP_OUI_TYPE_APPLE_1_2, &ies_info) != NULL) ||
        (mac_find_vendor_ie(MAC_WLAN_CHIP_OUI_APPLE2, MAC_WLAN_CHIP_OUI_TYPE_APPLE_2_1, &ies_info) != NULL)) &&
        (mac_find_vendor_ie(MAC_WLAN_CHIP_OUI_BROADCOM, MAC_WLAN_CHIP_OUI_TYPE_BROADCOM, &ies_info) != NULL)) {
        bss_dscr->en_btcoex_blacklist_chip_oui |= MAC_BTCOEX_BLACKLIST_LEV1;
    }
}

OAL_STATIC OAL_INLINE void hmac_scan_update_bss_by_more_chip_oui(mac_bss_dscr_stru *bss_dscr,
    uint8_t *frame_body, uint16_t frame_len)
{
    mac_ies_info ies_info = {0};
    ...
    if (mac_find_vendor_ie(MAC_WLAN_CHIP_OUI_HUAWEI, MAC_WLAN_CHIP_OUI_TYPE_1_HUAWEI, &ies_info) != NULL) {
        if (mac_find_vendor_ie(MAC_WLAN_CHIP_OUI_HUAWEI, MAC_WLAN_CHIP_OUI_TYPE_2_HUAWEI, &ies_info) == NULL) {
            bss_dscr->agc_compability_ap = OAL_TRUE;
        }
    }
}
                                                            ...
                                                            uint8_t *frame_body, uint16_t frame_len)
{
    ...
    if (mac_find_vendor_ie(MAC_WLAN_CHIP_OUI_ATHEROSC, MAC_WLAN_CHIP_OUI_TYPE_ATHEROSC, &ies_info) != NULL) {
        bss_dscr->en_atheros_chip_oui = OAL_TRUE;
    } else {
        bss_dscr->en_atheros_chip_oui = OAL_FALSE;
    }

    if (mac_find_vendor_ie(MAC_WLAN_CHIP_OUI_BROADCOM, MAC_WLAN_CHIP_OUI_TYPE_BROADCOM, &ies_info) != NULL) {
        bss_dscr->en_roam_blacklist_chip_oui = OAL_TRUE;
    } else {
        bss_dscr->en_roam_blacklist_chip_oui = OAL_FALSE;
    }

    if (mac_find_vendor_ie(MAC_WLAN_CHIP_OUI_MARVELL, MAC_WLAN_CHIP_OUI_TYPE_MARVELL, &ies_info) != NULL) {
    ...
    } else if (mac_find_vendor_ie(MAC_WLAN_CHIP_OUI_BROADCOM, MAC_WLAN_CHIP_OUI_TYPE_BROADCOM, &ies_info) != NULL) {
        bss_dscr->chip_oui = WLAN_AP_CHIP_OUI_BCM;
    } else if (mac_find_vendor_ie(MAC_WLAN_CHIP_OUI_QUALCOMM, MAC_WLAN_CHIP_OUI_TYPE_QUALCOMM, &ies_info) != NULL) {
    ...
    if (mac_find_vendor_ie(MAC_WLAN_CHIP_OUI_RALINK, MAC_WLAN_CHIP_OUI_TYPE_RALINK_2, &ies_info) != NULL &&
        mac_find_vendor_ie(MAC_WLAN_CHIP_OUI_MTK, MAC_WLAN_CHIP_OUI_TYPE_MTK, &ies_info) != NULL) {
        bss_dscr->keepalive_compatible_chip_oui = OAL_TRUE;
    }
    hmac_scan_update_bss_by_more_chip_oui(bss_dscr, frame_body, frame_len);
}

/*
 * 函 数 名  : hmac_scan_update_bss_list_dualband_roam_cap
 * 功能描述  : 判断是否支持双频漫游
 * 1.日    期  : 2024年4月7日
 *   修改内容  : 新生成函数
 */
static void hmac_scan_update_bss_list_dualband_roam_cap(mac_bss_dscr_stru *bss_dscr,
    uint8_t *frame_body, uint16_t frame_len)
{
    uint8_t *ie = NULL;
    ...
    bss_dscr->dualband_roam_cap = OAL_FALSE;
    bss_dscr->supp_eht_double_wifi = OAL_FALSE;
    ies_info.frame_ies = frame_body;
    ies_info.frame_len = frame_len;
    ie = mac_find_vendor_ie(MAC_WLAN_OUI_HUAWEI, MAC_WLAN_OUI_TYPE_HAUWEI_COWORK, &ies_info);
    if ((ie == NULL) || ((ie[1] + MAC_IE_HDR_LEN) <
        (sizeof(oal_sta_ap_cowork_ie_beacon) - sizeof(oal_cowork_ie_dc_roaming_stru)))) {
        return;
    }
    cowork_ie_beacon = (oal_sta_ap_cowork_ie_beacon *)ie;
#ifdef _PRE_WLAN_FEATURE_ML
    /* 自身和对端都支持EHT，并且定制化开关使能对端也支持端管则置位能力 */
    if ((g_wlan_spec_cfg->feature_11be_is_open == OAL_TRUE) && bss_dscr->eht_capable == OAL_TRUE &&
        hmac_mld_enable_eht_double_wifi_disbale_mlo_cap() == OAL_TRUE &&
        cowork_ie_beacon->capa_bitmap.supp_double_wifi == OAL_TRUE) {
        bss_dscr->supp_eht_double_wifi = OAL_TRUE;
    }
#endif
    if (cowork_ie_beacon->capa_bitmap.mdid_flag == OAL_TRUE) {
        return;
    }
    if (cowork_ie_beacon->capa_bitmap.dyn_data.cowork_cap_ex.support_zero_roaming == OAL_TRUE) {
        bss_dscr->dualband_roam_cap = OAL_TRUE;
    }
}
 ...
 * 1.日    期  : 2013年6月25日
  *   修改内容  : 新生成函数
 */
void hmac_scan_update_bss_list_protocol(hmac_vap_stru *hmac_vap, mac_bss_dscr_stru *bss_dscr,
    uint8_t *frame_body, uint16_t frame_len_input)
{
    uint8_t *ie = NULL;
    uint16_t offset_vendor_vht = MAC_WLAN_OUI_VENDOR_VHT_HEADER + MAC_IE_HDR_LEN;
    mac_ies_info ies_info = {0};
    uint16_t frame_len = frame_len_input;
    ...
    /* 查找私有vendor ie */
    ie = mac_find_vendor_ie(MAC_WLAN_OUI_BROADCOM_EPIGRAM, MAC_WLAN_OUI_VENDOR_VHT_TYPE, &ies_info);
    ...
    hmac_scan_update_bss_list_dualband_roam_cap(bss_dscr, frame_body, frame_len);
 ...
 * 1.日    期  : 2013年7月25日
  *   修改内容  : 新生成函数
 */
uint32_t hmac_scan_update_bss_list_rates(mac_bss_dscr_stru *bss_dscr, uint8_t *frame_body,
    uint16_t frame_len, mac_device_stru *mac_dev)
{
    ...
    if (oal_any_null_ptr3(bss_dscr, frame_body, mac_dev)) {
        return OAL_FAIL;
    }

    /* 设置Beacon帧的field偏移量 */
    offset = MAC_TIME_STAMP_LEN + MAC_BEACON_INTERVAL_LEN + MAC_CAP_INFO_LEN;
    ies_info.frame_ies = frame_body + offset;
    ies_info.frame_len = frame_len - offset;

    ie = mac_find_ie(MAC_EID_RATES, &ies_info);
    if (ie != NULL) {
    ...
    ie = mac_find_ie(MAC_EID_XRATES, &ies_info);
    if (ie != NULL) {
...
OAL_STATIC void hmac_scan_update_bss_dscr_basic_info(mac_device_stru *mac_device, hmac_vap_stru *hmac_vap,
    mac_rx_ctl_stru *rx_ctrl, uint8_t *frame_body, uint16_t frame_body_len, uint16_t frame_len,
    mac_scanned_result_extend_info_stru *scan_result_extend_info, mac_bss_dscr_stru *bss_dscr)
{
    ...
    /* 获取管理帧中的信道 */
    frame_channel = mac_ie_get_chan_num(frame_body, frame_body_len, offset,
                                        rx_ctrl->uc_channel_number, rx_ctrl->is_6ghz_flag);
    ...
    oal_netbuf_stru *netbuf, uint16_t buf_len, hmac_scanned_bss_info *scanned_bss, uint8_t ext_len)
{
    ...
    if (frame_len <= offset) {
        oam_error_log1(0, OAM_SF_SCAN, "hmac_scan_update_bss_dscr::frame_len err[%d]!", frame_len);
        return OAL_FAIL;
    }
    ...
    /* 拷贝管理帧内容 */
    if (memcpy_s(scanned_bss->st_bss_dscr_info.auc_mgmt_buff, (uint32_t)frame_len, mgmt_frame,
        (uint32_t)frame_len) != EOK) {
        return OAL_FAIL;
    }
    ...
    if (ext_len != DEFAULT_AKM_VALUE) {
        ...
        scanned_bss->st_bss_dscr_info.ext_cowork_ie = OAL_TRUE;
    }
#endif
...
/*
 * 函 数 名  : hmac_check_another_vap_adaptive11r
 * 功能描述  : 双STA且AP支持双频漫游能力时检验另一个vap是否支持11r
 * 1.日    期  : 2024年6月28日
 *   作    者  : wifi
 *   修改内容  : 新生成函数
 */
OAL_STATIC oal_bool_enum_uint8 hmac_check_another_vap_adaptive11r(hmac_vap_stru *hmac_vap,
    oal_netbuf_stru *netbuf, uint16_t mgmt_len)
{
    uint8_t *mgmt_frame = (uint8_t *)oal_netbuf_data(netbuf);
    ...
    hmac_vap_stru *another_up_hvap = NULL;
    oal_sta_ap_cowork_ie_beacon *cowork_ie = NULL;
    mac_ies_info ies_info = {0};

    if (mgmt_len < len_offset) {
        return OAL_FALSE;
    }
    ies_info.frame_ies = mgmt_frame + len_offset;
    ies_info.frame_len = mgmt_len - len_offset;

    cowork_ie = (oal_sta_ap_cowork_ie_beacon *)mac_find_vendor_ie(MAC_WLAN_OUI_HUAWEI,
        MAC_WLAN_OUI_TYPE_HAUWEI_COWORK, &ies_info);
    if ((cowork_ie == NULL) || ((cowork_ie->ie_len + MAC_IE_HDR_LEN) <
        (sizeof(oal_sta_ap_cowork_ie_beacon) - sizeof(oal_cowork_ie_dc_roaming_stru)))) {
        return OAL_FALSE;
    }

    if (mac_is_dual_sta_mode() != OAL_TRUE || cowork_ie->capa_bitmap.mdid_flag == OAL_TRUE) {
        return OAL_FALSE;
    }

    mac_dev = mac_res_get_dev(hmac_vap->st_vap_base_info.uc_device_id);
    if (mac_dev == NULL) {
        return OAL_FALSE;
    }
    another_up_vap = mac_device_find_another_up_vap(mac_dev, hmac_vap->st_vap_base_info.uc_vap_id);
    if (another_up_vap == NULL) {
        return OAL_FALSE;
    }
    another_up_hvap = mac_res_get_hmac_vap(another_up_vap->uc_vap_id);
    if (another_up_hvap == NULL) {
        return OAL_FALSE;
    }
    return another_up_hvap->bit_adaptive11r;
}

oal_bool_enum_uint8 hmac_scan_check_ap_support_mlo(oal_netbuf_stru *netbuf, uint16_t mgmt_len)
{
    uint8_t *ml_ie = NULL;
    uint8_t *data = oal_netbuf_data(netbuf);
    ...
    ies_info.frame_ies = data + offset;
    ies_info.frame_len = mgmt_len - offset;

    ml_ie = mac_find_ie_ext_ie(MAC_EID_MULTILINK, MAC_EID_EXT_EHT_ML, &ies_info);

    return (ml_ie == NULL) ? OAL_FALSE : OAL_TRUE;
}

oal_bool_enum_uint8 hmac_scan_check_ap_support_eht_double_wifi(oal_netbuf_stru *netbuf,
    uint16_t mgmt_len)
{
#ifdef _PRE_WLAN_FEATURE_ML
    uint8_t *ie = NULL;
    ...
    if (mgmt_len < offset || (g_wlan_spec_cfg->feature_11be_is_open == OAL_FALSE) ||
        hmac_mld_enable_eht_double_wifi_disbale_mlo_cap() == OAL_FALSE) {
        return OAL_FALSE;
    }
    ...
    if ((ie[1] + MAC_IE_HDR_LEN) <
        (sizeof(oal_sta_ap_cowork_ie_beacon) - sizeof(oal_cowork_ie_dc_roaming_stru))) {
        return OAL_FALSE;
    }
    cowork_ie_beacon = (oal_sta_ap_cowork_ie_beacon *)ie;
    return cowork_ie_beacon->capa_bitmap.supp_double_wifi;
#else
    return OAL_FALSE;
#endif
}
...
{
    uint16_t mgmt_len;
    oal_bool_enum_uint8 another_cap_adaptive11r = OAL_FALSE;

    /* 管理帧的长度等于上报的netbuf的长度减去上报的扫描结果的扩展字段的长度 */
    mgmt_len = buf_len - wlan_chip_get_scaned_payload_extend_len();
#ifdef _PRE_WLAN_FEATURE_ADAPTIVE11R
    /* 为避免和5x路由器密钥协商失败导致关联失败问题，STA支持MLD时不扩充MLD AP的RSN IE;
     * AP如果支持WIFI7端管保持原逻辑，驱动关闭MLD cap，正常填充rsn即可
     */
    if (hmac_vap->bit_11r_mlo_enable == OAL_FALSE &&
        is_custom_open_mlo_switch(&(hmac_vap->st_vap_base_info)) == OAL_TRUE &&
        hmac_scan_check_ap_support_mlo(netbuf, mgmt_len) == OAL_TRUE &&
        hmac_scan_check_ap_support_eht_double_wifi(netbuf, mgmt_len) == OAL_FALSE) {
        return mgmt_len;
    }

    another_cap_adaptive11r = hmac_check_another_vap_adaptive11r(hmac_vap, netbuf, mgmt_len);
    /* adaptive 11R模式下检查是否需要在RSN IE中添加akm suite */
    if (hmac_vap->bit_adaptive11r == OAL_TRUE || another_cap_adaptive11r == OAL_TRUE) {
...
uint8_t hmac_scan_check_chan(oal_netbuf_stru *netbuf, hmac_scanned_bss_info *scanned_bss)
{
    ...
    curr_chan = rx_ctrl->uc_channel_number;
    ...
    ies_info.frame_ies = frame_body + offset;
    ies_info.frame_len = frame_body_len - offset;

    /* 在DSSS Param set ie中解析chan num */
    ie_start_addr = mac_find_ie(MAC_EID_DSPARMS, &ies_info);
    ...
    ie_start_addr = mac_find_ie(MAC_EID_HT_OPERATION, &ies_info);
    ...
    mac_multi_bssid_frame_info_stru *mbss_frame_info, hmac_scanned_bss_info *new_scanned_bss)
{
    ...
    if (g_wlan_spec_cfg->feature_11ax_is_open) {
        ...
        if (is_custom_open_multi_bssid_switch(&hmac_vap->st_vap_base_info)) {
            ...
            new_scanned_bss->st_bss_dscr_info.st_mbssid_info.bssid_index = mbss_frame_info->uc_bssid_index;
        }
    }
#endif
}
                                           ...
                                           uint16_t buf_len, mac_multi_bssid_frame_info_stru *mbss_frame_info)
{
    ...
    hmac_scan_del_bss_from_list_nolock(old_scanned_bss, bss_mgmt);
    /* 解锁 */
    oal_spin_unlock(&(bss_mgmt->st_lock));
    /* 释放内存 */
...
add_bss:
    ...
    /* 将扫描结果添加到链表中 */
    hmac_scan_add_bss_to_list(new_scanned_bss, bss_mgmt);
    /* 释放上报的bss信息和beacon或者probe rsp帧的内存 */
    oal_netbuf_free(netbuf);
...
uint32_t hmac_scan_proc_scanned_bss(frw_event_mem_stru *event_mem)
{
    ...
    uint32_t ret;
    uint16_t min_len = MAC_80211_FRAME_LEN + MAC_TIME_STAMP_LEN + MAC_BEACON_INTERVAL_LEN + MAC_CAP_INFO_LEN +
                       wlan_chip_get_scaned_payload_extend_len();
    ...
    if (dtx_event->us_frame_len <= min_len) {
        oam_error_log1(0, 0, "{hmac_scan_proc_scanned_bss::frame_len[%d] too short.}", dtx_event->us_frame_len);
        return OAL_FAIL;
    }
#ifdef _PRE_WLAN_FEATURE_11AX
    if (g_wlan_spec_cfg->feature_11ax_is_open) {
...
#endif
#ifdef _PRE_WLAN_FEATURE_ML
    hmac_ml_scan_result_handle(hmac_vap, bss_mgmt_netbuf, dtx_event->us_frame_len);
#endif
