 ...
 * 1.日    期  : 2013年7月1日
  *   修改内容  : 新生成函数
 */
OAL_STATIC void hmac_prepare_scan_req(hmac_vap_stru *hmac_vap, mac_scan_req_stru *scan_params, int8_t *desired_ssid)
{
...
OAL_STATIC void hmac_cfg80211_scan_comp_cb(void *record)
{
    ...
    if (roam_info != NULL && scan_mgmt != NULL &&
        (hmac_roam_main_check_state(roam_info, MAC_VAP_STATE_UP, ROAM_MAIN_STATE_SCANING) == OAL_SUCC)) {
        oam_warning_log0(hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_SCAN, "{hmac_cfg80211_scan_comp_cb::roam scan.");
        hmac_roam_scan_comp_cb(&(scan_mgmt->st_scan_record_mgmt));
        return;
    }
...
uint32_t hmac_sta_initiate_scan(mac_vap_stru *mac_vap, uint16_t len, uint8_t *param)
{
    ...
    hmac_prepare_scan_req(hmac_vap, scan_params, desired_ssid);
    mac_device = mac_res_get_dev(mac_vap->uc_device_id);
    ...
    /* 记录扫描参数信息到device结构体 */
    memcpy_s(&(mac_device->st_scan_params), sizeof(mac_scan_req_stru), scan_params, sizeof(mac_scan_req_stru));
    /* 状态机调用: hmac_scan_proc_scan_req_event 或 hmac_scan_proc_scan_req_event_exception */
...
void hmac_cfg80211_free_scan_param_resource(mac_cfg80211_scan_param_stru *cfg80211_scan_param)
{
...
uint32_t hmac_find_p2p_listen_channel(mac_vap_stru *pst_mac_vap, uint16_t us_len, uint8_t *puc_param)
{
    ...
    ies_info.frame_ies = puc_param;
    ies_info.frame_len = us_len;
    ...
    if ((int32_t)((puc_listen_channel_ie[BYTE_OFFSET_2] << NUM_8_BITS) +
        puc_listen_channel_ie[1]) != MAC_P2P_LISTEN_CHN_ATTR_LEN) {
        oam_warning_log1(pst_mac_vap->uc_vap_id, OAM_SF_CFG,
...
void hmac_cfg80211_pre_start_scan(mac_scan_req_stru *scan_params, mac_vap_stru *mac_vap,
    mac_cfg80211_scan_param_stru *cfg80211_scan_param)
{
...
mac_cfg80211_scan_param_stru* hmac_cfg80211_get_scan_param(mac_vap_stru *mac_vap, uint8_t *param)
{
...
void hmac_handle_scan_rsp_sta(hmac_vap_stru *hmac_vap, uint8_t *msg)
{
    ...
    if (memcpy_s(scan_rsp, sizeof(hmac_scan_rsp_stru), msg, sizeof(hmac_scan_rsp_stru)) != EOK) {
...
void hmac_handle_mld_scan_rsp_sta(hmac_vap_stru *hmac_vap, uint8_t *msg)
{
#ifdef _PRE_WLAN_FEATURE_ML
    ...
    bss_dscr_tmp = (mac_bss_dscr_stru *)hmac_scan_get_scanned_bss_by_bssid(&hmac_vap->st_vap_base_info,
        hmac_vap->ml_scan_param.sta_mac_addr);
    if (bss_dscr_tmp == NULL) { // 没找到另一个link，重传次数允许下，再进行单播扫描
        oam_warning_log1(0, OAM_SF_ASSOC, "hmac_handle_mld_scan_rsp_sta::no link bss, scan_req_cnt[%d]!",
            hmac_vap->ml_scan_param.scan_req_cnt);
        if (hmac_vap->ml_scan_param.scan_req_cnt < ML_SCAN_MAX_TIME) {
            hmac_sta_initiate_scan(&hmac_vap->st_vap_base_info, sizeof(uint8_t), &is_p2p0_scan);
            return;
        }
    } else {
        oam_warning_log0(0, OAM_SF_ASSOC, "hmac_handle_mld_scan_rsp_sta::find link bss, inform it!");
        hmac_handle_scan_rsp_sta(hmac_vap, msg);
    }

    if (hmac_vap->st_vap_base_info.en_vap_state == MAC_VAP_STATE_STA_WAIT_MLD_SCAN) {
        /* MLD扫描状态，直接关联 */
        mac_vap_state_change(&hmac_vap->st_vap_base_info, hmac_vap->ml_scan_param.vap_state_bak);
        hmac_config_connect(&hmac_vap->st_vap_base_info, sizeof(hmac_conn_param_stru),
                            (uint8_t *)&hmac_vap->ml_scan_param.connect_param);
    } else if (hmac_vap->st_vap_base_info.en_vap_state == MAC_VAP_STATE_UP) {
        /* 漫游MLD扫描处理 */
        scan_mgmt = hmac_get_scam_mgmt(hmac_vap);
        if (scan_mgmt != NULL) {
            hmac_roam_scan_comp_cb(&(scan_mgmt->st_scan_record_mgmt));
        ...
        oam_error_log1(hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_ASSOC,
            "hmac_handle_mld_scan_rsp_sta::invalid vap state %d!",
    ...
    uint8_t *peer_addr)
{
    ...
    if (hmac_11r_report_connect_failed_result(hmac_vap) == OAL_SUCC) {
        return;
    }
#endif
...
static uint32_t hmac_prepare_external_auth_req_ext(hmac_vap_stru *hmac_vap, hmac_external_auth_req_stru *ext_auth_req)
{
    int32_t ret;
    if (hmac_vap_is_ml_mode(hmac_vap) == OAL_TRUE) {
        ...
        ret = memcpy_s(ext_auth_req->ap_mld_addr, WLAN_MAC_ADDR_LEN,
            hmac_vap->mld_vap_link_info.mld_vap_manage->mld_bssid, WLAN_MAC_ADDR_LEN);
        if (ret != EOK) {
            oam_error_log0(0, OAM_SF_SAE, "hmac_prepare_external_auth_req_ext::memcpy fail!");
            return OAL_FAIL;
        }
    }
    return OAL_SUCC;
...
#if (defined(_PRE_WLAN_FEATURE_ML) && defined(CONFIG_HISI_WIFI7_MLO))
    ret += hmac_prepare_external_auth_req_ext(hmac_vap, &ext_auth_req);
#endif
    if (ret != OAL_SUCC) {
        return ret;
    }
...
void hmac_handle_auth_rsp_sta(hmac_vap_stru *hmac_vap, uint8_t *msg)
{
    ...
    if ((auth_rsp->us_status_code == MAC_UNSUPT_ALG ||
        mac_mib_get_StaAuthCount(&hmac_vap->st_vap_base_info) >= MAX_AUTH_CNT) &&
        (mac_mib_get_AuthenticationMode(&hmac_vap->st_vap_base_info) == WLAN_WITP_AUTH_AUTOMATIC)) {
...
void hmac_handle_asoc_rsp_sta(hmac_vap_stru *hmac_vap, uint8_t *msg)
{
    ...
    if (asoc_rsp->en_result_code == HMAC_MGMT_SUCCESS) {
        ...
        oam_warning_log2(hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_MLOG,
