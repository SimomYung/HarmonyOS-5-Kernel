...
uint32_t  hmac_wifi6_self_cure_close_htc_handle(mac_vap_stru *p_mac_vap)
{
    ...
    if (p_hmac_user->c_rssi > WLAN_FAR_DISTANCE_RSSI) {
        ...
        hmac_roam_start(p_hmac_vap, &roam_start_params);
    }
    return OAL_SUCC;
}
 ...
 *   修改内容  : 新生成函数
 */
oal_bool_enum_uint8 hmac_wifi6_self_cure_mac_is_wifi6_blacklist_type(uint8_t *mac)
{
    uint8_t loop;
    hmac_wifi6_self_cure_info_stru *p_save_info       = NULL;
    ...
    for (loop = 0; loop < p_save_info->blaclist_num; loop++) {
        ...
        if (!OAL_MEMCMP(p_wifi6_list->user_mac_addr, mac, WLAN_MAC_ADDR_LEN) &&
 ...
 *   修改内容  : 新生成函数
 */
void hmac_wifi6_self_cure_init(hmac_vap_stru *hmac_vap)
{
