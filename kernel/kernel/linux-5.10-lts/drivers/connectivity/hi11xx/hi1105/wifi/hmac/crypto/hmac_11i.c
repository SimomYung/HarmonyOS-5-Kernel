...
{
    // DPA模式下需要抛事件
    dpe_hmac_user_clear_ptk_rx_pn(hmac_user->st_user_base_info.us_assoc_id);
}
void hmac_user_clear_gtk_rx_pn(hmac_user_stru *hmac_user)
{
    // DPA模式下需要抛事件
    dpe_hmac_user_clear_gtk_rx_pn(hmac_user->st_user_base_info.us_assoc_id);
}
/*
 ...
 * 1.日    期  : 2014年1月6日
  *   修改内容  : 新生成函数
 */
uint32_t hmac_11i_ether_type_filter(uint8_t vap_idx, uint16_t user_idx, uint16_t us_ether_type)
{
    hmac_user_stru *hmac_user = mac_res_get_hmac_user(user_idx);
    if (hmac_user == NULL) {
        return OAL_ERR_CODE_SECURITY_USER_INVAILD;
    }
    if (mac_user_get_port(&hmac_user->st_user_base_info) == OAL_FALSE) { /* 判断端口是否打开 */
        ...
        if (oal_byteorder_host_to_net_uint16(ETHER_TYPE_PAE) != us_ether_type) {
#endif
            oam_warning_log1(vap_idx, OAM_SF_WPA,
