...
uint32_t hmac_crypto_tkip_enmic(wlan_priv_key_param_stru *pst_key, oal_netbuf_stru *pst_netbuf)
{
    ...
    if (oal_netbuf_next(pst_netbuf) != NULL) {
        return OAL_ERR_CODE_SECURITY_BUFF_NUM;
    }
    ...
    if (pst_cb == NULL) {
        oam_error_log0(0, OAM_SF_WPA, "{hmac_crypto_tkip_enmic::pst_cb null.}");
        return OAL_ERR_CODE_PTR_NULL;
    }

    /* 4.1 计算mic */
    ret = hmac_crypto_tkip_michael_mic(puc_tx_mic_key, pst_netbuf,
        (mac_ieee80211_frame_stru *)(uintptr_t)mac_get_cb_frame_header_addr(pst_cb), hdrlen,
...
uint32_t hmac_crypto_tkip_demic(wlan_priv_key_param_stru *pst_key, oal_netbuf_stru *pst_netbuf)
{
    ...
    if (((uint8_t)pst_key->cipher) != WLAN_80211_CIPHER_SUITE_TKIP) {
    ...
    /* 1.1 计算整个报文的长度，不处理存在多个netbuf的情况 */
    pktlen = oal_netbuf_len(pst_netbuf);
    if (oal_netbuf_list_next(pst_netbuf) != NULL) {
        return OAL_ERR_CODE_SECURITY_BUFF_NUM;
    }
    ...
    if (pst_cb == NULL) {
        oam_error_log0(0, OAM_SF_WPA, "{hmac_crypto_tkip_demic::pst_cb null.}");
        return OAL_ERR_CODE_PTR_NULL;
    }
    ...
    ret = hmac_crypto_tkip_michael_mic(puc_rx_mic_key, pst_netbuf,
        (mac_ieee80211_frame_stru *)(uintptr_t)mac_get_rx_cb_mac_header_addr(pst_cb),
