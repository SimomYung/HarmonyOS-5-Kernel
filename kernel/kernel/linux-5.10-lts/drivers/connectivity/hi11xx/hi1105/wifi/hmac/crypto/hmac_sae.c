...
{
    return (akm == MAC_RSN_AKM_PSK) ? OAL_TRUE : OAL_FALSE;
}
...
{
    return (akm == MAC_RSN_AKM_SAE_EXT_KEY) ? OAL_TRUE : OAL_FALSE;
}

oal_bool_enum_uint8 hmac_key_mgmt_sae(uint32_t akm)
{
    return ((akm == MAC_RSN_AKM_SAE || akm == MAC_RSN_AKM_SAE_EXT_KEY) ? OAL_TRUE : OAL_FALSE);
}

oal_bool_enum_uint8 hmac_key_mgmt_psk(uint32_t akm)
{
    return (hmac_key_mgmt_psk2(akm) || hmac_key_mgmt_sae8(akm) || hmac_key_mgmt_sae_ext(akm)) ?
        OAL_TRUE : OAL_FALSE;
}

oal_bool_enum_uint8 hmac_key_mgmt_valid(uint32_t akm)
{
    uint32_t i;
    uint32_t key_mgmt_array[] = {
        MAC_RSN_AKM_8021X, MAC_RSN_AKM_PSK, MAC_RSN_AKM_FT_OVER_8021X, MAC_RSN_AKM_FT_PSK,
        MAC_RSN_AKM_PSK_SHA256, MAC_RSN_AKM_SAE, MAC_RSN_AKM_FT_OVER_SAE, MAC_RSN_AKM_FT_OVER_8021X_SHA384,
        MAC_RSN_AKM_OWE, MAC_RSN_AKM_SAE_EXT_KEY
    };
    for (i = 0; i < oal_array_size(key_mgmt_array); i++) {
        if (akm == key_mgmt_array[i]) {
            return OAL_TRUE;
        }
    }
    return OAL_FALSE;
...
{
    uint32_t i;
    uint32_t ft_key_mgmt_array[] = {
        MAC_RSN_AKM_FT_PSK
    };
    for (i = 0; i < oal_array_size(ft_key_mgmt_array); i++) {
        if (akm == ft_key_mgmt_array[i]) {
            return OAL_TRUE;
        }
    }
    return OAL_FALSE;
...
{
    uint32_t i;
    uint32_t support_ft_psk = OAL_FALSE;
    for (i = 0; i < len; i++) {
        if (hmac_key_mgmt_valid(akm[i]) == OAL_FALSE) {
            continue;
        }
        if (hmac_key_mgmt_ft_psk(akm[i]) == OAL_TRUE) {
            support_ft_psk = OAL_TRUE;
            continue;
        } else {
            return OAL_FALSE;
        }
    }
    return support_ft_psk;
}
...
uint32_t hmac_sta_sae_connect_info_wpas(hmac_vap_stru *hmac_sta, hmac_auth_req_stru *auth_req)
{
    ...
    if (hmac_user_ap == NULL) {
        hmac_sta_fill_user_info(hmac_sta, hmac_sta->st_vap_base_info.auc_bssid, &user_info);
        ret = hmac_user_add(&(hmac_sta->st_vap_base_info), &user_info, &us_user_index);
        if (ret != OAL_SUCC) {
...
uint32_t hmac_sta_process_sae_commit(hmac_vap_stru *sta_vap, oal_netbuf_stru *netbuf)
{
    ...
    hmac_user_ap = mac_res_get_hmac_user(sta_vap->st_vap_base_info.us_assoc_vap_id);
    if (hmac_user_ap == NULL) {
        oam_error_log1(sta_vap->st_vap_base_info.uc_vap_id, OAM_SF_AUTH,
            "{hmac_sta_wait_auth_seq2_rx::pst_hmac_user[%d] null.}", sta_vap->st_vap_base_info.us_assoc_vap_id);
        return OAL_ERR_CODE_PTR_NULL;
    }

    rx_ctl = (mac_rx_ctl_stru *)oal_netbuf_cb(netbuf); /* 每一个MPDU的控制信息 */
    if (rx_ctl == NULL) {
        return OAL_ERR_CODE_PTR_NULL;
    }
    ...
    oam_warning_log2(sta_vap->st_vap_base_info.uc_vap_id, OAM_SF_SAE,
        "{hmac_sta_wait_auth_seq2_rx::rx sae auth frame, status_code %d, seq_num %d.}",
        auth_status, auth_seq);
    if (auth_status == MAC_AP_FULL) {
        hmac_sta_process_auth_status_ap_full(sta_vap);
        return OAL_SUCC;
    }

    /* SAE commit帧的seq number是1，confirm帧的seq number是2 */
    if (auth_seq != WLAN_AUTH_TRASACTION_NUM_ONE) {
        oam_warning_log0(sta_vap->st_vap_base_info.uc_vap_id, OAM_SF_SAE,
            "{hmac_sta_wait_auth_seq2_rx::drop sae auth frame for wrong seq num}");
        return OAL_SUCC;
    }

    sta_vap->duplicate_auth_seq4_flag = OAL_FALSE; // seq4置位

    /* 取消定时器 */
    frw_timer_immediate_destroy_timer_m(&sta_vap->st_mgmt_timer);

#ifdef _PRE_WLAN_FEATURE_ML
    hmac_mld_rx_mgmt_change_addr(sta_vap, netbuf);
#endif

    /* SAE 判断seq number以后，上传给wpas 处理 */
    hmac_rx_mgmt_send_to_host(sta_vap, netbuf);
    ...
    if ((auth_status != MAC_ANTI_CLOGGING) && (auth_status != MAC_FINITE_CYCLIC_GROUP_NOT_SUPPORTED)) {
        ...
        hmac_fsm_change_state(sta_vap, MAC_VAP_STATE_STA_WAIT_AUTH_SEQ4);
        sta_vap->st_mgmt_timetout_param.en_state = MAC_VAP_STATE_STA_WAIT_AUTH_SEQ4;
    } else {
        ...
        sta_vap->st_mgmt_timetout_param.en_state = MAC_VAP_STATE_STA_WAIT_AUTH_SEQ2;
    }
    sta_vap->st_mgmt_timetout_param.uc_vap_id = sta_vap->st_vap_base_info.uc_vap_id;
    sta_vap->st_mgmt_timetout_param.us_user_index = hmac_user_ap->st_user_base_info.us_assoc_id;
    frw_timer_create_timer_m(&sta_vap->st_mgmt_timer, hmac_mgmt_timeout_sta, sta_vap->st_mgmt_timer.timeout,
        &sta_vap->st_mgmt_timetout_param, OAL_FALSE, OAM_MODULE_ID_HMAC, sta_vap->st_vap_base_info.core_id);
...
uint32_t hmac_sta_process_sae_confirm(hmac_vap_stru *sta_vap, oal_netbuf_stru *netbuf)
{
    ...
    if (sta_vap->duplicate_auth_seq4_flag == OAL_TRUE) { // wpa3 auth seq4重复帧过滤，禁止上报waps
        oam_warning_log2(sta_vap->st_vap_base_info.uc_vap_id, OAM_SF_SAE,
            "{hmac_sta_wait_auth_seq4_rx::drop sae auth frame, status_code %d, seq_num %d.}",
            auth_status, auth_seq);
        return OAL_SUCC;
    ...
    oam_warning_log2(sta_vap->st_vap_base_info.uc_vap_id, OAM_SF_SAE,
                     ...
                     auth_status, auth_seq);
    sta_vap->duplicate_auth_seq4_flag = OAL_TRUE;
    return OAL_SUCC;
}
 ...
 * 1.日    期  : 2019年1月2日
  *   修改内容  : 新生成函数
 */
uint32_t hmac_config_external_auth(mac_vap_stru *mac_vap, uint16_t len, uint8_t *param)
{
    uint32_t ret;
    hmac_external_auth_req_stru *ext_auth;
    ...
    if (ext_auth->us_status == MAC_UNSPEC_FAIL) {
        ...
        // 漫游时不需要上报上层关联失败，已关联场景上报关联失败，会导致内核connected状态异常
        if (hmac_vap->st_vap_base_info.en_vap_state != MAC_VAP_STATE_ROAMING) {
            ...
            hmac_roam_notify_sae_auth_fail(hmac_vap);
        }
        return OAL_SUCC;
...
void hmac_update_sae_connect_param(hmac_vap_stru *pst_hmac_vap, uint8_t *puc_ie, uint32_t ie_len)
{
    ...
    if (hmac_key_mgmt_sae(st_crypto.aul_akm_suite[0]) == OAL_FALSE &&
        st_crypto.aul_akm_suite[0] != MAC_RSN_AKM_TBPEKE) {
    ...
    if (hmac_key_mgmt_sae(st_crypto.aul_akm_suite[0]) == OAL_TRUE) {
