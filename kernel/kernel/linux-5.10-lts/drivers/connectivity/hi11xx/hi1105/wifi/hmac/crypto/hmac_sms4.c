 ...
 * 修改内容 : 新生成函数
 */
void hmac_sms4_keyext(uint8_t *key, uint32_t *rk, uint32_t rk_len)
{
    uint32_t r;
    uint32_t mid;
    uint32_t x[SBOX_NUM];

    if (memcpy_s(x, sizeof(x), key, sizeof(x)) != EOK) {
        return;
    } /* 注意输入参数地址对齐问题 */
#if (_PRE_LITTLE_CPU_ENDIAN == _PRE_CPU_ENDIAN)
    hmac_sms4_little_endian_convert(x, SBOX_NUM);
