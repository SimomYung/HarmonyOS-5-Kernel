...
hmac_user_stru *hmac_soft_encrypt_get_hmac_user(mac_vap_stru *mac_vap, oal_netbuf_stru *netbuf)
{
    ...
    if (tx_ctl == NULL) {
        oam_error_log0(0, 0, "hmac_soft_encrypt_get_hmac_user:tx_ctl null");
        return NULL;
    }
    mac_hdr = (mac_ieee80211_frame_stru *)(uintptr_t)mac_get_cb_frame_header_addr(tx_ctl);
    if (mac_hdr == NULL) {
        oam_error_log0(0, 0, "hmac_soft_encrypt_get_hmac_user:mac_hdr null");
        return NULL;
    }
...
oal_bool_enum_uint8 hmac_check_soft_encrypt_port(mac_vap_stru *mac_vap, oal_netbuf_stru *netbuf)
{
    ...
    if ((mac_vap->pwl_enable == OAL_TRUE) &&
        ((hmac_user->pwl.port_valid == OAL_TRUE) || (hmac_user->pwl_pre.port_valid == OAL_TRUE))) {
        return OAL_TRUE;
    }
#endif
    return OAL_FALSE;
...
uint32_t hmac_soft_encrypt_netbuf(mac_vap_stru *mac_vap, oal_netbuf_stru *netbuf, oal_netbuf_stru **netbuf_new)
{
    ...
    *netbuf_new = netbuf;
#ifdef _PRE_WLAN_FEATURE_PWL
    if ((mac_vap->pwl_enable == OAL_TRUE) && (mac_vap->soft_encrypt_mode != PWL_HOST_ENCRYPT)) {
        return OAL_SUCC;
    }
#endif
...
void hmac_tx_soft_encrypt_schedule(void)
{
    ...
    while (!oal_netbuf_list_empty(netbuf_header)) {
        ...
        if (tx_ctl == NULL) {
            oam_error_log0(0, OAM_SF_RX, "{hmac_tx_soft_encrypt_schedule::tx_ctl null.}");
            continue;
        }
        mac_hdr = (mac_ieee80211_frame_stru *)(uintptr_t)mac_get_cb_frame_header_addr(tx_ctl);
