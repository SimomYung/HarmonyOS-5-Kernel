...
oal_netbuf_stru *hmac_wapi_encrypt_netbuf(hmac_user_stru *hmac_user, oal_netbuf_stru *netbuf)
{
    ...
    /* buf的初始位置在snap头的llc处 */
    if (mac_get_data_type_from_80211(netbuf, 0, oal_netbuf_get_len(netbuf)) == MAC_DATA_WAPI) {
    ...
    oal_netbuf_stru *netbuf)
{
    ...
    if (tx_ctl == NULL) {
        oam_error_log0(hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_ANY,
                       "{hmac_wapi_netbuf_tx_encrypt_mp16:tx_ctl null}");
        return NULL;
    }
...
static oal_netbuf_stru *hmac_wapi_netbuff_rx_handle(hmac_user_stru *hmac_user, oal_netbuf_stru *buf)
{
    ...
    /* 非加密帧，不进行解密 */
    if (oal_netbuf_data(buf) != NULL && (!((oal_netbuf_data(buf))[1] & 0x40))) {
        return buf;
    }
 ...
 */
oal_netbuf_stru *hmac_wapi_netbuf_rx_decrypt_sw(uint8_t vap_idx, uint16_t user_idx, oal_netbuf_stru *netbuf)
{
    oal_netbuf_stru *netbuf_ret;
    ...
    if (hmac_vap == NULL || hmac_user == NULL || rx_ctrl == NULL) {
        return NULL;
    }
    ...
    netbuf_ret = wap_info->wapi_netbuff_rxhandle(hmac_user, netbuf);
    if (netbuf_ret == NULL) {
...
oal_netbuf_stru *hmac_rx_lan_frame_wapi_proc(hmac_vap_stru *hmac_vap, uint16_t user_idx, oal_netbuf_stru *netbuf,
 ...
 *   修改内容  : 创建
 */
uint32_t hmac_wapi_display_info(mac_vap_stru *mac_vap, uint16_t usr_idx)
{
    hmac_user_stru *hmac_user = NULL;
    ...
    if (hmac_user == NULL) {
        oam_warning_log1(mac_vap->uc_vap_id, OAM_SF_ANY, "Err! ucast usr %u does not exist!", usr_idx);
        return OAL_ERR_CODE_PTR_NULL;
    }
