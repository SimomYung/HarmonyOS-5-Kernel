...
{
    mac_get_rx_cb_mac_header_addr(rx_ctl) = (uint64_t)(uintptr_t)oal_netbuf_header(netbuf);
    ...
    oal_netbuf_stru *netbuf, uint8_t *pn, hmac_encrypt_params_stru *encrypt_params, uint8_t *mic, uint8_t mic_len)
{
    ...
    mac_hdr = (mac_ieee80211_frame_stru *)(uintptr_t)mac_get_cb_frame_header_addr(tx_ctl);
    /* 为计算mic的数据申请内存 */
    mic_data = hmac_wpi_mic_data_alloc(mac_frame_is_qos_data(&mac_hdr->st_frame_control), payload_len, &mic_data_len);
    if (mic_data == NULL) {
    ...
    hmac_decrypt_params_stru *decrypt_params)
{
    ...
    uint8_t key_index;
    if (rx_ctl == NULL) {
        oam_error_log0(0, OAM_SF_ANY, "hmac_wpi_get_decrypt_param:rx_ctl null");
        return OAL_FAIL;
    }
    // 获取数据帧MAC头的长度
    mac_hdr_len = rx_ctl->uc_mac_header_len;
...
OAL_STATIC uint32_t hmac_wpi_decrypt_rx_netbuf(hmac_decrypt_params_stru *decrypt_params, oal_netbuf_stru *netbuf)
{
    ...
    if (rx_ctl == NULL) {
        oam_error_log0(0, OAM_SF_ANY, "hmac_wpi_decrypt_rx_netbuf:rx_ctl null");
        return OAL_FAIL;
    }
    // 获取数据帧MAC头的长度
    mac_hdr_len = rx_ctl->uc_mac_header_len;
    ...
    if (mac_hdr == NULL) {
        oam_error_log0(0, OAM_SF_ANY, "hmac_wpi_decrypt_rx_netbuf:mac_hdr null");
        return OAL_FAIL;
    }
    // 将MAC头里帧控制的加密标记置为0
...
OAL_STATIC void hmac_wpi_update_tx_ctl(oal_netbuf_stru *netbuf, uint16_t payload_len)
{
    mac_tx_ctl_stru *tx_ctl = (mac_tx_ctl_stru *)oal_netbuf_cb(netbuf);
    if (tx_ctl == NULL) {
        oam_error_log0(0, OAM_SF_ANY, "hmac_wpi_update_tx_ctl:tx_ctl null");
        return;
    }
    ...
    oal_netbuf_stru *netbuf)
{
    ...
    if (tx_ctl == NULL) {
        oam_error_log0(0, OAM_SF_ANY, "hmac_wpi_encrypt_tx_netbuf:tx_ctl null");
        return NULL;
    }
    mac_hdr = (mac_ieee80211_frame_stru *)(uintptr_t)mac_get_cb_frame_header_addr(tx_ctl);
