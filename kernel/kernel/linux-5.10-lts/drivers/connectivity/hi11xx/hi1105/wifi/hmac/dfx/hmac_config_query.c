...
uint32_t hmac_config_query_psm_rsp(mac_vap_stru *pst_mac_vap, uint8_t uc_len, uint8_t *puc_param)
{
    ...
    oam_warning_log0(0, OAM_SF_CFG, "{hmac_config_query_psm_rsp::device msg to host}");
    ...
    oam_warning_log2(0, OAM_SF_CFG, "{hmac_config_query_psm_rsp::query_type[%d], is_complete[%d]}",
        pst_query_rsp_msg->en_query_type, pst_hmac_psm_query->auc_complete_flag[pst_query_rsp_msg->en_query_type]);
    return OAL_SUCC;
}
...
uint32_t hmac_config_query_lowpower_stat_rsp(mac_vap_stru *mac_vap, uint8_t len, uint8_t *param)
{
    ...
    mac_psm_query_stat_stru *query_rsp_msg = NULL;

    oam_warning_log0(0, OAM_SF_CFG, "{hmac_config_query_lowpower_stat_rsp::device msg to host}");
    hmac_device = hmac_res_get_mac_dev(mac_vap->uc_device_id);
    if (hmac_device == NULL) {
        oam_error_log1(mac_vap->uc_vap_id, OAM_SF_CFG,
            "hmac_config_query_lowpower_stat_rsp: hmac_device is null ptr. device id:%d",
            mac_vap->uc_device_id);
        return OAL_ERR_CODE_PTR_NULL;
    }
    lowpower_stat_query = &hmac_device->lowpower_stat_query;
    query_rsp_msg = (mac_psm_query_stat_stru*)param;

    lowpower_stat_query->query_stat.query_item = query_rsp_msg->query_item;
    if (memcpy_s(lowpower_stat_query->query_stat.aul_val, MAC_PSM_QUERY_MSG_MAX_STAT_ITEM * sizeof(uint32_t),
        query_rsp_msg->aul_val, query_rsp_msg->query_item * sizeof(uint32_t)) != EOK) {
        oam_error_log1(0, OAM_SF_CFG, "{hmac_config_query_lowpower_stat_rsp::memcpy fail, query_item[%d]}",
            query_rsp_msg->query_item);
        return OAL_FAIL;
    }

    /* 设置wait条件为true */
    lowpower_stat_query->query_completed_flag = OAL_TRUE;
    oal_wait_queue_wake_up_interrupt(&lowpower_stat_query->wait_queue);
    oam_warning_log1(0, OAM_SF_CFG, "{hmac_config_query_lowpower_stat_rsp::is_complete[%d]}",
        lowpower_stat_query->query_completed_flag);
    return OAL_SUCC;
}
...
{
    uint32_t ret;
    dmac_query_station_info_request_event  *query_requset_event = (dmac_query_station_info_request_event *)param;
    ...
    /***************************************************************************
        抛事件到DMAC层, 同步DMAC数据
    ***************************************************************************/
    ret = hmac_config_send_event(mac_vap, WLAN_CFGID_QUERY_STATION_STATS, len, param);
...
uint32_t hmac_config_query_lowpower_stat(mac_vap_stru *mac_vap, uint16_t len, uint8_t *param)
{
    ...
    /***************************************************************************
        抛事件到DMAC层
    ***************************************************************************/
    ret = hmac_config_send_event(mac_vap, WLAN_CFGID_GET_LOWPOWER_STATISTICS, len, param);
