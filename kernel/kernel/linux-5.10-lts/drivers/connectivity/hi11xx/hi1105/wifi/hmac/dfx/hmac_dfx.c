...
void hmac_chr_get_rx_info(hmac_chr_info_new_arch *chr_info_new_arch)
{
    ...
    chr_info_new_arch->rx_info.total_rx_packets = device_stats->total_rx_packets;
...
static void hmac_chr_get_tx_ring_info(mac_vap_stru *mac_vap, hmac_chr_info_new_arch *chr_info_new_arch)
{
    ...
    for (tid = 0; tid < WLAN_TID_MAX_NUM - 1; tid++) {
        tid_info = hmac_get_tx_tid_info(hmac_user->st_user_base_info.us_assoc_id, tid);
        if (tid_info == NULL) {
            continue;
        }
...
uint32_t hmac_tx_stats_set_log_on(mac_vap_stru *mac_vap, uint32_t *params)
{
    ...
    ret = dpe_wlan_tx_stats_set_log_on(params);
#endif
    return ret;
}