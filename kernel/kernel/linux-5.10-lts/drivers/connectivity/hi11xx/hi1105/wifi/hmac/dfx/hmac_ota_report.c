/*
 * Copyright (c) @CompanyNameMagicTag 2021-2023. All rights reserved.
 * 功能说明 : ota_report
 * 创建日期 : 2020年7月31日
 */

#include "hmac_ota_report.h"
#include "hmac_vap.h"
#include "hmac_tx_data.h"
#include "hmac_config.h"
#include "oam_event_wifi.h"
#undef THIS_FILE_ID
#define THIS_FILE_ID OAM_FILE_ID_HMAC_OTA_REPORT_C


/* 4 函数实现 */
/*
 * 函 数 名  : mac_report_beacon
 * 功能描述  : 接收流程打印beacon帧
 * 1.日    期  : 2014年5月22日
 *   作    者  : wifi
 *   修改内容  : 新生成函数
 */
void mac_report_beacon(mac_rx_ctl_stru *pst_rx_cb, oal_netbuf_stru *pst_netbuf)
{
    uint32_t ret;

    ret = oam_report_beacon((uint8_t *)(uintptr_t)mac_get_rx_cb_mac_header_addr(pst_rx_cb),
                            pst_rx_cb->uc_mac_header_len,
                            (uint8_t *)((uintptr_t)mac_get_rx_cb_mac_header_addr(pst_rx_cb) +
                            pst_rx_cb->uc_mac_header_len),
                            pst_rx_cb->us_frame_len,
                            OAM_OTA_FRAME_DIRECTION_TYPE_RX);
    if (ret != OAL_SUCC) {
        oam_warning_log1(0, OAM_SF_WIFI_BEACON,
                         "{mac_report_beacon::oam_report_beacon return err: 0x%x.}\r\n", ret);
    }
}
uint32_t hmac_report_80211_get_common_switch(mac_vap_stru *mac_vap, mac_ieee80211_frame_stru *frame_hdr,
    oam_80211_frame_ctx_union *oam_frame_report_un, uint8_t frame_type)
{
    uint16_t                   user_idx = 0xffff;
    uint8_t                   *da_addr = NULL;
    uint32_t                   ret;

    if (frame_hdr == NULL) {
        oam_error_log0(mac_vap->uc_vap_id, OAM_SF_ANY, "{mac_report_80211_get_switch::frame_hdr null.}");

        return OAL_ERR_CODE_PTR_NULL;
    }

    mac_rx_get_da(frame_hdr, &da_addr);

    if (ether_is_multicast(da_addr)) {
        ret = oam_report_80211_mcast_get_switch(OAM_OTA_FRAME_DIRECTION_TYPE_RX, frame_type, oam_frame_report_un);
        if (ret != OAL_SUCC) {
            oam_warning_log2(0, OAM_SF_RX,
                "{hmac_report_80211_get_switch::80211_mcast_get_switch failed! ret[%d], frame_type[%d]}",
                ret, frame_type);
            return ret;
        }
    } else {
        ret = mac_vap_find_user_by_macaddr(mac_vap, frame_hdr->auc_address2, &user_idx);
        if (ret == OAL_ERR_CODE_PTR_NULL) {
            oam_warning_log0(mac_vap->uc_vap_id, OAM_SF_ANY,
                "{hmac_report_80211_get_switch::mac_vap_find_user_by_macaddr failed.}");
            return ret;
        }
        if (ret == OAL_FAIL) {
            oam_frame_report_un->value = 0;
            return OAL_FAIL;
        }
        ret = oam_report_80211_ucast_get_switch(OAM_OTA_FRAME_DIRECTION_TYPE_RX, frame_type, oam_frame_report_un,
            user_idx);
        if (ret != OAL_SUCC) {
            oam_warning_log3(mac_vap->uc_vap_id, OAM_SF_RX,
                "{hmac_report_80211_get_switch::ucast_get_switch failed! ret[%d], type[%d], user_idx[%d]}",
                ret, frame_type, user_idx);
            oam_warning_log3(mac_vap->uc_vap_id, OAM_SF_RX,
                "{oam_report_80211_ucast_get_switch::frame_switch[%x], cb_switch[%x], dscr_switch[%x]",
                oam_frame_report_un->frame_ctx.bit_content, oam_frame_report_un->frame_ctx.bit_cb,
                oam_frame_report_un->frame_ctx.bit_dscr);

            return ret;
        }
    }

    return OAL_SUCC;
}
/*
 * 函 数 名  : mac_report_80211_get_switch
 * 功能描述  : 为上报接收到的80211帧获取打印开关
 * 1.日    期  : 2014年5月22日
 *   作    者  : wifi
 *   修改内容  : 新生成函数
 */
uint32_t mac_report_80211_get_switch(mac_vap_stru *mac_vap,
                                     mac_rx_ctl_stru *rx_ctl,
                                     oam_80211_frame_ctx_union *oam_frame_report_un)
{
    mac_ieee80211_frame_stru    *frame_hdr;
    uint8_t                    frame_type = 0;
    uint32_t                   ret;

    frame_hdr = (mac_ieee80211_frame_stru *)(uintptr_t)(mac_get_rx_cb_mac_header_addr(rx_ctl));
    if (frame_hdr == NULL) {
        oam_error_log0(mac_vap->uc_vap_id, OAM_SF_ANY, "{mac_report_80211_get_switch::frame_hdr null.}");

        return OAL_ERR_CODE_PTR_NULL;
    }
    if ((frame_hdr->st_frame_control.bit_type == WLAN_CONTROL) ||
        (frame_hdr->st_frame_control.bit_type == WLAN_MANAGEMENT)) {
        frame_type = OAM_USER_TRACK_FRAME_TYPE_MGMT;
    }

    if (frame_hdr->st_frame_control.bit_type == WLAN_DATA_BASICTYPE) {
        frame_type = OAM_USER_TRACK_FRAME_TYPE_DATA;
    }

    /* probe request 和 probe response太多，单独过滤一次 */
    if (frame_hdr->st_frame_control.bit_type == WLAN_MANAGEMENT) {
        if (frame_hdr->st_frame_control.bit_sub_type == WLAN_PROBE_REQ ||
            frame_hdr->st_frame_control.bit_sub_type == WLAN_PROBE_RSP) {
            ret = oam_report_80211_probe_get_switch(OAM_OTA_FRAME_DIRECTION_TYPE_RX, oam_frame_report_un);
            if (ret != OAL_SUCC) {
                oam_warning_log0(mac_vap->uc_vap_id, OAM_SF_ANY,
                    "{mac_report_80211_get_switch::oam_report_80211_probe_get_switch failed.}");

                return ret;
            }

            return OAL_SUCC;
        } else if (frame_hdr->st_frame_control.bit_sub_type == WLAN_DEAUTH ||
                   frame_hdr->st_frame_control.bit_sub_type == WLAN_DISASOC) {
            oam_frame_report_un->frame_ctx.bit_cb = OAL_TRUE;
            oam_frame_report_un->frame_ctx.bit_dscr = OAL_TRUE;
            oam_frame_report_un->frame_ctx.bit_content = OAL_TRUE;
            return OAL_SUCC;
        } else if (frame_hdr->st_frame_control.bit_sub_type == WLAN_ACTION ||
                   frame_hdr->st_frame_control.bit_sub_type == WLAN_ACTION_NO_ACK) {
            oam_frame_report_un->frame_ctx.bit_cb = OAL_TRUE;
            oam_frame_report_un->frame_ctx.bit_dscr = OAL_TRUE;
            oam_frame_report_un->frame_ctx.bit_content = OAL_TRUE;
            return OAL_SUCC;
        }
    }

    return hmac_report_80211_get_common_switch(mac_vap, frame_hdr, oam_frame_report_un, frame_type);
}

/*
 * 函 数 名  : mac_report_80211_get_user_macaddr
 * 功能描述  : 获取发送端用户mac地址，用于SDT过滤
 * 1.日    期  : 2014年5月23日
 *   作    者  : wifi
 *   修改内容  : 新生成函数
 */
uint32_t mac_report_80211_get_user_macaddr(mac_rx_ctl_stru *pst_rx_cb, uint8_t auc_user_macaddr[],
    size_t macaddr_len)
{
    mac_ieee80211_frame_stru *pst_frame_hdr;
    uint8_t                *puc_da = NULL;

    pst_frame_hdr = (mac_ieee80211_frame_stru *)(uintptr_t)(mac_get_rx_cb_mac_header_addr(pst_rx_cb));
    if (pst_frame_hdr == NULL) {
        oam_error_log0(0, OAM_SF_ANY, "{mac_report_80211_get_user_macaddr::pst_frame_hdr null.}");
        return OAL_ERR_CODE_PTR_NULL;
    }
    mac_rx_get_da(pst_frame_hdr, &puc_da);

    if (ether_is_multicast(puc_da)) {
        oal_set_mac_addr(&auc_user_macaddr[0], BROADCAST_MACADDR);
    } else {
        oal_set_mac_addr(&auc_user_macaddr[0], pst_frame_hdr->auc_address2);
    }

    return OAL_SUCC;
}

/*
 * 函 数 名  : mac_report_80211_frame
 * 功能描述  : 上报接收到的80211帧，不包含beacon
 * 输入参数  ：en_ota_type->oam_ota_type_enum
 * 1.日    期  : 2014年5月22日
 *   作    者  : wifi
 *   修改内容  : 新生成函数
 */
uint32_t mac_report_80211_frame(uint8_t *puc_mac_vap, uint8_t *puc_rx_cb, oal_netbuf_stru *pst_netbuf,
    uint8_t *puc_des_addr, uint8_t en_ota_type)
{
    oal_switch_enum_uint8    en_frame_switch;
    oal_switch_enum_uint8    en_cb_switch;
    oal_switch_enum_uint8    en_dscr_switch;
    uint32_t               ret;
    uint8_t                auc_user_macaddr[WLAN_MAC_ADDR_LEN] = {0};
    mac_vap_stru            *pst_mac_vap = (mac_vap_stru *)puc_mac_vap;
    mac_rx_ctl_stru         *pst_rx_cb = (mac_rx_ctl_stru *)puc_rx_cb;
    oam_80211_frame_ctx_union oam_frame_report_un = {0};

    /* 获取打印开关 */
    ret = mac_report_80211_get_switch(pst_mac_vap, pst_rx_cb, &oam_frame_report_un);
    if (ret == OAL_ERR_CODE_PTR_NULL) {
        oam_warning_log0(pst_mac_vap->uc_vap_id, OAM_SF_ANY, "{mac_report_80211_frame::report 80211 get switch failed");
        return ret;
    }

    if (ret != OAL_SUCC) {
        return ret;
    }

    en_frame_switch = oam_frame_report_un.frame_ctx.bit_content;
    en_cb_switch = oam_frame_report_un.frame_ctx.bit_cb;
    en_dscr_switch = oam_frame_report_un.frame_ctx.bit_dscr;

    /* 获取发送端用户地址，用户SDT过滤,如果是组播\广播帧，则地址填为全F */
    ret = mac_report_80211_get_user_macaddr(pst_rx_cb, &auc_user_macaddr[0], WLAN_MAC_ADDR_LEN);
    if (ret != OAL_SUCC) {
        oam_warning_log0(pst_mac_vap->uc_vap_id, OAM_SF_ANY, "{mac_report_80211_frame::get user mac_addr failed}");
        return ret;
    }

    if ((en_ota_type == OAM_OTA_TYPE_RX_DSCR) || (en_ota_type == OAM_OTA_TYPE_RX_DSCR_PILOT)) {
        /* 上报接收帧的接收描述符 */
        if (en_dscr_switch == OAL_SWITCH_ON) {
            ret = oam_report_dscr(&auc_user_macaddr[0], puc_des_addr, (uint16_t)WLAN_RX_DSCR_SIZE, en_ota_type);
            if (ret != OAL_SUCC) {
                oam_warning_log1(0, OAM_SF_RX, "{mac_report_80211_frame::oam_report_dscr return err:0x%x}\r\n", ret);
            }
        }
    } else {
        /* 上报接收到的帧 */
        if (en_frame_switch == OAL_SWITCH_ON) {
            ret = oam_report_80211_frame(&auc_user_macaddr[0],
                (uint8_t *)(uintptr_t)mac_get_rx_cb_mac_header_addr(pst_rx_cb),
                pst_rx_cb->uc_mac_header_len,(uint8_t *)((uintptr_t)mac_get_rx_cb_mac_header_addr(pst_rx_cb) + \
                pst_rx_cb->uc_mac_header_len), pst_rx_cb->us_frame_len, OAM_OTA_FRAME_DIRECTION_TYPE_RX);
            if (ret != OAL_SUCC) {
                oam_warning_log1(0, OAM_SF_RX, "{mac_report_80211_frame::oam_report_80211_frame err:0x%x}\r\n", ret);
            }
        }

        /* 上报接收帧的CB字段 */
        if (en_cb_switch == OAL_SWITCH_ON) {
            ret = oam_report_netbuf_cb(auc_user_macaddr, (uint8_t *)pst_rx_cb, sizeof(*pst_rx_cb), en_ota_type);
            if (ret != OAL_SUCC) {
                oam_warning_log1(0, OAM_SF_RX, "{mac_report_80211_frame::oam_report_netbuf_cb error:0x%x}\r\n", ret);
            }
        }
    }

    return OAL_SUCC;
}

/*
 * 函 数 名  : mac_rx_report_80211_frame
 * 功能描述  : 接收流程上报80211帧，区分beacon帧和其他帧
 * 输入参数  ：en_ota_type->oam_ota_type_enum
 * 1.日    期  : 2014年5月22日
 *   作    者  : wifi
 *   修改内容  : 新生成函数
 */
uint32_t mac_rx_report_80211_frame(uint8_t vap_idx, uint8_t *rx_cb, oal_netbuf_stru *netbuf, uint8_t ota_type)
{
    uint8_t        uc_sub_type;
    mac_rx_ctl_stru *pst_mac_rx_cb = NULL;

    hmac_vap_stru *hmac_vap = mac_res_get_hmac_vap(vap_idx);
    if (hmac_vap == NULL) {
        oam_error_log1(0, OAM_SF_CFG, "{hmac_config_report_all_stat::vap[%d] is NULL}", vap_idx);
        return OAL_FAIL;
    }
    pst_mac_rx_cb = (mac_rx_ctl_stru *)rx_cb;

    uc_sub_type = mac_get_frame_type_and_subtype((uint8_t *)(uintptr_t)mac_get_rx_cb_mac_header_addr(pst_mac_rx_cb));
    if (uc_sub_type == (WLAN_FC0_SUBTYPE_BEACON | WLAN_FC0_TYPE_MGT)) {
        mac_report_beacon(pst_mac_rx_cb, netbuf);
    } else {
        mac_report_80211_frame((uint8_t *)(&hmac_vap->st_vap_base_info),
            (uint8_t *)pst_mac_rx_cb, netbuf, NULL, ota_type);
    }

    return OAL_SUCC;
}

#ifdef _PRE_WLAN_DFT_STAT
/*
 * 函 数 名  : hmac_config_report_all_stat
 * 功能描述  : 上报或者清零所有统计信息
 * 1.日    期  : 2014年8月20日
 *   作    者  : wifi
 *   修改内容  : 新生成函数
 */
uint32_t hmac_config_report_all_stat(mac_vap_stru *pst_mac_vap, uint16_t us_len, uint8_t *puc_param)
{
    uint32_t ret;

    /***************************************************************************
        抛事件到DMAC层, 同步DMAC数据
    ***************************************************************************/
    ret = hmac_config_send_event(pst_mac_vap, WLAN_CFGID_ALL_STAT, us_len, puc_param);
    if (oal_unlikely(ret != OAL_SUCC)) {
        oam_warning_log1(pst_mac_vap->uc_vap_id, OAM_SF_CFG,
                         "{hmac_config_report_all_stat::hmac_config_send_event failed[%d].}", ret);
    }

    return ret;
}
#endif

/*
 * 函 数 名  : hmac_rx_report_eth_frame
 * 功能描述  : 将要送往以太网的帧上报给SDT
 * 1.日    期  : 2014年5月20日
 *   作    者  : wifi
 *   修改内容  : 新生成函数
 */
#ifdef _PRE_WLAN_DFT_DUMP_FRAME
void hmac_rx_report_eth_frame(uint8_t vap_idx, oal_netbuf_stru *netbuf)
{
    uint16_t us_user_idx = 0;
    mac_ether_header_stru *pst_ether_hdr = NULL;
    uint32_t ret;
    hmac_vap_stru *hmac_vap = mac_res_get_hmac_vap(vap_idx);
    uint8_t auc_user_macaddr[WLAN_MAC_ADDR_LEN];
    oal_switch_enum_uint8 en_eth_switch = oam_ota_get_switch(OAM_OTA_SWITCH_RX_ETH_FRAME);
    if (oal_unlikely(hmac_vap == NULL || netbuf == NULL || en_eth_switch != OAL_SWITCH_ON)) {
        return;
    }

    /* 将skb的data指针指向以太网的帧头 */
    oal_netbuf_push(netbuf, ETHER_HDR_LEN);

    /* 获取目的用户资源池id */
    if (hmac_vap->st_vap_base_info.en_vap_mode == WLAN_VAP_MODE_BSS_AP) {
        pst_ether_hdr = (mac_ether_header_stru *)oal_netbuf_data(netbuf);
        if (oal_unlikely(pst_ether_hdr == NULL)) {
            oam_error_log0(vap_idx, OAM_SF_RX, "{hmac_rx_report_eth_frame::pst_ether_hdr null.}");
            oal_netbuf_pull(netbuf, ETHER_HDR_LEN);
            return;
        }

        ret = dpe_vap_find_user_by_macaddr(vap_idx, pst_ether_hdr->auc_ether_shost, &us_user_idx);
        if (ret == OAL_ERR_CODE_PTR_NULL) {
            oam_error_log0(vap_idx, OAM_SF_RX, "{hmac_rx_report_eth_frame::ul_ret null.}");
            oal_netbuf_pull(netbuf, ETHER_HDR_LEN);
            return;
        }

        if (ret == OAL_FAIL) {
            oal_netbuf_pull(netbuf, ETHER_HDR_LEN);
            return;
        }

        oal_set_mac_addr(auc_user_macaddr, pst_ether_hdr->auc_ether_shost);
    } else if (hmac_vap->st_vap_base_info.en_vap_mode == WLAN_VAP_MODE_BSS_STA) {
        if (hmac_vap->st_vap_base_info.us_user_nums == 0) {
            oal_netbuf_pull(netbuf, ETHER_HDR_LEN);
            /* SUCC , return */
            return;
        }

        us_user_idx = hmac_vap->st_vap_base_info.us_assoc_vap_id;
        oal_set_mac_addr(auc_user_macaddr, hmac_vap->st_vap_base_info.auc_bssid);
    }

    /* 将要送往以太网的帧上报 */
    ret = oam_report_eth_frame(auc_user_macaddr, oal_netbuf_data(netbuf),
        (uint16_t)oal_netbuf_len(netbuf), OAM_OTA_FRAME_DIRECTION_TYPE_RX);
    if (ret != OAL_SUCC) {
        oam_warning_log1(vap_idx, OAM_SF_RX,
            "{hmac_rx_report_eth_frame::oam_report_eth_frame return err: 0x%x.}\r\n", ret);
    }

    oal_netbuf_pull(netbuf, ETHER_HDR_LEN);
}
#endif

void hmac_rx_lan_frame_no_user_log(uint8_t vap_idx, oal_netbuf_stru *netbuf, mac_rx_ctl_stru *rx_ctrl)
{
    hmac_vap_stru *hmac_vap = mac_res_get_hmac_vap(vap_idx);
    if (hmac_vap == NULL) {
        return;
    }
    /* 打印此net buf相关信息 */
    oam_error_log3(vap_idx, OAM_SF_RX, "{hmac_rx_lan_frame_classify::cb info,vap_id=%d mac_hdr_len=%d frame_len=%d}",
        rx_ctrl->bit_vap_id, rx_ctrl->uc_mac_header_len, rx_ctrl->us_frame_len);
    oam_error_log1(vap_idx, OAM_SF_RX, "{hmac_rx_lan_frame_classify::user_idx=%d}", rx_ctrl->bit_rx_user_id);
#ifdef _PRE_WLAN_DFT_DUMP_FRAME
    mac_rx_report_80211_frame(hmac_vap->st_vap_base_info.uc_vap_id,
        (uint8_t *)rx_ctrl, netbuf, OAM_OTA_TYPE_RX_HMAC_CB);
#endif
}
