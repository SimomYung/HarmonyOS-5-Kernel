...
void hmac_hiex_game_sit_mark(oal_netbuf_stru *skb, mac_vap_stru *vap)
{
    ...
    if (vap->en_vap_mode == WLAN_VAP_MODE_BSS_STA) {
        ...
        if (_e == NULL) {
            oam_error_log0(0, OAM_SF_CFG, "{hmac_hiex_sit_mark::_e NULL}");
            return;
        }
    ...
    uint8_t *payload, uint32_t payload_len)
{
    ...
    ie = (mac_hiex_vendor_ie_stru *)mac_find_vendor_ie(MAC_WLAN_OUI_HUAWEI, MAC_HISI_HIEX_IE, &ies_info);
    if (ie == NULL) {
        return;
    }
 ...
 *   修改内容 : 新生成函数
 */
uint32_t hmac_hiex_rx_assoc_rsp(hmac_vap_stru *hmac_sta, hmac_user_stru *hmac_user, mac_ies_info *ies_info)
{
    mac_user_stru           *mac_user = NULL;
    mac_hiex_band_stru      *band = NULL;
    mac_hiex_vendor_ie_stru *ie = NULL;

    if (oal_unlikely(oal_any_null_ptr3(hmac_sta, hmac_user, ies_info))) {
    ...
    hmac_scanned_bss_info *scaned_bss)
{
    ...
    uint8_t disable_ersru_flag = OAL_FALSE;
    ...
    /* 如果同时支持ersru 和 11b+, 关闭ersru能力 */
    if (scaned_bss != NULL && scaned_bss->st_bss_dscr_info.hisi_11bplus_cap != 0) {
        disable_ersru_flag = OAL_TRUE;
        oam_warning_log1(0, OAM_SF_ASSOC, "{hmac_hiex_encap_ie::hisi_11bplus_cap[%d] disable_ersru_flag}",
            scaned_bss->st_bss_dscr_info.hisi_11bplus_cap);
    }
#endif

    return mac_hiex_set_vendor_ie(MAC_BAND_GET_HIEX_CAP(band), user_cap, frame, disable_ersru_flag);
}
/*
...
uint16_t hmac_assoc_req_set_max_tx_power_ie(uint8_t *buffer)
{
    ...
    len = NUM_2_BYTES;
    if (memcpy_s(max_power_ie->tx_pwr_detail_2g, len, g_max_power_info.tx_pwr_detail_2g, len) != EOK) {
        oam_error_log0(0, OAM_SF_HIEX, "{hmac_assoc_req_set_max_tx_power_ie::memcpy fail.}");
    }
    len = NUM_4_BYTES * NUM_2_BYTES;
    if (memcpy_s(max_power_ie->tx_pwr_detail_5g, len, g_max_power_info.tx_pwr_detail_5g, len) != EOK) {
        oam_error_log0(0, OAM_SF_HIEX, "{hmac_assoc_req_set_max_tx_power_ie::memcpy fail.}");
    }
...
void hmac_rx_assoc_rsp_parse_tb_frame_gain(mac_ies_info *ies_info)
{
    oal_ap_tb_frame_gain_ie *tb_gain_ie;

    tb_gain_ie = (oal_ap_tb_frame_gain_ie*)mac_find_vendor_ie(MAC_WLAN_OUI_HUAWEI,
        MAC_WLAN_OUI_TYPE_HAUWEI_TB_FRM_GAIN, ies_info);
