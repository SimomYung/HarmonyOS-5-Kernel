    ...
    uint8_t disable_ersru_flag)
{
    ...
    if (disable_ersru_flag == OAL_TRUE) {
        ie_cap->bit_ersru_enable = 0;
    }
...
oal_bool_enum_uint8 mac_hiex_pst_rx_mark(oal_netbuf_stru *netbuf, skb_header_type_enum hdr_type)
{
    ...
    if (hdr_type == SKB_HEADER_SNAP) {
        ...
        mac_hdr = (mac_ieee80211_frame_stru *)mac_get_rx_cb_mac_header_addr((mac_rx_ctl_stru *)oal_netbuf_cb(netbuf));
