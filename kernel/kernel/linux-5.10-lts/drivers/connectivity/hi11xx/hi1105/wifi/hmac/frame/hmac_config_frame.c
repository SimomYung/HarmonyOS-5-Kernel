 ...
 * 1.日    期  : 2014年3月26日
  *   修改内容  : 新生成函数
 */
uint32_t hmac_config_send_2040_coext(mac_vap_stru *mac_vap, uint16_t len, uint8_t *param)
{
    ...
    pst_2040_coexist = (mac_cfg_set_2040_coexist_stru *)param;
...
uint32_t hmac_config_set_wps_ie(mac_vap_stru *pst_mac_vap, uint16_t us_len, uint8_t *puc_param)
{
    ...
    ies_info.frame_ies = pst_wps_ie->auc_ie;
    ies_info.frame_len = pst_wps_ie->ie_len;
    if ((pst_wps_ie->en_app_ie_type == OAL_APP_BEACON_IE) && (pst_wps_ie->ie_len != 0)) {
        puc_ie = mac_find_vendor_ie(MAC_WLAN_OUI_MICROSOFT, MAC_WLAN_OUI_TYPE_MICROSOFT_WPS, &ies_info);
        if (puc_ie != NULL) {
            /* 设置WPS 功能使能 */
...
OAL_STATIC uint32_t hmac_connect_dual_sta_check(mac_vap_stru *mac_vap, hmac_conn_param_stru *connect_param)
{
    ...
    hmac_vap_stru *another_up_hmac_vap = NULL;
    ...
    /* 只在双sta模式下，检查wlan1 */
    if ((mac_is_dual_sta_mode() != OAL_TRUE) || (mac_is_secondary_sta(hmac_vap) != OAL_TRUE)) {
        return OAL_SUCC;
    }
    ...
    another_up_hmac_vap = (hmac_vap_stru *)mac_res_get_hmac_vap(another_up_vap->uc_vap_id);
    if (another_up_hmac_vap == NULL) {
        oam_warning_log0(another_up_vap->uc_vap_id, OAM_SF_CFG, "{hmac_dual_sta_connect_check::another_hmac_vap NULL}");
        return OAL_FAIL;
    }
    ...
    /* wlan0已经MLD关联,不允许wlan1关联 */
    if (hmac_vap_is_legacy_mld_mode(another_up_hmac_vap) == OAL_FALSE) {
        oam_warning_log0(another_up_vap->uc_vap_id, 0, "{wlan1 trying to connect when wlan0 is mld vap}");
        return OAL_FAIL;
    }
    ...
    hmac_vap_stru **hmac_vap)
{
    ...
    *hmac_vap = (hmac_vap_stru *)mac_res_get_hmac_vap(mac_vap->uc_vap_id);
    if (*hmac_vap == NULL) {
        oam_error_log1(mac_vap->uc_vap_id, OAM_SF_CFG,
            "{hmac_config_connect_param_check::connect failed, hmac_vap null.uc_vap_id[%d]}", mac_vap->uc_vap_id);
        hmac_free_connect_param_resource(connect_param);
        return OAL_ERR_CODE_PTR_NULL;
    }
...
static oal_bool_enum_uint8 hmac_connect_check_roam_bssid_stop_roam(hmac_vap_stru *hmac_vap,
    hmac_conn_param_stru *conn_param)
{
    hmac_roam_info_stru *roam_info = (hmac_roam_info_stru *)hmac_vap->pul_roam_info;
    ...
    /* 如果当前处于漫游扫描过程中，没有选到target bssid时清漫游状态返回，继续执行上层动作 */
    if (roam_info->en_main_state == ROAM_MAIN_STATE_SCANING) {
        hmac_roam_main_del_timer(roam_info);
        hmac_roam_main_clear(roam_info);
        return OAL_FALSE;
    }
    /* 如果上层下发bssid与驱动漫游选的target不一致则停止roam，执行上层自愈 */
    if ((oal_memcmp(roam_info->save_bss_dscr_info.auc_bssid, conn_param->auc_bssid, OAL_MAC_ADDR_LEN) != 0)) {
        hmac_roam_connect_complete(hmac_vap, roam_info, OAL_FAIL);
        return OAL_FALSE;
    }
    /* 相同bssid则停止上层自愈，等待驱动漫游完成上报 */
    return OAL_TRUE;
}
                                           ...
                                           hmac_conn_param_stru *conn_param)
{
    ...
    hmac_roam_start_params start_param = {0};

    if (hmac_connect_check_stop_roam(hmac_vap) == OAL_TRUE) {
        /* 如果上层自愈的bssid与驱动漫游target bssid一致则结束上层自愈 */
        if (hmac_connect_check_roam_bssid_stop_roam(hmac_vap, conn_param) == OAL_TRUE) {
            oam_warning_log0(mac_vap->uc_vap_id, OAM_SF_CFG,
                "{hmac_process_connect_by_vap_state:: roam bssid same connect return}");
            return OAL_SUCC;
        }
    ...
    if (mac_vap->en_vap_state == MAC_VAP_STATE_UP) {
        /* 相同ssid时，走漫游流程 */
        if (!oal_memcmp(mac_mib_get_DesiredSSID(mac_vap), conn_param->auc_ssid, conn_param->uc_ssid_len) &&
            (conn_param->uc_ssid_len == OAL_STRLEN(mac_mib_get_DesiredSSID(mac_vap)))) {
            ...
            oam_warning_log3(mac_vap->uc_vap_id, OAM_SF_CFG, "{hmac_process_connect_by_vap_state:: \
                roaming AP with ressoc frame, %02X:XX:XX:XX:%02X:%02X}", conn_param->auc_bssid[MAC_ADDR_0],
                conn_param->auc_bssid[MAC_ADDR_4], conn_param->auc_bssid[MAC_ADDR_5]);

            if (!oal_memcmp(mac_vap->auc_bssid, conn_param->auc_bssid, OAL_MAC_ADDR_LEN)) {
                /* reassociation */
                hmac_roam_start_fill_info(&start_param, ROAM_SCAN_CHANNEL_ORG_0, OAL_FALSE, NULL, ROAM_TRIGGER_APP);
                ret = hmac_roam_start(hmac_vap, &start_param);
            } else {
                /* 指定bssid尚未老化不用扫描 直接关联。防止down其他vap打断扫描，导致漫游失败。 */
                scan_type = hmac_roam_choose_scan_type_by_bssid(mac_vap, conn_param);

                /* 指定bssid漫游时判断当前场景是否为11v频繁漫游场景 */
                hmac_roam_check_conflict(hmac_vap, conn_param);
                hmac_roam_start_fill_info(&start_param, scan_type, OAL_TRUE, conn_param->auc_bssid, ROAM_TRIGGER_BSSID);
                /* roaming */
                ret = hmac_roam_start(hmac_vap, &start_param);
            }
            ...
            if (ret == OAL_SUCC) {
                hmac_free_connect_param_resource(conn_param);
                return OAL_SUCC;
...
static uint32_t hmac_interworking_check(hmac_vap_stru *pst_hmac_vap, uint8_t *puc_param)
{
    ...
    ies_info.frame_ies = pst_bss_dscr->auc_mgmt_buff + MAC_80211_FRAME_LEN + MAC_SSID_OFFSET;
    ies_info.frame_len = pst_bss_dscr->mgmt_len - MAC_80211_FRAME_LEN - MAC_SSID_OFFSET;
    /* 查找interworking ie */
    puc_extend_cap_ie = mac_find_ie(MAC_EID_EXT_CAPS, &ies_info);
...
static void hmac_connect_set_conn_security(hmac_conn_param_stru *connect_param, mac_conn_security_stru *conn_sec)
{
    ...
    ies_info.frame_ies = connect_param->puc_ie;
    ies_info.frame_len = connect_param->ie_len;
    ...
    if (mac_find_vendor_ie(MAC_WLAN_OUI_MICROSOFT, MAC_WLAN_OUI_TYPE_MICROSOFT_WPS, &ies_info) != NULL) {
...
#ifdef _PRE_WLAN_FEATURE_ML
    hmac_config_vap_ml_info_syn(hmac_vap);
#endif
    return OAL_SUCC;
}
...
/*
 * 函 数 名  : hmac_config_connect_process
 * 功能描述  : 配置connect参数
 * 1.日    期  : 2023年12月8日
 *   作    者  : wifi
 *   修改内容  : 新拆分函数
 */
uint32_t hmac_config_connect_process(hmac_vap_stru *hmac_vap, hmac_conn_param_stru *connect_param,
    mac_bss_dscr_stru *bss_dscr, mac_conn_security_stru *conn_sec, oal_bool_enum_uint8 sta_connected)
{
    ...
    ***************************************************************************/
    ...
    ret = hmac_sta_initiate_join(mac_vap, bss_dscr);
    if (sta_connected == OAL_TRUE && ret != OAL_SUCC) {
        hmac_proc_disasoc_inform(hmac_vap, MAC_DEAUTH_LV_SS, OAL_TRUE);
    }
    return ret;
}

OAL_STATIC uint32_t hmac_connect_param_check_process(hmac_vap_stru *hmac_vap, hmac_conn_param_stru *connect_param,
    mac_conn_security_stru *conn_sec, mac_bss_dscr_stru *bss_dscr)
{
    mac_vap_stru *mac_vap = &(hmac_vap->st_vap_base_info);
    ...
    if (hmac_vap_is_legacy_mld_mode(hmac_vap) != OAL_TRUE) {
        /* MLD STA重关联,退出EMLSR */
        hmac_vap_state_change_notify_emlsr(hmac_vap, NULL, COFEATURE_RECONNECT_EVENT, OAL_FALSE);
    } else {
        /* 非MLD VAP（GC）关联，退出EMLSR */
        hmac_emlsr_close_for_mvap(mac_vap, &bss_dscr->st_channel, COFEATURE_MVAP_EVENT);
    }
#endif
    ...
    /* sta、gc入网通知相关特性模块 */
    hmac_vap_state_change_notify_feature(hmac_vap, NULL, COFEATURE_MVAP_EVENT, COFEATURE_ENTER);

    return OAL_SUCC;
}

#ifdef _PRE_WLAN_FEATURE_ML
static void hmac_connect_clear_pre_resource(hmac_vap_stru *hmac_vap)
{
    ...
    oal_net_device_stru *link_net_device = NULL;
    ...
    if (net_device == NULL) {
        oam_error_log0(0, OAM_SF_ANY, "{hmac_connect_clear_pre_resource::net_dev null!}");
        return;
    }
    ...
    for (idx = 0; idx < WLAN_MAX_ML_LINK_NUM; idx++) {
        link_net_device = ml_net_dev_priv->mlo_cfg_info.link_netdev[idx];
        if (link_net_device == NULL) {
            continue;
        }
        link_net_dev_priv = (oal_netdev_priv_stru *)oal_net_dev_wireless_priv(link_net_device);
        if (link_net_dev_priv == NULL) {
            continue;
        }
        if (link_net_dev_priv->mlo_cfg_info.bind_with_ml_vap == OAL_FALSE) {
            /* 如果link vap已经为空，则不需要down vap操作 */
            link_mac_vap = oal_net_dev_priv(link_net_device);
            if (oal_unlikely(link_mac_vap == NULL)) {
                continue;
            }
            link_hmac_vap = (hmac_vap_stru *)mac_res_get_hmac_vap(link_mac_vap->uc_vap_id);
            if (oal_unlikely(link_hmac_vap == NULL)) {
                continue;
            }

            /* 针对 "link" netdev下有vap、user未删除场景，若有vap、user未删除则vap state一定为link_down */
            if (link_mac_vap->en_vap_state != MAC_VAP_STATE_STA_LINK_DOWN) {
                continue;
            }

            down_vap.en_p2p_mode = link_mac_vap->en_p2p_mode;
            down_vap.pst_net_dev = link_hmac_vap->pst_net_device;
            /* 只需要down掉vap并del user，不需要del vap */
            if (hmac_config_down_vap(link_mac_vap,
                                     sizeof(mac_cfg_down_vap_param_stru), (uint8_t *)&down_vap) != OAL_SUCC) {
                oam_error_log0(0, OAM_SF_ANY, "{hmac_connect_clear_pre_resource::down vap fail}");
            }
            oam_error_log1(link_mac_vap->uc_vap_id, OAM_SF_MLD, "{hmac_connect_clear_pre_resource::link_id[%d]}",
 ...
 *   修改内容  : 新生成函数
 */
uint32_t hmac_config_connect(mac_vap_stru *mac_vap, uint16_t len, uint8_t *param)
{
    uint32_t ret;
    ...
    hmac_conn_param_stru *connect_param = NULL;
    mac_conn_security_stru conn_sec;
    oal_bool_enum_uint8 sta_connected = OAL_FALSE;

    ret = hmac_config_connect_param_check(mac_vap, len, param, &hmac_vap);
    if (ret != OAL_SUCC) {
        return ret;
    }

    connect_param = (hmac_conn_param_stru *)param;
    ...
    if (hmac_connect_param_check_process(hmac_vap, connect_param, &conn_sec, bss_dscr) != OAL_SUCC) {
        return OAL_FAIL;
    }
    hmac_connect_set_reassoc_flag(hmac_vap, connect_param, bss_dscr);
    sta_connected = mac_vap_is_up(mac_vap);
    /* 不同的vap state走不同的流程 */
...
#ifdef _PRE_WLAN_FEATURE_ML
    /* 检查是否有未清空的vap、user，提前清除 */
    hmac_connect_clear_pre_resource(hmac_vap);
    /* wifi7相关能力位置位 */
    hmac_wifi7_get_connect_flag_cmd(mac_vap, connect_param, bss_dscr);
    /* 判断待关联ap是否支持multilink，是否执行MLD关联; 找不到另外一路link直接执行非mld关联 */
    if (hmac_check_is_mld_connect(mac_vap, bss_dscr) == OAL_TRUE &&
        hmac_check_single_link_enable(bss_dscr) == OAL_TRUE) {
        ret = hmac_mld_connect_process(hmac_vap, bss_dscr, connect_param, &conn_sec, sta_connected);
        hmac_free_connect_param_resource(connect_param);
        return ret;
    }
#endif
    ...
    /* 非MLD模式走普通流程 */
    ret = hmac_config_connect_process(hmac_vap, connect_param, bss_dscr, &conn_sec, sta_connected);
    hmac_free_connect_param_resource(connect_param);
    return ret;
}
...
static uint32_t hmac_app_ie_remove(oal_app_ie_stru *app_ie, uint8_t eid)
{
    ...
    ies_info.frame_ies = app_ie->auc_ie;
    ies_info.frame_len = app_ie->ie_len;

    ie = mac_find_ie(eid, &ies_info);
    if (ie == NULL) {
        return OAL_SUCC;
    }
 ...
 * 1.日    期  : 2014年11月26日
  *   修改内容  : 新生成函数
 */
uint32_t hmac_config_set_wps_p2p_ie(mac_vap_stru *mac_vap, uint16_t len, uint8_t *param)
{
    oal_app_ie_stru app_ie;
    hmac_vap_stru *hmac_vap = NULL;
    uint32_t ret;
    uint32_t delta_time;
    oal_w2h_app_ie_stru *w2h_wps_p2p_ie = (oal_w2h_app_ie_stru *)param;
    mac_ies_info ies_info = {0};
    ...
    if (delta_time >= (HMAC_WAIT_EVENT_RSP_TIME * HMAC_S_TO_MS)) {
        oam_warning_log4(mac_vap->uc_vap_id, OAM_SF_CFG, "{hmac_config_set_wps_p2p_ie::schedule event timeout[%d]ms, \
    ...
    if (!hmac_get_feature_switch(HMAC_MIRACAST_REDUCE_LOG_SWITCH)) {
        oam_warning_log2(mac_vap->uc_vap_id, OAM_SF_CFG, "{hmac_config_set_wps_p2p_ie::p2p_ie_type=[%d], \
            p2p_ie_len=[%d].}", w2h_wps_p2p_ie->en_app_ie_type, w2h_wps_p2p_ie->ie_len);
    }
    ...
    if (app_ie.en_app_ie_type == OAL_APP_BEACON_IE) {
        if (app_ie.ie_len != 0 &&
            mac_find_vendor_ie(MAC_WLAN_OUI_MICROSOFT, MAC_WLAN_OUI_TYPE_MICROSOFT_WPS, &ies_info) != NULL) {
            /* 设置WPS 功能使能 */
...
uint32_t hmac_config_delba_req(mac_vap_stru *pst_mac_vap, uint16_t us_len, uint8_t *puc_param)
{
    ...
    hmac_vap_stru *pst_hmac_vap = NULL;
    mac_action_mgmt_args_stru st_action_args; /* 用于填写ACTION帧的参数 */
    hmac_tid_ba_stru *pst_hmac_tid = NULL;
    ...
    pst_hmac_tid = hmac_get_tid_ba_info(pst_hmac_user->st_user_base_info.us_assoc_id, pst_delba_req->uc_tidno);
    if (pst_hmac_tid == NULL) {
        return OAL_ERR_CODE_PTR_NULL;
    }
    ba_info = hmac_get_ba_info(pst_hmac_user, pst_delba_req->uc_tidno);

    /* 查看会话是否存在 */
    if (pst_delba_req->en_direction == MAC_RECIPIENT_DELBA) {
        if (pst_delba_req->en_trigger == MAC_DELBA_TRIGGER_COMM) {
            ba_info->en_ba_handle_rx_enable = OAL_FALSE;
        }

        if (ba_info->en_ba_status == DMAC_BA_INIT) {
            oam_warning_log0(pst_mac_vap->uc_vap_id, OAM_SF_CFG, "{hmac_config_delba_req::the rx hdl is not exist.}");
            return OAL_SUCC;
        }
    } else {
        if (pst_delba_req->en_trigger == MAC_DELBA_TRIGGER_COMM) {
            ba_info->en_ba_handle_tx_enable = OAL_FALSE;
        }
