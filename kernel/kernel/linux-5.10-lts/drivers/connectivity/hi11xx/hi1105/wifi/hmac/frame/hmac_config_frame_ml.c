...
{
    if (hmac_mld_enable_single_link() == OAL_FALSE) {
        return (bss_dscr->ml_info.ml_link_num > WLAN_ML_SINGLE_RADIO);
    }
...
static void hmac_mld_back_to_legacy_mode(hmac_vap_stru *hmac_vap, hmac_vap_stru *link_hmac_vap)
{
    hmac_vap_mld_mode_change(link_hmac_vap, MLD_CHANGE_EVENT_CHANGE_TO_LEGACY,
        hmac_vap->st_vap_base_info.uc_vap_id);
    /* 不支持单link入网时把ml_vap也改成legacy */
    if (hmac_mld_enable_single_link() == OAL_FALSE) {
        hmac_vap_mld_mode_change(hmac_vap, MLD_CHANGE_EVENT_CHANGE_TO_LEGACY,
            hmac_vap->st_vap_base_info.uc_vap_id);
    }
}
    ...
    mac_bss_dscr_stru *bss_dscr, mac_conn_security_stru *conn_sec, oal_bool_enum_uint8 sta_connected)
{
    ...
    for (link_id = 0; link_id < WLAN_MAX_ML_LINK_NUM; link_id++) {
        ...
        if (ret != OAL_SUCC) {
            ...
            hmac_mld_back_to_legacy_mode(hmac_vap, link_hmac_vap);
            continue;
        }
