 ...
 * 1.日    期  : 2018年1月2日
  *   修改内容  : 新生成函数
 */
uint8_t hmac_set_supported_rates_ie_asoc_rsp(mac_user_stru *pst_mac_user, uint8_t *puc_buffer)
{
    uint8_t uc_nrates;
    uint8_t uc_idx;
    /**************************************************************************
 ...
 * 1.日    期  : 2018年1月2日
  *   修改内容  : 新生成函数
 */
uint8_t hmac_set_exsup_rates_ie_asoc_rsp(mac_user_stru *pst_mac_user, uint8_t *puc_buffer)
{
    uint8_t uc_nrates;
    uint8_t uc_idx;
    /***************************************************************************
...
/*
 * 函 数 名  : hmac_mgmt_encap_asoc_rsp_header
 * 功能描述  : 组关联响应帧
 * 1.日    期  : 2013年7月9日
 *   作    者  : wifi
 *   修改内容  : 超过50行
...
#ifdef _PRE_WLAN_FEATURE_11BE
/*
 * 函 数 名   : hmac_mgmt_encap_asoc_rsp_ap_set_eht_ie
 * 功能描述   : 组关联响应帧，填写eht ie字段
 * 1.日    期   : 2023年10月19日
 */
OAL_STATIC uint8_t *hmac_mgmt_encap_asoc_rsp_ap_set_eht_ie(mac_vap_stru *mac_ap,
    mac_user_stru *mac_user, uint8_t *asoc_rsp)
{
    if ((g_wlan_spec_cfg->feature_11be_is_open) &&
        (mac_vap_is_work_eht_protocol(mac_ap) == OAL_TRUE) &&
        (mac_user_get_eht_capable(mac_user) == OAL_TRUE)) {
        /* 设置 EHT Capabilities 和 EHT Operation IE */
        asoc_rsp += mac_set_eht_ie(mac_ap, asoc_rsp, WLAN_ASSOC_RSP);
    }
    return asoc_rsp;
}
#endif

OAL_STATIC uint8_t *hmac_mgmt_encap_asoc_rsp_ap_set_protocol_mode_ie(mac_vap_stru *mac_ap,
    mac_user_stru *mac_user, uint8_t *asoc_rsp, uint16_t assoc_id)
{
    /* 设置HT IE */
...
OAL_STATIC uint32_t mac_set_hiex_feature_ie(mac_vap_stru *mac_ap, uint8_t *asoc_rsp)
{
    ...
    if (g_wlan_spec_cfg->feature_hiex_is_open) {
        return hmac_hiex_encap_ie((void *)mac_ap, NULL, asoc_rsp, NULL);
    }
#endif
    return 0;
}
 ...
 *   修改内容  : 新生成函数
 */
uint32_t hmac_mgmt_encap_asoc_rsp_ap(hmac_vap_stru *hmac_vap, const unsigned char *sta_addr,
    uint16_t assoc_id, mac_status_code_enum_uint16 status_code, uint8_t *asoc_rsp, uint16_t type)
{
    ...
    mac_vap_stru *mac_ap = NULL;

    if (oal_any_null_ptr5(hmac_vap, sta_addr, asoc_rsp, mac_user, hmac_user)) {
        oam_error_log0(0, OAM_SF_ASSOC, "{hmac_mgmt_encap_asoc_rsp_ap::mac_ap/sta_addr/asoc_req/mac_user is null.}");
        return 0;
    }

    mac_ap = &(hmac_vap->st_vap_base_info);
    ...
    asoc_rsp += hmac_set_supported_rates_ie_asoc_rsp(mac_user, asoc_rsp);

    /* 设置 Extended Supported Rates IE */
    asoc_rsp += hmac_set_exsup_rates_ie_asoc_rsp(mac_user, asoc_rsp);
    ...
    asoc_rsp += mac_set_timeout_interval_ie(mac_ap, asoc_rsp, tie_type, timeout);

    asoc_rsp = hmac_mgmt_encap_asoc_rsp_ap_set_protocol_mode_ie(mac_ap, mac_user, asoc_rsp, assoc_id);
#ifdef _PRE_WLAN_FEATURE_ML
    /* 设置ML IE:兼容性问题，ml ie需要在eht ie之前 */
    asoc_rsp += hmac_set_assoc_rsp_basic_ml_ie(hmac_vap, hmac_user, asoc_rsp);
#endif
#ifdef _PRE_WLAN_FEATURE_11BE
    /* 设置EHT IE */
    asoc_rsp = hmac_mgmt_encap_asoc_rsp_ap_set_eht_ie(mac_ap, mac_user, asoc_rsp);
#endif

#ifdef _PRE_WLAN_FEATURE_WUR
    asoc_rsp += hmac_mgmt_encap_asoc_rsp_ap_set_wur_ie(mac_ap, mac_user, asoc_rsp);
#endif

    asoc_rsp += mac_set_hisi_cap_vendor_ie(mac_ap, asoc_rsp);

    asoc_rsp += mac_set_hiex_feature_ie(mac_ap, asoc_rsp);
#ifdef _PRE_WLAN_CHBA_MGMT
    /* chba 2.0版本app_ie中除驱动侧组的chba ie外还可能携带上层下发ie, 而chba 1.0版本驱动侧默认只查找并处理首个chba ie
     * 考虑chba 2.0与chba 1.0连接时兼容性, 需保证驱动侧chba_ie组在上层下发的app_ie之前 */
    asoc_rsp += mac_set_vendor_chba_ie(mac_ap, hmac_user, asoc_rsp, sta_addr);
#endif
    /* 填充WPS信息 */
 ...
 *   修改内容  : 新生成函数
 */
oal_err_code_enum hmac_encap_auth_rsp_get_user_idx(mac_vap_stru *mac_vap, hmac_auth_req_info_stru *auth_req_info,
    uint8_t *auth_resend, uint16_t *user_index)
{
    uint32_t ret;
    ...
    if (hmac_vap == NULL) {
        oam_error_log0(mac_vap->uc_vap_id, OAM_SF_ANY, "{hmac_encap_auth_rsp_get_user_idx::hmac_vap NULL!}");
        return OAL_FAIL;
    }

    *auth_resend = OAL_FALSE;
    ret = mac_vap_find_user_by_macaddr(&(hmac_vap->st_vap_base_info), auth_req_info->addr2, user_index);
    /* 找到用户 */
    if (ret == OAL_SUCC) {
         ...
         */
        *auth_resend = ((hmac_user->st_user_base_info.en_user_asoc_state != MAC_USER_STATE_BUTT) ||
            (auth_req_info->auth_type == WLAN_MIB_AUTH_ALG_SAE)) ? OAL_TRUE : OAL_FALSE;
        ...
        if (hmac_user->st_user_base_info.en_user_asoc_state == MAC_USER_STATE_ASSOC) {
            ...
            if (net_device != NULL) {
                ...
                oal_kobject_uevent_env_sta_leave(net_device, auth_req_info->addr2);
#endif
            }
        }
    ...
    hmac_onechip_delete_other_vap_user(hmac_vap, auth_req_info->addr2);

    /* 在收到第一个认证帧时用户已创建 */
    if (!auth_req_info->is_auth_seq1) {
        oam_warning_log0(mac_vap->uc_vap_id, OAM_SF_ANY, "{hmac_encap_auth_rsp_get_user_idx::user added at seq1!}");
        return OAL_FAIL;
    }

    memset_s(&user_info, sizeof(hmac_add_user_input_stru), 0, sizeof(hmac_add_user_input_stru));
    memcpy_s(user_info.mac_addr, WLAN_MAC_ADDR_LEN, auth_req_info->addr2, WLAN_MAC_ADDR_LEN);
    memcpy_s(user_info.mld_mac_addr, WLAN_MAC_ADDR_LEN, auth_req_info->mld_mac_addr, WLAN_MAC_ADDR_LEN);
    ret = hmac_user_add(mac_vap, &user_info, user_index);
    if (ret != OAL_SUCC) {
    ...
    if (hmac_chba_vap_start_check(hmac_vap)) {
        ...
        hmac_chba_user_set_connect_role(chba_vap_user, CHBA_CONN_RESPONDER, CHBA_NORMAL_CONNECT_FLAG);
    }
#endif
    return OAL_SUCC;
}
    ...
    hmac_auth_rsp_param_stru *pst_auth_rsp_param, uint8_t *puc_code, hmac_user_stru *hmac_user_sta)
{
    ...
    if ((pst_auth_rsp_param->en_user_asoc_state == MAC_USER_STATE_ASSOC) &&
        ...
        if (mac_mib_get_dot11RSNAMFPC(pst_mac_vap) == OAL_TRUE) {
            hmac_user_sta->st_user_base_info.en_user_asoc_state = MAC_USER_STATE_ASSOC;
        } else {
            hmac_user_sta->st_user_base_info.en_user_asoc_state = MAC_USER_STATE_AUTH_COMPLETE;
        }
...
uint32_t hmac_encap_auth_rsp_support(hmac_vap_stru *pst_hmac_vap, uint16_t us_auth_type)
{
    ...
    if ((mac_mib_get_AuthenticationMode(&pst_hmac_vap->st_vap_base_info) != us_auth_type) &&
        (mac_mib_get_AuthenticationMode(&pst_hmac_vap->st_vap_base_info) != WLAN_WITP_AUTH_AUTOMATIC)) {
        return OAL_ERR_CODE_CONFIG_UNSUPPORT;
    }
    return OAL_SUCC;
}

void hmac_clear_amsdu_queue(hmac_user_stru *hmac_user, uint8_t tid)
{
    oal_netbuf_stru *amsdu_net_buf = NULL;
    hmac_amsdu_stru *amsdu = &(hmac_user->ast_hmac_amsdu[tid]);
    ...
    /* 清空聚合队列 */
    if (amsdu->msdu_num != 0) {
        while (oal_netbuf_list_empty(&amsdu->st_msdu_head) != OAL_TRUE) {
...
void hmac_clear_ba_info(hmac_vap_stru *hmac_vap, hmac_user_stru *hmac_user, uint8_t tid)
{
    hmac_ba_stru *ba_info = hmac_get_ba_info(hmac_user, tid);

    if (ba_info->st_ba_tx_info.st_addba_timer.en_is_registerd == OAL_TRUE) {
        frw_timer_immediate_destroy_timer_m(&(ba_info->st_ba_tx_info.st_addba_timer));
    }
    if (ba_info->st_ba_tx_info.en_ba_status != DMAC_BA_INIT) {
        /* 存在TX BA会话句柄，要-- */
        hmac_tx_ba_session_decr(hmac_vap, tid);
    }

    /* 初始化ba tx操作句柄 */
    ba_info->st_ba_tx_info.en_ba_status = DMAC_BA_INIT;
    ba_info->st_ba_tx_info.uc_addba_attemps = 0;
    ba_info->st_ba_tx_info.uc_dialog_token = 0;
    ba_info->st_ba_tx_info.uc_ba_policy = 0;
}
 ...
 *   修改内容  : 新生成函数
 */
void hmac_tid_clear(mac_vap_stru *pst_mac_vap, hmac_user_stru *hmac_user, oal_bool_enum_uint8 force_clear_ba_info)
{
    uint8_t uc_loop;
    hmac_vap_stru *pst_hmac_vap = NULL;
    hmac_ba_stru *ba_info = NULL;
    if (hmac_user == NULL) {
        oam_error_log0(0, OAM_SF_AUTH, "{hmac_tid_clear::hmac_user is null.}");
        return;
    }
    ...
    for (uc_loop = 0; uc_loop < WLAN_TID_MAX_NUM; uc_loop++) {
        hmac_clear_amsdu_queue(hmac_user, uc_loop);
    }
    /* 删用户重置ba时只允许只有1个user使用tid资源，在mld场景和mld漫游非mld场景生效 防止删用户和建ba异步导致的状态错误 */
    if (hmac_user->ba_info == NULL ||
        (hmac_user->ba_info->work_flag != 1 && force_clear_ba_info == OAL_FALSE)) {
        return;
    }

    for (uc_loop = 0; uc_loop < WLAN_TID_MAX_NUM; uc_loop++) {
        hmac_clear_ba_info(pst_hmac_vap, hmac_user, uc_loop);
        hmac_rx_ba_reset_pre_handle(pst_hmac_vap, hmac_user, uc_loop);
        hmac_clear_one_tid_queue(hmac_user->st_user_base_info.us_assoc_id, uc_loop);
        ba_info = hmac_get_ba_info(hmac_user, uc_loop);
...
static void hmac_encap_auth_rsp_frame_head(uint8_t *data, mac_vap_stru *mac_vap, uint8_t *addr2)
{
...
static void hmac_encap_auth_rsp_fixed_params(uint8_t *frame, uint16_t auth_type, uint16_t auth_seq)
{
    /* 设置认证类型IE */
    frame[0] = (auth_type & 0x00FF);
    frame[BYTE_OFFSET_1] = (auth_type & 0xFF00) >> BIT_OFFSET_8;

    /* 将收到的auth seq + 1后复制给新的认证响应帧 */
...
{
    oam_warning_log3(mac_vap->uc_vap_id, OAM_SF_AUTH,
        "{hmac_encap_auth_rsp::user mac:%02X:XX:XX:XX:%02X:%02X in mac_filter_list}",
        addr2[MAC_ADDR_0], addr2[MAC_ADDR_4], addr2[MAC_ADDR_5]);
...
{
    oam_warning_log1(mac_vap->uc_vap_id, OAM_SF_AUTH,
                     "{hmac_encap_auth_rsp::auth type[%d] not support!}", auth_type);
    frame[BYTE_OFFSET_4] = MAC_UNSUPT_ALG;
    chr_exception_report(CHR_PLATFORM_EXCEPTION_EVENTID, CHR_SYSTEM_WIFI,
                         CHR_LAYER_DRV, CHR_WIFI_DRV_EVENT_SOFTAP_CONNECT, frame[BYTE_OFFSET_4]);
    mac_get_cb_tx_user_idx(tx_ctl) = user_idx;
    mac_get_cb_mpdu_len(tx_ctl) = auth_rsp_len;
    hmac_user_set_asoc_state(mac_vap, &hmac_user_sta->st_user_base_info, MAC_USER_STATE_BUTT);
}
                                        ...
                                        hmac_ap_auth_process_code_enum *auth_proc_rst)
{
    ...
    if (auth_rsp_handle->st_auth_rsp_fun != NULL) {
        *auth_proc_rst = auth_rsp_handle->st_auth_rsp_fun(mac_vap, &(auth_rsp_handle->st_auth_rsp_param),
        ...
        /* 清空 HMAC层TID信息 */
        hmac_tid_clear(mac_vap, hmac_user_sta, OAL_TRUE);
    } else {
...
static oal_bool_enum_uint8 hmac_process_auth_rsp_is_invalid_seq(mac_vap_stru *mac_vap,
    hmac_user_stru *hmac_user, hmac_auth_req_info_stru *auth_req_info)
{
    if (hmac_user->st_user_base_info.en_user_asoc_state == MAC_USER_STATE_ASSOC &&
        (auth_req_info->auth_type != WLAN_WITP_AUTH_OPEN_SYSTEM ||
        (auth_req_info->auth_type == WLAN_WITP_AUTH_OPEN_SYSTEM &&
        auth_req_info->auth_seq != WLAN_AUTH_TRASACTION_NUM_ONE))) {
        oam_warning_log2(mac_vap->uc_vap_id, OAM_SF_AUTH,
            "{hmac_encap_auth_rsp::auth recv invalid seq, auth seq [%d], auth type[%d]}",
            auth_req_info->auth_seq, auth_req_info->auth_type);
        return OAL_TRUE;
    }
 ...
 */
oal_bool_enum_uint8 hmac_ap_rx_filter_dup_auth_req(hmac_user_stru *hmac_user,
    hmac_auth_req_info_stru *auth_req_info, uint8_t auth_resend)
{
    uint32_t now;
    uint16_t auth_seq = auth_req_info->auth_seq;
    uint16_t auth_type = auth_req_info->auth_type;
...
/* 解析auth req帧中固定字段，解析结果保存在auth_req_info用于后续使用，返回失败则不回复auth rsp */
uint32_t hmac_parse_auth_req(mac_vap_stru *mac_vap, oal_netbuf_stru *auth_req, uint8_t *auth_req_payload,
    uint16_t auth_req_payload_len, hmac_auth_req_info_stru *auth_req_info)
{
    memset_s(auth_req_info, sizeof(hmac_auth_req_info_stru), 0, sizeof(hmac_auth_req_info_stru));

    /* 解析对端mac地址 */
    mac_get_address2(oal_netbuf_header(auth_req), auth_req_info->addr2, WLAN_MAC_ADDR_LEN);

    /* 解析auth alg */
    auth_req_info->auth_type = mac_get_auth_alg(auth_req_payload, auth_req_payload_len);

    /* 解析auth seq */
    auth_req_info->auth_seq = mac_get_auth_seq_num(auth_req_payload, auth_req_payload_len);
    if (auth_req_info->auth_seq > HMAC_AP_AUTH_SEQ3_WEP_COMPLETE) {
        oam_warning_log1(mac_vap->uc_vap_id, OAM_SF_AUTH,
            "{hmac_parse_auth_req::auth recieve invalid seq, auth seq [%d]}", auth_req_info->auth_seq);
        return OAL_FAIL;
    }

    /* auth req是否是seq1 */
    auth_req_info->is_auth_seq1 = (auth_req_info->auth_seq == WLAN_AUTH_TRASACTION_NUM_ONE);

#ifdef _PRE_WLAN_FEATURE_ML
    mac_parse_auth_req_ml_info(auth_req_payload, auth_req_payload_len, auth_req_info);
#endif

    return OAL_SUCC;
}

uint16_t hmac_encap_auth_rsp(mac_vap_stru *mac_vap, oal_netbuf_stru *auth_rsp, oal_netbuf_stru *auth_req,
    uint8_t *chtxt, uint8_t chtxt_len)
{
    uint16_t auth_rsp_len = 0;
    hmac_user_stru *hmac_user_sta = NULL;
    uint8_t *frame = NULL;
    ...
    data = (uint8_t *)oal_netbuf_header(auth_rsp);
    ...
    /*                                                                       */
    /*************************************************************************/
    ret = hmac_parse_auth_req(mac_vap, auth_req, auth_req_payload, auth_req_payload_len, &auth_req_info);
    if (ret != OAL_SUCC) {
        return auth_rsp_len;
    }

    frame = (uint8_t *)(data + MAC_80211_FRAME_LEN);

    /* 计算auth rsp帧的长度 */
    auth_rsp_len = MAC_80211_FRAME_LEN + MAC_AUTH_ALG_LEN + MAC_AUTH_TRANS_SEQ_NUM_LEN + MAC_STATUS_CODE_LEN;

    hmac_encap_auth_rsp_frame_head(data, mac_vap, auth_req_info.addr2);

    hmac_encap_auth_rsp_fixed_params(frame, auth_req_info.auth_type, auth_req_info.auth_seq);

    oam_warning_log3(mac_vap->uc_vap_id, OAM_SF_MLOG,
        "{hmac_encap_auth_rsp:: AUTH_RSP tx : user mac:%02X:XX:XX:XX:%02X:%02X}", auth_req_info.addr2[MAC_ADDR_0],
        auth_req_info.addr2[MAC_ADDR_4], auth_req_info.addr2[MAC_ADDR_5]);

    /* 判断对端mac地址是否为有效，不能为全0 */
    if (mac_addr_is_zero(auth_req_info.addr2)) {
        hmac_process_auth_resp_mac_zero_fail(frame, mac_vap, tx_ctl, auth_req_info.addr2, auth_rsp_len);
        return auth_rsp_len;
    }
    /* 因黑名单拒绝回auth,status code回37 */
    if (hmac_blacklist_filter(mac_vap, auth_req_info.addr2, WLAN_MAC_ADDR_LEN) == OAL_TRUE) {
        hmac_process_auth_resp_blacklist_check_fail(frame, mac_vap, tx_ctl, auth_req_info.addr2, auth_rsp_len);
        return auth_rsp_len;
    }

    /* 获取用户idx */
    ret = hmac_encap_auth_rsp_get_user_idx(mac_vap, &auth_req_info,
        &auth_rsp_handle.st_auth_rsp_param.uc_auth_resend, &user_idx);
    if (ret != OAL_SUCC) {
        hmac_process_auth_resp_get_user_idx_fail(frame, mac_vap, tx_ctl, ret, auth_rsp_len);
    ...
    /* 获取hmac user指针 */
    hmac_user_sta = mac_res_get_hmac_user(user_idx);
    if (hmac_user_sta == NULL) {
        hmac_process_auth_resp_get_user_fail(frame, mac_vap, tx_ctl, auth_rsp_len, user_idx);
        return auth_rsp_len;
    }
    if (hmac_ap_rx_filter_dup_auth_req(hmac_user_sta, &auth_req_info,
        auth_rsp_handle.st_auth_rsp_param.uc_auth_resend) == OAL_TRUE) {
        return 0;
    }
    /* 获取hmac vap指针 */
    hmac_vap = (hmac_vap_stru *)mac_res_get_hmac_vap(mac_vap->uc_vap_id);
    if (hmac_vap == NULL) {
        hmac_process_auth_resp_get_vap_fail(frame, mac_vap, tx_ctl, auth_rsp_len, hmac_user_sta);
        return auth_rsp_len;
    }
#ifdef _PRE_WLAN_FEATURE_PWL
    /* 从帧体里面取出pn并给user添加pwl预置单播秘钥 */
    hmac_pwl_set_ap_user_pre_pairwise_key(hmac_vap, hmac_user_sta, auth_req);
#endif

    if (hmac_process_auth_rsp_is_invalid_seq(mac_vap, hmac_user_sta, &auth_req_info) == OAL_TRUE) {
        return 0;
    }

    /* 判断算法是否支持 */
    alg_suppt = hmac_encap_auth_rsp_support(hmac_vap, auth_req_info.auth_type);
    if (alg_suppt != OAL_SUCC) {
        hmac_process_auth_resp_alg_unsupport(frame, mac_vap, hmac_user_sta, auth_req_info.auth_type,
            tx_ctl, auth_rsp_len, user_idx);
        return auth_rsp_len;
    }

    hmac_process_auth_resp_init_rsp_param(&auth_rsp_handle, mac_vap, hmac_user_sta,
        auth_req_info.auth_type, auth_req_info.auth_seq);

    hmac_process_auth_seq1_seq3(&auth_rsp_handle, mac_vap, hmac_user_sta, frame, &auth_proc_rst);

    auth_rsp_timeout = hmac_get_auth_rsp_timout(mac_vap);

    oam_warning_log3(mac_vap->uc_vap_id, OAM_SF_MLOG, "{hmac_encap_auth_rsp::user[%d] \
        auth proc_rst:%d, auth_rsp_timeout:%d}", user_idx, auth_proc_rst, auth_rsp_timeout);

    /*  根据返回的code进行后续处理 */
    switch (auth_proc_rst) {
        case HMAC_AP_AUTH_SEQ1_OPEN_ANY:
            hmac_process_auth_seq1_open(hmac_user_sta, mac_vap, auth_rsp_timeout);
            break;
        case HMAC_AP_AUTH_SEQ1_WEP_NOT_RESEND:
            hmac_process_auth_seq1_wep_no_resend(hmac_user_sta, mac_vap,
                frame, chtxt, &auth_rsp_len);
            break;
        case HMAC_AP_AUTH_SEQ1_WEP_RESEND:
            hmac_process_auth_seq1_wep_resend(hmac_user_sta, frame, chtxt, hmac_vap, &auth_rsp_len);
            break;
        case HMAC_AP_AUTH_SEQ3_OPEN_ANY:
            mac_user_init_key(&hmac_user_sta->st_user_base_info);
            break;
        case HMAC_AP_AUTH_SEQ3_WEP_COMPLETE:
            hmac_process_auth_seq3_wep_complete(hmac_user_sta, frame, chtxt, auth_req, mac_vap);
            break;
        case HMAC_AP_AUTH_SEQ3_WEP_ASSOC:
            hmac_process_auth_seq3_wep_assoc(hmac_user_sta, mac_vap, frame, chtxt, &auth_rsp_len);
            break;
        case HMAC_AP_AUTH_DUMMY:
            break;
        case HMAC_AP_AUTH_BUTT:
        default:
            hmac_process_auth_default(hmac_user_sta);
            break;
    }

#ifdef _PRE_WLAN_FEATURE_ML
    auth_rsp_len += hmac_set_auth_resp_basic_ml_ie(hmac_vap, data + auth_rsp_len, auth_req_info.is_ml_setup);
#endif
    ...
    ret = hmac_config_user_asoc_state_syn(&(hmac_vap->st_vap_base_info), &hmac_user_sta->st_user_base_info);
    if (ret != OAL_SUCC) {
