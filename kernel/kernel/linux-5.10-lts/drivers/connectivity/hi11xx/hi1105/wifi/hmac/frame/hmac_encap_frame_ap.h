...
typedef struct {
    uint8_t addr2[WLAN_MAC_ADDR_LEN]; /* auth req帧中的ADDR2地址，即请求方MAC地址 */
    uint16_t auth_type;              /* auth req帧中auth alg */
    uint16_t auth_seq;               /* auth seq序号，正常是1和3 */
    uint8_t is_ml_setup;
    uint8_t is_auth_seq1;
    uint8_t mld_mac_addr[WLAN_MAC_ADDR_LEN]; /* 请求方mld mac地址 */
} hmac_auth_req_info_stru;
...
oal_bool_enum_uint8 hmac_ap_rx_filter_dup_auth_req(hmac_user_stru *hmac_user, hmac_auth_req_info_stru *auth_req_info,
    uint8_t auth_resend);
