 ...
 *   修改内容  : 新生成函数
 */
static void hmac_add_cowork_ie(hmac_vap_stru *hmac_vap, uint8_t *buffer, uint16_t *ie_len)
{
    ...
    *ie_len = 0;
    ...
    if (curr_bss_dscr->ext_cowork_ie != OAL_TRUE) {
        return;
    }

    puc_frame_body = curr_bss_dscr->auc_mgmt_buff + MAC_80211_FRAME_LEN;
    if (curr_bss_dscr->mgmt_len < (MAC_80211_FRAME_LEN + us_offset)) {
        return;
    }
    ...
    if (mac_find_vendor_ie(MAC_WLAN_OUI_HUAWEI, MAC_WLAN_OUI_TYPE_HAUWEI_COWORK, &ies_info) == NULL) {
        return;
    }

    if (encap_cowork_ie_adaptive_11r(hmac_vap, &cowork_ie) == 0 &&
        encap_cowork_ie_narrow_band(hmac_vap, &cowork_ie) == 0) {
        return;
    }

    /* 为避免和5x路由器密钥协商失败导致关联失败问题，ap支持mld，关联请求不包含COWORK IE;AP支持wifi7端管则携带cowork */
    if (is_custom_open_mlo_switch(&(hmac_vap->st_vap_base_info)) == OAL_TRUE && mac_find_ie_ext_ie(MAC_EID_MULTILINK,
        MAC_EID_EXT_EHT_ML, &ies_info) != NULL && curr_bss_dscr->supp_eht_double_wifi == OAL_FALSE) {
        oam_warning_log0(hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_CFG, "{hmac_add_cowork_ie::do not add cowork}");
        return;
    }

    if (memcpy_s(buffer, sizeof(oal_sta_ap_cowork_ie), &cowork_ie, sizeof(oal_sta_ap_cowork_ie)) != EOK) {
        return;
    }
    *ie_len = sizeof(oal_sta_ap_cowork_ie);
...
hmac_scanned_bss_info *hmac_vap_get_scan_bss_info(mac_vap_stru *mac_vap)
{
    ...
    if (pst_hmac_device == NULL) {
        oam_warning_log0(mac_vap->uc_vap_id, OAM_SF_QOS, "{mac_vap_get_scan_bss_info::pst_mac_device null.}");
        return NULL;
    }
    pst_bss_mgmt = &(pst_hmac_device->st_scan_mgmt.st_scan_record_mgmt.st_bss_mgmt);
    pst_scaned_bss = hmac_scan_find_scanned_bss_by_bssid(pst_bss_mgmt, mac_vap->auc_bssid, WLAN_MAC_ADDR_LEN);
...
void hmac_assoc_set_siso_mode(mac_vap_stru *pst_mac_vap, uint8_t *puc_req_frame)
{
    ...
    if ((pst_hmac_user->en_user_ap_type & MAC_AP_TYPE_MIMO_BLACKLIST) &&
        (oal_compare_mac_addr(pst_hmac_user->auc_mimo_blacklist_mac, pst_mac_vap->auc_bssid) == 0)) {
                                                ...
                                                en_app_ie_type_uint8 en_app_ie_type)
{
    ...
    ies_info.frame_ies = mac_vap->ast_app_ie[en_app_ie_type].puc_ie;
    ies_info.frame_len = mac_vap->ast_app_ie[en_app_ie_type].ie_len;
    ...
    if (hmac_sta->bit_11r_enable == OAL_TRUE) {
        ...
        if ((hmac_sta->bit_reassoc_flag == OAL_FALSE && mac_find_ie(MAC_EID_MOBILITY_DOMAIN, &ies_info) == NULL)
...
uint16_t hmac_set_assoc_req_frame_ht_ie(mac_vap_stru *mac_vap, uint8_t *req_frame,
                                        hmac_scanned_bss_info *scaned_bss)
{
    uint8_t ie_len = 0;
    uint16_t out_ie_len = 0;

    if (oal_any_null_ptr3(mac_vap, req_frame, scaned_bss)) {
        return 0;
    }
...
static uint16_t hmac_set_assoc_req_opmode_ie(mac_vap_stru *mac_vap, uint8_t *req_frame)
{
    ...
    if ((hmac_user != NULL) && (mac_vap->st_cap_flag.bit_opmode == OAL_TRUE) &&
        (mac_vap->en_protocol == WLAN_VHT_MODE || mac_vap->en_protocol == WLAN_VHT_ONLY_MODE)) { /* vht需携带opmode */
...
*/
...
#ifdef _PRE_WLAN_FEATURE_ML
        req_frame += hmac_set_assoc_req_basic_ml_ie(hmac_sta, req_frame);
#endif

#ifdef _PRE_WLAN_FEATURE_11BE
        /* 封装EHT IE信息 */
        if (g_wlan_spec_cfg->feature_11be_is_open) {
            req_frame += mac_set_eht_ie(mac_vap, req_frame, WLAN_ASSOC_REQ);
        }
#endif
    ...
    if (g_wlan_spec_cfg->feature_hiex_is_open) {
        req_frame += hmac_hiex_encap_ie(mac_vap, NULL, req_frame, pst_scaned_bss);
        req_frame += hmac_assoc_req_set_max_tx_power_ie(req_frame);
    }
#endif
    req_frame += mac_set_hisi_cap_vendor_ie(mac_vap, req_frame);
#ifdef _PRE_WLAN_CHBA_MGMT
    ...
    if (hmac_chba_vap_start_check(hmac_sta) == OAL_TRUE) {
        uc_ie_len = hmac_chba_encap_assoc_attribute(req_frame, 0);
        req_frame += uc_ie_len;
    }
#endif
    /* 填充P2P/WPS IE 信息 */
...
#ifdef _PRE_WLAN_FEATURE_WUR
    req_frame += mac_encap_wur_ie(mac_vap, req_frame, WLAN_ASSOC_REQ);
#endif
...
#ifdef _PRE_WLAN_FEATURE_ML
OAL_STATIC uint16_t hmac_set_ml_reconfig_req_frame_base_ie(hmac_vap_stru *hmac_sta, mac_vap_stru *mac_vap,
    uint8_t *req_frame)
{
    uint8_t ie_len = 0;
    uint16_t out_ie_len = 0;
...
OAL_STATIC uint16_t hmac_set_reconfiguration_ml_ie_sta_complete_ie_sub(hmac_vap_stru *hmac_sta, mac_vap_stru *mac_vap,
    uint8_t *frame, hmac_scanned_bss_info *scaned_bss)
{
...
#ifdef _PRE_WLAN_FEATURE_HIEX
    if (g_wlan_spec_cfg->feature_hiex_is_open) {
        ie_len = hmac_hiex_encap_ie(mac_vap, NULL, req_frame, scaned_bss);
        req_frame += ie_len;
        ie_len = hmac_assoc_req_set_max_tx_power_ie(req_frame);
        req_frame += ie_len;
    }
#endif
     ...
     * 考虑chba 2.0与chba 1.0连接时兼容性, 需保证驱动侧chba_ie组在上层下发的app_ie之前 */
    if (hmac_chba_vap_start_check(hmac_sta) == OAL_TRUE) {
        ie_len = hmac_chba_encap_assoc_attribute(req_frame, 0);
        req_frame += ie_len;
    }
#endif
    /* 填充P2P/WPS IE 信息 */
    mac_add_app_ie(mac_vap, req_frame, &app_ie_len, OAL_APP_ASSOC_REQ_IE);
    /*  P2P联调 解决发送的关联请求没有P2P WPS IE信息 */
    req_frame += app_ie_len;

#ifdef _PRE_WLAN_FEATURE_11R
    ie_len = hmac_set_assoc_req_frame_11r_ie(hmac_sta, mac_vap, req_frame, OAL_APP_ASSOC_REQ_IE);
    req_frame += ie_len;
#endif  // _PRE_WLAN_FEATURE_11R

    /* 填充端管协同IE */
    hmac_add_cowork_ie(hmac_sta, req_frame, &app_ie_len);
    req_frame += app_ie_len;

    return (uint16_t)(req_frame - frame);
}
    ...
    uint8_t *frame, hmac_scanned_bss_info *scaned_bss)
{
    ...
    if (hmac_sta == NULL || mac_vap == NULL || frame == NULL) {
        oam_error_log0(0, OAM_SF_ANY, "{hmac_set_reconfiguration_ml_ie_sta_complete_ie::null.}");
        return OAL_ERR_CODE_PTR_NULL;
    }
    req_frame += hmac_set_ml_reconfig_req_frame_base_ie(hmac_sta, mac_vap, req_frame);
    if (scaned_bss != NULL) {
#if defined(_PRE_WLAN_FEATURE_11K) ||  defined(_PRE_WLAN_FEATURE_FTM)
    ie_len = hmac_set_assoc_req_frame_11k_ie(mac_vap, req_frame, scaned_bss, hmac_sta);
    req_frame += ie_len;
#endif  // _PRE_WLAN_FEATURE_11K
    }
    /* 设置 Extended Capability IE */
    /*  如果AP 不包含Extended Capability IE，则在assoc req 中不包含该IE */
    if (hmac_sta_check_need_set_ext_cap_ie(mac_vap) == OAL_TRUE) {
        mac_set_ext_capabilities_ie(mac_vap, req_frame, &ie_len);
        req_frame += ie_len;
    }
    if (scaned_bss != NULL) {
        ie_len = hmac_set_assoc_req_frame_ht_ie(mac_vap, req_frame, scaned_bss);
        req_frame += ie_len;
    }

#ifdef _PRE_WLAN_FEATURE_OPMODE_NOTIFY
    ie_len = hmac_set_assoc_req_opmode_ie(mac_vap, req_frame);
    req_frame += ie_len;
#endif
    if (scaned_bss != NULL) {
#ifdef _PRE_WLAN_FEATURE_11AX
        if (g_wlan_spec_cfg->feature_11ax_is_open) {
            ie_len = hmac_set_he_capabilities_all_ie(scaned_bss, mac_vap, req_frame);
            req_frame += ie_len;
        }
#endif
        ...
        /* 封装EHT IE信息 */
        if (g_wlan_spec_cfg->feature_11be_is_open) {
            ie_len = mac_set_eht_ie(mac_vap, req_frame, WLAN_ASSOC_REQ);
            req_frame += ie_len;
        }
#endif
        ie_len = hmac_set_assoc_req_frame_vendor_ie(mac_vap, req_frame, scaned_bss);
        req_frame += ie_len;
    }

    ie_len = hmac_set_reconfiguration_ml_ie_sta_complete_ie_sub(hmac_sta, mac_vap, req_frame, scaned_bss);
    req_frame += ie_len;

    oam_warning_log1(0, OAM_SF_ANY, "complete profile len = %d", req_frame - frame);
    return (uint16_t)(req_frame - frame);
}
#endif
...
#ifdef _PRE_WLAN_FEATURE_11R
    if (hmac_sta->bit_11r_enable == OAL_TRUE &&
        (mac_mib_get_ft_trainsistion(&hmac_sta->st_vap_base_info) == OAL_TRUE) &&
...
uint16_t hmac_mgmt_encap_auth_req(hmac_vap_stru *hmac_sta, uint8_t *mgmt_frame, uint8_t *dest_addr)
{
   ...
   uint16_t ml_ie_len;
    ...
    hmac_sta_fill_user_info(hmac_sta, dest_addr, &user_info);
...
#ifdef _PRE_WLAN_FEATURE_ML
    if (hmac_vap_is_ml_mode(hmac_sta)) {
        ml_ie_len = hmac_set_auth_req_basic_ml_ie(hmac_sta, mgmt_frame);
        auth_req_len += ml_ie_len;
        mgmt_frame += ml_ie_len;
    }
#endif

    user_ap = (hmac_user_stru *)mac_res_get_hmac_user(hmac_sta->st_vap_base_info.us_assoc_vap_id);
    if (user_ap == NULL) {
        if (hmac_user_add(&hmac_sta->st_vap_base_info, &user_info, &user_idx) != OAL_SUCC) {
            auth_req_len = 0;
        }
    }
    return auth_req_len;
