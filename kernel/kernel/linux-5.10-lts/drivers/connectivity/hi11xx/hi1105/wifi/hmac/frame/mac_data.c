...
uint8_t mac_get_data_type_from_80211(oal_netbuf_stru *pst_netbuff, uint16_t us_mac_hdr_len, uint32_t payload_len)
{
    ...
    uc_datatype = mac_get_data_type_from_8023(pst_netbuff, (uint8_t *)pst_snap, MAC_FRAME_TYPE_8023, payload_len,
...
uint8_t mac_get_eth_data_subtype(oal_netbuf_stru *netbuff,  uint32_t frame_len)
{
    ...
    if (oal_host2net_short(ether_hdr->us_ether_type) <= MAC_DATA_TYPE_INVALID_MIN_VALUE) {
        return MAC_FRAME_TYPE_8023;
    }

    return MAC_FRAME_TYPE_RTH;
}
...
uint8_t mac_get_data_type_by_snap(oal_netbuf_stru *netbuff)
{
    ...
    datatype = mac_get_data_type_from_8023(netbuff, (uint8_t *)snap, MAC_FRAME_TYPE_8023,
...
{
    if (oal_likely(snap_type == SNAP_LLC_LSAP)) {
        return OAL_TRUE;
    } else {
        oam_warning_log1(0, OAM_SF_RX, "mac_snap_is_protocol_type:: rx a frame not 0xaa snap type[%x]", snap_type);
        if (snap_type == SNAP_BRIDGE_SPANNING_TREE || snap_type == SNAP_IBM_LAN_MANAGEMENT) {
            return OAL_FALSE;
        }
    }
...
oal_bool_enum_uint8 mac_is_dhcp6(oal_ipv6hdr_stru *ipv6hdr, uint32_t payload_len)
{
    ...
    if (ipv6hdr->nexthdr == MAC_UDP_PROTOCAL) {
        pst_udp_hdr = (udp_hdr_stru *)(ipv6hdr + 1);
    ...
    uint32_t payload_len, uint8_t direction)
{
    ...
    if (hdr_type == MAC_FRAME_TYPE_RTH) {          /* EthernetⅡ标准帧格式 */
        shift_len = ETHER_HDR_LEN;
    } else if (hdr_type == MAC_FRAME_TYPE_8023) {  /* IEEE 802.3/802.2 SNAP标准帧格式 */
    ...
    if (hdr_type == MAC_FRAME_TYPE_RTH) {
