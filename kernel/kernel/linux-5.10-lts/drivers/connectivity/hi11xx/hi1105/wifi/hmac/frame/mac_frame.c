...
/*
 * 函 数 名  : mac_check_ie_is_non_inheritance
 * 功能描述  : 判断目标IE是否属于非继承IE
 */
OAL_STATIC uint8_t mac_check_ie_is_non_inheritance(uint8_t eid, uint8_t *non_inheritance_ies,
    int32_t non_inheritance_ie_len)
{
    uint8_t non_inheritance_ie_cnt;
    uint8_t offset;

    if (non_inheritance_ie_len <= MAC_IE_EXT_HDR_LEN) {
        return OAL_FALSE;
    }

    offset = MAC_IE_EXT_HDR_LEN;
    non_inheritance_ie_cnt = non_inheritance_ies[offset];
    offset += 1;
    while ((offset < non_inheritance_ie_len) && (non_inheritance_ie_cnt > 0)) {
        if (non_inheritance_ies[offset] == eid) {
            return OAL_TRUE;
        }
        offset++;
        non_inheritance_ie_cnt--;
    }
...
/*
 * 函 数 名  : mac_check_ext_ie_is_non_inheritance
 * 功能描述  : 判断目标IE是否属于非继承IE
 */
OAL_STATIC uint8_t mac_check_ext_ie_is_non_inheritance(uint8_t ext_eid, uint8_t *non_inheritance_ies,
    int32_t non_inheritance_ie_len)
{
    uint8_t non_inheritance_ie_cnt;
    uint8_t non_inheritance_ext_ie_cnt;
    uint8_t offset;
    if (non_inheritance_ie_len <= MAC_IE_EXT_HDR_LEN) {
        return OAL_FALSE;
    }
    ...
    offset = MAC_IE_EXT_HDR_LEN;
    non_inheritance_ie_cnt = non_inheritance_ies[offset];
    if (non_inheritance_ie_len <= (offset + non_inheritance_ie_cnt + 1)) {
        return OAL_FALSE;
    }

    offset += (non_inheritance_ie_cnt + 1);
    non_inheritance_ext_ie_cnt = non_inheritance_ies[offset];
    offset += 1;
    while ((offset < non_inheritance_ie_len) && (non_inheritance_ext_ie_cnt > 0)) {
        if (non_inheritance_ies[offset] == ext_eid) {
            return OAL_TRUE;
        }
        offset++;
        non_inheritance_ext_ie_cnt--;
    }
...
/*
 * 函 数 名  : mac_check_ie_in_non_inheritance_ie
 * 功能描述  : 在per-STA profile中查找 非继承IE，若存在则判断目标IE是否属于非继承IE
 */
uint32_t mac_check_ie_in_non_inheritance_ie(uint8_t eid, uint8_t *inheritance_ies,
    int32_t inheritance_ie_len)
{
    uint8_t *non_inheritance_ies = NULL;

    non_inheritance_ies = mac_find_ie_ext_ie_in_frame(MAC_EID_EXTENSION, MAC_EID_EXT_NON_INHERITANCE,
        inheritance_ies, inheritance_ie_len);
    if (non_inheritance_ies == NULL) {
        return OAL_FALSE;
    }

    return mac_check_ie_is_non_inheritance(eid, non_inheritance_ies, (int32_t)non_inheritance_ies[1]);
}

/*
 * 函 数 名  : mac_check_ext_ie_in_non_inheritance_ie
 * 功能描述  : 在per-STA profile中查找 非继承 扩展IE，若存在则判断目标IE是否属于非继承IE
 */
uint32_t mac_check_ext_ie_in_non_inheritance_ie(uint8_t ext_eid, uint8_t *inheritance_ies,
    int32_t inheritance_ie_len)
{
    uint8_t *non_inheritance_ies = NULL;
    ...
    return mac_check_ext_ie_is_non_inheritance(ext_eid, non_inheritance_ies, (int32_t)non_inheritance_ies[1]);
}
 ...
 *   修改内容  : 新生成函数
 */
uint8_t *mac_find_ie_ext_ie_in_frame(uint8_t uc_eid, uint8_t uc_ext_eid, uint8_t *puc_ies, int32_t l_len)
{
    if (puc_ies == NULL) {
        return NULL;
    }
...
uint8_t *mac_find_ie_ext_ie(uint8_t eid, uint8_t ext_eid, mac_ies_info *ies_info)
{
    uint8_t is_non_inheritance;
    uint8_t *ie_tmp = NULL;

    if (ies_info->inheritance_ies != NULL) {
        // 查找对应IE是否特殊信息，如果非空表示找到对应IE，直接返回
        ie_tmp = mac_find_ie_ext_ie_in_frame(eid, ext_eid,
                                             ies_info->inheritance_ies, ies_info->inheritance_ie_len);
        if (ie_tmp != NULL) {
            return ie_tmp;
        }

        // 查找对应IE是否非继承，如果为TRUE，表示非继承，不处理直接返回，否则在帧体中继续查找
        is_non_inheritance = mac_check_ext_ie_in_non_inheritance_ie(ext_eid, ies_info->inheritance_ies,
            ies_info->inheritance_ie_len);
        if (is_non_inheritance == OAL_TRUE) {
            return NULL;
        }
    }

    return mac_find_ie_ext_ie_in_frame(eid, ext_eid, ies_info->frame_ies, ies_info->frame_len);
}

/*
 * 函 数 名  : mac_hisi_cap_ie_set_emlsr_cap
 * 功能描述  : STAUT自身支持emlsr端管协同，且对端AP支持emlsr私有能力，hisi vendor ie内置位bit21 emlsr支持能力
 */
static void mac_hisi_cap_ie_set_emlsr_cap(mac_vap_stru *mac_vap, mac_hisi_cap_vendor_ie_stru *vendor_ie_param)
{
    ...
    /* 只有STA组assoc req帧才携带emlsr私有能力位 */
    if (is_legacy_sta(mac_vap) == OAL_FALSE) {
        return;
    }
    hmac_device = hmac_res_get_mac_dev(mac_vap->uc_device_id);
    if (hmac_device == NULL) {
        oam_error_log0(mac_vap->uc_vap_id, OAM_SF_QOS, "{mac_set_hisi_cap_vendor_ie_ext::hmac_device null.}");
        return;
    }

    bss_mgmt = &(hmac_device->st_scan_mgmt.st_scan_record_mgmt.st_bss_mgmt);
    scaned_bss = hmac_scan_find_scanned_bss_by_bssid(bss_mgmt, mac_vap->auc_bssid, WLAN_MAC_ADDR_LEN);
    if (scaned_bss == NULL) {
        oam_error_log0(mac_vap->uc_vap_id, OAM_SF_QOS, "{mac_set_hisi_cap_vendor_ie_ext::bss null}");
        return;
    }

    /* 置位bit21 emlsr支持能力 */
    if (scaned_bss->st_bss_dscr_info.hisi_emlsr_capable == OAL_TRUE && hmac_get_emlsr_hisi_switch() == OAL_TRUE) {
        vendor_ie_param->bit_emlsr_support = OAL_TRUE;
    }
#endif
}

static void mac_set_hisi_cap_vendor_ie_ext(mac_vap_stru *mac_vap, mac_hisi_cap_vendor_ie_stru *vendor_ie_param)
{
#ifdef _PRE_WLAN_FEATURE_WUR
    if (wlan_chip_get_wur_switch() == OAL_TRUE && mac_mib_get_WUROptionImplemented(mac_vap) == OAL_TRUE) {
        vendor_ie_param->bit_wur_support = OAL_TRUE;
    }
#endif
#ifdef _PRE_WLAN_FEATURE_EMLSR
    mac_hisi_cap_ie_set_emlsr_cap(mac_vap, vendor_ie_param);
#endif
}
 ...
 *   修改内容  : 新生成函数
 */
uint8_t mac_set_hisi_cap_vendor_ie(mac_vap_stru *mac_vap, uint8_t *puc_buffer)
{
...
#ifdef _PRE_WLAN_FEATURE_11AX
    if (mac_vap_is_he_eht_protocol(mac_vap)) {
        /* 初始化私有IE MAC_HISI_CAP_IE能力, 由定制化获取 */
        memcpy_s(pst_vendor_ie_param, sizeof(mac_hisi_cap_vendor_ie_stru),
                 &mac_device->device_cap->hisi_priv_cap, sizeof(mac_hisi_cap_vendor_ie_stru));
        pst_vendor_ie_param->bit_11ax_support = OAL_TRUE; /* 重新单独赋值 */
#ifdef _PRE_WLAN_FEATURE_11BE
        if (mac_vap->en_protocol != WLAN_EHT_MODE) {
            pst_vendor_ie_param->bit_11be_support = OAL_FALSE;
        }
#endif
    ...
    if (oal_value_eq_any2(mac_vap->en_protocol, WLAN_VHT_MODE, WLAN_VHT_ONLY_MODE)) {
        pst_vendor_ie_param->bit_1024qam_cap = mac_device->device_cap->hisi_priv_cap.bit_1024qam_cap;
        pst_vendor_ie_param->bit_vht_3nss_80m_mcs6 = mac_device->device_cap->hisi_priv_cap.bit_vht_3nss_80m_mcs6;
    }

    pst_vendor_ie_param->bit_p2p_csa_support = OAL_TRUE;
    pst_vendor_ie_param->bit_p2p_scenes = mac_device->st_p2p_info.p2p_scenes;

    mac_set_hisi_cap_vendor_ie_ext(mac_vap, pst_vendor_ie_param);

    return sizeof(mac_hisi_cap_ie_stru);
}
    ...
    mac_hisi_cap_vendor_ie_stru *hisi_cap_ie)
{
    ...
    ie_tmp = mac_find_vendor_ie(MAC_HUAWEI_VENDER_IE, MAC_HISI_CAP_IE, &ies_info);
    if (ie_tmp != NULL) {
...
OAL_STATIC void mac_set_ieee80211_vendor_ie(uint8_t *buffer, uint8_t ie_len, uint8_t oui_type)
{
    mac_ieee80211_vendor_ie_stru *vendor_ie = NULL;

    vendor_ie = (mac_ieee80211_vendor_ie_stru *)buffer;
    vendor_ie->uc_element_id = MAC_EID_VENDOR;
    vendor_ie->uc_len = ie_len - MAC_IE_HDR_LEN;
...
{
    mac_ieee80211_vendor_ie_stru *vendor_ie = NULL;
    mac_1024qam_vendor_ie_stru *mac_1024qam_vendor_ie = NULL;

    if (mac_vap->st_cap_flag.bit_1024qam != OAL_TRUE) {
        *ie_len = 0;
        return;
    }
    mac_1024qam_vendor_ie = (mac_1024qam_vendor_ie_stru *)buffer;
    memset_s(mac_1024qam_vendor_ie, sizeof(mac_1024qam_vendor_ie_stru), 0, sizeof(mac_1024qam_vendor_ie_stru));
    vendor_ie = &mac_1024qam_vendor_ie->ieee80211_vendor_ie;

    *ie_len = (uint8_t)sizeof(mac_1024qam_vendor_ie_stru);
    mac_set_ieee80211_vendor_ie((uint8_t *)vendor_ie, *ie_len, MAC_HISI_1024QAM_IE);
}
#endif
...
void mac_set_ext_capabilities_ftm_twt(mac_vap_stru *mac_vap, uint8_t *puc_buffer, uint8_t *puc_ie_len)
{
    ...
    if ((mac_mib_get_WirelessManagementImplemented(mac_vap) == OAL_TRUE) &&
        (mac_mib_get_MgmtOptionBSSTransitionImplemented(mac_vap) == OAL_TRUE) &&
        (mac_mib_get_MgmtOptionBSSTransitionActivated(mac_vap) == OAL_TRUE)) {
    ...
    if (g_wlan_spec_cfg->feature_twt_is_open) {
        if (mac_mib_get_he_TWTOptionActivated(mac_vap) == OAL_TRUE) {
    ...
    if (g_wlan_spec_cfg->feature_11ax_is_open || g_wlan_spec_cfg->feature_11be_is_open) {
...
void mac_set_vht_opern_ie_channel_width(mac_vap_stru *mac_vap, mac_vht_opern_stru *vht_opern)
{
    if ((mac_vap->st_channel.en_bandwidth >= WLAN_BAND_WIDTH_80PLUSPLUS) &&
        ...
        vht_opern->uc_channel_width = WLAN_MIB_VHT_OP_WIDTH_80_160;
#ifdef _PRE_WLAN_FEATURE_160M
...
uint8_t *mac_get_wmm_ie_ram(uint8_t *puc_beacon_body, uint16_t us_frame_len)
{
    ...
    ies_info.frame_ies = puc_beacon_body;
    ies_info.frame_len = us_frame_len;
    puc_wmmie = mac_find_vendor_ie(MAC_WLAN_OUI_MICROSOFT, MAC_WLAN_OUI_TYPE_MICROSOFT_WMM, &ies_info);
    if (puc_wmmie == NULL) {
        return NULL;
    }
 ...
 *   修改内容  : 新生成函数
 */
uint8_t *mac_find_ie_in_frame(uint8_t uc_eid, uint8_t *puc_ies, int32_t l_len)
{
    if (puc_ies == NULL) {
        return NULL;
    }
...
uint8_t *mac_find_ie(uint8_t eid, mac_ies_info *ies_info)
{
    uint8_t is_non_inheritance;
    uint8_t *ie_tmp = NULL;
    ...
    if (ies_info->inheritance_ies != NULL) {
        // 查找对应IE是否特殊信息，如果非空表示找到对应IE，直接返回
        ie_tmp = mac_find_ie_in_frame(eid, ies_info->inheritance_ies, ies_info->inheritance_ie_len);
        if (ie_tmp != NULL) {
            return ie_tmp;
        }

        // 查找对应IE是否非继承，如果为TRUE，表示非继承，不处理直接返回，否则在帧体中继续查找
        is_non_inheritance = mac_check_ie_in_non_inheritance_ie(eid,
            ies_info->inheritance_ies, ies_info->inheritance_ie_len);
        if (is_non_inheritance == OAL_TRUE) {
            return NULL;
        }
    ...
    return mac_find_ie_in_frame(eid, ies_info->frame_ies, ies_info->frame_len);
}
 ...
 * 1.日    期  : 2015年7月13日
  *   修改内容  : 新生成函数,摘自linux
 */
uint8_t *mac_find_vendor_ie_in_frame(uint32_t oui, uint8_t uc_oui_type, uint8_t *puc_ies, int32_t l_len)
{
    ...
    while (puc_pos < puc_end) {
        puc_pos = mac_find_ie_in_frame(MAC_EID_VENDOR, puc_pos, (int32_t)(puc_end - puc_pos));
        if (puc_pos == NULL) {
            return NULL;
        }
...
uint8_t *mac_find_vendor_ie(uint32_t oui, uint8_t uc_oui_type, mac_ies_info *ies_info)
{
    uint8_t is_non_inheritance;
    uint8_t *ie_tmp = NULL;
    ...
    if (ies_info->inheritance_ies != NULL) {
        // 查找对应IE是否特殊信息，如果非空表示找到对应IE，直接返回
        ie_tmp = mac_find_vendor_ie_in_frame(oui, uc_oui_type, ies_info->inheritance_ies, ies_info->inheritance_ie_len);
        if (ie_tmp != NULL) {
            return ie_tmp;
        }

        // 查找对应IE是否非继承，如果为TRUE，表示非继承，不处理直接返回，否则在帧体中继续查找
        is_non_inheritance = mac_check_ie_in_non_inheritance_ie(MAC_EID_VENDOR, ies_info->inheritance_ies,
            ies_info->inheritance_ie_len);
        if (is_non_inheritance == OAL_TRUE) {
            return NULL;
        }
    ...
    return mac_find_vendor_ie_in_frame(oui, uc_oui_type, ies_info->frame_ies, ies_info->frame_len);
}

#ifdef _PRE_WLAN_FEATURE_ML
uint8_t *mac_find_ml_ie(uint8_t eid, uint8_t ext_eid, uint8_t ml_ie_type, mac_ies_info *ies_info)
{
    mac_eht_ml_ie_header_stru  *ml_ie = NULL;
    uint8_t                    *puc_pos = NULL;
    uint8_t                    *puc_end = NULL;

    if (ies_info == NULL || ies_info->frame_ies == NULL) {
        return NULL;
    }

    puc_pos = ies_info->frame_ies;
    puc_end = ies_info->frame_ies + ies_info->frame_len;

    while (puc_pos < puc_end) {
        puc_pos = mac_find_ie_ext_ie_in_frame(MAC_EID_MULTILINK, MAC_EID_EXT_EHT_ML,
                                              puc_pos, (int32_t)(puc_end - puc_pos));
        if (puc_pos == NULL) {
            return NULL;
        }

        ml_ie = (mac_eht_ml_ie_header_stru *)puc_pos;
        if (ml_ie->ie_len >= (sizeof(*ml_ie) - MAC_IE_HDR_LEN) &&
            ml_ie->ml_control.reconfiguration_ml_control.ml_control_type == ml_ie_type) {
            return puc_pos;
        }
        puc_pos += MAC_IE_HDR_LEN + ml_ie->ie_len; /* 跳过ID和length 2byte */
    }
    return NULL;
}
#endif
...
void mac_set_cap_info_ap(mac_vap_stru *mac_vap, uint8_t *puc_cap_info)
{
    ...
    ***************************************************************************/
    ...
    if (mac_mib_get_DesiredBSSType(mac_vap) == WLAN_MIB_DESIRED_BSSTYPE_INDEPENDENT) {
        pst_cap_info->bit_ibss = 1;
    } else if (mac_mib_get_DesiredBSSType(mac_vap) == WLAN_MIB_DESIRED_BSSTYPE_INFRA) {
...
uint8_t mac_set_supported_rates_ie(mac_vap_stru *mac_vap, uint8_t *puc_buffer, uint32_t len)
{
    ...
    if (mac_vap->en_vap_mode == WLAN_VAP_MODE_BSS_STA && ((mac_vap->en_protocol == WLAN_VHT_MODE) ||
        ...
        (g_wlan_spec_cfg->feature_11be_is_open && (mac_vap->en_protocol == WLAN_EHT_MODE)))) {
 ...
 * 1.日    期  : 2013年4月11日
  *   修改内容  : 新生成函数
 */
uint8_t mac_set_dsss_params(mac_vap_stru *mac_vap, uint8_t *buffer, uint8_t chan_num)
{
    /***************************************************************************
                        ----------------------------------------
    ...
    ***************************************************************************/
     ...
     */
    buffer[BYTE_OFFSET_0] = MAC_EID_DSPARMS;
    buffer[BYTE_OFFSET_1] = MAC_DSPARMS_LEN;
    buffer[BYTE_OFFSET_2] = chan_num;
...
uint8_t mac_set_country_ie(mac_vap_stru *mac_vap, uint8_t *puc_buffer)
{
    ...
    if ((mac_mib_get_dot11MultiDomainCapabilityActivated(mac_vap) != OAL_TRUE) &&
        (mac_mib_get_dot11SpectrumManagementRequired(mac_vap) != OAL_TRUE) &&
        (mac_mib_get_dot11RadioMeasurementActivated(mac_vap) != OAL_TRUE)) {
        /* 没有使能管制域ie */
        return 0;
    }
...
uint8_t mac_set_pwrconstraint_ie(mac_vap_stru *mac_vap, uint8_t *puc_buffer)
{
    ...
    减去实际使用时的最大值
    ***************************************************************************/
    if (mac_mib_get_dot11SpectrumManagementRequired(mac_vap) == OAL_FALSE) {
        return 0;
    }
...
uint8_t mac_set_tpc_report_ie(mac_vap_stru *mac_vap, uint8_t *puc_buffer)
{
    ...
    ***************************************************************************/
    if ((mac_mib_get_dot11SpectrumManagementRequired(mac_vap) == OAL_FALSE) &&
        (mac_mib_get_dot11RadioMeasurementActivated(mac_vap) == OAL_FALSE)) {
        return 0;
    }
...
uint8_t mac_set_erp_ie(mac_vap_stru *mac_vap, uint8_t *puc_buffer)
{
    ...
    *(puc_buffer + BYTE_OFFSET_2) = 0; /* 初始清0 */
    ...
    if ((mac_vap->st_protection.uc_sta_no_short_preamble_num != 0) ||
        (mac_mib_get_ShortPreambleOptionImplemented(mac_vap) == OAL_FALSE)) {
    ...
    uint32_t group_mgmt_suit, int32_t *l_ret)
{
    ...
    if ((mac_mib_get_dot11RSNAMFPC((mac_vap_stru *)pst_mac_vap) == OAL_TRUE) && group_mgmt_suit) {
...
uint8_t mac_set_rsn_ie(mac_vap_stru *mac_vap, uint8_t *puc_pmkid, uint8_t *puc_buffer)
{
    ...
    if ((mac_mib_get_rsnaactivated(mac_vap) != OAL_TRUE) || (mac_vap->st_cap_flag.bit_wpa2 != OAL_TRUE)) {
        return 0;
    }
...
uint8_t mac_set_wpa_ie(mac_vap_stru *mac_vap, uint8_t *puc_buffer)
{
    ...
    if ((mac_mib_get_rsnaactivated(mac_vap) != OAL_TRUE) || (mac_vap->st_cap_flag.bit_wpa != OAL_TRUE)) {
        return 0;
    }
    ...
    for (uc_loop = 0; uc_loop < uc_pair_suites_num; uc_loop++) {
        if (memcpy_s(&puc_buffer[uc_index], sizeof(uint32_t), (uint8_t *)(&pcip[uc_loop]), sizeof(uint32_t)) != EOK) {
            oam_error_log0(0, OAM_SF_ANY, "mac_set_wpa_ie::memcpy fail!");
        }
        uc_index += INDEX_OFFSET_4;
    }
    ...
    for (uc_loop = 0; uc_loop < uc_akm_suites_num; uc_loop++) {
        if (memcpy_s(&puc_buffer[uc_index], sizeof(uint32_t), (uint8_t *)(&akm[uc_loop]), sizeof(uint32_t)) != EOK) {
            oam_error_log0(0, OAM_SF_ANY, "mac_set_wpa_ie::memcpy fail!");
        }
        uc_index += INDEX_OFFSET_4;
    }
...
void mac_set_sup_mcs_set_field(mac_vap_stru *mac_vap, uint8_t *puc_buffer)
{
    ...
    *************************************************************************/
    ...
    if (memcpy_s(pst_sup_mcs_set->auc_rx_mcs, WLAN_HT_MCS_BITMASK_LEN,
        mac_mib_get_SupportedMCSRx(mac_vap), WLAN_HT_MCS_BITMASK_LEN) != EOK) {
        return;
    }

    pst_sup_mcs_set->bit_rx_highest_rate = mac_mib_get_HighestSupportedDataRate(mac_vap);

    if (mac_mib_get_TxMCSSetDefined(mac_vap) == OAL_TRUE) {
        pst_sup_mcs_set->bit_tx_mcs_set_def = 1;

        if (mac_mib_get_TxRxMCSSetNotEqual(mac_vap) == OAL_TRUE) {
            ...
            if (mac_mib_get_TxUnequalModulationSupported(mac_vap) == OAL_TRUE) {
...
void mac_set_ht_extcap_field(mac_vap_stru *mac_vap, uint8_t *puc_buffer)
{
    ...
    ***********************************************************************/
    ...
    if (mac_mib_get_pco_option_implemented(mac_vap) == OAL_TRUE) {
 ...
 *   修改内容  : 新生成函数
 */
uint8_t mac_set_timeout_interval_ie(mac_vap_stru *mac_vap, uint8_t *puc_buffer, uint32_t type, uint32_t timeout)
{
    ...
    */
    ...
    return MAC_IE_HDR_LEN + MAC_TIMEOUT_INTERVAL_INFO_LEN;
#else
    return 0;
#endif
}
...
{
    if ((mac_mib_get_HighThroughputOptionImplemented(mac_vap) != OAL_TRUE) ||
        (((mac_is_wep_enabled(mac_vap) == OAL_TRUE) ||
          (mac_is_tkip_only(mac_vap) == OAL_TRUE)) &&
...
uint8_t mac_set_obss_scan_params(mac_vap_stru *mac_vap, uint8_t *puc_buffer)
{
    ...
    if ((mac_vap->st_channel.en_band != WLAN_BAND_2G) ||
        (mac_mib_get_FortyMHzOperationImplemented(mac_vap) != OAL_TRUE)) {
        return 0;
    }
...
void mac_set_ext_capabilities_ie(mac_vap_stru *mac_vap, uint8_t *puc_buffer, uint8_t *puc_ie_len)
{
    ...
    ***************************************************************************/
    ...
    if ((mac_mib_get_2040BSSCoexistenceManagementSupport(mac_vap) == OAL_TRUE) &&
    ...
    if ((mac_mib_get_VHTOptionImplemented(mac_vap) == OAL_TRUE) &&
        (((mac_is_wep_enabled(mac_vap) == OAL_FALSE) &&
          (mac_is_tkip_only(mac_vap) == OAL_FALSE)) ||
         (mac_vap->en_vap_mode != WLAN_VAP_MODE_BSS_AP))) {
...
{
    return ((mac_mib_get_VHTOptionImplemented(mac_vap) != OAL_TRUE) ||
            (((mac_is_wep_enabled(mac_vap) == OAL_TRUE) ||
              (mac_is_tkip_only(mac_vap) == OAL_TRUE)) &&
             (mac_vap->en_vap_mode == WLAN_VAP_MODE_BSS_AP))
...
uint8_t *mac_get_ssid(uint8_t *puc_beacon_body, int32_t l_frame_body_len, uint8_t *puc_ssid_len)
{
    ...
    *puc_ssid_len = 0;
    ...
    ies_info.frame_ies = puc_beacon_body + us_offset;
    ies_info.frame_len = l_frame_body_len - us_offset;
    /* 查找ssid的ie */
    puc_ssid_ie = mac_find_ie(MAC_EID_SSID, &ies_info);
...
uint8_t mac_get_dtim_period(uint8_t *puc_frame_body, uint16_t us_frame_body_len)
{
    ...
    ies_info.frame_ies = puc_frame_body + us_offset;
    ies_info.frame_len = us_frame_body_len - us_offset;

    puc_ie = mac_find_ie(MAC_EID_TIM, &ies_info);
 ...
 * 1.日    期  : 2013年6月25日
  *   修改内容  : 新生成函数
 */
uint8_t *mac_get_wmm_ie_in_frame(uint8_t *puc_beacon_body, uint16_t us_frame_len)
{
    uint8_t *puc_wmmie = NULL;

    puc_wmmie = mac_find_vendor_ie_in_frame(MAC_WLAN_OUI_MICROSOFT, MAC_WLAN_OUI_TYPE_MICROSOFT_WMM,
...
uint8_t *mac_get_wmm_ie(mac_ies_info *ies_info)
{
    uint8_t is_non_inheritance;
    uint8_t *ie_tmp = NULL;
    ...
    if (ies_info->inheritance_ies != NULL) {
        // 查找对应IE是否特殊信息，如果非空表示找到对应IE，直接返回
        ie_tmp = mac_get_wmm_ie_in_frame(ies_info->inheritance_ies, ies_info->inheritance_ie_len);
        if (ie_tmp != NULL) {
            return ie_tmp;
        }
    ...
    return mac_get_wmm_ie_in_frame(ies_info->frame_ies, ies_info->frame_len);
}
...
void mac_set_power_cap_ie(uint8_t *pst_vap, uint8_t *puc_buffer, uint8_t *puc_ie_len)
{
    ...
    if ((mac_mib_get_dot11SpectrumManagementRequired(pst_mac_vap) == OAL_FALSE) &&
        (mac_mib_get_dot11RadioMeasurementActivated(pst_mac_vap) == OAL_FALSE)) {
...
OAL_STATIC uint8_t mac_support_channel_cal(mac_vap_stru *mac_vap, uint8_t *buffer, uint8_t channel_max_num)
{
    ...
    for (channel_idx = 0; channel_idx < channel_max_num; channel_idx++) {
        ...
        if (mac_is_channel_idx_valid(mac_vap->st_channel.en_band, channel_idx, mac_vap->st_channel.ext6g_band) ==
            OAL_SUCC) {
            channel_idx_cnt++;
...
void mac_set_supported_channel_ie(uint8_t *pst_vap, uint8_t *puc_buffer, uint8_t *puc_ie_len)
{
    ...
    if ((mac_mib_get_dot11SpectrumManagementRequired(pst_mac_vap) == OAL_FALSE) ||
        (mac_mib_get_dot11ExtendedChannelSwitchActivated(pst_mac_vap) == OAL_TRUE)) {
        *puc_ie_len = 0;
        return;
    }
...
void mac_set_wmm_ie_sta(uint8_t *pst_vap, uint8_t *puc_buffer, uint8_t *puc_ie_len)
{
    ...
    /* ------------------------------------------------------------- */
    /* 判断STA是否支持WMM */
    if (mac_mib_get_dot11QosOptionImplemented(pst_mac_vap) != OAL_TRUE) {
        *puc_ie_len = 0;
        return;
    }
 ...
 *   修改内容  : 新生成函数
 */
uint32_t mac_check_mac_privacy(uint16_t cap_info, uint8_t *mac_vap)
{
    mac_vap_stru        *pst_mac_vap = NULL;
    mac_cap_info_stru   *pst_cap_info = (mac_cap_info_stru *)&cap_info;

    if (mac_vap == NULL) {
        return OAL_ERR_CODE_PTR_NULL;
    }

    pst_mac_vap = (mac_vap_stru *)mac_vap;
...
oal_bool_enum_uint8 mac_frame_is_null_data(oal_netbuf_stru *pst_net_buf)
{
    ...
    pst_mac_header = (uint8_t *)(uintptr_t)(mac_get_cb_frame_header_addr(pst_tx_ctl));
...
void mac_set_ccfs_160m(mac_channel_stru *channel, uint8_t band_width, uint8_t *ccfs0, uint8_t *ccfs1)
{
    if (oal_unlikely(oal_any_null_ptr3(channel, ccfs0, ccfs1))) {
        return;
    }
        ...
        case WLAN_BAND_WIDTH_160PLUSPLUSPLUS:
            ...
            | 主20 | 从20 | 从40     |从80                |
                          ...
                          |主80中心频率相对于主20偏6个信道
                                     ...
                                     |中心频率相对于主20偏14个信道
            ************************************************************************/
            *ccfs0 = channel->uc_chan_number + 6;  // 6: 主80中心频率相对于主20偏6个信道
            *ccfs1 = channel->uc_chan_number + 14; // 14: 中心频率相对于主20偏14个信道
            break;
        case WLAN_BAND_WIDTH_160PLUSPLUSMINUS:
            /***********************************************************************
            |从80                | 主20 | 从20 | 从40     |
            ...
            ************************************************************************/
            *ccfs0 = channel->uc_chan_number + 6; // 6: 主80中心频率相对于主20偏6个信道
            *ccfs1 = channel->uc_chan_number - 2; // 2: 中心频率相对于主20偏-2个信道
            break;
        case WLAN_BAND_WIDTH_160PLUSMINUSPLUS:
            /***********************************************************************
            | 从40     | 主20 | 从20 |从80                |
                       ...
                       |主80中心频率相对于主20偏-2个信道
                                     ...
                                     |中心频率相对于主20偏6个信道
            ************************************************************************/
            *ccfs0 = channel->uc_chan_number - 2; // 2: 主80中心频率相对于主20偏-2个信道
            *ccfs1 = channel->uc_chan_number + 6; // 6: 中心频率相对于主20偏6个信道
            break;
        case WLAN_BAND_WIDTH_160PLUSMINUSMINUS:
            /***********************************************************************
            |从80                | 从40     | 主20 | 从20 |
            ...
            ************************************************************************/
            *ccfs0 = channel->uc_chan_number - 2;  // 2: 主80中心频率相对于主20偏-2个信道
            *ccfs1 = channel->uc_chan_number - 10; // 10: 中心频率相对于主20偏-10个信道
            break;
        case WLAN_BAND_WIDTH_160MINUSPLUSPLUS:
            /***********************************************************************
            | 从20 | 主20 | 从40     |从80                |
                          ...
                          |主80中心频率相对于主20偏2个信道
                                     ...
                                     |中心频率相对于主20偏10个信道
            ************************************************************************/
            *ccfs0 = channel->uc_chan_number + 2;  // 2: 主80中心频率相对于主20偏2个信道
            *ccfs1 = channel->uc_chan_number + 10; // 10: 中心频率相对于主20偏10个信道
            break;
        case WLAN_BAND_WIDTH_160MINUSPLUSMINUS:
            /***********************************************************************
            |从80                | 从20 | 主20 | 从40     |
            ...
            ************************************************************************/
            *ccfs0 = channel->uc_chan_number + 2; // 2: 主80中心频率相对于主20偏2个信道
            *ccfs1 = channel->uc_chan_number - 6; // 6: 中心频率相对于主20偏-6个信道
            break;
        case WLAN_BAND_WIDTH_160MINUSMINUSPLUS:
            /***********************************************************************
            | 从40     | 从20 | 主20 |从80                |
                       ...
                       |主80中心频率相对于主20偏-6个信道
                                     ...
                                     |中心频率相对于主20偏2个信道
            ************************************************************************/
            *ccfs0 = channel->uc_chan_number - 6; // 6: 主80中心频率相对于主20偏-6个信道
            *ccfs1 = channel->uc_chan_number + 2; // 2: 中心频率相对于主20偏2个信道
            break;
        case WLAN_BAND_WIDTH_160MINUSMINUSMINUS:
            /***********************************************************************
            |从80                |  从40     | 从20 | 主20 |
            ...
            ************************************************************************/
            *ccfs0 = channel->uc_chan_number - 6;  // 6: 主80中心频率相对于主20偏-6个信道
            *ccfs1 = channel->uc_chan_number - 14; // 14: 中心频率相对于主20偏-14个信道
            break;
        default:
            *ccfs0 = 0;
            *ccfs1 = 0;
...
void mac_set_ccfs_below_80m(mac_channel_stru *channel, uint8_t band_width, uint8_t *ccfs0)
{
    if (oal_unlikely(oal_any_null_ptr2(channel, ccfs0))) {
        return;
    }

    switch (band_width) {
        case WLAN_BAND_WIDTH_20M:
            *ccfs0 = channel->uc_chan_number;
            break;
        case WLAN_BAND_WIDTH_40PLUS:
            *ccfs0 = channel->uc_chan_number + 2; // 2: 中心频率相对于主20偏2个信道
            break;
        case WLAN_BAND_WIDTH_40MINUS:
            *ccfs0 = channel->uc_chan_number - 2; // 2: 中心频率相对于主20偏-2个信道
            break;
        case WLAN_BAND_WIDTH_80PLUSPLUS:
            /***********************************************************************
            | 主20 | 从20 | 从40       |
                          ...
                          |中心频率相对于主20偏6个信道
            ************************************************************************/
            *ccfs0 = channel->uc_chan_number + 6; // 6: 中心频率相对于主20偏6个信道
            break;
        case WLAN_BAND_WIDTH_80PLUSMINUS:
            /***********************************************************************
            | 从40        | 主20 | 从20 |
        ...
        case WLAN_BAND_WIDTH_80MINUSMINUS:
            ...
            | 从40        | 从20 | 主20 |
                          ...
                          |中心频率相对于主20偏-6个信道
            ************************************************************************/
            *ccfs0 = channel->uc_chan_number - 6; // 6: 中心频率相对于主20偏-6个信道
            break;
        default:
            *ccfs0 = 0;
            break;
