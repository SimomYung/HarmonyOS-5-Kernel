    ...
    uint8_t action_type, uint8_t resp_status)
{
    ...
    if (oal_unlikely(oal_any_null_ptr2(mac_vap, buffer))) {
        oam_error_log0(0, OAM_SF_ANY, "{mac_set_wur_mode_ie::input NULL.}");
        return 0;
    }
...
uint16_t hmac_mgmt_encap_asoc_rsp_ap_set_wur_ie(mac_vap_stru *mac_vap, mac_user_stru *mac_user,
    uint8_t *buffer)
{
    if (oal_any_null_ptr3(mac_vap, mac_user, buffer)) {
        return 0;
    }
...
OAL_STATIC uint32_t mac_process_wur_mode_req(mac_vap_stru *mac_vap, hmac_user_stru *hmac_user,
    uint8_t *ie_payload, uint8_t ie_payload_len)
{
    mac_frame_wur_mode_fixed_ie_stru *fixed_ie = NULL;
    ...
    if (ie_payload_len < sizeof(mac_frame_wur_mode_fixed_ie_stru) + sizeof(wur_sta_params_stru)) {
        return OAL_FAIL;
    }

    fixed_ie = (mac_frame_wur_mode_fixed_ie_stru *)ie_payload;

    ie_payload += sizeof(mac_frame_wur_mode_fixed_ie_stru);

    wur_params = (wur_sta_params_stru *)ie_payload;
...
OAL_STATIC void mac_process_wur_mode_resp_update_cfg_info(hmac_vap_stru *hmac_vap, mac_cfg_wur_stru *cfg_info)
{
    uint32_t compressed_id = 0;

    /* transmit_id、embedded_bssid不在帧内携带，通过ap的bssid计算而来 */
    compressed_id = hmac_wur_calc_compressed_id(hmac_vap->st_vap_base_info.auc_bssid, WLAN_MAC_ADDR_LEN);
...
OAL_STATIC uint32_t mac_wur_parse_group_id(mac_cfg_wur_stru *cfg_info, mac_frame_wur_mode_fixed_ie_stru *fixed_ie,
    uint8_t *buffer, uint8_t remain_len)
{
    ...
    if (fixed_ie->wur_params_control.wur_group_id_list_present == OAL_TRUE) {
        if (len < sizeof(wur_group_id_list_stru)) {
            return OAL_FAIL;
        }
        groud_id_list = (wur_group_id_list_stru *)buffer;
        buffer += sizeof(wur_group_id_list_stru);
        len -= sizeof(wur_group_id_list_stru);
        ...
        if (groud_id_list->wur_group_id_bitmap_size == MAC_WUR_GROUP_ID_BITMAP_SIZE_0BIT) {
            ...
            cfg_info->group_id_bitmap = oal_make_word16(buffer[0], buffer[1]);
        } else if (groud_id_list->wur_group_id_bitmap_size == MAC_WUR_GROUP_ID_BITMAP_SIZE_32BIT) {
            if (len != sizeof(uint32_t)) {
                return OAL_FAIL;
            }
            cfg_info->group_id_bitmap =
                oal_join_word32(buffer[0], buffer[1], buffer[BYTE_OFFSET_2], buffer[BYTE_OFFSET_3]);
        } else if (groud_id_list->wur_group_id_bitmap_size == MAC_WUR_GROUP_ID_BITMAP_SIZE_64BIT) {
            if (len != sizeof(uint64_t)) {
                return OAL_FAIL;
            }
            lo = oal_join_word32(buffer[0], buffer[1], buffer[BYTE_OFFSET_2], buffer[BYTE_OFFSET_3]);
            hi = oal_join_word32(buffer[BYTE_OFFSET_4], buffer[BYTE_OFFSET_5], buffer[BYTE_OFFSET_6],
                buffer[BYTE_OFFSET_7]);
            cfg_info->group_id_bitmap = oal_make_word64(lo, hi);
        }
    } else {
...
OAL_STATIC uint32_t mac_process_wur_mode_resp(mac_vap_stru *mac_vap, hmac_user_stru *hmac_user,
    uint8_t *buffer, uint8_t ie_payload_len)
{
    mac_frame_wur_mode_fixed_ie_stru *fixed_ie = NULL;
    hmac_vap_stru *hmac_vap = NULL;
    ...
    if (fixed_ie->wur_mode_resp_status != WUR_ACCEPT) {
        oam_warning_log1(0, OAM_SF_ANY, "{mac_process_wur_mode_resp::reject![%d]}", fixed_ie->wur_mode_resp_status);
        hmac_vap_state_change_notify_feature(hmac_vap, hmac_user, COFEATURE_AP_DISABLE_EVENT, COFEATURE_ENTER);
        return OAL_FAIL;
    }

    if (ie_payload_len < sizeof(mac_frame_wur_mode_fixed_ie_stru) + sizeof(wur_ap_params_stru)) {
        return OAL_FAIL;
    }
    ...
    remain_len = ie_payload_len - sizeof(mac_frame_wur_mode_fixed_ie_stru) - sizeof(wur_ap_params_stru);
    if (mac_wur_parse_group_id(cfg_info, fixed_ie, buffer, remain_len) != OAL_SUCC) {
        return OAL_FAIL;
    }

    mac_process_wur_mode_resp_update_cfg_info(hmac_vap, cfg_info);
...
uint32_t mac_process_wur_mode_ie(mac_vap_stru *mac_vap, hmac_user_stru *hmac_user, uint8_t *wur_mode_ie)
{
    ...
    len = wur_mode_ie[1];
    /* ap接受时最小长度14, 拒绝时长度固定6 */
    if (is_sta(mac_vap) && len < MAC_WUR_MODE_AP_MIN_IE_LEN && len != MAC_WUR_MODE_AP_REJECT_IE_LEN) {
    ...
    ie_payload = wur_mode_ie + MAC_IE_EXT_HDR_LEN;
    ie_payload_len = len - MAC_IE_EID_LEN;
    fixed_ie = (mac_frame_wur_mode_fixed_ie_stru *)ie_payload;
    switch (fixed_ie->action_type) {
        case ENTER_WUR_MODE_REQ:
            ret = mac_process_wur_mode_req(mac_vap, hmac_user, ie_payload, ie_payload_len);
            break;
        case ENTER_WUR_MODE_RESP:
            ret = mac_process_wur_mode_resp(mac_vap, hmac_user, ie_payload, ie_payload_len);
            break;
    ...
    hmac_user_stru *hmac_user, uint8_t *payload, uint32_t msg_len)
{
    ...
    if (memset_s(&mac_wur_hdl, sizeof(mac_wur_hdl_stru), 0, sizeof(mac_wur_hdl_stru)) != EOK) {
        return OAL_FAIL;
    }
    mac_user_set_wur_hdl(mac_user, &mac_wur_hdl);
    ...
    if (wur_cap_ie != NULL) {
        mac_process_wur_capabilities_ie(mac_vap, mac_user, wur_cap_ie);
    }

    /* 更新WUR Operation ie */
    wur_oper_ie = mac_find_ie_ext_ie_in_frame(MAC_EID_WUR, MAC_EID_EXT_WUR_OPERATION, payload, (int32_t)msg_len);
    if (wur_oper_ie != NULL) {
        mac_process_wur_operation_ie(mac_vap, mac_user, wur_oper_ie);
    }
