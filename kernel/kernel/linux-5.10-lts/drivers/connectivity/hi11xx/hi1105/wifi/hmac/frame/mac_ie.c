 ...
 * 修改内容  : 新生成函数
 */
uint32_t mac_ie_proc_ht_sta(mac_vap_stru *mac_sta, uint8_t *payload,
 ...
 * 修改内容  : 新生成函数
 */
wlan_mib_mimo_power_save_enum mac_ie_proc_sm_power_save_field(mac_user_stru *mac_user, uint8_t smps)
{
 ...
 * 修改内容  : 新生成函数
 */
oal_bool_enum_uint8 mac_proc_he_uora_parameter_set_ie(mac_vap_stru *mac_vap, mac_ies_info *ies_info)
{
    uint8_t *uora_ie = NULL;
    mac_he_uora_para_stru *vap_he_uora_para = NULL;
    mac_he_uora_para_stru he_uora_para = { 0 };

    if (oal_any_null_ptr2(mac_vap, ies_info)) {
        oam_error_log0(0, OAM_SF_11AX, "{mac_proc_he_uora_parameter_set_ie::mac_vap or ies_info is null.}");
        return OAL_FALSE;
    }
     ...
     * -------------------------------------------------------------------------
     */
    uora_ie = mac_find_ie_ext_ie(MAC_EID_HE, MAC_EID_EXT_UORA_PARAMETER_SET, ies_info);
    ...
    mac_frame_he_mu_edca_parameter_ie_stru *he_mu_edca_value)
{
    ...
    if (oal_unlikely(oal_any_null_ptr2(he_edca_ie, he_mu_edca_value))) {
    ...
    he_edca_ie = he_edca_ie + IE_BUFFER_HEADER;

    /* 解析HE MU EDCA  Parameters Set Element */
    pst_he_edca = (mac_frame_he_mu_edca_parameter_ie_stru*)he_edca_ie;
    if (memcpy_s((uint8_t*)(he_mu_edca_value), sizeof(mac_frame_he_mu_edca_parameter_ie_stru),
        pst_he_edca, sizeof(mac_frame_he_mu_edca_parameter_ie_stru)) != EOK) {
 ...
 * 修改内容  : 新生成函数
 */
uint32_t mac_ie_parse_spatial_reuse_parameter(uint8_t *he_srp_ie,
    mac_frame_he_spatial_reuse_parameter_set_ie_stru *he_srp_value)
{
    ...
    int32_t l_ret;
    uint32_t ie_len;
    if (oal_any_null_ptr2(he_srp_ie, he_srp_value)) {
        oam_error_log0(0, OAM_SF_11AX,
            "{mac_ie_parse_spatial_reuse_parameter::he_srp_ie or he_srp_value is null.}");
        return OAL_ERR_CODE_PTR_NULL;
    }
    if ((he_srp_ie[1] < MAC_HE_SRP_IE_MIN_LEN) || (he_srp_ie[1] > MAC_HE_SRP_IE_MAX_LEN)) {
     ...
     */
    he_buffer = he_srp_ie + IE_BUFFER_HEADER;
    ...
    l_ret = memcpy_s((uint8_t*)(&he_srp_value->st_sr_control), sizeof(mac_frame_he_sr_control_stru),
    ...
    if (he_srp_ie[1] < ie_len) {
        oam_error_log2(0, OAM_SF_11AX, "mac_ie_parse_spatial_reuse_parameter:ie[%u] len[%u]", he_srp_ie[1], ie_len);
        return OAL_FAIL;
    }

    if (pst_he_sr_control->bit_non_srg_offset_present == 1) {
        /* Non-SRG OBSS PD Max Offset  */
        he_srp_value->uc_non_srg_obss_pd_max_offset = *he_buffer;
        he_buffer += 1;
    }

    if (pst_he_sr_control->bit_srg_information_present == 1) {
        /* SRG OBSS PD Min Offset */
        he_srp_value->uc_srg_obss_pd_min_offset = *he_buffer;
        he_buffer += 1; /* (可选字段)获取 SRG OBSS PD Min Offset 长度：规范长度1字节 */

        /* SRG OBSS PD Max Offset */
        he_srp_value->uc_srg_obss_pd_max_offset = *he_buffer;
        he_buffer += 1; /* (可选字段)获取 SRG BSS PD Max Offset 长度：规范长度1字节 */

        /* SRG BSS Color Bitmap */
        l_ret += memcpy_s((uint8_t*)(&he_srp_value->auc_srg_bss_color_bitmap), MAC_HE_SRG_BSS_COLOR_BITMAP_LEN,
        ...
        /* SRG BSS Color Bitmap */
        l_ret += memcpy_s((uint8_t*)(&he_srp_value->auc_srg_partial_bssid_bitmap),
    ...
    mac_frame_bss_color_change_annoncement_ie_stru *bss_color)
{
    ...
    if (memcpy_s(bss_color, sizeof(mac_frame_bss_color_change_annoncement_ie_stru), pst_bss_color_info,
        sizeof(mac_frame_bss_color_change_annoncement_ie_stru)) != EOK) {
    ...
    mac_multi_bssid_frame_info_stru *mbssid_frame_info, uint8_t *mbssid_body_ie_len)
{
    ...
    ies_info.frame_ies = data;
    ies_info.frame_len = left_len;
    /* 解析non-transmitted bssid cap IE83 */
    non_trans_bssid_cap = mac_find_ie(MAC_EID_NONTRANSMITTED_BSSID_CAP, &ies_info);
    ...
    ies_info.frame_ies += (ie_len + MAC_IE_HDR_LEN);
    ies_info.frame_len -= (ie_len + MAC_IE_HDR_LEN);

    /* 解析 ssid元素 */
    ssid = mac_find_ie(MAC_EID_SSID, &ies_info);
    if (ssid == NULL || ssid[1] > WLAN_SSID_MAX_LEN) {
        return OAL_FAIL;
    }
    ie_len = ssid[1];
    if (memcpy_s(mbssid_frame_info->auc_non_transmitted_ssid, WLAN_SSID_MAX_LEN, ssid + MAC_IE_HDR_LEN,
        ie_len) != EOK) {
    ...
    mbssid_idx = mac_find_ie(MAC_EID_MULTI_BSSID_INDEX, &ies_info);
...
uint8_t *mac_ie_find_vendor_vht_ie(uint8_t *frame, uint16_t frame_len)
{
    ...
    ies_info.frame_ies = frame;
    ies_info.frame_len = frame_len;
    vendor_ie =
        mac_find_vendor_ie(MAC_WLAN_OUI_BROADCOM_EPIGRAM, MAC_WLAN_OUI_VENDOR_VHT_TYPE, &ies_info);
    if ((vendor_ie != NULL) && (vendor_ie[1] >= MAC_WLAN_OUI_VENDOR_VHT_HEADER)) {
        ies_info.frame_ies = vendor_ie + offset_vendor_vht;
        ies_info.frame_len = vendor_ie[1] - MAC_WLAN_OUI_VENDOR_VHT_HEADER;
...
uint8_t mac_ie_get_chan_num(uint8_t *frame_body, uint16_t frame_len,
                            uint16_t offset, uint8_t channel_number, uint16_t is_6ghz_flag)
{
    uint8_t chan_num;
...
uint32_t mac_ie_proc_chwidth_field(mac_vap_stru *mac_vap, mac_user_stru *mac_user, uint8_t chwidth)
{
    ...
    mac_vap_get_bandwidth_cap(mac_vap, &vap_bw_cap);
    vap_bw_cap = oal_min(vap_bw_cap, (wlan_bw_cap_enum_uint8)chwidth);
...
uint32_t mac_ie_proc_vht_opern_ie(mac_vap_stru *mac_vap, uint8_t *payload, mac_user_stru *mac_user)
{
    ...
    if (vht_hdl.en_channel_width > WLAN_MIB_VHT_OP_WIDTH_80PLUS80_DEPRECATED) {
...
wlan_channel_bandwidth_enum_uint8 mac_get_160mbandwith_from_offset_channel(int8_t offset_channel)
{
    switch (offset_channel) {
...
wlan_channel_bandwidth_enum_uint8 mac_get_40mbandwith_from_offset_channel(int8_t offset_channel)
{
    switch (offset_channel) {
        case 2: // channlel offset 2
            /***********************************************************************
            | 主20 | 从20 | 从40       |
                          |
                          |中心频率相对于主20偏2个信道
...
{
    /* 80+80不支持，暂按80M处理 */
    if (chan_width == WLAN_MIB_VHT_OP_WIDTH_80_160 || chan_width == WLAN_MIB_VHT_OP_WIDTH_80PLUS80_DEPRECATED) {
#ifdef _PRE_WLAN_FEATURE_160M
...
#ifdef _PRE_WLAN_FEATURE_160M
        if (chan_width == WLAN_MIB_VHT_OP_WIDTH_160_DEPRECATED) {
            if ((chan_center_freq1 - chan_center_freq0 == CHAN_OFFSET_PLUS_8) ||
                (chan_center_freq0 - chan_center_freq1 == CHAN_OFFSET_PLUS_8)) {
                return mac_get_160mbandwith_from_offset_channel((int8_t)(chan_center_freq1 - channel));
            } else {
...
#endif
        if (chan_width == WLAN_MIB_VHT_OP_WIDTH_20_40) {
    ...
    uint8_t chan_width, uint8_t channel, uint8_t chan_center_freq0, uint8_t chan_center_freq1)
{
    ...
    ******************************************************************/
    if (chan_width == WLAN_MIB_HE_6GHZ_OP_WIDTH_40) {
        return mac_get_40mbandwith_from_offset_channel((int8_t)(chan_center_freq0 - channel));
    }
    /* 80M带宽 */
