...
uint8_t g_wlan_freq_timer_period = WLAN_FREQ_TIMER_PERIOD;
#ifdef _PRE_WLAN_FEATURE_NEGTIVE_DET
...
OAL_STATIC void hmac_pps_calc(void)
{
    ...
    tx_packets = (uint32_t)oal_atomic_read(&device_stats->tx_packets);
    rx_packets = (uint32_t)oal_atomic_read(&device_stats->rx_packets);
    host_ring_rx_packets = (uint32_t)oal_atomic_read(&device_stats->host_ring_rx_packets);
    oal_atomic_set(&device_stats->tx_packets, 0);
    oal_atomic_set(&device_stats->rx_packets, 0);
    oal_atomic_set(&device_stats->host_ring_rx_packets, 0);
    device_stats->total_tx_packets += tx_packets;
    device_stats->total_rx_packets += rx_packets;
    ...
    if ((sdio_dur_ms == 0) || (sdio_dur_ms < (g_wlan_freq_timer_period >> 1))) {
        return;
    }
...
void hmac_wifi_auto_freq_ctrl_init(void)
{
    ...
    if (g_freq_lock_control.en_is_inited != OAL_TRUE) {
        OAL_MUTEX_INIT(&g_freq_lock_control.st_lock_freq_mtx);
        oal_mutex_lock(&g_freq_lock_control.st_lock_freq_mtx);
    ...
    switch (pst_mac_user->en_protocol_mode) {
        ...
        case WLAN_EHT_MODE:
            if (g_wlan_spec_cfg->feature_11be_is_open) {
                g_st_wifi_pk_mode_status.en_curr_protocol_cap = WLAN_PROTOCOL_CAP_EHT;
                break;
            }
            g_st_wifi_pk_mode_status.en_curr_protocol_cap = WLAN_PROTOCOL_CAP_BUTT;
            break;
...
{
    /*  避免2.4g ht 模式调整edca参数和txop存在负收益，只在VHT/HE/EHT才支持pk mode */
...
OAL_STATIC void hmac_calc_throughput_cpu_freq(
    uint32_t tx_throughput_mbps, uint32_t rx_throughput_mbps, uint32_t trx_pps)
{
    ...
    uint32_t cpu_freq_pps_high = WLAN_CPU_FREQ_PPS_THRESHOLD_HIGH;
    uint32_t cpu_freq_pps_low = WLAN_CPU_FREQ_PPS_THRESHOLD_LOW;
    ...
    /* pps门限:小包场景不满足吞吐量条件，使用pps判断条件(h_th:50000 l_th:10000) */
    if ((g_freq_lock_control.irq_pps_high) && (g_freq_lock_control.irq_pps_low)) {
        cpu_freq_pps_high = g_freq_lock_control.irq_pps_high << 1;
        cpu_freq_pps_low = g_freq_lock_control.irq_pps_low << 1;
    }
#ifdef _PRE_WLAN_CHBA_MGMT
    ...
    if ((tx_throughput_mbps > us_cpu_freq_high) ||
        (rx_throughput_mbps > us_cpu_freq_high) || trx_pps > cpu_freq_pps_high) {
        g_freq_lock_control.uc_req_super_cpu_freq = WLAN_CPU_FREQ_SUPER;
    } else if ((tx_throughput_mbps < us_cpu_freq_low) &&
               (rx_throughput_mbps < us_cpu_freq_low) && trx_pps < cpu_freq_pps_low) {
...
void hmac_perform_calc_throughput(uint32_t tx_throughput_mbps, uint32_t rx_throughput_mbps, uint32_t trx_pps)
{
    ...
    /* 自适应调频 */
    hmac_calc_throughput_cpu_freq(tx_throughput_mbps, rx_throughput_mbps, trx_pps);
}

OAL_STATIC oal_bool_enum_uint8 hmac_adjust_throughput_stat_txrx_bytes(uint32_t dur_time)
{
    /* 小于一个周期,不统计 */
    if (dur_time < g_wlan_freq_timer_period - 10) { // 10是当前定时器周期需要减去10ms为一个周期
        return OAL_FALSE;
    }

    /* 超过一定时常则不统计,排除定时器异常 */
    if (dur_time > ((g_wlan_freq_timer_period * WLAN_THROUGHPUT_STA_PERIOD) << BIT_OFFSET_2)) {
        dpe_wlan_device_stat_stru *device_stats = dpe_wlan_device_stat_get();
        g_freq_lock_control.pre_time = jiffies;
        g_freq_lock_control.uc_timer_cycles = 0;

        device_stats->total_rx_bytes += (uint64_t)oal_atomic_read(&device_stats->rx_bytes);
        device_stats->total_tx_bytes += (uint64_t)oal_atomic_read(&device_stats->tx_bytes);
        oal_atomic_set(&device_stats->rx_bytes, 0);
        oal_atomic_set(&device_stats->tx_bytes, 0);
...
{
    // 高流量场景下，不做forbid_sleep监测
    oal_bool_enum_uint8 check_forbid_sleep =
        g_freq_lock_control.tx_pps >= HMAC_PSM_TX_SWITCH_HIGH_PPS ? OAL_FALSE : OAL_TRUE;
    hal_pm_set_check_forbid_sleep_enable(check_forbid_sleep);
    ...
    /* fastsleep 监测 */
    hmac_fastsleep_high_pps_close_check(tx_throughput_mbps + rx_throughput_mbps);

#ifdef _PRE_WLAN_FEATURE_WUR
    /* wur 流量监测 */
    hmac_wur_high_pps_check(tx_throughput_mbps + rx_throughput_mbps);
#endif
...
void hmac_update_rx_checksum_stat(void)
{
    ...
    device_stats->rx_tcp_checksum_total_fail +=
        (uint64_t)oal_atomic_read(&device_stats->rx_tcp_checksum_fail);
    device_stats->rx_tcp_checksum_total_pass +=
        (uint64_t)oal_atomic_read(&device_stats->rx_tcp_checksum_pass);
    device_stats->rx_ip_checksum_total_fail +=
        (uint64_t)oal_atomic_read(&device_stats->rx_ip_checksum_fail);
    device_stats->rx_ip_checksum_total_pass +=
        (uint64_t)oal_atomic_read(&device_stats->rx_ip_checksum_pass);
    oal_atomic_set(&device_stats->rx_tcp_checksum_fail, 0);
    oal_atomic_set(&device_stats->rx_tcp_checksum_pass, 0);
    oal_atomic_set(&device_stats->rx_ip_checksum_fail, 0);
    oal_atomic_set(&device_stats->rx_ip_checksum_pass, 0);
    ...
    oam_warning_log4(0, 0,
        ...
        device_stats->rx_ip_checksum_total_pass,
        device_stats->rx_ip_checksum_total_fail);
    device_stats->rx_tcp_checksum_total_fail = 0;
    device_stats->rx_tcp_checksum_total_pass = 0;
    device_stats->rx_ip_checksum_total_fail = 0;
    device_stats->rx_ip_checksum_total_pass = 0;
}
...
OAL_STATIC void hmac_update_aspm_switch_on_by_pps(uint32_t trx_pps)
{
    ...
    /* 防止ASPM异常处于长时间关闭导致功耗上升 */
    if (g_freq_lock_control.hcc_aspm_switch_on == OAL_FALSE) {
        g_freq_lock_control.hcc_aspm_switch_off_protect_cnt++;
        if (g_freq_lock_control.hcc_aspm_switch_off_protect_cnt > WLAN_ASPM_SWITCH_OPEN_PROTECT_CNT) {
            hmac_update_aspm_switch(OAL_TRUE);
            g_freq_lock_control.hcc_aspm_switch_off_protect_cnt = 0;
        }
    }
...
void hmac_adjust_throughput(void)
{
    ...
    /* 总字节数 */
    rx_throughput = (uint32_t)oal_atomic_read(&device_stats->rx_bytes);
    tx_throughput = (uint32_t)oal_atomic_read(&device_stats->tx_bytes);
    if (dur_time != 0) {
...
void hmac_freq_timer_param_reset(void)
{
    dpe_wlan_device_stat_stru *device_stats = dpe_wlan_device_stat_get();
    /* 重启定时器后，ASPM相关控制变量清理 */
    g_freq_lock_control.hcc_aspm_switch_off_cnt = 0;
    ...
    device_stats->total_rx_packets += (uint32_t)oal_atomic_read(&device_stats->rx_packets);
    device_stats->total_rx_bytes += (uint64_t)oal_atomic_read(&device_stats->rx_bytes);
    device_stats->total_tx_packets += (uint32_t)oal_atomic_read(&device_stats->tx_packets);
    device_stats->total_tx_bytes += (uint64_t)oal_atomic_read(&device_stats->tx_bytes);
    oal_atomic_set(&device_stats->rx_packets, 0);
    oal_atomic_set(&device_stats->rx_bytes, 0);
    oal_atomic_set(&device_stats->tx_packets, 0);
...
void hmac_wifi_on_init(void)
{
    ...
    for (dev_id = 0; dev_id < dev_num; dev_id++) {
        hal_host_intr_regs_init(dev_id);
#ifdef _PRE_WLAN_FEATURE_FTM
        hal_host_ftm_reg_init(dev_id);
#endif
    }
    ...
    if (mac_device != NULL) {
        ...
        dpe_wlan_device_init_dyn_pcie(0);
    }
}
void hmac_wifi_off_deinit(void)
{
    uint8_t dev_num = mac_chip_get_hal_dev_num();
    uint8_t dev_id;
#ifdef _PRE_WLAN_FEATURE_LOW_LATENCY_SWITCHING
    ...
    for (dev_id = 0; dev_id < dev_num; dev_id++) {
        hal_host_device_exit(dev_id);
    }
    mac_device_reset_state_flag();
#ifdef _PRE_WLAN_FEATURE_COMPETE_OPT
    hmac_bind_wifi_thread(WLAN_IRQ_AFFINITY_BUSY_CPU);
#endif
#ifdef _PRE_WLAN_FEATURE_TAS_ANT_SWITCH
    g_tas_switch_status = OAL_FALSE;
#endif
}
/*
 * 功能描述  : 平台上下电通知wifi侧业务
