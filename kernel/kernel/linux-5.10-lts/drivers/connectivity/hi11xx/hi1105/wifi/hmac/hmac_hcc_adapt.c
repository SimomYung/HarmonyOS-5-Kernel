...
void hmac_tx_sched_info_init(void)
{
    ...
    /* 来自HOST的事件，如果从Kernel Net过来选择网络层流控+丢包的方式，如果是Wlan To Wlan 的方式，直接丢包! */
...
OAL_STATIC void hmac_hcc_extend_hdr_special_config(struct hcc_handler *hcc,
                                                   frw_event_hdr_stru *pst_event_hdr,
                                                   struct frw_hcc_extend_hdr *pst_extend_hdr)
{
#ifdef _PRE_DUAL_CHIP
    if ((hcc != NULL) && (hcc->bus_dev->dev_id == HCC_EP_WIFI_DEV1)) {
        pst_extend_hdr->chip_id = HCC_EP_WIFI_DEV1;
        pst_extend_hdr->device_id = HCC_EP_WIFI_DEV1;
        if (pst_extend_hdr->vap_id == 0) {
            pst_extend_hdr->vap_id = EP_WIFI_DEV1_CFG_VAP_ID;
        }
    } else if ((pst_event_hdr != NULL) &&
               ((pst_event_hdr->uc_device_id == HCC_EP_WIFI_DEV1) || (pst_event_hdr->uc_chip_id == HCC_EP_WIFI_DEV1))) {
        pst_extend_hdr->device_id = 0;
        pst_extend_hdr->chip_id = 0;
        if (pst_extend_hdr->vap_id == EP_WIFI_DEV1_CFG_VAP_ID) {
            pst_extend_hdr->vap_id = 0;
        }
    }
#endif
}
                                                          ...
                                                          oal_netbuf_stru *pst_netbuf)
{
    ...
    hmac_hcc_extend_hdr_special_config(NULL, pst_event_hdr, pst_hdr);
}
...
uint32_t check_headroom_add_length(mac_tx_ctl_stru *tx_ctl,
                                   uint8_t nest_type,
                                   uint8_t nest_sub_type, uint8_t cb_length)
{
    uint32_t headroom_add;

    if (mac_get_cb_80211_mac_head_type(tx_ctl) == 1) {
        /* case 1: data from net, mac head is maintence in netbuff */
        headroom_add = cb_length + MAC_80211_QOS_HTC_4ADDR_FRAME_LEN;  // 结构体肯定大于4
    } else if ((nest_type == FRW_EVENT_TYPE_WLAN_CTX) && (nest_sub_type == DMAC_WLAN_CTX_EVENT_SUB_TYPE_MGMT)) {
        /* case 2: mgmt frame, mac header is maintence in payload part */
        headroom_add = cb_length + (MAX_MAC_HEAD_LEN - MAC_80211_FRAME_LEN);
    } else {
        /* case 3: data from net, mac head not maintence in netbuff */
        /* case 4: netbuff alloced in adapt layer */
        headroom_add = cb_length + MAX_MAC_HEAD_LEN;
    }
    ...
    uint8_t nest_type, uint8_t nest_sub_type)
{
    ...
    puc_data_hdr = oal_netbuf_data(netbuf);
    ...
    if ((nest_type == FRW_EVENT_TYPE_WLAN_CTX) && (nest_sub_type == DMAC_WLAN_CTX_EVENT_SUB_TYPE_MGMT)) {
        ...
        l_ret += memcpy_s(puc_data_hdr, uc_cb_length, (uint8_t *)tx_ctrl, uc_cb_length);
        /* copy mac hdr */
        l_ret += memmove_s(puc_data_hdr + uc_cb_length, MAC_80211_FRAME_LEN,
                           ...
                           MAC_80211_FRAME_LEN);
    } else if (mac_get_cb_frame_header_addr(tx_ctrl) != 0) {
        ...
        if (mac_get_cb_80211_mac_head_type(tx_ctrl) == 0) {
            l_ret += memmove_s(puc_data_hdr + uc_cb_length, mac_get_cb_frame_header_length(tx_ctrl),
                               (uint8_t *)(uintptr_t)mac_get_cb_frame_header_addr(tx_ctrl),
                               mac_get_cb_frame_header_length(tx_ctrl));
            oal_mem_free_m((uint8_t *)(uintptr_t)mac_get_cb_frame_header_addr(tx_ctrl), OAL_TRUE);
        }
    }
    if (l_ret != EOK) {
    ...
    uint8_t nest_type, uint8_t nest_sub_type)
{
    ...
    if ((nest_type == FRW_EVENT_TYPE_WLAN_CTX) && (nest_sub_type == DMAC_WLAN_CTX_EVENT_SUB_TYPE_MGMT)) {
        ...
        l_ret += memmove_s(data_hdr + cb_len + MAX_MAC_HEAD_LEN, mac_get_cb_frame_header_length(tx_ctrl),
            (uint8_t *)(uintptr_t)mac_get_cb_frame_header_addr(tx_ctrl), mac_get_cb_frame_header_length(tx_ctrl));
        /* 帧头和帧体不连续，帧头重新申请了事件内存，此处需要释放 */
        if (mac_get_cb_80211_mac_head_type(tx_ctrl) == 0) {
            oal_mem_free_m((uint8_t *)(uintptr_t)mac_get_cb_frame_header_addr(tx_ctrl), OAL_TRUE);
        }
...
uint32_t hmac_hcc_tx_update_netbuf_addr(oal_netbuf_stru *netbuf, uint32_t hdr_len)
{
    ...
    if (addr_offset) {
        ...
        if (memmove_s((uint8_t *)oal_netbuf_data(netbuf) - addr_offset, oal_netbuf_len(netbuf),
            (uint8_t *)oal_netbuf_data(netbuf), oal_netbuf_len(netbuf)) != EOK) {
    ...
    struct hcc_transfer_param *hcc_transfer_param)
{
    ...
    if (event_hdr->uc_device_id >= HCC_CHIP_MAX_DEV) {
        oal_io_print("[E]hmac_tx event uc_device_id[%d] is exceed!\n", event_hdr->uc_device_id);
        return OAL_FAIL;
    }

    ret = (uint32_t)hcc_tx(hcc_get_handler(event_hdr->uc_device_id), netbuf, hcc_transfer_param);
...
OAL_STATIC void hmac_hcc_tx_netbuf_dump_key_event(frw_event_mem_stru *pst_hcc_event_mem,
    frw_event_hdr_stru *pst_event_hdr)
{
    frw_event_stru *pst_event = NULL;
    hmac_to_dmac_cfg_msg_stru *hmac2dmac_msg = NULL;

    pst_event = frw_get_event_stru(pst_hcc_event_mem);
    if (oal_unlikely(pst_event == NULL)) {
        oam_error_log0(0, OAM_SF_ANY, "{hmac_hcc_tx_netbuf::pst_event NULL}");
        return;
    }
    hmac2dmac_msg = (hmac_to_dmac_cfg_msg_stru *)pst_event->auc_event_data;
    if (oal_unlikely(hmac2dmac_msg == NULL)) {
        oam_error_log0(0, OAM_SF_ANY, "{hmac_hcc_tx_netbuf::hmac2dmac_msg NULL}");
        return;
    }
    /* add/del vap 关键事件打印 */
    if ((pst_event_hdr->en_type == FRW_EVENT_TYPE_HOST_CRX &&
        pst_event_hdr->uc_sub_type == HMAC_TO_DMAC_SYN_CFG) &&
        (hmac2dmac_msg->en_syn_id == WLAN_CFGID_DEL_VAP ||
        hmac2dmac_msg->en_syn_id == WLAN_CFGID_ADD_VAP)) {
        oam_warning_log4(pst_event_hdr->uc_vap_id, OAM_SF_ANY,
                         "hmac tx adapt key event, syn_id:%d, len:%d, device:%d, pipe:%d",
                         hmac2dmac_msg->en_syn_id, hmac2dmac_msg->us_len,
                         pst_event_hdr->uc_device_id, pst_event_hdr->en_pipeline);
    }
}
                            ...
                            uint32_t queue_id)
{
    ...
    hmac_hcc_tx_netbuf_dump_key_event(pst_hcc_event_mem, pst_event_hdr);
    ...
    uint32_t headroom_add)
{
    ...
    mac_get_cb_frame_header_addr(tx_ctrl) =
        (uint64_t)(uintptr_t)(oal_netbuf_header(netbuf) - MAC_80211_QOS_HTC_4ADDR_FRAME_LEN);

    return memcpy_s((uint8_t *)(uintptr_t)mac_get_cb_frame_header_addr(tx_ctrl),
        mac_get_cb_frame_header_length(tx_ctrl), (uint8_t *)(&mac_hdr), mac_get_cb_frame_header_length(tx_ctrl));
}
    ...
    uint32_t headroom_add)
{
    ...
    if (mac_get_cb_frame_header_addr(tx_ctrl) == 0 || mac_get_cb_80211_mac_head_type(tx_ctrl) == 0) {
                                         ...
                                         uint32_t payload_size)
{
    ...
    if (memcpy_s((uint8_t *)(oal_netbuf_data(pst_netbuf)),
        payload_size, (uint8_t *)pst_buf, payload_size) != EOK) {
...
/* mp13 mp15 的mac_user_id是通过rx_ctrl字段中的bit_ta_user_idx传递上来的,
   mp16/mp16c/mp17c/gf61等通过bit_rx_user_id传递mac_user_id为了保持归一所以做一次转换 */
void hmac_hcc_rx_update_rx_ctrl(mac_rx_ctl_stru *rx_ctrl)
{
    if (rx_ctrl == NULL) {
        return;
    }
    rx_ctrl->bit_rx_user_id = rx_ctrl->bit_ta_user_idx;
}
/*
...
uint32_t hmac_hcc_rx_event_comm_adapt(frw_event_mem_stru *pst_hcc_event_mem)
{
    ...
    if (bit_mac_header_len) {
        ...
        mac_get_rx_cb_mac_header_addr(pst_rx_ctrl) =
            (uint64_t)(uintptr_t)(puc_hcc_extend_hdr +
            OAL_MAX_CB_LEN + MAX_MAC_HEAD_LEN - pst_rx_ctrl->uc_mac_header_len);

        /* 将mac header的内容向高地址偏移8个字节拷贝，使得mac header和payload的内容连续 */
        l_ret += memmove_s((uint8_t *)(uintptr_t)mac_get_rx_cb_mac_header_addr(pst_rx_ctrl),
            pst_rx_ctrl->uc_mac_header_len,
            (uint8_t *)((uint8_t *)((uintptr_t)mac_get_rx_cb_mac_header_addr(pst_rx_ctrl) -
    ...
    wlan_chip_hcc_rx_update_rx_ctrl(pst_rx_ctrl);
    return OAL_SUCC;
}
                                                        ...
                                                        uint32_t revert_size_temp)
{
    ...
    if (revert_size) {
        if (memcpy_s((uint8_t *)frw_get_event_payload(pst_event_mem), revert_size,
            (uint8_t *)oal_netbuf_data(pst_hcc_netbuf), revert_size) != EOK) {
...
uint32_t hmac_hcc_rx_event_wow_comm_adapt(frw_event_mem_stru *pst_hcc_event_mem)
{
    ...
    if (bit_mac_header_len) {
        ...
        /* 需要修改pst_rx_ctrl中所有指针 */
        mac_get_rx_cb_mac_header_addr(pst_rx_ctrl) = (uint64_t)(uintptr_t)(cb + OAL_MAX_CB_LEN + MAX_MAC_HEAD_LEN);
    }
    wlan_chip_hcc_rx_update_rx_ctrl(pst_rx_ctrl);
...
/*
 * 函 数 名  : hmac_pkt_filter_program_report_rx_adapt
 * 功能描述  : hmac_pkt_filter_program_report_event接收适配
 * 1.日    期  : 2018年3月17日
 *   作    者  : wifi
 *   修改内容  : 新生成函数
 */
frw_event_mem_stru *hmac_pkt_filter_program_report_rx_adapt(frw_event_mem_stru *pst_hcc_event_mem)
{
    hcc_event_stru *pst_hcc_event_payload;
    frw_event_mem_stru *pst_event_mem = NULL;
    dmac_pkt_filter_report_event_stru *pst_report_event = NULL;
    ...
    pst_event_mem = hmac_hcc_expand_rx_adpat_event(pst_hcc_event_mem, sizeof(dmac_pkt_filter_report_event_stru));
    if (pst_event_mem == NULL) {
        return NULL;
    }

    /* 填业务事件信息 */
    pst_report_event = (dmac_pkt_filter_report_event_stru *)frw_get_event_payload(pst_event_mem);
...
uint32_t hmac_hcc_tx_device_ring_tx_adapt(frw_event_mem_stru *frw_event)
{
    ...
    if (memcpy_s(oal_netbuf_data(netbuf), OAL_MAX_CB_LEN, oal_netbuf_cb(netbuf), OAL_MAX_CB_LEN) != EOK) {
...
{
    if (pst_extend_hdr == NULL || oal_unlikely(pst_extend_hdr->en_nest_type >= FRW_EVENT_TYPE_BUTT ||
...
void hmac_rx_wifi_pm_wkup(struct frw_hcc_extend_hdr *extend_hdr, oal_netbuf_stru *netbuf)
{
    ...
    if (extend_hdr->en_nest_type == FRW_EVENT_TYPE_WLAN_DRX) {
        // 数据帧解析使用单独开关控制，避免在解析之前开关被别的事件关闭
        hmac_pm_wkup_src_parse_set(OAL_TRUE);
    ...
    if (!((extend_hdr->en_nest_type == FRW_EVENT_TYPE_WLAN_CRX) &&
        (extend_hdr->uc_nest_sub_type == DMAC_WLAN_CRX_EVENT_SUB_TYPE_RX ||
         extend_hdr->uc_nest_sub_type == DMAC_WLAN_CRX_EVENT_SUB_TYPE_RX_WOW))) {
            wlan_pm_wkup_src_debug_set(OAL_FALSE);
    }
}
#endif
    ...
    struct frw_hcc_extend_hdr *extend_hdr)
{
    ...
    if (rx_ctl == NULL) {
        oal_io_print("[WARN]rx_ctl is NULL!\n");
        return -OAL_ENOMEM;
    }
                                          ...
                                          hcc_netbuf_stru *pst_hcc_netbuf, uint8_t *pst_context)
{
    ...
    hmac_hcc_extend_hdr_special_config(hcc, NULL, pst_extend_hdr);
...
OAL_STATIC void hmac_rx_register_hcc_action_type_wifi(void)
{
    uint32_t bus_dev = 0;
    ...
    for (bus_dev = 0; bus_dev < max_dev_num; bus_dev++) {
        if (bus_dev >= HCC_CHIP_MAX_DEV) {
            oal_io_print("[E]hmac_rx_register_hcc_action_type_wifi:: bus_dev[%d] is exceed!\n", bus_dev);
            return;
        }

        pst_hcc = hcc_get_handler(bus_dev);
        if (pst_hcc == NULL) {
            oal_io_print("[E]hmac_rx_register_hcc_action_type_wifi:: dev[%d] pst_hcc is NULL!\n", bus_dev);
            return;
        }
        hcc_rx_register(pst_hcc, HCC_ACTION_TYPE_WIFI, hmac_rx_wifi_post_action_function, NULL);
    }
}
...
int32_t hmac_hcc_adapt_init(void)
{
    ...
    /* WCPU上送HCC接口注册 */
    hmac_rx_register_hcc_action_type_wifi();
#ifdef _PRE_CONFIG_HISI_PANIC_DUMP_SUPPORT
