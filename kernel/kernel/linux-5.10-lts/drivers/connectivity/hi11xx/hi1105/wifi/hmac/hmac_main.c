...
uint32_t hmac_sdt_up_dpd_data(frw_event_mem_stru *pst_event_mem)
{
    ...
    if (puc_payload == NULL) {
        oam_error_log0(0, OAM_SF_WPA, "{hmac_sdt_up_sample_data::puc_payload null.}");
        return OAL_ERR_CODE_PTR_NULL;
    }
...
OAL_STATIC uint32_t hmac_create_ba_event(frw_event_mem_stru *pst_event_mem)
{
    ...
    /* 该tid下不允许建BA，配置命令需求 */
    ba_info = hmac_get_ba_info(pst_hmac_user, uc_tidno);
    if (ba_info->en_ba_handle_tx_enable == OAL_FALSE) {
        return OAL_FAIL;
    }
...
OAL_STATIC void hmac_event_fsm_netbuf_tx_adapt_subtable_register(void)
{
    ...
    g_ast_host_wlan_ctx_event_sub_table[DMAC_WLAN_CTX_EVENT_SUB_TYPE_PKT_FILTER_CMD].p_tx_adapt_func =
        hmac_send_event_netbuf_tx_adapt;
#endif
#ifdef _PRE_WLAN_CHBA_MGMT
...
#endif
#ifdef _PRE_WLAN_FEATURE_EMLSR
    g_ast_host_wlan_ctx_event_sub_table[DMAC_WLAN_CTX_EVENT_SUB_TYPE_EMLSR_HMAC2DMAC].p_tx_adapt_func =
        hmac_send_event_netbuf_tx_adapt;
#endif
    g_ast_host_wlan_ctx_event_sub_table[DMAC_WLAN_CTX_EVENT_SUB_TYPE_MVAP_TRANS_TBL].p_tx_adapt_func =
        hmac_send_event_netbuf_tx_adapt;
}
/*
...
OAL_STATIC void hmac_event_fsm_tx_adapt_subtable_register(void)
{
    ...
    g_ast_host_wlan_ctx_event_sub_table[DMAC_WLAN_CTX_EVENT_SUB_TYPE_IP_FILTER].p_tx_adapt_func =
        hmac_send_event_netbuf_tx_adapt;
    g_ast_host_wlan_ctx_event_sub_table[DMAC_WLAN_CTX_EVENT_SUB_TYPE_ACTION_FILTER].p_tx_adapt_func =
        hmac_send_event_netbuf_tx_adapt;
    ...
    g_ast_host_wlan_ctx_event_sub_table[DMAC_WLAN_CTX_EVENT_SUB_TYPE_ELP_SCAN_REQ].p_tx_adapt_func =
        hmac_send_event_netbuf_tx_adapt;
#endif
...
#ifdef _PRE_WLAN_FEATURE_WUR
    g_ast_host_wlan_ctx_event_sub_table[DMAC_WLAN_CTX_EVENT_SUB_TYPE_UPDATE_WUR].p_tx_adapt_func =
        hmac_proc_rx_process_wur_sync_event_tx_adapt;
#endif
...
OAL_STATIC void hmac_event_fsm_rx_adapt_subtable_register(void)
{
...
#ifdef _PRE_WLAN_FEATURE_PKT_FILTER
    g_ast_hmac_wlan_misc_event_sub_table[DMAC_MISC_SUB_TYPE_PKT_FILTER_REPORT].p_rx_adapt_func =
        hmac_pkt_filter_program_report_rx_adapt;
#endif
#ifdef _PRE_WLAN_FEATURE_PSD_ANALYSIS
    ...
    g_ast_hmac_wlan_crx_event_sub_table[DMAC_WLAN_CRX_EVENT_SUB_TYPE_ALG_MASS_STATS_REPORT].p_rx_adapt_func =
        hmac_rx_convert_netbuf_to_netbuf_default;
}
...
OAL_STATIC void hmac_crx_event_subtable_register_pre_part(void)
{
    /* 注册HMAC模块WLAN_CRX事件子表 */
    ...
    g_ast_hmac_wlan_crx_event_sub_table[DMAC_WLAN_CRX_EVENT_SUB_TYPE_RX_WOW].p_func = hmac_rx_process_wow_mgmt_event;
...
#endif
#ifdef _PRE_WLAN_FEATURE_SNIFFER
    g_ast_hmac_wlan_crx_event_sub_table[DMAC_WLAN_DRX_EVENT_SUB_TYPE_RX_SNIFF_INFO].p_func = hmac_sniffer_rx_info_event;
#endif
#ifdef _PRE_DELAY_DEBUG
    ...
    g_ast_hmac_wlan_crx_event_sub_table[DMAC_WLAN_CRX_EVENT_SUB_TYPE_ALG_MASS_STATS_REPORT].p_func =
        hmac_proc_query_additional_response_event2;
}

OAL_STATIC void hmac_crx_event_subtable_register_post_part(void)
{
    g_ast_hmac_wlan_crx_event_sub_table[DMAC_WLAN_CRX_EVENT_SUB_TYPE_MLSR_LINK_SWITCH].p_func =
        hmac_mlsr_link_switch_event_process;
    g_ast_hmac_wlan_crx_event_sub_table[DMAC_WLAN_CRX_EVENT_SUB_TYPE_MLSR_LINK_SWITCH_POST_EVENT].p_func =
        hmac_mlsr_link_switch_post_event_process;
#ifdef _PRE_WLAN_FEATURE_EMLSR
    g_ast_hmac_wlan_crx_event_sub_table[DMAC_WLAN_CRX_EVNET_SUB_TYPE_EMLSR_OMN_SEND].p_func = hmac_emlsr_event_process;
    g_ast_hmac_wlan_crx_event_sub_table[DMAC_WLAN_CRX_EVNET_SUB_TYPE_EMLSR_FORCE_CLOSE].p_func =
        hmac_emlsr_force_close_event_process;
    g_ast_hmac_wlan_crx_event_sub_table[DMAC_WLAN_CRX_EVENT_SUB_TYPE_EMLSR_SWITCH].p_func = hmac_emlsr_switch_event;
    g_ast_hmac_wlan_crx_event_sub_table[DMAC_WLAN_CRX_EVENT_SUB_TYPE_NOTIFY_EMLSR_ALLOWED].p_func =
        hmac_emlsr_enter_event_process;
#endif
    g_ast_hmac_wlan_crx_event_sub_table[DMAC_WLAN_CRX_EVENT_SUB_TYPE_POWER_OFF_READY].p_func =
        hmac_d2h_power_off_ready_event_process;
#ifdef _PRE_WLAN_FEATURE_WUR
    g_ast_hmac_wlan_crx_event_sub_table[DMAC_WLAN_CRX_EVENT_SUB_TYPE_CFG_WUR_PARAM].p_func =
        hmac_config_device_allow_tx_wur_setup;
#endif
#ifdef _PRE_WLAN_FEATURE_WUR
    g_ast_hmac_wlan_crx_event_sub_table[DMAC_WLAN_CRX_EVENT_SUB_TYPE_WUR_TEARDOWN_EVENT].p_func =
        hmac_wur_teardown_event_process;
    g_ast_hmac_wlan_crx_event_sub_table[DMAC_WLAN_CRX_EVENT_SUB_TYPE_WUR_BT_RELEASE_CHAN].p_func =
        hmac_wur_bt_release_channel_event_process;
#endif
}
...
OAL_STATIC void hmac_ctx_event_subtable_register(void)
{
    ...
    g_ast_hmac_wlan_ctx_event_sub_table[DMAC_TO_HMAC_USER_INFO_SYN].p_func = hmac_user_info_report_event;
#endif
}
...
OAL_STATIC void hmac_event_fsm_action_subtable_register(void)
{
    ...
    hmac_crx_event_subtable_register_pre_part();
    hmac_crx_event_subtable_register_post_part();
...
#ifdef _PRE_WLAN_FEATURE_PKT_FILTER
    g_ast_hmac_wlan_misc_event_sub_table[DMAC_MISC_SUB_TYPE_PKT_FILTER_REPORT].p_func =
        hmac_pkt_filter_program_report_event;
#endif
