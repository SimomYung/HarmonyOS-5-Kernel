    ...
    mac_mld_mode_switch_stru *mld_mode_switch)
{
    ...
    if (netdev == NULL) {
        oam_error_log0(hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_MLD,
            "hmac_mld_mode_switch_enter_mlsr_process::get net_device NULL");
        return;
    }
    ...
    /* 更新vap user的mld参数 */
    hmac_mld_change_vap_user_mld_param(hmac_switch_vap, hmac_vap, mld_vap_manage);
    /* 交换netdev<--->vap指针关系 */
    hmac_mlsr_change_netdev_vap_relation(hmac_switch_vap, hmac_vap);
    oal_net_tx_wake_all_queues(netdev); /* 启动发送队列 */
}
void hmac_set_mlsr_emlsr_bitmap(hmac_vap_stru *hmac_vap, mac_mld_mode_switch_enum next_mld_mode)
{
    uint8_t multi_link_bitmap = 0xff;
    ...
    if (next_mld_mode == MLD_MODE_SWITCH_EMLSR || next_mld_mode == MLD_MODE_SWITCH_STR) {
        multi_link_bitmap = HAL_MULTILINK_TX_BITMAP_ALL_LINK;
    } else if (next_mld_mode == MLD_MODE_SWITCH_MLSR) {
        multi_link_bitmap = HAL_MULTILINK_TX_BITMAP_LINK0;
    }
    if (multi_link_bitmap != 0xff) {
        hmac_tx_ring_set_mld_bitmap_next(hmac_user->st_user_base_info.us_assoc_id, multi_link_bitmap);
    }
}

OAL_STATIC void hmac_mld_mode_switch_scan_abort(hmac_vap_stru *hmac_vap)
{
    uint32_t pedding_data = 0;
    hmac_config_send_event(&(hmac_vap->st_vap_base_info), WLAN_CFGID_SCAN_ABORT,
        sizeof(uint32_t), (uint8_t *)&pedding_data);
}

uint32_t hmac_config_mld_mode_switch(hmac_vap_stru *hmac_vap, mac_mld_mode_switch_enum next_mld_mode, uint8_t reason)
{
    ...
    hmac_set_mlsr_emlsr_bitmap(hmac_vap, next_mld_mode);
    /* device同步host的emlsr退出事件，device已经退出，host不需要抛事件到device */
    if (reason == EMLSR_CLOSE_FOR_BT) {
        return ret;
    }

    /* 仅仅抛事件到device，停止扫描。等待device扫描完成事件上报host，再执行扫描回调函数.
     * 避免emlsr场景，漫游扫描时，上层下发退出emlsr。引起device在emlsr状态漫游，后续不能退出emlsr状态，导致device异常 */
    hmac_mld_mode_switch_scan_abort(hmac_vap);
...
uint32_t hmac_config_mld_mode_switch_cmd(mac_vap_stru *mac_vap, uint32_t *param)
{
    ...
    hmac_config_mld_mode_switch(hmac_vap, next_mld_mode, 0xff);
    return OAL_SUCC;
}
#endif
