...
void hmac_save_ftm_range(hmac_vap_stru *hmac_vap, ftm_initiator_stru *ftm_initiator_session)
{
    ...
    oam_warning_log4(hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_FTM,
        "{hmac_save_ftm_range::AP[%d], count[%d], bit_range[%d], bit_max_error_expt[%d].}",
    ...
    uint16_t us_frame_len, ftm_initiator_stru *ftm_initiator_session)
{
    ...
    puc_ie = mac_find_ie_ext_ie(MAC_EID_FTMSI, MAC_EID_EXT_FTMSI, &ies_info);
    ...
    mac_ftmp = (mac_ftm_parameters_ie_stru *)mac_find_ie(MAC_EID_FTMP, &ies_info);
...
void hmac_ftm_responder_rx_req(hmac_vap_stru *hmac_vap, oal_netbuf_stru *netbuf)
{
    ...
    if (oal_any_null_ptr3(hmac_ftm_info, hal_device, rx_info)) {
        return;
    }
    ...
    /* ftm initiator */
    if (mac_mib_get_FtmResponderModeActivated(&hmac_vap->st_vap_base_info) == OAL_FALSE) {
        oam_warning_log0(hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_FTM,
    ...
    ies_info.frame_ies = payload + FTM_FRAME_IE_OFFSET;
    ies_info.frame_len = frame_len - FTM_FRAME_IE_OFFSET;
    mac_ftmp = (mac_ftm_parameters_ie_stru *)mac_find_ie(MAC_EID_FTMP, &ies_info);
    ...
    int32_t frame_len, uint16_t *len_to_skip)
{
    ...
    if (ftm_initiator_session->lci_ie) {
        meas_rep = mac_find_ie(MAC_EID_MEASREP, &ies_info);
        if (meas_rep != NULL) {
            *len_to_skip += (*(meas_rep + 1) + MAC_IE_HDR_LEN);
        }
    }

    ies_info.frame_ies = payload + *len_to_skip;
    ies_info.frame_len = frame_len - *len_to_skip;
    ...
    int32_t frame_len, ftm_initiator_stru *ftm_initiator_session)
{
    ...
    /* 初始化保存ftm测量结构体 */
    if (hmac_ftm_initiator_update_ftm_parameters(hmac_vap, payload + len_to_skip,
        frame_len - len_to_skip, ftm_initiator_session) == OAL_SUCC) {
...
uint32_t hmac_ftm_initiator_rx_rsp(hmac_vap_stru *hmac_vap, oal_netbuf_stru *netbuf, mac_rx_ctl_stru *rx_ctrl)
{
    uint8_t *payload = NULL;
    int32_t frame_len; /* 消息总长度,不包括FCS */
    uint16_t len_to_skip = FTM_FRAME_OPTIONAL_IE_OFFSET;
    ...
    /* 获取帧体指针 */
    payload = mac_get_80211_mgmt_payload(netbuf);
    frame_len = mac_get_rx_cb_payload_len(rx_ctrl); /* 帧体长度 */

    /* ftm initiator */
    if (mac_mib_get_FtmInitiatorModeActivated(&hmac_vap->st_vap_base_info) == OAL_FALSE) {
        oam_warning_log0(hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_FTM,
...
uint32_t hmac_handle_ftm_isr(mac_vap_stru *mac_vap, uint8_t len, uint8_t *param)
{
    ...
    mac_vap_id = hal_host_vap_get_by_hw_vap_id(hal_ftm_info.device_id, hal_ftm_info.ftm_vap_index);
...
uint32_t hmac_rrm_parse_ftm_range_req(hmac_vap_stru *hmac_vap_sta, mac_meas_req_ie_stru *meas_req_ie)
{
    ...
    for (ap_cnt = 0; ap_cnt < ftm_range_req->minimum_ap_count; ap_cnt++) {
        ies_info.frame_ies = subelements;
        ies_info.frame_len = len_left;
        ie = mac_find_ie(MAC_EID_NEIGHBOR_REPORT, &ies_info);
...
oal_bool_enum_uint8 hmac_rrm_proc_rm_request_mp16(hmac_vap_stru *hmac_vap_sta, oal_netbuf_stru *netbuf)
{
    ...
    uint16_t framebody_len;
    uint16_t framebody_len_left;
    if (rx_info == NULL) {
        return ret;
    }
    framebody_len = mac_get_rx_cb_payload_len(rx_info);
    if (framebody_len <= (MAC_MEASUREMENT_REQUEST_IE_OFFSET + MAC_IE_HDR_LEN)) {
        oam_warning_log1(hmac_vap_sta->st_vap_base_info.uc_vap_id, OAM_SF_RRM, "{hmac_rrm_proc_rm_request:: \
                Error len, framebody_len[%d]!}", framebody_len);
        return ret;
    }

    framebody_len_left = framebody_len - MAC_MEASUREMENT_REQUEST_IE_OFFSET;
