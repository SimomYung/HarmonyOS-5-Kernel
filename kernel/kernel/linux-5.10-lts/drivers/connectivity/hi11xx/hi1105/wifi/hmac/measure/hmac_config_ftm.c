    ...
    mac_send_gas_init_req_stru *pst_gas_req)
{
    ...
    if (pst_hmac_device == NULL) {
        oam_error_log0(0, OAM_SF_CFG, "{check_interworking_support::device null!}");
        return NULL;
    }
...
static uint32_t hmac_ftm_update_gas_init_req(mac_bss_dscr_stru *dscr, mac_send_gas_init_req_stru *gas_req)
{
    ...
    ies_info.frame_ies = dscr->auc_mgmt_frame_body;
    ies_info.frame_len = dscr->mgmt_len - MAC_80211_QOS_HTC_4ADDR_FRAME_LEN;
...
uint32_t  hmac_ftm_rx_gas_initial_response_frame(hmac_vap_stru *pst_hmac_vap, oal_netbuf_stru *pst_netbuf)
{
    ...
    puc_data = (uint8_t *)((uintptr_t)mac_get_rx_cb_mac_header_addr(pst_rx_info) + pst_rx_info->uc_mac_header_len);
