 ...
 *   修改内容  : 新生成函数
 */
uint32_t hmac_arp_offload_set_ip_addr_for_single_link(mac_vap_stru *mac_vap,
    ...
    const void *ip_addr, const void *mask_addr)
{
    ...
    if (oal_unlikely(oal_any_null_ptr3(mac_vap, ip_addr, mask_addr))) {
        oam_error_log0(0, OAM_SF_PWR,
            "{hmac_arp_offload_set_ip_addr_for_single_link::The mac_vap or ip_addr or mask_addr is NULL.}");
    ...
    if (type == DMAC_CONFIG_IPV4) {
        ...
        oam_error_log1(mac_vap->uc_vap_id, OAM_SF_PWR,
                       "{hmac_arp_offload_set_ip_addr_for_single_link::The type[%d] is error.}", type);
        return OAL_ERR_CODE_MAGIC_NUM_FAIL;
    }
    if (ret2 != EOK) {
        oam_error_log0(0, OAM_SF_PWR, "hmac_arp_offload_set_ip_addr_for_single_link::memcpy fail!");
        return OAL_FAIL;
    }
    ...
    if (oal_unlikely(oper >= DMAC_IP_OPER_BUTT)) {
        oam_error_log1(mac_vap->uc_vap_id, OAM_SF_PWR,
                       "{hmac_arp_offload_set_ip_addr_for_single_link::The operation[%d] is error.}", oper);
    ...
    const void *ip_addr, const void *mask_addr)
{
    ...
    if (hmac_vap_is_legacy_mld_mode(hmac_vap) == OAL_TRUE) {
        return hmac_arp_offload_set_ip_addr_for_single_link(mac_vap, type, oper, ip_addr, mask_addr);
    }

#ifdef _PRE_WLAN_FEATURE_ML
    mld_vap_manage = hmac_vap->mld_vap_link_info.mld_vap_manage;
    if (mld_vap_manage == NULL) {
        oam_error_log0(0, OAM_SF_PWR, "hmac_arp_offload_set_ip_addr::mld_vap_manange is NULL!");
        return OAL_ERR_CODE_PTR_NULL;
    }
    ...
    for (link_id = 0; link_id < mld_vap_manage->mld_link_cnt; link_id++) {
        ...
        ret = hmac_arp_offload_set_ip_addr_for_single_link(link_mac_vap, type, oper, ip_addr, mask_addr);
        if (ret != OAL_SUCC) {
            return ret;
        }
    }
