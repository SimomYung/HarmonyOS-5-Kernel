...
static uint32_t hmac_add_wlan_ctx_action_param(hmac_user_stru *hmac_user, oal_netbuf_stru *netbuf, uint16_t frame_len)
{
    ...
    if (memcpy_s((uint8_t *)(oal_netbuf_data(netbuf) + frame_len),
        (WLAN_MEM_NETBUF_SIZE2 - frame_len), (uint8_t *)&wlan_ctx_action, sizeof(dmac_ctx_action_event_stru)) != EOK) {
    ...
    uint8_t *payload)
{
    ...
    mac_btwt_action_stru *btwt_action_args = (mac_btwt_action_stru *)(payload);
    mac_itwt_ie_stru *itwt_element = (mac_itwt_ie_stru *)(payload);
    mac_twt_command_enum_uint8 twt_command =
        (mac_twt_command_enum_uint8)itwt_element->st_request_type.bit_setup_command;

    if (hmac_sta_rx_twt_setup_check(hmac_vap, hmac_user, payload) != OAL_SUCC) {
        return OAL_FAIL;
    }
    ...
    if (twt_command == MAC_TWT_COMMAND_ACCEPT) {
        ...
        /* 在dmac发送完成中断收到ack后再设置twt参数 */
        hmac_mgmt_tx_twt_setup(hmac_vap, hmac_user, (mac_twt_action_mgmt_args_stru *)payload);
    }
    return OAL_SUCC;
}
 ...
 *   修改内容  : 新生成函数
 */
uint32_t hmac_sta_rx_twt_teardown_frame(hmac_vap_stru *hmac_vap, hmac_user_stru *hmac_user, uint8_t *payload)
{
    ...
    /*                                                                */
    /******************************************************************/
    twt_teardown = (mac_twt_teardown_stru *)(payload);
 ...
 *   修改内容  : 新生成函数
 */
uint32_t hmac_dbac_teardown_twt_session(mac_device_stru *mac_dev)
{
    ...
    chip = hmac_res_get_mac_chip(mac_dev->uc_chip_id);
    if (chip == NULL) {
    ...
    if ((mac_dev->en_dbdc_running == OAL_FALSE) && (vap_num >= HMAC_TWT_P2P_RUNNING_VAP_NUM)) {
        /* 先找到wlan vap */
        ret = mac_device_find_up_sta_wlan(mac_dev, &mac_vap_temp);
 ...
 * 1.日    期  : 2018年12月11日
  *   修改内容  : 新生成函数
 */
uint32_t hmac_config_twt_setup_req(mac_vap_stru *mac_vap, uint16_t len, uint8_t *param)
{
    mac_cfg_twt_setup_req_param_stru *twt_setup_req = (mac_cfg_twt_setup_req_param_stru *)param;
    hmac_user_stru *hmac_user = NULL;
    hmac_vap_stru *hmac_vap = NULL;
 ...
 * 1.日    期  : 2018年12月11日
  *   修改内容  : 新生成函数
 */
uint32_t hmac_config_twt_teardown_req(mac_vap_stru *mac_vap, uint16_t len, uint8_t *param)
{
    ...
    twt_teardown_req = (mac_cfg_twt_teardown_req_param_stru *)param;
    hmac_vap = (hmac_vap_stru *)mac_res_get_hmac_vap(mac_vap->uc_vap_id);
    if (hmac_vap == NULL) {
 ...
 *   修改内容  : 新生成函数
 */
uint32_t mac_device_find_up_sta_wlan(mac_device_stru *mac_device, mac_vap_stru **mac_vap_out)
{
    uint8_t vap_idx;
    mac_vap_stru *mac_vap = NULL;
...
void hmac_sta_rx_btwt(uint8_t *payload, uint16_t msg_len, hmac_vap_stru *hmac_vap, hmac_user_stru *hmac_user)
{
    ...
    btwt_element = mac_find_ie(MAC_EID_TWT, &ies_info);
    if ((btwt_element == NULL) || ((btwt_element[1] + MAC_IE_HDR_LEN) != sizeof(mac_btwt_ie_stru))) {
        return;
    }
