...
#define OMI_BW_SWITCH_CMD_LEN  2
...
static const mac_chan_band_offset_stru g_chan_center_freq_offset[] = {
    ...
    { WLAN_BAND_WIDTH_40PLUS,              2,   0,  0 },
    { WLAN_BAND_WIDTH_40MINUS,             -2,   0,  0 },
    /*
     * | 主20 | 从20 | 从40       |
     *               |
     *               |中心频率相对于主20偏6个信道
 ...
 */
#ifdef _PRE_WLAN_FEATURE_6G_EXTEND
uint32_t hmac_proc_he_6ghz_band_cap_ie(mac_vap_stru *mac_vap,
    hmac_user_stru *hmac_user, uint8_t *payload)
{
    mac_he_6ghz_band_cap_info_stru he_6ghz_band_cap_ie_value;
    uint32_t ret;

    if (oal_any_null_ptr3(mac_vap, hmac_user, payload)) {
        oam_error_log0(0, OAM_SF_ASSOC,
    ...
    /* 支持11ax，才进行后续的处理 */
    if (mac_mib_get_HEOptionImplemented(mac_vap) == OAL_FALSE) {
        return MAC_NO_CHANGE;
    }
 ...
 *   修改内容  : 新生成函数
 */
void hmac_sta_up_update_he_edca_params(mac_ies_info *ies_info,
                                       ...
                                       hmac_user_stru *pst_hmac_user)
{
    ...
    if (mac_mib_get_HEOptionImplemented(&(pst_hmac_sta->st_vap_base_info)) == OAL_FALSE) {
        return;
    }
    ...
    /* --------------------------------------------------------------------------------------------- */
    /**************************************************************************/
    puc_ie = mac_find_ie_ext_ie(MAC_EID_HE, MAC_EID_EXT_HE_EDCA, ies_info);
    if (puc_ie != NULL) {
 ...
 *   修改内容  : 新生成函数
 */
void hmac_sta_up_update_he_nfrp_params(mac_ies_info *ies_info,
    hmac_vap_stru *hmac_sta, uint8_t frame_sub_type, hmac_user_stru *hmac_user)
{
    ...
    /* |  1  |  1  |   1    |                     1                     |  */
    /* ------------------------------------------------------------------- */
    ie = mac_find_ie_ext_ie(MAC_EID_HE, MAC_EID_EXT_HE_NDP_FEEDBACK_REPORT, ies_info);
    if (ie == NULL) {
        return;
    }
    ...
    /* 拷贝参数 */
    if (memcpy_s(frw_get_event_payload(event_mem), sizeof(uint8_t), &tmp_nfrp_thr_exp, sizeof(uint8_t)) != EOK) {
 ...
 *   修改内容  : 新生成函数
 */
uint32_t hmac_sta_up_update_mu_edca_params_machw(hmac_vap_stru *hmac_sta,
                                                 mac_wmm_set_param_type_enum_uint8 en_type)
{
    ...
    st_asoc_set_edca_reg_param.uc_vap_id = hmac_sta->st_vap_base_info.uc_vap_id;
    ...
    memcpy_s((uint8_t *)&st_asoc_set_edca_reg_param.ast_wlan_mib_qap_edac,
             ...
             (uint8_t *)hmac_sta->st_vap_base_info.st_wlan_mib_mu_edca.ast_wlan_mib_qap_edac,
             (sizeof(wlan_mib_Dot11QAPEDCAEntry_stru) * WLAN_WME_AC_BUTT));
 ...
 *   修改内容  : 新生成函数
 */
void hmac_sta_up_update_assoc_rsp_sr_params(mac_ies_info *ies_info,
                                            hmac_vap_stru *pst_hmac_sta,
                                            uint8_t uc_frame_sub_type,
                                            hmac_user_stru *pst_hmac_user)
{
    ...
    puc_ie = mac_find_ie_ext_ie(MAC_EID_HE, MAC_EID_EXT_HE_SRP, ies_info);
    if (puc_ie == NULL) {
        return;
    }
...
uint32_t hmac_ie_check_he_sta(hmac_vap_stru *pst_hmac_sta,
    hmac_user_stru *pst_hmac_user, mac_ies_info *ies_info)
{
    ...
    puc_tmp_ie = mac_find_ie_ext_ie(MAC_EID_HE, MAC_EID_EXT_HE_CAP, ies_info);
    if (puc_tmp_ie != NULL) {
        hmac_proc_he_cap_ie(pst_mac_vap, pst_hmac_user, puc_tmp_ie);
    }

    puc_tmp_ie = mac_find_ie_ext_ie(MAC_EID_HE, MAC_EID_EXT_HE_OPERATION, ies_info);
    if (puc_tmp_ie != NULL) {
    ...
    if (pst_mac_vap->st_channel.ext6g_band == OAL_TRUE) {
        puc_tmp_ie = mac_find_ie_ext_ie(MAC_EID_HE, MAC_EID_EXT_HE_6GHZ_BAND_CAP, ies_info);
        if (puc_tmp_ie != NULL) {
    ...
    uora_para_change = mac_proc_he_uora_parameter_set_ie(pst_mac_vap, ies_info);
 ...
 *   修改内容  : 新生成函数
 */
OAL_STATIC uint8_t g_ax_triggered_mac_padding_val = 0;
OAL_STATIC uint8_t g_ax_triggered_mac_padding_flag = OAL_FALSE;
uint32_t hmac_config_ax_debug(mac_vap_stru *pst_mac_vap, uint16_t us_len, uint8_t *puc_param)
{
    ...
    hmac_vap_stru *hmac_vap = mac_res_get_hmac_vap(pst_mac_vap->uc_vap_id);
    if (hmac_vap == NULL) {
        return OAL_ERR_CODE_PTR_NULL;
    }
    ...
    if (uc_cmd_cnt > MAC_STR_CMD_MAX_SIZE) {
        oam_error_log1(pst_mac_vap->uc_vap_id, OAM_SF_ANY, "{hmac_config_ax_debug::cnt err[%d].}", uc_cmd_cnt);
        return OAL_ERR_CODE_PTR_NULL;
    }
    while (uc_cmd_cnt > 0) {
        ...
        if (uc_set_id == MAC_VAP_11AX_DEBUG_MAC_PADDING) {
            if (value <= 2) { // 2: HE MAC padding duration合法值0/1/2
                g_ax_triggered_mac_padding_flag = OAL_TRUE;
                g_ax_triggered_mac_padding_val = value;
            } else {
                g_ax_triggered_mac_padding_flag = OAL_FALSE;
            }
        ...
        oam_warning_log2(pst_mac_vap->uc_vap_id, OAM_SF_ANY, "{hmac_config_ax_debug:set id[%d] set data[%d].}",
            uc_set_id, value);
        uc_cmd_index++;
        uc_cmd_cnt--;
    }
...
oal_bool_enum_uint8 mac_hiex_ie_get_bit_chip_type(uint8_t *frame_body, uint16_t us_frame_len)
{
    ...
    ie = mac_find_vendor_ie(MAC_WLAN_OUI_HUAWEI, MAC_HISI_HIEX_IE, &ies_info);
                                                      ...
                                                      uint16_t frame_len)
{
    ...
    /* HE CAP */
    ie = mac_find_ie_ext_ie(MAC_EID_HE, MAC_EID_EXT_HE_CAP, &ies_info);
    if ((ie != NULL) && (ie[1] >= MAC_HE_CAP_MIN_LEN)) {
        ...
        if ((he_cap_ie.st_he_phy_cap.bit_ldpc_coding_in_paylod == OAL_FALSE) &&
            ...
            (!(g_optimized_feature_switch_bitmap & BIT(CUSTOM_OPTIMIZE_FEATURE_CERTIFY_MODE)))) {
            bss_dscr->en_he_capable = OAL_FALSE;
            oam_warning_log0(0, OAM_SF_SCAN,
                "{hmac_scan_update_bss_list_11ax_he_cap::do not support ldpc, he_capable switch to false}");
        }
                                                         ...
                                                         mac_frame_he_oper_ie_stru *he_oper_ie_value)
{
    ...
    if (he_oper_ie_value->st_vht_operation_info.uc_channel_width == WLAN_MIB_VHT_OP_WIDTH_80_160) { /* 80MHz */
...
#ifdef _PRE_WLAN_FEATURE_160M
    if (he_oper_ie_value->st_vht_operation_info.uc_channel_width == WLAN_MIB_VHT_OP_WIDTH_160_DEPRECATED) { /* 160MHz */
...
OAL_STATIC void hmac_scan_update_bss_list_11ax(mac_bss_dscr_stru *bss_dscr, uint8_t *frame_body, uint16_t frame_len)
{
    ...
    /* HE Oper */
    ie = mac_find_ie_ext_ie(MAC_EID_HE, MAC_EID_EXT_HE_OPERATION, &ies_info);
                              ...
                              uint16_t frame_len)
{
    if (is_custom_open_11ax_switch(&hmac_vap->st_vap_base_info)) {
        hmac_scan_update_bss_list_11ax(bss_dscr, frame_body, frame_len);
    }
#ifdef _PRE_WLAN_FEATURE_HIEX
    if (g_wlan_spec_cfg->feature_hiex_is_open) {
        bss_dscr->en_punctured_preamble = mac_hiex_ie_get_bit_chip_type(frame_body, frame_len);
    }
#endif
}
/*
    ...
    uint16_t left_len, mac_ieee80211_frame_stru *frame_header)
{
    ...
    if (mbssid_ie_len == 0 || mbssid_ie_len > left_len || frame_header == NULL) {
        return 0;
    }
...
uint32_t hmac_scan_proc_multi_bssid_scanned_bss(hmac_vap_stru *hmac_vap, oal_netbuf_stru *netbuf, uint16_t buf_len)
{
    ...
    ies_info.frame_ies = ie_data;
    ies_info.frame_len = left_len;
    /* 查找multi-bssid ie */
    mbssid_ie = mac_find_ie(MAC_EID_MULTI_BSSID, &ies_info);
...
*****************************************************************************/
uint8_t mac_device_trans_bandwith_to_he_capinfo(mac_vap_stru *mac_vap, wlan_bw_cap_enum_uint8 en_max_op_bd)
{
    ...
    switch (en_max_op_bd) {
        ...
        case WLAN_BW_CAP_40M:
            uc_he_band_width_set |= (mac_vap->st_channel.en_band == WLAN_BAND_2G ? BIT0 : BIT1);
            return uc_he_band_width_set;
...
OAL_STATIC void mac_set_he_mac_capinfo_field(mac_vap_stru *mac_vap, uint8_t *puc_buffer)
{
    ...
    ***************************************************************************/
    ...
    if (g_ax_triggered_mac_padding_flag == OAL_TRUE) {
        pst_he_mac_capinfo->bit_trigger_mac_padding_duration = g_ax_triggered_mac_padding_val;
    }
    ...
    if (is_sta(mac_vap)) {
        ...
        if (g_wlan_spec_cfg->multi_tid_aggr_tx_support == OAL_TRUE) {
            /* B39-B42:Multi-TID Aggregation Tx Support 支持2 tid聚合发包 */
            pst_he_mac_capinfo->bit_mtid_aggregation_tx_support = 0;
        }
...
static void mac_set_he_phy_cap_4ltf_08gi_support(mac_frame_he_phy_cap_stru *he_phy_capinfo, uint8_t is_support)
{
    /* B58:HE SU PPDU And HE MU PPDU with 4x HE-LTF And 0.8usGI */
    he_phy_capinfo->bit_he_su_ppdu_he_mu_ppdu_4x_he_ltf_08us_gi = is_support;
    /* B64:HE ER SU PPDU with 4x HE-LTF And 0.8usGI */
    he_phy_capinfo->bit_he_er_su_ppdu_4x_he_ltf_08us_gi = is_support;
}
static void mac_set_he_phy_cap_4ltf_08gi(mac_vap_stru *mac_vap, mac_frame_he_phy_cap_stru *he_phy_capinfo)
{
    mac_set_he_phy_cap_4ltf_08gi_support(he_phy_capinfo, 1);
    ...
    if (((g_cust_cap.enhanced_phy_cap_mask & CUSTOM_HAL_PHY_CAP_RX_DELAY_OPT_MASK) != 0) &&
        ...
        mac_set_he_phy_cap_4ltf_08gi_support(he_phy_capinfo, 0);
    }
#endif
}
    ...
    mac_frame_he_phy_cap_stru *pst_he_phy_capinfo)
{
    ...
    /* B58:HE SU PPDU And HE MU PPDU with 4x HE-LTF And 0.8usGI */
    /* B64:HE ER SU PPDU with 4x HE-LTF And 0.8usGI */
    mac_set_he_phy_cap_4ltf_08gi(pst_mac_vap, pst_he_phy_capinfo);
    ...
    if (is_sta(pst_mac_vap)) {
        ...
        pst_he_phy_capinfo->bit_stbc_tx_over_80mhz = mac_device_get_cap_txstbc(mac_dev);
    }

    /* B63:STBC Rx>80MHz 确认HE160M模式rx stbc有收益,mp16及以后项目打开  */
    pst_he_phy_capinfo->bit_stbc_rx_over_80mhz = (g_wlan_spec_cfg->rx_stbc_160 == OAL_TRUE) ?
        mac_device_get_cap_rxstbc(mac_dev) : OAL_FALSE;
...
/*
 * 函 数 名  : mac_get_he_channel_width
 * 功能描述  : 获取he phy channel width字段中的带宽
 * 1.日    期  : 2019年11月14日
 *   作    者  : wifi
 *   修改内容  : 新生成函数
 */
uint8_t mac_get_he_channel_width(mac_vap_stru *mac_vap)
{
    /* 2g时，对5g的带宽不进行置位，只根据是否支持2.4g 40M对Bit0进行置位 */
    if (mac_vap->st_channel.en_band == WLAN_BAND_2G) {
        return mac_mib_get_2GFortyMHzOperationImplemented(mac_vap);
    }

    if (is_ap(mac_vap)) {
        return mac_device_trans_bandwith_to_he_capinfo(mac_vap,
            mac_vap_bw_mode_to_bw(mac_vap->st_channel.en_bandwidth));
    } else {
        return mac_device_trans_bandwith_to_he_capinfo(mac_vap, mac_mib_get_dot11VapMaxBandWidth(mac_vap));
    }
}
...
OAL_STATIC void mac_set_he_phy_capinfo_field(mac_vap_stru *mac_vap, uint8_t *puc_buffer)
{
    ...
    ************************************************************************************************/
    ...
    pst_he_phy_capinfo->bit_channel_width_set = mac_get_he_channel_width(mac_vap);
    ...
    /* B12:device class */
    pst_he_phy_capinfo->bit_device_class = (is_sta(mac_vap) ? PHY_DEVICE_CLASS_A : 0);
...
OAL_STATIC oal_bool_enum_uint8 mac_forbid_encap_he_ie(mac_vap_stru *mac_vap)
{
    if ((mac_mib_get_HEOptionImplemented(mac_vap) != OAL_TRUE) ||
        mac_vap_is_he_eht_protocol(mac_vap) == OAL_FALSE) {
        return OAL_TRUE;
    }
...
void mac_set_he_capabilities_ie(mac_vap_stru *mac_vap, uint8_t *buffer, uint8_t *ie_len)
{
    ...
    *ie_len = 0;
    ...
    /* PF认证要求对于WEP、TKIP 加密方式不能关联在HE模式 */
    if (mac_forbid_encap_he_ie(mac_vap) == OAL_TRUE) {
        return;
    }
    ...
    *ie_len += 1;
    ...
    mac_set_he_mac_capinfo_field(mac_vap, buffer);
    buffer += MAC_HE_MAC_CAP_LEN;
    *ie_len += MAC_HE_MAC_CAP_LEN;

    /* 填充HE PHY Capabilities Information 域信息 */
    pst_he_phy_capinfo = (mac_frame_he_phy_cap_stru *)buffer;
    mac_set_he_phy_capinfo_field(mac_vap, buffer);
    ...
    *ie_len += MAC_HE_PHY_CAP_LEN;
    ...
    uc_info_length = mac_set_he_tx_rx_mcs_nss_field(mac_vap, uc_channel_width_b2, uc_channel_width_b3, buffer);
    buffer += uc_info_length;
    *ie_len += uc_info_length;

    /* 填充 PPE Thresholds field */
    if (mac_mib_get_PPEThresholdsRequired(mac_vap)) {
        uc_info_length = mac_set_he_ppe_thresholds_field(buffer);
        buffer += uc_info_length;
        *ie_len += uc_info_length;
    }
    *puc_ie_length = *ie_len - MAC_IE_HDR_LEN;
}
 ...
 *   修改内容  : 新生成函数
 */
OAL_STATIC oal_bool_enum_uint8 hmac_sta_join_can_not_start_he(hmac_vap_stru *hmac_vap, mac_bss_dscr_stru *p_bss_dscr)
{
#ifdef _PRE_WLAN_CHBA_MGMT
    mac_device_stru *mac_dev = NULL;
#endif
    mac_vap_stru *p_mac_vap = &(hmac_vap->st_vap_base_info);
    ...
    if (p_bss_dscr->en_p2p_scenes ==  MAC_P2P_SCENES_LOW_LATENCY || mac_is_secondary_sta(hmac_vap) ||
 ...
 *   修改内容  : 新生成函数
 */
void hmac_sta_join_update_protocol_mib(hmac_vap_stru *hmac_vap, mac_bss_dscr_stru *bss_dscr)
{
    mac_vap_stru *mac_vap = &(hmac_vap->st_vap_base_info);

    mac_mib_set_HEOptionImplemented(mac_vap, OAL_FALSE);
    if ((bss_dscr->en_he_capable == OAL_TRUE) && (is_custom_open_11ax_switch(mac_vap))) {
        mac_mib_set_HEOptionImplemented(mac_vap, OAL_TRUE);
        if (hmac_sta_join_can_not_start_he(hmac_vap, bss_dscr) == OAL_TRUE) {
            bss_dscr->en_he_capable = OAL_FALSE;
            mac_mib_set_HEOptionImplemented(mac_vap, OAL_FALSE);
        }
    }
    ...
    hmac_vap_join_avoid_dbac_back_to_11n_handle(mac_vap, bss_dscr);
    oam_warning_log3(0, OAM_SF_SCAN, "hmac_sta_join_update_protocol_mib::bss_he_cap=%d,bss_vht_cap=%d \
        en_p2p_scenes=%d", bss_dscr->en_he_capable, bss_dscr->en_vht_capable,
        bss_dscr->en_p2p_scenes);
    oam_warning_log3(0, OAM_SF_SCAN, "hmac_sta_join_update_protocol_mib::he_mib=%d, \
        VAP_IS_SUPPORT_11AX=%d, MAC_VAP_GET_CAP_BW=%d",
        mac_mib_get_HEOptionImplemented(mac_vap),
        mac_vap_is_support_11ax(mac_vap), mac_vap->st_channel.en_bandwidth);
}
/*
 * 1.日    期  : 2017年5月27日
 *   作    者  : wifi
    ...
    uint8_t *puc_bss_color_ie)
{
    ...
    if (mac_ie_parse_he_bss_color_change_announcement_ie(puc_bss_color_ie, &st_bss_color_info) != OAL_SUCC) {
        return OAL_SUCC;
    }
...
hmac_vap_stru *hmac_find_omi_bw_switch_vap(void)
{
    uint8_t vap_idx;
    ...
    for (vap_idx = 0; vap_idx < mac_device->uc_vap_num; vap_idx++) {
        ...
        if (hmac_vap->omi_bw_switch_flag == OAL_TRUE) {
            return hmac_vap;
        }
    }
...
void hmac_omi_force_close_process(void)
{
    uint8_t auc_value[OMI_BW_SWITCH_CMD_LEN];
    hmac_vap_stru *hmac_vap = NULL;
    hmac_vap = hmac_find_omi_bw_switch_vap();
    if (hmac_vap == NULL) {
        return;
    }
    /* 配置参数进入退出流程 */
    auc_value[0] = OAL_FALSE;
    auc_value[1] = WLAN_BW_CAP_20M;
    hmac_config_set_omi_bw_switch(&hmac_vap->st_vap_base_info, OMI_BW_SWITCH_CMD_LEN * sizeof(uint8_t), auc_value);
}

uint32_t hmac_config_d2h_notify_omi_bw_switch(mac_vap_stru *mac_vap, uint8_t len, uint8_t *param)
{
    hmac_vap_stru *hmac_vap = NULL;
    hmac_vap = mac_res_get_hmac_vap(mac_vap->uc_vap_id);
    if (hmac_vap == NULL) {
        oam_error_log0(mac_vap->uc_vap_id, OAM_SF_CFG, "{hmac_config_d2h_notify_omi_bw_switch::hmac_vap null}");
        return OAL_ERR_CODE_PTR_NULL;
    }

    hmac_vap->omi_bw_switch_flag = (oal_bool_enum_uint8)*param;
    oam_warning_log1(mac_vap->uc_vap_id, OAM_SF_CFG,
                     "hmac_config_d2h_notify_omi_bw_switch::omi_bw_switch_flag[%d]", hmac_vap->omi_bw_switch_flag);
