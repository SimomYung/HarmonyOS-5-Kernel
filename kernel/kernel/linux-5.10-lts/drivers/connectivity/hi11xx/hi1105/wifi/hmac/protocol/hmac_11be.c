...
OAL_STATIC void hmac_update_eht_bss_channel_bandwidth(mac_bss_dscr_stru *bss_dscr,
    const eht_operation_information_stru *eht_oper_info)
{
    uint8_t channel = bss_dscr->st_channel.uc_chan_number;
    uint8_t channel_width = eht_oper_info->channel_width;
    uint8_t ccfs0 = eht_oper_info->ccfs0;
    uint8_t ccfs1 = eht_oper_info->ccfs1;

    if (channel_width == WLAN_EHT_CHANNEL_WIDTH_320) {
        /* 当前设备不支持320MHz带宽，不解析EHT IE指示的带宽，使用HE VHT HT已解析的带宽 */
#ifdef _PRE_WLAN_FEATURE_160M
    } else if (channel_width == WLAN_EHT_CHANNEL_WIDTH_160) {
        if (ccfs1 - ccfs0 == CHAN_OFFSET_PLUS_8 ||
            ccfs0 - ccfs1 == CHAN_OFFSET_PLUS_8) {
            bss_dscr->en_channel_bandwidth = mac_get_160mbandwith_from_offset_channel((int8_t)(ccfs0 - channel));
        } else {
            bss_dscr->en_channel_bandwidth = mac_get_160mbandwith_from_offset_channel((int8_t)(ccfs1 - channel));
        }
#endif
    } else if (channel_width == WLAN_EHT_CHANNEL_WIDTH_80) {
        bss_dscr->en_channel_bandwidth = mac_get_80mbandwith_from_offset_channel((int8_t)(ccfs0 - channel));
    } else {  /* 20/40MHz */
        /*********************************************************************************
           802.11be协议 35.15.1 阐述了EHT IE携带和不携带带宽能力字段时的带宽确定方法
           20M 和 40M带宽在HT VHT HE EHT协议下的枚举值相同，均为0 1
           当EHT协议下，带宽为20M或40M时，HT VHT HE EHT带宽解析的结果相同，故此处不重复解析
        **********************************************************************************/
    }
}
    ...
    mac_bss_dscr_stru *bss_dscr, uint8_t *frame_body, uint16_t frame_len)
{
    ...
    if ((ie != NULL) && (ie[1] >= MAC_EHT_OPER_MIN_LEN)) {
        ...
        if (eht_oper->eht_oper_parameters.eht_operation_information_present) {
            ...
            /* 更新带宽信息 */
            hmac_update_eht_bss_channel_bandwidth(bss_dscr, &(eht_oper->eht_oper_info));
        }
    }
}
    ...
    hmac_user_stru *hmac_user, mac_ies_info *ies_info)
{
    ...
    he_cap_ie = mac_find_ie_ext_ie(MAC_EID_EHT, MAC_EID_EXT_HE_CAP, ies_info);
#endif
    ...
    if (eht_cap_ie != NULL) {
        ...
        mac_process_eht_capabilities_ie(mac_vap, &hmac_user->st_user_base_info, eht_cap_ie, he_cap_ie);
    }
