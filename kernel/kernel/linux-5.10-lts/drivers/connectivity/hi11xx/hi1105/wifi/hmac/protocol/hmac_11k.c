                                   ...
                                   uint16_t frame_len)
{
    ...
    ie = mac_find_ie(MAC_EID_RRM, &ies_info);
...
{
    uint32_t ret;
    mac_tx_ctl_stru *tx_ctl = (mac_tx_ctl_stru *)oal_netbuf_cb(action_table_bcn_rpt);
    if (tx_ctl == NULL) {
        oam_error_log0(mac_vap->uc_vap_id, OAM_SF_ANY, "{hmac_scan_set_tx_cb_and_post_event::tx_ctl null");
        return OAL_ERR_CODE_PTR_NULL;
    }
