...
void hmac_11v_roam_start_worker(oal_work_stru *hmac_11v_roam_work)
{
    ...
    if (user_11v_ctrl_info->uc_11v_roam_scan_times < MAC_11V_ROAM_SCAN_ONE_CHANNEL_LIMIT) {
        ...
        hmac_roam_start_fill_info(&roam_start_params, ROAM_SCAN_CHANNEL_ORG_5, OAL_TRUE, NULL, ROAM_TRIGGER_11V);
        ...
        hmac_roam_start_fill_info(&roam_start_params, ROAM_SCAN_CHANNEL_ORG_DBDC, OAL_TRUE, NULL, ROAM_TRIGGER_11V);
        hmac_roam_start(hmac_vap, &roam_start_params);
    }
...
OAL_STATIC oal_bool_enum_uint8 hmac_roam_check_all_bsst_req_bssid_in_blacklist(hmac_vap_stru *hmac_vap,
    hmac_roam_info_stru *roam_info)
{
    uint8_t idx;
    oal_bool_enum_uint8 ret;
    uint8_t *neighbor_bssid = NULL;

    for (idx = 0; idx < roam_info->uc_neighbor_rpt_bssid_num; idx++) {
        neighbor_bssid = &(roam_info->auc_neighbor_rpt_bssid[idx][0]);

        // 检查BTMR帧中的bssid是否在roam blacklist中
        ret = hmac_roam_alg_find_in_blacklist(roam_info, neighbor_bssid);
        if (ret == OAL_TRUE) {
            continue;
        }

        // 检查BTMR帧中的bssid是否在blacklist中
        ret = hmac_blacklist_filter(&hmac_vap->st_vap_base_info, neighbor_bssid, WLAN_MAC_ADDR_LEN);
        if (ret != OAL_TRUE) {
            return OAL_FALSE;
        }
    }
    ...
    hmac_bsst_req_info_stru *bsst_req_info)
{
    ...
    /* 检查BTMR帧中所有的bssid是否都在blacklist中 */
    if (hmac_roam_check_all_bsst_req_bssid_in_blacklist(hmac_vap, roam_info) == OAL_TRUE) {
        need_roam = OAL_FALSE;
    }
    ...
    mac_scan_req_stru *scan_params, hmac_neighbor_bss_info_stru *neighbor_bss_info, uint8_t bss_num)
{
    ...
    for (bss_list_index = 0; bss_list_index < bss_num; bss_list_index++) {
        ...
        if (mac_is_channel_num_valid(band, channum, OAL_FALSE) == OAL_SUCC) {
            hmac_roam_neighbor_report_add_chan(scan_params, channum, band);
        }
...
#ifdef _PRE_WLAN_FEATURE_ML
OAL_STATIC hmac_neighbor_bss_info_stru *mld_bsst_links_find(hmac_user_stru *hmac_user,
    hmac_bsst_req_info_stru *bsst_req_info)
{
    hmac_neighbor_bss_info_stru *neighbor_bss = NULL;
    uint32_t bss_list_index;

    if (bsst_req_info->pst_neighbor_bss_list == NULL) {
        return NULL;
    }

    for (bss_list_index = 0; bss_list_index < bsst_req_info->uc_bss_list_num; bss_list_index++) {
        neighbor_bss = &(bsst_req_info->pst_neighbor_bss_list[bss_list_index]);
        if (neighbor_bss->valid == OAL_FALSE) {
            continue;
        }
        if (neighbor_bss->uc_max_candidate_perf != neighbor_bss->uc_candidate_perf) {
    ...
    if (bss_list_index >= bsst_req_info->uc_bss_list_num) {
        /* 无候选设备列表 */
        return NULL;
    }

    oam_warning_log4(0, OAM_SF_ROAM,
        "{mld_bsst_links_find::find a candidate bss,index=%d,bss_num=%d,link_num:%d,link_id:%d}",
        bss_list_index, bsst_req_info->uc_bss_list_num,
        neighbor_bss->ap_mld_info.ml_link_num, neighbor_bss->ap_mld_info.link_id);

    if (neighbor_bss->ap_mld_info.ml_link_num == 0) {
        /* 候选AP 非AP MLD设备 */
        return NULL;
    }
    if (oal_memcmp(neighbor_bss->ap_mld_info.ml_mac_addr, hmac_user->mld_user_link_info.mld_user_manage->mld_addr,
        WLAN_MAC_ADDR_LEN) != 0) {
        /* 非AP MLD link间漫游 */
        return NULL;
    }
...
OAL_STATIC void mld_bsst_tid_to_link_init(hmac_vap_stru *hmac_vap, hmac_user_stru *hmac_user,
    uint8_t link_id, mac_cfg_ttlm_frame_param *ttlm_req_param)
{
    uint8_t tid;
    oal_set_mac_addr(ttlm_req_param->user_addr, hmac_user->st_user_base_info.auc_user_mac_addr);
    ttlm_req_param->cfg_ttlm_ie.ttlm_ctrl.direction = WLAN_DOWNUPLINK;
    ttlm_req_param->cfg_ttlm_ie.ttlm_ctrl.default_link_mapping = OAL_FALSE;
    ttlm_req_param->cfg_ttlm_ie.ttlm_ctrl.link_mapping_size = 1; /* 1 字节 */
    ttlm_req_param->cfg_ttlm_ie.ttlm_ctrl.link_mapping_presence_bitmap = 0xff;
    ttlm_req_param->dialog_token = 1; /* token 1 */
    for (tid = 0; tid < WLAN_TIDNO_BUTT; tid++) {
        ttlm_req_param->cfg_ttlm_ie.link_bitmap[tid] = (1 << link_id);
    }
}

/* 函数功能：mld场景下，解析btm后 发起ttlm协商流程 */
OAL_STATIC uint32_t hmac_btm_mld_trigger_ttlm_proc(hmac_vap_stru *hmac_vap, hmac_user_stru *hmac_user, uint8_t link_id)
{
    mac_cfg_ttlm_frame_param ttlm_req_param;

    memset_s(&ttlm_req_param, sizeof(ttlm_req_param), 0, sizeof(ttlm_req_param));
    mld_bsst_tid_to_link_init(hmac_vap, hmac_user, link_id, &ttlm_req_param);

    /* 调用TID-TO-LINK模块发送 tid-to-link request报文 */
    return hmac_mld_ttlm_request_process(&hmac_vap->st_vap_base_info, hmac_user, &ttlm_req_param);
}

/* 函数功能：mld场景下判断是否btm req帧是否指示link间漫游（切换工作link） */
OAL_STATIC oal_bool_enum_uint8 hmac_btm_mld_handle_process(hmac_vap_stru *hmac_vap,
    hmac_user_stru *hmac_user, hmac_bsst_req_info_stru *bsst_req_info)
{
    ...
    if (mac_vap == NULL || mac_vap->pst_mib_info == NULL) {
        oam_error_log0(0, OAM_SF_ANY, "{hmac_btm_mld_handle_process::mac_vap is null.}");
        return OAL_FALSE;
    }

    if (mac_mib_get_TIDtoLinkMappingActivated(mac_vap->pst_mib_info) == OAL_FALSE) {
        oam_warning_log0(mac_vap->uc_vap_id, OAM_SF_ANY,
            "{hmac_btm_mld_handle_process::tid_to_link_mapping mib false.}");
        return OAL_FALSE;
    }

    if (hmac_user_is_ml_mode(hmac_user) == OAL_FALSE) {
        oam_warning_log1(mac_vap->uc_vap_id, OAM_SF_ANY, "{hmac_btm_mld_handle_process::mld mode:%d.}",
                         hmac_user->mld_user_link_info.mld_mode);
        return OAL_FALSE;
    }

    if (bsst_req_info->st_request_mode.bit_link_removal_imminent == OAL_TRUE &&
        bsst_req_info->st_request_mode.bit_termination_include == OAL_TRUE) {
        /* 删除link操作,不做处理,使用multi link reconfig协商删除link流程,返回true不执行传统漫游 */
        oam_warning_log0(mac_vap->uc_vap_id, OAM_SF_ANY, "{hmac_btm_mld_handle_process::need remove link.}");
        return OAL_TRUE;
    }

    neighbor_bss = mld_bsst_links_find(hmac_user, bsst_req_info);
    if (neighbor_bss == NULL) {
        return OAL_FALSE;
    }

    link_id = neighbor_bss->ap_mld_info.link_id;
    link_hmac_vap = hmac_mld_get_sta_vap_by_ap_link_id(hmac_vap, link_id);
    if (link_hmac_vap == NULL) {
        oam_warning_log1(mac_vap->uc_vap_id, OAM_SF_ANY,
            "{hmac_btm_mld_handle_process::not find vap match link_id[%d]}", link_id);
        return OAL_FALSE;
    }

    oam_warning_log2(mac_vap->uc_vap_id, OAM_SF_ANY, "{hmac_btm_mld_handle_process::link id:%u,asoc_statue:%u.}",
        link_id, link_hmac_vap->mld_vap_link_info.link_ap_info.asoc_succ);
    if (link_hmac_vap->mld_vap_link_info.link_ap_info.asoc_succ == 0) {
        return OAL_FALSE;
    }

    /* 满足BTM request指示的Link间漫游条件，触发ttlm协商交互 */
    if (hmac_btm_mld_trigger_ttlm_proc(hmac_vap, hmac_user, link_id) != OAL_SUCC) {
        oam_warning_log0(mac_vap->uc_vap_id, OAM_SF_ANY,
            "{hmac_btm_mld_handle_process::send ttlm request fail}");
    }

    return OAL_TRUE;
}

OAL_STATIC uint32_t hmac_parse_neighbor_rpt_sub_basic_ml_ie(uint8_t *ie_data, int16_t sub_ie_len,
    ml_info_stru *ap_mld_info)
{
    mac_eht_ml_ie_header_stru ml_ie_stru = { 0 };
    mac_ml_control_union  *ml_control = NULL;
    uint8_t common_info_len = 0;
    uint8_t *common_info = NULL;

    ml_control = &(ml_ie_stru.ml_control);
    memcpy_s(ml_control, sizeof(mac_ml_control_union), ie_data + MAC_IE_HDR_LEN, sizeof(mac_ml_control_union));
    if (ml_control->basic_ml_control.ml_control_type != BASIC_MULTI_LINK) {
        oam_warning_log1(0, OAM_SF_ANY, "{hmac_parse_neighbor_rpt_sub_basic_ml_ie::invalid ml_control_type:%u.}",
            ml_control->basic_ml_control.ml_control_type);
        return OAL_FAIL;
    }

    /* 解析ML control字段，计算common info长度 */
    common_info_len = mac_count_basic_ml_common_info_length(&ml_ie_stru);
    if (MAC_IE_HDR_LEN + ML_CONTROL_FIELD_LEN + common_info_len > sub_ie_len) {
        oam_warning_log1(0, OAM_SF_ANY, "{hmac_parse_neighbor_rpt_sub_basic_ml_ie::common_info_len[%u]}",
            common_info_len);
        return OAL_FAIL;
    }
    common_info = ie_data + (MAC_IE_HDR_LEN + ML_CONTROL_FIELD_LEN);
    ml_ie_stru.ie_len = 1 + ML_CONTROL_FIELD_LEN + common_info_len;
    /* 解析 Common Info */
    if (hmac_sta_parse_basic_ml_common_info(common_info, &ml_ie_stru, ap_mld_info, common_info_len) != OAL_SUCC) {
        oam_error_log0(0, OAM_SF_ANY, "{hmac_parse_sub_basic_multilink_element::parse ml common info failed.}");
        return OAL_FAIL;
    }
...
OAL_STATIC void hmac_bsst_req_info_init(hmac_vap_stru *hmac_vap, mac_user_stru *mac_user,
    hmac_bsst_req_info_stru *bsst_req_info, uint8_t *data, uint16_t frame_len)
{
    uint16_t handle_len;
...
uint32_t hmac_rx_bsst_req_action(hmac_vap_stru *hmac_vap, hmac_user_stru *hmac_user, oal_netbuf_stru *netbuf)
{
    ...
    rx_info = (mac_rx_ctl_stru *)oal_netbuf_cb(netbuf);
    /* 获取帧体指针 */
    ...
    hmac_bsst_req_info_init(hmac_vap, mac_user, &bsst_req_info, data, frame_len);

#ifdef _PRE_WLAN_FEATURE_ML
    /* 判断是否mld场景下的btm处理，执行link切换 */
    ret = hmac_btm_mld_handle_process(hmac_vap, hmac_user, &bsst_req_info);
    if (ret == OAL_TRUE) {
        /* 释放动态申请的内存 */
        hmac_rx_bsst_free_res(&bsst_req_info);
        oam_warning_log0(0, OAM_SF_ANY, "{hmac_rx_bsst_req_action::links internal roam.}");
        return OAL_SUCC;
    }
#endif
...
#ifdef _PRE_WLAN_FEATURE_ML
            case HMAC_NEIGH_SUB_ID_BASIC_MULTI_LINK:
                if (sub_ie_len < 11) { // 子ie至少包含11个字节
                    oam_error_log1(0, OAM_SF_ANY,
                        "hmac_parse_neighbor_report_subelement::invalid sub basic multi_link len[%d]", sub_ie_len);
                    return;
                }
                hmac_parse_neighbor_rpt_sub_basic_ml_ie(ie_data, sub_ie_len,
                    &bss_list_alloc[bss_list_index].ap_mld_info);
    ...
    uint8_t bss_list_index, uint8_t *ie_data)
{
    ...
    bss_list_alloc[bss_list_index].st_bssid_info.bit_very_high_throughput =
        (ie_data[BYTE_OFFSET_9] & BIT4) ? OAL_TRUE : OAL_FALSE; /* bit4 */
    bss_list_alloc[bss_list_index].st_bssid_info.bit_ftm =
        (ie_data[BYTE_OFFSET_9] & BIT5) ? OAL_TRUE : OAL_FALSE; /* bit5 */
    bss_list_alloc[bss_list_index].st_bssid_info.bit_high_efficiency =
        (ie_data[BYTE_OFFSET_9] & BIT6) ? OAL_TRUE : OAL_FALSE; /* bit6 */
    bss_list_alloc[bss_list_index].st_bssid_info.bit_er_bss =
        (ie_data[BYTE_OFFSET_9] & BIT7) ? OAL_TRUE : OAL_FALSE; /* bit7 */

    bss_list_alloc[bss_list_index].st_bssid_info.bit_colocated_ap =
        (ie_data[BYTE_OFFSET_10] & BIT0) ? OAL_TRUE : OAL_FALSE; /* bit0 */
    bss_list_alloc[bss_list_index].st_bssid_info.bit_probe_rsp_active =
        (ie_data[BYTE_OFFSET_10] & BIT1) ? OAL_TRUE : OAL_FALSE; /* bit1 */
    bss_list_alloc[bss_list_index].st_bssid_info.bit_members_colocated_ap =
        (ie_data[BYTE_OFFSET_10] & BIT2) ? OAL_TRUE : OAL_FALSE; /* bit2 */
    bss_list_alloc[bss_list_index].st_bssid_info.bit_oct_supported =
        (ie_data[BYTE_OFFSET_10] & BIT3) ? OAL_TRUE : OAL_FALSE; /* bit3 */
    bss_list_alloc[bss_list_index].st_bssid_info.bit_colocated_with_6g =
        (ie_data[BYTE_OFFSET_10] & BIT4) ? OAL_TRUE : OAL_FALSE; /* bit4 */
    bss_list_alloc[bss_list_index].st_bssid_info.bit_extremely_high_throughput =
        (ie_data[BYTE_OFFSET_10] & BIT5) ? OAL_TRUE : OAL_FALSE; /* bit5 */
...
static uint8_t hmac_get_neighbor_ie_number(uint8_t *ie_data_find, uint16_t len)
{
    ...
    while (ie_data_find != NULL) {
        ies_info.frame_ies = ie_data_find;
        ies_info.frame_len = len;

        ie_data = mac_find_ie(MAC_EID_NEIGHBOR_REPORT, &ies_info);
        /* 没找到则退出循环 */
        if (ie_data == NULL) {
            break;
        }
