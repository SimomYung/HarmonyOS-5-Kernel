...
/* 功能描述: 更新nol timer状态 */
OAL_STATIC oal_bool_enum_uint8 hmac_dfs_nol_timer_is_waitting(mac_device_stru *mac_device,
    const mac_channel_list_stru *chan_list_info, uint8_t chan_num)
{
    oal_dlist_head_stru *nol_list_head = NULL;
    mac_dfs_nol_node_stru *nol_node = NULL;
    oal_dlist_head_stru *nol_list_pos = NULL;
    ...
    oal_dlist_search_for_each_safe(nol_list_pos, nol_list_temp, nol_list_head)
    {
        ...
        if (run_time >= nol_node->dfs_nol_wait_time) { /* 已超过定时时长, 停止frw_timer */
            oam_warning_log4(0, OAM_SF_ANY,
                "{hmac_dfs_nol_timer_is_waitting:chan_idx[%d] timeout! [0x%xms -> 0x%xms], run %dms}",
...
oal_bool_enum_uint8 hmac_check_channel_block_status(mac_vap_stru *mac_vap, mac_channel_stru *channel_param)
{
    ...
    for (chan_num = 0; chan_num < chan_list_info.channels; chan_num++) {
        ...
        if ((chan_status == MAC_CHAN_BLOCK_DUE_TO_RADAR) &&
            (hmac_dfs_nol_timer_is_waitting(mac_device, &chan_list_info, chan_num) == OAL_TRUE)) {
            oam_warning_log1(mac_vap->uc_vap_id, OAM_SF_CHAN,
