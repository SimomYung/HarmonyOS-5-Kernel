...
{
    return ((freq >= 5600) && (freq <= 5650)); // 5600 - 5650：天气雷达信道
}

OAL_STATIC uint32_t hmac_dfs_nol_addchan(mac_device_stru *mac_device, mac_vap_stru *mac_vap, uint8_t chan_idx);
...
void hmac_dfs_init(mac_device_stru *mac_device)
{
    ...
    OAL_MUTEX_INIT(&(mac_device->st_dfs.dfs_nol_lock));
}
...
void hmac_dfs_channel_list_init(mac_device_stru *mac_device)
{
    ...
    for (idx = 0; idx < MAC_MAX_SUPP_CHANNEL; idx++) {
        ...
        /* radar block状态保留 */
        if (mac_device->st_ap_channel_list[idx].en_ch_status  == MAC_CHAN_BLOCK_DUE_TO_RADAR) {
            continue;
        }
    ...
    hmac_cac_event_stru *cac_event)
{
    ...
    /* cac事件 */
    if (memcpy_s(frw_get_event_payload(event_mem), sizeof(hmac_cac_event_stru),
        (const void *)cac_event, sizeof(hmac_cac_event_stru)) != EOK) {
...
uint32_t hmac_dfs_radar_detect_event(frw_event_mem_stru *event_mem)
{
    ...
    if (mac_device->st_dfs.st_dfs_info.dfsmode != DFS_NO_DEBUG_MODE) {
        oam_error_log0(hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_DFS, "Real radar det,return before switch channel");
        return OAL_FAIL;
    }
 ...
 *   修改内容  : 新生成函数
 */
OAL_STATIC uint32_t hmac_dfs_update_available_channel_list(mac_device_stru *mac_device, mac_vap_stru *mac_vap,
    uint8_t chan_idx, oal_bool_enum_uint8 radar_detected)
{
    uint32_t ret;

    ret = mac_is_channel_idx_valid(mac_vap->st_channel.en_band, chan_idx, OAL_FALSE);
    if (ret != OAL_SUCC) {
    ...
    if (mac_device->st_ap_channel_list[chan_idx].en_ch_status == MAC_CHAN_AVAILABLE_ALWAYS) {
        oam_warning_log1(0, OAM_SF_DFS,
            "{hmac_dfs_update_available_channel_list::Radar detected in Non-Radar Chan idx(%d)!}", chan_idx);
        return OAL_ERR_CODE_INVALID_CONFIG;
    }
...
static uint32_t hmac_dfs_start_all_bss_of_device(hmac_device_stru *hmac_dev)
{
    ...
    if (hmac_dev == NULL || hmac_dev->pst_device_base_info == NULL) {
        oam_error_log0(0, OAM_SF_ANY, "{hmac_dfs_start_all_bss_of_device::hmac_dev or device_base_info null!}");
        return OAL_ERR_CODE_PTR_NULL;
    }
    oam_warning_log1(0, OAM_SF_ACS, "{hmac_dfs_start_all_bss_of_device:device id=%d}",
                     hmac_dev->pst_device_base_info->uc_device_id);

    dev = hmac_dev->pst_device_base_info;
...
uint32_t hmac_dfs_cac_timeout_fn(void *p_arg)
{
    ...
    for (idx = 0; idx < st_chan_info.channels; idx++) {
        ret = hmac_dfs_update_available_channel_list(mac_device, &(hmac_vap->st_vap_base_info),
...
OAL_STATIC void hmac_dfs_cac_start_save_dfs_result(hmac_vap_stru *hmac_vap)
{
    ...
    if (mac_vap->st_channel.uc_chan_idx >= MAC_CHANNEL_FREQ_5_BUTT) {
        return;
    }
    ...
    if (memcpy_s(g_st_dfs_result.auc_name, OAL_IF_NAME_SIZE, \
        hmac_vap->auc_name, OAL_IF_NAME_SIZE) != EOK) {
        oam_error_log0(0, OAM_SF_CFG, "hmac_dfs_cac_start_save_dfs_result::memcpy fail!");
    }
}

/*
 * 函 数 名  : hmac_dfs_ap_cac_check
 * 功能描述  : 检查GO是否需要进行CAC
 * 1.日    期  : 2020年2月8日
 *   作    者  : wifi
 *   修改内容  : 新生成函数
 */
uint32_t hmac_dfs_ap_cac_check(mac_vap_stru *mac_vap)
{
    ...
    if (hmac_vap == NULL) {
        oam_warning_log0(mac_vap->uc_vap_id, OAM_SF_M2S, "{hmac_dfs_ap_cac_check::pst_hmac_vap null.}");
        return OAL_FAIL;
    }

    hmac_dfs_status_set(HMAC_INS_START);
    hmac_dfs_cac_start_save_dfs_result(hmac_vap);
    /* 新增APUT可以跳过CAC检测，包含P2P_GO/APUT/CHBA，复用原来GO模式的指令和变量 */
    if (is_ap(mac_vap) == OAL_TRUE) {
        if (g_ap_cac == OAL_FALSE) {
            g_ap_cac = OAL_TRUE;
            oam_warning_log0(mac_vap->uc_vap_id, OAM_SF_M2S, "{hmac_dfs_ap_cac_check::skip cur cac.}");
            return OAL_FAIL;
        }
        ...
        for (idx = 0; idx < up_vap_num; idx++) {
            if (up_vap_array[idx] != NULL && up_vap_array[idx]->uc_vap_id != mac_vap->uc_vap_id &&
                ...
                oam_warning_log0(mac_vap->uc_vap_id, OAM_SF_DFS,
                    "{hmac_dfs_ap_cac_check::one vap already on radar channel, ap not need cac!}");
                return OAL_FAIL;
            }
        }
    }
...
void hmac_dfs_radar_wait(mac_device_stru *mac_device, hmac_vap_stru *hmac_vap)
{
    ...
    /* 如果雷达使能没有开启直接返回 */
    if (mac_dfs_get_dfs_enable(&(hmac_vap->st_vap_base_info)) == OAL_FALSE) {
        return;
    }
...
OAL_STATIC uint32_t hmac_dfs_nol_timeout_fn(void *p_arg)
{
    ...
    oal_mutex_lock(&mac_device->st_dfs.dfs_nol_lock);
    ret = hmac_dfs_nol_delchan(mac_device, nol_node);
    oal_mutex_unlock(&mac_device->st_dfs.dfs_nol_lock);
    return ret;
}

OAL_STATIC uint32_t hmac_dfs_get_cur_boottime_ms(void)
{
    uint64_t cur_boottime_ms = (uint64_t)ktime_to_ms(ktime_get_boottime());
    return (uint32_t)cur_boottime_ms;
}
/*
...
OAL_STATIC uint32_t hmac_dfs_nol_addchan(mac_device_stru *mac_device, mac_vap_stru *mac_vap, uint8_t chan_idx)
{
    ...
    if (mac_get_channel_num_from_idx(mac_vap->st_channel.en_band, chan_idx, OAL_FALSE, &chan_num) != OAL_SUCC) {
        oam_error_log2(mac_vap->uc_vap_id, OAM_SF_DFS, "hmac_dfs_nol_addchan:band[%d]chan_idx[%d]",
            mac_vap->st_channel.en_band, chan_idx);
        return OAL_FAIL;
    }
    /* 更新可用信道列列表 */
    ret = hmac_dfs_update_available_channel_list(mac_device, mac_vap, chan_idx, OAL_TRUE);
    if (ret == OAL_SUCC) {
        ...
        if (oal_unlikely(nol_node == NULL)) {
            oam_error_log0(mac_vap->uc_vap_id, OAM_SF_DFS, "{hmac_dfs_nol_addchan::memory not enough.}");
        ...
        nol_node->dfs_nol_start_time_ms = hmac_dfs_get_cur_boottime_ms();
        nol_node->dfs_nol_wait_time = mac_device->st_dfs.st_dfs_info.dfs_non_occupancy_period_time_ms;
        ...
        oam_warning_log4(mac_vap->uc_vap_id, OAM_SF_DFS,
            "{[DFS]hmac_dfs_nol_addchan, add channel %d to NOL, cur stamp_time 0x%xms/boot_time 0x%xms, wait %dms}",
            chan_num, (uint32_t)oal_time_get_stamp_ms(), nol_node->dfs_nol_start_time_ms,
            mac_device->st_dfs.st_dfs_info.dfs_non_occupancy_period_time_ms);
        oal_mutex_lock(&mac_device->st_dfs.dfs_nol_lock);
 ...
 *   修改内容  : 新生成函数
 */
uint32_t hmac_dfs_nol_clear(oal_dlist_head_stru *dlist_head)
{
    mac_device_stru *mac_device = NULL;
    mac_dfs_nol_node_stru *nol_node = NULL;
    ...
    mac_vap_stru *mac_vap)
{
    ...
    for (idx = 0; idx < st_chan_info.channels; idx++) {
        // 只要有一条信道标记成功，就需要切换信道
        if (hmac_dfs_nol_addchan(mac_device, mac_vap, st_chan_info.ast_channels[idx].uc_idx) == OAL_SUCC) {
            cnt++;
        }
    }
...
OAL_STATIC void hmac_get_available_chan_80M(uint32_t chan_bitmap, uint8_t *available_chan_idx,
    uint8_t *available_chan_cnt)
{
    const uint32_t    window = 0xF;     /* 1111b */
    const uint8_t     step = 4;      /* 4代表步长 */
    uint8_t           idx;

    chan_bitmap &= HMAC_80MHZ_BITMAP_MASK;

    /* channel 36 ~ 165 */
    for (idx = 0; idx < MAC_CHANNEL165; idx += step) {   /* 4条信道组不了80MHz, 参考802.11 ac Annex E */
        if ((window << idx) == (chan_bitmap & (window << idx))) {
...
OAL_STATIC uint8_t hmac_dfs_available_chan_idx_set(wlan_channel_bandwidth_enum_uint8 try_bandwidth,
    uint8_t idx_temp, uint8_t *available_chan_idx, uint32_t chan_bitmap, uint8_t num_supp_chan)
{
    uint32_t window;
    uint8_t step;
    uint8_t available_chan_cnt = 0;
    uint8_t idx = idx_temp;
    /* 20MHz */
...
#ifdef _PRE_WLAN_FEATURE_MULTI_AP
        hmac_get_available_chan_80M(chan_bitmap, available_chan_idx, &available_chan_cnt);
#else
...
OAL_STATIC uint8_t hmac_dfs_get_available_channel_list(mac_vap_stru *mac_vap, mac_device_stru *mac_device,
    uint8_t *available_chan_idx, wlan_channel_bandwidth_enum_uint8 try_bandwidth, uint8_t *ret_idx)
{
    uint8_t idx;
    uint8_t available_chan_cnt;
    uint32_t chan_bitmap = 0;
    ...
    for (idx = 0; idx < num_supp_chan; idx++) {
#ifdef _PRE_WLAN_FEATURE_MULTI_AP
        if (idx == MAC_CHANNEL120 || idx == MAC_CHANNEL124 || idx == MAC_CHANNEL128) {
            oam_warning_log1(mac_vap->uc_vap_id, OAM_SF_DFS,
                "{hmac_dfs_get_available_channel_list::skip weather radar channel idx=%u!}", idx);
            continue;
        }
#endif
        hmac_dfs_set_chan_bitmap(mac_device, idx, &chan_bitmap);
    ...
    oam_warning_log2(mac_vap->uc_vap_id, OAM_SF_DFS,
        "{hmac_dfs_get_available_channel_list::custom_chanlist_bitmap=0x%x,chan_bitmap=0x%x!}",
...
OAL_STATIC uint32_t hmac_dfs_select_random_below_160m_channel(mac_vap_stru *mac_vap,
    wlan_channel_bandwidth_enum_uint8 try_bandwidth_temp, uint8_t *new_channel,
    wlan_channel_bandwidth_enum_uint8 *new_bandwidth)
{
    mac_device_stru *mac_device = mac_res_get_dev(mac_vap->uc_device_id);
    uint8_t available_chan_idx[MAC_CHANNEL_FREQ_5_BUTT];
    uint8_t available_chan_cnt, idx, idx_tmp;
    wlan_channel_bandwidth_enum_uint8 try_bandwidth = try_bandwidth_temp;

    if (mac_device == NULL) {
        oam_error_log0(mac_vap->uc_vap_id, OAM_SF_DFS, "{hmac_dfs_select_random_below_160m_channel::mac_device null.}");
        return OAL_FAIL;
    }
    available_chan_cnt = hmac_dfs_get_available_channel_list(mac_vap, mac_device,
        available_chan_idx, try_bandwidth, &idx);
...
#if defined(_PRE_WLAN_FEATURE_160M) && defined(_PRE_WLAN_FEATURE_MULTI_AP)
    if (try_bandwidth >= WLAN_BAND_WIDTH_160PLUSPLUSPLUS && try_bandwidth <= WLAN_BAND_WIDTH_160MINUSMINUSMINUS) {
        /* 160MHz上检测到有问题,会选到气象雷达,带宽降为80M */
...
#ifdef _PRE_WLAN_FEATURE_ATP_REPORT
    hmac_dfs_switch_channel_report(mac_vap, new_channel);
#endif
...
uint32_t hmac_dfs_ap_wait_start_radar_handler(hmac_vap_stru *hmac_vap)
{
...
#ifdef _PRE_WLAN_CHBA_MGMT
    hmac_chba_dfs_detect_radar_report(mac_vap);
#endif
    ...
    /* 判断是否需要进行 */
    if (hmac_dfs_need_for_cac(mac_device, &hmac_vap->st_vap_base_info) == OAL_TRUE) {
...
OAL_STATIC uint8_t hmac_dfs_switch_channel_for_radar_get_new_bandwidth(mac_device_stru *mac_device,
    mac_vap_stru *mac_vap)
{
    if (mac_device->st_dfs.st_dfs_info.en_next_ch_width_type != WLAN_BAND_WIDTH_BUTT) {
        return mac_device->st_dfs.st_dfs_info.en_next_ch_width_type;
    }

    oam_warning_log0(mac_vap->uc_vap_id, OAM_SF_DFS,
        "{hmac_dfs_switch_channel_for_radar::user not set next ch bw.}");
    return mac_vap->st_channel.en_bandwidth;
}
    ...
    mac_vap_stru *mac_vap)
{
    ...
    if (mac_vap->st_ch_switch_info.en_ch_switch_status == WLAN_CH_SWITCH_STATUS_1) {
        return OAL_SUCC;
    }

    if (mac_device->st_dfs.st_dfs_info.uc_custom_next_chnum) {
        new_channel = mac_device->st_dfs.st_dfs_info.uc_custom_next_chnum;
        new_bandwidth = hmac_dfs_switch_channel_for_radar_get_new_bandwidth(mac_device, mac_vap);
        oam_warning_log2(mac_vap->uc_vap_id, OAM_SF_CHAN, "{hmac_dfs_switch_channel_for_radar::\
            next ch(%d) and bw(%d) set by user.}", new_channel, new_bandwidth);

        if (hmac_dfs_is_channel_support_bw(mac_vap, new_channel, new_bandwidth) != OAL_SUCC) {
            oam_warning_log2(mac_vap->uc_vap_id, OAM_SF_DFS, "{hmac_dfs_switch_channel_for_radar::\
                nnew chan(%d) not support current bw mode(%d).}", new_channel, new_bandwidth);
            // 如果应用设定的下一跳信道不支持当前的带宽模式，那么按照驱动的随机策略选择
            ret = hmac_dfs_select_random_channel(mac_vap, &new_channel, &new_bandwidth);
            if (ret != OAL_SUCC) {
                oam_warning_log0(mac_vap->uc_vap_id, OAM_SF_DFS,
    ...
    hmac_device_stru *hmac_device)
{
    ...
    for (vap_id = 0; vap_id < mac_device->uc_vap_num; vap_id++) {
        ...
        if (vap != NULL && vap->en_vap_mode == WLAN_VAP_MODE_BSS_AP) {
            ...
            /* DBAC场景下依然可以满足 */
            if (hmac_dfs_try_cac(hmac_device, vap) == OAL_TRUE) {
 ...
 */
#ifdef _PRE_WLAN_FEATURE_DFS
OAL_STATIC mac_device_stru *hmac_chan_multi_switch_get_mac_device(mac_vap_stru *mac_vap)
{
    mac_device_stru *mac_device = mac_res_get_dev(mac_vap->uc_device_id);
    if (mac_device == NULL) {
        oam_error_log1(mac_vap->uc_vap_id, OAM_SF_2040,
            "{hmac_chan_multi_switch_to_new_channel::pst_device(%d) null.}", mac_vap->uc_device_id);
        return NULL;
    }

    if (mac_device->uc_vap_num == 0) {
        oam_error_log0(mac_vap->uc_vap_id, OAM_SF_2040, "{hmac_chan_multi_switch_to_new_channel::none vap.}");
        return NULL;
    }
    return mac_device;
}
    ...
    uint8_t uc_channel, wlan_channel_bandwidth_enum_uint8 en_bandwidth)
{
    ...
    pst_mac_device = hmac_chan_multi_switch_get_mac_device(pst_mac_vap);
    ...
    for (uc_vap_idx = 0; uc_vap_idx < pst_mac_device->uc_vap_num; uc_vap_idx++) {
        ...
        if (is_ap(pst_vap) == OAL_FALSE) { /* sta/gc需跟随ap/go切信道, 无法主动切信道, 此次不处理 */
            continue;
        }
        /* 如果某个vap覆盖的信道没有被block，说明不需要切信道 */
        if (hmac_check_channel_block_status(pst_vap, &(pst_vap->st_channel)) == OAL_FALSE) {
            continue;
        }
#ifdef _PRE_WLAN_CHBA_MGMT
        if (mac_is_chba_mode(pst_vap) == OAL_TRUE) {
            hmac_chba_adjust_channel_for_radar(pst_vap, uc_channel, en_bandwidth);
            continue;
        }
#endif
 ...
 *   修改内容  : 新生成函数
 */
oal_bool_enum_uint8 mac_dfs_get_dfs_enable(mac_vap_stru *mac_vap)
{
    mac_device_stru *mac_device = mac_res_get_dev(mac_vap->uc_device_id);
    if (mac_device == NULL) {
        oam_warning_log1(mac_vap->uc_vap_id, OAM_SF_CFG, "{mac_vap_get_dfs_enable::mac_res_get_dev[%d] Error.}",
            mac_vap->uc_device_id);
        return OAL_FALSE;
    }
    /* set_channel时如果up_vap>=2，device下信息不会更新，根据vap判断是准确的 */
    if (mac_vap->st_channel.en_band == WLAN_BAND_5G) {
        return mac_device->st_dfs.st_dfs_info.en_dfs_switch;
    }
 ...
 *   修改内容  : 新生成函数
 */
oal_bool_enum_uint8 mac_vap_get_dfs_enable(mac_vap_stru *mac_vap)
{
    /* 先判断vap是否支持，再判断mac_device的dfs开关是否打开 */
    if (mac_vap_is_support_dfs(mac_vap) == OAL_TRUE) {
        return mac_dfs_get_dfs_enable(mac_vap);
    }
    return OAL_FALSE;
}
...
/* 功能描述: dfs定时器运行状态维测 */
OAL_STATIC void hmac_dfs_nol_timer_debug(mac_device_stru *mac_device)
{
    uint8_t nol_timer_cnt = 0;
    oal_dlist_head_stru *nol_list_head = NULL;
    mac_dfs_nol_node_stru *nol_node = NULL;
    oal_dlist_head_stru *nol_list_pos = NULL;
    oal_dlist_head_stru *nol_list_temp = NULL;
    ...
    oal_mutex_lock(&mac_device->st_dfs.dfs_nol_lock);
    nol_list_head = &mac_device->st_dfs.st_dfs_nol;
    oal_dlist_search_for_each_safe(nol_list_pos, nol_list_temp, nol_list_head)
    {
        nol_node = oal_dlist_get_entry(nol_list_pos, mac_dfs_nol_node_stru, st_entry);
        oam_warning_log2(0, OAM_SF_ANY,
            "{hmac_dfs_nol_timer_debug:chan_idx[%d] nol_timer is registerd[%d]}",
            nol_node->uc_chan_idx, nol_node->st_dfs_nol_timer.en_is_registerd);
        nol_timer_cnt++;
    }
    oal_mutex_unlock(&mac_device->st_dfs.dfs_nol_lock);
    oam_warning_log2(0, OAM_SF_ANY, "{hmac_dfs_nol_timer_debug:cur_time 0x%xms, nol_timer_cnt[%d]}",
        (uint32_t)oal_time_get_stamp_ms(), nol_timer_cnt);
}

/* 功能描述: 更新nol timer状态 */
OAL_STATIC void hmac_dfs_nol_timer_status_update(mac_device_stru *mac_device, mac_channel_list_stru *chan_list_info,
    uint8_t chan_num)
{
    oal_dlist_head_stru *nol_list_head = NULL;
    mac_dfs_nol_node_stru *nol_node = NULL;
    oal_dlist_head_stru *nol_list_pos = NULL;
    oal_dlist_head_stru *nol_list_temp = NULL;
    uint32_t cur_time = hmac_dfs_get_cur_boottime_ms();
    uint32_t run_time;
    ...
    {
        nol_node = oal_dlist_get_entry(nol_list_pos, mac_dfs_nol_node_stru, st_entry);
        if (chan_list_info->ast_channels[chan_num].uc_idx != nol_node->uc_chan_idx) {
            continue;
        }
        run_time = oal_time_get_runtime(nol_node->dfs_nol_start_time_ms, cur_time);
        if (run_time >= nol_node->dfs_nol_wait_time) { /* 已超过定时时长, 停止frw_timer */
            oam_warning_log4(0, OAM_SF_ANY,
                "{hmac_dfs_nol_timer_status_update:force chan_idx[%d] timeout! [0x%xms -> 0x%xms], run %dms}",
                nol_node->uc_chan_idx, nol_node->dfs_nol_start_time_ms, cur_time, run_time);
            frw_timer_destroy_timer(&(nol_node->st_dfs_nol_timer));
            hmac_dfs_nol_delchan(mac_device, nol_node);
        }
    }
    oal_mutex_unlock(&mac_device->st_dfs.dfs_nol_lock);
}

/* 配信道时检查是否被block，true为block */
oal_bool_enum_uint8 hmac_check_channel_block_status(mac_vap_stru *mac_vap, mac_channel_stru *channel_param)
{
    mac_channel_list_stru chan_list_info = {0};
    mac_device_stru *mac_device = NULL;
    uint8_t chan_num;
    mac_chan_status_enum_uint8 chan_status;

    mac_device = mac_res_get_dev(mac_vap->uc_device_id);
    if (oal_unlikely(mac_device == NULL)) {
        return OAL_TRUE;
    }
    if (channel_param->en_band == WLAN_BAND_2G) {
        return OAL_FALSE;
    }
    /* debug模式打开返回false */
    if (mac_device->st_dfs.st_dfs_info.dfsmode != DFS_NO_DEBUG_MODE) {
        oam_warning_log2(mac_vap->uc_vap_id, OAM_SF_CHAN, "hmac_check_channel_block_status:return:band[%d] dfsmode[%d]",
            mac_vap->st_channel.en_band, mac_device->st_dfs.st_dfs_info.dfsmode);
        return OAL_FALSE;
    }
    /* 获取覆盖的信道索引表 */
    mac_get_ext_chan_info(channel_param->uc_chan_idx, channel_param->en_bandwidth, &chan_list_info);
    for (chan_num = 0; chan_num < chan_list_info.channels; chan_num++) {
        if ((chan_num >= MAC_MAX_20M_SUB_CH) ||
            (chan_list_info.ast_channels[chan_num].uc_idx >= MAC_MAX_SUPP_CHANNEL)) {
            continue;
        }
        chan_status = mac_device->st_ap_channel_list[chan_list_info.ast_channels[chan_num].uc_idx].en_ch_status;
        
        if (chan_status == MAC_CHAN_BLOCK_DUE_TO_RADAR) {
            hmac_dfs_nol_timer_status_update(mac_device, &chan_list_info, chan_num);
            chan_status = mac_device->st_ap_channel_list[chan_list_info.ast_channels[chan_num].uc_idx].en_ch_status;
        }
        if (chan_status == MAC_CHAN_BLOCK_DUE_TO_RADAR) {
            oam_warning_log1(mac_vap->uc_vap_id, OAM_SF_CHAN,
                "hmac_check_channel_block_status::channel[%d] is blocked by radar", channel_param->uc_chan_number);
            hmac_dfs_nol_timer_debug(mac_device); /* 检测到信道block, 打印nol timer维测信息 */
            return OAL_TRUE;
        }
    }
    return OAL_FALSE;
...
#ifdef _PRE_WLAN_FEATURE_ATP_REPORT
/* 检测到雷达跳转到新信道，需要上报 */
void hmac_dfs_switch_channel_report(mac_vap_stru *mac_vap, uint8_t new_channel)
{
    hmac_vap_dfs_channel_switch_stru dfs_channel_switch = {0};
    dfs_channel_switch.dest_channel = new_channel;
    dfs_channel_switch.src_channel = mac_vap->st_channel.uc_chan_number;
    ATP_Netlink_SendToUserspace(ATP_MSG_WIFI_DFS_CHANNEL_SWITCH, (void *)&dfs_channel_switch,
        sizeof(dfs_channel_switch));
}
#endif
