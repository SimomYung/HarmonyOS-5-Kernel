...
/* dfs切信道上报id */
#define ATP_MSG_WIFI_DFS_CHANNEL_SWITCH 42428
typedef struct {
    uint32_t src_channel;
    uint32_t dest_channel;
} hmac_vap_dfs_channel_switch_stru;
#endif

extern uint8_t g_ap_cac;

typedef struct {
    uint8_t uc_chan_idx;  /* 信道索引 */
    uint8_t uc_device_id; /* device id */
    uint8_t auc_resv[NUM_2_BYTES];
    uint32_t dfs_nol_wait_time; /* NOL定时时长 */
    uint32_t dfs_nol_start_time_ms; /* 记录NOL启动时间戳 */
...
uint32_t hmac_dfs_init_scan_hook(hmac_scan_record_stru *scan_record, hmac_device_stru *hmac_device);
    ...
    mac_vap_stru *mac_vap)
{
    ...
    /* dfs使能位 */
    if (mac_vap_get_dfs_enable(mac_vap) == OAL_FALSE) {
    ...
    /* CAC使能位 */
    if (mac_dfs_get_cac_enable(mac_device) == OAL_FALSE) {
        oam_warning_log0(0, OAM_SF_DFS, "cac not enabled");
        return OAL_FALSE;
    }

    if (hmac_dfs_ap_cac_check(mac_vap) == OAL_FAIL) {
