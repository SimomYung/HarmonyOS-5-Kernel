...
{
    if (memcpy_s(pst_bss_list_alloc[uc_bss_list_index].st_assoc_delay_attr_mbo_ie.auc_oui,
        sizeof(pst_bss_list_alloc[uc_bss_list_index].st_assoc_delay_attr_mbo_ie.auc_oui),
        puc_ie_data + MAC_IE_HDR_LEN, MAC_MBO_OUI_LENGTH) != EOK) {
                                                ...
                                                uint16_t us_frame_len)
{
    ...
    ies_info.frame_ies = puc_frame_body;
    ies_info.frame_len = us_frame_len;
    puc_mbo_ie = mac_find_vendor_ie(MAC_MBO_VENDOR_IE, MAC_MBO_IE_OUI_TYPE, &ies_info);
    if (puc_mbo_ie == NULL) {
