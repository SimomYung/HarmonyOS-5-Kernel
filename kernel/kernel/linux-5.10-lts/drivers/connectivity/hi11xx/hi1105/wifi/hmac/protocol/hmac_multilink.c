    ...
    uint8_t *common_info, mac_ml_control_union ml_control)
{
    ...
    if (ml_control.basic_ml_control.ml_eml_capabilities_present == 1) {
        mac_set_eml_capabilities_field(mld_vap_manage, tmp_common_info);
...
OAL_STATIC uint32_t hmac_scan_update_bss_multi_link_info(hmac_vap_stru *hmac_vap, mac_bss_dscr_stru *bss_dscr)
{
    ...
    if (is_custom_open_emlsr_switch(&hmac_vap->st_vap_base_info) == OAL_FALSE) {
        memset_s(&bss_dscr->ml_info.eml_cap, sizeof(mac_eml_capabilities_stru), 0, sizeof(mac_eml_capabilities_stru));
    }
...
hmac_vap_stru *hmac_mld_get_sta_vap_by_ap_link_id(hmac_vap_stru *hmac_vap, int8_t ap_link_id)
{
    ...
    if (mld_vap_manage == NULL) {
        oam_warning_log0(hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_CFG,
            "{hmac_mld_get_link_vap_by_link_id::mld_vap_manage null}");
        return NULL;
    }
    ...
    uint16_t link_info_len, ml_info_stru *ap_mld_info, uint8_t frame_type)
{
    ...
    *********************************************************************************/
    ...
    if (defrag_sub_ie == NULL) {
        oam_error_log1(0, OAM_SF_ASSOC, "{hmac_sta_parse_basic_ml_link_info::alloc %d fail.}",
            defrag_sub_ie_len);
        return OAL_ERR_CODE_PTR_NULL;
    }
    ...
    ml_info_stru *ap_mld_info, uint8_t common_info_len)
{
    ...
    if ((common_info[0] != common_info_len) ||
        ...
        oam_warning_log0(0, OAM_SF_ANY, "{hmac_sta_parse_basic_ml_common_info::invalid common info length.}");
        return OAL_FAIL;
    }
...
OAL_STATIC uint32_t hmac_scan_parse_rnr_ie(uint8_t *neigh_rpt_ie, uint8_t ie_len, mac_bss_dscr_stru *bss_dscr)
{
    uint16_t offset = 0;
    uint8_t idx = 1;
    uint8_t tbtt_info_idx = 0;
    mac_neighbor_ap_info_stru *neigh_ap_info = NULL;
    mac_mld_tbtt_info_stru *mld_tbtt_info = NULL;

    while ((neigh_rpt_ie[1] > offset) && (idx < WLAN_AP_MAX_ML_LINK_INFO_NUM) &&
           (ie_len > offset + sizeof(mac_neighbor_ap_info_stru))) {
        neigh_ap_info = (mac_neighbor_ap_info_stru *)((uint8_t *)neigh_rpt_ie + MAC_IE_HDR_LEN + offset);
        /* field_type非0 或 TBTT_INFO字段不包含MLD参数，直接跳过该段信息 */
        if ((neigh_ap_info->tbtt_info_hdr.tbtt_infomation_field_type != 0) ||
            (neigh_ap_info->tbtt_info_hdr.tbtt_infomation_length) < TBTT_INFO_INCLUDE_MLD_LEN_TYPE0) {
            offset += neigh_ap_info->tbtt_info_hdr.tbtt_infomation_length *
                (neigh_ap_info->tbtt_info_hdr.tbtt_infomation_count + 1) + NEIGHBOR_AP_TBTT_INFO_SET_OFFSET;
            continue;
        }
        ...
        if (neigh_rpt_ie[1] < offset) {
            oam_error_log0(0, OAM_SF_SCAN, "hmac_scan_parse_rnr_ie::not enough rnr info.");
            break;
        }
        /* 解析TBTT Infomation */
        mld_tbtt_info = (mac_mld_tbtt_info_stru *)((uint8_t *)neigh_ap_info + NEIGHBOR_AP_TBTT_INFO_SET_OFFSET);
        for (tbtt_info_idx = 0; tbtt_info_idx <= neigh_ap_info->tbtt_info_hdr.tbtt_infomation_count; tbtt_info_idx++) {
            if ((mld_tbtt_info->bss_param.same_ssid != OAL_TRUE) || (mld_tbtt_info->mld_param.mld_id == 0xff)) {
                mld_tbtt_info += 1;
                continue;
            }

            idx = hmac_add_link_info_find_idx(bss_dscr->ml_info.ml_link_info, mld_tbtt_info->bssid, bss_dscr);
            if (memcpy_s(bss_dscr->ml_info.ml_link_info[idx].sta_mac_addr, WLAN_MAC_ADDR_LEN,
                         mld_tbtt_info->bssid, WLAN_MAC_ADDR_LEN) != EOK) {
                oam_error_log0(0, OAM_SF_SCAN, "hmac_scan_parse_rnr_ie::copy mac_addr failed");
                return OAL_FAIL;
            }

            /* 解析neighbor AP info字段中指示的其他link的channel number */
            bss_dscr->ml_info.ml_link_info[idx].st_channel.uc_chan_number = neigh_ap_info->channel_num;
            bss_dscr->ml_info.ml_link_info[idx].st_channel.en_band =
                mac_get_band_by_channel_num(neigh_ap_info->channel_num);
            bss_dscr->ml_info.ml_link_info[idx].link_id = mld_tbtt_info->mld_param.link_id;
            bss_dscr->ml_info.ml_link_info[idx].is_add_link_list = OAL_TRUE;
            bss_dscr->ml_info.mld_id = mld_tbtt_info->mld_param.mld_id;

            mld_tbtt_info += 1;
        }
    }

    if (neigh_rpt_ie[1] != offset && (idx < WLAN_AP_MAX_ML_LINK_INFO_NUM)) {
        oam_error_log3(0, OAM_SF_ANY,
            "{hmac_scan_parse_reduced_neighbor_report_ie::invalid length. ie_len %u, offset %u, idx %u}",
            neigh_rpt_ie[1], offset, idx);
        return OAL_FAIL;
    }
    return OAL_SUCC;
}
    ...
    uint8_t *frame_body, uint16_t frame_len)
{
    ...
    if (bss_dscr->eht_capable == OAL_FALSE ||
        is_custom_open_mlo_switch(&hmac_vap->st_vap_base_info) == OAL_FALSE ||
    ...
    /* 扫描结果更新 */
    ret = hmac_scan_update_bss_multi_link_info(hmac_vap, bss_dscr);
    if (ret != OAL_SUCC) {
...
uint32_t hmac_mld_update_mld_user_info(hmac_vap_stru *hmac_vap, hmac_user_stru *hmac_user, ml_info_stru *ap_ml_info)
{
    ...
    if (mac_mib_get_EHTEMLSROptionActivated(&hmac_vap->st_vap_base_info) == OAL_TRUE) {
        memcpy_s(&mld_user_manage->eml_hdl.eml_cap_param, sizeof(mac_eml_capabilities_stru),
            &ap_ml_info->eml_cap, sizeof(mac_eml_capabilities_stru));
        mld_user_manage->eml_hdl.emlsr_capable = OAL_TRUE;
    }
    ...
    uint8_t *common_info, mac_eht_ml_ie_header_stru *ml_ie_stru, uint8_t common_info_len)
{
    ...
    // EML Cap 字段解析
    if (ml_ie_stru->ml_control.basic_ml_control.ml_eml_capabilities_present == 1) {
        if (memcpy_s(&mld_user_manage->eml_hdl.eml_cap_param, EML_CAPBILITIES_FIELD_LEN,
            &common_info[offset], EML_CAPBILITIES_FIELD_LEN) != EOK) {
            return OAL_FAIL;
        }
        offset += EML_CAPBILITIES_FIELD_LEN;
