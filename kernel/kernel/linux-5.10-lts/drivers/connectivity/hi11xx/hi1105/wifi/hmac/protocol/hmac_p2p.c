...
mac_vap_state_priority_stru g_mac_vap_state_priority_table[MAC_VAP_STATE_BUTT] = {
    ...
    { 10 }, /* {MAC_VAP_STATE_STA_WAIT_MLD_SCAN */
    { 0 }, /* {MAC_VAP_STATE_STA_LINK_DOWN */
...
oal_bool_enum_uint8 hmac_enter_state_check_vap_connect_state(mac_vap_stru *mac_vap,
    hmac_fsm_input_type_enum input_req, oal_bool_enum_uint8 is_high_prio_event)
{
#ifdef _PRE_WLAN_CHBA_MGMT
    /* chba连接状态需做特殊判断，除普通user状态外, chba vap处于极速连接响应端、vap等待同步或域合并状态都需要禁止扫描 */
    if (hmac_chba_is_connecting_or_syncing(mac_vap) == OAL_TRUE) {
        return OAL_TRUE;
    }
#endif
...
#ifdef _PRE_WLAN_FEATURE_ML
        /* 当前join vap与另一vap均为mld模式时，为MLD双link入网场景，不根据vap state拦截 */
        if (hmac_mlo_check_can_enter_state(mac_vap, other_vap) == OAL_TRUE) {
            continue;
        }
#endif
...
uint32_t hmac_add_p2p_cl_vap(mac_vap_stru *mac_vap, uint16_t len, uint8_t *param)
{
    ...
    if (ret != OAL_SUCC) {
        oam_warning_log1(mac_vap->uc_vap_id, 0, "{hmac_add_p2p_cl_vap::hmac_config_check_vap_num failed[%d].}", ret);
        return ret;
    }
    ...
    /* 包括'\0' */
    if (memcpy_s(hmac_vap->auc_name, OAL_IF_NAME_SIZE, vap_param->pst_net_dev->name, OAL_IF_NAME_SIZE) != EOK) {
    ...
    dpe_wlan_config_add_vap(param, hmac_vap->st_vap_base_info.uc_device_id,
        hmac_vap->st_vap_base_info.uc_chip_id, hmac_vap->st_vap_base_info.core_id);
    /* 抛事件到DMAC层, 同步DMAC数据 */
...
uint32_t hmac_p2p_cancel_remain_on_channel(mac_vap_stru *pst_mac_vap, uint16_t us_len, uint8_t *puc_param)
{
    ...
    if (pst_mac_vap->en_vap_state == MAC_VAP_STATE_STA_LISTEN) {
        ...
        hmac_reset_same_chan_remain_on_chan(pst_mac_vap); /* 功能描述: p2p扫描结束后处理 */
    ...
    hmac_device_stru *hmac_device, wlan_ieee80211_roc_type_uint8 roc_type)
{
    oal_bool_enum_uint8 is_high_prio_scan =
        ((roc_type == IEEE80211_ROC_TYPE_CHBA_HIGH_PRIORITY_MGMT_TX) || (roc_type == IEEE80211_ROC_TYPE_CHBA_MGMT_RX));
