 ...
 *   其    他  : 3地址情况下使用
 */
OAL_STATIC OAL_INLINE void mac_get_qos_ctrl(uint8_t *mac_hdr, uint8_t mac_hdr_len,
    uint8_t *qos_ctrl, uint8_t qos_ctl_len)
{
    if (mac_is_4addr((mac_ieee80211_frame_stru *)mac_hdr) != OAL_TRUE) {
        if (mac_hdr_len < MAC_80211_QOS_FRAME_LEN) {
            return;
        }
        if (memcpy_s(qos_ctrl, qos_ctl_len, mac_hdr + MAC_QOS_CTRL_FIELD_OFFSET, MAC_QOS_CTL_LEN) != EOK) {
            return;
        }
        return;
    }
    if (mac_hdr_len < MAC_QOS_CTRL_FIELD_OFFSET_4ADDR + MAC_QOS_CTL_LEN) {
        return;
    }
    if (memcpy_s(qos_ctrl, qos_ctl_len, mac_hdr + MAC_QOS_CTRL_FIELD_OFFSET_4ADDR, MAC_QOS_CTL_LEN) != EOK) {
        return;
    }
    return;
}
