    ...
    wlan_bw_cap_enum_uint8 bw_cap)
{
    ...
    if (channel <= 14) { /* 14为2G最大信道号 */
        if (idx < WLAN_2G_CHANNEL_NUM) {
            /* 2G信道带宽超过该信道最大值时, 为防止越界, bw_cap取该信道合法的最大带宽 */
            if (bw_cap >= g_bw_mode_table_2g[idx].uc_cnt) {
                oam_warning_log2(0, OAM_SF_ANY,
                    "{mac_regdomain_get_bw_by_channel_bw_cap:invalid 2G chan, idx[%d] bw[%d]", idx, bw_cap);
                bw_cap = (g_bw_mode_table_2g[idx].uc_cnt >= 1) ? (g_bw_mode_table_2g[idx].uc_cnt - 1) : 0;
            }
            channel_bw = g_bw_mode_table_2g[idx].aen_supp_bw[bw_cap];
        }
    } else {
        if (idx < WLAN_5G_CHANNEL_NUM) {
            /* 5G信道带宽超过该信道最大值时, 为防止越界, bw_cap取该信道合法的最大带宽 */
            if (bw_cap >= g_bw_mode_table_5g[idx].uc_cnt) {
                oam_warning_log2(0, OAM_SF_ANY,
                    "{mac_regdomain_get_bw_by_channel_bw_cap:invalid 5G chan, idx[%d] bw[%d]", idx, bw_cap);
                bw_cap = (g_bw_mode_table_5g[idx].uc_cnt >= 1) ? (g_bw_mode_table_5g[idx].uc_cnt - 1) : 0;
            }
