...
uint32_t hmac_config_set_freq(mac_vap_stru *mac_vap, uint16_t len, uint8_t *param)
{
    ...
    if (mac_device->uc_vap_num == 1 || mac_device->uc_max_channel == 0) {
        ...
        /***************************************************************************
            抛事件到DMAC层, 同步DMAC数据
        ***************************************************************************/
        ret = hmac_config_send_event(mac_vap, WLAN_CFGID_CURRENT_CHANEL, len, param);
        if (oal_unlikely(ret != OAL_SUCC)) {
...
uint32_t hmac_config_set_wmm_params(mac_vap_stru *mac_vap, uint16_t len, uint8_t *param)
{
    ...
    pst_cfg_stru = (hmac_config_wmm_para_stru *)param;
    ...
    /***************************************************************************
        抛事件到DMAC层, 同步DMAC数据
    ***************************************************************************/
    ret = hmac_config_send_event(mac_vap, en_cfg_id, len, param);
    if (oal_unlikely(ret != OAL_SUCC)) {
