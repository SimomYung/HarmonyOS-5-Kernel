...
OAL_STATIC hmac_protocol_stru g_st_protocol_mode_list[WLAN_PROTOCOL_BUTT] = {
    ...
    { WLAN_HE_MODE, { 0 }, "11ax" },
#endif
#if defined(_PRE_WLAN_FEATURE_11BE)
    { WLAN_EHT_MODE, { 0 }, "11be" },
#endif
};
...
uint32_t hmac_config_user_rate_info_syn(mac_vap_stru *pst_mac_vap, mac_user_stru *pst_mac_user)
{
    ...
    if (g_wlan_spec_cfg->feature_11be_is_open) {
        /* eht速率集信息，同步到dmac */
        mac_user_get_eht_hdl(pst_mac_user, &(st_mac_h2d_usr_rate_info.st_eht_hdl));
    }
#endif
...
uint32_t hmac_config_user_info_syn(mac_vap_stru *pst_mac_vap, mac_user_stru *pst_mac_user)
{
    ...
    ***************************************************************************/
    ...
    dpe_wlan_user_info_sync((uint8_t *)(&st_mac_h2d_usr_info));
    return ret;
}
...
uint32_t hmac_config_user_ml_info_syn(mac_vap_stru *mac_vap, mac_user_stru *mac_user)
{
    ...
    mac_h2d_usr_ml_info_stru mac_h2d_usr_ml_info = { 0 };
    uint32_t ret;

    if (mac_vap == NULL || mac_user == NULL) {
        oam_error_log0(0, OAM_SF_CFG, "hmac_config_user_ml_info_syn: invalid input param");
        return OAL_ERR_CODE_PTR_NULL;
    }

    hmac_user = mac_res_get_hmac_user(mac_user->us_assoc_id);
    if (hmac_user == NULL) {
        oam_error_log1(mac_vap->uc_vap_id, OAM_SF_CFG, "hmac_config_user_ml_info_syn: hmac_user is NULL, user_id=%u",
            mac_user->us_assoc_id);
        return OAL_ERR_CODE_PTR_NULL;
    }
    ...
    mac_h2d_usr_ml_info.user_idx = mac_user->us_assoc_id;
    mac_h2d_usr_ml_info.bpcc = hmac_user->mld_user_link_info.bpcc;

    /***************************************************************************
        抛事件到DMAC层, 同步USER最新ML信息到DMAC
    ***************************************************************************/
    ret = hmac_config_send_event(mac_vap, WLAN_CFGID_USER_ML_INFO_SYN,
                                 sizeof(mac_h2d_usr_ml_info), (uint8_t *)(&mac_h2d_usr_ml_info));
    if (oal_unlikely(ret != OAL_SUCC)) {
        oam_error_log2(mac_vap->uc_vap_id, OAM_SF_CFG,
            "hmac_config_user_ml_info_syn: hmac_config_send_event failed, ret=%u, user_id=%u",
            ret, mac_user->us_assoc_id);
        return OAL_FAIL;
    }
    return OAL_SUCC;
}
...
uint32_t hmac_config_user_mld_cap_syn(mac_vap_stru *mac_vap, mac_user_stru *mac_user)
{
    ...
    mac_h2d_usr_mld_cap_stru mac_h2d_usr_mld_cap = {0};
    uint32_t ret;
    if (oal_unlikely(mac_vap == NULL || mac_user == NULL)) {
        oam_error_log0(0, OAM_SF_CFG, "hmac_config_user_mld_cap_syn: invalid input param");
        return OAL_ERR_CODE_PTR_NULL;
    }
    hmac_user = mac_res_get_hmac_user(mac_user->us_assoc_id);
    if (oal_unlikely(hmac_user == NULL)) {
        oam_error_log1(mac_vap->uc_vap_id, OAM_SF_CFG, "hmac_config_user_mld_cap_syn: hmac_user is NULL, user_id=%u",
            mac_user->us_assoc_id);
        return OAL_ERR_CODE_PTR_NULL;
    }
    ...
    if (oal_unlikely(mld_user_manage == NULL)) {
        oam_error_log1(mac_vap->uc_vap_id, OAM_SF_CFG, "hmac_config_user_mld_cap_syn: mld_user_manage is NULL,\
            user_id=%u", mac_user->us_assoc_id);
        return OAL_ERR_CODE_PTR_NULL;
    }
    mac_h2d_usr_mld_cap.user_idx = mac_user->us_assoc_id;
    memcpy_s(&(mac_h2d_usr_mld_cap.mld_cap), sizeof(mac_mld_capabilities_stru),
        &(mld_user_manage->mld_cap),
        sizeof(mac_mld_capabilities_stru));
    memcpy_s(&mac_h2d_usr_mld_cap.eml_cap, sizeof(mac_eml_capabilities_stru),
        &mld_user_manage->eml_hdl.eml_cap_param, sizeof(mac_eml_capabilities_stru));
    memcpy_s(&mac_h2d_usr_mld_cap.medium_cap, sizeof(mac_medium_sync_info_stru),
        &mld_user_manage->medium_cap, sizeof(mac_medium_sync_info_stru));
    /***************************************************************************
        抛事件到DMAC层, 同步USER最新MLD_CAP信息到DMAC
    ***************************************************************************/
    ret = hmac_config_send_event(mac_vap, WLAN_CFGID_MLD_CAP_SYN,
                                 sizeof(mac_h2d_usr_mld_cap_stru), (uint8_t *)(&mac_h2d_usr_mld_cap));
    if (oal_unlikely(ret != OAL_SUCC)) {
        oam_error_log2(mac_vap->uc_vap_id, OAM_SF_CFG,
            "hmac_config_user_mld_cap_syn: hmac_config_send_event failed, ret=%u, user_id=%u",
            ret, mac_user->us_assoc_id);
        return OAL_FAIL;
    }
    return OAL_SUCC;
...
OAL_STATIC void mac_vap_init_user_security_port(mac_vap_stru *pst_mac_vap, hmac_user_stru *hmac_user)
{
    if (mac_mib_get_rsnaactivated(pst_mac_vap) != OAL_TRUE) {
        mac_user_set_port(&(hmac_user->st_user_base_info), OAL_TRUE);
    }
#ifdef _PRE_WLAN_CHBA_MGMT
    /* chba非加密连接模式, 直接开启用户的加密端口 */
    if ((mac_is_chba_mode(pst_mac_vap) == OAL_TRUE) &&
        (hmac_chba_check_no_encry_connect_flag(hmac_user->chba_user.connect_info.conn_flag) == OAL_TRUE)) {
        mac_user_set_port(&(hmac_user->st_user_base_info), OAL_TRUE);
    }
#endif
}
...
uint32_t hmac_init_user_security_port(mac_vap_stru *pst_mac_vap, hmac_user_stru *hmac_user)
{
    ...
    /* 初始化认证端口信息 */
    mac_vap_init_user_security_port(pst_mac_vap, hmac_user);
    /* 同步至device */
    st_init_port.en_port_valid = mac_user_get_port(mac_user);
    memcpy_s(st_init_port.auc_mac_addr, OAL_MAC_ADDR_LEN, mac_user->auc_user_mac_addr, OAL_MAC_ADDR_LEN);
...
uint32_t hmac_config_user_num_spatial_stream_cap_syn(mac_vap_stru *pst_mac_vap, mac_user_stru *pst_mac_user)
{
    ...
    if (pst_mac_vap == NULL) {
        oam_error_log0(pst_mac_user->uc_vap_id, OAM_SF_CFG,
                       "hmac_config_user_num_spatial_stream_cap_syn::mac vap is null!");
        return ret;
    }
 ...
 */
void hmac_start_kick_user_timer(mac_vap_stru *mac_vap, hmac_user_stru *hmac_user)
{
    ...
    frw_timer_create_timer_m(&(hmac_user->kick_user_timer), hmac_kick_user_timeout,
        kick_user_timeout, (void *)hmac_user, OAL_FALSE, OAM_MODULE_ID_HMAC, mac_vap->core_id);
}
    ...
    uint16_t reason_code, oal_bool_enum_uint8 is_protected)
{
    ...
    if (is_legacy_ap(mac_vap) && !mac_is_chba_mode(mac_vap) &&
        ...
        hmac_mgmt_send_deauth_frame(mac_vap->uc_vap_id, da, reason_code, is_protected);
    } else {
    ...
    mac_cfg_kick_user_param_stru *kick_user)
{
    ...
    if (is_legacy_ap(&hmac_vap->st_vap_base_info) == OAL_TRUE ||
        ...
        if (hmac_vap->st_vap_base_info.nbw_mode == NBW_MODE) {
            oam_warning_log0(mac_vap->uc_vap_id, OAM_SF_UM, "{hmac_config_kick_user release nbw timeslot}");
            hmac_nbw_release_timeslot(hmac_user->st_user_base_info.auc_user_mac_addr, WLAN_MAC_ADDR_LEN,
                mac_mib_get_StationID(&hmac_vap->st_vap_base_info), WLAN_MAC_ADDR_LEN);
        }
#endif
        hmac_start_kick_user_timer(&hmac_vap->st_vap_base_info, hmac_user);
        return OAL_SUCC;
    }
