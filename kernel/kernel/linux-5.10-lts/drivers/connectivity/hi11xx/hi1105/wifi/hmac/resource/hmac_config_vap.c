...
oal_wait_queue_head_stru g_power_off_ready_wait_queue;
oal_bool_enum_uint8 g_power_off_ready = OAL_TRUE;
/*
...
/*
 * 函 数 名  : hmac_config_vap_widebw_ap_bw_syn
 * 功能描述  : dmac_offload架构下 widebw 特性同步ap bw到dmac_vap
 * 1.日    期  : 2024年3月13日
 *   作    者  : wifi
 *   修改内容  : 新生成函数
 */
uint32_t hmac_config_vap_widebw_ap_bw_syn(mac_vap_stru *mac_vap, uint16_t len, uint8_t *param)
{
    uint32_t ret;
    /***************************************************************************
        抛事件到DMAC层, 同步ap bw 到 DMAC
    ***************************************************************************/
    ret = hmac_config_send_event(mac_vap, WLAN_CFGID_WIDEBW_SYN, len, param);
    if (oal_unlikely(ret != OAL_SUCC)) {
        oam_warning_log1(mac_vap->uc_vap_id, OAM_SF_CFG,
                         "{hmac_config_vap_widebw_ap_bw_syn::hmac_config_send_event failed[%d].}", ret);
    }
    return ret;
}
/*
...
uint32_t hmac_set_ipaddr_timeout(void *puc_para)
{
    ...
    if ((pst_hmac_vap->uc_ps_mode == MAX_FAST_PS) || (pst_hmac_vap->uc_ps_mode == AUTO_FAST_PS)) {
        wlan_pm_set_host_timeout(pst_hmac_vap->st_vap_base_info.uc_device_id,
            (g_wlan_min_fast_ps_idle > 1) ? (g_wlan_min_fast_ps_idle - 1) : g_wlan_min_fast_ps_idle);
    } else {
        wlan_pm_set_host_timeout(pst_hmac_vap->st_vap_base_info.uc_device_id, WLAN_SLEEP_DEFAULT_CHECK_CNT);
    }
#endif
...
uint32_t hmac_config_d2h_hal_vap_syn(mac_vap_stru *mac_vap, uint8_t len, uint8_t *p_param)
{
    ...
    if ((p_hal_vap_info->hal_dev_id >= mac_chip_get_hal_dev_num()) ||
        (p_hal_vap_info->hal_vap_id >= mac_chip_get_hal_max_vap_num())) {
        return OAL_FAIL;
    }
    ...
    mac_device_stru *mac_device)
{
    ...
    if (oal_unlikely(mac_vap->st_channel.uc_chan_idx >= MAC_CHANNEL_FREQ_5_BUTT)) {
        oam_error_log1(mac_vap->uc_vap_id, OAM_SF_CFG, "hmac_config_down_vap_ap_mode_handle:chan_idx[%d] invalid!",
            mac_vap->st_channel.uc_chan_idx);
        return;
    }
    /* 只有vap所处频率和cac记录频率一致时取消定时器，防止2G down_ap关闭5G的cac定时器 */
    if ((mac_vap->st_channel.en_band == WLAN_BAND_5G) &&
        (g_ast_freq_map_5g[mac_vap->st_channel.uc_chan_idx].us_freq == g_st_dfs_result.us_freq)) {
...
uint32_t hmac_config_down_vap(mac_vap_stru *pst_mac_vap, uint16_t us_len, uint8_t *puc_param)
{
    ...
    dpe_wlan_config_down_vap(pst_mac_vap->uc_vap_id, pst_param);
    /***************************************************************************
                         抛事件到DMAC层, 同步DMAC数据
    ***************************************************************************/
    ...
    /* 如果正在进行长期同信道监听, down vap后检查其他vap能否继续同信道监听, 如果不行则abor监听 */
    hmac_abort_long_same_chan_remain_on_chan(pst_mac_vap, OAL_FALSE, MAC_ABORT_SAME_CHAN_REMAIN_ON_CHAN_DOWN_VAP);
...
#endif
    oam_warning_log3(pst_mac_vap->uc_vap_id, OAM_SF_MLOG, "{hmac_config_down_vap::SUCC! vap_id[%d] \
        Now remaining %d vaps in device[%d].}",
        pst_mac_vap->uc_vap_id, pst_mac_device->uc_vap_num, pst_mac_device->uc_device_id);
    return OAL_SUCC;
}
...
#endif
#ifdef _PRE_WLAN_FEATURE_ML
    if (pst_hmac_vap->delete_timer_info.delete_timer.en_is_registerd == OAL_TRUE) {
        frw_timer_immediate_destroy_timer_m(&pst_hmac_vap->delete_timer_info.delete_timer);
    }
    if (pst_hmac_vap->reconfig_op_info.req_timer.en_is_registerd == OAL_TRUE) {
        frw_timer_immediate_destroy_timer_m(&pst_hmac_vap->reconfig_op_info.req_timer);
    }
#endif
}
#if (_PRE_OS_VERSION_LINUX == _PRE_OS_VERSION)
static void hmac_del_vap_disable_hal_vap(hmac_vap_stru *hmac_vap)
{
    ...
    for (loop = 0; loop < mac_chip_get_hal_max_vap_num(); loop++) {
        if (haldev->hal_vap_sts_info[loop].mac_vap_id ==
            ...
            oam_warning_log2(0, OAM_SF_ANY, "{hmac_del_vap_disable_hal_vap::mac vap_id[%d],hal id[%d].}",
                hmac_vap->st_vap_base_info.uc_vap_id, loop);
            return;
        }
    }

    oam_warning_log1(0, OAM_SF_ANY, "{hmac_del_vap_disable_hal_vap::mac vap_id[%d] not find hal}",
        hmac_vap->st_vap_base_info.uc_vap_id);
...
uint32_t hmac_del_vap_post_process(mac_vap_stru *pst_vap)
{
    ...
    if (uc_vap_num != 0) {
        ...
        /* 清零powersaving cmd bitmap */
        hmac_down_vap_update_ps_cmd_bitmap(pst_vap);
#endif
        return OAL_SUCC;
    }
...
#ifdef _PRE_WLAN_FEATURE_ML
    hmac_mld_vap_deinit(hmac_vap);
#endif
#ifdef _PRE_WLAN_FEATURE_ESPE
    hmac_espe_deinit(hmac_vap);
#endif
    if ((is_legacy_ap(mac_vap)) && (mac_device_calc_up_vap_num(mac_device) == 0)) {
        mac_device_set_mode_smps(mac_device, WLAN_MIB_MIMO_POWER_SAVE_MIMO);
    }
}
...
static void hmac_config_del_cfg_vap(hmac_vap_stru *hmac_vap)
{
    ...
    dpe_wlan_config_del_cfg_vap(hmac_vap->st_vap_base_info.uc_vap_id);
    oal_smp_mb();
    ...
    mac_vap_stru *mac_vap, hmac_vap_stru *hmac_vap, uint16_t len, uint8_t *param)
{
    ...
    mac_res_free_mac_vap(mac_vap->uc_vap_id);

    hmac_vap_state_change_notify_feature(hmac_vap, NULL, COFEATURE_MVAP_EVENT, COFEATURE_EXIT);
#ifdef _PRE_WLAN_FEATURE_EMLSR
    hmac_vap_state_change_notify_emlsr(hmac_vap, NULL, COFEATURE_MVAP_EVENT, OAL_TRUE);
#endif
}
...
uint32_t hmac_config_del_vap(mac_vap_stru *mac_vap, uint16_t len, uint8_t *param)
{
...
#endif
    /* 如果正在进行长期同信道监听, del vap后检查其他vap能否继续同信道监听, 如果不行则abor监听 */
    hmac_del_vap_same_chan_remain_handler(mac_vap);
    ...
    hmac_config_del_vap_deinit_features(hmac_vap);
    ...
    dpe_wlan_config_del_vap(mac_vap->uc_vap_id, param, hmac_vap->pst_net_device);
    /***************************************************************************
                          抛事件到DMAC层, 同步DMAC数据
    ***************************************************************************/
...
{
    if ((g_p2p_go_max_user_num != 0) && (g_p2p_go_max_user_num < mac_chip_get_max_asoc_user(0))) {
 ...
 *   修改内容  : 新生成函数
 */
uint32_t hmac_config_set_max_user(mac_vap_stru *mac_vap, uint32_t max_user_num_temp)
{
    uint32_t p2p_max_user_num = hmac_get_p2p_max_user();
    uint32_t max_assoc_num = mac_chip_get_max_asoc_user(mac_vap->uc_chip_id);
    uint32_t max_user_num = max_user_num_temp;
...
OAL_STATIC uint32_t hmac_config_check_vap_comb(mac_device_stru *mac_device, mac_cfg_add_vap_param_stru *add_vap_param)
{
    ...
    if (wlan_chip_ap_p2p_device_coex_check_valid(mac_device, add_vap_param) == OAL_FALSE) {
        oam_warning_log0(0, OAM_SF_CFG, "{hmac_config_check_vap_comb::cannot add vap while ap work with p2p dev!}");
        return OAL_ERR_CODE_CONFIG_EXCEED_SPEC;
    }
...
static void hmac_vap_station_info_init(hmac_vap_stru *hmac_vap)
{
    /* 防止未进行get station查询，漫游扫描后当前bss的rssi被signal赋值成初始值0，误识别当前rssi很大，导致漫游选网失败 */
    hmac_vap->station_info.signal = ROAM_RSSI_CMD_TYPE;
}

static void hmac_config_add_vap_init_features(hmac_vap_stru *hmac_vap, mac_cfg_add_vap_param_stru *param)
{
    mac_vap_stru *mac_vap = &hmac_vap->st_vap_base_info;
    mac_device_stru *mac_dev = mac_res_get_dev(mac_vap->uc_device_id);

    if (is_p2p_go(mac_vap)) {
        hmac_config_set_max_user(mac_vap, hmac_get_p2p_max_user());
    }
#ifdef _PRE_WLAN_FEATURE_TWT
    if ((g_wlan_spec_cfg->feature_twt_is_open) && (mac_dev != NULL)) {
    ...
    hmac_vap_station_info_init(hmac_vap);

#ifdef _PRE_WLAN_CHBA_MGMT
    /* 启动chba supplicant时驱动就会add vap，但是chba mode是在hml start时才会置位，这里提前置位便于识别chba vap */
    hmac_chba_vap_set_chba_mode(mac_vap, param);
#endif

#ifdef _PRE_WLAN_FEATURE_ML
    hmac_vap_mld_init(hmac_vap, param);
#endif

#ifdef _PRE_WLAN_FEATURE_ESPE
    hmac_espe_init(hmac_vap);
#endif
    /* 下发多vap迁移表 */
    wlan_chip_mvap_send_trans_tbl(hmac_vap);
    wlan_chip_update_action_filter(mac_vap);
}
 ...
 *   修改内容  : 新生成函数
 */
uint32_t hmac_config_add_vap(mac_vap_stru *mac_vap, uint16_t len, uint8_t *param_input)
{
    mac_cfg_add_vap_param_stru *param = (mac_cfg_add_vap_param_stru *)param_input;
    hmac_vap_stru *hmac_vap = NULL;
    uint32_t ret;
    mac_device_stru *mac_dev = mac_res_get_dev(mac_vap->uc_device_id);
    ...
    if (g_wlan_spec_cfg->p2p_device_gc_use_one_vap && param->en_p2p_mode == WLAN_P2P_CL_MODE) {
        return hmac_add_p2p_cl_vap(mac_vap, len, param_input);
    }
    ...
    dpe_wlan_config_add_vap(param_input, new_mac_vap->uc_device_id, new_mac_vap->uc_chip_id, new_mac_vap->core_id);
    /***************************************************************************
        抛事件到DMAC层, 同步DMAC数据
    ***************************************************************************/
    ret = hmac_config_send_event(new_mac_vap, WLAN_CFGID_ADD_VAP, len, param_input);
    if (oal_unlikely(ret != OAL_SUCC)) {
    ...
    hmac_config_add_vap_init_features(hmac_vap, param);
...
uint32_t hmac_config_start_vap_event(mac_vap_stru *pst_mac_vap, oal_bool_enum_uint8 en_mgmt_rate_init_flag)
{
    ...
    dpe_wlan_config_start_vap(pst_mac_vap->uc_vap_id, &st_start_vap_param);
    if (oal_unlikely(ret != OAL_SUCC)) {
        oam_warning_log1(pst_mac_vap->uc_vap_id, OAM_SF_CFG,
...
#else
OAL_STATIC void hmac_config_start_vap_check_dfs(mac_device_stru *mac_device, mac_vap_stru *mac_vap)
{
 ...
 *   修改内容  : 新生成函数
 */
uint32_t hmac_config_start_vap(mac_vap_stru *mac_vap, uint16_t len, uint8_t *param)
{
    uint32_t ret;
    ...
    /* APUT GO CHBA启动前，退出emlsr omi */
    if (mac_vap->en_vap_mode == WLAN_VAP_MODE_BSS_AP) {
#ifdef _PRE_WLAN_FEATURE_11AX
        hmac_omi_force_close_process();
#endif
        hmac_mvap_stop_fastsleep();
    }
...
#ifdef _PRE_WLAN_FEATURE_DFS
#if (defined _PRE_WLAN_FEATURE_NBW) && (defined _PRE_WLAN_NBW_DEMO)
    /* 07c nbw demo指定在100信道工作，需要关闭cac */
#else
    hmac_config_start_vap_check_dfs(mac_device, mac_vap);
#endif
#endif

    oam_warning_log4(mac_vap->uc_vap_id, OAM_SF_MLOG, "{hmac_config_start_vap::host start vap ok. event to dmac. vap\
        id[%d] mode[%d], p2p mode[%d] bw[%d]}", mac_vap->uc_vap_id, mac_vap->en_vap_mode, mac_vap->en_p2p_mode,
...
#ifdef _PRE_WLAN_FEATURE_ML
int8_t *hmac_config_mld_assoc_mode2string(uint8_t mld_assoc_mode)
{
    int8_t *assoc_mode2string[] = {"mlsr", "str", "error"};
    return hmac_config_index2string(mld_assoc_mode, assoc_mode2string, sizeof(assoc_mode2string) / sizeof(int8_t *));
}
#endif
...
int8_t *hmac_config_protocol2string(uint32_t protocol)
{
    int8_t *pac_protocol2string[] = {
        ...
        "11ac", "11nonly", "11aconly", "11ng", "11ax", "error", "error", "11be"
    };
                                             ...
                                             int8_t *print_buff, uint32_t string_len)
{
    ...
    string_tmp_len = snprintf_s(print_buff + string_len, ((OAM_REPORT_MAX_STRING_LEN - string_len) - 1),
        ...
        "vap state: %u\n"
        "vap mode: %u   P2P mode:%u  chba mode:%u\n"
        "ssid: %.32s\n"
        "hide_ssid :%u\n"
        ...
        mac_vap->en_p2p_mode, mac_is_chba_mode(mac_vap), mac_mib_get_DesiredSSID(mac_vap),
        mac_vap->st_cap_flag.bit_hide_ssid,
 ...
 *   修改内容  : 新生成函数
 */
uint32_t hmac_config_vap_info(mac_vap_stru *mac_vap, uint16_t len, uint8_t *param)
{
    mac_device_stru *mac_device = NULL;
    hmac_vap_stru *hmac_vap = NULL;
    ...
    if ((g_tas_switch_en[WLAN_RF_CHANNEL_ZERO] == OAL_TRUE) || (g_tas_switch_en[WLAN_RF_CHANNEL_ONE] == OAL_TRUE)) {
        string_tmp_len = snprintf_s(print_buff + string_len, ((OAM_REPORT_MAX_STRING_LEN - string_len) - 1),
            ((OAM_REPORT_MAX_STRING_LEN - string_len) - 1) - 1, "tas_gpio[%d]\n", hal_get_wifi_tas_gpio_state());
        if (string_tmp_len < 0) {
            goto sprint_fail;
        }
        string_len += (uint32_t)string_tmp_len;
    ...
    /***************************************************************************
        抛事件到DMAC层, 同步DMAC数据
    ***************************************************************************/
    ret = hmac_config_send_event(mac_vap, WLAN_CFGID_VAP_INFO, len, param);
    if (oal_unlikely(ret != OAL_SUCC)) {
...
#ifdef _PRE_WLAN_FEATURE_ML
/* 函数功能：打印mld关联模式下，每个link的mld相关信息 */
OAL_STATIC int32_t hmac_print_mld_link_info(hmac_vap_stru *hmac_vap, int8_t *print_buff, uint32_t string_len)
{
    mac_vap_stru *mac_vap = NULL;
    int32_t string_tmp_len = 0;
    ...
    string_tmp_len = snprintf_s(print_buff + string_len, ((OAM_REPORT_MAX_STRING_LEN - string_len) - 1),
                                ((OAM_REPORT_MAX_STRING_LEN - string_len) - 1) - 1,
                                "link %u: vap_id:%u  vap_state:%u\n"
                                "ap_link_id:%u\n"
                                "protocol:[%s]  band: %s\n"
                                "bandwidth: %s  channel number: %u\n",
                                hmac_vap->mld_vap_link_info.link_id,
                                mac_vap->uc_vap_id, mac_vap->en_vap_state,
                                hmac_vap->mld_vap_link_info.link_ap_info.link_id,
                                hmac_config_protocol2string(mac_vap->en_protocol),
                                hmac_config_band2string(mac_vap->st_channel.en_band),
                                hmac_config_bw2string((mac_vap->en_protocol <= WLAN_MIXED_TWO_11G_MODE) ?
                                WLAN_BAND_WIDTH_20M : mac_vap->st_channel.en_bandwidth),
...
OAL_STATIC uint32_t hmac_config_vap_mld_info(hmac_vap_stru *ml_hmac_vap, hmac_mld_manage_stru *mld_vap_manage)
{
    ...
    print_buff = (int8_t *)oal_mem_alloc_m(OAL_MEM_POOL_ID_LOCAL, OAM_REPORT_MAX_STRING_LEN, OAL_TRUE);
    if (print_buff == NULL) {
        oam_error_log0(ml_hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_CFG,
            "{hmac_config_vap_mld_info::print_buff null.}");
        return OAL_ERR_CODE_PTR_NULL;
    }
    memset_s(print_buff, OAM_REPORT_MAX_STRING_LEN, 0, OAM_REPORT_MAX_STRING_LEN);
    ...
    string_tmp_len = snprintf_s(print_buff + string_len, ((OAM_REPORT_MAX_STRING_LEN - string_len) - 1),
        ...
        "mld_assoc_mode: %s\n"
        "link_cnt: %u\n",
        hmac_config_mld_assoc_mode2string(mld_vap_manage->mld_assoc_mode), mld_vap_manage->mld_link_cnt);
    if (string_tmp_len < 0) {
        goto sprint_fail;
    }
    string_len += (uint32_t)string_tmp_len;

    string_tmp_len = hmac_print_mld_link_info(ml_hmac_vap, print_buff, string_len);
    if (string_tmp_len < 0) {
        goto sprint_fail;
    }
    string_len += (uint32_t)string_tmp_len;

    link_hmac_vap = hmac_mld_get_link_vap(ml_hmac_vap);
    string_tmp_len = hmac_print_mld_link_info(link_hmac_vap, print_buff, string_len);
    if (string_tmp_len < 0) {
        goto sprint_fail;
    }
    string_len += (uint32_t)string_tmp_len;
...
uint32_t hmac_config_vaps_info(mac_vap_stru *mac_vap, uint16_t len, uint8_t *param)
{
#ifdef _PRE_WLAN_FEATURE_ML
    ...
    /* 非MLD vap只打印对应单vap的信息 */
    if (hmac_vap_is_ml_mode(hmac_vap) != OAL_TRUE) {
        return hmac_config_vap_info(mac_vap, len, param);
    }
    ...
    /* 遍历mld vap管理结构体下的link vap，打印全部vap信息 */
    for (link_id = 0; link_id < mld_vap_manage->mld_link_cnt; link_id++) {
        ...
        /* 打印辅link vap信息 */
        ret = hmac_config_vap_info(&(link_hmac_vap->st_vap_base_info), len, param);
        if (ret != OAL_SUCC) {
            return ret;
        }
    }
    ...
    /* 打印主link vap信息 */
    ml_hmac_vap = hmac_mld_get_master_link_vap(hmac_vap);
    if (ml_hmac_vap == NULL) {
        return OAL_FAIL;
    }
    ret = hmac_config_vap_info(&(ml_hmac_vap->st_vap_base_info), len, param);
    if (ret != OAL_SUCC) {
        return ret;
    }

    return hmac_config_vap_mld_info(ml_hmac_vap, mld_vap_manage);
 ...
 *   修改内容  : 新生成函数
 */
uint32_t hmac_config_user_info(mac_vap_stru *mac_vap, uint16_t len, uint8_t *param)
{
    ...
    hmac_event = (mac_cfg_user_info_param_stru *)param;
    ...
    /***************************************************************************
        抛事件到DMAC层, 同步DMAC数据
    ***************************************************************************/
    ret = hmac_config_send_event(mac_vap, WLAN_CFGID_USER_INFO, len, param);
    if (oal_unlikely(ret != OAL_SUCC)) {
...
#ifdef _PRE_WLAN_FEATURE_ML
uint32_t hmac_config_vap_ml_info_syn(hmac_vap_stru *hmac_vap)
{
    mac_h2d_vap_ml_info_stru mac_h2d_vap_ml_info = { 0 };
    uint32_t ret;

    if (hmac_vap == NULL) {
        oam_error_log0(0, OAM_SF_CFG, "hmac_config_vap_ml_info_syn: hmac_vap is NULL");
        return OAL_ERR_CODE_PTR_NULL;
    }
    ...
    mac_h2d_vap_ml_info.ap_link_id = hmac_vap->mld_vap_link_info.link_ap_info.link_id;

    /***************************************************************************
        抛事件到DMAC层, 同步USER最新ML信息到DMAC
    ***************************************************************************/
    ret = hmac_config_send_event(&hmac_vap->st_vap_base_info, WLAN_CFGID_VAP_ML_INFO_SYN,
                                 sizeof(mac_h2d_vap_ml_info), (uint8_t *)(&mac_h2d_vap_ml_info));
    if (oal_unlikely(ret != OAL_SUCC)) {
        oam_error_log2(hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_CFG,
            "hmac_config_vap_ml_info_syn: hmac_config_send_event failed, ret=%u, link_id=%u",
            ret, hmac_vap->mld_vap_link_info.link_id);
        return OAL_FAIL;
    }
...
oal_bool_enum_uint8 hmac_check_power_off_ready(void)
{
    return g_power_off_ready;
}

void hmac_set_power_off_ready(oal_bool_enum_uint8 is_power_off_ready)
{
    g_power_off_ready = is_power_off_ready;
    return;
}

uint32_t hmac_d2h_power_off_ready_event_process(frw_event_mem_stru *event_mem)
{
    hmac_set_power_off_ready(OAL_TRUE);
    if (g_wlan_spec_cfg->power_off_ready_support) {
        oal_wait_queue_wake_up_interrupt(&g_power_off_ready_wait_queue);
    }
    return OAL_SUCC;
}
