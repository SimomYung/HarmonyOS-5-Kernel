...
/* 3 函数实现 */
OAL_STATIC void hmac_device_scan_exit(hmac_device_stru *hmac_device)
{
    hmac_scan_exit(&(hmac_device->st_scan_mgmt));
#ifdef _PRE_WLAN_FEATURE_ELP
    hmac_scan_exit(hmac_get_wfsc_scan_mgmt());
#endif
}
/*
...
uint32_t hmac_device_exit(mac_board_stru *pst_board, mac_chip_stru *pst_chip, hmac_device_stru *pst_hmac_device)
{
    ...
    /* 扫描模块去初始化 */
    hmac_device_scan_exit(pst_hmac_device);
 ...
 *   修改内容  : 新生成函数
 */
void hmac_clear_vap_stru(hmac_vap_stru *hmac_vap)
{
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(5, 10, 0))
    size_t offset_query_wait_q = OAL_OFFSET_OF(hmac_vap_stru, query_wait_q);
    size_t sizeof_query_wait_q;

    if (hmac_vap == NULL) {
        return;
    }
    sizeof_query_wait_q = sizeof(hmac_vap->query_wait_q);
    if (hmac_vap->query_wait_q.head.next != &(hmac_vap->query_wait_q.head) ||
        hmac_vap->query_wait_q.head.prev != &(hmac_vap->query_wait_q.head)) {
        // 清零query_wait_q之前的内存
        memset_s(hmac_vap, offset_query_wait_q, 0, offset_query_wait_q);
        // 清零query_wait_q之后的内存
        memset_s(((uint8_t*)hmac_vap) + sizeof_query_wait_q + offset_query_wait_q,
            sizeof(hmac_vap_stru) - offset_query_wait_q - sizeof_query_wait_q, 0,
            sizeof(hmac_vap_stru) - offset_query_wait_q - sizeof_query_wait_q);
    } else {
    ...
    mac_cfg_suspend_stru *suspend_param, uint8_t uc_in_suspend)
{
    ...
    suspend_param->force_sleep = g_force_sleep_switch;
...
static oal_bool_enum hmac_is_device_pm_swith_on(void)
{
    if (g_wlan_device_pm_switch == WLAN_PM_ALL_ENABLE || g_wlan_device_pm_switch == WLAN_PM_LIGHT_SLEEP_SWITCH_EN ||
        g_wlan_device_pm_switch == WLAN_PM_ALL_ENABLE_HW_RX_BCN_EN) {
        return OAL_TRUE;
    }
    return OAL_FALSE;
}
    ...
    uint8_t *puc_evt, uint32_t evt_len)
{
    ...
    if (memcpy_s((void *)pst_event->auc_event_data, evt_len, (void *)puc_evt, evt_len) != EOK) {
...
uint32_t hmac_config_host_dev_exit(hmac_device_stru *hmac_device)
{
    if (oal_unlikely(hmac_device == NULL)) {
        oam_error_log0(0, OAM_SF_ANY, "{hmac_config_host_dev_exit::pst_hmac_device is null!}");
        return OAL_ERR_CODE_PTR_NULL;
    }
...
static void hmac_device_init_roam_blacklist(hmac_device_stru *hmac_device)
{
    uint32_t i = 0;
    for (i = 0; i < ROAM_LIST_MAX; i++) {
        hmac_device->st_blacklist.ast_bss[i].us_count_limit = ROAM_BLACKLIST_COUNT_LIMIT;
        hmac_device->st_blacklist.ast_bss[i].timeout = ROAM_BLACKLIST_NORMAL_AP_TIME_OUT;
    }
}
...
OAL_STATIC void hmac_device_init_ext(hmac_device_stru *pst_hmac_device)
{
    ...
    /* 扫描模块初始化 */
    hmac_scan_init(&(pst_hmac_device->st_scan_mgmt));
#ifdef _PRE_WLAN_FEATURE_ELP
    hmac_scan_init(hmac_get_wfsc_scan_mgmt());
#endif
    /* 初始化P2P 等待队列 */
    ...
    oal_wait_queue_init_head(&(pst_hmac_device->lowpower_stat_query.wait_queue));
    if (g_wlan_spec_cfg->power_off_ready_support) {
        oal_wait_queue_init_head(&g_power_off_ready_wait_queue);
    }
#ifdef _PRE_WLAN_TCP_OPT
...
#endif
#ifdef _PRE_WLAN_FEATURE_TCP_ACK_BUFFER
    hmac_tcp_ack_buf_init_para(pst_hmac_device);
#endif
    ...
    hmac_device_init_roam_blacklist(pst_hmac_device);
}

OAL_STATIC void hmac_cfg_vap_net_dev_info_sync(uint8_t cfg_vap_idx)
{
    hmac_vap_stru *hmac_cfg_vap = mac_res_get_hmac_vap(cfg_vap_idx);
    if (hmac_cfg_vap == NULL) {
        return;
    }
    dpe_wlan_cfg_vap_net_dev_set(cfg_vap_idx, hmac_cfg_vap->pst_net_device);
}
/*
 * 函 数 名  : hmac_device_init
...
uint32_t hmac_device_init(uint8_t *puc_device_id, mac_chip_stru *pst_chip)
{
    ...
    if (oal_unlikely(pst_hmac_device == NULL)) {
        oam_error_log1(0, OAM_SF_ANY, "{hmac_device_init::pst_hmac_device[%d] null!}", uc_dev_id);
        return OAL_ERR_CODE_PTR_NULL;
    }
...
#endif
    dpe_wlan_device_init(pst_mac_device->uc_device_id, pst_mac_device->uc_cfg_vap_id, pst_mac_device->uc_chip_id);
    hmac_cfg_vap_net_dev_info_sync(pst_mac_device->uc_cfg_vap_id);
    return OAL_SUCC;
}
...
uint32_t hmac_board_init(mac_board_stru *pst_board)
{
    ...
    for (chip = 0; chip < chip_max_num; chip++) {
        ret = hmac_chip_init(&pst_board->ast_chip[chip], chip);
        if (ret != OAL_SUCC) {
        ...
        oal_bit_set_bit_one_byte(&pst_board->uc_chip_id_bitmap, chip);
    }
    return OAL_SUCC;
}
...
void hmac_device_create_random_mac_addr(mac_device_stru *pst_mac_dev, mac_vap_stru *pst_mac_vap)
{
    ...
    oam_warning_log3(pst_mac_vap->uc_vap_id, OAM_SF_SCAN,
                     "{hmac_device_create_random_mac_addr::rand_mac_addr[%02X:XX:XX:XX:%02X:%02X].}",
                     pst_hmac_dev->st_scan_mgmt.auc_random_mac[0],
                     pst_hmac_dev->st_scan_mgmt.auc_random_mac[MAC_ADDR_4],
                     pst_hmac_dev->st_scan_mgmt.auc_random_mac[MAC_ADDR_5]);
