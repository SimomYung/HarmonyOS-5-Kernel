...
{
    return g_st_mac_device_custom_cfg.custom_cfg_11be.emlsr_switch == OAL_TRUE;
}
...
static void hmac_ml_vap_init(hmac_vap_stru *hmac_vap, oal_net_device_stru *mld_net_dev, uint8_t mld_mode)
{
    ...
    hmac_vap->mld_vap_link_info.emlsr_cap.emlsr_support =
        (hmac_mld_enable_eml_cap() == OAL_TRUE) ? OAL_TRUE : OAL_FALSE;
    hmac_vap->mld_vap_link_info.emlsr_cap.emlsr_padding_delay = wlan_chip_get_emlsr_padding_delay();
    hmac_vap->mld_vap_link_info.emlsr_cap.emlsr_transition_delay = wlan_chip_get_emlsr_transation_delay();
...
static void hmac_proc_mld_vap_deinit(hmac_vap_stru *hmac_vap, uint8_t mld_mode)
{
    ...
    if (mld_vap_manage->mld_link_cnt == 0) {
        if (mld_vap_manage->link_switch_record.link_switch_wait_timer.en_is_registerd == OAL_TRUE) {
            oam_error_log0(hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_CFG,
                "{hmac_proc_mld_vap_deinit::link_switch_wait_timer is still on!!}");
...
void hmac_vap_ml_to_legacy_mode_change(hmac_vap_stru *hmac_vap, uint8_t mld_vap_id, uint8_t mld_change_fsm_event)
{
    ...
    hmac_vap_set_mld_mode(hmac_vap, new_mld_mode);
    ...
    mld_change_fsm_event_type_enum mld_change_event)
{
    ...
    oal_net_device_stru *ml_net_dev = net_dev_priv->mlo_cfg_info.ml_netdev;
    oal_netdev_priv_stru *ml_net_dev_priv = (oal_netdev_priv_stru *)oal_net_dev_wireless_priv(ml_net_dev);
    ...
    hmac_mld_manage_stru *mld_vap_manage)
{
    ...
    hmac_user_change_tid_info_user_id(cur_work_user, next_work_user);
    ...
    hmac_mld_manage_stru *mld_vap_manage)
{
    ...
    ret = hmac_report_channel_switch(next_work_vap, &(next_work_vap->st_vap_base_info.st_channel));
#ifdef CONFIG_HISI_WIFI7_MLO
    oal_inform_assoc_link_id(netdev, LINK_SWITCH_EVENT);
#endif

    if (ret != OAL_SUCC) {
        oam_warning_log1(next_work_vap->st_vap_base_info.uc_vap_id, OAM_SF_ASSOC,
            "hmac_mlsr_link_switch_post_action::return fail is[%d]", ret);
    }
}
...
uint32_t hmac_mld_delete_link(mac_vap_stru *mac_vap)
{
    ...
    another_hmac_vap = hmac_mld_get_link_vap(hmac_vap);
    ...
    ml_link_switch_post_event_stru *mlsr_event)
{
    ...
    mld_vap_manage = cur_work_vap->mld_vap_link_info.mld_vap_manage;
    if (mld_vap_manage == NULL) {
        oam_warning_log0(0, OAM_SF_MLD, "{hmac_mlsr_link_switch_post_event_process::mld_vap_manage NULL}");
        return OAL_FALSE;
    }
    ...
    if (mld_vap_manage->link_switch_record.is_link_switching == OAL_FALSE) {
        oam_warning_log0(cur_work_vap->st_vap_base_info.uc_vap_id, OAM_SF_SCAN,
            "{hmac_mlsr_link_switch_post_event_process::not in link_switching, return}");
        return OAL_FALSE;
    }
...
uint32_t hmac_mlsr_link_switch_post_event_process(frw_event_mem_stru *event_mem)
{
    ...
    hmac_mld_destroy_mlsr_link_switch_timer(cur_work_vap);
    ...
    return hmac_mlsr_link_switch_post_proc(cur_work_vap, next_work_vap, switch_info->reason, switch_info->sub_reason);
#else
    return OAL_SUCC;
#endif
}
