...
#define ML_LINK_SWITCH_WAIT_DEAUTH_TIME 90000 /* link切换后1.5分钟内收到ap deauth，则执行重关联 */
...
typedef struct {
    ...
    uint8_t omn_retry_cnt;
    frw_timeout_stru omn_retry_timer; /* omn重传定时器 */
    frw_timeout_stru emlsr_open_timer; /* emlsr进入超时定时器 */
...
typedef struct hmac_mld_manage {
    ...
    hmac_mld_emlsr_cap_stru     emlsr_cap;          /* sta自身emlsr能力 */
    mac_emlsr_record_info       emlsr_record_info;  /* emlsr进入退出流程状态、参数记录 */
