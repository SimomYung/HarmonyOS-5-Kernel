...
hmac_user_ba_stru g_hmac_user_ba_res[WLAN_ASSOC_USER_MAX_NUM];
uint32_t g_user_ba_res_bitmap = 0;

void hmac_ba_res_init(void)
{
    memset_s(g_hmac_user_ba_res, sizeof(hmac_user_ba_stru) * WLAN_ASSOC_USER_MAX_NUM,
        0, sizeof(hmac_user_ba_stru) * WLAN_ASSOC_USER_MAX_NUM);
}

OAL_STATIC hmac_user_ba_stru *hmac_user_alloc_ba_res(hmac_user_stru *hmac_user)
{
#ifdef _PRE_WLAN_FEATURE_ML
    hmac_user_stru *master_user = NULL;

    if (hmac_user->mld_user_link_info.mld_user_manage == NULL) {
        return NULL;
    }
    master_user = mac_res_get_hmac_user(hmac_user->mld_user_link_info.mld_user_manage->ml_user_id);
    if (master_user == NULL) {
        return NULL;
    }
    if (master_user->ba_info != NULL) {
        return master_user->ba_info;
    }
#endif
    return NULL;
}

OAL_STATIC hmac_user_ba_stru *hmac_alloc_ba_res(hmac_user_stru *hmac_user)
{
    uint8_t tid_res_id;
    uint8_t loop;
    hmac_user_ba_stru *ba_info = hmac_user_alloc_ba_res(hmac_user);

    if (ba_info != NULL) {
        oam_warning_log3(0, 0, "hmac_user_tid_init::res_id %d user_id %d bitmap 0x%x",
            ba_info->tid_res_id, hmac_user->st_user_base_info.us_assoc_id, g_user_ba_res_bitmap);
        return ba_info;
    }

    tid_res_id = oal_bit_find_first_zero_four_byte(g_user_ba_res_bitmap);
    if (tid_res_id >= WLAN_ASSOC_USER_MAX_NUM) {
        oam_error_log1(0, 0, "hmac_alloc_tid_res:: tid_info_res_id[%d] error", tid_res_id);
        return NULL;
    }

    oal_bit_set_bit_four_byte(&g_user_ba_res_bitmap, (oal_bitops)tid_res_id);

    ba_info = &g_hmac_user_ba_res[tid_res_id];
    if (ba_info->work_flag != 0) {
        oam_error_log2(0, 0, "hmac_alloc_tid_res:: res_id[%d] work_flag[%d] error", tid_res_id, ba_info->work_flag);
    }

    for (loop = 0; loop < WLAN_TID_MAX_NUM; loop++) {
        if (ba_info->ba_info[loop].st_ba_tx_info.st_addba_timer.en_is_registerd == OAL_TRUE) {
            frw_timer_immediate_destroy_timer_m(&(ba_info->ba_info[loop].st_ba_tx_info.st_addba_timer));
        }
    }

    memset_s(ba_info, sizeof(hmac_user_ba_stru), 0, sizeof(hmac_user_ba_stru));
    ba_info->tid_res_id = tid_res_id;

    oam_warning_log3(0, 0, "hmac_alloc_ba_res::res_id %d user_id %d bitmap 0x%x",
        tid_res_id, hmac_user->st_user_base_info.us_assoc_id, g_user_ba_res_bitmap);
    return ba_info;
}
    ...
    mac_ap_type_enum_uint16 ap_type)
{
    ...
    /* 申请user的lut index */
    lut_idx = mac_user_get_lut_index(mac_chip->st_lut_table.auc_ra_lut_index_table, 0, WLAN_ASSOC_USER_MAX_NUM);
...
{
    uint32_t payload_len = payload_len_temp;
    if (reass_flag) {
...
hmac_ba_stru *hmac_get_ba_info(hmac_user_stru *hmac_user, uint8_t tid)
{
#ifdef _PRE_WLAN_FEATURE_ML
    hmac_user_stru *master_user = NULL;
    ...
    /* 非MLD, 使用自己的资源 */
    if (hmac_user->mld_user_link_info.mld_user_manage == NULL) {
        return &hmac_user->ba_info->ba_info[tid];
    }
    ...
    return &master_user->ba_info->ba_info[tid];
#else
    return &hmac_user->ba_info->ba_info[tid];
#endif
}

OAL_STATIC void hmac_ba_info_init(hmac_user_stru *hmac_user)
{
    hmac_ba_stru *tid_ba = NULL;
    hmac_ba_tx_stru *tx_ba = NULL;
    uint8_t tid;

    hmac_user->ba_info = hmac_alloc_ba_res(hmac_user);
    if (oal_unlikely(hmac_user->ba_info == NULL)) {
        oam_error_log1(0, 0, "hmac_ba_info_init::user[%d] tid res null",
            hmac_user->st_user_base_info.us_assoc_id);
        return;
    }

    if (hmac_user->ba_info->work_flag != 0) { // 说明已初始化
        hmac_user->ba_info->work_flag++;
        return;
    }
    hmac_user->ba_info->work_flag++;

    for (tid = 0; tid < WLAN_TID_MAX_NUM; tid++) {
        tid_ba = hmac_get_ba_info(hmac_user, tid);

        tid_ba->en_ba_handle_tx_enable = OAL_TRUE;
        tid_ba->en_ba_handle_rx_enable = OAL_TRUE;

        oal_spin_lock_init(&(tid_ba->st_ba_tx_info.st_ba_status_lock));
        tid_ba->st_ba_tx_info.en_ba_status = DMAC_BA_INIT;
        tid_ba->st_ba_tx_info.uc_addba_attemps = 0;
        tid_ba->st_ba_tx_info.uc_dialog_token = 0;
        tid_ba->st_ba_tx_info.uc_ba_policy = 0;

        /* addba req超时处理函数入参填写 */
        tx_ba = &tid_ba->st_ba_tx_info;
        ...
        tid_ba->en_ba_status = DMAC_BA_INIT;
        tid_ba->uc_lut_index = MAC_INVALID_RX_BA_LUT_INDEX;
    }
}

void hmac_ba_info_deinit(hmac_user_stru *hmac_user)
{
    hmac_user_ba_stru *ba_info = hmac_user->ba_info;

    if (ba_info == NULL) {
        oam_error_log0(0, 0, "hmac_user_tid_res_deinit:: work flag error!");
        return;
    }
    ba_info->work_flag = (ba_info->work_flag == 0) ? 0 : ba_info->work_flag - 1;

    /* 所有user都已释放该资源，对应bitmap可置为0 */
    if (ba_info->work_flag == 0) {
        oal_bit_clear_bit_four_byte(&g_user_ba_res_bitmap, (oal_bitops)ba_info->tid_res_id);
    }
}
...
uint32_t hmac_user_init(hmac_user_stru *hmac_user)
{
    ...
    /* 初始化ba信息 */
    hmac_ba_info_init(hmac_user);

    /* 初始化tid信息 */
    hmac_user_tid_init(hmac_user->st_user_base_info.us_assoc_id);

#ifdef _PRE_WLAN_FEATURE_MCAST_AMPDU
    if (hmac_vap == NULL) {
        return OAL_ERR_CODE_PTR_NULL;
    }
    hmac_user->is_mcast_ampdu = mac_get_mcast_ampdu_switch();
    dpe_wlan_user_set_mcast_ampdu(hmac_user->st_user_base_info.us_assoc_id, hmac_user->is_mcast_ampdu);
    /* 初始化聚合组播 rx_ba 信息 */
    if ((hmac_vap->st_vap_base_info.en_vap_mode == WLAN_VAP_MODE_BSS_STA) &&
        (hmac_user->is_mcast_ampdu == OAL_TRUE)) {
        ba_rx_info.us_baw_start = 0;
        ba_rx_info.us_baw_size = WLAN_AMPDU_RX_BUFFER_SIZE;
        ba_rx_info.uc_lut_index = 0;
        dpe_wlan_rx_ba_init(&ba_rx_info, hmac_vap->st_vap_base_info.uc_vap_id,
            hmac_user->st_user_base_info.us_assoc_id, WLAN_TIDNO_BCAST);
    }
#endif
...
OAL_STATIC uint32_t hmac_user_set_num_spatial_stream_vht_capable(mac_user_stru *pst_mac_user,
    ...
    if (pst_mac_vht_hdl->st_rx_max_mcs_map.us_max_mcs_4ss != MAX_MCS_3) {
        ...
        *pen_user_num_spatial_stream = WLAN_SINGLE_NSS;
        ...
        oam_warning_log0(pst_mac_user->uc_vap_id, OAM_SF_ANY,
                         "{hmac_user_set_num_spatial_stream_vht_capable::invalid vht nss.}");
...
/*
 * 函 数 名  : mac_user_set_avail_nss_by_ie
 * 功能描述  : 获取用户的空间流数
 * 1.日    期  : 2023年10月31日
 *   修改内容  : 新生成函数，拆分大函数
 */
OAL_STATIC uint32_t mac_user_set_avail_nss_by_ie(mac_user_stru *mac_user, uint8_t *user_num_spatial_stream)
{
    mac_user_ht_hdl_stru *mac_ht_hdl;
    mac_vht_hdl_stru *mac_vht_hdl;
#ifdef _PRE_WLAN_FEATURE_11AX
    mac_he_hdl_stru *pst_mac_he_hdl = MAC_USER_HE_HDL_STRU(mac_user);
#endif
#ifdef _PRE_WLAN_FEATURE_11BE
    mac_eht_hdl_stru *mac_eht_hdl = &(mac_user->st_eht_hdl);
#endif

    mac_ht_hdl = &(mac_user->st_ht_hdl);
    mac_vht_hdl = &(mac_user->st_vht_hdl);
#ifdef _PRE_WLAN_FEATURE_11BE
    if (g_wlan_spec_cfg->feature_11be_is_open && mac_eht_hdl->en_eht_capable == OAL_TRUE) {
        return mac_user_set_avail_nss_eht(mac_user, mac_eht_hdl, user_num_spatial_stream);
    }
#endif

#ifdef _PRE_WLAN_FEATURE_11AX
    if (g_wlan_spec_cfg->feature_11ax_is_open && pst_mac_he_hdl->en_he_capable == OAL_TRUE) {
        return mac_ie_from_he_cap_get_nss(pst_mac_he_hdl, user_num_spatial_stream);
    }
#endif

    if (mac_vht_hdl->en_vht_capable == OAL_TRUE) {
        // 降低圈复杂度
        if (hmac_user_set_num_spatial_stream_vht_capable(mac_user, mac_vht_hdl,
            user_num_spatial_stream) != OAL_SUCC) {
            return OAL_FAIL;
        }
    }

    if (mac_ht_hdl->en_ht_capable == OAL_TRUE) {
        if (mac_ht_hdl->uc_rx_mcs_bitmask[BYTE_OFFSET_3] > 0) {
            *user_num_spatial_stream = WLAN_FOUR_NSS;
        } else if (mac_ht_hdl->uc_rx_mcs_bitmask[BYTE_OFFSET_2] > 0) {
            *user_num_spatial_stream = WLAN_TRIPLE_NSS;
        } else if (mac_ht_hdl->uc_rx_mcs_bitmask[BYTE_OFFSET_1] > 0) {
            *user_num_spatial_stream = WLAN_DOUBLE_NSS;
        } else if (mac_ht_hdl->uc_rx_mcs_bitmask[BYTE_OFFSET_0] > 0) {
            *user_num_spatial_stream = WLAN_SINGLE_NSS;
        } else {
            oam_warning_log0(mac_user->uc_vap_id, OAM_SF_ANY,
 ...
 *   修改内容  : 新生成函数
 */
uint32_t hmac_user_set_avail_num_space_stream(mac_user_stru *mac_user, wlan_nss_enum_uint8 vap_nss)
{
    wlan_nss_enum_uint8 user_num_spatial_stream = 0;
    ...
    ret = mac_user_set_avail_nss_by_ie(mac_user, &user_num_spatial_stream);

    /* 赋值给用户结构体变量 */
    mac_user_set_num_spatial_stream(mac_user, user_num_spatial_stream);
    mac_user_set_avail_num_spatial_stream(mac_user,
                                          oal_min(mac_user->en_user_max_cap_nss, vap_nss));
    mac_user_set_smps_opmode_notify_nss(mac_user, WLAN_NSS_LIMIT);
    ...
    uint8_t *puc_mac_addr)
{
    ...
    if ((pst_bss_dscr != NULL) &&
        (pst_bss_dscr->agc_compability_ap == OAL_TRUE) && (pst_bss_dscr->en_he_capable != OAL_TRUE)) {
        en_ap_type |= MAC_AP_TYPE_AGC_COMPABILITY_AP;
    }
...
void hmac_dbdc_need_kick_user(mac_vap_stru *pst_mac_vap, mac_device_stru *pst_mac_dev)
{
    ...
    if (pst_mac_dev->en_dbdc_running == OAL_FALSE ||
        hmac_vap_is_legacy_mld_mode(hmac_vap) == OAL_FALSE) {
        return;
    }
...
static void hmac_user_del_update_hal_lut(uint8_t hal_dev_id, uint8_t user_index)
{
    ...
    for (loop = 0; loop < HAL_MAX_LUT; loop++) {
        if (hal_chip->user_sts_info[loop].user_id == user_index) {
            hal_chip->user_sts_info[loop].user_valid = 0;
            oam_warning_log2(hal_dev_id, OAM_SF_ANY, "{hmac_update_hal_lut::lut[%d] user_index[%d]}",
                             loop, user_index);
            return;
        }
    }
    oam_warning_log1(hal_dev_id, OAM_SF_ANY, "{hmac_update_hal_lut::user_index[%d]}", user_index);
    return;
}
#endif
...
static void hmac_user_del_vap_init(hmac_vap_stru *hmac_vap, mac_device_stru *mac_device, hmac_user_stru *hmac_user)
{
    if (hmac_vap->st_vap_base_info.en_vap_mode == WLAN_VAP_MODE_BSS_STA) {
        ...
        hmac_mlsr_link_switch_deinit(hmac_vap);
        hmac_mld_destroy_mld_related_timer(hmac_vap);
        hmac_vap_mld_mode_change(hmac_vap, MLD_CHANGE_EVENT_CHANGE_TO_LEGACY, 0);
#endif
...
OAL_STATIC void hmac_sta_del_user(mac_device_stru *mac_device, hmac_vap_stru *hmac_vap)
{
    ...
    /* 如果正在进行长期同信道监听, sta/gc去关联后检查能否继续同信道监听, 如果不行则abor监听 */
    hmac_abort_long_same_chan_remain_on_chan(&(hmac_vap->st_vap_base_info), OAL_FALSE,
        MAC_ABORT_SAME_CHAN_REMAIN_ON_CHAN_DEL_USER);
    ...
    /* 如果是11r auth fail触发的漫游失败去关联，由于驱动不通知上层直接发起关联，ip标记被清无法再次发起漫游 */
    if (hmac_vap->bit_11r_auth_fail == OAL_FALSE) {
        hmac_roam_wpas_connect_state_notify_for_single_link(hmac_vap, WPAS_CONNECT_STATE_IPADDR_REMOVED, OAL_FALSE);
    }
    hmac_roam_exit(hmac_vap);
    hmac_vap_state_change_notify_feature(hmac_vap, NULL, COFEATURE_DEL_USER_EVENT, OAL_TRUE);
}
...
#endif
    hmac_user_tx_ring_deinit(hmac_user->st_user_base_info.us_assoc_id);
#if (_PRE_OS_VERSION_LINUX == _PRE_OS_VERSION)
...
OAL_STATIC void hmac_user_del_refresh_lut(mac_chip_stru *mac_chip, hmac_user_stru *hmac_user)
{
    ...
    mac_user_del_lut_index(mac_chip->st_lut_table.auc_ra_lut_index_table, hmac_user->lut_index);
    hmac_user->lut_index = WLAN_ASSOC_USER_MAX_NUM;
    oam_warning_log2(0, 0, "hmac_user_del_refresh_lut::del user[%d] lut_id[%d]",
        hmac_user->st_user_base_info.us_assoc_id, hmac_user->lut_index);
}
...
static void hmac_user_free_ml_lut(mac_vap_stru *mac_vap, hmac_user_stru *hmac_user, hmac_user_del_ctx *ctx,
    uint16_t user_index)
{
    if (hmac_user->ml_user_lut_idx >= WLAN_USER_MAX_USER_LIMIT) {
        oam_error_log1(mac_vap->uc_vap_id, OAM_SF_UM,
            "{hmac_user_free_ml_lut::too large ml_user_lut_idx[%d]", hmac_user->ml_user_lut_idx);
        return;
    }
    ctx->mac_chip->mld_lut_bitmap[hmac_user->ml_user_lut_idx] &= ~(uint32_t)BIT(user_index);
    if (ctx->mac_chip->mld_lut_bitmap[hmac_user->ml_user_lut_idx] == 0) {
        mac_user_del_lut_index(ctx->mac_chip->st_lut_table.ml_user_idx_table, hmac_user->ml_user_lut_idx);
    }
#ifdef _PRE_WLAN_FEATURE_ML
    oam_warning_log4(mac_vap->uc_vap_id, OAM_SF_UM,
        "hmac_user_free_ml_lut::del user[%d] ml_user_lut_id[%d] mld_mode[%d] mld_lut_bitmap[0x%x]",
        user_index, hmac_user->ml_user_lut_idx, ctx->hmac_vap->mld_vap_link_info.mld_mode,
        ctx->mac_chip->mld_lut_bitmap[hmac_user->ml_user_lut_idx]);
#endif
}
...
uint32_t hmac_user_del(mac_vap_stru *mac_vap, hmac_user_stru *hmac_user)
{
    ...
    oam_warning_log3(mac_vap->uc_vap_id, OAM_SF_MLOG, "{hmac_user_del::user mac:%02X:XX:XX:XX:%02X:%02X}",
    ...
    oam_warning_log3(mac_vap->uc_vap_id, OAM_SF_MLOG,
        "{hmac_user_del::del user[%d] start,is multi user[%d], rssi[%d]}",
        ctx.mac_user->us_assoc_id, ctx.mac_user->en_is_multi_user, hmac_user->c_rssi);
    ...
    hmac_tid_clear(mac_vap, hmac_user, OAL_FALSE);
    ...
    hmac_user_free_ml_lut(mac_vap, hmac_user, &ctx, user_index);

#ifdef _PRE_WLAN_CHBA_MGMT
    hmac_update_chba_vap_info(ctx.hmac_vap, hmac_user);
    hmac_user_del_clear_vap_user_bitmap(ctx.hmac_vap, user_index);
#endif
    hmac_ba_info_deinit(hmac_user);
    hmac_user_tid_res_deinit(hmac_user->st_user_base_info.us_assoc_id);
#ifdef _PRE_WLAN_FEATURE_ML
    hmac_mld_user_deinit(ctx.hmac_vap, hmac_user);
#endif
    /* 释放用户内存 */
 ...
 *   修改内容  : 新生成函数
 */
OAL_STATIC uint32_t hmac_user_add_post_event(mac_vap_stru *mac_vap, hmac_user_stru *hmac_user,
                                             uint8_t *mac_addr, uint16_t user_idx)
{
    ...
    add_user_payload->mld_lut_idx = hmac_user->ml_user_lut_idx;
    /* 非MLD时MLD MAC ADDR = user_mac_addr */
#ifdef _PRE_WLAN_FEATURE_ML
    if (hmac_user->mld_user_link_info.mld_user_manage != NULL) {
        oal_set_mac_addr(add_user_payload->user_mld_mac_addr, hmac_user->mld_user_link_info.mld_user_manage->mld_addr);
        add_user_payload->mld_man_id = hmac_user->mld_user_link_info.mld_user_manage->mld_user_man_id;
        add_user_payload->ml_user_id = hmac_user->mld_user_link_info.mld_user_manage->ml_user_id;
    } else {
        oal_set_mac_addr(add_user_payload->user_mld_mac_addr, mac_addr);
        add_user_payload->ml_user_id = 0xFF;
    }
    add_user_payload->mld_mode = hmac_user->mld_user_link_info.mld_mode;
#else
    add_user_payload->mld_mode = WLAN_LEGACY_MLD_MODE;
    oal_set_mac_addr(add_user_payload->user_mld_mac_addr, mac_addr);
#endif
    ...
    add_user_payload->c_rssi = (bss_dscr != NULL) ?
        bss_dscr->c_rssi : oal_get_real_rssi((int16_t)OAL_RSSI_INIT_MARKER);
...
static uint32_t hmac_user_sub_feature_init(hmac_vap_stru *hmac_vap, hmac_add_user_input_stru *user_info,
    hmac_user_stru *hmac_user, uint16_t user_idx)
{
    ...
    /* 初始化mac_user_stru */
    mac_user_init(&(hmac_user->st_user_base_info), user_idx, user_info->mac_addr,
...
uint32_t hmac_alloc_ml_user_lut_index(hmac_user_stru *hmac_user, mac_chip_stru *mac_chip)
{
    uint8_t ml_user_id;
    ...
    /* 申请ml user index */
    ml_user_id = mac_user_get_lut_index(mac_chip->st_lut_table.ml_user_idx_table, 0, WLAN_ASSOC_USER_MAX_NUM);
    if (ml_user_id >= mac_chip_get_max_active_user()) {
        oam_warning_log0(0, OAM_SF_ANY, "{hmac_alloc_ml_user_index::ml_user_id >= WLAN_ASSOC_USER_MAX_NUM.}");
        return OAL_FAIL;
    }

    hmac_user->ml_user_lut_idx = ml_user_id;

    return OAL_SUCC;
}
uint32_t hmac_user_alloc_ml_lut(hmac_vap_stru *hmac_vap, hmac_user_stru *hmac_user)
{
    uint32_t ret;
    mac_chip_stru *mac_chip = hmac_res_get_mac_chip(hmac_vap->st_vap_base_info.uc_chip_id);

#ifdef _PRE_WLAN_FEATURE_ML
    if (hmac_vap_is_link_mode(hmac_vap) == OAL_TRUE) {
        ret = hmac_mld_user_get_ml_user_lut_idx(hmac_vap, hmac_user, &(hmac_user->ml_user_lut_idx));
    } else {
        ret = hmac_alloc_ml_user_lut_index(hmac_user, mac_chip);
    }
    if (hmac_user->ml_user_lut_idx >= WLAN_USER_MAX_USER_LIMIT) {
        oam_error_log1(hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_UM,
            "{hmac_user_alloc_ml_lut::too large ml_user_lut_idx[%d]", hmac_user->ml_user_lut_idx);
        return OAL_FAIL;
    }
    mac_chip->mld_lut_bitmap[hmac_user->ml_user_lut_idx] |= (uint32_t)BIT(hmac_user->st_user_base_info.us_assoc_id);
    oam_warning_log4(hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_UM,
        "{hmac_user_alloc_ml_lut::user[%d] lut_id[%d] ml_user_lut_id[%d] mld_lut_bitmap[0x%x]}",
        hmac_user->st_user_base_info.us_assoc_id, hmac_user->lut_index, hmac_user->ml_user_lut_idx,
        mac_chip->mld_lut_bitmap[hmac_user->ml_user_lut_idx]);
#else
    ret = hmac_alloc_ml_user_lut_index(hmac_user, mac_chip);
#endif
    return ret;
...
/* 申请单播用户 */
OAL_STATIC hmac_user_stru *hmac_alloc_unicast_user(hmac_vap_stru *hmac_vap, uint8_t *mac_addr, uint16_t *user_index)
{
    uint32_t ret;
    uint16_t user_idx;
...
#endif
#ifdef _PRE_WLAN_FEATURE_ML
   hmac_ml_add_link_user(hmac_vap);
#endif
 ...
 *   修改内容  : 新生成函数
 */
uint32_t hmac_user_add(mac_vap_stru *mac_vap, hmac_add_user_input_stru *user_info, uint16_t *user_idx)
{
    hmac_vap_stru *hmac_vap = NULL;
    hmac_user_stru *hmac_user = NULL;
    uint32_t ret;
    ...
    if (oal_unlikely(oal_any_null_ptr3(mac_vap, user_info, user_idx))) {
        return OAL_ERR_CODE_PTR_NULL;
    }

    hmac_vap = (hmac_vap_stru *)mac_res_get_hmac_vap(mac_vap->uc_vap_id);
    mac_chip = hmac_res_get_mac_chip(mac_vap->uc_chip_id);
    ret = hmac_check_add_user_conditions(mac_vap, user_info->mac_addr);
    if (ret != OAL_SUCC) {
        return ret;
    }
    ...
    /* 获取hmac user */
    hmac_user = hmac_alloc_unicast_user(hmac_vap, user_info->mac_addr, user_idx);
    if (hmac_user == NULL) {
        oam_warning_log1(mac_vap->uc_vap_id, OAM_SF_UM, "{hmac_user_add::pst_hmac_user[%d] null}", *user_idx);
        return OAL_ERR_CODE_PTR_NULL;
    }

    if (hmac_user_sub_feature_init(hmac_vap, user_info, hmac_user, *user_idx) != OAL_SUCC) {
        hmac_user_free(*user_idx);
        return OAL_FAIL;
    }
    if (hmac_user_alloc_ml_lut(hmac_vap, hmac_user)) {
        hmac_user_free(*user_idx);
        return OAL_FAIL;
    }
    ...
    /***************************************************************************
        抛事件到DMAC层, 创建dmac用户
    ***************************************************************************/
    if (hmac_user_add_post_event(mac_vap, hmac_user, user_info->mac_addr, *user_idx) != OAL_SUCC) {
    ...
    /* add_user 获取完lut idx后，清理原本ucast rx pn计数 */
    hmac_user_clear_ptk_rx_pn(hmac_user);

    /* 添加用户到MAC VAP */
    if (mac_vap_add_assoc_user(mac_vap, *user_idx) != OAL_SUCC) {
    ...
    /* 打开80211单播管理帧开关，观察关联过程，关联成功了就关闭 */
    hmac_add_user_set_80211_mgmt_frame_features(mac_vap, user_info->mac_addr);

    hmac_vap->en_roam_prohibit_on = (hmac_user->en_user_ap_type & MAC_AP_TYPE_ROAM) ? OAL_TRUE : OAL_FALSE;

    hmac_user_add_feature_proc(hmac_vap, hmac_user, user_info->mac_addr, *user_idx);

    oam_warning_log4(mac_vap->uc_vap_id, OAM_SF_UM, "{hmac_user_add::user[%d] mac:%02X:XX:XX:XX:%02X:%02X}",
        *user_idx, user_info->mac_addr[MAC_ADDR_0], user_info->mac_addr[MAC_ADDR_4], user_info->mac_addr[MAC_ADDR_5]);
    return OAL_SUCC;
}
 ...
 *   修改内容  : 新生成函数
 */
uint32_t hmac_config_add_user(mac_vap_stru *mac_vap, uint16_t len, uint8_t *param)
{
    mac_cfg_add_user_param_stru *add_user = (mac_cfg_add_user_param_stru *)param;
    uint16_t user_idx = 0;
    uint32_t ret;
    hmac_vap_stru *hmac_vap = NULL;
    hmac_user_stru *hmac_user = NULL;
    ...
    hmac_vap = (hmac_vap_stru *)mac_res_get_hmac_vap(mac_vap->uc_vap_id);
    if (hmac_vap == NULL) {
        oam_warning_log0(0, OAM_SF_UM, "{hmac_config_add_user::hmac_vap null.}");
        return OAL_FAIL;
    }
    hmac_sta_fill_user_info(hmac_vap, add_user->auc_mac_addr, &user_info);
    ret = hmac_user_add(mac_vap, &user_info, &user_idx);
    if (ret != OAL_SUCC) {
        oam_warning_log1(mac_vap->uc_vap_id, OAM_SF_UM, "{hmac_config_add_user::hmac_user_add failed.}", ret);
        return ret;
    }

    hmac_user = (hmac_user_stru *)mac_res_get_hmac_user(user_idx);
    if (hmac_user == NULL) {
        oam_warning_log1(mac_vap->uc_vap_id, OAM_SF_UM, "{hmac_user[%d] null.}", user_idx);
        return OAL_ERR_CODE_PTR_NULL;
    }
    ...
    /* 设置HT域 */
    hmac_config_user_set_ht_hdl(mac_vap, hmac_user, add_user);
    /* 设置vht域 */
    hmac_config_user_set_vht_hdl(mac_vap, hmac_user, add_user);
    ...
    ***************************************************************************/
    /* 重新设置DMAC需要的参数 */
    add_user->us_user_idx = user_idx;

    ret = hmac_config_send_event(&hmac_vap->st_vap_base_info, WLAN_CFGID_ADD_USER, len, param);
    if (oal_unlikely(ret != OAL_SUCC)) {
        /* 异常处理，释放内存 */
        if (hmac_config_del_added_user(mac_vap, user_idx) != OAL_SUCC) {
            return OAL_ERR_CODE_PTR_NULL;
        }
        oam_warning_log1(mac_vap->uc_vap_id, OAM_SF_UM, "{hmac_config_send_event failed[%d].}", ret);
        return ret;
    }
    /* 设置、同步速率、带宽、空间流信息 */
    hmac_config_user_sync_info(hmac_user, hmac_vap, mac_vap, user_idx);

    if (is_legacy_vap(mac_vap)) { //  添加用户后，对于P2P 不需要修改vap 状态
        mac_vap_state_change(mac_vap, MAC_VAP_STATE_UP);
    }
 ...
 *   修改内容  : 新生成函数
 */
uint32_t hmac_user_add_multi_user(mac_vap_stru *mac_vap, uint16_t *user_index, uint8_t *multi_user_lut_idx)
{
    uint32_t ret;
    uint16_t us_user_index;
    ...
    *user_index = us_user_index;
    ...
    /* 组播聚合场景初始化tid、ring信息 */
    if (mac_get_mcast_ampdu_switch() == OAL_TRUE) {
        hmac_mcast_ampdu_user_tid_init(mac_vap, pst_hmac_user, multi_user_lut_idx);
    }
#endif
...
#endif
#ifdef _PRE_WLAN_FEATURE_PWL
    if (pst_mac_vap->pwl_enable == OAL_TRUE) {
...
static void hmac_user_add_sync_hdl(hmac_user_stru *pst_hmac_user, dmac_ctx_add_user_stru *add_user_payload)
{
    mac_user_get_vht_hdl(&pst_hmac_user->st_user_base_info, &add_user_payload->st_vht_hdl);
    mac_user_get_ht_hdl(&pst_hmac_user->st_user_base_info, &add_user_payload->st_ht_hdl);
#ifdef _PRE_WLAN_FEATURE_11AX
    if (g_wlan_spec_cfg->feature_11ax_is_open) {
        /* he速率集信息，同步到dmac */
        mac_user_get_he_hdl(&pst_hmac_user->st_user_base_info, &(add_user_payload->he_hdl));
    }
#endif
#ifdef _PRE_WLAN_FEATURE_11BE
    if (g_wlan_spec_cfg->feature_11be_is_open) {
        /* eht速率集信息，同步到dmac */
        mac_user_get_eht_hdl(&pst_hmac_user->st_user_base_info, &(add_user_payload->eht_hdl));
    }
#endif
}
/*
...
uint32_t hmac_user_add_notify_alg(mac_vap_stru *pst_mac_vap, uint16_t us_user_idx)
{
    ...
    add_user_payload = (dmac_ctx_add_user_stru *)pst_event->auc_event_data;
    add_user_payload->us_user_idx = us_user_idx;
    add_user_payload->us_sta_aid = pst_mac_vap->us_sta_aid;
    ...
    hmac_user_add_sync_hdl(pst_hmac_user, add_user_payload);
    /* 填充事件头 */
    frw_event_hdr_init(&(pst_event->st_event_hdr),
                       FRW_EVENT_TYPE_WLAN_CTX, DMAC_WLAN_CTX_EVENT_SUB_TYPE_NOTIFY_ALG_ADD_USER,
                       sizeof(dmac_ctx_add_user_stru), FRW_EVENT_PIPELINE_STAGE_1,
...
void hmac_user_cap_update_by_hisi_cap_ie(hmac_user_stru *pst_hmac_user, mac_ies_info *ies_info)
{
    mac_hisi_cap_vendor_ie_stru st_hisi_cap_ie = {0};

    if (mac_get_hisi_cap_vendor_ie(ies_info->frame_ies, ies_info->frame_len, &st_hisi_cap_ie) != OAL_SUCC) {
        return;
    }
#ifdef _PRE_WLAN_FEATURE_11AX
    ...
    oam_warning_log3(0, OAM_SF_SCAN,
        "hmac_user_cap_update_by_hisi_cap_ie::he_ltf_repeat=%d, 11b_plus_cap=%d hisi_priv_cap=0x%x",
        pst_hmac_user->st_user_base_info.hisi_priv_cap.bit_he_ltf_repeat,
        pst_hmac_user->st_user_base_info.hisi_priv_cap.bit_11b_plus_cap,
        *((uint32_t *)&st_hisi_cap_ie));
}
