...
typedef struct {
    ...
    uint8_t user_ps_service_type_mode; /* 要求修改user低功耗状态的特殊业务标记 */
    uint8_t resv[3];
...
typedef struct {
    ...
    dmac_ba_conn_status_enum_uint8 en_ba_status; /* 该TID对应的BA会话的状态 rx ba使用 */
    uint8_t uc_lut_index;                        /* LUT索引 rx ba使用 */
...
typedef struct {
    uint8_t tid_res_id;
    uint8_t work_flag;
    hmac_ba_stru ba_info[WLAN_TID_MAX_NUM];
} hmac_user_ba_stru;
...
typedef struct hmac_user_tag {
    ...
    oal_bool_enum_uint8 is_recv_deauth_disassoc;  /* 当前用户是否收到了deauth/disassoc */
    uint8_t resv;
...
extern hmac_tid_stru g_hmac_tid_res[WLAN_ASSOC_USER_MAX_NUM];
