...
typedef struct {
    ...
    uint8_t en_ba_handle_tx_enable; /* 该tid下发送方向是否允许建BA，配置命令需求 */
    uint8_t en_ba_handle_rx_enable; /* 该tid下接送方向是否允许建BA，配置命令需求 */
...
typedef struct {
    uint8_t mac_addr[WLAN_MAC_ADDR_LEN];      /* user mac地址 */
    uint8_t mld_mac_addr[WLAN_MAC_ADDR_LEN]; /* user mld mac地址 */
} hmac_add_user_input_stru;
...
typedef struct {
    ...
    uint8_t conn_flag; /* 标记连接模式 */
    mac_user_asoc_state_enum_uint8 last_asoc_state; /* 用于保存上一次用户关联状态, 极速连接响应方使用 */
...
typedef struct {
    ...
    uint32_t last_tsf;
    uint32_t trx_tsf_interval[CHBA_THROUGHPUT_WINDOW_LEN];
    uint8_t peer_cnt;
    uint8_t trx_windows_cnt; /* 流量窗口中保存的有吞吐的周期计数 */
    uint8_t high_trx_windows_cnt; /* 流量窗口中保存的有高吞吐的周期计数 */
    uint8_t large_tsf_interval_cnt; /* 窗口中保存的大时间间隔的采样周期计数 */
    uint8_t resv;
...
#define SUPPORTED_CHANNEL_NUM_MAX_CNT 35 /* 当前仅考虑2.4G和5G的情况，后续6G需要修改实现为bitmap */
typedef struct {
    uint8_t num;
...
typedef struct hmac_user_tag {
    ...
    hmac_tid_res_stru *tid_res;
    /* 此项变量仅能处于HMAC USER结构体内的最后一项 */
    mac_user_stru st_user_base_info; /* hmac user与dmac user公共部分 */
} hmac_user_stru;

static inline hmac_tid_ba_stru *hmac_get_tid_ba_info(hmac_user_stru *hmac_user, uint8_t tid)
{
    uint8_t is_mcast_ampdu = OAL_FALSE;
#ifdef _PRE_WLAN_FEATURE_MCAST_AMPDU
    is_mcast_ampdu = mac_get_mcast_ampdu_switch() == OAL_TRUE;
#endif
    /* 不支持组播聚合的组播用户 没有ring和ba */
    if ((hmac_user->st_user_base_info.en_is_multi_user == OAL_TRUE) && is_mcast_ampdu == OAL_FALSE) {
        return NULL;
    }
    if (hmac_user->tid_res == NULL) {
        oam_error_log1(0, 0, "hmac_get_tid_ba_info::tid res null user[%d]", hmac_user->st_user_base_info.us_assoc_id);
        return NULL;
    }
    return &hmac_user->tid_res->tid_ba[tid];
}

static inline hmac_tid_info_stru *hmac_get_tx_tid_info(hmac_user_stru *hmac_user, uint8_t tid)
{
    uint8_t is_mcast_ampdu = OAL_FALSE;
#ifdef _PRE_WLAN_FEATURE_MCAST_AMPDU
    is_mcast_ampdu = mac_get_mcast_ampdu_switch() == OAL_TRUE;
    ...
    if (hmac_user->tid_res == NULL) {
        oam_error_log1(0, 0, "hmac_get_tx_tid_info::tid res null user[%d]", hmac_user->st_user_base_info.us_assoc_id);
        return NULL;
    }
    return &hmac_user->tid_res->tid_info[tid];
}
...
#ifdef _PRE_WLAN_FEATURE_ML
OAL_STATIC OAL_INLINE uint8_t hmac_user_is_single_ml_mode(hmac_user_stru *hmac_user)
{
    return hmac_user->mld_user_link_info.mld_mode == WLAN_ML_MODE;
}
OAL_STATIC OAL_INLINE uint8_t hmac_user_is_ml_mode(hmac_user_stru *hmac_user)
{
    return (hmac_user->mld_user_link_info.mld_mode == WLAN_ML_MODE ||
        hmac_user->mld_user_link_info.mld_mode == WLAN_ML_LINK_MODE);
}
OAL_STATIC OAL_INLINE uint8_t hmac_user_is_legacy_mld_mode(hmac_user_stru *hmac_user)
{
    return hmac_user->mld_user_link_info.mld_mode == WLAN_LEGACY_MLD_MODE;
}
OAL_STATIC OAL_INLINE uint8_t hmac_user_is_link_mode(hmac_user_stru *hmac_user)
{
    return hmac_user->mld_user_link_info.mld_mode == WLAN_LINK_MODE;
}
#else
OAL_STATIC OAL_INLINE uint8_t hmac_user_is_single_ml_mode(hmac_user_stru *hmac_user)
{
...
OAL_STATIC OAL_INLINE hmac_last_mcast_pn_stru *hmac_get_last_mcast_pn(hmac_user_stru *hmac_user)
{
    return &g_last_mcast_pn[hmac_user->lut_index];
}

OAL_STATIC OAL_INLINE hmac_last_ucast_pn_stru *hmac_get_last_ucast_pn(hmac_user_stru *hmac_user)
{
    return &g_last_ucast_pn[hmac_user->ml_user_lut_idx];
}
...
void hmac_user_set_amsdu_level_by_max_mpdu_len(hmac_user_stru *hmac_user, uint16_t max_mpdu_len);
void hmac_user_set_amsdu_level(hmac_user_stru *hmac_user, wlan_tx_amsdu_enum_uint8 amsdu_level);
#ifdef _PRE_WLAN_FEATURE_WAPI
