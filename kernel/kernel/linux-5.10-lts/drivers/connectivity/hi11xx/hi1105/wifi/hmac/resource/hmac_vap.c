...
#if defined(_PRE_WLAN_FEATURE_11K) || defined(_PRE_WLAN_FEATURE_11R) || defined(_PRE_WLAN_FEATURE_11V_ENABLE)
OAL_STATIC void hmac_vap_11kvr_cap_init(hmac_vap_stru *hmac_vap, mac_cfg_add_vap_param_stru *param)
{
    ...
    hmac_vap->bit_11r_mlo_enable = (hmac_vap->bit_11r_enable &&
        g_st_mac_device_custom_cfg.st_voe_custom_cfg.bit_11r_mlo) ? OAL_TRUE : OAL_FALSE;
    ...
    uint8_t device_id, uint8_t vap_id, mac_cfg_add_vap_param_stru *param)
{
    ...
    if (hmac_vap->query_wait_q.head.next == NULL || hmac_vap->query_wait_q.head.prev == NULL) {
    ...
    hmac_vap->roam_wpa3_auth_mode = ROAM_WPA3_FIXED_SAE_AUTH;
...
oal_net_device_stru *hmac_vap_get_net_device(uint8_t vap_id)
{
    ...
    /* 只有mld netdev才会挂接ml_netdev指针，为空说明是非mld vap，直接返回当前netdev即可 */
    if (net_dev_priv->mlo_cfg_info.ml_netdev == NULL) {
        return net_device;
    }
    /* mld netdev不管是wlan0还是link都会挂接ml_netdev指针，统一转化为ml_netdev返回 */
...
oal_net_device_stru *hmac_vap_get_link_net_device(void)
{
    oal_net_device_stru *net_device = NULL;
...
#ifdef _PRE_WLAN_FEATURE_ML
    link_vap = hmac_mld_get_link_vap(hmac_vap);
    if (link_vap == NULL) {
        return NULL;
    }
    net_device = link_vap->pst_net_device;
#endif
...
void hmac_vap_net_stopall(void)
{
    ...
    oal_net_device_stru *net_device = NULL;
    hmac_vap_stru *hmac_vap = NULL;
    for (vap_id = 0; vap_id < mac_chip_get_vap_support_max_num_limit(); vap_id++) {
        hmac_vap = (hmac_vap_stru *)mac_res_get_hmac_vap(vap_id);
        if (hmac_vap == NULL) {
            break;
        }
...
OAL_STATIC void hmac_vap_wake_subq(uint8_t vap_id, uint16_t queue_idx)
{
    ...
    net_dev_priv = (oal_netdev_priv_stru *)oal_net_dev_wireless_priv(net_device);
    if (net_dev_priv->mlo_cfg_info.is_link_netdev == OAL_TRUE) {
        return;
    }
...
void hmac_vap_net_start_subqueue(uint16_t queue_idx)
{
    ...
    if (g_uc_start_subq_flag == 0) {
        ...
        for (; vap_id < mac_chip_get_service_vap_limit_max_id(); vap_id++) {
            hmac_vap_wake_subq(vap_id, queue_idx);
        }
    } else {
        g_uc_start_subq_flag = 0;
        vap_id = mac_chip_get_service_vap_limit_max_id();
...
void hmac_vap_net_stop_subqueue(uint16_t queue_idx)
{
    ...
    if (g_uc_stop_subq_flag == 0) {
        ...
        for (; vap_id < mac_chip_get_vap_support_max_num_limit(); vap_id++) {
            hmac_vap_stop_subq(vap_id, queue_idx);
        }
    } else {
        g_uc_stop_subq_flag = 0;
        vap_id = mac_chip_get_vap_support_max_num_limit();
...
/*
 * 函 数 名  : hmac_vap_roam_get_pmksa
 * 功能描述  : 输入参数  : 无
 * 1.日    期  : 2016年4月8日
 *   作    者  : wifi
 *   修改内容  : 新生成函数
 */
uint8_t *hmac_vap_roam_get_pmksa(hmac_vap_stru *hmac_vap, mac_bss_dscr_stru *bss_dscr)
{
    uint8_t bssid[WLAN_MAC_ADDR_LEN];

    if (oal_any_null_ptr2(hmac_vap, bss_dscr)) {
        oam_error_log0(0, OAM_SF_CFG, "{hmac_vap_roam_get_pmksa param null}\r\n");
        return NULL;
    }
    if (hmac_vap->en_sae_connect == OAL_TRUE && hmac_vap->bit_sae_connect_with_pmkid == OAL_FALSE) {
        return NULL;
    }
#ifdef _PRE_WLAN_FEATURE_ML
    if (bss_dscr->ml_info.ml_cap == OAL_TRUE && hmac_vap_is_legacy_mld_mode(hmac_vap) == OAL_FALSE) {
    ...
    oal_set_mac_addr(bssid, bss_dscr->auc_bssid);
#endif
    return hmac_vap_get_pmksa(hmac_vap, bssid);
}
...
{
    return (vap_state >= MAC_VAP_STATE_STA_JOIN_COMP && vap_state <= MAC_VAP_STATE_STA_WAIT_MLD_SCAN);
}
/*
...
void hmac_sta_fill_user_info(hmac_vap_stru *hmac_sta, uint8_t *dest_addr, hmac_add_user_input_stru *user_info)
{
#ifdef _PRE_WLAN_FEATURE_ML
    hmac_mld_manage_stru       *mld_vap_manage = NULL;
#endif
     if (oal_unlikely(oal_any_null_ptr3(hmac_sta, dest_addr, user_info))) {
        oam_error_log0(0, OAM_SF_ANY, "{hmac_sta_fill_user_info::param null.}");
        return;
    }
    memset_s(user_info, sizeof(hmac_add_user_input_stru), 0, sizeof(hmac_add_user_input_stru));
    memcpy_s(user_info->mac_addr, WLAN_MAC_ADDR_LEN, dest_addr, WLAN_MAC_ADDR_LEN);
#ifdef _PRE_WLAN_FEATURE_ML
    mld_vap_manage = hmac_sta->mld_vap_link_info.mld_vap_manage;
    if (mld_vap_manage != NULL) {
        memcpy_s(user_info->mld_mac_addr, WLAN_MAC_ADDR_LEN, mld_vap_manage->mld_bssid, WLAN_MAC_ADDR_LEN);
    }
#endif
}

void hmac_update_vap_mac_addr(hmac_vap_stru *hmac_vap, uint8_t *new_addr, uint8_t addr_len)
{
    uint8_t *cur_mac_addr = NULL;
    mac_vap_stru *mac_vap;
    mac_cfg_staion_id_param_stru h2d_param;

    if (hmac_vap == NULL || new_addr == NULL) {
        return;
    }
    mac_vap = &hmac_vap->st_vap_base_info;
    cur_mac_addr = mac_mib_get_StationID(mac_vap);
    oam_warning_log3(hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_ANY,
        "{hmac_update_vap_mac_addr::ori mac addr[%02x:xx:xx:xx:%02x:%02x]}",
        cur_mac_addr[BYTE_OFFSET_0], cur_mac_addr[BYTE_OFFSET_4], cur_mac_addr[BYTE_OFFSET_5]);
    oam_warning_log3(hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_ANY,
        "{hmac_update_vap_mac_addr::new mac addr[%02x:xx:xx:xx:%02x:%02x]}",
        new_addr[BYTE_OFFSET_0], new_addr[BYTE_OFFSET_4], new_addr[BYTE_OFFSET_5]);
    oal_set_mac_addr(cur_mac_addr, new_addr);
    h2d_param.en_p2p_mode = WLAN_LEGACY_VAP_MODE;
    oal_set_mac_addr(h2d_param.auc_station_id, cur_mac_addr);
    hmac_config_set_mac_addr(mac_vap, sizeof(mac_cfg_staion_id_param_stru), (uint8_t *)&h2d_param);
}

void hmac_cofeture_state_change_notify_sta(hmac_vap_stru *hmac_vap,
    uint8_t feature, uint8_t feature_state, uint8_t reason)
{
    frw_event_mem_stru *event_mem = NULL;
    frw_event_stru *event = NULL;
    hmac_cofeature_rpt_stru feature_info = {0};
    uint32_t ret;
    int32_t ret1;
    size_t info_len;

    if (hmac_vap == NULL) {
        oam_error_log0(0, OAM_SF_SCAN, "{hmac_cofeture_state_change_notify_sta::hmac_vap null!}");
        return;
    }

    if ((feature == COFEATURE_TYPE_MLO && feature_state >= MLO_WORK_STATE_BUTT) ||
        (feature == COFEATURE_TYPE_WUR && feature_state >= WUR_WORK_STATE_BUTT)) {
        oam_warning_log3(hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_SCAN,
            "{hmac_cofeture_state_change_notify_sta::feature[%d] state[%d] reason_code[%d], not rpt}",
            feature, feature_state, reason);
        return;
    }

    feature_info.feature = feature;
    feature_info.state = feature_state;
    feature_info.reason_code = reason;

    info_len = sizeof(hmac_cofeature_rpt_stru);
    /* 抛加入完成事件到WAL */
    event_mem = frw_event_alloc_m(info_len);
    if (event_mem == NULL) {
        oam_error_log0(0, OAM_SF_SCAN, "{hmac_cofeture_state_change_notify_sta::frw_event_alloc_m fail!}");
        return;
    }
    ...
    frw_event_hdr_init(&(event->st_event_hdr), FRW_EVENT_TYPE_HOST_CTX,
                       HMAC_HOST_CTX_EVENT_SUB_TYPE_COFEATURE_STATE_NOTIFY, (uint16_t)info_len,
    ...
    ret1 = memcpy_s((uint8_t *)frw_get_event_payload(event_mem), info_len, (uint8_t *)&feature_info, info_len);
    if (ret1 != EOK) {
        oam_error_log0(0, OAM_SF_AUTH, "hmac_cofeture_state_change_notify_sta::memcpy fail!");
    ...
    if (oal_unlikely(ret != OAL_SUCC)) {
        oam_warning_log1(hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_AUTH,
                         "hmac_cofeture_state_change_notify_sta::frw_event_dispatch_event fail[%d]", ret);
    }

    frw_event_free_m(event_mem);
}

/* vap状态变化通知其他特性，包括wur，emslr */
void hmac_vap_state_change_notify_feature(hmac_vap_stru *hmac_vap, hmac_user_stru *hmac_user,
    uint8_t event_type, uint8_t event_subtype)
{
#ifdef _PRE_WLAN_FEATURE_WUR
    hmac_vap_state_change_notify_wur(hmac_vap, hmac_user, event_type, event_subtype);
#endif
#ifdef _PRE_WLAN_FEATURE_EMLSR
    hmac_vap_state_change_notify_emlsr_common_proc(hmac_vap, hmac_user, event_type, event_subtype);
#endif
}