...
typedef struct {
    uint16_t tbtt_num;
    uint8_t resv[2];
    frw_timeout_stru delete_timer;
} ml_reconfig_delete_timer_info;

typedef struct {
    uint8_t diag_token;
    uint8_t op_num;
    uint8_t op_valid;   // 是否交互中
    uint8_t need_add;
    uint8_t need_del;
    uint8_t resv[3];    // 对齐
    uint8_t op_type[WLAN_MAX_ML_LINK_NUM];
    uint8_t op_link_id[WLAN_MAX_ML_LINK_NUM];
    uint16_t rsp_status[WLAN_MAX_ML_LINK_NUM];
    frw_timeout_stru req_timer;
} ml_reconfig_operation_info;
#endif
...
typedef struct hmac_vap_tag {
    ...
    uint8_t bit_11r_mlo_enable : 1; /* 是否支持MLO 11r流程. 0:不支持；1：支持 */
    uint8_t bit_resv2 : 3;
    uint8_t uc_resv;
#else
    uint8_t bit_resv1 : 1;
    ...
    uint8_t uc_ps_mode;
    uint8_t fastsleep_en : 1,
            fastsleep3_en : 1,
            resv1 : 6;
    uint8_t fastsleep_exit_cnt;
    uint8_t fastsleep_entry_cnt;
...
#define MAX_TWT_NUM 4
    ...
    int8_t roam_rssi;
#ifdef _PRE_WLAN_FEATURE_ML
    hmac_mld_vap_info_stru mld_vap_link_info;
    ml_reconfig_delete_timer_info delete_timer_info;
    ml_reconfig_operation_info reconfig_op_info;
    mld_bss_critical_update_stru bss_critical_update;
    hmac_ml_scan_param_stru ml_scan_param;
#endif
#ifdef _PRE_WLAN_FEATURE_ESPE
    hmac_vap_espe_stru espe_info;
#endif
#ifdef _PRE_WLAN_FEATURE_ISOLATION
    uint8_t ap_isolation;   /* bit0:广播  bit1:组播  bit2:单播 */
#endif
#ifdef _PRE_WLAN_FEATURE_WUR
    mac_wur_stru wur_info;
#endif

    wlan_priv_key_param_stru bigtk_key[WLAN_NUM_BIGTK]; /* beacon 保护 key缓存 */
    uint8_t roam_wpa3_auth_mode;
...
#ifdef _PRE_WLAN_FEATURE_ML
OAL_STATIC OAL_INLINE uint8_t hmac_vap_is_ml_mode(hmac_vap_stru *hmac_vap)
{
    if (hmac_vap == NULL) {
        return OAL_FALSE;
    }
    return (hmac_vap->mld_vap_link_info.mld_mode == WLAN_ML_MODE ||
        hmac_vap->mld_vap_link_info.mld_mode == WLAN_ML_LINK_MODE);
}
OAL_STATIC OAL_INLINE uint8_t hmac_vap_is_legacy_mld_mode(hmac_vap_stru *hmac_vap)
{
    if (hmac_vap == NULL) {
        return OAL_FALSE;
    }
    return hmac_vap->mld_vap_link_info.mld_mode == WLAN_LEGACY_MLD_MODE;
}
OAL_STATIC OAL_INLINE uint8_t hmac_vap_is_link_mode(hmac_vap_stru *hmac_vap)
{
    if (hmac_vap == NULL) {
        return OAL_FALSE;
    }
    return hmac_vap->mld_vap_link_info.mld_mode == WLAN_LINK_MODE;
}
OAL_STATIC OAL_INLINE uint8_t hmac_vap_is_link_vap(hmac_vap_stru *hmac_vap)
{
    ...
    return net_dev_priv->mlo_cfg_info.is_link_netdev;
}
#else
OAL_STATIC OAL_INLINE uint8_t hmac_vap_is_ml_mode(hmac_vap_stru *hmac_vap)
{
...
#endif
static inline oal_bool_enum_uint8 mac_is_secondary_sta(hmac_vap_stru *hmac_vap)
{
    uint8_t is_link_vap;
    uint8_t is_pl_vap;
    ...
    is_link_vap = hmac_vap_is_link_vap(hmac_vap);
    mac_vap = &(hmac_vap->st_vap_base_info);
    is_pl_vap = mac_is_primary_legacy_vap(mac_vap);
    return (is_pl_vap == OAL_FALSE && is_legacy_sta(mac_vap) && is_link_vap == OAL_FALSE) ? OAL_TRUE : OAL_FALSE;
}
