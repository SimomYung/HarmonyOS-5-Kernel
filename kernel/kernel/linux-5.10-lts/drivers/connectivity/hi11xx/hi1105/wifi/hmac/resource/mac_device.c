...
OAL_STATIC uint32_t mac_device_init_protocol_cap(mac_device_stru *pst_mac_device,
    uint32_t chip_ver, uint8_t uc_device_id)
{
    ...
    /* 支持11be则更新协议能力为EHT，否则不变 */
    pst_mac_device->en_protocol_cap = g_wlan_spec_cfg->feature_11be_is_open ?
        WLAN_PROTOCOL_CAP_EHT : pst_mac_device->en_protocol_cap;
#endif
...
static void mac_device_init_params(mac_device_stru *pst_mac_device)
{
    ...
    OAL_MUTEX_INIT(&pst_mac_device->pcie_switch_lock);
}
...
uint32_t mac_device_init(mac_device_stru *mac_device, uint32_t chip_ver, uint8_t chip_id, uint8_t device_id)
{
    ...
    memset_s(mac_device, sizeof(mac_device_stru), 0, sizeof(mac_device_stru));

    /* 初始化device的索引 */
    mac_device->uc_chip_id = chip_id;
    mac_device->uc_device_id = device_id;

    /* 初始化device级别的一些参数 */
    mac_device_init_params(mac_device);
    mac_device_init_dyn_pcie(mac_device);

    /* mp13两个业务device,00 01,取不同定制化,51双芯片00 11,取同一个定制化 */
    uc_device_id_per_chip = device_id - chip_id;
    if (uc_device_id_per_chip >= WLAN_SERVICE_DEVICE_MAX_NUM_PER_CHIP) {
        oam_error_log3(0, OAM_SF_ANY, "{mac_device_init::mac_device device id[%d] chip id[%d] >support[%d].}",
                       device_id, chip_id, WLAN_SERVICE_DEVICE_MAX_NUM_PER_CHIP);
        uc_device_id_per_chip = 0;
    }

    /* 将定制化的信息保存到mac device结构体下 */
    mac_device->device_cap = &g_pst_mac_device_capability[uc_device_id_per_chip];
    ...
    mac_device->en_device_state = OAL_TRUE;
    mac_device->en_reset_switch = OAL_FALSE;

    /* 默认关闭wmm,wmm超时计数器设为0 */
    mac_device->en_wmm = OAL_TRUE;

    /* 根据芯片版本初始化device能力信息 */
    if (mac_device_init_protocol_cap(mac_device, chip_ver, device_id) != OAL_SUCC) {
        return OAL_ERR_CODE_CONFIG_UNSUPPORT;
    }

    /* 初始化vap num统计信息 */
    mac_device->uc_vap_num = 0;
    mac_device->uc_sta_num = 0;
    mac_device_p2p_info_init(&mac_device->st_p2p_info);
    ...
    /* 初始化复位状态 */
    mac_device_set_dfr_reset(mac_device, OAL_FALSE);
    mac_device->us_device_reset_num = 0;

    /* 默认关闭DBAC特性 */
    mac_device->en_dbac_enabled = OAL_TRUE;

    mac_device->en_dbdc_running = OAL_FALSE;
    mac_set_2040bss_switch(mac_device, OAL_FALSE);

    mac_device->uc_arpoffload_switch = OAL_FALSE;

    mac_random_mac_oui_init(mac_device);

#ifdef _PRE_WLAN_FEATURE_DFS
    mac_dfs_init(mac_device);
#endif /* #ifdef _PRE_WLAN_FEATURE_DFS */

#ifdef _PRE_WLAN_FEATURE_HIEX
    mac_hiex_cap_init(mac_device);
#endif
    ...
    wlan_vap_mode_enum_uint8 vap_mode)
{
    ...
    if (vap_mode == WLAN_VAP_MODE_BSS_STA) {
        ...
        mac_vap_set_assoc_id(mac_vap, g_wlan_spec_cfg->invalid_user_id);
    }
    ...
    wlan_p2p_mode_enum_uint8 en_p2p_mode, uint8_t is_add_vap)
{
    if (is_add_vap) {
        ...
        if (en_vap_mode == WLAN_VAP_MODE_BSS_STA) {
            ...
            /* ???us_assoc_vap_id??????ap??? */
            mac_vap_set_assoc_id(pst_mac_vap, g_wlan_spec_cfg->invalid_user_id);
        }
...
mac_vap_stru *mac_device_find_another_up_vap(mac_device_stru *mac_device, uint8_t vap_id_self)
{
    ...
    for (vap_idx = 0; vap_idx < mac_device->uc_vap_num; vap_idx++) {
        ...
        if (mac_vap_is_up(mac_vap) == OAL_TRUE) {
            return mac_vap;
        }
    }
...
uint32_t mac_device_calc_up_vap_num(mac_device_stru *pst_mac_device)
{
    ...
    for (uc_vap_idx = 0; uc_vap_idx < pst_mac_device->uc_vap_num; uc_vap_idx++) {
        ...
        if (mac_vap_is_up(pst_vap) == OAL_TRUE) {
            up_vap_num++;
        }
    }
...
uint32_t mac_device_find_up_vap(mac_device_stru *pst_mac_device, mac_vap_stru **ppst_mac_vap)
{
    ...
    for (uc_vap_idx = 0; uc_vap_idx < pst_mac_device->uc_vap_num; uc_vap_idx++) {
        ...
        if (mac_vap_is_up(pst_mac_vap) == OAL_TRUE) {
            *ppst_mac_vap = pst_mac_vap;
...
uint32_t mac_device_find_up_vaps(mac_device_stru *mac_device, mac_vap_stru **mac_vap_array, uint32_t vap_array_num)
{
    ...
    for (uc_vap_idx = 0; uc_vap_idx < mac_device->uc_vap_num; uc_vap_idx++) {
        mac_vap = mac_res_get_mac_vap(mac_device->auc_vap_id[uc_vap_idx]);
        if (oal_unlikely(mac_vap == NULL)) {
...
uint32_t hmac_m2s_sync_event(frw_event_mem_stru *pst_event_mem)
{
    ...
    if (pst_m2s_event->en_m2s_type == WLAN_M2S_TYPE_HW) {
...
uint8_t mac_device_pcie_down_get_vap_id(uint8_t mac_dev_idx)
{
    uint32_t ret;
    uint8_t vap_num;
    uint8_t vap_idx = 0xFF;
    mac_vap_stru *mac_vap = NULL;
    mac_vap_stru *up_mac_vap[WLAN_SERVICE_VAP_MAX_NUM_PER_DEVICE] = {NULL};
    mac_device_stru *mac_device = mac_res_get_dev(mac_dev_idx);
