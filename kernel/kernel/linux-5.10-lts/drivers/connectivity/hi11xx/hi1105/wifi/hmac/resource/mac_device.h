...
typedef struct {
    mac_pkt_filter_cmd_type_uint8 en_cmd_type;
    uint16_t us_program_len;
    uint8_t *puc_program;
} mac_pkt_filter_cmd_stru;
#endif
...
typedef struct {
    ...
    uint8_t  supported_channel_width : 2, /* vht cap能力，用于带宽计算 */
             ...
             wur_capable             : 1,
             supp_eht_double_wifi    : 1, /* 对端支持wifi7灵犀双wifi  */
             ext_cowork_ie           : 1,
             agc_compability_ap      : 1; /* 对端AP可能有agc兼容性问题 */
    ...
    uint8_t eht_capable; /* 是否支持11be */
    uint8_t hisi_emlsr_capable; /* 对端支持emlsr私有对通 */
    uint8_t resv4[2];    /* 4字节对齐, 预留3字节 */
#endif
#ifdef _PRE_WLAN_FEATURE_ML
    ml_info_stru ml_info; /* 记录 ml ie和rnr ie中 multi-link 相关信息 */
#endif
#ifdef _PRE_WLAN_FEATURE_11B_PLUS
    uint8_t hisi_11bplus_cap;
#endif
    oal_bool_enum_uint8 dualband_roam_cap;
...
typedef struct {
    ...
    uint8_t en_11ax_switch         : 1, /* 11ax开关 */
            ...
            bit_11ax_rsv           : 6;
    uint8_t switch_11be : 1,            /* 11be开关 */
            switch_mlo  : 1,
            switch_emlsr : 1,
            rsv_11be    : 5;
    uint8_t _rom[NUM_2_BYTES];
...
typedef struct {
    ...
    uint16_t bit_11r_mlo : 1, /* 是否支持MLO 11r流程. 0:不支持；1：支持 */
            bit_res : 15;
...
#ifdef _PRE_WLAN_FEATURE_11BE
typedef struct {
    uint16_t sta_11be_switch : 1,
             aput_11be_switch : 1,
             aput_11be_hisi_switch : 1,
             non_ofdma_su_puncture : 1,
             eht_1024qam_support : 1,
             eht_4096qam_support : 1,
             chba_11be_switch : 1,
             resv1 : 9;
    uint16_t mlo_switch : 1,
             single_radio_switch : 1,
             emlsr_switch : 1,
             ml_assoc_mode : 1,
             bpcc_switch : 1,     /* beacon中处理bpcc的功能开关 */
             aput_mlo_switch : 1, /* aput使能MLD功能开关 */
             ttlm_switch : 1,     /* ttlm特性使能开关 */
             reconfig_switch : 1, /* reconfig特性使能开关 */
             emlsr_enter_switch : 1, /* emlsr默认进入开关 */
             emlsr_hisi_switch : 1,  /* emlsr私有对通开关 */
             resv2 : 6;
} mac_device_11be_custom_stru;
#endif
...
typedef struct {
    ...
    uint8_t  bit_forbit_open_auth : 1,
               bit_rsv              : 7;
#ifdef _PRE_WLAN_FEATURE_11BE
    mac_device_11be_custom_stru custom_cfg_11be;
#endif
...
typedef struct {
    ...
    uint8_t ml_user_idx_table[WLAN_ACTIVE_USER_IDX_BMAP_LEN]; /* ml用户 lut表位图 */
} mac_lut_table_stru;
...
typedef struct {
    ...
    uint32_t mld_lut_bitmap[WLAN_USER_MAX_USER_LIMIT];
 ...
 * 输入参数  : chip version
 */
static inline uint8_t oal_chip_get_device_num(uint32_t chip_ver)
{
 ...
 */
static inline uint8_t oal_board_get_service_vap_start_id(void)
{
    uint8_t uc_device_num_per_chip = oal_chip_get_device_num(0);
#ifdef _PRE_DUAL_CHIP
    /* 不修改device, 保持业务vap的start id与单芯片时一致, 即为1 */
    return uc_device_num_per_chip;
#else
...
uint32_t mac_device_init(mac_device_stru *mac_device, uint32_t chip_ver, uint8_t chip_id, uint8_t device_id);
...
uint8_t mac_device_pcie_down_get_vap_id(uint8_t mac_dev_idx);
