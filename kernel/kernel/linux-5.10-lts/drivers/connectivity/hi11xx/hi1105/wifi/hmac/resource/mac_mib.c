...
void mac_vap_init_mib_11ax_mp16(mac_vap_stru *mac_vap, uint32_t nss_num)
{
    ...
    if (mac_mib_get_he_SUBeamformee(mac_vap) == OAL_TRUE) {
        mac_mib_set_HENg16SUFeedback(mac_vap, OAL_TRUE);
        mac_mib_set_HENg16MUFeedback(mac_vap, OAL_TRUE);
        mac_mib_set_HECodebook75MUFeedback(mac_vap, OAL_TRUE);
    }
}
...
void mac_vap_init_mib_11ax(mac_vap_stru *mac_vap)
{
    ...
    if (pst_mac_dev == NULL) {
        oam_error_log1(mac_vap->uc_vap_id, OAM_SF_11AX,
            "{mac_vap_init_mib_11ax::pst_mac_dev[%d] null.}", mac_vap->uc_device_id);

        return;
    }

    /* MAC Capabilities Info */
    mac_mib_set_HEOptionImplemented(mac_vap, OAL_TRUE);
    mac_mib_set_OMIOptionImplemented(mac_vap, mac_vap->bit_rom_custom_switch);
    mac_mib_set_he_TWTOptionActivated(mac_vap, OAL_TRUE); /* 第一阶段暂不支持 */
    mac_mib_set_he_OperatingModeIndication(mac_vap, OAL_TRUE);
    mac_mib_set_he_TriggerMacPaddingDuration(mac_vap, MAC_TRIGGER_FRAME_PADDING_DURATION16US); /* 16us */
    mac_mib_set_he_MaxAMPDULengthExponent(mac_vap, MAC_HE_MAX_AMPDU_LEN_EXP_0); /* 2^(20+factor)-1字节 */
    mac_mib_set_he_MultiBSSIDImplemented(mac_vap, OAL_TRUE);
    mac_mib_set_he_BSRSupport(mac_vap, OAL_TRUE);

    if ((g_wlan_spec_cfg->feature_11ax_uora_is_supported == OAL_TRUE) && is_legacy_sta(mac_vap)) {
        mac_mib_set_he_OFDMARandomAccess(mac_vap, OAL_TRUE);
    }

    /* PHY Capabilities Info */
    if (pst_mac_dev->en_band_cap == WLAN_BAND_CAP_2G_5G) {
        mac_mib_set_he_DualBandSupport(mac_vap, OAL_TRUE); /* 支持双频 */
    }
    mac_mib_set_he_LDPCCodingInPayload(mac_vap, mac_device_get_cap_ldpc(pst_mac_dev)); /* 支持LDPC编码 */
    mac_mib_set_he_SUBeamformer(mac_vap, mac_device_get_cap_subfer(pst_mac_dev));
    mac_mib_set_he_SUBeamformee(mac_vap, mac_device_get_cap_subfee(pst_mac_dev));
    mac_mib_set_he_MUBeamformer(mac_vap, mac_device_get_cap_mubfer(pst_mac_dev));
    /* 以下mib项只有host使用 */
    wlan_chip_mac_vap_init_mib_11ax(mac_vap, mac_device_get_nss_num(pst_mac_dev));  /* 针对不同芯片初始化11ax能力 */

    mac_mib_set_HECodebook42SUFeedback(mac_vap, OAL_TRUE);
    mac_mib_set_he_STBCTxBelow80M(mac_vap, mac_device_get_cap_txstbc(pst_mac_dev));
    mac_mib_set_he_STBCRxBelow80M(mac_vap, mac_device_get_cap_rxstbc(pst_mac_dev));
    mac_mib_set_PPEThresholdsRequired(mac_vap, OAL_TRUE);
    mac_mib_set_HESUPPDUwith1xHELTFand0point8GIlmplemented(mac_vap, OAL_TRUE);

    /* Tx Rx MCS NSS */
    mac_mib_set_he_HighestNSS(mac_vap, mac_device_get_nss_num(pst_mac_dev));

    /* Multi BSSID */
    mac_mib_set_he_MultiBSSIDActived(mac_vap, mac_vap->bit_multi_bssid_custom_switch);

    if (mac_vap->en_vap_mode == WLAN_VAP_MODE_BSS_AP) {
        mac_mib_set_he_MultiBSSIDActived(mac_vap, OAL_FALSE);
        mac_mib_set_he_TWTOptionActivated(mac_vap, OAL_FALSE);
        mac_mib_set_he_TriggerMacPaddingDuration(mac_vap, MAC_TRIGGER_FRAME_PADDING_DURATION0US);
        mac_mib_set_he_BSRSupport(mac_vap, OAL_FALSE);
        mac_mib_set_he_SUBeamformee(mac_vap, OAL_TRUE);
        mac_mib_set_he_MUBeamformer(mac_vap, OAL_FALSE);
    }
}
...
OAL_STATIC void mac_vap_init_eht_ppe_thresholds_mib(mac_vap_stru *mac_vap)
{
    uint8_t nss_idx;
    uint8_t ru_idx;

    mac_mib_set_eht_Nss(mac_vap->pst_mib_info, MAC_EHT_MAX_PPET_NSS);
    mac_mib_set_eht_RuIndexMask(mac_vap->pst_mib_info, MAC_EHT_PPET_RU_BITMASK_FIELD_VALUE);

    for (nss_idx = 0; nss_idx < MAC_EHT_MAX_PPET_NSS + 1; nss_idx++) {
        for (ru_idx = 0; ru_idx < MAC_EHT_PPET_RU_BITMASK_FIELD_BIT; ru_idx++) {
            mac_mib_set_eht_NssRuPpetx(mac_vap->pst_mib_info, MAC_EHT_PPE_THR_BPSK, nss_idx, ru_idx);
            mac_mib_set_eht_NssRuPpet8(mac_vap->pst_mib_info, MAC_EHT_PPE_THR_NONE, nss_idx, ru_idx);
        }
    }
}

OAL_STATIC void mac_vap_init_eht_station_config_mib(mac_vap_stru *mac_vap)
{
    mac_mib_set_EHTOptionImplemented(mac_vap->pst_mib_info, OAL_TRUE);
    mac_mib_set_EHTTXOPSharingTFOptionImplemented(mac_vap->pst_mib_info, OAL_FALSE);
    mac_mib_set_EHTMaxAmpduLengthExponent(mac_vap->pst_mib_info, 0);
    mac_mib_set_RestrictedTWTOptionImplemented(mac_vap->pst_mib_info, 0);
    mac_mib_set_TIDtoLinkMappingActivated(mac_vap->pst_mib_info, OAL_FALSE);
}

OAL_STATIC void mac_vap_init_eht_oper_mib(mac_vap_stru *mac_vap)
{
    mac_mib_set_EHTCurrentChannelWidth(mac_vap->pst_mib_info, WLAN_BW_CAP_160M);
    mac_mib_set_EHTDisabledSubchannelBitmapPresent(mac_vap->pst_mib_info, 0);
    mac_mib_set_EHTOmControlSupport(mac_vap->pst_mib_info, mac_vap->bit_rom_custom_switch);
}

OAL_STATIC void mac_vap_init_eht_phy_mib_part1(mac_vap_stru *mac_vap)
{
    oal_bool_enum_uint8 rx4kqam = OAL_FALSE;
    mac_mib_set_EHTTx1024QAMand4096QAMLessThan242ToneRUImplemented(mac_vap->pst_mib_info, OAL_TRUE);
    mac_mib_set_EHTRx1024QAMand4096QAMLessThan242ToneRUImplemented(mac_vap->pst_mib_info, OAL_TRUE);

    /* 此属性为true时，表示STA能够使用额外的EHT-LTF符号接收EHT非OFDMA传输 */
    mac_mib_set_EHTExtraLTFsImplemented(mac_vap->pst_mib_info, OAL_TRUE);
    if (mac_vap->en_vap_mode == WLAN_VAP_MODE_BSS_STA) {
        /* 1024qam、4096qam是否可以不支持 */
        mac_mib_set_Rx1024QamInWiderBandwidthDlOfdmaSupport(mac_vap->pst_mib_info, OAL_TRUE);
        if (g_st_mac_device_custom_cfg.custom_cfg_11be.eht_4096qam_support == OAL_TRUE) {
            rx4kqam = OAL_TRUE;
        }
        mac_mib_set_Rx4096QamInWiderBandwidthDlOfdmaSupport(mac_vap->pst_mib_info, rx4kqam);
    }
}

OAL_STATIC void mac_vap_init_eht_phy_mib(mac_vap_stru *mac_vap)
{
    mac_device_stru *mac_dev = mac_res_get_dev(mac_vap->uc_device_id);
    if (mac_dev == NULL) {
        oam_error_log1(mac_vap->uc_vap_id, OAM_SF_11AX,
            "{mac_vap_init_eht_phy_mib::mac_dev[%d] null.}", mac_vap->uc_device_id);
        return;
    }

    /* AP是否支持以non-OFDMA UL MU-MIMO模式接收20/40/80M、160M、320MHz的EHT TB PPDU，STA不需要该能力 */
    mac_mib_set_EHTNonOFDMAULMUMIMOLessThanOrEqualto80Implemented(mac_vap->pst_mib_info, OAL_FALSE);
    mac_mib_set_EHTNonOFDMAULMUMIMOEqualto160Implemented(mac_vap->pst_mib_info, OAL_FALSE);

    /* 对于AP，是否支持接收使用OFDMA UL MU-MIMO发送的EHT TB PPDU
       对于non-AP STA，是否支持使用OFDMA UL MU-MIMO发送的EHT TB PPDU */
    mac_mib_set_EHTMUPPDUwith4xEHTLTFand0point8usecGIImplemented(mac_vap->pst_mib_info, OAL_TRUE);

    mac_vap_init_eht_phy_mib_part1(mac_vap);

    /* 工作在20 MHz带宽下的non-AP STA，是否支持接收有242-tone RU并且带宽大于20 MHz的PPDU，AP不需要该能力 */
    if (mac_vap->en_vap_mode == WLAN_VAP_MODE_BSS_STA) {
        /* TBD:mpw不支持，fpga支持，是否打开 */
        mac_mib_set_EHTSupportFor242ToneRUInBWWiderThan20Implemented(mac_vap->pst_mib_info, OAL_FALSE);
    }

    /* 是否支持SU Beamformer，如果MU Beamformer(<=80 =160 =320)有任意一个为true，那SU Beamformer也要为true */
    mac_mib_set_EHTSUBeamformerImplemented(mac_vap->pst_mib_info, mac_device_get_cap_subfer(mac_dev));
    /* 是否支持SU Beamformee，AP选择是否支持0/1，STA直接设为1 */
    mac_mib_set_EHTSUBeamformeeImplemented(mac_vap->pst_mib_info, mac_device_get_cap_subfee(mac_dev));

    /* 对于AP，是否支持从EHT TB sounding sequence中接收部分和全部带宽的SU反馈
       对于non-AP STA，是否支持在EHT TB sounding sequence中发送部分和全部带宽的SU反馈 */
    mac_mib_set_EHTTriggeredSUBeamformingFeedbackImplemented(mac_vap->pst_mib_info, OAL_TRUE);
    mac_mib_set_EHTTriggeredMUBeamformingPartialBWFeedbackImplemented(mac_vap->pst_mib_info, OAL_TRUE);
    mac_mib_set_EHTTriggeredCQIFeedbackImplemented(mac_vap->pst_mib_info, OAL_TRUE);

    /* 对于AP，是否支持从EHT TB sounding sequence中接收部分和全部带宽的CQI反馈
       对于non-AP STA，是否支持在EHT TB sounding sequence中发送部分和全部带宽的CQI反馈 */
    mac_mib_set_EHTNonTriggeredCQIFeedbackImplemented(mac_vap->pst_mib_info, OAL_TRUE);

    /* EHT beamformee是否支持SU反馈的EHT Compressed Beamforming Report field中的16个子载波分组 */
    mac_mib_set_EHTNG16SUFeedbackImplemented(mac_vap->pst_mib_info, OAL_TRUE);
    /* EHT beamformee是否支持MU反馈的EHT Compressed Beamforming Report field中的16个子载波分组 */
    mac_mib_set_EHTNG16MUFeedbackImplemented(mac_vap->pst_mib_info, OAL_TRUE);

    /* 下面两个codebook mib项，如果EHT STA不是EHT Beamformee，保留 */
    mac_mib_set_EHTCodebookSizePhi4Psi2SUFeedbackImplemented(mac_vap->pst_mib_info, OAL_TRUE);
    mac_mib_set_EHTCodebookSizePhi7Psi5MUFeedbackImplemented(mac_vap->pst_mib_info, OAL_TRUE);

    /* 对于beamformee，是否支持接收4x EHT-LTF和3.2 µs保护间隔的EHT sounding NDP */
    if (mac_mib_get_EHTSUBeamformeeImplemented(mac_vap->pst_mib_info)) {
        mac_mib_set_EHTNDPwith4xEHTLTFand3point2GIImplemented(mac_vap->pst_mib_info, OAL_TRUE);
    }
    mac_mib_set_EHTMaxNumberOfSupportedEHTLTFsForSU(mac_vap->pst_mib_info, SUPPORT_EHT_LTFS_DISABLE);
    mac_mib_set_EHTMaxNumberOfSupportedEHTLTFsForMUandNDP(mac_vap->pst_mib_info, SUPPORT_EHT_LTFS_ENABLE);
    if (g_wlan_spec_cfg->bfee_ntx_supports < 1) {
        oam_error_log0(mac_vap->uc_vap_id, OAM_SF_11AX, "{mac_vap_init_eht_phy_mib::bfee_ntx_supports invalid.}");
        return;
    }
    mac_mib_set_EHTBeamformeeSSLessThanOrEqualTo80(mac_vap->pst_mib_info, g_wlan_spec_cfg->bfee_ntx_supports - 1);
    if (g_wlan_spec_cfg->max_5g_bw >= WLAN_BANDWIDTH_160) {
        mac_mib_set_EHTBeamformeeSSEqualTo160(mac_vap->pst_mib_info, g_wlan_spec_cfg->bfee_ntx_supports - 1);
    } else {
        mac_mib_set_EHTBeamformeeSSEqualTo160(mac_vap->pst_mib_info, 0);
    }

    mac_mib_set_EHTNumberSoundingDimensionsLessThanOrEqualTo80(mac_vap->pst_mib_info,
        mac_device_get_nss_num(mac_dev));
    mac_mib_set_EHTNumberSoundingDimensionsEqualTo160(mac_vap->pst_mib_info,
        mac_device_get_nss_num(mac_dev)); /* 最大流数-1 */
    mac_mib_set_EHTMaxNc(mac_vap->pst_mib_info, mac_device_get_nss_num(mac_dev)); /* 最大流数-1 */
}

OAL_STATIC void mac_vap_init_mib_mlo_sta(mac_vap_stru *mac_vap)
{
    if (g_st_mac_device_custom_cfg.custom_cfg_11be.mlo_switch == OAL_TRUE) {
        mac_mib_set_MultiLinkActivated(mac_vap->pst_mib_info, OAL_TRUE);
    }

    if (g_st_mac_device_custom_cfg.custom_cfg_11be.ttlm_switch == OAL_TRUE) {
        mac_mib_set_TIDtoLinkMappingActivated(mac_vap->pst_mib_info, OAL_TRUE);
    }

    if (g_st_mac_device_custom_cfg.custom_cfg_11be.reconfig_switch == OAL_TRUE) {
        mac_mib_set_EHTLinkReconfigurationOperationActivated(mac_vap->pst_mib_info, OAL_TRUE);
    }
}

void mac_vap_init_mib_mlo(mac_vap_stru *mac_vap)
{
    ...
    /* legacy sta设置mlo相关mib能力 */
    if (is_legacy_sta(mac_vap) == OAL_TRUE) {
        mac_vap_init_mib_mlo_sta(mac_vap);
    }

    /* legacy ap且非chba，定制化支持mlo，设置mib能力 */
    if ((is_legacy_ap(mac_vap) == OAL_TRUE) &&
        (mac_is_chba_mode(mac_vap) != OAL_TRUE) &&
        (g_st_mac_device_custom_cfg.custom_cfg_11be.aput_mlo_switch == OAL_TRUE)) {
        mac_mib_set_MultiLinkActivated(mac_vap->pst_mib_info, OAL_TRUE);
    }
}

/*
 * 功能描述  : 初始化11be协议 mib能力
 * 1.日    期  : 2013年4月27日
 *   修改内容  : 新生成函数
 */
void mac_vap_init_mib_11be(mac_vap_stru *mac_vap)
{
    if (g_wlan_spec_cfg->feature_11be_is_open != OAL_TRUE) {
        return;
    }

    /* EHT Station Config MIB */
    mac_vap_init_eht_station_config_mib(mac_vap);

    /* EHT Operation Info MIB */
    mac_vap_init_eht_oper_mib(mac_vap);

    /* EHT PHY Capabilities Info MIB */
    mac_vap_init_eht_phy_mib(mac_vap);

    /* PPE Thresholds field */
    mac_vap_init_eht_ppe_thresholds_mib(mac_vap);
...
void mac_vap_init_mib_wur(mac_vap_stru *mac_vap)
{
    /* 默认wur支持 */
    mac_mib_set_WUROptionImplemented(mac_vap, OAL_FALSE);

    /* 默认wur beacon帧和wur wake up帧同信道，不支持FDMA */
    mac_mib_set_WURFDMAChannelSwitchImplemented(mac_vap, OAL_FALSE);

    /* 默认wur beacon周期和beacon周期一致，在配置beacon_period的mib项时配置 */
    mac_mib_set_WURBeaconPeriod(mac_vap, 0);

    /* 暂不支持wur discovery */
    mac_mib_set_WURDiscoveryImplemented(mac_vap, OAL_FALSE);
    mac_mib_set_WURNeighborDiscoveryImplemented(mac_vap, OAL_FALSE);
    mac_mib_set_WURDiscoveryPeriod(mac_vap, 0);
}
#endif
...
#if defined(_PRE_WLAN_FEATURE_11V_ENABLE)
    /* 11k */
    mac_vap_init_mib_11v(mac_vap);
#endif

#ifdef _PRE_WLAN_FEATURE_TXOPPS
    /* txopps 初始化关闭状态 */
    mac_mib_set_txopps(mac_vap, OAL_FALSE);
#endif

#ifdef _PRE_WLAN_FEATURE_FTM
    mac_ftm_mib_init(mac_vap);
#endif

    mac_mib_set_dot11VHTExtendedNSSBWCapable(mac_vap, OAL_TRUE);
}
...
uint32_t mac_mib_set_station_id(mac_vap_stru *mac_vap, uint8_t len, uint8_t *param)
{
    ...
    pst_param = (mac_cfg_staion_id_param_stru *)param;

    oal_set_mac_addr(mac_mib_get_StationID(mac_vap), pst_param->auc_station_id);
...
uint32_t mac_mib_set_bss_type(mac_vap_stru *mac_vap, uint8_t len, uint8_t *param)
{
    ...
    l_value = *((int32_t *)param);

    mac_mib_set_DesiredBSSType(mac_vap, (uint8_t)l_value);
...
uint32_t mac_mib_set_ssid(mac_vap_stru *mac_vap, uint8_t len, uint8_t *param)
{
    ...
    pst_param = (mac_cfg_ssid_param_stru *)param;
    ...
    puc_mib_ssid = mac_mib_get_DesiredSSID(mac_vap);
...
uint32_t mac_mib_get_ssid(mac_vap_stru *mac_vap, uint8_t *len, uint8_t *param)
{
    ...
    *len = sizeof(mac_cfg_ssid_param_stru);
...
uint32_t mac_mib_set_beacon_period(mac_vap_stru *mac_vap, uint8_t len, uint8_t *param)
{
    ...
    mac_mib_set_BeaconPeriod(mac_vap, value);
...
uint32_t mac_mib_set_dtim_period(mac_vap_stru *mac_vap, uint8_t len, uint8_t *param)
{
    ...
    mac_mib_set_dot11dtimperiod(mac_vap, (uint32_t)l_value);
...
uint32_t mac_mib_set_shpreamble(mac_vap_stru *mac_vap, uint8_t len, uint8_t *param)
{
    ...
    if (l_value != 0) {
        mac_mib_set_ShortPreambleOptionImplemented(mac_vap, OAL_TRUE);
    } else {
        mac_mib_set_ShortPreambleOptionImplemented(mac_vap, OAL_FALSE);
    }
...
{
    *((int32_t *)param) = mac_mib_get_DesiredBSSType(mac_vap);
...
{
    *((uint32_t *)param) = mac_mib_get_BeaconPeriod(mac_vap);
...
{
    *((uint32_t *)param) = mac_mib_get_dot11dtimperiod(mac_vap);
...
uint32_t mac_mib_get_shpreamble(mac_vap_stru *mac_vap, uint8_t *len, uint8_t *param)
{
    ...
    l_value = mac_mib_get_ShortPreambleOptionImplemented(mac_vap);

    *((int32_t *)param) = l_value;
...
void mac_vap_init_mib_11ac(mac_vap_stru *pst_mac_vap)
{
    ...
    mac_mib_set_VHTShortGIOptionIn160and80p80Implemented(pst_mac_vap,
        (mac_device_get_cap_bw(pst_mac_dev) >= WLAN_BW_CAP_160M) ? WLAN_HAL0_VHT_SGI_SUPP_160_80P80 : OAL_FALSE);
    ...
    mac_mib_set_VHTBeamformeeNTxSupport(pst_mac_vap, g_wlan_spec_cfg->bfee_ntx_supports);
...
void mac_vap_init_mib_11i(mac_vap_stru *pst_vap)
{
    ...
    mac_mib_set_BeaconProtectionEnabled(pst_vap, OAL_FALSE);
}
...
void mac_init_mib(mac_vap_stru *mac_vap)
{
    ...
    if (oal_unlikely(mac_vap == NULL)) {
        oam_error_log0(0, OAM_SF_ANY, "{mac_init_mib::mac_vap null.}");
        return;
    }

    pst_mac_dev = mac_res_get_dev(mac_vap->uc_device_id);
    if (pst_mac_dev == NULL) {
        oam_error_log1(0, OAM_SF_ANY, "{mac_init_mib::pst_mac_dev[%d] null.}", mac_vap->uc_device_id);
        return;
    }

    /* 公共特性mib值初始化 */
    mac_mib_set_RTSThreshold(mac_vap, WLAN_RTS_MAX);
    mac_mib_set_FragmentationThreshold(mac_vap, WLAN_FRAG_THRESHOLD_MAX);

    mac_init_mib_dot11_station_config_entry(mac_vap, pst_mac_dev);
    mac_init_mib_dot11_phy_dsss_entry(mac_vap);

    /* 不同芯片，auth超时时间配置不同值 */
    wlan_chip_mac_mib_set_auth_rsp_time_out(mac_vap);

    mac_init_mib_dot11_operation_entry(mac_vap);

    mac_mib_set_dot11AssociationResponseTimeOut(mac_vap, WLAN_ASSOC_TIMEOUT);
    mac_mib_set_dot11AssociationSAQueryMaximumTimeout(mac_vap, WLAN_SA_QUERY_MAXIMUM_TIME);
    mac_mib_set_dot11AssociationSAQueryRetryTimeout(mac_vap, WLAN_SA_QUERY_RETRY_TIME);

    /* WEP 缺省Key表初始化 */
    for (uc_idx = 0; uc_idx < WLAN_NUM_DOT11WEPDEFAULTKEYVALUE; uc_idx++) {
        /* 大小初始化为 WEP-40 */
        mac_mib_set_wep(mac_vap, uc_idx, WLAN_WEP_40_KEY_SIZE);
    }

    /* 相关私有表初始化  */
    mac_mib_set_privacyinvoked(mac_vap, OAL_FALSE);
    mac_set_wep_default_keyid(mac_vap, 0);
    ...
    mac_vap_init_mib_11ac(mac_vap);
#ifdef _PRE_WLAN_FEATURE_11AX
    mac_vap_init_mib_11ax(mac_vap);
#endif

#ifdef _PRE_WLAN_FEATURE_11BE
    mac_vap_init_mib_11be(mac_vap);
#endif
#ifdef _PRE_WLAN_FEATURE_WUR
    mac_vap_init_mib_wur(mac_vap);
#endif
    /* staut低功耗mib项初始化 */
    mac_mib_set_powermanagementmode(mac_vap, WLAN_MIB_PWR_MGMT_MODE_ACTIVE);

    /* 11k */
    mac_vap_init_mib_11k(mac_vap);
    mac_init_mib_extend(mac_vap);
}
    ...
    wlan_channel_band_enum_uint8 band, wlan_channel_bandwidth_enum_uint8 bandwidth)
{
    ...
    pst_mib_info = mac_vap->pst_mib_info;

    if (pst_mib_info == NULL) {
        oam_error_log0(mac_vap->uc_vap_id, OAM_SF_ANY, "{mac_vap_change_mib_by_bandwidth::pst_mib_info null.}");
        return;
    }

    /* 更新40M使能mib, 默认使能 */
    mac_mib_set_FortyMHzOperationImplemented(mac_vap, OAL_TRUE);

    // 去掉shortGI强制使能，使用iwpriv命令设置，否则用户配置会被覆盖
    /* 更新short gi使能mib, 默认全使能，根据带宽信息更新 */
    mac_mib_set_ShortGIOptionInTwentyImplemented(mac_vap, OAL_TRUE);
    mac_mib_set_ShortGIOptionInFortyImplemented(mac_vap, OAL_TRUE);

    if ((mac_vap->st_channel.en_band == WLAN_BAND_2G) &&
        (mac_vap->st_cap_flag.bit_disable_2ght40 == OAL_TRUE)) {
        mac_mib_set_FortyMHzOperationImplemented(mac_vap, OAL_FALSE);
    }

    // 去掉shortGI强制使能，使用iwpriv命令设置，否则用户配置会被覆盖
    mac_mib_set_VHTShortGIOptionIn80Implemented(mac_vap, OAL_TRUE);

    en_bw = mac_vap_bw_mode_to_bw(bandwidth);
    ...
    en_40m_enable = (bandwidth != WLAN_BAND_WIDTH_20M);

    if (band == WLAN_BAND_2G) {
        mac_mib_set_2GFortyMHzOperationImplemented(mac_vap, en_40m_enable);
    } else {
        mac_mib_set_5GFortyMHzOperationImplemented(mac_vap, en_40m_enable);
    }
#endif
}
...
void mac_mib_set_wep(mac_vap_stru *mac_vap, uint8_t key_id, uint8_t key_value)
{
    ...
    pst_wlan_mib_wep_dflt_key = &(mac_vap->pst_mib_info->st_wlan_mib_privacy.ast_wlan_mib_wep_dflt_key[0]);
    ...
    /* 初始化wep相关MIB信息 */
    memset_s(pst_wlan_mib_wep_dflt_key[key_id].auc_dot11WEPDefaultKeyValue,
             WLAN_MAX_WEP_STR_SIZE, 0,
             WLAN_MAX_WEP_STR_SIZE);
    pst_wlan_mib_wep_dflt_key[key_id].auc_dot11WEPDefaultKeyValue[WLAN_WEP_SIZE_OFFSET] = key_value;
}
...
uint32_t mac_mib_init_ft_cfg(mac_vap_stru *mac_vap, uint8_t *mde)
{
    ...
    if (mac_mib_get_AuthenticationMode(mac_vap) != WLAN_WITP_AUTH_FT) {
        mac_mib_set_ft_trainsistion(mac_vap, OAL_FALSE);
        return OAL_SUCC;
    }

    if ((mde == NULL) || (mde[0] != MAC_EID_MOBILITY_DOMAIN) || (mde[1] != 3)) { /* MDIE lenth 3 */
        mac_mib_set_ft_trainsistion(mac_vap, OAL_FALSE);
        return OAL_SUCC;
    }

    mac_mib_set_ft_trainsistion(mac_vap, OAL_TRUE);
    mac_mib_set_ft_mdid (mac_vap, (uint16_t)((mde[BYTE_OFFSET_3] << BIT_OFFSET_8) | mde[BYTE_OFFSET_2]));
    mac_mib_set_ft_over_ds(mac_vap, (mde[BYTE_OFFSET_4] & BIT0) ? OAL_TRUE : OAL_FALSE);
    mac_mib_set_ft_resource_req(mac_vap, (mde[BYTE_OFFSET_4] & BIT1) ? OAL_TRUE : OAL_FALSE);
...
uint32_t mac_mib_get_md_id(mac_vap_stru *mac_vap, uint16_t *mdid)
{
    ...
    if (mac_mib_get_ft_trainsistion(mac_vap) == OAL_FALSE) {
        return OAL_FAIL;
    }

    *mdid = mac_mib_get_ft_mdid(mac_vap);
    return OAL_SUCC;
}
