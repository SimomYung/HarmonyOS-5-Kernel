...
OAL_STATIC hmac_vap_stru g_ast_hmac_vap[WLAN_VAP_MAX_NUM_LIMIT];
...
{
    if (oal_unlikely(vap_id == 0 || vap_id >= mac_chip_get_vap_support_max_num_limit())) {
        return NULL;
    }
...
void mac_res_vap_init(void)
{
    ...
    oal_queue_set(&(g_pst_mac_res->st_vap_res.st_queue),
                  ...
                  mac_chip_get_vap_support_max_num_limit());
    g_pst_mac_res->st_vap_res.us_hmac_priv_size = (uint16_t)OAL_OFFSET_OF(hmac_vap_stru, st_vap_base_info);

    for (loop = 0; loop < mac_chip_get_vap_support_max_num_limit(); loop++) {
...
{
    uint32_t id;
    if (oal_unlikely(idx >= mac_chip_get_vap_support_max_num_limit())) {
        return OAL_FAIL;
    }

    id = (uint32_t)mac_res_vap_id_special_config((uint8_t)idx);
    if (g_pst_mac_res->st_vap_res.auc_user_cnt[id] == 0) {
        oam_error_log1(0, OAM_SF_ANY, "mac_res_free_mac_vap::cnt==0! idx:%d", id);
        oal_dump_stack();
        return OAL_SUCC;
    }

    (g_pst_mac_res->st_vap_res.auc_user_cnt[id])--;
    ...
    /* 入队索引值需要加1操作 */
    oal_queue_enqueue(&(g_pst_mac_res->st_vap_res.st_queue), (void *)(uintptr_t)((unsigned long)id + 1));
...
/* 功能描述: 获取最大vap个数, 即配置vap个数+WIFI业务vap个数 */
uint8_t mac_chip_get_vap_support_max_num_limit(void)
{
    return g_wlan_spec_cfg->vap_support_max_num_limit;
}

/* 功能描述: 获取最大hal_vap个数 */
uint8_t mac_chip_get_hal_max_vap_num(void)
{
    return g_wlan_spec_cfg->hal_max_vap_num;
}

/* 功能描述  : 获取board上的业务vap的最大id限制，且id到不了该id号 */
uint8_t mac_chip_get_service_vap_limit_max_id(void)
{
#ifdef _PRE_DUAL_CHIP
    /* 兼容单芯片的场景，增加判断总线上是否是双芯片 */
    if (oal_bus_get_chip_num() == WLAN_CHIP_MAX_NUM_PER_BOARD) {
        /* 不修改device，host侧id max 和 id 1 做了互换，即使0和max都是配置vap，所以此处限制业务vap数量为(max数量 - 1） */
        return (mac_chip_get_vap_support_max_num_limit() - 1);
    }
#endif
    return mac_chip_get_vap_support_max_num_limit();
}