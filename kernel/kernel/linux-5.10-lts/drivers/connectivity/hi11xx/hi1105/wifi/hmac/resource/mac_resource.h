...
typedef struct {
    void *past_vap_info[WLAN_VAP_MAX_NUM_LIMIT];
    oal_queue_stru st_queue;
    unsigned long aul_idx[WLAN_VAP_MAX_NUM_LIMIT];
    uint16_t us_hmac_priv_size;
    uint8_t auc_user_cnt[WLAN_VAP_MAX_NUM_LIMIT];
    uint8_t auc_resv[NUM_2_BYTES];
} mac_res_vap_stru;
...
/*
 * 功能描述  : 特殊处理vap id, 当前仅用于处理双1105c场景
 * 1.日    期  : 2024年12月23日
 *   修改内容  : 新生成函数
 */
OAL_STATIC OAL_INLINE uint8_t mac_res_vap_id_special_config(uint8_t vap_id)
{
    uint8_t ret_id = vap_id;
    ...
    // 双芯片场景下 id 1 与 id 5 映射互换，保证device侧的业务vap不会配置到5
    if (vap_id == 1) {
        ret_id = EP_WIFI_DEV1_CFG_VAP_ID;
    } else if (vap_id == EP_WIFI_DEV1_CFG_VAP_ID) {
        ret_id = 1;
    }
#endif
    return ret_id;
...
OAL_STATIC OAL_INLINE uint32_t mac_res_alloc_hmac_vap(uint8_t *idx, uint16_t hmac_priv_size)
{
    ...
    *idx = mac_res_vap_id_special_config((*idx));
...
{
    if (oal_unlikely(idx >= WLAN_VAP_MAX_NUM_LIMIT)) {
        return OAL_FAIL;
    }
...
OAL_STATIC OAL_INLINE void *mac_res_get_hmac_vap(uint32_t idx)
{
    ...
    return (void *)(g_pst_mac_res->st_vap_res.past_vap_info[id]);
}
...
OAL_STATIC OAL_INLINE void *mac_res_get_mac_vap(uint8_t idx)
{
    ...
    id = mac_res_vap_id_special_config(idx);

    /* 这里返回偏移内存空间 */
    return (void *)((uint8_t *)(g_pst_mac_res->st_vap_res.past_vap_info[id]) +
