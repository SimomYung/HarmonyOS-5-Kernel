...
uint32_t mac_user_update_ap_bandwidth_cap(mac_user_stru *pst_mac_user)
{
    ...
    if (pst_mac_vht_hdl->en_vht_capable == OAL_TRUE) {
        ...
        if (pst_mac_vht_hdl->en_channel_width == WLAN_MIB_VHT_OP_WIDTH_80_160) {
        ...
        } else if (pst_mac_vht_hdl->en_channel_width == WLAN_MIB_VHT_OP_WIDTH_160_DEPRECATED) {
            pst_mac_user->en_bandwidth_cap = WLAN_BW_CAP_160M;
        } else if (pst_mac_vht_hdl->en_channel_width == WLAN_MIB_VHT_OP_WIDTH_80PLUS80_DEPRECATED) {
...
#endif
    /* EHT协议模式下，不支持320M带宽；160M以下带宽，从VHT就可以解析出来 */
...
uint32_t mac_user_init_rom(mac_user_stru *pst_mac_user, uint16_t us_user_idx)
{
    ...
    if (pst_mac_user->en_is_multi_user == OAL_TRUE) {
        pst_mac_user->en_port_valid = OAL_TRUE;
    }
    return OAL_SUCC;
}
...
{
    oam_warning_log3(pst_mac_user->uc_vap_id, OAM_SF_ANY, "mac_user_set_port::user[%d] old %d, new %d",
        pst_mac_user->us_assoc_id, pst_mac_user->en_port_valid, en_port_valid);
    pst_mac_user->en_port_valid = en_port_valid;
    dpe_user_set_port_sync(pst_mac_user->us_assoc_id, en_port_valid);
}
    ...
    uint8_t uc_device_id, uint8_t uc_vap_id)
{
    ...
    pst_mac_user->en_port_valid = OAL_FALSE;
    ...
    wlan_bw_cap_enum_uint8 *pen_bandwidth_cap)
{
    ...
    pst_mac_ht_hdl = &(pst_mac_user->st_ht_hdl);
    pst_mac_vht_hdl = &(pst_mac_user->st_vht_hdl);
    /* 11be无适配，因为eht CAP IE只携带了6G 320M的能力位，当前无支持320M带宽能力的0x芯片 */
#ifdef _PRE_WLAN_FEATURE_11AX
    if (g_wlan_spec_cfg->feature_11ax_is_open && (mac_user_is_he_user(pst_mac_user) == OAL_TRUE)) {
    ...
    if (pst_mac_vht_hdl->en_vht_capable == OAL_TRUE) {
        ...
        if (pst_mac_vap->st_cap_flag.bit_2040_autoswitch == OAL_TRUE && pst_mac_ht_hdl->bit_forty_mhz_intolerant == 1) {
            en_bandwidth_cap = WLAN_BW_CAP_20M;
        }
    } else {
        en_bandwidth_cap = WLAN_BW_CAP_20M;
...
void mac_user_set_asoc_state(mac_user_stru *pst_mac_user, mac_user_asoc_state_enum_uint8 en_value)
{
    ...
    dpe_wlan_user_assoc_state_sync(pst_mac_user->us_assoc_id, en_value);
}
/*
 ...
 *   修改内容  : 新生成函数
 */
void mac_user_get_vht_hdl(mac_user_stru *mac_user, mac_vht_hdl_stru *vht_hdl)
{
    int32_t l_ret;

    l_ret = memcpy_s((uint8_t *)vht_hdl, sizeof(mac_vht_hdl_stru),
                     (uint8_t *)(&mac_user->st_vht_hdl), sizeof(mac_vht_hdl_stru));
    if (l_ret != EOK) {
...
/*
 * 函 数 名   : mac_user_set_eht_capable
 * 功能描述   : 设置EHT cap 信息
 * 修改历史
 * 1.日    期   : 2023年9月25日
 *   修改内容   : 新生成函数
 */
void mac_user_set_eht_capable(mac_user_stru *mac_user, uint8_t eht_capable)
{
    if (oal_unlikely(mac_user == NULL)) {
        return;
    }

    mac_user->st_eht_hdl.en_eht_capable = eht_capable;
}

/*
 * 函 数 名   : mac_user_set_eht_operation
 * 功能描述   : 设置EHT operation标志位
 * 修改历史   :
 * 1.日    期   : 2023年9月25日
 *   修改内容   : 新生成函数
 */
void mac_user_set_eht_operation(mac_user_stru *mac_user, uint8_t eht_operation)
{
    if (oal_unlikely(mac_user == NULL)) {
        return;
    }

    mac_user->st_eht_hdl.eht_oper_flag = eht_operation;
}

/*
 * 函 数 名   : mac_user_get_eht_hdl
 * 功能描述   : 获取用户的eht信息
 * 修改历史   :
 * 1.日    期   : 2023年9月25日
 *   修改内容   : 新生成函数
 */
void mac_user_get_eht_hdl(mac_user_stru *mac_user, mac_eht_hdl_stru *eht_hdl)
{
    errno_t ret;
    if (oal_unlikely(oal_any_null_ptr2(mac_user, eht_hdl))) {
        return;
    }

    ret = memcpy_s((uint8_t *)eht_hdl, sizeof(mac_eht_hdl_stru), (uint8_t *)(&mac_user->st_eht_hdl),
                   sizeof(mac_eht_hdl_stru));
    if (oal_unlikely(ret != EOK)) {
        oam_error_log1(0, OAM_SF_ANY, "{mac_user_get_eht_hdl::memory copy failed[%d].}", ret);
    }
}

/*
 * 函 数 名   : mac_user_set_eht_hdl
 * 功能描述   : 设置用户的eht信息
 * 修改历史   :
 * 1.日    期   : 2023年9月25日
 *   修改内容   : 新生成函数
 */
void mac_user_set_eht_hdl(mac_user_stru *mac_user, mac_eht_hdl_stru *eht_hdl)
{
    errno_t ret;
    if (oal_unlikely(oal_any_null_ptr2(mac_user, eht_hdl))) {
        return;
    ...
    ret = memcpy_s((uint8_t *)(&mac_user->st_eht_hdl), sizeof(mac_eht_hdl_stru), (uint8_t *)eht_hdl,
                   sizeof(mac_eht_hdl_stru));
    if (oal_unlikely(ret != EOK)) {
        oam_error_log1(0, OAM_SF_ANY, "{mac_user_set_eht_hdl::memory copy failed[%d].}", ret);
    }
}

/*
 * 函 数 名   : mac_user_is_eht_user
 * 功能描述   : 判断用户是否支持EHT协议模式
 * 修改历史   :
 * 1.日    期   : 2023年9月28日
 *   修改内容   : 新生成函数
 */
uint8_t mac_user_is_eht_user(mac_user_stru *mac_user)
{
...
void mac_user_set_wur_capable(mac_user_stru *mac_user, uint8_t wur_capable)
{
    if (oal_unlikely(mac_user == NULL)) {
        return;
    }

    mac_user->wur_hdl.wur_capable = wur_capable;
}

void mac_user_set_wur_operation(mac_user_stru *mac_user, uint8_t wur_operation)
{
    if (oal_unlikely(mac_user == NULL)) {
        return;
    }

    mac_user->wur_hdl.wur_oper_flag = wur_operation;
}

void mac_user_get_wur_hdl(mac_user_stru *mac_user, mac_wur_hdl_stru *wur_hdl)
{
    errno_t ret;
    if (oal_unlikely(oal_any_null_ptr2(mac_user, wur_hdl))) {
        return;
    }

    ret = memcpy_s((uint8_t *)wur_hdl, sizeof(mac_wur_hdl_stru), (uint8_t *)(&mac_user->wur_hdl),
        sizeof(mac_wur_hdl_stru));
    if (oal_unlikely(ret != EOK)) {
        oam_error_log1(0, OAM_SF_ANY, "{mac_user_get_wur_hdl::memory copy failed[%d].}", ret);
    }
}

void mac_user_set_wur_hdl(mac_user_stru *mac_user, mac_wur_hdl_stru *wur_hdl)
{
    errno_t ret;
    if (oal_unlikely(oal_any_null_ptr2(mac_user, wur_hdl))) {
        return;
    ...
    ret = memcpy_s((uint8_t *)(&mac_user->wur_hdl), sizeof(mac_wur_hdl_stru), (uint8_t *)wur_hdl,
                   sizeof(mac_wur_hdl_stru));
    if (oal_unlikely(ret != EOK)) {
        oam_error_log1(0, OAM_SF_ANY, "{mac_user_set_wur_hdl::memory copy failed[%d].}", ret);
    }
}

uint8_t mac_user_is_wur_user(mac_user_stru *mac_user)
{
    return mac_user->wur_hdl.wur_capable;
}
