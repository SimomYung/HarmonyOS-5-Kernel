...
typedef enum {
    MAC_TX_INVALID = 0, /* 无效 */
    MAC_TX_SUCC = 1, /* 成功 */
    MAC_TX_FAIL = 2, /* 发送失败 (超时或者响应帧错误) */
} mac_tx_dscr_status_enum;
...
typedef struct {
    ...
    /* com_alg调用，不作编程规范整改 */
    mac_eht_hdl_stru st_eht_hdl; /* EHT Capability IE */
#endif
    ...
    mac_wur_hdl_stru wur_hdl; /* 保存user的wur cap、 wur operation */
...
{
    return (oal_bool_enum_uint8)(((macaddr_is_zero) != OAL_TRUE) && ((pairwise) == OAL_TRUE));
}
/*
 ...
 *   修改内容  : 新生成函数
 */
OAL_STATIC OAL_INLINE uint8_t mac_user_get_lut_index(uint8_t *puc_index_table,
...
/*
 * 函 数 名  : mac_user_del_lut_index
 * 功能描述  : 删除用户对应的lut index
 * 1.日    期  : 2013年6月7日
 *   修改内容  : 新生成函数
 */
OAL_STATIC OAL_INLINE void mac_user_del_lut_index(uint8_t *puc_index_table, uint8_t uc_ra_lut_index)
{
