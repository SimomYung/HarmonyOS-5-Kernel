...
mac_vap_stru *mac_vap_find_another_up_vap_by_mac_vap(mac_vap_stru *mac_vap)
{
    mac_device_stru *pst_mac_device = mac_res_get_dev(mac_vap->uc_device_id);
    ...
    /* 获取另一个vap */
    pst_another_vap_up = mac_device_find_another_up_vap(pst_mac_device, mac_vap->uc_vap_id);
...
oal_bool_enum_uint8 mac_get_rx_6g_flag_mp16(mac_rx_ctl_stru *rx_ctrl)
{
    return rx_ctrl->is_6ghz_flag;
}

oal_bool_enum_uint8 mac_get_rx_6g_flag_mp13(mac_rx_ctl_stru *rx_ctrl)
{
    return OAL_FALSE;
}
...
void mac_vap_tx_data_set_user_htc_cap(mac_vap_stru *pst_mac_vap, mac_user_stru *pst_mac_user)
{
    if (g_wlan_spec_cfg->feature_11ax_is_open) {
        ...
        if ((pst_mac_vap->bit_htc_include_custom_switch == OAL_TRUE) &&
            (mac_vap_is_he_eht_protocol(pst_mac_vap) == OAL_TRUE) && mac_user_is_he_user(pst_mac_user)) {
...
#ifdef _PRE_WLAN_FEATURE_11AX
/*
    he_chan_width含义: HE PHY Cap带宽能力位
    ——bit0: 2.4G 20/40M是否支持
    ——bit1：5G 40M/80M是否支持
    ——bit2: 5G 160M是否支持
    vht_oper_chan_width：vht/he/eht带宽参考此字段获取
    ——0:20M/40M
    ——1:80M/160M/80+80M
    ——2:160M（已废弃）
    ——3:80+80M（已废弃）
*/
wlan_bw_cap_enum_uint8 mac_get_bw_cap_from_he_cap_for_5g(uint8_t he_chan_width, uint8_t vht_oper_chan_width)
{
    if ((he_chan_width & BIT2) != 0 && vht_oper_chan_width >= WLAN_MIB_VHT_OP_WIDTH_80_160) {
    ...
    if ((he_chan_width & BIT1) != 0 && (vht_oper_chan_width >= WLAN_MIB_VHT_OP_WIDTH_80_160)) {
        return WLAN_BW_CAP_80M;
    }
    if ((he_chan_width & BIT1) != 0 && (vht_oper_chan_width == WLAN_MIB_VHT_OP_WIDTH_20_40)) {
        return WLAN_BW_CAP_40M;
    }
    return WLAN_BW_CAP_20M;
}
wlan_channel_bandwidth_enum_uint8 mac_get_bandwith_from_center_freq_seg0_seg1_by_bw_cap(
    wlan_bw_cap_enum_uint8 bw_cap, uint8_t vht_oper_chan_width, uint8_t channel, uint8_t chan_center_freq0,
    uint8_t chan_center_freq1)
{
#ifdef _PRE_WLAN_FEATURE_160M
    if (bw_cap == WLAN_BW_CAP_160M) {
        ...
        } else if (vht_oper_chan_width == WLAN_MIB_VHT_OP_WIDTH_160_DEPRECATED) {
    ...
    if (bw_cap == WLAN_BW_CAP_40M) {
        return mac_get_40mbandwith_from_offset_channel((int8_t)(chan_center_freq0 - channel));
    }
    return WLAN_BAND_WIDTH_20M;
}
wlan_channel_bandwidth_enum_uint8 mac_get_he_bw_from_vht_hdl(mac_vap_stru *mac_sta, uint8_t he_chan_width,
    uint8_t vht_chan_width, uint8_t ccfs0, uint8_t ccfs1)
{
    wlan_bw_cap_enum_uint8 bw_cap;
    wlan_channel_bandwidth_enum_uint8 bandwidth_ap;
    if (mac_sta->st_channel.en_band == WLAN_BAND_2G) {
        bandwidth_ap = mac_get_bandwith_from_center_freq_seg0_seg1(vht_chan_width,
            mac_sta->st_channel.uc_chan_number, ccfs0, ccfs1);
    } else if (mac_sta->st_channel.en_band == WLAN_BAND_5G) {
        // 5G带宽需要参考he phy cap能力，解决与V721光猫兼容性问题（HE带宽与vht填写不一致）
        bw_cap = mac_get_bw_cap_from_he_cap_for_5g(he_chan_width, vht_chan_width);
        bandwidth_ap = mac_get_bandwith_from_center_freq_seg0_seg1_by_bw_cap(
            bw_cap, vht_chan_width, mac_sta->st_channel.uc_chan_number, ccfs0, ccfs1);
    } else {
    ...
    mac_vap_stru *mac_sta, mac_user_stru *mac_user, wlan_channel_bandwidth_enum_uint8 pre_bandwidth_ap)
{
    ...
    if (g_wlan_spec_cfg->feature_11ax_is_open == OAL_FALSE || (mac_user_is_he_user(mac_user) == OAL_FALSE)) {
        return bandwidth_ap;
    }
    ...
    // 大部分ap vht present标记置0
    if (mac_he_hdl->st_he_oper_ie.st_he_oper_param.bit_vht_operation_info_present == OAL_FALSE) {
        mac_user_get_vht_hdl(mac_user, &vht_hdl);
        bandwidth_ap = mac_get_he_bw_from_vht_hdl(mac_sta, mac_he_hdl->st_he_cap_ie.st_he_phy_cap.bit_channel_width_set,
            vht_hdl.en_channel_width, vht_hdl.uc_channel_center_freq_seg0, vht_hdl.uc_channel_center_freq_seg1);
    }
    if (mac_he_hdl->st_he_oper_ie.st_he_oper_param.bit_vht_operation_info_present == OAL_TRUE) {
        bandwidth_ap = mac_get_he_bw_from_vht_hdl(mac_sta, mac_he_hdl->st_he_cap_ie.st_he_phy_cap.bit_channel_width_set,
...
#ifdef _PRE_WLAN_FEATURE_11BE
/*
 * 函 数 名  : mac_vap_get_ap_bandwidth_in_eht_opern
 * 功能描述  : 获取EHT模式下eht opern info子字段存在时 ap的工作带宽
 */
OAL_STATIC wlan_channel_bandwidth_enum_uint8 mac_vap_get_ap_bandwidth_in_eht_opern(
    mac_vap_stru *mac_sta, mac_user_stru *mac_user, wlan_channel_bandwidth_enum_uint8 pre_bandwidth_ap)
{
    ...
    if ((mac_user_is_eht_user(mac_user) == OAL_FALSE) ||
        (mac_eht_hdl->eht_oper_ie.eht_oper_parameters.eht_operation_information_present == OAL_FALSE)) {
        return bandwidth_ap;
    }

    bandwidth_ap = mac_get_eht_bandwith_from_center_freq_seg0_seg1(
        mac_eht_hdl->eht_oper_ie.eht_oper_info.channel_width,
        mac_sta->st_channel.uc_chan_number,
        mac_eht_hdl->eht_oper_ie.eht_oper_info.ccfs0,
        mac_eht_hdl->eht_oper_ie.eht_oper_info.ccfs1);
    ...
    wlan_channel_bandwidth_enum_uint8 *en_bandwidth_ap)
{
    ...
    **********************************************************************/
    ...
    if (pst_mac_vht_hdl->en_vht_capable == OAL_TRUE) {
        if (pst_mac_ht_hdl->bit_supported_channel_width == 0) {
...
#ifdef _PRE_WLAN_FEATURE_11BE
    *en_bandwidth_ap = mac_vap_get_ap_bandwidth_in_eht_opern(pst_mac_sta, pst_mac_user, *en_bandwidth_ap);
#endif
    /* ht 20/40M带宽的处理 */
    if ((pst_mac_ht_hdl->en_ht_capable == OAL_TRUE) && (*en_bandwidth_ap <= WLAN_BAND_WIDTH_40MINUS) &&
        (mac_mib_get_FortyMHzOperationImplemented(pst_mac_sta) == OAL_TRUE)) {
        /* 更新带宽模式 */
...
#ifdef _PRE_WLAN_CHBA_MGMT
    /* chba连接发起方视为AP设备, 其vap带宽在建链时不更新  */
    if (mac_is_chba_mode(mac_vap) == OAL_TRUE) {
        oam_warning_log2(mac_vap->uc_vap_id, OAM_SF_ASSOC, "mac_vap_set_bw_check:chba vap_bw[%d], new_user_bw[%d]",
...
void mac_vap_update_txopps(mac_vap_stru *pst_vap, mac_user_stru *pst_user)
{
    ...
    if ((pst_user->st_vht_hdl.bit_vht_txop_ps == OAL_TRUE) && (mac_mib_get_txopps(pst_vap) == OAL_TRUE)) {
...
void mac_vap_set_assoc_id(mac_vap_stru *pst_vap, uint16_t us_assoc_vap_id)
{
    ...
    // dpe模式下需要改为抛事件
    dpe_vap_set_assoc_id(pst_vap->uc_vap_id, us_assoc_vap_id);
}
 ...
 */
uint32_t mac_vap_save_app_ie(mac_vap_stru *mac_vap, oal_app_ie_stru *app_ie, en_app_ie_type_uint8 en_type)
{
    uint8_t           *puc_ie = NULL;
    uint32_t           ie_len = app_ie->ie_len;
    ...
    if ((mac_vap->ast_app_ie[en_type].ie_max_len < ie_len) || (mac_vap->ast_app_ie[en_type].puc_ie == NULL)) {
        ...
        if (puc_ie == NULL) {
            oam_warning_log2(mac_vap->uc_vap_id, OAM_SF_CFG, "{mac_vap_save_app_ie::LOCAL_MEM_POOL is empty!,\
                len[%d], en_type[%d].}", app_ie->ie_len, en_type);
 ...
 *   修改内容  : 函数拆分
 */
OAL_STATIC void mac_vap_update_mib_by_protocol(mac_vap_stru *pst_mac_vap, wlan_protocol_enum_uint8 protocol_mode)
{
    /* 初始化false */
    ...
    if (g_wlan_spec_cfg->feature_11be_is_open) {
        mac_mib_set_EHTOptionImplemented(pst_mac_vap->pst_mib_info, OAL_FALSE);
    }
#endif
    ...
    switch (protocol_mode) {
        ...
        case WLAN_EHT_MODE:
            ...
            mac_mib_set_VHTOptionImplemented(pst_mac_vap, OAL_TRUE);
            mac_mib_set_HEOptionImplemented(pst_mac_vap, OAL_TRUE);
            mac_mib_set_EHTOptionImplemented(pst_mac_vap->pst_mib_info, OAL_TRUE);
            break;
#endif
        default :
            break;
 ...
 *   修改内容  : 新生成函数
 */
uint32_t mac_vap_config_mib_by_protocol(mac_vap_stru *pst_mac_vap)
{
    ...
    /* 根据协议模式更新 HT/VHT/HE/EHT mib值 */
    mac_vap_update_mib_by_protocol(pst_mac_vap, pst_mac_vap->en_protocol);
 ...
 *   修改内容  : 新生成函数
 */
wlan_bw_cap_enum_uint8 mac_vap_bw_mode_to_bw(wlan_channel_bandwidth_enum_uint8 en_mode_temp)
{
    wlan_channel_bandwidth_enum_uint8 en_mode = en_mode_temp;
...
{
    if (memcpy_s(mac_vap->auc_bssid, WLAN_MAC_ADDR_LEN, bssid, WLAN_MAC_ADDR_LEN) != EOK) {
...
void mac_vap_state_change(mac_vap_stru *pst_mac_vap, mac_vap_state_enum_uint8 en_vap_state)
{
    ...
    dpe_wlan_vap_state_sync(pst_mac_vap->uc_vap_id, &en_vap_state);
...
void mac_vap_get_bandwidth_cap(mac_vap_stru *mac_vap, wlan_bw_cap_enum_uint8 *pen_cap)
{
    ...
    if ((mac_vap->st_channel.en_bandwidth == WLAN_BAND_WIDTH_40PLUS) ||
        ...
        if ((mac_vap->st_cap_flag.bit_2040_autoswitch == OAL_TRUE) && (mac_vap->en_40m_intol_user == OAL_TRUE)) {
            en_band_cap = WLAN_BW_CAP_20M;
...
wlan_channel_bandwidth_enum_uint8 mac_vap_cap_40m_get_bandwith(wlan_channel_bandwidth_enum_uint8 bss_bandwith)
{
    ...
    } else if ((bss_bandwith >= WLAN_BAND_WIDTH_80PLUSPLUS) && (bss_bandwith <= WLAN_BAND_WIDTH_80PLUSMINUS)) {
        band_with = WLAN_BAND_WIDTH_40PLUS;
    } else if ((bss_bandwith >= WLAN_BAND_WIDTH_80MINUSPLUS) && (bss_bandwith <= WLAN_BAND_WIDTH_80MINUSMINUS)) {
        band_with = WLAN_BAND_WIDTH_40MINUS;
    }
#ifdef _PRE_WLAN_FEATURE_160M
    if ((bss_bandwith >= WLAN_BAND_WIDTH_160PLUSPLUSPLUS) && (bss_bandwith <= WLAN_BAND_WIDTH_160PLUSMINUSMINUS)) {
        band_with = WLAN_BAND_WIDTH_40PLUS;
    } else if ((bss_bandwith >= WLAN_BAND_WIDTH_160MINUSPLUSPLUS) &&
...
wlan_channel_bandwidth_enum_uint8 mac_vap_cap_80m_get_bandwith(wlan_channel_bandwidth_enum_uint8 bss_bandwith)
{
...
#ifdef _PRE_WLAN_FEATURE_160M
    if ((bss_bandwith >= WLAN_BAND_WIDTH_160PLUSPLUSPLUS) && (bss_bandwith <= WLAN_BAND_WIDTH_160PLUSPLUSMINUS)) {
        band_with = WLAN_BAND_WIDTH_80PLUSPLUS;
    } else if ((bss_bandwith >= WLAN_BAND_WIDTH_160PLUSMINUSPLUS) &&
    ...
    } else if ((bss_bandwith >= WLAN_BAND_WIDTH_160MINUSMINUSPLUS) &&
        (bss_bandwith <= WLAN_BAND_WIDTH_160MINUSMINUSMINUS)) {
...
#endif
            break;
        /* 07C不支持320M带宽，不做适配 en_bss_cap判断提前返回 */
        case WLAN_BW_CAP_320M:
        default:
...
oal_bool_enum_uint8 mac_vap_check_bss_cap_info_phy_ap(uint16_t us_cap_info, mac_vap_stru *pst_mac_vap)
{
    ...
    /* PBCC */
    if ((mac_mib_get_PBCCOptionImplemented(pst_mac_vap) == OAL_FALSE) &&
    ...
    /* Channel Agility */
    if ((mac_mib_get_ChannelAgilityPresent(pst_mac_vap) == OAL_FALSE) &&
    ...
    if ((mac_mib_get_DSSSOFDMOptionActivated(pst_mac_vap) == OAL_FALSE) &&
...
wlan_prot_mode_enum_uint8 mac_vap_get_user_protection_mode(mac_vap_stru *mac_vap_sta, mac_user_stru *mac_user)
{
    ...
    if (oal_any_null_ptr2(mac_vap_sta, mac_user)) {
        return en_protection_mode;
    }

    /* 在2G频段下，如果AP发送的beacon帧ERP ie中Use Protection bit置为1，则将保护级别设置为ERP保护 */
    if ((mac_vap_sta->st_channel.en_band == WLAN_BAND_2G) &&
        (mac_user->st_cap_info.bit_erp_use_protect == OAL_TRUE)) {
        en_protection_mode = WLAN_PROT_ERP;
    } else if ((mac_user->st_ht_hdl.bit_ht_protection == WLAN_MIB_HT_NON_HT_MIXED) ||
        (mac_user->st_ht_hdl.bit_ht_protection == WLAN_MIB_HT_NONMEMBER_PROTECTION)) {
        /* 如果AP发送的beacon帧ht operation ie中ht protection字段为mixed或non-member，则将保护级别设置为HT保护 */
        en_protection_mode = WLAN_PROT_HT;
    } else if (mac_user->st_ht_hdl.bit_nongf_sta_present == OAL_TRUE) {
    ...
    mac_crypto_settings_stru             *crypto)
{
    ...
    mac_mib_set_BeaconProtectionEnabled(mac_vap, beacon_param->beacon_protection_enabled);
    return OAL_SUCC;
}
...
oal_bool_enum_uint8 mac_vap_roaming_is_link_down_vap(mac_vap_stru *mac_vap)
{
    ...
    return roam_info->is_link_down_vap;
}
...
oal_bool_enum_uint8 mac_vap_is_up(mac_vap_stru *mac_vap)
{
    ...
    return ((mac_vap->en_vap_state == MAC_VAP_STATE_UP) ||
            ...
            mac_vap_roaming_is_link_down_vap(mac_vap) == OAL_FALSE) ||
