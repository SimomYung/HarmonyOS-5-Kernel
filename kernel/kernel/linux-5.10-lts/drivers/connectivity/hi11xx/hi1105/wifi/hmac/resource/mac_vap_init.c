...
void mac_vap_init_mib_11n_txbf(mac_vap_stru *mac_vap)
{
    ...
    mac_device_stru *pst_dev = mac_res_get_dev(mac_vap->uc_device_id);
    if (pst_dev == NULL) {
        return;
    }

    mac_mib_set_TransmitStaggerSoundingOptionImplemented(mac_vap, mac_device_get_cap_subfer(pst_dev));
    mac_mib_set_ReceiveStaggerSoundingOptionImplemented(mac_vap, mac_device_get_cap_subfee(pst_dev));
    mac_mib_set_ExplicitCompressedBeamformingFeedbackOptionImplemented(mac_vap, WLAN_MIB_HT_ECBF_DELAYED);
    mac_mib_set_NumberCompressedBeamformingMatrixSupportAntenna(mac_vap, HT_BFEE_NTX_SUPP_ANTA_NUM);
#else
    mac_mib_set_TransmitStaggerSoundingOptionImplemented(mac_vap, OAL_FALSE);
    mac_mib_set_ReceiveStaggerSoundingOptionImplemented(mac_vap, OAL_FALSE);
    mac_mib_set_ExplicitCompressedBeamformingFeedbackOptionImplemented(mac_vap, WLAN_MIB_HT_ECBF_INCAPABLE);
    mac_mib_set_NumberCompressedBeamformingMatrixSupportAntenna(mac_vap, 1);
#endif
}
...
uint32_t mac_vap_protocol_init(mac_vap_stru *pst_vap, mac_device_stru *pst_mac_device)
{
    switch (pst_mac_device->en_protocol_cap) {
        ...
        case WLAN_PROTOCOL_CAP_EHT:
            if (g_wlan_spec_cfg->feature_11be_is_open) {
                pst_vap->en_protocol = g_pst_mac_device_capability[0].switch_11be ?
                    WLAN_EHT_MODE : WLAN_HE_MODE;
                break;
            }
            oam_warning_log1(pst_vap->uc_vap_id, OAM_SF_CFG,
                "{mac_vap_init::en_protocol_cap[%d] is not supportted.}", pst_mac_device->en_protocol_cap);
    ...
    mac_cfg_add_vap_param_stru *param)
{
    ...
    if (g_optimized_feature_switch_bitmap & BIT(CUSTOM_OPTIMIZE_FEATURE_CERTIFY_MODE)) {
        vap->st_cap_flag.bit_2040_autoswitch = OAL_TRUE;
    } else {
...
#ifdef _PRE_WLAN_FEATURE_11BE
void mac_vap_init_rates_in_eht_protocol(mac_vap_stru *vap, mac_device_stru *mac_dev, mac_data_rate_stru *rates)
{
    if (vap->en_vap_mode == WLAN_VAP_MODE_BSS_STA) {
        /* 用于STA全信道扫描 5G时 填写支持速率集ie */
        mac_vap_init_legacy_rates(vap, rates);
    ...
    mac_vap_init_11ac_rates(vap, mac_dev);
    mac_vap_init_11ax_rates(vap, mac_dev);
    mac_vap_init_11be_rates(vap, mac_dev);
}
#endif

/*
 * 功能描述  : 初始化非HE EHT协议模式的速率集
 * 1.日    期  : 2023年10月31日
 *   修改内容  : 新生成函数，拆分大函数
 */
OAL_STATIC void mac_vap_init_rates_in_non_he_eht_protocol(mac_vap_stru *mac_vap, mac_device_stru *mac_dev,
    wlan_protocol_enum_uint8 vap_protocol, mac_data_rate_stru *rates)
{
    if (mac_vap->en_vap_mode == WLAN_VAP_MODE_BSS_STA && vap_protocol == WLAN_VHT_MODE) {
        /* 用于STA全信道扫描 5G时 填写支持速率集ie */
        mac_vap_init_legacy_rates(mac_vap, rates);
        memcpy_s(&mac_vap->ast_sta_sup_rates_ie[WLAN_BAND_5G], sizeof(mac_curr_rateset_stru),
                 &mac_vap->st_curr_sup_rates, sizeof(mac_vap->st_curr_sup_rates));

        /* 用于STA全信道扫描 2G时 填写支持速率集ie */
        mac_vap_init_11g_mixed_one_rates(mac_vap, rates);
        memcpy_s(&mac_vap->ast_sta_sup_rates_ie[WLAN_BAND_2G], sizeof(mac_curr_rateset_stru),
                 &mac_vap->st_curr_sup_rates, sizeof(mac_vap->st_curr_sup_rates));

        mac_vap_init_11n_rates(mac_vap, mac_dev);
        mac_vap_init_11ac_rates(mac_vap, mac_dev);
    } else if (oal_value_eq_any2(vap_protocol, WLAN_VHT_ONLY_MODE, WLAN_VHT_MODE)) {
#ifdef _PRE_WLAN_FEATURE_11AC2G
        (mac_vap->st_channel.en_band == WLAN_BAND_2G) ? mac_vap_init_11g_mixed_one_rates(mac_vap, rates) :
            mac_vap_init_legacy_rates(mac_vap, rates);
#else
        mac_vap_init_legacy_rates(mac_vap, rates);
#endif
        mac_vap_init_11n_rates(mac_vap, mac_dev);
        mac_vap_init_11ac_rates(mac_vap, mac_dev);
    } else if (oal_value_eq_any2(vap_protocol, WLAN_HT_ONLY_MODE, WLAN_HT_MODE)) {
        if (mac_vap->st_channel.en_band == WLAN_BAND_5G) {
            mac_vap_init_legacy_rates(mac_vap, rates);
        } else if (mac_vap->st_channel.en_band == WLAN_BAND_2G) {
            mac_vap_init_11g_mixed_one_rates(mac_vap, rates);
        }

        mac_vap_init_11n_rates(mac_vap, mac_dev);
    } else if (oal_value_eq_any2(vap_protocol, WLAN_LEGACY_11A_MODE, WLAN_LEGACY_11G_MODE)) {
        mac_vap_init_legacy_rates(mac_vap, rates);
...
void mac_vap_init_rates_by_protocol(mac_vap_stru *mac_vap,
    wlan_protocol_enum_uint8 vap_protocol, mac_data_rate_stru *rates)
{
    ...
    if (!is_legacy_vap(mac_vap)) {
        mac_vap_init_p2p_rates(mac_vap, vap_protocol, rates);
        return;
    }

#ifdef _PRE_WLAN_FEATURE_11BE
    if (vap_protocol == WLAN_EHT_MODE) {
        mac_vap_init_rates_in_eht_protocol(mac_vap, mac_dev, rates);
    }
#endif

#ifdef _PRE_WLAN_FEATURE_11AX
    if (g_wlan_spec_cfg->feature_11ax_is_open && vap_protocol == WLAN_HE_MODE) {
        mac_vap_init_rates_in_he_protocol(mac_vap, mac_dev, rates);
    }
#endif

    /* 初始化非HE EHT协议模式的速率集 */
    mac_vap_init_rates_in_non_he_eht_protocol(mac_vap, mac_dev, vap_protocol, rates);
}
...
#endif
    oam_warning_log4(pst_mac_vap->uc_vap_id, OAM_SF_CFG, "{mac_vap_init_rx_nss_by_protocol:protocol[%d], device nss[%d]\
        vap_rx_nss[%d], mac device smps mode[%d].}", en_protocol, nss_num_cap, pst_mac_vap->en_vap_rx_nss,
        mac_device_get_mode_smps(pst_mac_device));
}
...
uint32_t mac_vap_init_by_protocol(mac_vap_stru *pst_mac_vap, wlan_protocol_enum_uint8 en_protocol)
{
    ...
    /* 根据协议模式更新mib值 */
    if (mac_vap_config_mib_by_protocol(pst_mac_vap) != OAL_SUCC) {
        return OAL_FAIL;
    }
 ...
 *   修改内容  : 新生成函数
 */
void mac_sta_init_bss_rates(mac_vap_stru *mac_vap, mac_bss_dscr_stru *bss_dscr)
{
    mac_device_stru               *pst_mac_dev;
    wlan_protocol_enum_uint8       en_vap_protocol;
    mac_data_rate_stru            *pst_rates = NULL;
    ...
    if (pst_mac_dev == NULL) {
        oam_error_log1(mac_vap->uc_vap_id, OAM_SF_ANY, "{mac_vap_init_rates::pst_mac_dev[%d] null.}",
    ...
    if (bss_dscr == NULL) {
        en_vap_protocol = mac_vap->en_protocol;
        mac_vap_init_rates_by_protocol(mac_vap, en_vap_protocol, pst_rates);
        return ;
    }
 ...
 *   修改内容  : 新生成函数
 */
uint32_t mac_vap_init_privacy(mac_vap_stru *mac_vap, mac_conn_security_stru *conn_sec)
{
    ...
    mac_mib_set_rsnaactivated(mac_vap, OAL_FALSE);
    /* 清除加密套件信息 */
    mac_mib_init_rsnacfg_suites(mac_vap);

    mac_vap->st_cap_flag.bit_wpa = OAL_FALSE;
    mac_vap->st_cap_flag.bit_wpa2 = OAL_FALSE;

    /*  刷新pmf能力,解决从pmf ap下关联至open ap, pmf未更新导致收发异常 */
#ifdef _PRE_WLAN_FEATURE_PMF
    ret = mac_vap_init_pmf(mac_vap, conn_sec->en_pmf_cap, conn_sec->en_mgmt_proteced);
    if (ret != OAL_SUCC) {
    ...
    if (conn_sec->uc_wep_key_len != 0) {
        return mac_vap_add_wep_key(mac_vap, conn_sec->uc_wep_key_index,
    ...
    /* 初始化RSNA mib 为 TRUR */
    mac_mib_set_privacyinvoked(mac_vap, OAL_TRUE);
    mac_mib_set_rsnaactivated(mac_vap, OAL_TRUE);

    /* 初始化单播密钥套件 */
    if (pst_crypto->wpa_versions == WITP_WPA_VERSION_1) {
        mac_vap->st_cap_flag.bit_wpa = OAL_TRUE;
        mac_mib_set_wpa_pair_suites(mac_vap, pst_crypto->aul_pair_suite);
        mac_mib_set_wpa_akm_suites(mac_vap, pst_crypto->aul_akm_suite);
        mac_mib_set_wpa_group_suite(mac_vap, pst_crypto->group_suite);
    } else if (pst_crypto->wpa_versions == WITP_WPA_VERSION_2) {
        mac_vap->st_cap_flag.bit_wpa2 = OAL_TRUE;
        mac_mib_set_rsn_pair_suites(mac_vap, pst_crypto->aul_pair_suite);
        mac_mib_set_rsn_akm_suites(mac_vap, pst_crypto->aul_akm_suite);
        mac_mib_set_rsn_group_suite(mac_vap, pst_crypto->group_suite);
        mac_mib_set_rsn_group_mgmt_suite(mac_vap, pst_crypto->group_mgmt_suite);
    }
