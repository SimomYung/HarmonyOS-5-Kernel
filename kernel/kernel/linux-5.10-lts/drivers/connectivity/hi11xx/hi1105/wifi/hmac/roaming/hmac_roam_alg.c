...
void hmac_roam_alg_init(hmac_roam_info_stru *roam_info, int8_t current_rssi)
{
    uint8_t vap_id = 0;
    hmac_roam_alg_stru *roam_alg = NULL;

    if (roam_info == NULL) {
        oam_error_log0(0, OAM_SF_ROAM, "{hmac_roam_alg_init::roam_info null.}");
        return;
    }
    vap_id = (roam_info && roam_info->pst_hmac_vap) ? roam_info->pst_hmac_vap->st_vap_base_info.uc_vap_id : 0;
    oam_warning_log1(vap_id, OAM_SF_ROAM, "{hmac_roam_alg_init c_current_rssi = %d.}", current_rssi);
    roam_alg = &(roam_info->st_alg);
    if (current_rssi == ROAM_RSSI_LINKLOSS_TYPE) {
    ...
    mac_bss_dscr_stru *bss_dscr, int8_t c_target_weight_rssi)
{
    ...
    /* 当前关联5G时，需要对自身rssi做加权处理，target不区分2G/5G，避免5G之间rssi差异不大时发生漫游；5G target rssi 选网已加权 */
    if (mac_vap->st_channel.en_band == WLAN_BAND_5G) {
        ...
        oam_warning_log1(0, OAM_SF_ROAM, "{roam_alg_rssi_incr::cur 5g adjust cur_rssi:%d}", s_current_rssi);
    }
                                     ...
                                     roam_blacklist_type_enum list_type)
{
    ...
    if (oal_any_null_ptr3(roam_info, bssid, roam_info->pst_hmac_vap)) {
        return OAL_ERR_CODE_PTR_NULL;
    }
    hmac_device = hmac_res_get_mac_dev(roam_info->pst_hmac_vap->st_vap_base_info.uc_device_id);
    if (hmac_device == NULL) {
        return OAL_FAIL;
    }
    ret = hmac_roam_alg_add_bsslist(&hmac_device->st_blacklist, bssid, list_type);
    if (ret != OAL_SUCC) {
 ...
 *   修改内容  : 新生成函数
 */
oal_bool_enum_uint8 hmac_roam_alg_find_in_blacklist(hmac_roam_info_stru *roam_info, uint8_t *bssid)
{
    hmac_device_stru *hmac_device = NULL;
    ...
    /* 上层发起指定bssid漫游不检查黑名单，防止关联失败 */
    if (roam_info->en_roam_trigger == ROAM_TRIGGER_BSSID) {
        return OAL_FALSE;
    }
    return hmac_roam_alg_find_in_bsslist(&hmac_device->st_blacklist, bssid);
}
 ...
 *   修改内容  : 新生成函数
 */
uint32_t hmac_roam_alg_add_history(hmac_roam_info_stru *roam_info, uint8_t *bssid)
{
    uint32_t ret;

    if (oal_any_null_ptr2(roam_info, bssid)) {
        return OAL_ERR_CODE_PTR_NULL;
    }

    ret = hmac_roam_alg_add_bsslist(&roam_info->st_alg.st_history,
                                    bssid, ROAM_BLACKLIST_TYPE_NORMAL_AP);
    if (ret != OAL_SUCC) {
        oam_error_log1(0, OAM_SF_ROAM,
 ...
 *   修改内容  : 新生成函数
 */
oal_bool_enum_uint8 hmac_roam_alg_find_in_history(hmac_roam_info_stru *roam_info, uint8_t *bssid)
{
    if (oal_any_null_ptr2(roam_info, bssid)) {
        return OAL_FALSE;
    }

    return hmac_roam_alg_find_in_bsslist(&roam_info->st_alg.st_history, bssid);
}
...
void hmac_roam_neighbor_report_add_chan(mac_scan_req_stru *scan_params,
    uint8_t channum, wlan_channel_band_enum_uint8 en_band)
{
    uint8_t   loop;
    int ret = 0;
    uint8_t channel_idx;
    ...
    if (mac_get_channel_idx_from_num(en_band, channum, OAL_FALSE, &channel_idx) != OAL_SUCC) {
        oam_warning_log2(0, OAM_SF_ROAM, "{hmac_roam_neighbor_report_add_chan::invalid channel %d, band %d}",
            channum, en_band);
        return;
    }
 ...
 *   修改内容  : 新生成函数
 */
void hmac_roam_neighbor_report_add_bssid(hmac_roam_info_stru *roam_info, uint8_t *bssid)
{
    uint8_t *puc_idex = NULL;

    if (roam_info == NULL || bssid == NULL) {
...
/* 功能描述：判断扫描结果是否当前关联AP.
 *          非MLD 模式，判断BSSID是否相同;MLD 模式，判断mld_addr是否相同.
 * 返回值：OAL_TRUE:非当前关联AP; OAL_FALSE:当前关联AP
 */
static oal_bool_enum_uint8 hmac_roam_scan_is_another_bss(hmac_vap_stru *hmac_vap, mac_bss_dscr_stru *bss_dscr)
{
#ifdef _PRE_WLAN_FEATURE_ML
    ...
    if (hmac_vap_is_legacy_mld_mode(hmac_vap) == OAL_TRUE ||
        bss_dscr->ml_info.ml_cap == OAL_FALSE ||
        mld_vap_manage == NULL) {
        return (oal_compare_mac_addr(hmac_vap->st_vap_base_info.auc_bssid, bss_dscr->auc_bssid) != 0) ?
            OAL_TRUE : OAL_FALSE;
    }

    return (oal_compare_mac_addr(mld_vap_manage->mld_bssid, bss_dscr->ml_info.ml_mac_addr) != 0) ?
        OAL_TRUE : OAL_FALSE;
#else
    return (oal_compare_mac_addr(hmac_vap->st_vap_base_info.auc_bssid, bss_dscr->auc_bssid) != 0) ?
        OAL_TRUE : OAL_FALSE;
...
uint32_t hmac_roam_alg_bss_in_ess(hmac_roam_info_stru *roam_info, mac_bss_dscr_stru *bss_dscr)
{
    ...
    if (oal_compare_mac_addr(mac_vap->auc_bssid, bss_dscr->auc_bssid) != 0) {
        if (hmac_roam_scan_is_another_bss(hmac_vap, bss_dscr) == OAL_TRUE) {
...
#endif
    ie = mac_find_ie(MAC_EID_MOBILITY_DOMAIN, &ies_info);
    /* SAE不支持11R漫游 */
...
static uint32_t hmac_roam_check_cipher_limit(hmac_roam_info_stru *roam_info, mac_bss_dscr_stru *bss_dscr)
{
    ...
    if ((bss_dscr->puc_rsn_ie == NULL) &&
        ...
        oam_info_log0(0, OAM_SF_ROAM, "{hmac_roam_check_cipher_limit::check_privacy fail.}");
        return OAL_ERR_CODE_ROAM_NO_VALID_BSS;
    }

    /*  open加密方式到wpa/wpa2直接过滤掉 */
    if ((cap_info->bit_privacy == 0) != (mac_mib_get_privacyinvoked(mac_vap) != OAL_TRUE)) {
        oam_info_log0(0, OAM_SF_ROAM, "{hmac_roam_check_cipher_limit::open to wpa fail.}");
        return OAL_ERR_CODE_ROAM_NO_VALID_BSS;
    }
                                                                ...
                                                                uint8_t *mac_addr)
{
    ...
    if (roam_info == NULL || mac_addr == NULL) {
                                                   ...
                                                   mac_bss_dscr_stru *bss_dscr)
{
    ...
    if (roam_info->current_bss_policy == ROAM_IGNORE_CUR_BSS) {
...
OAL_STATIC uint32_t hmac_roam_check_p2p_dbac(mac_vap_stru *up_mac_vap[], mac_vap_stru *mac_vap,
    mac_bss_dscr_stru *bss_dscr, uint32_t up_vap_num, uint8_t state_trans)
{
    uint32_t i;
...
OAL_STATIC uint32_t hmac_roam_alg_check_cur_bss_info(hmac_vap_stru *hmac_vap,
    hmac_roam_info_stru *roam_info, mac_bss_dscr_stru *bss_dscr)
{
    mac_vap_stru *mac_vap = &(hmac_vap->st_vap_base_info);

    /* 检查漫游到自己 */
    if ((roam_info->current_bss_policy == ROAM_ONLY_CUR_BSS) &&
    ...
    if (roam_info->cur_mld_link_bss_policy == ROAM_ONLY_SAME_MLD_LINK_SWITCH ||
        roam_info->cur_mld_link_bss_policy == ROAM_ONLY_SAME_MLD_LINK_REASSOC) {
        if (hmac_roam_mld_check_target_bss_is_link_vap(hmac_vap, bss_dscr) != OAL_TRUE) {
            return OAL_ERR_CODE_ROAM_NO_VALID_BSS;
        }
    }
#endif
...
static uint32_t hmac_roam_alg_check_common_info(hmac_vap_stru *hmac_vap,
                                                ...
                                                mac_bss_dscr_stru *bss_dscr)
{
    ...
    ret = hmac_roam_alg_check_cur_bss_info(hmac_vap, roam_info, bss_dscr);
    if (ret != OAL_SUCC) {
        return OAL_ERR_CODE_ROAM_NO_VALID_BSS;
    }
    ...
    if (ret != OAL_SUCC) {
        oam_info_log1(mac_vap->uc_vap_id, OAM_SF_ROAM,
            "{hmac_roam_alg_check_common_info::hmac_roam_alg_check_channel fail[%d].}", ret);
    ...
    if (ret != OAL_SUCC) {
        oam_info_log1(mac_vap->uc_vap_id, OAM_SF_ROAM,
            "{hmac_roam_alg_check_common_info::check_aging_time fail[%d].}", ret);
        return OAL_ERR_CODE_ROAM_NO_VALID_BSS;
    }
    ...
    if (ret != OAL_SUCC) {
        oam_info_log1(mac_vap->uc_vap_id, OAM_SF_ROAM,
            "{hmac_roam_alg_check_common_info::check_bssid_limit fail[%d].}", ret);
        return OAL_ERR_CODE_ROAM_NO_VALID_BSS;
    }
    ...
    if (ret == OAL_TRUE) {
        oam_info_log1(mac_vap->uc_vap_id, OAM_SF_ROAM,
            "{hmac_roam_alg_check_common_info::blacklist_filter fail[%d].}", ret);
        return OAL_ERR_CODE_ROAM_NO_VALID_BSS;
    }
    ...
    if (roam_info->current_bss_policy == ROAM_IGNORE_CUR_BSS) {
        ...
        if (oal_compare_mac_addr(mac_vap->auc_bssid, bss_dscr->auc_bssid) == 0) {
            oam_info_log0(mac_vap->uc_vap_id, OAM_SF_ROAM,
                "{hmac_roam_alg_check_common_info::cur bss ignore.}");
            return OAL_ERR_CODE_ROAM_NO_VALID_BSS;
        }
    }
    ...
    if (hmac_roam_check_cipher_limit(roam_info, bss_dscr) != OAL_SUCC) {
        oam_info_log0(mac_vap->uc_vap_id, OAM_SF_ROAM,
            "{hmac_roam_alg_check_common_info::check_cipher_limit fail.}");
        return OAL_ERR_CODE_ROAM_NO_VALID_BSS;
    }
    return OAL_SUCC;
}

int8_t hmac_roam_alg_adjust_rssi_weight(mac_bss_dscr_stru *bss_dscr)
{
    int8_t rssi;
...
uint32_t hmac_roam_alg_bss_check_ignore_bss(hmac_roam_info_stru *roam_info,
    mac_bss_dscr_stru *bss_dscr, int8_t c_tmp_rssi)
{
    ...
    if (hmac_is_roam_weak_rssi_condition(bss_dscr->c_rssi, bss_dscr)) {
        oam_warning_log1(0, OAM_SF_ROAM, "{hmac_roam_alg_bss_check_ignore_bss::bss rssi[%d] is weak!}",
            bss_dscr->c_rssi);
        return OAL_ERR_CODE_ROAM_NO_VALID_BSS;
    }
    /* c_current_rssi为0时，表示linkloss上报的触发，不需要考虑rssi增益 */
    if (hmac_roam_alg_compare_rssi_increase(roam_info, bss_dscr, c_tmp_rssi) <= 0) {
        oam_info_log0(0, OAM_SF_ROAM, "{hmac_roam_alg_bss_check_ignore_bss compare_rssi_increase fail}");
        return OAL_ERR_CODE_ROAM_NO_VALID_BSS;
    }

    /* 11v漫游和指定bssid关联场景下，拒绝11v漫游 */
    if (hmac_roam_alg_compare_conflict_bssid(roam_info, bss_dscr) == OAL_TRUE) {
        oam_info_log0(0, OAM_SF_ROAM, "{hmac_roam_alg_bss_check_ignore_bss compare_conflict_bssid fail}");
        return OAL_ERR_CODE_ROAM_NO_VALID_BSS;
    }
    return OAL_SUCC;
}

static void hmac_roam_print_selected_bss_info(mac_bss_dscr_stru *bss_dscr, uint8_t in_type)
{
    oam_info_log4(0, OAM_SF_ROAM,
        "{hmac_roam_select_bss_info::in_type[%d],[%02X:XX:XX:XX:%02X:%02X]}",
        in_type, bss_dscr->auc_bssid[BYTE_OFFSET_0],
        bss_dscr->auc_bssid[BYTE_OFFSET_4], bss_dscr->auc_bssid[BYTE_OFFSET_5]);
}

void hmac_roam_update_preferred_bss(hmac_roam_alg_stru *roam_alg, mac_bss_dscr_stru *bss_dscr,
    int8_t c_rssi)
{
    if (oal_any_null_ptr2(roam_alg, bss_dscr)) {
        return;
    }
    roam_alg->c_max_rssi = c_rssi;
    roam_alg->pst_max_rssi_bss = bss_dscr;
}

void hmac_roam_clear_perferred_bss(hmac_roam_alg_stru *roam_alg)
{
    if (roam_alg == NULL) {
        oam_error_log0(0, OAM_SF_ROAM, "{hmac_roam_clear_perferred_bss::roam_alg is NULL}");
        return;
    }
    roam_alg->pst_max_rssi_bss = NULL;
}

static uint8_t hmac_roam_alg_bss_check_pmksa(hmac_vap_stru *hmac_vap, mac_bss_dscr_stru *bss_dscr)
{
    /* 1、AP能力与自己能力都满足才加分,目前是自己的能力取的不是漫游的能力待改进；2、wpa3 暂不加分 */
    if (hmac_vap->en_sae_connect == OAL_FALSE) {
        return hmac_vap_get_pmksa(hmac_vap, bss_dscr->auc_bssid) != NULL;
    }
    return OAL_FALSE;
}

uint32_t hmac_roam_check_reg_domain_valid(mac_bss_dscr_stru *bss_dscr)
{
    uint32_t ret = mac_is_channel_num_valid(bss_dscr->st_channel.en_band,
        bss_dscr->st_channel.uc_chan_number, bss_dscr->st_channel.ext6g_band);
    if (ret != OAL_SUCC) {
        oam_warning_log2(0, OAM_SF_ROAM, "{hmac_roam_check_reg_domain_valid::channel[%d] band[%d] invalid!}",
            bss_dscr->st_channel.uc_chan_number, bss_dscr->st_channel.en_band);
    }
    return ret;
}
...
#endif
    hmac_roam_print_selected_bss_info(bss_dscr, ROAM_SELECT_BSS_IN);

    if ((OAL_STRLEN(bss_dscr->ac_ssid) != cfg_ssid.uc_ssid_len) ||
        (oal_memcmp(cfg_ssid.ac_ssid, bss_dscr->ac_ssid, cfg_ssid.uc_ssid_len) != 0) ||
        hmac_roam_alg_check_common_info(hmac_vap, roam_info, bss_dscr) != OAL_SUCC) {
        return OAL_ERR_CODE_ROAM_NO_VALID_BSS;
    }

    /* 根据管制域判断AP是否可以漫游 */
    if (hmac_roam_check_reg_domain_valid(bss_dscr) != OAL_SUCC) {
        return OAL_ERR_CODE_ROAM_NO_VALID_BSS;
    }

#ifdef _PRE_WLAN_FEATURE_ML
    /* MLSR未工作link漫游策略 */
    if (hmac_roam_mld_bssid_check_ignore_bss(roam_info, hmac_vap, bss_dscr) == OAL_TRUE) {
        oam_warning_log0(0, OAM_SF_ROAM, "{hmac_roam_alg_bss_check::ignore mld_link bss}");
        return OAL_ERR_CODE_ROAM_NO_VALID_BSS;
    }
#endif
    if (hmac_is_roam_homenetwork_2gto5g_condition(hmac_vap, roam_info, bss_dscr)) {
        hmac_roam_update_preferred_bss(roam_alg, bss_dscr, bss_dscr->c_rssi);
        hmac_roam_print_selected_bss_info(bss_dscr, ROAM_SELECT_BSS_HOME_NETWORK);
        return OAL_SUCC;
    }
    c_tmp_rssi = hmac_roam_alg_adjust_rssi_weight(bss_dscr);
    if (hmac_roam_alg_bss_check_ignore_bss(roam_info, bss_dscr, c_tmp_rssi) != OAL_SUCC) {
        return OAL_ERR_CODE_ROAM_NO_VALID_BSS;
    }
    /* 对于已存在pmk缓存的bss进行加分处理 */
    if (hmac_roam_alg_bss_check_pmksa(hmac_vap, bss_dscr) == OAL_TRUE) {
        c_tmp_rssi += ROAM_RSSI_DIFF_4_DB;
    }

    if (hmac_roam_alg_compare_bss(hmac_vap, roam_alg, bss_dscr, c_tmp_rssi) == OAL_TRUE) {
        hmac_roam_update_preferred_bss(roam_alg, bss_dscr, c_tmp_rssi);
        hmac_roam_print_selected_bss_info(bss_dscr, ROAM_SELECT_BSS_UPDATE_BSS);
    }

    /* 当2G AP RSSI和5G AP加权RSSI相等时，优选5G低band AP */
    if ((c_tmp_rssi == roam_alg->c_max_rssi) && (roam_alg->pst_max_rssi_bss->st_channel.en_band == WLAN_BAND_2G) &&
        (bss_dscr->st_channel.en_band == WLAN_BAND_5G)) {
        hmac_roam_update_preferred_bss(roam_alg, bss_dscr, c_tmp_rssi);
        hmac_roam_print_selected_bss_info(bss_dscr, ROAM_SELECT_BSS_PREFER_5G);
    }
...
/* 函数功能：判断是否需要停止漫游
 * 返回值：OAL_TRUE:需要停止漫游；OAL_FALSE:不需要停止漫游
 */
...
mac_bss_dscr_stru *hmac_roam_alg_select_bss(hmac_roam_info_stru *roam_info)
{
    ...
    if (bss_dscr == NULL) {
        oam_info_log0(0, OAM_SF_ROAM, "{hmac_roam_alg_select_bss::no valid bss}");
        /* should not be here */
        return NULL;
    }
    ...
    if (delta_rssi <= 0) {
        oam_info_log4(0, OAM_SF_ROAM,
            "{hmac_roam_alg_select_bss::compare_rssi fail max_rssi=%d, [%02X:XX:XX:XX:%02X:%02X]}",
            roam_alg->c_max_rssi, bss_dscr->auc_bssid[BYTE_OFFSET_0],
            bss_dscr->auc_bssid[BYTE_OFFSET_4], bss_dscr->auc_bssid[BYTE_OFFSET_5]);
        return NULL;
    }
