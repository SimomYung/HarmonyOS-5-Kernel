...
void hmac_roam_alg_init(hmac_roam_info_stru *roam_info, int8_t current_rssi)
{
    ...
    oam_warning_log1(0, OAM_SF_ROAM, "{hmac_roam_alg_init c_current_rssi = %d.}", current_rssi);
...
int8_t hmac_roam_alg_adjust_rssi_weight(mac_bss_dscr_stru *bss_dscr, hmac_vap_stru *hmac_vap)
{
    int8_t rssi;
...
static void hmac_roam_update_preferred_bss(hmac_roam_alg_stru *roam_alg, mac_bss_dscr_stru *bss_dscr, int8_t c_rssi)
{
...
uint32_t hmac_roam_alg_bss_check(hmac_roam_info_stru *roam_info, mac_bss_dscr_stru *bss_dscr)
{
...
#ifdef _PRE_WLAN_FEATURE_ML
    /* 不选择同一个mld ap 的2g 5g */
    if (hmac_roam_mld_bssid_check(roam_info, hmac_vap, bss_dscr) == OAL_TRUE) {
        oam_warning_log0(0, OAM_SF_ROAM, "{hmac_roam_alg_bss_check::ignore same mld_ap bss}");
        return OAL_ERR_CODE_ROAM_NO_VALID_BSS;
    }
#endif
    ...
    c_tmp_rssi = hmac_roam_alg_adjust_rssi_weight(bss_dscr, hmac_vap);
