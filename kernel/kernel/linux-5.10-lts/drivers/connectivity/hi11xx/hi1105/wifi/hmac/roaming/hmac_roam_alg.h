...
typedef enum {
    ROAM_SELECT_BSS_IN = 0,
    ROAM_SELECT_BSS_HOME_NETWORK = 1,
    ROAM_SELECT_BSS_UPDATE_BSS = 2,
    ROAM_SELECT_BSS_PREFER_5G = 3,
} roam_selete_bss_enum;
...
typedef struct {
    uint8_t mld_scan;   /* 记录漫游扫描是否为mld scan；如果是漫游mld scan，mld_scan后不用重新选网，直接使用前一次漫游选网结果触发漫游关联 */
...
typedef struct {
    ...
    uint8_t max_rssi_bss_bssid[WLAN_MAC_ADDR_LEN];
} hmac_roam_alg_stru;
...
#endif
    uint8_t ssid[WLAN_SSID_MAX_LEN];         /* 加入的AP的SSID  */
...
/* 漫游主结构体 */
typedef struct hmac_roam_info_tag {
    ...
    oal_bool_enum_uint8 current_bss_policy; /* 是否支持漫游回自己(支持驱动发起重关联) */
    ...
    oal_bool_enum_uint8 is_support_dualband_roam;    /* 是否支持双频漫游   0:不支持 1:支持 */
    mac_bss_dscr_stru save_other_bss_dscr_info;      /* 保留mld漫游时当前选择的bss信息另一频段的bss dscr */
    uint8_t is_link_down_vap;
...
uint32_t hmac_roam_alg_add_blacklist(hmac_roam_info_stru *roam_info, uint8_t *bssid,
...
void hmac_roam_update_preferred_bss(hmac_roam_alg_stru *roam_alg, mac_bss_dscr_stru *bss_dscr, int8_t c_rssi);
