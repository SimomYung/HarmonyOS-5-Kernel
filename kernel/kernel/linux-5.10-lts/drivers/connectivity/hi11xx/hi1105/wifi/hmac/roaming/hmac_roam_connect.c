...
OAL_STATIC uint32_t hmac_roam_connect_succ(hmac_roam_info_stru *roam_info);
OAL_STATIC uint32_t hmac_roam_connect_succ_for_single_link(hmac_roam_info_stru *roam_info, void *param);
...
void hmac_roam_connect_fsm_init(void)
{
    ...
    g_hmac_roam_connect_fsm_func[ROAM_CONNECT_STATE_HANDSHAKING][ROAM_CONNECT_FSM_EVENT_KEY_DONE] =
        hmac_roam_connect_succ_for_single_link;
 ...
 *   修改内容  : 新生成函数
 */
void hmac_roam_connect_change_state_for_single_link(hmac_roam_info_stru *roam_info,
    roam_connect_state_enum state)
{
    uint32_t ret;
    uint8_t h2d_sync_state = (uint8_t)state;
    ...
    oam_warning_log2(mac_vap->uc_vap_id, OAM_SF_ROAM,
        "{hmac_roam_connect_change_state::[%d]->[%d]}", roam_info->st_connect.en_state, state);
    roam_info->st_connect.en_state = state;
    ret = hmac_config_send_event(mac_vap, WLAN_CFGID_VAP_ROAM_CONN_STATE_SYN, sizeof(h2d_sync_state), &h2d_sync_state);
    if (ret != OAL_SUCC) {
        oam_warning_log1(0, OAM_SF_CFG, "{hmac_roam_connect_change_state:hmac_config_send_event failed[%d].}", ret);
    }
}

#ifdef _PRE_WLAN_FEATURE_ML
static void hmac_roam_connect_change_state_for_ml(hmac_vap_stru *hmac_vap, roam_connect_state_enum state)
{
    uint8_t               link_id;
    ...
    if (mld_vap_manage == NULL) {
        oam_error_log0(hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_CFG,
            "{hmac_roam_connect_change_state_for_ml::get mld_vap_mgmt fail.");
        return;
    }
    ...
    for (link_id = 0; link_id <  WLAN_MAX_ML_LINK_NUM; link_id++) {
        ...
        if (link_roam_info == NULL || link_roam_info->en_main_state != ROAM_MAIN_STATE_CONNECTING) {
            continue;
        }

        /* 切换slave link的漫游关联子状态机 */
        hmac_roam_connect_change_state_for_single_link(link_roam_info, state);
    }
}
#endif
                                               ...
                                               roam_connect_state_enum state)
{
    ...
    if (hmac_vap_is_legacy_mld_mode(hmac_vap) == OAL_TRUE) {
        hmac_roam_connect_change_state_for_single_link(roam_info, state);
        return;
    }
    /* 关联子状态机两个link联动 */
#ifdef _PRE_WLAN_FEATURE_ML
    hmac_roam_connect_change_state_for_ml(hmac_vap, state);
#endif
}
                                                  ...
                                                  roam_connect_state_enum connect_state)
{
    ...
    if (hmac_vap_is_link_vap(roam_info->pst_hmac_vap) == OAL_TRUE) {
        return OAL_SUCC;
    }
...
static void hmac_roam_connect_del_timer_for_single_link(hmac_roam_info_stru *roam_info)
{
...
OAL_STATIC void hmac_roam_connect_del_timer(hmac_roam_info_stru *roam_info)
{
    ...
    hmac_roam_info_stru  *link_roam_info = NULL;
#endif
    hmac_roam_connect_del_timer_for_single_link(roam_info);
#ifdef _PRE_WLAN_FEATURE_ML
    hmac_vap = roam_info->pst_hmac_vap;
    if (hmac_vap == NULL) {
        return;
    ...
    for (link_id = 0; link_id < WLAN_MAX_ML_LINK_NUM; link_id++) {
        ...
        if (link_roam_info->st_connect.st_timer.en_is_registerd) {
            oam_error_log1(link_hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_CFG,
                "{hmac_roam_connect_del_timer::main state[%d]!=connecting.", link_roam_info->en_main_state);
        }
    }
#endif
}

static void hmac_roam_connect_set_join_reg_update_mld_addr(hmac_vap_stru *hmac_vap, mac_bss_dscr_stru *bss_dscr)
{
#ifdef _PRE_WLAN_FEATURE_ML
    ...
    if (mld_vap_manage != NULL) {
        oal_set_mac_addr(bss_dscr->ml_info.ml_mac_addr, mld_vap_manage->mld_bssid);
    } else {
        oal_set_mac_addr(bss_dscr->ml_info.ml_mac_addr, mac_vap->auc_bssid);
    }
#endif
}

static uint32_t hmac_roam_back_set_join_dscr(hmac_vap_stru *hmac_vap, hmac_user_stru *hmac_user,
    mac_bss_dscr_stru *join_bss_dscr)
{
    mac_vap_stru *mac_vap = &hmac_vap->st_vap_base_info;

    oal_set_mac_addr(join_bss_dscr->auc_bssid, mac_vap->auc_bssid);

    /* 设置需要写入寄存器的BSSID信息 */
    hmac_roam_connect_set_join_reg_update_mld_addr(hmac_vap, join_bss_dscr);

    /* 以old user信息塑造虚假的入网结构体，调用函数 */
    /* 填写速率相关信息 */
    join_bss_dscr->uc_num_supp_rates = hmac_user->st_op_rates.uc_rs_nrates;
    if (memcpy_s(join_bss_dscr->auc_supp_rates, sizeof(uint8_t) * WLAN_USER_MAX_SUPP_RATES,
        hmac_user->st_op_rates.auc_rs_rates, sizeof(uint8_t) * WLAN_MAX_SUPP_RATES) != EOK) {
        oam_error_log0(0, OAM_SF_ROAM, "hmac_roam_back_set_join_dscr::memcpy fail!");
        return OAL_FAIL;
    }
    join_bss_dscr->en_ht_capable = hmac_user->st_user_base_info.st_ht_hdl.en_ht_capable;
    join_bss_dscr->en_vht_capable = hmac_user->st_user_base_info.st_vht_hdl.en_vht_capable;

    /* 设置dtim period信息 */
    join_bss_dscr->us_beacon_period = (uint16_t)mac_mib_get_BeaconPeriod(mac_vap);

    join_bss_dscr->sap_mode = hmac_user->sap_mode;
#ifdef _PRE_WLAN_FEATURE_WUR
    join_bss_dscr->wur_capable = mac_mib_get_WUROptionImplemented(mac_vap);
#endif
#ifdef _PRE_WLAN_FEATURE_EMLSR
    join_bss_dscr->hisi_emlsr_capable = mac_mib_get_EHTEMLSROptionActivated(mac_vap);
#endif
    return OAL_SUCC;
}
 ...
 *   修改内容  : 新生成函数
 */
uint32_t hmac_roam_connect_set_join_reg(hmac_vap_stru *hmac_vap, hmac_user_stru *hmac_user)
{
    mac_vap_stru *mac_vap = &hmac_vap->st_vap_base_info;
    ...
    mac_bss_dscr_stru *join_bss_dscr = &st_join_req.st_bss_dscr;
    memset_s(&st_join_req, sizeof(hmac_join_req_stru), 0, sizeof(hmac_join_req_stru));

    if (hmac_roam_back_set_join_dscr(hmac_vap, hmac_user, join_bss_dscr) != OAL_SUCC) {
        return OAL_FAIL;
    }
    ...
    frw_event_hdr_init(&(event->st_event_hdr), FRW_EVENT_TYPE_WLAN_CTX,
                       DMAC_WLAN_CTX_EVENT_SUB_TYPE_JOIN_SET_REG, sizeof(dmac_ctx_join_req_set_reg_stru),
    ...
    hmac_sta_update_h2d_join_req_params(hmac_vap, reg_params, join_bss_dscr);
                                             ...
                                             hmac_asoc_rsp_stru asoc_rsp, frw_event_mem_stru *event_mem)
{
    ...
    /* wpa_supplicant: wlan0: WPA: EAPOL-Key Replay Counter did not increase - dropping packet */
        oam_warning_log3(hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_ROAM,
                         "{hmac_roam_connect_notify_wpas::roam_to the same bssid [%02X:XX:XX:XX:%02X:%02X]}",
                         roam_rsp.auc_bssid[BYTE_OFFSET_0],
...
static void hmac_roam_notify_wpas_port_authed_roam_process_single_link(hmac_roam_info_stru *roam_info)
{
...
void hmac_roam_notify_wpas_port_authed_roam_process(hmac_vap_stru *hmac_vap, hmac_roam_info_stru *roam_info)
{
#ifdef _PRE_WLAN_FEATURE_ML
    uint8_t               link_id;
    hmac_mld_manage_stru *mld_vap_manage = NULL;
...
#endif
    if (hmac_vap_is_legacy_mld_mode(hmac_vap) == OAL_TRUE) {
        hmac_roam_notify_wpas_port_authed_roam_process_single_link(roam_info);
        return;
    }
    ...
    mld_vap_manage = hmac_vap->mld_vap_link_info.mld_vap_manage;
    if (mld_vap_manage == NULL) {
        oam_error_log0(0, OAM_SF_CFG, "{hmac_roam_notify_wpas_port_authed_roam_process::get mld_vap_mgmt fail.");
        return;
    }
    ...
    for (link_id = 0; link_id < mld_vap_manage->mld_link_cnt; link_id++) {
        ...
        /* 如果是master link，不在循环内切换 */
        if (link_roam_info == NULL) {
            oam_error_log0(link_hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_CFG,
                "{hmac_roam_notify_wpas_port_authed_roam_process::link_roam_info NULL.");
            continue;
        }
        if (link_roam_info->en_main_state == ROAM_MAIN_STATE_INIT) {
            oam_warning_log1(link_hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_CFG,
                "{hmac_roam_notify_wpas_port_authed_roam_process::roam main state[%d].", link_roam_info->en_main_state);
            continue;
        }
        /* slave link的漫游结果处理 */
        hmac_roam_notify_wpas_port_authed_roam_process_single_link(link_roam_info);
    }
    /* master link的漫游结果处理 */
    hmac_roam_notify_wpas_port_authed_roam_process_single_link(roam_info);
#endif
...
void hmac_roam_notify_wpas_port_authed(hmac_vap_stru *hmac_vap)
{
    ...
    /* 停止漫游状态机跳转, 删除相关定时器 */
    hmac_roam_notify_wpas_port_authed_roam_process(hmac_vap, roam_info);
...
#ifdef _PRE_WLAN_FEATURE_ML
    hmac_cofeture_state_change_notify_sta(hmac_vap, COFEATURE_TYPE_MLO, hmac_get_mlo_work_state(hmac_vap), 0);
#endif
...
OAL_STATIC uint32_t hmac_roam_process_ft_rsp(hmac_roam_info_stru *roam_info, void *param)
{
    ...
    mac_hdr = (uint8_t *)(uintptr_t)mac_get_rx_cb_mac_header_addr(rx_ctrl);
                                            ...
                                            uint8_t *buffer_len, uint32_t buffer_max_len)
{
    ...
    ies_info.frame_ies = mac_vap->ast_app_ie[OAL_APP_ASSOC_REQ_IE].puc_ie; /* app_le */
    ies_info.frame_len = mac_vap->ast_app_ie[OAL_APP_ASSOC_REQ_IE].ie_len; /* app_ie len */
    ie = mac_find_ie_ext_ie(MAC_EID_EXTENSION, MAC_EID_EXT_OWE_DH_PARAM, &ies_info);
    if (ie == NULL) {
...
OAL_STATIC uint32_t hmac_roam_set_rsnx_ie(hmac_vap_stru *hmac_vap, uint8_t *buffer,
    uint32_t left_len, uint8_t *bssid)
{
    ...
    mac_rsnx_cap_stru *rsnx_cap = NULL;

    if (left_len < sizeof(mac_rsnx_cap_stru) + MAC_IE_HDR_LEN) {
        return 0;
    }

    /* 漫游选网会判断，如果双方都支持akm8或akm24，en_sae_connect会置true，此时需要携带RSNX。 */
    ...
    bss_info = hmac_scan_find_scanned_bss_by_bssid(bss_mgmt, bssid, OAL_MAC_ADDR_LEN);
    if (bss_info == NULL) {
        return 0;
    }
    if (bss_info->st_bss_dscr_info.mgmt_len <= MAC_80211_QOS_HTC_4ADDR_FRAME_LEN) {
        return 0;
    }

    /* MAC_EID_WPS,MAC_EID_P2P ID均为221 */
    ie = mac_find_ie_in_frame(MAC_EID_RSNX, bss_info->st_bss_dscr_info.auc_mgmt_frame_body,
        bss_info->st_bss_dscr_info.mgmt_len - MAC_80211_QOS_HTC_4ADDR_FRAME_LEN);
    /* 扫描结果没有rsnx，直接返回 */
    if (ie == NULL) {
        oam_warning_log0(mac_vap->uc_vap_id, OAM_SF_ROAM, "hmac_roam_set_rsnx_ie::not find rsnx ie");
        return 0;
    }
    if (memset_s(buffer, left_len, 0, sizeof(mac_rsnx_cap_stru) + MAC_IE_HDR_LEN) != EOK) {
        return 0;
    }
    buffer[BYTE_OFFSET_0] = MAC_EID_RSNX;
    buffer[BYTE_OFFSET_1] = (uint8_t)sizeof(mac_rsnx_cap_stru);
    /* wpa3默认采用h2e模式进行加密 */
    rsnx_cap = (mac_rsnx_cap_stru *)(buffer + BYTE_OFFSET_2);
    rsnx_cap->bit_sae_h2e = OAL_TRUE;

    return MAC_IE_HDR_LEN + buffer[BYTE_OFFSET_1];
}
...
OAL_STATIC void hmac_roam_set_app_ie(hmac_vap_stru *hmac_vap, mac_bss_dscr_stru *bss_dscr)
{
    ...
    if (hmac_vap->st_vap_base_info.st_cap_flag.bit_wpa2 == OAL_TRUE) {
        ...
        pmkid = hmac_vap_roam_get_pmksa(hmac_vap, bss_dscr);
        ie_len = mac_set_rsn_ie(&hmac_vap->st_vap_base_info, pmkid, app_ie.auc_ie);
        ie_len += hmac_roam_set_rsnx_ie(hmac_vap, app_ie.auc_ie + ie_len, WLAN_WPS_IE_MAX_SIZE - ie_len,
            bss_dscr->auc_bssid);
        /* 设置OWE DH参数     */
...
static uint32_t hmac_roam_start_join_pre_check(mac_device_stru *mac_dev, hmac_roam_info_stru *roam_info)
{
    uint32_t ret;

    if (mac_dev == NULL) {
        oam_error_log0(0, OAM_SF_ROAM, "hmac_roam_start_join_pre_check::mac_dev is NULL!");
        return OAL_FAIL;
    }
    ret = hmac_roam_connect_check_state(roam_info, MAC_VAP_STATE_ROAMING,
                                        ROAM_MAIN_STATE_CONNECTING, ROAM_CONNECT_STATE_INIT);
    if (ret != OAL_SUCC) {
        oam_warning_log1(0, OAM_SF_ROAM, "{hmac_roam_start_join_pre_check::check_state fail[%d]!}", ret);
        return ret;
    }
    return OAL_SUCC;
}

uint32_t hmac_roam_handle_start_join(hmac_vap_stru *hmac_vap, mac_bss_dscr_stru *bss_dscr)
{
    uint32_t ret;
    uint8_t rate_num;
    ...
    if (memcpy_s(hmac_vap->auc_supp_rates, WLAN_MAX_SUPP_RATES, bss_dscr->auc_supp_rates, rate_num) != EOK) {
        oam_error_log0(0, OAM_SF_ROAM, "hmac_roam_handle_start_join::memcpy fail!");
        return OAL_FAIL;
    }
    ...
    if (ret != OAL_SUCC) {
        oam_error_log1(0, OAM_SF_ROAM, "{hmac_roam_handle_start_join::hmac_sta_update_join_req_params fail[%d].}", ret);
        return ret;
    }
    ...
    if (ret != OAL_SUCC) {
        oam_error_log1(0, OAM_SF_ROAM, "{hmac_roam_handle_start_join::hmac_roam_connect_set_dtim_param fail[%d].}",
            ret);
    }
    return ret;
}
...
OAL_STATIC uint32_t hmac_roam_start_join(hmac_roam_info_stru *roam_info, void *param)
{
    ...
    ret = hmac_roam_start_join_pre_check(mac_dev, roam_info);
    if (ret != OAL_SUCC) {
        return ret;
    }
    ret = hmac_roam_handle_start_join(hmac_vap, bss_dscr);
    if (ret != OAL_SUCC) {
        return ret;
    }

    /* 非关联link不走 */
    if (hmac_vap_is_link_mode(hmac_vap)) {
        /* 更新用户mac */
        oal_set_mac_addr(roam_info->pst_hmac_user->st_user_base_info.auc_user_mac_addr,
                         roam_info->st_connect.pst_bss_dscr->auc_bssid);
        dpe_wlan_user_addr_sync(roam_info->pst_hmac_user->st_user_base_info.us_assoc_id,
            roam_info->st_connect.pst_bss_dscr->auc_bssid);
        return OAL_SUCC;
    }
    ...
    if (ret != OAL_SUCC) {
        ...
        /* 通知ROAM主状态机 */
        hmac_roam_connect_complete(hmac_vap, roam_info, OAL_FAIL);
...
OAL_STATIC uint32_t hmac_roam_triger_sae_auth(hmac_roam_info_stru *roam_info)
{
    ...
    oal_set_mac_addr(hmac_user->st_user_base_info.auc_user_mac_addr,
                     roam_info->st_connect.pst_bss_dscr->auc_bssid);
    dpe_wlan_user_addr_sync(hmac_user->st_user_base_info.us_assoc_id, roam_info->st_connect.pst_bss_dscr->auc_bssid);
...
static void hmac_roam_okc_update_app_ie(hmac_vap_stru *hmac_vap, mac_bss_dscr_stru *bss_dscr)
{
    uint32_t rsn_akm_suites[WLAN_PAIRWISE_CIPHER_SUITES] = { 0 };
    uint32_t akm_num;
    ...
    akm_num = mac_mib_get_rsn_akm_suites_s(&hmac_vap->st_vap_base_info, rsn_akm_suites, sizeof(rsn_akm_suites));
    if (hmac_key_mgmt_8021x(rsn_akm_suites, akm_num) != OAL_TRUE) {
        return;
    }
    oam_warning_log0(0, OAM_SF_ROAM, "{hmac_roam_okc_update_app_ie::is 802.1x, update app ie");
    hmac_roam_set_app_ie(hmac_vap, bss_dscr);
}

static void hmac_roam_set_reassoc_req_cb(oal_netbuf_stru *assoc_req_frame, uint16_t assoc_len, uint16_t user_idx)
{
    mac_tx_ctl_stru *tx_ctl = (mac_tx_ctl_stru *)oal_netbuf_cb(assoc_req_frame);
    if (tx_ctl == NULL) {
        return;
    }
    mac_get_cb_mpdu_len(tx_ctl) = assoc_len;
...
OAL_STATIC uint32_t hmac_roam_send_reassoc_req(hmac_roam_info_stru *roam_info)
{
    ...
    hmac_roam_okc_update_app_ie(hmac_vap, roam_info->st_connect.pst_bss_dscr);
    ...
    /* 为填写发送描述符准备参数 */
    hmac_roam_set_reassoc_req_cb(assoc_req_frame, (uint16_t)assoc_len, hmac_user->st_user_base_info.us_assoc_id);
 ...
 * 修改内容 : 新生成函数
 */
OAL_STATIC uint32_t hmac_roam_rx_auth_check_sae(hmac_vap_stru *hmac_vap, hmac_roam_info_stru *roam_info,
    dmac_wlan_crx_event_stru *crx_event, uint16_t auth_status, uint16_t auth_seq_num)
{
    ...
    /* 与TP-LINK BE3600重关联，ap回复auth_status=temp_reject。未超过auth最大次数，staut尝试重试 */
    if (auth_status == MAC_REJECT_TEMP && roam_info->st_connect.uc_auth_num < MAX_AUTH_CNT) {
        /* 更新AUTH的次数 */
        roam_info->st_connect.uc_auth_num++;
        /* 重新发起关联动作 */
        hmac_roam_triger_sae_auth(roam_info);
        return OAL_SUCC;
    }
...
#endif
#ifdef _PRE_WLAN_FEATURE_11R
uint32_t hmac_11r_report_connect_failed_result(hmac_vap_stru *hmac_vap)
{
    if (hmac_vap->bit_11r_enable == OAL_TRUE && hmac_vap->bit_11r_auth_fail == OAL_TRUE) {
        hmac_vap->bit_11r_auth_fail = OAL_FALSE;
        hmac_proc_disasoc_inform(hmac_vap, MAC_DEAUTH_LV_SS, OAL_TRUE);
        oam_warning_log0(hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_SAE,
            "{hmac_11r_report_connect_failed_result::11r connect fail,report disassoc comp to kernel}");
        return OAL_SUCC;
    }
    return OAL_FAIL;
}
    ...
    hmac_roam_info_stru *roam_info, uint8_t *payload, uint16_t payload_len)
{
    ...
    ret = hmac_sta_initiate_join(&hmac_vap->st_vap_base_info, &roam_info->save_bss_dscr_info);
    // 如果join失败，上报上层去关联完成；发帧失败，由关联状态机完成上报
    if (ret != SUCC) {
        hmac_vap->bit_11r_auth_fail = OAL_FALSE;
        hmac_proc_disasoc_inform(hmac_vap, MAC_DEAUTH_LV_SS, OAL_TRUE);
    }
...
#ifdef _PRE_WLAN_FEATURE_SAE
    if (mac_mib_get_AuthenticationMode(&(hmac_vap->st_vap_base_info)) == WLAN_WITP_AUTH_SAE) {
        return hmac_roam_rx_auth_check_sae(hmac_vap, roam_info, crx_event, auth_status, auth_seq_num);
    }
#endif
    ...
    dmac_wlan_crx_event_stru *crx_event)
{
    ...
    if (asoc_status != MAC_SUCCESSFUL_STATUSCODE) {
        ...
        if ((hmac_vap->st_vap_base_info.en_user_pmf_cap == OAL_TRUE) && (asoc_status == MAC_REJECT_TEMP) &&
            (oal_compare_mac_addr(bss_addr, roam_info->st_old_bss.auc_bssid) == 0)) {
            return OAL_SUCC_GO_ON;
        }
#endif
        return OAL_SUCC;
...
OAL_STATIC uint32_t hmac_roam_process_connect_result(hmac_roam_info_stru *roam_info)
{
    ...
    if (mac_mib_get_privacyinvoked(&hmac_vap->st_vap_base_info) != OAL_TRUE) {
        ...
        hmac_roam_connect_succ(roam_info);
    } else {
#ifdef _PRE_WLAN_FEATURE_11R
        if (hmac_vap->bit_11r_enable == OAL_TRUE) {
...
static uint32_t hmac_roam_process_assoc_rsp_frame(hmac_vap_stru *hmac_vap, hmac_user_stru *hmac_user, uint8_t *mac_hdr,
...
#ifdef _PRE_WLAN_FEATURE_ML
    ret = hmac_mld_process_asoc_rsp(hmac_vap, hmac_user, mac_hdr, payload, msg_len);
    if (ret != OAL_SUCC) {
        oam_warning_log1(mac_vap->uc_vap_id, OAM_SF_ASSOC,
                         "{hmac_roam_process_assoc_rsp_frame::hmac_process_assoc_rsp mld failed[%d].}", ret);
        return ret;
    }
#endif
    ret = hmac_process_assoc_rsp(hmac_vap, hmac_user, mac_hdr, hdr_len, payload, msg_len);
    if (ret != OAL_SUCC) {
        oam_warning_log1(mac_vap->uc_vap_id, OAM_SF_ROAM,
            "{hmac_roam_process_assoc_rsp_frame:proc_asoc_rsp fail[%d].}", ret);
        return ret;
    }
    return OAL_SUCC;
}

OAL_STATIC void hmac_roam_succ_report_trigger_type(hmac_vap_stru *hmac_vap, hmac_roam_info_stru *roam_info)
{
...
OAL_STATIC uint32_t hmac_roam_process_assoc_rsp(hmac_roam_info_stru *roam_info, void *param)
{
    ...
    if (hmac_roam_process_assoc_rsp_frame(hmac_vap, hmac_user, mac_hdr, rx_ctrl) != OAL_SUCC) {
        return OAL_FAIL;
    }
#ifdef _PRE_WLAN_FEATURE_ML
    hmac_link_sta_wait_asoc_rx_succ_handle(hmac_vap);
#endif
    ...
    hmac_roam_succ_report_trigger_type(hmac_vap, roam_info);
...
OAL_STATIC uint32_t hmac_roam_connect_succ(hmac_roam_info_stru *roam_info)
{
    ...
    /* 删除定时器 */
    hmac_roam_connect_del_timer(roam_info); // link不需要，他没有起连接定时器

    /* 通知ROAM主状态机 */
    hmac_roam_connect_complete(roam_info->pst_hmac_vap, roam_info, OAL_SUCC);
    return OAL_SUCC;
}
...
OAL_STATIC uint32_t hmac_roam_connect_succ_for_single_link(hmac_roam_info_stru *roam_info, void *param)
{
    ...
    ret = hmac_roam_connect_check_state(roam_info, MAC_VAP_STATE_ROAMING,
                                        ROAM_MAIN_STATE_CONNECTING, ROAM_CONNECT_STATE_HANDSHAKING);
    if (ret != OAL_SUCC) {
        oam_warning_log1(0, OAM_SF_ROAM, "{hmac_roam_connect_succ_for_single_link::check_state fail[%d]!}", ret);
        return ret;
    }

    hmac_roam_connect_change_state_for_single_link(roam_info, ROAM_CONNECT_STATE_UP);

    /* 删除定时器 */
    hmac_roam_connect_del_timer_for_single_link(roam_info);

    /* 通知ROAM主状态机 */
    hmac_roam_connect_complete_for_single_link(roam_info->pst_hmac_vap, OAL_SUCC);
    return OAL_SUCC;
}
...
OAL_STATIC uint32_t hmac_roam_connect_fail(hmac_roam_info_stru *roam_info)
{
    ...
    oam_warning_log1(hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_ROAM,
                     "{hmac_roam_connect_fail::hmac_roam_alg_add_blacklist,connect_state[%d]!}", connect_state);
#ifdef _PRE_WLAN_FEATURE_PWL
    /* 下面的失败都会删user，需要先恢复user下存的秘钥，否则clear_bitmap会清理到其他的pwl_user */
    hmac_roam_pwl_restore_old_key(roam_info);
#endif
    /* 通知ROAM主状态机，BSS回退由主状态机完成 */
    if (connect_state == ROAM_CONNECT_STATE_HANDSHAKING) {
        hmac_roam_connect_complete(hmac_vap, roam_info, OAL_ERR_CODE_ROAM_HANDSHAKE_FAIL);
    } else if (connect_state == ROAM_CONNECT_STATE_WAIT_ASSOC_COMP ||
               connect_state == ROAM_CONNECT_STATE_WAIT_AUTH_COMP) {
        hmac_roam_connect_complete(hmac_vap, roam_info, OAL_ERR_CODE_ROAM_NO_RESPONSE);
    } else {
        hmac_roam_connect_complete(hmac_vap, roam_info, OAL_FAIL);
    }
...
uint32_t hmac_roam_connect_ft_reassoc(hmac_vap_stru *hmac_vap)
{
    ...
    if (roam_info->pst_hmac_user != NULL) {
        ...
        dpe_wlan_user_addr_sync(roam_info->pst_hmac_user->st_user_base_info.us_assoc_id, bss_dscr->auc_bssid);
    }
...
void hmac_roam_notify_sae_auth_fail(hmac_vap_stru *hmac_vap)
{
    hmac_roam_info_stru *roam_info = (hmac_roam_info_stru *)hmac_vap->pul_roam_info;
    if (roam_info != NULL) {
        roam_info->st_connect.en_status_code = MAC_REJECT_TEMP;
...
uint32_t hmac_roam_set_roam_auth_mode(mac_vap_stru *mac_vap, uint32_t *param)
{
    hmac_vap_stru *hmac_vap = mac_res_get_hmac_vap(mac_vap->uc_vap_id);
    if (hmac_vap == NULL) {
        return OAL_ERR_CODE_PTR_NULL;
    }
    hmac_vap->roam_wpa3_auth_mode = param[0];
    oam_warning_log1(hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_ROAM,
        "{hmac_roam_set_roam_auth_mode::wpa3_auth_mode [%d]0:auto;1 fix sae;2 fix open auth.}",
        hmac_vap->roam_wpa3_auth_mode);
    return OAL_SUCC;
}

/* 功能描述：802.1X 场景, 上报OKC 事件给wpa_s, 触发wpa_s更新pmkid */
void hmac_roam_notify_okc_start(hmac_vap_stru *hmac_vap, mac_bss_dscr_stru *bss_dscr)
{
    uint8_t *target_ap_addr = NULL;
    frw_event_mem_stru *event_mem = NULL;
    frw_event_stru *event = NULL;
    ...
    if (hmac_vap == NULL || bss_dscr == NULL) {
        oam_error_log0(0, OAM_SF_ROAM, "{hmac_roam_notify_okc_start::hmac_vap or bss_dscr is NULL!}");
        return;
    }
    ...
    oam_warning_log0(0, OAM_SF_ROAM, "{hmac_roam_notify_okc_start::is 802.1x, notify okc start");

    event_mem = frw_event_alloc_m(OAL_MAC_ADDRESS_LEN);
    if (event_mem == NULL) {
        oam_error_log0(0, OAM_SF_ROAM, "{hmac_roam_notify_okc_start::frw_event_alloc_m fail!}");
        return;
    }
    event = frw_get_event_stru(event_mem);
    frw_event_hdr_init(&(event->st_event_hdr),
                       FRW_EVENT_TYPE_HOST_CTX,
                       HMAC_HOST_CTX_EVENT_SUB_TYPE_OKC_ROAM,
                       OAL_MAC_ADDRESS_LEN,
                       FRW_EVENT_PIPELINE_STAGE_0,
                       hmac_vap->st_vap_base_info.uc_chip_id,
                       hmac_vap->st_vap_base_info.uc_device_id,
                       hmac_vap->st_vap_base_info.uc_vap_id);
    target_ap_addr = (uint8_t *)frw_get_event_payload(event_mem);
    oal_set_mac_addr(target_ap_addr, bss_dscr->auc_bssid);

    /* 分发事件 */
    result = frw_event_dispatch_event(event_mem);
    if (oal_unlikely(result != OAL_SUCC)) {
        oam_warning_log1(0, OAM_SF_ROAM,
                         "{hmac_roam_notify_okc_start::frw_event_dispatch_event failed[%d].}", result);
    }
    frw_event_free_m(event_mem);
}
