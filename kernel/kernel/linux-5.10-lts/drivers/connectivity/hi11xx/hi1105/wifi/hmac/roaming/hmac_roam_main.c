...
OAL_STATIC uint32_t hmac_roam_to_old_bss(hmac_roam_info_stru *roam_info);
...
void hmac_roam_main_change_state(hmac_roam_info_stru *roam_info, roam_main_state_enum state)
{
    ...
    if (roam_info != NULL) {
        if (roam_info->pst_hmac_vap != NULL) {
            vap_id = roam_info->pst_hmac_vap->st_vap_base_info.uc_vap_id;
        }
...
OAL_STATIC uint32_t hmac_roam_main_timeout(void *arg)
{
    ...
    oam_warning_log2(vap_id, OAM_SF_ROAM, "{hmac_roam_main_timeout::MAIN_STATE[%d] CONNECT_STATE[%d].}",
...
uint32_t hmac_roam_init(hmac_vap_stru *hmac_vap)
{
    ...
    roam_info->current_bss_policy = ROAM_IGNORE_CUR_BSS;
...
uint32_t hmac_roam_info_init(hmac_vap_stru *hmac_vap)
{
    ...
    hmac_roam_clear_perferred_bss(&(roam_info->st_alg));
...
#ifdef _PRE_WLAN_FEATURE_11V_ENABLE
    oal_cancel_work_sync(&(roam_info->hmac_11v_roam_work));
#endif

#ifdef _PRE_WLAN_FEATURE_ML
    if (hmac_vap->st_vap_base_info.en_vap_state == MAC_VAP_STATE_ROAMING &&
        roam_info->en_main_state == ROAM_MAIN_STATE_CONNECTING &&
        roam_info->st_connect.en_state != ROAM_CONNECT_STATE_INIT) {
        oam_warning_log1(hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_ROAM,
            "{hmac_roam_exit::connect_state[%d]need recover mlsr Relation}", roam_info->st_connect.en_state);
 ...
 *   修改内容  : 新生成函数
 */
OAL_STATIC void hmac_roam_param_update(hmac_roam_info_stru *roam_info, hmac_roam_start_params *roam_start_params)
{
    roam_info->st_config.uc_scan_orthogonal = roam_start_params->scan_type;
    roam_info->current_bss_policy = roam_start_params->current_bss_policy; /* false表示漫游到自己 */
    roam_info->en_roam_trigger = roam_start_params->roam_trigger;
    roam_info->cur_mld_link_bss_policy = roam_start_params->cur_mld_link_bss_policy;
#ifdef _PRE_WLAN_FEATURE_11R
    roam_info->st_connect.uc_ft_force_air = OAL_TRUE;
    roam_info->st_connect.uc_ft_failed = OAL_FALSE;
#endif
    oal_set_mac_addr(roam_info->auc_target_bssid, roam_start_params->target_bssid);
}

OAL_STATIC uint32_t hmac_roam_start_proc(hmac_vap_stru *hmac_vap, roam_channel_org_enum scan_type,
    hmac_roam_info_stru *roam_info)
{
    uint32_t ret = OAL_SUCC;
    oal_net_device_stru *netdevice = NULL;
    int32_t event = DRV_EVENT_ROAM_TRIGGER;
    uint32_t scan_mode = WLAN_SCAN_MODE_ROAM_SCAN;
    ...
    if (scan_type == ROAM_SCAN_CHANNEL_ORG_0) {
        ...
        ret = hmac_roam_main_fsm_action(roam_info, ROAM_MAIN_FSM_EVENT_START, &scan_mode);
        if (ret != OAL_SUCC) {
            oam_error_log1(vap_id, OAM_SF_ROAM, "{hmac_roam_start::START fail[%d].}", ret);
            return ret;
        }
    ...
    if (roam_info != NULL && roam_info->pst_hmac_vap != NULL && roam_info->pst_hmac_vap->pst_net_device != NULL) {
        netdevice = hmac_roam_get_netdev(roam_info->pst_hmac_vap);
        if (netdevice != NULL) {
            oal_cfg80211_wifi_event_report(netdevice, GFP_KERNEL, (uint8_t *)&event, sizeof(int32_t));
        }
    }
...
static uint32_t hmac_roam_start_check(hmac_vap_stru *hmac_vap, oal_bool_enum_uint8 current_bss_ignore)
{
    uint32_t ret;
    ...
    if (current_bss_ignore == OAL_TRUE) {
        ...
        if (hmac_vap->en_roam_prohibit_on == OAL_TRUE) {
            oam_warning_log0(hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_ROAM,
                "{hmac_roam_start_check::blacklist ap not support roam!}");
            return OAL_FAIL;
        }
    }
    return OAL_SUCC;
...
static uint32_t hmac_roam_start_check_param(hmac_vap_stru *hmac_vap, oal_bool_enum_uint8 current_bss_policy)
{
    uint32_t ret;
    ...
    if (hmac_mlsr_is_link_switching() == OAL_TRUE) {
        oam_warning_log0(hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_SCAN,
            "{hmac_roam_start_check_param::mlsr link is switching, reject roam}");
        return OAL_FAIL;
    }
#endif

    /*  漫游到自己做重关联业务不做检查，即使在信号桥下也支持，比如btcoex m2s 和上层下发的重关联到自己 */
    ret = hmac_roam_start_check(hmac_vap, current_bss_policy);
    if (ret != OAL_SUCC) {
        return ret;
    }
 ...
 *   修改内容  : 新生成函数
 */
uint32_t hmac_roam_start(hmac_vap_stru *hmac_vap, hmac_roam_start_params *roam_start_params)
{
    uint32_t ret;
    hmac_roam_info_stru *roam_info = NULL;
    roam_channel_org_enum scan_type;
    oal_bool_enum_uint8 current_bss_policy;
    roam_trigger_enum roam_trigger;

    if ((hmac_vap == NULL) || (roam_start_params == NULL)) {
        oam_error_log0(0, OAM_SF_ROAM, "{hmac_roam_start::hmac_vap or roam_start_params null.}");
        return OAL_ERR_CODE_PTR_NULL;
    }
    scan_type = roam_start_params->scan_type;
    current_bss_policy = roam_start_params->current_bss_policy;
    roam_trigger = roam_start_params->roam_trigger;

    ret = hmac_roam_start_check_param(hmac_vap, current_bss_policy);
    if (ret != OAL_SUCC) {
        return ret;
    }
    ...
    if (hmac_vap->st_vap_base_info.st_mbo_para_info.uc_mbo_enable != OAL_TRUE) {
        ...
        if (hmac_vap->ipaddr_obtained == OAL_FALSE &&
            ((g_optimized_feature_switch_bitmap & BIT(CUSTOM_OPTIMIZE_FEATURE_CERTIFY_MODE)) == 0)) {
            /* 认证测试模式不用检查是否配置IP地址，都允许漫游 */
            oam_warning_log0(hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_ROAM,
    ...
    hmac_roam_param_update(roam_info, roam_start_params);

    oam_warning_log4(hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_ROAM,
                     "{hmac_roam_start::START succ,scan_type=%d, current_bss_policy=%d,mld_policy=%d,roam_trigger=%d.}",
                     scan_type, current_bss_policy, roam_info->cur_mld_link_bss_policy, roam_trigger);

    hmac_roam_alg_init_rssi(hmac_vap, roam_info);

    return hmac_roam_start_proc(hmac_vap, scan_type, roam_info);
}
...
uint32_t hmac_roam_handle_home_network(hmac_vap_stru *hmac_vap)
{
    ...
    if ((roam_info->st_config.uc_scenario_enable == OAL_TRUE) &&
        ...
        if (oal_time_after32(current_timestamp,
            ...
            hmac_roam_start_fill_info(&roam_start_params,  ROAM_SCAN_CHANNEL_ORG_1,
...
oal_bool_enum_uint8 hmac_roam_is_neighbor_report_allowed(hmac_vap_stru *hmac_vap)
{
    ...
    ies_info.frame_ies = curr_bss_dscr->auc_mgmt_buff + MAC_80211_FRAME_LEN + offset;
    ies_info.frame_len = curr_bss_dscr->mgmt_len - MAC_80211_FRAME_LEN - offset;
    cowork_ie = mac_find_vendor_ie(MAC_WLAN_OUI_HUAWEI, MAC_WLAN_OUI_TYPE_HAUWEI_COWORK, &ies_info);
    if (cowork_ie == NULL) {
...
OAL_STATIC uint32_t hmac_roam_connecting_timeout(hmac_roam_info_stru *roam_info, void *param)
{
    ...
    ret = hmac_fsm_call_func_sta(roam_info->pst_hmac_vap, HMAC_FSM_INPUT_ROAMING_STOP, (void *)&post_dmac);
    if (ret != OAL_SUCC) {
...
static uint32_t hmac_roam_handle_fail_handshake_phase_post_do(hmac_roam_info_stru *roam_info)
{
    ...
    if (roam_info->st_alg.c_current_rssi > ROAM_RSSI_MAX_TYPE) {
         ...
         * 四次握手失败秘钥变更，此时立即触发漫游后可能选到另外一路link不能走link切换，需要重新入网秘钥协商
         */
        return OAL_SUCC;
    }
...
OAL_STATIC uint32_t hmac_roam_handle_fail_handshake_phase(hmac_roam_info_stru *roam_info, void *param)
{
    ...
    hmac_roam_main_clear(roam_info);
    hmac_roam_main_del_timer(roam_info);
    if (hmac_vap_is_link_mode(roam_info->pst_hmac_vap) == OAL_FALSE) {
        return hmac_roam_handle_fail_handshake_phase_post_do(roam_info);
    }
    return ret;
}
...
OAL_STATIC oal_bool_enum_uint8 hmac_roam_is_over_ds_allowed(hmac_roam_info_stru *roam_info)
{
    ...
    ies_info.frame_ies = curr_bss_dscr->auc_mgmt_buff + MAC_80211_FRAME_LEN + us_offset;
    ies_info.frame_len = curr_bss_dscr->mgmt_len - MAC_80211_FRAME_LEN - us_offset;
    puc_cowork_ie = mac_find_vendor_ie(MAC_WLAN_OUI_HUAWEI, MAC_WLAN_OUI_TYPE_HAUWEI_COWORK, &ies_info);
...
void hmac_roam_connect_save_old_bss(hmac_roam_old_bss_stru *old_bss, hmac_vap_stru *hmac_vap,
    hmac_user_stru *hmac_user)
{
...
#ifdef _PRE_WLAN_FEATURE_11BE
    memcpy_s(&old_bss->eht_hdl, sizeof(mac_eht_hdl_stru),
             &(hmac_user->st_user_base_info.st_eht_hdl), sizeof(mac_eht_hdl_stru));
#endif
...
#ifdef _PRE_WLAN_FEATURE_WUR
    memcpy_s(&old_bss->wur_info, sizeof(mac_wur_stru),
             &(hmac_vap->wur_info), sizeof(mac_wur_stru));
    memcpy_s(&old_bss->wur_hdl, sizeof(mac_wur_hdl_stru),
             &(hmac_user->st_user_base_info.wur_hdl), sizeof(mac_wur_hdl_stru));
#endif
    ...
    old_ssid = mac_mib_get_DesiredSSID(&(hmac_vap->st_vap_base_info));
    memcpy_s(old_bss->ssid, WLAN_SSID_MAX_LEN, old_ssid, WLAN_SSID_MAX_LEN);
    old_bss->ssid[WLAN_SSID_MAX_LEN - 1] = '\0';
}
...
OAL_STATIC uint32_t hmac_roam_connect_to_bss_check(hmac_vap_stru *hmac_vap, hmac_roam_info_stru *roam_info)
{
    uint32_t ret;
...
OAL_STATIC uint32_t hmac_roam_connect_to_bss(hmac_roam_info_stru *roam_info, void *param)
{
    ...
    ret = hmac_roam_connect_to_bss_check(hmac_vap, roam_info);
    if (ret != OAL_SUCC) {
    ...
    /* 非关联link不能启动启动connect超时定时器 */
    hmac_roam_main_start_timer(roam_info, ROAM_CONNECT_TIME_MAX);

    /* 802.1X 场景, 上报OKC 事件给wpa_s, 触发wpa_s更新pmkid */
    hmac_roam_notify_okc_start(hmac_vap, bss_dscr);
                                          ...
                                          hmac_roam_old_bss_stru *old_bss)
{
...
#ifdef _PRE_WLAN_FEATURE_11BE
    memcpy_s(&(hmac_user->st_user_base_info.st_eht_hdl), sizeof(mac_eht_hdl_stru),
             &old_bss->eht_hdl, sizeof(mac_eht_hdl_stru));

    if (g_wlan_spec_cfg->feature_11be_is_open) {
        hmac_roam_update_11be_protocol_mib(hmac_vap, old_bss->auc_bssid);
    }
#endif
...
#ifdef _PRE_WLAN_FEATURE_WUR
    memcpy_s(&(hmac_vap->wur_info), sizeof(mac_wur_stru),
             &(old_bss->wur_info), sizeof(mac_wur_stru));
    memcpy_s(&(hmac_user->st_user_base_info.wur_hdl), sizeof(mac_wur_hdl_stru),
             &old_bss->wur_hdl, sizeof(mac_wur_hdl_stru));
#endif
    ...
    cur_ssid = mac_mib_get_DesiredSSID(&(hmac_vap->st_vap_base_info));
    memcpy_s(cur_ssid, WLAN_SSID_MAX_LEN, old_bss->ssid, WLAN_SSID_MAX_LEN);
...
uint32_t hmac_proc_roam_to_old_bss(hmac_vap_stru *hmac_vap, hmac_roam_info_stru *roam_info)
{
    uint32_t ret;
    uint8_t vap_id = hmac_vap->st_vap_base_info.uc_vap_id;
    hmac_user_stru *hmac_user = roam_info->pst_hmac_user;
    ...
    if (ret != OAL_SUCC) {
        oam_error_log1(vap_id, OAM_SF_ROAM,
            "{hmac_proc_roam_to_old_bss::hmac_config_start_vap_event failed[%d].}", ret);
        return ret;
    }
    /* old channel校准数据下发 */
    ...
    /* 相关参数需要配置到dmac */
    hmac_roam_connect_set_join_reg(hmac_vap, hmac_user);

    /* 更新用户的mac地址，漫游时mac会更新 */
    oal_set_mac_addr(hmac_user->st_user_base_info.auc_user_mac_addr, hmac_vap->st_vap_base_info.auc_bssid);
    dpe_wlan_user_addr_sync(hmac_user->st_user_base_info.us_assoc_id, hmac_vap->st_vap_base_info.auc_bssid);
    ...
    /* hmac vap下wur参数恢复后，下发到dmac */
    hmac_vap_state_change_notify_feature(hmac_vap, hmac_user, COFEATURE_ROAM_EVENT, COFEATURE_ENTER_FAIL);

    ret = hmac_config_user_info_syn(&(hmac_vap->st_vap_base_info), &(hmac_user->st_user_base_info));
    if (ret != OAL_SUCC) {
        oam_error_log1(vap_id, OAM_SF_ROAM, "{hmac_proc_roam_to_old_bss::hmac_syn_vap_state failed[%d].}", ret);
    }

    /* 在漫游过程中可能又建立了聚合，因此回退时需要删除掉 */
    hmac_tid_clear(&hmac_vap->st_vap_base_info, hmac_user, OAL_TRUE);
...
#ifdef _PRE_WLAN_FEATURE_ML
    return hmac_proc_mld_roam_to_old_bss(hmac_vap, roam_info);
#else
...
static void hmac_roam_to_new_bss_notify_dmac(hmac_roam_info_stru *roam_info, hmac_vap_stru *hmac_vap)
{
    uint32_t ret;
    wlan_roam_main_band_state_enum_uint8 roam_band_state;

    if (hmac_vap->st_vap_base_info.en_vap_state == MAC_VAP_STATE_STA_LINK_DOWN) {
        return;
    }
...
OAL_STATIC uint32_t hmac_roam_to_new_bss(hmac_roam_info_stru *roam_info, void *param)
{
    ...
    oam_warning_log3(hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_ROAM,
        "{hmac_roam_to_new_bss::roam to [%02X:XX:XX:XX:%02X:%02X] succ}",
        hmac_vap->st_vap_base_info.auc_bssid[MAC_ADDR_0],
        hmac_vap->st_vap_base_info.auc_bssid[MAC_ADDR_4],
        hmac_vap->st_vap_base_info.auc_bssid[MAC_ADDR_5]);
    hmac_roam_main_clear(roam_info);

    /* 如果正在进行长期同信道监听, roam至新bss后检查能否继续同信道监听, 如果不行则abor监听 */
    hmac_abort_long_same_chan_remain_on_chan(&hmac_vap->st_vap_base_info, OAL_TRUE,
        MAC_ABORT_SAME_CHAN_REMAIN_ON_CHAN_ROAM);

    hmac_roam_to_new_bss_notify_dmac(roam_info, hmac_vap);

    hmac_vap_state_change_notify_feature(hmac_vap, hmac_user, COFEATURE_ROAM_EVENT, COFEATURE_ENTER_SUCC);
 ...
 *   修改内容  : 新生成函数
 */
uint32_t hmac_roam_rssi_trigger_type_single_link(hmac_vap_stru *hmac_vap, roam_scenario_enum val)
{
    hmac_roam_info_stru *roam_info = NULL;
    uint32_t ret;
    uint8_t vap_id;
    ...
    } else if (val == ROAM_ENV_SPARSE_AP_TH1) {
    ...
    if (ret != OAL_SUCC) {
        oam_error_log2(vap_id, OAM_SF_CFG,
                       "{hmac_roam_rssi_trigger_type::send event[%d] failed[%d].}", val, ret);
        return ret;
    }
    roam_info->uc_rssi_type = val;

    oam_warning_log1(vap_id, OAM_SF_ROAM, "{hmac_roam_rssi_trigger_type::[%d]}", val);
    return OAL_SUCC;
}
...
uint32_t hmac_roam_rssi_trigger_type(hmac_vap_stru *hmac_vap, roam_scenario_enum val)
{
    ...
    hmac_roam_rssi_trigger_type_single_link(hmac_vap, val);
#ifdef _PRE_WLAN_FEATURE_ML
    ...
    if (hmac_another_link == NULL) {
        oam_error_log0(0, OAM_SF_ROAM, "{hmac_roam_rssi_trigger_type::hmac_another_link null!}");
        return OAL_ERR_CODE_PTR_NULL;
    }

    vap_id = hmac_another_link->st_vap_base_info.uc_vap_id;
    roam_info = (hmac_roam_info_stru *)hmac_another_link->pul_roam_info;
    if (roam_info == NULL) {
        oam_error_log0(vap_id, OAM_SF_ROAM, "{hmac_roam_rssi_trigger_type::roam_info null!}");
        return OAL_ERR_CODE_ROAM_INVALID_VAP;
    }

    /* EMLSR场景退出漫游，需要更新另外一个link 漫游门限；
     * EMLSR场景如果当前禁止漫游，更新漫游rssi_type，同样需要更新另外一个link 漫游门限；
     */
    if ((val == ROAM_ENV_LINKLOSS) || (roam_info->uc_rssi_type == ROAM_ENV_LINKLOSS)) {
        hmac_roam_rssi_trigger_type_single_link(hmac_another_link, val);
    }
#endif
    return OAL_SUCC;
}

static uint32_t hmac_roam_send_roam_mode(mac_vap_stru *mac_vap, mac_bss_dscr_stru *bss_dscr)
{
    mac_h2d_roam_start_stru roam_start_param;
    uint32_t ret;
    roam_start_param.roaming_mode = 1;
    roam_start_param.channel = bss_dscr->st_channel;
    ret = hmac_config_send_event(mac_vap, WLAN_CFGID_SET_ROAMING_MODE,
        sizeof(roam_start_param), (uint8_t *)&roam_start_param);
    return ret;
}

oal_net_device_stru *hmac_roam_get_netdev(hmac_vap_stru *hmac_vap)
{
    oal_netdev_priv_stru *net_dev_priv;
    oal_net_device_stru *net_device = hmac_vap->pst_net_device;
    if (net_device == NULL) {
        oam_error_log0(hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_ROAM, "{hmac_roam_get_netdev::net_device null!}");
        return NULL;
    }
    if (hmac_vap_is_legacy_mld_mode(hmac_vap) == OAL_TRUE) {
        return net_device;
    } else {
        net_dev_priv = (oal_netdev_priv_stru *)oal_net_dev_wireless_priv(net_device);
        if (net_dev_priv->mlo_cfg_info.ml_netdev == NULL) {
            return net_device;
        }
        return net_dev_priv->mlo_cfg_info.ml_netdev;
    }
}

/* 功能描述: 漫游流程pause发送时，需要清除原有信息 */
static uint32_t hmac_roam_pause_user_clear_proc(mac_vap_stru *mac_vap, hmac_roam_info_stru *roam_info)
{
    uint32_t ret;

    /* up vap执行漫游，清空前一次link down vap标记 */
    if (roam_info->is_link_down_vap == OAL_TRUE) {
        oam_error_log0(mac_vap->uc_vap_id, OAM_SF_CFG, "{hmac_roam_pause_user_clear_proc::link_down_vap flag wrong!}");
        roam_info->is_link_down_vap = OAL_FALSE;
    }

    /* 清空 HMAC层TID信息 */
    hmac_tid_clear(mac_vap, roam_info->pst_hmac_user, OAL_TRUE);
...
uint32_t hmac_roam_pause_user(hmac_vap_stru *hmac_vap, void *param)
{
    ...
    if (hmac_vap == NULL || param == NULL) {
        oam_error_log0(0, OAM_SF_ROAM, "{hmac_roam_pause_user::vap or param null!}");
        return OAL_ERR_CODE_ROAM_INVALID_VAP;
    }
    mac_vap = &hmac_vap->st_vap_base_info;
    roam_info = (hmac_roam_info_stru *)hmac_vap->pul_roam_info;
    net_device = hmac_roam_get_netdev(hmac_vap);
    ...
    /* 暂停所有协议层数据，这样就不需要再hmac搞一个缓存队列了,两个link轮流调用，是否有问题？ */
    oal_net_tx_stop_all_queues(net_device);
    oal_net_wake_subqueue(net_device, WLAN_HI_QUEUE);

    ret = hmac_roam_pause_user_clear_proc(mac_vap, roam_info);
    if (ret != OAL_SUCC) {
        oal_net_tx_wake_all_queues(net_device);
        return ret;
    }

    ret = hmac_roam_send_roam_mode(mac_vap, (mac_bss_dscr_stru *)param);
    if (ret != OAL_SUCC) {
    ...
    hmac_mld_reset_mode_switch_state(hmac_vap);

    if (mac_vap->pst_vowifi_cfg_param != NULL &&
        mac_vap->pst_vowifi_cfg_param->en_vowifi_mode == VOWIFI_LOW_THRES_REPORT) {
        /* 针对漫游和去关联场景,切换vowifi语音状态 */
        hmac_config_vowifi_report(mac_vap, 0, NULL);
    }
    hmac_fsm_change_state_for_single_link(hmac_vap, MAC_VAP_STATE_ROAMING);

    oam_warning_log0(mac_vap->uc_vap_id, OAM_SF_ROAM, "{hmac_roam_pause_user::queues stopped!}");
    return OAL_SUCC;
}

uint32_t hmac_roam_pause_user_in_link_down(hmac_vap_stru *hmac_vap, void *param)
{
    hmac_roam_info_stru *roam_info = NULL;
    oal_net_device_stru *net_device = NULL;
    mac_vap_stru *mac_vap = NULL;
    ...
    net_device = hmac_roam_get_netdev(hmac_vap);
    if (roam_info == NULL) {
        oam_error_log0(mac_vap->uc_vap_id, OAM_SF_ROAM, "{hmac_roam_pause_user::roam_info null!}");
        return OAL_ERR_CODE_ROAM_INVALID_VAP;
    }
    if (roam_info->uc_enable == 0) {
        oam_error_log0(mac_vap->uc_vap_id, OAM_SF_ROAM, "{hmac_roam_pause_user::roam disable!}");
        return OAL_ERR_CODE_ROAM_INVALID_VAP;
    }
    roam_info->is_link_down_vap = OAL_TRUE;
    hmac_fsm_change_state_for_single_link(hmac_vap, MAC_VAP_STATE_ROAMING);
    return OAL_SUCC;
}
...
{
    mac_h2d_roam_start_stru roam_start_param = { 0 };
    uint32_t ret;

    if (need_post_stop_event ==  OAL_TRUE) {
        roam_start_param.roaming_mode = 0;
...
uint32_t hmac_roam_resume_user(hmac_vap_stru *hmac_vap, void *param)
{
    ...
    if (roam_info->is_link_down_vap) {
        hmac_fsm_change_state_for_single_link(hmac_vap, MAC_VAP_STATE_STA_LINK_DOWN);
        return OAL_SUCC;
    }
    net_device = hmac_roam_get_netdev(hmac_vap);
    if (net_device == NULL) {
    ...
    hmac_fsm_change_state_for_single_link(hmac_vap, MAC_VAP_STATE_UP);
...
static uint32_t hmac_roam_trigger_handle_check_param(hmac_vap_stru *hmac_vap)
{
    uint32_t ret;

    /* 黑名单路由器，不支持漫游，防止漫游出现异常 */
    if (hmac_vap->en_roam_prohibit_on == OAL_TRUE) {
        oam_warning_log0(0, OAM_SF_ROAM, "{hmac_roam_trigger_handle_check_param::blacklist ap not support roam!}");
        return OAL_FAIL;
    }
    ...
    if (hmac_mlsr_is_link_switching() == OAL_TRUE) {
        oam_warning_log0(hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_SCAN,
            "{hmac_roam_trigger_handle_check_param::mlsr link is switching, reject roam}");
        return OAL_FAIL;
    }
#endif
 ...
 *   修改内容  : 新生成函数
 */
uint32_t hmac_roam_trigger_handle(hmac_vap_stru *hmac_vap, int8_t rssi,
    oal_bool_enum_uint8 current_bss_ignore, uint8_t cur_mld_link_bss_policy)
{
    uint32_t ret;
    hmac_roam_info_stru *roam_info = NULL;
    int32_t event = DRV_EVENT_ROAM_TRIGGER;
    ...
    ret = hmac_roam_trigger_handle_check_param(hmac_vap);
    if (ret != OAL_SUCC) {
        return ret;
    }
    ...
    if (roam_info == NULL) {
        oam_warning_log0(hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_ROAM,
            "{hmac_roam_trigger_handle::hmac_vap->pul_roam_info is NULL,Return!}");
        return OAL_ERR_CODE_ROAM_EVENT_UXEXPECT;
    }

    /* 每次漫游前，刷新是否支持漫游到自己的参数 */
    roam_info->current_bss_policy = current_bss_ignore;
    roam_info->en_roam_trigger = ROAM_TRIGGER_DMAC;
    roam_info->cur_mld_link_bss_policy = cur_mld_link_bss_policy;
#ifdef _PRE_WLAN_FEATURE_11R
    roam_info->st_connect.uc_ft_force_air = OAL_TRUE;
    roam_info->st_connect.uc_ft_failed = OAL_FALSE;
#endif
    ...
    if (ret != OAL_SUCC) {
        oam_warning_log1(hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_ROAM,
            "{hmac_roam_trigger_handle::check_state fail[%d]!}", ret);
        return ret;
    }

    hmac_roam_alg_init(roam_info, rssi);
    /* 调用漫游状态机函数：hmac_roam_scan_init */
    ret = hmac_roam_main_fsm_action(roam_info, ROAM_MAIN_FSM_EVENT_START, &scan_mode);
    if (ret != OAL_SUCC) {
        oam_warning_log1(hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_ROAM,
    ...
    oal_cfg80211_wifi_event_report(hmac_roam_get_netdev(hmac_vap), GFP_KERNEL, (uint8_t *)&event, sizeof(int32_t));
...
OAL_STATIC uint32_t hmac_wait_neighbor_response_timeout(hmac_roam_info_stru *roam_info, void *param)
{
    ...
    if (scan_params->uc_channel_nums != 0) {
        ...
        /* 漫游等待neighbor report response超时,配置为全信道扫描 */
        roam_info->st_config.uc_scan_orthogonal = ROAM_SCAN_CHANNEL_ORG_FULL;
...
uint32_t hmac_sta_roam_rx_mgmt(hmac_vap_stru *hmac_vap, void *param)
{
    ...
    rx_info = (mac_rx_ctl_stru *)oal_netbuf_cb(crx_event->pst_netbuf);
    if (rx_info == NULL) {
        return OAL_ERR_CODE_PTR_NULL;
    }
    mac_hdr = (uint8_t *)(uintptr_t)mac_get_rx_cb_mac_header_addr(rx_info);
 ...
 *   修改内容  : 新生成函数
 */
void hmac_roam_wpas_connect_state_notify_for_single_link(hmac_vap_stru *hmac_vap,
    hmac_wpas_connect_state_enum_uint32 conn_state,
    oal_bool_enum_uint8 ipaddr_obtained)
{
    uint32_t ret;
    uint32_t notify_data;

    if (hmac_vap == NULL) {
        oam_error_log0(0, OAM_SF_MLD, "hmac_roam_wpas_connect_state_notify_for_single_link::hmac_vap is NULL!");
        return;
    }
    ...
    oam_warning_log2(hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_ROAM,
        "{hmac_roam_wpas_connect_state_notify_for_single_link::conn_state:%d, ipaddr_obtained:%d}",
        conn_state, ipaddr_obtained);

    hmac_vap->ipaddr_obtained = ipaddr_obtained;
    ...
    if (ret != OAL_SUCC) {
        oam_error_log1(hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_CFG,
            "{hmac_roam_wpas_connect_state_notify_for_single_link::send event failed[%d].}", ret);
        return;
    }
}

#ifdef _PRE_WLAN_FEATURE_ML
/* 功能描述：MLD场景，根据网络设备关系，查找link_vap，并在link_vap 执行对应功能函数 */
static uint32_t hmac_mld_set_link_vap_param(hmac_vap_stru *hmac_vap, uint8_t *param, uint16_t len,
    uint32_t (*func)(hmac_vap_stru *hmac_vap, uint8_t *param, uint16_t len))
{
    ...
    if (oal_any_null_ptr4(hmac_vap, param, func, hmac_vap->pst_net_device)) {
        return OAL_ERR_CODE_PTR_NULL;
    }
    ml_net_dev_priv = (oal_netdev_priv_stru *)oal_net_dev_wireless_priv(hmac_vap->pst_net_device);
    ...
    for (idx = 0; idx < WLAN_MAX_ML_LINK_NUM; idx++) {
        ...
        if (link_hmac_vap == NULL) {
            oam_error_log0(link_mac_vap->uc_vap_id, OAM_SF_CFG, "{hmac_mld_set_link_vap_data::link vap not add");
            return OAL_ERR_CODE_PTR_NULL;
        }

        ret = func(link_hmac_vap, param, len);
        if (oal_unlikely(ret != OAL_SUCC)) {
            oam_warning_log1(link_mac_vap->uc_vap_id, OAM_SF_CFG,
                "{hmac_mld_set_link_vap_data::failed[%d].}", ret);
            return ret;
        }
    }
...
typedef struct {
    hmac_wpas_connect_state_enum_uint32 conn_state;
    oal_bool_enum_uint8 ipaddr_obtained;
} hmac_connect_state_nofify_stru;

static uint32_t hmac_roam_wpas_connect_state_notify_link_vap(hmac_vap_stru *hmac_vap, uint8_t *param, uint16_t len)
{
    hmac_connect_state_nofify_stru *data = (hmac_connect_state_nofify_stru *)param;
    hmac_roam_wpas_connect_state_notify_for_single_link(hmac_vap, data->conn_state, data->ipaddr_obtained);
    return OAL_SUCC;
}
#endif /* _PRE_WLAN_FEATURE_ML */

void hmac_roam_wpas_connect_state_notify(hmac_vap_stru *hmac_vap,
    hmac_wpas_connect_state_enum_uint32 conn_state, oal_bool_enum_uint8 ipaddr_obtained)
{
#ifdef _PRE_WLAN_FEATURE_ML
    hmac_connect_state_nofify_stru connect_state_nofify = {
        .conn_state = conn_state,
        .ipaddr_obtained = ipaddr_obtained,
    };
#endif

    if (hmac_vap == NULL) {
        oam_error_log0(0, OAM_SF_MLD, "hmac_roam_wpas_connect_state_notify::hmac_vap is NULL!");
        return;
    }

    hmac_roam_wpas_connect_state_notify_for_single_link(hmac_vap, conn_state, ipaddr_obtained);

#ifdef _PRE_WLAN_FEATURE_ML
    /* MLD场景，配置link_vap ipaddr_obtain标志 */
    hmac_mld_set_link_vap_param(hmac_vap, (uint8_t *)&connect_state_nofify, sizeof(hmac_connect_state_nofify_stru),
        hmac_roam_wpas_connect_state_notify_link_vap);
#endif
}
    ...
    mac_neighbor_rpt_ie_stru *neighbor_rpt_ie)
{
    ...
    if (mac_is_channel_num_valid(en_band, channl_num, OAL_FALSE) == OAL_SUCC) {
        hmac_roam_neighbor_report_add_chan(scan_params, channl_num, en_band);
    }
 ...
 *   修改内容  : 新生成函数
 */
OAL_STATIC uint32_t hmac_roam_parse_neighbor_response(hmac_roam_info_stru *roam_info, oal_netbuf_stru *netbuf)
{
    ...
    framebody_len = (int32_t)mac_get_rx_cb_payload_len(rx_ctrl);
    ...
    while (framebody_len >= HMAC_NEIGHBOR_RPT_MIN_LEN + MAC_IE_HDR_LEN) {
        ...
        ies_info.frame_ies = puc_ie;
        ies_info.frame_len = framebody_len;
        puc_neighbor_rpt_ie = mac_find_ie(MAC_EID_NEIGHBOR_REPORT, &ies_info);
...
uint32_t hmac_roam_rx_neighbor_response_action(hmac_vap_stru *hmac_vap, oal_netbuf_stru *netbuf)
{
    ...
    if (ret != OAL_SUCC) {
        hmac_roam_parse_neighbor_response(roam_info, netbuf);
        return ret;
    }
    /* 调用漫游状态机函数：hmac_roam_neighbor_response_process */
...
OAL_STATIC uint32_t hmac_roam_neighbor_response_process(hmac_roam_info_stru *roam_info, void *param)
{
    ...
    ret = hmac_roam_parse_neighbor_response(roam_info, (oal_netbuf_stru *)param);
    if (ret != OAL_SUCC) {
...
OAL_STATIC void hmac_roam_handle_dualband_roam_result_report(hmac_vap_stru *hmac_vap, uint32_t result)
{
    uint32_t ret;
    hmac_amax_roam_start_struct event;
    hmac_roam_info_stru *roam_info = NULL;
    mac_bss_dscr_stru *new_bss_dscr = NULL;

    roam_info = (hmac_roam_info_stru *)hmac_vap->pul_roam_info;
    if (roam_info == NULL) {
        oam_error_log0(hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_ROAM,
                       "{hmac_roam_handle_dualband_roam_result_report::roam_info null!}");
        return;
    }

    if (roam_info->is_support_dualband_roam != 1) {
        oam_warning_log1(hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_ROAM,
            "{hmac_roam_handle_dualband_roam_result_report::support_zero_roaming result[%d]}",
            roam_info->is_support_dualband_roam);
        return;
    }

    new_bss_dscr = roam_info->st_connect.pst_bss_dscr;
    if (new_bss_dscr == NULL) {
        return;
    }

    event.type = result == OAL_SUCC ? DRV_EVENT_ROAM_SUCC : DRV_EVENT_ROAM_FAIL;
    event.band = new_bss_dscr->st_channel.en_band;
    event.support_zero_roaming = roam_info->is_support_dualband_roam;
    ret = memcpy_s(event.auc_target_bssid, WLAN_MAC_ADDR_LEN, hmac_vap->st_vap_base_info.auc_bssid, WLAN_MAC_ADDR_LEN);
    if (ret != EOK) {
        oam_warning_log0(0, OAM_SF_ROAM, "{hmac_roam_handle_dualband_roam_result_report::roam_info bssid copy failed}");
        return;
    }

    oal_cfg80211_wifi_event_report(hmac_roam_get_netdev(hmac_vap), GFP_KERNEL,
        (uint8_t *)&event, sizeof(hmac_amax_roam_start_struct));
}
 ...
 *   修改内容  : 新生成函数
 */
void hmac_roam_connect_complete_for_single_link(hmac_vap_stru *hmac_vap, uint32_t result)
{
    /* 两个link分别通知 */
    hmac_roam_info_stru *roam_info = NULL;

    if (hmac_vap == NULL) {
        oam_error_log0(0, OAM_SF_ROAM, "{hmac_roam_connect_complete_for_single_link::vap null!}");
        return;
    }
    ...
    if (roam_info == NULL) {
        oam_error_log0(hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_ROAM,
                       "{hmac_roam_connect_complete_for_single_link::roam_info null!}");
        return;
    }
    ...
    if (roam_info->uc_enable == 0) {
        oam_error_log0(hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_ROAM,
                       "{hmac_roam_connect_complete_for_single_link::roam disabled!}");
        return;
    }
    ...
    if (result == OAL_SUCC) {
        ...
        /* 调用漫游状态机：hmac_roam_connecting_timeout */
        hmac_roam_main_fsm_action(roam_info, ROAM_MAIN_FSM_EVENT_TIMEOUT, NULL);
    }
#ifdef _PRE_WLAN_FEATURE_ML
    hmac_roam_connect_comp_postdo_notify_mld(hmac_vap, roam_info, result);
#endif
}

/* 函数功能：为了提高漫游成功的概率，在Auth/Reassoc No rsp/四次握手失败时立即触发重新漫游 */
uint32_t hmac_roam_connect_fail_restart_roam(hmac_vap_stru *hmac_vap, hmac_roam_info_stru *roam_info,
    uint32_t result, oal_bool_enum_uint8 disconnect_reported)
{
    if (roam_info == NULL) {
        return OAL_ERR_CODE_PTR_NULL;
    }

    if (result == OAL_SUCC || disconnect_reported == OAL_TRUE) {
        return OAL_SUCC;
    }
    /* (1)密钥协商（4次握手）失败
     * (2)Auth/Reassoc No rsp
     */
    if (roam_info->st_alg.c_current_rssi > ROAM_RSSI_MAX_TYPE) {
        /* 如果是弱信号触发的漫游，先把rssi修改成ROAM_RSSI_LINKLOSS_TYPE来将弱信号跟LINKLOSS触发的重漫游归一 */
        if (roam_info->st_alg.c_current_rssi > ROAM_RSSI_LINKLOSS_TYPE) {
        ...
        /* 漫游失败（四次握手失败/未接收到响应）时，rssi 逐次减1dBm，一直到到ROAM_RSSI_MAX_TYPE。这样可以最多触发5次重漫游 */
        if (result == OAL_ERR_CODE_ROAM_HANDSHAKE_FAIL) {
            /* 四次握手失败秘钥变更，此时立即触发漫游后可能选到另外一路link不能走link切换，需要重新入网秘钥协商 */
            return hmac_roam_trigger_handle(hmac_vap, roam_info->st_alg.c_current_rssi - 1, OAL_TRUE,
                ROAM_SAME_MLD_LINK_REASSOC);
        } else if (result == OAL_ERR_CODE_ROAM_NO_RESPONSE) {
            return hmac_roam_trigger_handle(hmac_vap, roam_info->st_alg.c_current_rssi - 1, OAL_TRUE,
                roam_info->cur_mld_link_bss_policy);
        }
    }
    return OAL_SUCC;
}

void hmac_roam_connect_complete(hmac_vap_stru *hmac_vap, hmac_roam_info_stru *roam_info, uint32_t result)
{
#ifdef _PRE_WLAN_FEATURE_ML
    uint8_t               link_id;
    ...
    oal_bool_enum_uint8 disconnect_reported;
#endif
    if (hmac_vap_is_legacy_mld_mode(hmac_vap) == OAL_TRUE) {
        hmac_roam_connect_complete_for_single_link(hmac_vap, result);
        /* 为了提高漫游成功的概率，漫游失败立即触发重新漫游 */
        hmac_roam_connect_fail_restart_roam(hmac_vap, roam_info, result, OAL_FALSE);
        return;
    }
#ifdef _PRE_WLAN_FEATURE_ML
    mld_vap_manage = hmac_vap->mld_vap_link_info.mld_vap_manage;
    if (mld_vap_manage == NULL) {
        oam_error_log0(0, OAM_SF_CFG, "{hmac_roam_connect_change_state::get mld_vap_mgmt fail.");
        return;
    }
    /* TDO 2->1漫游失败,会有一个link未处理到，失败要特殊处理 */
    for (link_id = 0; link_id < mld_vap_manage->mld_link_cnt; link_id++) {
        ...
        /* 如果是master link，不在循环内切换 */
        if (link_roam_info == NULL || link_roam_info->en_main_state == ROAM_MAIN_STATE_INIT) {
            continue;
        }
        /* slave link的漫游结果处理 */
        hmac_roam_connect_complete_for_single_link(link_hmac_vap, result);
    }
    /* master link的漫游结果处理 */
    hmac_roam_connect_complete_for_single_link(hmac_vap, result);

    /* 漫游失败，回old bss，恢复mac地址 */
    if (result != OAL_SUCC) {
        hmac_mld_roam_back_switch_mac_addr(hmac_vap);
    }

    /* 针对link切换，接收到deauth/disassoc触发漫游，漫游结果处理 */
    disconnect_reported = hmac_roam_connect_complete_for_link_switch_rx_deauth_disassoc(hmac_vap, result);

    /* 为了提高漫游成功的概率，漫游失败立即触发重新漫游 */
    hmac_roam_connect_fail_restart_roam(hmac_vap, roam_info, result, disconnect_reported);
#endif
}
...
void hmac_roam_start_fill_info(hmac_roam_start_params *roam_start_params, roam_channel_org_enum scan_type,
    oal_bool_enum_uint8 current_bss_policy, uint8_t *target_bssid, roam_trigger_enum roam_trigger)
{
    roam_start_params->scan_type = scan_type;
    roam_start_params->current_bss_policy = current_bss_policy;
    roam_start_params->cur_mld_link_bss_policy = ROAM_SAME_MLD_LINK_SWITCH;
    roam_start_params->roam_trigger = roam_trigger;
    if (target_bssid != NULL) {
        oal_set_mac_addr(roam_start_params->target_bssid, target_bssid);
    }
}
