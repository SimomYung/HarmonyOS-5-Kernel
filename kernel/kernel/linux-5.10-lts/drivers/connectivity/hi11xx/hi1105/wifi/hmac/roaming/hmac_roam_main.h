...
#define DRV_EVENT_ROAM_START           (3)         /* 扫描后漫游开始时间上报 */
#define DRV_EVENT_ROAM_SUCC            (4)         /* 扫描后漫游成功上报 */
#define DRV_EVENT_ROAM_FAIL            (5)         /* 扫描后漫游失败上报 */
...
typedef enum {
    ROAM_ONLY_CUR_BSS = 0,
    ROAM_IGNORE_CUR_BSS = 1,
    ROAM_ONLY_SWITCH_MLD_ANOTHER_LINK = 2,
    ROAM_ONLY_REASSOC_MLD_ANOTHER_LINK = 3,
} roam_ignore_cur_bss_enum;

typedef enum {
    ROAM_SAME_MLD_LINK_IGNORE = 0,  /* roam不需要同一个mld的另一个link */
    ROAM_SAME_MLD_LINK_SWITCH = 1,  /* roam不需要同一个mld的另一个link */
    ROAM_SAME_MLD_LINK_REASSOC = 2,  /* roam不需要同一个mld的另一个link */
    ROAM_ONLY_SAME_MLD_LINK_SWITCH = 3,  /* roam需要同一个mld的另一个link做link switch */
    ROAM_ONLY_SAME_MLD_LINK_REASSOC = 4, /* roam需要同一个mld的另一个link做reassoc */
} roam_same_mld_roam_policy_enum;
...
/* 双频漫游开始上报结构体 */
typedef struct {
    uint32_t type;                    /* WIFI QOE所需漫游事件上报标志 */
    wlan_channel_band_enum_uint8 band;
    uint8_t support_zero_roaming;
    uint8_t auc_target_bssid[WLAN_MAC_ADDR_LEN];
} hmac_amax_roam_start_struct;

typedef struct {
    roam_channel_org_enum scan_type;
    oal_bool_enum_uint8 current_bss_policy;
    roam_trigger_enum roam_trigger;
    uint8_t cur_mld_link_bss_policy;
    uint8_t target_bssid[WLAN_MAC_ADDR_LEN];
} hmac_roam_start_params;
...
typedef enum {
    ROAM_AUTH_AUTO_MODE = 0,
    ROAM_WPA3_FIXED_SAE_AUTH = 1,
    ROAM_WPA3_FIXED_OPEN_AUTH_WITH_PMKID = 2,
} hmac_wpa3_auth_mode_enum;
...
void hmac_roam_connect_complete(hmac_vap_stru *hmac_vap, struct hmac_roam_info_tag *roam_info, uint32_t result);
