...
uint8_t hmac_roam_mld_check_target_bss_is_link_vap(hmac_vap_stru *hmac_vap, mac_bss_dscr_stru *bss_dscr)
{
    ...
    if (oal_memcmp(bss_dscr->ml_info.ml_mac_addr, hmac_get_mld_bssid(hmac_vap), WLAN_MAC_ADDR_LEN) == 0 &&
        ...
        oam_warning_log0(hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_ROAM,
            "{hmac_roam_mld_check_target_bss_is_link_vap::same conn mld link}");
        return OAL_TRUE;
    }
...
uint8_t hmac_roam_mld_bssid_check_ignore_bss(hmac_roam_info_stru *roam_info, hmac_vap_stru *hmac_vap,
...
static void hmac_roam_restore_vap_old_mld_params(hmac_roam_info_stru *roam_info)
{
    ...
    dpe_wlan_vap_mld_addr_set(hmac_vap->st_vap_base_info.uc_vap_id, mld_vap_manage->mld_addr);
}
...
static void hmac_mld_roam_start_switch_mac_addr(hmac_vap_stru *hmac_vap)
{
    ...
    if (mld_vap_manage == NULL) {
        oam_error_log0(hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_ROAM,
            "{hmac_mld_roam_start_switch_mac_addr::mld_vap_manage null");
        return;
    }
...
void hmac_mld_roam_back_switch_mac_addr(hmac_vap_stru *hmac_vap)
{
    uint8_t link_id;
    hmac_vap_stru *link_hmac_vap = NULL;
    ...
    if (hmac_vap == NULL) {
        oam_error_log0(0, OAM_SF_ROAM, "{hmac_mld_roam_back_switch_mac_addr::hmac_vap null");
        return;
    }
    ...
    if (mld_vap_manage == NULL) {
        oam_warning_log0(hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_ROAM,
            "{hmac_mld_roam_back_switch_mac_addr::mld_vap_manage null");
        return;
    }
...
static uint32_t hmac_roam_mld_scan(hmac_vap_stru *hmac_vap, hmac_roam_info_stru *roam_info, mac_bss_dscr_stru *bss_dscr)
{
    uint32_t ret;
    errno_t memcpy_ret;
    uint32_t scan_mode = WLAN_SCAN_MODE_MLD_SCAN;
    ml_link_info_stru *mld_link_info = hmac_get_bss_mld_link_info(hmac_vap, bss_dscr);
    if (mld_link_info == NULL) {
        oam_error_log0(0, OAM_SF_CFG, "{hmac_roam_mld_scan::mld_link_info is NULL}");
        return OAL_ERR_CODE_PTR_NULL;
    }
    ...
    if (ret != OAL_SUCC) {
        oam_error_log0(0, OAM_SF_ROAM,
            "{hmac_roam_mld_scan::can not mld_scan when roam.}");
        return ret;
    }

    /* 修改漫游主状态机为INIT状态，发起MLD_SCAN */
    hmac_roam_main_change_state(roam_info, ROAM_MAIN_STATE_INIT);

    /* mld 扫描，选择单信道扫描，指定bssid/ssid 扫描，配置扫描模式为MLD_SCAN */
    hmac_vap->ml_scan_param.mld_id = bss_dscr->ml_info.mld_id;
    memcpy_ret = memcpy_s(hmac_vap->ml_scan_param.ssid, WLAN_SSID_MAX_LEN, bss_dscr->ac_ssid, WLAN_SSID_MAX_LEN);
    memcpy_ret += memcpy_s(hmac_vap->ml_scan_param.bssid, WLAN_MAC_ADDR_LEN, bss_dscr->auc_bssid, WLAN_MAC_ADDR_LEN);
    memcpy_ret += memcpy_s(hmac_vap->ml_scan_param.sta_mac_addr, WLAN_MAC_ADDR_LEN,
        mld_link_info->sta_mac_addr, WLAN_MAC_ADDR_LEN);
    if (memcpy_ret != EOK) {
        oam_error_log0(0, OAM_SF_CFG, "{hmac_roam_mld_scan::copy ssid bssid fail}");
        hmac_roam_main_change_state(roam_info, ROAM_MAIN_STATE_SCANING);
        return OAL_FAIL;
    }
    memcpy_ret = memcpy_s(&hmac_vap->ml_scan_param.mac_channel, sizeof(mac_channel_stru),
        &bss_dscr->st_channel, sizeof(mac_channel_stru));
    if (memcpy_ret != EOK) {
        oam_error_log0(0, OAM_SF_CFG, "{hmac_roam_mld_scan::copy mac_channel fail}");
        hmac_roam_main_change_state(roam_info, ROAM_MAIN_STATE_SCANING);
        return OAL_FAIL;
    }
    /* 驱动自发，避免重复触发11k扫描 */
    ...
    roam_info->en_roam_trigger = ROAM_TRIGGER_APP;
    /* 调用漫游状态机函数： hmac_roam_scan_init */
    ret = hmac_roam_main_fsm_action(roam_info, ROAM_MAIN_FSM_EVENT_START, &scan_mode);
    if (ret != OAL_SUCC) {
        oam_error_log0(0, OAM_SF_ROAM, "{hmac_roam_mld_scan::mld_scan fail when roam.}");
        hmac_roam_main_change_state(roam_info, ROAM_MAIN_STATE_SCANING);
    }
    return ret;
}
...
uint32_t hmac_mld_start_roam_connect(hmac_roam_info_stru  *roam_info, uint8_t is_roam_mld_connect,
    uint8_t is_cur_mld_connect, mac_bss_dscr_stru *bss_dscr)
{
    ...
    /* 如果没有另一个link的扫描信息或者对方是单link  ，暂且不用mld漫游 */
    if (hmac_sta_check_start_ml_scan(hmac_vap, bss_dscr) == OAL_TRUE) {
        if (hmac_roam_mld_scan(hmac_vap, roam_info, bss_dscr) == OAL_SUCC) {
            /* MLD 扫描成功，则等待MLD扫描完成后，再发起漫游关联； */
            roam_info->scan_info.mld_scan = OAL_TRUE;
            oal_set_mac_addr(roam_info->st_alg.max_rssi_bss_bssid, bss_dscr->auc_bssid);
            return OAL_SUCC;
        }
    }

    roam_info->scan_info.mld_scan = OAL_FALSE;
    memset_s(roam_info->st_alg.max_rssi_bss_bssid, WLAN_MAC_ADDR_LEN, 0, WLAN_MAC_ADDR_LEN);
    // MLD 扫描失败或不用mld 扫描，清除mld 扫描参数 ;
...
uint32_t hmac_mld_rx_deauth_do_reassoc(hmac_vap_stru *hmac_vap)
{
    hmac_roam_start_params roam_start_params = {0};

    if (hmac_vap == NULL) {
        oam_error_log0(0, OAM_SF_ROAM, "{hmac_mld_rx_deauth_do_reassoc::hmac_vap is NULL!");
        return OAL_ERR_CODE_PTR_NULL;
    }

    /* 指定bssid漫游 */
    hmac_roam_start_fill_info(&roam_start_params, ROAM_SCAN_CHANNEL_ORG_0,
        OAL_FALSE, hmac_vap->st_vap_base_info.auc_bssid, ROAM_TRIGGER_BSSID);
    roam_start_params.cur_mld_link_bss_policy = ROAM_SAME_MLD_LINK_REASSOC;
    hmac_roam_start(hmac_vap, &roam_start_params);

    /* 检查漫游状态，
    * (1)触发漫游失败，非漫游状态，断联，删用户；
    * (2)触发漫游成功，保存接收到deauth标志。
    * (2.1)漫游失败后，断联；
    * (2.2)漫游成功，清除接收到deauth标志。
    */
    if (hmac_roam_main_check_state((hmac_roam_info_stru *)hmac_vap->pul_roam_info,
        MAC_VAP_STATE_ROAMING, ROAM_MAIN_STATE_CONNECTING) != OAL_SUCC) {
        oam_warning_log0(hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_ROAM,
            "{hmac_mld_rx_deauth_do_reassoc::trigger roam fail!");
        return OAL_FAIL;
    }
    return OAL_SUCC;
}

/* 函数功能：link切换后，接收到deauth，调用漫游接口触发重关联 */
uint32_t hmac_mld_link_switch_deauth_force_reassoc(hmac_vap_stru *hmac_vap, hmac_user_stru *hmac_user)
{
    hmac_mld_manage_stru *mld_vap_manage = NULL;
    hmac_vap_stru *another_hmac_vap = NULL;
    hmac_user_stru *another_hmac_user = NULL;

    if (hmac_vap == NULL || hmac_user == NULL) {
        oam_error_log0(0, OAM_SF_ROAM, "{hmac_mld_link_switch_deauth_force_reassoc::vap or user is NULL!");
        return OAL_ERR_CODE_PTR_NULL;
    }
    ...
    if (mld_vap_manage == NULL) {
        oam_error_log0(hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_ROAM,
            "{hmac_mld_link_switch_deauth_force_reassoc::mld_vap_manage null!");
        return OAL_FAIL;
    }
    ...
    if (another_hmac_vap == NULL) {
        oam_error_log0(hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_ROAM,
            "{hmac_mld_link_switch_deauth_force_reassoc::another_hmac_vap null!");
        return OAL_FAIL;
    }
    another_hmac_user = mac_res_get_hmac_user(another_hmac_vap->st_vap_base_info.us_assoc_vap_id);
    if (another_hmac_user == NULL) {
        oam_error_log0(another_hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_ROAM,
            "{hmac_mld_link_switch_deauth_force_reassoc::another_hmac_user null!");
        return OAL_ERR_CODE_PTR_NULL;
    }
    hmac_user->is_recv_deauth_disassoc = OAL_TRUE;
    another_hmac_user->is_recv_deauth_disassoc = OAL_TRUE;

    /* 判断vap mac地址和ml_addr是否相同：
     * 1)相同，未触发过link切换，直接重关联；
     * 2)不同，有过link切换，需要先切换回来，再重关联
     */
    if (memcmp(mld_vap_manage->mld_addr, mac_mib_get_StationID(&(hmac_vap->st_vap_base_info)),
        WLAN_MAC_ADDR_LEN) == 0) {
        return hmac_mld_rx_deauth_do_reassoc(hmac_vap);
    }
    /* link切换过，先执行link切换，link切换后，再执行漫游 */
    return hmac_mlsr_link_switch(hmac_vap, another_hmac_vap, LINK_SWITCH_FOR_ROAM, 1);
}

oal_bool_enum_uint8 hmac_roam_connect_complete_for_link_switch_rx_deauth_disassoc(hmac_vap_stru *hmac_vap,
    uint32_t result)
{
    ...
    oam_warning_log2(hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_ROAM,
        "{hmac_roam_connect_complete_for_link_switch_rx_deauth_disassoc::roam result %d, user recv_deauth_disassoc %d!",
        result, hmac_user->is_recv_deauth_disassoc);

    if (hmac_user->is_recv_deauth_disassoc == OAL_FALSE) {
        return OAL_FALSE;
    }
    hmac_user->is_recv_deauth_disassoc = OAL_FALSE;
    if (result != OAL_SUCC) {
        /* 漫游失败，上报上层应用断联 */
