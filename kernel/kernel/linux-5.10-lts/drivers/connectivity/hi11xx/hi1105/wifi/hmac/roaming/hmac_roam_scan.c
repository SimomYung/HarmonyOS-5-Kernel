...
static uint32_t hmac_roam_select_rsn_akm(uint32_t *ap_akm_suite, const uint32_t ap_akm_suite_num)
{
    uint32_t idx;
    uint32_t new_akm = 0;
    for (idx = 0; idx < ap_akm_suite_num; idx++) {
        /* 优选AKM24 */
        if (hmac_key_mgmt_sae_ext(ap_akm_suite[idx]) == OAL_TRUE) {
    ...
    for (idx = 0; idx < ap_akm_suite_num; idx++) {
        /* 其次优选AKM8 */
        if (hmac_key_mgmt_sae8(ap_akm_suite[idx]) == OAL_TRUE) {
            new_akm = ap_akm_suite[idx];
            return new_akm;
        }
    }
    ...
    for (idx = 0; idx < ap_akm_suite_num; idx++) {
        /* 最后选AKM2 */
        if (hmac_key_mgmt_psk2(ap_akm_suite[idx]) == OAL_TRUE) {
            new_akm = ap_akm_suite[idx];
            return new_akm;
        }
    }
...
/* 功能描述：psk组网（PSK/SAE/AKM24），优选AKM24,其次SAE,最后PSK */
static uint32_t hmac_roam_renew_privacy_rsn_akm(uint32_t *ap_akm_suite, const uint32_t ap_akm_suite_num,
    uint32_t match_suite, uint32_t vap_akm, oal_bool_enum_uint8 ft_enabled)
{
    uint32_t new_akm = match_suite;

    if (match_suite != 0) {
        /* 有匹配key_mgmt */
        if ((match_suite == MAC_RSN_AKM_PSK) || (match_suite == MAC_RSN_AKM_SAE) ||
            (hmac_key_mgmt_ft_psk(match_suite) == OAL_TRUE && ft_enabled == OAL_FALSE)) {
            new_akm = hmac_roam_select_rsn_akm(ap_akm_suite, ap_akm_suite_num);
        }
    } else {
        /* 无匹配key_mgmt，vap是PSK/SAE/SAE-EXT-KEY 三个之一,允许在PSK/SAE/SAE-EXT-KEY 间漫游 */
        if (hmac_key_mgmt_psk(vap_akm) == OAL_TRUE) {
            new_akm = hmac_roam_select_rsn_akm(ap_akm_suite, ap_akm_suite_num);
        }
    }
    oam_warning_log2(0, OAM_SF_ROAM,
        "hmac_roam_renew_privacy_rsn_akm:match_suite:0x%x,new_akm 0x%x",
        match_suite, new_akm);
    return new_akm;
}
    ...
    mac_bss_dscr_stru *bss_dscr)
{
    ...
    match_suite = mac_mib_rsn_akm_match_suites_s(mac_vap, conn_sec->st_crypto.aul_akm_suite,
                                                 sizeof(conn_sec->st_crypto.aul_akm_suite));
    /* 对于psk组网（PSK/SAE/AKM24），优选AKM24,其次SAE,最后PSK */
    match_suite = hmac_roam_renew_privacy_rsn_akm(conn_sec->st_crypto.aul_akm_suite, WLAN_AUTHENTICATION_SUITES,
        match_suite, rsn_akm_suites[0], mac_mib_get_ft_trainsistion(&(hmac_vap->st_vap_base_info)));
    if (match_suite != 0) {
        conn_sec->st_crypto.aul_akm_suite[0] = match_suite;
    }
...
static void hmac_roam_get_pmksa_bssid(hmac_vap_stru *hmac_vap, uint8_t *bssid, mac_bss_dscr_stru *bss_dscr)
{
#ifdef _PRE_WLAN_FEATURE_ML
    if (hmac_vap_is_legacy_mld_mode(hmac_vap) != OAL_TRUE &&
        hmac_check_is_mld_connect(&(hmac_vap->st_vap_base_info), bss_dscr)) {
        oal_set_mac_addr(bssid, bss_dscr->ml_info.ml_mac_addr);
    } else {
        oal_set_mac_addr(bssid, bss_dscr->auc_bssid);
    }
#else
    oal_set_mac_addr(bssid, bss_dscr->auc_bssid);
#endif
...
static uint8_t hmac_roam_check_use_open_auth_with_pmkid(hmac_vap_stru *hmac_vap, mac_bss_dscr_stru *bss_dscr)
{
    uint8_t *pmkid;
    uint8_t bssid[WLAN_MAC_ADDR_LEN];
    uint8_t use_open_auth_with_pmkid = OAL_FALSE;

    /* mld模式非关联到自己的场景 */
    if (hmac_vap->roam_wpa3_auth_mode == ROAM_AUTH_AUTO_MODE) {
        if (hmac_check_is_mld_connect(&(hmac_vap->st_vap_base_info), bss_dscr) == OAL_TRUE &&
            (oal_memcmp(hmac_vap->st_vap_base_info.auc_bssid, bss_dscr->auc_bssid, OAL_MAC_ADDR_LEN) != 0)) {
            use_open_auth_with_pmkid = OAL_TRUE;
        }
    } else if (hmac_vap->roam_wpa3_auth_mode == ROAM_WPA3_FIXED_SAE_AUTH) {
        use_open_auth_with_pmkid = OAL_FALSE;
    } else if (hmac_vap->roam_wpa3_auth_mode == ROAM_WPA3_FIXED_OPEN_AUTH_WITH_PMKID) {
        use_open_auth_with_pmkid = OAL_TRUE;
    }
    if (use_open_auth_with_pmkid == OAL_TRUE) {
        hmac_roam_get_pmksa_bssid(hmac_vap, bssid, bss_dscr);
        pmkid = hmac_vap_get_pmksa(hmac_vap, bssid);
        if (pmkid != NULL) {
            oam_warning_log0(hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_ROAM,
                "hmac_roam_check_use_open_auth_with_pmkid:use open auth with pmkid");
            return OAL_TRUE;
        }
    }
    return OAL_FALSE;
    ...
    mac_bss_dscr_stru *bss_dscr)
{
    ...
    if (conn_sec->st_crypto.wpa_versions == WITP_WPA_VERSION_1) {
        ...
        if (hmac_key_mgmt_sae(akm) == OAL_TRUE) {
            open_auth_with_pmkid = hmac_roam_check_use_open_auth_with_pmkid(hmac_vap, bss_dscr);
            if (open_auth_with_pmkid == OAL_FALSE) {
                ...
                mac_mib_set_AuthenticationMode(&(hmac_vap->st_vap_base_info), WLAN_WITP_AUTH_OPEN_SYSTEM);
                hmac_vap->en_sae_connect = OAL_TRUE;
                hmac_vap->bit_sae_connect_with_pmkid = OAL_TRUE;
            }
        } else {
            /* 选中akm为PSK且非SAE模式，以非sae方式漫游 */
            mac_mib_set_AuthenticationMode(&(hmac_vap->st_vap_base_info), WLAN_WITP_AUTH_OPEN_SYSTEM);
    ...
    oam_warning_log3(hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_ROAM,
        "hmac_roam_renew_privacy_between_wpa_wpa2_wpa3: after match. \
 ...
 *   修改内容  : 新生成函数
 */
uint32_t hmac_roam_renew_privacy(hmac_vap_stru *hmac_vap, mac_bss_dscr_stru *bss_dscr)
{
    uint32_t ret;
    ...
    hmac_roam_renew_privacy_between_wpa_wpa2_wpa3(hmac_vap, &conn_sec, bss_dscr);
...
OAL_STATIC void hmac_roam_stop_p2p_listen(mac_vap_stru *mac_vap)
{
    ...
    /* 漫游扫描允许打断普通同信监听 */
    hmac_reset_same_chan_remain_on_chan(mac_vap);
...
void hmac_roam_scan_comp_cb(void *param)
{
    ...
    if (hmac_device == NULL) {
        oam_error_log0(0, OAM_SF_ROAM, "{hmac_roam_scan_comp_cb::device null!}");
        return;
    }
    ...
    oam_info_log0(scan_record->uc_vap_id, OAM_SF_ROAM, "{hmac_roam_scan_comp_cb::handling scan result!}");

    /* 调用漫游状态机： hmac_roam_handle_scan_result */
    ret = hmac_roam_main_fsm_action(roam_info, ROAM_MAIN_FSM_EVENT_SCAN_RESULT, (void *)scan_bss_mgmt);
    if (ret != OAL_SUCC) {
        oam_warning_log1(0, OAM_SF_ROAM, "{hmac_roam_scan_comp_cb::handling scan result fail! ret:%d}", ret);
    }
}
    ...
    wlan_scan_mode_enum_uint8 scan_mode)
{
    ...
    scan_params->en_scan_mode = scan_mode;
...
uint32_t hmac_roam_check_dual_threshold(hmac_roam_info_stru *roam_info)
{
    ...
    if (mac_is_secondary_sta(hmac_vap) == OAL_TRUE) {
        return OAL_FALSE;
    }
...
/* 函数功能：判断漫游扫描，是否需要指定信道扫描
 * 返回值：TRUE:指定信道扫描；FALSE:非指定信道扫描（全信道扫描）
 */
...
static oal_bool_enum_uint8 hmac_roam_scan_init_need_send_neighbor_req(hmac_roam_info_stru *roam_info)
{
    return (roam_info->pst_hmac_vap->bit_nb_rpt_11k &&
        (roam_info->en_roam_trigger == ROAM_TRIGGER_DMAC || roam_info->is_roaming_trigged_by_cmd == OAL_TRUE) &&
        roam_info->pst_hmac_vap->bit_11k_enable && hmac_roam_is_neighbor_report_allowed(roam_info->pst_hmac_vap));
}

static void hmac_roam_scan_init_prepare_ml_scan_params(hmac_vap_stru *hmac_vap, mac_scan_req_stru *scan_params,
    uint32_t scan_mode)
{
    if (scan_mode != WLAN_SCAN_MODE_MLD_SCAN) {
        return;
    }
#ifdef _PRE_WLAN_FEATURE_ML
    hmac_prepare_ml_scan_req(hmac_vap, scan_params);
#endif
}
...
{
    uint32_t ret;
    uint32_t scan_mode = *(uint32_t *)param; /* scan_mode 枚举 wlan_scan_mode_enum_uint8 */
    ...
    memset_s(&roam_info->st_scan_h2d_params, sizeof(mac_scan_req_h2d_stru), 0, sizeof(mac_scan_req_h2d_stru));
    ...
    hmac_roam_scan_params_init(scan_params, cur_ssid, mac_vap, scan_mode);

#ifdef _PRE_WLAN_FEATURE_11K
    if (hmac_roam_scan_init_need_send_neighbor_req(roam_info) == OAL_TRUE) {
    ...
    hmac_roam_scan_init_prepare_ml_scan_params(roam_info->pst_hmac_vap, scan_params, scan_mode);

    hmac_roam_main_change_state(roam_info, ROAM_MAIN_STATE_SCANING);

    /* 调用漫游状态机函数： hmac_roam_scan_channel */
...
{
    uint32_t ret;
    uint32_t timeout = ROAM_SCAN_TIME_MAX;
    uint32_t extral_timeout = 0;
    mac_device_stru *mac_device = NULL;
    ...
    /* 发起背景扫描: hmac_scan_proc_scan_req_event */
    ...
    if (roam_info->pst_hmac_vap != NULL) {
        mac_device = mac_res_get_dev(roam_info->pst_hmac_vap->st_vap_base_info.uc_device_id);
        if (mac_device != NULL && hmac_scan_is_ap_up_connect_user(mac_device) == OAL_TRUE) {
            /* 有ap关联用户，增加额外超时等待时间 */
            extral_timeout = WLAN_AP_USER_CONNECT_EXTRA_WAIT_TIME;
        }
    }
    /* 启动扫描超时定时器 */
    timeout += extral_timeout;
    hmac_roam_main_start_timer(roam_info, timeout);
...
void hmac_roam_scene4_process(hmac_vap_stru *hmac_vap, hmac_roam_info_stru *roam_info)
{
    ...
    if ((roam_info->st_config.uc_scenario_enable == OAL_TRUE) &&
        ...
        if (roam_info->st_alg.uc_better_rssi_scan_period == ROAM_ENV_BETTER_RSSI_PERIOD) {
            ...
            roam_info->st_alg.uc_better_rssi_null_period = 0;
            hmac_roam_start_fill_info(&roam_start_params, ROAM_SCAN_CHANNEL_ORG_0, OAL_TRUE, NULL, ROAM_TRIGGER_APP);
            hmac_roam_start(hmac_vap, &roam_start_params);
        }
    } else {
...
uint32_t hmac_roam_check_bkscan_result(hmac_vap_stru *hmac_vap, hmac_scan_record_stru *scan_record)
{
    ...
    resume_roaming = (roam_info->st_alg.uc_another_bss_scaned) &&
    ...
    /* 漫游场景识别3: 关闭漫游 */
    if (hmac_roam_alg_need_to_stop_roam_trigger(roam_info) == OAL_TRUE) {
...
{
    if (memcpy_s(roam_info->st_bsst_rsp_info.auc_target_bss_addr, WLAN_MAC_ADDR_LEN,
...
OAL_STATIC uint32_t hmac_roam_check_bss_in_scan_list(hmac_roam_info_stru *roam_info,
    hmac_bss_mgmt_stru *bss_mgmt, hmac_vap_stru *hmac_vap)
{
    oal_dlist_head_stru *entry = NULL;
    hmac_scanned_bss_info *scanned_bss = NULL;
    mac_bss_dscr_stru *bss_dscr = NULL;
    ...
    if (oal_any_null_ptr3(roam_info, bss_mgmt, hmac_vap)) {
    ...
    /* 漫游MLD 扫描结束，不用重新选网，采用之前选网结果触发漫游关联 */
    if ((roam_info->scan_info.mld_scan == OAL_TRUE) &&
        (ether_is_all_zero(roam_alg->max_rssi_bss_bssid) == OAL_FALSE)) {
        bss_dscr = (mac_bss_dscr_stru *)hmac_scan_get_scanned_bss_by_bssid(&(hmac_vap->st_vap_base_info),
            roam_alg->max_rssi_bss_bssid);
        memset_s(roam_alg->max_rssi_bss_bssid, WLAN_MAC_ADDR_LEN, 0, WLAN_MAC_ADDR_LEN);
        if (bss_dscr != NULL) {
            /* 漫游ml 扫描后，赋值给roam_alg.max_rssi 需要采用对bss.rssi 加权计算后的rssi */
            tmp_rssi = hmac_roam_alg_adjust_rssi_weight(bss_dscr);
            hmac_roam_update_preferred_bss(roam_alg, bss_dscr, tmp_rssi);
            return OAL_SUCC;
        }
    }
...
OAL_STATIC void hmac_roam_ft_to_non_ft(mac_vap_stru *mac_vap, mac_bss_dscr_stru *bss_dscr)
{
    wlan_auth_alg_mode_enum_uint8 auth_mode;
    uint8_t *md_ie = NULL;
    mac_ies_info ies_info = {0};
    mac_crypto_settings_stru ap_crypto_settings = {0};

    if (bss_dscr->mgmt_len <= MAC_80211_FRAME_LEN + MAC_SSID_OFFSET) {
        return;
    }
    ies_info.frame_ies = bss_dscr->auc_mgmt_buff + MAC_80211_FRAME_LEN + MAC_SSID_OFFSET;
    ies_info.frame_len = bss_dscr->mgmt_len - MAC_80211_FRAME_LEN - MAC_SSID_OFFSET;

    auth_mode = mac_mib_get_AuthenticationMode(mac_vap);
    md_ie = mac_find_ie(MAC_EID_MOBILITY_DOMAIN, &ies_info);
    mac_ie_get_rsn_cipher(bss_dscr->puc_rsn_ie, &ap_crypto_settings);
    /* FT到非FT条件:
     * (1)目标AP不支持FT;
     * (2)自身支持MLD, 目标AP为MLD 且非FT olny; */
    if ((auth_mode == WLAN_WITP_AUTH_FT) && ((md_ie == NULL)
#ifdef _PRE_WLAN_FEATURE_ML
        || (mac_mib_get_MultiLinkActivated(mac_vap->pst_mib_info) == OAL_TRUE &&
            bss_dscr->ml_info.ml_cap == OAL_TRUE &&
            hmac_key_mgmt_only_ft(ap_crypto_settings.aul_akm_suite, WLAN_AUTHENTICATION_SUITES) == OAL_FALSE)
#endif
            )) {
        oam_warning_log0(mac_vap->uc_vap_id, OAM_SF_ROAM,
            "hmac_roam_ft_to_non_ft::roam from FT to Non_FT, change to open system");
...
OAL_STATIC mac_bss_dscr_stru *hmac_roam_get_bss_dscr(hmac_roam_info_stru *roam_info, hmac_vap_stru *hmac_vap)
{
    ...
    if ((g_wlan_spec_cfg->feature_11be_is_open == OAL_TRUE) &&
        (hmac_roam_mld_check_target_bss_is_link_vap(hmac_vap, bss_dscr) == OAL_FALSE)) {
        hmac_roam_update_11be_protocol_mib(hmac_vap, bss_dscr->auc_bssid);
    }
#endif

#ifdef _PRE_WLAN_FEATURE_11R
    hmac_roam_ft_to_non_ft(&(hmac_vap->st_vap_base_info), bss_dscr);
#endif
...
static void hmac_roam_handle_dualband_roam_report(hmac_vap_stru *hmac_vap, hmac_roam_info_stru *roam_info,
                                                  mac_bss_dscr_stru *bss_dscr)
{
    uint32_t ret;
    ...
    roam_info->is_support_dualband_roam = (bss_dscr->dualband_roam_cap && curr_bss_dscr->dualband_roam_cap);
    event.type = DRV_EVENT_ROAM_START;
    event.band = bss_dscr->st_channel.en_band;
    ...
    oam_warning_log2(hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_ROAM,
        "{hmac_roam_handle_dualband_roam_report::support_zero_roaming[%d][%d]}",
        bss_dscr->dualband_roam_cap, curr_bss_dscr->dualband_roam_cap);
    if (event.support_zero_roaming != 1) {
        return;
    }
    ret = memcpy_s(event.auc_target_bssid, WLAN_MAC_ADDR_LEN, bss_dscr->auc_bssid, WLAN_MAC_ADDR_LEN);
    if (ret != EOK) {
        oam_warning_log0(0, OAM_SF_ROAM, "{hmac_roam_handle_dualband_roam_report::roam_info bssid copy failed}");
        return;
    }
                                                          ...
                                                          hmac_bss_mgmt_stru *bss_mgmt)
{
    ...
    ret = hmac_roam_check_bss_in_scan_list(roam_info, bss_mgmt, hmac_vap);
    if (ret != OAL_SUCC) {
    ...
    hmac_roam_handle_dualband_roam_report(hmac_vap, roam_info, bss_dscr);
#ifdef _PRE_WLAN_FEATURE_11V_ENABLE
    /* 发送携带状态码的 BTM resp */
    if (roam_info->en_roam_trigger == ROAM_TRIGGER_11V) {
...
/* 函数大小超50行整改 */
static void hmac_roam_scan_select_bss_succ_handle_ext(hmac_roam_info_stru *roam_info)
{
#ifdef _PRE_WLAN_FEATURE_PMF
    /* 漫游选网后发起join时cancel前一次发起的sa query定时器 */
    if (roam_info->pst_hmac_user != NULL) {
        hmac_stop_sa_query_timer(roam_info->pst_hmac_user);
    }
#endif
#ifdef _PRE_WLAN_FEATURE_11AX
    /* 漫游选网后，退出omi */
...
static uint32_t hmac_roam_scan_select_bss_succ_handle(hmac_roam_info_stru *roam_info, mac_bss_dscr_stru *bss_dscr)
{
    ...
    hmac_user_stru *hmac_user = roam_info->pst_hmac_user;
#ifdef _PRE_WLAN_FEATURE_ML
    uint8_t is_roam_mld = hmac_check_is_mld_connect(&(hmac_vap->st_vap_base_info), bss_dscr);
    uint8_t is_cur_mld_conn = OAL_TRUE;
#endif
...
#endif
    /* 函数大小超50行整改 */
    hmac_roam_scan_select_bss_succ_handle_ext(roam_info);

#ifdef _PRE_WLAN_FEATURE_ML
    is_cur_mld_conn = (hmac_vap_is_legacy_mld_mode(hmac_vap) == OAL_TRUE) ? OAL_FALSE : OAL_TRUE;
    if (hmac_roam_check_need_link_switch(roam_info, hmac_vap, bss_dscr, is_roam_mld, is_cur_mld_conn) == OAL_TRUE) {
        /* 删除漫游定时器，清除漫游变量，执行link切换 */
        ...
        if (hmac_check_work_in_emlsr(hmac_vap) == OAL_TRUE) {
            oam_warning_log0(hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_ROAM,
                "hmac_roam_scan_select_bss_succ_handle::roam seletc same ap. not roam or link_switch in EMLSR mode");
            return OAL_SUCC;
        } else {
            return hmac_mlsr_link_switch(hmac_vap, hmac_mld_get_another_vap(hmac_vap), LINK_SWITCH_FOR_ROAM, 0);
        }
    }

#ifdef _PRE_WLAN_FEATURE_EMLSR
    /* 漫游选网后，退出emlsr */
    hmac_vap_state_change_notify_emlsr(hmac_vap, NULL, COFEATURE_ROAM_EVENT, OAL_FALSE);
#endif

    /* 漫游入网前，通知其他模块wur、emlsr */
    hmac_vap_state_change_notify_feature(hmac_vap, hmac_user, COFEATURE_ROAM_EVENT, COFEATURE_ENTER);

    /* 非mld方式,不涉及MLD模式切换 */
    if (is_roam_mld == OAL_FALSE && is_cur_mld_conn == OAL_FALSE) {
        ...
        return hmac_mld_start_roam_connect(roam_info, is_roam_mld, is_cur_mld_conn, bss_dscr);
    }
#else
    return hmac_roam_main_fsm_action(roam_info, ROAM_MAIN_FSM_EVENT_START_CONNECT,
        (void *)&roam_info->save_bss_dscr_info);
...
/*
 * 功能描述  : 漫游扫描结果没有可用bss时处理流程
 */
static uint32_t hmac_roam_scan_no_valid_bss_handle_proc(hmac_roam_info_stru *roam_info, mac_device_stru *mac_dev)
{
    int32_t event = DRV_EVENT_ROAM_NO_CANDIDATE;

    oal_cfg80211_wifi_event_report(hmac_roam_get_netdev(roam_info->pst_hmac_vap), GFP_KERNEL,
...
uint32_t hmac_roam_handle_scan_result(hmac_roam_info_stru *roam_info, void *param)
{
    ...
    if (hmac_roam_main_check_state(roam_info, MAC_VAP_STATE_UP, ROAM_MAIN_STATE_SCANING) != OAL_SUCC) {
        ...
        /* 漫游状态检查失败停定时器恢复漫游初始化，防止后续扫描定时器超时 */
        hmac_roam_main_del_timer(roam_info);
        hmac_roam_main_clear(roam_info);
        return OAL_FAIL;
    }
    ...
    if (hmac_mlsr_is_link_switching() == OAL_TRUE) {
        ...
        oam_warning_log0(0, OAM_SF_ROAM, "{hmac_roam_handle_scan_result::mlsr link is switching, reject roam}");
        return hmac_roam_scan_no_valid_bss_handle_proc(roam_info, mac_dev);
    }
#endif

    bss_dscr = hmac_roam_check_scan_result(roam_info, (hmac_bss_mgmt_stru *)param);
    if (bss_dscr != NULL) {
        return hmac_roam_scan_select_bss_succ_handle(roam_info, bss_dscr);
    }

    /* 漫游扫描结果没有可用bss时，后续处理流程 */
    return hmac_roam_scan_no_valid_bss_handle_proc(roam_info, mac_dev);
}

uint8_t hmac_check_roam_main_state_is_roaming(hmac_vap_stru *hmac_vap)
{
    hmac_roam_info_stru *roam_info = NULL;
    ...
    roam_info = (hmac_roam_info_stru *)hmac_vap->pul_roam_info;
    if (roam_info != NULL && roam_info->en_main_state != ROAM_MAIN_STATE_INIT) {
        return OAL_TRUE;
    }
    ...
    hmac_conn_param_stru *connect_param)
{
    ...
    if (hmac_device == NULL) {
        return ROAM_SCAN_CHANNEL_ORG_0;
    }
