                                 ...
                                 uint8_t uc_tid)
{
    ...
    hmac_tid = hmac_get_tid_ba_info(pst_hmac_user, uc_tid);
    if (hmac_tid == NULL) {
        return;
    }
    /* 如果超时定时器已经被注册则返回 */
...
OAL_STATIC void hmac_ba_rx_baw_init_he_eht(hmac_user_stru *hmac_user,
    hmac_ba_rx_stru *ba_rx_stru, uint8_t tid, uint16_t baw_size)
{
...
void hmac_ba_rx_win_init(hmac_vap_stru *hmac_vap, hmac_user_stru *hmac_user, uint8_t rx_tid)
{
    ...
    mac_device_stru *mac_dev = mac_res_get_dev(hmac_vap->st_vap_base_info.uc_device_id);
    uint16_t index;
    hmac_tid_ba_stru *hmac_tid = hmac_get_tid_ba_info(hmac_user, rx_tid);
    if (hmac_tid == NULL) {
        return;
    }
 ...
 *   修改内容  : 新生成函数
 */
uint32_t hmac_ba_send_reorder_timeout(hmac_ba_rx_stru *rx_ba, hmac_vap_stru *hmac_vap,
    hmac_ba_alarm_stru *alarm_data, uint32_t *timeout)
{
    ...
    while (us_baw_head != us_baw_end) {
        ...
        ret = hmac_ba_rx_prepare_bufflist(hmac_vap, pst_rx_buf, &st_netbuf_head);
        if (ret != OAL_SUCC) {
            uc_buff_count++;
            us_baw_head = hmac_ba_seqno_add(us_baw_head, 1);
            continue;
    ...
    if (us_baw_start != rx_ba->us_baw_start) {
        ...
        oam_warning_log4(hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_BA,
...
uint32_t hmac_ba_timeout_fn(void *p_arg)
{
    ...
    if (hmac_ba_timeout_param_is_valid(alarm_data, hmac_vap, hmac_user, rx_tid) != OAL_TRUE) {
        return OAL_ERR_CODE_PTR_NULL;
    }

    hmac_tid = hmac_get_tid_ba_info(hmac_user, rx_tid);
    mac_device = mac_res_get_dev(hmac_vap->st_vap_base_info.uc_device_id);
    if (oal_any_null_ptr2(hmac_tid, mac_device)) {
        return OAL_ERR_CODE_PTR_NULL;
    }
                                     ...
                                     hmac_tid_ba_stru *pst_tid_info)
{
    ...
    if (pst_tid_info->en_ba_handle_rx_enable == OAL_FALSE) {
        oam_warning_log2(pst_hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_BA,
                         ...
                         pst_tid_info->uc_tid_no, pst_hmac_user->st_user_base_info.us_assoc_id);
        return MAC_REQ_DECLINED;
    }
    return MAC_SUCCESSFUL_STATUSCODE;
}
...
void hmac_up_rx_bar(hmac_vap_stru *hmac_vap, mac_rx_ctl_stru *rx_info, oal_netbuf_stru *netbuf)
{
    ...
    /*  获取用户指针 */
    pst_ta_user = mac_vap_get_hmac_user_by_addr(&(hmac_vap->st_vap_base_info), puc_sa_addr, WLAN_MAC_ADDR_LEN);
    if (pst_ta_user == NULL) {
    ...
    hmac_tid = hmac_get_tid_ba_info(pst_ta_user, uc_tidno);
    if (hmac_tid == NULL) {
        return;
    }
    pst_ba_rx_info = hmac_tid->pst_ba_rx_info;

    hmac_reorder_ba_rx_buffer_bar(pst_ba_rx_info, us_start_seqnum, &(hmac_vap->st_vap_base_info));
}
 ...
 *   修改内容   : 新生成函数
 */
uint32_t hmac_ba_rx_reo_timer_restart(hmac_tid_ba_stru *hmac_tid,
    hmac_ba_rx_stru *ba_rx_hdl, oal_netbuf_stru *netbuf_mpdu)
{
    ...
    frw_timer_create_timer_m(&(hmac_tid->st_ba_timer), hmac_ba_timeout_fn,
        timeout, &(ba_rx_hdl->st_alarm_data), OAL_FALSE,
    ...
    hmac_ba_rx_stru *ba_rx_hdl, oal_netbuf_head_stru *rpt_list, uint8_t *buff_is_valid)
{
    ...
    if (oal_any_null_ptr3(netbuf, rx_ctl, hmac_user)) {
        return OAL_FAIL;
    }
    hmac_tid = hmac_get_tid_ba_info(hmac_user, mac_get_rx_cb_tid(rx_ctl));
    if (hmac_tid == NULL) {
    ...
    uint32_t payload_len)
{
    ...
    tid = hmac_get_tid_ba_info(hmac_user, tid_id);
    if (tid == NULL) {
        oam_error_log0(hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_BA, "{hmac_mgmt_rx_delba::tid is null.}");
        return OAL_ERR_CODE_PTR_NULL;
    }
 ...
 */
void hmac_process_addba_req_exten_ie(hmac_ba_rx_stru *ba_rx_stru, hmac_user_stru *hmac_user, uint8_t *payload,
    ...
    hmac_vap_stru *hmac_vap, hmac_user_stru *hmac_user, hmac_ba_rx_rsp_stru *ba_rx_rsp)
{
    ...
    hmac_tid_ba_stru *tid_info = NULL;
    uint8_t mib_amsdu_active;

    tid_info = hmac_get_tid_ba_info(hmac_user, ba_rx_rsp->tidno);
    if (tid_info == NULL) {
        return;
    }
...
{
    mac_vap_stru *mac_vap = &(hmac_vap->st_vap_base_info); // 有效性在调用处保证
    hmac_tid_ba_stru *tid_info = hmac_get_tid_ba_info(hmac_user, ba_rx_rsp->tidno);
    if (tid_info == NULL) {
        oam_error_log0(mac_vap->uc_vap_id, OAM_SF_BA, "{hmac_mgmt_rx_addba_rsp::tid_info null}");
        return OAL_ERR_CODE_PTR_NULL;
    }
    /*
    ...
    hmac_vap_stru *hmac_vap, hmac_user_stru *hmac_user, uint8_t *payload, uint32_t frame_body_len)
{
    ...
    hmac_tid = hmac_get_tid_ba_info(hmac_user, ba_rx_rsp.tidno);
    if (hmac_tid == NULL) {
        return OAL_FAIL;
    }
    /* 对tid对应的tx BA会话状态加锁 */
...
uint32_t hmac_del_ba_event(frw_event_mem_stru *pst_event_mem)
{
    ...
    for (uc_tid = 0; uc_tid < WLAN_TID_MAX_NUM; uc_tid++) {
        pst_hmac_tid = hmac_get_tid_ba_info(pst_hmac_user, uc_tid);
