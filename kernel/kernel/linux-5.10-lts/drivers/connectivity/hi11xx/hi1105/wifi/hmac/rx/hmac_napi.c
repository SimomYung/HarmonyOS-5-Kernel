...
int32_t hmac_rxdata_polling(struct napi_struct *napi, int32_t weight)
{
    int32_t l_rx_num = 0;
    int32_t l_rx_max;
    uint32_t need_resched = OAL_FALSE;
    oal_netbuf_stru *pst_netbuf = NULL;
    oal_netdev_priv_stru *netdev_priv = oal_container_of(napi, oal_netdev_priv_stru, st_napi);
    if (netdev_priv == NULL) {
    ...
    l_rx_max = hmac_rxdata_polling_softirq_check(weight);
    while (l_rx_num < l_rx_max) {
        ...
        if (netdev_priv->uc_gro_enable == OAL_TRUE) {
            oal_napi_gro_receive(napi, pst_netbuf);
        } else {
    ...
    /* 队列包量比较少 */
    if (l_rx_num < weight) {
        oal_napi_complete(napi);
        /* 没有按照napi weight阈值取帧并且队列未取空,重新出发调度出队 */
        if ((l_rx_max != weight) && (need_resched == OAL_TRUE)) {
