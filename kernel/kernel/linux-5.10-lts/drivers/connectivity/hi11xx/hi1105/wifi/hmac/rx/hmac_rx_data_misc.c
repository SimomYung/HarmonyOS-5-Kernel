 ...
 *   修改内容  : 新生成函数
 */
void hmac_parse_packet(mac_vap_stru *mac_vap, oal_netbuf_stru *netbuf_eth)
{
    uint16_t us_type;
    uint32_t buf_len;
...
OAL_INLINE void hmac_dev_build_rx_context(oal_netbuf_stru *netbuf, hmac_host_rx_context_stru *rx_context)
{
    ...
    rx_context->hmac_user = (hmac_user_stru *)mac_res_get_hmac_user(cb->bit_rx_user_id);
 ...
 *   修改内容  : 新生成函数
 */
OAL_INLINE void hmac_dev_rx_unique_proc(hmac_vap_stru *hmac_vap, hmac_host_rx_context_stru *rx_context)
{
    ...
    for (; netbuf != NULL; netbuf = oal_netbuf_next(netbuf)) {
        ...
        if (wlan_pm_wkup_src_debug_get() == OAL_TRUE) {
            ...
            hmac_parse_packet(&hmac_vap->st_vap_base_info, netbuf);
...
uint32_t hmac_dev_rx_process_data_event(frw_event_mem_stru *event_mem)
{
...
#endif
    hmac_user = (hmac_user_stru *)mac_res_get_hmac_user(cb->bit_rx_user_id);
    if (hmac_user == NULL) {
    ...
    hmac_dev_rx_unique_proc(hmac_vap, &rx_context);
 ...
 *   修改内容   : 生成函数
 */
uint64_t hmac_user_pn_get(hmac_user_stru *hmac_user, mac_rx_ctl_stru *rx_ctl, uint8_t qos)
{
 ...
 *   修改内容   : 生成函数
 */
OAL_STATIC void hmac_user_pn_update(hmac_user_stru *hmac_user, mac_rx_ctl_stru *rx_ctl, uint8_t qos)
{
    hmac_last_mcast_pn_stru *last_mcast_pn = hmac_get_last_mcast_pn(hmac_user);
    hmac_last_ucast_pn_stru *last_ucast_pn = hmac_get_last_ucast_pn(hmac_user);
    if (last_mcast_pn == NULL || last_ucast_pn == NULL) {
...
uint32_t hmac_rx_pn_check(oal_netbuf_stru *netbuf)
{
    ...
    if (pst_user == NULL) {
        oam_warning_log1(0, OAM_SF_RX, "{hmac_rx_pn_check::hmac_user[%d] null.}", rx_ctl->bit_rx_user_id);
        return OAL_SUCC;
    }

    rx_pn_val = ((uint64_t)rx_ctl->us_rx_msb_pn << BIT_OFFSET_32) | (rx_ctl->rx_lsb_pn);
    last_pn_val = hmac_user_pn_get(pst_user, rx_ctl, qos_flg);
    /* 首包pn为0，此处对首包不检查 */
    if (hmac_rx_pn_is_invalid(rx_pn_val, last_pn_val, rx_ctl->bit_mcast_bcast,
        ...
        if (hmac_need_pn_windod_shift(rx_pn_val, last_pn_val)) {
            hmac_user_pn_update(pst_user, rx_ctl, qos_flg);
        }
        /* 不做PN号检查 */
...
void hmac_rx_data_ap_proc(hmac_vap_stru *hmac_vap, oal_netbuf_head_stru *p_netbuf_head)
{
    ...
    while (netbuf_num != 0) {
        ...
        rx_ctl     = (mac_rx_ctl_stru *)oal_netbuf_cb(netbuf);
        if (hmac_rx_is_vap_status_valid(rx_ctl, &hmac_vap_temp) != OAL_TRUE) {
