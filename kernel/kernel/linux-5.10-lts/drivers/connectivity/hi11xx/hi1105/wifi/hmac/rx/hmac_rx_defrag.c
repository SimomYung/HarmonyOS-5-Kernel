...
#undef THIS_FILE_ID
#define THIS_FILE_ID OAM_FILE_ID_HMAC_RX_DEFRAG_C
 ...
 *   修改内容  : 新生成函数
 */
OAL_STATIC uint32_t hmac_defrag_timeout_fn(void *p_arg)
{
    ...
    oal_netbuf_stru *pst_netbuf)
{
    ...
    if (pst_mac_device == NULL) {
        oam_error_log4(pst_hmac_user->st_user_base_info.uc_vap_id, OAM_SF_ANY,
                       "{hmac_defrag_process::user index[%d] user mac:%02X:XX:XX:XX:%02X:%02X}",
                       pst_hmac_user->st_user_base_info.us_assoc_id,
                       pst_hmac_user->st_user_base_info.auc_user_mac_addr[MAC_ADDR_0],
                       pst_hmac_user->st_user_base_info.auc_user_mac_addr[MAC_ADDR_4],
                       pst_hmac_user->st_user_base_info.auc_user_mac_addr[MAC_ADDR_5]);
        /* user异常，丢弃报文 */
        oal_netbuf_free(pst_netbuf);
    ...
    mac_get_rx_cb_mac_header_addr(pst_rx_ctl) = (uint64_t)(uintptr_t)oal_netbuf_header(pst_new_buf);
    ...
    oal_netbuf_stru *pst_netbuf, uint32_t hrdsize)
{
    ...
    if (pst_mac_hdr == NULL || pst_last_hdr == NULL || rx_ctl == NULL || last_rx_ctl == NULL) {
        return;
    }
...
oal_netbuf_stru *hmac_defrag_process(uint16_t user_idx, oal_netbuf_stru *pst_netbuf, uint32_t hrdsize)
{
    ...
    mac_ieee80211_frame_stru *pst_mac_hdr = (mac_ieee80211_frame_stru *)oal_netbuf_data(pst_netbuf);
    hmac_user_stru *pst_hmac_user = mac_res_get_hmac_user(user_idx);
    if (hrdsize == 0 || pst_mac_hdr == NULL || pst_hmac_user == NULL) {
        oal_netbuf_free(pst_netbuf);
        oam_warning_log0(0, OAM_SF_ANY, "{hmac_defrag_process::mac head len is 0.}");
        return NULL;
    }
