 ...
 *   修改内容  : 新生成函数
 */
void hmac_edca_opt_rx_pkts_stat(hmac_user_stru *hmac_user, uint8_t tidno, mac_ip_header_stru *mac_ip)
{
    /* 过滤IP_LEN 小于 HMAC_EDCA_OPT_MIN_PKT_LEN的报文 */
    if (oal_net2host_short(mac_ip->us_tot_len) < HMAC_EDCA_OPT_MIN_PKT_LEN) {
        return;
    }

    if (mac_ip->uc_protocol == MAC_UDP_PROTOCAL) {
        hmac_user->aaul_txrx_data_stat[WLAN_WME_TID_TO_AC(tidno)][WLAN_RX_UDP_DATA]++;
        oam_info_log4(0, OAM_SF_RX,
            "{hmac_edca_opt_rx_pkts_stat:is udp_data, assoc_id = %d, tidno = %d, type = %d, num = %d",
            hmac_user->st_user_base_info.us_assoc_id, tidno, WLAN_RX_UDP_DATA,
            hmac_user->aaul_txrx_data_stat[WLAN_WME_TID_TO_AC(tidno)][WLAN_RX_UDP_DATA]);
    } else if (mac_ip->uc_protocol == MAC_TCP_PROTOCAL) {
        if (hmac_edca_opt_check_is_tcp_data(mac_ip)) {
            hmac_user->aaul_txrx_data_stat[WLAN_WME_TID_TO_AC(tidno)][WLAN_RX_TCP_DATA]++;
            oam_info_log4(0, OAM_SF_RX,
                "{hmac_edca_opt_rx_pkts_stat:is tcp_data, assoc_id = %d, tidno = %d, type = %d, num = %d",
                hmac_user->st_user_base_info.us_assoc_id, tidno, WLAN_RX_TCP_DATA,
                hmac_user->aaul_txrx_data_stat[WLAN_WME_TID_TO_AC(tidno)][WLAN_RX_TCP_DATA]);
        }
    } else {
