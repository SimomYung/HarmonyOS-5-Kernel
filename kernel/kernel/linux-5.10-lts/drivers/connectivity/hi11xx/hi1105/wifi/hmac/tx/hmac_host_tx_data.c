    ...
    mac_tx_ctl_stru *tx_ctl, oal_netbuf_stru *netbuf)
{
    ...
    if ((tx_ctl->bit_is_m2u_data == OAL_FALSE) &&
        (hmac_tid_need_ba_session(hmac_vap, hmac_user, mac_get_cb_wme_tid_type(tx_ctl), netbuf) == OAL_TRUE)) {
        return OAL_TRUE;
    }
    return OAL_FALSE;
}
    ...
    hmac_vap_stru *hmac_vap, hmac_user_stru *hmac_user, oal_netbuf_stru **origin_netbuf)
{
    ...
    tid_info = hmac_get_tx_tid_info(hmac_user, mac_get_cb_wme_tid_type(tx_ctl));
    if (tid_info == NULL || (tid_info != NULL && !hmac_tid_info_inited(tid_info))) {
...
static uint8_t hmac_tid_ring_tx_allowed(mac_tx_ctl_stru *tx_ctl)
{
    ...
    hmac_user = mac_res_get_hmac_user(mac_get_cb_tx_user_idx(tx_ctl));
    if (oal_unlikely(hmac_user == NULL)) {
        oam_warning_log0(0, OAM_SF_TX, "{hmac_tid_ring_tx_allowed::hmac_user NULL}");
        return OAL_FALSE;
    }

    /* 不支持组播聚合的组播用户预期没有ring，直接返回 */
    if ((hmac_user->st_user_base_info.en_is_multi_user == OAL_TRUE) && (mac_get_mcast_ampdu_switch() == OAL_FALSE)) {
        return OAL_FALSE;
    }
...
uint32_t hmac_host_tx_data(hmac_vap_stru *hmac_vap, hmac_user_stru *hmac_user, oal_netbuf_stru *netbuf)
{
    ...
    if (oal_likely(hmac_get_tid_schedule_list()->tid_schedule_thread)) {
        ...
        tid = mac_get_cb_wme_tid_type(tx_ctl);
        ret = hmac_host_tx_tid_enqueue(hmac_get_tx_tid_info(hmac_user, tid), netbuf);
