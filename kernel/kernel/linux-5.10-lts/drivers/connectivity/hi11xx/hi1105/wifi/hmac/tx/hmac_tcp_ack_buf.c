...
/* 2 全局变量定义 */
OAL_STATIC hmac_tcp_ack_buf_para_stru g_tcp_ack_buf_para[HCC_CHIP_MAX_DEV];
OAL_STATIC hmac_tcp_ack_buf_stru g_tcp_ack_buf[HCC_CHIP_MAX_DEV];
...
uint32_t hmac_tcp_ack_buff_config_mp16(mac_vap_stru *mac_vap, uint16_t len, uint8_t *param)
{
    ...
    switch (tcp_ack_param->en_cmd) {
        case MAC_TCP_ACK_BUF_ENABLE:
            g_tcp_ack_buf_para[HCC_EP_WIFI_DEV].tcp_ack_buf[mac_vap->uc_vap_id] = tcp_ack_param->en_enable;
            break;
        case MAC_TCP_ACK_BUF_TIMEOUT:
            g_tcp_ack_buf_para[HCC_EP_WIFI_DEV].tcp_ack_timeout = tcp_ack_param->uc_timeout_ms;
            break;
        case MAC_TCP_ACK_BUF_MAX:
            g_tcp_ack_buf_para[HCC_EP_WIFI_DEV].tcp_ack_max_num = tcp_ack_param->uc_count_limit;
            break;
        default:
            break;
    }
    ...
    oam_warning_log3(mac_vap->uc_vap_id, OAM_SF_CFG,
                     ...
                     g_tcp_ack_buf_para[HCC_EP_WIFI_DEV].tcp_ack_buf[mac_vap->uc_vap_id],
                     g_tcp_ack_buf_para[HCC_EP_WIFI_DEV].tcp_ack_timeout,
                     g_tcp_ack_buf_para[HCC_EP_WIFI_DEV].tcp_ack_max_num);
    return OAL_SUCC;
}
 ...
 *   修改内容  : 新生成函数
 */
void hmac_tcp_ack_buf_init_para(hmac_device_stru *pst_hmac_device)
{
    hmac_tcp_ack_buf_para_stru *tcp_ack_buf = NULL;

    if (pst_hmac_device->pst_device_base_info->uc_chip_id >= HCC_CHIP_MAX_DEV) {
        oam_warning_log1(0, OAM_SF_TX, "{hmac_tcp_ack_buf_init_para:: uc_chip_id[%d] is exceed!}",
            pst_hmac_device->pst_device_base_info->uc_chip_id);
        return;
    }

    tcp_ack_buf = &g_tcp_ack_buf_para[pst_hmac_device->pst_device_base_info->uc_chip_id];
...
void hmac_tcp_ack_buf_init_user(hmac_user_stru *hmac_user)
{
    ...
    if (hmac_user->st_user_base_info.uc_chip_id >= HCC_CHIP_MAX_DEV) {
        oam_warning_log1(0, OAM_SF_TX, "{hmac_tcp_ack_buf_init_user:: uc_chip_id[%d] is exceed!}",
            hmac_user->st_user_base_info.uc_chip_id);
        return;
    }

    tcp_ack_buf = &g_tcp_ack_buf[hmac_user->st_user_base_info.uc_chip_id];
...
uint32_t hmac_tx_tcp_ack_buf_send(void *p_arg)
{
    ...
    while (oal_netbuf_list_empty(&st_head_t) != OAL_TRUE) {
        ...
        if (hmac_host_tx_data(hmac_vap->st_vap_base_info.uc_vap_id,
            hmac_user->st_user_base_info.us_assoc_id, netbuf) != OAL_SUCC) {
            oal_netbuf_free(netbuf);
            oam_warning_log1(0, OAM_SF_TX, "{hmac_tx_tcp_ack_buf_send::netbuf_num=%d }", netbuf_num);
            continue;
        }
        netbuf_num++;
    }
...
{
    hmac_tcp_ack_buf_stru *tcp_ack_buf = NULL;
    hmac_tcp_ack_buf_para_stru *tcp_ack_buf_para = NULL;
    mac_tx_ctl_stru *tx_ctl = (mac_tx_ctl_stru *)oal_netbuf_cb(netbuf);
    tcp_ack_buf_para = &g_tcp_ack_buf_para[hmac_vap->st_vap_base_info.uc_chip_id];
...
OAL_STATIC void hmac_set_vap_throughput_threshould_by_bw(mac_vap_stru *mac_vap[], uint32_t *limit_throughput_high,
    uint32_t *limit_throughput_low, uint32_t rx_throughput_mbps, uint8_t vap_num_temp)
{
    uint8_t  vap_idx;
    uint8_t vap_num = vap_num_temp;
...
static uint8_t hmac_device_tcp_buf_check_is_single_sta(void)
{
    mac_device_stru *mac_device = mac_res_get_dev(0);
    mac_vap_stru *mac_vap[WLAN_SERVICE_VAP_MAX_NUM_PER_DEVICE] = {NULL};
    uint8_t  up_vap_num;
    ...
    if ((en_mu_vap == OAL_TRUE) || ((!en_mu_vap) && (!is_sta(mac_vap[0])))) {
        return OAL_FALSE;
    }
    return OAL_TRUE;
}
...
void hmac_device_tcp_buf_switch(uint32_t tx_throughput_mbps, uint32_t rx_throughput_mbps)
{
    ...
    /* 多vap或 单ap场景，去使能 */
    if (hmac_device_tcp_buf_check_is_single_sta() == OAL_FALSE) {
        if (g_device_tcp_buf_mgmt.last_status) {
            g_device_tcp_buf_mgmt.last_status = OAL_FALSE;
