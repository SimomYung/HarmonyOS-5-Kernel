 ...
 *   修改内容  : 新生成函数
 */
oal_bool_enum_uint8 hmac_judge_rx_netbuf_classify(mac_llc_snap_stru *snap, uint32_t buf_len)
{
    if (snap == NULL) {
        return OAL_FALSE;
    }
#ifdef _PRE_WLAN_TCP_OPT_DEBUG
    oam_warning_log1(0, OAM_SF_ANY, "**hmac_judge_rx_netbuf_classify, us_ether_type = %d**", snap->us_ether_type);
#endif
    return hmac_judge_rx_netbuf_is_tcp_ack(snap, buf_len);
}
    ...
    hcc_chan_type dir, hmac_tcp_opt_queue type)
{
    ...
    if (dir == HCC_RX) {
        ...
        if (pst_hmac_vap->st_vap_base_info.en_vap_mode == WLAN_VAP_MODE_BSS_AP) {
            hmac_rx_process_data_ap_tcp_ack_opt(pst_hmac_vap->st_vap_base_info.uc_vap_id, &st_head_t);
        } else if (pst_hmac_vap->st_vap_base_info.en_vap_mode == WLAN_VAP_MODE_BSS_STA) {
            /* 将需要上报的帧逐一出队处理 */
            hmac_rx_lan_frame(&st_head_t);
        }
    } else {
        oal_netbuf_head_init(&st_head_t);
        head = &pst_hmac_vap->st_hmac_tcp_ack[dir].data_queue[type];
...
int32_t hmac_tcp_ack_process(uint32_t bus_dev)
#else
...
#endif
{
    ...
    uint32_t bus_dev = HCC_EP_WIFI_DEV;
#endif
    ...
    if (bus_dev >= HCC_CHIP_MAX_DEV) {
        oam_error_log1(0, OAM_SF_TX, "{hmac_tcp_ack_process::pst_mac_chip[%d] is exceed.}", bus_dev);
        return 0;
    }

    pst_mac_chip = hmac_res_get_mac_chip(bus_dev);
    if (pst_mac_chip == NULL) {
        oam_error_log1(0, OAM_SF_TX, "{hmac_tcp_ack_process::pst_mac_chip[%d] null.}", bus_dev);
        return OAL_FALSE;
    }
...
#ifdef _PRE_DUAL_CHIP
oal_bool_enum_uint8 hmac_tcp_ack_need_schedule(uint32_t bus_dev)
#else
...
#endif
{
    ...
    if (bus_dev >= HCC_CHIP_MAX_DEV) {
        oam_error_log1(0, OAM_SF_TX, "{hmac_tcp_ack_need_schedule::pst_mac_chip[%d] is exceed.}", bus_dev);
        return 0;
    }
    pst_mac_chip = hmac_res_get_mac_chip(bus_dev);
    if (pst_mac_chip == NULL) {
        oam_error_log1(0, OAM_SF_TX, "{hmac_tcp_ack_need_schedule::pst_mac_chip[%d] null.}", bus_dev);
        return OAL_FALSE;
    }
...
void hmac_sched_transfer(uint32_t bus_dev)
{
    struct hcc_handler *hcc = hcc_get_handler(bus_dev);
    if (hcc == NULL) {
        oam_error_log1(0, OAM_SF_ANY, "{hmac_sched_transfer::hcc[%d] null.}", bus_dev);
        return;
    }
...
int32_t hmac_set_hmac_tcp_ack_process_func(uint32_t bus_dev, hmac_tcp_ack_process_func p_func)
{
    struct hcc_handler *hcc = hcc_get_handler(bus_dev);

    if (hcc == NULL) {
        oam_error_log1(0, OAM_SF_TX, "{hmac_set_hmac_tcp_ack_process_func::hcc[%d] null.}", bus_dev);
    } else {
...
int32_t hmac_set_hmac_tcp_ack_need_schedule(uint32_t bus_dev, hmac_tcp_ack_need_schedule_func p_func)
{
    struct hcc_handler *hcc = hcc_get_handler(bus_dev);

    if (hcc == NULL) {
        oam_error_log1(0, OAM_SF_TX, "{hmac_set_hmac_tcp_ack_need_schedule::hcc[%d] null.}", bus_dev);
    } else {
    ...
    hmac_vap_stru *hmac_vap, oal_netbuf_stru *netbuf)
{
    ...
    if (hmac_judge_rx_netbuf_classify(pst_mac_llc_snap_netbuf, buf_len) == OAL_TRUE) {
        ...
        hmac_sched_transfer(pst_hmac_device->pst_device_base_info->uc_device_id);
        return OAL_TRUE;
    }
#endif
    return OAL_FALSE;
                             ...
                             oal_netbuf_head_stru *pst_netbuf_header)
{
    ...
    while (!!(pst_netbuf = oal_netbuf_delist_nolock(pst_netbuf_header))) {
        if (hmac_transfer_rx_tcp_ack_handler(pst_hmac_device, pst_hmac_vap, pst_netbuf) == OAL_FALSE) {
