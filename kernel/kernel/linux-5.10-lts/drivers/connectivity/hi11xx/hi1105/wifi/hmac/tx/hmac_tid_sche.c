...
static void hmac_tid_push_ring_fail_handle_ptr(hmac_user_stru *hmac_user, hmac_tid_info_stru *tid_info)
{
    ...
    oal_netbuf_stru *netbuf = NULL;
    uint16_t netbuf_index;
    un_rw_ptr need_release_ptr;
    uint8_t need_recover = 0;

    if ((tx_ring->base_ring_info.read_index != tx_ring->base_ring_info.write_index) ||
        (tx_ring->release_index != tx_ring->base_ring_info.read_index) ||
        (oal_atomic_read(&tid_info->ring_tx_mode) != H2D_SWITCHING_MODE)) {
        return;
    }
    /* device侧ring硬件读写指针相等但卡在H2D_SWITCHING_MODE,怀疑host侧软件维护msdu_cnt异常或write_index异常，增加维测+自愈 */
    oam_error_log3(0, 0, "{hmac_tid_push_ring_fail_handle_ptr::H2D_SWITCHING_MODE recover!! \
        write_ptr[%d] release_ptr[%d], msdu_cnt[%d] }",
        tx_ring->base_ring_info.write_index, tx_ring->release_index, oal_atomic_read(&tx_ring->msdu_cnt));
    mutex_lock(&tid_info->tx_ring.tx_comp_lock);
    /* 遍历tx_ring是否存在netbuf, 存在则释放 */
    for (netbuf_index = 0; netbuf_index < host_ring_size; netbuf_index++) {
        netbuf = tx_ring->netbuf_list[netbuf_index];
        if (netbuf != NULL) {
            need_release_ptr.rw_ptr = netbuf_index;
            hmac_tx_ring_unmap_netbuf(tx_ring, netbuf, need_release_ptr.st_rw_ptr.bit_rw_ptr);
            hmac_tx_ring_release_netbuf(tx_ring, netbuf, need_release_ptr.st_rw_ptr.bit_rw_ptr);
            hmac_tx_reset_msdu_info(tx_ring, need_release_ptr);
            need_recover |= 1;
            oam_error_log0(0, 0, "{hmac_tid_push_ring_fail_handle_ptr::there is a netbuf in tx_ring, write_ptr err!}");
        }
    }
    need_recover |= (oal_atomic_read(&tx_ring->msdu_cnt) != 0) ? 1 : 0;
...
static void hmac_tid_push_ring_fail_handle(hmac_vap_stru *hmac_vap, hmac_user_stru *hmac_user,
    hmac_tid_info_stru *tid_info)
{
    ...
    if (++tid_info->push_fail_cnt % HMAC_TID_PUSH_RING_FAIL_DUMP_CNT == 0) {
        ...
        /* 打印host侧tx_ring指针信息，检查是否更新 */
        oam_warning_log4(0, 0, "{hmac_tid_push_ring_fail_handle::read[%d] write[%d] release[%d] msdu_cnt[%d]!}",
        ...
        if (tid_info->push_fail_cnt == HMAC_TID_PUSH_RING_FAIL_DUMP_CNT || tid_debug_info.need_recover == 1) {
            ...
            tid_debug_info.user_idx = hmac_user->st_user_base_info.us_assoc_id;
            tid_debug_info.tid_no = tid_info->tid_no;
            tid_debug_info.ring_tx_mode = (uint8_t)oal_atomic_read(&tid_info->ring_tx_mode);
            hmac_config_send_event(&hmac_vap->st_vap_base_info, WLAN_CFGID_RING_SWITCH_FAIL_DEBUG,
                sizeof(h2d_tid_debug_info_stru), (uint8_t *)&tid_debug_info);
            hmac_tid_push_ring_fail_handle_ptr(hmac_user, tid_info);
        }
    }
}
...
static uint8_t hmac_tid_push_ring_process(hmac_tid_info_stru *tid_info, void *param)
{
    ...
    if (oal_unlikely(func == NULL)) {
        hmac_tid_push_ring_fail_handle(hmac_vap, hmac_user, tid_info);
        return OAL_CONTINUE;
    }
...
static inline uint8_t hmac_dump_tid_queue(hmac_tid_info_stru *tid_info, void *param)
{
    if (oal_unlikely(tid_info == NULL)) {
    ...
    if (oal_netbuf_list_len(&tid_info->tid_queue)) {
        oam_warning_log3(0, 0, "{hmac_dump_tid_queue::tid_no[%d] user_index[%d] tid_res_bitmap[%d]}",
            tid_info->tid_no, tid_info->user_index, hmac_tid_get_res_bitmap());
        return OAL_RETURN;
    }
