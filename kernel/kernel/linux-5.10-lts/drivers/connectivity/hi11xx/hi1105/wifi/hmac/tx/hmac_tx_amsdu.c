...
OAL_STATIC uint32_t hmac_amsdu_send(hmac_vap_stru *pst_vap, hmac_user_stru *pst_user, hmac_amsdu_stru *pst_amsdu)
{
    ...
    if (pst_amsdu->msdu_num == 1) {
        if (memmove_s(oal_netbuf_data(pst_net_buf) + SNAP_LLC_FRAME_LEN, OAL_MAC_ADDR_LEN + OAL_MAC_ADDR_LEN,
            oal_netbuf_data(pst_net_buf), OAL_MAC_ADDR_LEN + OAL_MAC_ADDR_LEN) != EOK) {
...
OAL_STATIC OAL_INLINE uint32_t hmac_amsdu_alloc_netbuf(hmac_amsdu_stru *pst_amsdu, oal_netbuf_stru *pst_buf)
{
    ...
    if (memcpy_s(oal_netbuf_cb(pst_dest_buf), sizeof(mac_tx_ctl_stru),
        oal_netbuf_cb(pst_buf), sizeof(mac_tx_ctl_stru)) != EOK) {
...
uint32_t hmac_amsdu_tx_process(hmac_vap_stru *pst_vap, hmac_user_stru *pst_user, oal_netbuf_stru *pst_buf)
{
    ...
    if (pst_amsdu->msdu_num == 0) {
        ...
        /* 申请netbuf用于聚合amsdu */
        if (hmac_amsdu_alloc_netbuf(pst_amsdu, pst_buf) != OAL_SUCC) {
    ...
    oal_netbuf_stru *buf, mac_tx_ctl_stru *tx_ctl)
{
    ...
    if (hmac_user_is_amsdu_support(user, tid_no) == OAL_FALSE) {
        return;
    }
    ...
    oal_netbuf_stru *netbuf)
{
    ...
    uint8_t tid_no = 0;
    if (tx_ctl == NULL) {
        oam_error_log0(hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_AMSDU, "{hmac_amsdu_get_netbuf_cap::tx_ctl null}");
        return OAL_FALSE;
    }
    tid_no = mac_get_cb_wme_tid_type(tx_ctl);
