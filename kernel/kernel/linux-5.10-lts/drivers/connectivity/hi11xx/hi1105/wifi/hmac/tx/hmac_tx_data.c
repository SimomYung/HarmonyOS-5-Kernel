...
#include "hmac_pwl.h"
#endif
#ifdef _FEATURE_PREVENT_ARP_DECEPTION
#include "wlan_security.h"
#endif
#ifdef _PRE_WLAN_FEATURE_CLAT
#include "hmac_clat.h"
#endif
#undef THIS_FILE_ID
...
void hmac_tx_report_eth_frame(hmac_vap_stru *pst_hmac_vap, oal_netbuf_stru *pst_netbuf)
{
    ...
    /* 统计以太网下来的数据包统计 */
    hmac_stat_vap_tx_netbuf(pst_hmac_vap->st_vap_base_info.uc_vap_id, pst_netbuf);
    ...
    mac_tx_ctl_stru *pst_tx_ctl)
{
     ...
     */
    if (pktmark(pst_buf) == VIP_APP_MARK && pst_tx_ctl != NULL) {
    ...
    mac_ether_header_stru *ether_header, mac_tx_ctl_stru *tx_ctl, uint16_t netbuf_len)
{
    ...
    if (mac_mib_get_TxTrafficClassifyFlag(&hmac_vap->st_vap_base_info) == OAL_SWITCH_ON) {
        if (curr_tid != 0) {
            return curr_tid;
        }
    ...
    mac_ether_header_stru *ether_header, mac_tx_ctl_stru *tx_ctl)
{
    ...
    if (mac_is_eapol_key_ptk((mac_eapol_header_stru *)(ether_header + 1)) == OAL_TRUE) {
    ...
    oal_netbuf_stru *netbuf, mac_tx_ctl_stru *tx_ctl)
{
    ...
    if (ether_header == NULL || netbuf_len < sizeof(mac_ether_header_stru)) {
    ...
    switch (ether_header->us_ether_type) {
        ...
        case oal_host2net_short(ETHER_TYPE_VLAN):
            ...
            | 3bit    | 1bit |      12bit      |
             ------------------------------------------------------------------ */
            if (vlan_ethhdr == NULL) {
                oam_error_log0(0, OAM_SF_ANY, "hmac_tx_classify_lan_to_wlan_eth_type_handle:vlan_ethhdr null");
                return curr_tid;
            }
...
uint8_t hmac_tx_wmm_acm(oal_bool_enum_uint8 en_wmm, hmac_vap_stru *pst_hmac_vap, uint8_t *puc_tid)
{
    ...
    while ((g_uc_ac_new != WLAN_WME_AC_BK) &&
           (mac_mib_get_QAPEDCATableMandatory(&pst_hmac_vap->st_vap_base_info, g_uc_ac_new) == OAL_TRUE)) {
        switch (g_uc_ac_new) {
            case WLAN_WME_AC_VO:
 ...
 *   修改内容  : 新生成函数
 */
void hmac_tx_classify(hmac_vap_stru *hmac_vap, hmac_user_stru *hmac_user,
    oal_netbuf_stru *netbuf, mac_tx_ctl_stru *tx_ctl)
{
    uint8_t uc_tid = 0;
    ...
    if (tx_ctl == NULL) {
        oam_error_log0(hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_TX, "{hmac_tx_classify::tx_ctl null.}");
        return;
    }
    ether_type = mac_get_cb_ether_type(tx_ctl);

    /* 非qos下同样需要对EAPOL进行业务识别 */
    hmac_tx_classify_lan_to_wlan(hmac_vap, hmac_user, netbuf, &uc_tid);
    /* 非QoS站点，直接返回，默认走TID6 */
    if (oal_unlikely(hmac_user->st_user_base_info.st_cap_info.bit_qos != OAL_TRUE)) {
        ...
        if ((ether_type == oal_host2net_short(ETHER_TYPE_WAI)) || (ether_type == oal_host2net_short(ETHER_TYPE_PAE)) ||
        ...
        oam_info_log0(hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_TX, "{hmac_tx_classify::user isn't a QoS sta.}");
        return;
    }

    pst_mac_dev = mac_res_get_dev(hmac_user->st_user_base_info.uc_device_id);
    if (oal_unlikely(pst_mac_dev == NULL)) {
        oam_warning_log0(hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_TX, "{hmac_tx_classify::pst_mac_dev null.}");
        return;
    }

    hmac_tx_wmmac_classify(hmac_vap, tx_ctl, pst_mac_dev, &(hmac_user->st_user_base_info), &uc_tid);

#ifdef _PRE_WLAN_FEATURE_HIEX
    if (g_wlan_spec_cfg->feature_hiex_is_open) {
        /* 如果是游戏标记数据：若支持网络切片队列，进NS队列；此外，进VO队列 */
        hmac_hiex_judge_is_game_marked_enter_to_vo(hmac_vap, hmac_user, netbuf, &uc_tid);
    }
#endif
    hmac_tx_classify_get_tid(pst_mac_dev, hmac_vap, tx_ctl, &uc_tid);
#ifdef _PRE_WLAN_FEATURE_VSP
...
uint32_t hmac_tx_filter_security(hmac_vap_stru *hmac_vap, oal_netbuf_stru *netbuf, hmac_user_stru *hmac_user)
{
    ...
    mac_tx_ctl_stru *tx_ctl = (mac_tx_ctl_stru *)oal_netbuf_cb(netbuf);
    if (tx_ctl == NULL) {
        oam_error_log0(hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_TX, "{hmac_tx_filter_security::tx_ctl null.}");
        return OAL_ERR_CODE_PTR_NULL;
    }
    ether_type = mac_get_cb_ether_type(tx_ctl);

    pst_mac_vap = &(hmac_vap->st_vap_base_info);
    pst_mac_user = &(hmac_user->st_user_base_info);
    ...
    if (mac_mib_get_rsnaactivated(pst_mac_vap) == OAL_TRUE) { /* 判断是否使能WPA/WPA2 */
        if (mac_user_get_port(pst_mac_user) != OAL_TRUE) { /* 判断端口是否打开 */
            ...
            if (oal_byteorder_host_to_net_uint16(ETHER_TYPE_PAE) != ether_type) {
                ...
                oam_warning_log3(hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_TX,
 ...
 *   修改内容  : 新生成函数
 */
void hmac_tx_ba_setup(hmac_vap_stru *hmac_vap, hmac_user_stru *hmac_user, uint8_t tidno)
{
    ...
    */
    ...
    st_action_args.arg1 = tidno; /* 该数据帧对应的TID号 */
    ...
    /* 建立BA会话 */
    hmac_mgmt_tx_action(hmac_vap, hmac_user, &st_action_args);
}
 ...
 *   修改内容  : 新生成函数
 */
uint32_t hmac_tx_ucast_process(hmac_vap_stru *hmac_vap,
    oal_netbuf_stru *netbuf, hmac_user_stru *hmac_user, mac_tx_ctl_stru *tx_ctl)
{
    uint32_t ret;
    /* 安全过滤 */
    if (oal_unlikely(hmac_tx_filter_security(hmac_vap, netbuf, hmac_user) != OAL_SUCC)) {
        oam_stat_vap_incr(hmac_vap->st_vap_base_info.uc_vap_id, tx_security_check_faild, 1);
    ...
    if (hmac_tid_need_ba_session(hmac_vap, hmac_user, mac_get_cb_wme_tid_type(tx_ctl)) == OAL_TRUE) {
    ...
    mac_tx_ctl_stru *tx_ctl, oal_netbuf_stru *netbuf, uint8_t **mac_hdr)
{
    ...
    if (headroom_size >= MAC_80211_QOS_HTC_4ADDR_FRAME_LEN) {
        *mac_hdr = (oal_netbuf_header(netbuf) - MAC_80211_QOS_HTC_4ADDR_FRAME_LEN);
        ...
        oam_error_log1(hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_TX,
            "{hmac_tx_encap_mac_hdr:: headroom_size expand fail.}", headroom_size);
        return OAL_ERR_CODE_ALLOC_MEM_FAIL;
    }
    /* 挂接802.11头 */
    mac_get_cb_frame_header_addr(tx_ctl) = (uint64_t)(uintptr_t)(*mac_hdr);
    return OAL_SUCC;
}
#ifdef _PRE_WLAN_FEATURE_PWL
    ...
    mac_tx_ctl_stru *pst_tx_ctl, hmac_user_stru *pst_hmac_user, mac_ieee80211_qos_htc_frame_addr4_stru *pst_hdr_addr4)
{
    ...
    /* 更新帧头长度 */
    if (mac_get_cb_is_4address(pst_tx_ctl) == OAL_FALSE) {
...
void hmac_tx_set_qos_frame_htc(hmac_vap_stru *hmac_vap,
    mac_tx_ctl_stru *tx_ctl, hmac_user_stru *hmac_user, mac_ieee80211_qos_htc_frame_addr4_stru *pst_hdr_addr4)
{
    /* tid7 no ring nor not support */
    if (mac_get_cb_wme_tid_type(tx_ctl) == WLAN_TIDNO_BCAST) {
        return;
    }
    if (mac_user_not_support_htc(&hmac_user->st_user_base_info)) {
        return;
    }
    /* 满足条件时在host标记himit帧并将htc字段配置为全F,在dmac判断OM当前是否使用HTC字段,OM特性不用时,himit特性配置HTC */
#ifdef _PRE_WLAN_FEATURE_HIEX
    if ((g_wlan_spec_cfg->feature_hiex_is_open) && (hmac_tx_enable_himit_htc(hmac_user, tx_ctl) == OAL_TRUE)) {
        mac_get_cb_is_himit_frame(tx_ctl) = OAL_TRUE;
    }
#endif
...
uint32_t hmac_tx_encap_ieee80211_header(hmac_vap_stru *pst_vap, hmac_user_stru *pst_user, oal_netbuf_stru *pst_buf)
{
    ...
    uint8_t uc_buf_is_amsdu = 0;
    uint8_t uc_ic_header = 0;
    mac_ether_header_stru st_ether_hdr;

    if (pst_tx_ctl == NULL) {
        oam_error_log0(0, OAM_SF_TX, "hmac_tx_encap_ieee80211_header::pst_tx_ctl null!");
        return OAL_ERR_CODE_PTR_NULL;
    }

    mac_get_cb_data_type(pst_tx_ctl) = DATA_TYPE_80211;
    ...
    if (memcpy_s(&st_ether_hdr, sizeof(st_ether_hdr), oal_netbuf_data(pst_buf), ETHER_HDR_LEN) != EOK) {
...
#ifdef _PRE_WLAN_FEATURE_MCAST_AMPDU
    if ((pst_user->is_mcast_ampdu == OAL_TRUE) || (mac_get_cb_is_mcast(pst_tx_ctl) == OAL_FALSE)) {
#else
    ...
    oal_netbuf_stru *pst_buf, mac_tx_ctl_stru *pst_tx_ctl)
{
    ...
    if (oal_byteorder_host_to_net_uint16(ETHER_TYPE_ARP) == mac_get_cb_ether_type(pst_tx_ctl)) {
        ...
        if ((puc_ether_payload[BYTE_OFFSET_6] == 0x00) &&
            ...
            /* link切换后，上层下发的arp帧体中SA填的是mld mac地址，与此时link的mac地址不同，不应该替换
               只在非mld模式下检查并替换arp帧中的SA */
            (pst_vap->mld_vap_link_info.mld_mode == WLAN_LEGACY_MLD_MODE) &&
#endif
...
#ifdef _PRE_WLAN_FEATURE_MCAST_AMPDU
    /* 如果组播聚合打开，组播帧走tid2 */
    if ((*hmac_user)->is_mcast_ampdu == OAL_TRUE) {
...
void hmac_tx_vip_data_info_report(mac_vap_stru *mac_vap, mac_ieee80211_qos_htc_frame_addr4_stru *mac_header)
{
    uint16_t user_idx = 0xffff;
    hmac_user_stru *hmac_user = mac_vap_get_hmac_user_by_addr(mac_vap, mac_header->auc_address1, WLAN_MAC_ADDR_LEN);
    if (hmac_user != NULL) {
        user_idx = hmac_user->st_user_base_info.us_assoc_id;
    }
    oam_warning_log3(mac_vap->uc_vap_id, OAM_SF_MLOG, "{hmac_tx_vip_info::smac: %02X:XX:XX:XX:%02X:%02X",
        mac_header->auc_address2[MAC_ADDR_0],
        mac_header->auc_address2[MAC_ADDR_4], mac_header->auc_address2[MAC_ADDR_5]);
    oam_warning_log4(mac_vap->uc_vap_id, OAM_SF_MLOG, "{hmac_tx_vip_info::user_id[%d] dmac:%02X:XX:XX:XX:%02X:%02X",
        user_idx, mac_header->auc_address1[MAC_ADDR_0], mac_header->auc_address1[MAC_ADDR_4],
        mac_header->auc_address1[MAC_ADDR_5]);
}
    ...
    oal_netbuf_stru *pst_buf, mac_tx_ctl_stru *pst_tx_ctl)
{
    ...
    pst_mac_header = (mac_ieee80211_qos_htc_frame_addr4_stru *)(uintptr_t)mac_get_cb_frame_header_addr(pst_tx_ctl);
    ...
    if (uc_data_type == MAC_DATA_EAPOL) {
        ...
        oam_warning_log2(pst_vap->uc_vap_id, OAM_SF_MLOG, "{hmac_tx_vip_info::EAPOL type=%u, len==%u} \
        ...
        hmac_ht_self_cure_event_set(pst_vap->uc_vap_id, pst_mac_header->auc_address1,
            HMAC_HT_SELF_CURE_EVENT_TX_DHCP_FRAME);
        oam_warning_log2(pst_vap->uc_vap_id, OAM_SF_MLOG, "{hmac_tx_vip_info::vapid[%d] DHCP type=%d.[1:discovery  \
            2:offer 3:request 4:decline 5:ack 6:nack 7:release 8:inform]", pst_vap->uc_vap_id, uc_dhcp_type);
        oam_warning_log4(pst_vap->uc_vap_id, OAM_SF_MLOG, "{hmac_tx_vip_info:: DHCP sip: %d.%d, dip: %d.%d.",
    ...
    hmac_tx_vip_data_info_report(pst_vap, pst_mac_header);
}
...
uint32_t hmac_tx_lan_to_wlan_no_tcp_opt_tx_pass_handle(mac_vap_stru *pst_vap, oal_netbuf_stru *pst_buf)
{
    ...
    data_type = pst_tx_ctl->uc_frame_subtype;
...
#ifdef _FEATURE_PREVENT_ARP_DECEPTION
OAL_STATIC uint32_t hmac_check_arp_deception(mac_vap_stru *mac_vap, oal_netbuf_stru *netbuf)
{
    uint8_t data_type;
    data_type = mac_get_data_type_from_8023(netbuf, oal_netbuf_data(netbuf),
        MAC_NETBUFF_PAYLOAD_ETH, oal_netbuf_get_len(netbuf), OAM_OTA_FRAME_DIRECTION_TYPE_TX);
    if ((data_type == MAC_DATA_ARP_REQ || data_type == MAC_DATA_ARP_RSP) &&
        (mac_vap->en_vap_mode == WLAN_VAP_MODE_BSS_AP)) {
        if (wlan_check_arp_spoofing((struct net_device *)(hmac_vap_get_net_device(mac_vap->uc_vap_id)),
            (struct sk_buff *)netbuf) == OAL_TRUE) {
            oam_report_eth_frame(mac_vap->auc_bssid, oal_netbuf_data(netbuf),
                (uint16_t)oal_netbuf_get_len(netbuf), OAM_OTA_FRAME_DIRECTION_TYPE_TX);
            return OAL_FAIL;
        }
    }
    return OAL_SUCC;
...
oal_bool_enum_uint8 hmac_is_ring_tx(mac_vap_stru *mac_vap, oal_netbuf_stru *netbuf, mac_tx_ctl_stru *tx_ctl)
{
    ...
    if (mac_vap->pwl_enable == OAL_TRUE) {
        /* 正式秘钥下来之前都在dmac做加解密 */
        if (hmac_pwl_is_ring_tx(mac_vap, netbuf) == OAL_FALSE) {
            return OAL_FALSE;
        }
    }
#endif
    return dpe_is_ring_tx(mac_vap->uc_vap_id, netbuf, tx_ctl);
}
/*
...
#ifdef _FEATURE_PREVENT_ARP_DECEPTION
    if (hmac_check_arp_deception(mac_vap, *netbuf) != OAL_SUCC) {
        oam_warning_log0(mac_vap->uc_vap_id, OAM_SF_TX, "hmac detect ARP deception");
        oam_stat_vap_incr(mac_vap->uc_vap_id, tx_abnormal_msdu_dropped, 1);
        return OAL_FAIL;
    }
#endif
...
OAL_STATIC uint32_t hmac_transfer_tx_handler(hmac_vap_stru *hmac_vap, oal_netbuf_stru *netbuf)
{
    ...
    if (tx_ctl == NULL) {
        oam_error_log0(0, OAM_SF_TX, "{hmac_transfer_tx_handler::tx_ctl null.}\r\n");
    ...
    if (!mac_get_cb_urgent_tcp_ack(tx_ctl)) {
        oal_spin_unlock_bh(&hmac_vap->st_hmac_tcp_ack[HCC_TX].data_queue_lock[HMAC_TCP_ACK_QUEUE]);
        hmac_sched_transfer(hmac_vap->st_vap_base_info.uc_device_id);
    } else {
        oal_spin_unlock_bh(&hmac_vap->st_hmac_tcp_ack[HCC_TX].data_queue_lock[HMAC_TCP_ACK_QUEUE]);
#ifdef _PRE_DUAL_CHIP
        hmac_tcp_ack_process(hmac_vap->st_vap_base_info.uc_device_id);
#else
...
void hmac_set_trx_switch(uint32_t trx_switch_ini_param)
{
    ...
    dpe_trx_switch_sync(&g_trx_switch);
...
uint16_t hmac_tx_get_user_id(mac_vap_stru *mac_vap, mac_ether_header_stru *ether_hdr)
{
    ...
    if (mac_vap->en_vap_mode == WLAN_VAP_MODE_BSS_AP) {
        if (ether_hdr == NULL) {
            oam_error_log0(mac_vap->uc_vap_id, OAM_SF_TX, "{hmac_get_tx_user_id::ether_hdr null}");
...
uint32_t hmac_tx_lan_to_wlan(mac_vap_stru *mac_vap, oal_netbuf_stru **netbuf)
{
    ...
    if (oal_unlikely((hmac_vap == NULL) || (hmac_user == NULL))) {
        oam_warning_log0(0, OAM_SF_TX, "{hmac_tx_lan_to_wlan::hmac_vap  or user null.}\r\n");
        return OAL_FAIL;
    }
    /* 初始化CB tx rx字段 , CB字段在前面已经被清零， 在这里不需要重复对某些字段赋零值 */
    hmac_tx_cb_field_init(tx_ctl, mac_vap->uc_vap_id, mac_vap->en_vap_mode, us_user_idx, ether_hdr);
#ifdef _PRE_WLAN_CHBA_MGMT
    /* 用户帧统计 */
    ...
    oal_net_device_stru *pst_dev, mac_vap_stru **pst_vap, hmac_vap_stru **pst_hmac_vap)
{
    ...
    *pst_hmac_vap = (hmac_vap_stru *)mac_res_get_hmac_vap((*pst_vap)->uc_vap_id);
    if (*pst_hmac_vap == NULL) {
        oam_error_log0((*pst_vap)->uc_vap_id, OAM_SF_CFG, "{hmac_bridge_vap_xmit::hmac_vap null.}");
        oal_netbuf_free(pst_buf);
        return OAL_NETDEV_TX_OK;
    }
    ...
    hmac_vap_stru *pst_hmac_vap, mac_vap_stru *pst_vap)
{
    ...
    for (user_bitmap_idx = 0; user_bitmap_idx < g_wlan_spec_cfg->max_user_limit; user_bitmap_idx++) {
        ...
        tx_ctl = (mac_tx_ctl_stru *)oal_netbuf_cb(copy_buf);
        tx_ctl->bit_is_m2u_data = OAL_TRUE;
...
static uint32_t hmac_bridge_vap_state_check(hmac_vap_stru *hmac_vap, oal_netbuf_stru *netbuf)
{
    uint8_t uc_data_type;
    mac_vap_stru *pst_vap = &(hmac_vap->st_vap_base_info);

    /* 考虑VAP状态与控制面互斥，需要加锁保护 */
    oal_spin_lock_bh(&hmac_vap->st_lock_state);
    /* 判断VAP的状态，如果ROAM，则丢弃报文 MAC_DATA_DHCP/MAC_DATA_ARP */
    if (pst_vap->en_vap_state == MAC_VAP_STATE_ROAMING) {
        uc_data_type = mac_get_data_type_from_8023(netbuf, (uint8_t *)oal_netbuf_payload(netbuf),
            MAC_NETBUFF_PAYLOAD_ETH, oal_netbuf_get_len(netbuf), OAM_OTA_FRAME_DIRECTION_TYPE_TX);
#ifdef _PRE_WLAN_FEATURE_PWL
        if (((hmac_vap->st_vap_base_info.pwl_enable == OAL_TRUE) && (uc_data_type != MAC_DATA_WAPI)) ||
            ((hmac_vap->st_vap_base_info.pwl_enable == OAL_FALSE) && (uc_data_type != MAC_DATA_EAPOL))) {
#else
        if (uc_data_type != MAC_DATA_EAPOL) {
#endif
        ...
        if (hmac_bridge_vap_xmit_check_drop_frame(pst_vap, netbuf) == OAL_TRUE) {
    ...
    oal_netbuf_stru *pst_buf)
{
...
#endif
    if (is_legacy_sta(pst_vap) == OAL_TRUE) {
...
#ifdef _PRE_WLAN_FEATURE_CLAT
    if (hmac_clat_proc(pst_dev, pst_buf, IPV6_TO_IPV4) != OAL_SUCC) {
        oam_warning_log0(0, OAM_SF_RX, "Clat change ip V6-> ip V4 failed");
    }
#endif
    /* 非组播或非chba mode */
#ifdef _PRE_WLAN_CHBA_MGMT
    ...
    if (ether_hdr == NULL) {
        oam_error_log0(0, OAM_SF_RX, "hmac_vap_xmit::ether_hdr null");
        hmac_free_netbuf_list(pst_buf);
        return;
    }
