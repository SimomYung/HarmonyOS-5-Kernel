...
OAL_STATIC OAL_INLINE oal_bool_enum_uint8 hmac_tid_ba_is_setup(hmac_user_stru *pst_hmac_user, uint8_t uc_tidno)
{
    ...
    hmac_tid = hmac_get_ba_info(pst_hmac_user, uc_tidno);

    return (hmac_tid->st_ba_tx_info.en_ba_status == DMAC_BA_COMPLETE);
}
...
OAL_STATIC OAL_INLINE oal_bool_enum_uint8 hmac_vap_ba_is_setup(hmac_user_stru *pst_hmac_user)
{
    ...
    for (uc_tidno = 0; uc_tidno < WLAN_TID_MAX_NUM; uc_tidno++) {
        ...
        if (ba_info->st_ba_tx_info.en_ba_status == DMAC_BA_COMPLETE) {
            return OAL_TRUE;
        }
    }
...
OAL_STATIC OAL_INLINE void hmac_tx_ba_del(hmac_vap_stru *pst_hmac_vap, hmac_user_stru *pst_hmac_user, uint8_t uc_tidno)
{
    ...
    hmac_user_stru *pst_hmac_user, uint8_t uc_tidno)
{
    ...
    if (mac_mib_get_CfgAmpduTxAtive(&pst_hmac_vap->st_vap_base_info) == OAL_FALSE &&
        (hmac_tid_ba_is_setup(pst_hmac_user, uc_tidno) == OAL_TRUE)) {
...
{
    hmac_ba_stru *pst_hmac_tid_info = hmac_get_ba_info(hmac_user, tidno);
...
OAL_STATIC OAL_INLINE oal_bool_enum_uint8 hmac_tid_need_ba_session(hmac_vap_stru *pst_hmac_vap,
    hmac_user_stru *pst_hmac_user, uint8_t uc_tidno)
{
    hmac_ba_stru *ba_info = hmac_get_ba_info(pst_hmac_user, uc_tidno);
    if (ba_info->en_ba_handle_tx_enable == OAL_FALSE) {
        return OAL_FALSE;
    }

    if (hmac_tid_ba_is_setup(pst_hmac_user, uc_tidno) == OAL_TRUE) {
        if (mac_mib_get_CfgAmpduTxAtive(&pst_hmac_vap->st_vap_base_info) == OAL_TRUE) {
            return OAL_FALSE;
        }
    ...
    if (mac_mib_get_AddBaMode(&pst_hmac_vap->st_vap_base_info) != WLAN_ADDBA_MODE_AUTO) {
        return OAL_FALSE;
    }
