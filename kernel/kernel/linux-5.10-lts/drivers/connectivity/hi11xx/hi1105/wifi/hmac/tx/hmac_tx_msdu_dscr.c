 ...
 *   修改内容 : 新生成函数
 */
static void hmac_tx_reg_write_info_ring_base_addr(hmac_tid_info_stru *tid_info, uintptr_t devva)
{
    hal_host_tx_ring_ops_param_stru info_ring_lsb_param = {
        .tx_ring_table_addr = tid_info->tx_ring.tx_ring_table_addr,
...
static void hmac_tx_reg_write_addr_ring_base_addr(hmac_tid_info_stru *tid_info, uintptr_t devva)
{
...
static void hmac_tx_reg_write_base_addr(hmac_tid_info_stru *tid_info,
    uintptr_t info_ring_devva, uintptr_t addr_ring_devva)
{
 ...
 *   修改内容 : 新生成函数
 */
static void hmac_tx_reg_write_wptr(hmac_tid_info_stru *tid_info, uint16_t wptr)
{
 ...
 *   修改内容 : 新生成函数
 */
static void hmac_tx_reg_write_rptr(hmac_tid_info_stru *tid_info, uint16_t rptr)
{
    hal_host_tx_ring_ops_param_stru param = {
        .tx_ring_table_addr = tid_info->tx_ring.tx_ring_table_addr,
        .val = rptr,
...
static void hmac_tx_reg_write_ml1_rptr(hmac_tid_info_stru *tid_info, uint16_t rptr)
{
 ...
 *   修改内容 : 新生成函数
 */
static void hmac_tx_reg_write_ring_depth(hmac_tid_info_stru *tid_info, uint16_t depth)
{
    hal_host_tx_ring_ops_param_stru param = {
        .tx_ring_table_addr = tid_info->tx_ring.tx_ring_table_addr,
        .val = depth,
 ...
 *   修改内容 : 新生成函数
 */
void hmac_tx_reg_write_max_amsdu_num(hmac_tid_info_stru *tid_info, uint16_t max_amsdu_num)
{
    hal_host_tx_ring_ops_param_stru param = {
        .tx_ring_table_addr = tid_info->tx_ring.tx_ring_table_addr,
        .val = max_amsdu_num,
...
#ifdef _PRE_WLAN_FEATURE_ML
void hmac_tx_reg_write_ml_mode(hmac_tid_info_stru *tid_info, uint8_t ml_mode)
{
    hal_host_tx_ring_ops_param_stru param = {
        .tx_ring_table_addr = tid_info->tx_ring.tx_ring_table_addr,
        .val = ml_mode,
...
void hmac_tx_reg_write_ml_creat(hmac_tid_info_stru *tid_info, hmac_msdu_info_ring_stru *tx_ring)
{
#ifdef _PRE_WLAN_FEATURE_ML
...
void hmac_tx_reg_write_ml_reset(hmac_tid_info_stru *tid_info)
{
#ifdef _PRE_WLAN_FEATURE_ML
 ...
 *   修改内容 : 新生成函数
 */
OAL_STATIC void hmac_reg_write_create_ring_info(hmac_tid_info_stru *tid_info)
{
    uint64_t info_ring_devva = 0;
    uint64_t addr_ring_devva = 0;
    hmac_msdu_info_ring_stru *tx_ring = &tid_info->tx_ring;

    if (hal_host_tx_get_msdu_info_devva(&tx_ring->ring_buf, &info_ring_devva, &addr_ring_devva) != OAL_SUCC) {
    ...
    hmac_tx_reg_write_ml1_rptr(tid_info, tx_ring->base_ring_info.ml1_read_index);
    hmac_tx_reg_write_wptr(tid_info, tx_ring->base_ring_info.write_index);
    hmac_tx_reg_write_ml_creat(tid_info, tx_ring);
    /* info_ring和addr_ring的地址设置放在最后 */
    hmac_tx_reg_write_base_addr(tid_info, (uintptr_t)info_ring_devva, (uintptr_t)addr_ring_devva);
}
...
static uint16_t hmac_tx_reg_read_sw_wptr(hmac_tid_info_stru *tid_info)
{
    hal_host_tx_ring_ops_param_stru param = {
        .tx_ring_table_addr = tid_info->tx_ring.tx_ring_table_addr,
        .lut_idx = tid_info->lut_index,
...
void hmac_tx_reg_write_ring_info(hmac_tid_info_stru *tid_info, tid_cmd_enum_uint8 cmd)
{
    if (cmd == TID_CMDID_ENQUE) {
        ...
        hmac_reg_write_create_ring_info(tid_info);
    ...
    hmac_msdu_info_ring_stru *tx_ring, msdu_info_stru *msdu_info, uint32_t netbuf_len)
{
    ...
    return hmac_tx_dma_unmap_addr((uint8_t *)(uintptr_t)netbuf_iova, netbuf_len + hal_host_tx_get_msdu_dscr_len());
}
...
uint8_t *hmac_tx_netbuf_init_msdu_dscr(oal_netbuf_stru *netbuf)
{
    ...
    uint8_t *msdu_dscr_addr = (uint8_t *)((uintptr_t)oal_netbuf_data(netbuf) - msdu_dscr_len);
 ...
 *   修改内容 : 重构函数
 */
uint32_t hmac_tx_set_msdu_info(mac_vap_stru *mac_vap, hmac_msdu_info_ring_stru *tx_ring, oal_netbuf_stru *netbuf)
{
    uint32_t ret;
    uint64_t devva = 0;
    ...
    msdu_dscr_addr = hmac_tx_netbuf_init_msdu_dscr(netbuf);
    ret = hmac_tx_hostva_to_devva(msdu_dscr_addr, netbuf->len + hal_host_tx_get_msdu_dscr_len(), &devva);
    if (ret != OAL_SUCC) {
    ...
    mac_vap_stru *mac_vap, hmac_msdu_info_ring_stru *tx_msdu_ring, oal_netbuf_stru *netbuf)
{
    ...
    if (oal_unlikely(hmac_tx_set_msdu_info(mac_vap, tx_msdu_ring, netbuf) != OAL_SUCC)) {
        return OAL_ERR_CODE_PTR_NULL;
    }
