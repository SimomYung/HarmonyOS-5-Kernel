...
static inline uint16_t hmac_vsp_source_get_layer_succ_pkts(hmac_vsp_tx_slice_stru *slice, uint16_t layer_number)
{
    return (slice->tx_result.layer_suc_num[layer_number] & 0x7FFF);
}

static inline uint32_t hmac_vsp_source_get_layer_succ_bytes(hmac_vsp_tx_slice_stru *slice, uint16_t layer_number,
    uint16_t normal_pack_length)
{
    return hmac_vsp_source_get_layer_succ_pkts(slice, layer_number) * normal_pack_length;
}
...
uint16_t hmac_vsp_source_get_layerall_max(hmac_vsp_source_stru *source, hmac_vsp_tx_slice_stru *slice)
{
    ...
    for (layer_idx = 0; layer_idx < MAX_LAYER_NUM; layer_idx++) {
        if (slice->layer_info[layer_idx].layer_number == INVALID_LAYER_NUM) {
            break;
        }
        bytes += hmac_vsp_source_get_layer_succ_bytes(slice, slice->layer_info[0].layer_number,
                    slice->layer_info[0].normal_pack_length);
        succ_pkts += hmac_vsp_source_get_layer_succ_pkts(slice, slice->layer_info[0].layer_number);
        total_pkts += slice->layer_info[layer_idx].paket_number;
    }
...
{
    uint32_t bytes = hmac_vsp_source_get_layer_succ_bytes(slice, slice->layer_info[0].layer_number,
        slice->layer_info[0].normal_pack_length);
    uint16_t pkts = hmac_vsp_source_get_layer_succ_pkts(slice, slice->layer_info[0].layer_number);
    if (bytes < source->layer_max_mgmt.layer0_max &&
        hmac_vsp_source_tx_low_ratio(pkts, slice->layer_info[0].paket_number)) {
