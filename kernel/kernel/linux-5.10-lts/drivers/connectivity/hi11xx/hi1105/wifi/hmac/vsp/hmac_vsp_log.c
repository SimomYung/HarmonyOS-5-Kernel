...
{
    uint8_t layer_id;
    uint8_t layer_succ = 0;
    vsp_feedback_frame *feedback = (vsp_feedback_frame *)data;

    for (layer_id = 0; layer_id < feedback->layer_num; layer_id++) {
        if (feedback->layer_succ_num[layer_id] & BIT_OFFSET_15) {
            layer_succ |= (1 << layer_id);
        }
    }
    oam_warning_log4(0, 0, "{hmac_vsp_feedback_log::frame[%d] slice[%d] total_layer[0x%x], layer_succ[0x%x]}",
        feedback->frm_id, feedback->slice_id, feedback->layer_num, layer_succ);
...
static void hmac_vsp_rtt_log(uint8_t *data)
{
    hmac_vsp_tx_slice_stru *slice = (hmac_vsp_tx_slice_stru *)data;
    oam_warning_log4(0, 0, "{hmac_vsp_rtt_log::frame_id[%d] slice_id[%d] arrive_ts[%d], rtt[%d]}",
        slice->frame_id, slice->slice_id, slice->arrive_ts, slice->slice_rtt);
}
...
hmac_vsp_log_func g_vsp_log_func[VSP_LOG_DATATYPE_BUTT] = {
    ...
    hmac_vsp_rtt_log,            /* VSP_LOG_DATATYPE_RTT */
};

uint32_t g_vsp_log_bitmap = VSP_LOG_BITMAP_DISABLE;
...
{
    if (((g_vsp_log_bitmap & log_bitmap) == 0) || !data ||
        datatype >= VSP_LOG_DATATYPE_BUTT || type >= VSP_LOG_TYPE_BUTT) {
        return;
    }
...
{
    g_vsp_log_bitmap = params[0];

    oam_warning_log1(0, 0, "{hmac_vsp_set_debug_lvl::log bitmap set to [0x%x]}", g_vsp_log_bitmap);
