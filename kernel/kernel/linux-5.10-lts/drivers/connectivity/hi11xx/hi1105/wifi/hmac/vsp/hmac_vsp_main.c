...
static void hmac_vsp_cfg_param(hmac_vsp_cmd *cmd, vsp_param *param)
{
    ...
    hmac_vsp_log((uint8_t *)param, VSP_LOG_BITMAP_SLICE_INFO, VSP_LOG_DATATYPE_PARAM, VSP_LOG_TYPE_CFG_PARAM);
}
...
uint32_t hmac_vsp_stop(hmac_user_stru *hmac_user)
{
    ...
    oam_warning_log0(0, 0, "hmac_vsp_stop::finish");
...
uint32_t hmac_vsp_set_times(hmac_vap_stru *hmac_vap, hmac_user_stru *hmac_user, hmac_vsp_cmd *cmd)
{
    hmac_vsp_info_stru *vsp_info = hmac_user->vsp_hdl;
    ...
    vsp_info->param.tmax = cmd->tmax * HMAC_MS_TO_US;
    vsp_info->param.tfeedback = cmd->tfeedback * HMAC_MS_TO_US;
    oam_warning_log3(0, 0, "{hmac_vsp_set_times::vsp_mode[%d], tmax[%d] tfeedback[%d]!}",
                     vsp_info->mode, vsp_info->param.tmax, vsp_info->param.tfeedback);
    return OAL_SUCC;
}
/*
 * 功能描述  : vsp tx通用定时器超时
...
uint32_t hmac_vsp_rx_proc(uint16_t user_idx, oal_netbuf_stru *netbuf)
{
    ...
    if (mac_get_rx_cb_tid(rx_ctl) != WLAN_TIDNO_VSP) {
        return VSP_RX_REFUSE;
    }
...
static uint32_t hmac_vsp_debug_link_cmd_parse(hmac_vsp_debug_cmd *debug_cmd, hmac_vsp_cmd *vsp_cmd)
{
    ...
    hmac_vsp_log((uint8_t *)vsp_cmd, VSP_LOG_BITMAP_SLICE_INFO, VSP_LOG_DATATYPE_CMD, VSP_LOG_TYPE_PARSE_CMD);
