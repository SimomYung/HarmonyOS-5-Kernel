...
static rx_slice_mgmt *hmac_vsp_sink_alloc_rx_slice(hmac_vsp_sink_stru *sink, vsp_msdu_hdr_stru *vsp_hdr)
{
    ...
    hmac_vsp_log((uint8_t *)priv, VSP_LOG_BITMAP_SLICE_INFO, VSP_LOG_DATATYPE_RX_SLICE_PRIV,
...
static inline void hmac_vsp_sink_init_timer_conext(hmac_vsp_sink_stru *sink, vsp_rx_slice_priv_stru *priv)
{
    ...
    hmac_vsp_log((uint8_t *)priv, VSP_LOG_BITMAP_TIMER, VSP_LOG_DATATYPE_RX_SLICE_PRIV, VSP_LOG_TYPE_INIT_RX_TIMER);
}
...
static inline void hmac_vsp_sink_start_rx_slice_timer(hmac_vsp_sink_stru *sink, vsp_rx_slice_priv_stru *priv)
{
    ...
    hmac_vsp_set_slice_timeout(&sink->vsp_base, priv->slice_id, OAL_FALSE);
}
...
static void hmac_vsp_sink_encap_feedback(oal_netbuf_stru *netbuf, rx_slice_mgmt *rx_slice, uint8_t if_cfg_pkt)
{
    vsp_rx_slice_priv_stru *priv = hmac_vsp_sink_get_rx_slice_priv(rx_slice);
    ...
    if (feedback == NULL) {
        oam_error_log0(0, 0, "{hmac_vsp_sink_encap_feedback::feedback null}");
        return;
    }
    ...
    feedback->is_cfg_pkt = if_cfg_pkt;
    ...
    hmac_vsp_log((uint8_t *)feedback, VSP_LOG_BITMAP_PKT_FEEDBACK, VSP_LOG_DATATYPE_FEEDBACK,
...
static uint32_t hmac_vsp_sink_send_feedback(rx_slice_mgmt *rx_slice, hmac_vsp_info_stru *vsp_info, uint8_t if_cfg_pkt)
{
    ...
    hmac_vsp_sink_encap_feedback(netbuf, rx_slice, if_cfg_pkt);
    ...
    hmac_vsp_log((uint8_t *)rx_slice, VSP_LOG_BITMAP_PKT_FEEDBACK, VSP_LOG_DATATYPE_RX_SLICE,
                 VSP_LOG_TYPE_SEND_FEEDBACK);
...
static void hmac_vsp_sink_rx_slice_complete_proc(hmac_vsp_sink_stru *sink, rx_slice_mgmt *rx_slice)
{
    ...
    hmac_vsp_sink_send_feedback(rx_slice, &sink->vsp_base, OAL_FALSE);
...
{
    rx_slice_mgmt *rx_slice = NULL;

    if (hmac_vsp_info_next_timeout_info_invalid(sink)) {
        return;
    }

    rx_slice = hmac_vsp_sink_get_next_timeout_rx_slice(sink);
    if (!rx_slice) {
...
{
    hmac_vsp_fill_rx_slice_info(rx_slice, sink);
    hmac_vsp_sink_send_feedback(rx_slice, &sink->vsp_base, OAL_TRUE);
    ...
    hmac_vsp_sink_stru *sink, vsp_msdu_hdr_stru *vsp_hdr, oal_netbuf_stru *netbuf)
{
    ...
    rx_slice = hmac_vsp_sink_get_pkt_rx_slice(sink, vsp_hdr);
    if (oal_unlikely(!rx_slice)) {
        return OAL_FAIL;
    }
    ...
    oam_warning_log1(0, 0, "hmac_vsp_sink_rx_cfg_pkt_proc::rx_cfg_pkt comp, timestamp[%d]", vsp_hdr->timestamp);
    ...
    hmac_vsp_sink_stru *sink, vsp_msdu_hdr_stru *vsp_hdr, oal_netbuf_stru *netbuf)
{
    if (oal_unlikely(hmac_vsp_sink_rx_expired_pkt(sink, vsp_hdr))) {
        hmac_vsp_log((uint8_t *)vsp_hdr, VSP_LOG_BITMAP_PKT_FEEDBACK, VSP_LOG_DATATYPE_PKT,
...
static void hmac_vsp_sink_rx_slice_tbl_deinit(hmac_vsp_sink_stru *sink)
{
    ...
    for (idx = 0; idx < VSP_FRAME_ID_TOTAL * sink->vsp_base.param.slice_cnt; idx++) {
        rx_slice = sink->rx_slice_tbl[idx];
        if (rx_slice == NULL) {
            continue;
        }
        vsp_vcodec_rx_slice_done(rx_slice);
...
uint32_t hmac_vsp_sink_cfg(hmac_vap_stru *hmac_vap, hmac_user_stru *hmac_user, vsp_param *param)
{
    ...
    if (hmac_vsp_context_init(&g_vsp_sink.vsp_base, hmac_vap, hmac_user) != OAL_SUCC) {
        hmac_vsp_sink_deinit();
        oam_error_log0(0, 0, "hmac_vsp_sink_cfg:hmac_vsp_context_init FAIL!");
        return OAL_FAIL;
    }
    hmac_vsp_set_current_handle(&g_vsp_sink.vsp_base);
    g_vsp_sink.vsp_base.enable = OAL_TRUE;
