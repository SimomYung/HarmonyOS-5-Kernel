...
typedef struct {
    ...
    uint32_t slice_rtt; /* 记录slice级别从tx pkt到发送完成的时间 */
    ...
    hmac_vsp_layer_stru layer_info[MAX_LAYER_NUM];
...
typedef struct {
    ...
    frw_timeout_stru vsp_watchdog_timer;
    uint32_t vsp_watchdog_cnt;
...
static inline uint8_t hmac_vsp_source_tx_layer_comp(
    hmac_vsp_tx_slice_stru *slice, uint8_t layer_id)
{
    return (slice->layer_info[layer_id].paket_number != 0) &&
        (slice->layer_info[layer_id].paket_number == slice->tx_result.layer_suc_num[layer_id]);
}
...
{
    return hmac_vsp_source_get_pkt_vsp_hdr(va) - hal_host_tx_get_msdu_dscr_len();
}
...
static inline uint8_t hmac_vsp_slice_queue_foreach_safe(
...
static inline void hmac_vsp_slice_delete_entry(hmac_vsp_slice_queue_stru *queue, hmac_vsp_tx_slice_stru *slice)
{
    oal_spin_lock(&queue->lock);
