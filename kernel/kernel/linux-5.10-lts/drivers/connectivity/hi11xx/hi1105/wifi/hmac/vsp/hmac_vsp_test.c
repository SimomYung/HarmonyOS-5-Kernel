...
static void wifi_tx_send_done_test(send_result *send_result);

static hmac_vsp_vcodec_vdec_ops test_vdec_ops = {
    .alloc_slice_mgmt = alloc_vdec_slice_buffer_test,
    .rx_slice_done = wifi_rx_slice_done_test,
};

static hmac_vsp_vcodec_venc_ops test_venc_ops = {
    .wifi_tx_pkg_done = wifi_tx_pkg_done_test,
    .wifi_tx_send_done = wifi_tx_send_done_test,
};
...
static tx_layer_ctrl *vsp_test_prepare_slice(void)
{
    ...
    if (++g_vsp_test_hdl.curr_slice >= g_vsp_test_hdl.slice_num) {
        g_vsp_test_hdl.curr_frame = (g_vsp_test_hdl.curr_frame + 1) % VSP_FRAME_ID_TOTAL;
...
uint32_t hmac_vsp_test_start_source(uint32_t frame_cnt)
{
    ...
    hmac_vsp_set_vcodec_vdec_ops(&test_vdec_ops);
    hmac_vsp_set_vcodec_venc_ops(&test_venc_ops);
