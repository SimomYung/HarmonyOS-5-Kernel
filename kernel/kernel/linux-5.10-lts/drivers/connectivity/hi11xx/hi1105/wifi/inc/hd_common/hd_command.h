...
typedef enum {
    ...
    OTHER_CTS_DEBUG = 26,
#endif
    SU_PUNCTURE_ENABLE = 27,
    SU_PUNCTURE_BITMAP = 28,
...
typedef struct {
    ...
    uint32_t base_addr_lsb;
    uint32_t base_addr_msb;
    uint32_t al_tx_netbuf_num;   /* 常发的个数 */
    uint32_t msdu_info_addr_lsb;
    uint32_t msdu_info_addr_msb;
    uint32_t msdu_addr_addr_lsb;
    uint32_t msdu_addr_addr_msb;
} mac_cfg_altx_msdu_info_stru; /* 扩大host_ring结构体兼容之前的结构体 */
typedef struct {
    uint8_t ru_size;               /* ru size */
    wlan_phy_protocol_enum_uint8 ru_allocation_b0;
    mac_rf_payload_enum_uint8 ru_index;
    wlan_tx_ack_policy_enum_uint8 ack_policy;
    uint32_t payload_len;
    uint32_t write_index;
    uint32_t base_addr_lsb;
    uint32_t base_addr_msb;
} mac_cfg_ru_idx_stru; /* hd_event */
typedef struct {
    uint8_t phase_incr_en;
    uint16_t phase_incr;
} mac_cfg_phase_incr_stru; /* hd_event */
typedef struct {
...
typedef struct {
    uint32_t cfg_switch; // 命令控制，0表示软件计算和上层下发的命令选择最优，1表示强制使用上层下发的参数
    uint32_t nav_duration;
    uint32_t edca_win; // 发送队列的ECWMAX ECWMIN
    uint32_t self_cts_switch; // vsp专用的self_cts开关, 0关 1开
} hmac_vsp_nav_edca_cmd;
typedef struct {
    uint8_t user_id;
    hmac_vsp_nav_edca_cmd vsp_nav_edca;
} vsp_nav_edca_stru;
#ifdef _PRE_WLAN_FEATURE_GET_STATION_INFO_EXT
typedef struct {
    uint16_t cfg_id;
    uint16_t cdf_th[NUM_7_BYTES];
} station_info_ext_cdf_th;
#endif
typedef struct {
    uint32_t rx_pps;
...
#define TTLM_TIDNO_BUTT                 8
#define TTLM_STATUS_MAX_SIZE (TTLM_TIDNO_BUTT * WLAN_MAX_ML_LINK_NUM)
typedef struct {
    uint8_t tid;
    uint8_t link_user_id;
    uint8_t enable;                        /* 表示link_user_id 的tid 调度是否是被TTLM 使能 */
    uint8_t resv;
} mac_ttlm_status_stru;
typedef struct {
    mac_ttlm_status_stru  ttlm_status[TTLM_STATUS_MAX_SIZE];
    uint32_t count;
} mac_ttlm_msg_stru;
#endif
