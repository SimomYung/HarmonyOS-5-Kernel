...
typedef struct {
    uint32_t ret_val;
    oal_bool_enum_uint8 cur_tas_status;
} dmac_tas_switch_status_notify_stru; /* hd_event */
#endif
...
typedef enum {
    PKT_FILTER_SET_FILTER_CMD,
    PKT_FILTER_GET_FILTER_CMD
} mac_pkt_filter_cmd_type_enum;
typedef uint8_t mac_pkt_filter_cmd_type_uint8;

typedef struct {
    void *p_program; /* netbuf链表一个元素 */
    uint16_t us_program_len;
} dmac_pkt_filter_report_event_stru; /* hd_event */
#endif
...
typedef struct {
    ...
    uint8_t ap_support_max_nss; /* 废弃 */
    uint8_t mld_mac_addr[WLAN_MAC_ADDR_LEN];
...
typedef enum {
    H2D_EMLSR_SHARE_RAM_TYPE = 0,
    H2D_EMLSR_TRX_TIME_RAM_TYPE = 1,
    H2D_EMLSR_TX_FREQ_RAM_TYPE = 2,
    H2D_EMLSR_RX_FREQ_RAM_TYPE = 3,

    H2D_EMLSR_DATA_TYPE_BUTT,
} h2d_emlsr_data_type_enum;

typedef struct {
    uint8_t mac_vap_id;
    h2d_emlsr_data_type_enum emlsr_ram_type;
    uint8_t netbuf_idx;
    uint8_t finish; // 表示这是最后一个netbuf，方便device关闭跨状态访问
    int32_t curr_send_data_len;
    int32_t data_total_len;
} h2d_emlsr_ram_trans_stru;
...
typedef struct {
    ...
    uint8_t reason;
    uint8_t sub_reason;
} ml_link_switch_info_stru;

typedef struct {
    ml_link_switch_info_stru switch_info;
    uint8_t allow_link_switch;
    uint8_t null_send_state;
    uint8_t resv[2];
} ml_link_switch_post_event_stru;

typedef struct {
    uint8_t enable;
    uint8_t resv[3];
    uint32_t duration;
} mlsr_switch_enable_stru;

typedef struct {
    uint8_t vap_id;             /* 接收omn发送完成中断/接收omn响应的vap */
    uint8_t emlsr_switch_event; /* emlsr事件类型：进入 or 退出 */
    uint8_t emlsr_pm;           /* emlsr是否等待omn响应 */
    uint8_t resv;
} emlsr_switch_event_stru;

typedef struct {
    uint8_t vap_id;             /* 退出后的工作link对应vap_id */
    uint8_t emlsr_close_reason; /* emlsr退出原因 */
    uint8_t resv[2];
} emlsr_close_event_stru;
