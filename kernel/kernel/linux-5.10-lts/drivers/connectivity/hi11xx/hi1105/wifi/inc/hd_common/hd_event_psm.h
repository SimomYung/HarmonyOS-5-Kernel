...
typedef enum {
    MAC_LOWPOWER_QUERY_WUR = 0,
    MAC_LOWPOWER_QUERY_LP_EXTLNA_POWERSAVING = 1,
    MAC_LOWPOWER_QUERY_ALL = 2,

    MAC_LOWPOWER_QUERY_TYPE_BUTT
} mac_lowpower_query_type_enum;
...
typedef enum {
    WLAN_WUR_DEEPSLEEP = 0x0, /* 深睡进入wur，浅睡不进wur */
    WLAN_WUR_LIGHTSLEEP = 0x1, /* 浅睡进入wur，不进深睡 */
    WLAN_WUR_MIXSLEEP = 0x2, /* 深睡浅睡结合，浅睡进wur，维持一段时间后，进入深睡 */
    WLAN_WUR_SLEEPMODE
} pm_wlan_wur_sleep_mode_enum;
...
typedef struct {
    ...
    uint8_t ls_listen_interval; /* 进入浅睡后维持多久进入深睡，深睡浅睡结合模式使用 */
} mac_cfg_wur_fastsleep_param_stru;

typedef struct {
    uint8_t wur_enable; /* 使能wur */
    uint8_t wur_sleep_mode;   /* （深睡0/浅睡1/深睡浅睡结合2） */
    uint8_t listen_interval; /* 模式为0或1时，表示进入深睡或者浅睡的listen时间，模式为2，表示由work进入浅睡的listen时间 */
    uint8_t ls_listen_interval; /* 模式为2，表示由浅睡进入深睡的listen时间 */
} mac_vendor_wur_param_stru;
typedef struct {
    uint32_t cmd_bitmap;
