...
typedef struct {
    ...
    uint8_t bit_is_ap_up_has_user : 1;               /* legacy AP有已关联用户标记，扫描超时时间延长 */
    uint8_t bit_rsv : 1;                          /* 保留位 */
...
typedef struct {
    ...
    wlan_scan_mode_enum_uint8 en_scan_mode; /* 扫描模式:前景扫描 or 背景扫描 */
    uint8_t mld_id;        /* ml单播扫描使用，标记mld_id 从扫描结果中获取host下发到dev */
    uint8_t uc_scan_func; /* DMAC SCANNER 扫描模式 */
    uint8_t uc_vap_id;    /* 下发扫描请求的vap id */
    uint64_t ull_cookie;  /* P2P 监听下发的cookie 值 */
...
typedef struct {
    uint8_t remain_on_chan_bitmap;
    uint8_t listen_vap_id; /* 监听vap id, 当前业务只使用p2p dev监听 */
    uint8_t listen_chan_number; /* 监听信道号, update_listen_cfg_flag置位时使用 */
    uint8_t is_mgmt_tx; /* 标记当前是否是mgmt_tx引起的同信道监听, 使用p2p_dev mgmt_tx时需做特殊处理 */
} mac_same_chan_remain_on_chan_h2d_info_stru; /* 同步remain_on_chan状态至device */
