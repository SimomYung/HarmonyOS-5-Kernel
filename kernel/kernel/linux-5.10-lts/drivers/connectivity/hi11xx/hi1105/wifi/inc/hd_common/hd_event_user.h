...
typedef struct {
    uint16_t user_idx;
    uint8_t  bpcc;
    uint8_t  rsv[1]; /* 1为数组大小 */
} mac_h2d_usr_ml_info_stru; /* hd_event */
#endif
...
typedef struct {
    ...
    uint8_t mld_lut_idx;
    uint8_t mld_mode;
    uint8_t user_mld_mac_addr[WLAN_MAC_ADDR_LEN];
#if defined(_PRE_WLAN_FEATURE_11AX) || defined(_PRE_WLAN_FEATURE_11AX_ROM)
...
typedef struct {
    ...
    uint16_t resv;
} mac_h2d_usr_mld_cap_stru; /* hd_event */
...
} h2d_tid_debug_info_stru; /* chba ring switch持续失败问题维测, 问题定位后删除 */
