...
typedef enum {
    ...
    MAC_VAP_STATE_STA_WAIT_MLD_SCAN = 12, /* 入网阶段ML单播扫描独有状态 */
    MAC_VAP_STATE_STA_LINK_DOWN = 13,
...
typedef struct {
    ...
    uint32_t tx_rate;     /* 当前发送速率 */
    uint32_t rx_rate;     /* 当前接收速率 */
} mac_user_report_info;
#define MAC_USER_REPORT_INFO_SIZE 12

typedef struct {
    uint8_t user_num;
    uint8_t rev[3];
    mac_user_report_info report_info[0];
} mac_all_user_report_info;
#define MAC_USER_REPORT_INFO_HEAD_SIZE 4
...
typedef struct {
    ...
    uint8_t bit_is_nbw_mode : 1;
    uint8_t bit_reserve1 : 6;
    ...
    mac_cfg_add_mld_vap_param_stru   mld_vap_param;
    oal_net_device_stru             *mld_net_device; /* wal层抛事件到hmac，MLD需要使用的netdev指针 */
#endif
    oal_net_device_stru *pst_net_dev; /* add vap时，wal层抛事件到hmac需要指定netdev */
...
typedef struct {
    ...
    oal_bool_enum_uint8 beacon_protection_enabled;
    uint8_t auc_resv1[1];
...
typedef enum {
    /* 0x1 窄带按位控制功能，AP模式不限制档位，STA模式限制在0挡或1挡 */
    NARROW_RATE_UNLIMIT_AP     = 0x1,
    /* 0x2 窄带按位控制功能，STA模式不限制档位，AP模式限制在0挡或1挡 */
    NARROW_RATE_UNLIMIT_STA    = 0x2,
    /* 0x3 窄带按位控制功能，AP模式不限制档位，STA模式不限制档位 */
    NARROW_RATE_UNLIMIT_AP_STA = 0x3,
} mac_narrow_unlimit_enum;
typedef struct {
    uint8_t freq_5g_num : 4;   /* 5G  开机自动跳频信道个数，最小0，最大6，为0表示关闭自动跳频 */
    uint8_t freq_2g_num : 4;   /* 2.4G开机自动跳频信道个数，最小0，最大6，为0表示关闭自动跳频 */
} mac_freq_select;
typedef struct {
    uint8_t autorate_en;
    uint8_t max_rate_level;
    uint8_t current_rate_level;
    mac_freq_select change_freq_num;
...
typedef struct {
    uint16_t chan; /* 信道 */
    uint16_t div_int;    /* 实部 */
    uint16_t div_frac;  /* 虚部 */
    uint8_t resv[2];
} mac_cfg_set_div_stru;
#endif
#endif
...
} mac_h2d_vap_ml_info_stru; /* hd_event */
#endif

typedef struct {
    uint8_t edca_ac;                  /* edca参数ac队列：BE,BK,VI,VO */
    uint8_t flag_aifsn;               /* edca参数AIFSN配置标记 0：不配置 1：配置 */
    uint8_t flag_cwmin;               /* edca参数CWmin配置标记 0：不配置 1：配置 */
    uint8_t flag_cwmax;               /* edca参数CWmax配置标记 0：不配置 1：配置 */
    uint8_t flag_txoplimit;           /* edca参数TXOP limit配置标记 0：不配置 1：配置 */
    uint8_t edca_aifsn;               /* edca参数AIFSN */
    uint8_t edca_cwmin;               /* edca参数CWmin */
    uint8_t edca_cwmax;               /* edca参数CWmax */
    uint16_t edca_txoplimit;          /* edca参数TXOP limit */
...
typedef enum {
    WUR_CFGVENDOR_EVENT           = 0, /* vendor命令触发进入退出wur */
    COFEATURE_MVAP_EVENT          = 1,
    COFEATURE_ROAM_EVENT          = 2,
    COFEATURE_BT_EVENT            = 3,
    COFEATURE_EMLSR_EVENT         = 4,
    COFEATURE_LINK_SWITCH_EVENT   = 5,
    COFEATURE_DEL_USER_EVENT      = 6, /* wur sta删用户退出wur */
    COFEATURE_HIGH_PPS_EVENT      = 7, /* 高流量wur使用快睡1.0睡眠定时器参数，低流量wur使用上层下发参数 */
    COFEATURE_PM_ENABLE_EVENT     = 8, /* 使能/关闭协议低功耗事件 */
    COFEATURE_AP_DISABLE_EVENT    = 9, /* 被ap 拒绝或者teardown */
    COFEATURE_FAILURE_DETECT_EVENT = 10, /* 失效检测事件 */

    COFEATURE_MLD_DEINIT_EVENT   = 17,
    COFEATURE_RECONFIG_EVENT     = 18,
    COFEATURE_TTLM_EVENT         = 19,
    EMLSR_CFGVENDOR_EVENT        = 20,
    COFEATURE_RECONNECT_EVENT    = 21,  /* MLD STA自愈重关联，退出EMLSR事件 */
    COFEATURE_NOTIFY_EVENT_BUTT
} cofeature_notify_event_enum; /* vap状态变化事件通知其他特性枚举类型 */

typedef enum {
    COFEATURE_EXIT        = 0, /* 事件退出、结束 */
    COFEATURE_ENTER       = 1, /* 事件进入、开始 */
    COFEATURE_ENTER_SUCC  = 2, /* 事件进入成功 */
    COFEATURE_ENTER_FAIL  = 3, /* 事件进入失败  */
    COFEATURE_NO_STATE    = 4, /* 无需状态 */

    COFEATURE_NOTIFY_EVENT_TYPE_BUTT
} cofeature_notify_event_subtype_enum; /* 事件状态 */
