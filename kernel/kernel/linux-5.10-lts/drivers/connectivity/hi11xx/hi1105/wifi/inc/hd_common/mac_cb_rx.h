...
typedef struct {
    ...
    /* byte 31-38 */
    uint64_t val_mac_hdr_start_addr;    /* 存放对应的帧的帧头地址值,虚拟地址 */
    /* byte 39-46 */
...
#if (defined(_PRE_PRODUCT_ID) && ((defined(_PRE_PRODUCT_ID_HI1103_DEV) && (_PRE_PRODUCT_ID_HI1103_DEV == \
_PRE_PRODUCT_ID)) || (defined(_PRE_PRODUCT_ID_HI1105_DEV) && (_PRE_PRODUCT_ID_HI1105_DEV == _PRE_PRODUCT_ID))))
...
#else
#define mac_get_rx_cb_mac_header_addr(_pst_rx_ctl)     ((_pst_rx_ctl)->val_mac_hdr_start_addr)
#define mac_get_rx_payload_addr(_rx_ctl, _pst_buf) \
    ((uint8_t *)(uintptr_t)(mac_get_rx_cb_mac_header_addr(_rx_ctl)) + mac_get_rx_cb_mac_header_len(_rx_ctl))
#endif
