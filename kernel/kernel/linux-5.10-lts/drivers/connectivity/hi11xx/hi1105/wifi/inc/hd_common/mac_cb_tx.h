...
typedef enum {
    ...
    WLAN_ACTION_ML_RECONFIG_REQUEST = 19,
    WLAN_ACTION_MLD_TTLM_REQUEST = 20,
    WLAN_ACTION_MLD_TTLM_RESPONSE = 21,
    WLAN_ACTION_MLD_TTLM_TEARDOWN = 22,
#endif
#ifdef _PRE_WLAN_FEATURE_WUR
    WLAN_ACTION_WUR_SETUP = 23,
    WLAN_ACTION_WUR_TEARDOWN = 24,
    WLAN_ACTION_WUR_BEACON = 25,
    WLAN_ACTION_WUR_WAKEUP = 26,
#endif
#ifdef _PRE_WLAN_FEATURE_EMLSR
    WLAN_ACTION_EMLSR_OMN = 27,
#endif
...
#pragma pack(push, 1)
typedef struct {
    uint64_t mac_frm_hdr_addr;                /* 该MPDU的帧头地址, DPA禁止使用 */
    uint64_t pcie_dma_addr;
    uint32_t pcie_dma_len;
    uint16_t us_seqnum;                       /* 记录软件分配的seqnum */
    uint16_t ether_type;                      /* 以太网类型 */

    uint16_t is_icmp : 1;                      /* 是否为icmp */
    uint16_t ipv6_tcp_ack : 1;                 /* 是否为IPV6 tcp_ack */
    uint16_t mac80211_head_type : 1;          /* 0: 802.11 mac头不在skb中,另外申请了内存存放;1:802.11 mac头在skb中 */
    uint16_t bit_more_frag : 1;
    uint16_t bit_frag_num : 4;
    uint16_t dma_mapped : 1;
    uint16_t rsv : 7;
...
#ifdef _PRE_WLAN_FEATURE_WUR
#define mac_get_cb_is_wur_setup_frame(_pst_tx_ctrl)                        \
    ((WLAN_CB_FRAME_TYPE_ACTION == mac_get_cb_frame_type(_pst_tx_ctrl)) && \
        (WLAN_ACTION_WUR_SETUP == mac_get_cb_frame_subtype(_pst_tx_ctrl)))
#define mac_get_cb_is_wur_teardown_frame(_pst_tx_ctrl)                        \
    ((WLAN_CB_FRAME_TYPE_ACTION == mac_get_cb_frame_type(_pst_tx_ctrl)) && \
        (WLAN_ACTION_WUR_TEARDOWN == mac_get_cb_frame_subtype(_pst_tx_ctrl)))
#define mac_get_cb_is_wur_beacon_frame(_pst_tx_ctrl)                        \
    ((WLAN_CB_FRAME_TYPE_MGMT == mac_get_cb_frame_type(_pst_tx_ctrl)) && \
        (WLAN_ACTION_WUR_BEACON == mac_get_cb_frame_subtype(_pst_tx_ctrl)))
#define mac_get_cb_is_wur_wakeup_frame(_pst_tx_ctrl)                        \
    ((WLAN_CB_FRAME_TYPE_MGMT == mac_get_cb_frame_type(_pst_tx_ctrl)) && \
        (WLAN_ACTION_WUR_WAKEUP == mac_get_cb_frame_subtype(_pst_tx_ctrl)))
#endif
...
#define mac_get_cb_seq_num(_pst_tx_ctrl)             \
    (((mac_ieee80211_frame_stru *)((uint8_t *)(_pst_tx_ctrl) + MAC_CB_FRM_OFFSET))->bit_seq_num)
...
#define mac_get_cb_frame_header_addr(_pst_tx_ctrl) \
    ((mac_ieee80211_frame_stru *)((uint8_t *)(_pst_tx_ctrl) + MAC_CB_FRM_OFFSET))
...
#define mac_get_cb_frame_header_addr(_pst_tx_ctrl)          ((_pst_tx_ctrl)->st_expand_cb.mac_frm_hdr_addr)
#define mac_set_cb_80211_mac_head_type(_pst_tx_ctrl, _flag) ((_pst_tx_ctrl)->st_expand_cb.mac80211_head_type = (_flag))
#define mac_get_cb_80211_mac_head_type(_pst_tx_ctrl)        ((_pst_tx_ctrl)->st_expand_cb.mac80211_head_type)
