...
#define MAC_CHBA_MAX_LINK_NUM 8
...
#define MAC_CHBA_NEGOTIATION_HIGH_PRIO_ACTION_MIN_LEN 12 /* chba negotiation高优先级action帧最短长度 */
...
typedef enum {
    ...
    MAC_NBW_TIMESLOT_NEG_ACTION = 11, /* nbw时隙申请相关的action帧，包括req/rsp/ind等 */
    MAC_NBW_TIMESLOT_ADJUST_ACTION = 12, /* nbw时隙调整相关的action帧 */
#endif
...
/* chba vap mode枚举 */
typedef enum {
    NBW_LEGACY_MODE = 0, /* 非nbw模式 */
    NBW_MODE = 1, /* NBW模式 */
    CHBA_VAP_NBW_MODE_BUTT,
} chba_vap_nbw_mode_enum;
#endif
/* chba vap的状态枚举 */
typedef enum {
    CHBA_INIT = 1,
...
typedef enum {
    ...
    HMAC_SAME_CHAN_REMAIN_ON_CHAN_ABORT      = 10, /* 同信道监听事件被打断 */
    HMAC_CHBA_UPDATE_SYNC_STATUS             = 11, /* chba同步状态更新 */
    HMAC_CHBA_DETECT_RADAR_REPORT            = 12, /* chba检测到雷达信号上报上层 */
    HMAC_AI_WIFI_DATA_REPORT       = 100, /* ai wifi数据上报通路 */
...
typedef struct {
    ...
    uint8_t schedule_period;                         /* CHBA调度周期，slot为单位 */
#ifdef _PRE_WLAN_FEATURE_NBW
    uint8_t nbw_uslot_duaration;                    /* 每个uslot的长度，当前设置为4ms */
    uint8_t nbw_uslot_cnt_per_slot;                 /* 每个slot里的uslot个数，nbw为4 or 8 */
#endif
...
typedef struct {
    uint8_t report_type;
    uint8_t resv[3];
} mac_detect_radar_report_stru;

typedef struct {
    uint8_t tx_status;
    uint8_t retry_num;
    uint16_t user_idx;
} mac_chba_tx_assoc_rsp_complete_stru;

typedef struct {
    uint8_t report_type; /* 内核用来区分消息类型 */
    uint8_t chba_sync_status; /* 取chba_master_election_enum枚举类型 */
    uint8_t resv[2];
} mac_chba_sync_status_report_stru; /* chba同步状态变化通知上层消息结构体 */
...
struct chba_device_capability {
    ...
    uint8_t resv[MAC_CHBA_DEVICE_CAPABILITY_RES_LEN];
} DECLARE_PACKED;
typedef struct chba_device_capability chba_dev_cap_stru; /* chba device_capability_ie */

struct chba_device_capability_old {
    uint8_t eid;
    uint8_t len;
    uint8_t max_channel_switch_time;
    uint8_t social_channel_support;
} DECLARE_PACKED;
typedef struct chba_device_capability_old chba_dev_cap_old_stru; /* chba device_capability_ie旧定义 */
