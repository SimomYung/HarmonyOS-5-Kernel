...
typedef struct {
    ...
    uint16_t read_index;
    uint16_t ml1_read_index;
    uint16_t write_index;
} msdu_info_ring_stru;

typedef struct {
    uint8_t ml_mode            : 1;
    uint8_t ml0_dup_en         : 1;
    uint8_t ml1_dup_en         : 1;
    uint8_t multi_link_bitmap  : 4;
    uint8_t reversed0          : 1;
    uint8_t multi_link_bitmap_switch  : 1;
    uint8_t multi_link_bitmap_next : 4;
    uint8_t reversed1          : 3;
    uint8_t reserved2[2];
} msdu_info_ring_ml_stru;
...
typedef struct {
    ...
    uint8_t  resv;
} mac_cfg_remove_ml_link_ap_param_stru;

typedef struct {
    uint8_t ap_link_id[WLAN_MAX_ML_LINK_NUM];  // 注意这里是对端AP的linkid
    uint8_t op_type[WLAN_MAX_ML_LINK_NUM];
    uint8_t link_op_num;    // 用于指示要操作的link数
    uint8_t diag_token;
    uint8_t resv[2];
} mac_cfg_tx_ml_reconfig_param_stru;
...
typedef struct {
    ...
    uint8_t enter_reason;
    uint8_t resv;
} mac_cfg_emlsr_action_param_stru;

typedef struct {
    uint8_t enable_link_id; /* 0 or 1 */
    uint8_t trigger_reason; /* mlsr_link_switch_reason_enum */
} mac_link_switch_param_stru;

/* EMLSR常发常收控制结构体，复用mac_cfg_emlsr_action_param_stru结构 */
typedef struct {
    uint8_t emlsr_al_mode;
    uint8_t emlsr_soft_status; // AL模式下支持直接控制EMLSR软件标记位
    uint8_t emlsr_status; // 期望的EMLSR状态
    uint8_t resv;
} emlsr_al_ctl;
#endif
typedef struct {
...
typedef struct {
    ...
    uint32_t              wp_mem_num;       /* 事件统计点数 */
    uint32_t              wp_event0_type_sel;  /* event0事件类型选择参数 */
    uint32_t              wp_event1_type_sel;  /* event1事件类型选择参数 */
    uint32_t              wp_event2_type_sel;  /* event3事件类型选择参数 */
    uint32_t              wp_event3_type_sel;  /* event4事件类型选择参数 */
