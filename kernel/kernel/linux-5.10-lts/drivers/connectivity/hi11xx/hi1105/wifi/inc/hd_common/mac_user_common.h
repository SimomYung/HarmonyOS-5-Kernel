...
typedef struct {
    ...
    uint8_t bigtk_key_index; /* beacon保护密钥idx */
} mac_key_mgmt_stru;
...
typedef struct {
    /* com_alg使用，保留en_前缀 */
    uint8_t en_eht_capable; /* 是否支持EHT协议模式 */
    uint8_t eht_oper_flag;  /* EHT Operation标记位 */
    /* com_alg使用，保留st_前缀 */
    mac_frame_eht_capabilities_ie_stru st_eht_cap_ie;  /* EHT Capabilities ie */
    mac_frame_eht_operation_ie_stru    eht_oper_ie;    /* EHT Operation ie */
} mac_eht_hdl_stru;
#endif
#ifdef _PRE_WLAN_FEATURE_ML
typedef struct {
    uint8_t medium_capable;
    uint8_t resv;
    mac_ml_medium_sync_delay_info_stru medium_sync;
} mac_medium_sync_info_stru;
#endif

#ifdef _PRE_WLAN_FEATURE_WUR
typedef struct {
    uint8_t wur_capable;    /* 是否支持wur */
    uint8_t wur_oper_flag;  /* wur Operation标记位 */
    mac_frame_wur_cap_ie_stru wur_cap_ie;   /* wur Capabilities ie */
    mac_frame_wur_oper_ie_stru wur_oper_ie; /* wur Operation ie */
} mac_wur_hdl_stru;

typedef struct {
    uint32_t wur_id;         /* ap记录分配给user的wur id */
    uint32_t group_id;       /* ap记录分配user的group id */
} mac_user_wur_info_stru;
#endif
