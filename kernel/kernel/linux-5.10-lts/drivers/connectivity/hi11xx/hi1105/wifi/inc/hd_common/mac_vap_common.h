...
typedef struct {
    uint8_t  negotiated; /* sta的setup交互完毕后，置位 */
    uint8_t  is_always_on; /* wur是否要配置为always on模式 */
    uint8_t  wur_channel; /* wur工作信道 */
    uint8_t  resv;
    uint16_t wur_id;      /* ap给sta分配的wur id，用来标识sta */
    uint16_t transmit_id; /* ap的transmit_id，为compressed_bssid的低12位 */
    uint16_t embedded_bssid; /* ap的embedded_bssid，为compressed_bssid的高16位，compressed BSSID=CRC-32(BSSID) */
    uint16_t start_group_id; /* ap分配给sta的start_group_id */
    uint64_t group_id_bitmap; /* (start_group_id + 本字段的bit N)为sta的group id */
    uint32_t on_duration;    /* wur的唤醒时长，单位：4us，同寄存器单位 */
    uint32_t duty_cycle_period; /* wur周期，单位：4us，同寄存器单位 */
    uint64_t duty_cycle_start_time; /* wur开始时间，单位：1us，同寄存器单位 */
    uint8_t  pm_timeout;  /* wur极致快睡，睡眠定时器超时时间 */
    uint8_t  psm_max_timer_restart_cnt; /* wur极致快睡，睡眠定时器最大超时次数 */
    uint8_t  sleep_mode; /* 0：仅深睡进wur,浅睡不进  1：浅睡进wur，关闭深睡  2:深睡浅睡结合 */
    uint8_t  ls_listen_interval; /* sleep_mode=2时，由浅睡进入深睡的listen时间 */
    uint8_t  rsv[NUM_4_BYTES];
} mac_cfg_wur_stru; /* 需要下发到dmac或是要保存在soc寄存器里的参数 */

typedef struct {
    uint8_t  min_wake_up_duration; /* ap支持的wur最小唤醒时长，单位：256us */
    uint8_t  host_pm_timeout; /* host睡眠定时器 */
    uint8_t  high_pps_enter_cnt;
    uint8_t  high_pps_exit_cnt;
    uint16_t duty_cycle_period_units; /* 单位：4us，ap的wur oper ie携带 */
    uint16_t duty_cycle_period_num; /* 本字段由sta决定，duty_cycle_period_units*本字段 = 实际的duty_cycle_period */
    uint16_t wur_beacon_period; /* wur beacon帧周期，单位：TU */
    uint16_t offset_of_twbtt; /* 第一帧wur beacon的发送时间与tsf 0的时间差，单位：TU  */
    uint8_t  is_high_pps; /* 高流量检测，wur使用快睡1.0参数 */
    uint8_t  is_enabled; /* 上层是否下发使能 */
    uint8_t  is_negotiated_before_roam;  /* 漫游前已经协商标记,用于漫游失败回原bss恢复wur */
    uint8_t  resv;
    mac_cfg_wur_stru cfg_info; /* 需要下发到dmac或是要保存在soc寄存器里的参数 */
} mac_wur_stru; /* vap下保存wur相关信息 */
#endif
...
typedef struct {
    uint16_t uplink_delay[NUM_4_BYTES][NUM_8_BYTES]; /* BE/BK/VI/VO上行时延 */
    uint16_t tx_time[NUM_4_BYTES][NUM_8_BYTES];      /* BE/BK/VI/VO上一帧空口发送时间 */
    uint32_t tx_mpdu_cnt;
    uint32_t tx_retry_mpdu_cnt;
    uint16_t max_ul_delay;
    uint16_t max_tx_time;
    uint8_t rx_mcs;
    uint8_t rx_bw;
    uint8_t rx_nss;
    uint8_t resv0;
    uint8_t edca_para[NUM_4_BYTES];
} station_info_extend_tx_stats_stru;
...
typedef struct {
    ...
    int8_t rpt_rssi[NUM_2_BYTES];       /* C0/C1 rssi上报 */
    station_info_extend_tx_stats_stru tx_stats;
