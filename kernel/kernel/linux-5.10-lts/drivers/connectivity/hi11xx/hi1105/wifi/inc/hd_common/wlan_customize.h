...
#define HAL_MAX_BAND_NUM    3  /* 2G/5G/6G */
...
typedef enum {
    ...
    WLAN_CFGID_SET_TAS_CUS_FCC_CE_POW = 7,      /* TAS功率fcc ce参数定制化 */
...
typedef struct {
    uint16_t dyn_cali_dscr_interval[HAL_MAX_BAND_NUM]; /* 动态校准开关2.4g 5g 6g */
...
typedef struct {
    uint8_t wlan_min_fast_ps_idle_p2p;
    uint8_t wlan_max_fast_ps_idle_p2p;
    uint8_t wlan_auto_ps_thresh_screen_on_p2p;
    uint8_t wlan_auto_ps_thresh_screen_off_p2p;
} cust_p2p_fast_pow_save_stru;

/* 校准所需要使用的定制化 */
typedef struct {
    uint32_t cali_mask[HAL_MAX_BAND_NUM]; /* offline校准开关, 2G/5G */
...
typedef struct {
    /* 5g C0 */
    int8_t gain_diff_db_c0;
    /* LP */
    int8_t gain_diff_db_lp;
    int8_t  rsv[2];
} mac_cust_rf_tas_loss_gain_db_stru;
#endif
...
typedef struct {
    int16_t mru_pwr_2g[WLAN_HE_MRU_SIZE_484_242];
} wlan_cust_2g_tpc_mru_pow_stru;
typedef struct {
    int16_t mru_pwr_5g[WLAN_HE_MRU_SIZE_BUTT];
} wlan_cust_5g_tpc_mru_pow_stru;
typedef struct {
    int16_t mru_pwr_6g[WLAN_HE_MRU_SIZE_BUTT];
} wlan_cust_6g_tpc_mru_pow_stru;
...
typedef struct {
    ...
    int8_t offline_pow_cali_amend[HD_EVENT_RF_NUM][HAL_MAX_BAND_NUM];
    uint32_t wifi_11be_switch_mask;
    uint32_t lp_cap_mask;
    int8_t rf_tas_rssi_amend[CUS_NUM_5G_AMEND_RSSI_NUM];
    int16_t tas_tpc_ru_pow_diff;
    uint8_t phy_lut_access;
    int8_t tas_max_txpwr_base_diff;
    int8_t rf_tas_loss_diff;
    int8_t rf_tas_lp_loss_diff;
    uint8_t support_wur_switch;
    uint8_t dpd_support_11b;
    cust_p2p_fast_pow_save_stru p2p_fast_ps;
    uint8_t fem_type[HAL_MAX_BAND_NUM];
    uint8_t fem_rsv; /* 四字节对齐 */
    uint8_t mlsr_switch_alg_en;
    int8_t mlsr_switch_rssi_low_th;
    int8_t mlsr_switch_rssi_high_th;
    uint8_t mlsr_switch_prefer_factor;
    uint32_t mlsr_switch_pps_low_th;
    uint32_t mlsr_switch_pps_high_th;
    uint32_t mlsr_switch_goodput_th;

    uint8_t emlsr_transation_delay;
    uint8_t emlsr_padding_delay;
    uint8_t triangle_slope;
    uint8_t tas_gpio_ctrl;

    uint32_t enhanced_phy_cap_mask;
    uint16_t sar_slide_pow_offset[HAL_MAX_BAND_NUM];
    uint8_t support_omi_bw_switch;
    uint8_t tas_abf_en;

    uint32_t coex_feature_bitmap;
...
#ifdef _PRE_WLAN_FEATURE_TAS_ANT_SWITCH
typedef struct {
    int8_t dpn_tas[WLAN_BANDWIDTH_BUTT][MAC_NUM_5G_BAND];
} cus_dy_cali_tas_dpn_stru;
#endif
...
typedef struct {
    ...
    int16_t fcc_txpwr_limit_5g_80m[HAL_NUM_5G_80M_SIDE_BAND];
    int16_t fcc_txpwr_limit_5g_160m[HAL_NUM_5G_160M_SIDE_BAND];
} hal_cfg_custom_tas_fcc_txpwr_limit_stru;

typedef struct {
    uint8_t sar_ctrl_params[HAL_CUS_NUM_OF_SAR_PARAMS];
} wlan_cust_tas_sar_cfg_stru;

typedef struct {
    uint16_t sar_total_time[HAL_MAX_BAND_NUM];
...
typedef struct {
    int16_t mru_pwr_2g[WLAN_HE_MRU_SIZE_484_242];
} wlan_cust_2g_mru_pow_stru;
typedef struct {
    int16_t mru_pwr_5g[WLAN_HE_MRU_SIZE_BUTT];
} wlan_cust_5g_mru_pow_stru;
typedef struct {
    int16_t mru_pwr_6g[WLAN_HE_MRU_SIZE_BUTT];
} wlan_cust_6g_mru_pow_stru;
