...
#ifndef WLAN_CUSTOMIZE_HOST_H
#define WLAN_CUSTOMIZE_HOST_H
...
#endif
#if defined (_PRE_WLAN_FEATURE_FTM_CALI)
    WLAN_RF_CALI_FTM             = 24,
#elif defined (_PRE_WLAN_FEATURE_WUR)
    WLAN_RF_CALI_WUR_RX_DC       = 24,
#endif
...
typedef enum {
    ...
    WLAN_D2H_CALI_DATA_TYPE_BUTT = 43,
    WLAN_CALI_DATA_DBDC_C0 = WLAN_D2H_CALI_DATA_TYPE_BUTT,
    WLAN_CALI_DATA_DBDC_C1 = 44,
    WLAN_CALI_DATA_DBDC_C2 = 45,
    WLAN_CALI_DATA_DBDC_C3 = 46,
    WLAN_CALI_DATA_COMMON_EXT = 47,
    WLAN_D2H_CALI_DATA_TYPE_EXT_BUTT,
...
typedef enum {
    ...
    WLAN_ONLINE_LNADET_CALI = 6,
    WLAN_ONLINE_TXDC_CALI   = 7,
...
typedef enum {
    ...
    WLAN_CALI_CTL_EXT = WLAN_H2D_CALI_DATA_TYPE_EXTEND_BUTT,    /* 通用数据结构扩展部分 */
    WLAN_H2D_CALI_DATA_TYPE_EXT_BUTT,
...
/* SAW检测状态 */
#define WLAN_SAW_STATUS_OFFSET 12
#define WLAN_SAW_STATUS_MASK 0xF000
/* 支持切换TAS的天线索引 */
#define WLAN_RF_TAS_ANT_IDX 0

/* 07c LUT裁剪到40，共代码校准数据存储仍用48 */
#define MP1X_WLAN_DPD_EVEN_LUT_DATA_LENGTH      48
#define MP1X_WLAN_DPD_ODD_LUT_DATA_LENGTH       48
#define MP1X_WLAN_DPD_GLUE_LUT_DATA_LENGTH      45
#define WLAN_DPD_TPC_LEVEL_NUM        4
#define WLAN_DPD_RX_DC_FB_LVL         4
#define WLAN_DPD_ENG_MU_NUM 2 /* POST、PRE mu值个数 */

#define MP12_WLAN_DPD_EVEN_LUT_DATA_LENGTH      24
#define MP12_WLAN_DPD_ODD_LUT_DATA_LENGTH       24
#define MP12_WLAN_DPD_GLUE_LUT_DATA_LENGTH      21

typedef enum {
    WLAN_HW_STATUS_CHECK_FEM_PA = 0,
    WLAN_HW_STATUS_CHECK_LNA = 1,
    WLAN_HW_STATUS_CHECK_ELNA = 2,
    WLAN_HW_STATUS_CHECK_SAW = 3,
...
typedef struct {
    ...
    mac_custom_ext_rf_stru ext_rf[HAL_MAX_BAND_NUM];   /* 2.4g 5g fem */
...
typedef struct {
    ...
    cus_dy_cali_tas_dpn_stru dpn_tas_para[HAL_CUS_FB_LVL_NUM];
#endif
}mac_cust_dyn_pow_sru;

typedef struct {
    uint16_t dpd_eng_mu_post_2g[WLAN_DPD_TPC_LEVEL_NUM]; /* 2g post mu */
    uint16_t dpd_eng_mu_pre_2g[WLAN_DPD_TPC_LEVEL_NUM]; /* 2g pre mu */
    uint16_t dpd_eng_mu_post_5g[WLAN_DPD_TPC_LEVEL_NUM]; /* 5g post mu */
    uint16_t dpd_eng_mu_pre_5g[WLAN_DPD_TPC_LEVEL_NUM]; /* 5g pre mu */
    uint32_t dpd_cal_eng_mu_post_2g[WLAN_DPD_TPC_LEVEL_NUM]; /* 2g cal post mu */
    uint32_t dpd_cal_eng_mu_pre_2g[WLAN_DPD_TPC_LEVEL_NUM]; /* 2g cal pre mu */
    uint32_t dpd_cal_eng_mu_post_5g[WLAN_DPD_TPC_LEVEL_NUM]; /* 5g cal pre mu */
    uint32_t dpd_cal_eng_mu_pre_5g[WLAN_DPD_TPC_LEVEL_NUM]; /* 5g cal pre mu */
    uint8_t dpd_tpc_cali_code_2g[WLAN_DPD_TPC_LEVEL_NUM];   /* 2g 各个档位的tpc code, 最大4档 */
    uint8_t dpd_tpc_min_2g[WLAN_DPD_TPC_LEVEL_NUM];         /* 2g 各个档位下限值，用于档位分界 */
    uint8_t dpd_tpc_cali_code_5g[WLAN_DPD_TPC_LEVEL_NUM];   /* 5g 各个档位的tpc code */
    uint8_t dpd_tpc_min_5g[WLAN_DPD_TPC_LEVEL_NUM];         /* 5g 各个档位下限值，用于档位分界 */

    uint8_t dpd_core_alpha_2g[WLAN_DPD_TPC_LEVEL_NUM];      /* 2g 各个档位的alpha 值 */
    uint16_t dpd_core_scale_2g[WLAN_DPD_TPC_LEVEL_NUM];     /* 2g 各个档位的scale 值 */
    uint16_t dpd_vtc_scale_2g[WLAN_DPD_TPC_LEVEL_NUM];      /* 2g 各个档位的vtc_scale 值 */
    uint8_t dpd_core_alpha_5g[WLAN_DPD_TPC_LEVEL_NUM];
    uint16_t dpd_core_scale_5g[WLAN_DPD_TPC_LEVEL_NUM];
    uint16_t dpd_vtc_scale_5g[WLAN_DPD_TPC_LEVEL_NUM];
    uint8_t dpd_core_alpha_6g[WLAN_DPD_TPC_LEVEL_NUM];
    uint16_t dpd_core_scale_6g[WLAN_DPD_TPC_LEVEL_NUM];
    uint16_t dpd_vtc_scale_6g[WLAN_DPD_TPC_LEVEL_NUM];
} wlan_cust_dpd_param_stru;
...
typedef struct {
    ...
    int8_t delt_txpwr_base_params_2[HAL_MAX_BAND_NUM][HD_EVENT_RF_NUM]; /* 2*2MIMO相对SISO的base power差值 */
    int8_t delt_txpwr_base_params_3[HAL_MAX_BAND_NUM][HD_EVENT_RF_NUM]; /* 3*3MIMO相对SISO的base power差值 */
    int8_t delt_txpwr_base_params_4[HAL_MAX_BAND_NUM][HD_EVENT_RF_NUM]; /* 4*4MIMO相对SISO的base power差值 */
    /* FCC/CE边带功率定制项 */
...
#ifdef _PRE_WLAN_FEATURE_TAS_ANT_SWITCH
    /* TAS CTRL */
    uint8_t tas_ctrl_params[HAL_MAX_BAND_NUM][HD_EVENT_RF_NUM];
#endif
    ...
    uint8_t tpc_tb_ru_max_power[HAL_MAX_BAND_NUM];
    uint8_t rsv[1];    /* 保留字段:1字节 */

    uint32_t fcc_ce_max_pwr_for_5g_high_band;

    wlan_cust_2g_tpc_mru_pow_stru tpc_tb_mru_2g_max_pow[HAL_POW_RF_SEL_CHAIN_BUTT];
    wlan_cust_5g_tpc_mru_pow_stru tpc_tb_mru_5g_max_pow[HAL_POW_RF_SEL_CHAIN_BUTT];
    wlan_cust_6g_tpc_mru_pow_stru tpc_tb_mru_6g_max_pow[HAL_POW_RF_SEL_CHAIN_BUTT];

    wlan_cust_2g_tpc_mru_pow_stru tpc_tb_mru_2g_2x2_max_pow[HAL_POW_RF_SEL_CHAIN_BUTT];
    wlan_cust_5g_tpc_mru_pow_stru tpc_tb_mru_5g_2x2_max_pow[HAL_POW_RF_SEL_CHAIN_BUTT];
    wlan_cust_6g_tpc_mru_pow_stru tpc_tb_mru_6g_2x2_max_pow[HAL_POW_RF_SEL_CHAIN_BUTT];

    wlan_cust_2g_mru_pow_stru  full_bw_mru_pow_2g[WLAN_BW_CAP_80M];
    wlan_cust_5g_mru_pow_stru  full_bw_mru_pow_5g[WLAN_BW_CAP_BUTT];
    wlan_cust_6g_mru_pow_stru  full_bw_mru_pow_6g[WLAN_BW_CAP_BUTT];

    wlan_cust_dpd_param_stru cust_dpd_param;
    /* TAS FCC/CE边带功率定制项 */
    hal_cfg_custom_tas_fcc_txpwr_limit_stru tas_fcc_ce_param;
    /* TAS SAR CTRL */
    wlan_cust_tas_sar_cfg_stru tas_sar_ctrl_params[CUS_SAR_LVL_NUM];
    uint8_t dac_spur_opt[HAL_MAX_BAND_NUM];  /* dac时钟采样带外spur优化开关 */
    uint8_t rsv1; /* 4字节对齐预留 */
    uint16_t saw_det_amp[HAL_MAX_BAND_NUM];  /* saw器件失效检测幅值门限 */
    uint16_t rsv2; /* 4字节对齐预留 */
}wlan_cust_pow_stru;
...
typedef struct {
    ...
    uint8_t offline_cali_status : 1, /* 标识离线校准的状态 */
            resv : 7;
...
typedef struct {
    uint8_t band;
    oal_bool_enum_uint8 is_exclusive_mode; /* true表示独占模式, 只能通过扫描remain_on_chan的方式发起 */
    uint16_t center_freq;
    uint32_t duration; /* remain_on_channel时长 */
} wlan_remain_on_channel_info_stru;

typedef struct {
    uint8_t ps_service_type; /* 标记要求调整低功耗的业务 */
    uint8_t resv;
    uint8_t user_mac_addr[OAL_MAC_ADDR_LEN]; /* 该ioctl命令对应user的mac地址 */
} wlan_chba_service_ask_awake_stru;
