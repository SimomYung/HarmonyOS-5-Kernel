...
typedef enum {
    WLAN_LEGACY_MLD_MODE = 0, /* 非MLD设备 */
    WLAN_ML_MODE    = 1, /* MLD模式 */
    WLAN_LINK_MODE   = 2, /* LINK模式 */
    WLAN_ML_LINK_MODE = 3, /* MLD+LINK模式 */

    WLAN_MLD_MODE_BUTT
} wlan_mld_mode_enum;

/* MAC_LINK硬件资源枚举 针对硬件MAC资源，不与频段绑定 */
typedef enum {
    MAC_TRX_LINK_ID_0 = 0,
    MAC_TRX_LINK_ID_1 = 1,
    MAC_TRX_LINK_ID_2 = 2,

    MAC_TRX_LINK_ID_BUTT
} mac_ml_trx_idx_enum;
...
/* 带宽能力枚举 */
/****************************************************************************************************
   host向device下发功率定制化数据时，会使用WLAN_BW_CAP_BUTT作为数组大小
   bisheng device侧编译时也使用WLAN_BW_CAP_BUTT
   WLAN_BW_CAP_320M加入wlan_bw_cap_enum会改变WLAN_BW_CAP_BUTT的值，导致bisheng的功率定制化数据下发失败
   考虑 1）07c硬件不支持320M带宽 2）WLAN_BW_CAP_320M只用于判断或user能力赋值，暂定义为宏
****************************************************************************************************/
#define WLAN_BW_CAP_320M 5
typedef enum {
    WLAN_BW_CAP_20M,
    WLAN_BW_CAP_40M,
    WLAN_BW_CAP_80M,
...
/* 链路类型(DL/UL) */
typedef enum {
    WLAN_DOWNLINK = 0, /* 下行链路 */
    WLAN_UPLINK = 1,   /* 上行链路 */
    WLAN_DOWNUPLINK = 2, /* 上下行双向链路 */
    WLAN_LINK_BUTT,
} wlan_link_type_enum;
...
typedef enum {
    WLAN_AP_WIDEBW_BANDWIDTH_20M = 0,
    WLAN_AP_WIDEBW_BANDWIDTH_40M = 1,
    WLAN_AP_WIDEBW_BANDWIDTH_80M = 2,
    WLAN_AP_WIDEBW_BANDWIDTH_160M = 3,
    WLAN_AP_WIDEBW_BANDWIDTH_BUTT,
} wlan_vendor_ap_widerbw_bandwidth_enum;
...
typedef enum {
    ...
    WLAN_SCAN_MODE_MLD_SCAN = 8, /* MLD单播probe req扫描 */
...
/* 3.9 roc场景枚举 */
typedef enum {
    IEEE80211_ROC_TYPE_NORMAL = 0, /* cfg80211监听模式, 不发送 */
    IEEE80211_ROC_TYPE_MGMT_TX = 1, /* 非chba常见管理帧发送 */
    IEEE80211_ROC_TYPE_CHBA_LOW_PRIORITY_MGMT_TX = 2, /* chba低优先级管理帧发送 */
    IEEE80211_ROC_TYPE_CHBA_HIGH_PRIORITY_MGMT_TX = 3, /* chba高优先级管理帧发送 */
    IEEE80211_ROC_TYPE_CHBA_MGMT_RX = 4, /* chba管理帧接收监听 */
