...
typedef enum {
    ...
    HMAC_HOST_CTX_EVENT_SUB_TYPE_COFEATURE_STATE_NOTIFY,
...
typedef struct {
    ...
    oal_net_device_stru *past_netdev[WLAN_VAP_MAX_NUM_LIMIT];
    uint32_t netdev_num;
...
typedef enum {
    ...
    COFEATURE_TYPE_BUTT
} hmac_cofeature_type_enum;

typedef enum {
    MLO_WORK_STATE_DEFAULT      = 0, /* 非wifi7 */
    MLO_WORK_STATE_SINGLE_LINK  = 1, /* mld单link工作模式 */
    MLO_WORK_STATE_MLSR         = 2, /* mlsr双link工作模式 */
    MLO_WORK_STATE_EMLSR        = 3, /* emlsr工作模式 */
    MLO_WORK_STATE_STR          = 4, /* str工作模式 */
    MLO_WORK_STATE_LEGACY_WIFI7 = 5, /* 11be非mld工作模式 */
    MLO_WORK_STATE_BUTT
} hmac_mlo_state_enum;
...
typedef struct {
    uint8_t feature;      /* 0:MLO 1:WUR */
    uint8_t state;        /* hmac_mlo_state_enum */
    uint16_t reason_code; /* emlsr_close_reason_enum */
} hmac_cofeature_rpt_stru;
...
void hmac_mgmt_send_deauth_frame(uint8_t vap_idx, const unsigned char *da_addr, uint16_t err_code,
...
uint32_t hmac_config_set_pkt_filter(mac_vap_stru *pst_mac_vap, uint16_t us_len, uint8_t *puc_param);
#endif
