...
void host_chip2_rf_cali_init(void)
{
    /* 执行chip2的校准前，先判断chip2已在总线上 */
    if (oal_bus_get_chip_num() < WLAN_CHIP_MAX_NUM_PER_BOARD) {
        return;
    }

    g_custom_cali_done_mpxx_dev1 = OAL_FALSE;
    oal_init_completion(hmac_get_wlan_cali_completed_addr());
    wlan_pm_open_dev(HCC_EP_WIFI_DEV1, OAL_TRUE);
    ...
    if (oal_wait_for_completion_timeout(hmac_get_wlan_cali_completed_addr(),
        ...
        oam_error_log1(0, OAM_SF_ANY, "{host_chip2_rf_cali_init::wait [%d]ms g_wlan_cali_completed timeout)!}",
            MAIN_WAL_WAIT_CALI_COMPLETE_TIME);
    }
    wlan_pm_close_dev(HCC_EP_WIFI_DEV1);
}
#endif
...
void host_rf_cali_init(void)
{
    ...
    if (cali_first_pwr_on == OAL_TRUE) {
        ...
        wal_hipriv_init_mac_addr_from_efuse();
#endif

        wlan_pm_close_dev(HCC_EP_WIFI_DEV);

#ifdef _PRE_DUAL_CHIP
        host_chip2_rf_cali_init();
#endif
    }
...
/*
 * 函 数 名  : hcc_flowctl_func_register
 * 功能描述  : 注册hcc flowctl相关操作函数
 */
static void hcc_flowctl_func_register(void)
{
    uint32_t bus_dev = 0;
    ...
    for (bus_dev = 0; bus_dev < max_dev_num; bus_dev++) {
        if (bus_dev >= HCC_CHIP_MAX_DEV) {
            oal_io_print("hmac_flowctl_fun_register failed!:bus_dev[%d] is exceed\n", bus_dev);
            return;
        }
        hcc_flowctl_get_device_mode_register(hmac_flowctl_check_device_is_sta_mode, bus_dev);
        hcc_flowctl_operate_subq_register(hmac_vap_net_start_subqueue, hmac_vap_net_stop_subqueue, bus_dev);
    }
}
...
int32_t wifi_sysfs_init(void)
{
    ...
    if ((is_hisi_chiptype(BOARD_VERSION_MP13) == false) &&
        ...
        (is_hisi_chiptype(BOARD_VERSION_MP17C) == false) &&
        (is_hisi_chiptype(BOARD_VERSION_MP12) == false)) {
        return OAL_SUCC;
    }
