                                                  ...
                                                  uint8_t en_frame_direction)
{
    ...
    uint32_t ul_ret;
    uint16_t mac_frame_len = mac_frame_len_input;
    if (oal_unlikely(g_oam_sdt_func_hook.p_sdt_report_data_func == NULL)) {
        return OAL_ERR_CODE_PTR_NULL;
    }

    /* 为上报80211帧申请空间 */
    us_skb_len = mac_frame_len + sizeof(oam_ota_hdr_stru);
    if (us_skb_len > SDT_NETBUF_MAX_PAYLOAD) {
        us_skb_len = SDT_NETBUF_MAX_PAYLOAD;
        mac_frame_len = (uint16_t)(SDT_NETBUF_MAX_PAYLOAD - sizeof(oam_ota_hdr_stru));
    }
    ...
    pst_ota_data->st_ota_hdr.us_ota_data_len = mac_frame_len;
    ...
    ul_ret = (uint32_t)memcpy_s((void *)pst_ota_data->auc_ota_data,
        (uint32_t)mac_frame_len, (const void *)puc_mac_hdr_addr, (uint32_t)uc_mac_hdr_len);
    /* 复制帧体 */
    ul_ret += (uint32_t)memcpy_s((void *)(pst_ota_data->auc_ota_data + uc_mac_hdr_len),
        (uint32_t)(mac_frame_len - uc_mac_hdr_len), (const void *)puc_mac_body_addr,
        (uint32_t)(mac_frame_len - uc_mac_hdr_len));
    if (ul_ret != EOK) {
                                ...
                                uint8_t en_frame_direction)
{
    ...
    if ((uc_mac_hdr_len > WLAN_MAX_FRAME_HEADER_LEN) || (uc_mac_hdr_len < WLAN_MIN_FRAME_HEADER_LEN)) {
        oam_warning_log4(0, OAM_SF_ANY, "{oam_report_80211_frame:HEAD CHECK! HDR_LEN_INVALID!!hearder_len = %d, \
            ...
            uc_mac_hdr_len, mac_frame_len, en_frame_direction, ul_return_addr);
    }

    /* 检查mac帧总长度合法性 */
    if (uc_mac_hdr_len > mac_frame_len) {
    ...
    mac_frame_len = (mac_frame_len > WLAN_MAX_FRAME_LEN) ? WLAN_MAX_FRAME_LEN : mac_frame_len;
    ...
    switch (g_oam_mng_ctx[OAM_CTX_WIFI].en_output_type) {
        ...
        case OAM_OUTPUT_TYPE_CONSOLE:
            ul_ret = oam_report_80211_frame_to_console(puc_mac_hdr_addr, uc_mac_hdr_len,
                puc_mac_body_addr, mac_frame_len, en_frame_direction);
            break;
        /* 输出至SDT工具 */
        case OAM_OUTPUT_TYPE_SDT:
            ul_oam_ret = oam_report_80211_frame_to_sdt(puc_user_macaddr, puc_mac_hdr_addr,
                uc_mac_hdr_len, puc_mac_body_addr, mac_frame_len, en_frame_direction);
            break;
        default:
                                             ...
                                             uint8_t en_beacon_direction)
{
    ...
    oam_ota_stru *pst_ota_data = NULL;
    int32_t ret;
    uint16_t beacon_len = beacon_len_input;
    if (oal_unlikely(g_oam_sdt_func_hook.p_sdt_report_data_func == NULL)) {
        return OAL_ERR_CODE_PTR_NULL;
    }

    /* 为上报beacon帧申请空间 */
    us_skb_len = beacon_len + sizeof(oam_ota_hdr_stru);
    if (us_skb_len > SDT_NETBUF_MAX_PAYLOAD) {
        us_skb_len = SDT_NETBUF_MAX_PAYLOAD;
        beacon_len = SDT_NETBUF_MAX_PAYLOAD - (uint16_t)sizeof(oam_ota_hdr_stru);
    }
    ...
    pst_ota_data->st_ota_hdr.us_ota_data_len = beacon_len;
    pst_ota_data->st_ota_hdr.en_frame_direction = en_beacon_direction;
    oam_set_ota_board_type(pst_ota_data);

    /* 复制数据,填写ota数据 */
    ret = memcpy_s((void *)pst_ota_data->auc_ota_data, (uint32_t)beacon_len,
        (const void *)puc_beacon_hdr_addr, (uint32_t)uc_beacon_hdr_len);

    ret += memcpy_s((void *)(pst_ota_data->auc_ota_data + uc_beacon_hdr_len),
        (uint32_t)(beacon_len - uc_beacon_hdr_len), (const void *)puc_beacon_body_addr,
        (uint32_t)(beacon_len - uc_beacon_hdr_len));
    if (ret != EOK) {
        oal_mem_sdt_netbuf_free(pst_netbuf, OAL_TRUE);
