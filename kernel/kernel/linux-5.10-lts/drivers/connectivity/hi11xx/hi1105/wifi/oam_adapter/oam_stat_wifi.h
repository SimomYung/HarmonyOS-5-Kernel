...
#define oam_stat_vap_incr(_uc_vap_id, _member, _num)                                 \
    do {                                                                             \
        if ((_uc_vap_id) < WLAN_VAP_MAX_NUM_LIMIT) {                           \
...
typedef struct {
    ...
    uint32_t aul_tbtt_cnt[WLAN_VAP_MAX_NUM_LIMIT]; /* tbtt中断,0~3代表ap的，4代表sta的 */
...
typedef struct {
    /* 设备级别的统计信息 */
    oam_device_stat_info_stru ast_dev_stat_info[WLAN_DEVICE_MAX_NUM_PER_CHIP];

    /* VAP级别的统计信息 */
    oam_vap_stat_info_stru ast_vap_stat_info[WLAN_VAP_MAX_NUM_LIMIT];
