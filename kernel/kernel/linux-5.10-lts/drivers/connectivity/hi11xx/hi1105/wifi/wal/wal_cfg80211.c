...
/* 内核变量g_nl80211_fam，g_nl80211_mlme_mcgrp */
...
void oal_kobject_uevent_env_sta_leave(oal_net_device_stru *net_device, const unsigned char *mac_addr)
{
#ifdef _PRE_WINDOWS_SUPPORT
    return;
#else
    ...
    add_uevent_var(&g_env, "SOFTAP=STA_LEAVE wlan0 wlan0 %02x:%02x:%02x:%02x:%02x:%02x",
                   mac_addr[BYTE_OFFSET_0], mac_addr[BYTE_OFFSET_1], mac_addr[BYTE_OFFSET_2],
                   mac_addr[BYTE_OFFSET_3], mac_addr[BYTE_OFFSET_4], mac_addr[BYTE_OFFSET_5]);
#if ( KERNEL_VERSION(4, 1, 0) <= LINUX_VERSION_CODE)
...
void oal_inform_assoc_link_id(oal_net_device_stru *net_device, uint8_t event_type)
{
    ...
    if (hmac_vap == NULL || hmac_vap_is_legacy_mld_mode(hmac_vap) == OAL_TRUE) {
        return;
    }
    assoc_link_id = hmac_vap->mld_vap_link_info.link_ap_info.link_id;
    if (event_type == LINK_SWITCH_EVENT) {
        cfg80211_inform_link_switch(net_device, assoc_link_id);
    } else {
        cfg80211_inform_mlo_assoc_link_id(net_device, assoc_link_id);
    }
}
#endif
#if (KERNEL_VERSION(4, 12, 0) <= LINUX_VERSION_CODE)
static void common_info_prepare_connect_resp_param(struct cfg80211_connect_resp_params *params,
...
static uint32_t prepare_connect_resp_params(oal_net_device_stru *net_device, const uint8_t *bssid,
    const uint8_t *req_ie, uint32_t req_ie_len, const uint8_t *resp_ie, uint32_t resp_ie_len,
    uint16_t status, struct cfg80211_connect_resp_params *params)
{
#ifdef CONFIG_HISI_WIFI7_MLO
    ...
    hmac_vap_stru *hmac_vap = NULL;
    uint8_t sta_link_id, ap_link_id;
    if (mac_vap == NULL) {
    ...
    common_info_prepare_connect_resp_param(params, req_ie, req_ie_len, resp_ie, resp_ie_len, status);
    if (hmac_vap->mld_vap_link_info.mld_vap_manage == NULL ||
        ...
        params->links[0].bss = NULL;
        params->links[0].status = status;
    } else {
        params->ap_mld_addr = hmac_vap->mld_vap_link_info.mld_vap_manage->mld_bssid;
        for (sta_link_id = 0; sta_link_id < WLAN_MAX_ML_LINK_NUM; sta_link_id++) {
            link_hmac_vap = (hmac_vap_stru *)hmac_vap->mld_vap_link_info.mld_vap_manage->link_vap[sta_link_id];
            if (link_hmac_vap == NULL) {
                continue;
            }
            params->valid_links |= oal_set_bit(link_hmac_vap->mld_vap_link_info.link_ap_info.link_id);
            ap_link_id = link_hmac_vap->mld_vap_link_info.link_ap_info.link_id;
            if (ap_link_id > WLAN_MAX_AP_LINK_ID) {
                continue;
            }
            params->links[ap_link_id].addr = (const uint8_t*)mac_mib_get_StationID(&link_hmac_vap->st_vap_base_info);
            params->links[ap_link_id].bssid = link_hmac_vap->st_vap_base_info.auc_bssid;
            params->links[ap_link_id].status = 0;
        }
    }
#else
    common_info_prepare_connect_resp_param(params, req_ie, req_ie_len, resp_ie, resp_ie_len, status);
...
{
#if (KERNEL_VERSION(4, 12, 0) <= LINUX_VERSION_CODE)
    struct cfg80211_connect_resp_params params = {0};
    uint32_t ret;
    ret = prepare_connect_resp_params(pst_net_device, puc_bssid, puc_req_ie, ul_req_ie_len, puc_resp_ie,
        ul_resp_ie_len, us_status, &params);
    if (ret != OAL_SUCC) {
        return OAL_ERR_CODE_PTR_NULL;
    }
    cfg80211_connect_done(pst_net_device, &params, en_gfp);
    return OAL_SUCC;
...
static struct ieee80211_channel *get_roam_channel_info(mac_vap_stru *vap_base_info)
{
    struct ieee80211_channel *channel = NULL;
    mac_device_stru *mac_device = NULL;
    uint8_t band;
    int32_t l_freq;
    mac_device = mac_res_get_dev(vap_base_info->uc_device_id);
    if (mac_device == NULL) {
        oam_error_log1(0, OAM_SF_ROAM, "{get_roam_channel_info::pst_mac_device is null: %d!}",
            vap_base_info->uc_device_id);
        return NULL;
    }

    band = hmac_get_80211_band_type(&vap_base_info->st_channel);
    l_freq = oal_ieee80211_channel_to_frequency(vap_base_info->st_channel.uc_chan_number, band);
    channel = (struct ieee80211_channel *)oal_ieee80211_get_channel(mac_device->pst_wiphy, l_freq);
    return channel;
}
#endif

static void common_info_prepare_roam_resp_param(struct cfg80211_roam_info *params,
                                                const uint8_t *req_ie,
                                                uint32_t req_ie_len,
                                                const uint8_t *resp_ie,
                                                uint32_t resp_ie_len)
...
static uint32_t prepare_roam_resp_params(oal_net_device_stru *net_device, struct ieee80211_channel *channel,
    const uint8_t *bssid, const uint8_t *req_ie, uint32_t req_ie_len, const uint8_t *resp_ie,
    uint32_t resp_ie_len, struct cfg80211_roam_info *params)
{
#ifdef CONFIG_HISI_WIFI7_MLO
#ifdef _PRE_WLAN_FEATURE_ML
    mac_vap_stru *mac_vap = oal_net_dev_priv(net_device);
    ...
    if (hmac_vap->mld_vap_link_info.mld_vap_manage == NULL ||
        hmac_vap->mld_vap_link_info.mld_vap_manage->in_use != OAL_TRUE) {
        params->links[0].bssid = bssid;
        params->links[0].channel = channel;
    } else {
        params->ap_mld_addr = hmac_vap->mld_vap_link_info.mld_vap_manage->mld_bssid;
        for (sta_link_id = 0; sta_link_id < WLAN_MAX_ML_LINK_NUM; sta_link_id++) {
            link_hmac_vap = (hmac_vap_stru *)hmac_vap->mld_vap_link_info.mld_vap_manage->link_vap[sta_link_id];
            ...
            params->links[ap_link_id].addr = (const uint8_t*)mac_mib_get_StationID(&link_hmac_vap->st_vap_base_info);
            params->links[ap_link_id].bssid = link_hmac_vap->st_vap_base_info.auc_bssid;
            params->links[ap_link_id].channel = get_roam_channel_info(&(link_hmac_vap->st_vap_base_info));
            if (params->links[ap_link_id].channel == NULL) {
                return OAL_ERR_CODE_PTR_NULL;
            }
        }
    }
...
#endif // CONFIG_HISI_WIFI7_MLO
    common_info_prepare_roam_resp_param(params, req_ie, req_ie_len, resp_ie, resp_ie_len);
    return OAL_SUCC;
}
#endif
                             ...
                             oal_gfp_enum_uint8 en_gfp)
{
    ...
    uint32_t ret;
    ret = prepare_roam_resp_params(pst_net_device, pst_channel, puc_bssid, puc_req_ie, ul_req_ie_len, puc_resp_ie,
        ul_resp_ie_len, &info);
    if (ret != OAL_SUCC) {
        return OAL_ERR_CODE_PTR_NULL;
    }
 ...
 * 功能描述  : 上报mic攻击
 */
void oal_cfg80211_mic_failure(oal_net_device_stru *net_device,
                              ...
                              oal_gfp_enum_uint8 en_gfp)
{
    ...
    if (oal_any_null_ptr1(net_device)) {
        return;
    }
    pst_wdev = net_device->ieee80211_ptr;
    ...
    oal_nla_put_u32_m(pst_msg, PRIV_NL80211_ATTR_IFINDEX, net_device->ifindex);

    if (mac_addr != NULL) {
        oal_nla_put_m(pst_msg, PRIV_NL80211_ATTR_MAC, OAL_ETH_ALEN_SIZE, mac_addr);
    }
    oal_nla_put_u32_m(pst_msg, PRIV_NL80211_ATTR_KEY_TYPE, key_type);
    oal_nla_put_u8(pst_msg, PRIV_NL80211_ATTR_KEY_IDX, key_id);
    if (tsc != NULL) {
        oal_nla_put_m(pst_msg, PRIV_NL80211_ATTR_KEY_SEQ, 6, tsc); /* 6为附加的长度 */
    }
...
{
#ifdef CONFIG_HISI_WIFI7_MLO
    mac_vap_stru *mac_vap = oal_net_dev_priv(pst_netdev);
    hmac_vap_stru *hmac_vap = NULL;
    ...
    /* mld场景下信道切换事件上报需要携带vap的link id，非mld场景无link id参数默认为0 */
    if (hmac_vap_is_legacy_mld_mode(hmac_vap) == OAL_TRUE) {
        cfg80211_ch_switch_notify(pst_netdev, pst_chandef, 0, 0);
    } else {
        cfg80211_ch_switch_notify(pst_netdev, pst_chandef, hmac_vap->mld_vap_link_info.link_ap_info.link_id, 0);
    }
#else
...
/*
 * 函 数 名  : oal_cfg80211_pmksa_candidate_notify
 * 功能描述  : STA上报给wpa_supplicatn漫游ap bssid
 */
void oal_cfg80211_pmksa_candidate_notify(oal_net_device_stru *dev, int index, const uint8_t *bssid,
    bool preauth, oal_gfp_enum_uint8 gfp)
{
    cfg80211_pmksa_candidate_notify(dev, index, bssid, preauth, gfp);
}
#endif
