...
uint32_t wal_cofeature_state_notify(frw_event_mem_stru *event_mem)
{
    ...
    hmac_cofeature_rpt_stru *feature_info = NULL;

    if (oal_unlikely(event_mem == NULL)) {
        oam_error_log0(0, OAM_SF_ASSOC, "{wal_cofeature_state_notify::event_mem is null!}");
        return OAL_ERR_CODE_PTR_NULL;
    }

    event = frw_get_event_stru(event_mem);
    feature_info = (hmac_cofeature_rpt_stru *)event->auc_event_data;

    /* 获取net_device */
    net_device = hmac_vap_get_net_device(event->st_event_hdr.uc_vap_id);
    if (net_device == NULL) {
        oam_error_log0(event->st_event_hdr.uc_vap_id, OAM_SF_ASSOC,
                       "{wal_cofeature_state_notify::get net device ptr is null!}");
        return OAL_ERR_CODE_PTR_NULL;
    }

#ifdef CONFIG_HW_EXT_AI_WIFI
    cfg80211_inform_cofeature_state(net_device, feature_info->feature, feature_info->state,
        feature_info->reason_code);
#endif

    oam_warning_log3(event->st_event_hdr.uc_vap_id, OAM_SF_ASSOC,
        "{wal_cofeature_state_notify::feature[%d] state[%d] reason_code[%d]}",
        feature_info->feature, feature_info->state, feature_info->reason_code);
