...
uint32_t wal_set_addts_req(mac_vap_stru *mac_vap, uint32_t *params)
{
    ...
    */
    ...
    if (mac_mib_get_QAPEDCATableMandatory(&(hmac_vap->st_vap_base_info), ac) == OAL_FALSE) {
...
uint32_t wal_hipriv_phy_event_rpt(mac_vap_stru *mac_vap, uint32_t *params)
{
    ...
    phy_event_cfg.wp_event2_type_sel = params[BIT_OFFSET_4];
    phy_event_cfg.wp_event3_type_sel = params[BIT_OFFSET_5];
    oam_warning_log2(mac_vap->uc_vap_id, OAM_SF_CFG, "wal_hipriv_phy_event_rpt en[%d] mem num[%d]",
        phy_event_cfg.event_rpt_en, phy_event_cfg.wp_mem_num);
    oam_warning_log4(mac_vap->uc_vap_id, OAM_SF_CFG,
        "wal_hipriv_phy_event_rpt event0 sel 0x%x event1 sel 0x%x event2 sel 0x%x event3 sel 0x%x",
        phy_event_cfg.wp_event0_type_sel, phy_event_cfg.wp_event1_type_sel,
        phy_event_cfg.wp_event2_type_sel, phy_event_cfg.wp_event3_type_sel);
    return OAL_SUCC;
}
#endif
...
uint32_t wal_test_dfr_start(mac_vap_stru *mac_vap, uint32_t *params)
{
    ...
    memset_s((uint8_t *)(g_st_dfr_info.past_netdev),
             sizeof(g_st_dfr_info.past_netdev[0]) * WLAN_VAP_MAX_NUM_LIMIT, 0,
             sizeof(g_st_dfr_info.past_netdev[0]) * WLAN_VAP_MAX_NUM_LIMIT);
 ...
 *   修改内容  : 新生成函数
 */
uint32_t wal_hipriv_set_ip_filter(oal_net_device_stru *net_dev, int8_t *param)
{
    uint32_t ret;
    uint32_t off_set = 0;
    ...
    /* 其取出子命令 */
    ret = wal_get_cmd_one_arg(param, ac_cmd, WAL_HIPRIV_CMD_NAME_MAX_LEN, &off_set);
    if (ret != OAL_SUCC) {
    ...
    param += off_set;
    ret = wal_get_cmd_one_arg(param, ac_cmd_param, WAL_HIPRIV_CMD_NAME_MAX_LEN, &off_set);
    if (ret != OAL_SUCC) {
    ...
    if (oal_strncmp(ac_cmd, CMD_SET_RX_FILTER_ENABLE, OAL_STRLEN(CMD_SET_RX_FILTER_ENABLE)) == 0) {
        ...
        return wal_ipriv_ip_filter_items(param, ac_cmd_param, &off_set);
    } else {
...
#ifdef _PRE_WLAN_FEATURE_PKT_FILTER
#define PKT_FILTER_LIST_PROGRAM_LEN 200
/*
 * 函 数 名  : wal_hipriv_pkt_filter_list
 * 功能描述  : hipriv命令 显示pkt filter到sdt
 * 1.日    期  : 2017年11月23日
 *   作    者  : wifi
 *   修改内容  : 新生成函数
 */
uint32_t wal_hipriv_pkt_filter_list(oal_net_device_stru *net_dev, int8_t *pc_param)
{
    wal_msg_write_stru write_msg = {0};
    uint32_t off_set;
    int8_t ac_name[WAL_HIPRIV_CMD_NAME_MAX_LEN] = {0};
    int32_t ret;
    mac_pkt_filter_cmd_stru st_pkt_filter_cmd;
    uint8_t program[PKT_FILTER_LIST_PROGRAM_LEN] = { 0 };

    memset_s(&st_pkt_filter_cmd, sizeof(mac_pkt_filter_cmd_stru), 0, sizeof(mac_pkt_filter_cmd_stru));
    ret = wal_get_cmd_one_arg(pc_param, ac_name, WAL_HIPRIV_CMD_NAME_MAX_LEN, &off_set);
    if (ret != OAL_SUCC) {
        oam_warning_log1(0, OAM_SF_CFG,
            "{wal_hipriv_pkt_filter_list::wal_get_cmd_one_arg return err_code %d!}\r\n", ret);
        return ret;
    }
    if ((oal_strcmp("0", ac_name) != 0) && (oal_strcmp("1", ac_name) != 0)) {
        oam_error_log0(0, OAM_SF_CFG, "{wal_hipriv_pkt_filter_list::invalid parameter.}\r\n");
        return OAL_ERR_CODE_INVALID_CONFIG;
    }
    st_pkt_filter_cmd.en_cmd_type = (uint8_t)oal_atoi(ac_name);
    if (st_pkt_filter_cmd.en_cmd_type == PKT_FILTER_SET_FILTER_CMD) {
        st_pkt_filter_cmd.us_program_len = PKT_FILTER_LIST_PROGRAM_LEN;
        st_pkt_filter_cmd.puc_program = program;
    }

    /* 抛事件到wal层处理 */
    wal_write_msg_hdr_init(&write_msg, WLAN_CFGID_SET_PKT_FILTER, sizeof(st_pkt_filter_cmd));
    ret = memcpy_s(write_msg.auc_value, sizeof(write_msg.auc_value),
                   &st_pkt_filter_cmd, sizeof(st_pkt_filter_cmd));
    if (ret != EOK) {
        oam_error_log0(0, OAM_SF_ANY, "wal_hipriv_pkt_filter_list::memcpy fail!");
        return OAL_FAIL;
    }
    /* 发送消息 */
    ret = wal_send_cfg_event(net_dev,
                             WAL_MSG_TYPE_WRITE,
                             WAL_MSG_WRITE_MSG_HDR_LENGTH + sizeof(st_pkt_filter_cmd),
                             (uint8_t *)&write_msg,
                             OAL_FALSE,
                             NULL);
    if (oal_unlikely(ret != OAL_SUCC)) {
...
static void wal_hipriv_autorate_param(oal_net_device_stru *net_dev)
{
    ...
    mac_cfg_set_autorate_param autorate_param = {
        NARROW_RATE_UNLIMIT_AP,         /* AP模式不限制档位(仅仅1对1模式)，STA模式限制在0挡或1挡 */
//      NARROW_RATE_UNLIMIT_STA,     /* STA模式不限制档位，AP模式限制在0挡或1挡 */
//      NARROW_RATE_UNLIMIT_AP_STA,  /* AP模式不限制档位(仅仅1对1模式)，STA模式不限制档位 */
        5, /* 最大档位 */
        0, /* 当前档位 */
        {4, 3},
        {{85, -90, NARROW_BW_5M, 111, 3, 700, {0, 0}, WLAN_LEGACY_OFDM_6M_BPS },  /* 5G 0档位参数5M-6m */
         {85, -85, NARROW_BW_10M, 111, 3, 700, {0, 0}, WLAN_LEGACY_OFDM_6M_BPS },  /* 5G 1档位参数10M-6m */
         {85, -80, NARROW_BW_10M, 100, 3, 700, {0, 0}, WLAN_LEGACY_OFDM_9M_BPS },  /* 5G 2档位参数10M-9m */
         {85, -75, NARROW_BW_10M, 100, 2, 700, {0, 0}, WLAN_LEGACY_OFDM_18M_BPS }, /* 5G 3档位参数10M-18m */
         {85, -70, NARROW_BW_10M, 80, 2, 700, {0, 0}, WLAN_LEGACY_OFDM_24M_BPS },  /* 5G 4档位参数10M-24m */
         {85, -60, NARROW_BW_10M, 80, 0, 650, {0, 0}, WLAN_LEGACY_OFDM_48M_BPS },  /* 5G 5档位参数10M-48m */
         {85, -90, NARROW_BW_5M, 900, 2, 750, {0, 0}, WLAN_LEGACY_OFDM_6M_BPS },  /* 2G 0档位参数5M-6m */
         {85, -85, NARROW_BW_10M, 900, 2, 750, {0, 0}, WLAN_LEGACY_OFDM_6M_BPS },  /* 2G 1档位参数10M-6m */
         {85, -80, NARROW_BW_10M, 600, 2, 750, {0, 0}, WLAN_LEGACY_OFDM_9M_BPS },  /* 2G 2档位参数10M-9m */
         {85, -75, NARROW_BW_10M, 500, 2, 750, {0, 0}, WLAN_LEGACY_OFDM_18M_BPS }, /* 2G 3档位参数10M-18m */
         {85, -70, NARROW_BW_10M, 400, 2, 750, {0, 0}, WLAN_LEGACY_OFDM_24M_BPS },  /* 2G 4档位参数10M-24m */
         {85, -60, NARROW_BW_10M, 300, 2, 750, {0, 0}, WLAN_LEGACY_OFDM_48M_BPS }}  /* 2G 5档位参数10M-48m */
    };
    uint32_t ret;
    uint32_t pow_param;
    uint8_t loop;

    if (g_nb_power_enable) {
        for (loop = 0; loop < EXPORT_POW_LEVEL; loop++) {
            pow_param = hwifi_get_init_value(CUS_TAG_INI, WLAN_CFG_INIT_5G_POW_PARA_L0 + loop);
            /* ini中功率参数为8位16进制数,低3位表示dbb值,中2位表示lpf值,高3位表示upc值 */
            autorate_param.autorate_set[loop].dbb = pow_param & 0xfff;
            autorate_param.autorate_set[loop].lpf = (pow_param >> EXPORT_POW_LPF_OFFSET) & 0xff;
            autorate_param.autorate_set[loop].upc = (pow_param >> EXPORT_POW_UPC_OFFSET) & 0xfff;
        }
        autorate_param.autorate_set[0].bandwith = NARROW_BW_5M;
        autorate_param.autorate_set[EXPORT_2G_NB_PARAM_OFFSET].bandwith = NARROW_BW_5M;
    }
...
uint32_t wal_hipriv_get_sw_version(oal_net_device_stru *net_dev, int8_t *param)
{
    uint32_t ret;
    wal_msg_write_stru write_msg;
    ...
    ***************************************************************************/
    ...
    if (oal_unlikely(ret != OAL_SUCC)) {
        oam_warning_log1(0, OAM_SF_ANY, "{wal_hipriv_get_sw_version::return err code %d!}\r\n", ret);
        return ret;
    }
    return OAL_SUCC;
}
...
uint32_t wal_hipriv_export_set_tpc_param(oal_net_device_stru *net_dev, int8_t *param)
{
    ...
    param = param + off_set;
    cfg_power->value = (uint32_t)oal_atoi(ac_arg);
...
uint32_t wal_hipriv_nb_power_enable(oal_net_device_stru *net_dev, int8_t *param)
{
    int32_t ret;
    uint32_t offset;
    int8_t ac_arg[WAL_HIPRIV_CMD_NAME_MAX_LEN];
    ...
    ret = wal_get_cmd_one_arg(param, ac_arg, sizeof(ac_arg), &offset);
    if (ret != OAL_SUCC) {
        oam_warning_log1(0, OAM_SF_ANY, "{wal_hipriv_nb_power_enable::wal_get_cmd_one_arg fail err_code [%d]!}", ret);
        return ret;
    }
    g_nb_power_enable = (uint8_t)oal_atoi(ac_arg);
    wal_hipriv_autorate_param(net_dev);
    return OAL_SUCC;
}

uint32_t wal_hipriv_hitalk_bg_listen(oal_net_device_stru *pst_net_dev, int8_t *pc_param)
{
    wal_msg_write_stru st_write_msg;
    mac_cfg_set_div_stru *pst_set_div;
    uint32_t ul_ret;
    int32_t ret;
    int8_t ac_arg[WAL_HIPRIV_CMD_NAME_MAX_LEN];
    uint32_t ul_off_set;
    ...
    /* 入参合法判断 */
    if (oal_unlikely((pst_net_dev == NULL) || (pc_param == NULL))) {
    ...
    /***************************************************************************
                                抛事件到wal层处理
    ***************************************************************************/
    wal_write_msg_hdr_init(&st_write_msg, WLAN_CFGID_DIV_SET, sizeof(mac_cfg_set_div_stru));

    /* 解析并设置配置命令参数 */
    pst_set_div = (mac_cfg_set_div_stru *)(st_write_msg.auc_value);

    ul_ret = wal_get_cmd_one_arg(pc_param, ac_arg, sizeof(ac_arg), &ul_off_set);
    if (ul_ret != OAL_SUCC) {
        oam_warning_log1(0, OAM_SF_ANY, "{wal_hipriv_hitalk_bg_listen::get enable[%d]!}\r\n", ul_ret);
        return ul_ret;
    }
    pc_param = pc_param + ul_off_set;
    pst_set_div->chan = (uint16_t)oal_atoi(ac_arg);
    ...
    pc_param = pc_param + ul_off_set;
    pst_set_div->div_int = (uint16_t)oal_atoi(ac_arg);
    ...
    pc_param = pc_param + ul_off_set;
    pst_set_div->div_frac = (uint16_t)oal_atoi(ac_arg);

    oam_warning_log3(0, OAM_SF_ANY,
        "{wal_hipriv_hitalk_bg_listen: chan=%d int=%d frac=%d", pst_set_div->chan, pst_set_div->div_int,
        pst_set_div->div_frac);

    ret = wal_send_cfg_event(pst_net_dev, WAL_MSG_TYPE_WRITE,
        WAL_MSG_WRITE_MSG_HDR_LENGTH + sizeof(mac_cfg_set_div_stru), (uint8_t *)&st_write_msg, OAL_FALSE, NULL);
    if (oal_unlikely(ret != OAL_SUCC)) {
        return OAL_FAIL;
    }
...
uint32_t wal_hipriv_hitalk_set(oal_net_device_stru *net_dev, int8_t *param)
{
    /* 入参合法判断 */
    ...
    g_hitalk_set = OAL_TRUE;

    return OAL_SUCC;
}
#endif
static void wal_hipriv_narrow_bw_config_alg(oal_net_device_stru *pst_net_dev, oal_bool_enum_uint8 nb_is_open)
{
    if (nb_is_open == OAL_TRUE) {
...
uint32_t wal_hipriv_narrow_bw(oal_net_device_stru *pst_net_dev, int8_t *pc_param)
{
    ...
    ***************************************************************************/
    ...
    wal_hipriv_narrow_bw_config_alg(pst_net_dev, pst_nrw_bw->en_open);
#ifdef _PRE_WLAN_EXPORT
    ...
    if ((oal_strcmp("0.25m", ac_arg)) == 0) {
        pst_nrw_bw->en_bw = NARROW_BW_0_25M;
    } else if ((oal_strcmp("1m", ac_arg)) == 0) {
        ...
        oam_warning_log0(0, OAM_SF_ANY, "{wal_hipriv_narrow_bw::bw should be 0.25/1/5/10 m");
        return OAL_ERR_CODE_INVALID_CONFIG;
    }
...
OAL_STATIC uint32_t wal_hipriv_nan_set_param(oal_net_device_stru *net_device, int8_t *param)
{
    ...
    if (wal_send_cfg_event(net_device, WAL_MSG_TYPE_WRITE,
...
OAL_STATIC uint32_t wal_hipriv_vsp_link_en(oal_net_device_stru *net_device, int8_t *param)
{
    ...
    if (wal_send_cfg_event(net_device, WAL_MSG_TYPE_WRITE, WAL_MSG_WRITE_MSG_HDR_LENGTH + sizeof(hmac_vsp_debug_cmd),
        (uint8_t*)&write_msg, OAL_FALSE, NULL) != OAL_SUCC) {
...
uint32_t wal_hipriv_11v_tx_query(oal_net_device_stru *pst_net_dev, int8_t *pc_param)
{
    ...
    ***************************************************************************/
    ...
    /* 设置配置命令参数 */
    if (memcpy_s(st_write_msg.auc_value, sizeof(st_write_msg.auc_value), auc_mac_addr, WLAN_MAC_ADDR_LEN) != EOK) {
...
uint32_t wal_hipriv_ml_reconfig_remove_link(oal_net_device_stru *net_dev, int8_t *param)
{
    int32_t l_ret;
    uint32_t ret;
    uint32_t off_set = 0;
    char name[WAL_HIPRIV_CMD_NAME_MAX_LEN] = { 0 };
    mac_cfg_remove_ml_link_ap_param_stru cfg_remove_link_param = { 0 };
    wal_msg_write_stru write_msg = { 0 };

    if (net_dev == NULL || param == NULL) {
        oam_error_log0(0, OAM_SF_ANY, "{wal_hipriv_ml_reconfig_remove_link::NULL!}");
        return OAL_FAIL;
    }

    /* 获取预期的delete link时间，单位为tbtt周期 */
    ret = wal_get_cmd_one_arg(param, name, WAL_HIPRIV_CMD_NAME_MAX_LEN, &off_set);
    if (ret != OAL_SUCC) {
        oam_warning_log1(0, OAM_SF_ANY, "{wal_hipriv_ml_reconfig_remove_link::wal_get_cmd_one_arg fail[%u]!}", ret);
        return ret;
    }
    param += off_set;
    cfg_remove_link_param.tbtt_num = (uint16_t)oal_atoi(name);

    /* 获取delete的link vap id */
    ret = wal_get_cmd_one_arg(param, name, WAL_HIPRIV_CMD_NAME_MAX_LEN, &off_set);
    if (ret != OAL_SUCC) {
        oam_warning_log1(0, OAM_SF_ANY, "{wal_hipriv_ml_reconfig_remove_link::wal_get_cmd_one_arg fail[%u]!}", ret);
        return ret;
    ...
    param += off_set;
    cfg_remove_link_param.delete_link_id = (uint16_t)oal_atoi(name);

    wal_write_msg_hdr_init(&write_msg, WLAN_CFGID_ML_RECONFIG_REMOVE_LINK,
        sizeof(mac_cfg_remove_ml_link_ap_param_stru));

    if (memcpy_s(write_msg.auc_value, sizeof(write_msg.auc_value), &cfg_remove_link_param,
        sizeof(mac_cfg_remove_ml_link_ap_param_stru)) != EOK) {
        oam_error_log0(0, OAM_SF_ANY, "{wal_hipriv_ml_reconfig_remove_link::memcpy fail!}");
        return OAL_FAIL;
    }

    l_ret = wal_send_cfg_event(net_dev, WAL_MSG_TYPE_WRITE,
        WAL_MSG_WRITE_MSG_HDR_LENGTH + sizeof(mac_cfg_remove_ml_link_ap_param_stru),
        (uint8_t *)&write_msg, OAL_FALSE, NULL);
    if (l_ret != OAL_SUCC) {
        oam_error_log1(0, OAM_SF_ANY, "{wal_hipriv_ml_reconfig_remove_link::send cfg event fail, err code[%d]!}",
            l_ret);
        return OAL_FAIL;
    }
    return OAL_SUCC;
...
uint32_t wal_hipriv_tx_action_ml_reconfig_request(oal_net_device_stru *netdev, int8_t *param)
{
    int32_t l_ret;
    uint32_t ret;
    uint32_t off_set = 0;
    char name[WAL_HIPRIV_CMD_NAME_MAX_LEN] = { 0 };
    mac_cfg_tx_ml_reconfig_param_stru cfg_tx_ml_reconfig_param;
    wal_msg_write_stru write_msg;

    if (netdev == NULL || param == NULL) {
        oam_error_log0(0, OAM_SF_ANY, "{wal_hipriv_tx_action_ml_reconfig_request::NULL!}");
        return OAL_FAIL;
    }
    memset_s(&cfg_tx_ml_reconfig_param, sizeof(mac_cfg_tx_ml_reconfig_param_stru), 0,
        sizeof(mac_cfg_tx_ml_reconfig_param_stru));
    memset_s(&write_msg, sizeof(wal_msg_write_stru), 0, sizeof(wal_msg_write_stru));
    cfg_tx_ml_reconfig_param.diag_token = 0;
    /* 解析linkid */
    ret = wal_get_cmd_one_arg(param, name, WAL_HIPRIV_CMD_NAME_MAX_LEN, &off_set);
    if (ret != OAL_SUCC) {
        oam_warning_log1(0, OAM_SF_ANY, "{wal_hipriv_tx_action_ml_reconfig_request::get arg fail[%u]!}", ret);
        return ret;
    }
    cfg_tx_ml_reconfig_param.ap_link_id[0] = (uint8_t)oal_atoi(name);
    cfg_tx_ml_reconfig_param.link_op_num = 1;   // 指令暂时只支持一次操作一个link
    param += off_set;

    /* 解析operation_type ap removal=0 add link=2 remove link = 3 */
    ret = wal_get_cmd_one_arg(param, name, WAL_HIPRIV_CMD_NAME_MAX_LEN, &off_set);
    if (ret != OAL_SUCC) {
        oam_warning_log1(0, OAM_SF_ANY, "{wal_hipriv_tx_action_ml_reconfig_request::get arg fail[%u]!}", ret);
        return ret;
    }
    cfg_tx_ml_reconfig_param.op_type[0] = (uint8_t)oal_atoi(name);
    param += off_set;

    wal_write_msg_hdr_init(&write_msg, WLAN_CFGID_TX_ML_RECONFIG_REQUEST,
        sizeof(mac_cfg_tx_ml_reconfig_param_stru));

    if (memcpy_s(write_msg.auc_value, sizeof(write_msg.auc_value), &cfg_tx_ml_reconfig_param,
        sizeof(mac_cfg_tx_ml_reconfig_param_stru)) != EOK) {
        oam_error_log0(0, OAM_SF_ANY, "{wal_hipriv_tx_action_ml_reconfig_request::memcpy fail!}");
        return OAL_FAIL;
    }

    l_ret = wal_send_cfg_event(netdev, WAL_MSG_TYPE_WRITE,
        WAL_MSG_WRITE_MSG_HDR_LENGTH + sizeof(mac_cfg_tx_ml_reconfig_param_stru),
        (uint8_t*)&write_msg, OAL_FALSE, NULL);
    if (l_ret != OAL_SUCC) {
        oam_error_log1(0, OAM_SF_ANY, "{wal_hipriv_tx_action_ml_reconfig_request::send cfg event fail, err code[%u]!}",
            l_ret);
        return (uint32_t)l_ret;
    }
    return OAL_SUCC;
...
OAL_STATIC uint32_t wal_hipriv_hw_ml_mode_switch_parase_param(oal_net_device_stru *net_dev, int8_t *param,
    uint8_t *mld_mode, uint8_t len)
{
    int idx;
    uint32_t off_set = 0;
    char name[WAL_HIPRIV_CMD_NAME_MAX_LEN] = { 0 };

    if (len < WAL_HIPRIV_ML_MODE_PARAM) {
        return OAL_FAIL;
    }

    for (idx = 0; idx < WAL_HIPRIV_ML_MODE_PARAM; idx++) {
        param += off_set;
        if (wal_get_cmd_one_arg(param, name, WAL_HIPRIV_CMD_NAME_MAX_LEN, &off_set) != OAL_SUCC) {
            return OAL_FAIL;
        }
        mld_mode[idx] = (uint8_t)oal_atoi(name);
    }

    return OAL_SUCC;
}

uint32_t wal_hipriv_set_str_ml_tx_mode(oal_net_device_stru *net_dev, int8_t *param)
{
    wal_msg_write_stru write_msg = { 0 };

    /* 本命令带5个参数,mld_user_idx tid ml0_dup_en ml1_dup_en dup/split_switch_en */
    uint32_t ret = wal_hipriv_hw_ml_mode_switch_parase_param(net_dev, param, write_msg.auc_value,
        WAL_HIPRIV_ML_MODE_PARAM);
    if (ret != OAL_SUCC) {
        return ret;
    }
    wal_write_msg_hdr_init(&write_msg, WLAN_CFGID_ML_MODE_SWITCH, WAL_HIPRIV_ML_MODE_PARAM);
    return (uint32_t)wal_send_cfg_event(net_dev, WAL_MSG_TYPE_WRITE,
                                        WAL_MSG_WRITE_MSG_HDR_LENGTH + WAL_HIPRIV_ML_MODE_PARAM,
                                        (uint8_t*)&write_msg, OAL_FALSE, NULL);
}

uint32_t wal_hipriv_set_mld_assoc_mode(oal_net_device_stru *net_dev, int8_t *param)
{
    wal_msg_write_stru    write_msg = { 0 };
    uint32_t off_set = 0;
    char name[WAL_HIPRIV_CMD_NAME_MAX_LEN] = { 0 };
    ...
    mld_assoc_mode = (uint8_t)oal_atoi(name);
    write_msg.auc_value[0] = mld_assoc_mode;

    wal_write_msg_hdr_init(&write_msg, WLAN_CFGID_SET_MLD_ASSOC_MODE, sizeof(uint8_t));
    return (uint32_t)wal_send_cfg_event(net_dev, WAL_MSG_TYPE_WRITE,
                                        WAL_MSG_WRITE_MSG_HDR_LENGTH + sizeof(uint8_t),
                                        (uint8_t*)&write_msg, OAL_FALSE, NULL);
}

uint32_t wal_hipriv_mlsr_work_link_switch(oal_net_device_stru *net_dev, int8_t *param)
{
    wal_msg_write_stru write_msg = { 0 };
    uint32_t off_set = 0;
    char name[WAL_HIPRIV_CMD_NAME_MAX_LEN] = { 0 };
    ...
    link_switch_param.enable_link_id = (uint8_t)oal_atoi(name);
    link_switch_param.trigger_reason = LINK_SWITCH_FOR_HIPRIV;
    if (memcpy_s(write_msg.auc_value, sizeof(write_msg.auc_value), &link_switch_param,
        sizeof(mac_link_switch_param_stru)) != EOK) {
        oam_error_log0(0, OAM_SF_ANY, "{wal_hipriv_mlsr_work_link_switch::memcpy fail!}");
        return OAL_FAIL;
    }

    wal_write_msg_hdr_init(&write_msg, WLAN_CFGID_MLSR_WROK_LINK_SWITCH, sizeof(mac_link_switch_param_stru));
    return (uint32_t)wal_send_cfg_event(net_dev, WAL_MSG_TYPE_WRITE,
                                        WAL_MSG_WRITE_MSG_HDR_LENGTH + sizeof(mac_link_switch_param_stru),
                                        (uint8_t*)&write_msg, OAL_FALSE, NULL);
}

OAL_STATIC uint32_t wal_hipriv_parse_link_mapping_info(int8_t *param, mac_cfg_ttlm_frame_param *ttlm_req_param)
{
    uint8_t         tid;
    uint32_t        ret;
    uint32_t        off_set = 0;
    ...
    /* 获取link mapping presence bitmap */
    ret = wal_get_cmd_one_arg(param, name, WAL_HIPRIV_CMD_NAME_MAX_LEN, &off_set);
    if (ret != OAL_SUCC) {
        oam_warning_log1(0, OAM_SF_ANY, "{wal_hipriv_parse_link_mapping_info::wal_get_cmd_one_arg fail[%u]!}", ret);
        return ret;
    }
    ttlm_req_param->cfg_ttlm_ie.ttlm_ctrl.link_mapping_presence_bitmap = (uint8_t)oal_atoi(name);
    if (ttlm_req_param->cfg_ttlm_ie.ttlm_ctrl.link_mapping_presence_bitmap == 0) {
        oam_error_log1(0, OAM_SF_ANY, "{wal_hipriv_parse_link_mapping_info::invalid link_mapping_presence_bitmap=%u}",
            ttlm_req_param->cfg_ttlm_ie.ttlm_ctrl.link_mapping_presence_bitmap);
        return OAL_ERR_CODE_INVALID_CONFIG;
    }
    param += off_set;

    /* Link Mapping Of TID x */
    for (tid = 0; tid < WLAN_TIDNO_BUTT; tid++) {
        ret = wal_get_cmd_one_arg(param, name, WAL_HIPRIV_CMD_NAME_MAX_LEN, &off_set);
        if (ret != OAL_SUCC) {
            oam_warning_log1(0, OAM_SF_ANY, "{wal_hipriv_parse_link_mapping_info::get link_bitmap fail[%u]!}", ret);
            return ret;
        }
        ttlm_req_param->cfg_ttlm_ie.link_bitmap[tid] = (uint8_t)oal_atoi(name);
        if (ttlm_req_param->cfg_ttlm_ie.link_bitmap[tid] >= (1 << WLAN_MAX_ML_LINK_NUM)) {
            oam_warning_log0(0, OAM_SF_ANY, "{wal_hipriv_parse_link_mapping_info::invalid link_bitmap value}");
            return OAL_ERR_CODE_INVALID_CONFIG;
        }
        param += off_set;
    }
...
OAL_STATIC uint32_t wal_hipriv_parse_ttlm_request_param(int8_t *param, mac_cfg_ttlm_frame_param *ttlm_req_param)
{
    uint32_t        ret;
    uint32_t        off_set = 0;
    char            name[WAL_HIPRIV_CMD_NAME_MAX_LEN] = {0};
    ...
    ret = wal_get_cmd_one_arg(param, name, WAL_HIPRIV_CMD_NAME_MAX_LEN, &off_set);
    if (ret != OAL_SUCC) {
        oam_warning_log1(0, OAM_SF_ANY, "{wal_hipriv_parse_ttlm_request_param::get mac addr fail[%u]!}", ret);
        return ret;
    }
    oal_strtoaddr(name, sizeof(name), ttlm_req_param->user_addr, WLAN_MAC_ADDR_LEN);
    param += off_set;
    /* 获取dialog_token */
    ret = wal_get_cmd_one_arg(param, name, WAL_HIPRIV_CMD_NAME_MAX_LEN, &off_set);
    if (ret != OAL_SUCC) {
        oam_warning_log1(0, OAM_SF_ANY, "{wal_hipriv_parse_ttlm_request_param::get dialog_token fail[%u]!}", ret);
        return ret;
    }
    ttlm_req_param->dialog_token = (uint8_t)oal_atoi(name);
    if (ttlm_req_param->dialog_token > 0xff) {
        oam_error_log1(0, OAM_SF_ANY, "{wal_hipriv_parse_ttlm_request_param::invalid dialog_token=%u!}",
            ttlm_req_param->dialog_token);
        return OAL_ERR_CODE_INVALID_CONFIG;
    }
    param += off_set;
    /* 获取direction */
    ret = wal_get_cmd_one_arg(param, name, WAL_HIPRIV_CMD_NAME_MAX_LEN, &off_set);
    if (ret != OAL_SUCC) {
        oam_warning_log1(0, OAM_SF_ANY, "{wal_hipriv_parse_ttlm_request_param::get direction fail[%u]!}", ret);
        return ret;
    }
    ttlm_req_param->cfg_ttlm_ie.ttlm_ctrl.direction = (uint8_t)oal_atoi(name);
    if (ttlm_req_param->cfg_ttlm_ie.ttlm_ctrl.direction > WLAN_DOWNUPLINK) {
        oam_error_log1(0, OAM_SF_ANY, "{wal_hipriv_parse_ttlm_request_param::invalid direction = %u!}",
            ttlm_req_param->cfg_ttlm_ie.ttlm_ctrl.direction);
        return OAL_ERR_CODE_INVALID_CONFIG;
    }
    param += off_set;
    return wal_hipriv_parse_link_mapping_info(param, ttlm_req_param);
}

uint32_t wal_hipriv_mld_tx_ttlm_request(oal_net_device_stru *netdev, int8_t *param)
{
    int32_t                  ret;
    mac_cfg_ttlm_frame_param ttlm_req_param;
    wal_msg_write_stru    write_msg = { 0 };

    memset_s((uint8_t *)&ttlm_req_param, sizeof(mac_cfg_ttlm_frame_param), 0, sizeof(mac_cfg_ttlm_frame_param));
    oam_warning_log0(0, OAM_SF_ANY, "{wal_hipriv_parse_ttlm_request_param1}");

    if (wal_hipriv_parse_ttlm_request_param(param, &ttlm_req_param) != OAL_SUCC) {
        oam_warning_log0(0, OAM_SF_ANY, "{wal_hipriv_mld_tx_ttlm_request::parse send ttlm request fail!}");
        return OAL_FAIL;
    }
    ret = memcpy_s(write_msg.auc_value, sizeof(write_msg.auc_value), &ttlm_req_param, sizeof(mac_cfg_ttlm_frame_param));
    if (ret != EOK) {
        oam_error_log0(0, OAM_SF_ANY, "{wal_hipriv_mld_tx_ttlm_request::memcpy fail!}");
        return OAL_FAIL;
    }
    oam_warning_log0(0, OAM_SF_ANY, "{wal_hipriv_parse_ttlm_request_param2}");

    wal_write_msg_hdr_init(&write_msg, WLAN_CFGID_ML_TTLM_REQUEST, sizeof(mac_cfg_ttlm_frame_param));

    return (uint32_t)wal_send_cfg_event(netdev, WAL_MSG_TYPE_WRITE,
                                        WAL_MSG_WRITE_MSG_HDR_LENGTH + sizeof(mac_cfg_ttlm_frame_param),
                                        (uint8_t*)&write_msg, OAL_FALSE, NULL);
}


uint32_t wal_hipriv_mld_tx_ttlm_teardown(oal_net_device_stru *netdev, int8_t *param)
{
    uint32_t                 ret;
    uint32_t                 off_set = 0;
    mac_cfg_ttlm_frame_param teardown_param;
    char                     name[WAL_HIPRIV_CMD_NAME_MAX_LEN] = {0};
    wal_msg_write_stru    write_msg = { 0 };

    memset_s((uint8_t *)&teardown_param, sizeof(mac_cfg_ttlm_frame_param), 0, sizeof(mac_cfg_ttlm_frame_param));

    /* 获取mac地址 */
    ret = wal_get_cmd_one_arg(param, name, WAL_HIPRIV_CMD_NAME_MAX_LEN, &off_set);
    if (ret != OAL_SUCC) {
        oam_warning_log1(0, OAM_SF_ANY, "{wal_hipriv_mld_tx_ttlm_teardown::get mac addr fail ret[%u]!}", ret);
        return ret;
    }
    oal_strtoaddr(name, sizeof(name), teardown_param.user_addr, WLAN_MAC_ADDR_LEN);
    param += off_set;
    ret = memcpy_s(write_msg.auc_value, sizeof(write_msg.auc_value), &teardown_param, sizeof(mac_cfg_ttlm_frame_param));
    if (ret != EOK) {
        oam_error_log0(0, OAM_SF_ANY, "{wal_hipriv_mld_tx_ttlm_teardown::memcpy fail!}");
        return OAL_FAIL;
    }
    wal_write_msg_hdr_init(&write_msg, WLAN_CFGID_ML_TTLM_TEARDOWN, sizeof(mac_cfg_ttlm_frame_param));
    return (uint32_t)wal_send_cfg_event(netdev, WAL_MSG_TYPE_WRITE,
                                        WAL_MSG_WRITE_MSG_HDR_LENGTH + sizeof(mac_cfg_ttlm_frame_param),
                                        (uint8_t*)&write_msg, OAL_FALSE, NULL);
}
...
OAL_STATIC uint32_t wal_hipriv_parse_mld_send_emlsr(oal_net_device_stru *net_dev, int8_t *param,
    mac_cfg_emlsr_action_param_stru *emlsr_cfg)
{
    uint32_t off_set = 0;
    char name[WAL_HIPRIV_CMD_NAME_MAX_LEN] = { 0 };
    ...
    emlsr_cfg->emlsr_mode = (uint8_t)oal_atoi(name);
    if ((emlsr_cfg->emlsr_mode != 0) && (emlsr_cfg->emlsr_mode != 1)) {
        oam_error_log0(0, OAM_SF_ANY, "{wal_hipriv_parse_mld_send_emlsr::emlsr_mode is not correct!}\r\n");
    }
    param += off_set;
    ...
    emlsr_cfg->emlsr_pm = (uint8_t)oal_atoi(name);
    if ((emlsr_cfg->emlsr_pm != 0) && (emlsr_cfg->emlsr_pm != 1)) {
        oam_error_log0(0, OAM_SF_ANY, "{wal_hipriv_parse_mld_send_emlsr::emlsr_pm is not correct!}\r\n");
    }
    param += off_set;

    /* 获取emlsr padding delay */
    if (wal_get_cmd_one_arg(param, name, WAL_HIPRIV_CMD_NAME_MAX_LEN, &off_set) != OAL_SUCC) {
        return OAL_FAIL;
    }
    emlsr_cfg->emlsr_padding_delay = (uint8_t)oal_atoi(name);
    if (emlsr_cfg->emlsr_padding_delay > EMLSR_PADDING_DELAY_BUTT) {
        oam_error_log0(0, OAM_SF_ANY, "{wal_hipriv_parse_mld_send_emlsr::emlsr_padding_delay is error!}\r\n");
    }
    param += off_set;

    /* 获取emlsr emlsr_transition_delay */
    if (wal_get_cmd_one_arg(param, name, WAL_HIPRIV_CMD_NAME_MAX_LEN, &off_set) != OAL_SUCC) {
        return OAL_FAIL;
    }
    emlsr_cfg->emlsr_transition_delay = (uint8_t)oal_atoi(name);
    if (emlsr_cfg->emlsr_transition_delay > EMLSR_TRANSITION_DELAY_BUTT) {
        oam_error_log0(0, OAM_SF_ANY, "{wal_hipriv_parse_mld_send_emlsr::emlsr_transition_delay is error!}\r\n");
    }

    return OAL_SUCC;
}

uint32_t wal_hipriv_emlsr_enable(oal_net_device_stru *net_dev, int8_t *param)
{
    wal_msg_write_stru    write_msg = { 0 };
    mac_cfg_emlsr_action_param_stru emlsr_param = { 0 };
    int32_t ret;

    ret = wal_hipriv_parse_mld_send_emlsr(net_dev, param, &emlsr_param);
    if (ret != OAL_SUCC) {
        return ret;
    }
    /* hipriv下发标记位，后面不会进入拦截 */
    emlsr_param.enter_reason = EMLSR_ENTER_HIPRIV;
    ret = memcpy_s(write_msg.auc_value, sizeof(write_msg.auc_value),
                   &emlsr_param, sizeof(emlsr_param));
    if (ret != EOK) {
        oam_error_log0(0, OAM_SF_ANY, "wal_hipriv_emlsr_enable::memcpy fail!");
        return OAL_FAIL;
    }
...
OAL_STATIC uint32_t wal_hipriv_parse_mld_emlsr_cap(oal_net_device_stru *net_dev, int8_t *param,
    hmac_mld_emlsr_cap_stru *emlsr_cap)
{
    uint32_t off_set = 0;
    char name[WAL_HIPRIV_CMD_NAME_MAX_LEN] = { 0 };

    /* 获取emlsr emlsr_support */
    if (wal_get_cmd_one_arg(param, name, WAL_HIPRIV_CMD_NAME_MAX_LEN, &off_set) != OAL_SUCC) {
        return OAL_FAIL;
    }
    emlsr_cap->emlsr_support = (uint8_t)oal_atoi(name);
    if ((emlsr_cap->emlsr_support != 0) && (emlsr_cap->emlsr_support != 1)) {
        oam_error_log0(0, OAM_SF_ANY, "{wal_hipriv_parse_mld_emlsr_cap::emlsr_support is not correct!}\r\n");
    }
    param += off_set;

    /* 获取emlsr emlsr_padding_delay */
    if (wal_get_cmd_one_arg(param, name, WAL_HIPRIV_CMD_NAME_MAX_LEN, &off_set) != OAL_SUCC) {
        return OAL_FAIL;
    }
    emlsr_cap->emlsr_padding_delay = (uint8_t)oal_atoi(name);
    if (emlsr_cap->emlsr_padding_delay > EMLSR_PADDING_DELAY_BUTT) {
        oam_error_log0(0, OAM_SF_ANY, "{wal_hipriv_parse_mld_emlsr_cap::emlsr_padding_delay is not correct!}\r\n");
    }
    param += off_set;
    ...
    emlsr_cap->emlsr_transition_delay = (uint8_t)oal_atoi(name);
    if (emlsr_cap->emlsr_transition_delay > EMLSR_TRANSITION_DELAY_BUTT) {
        oam_error_log0(0, OAM_SF_ANY, "{wal_hipriv_parse_mld_emlsr_cap::emlsr_transition_delay is error!}\r\n");
    }
    param += off_set;

    oam_warning_log1(0, OAM_SF_ANY, "{wal_hipriv_parse_mld_emlsr_cap::emlsr_omn:0x%x.}", *(uint32_t *)emlsr_cap);
    return OAL_SUCC;
}
uint32_t wal_hipriv_set_emlsr_cap(oal_net_device_stru *net_dev, int8_t *param)
{
    wal_msg_write_stru    write_msg = { 0 };
    hmac_mld_emlsr_cap_stru emlsr_cap = { 0 };
    int32_t ret;

    ret = wal_hipriv_parse_mld_emlsr_cap(net_dev, param, &emlsr_cap);
    if (ret != OAL_SUCC) {
        return ret;
    }
    ret = memcpy_s(write_msg.auc_value, sizeof(write_msg.auc_value),
                   &emlsr_cap, sizeof(emlsr_cap));
    if (ret != EOK) {
        oam_error_log0(0, OAM_SF_ANY, "wal_hipriv_emlsr_enable::memcpy fail!");
        return OAL_FAIL;
    }
    wal_write_msg_hdr_init(&write_msg, WLAN_CFGID_EMLSR_CAP, sizeof(mac_cfg_emlsr_action_param_stru));
    return (uint32_t)wal_send_cfg_event(net_dev, WAL_MSG_TYPE_WRITE,
                                        WAL_MSG_WRITE_MSG_HDR_LENGTH + sizeof(mac_cfg_emlsr_action_param_stru),
                                        (uint8_t*)&write_msg, OAL_FALSE, NULL);
}
uint32_t wal_hipriv_cfg_emlsr_link(oal_net_device_stru *net_dev, int8_t *param)
{
    wal_msg_write_stru    write_msg = { 0 };
    uint8_t emlsr_cfg_link;
    int32_t ret;
    uint32_t off_set = 0;
    char name[WAL_HIPRIV_CMD_NAME_MAX_LEN] = { 0 };
    ...
    emlsr_cfg_link = (uint8_t)oal_atoi(name);
    if (emlsr_cfg_link > 3) { // 3:EMLSR上限
        oam_error_log0(0, OAM_SF_ANY, "{wal_hipriv_cfg_emlsr_link::emlsr_cfg_link is error!}\r\n");
    }
    param += off_set;

    ret = memcpy_s(write_msg.auc_value, sizeof(write_msg.auc_value),
                   &emlsr_cfg_link, sizeof(emlsr_cfg_link));
    if (ret != EOK) {
        oam_error_log0(0, OAM_SF_ANY, "wal_hipriv_cfg_emlsr_link::memcpy fail!");
        return OAL_FAIL;
    }
    wal_write_msg_hdr_init(&write_msg, WLAN_CFGID_EMLSR_CFG_LINK, sizeof(mac_cfg_emlsr_action_param_stru));
    return (uint32_t)wal_send_cfg_event(net_dev, WAL_MSG_TYPE_WRITE,
                                        WAL_MSG_WRITE_MSG_HDR_LENGTH + sizeof(mac_cfg_emlsr_action_param_stru),
                                        (uint8_t*)&write_msg, OAL_FALSE, NULL);
}
OAL_STATIC uint32_t wal_hipriv_parse_al_emlsr(oal_net_device_stru *net_dev, int8_t *param,
    emlsr_al_ctl *emlsr_cfg)
{
    uint32_t off_set = 0;
    char name[WAL_HIPRIV_CMD_NAME_MAX_LEN] = { 0 };
    ...
    emlsr_cfg->emlsr_al_mode = (uint8_t)oal_atoi(name);
    if ((emlsr_cfg->emlsr_al_mode != 0) && (emlsr_cfg->emlsr_al_mode != 1)) {
        oam_error_log0(0, OAM_SF_ANY, "{wal_hipriv_parse_al_emlsr::emlsr_mode is not correct!}\r\n");
    }
    param += off_set;

    /* 获取emlsr emlsr_soft_status */
    if (wal_get_cmd_one_arg(param, name, WAL_HIPRIV_CMD_NAME_MAX_LEN, &off_set) != OAL_SUCC) {
        return OAL_FAIL;
    }
    emlsr_cfg->emlsr_soft_status = (uint8_t)oal_atoi(name);
    if ((emlsr_cfg->emlsr_soft_status != 0) && (emlsr_cfg->emlsr_soft_status != 1)) {
        oam_error_log0(0, OAM_SF_ANY, "{wal_hipriv_parse_al_emlsr::emlsr_soft_status is not correct!}\r\n");
    }
    param += off_set;

    /* 获取emlsr emlsr_statusy */
    if (wal_get_cmd_one_arg(param, name, WAL_HIPRIV_CMD_NAME_MAX_LEN, &off_set) != OAL_SUCC) {
        return OAL_FAIL;
    }
    emlsr_cfg->emlsr_status = (uint8_t)oal_atoi(name);
    param += off_set;

    oam_warning_log1(0, OAM_SF_ANY, "{wal_hipriv_parse_al_emlsr::emlsr_omn:0x%x.}", *(uint32_t *)emlsr_cfg);
    return OAL_SUCC;
}

uint32_t wal_hipriv_al_emlsr_enable(oal_net_device_stru *net_dev, int8_t *param)
{
    wal_msg_write_stru    write_msg = { 0 };
    emlsr_al_ctl emlsr_param = { 0 };
    int32_t ret;

    ret = (int32_t)wal_hipriv_parse_al_emlsr(net_dev, param, &emlsr_param);
    if (ret != OAL_SUCC) {
        return ret;
    }
    ret = memcpy_s(write_msg.auc_value, sizeof(write_msg.auc_value),
                   &emlsr_param, sizeof(emlsr_param));
    if (ret != EOK) {
        oam_error_log0(0, OAM_SF_ANY, "wal_hipriv_al_emlsr_enable::memcpy fail!");
        return OAL_FAIL;
    }
    wal_write_msg_hdr_init(&write_msg, WLAN_CFGID_AL_EMLSR_ENABLE, sizeof(emlsr_al_ctl));
    return (uint32_t)wal_send_cfg_event(net_dev, WAL_MSG_TYPE_WRITE,
                                        WAL_MSG_WRITE_MSG_HDR_LENGTH + sizeof(emlsr_al_ctl),
                                        (uint8_t*)&write_msg, OAL_FALSE, NULL);
}
uint32_t wal_hipriv_cfg_emlsr_dscr(oal_net_device_stru *net_dev, int8_t *param)
{
    wal_msg_write_stru    write_msg = { 0 };
    uint8_t emlsr_cfg_link;
    int32_t ret;
    ...
    emlsr_cfg_link = (uint8_t)oal_atoi(name);
    if (emlsr_cfg_link > 1) { // 只支持0和1
        oam_error_log0(0, OAM_SF_ANY, "{wal_hipriv_cfg_emlsr_dscr::emlsr_cfg_link is error!}\r\n");
    }
    param += off_set;
    ...
    ret = memcpy_s(write_msg.auc_value, sizeof(write_msg.auc_value),
                   &emlsr_cfg_link, sizeof(emlsr_cfg_link));
    if (ret != EOK) {
        oam_error_log0(0, OAM_SF_ANY, "wal_hipriv_cfg_emlsr_dscr::memcpy fail!");
        return OAL_FAIL;
    }
    wal_write_msg_hdr_init(&write_msg, WLAN_CFGID_EMLSR_CFG_SET_DSCR, sizeof(uint8_t));
    return (uint32_t)wal_send_cfg_event(net_dev, WAL_MSG_TYPE_WRITE,
                                        WAL_MSG_WRITE_MSG_HDR_LENGTH + sizeof(uint8_t),
                                        (uint8_t*)&write_msg, OAL_FALSE, NULL);
}
...
#ifdef _PRE_WLAN_FEATURE_WUR
OAL_STATIC uint32_t wal_wur_debug_tx_wur_wakeup(int8_t *param, mac_cfg_wur_debug_param_stru *wur_debug)
{
    uint32_t offset = 0;
    int8_t name[WAL_HIPRIV_CMD_NAME_MAX_LEN] = {0};

    /*
     *  发wake up帧: "wlan0 wur_debug tx_wur_wakeup [mac地址] [0|1](is_short_wakeup)"
    */

    /* 获取mac地址 */
    if (wal_get_cmd_one_arg(param, name, WAL_HIPRIV_CMD_NAME_MAX_LEN, &offset) != OAL_SUCC) {
        return OAL_FAIL;
    }
    oal_strtoaddr(name, sizeof(name), wur_debug->wakeup_params.mac_addr, WLAN_MAC_ADDR_LEN);
    ...
    wur_debug->wakeup_params.is_short_wake_up = (uint8_t)oal_atoi(name);

    wur_debug->cmd_bitmap |= BIT(MAC_TX_WUR_WAKEUP);
    return OAL_SUCC;
}

OAL_STATIC void wal_wur_debug_tx_wur_beacon(int8_t *param, mac_cfg_wur_debug_param_stru *wur_debug)
{
    /*
     *  发wake beacon帧: "wlan0 wur_debug tx_wur_beacon"
    */
    wur_debug->cmd_bitmap |= BIT(MAC_TX_WUR_BEACON);
}

OAL_STATIC uint32_t wal_wur_debug_tx_wur_setup(int8_t *param, mac_cfg_wur_debug_param_stru *wur_debug)
{
    uint32_t offset = 0;
    int8_t name[WAL_HIPRIV_CMD_NAME_MAX_LEN] = {0};

    /*
     *  发wur setup帧: "wlan0 wur_debug tx_wur_setup [mac地址] [is_always_on] [duty cycle period] [service_time ratio]"
    */
    ...
    oal_strtoaddr(name, sizeof(name), wur_debug->setup_params.mac_addr, WLAN_MAC_ADDR_LEN);
    param += offset;
    ...
    wur_debug->setup_params.is_always_on = (uint8_t)oal_atoi(name);
    param += offset;
    ...
    wur_debug->setup_params.count = (uint8_t)oal_atoi(name);
    param += offset;
    ...
    wur_debug->setup_params.ratio = (uint8_t)oal_atoi(name);

    wur_debug->cmd_bitmap |= BIT(MAC_TX_WUR_SETUP);
    return OAL_SUCC;
}

OAL_STATIC uint32_t wal_wur_debug_tx_wur_teardown(int8_t *param, mac_cfg_wur_debug_param_stru *wur_debug)
{
    uint32_t offset = 0;
    int8_t name[WAL_HIPRIV_CMD_NAME_MAX_LEN] = {0};

    /*
     *  发teardown帧: hipriv "wlan0 tx_wur_teardown [mac地址]"
    */
    ...
    oal_strtoaddr(name, sizeof(name), wur_debug->teardown_params.mac_addr, WLAN_MAC_ADDR_LEN);

    wur_debug->cmd_bitmap |= BIT(MAC_TX_WUR_TEARDOWN);
    return OAL_SUCC;
}

OAL_STATIC uint32_t wal_wur_debug_set_fastsleep_param(int8_t *param, mac_cfg_wur_debug_param_stru *wur_debug)
{
    uint32_t offset = 0;
    int8_t name[WAL_HIPRIV_CMD_NAME_MAX_LEN] = {0};

    /*
     *  hipriv.sh "wlan0 wur_debug set_fastsleep_param [pm_timeout] [psm_max_timer_restart_cnt] [host_pm_timeout]"
    */
    ...
    wur_debug->fastsleep_params.pm_timeout = (uint8_t)oal_atoi(name);
    param += offset;
    /* 获取psm_max_timer_restart_cnt */
    if (wal_get_cmd_one_arg(param, name, WAL_HIPRIV_CMD_NAME_MAX_LEN, &offset) != OAL_SUCC) {
        return OAL_FAIL;
    }
    wur_debug->fastsleep_params.psm_max_timer_restart_cnt = (uint8_t)oal_atoi(name);
    param += offset;
    /* 获取host_pm_timeout */
    if (wal_get_cmd_one_arg(param, name, WAL_HIPRIV_CMD_NAME_MAX_LEN, &offset) != OAL_SUCC) {
        return OAL_FAIL;
    }
    wur_debug->fastsleep_params.host_pm_timeout = (uint8_t)oal_atoi(name);

    wur_debug->cmd_bitmap |= BIT(MAC_WUR_FASTSLEEP_PARAM);
    return OAL_SUCC;
}

OAL_STATIC uint32_t wal_wur_debug_set_sleep_mode(int8_t *param, mac_cfg_wur_debug_param_stru *wur_debug)
{
    uint32_t offset = 0;
    int8_t name[WAL_HIPRIV_CMD_NAME_MAX_LEN] = {0};

    /*
     *  hipriv.sh "wlan0 wur_debug set_sleep_mode [0:深睡进wur 1：浅睡进wur]"
    */
    ...
    wur_debug->sleep_mode = (uint32_t)oal_atoi(name);

    wur_debug->cmd_bitmap |= BIT(MAC_WUR_SLEEP_MODE);
    return OAL_SUCC;
}

OAL_STATIC uint32_t wal_hipriv_wur_debug_param_prepare(mac_cfg_wur_debug_param_stru *wur_debug, int8_t *param)
{
    uint32_t offset = 0;
    int8_t name[WAL_HIPRIV_CMD_NAME_MAX_LEN] = {0};
    uint32_t ret;
    ...
    /* 命令分类 */
    if (oal_strcmp("tx_wur_wakeup", name) == 0) {
        wal_wur_debug_tx_wur_wakeup(param, wur_debug);
    } else if (oal_strcmp("tx_wur_beacon", name) == 0) {
        wal_wur_debug_tx_wur_beacon(param, wur_debug);
    } else if (oal_strcmp("tx_wur_setup", name) == 0) {
        wal_wur_debug_tx_wur_setup(param, wur_debug);
    } else if (oal_strcmp("tx_wur_teardown", name) == 0) {
        wal_wur_debug_tx_wur_teardown(param, wur_debug);
    } else if (oal_strcmp("set_fastsleep_param", name) == 0) {
        wal_wur_debug_set_fastsleep_param(param, wur_debug);
    } else if (oal_strcmp("set_sleep_mode", name) == 0) {
        wal_wur_debug_set_sleep_mode(param, wur_debug);
    } else {
        oam_error_log0(0, OAM_SF_ANY, "wal_hipriv_wur_debug_param_prepare::wrong cmd!");
    }

    return OAL_SUCC;
}

uint32_t wal_hipriv_wur_debug_process(oal_net_device_stru *net_dev, int8_t *param)
{
    wal_msg_write_stru write_msg = {0};
    mac_cfg_wur_debug_param_stru wur_debug;
    uint32_t ret;

    memset_s(&wur_debug, sizeof(mac_cfg_wur_debug_param_stru), 0, sizeof(mac_cfg_wur_debug_param_stru));

    ret = wal_hipriv_wur_debug_param_prepare(&wur_debug, param);
    if (ret != OAL_SUCC) {
        return ret;
    }
    oam_warning_log1(0, OAM_SF_ANY, "{wal_hipriv_wur_debug_process::cmd_bitmap: 0x%08x.}", wur_debug.cmd_bitmap);

    /* 设置配置命令参数 */
    ret = (uint32_t)memcpy_s(write_msg.auc_value, sizeof(write_msg.auc_value),
                   &wur_debug, sizeof(mac_cfg_wur_debug_param_stru));
    if (ret != EOK) {
        oam_error_log0(0, OAM_SF_ANY, "wal_hipriv_tx_wur_wakeup::memcpy fail!");
        return OAL_FAIL;
    }

    /* 抛事件到wal层处理 */
    wal_write_msg_hdr_init(&write_msg, WLAN_CFGID_WUR_DEBUG, sizeof(mac_cfg_wur_debug_param_stru));
    return (uint32_t)wal_send_cfg_event(net_dev, WAL_MSG_TYPE_WRITE,
                                        WAL_MSG_WRITE_MSG_HDR_LENGTH + sizeof(mac_cfg_wur_debug_param_stru),
                                        (uint8_t *)&write_msg, OAL_FALSE, NULL);
}
#endif
#ifdef _PRE_WLAN_FEATURE_11AX
uint32_t wal_hipriv_omi_bw_switch_cfg(oal_net_device_stru *net_dev, int8_t *param)
{
    wal_msg_write_stru write_msg = {0};
    uint32_t offset;
    int8_t ac_name[WAL_HIPRIV_CMD_NAME_MAX_LEN] = {0};
    int32_t ret;
    if (wal_get_cmd_one_arg(param, ac_name, WAL_HIPRIV_CMD_NAME_MAX_LEN, &offset) != OAL_SUCC) {
        return OAL_FAIL;
    }
    /* 第一个参数 获取开关 */
    write_msg.auc_value[0] = (uint8_t)oal_atoi(ac_name);
    param += offset;
    ret = (int32_t)wal_get_cmd_one_arg(param, ac_name, WAL_HIPRIV_CMD_NAME_MAX_LEN, &offset);
    if (ret == OAL_SUCC) {
        /* 第二个参数 获取带宽 */
    ...
    if (ret != OAL_SUCC) {
        oam_error_log1(0, OAM_SF_ANY, "{wal_hipriv_omi_bw_switch_cfg::return err code[%d]!}", ret);
        return OAL_FAIL;
    }
