...
OAL_STATIC uint32_t wal_get_cmd_netdev(int8_t *cmd, oal_net_device_stru **netdev, uint32_t *offset)
{
    ...
    /* 根据dev_name找到dev */
    temp_netdev = wal_config_get_cmd_netdev(dev_name, OAL_STRLEN(dev_name));
    if (temp_netdev == NULL) {
...
/* MLD工作模式切换 */
const int32_t g_mld_mode_switch_table[] = {
    /* MLD模式切换枚举 mac_mld_mode_switch_enum
     * 0 : 保留
     * 1 : MLSR
     * 2 : EMLSR
     * 3 : STR
...
host_cmd_stru g_host_cmd_table[] = {
    ...
    { "set_fastsleep3_para", 5, NULL, NULL, hmac_package_fastsleep3_params, OAL_TRUE, WLAN_CFGID_SET_FASTSLEEP3_PARA },
    /* 设置PSPOLL能力 sh hipriv.sh 'wlan0 set_ps_mode 3' */
    { "set_ps_mode", 1, NULL, hmac_set_sta_ps_mode, hmac_package_sta_ps_mode_params, OAL_TRUE, WLAN_CFGID_SET_PS_MODE },
    ...
    /* 配置使能elp scan，上层下发后，后续扫描由elp scan接管 */
    { "elp_scan_enable", 1, g_switch_check_table, wal_set_elp_scan_type, NULL, OAL_FALSE, 0 },
#endif
    ...
    /* 调整LP唤醒门限: sh hipriv.sh 'wlan0 lp_wakeup_th xx(xx:门限，十六进制) */
    { "lp_wakeup_th", 1, NULL, NULL, hmac_package_uint8_params, OAL_TRUE, WLAN_CFGID_SET_LP_WAKEUP_TH },
    /* 配置硬化常收: sh hipriv.sh 'wlan0 lp_eg_alrx xx(依次下发1/2/3) */
    { "lp_eg_alrx", 1, NULL, NULL, hmac_package_uint8_params, OAL_TRUE, WLAN_CFGID_SET_LP_EG_ALRX },
    /* 配置硬化常收: sh hipriv.sh 'wlan0 hw_pm_switch xx(0/1/xxx) */
    { "hw_pm_switch", 1, NULL, NULL, hmac_package_uint8_params, OAL_TRUE, WLAN_CFGID_SET_HW_PM_SWITCH },
    ...
    { "phy_event", 6, g_phy_event_rpt_check_table, wal_hipriv_phy_event_rpt, hmac_phy_event_rpt_params,
    ...
    /* 配置uwbcoex的测试命令当前只有mp16c使用 hipriv.sh "wlan0 uwbcoex_msg value" */
    ...
    { "skip_cur_cac", 0, NULL, wal_hipriv_skip_cur_cac, NULL, OAL_FALSE, 0 },
#endif
    ...
    { "chba_service_ask_user_all_awake", 2, NULL, wal_hipriv_chba_service_ask_user_all_awake, NULL, OAL_FALSE, 0},
    ...
    { "vsp_nav_edca", 4, NULL, NULL, hmac_package_vsp_params, OAL_TRUE, WLAN_CFGID_VSP_NAV_EDCA },
#endif
    ...
    /* 开关CF-END和long_nav_enable: sh hipriv.sh "wlan0 cf_end_enable 0|1"(1:使能 0:关闭) */
    { "cf_end_enable", 1, g_switch_check_table, NULL, hmac_package_uint8_params, OAL_TRUE, WLAN_CFGID_CF_END_ENABLE },
#ifdef _PRE_WLAN_FEATURE_TX_STAT
    /* 配置打开tx统计日志: sh hipriv.sh 'wlan0 tx_stats_log 0/1(0不使能，1使能) [100-10000] 设置上报周期参数 */
    { "tx_stat_log", 2, NULL, hmac_tx_stats_set_log_on, NULL, OAL_FALSE, 0 },
#endif
#ifdef _PRE_WLAN_FEATURE_ISOLATION
    /* 配置AP隔离：sh hipriv.sh "wlan0 set_isolation val[bit0:广播 | bit1:组播 | bit2:单播]" */
    { "set_isolation", 1, NULL, hmac_hipriv_set_isolation, NULL, OAL_FALSE, 0 },
#endif
#ifdef _PRE_WLAN_FEATURE_ML
    { "ml_scan", 1, NULL, hmac_set_hipriv_ml_scan, NULL, OAL_FALSE, 0 },
    /* 配置MLD 模式切换：sh hipriv.sh "wlan0 mld_mode_switch ml_mode"
     * ml_mode[0:输出调试信息 1:MLSR 2:EMLSR 3:STR]
     */
    { "mld_mode_switch", 1, g_mld_mode_switch_table, hmac_config_mld_mode_switch_cmd, NULL, OAL_FALSE, 0 },
#endif
#ifdef _PRE_WLAN_FEATURE_QUERY_INFO
    /* E5查询vap信息：sh hipriv.sh "wlan0 query_vap_info" */
    { "query_vap_info", 0, NULL, hmac_hipriv_query_vap_info, NULL, OAL_FALSE, 0 },
#endif
#ifdef _PRE_WLAN_FEATURE_WUR
    /* 打开wur维测：sh hipriv.sh "wlan0 wur_debug_info 1" */
    { "wur_debug_info", 1, NULL, NULL, hmac_package_uint8_params, OAL_TRUE, WLAN_CFGID_WUR_DEBUG_INFO },
#endif
    { "roam_set_auth_mode", 1, NULL, hmac_roam_set_roam_auth_mode, NULL, OAL_FALSE, 0 },
    /* 参数：0不延迟 1延迟 */
    { "ssi_delay_switch", 1, NULL, NULL, hmac_package_uint8_params, OAL_TRUE, WLAN_CFGID_SSI_DELAY_SWITCH_EVENT },
    { "ai_csma_tx_stats_debug", 1, NULL, hmac_set_ai_csma_tx_stats_debug, NULL, OAL_FALSE, 0 },
    /* 屏蔽寄存器打印 hipriv.sh "Hisilicon0 reginfo_mask [mask_bitmap]"
       mask_bitmap为0时不屏蔽寄存器打印, bit0置1屏蔽mac打印, bit1置1屏蔽phy打印, bit2置1屏蔽rf打印, bit3置1屏蔽soc打印 */
    { "reginfo_mask", 1, NULL, NULL, hmac_package_uint8_params, OAL_TRUE, WLAN_CFGID_SE_REGINFO_MASK },
