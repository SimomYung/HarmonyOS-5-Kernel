...
#ifdef _PRE_WLAN_CHBA_MGMT
/* 功能描述: 维测上库, ota开启时打开chba log level，问题定位后删除 */
static void wal_start_chba_log_with_ota(oal_switch_enum_uint8 all_ota_switch)
{
    ...
    chba_log_level = ((all_ota_switch == OAL_SWITCH_ON) ? CHBA_DEBUG_ALL_LOG : CHBA_DEBUG_NO_LOG);
    hmac_config_chba_log_level(&hmac_vap->st_vap_base_info, sizeof(chba_log_level), &chba_log_level);
}
#endif
...
#ifdef _PRE_WLAN_CHBA_MGMT
    /* 维测上库定位chba收发问题, 在ota开启的同时开启chba维测, 问题定位后删除 */
    wal_start_chba_log_with_ota(all_ota_switch);
#endif
    return OAL_SUCC;
}
...
uint32_t wal_hipriv_get_station_info(oal_net_device_stru *net_dev, int8_t *param)
{
    ...
    ********************************************************************************/
    ...
    leftime = oal_wait_event_interruptible_timeout_m(hmac_vap->query_wait_q,
        (hmac_vap->station_info_query_completed_flag == OAL_TRUE), 5 * OAL_TIME_HZ); /* 5s超时 */
    if (leftime <= 0) {
...
#ifdef _PRE_PLAT_FEATURE_CUSTOMIZE
    if (oal_strcmp("Hisilicon0", net_dev->name) == 0) {
...
uint8_t wal_hipriv_set_str_get_map_index(int8_t *name, uint8_t name_len)
{
    ...
    for (map_index = 0; map_index < g_set_str_table_size; map_index++) {
        ...
        if (oal_strcmp(cmd_table.pc_name, name) == 0) {
            return map_index;
        }
    }
    ...
    mac_psm_query_type_enum_uint8 en_query_type, wal_wifi_priv_cmd_stru *priv_cmd)
{
    ...
    wal_msg_write_stru write_msg = {0};
    hmac_psm_flt_stat_query_stru *hmac_psm_query = NULL;
    mac_psm_query_stat_stru *psm_stat = NULL;
    int32_t ret;
    ...
    hmac_psm_query = &hmac_device->st_psm_flt_stat_query;
    hmac_psm_query->auc_complete_flag[en_query_type] = OAL_FALSE;
    ...
    *(mac_psm_query_type_enum_uint8*)(write_msg.auc_value) = en_query_type;
    ...
    /* 等待查询返回 */
    ret = wal_psm_query_wait_complete(hmac_psm_query, en_query_type);
    /* 超时或异常 */
    if (ret <= 0) {
    ...
    psm_stat = &hmac_psm_query->ast_psm_stat[en_query_type];
    if ((psm_stat->query_item > MAC_PSM_QUERY_MSG_MAX_STAT_ITEM) ||
        (priv_cmd->total_len < (psm_stat->query_item * sizeof(uint32_t)))) {
        oam_error_log2(0, OAM_SF_CFG, "wal_ioctl_get_psm_stat:total_len=%d, query_item=%d",
            priv_cmd->total_len, psm_stat->query_item);
        return OAL_FAIL;
    }

    if (oal_copy_to_user(priv_cmd->puc_buf, psm_stat->aul_val, psm_stat->query_item * sizeof(uint32_t)) != EOK) {
        oam_error_log0(0, OAM_SF_CFG, "{wal_ioctl_get_psm_stat::Failed to copy ioctl_data to user !}");
        return OAL_FAIL;
    }

    priv_cmd->used_len = psm_stat->query_item * sizeof(uint32_t);
    return OAL_SUCC;
}
    ...
    oal_iw_request_info_stru *pst_info, oal_iwreq_data_union *pst_wrqu, char *pc_extra)
{
    ...
    if (memcpy_s(pst_wrqu->name, OAL_IF_NAME_SIZE, ac_iwname, sizeof(ac_iwname)) != EOK) {
    ...
    oal_iwreq_data_union *pst_data, char *pc_ssid)
{
    ...
    if (memcpy_s(pc_ssid, pst_essid->length, pst_ssid->ac_ssid, pst_ssid->uc_ssid_len) != EOK) {
    ...
    switch (event) {
        case NETDEV_UP: {
            ...
            if (mac_vap->en_vap_mode == WLAN_VAP_MODE_BSS_STA) {
                ...
                /* 获取到的时候通知漫游计时 */
                hmac_roam_wpas_connect_state_notify(pst_hmac_vap, WPAS_CONNECT_STATE_IPADDR_OBTAINED, OAL_TRUE);
            }
            break;
        }
        ...
        case NETDEV_DOWN: {
            ...
            if (mac_vap->en_vap_mode == WLAN_VAP_MODE_BSS_STA) {
                ...
                hmac_roam_wpas_connect_state_notify(pst_hmac_vap, WPAS_CONNECT_STATE_IPADDR_REMOVED, OAL_FALSE);
            }
            break;
        }
...
uint32_t hmac_hipriv_query_vap_info(mac_vap_stru *mac_vap, uint32_t *param)
{
    ...
    dpe_wlan_vap_stat_stru vap_stats = {0};
    uint32_t ret;
    if (oal_any_null_ptr2(mac_vap, param)) {
        return OAL_FAIL;
    }
    hmac_vap = (hmac_vap_stru *)mac_res_get_hmac_vap(mac_vap->uc_vap_id);
    ret = dpe_wlan_vap_stat_sync(mac_vap->uc_vap_id, (uint8_t *)(&vap_stats), sizeof(dpe_wlan_vap_stat_stru));
    if (hmac_vap == NULL || ret != OAL_SUCC) {
        oam_warning_log0(0, OAM_SF_CFG, "{hmac_hipriv_query_vap_info::hmac_vap is null.}");
        return OAL_FAIL;
    }
    /* 打印vap信息 */
    oam_warning_log4(mac_vap->uc_vap_id, 0, "hmac_hipriv_query_vap_info:user_cnt[%d], vap_mac[%02X:***:%02X:%02X]",
        mac_vap->us_user_nums, mac_vap->auc_bssid[BYTE_OFFSET_0],
        mac_vap->auc_bssid[BYTE_OFFSET_4], mac_vap->auc_bssid[BYTE_OFFSET_5]);
    oam_warning_log4(mac_vap->uc_vap_id, 0, "hmac_hipriv_query_vap_info:chan[%d] bw[%d] tx_bytes[%llu] rx_bytes[%llu]",
        mac_vap->st_channel.uc_chan_number, mac_vap->st_channel.en_bandwidth, vap_stats.tx_bytes, vap_stats.rx_bytes);
    oam_warning_log4(mac_vap->uc_vap_id, 0, "hmac_hipriv_query_vap_info:tx_packets[%llu] tx_failed[%llu] \
        nrx_packets[%llu] rx_dropped[%llu]", vap_stats.tx_packets, vap_stats.tx_failed,
        vap_stats.rx_packets, vap_stats.rx_dropped_misc);
    /* 打印user信息 */
    oal_dlist_search_for_each(entry, &(mac_vap->st_mac_user_list_head))
    {
        ...
        oam_warning_log4(mac_vap->uc_vap_id, 0, "user_info:user_idx[%d] user_mac[%02X:***:%02X:%02X]",
            mac_user->us_assoc_id, mac_user->auc_user_mac_addr[BYTE_OFFSET_0],
            mac_user->auc_user_mac_addr[BYTE_OFFSET_4], mac_user->auc_user_mac_addr[BYTE_OFFSET_5]);
        oam_warning_log4(mac_vap->uc_vap_id, 0, "user_info:state[%d] band_w[%d] nss[%d] rssi[%d]",
            mac_user->en_user_asoc_state, mac_user->en_bandwidth_cap, mac_user->en_user_max_cap_nss, hmac_user->c_rssi);
        oam_warning_log4(mac_vap->uc_vap_id, 0, "user_info:protocol[%d] tx_rate_max[%d] tx_rate[%d] online_time[%d ms]",
            mac_user->en_protocol_mode, hmac_user->tx_rate_max, hmac_user->tx_rate,
            (uint32_t)oal_time_get_stamp_ms() - hmac_user->first_add_time);
        oam_warning_log3(mac_vap->uc_vap_id, 0, "user_info:rx_rate_min[%d] rx_rate_max[%d] rx_rate[%d]",
            hmac_user->rx_rate_min, hmac_user->rx_rate_max, hmac_user->rx_rate);
        oam_warning_log4(mac_vap->uc_vap_id, 0, "user_info:tx_packets[%d] tx_failed[%d] rx_packets[%d] tx_dropped[%d]",
            user_stats.tx_packets, user_stats.tx_failed, user_stats.rx_packets, user_stats.rx_dropped_misc);
        oam_warning_log3(mac_vap->uc_vap_id, 0, "user_info:tx_bytes[%d] rx_bytes[%d] tx_retries[%d]",
            user_stats.tx_bytes, user_stats.rx_bytes, user_stats.tx_retries);
    }
    return OAL_SUCC;
}
#endif
