...
uint32_t wal_set_stbc_cap(mac_vap_stru *mac_vap, uint32_t *params)
{
    ...
    oal_bool_enum_uint8 tx_stbc_cap = cap_value;
    ...
    if (mac_vap->en_vap_rx_nss == WLAN_SINGLE_NSS) {
        tx_stbc_cap = OAL_FALSE;
    }
    mac_dev->device_cap->en_rx_stbc_is_supp = cap_value;
    mac_dev->device_cap->en_tx_stbc_is_supp = tx_stbc_cap;

    mac_mib_set_TxSTBCOptionImplemented(mac_vap, tx_stbc_cap);
    mac_mib_set_TxSTBCOptionActivated(mac_vap, tx_stbc_cap);
    mac_mib_set_VHTTxSTBCOptionImplemented(mac_vap, tx_stbc_cap);
    mac_mib_set_he_STBCTxBelow80M(mac_vap, tx_stbc_cap);
    ...
    oam_warning_log2(mac_vap->uc_vap_id, OAM_SF_CFG, "{wal_set_stbc_cap::set stbc cap TX[%d] RX[%d].}",
        tx_stbc_cap, cap_value);
    return OAL_SUCC;
}
/*
...
uint32_t wal_set_ldpc_cap(mac_vap_stru *mac_vap, uint32_t *params)
{
    ...
    mac_dev->device_cap->en_ldpc_is_supp = cap_value;

    mac_mib_set_LDPCCodingOptionImplemented(mac_vap, mac_device_get_cap_ldpc(mac_dev));
    mac_mib_set_LDPCCodingOptionActivated(mac_vap, mac_device_get_cap_ldpc(mac_dev));
    mac_mib_set_VHTLDPCCodingOptionImplemented(mac_vap, mac_device_get_cap_ldpc(mac_dev));
#ifdef _PRE_WLAN_FEATURE_11AX
    if (g_wlan_spec_cfg->feature_11ax_is_open) {
        mac_mib_set_he_LDPCCodingInPayload(mac_vap, mac_device_get_cap_ldpc(mac_dev));
    }
#endif

    oam_warning_log1(0, 0, "{wal_set_ldpc_cap::cap_value[%d]}", cap_value);
...
uint32_t wal_set_txbf_cap(mac_vap_stru *mac_vap, uint32_t *params)
{
    ...
    *************************************************************/
    ...
    rx_sts_num = (rx_switch == OAL_TRUE) ? g_wlan_spec_cfg->bfee_ntx_supports : 1;
    mac_device->device_cap->en_su_bfmee_is_supp = rx_switch;
    mac_device->device_cap->en_su_bfmer_is_supp = tx_switch;

    mac_mib_set_VHTSUBeamformerOptionImplemented(mac_vap, mac_device_get_cap_subfer(mac_device));
    mac_mib_set_VHTSUBeamformeeOptionImplemented(mac_vap, mac_device_get_cap_subfee(mac_device));
#ifdef _PRE_WLAN_FEATURE_11AX
    mac_mib_set_he_SUBeamformer(mac_vap, mac_device_get_cap_subfer(mac_device));
    mac_mib_set_he_SUBeamformee(mac_vap, mac_device_get_cap_subfee(mac_device));

    mac_mib_set_HENg16SUFeedback(mac_vap, rx_switch);
    mac_mib_set_HENg16MUFeedback(mac_vap, rx_switch);
    mac_mib_set_HECodebook75MUFeedback(mac_vap, rx_switch);
#endif
#ifdef _PRE_WLAN_FEATURE_11BE
    mac_mib_set_EHTSUBeamformerImplemented(mac_vap->pst_mib_info, mac_device_get_cap_subfer(mac_device));
    mac_mib_set_EHTSUBeamformeeImplemented(mac_vap->pst_mib_info, mac_device_get_cap_subfee(mac_device));
#endif
...
uint32_t wal_hipriv_tcp_ack_buf_cfg(oal_net_device_stru *net_dev, int8_t *pc_param)
{
    ...
    for (uc_idx = 0; uc_idx < MAC_TCP_ACK_BUF_TYPE_BUTT; uc_idx++) {
        if (oal_strcmp(ac_name, g_ast_hmac_tcp_ack_buf_cfg_table[uc_idx].puc_string) == 0) {
            break;
        }
    }
...
{
    return wlan_chip_always_tx_write_msg(write_msg, en_tx_flag, en_payload_flag, len);
}
/*
...
uint32_t wal_hipriv_always_tx(oal_net_device_stru *net_dev, int8_t *pc_param)
{
    ...
    ret = (uint32_t)wal_send_cfg_event(net_dev, WAL_MSG_TYPE_WRITE,
        WAL_MSG_WRITE_MSG_HDR_LENGTH + write_msg.us_len, (uint8_t *)&write_msg, OAL_TRUE, &rsp_msg);
    if (oal_unlikely(ret != OAL_SUCC)) {
...
const int8_t *g_ru_size_tbl[WLAN_RU_SIZE_BUTT] = {
    "26tone",
    "52tone",
    "106tone",
    "242tone",
...
uint32_t wal_hipriv_set_ru_index(oal_net_device_stru *net_dev, int8_t *param)
{
    ...
    uint32_t off_set = 0;
    uint32_t ret;
    mac_cfg_ru_idx_stru *set_param = NULL;
    int8_t name[WAL_HIPRIV_CMD_NAME_MAX_LEN] = {0};
    wal_msg_stru *rsp_msg = NULL;

    /* 抛事件到wal层处理 */
    wal_write_msg_hdr_init(&write_msg, WLAN_CFGID_SET_RU_INDEX, sizeof(mac_cfg_ru_idx_stru));
    /* 解析并设置配置命令参数 */
    set_param = (mac_cfg_ru_idx_stru *)(write_msg.auc_value);
    ...
    for (set_param->ru_size = 0; set_param->ru_size < WLAN_RU_SIZE_BUTT; set_param->ru_size++) {
        if (!oal_strcmp(g_ru_size_tbl[set_param->ru_size], name)) {
            break;
        }
    }
    /* 检查命令是否打错 */
    if (set_param->ru_size >= WLAN_RU_SIZE_BUTT) {
    ...
    set_param->ru_index = (uint8_t)oal_atoi(name);
    ...
    set_param->ru_allocation_b0 = (uint8_t)oal_atoi(name);

    ret = (uint32_t)wal_send_cfg_event(net_dev, WAL_MSG_TYPE_WRITE, WAL_MSG_WRITE_MSG_HDR_LENGTH +
        sizeof(mac_cfg_ru_idx_stru), (uint8_t *)&write_msg, OAL_TRUE, &rsp_msg);
    if (oal_unlikely(ret != OAL_SUCC)) {
 ...
 *   修改内容  : 新生成函数
 */
uint32_t wal_hipriv_set_phase_incr(oal_net_device_stru *net_dev, int8_t *param)
{
    wal_msg_write_stru write_msg = {0};
    uint32_t off_set = 0;
    uint32_t ret;
    mac_cfg_phase_incr_stru *set_param = NULL;
    int8_t name[WAL_HIPRIV_CMD_NAME_MAX_LEN] = {0};
    wal_msg_stru *rsp_msg = NULL;

    /* 抛事件到wal层处理 */
    wal_write_msg_hdr_init(&write_msg, WLAN_CFGID_SET_PHASE_INCR, sizeof(mac_cfg_phase_incr_stru));
    /* 解析并设置配置命令参数 */
    set_param = (mac_cfg_phase_incr_stru *)(write_msg.auc_value);

    ret = wal_get_cmd_one_arg(param, name, WAL_HIPRIV_CMD_NAME_MAX_LEN, &off_set);
    if (ret != OAL_SUCC) {
        oam_warning_log1(0, OAM_SF_ANY, "{wal_hipriv_set_phase_incr::get cmd one arg err[%d]!}", ret);
        return ret;
    }
    set_param->phase_incr_en = (uint8_t)oal_atoi(name);
    /* 检查命令是否打错 */
    if (set_param->phase_incr_en > 1) {
        oam_warning_log0(0, OAM_SF_ANY, "{wal_hipriv_set_phase_incr::invalid phase_incr_en!}");
        return OAL_ERR_CODE_INVALID_CONFIG;
    }
    ...
    set_param->phase_incr = (uint16_t)oal_atoi(name);

    ret = (uint32_t)wal_send_cfg_event(net_dev, WAL_MSG_TYPE_WRITE, WAL_MSG_WRITE_MSG_HDR_LENGTH +
        sizeof(mac_cfg_phase_incr_stru), (uint8_t *)&write_msg, OAL_TRUE, &rsp_msg);
    if (oal_unlikely(ret != OAL_SUCC)) {
        oam_warning_log1(0, OAM_SF_ANY, "{wal_hipriv_set_phase_incr::err [%d]!}", ret);
        return ret;
    }
    ...
    ret = wal_check_and_release_msg_resp(rsp_msg);
    if (ret != OAL_SUCC) {
        oam_warning_log1(0, OAM_SF_DFR, "{wal_hipriv_set_phase_incr fail, err code[%u]!}", ret);
        return ret;
    }
...
uint32_t _wal_hipriv_set_mcsax_er(oal_net_device_stru *net_dev, int8_t *param)
{
    if (g_wlan_spec_cfg->feature_11ax_er_su_dcm_is_open) {
        return wal_hipriv_set_mcsax_er(net_dev, param);
    }
...
uint32_t wal_hipriv_pkt_time_switch(oal_net_device_stru *net_dev, int8_t *pc_param)
{
    ...
    } else if ((oal_strcmp("off", ac_arg)) == 0) {
