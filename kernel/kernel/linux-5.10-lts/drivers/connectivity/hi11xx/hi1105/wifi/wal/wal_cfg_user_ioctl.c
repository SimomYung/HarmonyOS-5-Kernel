                                                ...
                                                int8_t *ac_ssid, uint32_t max_len)
{
    ...
    if (memcpy_s(param->ac_ssid, sizeof(param->ac_ssid), ssid, ssid_len) != EOK) {
...
OAL_STATIC uint8_t wal_hipriv_roam_start_parse_bss_ignore(uint32_t cur_bss_ignore_param, uint8_t *ingnore_cur_mld_bss)
{
    // roam_start 4 0或4 1，0表示不漫游到当前bss，1表示漫游到当前bss
    uint8_t ignore_cur_bss = ROAM_IGNORE_CUR_BSS;
    *ingnore_cur_mld_bss = ROAM_SAME_MLD_LINK_IGNORE;
    switch (cur_bss_ignore_param) {
        case 0:
            ignore_cur_bss = ROAM_IGNORE_CUR_BSS;
            break;
        case 1:
            ignore_cur_bss = ROAM_ONLY_CUR_BSS;
            break;
        case ROAM_ONLY_SWITCH_MLD_ANOTHER_LINK:
            ignore_cur_bss = ROAM_IGNORE_CUR_BSS;
            *ingnore_cur_mld_bss = ROAM_ONLY_SAME_MLD_LINK_SWITCH;
            break;
        case ROAM_ONLY_REASSOC_MLD_ANOTHER_LINK:
            ignore_cur_bss = ROAM_IGNORE_CUR_BSS;
            *ingnore_cur_mld_bss = ROAM_ONLY_SAME_MLD_LINK_REASSOC;
            break;
        default:
            break;
    }

    return ignore_cur_bss;
}

static void wal_hipriv_set_roam_start_params(mac_cfg_set_roam_start_stru *roam_start, uint8_t scan_type,
    oal_bool_enum_uint8 current_bss_policy, oal_bool_enum_uint8 trigged_by_cmd, uint8_t cur_mld_link_bss_policy)
{
    roam_start->uc_scan_type = scan_type;
    roam_start->en_current_bss_ignore = current_bss_policy;
    roam_start->is_roaming_trigged_by_cmd = trigged_by_cmd;
    roam_start->cur_mld_link_bss_policy = cur_mld_link_bss_policy;
}
...
OAL_STATIC uint32_t wal_hipriv_roam_start_para_prepare(wal_msg_write_stru *write_msg, int8_t *param)
{
    ...
    oal_bool_enum_uint8 current_bss_policy = ROAM_IGNORE_CUR_BSS;
    oal_bool_enum_uint8 trigged_by_cmd = OAL_FALSE;
    ...
    int8_t ac_name[WAL_HIPRIV_CMD_NAME_MAX_LEN] = {0};
    uint8_t cur_mld_bss_policy = ROAM_SAME_MLD_LINK_IGNORE;
    ...
    ret = wal_get_cmd_one_arg(param, ac_name, WAL_HIPRIV_CMD_NAME_MAX_LEN, &off_set);
    if (ret == OAL_SUCC) {
        current_bss_policy = wal_hipriv_roam_start_parse_bss_ignore((uint32_t)oal_atoi(ac_name), &cur_mld_bss_policy);
        param += off_set;
    } else if (ret == OAL_ERR_CODE_CONFIG_ARGS_OVER) {
        memset_s(bssid, sizeof(bssid), 0, sizeof(bssid));
    } else {
    ...
    if (ret == OAL_SUCC) {
    /* 漫游是否由命令触发 */
        trigged_by_cmd = (uint8_t)oal_atoi(ac_name);
        param += off_set;
    } else if (ret == OAL_ERR_CODE_CONFIG_ARGS_OVER) {
        memset_s(bssid, sizeof(bssid), 0, sizeof(bssid));
    } else {
    ...
    /***************************************************************************
                                抛事件到wal层处理
    ***************************************************************************/
    wal_hipriv_set_roam_start_params(roam_start, scan_type, current_bss_policy, trigged_by_cmd, cur_mld_bss_policy);
