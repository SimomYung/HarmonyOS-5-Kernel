    ...
    wlan_vap_mode_enum_uint8 *vap_mode, wlan_p2p_mode_enum_uint8 *p2p_mode)
{
    ...
    /* 根据dev_name找到dev */
    net_dev = wal_config_get_cmd_netdev(ac_name, OAL_STRLEN(ac_name));
    if (net_dev != NULL) {
    ...
    wlan_p2p_mode_enum_uint8 p2p_mode)
{
    ...
    if (oal_unlikely(result != OAL_SUCC)) {
        ...
        if (wal_check_and_release_msg_resp(rsp_msg) == OAL_SUCC) {
            if (wal_del_vap_try_wlan_pm_close(wal_get_net_device_id(net_dev)) != OAL_SUCC) {
...
uint32_t wal_hipriv_del_vap(oal_net_device_stru *net_dev, int8_t *param)
{
    ...
    if (oal_unlikely((OAL_IFF_RUNNING & oal_netdevice_flags(net_dev)) != 0)) {
...
int32_t wal_set_random_mac_to_mib(oal_net_device_stru *net_dev)
{
    ...
    switch (net_dev->ieee80211_ptr->iftype) { /* 对于P2P 场景，p2p0 和 p2p-p2p0 MAC 地址从wlan0 获取 */
        ...
        case NL80211_IFTYPE_P2P_GO:
            ...
            break;
        default:
            if ((oal_strcmp("p2p0", net_dev->name)) == 0) {
                en_p2p_mode = WLAN_P2P_DEV_MODE;
                /* 产生P2P device MAC 地址，将本地mac 地址bit 设置为1 */
                ret += memcpy_s(auc_p2p0_addr, WLAN_MAC_ADDR_LEN, oal_netdevice_mac_addr(net_dev), WLAN_MAC_ADDR_LEN);
                break;
                                                  ...
                                                  oal_wireless_dev_stru *pst_wdev, oal_net_device_stru *net_dev)
{
    ...
    int8_t ac_link_netdev_name[MAC_NET_DEVICE_NAME_LENGTH];
    char *wlan_netdev_name = ac_wlan_netdev_name;
    char *p2p_netdev_name = ac_p2p_netdev_name;
    char *link_netdev_name = ac_link_netdev_name;
    ...
    if (snprintf_s(link_netdev_name, NET_DEV_NAME_LEN, NET_DEV_NAME_LEN - 1, "link") < 0) {
        oam_warning_log0(0, OAM_SF_ANY, "{wal_wlan_mac_cfg_add_vap_param_set::snprintf_s fail!}");
    }
    ...
    if ((pst_wdev->iftype == NL80211_IFTYPE_STATION) || (pst_wdev->iftype == NL80211_IFTYPE_P2P_DEVICE)) {
        if (oal_any_zero_value3(oal_strcmp(wlan_netdev_name, net_dev->name),
            oal_strcmp(WLAN1_NETDEV_NAME, net_dev->name), oal_strcmp(LINK_NETDEV_NAME, net_dev->name))) {
            add_vap_param->en_vap_mode = WLAN_VAP_MODE_BSS_STA;
        } else if ((oal_strcmp(p2p_netdev_name, net_dev->name)) == 0) {
            add_vap_param->en_vap_mode = WLAN_VAP_MODE_BSS_STA;
    ...
    if (oal_strcmp(wlan_netdev_name, net_dev->name) == 0 || oal_strcmp(link_netdev_name, net_dev->name) == 0) {
...
#ifdef _PRE_WLAN_CHBA_MGMT
static void wal_chba_add_vap_set_mode(oal_net_device_stru *net_dev, mac_cfg_add_vap_param_stru *add_vap_param)
{
    ...
    if ((g_optimized_feature_switch_bitmap & BIT(CUSTOM_OPTIMIZE_CHBA)) &&
        ...
        if ((g_expand_feature_switch_bitmap & CUSTOM_NBW_CAP) != 0) {
            add_vap_param->bit_is_nbw_mode = OAL_TRUE;
        }
#endif
    }
}
...
{
#ifdef _PRE_WLAN_CHBA_MGMT
    wal_chba_add_vap_set_mode(net_dev, add_vap_param);
#endif
    /* 抛事件到wal层处理 */
    ...
    wlan_p2p_mode_enum_uint8 en_p2p_mode)
{
    ...
    hmac_set_power_off_ready(OAL_FALSE);
    wal_write_msg_hdr_init(&write_msg, WLAN_CFGID_DEL_VAP, sizeof(mac_cfg_del_vap_param_stru));
    ret = wal_send_cfg_event(net_dev, WAL_MSG_TYPE_WRITE,
        WAL_MSG_WRITE_MSG_HDR_LENGTH + sizeof(mac_cfg_del_vap_param_stru),
        (uint8_t *)&write_msg, OAL_TRUE, &rsp_msg);
...
int32_t wal_deinit_wlan_vap(oal_net_device_stru *net_dev)
{
    ...
    /* 仅用于WIFI和AP关闭时删除VAP */
#ifndef _PRE_WLAN_FEATURE_MULTI_AP
...
uint32_t wal_hipriv_setcountry(oal_net_device_stru *net_dev, int8_t *param)
{
    ...
    /* 设备在up状态不允许配置，必须先down */
    if ((OAL_IFF_RUNNING & oal_netdevice_flags(net_dev)) != 0) {
    ...
    if (wlan_pm_is_poweron() == OAL_FALSE) {
        oam_error_log1(0, OAM_SF_ANY, "{wal_hipriv_setcountry::device is down:%d!}", wlan_pm_is_poweron());
        return OAL_EBUSY;
    }
#endif

    /* 获取国家码字符串 */
    result = wal_get_cmd_one_arg(param, ac_arg, WAL_HIPRIV_CMD_NAME_MAX_LEN, &off_set);
    if (result != OAL_SUCC) {
...
#ifndef _PRE_PC_LINT
        if (oal_likely(oal_netdevice_mac_addr(pst_mac_device->st_p2p_info.pst_primary_net_device) != NULL)) {
...
oal_net_device_stru *wal_init_netdev(oal_wiphy_stru *pst_wiphy, const char *dev_name, enum nl80211_iftype en_type)
{
    ...
    /* 非NULL netdev 如果已经存在，直接返回 */
    if ((oal_strcmp(LINK_NETDEV_NAME, dev_name)) != 0) {
...
static void wal_p2p_record_wlan_net_dev(mac_device_stru *pst_mac_device, const char *dev_name,
    enum nl80211_iftype en_type, oal_net_device_stru *new_netdev)
{
    int8_t ac_wlan_netdev_name[MAC_NET_DEVICE_NAME_LENGTH];
    int32_t ret;
    ret = snprintf_s(ac_wlan_netdev_name, MAC_NET_DEVICE_NAME_LENGTH, MAC_NET_DEVICE_NAME_LENGTH - 1,
    ...
    if ((en_type == NL80211_IFTYPE_STATION) && ((oal_strcmp(ac_wlan_netdev_name, dev_name)) == 0)) {
        /* 如果创建wlan0， 则保存wlan0 为主net_device,p2p0 和p2p-p2p0 MAC 地址从主netdevice 获取 */
        pst_mac_device->st_p2p_info.pst_primary_net_device = new_netdev;
    } else if ((en_type == NL80211_IFTYPE_STATION) && ((oal_strcmp(WLAN1_NETDEV_NAME, dev_name)) == 0)) {
        /* netdevice 指针暂时挂载，未使用 */
        pst_mac_device->st_p2p_info.pst_second_net_device = new_netdev;
    } else if (en_type == NL80211_IFTYPE_P2P_DEVICE) {
        pst_mac_device->st_p2p_info.pst_p2p_net_device = new_netdev;
    }
}
/*
...
uint32_t wal_init_wlan_netdev(oal_wiphy_stru *pst_wiphy, const char *dev_name)
{
    ...
    oal_net_device_stru  *new_netdev;
    int8_t ac_wlan_netdev_name[MAC_NET_DEVICE_NAME_LENGTH];
    int8_t ac_p2p_netdev_name[MAC_NET_DEVICE_NAME_LENGTH];
    mac_device_stru *pst_mac_device = NULL;
    mac_wiphy_priv_stru *wiphy_priv = NULL;
    ...
    /* 信道跟随--添加hwlan name判断 */
    if ((oal_strcmp(ac_wlan_netdev_name, dev_name)) == 0 || (oal_strcmp(WLAN1_NETDEV_NAME, dev_name)) == 0) {
        en_type = NL80211_IFTYPE_STATION;
    } else if ((oal_strcmp(ac_p2p_netdev_name, dev_name)) == 0) {
    ...
    new_netdev = wal_init_netdev(pst_wiphy, dev_name, en_type);
    if (new_netdev == NULL) {
        return OAL_FAIL;
    }
    wal_set_mac_addr(new_netdev);
    ret = hmac_net_register_netdev(new_netdev); /* 注册net_device */
    if (oal_unlikely(ret != OAL_SUCC)) {
        oal_mem_free_m(oal_netdevice_wdev(new_netdev), OAL_FALSE);
        oal_net_free_netdev(new_netdev);
        oam_warning_log1(0, OAM_SF_ANY, "{wal_init_wlan_netdev::hmac_net_register_netdev err:%d!}", ret);
        return OAL_FAIL;
    }
    wal_p2p_record_wlan_net_dev(pst_mac_device, dev_name, en_type, new_netdev);
    return OAL_SUCC;
}
/*
 * 函 数 名  : wal_setup_ap
...
int32_t wal_setup_ap(oal_net_device_stru *net_dev)
{
    ...
    wal_set_power_mgmt_on(net_dev, OAL_FALSE);
