...
#ifdef _PRE_WLAN_FEATURE_EMLSR
    hmac_emlsr_close_for_mvap(pst_mac_vap, &channel, COFEATURE_MVAP_EVENT);
#endif
...
{
    return hmac_config_vaps_info(pst_mac_vap, us_len, puc_param);
}
...
uint32_t wal_set_rx_mode(oal_net_device_stru *pst_net_dev, uint8_t enable)
{
    mac_vap_stru *mac_vap = oal_net_dev_priv(pst_net_dev);
    if (mac_vap != NULL) {
        hmac_config_set_rx_mode(mac_vap, enable); // BIT4 host ring收,高功耗
    }
    return OAL_SUCC;
}
/*
...
OAL_STATIC uint32_t wal_config_tas_ant_switch(mac_vap_stru *pst_mac_vap, uint16_t us_len, uint8_t *puc_param)
{
    return hmac_config_tas_ant_switch(pst_mac_vap, us_len, puc_param);
}
#endif
#endif
...
OAL_STATIC uint32_t wal_config_set_rate(mac_vap_stru *pst_mac_vap, uint16_t us_len, uint8_t *puc_param)
{
    ...
    memset_s(&st_event_set_bcast, sizeof(mac_cfg_tx_comp_stru), 0, sizeof(mac_cfg_tx_comp_stru));
    if (oal_unlikely(oal_any_null_ptr2(pst_mac_vap, puc_param))) {
...
#endif
#ifdef _PRE_WLAN_FEATURE_11BE
OAL_STATIC uint32_t wal_config_set_11be_wifi7_black_list(mac_vap_stru *p_mac_vap,
    uint16_t len, uint8_t *p_param)
{
    return hmac_wifi7_self_cure_black_list_set(p_mac_vap, len, p_param);
}
OAL_STATIC uint32_t wal_config_set_mcsbe(mac_vap_stru *mac_vap, uint16_t len, uint8_t *param)
{
    uint32_t ret;

    if (oal_unlikely(oal_any_null_ptr2(mac_vap, param))) {
        oam_warning_log0(0, OAM_SF_ANY, "{wal_config_set_mcsbe::mac_vap/param is null ptr!}\r\n");
        return OAL_ERR_CODE_PTR_NULL;
    }

    /* 设置参数 */
    ret = hmac_config_set_mcsbe(mac_vap, len, param);
    if (oal_unlikely(ret != OAL_SUCC)) {
        oam_warning_log0(mac_vap->uc_vap_id, OAM_SF_ANY,
                         "{wal_config_set_mcsbe::hmac_config_set_mcsbe error!}\r\n");
        return ret;
    }

    return ret;
}
OAL_STATIC uint32_t _wal_config_set_mcsbe(mac_vap_stru *mac_vap,
    uint16_t len, uint8_t *param)
{
    if (g_wlan_spec_cfg->feature_11be_is_open) {
        return wal_config_set_mcsbe(mac_vap, len, param);
    }
...
#endif
/*
 * 函 数 名  : wal_config_get_cmd_netdev
 * 功能描述  : hipriv命令获取netdev, 新适配link下发命令
 * 1.日    期  : 2019年8月12日
 *   作    者  : wifi
 *   修改内容  : 新生成函数
 */
oal_net_device_stru *wal_config_get_cmd_netdev(const int8_t *pc_name, int32_t array_len)
{
#ifdef _PRE_WINDOWS_SUPPORT
    int n;
#endif
    ...
    if ((oal_strcmp("link", pc_name)) == 0) {
        return hmac_vap_get_link_net_device();
    }
...
{
    return hmac_config_add_key(pst_mac_vap, us_len, puc_param);
}
/*
...
{
    /* 通过函数调用，hmac具体实现 */
    return hmac_config_remove_key(pst_mac_vap, us_len, puc_param);
}
/*
...
OAL_STATIC uint32_t wal_config_query_lowpower_stat(mac_vap_stru *mac_vap, uint16_t len, uint8_t *param)
{
    return hmac_config_query_lowpower_stat(mac_vap, len, param);
}
    ...
    uint8_t *puc_param)
{
    ...
    hmac_roam_start_fill_info(&start_param, ROAM_SCAN_CHANNEL_ORG_DBDC, OAL_TRUE, NULL, ROAM_TRIGGER_11K_FAIL_RESTART);
    return hmac_roam_start(hmac_vap, &start_param);
}
...
/*
 * 函 数 名  : wal_config_set_pkt_filter_program
 * 功能描述  : pkt_filter命令
 * 1.日    期  : 2017年11月23日
 *   作    者  : wifi
 *   修改内容  : 新生成函数
 */
OAL_STATIC uint32_t wal_config_pkt_filter_cmd(mac_vap_stru *pst_mac_vap, uint16_t us_len, uint8_t *puc_param)
{
    return hmac_config_pkt_filter_cmd(pst_mac_vap, us_len, puc_param);
}
...
OAL_STATIC uint32_t wal_config_chba_cancel_conn_ntf(mac_vap_stru *mac_vap, uint16_t len, uint8_t *param)
{
    return hmac_chba_cancel_response_connect(mac_vap, len, param);
}
...
OAL_STATIC uint32_t wal_config_chba_service_ask_user_all_awake(mac_vap_stru *mac_vap, uint16_t len, uint8_t *param)
{
    return hmac_chba_service_ask_user_all_awake(mac_vap, len, param);
}
#endif
...
#ifdef _PRE_WLAN_FEATURE_ISOLATION
static uint32_t wal_config_set_isolation(mac_vap_stru *mac_vap, uint16_t len, uint8_t *param)
{
    return hmac_config_set_isolation(mac_vap, param);
}
#endif

static uint32_t wal_config_cancel_same_chan_remain_on_chan(mac_vap_stru *mac_vap, uint16_t len, uint8_t *param)
{
    return hmac_cancel_same_chan_remain_on_chan(mac_vap, len, param);
}
static uint32_t wal_config_tx_start_same_chan_remain_on_chan(mac_vap_stru *mac_vap, uint16_t len, uint8_t *param)
{
    mac_same_chan_remain_on_chan_cfg *remain_on_chan_cfg = (mac_same_chan_remain_on_chan_cfg *)param;
    return hmac_tx_mgmt_start_same_chan_remain_on_chan(mac_vap, remain_on_chan_cfg->mac_work_vap,
        remain_on_chan_cfg->remian_on_chan_need_wait, remain_on_chan_cfg->wait_time);
}
static uint32_t wal_config_rx_start_same_chan_remain_on_chan(mac_vap_stru *mac_vap, uint16_t len, uint8_t *param)
{
    mac_same_chan_remain_on_chan_cfg *remain_on_chan_cfg = (mac_same_chan_remain_on_chan_cfg *)param;
    return hmac_listen_start_same_chan_remain_on_chan(mac_vap, remain_on_chan_cfg->mac_work_vap,
        remain_on_chan_cfg->wait_time);
}

#ifdef _PRE_WLAN_FEATURE_WUR
static uint32_t wal_config_set_wur_param(mac_vap_stru *mac_vap, uint16_t len, uint8_t *param)
{
    return hmac_config_set_wur_param(mac_vap, param);
}
#endif

static uint32_t wal_config_set_lp_long_dtim(mac_vap_stru *mac_vap, uint16_t len, uint8_t *param)
{
    return hmac_config_set_lp_long_dtim(mac_vap, len, param);
}
#ifdef _PRE_WLAN_FEATURE_11AX
static uint32_t wal_config_set_omi_bw_switch(mac_vap_stru *mac_vap, uint16_t len, uint8_t *param)
{
    return hmac_config_set_omi_bw_switch(mac_vap, len, param);
}
#endif
/* cfgid操作全局变量 */
OAL_STATIC const wal_wid_op_stru g_ast_board_wid_op[] = {
...
#endif
#ifdef _PRE_WLAN_FEATURE_11BE
    {WLAN_CFGID_SET_11BE_WIFI7_BLACKLIST, OAL_FALSE, { 0 }, NULL, wal_config_set_11be_wifi7_black_list},
    { WLAN_CFGID_SET_MCSBE, OAL_FALSE, { 0 }, NULL, _wal_config_set_mcsbe },
#endif
    ...
    { WLAN_CFGID_TAS_ANT_SWITCH, OAL_FALSE, { 0 }, NULL, wal_config_tas_ant_switch },
#endif
#endif
#ifdef _PRE_WLAN_FEATURE_TAS_ANT_SWITCH
...
#endif
#ifdef _PRE_WLAN_FEATURE_PKT_FILTER
    { WLAN_CFGID_SET_PKT_FILTER, OAL_FALSE, { 0 }, NULL, wal_config_pkt_filter_cmd },
#endif
    ...
    { WLAN_CFGID_CHBA_CANCEL_CONN_NOTIFY, OAL_FALSE, { 0 }, NULL, wal_config_chba_cancel_conn_ntf },
    { WLAN_CFGID_CHBA_SERVICE_ASK_USER_ALL_AWAKE, OAL_FALSE, { 0 }, NULL, wal_config_chba_service_ask_user_all_awake },
#endif
...
#ifdef _PRE_WLAN_FEATURE_ISOLATION
    { WLAN_CFGID_SET_ISOLATION, OAL_FALSE, {0}, NULL, wal_config_set_isolation },
#endif
    { WLAN_CFGID_TX_START_SAME_CHAN_REMAIN_ON_CHAN, OAL_FALSE, {0}, NULL,
        wal_config_tx_start_same_chan_remain_on_chan },
    { WLAN_CFGID_RX_START_SAME_CHAN_REMAIN_ON_CHAN, OAL_FALSE, {0}, NULL,
        wal_config_rx_start_same_chan_remain_on_chan },
    { WLAN_CFGID_CANCAL_SAME_CHAN_REMAIN_ON_CHAN_SYNC, OAL_FALSE, {0}, NULL,
        wal_config_cancel_same_chan_remain_on_chan },
#ifdef _PRE_WLAN_FEATURE_WUR
    { WLAN_CFGID_SET_WUR_PARAM, OAL_FALSE, { 0 }, NULL, wal_config_set_wur_param },
#endif
    { WLAN_CFGID_SET_LP_LONG_DTIM, OAL_FALSE, { 0 }, NULL, wal_config_set_lp_long_dtim },
#ifdef _PRE_WLAN_FEATURE_11AX
    { WLAN_CFGID_SET_OMI_BW_SWITCH, OAL_FALSE, { 0 }, NULL, wal_config_set_omi_bw_switch },
#endif
    { WLAN_CFGID_BUTT, OAL_FALSE, { 0 }, 0, 0 },
#ifdef _PRE_WLAN_FEATURE_ELP
    { WLAN_CFGID_START_ELP_SCAN, OAL_FALSE, { 0 }, NULL, wal_config_start_elp_scan},
#endif
    { WLAN_CFGID_GET_LOWPOWER_STATISTICS, OAL_FALSE, { 0 }, NULL, wal_config_query_lowpower_stat },
};
...
OAL_STATIC wal_wid_op_stru *wal_config_get_wid_map(wal_wid_op_stru *pst_wid_map, uint16_t en_wid, uint32_t wid_nums)
{
    ...
    for (cfgid = 0; cfgid < wid_nums; cfgid++) {
        pst_current_wid = pst_wid_map + cfgid;
