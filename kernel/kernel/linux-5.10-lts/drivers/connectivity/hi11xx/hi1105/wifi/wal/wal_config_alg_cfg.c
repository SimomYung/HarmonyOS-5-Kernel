...
OAL_STATIC int32_t g_al_mcs_min_table[WAL_CFG_MCS_TYPE_BUTT] = {
...
#endif
    WAL_HIPRIV_EHT_MCS_MIN,
};

OAL_STATIC int32_t g_al_mcs_max_table[WAL_CFG_MCS_TYPE_BUTT] = {
...
const wal_ioctl_alg_cfg_stru g_ast_alg_cfg_map[] = {
    ...
    {"mlsr_switch_en", MAC_ALG_MLSR_SWITCH_ENABLE},
    {"mlsr_switch_log", MAC_ALG_MLSR_SWITCH_LOG},
    {"mlsr_5g_low_rssi", MAC_ALG_MLSR_5G_LOW_RSSI_TH},
    {"mlsr_2g_low_rssi", MAC_ALG_MLSR_2G_LOW_RSSI_TH},
    {"mlsr_per_th", MAC_ALG_MLSR_PER_TH},
    {"mlsr_pps_th", MAC_ALG_MLSR_PPS_TH},
    {"mlsr_intf_th", MAC_ALG_MLSR_INTF_TH},
    {"mlsr_tx_fail_ratio_th", MAC_ALG_MLSR_TX_FAIL_RATIO_TH},
#ifdef _PRE_WLAN_FEATURE_EMLSR
    {"emlsr_sch_en",    MAC_ALG_CFG_EMLSR_SCH_ENABLE },
    {"emlsr_debug_log", MAC_ALG_CFG_EMLSR_SCH_DEBUG },
    {"emlsr_weight",    MAC_ALG_CFG_EMLSR_SCH_WEIGHT },
    {"emlsr_strategy",    MAC_ALG_CFG_EMLSR_SCH_STRATEGY },
#endif

    {"punc_en",            MAC_ALG_CFG_SU_PUNC_ENABLE},
    {"punc_follow_bss",    MAC_ALG_CFG_SU_PUNC_FOLLOW_BSS},
    {"punc_bitmap",        MAC_ALG_CFG_SU_PUNC_BITMAP},
    {"punc_log",           MAC_ALG_CFG_SU_PUNC_LOG},
    {"punc_prim_th",       MAC_ALG_CFG_SU_PUNC_PRIM_CHAN_BUSY_TH},
    {"punc_th",            MAC_ALG_CFG_SU_PUNC_CHAN_BUSY_TH},
    {"punc_gain_th",       MAC_ALG_CFG_SU_PUNC_GAIN_TH},
    {"punc_tx_fail_th",    MAC_ALG_CFG_SU_PUNC_TX_FAIL_TH},

    {"smartant_mode",      MAC_ALG_CFG_SMARTANT_MODE},
    {"smartant_fixant",    MAC_ALG_CFG_SMARTANT_FIX_ANT},
    {"smartant_debug",     MAC_ALG_CFG_SMARTANT_DEBUG},
...
uint32_t alg_cfg_args_analysis_waterfilling_mode(alg_cfg_param_stru *alg_cfg_param,
                                                 uint8_t *mem_block, uint16_t *len)
{
    uint32_t ret;

    ret = alg_cfg_param_analyze_waterfilling_mode(alg_cfg_param,
        (alg_cfg_param_waterfilling_mode_stru *)mem_block);
    if (ret != OAL_SUCC) {
        oam_error_log1(0, OAM_SF_ANY,
    ...
    wal_dscr_param_enum_uint8 en_param_index)
{
    ...
    if (pst_set_non_ht_rate_param->en_protocol_mode == WLAN_PHY_PROTOCOL_MODE_11B_PLUS) {
        return OAL_SUCC;
    }
#endif
    ...
    int8_t *ac_arg, mac_cfg_set_dscr_param_stru *pc_stu)
{
...
#ifdef _PRE_WLAN_FEATURE_11B_PLUS
        if (en_param_index < WLAN_11BPLUS_RATE_VALUE_BUTT &&
            (!oal_strcmp(g_pauc_11bplus_rate_tbl[en_param_index], ac_arg))) {
            pst_set_non_ht_rate_param->en_rate = en_param_index;
            pst_set_non_ht_rate_param->en_protocol_mode = WLAN_PHY_PROTOCOL_MODE_11B_PLUS;
            break;
        }
#endif
    }
    ...
    if (wal_hipriv_set_non_ht_rate(pst_set_non_ht_rate_param, en_param_index) != OAL_SUCC) {
        return OAL_ERR_CODE_INVALID_CONFIG;
    }

    if (wal_hipriv_set_fix_rate_pre_config(net_dev, mac_vap, OAL_TRUE, pc_stu) != OAL_SUCC) {
        return OAL_FAIL;
    }

    return OAL_SUCC;
}
OAL_STATIC uint8_t wal_hipriv_get_cfg_mcs_type_idx(uint8_t function_index)
{
    return (uint8_t)((function_index == WAL_DSCR_PARAM_MCSBE) ? WAL_CFG_MCS_TYPE_MCSBE :
        (function_index - WAL_DSCR_PARAM_MCS));
}
    ...
    mac_cfg_set_dscr_param_stru *pc_stu)
{
    ...
    mcs_index = wal_hipriv_get_cfg_mcs_type_idx(pc_stu->uc_function_index);
    if (mcs_index >= WAL_CFG_MCS_TYPE_BUTT) {
        return OAL_ERR_CODE_ARRAY_OVERFLOW;
    }
    if (l_val < g_al_mcs_min_table[mcs_index] || l_val > g_al_mcs_max_table[mcs_index]) {
...
uint32_t wal_hipriv_set_atf_user(oal_net_device_stru *net_dev, int8_t *pc_param)
{
    ...
    atf_cfg_user_param_stru *user_param = (atf_cfg_user_param_stru *)(wal_write_msg.auc_value);

    ret = wal_get_cmd_one_arg(pc_param, ac_name, WAL_HIPRIV_CMD_NAME_MAX_LEN, &off_set);
    if (ret != OAL_SUCC) {
        oam_warning_log1(0, OAM_SF_ANY, "{wal_hipriv_set_atf_user::wal_get_cmd_one_arg err [%d]!}", ret);
        return ret;
    }

    oal_strtoaddr(ac_name, sizeof(ac_name), user_param->user_val.user_mac_addr, WLAN_MAC_ADDR_LEN);

    ret = wal_get_cmd_one_arg(pc_param + off_set, ac_name, WAL_HIPRIV_CMD_NAME_MAX_LEN, &off_set);
    if (ret != OAL_SUCC) {
        oam_warning_log1(0, OAM_SF_ANY, "{wal_hipriv_set_atf_user::get second arg err[%d]!}", ret);
        return ret;
    }

    user_param->user_val.atf_user_val = (uint16_t)oal_strtol(ac_name, NULL, BYTE_OFFSET_10);
    user_param->mac_alg_param.en_alg_cfg = MAC_ALG_CFG_ATF_USER_VAL;

    oam_warning_log4(0, OAM_SF_ANY,
        "{wal_hipriv_set_atf_user::user [%02X:XX:XX:XX:%02X:%02X] atf_user_val[%d].}",
        user_param->user_val.user_mac_addr[MAC_ADDR_0],
        user_param->user_val.user_mac_addr[MAC_ADDR_4],
        user_param->user_val.user_mac_addr[MAC_ADDR_5],
        user_param->user_val.atf_user_val);
    ...
    if (ret != OAL_SUCC) {
        oam_error_log1(0, OAM_SF_ANY, "{wal_hipriv_set_atf_user::return err code[%d]!}", ret);
        return ret;
    }
