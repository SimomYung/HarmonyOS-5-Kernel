...
extern uint8_t alg_cfg_param_get_tbl_size(alg_cfg_param_tbl_size_enum_uint8 param_type);
extern uint32_t  alg_cfg_get_cmd_one_arg(const char *cmd, char *arg,
                                         uint32_t arg_size, uint32_t *cmd_offset);
int32_t wal_wlan_cfg_module_process_entry(wlan_cfg_param_stru *wlan_cfg_param);
extern alg_cfg_process_info_stru *alg_cfg_search_process_info_by_cfg_name(const char *alg_cfg_name);
extern alg_cfg_process_info_stru *alg_cfg_search_process_info_by_cfg_id(mac_alg_cfg_enum_uint16 alg_cfg_id,
                                                                        alg_cfg_type_enum_uint8 cfg_type);
extern alg_cfg_pkt_type_enum_uint8 alg_cfg_get_pkt_type(const char *cmd_args);
extern uint32_t alg_cfg_analysis_args_head(char *real_args[WLAN_CFG_MAX_ARGS_NUM],
    alg_cfg_process_info_stru **real_alg_cfg_info, uint8_t *real_args_offset,
    alg_cfg_pkt_type_enum_uint8 *real_pkt_type);
extern uint32_t alg_cfg_analysis_args(char *args[WLAN_CFG_MAX_ARGS_NUM], uint8_t args_num,
    alg_cfg_param_stru *alg_cfg_param, alg_cfg_hdr_stru *alg_cfg_hdr,
    wlan_cfg_process_info_stru *process_info);
extern uint32_t alg_cfg_get_match_value(const alg_cfg_param_tbl_stru *tbl, uint16_t tbl_size,
    const uint8_t *str, uint32_t *out_value);
extern int32_t wal_wlan_cfg_alg_process_entry(wlan_cfg_param_stru *wlan_cfg_param,
...
/* txmode Start */
extern uint32_t alg_cfg_args_analysis_txmode_user_fix_mode(alg_cfg_param_stru *alg_cfg_param,
    uint8_t *mem_block, uint16_t *len);
...
extern uint32_t alg_cfg_args_analysis_txmode_debug_log_switch(alg_cfg_param_stru *alg_cfg_param,
                                                              uint8_t *puc_mem_block, uint16_t *pus_len);
extern uint32_t alg_cfg_args_analysis_txmode_mode_sw(alg_cfg_param_stru *alg_cfg_param,
