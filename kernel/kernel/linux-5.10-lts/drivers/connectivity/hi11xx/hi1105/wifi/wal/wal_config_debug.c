...
OAL_STATIC uint32_t wal_config_hitalk_bg_listen(mac_vap_stru *pst_mac_vap, uint16_t us_len, uint8_t *puc_param)
{
    return hmac_config_send_event(pst_mac_vap, WLAN_CFGID_DIV_SET, us_len, puc_param);
}
#endif
#endif
...
OAL_STATIC uint32_t wal_config_set_str_ml_tx_mode_switch(mac_vap_stru *mac_vap, uint16_t len, uint8_t *param)
{
    return hmac_config_str_ml_tx_mode_switch(mac_vap, len, param);
}

OAL_STATIC uint32_t wal_config_set_mld_assoc_mode(mac_vap_stru *mac_vap, uint16_t len, uint8_t *param)
{
    return hmac_config_mld_assoc_mode(mac_vap, len, param);
}
OAL_STATIC uint32_t wal_config_mlsr_work_link_switch(mac_vap_stru *mac_vap, uint16_t len, uint8_t *param)
{
    return hmac_config_mlsr_work_link_switch(mac_vap, len, param);
}

OAL_STATIC uint32_t wal_config_ml_reconfig_remove_link(mac_vap_stru *mac_vap, uint16_t len, uint8_t *param)
{
    return hmac_config_ml_reconfig_remove_link(mac_vap, len, param);
}
OAL_STATIC uint32_t wal_config_tx_action_ml_reconfig_request(mac_vap_stru *mac_vap, uint16_t len, uint8_t *param)
{
    return hmnac_config_tx_action_ml_reconfig_request(mac_vap, len, param);
}

OAL_STATIC uint32_t wal_config_mld_tx_ttlm_request(mac_vap_stru *mac_vap, uint16_t len, uint8_t *param)
{
    return hmac_config_mld_tx_ttlm_request(mac_vap, len, param);
}

OAL_STATIC uint32_t wal_config_mld_tx_ttlm_teardown(mac_vap_stru *mac_vap, uint16_t len, uint8_t *param)
{
    return hmac_config_mld_tx_ttlm_teardown(mac_vap, len, param);
}

#ifdef _PRE_WLAN_FEATURE_EMLSR
OAL_STATIC uint32_t wal_config_emlsr_enable(mac_vap_stru *mac_vap, uint16_t len, uint8_t *param)
{
    return hmac_config_ml_emlsr_enable(mac_vap, param);
}

OAL_STATIC uint32_t wal_config_emlsr_cfg_link(mac_vap_stru *mac_vap, uint16_t len, uint8_t *param)
{
    return hmac_config_ml_emlsr_cfg_link(mac_vap, len, param);
}
OAL_STATIC uint32_t wal_config_al_emlsr_enable(mac_vap_stru *mac_vap, uint16_t len, uint8_t *param)
{
    return hmac_config_al_ml_emlsr_enable(mac_vap, len, param);
}
OAL_STATIC uint32_t wal_config_set_emlsr_dscr(mac_vap_stru *mac_vap, uint16_t len, uint8_t *param)
{
    return hmac_config_ml_set_emlsr_dscr(mac_vap, len, param);
}
#endif
#endif

#ifdef _PRE_WLAN_FEATURE_WUR
OAL_STATIC uint32_t wal_config_wur_debug_process(mac_vap_stru *mac_vap, uint16_t len, uint8_t *param)
{
    return hmac_config_wur_debug_process(mac_vap, len, param);
}
#endif

OAL_STATIC uint32_t wal_config_phase_incr(mac_vap_stru *mac_vap, uint16_t len, uint8_t *param)
{
    uint32_t ret;

    if (oal_unlikely(oal_any_null_ptr2(mac_vap, param))) {
        oam_warning_log0(0, OAM_SF_ANY, "{wal_config_phase_incr::pst_mac_vap/puc_param is null ptr!}\r\n");
        return OAL_ERR_CODE_PTR_NULL;
    }

    ret = hmac_config_phase_incr(mac_vap, len, param);
    if (oal_unlikely(ret != OAL_SUCC)) {
        oam_warning_log0(mac_vap->uc_vap_id, OAM_SF_ANY,
                         "{wal_config_phase_incr::hmac_config_always_tx failed!}\r\n");
        return ret;
    }
...
const wal_wid_op_stru g_ast_board_wid_op_debug[] = {
    ...
    { WLAN_CFGID_DIV_SET, OAL_FALSE, { 0 }, NULL, wal_config_hitalk_bg_listen },
#endif
#endif
...
#ifdef _PRE_WLAN_FEATURE_ML
    { WLAN_CFGID_ML_MODE_SWITCH, OAL_FALSE, { 0 }, NULL, wal_config_set_str_ml_tx_mode_switch },
    { WLAN_CFGID_SET_MLD_ASSOC_MODE, OAL_FALSE, { 0 }, NULL, wal_config_set_mld_assoc_mode },
    { WLAN_CFGID_MLSR_WROK_LINK_SWITCH, OAL_FALSE, { 0 }, NULL, wal_config_mlsr_work_link_switch },
    { WLAN_CFGID_ML_RECONFIG_REMOVE_LINK, OAL_FALSE, { 0 }, NULL, wal_config_ml_reconfig_remove_link },
    { WLAN_CFGID_TX_ML_RECONFIG_REQUEST, OAL_FALSE, { 0 }, NULL, wal_config_tx_action_ml_reconfig_request },
    { WLAN_CFGID_ML_TTLM_REQUEST, OAL_FALSE, { 0 }, NULL, wal_config_mld_tx_ttlm_request },
    { WLAN_CFGID_ML_TTLM_TEARDOWN, OAL_FALSE, { 0 }, NULL, wal_config_mld_tx_ttlm_teardown },

#ifdef _PRE_WLAN_FEATURE_EMLSR
    { WLAN_CFGID_EMLSR_ENABLE, OAL_FALSE, { 0 }, NULL, wal_config_emlsr_enable },
    { WLAN_CFGID_EMLSR_CFG_LINK, OAL_FALSE, { 0 }, NULL, wal_config_emlsr_cfg_link },
    { WLAN_CFGID_AL_EMLSR_ENABLE, OAL_FALSE, { 0 }, NULL, wal_config_al_emlsr_enable },
    { WLAN_CFGID_EMLSR_CFG_SET_DSCR, OAL_FALSE, { 0 }, NULL, wal_config_set_emlsr_dscr },
#endif
#ifdef _PRE_WLAN_FEATURE_WUR
    { WLAN_CFGID_WUR_DEBUG, OAL_FALSE, { 0 }, NULL, wal_config_wur_debug_process },
#endif
#endif
    { WLAN_CFGID_SET_PHASE_INCR, OAL_FALSE, { 0 }, NULL, wal_config_phase_incr },
