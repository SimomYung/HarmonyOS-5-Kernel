...
OAL_STATIC int32_t wal_dfr_destroy_vap(oal_net_device_stru *pst_netdev)
{
    ...
    if (wal_del_vap_try_wlan_pm_close(wal_get_net_device_id(pst_netdev)) != OAL_SUCC) {
...
uint32_t  wal_host_resume_work(void)
{
    ...
    for (; (g_st_recovery_info.netdev_num > 0 &&
        g_st_recovery_info.netdev_num < mac_chip_get_vap_support_max_num_limit()); g_st_recovery_info.netdev_num--) {
        ret = OAL_SUCC;
...
OAL_STATIC void wal_dfr_recovery_report(void)
{
    ...
    for (; (netdev_idx > 0 && netdev_idx < WLAN_VAP_MAX_NUM_LIMIT); netdev_idx--) {
...
OAL_STATIC int32_t wal_dfr_recovery_all_vap(void)
{
    ...
    for (; (netdev_idx > 0 && netdev_idx < mac_chip_get_vap_support_max_num_limit()); netdev_idx--) {
        netdev = g_st_dfr_info.past_netdev[netdev_idx - 1];
...
#ifdef _PRE_CONFIG_HISI_S3S4_POWER_STATE
#if (_PRE_OS_VERSION_LINUX == _PRE_OS_VERSION)
    oal_mutex_lock(&g_st_recovery_info.wifi_recovery_mutex);
...
#if (_PRE_OS_VERSION_LINUX == _PRE_OS_VERSION)
    oal_mutex_unlock(&g_st_recovery_info.wifi_recovery_mutex);
...
uint32_t  wal_host_suspend_work(void)
{
    ...
    memset_s((uint8_t *)(g_st_recovery_info.netdev),
        sizeof(g_st_recovery_info.netdev[0]) * WLAN_VAP_MAX_NUM_LIMIT,
        0,
        sizeof(g_st_recovery_info.netdev[0]) * WLAN_VAP_MAX_NUM_LIMIT);
...
#ifdef _PRE_CONFIG_HISI_S3S4_POWER_STATE
    OAL_MUTEX_INIT(&g_st_recovery_info.wifi_recovery_mutex);
