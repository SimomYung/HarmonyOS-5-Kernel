...
uint32_t wal_hipriv_chba_chan_switch(oal_net_device_stru *pst_net_dev, int8_t *pc_param)
{
    ...
    /* 设置配置命令参数 */
    if (memcpy_s(st_write_msg.auc_value, sizeof(test_params),
        (const void *)&test_params, sizeof(test_params)) != EOK) {
...
uint32_t wal_hipriv_chba_set_user_bitmap(oal_net_device_stru *net_dev, int8_t *pc_param)
{
    ...
    */
    ...
    /* 设置配置命令参数 */
    if (memcpy_s(st_write_msg.auc_value, sizeof(ps_bitmap_info),
        (const void *)&ps_bitmap_info, sizeof(ps_bitmap_info)) != EOK) {
...
uint32_t wal_hipriv_set_chba_feature_switch(oal_net_device_stru *pst_net_dev, int8_t *pc_param)
{
    ...
    /* 设置配置命令参数 */
    if (memcpy_s(st_write_msg.auc_value, sizeof(cfg_params),
        (const void *)&cfg_params, sizeof(cfg_params)) != EOK) {
...
/* 功能描述: 发起一次监听的hipriv命令接口 */
uint32_t wal_hipriv_rx_mgmt_remain_on_channel(oal_net_device_stru *net_dev, int8_t *param)
{
    uint32_t ret;
    uint32_t off_set;
    int8_t ac_name[WAL_HIPRIV_CMD_NAME_MAX_LEN] = { 0 };
    wlan_remain_on_channel_info_stru remain_on_chan_info = { 0 };

    /* 获取center_freq */
    ret = wal_get_cmd_one_arg(param, ac_name, WAL_HIPRIV_CMD_NAME_MAX_LEN, &off_set);
    if (ret != OAL_SUCC) {
        oam_error_log0(0, OAM_SF_ANY, "{wal_hipriv_rx_mgmt_remain_on_channel:fail to get center_freq}");
        return ret;
    }
    param += off_set;
    remain_on_chan_info.center_freq = (uint16_t)oal_atoi(ac_name);
    /* 下发hipriv命令中center freq为0时,意味着取消当前长期监听 */
    if (remain_on_chan_info.center_freq == 0) {
        wal_ioctl_cancel_same_chan_remain_on_chan(net_dev);
        return OAL_SUCC;
    }
    ...
    ret = wal_get_cmd_one_arg(param, ac_name, WAL_HIPRIV_CMD_NAME_MAX_LEN, &off_set);
    if (ret != OAL_SUCC) {
        oam_error_log0(0, OAM_SF_ANY, "{wal_hipriv_rx_mgmt_remain_on_channel:fail to get band}");
        return ret;
    }
    param += off_set;
    remain_on_chan_info.band = (uint8_t)oal_atoi(ac_name);

    /* 获取remain_on_channel时长 */
    ret = wal_get_cmd_one_arg(param, ac_name, WAL_HIPRIV_CMD_NAME_MAX_LEN, &off_set);
    if (ret != OAL_SUCC) {
        oam_error_log0(0, OAM_SF_ANY, "{wal_hipriv_rx_mgmt_remain_on_channel:fail to get duration!}");
        return ret;
    }
    param += off_set;
    remain_on_chan_info.duration = (uint32_t)oal_atoi(ac_name);
    ...
    param += off_set;
    remain_on_chan_info.is_exclusive_mode = (uint8_t)oal_atoi(ac_name);

    wal_ioctl_start_rx_mgmt_remain_on_channel(net_dev, &remain_on_chan_info);
    return OAL_SUCC;
}

/* 功能描述: 取消长期同信道监听的hipriv命令接口 */
uint32_t wal_hipriv_cancel_same_chan_remain_on_chan(oal_net_device_stru *net_dev, int8_t *param)
{
    ...
    ***************************************************************************/
    ...
    wal_send_cfg_event(net_dev, WAL_MSG_TYPE_WRITE, WAL_MSG_WRITE_MSG_HDR_LENGTH + sizeof(uint8_t),
        (uint8_t *)&write_msg, OAL_FALSE, NULL);
    oam_warning_log0(0, OAM_SF_ANY, "{wal_hipriv_cancel_same_chan_remain_on_chan}");
    return OAL_SUCC;
}

uint32_t wal_hipriv_chba_service_ask_user_all_awake(mac_vap_stru *mac_vap, uint32_t *params)
{
    int32_t ret;
    wal_msg_write_stru write_msg = { 0 };
    hmac_user_stru *hmac_user = NULL;
    hmac_vap_stru *hmac_vap = NULL;
    wlan_chba_service_ask_awake_stru service_info = { 0 };

    hmac_vap = (hmac_vap_stru *)mac_res_get_hmac_vap(mac_vap->uc_vap_id);
    if ((hmac_vap == NULL) || (hmac_vap->pst_net_device == NULL)) {
        return OAL_FAIL;
    }
    hmac_user = (hmac_user_stru *)mac_res_get_hmac_user((uint16_t)params[BYTE_OFFSET_0]);
    if (hmac_user == NULL) {
        oam_error_log1(mac_vap->uc_vap_id, OAM_SF_ANY, "{wal_hipriv_chba_service_ask_user_all_awake:fail find use[%d]}",
            (uint16_t)params[BYTE_OFFSET_0]);
        return OAL_FAIL;
    }
    oal_set_mac_addr(service_info.user_mac_addr, hmac_user->st_user_base_info.auc_user_mac_addr);
    service_info.ps_service_type = params[BYTE_OFFSET_1];
    /* 获取目标user mac地址与业务类型 */
    if (memcpy_s(&write_msg.auc_value, sizeof(wlan_chba_service_ask_awake_stru), &service_info,
        sizeof(wlan_chba_service_ask_awake_stru)) != EOK) {
        oam_error_log0(0, OAM_SF_ANY, "wal_hipriv_chba_service_ask_user_all_awake:memcpy fail!");
        return OAL_FAIL;
    }
    ...
    ***************************************************************************/
    ...
    ret = wal_send_cfg_event(hmac_vap->pst_net_device, WAL_MSG_TYPE_WRITE, WAL_MSG_WRITE_MSG_HDR_LENGTH +
