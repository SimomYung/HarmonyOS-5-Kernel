...
typedef enum {
    WAL_ATCMDSRV_GET_BOARD_NCID_CMD,
    WAL_ATCMDSRV_GET_SUPPORT_NCID_CMD,
} get_board_ncid_cmd;
...
const wal_atcmdsrv_mode_stru g_ast_atcmdsrv_mode_table[] = {
    ...
    { WLAN_EHT_MODE_5G,        WLAN_BAND_5G },
    { WLAN_EHT_MODE_2G,        WLAN_BAND_2G },
...
#define WAL_ATCMD_JUDGE_PLL_STATE_TIME 3
#endif
...
OAL_STATIC void wal_get_mp16_fem_pa_status(oal_net_device_stru *pst_net_dev, int32_t *pl_fem_pa_status)
{
    ...
    cali_data = (mp1x_wlan_cali_data_para_stru *)get_cali_data_buf_addr();
    if (cali_data == NULL) {
        return;
    }
    ...
    if (*pl_fem_pa_status != 0) {
        ...
        if (check_cnt > WAL_ATCMDSRV_FEM_DSM_THRESHOLD) {
            hw_mpxx_dsm_client_notify(SYSTEM_TYPE_WIFI, DSM_WIFI_FEMERROR, "%s: fem error,err_chain=0x%x,err_band %x",
                __FUNCTION__, *pl_fem_pa_status, cali_data->common_cali_data.cali_ctl_data.fem_error_band);
        }
#endif
    }
    return;
...
OAL_STATIC void wal_get_mp12_fem_pa_status(oal_net_device_stru *pst_net_dev, int32_t *pl_fem_pa_status)
{
    mp12_wlan_cali_data_para_stru *cali_data;
    uint32_t *check_hw_status = NULL;
    uint32_t status = (uint32_t)(*pl_fem_pa_status);
    uint8_t rf_id;
    static uint16_t check_cnt = 0;

    cali_data = (mp12_wlan_cali_data_para_stru *)get_cali_data_buf_addr();
    if (cali_data == NULL) {
        return;
    }
    check_hw_status = cali_data->common_cali_data.cali_ctl_data.check_hw_status;
    ...
    for (rf_id = 0; rf_id < MP12_HD_EVENT_RF_NUM; rf_id++) {
        status |= ((check_hw_status[rf_id] & WLAN_FEM_PA_STATUS_MASK) << (BIT_OFFSET_4 * rf_id));
    }
    *pl_fem_pa_status = (int32_t)status;
    if (*pl_fem_pa_status != 0) {
        chr_exception_report(CHR_PLATFORM_EXCEPTION_EVENTID, CHR_SYSTEM_WIFI, CHR_LAYER_DEV,
                             CHR_WIFI_DEV_EVENT_CHIP, CHR_WIFI_DEV_ERROR_FEM_FAIL);
        oam_error_log2(0, OAM_SF_ANY,
...
OAL_STATIC void wal_get_mp12_lna_gain_status(oal_net_device_stru *net_dev, int32_t *lna_gain_status)
{
    mp12_wlan_cali_data_para_stru *cali_data;
    uint32_t *check_hw_status = NULL;
    uint32_t status = (uint32_t)(*lna_gain_status);
    uint8_t rf_id;
    ...
    for (rf_id = 0; rf_id < MP12_HD_EVENT_RF_NUM; rf_id++) {
        status |=
            (((check_hw_status[rf_id] & WLAN_LNA_GAIN_STATUS_MASK) >> BIT_OFFSET_4) << (BIT_OFFSET_4 * rf_id));
        check_hw_status[rf_id] &= ~WLAN_LNA_GAIN_STATUS_MASK;
    }
    *lna_gain_status = (int32_t)status;
    if (*lna_gain_status != 0) {
        oam_error_log1(0, OAM_SF_ANY,
            "{wal_get_mp16_lna_gain_status::LNA gain status[%x].Bit:[0-3]c0,[4-7]c1,[8-11]c2,[12-15]c3,6g/5g/2g}",
            *lna_gain_status);
...
OAL_STATIC void wal_get_mp12_elna_gain_status(oal_net_device_stru *net_dev, int32_t *elna_gain_status)
{
    mp12_wlan_cali_data_para_stru *cali_data;
    uint32_t *check_hw_status = NULL;
    uint32_t status = (uint32_t)(*elna_gain_status);
    uint8_t rf_id;
    ...
    for (rf_id = 0; rf_id < MP12_HD_EVENT_RF_NUM; rf_id++) {
        status |=
            (((check_hw_status[rf_id] & WLAN_ELNA_GAIN_STATUS_MASK) >> WLAN_ELNA_GAIN_STATUS_OFFSET) <<
            (BIT_OFFSET_4 * rf_id));
    }
    *elna_gain_status = (int32_t)status;
    if (*elna_gain_status != 0) {
        oam_error_log1(0, OAM_SF_ANY,
            "{wal_get_mp16_elna_gain_status::ELNA gain status[0x%x].Bit:[0-3]c0,[4-7]c1,[8-11]c2,[12-15]c3,6g/5g/2g}",
            *elna_gain_status);
...
OAL_STATIC void wal_get_mp15_saw_status(oal_net_device_stru *net_dev, int32_t *saw_status)
{
    mp15_cali_param_stru *cali_data;
    uint8_t rf_id;
    ...
    /* 没做检测则不获取检测结果 */
    if (cali_data[WLAN_RF_CHANNEL_ZERO].elna_det[WLAN_BAND_2G].saw_det_flag == OAL_FALSE &&
        cali_data[WLAN_RF_CHANNEL_ONE].elna_det[WLAN_BAND_2G].saw_det_flag == OAL_FALSE) {
        return;
    }
    for (rf_id = 0; rf_id < WLAN_RF_CHANNEL_NUMS; rf_id++) {
        if (cali_data[rf_id].elna_det[WLAN_BAND_2G].saw_det_flag) {
            *saw_status |= (cali_data[rf_id].elna_det[WLAN_BAND_2G].check_saw_status << (BIT_OFFSET_4 * rf_id));
            cali_data[rf_id].elna_det[WLAN_BAND_2G].saw_det_flag = OAL_FALSE;
        }
    }
    if (*saw_status != 0) {
        oam_error_log1(0, OAM_SF_ANY,
            "{wal_get_mp15_saw_status::ELNA gain status[0x%x].Bit:[0-3]c0,[4-7]c1,2g/5g}",
            *saw_status);
#if defined(CONFIG_HUAWEI_OHOS_DSM) || defined(CONFIG_HUAWEI_DSM)
        hw_mpxx_dsm_client_notify(SYSTEM_TYPE_WIFI, DSM_WIFI_SAWERROR, "%s: saw error,error_chain=0x%x",
            __FUNCTION__, *saw_status);
#endif
    }
...
OAL_STATIC void wal_get_mp12_saw_status(oal_net_device_stru *net_dev, int32_t *saw_status)
{
}

OAL_STATIC void wal_get_mp16_saw_status(oal_net_device_stru *net_dev, int32_t *saw_status)
{
    mp1x_wlan_cali_data_para_stru *cali_data;
    uint32_t *check_hw_status = NULL;
    uint8_t rf_id;
    ...
    check_hw_status = cali_data->common_cali_data.cali_ctl_data.check_hw_status;
    /* 没做检测则不获取检测结果 */
    if (cali_data->common_cali_data.cali_ctl_data.cali_update_info.saw_det_flag == OAL_FALSE) {
        return;
    }
    cali_data->common_cali_data.cali_ctl_data.cali_update_info.saw_det_flag = OAL_FALSE;
    for (rf_id = 0; rf_id < HD_EVENT_RF_NUM; rf_id++) {
        *saw_status |=
            (int32_t)(((check_hw_status[rf_id] & WLAN_SAW_STATUS_MASK) >> WLAN_SAW_STATUS_OFFSET) <<
            (BIT_OFFSET_4 * rf_id));
    }
    if (*saw_status != 0) {
        oam_error_log1(0, OAM_SF_ANY,
            "{wal_get_mp16_saw_status::saw status[0x%x].Bit:[0-3]c0,[4-7]c1,[8-11]c2,[12-15]c3,6g/5g/2g}",
            *saw_status);
        /* DMD上报复用ilna */
#if defined(CONFIG_HUAWEI_OHOS_DSM) || defined(CONFIG_HUAWEI_DSM)
        hw_mpxx_dsm_client_notify(SYSTEM_TYPE_WIFI, DSM_WIFI_SAWERROR, "%s: saw error,error_chain=0x%x",
...
void wal_atcmsrv_ioctl_get_fem_pa_status(oal_net_device_stru *pst_net_dev, int32_t *pl_fem_pa_status)
{
    ...
    if (l_chip_type == BOARD_VERSION_MP12) {
        wal_get_mp12_fem_pa_status(pst_net_dev, pl_fem_pa_status);
...
void wal_atcmsrv_ioctl_get_lna_gain_status(oal_net_device_stru *net_dev, int32_t *lna_gain_status)
{
    ...
    l_chip_type = get_mpxx_subchip_type();
    if (l_chip_type == BOARD_VERSION_MP12) {
        wal_get_mp12_lna_gain_status(net_dev, lna_gain_status);
    } else if (l_chip_type >= BOARD_VERSION_MP16) {
...
void wal_atcmsrv_ioctl_get_elna_gain_status(oal_net_device_stru *net_dev, int32_t *elna_gain_status)
{
    ...
    l_chip_type = get_mpxx_subchip_type();
    if (l_chip_type == BOARD_VERSION_MP12) {
        wal_get_mp12_elna_gain_status(net_dev, elna_gain_status);
    } else if (l_chip_type >= BOARD_VERSION_MP16) {
...
void wal_atcmsrv_ioctl_get_saw_status(oal_net_device_stru *net_dev, int32_t *saw_status)
{
    int32_t l_chip_type = get_mpxx_subchip_type();
    if (l_chip_type == BOARD_VERSION_MP12) {
        wal_get_mp12_saw_status(net_dev, saw_status);
    } else if (l_chip_type >= BOARD_VERSION_MP16) {
        wal_get_mp16_saw_status(net_dev, saw_status);
    } else if (l_chip_type == BOARD_VERSION_MP15) {
        wal_get_mp15_saw_status(net_dev, saw_status);
    }
}
#ifdef PLATFORM_DEBUG_ENABLE
 ...
 * 日    期  : 2019年7月29日
 */
int32_t wal_atcmsrv_ioctl_get_rx_pckg_old(oal_net_device_stru *net_dev, int32_t *rx_pckg_succ_num)
{
    int32_t l_ret;
    ...
    mac_cfg_mode_param_stru *pst_mode_param, int32_t l_mode)
{
    ...
    switch (l_mode) {
        ...
        case WLAN_HE_MODE_2G:
            ...
            break;
        case WLAN_EHT_MODE_5G:
        case WLAN_EHT_MODE_2G:
            pst_mode_param->en_protocol = WLAN_EHT_MODE;
            ret = wal_hipriv_set_mcast_data_dscr_param(pst_net_dev, mcast_data);
            if (ret != OAL_SUCC) {
                oam_error_log0(0, 0, "wal_hipriv_set_mcast_data_dscr_param set failed\n");
                return -OAL_EINVAL;
...
OAL_STATIC int32_t wal_atcmsrv_set_bw(uint8_t *pen_bw)
{
    ...
    errno_t err;
    if ((g_l_bandwidth >= WLAN_BAND_WIDTH_160PLUSPLUSPLUS) && (g_l_bandwidth <= WLAN_BAND_WIDTH_160MINUSMINUSMINUS)) {
        err = strncpy_s(ac_bw_str, sizeof(ac_bw_str), c_bw160, oal_min(oal_array_size(ac_bw_str), OAL_STRLEN(c_bw160)));
    } else if ((g_l_bandwidth >= WLAN_BAND_WIDTH_80PLUSPLUS) && (g_l_bandwidth <= WLAN_BAND_WIDTH_80MINUSMINUS)) {
        err = strncpy_s(ac_bw_str, sizeof(ac_bw_str), c_bw80, oal_min(oal_array_size(ac_bw_str), OAL_STRLEN(c_bw80)));
    } else if ((g_l_bandwidth >= WLAN_BAND_WIDTH_40PLUS) && (g_l_bandwidth <= WLAN_BAND_WIDTH_40MINUS)) {
...
#ifdef PLATFORM_DEBUG_ENABLE
/*
 * 函 数 名  : wal_atcmsrv_judge_pll_state
 * 功能描述  : 长发时检测PLL1状态
 * 1.日    期  : 2023年9月18日
  *   修改内容  : 新生成函数
 */
OAL_STATIC int32_t wal_atcmsrv_judge_pll_state(void)
{
    uint8_t *mem_addr = (uint8_t *)get_cali_data_buf_addr();
    mp1x_wlan_cali_data_para_stru *cali_data_mp1x = (mp1x_wlan_cali_data_para_stru *)mem_addr;
    mp12_wlan_cali_data_para_stru *cali_data_mp12 = (mp12_wlan_cali_data_para_stru *)mem_addr;
    int32_t type = get_mpxx_subchip_type();
    OAL_STATIC int8_t count = 0;
    ...
    if (type == BOARD_VERSION_MP12) {
        if (cali_data_mp12->common_cali_data.cali_ctl_data.pll1_err_flag == 0) {
            count = 0;
            return OAL_SUCC;
        } else {
            oam_error_log2(0, OAM_SF_CALIBRATE, "wal_atcmsrv_judge_pll_state::pll1_err_flag[%d],count[%d]",
                cali_data_mp12->common_cali_data.cali_ctl_data.pll1_err_flag, count);
            if (++count >= WAL_ATCMD_JUDGE_PLL_STATE_TIME) {
                cali_data_mp12->common_cali_data.cali_ctl_data.pll1_err_flag = 0;
                count = 0;
            }
            return -OAL_EFAIL;
        }
    } else {
        if (cali_data_mp1x->common_cali_data.cali_ctl_data.pll1_err_flag == 0) {
            count = 0;
            return OAL_SUCC;
        } else {
            oam_error_log2(0, OAM_SF_CALIBRATE, "wal_atcmsrv_judge_pll_state::pll1_err_flag[%d],count[%d]",
                cali_data_mp1x->common_cali_data.cali_ctl_data.pll1_err_flag, count);
            if (++count >= WAL_ATCMD_JUDGE_PLL_STATE_TIME) {
                cali_data_mp1x->common_cali_data.cali_ctl_data.pll1_err_flag = 0;
                count = 0;
            }
            return -OAL_EFAIL;
        }
                                                   ...
                                                   wal_atcmdsrv_wifi_priv_cmd_stru *pst_priv_cmd)
{
    ...
    ***************************************************************************/
    ...
    return wal_atcmsrv_judge_pll_state();
}
#endif
/*
 * 功能描述  : 转换dbb版本号
...
OAL_STATIC int32_t wal_atcmsrv_ioctl_lte_gpio_mode(oal_net_device_stru *pst_net_dev, int32_t l_check_lte_gpio_step)
{
    ...
    *(int32_t *)(st_write_msg.auc_value) = l_check_lte_gpio_step;
    ...
    i_leftime = oal_wait_event_interruptible_timeout_m(pst_hmac_vap->query_wait_q,
        (uint32_t)(pst_hmac_vap->st_atcmdsrv_get_status.uc_lte_gpio_check_flag == OAL_TRUE),
        WAL_ATCMDSRB_DBB_NUM_TIME);
    if (i_leftime == 0) {
        /* 超时还没有上报扫描结束 */
    ...
    wal_atcmdsrv_wifi_priv_cmd_stru *pst_priv_cmd)
{
    ...
    if (l_chip_type == BOARD_VERSION_MP12) {
        wal_get_mp12_fem_pa_status(pst_net_dev, (&(pst_priv_cmd->pri_data.l_fem_pa_status)));
    } else if (l_chip_type >= BOARD_VERSION_MP16) {
                                                 ...
                                                 wal_atcmdsrv_wifi_priv_cmd_stru *pst_priv_cmd)
{
    ...
    int8_t pc_param_mp16[][NUM_9_BYTES] = { "3 0001 0", "3 0010 0", "3 0100 0", "3 1000 0",
                                            "3 1111 0", "3 0011 0", "3 1100 0" };
                                                       ...
                                                       wal_atcmdsrv_wifi_priv_cmd_stru *pst_priv_cmd)
{
    ...
    if (memcpy_s(c_pd_param, sizeof(pst_priv_cmd->pri_data.ac_pd_tran_param),
        pc_pd_info, oal_min(WAL_ATCMDSRV_NV_WINVRAM_LENGTH, CUS_PARAMS_LEN_MAX)) != EOK) {
                                                  ...
                                                  wal_atcmdsrv_wifi_priv_cmd_stru *pst_priv_cmd)
{
    if (g_wlan_chip_ops->set_tas_ant_switch != NULL) {
        g_wlan_chip_ops->set_tas_ant_switch(pst_net_dev, pst_priv_cmd->pri_data.l_tas_ant_set);
    }
    ...
    struct wal_atcmdsrv_wifi_connect_info *connect_info)
{
    ...
    if (memcpy_s(connect_info->auc_ssid, WLAN_SSID_MAX_LEN,
        scanned_bss_info->st_bss_dscr_info.ac_ssid, WLAN_SSID_MAX_LEN) != EOK) {
    ...
    wal_atcmdsrv_wifi_priv_cmd_stru *pst_priv_cmd)
{
    ...
    hmac_vap_stru *pst_hmac_vap = NULL;
    /* 检查是否设置校准通道 */
    if (pst_priv_cmd->pri_data.dpd_cali_info.cali_step == CONN_DPD_CALI_SAVE_NV) {
        return conn_nve_write_ram_sync_to_emmc();
    } else if (pst_priv_cmd->pri_data.dpd_cali_info.cali_step == CONN_DPD_CALI_GET_CHN) {
        pst_priv_cmd->pri_data.dpd_cali_info.cali_chn = conn_nve_get_data_status(CONN_STATUS_DPD, OAL_TRUE);
        return OAL_SUCC;
    } else if (pst_priv_cmd->pri_data.dpd_cali_info.cali_step == CONN_DPD_CALI_SET_CHN) {
        hwifi_config_set_cali_chan(pst_priv_cmd->pri_data.dpd_cali_info.cali_chn);
        return OAL_SUCC;
    } else if (pst_priv_cmd->pri_data.dpd_cali_info.cali_step == CONN_DPD_CALI_CLEAR_RAM) {
        conn_nve_clear_by_cmd(pst_priv_cmd->pri_data.dpd_cali_info.cali_chn);
        return OAL_SUCC;
    }
    // CONN_DPD_CALI_SAVE_RAM处理
    if (pst_mac_vap == NULL) {
        return -OAL_EINVAL;
    }
    ...
    wal_atcmdsrv_wifi_priv_cmd_stru *pst_priv_cmd)
{
    ...
    if (memcpy_s(st_write_msg.auc_value, sizeof(nv_pow_stru_gf61),
        &g_pow_cail, sizeof(nv_pow_stru_gf61)) != EOK) {
                                              ...
                                              wal_atcmdsrv_wifi_priv_cmd_stru *pst_priv_cmd)
{
    ...
    /***************************************************************************
                                抛事件到wal层处理
    ***************************************************************************/
    if (memcpy_s(st_write_msg.auc_value, sizeof(wal_atcmdsrv_ssi_stru),
        &st_ssi_stru, sizeof(wal_atcmdsrv_ssi_stru)) != EOK) {
...
OAL_STATIC int32_t wal_atcmsrv_ioctl_get_ncid(oal_net_device_stru *pst_net_dev,
                                              wal_atcmdsrv_wifi_priv_cmd_stru *pst_priv_cmd)
{
    int32_t ret;
    int32_t *ncid_buff = pst_priv_cmd->pri_data.ncid_info.ncid_list;
    uint32_t ncid_num;
    uint32_t cmd = pst_priv_cmd->pri_data.ncid_info.cmd_type;
    if (cmd == WAL_ATCMDSRV_GET_BOARD_NCID_CMD) {
        ret = mpxx_get_ncid_from_dts(ncid_buff);
        pst_priv_cmd->pri_data.ncid_info.ncid_num = 1;
    } else if (cmd == WAL_ATCMDSRV_GET_SUPPORT_NCID_CMD) {
        ret = mpxx_get_support_ncid_from_dts(ncid_buff, WAL_ATCMDSRV_BOARD_NCID_MAX_NUM, &ncid_num);
        pst_priv_cmd->pri_data.ncid_info.ncid_num = ncid_num;
    } else {
        oam_warning_log1(0, OAM_SF_ANY, "{wal_atcmsrv_ioctl_get_ncid::cmd[%d] is not supported!}", cmd);
        return -OAL_EINVAL;
    }

    if (oal_unlikely(ret != OAL_SUCC)) {
        oam_warning_log1(0, OAM_SF_ANY, "{wal_atcmsrv_ioctl_get_ncid::return err code %d!}", ret);
        return ret;
    }
...
OAL_STATIC int32_t wal_atcmdsrv_ioctl_enhance_pcie_test(oal_net_device_stru *pst_net_dev,
                                                        wal_atcmdsrv_wifi_priv_cmd_stru *pst_priv_cmd)
{
    int32_t ret;
    if (pst_priv_cmd->pri_data.enhance_pcie_test.cmd_type == 0) {
        ret = conn_test_pcie_loop_enhance_stop(&pst_priv_cmd->pri_data.enhance_pcie_test.result);
    } else {
        ret = conn_test_pcie_loop_enhance_start(pst_priv_cmd->pri_data.enhance_pcie_test.timeout,
                                                pst_priv_cmd->pri_data.enhance_pcie_test.interval);
    }

    if (oal_unlikely(ret != OAL_SUCC)) {
        oam_warning_log1(0, OAM_SF_ANY, "{wal_atcmdsrv_ioctl_enhance_pcie_test::return err code %d!}", ret);
        return ret;
    }
...
#ifdef CONFIG_IODIE_HKADC
    { WAL_ATCMDSRV_IOCTL_CMD_GET_NCID,           wal_atcmsrv_ioctl_get_ncid,               1 },
#endif
    {WAL_ATCMDSRV_IOCTL_CMD_ENHANCE_PCCE_TEST,   wal_atcmdsrv_ioctl_enhance_pcie_test,     1 },
};
...
int32_t wal_atcmdsrv_wifi_priv_cmd(oal_net_device_stru *pst_net_dev, oal_ifreq_stru *pst_ifr, int32_t cmd)
{
    ...
    for (i = 0; i < arr_size; i++) {
        if (g_ast_wal_atcmd_table[i].l_cmd_id == st_priv_cmd.cmd) {
            break;
        }
    }
    if (i >= arr_size) {
    ...
    if (g_ast_wal_atcmd_table[i].p_func != NULL) {
        l_ret = g_ast_wal_atcmd_table[i].p_func(pst_net_dev, &st_priv_cmd);
        if (g_ast_wal_atcmd_table[i].l_copy_flag) {
            if (oal_copy_to_user(pst_ifr->ifr_data, &st_priv_cmd, sizeof(wal_atcmdsrv_wifi_priv_cmd_stru)) != EOK) {
                oam_error_log0(0, OAM_SF_CFG, "{wal_atcmdsrv_wifi_priv_cmd::Failed to copy data to user!}");
            }
        }
        return l_ret;
    }

    oam_error_log1(0, 0, "wal_atcmdsrv_wifi_priv_cmd::p_func not register, cmd_idx is %d", i);
    return -OAL_EINVAL;
}
#endif
