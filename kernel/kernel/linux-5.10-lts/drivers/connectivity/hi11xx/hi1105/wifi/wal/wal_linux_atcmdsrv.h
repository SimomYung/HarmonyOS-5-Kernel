...
#define WAL_ATCMDSRV_BOARD_NCID_MAX_NUM 16
...
#define WLAN_EHT_MODE_5G                (WLAN_EHT_MODE + 1)
#define WLAN_EHT_MODE_2G                (WLAN_EHT_MODE + 2)
...
enum WAL_ATCMDSRV_IOCTL_CMD {
    ...
    WAL_ATCMDSRV_IOCTL_CMD_GET_NCID,
    WAL_ATCMDSRV_IOCTL_CMD_ENHANCE_PCCE_TEST,
...
typedef struct {
    uint32_t cmd_type;
    int32_t ncid_list[WAL_ATCMDSRV_BOARD_NCID_MAX_NUM];
    uint32_t ncid_num;
} board_ncid_info;
...
typedef struct {
    ...
    uint32_t timeout;
    uint32_t interval;
    hcc_test_enhance_result result;
} enhance_pcie_test_info;
...
typedef struct wal_atcmdsrv_wifi_priv_cmd {
    ...
    union {
        ...
        board_ncid_info ncid_info;
        enhance_pcie_test_info enhance_pcie_test;
        uint8_t lp_mode;
    } pri_data;
