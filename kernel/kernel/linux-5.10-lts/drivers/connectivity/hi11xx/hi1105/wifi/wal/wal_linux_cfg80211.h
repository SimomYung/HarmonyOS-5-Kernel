...
#define WAL_SCAN_TIME_AP_UP_CONNECT_USER_EXTRA_TIME (1000) /* APUT关联用户场景上报扫描结果超时时间延长1s */
...
#define WLA_CHBA_LOW_PRIORITY_MGMT_TX_TIMEOUT_MSEC 100 /* WAL 发送chba低优先级管理帧超时时间 */
#define WAL_MGMT_TX_RETRY_CNT 8      /* WAL 发送管理帧最大重传次数 */
#define WAL_CFG80211_REMAIN_ON_CHAN_TIMEOUT_MS 100 /* wal_cfg80211_remain_on_channel默认扫描时长 */
...
typedef struct {
    uint32_t duration;
    oal_bool_enum_uint8 continue_wait_after_tx;
    wlan_ieee80211_roc_type_uint8 tx_roc;
    uint8_t resv[2];
} wal_mgmt_tx_remain_on_chan_params_stru;
...
void wal_chba_start_ap_set_beacon_info(oal_net_device_stru *netdev, mac_beacon_param_stru *beacon_param);
#endif
...
int32_t wal_cfg80211_add_key(oal_wiphy_stru *pst_wiphy, oal_net_device_stru *pst_netdev, int32_t link_id,
    uint8_t uc_key_index, bool en_pairwise, const uint8_t *puc_mac_addr, oal_key_params_stru *pst_params);
int32_t wal_cfg80211_get_key(oal_wiphy_stru *pst_wiphy, oal_net_device_stru *pst_netdev, int32_t link_id,
...
int32_t wal_cfg80211_remove_key(oal_wiphy_stru *pst_wiphy, oal_net_device_stru *pst_netdev, int32_t link_id,
    uint8_t uc_key_index, bool en_pairwise, const uint8_t *puc_mac_addr);
int32_t wal_cfg80211_set_default_key(oal_wiphy_stru *pst_wiphy, oal_net_device_stru *pst_netdev, int32_t link_id,
