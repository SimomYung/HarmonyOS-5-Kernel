    ...
    oal_net_device_stru *net_dev, const uint8_t *mac_addr, oal_station_info_stru *sta_info)
{
    ...
    if (oal_any_null_ptr4(wiphy, net_dev, mac_addr, sta_info)) {
    ...
    hmac_vap = (hmac_vap_stru *)mac_res_get_hmac_vap(uc_vap_id);
    if (hmac_vap == NULL) {
        oam_error_log0(uc_vap_id, OAM_SF_ANY, "{wal_cfg80211_get_station::mac_res_get_hmac_vap fail.}");
        return -OAL_EINVAL;
    }

    if (wal_cfg8211_get_station_get_real_mac_addr(hmac_vap, mac_addr, real_macaddr, WLAN_MAC_ADDR_LEN) != OAL_SUCC) {
        return -OAL_EINVAL;
    }

    /* 固定时间内最多更新一次RSSI */
    if (wal_cfg80211_get_station_filter(&hmac_vap->st_vap_base_info, (uint8_t *)real_macaddr) == OAL_FALSE) {
        wal_cfg80211_fill_station_info(sta_info, &hmac_vap->station_info);
        wal_cfg80211_fill_station_info_ext(sta_info, hmac_vap);
        return OAL_SUCC;
    }
    ...
    ********************************************************************************/
    wal_cfg80211_get_station_set_query_msg(hmac_vap, real_macaddr, OAL_MAC_ADDRESS_LEN, &write_msg);
    /* 3.1 填写 msg 消息头 */
    wal_write_msg_hdr_init(&write_msg, WLAN_CFGID_QUERY_STATION_STATS, sizeof(dmac_query_station_info_request_event));
    l_ret = wal_send_cfg_event(net_dev, WAL_MSG_TYPE_WRITE, WAL_MSG_WRITE_MSG_HDR_LENGTH +
    ...
    return wal_cfg80211_get_station_check_left_time(sta_info, hmac_vap);
}
