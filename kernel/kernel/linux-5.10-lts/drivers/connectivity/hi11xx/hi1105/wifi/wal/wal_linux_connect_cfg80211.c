...
#define WAL_AI_CSMA_4_BIT 0x0F
#define WAL_AI_CSMA_BIT_INDEX 4
...
void wal_cfg80211_connect_wait_scan_complete(oal_net_device_stru *net_device)
{
...
#if (_PRE_OS_VERSION_LINUX == _PRE_OS_VERSION)
    wlan_pm_set_host_timeout(wal_get_net_device_id(pst_net_device), WLAN_SLEEP_LONG_CHECK_CNT);
#endif
...
uint32_t wal_set_netdevice_mac_addr(oal_net_device_stru *net_device, uint8_t *mac_addr, uint32_t len)
{
    mac_vap_stru *mac_vap = NULL;
    ...
    mac_mib_set_StationID(mac_vap, mac_addr);
    wal_set_random_mac_to_mib_event(net_device, mac_vap, WLAN_LEGACY_VAP_MODE);

    oam_warning_log3(mac_vap->uc_vap_id, OAM_SF_ANY,
        "{wal_set_netdevice_mac_addr::mac_addr %02X:XX:XX:XX:%02X:%02X}",
        mac_addr[BYTE_OFFSET_0], mac_addr[BYTE_OFFSET_4], mac_addr[BYTE_OFFSET_5]);

    return OAL_SUCC;
}

void wal_cfg80211_disconnect_reconfig_vap_mac_addr(oal_net_device_stru *net_device)
{
    uint32_t idx;
    ...
    uint8_t link_mac_addr[WLAN_MAC_ADDR_LEN] = { 0 };
    mac_vap_stru *mac_vap = NULL;
    ...
    ml_net_dev_priv = (oal_netdev_priv_stru *)oal_net_dev_wireless_priv(net_device);
    if (ml_net_dev_priv == NULL || ml_net_dev_priv->mlo_cfg_info.is_ml_netdev == OAL_FALSE) {
        return;
    }

    oal_set_mac_addr(primary_mac_addr, oal_netdevice_mac_addr(net_device));
    oal_set_mac_addr(link_mac_addr, primary_mac_addr);

    /* 更新ml网络设备对应vap的mac地址 */
    wal_set_netdevice_mac_addr(net_device, primary_mac_addr, WLAN_MAC_ADDR_LEN);

    /* 更新link网络设备对应vap的mac地址 */
    for (idx = 0; idx < WLAN_MAX_ML_LINK_NUM; idx++) {
        ...
        link_mac_addr[WLAN_MAC_ADDR_LEN - 1] += 1; /* link mac地址最后一位，相较于wlan0 mac地址+1 */
        wal_set_netdevice_mac_addr(link_netdev, link_mac_addr, WLAN_MAC_ADDR_LEN);
    }
}
 ...
 *   修改内容  : 新生成函数
 */
int32_t wal_cfg80211_disconnect(oal_wiphy_stru *pst_wiphy, oal_net_device_stru *pst_net_device, uint16_t us_reason_code)
{
    ...
    if (is_sta(pst_mac_vap) != OAL_TRUE) {
        oam_error_log1(0, 0, "{wal_cfg80211_disconnect::wrong mac vap mode [%d]!}", pst_mac_vap->en_vap_mode);
        return OAL_SUCC;
    }
    /* 填写和sta关联的ap mac 地址 */
    ...
    if (wal_cfg80211_start_disconnect(pst_net_device, &st_mac_cfg_kick_user_param) != OAL_SUCC) {
        return -OAL_EFAIL;
    }

    wal_cfg80211_disconnect_reconfig_vap_mac_addr(pst_net_device); /* 断联，重新配置ml_vap/link_vap mac地址 */
    return OAL_SUCC;
}
/*
...
void wal_parse_wpa_wpa2_ie(oal_beacon_parameters *pst_beacon_info, mac_beacon_param_stru *pst_beacon_param)
{
    ...
    ies_info.frame_ies = pst_beacon_info->tail;
    ies_info.frame_len = pst_beacon_info->tail_len;
    ...
    if (us_capability_info & WLAN_WITP_CAPABILITY_PRIVACY) {
        ...
        /* 查找 RSN 信息元素 */
        puc_ie_tmp = mac_find_ie(MAC_EID_RSN, &ies_info);
        if (puc_ie_tmp != NULL) {
        ...
        puc_ie_tmp = mac_find_vendor_ie(MAC_WLAN_OUI_MICROSOFT, MAC_OUITYPE_WPA, &ies_info);
        if (puc_ie_tmp != NULL) {
                          ...
                          mac_vap_stru *pst_mac_vap, oal_beacon_parameters *pst_beacon_info)
{
    ...
    ies_info.frame_len = pst_beacon_info->tail_len;
    /*  查找wmm_ie  */
    puc_wmm_ie = mac_find_vendor_ie(MAC_WLAN_OUI_MICROSOFT, MAC_WLAN_OUI_TYPE_MICROSOFT_WMM, &ies_info);
    if (puc_wmm_ie == NULL) {
        /* wmm ie未找到，则说明wmm 关 */
        uc_wmm = OAL_FALSE;
...
OAL_STATIC oal_bool_enum_uint8 wal_start_ap_parse_beacon_protection(mac_vap_stru *mac_vap,
    uint8_t *beacon, uint32_t beacon_len)
{
    uint8_t *ie = NULL;
    mac_ext_cap_ie_stru *ext_cap = NULL;
    mac_ies_info ies_info = {
        .frame_ies = beacon,
        .frame_len = beacon_len,
    };
    ie = mac_find_ie(MAC_EID_EXT_CAPS, &ies_info);
    if (ie == NULL) {
        return OAL_FALSE;
    }
    /* Bit 84 - Beacon Protection Enabled, 10: Beacon Protection在第10字节, 11: ext_cap字段的长度 */
    if (ie[MAC_IE_EID_LEN] < 11) {
        return OAL_FALSE;
    }
    ext_cap = (mac_ext_cap_ie_stru *)(ie + MAC_IE_HDR_LEN);
    oam_warning_log1(mac_vap->uc_vap_id, OAM_SF_ANY,
        "{wal_start_ap_parse_beacon_protection::beacon protection enabled %d}}",
        ext_cap->bit_beacon_protection_enabled);
    return ext_cap->bit_beacon_protection_enabled;
}
    ...
    oal_beacon_data_stru *pst_beacon_info, mac_beacon_param_stru *pst_beacon_param)
{
    ...
    /* 解析是否隐藏SSID,隐藏SSID len 字段长度为0 */
    pst_beacon_param->uc_hidden_ssid = (puc_beacon_info_tmp[WAL_BCN_BSSID_LENGTH_ADDR] == 0) ? 1 : 0;
    ...
    pst_beacon_param->beacon_protection_enabled = wal_start_ap_parse_beacon_protection(pst_mac_vap,
        puc_beacon_info_tmp, beacon_head_len + beacon_tail_len);
    /* 释放临时申请的内存 */
    oal_free(puc_beacon_info_tmp);
    return OAL_SUCC;
}
    ...
    oal_beacon_data_stru *pst_beacon_info)
{
    ...
    /* 填写 msg 消息体 */
    if (memcpy_s(st_write_msg.auc_value, sizeof(mac_beacon_param_stru),
        &st_beacon_param, sizeof(mac_beacon_param_stru)) != EOK) {
    ...
    )
{
    ...
    oam_warning_log4(pst_mac_vap->uc_vap_id, OAM_SF_ANY,
        "{wal_cfg80211_del_station:del user[%02x:XX:XX:XX:%02x:%02x], reason_code[%d]}",
        st_kick_user_param.auc_mac_addr[BYTE_OFFSET_0], st_kick_user_param.auc_mac_addr[BYTE_OFFSET_4],
        st_kick_user_param.auc_mac_addr[BYTE_OFFSET_5], st_kick_user_param.us_reason_code);
...
OAL_STATIC void wal_cfg80211_fill_station_info_ext_ai_info(oal_station_info_stru *sta_info,
    hmac_vap_stru *hmac_vap)
{
#ifdef CONFIG_HW_EXT_AI_WIFI
    sta_info->filled |= BIT(NL80211_STA_INFO_TX_PPDU_CNT);
    sta_info->tx_ppdu_cnt = hmac_vap->station_info_extend2.tx_stats.tx_mpdu_cnt;

    sta_info->filled |= BIT(NL80211_STA_INFO_TX_PPDU_RETRY_CNT);
    sta_info->tx_ppdu_retry_cnt = hmac_vap->station_info_extend2.tx_stats.tx_retry_mpdu_cnt;
    sta_info->filled |= BIT(NL80211_STA_INFO_TX_MCS);
    sta_info->tx_mcs = hmac_vap->station_info.txrate.mcs;
    sta_info->filled |= BIT(NL80211_STA_INFO_CWMAX);
    sta_info->cw_max = (hmac_vap->station_info_extend2.tx_stats.edca_para[0] >> WAL_AI_CSMA_BIT_INDEX) &
        WAL_AI_CSMA_4_BIT;
    sta_info->filled |= BIT(NL80211_STA_INFO_CWMIN);
    sta_info->cw_min = hmac_vap->station_info_extend2.tx_stats.edca_para[0] & WAL_AI_CSMA_4_BIT;
    sta_info->filled |= BIT(NL80211_STA_INFO_ULDELAY_CDF);
    if (memcpy_s(sta_info->uldelay_cdf, sizeof(sta_info->uldelay_cdf),
        hmac_vap->station_info_extend2.tx_stats.uplink_delay,
        sizeof(hmac_vap->station_info_extend2.tx_stats.uplink_delay)) != EOK) {
        return;
    }
    sta_info->filled |= BIT(NL80211_STA_INFO_TX_TIME_CDF);
    if (memcpy_s(sta_info->txtime_cdf, sizeof(sta_info->txtime_cdf),
        hmac_vap->station_info_extend2.tx_stats.tx_time,
        sizeof(hmac_vap->station_info_extend2.tx_stats.tx_time)) != EOK) {
        return;
    }
#endif
}
    ...
    hmac_vap_stru *hmac_vap)
{
    ...
    sta_info->filled |= BIT(NL80211_STA_INFO_SIGNAL_DUAL);
    sta_info->rpt_signal[0] = hmac_vap->station_info_extend2.rpt_rssi[0];
    sta_info->rpt_signal[1] = hmac_vap->station_info_extend2.rpt_rssi[1];
#endif
}
    ...
    hmac_vap_stru *hmac_vap)
{
    ...
    (hmac_vap->station_info_query_completed_flag == OAL_TRUE), QUERY_STATION_INFO_TIME);
    if (i_leftime == 0) {
        /* 超时还没有上报扫描结束 */
        ...
        wal_cfg80211_fill_station_info_ext_ai_info(sta_info, hmac_vap);
...
OAL_STATIC void wal_cfg80211_get_station_set_query_msg(hmac_vap_stru *hmac_vap, const uint8_t *mac_addr,
    uint32_t macaddr_len, wal_msg_write_stru *write_msg)
{
    dmac_query_station_info_request_event *query_station_info =
        (dmac_query_station_info_request_event *)write_msg->auc_value;
    query_station_info->query_event = OAL_QUERY_STATION_INFO_EVENT;
    if (macaddr_len != OAL_MAC_ADDRESS_LEN) {
        /* mac地址长度错误 */
        oam_warning_log1(0, OAM_SF_ANY, "{wal_cfg80211_get_station::mac addr len %d invalid!}", macaddr_len);
    }
    oal_set_mac_addr(query_station_info->auc_query_sta_addr, (uint8_t *)mac_addr);

    hmac_vap->station_info_query_completed_flag = OAL_FALSE;
}

OAL_STATIC int32_t wal_cfg8211_get_station_get_real_mac_addr(hmac_vap_stru *hmac_vap,
    const uint8_t *puc_mac, uint8_t *real_macaddr, uint32_t macaddr_len)
{
    mac_user_stru *mac_user = NULL;
    mac_vap_stru *mac_vap = &hmac_vap->st_vap_base_info;

    memcpy_s(real_macaddr, macaddr_len, puc_mac, macaddr_len);

    if (is_ap(mac_vap)) {
        return OAL_SUCC;
    }
    // 后续上层根据link0-link1的mac来获取sta info时，需要将下面的逻辑删掉。
    if (hmac_vap_is_legacy_mld_mode(hmac_vap) == OAL_TRUE) {
        return OAL_SUCC;
    }
    mac_user = mac_res_get_mac_user(mac_vap->us_assoc_vap_id);
    if (mac_user == NULL) {
        oam_warning_log3(mac_vap->uc_vap_id, OAM_SF_MLD,
            "{wal_cfg80211_get_station:: no mac_user match mac_addr[%02x:XX:XX:XX:%02x:%02x]}",
            puc_mac[MAC_ADDR_0], puc_mac[MAC_ADDR_4], puc_mac[MAC_ADDR_5]);
        return -OAL_EINVAL;
    }
    // 使用主link的mac获取sta info
    memcpy_s(real_macaddr, macaddr_len, mac_user->auc_user_mac_addr, macaddr_len);
    return OAL_SUCC;
}

static int32_t wal_get_station_find_query_vap_correspond_mac_addr(hmac_vap_stru *hmac_vap,
    const uint8_t *mac_addr, uint8_t *real_macaddr, oal_net_device_stru **query_net_dev, hmac_vap_stru **query_hmac_vap)
{
    int32_t ret;

#ifdef _PRE_WLAN_FEATURE_EMLSR
    /* emlsr工作状态，mac_addr可能是辅link的地址，需更新net_dev和hmac_vap参数才能正确查询 */
    if (hmac_check_work_in_emlsr(hmac_vap) == OAL_TRUE) {
        ret = hmac_emlsr_str_get_station_change_query_info(
            hmac_vap, mac_addr, real_macaddr, query_net_dev, query_hmac_vap);
        /* 找到mac_addr对应link的信息，则直接进行查询 */
        if (ret == OAL_SUCC) {
            return ret;
        }
    }
    ...
    /* 非emlsr状态、emlsr下查询mac_addr对应link失败 场景，默认查询上报mld_mac_addr对应主link的信息 */
    ret = wal_cfg8211_get_station_get_real_mac_addr(
        *query_hmac_vap, mac_addr, real_macaddr, WLAN_MAC_ADDR_LEN);
    if (ret != OAL_SUCC) {
        oam_error_log0(hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_ANY,
            "{wal_cfg80211_get_station::get real_mac_addr fail!}");
        return -OAL_EINVAL;
    }
 ...
 *   修改内容  : 新生成函数
 */
int32_t wal_cfg80211_get_station(oal_wiphy_stru *wiphy,
    oal_net_device_stru *net_dev, const uint8_t *mac_addr, oal_station_info_stru *sta_info)
{
    oal_net_device_stru *query_net_dev = net_dev;
    mac_vap_stru *mac_vap = NULL;
    hmac_vap_stru *hmac_vap = NULL;
    hmac_vap_stru *query_hmac_vap = NULL;
    wal_msg_write_stru write_msg = {0};
    wal_msg_stru *rsp_msg = NULL;
    uint8_t real_macaddr[WLAN_MAC_ADDR_LEN] = {0};
    int32_t l_ret;
    g_get_station_running_time = (uint32_t)oal_time_get_stamp_ms();

    if (oal_any_null_ptr4(wiphy, query_net_dev, mac_addr, sta_info)) {
        oam_error_log0(0, OAM_SF_ANY, "{wal_cfg80211_get_station::input params null!}");
        return -OAL_EINVAL;
    }

    mac_vap = oal_net_dev_priv(query_net_dev);
    if (mac_vap == NULL) {
        oam_warning_log0(0, OAM_SF_ANY, "{wal_cfg80211_get_station::oal_net_dev_priv, return null!}");
        return -OAL_EINVAL;
    }

    hmac_vap = (hmac_vap_stru *)mac_res_get_hmac_vap(mac_vap->uc_vap_id); /* wlan0对应vap */
    if (hmac_vap == NULL) {
        oam_error_log0(mac_vap->uc_vap_id, OAM_SF_ANY, "{wal_cfg80211_get_station::mac_res_get_hmac_vap fail.}");
        return -OAL_EINVAL;
    }
    query_hmac_vap = hmac_vap;

    /* 判断emlsr状态：
    1、emlsr：根据puc_mac查找对应link的vap、user
    2、非emlsr：根据wlan0对应的vap，查找user
     */
    if (wal_get_station_find_query_vap_correspond_mac_addr(
        hmac_vap, mac_addr, real_macaddr, &query_net_dev, &query_hmac_vap) != OAL_SUCC) {
        return -OAL_EINVAL;
    }

    /* 固定时间内最多更新一次RSSI */
    if (wal_cfg80211_get_station_filter(&query_hmac_vap->st_vap_base_info, (uint8_t *)real_macaddr) == OAL_FALSE) {
        wal_cfg80211_fill_station_info(sta_info, &query_hmac_vap->station_info);
        wal_cfg80211_fill_station_info_ext(sta_info, query_hmac_vap);
        wal_cfg80211_fill_station_info_ext_ai_info(sta_info, hmac_vap);
        return OAL_SUCC;
    }
    ...
    ********************************************************************************/
    wal_cfg80211_get_station_set_query_msg(query_hmac_vap, real_macaddr, OAL_MAC_ADDRESS_LEN, &write_msg);
    /* 3.1 填写 msg 消息头 */
    wal_write_msg_hdr_init(&write_msg, WLAN_CFGID_QUERY_STATION_STATS, sizeof(dmac_query_station_info_request_event));
    l_ret = wal_send_cfg_event(query_net_dev, WAL_MSG_TYPE_WRITE, WAL_MSG_WRITE_MSG_HDR_LENGTH +
        sizeof(dmac_query_station_info_request_event), (uint8_t *)&write_msg, OAL_TRUE, &rsp_msg);
    if (oal_unlikely(l_ret != OAL_SUCC)) {
        oam_warning_log1(mac_vap->uc_vap_id, 0, "{wal_cfg80211_get_station::err code %d!}", l_ret);
        return -OAL_EFAIL;
    }
    /* 读取返回的错误码 */
    ...
    return wal_cfg80211_get_station_check_left_time(sta_info, query_hmac_vap);
}
