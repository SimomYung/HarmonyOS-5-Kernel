    ...
    uint16_t us_len, wlan_cfgid_enum_uint16 en_wid, oal_bool_enum_uint8 en_need_rsp)
{
    ...
    if (memcpy_s(st_write_msg.auc_value, sizeof(st_write_msg.auc_value), ps_param, us_len) != EOK) {
 ...
 *   修改内容  : 新生成函数
 */
int32_t wal_cfg80211_start_connect(oal_net_device_stru *net_dev, hmac_conn_param_stru *mac_conn_param)
{
    return wal_cfg80211_start_req(net_dev,
                                  mac_conn_param,
...
uint32_t wal_cfg80211_do_external_auth(oal_net_device_stru *pst_netdev, hmac_external_auth_req_stru *pst_ext_auth)
{
    return (uint32_t)wal_cfg80211_start_req(pst_netdev, pst_ext_auth,
                                            sizeof(hmac_external_auth_req_stru),
