    ...
    wlan_remain_on_channel_info_stru *remain_on_chan_info)
{
    ...
    if (g_wlan_spec_cfg->feature_same_chan_remain_on_chan_support == OAL_FALSE) {
        return wal_drv_start_mgmt_rx_remain_on_channel(wdev, &chan_info, duration, &cookie,
            IEEE80211_ROC_TYPE_CHBA_MGMT_RX);
    }
...
OAL_STATIC int32_t wal_ioctl_cancel_same_chan_remain_on_chan(oal_net_device_stru *net_dev,
    int8_t *command, wal_wifi_priv_cmd_stru priv_cmd)
{
    uint32_t ret;
    wal_msg_write_stru write_msg = { 0 };
    ...
    uint8_t *pc_cmd, wal_wifi_priv_cmd_stru *priv_cmd)
{
    if (oal_strncasecmp(pc_cmd, CMD_GET_CURRENT_TSF, CMD_GET_CURRENT_TSF_LEN) == 0) {
        ...
        return wal_ioctl_cancel_same_chan_remain_on_chan(pst_net_dev, pc_cmd, *priv_cmd);
