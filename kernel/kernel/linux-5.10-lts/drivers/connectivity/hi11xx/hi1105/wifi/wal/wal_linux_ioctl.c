...
uint32_t wal_init_wlan_netdev(oal_wiphy_stru *pst_wiphy, const char *dev_name);
...
OAL_STATIC const oal_iw_priv_args_stru g_ast_iw_priv_args[] = {
    ...
    { WAL_IOCTL_PRIV_AP_MAC_FLTR, OAL_IW_PRIV_TYPE_CHAR | OAL_IW_PRIV_TYPE1024,
...
const int8_t *g_pauc_11bplus_rate_tbl[WLAN_11BPLUS_RATE_VALUE_BUTT] = {
    "0.5ldpc",
    "1ldpc",
    "2.75ldpc",
    "5.5ldpc",
...
#endif
    {"puncture_en",                  SU_PUNCTURE_ENABLE},
    {"puncture_bitmap",              SU_PUNCTURE_BITMAP},
    { NULL }
};
...
OAL_STATIC const wal_ioctl_tlv_stru g_ast_debug_log_table[] = {
    ...
    { "dur_delta", MAC_LOG_DEVICE_DURATION_DELTA },
    { NULL }
};
...
const uint8_t g_set_str_table_size = oal_array_size(g_ast_set_str_table);
...
uint32_t wal_alloc_cfg_event(oal_net_device_stru *net_dev, frw_event_mem_stru **event_mem, const void *resp_addr,
    wal_msg_stru **cfg_msg, uint16_t len_input)
{
    ...
    uint16_t us_len, oal_bool_enum_uint8 en_need_rsp, wal_msg_stru **response)
{
    ...
    l_ret = oal_wait_event_timeout_m((g_wal_wid_msg_queue.st_wait_queue),
        (oal_bool_enum_uint8)(wal_request_wait_event_condition(msg_request) == OAL_TRUE),
 ...
 *   修改内容  : 新生成函数
 */
int32_t wal_send_cfg_event(oal_net_device_stru *net_dev, uint8_t msg_type, uint16_t us_len,
    uint8_t *puc_param, oal_bool_enum_uint8 en_need_rsp, wal_msg_stru **response)
{
    ...
    ret = wal_alloc_cfg_event(net_dev, &pst_event_mem, ((en_need_rsp == OAL_TRUE) ? &st_msg_request : NULL),
...
void wal_set_netdev_priv(oal_net_device_stru *net_dev, oal_netdev_priv_stru *pst_netdev_priv)
{
    memset_s(&(pst_netdev_priv->mlo_cfg_info), sizeof(mlo_cfg_info_stru), 0, sizeof(mlo_cfg_info_stru));

#if defined(_PRE_CONFIG_HISI_110X_NAPI_DISABLE) || defined(_PRE_WLAN_FEATURE_ESPE)
...
void wal_init_netdev_priv(oal_net_device_stru *net_dev)
{
    ...
    wal_set_netdev_priv(net_dev, (oal_netdev_priv_stru *)oal_net_dev_wireless_priv(net_dev));
}
/*
 * 功能描述  : 开启或关闭ampdu发送功能
 * 1.日    期  : 2013年8月27日
...
OAL_STATIC uint32_t wal_hipriv_get_cmd_net_dev(int8_t *pc_cmd, oal_net_device_stru **net_device, uint32_t *off_set)
{
    ...
    /* 根据dev_name找到dev */
    net_dev = wal_config_get_cmd_netdev(ac_dev_name, OAL_STRLEN(ac_dev_name));
    if (net_dev == NULL) {
    ...
    const char *buf, oal_size_t count)
{
    ...
    if (memcpy_s(pc_cmd, WAL_HIPRIV_CMD_MAX_LEN, buf, len) != EOK) {
...
OAL_STATIC oal_ssize_t wal_hipriv_sys_read(struct kobject *dev, struct kobj_attribute *attr, char *buf)
{
    ...
    for (cmd_idx = 0; cmd_idx < oal_array_size(g_ast_hipriv_cmd); cmd_idx++) {
        buff_index += snprintf_s(buf + buff_index, (SYS_READ_MAX_STRING_LEN - buff_index),
    ...
    char *out_param, signed long out_len)
{
    ...
    if (result != EOK) {
        oam_error_log2(0, OAM_SF_CALIBRATE, "copy fail len[%d] ret[%d]", input_length, ret);
        return OAL_FAIL;
    }
    ...
    int8_t *command, wal_wifi_priv_cmd_stru priv_cmd)
{
    ...
    mac_pkt_filter_cmd_stru pkt_filter_cmd;
    wal_msg_write_stru write_msg;
    wal_msg_stru *rsp_msg = NULL;

    if (wal_ioctl_judge_input_param_length(priv_cmd, CMD_SET_PKT_FILTER_LEN, sizeof(program_len)) != OAL_SUCC) {
    ...
    pkt_filter_cmd.us_program_len = program_len;
    pkt_filter_cmd.puc_program = command + CMD_SET_PKT_FILTER_LEN + 1 + sizeof(program_len);
    pkt_filter_cmd.en_cmd_type = PKT_FILTER_SET_FILTER_CMD;

    /***************************************************************************
        抛事件到wal层处理
    ***************************************************************************/
    wal_write_msg_hdr_init(&write_msg, WLAN_CFGID_SET_PKT_FILTER, sizeof(pkt_filter_cmd));
    if (memcpy_s(write_msg.auc_value, sizeof(write_msg.auc_value),
        &pkt_filter_cmd, sizeof(pkt_filter_cmd)) != EOK) {
        return -OAL_EFAIL;
    }
    ...
    if (wal_send_cfg_event(net_dev, WAL_MSG_TYPE_WRITE, WAL_MSG_WRITE_MSG_HDR_LENGTH +
        sizeof(pkt_filter_cmd), (uint8_t *)&write_msg, OAL_TRUE, &rsp_msg) != OAL_SUCC) {
    ...
    oam_warning_log1(0, 0, "wal_ioctl_priv_cmd_pkt_filter: program len %d", pkt_filter_cmd.us_program_len);
    return OAL_SUCC;
}

/* 功能描述: 检查监听参数是否合法 */
oal_bool_enum_uint8 wal_remain_on_chan_param_check(oal_net_device_stru *net_dev, uint16_t center_freq)
{
    uint8_t channel_num;
    mac_device_stru *mac_device = NULL;
    mac_vap_stru *mac_vap = oal_net_dev_priv(net_dev);
    if (mac_vap == NULL) {
        oam_error_log0(0, OAM_SF_ANY, "{wal_check_remain_on_chan_param:get mac vap failed!}");
        return OAL_FALSE;
    }
    mac_device = mac_res_get_dev(mac_vap->uc_device_id);
    if (mac_device == NULL) {
        oam_error_log1(mac_vap->uc_vap_id, OAM_SF_ANY, "{wal_check_remain_on_chan_param:fail find mac_device[%d]}",
            mac_vap->uc_device_id);
        return OAL_FALSE;
    }

    channel_num = (uint8_t)oal_ieee80211_frequency_to_channel((int32_t)center_freq);
    if (mac_is_channel_num_valid(mac_get_band_by_channel_num(channel_num), channel_num, OAL_FALSE) != OAL_SUCC) {
        oam_error_log1(mac_vap->uc_vap_id, OAM_SF_ANY, "{wal_check_remain_on_chan_param:center_freq[%d]}", center_freq);
        return OAL_FALSE;
    }
    return OAL_TRUE;
}
...
OAL_STATIC int32_t wal_rx_mgmt_same_chan_remain_on_chan(oal_net_device_stru *net_dev, mac_vap_stru *mac_work_vap,
    uint32_t wait_time)
{
    int ret;
    ...
    /***************************************************************************
        抛事件到wal层处理
    ***************************************************************************/
    wal_write_msg_hdr_init(&msg, WLAN_CFGID_RX_START_SAME_CHAN_REMAIN_ON_CHAN,
    ...
    ret = wal_send_cfg_event(net_dev, WAL_MSG_TYPE_WRITE,
        WAL_MSG_WRITE_MSG_HDR_LENGTH + sizeof(mac_same_chan_remain_on_chan_cfg), (uint8_t *)&msg, OAL_FALSE, NULL);
    if (ret != OAL_SUCC) {
        oam_error_log1(0, OAM_SF_ANY, "{wal_rx_mgmt_same_chan_remain_on_chan::fail to start, error[%d]}", ret);
        return -OAL_EFAIL;
    }
...
OAL_STATIC int32_t wal_ioctl_start_rx_mgmt_remain_on_chan_without_work_vap(oal_wireless_dev_stru *wdev,
    struct ieee80211_channel *chan_info, uint32_t duration, uint64_t *cookie)
{
    if (duration == 0) { /* duration=0为长监听命令, 长监听时必须有同信道vap, 不能走扫描监听 */
        oam_warning_log0(0, OAM_SF_ANY,
            "{wal_ioctl_start_rx_mgmt_remain_on_channel:long remain on chan fail to find work vap!}");
        return -OAL_EINVAL;
    }
    return wal_drv_start_mgmt_rx_remain_on_channel(wdev, chan_info, duration, cookie, IEEE80211_ROC_TYPE_CHBA_MGMT_RX);
}

int32_t wal_ioctl_start_rx_mgmt_remain_on_channel(oal_net_device_stru *net_dev,
    wlan_remain_on_channel_info_stru *remain_on_chan_info)
{
    int32_t ret;
    uint64_t cookie;
    uint32_t duration = 0;
    ...
    if (wdev == NULL) {
        oam_warning_log0(0, OAM_SF_P2P, "{wal_ioctl_start_rx_mgmt_remain_on_channel:fail to get wdev!}");
        return -OAL_EFAIL;
    }

    wal_set_rx_mgmt_remain_on_chann_param(remain_on_chan_info, &chan_info, &duration);
    if (wal_remain_on_chan_param_check(net_dev, (int32_t)chan_info.center_freq) == OAL_FALSE) {
        oam_error_log0(0, OAM_SF_ANY, "{wal_ioctl_start_rx_mgmt_remain_on_channel:param is invalid!}");
        return -OAL_EFAIL;
    }
    mac_vap = oal_net_dev_priv(net_dev);
    mac_device = mac_res_get_dev(mac_vap->uc_device_id);
    channel_num = (uint8_t)oal_ieee80211_frequency_to_channel((int32_t)chan_info.center_freq);
    oam_warning_log4(mac_vap->uc_vap_id, OAM_SF_ANY,
        "{wal_ioctl_start_rx_mgmt_remain_on_channel:listen_freq[%d], is_exclusive_mode[%d], bw[%d], time[%d]}",
        chan_info.center_freq, remain_on_chan_info->is_exclusive_mode, chan_info.band, duration);

    /* wait_time=0时为长期同信道监听, 优先级最低，不应干扰已在进行的扫描; 若wait_time不为0, 则应打断已进行的扫描与监听 */
    if (duration != 0) {
        ret = wal_cfg80211_try_scan_abort_before_remain_on_chan(net_dev, mac_device, mac_vap,
            IEEE80211_ROC_TYPE_CHBA_MGMT_RX);
        if (ret != OAL_SUCC) {
            oam_error_log0(0, OAM_SF_ANY, "{wal_ioctl_start_rx_mgmt_remain_on_channel:fail to scan abort}");
            return ret;
        }
    }
    ...
    if (g_wlan_spec_cfg->feature_same_chan_remain_on_chan_support == OAL_FALSE) {
        return wal_ioctl_start_rx_mgmt_remain_on_chan_without_work_vap(wdev, &chan_info, duration, &cookie);
    }
    /* 独占模式直接发起扫描remain_on_chan，通过扫描独占信道 */
    if (remain_on_chan_info->is_exclusive_mode == OAL_TRUE) {
        return wal_ioctl_start_rx_mgmt_remain_on_chan_without_work_vap(wdev, &chan_info, duration, &cookie);
    }
    /* 在目标信道查找已工作vap, 能找到便发起同信道监听, 找不到则按通用扫描流程发起remain_on_channel */
    mac_work_vap = mac_find_listen_chan_work_vap(mac_device, mac_vap, OAL_TRUE, channel_num);
    if (mac_work_vap == NULL) {
        return wal_ioctl_start_rx_mgmt_remain_on_chan_without_work_vap(wdev, &chan_info, duration, &cookie);
    } else {
        return wal_rx_mgmt_same_chan_remain_on_chan(net_dev, mac_work_vap, duration);
    }
}
    ...
    int8_t *command, wal_wifi_priv_cmd_stru priv_cmd)
{
    ...
    return wal_ioctl_start_rx_mgmt_remain_on_channel(net_dev, remain_on_chan_info);
}

int32_t wal_ioctl_cancel_same_chan_remain_on_chan(oal_net_device_stru *net_dev)
{
    uint32_t ret;
    wal_msg_write_stru write_msg = { 0 };

    if (net_dev == NULL) {
        oam_error_log0(0, OAM_SF_ANY, "{wal_ioctl_cancel_same_chan_remain_on_chan:net_dev is NULL!}");
        return -OAL_EFAIL;
    }
    /***************************************************************************
        抛事件到wal层处理
    ***************************************************************************/
    wal_write_msg_hdr_init(&write_msg, WLAN_CFGID_CANCAL_SAME_CHAN_REMAIN_ON_CHAN_SYNC, sizeof(uint8_t));

    oam_warning_log0(0, OAM_SF_ANY, "{wal_ioctl_cancel_same_chan_remain_on_chan}");
    /* 需要将发送该函数设置为同步，否则hmac处理时会使用已释放的内存 */
    ret = (uint32_t)wal_send_cfg_event(net_dev, WAL_MSG_TYPE_WRITE, WAL_MSG_WRITE_MSG_HDR_LENGTH + sizeof(uint8_t),
        (uint8_t *)&write_msg, OAL_FALSE, NULL);
    if (ret != OAL_SUCC) {
        oam_error_log0(0, OAM_SF_ANY, "{wal_ioctl_cancel_same_chan_remain_on_chan:wal_send_cfg_event fail!}");
        return -OAL_EFAIL;
    }
    return OAL_SUCC;
}

int32_t wal_ioctl_chba_service_ask_user_all_awake(oal_net_device_stru *net_dev,
    wal_wifi_priv_cmd_stru *priv_cmd, uint8_t *command, uint32_t cmd_name_len)
{
    int32_t ret;
    wal_msg_write_stru write_msg = { 0 };

    if (wal_ioctl_judge_input_param_length(*priv_cmd, cmd_name_len,
        sizeof(wlan_chba_service_ask_awake_stru)) != OAL_SUCC) {
        oam_error_log0(0, OAM_SF_ANY, "{wal_ioctl_chba_service_ask_user_all_awake:invalid cmd len!}");
        return -OAL_EFAIL;
    }
    /* 获取目标user mac地址与业务类型 */
    if (memcpy_s(&write_msg.auc_value, sizeof(wlan_chba_service_ask_awake_stru),
        command + cmd_name_len + 1, sizeof(wlan_chba_service_ask_awake_stru)) != EOK) {
        oam_error_log0(0, OAM_SF_ANY, "wal_ioctl_chba_service_ask_user_all_awake:memcpy fail!");
        return -OAL_EFAIL;
    }
    /***************************************************************************
        抛事件到wal层处理
    ***************************************************************************/
    wal_write_msg_hdr_init(&write_msg, WLAN_CFGID_CHBA_SERVICE_ASK_USER_ALL_AWAKE,
        sizeof(wlan_chba_service_ask_awake_stru));
    ret = wal_send_cfg_event(net_dev, WAL_MSG_TYPE_WRITE, WAL_MSG_WRITE_MSG_HDR_LENGTH +
        sizeof(wlan_chba_service_ask_awake_stru), (uint8_t *)&write_msg, OAL_FALSE, NULL);
    if (ret != OAL_SUCC) {
        oam_error_log0(0, OAM_SF_ANY, "{wal_ioctl_chba_service_ask_user_all_awake:wal_send_cfg_event fail!}");
        return -OAL_EFAIL;
    }
    return OAL_SUCC;
}
static int32_t wal_ioctl_priv_cmd_get_chba_cap(oal_net_device_stru *net_dev, wal_wifi_priv_cmd_stru *priv_cmd)
{
    uint8_t val;
    if ((uint32_t)priv_cmd->total_len < sizeof(uint8_t)) {
        oam_error_log1(0, 0, "{wal_ioctl_priv_cmd_get_chba_cap::CMD_GET_CHBA_CAP len[%d] err}", priv_cmd->total_len);
        return -OAL_EFAIL;
    }

    /* 判断CHBA定制化是否打开 */
    if (g_optimized_feature_switch_bitmap & BIT(CUSTOM_OPTIMIZE_CHBA)) {
        val = CMD_CHBA_CAP_SUPP;
    } else {
        val = CMD_CHBA_CAP_NOT_SUPP;
    }

    if (oal_copy_to_user(priv_cmd->puc_buf, &val, sizeof(uint8_t))) {
        oam_error_log0(0, OAM_SF_CFG, "{wal_ioctl_priv_cmd_get_chba_cap::Failed to copy data to user !}");
        return -OAL_EFAIL;
    }
    ...
    uint8_t *pc_cmd, wal_wifi_priv_cmd_stru *priv_cmd)
{
    ...
    } else if (oal_strncasecmp(pc_cmd, CMD_CANCEL_SAME_CHAN_REMAIN_ON_CHAN,
        CMD_CANCEL_SAME_CHAN_REMAIN_ON_CHAN_LEN) == 0) {
        return wal_ioctl_cancel_same_chan_remain_on_chan(pst_net_dev);
    } else if (oal_strncasecmp(pc_cmd, CMD_GET_CHBA_CAP, CMD_GET_CHBA_CAP_LEN) == 0) {
        return wal_ioctl_priv_cmd_get_chba_cap(pst_net_dev, priv_cmd);
#endif
    ...
    } else if (oal_strncasecmp(pc_cmd, CMD_SET_LP_LONG_DTIM, CMD_SET_LP_LONG_DTIM_LEN) == 0) {
        return wal_vendor_set_lp_long_dtim_handle(pst_net_dev, priv_cmd, pc_cmd);
#ifdef _PRE_WLAN_FEATURE_11AX
    } else if (oal_strncasecmp(pc_cmd, CMD_SET_OMI_BW_SWITCH, CMD_SET_OMI_BW_SWITCH_LEN) == 0) {
        return wal_vendor_set_omi_bw_switch_handle(pst_net_dev, priv_cmd, pc_cmd);
#endif
    } else if (oal_strncasecmp(pc_cmd, CMD_GET_LOWPOWER_STATISTICS, CMD_GET_LOWPOWER_STATISTICS_LEN) == 0) {
        return wal_ioctl_get_lowpower_statistics(pst_net_dev, priv_cmd, pc_cmd);
    }

    return wal_ioctl_cmd_process(pst_net_dev, priv_cmd, pc_cmd);
}
...
uint32_t wal_ioctl_set_al_rx(oal_net_device_stru *net_dev, int8_t *pc_param)
{
    ...
    if (uc_rx_flag > HAL_ALWAYS_RX_BUTT) {
