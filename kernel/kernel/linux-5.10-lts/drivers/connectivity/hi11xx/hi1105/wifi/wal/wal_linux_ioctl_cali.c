...
uint32_t wal_hipriv_dyn_cali_cfg(oal_net_device_stru *pst_net_dev, int8_t *puc_param)
{
    ...
    for (map_index = 0; map_index < g_dyn_cali_cfg_map_size; map_index++) {
        ...
        if (oal_strcmp(cyn_cali_cfg.pc_name, ac_name) == 0) {
            break;
        }
    }
    /* 没有找到对应的命令，则报错 */
...
uint32_t wal_hipriv_cali_debug(oal_net_device_stru *net_dev, int8_t *pc_param)
{
    ...
    while (curr_pos < para_len) {
        ...
        if (memcpy_s((int8_t*)(cali_cfg_para->input_args[arg_idx]),
            WLAN_CALI_CFG_CMD_MAX_LEN, ac_name, src_str_len) != EOK) {
            return OAL_FAIL;
        }
        pc_param += offset;
        curr_pos += offset;
...
uint32_t wal_hipriv_set_txpower(oal_net_device_stru *net_dev, int8_t *param)
{
    ...
    ret = wal_get_cmd_one_arg(param, ac_val, WAL_HIPRIV_CMD_NAME_MAX_LEN, &off_set);
    if (ret != OAL_SUCC) {
        oam_warning_log1(0, OAM_SF_ANY,
