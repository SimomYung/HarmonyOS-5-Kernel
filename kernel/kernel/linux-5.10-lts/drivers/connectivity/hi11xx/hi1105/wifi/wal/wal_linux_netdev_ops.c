    ...
    /* dfr 暂未适配wlan1 */
        if (wal_dfr_recovery_check(net_dev)) {
            ...
            if ((oal_bus_get_chip_num() == WLAN_CHIP_MAX_NUM_PER_BOARD) &&
                (oal_strncmp("wlan1", net_dev->name, OAL_STRLEN("wlan1")) == 0)) {
...
int32_t wal_netdev_start_vap(oal_net_device_stru *net_dev)
{
    int32_t l_ret;
    uint32_t err_code;
    ...
    if (oal_unlikely(net_dev == NULL)) {
        oam_error_log0(0, OAM_SF_ANY, "{wal_netdev_start_vap::net_dev null!}");
        return -OAL_EFAUL;
    }
    /* 抛事件到wal层处理 填写消息 */
    ...
    if (en_p2p_mode == WLAN_P2P_BUTT) {
        oam_error_log0(0, OAM_SF_ANY, "{wal_netdev_start_vap::wal_wireless_iftype_to_mac_p2p_mode return BUFF}");
    ...
    if (oal_unlikely(l_ret != OAL_SUCC)) {
        oam_warning_log1(0, OAM_SF_ANY, "{wal_netdev_start_vap::wal_alloc_cfg_event err %d!}", l_ret);
        return l_ret;
    }
    ...
    if (err_code != OAL_SUCC) {
        oam_warning_log1(0, OAM_SF_ANY, "{wal_netdev_start_vap::hmac start vap fail,err code[%u]!}", err_code);
        return -OAL_EINVAL;
    }
...
void wal_netdev_open_get_hw_status(oal_net_device_stru *pst_net_dev)
{
    ...
    if ((pst_net_dev->ieee80211_ptr->iftype == NL80211_IFTYPE_STATION) &&
        ((oal_strcmp("wlan0", pst_net_dev->name)) == 0)) {
        ...
        wal_atcmsrv_ioctl_get_saw_status(pst_net_dev, &saw_status);
    }
}
...
OAL_STATIC int32_t _wal_netdev_open(oal_net_device_stru *pst_net_dev, uint8_t uc_entry_flag, int32_t pm_ret)
{
    ...
    if (wal_netdev_start_vap(pst_net_dev) != OAL_SUCC) {
        oam_warning_log0(0, OAM_SF_ANY, "{_wal_netdev_open::wal_netdev_start_vap fail}");
        return -OAL_EFAIL;
    }
    ...
    /* link vap初始化 */
    wal_init_mlo_link_vap(pst_net_dev);
#endif
...
int32_t wal_netdev_open_ext(oal_net_device_stru *pst_net_dev)
{
    ...
    if (g_st_dfr_info.bit_device_reset_process_flag == OAL_TRUE) {
        oam_warning_log0(0, OAM_SF_CFG, "{wal_netdev_open_ext::dfr revoverying, not process kernel netdev open!}");
        return OAL_SUCC;
    }
...
OAL_STATIC void wal_record_wlan0_up_time(oal_net_device_stru *net_dev)
{
    if (oal_strncmp("wlan0", net_dev->name, OAL_STRLEN("wlan0")) == 0) {
        g_st_wifi_radio_stat.ull_wifi_on_time_stamp = OAL_TIME_JIFFY;
    }
}

uint32_t wal_get_net_device_id(oal_net_device_stru *pst_net_device)
{
#ifdef _PRE_DUAL_CHIP
    if ((oal_bus_get_chip_num() == WLAN_CHIP_MAX_NUM_PER_BOARD) &&
        ((oal_strcmp("wlan1", pst_net_device->name) == 0) || (oal_strcmp("p2p1", pst_net_device->name) == 0))) {
        oal_io_print("{wal_get_net_device_id: pst_net_device->dev_id is [%d]}", HCC_EP_WIFI_DEV1);
        return HCC_EP_WIFI_DEV1;
    }
#endif
    return HCC_EP_WIFI_DEV;
}

struct wlan_pm_s *wal_netdev_pm_get_dev(oal_net_device_stru *pst_net_dev)
{
#ifdef _PRE_DUAL_CHIP
    return wlan_pm_get_drv_by_id(wal_get_net_device_id(pst_net_dev));
#else
    return wlan_pm_get_drv();
#endif
}
...
int32_t wal_netdev_open(oal_net_device_stru *pst_net_dev, uint8_t uc_entry_flag)
{
...
#endif
    pm_ret = wlan_pm_open_dev(wal_get_net_device_id(pst_net_dev), OAL_TRUE);
    if (pm_ret == OAL_FAIL) {
    ...
    /* 记录wlan0 开的时间 */
    wal_record_wlan0_up_time(pst_net_dev);
#endif
...
#ifdef _PRE_WLAN_VAP_PROC
     wal_add_vap_proc_file(pst_net_dev);
#endif
...
oal_net_device_stats_stru *wal_netdev_get_stats(oal_net_device_stru *pst_net_dev)
{
    ...
    /* 更新统计信息到net_device */
    pst_stats->rx_packets = vap_stats.rx_packets;
    pst_stats->rx_bytes = vap_stats.rx_bytes;

    pst_stats->tx_packets = vap_stats.tx_packets;
    pst_stats->tx_bytes = vap_stats.tx_bytes;
    return pst_stats;
}
...
void wal_set_napi_disable(oal_net_device_stru *pst_net_dev)
{
    ...
    pst_netdev_priv = (oal_netdev_priv_stru *)oal_net_dev_wireless_priv(pst_net_dev);
    if (pst_netdev_priv->uc_napi_enable && ((pst_net_dev->ieee80211_ptr->iftype == NL80211_IFTYPE_STATION) ||
...
OAL_STATIC int32_t _wal_netdev_stop(oal_net_device_stru *pst_net_dev)
{
    ...
    /* ap模式不涉及ml场景，启动时不会创建link vap */
#ifdef _PRE_WLAN_FEATURE_ML
    wal_deinit_mlo_link_vap(pst_net_dev);
#endif
    ...
    /* 抛事件到wal层处理 */
    l_ret = wal_netdev_down_vap(pst_net_dev);
    if (l_ret != OAL_SUCC) {
...
int32_t wal_netdev_stop_ap(oal_net_device_stru *pst_net_dev)
{
    ...
    oal_io_print("wal_netdev_stop_ap, iftype[%d], dev_name[%s]\n",
        pst_net_dev->ieee80211_ptr->iftype, pst_net_dev->name);
    if (pst_net_dev->ieee80211_ptr->iftype != NL80211_IFTYPE_AP) {
#ifdef _PRE_WLAN_CHBA_MGMT
    ...
    /* 设置aput模式允许下电标志 */
    wal_set_power_mgmt_on(pst_net_dev, OAL_TRUE);
...
#endif
#endif
    wal_del_vap_try_wlan_pm_close(wal_get_net_device_id(pst_net_dev));
    return ret;
}

OAL_STATIC void _wal_netdev_set_link_mac_addr(oal_net_device_stru *net_dev, oal_sockaddr_stru *mac_addr)
{
    ...
    oal_set_mac_addr(primary_mac_addr, (uint8_t *)(mac_addr->sa_data));
    for (idx = 0; idx < WLAN_MAX_ML_LINK_NUM; idx++) {
        link_netdev = ml_net_dev_priv->mlo_cfg_info.link_netdev[idx];
        if (link_netdev == NULL || link_netdev == net_dev) {
            continue;
        }
        ...
        if (link_net_dev_priv->mlo_cfg_info.bind_with_ml_vap == OAL_FALSE &&
            link_netdev != net_dev) {
            primary_mac_addr[WLAN_MAC_ADDR_LEN - 1] += 1; /* link mac地址最后一位，相较于wlan0 mac地址+1 */
            oal_set_mac_addr((uint8_t *)(link_netdev->dev_addr), primary_mac_addr);
            oam_warning_log4(0, OAM_SF_ANY,
                "{_wal_netdev_set_link_mac_addr::link_id[%d], mac_addr[%.2x:xx:xx:xx:%.2x:%.2x]}",
                link_net_dev_priv->mlo_cfg_info.link_id, link_netdev->dev_addr[MAC_ADDR_0],
                link_netdev->dev_addr[MAC_ADDR_4], link_netdev->dev_addr[MAC_ADDR_5]);
        }
    }
}
...
#endif
    _wal_netdev_set_link_mac_addr(net_dev, mac_addr);
    return OAL_SUCC;
}
