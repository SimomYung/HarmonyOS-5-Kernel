...
OAL_STATIC uint32_t wal_parse_protocol_mode_by_ie(uint8_t *vht_ie, uint8_t *ht_ie,
    uint8_t *he_ie, uint8_t *eht_ie, uint8_t he_flag, wlan_protocol_enum_uint8 *protocol)
{
#ifdef _PRE_WLAN_FEATURE_11BE
    if (eht_ie != NULL ||
        g_st_mac_device_custom_cfg.custom_cfg_11be.aput_11be_switch == OAL_TRUE) {
        if (g_wlan_spec_cfg->feature_11be_is_open) {
            /* 设置AP 为11be模式 */
            *protocol = WLAN_EHT_MODE;
            return OAL_SUCC;
        }
    }
#endif
    ...
    if (he_ie != NULL) {
#else
    if ((he_ie != NULL) || (he_flag == OAL_TRUE)) {
#endif
    ...
    uint8_t *ext_rates_num)
{
    ...
    ies_info.frame_ies = beacon_info->head + offset;
    ies_info.frame_len = beacon_info->head_len - offset;
    base_rates_ie = mac_find_ie(MAC_EID_RATES, &ies_info);
    if (base_rates_ie != NULL) {
        *base_rates_num = base_rates_ie[1];
    }

    ies_info.frame_ies = beacon_info->tail;
    ies_info.frame_len = beacon_info->tail_len;
    ext_rates_ie = mac_find_ie(MAC_EID_XRATES, &ies_info);
 ...
 * 1.日    期  : 2014年6月12日
  *   修改内容  : 新生成函数
 */
OAL_STATIC uint32_t wal_parse_protocol_mode(wlan_channel_band_enum_uint8 band,
    oal_beacon_parameters *beacon_info, uint8_t *ht_ie, uint8_t *vht_ie,
    uint8_t *he_ie, uint8_t *eht_ie, wlan_protocol_enum_uint8 *protocol, uint8_t he_flag)
{
    uint8_t *base_rates_ie = NULL;
    uint8_t *ext_rates_ie = NULL;
    ...
    *protocol = WLAN_PROTOCOL_BUTT;
    ...
    if (wal_parse_protocol_mode_by_ie(vht_ie, ht_ie, he_ie, eht_ie, he_flag, protocol) == OAL_SUCC) {
        return OAL_SUCC;
    }
    ...
    if (band == WLAN_BAND_2G) {
        ...
        if (wal_parse_protocol_mode_rate_nums_check(rate_nums, base_rates_num, ext_rates_num, protocol) == OAL_FAIL) {
            return OAL_FAIL;
        }
        if (rate_nums == 4) { /* 4表示11b的速率 */
            ...
            *protocol = WLAN_MIXED_ONE_11G_MODE;
            if (wal_check_support_basic_rate_6m(base_rates_ie, base_rates_num,
                ext_rates_ie, ext_rates_num) == OAL_TRUE) {
    ...
    oal_beacon_parameters  *pst_beacon_info)
{
    ...
    if (g_wlan_spec_cfg->feature_11ax_is_open) {
        ...
        /* 通过hostapd下发ax能力 */
        puc_he_ie = mac_find_ie_ext_ie(MAC_EID_HE, MAC_EID_EXT_HE_CAP, &ies_info);
    }
#endif
    return puc_he_ie;
}
...
OAL_STATIC void wal_go_chba_protocol_handle(mac_vap_stru *mac_vap, mac_beacon_param_stru *beacon_param)
{
    if (is_p2p_go(mac_vap)) {
        ...
        if (mac_vap->en_vap_state == MAC_VAP_STATE_UP || mac_vap->en_vap_state == MAC_VAP_STATE_PAUSE) {
            beacon_param->en_protocol = mac_vap->en_protocol;
            return;
        }

#ifdef _PRE_WLAN_FEATURE_11BE
        if (g_wlan_spec_cfg->feature_11be_is_open) {
            if (beacon_param->en_protocol == WLAN_EHT_MODE) {
                return;
            }
        }
#endif
    } else if (mac_is_chba_mode(mac_vap) == OAL_TRUE) {
#ifdef _PRE_WLAN_FEATURE_11BE
        if (beacon_param->en_protocol == WLAN_EHT_MODE) {
            if (g_st_mac_device_custom_cfg.custom_cfg_11be.chba_11be_switch == OAL_TRUE) {
                return;
            }
            beacon_param->en_protocol = WLAN_HE_MODE;
        }
#endif
    } else {
        return;
...
#ifdef _PRE_WLAN_FEATURE_11AX
    if (g_wlan_spec_cfg->feature_11ax_is_open) {
        if (beacon_param->en_protocol == WLAN_HE_MODE) {
            return;
        }
    }
#endif
    ...
    if (mac_vap->st_channel.en_band == WLAN_BAND_2G) {
        beacon_param->en_protocol = (mac_vap->st_cap_flag.bit_11ac2g == OAL_TRUE) ? WLAN_VHT_MODE : WLAN_HT_MODE;
    }
    if (mac_vap_avoid_dbac_close_vht_protocol(mac_vap) == OAL_TRUE) {
        beacon_param->en_protocol = WLAN_HT_MODE;
        oam_warning_log1(mac_vap->uc_vap_id, OAM_SF_ANY, "{wal_go_protocol_handle::close vht = %d!}",
            beacon_param->en_protocol);
    }
}
 ...
 * 1.日    期  : 2014年4月4日
  *   修改内容  : 新生成函数
 */
uint32_t wal_parse_ht_vht_ie(oal_net_device_stru *netdev,
    mac_vap_stru *mac_vap, oal_beacon_parameters *beacon_info, mac_beacon_param_stru *beacon_param)
{
    uint8_t *ht_ie, *vht_ie;
    uint8_t *he_ie = NULL;
    uint8_t *eht_ie = NULL;
    uint32_t ret;
    mac_frame_ht_cap_stru *ht_cap = NULL;
    mac_vht_cap_info_stru *vht_cap = NULL;
    ...
    ht_ie = mac_find_ie(MAC_EID_HT_CAP, &ies_info);
    vht_ie = mac_find_ie(MAC_EID_VHT_CAP, &ies_info);
    he_ie = wal_parse_he_ie(mac_vap, &he_flag, beacon_info);
    wal_he_flag_handle(netdev, mac_vap, &he_ie, &he_flag);

#ifdef _PRE_WLAN_FEATURE_11BE
    eht_ie = mac_find_ie_ext_ie(MAC_EID_EHT, MAC_EID_EXT_EHT_CAP, &ies_info);
#endif

    /* 解析协议模式 */
    ret = wal_parse_protocol_mode(mac_vap->st_channel.en_band, beacon_info,
        ht_ie, vht_ie, he_ie, eht_ie, &beacon_param->en_protocol, he_flag);
    if (ret != OAL_SUCC) {
        oam_error_log1(mac_vap->uc_vap_id, OAM_SF_ANY, "{wal_parse_ht_vht_ie::return err code!}\r\n", ret);
        return ret;
    }

    wal_go_chba_protocol_handle(mac_vap, beacon_param);
    /* 如果ht vht均为空，返回 */
    if ((ht_ie == NULL) && (vht_ie == NULL)) {
        return OAL_SUCC;
    }
    /* 解析short gi能力 */
    if (ht_ie != NULL) {
        ...
        beacon_param->en_shortgi_20 = (uint8_t)ht_cap->bit_short_gi_20mhz;
        beacon_param->en_shortgi_40 = 0;

        if ((mac_vap->st_channel.en_bandwidth > WLAN_BAND_WIDTH_20M) &&
            (mac_vap->st_channel.en_bandwidth != WLAN_BAND_WIDTH_BUTT)) {
            beacon_param->en_shortgi_40 = (uint8_t)ht_cap->bit_short_gi_40mhz;
        }
    }
    ...
    if (vht_ie[1] < sizeof(mac_vht_cap_info_stru)) {
        oam_warning_log1(mac_vap->uc_vap_id, 0, "{wal_parse_ht_vht_ie::invalid ht cap ie len[%d]!}", vht_ie[1]);
        return OAL_SUCC;
    }

    vht_cap = (mac_vht_cap_info_stru *)(vht_ie + MAC_IE_HDR_LEN);

    beacon_param->en_shortgi_80 = 0;

    if ((mac_vap->st_channel.en_bandwidth > WLAN_BAND_WIDTH_40MINUS) &&
        (mac_vap->st_channel.en_bandwidth != WLAN_BAND_WIDTH_BUTT)) {
        beacon_param->en_shortgi_80 = vht_cap->bit_short_gi_80mhz;
    }
...
OAL_STATIC int32_t wal_cfg80211_scan_post_check(hmac_scan_stru *scan_mgmt, mac_vap_stru *pst_mac_vap,
                                                oal_net_device_stru *pst_netdev)
{
...
int32_t wal_cfg80211_scan(oal_wiphy_stru *pst_wiphy, oal_cfg80211_scan_request_stru *pst_request)
{
    ...
    if (wal_cfg80211_elp_scan(pst_wiphy, pst_request) == OAL_SUCC) {
        return OAL_SUCC;
    }
#endif
    ...
    if (wal_cfg80211_scan_post_check(scan_mgmt, pst_mac_vap, pst_netdev) != OAL_SUCC) {
        return -OAL_EFAIL;
    }
    ...
    if (wal_scan_work_func(scan_mgmt, pst_netdev, pst_request) != OAL_SUCC) {
...
uint8_t wal_cfg80211_convert_value_to_vht_width(int32_t l_channel_value)
{
    ...
    switch (l_channel_value) {
        ...
        case 40:  /* 40M带宽 */
            ...
            break;
        case 80:  /* 80M带宽 */
            uc_channel_vht_width = WLAN_MIB_VHT_OP_WIDTH_80_160;
            break;
        case 160: /* 160M带宽 */
            uc_channel_vht_width = WLAN_MIB_VHT_OP_WIDTH_160_DEPRECATED;
            break;
        default:
            break;
    }
...
static wlan_channel_bandwidth_enum_uint8 wal_cfg80211_set_channel_cal_bw(oal_net_device_stru *netdev)
{
...
static int32_t wal_cfg80211_channel_info_send_event(oal_net_device_stru *netdev, oal_ieee80211_channel *channel,
    uint8_t is_6ghz, wlan_channel_bandwidth_enum_uint8 en_bandwidth)
{
    int32_t ret;
    uint32_t err_code;
    wal_msg_write_stru write_msg;
    wal_msg_stru *rsp_msg = NULL;
    mac_cfg_channel_param_stru *channel_param = NULL;
    mac_vap_stru *mac_vap = oal_net_dev_priv(netdev);
    uint8_t vap_id = mac_vap->uc_vap_id;

    /* 填写消息 */
    channel_param = (mac_cfg_channel_param_stru *)(write_msg.auc_value);
    memset_s(channel_param, sizeof(mac_cfg_channel_param_stru), 0, sizeof(mac_cfg_channel_param_stru));
    channel_param->uc_channel = (uint8_t)channel->hw_value;
    channel_param->en_band = channel->band;
    channel_param->ext6g_band = (is_6ghz == OAL_TRUE) ? 1 : 0; /* 信道为6G时 标志位置1 */
    channel_param->en_bandwidth = en_bandwidth;
#if (defined _PRE_WLAN_FEATURE_NBW) && (defined _PRE_WLAN_NBW_DEMO)
    if (mac_vap->nbw_mode == NBW_MODE) {
        channel_param->uc_channel = 100; /* 07c demo阶段，NBW的工作信道直接指定为100信道 */
        channel_param->en_band = 1;
        channel_param->ext6g_band = (is_6ghz == OAL_TRUE) ? 1 : 0;
        channel_param->en_bandwidth = 3; /* 07c demo阶段，NBW的带宽直接指定为3 */
    }
#endif

    oam_warning_log3(vap_id, OAM_SF_ANY, "{wal_cfg80211_set_channel::uc_channel=%d, en_band=%d, en_bandwidth=%d.}",
                     channel_param->uc_channel, channel_param->en_band, channel_param->en_bandwidth);

    wal_write_msg_hdr_init(&write_msg, WLAN_CFGID_CFG80211_SET_CHANNEL, sizeof(mac_cfg_channel_param_stru));
    ...
    if (ret != OAL_SUCC) {
        oam_warning_log1(0, OAM_SF_P2P, "{wal_cfg80211_set_channel_info::wal_send_cfg_event return err:%d}", ret);
        return -OAL_EFAIL;
    }
 ...
 *   修改内容  : 新生成函数
 */
int32_t wal_cfg80211_set_channel_info(oal_wiphy_stru *pst_wiphy, oal_net_device_stru *netdev)
{
    int32_t ret;
    ...
    oal_ieee80211_channel *channel = NULL;
    int32_t l_channel;
    uint8_t is_6ghz = OAL_FALSE;

    if (oal_unlikely(netdev == NULL || netdev->ieee80211_ptr == NULL ||
        netdev->ieee80211_ptr->preset_chandef.chan == NULL)) {
        oam_error_log0(0, OAM_SF_ANY, "{wal_cfg80211_set_channel_cal_bw::netdev or ieee80211_ptr or chan null!}");
        return -OAL_EINVAL;
    }

    mac_vap = oal_net_dev_priv(netdev);
    if (oal_unlikely(mac_vap == NULL)) {
        oam_error_log0(0, OAM_SF_ANY, "{wal_cfg80211_set_channel_cal_bw::mac_vap null!}");
        return -OAL_EINVAL;
    }
    vap_id = mac_vap->uc_vap_id;
    l_bandwidth = netdev->ieee80211_ptr->preset_chandef.width;
    l_center_freq1 = (int32_t)netdev->ieee80211_ptr->preset_chandef.center_freq1;
    l_center_freq2 = (int32_t)netdev->ieee80211_ptr->preset_chandef.center_freq2;
    channel = netdev->ieee80211_ptr->preset_chandef.chan;
    l_channel = channel->hw_value;

    oam_warning_log4(vap_id, OAM_SF_ANY, "{wal_cfg80211_set_channel_info::l_bandwidth = %d, l_center_freq1 = %d,\
        l_center_freq2 = %d, l_channel = %d.}", l_bandwidth, l_center_freq1, l_center_freq2, l_channel);

    /* 内核下发6G信道类型为3时，驱动侧将信道类型写为1（5G），并置位6g标志位 */
    is_6ghz = wal_cfg80211_reset_6g_band_type(channel);

    ret = (int32_t)mac_is_channel_num_valid(channel->band, (uint8_t)l_channel,
        (uint8_t)(channel->center_freq >= 5955)); /* 5955是6G的起始中心频点 */
    if (ret != OAL_SUCC) {
        oam_warning_log2(vap_id, OAM_SF_ANY,
            "{wal_cfg80211_set_channel_info::chan num invalid. band, ch num [%d] [%d]!}", channel->band, l_channel);
        return -OAL_EINVAL;
    }

    en_bandwidth = wal_cfg80211_set_channel_cal_bw(netdev);

    return wal_cfg80211_channel_info_send_event(netdev, channel, is_6ghz, en_bandwidth);
}
