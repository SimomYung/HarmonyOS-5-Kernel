...
OAL_STATIC uint32_t wal_scan_comp_proc_sta_scan_req(hmac_scan_rsp_stru *pst_scan_rsp, hmac_scan_stru *pst_scan_mgmt,
                                                    oal_wiphy_stru *pst_wiphy, hmac_vap_stru *pst_hmac_vap)
{
...
uint32_t wal_scan_comp_proc_sta(frw_event_mem_stru *pst_event_mem)
{
    ...
    pst_scan_mgmt = hmac_get_scam_mgmt(pst_hmac_vap);
    if (oal_any_null_ptr3(pst_hmac_vap, pst_hmac_device, pst_scan_mgmt)) {
    ...
    if (wal_elp_scan_refuse_proc(pst_scan_rsp, pst_scan_mgmt, pst_wiphy, pst_hmac_vap) == OAL_SUCC) {
        return OAL_SUCC;
    }
#endif
    return wal_scan_comp_proc_sta_scan_req(pst_scan_rsp, pst_scan_mgmt, pst_wiphy, pst_hmac_vap);
}
    ...
    oal_connet_result_stru *connet_result)
{
    ...
    * 避免由于关联前不扫描，内核将bss 老化，4次握手完成后下发秘钥提示失败情况 */
    ...
    oal_inform_assoc_link_id(net_device, CONNECT_EVENT);
#endif
...
#ifdef CONFIG_HISI_WIFI7_MLO
    oal_inform_assoc_link_id(pst_net_device, ROAM_EVENT);
#endif
...
uint32_t wal_notify_wpa_okc_start(frw_event_mem_stru *event_mem)
{
    frw_event_stru *event = NULL;
    oal_net_device_stru *net_device = NULL;
    uint8_t *target_ap_bssid = NULL;
    oal_gfp_enum_uint8 kflags = oal_in_atomic() ? GFP_ATOMIC : GFP_KERNEL;

    if (oal_unlikely(event_mem == NULL)) {
        oam_error_log0(0, OAM_SF_ASSOC, "{wal_notify_wpa_okc_start::event_mem is null!}");
        return OAL_ERR_CODE_PTR_NULL;
    }

    event = frw_get_event_stru(event_mem);
    target_ap_bssid = (uint8_t *)event->auc_event_data;

    /* 获取net_device */
    net_device = hmac_vap_get_net_device(event->st_event_hdr.uc_vap_id);
    if (net_device == NULL) {
        oam_error_log0(event->st_event_hdr.uc_vap_id, OAM_SF_ROAM,
                       "{wal_notify_wpa_okc_start::get net device ptr is null!}");
        return OAL_ERR_CODE_PTR_NULL;
    }
    oal_cfg80211_pmksa_candidate_notify(net_device, 0, target_ap_bssid, OAL_FALSE, kflags);
    return OAL_SUCC;
}
...
uint32_t wal_disconnect_sta_proc_ap(frw_event_mem_stru *pst_event_mem)
{
    ...
    /* 获取去关联user mac addr */
    if (memcpy_s(user_addr, WLAN_MAC_ADDR_LEN,
        (uint8_t *)pst_event->auc_event_data, WLAN_MAC_ADDR_LEN) != EOK) {
...
uint32_t wal_send_mgmt_to_host(frw_event_mem_stru *pst_event_mem)
{
    ...
    if (pst_net_device == NULL || pst_net_device->ieee80211_ptr == NULL) {
        oam_warning_log0(pst_event->st_event_hdr.uc_vap_id, OAM_SF_ANY,
            "{wal_send_mgmt_to_host::get net device or ieee80211_ptr ptr is null!}");
        oal_io_print("wal_send_mgmt_to_host:netdevice %.16s\r\n", pst_mgmt_frame->ac_name);
    ...
    oal_cfg80211_external_auth_stru *data, uint32_t len)
{
    ...
    if (oal_nla_put_u32(skb, ROAM_ATTRIBUTE_EXTERNAL_AUTH_AKM, data->key_mgmt_suite) ||
        ...
        (!mac_addr_is_zero(data->mld_addr) &&
        oal_nla_put(skb, ROAM_ATTRIBUTE_EXTERNAL_AUTH_MLD_ADDR, OAL_ETH_ALEN_SIZE, data->mld_addr))) {
#else
        oal_nla_put(skb, ROAM_ATTRIBUTE_EXTERNAL_AUTH_SSID, data->ssid.ssid_len, data->ssid.ssid)) {
#endif
        oal_netbuf_free(skb);
...
#if (defined(_PRE_WLAN_FEATURE_ML) && defined(CONFIG_HISI_WIFI7_MLO))
OAL_STATIC int32_t wal_add_mld_addr_external_auth(oal_cfg80211_external_auth_stru* auth_req,
    hmac_external_auth_req_stru *pst_ext_auth_req)
{
    int32_t l_memcpy_ret;
    uint8_t mac_zero[OAL_MAC_ADDR_LEN] = {0};
    if (!mac_addr_is_zero(pst_ext_auth_req->ap_mld_addr)) {
        l_memcpy_ret = memcpy_s(auth_req->mld_addr, OAL_ETH_ALEN_SIZE, pst_ext_auth_req->ap_mld_addr, ETH_ALEN);
    } else {
        l_memcpy_ret = memcpy_s(auth_req->mld_addr, OAL_ETH_ALEN_SIZE, mac_zero, ETH_ALEN);
    }
    return l_memcpy_ret;
}
#endif
...
uint32_t wal_report_external_auth_req(frw_event_mem_stru *pst_event_mem)
{
    ...
    if (pst_net_device == NULL) {
        oam_error_log1(pst_event->st_event_hdr.uc_vap_id, OAM_SF_SAE,
            "{wal_report_external_auth_req::get net device ptr is null!vap_id %d}", pst_event->st_event_hdr.uc_vap_id);
        return OAL_ERR_CODE_PTR_NULL;
    }
...
#if (defined(_PRE_WLAN_FEATURE_ML) && defined(CONFIG_HISI_WIFI7_MLO))
    l_memcpy_ret += wal_add_mld_addr_external_auth(&st_external_auth_req, pst_ext_auth_req);
#endif
    if (l_memcpy_ret != EOK) {
    ...
    oam_warning_log4(pst_event->st_event_hdr.uc_vap_id, OAM_SF_SAE,
        ...
        st_external_auth_req.bssid[MAC_ADDR_0], st_external_auth_req.bssid[MAC_ADDR_4],
        st_external_auth_req.bssid[MAC_ADDR_5], l_ret);
...
uint32_t wal_cfg80211_m2s_status_report(frw_event_mem_stru *pst_event_mem)
{
    ...
    /* 上报内核 */
    oal_cfg80211_m2s_status_report(netdev, GFP_KERNEL,
