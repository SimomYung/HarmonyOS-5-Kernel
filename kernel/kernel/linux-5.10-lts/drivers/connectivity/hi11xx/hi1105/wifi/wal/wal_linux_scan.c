...
void wal_update_bss(oal_wiphy_stru *wiphy, hmac_bss_mgmt_stru *bss_mgmt, uint8_t *bssid)
{
    ...
    if (bss_dscr == NULL) {
        oam_warning_log3(0, OAM_SF_ASSOC, "{wal_update_bss::do not find correspond bss from scan result."
                         "%02X:XX:XX:XX:%02X:%02X}",
                         bssid[BYTE_OFFSET_0], bssid[BYTE_OFFSET_4], bssid[BYTE_OFFSET_5]);
        return;
    }
...
OAL_STATIC void wal_countrycode_selstudy_statis(mac_bss_dscr_stru *bss_dscr,
                                                uint8_t *bssid_name,
                                                wal_country_code_stru *wal_country_code,
                                                uint8_t bssid_num_temp)
{
    uint8_t bssid_num = bssid_num_temp;
...
OAL_STATIC void wal_countrycode_selstudy_result(wal_country_code_stru* wal_country_code, uint8_t vap_id)
{
    ...
    if (hmac_vap == NULL) {
        oam_error_log0(vap_id, OAM_SF_SCAN, "{wal_countrycode_selstudy_result::hmac_vap is null!}");
        return;
    }
    if (g_cust_country_code_ignore_flag.en_country_code_study_ingore_hipriv_flag == OAL_FALSE) {
    /* 国家码结果确认 */
...
OAL_STATIC uint32_t wal_bss_filter_by_timestamps(mac_bss_dscr_stru *bss_dscr, uint8_t vap_id)
{
    ...
    hmac_vap = (hmac_vap_stru *)mac_res_get_hmac_vap(vap_id);
    if (hmac_vap == NULL) {
        oam_error_log0(vap_id, OAM_SF_SCAN, "{wal_inform_all_bss::hmac_vap is null!}");
        return OAL_FAIL;
    }
...
OAL_STATIC oal_bool_enum_uint8 wal_check_bss_is_include_wfd(mac_bss_dscr_stru *bss_dscr, uint8_t vap_id)
{
    ...
    ies_info.frame_ies = bss_dscr->auc_mgmt_buff + offset;
    ies_info.frame_len = bss_dscr->mgmt_len - offset;
    p2p_ie = mac_find_vendor_ie(MAC_WLAN_OUI_WFA, MAC_WLAN_OUI_TYPE_WFA_WFD, &ies_info);
                                  ...
                                  mac_cfg80211_scan_param_stru *pst_scan_param)
{
    ...
    if ((l_ssid_num > 0) && (request->ssids != NULL)) {
        ...
        for (l_loop = 0; l_loop < l_ssid_num; l_loop++) {
            ...
            if (memcpy_s(pst_scan_param->st_ssids[l_loop].auc_ssid,
                sizeof(pst_scan_param->st_ssids[l_loop].auc_ssid),
                request->ssids[l_loop].ssid,
                request->ssids[l_loop].ssid_len) != EOK) {
...
OAL_STATIC uint32_t wal_wait_for_scan_timeout_fn(void *p_arg)
{
    ...
    if (scan_mgmt == NULL) {
        oam_warning_log1(0, OAM_SF_SCAN, "{wal_wait_for_scan_complete_time_out::scan_mgmt[%d] is null.}",
                         mac_vap->uc_device_id);
        return OAL_FAIL;
    }
...
OAL_STATIC void wal_start_timer_for_scan_timeout(uint8_t vap_id)
{
    ...
    uint32_t timeout = WAL_MAX_SCAN_TIME_PER_SCAN_REQ; /* WAL层默认扫描超时时间为5秒 */
    ...
    if (hmac_scan_is_ap_up_connect_user(mac_device) == OAL_TRUE) {
        /* 有ap关联用户，增加额外超时等待时间 */
        timeout += WAL_SCAN_TIME_AP_UP_CONNECT_USER_EXTRA_TIME;
    }
    ...
    oal_cfg80211_scan_request_stru *request)
{
    ...
    if (memcpy_s(ie, request->ie_len, request->ie, request->ie_len) != EOK) {
...
int32_t wal_send_scan_abort_msg(oal_net_device_stru *net_dev)
{
    ...
    if (memcpy_s(st_write_msg.auc_value, sizeof(st_write_msg.auc_value),
        (int8_t *)&pedding_data, sizeof(pedding_data)) != EOK) {
...
uint32_t wal_force_scan_complete_scan_mgmt_proc(oal_net_device_stru *net_dev, hmac_device_stru *hmac_device,
    hmac_vap_stru *hmac_vap, hmac_scan_stru *scan_mgmt, oal_bool_enum en_is_aborted)
{
    /* 如果是来自内部的扫描 */
    if (scan_mgmt->pst_request == NULL) {
        wal_force_internal_scan_complete(net_dev, scan_mgmt, &(hmac_vap->st_vap_base_info));
        return OAL_SUCC;
    }
    ...
    if ((scan_mgmt->pst_request != NULL) && oal_wdev_match(net_dev, scan_mgmt->pst_request)) {
        ...
        wal_inform_all_bss(hmac_device->pst_device_base_info->pst_wiphy,
                           &(hmac_device->st_scan_mgmt.st_scan_record_mgmt.st_bss_mgmt),
        ...
        /* 清除扫描结果上报的回调函数，无需上报 */
        if (scan_mgmt->st_scan_record_mgmt.uc_vap_id == hmac_vap->st_vap_base_info.uc_vap_id) {
            scan_mgmt->st_scan_record_mgmt.p_fn_cb = NULL;
...
int32_t wal_force_scan_complete(oal_net_device_stru *net_dev, oal_bool_enum en_is_aborted)
{
    ...
    wal_elp_scan_abort_proc_imm(net_dev, hmac_device, en_is_aborted);
#endif

    scan_mgmt = &(hmac_device->st_scan_mgmt);
    return wal_force_scan_complete_scan_mgmt_proc(net_dev, hmac_device, hmac_vap, scan_mgmt, en_is_aborted);
}

uint32_t wal_force_scan_abort_then_scan_comp_scan_mgmt_proc(oal_net_device_stru *net_dev, hmac_vap_stru *hmac_vap,
    hmac_scan_stru *scan_mgmt)
{
    ...
    if (scan_mgmt->pst_request == NULL) {
        ...
        if ((scan_mgmt->en_is_scanning == OAL_TRUE) &&
            (hmac_vap->st_vap_base_info.uc_vap_id == scan_mgmt->st_scan_record_mgmt.uc_vap_id)) {
...
#ifdef _PRE_WLAN_FEATURE_ELP
    wal_elp_scan_abort_then_scan_comp_proc(net_dev);
#endif
    scan_mgmt = &(hmac_device->st_scan_mgmt);

    return wal_force_scan_abort_then_scan_comp_scan_mgmt_proc(net_dev, hmac_vap, scan_mgmt);
}
...
/* 功能描述: 结束当前device正在运行的扫描并等待直到扫描结束 */
void wal_force_abort_device_scan_and_wait_compelte(uint8_t device_id)
{
    oal_net_device_stru *scan_net_dev = NULL;
    hmac_scan_stru *scan_mgmt = NULL;
    hmac_device_stru *hmac_device = NULL;

    hmac_device = hmac_res_get_mac_dev(device_id);
    if (hmac_device == NULL) {
        oam_error_log0(device_id, OAM_SF_SCAN, "{wal_force_abort_device_scan_and_wait_compelte:fail to get device}");
        return;
    }
    scan_mgmt = &(hmac_device->st_scan_mgmt);
    ...
    /* 获取正在进行扫描的netdev */
    scan_net_dev = hmac_vap_get_net_device(scan_mgmt->st_scan_record_mgmt.uc_vap_id);
    if (scan_net_dev == NULL) {
        oam_error_log1(device_id, OAM_SF_SCAN,
            "{wal_force_abort_device_scan_and_wait_compelte:fail to get netdev, is_kernel_scan[%d]}",
            scan_mgmt->pst_request != NULL);
        return;
    }

    /* 打断并等待扫描结束 */
    oam_warning_log1(device_id, OAM_SF_SCAN,
        "{wal_force_abort_device_scan_and_wait_compelte:start abort scan, is_kernel_scan[%d]}",
        scan_mgmt->pst_request != NULL);
    wal_force_scan_abort_then_scan_comp(scan_net_dev);
    wal_cfg80211_connect_wait_scan_complete(scan_net_dev);
}
#ifdef _PRE_WLAN_FEATURE_ELP
uint32_t wal_cfg80211_elp_scan(oal_wiphy_stru *wiphy, oal_cfg80211_scan_request_stru *request)
{
    int32_t result = 0;
    oal_net_device_stru *netdev = request->wdev->netdev;
    mac_vap_stru *mac_vap = oal_net_dev_priv(netdev);   /* 下发扫描netdev对应的vap */
    mac_device_stru *mac_device = mac_res_get_dev(mac_vap->uc_device_id);
    hmac_scan_stru *scan_mgmt = hmac_get_wfsc_scan_mgmt();
    mac_cfg80211_scan_param_stru scan_param;
    hmac_vap_stru *hmac_vap = mac_res_get_hmac_vap(mac_device->uc_cfg_vap_id);   /* lp scan扫描使用cfg vap */
    wal_msg_write_stru    write_msg = { 0 };
    /* 未使能elp scan不使用 */
    if (g_elp_scan_disable_bitmap != 0) {
        oam_warning_log1(0, OAM_SF_CFG, "{wal_cfg80211_elp_scan::not support %x.}", g_elp_scan_disable_bitmap);
        return OAL_FAIL;
    }

    /* 扫描请求被dmac拒绝 */
    if (scan_mgmt->pst_request != NULL && scan_mgmt->st_scan_record_mgmt.en_scan_rsp_status == MAC_SCAN_REFUSED) {
        return OAL_FAIL;
    }
    ...
    /* 保存当前内核下发的扫描请求到本地 */
    scan_mgmt->pst_request = request;

    /* 解析内核下发的扫描信道列表 */
    memset_s(&scan_param, sizeof(mac_cfg80211_scan_param_stru), 0, sizeof(mac_cfg80211_scan_param_stru));
    if (wal_set_scan_channel(request, &scan_param) != OAL_SUCC) {
        hmac_free_scan_param_resource(&scan_param);
        return OAL_FAIL;
    }
    scan_param.bit_is_p2p0_scan = OAL_FALSE;
    scan_mgmt->en_complete = OAL_FALSE;

    /* 首先启动，post event过程中可能触发调度，导致扫描完成事件先于定时器启动，最终导致超时定时器残留 */
    wal_start_timer_for_scan_timeout(mac_device->uc_cfg_vap_id);

    /* 抛事件，通知驱动启动扫描 */
    result = memcpy_s(write_msg.auc_value, sizeof(write_msg.auc_value),
        (void *)(&scan_param.st_ssids[0]), sizeof(mac_cfg80211_scan_param_stru));
    if (result != EOK) {
        oam_error_log0(0, OAM_SF_ANY, "wal_cfg80211_elp_scan::memcpy fail!");
        return OAL_FAIL;
    }
    wal_write_msg_hdr_init(&write_msg, WLAN_CFGID_START_ELP_SCAN, sizeof(mac_cfg80211_scan_param_stru));
    result = wal_send_cfg_event(hmac_vap->pst_net_device, WAL_MSG_TYPE_WRITE,
        WAL_MSG_WRITE_MSG_HDR_LENGTH + sizeof(mac_cfg80211_scan_param_stru),
        (uint8_t*)&write_msg, OAL_FALSE, NULL);
    if (result != OAL_SUCC) {
        if (hmac_vap != NULL && hmac_vap->st_scan_timeout.en_is_registerd) {
            frw_timer_immediate_destroy_timer_m(&hmac_vap->st_scan_timeout);
...
uint32_t wal_elp_scan_refuse_proc(hmac_scan_rsp_stru *pst_scan_rsp, hmac_scan_stru *pst_scan_mgmt,
                                  oal_wiphy_stru *pst_wiphy, hmac_vap_stru *pst_hmac_vap)
{
    if (pst_hmac_vap->st_vap_base_info.uc_vap_id != 0 || pst_scan_rsp->uc_result_code != MAC_SCAN_REFUSED) {
        return OAL_FAIL;
    }
    /* 对于内核下发的扫描request资源加锁 */
    ...
    /* 切换到主路去扫描 */
    if (wal_cfg80211_scan(pst_wiphy, pst_scan_mgmt->pst_request) != OAL_SUCC) {
        /* 通知 kernel scan 已经结束 */
        oal_cfg80211_scan_done(pst_scan_mgmt->pst_request, OAL_TRUE);
    }
    /* 释放lp扫描使用到的资源 */
    pst_scan_mgmt->pst_request = NULL;
    pst_scan_mgmt->en_complete = OAL_TRUE;
    pst_scan_mgmt->st_scan_record_mgmt.en_scan_rsp_status = MAC_SCAN_STATUS_BUTT;
    pst_scan_mgmt->en_is_scanning = OAL_FALSE;
    oal_wait_queue_wake_up_interrupt(&pst_scan_mgmt->st_wait_queue);
    /* 对于内核下发的扫描request资源解锁 */
    oal_spin_unlock(&(pst_scan_mgmt->st_scan_request_spinlock));
    return OAL_SUCC;
}

uint32_t wal_elp_scan_abort_then_scan_comp_proc(oal_net_device_stru *net_dev)
{
    mac_vap_stru *mac_vap = oal_net_dev_priv(net_dev);   /* 下发扫描netdev对应的vap */
    mac_device_stru *mac_device = mac_res_get_dev(mac_vap->uc_device_id);
    hmac_vap_stru *hmac_vap = mac_res_get_hmac_vap(mac_device->uc_cfg_vap_id);   /* lp scan扫描使用cfg vap */
    hmac_scan_stru *scan_mgmt = hmac_get_wfsc_scan_mgmt();
...
void wal_elp_scan_abort_proc_imm(oal_net_device_stru *net_dev, hmac_device_stru *hmac_device, oal_bool_enum is_aborted)
{
    mac_vap_stru *mac_vap = oal_net_dev_priv(net_dev);   /* 下发扫描netdev对应的vap */
    mac_device_stru *mac_device = mac_res_get_dev(mac_vap->uc_device_id);
    hmac_vap_stru *hmac_vap = mac_res_get_hmac_vap(mac_device->uc_cfg_vap_id);   /* lp scan扫描使用cfg vap */
    ...
    wal_force_scan_complete_scan_mgmt_proc(net_dev, hmac_device, hmac_vap, scan_mgmt, is_aborted);
}

uint32_t wal_config_start_elp_scan(mac_vap_stru *pst_mac_vap, uint16_t us_len, uint8_t *puc_param)
{
    return hmac_elp_scan_proc(pst_mac_vap, us_len, puc_param);
}
