...
#endif
#include "hmac_scan_same_chan_remain_on_chan.h"
...
OAL_STATIC uint32_t wal_cfg80211_del_vap(mac_cfg_del_vap_param_stru *pst_del_vap_param)
{
    ...
    ***************************************************************************/
    ...
    if (wal_send_cfg_event(pst_net_dev, WAL_MSG_TYPE_WRITE,
        WAL_MSG_WRITE_MSG_HDR_LENGTH + sizeof(mac_cfg_del_vap_param_stru),
        (uint8_t *)&st_write_msg, OAL_TRUE, &pst_rsp_msg) != OAL_SUCC) {
                                         ...
                                         uint8_t uc_ssid_len)
{
    ...
    if (memcpy_s(pst_ssid_param->ac_ssid, sizeof(pst_ssid_param->ac_ssid),
        (int8_t *)puc_ssid_ie, uc_ssid_len) != EOK) {
...
#ifdef _PRE_WLAN_CHBA_MGMT
    /* 对于CHBA VAP的特殊启动 */
    wal_chba_start_ap_set_beacon_info(netdev, &st_beacon_param);
#endif
...
static void wal_cfg80211_start_ap_force_scan_abort(mac_vap_stru *mac_vap)
{
    hmac_device_stru *hmac_device = hmac_res_get_mac_dev(mac_vap->uc_device_id);
    hmac_scan_stru *scan_mgmt = NULL;
    hmac_vap_stru *hmac_scan_vap = NULL;
    if (hmac_device == NULL) {
        oam_warning_log0(mac_vap->uc_vap_id, OAM_SF_SCAN, "{wal_cfg80211_start_ap_force_scan_abort::hmac_device null}");
        return;
    }
    ...
    hmac_scan_vap = (hmac_vap_stru *)mac_res_get_hmac_vap(scan_mgmt->st_scan_record_mgmt.uc_vap_id);
    if (hmac_scan_vap == NULL) {
        oam_warning_log0(scan_mgmt->st_scan_record_mgmt.uc_vap_id, OAM_SF_SCAN,
                         "{wal_cfg80211_start_ap_force_scan_abort::hmac_vap is null!}");
        return;
    }
    wal_force_scan_abort_then_scan_comp(hmac_scan_vap->pst_net_device);
    return;
}
/*
    ...
    oal_ap_settings_stru *ap_settings)
{
    ...
    wal_cfg80211_start_ap_force_scan_abort(mac_vap);
    wal_cfg80211_start_ap_set_chba_mode(netdev, mac_vap);

    /*****************************************************************************
        1.设置信道
    *****************************************************************************/
    if (wal_cfg80211_set_channel_info(wiphy, netdev) != OAL_SUCC) {
        return -OAL_EFAIL;
    }
    ...
    /*****************************************************************************
        2.1 设置ssid信息
    *****************************************************************************/
    if (wal_cfg80211_start_ap_set_ssid(netdev, ap_settings, mac_vap) != OAL_SUCC) {
        return -OAL_EFAIL;
    }
    ...
    /*****************************************************************************
        2.2 设置auth mode信息
    *****************************************************************************/
    if (wal_cfg80211_set_auth_mode(netdev, ap_settings->auth_type) != OAL_SUCC) {
        return -OAL_EFAIL;
    }
...
int32_t wal_cfg80211_stop_ap(oal_wiphy_stru *pst_wiphy, oal_net_device_stru *pst_netdev)
{
    ...
    if ((g_optimized_feature_switch_bitmap & BIT(CUSTOM_OPTIMIZE_CHBA)) &&
        ...
        wal_del_vap_try_wlan_pm_close(wal_get_net_device_id(pst_netdev));
        return ret;
    }
#endif
...
static int32_t wal_cfg80211_change_intf_process(oal_net_device_stru *net_dev, enum nl80211_iftype if_type)
{
    ...
    if (wal_cfg80211_del_vap(&del_vap_param)) {
        wal_del_vap_try_wlan_pm_close(wal_get_net_device_id(net_dev));
        return -OAL_EFAIL;
    }
    ...
    mac_device_stru *mac_device, oal_wireless_dev_stru **wdev)
{
    ...
    *net_dev = wal_config_get_netdev(name, OAL_STRLEN(name));
    ...
    if (wal_is_p2p_group_exist(mac_device) == OAL_TRUE) {
        ...
        /* 后续可以考虑不下电，缩短时间 */
        wal_del_vap_try_wlan_pm_close(mac_device->uc_device_id);
        return ret;
    }
    return OAL_SUCC;
}
    ...
    oal_wireless_dev_stru **wdev)
{
    ...
    /* 申请内存 */
    if (memcpy_s(local_name, OAL_IF_NAME_SIZE, name, OAL_STRLEN(name)) != EOK) {
...
/* 功能描述: 发起remain_on_chan前打断扫描 */
int32_t wal_cfg80211_try_scan_abort_before_remain_on_chan(oal_net_device_stru *netdev, mac_device_stru *mac_device,
    mac_vap_stru *mac_vap, wlan_ieee80211_roc_type_uint8 roc_type)
{
    if (oal_any_null_ptr3(netdev, mac_device, mac_vap)) {
        oam_error_log0(0, OAM_SF_SCAN, "{wal_cfg80211_try_scan_abort_before_remain_on_chan:null ptr!}");
        return -OAL_EFAIL;
    }
    /* 有vap正在入网或扫描, 需避免对其干扰 */
    if (wal_drv_check_vap_state(mac_device, mac_vap, roc_type) != OAL_SUCC) {
        oam_warning_log0(mac_vap->uc_vap_id, OAM_SF_P2P,
            "{wal_cfg80211_try_scan_abort_before_remain_on_chan:check_vap_state fail!}");
        return -OAL_EFAIL;
    }
    oam_warning_log2(mac_vap->uc_vap_id, OAM_SF_P2P,
        "{wal_cfg80211_try_scan_abort_before_remain_on_chan:new roc type[%d], cur_vap_state[%d]!}",
        roc_type, mac_vap->en_vap_state);

    /* tx mgmt roc 优先级低, 可以被后来remain_on_chan命令打断 */
    if (mac_vap->en_vap_state == MAC_VAP_STATE_STA_LISTEN) {
        if (wal_p2p_stop_roc(mac_vap, netdev) < 0) {
            oam_error_log0(mac_vap->uc_vap_id, OAM_SF_P2P,
                "{wal_cfg80211_try_scan_abort_before_remain_on_chan:fail to stop roc!}");
            return -OAL_EFAIL;
        }
    }
    /* 定时同信道remain_on_chan，可以被后来命令打断 */
    hmac_reset_same_chan_remain_on_chan(mac_vap);

    /* 高优先级帧发送或接收, 允许打断其他vap进行的普通扫描 */
    if ((roc_type == IEEE80211_ROC_TYPE_CHBA_HIGH_PRIORITY_MGMT_TX) || (roc_type == IEEE80211_ROC_TYPE_CHBA_MGMT_RX)) {
    ...
    wlan_ieee80211_roc_type_uint8 roc_type)
{
    ...
    remain_on_channel.ull_cookie = *cookie;
    ...
    ***************************************************************************/
    ...
    /* 3.2 填写 msg 消息体 */
    if (memcpy_s(msg.auc_value, WAL_MSG_WRITE_MAX_LEN,
        &remain_on_channel, sizeof(hmac_remain_on_channel_param_stru)) != EOK) {
...
/* 功能描述:启动一次管理帧接收监听remain_on_chan */
int32_t wal_drv_start_mgmt_rx_remain_on_channel(oal_wireless_dev_stru *wdev,
    struct ieee80211_channel *chan, uint32_t duration, uint64_t *cookie, wlan_ieee80211_roc_type_uint8 roc_type)
{
    int32_t ret;
    uint64_t cur_cookie;
    mac_device_stru *mac_device = mac_res_get_dev(0);
    if (mac_device == NULL) {
        oam_error_log0(0, OAM_SF_ANY, "{wal_drv_start_mgmt_rx_remain_on_channel:fail to get mac_device}");
        return -OAL_EINVAL;
    }

    if (duration == 0) {
        oam_error_log0(0, OAM_SF_ANY, "{wal_drv_start_mgmt_rx_remain_on_channel:duration is 0!}");
        return -OAL_EINVAL;
    }
    cur_cookie = mac_device->st_p2p_info.ull_last_roc_id;
    ...
    *cookie = mac_device->st_p2p_info.ull_last_roc_id;
    ...
    ret = wal_drv_remain_on_channel(wdev, chan, duration, &cur_cookie, roc_type);
    if (ret == OAL_SUCC) {
        /* 上报暂停在指定信道成功 */
...
{
    int32_t ret;
    uint32_t duration_val = duration;
    oal_net_device_stru *netdev = NULL;
    mac_vap_stru *mac_vap = NULL;
    mac_device_stru *mac_device = mac_res_get_dev(0);
    if (oal_any_null_ptr5(pst_wiphy, pst_wdev, pst_chan, pull_cookie, mac_device)) {
        oam_error_log0(0, OAM_SF_P2P, "{wal_cfg80211_remain_on_channel::wiphy/wdev/chan/cookie/device is null!}");
        return -OAL_EINVAL;
    }

    netdev = pst_wdev->netdev;
    if (netdev == NULL) {
        oam_error_log0(0, OAM_SF_P2P, "{wal_cfg80211_remain_on_channel:netdev is NULL!}");
        return -OAL_EFAIL;
    }
    mac_vap = oal_net_dev_priv(netdev);
    if (mac_vap == NULL) {
        oam_warning_log0(0, OAM_SF_P2P, "{wal_cfg80211_remain_on_channel:mac_vap is NULL!}");
        return -OAL_EINVAL;
    }

    mac_device = mac_res_get_dev(mac_vap->uc_device_id);
    if (mac_device == NULL) {
        oam_warning_log1(0, OAM_SF_P2P, "{wal_cfg80211_remain_on_channel:mac_device[%d] NULL!}", mac_vap->uc_device_id);
        return -OAL_EINVAL;
    }

    /* 提前打断现有扫描 */
    ret = wal_cfg80211_try_scan_abort_before_remain_on_chan(netdev, mac_device, mac_vap, IEEE80211_ROC_TYPE_NORMAL);
    if (ret != OAL_SUCC) {
        oam_error_log0(0, OAM_SF_ANY, "{wal_cfg80211_remain_on_channel:fail to scan abort}");
        return ret;
    }

    if (duration_val == 0) {
        duration_val = WAL_CFG80211_REMAIN_ON_CHAN_TIMEOUT_MS;
        oam_warning_log1(mac_vap->uc_vap_id, OAM_SF_ANY, "{wal_cfg80211_remain_on_channel:duration is 0, set to %d ms}",
            WAL_CFG80211_REMAIN_ON_CHAN_TIMEOUT_MS);
    }
    return wal_drv_start_mgmt_rx_remain_on_channel(pst_wdev, pst_chan, duration_val, pull_cookie,
    ...
    oal_wireless_dev_stru *pst_wdev, uint64_t ull_cookie)
{
    ...
    ***************************************************************************/
    ...
    /* 3.2 填写 msg 消息体 */
    if (memcpy_s(st_write_msg.auc_value, sizeof(hmac_remain_on_channel_param_stru),
        &st_cancel_remain_on_channel, sizeof(hmac_remain_on_channel_param_stru)) != EOK) {
