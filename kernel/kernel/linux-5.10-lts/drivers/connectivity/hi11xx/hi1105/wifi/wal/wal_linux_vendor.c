    ...
    wal_wifi_priv_cmd_stru *priv_cmd, uint8_t *cmd)
{
    ...
    if (priv_cmd->total_len < sizeof(uint8_t)) {
        oam_error_log1(0, OAM_SF_ANY, "wal_vendor_set_change_go_channel:total_len=%d", priv_cmd->total_len);
        return -OAL_EFAIL;
    }

    if (oal_copy_to_user(priv_cmd->puc_buf, &val, sizeof(uint8_t)) != EOK) {
        oam_warning_log0(0, OAM_SF_CFG, "{wal_vendor_set_change_go_channel::Failed to copy ioctl_data to user !}");
        return OAL_FAIL;
    }
    priv_cmd->used_len = sizeof(uint8_t);
    return OAL_SUCC;
}
...
int32_t wal_vendor_get_radar_result(oal_net_device_stru *pst_net_dev, wal_wifi_priv_cmd_stru *priv_cmd, int8_t *pc_cmd)
{
    ...
    /* 8 为偏移头字节 */
    if (oal_copy_to_user(priv_cmd->puc_buf, (void *)pst_radar_result, sizeof(hmac_dfs_radar_result_stru)) != EOK) {
        oam_error_log0(0, OAM_SF_ANY, "wal_vendor_get_radar_result: Failed to copy ioctl_data to user !");
        return -OAL_EFAIL;
    }
    priv_cmd->used_len = sizeof(hmac_dfs_radar_result_stru);
#endif
...
static int32_t wal_vendor_parse_wur_param(int8_t *cmd, mac_vendor_wur_param_stru *cfg_wur_param)
{
    uint8_t *cmd_data = NULL;
    uint32_t offset;
    int8_t ac_name[WAL_HIPRIV_CMD_NAME_MAX_LEN] = {0};

    cmd_data = cmd + CMD_SET_WUR_PARAM_LEN + 1;
    if (wal_get_cmd_one_arg(cmd_data, ac_name, WAL_HIPRIV_CMD_NAME_MAX_LEN, &offset) != OAL_SUCC) {
        return -OAL_EFAIL;
    }
    cfg_wur_param->wur_enable = (uint8_t)oal_atoi(ac_name);
    cmd_data += offset;
    ...
    if (cfg_wur_param->wur_enable == OAL_TRUE) {
        ...
        cfg_wur_param->wur_sleep_mode = (uint8_t)oal_atoi(ac_name);
        cmd_data += offset;

        if (wal_get_cmd_one_arg(cmd_data, ac_name, WAL_HIPRIV_CMD_NAME_MAX_LEN, &offset) != OAL_SUCC) {
            return -OAL_EFAIL;
        }
        cfg_wur_param->listen_interval = (uint8_t)oal_atoi(ac_name);
        cmd_data += offset;

        if (wal_get_cmd_one_arg(cmd_data, ac_name, WAL_HIPRIV_CMD_NAME_MAX_LEN, &offset) != OAL_SUCC) {
            return -OAL_EFAIL;
        }
        cfg_wur_param->ls_listen_interval = (uint8_t)oal_atoi(ac_name);
    }

    return OAL_SUCC;
}

int32_t wal_vendor_set_wur_param_handle(oal_net_device_stru *net_dev, wal_wifi_priv_cmd_stru *priv_cmd, int8_t *cmd)
{
    wal_msg_write_stru write_msg;
    wal_msg_stru *rsp_msg = NULL;
    mac_vendor_wur_param_stru cfg_wur_param = {0};
    int32_t ret;
    uint32_t err_code;
    ...
    ret = wal_vendor_parse_wur_param(cmd, &cfg_wur_param);
    if (ret != OAL_SUCC) {
        return -OAL_EFAIL;
    }

    oam_warning_log4(0, OAM_SF_CFG,
        "wal_vendor_set_wur_param_handle::wur_enable[%d], sleep_mode[%d], listen_interval[%d], ls_listen_interval[%d]",
        cfg_wur_param.wur_enable, cfg_wur_param.wur_sleep_mode,
        cfg_wur_param.listen_interval, cfg_wur_param.ls_listen_interval);

    /* 抛事件到wal层处理 */
    wal_write_msg_hdr_init(&write_msg, WLAN_CFGID_SET_WUR_PARAM, sizeof(mac_vendor_wur_param_stru));

    /* 设置配置命令参数 */
    if (memcpy_s(write_msg.auc_value, sizeof(write_msg.auc_value),
        &cfg_wur_param, sizeof(mac_vendor_wur_param_stru)) != EOK) {
        oam_warning_log0(0, OAM_SF_CFG, "{wal_vendor_set_wur_param_handle::memcpy fail!}");
        return -OAL_EFAIL;
    }

    ret = wal_send_cfg_event(net_dev, WAL_MSG_TYPE_WRITE,
        WAL_MSG_WRITE_MSG_HDR_LENGTH + sizeof(mac_vendor_wur_param_stru), (uint8_t *)&write_msg, OAL_TRUE, &rsp_msg);
    if ((ret != OAL_SUCC) || (rsp_msg == NULL)) {
        oam_error_log1(0, OAM_SF_P2P, "{wal_vendor_set_wur_param_handle:: wal_alloc_cfg_event err %d!}", ret);
        return ret;
    }
    ...
    if (err_code != OAL_SUCC) {
        oam_warning_log1(0, OAM_SF_P2P, "{wal_vendor_set_wur_param_handle::check_msg_resp fail[%d]!}", err_code);
        return -OAL_EFAIL;
    }
    ...
    uint32_t cmd_name_len)
{
    ...
    /* 一个空格一个参数，最少2字节 */
    if (wal_ioctl_judge_input_param_length(*priv_cmd, cmd_name_len, 1) != OAL_SUCC) {
        oam_warning_log0(0, OAM_SF_ANY, "{wal_vendor_set_ap_isolation::length is too short!}");
        return OAL_FAIL;
    }

    wal_write_msg_hdr_init(&write_msg, WLAN_CFGID_SET_ISOLATION, 1);
    write_msg.auc_value[0] = *(cmd + cmd_name_len + 1);

    ret = wal_send_cfg_event(net_dev, WAL_MSG_TYPE_WRITE,
        WAL_MSG_WRITE_MSG_HDR_LENGTH + 1, (uint8_t *)&write_msg, OAL_FALSE, NULL);
    if (ret != OAL_SUCC) {
        oam_warning_log1(0, OAM_SF_CFG,
            "{wal_vendor_set_ap_isolation::wal_send_cfg_event return err code %d!}", ret);
        return ret;
    }
...
#define WLAN_BAND_WIDTH_20  20
#define WLAN_BAND_WIDTH_40  40
#define WLAN_BAND_WIDTH_80  80
#define WLAN_BAND_WIDTH_160 160
uint8_t wal_get_vap_band_width(wlan_channel_bandwidth_enum_uint8 enum_band_width)
{
    uint8_t band_width = WLAN_BAND_WIDTH_20;
    switch (enum_band_width) {
        case WLAN_BAND_WIDTH_20M:
            band_width = WLAN_BAND_WIDTH_20;
            break;
        case WLAN_BAND_WIDTH_40PLUS:
        case WLAN_BAND_WIDTH_40MINUS:
            band_width = WLAN_BAND_WIDTH_40;
            break;
        case WLAN_BAND_WIDTH_80PLUSPLUS:
        case WLAN_BAND_WIDTH_80PLUSMINUS:
        case WLAN_BAND_WIDTH_80MINUSPLUS:
        case WLAN_BAND_WIDTH_80MINUSMINUS:
            band_width = WLAN_BAND_WIDTH_80;
            break;
        ...
        case WLAN_BAND_WIDTH_160MINUSMINUSMINUS:
            band_width = WLAN_BAND_WIDTH_160;
            break;
#endif
        default:
            band_width = WLAN_BAND_WIDTH_20;
...
{
    uint32_t ret_number = 0;
    switch (bandwidth) {
        case WLAN_BW_CAP_20M:
            ret_number = WLAN_BAND_WIDTH_20;
            break;
        case WLAN_BW_CAP_40M:
            ret_number = WLAN_BAND_WIDTH_40;
            break;
        case WLAN_BW_CAP_80M:
            ret_number = WLAN_BAND_WIDTH_80;
            break;
        default :
            ret_number = WLAN_BAND_WIDTH_160;
            break;
    }
    return ret_number;
}
...
#define HISI_QUERY_ALL_USER_INFO "query_all_user_info"
#define KBPS_100 (100)

static void wal_vendor_get_vap_info(hmac_vap_stru *hmac_vap, hmac_vap_info_stru *hmac_vap_info)
{
    mac_vap_stru *mac_vap = &(hmac_vap->st_vap_base_info);
    if (memcpy_s(hmac_vap_info->mac_addr, MAC_ADDR_LEN, mac_vap->auc_bssid, MAC_ADDR_LEN) != EOK) {
        oam_error_log0(0, OAM_SF_ANY, "wal_vendor_get_vap_info:memcpy_s failed!");
    }
    hmac_vap_info->chan_num = mac_vap->st_channel.uc_chan_number;
    hmac_vap_info->bandwidth = wal_get_vap_band_width(mac_vap->st_channel.en_bandwidth);
    hmac_vap_info->dig = 0;
    hmac_vap_info->cca_shreshold = 0;
#ifdef CONFIG_HW_GET_EXT_SIG
    hmac_vap_info->channel_usage = hmac_vap->station_info.chload;
#endif
    hmac_vap_info->user_cnt = mac_vap->us_user_nums;
    hmac_vap_info->cca = 0;
    hmac_vap_info->channel_busy = 0;
    hmac_vap_info->vap_state = mac_vap->en_vap_state;
    hmac_vap_info->free_power = hmac_vap->s_free_power;
}

static void wal_vendor_get_user_info(mac_vap_stru *mac_vap, hmac_user_stru *hmac_user,
    hmac_user_info_stru *hmac_user_info)
{
    hmac_vap_stru *hmac_vap = NULL;
    mac_user_stru *mac_user = NULL;
    dpe_wlan_user_stat_stru user_stats = {0};

    hmac_vap = mac_res_get_hmac_vap(mac_vap->uc_vap_id);
    if (hmac_vap == NULL) {
        oam_error_log1(mac_vap->uc_vap_id, OAM_SF_ANY, "wal_vendor_get_user_info: hmac vap is null. vap id:%d",
            mac_vap->uc_vap_id);
        return;
    }

    mac_user = &(hmac_user->st_user_base_info);
    if (memcpy_s(hmac_user_info->mac_addr, MAC_ADDR_LEN, mac_user->auc_user_mac_addr,
        MAC_ADDR_LEN) != EOK) {
        oam_error_log0(0, OAM_SF_ANY, "wal_vendor_get_user_info:memcpy_s failed!");
    }
    if (dpe_wlan_user_stat_sync(mac_user->us_assoc_id,
        (uint8_t *)(&user_stats), sizeof(dpe_wlan_user_stat_stru)) != OAL_SUCC) {
        return;
    }
#ifdef _PRE_WLAN_FEATURE_HIEX
    hmac_user_info->dot11_cap = ((mac_user->st_hiex_cap.bit_ersru_enable) << BIT_OFFSET_2);
#endif
    hmac_user_info->wl_mode = mac_user->en_protocol_mode;
    hmac_user_info->rssi = hmac_user->c_rssi;
    hmac_user_info->snr = 0;
    hmac_user_info->tx_bandwidth = wal_bw_en_to_number(mac_user->en_cur_bandwidth);
    hmac_user_info->tx_rate = (uint16_t)(hmac_user->tx_rate / KBPS_100); /* 以100kbps为单位 */
    hmac_user_info->rx_rate = (uint16_t)(hmac_user->rx_rate / KBPS_100); /* 以100kbps为单位 */
    hmac_user_info->tx_retries = hmac_vap->station_info.tx_retries;
    hmac_user_info->tx_packets = user_stats.tx_packets;
    hmac_user_info->tx_failed = hmac_vap->station_info.tx_failed;
    hmac_user_info->rx_packets = user_stats.rx_packets;
    hmac_user_info->tx_unicast_bytes = 0;
    hmac_user_info->rx_unicast_bytes = 0;
    hmac_user_info->tx_mcast_bytes = 0;
    hmac_user_info->rx_mcast_bytes = 0;
    hmac_user_info->online_time = (oal_time_get_stamp_ms() - hmac_user->first_add_time) / MS_TO_S;
    return;
}
    ...
    uint32_t cmd_name_len)
{
    ...
    hmac_vap_stru *hmac_vap = NULL;
    hmac_vap_info_stru hmac_vap_info = {0};

    if (oal_any_null_ptr3(net_dev, priv_cmd, cmd)) {
        return OAL_FAIL;
    }
    if (priv_cmd->total_len < sizeof(hmac_vap_info_stru)) {
        oam_warning_log1(0, OAM_SF_ANY, "{wal_vendor_query_vap_info::length is too short[%d]!}", priv_cmd->total_len);
        return OAL_FAIL;
    }
    mac_vap = oal_net_dev_priv(net_dev);
    if (mac_vap == NULL) {
        oam_warning_log0(0, OAM_SF_CFG, "{wal_vendor_query_vap_info::netdev->mac_vap is null.}");
        return OAL_FAIL;
    }
    hmac_vap = (hmac_vap_stru *)mac_res_get_hmac_vap(mac_vap->uc_vap_id);
    if (hmac_vap == NULL) {
        oam_warning_log0(0, OAM_SF_CFG, "{wal_vendor_query_vap_info::hmac_vap is null.}");
        return OAL_FAIL;
    }
    /* 从vap获取信息 */
#ifdef _PRE_WLAN_VAP_PROC
    wal_vendor_query_vap_info_ext(mac_vap);
#endif
    wal_vendor_get_vap_info(hmac_vap, &hmac_vap_info);
    if (oal_copy_to_user(priv_cmd->puc_buf, (void *)&hmac_vap_info, sizeof(hmac_vap_info_stru)) != EOK) {
        oam_error_log0(0, OAM_SF_ANY, "wal_vendor_query_vap_info:Failed to copy data to user!");
        return OAL_FAIL;
    }
    priv_cmd->used_len = sizeof(hmac_vap_info_stru);

    oam_warning_log0(mac_vap->uc_vap_id, OAM_SF_CFG, "wal_vendor_query_vap_info:report_vap_info_succ");
    return OAL_SUCC;
}
    ...
    uint32_t cmd_name_len)
{
    ...
    hmac_user_info_stru hmac_user_info = {0};

    if (oal_any_null_ptr3(net_dev, priv_cmd, cmd)) {
        return OAL_FAIL;
    }
    if ((priv_cmd->total_len < sizeof(hmac_user_info_stru)) ||
        (wal_ioctl_judge_input_param_length(*priv_cmd, cmd_name_len, MAC_ADDR_LEN) != OAL_SUCC)) {
        oam_warning_log1(0, OAM_SF_ANY, "{wal_vendor_query_user_info::length is too short[%d]!}", priv_cmd->total_len);
        return OAL_FAIL;
    }
    mac_vap = oal_net_dev_priv(net_dev);
    if (mac_vap == NULL) {
        oam_warning_log0(0, OAM_SF_CFG, "{wal_vendor_query_user_info::netdev->mac_vap is null.}");
        return OAL_FAIL;
    }
    /* 从下发参数拷贝mac，命令和参数之间有一个空格 */
    if (memcpy_s(mac_addr, MAC_ADDR_LEN, cmd + cmd_name_len + 1, MAC_ADDR_LEN) != EOK) {
        oam_error_log0(0, OAM_SF_ANY, "wal_vendor_query_user_info:memcpy_s failed!");
        return OAL_FAIL;
    }
    hmac_user = mac_vap_get_hmac_user_by_addr(mac_vap, mac_addr, MAC_ADDR_LEN);
    if (hmac_user == NULL) {
        oam_warning_log0(0, OAM_SF_CFG, "{wal_vendor_query_user_info::find_user_by_mac_failed!}");
        return OAL_FAIL;
    }

    /* 从user获取信息 */
#ifdef _PRE_WLAN_VAP_PROC
    wal_vendor_get_user_info_ext(mac_vap, mac_addr);
#endif
    wal_vendor_get_user_info(mac_vap, hmac_user, &hmac_user_info);
    if (oal_copy_to_user(priv_cmd->puc_buf, (void *)&hmac_user_info, sizeof(hmac_user_info_stru)) != EOK) {
        oam_error_log0(0, OAM_SF_ANY, "wal_vendor_query_user_info:Failed to copy data to user!");
        return OAL_FAIL;
    }
    priv_cmd->used_len = sizeof(hmac_user_info_stru);

    oam_warning_log0(mac_vap->uc_vap_id, OAM_SF_CFG, "wal_vendor_query_user_info:report_user_info_succ");
    return OAL_SUCC;
}

#if defined(CONFIG_COMPAT) && defined(_PRE_HISI_HERTZ)
OAL_STATIC uint32_t wal_vendor_update_vap_info(wal_sta_status_diag_req_stru_compat *sta_diag_req_compat,
    mac_vap_stru *mac_vap, wal_wifi_priv_cmd_stru *priv_cmd, wal_sta_status_diag_req_stru *sta_diag_req)
{
    uint32_t ret = 0;

    if (sta_diag_req_compat == NULL || mac_vap == NULL || priv_cmd == NULL || sta_diag_req == NULL) {
        oam_error_log0(0, OAM_SF_ANY, "{wal_vendor_update_vap_info::para is null.}");
        return OAL_FAIL;
    }
    oam_warning_log2(0, OAM_SF_ANY, "{wal_vendor_update_vap_info::total_len is [%d] user_cnt[%d]}",
                     priv_cmd->total_len, mac_vap->us_user_nums);
    priv_cmd->used_len = strlen(HISI_QUERY_ALL_USER_INFO);
    ret = oal_copy_from_user(sta_diag_req_compat, priv_cmd->puc_buf + priv_cmd->used_len,
        sizeof(wal_sta_status_diag_req_stru_compat));
    if (ret != OAL_SUCC) {
        oam_error_log1(0, OAM_SF_ANY, "{wal_vendor_update_vap_info::oal_copy_from_user return error code %d!}",
            ret);
        return OAL_FAIL;
    }
    sta_diag_req->data_len = sta_diag_req_compat->data_len;
    sta_diag_req->sta_cnt  = sta_diag_req_compat->sta_cnt;
    sta_diag_req->channel  = sta_diag_req_compat->channel;
    sta_diag_req->data     = compat_ptr(sta_diag_req_compat->data);

    oam_warning_log3(0, OAM_SF_ANY, "wal_vendor_update_vap_info data_len:[%d] sta_cnt[%d] channel :%d !}",
        sta_diag_req->data_len, sta_diag_req->sta_cnt, sta_diag_req->channel);
    if ((sta_diag_req->data_len < sizeof(hmac_user_info_stru)) ||
        (sta_diag_req->data_len > (sizeof(hmac_user_info_stru) * WLAN_ASSOC_USER_MAX_NUM))) {
        oam_error_log1(0, OAM_SF_ANY, "{wal_vendor_update_vap_info:: buf_len is [%d].}",
            sta_diag_req->data_len);
        return OAL_FAIL;
    }
    return OAL_SUCC;
}

static uint32_t wal_vendor_upload_vap_info(wal_sta_status_diag_req_stru_compat *sta_diag_req_compat,
    wal_wifi_priv_cmd_stru *priv_cmd, wal_sta_status_diag_req_stru *sta_diag_req,
    hmac_user_info_stru *all_sta_diag_info, mac_vap_stru *mac_vap)
{
    if (sta_diag_req_compat == NULL || priv_cmd == NULL ||
        sta_diag_req == NULL || mac_vap == NULL) {
        oam_error_log0(0, OAM_SF_ANY, "{wal_vendor_upload_vap_info::para is null.}");
        return OAL_FAIL;
    }

    oam_warning_log1(0, OAM_SF_ANY, "sta_diag_req.sta_cnt * sizeof(hmac_user_info_stru):%d!",
        sta_diag_req->sta_cnt * sizeof(hmac_user_info_stru));
    if (oal_copy_to_user(sta_diag_req->data, all_sta_diag_info, sta_diag_req->data_len) != EOK) {
        oam_error_log0(0, OAM_SF_ANY, "wal_vendor_upload_vap_info fail!");
        return OAL_FAIL;
    }

    sta_diag_req_compat->sta_cnt = mac_vap->us_user_nums;
    if (mac_vap->us_user_nums > 0) {
        sta_diag_req_compat->channel = mac_vap->st_channel.uc_chan_number;
    } else {
        sta_diag_req_compat->channel = 0;
    }
    if (oal_copy_to_user(priv_cmd->puc_buf + priv_cmd->used_len, sta_diag_req_compat,
        sizeof(wal_sta_status_diag_req_stru_compat)) != EOK) {
        oam_error_log0(0, OAM_SF_ANY, "wal_vendor_upload_vap_info fail!");
        return OAL_FAIL;
    }
...
OAL_STATIC uint32_t wal_vendor_get_all_user_info(mac_vap_stru *mac_vap, hmac_user_info_stru *all_sta_diag_info)
{
    ...
    if (mac_vap == NULL) {
        oam_error_log0(0, OAM_SF_ANY, "{wal_vendor_get_all_user_info::para is null.}");
        return OAL_FAIL;
    }

    oal_dlist_search_for_each_safe(entry, entry_temp, &(mac_vap->st_mac_user_list_head))
    {
        ...
        /* 从user获取信息 */
#ifdef _PRE_WLAN_VAP_PROC
        wal_vendor_get_user_info_ext(mac_vap, mac_user->auc_user_mac_addr);
#endif
        wal_vendor_get_user_info(mac_vap, hmac_user, &hmac_user_info);
        if (memcpy_s(all_sta_diag_info + sta_count, sizeof(hmac_user_info_stru), (void *)&hmac_user_info,
            sizeof(hmac_user_info_stru)) != EOK) {
            oam_error_log0(0, OAM_SF_ANY, "wal_vendor_get_all_user_info::memcpy fail!");
            return OAL_FAIL;
        }
        sta_count += 1;
    }
...
{
    mac_vap_stru *mac_vap = NULL;
    wal_sta_status_diag_req_stru sta_diag_req;
    hmac_user_info_stru *all_sta_diag_info = NULL;
    uint32_t ret = 0;
    wal_sta_status_diag_req_stru_compat sta_diag_req_compat;

    if (oal_any_null_ptr3(net_dev, priv_cmd, cmd)) {
        oam_error_log0(0, OAM_SF_ANY, "{wal_vendor_query_all_user_info::oal_any_null_ptr3.}");
        return OAL_FAIL;
    }
    mac_vap = oal_net_dev_priv(net_dev);
    if (mac_vap == NULL) {
        oam_warning_log0(0, OAM_SF_CFG, "{wal_vendor_query_all_user_info::netdev->mac_vap is null.}");
        return OAL_FAIL;
    }

    ret = wal_vendor_update_vap_info(&sta_diag_req_compat, mac_vap, priv_cmd, &sta_diag_req);
    if (ret != OAL_SUCC) {
        oam_error_log1(0, OAM_SF_ANY, "{wal_vendor_update_vap_info::return error code %d!}", ret);
        return OAL_FAIL;
    }

    all_sta_diag_info = (hmac_user_info_stru *)oal_memalloc(sta_diag_req.data_len);
    if (all_sta_diag_info == NULL) {
        oam_error_log1(0, OAM_SF_ANY, "{wal_vendor_query_all_user_info:: OAL_MALLOC failed buf len [%d].}",
            sta_diag_req.data_len);
        return OAL_FAIL;
    }
    memset_s(all_sta_diag_info, sta_diag_req.data_len, 0, sta_diag_req.data_len);

    ret = wal_vendor_get_all_user_info(mac_vap, all_sta_diag_info);
    if (ret != OAL_SUCC) {
        oam_error_log1(0, OAM_SF_ANY, "{wal_vendor_get_all_user_info::return error code %d!}", ret);
        oal_free(all_sta_diag_info);
        return OAL_FAIL;
    }
    ret = wal_vendor_upload_vap_info(&sta_diag_req_compat, priv_cmd, &sta_diag_req, all_sta_diag_info, mac_vap);
    if (ret != OAL_SUCC) {
        oam_error_log1(0, OAM_SF_ANY, "{wal_vendor_upload_vap_info::return error code %d!}", ret);
        oal_free(all_sta_diag_info);
        return OAL_FAIL;
    }
    oal_free(all_sta_diag_info);
    oam_warning_log0(mac_vap->uc_vap_id, OAM_SF_CFG, "wal_vendor_query_all_user_info:report_all_user_info_succ");
    return OAL_SUCC;
}
#endif /* end if defined(CONFIG_COMPAT) && defined(_PRE_HISI_HERTZ) here */
#endif
...
int32_t wal_vendor_get_radar_block_status(oal_net_device_stru *net_dev, wal_wifi_priv_cmd_stru *priv_cmd, uint8_t *cmd,
    uint32_t cmd_name_len)
{
    uint8_t channel_number = 0; // 信道号
    uint8_t channel_index = 0; // 信道索引号
    mac_device_stru *mac_device = NULL;
    uint8_t block_status = OAL_FALSE;

    if (oal_any_null_ptr3(net_dev, priv_cmd, cmd)) {
        oam_error_log0(0, OAM_SF_ANY, "{wal_vendor_get_radar_block_status::oal_any_null_ptr3.}");
        return OAL_FAIL;
    }

    mac_device = mac_res_get_dev(0);
    if (oal_unlikely(mac_device == NULL)) {
        oam_warning_log0(0, OAM_SF_CFG, "{wal_vendor_get_radar_block_status::mac_device is null.}");
        return OAL_FAIL;
    }

    if (wal_ioctl_judge_input_param_length(*priv_cmd, cmd_name_len, sizeof(uint8_t)) != OAL_SUCC) {
        oam_warning_log1(0, OAM_SF_ANY, "{wal_vendor_get_radar_block_status::length is too short[%d]!}",
            priv_cmd->total_len);
        return OAL_FAIL;
    }

    //  先从cmd 里拷贝1字节的，作为 信道号 参数
    channel_number = cmd[cmd_name_len + 1];

    // 获取 hal_dev，如果是 block  ，状态写 1 ，回传，通过 cpy to user，  否则写 0，通过 priv_cmd 回传.
    if (mac_get_channel_idx_from_num(MAC_RC_START_FREQ_5, channel_number, OAL_FALSE, &channel_index) != OAL_SUCC) {
        oam_warning_log0(0, 0, "{wal_vendor_get_radar_block_status get idx from num fail}");
        return OAL_FAIL;
    }
    if (channel_index >= MAC_CHANNEL_FREQ_5_BUTT) {
        oam_warning_log1(0, 0, "{wal_vendor_get_radar_block_status channel_index invalid [%d]}", channel_index);
        return OAL_FAIL;
    }

    if ((mac_device->st_ap_channel_list[channel_index].en_ch_status == MAC_CHAN_BLOCK_DUE_TO_RADAR) &&
        (mac_device->st_dfs.st_dfs_info.dfsmode == DFS_NO_DEBUG_MODE)) {
        block_status = OAL_TRUE; // block 时，回传1，否则回传0
    }
    oam_warning_log2(0, 0, "{wal_vendor_get_radar_block_status check channel[%d] radar block_status is [%d]}",
        channel_number, block_status);
    if (oal_copy_to_user(priv_cmd->puc_buf, (void *)&block_status, sizeof(uint8_t)) != EOK) {
        oam_error_log0(0, OAM_SF_ANY, "{wal_vendor_get_radar_block_status fail to copy data to user}");
        return OAL_FAIL;
    }
    return OAL_SUCC;
}
...
OAL_STATIC uint32_t wal_ioctl_set_rx_listen_ps_switch(oal_net_device_stru *pst_net_dev, int8_t *pc_command)
{
    ...
    /* 抛事件到wal层处理 */
    wal_write_msg_hdr_init(&st_write_msg, WLAN_CFGID_RX_LISTEN_PS_SWITCH, sizeof(mac_rx_listen_ps_switch_stru));
    /* 设置配置命令参数 */
...
int32_t wal_vendor_priv_cmd_power_saving(oal_net_device_stru *pst_net_dev, oal_ifreq_stru *pst_ifr, uint8_t *pc_cmd,
    wal_wifi_priv_cmd_stru *priv_cmd, int32_t *ps_ret)
{
    if (oal_strncasecmp(pc_cmd, CMD_GET_FAST_SLEEP_CNT, CMD_GET_FAST_SLEEP_CNT_LEN) == 0) {
        *ps_ret = wal_ioctl_get_psm_stat(pst_net_dev, MAC_PSM_QUERY_FASTSLEEP_STAT, priv_cmd);
        return OAL_SUCC;
    }
    ...
    if (g_wlan_spec_cfg->feature_psm_dfx_ext_is_open) {
        if (oal_strncasecmp(pc_cmd, CMD_GET_BEACON_CNT, CMD_GET_BEACON_CNT_LEN) == 0) {
            *ps_ret = wal_ioctl_get_psm_stat(pst_net_dev, MAC_PSM_QUERY_BEACON_CNT, priv_cmd);
            return OAL_SUCC;
        }
    }
#endif
    ...
    if (g_wlan_spec_cfg->rx_listen_ps_is_open) {
        ...
        if (oal_strncasecmp(pc_cmd, CMD_GET_RX_LISTEN_STATE, OAL_STRLEN(CMD_GET_RX_LISTEN_STATE)) == 0) {
            *ps_ret = wal_ioctl_get_psm_stat(pst_net_dev, MAC_PSM_QUERY_RX_LISTEN_STATE, priv_cmd);
            return OAL_SUCC;
        }
        if (oal_strncasecmp(pc_cmd, CMD_SET_RX_LISTEN_PS_SWITCH, OAL_STRLEN(CMD_SET_RX_LISTEN_PS_SWITCH)) == 0) {
            *ps_ret = wal_ioctl_set_rx_listen_ps_switch(pst_net_dev, pc_cmd);
            return OAL_SUCC;
        }
    }
#endif
...
#ifdef _PRE_WLAN_FEATURE_11BE
#define CMD_BE_BLACKLIST_MIN_DATA_LEN 2
static int32_t wal_vendor_11be_wifi7_blacklist_handle(oal_net_device_stru *pst_net_dev, uint8_t *pc_cmd,
    uint32_t total_len)
{
    int32_t  ret;
    uint8_t  blacklist_cnt;
    ...
    if (total_len < (CMD_SET_BE_BLACKLIST_LEN + CMD_BE_BLACKLIST_MIN_DATA_LEN)) {
        oam_warning_log1(0, OAM_SF_11BE, "{wal_vendor_11be_wifi7_blacklist_handle::len %d is too small.}", total_len);
        return OAL_FAIL;
    }
    remain_len = total_len - (CMD_SET_BE_BLACKLIST_LEN + CMD_BE_BLACKLIST_MIN_DATA_LEN);
    cmd_data = pc_cmd + CMD_SET_BE_BLACKLIST_LEN + 1;
    blacklist_cnt = (uint8_t)*cmd_data; /* blacklist_cnt=0表示清空BE黑名单 */
    cmd_data += 1;
    /* 命令中黑名单个数与后面的黑名单信息不匹配，返回失败 */
    if ((blacklist_cnt > HMAC_WIFI7_SELF_CURE_BLACK_LIST_MAX_NUM) ||
        (blacklist_cnt * sizeof(hmac_wifi7_self_cure_blacklist_stru) != remain_len)) {
        oam_warning_log3(0, OAM_SF_11BE, "{wal_vendor_11be_wifi7_blacklist_handle::blacklist_cnt=[%d],\
            max_num=[%d] remain[%d].}", blacklist_cnt, HMAC_WIFI7_SELF_CURE_BLACK_LIST_MAX_NUM, remain_len);
        return OAL_FAIL;
    }

    wal_write_msg_hdr_init(&write_msg, WLAN_CFGID_SET_11BE_WIFI7_BLACKLIST, sizeof(hmac_wifi7_self_cure_info_stru));
    wifi7_self_cure_info = (hmac_wifi7_self_cure_info_stru *)write_msg.auc_value;
    wifi7_self_cure_info->blaclist_num = blacklist_cnt;
    if (blacklist_cnt != 0) {
        ret = memcpy_s(wifi7_self_cure_info->wifi7_blacklist,
            blacklist_cnt * sizeof(hmac_wifi7_self_cure_blacklist_stru),
            cmd_data, blacklist_cnt * sizeof(hmac_wifi7_self_cure_blacklist_stru));
        if (oal_unlikely(ret != OAL_SUCC)) {
            oam_warning_log1(0, OAM_SF_11BE, "{wal_vendor_11be_wifi7_blacklist_handle::memcpy err = [%d].}\r\n", ret);
            return ret;
        }
    }
    ret = wal_send_cfg_event(pst_net_dev, WAL_MSG_TYPE_WRITE,
        WAL_MSG_WRITE_MSG_HDR_LENGTH + sizeof(hmac_wifi7_self_cure_info_stru), (uint8_t *)&write_msg, OAL_FALSE, NULL);
    if (oal_unlikely(ret != OAL_SUCC)) {
        oam_warning_log1(0, OAM_SF_11BE, "{wal_vendor_11be_wifi7_blacklist_handle::return err code = [%d].}\r\n", ret);
        return ret;
    }
    return OAL_SUCC;
}
...
#ifdef _PRE_WLAN_FEATURE_ML
static int32_t wal_vendor_set_mlsr_link_switch(oal_net_device_stru *net_dev, uint8_t *pc_cmd, uint32_t total_len)
{
    mac_vap_stru *mac_vap = NULL;
    hmac_vap_stru *hmac_vap = NULL;
    wal_msg_write_stru write_msg;
    uint8_t *cmd_str = pc_cmd + CMD_SET_MLSR_LINK_SWITCH_LEN + 1;
    int8_t ac_name[WAL_HIPRIV_CMD_NAME_MAX_LEN] = {0};
    uint8_t expect_macaddr[WLAN_MAC_ADDR_LEN] = {0};
    mac_link_switch_param_stru link_switch_param = {0};
    uint32_t off_set;
    int32_t ret;

    mac_vap = oal_net_dev_priv(net_dev);
    if (oal_unlikely(mac_vap == NULL)) {
        oam_warning_log0(0, OAM_SF_ANY, "{wal_vendor_set_mlsr_link_switch::OAL_NET_DEV_PRIV(net_dev) null!}");
        return OAL_ERR_CODE_PTR_NULL;
    }
    hmac_vap = (hmac_vap_stru *)mac_res_get_hmac_vap(mac_vap->uc_vap_id);
    if (oal_unlikely(hmac_vap == NULL)) {
        oam_error_log0(mac_vap->uc_vap_id, OAM_SF_11BE, "{wal_vendor_set_mlsr_link_switch::hmac_vap is null");
        return OAL_ERR_CODE_PTR_NULL;
    }

    if (wal_get_cmd_one_arg(cmd_str, ac_name, WAL_HIPRIV_CMD_NAME_MAX_LEN, &off_set) != OAL_SUCC) {
        oam_error_log0(mac_vap->uc_vap_id, OAM_SF_11BE, "{wal_vendor_set_mlsr_link_switch::get_cmd_arg fail");
        return OAL_FAIL;
    }
    oal_strtoaddr(ac_name, sizeof(ac_name), expect_macaddr, WLAN_MAC_ADDR_LEN);

    /* 解析命令后携带的mac地址，确认是否为MLSR关联的link 且 当前为工作，取link id下发 */
    link_switch_param.enable_link_id = hmac_mld_get_link_id_by_macaddr(hmac_vap, expect_macaddr);
    link_switch_param.trigger_reason = LINK_SWITCH_FOR_IOCTL;
    if (link_switch_param.enable_link_id >= WLAN_MAX_ML_LINK_NUM) {
        oam_error_log1(mac_vap->uc_vap_id, OAM_SF_11BE, "{wal_vendor_set_mlsr_link_switch::link_id[%d]",
            link_switch_param.enable_link_id);
        return OAL_FAIL;
    }

    if (memcpy_s(write_msg.auc_value, sizeof(write_msg.auc_value), &link_switch_param,
        sizeof(mac_link_switch_param_stru)) != EOK) {
        oam_error_log0(0, OAM_SF_ANY, "{wal_vendor_set_mlsr_link_switch::memcpy fail!}");
        return OAL_FAIL;
    }
    wal_write_msg_hdr_init(&write_msg, WLAN_CFGID_MLSR_WROK_LINK_SWITCH, sizeof(mac_link_switch_param_stru));

    ret = wal_send_cfg_event(net_dev, WAL_MSG_TYPE_WRITE,
        WAL_MSG_WRITE_MSG_HDR_LENGTH + sizeof(mac_link_switch_param_stru), (uint8_t*)&write_msg, OAL_FALSE, NULL);
    if (oal_unlikely(ret != OAL_SUCC)) {
        oam_warning_log1(0, OAM_SF_11BE, "{wal_vendor_set_mlsr_link_switch::return err code = [%d].}\r\n", ret);
        return ret;
    }

    oam_warning_log4(mac_vap->uc_vap_id, OAM_SF_11BE,
        "{wal_vendor_set_mlsr_link_switch::dest_macaddr[%x:xx:xx:xx:%x:%x] link_id[%d]",
        expect_macaddr[BYTE_OFFSET_0], expect_macaddr[BYTE_OFFSET_4], expect_macaddr[BYTE_OFFSET_5],
        link_switch_param.enable_link_id);
...
uint32_t wal_vendor_config_emlsr_switch(oal_net_device_stru *net_dev, uint8_t *cmd, uint32_t total_len)
{
    mac_vap_stru *mac_vap = NULL;
    wal_msg_write_stru write_msg = {0};
    uint8_t *cmd_str = cmd + CMD_EMLSR_SWITCH_LEN + 1;
    char ac_name[WAL_HIPRIV_CMD_NAME_MAX_LEN] = {0};
    mac_cfg_emlsr_action_param_stru *emlsr_param = (mac_cfg_emlsr_action_param_stru *)write_msg.auc_value;
    uint32_t off_set;
    int32_t ret;

    if (total_len <= CMD_EMLSR_SWITCH_LEN + 1) {
        oam_error_log0(0, OAM_SF_11BE, "{wal_vendor_config_emlsr_switch::total_len[%d] invalid!}");
        return OAL_FAIL;
    }

    mac_vap = oal_net_dev_priv(net_dev);
    if (oal_unlikely(mac_vap == NULL)) {
        oam_error_log0(0, OAM_SF_ANY, "{wal_vendor_set_mlsr_link_switch::OAL_NET_DEV_PRIV(net_dev) null!}");
        return OAL_ERR_CODE_PTR_NULL;
    }
    ...
    emlsr_param->enter_reason = EMLSR_ENTER_IOCTRL;
    emlsr_param->emlsr_mode = (uint8_t)oal_atoi(ac_name);
    oam_warning_log1(mac_vap->uc_vap_id, OAM_SF_EMLSR,
        "{wal_vendor_config_emlsr_switch::emlsr_switch[%d]", emlsr_param->emlsr_mode);

    wal_write_msg_hdr_init(&write_msg, WLAN_CFGID_EMLSR_ENABLE, sizeof(mac_cfg_emlsr_action_param_stru));
    ret = wal_send_cfg_event(net_dev, WAL_MSG_TYPE_WRITE,
                             WAL_MSG_WRITE_MSG_HDR_LENGTH + sizeof(mac_cfg_emlsr_action_param_stru),
                             (uint8_t*)&write_msg, OAL_FALSE, NULL);
    if (oal_unlikely(ret != OAL_SUCC)) {
        oam_warning_log1(0, OAM_SF_11BE, "{wal_vendor_config_emlsr_switch::return err code = [%d].}\r\n", ret);
    }
...
static int32_t wal_vendor_get_ap_bandwidth(oal_net_device_stru *pst_net_dev, wal_wifi_priv_cmd_stru *priv_cmd)
{
    ...
    } else if (pst_mac_vap->st_channel.en_bandwidth <= WLAN_BAND_WIDTH_80MINUSMINUS) {
        uc_current_bandwidth = WLAN_AP_BANDWIDTH_80M;
    } else {
#ifdef _PRE_WLAN_FEATURE_160M
        if (pst_mac_vap->st_channel.en_bandwidth > WLAN_BAND_WIDTH_160MINUSMINUSMINUS) {
            return -OAL_EFAIL;
        }
    ...
    if (oal_copy_to_user(priv_cmd->puc_buf, &uc_current_bandwidth, sizeof(uint8_t)) != EOK) {
...
/*
 * 函 数 名  : wal_vendor_get_wifi_category_supported
 * 功能描述  : 获取定制化STA支持的wifi类型
 * 1.日    期  : 2019.10.22
 *   作    者  : wifi
 *   修改内容  : 新增函数
 */
int32_t wal_vendor_get_wifi_category_supported(oal_net_device_stru *pst_net_dev, wal_wifi_priv_cmd_stru *priv_cmd)
{
    uint32_t l_priv_value_11ax, l_priv_value_11be;
    uint8_t uc_val = 0;

#ifdef _PRE_PLAT_FEATURE_CUSTOMIZE
    l_priv_value_11ax = wlan_chip_get_11ax_switch_mask();
    l_priv_value_11be = wlan_chip_get_11be_switch_mask();
#else
    l_priv_value_11ax = 0;
    l_priv_value_11be = 0;
#endif
    /* 判断STA 11be定制化是否打开 */
    if ((uint32_t)l_priv_value_11be & BIT0) {
        uc_val |= PROTOCOL_WIFI7_SUPPORT;
    }
    /* 判断STA 11ax定制化是否打开 */
    if ((uint32_t)l_priv_value_11ax & BIT0) {
        uc_val |= PROTOCOL_WIFI6_SUPPORT;
    }
    if (uc_val == 0) {
        uc_val = PROTOCOL_DEFAULT;
    }
    if (oal_copy_to_user(priv_cmd->puc_buf, &uc_val, sizeof(uint8_t)) != EOK) {
        oam_error_log0(0, OAM_SF_CFG, "{wal_vendor_get_wifi_category_supported::Failed to copy ioctl_data to user !}");
        return OAL_FAIL;
    }
    priv_cmd->used_len = (uint32_t)sizeof(uint8_t);
    return OAL_SUCC;
}
...
#endif
    if (oal_copy_to_user(priv_cmd->puc_buf, &aput_support_160m, sizeof(oal_bool_enum_uint8)) != EOK) {
        return -OAL_EFAIL;
    }
    priv_cmd->used_len = sizeof(oal_bool_enum_uint8);
    return OAL_SUCC;
}
...
int32_t wal_ioctl_judge_input_param_length(wal_wifi_priv_cmd_stru priv_cmd, uint32_t cmd_length, uint16_t adjust_val)
{
    ...
    if (priv_cmd.total_len < cmd_length + 1 + adjust_val) {
    ...
    int8_t *pc_command, wal_wifi_priv_cmd_stru st_priv_cmd)
{
    ...
    if (memcpy_s(auc_country_code, sizeof(auc_country_code), country_code, NUM_2_BYTES) != EOK) {
    ...
    uint8_t *p_command)
{
    ...
    while (wal_get_cmd_one_arg(p_tmp, arg, WAL_HIPRIV_CMD_NAME_MAX_LEN, &offset) == OAL_SUCC) {
...
OAL_STATIC int32_t wal_get_tb_frame_gain(oal_net_device_stru *net_dev, wal_wifi_priv_cmd_stru *priv_cmd)
{
    ...
    if (oal_copy_to_user(priv_cmd->puc_buf, narrowband_gain_info, sizeof(narrowband_gain_info) != EOK)) {
        oam_error_log0(0, OAM_SF_CFG, "{wal_get_narrowband_gain::Failed to copy data to user!}");
        return -OAL_EFAIL;
    }
    priv_cmd->used_len = sizeof(narrowband_gain_info);
    return OAL_SUCC;
}
#endif
/*
    ...
    uint8_t *pc_cmd, wal_wifi_priv_cmd_stru *p_priv_cmd)
{
    ...
    } else if  (oal_strncasecmp(pc_cmd, CMD_PK_APP, CMD_PK_APP_LEN) == 0) {
        if (OAL_STRLEN(pc_cmd) < (CMD_PK_APP_LEN + CMD_LENGTH)) {
            oam_error_log0(0, OAM_SF_ANY, "{wal_vendor_priv_cmd_hid2d::napi ctrl, cmd invalid}");
            return -OAL_EFAIL;
        }
        return wal_set_rx_mode(pst_net_dev, oal_atoi(pc_cmd + CMD_PK_APP_LEN + 1));
...
{
    if (oal_strncasecmp(p_cmd, CMD_GET_WIFI_CATEGORY_SUPPORT, CMD_GET_WIFI_CATEGORY_SUPPORT_LEN) == 0) {
        return wal_vendor_get_wifi_category_supported(p_net_dev, p_priv_cmd);
    }
#ifdef _PRE_WLAN_FEATURE_11AX
...
#endif
#ifdef _PRE_WLAN_FEATURE_11BE
    if (oal_strncasecmp(p_cmd, CMD_SET_BE_BLACKLIST, CMD_SET_BE_BLACKLIST_LEN) == 0) {
        return wal_vendor_11be_wifi7_blacklist_handle(p_net_dev, p_cmd, p_priv_cmd->used_len);
    }
#endif
#ifdef _PRE_WLAN_FEATURE_ML
    if (oal_strncasecmp(p_cmd, CMD_SET_MLSR_LINK_SWITCH, CMD_SET_MLSR_LINK_SWITCH_LEN) == 0) {
        return wal_vendor_set_mlsr_link_switch(p_net_dev, p_cmd, p_priv_cmd->used_len);
    }
#endif
...
#ifdef _PRE_WLAN_FEATURE_WUR
    if (oal_strncasecmp(p_cmd, CMD_SET_WUR_PARAM, CMD_SET_WUR_PARAM_LEN) == 0) {
        return wal_vendor_set_wur_param_handle(p_net_dev, p_priv_cmd, p_cmd);
    }
#endif
    ...
    oal_ifreq_stru *pst_ifr, wal_wifi_priv_cmd_stru *p_priv_cmd, uint8_t *pc_cmd)
{
    ...
    if (wal_vendor_priv_cmd_power_saving(pst_net_dev, pst_ifr, pc_cmd, p_priv_cmd, &ret) == OAL_SUCC) {
        if (ret != OAL_SUCC) {
    ...
    if (oal_strncasecmp(pc_cmd, CMD_GET_RADAR_RESULT, CMD_GET_RADAR_RESULT_LEN) == 0) {
        return wal_vendor_get_radar_result(pst_net_dev, p_priv_cmd, pc_cmd);
    }
#ifdef _PRE_WLAN_FEATURE_HIEX
    if (oal_strncasecmp(pc_cmd, CMD_GET_TB_FRAME_GAIN, CMD_GET_TB_FRAME_GAIN_LEN) == 0) {
        return wal_get_tb_frame_gain(pst_net_dev, p_priv_cmd);
    }
#endif
    if (oal_strncasecmp(pc_cmd, CMD_SET_P2P_SCENE, CMD_SET_P2P_SCENE_LEN) == 0) {
    ...
    if (oal_strncasecmp(pc_cmd, CMD_SET_CHANGE_GO_CHANNEL, CMD_SET_CHANGE_GO_CHANNEL_LEN) == 0) {
        return wal_vendor_set_change_go_channel(pst_net_dev, p_priv_cmd, pc_cmd);
    }
    if (oal_strncasecmp(pc_cmd, CMD_SET_DYNAMIC_DBAC_MODE, CMD_SET_DYNAMIC_DBAC_MODE_LEN) == 0) {
        return wal_ioctl_set_dynamic_dbac_mode(pst_net_dev, p_priv_cmd, pc_cmd);
    }
#ifdef _PRE_WLAN_FEATURE_EMLSR
    if (oal_strncasecmp(pc_cmd, CMD_SET_EMLSR_SWITCH, CMD_EMLSR_SWITCH_LEN) == 0) {
        return wal_vendor_config_emlsr_switch(pst_net_dev, pc_cmd, p_priv_cmd->used_len);
    }
#endif
    ...
    wal_wifi_priv_cmd_stru *pst_priv_cmd)
{
    ...
    for (uc_cfg_id = VOWIFI_SET_MODE; uc_cfg_id < VOWIFI_CMD_BUTT; uc_cfg_id++) {
        if ((oal_strncmp(puc_command, g_ast_vowifi_cmd_table[uc_cfg_id].pc_priv_cmd,
            OAL_STRLEN(g_ast_vowifi_cmd_table[uc_cfg_id].pc_priv_cmd)) == 0) &&
...
static uint32_t wal_ioctl_parse_wps_p2p_ie(oal_app_ie_stru *pst_app_ie, uint8_t *puc_src, uint32_t src_len)
{
    ...
    while (len_remain > MAC_IE_HDR_LEN) {
        ies_info.frame_ies = puc_buf_remain;
        ies_info.frame_len = len_remain;
        /* MAC_EID_WPS,MAC_EID_P2P ID均为221 */
        puc_ie = mac_find_ie(MAC_EID_P2P, &ies_info);
        if (puc_ie != NULL) {
            ...
            if (memcpy_s(&(pst_app_ie->auc_ie[len]), (WLAN_WPS_IE_MAX_SIZE - len), puc_ie, ie_len) != EOK) {
...
OAL_STATIC void wal_ioctl_set_fastsleep_switch_g_variable_assignment(oal_net_device_stru *net_dev,
    uint8_t *para_val, uint8_t para_len)
{
    ...
    if (is_legacy_sta(mac_vap)) {
        g_wlan_min_fast_ps_idle = para_val[0];
        g_wlan_max_fast_ps_idle = para_val[1];
        g_wlan_auto_ps_screen_on = para_val[BYTE_OFFSET_2];
        g_wlan_auto_ps_screen_off = para_val[BYTE_OFFSET_3];
    } else {
        g_wlan_min_fast_ps_idle_p2p = para_val[0];
        g_wlan_max_fast_ps_idle_p2p = para_val[1];
        g_wlan_auto_ps_screen_on_p2p = para_val[BYTE_OFFSET_2];
        g_wlan_auto_ps_screen_off_p2p = para_val[BYTE_OFFSET_3];
    }
    oam_warning_log3(0, OAM_SF_CFG, "wal_ioctl_set_fastsleep_switch:p2p[%d] g_wlan_min_fast_ps_idle[%d] \
        max_fast_ps_idle[%d]!", mac_vap->en_p2p_mode, para_val[0], para_val[1]);
    oam_warning_log2(0, OAM_SF_CFG, "wal_ioctl_set_fastsleep_switch: \
        auto_ps_thresh_screen_on[%d], auto_ps_thresh_screen_off[%d]!",
        para_val[BYTE_OFFSET_2], para_val[BYTE_OFFSET_3]);
}

OAL_STATIC uint32_t wal_ioctl_set_fastsleep_param(oal_net_device_stru *net_dev, int8_t *para_str)
{
    mac_vap_stru *mac_vap = oal_net_dev_priv(net_dev);
    hmac_vap_stru *hmac_vap = NULL;
    uint8_t para_val[BYTE_OFFSET_4] = { 0 };
    uint8_t str_tmp[WAL_HIPRIV_CMD_NAME_MAX_LEN] = {0};
    uint32_t cmd_offset;
    uint32_t para_cnt;
    uint32_t ret;
    ...
    if (hmac_vap->wur_info.is_enabled == OAL_TRUE) {
        oam_error_log0(mac_vap->uc_vap_id, OAM_SF_CFG, "wal_ioctl_set_fastsleep_param::wur is enabled, NOT enter fs");
        return OAL_FAIL;
    }
#endif
    ...
    for (para_cnt = 0; para_cnt < BYTE_OFFSET_4; para_cnt++) {
        ret = wal_get_cmd_one_arg(para_str, str_tmp, WAL_HIPRIV_CMD_NAME_MAX_LEN, &cmd_offset);
        if (ret != OAL_SUCC) {
            oam_warning_log2(0, OAM_SF_CFG, "{wal_ioctl_set_fastsleep_switch::get para[%d] fail, err_code[%d]}",
                para_cnt, ret);
            return OAL_FAIL;
        }
        para_val[para_cnt] = (uint8_t)oal_atoi(str_tmp);
        para_str += cmd_offset;
    }
    para_val[0] /= WLAN_SLEEP_TIMER_PERIOD;
    para_val[1] /= WLAN_SLEEP_TIMER_PERIOD;
    wal_ioctl_set_fastsleep_switch_g_variable_assignment(net_dev, para_val, BYTE_OFFSET_4);
    /* 下发参数 */
    wal_ioctl_set_fast_sleep_para(net_dev, para_val, BYTE_OFFSET_4);
    /* 下发ps mode */
    wal_ioctl_set_sta_ps_mode(net_dev, AUTO_FAST_PS);
    wal_set_fastsleep2_mode(net_dev, OAL_TRUE);
    return OAL_SUCC;
}

OAL_STATIC uint32_t wal_ioctl_set_fastsleep3_param(oal_net_device_stru *net_dev, int8_t *para_str)
{
    mac_vap_stru *mac_vap = oal_net_dev_priv(net_dev);
    uint8_t para_val[BYTE_OFFSET_5] = { 0 };
    uint8_t str_tmp[WAL_HIPRIV_CMD_NAME_MAX_LEN] = {0};
    uint32_t cmd_offset;
    uint32_t para_cnt;
    uint32_t ret;
    ...
    if (wal_fastsleep3_check_param_can_update(mac_vap) == OAL_FALSE) {
        return OAL_FAIL;
    }

    para_str += 3; // 偏移3个字节
    for (para_cnt = 0; para_cnt < BYTE_OFFSET_5; para_cnt++) {
        ret = wal_get_cmd_one_arg(para_str, str_tmp, WAL_HIPRIV_CMD_NAME_MAX_LEN, &cmd_offset);
        if (ret != OAL_SUCC) {
            return OAL_FAIL;
        }
    ...
    para_val[BYTE_OFFSET_0] /= WLAN_SLEEP_TIMER_PERIOD;
    para_val[BYTE_OFFSET_1] /= WLAN_SLEEP_TIMER_PERIOD;
    para_val[BYTE_OFFSET_2] /= WLAN_SLEEP_TIMER_PERIOD;
    /* 修改host睡眠定时超时参数 */
    g_wlan_min_fast_ps_idle = para_val[BYTE_OFFSET_0];
    wal_ioctl_set_fastsleep3_para(net_dev, para_val, BYTE_OFFSET_5);
    wal_set_fastsleep3_para(net_dev, para_val, sizeof(para_val));
    oam_warning_log3(0, OAM_SF_CFG, "wal_ioctl_set_fastsleep_switch:fastsleep3 t0[%d] t1[%d] t2[%d]!",
        para_val[BYTE_OFFSET_0], para_val[BYTE_OFFSET_1], para_val[BYTE_OFFSET_2]);
    oam_warning_log2(0, OAM_SF_CFG, "wal_ioctl_set_fastsleep_switch:fastsleep3 total_rx[%d] t2_rx[%d]!",
        para_val[BYTE_OFFSET_3], para_val[BYTE_OFFSET_4]);
    /* 下发ps mode */
    wal_ioctl_set_sta_ps_mode(net_dev, AUTO_FAST_PS);
    wal_set_fastsleep3_mode(net_dev, OAL_TRUE);
    return OAL_SUCC;
}
#define NORMAL_RX_LISTEN_TIMER_CNT 10
/*
...
OAL_STATIC int32_t wal_ioctl_set_fastsleep_switch(oal_net_device_stru *pst_net_dev, int8_t *pc_command)
{
    ...
    if (!oal_strncasecmp(puc_para_str, "0", 1)) {
        /* 关闭fastsleep */
        auc_para_val[BYTE_OFFSET_0] = NORMAL_RX_LISTEN_TIMER_CNT;
        auc_para_val[BYTE_OFFSET_1] = NORMAL_RX_LISTEN_TIMER_CNT;
        auc_para_val[BYTE_OFFSET_2] = 0;
        auc_para_val[BYTE_OFFSET_3] = 0;
        wal_ioctl_set_fast_sleep_para(pst_net_dev, auc_para_val, BYTE_OFFSET_4);
        wal_ioctl_set_sta_ps_mode(pst_net_dev, MIN_FAST_PS);
        wal_clear_fastsleep_param(pst_net_dev);
        oam_warning_log0(0, OAM_SF_CFG, "wal_ioctl_set_fastsleep_switch:disable fast sleep!");
    } else if (!oal_strncasecmp(puc_para_str, "1", 1)) {
        /* 下发使能参数前，先清掉所有fastsleep模式的使能标记以及流量监测的计数 */
        wal_clear_fastsleep_param(pst_net_dev);
        return wal_ioctl_set_fastsleep_param(pst_net_dev, puc_para_str);
    } else if (!oal_strncasecmp(puc_para_str, "FS3", 3)) { // 长度为3
        /* 下发使能参数前，先清掉所有fastsleep模式的使能标记以及流量监测的计数 */
        wal_clear_fastsleep_param(pst_net_dev);
        return wal_ioctl_set_fastsleep3_param(pst_net_dev, puc_para_str);
    } else {
...
/*
 * 函 数 名  : wal_ioctl_get_wifi7_priv_feature_cap
 * 功能描述  : 获取wifi7 特性能力位
 * 1.日    期  : 2024年9月13日
 *   作    者  : wifi
 *   修改内容  : 新生成函数
 */
OAL_STATIC uint32_t wal_ioctl_get_wifi7_priv_feature_cap(hmac_vap_stru *pst_hmac_vap)
{
    uint32_t value = 0;

#ifdef _PRE_WLAN_FEATURE_ML
    if (hmac_vap_is_legacy_mld_mode(pst_hmac_vap) == OAL_FALSE) {
        value |= BIT(WAL_WIFI_CONNECT_MLO);
    }
#endif
#ifdef _PRE_WLAN_FEATURE_EMLSR
    if (g_pst_mac_device_capability[0].switch_emlsr == OAL_TRUE) {
        value |= BIT(WAL_WIFI_FEATURE_SUPPORT_EMLSR);
    }
#endif
#ifdef _PRE_WLAN_FEATURE_WUR
    if (wlan_chip_get_wur_switch() == OAL_TRUE) {
        value |= BIT(WAL_WIFI_FEATURE_SUPPORT_WUR);
    }
#endif
    return value;
}
...
#endif
    value |= wal_ioctl_get_wifi7_priv_feature_cap(pst_hmac_vap);
...
OAL_STATIC int32_t wal_ioctl_priv_cmd_set_memo_change(int8_t *pc_command, wal_wifi_priv_cmd_stru st_priv_cmd)
{
    ...
    if (oal_any_true_value2(g_tas_switch_en[WLAN_RF_CHANNEL_ZERO], g_tas_switch_en[WLAN_RF_CHANNEL_ONE])) {
        ...
        l_ret = hal_wifi_tas_set(l_param_1);
    }
    oal_free(pc_command);
    return l_ret;
}
...
OAL_STATIC int32_t wal_ioctl_priv_cmd_set_tas_ant_switch(oal_net_device_stru *pst_net_dev,
    int8_t *pc_command, wal_wifi_priv_cmd_stru st_priv_cmd)
{
    int32_t l_ret;
#ifdef _PRE_WLAN_FEATURE_TAS_ANT_SWITCH
    ...
    l_ret = wal_ioctl_judge_input_param_length(st_priv_cmd, CMD_SET_TAS_ANT_SWITCH_LEN, 1);
    if (l_ret != OAL_SUCC) {
        oam_error_log0(0, OAM_SF_ANY, "{wal_vendor_priv_cmd::CMD_SET_TAS_ANT_SWITCH cmd len error}");
        return -OAL_EFAIL;
    }
    tas_switch = !!oal_atoi(pc_command + CMD_SET_TAS_ANT_SWITCH_LEN + 1);
    oam_warning_log1(0, OAM_SF_ANY, "{wal_vendor_priv_cmd::CMD_SET_TAS_ANT_SWITCH tas_switch[%d].}", tas_switch);
    /* 打开/关闭TAS天线 */
    if (g_wlan_chip_ops->set_tas_ant_switch != NULL) {
        return g_wlan_chip_ops->set_tas_ant_switch(pst_net_dev, tas_switch);
    }
    return OAL_FAIL;
}
...
OAL_STATIC void wal_get_gust_info(cust_info_rpt *cust_info)
{
    /* 赋值ht40禁止位 */
    cust_info->disable_capab_2ght40 = (uint32_t)g_wlan_cust.uc_disable_capab_2ght40;

    /* 赋值11be swtich mask */
    cust_info->wifi_11be_switch_mask = g_cust_cap.wifi_11be_switch_mask;

    cust_info->optimized_feature_switch_bitmap = g_optimized_feature_switch_bitmap;
}
    ...
    int8_t *pc_command, wal_wifi_priv_cmd_stru *priv_cmd)
{
    ...
    wal_get_gust_info(&cust_info);

    if (priv_cmd->total_len < sizeof(cust_info_rpt)) {
        oam_error_log1(0, OAM_SF_ANY, "wal_ioctl_priv_cmd_wpas_get_gust:total_len=%d", priv_cmd->total_len);
        return -OAL_EFAIL;
    }

    if (oal_copy_to_user(priv_cmd->puc_buf, (void *)&cust_info, sizeof(cust_info_rpt)) != EOK) {
    ...
    priv_cmd->used_len = sizeof(cust_info_rpt);
    return OAL_SUCC;
}
#endif
    ...
    int8_t *pc_command, wal_wifi_priv_cmd_stru *priv_cmd, oal_ifreq_stru *pst_ifr)
{
    ...
    if (oal_unlikely(l_ret != OAL_SUCC)) {
        oam_warning_log2(0, OAM_SF_ANY, "{wal_vendor_priv_cmd::CMD_VOWIFI_GET_MODE(%d) err [%d]!}", l_value, l_ret);
        return -OAL_EFAIL;
    }
    ...
    *pl_value = l_value;
    ...
    if (oal_copy_to_user(priv_cmd->puc_buf, pc_command, priv_cmd->total_len) != EOK) {
        oam_error_log0(0, OAM_SF_ANY, "wal_vendor_priv_cmd:CMD_VOWIFi_GET_MODE copy ioctl_data to user fail!");
        return -OAL_EFAIL;
    }
    priv_cmd->used_len = sizeof(int32_t);
    return l_ret;
}
    ...
    wal_wifi_priv_cmd_stru *priv_cmd, oal_ifreq_stru *pst_ifr, int32_t *l_memcpy_ret)
{
    ...
    *l_memcpy_ret += memcpy_s(pc_command,
    ...
    if (oal_copy_to_user(priv_cmd->puc_buf, pc_command, ret_len) != EOK) {
        oam_error_log0(0, OAM_SF_ANY, "wal_vendor_priv_cmd:CMD_VOWIFI_IS_SUPPORT Failed to copy to user !");
        return -OAL_EFAIL;
    }
    priv_cmd->used_len = CMD_VOWIFI_IS_SUPPORT_REPLY_LEN;
    return OAL_SUCC;
}
...
static int32_t wal_ioctl_set_atf_user_val(oal_net_device_stru *net_dev, atf_cfg_user_param_stru *atf_params)
{
    int32_t ret;
    wal_msg_write_stru wal_write_msg = {0};

    wal_write_msg_hdr_init(&wal_write_msg, WLAN_CFGID_ALG_PARAM, sizeof(atf_cfg_user_param_stru));
    ret = memcpy_s(wal_write_msg.auc_value, sizeof(atf_cfg_user_param_stru),
        atf_params, sizeof(atf_cfg_user_param_stru));
    if (oal_unlikely(ret != EOK)) {
        oam_error_log0(0, OAM_SF_ANY, "{wal_ioctl_set_atf_user_val::memcpy_s failed!}");
        return OAL_FAIL;
    }

    ret = wal_send_cfg_event(net_dev, WAL_MSG_TYPE_WRITE,
        WAL_MSG_WRITE_MSG_HDR_LENGTH + sizeof(atf_cfg_user_param_stru),
        (uint8_t *)&wal_write_msg, OAL_FALSE, NULL);
    if (ret != OAL_SUCC) {
        oam_error_log1(0, OAM_SF_ANY, "{wal_ioctl_set_atf_user_val::return err code[%d]!}", ret);
        return OAL_FAIL;
    }
...
{
    int32_t ret;
    atf_cfg_user_param_stru atf_params = {0};

    ret = wal_ioctl_judge_input_param_length(*priv_cmd, CMD_ATF_SET_USER_PARAM_LEN, sizeof(atf_cfg_user_val_stru));
    if (ret != OAL_SUCC) {
        oam_error_log1(0, OAM_SF_ANY, "{wal_vendor_set_atf_user_val::length is too short! at least need [%d]!}",
            CMD_ATF_SET_USER_PARAM_LEN + sizeof(atf_cfg_user_val_stru) + 1);
        return OAL_FAIL;
    }

    ret = memcpy_s(&atf_params.user_val, sizeof(atf_cfg_user_val_stru), command + CMD_ATF_SET_USER_PARAM_LEN + 1,
        sizeof(atf_cfg_user_val_stru));
    if (oal_unlikely(ret != EOK)) {
        oam_error_log0(0, OAM_SF_ANY, "{wal_vendor_set_atf_user_val::memcpy_s failed!}");
        return OAL_FAIL;
    }

    atf_params.mac_alg_param.en_alg_cfg = MAC_ALG_CFG_ATF_USER_VAL;

    return wal_ioctl_set_atf_user_val(net_dev, &atf_params);
}
#endif

static int32_t wal_ioctl_set_edca_param(oal_net_device_stru *net_dev, mac_phy_debug_switch_stru *phy_debug_switch)
{
    int32_t ret;
    wal_msg_write_stru wal_write_msg = {0};

    wal_write_msg_hdr_init(&wal_write_msg, WLAN_CFGID_PHY_DEBUG_SWITCH, sizeof(mac_phy_debug_switch_stru));
    ret = memcpy_s(wal_write_msg.auc_value, sizeof(wal_write_msg.auc_value),
        phy_debug_switch, sizeof(mac_phy_debug_switch_stru));
    if (oal_unlikely(ret != EOK)) {
        oam_error_log0(0, OAM_SF_ANY, "{wal_ioctl_set_edca_param::memcpy_s failed!}");
        return OAL_FAIL;
    }

    ret = wal_send_cfg_event(net_dev, WAL_MSG_TYPE_WRITE,
        WAL_MSG_WRITE_MSG_HDR_LENGTH + sizeof(mac_phy_debug_switch_stru),
        (uint8_t *)&wal_write_msg, OAL_FALSE, NULL);
    if (ret != OAL_SUCC) {
        oam_error_log1(0, OAM_SF_ANY, "{wal_ioctl_set_edca_param::return err code[%d]!}", ret);
        return OAL_FAIL;
    }
...
{
    int32_t ret;
    mac_cfg_edca_stru edca_params = {0};
    mac_phy_debug_switch_stru phy_debug_switch = {0};

    ret = wal_ioctl_judge_input_param_length(*priv_cmd, CMD_SET_EDCA_PARAM_LEN, sizeof(mac_cfg_edca_stru));
    if (ret != OAL_SUCC) {
        oam_error_log1(0, OAM_SF_ANY, "{wal_vendor_set_edca_param::length is too short! at least need [%d]!}",
            CMD_SET_EDCA_PARAM_LEN + sizeof(mac_cfg_edca_stru) + 1);
        return OAL_FAIL;
    }

    ret = memcpy_s(&edca_params, sizeof(mac_cfg_edca_stru), command + CMD_SET_EDCA_PARAM_LEN + 1,
        sizeof(mac_cfg_edca_stru));
    if (oal_unlikely(ret != EOK)) {
        oam_error_log0(0, OAM_SF_ANY, "{wal_vendor_set_edca_param::memcpy_s failed!}");
        return OAL_FAIL;
    }
    wal_hipriv_phy_debug_init(&phy_debug_switch);
    phy_debug_switch.uc_edca_param_switch |= edca_params.edca_ac << BIT_OFFSET_4;
    if (edca_params.flag_aifsn != 0) {
        phy_debug_switch.uc_edca_param_switch |= (uint8_t)BIT3;
        phy_debug_switch.uc_edca_aifsn = edca_params.edca_aifsn;
    }
    if (edca_params.flag_cwmin != 0) {
        phy_debug_switch.uc_edca_param_switch |= (uint8_t)BIT2;
        phy_debug_switch.uc_edca_cwmin = edca_params.edca_cwmin;
    }
    if (edca_params.flag_cwmax != 0) {
        phy_debug_switch.uc_edca_param_switch |= (uint8_t)BIT1;
        phy_debug_switch.uc_edca_cwmax = edca_params.edca_cwmax;
    }
    if (edca_params.flag_txoplimit != 0) {
        phy_debug_switch.uc_edca_param_switch |= (uint8_t)BIT0;
        phy_debug_switch.us_edca_txoplimit = edca_params.edca_txoplimit;
    }

    return wal_ioctl_set_edca_param(net_dev, &phy_debug_switch);
}
    ...
    wal_wifi_priv_cmd_stru *st_priv_cmd, oal_ifreq_stru *pst_ifr, int32_t *l_memcpy_ret)
{
...
#ifndef CONFIG_HAS_EARLYSUSPEND
    } else if (oal_strncasecmp(pc_command, CMD_SETSUSPENDMODE, CMD_SETSUSPENDMODE_LEN) == 0) {
        l_ret = wal_ioctl_priv_cmd_set_suspend_mode(pst_net_dev, pc_command, *st_priv_cmd);
#endif
    } else if (oal_strncasecmp(pc_command, CMD_SET_STA_PM_ON, CMD_SET_STA_PM_ON_LEN) == 0) {
        l_ret = wal_ioctl_set_sta_pm_on(pst_net_dev, pc_command, BUFF_SIZE);
    } else if (wal_vendor_cmd_gather(pc_command) == OAL_TRUE) {
        l_ret = wal_vendor_cmd_gather_handler(pst_net_dev, pc_command);
    } else if (oal_strncasecmp(pc_command, CMD_RXFILTER_START, CMD_RXFILTER_START_LEN) == 0) {
        wal_ioctl_force_stop_filter(pst_net_dev, OAL_FALSE);
    } else if (oal_strncasecmp(pc_command, CMD_RXFILTER_STOP, CMD_RXFILTER_STOP_LEN) == 0) {
    ...
    } else if (oal_strncasecmp(pc_command, CMD_SET_FASTSLEEP_SWITCH, CMD_SET_FASTSLEEP_SWITCH_LEN) == 0) {
        l_ret = wal_ioctl_set_fastsleep_switch(pst_net_dev, pc_command);
    } else if (oal_strncasecmp(pc_command, CMD_SET_TCP_ACK_CTL, CMD_SET_TCP_ACK_CTL_LEN) == 0) {
    ...
    wal_wifi_priv_cmd_stru *st_priv_cmd, oal_ifreq_stru *pst_ifr, int32_t *l_memcpy_ret)
{
    ...
    if (oal_strncasecmp(pc_command, CMD_GET_CAPA_DBDC, CMD_GET_CAPA_DBDC_LEN) == 0) {
    ...
    } else if (oal_strncasecmp(pc_command, CMD_SET_TAS_ANT_SWITCH, CMD_SET_TAS_ANT_SWITCH_LEN) == 0) {
        l_ret = wal_ioctl_priv_cmd_set_tas_ant_switch(pst_net_dev, pc_command, *st_priv_cmd);
    ...
    wal_wifi_priv_cmd_stru *priv_cmd, oal_ifreq_stru *ifr, int32_t *memcpy_ret)
{
    ...
    } else if (oal_strncasecmp(command, CMD_MIRACAST_START, CMD_MIRACAST_START_LEN) == 0) {
        oam_warning_log0(0, OAM_SF_M2S, "{wal_vendor_priv_cmd::Miracast start.}");
        ret = wal_ioctl_set_p2p_miracast_status(net_dev, OAL_TRUE);
    } else if (oal_strncasecmp(command, CMD_MIRACAST_STOP, CMD_MIRACAST_STOP_LEN) == 0) {
    ...
    } else if (oal_strncasecmp(command, CMD_SET_POWER_ON, CMD_SET_POWER_ON_LEN) == 0) {
        ret = wal_ioctl_priv_cmd_set_power_on(net_dev, command);
    } else if (oal_strncasecmp(command, CMD_SET_POWER_MGMT_ON, CMD_SET_POWER_MGMT_ON_LEN) == 0) {
        ret = wal_ioctl_priv_cmd_set_power_mgmt_on(command);
    } else if (oal_strncasecmp(command, CMD_COUNTRY, CMD_COUNTRY_LEN) == 0) {
...
#if defined(CONFIG_COMPAT) && defined(_PRE_HISI_HERTZ)
static int32_t wal_vendor_copy_from_user_compat(oal_ifreq_stru *pst_ifr, wal_wifi_priv_cmd_stru *priv_cmd)
{
    wal_wifi_priv_cmd_stru_compat priv_cmd_compat;
    if ((pst_ifr->ifr_data == NULL) ||
        oal_copy_from_user((uint8_t *)&priv_cmd_compat, pst_ifr->ifr_data, sizeof(wal_wifi_priv_cmd_stru_compat))) {
        return -OAL_EINVAL;
    }
    priv_cmd->total_len = priv_cmd_compat.total_len;
    priv_cmd->used_len = priv_cmd_compat.used_len;
    priv_cmd->puc_buf = compat_ptr(priv_cmd_compat.puc_buf);
    return EOK;
}
#endif
/*
...
int32_t wal_vendor_priv_cmd(oal_net_device_stru *pst_net_dev, oal_ifreq_stru *pst_ifr, int32_t cmd)
{
...
#if defined(CONFIG_COMPAT) && defined(_PRE_HISI_HERTZ)
    l_ret = wal_vendor_copy_from_user_compat(pst_ifr, &priv_cmd);
    if (l_ret != EOK) {
        return -OAL_EINVAL;
    }
#else
    if ((pst_ifr->ifr_data == NULL) ||
        oal_copy_from_user((uint8_t *)&priv_cmd, pst_ifr->ifr_data, sizeof(wal_wifi_priv_cmd_stru)) != 0) {
        return -OAL_EINVAL;
    }
#endif
    ...
    if (oal_strncasecmp(command, CMD_SET_MEMO_CHANGE, CMD_SET_MEMO_CHANGE_LEN) == 0) {
        ...
        oal_free(command);
        if (g_wlan_chip_ops->get_tas_ant_status != NULL) {
            return g_wlan_chip_ops->get_tas_ant_status(pst_net_dev);
        }
        return hal_get_wifi_tas_gpio_state();
    }
#endif
    ...
    if (oal_copy_to_user(pst_ifr->ifr_data, &priv_cmd, sizeof(wal_wifi_priv_cmd_stru))) {
        return -OAL_EFAIL;
    }
    return l_ret;
}
    ...
    oal_ifreq_stru *pst_ifr, wal_wifi_priv_cmd_stru *p_priv_cmd, uint8_t *pc_cmd)
{
    ...
    if (oal_strncasecmp(pc_cmd, CMD_SET_VHT160_FEM_LOWER, CMD_SET_VHT160_FEM_LOWER_LEN) == 0) {
    ...
    } else if (oal_strncasecmp(pc_cmd, CMD_GET_VHT160_SUPPORTED, CMD_GET_VHT160_SUPPORTED_LEN) == 0) {
        return wal_vendor_get_160m_supported(pst_net_dev, p_priv_cmd);
    } else if (oal_strncasecmp(pc_cmd, CMD_GET_AP_BANDWIDTH, CMD_GET_AP_BANDWIDTH_LEN) == 0) {
        return wal_vendor_get_ap_bandwidth(pst_net_dev, p_priv_cmd);
    } else if (oal_strncasecmp(pc_cmd, CMD_SET_DC_STATE, CMD_SET_DC_STATE_LEN) == 0) {
    ...
    } else if (oal_strncasecmp(pc_cmd, CMD_SET_CALI_FEM_MODE, OAL_STRLEN(CMD_SET_CALI_FEM_MODE)) == 0) {
        return wal_vendor_set_cali_fem_mode(pst_net_dev, pst_ifr, pc_cmd);
#ifdef _PRE_WLAN_FEATURE_NRCOEX
    } else if (oal_strncasecmp(pc_cmd, CMD_SET_NRCOEX_PRIOR, OAL_STRLEN(CMD_SET_NRCOEX_PRIOR)) == 0) {
        return wal_vendor_set_nrcoex_priority(pst_net_dev, pst_ifr, pc_cmd);
    } else if (oal_strncasecmp(pc_cmd, CMD_GET_NRCOEX_INFO, OAL_STRLEN(CMD_GET_NRCOEX_INFO)) == 0) {
    ...
    } else if (oal_strncasecmp(pc_cmd, CMD_GET_ABNORMAL_PKTS_CNT, CMD_GET_ABNORMAL_PKTS_CNT_LEN) == 0) {
        return wal_ioctl_get_psm_stat(pst_net_dev, MAC_PSM_QUERY_ABNORMAL_PKTS_CNT, p_priv_cmd);
#endif
#if (defined(_PRE_CONFIG_HILINK_AMAX) && defined(_PRE_WLAN_TCP_OPT))
    } else if (oal_strncasecmp(pc_cmd, CMD_MULTILINK, CMD_MULTILINK_LEN) == 0) {
...
#endif
#if defined(_PRE_PRODUCT_HI3751V811) || defined(CONFIG_OPEN_HARMONY_TV)
static int32_t wal_vendor_cmd_set_m2s_switch(oal_net_device_stru *pst_net_dev, uint8_t uc_m2s_switch_on)
{
    ...
    if (l_chip_type != BOARD_VERSION_MP13 && l_chip_type != BOARD_VERSION_MP15 &&
        l_chip_type != BOARD_VERSION_MP17C) {
        return 0;
    }
    /* 03 05的切天线命令与07C的参数写法不一样,需要区分 */
    if (l_chip_type == BOARD_VERSION_MP17C) {
        if (uc_m2s_switch_on == MAC_M2S_COMMAND_MODE_SET_SISO_C0) {
            return (int32_t)wal_hipriv_set_m2s_switch(pst_net_dev, "3 0001 0");
        } else if (uc_m2s_switch_on == MAC_M2S_COMMAND_MODE_SET_SISO_C1) {
            return (int32_t)wal_hipriv_set_m2s_switch(pst_net_dev, "3 0010 0");
        } else if (uc_m2s_switch_on == MAC_M2S_COMMAND_MODE_SET_MIMO) {
            return (int32_t)wal_hipriv_set_m2s_switch(pst_net_dev, "3 0011 0");
        } else {
            return 0;
        }
    } else {
        if (uc_m2s_switch_on == MAC_M2S_COMMAND_MODE_SET_SISO_C0) {
            ...
            return (int32_t)wal_hipriv_set_m2s_switch(pst_net_dev, "3 2 0");
        } else {
            return 0;
        }
    }
...
int32_t wal_vendor_cmd_gather_handler(oal_net_device_stru *pst_net_dev, int8_t *pc_command)
{
    ...
    if (oal_strncasecmp(pc_command, CMD_SET_SOFTAP_MIMOMODE, OAL_STRLEN(CMD_SET_SOFTAP_MIMOMODE)) == 0) {
...
#ifdef _PRE_WLAN_FEATURE_M2S
    } else if (oal_strncasecmp(pc_command, CMD_SET_M2S_SWITCH, OAL_STRLEN(CMD_SET_M2S_SWITCH)) == 0) {
...
#endif
#ifdef _PRE_WLAN_FEATURE_LP_MIRACAST
    } else if (oal_strncasecmp(pc_command, CMD_SET_LP_ANT_SWITCH, OAL_STRLEN(CMD_SET_LP_ANT_SWITCH)) == 0) {
...
static uint8_t wal_vendor_cmd_gather(int8_t *pc_command)
{
    ...
    if (g_wlan_spec_cfg->feature_m2s_modem_is_open) {
        en_cmd_is_invalid =
            ((oal_strncasecmp(pc_command, CMD_SET_SOFTAP_MIMOMODE, OAL_STRLEN(CMD_SET_SOFTAP_MIMOMODE)) == 0)
#ifdef _PRE_WLAN_FEATURE_LP_MIRACAST
            || (oal_strncasecmp(pc_command, CMD_SET_LP_ANT_SWITCH, OAL_STRLEN(CMD_SET_LP_ANT_SWITCH)) == 0) ||
            (oal_strncasecmp(pc_command, CMD_GET_LP_ANT_SWITCH, OAL_STRLEN(CMD_GET_LP_ANT_SWITCH)) == 0)
#endif
#ifdef _PRE_WLAN_FEATURE_M2S
            || (oal_strncasecmp(pc_command, CMD_SET_M2S_SWITCH, OAL_STRLEN(CMD_SET_M2S_SWITCH)) == 0)
#ifdef _PRE_WLAN_FEATURE_M2S_MODEM
            || (oal_strncasecmp(pc_command, CMD_SET_M2S_MODEM, OAL_STRLEN(CMD_SET_M2S_MODEM)) == 0)
#endif
#endif
...
static int32_t wal_vsp_priv_cmd_proc(hmac_vap_stru *hmac_vap, hmac_user_stru *hmac_user, hmac_vsp_cmd *vsp_cmd)
{
    ...
    switch (vsp_cmd->cmd) {
        ...
        case VSP_CMD_STOP:
            ...
            break;
        case VSP_CMD_SET_TIME:
            ret = hmac_vsp_set_times(hmac_vap, hmac_user, vsp_cmd);
            break;
        default:
            break;
    }
    ...
    uint32_t cmd_name_len)
{
    ...
    if (oal_any_null_ptr3(net_dev, priv_cmd, command)) {
        oam_warning_log0(0, OAM_SF_ANY, "{wal_vsp_set_cfg_param::parameter is NULL!}");
        return OAL_ERR_CODE_PTR_NULL;
    }

    if (wal_ioctl_judge_input_param_length(*priv_cmd, cmd_name_len, sizeof(hmac_vsp_cmd)) != OAL_SUCC) {
        oam_warning_log0(0, OAM_SF_ANY, "{wal_vsp_set_cfg_param::length is too short!}");
        return OAL_FAIL;
    }
    ret = memcpy_s(&vsp_cmd, sizeof(hmac_vsp_cmd), command + cmd_name_len + 1, sizeof(hmac_vsp_cmd));
    if (oal_unlikely(ret != EOK)) {
        oam_error_log0(0, OAM_SF_ANY, "{wal_vsp_set_cfg_param::memcpy_s failed!}");
        return OAL_FAIL;
    }
...
int32_t wal_vsp_set_nav_edca(oal_net_device_stru *net_dev, wal_wifi_priv_cmd_stru *priv_cmd, uint8_t *command,
    uint32_t cmd_name_len)
{
    hmac_vsp_nav_edca_cmd vsp_cmd = { 0 };
    mac_vap_stru *mac_vap = NULL;
    uint32_t ret;

    if (oal_any_null_ptr3(net_dev, priv_cmd, command)) {
        oam_warning_log0(0, OAM_SF_ANY, "{wal_vsp_set_nav_edca::parameter is NULL!}");
        return OAL_ERR_CODE_PTR_NULL;
    }

    if (wal_ioctl_judge_input_param_length(*priv_cmd, cmd_name_len, sizeof(hmac_vsp_nav_edca_cmd)) != OAL_SUCC) {
        oam_warning_log0(0, OAM_SF_ANY, "{wal_vsp_set_nav_edca::length is too short!}");
        return OAL_FAIL;
    }
    ret = memcpy_s(&vsp_cmd, sizeof(hmac_vsp_nav_edca_cmd), command + cmd_name_len + 1, sizeof(hmac_vsp_nav_edca_cmd));
    if (oal_unlikely(ret != EOK)) {
        oam_error_log0(0, OAM_SF_ANY, "{wal_vsp_set_nav_edca::memcpy_s failed!}");
        return OAL_FAIL;
    }
    ...
    return (int32_t)hmac_vsp_set_nav_edca_param(mac_vap, &vsp_cmd);
}
#endif
...
{
    station_info_ext_cdf_th cdf_th_info = { 0 };
    mac_vap_stru *mac_vap = NULL;
    uint32_t ret;

    if (oal_any_null_ptr3(net_dev, priv_cmd, command)) {
        oam_warning_log0(0, OAM_SF_ANY, "{wal_vendor_set_station_info_ext_cdf_th::parameter is NULL!}");
        return OAL_ERR_CODE_PTR_NULL;
    }

    if (wal_ioctl_judge_input_param_length(*priv_cmd, cmd_name_len, sizeof(station_info_ext_cdf_th)) != OAL_SUCC) {
        oam_warning_log0(0, OAM_SF_ANY, "{wal_vendor_set_station_info_ext_cdf_th::length is too short!}");
        return OAL_FAIL;
    }
    ret = memcpy_s(&cdf_th_info, sizeof(station_info_ext_cdf_th),
        command + cmd_name_len + 1, sizeof(station_info_ext_cdf_th));
    if (oal_unlikely(ret != EOK)) {
        oam_error_log0(0, OAM_SF_ANY, "{wal_vendor_set_station_info_ext_cdf_th::memcpy_s failed!}");
        return OAL_FAIL;
    }
    mac_vap = (mac_vap_stru *)oal_net_dev_priv(net_dev);
    if (!mac_vap) {
    ...
    return (int32_t)hmac_config_set_station_ext_info_cdf_th(mac_vap,
        sizeof(station_info_ext_cdf_th), (uint8_t*)&cdf_th_info);
}
#endif
...
{
    uint32_t ret;
    mac_ioctl_alg_ai_csma_edca_stru edca_param = {0};
    mac_ioctl_alg_param_stru alg_param = {0};
    mac_vap_stru *mac_vap = NULL;
    hmac_vap_stru *hmac_vap = NULL;

    if (wal_ioctl_judge_input_param_length(*priv_cmd, cmd_name_len, sizeof(mac_ioctl_alg_ai_csma_edca_stru)) !=
        OAL_SUCC) {
        oam_warning_log0(0, OAM_SF_ANY, "{wal_vendor_set_ai_csma_edca_param::length is too short!}");
        return WAL_AI_CSMA_EDCA_HOST_ERR;
    }
    ret = memcpy_s(&edca_param, sizeof(mac_ioctl_alg_ai_csma_edca_stru),
        cmd + cmd_name_len + 1, sizeof(mac_ioctl_alg_ai_csma_edca_stru));
    if (oal_unlikely(ret != EOK)) {
        oam_error_log0(0, OAM_SF_ANY, "{wal_vendor_set_ai_csma_edca_param::memcpy_s failed!}");
        return WAL_AI_CSMA_EDCA_HOST_ERR;
    }
    ...
    alg_param.en_alg_cfg = MAC_ALG_CFG_AI_CSMA_EDCA_PARAM_CFG;
    alg_param.value = *(uint32_t*)(&edca_param);
    return hmac_config_alg_param(mac_vap, sizeof(mac_ioctl_alg_param_stru), (uint8_t *)&alg_param);
}
...
{
    g_ap_cac = OAL_FALSE;
    oam_warning_log0(0, OAM_SF_M2S, "{wal_hipriv_skip_cur_cac:skip cac}");
    return OAL_SUCC;
}
#endif
