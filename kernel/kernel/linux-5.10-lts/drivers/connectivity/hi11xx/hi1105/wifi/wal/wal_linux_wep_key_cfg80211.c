...
void wal_set_crypto_info(hmac_conn_param_stru *pst_conn_param, oal_cfg80211_conn_stru *pst_sme)
{
    ...
    ies_info.frame_ies = (uint8_t *)pst_sme->ie;
    ies_info.frame_len = pst_sme->ie_len;
    ...
    if (pst_sme->privacy) {
        if ((pst_sme->key_len != 0) && (pst_sme->key != NULL)) {
            ...
            if (memcpy_s(puc_wep_key, (uint32_t)pst_sme->key_len,
                (uint8_t *)pst_sme->key, (uint32_t)(pst_sme->key_len)) != EOK) {
        ...
        if (puc_ie_tmp != NULL) {
            ...
            if (puc_ie_tmp != NULL) {
                ...
                oam_warning_log2(0, OAM_SF_WPA, "wal_set_crypto_info:wpa_cipher pair_suite[0]:0x%x pair_suite[1]:0x%x",
                    pst_conn_param->st_crypto.aul_pair_suite[0], pst_conn_param->st_crypto.aul_pair_suite[1]);
            }
        }
    }
...
OAL_STATIC int32_t wal_get_key_value(mac_addkey_param_stru *payload_para, oal_key_params_stru *params)
{
    int32_t ret = EOK;
    payload_para->st_key.key_len = params->key_len;
    payload_para->st_key.seq_len = params->seq_len;
    payload_para->st_key.cipher = params->cipher;

    ret += memcpy_s(payload_para->st_key.auc_key, OAL_WPA_KEY_LEN, params->key, (uint32_t)params->key_len);
    oam_warning_log4(0, OAM_SF_ANY, "{wal_cfg80211_add_key::key id:%d len:%d cipher 0x%x, ret=0X%X!}",
        payload_para->uc_key_index, params->key_len, params->cipher, ret);

    if (params->seq_len != 0) {
        ret += memcpy_s(payload_para->st_key.auc_seq, OAL_WPA_SEQ_LEN, params->seq, (uint32_t)params->seq_len);
...
int32_t wal_cfg80211_add_key(oal_wiphy_stru *pst_wiphy,
    ...
    oal_net_device_stru *pst_netdev, int32_t link_id, uint8_t uc_key_index, bool en_pairwise,
#else
    ...
    const uint8_t *puc_mac_addr, oal_key_params_stru *pst_params)
{
...
#ifdef CONFIG_HISI_WIFI7_MLO
    payload_para.link_id = link_id;
#endif

    /* 2.2 获取相关密钥值 */
    ret += wal_get_key_value(&payload_para, pst_params);
    ...
    ***************************************************************************/
    ...
    if (wal_send_cfg_event(pst_netdev, WAL_MSG_TYPE_WRITE, WAL_MSG_WRITE_MSG_HDR_LENGTH +
        sizeof(mac_addkey_param_stru), (uint8_t *)&write_msg, OAL_TRUE, &p_rsp_msg) != OAL_SUCC) {
        oam_error_log0(0, OAM_SF_ANY, "{wal_cfg80211_add_key::wal_send_cfg_event fail!}");
        return -OAL_EFAIL;
    }
    if (wal_check_and_release_msg_resp(p_rsp_msg) != OAL_SUCC) {
...
#ifdef CONFIG_HISI_WIFI7_MLO
    int32_t link_id, uint8_t uc_key_index, bool en_pairwise, const uint8_t *puc_mac_addr,
#else
...
#ifdef CONFIG_HISI_WIFI7_MLO
    st_payload_params.link_id = link_id;
#endif
    /***************************************************************************
        抛事件到wal层处理
    ***************************************************************************/
    ...
    /* 由于消息中使用了局部变量指针，因此需要将发送该函数设置为同步，否则hmac处理时会使用野指针 */
    if (wal_send_cfg_event(pst_netdev, WAL_MSG_TYPE_WRITE,
...
#ifdef CONFIG_HISI_WIFI7_MLO
    int32_t link_id, uint8_t uc_key_index, bool en_unicast, bool en_multicast)
#else
...
#endif
{
    ...
    uint32_t ret;
#ifdef CONFIG_HISI_WIFI7_MLO
    oal_net_device_stru *tmp_netdev = NULL;
#endif
...
#ifdef CONFIG_HISI_WIFI7_MLO
    tmp_netdev = wal_mld_get_netdev_by_ap_link_id(pst_netdev, link_id);
    if (tmp_netdev == NULL) {
        oam_error_log0(0, OAM_SF_ANY, "wal_cfg80211_set_default_key:find NETDEV FAIL");
        return -OAL_EFAIL;
    }
    if (wal_process_cmd_params(tmp_netdev, cmd_id, params) != OAL_SUCC) {
#else
...
#ifdef CONFIG_HISI_WIFI7_MLO
    int32_t link_id, uint8_t uc_key_index)
#else
    ...
    oal_net_device_stru *pst_net_device, oal_cfg80211_pmksa_stru *pmksa)
{
    ...
    if (g_st_dfr_info.bit_device_reset_process_flag == OAL_TRUE) {
        oam_warning_log0(0, OAM_SF_CFG, "{wal_cfg80211_del_pmksa::dfr revoverying, not process del_pmksa!}");
        return OAL_SUCC;
    }
