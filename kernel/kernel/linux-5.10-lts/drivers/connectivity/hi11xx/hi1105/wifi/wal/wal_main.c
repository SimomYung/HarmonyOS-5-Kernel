...
#endif
    g_ast_wal_host_ctx_table[HMAC_HOST_CTX_EVENT_SUB_TYPE_COFEATURE_STATE_NOTIFY].p_func = wal_cofeature_state_notify;
}
/*****************************************************************************
...
#endif
    g_ast_wal_host_ctx_table[HMAC_HOST_CTX_EVENT_SUB_TYPE_OKC_ROAM].p_func = wal_notify_wpa_okc_start;
...
#define MP16_11AX_MCS9_160M_4STREAMS_RATE "3.84Gbps"
...
OAL_STATIC ssize_t wal_get_wifi_devices_info_print(char *buf, int32_t buf_len)
{
    ...
    char *netcard_info[][NETCARD_INFO] = {
        ...
        { "Mp16", MP16_11AX_MCS9_160M_4STREAMS_RATE }
    };
...
OAL_STATIC ssize_t wal_get_bt_wifi_device_info(struct kobject *dev, struct kobj_attribute *attr, char *buf)
{
    ...
    const char *netcard_info[NETCARD_TYPE] = {
        "Hi1102", "Hi1103", "Hi1102a", "Hi1105", "Mp16"
    };
...
void nb_netlink_cmd_proc(oal_net_device_stru *net_dev, uint32_t cmd)
{
    if (cmd == NB_GET_REPORT_INFO) {
        wal_hipriv_nb_info_report(net_dev, "nb_report_info");
    }
}
void nb_netlink_rev(oal_netbuf_stru *skb)
{
    ...
    if (pst_skb->len >= oal_nlmsg_space(0)) {
        ...
        /* 检测报文长度正确性 */
        if (!oal_nlmsg_ok(pst_nlh, (uint32_t)pst_skb->len)) {
        ...
        nb_netlink_cmd_proc(net_dev, st_msg_hdr.cmd);
    }
    kfree_skb(pst_skb);
    return;
}

int32_t nb_netlink_create(void)
{
    g_nb_handler_data.nlsk = oal_netlink_kernel_create(&OAL_INIT_NET, NETLINK_NB_REPORT, 0,
        nb_netlink_rev, NULL, OAL_THIS_MODULE);
    if (g_nb_handler_data.nlsk == NULL) {
        oal_io_print("WIFI nb:fail to create netlink socket \n");
        return -OAL_EFAIL;
    }

    oal_io_print("WIFI nb:suceed to create netlink socket\n");
    return OAL_SUCC;
}
int32_t init_nb_handler(void)
{
    int32_t ret;

    oal_io_print("nb: into init_nb_enable_handler\n");

    memset_s((uint8_t *)&g_nb_handler_data, sizeof(g_nb_handler_data),
             0, sizeof(g_nb_handler_data));

    g_nb_handler_data.data = (uint8_t *)kzalloc(OAL_NB_DATA_BUF_LEN, GFP_KERNEL);
    if (g_nb_handler_data.data == NULL) {
        oal_io_print("nb: alloc g_nb_handler_data.puc_data fail, len = %d.\n", OAL_NB_DATA_BUF_LEN);
        g_nb_handler_data.data = NULL;
        return -OAL_EFAIL;
    }
...
void wal_destroy_all_vap(void)
{
    ...
    for (vap_id = oal_board_get_service_vap_start_id(); vap_id < mac_chip_get_service_vap_limit_max_id(); vap_id++) {
        pst_net_dev = hmac_vap_get_net_device(vap_id);
        if (pst_net_dev != NULL) {
