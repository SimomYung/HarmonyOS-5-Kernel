...
void wal_hipriv_phy_debug_init(mac_phy_debug_switch_stru *phy_debug_switch)
{
...
uint32_t wal_hipriv_mac_addr_write(oal_net_device_stru *net_dev, int8_t *param)
{
    ...
    ***************************************************************************/
    ...
    ret = wal_send_cfg_event(net_dev, WAL_MSG_TYPE_WRITE, WAL_MSG_WRITE_MSG_HDR_LENGTH + len,
                             (uint8_t *)&write_msg, OAL_FALSE, NULL);
    if (oal_unlikely(ret != OAL_SUCC)) {
        oam_warning_log1(0, OAM_SF_ANY, "{wal_hipriv_mac_addr_write::send cfg event fail, ret[%d]!}", ret);
        return (uint32_t)ret;
    }
...
OAL_STATIC uint32_t wal_hipriv_set_mac_addr_from_efuse(oal_net_device_stru *net_dev)
{
    ...
    /* 检查 netdevice mac addr 是否已经配配置过 */
    if (oal_memcmp(g_wlan_device_mac_addr, oal_netdevice_mac_addr(net_dev), WLAN_MAC_ADDR_LEN) == 0) {
        oam_warning_log0(0, OAM_SF_ANY, "{wal_hipriv_set_mac_addr_from_efuse::netdevice mac addr is already set}");
        return OAL_SUCC;
    }
    /* 配置设备mac地址为efuse中读取的地址 */
    mac_vap = oal_net_dev_priv(net_dev);
    if (mac_vap == NULL) {
        oam_error_log0(0, OAM_SF_ANY, "{wal_hipriv_set_mac_addr_from_efuse::mac_vap = null!}");
        return -OAL_EINVAL;
    }
    ...
    if ((g_wlan_device_mac_addr[MAC_ADDR_3] == 0) && (g_wlan_device_mac_addr[MAC_ADDR_4] == 0) &&
        ...
        oam_error_log0(0, OAM_SF_ANY,
            "{wal_hipriv_set_mac_addr_from_efuse::the last 3 bytes of mac addr use random values}");
    ...
    if (ret != OAL_SUCC) {
        oam_warning_log1(0, OAM_SF_ANY, "{wal_hipriv_set_mac_addr_from_efuse::set mac addr fail[%d].}", ret);
    }
    return ret;
}
...
uint32_t wal_hipriv_mac_addr_read(oal_net_device_stru *net_dev, int8_t *param)
{
    ...
    mac_vap = oal_net_dev_priv(net_dev);
    if (mac_vap == NULL) {
        oam_error_log0(0, OAM_SF_ANY, "{wal_hipriv_mac_addr_read::mac_vap = null!}");
        return -OAL_EINVAL;
    }
    hmac_vap = (hmac_vap_stru *)mac_res_get_hmac_vap(mac_vap->uc_vap_id);
    if (hmac_vap == NULL) {
        oam_error_log0(mac_vap->uc_vap_id, OAM_SF_ANY, "wal_hipriv_mac_addr_read::hmac_vap =  null!");
        return -OAL_EINVAL;
    }
    ...
    ret = wal_send_cfg_event(net_dev, WAL_MSG_TYPE_WRITE, WAL_MSG_WRITE_MSG_HDR_LENGTH + len,
                             (uint8_t *)&write_msg, OAL_FALSE, NULL);
    if (oal_unlikely(ret != OAL_SUCC)) {
        oam_warning_log1(0, OAM_SF_ANY, "{wal_hipriv_mac_addr_read::wal_send_cfg_event failed, ret[%d]!}", ret);
        return (uint32_t)ret;
    }
    ...
    if (leftime <= 0) {
        /* 超时或定时器内部错误上报 */
        oam_warning_log1(mac_vap->uc_vap_id, OAM_SF_ANY, "{wal_hipriv_mac_addr_read::wait efuse report for %ld ms!}",
                         ((WAL_ATCMDSRB_DBB_NUM_TIME * 1000) / OAL_TIME_HZ)); /* 1s等于1000ms */
        return -OAL_EINVAL;
    }
    oal_io_print("{wal_hipriv_mac_addr_read::mac_addr:[%02X:%02X:%02X:%02X:%02X:%02X]!}",
        g_wlan_device_mac_addr[MAC_ADDR_0], g_wlan_device_mac_addr[MAC_ADDR_1], g_wlan_device_mac_addr[MAC_ADDR_2],
        g_wlan_device_mac_addr[MAC_ADDR_3], g_wlan_device_mac_addr[MAC_ADDR_4], g_wlan_device_mac_addr[MAC_ADDR_5]);

    ret = wal_hipriv_set_mac_addr_from_efuse(net_dev);
    if (ret != OAL_SUCC) {
        oam_warning_log1(0, OAM_SF_ANY, "{wal_hipriv_mac_addr_read::set mac addr from efuse fail[%d].}", ret);
    }
    return OAL_SUCC;
}

/*
 * 函 数 名  : wal_hipriv_init_mac_addr_from_efuse
 * 功能描述  : 输入参数  : 无
 * 1.日    期  : 2023年11月03日
 *   修改内容  : 新生成函数
 */
void wal_hipriv_init_mac_addr_from_efuse(void)
{
    uint8_t auc_primary_mac_addr[WLAN_MAC_ADDR_LEN] = { 0 };
    oal_net_device_stru *pst_net_dev = NULL;
    int32_t ret;
    uint8_t netdev_cnt, mac_addr0;

    pst_net_dev = wal_config_get_netdev("wlan0", OAL_STRLEN("wlan0"));
    if (pst_net_dev == NULL) {
        oal_io_print("wal_hipriv_init_mac_addr_from_efuse: wlan0 not exist!}");
        return;
    }
    oal_dev_put(pst_net_dev); /* 调用oal_dev_get_by_name后，必须调用oal_dev_put使net_dev的引用计数减一 */

    if (hmac_get_wlan_first_power_on_cali_flag()) {
        ret = wal_init_wlan_vap(pst_net_dev);
        if (ret != OAL_SUCC) {
            oam_error_log1(0, OAM_SF_ANY, "{wal_hipriv_init_mac_addr_from_efuse::wal_init_wlan_vap FAIL %d}", ret);
            return;
        }
        wal_hipriv_mac_addr_read(pst_net_dev, "mac_read");
        ret = wal_deinit_wlan_vap(pst_net_dev);
        if (ret != OAL_SUCC) {
            oam_error_log1(0, OAM_SF_ANY, "{wal_hipriv_init_mac_addr_from_efuse::wal_deinit_wlan_vap FAIL %d}", ret);
            return;
        }
        /* 复制wlan0的mac地址 */
        if (EOK != memcpy_s(auc_primary_mac_addr, WLAN_MAC_ADDR_LEN,
                            oal_netdevice_mac_addr(pst_net_dev), WLAN_MAC_ADDR_LEN)) {
            oam_error_log0(0, OAM_SF_ANY, "wal_hipriv_init_mac_addr_from_efuse::memcpy fail!");
            return;
        }
        mac_addr0 = auc_primary_mac_addr[MAC_ADDR_0];
        /* 配置P2P0和wlan1的Mac地址 */
        for (netdev_cnt = 0; netdev_cnt < NUM_2_BYTES; netdev_cnt++) {
            if (netdev_cnt == 0) {
                pst_net_dev = wal_config_get_netdev("p2p0", OAL_STRLEN("p2p0"));
                auc_primary_mac_addr[MAC_ADDR_0] ^= 0x02; /* 将p2p0 mac地址首字节的bit1设置为与wlan0相反 */
            } else if (netdev_cnt == 1) {
                pst_net_dev = wal_config_get_netdev("wlan1", OAL_STRLEN("wlan1"));
                auc_primary_mac_addr[MAC_ADDR_0] ^= 0x04; /* 将wlan1 mac地址首字节的bit2设置为与wlan0相反 */
            }
            if (pst_net_dev == NULL) {
                oal_io_print("wal_hipriv_init_mac_addr_from_efuse: netdev[%d] not exist!}", netdev_cnt);
                return;
            }
            oal_dev_put(pst_net_dev); /* 调用oal_dev_get_by_name后，必须调用oal_dev_put使net_dev的引用计数减一 */

            oal_set_mac_addr((uint8_t *)oal_netdevice_mac_addr(pst_net_dev), auc_primary_mac_addr);
            auc_primary_mac_addr[MAC_ADDR_0] = mac_addr0;
        }
    }
}
#endif
...
const wal_ioctl_mode_map_stru g_ast_mode_map[] = {
    ...
    /* 11be */
    { "11be2g20",           WLAN_EHT_MODE, WLAN_BAND_2G, WLAN_BAND_WIDTH_20M },
    { "11be2g40plus",       WLAN_EHT_MODE, WLAN_BAND_2G, WLAN_BAND_WIDTH_40PLUS },
    { "11be2g40minus",      WLAN_EHT_MODE, WLAN_BAND_2G, WLAN_BAND_WIDTH_40MINUS },
    { "11be5g20",           WLAN_EHT_MODE, WLAN_BAND_5G, WLAN_BAND_WIDTH_20M },
    { "11be5g40plus",       WLAN_EHT_MODE, WLAN_BAND_5G, WLAN_BAND_WIDTH_40PLUS },
    { "11be5g40minus",      WLAN_EHT_MODE, WLAN_BAND_5G, WLAN_BAND_WIDTH_40MINUS },
    { "11be5g80plusplus",   WLAN_EHT_MODE, WLAN_BAND_5G, WLAN_BAND_WIDTH_80PLUSPLUS },
    { "11be5g80plusminus",  WLAN_EHT_MODE, WLAN_BAND_5G, WLAN_BAND_WIDTH_80PLUSMINUS },
    { "11be5g80minusplus",  WLAN_EHT_MODE, WLAN_BAND_5G, WLAN_BAND_WIDTH_80MINUSPLUS },
    { "11be5g80minusminus", WLAN_EHT_MODE, WLAN_BAND_5G, WLAN_BAND_WIDTH_80MINUSMINUS },
#ifdef _PRE_WLAN_FEATURE_160M
    { "11be5g160plusplusplus",    WLAN_EHT_MODE, WLAN_BAND_5G, WLAN_BAND_WIDTH_160PLUSPLUSPLUS },
    { "11be5g160plusplusminus",   WLAN_EHT_MODE, WLAN_BAND_5G, WLAN_BAND_WIDTH_160PLUSPLUSMINUS },
    { "11be5g160plusminusplus",   WLAN_EHT_MODE, WLAN_BAND_5G, WLAN_BAND_WIDTH_160PLUSMINUSPLUS },
    { "11be5g160plusminusminus",  WLAN_EHT_MODE, WLAN_BAND_5G, WLAN_BAND_WIDTH_160PLUSMINUSMINUS },
    { "11be5g160minusplusplus",   WLAN_EHT_MODE, WLAN_BAND_5G, WLAN_BAND_WIDTH_160MINUSPLUSPLUS },
    { "11be5g160minusplusminus",  WLAN_EHT_MODE, WLAN_BAND_5G, WLAN_BAND_WIDTH_160MINUSPLUSMINUS },
    { "11be5g160minusminusplus",  WLAN_EHT_MODE, WLAN_BAND_5G, WLAN_BAND_WIDTH_160MINUSMINUSPLUS },
    { "11be5g160minusminusminus", WLAN_EHT_MODE, WLAN_BAND_5G, WLAN_BAND_WIDTH_160MINUSMINUSMINUS },
#endif
#endif
    { NULL }
};
 ...
 *   修改内容  : 新生成函数
 */
uint32_t wal_ioctl_set_ap_mode(oal_net_device_stru *net_dev, uint8_t param)
{
    wal_msg_write_stru write_msg = {0};
    int32_t ret;
    ...
    oam_warning_log1(0, OAM_SF_ANY, "{wal_ioctl_set_ap_mode::param[%d].}", param);
    return OAL_SUCC;
}
...
int32_t wal_init_wifi_tas_state(void)
{
    ...
    if (hal_get_wifi_tas_gpio_state() != wifi_tas_state) {
        return hal_wifi_tas_set(wifi_tas_state);
    } else {
