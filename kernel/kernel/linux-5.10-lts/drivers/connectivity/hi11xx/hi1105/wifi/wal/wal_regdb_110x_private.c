...
int32_t wal_regdb_find_db_from_private_file(int8_t *str)
{
    ...
    uint32_t str_len = (uint32_t)OAL_STRLEN(str);
    if (oal_unlikely((str == NULL) || (str_len < NUM_2_BYTES))) {
        ini_error("input is fail, str_len [%d] please check", str_len);
        return OAL_FAIL;
    }
