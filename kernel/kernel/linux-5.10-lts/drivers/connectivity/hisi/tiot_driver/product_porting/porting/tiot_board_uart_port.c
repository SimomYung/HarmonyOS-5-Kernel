...
{
    static unsigned int tty_port_wait_default = ~0U;
    ...
    port = tty->port;
    /* 保存内核默认值 */
    if (tty_port_wait_default == (~0U)) {
        tty_port_wait_default = port->closing_wait;
    }
    /* 每次open时，将closing_wait恢复为内核默认值 */
    port->closing_wait = tty_port_wait_default;
    /* 打开后再配置波特率 */
    tty_unlock(tty);
    /* 先注册ldisc再set ldisc */
