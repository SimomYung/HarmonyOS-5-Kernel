...
struct devfreq *find_devfreq_by_name(const char *name)
{
	...
	list_for_each_entry(tmp_devfreq, &devfreq_list, node) {
		if (!strcmp(dev_name(&tmp_devfreq->dev), name)) {
...
unsigned long find_available_max_freq(struct devfreq *devfreq)
{
	struct dev_pm_opp *opp;
			     ...
			     char *buf)
{
	...
	if (df->profile->get_cur_freq &&
		...
		if (df->profile->freq_hide_convert && !liblinux_pal_get_freq_privileged_user())
			freq = df->profile->freq_hide_convert(freq);
...
#ifdef CONFIG_LIBLINUX
	if (df->profile->freq_hide_convert && !liblinux_pal_get_freq_privileged_user())
		min_freq = df->profile->freq_hide_convert(min_freq);
#endif
...
#ifdef CONFIG_LIBLINUX
	if (df->profile->freq_hide_convert && !liblinux_pal_get_freq_privileged_user())
		max_freq = df->profile->freq_hide_convert(max_freq);
#endif
...
#ifdef CONFIG_LIBLINUX
	if (df->profile->freq_hide_check && !liblinux_pal_get_freq_privileged_user()) {
		for (i = 0; i < df->profile->max_state; i++) {
			if (df->profile->freq_hide_check(df->profile->freq_table[i]))
				break;
			count += scnprintf(&buf[count], (PAGE_SIZE - count - 2),
					"%lu ", df->profile->freq_hide_convert(df->profile->freq_table[i]));
		}
...
#ifdef CONFIG_DEVFREQ_WQ_HIGHPRI
	devfreq_wq = alloc_workqueue("devfreq_wq", __WQ_LEGACY | WQ_FREEZABLE |
