...
#include "hifi_report/hifi_report.h"
#include "buck_boost_notifier/buck_boost_notifier.h"
#include "battery_state/battery_state.h"
#include "audio_info_collection/audio_info_collection.h"
#include "vendorcountry_state/vendorcountry_state.h"
#include "audio_maintain/audio_maintain.h"
#include "audio_mic_block/audio_mic_block.h"
#include "audio_wakeup_info/audio_wakeup_info.h"
...
static const struct audio_misc_module_table g_mod_table[] = {
	...
	{ "audio_info_collection", audio_info_collection_probe },
	{ "hifi_report", hifi_report_probe },
	{ "buck_boost_notifier", buck_boost_notifier_probe },
	{ "battery_state", battery_state_probe },
	{ "vendorcountry_state", vendorcountry_state_probe },
	{ "audio_maintain", audio_maintain_probe },
	{ "audio_mic_block", audio_mic_block_probe },
    { "audio_wakeup_info", audio_wakeup_info_probe }
};
...
void *audio_misc_module_get_priv(const char *name)
{
	struct audio_misc_module *p = NULL;
...
static void audio_misc_module_probe(struct device_node *np)
{
	...
	for (i = 0; i < sizeof(g_mod_table) / sizeof(struct audio_misc_module_table); ++i) {
		...
		if (!np_child)
			continue;
		ret = g_mod_table[i].probe(g_dev->dev, np_child);
		if (ret)
