...
static void adjust_hall_work(struct work_struct *work)
{
	struct vibrator_state_priv *priv = container_of(work, struct vibrator_state_priv, hall_work);

	mutex_lock(&priv->hall_state_lock);
	// bit 1  1 is PHONE_FOLD  0 is PHONE_UNFOLD
	priv->hall_state_number = (priv->hall_state_number >> EXT_HALL_BIT_HINGLE) & PHONE_FOLD;
	if (priv->hall_state_number) {
	...
	unsigned long hall_state, void *data)
{
    ...
    priv->hall_state_number = hall_state;
	schedule_work(&priv->hall_work);
	return NOTIFY_DONE;
}
...
int hall_state_init(struct device_node *np, struct vibrator_state_priv *priv)
{
	...
	if (!of_property_read_u32(np, "is_support_hall_notifier", &tmp)) {
		is_support_hall_notifier = tmp;
		hwlog_info("is_support_hall_notifier = %u.\n", is_support_hall_notifier);
	}
	if (is_support_hall_notifier) {
		INIT_WORK(&priv->hall_work, adjust_hall_work);
