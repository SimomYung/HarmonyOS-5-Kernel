			   ...
			   const char *propname, int boxid, int index)
{
	...
	/* set gpio to output low status */
	if (device_boxid.is_switch_direction) {
		ret = gpio_direction_output(info[boxid].gpio_id[index], 0);
		if (ret)
			pr_err("%s: set gpio to output low error! ret is %d\n",
				__func__, ret);
	}
...
static bool get_boxid_is_switch_direction(struct device_node *node)
{
	if (of_property_read_bool(node, "is_switch_direction")) {
		pr_info("%s: enable gpio switch\n", __func__);
		return true;
	}

	pr_info("%s: disable gpio switch\n", __func__);
	return false;
}
...
static int get_boxid_info(struct device_node *node)
{
	...
	device_boxid.is_switch_direction = get_boxid_is_switch_direction(node);
...
static void update_spkbox_voltage_status(unsigned int voltage)
{
	struct box_info_st *info = device_boxid.box_info;
	...
	pr_info("hkadc voltage = %u, spkbox_voltage_status = %d\n",
...
static int boxid_probe_use_hkadc(struct platform_device *pdev)
{
	...
	update_spkbox_voltage_status((unsigned int)hkadc_voltage);
	...
	unsigned int *array, unsigned int arr_len)
{
	...
	if (elem_num_dt_parse != test->total_elem_num) {
		pr_err("%s: total_elem_num:%u and %u is error, is not match\n",
			__func__, test->total_elem_num, elem_num_dt_parse);
		return false;
	}

	if (elem_num_dt_parse > arr_len) {
		pr_err("%s: elem_num_dt_parse:%u is invaild, pls check\n",
	...
	struct box_consistent_test *test)
{
	...
	for (i = 0; i < test->group_num; i++) {
		...
		for (j = 0; j < test->groups[i].num; j++) {
			...
			test->groups[i].boxid[j] = (int)test_group_dt_parse[index];
			index++;
		}
	}
