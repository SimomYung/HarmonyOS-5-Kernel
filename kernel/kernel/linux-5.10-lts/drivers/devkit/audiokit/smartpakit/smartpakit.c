...
#endif
#if IS_ENABLED(CONFIG_COLLEC_AUDIO_INFO)
    send_pa_manufacturer_to_collec_audio_info(info.chip_model);
...
static int smartpakit_switch_dpin_gpio(struct smartpakit_priv *pakit_priv,
	void __user *arg)
{
	int ret = -1;
	int dpin_status = 0; 
	...
	if (pakit_priv->dpin_gpio == 0) {
		hwlog_err("%s get dpin_gpio value fail\n", __func__);
		return ret;
	}

	ret = (int)copy_from_user(&dpin_status, arg, sizeof(int));
	if (ret != 0) {
		hwlog_err("%s: get smartpakit_switch_dpin_gpio state fail, ret = %d\n", __func__, ret);
		return ret;
	}

	ret = gpio_direction_output(pakit_priv->dpin_gpio, dpin_status);
	if (ret != 0)
        hwlog_err("%s: set dpin_gpio direction output fail, ret = %d\n", __func__, ret);

	hwlog_info("%s: dpin_gpio = %u, dpin_status = %d\n", __func__, pakit_priv->dpin_gpio, dpin_status);
	return ret;
}
	...
	void __user *arg, int compat_mode)
{
	...
	case SMARTPAKIT_HIGH_POWER_R_INFO:
		...
		break;
	case SMARTPAKIT_SWITCH_DPIN_GPIO:
	    ret = smartpakit_switch_dpin_gpio(pakit_priv, arg);
		break;
	default:
...
static void smartpakit_free_dpin_gpio(struct smartpakit_priv *pakit_priv)
{
	if (!pakit_priv) {
		hwlog_err("%s: invalid argument\n", __func__);
		return;
    ...
    if (pakit_priv->dpin_gpio > 0) {
        if (gpio_is_valid(pakit_priv->dpin_gpio))
            gpio_free(pakit_priv->dpin_gpio);
    }
}
...
static void smartpakit_init_dpin_gpio(struct smartpakit_priv *pakit_priv)
{
	if (!pakit_priv) {
		hwlog_err("%s: invalid argument\n", __func__);
		return;
    ...
    if (pakit_priv->dpin_gpio > 0) {
        if (gpio_request(pakit_priv->dpin_gpio, "dpin_gpio") < 0)
            hwlog_err("%s: request gpio dpin_gpio fail\n", __func__);
    }
}
	...
	struct smartpakit_priv *pakit_priv)
{
	...
	const char *dpin_gpio_str = "dpin_gpio";
	int ret;
	...
	ret = smartpakit_get_prop_of_u32_type(pdev->dev.of_node, dpin_gpio_str,
		&pakit_priv->dpin_gpio, true);
...
static void smartpakit_free(struct smartpakit_priv *pakit_priv)
{
    ...
    smartpakit_free_dpin_gpio(pakit_priv);
...
static int smartpakit_probe(struct platform_device *pdev)
{
	...
	smartpakit_init_dpin_gpio(pakit_priv);
