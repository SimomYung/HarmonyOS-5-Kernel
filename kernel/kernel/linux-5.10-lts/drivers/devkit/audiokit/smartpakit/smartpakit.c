 ...
 * smartpakit driver
 *
 * Copyright (c) 2017-2025 Huawei Technologies Co., Ltd.
 *
...
#include "sts.h"
#ifdef CONFIG_AUDIO_IO_AGGRE
#include <audio_io_aggre/audio_io_aggre.h>
#endif /* CONFIG_AUDIO_IO_AGGRE */
#endif /* CONFIG_CONTEXTHUB_IO_DIE_STS */

#ifdef CONFIG_AUDIO_BATTERY_STATE
#include "audio_misc/battery_state/battery_state.h"
#endif
...
#ifdef CONFIG_CONTEXTHUB_IO_DIE_STS
#ifdef CONFIG_AUDIO_IO_AGGRE
static void smartpa_aggre_inited_callback()
{
	audio_io_aggre_sts_inited_vote();
}

static void smartpa_aggre_recovery_callback() {}
...
static int smartpakit_init_chips(struct smartpakit_priv *pakit_priv)
{
	...
	if (pakit_priv->is_expand_smartpakit) {
		/* resume chips */
		for (i = 0; i < pakit_priv->pa_num; i++) {
			sequence = &pakit_priv->resume_sequence_v2[i];
			i2c_priv_p = pakit_priv->i2c_priv[i];
			hwlog_info("%s: resume pa %u\n", __func__, sequence->pa_ctl_index[0]);
			ret = pakit_priv->ioctl_ops->write_regs(i2c_priv_p, sequence->param_num, sequence->node);
			if (ret < 0)
				hwlog_err("%s: resume pa %d failed\n", __func__, i);
		}
	} else {
		/* init chips */
...
int smartpakit_resume_chips(struct smartpakit_priv *pakit_priv)
{
	...
	ret = smartpakit_init_chips(pakit_priv);
	if (ret < 0)
		goto err_out;
	...
	unsigned int ctl_mask)
{
	...
	for (i = 0; i < SMARTPAKIT_PA_ID_MAX; i++) {
		...
		if (smartpakit_priv_data->is_expand_smartpakit)
			sequence->pa_ctl_index[i] = i;
		else
	...
	struct smartpakit_pa_ctl_sequence *sequence)
{
	...
	for (i = 0; i < sequence->pa_ctl_num; i++) {
...
static int smartpakit_set_sequence_regs_deal_switch(unsigned int index, struct smartpakit_priv *pakit_priv,
	struct smartpakit_pa_ctl_sequence *sequence, struct smartpakit_pa_ctl_sequence **save, int seq_choose)
{
	hwlog_info("%s: index=%u, seq_choose=%d\n", __func__, index, seq_choose);
	switch (seq_choose) {
	case SEQ_POWERON:
		*save = &pakit_priv->poweron_seq[index];
		break;
	case SEQ_RESUME:
		*save = &pakit_priv->resume_sequence_v2[index];
		break;
	default:
		hwlog_info("%s: unsupport seq_choose", __func__);
		return -1;
	}
	return 0;
}

static int smartpakit_set_sequence_regs(struct smartpakit_priv *pakit_priv,
	struct smartpakit_pa_ctl_sequence *sequence, int seq_choose)
{
	struct smartpakit_pa_ctl_sequence *save = NULL;
	struct smartpakit_param_node *node = NULL;
	unsigned int offset;
	unsigned int param_size;
	unsigned int index;
	...
	if ((pakit_priv == NULL) || (sequence == NULL) || (sequence->node == NULL) ||
		seq_choose < SEQ_BEGIN || seq_choose >= SEQ_END) {
		hwlog_err("%s: invalid argument\n", __func__);
		return -EINVAL;
	}

	hwlog_info("%s:mask=0x%x,poweron_flag=%u,ctl_num=%u,%u node per pa,pa_index=%u\n",
		__func__, sequence->pa_ctl_mask, sequence->pa_poweron_flag,
		sequence->pa_ctl_num, sequence->param_num_of_one_pa, sequence->pa_ctl_index_max);
	...
	index = sequence->pa_ctl_index_max;
	if (index > pakit_priv->pa_num) {
		hwlog_err("%s: invalid pa_num\n", __func__);
		return -1;
	}
	if (smartpakit_set_sequence_regs_deal_switch(index, pakit_priv, sequence, &save, seq_choose) != 0)
		return -1;
	smartpakit_reset_pa_ctl_sequence(save);
	if (sequence->pa_poweron_flag == SMARTPA_REGS_FLAG_POWER_OFF && seq_choose != SEQ_RESUME) {
	...
	node = sequence->node;
	offset = (index * SMARTPAKIT_PA_CTL_OFFSET);
	save->pa_ctl_mask = (1 << offset);
	save->pa_poweron_flag = SMARTPA_REGS_FLAG_NEED_RESUME;
	save->pa_ctl_num = 1;
	save->pa_ctl_index_max = sequence->pa_ctl_index_max;
	save->pa_ctl_index[index] = sequence->pa_ctl_index[index];
#ifndef CONFIG_FINAL_RELEASE
	hwlog_info("%s: set mask 0x%x, index %u\n", __func__,
		save->pa_ctl_mask, save->pa_ctl_index_max);
#endif
	save->param_num = sequence->param_num_of_one_pa;
	save->param_num_of_one_pa = sequence->param_num_of_one_pa;
	param_size = sizeof(*node) * save->param_num;
	save->node = kzalloc(param_size, GFP_KERNEL);
	if (save->node == NULL)
		return -ENOMEM;

	memcpy(save->node, node, param_size);
	...
	struct smartpakit_info *info)
{
	...
	if (pakit_priv->cust != NULL) {
		...
		cust_len = (cust_len < SMARTPAKIT_NAME_MAX) ? cust_len : 0;
		strncpy(info->cust, pakit_priv->cust, cust_len);
	}
	info->cust[cust_len] = '\0';
	...
	info->special_name_config[special_name_len] = '\0';
	...
	info->product_name[product_name_len] = '\0';
	...
	for (i = 0; i < SMARTPAKIT_PA_ID_MAX; i++) {
		...
		info->chip_model_list[i][chip_model_len] = '\0';
	}
	...
	void __user *arg)
{
...
#endif
#if IS_ENABLED(CONFIG_AUDIO_INFO_COLLECTION)
	send_pa_manufacturer_to_collection(info.chip_model);
	...
	for (i = 0; i < pakit_priv->pa_num; i++) {
		pakit_priv->initialized_pa_index[i] = false;
	}
#ifndef CONFIG_FINAL_RELEASE
...
static int smartpakit_get_single_irq_filter_cfgs(
	...
	unsigned int chip_id, struct list_head *cfg_list)
{
	unsigned int i;
	...
	for (i = 0; i < node_num; i++) {
		...
		if (node_type == SMARTPAKIT_PARAM_NODE_TYPE_IRQ_FILTER) {
			...
			hwlog_info("IRQ_FILTER,pa%u,reg 0x%x,m 0x%x, v 0x%x\n",
...
static void smartpakit_get_irq_filter_cfgs(struct smartpakit_priv *pakit_priv)
{
	...
	for (j = 0; j < sequence->pa_ctl_num; j++) {
		...
		if (ret < 0)
			hwlog_err("%s: get pa%u irq filter err\n", __func__,
				chip_id);
	}
}

static void smartpakit_get_irq_filter_cfgs_v2(struct smartpakit_priv *pakit_priv, unsigned int pa_index)
{
	struct smartpakit_pa_ctl_sequence *sequence = NULL;
	int param_num = 0;
	int ret;
	...
	sequence = &pakit_priv->resume_sequence_v2[pa_index];
	param_num = sequence->param_num_of_one_pa;
	ret = smartpakit_get_single_irq_filter_cfgs(sequence->node, param_num, pa_index, &pakit_priv->irq_filter_list);
	if (ret < 0)
		hwlog_err("%s: get pa%u irq filter err\n", __func__, pa_index);
}
...
static void smartpakit_avoid_init_repeatly(struct smartpakit_priv *pakit_priv)
{
	struct smartpakit_pa_ctl_sequence *sequence = NULL;
	int i;
...
static int smartpakit_write_regs_to_one_chip(
	struct smartpakit_priv *pakit_priv,
	struct smartpakit_pa_ctl_sequence *sequence)
{
	struct smartpakit_param_node *param_node = NULL;
	...
	if (sequence->param_num != sequence->param_num_of_one_pa) {
		hwlog_err("%s: sequence param num invalid\n", __func__);
		return -EINVAL;
	}

	/* only control one chip, let index_max be index */
	index = (int)sequence->pa_ctl_index_max;
	param_node = sequence->node;
	reg_num_need_ops = sequence->param_num;
	i2c_priv_p = pakit_priv->i2c_priv[index];
	ret = pakit_priv->ioctl_ops->write_regs(i2c_priv_p, reg_num_need_ops, param_node);
	return ret;
}

static int smartpakit_init_chips_with_i2c_v2(struct smartpakit_priv *pakit_priv,
	const void __user *arg, int compat_mode, unsigned int id)
{
	struct smartpakit_pa_ctl_sequence seq_temp;
	int ret = 0;
	unsigned int pa_index = 0;
	...
	if (pakit_priv->resume_sequence_permission_enable) {
		smartpakit_avoid_init_repeatly(pakit_priv);
		pakit_priv->resume_sequence_permission_enable = false;
	}

	ret = smartpakit_get_reg_node_seq_for_multi_chips(&seq_temp, arg,
		compat_mode);
	if (ret < 0) {
		mutex_unlock(&pakit_priv->resume_sequence_lock);
		mutex_unlock(&pakit_priv->i2c_ops_lock);
	...
	pa_index = seq_temp.pa_ctl_index_max;
	if (pakit_priv->initialized_pa_index[pa_index]) {
		mutex_unlock(&pakit_priv->resume_sequence_lock);
		mutex_unlock(&pakit_priv->i2c_ops_lock);
		hwlog_info("%s: pa %u has init\n", __func__, seq_temp.pa_ctl_index_max);
		return 0;
	}
	pakit_priv->initialized_pa_index[pa_index] = true;

	/* init chip regs */
	ret = smartpakit_set_sequence_regs(pakit_priv, &seq_temp, SEQ_RESUME);
	if (ret < 0)
		hwlog_err("%s: smartpakit_set_sequence_regs failed\n", __func__);

	smartpakit_get_irq_filter_cfgs_v2(pakit_priv, pa_index);

	ret += smartpakit_write_regs_to_one_chip(pakit_priv, &seq_temp);
	...
	const void __user *arg, int compat_mode, unsigned int id)
{
	...
	if (pakit_priv->is_expand_smartpakit)
		ret = smartpakit_set_sequence_regs(pakit_priv, &sequence, SEQ_POWERON);
	else
	...
	const void __user *arg, int compat_mode, unsigned int id)
{
	...
	if (pakit_priv->is_expand_smartpakit)
		ret = smartpakit_write_regs_to_one_chip(pakit_priv, &sequence);
	else 
		ret = smartpakit_write_regs_to_multi_chips(pakit_priv, &sequence);
...
static int smartpakit_shutdown_ringtone_save_pot_regs(struct smartpakit_priv *pakit_priv,
	void __user *arg, int compat_mode)
{
	int ret = 0;
	struct smartpakit_pa_ctl_sequence *sequence;
	struct smartpakit_pa_ctl_sequence *temp_seq;

	hwlog_info("%s: enter\n", __func__);
	if (!pakit_priv) {
		hwlog_err("%s: pakit_priv invalid\n", __func__);
		return -EINVAL;
	}

	mutex_lock(&pakit_priv->i2c_ops_lock);
	sequence = kzalloc(sizeof(*sequence), GFP_KERNEL);
	if (!sequence) {
		hwlog_err("%s: kzalloc faidled\n", __func__);
		mutex_unlock(&pakit_priv->i2c_ops_lock);
		return -ENOMEM;
	}
	ret = smartpakit_get_reg_node_seq_for_multi_chips(sequence, arg, compat_mode);
	if (ret < 0) {
		smartpakit_kfree_ops(sequence->node);
		smartpakit_kfree_ops(sequence);
		mutex_unlock(&pakit_priv->i2c_ops_lock);
		return ret;
	}

	if (!pakit_priv->i2c_priv[sequence->pa_ctl_index_max]->shutdown_ringtone) {
		hwlog_err("%s: shutdown_ringtone invalid\n", __func__);
		smartpakit_kfree_ops(sequence->node);
		smartpakit_kfree_ops(sequence);
		mutex_unlock(&pakit_priv->i2c_ops_lock);
		return -EINVAL;
	...
	temp_seq = pakit_priv->i2c_priv[sequence->pa_ctl_index_max]->shutdown_ringtone->shutdown_ringtone_seq;
	if (temp_seq) {
		smartpakit_kfree_ops(temp_seq->node);
		smartpakit_kfree_ops(temp_seq);
	}
	pakit_priv->i2c_priv[sequence->pa_ctl_index_max]->shutdown_ringtone->shutdown_ringtone_seq = sequence;
	hwlog_info("%s: save pa_%u shutdown_ringtone_seq\n", __func__, sequence->pa_ctl_index_max);
	mutex_unlock(&pakit_priv->i2c_ops_lock);
	return ret;
}

static int smartpakit_shutdown_ringtone_enable_pot(struct smartpakit_priv *pakit_priv)
{
	int ret = 0;
	unsigned int i = 0;
	...
	if (!pakit_priv->ioctl_ops || !pakit_priv->ioctl_ops->set_shutdown_ringtone) {
		hwlog_err("%s: set_shutdown_ringtone is invalid\n", __func__);
		return -EINVAL;
	}
	...
	for (i = 0; i < pakit_priv->pa_num; i++) {
		ret += pakit_priv->ioctl_ops->set_shutdown_ringtone(pakit_priv->i2c_priv[i], true);
	}
	return ret;
}
	...
	void __user *arg)
{
	...
	ret = (int)copy_from_user(&pa_check, arg, sizeof(int));
	if (ret != 0) {
	...
	void __user *arg, int compat_mode)
{
	...
	case SMARTPAKIT_HIGH_POWER_R_INFO:
		...
		break;
	case SMARTPAKIT_SAVE_POT_REG:
		ret = smartpakit_shutdown_ringtone_save_pot_regs(pakit_priv, arg, compat_mode);
		break;
	case SMARTPAKIT_ENABLE_POT:
		ret = smartpakit_shutdown_ringtone_enable_pot(pakit_priv);
		break;
	default:
...
int smartpakit_get_prop_num(struct device_node *node, const char *str)
{
	...
	const char *gpio_reset_num = "reset_gpio_num";

	if (node == NULL || str == NULL)
		return 0;

	if (of_property_read_bool(node, gpio_reset_num)) {
		ret = smartpakit_get_prop_of_u32_type(node, gpio_reset_num,
			(unsigned int *)&i, true);
		if (ret < 0)
			return 0;
...
static int smartpakit_pasre_gpio_from_dts(struct device_node *node,
	const char *gpio_name, int *gpio)
{
	int ret;
	...
	*gpio = of_get_named_gpio(node, gpio_name, 0);
	if (*gpio < 0) {
		hwlog_info("%s: %s of_get_named_gpio failed, %d\n",
			__func__, gpio_name, *gpio);
		ret = of_property_read_u32(node, gpio_name, (u32 *)gpio);
		if (ret < 0) {
	...
	struct smartpakit_switch_node **switch_ctl, unsigned int num)
{
	...
	ctl = kzalloc(sizeof(*ctl) * num, GFP_KERNEL);
	if (ctl == NULL)
		return -ENOMEM;
	...
	for_each_property_of_node(node, pp) {
		...
		if (i >= num) {
			hwlog_warn("%s: it exceed gpio num:%u\n", __func__, num);
			break;
		}
		...
		ret = smartpakit_pasre_gpio_from_dts(node, ctl[i].name,
			&ctl[i].gpio);
		if (ret < 0)
			goto err_out;

		if (!gpio_is_valid(ctl[i].gpio)) {
			hwlog_warn("%s: %s node %u gpio %d is invalid \n",
				__func__, pp->name, i, ctl[i].gpio);
			continue;
		}
		ctl[i].state = -1;
		i++;
	...
	struct smartpakit_priv *pakit_priv)
{
	...
	if ((count < 1) || (count > SMARTPAKIT_PA_ID_MAX)) {
		hwlog_err("%s: get pa_num failed %u, or pa_num %u>%d\n",
	...
	struct smartpakit_priv *pakit_priv)
{
	...
	pakit_priv->simple_pa_id.support_id_pinctrl = of_property_read_bool(node, "simple_pa_id_pinctrl");
...
get_simple_pa_id_status_phase1:
	...
	for (i = 0; i < gpio_id_num; i++) {
		if (pa_id_ctl[i].gpio < 0)
			continue;
		pa_id_ctl[i].state = gpio_get_value((unsigned int)pa_id_ctl[i].gpio);
		hwlog_info("%s: get pa_id[%d]:%d state:%d\n",
			__func__, i, pa_id_ctl[i].gpio, pa_id_ctl[i].state);
		if (pa_id_ctl[i].state < 0)
			continue;
		id_status |= ((unsigned int)pa_id_ctl[i].state << i);
	}
	...
	struct smartpakit_switch_node *ctl, struct property *pp)
{
	...
	ret = smartpakit_pasre_gpio_from_dts(node, pp->name, &ctl->gpio);
	if (ret < 0)
	...
	struct smartpakit_switch_node **pa_id_ctl, unsigned int gpio_id_num)
{
	...
	for_each_property_of_node(node, pp) {
		...
		ret = snprintf_s(ctl[i].name, (unsigned long)SMARTPAKIT_NAME_MAX,
			(unsigned long)SMARTPAKIT_NAME_MAX - 1, "simple_pa_id_%u", i);
		if (ret < 0) {
			hwlog_err("%s: set gpio name failed\n", __func__);
			ret = -EFAULT;
	...
	struct smartpakit_priv *pakit_priv)
{
	...
	const char *expand_smartpakit_str = "expand_smartpakit";
	int ret;
	...
	if (of_property_read_bool(pdev->dev.of_node, expand_smartpakit_str))
		pakit_priv->is_expand_smartpakit = true;
	else
		pakit_priv->is_expand_smartpakit = false;
...
static void smartpakit_init_chip_ctl_ops(struct smartpakit_priv *pakit_priv)
{
	...
	if ((pakit_priv->algo_in == SMARTPAKIT_ALGO_IN_SIMPLE) ||
		...
		if (pakit_priv->is_expand_smartpakit)
			smartpakit_ctrl_init_chips = smartpakit_init_chips_with_i2c_v2;
		else
...
#ifdef CONFIG_CONTEXTHUB_IO_DIE_STS
static void smartpakit_regist_aggre_callback(void)
{
	int ret;

#ifdef CONFIG_AUDIO_IO_AGGRE
	if (!smartpakit_priv_data->io_aggre_support)
		return;

	ret = audio_io_aggre_regist_event_callback(AGGRE_NOTIFIER_INITED, AGGRE_DEVICE_SMARTPA, smartpa_aggre_inited_callback);
	if (ret != 0)
		hwlog_err("%s: smartpakit regist aggre inited callback fail\n", __func__);
	
	ret = audio_io_aggre_regist_event_callback(AGGRE_NOTIFIER_RECOVERY, AGGRE_DEVICE_SMARTPA, smartpa_aggre_recovery_callback);
	if (ret != 0)
		hwlog_err("%s: smartpakit regist aggre recovery callback fail\n", __func__);
#else
	if (smartpakit_priv_data->io_aggre_support) {
		ret = sts_notify_register(STS_USER_AUDIO, sensorhub_state_cb);
		if (ret != 0)
			hwlog_err("%s: sts notify regist fail\n", __func__);
	}
#endif /* CONFIG_AUDIO_IO_AGGRE */
}
#endif /* CONFIG_CONTEXTHUB_IO_DIE_STS */
...
#ifdef CONFIG_CONTEXTHUB_IO_DIE_STS
	smartpakit_regist_aggre_callback();
#endif /* CONFIG_CONTEXTHUB_IO_DIE_STS */
...
#ifdef CONFIG_SND_SOC_AU_PA
	hwlog_info("%s: register hsicon reg ops\n", __func__);
