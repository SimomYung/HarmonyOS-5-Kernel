...
#define CHS_PMU_UWB_EN                           602
#define CHS_SHUTDONW_RINGTONE_SWITCH             605
#define SHUTDOWN_RINGTONE_SUPPLY_VOLTAGE         1800000
#define SHUTDOWN_RINGTONE_DEFAULT_TIME_MINUTE    60
...
enum irq_status_flag {
	DISABLE_SMARTPA_IRQ = 0,
	ENABLE_SMARTPA_IRQ,
};
...
static bool smartpakit_i2c_enable_virtual_pa_addr = false;
static int smartpakit_virtual_pa_probe_times = 0;
	...
	unsigned int status)
{
	...
	model_len = sizeof(dev_info->chip_model);
	strncpy(dev_info->chip_model, i2c_priv->chip_model, model_len - 1);
	dev_info->chip_model[model_len - 1] = '\0';
...
static int smartpakit_regmap_read(struct regmap *regmap,
...
static int smartpakit_regmap_write(struct regmap *regmap,
	unsigned int reg_addr, unsigned int value)
{
	int ret = 0;
	unsigned int i;
...
#if IS_ENABLED(CONFIG_AUDIO_INFO_COLLECTION)
    send_write_exception_to_collection();
#endif
...
static bool smartpakit_i2c_check_bgc_reg(struct smartpakit_priv *pakit_priv)
{
	int i;

	for (i = 0; i < pakit_priv->pa_num; ++i) {
		if (pakit_priv->i2c_priv[i] == NULL) {
			hwlog_warn("%s: priv of pa%d invalid", __func__, i);
			return false;
		}
		if (pakit_priv->i2c_priv[i]->bgc_normal_seq == NULL || pakit_priv->i2c_priv[i]->bgc_low_current_seq == NULL) {
			hwlog_warn("%s: reg config of pa%d invalid", __func__, i);
			return false;
		}
		if (smartpakit_check_i2c_regmap_cfg(pakit_priv->i2c_priv[i]) != 0) {
			hwlog_warn("%s: regmap of pa%d invalid", __func__, i);
			return false;
		}
	}
	return true;
...
void smartpakit_i2c_write_bgc_reg(bool is_low_current, bool need_lock)
{
	struct smartpakit_priv *pakit_priv = NULL;
	int i;
	struct smartpakit_reg_ctl_sequence *seq = NULL;
	struct regmap *regmap = NULL;
	unsigned int mask;

	pakit_priv = smartpakit_get_misc_priv();
	if (pakit_priv == NULL || !smartpakit_i2c_check_bgc_reg(pakit_priv))
		return;

	if (need_lock)
		mutex_lock(&pakit_priv->i2c_ops_lock);
	for (i = 0; i < pakit_priv->pa_num; ++i) {
		regmap = pakit_priv->i2c_priv[i]->regmap_cfg->regmap;
		seq = is_low_current ? pakit_priv->i2c_priv[i]->bgc_low_current_seq : pakit_priv->i2c_priv[i]->bgc_normal_seq;
		mask = pakit_priv->i2c_priv[0]->bgc_reg_mask;
		hwlog_info("%s: pa%d, reg = 0x%x, value = 0x%x, mask = 0x%x", __func__, i, seq->regs[0].addr,
			seq->regs[0].value, mask);
		smartpakit_regmap_update_bits(regmap, seq->regs[0].addr, mask, seq->regs[0].value);
	}
	if (need_lock)
		mutex_unlock(&pakit_priv->i2c_ops_lock);
}
EXPORT_SYMBOL_GPL(smartpakit_i2c_write_bgc_reg);
...
static int aw_smartpa_get_efuse_check(struct regmap *regmap, int *efuse_check)
{
    ...
    ret = smartpakit_regmap_read(regmap, AW_DBGCTRL_REG, &reg_val);
    if (ret < 0) {
    ...
    int efuse_check, short *icalk)
{
    ...
    ret = smartpakit_regmap_read(regmap, desc->icalk_reg, &reg_val);
    if (ret < 0) {
    ...
    ret = smartpakit_regmap_read(regmap, desc->icalkl_reg, &reg_val);
    if (ret < 0) {
    ...
    int efuse_check, short *vcalk)
{
    ...
    ret = smartpakit_regmap_read(regmap, desc->vcalk_reg, &reg_val);
    if (ret < 0) {
    ...
    ret = smartpakit_regmap_read(regmap, desc->vcalkl_reg, &reg_val);
    if (ret < 0) {
    ...
    unsigned int xml_mask, unsigned int vcal_factor)
{
    ...
    memset_s(desc, sizeof(struct aw_vcalb_desc), 0, sizeof(struct aw_vcalb_desc));
...
static int aw_smartpa_set_correction_v2(struct regmap *regmap, unsigned int value_factor,
    unsigned int xml_mask, unsigned int vcal_factor)
{
    int ret;
    int vcalb;
    ...
    if (!vcalk) {
        hwlog_err("%s: vcalk(0) is error", __func__);
        return -EINVAL;
    }
    vcalb = desc.vcal_factor * icalk / vcalk;
    reg_val = (unsigned int)vcalb;
    hwlog_info("%s: icalk=%d, vcalk=%d, vcalb=%d, reg_val=0x%04x",
	        __func__, icalk, vcalk, vcalb, reg_val);

    ret = smartpakit_regmap_write(regmap, desc.vcalb_reg, reg_val);
    if (ret < 0) {
        hwlog_err("%s: write reg 0x%x failed\n", __func__, desc.vcalb_reg);
        return ret;
    }

    hwlog_info("%s: write reg 0x%x(0x%04x) success\n", __func__, desc.vcalb_reg, reg_val);
    return ret;
}
...
static int smartpakit_i2c_set_shutdown_ringtone(struct smartpakit_i2c_priv *i2c_priv, bool isFactory)
{
	int ret = 0;
	...
	if (!i2c_priv->shutdown_ringtone) {
		hwlog_info("%s: no func\n", __func__);
		return ret;
	}
	if (!i2c_priv->shutdown_ringtone->shutdown_ringtone_seq) {
		hwlog_info("%s:invalid shutdown_ringtone_seq argument\n", __func__);
		return ret;
	}

	sequence = i2c_priv->shutdown_ringtone->shutdown_ringtone_seq;
	ret = smartpakit_i2c_ctrl_write_regs(i2c_priv, sequence->param_num, sequence->node);

	if (isFactory && i2c_priv->shutdown_ringtone->ldo_offd_supply) {
		ret += regulator_set_voltage(i2c_priv->shutdown_ringtone->ldo_offd_supply,
			SHUTDOWN_RINGTONE_DEFAULT_TIME_MINUTE, SHUTDOWN_RINGTONE_DEFAULT_TIME_MINUTE);
		if (ret == 0) {
			gpio_direction_output(CHS_SHUTDONW_RINGTONE_SWITCH, 1);
			hwlog_info("%s: set shutdown_ringtone power time and gpio success\n", __func__);
		}
	}

	hwlog_info("%s: set shutdown_ringtone_power_time end\n", __func__);
	return ret;
}
...
struct smartpakit_i2c_ctl_ops i2c_ctl_ops = {
	...
	.set_shutdown_ringtone = smartpakit_i2c_set_shutdown_ringtone,
};
...
#if IS_ENABLED(CONFIG_AUDIO_INFO_COLLECTION)
    send_painterrupt_to_collection();
#endif
	...
	struct smartpakit_gpio_sequence *sequence)
{
	...
	val_num = (int)(sizeof(struct smartpakit_gpio_state) / sizeof(unsigned int));
	if ((count % val_num) != 0) {
	...
	struct smartpakit_gpio_reset **reset)
{
	...
	ret = snprintf(reset_info->gpio_name,
		(unsigned long)SMARTPAKIT_NAME_MAX, "%s_gpio_reset_%u",
	...
	struct smartpakit_gpio_irq **irq)
{
	...
	ret = snprintf(irq_info->gpio_name, (unsigned long)SMARTPAKIT_NAME_MAX,
		"%s_gpio_irq_%u", i2c_priv->chip_model, i2c_priv->chip_id);
	ret += snprintf(irq_info->irq_name, (unsigned long)SMARTPAKIT_NAME_MAX,
		"%s_irq_%u", i2c_priv->chip_model, i2c_priv->chip_id);
	if (ret < 0) {
...
static void smartpakit_i2c_ctl_irq(struct smartpakit_gpio_irq *irq_handler,
	unsigned int irq_enable)
{
	if (irq_handler == NULL)
		return;

	if (irq_handler->ignore_same_irq)
		return;

	if (irq_enable == ENABLE_SMARTPA_IRQ) {
		enable_irq(irq_handler->irq);
		hwlog_info("%s: enable irq : %d\n",
			__func__, irq_handler->irq);
	} else {
		disable_irq(irq_handler->irq);
		hwlog_info("%s: disable_irq irq : %d\n",
			__func__, irq_handler->irq);
	}
}
	...
	struct smartpakit_i2c_priv *i2c_priv)
{
	...
	if (smartpakit_i2c_is_request_same_irq_gpio(gpio)) {
		...
		hwlog_info("%s: irq gpio %d had been requested, skip\n",
			__func__, gpio);
#endif
		irq_handler->ignore_same_irq = true;
		return 0;
	}
	...
	smartpakit_i2c_ctl_irq(irq_handler, DISABLE_SMARTPA_IRQ);
	return 0;
}
...
static int smartpakit_i2c_parse_dt_bgc_reg(struct device_node *pa_node, struct smartpakit_reg_ctl_sequence **seq,
	const char *bgc_cfg_str)
{
	int ret;

	if (!of_property_read_bool(pa_node, bgc_cfg_str)) {
		hwlog_warn("%s: %s not existed, skip", __func__, bgc_cfg_str);
		return 0;
	}
	ret = smartpakit_i2c_parse_reg_ctl(seq, pa_node, bgc_cfg_str);
	if (ret < 0) {
		hwlog_err("%s: parse reg fail", __func__);
	}
	return ret;
}

static int smartpakit_i2c_parse_dt_bgc(struct i2c_client *i2c, struct smartpakit_i2c_priv *i2c_priv)
{
	struct device_node *pa_node = NULL;
	int ret;
	...
	ret += smartpakit_i2c_parse_dt_bgc_reg(pa_node, &i2c_priv->bgc_normal_seq, "bgc_reg_normal");
	ret += smartpakit_i2c_parse_dt_bgc_reg(pa_node, &i2c_priv->bgc_low_current_seq, "bgc_reg_low_current");
	if (ret || (i2c_priv->bgc_normal_seq == NULL && i2c_priv->bgc_low_current_seq == NULL))
		return ret;
	if (i2c_priv->chip_id == 0) {
		i2c_priv->bgc_reg_mask = 0;
		ret += smartpakit_get_prop_of_u32_type(pa_node, "bgc_reg_mask", &i2c_priv->bgc_reg_mask, true);
		if (ret < 0) {
			hwlog_err("%s: parse bgc_reg_mask fail", __func__);
		}
	}
	return ret;
}

static void smartpakit_i2c_free_shutdown_ringtone(struct smartpakit_shutdown_ringtone **shutdown_ringtone_info)
{
	if (!shutdown_ringtone_info || !(*shutdown_ringtone_info))
		return;

	hwlog_info("%s: free shutdown_ringtone\n", __func__);
	if ((*shutdown_ringtone_info)->ldo_offd_supply)
		(*shutdown_ringtone_info)->ldo_offd_supply = NULL;
	if ((*shutdown_ringtone_info)->regulator_supply)
		(*shutdown_ringtone_info)->regulator_supply = NULL;
	if ((*shutdown_ringtone_info)->shutdown_ringtone_seq)
		smartpakit_kfree_ops((*shutdown_ringtone_info)->shutdown_ringtone_seq->node);
	smartpakit_kfree_ops((*shutdown_ringtone_info)->shutdown_ringtone_seq);
	smartpakit_kfree_ops(*shutdown_ringtone_info);
}

static void smartpakit_i2c_parse_dt_ldo_offd_supply(struct i2c_client *i2c, struct device_node *pa_node,
	struct smartpakit_shutdown_ringtone *shutdown_ringtone_info)
{
	const char *shutdown_ringtone_ctrl_str = "shutdown_ringtone_ctrl-supply";
	const char *shutdown_ringtone_ctrl_supply_str = "shutdown_ringtone_ctrl";

	if (!of_property_read_bool(pa_node, shutdown_ringtone_ctrl_str)) {
		hwlog_warn("%s: %s not existed, skip\n", __func__, shutdown_ringtone_ctrl_str);
		return;
	}

	shutdown_ringtone_info->ldo_offd_supply = devm_regulator_get(&i2c->dev, shutdown_ringtone_ctrl_supply_str);
	if (IS_ERR(shutdown_ringtone_info->ldo_offd_supply)) {
		hwlog_err("%s: no regulator found\n", __func__);
		if (shutdown_ringtone_info->ldo_offd_supply)
			shutdown_ringtone_info->ldo_offd_supply = NULL;
	}
}

static int smartpakit_i2c_parse_dt_shutdown_ringtone_supply(struct i2c_client *i2c,
	struct smartpakit_i2c_priv *i2c_priv)
{
	const char *shutdown_ringtone_supply_str = "shutdown_ringtone";
	const char *shutdown_ringtone_str = "shutdown_ringtone-supply";
	struct smartpakit_shutdown_ringtone *shutdown_ringtone_info = NULL;
	struct device_node *pa_node = NULL;
	int ret = 0;
	...
	if (!of_property_read_bool(pa_node, shutdown_ringtone_str)) {
		hwlog_warn("%s: %s not existed, skip", __func__, shutdown_ringtone_str);
		return 0;
	}

	shutdown_ringtone_info = kzalloc(sizeof(*shutdown_ringtone_info), GFP_KERNEL);
	if (!shutdown_ringtone_info) {
		hwlog_err("%s: kzalloc failed\n", __func__);
		return -ENOMEM;
	}

	smartpakit_i2c_parse_dt_ldo_offd_supply(i2c, pa_node, shutdown_ringtone_info);

	shutdown_ringtone_info->regulator_supply = devm_regulator_get(&i2c->dev, shutdown_ringtone_supply_str);
	if (IS_ERR(shutdown_ringtone_info->regulator_supply)) {
		hwlog_err("%s: no regulator found\n", __func__);
		smartpakit_i2c_free_shutdown_ringtone(&shutdown_ringtone_info);
		return ret;
	}

	ret = regulator_set_voltage(shutdown_ringtone_info->regulator_supply,
		SHUTDOWN_RINGTONE_SUPPLY_VOLTAGE, SHUTDOWN_RINGTONE_SUPPLY_VOLTAGE);
	if (ret != 0) {
		hwlog_err("%s: set voltage failed\n", __func__);
		devm_regulator_put(shutdown_ringtone_info->regulator_supply);
		smartpakit_i2c_free_shutdown_ringtone(&shutdown_ringtone_info);
		return ret;
	}
		
	ret = regulator_enable(shutdown_ringtone_info->regulator_supply);
	if (ret < 0) {
		hwlog_err("%s: regulator enable failed\n", __func__);
		smartpakit_i2c_free_shutdown_ringtone(&shutdown_ringtone_info);
		return ret;
	}

	i2c_priv->shutdown_ringtone = shutdown_ringtone_info;
	hwlog_info("%s: set shutdown_ringtone_supply end\n", __func__);
	return ret;
}
	...
	struct smartpakit_i2c_priv *i2c_priv)
{
	...
	hwlog_info("%s: chip_vendor:%u,chip_id:%u,chip_model:%s\n", __func__,
		...
		i2c_priv->chip_model);
#endif
	smartpakit_get_prop_of_u32_type(pa_node, high_power_pa_str,
		&(i2c_priv->high_power_pa), false);
	return 0;
	...
	struct smartpakit_i2c_priv *i2c_priv)
{
	...
	if (gpio == CHS_PMU_UWB_EN)
		goto skip_valid_check;
	...
	struct smartpakit_i2c_priv *i2c_priv)
{
	...
	i2c_priv->irq_handler->ignore_same_irq = false;
	...
	struct smartpakit_i2c_priv *i2c_priv)
{
	...
	smartpakit_i2c_free_reg_ctl(&i2c_priv->bgc_normal_seq);
	smartpakit_i2c_free_reg_ctl(&i2c_priv->bgc_low_current_seq);
	...
	struct list_head *version_req_list)
{
		...
		for (i = 0; i < version_val_check_seq->num; i++) {
			...
			if ((pos->reg_addr == version_val_regs[i].addr) &&
				...
				hwlog_info("%s:%s, pa %u, reg_addr 0x%x, "
					"value 0x%x, mask 0x%x, match_count:%u\n",
	...
	if (match_count == version_val_check_seq->num) {
		hwlog_info("%s:%s, pa_addr 0x%x, pa %u, "
			"match_count:%u, the version is matched\n",
			__func__, i2c_priv->chip_model, i2c_priv->i2c->addr,
...
static int smartpakit_i2c_need_to_retry(struct i2c_client *i2c)
{
    /* enable virtual pa global addr */
	bool is_enable_gbl_addr = 
	    of_property_read_bool(i2c->dev.of_node, "enable_virtual_pa_global_addr");
	if (is_enable_gbl_addr && (!smartpakit_i2c_enable_virtual_pa_addr)) {
		smartpakit_virtual_pa_probe_times++;
        hwlog_info("%s: this virtual pa driver need retry,retry times:%d\n",
		    __func__, smartpakit_virtual_pa_probe_times);
		return (smartpakit_virtual_pa_probe_times > 1) ?
		    -EINVAL : -EPROBE_DEFER;
	}
	return 0;
}

static int smartpakit_i2c_parse_enable_virtual_pa_regs(struct i2c_client *i2c,
	struct smartpakit_i2c_priv *i2c_priv)
{
	const char *enable_virtual_pa_regs_str = "enable_virtual_pa_regs";
	struct device_node *pa_node = NULL;
	int ret;
	...
	if (of_property_read_bool(pa_node, enable_virtual_pa_regs_str)) {
		ret = smartpakit_i2c_parse_reg_ctl(&i2c_priv->enable_virtual_pa_regs_seq,
				pa_node, enable_virtual_pa_regs_str);
		if (ret < 0) {
			hwlog_err("%s: enable_virtual_pa_regs get fail\n", __func__);
		}
	} else {
		hwlog_debug("%s:not need to enable virtual pa addr,skip\n",__func__);
		return 0;
	}
 	return ret;
}

static int smartpakit_i2c_enable_virtual_pa_global_addr(
	struct smartpakit_i2c_priv *i2c_priv)
{
	int ret;
	struct list_head enable_virtual_pa_global_addr_list;

	if (i2c_priv->enable_virtual_pa_regs_seq == NULL)
		return 0;

	INIT_LIST_HEAD(&enable_virtual_pa_global_addr_list);

	ret = smartpakit_i2c_dts_reg_ops(i2c_priv, i2c_priv->enable_virtual_pa_regs_seq,
		&enable_virtual_pa_global_addr_list, true);
	if (ret < 0) {
		hwlog_err("%s: %s, pa %u enable virtual pa global addr regs failed, %d\n",
			__func__, i2c_priv->chip_model, i2c_priv->chip_id, ret);
		goto enable_virtual_pa_global_addr_err;
	}
#ifndef CONFIG_FINAL_RELEASE
	hwlog_info("%s: pa %u, enable virtual pa global addr regs\n", __func__, 
	    i2c_priv->chip_id);
#endif
	smartpakit_i2c_enable_virtual_pa_addr = true;
	smartpakit_reg_info_del_list_all(&enable_virtual_pa_global_addr_list);
enable_virtual_pa_global_addr_err:
	return ret;
}

static int smartpakit_i2c_parse_params(struct i2c_client *i2c,
	struct smartpakit_i2c_priv *i2c_priv)
{
	int ret;
	...
	ret += smartpakit_i2c_parse_enable_virtual_pa_regs(i2c, i2c_priv);
	ret += smartpakit_i2c_parse_dt_bgc(i2c, i2c_priv);
	ret += smartpakit_i2c_parse_dt_shutdown_ringtone_supply(i2c, i2c_priv);
	return ret;
}
	...
	struct smartpakit_i2c_priv *i2c_priv, struct device_node **node)
{
	...
	ret = snprintf_s(name, SMARTPAKIT_NAME_MAX, SMARTPAKIT_NAME_MAX - 1, "smartpa%u", i2c_priv->pa_iter);
	if (ret < 0)
		return -EFAULT;
	...
	struct smartpakit_i2c_priv *i2c_priv)
{
	...
	ret = smartpakit_i2c_need_to_retry(i2c);
	if (ret < 0)
	    return ret;
	...
	for (i = 0; i < i2c_priv->pa_num_by_same_i2c_addr; i++) {
		...
		ret = smartpakit_i2c_parse_params(i2c, i2c_priv);
		if (ret < 0) {
			smartpakit_i2c_free_hw_reset(&i2c_priv->hw_reset);
			continue;
		}
		...
		ret = smartpakit_i2c_enable_virtual_pa_global_addr(i2c_priv);
		if (ret < 0)
			continue;
	...
	const struct i2c_device_id *id)
{
	...
	smartpakit_i2c_ctl_irq(i2c_priv->irq_handler, ENABLE_SMARTPA_IRQ);
...
void smartpakit_i2c_shutdown(struct i2c_client *i2c)
{
	...
	if (i2c_priv->shutdown_ringtone && i2c_priv->shutdown_ringtone->shutdown_ringtone_seq) {
		hwlog_info("%s:write shutdown ringtone reg\n", __func__);
		smartpakit_i2c_hw_reset(i2c_priv);
		if (smartpakit_i2c_set_shutdown_ringtone(i2c_priv, false) < 0)
			hwlog_err("%s: set shutdown ringtone failed\n", __func__); 
	}
}
...
#if (defined(CONFIG_HIBERNATION) || defined(CONFIG_HUAWEI_ARMPC_PLATFORM))
	smartpakit_i2c_ctl_irq(i2c_priv->irq_handler, DISABLE_SMARTPA_IRQ);
#endif
	if ((i2c_priv->regmap_cfg != NULL) &&
		(i2c_priv->regmap_cfg->regmap != NULL) &&
