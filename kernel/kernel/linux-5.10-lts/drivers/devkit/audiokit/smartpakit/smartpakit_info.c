...
static void smartpakit_get_simpa_pa_id_info(struct smartpakit_priv *pakit_priv)
{
	...
	for (j = 0; j < id_match_check_num; j++) {
		hwlog_info("%s: id_match:%d, id_status:%s, chip_model:%s\n",
			__func__, j, pa_id_match_check[j].id_status,
			pa_id_match_check[j].chip_model);
		smartpakit_append_info("id_match:%d,id_status:%s,chip_model:%s\n",
			j, pa_id_match_check[j].id_status,
			pa_id_match_check[j].chip_model);
	}
}
...
static int smartpakit_info_trigger_irq(struct smartpakit_priv *kit)
{
	...
	if ((reg_ctl_params.params_num < 1) || ((kit->pa_num <= INT_MAX) && (index >= (int)kit->pa_num))) {
...
static int smartpakit_info_reg_read(struct smartpakit_priv *kit)
{
	...
	if ((reg_ctl_params.params_num < KIT_REG_R_PARAM_NUM_MIN) ||
		((kit->pa_num <= INT_MAX) && (index >= (int)kit->pa_num))) {
