...
int lcd_kit_bridge_vdd_power_ctrl(int enable)
{
	struct lcd_kit_adapt_ops *adapt_ops = NULL;
	...
	if (power_hdl->lcd_bridge_vdd.buf == NULL) {
		LCD_KIT_ERR("can not get lcd bridge vdd!\n");
		return LCD_KIT_FAIL;
	}
	switch (power_hdl->lcd_bridge_vdd.buf[0]) {
	case GPIO_MODE:
		lcd_kit_gpio_power_ctrl(enable, LCD_KIT_BRIDGE_VDD, adapt_ops);
		break;
	case REGULATOR_MODE:
		lcd_kit_regulator_power_ctrl(enable, LCD_KIT_BRIDGE_VDD, adapt_ops);
		break;
	case NONE_MODE:
...
int lcd_kit_bridge_rst_power_ctrl(int enable)
{
	struct lcd_kit_adapt_ops *adapt_ops = NULL;
	...
	switch (power_hdl->lcd_bridge_rst.buf[0]) {
	case GPIO_MODE:
		if (enable == LCD_BRIDGE_RST_HIGH) {
			if (adapt_ops->gpio_enable)
				adapt_ops->gpio_enable(LCD_KIT_BRIDGE_RST);
		} else {
			if (adapt_ops->gpio_disable)
				adapt_ops->gpio_disable(LCD_KIT_BRIDGE_RST);
		}
		break;
	default:
		LCD_KIT_ERR("not support mode\n");
...
static int lcd_kit_on_cmds(void *hld)
{
	...
	if (adapt_ops->mipi_tx) {
		...
		if (common_info->flash_protect_support)
			ret = adapt_ops->mipi_tx(hld, &common_info->flash_protect_cmds);
		if (ret)
	...
	uint32_t delay)
{
	...
	case EVENT_BRIDGE_RST:
		ret = lcd_kit_bridge_rst_power_ctrl(data);
		break;
	case EVENT_BRIDGE_VDD:
		ret = lcd_kit_bridge_vdd_power_ctrl(data);
		break;
	case EVENT_BRIDGE:
		ret = lcd_kit_bridge_ctrl_event(data);
		break;
	...
	case BL_BIG_ENDIAN:
		if (common_info->backlight.bl_max <= 0xFF) {
			...
			common_info->backlight.bl_cmd.cmds[0].payload[1] =
				((bl_level >> 8) & 0xFF) | common_info->backlight.write_offset;
...
static void lcd_kit_parse_power_bridge_rst(const char *np)
{
	/* lcd bridge reset */
	if (power_hdl->lcd_bridge_rst.buf != NULL)
		return;
	lcd_kit_parse_array(np, "lcd-kit,lcd-bridge-rst",
...
static void lcd_kit_parse_power_bridge_vdd(const char *np)
{
	/* lcd bridge vdd */
	if (power_hdl->lcd_bridge_vdd.buf != NULL)
		return;
	lcd_kit_parse_array(np, "lcd-kit,lcd-bridge-vdd",
...
static int lcd_kit_parse_power(const char *np)
{
	...
	lcd_kit_parse_power_bridge_rst(np);
	lcd_kit_parse_power_bridge_vdd(np);
...
static int lcd_kit_panel_parse_backlight(const char *np)
{
	/* backlight */
	...
	lcd_kit_parse_get_u32_default(np, "lcd-kit,backlight-write-offset",
	...
	lcd_kit_parse_get_u32_default(np, "lcd-kit,panel-bl-ext-flag",
...
static int lcd_kit_parse_flash_protect(const char *np)
{
	/* flash protect */
	lcd_kit_parse_get_u32_default(np, "lcd-kit,flash-protect-support",
		&common_info->flash_protect_support, 0);
	if (common_info->flash_protect_support)
		lcd_kit_parse_dcs_cmds(np, "lcd-kit,flash-protect-cmds",
			"lcd-kit,flash-protect-cmds-state",
			&common_info->flash_protect_cmds);
	return LCD_KIT_OK;
}
...
int lcd_kit_common_init(const char *np)
{
	...
	lcd_kit_parse_flash_protect(np);
