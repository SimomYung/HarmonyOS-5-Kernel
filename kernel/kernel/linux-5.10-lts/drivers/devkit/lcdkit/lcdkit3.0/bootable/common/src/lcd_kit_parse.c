...
static void lcd_kit_free_pcmds_cmds_payload(struct lcd_kit_dsi_panel_cmds *pcmds,
	int cunt)
{
	int i;
	...
	if (cunt <= 0)
		return;
	for (i = 0; i < cunt; i++)
		free(pcmds->cmds[i].payload);
	free(pcmds->cmds);
}

static int lcd_kit_parse_buf_to_cmd(char *buf, int cnt, int *len,
	struct lcd_kit_dsi_panel_cmds *pcmds)
{
	...
	for (i = 0; i < cnt; i++) {
		...
		*len -= (int)sizeof(struct lcd_kit_dsi_cmd_desc_header);
		...
		if (!pcmds->cmds[i].payload) {
			lcd_kit_free_pcmds_cmds_payload(pcmds, i);
			return LCD_KIT_FAIL;
		}
	...
	struct lcd_kit_dsi_panel_cmds *pcmds)
{
	...
	pcmds->cmds = (struct lcd_kit_dsi_cmd_desc *)mem_alloc(cnt *
		sizeof(struct lcd_kit_dsi_cmd_desc));
	if (!pcmds->cmds)
		return LCD_KIT_FAIL;
	pcmds->cmd_cnt = cnt;
	ret = lcd_kit_parse_buf_to_cmd(buf, cnt, len, pcmds);
	if (ret < 0)
		return LCD_KIT_FAIL;
	return LCD_KIT_OK;
}
	...
	struct lcd_kit_array_data *out)
{
	...
	ret = memcpy_s((void *)buf, blen * sizeof(u32), (const void *)data, blen * sizeof(u32));
	if (ret != 0) {
		free(buf);
		return LCD_KIT_FAIL;
	}
...
static int lcd_kit_parse_buf_to_arrays(int *buf, int cnt, u32 num,
	struct lcd_kit_arrays_data *out)
{
	int i;
	int *bp = NULL;
	...
	struct lcd_kit_arrays_data *out, u32 num)
{
	...
	ret = lcd_kit_parse_buf_to_arrays(buf, cnt, num, out);
	if (ret != 0) {
		free(buf);
		return LCD_KIT_FAIL;
	}
	...
	const char *propertyname, struct lcd_kit_arrays_data *out, u32 num)
{
	...
	if (adpat_ops->get_dts_data_by_property)
		adpat_ops->get_dts_data_by_property(fdt, nodeoffset, propertyname, &data, &blen);
	if ((blen <= 0) || (blen > MAX_BLEN_LEN)) {
		LCD_KIT_ERR("blen <= 0 or > MAX_BLEN_LEN!\n");
		return LCD_KIT_FAIL;
	}
