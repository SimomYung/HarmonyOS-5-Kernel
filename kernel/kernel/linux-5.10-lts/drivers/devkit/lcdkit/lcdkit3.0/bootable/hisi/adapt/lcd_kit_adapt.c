	...
	const char *propertyname, char *out_str, unsigned int length)
{
	...
	ret = memcpy_s(out_str, (unsigned int)length, (char *)property->data, (unsigned int)length);
	if (ret != 0) {
...
static int lcd_kit_fwdtb_change_bl_dts(const char *name)
{
	...
	if (!disp_info->bl_name) {
		LCD_KIT_ERR("alloc error\n");
		return LCD_KIT_FAIL;
	}
	ret = memcpy_s(disp_info->bl_name, length, name, length - 1);
	if (ret != LCD_KIT_OK) {
		LCD_KIT_ERR("memcpy_s error\n");
		free(disp_info->bl_name);
		disp_info->bl_name = NULL;
		return LCD_KIT_FAIL;
	}
	fdt_ops = get_operators(FDT_MODULE_NAME_STR);
	if (fdt_ops == NULL) {
...
static int lcd_kit_fwdtb_change_bias_dts(const char *name)
{
	...
	if (!disp_info->bias_name) {
		LCD_KIT_ERR("alloc error\n");
		return LCD_KIT_FAIL;
	}
	ret = memcpy_s(disp_info->bias_name, length, name, length - 1);
	if (ret != LCD_KIT_OK) {
		LCD_KIT_ERR("memcpy_s error\n");
		free(disp_info->bias_name);
		disp_info->bias_name = NULL;
		return LCD_KIT_FAIL;
	}
	fdt_ops = get_operators(FDT_MODULE_NAME_STR);
	if (fdt_ops == NULL) {
