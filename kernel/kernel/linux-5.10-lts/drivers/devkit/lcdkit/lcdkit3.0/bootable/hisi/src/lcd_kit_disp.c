	...
	struct hisi_fb_data_type *hisifd)
{
	...
	case LCD_INIT_MIPI_LP_SEND_SEQUENCE:
		...
		if (disp_info->ddic_type == NEED_READ_GAMMA)
			lcd_kit_read_local_hbm_gamma(pinfo, hisifd);
#endif
		/* read project id */
	...
	struct hisi_fb_data_type *hisifd, uint32_t bl_level)
{
	...
	if (!hisifd->panel_info) {
		LCD_KIT_ERR("panel info in hisifd is NULL!\n");
		return LCD_KIT_FAIL;
	}
	pinfo = hisifd->panel_info;
...
void add_set_property_str(void)
{
#ifdef FASTBOOT_FW_DTB_ENABLE
	struct fdt_operators *fdt_ops = NULL;
#endif
	...
	fdt_ops->register_update_dts_func(lcd_add_fwdtb_set_property_str);
#endif
	LCD_KIT_INFO("-\n");
}
...
static int lcd_kit_init(struct system_table *systable)
{
	...
	if (lcd_type_ops->get_lcd_type() == LCD_KIT) {
		...
		disp_info->lcd_id = lcd_kit_check_lcd_id(disp_info->product_id,
			disp_info->lcd_id);
