...
uint32_t g_lcd_kit_iovcc;
uint32_t g_lcd_kit_vdd;
uint32_t g_lcd_kit_bridge_vdd;
/*
 * power type
 */
...
static struct lcd_kit_vci_type lcd_kit_ldo_map[] = {
	...
	{ LDO_9, DEVICE_SDIO },
	{ BUCK_0, DEVICE_LCDDDIC },
	{ LDO_13, DEVICE_LCDDIG },
	{ LDO_4, DEVICE_LCDIO },
	...
	{ LDO_12, DEVICE_SIM_JTAG2 },
	{ LDO_3, DEVICE_LCDDIG2 },
	{ LDO_48, DEVICE_LCDDIG3 },
...
void lcd_kit_gpio_tx(uint32_t type, uint32_t op)
{
	...
	case LCD_KIT_BRIDGE_RST:
		g_lcd_kit_gpio = power_hdl->lcd_bridge_rst.buf[POWER_NUMBER];
		break;
	case LCD_KIT_BRIDGE_VDD:
		g_lcd_kit_gpio = power_hdl->lcd_bridge_vdd.buf[POWER_NUMBER];
		break;
	default:
...
int lcd_kit_pmu_ctrl(uint32_t type, uint32_t enable)
{
	...
	case LCD_KIT_IOVCC:
		if (enable)
			pmu_ops->pmu_enable(g_lcd_kit_iovcc);
		else
			pmu_ops->pmu_disable(g_lcd_kit_iovcc);
		break;
	case LCD_KIT_VDD:
		if (enable)
	...
	case LCD_KIT_BRIDGE_VDD:
		if (enable)
			pmu_ops->pmu_enable(g_lcd_kit_bridge_vdd);
		else
			pmu_ops->pmu_disable(g_lcd_kit_bridge_vdd);
		break;
	default:
		break;
	}
...
static void lcd_kit_iovcc_init(struct pmu_operators *pmu_ops)
{
	if ((power_hdl->lcd_iovcc.buf) &&
		...
		if (power_hdl->lcd_iovcc.buf[POWER_NUMBER] > 0) {
			...
			pmu_ops->pmu_set_voltage(g_lcd_kit_iovcc,
...
static void lcd_kit_bridge_vdd_init(struct pmu_operators *pmu_ops)
{
	if ((power_hdl->lcd_bridge_vdd.buf) &&
		(power_hdl->lcd_bridge_vdd.buf[POWER_MODE] == REGULATOR_MODE)) {
		/* judge bridge vdd is valid */
		if (power_hdl->lcd_bridge_vdd.buf[POWER_NUMBER] > 0) {
			g_lcd_kit_bridge_vdd = lcd_ldo_sel(
				power_hdl->lcd_bridge_vdd.buf[POWER_NUMBER]);
			pmu_ops = get_operators(PMU_MODULE_NAME_STR);
			if (!pmu_ops) {
				LCD_KIT_ERR("can not get pmu_ops!\n");
				return;
			}
			pmu_ops->pmu_set_voltage(g_lcd_kit_bridge_vdd,
				power_hdl->lcd_bridge_vdd.buf[POWER_VOLTAGE]);
		}
	}
}
...
int lcd_kit_power_init(void)
{
	...
	lcd_kit_bridge_vdd_init(pmu_ops);
	return LCD_KIT_OK;
}
