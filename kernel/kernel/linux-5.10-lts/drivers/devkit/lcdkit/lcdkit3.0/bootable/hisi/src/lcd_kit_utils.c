...
#define FWDTB_SET_PROJECT_ID_SUPPORT 2
...
static void lcd_kit_dsc_pred_enable_config(const char *np, struct hisi_panel_info *pinfo)
{
	...
	lcd_kit_parse_get_u32_default(np, "lcd-kit,vesa-dsc-blk-pred-en-support",
...
#if defined(FASTBOOT_LOCAL_HBM)
int local_hbm_gamma_read_rx_cmd(struct hisi_fb_data_type *hisifd,
	uint8_t *hbm_gamma, int rgb_size, bool is_ext)
{
	if (is_ext)
		lcd_kit_dsi_cmds_rx(hisifd, hbm_gamma,
			rgb_size, &disp_info->hbm_ext.hbm_90_hz_gamma_read_ext_cmds);
	else
		lcd_kit_dsi_cmds_rx(hisifd, hbm_gamma,
			rgb_size, &disp_info->hbm_ext.hbm_90_hz_gamma_read_cmds);
	return LCD_KIT_OK;
}

static bool is_max_grayscale_register(int index)
{
	// 3 7 11 is 255 grayscale register index
	if (index == 3 || index == 7 || index == 11)
		return true;
	return false;
}

static bool gamma_read_flag = false;

int lcd_kit_read_local_hbm_gamma(struct hisi_panel_info *pinfo,
	struct hisi_fb_data_type *hisifd)
{
	int i;
	int cnt = 0;
	uint8_t addr_start;
	uint8_t hbm_gamma[RGB_SIZE_NORMAL];
	uint8_t hbm_gamma_ext[RGB_SIZE_EXT];
	uint8_t hbm_gamma_sum[RGB_SIZE];

	if (gamma_read_flag)
		return LCD_KIT_OK;

	addr_start = disp_info->hbm_ext.hbm_90_hz_gamma_read_cmds.cmds[GAMMA_CMD_INX].payload[0];

	for (i = 0; i < ADDR_NUM; i++) {
		if (is_max_grayscale_register(i)) {
			local_hbm_gamma_read_rx_cmd(hisifd, hbm_gamma_ext,
				RGB_SIZE_EXT, true);
			memcpy_s(hbm_gamma_sum + cnt, RGB_SIZE_EXT, hbm_gamma_ext,
				RGB_SIZE_EXT);
			cnt += RGB_SIZE_EXT;
		} else {
			local_hbm_gamma_read_rx_cmd(hisifd, hbm_gamma,
				RGB_SIZE_NORMAL, false);
			memcpy_s(hbm_gamma_sum + cnt, RGB_SIZE_NORMAL, hbm_gamma,
				RGB_SIZE_NORMAL);
			cnt += RGB_SIZE_NORMAL;
		}
		disp_info->hbm_ext.hbm_90_hz_gamma_read_cmds.cmds[GAMMA_CMD_INX].payload[0]++;
		disp_info->hbm_ext.hbm_90_hz_gamma_read_ext_cmds.cmds[GAMMA_CMD_INX].payload[0]++;
	}

	for (i = 0; i < RGB_SIZE; i++) {
		disp_info->hbm.hbm_gamma_local[i] = hbm_gamma_sum[i];
		LCD_KIT_INFO("Local_hbm_gamma[%d] = %d\n", i, disp_info->hbm.hbm_gamma_local[i]);
	}
	pinfo->hbm_gamma.buf = disp_info->hbm.hbm_gamma_local;
	pinfo->hbm_gamma.cnt = RGB_SIZE;
	gamma_read_flag = true;
	disp_info->hbm_ext.hbm_90_hz_gamma_read_cmds.cmds[GAMMA_CMD_INX].payload[0] = addr_start;
	disp_info->hbm_ext.hbm_90_hz_gamma_read_ext_cmds.cmds[GAMMA_CMD_INX].payload[0] = addr_start;
...
uint32_t lcd_kit_check_lcd_id(uint32_t product_id, uint32_t lcd_id)
{
	int def_lcd_id = 0x1A; // def_lcd_id is 0x1A
	unsigned int i;

	if (PUBLIC_CFG->product_type != LCD_DUAL_PENEL_TYPE)
		return lcd_id;

	/* find lcd_id from panel map */
	...
	if (LCD_ACTIVE_PANEL == LCD_EXT_PANEL) {
		(void)get_dts_value(DTS_COMP_LCD_PANEL_TYPE, "ext_def_lcd_id", &def_lcd_id);
	} else {
		(void)get_dts_value(DTS_COMP_LCD_PANEL_TYPE, "def_lcd_id", &def_lcd_id);
	}

	LCD_KIT_ERR("return def_lcd_id = 0x%x\n", def_lcd_id);
	return def_lcd_id;
}
...
#if defined(CONFIG_FULL_AOD_EFFECT)
static void lcd_kit_parse_anm_dimming(const char *np)
{
	lcd_kit_parse_get_u32_default(np,
		"lcd-kit,anm-dim-support",
		&disp_info->aod.anm_dim_support, 0);
	if (disp_info->aod.anm_dim_support) {
		lcd_kit_parse_dcs_cmds(np,
			"lcd-kit,panel-enter-anm-dimming",
			"lcd-kit,panel-enter-anm-dimming-state",
			&(disp_info->aod.anm_dim_enter_cmds));
		lcd_kit_parse_dcs_cmds(np,
			"lcd-kit,panel-exit-anm-dimming",
			"lcd-kit,panel-exit-anm-dimming-state",
			&(disp_info->aod.anm_dim_exit_cmds));
		lcd_kit_parse_dcs_cmds(np,
			"lcd-kit,fps-to-90-cmds",
			"lcd-kit,fps-to-90-cmds-state",
			&(disp_info->aod.fps_to_90_cmds));
	}
}
#endif

#if defined(FASTBOOT_LOCAL_HBM)
static void lcd_kit_parse_gamma_read_cmd(const char *np)
{
	...
	if (disp_info->hbm.local_hbm_support) {
		lcd_kit_parse_dcs_cmds(np,
			...
			&disp_info->hbm_ext.hbm_90_hz_gamma_read_cmds);
		lcd_kit_parse_dcs_cmds(np,
			"lcd-kit,hbm-90-hz-gamma-read-ext-cmds",
			"lcd-kit,hbm-90-hz-gamma-read-cmds-state",
			&disp_info->hbm_ext.hbm_90_hz_gamma_read_ext_cmds);
		lcd_kit_parse_get_u32_default(np, "lcd-kit,local-hbm-ddic",
			&disp_info->ddic_type, 0);
		LCD_KIT_INFO("ddic_type = %d\n", disp_info->ddic_type);
	}
}
#endif
...
static void get_init_panel_version(const char *np)
{
...
#if defined(FASTBOOT_PLAT_BALTIMORE) || defined(FASTBOOT_LOCAL_HBM)
...
static void lcd_kit_free_flash_erase_check_arr(u32 idx)
{
	u32 i;
	...
	if (idx <= 0)
		return;
	for (i = 0; i < idx; i++) {
		free(disp_info->flash_erase.read_check_arr[i].buf);
		disp_info->flash_erase.read_check_arr[i].buf = NULL;
	}
}
...
static void lcd_kit_parse_flash_erase_check(const char *compatible)
{
	...
	if (!disp_info->flash_erase.read_check_cmds) {
		...
		if (!disp_info->flash_erase.read_check_arr) {
			LCD_KIT_ERR("alloc fail\n");
			free(disp_info->flash_erase.read_check_cmds);
			disp_info->flash_erase.read_check_cmds = NULL;
			return;
		}
		for (i = 0; i < value; i++) {
			...
			if (!disp_info->flash_erase.read_check_arr[i].buf) {
				...
				free(disp_info->flash_erase.read_check_cmds);
				disp_info->flash_erase.read_check_cmds = NULL;
				lcd_kit_free_flash_erase_check_arr(i);
				free(disp_info->flash_erase.read_check_arr);
				disp_info->flash_erase.read_check_arr = NULL;
				return;
			}
...
static void lcd_kit_parse_pwr_off_time_optimize(const char *compatible)
{
	int i;
	struct lcd_kit_array_data *pevent = NULL;

	lcd_kit_parse_get_u32_default(compatible, "lcd-kit,pwr-off-time-optimize-support",
		&disp_info->pwr_off_optimize_info.support, 0);
	if (disp_info->pwr_off_optimize_info.support) {
		lcd_kit_parse_get_u32_default(compatible, "lcd-kit,pwr-off-time-work-delay",
		...
		for (i = 0; i < power_seq->panel_off_hs_seq.cnt; i++) {
			...
			if (pevent->buf[0] == EVENT_MIPI)
				pevent->buf[2] = disp_info->pwr_off_optimize_info.delay_time;
...
static int lcd_kit_parse_disp_info(const char *np)
{
...
#if defined(CONFIG_FULL_AOD_EFFECT)
	lcd_kit_parse_anm_dimming(np);
#endif
#if defined(FASTBOOT_LOCAL_HBM)
	lcd_kit_parse_gamma_read_cmd(np);
#endif
	...
	lcd_kit_parse_pwr_off_time_optimize(np);
	return LCD_KIT_OK;
}
...
static void fwdtb_set_property_str_hifastboot(const char *path, const char *dts_name,
	const char *str)
{
#if defined(FASTBOOT_DISPLAY_LOGO_DPUV501) || defined(FASTBOOT_DISPLAY_LOGO_DPUV330)
	return;
...
void lcd_kit_fwdtb_set_property_str(const char *path, const char *dts_name, const char *str)
{
#if defined(FASTBOOT_DISPLAY_LOGO_DPUV501) || defined(FASTBOOT_DISPLAY_LOGO_DPUV330)
#else
	fwdtb_set_property_str_hifastboot(path, dts_name, str);
#endif
}
 ...
 */
#if defined(CONFIG_FULL_AOD_EFFECT)
static void lcd_kit_anm_dim_init(struct hisi_panel_info *pinfo)
{
	pinfo->anm_dim_support = disp_info->aod.anm_dim_support;
	if (!pinfo->anm_dim_support) {
		LCD_KIT_INFO("full aod_dim_support is not support\n");
		return;
	}

	/* aod dimming on */
	pinfo->anm_enter_dim_len = disp_info->aod.anm_dim_enter_cmds.cmd_cnt;
	lcd_kit_aod_cmd_init(&(pinfo->anm_enter_dim_cmds),
		&(disp_info->aod.anm_dim_enter_cmds));

	/* aod dimming off */
	pinfo->anm_exit_dim_len = disp_info->aod.anm_dim_exit_cmds.cmd_cnt;
	lcd_kit_aod_cmd_init(&(pinfo->anm_exit_dim_cmds),
		&(disp_info->aod.anm_dim_exit_cmds));

	/* fps to 90 cmds */
	pinfo->fps_to_90_cmds_len = disp_info->aod.fps_to_90_cmds.cmd_cnt;
	lcd_kit_aod_cmd_init(&(pinfo->fps_to_90_cmds),
		&(disp_info->aod.fps_to_90_cmds));
}
#endif
...
#define MAX_HOTAINFO_LEN 2048
static void lcd_kit_parse_bl_boot(const char *np,
	struct hisi_panel_info *pinfo)
{
	uint32_t bl_def_night_upd = 0;
	static bool night_update = false;
	struct oeminfo_operators *oeminfo_ops = NULL;
	int ret = LCD_KIT_OK;
	char buf[MAX_HOTAINFO_LEN] = {0};

	lcd_kit_parse_get_u32_default(np, "lcd-kit,panel-bl-boot",
		&pinfo->bl_boot, 0);
	lcd_kit_parse_get_u32_default(np, "lcd-kit,panel-bl-night-upd", &bl_def_night_upd, 0);
    if (bl_def_night_upd == 0) {
        LCD_KIT_INFO("bl def night update is 0\n");
        return;
    }
    if (night_update == true) {
        pinfo->bl_boot = bl_def_night_upd;
        LCD_KIT_INFO("in night update\n");
        return;
    }
	oeminfo_ops = get_operators(OEMINFO_MODULE_NAME_STR);
	if (oeminfo_ops == NULL) {
		LCD_KIT_INFO("cannot get oeminfo_ops\n");
		return;
	}
	ret = oeminfo_ops->get_oeminfo(OEMINFO_FIX_HOTA_INFO, sizeof(buf), buf);
	if (ret != 0) {
		LCD_KIT_INFO("get_oeminfo fail\n");
		return;
	}
	if (strstr(buf, "night_update") != 0) {
		pinfo->bl_boot = bl_def_night_upd;
		night_update = true;
		LCD_KIT_INFO("bl_boot is %u\n", pinfo->bl_boot);
	}
}
	...
	struct hisi_panel_info *pinfo)
{
	...
	/* panel info */
	lcd_kit_parse_get_u32_default(np, "lcd-kit,reload-support",  &pinfo->reload_support, 0);
	...
	/* sec panel hbm*/
	u32 hbm_backlihgt_support = 0;
	lcd_kit_parse_get_u32_default(np, "lcd-kit,hbm-backlight-support", &hbm_backlihgt_support, 0);
	if (hbm_backlihgt_support && PUBLIC_CFG->product_type == LCD_DUAL_PENEL_TYPE) {
		u32 temp_bl_max = pinfo->bl_max;
		lcd_kit_parse_get_u32_default(np, "lcd-kit,normal-backlight-max", &pinfo->bl_max,
			temp_bl_max);
	}
	lcd_kit_parse_bl_boot(np, pinfo);
	...
	lcd_kit_parse_get_u32_default(np, "lcd-kit,panel-index",
		&pinfo->panel_index, 0);
...
#endif
#if defined(FASTBOOT_LOCAL_HBM)
	pinfo->ddic_type = disp_info->ddic_type;
#endif
...
#if defined(CONFIG_FULL_AOD_EFFECT)
		lcd_kit_anm_dim_init(pinfo);
#endif
	}
}

static void lcd_kit_backlight_offset_init(struct hisi_panel_info *pinfo)
{
	if (!pinfo) {
		LCD_KIT_ERR("pinfo is null\n");
		return;
	...
	if (pinfo->aod_support) {
		pinfo->backlight_order = common_info->backlight.order;
		pinfo->backlight_write_offset = common_info->backlight.write_offset;
	}
}
...
int lcd_kit_utils_init(struct hisi_panel_info *pinfo)
{
	...
	/* The backlight offset needs to be configured for the XHS */
	lcd_kit_backlight_offset_init(pinfo);
	...
	lcd_kit_dsc_pred_enable_config(disp_info->lcd_name, pinfo);
...
static int lcd_kit_fwdtb_set_project_id(void)
{
	LCD_KIT_INFO("+\n");
	if (disp_info->project_id.support == FWDTB_SET_PROJECT_ID_SUPPORT) {
		if (((sizeof(disp_info->project_id.id)/sizeof(disp_info->project_id.id[0])) != 0) &&
			(lcd_kit_check_project_id() == LCD_KIT_OK)) {
			LCD_KIT_INFO("project id is %s\n", disp_info->project_id.id);
			lcd_kit_fwdtb_set_property_str(DTS_LCD_PANEL_TYPE, "project_id",
				disp_info->project_id.id);
		} else {
			LCD_KIT_INFO("disp_info->project_id.default_project_id = %s\n",
				disp_info->project_id.default_project_id);
...
int lcd_add_fwdtb_set_property_str(void)
{
	LCD_KIT_INFO("+\n");
	/* fwdtb set project id to project_id in dts*/
	lcd_kit_fwdtb_set_project_id();
...
static int lcd_add_status_disabled_hifastboot(void)
{
	...
	for (i = 0; i < ARRAY_SIZE(lcd_kit_map); ++i) {
		...
		if (PUBLIC_CFG->product_type == LCD_DUAL_PANEL_SIM_DISPLAY_TYPE ||
			PUBLIC_CFG->product_type == LCD_FOLDER_TYPE ||
