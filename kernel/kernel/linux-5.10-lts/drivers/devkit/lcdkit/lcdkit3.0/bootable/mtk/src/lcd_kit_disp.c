...
static void lcd_kit_poweric_detect()
{
	...
	if (ret) {
		...
		if (common_ops->panel_off_hs) {
			if (common_ops->panel_off_hs(NULL) != LCD_KIT_OK)
				LCD_KIT_ERR("panel off hs fail\n");
		}
		if (common_ops->panel_power_on) {
			if (common_ops->panel_power_on(NULL) != LCD_KIT_OK)
				LCD_KIT_ERR("panel power on fail\n");
		}
		retry_times++;
		return;
	}
...
static void lcm_init(void)
{
	...
	if (common_ops->panel_power_on) {
		if (common_ops->panel_power_on((void *)NULL) != LCD_KIT_OK)
			LCD_KIT_ERR("panel power on fail\n");
