...
#define LCD_CHECK_AND_NO_RETVAL(condition, level, msg, ...) \
    do { \
        if (condition) { \
            LCD_KIT_##level(msg, ##__VA_ARGS__); \
            return; \
        } \
    } while (0)

#define LCD_CHECK_AND_RETURN(condition, ret, level, msg, ...) \
    do { \
        if (condition) { \
            LCD_KIT_##level(msg, ##__VA_ARGS__); \
            return ret; \
...
#define POWER_OFF_COMMAND 0
#define POWER_FORCE_OFF_COMMAND 2

#define SEQ_NUM 3

/* check thead period */
#define TEMPER_THREAD_TIME_PERIOD 5000
#define TEMPER_ALPHA_CMDS_NUM_MAX 40
/* pcd errflag */
...
/* async_ctrl_mode */
enum lcd_kit_async_ctrl_mode {
	LCD_KIT_DSI_SYNC_MODE,
	LCD_KIT_DSI_ASYNC_MODE,
};

/* refresh_flag */
enum lcd_kit_refresh_flag {
	LCD_KIT_DSI_REFRSH_NONE,
	LCD_KIT_DSI_REFRESH_USED,
};
...
enum lcd_kit_power_ldo_read {
	IS_ENABLED = 1,
	READ_LDO_USE_COUNT = 2,
};
#endif
...
enum lcd_kit_power_type {
	...
	LCD_KIT_MIPI_SWITCH,
	LCD_KIT_MIPI_SWITCH1,
	LCD_KIT_MIPI_SWITCH2,
	LCD_KIT_PMIC_CTRL,
	LCD_KIT_PMIC_RESET,
	LCD_KIT_PMIC_INIT,
	...
	LCD_KIT_POWER_IC_SWITCH,
	LCD_KIT_INTEG_SWITCH,
	LCD_KIT_INTEG_EN,
	LCD_KIT_BRIDGE_RST,
	LCD_KIT_BRIDGE_VDD,
};
...
enum lcd_kit_event {
	...
	EVENT_BOOST_CTRL,
	EVENT_BOOST_BYPASS,
	EVENT_COLOR_CALIB = 24,
	EVENT_SET_POWER = 25,
	EVENT_AOD_EXT_CMD = 26,
	EVENT_DVDD_AVS = 27,
	EVENT_PPC_ON = 28,
	EVENT_SVM_OTA = 29,
	EVENT_MIPI_SWITCH1 = 30,
	EVENT_MIPI_SWITCH2 = 31,
	EVENT_DUL_DSI = 32,
	EVENT_DSC_SWITCH = 33,
	EVENT_ELCTRL_V2 = 34,
	EVENT_DIEID_FORWAR = 35,
	EVENT_POWERIC_SWITCH = 36,
	EVENT_INTEG_SWITCH = 37,
	EVENT_INTEG_EN = 38,
	EVENT_BAND_SWITCH = 39,
	EVENT_BRIDGE_RST = 97,
	EVENT_BRIDGE_VDD = 98,
	EVENT_BRIDGE = 99,
...
enum backlight_status {
	LCD_KIT_BACKLIGHT_DEFAULT = 0,
	LCD_KIT_BACKLIGHT_HBM,
	LCD_KIT_BACKLIGHT_APL,
};
...
/* backlight grayscale mode */
enum {
	GRAYSCALE_MODE_DEF = 0,
	GRAYSCALE_MODE1 = 1,
	GRAYSCALE_MODE2 = 2,
	GRAYSCALE_MODE3 = 3,
	GRAYSCALE_MODE4 = 4,
...
struct lcd_kit_dsi_panel_cmds {
	...
	int link_state;
	u32 flags;
	struct lcd_kit_async_desc async_param;
};
...
struct lcd_kit_hbm {
	...
	/* local hbm cmd */
	u32 local_hbm_ddic;
	u32 local_hbm_fake_off_delay_support;
...
struct lcd_kit_grayscale_backlight {
	u32 support;
	u32 last_grayscale_mode;
	struct lcd_kit_array_data grayscale_mode1_backlightList;
	struct lcd_kit_array_data grayscale_mode2_backlightList;
	struct lcd_kit_array_data grayscale_mode3_backlightList;
	struct lcd_kit_array_data grayscale_mode4_backlightList;
	struct lcd_kit_array_data grayscale_mode5_backlightList;
	struct lcd_kit_dsi_panel_cmds panel_grayscale_mode1_cmds;
	struct lcd_kit_dsi_panel_cmds panel_grayscale_mode2_cmds;
	struct lcd_kit_dsi_panel_cmds panel_grayscale_mode3_cmds;
	struct lcd_kit_dsi_panel_cmds panel_grayscale_mode4_cmds;
	struct lcd_kit_dsi_panel_cmds panel_grayscale_mode5_cmds;
	struct lcd_kit_dsi_panel_cmds panel_grayscale_modedef_cmds;
};
...
struct lcd_kit_adapt_ops {
	...
	int (*mipi_rx)(int panel_id, void *hld, u8 *out, int out_len,
		struct lcd_kit_dsi_panel_cmds *cmds);
	int (*mipi_dual_rx)(int panel_id, void *hld, u8 *out0, u8 *out1, int out_len,
		struct lcd_kit_dsi_panel_cmds *cmds);
	...
	int (*write_nv_info)(uint32_t panel_id, int nv_num, int valid_size,
		char *info, uint32_t info_len);
#ifdef CONFIG_LCD_KIT_HYBRID
	int (*regulator_read)(int panel_id, uint32_t type, uint32_t read_type);
#endif
};
...
enum lcd_kit_get_temper_mode {
	LCD_KIT_THERMAL_PA_S = 0,
	LCD_KIT_THERMAL_SHELL_FRAME,
	LCD_KIT_THERMAL_SHELL_FRONT,
	LCD_KIT_THERMAL_MAX
};

struct lcd_kit_temper_thread {
	u32 enable;
	u32 temper_lhbm_flag;
	u32 dark_enable;
	u32 isAhTemper;
	int temperMax;
	int temperMin;
	...
	struct lcd_kit_array_data temperature_value;
	struct lcd_kit_dsi_panel_cmds temper_enter_cmd;
	struct lcd_kit_dsi_panel_cmds temper_exit_cmd;
	struct lcd_kit_dsi_panel_cmds temper_alpha_enter_cmds[TEMPER_ALPHA_CMDS_NUM_MAX];
	struct lcd_kit_dsi_panel_cmds temper_alpha_exit_cmds;
};

struct lcd_kit_multi_region_info {
    u32 ys; /* big region y start */
    u32 ye; /* big region y end */
    bool is_in; /* if in ABAB scene */
    bool cal_ip; /* if need cal ip */
};

struct lcd_kit_part_refresh {
	u32 support;
	u32 order;
	u32 ys; /* y start */
	u32 ye; /* y end */
	u32 ip1_offset_param;
	u32 ip2_offset_param;
    struct lcd_kit_multi_region_info multi_region;
	struct lcd_kit_dsi_panel_cmds part_refresh_enter_cmd;
	struct lcd_kit_dsi_panel_cmds part_refresh_exit_cmd;
	struct lcd_kit_dsi_panel_cmds part_refresh_cmd;
	struct lcd_kit_dsi_panel_cmds part_push_cmd;
	struct lcd_kit_dsi_panel_cmds part_refresh_reg_cmd;
	struct lcd_kit_dsi_panel_cmds bureau_brush_cmd;

	/* enter restrictions */
	u32 poweron_delay_frames;
	u32 applist_brightness_dbv;
	u32 non_applist_brightness_dbv;
	u32 temp_compensation_support;
	u32 minimum_limit_rows;
};

enum lcd_kit_ddic_cmd_type {
	/* Depth Color Calibration */
	LCD_KIT_DDIC_CMD_TYPE_C_CALIB = 0,
	LCD_KIT_DDIC_CMD_TYPE_MAX,
};

enum {
	LCD_KIT_DDIC_CMD_OP_CLOSE = 0,
	LCD_KIT_DDIC_CMD_OP_OPEN = 1,
};

struct lcd_kit_event_table {
	int event;
	int (*func_ptr)(int panel_id, void *hld,
...
struct lcd_kit_common_ops {
	...
	int (*panel_mipi_switch_on_lp)(int panel_id, void *hld);
	int (*panel_doze_lp)(int panel_id, void *hld);
	int (*panel_doze_suspend)(int panel_id, void *hld);
	...
	int (*enter_hbm)(int panel_id, void *hld, u32 backlight_status);
	int (*exit_hbm)(int panel_id, void *hld, u32 backlight_status);
	...
	int (*set_ddic_cmd)(int panel_id, int type, int op);
	int (*set_gray_backlight_cmd)(int panel_id, void *hld, u32 level);
};
...
struct lcd_kit_color_calib {
	u32 support;
	u32 need_ddic_c_calib;
	struct lcd_kit_dsi_panel_cmds open_cmd;
	struct lcd_kit_dsi_panel_cmds close_cmd;
};
...
struct lcd_kit_hbm_backlight {
	...
	u32 need_apl_prepare;
	u32 apl_threshold;
	u32 apl_status;
	struct lcd_kit_dsi_panel_cmds enter_apl_cmds;
	struct lcd_kit_dsi_panel_cmds exit_apl_cmds;
};
...
#define PANEL_EL_CTRL_VER_TWO 2
#define EL_CTRL_BIT_GAP 8

enum lcd_kit_el_ctrl_lut_value_index {
	ELVSS_LUT_VALUE_INDEX_500_NIT = 0,
	ELVSS_LUT_VALUE_INDEX_1600_NIT = 1,
	ELVSS_LUT_VALUE_INDEX_MAX = 2,
};

enum panel_el_ctrl_process_alg {
	PANEL_EL_CTRL_PROCESS_ALG1 = 1,
	PANEL_EL_CTRL_PROCESS_ALG2 = 2,
};
...
struct lcd_kit_panel_el_ctrl_info {
	...
	uint32_t normal_dbv;
	uint32_t normal_el_offset;
	uint32_t hbm_el_offset;
	/* el ctrl v2.0 elvss */
	uint32_t el_ctrl_plus_elvss;
	...
	struct lcd_kit_array_data elvss_reference;
};
...
struct lcd_kit_panel_svm {
	u32 support;
	bool is_svm_12pulse;
	struct lcd_kit_dsi_panel_cmds ota_12pulse_cmds[SOFTWARE_ID_CMDS_ADD_NUM_MAX];
	struct lcd_kit_dsi_panel_cmds ota_3pulse_cmds[SOFTWARE_ID_CMDS_ADD_NUM_MAX];
};
...
struct lcd_kit_dsc_switch {
	u32 support;
	uint8_t power_compress_enable;
	struct lcd_kit_dsi_panel_cmds lcd_compress_cmd;
	struct lcd_kit_dsi_panel_cmds lcd_uncompress_cmd;
};

struct lcd_kit_exce_mode {
	u32 support;
	struct lcd_kit_dsi_panel_cmds enter_cmd;
	struct lcd_kit_dsi_panel_cmds exit_cmd;
	struct lcd_kit_dsi_panel_cmds check_cmd;
};
...
struct lcd_kit_poweric_reuse {
	u32 support;
	struct lcd_kit_dsi_panel_cmds normal_band_cmd;
};
...
struct lcd_kit_common_info {
	...
	/* set cmds grayscale befor set backlight */
	struct lcd_kit_grayscale_backlight grayscale_backlight;
	...
	struct lcd_kit_dsi_panel_cmds panel_on_dual_cmds;
	...
	struct lcd_kit_dsi_panel_cmds panel_off_dual_cmds;
	/* customized check commond */
	struct lcd_kit_dsi_panel_cmds customized_cmds;
	/* esd check commond */
	...
	/* Partition Refresh */
	struct lcd_kit_part_refresh part_refresh;
	/* get_blmaxnit */
	...
	/* dsc switch */
	struct lcd_kit_dsc_switch dsc_switch;
	/* mipi switch end */
	bool mipi_switch_end;
	/* camera and lcd reuse mipi */
	unsigned int cam_lcd_mipi_reuse;
#ifdef CONFIG_LCD_KIT_HYBRID
	unsigned int power_check_enable_and_disable;
#endif
	/* dsc switch */
	struct lcd_kit_exce_mode exce_mode;
	/* notify displayengine when power on */
	unsigned int notify_de_support;
	struct lcd_kit_aod aod;
	struct lcd_kit_poweric_reuse poweric_reuse;
	/* whether to skip notification TP */
	bool need_skip_tp_notify;
};
...
struct lcd_kit_power_desc {
	...
	struct lcd_kit_array_data lcd_mipi_switch1;
	struct lcd_kit_array_data lcd_mipi_switch2;
	...
	struct lcd_kit_array_data pmic_irq;
	struct lcd_kit_array_data lcd_bridge_rst;
	struct lcd_kit_array_data lcd_bridge_vdd;
	struct lcd_kit_array_data lcd_power_ic_switch;
	struct lcd_kit_array_data lcd_integ_switch;
	struct lcd_kit_array_data lcd_integ_en;
};
...
struct lcd_kit_power_seq {
	...
	struct lcd_kit_arrays_data panel_mipi_switch_lp_on_seq;
