...
static struct bias_ic_config g_bias_ic_config[] = {
	...
	{"hw_lyw6360", ARRAY_SIZE(common_vol_table), &common_vol_table[0] }
};
	...
	unsigned char *outvsp, unsigned char *outvsn)
{
	...
	for (i = 0; i < ARRAY_SIZE(g_bias_ic_config); i++) {
		...
		for (j = 0; j < g_bias_ic_config[i].len; j++) {
			if (g_bias_ic_config[i].vol_table[j].voltage == (unsigned int)vsp) {
		...
		for (j = 0; j < g_bias_ic_config[i].len; j++) {
			if (g_bias_ic_config[i].vol_table[j].voltage == (unsigned int)vsn) {
	...
	struct lcd_kit_bias_info *pbias_config)
{
	...
	if ((unsigned int)prop->length != (pbias_config->num_of_init_cmd * BIAS_IC_CMD_LEN * sizeof(unsigned int))) {
	...
	for (i = 0; i < (int)pbias_config->num_of_init_cmd; i++) {
...
static void lcd_kit_bias_reg_init(void)
{
	...
	for (i = 0; i < (int)g_pbias_dev->config.num_of_init_cmd; i++) {
...
static int __init lcd_kit_bias_init(void)
{
	...
	if (memcpy_s(bias_ic_id[0].name, I2C_NAME_SIZE, g_bias_ic_name,
