...
#define TEMPER_INVALID_VALUE 100
...
/* temper fiset start timer*/
static bool first_start_timer_flag = false;
/* dsi test */
...
int lcd_kit_check_panel_id(unsigned int panel_id)
{
...
struct lcd_kit_common_info *lcd_kit_get_common_info(int panel_id)
{
	if (lcd_kit_check_panel_id((unsigned int)panel_id) != LCD_KIT_OK)
		return &g_lcd_kit_common_info[0];
...
{
	if (lcd_kit_check_panel_id((unsigned int)panel_id) != LCD_KIT_OK)
		return &g_lcd_kit_power_handle[0];
...
static int lcd_kit_set_bias_ctrl(int panel_id, void *hld,
		uint32_t enable, uint32_t delay)
{
	int ret = LCD_KIT_OK;
...
static int lcd_kit_set_bias_ctrl_event(int panel_id, void *hld,
		uint32_t enable, uint32_t delay)
{
	lcd_kit_set_bias_ctrl(panel_id, hld, enable, delay);
	return LCD_KIT_OK;
}

static int lcd_kit_vci_power_ctrl(int panel_id, void *hld,
		uint32_t enable, uint32_t delay)
{
	int ret = LCD_KIT_OK;
...
static int lcd_kit_iovcc_check_power_ctrl(int panel_id, int enable)
{
	int ret = LCD_KIT_FAIL;
	struct lcd_kit_adapt_ops *adapt_ops = NULL;
	unsigned int retry_safe = 20;
	...
	if (adapt_ops->regulator_read == NULL ||
		common_info->power_check_enable_and_disable != 1)
		return LCD_KIT_FAIL;

	ret = adapt_ops->regulator_read(panel_id, LCD_KIT_IOVCC, IS_ENABLED);  //is power off
	if (ret == POWER_OFF_COMMAND && enable == POWER_ON_COMMAND) { //power off status and we need power on
		LCD_KIT_ERR("%s before enable try disable\n", __func__);
		do {
			ret = adapt_ops->regulator_enable(panel_id, LCD_KIT_IOVCC, POWER_OFF_COMMAND);
			retry_safe--;
		} while (adapt_ops->regulator_read(panel_id, LCD_KIT_IOVCC, READ_LDO_USE_COUNT) != 0 && retry_safe > 0);
	}
	return ret;
}
#endif

static int lcd_kit_iovcc_power_ctrl(int panel_id, void *hld,
		uint32_t enable, uint32_t delay)
{
	int ret = LCD_KIT_OK;
	struct lcd_kit_adapt_ops *adapt_ops = NULL;
	...
	case REGULATOR_MODE:
		if (adapt_ops->regulator_enable) {
#ifdef CONFIG_LCD_KIT_HYBRID
			if (common_info->power_check_enable_and_disable == 1)
				ret = lcd_kit_iovcc_check_power_ctrl(panel_id, enable);
#endif
...
static int lcd_kit_vdd_power_ctrl(int panel_id, void *hld,
		uint32_t enable, uint32_t delay)
{
	int ret = LCD_KIT_OK;
	struct lcd_kit_adapt_ops *adapt_ops = NULL;
...
static int lcd_kit_none_event(int panel_id, void *hld,
		uint32_t enable, uint32_t delay)
{
...
static int lcd_kit_bridge_vdd_power_ctrl(int panel_id, void *hld,
		uint32_t enable, uint32_t delay)
{
	int ret = LCD_KIT_OK;
	struct lcd_kit_adapt_ops *adapt_ops = NULL;
...
static int lcd_kit_vsp_power_ctrl(int panel_id, void *hld,
		uint32_t enable, uint32_t delay)
{
	int ret = LCD_KIT_OK;
	struct lcd_kit_adapt_ops *adapt_ops = NULL;
...
static int lcd_kit_vsn_power_ctrl(int panel_id, void *hld,
		uint32_t enable, uint32_t delay)
{
	int ret = LCD_KIT_OK;
	struct lcd_kit_adapt_ops *adapt_ops = NULL;
...
static int lcd_kit_aod_power_ctrl(int panel_id, void *hld,
		uint32_t enable, uint32_t delay)
{
	struct lcd_kit_adapt_ops *adapt_ops = NULL;
...
int lcd_kit_set_mipi_switch_ctrl(int panel_id, void *hld,
		uint32_t enable, uint32_t delay)
{
	struct lcd_kit_adapt_ops *adapt_ops = NULL;
...
int lcd_kit_set_mipi_switch1_ctrl(int panel_id, void *hld,
		uint32_t enable, uint32_t delay)
{
	struct lcd_kit_adapt_ops *adapt_ops = NULL;
	...
	if (power_hdl->lcd_mipi_switch1.buf == NULL) {
		LCD_KIT_ERR("can not get lcd mipi switch1!\n");
		return LCD_KIT_FAIL;
	}
	switch (power_hdl->lcd_mipi_switch1.buf[0]) {
	case GPIO_MODE:
		if (enable) {
			if (adapt_ops->gpio_enable_nolock)
				adapt_ops->gpio_enable_nolock(panel_id, LCD_KIT_MIPI_SWITCH1);
		} else {
			if (adapt_ops->gpio_disable_nolock)
				adapt_ops->gpio_disable_nolock(panel_id, LCD_KIT_MIPI_SWITCH1);
		}
		break;
	case NONE_MODE:
		LCD_KIT_DEBUG("lcd mipi switch1 is none mode\n");
		break;
	default:
		LCD_KIT_ERR("lcd mipi switch1 is not normal\n");
		return LCD_KIT_FAIL;
	}
	return LCD_KIT_OK;
}
...
static void set_mipi_switch_end_state(int panel_id)
{
	if (!common_info->cam_lcd_mipi_reuse)
		return;
	common_info->mipi_switch_end = true;
	if (panel_id == 0) { /* primary panel */
		panel_id = 1; /* secondary panel */
		common_info->mipi_switch_end = false;
	} else if (panel_id == 1) { /* secondary panel */
		panel_id = 0; /* primary panel */
		common_info->mipi_switch_end = false;
	}
}

int lcd_kit_set_mipi_switch2_ctrl(int panel_id, void *hld,
		uint32_t enable, uint32_t delay)
{
	struct lcd_kit_adapt_ops *adapt_ops = NULL;
	...
	switch (power_hdl->lcd_mipi_switch2.buf[0]) {
	case GPIO_MODE:
		if (enable) {
			if (adapt_ops->gpio_enable_nolock)
				adapt_ops->gpio_enable_nolock(panel_id, LCD_KIT_MIPI_SWITCH2);
		} else {
			if (adapt_ops->gpio_disable_nolock)
				adapt_ops->gpio_disable_nolock(panel_id, LCD_KIT_MIPI_SWITCH2);
		}
		break;
	case NONE_MODE:
		LCD_KIT_DEBUG("lcd mipi switch2 is none mode\n");
		break;
	default:
		LCD_KIT_ERR("lcd mipi switch2 is not normal\n");
		return LCD_KIT_FAIL;
	}
	set_mipi_switch_end_state(panel_id);
	return LCD_KIT_OK;
}
...
int lcd_kit_reset_power_ctrl_event(int panel_id, void *hld,
		uint32_t enable, uint32_t delay)
{
	return lcd_kit_reset_power_ctrl(panel_id, enable);
}

int lcd_kit_bridge_rst_power_ctrl(int panel_id, void *hld,
		uint32_t enable, uint32_t delay)
{
...
static int lcd_kit_pmic_ctrl_event(int panel_id, void *hld,
		uint32_t enable, uint32_t delay)
{
	struct lcd_kit_adapt_ops *adapt_ops = NULL;
...
static bool is_other_panel_on(int panel_id)
{
	bool ret = true;
	int other_panel_on;

	if (!common_info->poweric_reuse.support) {
		return false;
	}
	panel_id = (panel_id == 0) ? 1 : 0;
	other_panel_on = lcd_kit_panel_is_power_on(panel_id) != 0;
	if (other_panel_on) {
		LCD_KIT_INFO("other panel is on\n");
		ret = true;
	} else {
		LCD_KIT_INFO("other panel is off\n");
		ret = false;
	}
	return ret;
}

static int lcd_kit_pmic_reset_event(int panel_id, void *hld,
		uint32_t enable, uint32_t delay)
{
	struct lcd_kit_adapt_ops *adapt_ops = NULL;
...
static int lcd_kit_pmic_init_event(int panel_id, void *hld,
		uint32_t enable, uint32_t delay)
{
	struct lcd_kit_adapt_ops *adapt_ops = NULL;
...
static int lcd_kit_boost_enable_event(int panel_id, void *hld,
		uint32_t enable, uint32_t delay)
{
	struct lcd_kit_adapt_ops *adapt_ops = NULL;
...
static int lcd_kit_boost_ctrl_event(int panel_id, void *hld,
		uint32_t enable, uint32_t delay)
{
	struct lcd_kit_adapt_ops *adapt_ops = NULL;
...
static int lcd_kit_boost_bypass_event(int panel_id, void *hld,
		uint32_t enable, uint32_t delay)
{
	struct lcd_kit_adapt_ops *adapt_ops = NULL;
...
static void lcd_kit_mipp_add(int panel_id, void *hld)
{
	...
	cmds_add_numb = (int)common_info->panel_software_id.number;
		...
		uint32_t enable, uint32_t delay)
{
	...
	display_id = (int)common_info->ppc_para.ppc_config_id;
	LCD_KIT_INFO("panel partial ctrl id:%u\n", display_id);

	for (i = 0; i < (int)common_info->ppc_para.cmds_cnt; i++) {
		master_index = (int)(display_id * common_info->ppc_para.dsi_cnt * common_info->ppc_para.cmds_cnt
			+ 0 * common_info->ppc_para.cmds_cnt + i); // 0:master dsi index
		slave_index = (int)(display_id * common_info->ppc_para.dsi_cnt * common_info->ppc_para.cmds_cnt
...
static int lcd_kit_ppc_on_cmds_event(int panel_id, void *hld,
		uint32_t enable, uint32_t delay)
{
	lcd_kit_ppc_on_cmds(panel_id, hld, enable, delay);
	return LCD_KIT_OK;
}

static int send_poweric_reuse_on_cmd(struct lcd_kit_adapt_ops *adapt_ops, int panel_id, void *hld)
{
	int ret = LCD_KIT_OK;
	...
	if (is_other_panel_on(panel_id)) {
		LCD_KIT_INFO("dual on\n");
		ret = adapt_ops->mipi_tx(panel_id, hld, &common_info->panel_on_dual_cmds);
	} else {
		LCD_KIT_INFO("single on\n");
		ret = adapt_ops->mipi_tx(panel_id, hld, &common_info->panel_on_cmds);
	}
	if (ret) {
		LCD_KIT_ERR("send panel on cmds error\n");
...
static int lcd_kit_on_cmds(int panel_id, void *hld)
{
	...
	if (common_info->poweric_reuse.support) {
		ret = send_poweric_reuse_on_cmd(adapt_ops, panel_id, hld);
		return ret;
	}
	/* init code */
	if (adapt_ops->mipi_tx) {
...
static int send_panel_off_cmds(int panel_id, void *hld, struct lcd_kit_adapt_ops *adapt_ops)
{
	int ret = LCD_KIT_OK;
	...
	if (is_other_panel_on(panel_id)) {
		LCD_KIT_INFO("dual off\n");
		ret = adapt_ops->mipi_tx(panel_id, hld, &common_info->panel_off_dual_cmds);
	} else {
		LCD_KIT_INFO("single off\n");
		ret = adapt_ops->mipi_tx(panel_id, hld, &common_info->panel_off_cmds);
	}
	return ret;
}
...
static int lcd_kit_off_cmds(int panel_id, void *hld)
{
	...
	ret = send_panel_off_cmds(panel_id, hld, adapt_ops);
	return ret;
}
...
static int lcd_kit_mipi_power_ctrl(int panel_id, void *hld,
		uint32_t enable, uint32_t delay)
{
	int ret = LCD_KIT_OK;
	int ret_check_reg;
...
static int lcd_kit_aod_mipi_ctrl(int panel_id, void *hld,
		uint32_t enable, uint32_t delay)
{
	int ret = LCD_KIT_OK;
	if (enable)
...
static int lcd_kit_early_ts_event(int panel_id, void *hld,
		uint32_t enable, uint32_t delay)
{
	if (common_info->need_skip_tp_notify) {
		LCD_KIT_INFO("need skip tp notify! return!");
		return LCD_KIT_OK;
	}
	if (enable)
		lcd_kit_ts_resume(panel_id, delay);
	else
		lcd_kit_ts_early_suspend(panel_id, delay);
	return LCD_KIT_OK;
}

static int lcd_kit_later_ts_event(int panel_id, void *hld,
		uint32_t enable, uint32_t delay)
{
	if (common_info->need_skip_tp_notify) {
		LCD_KIT_INFO("need skip tp notify! return!");
	...
	if (enable)
		lcd_kit_ts_after_resume(panel_id, delay);
	else
		lcd_kit_ts_suspend(panel_id, delay);
	return LCD_KIT_OK;
}

static int lcd_kit_2nd_power_off_ts_event(int panel_id, void *hld,
		uint32_t enable, uint32_t delay)
{
	lcd_kit_ts_2nd_power_off(delay);
	return LCD_KIT_OK;
}

static int lcd_kit_block_ts_event(int panel_id, void *hld,
		uint32_t enable, uint32_t delay)
{
	if (enable)
		lcd_kit_ts_block_tprst(delay);
	else
		lcd_kit_ts_unblock_tprst(delay);
	return LCD_KIT_OK;
}

static int lcd_kit_eink_power_ctrl_event(int panel_id, void *hld,
		uint32_t enable, uint32_t delay)
{
	struct lcd_kit_ops *lcd_ops = NULL;
...
static int lcd_kit_bridge_ctrl(int panel_id, void *hld,
		uint32_t enable, uint32_t delay)
{
...
static int lcd_kit_bridge_ctrl_event(int panel_id, void *hld,
		uint32_t enable, uint32_t delay)
{
	lcd_kit_bridge_ctrl(panel_id, hld, enable, delay);
	return LCD_KIT_OK;
}
...
static int lcd_kit_dvdd_hpm_ctrl(int panel_id, void *hld,
		uint32_t enable, uint32_t delay)
{
	struct lcd_kit_ops *lcd_ops = NULL;
	int ret = LCD_KIT_OK;
...
static int lcd_kit_dvdd_vol_ctrl_event(int panel_id, void *hld,
		uint32_t enable, uint32_t delay)
{
	lcd_kit_dvdd_vol_ctrl(panel_id, hld, enable, delay);
	return LCD_KIT_OK;
}

static int lcd_kit_avdd_mipi_ctrl(int panel_id, void *hld,
		uint32_t enable, uint32_t delay)
{
	struct lcd_kit_ops *lcd_ops = NULL;
	int ret = LCD_KIT_OK;
...
int lcd_kit_mipi_check_status(int panel_id, void *hld, uint32_t event)
{
	...
	if (common_info->exce_mode.support) {
		LCD_KIT_INFO("panel is in exce_mode\n");
		return LCD_KIT_OK;
	}
	if (event == EVENT_MIPI) {
		ret_check_reg = lcd_kit_check_reg_report_dsm(panel_id, hld,
...
static int lcd_kit_c_calib_send_cmd(int panel_id, int op)
{
	int ret;
	struct lcd_kit_adapt_ops *adapt_ops = NULL;
	...
	switch (op) {
	case LCD_KIT_DDIC_CMD_OP_CLOSE:
		ret = adapt_ops->mipi_tx(panel_id, NULL, &common_info->c_calib.close_cmd);
		break;
	case LCD_KIT_DDIC_CMD_OP_OPEN:
		ret = adapt_ops->mipi_tx(panel_id, NULL, &common_info->c_calib.open_cmd);
		break;
	default:
		LCD_KIT_ERR("op not exist\n");
		ret = -EINVAL;
		break;
	}

	LCD_KIT_INFO("set color calib commands, op:%d\n", op);

	return ret;
}

static int lcd_kit_color_calibration(int panel_id, void *hld,
		uint32_t enable, uint32_t delay)
{
	int ret;
	...
	ret = lcd_kit_c_calib_send_cmd(panel_id, common_info->c_calib.need_ddic_c_calib);

	return ret;
}

static int lcd_kit_power_set(int panel_id, void *hld,
		uint32_t enable, uint32_t delay)
{
	int ret = LCD_KIT_OK;
	struct lcd_kit_adapt_ops *adapt_ops = NULL;
...
static int lcd_kit_power_set_event(int panel_id, void *hld,
		uint32_t enable, uint32_t delay)
{
	lcd_kit_power_set(panel_id, hld, enable, delay);
	return LCD_KIT_OK;
}

static int lcd_kit_aod_ext_cmd(int panel_id, void *hld,
		uint32_t enable, uint32_t delay)
{
...
static int lcd_kit_svm_12pulse_cmd(int panel_id, void *hld,
		uint32_t enable, uint32_t delay)
{
	int ret = LCD_KIT_OK;
	struct lcd_kit_adapt_ops *adapt_ops = NULL;
	int cmds_add_num = (int)common_info->panel_software_id.number;
	...
	if (!common_info->panel_svm.support) {
		LCD_KIT_INFO("panel svm not support\n");
		return LCD_KIT_FAIL;
	}

	if (common_info->panel_svm.is_svm_12pulse &&
		common_info->panel_svm.ota_12pulse_cmds[cmds_add_num].cmds) {
		LCD_KIT_INFO("set 12 pulse ota cmds, cmds_add_num is %d\n", cmds_add_num);
		ret = adapt_ops->mipi_tx(panel_id, NULL,
			&common_info->panel_svm.ota_12pulse_cmds[cmds_add_num]);
	}

	return ret;
}

static int lcd_kit_svm_12pulse_cmd_event(int panel_id, void *hld,
		uint32_t enable, uint32_t delay)
{
	lcd_kit_svm_12pulse_cmd(panel_id, hld, enable, delay);
	return LCD_KIT_OK;
}

static int lcd_kit_set_dsc_switch_ctrl(int panel_id, void *hld,
		uint32_t enable, uint32_t delay)
{
	int ret = LCD_KIT_OK;
	struct lcd_kit_adapt_ops *adapt_ops = NULL;
	...
	if (!common_info->dsc_switch.power_compress_enable && common_info->dsc_switch.lcd_uncompress_cmd.cmds) {
		LCD_KIT_INFO("set uncompress mode!\n");
		ret = adapt_ops->mipi_tx(panel_id, NULL, &common_info->dsc_switch.lcd_uncompress_cmd);
	} else if (common_info->dsc_switch.power_compress_enable && common_info->dsc_switch.lcd_compress_cmd.cmds) {
		LCD_KIT_INFO("set compress mode!\n");
		ret = adapt_ops->mipi_tx(panel_id, NULL, &common_info->dsc_switch.lcd_compress_cmd);
	}

	return ret;
}

static int lcd_kit_dieid_forwar_ctrl(int panel_id, void *hld,
		uint32_t enable, uint32_t delay)
{
	int ret = LCD_KIT_OK;
	struct lcd_kit_ops *lcd_ops = NULL;
	...
	if (lcd_ops->dieid_forwar_read)
		ret = lcd_ops->dieid_forwar_read(panel_id);
	return ret;
}

static int lcd_kit_set_power_ic_switch(int panel_id, void *hld,
		uint32_t enable, uint32_t delay)
{
	struct lcd_kit_adapt_ops *adapt_ops = NULL;
	...
	if (power_hdl->lcd_power_ic_switch.buf == NULL) {
		LCD_KIT_ERR("can not get lcd power ic switch!\n");
		return LCD_KIT_FAIL;
	}
	switch (power_hdl->lcd_power_ic_switch.buf[0]) {
	case GPIO_MODE:
		if (enable) {
			if (adapt_ops->gpio_enable_nolock)
				adapt_ops->gpio_enable_nolock(panel_id, LCD_KIT_POWER_IC_SWITCH);
		} else {
			if (adapt_ops->gpio_disable_nolock)
				adapt_ops->gpio_disable_nolock(panel_id, LCD_KIT_POWER_IC_SWITCH);
		}
		break;
	case NONE_MODE:
		LCD_KIT_DEBUG("lcd power ic switch is none mode\n");
		break;
	default:
		LCD_KIT_ERR("lcd power ic switch is not normal\n");
		return LCD_KIT_FAIL;
	}
	return LCD_KIT_OK;
}

static int lcd_kit_set_integ_switch(int panel_id, void *hld,
		uint32_t enable, uint32_t delay)
{
	struct lcd_kit_adapt_ops *adapt_ops = NULL;
	...
	if (power_hdl->lcd_integ_switch.buf == NULL) {
		LCD_KIT_ERR("can not get lcd integrate switch!\n");
		return LCD_KIT_FAIL;
	}
	switch (power_hdl->lcd_integ_switch.buf[0]) {
	case GPIO_MODE:
		if (enable) {
			if (adapt_ops->gpio_enable_nolock)
				adapt_ops->gpio_enable_nolock(panel_id, LCD_KIT_INTEG_SWITCH);
		} else {
			if (adapt_ops->gpio_disable_nolock)
				adapt_ops->gpio_disable_nolock(panel_id, LCD_KIT_INTEG_SWITCH);
		}
		break;
	case NONE_MODE:
		LCD_KIT_DEBUG("lcd integrate switch is none mode\n");
		break;
	default:
		LCD_KIT_ERR("lcd integrate switch is not normal\n");
		return LCD_KIT_FAIL;
	}
	return LCD_KIT_OK;
}

static int lcd_kit_set_integ_en(int panel_id, void *hld,
		uint32_t enable, uint32_t delay)
{
	struct lcd_kit_adapt_ops *adapt_ops = NULL;
	...
	if (power_hdl->lcd_integ_en.buf == NULL) {
		LCD_KIT_ERR("can not get lcd integrate switch!\n");
		return LCD_KIT_FAIL;
	}
    if (is_other_panel_on(panel_id)) {
	...
	switch (power_hdl->lcd_integ_en.buf[0]) {
	case GPIO_MODE:
		if (enable) {
			if (adapt_ops->gpio_enable_nolock)
				adapt_ops->gpio_enable_nolock(panel_id, LCD_KIT_INTEG_EN);
		} else {
			if (adapt_ops->gpio_disable_nolock)
				adapt_ops->gpio_disable_nolock(panel_id, LCD_KIT_INTEG_EN);
		}
		break;
	case NONE_MODE:
		LCD_KIT_DEBUG("lcd integrate en is none mode\n");
		break;
	default:
		LCD_KIT_ERR("lcd integrate en is not normal\n");
		return LCD_KIT_FAIL;
	}
	return LCD_KIT_OK;
}

static int lcd_kit_band_switch(int panel_id, void *hld,
		uint32_t enable, uint32_t delay)
{
	int ret = LCD_KIT_OK;
	struct lcd_kit_adapt_ops *adapt_ops = NULL;
	...
	if (!common_info->poweric_reuse.support) {
		LCD_KIT_INFO("not support poweric reuse\n");
		return ret;
	}
	if (!enable)
		return ret;
	if (adapt_ops->mipi_tx) {
		ret = adapt_ops->mipi_tx(panel_id, hld, &common_info->poweric_reuse.normal_band_cmd);
		if (ret)
			LCD_KIT_ERR("send normal band cmds error\n");
	}
	return ret;
}

static struct lcd_kit_event_table g_lcd_kit_event_table[] = {
	{EVENT_VCI, lcd_kit_vci_power_ctrl},
	{EVENT_IOVCC, lcd_kit_iovcc_power_ctrl},
	{EVENT_VSP, lcd_kit_vsp_power_ctrl},
	{EVENT_VSN, lcd_kit_vsn_power_ctrl},
	{EVENT_RESET, lcd_kit_reset_power_ctrl_event},
	{EVENT_MIPI, lcd_kit_mipi_power_ctrl},
	{EVENT_EARLY_TS, lcd_kit_early_ts_event},
	{EVENT_LATER_TS, lcd_kit_later_ts_event},
	{EVENT_VDD, lcd_kit_vdd_power_ctrl},
	{EVENT_AOD, lcd_kit_aod_power_ctrl},
	{EVENT_NONE, lcd_kit_none_event},
	{EVENT_BIAS, lcd_kit_set_bias_ctrl_event},
	{EVENT_AOD_MIPI, lcd_kit_aod_mipi_ctrl},
	{EVENT_MIPI_SWITCH, lcd_kit_set_mipi_switch_ctrl},
	{EVENT_AVDD_MIPI, lcd_kit_avdd_mipi_ctrl},
	{EVENT_2ND_POWER_OFF_TS, lcd_kit_2nd_power_off_ts_event},
	{EVENT_BLOCK_TS, lcd_kit_block_ts_event},
	{EVENT_PMIC_CTRL, lcd_kit_pmic_ctrl_event},
	{EVENT_PMIC_RESET, lcd_kit_pmic_reset_event},
	{EVENT_PMIC_INIT, lcd_kit_pmic_init_event},
	{EVENT_PROCESS_HPM, lcd_kit_dvdd_hpm_ctrl},
	{EVENT_BOOST_ENABLE, lcd_kit_boost_enable_event},
	{EVENT_BOOST_CTRL, lcd_kit_boost_ctrl_event},
	{EVENT_BOOST_BYPASS, lcd_kit_boost_bypass_event},
	{EVENT_COLOR_CALIB, lcd_kit_color_calibration},
	{EVENT_SET_POWER, lcd_kit_power_set_event},
	{EVENT_AOD_EXT_CMD, lcd_kit_aod_ext_cmd},
	{EVENT_BRIDGE_RST, lcd_kit_bridge_rst_power_ctrl},
	{EVENT_BRIDGE_VDD, lcd_kit_bridge_vdd_power_ctrl},
	{EVENT_BRIDGE, lcd_kit_bridge_ctrl_event},
	{EVENT_DVDD_AVS, lcd_kit_dvdd_vol_ctrl_event},
	{EVENT_PPC_ON, lcd_kit_ppc_on_cmds_event},
	{EVENT_EINK_WAKEUP, lcd_kit_eink_power_ctrl_event},
	{EVENT_SVM_OTA, lcd_kit_svm_12pulse_cmd_event},
	{EVENT_MIPI_SWITCH1, lcd_kit_set_mipi_switch1_ctrl},
	{EVENT_MIPI_SWITCH2, lcd_kit_set_mipi_switch2_ctrl},
	{EVENT_DUL_DSI, lcd_kit_dual_dsi_ctrl},
	{EVENT_DSC_SWITCH, lcd_kit_set_dsc_switch_ctrl},
	{EVENT_DIEID_FORWAR, lcd_kit_dieid_forwar_ctrl},
	{EVENT_POWERIC_SWITCH, lcd_kit_set_power_ic_switch},
	{EVENT_INTEG_SWITCH, lcd_kit_set_integ_switch},
	{EVENT_INTEG_EN, lcd_kit_set_integ_en},
	{EVENT_BAND_SWITCH, lcd_kit_band_switch},
};
...
{
	int ret = LCD_KIT_OK;
	int table_count = sizeof(g_lcd_kit_event_table) / sizeof(struct lcd_kit_event_table);
	int i;
	uint32_t event_not_exist = 1;
	LCD_KIT_INFO("panel_id = %d, event = %d, data = %d, delay = %d\n", panel_id, event, data, delay);
	...
	for (i = 0; i < table_count; i++) {
		if (event == (int)g_lcd_kit_event_table[i].event &&
			(g_lcd_kit_event_table[i].func_ptr != NULL)) {
			ret = g_lcd_kit_event_table[i].func_ptr(panel_id, hld, data, delay);
			event_not_exist = 0;
			break;
		}
	}
...
static int lcd_kit_panel_mipi_switch_on_lp(int panel_id, void *hld)
{
	int ret = LCD_KIT_OK;
	int i;
	struct lcd_kit_array_data *pevent = NULL;

	pevent = power_seq->panel_mipi_switch_lp_on_seq.arry_data;
	for (i = 0; i < power_seq->panel_mipi_switch_lp_on_seq.cnt; i++) {
		if (!pevent || !pevent->buf) {
			LCD_KIT_ERR("pevent is null!\n");
			return LCD_KIT_FAIL;
		}
...
static int lcd_kit_panel_doze_lp(int panel_id, void *hld)
{
	return LCD_KIT_OK;
}

static int lcd_kit_panel_doze_suspend(int panel_id, void *hld)
{
	return LCD_KIT_OK;
}

static int lcd_kit_get_temper_handler(int panel_id, int *cur_thermal)
{
	int ret = LCD_KIT_OK;
	...
	if (common_info->temper_thread.isAhTemper)
		...
		if(common_info->temper_thread.get_temper_mode == LCD_KIT_THERMAL_PA_S)
			thermal_zone = thermal_zone_get_zone_by_name("pa_s");
		else if (common_info->temper_thread.get_temper_mode == LCD_KIT_THERMAL_SHELL_FRAME)
			thermal_zone = thermal_zone_get_zone_by_name("shell_frame");
		else if (common_info->temper_thread.get_temper_mode == LCD_KIT_THERMAL_SHELL_FRONT)
...
static int lcd_kit_find_alpha_temper(int panel_id, int thermal, int *index)
{
	int ret = LCD_KIT_FAIL;

	if (first_start_timer_flag) {
		for (int i = 0; i < common_info->temper_thread.temperature_value.cnt - 1; i++) {
			LCD_KIT_INFO("first_start_timer thermal = %d, (int)common_info->temper_thread.temperature_value.buf[%d] = %d\n",
				thermal, i, (int)common_info->temper_thread.temperature_value.buf[i]);
			if(thermal >= (int)common_info->temper_thread.temperature_value.buf[i] &&
						thermal < (int)common_info->temper_thread.temperature_value.buf[i + 1]) {
				*index = i;
				ret = LCD_KIT_OK;
				break;
			}
		...
		for (int i = 0; i < common_info->temper_thread.temperature_value.cnt; i++) {
			LCD_KIT_INFO("thermal = %d, (int)common_info->temper_thread.temperature_value.buf[%d] = %d\n",
				thermal, i, (int)common_info->temper_thread.temperature_value.buf[i]);
			if(thermal == (int)common_info->temper_thread.temperature_value.buf[i]) {
				*index = i;
				ret = LCD_KIT_OK;
...
static void lcd_kit_alpha_enter_temper(int panel_id, int cur_thermal)
{
	int ret = LCD_KIT_OK;
	int index = 0;
	int thermal;
	struct lcd_kit_adapt_ops *adapt_ops = NULL;
	int temperMax = common_info->temper_thread.temperMax;
	int temperMin = common_info->temper_thread.temperMin;
	int async_ctrl_mode;
	int mipi_ctrl_mode;
	adapt_ops = lcd_kit_get_adapt_ops();
	if (!adapt_ops || !adapt_ops->mipi_tx) {
	...
	if (temperMax == TEMPER_INVALID_VALUE && temperMin == TEMPER_INVALID_VALUE) {
		LCD_KIT_ERR("temper value is invalid\n");
		return;
	}
	thermal = (int)(cur_thermal / TMPER_ACCURACY);
	if (thermal >= temperMin && thermal <= temperMax) {
		ret = lcd_kit_find_alpha_temper(panel_id, thermal, &index);
		if (ret || index >= TEMPER_ALPHA_CMDS_NUM_MAX) {
			LCD_KIT_INFO("not find corresponding termperature or index array bound\n");
			return;
		}
		LCD_KIT_INFO("lcd_kit_alpha_enter_temper current_index = %d\n", index);
		if (common_info->temper_thread.temper_alpha_enter_cmds[index].cmds) {
			if (first_start_timer_flag) {
				async_ctrl_mode = common_info->temper_thread.temper_alpha_enter_cmds[index].async_param.sync_mode;
				mipi_ctrl_mode = common_info->temper_thread.temper_alpha_enter_cmds[index].link_state;
				common_info->temper_thread.temper_alpha_enter_cmds[index].async_param.sync_mode = LCD_KIT_DSI_SYNC_MODE;
				common_info->temper_thread.temper_alpha_enter_cmds[index].link_state = LCD_KIT_DSI_LP_MODE;
				ret = adapt_ops->mipi_tx(panel_id, NULL, &common_info->temper_thread.temper_alpha_enter_cmds[index]);
				first_start_timer_flag = false;
				common_info->temper_thread.temper_alpha_enter_cmds[index].async_param.sync_mode = async_ctrl_mode;
				common_info->temper_thread.temper_alpha_enter_cmds[index].link_state = mipi_ctrl_mode;
			} else {
				ret = adapt_ops->mipi_tx(panel_id, NULL, &common_info->temper_thread.temper_alpha_enter_cmds[index]);
			}
		}
	...
	if (ret) {
		LCD_KIT_ERR("temper alpha enter cmds send fail\n");
		return;
	}

#ifdef LCD_KIT_DKMD
	/* notify temperature to DE */
	display_engine_temper_brightness_update_ppu_status(panel_id, thermal);
#endif
}
...
static void lcd_kit_start_temper_hrtimer(int panel_id)
{
	...
	if (!hrtimer_active(&common_info->temper_thread.hrtimer)) {
		hrtimer_start(&common_info->temper_thread.hrtimer,
			...
			HRTIMER_MODE_REL);
		ret = lcd_kit_get_temper_handler(panel_id, &cur_thermal);
		if (ret) {
		...
		first_start_timer_flag = true;
		if (common_info->temper_thread.isAhTemper)
			lcd_kit_is_open_temper(panel_id, cur_thermal);
		else
			lcd_kit_alpha_enter_temper(panel_id, cur_thermal);
	}
}
...
static int lcd_kit_esd_event(int esd_value, int i, char read_value, int clear_esd_info_flag) {
	char expect_value, judge_type;
	judge_type = ((unsigned int)esd_value >> 8) & 0xFF;
	expect_value = (unsigned int)esd_value & 0xFF;
	int ret = 0;
	if (lcd_kit_judge_esd(judge_type, read_value, expect_value)) {
		if (clear_esd_info_flag == false) {
			lcd_kit_clear_esd_error_info();
			clear_esd_info_flag = true;
		}
		lcd_kit_record_esd_error_info(i, (int)read_value, expect_value);
		LCD_KIT_ERR("read_value[%d] = 0x%x, but expect_value = 0x%x!\n",
			i, read_value, expect_value);
		ret = LCD_KIT_ESD;
		return ret;
	}
	return ret;
...
static void lcd_kit_customized_check_start(int panel_id, void *hld, struct lcd_kit_adapt_ops *adapt_ops)
{
	int i;
	int ret = LCD_KIT_OK;
	char read_value[MAX_REG_READ_COUNT] = {0};
	...
	if (!adapt_ops || !common_info) {
		LCD_KIT_INFO("adapt_ops or common_info null ptr err\n");
		return;
	}

	u32 *esd_value = common_info->esd.value.buf;
	if (!esd_value){
		LCD_KIT_INFO("esd_value is null\n");
        return;
	}

	if (!common_info->customized_cmds.cmds || !common_info->customized_cmds.cmds->payload) {
		LCD_KIT_INFO("cmds or payload null ptr err\n");
		return;
	}

	cmds = common_info->customized_cmds.cmds;
	ret = adapt_ops->mipi_rx(panel_id, hld, read_value, MAX_REG_READ_COUNT - 1, &common_info->customized_cmds);
	if (ret) {
		LCD_KIT_INFO("mipi_rx err\n");
		return;
	}

	for (i = 0; i < common_info->esd.value.cnt; i++)
		LCD_KIT_INFO("read_value[0x%02x]\n", read_value[i]);
}
...
static int lcd_kit_esd_check_start(int panel_id, void *hld, struct lcd_kit_adapt_ops *adapt_ops)
{
	...
	char expect_value, judge_type;
	char read_value[MAX_REG_READ_COUNT] = {0};
	char read_value1[MAX_REG_READ_COUNT] = {0};
	u32 *esd_value = NULL;
	int clear_esd_info_flag = false;

	esd_value = common_info->esd.value.buf;
	if (common_info->esd.dual_support) 
		ret = adapt_ops->mipi_dual_rx(panel_id, hld, read_value, read_value1,  MAX_REG_READ_COUNT - 1,
	...
	for (i = 0; i < common_info->esd.value.cnt; i++) {
		judge_type = (esd_value[i] >> 8) & 0xFF;
		expect_value = esd_value[i] & 0xFF;
		ret = lcd_kit_esd_event(esd_value[i], i, read_value[i], clear_esd_info_flag);
		if (ret == LCD_KIT_ESD) 
			break;
		if (common_info->esd.dual_support) {
			ret = lcd_kit_esd_event(esd_value[i], i, read_value1[i], clear_esd_info_flag);
			if (ret == LCD_KIT_ESD) 
				break;
			LCD_KIT_INFO("judge_type = %d, esd_value[%d] = 0x%x, read_value0[%d] = 0x%x, read_value1[%d] = 0x%x "
				"expect_value = 0x%x\n", judge_type, i, esd_value[i], i, read_value[i], i, read_value1[i], expect_value);
		} else {
...
static int lcd_kit_esd_handle(int panel_id, void *hld)
{
	...
	if (common_info->esd.gpio_detect_support) {
		if (lcd_kit_gpio_detect_event(panel_id)){
			lcd_kit_customized_check_start(panel_id, hld, adapt_ops);
			return LCD_KIT_FAIL;
		}
...
{
	uint32_t index = common_info->backlight.bl51cmd_position_index;
 
	if (common_info->backlight.bl_cmd.cmd_cnt <= index) {
		LCD_KIT_ERR("bl cmd cnt is invalid\n");
		return;
	}
	switch (common_info->backlight.order) {
	case BL_BIG_ENDIAN:
		if (common_info->backlight.bl_max <= 0xFF) {
			common_info->backlight.bl_cmd.cmds[index].payload[1] = level;
		} else if (common_info->backlight.ext_flag) {
			/* change bl level to dsi cmds */
			common_info->backlight.bl_cmd.cmds[index].payload[1] = common_info->backlight.write_offset;
			common_info->backlight.bl_cmd.cmds[index].payload[2] = (level >> 8) & 0xFF;
			common_info->backlight.bl_cmd.cmds[index].payload[3] = level & 0xFF;
		} else {
			/* change bl level to dsi cmds */
			common_info->backlight.bl_cmd.cmds[index].payload[1] = ((level >> 8) & 0xFF) | common_info->backlight.write_offset;
			common_info->backlight.bl_cmd.cmds[index].payload[2] = level & 0xFF;
		}
		break;
	case BL_LITTLE_ENDIAN:
		if (common_info->backlight.bl_max <= 0xFF) {
			...
			/* change bl level to dsi cmds */
			common_info->backlight.bl_cmd.cmds[index].payload[1] = level & 0xFF;
			common_info->backlight.bl_cmd.cmds[index].payload[2] = (level >> 8) & 0xFF;
			common_info->backlight.bl_cmd.cmds[index].payload[3] = common_info->backlight.write_offset;
		} else {
			/* change bl level to dsi cmds */
			common_info->backlight.bl_cmd.cmds[index].payload[1] = level & 0xFF;
			common_info->backlight.bl_cmd.cmds[index].payload[2] = ((level >> 8) & 0xFF) | common_info->backlight.write_offset;
		}
		break;
	default:
...
struct grayscale_cmd_table {
	int grayscale_mode;
	struct lcd_kit_dsi_panel_cmds *cmds;
	struct lcd_kit_array_data *backlightList;
};

static int lcd_kit_set_grayscale_cmds(int panel_id, void *hld, u32 level)
{
	int ret = LCD_KIT_OK;
	int table_num;
	int i;
	...
	struct grayscale_cmd_table g_grayscale_cmd_table[] = {
		{ GRAYSCALE_MODE1, &common_info->grayscale_backlight.panel_grayscale_mode1_cmds,
			&common_info->grayscale_backlight.grayscale_mode1_backlightList },
		{ GRAYSCALE_MODE2, &common_info->grayscale_backlight.panel_grayscale_mode2_cmds,
			&common_info->grayscale_backlight.grayscale_mode2_backlightList },
		{ GRAYSCALE_MODE3, &common_info->grayscale_backlight.panel_grayscale_mode3_cmds,
			&common_info->grayscale_backlight.grayscale_mode3_backlightList },
		{ GRAYSCALE_MODE4, &common_info->grayscale_backlight.panel_grayscale_mode4_cmds,
			&common_info->grayscale_backlight.grayscale_mode4_backlightList },
		{ GRAYSCALE_MODE5, &common_info->grayscale_backlight.panel_grayscale_mode5_cmds,
			&common_info->grayscale_backlight.grayscale_mode5_backlightList },
	};

	table_num = sizeof(g_grayscale_cmd_table) / sizeof(g_grayscale_cmd_table[0]);
	for (i = 0; i < table_num; i++) {
		for (j = 0; j < g_grayscale_cmd_table[i].backlightList->cnt &&
		g_grayscale_cmd_table[i].backlightList->buf != NULL; j++) {
			if (g_grayscale_cmd_table[i].backlightList->buf[j] != level)
				continue;
			if (g_grayscale_cmd_table[i].grayscale_mode != (int)common_info->grayscale_backlight.last_grayscale_mode) {
				ret = adapt_ops->mipi_tx(panel_id, hld, g_grayscale_cmd_table[i].cmds);
				LCD_KIT_INFO("grayscale mode change to = %d\n", g_grayscale_cmd_table[i].grayscale_mode);
				common_info->grayscale_backlight.last_grayscale_mode = (u32)g_grayscale_cmd_table[i].grayscale_mode;
				return ret;
			} else {
				LCD_KIT_INFO("no need send cmds, grayscale mode = %d\n", g_grayscale_cmd_table[i].grayscale_mode);
				return ret;
			}
		}
	}

	if (common_info->grayscale_backlight.last_grayscale_mode != GRAYSCALE_MODE_DEF) {
		ret = adapt_ops->mipi_tx(panel_id, hld, &common_info->grayscale_backlight.panel_grayscale_modedef_cmds);
		LCD_KIT_INFO("grayscale mode change to = %d\n", GRAYSCALE_MODE_DEF);
		common_info->grayscale_backlight.last_grayscale_mode = GRAYSCALE_MODE_DEF;
	}
...
static void lcd_kit_panel_parse_off_command(int panel_id, const struct device_node *np)
{
	...
	lcd_kit_parse_dcs_cmds(np, "lcd-kit,panel-off-dual-cmds",
		"lcd-kit,panel-off-dual-cmds-state", &common_info->panel_off_dual_cmds);
#ifdef CONFIG_LCD_KIT_HYBRID
	lcd_kit_parse_u32(np, "lcd-kit,power_check_enable_and_disable",
		&common_info->power_check_enable_and_disable, 0);
	LCD_KIT_INFO("common_info->power_check_enable_and_disable=%d\n",
			common_info->power_check_enable_and_disable);
#endif
}
...
static void lcd_kit_panel_parse_on_command(int panel_id, const struct device_node *np)
{
	...
	lcd_kit_parse_dcs_cmds(np, "lcd-kit,panel-on-dual-cmds",
		"lcd-kit,panel-on-dual-cmds-state", &common_info->panel_on_dual_cmds);
}
...
static void lcd_kit_panel_parse_customized_check_cmds(int panel_id, const struct device_node *np)
{
	int32_t ret = LCD_KIT_FAIL;

	if (!np || !common_info) {
		LCD_KIT_ERR("%s np null ptr err\n", __func__);
		return;
	}

	/* customized command */
	ret = lcd_kit_parse_dcs_cmds(np, "lcd-kit,customized-reg-cmds",
		"lcd-kit,customized-reg-cmds-state", &common_info->customized_cmds);
	if (ret != LCD_KIT_OK) {
		common_info->customized_cmds.cmds = NULL;
		LCD_KIT_INFO("customized_check not support\n");
	} else
		LCD_KIT_INFO("customized_check support\n");
}
...
static void lcd_kit_panel_parse_esd_cmds(int panel_id, const struct device_node *np)
{
	...
	if (!common_info->esd.support)
		return;
	lcd_kit_parse_u32(np, "lcd-kit,dual-esd-support",
		&common_info->esd.dual_support, 0);
...
static void lcd_kit_panel_parse_dirty_region(int panel_id, const struct device_node *np)
{
	...
	lcd_kit_parse_u32(np, "lcd-kit,dirty-mstate-ordinate-offset",
		&common_info->dirty_region.ordinate_offset, 0);
	lcd_kit_parse_u32(np, "lcd-kit,dirty-region-page-offset",
		&common_info->dirty_region.page_offset, 0);
}

static void lcd_kit_panel_parse_backlight(int panel_id, const struct device_node *np)
{
	/* backlight */
	lcd_kit_parse_u32(np, "lcd-kit,panel-bl51cmd-position-index",
		&common_info->backlight.bl51cmd_position_index, 0);
	...
	lcd_kit_parse_dcs_cmds(np, "lcd-kit,backlight-part-cmds",
		"lcd-kit,backlight-cmds-state", &common_info->backlight.bl_part_cmd);
...
static void lcd_kit_panel_parse_temper_thread(int panel_id, const struct device_node *np)
{
	...
	lcd_kit_parse_u32(np, "lcd-kit,is-Ah-temper",
		&common_info->temper_thread.isAhTemper, 0);
	lcd_kit_parse_u32(np, "lcd-kit,panel-temper-max",
		&common_info->temper_thread.temperMax, TEMPER_INVALID_VALUE);
	lcd_kit_parse_u32(np, "lcd-kit,panel-temper-min",
		&common_info->temper_thread.temperMin, TEMPER_INVALID_VALUE);
	LCD_KIT_INFO("temperMAX = %d, temperMIN = %d\n", common_info->temper_thread.temperMax,
		common_info->temper_thread.temperMin);
	...
	if (common_info->temper_thread.isAhTemper) {
		...
		lcd_kit_parse_array_data(np, "lcd-kit,temper-version-value",
			&common_info->temper_thread.temperature_value);
		for (j = 0; j < common_info->temper_thread.temperature_value.cnt; j++){
			LCD_KIT_INFO("common_info->temper_thread.temperature_value[%d] = %d\n", j,
				common_info->temper_thread.temperature_value.buf[j]);
		}
		for (i = 0; i < TEMPER_ALPHA_CMDS_NUM_MAX; i++) {
			ret = snprintf_s(buf, MAX_STR_LEN, MAX_STR_LEN - 1, "lcd-kit,temper-alpha-enter-cmds%d", i);
			if (ret < 0) {
				LCD_KIT_ERR("snprintf_s cmd buf fail\n");
				return;
			}
			ret = snprintf_s(state_buf, MAX_STR_LEN, MAX_STR_LEN - 1, "lcd-kit,temper-alpha-enter-cmds%d-state", i);
			if (ret < 0) {
				LCD_KIT_ERR("snprintf_s cmd state buf fail\n");
				return;
			}
			lcd_kit_parse_dcs_cmds(np, buf, state_buf, &common_info->temper_thread.temper_alpha_enter_cmds[i]);
		}
		lcd_kit_parse_u32(np, "lcd-kit,panel-get-temper-mode", &common_info->temper_thread.get_temper_mode, 0);
	}
	return;
}

static void lcd_kit_panel_parse_part_refresh(int panel_id, const struct device_node *np)
{
	/* Partition Refresh */
#ifdef LCD_FACTORY_MODE
	common_info->part_refresh.support = 0;
#else
	lcd_kit_parse_u32(np, "lcd-kit,part-refresh-support",
		&common_info->part_refresh.support, 0);
#endif
	lcd_kit_parse_u32(np, "lcd-kit,part-refresh-poweron-delay-frames",
		&common_info->part_refresh.poweron_delay_frames, 0);
	lcd_kit_parse_u32(np, "lcd-kit,part-refresh-applist-brightness-dbv",
		&common_info->part_refresh.applist_brightness_dbv, 0);
	lcd_kit_parse_u32(np, "lcd-kit,part-refresh-nonapplist-brightness-dbv",
		&common_info->part_refresh.non_applist_brightness_dbv, 0);
	lcd_kit_parse_u32(np, "lcd-kit,part-refresh-temp-compensation-support",
		&common_info->part_refresh.temp_compensation_support, 0);
	lcd_kit_parse_u32(np, "lcd-kit,part-refresh-limit-rows",
		&common_info->part_refresh.minimum_limit_rows, 0);
	if (!common_info->part_refresh.support)
		return;
	lcd_kit_parse_dcs_cmds(np, "lcd-kit,part-refresh-enter-command",
		"lcd-kit,part-refresh-enter-command-state",
		&common_info->part_refresh.part_refresh_enter_cmd);
	lcd_kit_parse_dcs_cmds(np, "lcd-kit,part-refresh-exit-command",
		"lcd-kit,part-refresh-exit-command-state",
		&common_info->part_refresh.part_refresh_exit_cmd);
	lcd_kit_parse_dcs_cmds(np, "lcd-kit,part-refresh-command",
		"lcd-kit,part-refresh-command-state",
		&common_info->part_refresh.part_refresh_cmd);
	lcd_kit_parse_dcs_cmds(np, "lcd-kit,part-push-command",
		"lcd-kit,part-push-command-state",
		&common_info->part_refresh.part_push_cmd);
	lcd_kit_parse_dcs_cmds(np, "lcd-kit,part-refresh-reg-command",
		"lcd-kit,part-refresh-reg-command-state",
		&common_info->part_refresh.part_refresh_reg_cmd);
	lcd_kit_parse_dcs_cmds(np, "lcd-kit,bureau-brush-command",
		"lcd-kit,bureau-brush-command-state",
		&common_info->part_refresh.bureau_brush_cmd);
}
...
static void lcd_kit_panel_parse_color_calib(int panel_id, const struct device_node *np)
{
	...
	if (common_info->c_calib.support) {
		lcd_kit_parse_dcs_cmds(np, "lcd-kit,color-calib-open-cmds",
			"lcd-kit,color-calib-open-cmds-state",
			&common_info->c_calib.open_cmd);
		lcd_kit_parse_dcs_cmds(np, "lcd-kit,color-calib-close-cmds",
			"lcd-kit,color-calib-close-cmds-state",
			&common_info->c_calib.close_cmd);
	}
}
...
static void lcd_kit_panel_parse_apl_backlight(int panel_id, const struct device_node *np)
{
	/* apl backlight */
	lcd_kit_parse_u32(np, "lcd-kit,need-apl-prepare",
		&common_info->hbm_backlight.need_apl_prepare, 0);
	if (common_info->hbm_backlight.need_apl_prepare) {
		lcd_kit_parse_u32(np, "lcd-kit,apl-threshold",
			&common_info->hbm_backlight.apl_threshold, 0);
		lcd_kit_parse_dcs_cmds(np, "lcd-kit,enter-apl-cmds",
			"lcd-kit,enter-apl-mode-cmds-state", &common_info->hbm_backlight.enter_apl_cmds);
		lcd_kit_parse_dcs_cmds(np, "lcd-kit,exit-apl-cmds",
			"lcd-kit,exit-apl-mode-cmds-state", &common_info->hbm_backlight.exit_apl_cmds);
	}
}
...
static void lcd_kit_panel_parse_dsc_information(int panel_id, const struct device_node *np)
{
	/* hbm backlight */
	lcd_kit_parse_u32(np, "lcd-kit,dsc-switch-support",
		&common_info->dsc_switch.support, 0);
	if (common_info->dsc_switch.support) {
		lcd_kit_parse_u8(np, "lcd-kit,poweron-dsc-switch-enable", &common_info->dsc_switch.power_compress_enable, 0);

		lcd_kit_parse_dcs_cmds(np, "lcd-kit,dsc-switch-compress-cmds", "lcd-kit,dsc-switch-compress-cmds-state",
			&common_info->dsc_switch.lcd_compress_cmd);
		lcd_kit_parse_dcs_cmds(np, "lcd-kit,dsc-switch-uncompress-cmds", "lcd-kit,dsc-switch-uncompress-cmds-state",
			&common_info->dsc_switch.lcd_uncompress_cmd);
	}
}

static void lcd_kit_parse_panel_grayscale_backlight(int panel_id, const struct device_node *np)
{
	/* grayscale backlight */
	lcd_kit_parse_u32(np, "lcd-kit,panel-grayscale-backlight-support",
		&common_info->grayscale_backlight.support, 0);
	if (common_info->grayscale_backlight.support) {
		common_info->grayscale_backlight.last_grayscale_mode = GRAYSCALE_MODE_DEF;
		lcd_kit_parse_array_data(np, "lcd-kit,grayscale-mode1-backlightList",
			&common_info->grayscale_backlight.grayscale_mode1_backlightList);
		lcd_kit_parse_array_data(np, "lcd-kit,grayscale-mode2-backlightList",
			&common_info->grayscale_backlight.grayscale_mode2_backlightList);
		lcd_kit_parse_array_data(np, "lcd-kit,grayscale-mode3-backlightList",
			&common_info->grayscale_backlight.grayscale_mode3_backlightList);
		lcd_kit_parse_array_data(np, "lcd-kit,grayscale-mode4-backlightList",
			&common_info->grayscale_backlight.grayscale_mode4_backlightList);
		lcd_kit_parse_array_data(np, "lcd-kit,grayscale-mode5-backlightList",
			&common_info->grayscale_backlight.grayscale_mode5_backlightList);

		lcd_kit_parse_dcs_cmds(np, "lcd-kit,panel-grayscale-mode1-cmds",
			"lcd-kit,panel-grayscale-mode1-cmds-state",
			&common_info->grayscale_backlight.panel_grayscale_mode1_cmds);
		lcd_kit_parse_dcs_cmds(np, "lcd-kit,panel-grayscale-mode2-cmds",
			"lcd-kit,panel-grayscale-mode2-cmds-state",
			&common_info->grayscale_backlight.panel_grayscale_mode2_cmds);
		lcd_kit_parse_dcs_cmds(np, "lcd-kit,panel-grayscale-mode3-cmds",
			"lcd-kit,panel-grayscale-mode3-cmds-state",
			&common_info->grayscale_backlight.panel_grayscale_mode3_cmds);
		lcd_kit_parse_dcs_cmds(np, "lcd-kit,panel-grayscale-mode4-cmds",
			"lcd-kit,panel-grayscale-mode4-cmds-state",
			&common_info->grayscale_backlight.panel_grayscale_mode4_cmds);
		lcd_kit_parse_dcs_cmds(np, "lcd-kit,panel-grayscale-mode5-cmds",
			"lcd-kit,panel-grayscale-mode5-cmds-state",
			&common_info->grayscale_backlight.panel_grayscale_mode5_cmds);
		lcd_kit_parse_dcs_cmds(np, "lcd-kit,panel-grayscale-modedef-cmds",
			"lcd-kit,panel-grayscale-modedef-cmds-state",
			&common_info->grayscale_backlight.panel_grayscale_modedef_cmds);
	}
}

static void lcd_kit_parse_cam_lcd_mipi_reuse(int panel_id, const struct device_node *np)
{
	lcd_kit_parse_u32(np, "lcd-kit,cam-lcd-mipi-reuse",
		&common_info->cam_lcd_mipi_reuse, 0);
}

static void lcd_kit_panel_parse_exce_mode(int panel_id, const struct device_node *np)
{
	lcd_kit_parse_u32(np, "lcd-kit,ddic-exception-support", &common_info->exce_mode.support, 0);
	if (!common_info->exce_mode.support) {
		return;
	}
	lcd_kit_parse_dcs_cmds(np, "lcd-kit,panel-exception-enter-cmds",
		"lcd-kit,panel-exception-enter-cmds-state", &common_info->exce_mode.enter_cmd);
	lcd_kit_parse_dcs_cmds(np, "lcd-kit,panel-exception-exit-cmds",
		"lcd-kit,panel-exception-exit-cmds-state", &common_info->exce_mode.exit_cmd);
	lcd_kit_parse_dcs_cmds(np, "lcd-kit,panel-check-exce-cmds",
		"lcd-kit,panel-check-exce-cmds-state", &common_info->exce_mode.check_cmd);
}

static void lcd_kit_parse_notify_displayengine(int panel_id, const struct device_node *np)
{
	lcd_kit_parse_u32(np, "lcd-kit,notify-displayengine-support", &common_info->notify_de_support, 0);
}

static void lcd_kit_parse_poweric_reuse(int panel_id, const struct device_node *np)
{
	lcd_kit_parse_u32(np, "lcd-kit,poweric-reuse", &common_info->poweric_reuse.support, 0);
	if (common_info->poweric_reuse.support)
		lcd_kit_parse_dcs_cmds(np, "lcd-kit,normal-band-cmds", "lcd-kit,normal-band-cmds-state",
			&common_info->poweric_reuse.normal_band_cmd);
}
...
static void lcd_kit_panel_parse_util(int panel_id, const struct device_node *np)
{
	...
	lcd_kit_panel_parse_customized_check_cmds(panel_id, np);
	...
	lcd_kit_panel_parse_part_refresh(panel_id, np);
	...
	lcd_kit_panel_parse_apl_backlight(panel_id, np);
	lcd_kit_panel_parse_dsc_information(panel_id, np);
	/* panel software id */
	lcd_kit_parse_panel_software_id(panel_id, np);
	lcd_kit_parse_panel_grayscale_backlight(panel_id, np);
	/* camera and lcd reuse mipi */
	lcd_kit_parse_cam_lcd_mipi_reuse(panel_id, np);
	lcd_kit_panel_parse_exce_mode(panel_id, np);
	lcd_kit_parse_notify_displayengine(panel_id, np);
	lcd_kit_parse_poweric_reuse(panel_id, np);
}
...
static void lcd_kit_parse_power_seq(int panel_id, struct device_node *np)
{
	...
	lcd_kit_parse_arrays_data(np, "lcd-kit,mipi-switch-lp-on-stage",
		&power_seq->panel_mipi_switch_lp_on_seq, SEQ_NUM);
...
static void lcd_kit_parse_power_mipi_switch(int panel_id, const struct device_node *np)
{
	...
	if (power_hdl->lcd_mipi_switch1.buf != NULL)
		return;
	lcd_kit_parse_array_data(np, "lcd-kit,lcd-mipi-switch1",
		&power_hdl->lcd_mipi_switch1);
	if (power_hdl->lcd_mipi_switch2.buf != NULL)
		return;
	lcd_kit_parse_array_data(np, "lcd-kit,lcd-mipi-switch2",
		&power_hdl->lcd_mipi_switch2);
}
...
static void lcd_kit_parse_power_ic_switch(int panel_id, const struct device_node *np)
{
	/* pmic ctrl */
	if (power_hdl->lcd_power_ic_switch.buf != NULL)
		return;
	lcd_kit_parse_array_data(np, "lcd-kit,lcd-power-ic-switch",
		&power_hdl->lcd_power_ic_switch);
}

static void lcd_kit_parse_integ_switch(int panel_id, const struct device_node *np)
{
	/* pmic ctrl */
	if (power_hdl->lcd_integ_switch.buf != NULL)
		return;
	lcd_kit_parse_array_data(np, "lcd-kit,lcd-integ-switch",
		&power_hdl->lcd_integ_switch);
}

static void lcd_kit_parse_integ_en(int panel_id, const struct device_node *np)
{
	/* pmic ctrl */
	if (power_hdl->lcd_integ_en.buf != NULL)
		return;
	lcd_kit_parse_array_data(np, "lcd-kit,lcd-integ-en",
		&power_hdl->lcd_integ_en);
}
...
static void lcd_kit_parse_power(int panel_id, const struct device_node *np)
{
	...
	lcd_kit_parse_power_ic_switch(panel_id, np);
	lcd_kit_parse_integ_switch(panel_id, np);
	lcd_kit_parse_integ_en(panel_id, np);
}
...
static void lcd_kit_parse_iodie(int panel_id, const struct device_node *np)
{
	lcd_kit_parse_u32(np, "lcd-kit,iodie-support",
		&common_info->iodie_support, 0);
}

static void lcd_kit_parse_aod(int panel_id, const struct device_node *np)
{
	common_info->need_skip_tp_notify = false;
	lcd_kit_parse_u32(np, "lcd-kit,sensorhub-aod-support",
		&common_info->aod.support, 0);
	if (!common_info->aod.support)
		return;
	lcd_kit_parse_dcs_cmds(np, "lcd-kit,panel-exit-aod-cmds",
		"lcd-kit,panel-aod-off-cmds-state", &common_info->aod.exit_cmd);
}
...
static int lcd_kit_panel_parse_dt(int panel_id, struct device_node *np)
{
	...
	/* 9030 iodie */
	lcd_kit_parse_iodie(panel_id, np);
	/* aod info */
	lcd_kit_parse_aod(panel_id, np);
	return LCD_KIT_OK;
}
...
void lcd_kit_delay(int wait, int waittype, bool allow_sleep)
{
	...
	if ((waittype == LCD_KIT_WAIT_US) ||
		...
		udelay(wait);
	} else if ((unsigned int)waittype & LCD_KIT_WAIT_MS) {
	...
	long display_on_record_time)
{
	...
	for (i = 0; i < common_info->mipi_check.value.cnt; i++) {
		...
		if (read_value[i] != expect_ptr[i]) {
			...
			LCD_KIT_ERR("mipi check error[%d]: current error times:%d! total error times:%d, "
				"check-error-times/check-times:%d/%d\n", i, read_value[i], mipi_errors[i].total_errors,
				mipi_errors[i].mipi_error_times, mipi_errors[i].mipi_check_times);
#if (defined(CONFIG_HUAWEI_DSM) || defined(CONFIG_HUAWEI_OHOS_DSM))
...
static int lcd_kit_enter_hbm(int panel_id, void *hld, u32 backlight_status)
{
	int ret = LCD_KIT_OK;
	struct lcd_kit_adapt_ops *adapt_ops = lcd_kit_get_adapt_ops();
	if (adapt_ops == NULL || adapt_ops->mipi_tx == NULL) {
	...
	/* enter hbm */
	if (backlight_status == LCD_KIT_BACKLIGHT_HBM &&
		common_info->hbm_backlight.enter_hbm_backlight_cmds.cmds != NULL) {
	...
	} else if (backlight_status == LCD_KIT_BACKLIGHT_APL &&
		common_info->hbm_backlight.enter_apl_cmds.cmds != NULL) {
		ret = adapt_ops->mipi_tx(panel_id, hld, &common_info->hbm_backlight.enter_apl_cmds);
		common_info->hbm_backlight.apl_status = LCD_KIT_APL_ON;
		LCD_KIT_INFO("enter apl cmds already send, ret = %d!\n", ret);
	}
	return ret;
}

static int lcd_kit_exit_hbm(int panel_id, void *hld, u32 backlight_status)
{
	int ret = LCD_KIT_OK;
	struct lcd_kit_adapt_ops *adapt_ops = lcd_kit_get_adapt_ops();
	if (adapt_ops == NULL || adapt_ops->mipi_tx == NULL) {
	...
	/* exit hbm */
	if (backlight_status == LCD_KIT_BACKLIGHT_HBM &&
		common_info->hbm_backlight.exit_hbm_backlight_cmds.cmds != NULL) {
		...
		common_info->hbm_backlight.exit_apl_cmds.cmds != NULL) {
		ret = adapt_ops->mipi_tx(panel_id, hld, &common_info->hbm_backlight.exit_apl_cmds);
		common_info->hbm_backlight.apl_status = LCD_KIT_APL_OFF;
		LCD_KIT_INFO("exit apl cmds already send, ret = %d!\n", ret);
	}
	return ret;
}
...
static int lcd_kit_is_op_valid(int op)
{
	switch (op) {
	case LCD_KIT_DDIC_CMD_OP_CLOSE:
	case LCD_KIT_DDIC_CMD_OP_OPEN:
...
static int lcd_kit_c_calib_update_para(int panel_id, int op)
{
	int ret = LCD_KIT_OK;
	LCD_KIT_INFO("+\n");
	...
	if (!lcd_kit_is_op_valid(op))
		return -EINVAL;

	common_info->c_calib.need_ddic_c_calib = (u32)op;
	LCD_KIT_INFO("update color calib op:%d\n", common_info->c_calib.need_ddic_c_calib);
...
struct lcd_kit_ddic_cmd_group {
	enum lcd_kit_ddic_cmd_type cmd_type;
	int (*cmd_func)(int panel_id, int op);
};

struct lcd_kit_ddic_cmd_group g_lcd_kit_ddic_cmd_table[] = {
	{ LCD_KIT_DDIC_CMD_TYPE_C_CALIB,
		lcd_kit_c_calib_update_para },
};

static int lcd_kit_set_ddic_cmd(int panel_id, int type, int op)
{
	int i;
	...
	for (i = 0; i < ARRAY_SIZE(g_lcd_kit_ddic_cmd_table); i++)
		if (type == g_lcd_kit_ddic_cmd_table[i].cmd_type)
			return g_lcd_kit_ddic_cmd_table[i].cmd_func(panel_id, op);
...
struct lcd_kit_common_ops g_lcd_kit_common_ops = {
	...
	.panel_mipi_switch_on_lp = lcd_kit_panel_mipi_switch_on_lp,
	...
	.panel_doze_lp = lcd_kit_panel_doze_lp,
	.panel_doze_suspend = lcd_kit_panel_doze_suspend,
	...
	.set_gray_backlight_cmd = lcd_kit_set_grayscale_cmds,
};
...
{
	int ret = LCD_KIT_OK;
	struct gamma_linear_interpolation_info gamma_liner_info = {0};
...
{
	int ret;
	struct color_cmds_rgb color_cmds = {0};
	struct lcd_kit_adapt_ops *adapt_ops = NULL;
	int panel_id = get_panel_id();
	if (common_info->hbm.circle_color_cmds.cmds == NULL) {
		LCD_KIT_ERR("circle_color_cmds.cmds is NULL\n");
