		...
		uint32_t event, uint32_t data, uint32_t delay)
{
	...
	for (i = 0; i < table_count; i++) {
		if (event == g_lcd_kit_event_table[i].event &&
...
static int lcd_kit_find_alpha_temper(int panel_id, int thermal, int *index)
{
	...
	if (first_start_timer_flag) {
		for (int i = 0; i < common_info->temper_thread.temperature_value.cnt - 1; i++) {
			if(thermal >= common_info->temper_thread.temperature_value.buf[i] &&
						thermal < common_info->temper_thread.temperature_value.buf[i+ 1]){
				*index = i;
				ret = LCD_KIT_OK;
				break;
			}
		...
		for (int i = 0; i < common_info->temper_thread.temperature_value.cnt; i++) {
			LCD_KIT_INFO("thermal = %d, common_info->temper_thread.temperature_value.buf[%d] = %d\n",
				thermal, i, common_info->temper_thread.temperature_value.buf[i]);
			if(thermal == common_info->temper_thread.temperature_value.buf[i]){
				*index = i;
				ret = LCD_KIT_OK;
				break;
...
static int lcd_kit_esd_event(int esd_value, int i, char read_value, int clear_esd_info_flag) {
	char expect_value, judge_type;
	judge_type = (esd_value >> 8) & 0xFF;
	expect_value = esd_value & 0xFF;
	int ret = 0;
	...
	for (i = 0; i < table_num; i++) {
		...
		g_grayscale_cmd_table[i].backlightList->buf != NULL; j++) {
			if(g_grayscale_cmd_table[i].backlightList->buf[j] == level &&
				g_grayscale_cmd_table[i].grayscale_mode != common_info->grayscale_backlight.last_grayscale_mode ) {
				...
				common_info->grayscale_backlight.last_grayscale_mode = g_grayscale_cmd_table[i].grayscale_mode;
				return ret;
			}
		}
	}
...
static int lcd_kit_c_calib_update_para(int panel_id, int op)
{
	...
	common_info->c_calib.need_ddic_c_calib = op;
