...
static int lcd_kit_dbg_write_initial_code_file(struct lcd_kit_dsi_panel_cmds *pcmds)
{
	...
	for (i = 0; i < pcmds->cmd_cnt; i++) {
		...
		for (j = 0; j < DSI_CTRL_HEAD_SIZE; j++)
			(void)sprintf_s(
				dsi_ctrl_head + j * DSI_SINGLE_CHAR_WIDTH, DSI_CTRL_HEAD_SIZE* DSI_SINGLE_CHAR_WIDTH + 1,
				"0x%02X, ", head[j]);
...
static int lcd_kit_dbg_dual_mipi_rx(char *par, int len)
{
	...
	if (!master_cmds) {
		LCD_KIT_ERR("master_cmds kzalloc fail\n");
		return LCD_KIT_FAIL;
	}
	slave_cmds = kzalloc(LCD_KIT_CONFIG_TABLE_MAX_NUM, 0);
	if (!slave_cmds) {
		LCD_KIT_ERR("slave_cmds kzalloc fail\n");
		kfree(master_cmds);
		master_cmds = NULL;
		return LCD_KIT_FAIL;
	}
...
static int lcd_ddic_debug_parse_head(char *buf, uint32_t *head_val)
{
	...
	token = strsep(&cur, ",");
	while (token) {
		head_val[i++] = (uint32_t)simple_strtol(token, NULL, 16);
		token = strsep(&cur, ",");
		if (i >= DEBUG_HEAD_MAX) {
	...
	size_t count, loff_t *ppos)
{
	...
	if (*ppos)
		return 0;
	/* show usage */
	len = snprintf(cur, (size_t)buf_len, "Usage:\n");
	buf_len -= len;
	cur += len;

	len = snprintf(cur, (size_t)buf_len,
	...
	const char *item_state, struct lcd_kit_dsi_panel_cmds *pcmds)
{
	...
	item = str_convert(item);
	item_state = str_convert(item_state);
	if (!item || !item_state) {
		LCD_KIT_ERR("string convert fail\n");
		return LCD_KIT_FAIL;
	}
	LCD_KIT_INFO("item = %s\n", item);
	LCD_KIT_INFO("item_state = %s\n", item_state);
	LCD_KIT_INFO("path = %s\n", path);
	return parse_dcs(path, item, item_state, pcmds);
}
