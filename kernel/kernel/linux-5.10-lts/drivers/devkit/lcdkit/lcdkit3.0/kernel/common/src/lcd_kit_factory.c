...
ssize_t lcd_kit_panel_part_refresh_show(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	ssize_t ret = LCD_KIT_OK;
	struct lcd_fact_ops *fact_ops = NULL;
	...
	if (fact_ops->panel_part_refresh_show)
		ret = fact_ops->panel_part_refresh_show(dev, attr, buf);
	return ret;
}

ssize_t lcd_kit_panel_part_refresh_store(struct device *dev,
	struct device_attribute *attr, const char *buf, size_t count)
{
	ssize_t ret = LCD_KIT_OK;
	struct lcd_fact_ops *fact_ops = NULL;
	...
	if (fact_ops->panel_part_refresh_store) {
		ret = fact_ops->panel_part_refresh_store(dev, attr, buf, count);
		if (ret)
			LCD_KIT_ERR("lcd_kit_panel_part_refresh_store fail\n");
	}
	return count;
}
...
static DEVICE_ATTR(lcd_panel_part_refresh, 0644, lcd_kit_panel_part_refresh_show,
	lcd_kit_panel_part_refresh_store);
/* factory attrs */
struct attribute *lcd_factory_attrs[] = {
	...
	&dev_attr_lcd_panel_part_refresh.attr,
};
...
#ifdef LCD_KIT_DKMD
	if (FACT_INFO->test_delay.support && (!lcd_kit_get_panel_on_state(panel_id))) {
		msleep(FACT_INFO->test_delay.val);
		LCD_KIT_INFO("get test config delay:%d\n", FACT_INFO->test_delay.val);
	}
#endif
...
int lcd_kit_set_test_config(int panel_id, const char *buf)
{
	...
	if (FACT_INFO->test_delay.support && (!lcd_kit_get_panel_on_state(panel_id))) {
		msleep(FACT_INFO->test_delay.val);
		LCD_KIT_INFO("set test config delay:%d\n", FACT_INFO->test_delay.val);
	}
#endif
...
static void parse_dt_color_aod_detect(int panel_id, struct device_node *np)
{
	...
	if (FACT_INFO->color_aod_det.support) {
		...
		lcd_kit_parse_dcs_cmds(np, "lcd-kit,at-test-1dot5-fps_cmds",
			"lcd-kit,at-test-1dot5-fps-cmds-state",
...
static void parse_dt_test_delay(int panel_id, struct device_node *np)
{
	/* test delay */
	lcd_kit_parse_u32(np, "lcd-kit,test-delay-support",
		&FACT_INFO->test_delay.support, 0);
	if (FACT_INFO->test_delay.support) {
		lcd_kit_parse_u32(np, "lcd-kit,test-delay-val",
			&FACT_INFO->test_delay.val, 0);
	}
}
...
static void lcd_fact_parse_dt(int panel_id, struct device_node *np)
{
	...
	/* parse test delay */
	parse_dt_test_delay(panel_id, np);
}
