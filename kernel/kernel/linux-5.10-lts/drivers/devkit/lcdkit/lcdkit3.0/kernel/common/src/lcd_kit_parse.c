...
static int conver2le(uint32_t *src, uint32_t *desc, int len)
{
	int i;
	uint32_t temp;
	...
	const char *link_key, struct lcd_kit_dsi_panel_cmds *pcmds)
{
	...
	if (link_key) {
		...
		if (data && !strcmp(data, "dsi_lp_mode")) {
			...
			pcmds->async_param.sync_mode = LCD_KIT_DSI_SYNC_MODE;
		} else if (data && !strcmp(data, "dsi_lp_async_mode")) {
			pcmds->link_state = LCD_KIT_DSI_LP_MODE;
			pcmds->async_param.sync_mode = LCD_KIT_DSI_ASYNC_MODE;
		} else if (data && !strcmp(data, "dsi_hs_async_mode")) {
			pcmds->link_state = LCD_KIT_DSI_HS_MODE;
			pcmds->async_param.sync_mode = LCD_KIT_DSI_ASYNC_MODE;
		} else if (data && !strcmp(data, "dsi_lp_refresh_mode")) {
			pcmds->link_state = LCD_KIT_DSI_LP_MODE;
			pcmds->async_param.sync_mode = LCD_KIT_DSI_SYNC_MODE;
			pcmds->async_param.refresh_flag = LCD_KIT_DSI_REFRESH_USED;
		} else if (data && !strcmp(data, "dsi_hs_refresh_mode")) {
			pcmds->link_state = LCD_KIT_DSI_HS_MODE;
			pcmds->async_param.sync_mode = LCD_KIT_DSI_SYNC_MODE;
			pcmds->async_param.refresh_flag = LCD_KIT_DSI_REFRESH_USED;
		} else if (data && !strcmp(data, "dsi_lp_async_refresh_mode")) {
			pcmds->link_state = LCD_KIT_DSI_LP_MODE;
			pcmds->async_param.sync_mode = LCD_KIT_DSI_ASYNC_MODE;
			pcmds->async_param.refresh_flag = LCD_KIT_DSI_REFRESH_USED;
		} else if (data && !strcmp(data, "dsi_hs_async_refresh_mode")) {
			pcmds->link_state = LCD_KIT_DSI_HS_MODE;
			pcmds->async_param.sync_mode = LCD_KIT_DSI_ASYNC_MODE;
			pcmds->async_param.refresh_flag = LCD_KIT_DSI_REFRESH_USED;
		} else {
	...
	unsigned char *out, unsigned char def)
{
	...
	(defined(CONFIG_LCD_KIT_HISI) || defined(CONFIG_LCD_KIT_MTK))
	if (is_dpd_mode()) {
		if (lcd_parse_u8(np, prop_name, out, def) != LCD_KIT_OK)
			LCD_KIT_ERR("lcd parse u8 fail\n");
		return LCD_KIT_OK;
	}
#endif
	if (of_property_read_u32(np, prop_name, &temp)) {
