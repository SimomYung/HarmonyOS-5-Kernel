	...
	struct device_attribute *attr, char *buf)
{
	...
	for (i = 0; i < TIME_COUNT; i++) {
		...
		if (disp_info->lcd_on_time.delta[i] != 0) {
			sum += (int)disp_info->lcd_on_time.delta[i];
			count++;
		}
		len = snprintf(str_tmp, sizeof(str_tmp), "%d,", disp_info->lcd_on_time.delta[i]);
		if (len < 0) {
			LCD_KIT_ERR("snprintf fail\n");
			return LCD_KIT_FAIL;
		}
		strncat(str_ret, str_tmp, AVG_ON_TIME_MAX - 1);
	}
	LCD_KIT_INFO("\n");
	if (count == 0) {
		LCD_KIT_ERR("count is zero\n");
		return LCD_KIT_FAIL;
	}
	disp_info->lcd_on_time.avg = (uint32_t)(sum / count);
	LCD_KIT_INFO("disp_info->lcd_on_time.avg is %d\n", disp_info->lcd_on_time.avg);
	len = snprintf(str_tmp, sizeof(str_tmp), "avg:%d", disp_info->lcd_on_time.avg);
	if (len < 0) {
		LCD_KIT_ERR("snprintf fail\n");
		return LCD_KIT_FAIL;
	}
...
static int lcd_get_2d_barcode(int panel_id, char *oem_data)
{
	...
	if (disp_info->oeminfo.barcode_2d.support) {
		...
		LCD_CHECK_AND_RETURN((ret < 0),	LCD_KIT_FAIL, ERR, "get sn_code failed\n");
		...
		LCD_CHECK_AND_RETURN((strlen(oem_data) + strlen(read_value) >= OEM_INFO_SIZE_MAX),
			LCD_KIT_FAIL, ERR, "oem_data is out of bound\n");
		ret = strncat_s(oem_data, OEM_INFO_SIZE_MAX, read_value, strlen(read_value));
		LCD_CHECK_AND_RETURN((ret < 0),	LCD_KIT_FAIL, ERR, "strncat_s fail\n");
	}
	...
	struct device_attribute *attr, char *buf)
{
	...
	for (i = 0; i < oem_info_data[1]; i++) {
		...
		len = snprintf(str_tmp, sizeof(str_tmp), "%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,",
		...
		strncat(str_oem, str_tmp, OEM_INFO_SIZE_MAX);
	}
	...
	struct device_attribute *attr, char *buf)
{
	...
	for (i = 0; i < sizeof(dinfo->sn_code); i++) {
		...
		strncat(oem_info, oem_info_tmp, OEM_INFO_SIZE_MAX - 1);
	}
...
static int fps_list_cat(char *str, unsigned int len)
{
	int ret;
	int i;
	int fps_rate;
	...
	LCD_CHECK_AND_RETURN((ret < 0), LCD_KIT_FAIL, ERR, "snprintf fail\n");
	for (i = 0; i < disp_info->fps.panel_support_fps_list.cnt; i++) {
		fps_rate = (int)disp_info->fps.panel_support_fps_list.buf[i];
		if (i > 0) {
			LCD_CHECK_AND_RETURN((strlen(str) + strlen(",") >= len),
				LCD_KIT_FAIL, ERR, "str is out of bound\n");
			strncat(str, ",", len);
		}
		ret = snprintf_s(tmp_str, sizeof(tmp_str), sizeof(tmp_str) - 1, "%d", fps_rate);
		LCD_CHECK_AND_RETURN((ret < 0),	LCD_KIT_FAIL, ERR, "snprintf fail\n");
		LCD_CHECK_AND_RETURN((strlen(str) + strlen(tmp_str) >= len),
			LCD_KIT_FAIL, ERR, "str is out of bound\n");
...
static int product_panel_num_cat(char *str, unsigned int len)
{
	int ret;
	int panel_id = lcd_kit_get_active_panel_id();
	...
	LCD_CHECK_AND_RETURN((strlen(str) + strlen(";product_panel_num:") >= len),
		LCD_KIT_FAIL, ERR, "str is out of bound\n");
	strncat(str, ";product_panel_num:", len);
	ret = snprintf(tmp_str, sizeof(tmp_str), "%d", panel_num);
	LCD_CHECK_AND_RETURN((ret < 0),	LCD_KIT_FAIL, ERR, "snprintf fail\n");
	LCD_CHECK_AND_RETURN((strlen(str) + strlen(tmp_str) >= len),
		LCD_KIT_FAIL, ERR, "str is out of bound\n");
	strncat(str, tmp_str, len);
	LCD_CHECK_AND_RETURN((strlen(str) + strlen(";current_panel_id:") >= len),
		LCD_KIT_FAIL, ERR, "str is out of bound\n");
	strncat(str, ";current_panel_id:", len);
...
static int panel_id_type_fps_cat(char *str, unsigned int len)
{
	int ret;
	int panel_id;
	int fps_rate;
	...
	for (i = 0; i < panel_num; i++) {
		panel_id = i;
		LCD_CHECK_AND_RETURN((strlen(str) + strlen(";panel_id:") >= len),
			LCD_KIT_FAIL, ERR, "str is out of bound\n");
		strncat(str, ";panel_id:", len);
		ret = snprintf_s(tmp_str, sizeof(tmp_str), sizeof(tmp_str) - 1, "%d", i);
		LCD_CHECK_AND_RETURN((ret < 0), LCD_KIT_FAIL, ERR, "snprintf fail\n");
		LCD_CHECK_AND_RETURN((strlen(str) + strlen(tmp_str) >= len),
			LCD_KIT_FAIL, ERR, "str is out of bound\n");
		strncat(str, tmp_str, len);
		LCD_CHECK_AND_RETURN((strlen(str) + strlen(";panel_type:") >= len),
			LCD_KIT_FAIL, ERR, "str is out of bound\n");
		strncat(str, ";panel_type:", len);
		get_oled_type(panel_id, oled_type, PANEL_MAX);
		LCD_CHECK_AND_RETURN((strlen(str) + strlen(oled_type) >= len),
			LCD_KIT_FAIL, ERR, "str is out of bound\n");
		strncat(str, oled_type, len);
		LCD_CHECK_AND_RETURN((strlen(str) + strlen(";support_fps:") >= len),
			LCD_KIT_FAIL, ERR, "str is out of bound\n");
		strncat(str, ";support_fps:", len);
		for (j = 0; j < disp_info->fps.panel_support_fps_list.cnt; j++) {
			fps_rate = (int)disp_info->fps.panel_support_fps_list.buf[j];
			if (j > 0) {
				LCD_CHECK_AND_RETURN((strlen(str) + strlen(",") >= len),
					LCD_KIT_FAIL, ERR, "str is out of bound\n");
				strncat(str, ",", len);
...
static int fps_scence_show(char *str, unsigned int len)
{
	int ret;

	ret = fps_list_cat(str, len);
	LCD_CHECK_AND_RETURN((ret == LCD_KIT_FAIL), LCD_KIT_FAIL, ERR, "fps_list_cat fail\n");
	ret = product_panel_num_cat(str, len);
	LCD_CHECK_AND_RETURN((ret == LCD_KIT_FAIL), LCD_KIT_FAIL, ERR, "product_panel_num_cat fail\n");
	ret = panel_id_type_fps_cat(str, len);
	LCD_CHECK_AND_RETURN((ret == LCD_KIT_FAIL), LCD_KIT_FAIL, ERR, "panel_id_type_fps_cat fail\n");
	return ret;
}
	...
	struct device_attribute *attr, char *buf)
{
	...
	if (disp_info->fps.support) {
		fps_scence_show(fps_str, LCD_REG_LENGTH_MAX - 1);
	} else {
	...
	struct device_attribute *attr, char *buf)
{
	...
	if (!disp_info->fps.support) {
		...
		for (i = 0; i < disp_info->fps.panel_support_fps_list.cnt; i++) {
			fps_rate = (int)disp_info->fps.panel_support_fps_list.buf[i];
			if (i == 0)
				strncat(fps_str, ",", LCD_REG_LENGTH_MAX - 1);
			else
				strncat(fps_str, ";", LCD_REG_LENGTH_MAX - 1);
			ret += snprintf(str_tmp, sizeof(str_tmp), "%d:%d", fps_rate, ORDER_DELAY);
			strncat(fps_str, str_tmp, LCD_REG_LENGTH_MAX - 1);
		}
	}
	LCD_KIT_INFO("%s\n", fps_str);
	ret = snprintf(buf, PAGE_SIZE, "%s\n", fps_str);
...
static void lcd_get_parse_info_data(char *parse_data, char *buf)
{
	...
	for (i = 0; i < PANEL_INFO_LIMIT; i++) {
		memset(str_tmp, 0, sizeof(str_tmp));
		len = snprintf(str_tmp, sizeof(str_tmp),
