	...
	int len)
{
	...
	for (i = 0; i < len && i < LCD_KIT_CHECKSUM_SIZE_DKMD; i++) {
...
static void send_fps_lock_cmd(int panel_id, unsigned int val)
{
	...
	struct fps_cmd_table fps_lock_cmd[] = {
		{ FPS_0p1, &disp_info->fps.fps_to_0p1_cmds },
		{ FPS_0p5, &disp_info->fps.fps_to_0p5_cmds },
	...
	unsigned char *read_value, int *error_count, int loop_num)
{
	...
	if (!error_count) {
		LCD_KIT_ERR("error_count is null\n");
		return;
	}
...
static int lcd_kit_aod_detect_setting(uint32_t panel_id, uint32_t mode)
{
	...
	switch (mode) {
	case COLOR_AOD_DETECT_ENTER:
		ret = lcd_kit_cmds_tx(panel_id, NULL,
			&FACT_INFO->color_aod_det.enter_cmds);
		break;
	case COLOR_AOD_DETECT_EXIT:
		ret = lcd_kit_cmds_tx(panel_id, NULL,
			&FACT_INFO->color_aod_det.exit_cmds);
		break;
	case FAC_AT_TEST_1DOT5_FPS:
		LCD_KIT_INFO("at set fps 0.5 hz\n");
		ret = lcd_kit_cmds_tx(panel_id, NULL,
			&FACT_INFO->color_aod_det.at_test_1dot5_fps_cmds);
		break;
	default:
...
struct ppr_cmds_table {
	unsigned int index;
	char ppr_str[MAX_BUF];
...
enum {
	...
	PPR_ON_1_30_INDEX = 4,
	PPR_ON_1_60_INDEX = 5,
	PPR_ON_1_120_INDEX = 6,
	PPR_FULL_SCREEN_INDEX = 7,
	PPR_1_1_INDEX = 2001001,
	PPR_1_10_INDEX = 2001010,
	PPR_1_30_INDEX = 2001030,
	PPR_1_60_INDEX = 2001060,
	PPR_1_120_INDEX = 2001120,
	PPR_10_60_INDEX = 2060,
	PPR_1_60_1_INDEX = 3060,
	PPR_1_120_1_INDEX = 3120,
	PPR_MAX_INDEX,
};

static int ppr_cmd_send(int panel_id, struct lcd_kit_dsi_panel_cmds *cmds)
{
	struct lcd_kit_adapt_ops *adapt_ops = NULL;
	adapt_ops = lcd_kit_get_adapt_ops();
	if (!adapt_ops) {
...
static ssize_t lcd_panel_part_refresh_show(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	int ret = LCD_KIT_FAIL;
	int table_num, i;
	int panel_id = lcd_kit_get_active_panel_id();
	unsigned int ppr_cmds_index = FACT_INFO->ppr.set_cmds_index;

	struct ppr_cmds_table ppr_cmds[] = {
		{PPR_OFF_INDEX,		"PANEL_PART_REFRESH_OFF",		&FACT_INFO->ppr.off_cmds},
		{PPR_ON_INDEX,		"PANEL_PART_REFRESH_ON",		&FACT_INFO->ppr.on_cmds},
		{PPR_ON_MMI_INDEX,	"PANEL_PART_REFRESH_ON_MMI",	&FACT_INFO->ppr.on_mmi_cmds},
		{PPR_ON_1_10_INDEX,	"PANEL_PART_REFRESH_ON_1_10",	&FACT_INFO->ppr.on_top1_bot10_cmds},
		{PPR_ON_1_30_INDEX,	"PANEL_PART_REFRESH_ON_1_30",	&FACT_INFO->ppr.on_top1_bot30_cmds},
		{PPR_ON_1_60_INDEX,	"PANEL_PART_REFRESH_ON_1_60",	&FACT_INFO->ppr.on_top1_bot60_cmds},
		{PPR_ON_1_120_INDEX,	"PANEL_PART_REFRESH_ON_1_120",		&FACT_INFO->ppr.on_top1_bot120_cmds},
		{PPR_FULL_SCREEN_INDEX,	"PANEL_PART_REFRESH_FULL_SCREEN",	&FACT_INFO->ppr.full_screen_cmds},
		{PPR_1_1_INDEX,		"PANEL_PART_REFRESH_1_1",		&FACT_INFO->ppr.top1_bot1_cmds},
		{PPR_1_10_INDEX,	"PANEL_PART_REFRESH_1_10",		&FACT_INFO->ppr.top1_bot10_cmds},
		{PPR_1_30_INDEX,	"PANEL_PART_REFRESH_1_30",		&FACT_INFO->ppr.top1_bot30_cmds},
		{PPR_1_60_INDEX,	"PANEL_PART_REFRESH_1_60",		&FACT_INFO->ppr.top1_bot60_cmds},
		{PPR_1_120_INDEX,	"PANEL_PART_REFRESH_1_120",		&FACT_INFO->ppr.top1_bot120_cmds},
		{PPR_10_60_INDEX,	"PANEL_PART_REFRESH_10_60",		&FACT_INFO->ppr.top10_bot60_cmds},
		{PPR_1_60_1_INDEX,	"PANEL_PART_REFRESH_1_60_1",	&FACT_INFO->ppr.top1_mid60_bot1_cmds},
		{PPR_1_120_1_INDEX,	"PANEL_PART_REFRESH_1_120_1",	&FACT_INFO->ppr.top1_mid120_bot1_cmds},
		{PPR_MAX_INDEX,		"NULL",							NULL},
	};

	table_num = sizeof(ppr_cmds) / sizeof(ppr_cmds[0]);
	for (i = 0; i < table_num - 1; i++) {
		if (ppr_cmds_index == ppr_cmds[i].index)
			break;
	}

	ret = ppr_cmd_send(panel_id, ppr_cmds[i].cmds);
	if (ret) {
		ret = snprintf_s(buf, PAGE_SIZE, PAGE_SIZE - 1,
			"fail: lcd_panel_part_refresh_show, ppr_cmd_send error, cmds:%s, index:%u.\n",
			ppr_cmds[i].ppr_str, ppr_cmds[i].index);
		LCD_KIT_ERR("%s\n", buf);
		return ret;
	}

	FACT_INFO->ppr.current_cmds_index = ppr_cmds_index;

	ret = snprintf_s(buf, PAGE_SIZE, PAGE_SIZE - 1,
		"succ: lcd_panel_part_refresh_show, panel:%d, cmds:%s, current_cmds_index:%u\n",
		panel_id, ppr_cmds[i].ppr_str, FACT_INFO->ppr.current_cmds_index);

	return ret;
}

static ssize_t lcd_panel_part_refresh_store(struct device *dev,
	struct device_attribute *attr, const char *buf, size_t count)
{
	ssize_t ret;
	unsigned int ppr_cmds_index = 0;
	int panel_id = lcd_kit_get_active_panel_id();
	...
	/* decimal  parsing */
	ret = kstrtouint(buf, 10, &ppr_cmds_index);
	if (ret) {
		LCD_KIT_ERR("str to int fail\n");
		return LCD_KIT_FAIL;
	}

	FACT_INFO->ppr.set_cmds_index = ppr_cmds_index;
	LCD_KIT_INFO("ppr set_cmds_index:%u\n", FACT_INFO->ppr.set_cmds_index);
...
struct lcd_fact_ops g_fact_ops = {
	...
	.panel_part_refresh_show = lcd_panel_part_refresh_show,
	.panel_part_refresh_store = lcd_panel_part_refresh_store,
};
