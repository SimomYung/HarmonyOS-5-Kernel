...
enum display_engine_module_id {
	...
	DISPLAY_ENGINE_DDIC_DC = BIT(14),
	DISPLAY_ENGINE_SAFE_FRAME_RATE = BIT(15),
	DISPLAY_ENGINE_DEEP_CALIBRATION = BIT(16),
	DISPLAY_ENGINE_PPU_MULTI_REGION_INFO = BIT(17),
	DISPLAY_ENGINE_PPU_SAFE_FRAME_RATE = BIT(18),
	DISPLAY_ENGINE_PPU_ENTRY_RESTRICTION_CONFIG = BIT(19),
	DISPLAY_ENGINE_DMD_REPORT = BIT(20),
};
...
struct display_engine_panel_info {
	...
	int oled_type;
	int elvss_support;
	bool is_part_update_support;
	int ddic_type;
	int panel_part_update_support;
};
...
/* multi region info of panel partial refresh */
struct display_engine_ppu_multi_region_info {
	unsigned int region_top;
	unsigned int region_bottom;
	bool is_in;
};

/* entry restriction config of panel partial refresh */
struct display_engine_ppu_entry_restriction_config {
	unsigned int poweron_delay_frames;
	unsigned int applist_brightness_dbv;
	unsigned int nonapplist_brightness_dbv;
	bool temp_compensation_support;
	unsigned int minimum_limit_rows;
};

struct display_engine_dmd_info {
	unsigned int dmd_id;
	char content[DMD_MSG_LENGTH_MAX];
};
...
struct display_engine_param {
	...
	bool ddic_dc_enable;
	unsigned int safe_frame_rate;
	unsigned int safe_partial_frame_rate; // for safe framerate
	bool ddic_deep_calibration_enable;
	struct display_engine_ppu_multi_region_info ppu_multi_region_info; // for panel partial update
	unsigned int ppu_safe_frame_rate; // for panel partial update
	struct display_engine_ppu_entry_restriction_config ppu_entry_restriction_config;
	struct display_engine_dmd_info dmd_report_info;
};
