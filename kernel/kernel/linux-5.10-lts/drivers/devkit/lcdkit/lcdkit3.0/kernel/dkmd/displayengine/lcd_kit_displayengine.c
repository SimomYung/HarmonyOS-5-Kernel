...
static int display_engine_set_ddic_dc(struct display_engine_param* de_param);
static int display_engine_set_safe_frame_rate(struct display_engine_param* de_param);
static int display_engine_set_ddic_deep_calibration(struct display_engine_param* de_param);
static int display_engine_set_multi_region_info(struct display_engine_param *de_param);
static int display_engine_set_ppu_safe_frame_rate(struct display_engine_param *de_param);
static int display_engine_ppu_entry_restriction_config_get(uint32_t panel_id, struct display_engine_ppu_entry_restriction_config *config);
static int display_engine_check_and_report_dmd(struct display_engine_dmd_info* dmd_info);
static int display_engine_set_dmd_report(struct display_engine_param *de_param);
...
int display_engine_set_param_safe_frame_rate(int panel_id, uint32_t safe_frame_rate, uint32_t safe_partial_frame_rate)
{
	int ret = LCD_KIT_OK;
	LCD_KIT_INFO("safe frame rate:%u safe partial frame rate:%u panel id:%d\n",
		safe_frame_rate, safe_partial_frame_rate, panel_id);
	ret = lcd_kit_brightness_trigger_send_sfr(panel_id, safe_frame_rate, safe_partial_frame_rate);
	return ret;
}

static int display_engine_set_safe_frame_rate(struct display_engine_param* de_param)
{
	if (de_param->modules & DISPLAY_ENGINE_SAFE_FRAME_RATE) {
		if (display_engine_set_param_safe_frame_rate(de_param->panel_id,
			de_param->safe_frame_rate, de_param->safe_partial_frame_rate)) {
			LCD_KIT_WARNING("display_engine_set_safeframe failed!\n");
			return -1;
		}
	}
	return 0;
...
static int display_engine_set_ddic_temper(struct display_engine_param* de_param)
{
	if (de_param->modules & DISPLAY_ENGINE_DDIC_TEMPER) {
		bool temper_enable = (de_param->ddic_temper_enable == 1);
		if (display_engine_set_param_panel_temper(de_param->panel_id, temper_enable)) {
...
static int display_engine_set_ddic_deep_calibration(struct display_engine_param* de_param)
{
	if (de_param->modules & DISPLAY_ENGINE_DEEP_CALIBRATION) {
		if (display_engine_set_param_panel_deep_calibration(de_param->panel_id, de_param->ddic_deep_calibration_enable)) {
			LCD_KIT_WARNING("display_engine_set_param_panel_deep_calibration() failed!\n");
			return -1;
		}
	}
	return 0;
...
static int display_engine_check_and_report_dmd(struct display_engine_dmd_info* dmd_report_info)
{
	if (dmd_report_info->dmd_id < DSM_DEEPCALIB_DEMURA_ERROR ||
		dmd_report_info->dmd_id > DSM_DEEPCALIB_OTHER_ERROR) {
		LCD_KIT_WARNING("invalid dmd id %d!\n", dmd_report_info->dmd_id);
		return -1;
	}
	if (strlen(dmd_report_info->content) >= DMD_MSG_LENGTH_MAX) {
		LCD_KIT_WARNING("invalid content length %d!\n", strlen(dmd_report_info->content));
		return -1;
	}
#if (defined(CONFIG_HUAWEI_DSM) || defined(CONFIG_HUAWEI_OHOS_DSM))
	if (dsm_client_ocuppy(lcd_dclient)) {
		LCD_KIT_WARNING("display_engine_set_dmd_report() failed!\n");
		return -1;
	}
	LCD_KIT_INFO("dmd report, id: %d content: %s!\n",
		dmd_report_info->dmd_id, dmd_report_info->content);
	dsm_client_record(lcd_dclient, dmd_report_info->content);
	dsm_client_notify(lcd_dclient, dmd_report_info->dmd_id);
#endif
	return 0;
}
...
static int display_engine_set_dmd_report(struct display_engine_param *de_param)
{
	if (de_param->modules & DISPLAY_ENGINE_DMD_REPORT) {
		if (display_engine_check_and_report_dmd(&de_param->dmd_report_info)) {
			LCD_KIT_WARNING("display_engine_check_and_report_dmd() failed!\n");
			return -1;
		}
	}
	return 0;
...
static int display_engine_set_ddic_dc(struct display_engine_param* de_param)
{
	if (de_param->modules & DISPLAY_ENGINE_DDIC_DC) {
		LCD_KIT_INFO("display_engine_set_param dc\n");
		if (display_engine_set_param_panel_dc(de_param->panel_id, de_param->ddic_dc_enable)) {
			LCD_KIT_WARNING("display_engine_set_param_dc() failed!\n");
			return -1;
		}
	}
	return 0;
...
static int display_engine_set_multi_region_info(struct display_engine_param *de_param)
{
	if (de_param->modules & DISPLAY_ENGINE_PPU_MULTI_REGION_INFO) {
		if (lcd_kit_set_multi_region_info(de_param->panel_id, de_param->ppu_multi_region_info.region_top,
                de_param->ppu_multi_region_info.region_bottom, de_param->ppu_multi_region_info.is_in)) {
			LCD_KIT_WARNING("display_engine_set_multi_region_info() failed!\n");
			return -1;
		}
	}
	return 0;
...
int display_engine_set_ppu_sfr(int panel_id, uint32_t safe_frame_rate)
{
	int ret = LCD_KIT_OK;
	...
	ret = lcd_kit_ppu_trigger_send_sfr(panel_id, safe_frame_rate);
	return ret;
}

static int display_engine_set_ppu_safe_frame_rate(struct display_engine_param *de_param)
{
	if (de_param->modules & DISPLAY_ENGINE_PPU_SAFE_FRAME_RATE) {
		LCD_KIT_INFO("display_engine_set_ppu_safe_frame_rate: %d\n", de_param->ppu_safe_frame_rate);
		if (display_engine_set_ppu_sfr(de_param->panel_id, de_param->ppu_safe_frame_rate)) {
			LCD_KIT_WARNING("display_engine_set_ppu_safe_frame_rate() failed!\n");
			return -1;
		}
	}
	return 0;
...
static const display_engine_set_func set_func_list[] = {
	...
	display_engine_set_ddic_dc,
	display_engine_set_safe_frame_rate,
	display_engine_set_ddic_deep_calibration,
	display_engine_set_multi_region_info,
	display_engine_set_ppu_safe_frame_rate,
	display_engine_set_dmd_report,
};
...
int display_engine_get_param(void __user *argp)
{
	...
	struct display_engine_param de_param = {0};
	int ret = 0;
	if (!argp) {
	...
	if (de_param.modules & DISPLAY_ENGINE_PPU_ENTRY_RESTRICTION_CONFIG) {
		if (display_engine_ppu_entry_restriction_config_get(de_param.panel_id, &de_param.ppu_entry_restriction_config)) {
			LCD_KIT_WARNING("display_engine_ppu_entry_restriction_config_get() failed!\n");
			return -1;
		}
	}
...
static void print_sn_code(unsigned char sn_code[SN_CODE_LEN_MAX], unsigned int sn_code_length)
{
	...
	for (i = 0; i < sn_code_length; i++) {
		ret = sprintf_s(sn_text + (i << 1), SN_TEXT_SIZE, "%02x", sn_code[i]);
		if (ret == -1) {
			LCD_KIT_ERR("failed to get sn_code\n");
			return;
		}
	}
...
static int display_engine_panel_info_get(uint32_t panel_id, struct display_engine_panel_info *info)
{
	...
	if (!panel_info || !info || panel_id >= MAX_PANEL) {
		LCD_KIT_ERR("point is null or not support panel_id=%d\n", panel_id);
		return LCD_KIT_FAIL;
	}
	...
	info->is_part_update_support = common_info->dirty_region.support;
	info->ddic_type = panel_info->dfr_info.ddic_type;
	info->panel_part_update_support = common_info->part_refresh.support;

	LCD_KIT_INFO("panel_id=%u res(w=%u,h=%u) bl(min=%u,max=%u) name=%s local_hbm_support=%d oled_type=%d "
		"elvss_support=%d part_update=%d ddic_type=%d ppu_support=%d\n",
		panel_id, info->width, info->height, info->min_backlight, info->max_backlight, info->panel_name,
		info->local_hbm_support, info->oled_type, info->elvss_support, info->is_part_update_support,
		info->ddic_type, info->panel_part_update_support);
	return LCD_KIT_OK;
}
...
void display_engine_brightness_update(uint32_t panel_id, unsigned int bl_level)
{
	display_engine_temper_update_with_brightness(panel_id, bl_level);
	display_engine_local_hbm_brightness_update(bl_level);
	display_engine_sfr_update_with_brightness(panel_id, bl_level);
}
...
void lcd_kit_display_engine_param_init(int panel_id, struct device_node *np)
{
	...
	/* parse panel dc param */
	lcd_kit_parse_panel_dc(panel_id, np);
	/* parse safe frame decision info */
	lcd_kit_safe_frm_init(panel_id, np);
	safe_frm_rate_work_info_workqueue_init(panel_id);
	/* parse elvss ctrl info */
	lcd_kit_parse_el_ctrl_info(panel_id, np);
	/* parse elvss ctrl info 2.0 */
	lcd_kit_parse_el_ctrl_info_ver_two(panel_id, np);
	display_engine_temper_cmds_parse(panel_id, np);
}
...
static int display_engine_ppu_entry_restriction_config_get (uint32_t panel_id,
	struct display_engine_ppu_entry_restriction_config *config)
{
	if (!config || panel_id >= MAX_PANEL)
	{
		LCD_KIT_ERR("not support panel_id=%d\n", panel_id);
		return LCD_KIT_FAIL;
	}
	config->poweron_delay_frames = common_info->part_refresh.poweron_delay_frames;
	config->applist_brightness_dbv = common_info->part_refresh.applist_brightness_dbv;
	config->nonapplist_brightness_dbv = common_info->part_refresh.non_applist_brightness_dbv;
 	config->temp_compensation_support = (common_info->part_refresh.temp_compensation_support == 1);
	config->minimum_limit_rows = common_info->part_refresh.minimum_limit_rows;
	
	LCD_KIT_INFO("ppu enter restrictions: poweron delay frames = %d, applist brightness dbv = %d, "
		"nonapplist brightness dbv = %d, temp compensation support = %d, minimum limit rows = %d\n",
		config->poweron_delay_frames, config->applist_brightness_dbv, config->nonapplist_brightness_dbv,
		config->temp_compensation_support, config->minimum_limit_rows);
	return LCD_KIT_OK;
} 