...
static unsigned int set_lcd_power_ic_switch_gpio(int panel_id)
{
	if (power_hdl->lcd_power_ic_switch.buf == NULL) {
		LCD_KIT_ERR("null pointer\n");
		return LCD_KIT_OK;
	}
	return power_hdl->lcd_power_ic_switch.buf[POWER_NUM];
}

static unsigned int set_lcd_integ_switch_gpio(int panel_id)
{
	if (power_hdl->lcd_integ_switch.buf == NULL) {
		LCD_KIT_ERR("null pointer\n");
		return LCD_KIT_OK;
	}
	return power_hdl->lcd_integ_switch.buf[POWER_NUM];
}

static unsigned int set_lcd_integ_en_gpio(int panel_id)
{
	if (power_hdl->lcd_integ_en.buf == NULL) {
		LCD_KIT_ERR("null pointer\n");
		return LCD_KIT_OK;
	}
	return power_hdl->lcd_integ_en.buf[POWER_NUM];
}
...
static lcd_gpio_event_table g_lcd_gpio_event_table[] = {
		...
		{ LCD_KIT_POWER_IC_SWITCH, set_lcd_power_ic_switch_gpio },
		{ LCD_KIT_INTEG_SWITCH, set_lcd_integ_switch_gpio },
		{ LCD_KIT_INTEG_EN, set_lcd_integ_en_gpio },
	};
...
/* some event need exec even panel is power on, like mipi switch and poweric integ switch */
static bool need_exec_power_on(unsigned int type)
{
	switch (type) {
	case LCD_KIT_MIPI_SWITCH:
	case LCD_KIT_MIPI_SWITCH1:
	case LCD_KIT_MIPI_SWITCH2:
	case LCD_KIT_POWER_IC_SWITCH:
	case LCD_KIT_INTEG_SWITCH:
		return true;
	default:
		break;
	}
...
int lcd_kit_gpio_tx(int panel_id, unsigned int type, unsigned int op)
{
	...
	if (lcd_kit_get_aod_panel_state(panel_id) && !need_exec_power_on(type) && !is_in_power_off_seq(panel_id)) {
...
static void cmds_tx_delay(int wait, int waittype)
{
	if (wait) {
		if (waittype == WAIT_TYPE_US)
			...
			mdelay((unsigned long)(wait * 1000));  /* ms */
	}
}
...
static int lcd_kit_pmu_ctrl_primary_vci(int panel_id, uint32_t enable)
{
	...
	if (enable == 1) {
		...
		vdd_set_cmds->min_uv = (int)enable;
		vdd_set_cmds->max_uv = (int)enable;
...
static int lcd_kit_pmu_ctrl_secondary_iovcc(int panel_id, uint32_t enable)
{
	...
	if (enable == 1) {
		...
		sec_vdd_set_cmds->min_uv = (int)enable;
		sec_vdd_set_cmds->max_uv = (int)enable;
		ret = lcd_kit_vcc_cmds_tx(NULL, sec_vdd_set_cmds,
			ARRAY_SIZE(sec_vdd_set_cmds), panel_id);
	}
...
unsigned int lcd_kit_poweric_detect(struct device_node *np, unsigned int poweric_gpio)
{
	...
	poweric_gpio_val = (unsigned int)gpio_get_value(gpio);
	...
	unsigned int gpio_type)
{
	...
	gpio_value = (unsigned int)gpio_get_value(gpio);
	gpio_free(gpio);
 ...
 */
#ifdef LCD_KIT_DEBUG_ENABLE
static int dpd_vsp_init(struct platform_device *pdev, int panel_id)
{
	int ret;
	LCD_KIT_INFO("enter dpd_vsp_init\n");
	if (power_hdl->lcd_vsp.buf &&
		power_hdl->lcd_vsp.buf[POWER_MODE] == REGULATOR_MODE) {
		/* lcd scharger vcc get */
		ret = lcd_kit_vcc_cmds_tx(pdev, bias_get_cmds,
			ARRAY_SIZE(bias_get_cmds), panel_id);
		if (ret != 0) {
			LCD_KIT_ERR("LCD scharger vcc get failed!\n");
			return ret;
		}
		/* init bias/vsp/vsn */
		lcd_kit_power_set(panel_id, bias_set_cmds, ARRAY_SIZE(bias_set_cmds));
		/* set scharger vcc */
		ret = lcd_kit_vcc_cmds_tx(NULL, bias_set_cmds,
			ARRAY_SIZE(bias_set_cmds), panel_id);
		if (ret != 0) {
...
static int dpd_bl_init(struct platform_device *pdev, int panel_id)
{
	int ret;
	if (power_hdl->lcd_backlight.buf &&
		power_hdl->lcd_backlight.buf[POWER_MODE] == REGULATOR_MODE) {
		/* lcd scharger vcc get */
		ret = lcd_kit_vcc_cmds_tx(pdev, bl_get_cmds, ARRAY_SIZE(bl_get_cmds), panel_id);
		if (ret != 0) {
			LCD_KIT_ERR("LCD scharger vcc get failed!\n");
			return ret;
		}
...
static int lcd_power_init(struct platform_device *pdev, int panel_id, 
	struct vcc_desc *init_cmds, struct lcd_kit_array_data *power, int len) 
{
	int ret = LCD_KIT_FAIL;
	if (!power) {
		LCD_KIT_ERR("init cmds is null\n");
		return ret;
	}
	if (power->buf && power->buf[POWER_MODE] == REGULATOR_MODE) {
		/* judge power is valid */
		if (power->buf[POWER_NUM] > 0) {
			lcd_kit_power_set(panel_id, init_cmds, len);
			ret = lcd_kit_vcc_cmds_tx(pdev, init_cmds, len, panel_id);
			if (ret != 0) 
				return ret;
		}
	}
...
static int dpd_vdd_init(struct platform_device *pdev, int panel_id) 
{
	int ret = 0;
	int len = ARRAY_SIZE(vdd_init_cmds);
	ret = lcd_power_init(pdev, panel_id, vdd_init_cmds, &power_hdl->lcd_vdd, len);
	if (ret != 0) {
		LCD_KIT_ERR("LCD vdd init failed!\n");
	}
	return ret;
...
static int dpd_iovcc_init(struct platform_device *pdev, int panel_id) 
{
	int ret = 0;
	int len = ARRAY_SIZE(iovcc_init_cmds);
	ret = lcd_power_init(pdev, panel_id, iovcc_init_cmds, &power_hdl->lcd_iovcc, len);
	if (ret != 0) {
		LCD_KIT_ERR("LCD iovcc init failed!\n");
	}
	return ret;
...
static int dpd_vci_init(struct platform_device *pdev, int panel_id) 
{
	int ret = 0;
	int len = ARRAY_SIZE(vci_init_cmds);
	ret = lcd_power_init(pdev, panel_id, vci_init_cmds, &power_hdl->lcd_vci, len);
	if (ret != 0) {
		LCD_KIT_ERR("LCD iovcc init failed!\n");
	}
	return ret;
...
int dpd_regu_init(struct platform_device *pdev)
{
	...
	LCD_KIT_INFO("enter dpd_regu_init\n");
	if (!pdev) {
		LCD_KIT_ERR("pdev is null\n");
		return LCD_KIT_FAIL;
	}
	ret = dpd_vsp_init(pdev, panel_id);
	if (ret) {
		LCD_KIT_ERR("dpd vsp init fail\n");
		return LCD_KIT_FAIL;
	}
	ret = dpd_bl_init(pdev, panel_id);
	if (ret) {
		LCD_KIT_ERR("dpd bl init fail\n");
		return LCD_KIT_FAIL;
	}
	ret = dpd_vdd_init(pdev, panel_id);
	if (ret) {
		LCD_KIT_ERR("dpd vdd init fail\n");
		return LCD_KIT_FAIL;
	}
	ret = dpd_iovcc_init(pdev, panel_id);
	if (ret) {
		LCD_KIT_ERR("dpd iovcc init fail\n");
		return LCD_KIT_FAIL;
	}
	ret = dpd_vci_init(pdev, panel_id);
	if (ret) {
