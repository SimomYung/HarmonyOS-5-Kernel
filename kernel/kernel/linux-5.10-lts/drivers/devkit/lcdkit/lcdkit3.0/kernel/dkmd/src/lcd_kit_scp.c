...
#define LCD_KIT_DEFAULT_MAX_BATTERY_VOLTAGE 4500000
#define LCD_KIT_DEFAULT_LOW_BATTERY_VOLTAGE 3300000

#if (defined(CONFIG_HUAWEI_DSM) || defined(CONFIG_HUAWEI_OHOS_DSM))
static void lcd_kit_scp_dmd_report(int panel_id, char read_value, uint32_t battery_voltage)
{
	int ret;
	char dmd_info[DMD_DET_ERR_LEN] = {0};

	ret = snprintf_s(dmd_info, DMD_DET_ERR_LEN, DMD_DET_ERR_LEN -1,
		"lcd short circuit error: panel_id=%d, read_value=0x%x, battery_voltage=%duV\n",
		panel_id, read_value, battery_voltage);
	if (ret < 0) {
...
static void lcd_kit_set_short_circuit_state(int panel_id, uint32_t status, char read_value, uint32_t battery_voltage)
{
...
#if (defined(CONFIG_HUAWEI_DSM) || defined(CONFIG_HUAWEI_OHOS_DSM))
	lcd_kit_scp_dmd_report(panel_id, read_value, battery_voltage);
#endif
...
static int lcd_kit_get_vbat_voltage(void)
{
	int battery_voltage = 0;
	power_supply_get_int_property_value(POWER_PLATFORM_BAT_PSY_NAME,
		POWER_SUPPLY_PROP_VOLTAGE_NOW, &battery_voltage);

	return battery_voltage;
}

static void lcd_kit_vbat_discontinuous_read(int panel_id, long delta_time)
{
	/* avoid continuous read three times */
	if (delta_time > disp_info->short_circuit_protection.max_continuous_read_time) {
		disp_info->short_circuit_protection.battery_voltage = (u32)lcd_kit_get_vbat_voltage();
		LCD_KIT_INFO("battery_voltage = %duV delta_time = %ums\n",
			disp_info->short_circuit_protection.battery_voltage, delta_time);
	}
}
...
static int lcd_kit_scp_strategy(int panel_id, u32 *short_circuit_value, char *read_value)
{
	...
	for (i = 0; i < disp_info->short_circuit_protection.value.cnt; i++) {
		...
		lcd_kit_vbat_discontinuous_read(panel_id, delta_time);

		if (disp_info->short_circuit_protection.battery_voltage <
			disp_info->short_circuit_protection.short_circuit_vbat_thd) {
			LCD_KIT_INFO("battery voltage is lower than %duV, skip scp\n",
				disp_info->short_circuit_protection.short_circuit_vbat_thd);
			continue;
		}

		/* enter LVD accumulate 1 points */
		disp_info->short_circuit_protection.short_circuit_cnt++;
		LCD_KIT_INFO("cur_tv.tv_sec=%ds, record_tv->tv_sec=%ds delta_time=%ums short_circuit_cnt=%d\n",
			cur_tv.tv_sec, record_tv->tv_sec, delta_time, disp_info->short_circuit_protection.short_circuit_cnt);
		/* LVD occurs again after the restart, DDIC's LVD self protection ability, avoid reset repeatedly */
		if (delta_time < disp_info->short_circuit_protection.self_protection_time && delta_time >
			...
			disp_info->short_circuit_protection.short_circuit_cnt <
			(int)disp_info->short_circuit_protection.short_circuit_cnt_thd)
			continue;
		...
		if (disp_info->short_circuit_protection.short_circuit_cnt >=
			...
			lcd_kit_set_short_circuit_state(panel_id, true, read_value[i],
				disp_info->short_circuit_protection.battery_voltage);
...
static void lcd_kit_panel_parse_scp_cmds(int panel_id, const struct device_node *np)
{
	...
	/* short circuit vbat thd, default thd is 3.3v */
	lcd_kit_parse_u32(np, "lcd-kit,short-circuit-vbat-thd",
		&disp_info->short_circuit_protection.short_circuit_vbat_thd, LCD_KIT_DEFAULT_LOW_BATTERY_VOLTAGE);

	/* vbat default value 4.5v */
	disp_info->short_circuit_protection.battery_voltage = LCD_KIT_DEFAULT_MAX_BATTERY_VOLTAGE;
