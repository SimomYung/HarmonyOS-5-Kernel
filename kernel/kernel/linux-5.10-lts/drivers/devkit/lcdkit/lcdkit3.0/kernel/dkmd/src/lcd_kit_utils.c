...
{
	lcd_kit_parse_dcs_cmds(np, "lcd-kit,fps-to-0p1-cmds", "lcd-kit,fps-to-0p1-cmds-state",
		&disp_info->fps.fps_to_0p1_cmds);
	lcd_kit_parse_dcs_cmds(np, "lcd-kit,fps-to-0p5-cmds", "lcd-kit,fps-to-0p5-cmds-state",
		&disp_info->fps.fps_to_0p5_cmds);
	...
	lcd_kit_parse_dcs_cmds(np, "lcd-kit,refresh-cmds-ext", "lcd-kit,refresh-cmds-state",
		&disp_info->fps.refresh_cmds_ext);
...
static void lcd_kit_init_display(int panel_id, struct device_node *np)
{
	...
	if (panel_id == PRIMARY_PANEL)
		disp_info->lcd_power_seq = (unsigned int)PUBLIC_INFO->lcd_power_seq;
	else
		disp_info->lcd_power_seq = (unsigned int)PUBLIC_INFO->sec_lcd_power_seq;
	...
	struct dpu_panel_info *pinfo)
{
	...
	lcd_kit_parse_u32(np, "lcd-kit,mipi-dsi-bit-clk-val1", &pinfo->mipi.dsi_bit_clk_val1, 0);
	lcd_kit_parse_u32(np, "lcd-kit,mipi-dsi-bit-clk-val2", &pinfo->mipi.dsi_bit_clk_val2, 0);
	lcd_kit_parse_u32(np, "lcd-kit,mipi-dsi-bit-clk-val3", &pinfo->mipi.dsi_bit_clk_val3, 0);
	lcd_kit_parse_u32(np, "lcd-kit,mipi-dsi-bit-clk-val4", &pinfo->mipi.dsi_bit_clk_val4, 0);
	lcd_kit_parse_u32(np, "lcd-kit,mipi-dsi-bit-clk-val5", &pinfo->mipi.dsi_bit_clk_val5, 0);
	lcd_kit_parse_u32(np, "lcd-kit,mipi-vactive-timeout", &pinfo->mipi.vactive_timeout_thr, 0);
	lcd_kit_parse_u32(np, "lcd-kit,mipi-data-continue-timeout", &pinfo->mipi.data_continue_timeout_thr, 0);
...
static void lcd_kit_get_el_ctrl_cmds(int panel_id, unsigned int bl_level, struct dpu_elvss_state *el_ctrl_state)
{
	...
	if (common_info->el_ctrl_info.ddic_type == DDIC_TYPE_HXX) {
		...
		common_info->el_ctrl_info.vint2_ctrl_cmds.cmds[2].payload[1] =
			((unsigned char)common_info->el_ctrl_info.vint2_ctrl_cmds.cmds[2].payload[1] & 0xFC) |
...
static void lcd_kit_set_el_ctrl_state(int panel_id, int bl_level, struct dpu_elvss_state *el_ctrl_state)
{
	...
	el_ctrl_state->bl = (uint32_t)bl_level;
}
...
void lcd_kit_set_bl_cmds(int panel_id, int bl_level, struct lcd_kit_dsi_panel_cmds *bl_panel_cmd)
{
	...
	if (!bl_panel_cmd || !bl_panel_cmd->cmds) {
	...
	case BL_BIG_ENDIAN:
		if (common_info->backlight.bl_max <= 0xFF) {
			bl_panel_cmd->cmds[index].payload[1] = bl_level;
		} else if (common_info->backlight.ext_flag) {
			/* change bl level to dsi cmds */
			bl_panel_cmd->cmds[index].payload[1] = common_info->backlight.write_offset;
			bl_panel_cmd->cmds[index].payload[2] = ((unsigned int)bl_level >> 8) & 0xFF;
			bl_panel_cmd->cmds[index].payload[3] = (unsigned int)bl_level & 0xFF;
		} else {
			/* change bl level to dsi cmds */
			bl_panel_cmd->cmds[index].payload[1] = (((unsigned int)bl_level >> 8) & 0xFF) | common_info->backlight.write_offset;
			bl_panel_cmd->cmds[index].payload[2] = (unsigned int)bl_level & 0xFF;
		}
		break;
	case BL_LITTLE_ENDIAN:
		if (common_info->backlight.bl_max <= 0xFF) {
			...
			/* change bl level to dsi cmds */
			bl_panel_cmd->cmds[index].payload[1] = (unsigned int)bl_level & 0xFF;
			bl_panel_cmd->cmds[index].payload[2] = ((unsigned int)bl_level >> 8) & 0xFF;
			bl_panel_cmd->cmds[index].payload[3] = common_info->backlight.write_offset;
		} else {
			/* change bl level to dsi cmds */
			bl_panel_cmd->cmds[index].payload[1] = (unsigned int)bl_level & 0xFF;
			bl_panel_cmd->cmds[index].payload[2] = (((unsigned int)bl_level >> 8) & 0xFF) | common_info->backlight.write_offset;
		}
		break;
	default:
	...
	struct device_node *np, struct dpu_panel_info *pinfo)
{
	...
	pinfo->mipi.mipi_frm_rate_mode_num = (uint32_t)disp_info->fps.panel_support_fps_list.cnt;
	for (i = 0; i < (int)pinfo->mipi.mipi_frm_rate_mode_num; i++) {
	...
	struct device_node *np, struct dpu_panel_info *pinfo)
{
	...
	for (i = 0; i < cmd_num; i++) {
		pinfo->dfr_info.oled_info.ltpo_info.dsi_cmds[i].cmd_num = (uint32_t)ltpo_base_cmd[i].cmd_cnt;
	...
	struct dpu_panel_info *pinfo, unsigned int frame_rate, int num)
{
	...
	for (i = 0; i < cmd_num; i++) {
		if (frame_rate == (unsigned int)fps_safe_cmd[i].val) {
			pinfo->dfr_info.oled_info.ltpo_info.safe_frm_rates[num].dsi_cmds.cmd_num = (uint32_t)fps_safe_cmd[i].cmds->cmd_cnt;
	...
	struct device_node *np, struct dpu_panel_info *pinfo)
{
	...
	pinfo->dfr_info.oled_info.ltpo_info.safe_frm_rates_num = (uint32_t)safe_freq.cnt;
	...
	struct device_node *np, struct dpu_panel_info *pinfo)
{
	...
	if ((lcd_kit_parse_array_data(np, "lcd-kit,dimming-seq", &dimming_seq) == LCD_KIT_OK) &&
		...
		pinfo->dfr_info.oled_info.ltpo_info.dimming_sequence[DIMMING_SEQ_120HZ_INDEX].dimming_seq_num =
			(uint32_t)dimming_seq.cnt;
		for (i = 0; i < dimming_seq.cnt && dimming_seq.buf != NULL; i++) {
			pinfo->dfr_info.oled_info.ltpo_info.dimming_sequence[DIMMING_SEQ_120HZ_INDEX].dimming_seq_list[i].frm_rate =
				dimming_seq.buf[i];
			pinfo->dfr_info.oled_info.ltpo_info.dimming_sequence[DIMMING_SEQ_120HZ_INDEX].dimming_seq_list[i].repeat_num =
				repeat_num.buf[i];
	...
	struct device_node *np, struct dpu_panel_info *pinfo)
{
	...
	if ((lcd_kit_parse_array_data(np, "lcd-kit,90hz-dimming-seq", &dimming_seq) == LCD_KIT_OK) &&
		...
		pinfo->dfr_info.oled_info.ltpo_info.dimming_sequence[DIMMING_SEQ_90HZ_INDEX].dimming_seq_num =
			(uint32_t)dimming_seq.cnt;
		for (i = 0; i < dimming_seq.cnt && dimming_seq.buf != NULL; i++) {
			pinfo->dfr_info.oled_info.ltpo_info.dimming_sequence[DIMMING_SEQ_90HZ_INDEX].dimming_seq_list[i].frm_rate =
				dimming_seq.buf[i];
			pinfo->dfr_info.oled_info.ltpo_info.dimming_sequence[DIMMING_SEQ_90HZ_INDEX].dimming_seq_list[i].repeat_num =
				repeat_num.buf[i];
	...
	struct dpu_panel_info *pinfo)
{
	...
	pinfo->dfr_info.oled_info.ltpo_info.te_freq_num = (uint32_t)te_freq.cnt;
	pinfo->dfr_info.oled_info.ltpo_info.te_mask_num = (uint32_t)te_mask.cnt;
	...
	struct device_node *np, struct dpu_panel_info *pinfo)
{
	...
	pinfo->dfr_info.oled_info.fps_sup_num = (uint32_t)disp_info->fps.panel_support_fps_list.cnt;
	LCD_KIT_INFO("pinfo->dfr_info.oled_info.fps_sup_num %d\n",
		pinfo->dfr_info.oled_info.fps_sup_num);
	for (i = 0; i < (int)pinfo->dfr_info.oled_info.fps_sup_num; i++) {
...
static void lcd_kit_parse_panel_dieid(int panel_id, struct device_node *np)
{
	...
	if (disp_info->panel_dieid_info.support) {
		...
		lcd_kit_parse_u32(np, "lcd-kit,dieid-move-forwar-support",
		&disp_info->panel_dieid_info.move_forwar_support, 0);

		if (disp_info->panel_dieid_info.move_forwar_support) {
			if (panel_id == PRIMARY_PANEL)
				ret = memcpy_s(disp_info->panel_dieid_info.dieid_info_data, PANEL_DIEID_DATA_SIZE_MAX,
					PUBLIC_INFO->dieid_info, PANEL_DIEID_DATA_SIZE_MAX);
			else
				ret = memcpy_s(disp_info->panel_dieid_info.dieid_info_data, PANEL_DIEID_DATA_SIZE_MAX,
					PUBLIC_INFO->sec_dieid_info, PANEL_DIEID_DATA_SIZE_MAX);
			if (ret != LCD_KIT_OK) {
	...
	struct dpu_panel_info *pinfo)
{
	if (common_info->dirty_region.support) {
		...
		lcd_kit_parse_u32(np, "lcd-kit,software-cld-first",
			&pinfo->user_info.software_cld_first, 0);
	}
}
	...
	int panel_id, struct device_node *np, struct dpu_panel_info *pinfo)
{
	...
	ppc_index = id_index * (int)(common_info->ppc_para.dsi_cnt * common_info->ppc_para.cmds_cnt) +
		dsi_index * (int)common_info->ppc_para.cmds_cnt + cmd_index;
	if (ppc_index < 0 || ppc_index >= (int)(common_info->ppc_para.id_cnt * common_info->ppc_para.dsi_cnt
	...
	int panel_id, struct device_node *np, struct dpu_panel_info *pinfo)
{
	...
	pinfo->ppc_config_id_dsi_cmds[id_index][dsi_index].dsi_cmds[cmd_index].cnt = (uint32_t)cmd_cnt;
...
#ifdef LCD_FACTORY_MODE
	lcd_kit_parse_u32(np, "lcd-kit,ppc-enable-panel-estv-support-factory",
		&pinfo->ppc_enable_panel_estv_support_factory, 0);
...
static int lcd_dieid_forwar_read(uint32_t panel_id)
{
	int ret = LCD_KIT_OK;
	struct lcd_kit_adapt_ops *adapt_ops = lcd_kit_get_adapt_ops();
	...
	if (disp_info->panel_dieid_info.move_forwar_support) {
		ret = adapt_ops->mipi_rx(panel_id, NULL, disp_info->panel_dieid_info.dieid_info_data,
			PANEL_DIEID_DATA_SIZE_MAX,	&disp_info->panel_dieid_info.cmds);
		if (ret) {
			LCD_KIT_ERR("get panel dieid info fail\n");
			return LCD_KIT_FAIL;
		}
		LCD_KIT_INFO("panel dieid read Success!\n");
	}
...
static int lcd_get_panel_dieid_info(uint32_t panel_id, char *buff)
{
	...
	if (disp_info->panel_dieid_info.move_forwar_support) {
		ret = memcpy_s(buff, PANEL_DIEID_DATA_SIZE_MAX, disp_info->panel_dieid_info.dieid_info_data,
			PANEL_DIEID_DATA_SIZE_MAX);
		if (ret != 0) {
			LCD_KIT_ERR("memcpy_s error\n");
			return LCD_KIT_FAIL;
		}
		LCD_KIT_INFO("no need read dieid, using Stored Values!\n");
	} else {
...
static int lcd_kit_parse_panel_version_by_uefi(uint32_t panel_id)
{
	...
	for (i = 0; i < (int)disp_info->panel_version.value_number; i++) {
		index = (int)disp_info->panel_version.value_number - i - 1;
...
int lcd_kit_get_aod_panel_state(int panel_id)
{
	...
	unsigned int aod_state = dkmd_get_lcd_status();
...
static int lcd_kit_dvdd_vmin_set(int panel_id, struct lcd_kit_dsi_panel_cmds *dvdd_cmds)
{
	...
	for (i = 0; i < (int)dvdd_vmin_value->cnt; i++) {
...
struct lcd_kit_ops g_lcd_ops = {
	...
	.dieid_forwar_read = lcd_dieid_forwar_read,
