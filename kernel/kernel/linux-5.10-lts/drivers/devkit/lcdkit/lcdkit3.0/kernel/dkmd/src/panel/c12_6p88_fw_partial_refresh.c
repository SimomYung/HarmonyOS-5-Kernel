...
static void lcd_kit_rd_cal_corner(int panel_id)
{
	...
	g_rd_cal_param.dic_ye_cr3 = (int)(rd_roi_ye + REGION_BOUNDARY_BUFFER < minus_one(PANEL_HEIGHT)) ?
		(int)(rd_roi_ye + REGION_BOUNDARY_BUFFER) : minus_one(PANEL_HEIGHT);

	if (g_rd_cal_param.dic_ys_cr3 <= minus_one(REGION_MIN_HEIGHT))
		g_rd_cal_param.dic_ys_cr3_2 = 0;
	 else if (g_rd_cal_param.dic_ys_cr3 >= (int)(PANEL_HEIGHT - REGION_MIN_HEIGHT))
		g_rd_cal_param.dic_ys_cr3_2 = (int)(PANEL_HEIGHT - REGION_MIN_HEIGHT);
	 else
	...
	if (g_rd_cal_param.dic_ye_cr3 <= minus_one(REGION_MIN_HEIGHT))
	 ...
	 else if (g_rd_cal_param.dic_ye_cr3 >= (int)(PANEL_HEIGHT - REGION_MIN_HEIGHT))
...
static void lcd_kit_rd_cal_case(int panel_id)
{
	...
	g_rd_cal_param.adjust_f = (g_rd_cal_param.dic_ye_cr3_2 >= (int)(PANEL_HEIGHT - REGION_MIN_HEIGHT)) ? 1 : 0;
	g_rd_cal_param.adjust_g = (g_rd_cal_param.dic_ye_cr3_2 < (int)(PANEL_HEIGHT - REGION_MIN_HEIGHT)) ? 1 : 0;

	g_rd_cal_param.case_a = (int)((unsigned int)g_rd_cal_param.adjust_a & (unsigned int)g_rd_cal_param.adjust_d);
	g_rd_cal_param.case_b = (int)((unsigned int)g_rd_cal_param.adjust_a &
		(unsigned int)(g_rd_cal_param.adjust_f == 0) & (unsigned int)g_rd_cal_param.adjust_e);
	g_rd_cal_param.case_c = (int)((unsigned int)g_rd_cal_param.adjust_a & (unsigned int)g_rd_cal_param.adjust_f);
	g_rd_cal_param.case_d = (int)((unsigned int)g_rd_cal_param.adjust_b & (unsigned int)g_rd_cal_param.adjust_g);
	g_rd_cal_param.case_e = (int)((unsigned int)g_rd_cal_param.adjust_b &
		(unsigned int)(g_rd_cal_param.adjust_c == 0) & (unsigned int)g_rd_cal_param.adjust_f);
	g_rd_cal_param.case_f = (int)((unsigned int)g_rd_cal_param.adjust_b &
		(unsigned int)g_rd_cal_param.adjust_c & (unsigned int)g_rd_cal_param.adjust_f);
	return;
}
...
static void lcd_kit_rd_cal_region(int panel_id)
{
	if (g_rd_cal_param.case_a) {
		...
		g_rd_region_info.region_b_start = (uint32_t)plus_one(g_rd_cal_param.dic_ye_cr3_2);
		g_rd_region_info.region_c_start = (uint32_t)(g_rd_cal_param.dic_ye_cr3_2 + plus_one(REGION_MIN_HEIGHT));
		...
		g_rd_region_info.region_b_start = (uint32_t)g_rd_cal_param.dic_ys_cr3_2;
		g_rd_region_info.region_c_start = (uint32_t)plus_one(g_rd_cal_param.dic_ye_cr3_2);
	} else if (g_rd_cal_param.case_e) {
		g_rd_region_info.region_b_start = (uint32_t)(g_rd_cal_param.dic_ys_cr3_2 - REGION_MIN_HEIGHT);
		g_rd_region_info.region_c_start = (uint32_t)g_rd_cal_param.dic_ys_cr3_2;
...
static void lcd_kit_get_rd_region_info(int panel_id)
{
	if (common_info->part_refresh.multi_region.is_in) {
		rd_roi_ys = (int)common_info->part_refresh.multi_region.ys;
		rd_roi_ye = (int)minus_one(common_info->part_refresh.multi_region.ye);
	} else {
		rd_roi_ys = (int)common_info->part_refresh.ys;
		rd_roi_ye = (int)minus_one(common_info->part_refresh.ye);
	}

	lcd_kit_rd_cal_case(panel_id);
	lcd_kit_rd_cal_region(panel_id);
	LCD_KIT_DEBUG("rd regions[%d %d %d %d %d %d]\n", g_rd_region_info.region_b_start,
	...
	int val, int num, int start, bool order)
{
	...
	for (i = start; i <= start + num && temp >= 0; i++) {
		params->write_cmds.cmds[i].payload[PLYLOAD_ONE] =
			((unsigned int)val >> (temp * EIGHT_BIT)) & 0xff;
		if (order && temp < num) {
			temp++;
		} else if (temp > -1) {
			temp--;
...
{
	LCD_KIT_DEBUG("get_rd_refresh_cmds begin\n");
...
void lcd_kit_get_rd_region_refresh_info(const struct dkmd_cmds_info *cmds_info)
{
	...
	int offset = common_info->dirty_region.page_offset;
	unsigned int config_id = common_info->ppc_para.ppc_config_id;
	...
	common_info->dirty_region.cmds.cmds[CMDS_ZERO + offset].payload[PLYLOAD_ONE] =
		(((uint32_t)cmds_info->data[COORADINATE_X]) >> EIGHT_BIT) & 0xFF;
	common_info->dirty_region.cmds.cmds[CMDS_ZERO + offset].payload[PLYLOAD_TWO] =
		((uint32_t)cmds_info->data[COORADINATE_X]) & 0xff;
	common_info->dirty_region.cmds.cmds[CMDS_ZERO + offset].payload[PLYLOAD_THREE] =
		(((uint32_t)(cmds_info->data[COORADINATE_X] + cmds_info->data[COORADINATE_W] - 1)) >> EIGHT_BIT) & 0xff;
	common_info->dirty_region.cmds.cmds[CMDS_ZERO + offset].payload[PLYLOAD_FOUR] =
		((uint32_t)(cmds_info->data[COORADINATE_X] + cmds_info->data[COORADINATE_W] - 1)) & 0xff;

	if (common_info->ppc_para.panel_partial_ctrl_support && config_id == CONFIG_ID_M_MODE) {
		common_info->dirty_region.cmds.cmds[CMDS_ONE + offset].payload[PLYLOAD_ONE] =
			((cmds_info->data[COORADINATE_Y] + common_info->dirty_region.ordinate_offset) >> EIGHT_BIT) & 0xff;
		common_info->dirty_region.cmds.cmds[CMDS_ONE + offset].payload[PLYLOAD_TWO] =
			((uint32_t)(cmds_info->data[COORADINATE_Y] + common_info->dirty_region.ordinate_offset)) & 0xff;
		common_info->dirty_region.cmds.cmds[CMDS_ONE + offset].payload[PLYLOAD_THREE] =
			(((uint32_t)(cmds_info->data[COORADINATE_Y] + common_info->dirty_region.ordinate_offset +
			cmds_info->data[COORADINATE_H] - 1)) >> EIGHT_BIT) & 0xff;
		common_info->dirty_region.cmds.cmds[CMDS_ONE + offset].payload[PLYLOAD_FOUR] =
			((uint32_t)(cmds_info->data[COORADINATE_Y] + common_info->dirty_region.ordinate_offset +
			cmds_info->data[COORADINATE_H] - 1)) & 0xff;
		LCD_KIT_DEBUG("dirty_rect:[%d,%d,%d,%d]\n", cmds_info->data[COORADINATE_X],
		cmds_info->data[COORADINATE_Y] + common_info->dirty_region.ordinate_offset,
		cmds_info->data[COORADINATE_W], cmds_info->data[COORADINATE_H]);
	} else {
		common_info->dirty_region.cmds.cmds[CMDS_ONE + offset].payload[PLYLOAD_ONE] =
			((cmds_info->data[COORADINATE_Y]) >> EIGHT_BIT) & 0xff;
		common_info->dirty_region.cmds.cmds[CMDS_ONE + offset].payload[PLYLOAD_TWO] =
			((uint32_t)cmds_info->data[COORADINATE_Y]) & 0xff;
		common_info->dirty_region.cmds.cmds[CMDS_ONE + offset].payload[PLYLOAD_THREE] =
			(((uint32_t)(cmds_info->data[COORADINATE_Y] + cmds_info->data[COORADINATE_H] - 1)) >> EIGHT_BIT) & 0xff;
		common_info->dirty_region.cmds.cmds[CMDS_ONE + offset].payload[PLYLOAD_FOUR] =
			((uint32_t)(cmds_info->data[COORADINATE_Y] + cmds_info->data[COORADINATE_H] - 1)) & 0xff;
	}
}
