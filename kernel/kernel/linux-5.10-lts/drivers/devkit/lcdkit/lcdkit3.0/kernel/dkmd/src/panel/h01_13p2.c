	...
	char *payload, struct lcd_kit_tcon_update *upd, struct lcd_kit_adapt_ops *ops)
{
	int i;
	int ret;
	struct h01_13p2_tcon_upd_time upd_time = {0};
	uint32_t cmds_num;
...
static int h01_13p2_fw_version_check(void)
{
	...
	for (i = 0; i < H01_13P2_IC_DATA_LEN; i++) {
		ret = sprintf_s(ic_text + (i << 1), H01_13P2_FW_VER_INFO_LEN - (i << 1),
			"%02x", h01_13p2_ic_data[i]);
		if (ret < 0) {
			LCD_KIT_ERR("snprintf fail\n");
			return LCD_KIT_FAIL;
		}
		ret = sprintf_s(bin_text + (i << 1), H01_13P2_FW_VER_INFO_LEN - (i << 1),
	...
	struct lcd_kit_tcon_update *upd, struct lcd_kit_adapt_ops *ops)
{
	...
	if (!g_h01_13p2_fw_info.fw) {
		LCD_KIT_ERR("request_firmware failed, firmware is null\n");
		return LCD_KIT_FAIL;
	}
