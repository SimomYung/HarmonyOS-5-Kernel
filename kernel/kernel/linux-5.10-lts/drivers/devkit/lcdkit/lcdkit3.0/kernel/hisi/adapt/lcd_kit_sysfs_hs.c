...
static int read_sn_done = UNDONE;
	...
	struct device_attribute *attr, char *buf)
{
...
#endif
	default_fps = (int)(disp_info->fps.default_fps > 0 ? disp_info->fps.default_fps : LCD_FPS_60);
	if (disp_info->fps.fps_need_high_60 != FPS_CONFIG_EN)
		ret = snprintf(str, sizeof(str), "current_fps:%d;default_fps:%d", fps_rate, default_fps);
	else
		ret = snprintf(str, sizeof(str), "current_fps:%d;default_fps:%dn", fps_rate, default_fps);
	...
	strncat(str, ";support_fps_list:", LCD_REG_LENGTH_MAX - 1);
	for (i = 0; i < disp_info->fps.panel_support_fps_list.cnt; i++) {
		if (i > 0)
			strncat(str, ",", LCD_REG_LENGTH_MAX - 1);
		fps_rate = disp_info->fps.panel_support_fps_list.buf[i];
		ret += snprintf(tmp, sizeof(tmp), "%d", fps_rate);
		strncat(str, tmp, LCD_REG_LENGTH_MAX - 1);
	}
	ret += panel_info_cat(str, LCD_REG_LENGTH_MAX - 1);
	LCD_KIT_INFO("%s\n", str);
	ret = snprintf(buf, PAGE_SIZE, "%s\n", str);
	return ret;
}
...
static ssize_t lcd_kit_get_ext_panel_sn(struct hisi_fb_data_type *hisifd)
{
	ssize_t ret;
	struct lcd_kit_ops *lcd_ops = NULL;
	...
	if (lcd_kit_get_product_type() == LCD_DUAL_PENEL_TYPE &&
		read_sn_done == UNDONE && LCD_ACTIVE_PANEL == LCD_EXT_PANEL) {
		/* get sn code */
		if (!hisifd->panel_power_on) {
		...
		lcd_ops = lcd_kit_get_ops();
		if ((lcd_ops) && (lcd_ops->get_sn_code)) {
			LCD_KIT_INFO("get exit sn begin \n");
			hisifb_activate_vsync(hisifd);
			ret = lcd_ops->get_sn_code(LCD_EXT_PANEL);
			hisifb_deactivate_vsync(hisifd);
			if (ret < 0) {
				LCD_KIT_ERR("get exit sn code failed!\n");
				return LCD_KIT_FAIL;
			}
		}
		read_sn_done = DONE;
	...
	struct device_attribute *attr, char *buf)
{
	...
	if (lcd_kit_get_product_type() == LCD_DUAL_PENEL_TYPE && fold_panel_id == 0)
		hisifd = dpufd_list[PRIMARY_PANEL_IDX];
	if (!hisifd || !buf) {
		LCD_KIT_ERR("hisifd or buf is null\n");
		return LCD_KIT_FAIL;
	}
	...
	if (lcd_kit_get_product_type() == LCD_DUAL_PENEL_TYPE && fold_panel_id == 1) {
		if (lcd_kit_get_ext_panel_sn(hisifd))
			return LCD_KIT_FAIL;
	}
	memset(str_oem, 0, sizeof(str_oem));
