...
static int lcd_kit_backlight_handle(struct hisi_fb_data_type *hisifd, uint32_t bl_level, bool first_screenon);
...
static int lcd_kit_sot_err_nv_read(void)
{
	...
	if (ret) {
		LCD_KIT_ERR("nve_direct_access_interface read error %d\n", ret);
		return LCD_KIT_FAIL;
	}
...
static void lcd_kit_sot_err_nv_write(struct hisi_fb_data_type *hisifd)
{
	...
	ret = nve_direct_access_interface(&user_info);
	if (ret)
		LCD_KIT_ERR("nve_direct_access_interface write error %d\n", ret);
	...
	struct hisi_fb_data_type *hisifd)
{
	...
	if (disp_info->fps.support && disp_info->fps.fps_need_update_on) {
		...
		if (pinfo->fps_scence == LCD_FPS_SCENCE_60P && disp_info->fps.aod_te_switch_support) {
			cmds = &disp_info->fps.fps_to_cmds[LCD_FPS_SCENCE_60P];
			lcd_kit_dsi_cmds_tx_no_lock(hisifd, cmds);
			LCD_KIT_INFO("send TE to 60HZ\n");
			return;
		}
...
static int lcd_kit_hbm_backlight_handle(struct hisi_fb_data_type *hisifd, unsigned int bl_level, bool first_screenon)
{
	...
	if (lcd_kit_need_enter_hbm(bl_level, first_screenon)) {
		...
		if (common_ops->enter_hbm && common_info->hbm_backlight.hbm_status == LCD_KIT_HBM_OFF)
			ret = common_ops->enter_hbm(panel_id, hisifd, LCD_KIT_BACKLIGHT_HBM);
		...
		if (common_ops->exit_hbm && common_info->hbm_backlight.hbm_status == LCD_KIT_HBM_ON)
			ret = common_ops->exit_hbm(panel_id, hisifd, LCD_KIT_BACKLIGHT_HBM);
...
static int lcd_kit_set_backlight(struct platform_device *pdev, uint32_t bl_level)
{
	...
	case BL_SET_BY_MIPI:
		if (hisifd->masklayer_flag == MASK_LAYER_SCREENON
			...
			ret = lcd_kit_backlight_handle(hisifd, bl_level, first_screenon);
		}
		break;
	default:
	...
	if (first_screenon && common_info != NULL &&
		common_info->hbm.hbm_fps_command_support && hisifd->de_info.hbm_level > 0)
...
static int lcd_kit_backlight_handle(struct hisi_fb_data_type *hisifd, uint32_t bl_level, bool first_screenon)
{
	...
	if (common_info->hbm_backlight.support)
		return lcd_kit_hbm_backlight_handle(hisifd, bl_level, first_screenon);
	else
		return lcd_kit_mipi_set_backlight(hisifd, bl_level);
}
