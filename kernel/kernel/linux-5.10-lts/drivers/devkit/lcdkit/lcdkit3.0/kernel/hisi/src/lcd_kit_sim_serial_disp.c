...
{
	if (panel_id >= ARRAY_SIZE(lcd_kit_sim_serial_pwr_ref_count_on) ||
		panel_id >= ARRAY_SIZE(lcd_kit_sim_serial_pwr_ref_count_off) ||
		panel_id >= ARRAY_SIZE(lcd_kit_sim_backlight_preproc_cb) ||
		panel_id >= ARRAY_SIZE(lcd_kit_sim_backlight_postproc_cb)) {
		LCD_KIT_ERR("panel_id is out of bounds");
		return;
	}
...
{
	if (panel_id >= ARRAY_SIZE(g_sim_serial_panel)) {
		LCD_KIT_ERR("panel_id is out of bounds");
		return;
	}
	...
	struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (!ret) {
		LCD_KIT_ERR("sscanf return invalid:%zd\n", ret);
		return LCD_KIT_FAIL;
	}
...
static int lcd_kit_sim_serial_panel_probe(struct platform_device *pdev)
{
	...
	struct hisi_fb_panel_data pdata = {0};
