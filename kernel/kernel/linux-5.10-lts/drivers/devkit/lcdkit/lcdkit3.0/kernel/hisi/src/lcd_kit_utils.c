...
#if defined (CONFIG_LOG_EXCEPTION) && (defined(CONFIG_HUAWEI_DSM) || defined(CONFIG_HUAWEI_OHOS_DSM))
...
int lcd_kit_apl_backlight_handle(struct hisi_fb_data_type *hisifd, unsigned int bl_level);
	...
	struct lcd_kit_dsi_cmd_desc *cmds, uint32_t len)
{
	...
	struct dsi_cmd_desc lcd_reg_cmd = {0};
	struct hisi_fb_data_type *hisifd = NULL;
...
static bool lcd_kit_fps_check_delay(struct hisi_panel_info *pinfo)
{
	...
	struct timespec64 afe_tv = {0};
	struct timespec64 record_tv = {0};
...
void clear_time_by_region(int region)
{
	...
	if (region >= ARRAY_SIZE(disp_info->dbv_stat.last_time) ||
		region >= ARRAY_SIZE(disp_info->dbv_stat.hbm_last_time) ||
		region >= ARRAY_SIZE(disp_info->dbv_stat.pwon_last_time) ||
		region >= ARRAY_SIZE(disp_info->dbv_stat.hbmon_last_time)) {
		LCD_KIT_ERR("region is out of bounds");
		return;
	}
...
int lcd_kit_mipi_set_backlight(struct hisi_fb_data_type *hisifd, uint32_t level)
{
	...
	pinfo = &(hisifd->panel_info);
	max_level = (hisifd->panel_info.dbv_max > hisifd->panel_info.bl_max) ?
		hisifd->panel_info.dbv_max : hisifd->panel_info.bl_max;
	bl_level = (level < max_level) ? level : max_level;
	hisifb_display_effect_fine_tune_backlight(hisifd, (int)bl_level, (int *)&bl_level);
	lcd_kit_apl_backlight_handle(hisifd, bl_level);
...
int lcd_kit_apl_backlight_handle(struct hisi_fb_data_type *hisifd, unsigned int bl_level)
{
	...
	int panel_id = LCD_ACTIVE_PANEL;
	if (bl_level > common_info->hbm_backlight.apl_threshold) {
		if (common_ops->enter_hbm && common_info->hbm_backlight.apl_status == LCD_KIT_APL_OFF) {
			common_ops->enter_hbm(panel_id, hisifd, LCD_KIT_BACKLIGHT_APL);
			LCD_KIT_DEBUG("enter apl\n");
		}
	} else {
		if (common_ops->exit_hbm && common_info->hbm_backlight.apl_status == LCD_KIT_APL_ON) {
			common_ops->exit_hbm(panel_id, hisifd, LCD_KIT_BACKLIGHT_APL);
			LCD_KIT_DEBUG("exit apl\n");
		}
	}
#endif
	return LCD_KIT_OK;
...
void lcd_kit_fps_util(struct device_node *np)
{
	lcd_kit_parse_u32(np, "lcd-kit,aod-te-switch-support",
		&disp_info->fps.aod_te_switch_support, 0);
...
static void parse_panel_version_cmd(struct device_node *np)
{
...
static void lcd_kit_parse_panel_version(struct device_node *np)
{
	int i, ret;
	...
	uint32_t *expect_val, int cnt, uint32_t compare_mode)
{
	...
	if (compare_mode == PCD_COMPARE_MODE_EQUAL) {
		for (i = 0; i < cnt && i < LCD_KIT_PCD_SIZE; i++) {
...
void lcd_kit_mbist_detect_dmd_report(u8 reg_val[MBIST_LOOPS][DETECT_NUM][VAL_NUM], int panel_id)
{
#if (defined(CONFIG_HUAWEI_DSM) || defined(CONFIG_HUAWEI_OHOS_DSM)) && defined(LCD_FACTORY_MODE)
	int ret;
	char err_info[DMD_DET_ERR_LEN] = { 0 };
	...
	LCD_KIT_INFO("mbist detect DMD report\n");
	if (!reg_val) {
		LCD_KIT_ERR("reg_val is NULL\n");
		return;
	}
	...
	lcd_dmd_report_err(dmd_number, err_info, DMD_DET_ERR_LEN);
	LCD_KIT_INFO(" with DSM_LCD_MBIST_ERROR_NO \n");
#else
	return;
#endif
}
...
static int lcd_kit_get_sn_code(int panel_id)
{
	...
	if (common_info && common_info->sn_code.support) {
		hisifd = hisifd_list[PRIMARY_PANEL_IDX];
		if (lcd_kit_get_product_type() == LCD_DUAL_PENEL_TYPE &&
			panel_id == LCD_EXT_PANEL)
...
static void safe_fps_cmd_to_dsi_cmds(struct dpu_panel_info *pinfo, unsigned int frame_rate, int num)
{
	...
	if (num >= ARRAY_SIZE(pinfo->dfr_info.oled_info.ltpo_info.safe_frm_rates)) {
		LCD_KIT_ERR("num is out of bounds");
		return;
	}
	for (i = 0; i < table_num; i++) {
...
static void parse_ltpo1_info(struct device_node *np, struct hisi_panel_info *pinfo)
{
	...
	struct lcd_kit_array_data dimming_seq_90hz = {0};
	struct lcd_kit_array_data repeat_num = {0};
	struct lcd_kit_array_data repeat_num_90hz = {0};
