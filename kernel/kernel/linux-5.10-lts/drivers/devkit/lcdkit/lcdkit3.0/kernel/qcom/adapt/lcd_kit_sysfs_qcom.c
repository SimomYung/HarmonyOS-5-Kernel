...
static int lcd_get_2d_barcode(uint32_t panel_id, char *oem_data, uint32_t len);
static int lcd_get_project_id(uint32_t panel_id, char *oem_data, uint32_t len);
...
static int fps_list_cat(char *str, int len)
{
	...
	ret = snprintf(str, len, "current_fps:%u;default_fps:%u",
		disp_info->fps.current_fps, disp_info->fps.default_fps);
	strncat(str, ";support_fps_list:", len);
	...
	struct device_attribute *attr, char *buf)
{
	...
	if (!FACT_INFO->poweric_detect.poweric_num_length ||
		...
		strncat(str, tmp, LCD_PMIC_LENGTH_MAX - 1);
...
static int poweric_set_elvdd(uint32_t panel_id, int start, int end, int i)
{
	int j, k, ret, temp;
	int gpio_flag = 0;
	for (j = start; j < end; j++) {
...
static int lcd_get_project_id(uint32_t panel_id, char *oem_data, uint32_t len)
{
	...
	if (disp_info->project_id.support) {
		...
		for (i = 0; i < PROJECT_ID_LENGTH && i < len; i++)
...
static int lcd_get_2d_barcode(uint32_t panel_id, char *oem_data, uint32_t len)
{
	...
	if (disp_info->oeminfo.barcode_2d.support) {
		...
		strncat(oem_data, read_value, len);
	}
	return ret;
}

static void lcd_get_parse_data_info_data(char *lcd_parse_data, char *buf, uint32_t len)
{
	int i;
	...
	for (i = 0; i < buf[1]; i++) {
		...
		strncat(lcd_parse_data, str_tmp, len);
	}
}
	...
	for (i = 0; i < length; i++) {
		if (oem_info_type == oem_read_cmds[i].type) {
			...
			if (oem_read_cmds[i].func)
				(*oem_read_cmds[i].func)(panel_id, oem_info_data, OEM_INFO_SIZE_MAX - 1);
		}
	}
	memset(str_oem, 0, sizeof(str_oem));
	lcd_get_parse_data_info_data(str_oem, oem_info_data, OEM_INFO_SIZE_MAX - 1);
