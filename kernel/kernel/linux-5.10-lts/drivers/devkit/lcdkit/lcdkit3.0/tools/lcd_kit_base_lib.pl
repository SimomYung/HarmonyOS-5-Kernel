...
sub get_module_map_list
{
    ...
    if ($xml_file eq $parse_error_string) {
        warning_print("cann't find plat $xml_md xml files!\n");
...
# 0 dsi_lp_mode, 1 dsi_hs_mode, 2 dsi_lp_sync_mode, 3 dsi_lp_async_mode, 7 dsi_hs_async_refresh_mode
# 4 dsi_lp_refresh_mode
sub parser_cmd_state
{
    my $para = shift;
    if ($para eq "0")
    {
    ...
    if ($para eq "2")
    {
        return "dsi_lp_async_mode";
    }
    if ($para eq "3")
    {
        return "dsi_hs_async_mode";
    }
    if ($para eq "4")
    {
        return "dsi_lp_refresh_mode";
    }
    if ($para eq "5")
    {
        return "dsi_hs_refresh_mode";
    }
    if ($para eq "6")
    {
        return "dsi_lp_async_refresh_mode";
    }
    if ($para eq "7")
    {
        return "dsi_hs_async_refresh_mode";
    }
