...
#endif
#ifdef CONFIG_LIBLINUX
#define FW_PATH "/chip_prod/etc/firmware/ts"
...
static int ts_kit_input_tp_device_register(struct input_dev *dev)
{
	...
	struct ts_kit_platform_data *data = &g_ts_kit_platform_data;
#if defined(CONFIG_VENDOR_CHIP_PREFIX)
...
#if defined(CONFIG_VENDOR_CHIP_PREFIX)
	platform_name = CONFIG_VENDOR_CHIP_PREFIX;
	if (strcmp(platform_name, "atherton") == 0)
		input_dev->name = TS_INPUT_REPORT_NAME;
	else
#endif
...
static int ts_kit_update_firmware(void)
{
	...
	if ((!touch_recovery_mode && !charge_flag) ||
		(g_ts_kit_platform_data.recovery_update_fw)) {
#ifdef CONFIG_LIBLINUX
		while (ksys_access(FW_PATH, 0) != 0 && retry < MAX_RETRY_TIMES) {
			msleep(200);
			retry++;
		}
#endif
...
#ifdef CONFIG_LCD_KIT_HYBRID
	{ TS_HYBRID_SWITCH_CONTROL, ts_kit_hybrid_switch_control_func },
#endif
};
...
void ts_dmd_report(int dmd_num, const char *format, ...)
{
	...
	(void)vsnprintf(input_buf, TS_CHIP_DMD_REPORT_SIZE - 1, format, args);
	va_end(args);
	(void)snprintf(report_buf, TS_CHIP_DMD_REPORT_SIZE - 1,
