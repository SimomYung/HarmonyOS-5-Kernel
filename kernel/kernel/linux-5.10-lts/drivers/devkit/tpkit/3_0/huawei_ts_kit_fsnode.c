...
#if ((defined CONFIG_FB) && (defined CONFIG_HUAWEI_DEVKIT_HISI_3_0) && (!defined CONFIG_HUAWEI_TS_KIT_UDP) && \
	 (!defined CONFIG_EXT_INPUTHUB))
#include "hisi_fb.h"
#endif
...
static ssize_t ts_fw_update_boot_store(struct device *dev,
	struct device_attribute *attr,
	const char *buf, size_t count)
{
	int error;
...
static int anti_false_touch_get_param(const char *buf, char *tag)
{
	...
	TS_LOG_DEBUG("%s:%s\n", buf, tag);
	(void)snprintf(tmp_str, sizeof(tmp_str), "%s=", tag);
	...
	struct device_attribute *attr, char *buf)
{
	...
	(void)snprintf(buf, PAGE_SIZE - count - 1, "\n");
	count++;
	return count;
}
	...
	const char *buf, size_t count)
{
	...
	(void)snprintf(pdata->touch_switch_info, MAX_STR_LEN - 1, "%s", buf);
	memset(&cmd, 0, sizeof(struct ts_cmd_node));
...
int set_appinfo(const char *in_cmd, struct ts_cmd_node *cmd)
{
	...
	memset(pdata->touch_switch_info, 0, MAX_STR_LEN);
	(void)snprintf(pdata->touch_switch_info, MAX_STR_LEN - 1, "%s", in_cmd);
...
static DEVICE_ATTR(fw_update_boot, 0200, NULL, ts_fw_update_boot_store);
