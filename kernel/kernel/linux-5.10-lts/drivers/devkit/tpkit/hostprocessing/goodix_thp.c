	...
	struct thp_core_data *cd)
{
	...
	rc = of_property_read_u32(thp_node, "irq_sync_method",
		&config_data[cd->panel_id].irq_sync_method);
	if (!rc)
		thp_log_info("%s:irq_sync_method parsed:%d\n",
			__func__, config_data[cd->panel_id].irq_sync_method);
...
static int thp_power_on_iovdd_vcc_9897(struct thp_core_data *cd)
{
	...
	ret = thp_power_supply_ctrl(THP_IOVDD, THP_POWER_ON, AFTER_IOVDD_DELAY_9897, cd->panel_id);
	if (ret)
		thp_log_err("%s:vddio fail\n", __func__);
	ret = thp_power_supply_ctrl(THP_VCC, THP_POWER_ON, AFTER_DVDD_RST_DELAY_9897, cd->panel_id);
	if (ret)
		thp_log_err("%s:vcc fail\n", __func__);
	thp_log_info("%s pull up tp ic reset\n", __func__);
	return ret;
...
static int touch_driver_get_afe_info_for_9896(struct thp_device *tdev)
{
	...
	for (retry = 0; retry < FW_INFO_READ_RETRY; retry++) {
		...
		ret = touch_driver_spi_read(tdev, GOODIX_AFE_INFO_ADDR_GT9896, afe_data_buf, afe_data_len);
		if (ret) {
...
int touch_driver_chip_detect(struct thp_device *tdev)
{
	...
	if (cd->support_adapter_mode) {
		ret = support_power_adapter_mode(tdev);
		if (ret)
			goto exit;
		touch_driver_slowdown_flash_controller_speed(tdev);
	} else {
