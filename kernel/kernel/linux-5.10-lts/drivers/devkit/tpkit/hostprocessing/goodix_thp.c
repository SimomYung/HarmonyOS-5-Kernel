...
struct goodix_module_info module_info[TOTAL_PANEL_NUM];
struct goodix_module_info_for_gt9896 module_info_for_gt9896[TOTAL_PANEL_NUM];
struct goodix_module_info_for_gt9896 module_info_for_gt9897[TOTAL_PANEL_NUM];
struct goodix_module_info_for_gt9966 module_info_for_gt9966[TOTAL_PANEL_NUM];
static struct goodix_config_data config_data[TOTAL_PANEL_NUM];

static int goodix_frame_addr[TOTAL_PANEL_NUM];
static int goodix_frame_len[TOTAL_PANEL_NUM];
static int goodix_cmd_addr[TOTAL_PANEL_NUM];
static int goodix_fw_status_addr = GOODIX_FW_STATE_ADDR_DEFAULT;
static int goodix_fw_status_len = GOODIX_FW_STATE_LEN_DEFAULT;
/*
 * goodix_touch_addr is used only for tablet products.
 * Multi-screen resource extension is not supported currently.
 */
static int goodix_touch_addr = GOODIX_TOUCH_ADDR_DEFAULT_GT9897_DUAL;
static unsigned int g_thp_udfp_status[TOTAL_PANEL_NUM];
static unsigned int goodix_ring_setting_switch[TOTAL_PANEL_NUM];
static unsigned int goodix_stylus_status[TOTAL_PANEL_NUM];
static unsigned int work_status[TOTAL_PANEL_NUM];
static unsigned int g_aod_touch_status[TOTAL_PANEL_NUM];
static u8 *goodix_spi_tx_buf[TOTAL_PANEL_NUM];
static unsigned int g_power_off_staus[TOTAL_PANEL_NUM];
static char *tx_frame_buff[TOTAL_PANEL_NUM];
static char *rx_frame_buff[TOTAL_PANEL_NUM];
...
static void touch_driver_clear_irq(struct thp_device *tdev)
{
	int ret;
	u8 sync_flag = 0;
	u32 panel_id = tdev->thp_core->panel_id;

	if (!config_data[panel_id].irq_sync_method)
		return;

	ret = touch_driver_spi_write(tdev, goodix_frame_addr[panel_id], &sync_flag, 1);
	if (ret)
...
static int touch_driver_spi_read_check(struct thp_device *tdev, unsigned int len)
{
	if (tdev->thp_core->support_vendor_ic_type == GT9897) {
		if ((len + GOODIX_READ_WRITE_BYTE_OFFSET_GT9897) >
			thp_get_max_frame_size(tdev->thp_core)) {
		...
		if ((len + READ_WRITE_BYTE_OFFSET) > thp_get_max_frame_size(tdev->thp_core)) {
			thp_log_err("%s:invalid len:%d\n", __func__, len);
			return -EINVAL;
		}
	}
	...
	u8 *data, unsigned int len)
{
	...
	u8 *rx_buf = NULL;
	u8 *tx_buf = NULL;
	u8 *start_cmd_buf = goodix_spi_tx_buf[cd->panel_id];
	...
	check_flag = ((data == NULL) || (tdev->tx_buff == NULL) ||
		(tdev->rx_buff == NULL) || (cd->sdev == NULL));
	if (check_flag) {
	...
	u8 *data, unsigned int len)
{
	...
	check_flag = ((data == NULL) || (tx_frame_buff[cd->panel_id] == NULL) ||
		(cd->sdev == NULL) || (rx_frame_buff[cd->panel_id] == NULL));
	if (check_flag) {
		thp_log_err("%s: have null ptr\n", __func__);
		return -EINVAL;
	}
	...
	spi = cd->sdev;
	rx_buf = rx_frame_buff[cd->panel_id];
	tx_buf = tx_frame_buff[cd->panel_id];
	spi_message_init(&spi_msg);
	memset(xfers, 0, sizeof(xfers));
	...
	u8 cmd, u16 data)
{
	...
	for (i = 0; i < SEND_COMMAND_RETRY; i++) {
		ret = touch_driver_spi_write(tdev, goodix_cmd_addr[tdev->thp_core->panel_id],
		...
		ret = touch_driver_get_cmd_ack(tdev, goodix_cmd_addr[tdev->thp_core->panel_id] +
	...
	struct thp_device *tdev, u8 cmd, u16 data)
{
	...
	for (i = 0; i < SEND_COMMAND_RETRY; i++) {
		ret = touch_driver_spi_write(tdev, goodix_cmd_addr[cd->panel_id],
			cmd_send.buf, sizeof(cmd_send));
		if (ret < 0) {
			thp_log_err("%s: failed send command, ret %d\n", __func__, ret);
			return -EINVAL;
		}
		ret = touch_driver_get_cmd_ack_for_9897(tdev,
			goodix_cmd_addr[cd->panel_id] + GOODIX_CMD_ACK_OFFSET_GT9897);
		if (ret == CMD_ACK_ERROR) {
			thp_log_err("%s: cmd ack info is error\n", __func__);
			continue;
		} else {
	...
	u8 cmd, u8 status)
{
	...
	while (retry++ < SEND_COMMAND_RETRY) {
		...
		ret = touch_driver_spi_read(tdev,
			goodix_cmd_addr[cd->panel_id] + CMD_ACK_OFFSET,
	...
	struct thp_core_data *cd)
{
	...
	thp_parse_dts_u32(thp_node, "cmd_gesture_mode", &cd->cmd_gesture_mode, __func__);
	thp_log_info("%s:cd->cmd_gesture_mode = 0x%X\n", __func__, cd->cmd_gesture_mode);

	if (of_property_read_string(thp_node, "multi_vendor_name", &cd->thp_dev->multi_vendor_name))
		cd->thp_dev->multi_vendor_name = "null";

	cd->sensorhub_support_gesture = 0;
	thp_parse_dts_u32(thp_node, "sensorhub_support_gesture", &cd->sensorhub_support_gesture, __func__);

	cd->gesture_mode_double_tap = 0;
	thp_parse_dts_u32(thp_node, "gesture_mode_double_tap", &cd->gesture_mode_double_tap, __func__);

	thp_parse_dts_u32(thp_node, "support_fake_panel", &cd->support_fake_panel, __func__);

	/*
	 * 0: Asynchronous, 1:Synchronous
	 */
	config_data[cd->panel_id].irq_sync_method = 0;
	thp_parse_dts_u32(thp_node, "irq_sync_method",
		&config_data[cd->panel_id].irq_sync_method, __func__);
	...
	thp_parse_dts_u32(thp_node, "not_reuse_ic_type", &cd->not_reuse_ic_type, __func__);

	cd->disable_frame_report = 0;
	thp_parse_dts_u32(thp_node, "disable_frame_report", &cd->disable_frame_report, __func__);

	cd->need_extra_system_clk = 0;
	thp_parse_dts_u32(thp_node, "need_extra_system_clk", &cd->need_extra_system_clk, __func__);

	cd->support_fw_xtal_switch = 0;
	thp_parse_dts_u32(thp_node, "support_fw_xtal_switch", &cd->support_fw_xtal_switch, __func__);
	...
	struct thp_device *tdev)
{
	...
	len = sizeof(module_info_for_gt9897[panel_id]);
	memset(&module_info_for_gt9897[panel_id], 0, len);
	...
	for (retry = 0; retry < MODULE_INFO_READ_RETRY; retry++) {
		...
		ret = touch_driver_spi_read(tdev,
			...
			(u8 *)&module_info_for_gt9897[panel_id], len);
		/* print 32*3 data (rowsize=32,groupsize=3),0:no ascii */
		print_hex_dump(KERN_INFO, "[I/THP] module info: ",
			DUMP_PREFIX_NONE, 32, 3,
			(u8 *)&module_info_for_gt9897[panel_id], len, 0);
		if (!ret && !checksum16_cmp((u8 *)&module_info_for_gt9897[panel_id],
					len, GOODIX_LE_MODE))
			break;
	...
	if (memcmp(module_info_for_gt9897[panel_id].mask_id, GOODIX_MASK_ID_FOR_GT9897,
		GOODIX_MASK_ID_LEN)) {
		if (memcmp(module_info_for_gt9897[panel_id].mask_id,
			GOODIX_MASK_ID_FOR_GT9897_NOCODE, GOODIX_MASK_ID_LEN)) {
			memcpy(temp_buf, module_info_for_gt9897[panel_id].mask_id,
				GOODIX_MASK_ID_LEN);
			thp_log_err("invalied mask id %s != %s\n",
				temp_buf, GOODIX_MASK_ID_FOR_GT9897);
			return -EINVAL;
		}
		memcpy(temp_buf, module_info_for_gt9897[panel_id].mask_id, GOODIX_MASK_ID_LEN);
		thp_log_err("invalied mask id %s = %s\n",
	...
	thp_log_info("MASK_ID %s : ver %*ph\n", temp_buf,
		(u32)sizeof(module_info_for_gt9897[panel_id].mask_vid),
		module_info_for_gt9897[panel_id].mask_vid);
	thp_log_info("PID %s : ver %*ph\n", module_info_for_gt9897[panel_id].pid,
		(u32)sizeof(module_info_for_gt9897[panel_id].vid),
		module_info_for_gt9897[panel_id].vid);
	return NO_ERR;
}
...
static int touch_driver_communication_check_for_9966(struct thp_device *tdev)
{
	...
	len = sizeof(module_info_for_gt9966[panel_id]);
	memset(&module_info_for_gt9966[panel_id], 0, len);
	for (retry = 0; retry < MODULE_INFO_READ_RETRY; retry++) {
		ret = touch_driver_spi_read(tdev, GOODIX_MODULE_INFO_ADDR_GT9966,
			(u8 *)&module_info_for_gt9966[panel_id], len);

		thp_log_debug("module_info:%*ph\n", len, (u8 *)&module_info_for_gt9966[panel_id]);
		if (!ret && !checksum16_cmp((u8 *)&module_info_for_gt9966[panel_id],
					len, GOODIX_LE_MODE))
			break;
		/* retry need delay 10ms */
	...
	if (memcmp(module_info_for_gt9966[panel_id].rom_pid, GOODIX_MASK_ID_FOR_GT9916,
		GOODIX_MASK_ID_LEN)) {
		if (memcmp(module_info_for_gt9966[panel_id].rom_pid,
			GOODIX_MASK_ID_FOR_GT9916_NOCODE, GOODIX_MASK_ID_LEN)) {
			memcpy(temp_buf, module_info_for_gt9966[panel_id].rom_pid,
				GOODIX_MASK_ID_LEN);
			thp_log_err("invalied mask id %s != %s\n",
	...
	memcpy(temp_buf, module_info_for_gt9966[panel_id].rom_pid, sizeof(module_info_for_gt9966[panel_id].rom_pid));
	thp_log_info("%s:rom_pid:%s\n", __func__, temp_buf);
	thp_log_info("%s:rom_vid:%*ph\n", __func__, sizeof(module_info_for_gt9966[panel_id].rom_vid),
		module_info_for_gt9966[panel_id].rom_vid);
	memcpy(temp_buf, module_info_for_gt9966[panel_id].patch_pid, sizeof(module_info_for_gt9966[panel_id].patch_pid));
	thp_log_info("%s:patch_pid:%s\n", __func__, temp_buf);
	thp_log_info("%s:patch_vid:%*ph\n", __func__, sizeof(module_info_for_gt9966[panel_id].patch_vid),
		module_info_for_gt9966[panel_id].patch_vid);
	...
	struct thp_device *tdev)
{
	...
	len = sizeof(module_info_for_gt9896[panel_id]);
	memset(&module_info_for_gt9896[panel_id], 0, len);
	...
	for (retry = 0; retry < MODULE_INFO_READ_RETRY; retry++) {
		ret = touch_driver_spi_read(tdev,
			...
			(u8 *)&module_info_for_gt9896[panel_id], len);
		/* print 32*3 data (rowsize=32,groupsize=3),0:no ascii */
		print_hex_dump(KERN_INFO, "[I/THP] module info: ",
			DUMP_PREFIX_NONE, 32, 3,
			(u8 *)&module_info_for_gt9896[panel_id], len, 0);
		if (!ret)
			break;
		/* retry need delay 10ms */
		msleep(WAIT_FOR_COMMUNICATION_CHECK_DELAY);
	...
	if (memcmp(module_info_for_gt9896[panel_id].mask_id, GOODIX_MASK_ID_FOR_GT9896,
		GOODIX_MASK_ID_LEN)) {
		memcpy(temp_buf, module_info_for_gt9896[panel_id].mask_id,
			GOODIX_MASK_ID_LEN);
	...
	thp_log_info("%s: communication check passed\n", __func__);
	memcpy(temp_buf, module_info_for_gt9896[panel_id].mask_id, GOODIX_MASK_ID_LEN);
	temp_buf[GOODIX_MASK_ID_LEN] = '\0';
	thp_log_info("MASK_ID %s : ver %*ph\n", temp_buf,
		(u32)sizeof(module_info_for_gt9896[panel_id].mask_vid),
		module_info_for_gt9896[panel_id].mask_vid);
	thp_log_info("PID %s : ver %*ph\n", module_info_for_gt9896[panel_id].pid,
		(u32)sizeof(module_info_for_gt9896[panel_id].vid),
		module_info_for_gt9896[panel_id].vid);
	return NO_ERR;
}
...
static int touch_driver_communication_check(struct thp_device *tdev)
{
	...
	len = sizeof(module_info[cd->panel_id]);
	memset(&module_info[cd->panel_id], 0, len);

	for (retry = 0; retry < MODULE_INFO_READ_RETRY; retry++) {
		ret = touch_driver_spi_read(tdev, GOODIX_MODULE_INFO_ADDR,
				(u8 *)&module_info[cd->panel_id], len);
		/* print 32*3 data (rowsize=32,groupsize=3) */
		print_hex_dump(KERN_INFO, "[I/THP] module info: ", DUMP_PREFIX_NONE,
				32, 3, (u8 *)&module_info[cd->panel_id], len, 0);
		checksum = checksum_u8((u8 *)&module_info[cd->panel_id], len);
		if (!ret && !checksum)
			break;
	...
	if (memcmp(module_info[cd->panel_id].mask_id, GOODIX_MASK_ID,
			sizeof(GOODIX_MASK_ID) - 1)) {
		memcpy(temp_buf, module_info[cd->panel_id].mask_id, GOODIX_MASK_ID_LEN);
		thp_log_err("%s: invalied mask id %s != %s\n",
	...
	temp_buf[GOODIX_MASK_ID_LEN] = '\0';
	thp_log_info("MASK_ID %s : ver %*ph\n", temp_buf,
		(u32)sizeof(module_info[cd->panel_id].mask_vid), module_info[cd->panel_id].mask_vid);
	thp_log_info("PID %s : ver %*ph\n", module_info[cd->panel_id].pid,
		(u32)sizeof(module_info[cd->panel_id].vid), module_info[cd->panel_id].vid);
	return NO_ERR;
}
...
static int touch_driver_power_on_for_9896(struct thp_device *tdev)
{
	...
	ret = thp_power_supply_ctrl(THP_VCC, THP_POWER_ON,
		NO_DELAY, tdev->thp_core->panel_id);
	if (ret)
		thp_log_err("%s:vcc fail\n", __func__);
	udelay(AFTER_VCC_POWERON_DELAY);
	ret = thp_power_supply_ctrl(THP_IOVDD, THP_POWER_ON, 3,
		tdev->thp_core->panel_id); /* 3ms */
	if (ret)
...
static void touch_driver_slowdown_flash_controller_speed(struct thp_device *tdev)
{
	...
	touch_driver_spi_write(tdev, GOODIX_FLASH_CTRLLER_REG1_GT9897, &regval, 1);
	regval = GOODIX_FLASH_RW_MODE_GT9897;
	touch_driver_spi_write(tdev, GOODIX_FLASH_CTRLLER_REG2_GT9897, &regval, 1);
	mdelay(GOODIX_DELAY_5MS_9897);
	regval = GOODIX_FLASH_EN_MODE_GT9897;
	touch_driver_spi_write(tdev, GOODIX_FLASH_CTRLLER_REG3_GT9897, &regval, 1);
}
...
static int thp_power_on_iovdd_vcc_9897(struct thp_core_data *cd)
{
	int ret;
		
	ret = thp_power_supply_ctrl(THP_IOVDD, THP_POWER_ON,
		AFTER_IOVDD_DELAY_9897, cd->panel_id);
	if (ret)
		thp_log_err("%s:vddio fail\n", __func__);
	ret = thp_power_supply_ctrl(THP_VCC, THP_POWER_ON,
...
#endif
	gpio_direction_output(tdev->gpios->rst_gpio, GPIO_LOW);
	ret = thp_power_on_iovdd_vcc_9897(cd);
	gpio_direction_output(tdev->gpios->rst_gpio, GPIO_HIGH);
#else
	if (cd->support_control_cs_off) {
		pinctrl_select_state(tdev->thp_core->pctrl,
...
static void touch_driver_show_afe_debug_info(u8 *debug_buf, int len,
	int debug_info_count)
{
	int i;
	
	if ((len <= 0) || (len > GOODIX_MAX_AFE_LEN) ||
		((debug_info_count * DEBUG_AFE_DATA_BUF_OFFSET) > GOODIX_MAX_AFE_LEN)) {
		thp_log_err("%s:invalid len:%d, count:%d\n", __func__,
			len, debug_info_count);
		return;
	}

	for (i = 0; i < debug_info_count; i++)
		thp_log_info("debug_buf[%d-%d] %*ph\n", i * DEBUG_AFE_DATA_BUF_OFFSET,
			(i + 1) * DEBUG_AFE_DATA_BUF_OFFSET, DEBUG_AFE_DATA_BUF_LEN,
			debug_buf + i * DEBUG_AFE_DATA_BUF_OFFSET);
}

static void touch_driver_get_afe_addr_for_9896(const u8 * const afe_data_buf,
	unsigned int len, u32 panel_id)
{
	...
	thp_log_info("%s: frame addr 0x%x, len %d, cmd addr 0x%x\n",
		__func__, goodix_frame_addr[panel_id],
		goodix_frame_len[panel_id], goodix_cmd_addr[panel_id]);
}
...
static int touch_driver_get_afe_info_for_9896(struct thp_device *tdev)
{
	...
	for (retry = 0; retry < FW_INFO_READ_RETRY; retry++) {
		ret = touch_driver_spi_read(tdev, GOODIX_AFE_INFO_ADDR_GT9896, buf, sizeof(buf));
		if (ret) {
		...
		if ((afe_data_len == 0) || (afe_data_len > GOODIX_MAX_AFE_LEN_GT9896)) {
		...
		if (!checksum16_cmp(afe_data_buf, afe_data_len, GOODIX_BE_MODE)) {
			thp_log_info("%s: successfuly read afe data\n", __func__);
			break;
		}
		thp_log_err("%s: afe data checksum error\n", __func__);
		/* print 8 rounds of debug info  */
		touch_driver_show_afe_debug_info(afe_data_buf, GOODIX_MAX_AFE_LEN_GT9896, 8);
		mdelay(GET_AFE_INFO_RETRY_DELAY);
	}
	if (retry != FW_INFO_READ_RETRY) {
		touch_driver_get_afe_addr_for_9896(afe_data_buf,
			GOODIX_MAX_AFE_LEN_GT9896, tdev->thp_core->panel_id);
		return NO_ERR;
	}
error:
	thp_log_err("%s: failed get afe info, use default\n", __func__);
	goodix_frame_addr[tdev->thp_core->panel_id] = GOODIX_FRAME_ADDR_DEFAULT_GT9896;
	goodix_frame_len[tdev->thp_core->panel_id] = GOODIX_FRAME_LEN_MAX_GT9896;
	goodix_cmd_addr[tdev->thp_core->panel_id] = GOODIX_CMD_ADDR_DEFAULT_GT9896;
	return -EINVAL;
}
...
static int touch_driver_get_afe_info_for_9897(struct thp_device *tdev)
{
	...
	goodix_frame_addr[cd->panel_id] = GOODIX_FRAME_ADDR_DEFAULT_GT9897;
	goodix_frame_len[cd->panel_id] = GOODIX_FRAME_LEN_MAX_GT9897;
	goodix_cmd_addr[cd->panel_id] = GOODIX_CMD_ADDR_DEFAULT_GT9897;
	thp_log_info("addr info, frame 0x%x:%d, cmd 0x%x\n", goodix_frame_addr[cd->panel_id],
		goodix_frame_len[cd->panel_id], goodix_cmd_addr[cd->panel_id]);
	return NO_ERR;
}
...
static int touch_driver_get_afe_info_for_9966(struct thp_device *tdev)
{
	...
	if (cd->support_vendor_ic_type == GT9916) {
		goodix_frame_addr[cd->panel_id] = GOODIX_FRAME_ADDR_DEFAULT_GT9916;
		goodix_frame_len[cd->panel_id] = GOODIX_FRAME_LEN_MAX_GT9916;
	} else {
		goodix_frame_addr[cd->panel_id] = GOODIX_FRAME_ADDR_DEFAULT_GT9966;
		goodix_frame_len[cd->panel_id] = GOODIX_FRAME_LEN_MAX_GT9966;
	}
	goodix_cmd_addr[cd->panel_id] = GOODIX_CMD_ADDR_DEFAULT_GT9966;
	thp_log_info("%s:addr info, frame 0x%x:%d, cmd 0x%x\n",
		__func__, goodix_frame_addr[cd->panel_id],
		goodix_frame_len[cd->panel_id], goodix_cmd_addr[cd->panel_id]);
	return NO_ERR;
}

static void touch_driver_get_afe_addr_for_default(const u8 * const afe_data_buf,
	unsigned int len, u32 panel_id)
{
	if ((len == 0) || (len > GOODIX_MAX_AFE_LEN)) {
		thp_log_err("%s:invalid len:%d\n", __func__, len);
...
static int touch_driver_get_afe_info_for_default(struct thp_device *tdev)
{
	int ret;
	int retry;
	...
	for (retry = 0; retry < FW_INFO_READ_RETRY; retry++) {
		ret = touch_driver_spi_read(tdev, GOODIX_AFE_INFO_ADDR, buf, sizeof(buf));
		if (ret) {
			thp_log_err("%s: failed read afe data length, ret %d\n", __func__, ret);
			goto err_out;
		}
		...
		ret = touch_driver_spi_read(tdev, GOODIX_AFE_INFO_ADDR + INFO_ADDR_BUF_LEN,
			afe_data_buf, afe_data_len);
		if (ret) {
			thp_log_err("%s: failed read afe data, ret %d\n", __func__, ret);
			goto err_out;
		...
		thp_log_err("%s: afe data checksum error, retry\n", __func__);
		/* print 5 rounds of debug info  */
		touch_driver_show_afe_debug_info(afe_data_buf, GOODIX_MAX_AFE_LEN, 5);
		mdelay(GET_AFE_INFO_RETRY_DELAY);
	}

	if (retry != FW_INFO_READ_RETRY) {
		touch_driver_get_afe_addr_for_default(afe_data_buf, GOODIX_MAX_AFE_LEN, cd->panel_id);
		return NO_ERR;
	}
err_out:
	thp_log_err("%s: failed get afe info, use default\n", __func__);
	goodix_frame_addr[cd->panel_id] = GOODIX_FRAME_ADDR_DEFAULT;
	goodix_cmd_addr[cd->panel_id] = GOODIX_CMD_ADDR_DEFAULT;
	thp_log_info("%s: frame addr 0x%x, len %d, cmd addr 0x%x\n",
		__func__, goodix_frame_addr[cd->panel_id],
		goodix_frame_len[cd->panel_id], goodix_cmd_addr[cd->panel_id]);
	thp_log_info("%s: fw status addr 0x%x, len %d\n", __func__,
...
static int touch_driver_get_afe_info(struct thp_device *tdev)
{
	...
	return touch_driver_get_afe_info_for_default(tdev);
}

static void fastboot_booting_mm_release(struct thp_device *tdev)
{
	struct thp_core_data *cd = NULL;

	if (!tdev) {
		thp_log_err("%s: tdev is null, return\n", __func__);
		return;
	}
	cd = tdev->thp_core;
	if (tdev->thp_core->fast_booting_solution) {
		...
		kfree(tdev->rx_buff);
		tdev->rx_buff = NULL;
		kfree(tx_frame_buff[cd->panel_id]);
		tx_frame_buff[cd->panel_id] = NULL;
		kfree(rx_frame_buff[cd->panel_id]);
		rx_frame_buff[cd->panel_id] = NULL;
		kfree(tdev);
		tdev = NULL;
	}
}

static int alloc_spi_memory_in_dma(struct thp_core_data *cd)
{
	thp_log_info("%s: apply_spi_memory_in_dma: %d\n", __func__, cd->apply_spi_memory_in_dma);
	if (cd->apply_spi_memory_in_dma)
		goodix_spi_tx_buf[cd->panel_id] = kzalloc(READ_CMD_BUF_LEN, GFP_KERNEL | GFP_DMA);
	else
		goodix_spi_tx_buf[cd->panel_id] = kzalloc(READ_CMD_BUF_LEN, GFP_KERNEL);
	if (goodix_spi_tx_buf[cd->panel_id] == NULL) {
		thp_log_err("%s: panel %u goodix_spi_tx_buf null\n", __func__, cd->panel_id);
		return -ENOMEM;
	}
	return NO_ERR;
}

static int chip_detect_fail_check_communication(struct thp_device *tdev)
{
	int ret;
	struct thp_core_data *cd = tdev->thp_core;
	...
	if (touch_driver_communication_check(tdev)) {
		...
		if (cd->not_reuse_ic_type) {
			touch_driver_power_release(cd->panel_id);
			return -ENODEV;
		}
		/* check old panel */
...
int touch_driver_chip_detect(struct thp_device *tdev)
{
	...
	thp_log_info("%s: called\n", __func__);
	ret = alloc_spi_memory_in_dma(cd);
	if (ret)
		goto exit;

	thp_log_info("%s: ic_type %d\n", __func__, cd->support_vendor_ic_type);
	if (cd->support_adapter_mode) {
		ret = support_power_adapter_mode(tdev, THP_POWER_ON);
		if (ret)
			goto exit;
		touch_driver_slowdown_flash_controller_speed(tdev);
	} else {
	...
	ret = chip_detect_fail_check_communication(tdev);
	if (ret)
		goto exit;

	flag = (cd->send_bt_status_to_fw) && (cd->support_dual_chip_arch) &&
	...
	return NO_ERR;
exit:
	kfree(goodix_spi_tx_buf[cd->panel_id]);
	goodix_spi_tx_buf[cd->panel_id] = NULL;
	if (tdev->thp_core->support_adapter_mode)
		thp_power_adapter_release(&tdev->thp_core->power_adapter);
	fastboot_booting_mm_release(tdev);
	return ret;
}
...
static int touch_driver_get_next_frame_len(struct thp_device *tdev, u8 *buf)
{
	...
	if ((cur_head.nxt_frame_len < sizeof(cur_head)) ||
		(cur_head.nxt_frame_len > thp_get_max_frame_size(tdev->thp_core))) {
...
static int parse_event_info(const u8 *read_buff, unsigned int len,
	struct thp_udfp_data *udfp_data, u32 panel_id)
{
	...
	struct thp_udfp_data *udfp_data)
{
	...
	ret = touch_driver_spi_read(tdev, goodix_frame_addr[tdev->thp_core->panel_id],
		frame_buf, SCREENOFF_FRAME_LEN_GT9896);
	...
	ret = parse_event_info_gt9896(buf, UDFP_DATA_LEN_GT9896 + 1,
		udfp_data, tdev->thp_core->panel_id);
	if (ret) {
...
static int touch_driver_read_event_info_gt9897(struct thp_device *tdev, u8 *frame_buf, int length)
{
	...
	if (frame_buf[22] & (1 << 6)) {
		...
		msleep(180);
		thp_log_err("%s:enter esd_recovery\n", __func__);
		ret = touch_driver_suspend_ap(tdev);
		thp_log_err("%s:exit esd_recovery\n", __func__);
		return -EINVAL;
	}
	return ret;
}
	...
	struct thp_udfp_data *udfp_data)
{
	...
	if ((ret < 0) || le32_data_check(frame_buf, sizeof(frame_buf))) {
		thp_log_err("failed get valied frame data, %d,len %d",
			ret, sizeof(frame_buf));
		thp_log_err("fw_log:%*ph", FRAME_HEAD_LEN_GT9897S, frame_buf);
		ret = -EINVAL;
		goto out;
	}
	...
	thp_log_info("udfp:%*ph", UDFP_DATA_LEN_GT9897S - UDFP_DATA_LEN_MIN,
		buf + UDFP_DATA_LEN_MIN);
	ret = parse_event_info(buf, UDFP_DATA_LEN_GT9897S + 1, udfp_data, tdev->thp_core->panel_id);
	if (ret) {
		thp_log_err("%s: parse_event_info fail\n", __func__);
		goto out;
	}
	...
	char *buf, unsigned int len)
{
	...
	return touch_driver_spi_read_frame(tdev, goodix_frame_addr[tdev->thp_core->panel_id], buf, len);
}
	...
	unsigned int *gesture_wakeup_value)
{
	...
	if (retval != 0) {
		thp_log_err("[%s] ->get event failed\n", __func__);
		return -EINVAL;
	}
	return NO_ERR;
}
	...
	struct thp_device *tdev, u8 switch_value)
{
	...
	if (switch_value) {
		result = touch_driver_switch_cmd(tdev,
			cd->cmd_gesture_mode, FEATURE_ENABLE);
		if (result)
		...
		if (result || retval) {
			...
			work_status[cd->panel_id] |= IC_STATUS_GESTURE;
			thp_log_info("enable gesture mode\n");
		}
	} else {
		retval =  touch_driver_switch_cmd(tdev,
			cd->cmd_gesture_mode, FEATURE_DISABLE);
		if (retval) {
	...
	unsigned int *gesture_wakeup_value)
{
	...
	if (!(work_status[cd->panel_id] & IC_STATUS_GESTURE)) {
		thp_log_err("%s:please enable gesture mode first\n", __func__);
		retval = -EINVAL;
		goto err_out;
	}
	...
	for (i = 0; i < GESTURE_EVENT_RETRY_TIME; i++) {
		retval = touch_driver_spi_read(tdev, goodix_frame_addr[cd->panel_id],
	...
	/* clean sync flag */
	if (touch_driver_spi_write(tdev, goodix_frame_addr[cd->panel_id], &sync_flag, 1))
	...
	struct thp_device *tdev, unsigned int *gesture_wakeup_value)
{
	...
	gesture_enable = work_status[cd->panel_id] & IC_STATUS_GESTURE;
	if (!gesture_enable) {
		thp_log_err("please enable gesture mode first\n");
		retval = -EINVAL;
		goto err_out;
...
static int touch_driver_resume(struct thp_device *tdev)
{
	...
	thp_log_info("%s, ic_type %d\n", __func__,
		cd->support_vendor_ic_type);
	pen_clk_control(CLK_ENABLE, cd->panel_id);
	...
	if (cd->sensorhub_support_gesture) {
		if (g_power_off_staus[cd->panel_id] == WORK_IN_SUSPEND)
			return touch_driver_resume_gesture(tdev);
		else
			return touch_driver_power_on_for_9897(tdev);
	}
	resume_flag = (cd->easy_wakeup_info.sleep_mode == TS_GESTURE_MODE) ||
		g_thp_udfp_status[cd->panel_id] || goodix_stylus_status[cd->panel_id] ||
		g_aod_touch_status[cd->panel_id] || goodix_ring_setting_switch[cd->panel_id] ||
		cd->power_cfg.power_supply_mode;
	if (resume_flag) {
#ifdef CONFIG_HUAWEI_SHB_THP
...
static int touch_driver_after_resume(struct thp_device *tdev)
{
	...
	thp_do_time_delay(tdev->timing_config.boot_reset_after_delay_ms);
	if (!g_thp_udfp_status[cd->panel_id] && !is_pt_test_mode(tdev)) {
...
static bool touch_driver_get_stylus3_status(struct thp_device *tdev)
{
	...
	cur_stylus3_status = ((stylus3_bt_status == BT_STYLUS3_CONNECTED_MASK) &&
		(stylus_adsorption_status != STYLUS3_ADSORPTION_MASK) &&
		(current_stylus3_work_mode == 0) && (current_app_dsi_status == 0));
...
static int touch_driver_set_ap_state(struct thp_device *tdev)
{
	...
	if (g_thp_udfp_status[cd->panel_id]) {
...
static int touch_driver_suspend_ap(struct thp_device *tdev)
{
	...
	if (cd->easy_wakeup_info.sleep_mode == TS_GESTURE_MODE) {
		...
		if (((cd->support_vendor_ic_type == GT9897) ||
			(cd->support_vendor_ic_type == GT9916)) &&
			cd->gesture_mode_double_tap)
			ret = touch_driver_wakeup_gesture_enable_switch_9897(tdev, FEATURE_ENABLE);
		else
			ret = touch_driver_wakeup_gesture_enable_switch(tdev, FEATURE_ENABLE);
		if (ret)
...
#ifdef CONFIG_HUAWEI_SHB_THP
	if (cd->tsa_event_to_udfp && g_thp_udfp_status[cd->panel_id]) {
		mdelay(10);
		ret = send_tp_ap_event(sizeof(g_thp_udfp_status[cd->panel_id]),
			(void *)&g_thp_udfp_status[cd->panel_id],
...
static void touch_driver_poweroff_status(struct thp_device *tdev)
{
	...
	cd->poweroff_function_status =
		...
		(g_thp_udfp_status[cd->panel_id] << THP_TPUD_ON) |
		(cd->ring_setting_switch << THP_RING_SUPPORT) |
		(cd->ring_switch << THP_RING_ON) |
		(goodix_stylus_status[cd->panel_id] << THP_PEN_MODE_ON) |
...
static int touch_driver_suspend_gesture(struct thp_device *tdev)
{
	...
	thp_log_info("%s:aod_touch_status:%u\n", __func__, g_aod_touch_status[cd->panel_id]);
	if (cd->sensorhub_support_gesture == 1) {
		if ((!cd->easy_wakeup_info.aod_mode) &&
			!(g_aod_touch_status[cd->panel_id] && (cd->multi_panel_index == SINGLE_TOUCH_PANEL)))
...
static int touch_driver_suspend_poweroff_mode(struct thp_device *tdev)
{
	...
	g_power_off_staus[tdev->thp_core->panel_id] = POWER_OFF_IN_SUSPEND;
	if (tdev->thp_core->power_cfg.power_supply_mode)
...
static int touch_driver_suspend(struct thp_device *tdev)
{
	...
	cd = tdev->thp_core;
	g_thp_udfp_status[cd->panel_id] = cd->suspend_udfp_status;
	goodix_stylus_status[cd->panel_id] = !!(thp_get_status(THP_STATUS_STYLUS, cd->panel_id)) |
		(cd->suspend_stylus3_status & (cd->support_quick_note ? cd->quick_note : 1));
	g_aod_touch_status[cd->panel_id] = cd->aod_touch_status;
	if (((cd->support_vendor_ic_type == GT9897) ||
		(cd->support_vendor_ic_type == GT9966)) &&
		(cd->support_dual_chip_arch) &&
		(cd->easy_wakeup_info.sleep_mode == TS_POWER_OFF_MODE))
		goodix_stylus_status[cd->panel_id] = FEATURE_DISABLE;
	goodix_ring_setting_switch[cd->panel_id] = cd->ring_setting_switch;
	thp_log_info("%s:gesture_status:%d,finger_status:%u\n",
		__func__, cd->easy_wakeup_info.sleep_mode, g_thp_udfp_status[cd->panel_id]);
	thp_log_info(
		"%s:ring_support:%u,ring_switch:%u,phone_status:%u,ring_setting_switch:%u\n",
		__func__, cd->support_ring_feature, cd->ring_switch,
		cd->phone_status, goodix_ring_setting_switch[cd->panel_id]);
	thp_log_info("%s:stylus_status:%u,aod_touch_status:%u\n", __func__,
		goodix_stylus_status[cd->panel_id], cd->aod_touch_status);
	...
	gesture_status = cd->sensorhub_support_gesture &&
		...
		(g_aod_touch_status[cd->panel_id] && (cd->multi_panel_index == SINGLE_TOUCH_PANEL)));
	if (gesture_status) {
		g_power_off_staus[cd->panel_id] = WORK_IN_SUSPEND;
		return touch_driver_suspend_gesture(tdev);
	}
	if (is_pt_test_mode(tdev))
		ret = touch_driver_suspend_pt_mode(tdev);
	else if (g_thp_udfp_status[cd->panel_id] || goodix_ring_setting_switch[cd->panel_id] ||
...
static int touch_driver_get_sn(struct thp_device *tdev, char *out, unsigned int len)
{
	...
	if (tdev->thp_core->support_oem_info != THP_OEM_INFO_READ_FROM_TP) {
		thp_log_err("%s not support read sn\n", __func__);
		return -EINVAL;
	}
...
static int touch_driver_switch_int_sh(struct thp_device *tdev)
{
	...
	if ((tdev->thp_core->support_vendor_ic_type == GT9897) ||
		...
		retval = touch_driver_switch_cmd(tdev, CMD_SWITCH_INT_PIN,
			CMD_SWITCH_INT_SH);
	if (retval != 0) {
		thp_log_err("%s: switch int to shb failed\n", __func__);
		return -EINVAL;
	}
	return NO_ERR;
}
...
static int touch_driver_switch_int_ap(struct thp_device *tdev)
{
	...
	if ((tdev->thp_core->support_vendor_ic_type == GT9897) ||
		...
		retval = touch_driver_switch_cmd(tdev, CMD_SWITCH_INT_PIN,
			CMD_SWITCH_INT_AP);
	if (retval != 0) {
		thp_log_err("%s: switch int to ap failed\n", __func__);
		return -EINVAL;
	}
	return NO_ERR;
}
...
static int thp_module_init(u32 panel_id)
{
	...
	dev->tx_buff = kzalloc(thp_get_max_frame_size(cd), GFP_KERNEL);
	dev->rx_buff = kzalloc(thp_get_max_frame_size(cd), GFP_KERNEL);
	tx_frame_buff[cd->panel_id] = kzalloc(thp_get_max_frame_size(cd), GFP_KERNEL);
	rx_frame_buff[cd->panel_id] = kzalloc(thp_get_max_frame_size(cd), GFP_KERNEL);
	if ((dev->tx_buff == NULL) || (dev->rx_buff == NULL) ||
		(tx_frame_buff[cd->panel_id] == NULL) || (rx_frame_buff[cd->panel_id] == NULL)) {
