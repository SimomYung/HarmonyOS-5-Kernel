...
#include "bus_adapter/huawei_spi_bus_platform.h"
	...
	(!defined CONFIG_HUAWEI_THP_UDP) && (!defined CONFIG_DKMD_ENABLE))
#include "hisi_fb.h"
#endif
...
static u32 g_in_screen_switch;
static u32 g_stored_panel;
static u8 g_need_check_current_panel;
	...
	struct device_attribute *attr, char *buf)
{
	...
	int len = snprintf(buf, PAGE_SIZE, "%u -> %u\n",
	...
	struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (cd->not_support_cover_mode) {
		thp_log_err("%s: panel%d not support\n", __func__, panel_id);
		return count;
	}
...
static ssize_t thp_feature_status_store(struct device *dev,
	struct device_attribute *attr, const char *buf, size_t count)
{
	u32 panel_id = thp_get_panel_id_from_dev(dev);
	struct thp_core_data *cd = thp_get_core_data(panel_id);
	...
	const int scanf_num = 2; // used for sscanf_s to judge correct parse
	int ret;

	// The first four characters indicate feature, the rest indicate config
	thp_log_info("%s: buf = %s\n", __func__, buf);
	ret = sscanf_s(buf, "%4u%u", &feature, &status);
	if (ret != scanf_num) {
		thp_log_err("%s: illegal input\n", __func__);
		return -EINVAL;
	}

	if (!cd->support_quick_note) {
		thp_log_err("%s: not support quick note\n", __func__);
		return -EINVAL;
	}
	cd->quick_note = status;
	thp_log_info("%s: set feature_status %u to %u\n", __func__, feature, !!status);
	return count;
}

static ssize_t thp_feature_status_show(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	u32 panel_id = thp_get_panel_id_from_dev(dev);
	struct thp_core_data *cd = thp_get_core_data(panel_id);

	thp_log_info("%s: quick_note: %u\n", __func__, cd->quick_note);

	return snprintf_s(buf, PAGE_SIZE, PAGE_SIZE - 1, "quick_note: %u\n", cd->quick_note);
}
	...
	struct device_attribute *attr, const char *buf, size_t count)
{
	...
	cd->suspend_udfp_status = (u32)thp_get_status(THP_STATUS_UDFP, cd->panel_id);
...
static ssize_t send_stylus_status_in_mmi_test(int cur_status,
...
static int thp_screen_off_cap_test_control(int data, u32 panel_id)
{
	...
	cd->always_poweron_in_screenoff = (u32)data;
...
static void storage_screen_switch_status(u32 screen_status)
{
	g_need_check_current_panel = 1;
	switch (screen_status) {
	case SCREEN_FOLDED:
	case SCREEN_OFF_FOLD:
		g_stored_panel = SUB_TOUCH_PANEL;
		break;
	case SCREEN_UNFOLD:
		g_stored_panel = MAIN_TOUCH_PANEL;
		break;
	default :
		break;
	}
...
void revert_screen_status(void)
{
	...
	thp_power_lock();
	if (g_need_check_current_panel) {
		if (g_stored_panel != g_active_panel) {
			g_active_panel = g_stored_panel;
			cmd.command = g_stored_panel == MAIN_TOUCH_PANEL ? DUAL_THP_PANEL_REVERT_UNFOLD : DUAL_THP_PANEL_REVERT_FOLD;
...
static ssize_t thp_put_screen_status_switch_cmd(struct thp_core_data *cd,
...
static ssize_t thp_screen_status_switch(struct thp_core_data *cd,
	u32 screen_status, size_t count)
{
	...
	if (cd->support_quick_fold_unfold && g_in_screen_switch) {
		storage_screen_switch_status(screen_status);
		return count;
	}

	return thp_put_screen_status_switch_cmd(cd, screen_status, count);
}
...
static void set_jkm_status(struct thp_core_data *cd, u32 status)
{
	if (!cd || !cd->support_jkm_mode)
		return;
	if (cd->multi_screen_fold) {
		cd->jkm_status = status;
		thp_log_info("%s: status: %u\n", __func__, cd->jkm_status);
	}
}
...
static void sleep_mode_pen_clk_control(int sleep_mode_status, u32 panel_id)
{
	...
	if (sleep_mode_status) {
		stop_pen_clk_timer();
		(void)thp_pen_clk_disable(panel_id);
	} else {
		pen_clk_calibration();
	}
	thp_log_info("%s:out\n", __func__);
...
static DEVICE_ATTR(feature_status, 0660,
	thp_feature_status_show, thp_feature_status_store);
