...
#endif
#if (!defined CONFIG_HUAWEI_THP_QCOM) && (!defined CONFIG_HUAWEI_THP_MTK) && (defined CONFIG_LCDKIT_DRIVER)
...
#include "power_adapter/huawei_power_adapter.h"
...
#define SPI_SPEED_LIMIT 13000000
...
void thp_dmd_report(int dmd_num, const char *psz_format, ...)
{
	...
	if (ret < 0) {
		...
		strncpy(input_buf, PRINTF_ERROR_CODE, sizeof(PRINTF_ERROR_CODE));
	}
	va_end(args);
...
void thp_hiview_hisysevent(const char *casename, const char *psz_format, ...)
{
	...
	(void)snprintf((input_buf + len), (THP_HISYSEVENT_REPORT_SIZE - len),
...
void thp_parse_dts_u8(struct device_node *node, const char* para_name,
	u8 *para, const char *func_name)
{
	u32 value;

	if (!node || !para_name || !para || !func_name) {
		thp_log_err("%s: invalid input\n", __func__);
		return;
	}

	if (!of_property_read_u32(node, para_name, &value)) {
		*para = (u8)value;
		thp_log_info("%s:%s: %u\n", func_name, para_name, value);
	}
}
EXPORT_SYMBOL(thp_parse_dts_u8);

void thp_parse_dts_u16(struct device_node *node, const char* para_name,
	u16 *para, const char *func_name)
{
	u32 value;
...
EXPORT_SYMBOL(thp_parse_dts_u16);

void thp_parse_dts_u32(struct device_node *node, const char* para_name,
	u32 *para, const char *func_name)
{
	u32 value;
...
EXPORT_SYMBOL(thp_parse_dts_u32);

static void thp_get_lcd_product_type(void)
{
...
#endif
#ifdef CONFIG_HW_EDP_DRIVER
struct edp_ts_kit_ops edp_thp_ops = {
	.edp_ts_power_notify = thp_power_control_notify,
};
#endif
...
static long thp_check_shb_request_ap_reset(struct thp_core_data *cd)
{
	return cd->thp_shb_timeout_hard_reset;
}

#ifdef CONFIG_LIBLINUX
static long thp_check_slide_b2t(struct thp_core_data *cd)
{
	return cd->shb_slide_b2t_support;
}

static long thp_check_single_click_unlock(struct thp_core_data *cd)
{
	return cd->shb_single_click_unlock_support;
}
#endif
...
struct thp_feature_status_check thp_feature_check_table[] = {
	...
	{ THP_SUB_EVENT_REQUEST_AP_RESET, thp_check_shb_request_ap_reset },
#ifdef CONFIG_LIBLINUX
	{ THP_SUB_EVENT_SLIDE_B2T, thp_check_slide_b2t },
	{ THP_SUB_EVENT_SINGLE_CLICK_UNLOCK, thp_check_single_click_unlock },
#endif
};
...
static int thp_feature_interception_check(struct thp_core_data *cd, char sub_event)
{
	...
	for (i = 0; i < ARRAY_SIZE(thp_feature_check_table); i++) {
		if (sub_event == thp_feature_check_table[i].cmd) {
			ret = thp_feature_check_table[i].feature_check_fun(cd);
			if (!ret) {
				thp_log_err("%s: check func status done : sub_event %d not support\n", __func__, sub_event);
				return -EINVAL;
			}
			return NO_ERR;
		}
...
static int thp_report_stylus_and_press(struct thp_core_data *cd)
{
	__pm_wakeup_event(cd->thp_wake_lock, jiffies_to_msecs(HZ));
#ifdef CONFIG_LIBLINUX
	thp_inputkey_report(TS_STYLUS_WAKEUP_TO_MEMO, cd->panel_id);
#else
...
static int thp_shb_request_ap_reset(struct thp_core_data *cd)
{
	...
	gpio_direction_output(cd->gpios.rst_gpio, GPIO_LOW);
...
#ifdef CONFIG_LIBLINUX
static int thp_report_slide_b2t(struct thp_core_data *cd)
{
	__pm_wakeup_event(cd->thp_wake_lock, jiffies_to_msecs(HZ));
	thp_inputkey_report(AOD_SLIDE_UNLOCK, cd->panel_id);
	thp_log_info("%s: report slide b2t done\n", __func__);
	return NO_ERR;
}

static int thp_report_single_click_unlock(struct thp_core_data *cd)
{
	__pm_wakeup_event(cd->thp_wake_lock, jiffies_to_msecs(HZ));
	thp_inputkey_report(KEY_F4, cd->panel_id);
	thp_log_info("%s: report single click unlock done\n", __func__);
	return NO_ERR;
}
#endif

struct thp_execute_shb_event_group {
	unsigned int cmd;
	int (*execute_shb_event)(struct thp_core_data *cd);
};

struct thp_execute_shb_event_group thp_execute_shb_event_table[] = {
...
#ifdef CONFIG_LIBLINUX
	{ THP_SUB_EVENT_SLIDE_B2T, thp_report_slide_b2t },
	{ THP_SUB_EVENT_SINGLE_CLICK_UNLOCK, thp_report_single_click_unlock },
#endif
};

static int thp_execute_shb_event(struct thp_core_data *cd, char sub_event)
{
	int i;

	for (i = 0; i < ARRAY_SIZE(thp_execute_shb_event_table); i++) {
		if (sub_event == thp_execute_shb_event_table[i].cmd) {
			thp_log_info("%s: report event : %u\n", __func__, sub_event);
			return thp_execute_shb_event_table[i].execute_shb_event(cd);
		}
	}
...
int thp_send_event_to_drv(const char *data)
{
	...
	ret = thp_execute_shb_event(cd, sub_event);
	if (ret)
		return -EINVAL;
...
static struct thp_queue *thp_queue_init(struct thp_core_data *cd)
{
	...
	if (g_thp_queue_node_len > thp_get_max_frame_size(cd)) {
...
void thp_set_status_without_wakeup(int type, int status, u32 panel_id)
{
	struct thp_core_data *cd = thp_get_core_data(panel_id);
...
#ifdef CONFIG_DKMD_DKSM
static int thp_dkmd_notifier_call(struct notifier_block *nb,
	unsigned long action, void *data)
{
	struct thp_core_data *cd = thp_get_core_data(MAIN_TOUCH_PANEL);
	...
	if (!atomic_read(&cd->register_flag)) {
		thp_log_err("%s: thp have not registerd\n", __func__);
		return NOTIFY_OK;
	}
...
static struct notifier_block thp_dkmd_notifier_block = {
	.notifier_call = thp_dkmd_notifier_call,
};
#endif
...
static int send_thp_cmd_to_shb(unsigned char power_status, struct thp_core_data *cd)
{
	...
	if (cd->support_jkm_mode && cd->jkm_status) {
		cd->jkm_status = 0;
		return NO_ERR;
	}
	switch (power_status) {
...
void thp_easy_wake_mode_change(struct thp_core_data *cd)
{
...
EXPORT_SYMBOL(thp_easy_wake_mode_change);

void thp_easy_wake_mode_revert(struct thp_core_data *cd)
{
...
EXPORT_SYMBOL(thp_easy_wake_mode_revert);

static void multi_panel_suspend_device(struct thp_core_data *cd)
{
	if (cd->multi_panel_index != SINGLE_TOUCH_PANEL) {
		thp_easy_wake_mode_revert(cd);
		thp_set_status_without_wakeup(THP_STATUS_FOLD_AFT, THP_NO_NEED_FOLD_AFT, cd->panel_id);
	}
}
...
int multi_screen_status_switch_for_multi_thp(u32 current_status)
{
	struct thp_core_data *cd = thp_get_core_data(MAIN_TOUCH_PANEL);
	struct thp_core_data *cd_extra = thp_get_core_data(SUB_TOUCH_PANEL);
	...
	case SCREEN_FOLDED:
		...
		thp_log_info("%s:screen fold, status = %u\n",
			__func__, current_status);
		cd_extra->multi_screen_fold = false;
		thp_set_status_without_wakeup(THP_STATUS_FOLD_AFT, THP_NEED_FOLD_AFT, cd_extra->panel_id);
		multi_screen_switch_fold_for_dual_thp();
		break;
	case SCREEN_UNFOLD:
		cd->multi_screen_fold = false;
		thp_log_info("%s:screen unfold, status = %u\n",
			__func__, current_status);
		cd_extra->multi_screen_fold = true;
		thp_set_status_without_wakeup(THP_STATUS_FOLD_AFT, THP_NEED_FOLD_AFT, cd->panel_id);
		multi_screen_switch_unfold_for_dual_thp();
		break;
	default:
		thp_log_info("%s:invalid status = %u\n",
...
int multi_screen_status_switch(u32 current_status)
{
	...
	*/
	...
	if (cd->fold_scheme == 2) {
		(void)multi_screen_status_switch_for_multi_thp(current_status);
		revert_screen_status();
		return NO_ERR;
	}
	if ((cd->sleep_mode == TS_POWER_OFF_MODE) &&
...
static void ts_resume_device(struct thp_core_data *cd)
{
	...
	if (cd->support_daemon_init_protect && atomic_read(&(cd->fw_update_protect)) &&
	...
	if (cd->support_shb_thp) {
		/* 1: power on */
		ret = send_thp_driver_status_cmd(1, POWER_KEY_ON_CTRL, ST_CMD_TYPE_SET_SCREEN_STATUS);
		if (ret)
	...
	/* fp_event_to_udfp[UDFP_EVENT_DATA_LEN - 1] = 1 is finger up */
		...
		ret = send_tp_ap_event(UDFP_EVENT_DATA_LEN, fp_event_to_udfp,
			ST_CMD_TYPE_FINGERPRINT_EVENT);
		if (ret < 0)
			thp_log_err("%s:tsa_event notify fp err %d\n", __func__, ret);
		if (cd->tsa_event_to_udfp == TAS_EVENT_TO_UDFP_MODE_TWO) {
			ret = send_tp_ap_event(SWITCH_ENABLE_LENGTH, switch_enable, ST_CMD_TYPE_IRQ_SWITCH);
...
static void second_power_off(struct thp_core_data *cd)
{
	if (cd->thp_dev && cd->thp_dev->ops &&
		...
		cd->thp_dev->ops->second_poweroff(cd->thp_dev);
}
...
static int thp_power_control_notify_process(enum lcd_kit_ts_pm_type pm_type,
...
#endif
	if (cd->use_multi_thread_power_ctrl)
		return thp_power_control_notify_process(pm_type, cd);
	else
...
void thp_multi_resume(u32 panel_id)
{
	int err;
	struct thp_core_data *cd = thp_get_core_data(panel_id);
...
void thp_multi_suspend(u32 panel_id)
{
	int err;
	struct thp_core_data *cd = thp_get_core_data(panel_id);
...
void thp_suspend_thread(u32 panel_id)
{
	struct thp_core_data *cd = thp_get_core_data(panel_id);
...
void thp_resume_thread(u32 panel_id)
{
	struct thp_core_data *cd = thp_get_core_data(panel_id);
...
static int thp_open(struct inode *inode, struct file *filp)
{
	...
	cd->frame_size = thp_get_max_frame_size(cd);
#ifdef THP_NOVA_ONLY
	...
	mutex_unlock(&cd->mutex_frame);
	/* restore spi config */
	if (cd->support_ic_recovery && cd->tp_recovery_count == DAEMON_RESET_MAX_COUNT) {
		cd->tp_recovery_count = 0;
		ret = thp_set_spi_max_speed(SPI_SPEED_LIMIT, cd->panel_id);
	} else {
	...
	struct thp_ioctl_spi_xfer_data *spi_data, unsigned int xfer_num)
{
	...
	for (i = 0; i < xfer_num; i++) {
		if ((spi_data[i].len == 0) ||
			(spi_data[i].len > thp_get_max_frame_size(cd))) {
	...
	const void __user *data, unsigned int lock_status, u32 panel_id)
{
	...
	if (msg_len > thp_get_max_frame_size(cd) || !msg_len) {
	...
	for (i = 0, msg_len = 0; i < ioctl_spi_msg.xfer_num; i++) {
		if (ioctl_spi_xfer[i].tx) {
			if (copy_from_user(tx_buf + msg_len,
					ioctl_spi_xfer[i].tx,
	...
	for (i = 0, msg_len = 0; i < ioctl_spi_msg.xfer_num; i++) {
		if (ioctl_spi_xfer[i].rx) {
			if (copy_to_user(ioctl_spi_xfer[i].rx,
					rx_buf + msg_len,
					ioctl_spi_xfer[i].len)) {
...
void try_recovery_fake_panel_status(struct thp_core_data *cd,
...
static long thp_ioctl_get_frame(unsigned long arg, u32 panel_id)
{
	...
	if (data.buf == 0 || data.size == 0 ||
		data.size > thp_get_max_frame_size(cd) || data.tv == 0) {
...
#ifndef CONFIG_HUAWEI_THP_MTK
	gpio_direction_output(cd->gpios.rst_gpio, !!reset);
	value = gpio_get_value(cd->gpios.rst_gpio);
	thp_log_info("%s: get rst value : %d\n", __func__, value);
#else
...
static long thp_ioctl_spi_sync_ssl_bl(unsigned long arg, u32 panel_id)
{
	...
	if (sync_data.tx) {
		if (copy_from_user(tx_buf, sync_data.tx, sync_data.size)) {
	...
	if (sync_data.rx) {
		if (copy_to_user(sync_data.rx, rx_buf, sync_data.size)) {
...
static void thp_disable_irq_in_irq_process(u32 panel_id)
{
	...
	if (ret) {
		if (cd->irq_enabled || desc->depth == 0) {
...
#if defined(CONFIG_MODULE_KO_TP)
...
#if defined(CONFIG_TEE_TUI)
static void thp_tui_info_init(void)
{
	if (thp_tui_info) {
		thp_log_err("%s: thp_tui_info already init\n", __func__);
		return;
	}
	thp_tui_info = kzalloc(sizeof(struct thp_tui_data), GFP_KERNEL);
	if (!thp_tui_info)
		thp_log_err("%s: thp_tui_info out of memory\n", __func__);
}

static void thp_tui_info_release(void)
{
	if (!thp_tui_info) {
		thp_log_err("%s: thp_tui_info is null\n", __func__);
		return;
	}
	kfree(thp_tui_info);
	thp_tui_info = NULL;
...
static int multi_tp_tui_switch(void *data, int secure)
{
	...
	memcpy(thp_tui_info, &tp_tui_data[panel_id], sizeof(struct thp_tui_data));
...
static void thp_tui_init(struct thp_core_data *cd)
{
	...
	if (!thp_tui_info) {
		thp_log_err("%s: thp_tui_info not inited\n", __func__);
		return;
	}
	...
	memcpy(&tp_tui_data[MAIN_TOUCH_PANEL].tp_tui_normalized_data.thp_tui_data,
		thp_tui_info, sizeof(struct thp_tui_data));
	if (cd->multi_panel_index == SINGLE_TOUCH_PANEL) {
...
static void thp_display_ops_register(void)
{
	int rc;
...
#if defined(CONFIG_HW_EDP_DRIVER)
	rc = edp_ts_kit_ops_register(&edp_thp_ops);
	if (rc)
		thp_log_info("%s:edp_ts_kit_ops_register fail\n", __func__);
#endif
}

static void thp_display_ops_unregister(void)
{
	int rc;
...
#if defined(CONFIG_HW_EDP_DRIVER)
	rc = edp_ts_kit_ops_unregister(&edp_thp_ops);
	if (rc)
		thp_log_info("%s:edp_ts_kit_ops_unregister fail\n", __func__);
#endif
}
...
static int thp_core_init(struct thp_core_data *cd)
{
	...
	mutex_init(&cd->daemon_init_mutex);
	...
	struct thp_test_config *config)
{
	...
	thp_parse_dts_u32(test_node, "pt_station_test", &config->pt_station_test, __func__);
...
void thp_chip_detect(struct thp_cmd_node *in_cmd)
{
	int ret;
	...
	struct thp_core_data *cd)
{
	...
	thp_parse_dts_u32(spi_cfg_node, THP_SUPPORT_PINCTRL, &cd->support_pinctrl, __func__);
	thp_parse_dts_u32(spi_cfg_node, PINCTRL_INIT_ENABLE, &cd->pinctrl_init_enable, __func__);
	...
	struct thp_timing_config *timing)
{
	...
	thp_parse_dts_u32(timing_cfg_node, "boot_reset_hi_delay_ms", &timing->boot_reset_hi_delay_ms, __func__);

	timing->later_power_off_delay_ms = 0;
	thp_parse_dts_u32(timing_cfg_node, "later_power_off_delay_ms", &timing->later_power_off_delay_ms, __func__);

	thp_parse_dts_u32(timing_cfg_node, "boot_reset_low_delay_ms", &timing->boot_reset_low_delay_ms, __func__);
	thp_parse_dts_u32(timing_cfg_node, "boot_reset_after_delay_ms", &timing->boot_reset_after_delay_ms, __func__);
	thp_parse_dts_u32(timing_cfg_node, "resume_reset_after_delay_ms", &timing->resume_reset_after_delay_ms, __func__);
	thp_parse_dts_u32(timing_cfg_node, "suspend_reset_after_delay_ms", &timing->suspend_reset_after_delay_ms, __func__);
	thp_parse_dts_u32(timing_cfg_node, "spi_sync_cs_hi_delay_ns", &timing->spi_sync_cs_hi_delay_ns, __func__);
	thp_parse_dts_u32(timing_cfg_node, "spi_sync_cs_low_delay_ns", &timing->spi_sync_cs_low_delay_ns, __func__);

	timing->spi_sync_cs_low_delay_us = 0;
	thp_parse_dts_u32(timing_cfg_node, "spi_sync_cs_low_delay_us", &timing->spi_sync_cs_low_delay_us, __func__);

	thp_parse_dts_u32(timing_cfg_node, "boot_vcc_on_after_delay_ms", &timing->boot_vcc_on_after_delay_ms, __func__);
	thp_parse_dts_u32(timing_cfg_node, "boot_vddio_on_after_delay_ms", &timing->boot_vddio_on_after_delay_ms, __func__);

	timing->spi_transfer_delay_us = 0;
	thp_parse_dts_u32(timing_cfg_node, "spi_transfer_delay_us", &timing->spi_transfer_delay_us, __func__);

	thp_parse_dts_u32(timing_cfg_node, "early_suspend_delay_ms", &timing->early_suspend_delay_ms, __func__);

	timing->before_pen_clk_disable_delay_ms = 0;
	thp_parse_dts_u32(timing_cfg_node, "before_pen_clk_disable_delay_ms",
		&timing->before_pen_clk_disable_delay_ms, __func__);

	thp_parse_dts_u32(timing_cfg_node, "recovery_power_delay_ms", &timing->recovery_power_delay_ms, __func__);
	thp_parse_dts_u32(timing_cfg_node, "tui_exit_wait_time_ms", &timing->tui_exit_wait_time_ms, __func__);

	thp_parse_power_timing(timing_cfg_node, timing);
...
{
	thp_parse_dts_u32(thp_node, "pen_supported", &cd->pen_supported, __func__);
	thp_parse_dts_u32(thp_node, "pen_mt_enable_flag", &cd->pen_mt_enable_flag, __func__);
	thp_parse_dts_u32(thp_node, "pen_change_protocol", &cd->pen_change_protocol, __func__);
	thp_parse_dts_u32(thp_node, "send_bt_status_to_fw", &cd->send_bt_status_to_fw, __func__);
	thp_parse_dts_u32(thp_node, "send_adsorption_status_to_fw", &cd->send_adsorption_status_to_fw, __func__);
	thp_parse_dts_u32(thp_node, "support_stylus3_plam_suppression", &cd->support_stylus3_plam_suppression, __func__);
	thp_parse_dts_u32(thp_node, "send_stylus3_workmode_to_fw", &cd->send_stylus3_workmode_to_fw, __func__);
	thp_parse_dts_u32(thp_node, "unsupported_stylus", &cd->unsupported_stylus, __func__);
	thp_parse_dts_u32(thp_node, "app_send_stylus3_status_to_fw", &cd->app_send_stylus3_status_to_fw, __func__);
	thp_parse_dts_u32(thp_node, "support_adapter_mode", &cd->support_adapter_mode, __func__);
	thp_parse_dts_u32(thp_node, "support_stylus_high_hover", &cd->support_stylus_high_hover, __func__);
	thp_parse_dts_u32(thp_node, "support_stylus_extension", &cd->support_stylus_extension, __func__);
}

static void thp_parse_multi_panel_config(
	struct device_node *thp_node, struct thp_core_data *cd)
{
	thp_parse_dts_u32(thp_node, "support_screen_switch", &cd->support_screen_switch, __func__);
#ifdef CONFIG_LIBLINUX
	thp_parse_dts_u32(thp_node, "support_quick_fold_unfold", &cd->support_quick_fold_unfold, __func__);
#endif
	thp_parse_dts_u32(thp_node, "support_ic_recovery", &cd->support_ic_recovery, __func__);
	thp_parse_dts_u32(thp_node, "need_check_first_screen_switch", &cd->need_check_first_screen_switch, __func__);

	cd->multi_panel_index = SINGLE_TOUCH_PANEL;
	thp_parse_dts_u32(thp_node, "multi_panel_index", &cd->multi_panel_index, __func__);

	cd->support_multi_panel_attach = 0;
	thp_parse_dts_u32(thp_node, "support_multi_panel_attach", &cd->support_multi_panel_attach, __func__);

	cd->support_resend_status_to_daemon = 0;
	thp_parse_dts_u32(thp_node, "support_resend_status_to_daemon", &cd->support_resend_status_to_daemon, __func__);

	cd->support_input_location = 0;
	thp_parse_dts_u32(thp_node, "support_input_location", &cd->support_input_location, __func__);
}
	...
	struct device_node *thp_node, struct thp_core_data *cd)
{
	...
	thp_parse_dts_u32(thp_node, "support_factory_mode_extra_cmd", &cd->support_factory_mode_extra_cmd, __func__);

	rc = of_property_read_u32(thp_node, "support_no_cs_pinctrl_for_mtk", &value);
	if (!rc)
		cd->not_support_cs_control = value;
	else
		thp_parse_dts_u32(thp_node, "support_no_cs_for_qcom", &cd->not_support_cs_control, __func__);
	thp_log_info("%s:not_support_cs_control %u\n", __func__, value);

	cd->support_control_cs_off = 0;
	thp_parse_dts_u32(thp_node, "support_control_cs_off", &cd->support_control_cs_off, __func__);
	...
	thp_parse_dts_u32(thp_node, "use_fb_notify_to_sr", &cd->use_fb_notify_to_sr, __func__);
	thp_parse_dts_u32(thp_node, "use_dkmd_notify_to_sr", &cd->use_dkmd_notify_to_sr, __func__);
	thp_parse_dts_u32(thp_node, "support_shb_thp_tui_common_id", &cd->support_shb_thp_tui_common_id, __func__);
	thp_parse_dts_u32(thp_node, "thp_shb_timeout_hard_reset", &cd->thp_shb_timeout_hard_reset, __func__);
	thp_parse_dts_u32(thp_node, "support_diff_resolution", &cd->support_diff_resolution, __func__);
	thp_parse_dts_u32(thp_node, "need_notify_to_roi_algo", &cd->need_notify_to_roi_algo, __func__);
	thp_parse_dts_u32(thp_node, "support_daemon_init_protect", &cd->support_daemon_init_protect, __func__);

	cd->support_reuse_ic_type = 0;
	thp_parse_dts_u32(thp_node, "support_reuse_ic_type", &cd->support_reuse_ic_type, __func__);
	thp_parse_dts_u32(thp_node, "use_thp_queue", &cd->use_thp_queue, __func__);
	thp_parse_dts_u32(thp_node, "thp_queue_buf_len", &cd->thp_queue_buf_len, __func__);
	thp_parse_dts_u32(thp_node, "supported_charger", &cd->supported_charger, __func__);
	thp_parse_dts_u32(thp_node, "tp_time_sync_support", &cd->tp_time_sync_support, __func__);
	thp_parse_dts_u32(thp_node, "support_phone_sleep_mode", &cd->support_phone_sleep_mode, __func__);

	/* for goodix detect */
	cd->not_support_ic_change_init_mode = 0;
	thp_parse_dts_u32(thp_node, "not_support_ic_change_init_mode", &cd->not_support_ic_change_init_mode, __func__);

	cd->aptouch_daemon_version = 2; /* 2 is daemon default version */
	thp_parse_dts_u32(thp_node, "aptouch_daemon_version", &cd->aptouch_daemon_version, __func__);

	cd->tp_assisted_ultrasonic_algo = 0;
	thp_parse_dts_u32(thp_node, "tp_assisted_ultrasonic_algo", &cd->tp_assisted_ultrasonic_algo, __func__);

	cd->need_spi_communication_in_suspend = 0;
	thp_parse_dts_u32(thp_node, "need_spi_communication_in_suspend", &cd->need_spi_communication_in_suspend, __func__);
...
static void thp_parse_esd_reset_config(struct device_node *thp_node,
	struct thp_core_data *cd)
{
	thp_parse_dts_u32(thp_node, "support_esd_power_reset", &cd->esd_para.support_esd_power_reset, __func__);
	if(cd->esd_para.support_esd_power_reset) {
		thp_parse_dts_u32(thp_node, "esd_delay_time", &cd->esd_para.esd_delay_time, __func__);
		thp_parse_dts_u32(thp_node, "support_gesture_mode_revert", &cd->esd_para.support_gesture_mode_revert, __func__);
	}
}

static void thp_parse_gesture_config(struct device_node *thp_node,
	struct thp_core_data *cd)
{
	int rc;
	unsigned int value = 0;
	...
	if (!rc) {
		...
		thp_log_info("%s:support_gesture_mode configed %u\n",
			__func__, value);
		thp_parse_dts_u32(thp_node, "gesture_from_sensorhub", &cd->gesture_from_sensorhub, __func__);
	}
	thp_parse_dts_u32(thp_node, "aod_display_support", &cd->aod_display_support, __func__);
	thp_parse_dts_u32(thp_node, "support_fingerprint_switch", &cd->support_fingerprint_switch, __func__);
	thp_parse_dts_u32(thp_node, "use_ap_gesture", &cd->use_ap_gesture, __func__);
	thp_parse_dts_u32(thp_node, "use_aod_power_ctrl_notify", &cd->use_aod_power_ctrl_notify, __func__);
	thp_parse_dts_u32(thp_node, "need_check_panel_from_aod", &cd->need_check_panel_from_aod, __func__);
#ifdef CONFIG_LIBLINUX
	thp_parse_dts_u32(thp_node, "shb_slide_b2t_support", &cd->shb_slide_b2t_support, __func__);
	thp_parse_dts_u32(thp_node, "shb_single_click_unlock_support", &cd->shb_single_click_unlock_support, __func__);
#endif
	thp_parse_dts_u32(thp_node, "lcd_gesture_mode_support", &cd->lcd_gesture_mode_support, __func__);

	cd->double_click_use_ap_gesture = 0;
	thp_parse_dts_u32(thp_node, "double_click_use_ap_gesture", &cd->double_click_use_ap_gesture, __func__);
}

static void thp_parse_project_id_config(struct device_node *thp_node,
	struct thp_core_data *cd)
{
	int rc;
	...
	thp_parse_dts_u32(thp_node, "edit_product_in_project_id", &cd->edit_product_in_project_id, __func__);
	...
	thp_parse_dts_u32(thp_node, "projectid_from_panel_ver", &cd->projectid_from_panel_ver, __func__);

	cd->support_projectid_from_cmdline = 0;
	thp_parse_dts_u32(thp_node, "support_projectid_from_cmdline", &cd->support_projectid_from_cmdline, __func__);

	thp_parse_project_id_map_config(thp_node, cd);
}

static void thp_parse_report_interval_adjustment_config(struct device_node *thp_node,
	struct thp_core_data *cd)
{
	int rc;
	unsigned int value = 0;
...
static void thp_parse_feature_config_with_default_value(struct device_node *thp_node,
	struct thp_core_data *cd)
{
	cd->support_vendor_ic_type = 0;
	thp_parse_dts_u32(thp_node, "support_vendor_ic_type", &cd->support_vendor_ic_type, __func__);

	cd->need_resume_reset = 0;
	thp_parse_dts_u32(thp_node, "need_resume_reset", &cd->need_resume_reset, __func__);

	cd->disable_irq_gesture_suspend = 0;
	thp_parse_dts_u32(thp_node, "disable_irq_gesture_suspend", &cd->disable_irq_gesture_suspend, __func__);

	cd->support_dual_chip_arch = 0;
	thp_parse_dts_u32(thp_node, "support_dual_chip_arch", &cd->support_dual_chip_arch, __func__);

#if (defined(CONFIG_HUAWEI_THP_MTK) || defined(CONFIG_HUAWEI_THP_QCOM))
	cd->change_spi_driving_force = 0;
	thp_parse_dts_u32(thp_node, "change_spi_driving_force", &cd->change_spi_driving_force, __func__);

	cd->need_enable_irq_wake = 0;
	thp_parse_dts_u32(thp_node, "need_enable_irq_wake", &cd->need_enable_irq_wake, __func__);
#endif
	cd->gesture_retry_times = 20; /* defult gesture retry times:20 */
	thp_parse_dts_u32(thp_node, "gesture_retry_times", &cd->gesture_retry_times, __func__);

	cd->lcd_need_get_afe_status = 0;
	thp_parse_dts_u32(thp_node, "lcd_need_get_afe_status", &cd->lcd_need_get_afe_status, __func__);

	cd->iovdd_power_on_delay_ms = 1; /* default delay 1 ms */
	thp_parse_dts_u32(thp_node, "iovdd_power_on_delay_ms", &cd->iovdd_power_on_delay_ms, __func__);

	cd->change_vendor_name = 0;
	thp_parse_dts_u32(thp_node, "change_vendor_name", &cd->change_vendor_name, __func__);

	cd->check_tp_vendor = 0;
	thp_parse_dts_u32(thp_node, "check_tp_vendor", &cd->check_tp_vendor, __func__);
}
	...
	struct thp_core_data *cd)
{
	...
	thp_parse_dts_u32(thp_node, "self_control_power", &cd->self_control_power, __func__);
	thp_parse_dts_u32(thp_node, "project_in_tp", &cd->project_in_tp, __func__);
	thp_parse_dts_u32(thp_node, "read_color_in_tp", &cd->read_color_in_tp, __func__);
	thp_parse_dts_u32(thp_node, "lcd_esd_event_upload", &cd->lcd_esd_event_upload, __func__);
	thp_parse_dts_u32(thp_node, "supported_func_indicater", &cd->supported_func_indicater, __func__);
	thp_parse_dts_u32(thp_node, "use_hwlock", &cd->use_hwlock, __func__);
	thp_parse_dts_u32(thp_node, "support_shb_thp", &cd->support_shb_thp, __func__);
	thp_parse_dts_u32(thp_node, "support_shb_thp_log", &cd->support_shb_thp_log, __func__);
	thp_parse_dts_u32(thp_node, "support_shb_thp_app_switch", &cd->support_shb_thp_app_switch, __func__);
	thp_parse_dts_u32(thp_node, "tsa_event_to_udfp", &cd->tsa_event_to_udfp, __func__);
	thp_parse_dts_u32(thp_node, "use_ap_sh_common_int", &cd->use_ap_sh_common_int, __func__);
	thp_parse_dts_u32(thp_node, "delay_work_for_pm", &cd->delay_work_for_pm, __func__);
	thp_parse_dts_u32(thp_node, "suspend_delayms_early_to_before", &cd->suspend_delayms_early_to_before, __func__);
	thp_parse_dts_u32(thp_node, "suspend_delayms_before_to_later", &cd->suspend_delayms_before_to_later, __func__);
	thp_parse_dts_u32(thp_node, "resume_delayms_early_to_later", &cd->resume_delayms_early_to_later, __func__);
	thp_parse_dts_u32(thp_node, "suspend_no_reset", &cd->suspend_no_reset, __func__);
	thp_parse_dts_u32(thp_node, "use_multi_thread_power_ctrl", &cd->use_multi_thread_power_ctrl, __func__);
	thp_parse_dts_u32(thp_node, "not_support_cover_mode", &cd->not_support_cover_mode, __func__);
	thp_parse_dts_u32(thp_node, "support_jkm_mode", &cd->support_jkm_mode, __func__);

	/*
	 * TDDI(TP powered by LCD) download fw in afe screen on,
	 * need wait interruptible to make sure of screen on done.
	 */
	thp_parse_dts_u32(thp_node, "wait_afe_screen_on_support", &cd->wait_afe_screen_on_support, __func__);
	thp_parse_dts_u32(thp_node, "need_check_irq_status", &cd->need_check_irq_status, __func__);

#if defined(CONFIG_TEE_TUI)
	thp_parse_dts_u32(thp_node, "send_tui_exit_in_suspend", &cd->send_tui_exit_in_suspend, __func__);
	thp_parse_dts_u32(thp_node, "no_need_secos_bus_init", &cd->no_need_secos_bus_init, __func__);
#endif
	...
	thp_parse_dts_u32(thp_node, "support_ring_setting_switch", &cd->support_ring_setting_switch, __func__);
	thp_parse_dts_u32(thp_node, "support_extra_key_event_input", &cd->support_extra_key_event_input, __func__);
#if defined(CONFIG_LIBLINUX)
	thp_parse_dts_u32(thp_node, "support_hand_status_dev", &cd->support_hand_status_dev, __func__);
#endif
	thp_parse_dts_u32(thp_node, "hide_product_info_en", &cd->hide_product_info_en, __func__);
	thp_parse_dts_u32(thp_node, "support_oem_info", &cd->support_oem_info, __func__);
	thp_parse_dts_u32(thp_node, "thp_compatible_tskit", &cd->thp_compatible_tskit, __func__);
	thp_parse_dts_u32(thp_node, "support_polymeric_chip", &cd->support_polymeric_chip, __func__);
	thp_parse_dts_u32(thp_node, "support_quick_note", &cd->support_quick_note, __func__);

	thp_parse_feature_config_with_default_value(thp_node, cd);
	thp_parse_gesture_config(thp_node, cd);
	thp_parse_project_id_config(thp_node, cd);
	thp_parse_report_interval_adjustment_config(thp_node, cd);
...
#ifndef CONFIG_HUAWEI_THP_MTK
	gpio_direction_output(cd->thp_dev->gpios->rst_gpio, 0);
	if (!cd->not_support_cs_control)
		gpio_direction_output(cd->thp_dev->gpios->cs_gpio, 0);
#else
	...
	struct device_node *thp_node)
{
	...
	thp_parse_dts_u8(thp_node, "power_supply_mode", &cd->power_cfg.power_supply_mode, __func__);

	if (cd->power_cfg.power_supply_mode) {
		cd->power_cfg.power_sequence = 0;
		thp_parse_dts_u8(thp_node, "power_sequence", &cd->power_cfg.power_sequence, __func__);
	}
	...
	cd->irq_flag = IRQF_TRIGGER_FALLING;
	thp_parse_dts_u32(thp_node, "irq_flag", &cd->irq_flag, __func__);

	cd->use_mdelay = 0;
	thp_parse_dts_u32(thp_node, "use_mdelay", &cd->use_mdelay, __func__);

	cd->proximity_support = PROX_NOT_SUPPORT;
	thp_parse_dts_u32(thp_node, "proximity_support", &cd->proximity_support, __func__);

	cd->platform_type = THP_PLATFORM_HISI;
	thp_parse_dts_u32(thp_node, "platform_type", &cd->platform_type, __func__);
...
int support_power_adapter_mode(struct thp_device *tdev, int status)
{
	if (!tdev){
		thp_log_info("%s:tdev is null\n", __func__);
		return -EINVAL;
	}

	if (!tdev->thp_core->power_adapter) {
		tdev->thp_core->power_adapter = power_adapter_init(tdev->thp_core->power_cfg.power_supply_mode);
		if (!tdev->thp_core->power_adapter) {
			thp_log_info("%s:power adapter is null, please check power dts config\n", __func__);
			return -EINVAL;
		}
		tdev->thp_core->power_adapter->ops->init(tdev);
	}

    if (status)
		tdev->thp_core->power_adapter->ops->poweron(tdev, &tdev->timing_config.poweron_timing);
	else
		tdev->thp_core->power_adapter->ops->poweroff(tdev, &tdev->timing_config.poweroff_timing);
	return NO_ERR;
}
EXPORT_SYMBOL(support_power_adapter_mode);
...
int thp_spi_probe(struct spi_device *sdev)
{
	...
	frame_read_buf = kzalloc(thp_get_max_frame_size(thp_core), GFP_KERNEL);
	if (!frame_read_buf) {
	...
	if (thp_core->use_hwlock) {
		thp_core->hwspin_lock = hwspin_lock_request_specific(thp_core->hwspin_id);
...
#ifdef CONFIG_DKMD_DKSM
	if (thp_core->use_dkmd_notify_to_sr)
		dkmd_register_client(&thp_dkmd_notifier_block);
#endif
...
int thp_spi_remove(struct spi_device *sdev)
{
...
#ifdef CONFIG_DKMD_DKSM
	if (cd->use_dkmd_notify_to_sr)
		dkmd_unregister_client(&thp_dkmd_notifier_block);
#endif
	...
	(!defined CONFIG_HUAWEI_THP_MTK))
		if (cd->charger_detect_notify.notifier_call)
			chip_charger_type_notifier_unregister(&cd->charger_detect_notify);
#endif
	...
	if (cd->charger_detect_notify.notifier_call && cd->supported_charger) {
		if (power_event_bnc_unregister(POWER_BNT_CONNECT, &cd->charger_detect_notify))
...
void thp_spi_shutdown(struct spi_device *sdev)
{
	struct thp_core_data *cd = NULL;

	if (sdev)
        cd = spi_get_drvdata(sdev);

	if (cd && atomic_read(&cd->register_flag)) {
		thp_resume(cd);
		thp_easy_wake_mode_change(cd);
		thp_early_suspend(cd);
		thp_suspend(cd);
	}
	thp_log_info("%s: shutdown out\n", __func__);
}
...
u32 thp_get_max_frame_size(struct thp_core_data *cd)
{
	u32 current_max_frame_size;

	if (!cd)
		return THP_MAX_FRAME_SIZE;

	current_max_frame_size =
		cd->product_frame_packet_num * ONE_PACKET_FRAME_SIZE + FRAME_HEAD_DATA_LEN;

	if (current_max_frame_size <= THP_MAX_FRAME_SIZE)
		return THP_MAX_FRAME_SIZE;

	return current_max_frame_size;
}
EXPORT_SYMBOL(thp_get_max_frame_size);

static void thp_parse_spi_module_init_dts(struct thp_core_data *cd, struct device_node *thp_node)
{
	cd->apply_spi_memory_in_dma = 0;
	thp_parse_dts_u32(thp_node, "apply_spi_memory_in_dma", &cd->apply_spi_memory_in_dma, __func__);

	cd->multi_spi = 0;
	thp_parse_dts_u32(thp_node, "multi_spi", &cd->multi_spi, __func__);

	cd->hwspin_id = TP_HWSPIN_LOCK_CODE;
	thp_parse_dts_u32(thp_node, "hwspin_id", &cd->hwspin_id, __func__);
}
...
static int thp_parse_module_init_dts(struct thp_core_data *cd, u32 panel_id)
{
	...
	thp_parse_dts_u32(thp_node, "fast_booting_solution", &cd->fast_booting_solution, __func__);

	cd->dual_thp = 0;
	thp_parse_dts_u32(thp_node, "dual_thp", &cd->dual_thp, __func__);

	cd->fold_scheme = 2;
	thp_parse_dts_u32(thp_node, "fold_scheme", &cd->fold_scheme, __func__);

	cd->panel_id = 0;
	thp_parse_dts_u32(thp_node, "thp_panel_id", &cd->panel_id, __func__);

	thp_parse_spi_module_init_dts(cd, thp_node);
	thp_get_lcd_product_type();
	cd->product_frame_packet_num = 8; /* default max frame size: 8K */
	thp_parse_dts_u32(thp_node, "product_frame_packet_num", &cd->product_frame_packet_num, __func__);
	return NO_ERR;
}
...
static int thp_init(u32 panel_id)
{
	...
	if (g_thp_core[panel_id]->support_bus_adapter_mode) {
		g_thp_core[panel_id]->bus_adapter = thp_bus_transfer_adapter(g_thp_core[panel_id]->bus_tpye);
		if (!g_thp_core[panel_id]->bus_adapter) {
			thp_log_err("%s: bus adapter is null\n", __func__);
			goto err;
		}
		rc = g_thp_core[panel_id]->bus_adapter->driver_ops.register_driver(panel_id);
	} else {
		thp_log_info("%s: spi register driver no adapter\n", __func__);
...
static void __exit g_thp_module_exit(void)
{
	...
	thp_log_info("%s: E\n", __func__);
#if defined(CONFIG_TEE_TUI)
	thp_tui_info_release();
#endif
	thp_power_module_exit();
	mutex_destroy(&polymeric_chip_mutex);
	mutex_destroy(&tui_mutex);
	mutex_destroy(&penclk_mutex);
	for (i = 0; i <= extra_thp_num; i++) {
		if (g_thp_core[i]->support_bus_adapter_mode) {
			if (g_thp_core[i]->bus_adapter) {
				g_thp_core[i]->bus_adapter->driver_ops.unregister_driver(i);
			}
			thp_log_info("%s: bus adapter module exit\n", __func__);
			return;
		} else {
