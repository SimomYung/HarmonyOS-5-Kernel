...
static int touch_driver_spi_read_write(struct thp_device *tdev,
...
void touch_driver_sw_reset_idle(struct thp_device *tdev)
{
	...
	ret = touch_driver_spi_read_write(tdev, w_buf, r_buf, rw_len);
	if (ret)
...
void touch_driver_bootloader_reset(struct thp_device *tdev)
{
	uint8_t *w_buf = g_nvt_ts_spi_buf.w_buf;
	uint8_t *r_buf = g_nvt_ts_spi_buf.r_buf;
	/* This register addr is for NT36870 */
...
static void touch_driver_timing_work(struct thp_device *tdev)
{
	...
	touch_driver_sw_reset_idle(tdev);
...
static void clear_crc_err_flag(struct thp_device *tdev, uint8_t *w_buf,
	uint8_t *r_buf, uint32_t crcerr_addr)
{
	...
	ret = touch_driver_spi_read_write(tdev,
		w_buf, r_buf, rw_len);
	if (ret)
		thp_log_err("%s, spi write failed\n", __func__);
...
static void check_crc_err_flag(struct thp_device *tdev, uint8_t *w_buf,
	uint8_t *r_buf, uint32_t crcerr_addr)
{
	int ret;
	int rw_len;
...
static void stop_crc_reboot(struct thp_device *tdev,
	uint8_t *w_buf, uint8_t *r_buf)
{
	...
	for (retry = 5; retry > 0; retry--) {
		...
		/* clear CRC_ERR_FLAG */
		clear_crc_err_flag(tdev, w_buf, r_buf, addr_crcerr);

		/* check CRC_ERR_FLAG */
		check_crc_err_flag(tdev, w_buf, r_buf, addr_crcerr);
		if (r_buf[2] == 0xA5)
			break;
	}
	if (retry == 0)
...
static bool touch_driver_stop_crc_reboot(struct thp_device *tdev,
	uint8_t *w_buf, uint8_t *r_buf)
{
	bool need_stop_crc_reboot_flag = false;

	/* r_buf[2] ... r_buf[7] are trim id info data */
	thp_log_info("trim id: 0x%x 0x%x 0x%x 0x%x 0x%x 0x%x\n",
	...
	if (need_stop_crc_reboot_flag)
		stop_crc_reboot(tdev, w_buf, r_buf);

	return need_stop_crc_reboot_flag;
}

static int8_t touch_driver_check_chip_ver_trim(struct thp_device *tdev)
{
	uint8_t *w_buf = g_nvt_ts_spi_buf.w_buf;
	uint8_t *r_buf = g_nvt_ts_spi_buf.r_buf;
	...
	for (retry = NVT_CHIP_VER_TRIM_RETRY_TIME; retry > 0; retry--) {
		...
		touch_driver_bootloader_reset(tdev);
		touch_driver_sw_reset_idle(tdev);
		...
		if (cd->have_internal_flash) {
			if (touch_driver_stop_crc_reboot(tdev, w_buf, r_buf))
				continue;
		}
...
static int touch_driver_init(struct thp_device *tdev)
{
	...
	thp_parse_dts_u32(nova_node, "send_cmd_retry_delay_ms",
		&cd->send_cmd_retry_delay_ms, __func__);

	cd->support_spi_resume_delay = 0;
	thp_parse_dts_u32(nova_node, "support_spi_resume_delay",
		&cd->support_spi_resume_delay, __func__);

	cd->have_internal_flash = 0;
	thp_parse_dts_u32(nova_node, "have_internal_flash",
		&cd->have_internal_flash, __func__);
...
static int touch_driver_chip_detect(struct thp_device *tdev)
{
	...
	rc =  touch_driver_check_chip_ver_trim(tdev);
	if (rc) {
	...
	char *buf, unsigned int len)
{
	...
	memset(tdev->tx_buff, 0, thp_get_max_frame_size(tdev->thp_core));
	...
	uint8_t *r_buf, uint8_t *w_buf)
{
	...
	for (i = 0; i < tdev->thp_core->gesture_retry_times; i++) {
		retval = touch_driver_spi_read_write(tdev,
...
static int touch_driver_addr_write_high_bit(struct thp_device *tdev, u32 addr, uint8_t *w_buf, uint8_t *r_buf)
{
	...
	ret = touch_driver_spi_read_write(tdev, w_buf, r_buf, SEND_CMD_LEN);
	if (ret) {
...
static int touch_driver_addr_read(struct thp_device *tdev, u32 addr, uint8_t *w_buf, uint8_t *r_buf)
{
	...
	w_buf[0] = (uint8_t)(spi_read_mask(addr));
	ret = touch_driver_spi_read_write(tdev, w_buf, r_buf, 3);
	if (ret) {
	...
	bool delay_enable)
{
	...
	int stylus_adsroption_status = 0;
