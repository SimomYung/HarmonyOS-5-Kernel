...
static int pt_mode_set_for_3909(struct thp_device *tdev);
...
static int touch_driver_send_cmd(struct spi_device *client, struct thp_device *tdev,
	...
	enum dynamic_config_id id, unsigned int value)
{
	...
	return touch_driver_send_cmd(tdev->thp_core->sdev, tdev,
...
static int touch_driver_parse_ic_feature_config(
	struct device_node *thp_node, struct thp_core_data *cd)
{
	thp_log_debug("%s:Enter!\n", __func__);
	cd->support_get_frame_read_once = 0;
	thp_parse_dts_u32(thp_node, "support_get_frame_read_once", &cd->support_get_frame_read_once, __func__);

	thp_parse_dts_u32(thp_node, "support_reset_low_pinctrl_for_mtk",
		&cd->support_reset_low_pinctrl_for_mtk, __func__);

	cd->get_frame_size_max = 0;
	thp_parse_dts_u32(thp_node, "get_frame_size_max", &cd->get_frame_size_max, __func__);

	cd->send_one_cmd_for_ap = 0;
	thp_parse_dts_u32(thp_node, "send_one_cmd_for_ap", &cd->send_one_cmd_for_ap, __func__);
	...
	thp_parse_dts_u32(thp_node, "use_normalized_ap_protocol", &use_normalized_ap_protocol, __func__);
...
static int touch_driver_init(struct thp_device *tdev)
{
	...
	thp_parse_dts_u32(syna_node, "support_deepsleep_mode",
		&cd->support_deepsleep_mode, __func__);
...
static int touch_driver_chip_detect_for_tddi(struct thp_device *tdev)
{
	...
	if (rc < 0)
		thp_log_err("%s: spi write failed\n", __func__);
	fnnum[0] = 0;
	rc = touch_driver_spi_read(tdev->thp_core->sdev, fnnum, 1); // read one byte for fnnum
	if (rc < 0)
		thp_log_err("%s:spi read failed\n", __func__);
	thp_bus_unlock(tdev);
...
static int touch_driver_chip_detect_check(unsigned char *buffer, struct thp_device *tdev)
{
	int ret = NO_ERR;
	int i = 0;

	if (buffer[0] != MESSAGE_MARKER) {
...
static int touch_driver_read_data(struct thp_device *tdev, unsigned char *buffer)
{
	int ret = NO_ERR;
	...
	ret = touch_driver_spi_read(tdev->thp_core->sdev, buffer,
		MESSAGE_DATA_NUM);
	if (ret < 0) {
...
{
	int ret;
	unsigned char *buffer = spi_read_buf;
	struct syna_tcm_identification id_info;
	...
	ret = touch_driver_read_data(tdev, buffer);
	if (ret < 0) {
		thp_log_err("%s: read data fail, ret = %d\n", __func__, ret);
		return ret;
	}

	ret = touch_driver_chip_detect_check(buffer, tdev);
	if (ret) {
		thp_log_err("%s: check fail\n", __func__);
		return ret;
	}

	memcpy(&id_info, &buffer[MESSAGE_HEADER_SIZE], sizeof(id_info));
	...
	if (frame_buf[0] != MESSAGE_MARKER) { /* 0: frame head flag */
		...
		if (frame_buf[1] == STATUS_CONTINUED_READ) { /* 1: frame status flag */
			...
			touch_driver_spi_read(tdev->thp_core->sdev,
				tdev->rx_buff,
				thp_get_max_frame_size(tdev->thp_core)); /* drop one transaction */
		}
		retval = -ENODATA;
		goto error;
	}
	...
	char *frame_buf, unsigned int len)
{
	...
	if (length > (thp_get_max_frame_size(tdev->thp_core) - FIRST_FRAME_USEFUL_LEN)) {
		thp_log_info("%s: out of length\n", __func__);
		length = thp_get_max_frame_size(tdev->thp_core) - FIRST_FRAME_USEFUL_LEN;
	}
	if (length) {
#ifndef CONFIG_HUAWEI_THP_MTK
		retval = touch_driver_spi_read(tdev->thp_core->sdev,
...
static int resume_for_self_control_power(struct thp_device *tdev)
{
	int ret;
	struct spi_device *sdev = tdev->thp_core->sdev;
...
static int resume_for_not_self_control_power(struct thp_device *tdev)
{
	enum ts_sleep_mode gesture_status = tdev->thp_core->easy_wakeup_info.sleep_mode;
	...
	if (gesture_status == TS_GESTURE_MODE &&
		...
		if (!tdev->thp_core->not_support_cs_control)
			gpio_direction_output(tdev->gpios->cs_gpio, 1);
		/* keep TP rst  high before LCD  reset hign */
		gpio_direction_output(tdev->gpios->rst_gpio, 1);
	}
#else
	...
	if (gesture_status == TS_GESTURE_MODE && tdev->thp_core->lcd_gesture_mode_support) {
		thp_log_info("gesture mode exit\n");
...
static int touch_driver_resume(struct thp_device *tdev)
{
	...
	pen_clk_control(CLK_ENABLE, tdev->thp_core->panel_id);
	if (tdev->thp_core->self_control_power) {
		ret = resume_for_self_control_power(tdev);
	} else {
		ret = resume_for_not_self_control_power(tdev);
	}
...
static int touch_driver_suspend_self_control_power(struct thp_device *tdev)
{
	...
	if (is_pt_test_mode(tdev)) {
		...
		if (tdev->thp_core->support_adapter_mode)
			ret = support_power_adapter_mode(tdev, THP_POWER_OFF);
		else
			ret = touch_driver_power_off(tdev);
		if (ret < 0) {
			thp_log_err("%s: power off Failed\n", __func__);
...
{
#ifndef CONFIG_HUAWEI_THP_MTK
	gpio_direction_output(tdev->gpios->rst_gpio, 0);
	if (!tdev->thp_core->not_support_cs_control)
		gpio_direction_output(tdev->gpios->cs_gpio, 0);
#else
	if (tdev->thp_core->support_pinctrl == 0) {
		thp_log_info("%s: not support pinctrl\n",
			__func__);
...
static int clk_calibration(struct thp_device *tdev)
{
	...
	memset(spi_write_buf, 0, SPI_READ_WRITE_SIZE);
	memcpy(spi_write_buf, syna_clk_calibration_cmd, sizeof(syna_clk_calibration_cmd));
	ret = touch_driver_spi_write(cd->sdev, spi_write_buf, 4);
	if (ret < 0)
