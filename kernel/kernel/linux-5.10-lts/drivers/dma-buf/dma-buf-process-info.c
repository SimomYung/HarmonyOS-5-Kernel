...
void init_dma_buf_task_info(struct dma_buf *buf)
{
	...
	buf->pid = current->pid;
	get_task_comm(buf->exp_thread_comm, current);
	spin_lock(&g_dmabuf_info_cache.cache_lock);
	if (buf->pid == g_dmabuf_info_cache.pid &&
		strcmp(buf->exp_thread_comm, g_dmabuf_info_cache.exp_thread_comm) == 0) {
			buf->tgid = g_dmabuf_info_cache.tgid;
			(void)strncpy_s(buf->exp_task_comm, TASK_COMM_LEN, g_dmabuf_info_cache.exp_task_comm, TASK_COMM_LEN - 1);
	} else {
		spin_unlock(&g_dmabuf_info_cache.cache_lock);
		ret = liblinux_pal_query_main_thread(buf->pid, &buf->tgid,
											 buf->exp_task_comm, TASK_COMM_LEN);
		if (ret < 0) {
			buf->tgid = 0;
			strlcpy(buf->exp_task_comm, "[kernel task]", TASK_COMM_LEN);
		}
		spin_lock(&g_dmabuf_info_cache.cache_lock);
		g_dmabuf_info_cache.pid = buf->pid;
		g_dmabuf_info_cache.tgid = buf->tgid;
		(void)strncpy_s(g_dmabuf_info_cache.exp_thread_comm, TASK_COMM_LEN, buf->exp_thread_comm, TASK_COMM_LEN - 1);
		(void)strncpy_s(g_dmabuf_info_cache.exp_task_comm, TASK_COMM_LEN, buf->exp_task_comm, TASK_COMM_LEN - 1);
	}
	spin_unlock(&g_dmabuf_info_cache.cache_lock);
}
#else
				    ...
				    unsigned int fd)
{
	...
	seq_printf(tsk_info->seq,
		   "%-16s %-16d %-16u %-16zu %-16lu %-16d %-16s %-16s %-16s",
		   tsk->comm,
		   tsk->pid,
		   fd,
		   buf->size,
	...
	seq_printf(tsk_info->seq, "%-16s \t %-16s \t",
		is_direct_reclaim_dmabuf(buf) ? "true": "false",
		is_direct_reclaim_dmabuf(buf) ? (buffer_reclaim_status_get_lock(buf->priv) ? "true": "false"): "false");

	spin_lock(&buf->name_lock);
	seq_printf(tsk_info->seq, "%-16s\n", buf->type ?: "NULL");
	spin_unlock(&buf->name_lock);
...
static int dma_buf_process_info_show(struct seq_file *s, void *unused)
{
	...
	seq_printf(s, "%-16s %-16s %-16s %-16s %-16s %-16s %-16s %-16s %-16s %-16s %-16s %-16s\n",
		   "Process", "pid", "fd", "size_bytes", "ino", "exp_pid",
		   "exp_task_comm", "buf_name", "exp_name", "can_reclaim", "is_reclaim", "type");
