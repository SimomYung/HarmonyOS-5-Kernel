...
static LIST_HEAD(sys_pool_list);
#endif
...
{
	mutex_lock(&pool_list_lock);
	list_add(&pool->list, &sys_pool_list);
	mutex_unlock(&pool_list_lock);
}
#endif
...
static struct page *dmabuf_page_pool_remove_wmark(struct dmabuf_page_pool *pool, int wmark)
{
	struct page *page = NULL;

	/* try to free low pages first */
	if (pool->count[POOL_LOWPAGE] > wmark)
		page = dmabuf_page_pool_remove(pool, POOL_LOWPAGE);

	if ((!page) && (pool->count[POOL_HIGHPAGE] > wmark))
		page = dmabuf_page_pool_remove(pool, POOL_HIGHPAGE);
...
#ifdef CONFIG_DMABUF_HEAPS_SYSTEM_MULTI_WMARK
static atomic64_t dma_pool_wmark_high = ATOMIC64_INIT(0);

s64 get_dma_pool_wmark_high_value(void)
{
	return atomic64_read(&dma_pool_wmark_high);
}
#endif
...
unsigned int dmabuf_page_pool_reclaim(unsigned int order, struct page **reclaimed_pages, unsigned int len)
{
	...
	mutex_lock(&pool_list_lock);
	/* If any pool sets flag ZONE_EXTEND_FROM_IOFAST_ONLY, do not continue */
	list_for_each_entry(pool, &pool_list, list) {
		if (pool->extend_flag & ZONE_EXTEND_FROM_IOFAST_ONLY) {
                        mutex_unlock(&pool_list_lock);
			return reclaimed;
		}
	}
	...
	list_for_each_entry(pool, &pool_list, list) {
		...
		while (nr_reclaimed < nr_needed) {
			page = dmabuf_page_pool_remove_wmark(pool, DMA_BUF_SHRINK_WATERLINE);
			if (!page)
				break;
		...
		if (nr_reclaimed >= nr_needed)
			break;
	}

#ifdef CONFIG_DMABUF_HEAPS_SYSTEM_MULTI_WMARK
	long nr_sys_reclaimed = 0;
	long nr_sys_needed = min((long)(nr_needed - nr_reclaimed),
		(system_heap_sys_pool_count() - system_heap_sys_pool_wmark_high()));

	list_for_each_entry(pool, &sys_pool_list, list) {
		if (nr_sys_needed <= 0) {
			atomic64_inc(&dma_pool_wmark_high);
			break;
		}
		...
		while (nr_sys_reclaimed < nr_sys_needed) {
			page = dmabuf_page_pool_remove_wmark(pool, DMA_BUF_SHRINK_WATERLINE);
			if (!page)
				break;
			...
			reclaimed_pages[reclaimed] = page;
			reclaimed++;
			nr_sys_reclaimed += (1UL << pool->order);
		}

		if (nr_sys_reclaimed >= nr_sys_needed)
			break;
	}
	nr_reclaimed += nr_sys_reclaimed;
#endif
	mutex_unlock(&pool_list_lock);
...
#ifdef CONFIG_DMABUF_HEAPS_SYSTEM_MULTI_WMARK
	list_for_each_entry(pool, &sys_pool_list, list) {
		seq_printf(s, "sys pool %d order %u highmem pages %lu total\n",
				pool->count[POOL_HIGHPAGE], pool->order,
				(PAGE_SIZE << pool->order) * (pool->count[POOL_HIGHPAGE]));
		seq_printf(s, "sys pool %d order %u lowmem pages %lu total\n",
				pool->count[POOL_LOWPAGE], pool->order,
				(PAGE_SIZE << pool->order) * (pool->count[POOL_LOWPAGE]));
	}
#endif
...
EXPORT_SYMBOL_GPL(dmabuf_page_pool_do_shrink);
...
static int dmabuf_page_pool_shrink(gfp_t gfp_mask, int nr_to_scan)
{
	...
	list_for_each_entry(pool, &pool_list, list) {
		if (only_scan) {
			nr_total += dmabuf_page_pool_do_shrink(pool, gfp_mask, 0);
		} else {
			if (nr_to_scan <= 0)
				break;
	...
	list_for_each_entry(pool, &sys_pool_list, list) {
		if (only_scan) {
			nr_total_tmp += dmabuf_page_pool_do_shrink(pool, gfp_mask, 0);
		} else {
			if (nr_to_scan <= 0) {
				atomic64_inc(&dma_pool_wmark_high);
				break;
	...
	if (only_scan) {
		nr_total += max(0, (nr_total_tmp - system_heap_sys_pool_wmark_high()));
	}
#endif
	mutex_unlock(&pool_list_lock);
