...
#include <staging/hisysevent/hisysevent_builder.h>
#else
...
#define DMD_MAX_EVENT_NAME_LENGTH (MAX_EVENT_NAME_LENGTH + 1)
// oh dmd report interface
static void report_hievent(const struct dsm_client *client)
{
	char error_name[DMD_MAX_EVENT_NAME_LENGTH] = {0};
	int ret = -1;
	int event_type = FAULT;
	struct hiview_hisysevent *event = NULL;
	int running_test_switch = 0;

	if (client->cops == NULL || client->cops->errorno_to_str == NULL) {
		dsm_log_err("errorno_to_str is NULL: %d \n", client->error_no);
		return;
	}
	ret = client->cops->errorno_to_str(client->error_no, error_name, DMD_MAX_EVENT_NAME_LENGTH);
	if (ret != 0) {
		dsm_log_err("errorno_to_str failed: %d \n", client->error_no);
		return;
	}
	if (client->cops->errorno_get_event_type != NULL)
		event_type = client->cops->errorno_get_event_type(client->error_no);
	if (event_type <= 0)  event_type = FAULT;
	if (client->cops->errorno_get_running_test_switch != NULL)
		running_test_switch = client->cops->errorno_get_running_test_switch(client->error_no);
	event = hisysevent_create(client->client_name, error_name, event_type);
	if (event == NULL) {
	...
	hisysevent_put_integer(event, "ERROR_NO", client->error_no);
	hisysevent_put_integer(event, "RUNNING_TEST_SWITCH", running_test_switch);
...
static void report_fmd_msg(const struct fmd_msg *msg, int index)
{
	struct hiview_hisysevent *event = NULL;
	int ret;
	event = hisysevent_create("FMD", "DRIVERS_FMD_MSG", STATISTIC);
	if (event == NULL) {
		dsm_log_err("create hisysevent failed: %d \n", msg->events->error_code);
		return;
	}

	hisysevent_put_integer(event, "ERROR_CODE", msg->events[index].error_code);
	hisysevent_put_integer(event, "VERSION", msg->version);
	hisysevent_put_integer(event, "DATA_SOURCE", msg->data_source);
	hisysevent_put_integer(event, "NUMBER_EVENTS", msg->num_events);
	hisysevent_put_integer(event, "ITEM_ID", msg->events[index].item_id);
	hisysevent_put_integer(event, "CYCLE", msg->events[index].cycle);
	hisysevent_put_string(event, "RESULT", msg->events[index].result);
	hisysevent_put_string(event, "STATION", msg->events[index].station);
	hisysevent_put_string(event, "BSN", msg->events[index].bsn);
	hisysevent_put_string(event, "DEVICE_NAME", msg->events[index].device_name);
	hisysevent_put_string(event, "TEST_NAME", msg->events[index].test_name);
	hisysevent_put_string(event, "VALUE", msg->events[index].value);
	hisysevent_put_string(event, "MIN_THRESHOLD", msg->events[index].min_threshold);
	hisysevent_put_string(event, "MAX_THRESHOLD", msg->events[index].max_threshold);
	hisysevent_put_string(event, "FIRMWARE", msg->events[index].firmware);
	hisysevent_put_string(event, "DESCRIPTION", msg->events[index].description);
	hisysevent_put_string(event, "FRONT_ITEM", msg->events[index].front_item);
	hisysevent_put_string(event, "ITEM_COUNTER", msg->events[index].item_counter);
	hisysevent_put_string(event, "ENVIRONMENT", msg->events[index].environment);

	ret = hisysevent_write(event);
	if (ret < 0)
		dsm_log_err("report hisysevent failed: %d \n", msg->events[index].error_code);
	hisysevent_destroy(&event);
}
#else
static void report_fmd_msg(const struct fmd_msg *msg, int index)
...
int dsm_update_client_vendor_info(struct dsm_dev *dev)
{
	...
	for (client_num = 0; client_num < g_dsm_server.client_count; client_num++) {
		...
		if(tmp == NULL ||  g_dsm_server.client_list[client_num] == NULL) {
			dsm_log_err("get dsm_client failed");
			continue;
		}
...
static bool check_dsm_status(struct dsm_dev *dev)
{
...
struct dsm_client *dsm_register_client(struct dsm_dev *dev)
{
	...
	for (i = 0; i < CLIENT_SIZE; i++) {
		...
		if(g_dsm_server.client_list[i] == NULL) {
			dsm_log_err("get dsm_client failed");
			continue;
		}
...
void dsm_copy(struct dsm_dev *dev, struct dsm_client *ptr, int i)
{
...
int dsm_client_record(struct dsm_client *client, const char *fmt, ...)
{
	...
	va_end(ap);
	if (size >= (int)avail) {
		dsm_log_err("Failed to execute vsnprintf");
		return 0;
	}
	...
	client->used_size += (unsigned int)size;
...
unsigned int dsm_client_copy(struct dsm_client *client, void *src, unsigned int sz)
{
	unsigned int size;
...
static void dsm_get_report_info(char *string_buff)
{
	...
	if (ptr) {
		...
		ret = memcpy_s(client_name, CLIENT_NAME_LEN, ptr, size);
		if (ret != EOK) {
			dsm_log_err("memcpy_s failed\n");
			return;
		}
	}
...
static int dsm_client_debug(struct dsm_client *client)
{
	dsm_log_debug("%s enter\n", __func__);
 	if (!client) {
...
#ifdef CONFIG_HUAWEI_DATA_ACQUISITION
static ssize_t dsm_length_range(struct dsm_client *client, size_t count, char __user *buf)
{
    size_t len;
    size_t ret;
	ssize_t copy_size = 0;
...
static ssize_t dsm_read(struct file *file, char __user *buf,
 			size_t count, loff_t *ppos)
 {
	...
	ssize_t copy_size = 0;
	if(dsm_client_debug(client) != 0){
		return 0;
	}
 #ifdef CONFIG_HUAWEI_DATA_ACQUISITION
    ssize_t ret;
 	if (dsm_is_errno_in_da_range(client->error_no)) {
		ret = dsm_length_range(client, count, buf);
		if(ret < 0){
			return ret;
		}
		copy_size = ret;
