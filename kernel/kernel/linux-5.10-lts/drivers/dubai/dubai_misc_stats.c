...
#define MAX_TRANSMIT_SIZE 	40960
...
#define MAX_STAT_INFO_SIZE    \
	((MAX_TRANSMIT_SIZE - sizeof(struct dubai_binder_stats_transmit)) / sizeof(struct dubai_binder_stats_info))

extern int dubai_obtain_binder_stats_info(void *stat_info, int *count, int max_count);
...
#else
	dubai_obtain_binder_stats_info(&transmit->data[0], &transmit->count, MAX_STAT_INFO_SIZE);
#endif
	mutex_unlock(&binder_stats_hash_lock);
	atomic_set(&binder_client_count, 0);
