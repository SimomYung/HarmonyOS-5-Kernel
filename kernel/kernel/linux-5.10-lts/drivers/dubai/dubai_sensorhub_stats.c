 ...
 * Description: dubai secsorhub stats
 */
...
#define IOC_PCIE_GET DUBAI_SENSORHUB_DIR_IOC(R, 10, struct pcie_data)
#define IOC_LCD_STATUS_NUMBER_GET DUBAI_SENSORHUB_DIR_IOC(R, 11, struct aod_status_number)
...
static int dubai_get_pcie_data(void __user *argp);
static int dubai_get_lcd_status_number(void __user *argp);
...
long dubai_ioctl_sensorhub(unsigned int cmd, void __user *argp)
{
	...
	case IOC_AOD_GET_INFO:
		...
		break;
	case IOC_PCIE_GET:
		rc = dubai_get_pcie_data(argp);
		break;
	case IOC_LCD_STATUS_NUMBER_GET:
		rc = dubai_get_lcd_status_number(argp);
		break;
	default:
		rc = -EINVAL;
		break;
...
static int dubai_get_all_sensor_stats(void __user *argp)
{
	...
	struct sensor_time_transmit transmit = {0};
...
{
    int ret;
	struct aod_status_number number;

	ret = dubai_fetch_sensorhub_data(DUBAI_EVENT_LCD_STATUS_NUMBER,
		&number, sizeof(struct aod_status_number));
	if (ret < 0)
		return ret;
	if (copy_to_user(argp, &number, sizeof(struct aod_status_number)))
		return -EFAULT;
...
{
#ifdef CONFIG_CONTEXTHUB_IO_DIE_PCIE
	struct pcie_data data = {0};
	struct pcie_ap_data *temp_data;

	int ap_ret = -1;
	temp_data = send_pcie_data_to_dubai();
	if (temp_data == NULL) {
		ap_ret = -1;
		dubai_err("cannot get pcie data from ap");
	} else {
		data.gpio_count = temp_data->gpio_count;
		data.i2c_count = temp_data->i2c_count;
		data.ap_time = temp_data->l0_time;
	}

	struct pcie_sensorhub_data sensorhub_data = {0};
	int ret = dubai_fetch_sensorhub_data(DUBAI_EVENT_PCIE, &sensorhub_data, sizeof(struct pcie_sensorhub_data));
	if (ret < 0) {
		dubai_err("cannot get pcie data from sensorhub");
		if (ap_ret == -1) {
			return -EFAULT;
		}
	} else {
		data.ap_time += sensorhub_data.l0_time;
		data.coretotal = sensorhub_data.pu_time > data.ap_time ? sensorhub_data.pu_time - data.ap_time : 0;
		data.pd_time = sensorhub_data.pd_time;
	}
	pcie_data_reset();
	if (copy_to_user(argp, &data, sizeof(struct pcie_data))) {
		return -EFAULT;
	}
	return 0;
#else
