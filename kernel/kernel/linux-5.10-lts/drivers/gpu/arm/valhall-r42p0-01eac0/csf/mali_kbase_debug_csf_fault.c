...
int kbase_debug_csf_fault_init(struct kbase_device *kbdev)
{
	...
	if (unlikely(!pm)) {
		pr_warn("%s: kbdev->pm is NULL", __func__);
		return -EINVAL;
	}
	atomic_set(&pm->cs_fault_work_flag, 0);
	atomic_set(&pm->runtime_sh_flag, 0);
	pm->sh_cs_fault_wq = alloc_workqueue("kbase_pm_cs_faults_proc", WQ_HIGHPRI | WQ_UNBOUND, 1);
	if (pm->sh_cs_fault_wq == NULL) {
		dev_err(kbdev->dev, "alloc sh_cs_fault_wq fail");
		return -EPERM;
	}
	INIT_WORK(&pm->sh_cs_fault_work, kbase_pm_cs_faults_proc);
#endif
#if IS_ENABLED(CONFIG_DFX_DEBUG_FS)
...
{
#if CONFIG_GPU_EWP_HEAL
	if (kbdev->pm.sh_cs_fault_wq != NULL) {
		destroy_workqueue(kbdev->pm.sh_cs_fault_wq);
		kbdev->pm.sh_cs_fault_wq = NULL;
	}
#endif
}
