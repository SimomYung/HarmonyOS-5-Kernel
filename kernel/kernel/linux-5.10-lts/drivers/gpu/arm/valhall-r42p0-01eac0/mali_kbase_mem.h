...
#ifdef CONFIG_MALI_GPU_RECLAIM
static inline struct kbase_mem_phy_alloc *kbase_mem_phy_alloc_alias_get(struct kbase_mem_phy_alloc *alloc)
{
	kref_get(&alloc->kref);
	atomic_inc(&alloc->alias_count);
	return alloc;
}

static inline struct kbase_mem_phy_alloc *kbase_mem_phy_alloc_alias_put(struct kbase_mem_phy_alloc *alloc)
{
	atomic_dec(&alloc->alias_count);
...
#ifdef CONFIG_MALI_GPU_RECLAIM
	atomic_set(&alloc->alias_count, 0);
#endif
...
#ifdef CONFIG_MALI_GPU_RECLAIM
size_t kbase_mem_pool_shrink(struct kbase_mem_pool *pool,
		size_t nr_to_shrink);
#endif
