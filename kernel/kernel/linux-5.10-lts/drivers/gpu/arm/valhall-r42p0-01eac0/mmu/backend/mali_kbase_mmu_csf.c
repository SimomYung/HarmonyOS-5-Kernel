		...
		struct kbase_as *as, struct kbase_fault *fault)
{
	 ...
	 */
	if (kbdev->csf.scheduler.state != SCHED_BUSY) {
		kbase_csf_ctx_handle_fault(kctx, fault);
	} else {
		dev_err(kbdev->dev,"kbase_gpu_report_bus_fault_and_kill: handle_fault not exec cause SCHED_BUSY");
	}
		...
		struct kbase_fault *fault)
{
...
#if (defined(CONFIG_MALI_GPU_BUG_ON) && defined(CONFIG_DFX_BB)) || defined(CONFIG_HUAWEI_DSM) || defined (CONFIG_HUAWEI_OHOS_DSM)
	...
	if (!pool->dying) {
		...
		if (dsm_report_enable(kbdev->gpu_dev_data.error_num.page_fault,
			...
			gpu_dsm_report(kbdev, GPU_PAGE_FAULT_DSM_NO,
				"Unhandled Page fault!!!! Process pid: %d ,name:%s,"
				"Reason: %s,raw fault status: 0x%X,"
				"exception type 0x%X: %s,source id 0x%X\n",
				kctx->tgid, kctx->task->comm,
				reason_str, status,
				exception_type, kbase_gpu_exception_name(exception_type), source_id);
	}
	...
	if (kbdev->csf.scheduler.state != SCHED_BUSY) {
		kbase_csf_ctx_handle_fault(kctx, fault);
		 ...
		 dev_err(kbdev->dev,"kbase_mmu_report_fault_and_kill: handle_fault not exec cause SCHED_BUSY");
	}
...
static void kbase_mmu_gpu_fault_worker(struct work_struct *data)
{
	...
	if (kbdev->csf.scheduler.state != SCHED_BUSY) {
		...
		dev_err(kbdev->dev,"kbase_mmu_gpu_fault_worker: handle_fault not exec cause SCHED_BUSY");
	}
