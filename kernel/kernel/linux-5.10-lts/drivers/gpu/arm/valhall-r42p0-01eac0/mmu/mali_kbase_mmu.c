				    ...
				    const struct kbase_mmu_hw_op_param *op_param)
{
	...
	if (err) {
		...
		/* For normal runmode,report dmd every 40 times. */
		if (dsm_report_enable(kbdev->gpu_dev_data.error_num.mmu_flush_fail, 40,
		kbdev->gpu_dev_data.factory_mode)) {
			u32 freq = 0;
#ifdef CONFIG_HW_VOTE_GPU_FREQ
			hv_get_last(kbdev->gpu_dev_data.gpu_hvdev, &freq);
#endif
			gpu_dsm_report(kbdev, DSM_GPU_MMU_FLUSH_FAILED, "current freq is %u",
...
#endif
		kbdev->gpu_dev_data.error_num.mmu_flush_fail++;
