...
#define CQ_WAIT_ENTRY_INFO_NUM 25
...
struct cq_sw_writeable_context {
	...
	uint8_t reserve1[20]; /* reserve 160 bit */
	...
	struct {
		...
		uint64_t replay_order_buf_ptr;
		uint32_t replay_order_buf_size;
		uint32_t reserve2;
	...
	struct {
		...
		uint64_t mem_attribute;
		uint32_t trans_cfg;
		uint32_t mmu_reserve[3];
	} mmu_info; /* mmu info size: 256 bit */

	uint8_t sw_reserve[32]; /* reserve: 256 bit */
...
#define CQ_PRIORITY_LOW          CQ_GLOBAL_PRIORITY_LOW_COMPUTE_H
#define CQ_PRIORITY_HIGH         CQ_GLOBAL_PRIORITY_REALTIME_COMPUTE_H
#define CQ_PRIORITY_SPECIAL_LOW  CQ_GLOBAL_PRIORITY_LOW_COMPUTE_L
