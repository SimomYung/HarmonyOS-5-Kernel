...
#define HVGR_FFAP1_REG_PARA_NUM 2u
...
struct gpu_dmd_msg {
	...
	u32 dmd_sh_flag;
	u64 dmd_check_peri_ts[MAX_DMD_CHECK_PERI_CNT];
	u32 dmd_check_peri_cnt;
	u32 dmd_fault_peri_cnt;
	u32 dmd_fault_peri_idx;
	atomic_t dmd_gaf;
	bool enable_sh_act;
...
#else
	struct kthread_worker *lowpa_wq;
	struct kthread_work lowpa_work;
#endif

	uint64_t alloc_page_size;
	struct mutex mcu_lock;
	struct mutex mcu_pa_mutex;
	struct shrinker mcu_shrink;
};

#if defined(HVGR_FACTORY_MODE) && HVGR_FACTORY_MODE != 0
#define HVGR_FAC_PV_INFO_SIZE    8u
#define HVGR_FAC_PV_VOLT_SIZE    20u
struct hvgr_fac_gpu_nv_data {
	union {
		uint8_t value;
		struct {
			uint8_t reserve   : 6;
	...
	uint8_t info[HVGR_FAC_PV_INFO_SIZE];
	uint8_t volt[HVGR_FAC_PV_VOLT_SIZE];
};
#endif
...
struct hvgr_dm_dev {
	...
	struct hvgr_ffap1_resource ffap1_reg;
	/* fcp data struct */
	...
	struct mutex fac_lock;
	struct hvgr_fac_gpu_nv_data fac_nv_data;
#endif
#if defined(HVGR_STATE_BUNDLE2) && HVGR_STATE_BUNDLE2 > 0
	struct common_mcu mcu_data;
#endif
	bool bbox_register_succ;
#if hvgr_version_ge(350)
	char *bbox_buffer;
	uint32_t bbox_buffer_size;
	atomic_t bbox_buffer_wrlen;
#endif
};

#if defined( CONFIG_HVGR_DFX_SH) || defined(HVGR_FACTORY_MODE)
...
enum hvgr_fault_addr_type {
	HVGR_FAULT_ADDR_NONE,
	HVGR_FAULT_ADDR_READ,
	HVGR_FAULT_ADDR_WRITE,
	HVGR_FAULT_ADDR_EXE,
	HVGR_FAULT_ADDR_UNKNOW,
	HVGR_FAULT_ADDR_INVALID,
	HVGR_FAULT_ADDR_RELEVENT,
};

struct hvgr_fault_info {
	char description[HVGR_DESCRIPTION_SIZE_MAX];
	uint64_t reported_addr;
	uint64_t fault_code;
	uint64_t fault_data;
	enum hvgr_fault_addr_type type;
};
#endif
