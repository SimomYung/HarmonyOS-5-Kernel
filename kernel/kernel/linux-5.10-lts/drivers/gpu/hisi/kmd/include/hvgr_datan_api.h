...
/*
 * hvgr_datan_dump_debug_bus_one_reg - dump one reg
 *
 * @gdev:        The hvgr hvgr_device info
 * @offset:      = (Offset Addr) % 0x800
 * @core_id:     = (Base Address) (0x800000 * N)
 * @dest_id:     = (Base Address) (0x8000 * M)
 * @state_type:  = (Offset Addr) / 0x800
 *
 * Return: the reg read result
 */
u32 hvgr_datan_dump_debug_bus_one_reg(struct hvgr_device *gdev, u32 offset,
	u32 core_id, u32 dest_id, u32 state_type);
...
#endif
#if (defined CONFIG_HVGR_DFX_DATAN) && (hvgr_version_ge(350))
...
#if (defined CONFIG_HVGR_DFX_DATAN) && (hvgr_version_ge(500))
