...
#include "ffrt/hts/hvgr_hts_defs.h"
#endif

#if hvgr_fe_enabled(HVGR_FE_FFTS)
#include "hvgr_ffts_defs.h"
#endif
...
struct hvgr_com_ctx {
	struct hvgr_ctx *ctx;
#if defined(HVGR_STATE_BUNDLE2) && HVGR_STATE_BUNDLE2 > 0
	struct hvgr_paslice_buffer_data pa_slice_buffer_data;
#endif
};
#endif
...
#if hvgr_version_ge(350)
	DECLARE_BITMAP(asid_bitmap, HVGR_MAX_NR_AS);
#endif
...
#if defined(HVGR_FEATURE_HTS) && HVGR_FEATURE_HTS != 0
	struct hvgr_hts_dev hts_dev;
#endif

#if hvgr_fe_enabled(HVGR_FE_FFTS)
	struct hvgr_ffts_dev ffts_dev;
#endif
	/* record for status last change time */
	struct hvgr_last_time_record time_record;
...
#define SWITCH_STR_MAX_LEN                   (40)
#define MAX_INSTANCE_COUNT                   (10)

enum context_swtich_status {
	CTX_IS_FORE_GROUND= 0,
	CTX_IS_SWITCHING,
	CTX_STATUS_ERROR,
	CTX_IS_BACK_GROUND,
};
...
enum hvgr_token_process {
	TOKEN_PROCESS_IDLE = 0,
	TOKEN_PROCESS_IRQ,
	TOKEN_PROCESS_ENQUEUE,
	TOKEN_PROCESS_WAKEUP,
	TOKEN_PROCESS_DEQUEUE,
};

struct hvgr_switch_fs {
	uint32_t same_va_total_pages;
	uint32_t real_swaped_pages;
	uint32_t cache_pages;
	pid_t  tgid;
	u32 id;
	enum context_swtich_status status;
	char status_str[SWITCH_STR_MAX_LEN];
	uint32_t start_idx;
	uint32_t end_idx;
	uint32_t diff_time[MAX_STEP];
	struct list_head node;
};

struct hvgr_context_switch {
	atomic_t same_va_total_pages;
	uint32_t real_swaped_pages;
	uint32_t background_max_pages;
	int *extendid;
	bool is_background;
	atomic_t ctx_foreground_flag;
	enum context_swtich_status status;
	struct list_head swapped_list;
	struct mutex swapped_list_mutex;
	struct mutex swap_mutex;
};
...
struct hvgr_ctx {
...
#endif
	enum hvgr_token_process token_process_step;
	...
	/* context switch  */
	struct hvgr_context_switch ctx_sw;
#endif

#if defined(MALEOON_2D_MCU_FEATURE) && MALEOON_2D_MCU_FEATURE != 0
	struct hvgr_mem_area *mcu_2d_area;
	uint64_t itcm_2d_addr;
	uint64_t itcm_2d_kva;
	uint32_t set_uni_render;
#endif

	uint32_t hts_flag;
	uint32_t ffts_flag;
#if hvgr_fe_enabled(HVGR_FE_FFTS)
	struct list_head ffts_link;
	bool issueq_binded;
#endif
#if defined(HVGR_VK_EXT_DEVICE_FAULT) && HVGR_VK_EXT_DEVICE_FAULT != 0
	struct hvgr_fault_info *fault_info;
#endif
	uint32_t api_mode;
};

static inline bool hvgr_ctx_flag(struct hvgr_ctx *ctx, uint32_t flag)
{
