...
#endif
#define HVGR_FENCE    (uint32_t)(1UL << (uint32_t)HVGR_FENCE_LOG_BIT)
#define HVGR_DVFS     (uint32_t)(1UL << (uint32_t)HVGR_DVFS_LOG_BIT)
#define HVGR_HTS      (uint32_t)(1U << (uint32_t)HVGR_HTS_LOG_BIT)
#define HVGR_FCP      (uint32_t)(1U << (uint32_t)HVGR_FCP_LOG_BIT)
#define HVGR_FFTS     (uint32_t)(1U << (uint32_t)HVGR_FFTS_LOG_BIT)
#define HVGR_DFX      (uint32_t)(1U << (uint32_t)HVGR_DFX_LOG_BIT)
#define HVGR_LOCK     (uint32_t)(1U << (uint32_t)HVGR_LOCK_LOG_BIT)
#define HVGR_IMPORT   (uint32_t)(1U << (uint32_t)HVGR_IMPORT_LOG_BIT)

#if defined(HVGR_SUPPORT_SPEC_LOW) && HVGR_SUPPORT_SPEC_LOW != 0
#define HVGR_LOG_OPEN_ALL \
	(HVGR_PM | HVGR_DM | HVGR_MEM | HVGR_CQ | HVGR_DATAN | HVGR_SCH | \
	HVGR_SYNC | HVGR_SPEC_LOW | HVGR_FENCE | HVGR_DVFS | HVGR_HTS | \
	HVGR_FCP | HVGR_FFTS | HVGR_DFX | HVGR_LOCK | HVGR_IMPORT)
#else
#define HVGR_LOG_OPEN_ALL \
	(HVGR_PM | HVGR_DM | HVGR_MEM | HVGR_CQ | HVGR_DATAN | HVGR_SCH | \
	HVGR_SYNC | HVGR_FENCE | HVGR_DVFS | HVGR_HTS | HVGR_FCP | HVGR_FFTS | \
	HVGR_DFX | HVGR_LOCK | HVGR_IMPORT)
#endif
