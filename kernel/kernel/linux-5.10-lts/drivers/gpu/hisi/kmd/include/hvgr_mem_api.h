...
#define HVGR_4GVA_SIZE_3G   (3UL * 1024 * 1024 * 1024)
#define HVGR_4GVA_SIZE_1G   (1UL * 1024 * 1024 * 1024)
...
/*
 * hvgr_flush_mmu_faults_proc() - Flush mmu bus/page fault workqueue per ctx.
 * @ctx: Pointer to ctx.
 */
void hvgr_flush_mmu_faults_proc(struct hvgr_ctx * const ctx);

/*
 * hvgr_mem_flush_pt_serial() - serial flush pt after mmu set up or mmu take down
 * @ctx: Pointer to ctx.
 * @gva: gpu va.
 * @pages: page nums
 */
void hvgr_mem_flush_pt_serial(struct hvgr_ctx * const ctx, uint64_t gva,
...
bool hvgr_mem_cpu_cahce_bypass(struct hvgr_device * const gdev);

bool hvgr_mem_gpu_cahce_bypass(struct hvgr_device * const gdev);
#endif
...
void hvgr_mem_sync_page_batch(struct hvgr_ctx * const ctx, struct page **page_arr, uint64_t uva,
	uint64_t offset_page, uint64_t offset_page_byte, uint64_t size, uint64_t sync_pages,
	uint64_t direct);
#endif
...
/* check pt map unmap page nr, if noeq, print area info */
void hvgr_hm_area_check_ppl(struct hvgr_mem_area *area);
#endif

unsigned long hvgr_mem_get_ctx_using_page(struct hvgr_ctx * const ctx);

unsigned long hvgr_mem_get_dev_using_page(struct hvgr_device * const gdev);
#endif
