...
#else
#define MMU_SC_POLICY_MASK     ((uint64_t)0x78000FF000000000)
#endif

#define MMU_SH_MASK				((uint64_t)0x300)

#if hvgr_version_ge(350)
#define MMU_IRQ_MASK_DISABLE_ALL        0x0U
#define MMU_IRQ_MASK_ENABLE_ALL         0x7U
#define MMU_ADDR_SPACE_MAX               5
#else
#define MMU_IRQ_MASK_DISABLE_ALL        0x0U
...
struct hvgr_mmu_takedown_paras {
	uint64_t va;
	uint64_t pages;
	uint32_t bitmap;
...
#else
	struct kthread_worker *fault_wq;
	struct kthread_work pf_work;
	struct kthread_work bf_work;
#endif
};
...
enum hvgr_mmu_fault_proc_step {
	HVGR_MMU_FAULT_IRQ_RECV,
	HVGR_MMU_FAULT_WORK_ALLOC,
	HVGR_MMU_FAULT_WORKER_ENTER,
	HVGR_MMU_FAULT_GET_AREA_MUTEX,
	HVGR_MMU_FAULT_ALLOC_PAGE,
	HVGR_MMU_FAULT_RESUME
};

#ifdef CONFIG_DFX_DEBUG_FS
struct hvgr_mmu_fault_time_dfx_info {
	uint64_t irq_recv_ts;
	uint64_t work_alloc_ts;
	uint64_t worker_enter_ts;
	uint64_t get_area_mutex_ts;
	uint64_t alloc_page_ts;
...
#else
	struct kthread_work eventq_work;
#endif
#ifdef CONFIG_DFX_DEBUG_FS
	struct hvgr_mmu_fault_time_dfx_info dfx_time;
#endif
};

void hvgr_mmu_clear_mmu_as_info(struct hvgr_ctx * const ctx);
#endif
 ...
 * @area: Pointer to mem area, dfx for ppl in hm kernel.
...
uint32_t hvgr_hm_mmu_tbl_st_pages(struct hvgr_ctx *ctx);
#endif
