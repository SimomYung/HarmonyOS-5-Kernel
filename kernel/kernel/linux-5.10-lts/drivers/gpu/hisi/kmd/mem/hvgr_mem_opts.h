...
void hvgr_mem_4gva_unlock(struct hvgr_ctx *ctx);

#ifdef CONFIG_LIBLINUX
#ifdef CONFIG_DMABUF_RECLAIM
unsigned long hvgr_mem_get_dmabuf_ino(struct hvgr_ctx *ctx, struct dma_buf *dmabuf);
#endif
#endif
...
static inline int hvgr_mem_mm_write_killable_lock(struct mm_struct *mm)
{
	...
	if (unlikely(down_write_killable(&mm->mmap_sem) != 0))
		return down_write_killable(&mm->mmap_sem);

	return 0;
#else
	if (unlikely(down_write_killable(&mm->mmap_lock) != 0))
