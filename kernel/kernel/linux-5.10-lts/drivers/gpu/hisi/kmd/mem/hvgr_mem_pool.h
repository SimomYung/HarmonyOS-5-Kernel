...
#define HVGR_MEM_POOL_DEV_SIZE ((64U * 1024U * 1024U) >> 12U)
#define HVGR_MEM_POOL_CTX_SIZE ((64U * 1024U * 1024U) >> 12U)
#define HVGR_MEM_POOL_OHOS_DEV_SIZE ((128U * 1024U * 1024U) >> 12U)
...
void hvgr_ctx_clear_mem_pool(struct hvgr_ctx * const ctx, struct list_head* page_list);

void hvgr_ctx_free_mem_pool(struct hvgr_ctx * const ctx);

#ifdef HVGR_FEATURE_SYSTEM_CACHE
void hvgr_ctx_clear_mem_sc_pool(struct hvgr_ctx * const ctx);

void hvgr_ctx_free_mem_sc_pool(struct hvgr_ctx * const ctx);
#endif

#if defined(CONFIG_LIBLINUX) && !defined(CONFIG_LIBLINUX_CDC)
void hvgr_ctx_update_mem_pool_shrink_cfg(struct hvgr_ctx * const ctx);
#endif
