...
struct hvgr_mem_area {
	...
	bool free_in_bg;
#endif
	struct list_head leak_node;
	uint64_t jit_pool_pages;
};

/*
 * gva: GPU virtural address
...
struct hvgr_zone_opts {
	...
	long (*import)(struct hvgr_mem_area * const area, struct hvgr_mem_import_record *record);
};
...
#if hvgr_version_lt(300)
#define MEM_LBW_BIT_MASK 0x0U
#define MEM_OSH_BIT_MASK 0x0U
#define MEM_POOL_NUM 1U
#else
#ifdef CONFIG_MM_AMA
/* 0:NOOSH-BIG 1:NOOSH-LBW 2:OSH-BIG 3:OSH-LBW */
#define MEM_LBW_BIT_MASK 0x1U
#define MEM_OSH_BIT_MASK 0x2U
#define MEM_POOL_NUM 4U
#else
/* 0:NOOSH 1:OSH */
#define MEM_LBW_BIT_MASK 0x0U
#define MEM_OSH_BIT_MASK 0x1U
#define MEM_POOL_NUM 2U
#endif
#endif
...
struct hvgr_mem_ctx {
	...
	struct shrinker tbl_shrink;
	bool tbl_shrink_init;
#else
	uint64_t *mmu_buf;
#endif
...
struct hvgr_mem_statics {
	atomic_t alloc_pages;
	atomic_t free_pages;

	atomic_t mmu_alloc_pages;
	atomic_t mmu_free_pages;
	atomic_t mmu_free;

	atomic_t pool_sync_pages;
	atomic_t pool_clear_pages;
#ifdef HVGR_FEATURE_SYSTEM_CACHE
	atomic_t sc_alloc_pages;
	atomic_t sc_free_pages;
#endif
	atomic_t ama_alloc_big;
	atomic_t ama_alloc_low;

	atomic_t pool_shrink_pages;
	atomic_t dev_free_pages;
};

struct hvgr_mem_dfx_cfg {
	bool cpu_cache_bypass;
	bool gpu_cache_bypass;
};
#endif

#if hvgr_version_ge(350)
#ifdef CONFIG_DFX_DEBUG_FS
struct hvgr_mmu_cmdq_dfx_data {
	uint32_t cnt;
	uint64_t total_time;
	uint64_t max_time;
	uint64_t min_time;
...
struct hvgr_mmu_dfx {
	...
	/* cmdq invalid asid */
	struct hvgr_mmu_cmdq_dfx_data invld_asid;

	/* cmdq invalid cd */
	struct hvgr_mmu_cmdq_dfx_data invld_cd;

	/* cmdq fault resume */
	struct hvgr_mmu_cmdq_dfx_data resume;

	/* cmdq invalid va */
	struct hvgr_mmu_cmdq_dfx_data invld_va;
};
#endif
#endif
...
struct hvgr_mem_dev {
	...
	struct hvgr_mem_dfx_cfg dfx_cfg;
	bool mem_kutf_flag;
	bool sc_disable;
	...
	/* dev pool max_size in MB from dtsi, pool_cfg in Pages */
	uint32_t pool_cfg;
#endif
};
