...
#define RTU_POWER_OFF_ENABLE             0
#define RTU_DEC_CNT_ENABLE               1
#endif

/* Power-on/off timeout interval of the GPU core, in us. */
#define HVGR_PM_CORE_TRANS_OUT_TIME 20000

enum hvgr_pm_cbit_cfg {
	HVGR_PM_CBIT_CFG_BEFORE_BL = 0,
	HVGR_PM_CBIT_CFG_AFTER_BL,
};
 ...
 * or powered off. msleep is called in this function. Therefore, this interface
 * cannot be called during interrupts or spin locks.
 *
 * @param gdev - The gdev object pointer.
...
/*
 * hvgr_pm_driver_short_wait_core_trans_end -  Short wait until the core is powered on
 * or powered off, this func can be called in irq or spin locks.
 *
 * @param gdev - The gdev object pointer.
 * @cores - Core bitmap expected to be powered on or powered off.
 * @trans_reg - Reg address.
 ...
 * @date - 2024-11-05
 */
bool hvgr_pm_driver_short_wait_core_trans_end(struct hvgr_device *gdev,
	uint32_t cores, uint32_t trans_reg, hvgr_pm_is_core_trans_end pfunc);
...
int hvgr_pm_driver_clock_on_base(struct hvgr_device *gdev);

int hvgr_pm_driver_clock_off_base(struct hvgr_device *gdev);

bool hvgr_pm_driver_gpu_config_base(struct hvgr_device *gdev);

void hvgr_pm_driver_cbit_config_base(struct hvgr_device *gdev);

/*
 * @brief power on gpc by com ctrl
 */
bool hvgr_pm_driver_gpc_pwr_on_cc(struct hvgr_device *gdev);

/*
 * @brief power off gpc by com ctrl
 */
bool hvgr_pm_driver_gpc_pwr_off_cc(struct hvgr_device *gdev);

/*
 * @brief wait gpc pwr off by com ctrl
 */
bool hvgr_pm_driver_wait_gpc_pwr_off_cc(struct hvgr_device *gdev);

/*
 * @brief set gpc core mask by com ctrl
 */
void hvgr_pm_driver_set_core_mask_cc(struct hvgr_device *gdev);
...
#if (hvgr_version_ge(300) && defined(CONFIG_DFX_DEBUG_FS))
void hvgr_pm_cbit_cache_bypass(struct hvgr_device *gdev, enum hvgr_pm_cbit_cfg status);
#endif

#if (hvgr_version_ge(350) && defined(CONFIG_DFX_DEBUG_FS))
bool hvgr_hebc_fault_hang_status(struct hvgr_device * const gdev);
void hvgr_pm_cbit_hebc_fault_hang_en(struct hvgr_device *gdev);
#endif
