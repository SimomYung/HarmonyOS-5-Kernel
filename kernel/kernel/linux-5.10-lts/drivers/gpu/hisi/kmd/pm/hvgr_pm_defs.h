...
#include "hvgr_virtual_devfreq.h"
#endif
...
struct hvgr_pm_pwr {
	...
	/* available rtu identified in bootloader */
	u32 rtu_present;

#ifdef CONFIG_HVGR_DFX_SH
	atomic_t thermal_cores_ctl_flag;
	atomic_t tried_rm_core_mask;
	/* read from nvme */
...
#if hvgr_version_ge(350)
	bool entire_booting;
	struct hvgr_pm_fast_pwr_data fast_pwr_data;
	bool tool_hwcnt;
	bool tool_hwcnt_tmp;
#endif

	/*
	 * This lock must held whenever the user set ctrl flag by
	 * 1.debugfs; 2.sysfs; 3.interface apis.
	 */
...
struct hvgr_pm_reset {
	...
	/* protect soft_reset and crg_reset */
...
struct hvgr_gpu_release_wq_status {
...
struct hvgr_pm_dev {
	...
	/* GPU release wq status */
	struct hvgr_gpu_release_wq_status wq_status;
	/* GPU virtual devfreq */
#ifdef CONFIG_GPU_VIRTUAL_DEVFREQ
	struct hvgr_pm_vfreq pm_vfreq;
#endif
};
