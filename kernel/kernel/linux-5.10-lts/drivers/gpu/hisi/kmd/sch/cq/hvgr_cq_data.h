...
#define GPU_IDLE                          0U
#define GPU_STOPPED                       6U
#define CQ_STATUS_MOD_RECORD_MAX          50U
...
struct hvgr_cq_ctx {
	...
	bool mod_priv_his;
#else
	struct timer_list speclow_dfx_timer;
#endif
#endif
...
#ifdef CONFIG_DFX_DEBUG_FS
#define HVGR_TOKEN_FINISH_RECORD_MAX 50U
struct hvgr_token_finish_idx_record {
	uint32_t rd_idx[HVGR_TOKEN_FINISH_RECORD_MAX];
	uint32_t wr_idx[HVGR_TOKEN_FINISH_RECORD_MAX];
	uint32_t pos;
};
#endif

#define HVGR_IOCTL_CALL_RECORD_MAX 10U
...
#ifdef CONFIG_DFX_DEBUG_FS
	struct hvgr_token_finish_idx_record *idx_record;
#endif
 ...
 */
struct hvgr_cq_dev {
	struct hvgr_spin_lock schedule_lock;
	spinlock_t cq_mmu_lock; /* cq mode, will lock mmu. */
#if defined(HVGR_STATE_BUNDLE2) && HVGR_STATE_BUNDLE2 > 0
	/* irq mask lock */
	spinlock_t cq_irq_lock;
	struct hvgr_ctx *destroy_ctx[CQ_CHANNEL_NUM];
	uint32_t destroy_ctx_cnt;
	...
	enum hvgr_cq_gpu_sch_status gpu_normalq_sch_sts;
