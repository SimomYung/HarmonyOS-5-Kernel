...
#define CQ_SPECIAL_LOW_DFX_TIMEOUT         500u     /* 0.5s */
...
void hvgr_cq_show_status_mod_record(struct hvgr_device * const gdev);

/*
 * @brief check if cq is idle, including normalq and issueq
 */
bool hvgr_cq_sched_is_idle(struct hvgr_device * const gdev);

#if (defined(HVGR_SUPPORT_SPEC_LOW) && HVGR_SUPPORT_SPEC_LOW != 0) && hvgr_version_ge(300)
void hvgr_change_speclow_dfx_timeout(int timeout);
#endif
