...
#define USB_INTERFACE_SUBCLASS_NOBOOT 0

int g_hid_get_desc_tries = 4;
/*
 * Module parameters.
 */
...
void usb_set_hid_get_desc_tries(int hid_get_descriptor_tries)
{
    g_hid_get_desc_tries = min(4, max(hid_get_descriptor_tries, 1));
    pr_info("usb_set_hid_get_desc_tries: set %d real %d\n", hid_get_descriptor_tries, g_hid_get_desc_tries);
}
		...
		unsigned char type, void *buf, int size)
{
	...
	retries = min(retries, g_hid_get_desc_tries);
...
static int usbhid_start(struct hid_device *hid)
{
	 ...
	 */
	if ((interface->desc.bInterfaceSubClass == USB_INTERFACE_SUBCLASS_BOOT ||
	    interface->desc.bInterfaceSubClass == USB_INTERFACE_SUBCLASS_NOBOOT) &&
	    interface->desc.bInterfaceProtocol == USB_INTERFACE_PROTOCOL_MOUSE) {
		device_set_wakeup_enable(&dev->dev, 1);
		dev_dbg(&dev->dev,
				"mouse wakeup enable\n");
...
static int usbhid_probe(struct usb_interface *intf, const struct usb_device_id *id)
{
	...
	if (usbhid == NULL) {
#ifdef CONFIG_USB_NDEV_CONFIG	/* Tv Dual-core feature macro */
		hid_warn(hid, "can't alloc usbhid, try to realloc by vzalloc.");
		usbhid = vzalloc(sizeof(*usbhid));
		if (usbhid == NULL) {
#endif
		ret = -ENOMEM;
		goto err;
#ifdef CONFIG_USB_NDEV_CONFIG
		}
		usbhid->alloc_flag = 1;
#endif
	}
