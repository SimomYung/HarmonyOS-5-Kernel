...
#ifdef CONFIG_RPMB_AGENT_IO_PRIO
...
#ifdef CONFIG_SYNC_KERNEL_SYSTIME
#define SEC_MIN 0xFFFFF
static bool g_sync_systime_from_kernel = true;
#endif
...
void send_crashed_event_response_single(const struct tc_ns_dev_file *dev_file)
{
	...
	list_for_each_entry_safe(event_data, tmp, &g_agent_control.agent_list,
		...
		if (event_data->owner == dev_file) {
			...
			/* when driver fd is closed, the agent should be unregistered or crashed.
			 * In these cases, the agent_buffer_user is no need to unmap
			 */
...
void show_agent_event_status(void)
{
	...
	spin_lock_irqsave(&g_agent_control.lock, flags);
	list_for_each_entry(event_data, &g_agent_control.agent_list, head) {
		tlogw("[show_agent_event_cnt] agent_id=0x%x, nsid=%x, "
			"send_wake_up=%u, wait_event = %u, wake_up_succ=%u, send_response=%u, response_succ=%u, ret_flag = %d\n",
			event_data->agent_id, event_data->nsid,
			event_data->agent_send_wake_up_cnt, event_data->agent_wait_event_cnt, event_data->agent_wake_up_succ_cnt,
			event_data->agent_send_response_cnt, event_data->agent_response_succ_cnt,
			event_data->ret_flag);
	}
	spin_unlock_irqrestore(&g_agent_control.lock, flags);
}
	...
	struct smc_event_data **event_data)
{
	*event_data = find_event_control(agent_id, nsid);
	...
	if (is_tui_agent(agent_id)) {
		tloge("TEE_TUI_AGENT_ID: pid-%d", CURRENT_PID);
		set_tui_caller_info(smc_cmd->dev_file_id, CURRENT_PID);
	}
	isb();
	wmb();
...
static int wait_agent_response(struct smc_event_data *event_data)
{
	...
	bool need_freeze = !(CURRENT_FLAGS & PF_KTHREAD);
	bool sig_pending = is_current_pending_signal();
	bool answered = true;
	int rc;
	...
	do {
		...
		if (need_freeze && !sig_pending) {
			...
			if (rc != -ERESTARTSYS)
				continue;
			if (is_current_pending_signal())
				sig_pending = true;
			tlogi("agent wait event is interrupted by %s, with rc: %x\n",
				sig_pending ? "signal" : "freezer", rc);
			/*
	...
	unsigned int agent_id, unsigned int nsid)
{
	...
	if (init_agent_context(agent_id, nsid, smc_cmd, &event_data) != 0)
		return -EINVAL;
	...
	/* make sure ret_flag have been set to 1, before wake_up agent */
	asm volatile("dmb sy");
	/* Wake up the agent that will process the command */
	tlogd("agent process work: wakeup the agent");
	wake_up(&event_data->wait_event_wq);
	event_data->agent_send_wake_up_cnt++;
	cmd_monitor_set_agent_status(AGENT_WAKEUP_REE_THREAD);
	...
	put_agent_event(event_data);
	event_data->agent_response_succ_cnt++;
	cmd_monitor_set_agent_status(AGENT_REE_THREAD_WORK_DONE);
...
#define WAIT_EVENT_TIME_OUT 1000000 /* 1s */
int tc_ns_wait_event(unsigned int agent_id, unsigned int nsid)
{
	...
	event_data->agent_wait_event_cnt++;

	/* agent_process may not wakeup wait_event_interruptible, use timeout
	 * to handle agent_request if ret_flag has been set to 1.
	 */
retry:
	ret = wait_event_interruptible_timeout(event_data->wait_event_wq,
		event_data->ret_flag, usecs_to_jiffies(WAIT_EVENT_TIME_OUT));
	if (ret == 0) {
		/* return 0 means condition is false until timeout, retry wait */
		goto retry;
	} else if (ret == 1) {
		/* return 1 means condition is true until timeout, wake up */
		tlogw("agent wake up by timeout\n");
#ifndef CONFIG_DISABLE_SHOW_AGENT_STATUS
		show_agent_event_status();
#endif
		ret = 0;
	} else if (ret > 1) {
		/* wake up by agent_process */
		ret = 0;
	}

	event_data->agent_wake_up_succ_cnt++;
	put_agent_event(event_data);

	/* return error to avoid agent access invalid mem when tee is rebooting */
	if (is_tee_rebooting())
		ret = -EINVAL;
...
static struct timespec ktime_to_time_spec(const ktime_t kt)
{
	return ktime_to_timespec(kt);
}
#else
static struct timespec64 ktime_to_time_spec(const ktime_t kt)
{
...
void sync_system_time_from_kernel(void)
{
	...
	struct time_spec kreal_time = {0};
	struct time_spec kboot_time = {0};

	if (!g_sync_systime_from_kernel)
		return;

	kreal_time.ts = ktime_to_time_spec((const ktime_t)ktime_get_real());
	kboot_time.ts = ktime_to_time_spec((const ktime_t)ktime_get_boottime());
	if (kreal_time.ts.tv_sec <= kboot_time.ts.tv_sec || (kreal_time.ts.tv_sec - kboot_time.ts.tv_sec) < SEC_MIN) {
		tlogd("kernel real time is not ready\n");
		return;
	}
	...
	g_sync_systime_from_kernel = false;
}
#endif
...
static void process_send_event_response(struct smc_event_data *event_data)
{
	...
	event_data->ret_flag = 0;
	/* make sure ret_flag have been set to 0, before wake_up CA */
	asm volatile("dmb sy");
	...
	event_data->agent_send_response_cnt++;
}
	...
	struct smc_event_data *event_data)
{
	...
	atomic_set(&(event_data->ca_run), 0);
	event_data->agent_send_wake_up_cnt = 0;
	event_data->agent_wait_event_cnt = 0;
	event_data->agent_wake_up_succ_cnt = 0;
	event_data->agent_send_response_cnt = 0;
	event_data->agent_response_succ_cnt = 0;
}

static int create_new_agent_node(struct tc_ns_dev_file *dev_file,
	struct smc_event_data **event_data, const struct agent_pair *agent_pair,
	void **agent_buff, uint32_t agent_buff_size)
{
	...
	*event_data = tz_alloc(sizeof(**event_data));
...
#define AGENT_BUFFER_MAX_SIZE 8192
#else
#define AGENT_BUFFER_MAX_SIZE SZ_4K
#endif
static bool is_valid_agent(unsigned int agent_id,
	unsigned int buffer_size, bool user_agent)
{
	unsigned int agent_buffer_threshold = is_ccos() ? SZ_512K : AGENT_BUFFER_MAX_SIZE;
	(void)agent_id;
	...
	const void *agent_buff, uint32_t agent_buff_size)
{
	...
	mb_pack->operation.paramtypes = TEE_PARAM_TYPE_VALUE_INPUT |
		(TEE_PARAM_TYPE_VALUE_INPUT << TEE_PARAM_NUM);
	mb_pack->operation.params[0].value.a =
		(uint32_t)mailbox_virt_to_phys((uintptr_t)agent_buff);
	mb_pack->operation.params[0].value.b =
		(uint32_t)(mailbox_virt_to_phys((uintptr_t)agent_buff) >> ADDR_TRANS_NUM);
...
{
	unsigned long buffer_map;

	/* agent first start or restart, both need a remap */
	if (user_agent) {
		buffer_map = agent_buffer_map(
			mailbox_virt_to_phys((uintptr_t)event_data->agent_buff_kernel), event_data->agent_buff_size);
		event_data->agent_buff_user = (void *)(uintptr_t)buffer_map;
	...
	void **buffer, bool user_agent)
{
	...
	if (find_flag == AGENT_NO_EXIST) {
		...
		ret = create_new_agent_node(dev_file, &event_data, &agent_pair, &agent_buff, size_align);
		if (ret != 0)
			return ret;
	}

	ret = get_agent_buffer(event_data, user_agent, buffer);
	if (ret != 0)
		goto release_rsrc;

	/* find_flag is not existed or ready to unregester, means it should be register to tee */
...
release_rsrc:
	if (find_flag != AGENT_NO_EXIST)
		...
		tz_free(event_data); /* here event_data can never be NULL */
	if (agent_buff)
		mailbox_free(agent_buff);
	return ret;
}
...
int tc_ns_unregister_agent(unsigned int agent_id, unsigned int nsid)
{
	...
	mb_pack->operation.paramtypes = TEE_PARAM_TYPE_VALUE_INPUT |
		(TEE_PARAM_TYPE_VALUE_INPUT << TEE_PARAM_NUM);
	mb_pack->operation.params[0].value.a =
		(uint32_t)mailbox_virt_to_phys((uintptr_t)event_data->agent_buff_kernel);
	mb_pack->operation.params[0].value.b =
		(uint32_t)(mailbox_virt_to_phys((uintptr_t)event_data->agent_buff_kernel) >> ADDR_TRANS_NUM);
	...
	/* user cannot use the user agent memory
	 * when the agent is succeed to unregistered or pending to unregistered
	 *
	 * case list:
	 * 1.unregistered: user mmap is freed, event_data is removed from global agent list
	 * 2.pending: user mmap is freed, event_data is still in global agent list.
	 *            waiting tee's notification to free event control
	 */
	ret = tc_ns_smc(&smc_cmd);
	if (ret == 0) {
		/* In general, no ca is invoking agent when unregistering agent successed! */
		unmap_agent_buffer(event_data);
		free_event_control(event_data);
		tlogi("unregister agent(0x%x, 0x%x) success, and agent buffer is not used in tee\n",
			agent_id, nsid);
	} else if (ret == (int)TEEC_ERROR_BUSY) {
		ret = -EBUSY;
		atomic_set(&(event_data->agent_ready), AGENT_PENDING);
		unmap_agent_buffer(event_data);
		/* wakeup CA when CA is invoking the agent */
		process_send_event_response(event_data);
...
#ifdef CONFIG_RPMB_AGENT_IO_PRIO
	if (agent_instance->agent_id == TEE_RPMB_AGENT_ID) {
		struct sched_param param;
		param.sched_priority = 1;
		sched_setscheduler(agent_instance->agent_thread, SCHED_FIFO, &param);
	}
#endif
...
int tc_ns_set_tavertion_ctrl(struct ta_ctrl_info *arg, unsigned int cmd_id)
{
	...
	uint32_t buf_len = arg->size;
	if (buf_len == 0 || buf_len > HASH_FILE_MAX_SIZE) {
		tloge("buffer size is invalid, size is %u", buf_len);
		return -EFAULT;
	}

	uint8_t *buf_to_tee = mailbox_alloc(buf_len, 0);
	if (!buf_to_tee) {
		tloge("failed to alloc memory!\n");
		return -ENOMEM;
	}

	if (memcpy_s(buf_to_tee, buf_len, arg->buffer, arg->size) != EOK) {
	...
	struct tc_ns_smc_cmd smc_cmd = { {0}, 0 };
	int ret = 0;
	mb_pack->operation.paramtypes = TEE_PARAM_TYPE_VALUE_INPUT | (TEE_PARAM_TYPE_VALUE_INPUT << TEE_PARAM_NUM);
	mb_pack->operation.params[0].value.a = mailbox_virt_to_phys((uintptr_t)buf_to_tee);
	mb_pack->operation.params[0].value.b = (uint64_t)mailbox_virt_to_phys((uintptr_t)buf_to_tee) >> ADDR_TRANS_NUM;
	mb_pack->operation.params[1].value.a = buf_len;
	mb_pack->operation.params[1].value.b = arg->file_type_index;
	...
	if (tc_ns_smc(&smc_cmd) != 0) {
		ret = -EPERM;
		tloge("set ta vertion ctrl file failed, ret_val=%d\n", smc_cmd.ret_val);
	}
	mailbox_free(buf_to_tee);
	mailbox_free(mb_pack);
	return ret;
