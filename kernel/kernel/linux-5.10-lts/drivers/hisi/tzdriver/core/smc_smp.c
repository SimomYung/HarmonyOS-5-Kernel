...
struct task_struct *shadow_work_func(struct kthread_work *work)
{
	...
	*target_arg = s_work->target;
	...
	tlogi("%s: create shadow thread %lu: for target 0x%" PRIx64 "\n",
		__func__, (g_shadow_thread_id - 1), *target_arg);
#if (CONFIG_CPU_AFF_NR != 0)
...
static long wait_event_internal_done(struct pending_entry *pe, struct timeout_step_t *step, uint64_t pending_args)
{
	if (!CURRENT_MM) {
		 ...
		 */
#ifdef CONFIG_LIBLINUX
		long ret = wait_event_freezable_timeout(pe->wq, atomic_read(&pe->run) || liblinux_pal_thread_freezing(), step->steps[step->cur]);
		int freeze_ret;
		trace_ldk_rvh_thread_try_to_freeze(&freeze_ret);
		return ret;
#else
