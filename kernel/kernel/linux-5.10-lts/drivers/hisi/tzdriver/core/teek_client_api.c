	...
	const struct teec_registeredmemory_reference *memref)
{
	if ((memref->offset + memref->size > memref->parent->size) ||
			(UINT32_MAX - memref->offset < memref->size))
		return true;
	...
	uint32_t connection_method)
{
	...
	if (operation->params[3].tmpref.size > MAX_PACKAGE_NAME_LEN - 1) {
		...
		uint8_t *user_id = (uint8_t *)dev_file->pkg_name + dev_file->pkg_name_len;
		*(uint32_t *)user_id = INVALID_USERID;
...
#if defined(CONFIG_LIBLINUX_CDC_RDA)
	cli_context.file_size = 0;
	file_buffer = NULL;
#else
	teec_ret = (uint32_t)teek_get_app((void *)context->ta_path, &file_buffer,
...
#define CA_AUTH_RETRY_SLEEP_TIME 10
	...
	const struct teec_operation *operation, uint32_t *return_origin)
{
	...
	for (i = 0; i < RETRY_TIMES; i++) {
		...
		if ((ret != (uint32_t)TEEC_CLIENT_INTR) && (ret != (uint32_t)TEE_ERROR_CA_AUTHFILE_NOT_EXIST))
			return ret;
		if (ret == (uint32_t)TEE_ERROR_CA_AUTHFILE_NOT_EXIST) {
			tlogi("teek open session retry for ca auth file not exist\n");
			msleep(CA_AUTH_RETRY_SLEEP_TIME);
		}
	}
	return ret;
}
