...
#define DSTB_MAX_MSG_SIZE 8192

struct dstb_global_msg {
	uint64_t devid_buffer;
	uint32_t devid_size;
	uint64_t buffer;
	uint32_t size;
};
...
#define TC_NS_CLIENT_IOCTL_DSTB_GLOBAL_MSG \
	_IOWR(TC_NS_CLIENT_IOC_MAGIC, 33, struct dstb_global_msg)
#define TC_NS_CLIENT_IOCTL_SET_TA_CONTROL_VERSION \
	_IOWR(TC_NS_CLIENT_IOC_MAGIC, 34, struct ta_ctrl_info)
