...
#define TC_NS_CONTAINER_DEV     "tc_ns_container"
...
/* for caller CA from container */
#define TEE_REQ_FROM_CONTAINER_USER_MODE   2U
...
/* Increased extended size for user ID */
#define CA_USERID_SIZE 4
...
struct tc_ns_shared_mem {
	void *kernel_addr;   /* used in tzdriver */
	void *user_addr;     /* return to user caller addr */
...
struct tc_ns_dev_file {
	 ...
	 * key or uid(for non A-system services/daemons)
...
#ifdef CONFIG_TA_GET_CA_USERID
	uint8_t pkg_name[MAX_PACKAGE_NAME_LEN + CA_USERID_SIZE];
#else
...
struct tc_ns_smc_cmd {
	...
	unsigned int agent_id;    /* for posix proxy: agent_id store teleport's parent pid */
