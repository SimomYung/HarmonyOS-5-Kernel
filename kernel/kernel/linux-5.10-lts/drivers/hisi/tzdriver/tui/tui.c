...
#define TUI_FOLD_NEED_ROTATE 257
...
#define FONT_READ_SIZE (4 * 1024 * 1024)
static int32_t copy_tui_font_file(size_t font_file_size, const void *font_virt_addr)
{
	...
	struct file *filep = filp_open(TTF_NORMAL_FILE_PATH, O_RDONLY, 0);
	...
	char *font_buff = vmalloc(FONT_READ_SIZE);
	if (ZERO_OR_NULL_PTR((unsigned long)(uintptr_t)font_buff)) {
		tloge("font memory alloc failed\n");
		ret = -1;
		goto clean;
	}

	while (count < font_file_size) {
		int rsize = vfs_read(filep, (char __user *)font_buff, FONT_READ_SIZE, &pos);
		tloge("read ttf file %d\n", rsize);
		if (rsize < 0) {
			ret = -1;
			break;
		} else if (rsize == 0) {
			tloge("read ttf file end %d\n", count);
			break;
		}

		if (memcpy_s((void *)font_virt_addr + count, FONT_READ_SIZE, (void *)font_buff, rsize) != EOK) {
			tloge("font memcpy_s failed\n");
			ret = -1;
			break;
		}
...
static int32_t send_ttf_mem(tui_ion_mem *tui_ttf_mem)
{
	...
	__dma_map_area(tui_ttf_mem->tui_ion_virt_addr, tui_ttf_mem->len, DMA_BIDIRECTIONAL);
	__dma_map_area(tui_ttf_mem->tui_ion_virt_addr, tui_ttf_mem->len, DMA_TO_DEVICE);
	...
	void *pdata)
{
	...
	if (ZERO_OR_NULL_PTR((unsigned long)(uintptr_t)tui_drv))
		return -ENOMEM;
	...
	if (strncmp(name, TUI_TP_NAME, (size_t)TUI_DRV_NAME_MAX) == 0) {
		tui_tp_tmp = kzalloc(128, GFP_KERNEL);
		memcpy_s(tui_tp_tmp, 128, pdata, 16);
		tui_drv->pdata = tui_tp_tmp;
	} else {
		tui_drv->pdata = pdata;
	}
...
static int add_tui_msg(int type, int val, void *data)
{
	...
	if (ZERO_OR_NULL_PTR((unsigned long)(uintptr_t)tui_msg))
		return -ENOMEM;
	...
	const struct teec_tui_parameter *tui_param)
{
	...
	if (tui_param->fold_state == TUI_FOLD_NEED_ROTATE) {
		mb_pack->operation.params[2].value.a = active_rect.h;
		mb_pack->operation.params[2].value.b = active_rect.w;
	} else {
		mb_pack->operation.params[2].value.a = tui_param->width;
		mb_pack->operation.params[2].value.b = tui_param->height;
	}
	struct dpu_secure_info panel_info = { 0 };
	if (dpu_tui_get_panel_info(&panel_info) != 0) {
		tloge("get panel info failed\n");
		return;
	}
	mb_pack->operation.params[3].value.a = make32(panel_info.panel_type, tui_param->fold_state);
...
{
	uint32_t uid = current_uid().val;
	if (check_uid_valid(uid) == false) {
		tloge("get invalid uid = %u\n", uid);
		return -1;
	}
...
{
	if (tc_ns_unregister_agent(TEE_TUI_AGENT_ID, PROC_PID_INIT_INO) != 0)
...
static int agent_process_work_tui(void)
{
	...
	event_data = find_event_control(TEE_TUI_AGENT_ID, PROC_PID_INIT_INO);
...
static int32_t do_tui_ttf_work(void)
{
	...
	case TUI_CMD_EXIT:
		if (atomic_read(&g_tui_state) != TUI_STATE_UNUSED &&
			...
			secure_display_tui_work_start(false);
#endif
		}
		break;
...
static void process_tui_enable(void)
{
	...
	tlogi("tui enable\n");
#ifdef CONFIG_TEE_INDICATOR
	secure_display_tui_work_start(true);
#endif
...
static int tui_kthread_work_fn(void *data)
{
	...
	while (1) {
		tc_ns_wait_event(TEE_TUI_AGENT_ID, PROC_PID_INIT_INO);
		...
		if (tc_ns_send_event_response(TEE_TUI_AGENT_ID, PROC_PID_INIT_INO) != 0)
