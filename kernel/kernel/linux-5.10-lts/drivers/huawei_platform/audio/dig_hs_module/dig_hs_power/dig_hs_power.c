...
#endif
#include "buck_charge_ic_manager.h"
...
#endif
	if (g_pdata->gpio_one_ctrl == DIG_HS_POWER_GPIO_ONE) {
		...
		dig_hs_power_gpio_set_value(g_pdata->gpio_low_power_ctrl,
			DIG_HS_POWER_GPIO_SET);
	} else if (g_pdata->gpio_one_ctrl == DIG_HS_POWER_GPIO_V800) {
		hwlog_info("%s: use v800", __func__);
		pmu_buckboost_switch(DIG_HS_VBST_LOWPOWER, INT_MAX);
		charge_set_otg_switch_mode_enable(1);
	} else {
...
int dig_hs_normal_power(void)
{
	...
	if (g_pdata->audio_buckboost_enable) {
		if (g_pdata->gpio_one_ctrl == DIG_HS_POWER_GPIO_ONE) {
			...
			hwlog_info("%s: use v800", __func__);
			pmu_buckboost_switch(g_pdata->vbst_voltage_default, INT_MAX);
			charge_set_otg_switch_mode_enable(0);
		} else {
