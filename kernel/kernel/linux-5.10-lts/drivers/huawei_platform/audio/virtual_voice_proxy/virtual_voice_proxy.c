...
static struct virtual_voice_proxy_priv proxy_priv;
	...
	struct virtual_voice_memory_pool *mem_pool)
{
	...
	if ((free_node_count == (int)(mem_pool->block_num * VIR_MEM_BLOCK_NUM)) &&
		(free_node_count > 0)) {
		spin_unlock_bh(&mem_pool->mem_lock);
		return true;
	}
...
{
	spin_lock_bh(&proxy_priv.cnf_queue_lock);
	if (!list_empty_careful(&virtual_proxy_confirm_queue)) {
		spin_unlock_bh(&proxy_priv.cnf_queue_lock);
		return;
	}
	if (virtual_voice_memory_pool_free_node_full_idle(
		&proxy_priv.confirm_pool)) {
		logi("confirm mem pool prepare clear\n");
		virtual_voice_memory_pool_free(&proxy_priv.confirm_pool);
	}
	spin_unlock_bh(&proxy_priv.cnf_queue_lock);

	spin_lock_bh(&proxy_priv.command_lock);
	if (!list_empty_careful(&virtual_proxy_command_queue)) {
		spin_unlock_bh(&proxy_priv.command_lock);
		return;
	}
	if (virtual_voice_memory_pool_free_node_full_idle(
		&proxy_priv.command_pool)) {
		logi("cmd mem pool prepare clear\n");
		virtual_voice_memory_pool_free(&proxy_priv.command_pool);
	}
	spin_unlock_bh(&proxy_priv.command_lock);

	spin_lock_bh(&proxy_priv.mail_lock);
	if (!list_empty_careful(&virtual_proxy_mail_msg_queue)) {
		spin_unlock_bh(&proxy_priv.mail_lock);
		return;
	}
	if (virtual_voice_memory_pool_free_node_full_idle(
		&proxy_priv.msg_pool)) {
		logi("msg mem pool prepare clear\n");
		virtual_voice_memory_pool_free(&proxy_priv.msg_pool);
	}
	spin_unlock_bh(&proxy_priv.mail_lock);
}
...
int64_t virtual_voice_proxy_get_time_ms(void)
{
	...
	timems = time.tv_sec * MSEC_PER_SEC + time.tv_nsec / NSEC_PER_MSEC;
...
int32_t virtual_voice_proxy_add_cmd(uint16_t msg_id)
{
	...
	proxy_priv.command_wait_flag++;
	spin_unlock_bh(&proxy_priv.command_lock);
	wake_up(&proxy_priv.command_waitq);
	return 0;
}
	...
	int8_t *data, uint32_t size, uint16_t msg_id)
{
	...
	if (ret < 0) {
		virtual_voice_cmd_node_free(&proxy_priv.command_pool,
			(struct virtual_voice_proxy_cmd_node **)(&command));
		command = NULL;
		spin_unlock_bh(&proxy_priv.command_lock);
		return ret;
...
static int32_t send_mailbox_cnf_msg(uint16_t msg_id)
{
	...
	if (proxy_priv.send_mailbox_msg != NULL) {
		/* call the mailbox to send the message to hifi */
		ret = proxy_priv.send_mailbox_msg(
...
static void vir_voc_send_mailbox_cnf_work_queue(struct work_struct *work)
{
	...
	while (!list_empty_careful(&virtual_proxy_confirm_queue)) {
		...
		spin_lock_bh(&proxy_priv.cnf_queue_lock);
		virtual_voice_cmd_node_free(&proxy_priv.confirm_pool,
			(struct virtual_voice_proxy_cmd_node **)(&command));
		command = NULL;
	}
	spin_unlock_bh(&proxy_priv.cnf_queue_lock);
...
int32_t virtual_voice_proxy_add_work_queue_cmd(uint16_t msg_id)
{
	...
	if (!queue_work(proxy_priv.send_mailbox_cnf_wq,
		&proxy_priv.send_mailbox_cnf_work))
		...
		struct mb_queue *mail_handle, uint32_t mail_len)
{
	...
	if (proxy_priv.read_mailbox_msg != NULL) {
		ret_mail = proxy_priv.read_mailbox_msg(mail_handle,
	...
	proxy_priv.mail_wait_flag++;
	spin_unlock_bh(&proxy_priv.mail_lock);
	wake_up(&proxy_priv.mail_waitq);
}
...
static int32_t virtual_voice_proxy_mailbox_cb_init(void)
{
	...
	proxy_priv.send_mailbox_msg = virtual_voice_proxy_mailbox_send_msg_cb;
	proxy_priv.read_mailbox_msg =
...
{
	proxy_priv.send_mailbox_msg = NULL;
	proxy_priv.read_mailbox_msg = NULL;
...
static int virtual_voice_proxy_write_thread(void *arg)
{
	...
	while (!kthread_should_stop()) {
		ret = wait_event_interruptible(proxy_priv.command_waitq,
			proxy_priv.command_wait_flag != 0);
		if (ret) {
...
static int virtual_voice_proxy_handle_mail_thread(void *arg)
{
	...
	UNUSED_PARAMETER(arg);
	while (!kthread_should_stop()) {
		ret = wait_event_interruptible(proxy_priv.mail_waitq,
			proxy_priv.mail_wait_flag != 0);
		if (ret) {
			loge("wait event failed, 0x%x.\n", ret);
			continue;
		}
		...
		while (!list_empty_careful(&virtual_proxy_mail_msg_queue)) {
			...
			virtual_voice_cmd_node_free(&proxy_priv.msg_pool,
...
{
	proxy_priv.write_thread = kthread_run(virtual_voice_proxy_write_thread,
			NULL, "virtual_voice proxy write");

	if (IS_ERR_OR_NULL(proxy_priv.write_thread)) {
		loge("write_thread call kthread_run fail\n");
		return -EBUSY;
	}

	proxy_priv.mail_thread = kthread_run(
		virtual_voice_proxy_handle_mail_thread,
		NULL, "virtual_voice handle mail thread");

	if (IS_ERR_OR_NULL(proxy_priv.mail_thread)) {
...
{
	if (!IS_ERR_OR_NULL(proxy_priv.write_thread)) {
		kthread_stop(proxy_priv.write_thread);
		spin_lock_bh(&proxy_priv.command_lock);
		proxy_priv.command_wait_flag++;
		spin_unlock_bh(&proxy_priv.command_lock);
		wake_up(&proxy_priv.command_waitq);
		proxy_priv.write_thread = NULL;
	}

	if (!IS_ERR_OR_NULL(proxy_priv.mail_thread)) {
		kthread_stop(proxy_priv.mail_thread);
		spin_lock_bh(&proxy_priv.mail_lock);
		proxy_priv.mail_wait_flag++;
		spin_unlock_bh(&proxy_priv.mail_lock);
		wake_up(&proxy_priv.mail_waitq);
		proxy_priv.mail_thread = NULL;
	}
}
...
static int virtual_voice_proxy_probe(struct platform_device *pdev)
{
	...
	memset(&proxy_priv, 0, sizeof(proxy_priv));
	proxy_priv.dev = &pdev->dev;
	proxy_priv.command_wait_flag = 0;
	virtual_voice_memory_pool_init(&proxy_priv.command_pool,
		sizeof(struct virtual_voice_proxy_cmd_node));
	virtual_voice_memory_pool_init(&proxy_priv.confirm_pool,
		sizeof(struct virtual_voice_proxy_cmd_node));
	virtual_voice_memory_pool_init(&proxy_priv.msg_pool,
		sizeof(struct virtual_voice_proxy_cmd_node));
	spin_lock_init(&proxy_priv.command_lock);
	spin_lock_init(&proxy_priv.mail_lock);
	spin_lock_init(&proxy_priv.cnf_queue_lock);
	init_waitqueue_head(&proxy_priv.command_waitq);
	init_waitqueue_head(&proxy_priv.mail_waitq);

	proxy_priv.send_mailbox_cnf_wq =
		create_singlethread_workqueue("vir_voice_send_mailbox_cnf_wq");
	if (proxy_priv.send_mailbox_cnf_wq == NULL) {
		loge("workqueue create failed\n");
		return -EFAULT;
	}

	INIT_WORK(&proxy_priv.send_mailbox_cnf_work,
...
mailbox_cb_init_err:
	if (proxy_priv.send_mailbox_cnf_wq != NULL) {
		flush_workqueue(proxy_priv.send_mailbox_cnf_wq);
		destroy_workqueue(proxy_priv.send_mailbox_cnf_wq);
		proxy_priv.send_mailbox_cnf_wq = NULL;
	}
	return (int)ret;
}
