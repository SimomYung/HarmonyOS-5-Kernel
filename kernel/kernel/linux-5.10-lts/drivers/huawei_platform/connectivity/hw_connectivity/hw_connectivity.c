...
#else
static const struct proc_ops hwconn_proc_fops_chiptype = {
...
#else
static const struct proc_ops hwconn_proc_fops_nfc_chiptype = {
	.proc_read = hwconn_read_proc_nfc_chiptype,
};
#endif
...
static int read_chip_type_from_dts(char *buf, int buf_len)
{
	...
	if (np == NULL) {
		...
		if (strncpy_s(buf, buf_len, CHIP_TYPE_BCM, buf_len - 1) != EOK) {
			printk(KERN_ERR "[HW_CONN] %s strncpy_s error_1\n", __func__);
			return -EINVAL;
		}
		return 0;
	}
	...
	if (ret < 0 || chiptype == NULL) {
		...
		printk(KERN_ERR "[HW_CONN] %s: get chiptype failed, ret:%d, using default\n",
			__func__, ret);
		if (strncpy_s(buf, buf_len, CHIP_TYPE_BCM, buf_len - 1) != EOK) {
			printk(KERN_ERR "[HW_CONN] %s strncpy_s error_2\n", __func__);
			return -EINVAL;
		}
		ret = 0;
	} else {
		printk(KERN_ERR "[HW_CONN] %s:  chip_type :%s\n", __func__,
			chiptype);
		if (strncpy_s(buf, buf_len, chiptype, buf_len - 1) != EOK) {
			printk(KERN_ERR "[HW_CONN] %s strncpy_s error_3\n", __func__);
			return -EINVAL;
		}
	}
	return ret;
...
static int read_nfc_chip_type_from_dts(char *buf, int buf_len)
{
	...
	if (np == NULL) {
		...
		if (strncpy_s(buf, buf_len, NFC_CHIP_TYPE_NXP, buf_len - 1) != EOK) {
			printk(KERN_ERR "[HW_CONN] %s: strncpy_s error_1\n",__func__);
			return -EINVAL;
		}
		return 0;
...
static int hw_connectivity_probe(struct platform_device *pdev)
{
	...
	if (memset_s((void *)g_chip_type, sizeof(g_chip_type), 0, sizeof(g_chip_type)) != EOK ||
		memset_s((void *)g_nfc_chip_type, sizeof(g_nfc_chip_type), 0, sizeof(g_nfc_chip_type)) != EOK) {
		printk(KERN_ERR "[HW_CONN] %s: memset_s error\n", __func__);
		goto err_free;
	}
...
err_free:
	(void)memset_s((void *)g_chip_type, sizeof(g_chip_type), 0, sizeof(g_chip_type));
	(void)memset_s((void *)g_nfc_chip_type, sizeof(g_chip_type), 0, sizeof(g_nfc_chip_type));
	return ret;
}
...
bool isMyConnectivityChip(const char *name)
{
	...
	if (memset_s((void *)tmp, sizeof(tmp), 0, sizeof(tmp)) != EOK) {
		printk(KERN_ERR "[HW_CONN] %s: memset_s error\n",__func__);
		return false;
	}
	if (strlen(g_chip_type) > 0) {
		printk(KERN_INFO "[HW_CONN] g_chip_type is %s\n", g_chip_type);
		if (strncpy_s(tmp, CHIP_TYPE_LEN, g_chip_type, CHIP_TYPE_LEN - 1) != EOK) {
			printk(KERN_ERR "[HW_CONN] %s: strncpy_s error\n",__func__);
			return false;
		}
	} else {
...
bool isMyNfcChip(const char *name)
{
	...
	if (memset_s((void *)tmp, sizeof(tmp), 0, sizeof(tmp)) != EOK) {
		printk(KERN_ERR "[HW_CONN]  memset_s error in %s\n", __func__);
		return false;
	}
	if (strlen(g_nfc_chip_type) > 0) {
		printk(KERN_INFO "[HW_CONN] g_nfc_chip_type is %s\n",
			g_nfc_chip_type);
		if (strncpy_s(tmp, CHIP_TYPE_LEN, g_nfc_chip_type, CHIP_TYPE_LEN - 1) != EOK) {
			printk(KERN_ERR "[HW_CONN]  strncpy_s error in %s\n", __func__);
			return false;
		}
	} else {
