...
enum dec_detect_station {
	...
	CTG_DETECT = 16, /* mmi2 normal detect */
};
...
static char ctg_detect_flag[DEV_DETECT_NUMS + 1] = DEV_DETECT_DEFAULT;
/*
...
static ssize_t ctg_detect_flag_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	hwlog_info("%s: ctg_detect_flag=%s\n", __func__, ctg_detect_flag);
	return snprintf_s((char *)buf, PRINT_MAX_LEN, PRINT_MAX_LEN - 1,
		"%s\n", ctg_detect_flag);
}
...
static struct detect_flag_device dev_list[] = {
	{
		...
		.name = "dev_detect_ctg_flag",
		.index = 0,
		.attr = __ATTR(dev_detect_ctg_flag, 0444,
			ctg_detect_flag_show, NULL)
	},
};
...
static int create_dev_detect_sysfs(struct class *myclass)
{
	...
	for (dev_index = 0; dev_index < (size_t)ARRAY_SIZE(dev_list); dev_index++) {
...
static void get_dev_detect_dts_config(const struct device_node *np)
{
	...
	hwlog_info("%s: function begin!\n", __func__);
	for (i = 0; i < (size_t)ARRAY_SIZE(dev_detect_table); i++) {
		...
		if (!result) {
			...
			mmi2_detect_flag[i] =
				...
				? FLAG_SET : FLAG_NOT_SET;
			ctg_detect_flag[i] =
				((type != NO_DETECT) && (type & CTG_DETECT))
				? FLAG_SET : FLAG_NOT_SET;
		} else {
			...
			ctg_detect_flag[i] = FLAG_NOT_SET;
