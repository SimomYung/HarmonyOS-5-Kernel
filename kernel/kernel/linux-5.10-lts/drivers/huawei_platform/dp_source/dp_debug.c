...
int dp_debug_append_info(char *buf, int size, char *fmt, ...)
{
	...
	ret = strncat_s(buf, size, append, size - strlen(buf) - 1);
	if (ret != EOK)
		hwlog_err("%s: strncat_s is error\n", __func__);
...
static int dp_debug_vs_pe_force_get(char *buffer, const struct kernel_param *kp)
{
	...
	if (priv->vs_pe_state == DP_DEBUG_PARAMS_INVALID) {
		...
		return snprintf_s(buffer, (unsigned long)DP_DEBUG_BUF_SIZE,
				(unsigned long)DP_DEBUG_BUF_SIZE - 1, "invalid vs_force or pe_force\n");
	}

	if ((priv->vs_force == DP_DSM_VS_PE_MASK) ||
	   (priv->pe_force == DP_DSM_VS_PE_MASK)) {
		return snprintf_s(buffer, (unsigned long)DP_DEBUG_BUF_SIZE,
				(unsigned long)DP_DEBUG_BUF_SIZE - 1, "vs_force and pe_force not set\n");
	}

	return snprintf_s(buffer, (unsigned long)DP_DEBUG_BUF_SIZE,
			(unsigned long)DP_DEBUG_BUF_SIZE - 1, "vs_force: %u, pe_force: %u\n", priv->vs_force,
			priv->pe_force);
}
				    ...
				    const struct kernel_param *kp)
{
	...
	ret = sscanf_s(val, "0x%x 0x%x", &vs_force, &pe_force);
	if (ret != 2) {
					 ...
					 const struct kernel_param *kp)
{
	...
	if (priv->lanes_rate_state == DP_DEBUG_PARAMS_INVALID) {
		...
		return snprintf_s(buffer, (unsigned long)DP_DEBUG_BUF_SIZE,
				(unsigned long)DP_DEBUG_BUF_SIZE - 1, "invalid lanes_force or rate_force\n");
	}

	if ((priv->lanes_force == DP_LINK_LANES_MASK) ||
	   (priv->rate_force == DP_LINK_RATE_MASK)) {
		return snprintf_s(buffer, (unsigned long)DP_DEBUG_BUF_SIZE,
				(unsigned long)DP_DEBUG_BUF_SIZE - 1, "lanes_force and rate_force not set\n");
	}

	return snprintf_s(buffer, (unsigned long)DP_DEBUG_BUF_SIZE,
			(unsigned long)DP_DEBUG_BUF_SIZE - 1, "lanes_force: %u, rate_force: %u\n", priv->lanes_force,
			priv->rate_force);
}
					 ...
					 const struct kernel_param *kp)
{
	...
	ret = sscanf_s(val, "0x%x 0x%x", &lanes, &rate);
	if (ret != 2) {
		hwlog_err("%s: invalid params num %d\n", __func__, ret);
		return -EINVAL;
	}
					 ...
					 const struct kernel_param *kp)
{
	...
	if (priv->resolution_state == DP_DEBUG_PARAMS_INVALID) {
		...
		return snprintf_s(buffer, (unsigned long)DP_DEBUG_BUF_SIZE,
				(unsigned long)DP_DEBUG_BUF_SIZE - 1, "invalid user_mode or user_format\n");
	}

	if (priv->user_mode == DP_VIDEO_MODE_MASK)
		return snprintf_s(buffer, (unsigned long)DP_DEBUG_BUF_SIZE,
				(unsigned long)DP_DEBUG_BUF_SIZE - 1, "user_mode and user_format not set\n");

	return snprintf_s(buffer, (unsigned long)DP_DEBUG_BUF_SIZE,
			(unsigned long)DP_DEBUG_BUF_SIZE - 1, "user_mode: %u, user_format: %u\n", priv->user_mode,
			priv->user_format);
}
					 ...
					 const struct kernel_param *kp)
{
	...
	ret = sscanf_s(val, "0x%x 0x%x", &user_mode, &user_format);
	if (ret != 2) {
		hwlog_err("%s: invalid params num %d\n", __func__, ret);
		return -EINVAL;
	}
					  ...
					  const struct kernel_param *kp)
{
	...
	ret = sscanf_s(val, "%d 0x%x", &index, &pv);
	if (ret != 2) {
				     ...
				     const struct kernel_param *kp)
{
	...
	return snprintf_s(buffer, (unsigned long)DP_DEBUG_BUF_SIZE,
			(unsigned long)DP_DEBUG_BUF_SIZE - 1, "factory_mode %s\n",
				     ...
				     const struct kernel_param *kp)
{
	...
	ret = sscanf_s(val, "%d", &test_enable);
	if (ret != 1) {
		hwlog_err("%s: invalid params num %d\n", __func__, ret);
		return -EINVAL;
	}
