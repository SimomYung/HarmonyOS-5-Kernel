...
int dp_factory_get_test_result(char *buffer, int size)
{
	if ((buffer != NULL) && (size > 0))
		return snprintf_s(buffer, size, size - 1, "not support factory test\n");
...
int dp_factory_get_test_result(char *buffer, int size)
{
	...
	if (priv->link_status == DP_FACTORY_LINK_CR_FAILED) {
		return snprintf_s(buffer, (unsigned long)size,
			      (unsigned long)size - 1, "test failed: 0x%x %s\nlink cr failed!\n",
	...
	if ((priv->hotplug_retval == 0)) {
		if ((priv->link_rate == 0) && (priv->link_lanes == 0))
			return snprintf_s(buffer, size, size- 1, "Not yet tested\n");
	}

	return snprintf_s(buffer, (unsigned long)size, (unsigned long)size - 1,
	...
	if (event != NULL) {
		...
		if ((event_no > DP_LINK_STATE_CABLE_OUT) && (len > offset)) {
			if (strlen(priv->link_event) == 0) {
				rc = strcpy_s(priv->link_event, sizeof(priv->link_event), event + offset);
			} else {
				rc = strcat_s(priv->link_event, sizeof(priv->link_event), ",");
				rc |= strcat_s(priv->link_event, sizeof(priv->link_event), event + offset);
			}
		}
	} else if (cablein) {
		priv->hotplug_retval = 0;
		...
		memset_s(priv->link_event, DP_LINK_EVENT_BUF_MAX * DP_LINK_STATE_MAX, 0,
