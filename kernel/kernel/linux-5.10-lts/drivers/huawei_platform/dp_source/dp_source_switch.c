...
#define PRODUCT_ID_DAQIANF_0      0x0F /* EDID Byte10 */
#define PRODUCT_ID_DAQIANF_1      0x4E /* EDID Byte11 */
...
#ifdef CONFIG_SUPPORT_MULTI_DP_DISPLAY
#define MAX_DISPLAY_PORTS         6
struct dp_info {
	uint8_t edid[EDID_BLOCK_LENGTH];
};
static struct dp_info g_dp_info[MAX_DISPLAY_PORTS];
#endif

static int save_dp_edid_intf(uint8_t *edid_buf, uint32_t buf_len, uint8_t port_id);

#ifdef CONFIG_DKMD_DPU_DP
static struct dp_hook_ops dpu_dp_hook_ops = {
	.save_dp_edid = save_dp_edid_intf,
...
int save_dp_edid(uint8_t *edid_buf, uint32_t buf_len)
{
	...
	if (!edid_buf || (buf_len != EDID_BLOCK_LENGTH)) {
		...
		memset_s(g_dp_priv.edid, EDID_BLOCK_LENGTH, 0, EDID_BLOCK_LENGTH);
		return -EINVAL;
	}
	rc = memcpy_s(g_dp_priv.edid, EDID_BLOCK_LENGTH, edid_buf, EDID_BLOCK_LENGTH);
		hwlog_err("%s: memcpy_s fail = %d\n", __func__, rc);
...
#ifdef CONFIG_SUPPORT_MULTI_DP_DISPLAY
static int save_multi_dp_edid(uint8_t *edid_buf, uint32_t buf_len,
	uint8_t port_id)
{
	int ret;

	if (!edid_buf || (buf_len != EDID_BLOCK_LENGTH) ||
		(port_id > MAX_DISPLAY_PORTS - 1)) {
		hwlog_err("%s: edid_buf error\n", __func__);
		return -EINVAL;
	}

	hwlog_info("%s: port_id = %d\n", __func__, port_id);
	ret = memcpy_s(g_dp_info[port_id].edid, EDID_BLOCK_LENGTH,
		edid_buf, EDID_BLOCK_LENGTH);
	if (ret) {
		hwlog_err("%s: memcpy_s error\n", __func__);
		return -EINVAL;
	}

	return save_dp_edid(edid_buf, buf_len);
}
#endif

static int save_dp_edid_intf(uint8_t *edid_buf,
	uint32_t buf_len, uint8_t port_id)
{
#ifdef CONFIG_SUPPORT_MULTI_DP_DISPLAY
	return save_multi_dp_edid(edid_buf, buf_len, port_id);
#else
	UNUSED(port_id);
	return save_dp_edid(edid_buf, buf_len);
#endif
...
#ifdef CONFIG_SUPPORT_MULTI_DP_DISPLAY
static enum huawei_vision_type get_smart_display_type_by_portid(uint8_t port_id)
{
	if (port_id > MAX_DISPLAY_PORTS - 1)
		return VISION_TYPE_UNKNOWN;

	if ((g_dp_info[port_id].edid[EDID_BYTE_10] == PRODUCT_ID_HUASHAN_0 &&
		g_dp_info[port_id].edid[EDID_BYTE_11] == PRODUCT_ID_HUASHAN_1) ||
		(g_dp_info[port_id].edid[EDID_BYTE_10] == PRODUCT_ID_DAQIANF_0 &&
		g_dp_info[port_id].edid[EDID_BYTE_11] == PRODUCT_ID_DAQIANF_1))
...
static enum huawei_vision_type get_smart_display_type()
{
	...
	if (g_dp_priv.edid[EDID_BYTE_10] == PRODUCT_ID_DAQIANF_0 &&
		g_dp_priv.edid[EDID_BYTE_11] == PRODUCT_ID_DAQIANF_1)
		return VISION_TYPE_DAQIANF;

	return VISION_TYPE_UNKNOWN;
}

bool is_valid_timing_info(uint16_t hactive_pixels, uint16_t vactive_pixels,
	uint32_t fps, uint8_t port_id)
{
	(void)fps;
#ifdef CONFIG_SUPPORT_MULTI_DP_DISPLAY
	if (hactive_pixels * VACTIVE_PIXELS_RATIO != vactive_pixels * HACTIVE_PIXELS_RATIO &&
		get_smart_display_type_by_portid(port_id) == VISION_TYPE_HUASHAN) {
		hwlog_info("invalid_timing -- xres: %u, yres: %u, port_id: %u\n",
			hactive_pixels, vactive_pixels, port_id);
		return false;
	}

	hwlog_info("valid_timing -- xres: %u, yres: %u, port_id: %u\n",
		hactive_pixels, vactive_pixels, port_id);
	return true;
#else
	UNUSED(port_id);
	if (hactive_pixels * VACTIVE_PIXELS_RATIO != vactive_pixels * HACTIVE_PIXELS_RATIO &&
		(get_smart_display_type() == VISION_TYPE_HUASHAN || get_smart_display_type() == VISION_TYPE_DAQIANF)) {
...
void dp_send_event(enum dp_event_type event)
{
	...
	case DP_LINK_STATE_GOOD:
		snprintf_s(event_buf, sizeof(event_buf), sizeof(event_buf) - 1, "DP_LINK_STATE=GOOD");
		break;
	case DP_LINK_STATE_BAD:
		snprintf_s(event_buf, sizeof(event_buf), sizeof(event_buf) - 1, "DP_LINK_STATE=BAD");
		break;
	default:
		return;
	}
...
void dp_link_state_event(enum dp_link_state state, bool dptx_vr_flag)
{
	...
	for (i = 0; i < count; i++) {
		if ((g_dp_link_event[i].state == state) &&
			...
			(void)strncpy_s(event_buf, DP_LINK_EVENT_BUF_MAX, g_dp_link_event[i].event, size);
...
static int __init dp_source_mode_init(void)
{
	...
	hwlog_info("%s: enter\n", __func__);
	memset_s(&g_dp_priv, sizeof(g_dp_priv), 0, sizeof(g_dp_priv));
