...
#define RENESAS_VENDOR 0x1912
#define RENESAS_DEVICE 0x0015
...
typedef struct external_camera_data {
	...
	int flash_invalid;
	int isp_power_lock_valid;
	struct wakeup_source *delay_poweroff_wl;
	struct wakeup_source *isp_power_warklock;
	struct pcie_kport_register_event pcie_kport_event;
...
static bool match_power_pin_res_info(power_pin_t *pin, dev_power_res_t* pwr_res)
{
	...
	if ((pin == NULL) || (pwr_res == NULL)) {
		cam_err("power pin or pwr_res is NULL");
		return false;
	}
...
static int power_off(dev_power_seq_t *pwr_seq, external_device_type_t device_type)
{
	...
	if (!pwr_seq)
		return -EINVAL;
	cam_info("pwr_seq->num = %d device_type = %d", pwr_seq->num, device_type);
...
static int sensor_dt_data_init(struct platform_device *pdev)
{
	...
	for_each_child_of_node(sensor_node, node_temp) {
		i = SENSOR_NUM_MAX;
		ret = of_property_read_u32(node_temp, "isp_fw_sensor_idx", &i);
		if (ret < 0 || i >= SENSOR_NUM_MAX) {
			cam_err("get isp_fw_sensor_idx fail:%d, idx:%u", ret, i);
			return ret;
		}
		...
		ret = memcpy_s(g_all_sensor.info[i].sensor_name, SENSOR_NAME_LEN_MAX, name, strlen(name));
		if (ret != EOK)
			return ret;
...
static int isp_power_lock_data_init(struct platform_device *pdev)
{
	int ret;
	...
	if (!g_camera) {
		cam_err("g_camera is NULL");
		return -EINVAL;
	}

	g_camera->isp_power_lock_valid = 0;
	ret = of_property_read_u32(g_camera->dev->of_node, "vendor,isp_power_lock_valid",
		&g_camera->isp_power_lock_valid);
	if (ret < 0) {
		cam_err("get isp_power_lock_valid fail");
		return ret;
	}
...
static int delay_poweroff_wl_init(struct platform_device *pdev)
{
	...
	if (g_camera->isp_power_lock_valid == 1) {
		g_camera->isp_power_warklock = wakeup_source_register(&pdev->dev, "isp_power_warklock");
		if (!g_camera->isp_power_warklock) {
			return -EINVAL;
		}
	}
#else
	...
	power_status_t cfg_pwr_status, external_device_type_t device_type)
{
	...
	if (ret == 0) {
		if (device_type == EXTERNAL_DEVICE_TYPE_ISP && g_camera->isp_power_lock_valid == 1) {
			if ((*cur_pwr_status) == POWER_STATUS_ON) {
				cam_info("isp is power on and locked");
				__pm_stay_awake(g_camera->isp_power_warklock);
			} else if ((*cur_pwr_status) == POWER_STATUS_OFF) {
				cam_info("isp is power off and release lock");
				__pm_relax(g_camera->isp_power_warklock);
			}
		}
	}
...
static void external_camera_pcie_linkdown_callback(struct pcie_kport_notify *notify)
{
	external_camera_dsm_client_notify(DSM_FLASH_OPEN_SHOTR_ERROR_NO,
		"external camera pcie linkdown irq\n");
}

static void external_camera_pcie_register_linkdown_event(void)
{
	struct pci_dev *pdev = NULL;

	pdev = pci_get_device(RENESAS_VENDOR, RENESAS_DEVICE, NULL);
	if ((!g_camera) || (!pdev)) {
		return;
	}
	g_camera->pcie_kport_event.events = PCIE_KPORT_EVENT_LINKDOWN;
	g_camera->pcie_kport_event.user = pdev;
	g_camera->pcie_kport_event.mode = PCIE_KPORT_TRIGGER_CALLBACK;
	g_camera->pcie_kport_event.callback = external_camera_pcie_linkdown_callback;
	pcie_kport_register_event(&g_camera->pcie_kport_event);
}

static void external_camera_pcie_deregister_event(void)
{
	if((!g_camera) || (!g_camera->pcie_kport_event.user)) {
		return;
	}
    pcie_kport_deregister_event(&g_camera->pcie_kport_event);
}
...
static int cfg_ioctl_handler(unsigned long arg)
{
	...
	case PCIE2USB_ENUMERATE_DEVICE:
		...
		if (EOK == ret) {
			external_camera_pcie_register_linkdown_event();
		}
		break;
	case SENSOR_POWER_UP:
	case SENSOR_POWER_DOWN:
...
static int external_cam_dev_open(struct inode *inode, struct file *filp)
{
	...
	cam_info("open cnt[%d]", g_external_camera_ref);
...
static int external_cam_dev_release(struct inode *inode, struct file *filp)
{
	...
	mutex_lock(&g_mutex_ref);
	g_external_camera_ref--;
	cam_info("release cnt[%d]", g_external_camera_ref);
	...
	for (int i = 0; i < g_camera->sensor_data.num_of_sensor; i++) {
		if (g_camera->sensor_data.power_status[i] == POWER_STATUS_ON) {
			if (power_off(&g_camera->sensor_data.poweroff_seq[i], EXTERNAL_DEVICE_TYPE_SENSOR)) {
				cam_err("sensor power_off fail");
			} else {
				g_camera->sensor_data.power_status[i] = POWER_STATUS_OFF;
			}
...
static int external_camera_get_dts_init(struct platform_device *pdev)
{
	...
	ret = isp_power_lock_data_init(pdev);
	if (ret < 0) {
		cam_err("isp power lock dt data init fail");
	}
...
bool is_renesas_controller(struct usb_device *udev)
{
    struct device_node* extcam_node = NULL;
    struct pci_dev *pdev = NULL;
    uint32_t hiusb_donot_notify = 0;

    cam_info("Enter");
    extcam_node = of_find_node_by_name(NULL, "huawei_external_camera");
    if (extcam_node != NULL) {
        of_property_read_u32(extcam_node, "vendor,hiusb_donot_notify", &hiusb_donot_notify);
    }
    if (hiusb_donot_notify && udev != NULL && udev->bus != NULL && udev->bus->controller != NULL &&
        udev->bus->controller->bus != NULL && udev->bus->controller->bus->name != NULL) {
        if (!strcmp(udev->bus->controller->bus->name, pci_bus_type.name)) {
            pdev = to_pci_dev(udev->bus->controller);
            if (pdev != NULL && pdev->vendor == RENESAS_VENDOR && pdev->device == RENESAS_DEVICE) {
                return true;
            }
        }
    }
...
static int external_camera_probe(struct platform_device *pdev)
{
	...
	external_camera_register_dsm_client();
...
static int external_camera_remove(struct platform_device *pdev)
{
	...
	external_camera_pcie_deregister_event();

	wakeup_source_unregister(g_camera->delay_poweroff_wl);
	g_camera->delay_poweroff_wl = NULL;

	if (g_camera->isp_power_lock_valid == 1) {
		wakeup_source_unregister(g_camera->isp_power_warklock);
		g_camera->isp_power_warklock = NULL;
	}
	...
	external_camera_unregister_dsm_client();
...
static int external_camera_suspend(struct device *dev)
{
	...
	if (g_camera->isp_data.power_status == POWER_STATUS_ON) {
		cfg_pcie2usb_and_isp(&g_camera->isp_data.poweroff_seq, &g_camera->isp_data.power_status,
			POWER_STATUS_OFF, EXTERNAL_DEVICE_TYPE_ISP);
		external_camera_dsm_client_notify(DSM_FLASH_OPEN_SHOTR_ERROR_NO,
			"go to suspend, but userspace not close camera\n");
	}
