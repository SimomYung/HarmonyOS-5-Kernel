...
static int dump_acr_to_buf(struct bat_acr_info *acr, char *buf, int buf_size)
{
	...
	ret = snprintf(buf, (size_t)buf_size,
...
static int dump_eis_eacr_to_buf(struct eis_eacr_info *eacr, char *buf, int buf_size)
{
	int ret;

	if (!eacr || !buf || buf_size < 1) {
		hwlog_err("invalid eis_eacr_info or buf\n");
		return -EINVAL;
	}

	ret = snprintf_s(buf, buf_size, buf_size - 1,
		"batt_index: %u,acr: %d,cycle: %d,bat_temp: %d,chip_temp0: %d,chip_temp1: %d,vol: %d,",
		eacr->batt_index, eacr->batt_acr,
		eacr->batt_cycle, eacr->batt_temp,
		eacr->chip_temp[0], eacr->chip_temp[1],
		eacr->batt_vol);
	if (ret <= 0)
		hwlog_err("dump eis eacr info failed\n");

	hwlog_info("eis_eacr_info: %s\n", buf);
	return ret;
}
...
static int str_int(const char *name, int value, char *out, int out_size)
{
	int ret;
	int len;

	if (!name || !out || out_size < 1)
		return -EINVAL;

	ret = snprintf_s(out, out_size, out_size - 1, "%s: ", name);
	if (ret <= 0)
		return -EPERM;
	...
	ret = snprintf_s(out + len, out_size, out_size - len - 1, "%d ", value);
	if (ret <= 0)
		return -EPERM;
	len += ret;
	out[len - 1] = '\0';
...
static int dump_eis_freq_to_buf(struct eis_freq_array *eis_freq_a, char *buf, int buf_size)
{
	...
	len = str_int("batt_index", eis_freq_a->batt_index, 
...
static void icm_eis_uevent(struct icm_device_info *di)
{
	int ret;
	char *event_buf = NULL;
...
static void icm_eacr_uevent(struct icm_device_info *di)
{
	int ret;
	char *event_buf = NULL;
	...
	hwlog_info("eis_evt_to_send: %d\n", di->eis_evt_to_send);
	ret = dump_eis_eacr_to_buf(&di->eis_eacr_info, event_buf, BSOH_EVENT_NOTIFY_SIZE);
	if (ret > 0)
		bsoh_uevent_rcv(BSOH_EVT_SOH_ACR_UPDATE, event_buf);

	di->soh_evt_to_send &= ~BIT(EVT_EIS_EACR_UPDATE);
...
static void trans_array_eis_freq(struct icm_device_info *di)
{
	...
	di->eis_freq_a.batt_index = di->eis_freq_infos.batt_index;
	for (i = 0; i < nums; i++) {
	...
	unsigned long event, void *data)
{
	...
	case EVT_EIS_FREQ_UPDATE:
		...
		icm_eis_uevent(di);
		break;
	case EVT_EIS_EACR_UPDATE:
		if (!data)
			return NOTIFY_OK;
		di->eis_eacr_info = *(struct eis_eacr_info *)data;
		di->eis_evt_to_send |= BIT(EVT_EIS_EACR_UPDATE);
		power_event_notify_sysfs(g_sysfs_bsoh, "icm", "eis_eacr");
		icm_eacr_uevent(di);
		break;
	default:
		break;
...
{
	int ret;
	struct icm_device_info *di = dev_get_drvdata(dev);
	...
	ret = sscanf_s(buff, "freq: %d,temp: %d,cyc: %d,real: %d,img: %d",
		&(di->eis_freq_a.freq[0]), &(di->eis_freq_a.temp[0]),
		&(di->eis_freq_a.cycle[0]), &(di->eis_freq_a.real[0]),
		&(di->eis_freq_a.imag[0]));
	if (ret <= 0) {
...
static int icm_sys_init(struct bsoh_device *b_di)
{
	...
	g_icm_di = info;
	dev_set_drvdata(b_di->dev, g_icm_di);
