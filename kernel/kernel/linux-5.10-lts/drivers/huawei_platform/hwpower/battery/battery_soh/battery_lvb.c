...
static int lvb_dump_coul_data_to_buf(struct lvb_coul_data *data, char *buf, int len)
{
	return snprintf_s(buf, len, len - 1,
		"BSOH_EVT=EVT_VBAT_DROP@batt_index:%d,volt:%d,cur:%d,temp:%d,soc:%d,fcc:%d,time:%ld,vol_mv_drop[0]:%d,vol_mv_"
		"drop[1]:%d,vol_mv_drop[2]:%d",
		data->batt_index, data->volt, data->cur, data->temp, data->soc, data->fcc, data->time, data->vol_mv_drop[FIRST],
		data->vol_mv_drop[SECOND], data->vol_mv_drop[THIRD]);
}
...
int lvb_update_notifier_call(struct notifier_block *nb, unsigned long event, void *data)
{
	...
	if (event == BATT_COUL_DROP) {
		...
		for (int i = 0; i < V_DROP_LEN; i++)
			g_lvb_coul_data.vol_mv_drop[i] = drop_data->vol_mv_drop[i];
