...
unsigned int buck_charge_get_charge_core_data(int info_name)
{
	...
	case BUCK_CHARGE_CORE_VTERM:
		...
		break;
	case BUCK_CHARGE_CORE_VTERM_DEC:
		value = g_core_data->vterm_dec;
		break;
	default:
		break;
	}
...
int buck_charge_set_vterm_dec(unsigned int val)
{
	...
	g_core_data->vterm_dec = val;
	HWDUBAI_LOGE("DUBAI_TAG_BATTERY_CUTOFF_VOLTAGE", "voltage=%d", g_core_data->vterm_bsoh);
	hwlog_info("set charger terminal voltage is %u, vterm_dec=%u\n",
		g_core_data->vterm_bsoh, g_core_data->vterm_dec);
	return 0;
}
...
int buck_charge_check_batfet_single_status(void)
{
	return charge_check_batfet_single_status();
}

int buck_charge_get_vsys_mos_enable(bool *is_enable)
{
	int ret = charge_get_vsys_mos_enable(is_enable);

	hwlog_info("get vsys_mos=%u\n", *is_enable);
	return ret;
}

int buck_charge_set_vsys_mos_enable(bool enable)
{
	int ret = charge_set_vsys_mos_enable(enable);

	hwlog_info("set vsys_mos=%u\n", enable);
	return ret;
}

int buck_charge_get_curr_balance_mos_enable(bool *is_enable)
{
	int ret = charge_get_curr_balance_mos_enable(is_enable);

	hwlog_info("get curr_balance_mos=%u\n", *is_enable);
	return ret;
}

int buck_charge_set_curr_balance_mos_enable(bool enable)
{
	int ret = charge_set_curr_balance_mos_enable(enable);

	hwlog_info("set curr_balance_mos=%u\n", enable);
	return ret;
}
