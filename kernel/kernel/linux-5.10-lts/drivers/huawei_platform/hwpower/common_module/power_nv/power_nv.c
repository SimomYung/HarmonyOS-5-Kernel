...
static struct power_nv_data_info g_power_nv_data[] = {
	...
	{ POWER_NV_BATTINFO, 469, "BATTINFO" },
};
...
int power_common_nv_write(enum power_nv_common_type nv_common_type, void *value)
{
	...
	if (!value || (nv_common_type < POWER_NV_COMMON_BEGIN) ||
	...
	case POWER_NV_COMMON_BATHEAT:
		nv_common_info.bat_heat = (u32)(*(int *)value);
		break;
	case POWER_NV_COMMON_QVAL:
		ret = memcpy_s(&nv_common_info.qval, sizeof(nv_common_info.qval),
			value, sizeof(struct power_nv_fod_qval));
		if (ret)
			goto End;
		break;
	case POWER_NV_COMMON_CALI:
		nv_common_info.cali = *(u8 *)value;
		break;
	case POWER_NV_COMMON_TX_AVG_IIN:
		nv_common_info.coil_test_iin = *(u16 *)value;
		break;
	default:
		goto End;
	}
...
int power_common_nv_read(enum power_nv_common_type nv_common_type, void *value)
{
	...
	switch (nv_common_type) {
	case POWER_NV_COMMON_BATHEAT:
		*(int *)value = (int)nv_common_info.bat_heat;
		break;
	case POWER_NV_COMMON_QVAL:
		(void)memcpy_s(value, sizeof(struct power_nv_fod_qval),
			&nv_common_info.qval, sizeof(nv_common_info.qval));
		break;
	case POWER_NV_COMMON_CALI:
		*(u8 *)value = nv_common_info.cali;
		break;
	case POWER_NV_COMMON_TX_AVG_IIN:
		*(u16 *)value = nv_common_info.coil_test_iin;
		break;
	default:
		break;
	}
