...
static void read_acc_config5_from_dts(struct device_node *dn,
	struct sensor_detect_manager *sm)
{
	struct g_sensor_platform_data *pf_data =
		(struct g_sensor_platform_data *)sm->spara;
	...
	if (!of_property_read_u32(dn, "multi_device_enable", &temp)) {
		if (of_property_read_string(dn, "compatible", (const char **)&chip_info)) {
			hwlog_err("%s:read %s compatible fail\n", __func__, sm->sensor_name_str);
		} else {
			if (strncpy_s(pf_data->g_sensor_extend_data, SENSOR_PLATFORM_EXTEND_DATA_SIZE, chip_info,
				SENSOR_PLATFORM_EXTEND_DATA_SIZE - 1) != EOK) {
	...
	struct sensor_detect_manager *sm)
{
	...
	read_acc_config5_from_dts(dn, sm);
...
int acc_sensor_pre_detect(struct device_node *dn,
	struct sensor_detect_manager *sm, int index)
{
	...
	uint8_t spi_value = 0;
	uint8_t spi_addr;
	uint8_t spi_buf[SC7U22_SPI_BUF_LEN];
	struct detect_word det_wd = { { 0 }, 0 };
	if ((!dn) || (index < 0) || (index >= SENSOR_MAX) || (!sm))
		return RET_FAIL;
	if (of_property_read_string(dn, "compatible", (const char **)&chip_info)) {
		hwlog_err("%s read compatible err\n",  __func__);
		return RET_FAIL;
	}
	if (strncmp(chip_info, "huawei,acc_s009_003", sizeof("huawei,acc_s009_003")))
		return 0;
	if (of_property_read_u32(dn, "switch_page_check", &temp)) {
		hwlog_info("%s: no support switch_page_check configure\n", __func__);
		return 0;
	}
	ret = get_combo_prop(dn, &det_wd);
	if (ret < 0) {
		hwlog_err("%s acc sensor per detect get combo prop err\n",  __func__);
		return RET_FAIL;
	}
	if (det_wd.cfg.bus_type != TAG_SPI)
		return 0;
	spi_addr = SC7U22_POWER_DOWN_ERASE_REG | SC7U22_SPI_BIAS;
	ret = mcu_spi_rw(det_wd.cfg.bus_num, det_wd.cfg.ctrl,
			&spi_addr, 1, &spi_value, 1);
	if (ret < 0)
		return RET_FAIL;
	hwlog_info("%s mcu_spi_rw read spi value %d\n", __func__, spi_value);
	if (spi_value != SC7U22_CHIP_ELECT_DEFAULT) {
		hwlog_info("%s acc_s009_003 mcu page err\n", __func__);
		spi_buf[0] = SC7U22_POWER_DOWN_ERASE_REG;
		spi_buf[1] = SC7U22_CHIP_ELECT_DEFAULT;
		ret = mcu_spi_rw(det_wd.cfg.bus_num, det_wd.cfg.ctrl,
				&spi_buf[0], SC7U22_SPI_BUF_LEN, NULL, 0);
		if (ret < 0)
			hwlog_err("%s mcu_spi_rw write err addr %x, cmd %x\n", __func__, spi_buf[0], spi_buf[1]);
	}
	return 0;
}