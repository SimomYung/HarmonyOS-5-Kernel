...
#define ACC_CAL_CMD_MAX  6
#define ACC_CAL_CMD_MIN  1 
...
static int acc1_calibrate_on_enter(struct acc_device_info *dev_info)
{
	...
	if (sensor_status.opened[TAG_ACC1] == 0) {
		...
		ret = inputhub_sensor_enable(TAG_ACC1, true, HOST_DEFAULT);
		if (ret) {
	...
	if ((sensor_status.delay[TAG_ACC1] == 0) ||
		...
		ret = inputhub_sensor_setdelay(TAG_ACC1, &param, HOST_DEFAULT);
		if (ret) {
			dev_info->acc_calibration_res = COMMU_FAIL;
...
static int acc2_calibrate_on_enter(struct acc_device_info *dev_info)
{
	...
	if (sensor_status.opened[TAG_ACC2] == 0) {
		...
		ret = inputhub_sensor_enable(TAG_ACC2, true, HOST_DEFAULT);
		if (ret) {
			dev_info->acc_calibration_res = COMMU_FAIL;
	...
	if ((sensor_status.delay[TAG_ACC2] == 0) ||
		...
		param.period = ACC_PERIOD_MAX;
		ret = inputhub_sensor_setdelay(TAG_ACC2, &param, HOST_DEFAULT);
		if (ret) {
			dev_info->acc_calibration_res = COMMU_FAIL;
...
static void acc1_calibrate_on_exit(struct acc_device_info *dev_info)
{
	...
	if (dev_info->acc_opened_before_calibrate == 0) {
		...
		ret = inputhub_sensor_enable(TAG_ACC1, false, HOST_DEFAULT);
		if (ret) {
			dev_info->acc_calibration_res = COMMU_FAIL;
...
static void acc2_calibrate_on_exit(struct acc_device_info *dev_info)
{
	...
	if (dev_info->acc_opened_before_calibrate == 0) {
		...
		ret = inputhub_sensor_enable(TAG_ACC2, false, HOST_DEFAULT);
		if (ret) {
			dev_info->acc_calibration_res = COMMU_FAIL;
	...
	struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (val < ACC_CAL_CMD_MIN || val > ACC_CAL_CMD_MAX) {
...
static int acc_calibrate_on_enter(struct acc_device_info *dev_info)
{
	...
	if (sensor_status.opened[TAG_ACCEL] == 0) {
		...
		ret = inputhub_sensor_enable(TAG_ACCEL, true, HOST_DEFAULT);
		if (ret) {
			dev_info->acc_calibration_res = COMMU_FAIL;
	...
	if ((sensor_status.delay[TAG_ACCEL] == 0) ||
		...
		param.period = ACC_PERIOD_MAX;
		ret = inputhub_sensor_setdelay(TAG_ACCEL, &param, HOST_DEFAULT);
		if (ret) {
			dev_info->acc_calibration_res = COMMU_FAIL;
...
static void acc_calibrate_on_exit(struct acc_device_info *dev_info)
{
	...
	if (dev_info->acc_opened_before_calibrate == 0) {
		...
		ret = inputhub_sensor_enable(TAG_ACCEL, false, HOST_DEFAULT);
		if (ret) {
			dev_info->acc_calibration_res = COMMU_FAIL;
