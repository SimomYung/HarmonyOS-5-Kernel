...
static int airpress_calibrate_on_write(struct read_info *read_info)
{
	...
	hwlog_info("attr_airpress_calibrate_write: write_calibrate_data_to_nv success, cResult: %d %d %d %d %d %d %d %d, "\
		"tResult: %d %d %d %d %d %d %d %d\n", pressure_touch_calibrate_data.c_result.slope,
...
static int airpress_calibrate_on_enter(void)
{
	...
	if (sensor_status.opened[TAG_PRESSURE] == 0) { /* if airpress is not opened, open first */
		...
		ret = inputhub_sensor_enable(TAG_PRESSURE, true, HOST_DEFAULT);
		if (ret) {
	...
	if ((sensor_status.delay[TAG_PRESSURE] == 0) ||
		...
		ret = inputhub_sensor_setdelay(TAG_PRESSURE, &param, HOST_DEFAULT);
		if (ret) {
...
static int airpress_calibrate_on_exit(void)
{
	...
	if (airpress_close_after_touch_calibrate == true) {
		...
		ret = inputhub_sensor_enable(TAG_PRESSURE, false, HOST_DEFAULT);
		if (ret) {
