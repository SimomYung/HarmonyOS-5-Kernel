...
static struct als_platform_data als_data[ALS_DEV_COUNT_MAX] = {
	...
	.character_switch = 0,
	.alsc_pic_size = 90, /* set alsc pic size */
	},
};
static struct als_device_info g_als_dev_info[ALS_DEV_COUNT_MAX];

lcd_ddic_info g_lcd_ddic[ALS_DEV_COUNT_MAX] = {
	{TAG_ALS, LCD_OTHER, UNKNOWN_DDIC},
	{TAG_ALS1, LCD_OTHER, UNKNOWN_DDIC},
	{TAG_ALS2, LCD_OTHER, UNKNOWN_DDIC}
};

void als_set_lcd_ddic_info(uint8_t tag)
{
	int i;
	...
	for (i = 0; i < sizeof(g_lcd_ddic) / sizeof(g_lcd_ddic[0]); i++) {
		g_lcd_ddic[i].lcd = get_lcd(g_lcd_ddic[i].tag);
		g_lcd_ddic[i].ddic = get_ddic(g_lcd_ddic[i].tag);
		hwlog_info("%s get tag %d lcd %d, ddic %d\n",
			__func__, tag, g_lcd_ddic[i].lcd, g_lcd_ddic[i].ddic);
	}
}

uint8_t als_get_lcd_info(uint8_t tag)
{
	int i;

	for (i = 0; i < sizeof(g_lcd_ddic) / sizeof(g_lcd_ddic[0]); i++) {
		if (g_lcd_ddic[i].tag == tag)
			return g_lcd_ddic[i].lcd;
	}
	return LCD_OTHER;
}

uint8_t als_get_ddic_info(uint8_t tag)
{
	int i;

	for (i = 0; i < sizeof(g_lcd_ddic) / sizeof(g_lcd_ddic[0]); i++) {
		if (g_lcd_ddic[i].tag == tag)
			return g_lcd_ddic[i].ddic;
	}
	return UNKNOWN_DDIC;
}
...
#if defined DKMD_ALSC_NOISE
void dkmd_alsc_noise_read(struct device_node *dn, struct dkmd_alsc* hisi_alsc_init, int i)
{
	int temp = 0;
	int ret;
	if (of_property_read_u32(dn, "als_ud_size", &temp)) {
		...
		hisi_alsc_init->size = (uint32_t)temp;
		hwlog_info("hisi_dkmd_alsc_init.size:0x%x\n", hisi_alsc_init->size);
	}

	ret = fill_extend_data_in_dts(dn, "als_ud_addr_block",
		(unsigned char *)(hisi_alsc_init->addr_block), ALS_UD_BLOCK_LEN,
		EXTEND_DATA_TYPE_IN_DTS_WORD);
	if (ret) {
		hwlog_err("als_ud_addr_block get data fail\n");
	} else {
		for (i = 0; i < ALS_UD_BLOCK_LEN; i++)
			hwlog_info("hisi_dkmd_alsc_init.addr_block:0x%x\n", hisi_alsc_init->addr_block[i]);
	}

	ret = fill_extend_data_in_dts(dn, "als_ud_ave_block",
		(unsigned char *)(hisi_alsc_init->ave_block), ALS_UD_BLOCK_LEN,
		EXTEND_DATA_TYPE_IN_DTS_WORD);
	if (ret) {
		hwlog_err("als_ud_ave_block get data fail\n");
	} else {
		for (i = 0; i < ALS_UD_BLOCK_LEN; i++)
			hwlog_info("hisi_dkmd_alsc_init.ave_block:0x%x\n", hisi_alsc_init->ave_block[i]);
	}
}

void read_als_ud_from_dts_dkmd(struct device_node *dn, int32_t tag, int i)
{
	if (tag == TAG_ALS)
		dkmd_alsc_noise_read(dn, &hisi_dkmd_alsc_init, i);
	else
		dkmd_alsc_noise_read(dn, &hisi_dkmd_alsc1_init, i);
}
#endif

#if defined DSS_ALSC_NOISE
void read_als_ud_from_dts_dss(struct device_node *dn, int i)
{
	int temp = 0;
	int ret;
	if (of_property_read_u32(dn, "als_ud_size", &temp)) {
...
void read_als_ud_from_dts(struct als_device_info *dev_info,
	struct device_node *dn, int32_t tag)
{
	int temp = 0;
	int ret;
...
#if defined DSS_ALSC_NOISE
	read_als_ud_from_dts_dss(dn, i);
#endif
#if defined DKMD_ALSC_NOISE
	read_als_ud_from_dts_dkmd(dn, tag, i);
#endif
}
	...
	const char *chip_info)
{
	...
	} else if (!strncmp(chip_info, "als_s001_001",sizeof("als_s001_001"))) {
		dev_info->chip_type = ALS_CHIP_TSL2591;
	...
	} else if (!strncmp(chip_info,"huawei,cam_als",sizeof("huawei,cam_als"))){
	...
	} else if (!strncmp(chip_info, "huawei,als_s001010",
		sizeof("huawei,als_s001010"))) {
		dev_info->chip_type = ALS_CHIP_AMS_TCS3743;
	} else 
		return -1;
	return 0;
}
	...
	const char *chip_info)
{
	...
	} else if (!strncmp(chip_info, "huawei,als_s003_006",
		sizeof("huawei,als_s003_006"))) {
		dev_info->chip_type = ALS_CHIP_STK37670_RGB;
	...
	const char *chip_info)
{
	if (!strncmp(chip_info, "huawei,silergy_syh399",
		...
		dev_info->chip_type = ALS_CHIP_SY3133_RGB;
	} else if (!strncmp(chip_info, "als_s004_004",
		sizeof("als_s004_004"))) {
		dev_info->chip_type = ALS_CHIP_SY3135;
	} else if (!strncmp(chip_info, "huawei,als_s004_005",
		sizeof("huawei,als_s004_005"))) {
		dev_info->chip_type = ALS_CHIP_SY81135_RGB;
	} else {
		return -1;
	}
	...
	const char *chip_info)
{
	if (!strncmp(chip_info, "huawei,hx32062se-als",
		...
		dev_info->chip_type = ALS_CHIP_HX32062SE;
	} else if (!strncmp(chip_info, "huawei,als_s007003",
		sizeof("huawei,als_s007003"))) {
		dev_info->chip_type = ALS_CHIP_HX32062SECR;
	} else {
		return -1;
	}
...
static int als_ucs_detect(struct als_device_info *dev_info,
	const char *chip_info)
{
	if (!strncmp(chip_info, "huawei,ucs116e0-als",
		sizeof("huawei,ucs116e0-als"))) {
		dev_info->chip_type = ALS_CHIP_UCS116E0;
	} else if (!strncmp(chip_info, "huawei,als_s009001",
		sizeof("huawei,als_s009001"))) {
		dev_info->chip_type = ALS_CHIP_UCS116E0;
	} else {
		return -1;
	}
	...
	const char *chip_info)
{
	...
	} else if (!als_ucs_detect(dev_info, chip_info)) {
	} else {
		return -1;
	}
	...
	struct sensor_detect_manager *sm)
{
	...
	if (of_property_read_u32(dn, "corresponding_panel", &temp))
		hwlog_err("%s:read als corresponding_panel fail, use LCD_PANEL_MAIN\n", __func__);
	else
		pf_data->corresponding_panel = (int)temp;
	// product type
	if (of_property_read_u32(dn, "pdu_type", &temp))
		hwlog_err("%s:read pdu_type fail\n", __func__);
	else
		pf_data->pdu_type = (int)temp;
}

static void read_als_config4_from_dts(struct device_node *dn,
	struct sensor_detect_manager *sm)
{
	struct als_platform_data *pf_data =
		(struct als_platform_data *)sm->spara;
	...
	if (of_property_read_u32(dn, "is_cct_support", &temp))
		hwlog_err("%s:read is_cct_support fail\n", __func__);
	else
		pf_data->is_cct_support = (int)temp;
	if (of_property_read_u32(dn, "alsc_pic_size", &temp))
		hwlog_err("%s:read als alsc_pic_size fail\n", __func__);
	else
		pf_data->alsc_pic_size = (int)temp;
}
...
int cam_als_detect(struct device_node *dn,
	struct sensor_detect_manager *sm, int index)
{
	...
	if (cam_replace_als_flag) { // use camera als
	...
	struct sensor_detect_manager *sm, int index)
{
	...
	ret = cam_als_detect(dn, sm, index);
	if (ret) {  // sensor
