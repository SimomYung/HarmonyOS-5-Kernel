...
struct als_platform_data {
	...
	uint8_t set_bl_mode;
	uint8_t corresponding_panel;
	uint8_t pdu_type;
	uint8_t is_cct_support;
	uint8_t alsc_pic_size;
...
extern lcd_ddic_info g_lcd_ddic[ALS_DEV_COUNT_MAX];
...
int cam_als_detect(struct device_node *dn, struct sensor_detect_manager *sm, int index);
