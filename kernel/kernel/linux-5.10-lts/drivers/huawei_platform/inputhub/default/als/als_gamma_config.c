...
gamma_map_t g_gamma_map[] = {
	...
	{TAG_ALS, PRODUCT_HED, LCD_BOE, UNKNOWN_DDIC, DEFAULT_LCD_VER, 0, 4095, g_gamma_2_1, g_gamma_2_2, g_gamma_2_3},
	{TAG_ALS, PRODUCT_LMU, LCD_VISIONOX, UNKNOWN_DDIC, DEFAULT_LCD_VER, 0, 4095, g_gamma_2_1, g_gamma_2_2, g_gamma_2_3},
	{TAG_ALS, PRODUCT_LMR, LCD_VISIONOX, UNKNOWN_DDIC, DEFAULT_LCD_VER, 0, 4095, g_gamma_2_1, g_gamma_2_2, g_gamma_2_3},
...
threshold_map_t g_th_table[] = {
	...
	{HED, LCD_OTHER, TAG_END, {{766, 1631, 0}, {766, 1631, 0}, {766, 1631, 0}}},
	{LMR, LCD_OTHER, TAG_END, {{766, 1631, 0}, {766, 1631, 0}, {766, 1631, 0}}},
	{LMU, LCD_OTHER, TAG_END, {{766, 1631, 0}, {766, 1631, 0}, {766, 1631, 0}}},
};
	...
	for (i = 0; i < ARRAY_SIZE(g_gamma_nvdata_map); i++) {
		if (nvdata == g_gamma_nvdata_map[i].nvdata) {
			...
			*b_gamma = g_gamma_nvdata_map[i].b;
			hwlog_info("%s: g_gamma_nvdata_map %d found success\n", __func__, i);
			return;
		}
	}
...
static void get_gamma_cali_result(u32 tag, uint32_t mipi, u32** r_gamma, u32** g_gamma, u32** b_gamma)
{
	...
	if(dev_info->als_ud_nv_type == 1) {
		...
		for (int i = 0; i < ALS_GAMMA_CALI_LEN; i++) {
			g_gamma_cali[i] = (uint32_t)(dev_info->als_under_tp_cal_data.nv_type_defult.b[30 + 2 * i]) & 0x0ffff;
			hwlog_info("%s: g_gamma_cali[%d] = %d\n", __func__, i, g_gamma_cali[i]);
		}
	}
...
bool als_gamma_param_config(uint32_t tag, uint32_t mipi)
{
	...
	if (ddic == DDIC_HISI || ddic == DDIC_HISI05)
