...
#include "als_para_table_ams_tcs3743.h"
...
#include "als_para_table_stk37670.h"
...
uint8_t panel_c_product[] = {BARRY, ALLEN, ALTB, LEM, ADY, HBN, GRL, ADL, CLS, PLR, PLA, ICL, BKY, VDE, MIA, MRT, GLB,
								SUP};

static bool is_in_product_list(struct als_platform_data *pf_data, uint8_t product_list[], uint8_t len)
{
	for (int i = 0; i < len; i++) {
		if (product_list[i] == pf_data->als_phone_type) {
	...
	uint32_t arraysize)
{
	...
	if (is_in_product_list(pf_data, panel_c_product, ARRAY_SIZE(panel_c_product)))
		tplcd = get_multi_lcd_info(pf_data->corresponding_panel);
	else
...
static void select_ams_tcs3718_als_data_ex(struct als_platform_data *pf_data,
	struct als_device_info *dev_info, tcs3718_als_para_table *tcs3718_array,
	uint32_t table_size)
{
	u8 phone_color = get_phone_color();
	u8 tplcd;
	uint32_t i;
	tcs3718_als_para_table *tcs3718_tab = tcs3718_array;
	if (tcs3718_array == NULL)
		return;
	...
	for (i = 0; i < table_size; i++) {
		if ((tcs3718_tab->phone_type == pf_data->als_phone_type) &&
			(tcs3718_tab->phone_version == pf_data->als_phone_version) &&
			(tcs3718_tab->tp_lcd_manufacture == tplcd ||
			tcs3718_tab->tp_lcd_manufacture == DEFAULT_TPLCD ||
			tcs3718_tab->tp_lcd_manufacture == TS_PANEL_UNKNOWN) &&
			(tcs3718_tab->tp_color == phone_color ||
			tcs3718_tab->tp_color == OTHER))
			break;

		tcs3718_tab++;
	}

	tcs3718_tab -= ((i < table_size) ? 0 : i);
	dev_info->min_thres = tcs3718_tab->tcs3718_para[TCS3718_MIN_THRESHOLD_NUM];
	dev_info->max_thres = tcs3718_tab->tcs3718_para[TCS3718_MAX_THRESHOLD_NUM];

	dev_info->table_id = ((i < table_size) ? i : 0);

	pf_data->tp_info = tplcd;

	als_set_extend_data(pf_data, dev_info, tcs3718_tab->tcs3718_para,
		sizeof(tcs3718_tab->tcs3718_para), (i < table_size));
}
...
{
	u8 phone_color = get_phone_color();
	u8 tplcd = get_multi_lcd_info(pf_data->corresponding_panel);
	uint32_t i;
	...
	hwlog_info("%s phone_type=%d phone_version=%d tplcd=%d, phone_color=%d\n",
		__func__, pf_data->als_phone_type, pf_data->als_phone_version, tplcd, phone_color);
	for (i = 0; i < table_size; i++) {
		if ((sip3510_tab->phone_type == pf_data->als_phone_type) &&
			(sip3510_tab->phone_version == pf_data->als_phone_version) &&
			(sip3510_tab->tp_lcd_manufacture == tplcd ||
	...
	uint32_t table_size)
{
	...
	for (i = 0; i < table_size; i++) {
		if ((hx32062se_tab->phone_type == pf_data->als_phone_type) &&
			...
			(hx32062se_tab->tp_lcd_manufacture == tplcd ||
...
static void select_hx32062secr_als_data_ex(struct als_platform_data *pf_data,
	struct als_device_info *dev_info, hx32062se_als_para_table *hx32062secr_array,
	uint32_t table_size)
{
	u8 phone_color = get_phone_color();
	u8 tplcd = get_multi_lcd_info(pf_data->corresponding_panel);
	uint32_t i;
	hx32062se_als_para_table *hx32062secr_tab = hx32062secr_array;

	if (!hx32062secr_array)
		return;

	hwlog_info("%s phone_type=%d phone_version=%d tplcd=%d, phone_color=%d\n",
		__func__, pf_data->als_phone_type, pf_data->als_phone_version, tplcd, phone_color);
	for (i = 0; i < table_size; i++) {
		if ((hx32062secr_tab->phone_type == pf_data->als_phone_type) &&
			(hx32062secr_tab->phone_version == pf_data->als_phone_version) &&
			(hx32062secr_tab->tp_lcd_manufacture == tplcd ||
			hx32062secr_tab->tp_lcd_manufacture == DEFAULT_TPLCD ||
			hx32062secr_tab->tp_lcd_manufacture == TS_PANEL_UNKNOWN) &&
			(hx32062secr_tab->tp_color == phone_color ||
			hx32062secr_tab->tp_color == OTHER))
			break;

		hx32062secr_tab++;
	}

	hx32062secr_tab -= ((i < table_size) ? 0 : i);
	dev_info->min_thres = hx32062secr_tab->hx32062se_para[HX32062SE_MIN_THRESHOLD_NUM];
	dev_info->max_thres = hx32062secr_tab->hx32062se_para[HX32062SE_MAX_THRESHOLD_NUM];
	dev_info->table_id = ((i < table_size) ? i : 0);
	pf_data->tp_info = tplcd;

	als_set_extend_data(pf_data, dev_info, hx32062secr_tab->hx32062se_para,
		sizeof(hx32062secr_tab->hx32062se_para), (i < table_size));
}

static void select_ams_tcs3743_als_data_ex(struct als_platform_data *pf_data,
	struct als_device_info *dev_info, ams_tcs3743_als_para_table *ams_tcs3743_array,
	uint32_t table_size)
{
	u8 phone_color = get_phone_color();
	uint32_t i;
	u8 tplcd;
	ams_tcs3743_als_para_table *ams_tcs3743_tab = ams_tcs3743_array;

	if (!ams_tcs3743_array)
		return;
	...
	for (i = 0; i < table_size; i++) {
		if ((ams_tcs3743_tab->phone_type == pf_data->als_phone_type) &&
			(ams_tcs3743_tab->phone_version == pf_data->als_phone_version) &&
			(ams_tcs3743_tab->tp_lcd_manufacture == tplcd ||
			ams_tcs3743_tab->tp_lcd_manufacture == DEFAULT_TPLCD ||
			ams_tcs3743_tab->tp_lcd_manufacture == TS_PANEL_UNKNOWN) &&
			(ams_tcs3743_tab->tp_color == phone_color ||
			ams_tcs3743_tab->tp_color == OTHER))
			break;

		ams_tcs3743_tab++;
	}

	ams_tcs3743_tab -= ((i < table_size) ? 0 : i);
	dev_info->min_thres = ams_tcs3743_tab->ams_tcs3743_para[AMS_TCS3743_MIN_THRESHOLD_NUM];
	dev_info->max_thres = ams_tcs3743_tab->ams_tcs3743_para[AMS_TCS3743_MAX_THRESHOLD_NUM];
	dev_info->table_id = ((i < table_size) ? i : 0);
	pf_data->tp_info = tplcd;

	als_set_extend_data(pf_data, dev_info, ams_tcs3743_tab->ams_tcs3743_para,
		sizeof(ams_tcs3743_tab->ams_tcs3743_para), (i < table_size));
}
	...
	struct als_device_info *dev_info)
{
	...
	for (i = 0; i < table_size; i++) {
		if ((sy3133_tab->phone_type == pf_data->als_phone_type) &&
			...
			(sy3133_tab->tp_lcd_manufacture == tplcd) &&
...
void select_sy3135_als_data(struct als_platform_data *pf_data,
	struct als_device_info *dev_info)
{
	u8 phone_color = get_phone_color();
	uint32_t table_size;
	uint32_t i;
	sy3135_als_para_table *sy3135_tab = NULL;

	hwlog_info("%s phone_type=%d phone_version=%d phone_color=%d\n",
		__func__, pf_data->als_phone_type, pf_data->als_phone_version, phone_color);

	sy3135_tab = als_get_sy3135_first_table();
	table_size = als_get_sy3135_table_count();
	if (sy3135_tab == NULL)
		return;
	for (i = 0; i < table_size; i++) {
		if ((sy3135_tab->phone_type == pf_data->als_phone_type) &&
			(sy3135_tab->phone_version == pf_data->als_phone_version) &&
			(sy3135_tab->tp_color == phone_color || sy3135_tab->tp_color == OTHER))
			break;

		sy3135_tab++;
	}

	sy3135_tab -= ((i < table_size) ? 0 : i);
	dev_info->table_id = ((i < table_size) ? i : 0);
	als_set_extend_data(pf_data, dev_info, sy3135_tab->sy3135_para,
		sizeof(sy3135_tab->sy3135_para), (i < table_size));
}

void select_silergy_sy81135_als_data(struct als_platform_data *pf_data,
	struct als_device_info *dev_info)
{
	u8 phone_color = get_phone_color();
	uint32_t table_size;
	uint32_t i;
	u8 tplcd;
	sy81135_als_para_table *sy81135_tab = NULL;
	...
	sy81135_tab = als_get_sy81135_first_table();
	table_size = als_get_sy81135_table_count();
	if (sy81135_tab == NULL)
		return;
	for (i = 0; i < table_size; i++) {
		if ((sy81135_tab->phone_type == pf_data->als_phone_type) &&
			(sy81135_tab->phone_version == pf_data->als_phone_version) &&
			(sy81135_tab->tp_lcd_manufacture == tplcd) &&
			(sy81135_tab->tp_color == phone_color))
			break;
 
		sy81135_tab++;
	}
 
	sy81135_tab -= ((i < table_size) ? 0 : i);
	dev_info->min_thres = sy81135_tab->sy81135_para[SY81135_MIN_THRESHOLD_NUM];
	dev_info->max_thres = sy81135_tab->sy81135_para[SY81135_MAX_THRESHOLD_NUM];
 
	dev_info->table_id = ((i < table_size) ? i : 0);
 
	als_set_extend_data(pf_data, dev_info, sy81135_tab->sy81135_para,
		sizeof(sy81135_tab->sy81135_para), (i < table_size));
}

void select_stk37670_als_data(struct als_platform_data *pf_data,
	struct als_device_info *dev_info)
{
	u8 phone_color = get_phone_color();
	uint32_t table_size;
	u8 tplcd;
	uint32_t i;
	stk37670_als_para_table *stk37670_tab = NULL;
	...
	stk37670_tab = als_get_pad_stk37670_first_table();
	table_size = als_get_pad_stk37670_table_count();

	if (stk37670_tab == NULL)
		return;
	for (i = 0; i < table_size; i++) {
		if ((stk37670_tab->phone_type == pf_data->als_phone_type) &&
			(stk37670_tab->phone_version == pf_data->als_phone_version) &&
			(stk37670_tab->tp_lcd_manufacture == tplcd ||
			stk37670_tab->tp_lcd_manufacture == DEFAULT_TPLCD) &&
			(stk37670_tab->tp_color == phone_color ||
			stk37670_tab->tp_color == OTHER))
			break;

		stk37670_tab++;
	}

	stk37670_tab -= ((i < table_size) ? 0 : i);
	dev_info->min_thres = stk37670_tab->stk37670_para[STK37670_MIN_THRESHOLD_NUM];
	dev_info->max_thres = stk37670_tab->stk37670_para[STK37670_MAX_THRESHOLD_NUM];
	dev_info->table_id = ((i < table_size) ? i : 0);
	pf_data->tp_info = tplcd;

	hwlog_info("tpinfo %d\n", pf_data->tp_info);
	als_set_extend_data(pf_data, dev_info, stk37670_tab->stk37670_para,
		sizeof(stk37670_tab->stk37670_para), (i < table_size));
}
...
static void select_ucs116e0_als_data_ex(struct als_platform_data *pf_data,
	struct als_device_info *dev_info, ucs116e0_als_para_table *ucs116e0_array,
	uint32_t table_size)
{
	u8 phone_color = get_phone_color();
	uint32_t i;
	ucs116e0_als_para_table *ucs116e0_tab = ucs116e0_array;
	...
	for (i = 0; i < table_size; i++) {
		if ((ucs116e0_tab->phone_type == pf_data->als_phone_type) &&
			(ucs116e0_tab->phone_version == pf_data->als_phone_version) &&
			(ucs116e0_tab->tp_lcd_manufacture ==
			get_tplcd_manufacture() ||
			ucs116e0_tab->tp_lcd_manufacture == DEFAULT_TPLCD ||
			ucs116e0_tab->tp_lcd_manufacture == TS_PANEL_UNKNOWN) &&
			(ucs116e0_tab->tp_color == phone_color ||
			ucs116e0_tab->tp_color == OTHER))
			break;
 
		ucs116e0_tab++;
	}
 
	ucs116e0_tab -= ((i < table_size) ? 0 : i);
	dev_info->min_thres = ucs116e0_tab->ucs116e0_para[UCS116E0_MIN_THRESHOLD_NUM];
	dev_info->max_thres = ucs116e0_tab->ucs116e0_para[UCS116E0_MAX_THRESHOLD_NUM];
	dev_info->table_id = ((i < table_size) ? i : 0);
	pf_data->tp_info = get_tplcd_manufacture();
 
	als_set_extend_data(pf_data, dev_info, ucs116e0_tab->ucs116e0_para,
		sizeof(ucs116e0_tab->ucs116e0_para), (i < table_size));
}
...
void select_ams_tcs3718_als_data(struct als_platform_data *pf_data,
	struct als_device_info *dev_info)
{
	select_ams_tcs3718_als_data_ex(pf_data, dev_info,
...
void select_hx32062secr_als_data(struct als_platform_data *pf_data,
	struct als_device_info *dev_info)
{
	select_hx32062secr_als_data_ex(pf_data, dev_info,
		als_get_hx32062secr_first_table(),
		als_get_hx32062secr_table_count());
}
...
void select_ams_tcs3743_als_data(struct als_platform_data *pf_data,
	struct als_device_info *dev_info)
{
	select_ams_tcs3743_als_data_ex(pf_data, dev_info,
		als_get_ams_tcs3743_first_table(),
		als_get_ams_tcs3743_table_count());
}
...
void select_ucs116e0_als_data(struct als_platform_data *pf_data,
	struct als_device_info *dev_info)
{
	select_ucs116e0_als_data_ex(pf_data, dev_info,
		als_get_ucs116e0_first_table(),
		als_get_ucs116e0_table_count());
}
