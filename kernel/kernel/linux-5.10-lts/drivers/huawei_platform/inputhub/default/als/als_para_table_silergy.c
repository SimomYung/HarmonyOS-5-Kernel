/*
 * Copyright (c) Huawei Technologies Co., Ltd. 2020-2020. All rights reserved.
 * Description: als para table silergy source file
 * Author: linjianpeng <linjianpeng1@huawei.com>
 * Create: 2020-05-25
 */

#include "als_para_table_silergy.h"

#include <linux/err.h>
#include <linux/of.h>
#include <linux/of_device.h>
#include <linux/of_gpio.h>
#include <linux/slab.h>
#include <linux/types.h>

#include <securec.h>

#include "als_tp_color.h"
#include "contexthub_boot.h"
#include "contexthub_route.h"

als_para_normal_table syh399_als_para[] = {
	{ TEL, V3, TCL_TPLCD, SYH399_PARA_SIZE,
		{ 2055, 23010, 244, 120, 300, 7742, 10000, 2359, 4234, 9004, 804, 10000, 100, 600, 230, 25 }
	},

	{ TEL, V3, TM_TPLCD, SYH399_PARA_SIZE,
		{ 2189, 24089, 185, 120, 300, 7433, 10000, 3357, 3796, 8450, 768, 10000, 100, 600, 230, 25 }
	},

	{ TEL, V3, BOE_TPLCD, SYH399_PARA_SIZE,
		{ 2055, 23010, 244, 120, 300, 7742, 10000, 2359, 4234, 9004, 804, 10000, 100, 600, 230, 25 }
	},

	{ TEL, V3, INX_TPLCD, SYH399_PARA_SIZE,
		{ 2055, 23010, 244, 120, 300, 7742, 10000, 2359, 4234, 9004, 804, 10000, 100, 600, 230, 25 }
	},

	{ TEL, V3, BOE_TPLCD2, SYH399_PARA_SIZE,
		{ 2055, 23010, 244, 120, 300, 7742, 10000, 2359, 4234, 9004, 804, 10000, 100, 600, 230, 25 }
	},

	{ TEL, V3, TS_PANEL_UNKNOWN, SYH399_PARA_SIZE,
		{ 2055, 23010, 244, 120, 300, 7742, 10000, 2359, 4234, 9004, 804, 10000, 100, 600, 230, 25 }
	},

	{ JSC, V3, TS_PANEL_UNKNOWN, SYH399_PARA_SIZE,
		{ 2055, 23010, 244, 120, 300, 7742, 10000, 2359, 4234, 9004, 804, 10000, 100, 600, 230, 25 }
	},

	{ LNA, V3, TS_PANEL_UNKNOWN, SYH399_PARA_SIZE,
		{ 1309, 11104, 700, 120, 250, 2000, 9899, 10459, 11087, 9509, 16560, 1882, 4000, 250, 0, 0 }
	},

	{ BLK, V3, VISI_TPLCD, SYH399_PARA_SIZE,
		{ 180, 230, 420, 2000, 1505, 1559, 1627, 1676, 16560, 1882, 4000, 250, 0, 0, 0, 0 }
	},

	{ BLK, V3, CSOT_TPLCD, SYH399_PARA_SIZE,
		{ 220, 310, 350, 2000, 1246, 1295, 1360, 1423, 16560, 1882, 4000, 250, 0, 0, 0, 0 }
	},

	{ BLK, V3, BOE_TPLCD, SYH399_PARA_SIZE,
		{ 210, 300, 440, 2000, 1335, 1406, 1482, 1536, 16560, 1882, 4000, 250, 0, 0, 0, 0 }
	},

	{ BLK, V3, TS_PANEL_UNKNOWN, SYH399_PARA_SIZE,
		{ 210, 300, 440, 2000, 1335, 1406, 1482, 1536, 16560, 1882, 4000, 250, 0, 0, 0, 0 }
	},

	{ ADA, V3, CSOT_TPLCD, SYH399_PARA_SIZE,
		{ 190, 270, 450, 2000, 2068, 2161, 2295, 2348, 9930, 1020, 4000, 250, 0, 0, 0, 0 }
	},

	{ ADA, V3, BOE_TPLCD, SYH399_PARA_SIZE,
		{ 210, 300, 480, 2000, 1887, 1984, 2085, 2035, 9930, 1020, 4000, 250, 0, 0, 0, 0 }
	},

	{ ADA, V3, TS_PANEL_UNKNOWN, SYH399_PARA_SIZE,
		{ 210, 300, 480, 2000, 1887, 1984, 2085, 2035, 9930, 1020, 4000, 250, 0, 0, 0, 0 }
	},

	{ ADL, V3, CSOT_TPLCD, SYH399_PARA_SIZE,
		{ 200, 280, 480, 2000, 1995, 2048, 2090, 2160, 9930, 1020, 4000, 250, 0, 0, 0, 0 }
	},

	{ ADL, V3, BOE_TPLCD, SYH399_PARA_SIZE,
		{ 170, 260, 430, 2000, 1939, 2013, 2121, 2000, 9930, 1020, 4000, 250, 0, 0, 0, 0 }
	},

	{ ADL, V3, TS_PANEL_UNKNOWN, SYH399_PARA_SIZE,
		{ 180, 250, 420, 2000, 2521, 2678, 2969, 3046, 9930, 1020, 4000, 250, 0, 0, 0, 0 }
	},

	{ BLS, V3, VISI_TPLCD, SYH399_PARA_SIZE,
		{ 210, 300, 440, 2000, 1200, 1190, 1240, 1100, 16560, 1882, 4000, 250, 0, 0, 0, 0 }
	},

	{ BLS, V3, CSOT_TPLCD, SYH399_PARA_SIZE,
		{ 190, 300, 440, 2000, 1121, 1156, 1217, 1155, 16560, 1882, 4000, 250, 0, 0, 0, 0 }
	},

	{ BLS, V3, BOE_TPLCD, SYH399_PARA_SIZE,
		{ 190, 300, 440, 2000, 941, 977, 1082, 1087, 16560, 1882, 4000, 250, 0, 0, 0, 0 }
	},

	{ BLS, V3, TS_PANEL_UNKNOWN, SYH399_PARA_SIZE,
		{ 210, 300, 440, 2000, 1335, 1406, 1482, 1536, 16560, 1882, 4000, 250, 0, 0, 0, 0 }
	},
	{ MIS, V3, CSOT_TPLCD, SYH399_PARA_SIZE,
		{ 195, 300, 430, 2000, 2152, 2007, 1975, 2064, 9930, 1020, 4000, 250, 0, 0, 0, 0 }
	},
	{ MIS, V3, BOE_TPLCD, SYH399_PARA_SIZE,
		{ 210, 300, 480, 2000, 1887, 1984, 2085, 2035, 9930, 1020, 4000, 250, 0, 0, 0, 0 }
	},
	{ MIS, V3, TS_PANEL_UNKNOWN, SYH399_PARA_SIZE,
		{ 210, 300, 480, 2000, 1887, 1984, 2085, 2035, 9930, 1020, 4000, 250, 0, 0, 0, 0 }
	},
	{ BRES, V3, TM_TPLCD, SYH399_PARA_SIZE,
		{ 190, 300, 400, 2000, 1590, 1625, 1723, 1645, 10273, 1282, 4000, 250, 0, 0, 0, 0 }
	},
	{ BRES, V3, CSOT_TPLCD, SYH399_PARA_SIZE,
		{ 190, 300, 440, 2000, 1638, 1665, 1736, 1666, 10273, 1282, 4000, 250, 0, 0, 0, 0 }
	},
	{ BRES, V3, VISI_TPLCD, SYH399_PARA_SIZE,
		{ 190, 300, 430, 2000, 1686, 1711, 1765, 1691, 10273, 1282, 4000, 250, 0, 0, 0, 0 }
	},
	{ BRES, V3, TS_PANEL_UNKNOWN, SYH399_PARA_SIZE,
		{ 210, 300, 440, 2000, 1335, 1406, 1482, 1536, 10273, 1282, 4000, 250, 0, 0, 0, 0 }
	},
	/* xian platfom add {ALS_GAIN, IR_GAIN, H_LIGHT...(8 number lux coefs), als_target, ir_target,
		offset_max, offset_min, als_target_ratio, ir_target_ratio} */
	{ MIA, V3, CSOT_TPLCD, SYH399_PARA_SIZE,
		{ 4, 2, 920, 1452, 1903, 20000, 2344, 2424, 2591, 2505, 7600, 1800, 4000, 250, 1, 1 }
	},
	{ MIA, V3, BOE_TPLCD, SYH399_PARA_SIZE,
		{ 4, 2, 862, 1402, 1934, 20000, 2533, 2599, 2720, 2598, 7600, 1800, 4000, 250, 1, 1 }
	},
	{ MIA, V3, TS_PANEL_UNKNOWN, SYH399_PARA_SIZE,
		{ 4, 2, 920, 1452, 1903, 20000, 2344, 2424, 2591, 2505, 7600, 1800, 4000, 250, 1, 1 }
	}
};
static sy3133_als_para_table sy3133_als_para[] = {
	{ PCE, V3, DEFAULT_TPLCD, BLACK,
	{ 100, 120, 9113, 192, 7994, 15472, 1460, 807, 0, 120, 9113, 192,
		7994, 15472, 1460, 807, 0, 14752, 25094, 4040, 6436, 0, 100, 2048, 6, 0,
		0, 10000, 200 } },
	{ PCE, V3, DEFAULT_TPLCD, WHITE,
	{ 100, 120, 9113, 192, 7994, 15472, 1460, 807, 0, 120, 9113, 192,
		7994, 15472, 1460, 807, 0, 14752, 25094, 4040, 6436, 0, 100, 2048, 6, 0,
		0, 10000, 200 } },
	{ PCE, V3, TS_PANEL_VXN, BLACK,
	{ 100, 120, 9113, 192, 7994, 15500, 1400, 960, 0, 120, 9113, 192,
		7994, 17000, 1400, 960, 0, 12684, 21844, 3600, 6100, 0, 100, 2048, 6, 0,
		250, 10000, 200 } },
	{ PCE, V3, TS_PANEL_VXN, WHITE,
	{ 100, 120, 9113, 192, 7994, 15500, 1400, 960, 0, 120, 9113, 192,
		7994, 17000, 1400, 960, 0, 12684, 21844, 3600, 6100, 0, 100, 2048, 6, 0,
		250, 10000, 200 } },
	{ XYAO, V3, DEFAULT_TPLCD, BLACK,
	{ 100, 120, 9113, 192, 7994, 12472, 1260, 364, 0, 120, 9113, 192,
		7994, 12472, 1800, 364, 0, 23198, 25000, 8052, 8885, 15552, 50, 2048, 5, 0,
		300, 6000, 100 } },
	{ XYAO, V3, DEFAULT_TPLCD, WHITE,
	{ 100, 120, 9113, 192, 7994, 10800, 1050, 1300, 0, 120, 9113, 192,
		7994, 11500, 1100, 1300, 0, 11671, 21105, 5426, 4429, 0, 25, 2048, 4, 0,
		400, 6000, 100 } },
	{ XYAO, V3, TS_PANEL_VXN, BLACK,
	{ 100, 120, 9113, 192, 7994, 12472, 1260, 355, 0, 120, 9113, 192,
		7994, 12472, 1800, 355, 0, 23198, 25000, 8052, 8885, 15552, 50, 2048, 5, 0,
		300, 6000, 100 } },
	{ XYAO, VN1, DEFAULT_TPLCD, BLACK,
	{ 100, 120, 9113, 192, 7994, 12472, 1260, 364, 0, 120, 9113, 192,
		7994, 12472, 1800, 364, 0, 23198, 25000, 8052, 8885, 15552, 50, 2048, 5, 0,
		300, 6000, 100 } },
	{ XYAO, VN1, DEFAULT_TPLCD, WHITE,
	{ 100, 120, 9113, 192, 7994, 10800, 1050, 1300, 0, 120, 9113, 192,
		7994, 11500, 1100, 1300, 0, 11671, 21105, 5426, 4429, 0, 25, 2048, 4, 0,
		400, 6000, 100 } },
	{ XYAO, VN1, TS_PANEL_VXN, BLACK,
	{ 100, 120, 9113, 192, 7994, 12472, 1260, 355, 0, 120, 9113, 192,
		7994, 12472, 1800, 355, 0, 23198, 25000, 8052, 8885, 15552, 50, 2048, 5, 0,
		300, 6000, 100 } },
	{ XYAO, VN1, TS_PANEL_VXN, WHITE,
	{ 100, 120, 9113, 192, 7994, 10800, 1050, 1300, 0, 120, 9113, 192,
		7994, 11500, 1100, 1300, 0, 11671, 21105, 5426, 4429, 0, 25, 2048, 4, 0,
		400, 6000, 100 } },
	{ HARDEN, V3, DEFAULT_TPLCD, WHITE,
	{ 100, 120, 9113, 192, 7994, 10472, 1580, 420, 0, 120, 9113, 192,
		7994, 12472, 1280, 420, 0, 11348, 11821, 4208, 5260, 10000, 50, 2048, 5, 0,
		280, 10000, 200 } },
	{ HARDEN, V3, DEFAULT_TPLCD, BLACK,
	{ 100, 120, 9113, 192, 7994, 10472, 1580, 420, 0, 120, 9113, 192,
		7994, 12472, 1280, 420, 0, 11348, 11821, 4208, 5260, 10000, 50, 2048, 5, 0,
		280, 10000, 200 } },
	{ HARDEN, V4, DEFAULT_TPLCD, WHITE,
	{ 100, 120, 9113, 192, 7635, 10935, 1682, 500, 0, 120, 9113, 192,
		7635, 18093, -733, 500, 0, 11348, 11821, 4208, 5260, 10000, 50, 2048, 5, 0,
		280, 10000, 200 } },
	{ HARDEN, V4, DEFAULT_TPLCD, BLACK,
	{ 100, 120, 9113, 192, 7635, 10935, 1682, 500, 0, 120, 9113, 192,
		7635, 18093, -733, 500, 0, 11348, 11821, 4208, 5260, 10000, 50, 2048, 5, 0,
		280, 10000, 200 } },
	{ WEB, V3, BOE_TPLCD, BLACK,
		{2048, // normalized gain
		10, -39, 47, -40, 1, // screen-off para: c/r/g/b/dgf * 1000
		1, // dgf / ratio
		10, -39, 47, -40, 1,
		100, // normalized atime
		1, 21, 23203, 0, 0, 113, 13862, 0, 0, 1831, 1, 21,
		4, 6436, 14752, 25094, 4040, // ratio/(target/ration)c/r/g/b
		5000, 200
		} },
	{ WEB, V3, BOE_TPLCD, BLACK2,
		{2048, // normalized gain
		10, -39, 47, -40, 1, // screen-off para: c/r/g/b/dgf * 1000
		1, // dgf / ratio
		10, -39, 47, -40, 1,
		100, // normalized atime
		1, 21, 23203, 0, 0, 113, 13862, 0, 0, 1831, 1, 21,
		4, 6436, 14752, 25094, 4040, // ratio/(target/ration)c/r/g/b
		5000, 200
		} },
	{ WEB, V3, VISI_TPLCD, BLACK,
		{2048, // normalized gain
		10, -39, 47, -40, 1, // screen-off para: c/r/g/b/dgf * 1000
		1, // dgf / ratio
		10, -39, 47, -40, 1,
		100, // normalized atime
		1, 21, 23203, 0, 0, 113, 13862, 0, 0, 1831, 1, 21,
		4, 6436, 14752, 25094, 4040, // ratio/(target/ration)c/r/g/b
		5000, 200
		} },
	{ WEB, V3, VISI_TPLCD, BLACK2,
		{2048, // normalized gain
		10, -39, 47, -40, 1, // screen-off para: c/r/g/b/dgf * 1000
		1, // dgf / ratio
		10, -39, 47, -40, 1,
		100, // normalized atime
		1, 21, 23203, 0, 0, 113, 13862, 0, 0, 1831, 1, 21,
		4, 6436, 14752, 25094, 4040, // ratio/(target/ration)c/r/g/b
		5000, 200
		} },
};

static sy3135_als_para_table sy3135_als_para[] = {
	{ TXZ, V3, DEFAULT_TPLCD, OTHER,
	    { 128, 128, //als, ir normalized_gain
	      700, 700, 3000, 4000, // H, A, D, C light
	      60, 60, 60, 60, // H, A, D, C factor
	      31151, 20513, // als_target, ir_target
		  10000, 1, // max_threshold, min_threshold
		  1, 2 // als_target_ratio, ir_target_ratio
	    }
	},
};

static sy81135_als_para_table sy81135_als_para[] = {
	{ SLG, V3, DEFAULT_TPLCD, BLACK,
		{128, // normalized gain
		0, -11, 30, 2, 1, // screen-off para: c/r/g/b/dgf * 1000
		1, // dgf / ratio
		0, -11, 30, 2, 1,
		100, // normalized atime
		1, 30, 21328, 0, 0, -5292, 12571, 0, 0, 74, 1, 60,
		10, 6466, 6466, 9519, 3103, // ratio/(target/ration)c/r/g/b
		5000, 200 }
	},
	{ SLG, V3, DEFAULT_TPLCD, BLACK2,
		{128, // normalized gain
		0, -11, 30, 2, 1, // screen-off para: c/r/g/b/dgf * 1000
		1, // dgf / ratio
		0, -11, 30, 2, 1,
		100, // normalized atime
		1, 30, 21328, 0, 0, -5292, 12571, 0, 0, 74, 1, 60,
		10, 6466, 6466, 9519, 3103, // ratio/(target/ration)c/r/g/b
		5000, 200 }
	},
};

als_para_normal_table *als_get_syh399_table_by_id(uint32_t id)
{
	if (id >= ARRAY_SIZE(syh399_als_para))
		return NULL;
	return &(syh399_als_para[id]);
}

als_para_normal_table *als_get_syh399_first_table(void)
{
	return &(syh399_als_para[0]);
}

uint32_t als_get_syh399_table_count(void)
{
	return ARRAY_SIZE(syh399_als_para);
}

sy3133_als_para_table *als_get_sy3133_table_by_id(uint32_t id)
{
	if (id >= ARRAY_SIZE(sy3133_als_para))
		return NULL;
	return &(sy3133_als_para[id]);
}

sy81135_als_para_table *als_get_sy81135_table_by_id(uint32_t id)
{
	if (id >= ARRAY_SIZE(sy81135_als_para))
		return NULL;
	return &(sy81135_als_para[id]);
}

sy3133_als_para_table *als_get_sy3133_first_table(void)
{
	return &(sy3133_als_para[0]);
}

sy3135_als_para_table *als_get_sy3135_first_table(void)
{
	return &(sy3135_als_para[0]);
}

sy81135_als_para_table *als_get_sy81135_first_table(void)
{
	return &(sy81135_als_para[0]);
}

uint32_t als_get_sy3133_table_count(void)
{
	return ARRAY_SIZE(sy3133_als_para);
}

uint32_t als_get_sy3135_table_count(void)
{
	return ARRAY_SIZE(sy3135_als_para);
}

uint32_t als_get_sy81135_table_count(void)
{
	return ARRAY_SIZE(sy81135_als_para);
}
