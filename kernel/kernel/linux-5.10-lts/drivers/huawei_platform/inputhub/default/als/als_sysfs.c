	...
	uint16_t min_threshold, uint16_t max_threshold)
{
	...
	if (memcpy_s(offset_event.test_name, sizeof(offset_event.test_name),
		...
		1) != EOK)
		return;
	(void)snprintf_s(offset_event.value, MAX_VAL_LEN, MAX_VAL_LEN - 1,
		"%d", value);
	(void)snprintf_s(offset_event.min_threshold, MAX_VAL_LEN, MAX_VAL_LEN - 1,
		"%u", min_threshold);
	(void)snprintf_s(offset_event.max_threshold, MAX_VAL_LEN, MAX_VAL_LEN - 1,
		"%u", max_threshold);
...
#ifdef SENSOR_DATA_ACQUISITION
static const char *als_get_lcd_model_by_tag(int8_t tag)
{
	...
	if (pf_data->corresponding_panel == LCD_PANEL_MAIN) {
		if (of_property_read_string(np, "lcd_panel_type", &target_model))
			hwlog_err("als not find lcd_panel_type in dts\n");
	} else {
		if (of_property_read_string(np, "sec_lcd_panel_type", &target_model))
			hwlog_info("als not find sec_lcd_panel_type in dts\n");
	}
	hwlog_info("%s lcd %d model %s\n", __func__, pf_data->corresponding_panel, target_model);
...
static void als_test_name_join_with_device(struct als_device_info *dev_info)
{
	...
	const char *lcd_model = als_get_lcd_model_by_tag(dev_info->obj_tag);
	if (lcd_model != NULL) {
	...
	struct als_device_info *dev_info)
{
	...
	if ((sensor_status.delay[tag] == 0) ||
		...
		ret = inputhub_sensor_setdelay(tag, &param, HOST_DEFAULT);
		if (ret) {
			dev_info->als_calibration_res = COMMU_FAIL;
	...
	struct als_device_info *dev_info)
{
	...
	if (dev_info->als_opened_before_calibrate == 0) {
		...
		ret = inputhub_sensor_enable(tag, false, HOST_DEFAULT);
		if (ret) {
			dev_info->als_calibration_res = COMMU_FAIL;
