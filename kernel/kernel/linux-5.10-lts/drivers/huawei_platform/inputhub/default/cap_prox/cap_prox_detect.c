...
static void cap_prox_cover_config_print(struct sar_cover_attr *attr)
{
	...
	hwlog_info("cap_prox: cover offset %d %d, base %d %d %d, min %d %d %d\n",
		attr->offset_range[0], attr->offset_range[1],
		attr->base_offset_thred[0], attr->base_offset_thred[1],
		attr->base_offset_thred[2],	attr->min_offset_thred[0],
		attr->min_offset_thred[1], attr->min_offset_thred[2]);
}
	...
	struct sensor_detect_manager *sm)
{
	...
	if (of_property_read_u32_array(dn, "cover_min_offset_thred",
		attr->min_offset_thred, CAP_COVER_MIN_THREH_NUM))
		hwlog_err("%s:read cover_min_offset_thred fail\n", __func__);

	cap_prox_cover_config_print(attr);
}
...
{
	hwlog_info("enter cap_prox_data_from_mcu");
