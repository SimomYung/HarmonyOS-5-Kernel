...
#define CAP_COVER_BASE_THREH_NUM     4
#define CAP_COVER_BIG_SECTION_NUM    4
#define CAP_COVER_BIG_RATIO_NUM      4
#define CAP_PROX_MAX_CHN_NUM         10
#define COVER_TIME_CTRL_NUM          5
#define COVER_WAVE_TIME_NUM          4
#define COVER_WAVE_WINDOW_NUM        2
#define COVER_JUDGE_THRED_CNT_NUM    2
#define COVER_REALTIME_IDX           20
#define COVER_HABIT_IDX              18
...
struct sar_cover_attr {
	...
	uint8_t thred_filter_enable; // whether enable threshold count for nake filter window
	...
	int32_t base_offset_thred[4]; // big range diff threshold: small range saturation, big range cover thrd, no cover thrd
	int32_t big_section_thred[4];  // big range four section threshold when small range full
	uint16_t big_section_ratio[4];  // big range cover and naked section percent (100 times, precision 0f 0.01)
	uint16_t range[2]; // prox_ch range(fF); detect_ch range(fF)
	uint32_t time_ctrl[COVER_TIME_CTRL_NUM]; // time control params, ps: covered threshold days
	uint16_t wave_len[COVER_WAVE_TIME_NUM]; // covered wave time length; naked wave time length
	uint16_t wave_wind[COVER_WAVE_WINDOW_NUM]; // covered wave filter window count; naked wave filter window count
	uint16_t judge_thred[COVER_JUDGE_THRED_CNT_NUM]; // over covered threshold numbers; over naked threhold numbers
	int32_t valley_thred; // for wave valley threshold
};
...
struct cover_nv_data_t {
	uint32_t fac_rawdata[4];
	uint32_t fac_offset[4];
