...
static int cap_prox_calibrate_save(void *buf, int length)
{
	...
	pcaldata = (u32 *)buf;
	hwlog_err("%s:cap_prox calibrate ok, %d, %08x  %08x  %08x, %d %d %d\n",
		__func__, length, pcaldata[0], pcaldata[1], pcaldata[2],
		pcaldata[3], pcaldata[4], pcaldata[5]);
...
static int32_t cap_prox_cali_process(unsigned long val)
{
	...
	if (return_cap_prox_calibration == COMMU_FAIL || pkg_mcu.errno != 0)
		return -1;
	(void)memcpy_s(&dev_info->sar_calibrate_datas,
			sizeof(dev_info->sar_calibrate_datas),
			pkg_mcu.data, pkg_mcu.data_length);
	diff_value = dev_info->sar_calibrate_datas.diff;
	offset_value = dev_info->sar_calibrate_datas.offset;
	if (!strncmp(get_sensor_chip_info_address(CAP_PROX),
		...
		"huawei,sar_s004_001", strlen("huawei,sar_s004_001"))) {
