...
#if (defined CONFIG_HUAWEI_OHOS_DSM) || (defined CONFIG_HISYSEVENT)
...
static u8 g_mcu_all_ready_stat;
...
static int8_t get_lcd_info(uint8_t index)
{
	...
	if (!np) {
		hwlog_err("get_lcd_info not find lcd_panel_type node\n");
		return -1;
	}
...
struct error_no_map sensorhub_error_no_map_list[] = {
		{921001058, "SHB_B2_BATTERY_COVER_STATE_ERR"},
		{921001059, "SHB_B2_ALL_SWITCHS_STATE_ERR"},
		{921001060, "SHB_B2_POWER_SUPPLY_LOGIC_ERR"},
		{922001050, "DSM_SHB_ERR_DEEPCALIB_DEMURA"},
		{922001051, "DSM_SHB_ERR_DEEPCALIB_ODC"},
		{922001052, "DSM_SHB_ERR_DEEPCALIB_DBI"},
		{922001053, "DSM_SHB_ERR_DEEPCALIB_IRC"},
		{922001054, "DSM_SHB_ERR_DEEPCALIB_OTHER"},
		...
		{926006002, "SENSOR_ALS_SENSOR_ERR"},
		{925201620, "IO_DIE_ERROR_INFO"},
		{926005003, "SENSOR_DEVICE_DETECT_ERR"},
		{926005007, "SENSOR_PS_OIL_ERR"},
		{926005009, "DSM_SHB_ERR_ACCEL_DATA_UNCHANGE"},
		{926005010, "DSM_SHB_ERR_POSTURE_EXCEPTION"},
		{926006000, "DSM_SHB_ERR_GYRO_DATA_UNCHANGE"},
		{926006001, "SENSOR_MAG_UNCHANGED_ERR"},
		{926006005, "SENSOR_I2C_ERR"},
		...
		{926003007, "SENSOR_TOUCHKEY_PROBE_I2C_ERR"},
		{926003008, "SENSOR_TOUCHKEY_HANDLER_I2C_ERR"},
		{926003009, "SENSOR_TOUCHKEY_FW_UPDATE_ERR"},
		{926003010, "SENSOR_TOUCHKEY_WAKE_UP_ERR"},
		...
		{926007001, "SHB_ERR_GSENSOR_DATA_ABNORMAL"},
		{926007002, "SHB_ERR_GSENSOR_DATA_ALL_ZERO"},
		{926007003, "SHB_ERR_GSENSOR_DATA_NOT_UPDATE"},
		...
		{926007500, "SENSOR_BARO_I2C_ERR"},
		{926007501, "SENSOR_BARO_DATA_ERR"},
		{926007502, "SENSOR_PPG_I2C_ERR"},
		{926007503, "SENSOR_PPG_DATA_ERR"},
		{926006500, "AP_ERR_COLORSENSOR_DETECT" },
		{927013328, "AP_ERR_COLORSENSOR_I2C_ERR" },
		{926008001, "DSM_SHB_ERR_PUSH_PROXY_ERR"},
		{926005012, "DSM_SHB_ERR_ACC_OFFSET_ERR"},
		{926005013, "DSM_SHB_ERR_GYRO_OFFSET_ERR"},
		{926005017, "DSM_SHB_ERR_AG_CHIPID_ERR"},
};

static int sensorhub_errorno_to_str(int error_no, char *str, int bufflen)
{
	int i;
	
	int error_no_map_len = sizeof(sensorhub_error_no_map_list) / sizeof(sensorhub_error_no_map_list[0]);

	for (i = 0; i < error_no_map_len; i++) {
		if (sensorhub_error_no_map_list[i].error_no == error_no) {
			if (strlen(sensorhub_error_no_map_list[i].name) >= bufflen) {
				hwlog_err("sensorhub_errorno_to_str name too long\n");
				return -1;
			}
			if (strncpy_s(str, bufflen, sensorhub_error_no_map_list[i].name,
				strlen(sensorhub_error_no_map_list[i].name)) != EOK) {
...
static int sensorhub_errorno_get_event_type(int error_no)
{
	int statistic_event_num = sizeof(statistic_event) / sizeof(statistic_event[0]);
	for (int i = 0; i < statistic_event_num; i++) {
		if (error_no == statistic_event[i])
			return STATISTIC;
	}
	return FAULT;
}

static struct error_no_map sensorhub_error_no_map[] = {
	{ 926006500, "AP_ERR_COLORSENSOR_DETECT", 1 },
	{ 927013328, "AP_ERR_COLORSENSOR_I2C_ERR", 1 },
};

static int sensorhub_errorno_get_running_test_switch(int error_no)
{
	size_t i;
	size_t error_no_map_len = sizeof(sensorhub_error_no_map) / sizeof(sensorhub_error_no_map[0]);

	for (i = 0; i < error_no_map_len; i++) {
		if (sensorhub_error_no_map[i].error_no == error_no)
			return sensorhub_error_no_map[i].running_test_switch;
	}
	return 0;
}
#endif
...
struct dsm_client_ops sensorhub_ops = {
	...
	.errorno_get_event_type = sensorhub_errorno_get_event_type,
	.errorno_get_running_test_switch = sensorhub_errorno_get_running_test_switch,
#endif
};
...
bool is_mcu_all_ready(void)
{
	u8 mcu_dump_rec_stat = get_mcu_recovery_stat();
	if (g_mcu_all_ready_stat == 1 && mcu_dump_rec_stat == 1) {
		hwlog_info("detect mcu_all_ready ok");
		return true;
	} else {
		hwlog_info("detect mcu_all_ready fail, g_mcu_all_ready_stat=%u, mcu_dump_rec_stat=%u\n",
				g_mcu_all_ready_stat, mcu_dump_rec_stat);
		return false;
	}
}
...
static void read_reboot_reason_cmdline(void)
{
	...
	if (memcpy_s(reboot_reason_buf, SENSOR_REBOOT_REASON_MAX_LEN, pstr, (unsigned long)(dstr - pstr)))
		pr_err("reboot_reason_buf memcpy failed\n");
...
#if (!defined(CONFIG_LIBLINUX)) || (defined(CONFIG_CONTEXTHUB_LOG_LEVEL_INFO))
	log->log_level = INFO_LEVEL;
#else
	log->log_level = EMG_LEVEL;
#endif
