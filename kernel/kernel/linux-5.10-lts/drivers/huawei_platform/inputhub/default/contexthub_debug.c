...
static int open_sensor(int tag, int argv[], int argc)
{
	...
	if (tag == TAG_STEP_COUNTER)
		ret = inputhub_sensor_enable_stepcounter(true, TYPE_STANDARD, HOST_DEFAULT);
	else
		ret = inputhub_sensor_enable(tag, true, HOST_DEFAULT);
	if (!ret && (argc > 0)) {
		hwlog_info("set sensor %d delay %d ms\n", tag, argv[0]);
		ret = inputhub_sensor_setdelay(tag, &delay_param, HOST_DEFAULT);
	}
...
static int set_ag_data_print_interval(int tag, int argv[], int argc)
{
	int ret;
	...
	ag_ioctl_t ag_ioctl;
	memset_s(&pkg_ap, sizeof(struct write_info), 0, sizeof(struct write_info));

	ag_ioctl.sub_cmd = SUB_CMD_PRINT_REQ;
	ag_ioctl.selftest_code = argv[0];

	pkg_ap.tag = tag;
	pkg_ap.cmd = CMD_CMN_CONFIG_REQ;
	pkg_ap.wr_buf = &ag_ioctl;
	pkg_ap.wr_len = sizeof(ag_ioctl);
...
static int set_sdc_data_print_interval(int tag, int argv[], int argc)
{
	int ret;
	struct write_info pkg_ap;
	...
	sdc_print_config_t sdc_conf;
	memset_s(&pkg_ap, sizeof(struct write_info), 0, sizeof(struct write_info));

	sdc_conf.sub_cmd = SUB_CMD_SDC_PRINT_REQ;
	sdc_conf.print_interval = argv[0];

	pkg_ap.tag = tag;
	pkg_ap.cmd = CMD_CMN_CONFIG_REQ;
	pkg_ap.wr_buf = &sdc_conf;
	pkg_ap.wr_len = sizeof(sdc_conf);
...
static int set_ag_stub_data(int tag, int argv[], int argc)
{
	int ret;
	struct write_info pkg_ap;
	...
	ag_ioctl.sub_cmd = SUB_CMD_SET_STUB_DATA;
	ag_ioctl.selftest_code = argv[0];
...
static int close_sensor(int tag, int argv[], int argc)
{
	...
	if (tag == TAG_STEP_COUNTER)
		inputhub_sensor_enable_stepcounter(false, TYPE_STANDARD, HOST_DEFAULT);
	else
...
static int set_sensor_slave_addr(int tag, int argv[], int argc)
{
	...
	(void)memcpy_s(cpkt.para, sizeof(i2c_address), &i2c_address, sizeof(i2c_address));
...
static int set_sensor_softiron(int tag, int argv[], int argc)
{
	...
	(void)memset_s(&pkg_ap, sizeof(pkg_ap), 0, sizeof(pkg_ap));
	(void)memset_s(&cpkt, sizeof(cpkt), 0, sizeof(cpkt));
	...
	pkg_ap.wr_len = sizeof(int) * SOFTIRON_ARGS_NUM * MUTI_CONVER +
		SUBCMD_LEN;
	(void)memcpy_s(cpkt.para, sizeof(int) * SOFTIRON_ARGS_NUM * MUTI_CONVER, gyro_args_to_mcu,
...
static int set_fp_event(int tag, int data[], int data_count)
{
	...
	(void)memset_s((void *)&req_pkg, sizeof(req_pkg), 0, sizeof(req_pkg));
...
static int get_nv(int tag, int argv[], int argc)
{
	...
	case ACC_OFFSET_NV_NUM:
		strncpy_s(user_info.nv_name, sizeof(user_info.nv_name) + 1, "gsensor", sizeof(user_info.nv_name));
		user_info.valid_size = ACC_OFFSET_NV_SIZE;
		break;
	case ACC1_OFFSET_NV_NUM:
		strncpy_s(user_info.nv_name, sizeof(user_info.nv_name) + 1, "GSENSOR1", sizeof(user_info.nv_name));
		user_info.valid_size = ACC1_OFFSET_NV_SIZE;
		break;
	case GYRO_CALIDATA_NV_NUM:
		strncpy_s(user_info.nv_name, sizeof(user_info.nv_name) + 1, "GYRO", sizeof(user_info.nv_name));
		user_info.valid_size = GYRO_CALIDATA_NV_SIZE;
		break;
	case GYRO1_OFFSET_NV_NUM:
		strncpy_s(user_info.nv_name, sizeof(user_info.nv_name) + 1, "GYRO1", sizeof(user_info.nv_name));
...
#ifdef CONFIG_INPUTHUB_SENSORHUB_IPC
#define SENSORHUB_IPC_ARGC_MAX_LEN    1024
#define SENSORHUB_IPC_ARGC_MIN_LEN    4
#define SENSORHUB_IPC_MAX_LEN 2048

static int send_cmd_formats(unsigned char cmd_tag, unsigned char cmd_type, 
				unsigned int subtype, const char *buf, unsigned int count,
			    bool is_lock)
{
	char *buffer = NULL;
	int ret = 0;
	...
	buffer = kzalloc(SENSORHUB_IPC_MAX_LEN, GFP_KERNEL);
	if (buffer == NULL) {
		hwlog_err("%s malloc fail\n", __func__);
		return 0;
	}
	winfo.tag = cmd_tag;
	winfo.cmd = cmd_type;
	hwlog_info("[%s] : cmd_tag[0x%x] cmd[%d:0x%x]  count[%ld]\n",
		__func__, cmd_tag, cmd_type, subtype, count);

	if (CMD_CMN_CONFIG_REQ != cmd_type) {
		if (count) {
			ret = memcpy_s(buffer + sizeof(struct pkt_header), SENSORHUB_IPC_MAX_LEN - sizeof(struct pkt_header),
				       buf, count);
			if (EOK != ret) {
				hwlog_err("%s memcpy buffer fail, ret[%d]\n", __func__, ret);
				kfree(buffer);
				return -ENOMEM;
			}
		}
		...
		ret = write_customize_cmd(&winfo, NULL, is_lock);
	} else {
		unsigned int *subcmd = (unsigned int *)(((struct pkt_header *)buffer) + 1);
		*subcmd = subtype;
		if (count) {
			ret = memcpy_s(buffer + (sizeof(struct pkt_header) + sizeof(unsigned int)),
							SENSORHUB_IPC_MAX_LEN - (sizeof(struct pkt_header) + sizeof(unsigned int)),
							buf, count);
			if (EOK != ret) {
				hwlog_err("%s memcpy buffer fail, ret[%d]\n", __func__, ret);
				kfree(buffer);
...
static int ipc_dbg(int tag, int argv[], int argc)
{
	char *buf = NULL;
	uint32_t *buf32 = NULL;
	struct sensorhub_ipc_data *ipc_data = NULL;
	int ret;
	uint32_t i;

	if (argc > SENSORHUB_IPC_ARGC_MAX_LEN || argc <= SENSORHUB_IPC_ARGC_MIN_LEN) {
		hwlog_err("%s : invalid arg number %x.\n", __func__, argc);
		return -1;
	}

	buf = kzalloc(sizeof(struct sensorhub_ipc_data) + argc - SENSORHUB_IPC_ARGC_MIN_LEN, GFP_ATOMIC);
	if (buf == NULL) {
		hwlog_err("%s malloc fail\n", __func__);
		return 0;
	}
	...
	for (i = 0; i < SENSORHUB_IPC_ARGC_MIN_LEN; i++) {
		buf32[i] = argv[i];
	}

	buf += sizeof(struct sensorhub_ipc_data);

	for (i = 0; i < argc - SENSORHUB_IPC_ARGC_MIN_LEN; i++) {
		buf[i] = (char)argv[i + SENSORHUB_IPC_ARGC_MIN_LEN];
	}

	ipc_data = (struct sensorhub_ipc_data *)buf32;
	if ((argc - SENSORHUB_IPC_ARGC_MIN_LEN) != ipc_data->buf_len) {
		hwlog_err("%s : invalid buf_len %x.\n", __func__, ipc_data->buf_len);
		kfree(buf32);
		return -1;
	}
	ret = send_cmd_formats(ipc_data->cmd_tag, ipc_data->cmd_type,
		ipc_data->sub_type, ipc_data->buf, ipc_data->buf_len,true);
	if (ret) {
		hwlog_err("%s send_cmd_formats fail[%d]\n", __func__, ret);
	} else {
		hwlog_err("%s tag:%x cmd %x sub_cmd %x buf len %x\n", __func__,
			ipc_data->cmd_tag, ipc_data->cmd_type, ipc_data->sub_type, ipc_data->buf_len);
		hwlog_err("%s sucess\n", __func__);
	}
	kfree(buf32);
	return 0;
	...
	if ((cmd == CMD_CMN_OPEN_REQ) || (cmd == CMD_CMN_CLOSE_REQ)) {
		...
		for (i = 0; i < AR_UNKNOWN; i++) {
			if (activity & (1ULL << i)) {
				...
				(void)memcpy_s((char *)ar_start + sizeof(ar_start_cmd_t) +
					start_len, sizeof(ar_config_event_t), activity_list,
...
static int activity_common_test(int tag, int argv[], int argc)
{
	...
	(void)memset_s(&pkg_ap, sizeof(pkg_ap), 0, sizeof(pkg_ap));
	(void)memset_s(&pkg_mcu, sizeof(pkg_mcu), 0, sizeof(pkg_mcu));
	(void)memset_s(&ar_stop, sizeof(ar_stop), 0, sizeof(ar_stop));
...
#ifdef CONFIG_INPUTHUB_30
	register_sensorhub_debug_op(set_ag_data_print_interval);
	register_sensorhub_debug_op(set_sdc_data_print_interval);
	register_sensorhub_debug_op(set_ag_stub_data);
#endif
#ifdef CONFIG_INPUTHUB_SENSORHUB_IPC
	register_sensorhub_debug_op(ipc_dbg);
#endif
}
...
#ifdef CONFIG_INPUTHUB_30
	ret |= (unsigned int)unregister_sensorhub_debug_operation(set_ag_data_print_interval);
	ret |= (unsigned int)unregister_sensorhub_debug_operation(set_sdc_data_print_interval);
	ret |= (unsigned int)unregister_sensorhub_debug_operation(set_ag_stub_data);
#endif
#ifdef CONFIG_INPUTHUB_SENSORHUB_IPC
	ret |= (unsigned int)unregister_sensorhub_debug_operation(ipc_dbg);
#endif
	if (ret != 0)
		hwlog_err("%s err\n", __func__);
}
	...
	struct class_attribute *attr, char *buf)
{
	...
	offset += sprintf_s(buf + offset, MAX_COPY_STR,
	...
	list_for_each_entry_safe(node, n,
		&sensor_debug_operations_list.head, entry) {
		offset += sprintf_s(buf + offset, MAX_COPY_STR, "%s\n", node->str);
	}
	mutex_unlock(&sensor_debug_operations_list.mlock);
...
ssize_t cls_attr_mcu_all_ready_show_func(struct class *cls, struct class_attribute *attr, char *buf)
{
	hwlog_info("read sensorhub_mcu_all_ready node\n");
	if (is_mcu_all_ready() == true)
		return snprintf_s(buf, MAX_STR_SIZE, MAX_STR_SIZE - 1, "mcu all ready detected\n");
	else
		return snprintf_s(buf, MAX_STR_SIZE, MAX_STR_SIZE - 1, "mcu all ready not detected\n");
}

static struct class_attribute class_attr_sensorhub_mcu_all_ready =
	__ATTR(sensorhub_mcu_all_ready, 0660, cls_attr_mcu_all_ready_show_func, NULL); /* 0660:attr mode */
	...
	struct class_attribute *attr, char *buf)
{
	...
	hwlog_info("read %s %d\n", __func__, ver);
	(void)memcpy_s(buf, sizeof(ver), &ver, sizeof(ver));
	return sizeof(ver);
}
...
#ifdef CONFIG_DKMD_AOD_DEBUG
static ssize_t cls_attr_dump_aod_mem_show_func(struct class *cls,
	struct class_attribute *attr, char *buf)
{
	unsigned int offset = 0;

	offset += snprintf_s(buf + offset, MAX_STR_SIZE, MAX_STR_SIZE - 1,
			"for example(in hex):\n input: echo dump_mem [addr] [size] [cma-flag] > %s\n",
			attr->attr.name);
	return offset;
}

#define AOD_DUMP_DATA_DIR      "/data/vendor/log/sensorhub"
#define AOD_DUMP_DATA_FILE     "/data/vendor/log/sensorhub/aod_mem_info.data"
static void dump_aod_mem_info(unsigned int dump_addr, unsigned int length, unsigned int cma_flag)
{
	...
	if (ret) {
		ret = (int)vendor_mkdir(AOD_DUMP_DATA_DIR, 0770); /* 0770: dir rights */
		if (ret < 0) {
			hwlog_err("create %s failed, ret = %d\n", AOD_DUMP_DATA_DIR, ret);
			set_fs(old_fs);
			return;
		}
		...
		/* chown group */
		pfile = filp_open(AOD_DUMP_DATA_DIR, O_RDONLY, DIR_LIMIT);
		if (IS_ERR_OR_NULL(pfile)) {
			hwlog_err("open %s failed\n", AOD_DUMP_DATA_DIR);
			vendor_rmdir(AOD_DUMP_DATA_DIR);
			set_fs(old_fs);
			return;
		}
	...
	/* write content */
	fp = filp_open(AOD_DUMP_DATA_FILE, O_CREAT | O_WRONLY | O_TRUNC, FILE_LIMIT);
	if (IS_ERR(fp)) {
...
static ssize_t cls_attr_dump_aod_mem_store_func(struct class *cls,
	struct class_attribute *attr, const char *buf, size_t size)
{
	unsigned int dump_addr = 0;
	unsigned int len = 0;
	unsigned int cma_flag = 0;
	int one_param = 1;
	int ret = 0;
	const char *cmd_buf = buf;
	char *p = NULL;
	char *q = NULL;
	char *t = NULL;
	...
	cmd_buf = get_str_begin(cmd_buf);
	hwlog_info("cmd is %s\n", cmd_buf);

	p = strstr(cmd_buf, "0x");
	if (p == NULL)
		return size;
	ret = sscanf_s(p, "%x", &dump_addr);
	if (ret != one_param)
		return size;
	hwlog_info("addr is 0x%x\n", dump_addr);

	q = strchr(p + 2, 'x');
	if (q == NULL)
		return size;
	ret = sscanf_s(q - 1, "%x", &len);
	if (ret != one_param)
		return size;
	hwlog_info("length is 0x%x\n", len);

	t = strchr(q + 2, 'x');;
	if (t == NULL)
		return size;
	ret = sscanf_s(t - 1, "%x", &cma_flag);
	if (ret != one_param)
		return size;
	hwlog_info("flag is 0x%x\n", cma_flag);

	if (dump_addr != 0 && len != 0)
		dump_aod_mem_info(dump_addr, len, cma_flag);

	return size;
}

static struct class_attribute class_attr_attr_dump_aod_mem =
	__ATTR(dump_aod_mem, 0660, cls_attr_dump_aod_mem_show_func, /* 0660:attr mode */
	cls_attr_dump_aod_mem_store_func);
#endif
...
#ifdef CONFIG_DKMD_AOD_DEBUG
	if (class_create_file(sensors_class, &class_attr_attr_dump_aod_mem))
		hwlog_err("create files aod cma dump in %s\n", __func__);
#endif

	if (class_create_file(sensors_class, &class_attr_sensorhub_mcu_all_ready))
		hwlog_err("create files sensorhub_mcu_all_ready in %s\n", __func__);
}
...
static const char *get_iomcu_power_status(void)
{
	...
	(void)memset_s(show_str, MAX_STR_SIZE, 0, MAX_STR_SIZE);
	...
	switch (status) {
	case SUB_POWER_ON:
		(void)snprintf_s(show_str, MAX_STR_SIZE, MAX_STR_SIZE - 1, "%s", "SUB_POWER_ON");
		break;
	case SUB_POWER_OFF:
		(void)snprintf_s(show_str, MAX_STR_SIZE, MAX_STR_SIZE - 1, "%s", "SUB_POWER_OFF");
		break;
	default:
		(void)snprintf_s(show_str, MAX_STR_SIZE, MAX_STR_SIZE - 1, "%s", "unknown status");
		break;
	}
	return show_str;
}
	...
	for (i = 0; i < TAG_END; i++) {
		...
		if (i_power_status.app_status[i]) {
			if (obj_tag_str[i] != NULL) {
				...
				(void)strncpy_s(buf, copy_length + 1, obj_tag_str[i], copy_length);
			} else {
			...
			if (index < MAX_STR_SIZE) {
				if (strcat_s(show_str, MAX_STR_SIZE, buf) != 0)
	...
	for (i = 0; i < TAG_HW_PRIVATE_APP_END; i++) {
		...
		if (tf) {
			if (iomcu_app_id_str[i] != NULL) {
				...
				if (strncpy_s(buf, copy_length + 1, iomcu_app_id_str[i], copy_length) != 0)
...
{
	(void)memset_s(&i_power_status, sizeof(iomcu_power_status), 0, sizeof(iomcu_power_status));
