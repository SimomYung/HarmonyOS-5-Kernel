/*
 * Copyright (c) Huawei Technologies Co., Ltd. 2012-2020. All rights reserved.
 * Description: contexthub logbuff source file
 * Author: DIVS_SENSORHUB
 * Create: 2012-05-29
 */
#include "contexthub_logbuff.h"
#include "contexthub_log_throttling.h"
#include "securec.h"

#include <linux/delay.h>
#include <linux/device.h>
#include <linux/fs.h>
#include <linux/gpio.h>
#include <linux/init.h>
#include <linux/interrupt.h>
#include <linux/irq.h>
#include <linux/kernel.h>
#include <linux/module.h>
#include <linux/mutex.h>
#include <linux/platform_device.h>
#include <linux/proc_fs.h>
#include <linux/reboot.h>
#include <linux/rtc.h>
#include <linux/sched.h>
#include <linux/slab.h>
#include <linux/sysfs.h>
#include <linux/timer.h>

#include <asm/io.h>
#include <asm/uaccess.h>
#include <huawei_platform/log/hw_log.h>
#ifndef CONFIG_SECURITY_HEADER_FILE_REPLACE
#ifdef CONFIG_HEADER_FILE_REPLACE_EXTRACT
#include <iomcu_mem.h>
#else
#include <iomcu_ddr_map.h>
#endif
#else
#include <internal_security_interface.h>
#endif
#include "contexthub_route.h"
#include <platform_include/smart/linux/base/ap/protocol.h>
#include <linux/syscalls.h>

#define RUN_SSH_LOG_LABEL_GENERATE_MAX     10
#define LOG_BUFF_SIZE (1024*4)
#ifndef CONFIG_SECURITY_HEADER_FILE_REPLACE
#define DDR_LOG_BUFF_UPDATE_NR (DDR_LOG_BUFF_SIZE / LOG_BUFF_SIZE)
#define DDR_LOG_BUFF_UPDATE_WATER_MARK (DDR_LOG_BUFF_UPDATE_NR * 3 / 4)
#define DDR_LOG_BUFF_COPY_SIZE (DDR_LOG_BUFF_UPDATE_WATER_MARK * LOG_BUFF_SIZE)
#endif

#define LOG_SERIAL 1
#define LOG_BUFF 2
#define SPACESIZE 2
#define LOG_THRO_CASE1  1
#define LOG_THRO_CASE2  2

/* unite jiffies */
#define FLUSH_TIMEOUT (2*HZ)

static int is_opened;
static int is_new_data_available;
static int is_flush_complete;
static int flush_cnt;

static uint32_t sensorhub_log_r;
static uint32_t sensorhub_log_buf_head;
static uint32_t sensorhub_log_buf_rear;
static uint8_t *ddr_log_buff;
static uint8_t *local_log_buff;
static struct mutex logbuff_mutex;
static struct mutex logbuff_open_mutex;
static struct mutex logbuff_flush_mutex;
static struct proc_dir_entry *logbuff_dentry;
static uint32_t log_method = LOG_BUFF;
static uint32_t sensorhub_log_full_flag;
static bool inited;

#define CONFIG_FLUSH '1'
#define CONFIG_SERIAL '2'
#define CONFIG_BUFF '3'

#define EMG_LEVEL   0
#define ERR_LEVEL   1
#define WARN_LEVEL  2
#define INFO_LEVEL  3
#define DEBUG_LEVEL 4

#define NO_LOG_DEFAULT_LEVEL EMG_LEVEL

#define DEFAULT_SENSORHUB_LOG_PATH "/data/log/sensorhub-log/sensorhub-log"
#define DEFAULT_SENSORHUB_LOG_PATH1 "/data/log/sensorhub-log/sensorhub-log.1"

int32_t gmark = 0xF0;

static DECLARE_WAIT_QUEUE_HEAD(sensorhub_log_waitq);
static DECLARE_WAIT_QUEUE_HEAD(sensorhub_log_flush_waitq);

static inline void print_stat(int i)
{
	hwlog_debug("[%d][r %x][head %x][rear %x][full_flag %d]\n",
		i, sensorhub_log_r, sensorhub_log_buf_head,
		sensorhub_log_buf_rear, sensorhub_log_full_flag);
}

static int sensorhub_log_buff_left(void)
{
	uint32_t ddr_log_buff_size;
#ifdef CONFIG_SECURITY_HEADER_FILE_REPLACE
	ddr_log_buff_size = dts_ddr_size_get(DTSI_DDR_LOG_BUFF_ADDR_AP);
#else
	ddr_log_buff_size = DDR_LOG_BUFF_SIZE;
#endif
	hwlog_debug("%s %d\n", __func__,
		(sensorhub_log_buf_rear >= sensorhub_log_r) ?
		(sensorhub_log_buf_rear - sensorhub_log_r) :
		(ddr_log_buff_size - (sensorhub_log_r -
		sensorhub_log_buf_rear)));
	if (sensorhub_log_full_flag &&
		(sensorhub_log_buf_rear == sensorhub_log_r))
		return 0;
	return (sensorhub_log_buf_rear >= sensorhub_log_r) ?
		(sensorhub_log_buf_rear - sensorhub_log_r) :
		(ddr_log_buff_size - (sensorhub_log_r -
			sensorhub_log_buf_rear));
}

static void update_local_buff_index(uint32_t new_rear)
{
	uint32_t ddr_log_buff_copy_size;
	uint32_t ddr_log_buf_size;
#ifdef CONFIG_SECURITY_HEADER_FILE_REPLACE
	ddr_log_buf_size = dts_ddr_size_get(DTSI_DDR_LOG_BUFF_ADDR_AP);
	/* 3 / 4:ddr buffer water mark */
	ddr_log_buff_copy_size = (((ddr_log_buf_size / LOG_BUFF_SIZE) * 3 / 4) * LOG_BUFF_SIZE);
#else
	ddr_log_buf_size = DDR_LOG_BUFF_SIZE;
	ddr_log_buff_copy_size = DDR_LOG_BUFF_COPY_SIZE;
#endif
	/* update sensorhub_log_r */
	if (flush_cnt && (sensorhub_log_buff_left() >=
		(ddr_log_buf_size - ddr_log_buff_copy_size)))
		sensorhub_log_r = new_rear;
	/* update sensorhub_log_buf_head */
	if (flush_cnt)
		sensorhub_log_buf_head = new_rear;
	/* update sensorhub_log_buf_rear */
	sensorhub_log_buf_rear = new_rear;
	sensorhub_log_full_flag = 0;
	hwlog_debug("[%s] %d %d %d\n",
		__func__, sensorhub_log_r,
		sensorhub_log_buf_head, sensorhub_log_buf_rear);
}

static int sensorhub_logbuff_open(struct inode *inode, struct file *file)
{
	hwlog_info("[%s]\n", __func__);
	mutex_lock(&logbuff_open_mutex);
	if (is_opened) {
		hwlog_err("%s sensorhub logbuff already opened !\n", __func__);
		mutex_unlock(&logbuff_open_mutex);
		return -1;
	}
	mutex_lock(&logbuff_mutex);
	sensorhub_log_r = sensorhub_log_buf_head;
	mutex_unlock(&logbuff_mutex);
	is_opened = 1;
#ifdef CONFIG_LIBLINUX
	int level = NO_LOG_DEFAULT_LEVEL;
	if(ksys_access(DEFAULT_SENSORHUB_LOG_PATH, 0) == 0 || ksys_access(DEFAULT_SENSORHUB_LOG_PATH1, 0) == 0) {
		hwlog_info("%s: sensorhub-log exist, log version\n", __func__);
		level = INFO_LEVEL;
	} else {
		hwlog_info("%s: sensorhub-log not exist, nolog version\n", __func__);
		level = ERR_LEVEL;
	}
	set_log_level(TAG_LOG_BUFF, &level, 1);
#endif
	mutex_unlock(&logbuff_open_mutex);
	return 0;
}

static int sensorhub_logbuff_release(struct inode *inode, struct file *file)
{
	hwlog_info("sensorhub logbuff release\n");
	mutex_lock(&logbuff_open_mutex);
	is_opened = 0;
	mutex_unlock(&logbuff_open_mutex);
	return 0;
}

static void get_ddr_buff_size(uint32_t *log_buf_size, uint32_t *log_buff_copy_size)
{
#ifdef CONFIG_SECURITY_HEADER_FILE_REPLACE
	*log_buf_size = dts_ddr_size_get(DTSI_DDR_LOG_BUFF_ADDR_AP);
	/* 3 / 4: ddr buffer water mark */
	*log_buff_copy_size = (((*log_buf_size / LOG_BUFF_SIZE) * 3 / 4) * LOG_BUFF_SIZE);
#else
	*log_buf_size = DDR_LOG_BUFF_SIZE;
	*log_buff_copy_size = DDR_LOG_BUFF_COPY_SIZE;
#endif
}

static void copy_logbuff_to_logthro(uint8_t CMD, const uint8_t *local_log_buff_r, int cnt, int remain)
{
	const uint8_t *logbuff_copy_pointer = NULL;
	int len = 0;
    switch (CMD) {
	case LOG_THRO_CASE1:
		if (cnt > remain) {
			len = cnt - remain;
			logbuff_copy_pointer = local_log_buff_r;
		}
		break;
	case LOG_THRO_CASE2:
		len = cnt;
		logbuff_copy_pointer = local_log_buff_r;
		break;
	default:
		hwlog_err("copy_logbuff_to_logthro get unvalid cmd: %d\n", CMD);
		break;
	}

	if (collect_ssh_log_num(logbuff_copy_pointer, (size_t)len)) {
		return;
	}
}

static ssize_t sensorhub_logbuff_read(struct file *file, char __user *buf, size_t count, loff_t *ppos)
{
	int cnt, remain, error = 0;
	uint32_t ddr_log_buff_copy_size = 0, ddr_log_buf_size = 0;
	get_ddr_buff_size(&ddr_log_buf_size, &ddr_log_buff_copy_size);
	cnt = (count > ddr_log_buff_copy_size) ? ddr_log_buff_copy_size : count;
	if (!buf || !count || !local_log_buff)
		goto err;

	/* check cap */
	if (sensorhub_log_buff_left() >= count)
		goto copy;

	error = wait_event_interruptible(sensorhub_log_waitq, is_new_data_available != 0);
	if (error) {
		error = 0;
		goto err;
	}
copy:
	print_stat(2);
	mutex_lock(&logbuff_mutex);
	is_new_data_available = 0;

	if (sensorhub_log_r > ddr_log_buf_size) {
		remain = 0;
		hwlog_err("%s sensorhub_log_r is too large\n", __func__);
	} else {
		remain = ddr_log_buf_size - sensorhub_log_r;
	}
	error = cnt;
	/* copy to user */
	if (cnt > remain) {
		if (copy_to_user(buf + remain, local_log_buff, (cnt - remain))) {
			hwlog_err("%s failed to copy to user\n", __func__);
			error = -EFAULT;
			goto out;
		}
		copy_logbuff_to_logthro(LOG_THRO_CASE1, (const uint8_t *)local_log_buff, cnt, remain);
		cnt = remain;
	}

	if (sensorhub_log_r + cnt > ddr_log_buf_size) {
		error = -EFAULT;
		goto out;
	}

	if (copy_to_user(buf, (const uint8_t *)(local_log_buff + sensorhub_log_r), cnt)) {
		hwlog_err("%s failed to copy to user\n", __func__);
		error = -EFAULT;
		goto out;
	}
	copy_logbuff_to_logthro(LOG_THRO_CASE2, local_log_buff + sensorhub_log_r, cnt, remain);

	/* update reader pointer */
	sensorhub_log_r = (sensorhub_log_r + error) % ddr_log_buf_size;
	if (!sensorhub_log_full_flag && sensorhub_log_r == sensorhub_log_buf_rear)
		sensorhub_log_full_flag = 1;

out:
	mutex_unlock(&logbuff_mutex);
err:
	print_stat(3);
	return error;
}

#if (LINUX_VERSION_CODE >= KERNEL_VERSION(5, 10, 0))
static const struct proc_ops sensorhub_logbuff_operations = {
	.proc_open = sensorhub_logbuff_open,
	.proc_read = sensorhub_logbuff_read,
	.proc_release = sensorhub_logbuff_release,
};
#else
static const struct file_operations sensorhub_logbuff_operations = {
	.open = sensorhub_logbuff_open,
	.read = sensorhub_logbuff_read,
	.release = sensorhub_logbuff_release,
};
#endif

static int logbuff_full_callback(const struct pkt_header *head)
{
	int cnt;
	int remain;
	uint32_t update_index;
	uint32_t new_rear;
	log_buff_req_t *pkt = (log_buff_req_t *) head;
	uint32_t ddr_log_buff_size;
	int ddr_log_buf_copy_size;
#ifdef CONFIG_SECURITY_HEADER_FILE_REPLACE
	ddr_log_buff_size = dts_ddr_size_get(DTSI_DDR_LOG_BUFF_ADDR_AP);
	/* 3 / 4:ddr buffer water mark */
	ddr_log_buf_copy_size = (((ddr_log_buff_size / LOG_BUFF_SIZE) * 3 / 4) * LOG_BUFF_SIZE);
#else
	ddr_log_buff_size = DDR_LOG_BUFF_SIZE;
	ddr_log_buf_copy_size = DDR_LOG_BUFF_COPY_SIZE;
#endif
	cnt = ddr_log_buf_copy_size;
	if (pkt->index > ddr_log_buff_size / LOG_BUFF_SIZE) {
		hwlog_err("%s index is too large, log maybe lost\n", __func__);
		update_index = ddr_log_buff_size;
	} else {
		update_index = (pkt->index * LOG_BUFF_SIZE);
	}
	new_rear = (update_index + ddr_log_buf_copy_size) % ddr_log_buff_size;
	hwlog_info("[%s]\n", __func__);
	if (update_index != sensorhub_log_buf_rear)
		hwlog_err("%s unsync index, log maybe lost\n", __func__);
	/* get rotate log buff index */
	mutex_lock(&logbuff_mutex);
	print_stat(4); /* stat 4 */
	update_local_buff_index(new_rear);
	print_stat(5); /* stat 5 */
	remain = ddr_log_buff_size - update_index;
	/* update reader pointer */
	if (remain < ddr_log_buf_copy_size) {
		memcpy_s(local_log_buff, cnt - remain, ddr_log_buff, cnt - remain);
		cnt = remain;
	}
	memcpy_s(local_log_buff + update_index, cnt, ddr_log_buff + update_index, cnt);
	if (!flush_cnt)
		flush_cnt = 1;

	is_new_data_available = 1;
	mutex_unlock(&logbuff_mutex);

	/* wake up reader */
	hwlog_debug("%s wakeup\n", __func__);
	wake_up_interruptible(&sensorhub_log_waitq);
	return 0;
}

static void run_ssh_log_label_tabel_generate(void)
{
    static bool function_running = false;
    static bool function_executed = false;
    static uint8_t run_ssh_log_label_tabel_generate_times = 0;

    if (function_running) {
        hwlog_err("%s is running, cannot be called again!\n", __func__);
        return;
    }

    function_running = true;

    if (!function_executed && run_ssh_log_label_tabel_generate_times <= RUN_SSH_LOG_LABEL_GENERATE_MAX) {
        int res = ssh_log_label_tabel_generate();
        hwlog_warn("ssh_log_label_tabel_generate return %d\n", res);
        if (res == 0) {
            function_executed = true;
            function_running = false;
            return;
        }
        hwlog_warn("run_ssh_log_label_tabel_generate_times: %d\n", ++run_ssh_log_label_tabel_generate_times);
    }

    function_running = false;
}

static int logbuff_flush_callback(const struct pkt_header *head)
{
	/* sensorhub has flush tcm log buff
	 * we need update logbuff global vars and flush it to file system
	 */
	uint32_t ddr_log_buf_size;
	uint32_t ddr_log_buff_copy_size;
	log_buff_req_t *pkt = (log_buff_req_t *) head;
	uint32_t flush_head = (pkt->index * LOG_BUFF_SIZE);
	uint32_t flush_size;
	int remain;
	int timeout_cnt = 100;
#ifdef CONFIG_SECURITY_HEADER_FILE_REPLACE
	ddr_log_buf_size = dts_ddr_size_get(DTSI_DDR_LOG_BUFF_ADDR_AP);
	/* 3 / 4:ddr buffer water mark */
	ddr_log_buff_copy_size = (((ddr_log_buf_size / LOG_BUFF_SIZE) * 3 / 4) * LOG_BUFF_SIZE);
#else
	ddr_log_buff_copy_size = DDR_LOG_BUFF_COPY_SIZE;
	ddr_log_buf_size = DDR_LOG_BUFF_SIZE;
#endif
	flush_size = (flush_head > sensorhub_log_buf_rear) ?
		(flush_head - sensorhub_log_buf_rear) :
		(ddr_log_buf_size - (sensorhub_log_buf_rear - flush_head));
	hwlog_debug("[%s] index: %d\n", __func__, pkt->index);
	/* wait reader till we can update the head */
	while (sensorhub_log_buff_left() >
		(ddr_log_buff_copy_size - flush_size) && timeout_cnt--)
		msleep(1);
	if (timeout_cnt < 0)
		hwlog_warn("%s timeout, some log will lost", __func__);

	/* get rotate log buff index */
	mutex_lock(&logbuff_mutex);
	remain = ddr_log_buf_size - sensorhub_log_buf_rear;
	if (remain < flush_size) {
		memcpy_s(local_log_buff, flush_size - remain, ddr_log_buff, flush_size - remain);
		flush_size = remain;
	}

	memcpy_s(local_log_buff + sensorhub_log_buf_rear, flush_size,
		ddr_log_buff + sensorhub_log_buf_rear,
		flush_size);
	print_stat(6); /* stat 6 */
	update_local_buff_index(flush_head);
	print_stat(7); /* stat 7 */
	if (!flush_cnt)
		flush_cnt = 1;

	is_new_data_available = 1;
	mutex_unlock(&logbuff_mutex);

	/* wake up reader */
	wake_up_interruptible(&sensorhub_log_waitq);
	is_flush_complete = 1;
	wake_up_interruptible(&sensorhub_log_flush_waitq);

	run_ssh_log_label_tabel_generate();

	return 0;
}

static void __manual_flush(struct pkt_header *pkt, int size)
{
	struct write_info winfo;

	hwlog_debug("flush sensorhub log buff\n");
	/* do log flush */
	mutex_lock(&logbuff_flush_mutex);
	winfo.tag = pkt->tag;
	winfo.cmd = CMD_LOG_BUFF_FLUSH;
	winfo.wr_len = pkt->length;
	winfo.wr_buf = NULL;
	write_customize_cmd(&winfo, NULL, true);
	if (!wait_event_interruptible_timeout(
		sensorhub_log_flush_waitq,
		is_flush_complete != 0, FLUSH_TIMEOUT))
		hwlog_err("%s no response", __func__);
	hwlog_debug("flush sensorhub log buff done\n");
	is_flush_complete = 0;
	mutex_unlock(&logbuff_flush_mutex);
}

void dmd_manuflush(void)
{
	struct pkt_header pkt = {
		.tag = TAG_LOG_BUFF,
		.resp = NO_RESP,
		.length = 0
	};
	__manual_flush(&pkt, sizeof(pkt));
}

static ssize_t logbuff_config_set(
	struct device *dev,
	struct device_attribute *attr,
	const char *buf,
	size_t count)
{
	struct write_info winfo;
	struct pkt_header pkt = {
		.tag = TAG_LOG_BUFF,
		.resp = NO_RESP,
		.length = 0
	};
	winfo.tag = TAG_LOG_BUFF,
	winfo.wr_len = 0;
	winfo.wr_buf = NULL;

	if (buf[0] == CONFIG_FLUSH) {
		__manual_flush(&pkt, sizeof(pkt));
	} else if (buf[0] == CONFIG_SERIAL) {
		hwlog_info("sensorhub log use serial port\n");
		winfo.cmd = CMD_LOG_SER_REQ;
		write_customize_cmd(&winfo, NULL, true);
		log_method = LOG_SERIAL;
	} else if (buf[0] == CONFIG_BUFF) {
		hwlog_info("sensorhub log use log buff\n");
		winfo.cmd = CMD_LOG_USEBUF_REQ;
		write_customize_cmd(&winfo, NULL, true);
		log_method = LOG_BUFF;
	} else {
		hwlog_err("%s wrong input, \'1\' for flush \'2\' for serial \'3\' for buff\n", __func__);
		return -EINVAL;
	}
	return count;
}

static ssize_t logbuff_config_show(
	struct device *dev, struct device_attribute *attr, char *buf)
{
	size_t bufsize = sizeof("serial") + SPACESIZE;
	return sprintf_s(
		buf, bufsize, "%s\n", (log_method == LOG_SERIAL) ? "serial" : "buff");
}

static ssize_t logbuff_flush_show(
	struct device *dev, struct device_attribute *attr, char *buf)
{
	struct pkt_header pkt = {
		.tag = TAG_LOG_BUFF,
		.resp = NO_RESP,
		.length = 0
	};

	/* do flush here */
	__manual_flush(&pkt, sizeof(pkt));
	size_t bufsize = sizeof("0\n") + SPACESIZE;
	return sprintf_s(buf, bufsize, "0\n");
}

static DEVICE_ATTR(logbuff_config, S_IWUSR | S_IRUGO,
	logbuff_config_show, logbuff_config_set);

static DEVICE_ATTR(logbuff_flush, S_IRUGO, logbuff_flush_show, NULL);

static struct platform_device sensorhub_logbuff = {
	.name = "huawei_sensorhub_logbuff",
	.id = -1,
};

void reset_logbuff(void)
{
	sensorhub_log_r = 0;
	sensorhub_log_buf_head = 0;
	sensorhub_log_buf_rear = 0;
	flush_cnt = 0;
	unsigned int ddr_log_buf_size;
	/* write the head to ddr config block */
	struct log_config *log = iomcu_config_get(IOMCU_CONFIG_USR_LOG, sizeof(struct log_config));
	log->log_buff_cb_backup.mutex = 0;
#ifdef CONFIG_SECURITY_HEADER_FILE_REPLACE
	ddr_log_buf_size = dts_ddr_size_get(DTSI_DDR_LOG_BUFF_ADDR_AP);
#else
	ddr_log_buf_size = DDR_LOG_BUFF_SIZE;
#endif
	if (local_log_buff && ddr_log_buff) {
		memset_s(local_log_buff, ddr_log_buf_size, 0, ddr_log_buf_size);
		memset_s(ddr_log_buff, ddr_log_buf_size, 0, ddr_log_buf_size);
	}
}

void emg_flush_logbuff(void)
{
	log_buff_req_t pkt;

	pkt.index = sensorhub_log_buf_rear / LOG_BUFF_SIZE;
	hwlog_info("[%s] update head %x\n", __func__, pkt.index);
	if (!inited) {
		hwlog_err("logbuff not inited!\n");
		return;
	}
	/* notify userspace */
	logbuff_full_callback((const struct pkt_header *)&pkt);
	msleep(100); // sleep 100 ticks
}

static struct delayed_work sensorhub_logbuff_off_check_work;

static void  sensorhub_logbuff_off_check(struct work_struct *work)
{
	int level = NO_LOG_DEFAULT_LEVEL;

	hwlog_info("%s\n", __func__);
#ifndef CONFIG_LIBLINUX
	if (!is_opened)
		// set log level to emg
		set_log_level(TAG_LOG_BUFF, &level, 1);
#else
	if(ksys_access(DEFAULT_SENSORHUB_LOG_PATH, 0) == 0 || ksys_access(DEFAULT_SENSORHUB_LOG_PATH1, 0) == 0) {
		hwlog_info("%s: sensorhub-log exist, log version\n", __func__);
	} else {
		hwlog_info("%s: sensorhub-log not exist, nolog version\n", __func__);
		level = ERR_LEVEL;
		set_log_level(TAG_LOG_BUFF, &level, 1);
	}
#endif
}

int logbuff_dentry_init(void)
{
	logbuff_dentry = proc_create("sensorhub_logbuff", S_IRUSR | S_IRGRP,
		NULL, &sensorhub_logbuff_operations);
	if (!logbuff_dentry) {
		hwlog_err("%s failed to create logbuff_dentry\n", __func__);
		return -1;
	}
	return 0;
}

int logbuff_device_register(void)
{
	int ret;

	ret = platform_device_register(&sensorhub_logbuff);
	if (ret) {
		hwlog_err("%s: platform_device_register failed, ret:%d.\n",
			__func__, ret);
		return -1;
	}
	return 0;
}

int creat_logbuff_config_file(void)
{
	int ret;

	ret = device_create_file(&sensorhub_logbuff.dev,
		&dev_attr_logbuff_config);
	if (ret) {
		hwlog_err("%s: create %s file failed, ret:%d.\n",
			__func__, "dev_attr_logbuff_config", ret);
		return -1;
	}
	return 0;
}

int creat_logbuff_flush_file(void)
{
	int ret;

	ret = device_create_file(&sensorhub_logbuff.dev,
		&dev_attr_logbuff_flush);
	if (ret) {
		hwlog_err("%s: create %s file failed, ret:%d.\n",
			__func__, "dev_attr_logbuff_flush", ret);
		return -1;
	}
	return 0;
}

int logbuff_register_alert_notifier(void)
{
	int ret;

	ret = register_mcu_event_notifier(TAG_LOG_BUFF,
		CMD_LOG_BUFF_ALERT, logbuff_full_callback);
	if (ret) {
		hwlog_err("%s failed register notifier CMD_LOG_BUFF_ALERT:ret %d\n",
			__func__, ret);
		return -1;
	}
	return 0;
}

int logbuff_register_event_notifier(void)
{
	int ret;

	ret = register_mcu_event_notifier(TAG_LOG_BUFF,
		CMD_LOG_BUFF_FLUSHP, logbuff_flush_callback);
	if (ret) {
		hwlog_err("%s failed register notifier CMD_LOG_BUFF_FLUSH:ret %d\n",
			__func__, ret);
		return -1;
	}
	return 0;
}

int remap_ddr_log_buff(void)
{
	unsigned int ddr_log_buf_base_addr;
	unsigned int ddr_log_buf_size;
#ifdef CONFIG_SECURITY_HEADER_FILE_REPLACE
	ddr_log_buf_size = dts_ddr_size_get(DTSI_DDR_LOG_BUFF_ADDR_AP);
	ddr_log_buf_base_addr = dts_ddr_addr_get(DTSI_DDR_LOG_BUFF_ADDR_AP);
#else
	ddr_log_buf_size = DDR_LOG_BUFF_SIZE;
	ddr_log_buf_base_addr = DDR_LOG_BUFF_ADDR_AP;
#endif
	ddr_log_buff = (uint8_t *)ioremap_wc(ddr_log_buf_base_addr, ddr_log_buf_size);
	if (!ddr_log_buff) {
		hwlog_err("%s failed remap log buff\n", __func__);
		return -1;
	}
	return 0;
}

int mloc_local_log_buff(void)
{
	unsigned int ddr_log_buf_size;
#ifdef CONFIG_SECURITY_HEADER_FILE_REPLACE
	ddr_log_buf_size = dts_ddr_size_get(DTSI_DDR_LOG_BUFF_ADDR_AP);
#else
	ddr_log_buf_size = DDR_LOG_BUFF_SIZE;
#endif
	local_log_buff = (uint8_t *) vmalloc(ddr_log_buf_size);
	if (!local_log_buff) {
		hwlog_err("%s failed to malloc\n", __func__);
		return -1;
	}
	return 0;
}

static void logbuff_init_success(void)
{
	unsigned int ddr_log_buf_size;
#ifdef CONFIG_SECURITY_HEADER_FILE_REPLACE
	ddr_log_buf_size = dts_ddr_size_get(DTSI_DDR_LOG_BUFF_ADDR_AP);
#else
	ddr_log_buf_size = DDR_LOG_BUFF_SIZE;
#endif
	memset_s(local_log_buff, ddr_log_buf_size, 0, ddr_log_buf_size);
	mutex_init(&logbuff_mutex);
	mutex_init(&logbuff_open_mutex);
	mutex_init(&logbuff_flush_mutex);
	inited = true;
	INIT_DELAYED_WORK(&sensorhub_logbuff_off_check_work,
		sensorhub_logbuff_off_check);
	queue_delayed_work(system_freezable_wq,
		&sensorhub_logbuff_off_check_work, 5 * 60 * HZ); // delay 300 Hz
	hwlog_info("[%s] done\n", __func__);
}

#ifdef CONFIG_INPUTHUB_30
static int shb_logbuff_recovery_notifier(struct notifier_block *nb,
		unsigned long foo, void *bar)
{
	/* prevent access the emmc now: */
	switch (foo) {
	case IOM3_RECOVERY_START:
		emg_flush_logbuff();
		break;

	case IOM3_RECOVERY_MINISYS:
		reset_logbuff();
		break;

	case IOM3_RECOVERY_3RD_DOING:
	case IOM3_RECOVERY_FAILED:
	case IOM3_RECOVERY_IDLE:
	case IOM3_RECOVERY_DOING:
		break;
	default:
		hwlog_err("%s -unknow state %ld\n", __func__, foo);
		break;
	}
	return 0;
}

static struct notifier_block shb_logbuff_recovery_notify = {
	.notifier_call = shb_logbuff_recovery_notifier,
	.priority = -1,
};
#endif

static int sensorhub_logbuff_init(void)
{
	if (is_sensorhub_disabled())
		return -1;
	hwlog_info("[%s]\n", __func__);
	if (!get_sensor_mcu_mode()) {
		hwlog_err("%s :mcu boot fail, logbuff init err\n", __func__);
		return -1;
	}
	if (logbuff_dentry_init())
		goto PROC_ERR;
	if (logbuff_device_register())
		goto REGISTER_ERR;
	if (creat_logbuff_config_file())
		goto SYSFS_ERR_1;
	if (creat_logbuff_flush_file())
		goto SYSFS_ERR_2;
	if (logbuff_register_alert_notifier())
		goto NOTIFY_ERR;
	if (logbuff_register_event_notifier())
		goto NOTIFY_ERR_1;
	if (remap_ddr_log_buff())
		goto REMAP_ERR;
	if (mloc_local_log_buff())
		goto MALLOC_ERR;
	logbuff_init_success();

#ifdef CONFIG_INPUTHUB_30
	register_iom3_recovery_notifier(REC_USR_LOG, &shb_logbuff_recovery_notify);
#endif
	return 0;
MALLOC_ERR:
	iounmap(ddr_log_buff);
REMAP_ERR:
	unregister_mcu_event_notifier(TAG_LOG_BUFF, CMD_LOG_BUFF_FLUSHP, logbuff_flush_callback);
NOTIFY_ERR_1:
	unregister_mcu_event_notifier(TAG_LOG_BUFF, CMD_LOG_BUFF_ALERT, logbuff_full_callback);
NOTIFY_ERR:
	device_remove_file(&sensorhub_logbuff.dev, &dev_attr_logbuff_flush);
SYSFS_ERR_2:
	device_remove_file(&sensorhub_logbuff.dev, &dev_attr_logbuff_config);
SYSFS_ERR_1:
	platform_device_unregister(&sensorhub_logbuff);
REGISTER_ERR:
	proc_remove(logbuff_dentry);
PROC_ERR:
	return -1;
}

static void sensorhub_logbuff_exit(void)
{
	iounmap(ddr_log_buff);
	ddr_log_buff = NULL;
	vfree(local_log_buff);
	local_log_buff = NULL;

#ifdef CONFIG_INPUTHUB_30
	unregister_iom3_recovery_notifier(REC_USR_LOG, &shb_logbuff_recovery_notify);
#endif

	unregister_mcu_event_notifier(
		TAG_LOG_BUFF, CMD_LOG_BUFF_FLUSHP, logbuff_flush_callback);
	unregister_mcu_event_notifier(
		TAG_LOG_BUFF, CMD_LOG_BUFF_ALERT, logbuff_full_callback);
	device_remove_file(&sensorhub_logbuff.dev, &dev_attr_logbuff_flush);
	device_remove_file(&sensorhub_logbuff.dev, &dev_attr_logbuff_config);
	platform_device_unregister(&sensorhub_logbuff);
	proc_remove(logbuff_dentry);
}

late_initcall_sync(sensorhub_logbuff_init);
module_exit(sensorhub_logbuff_exit);

MODULE_LICENSE("GPL");
