...
static int double_host_backup[TAG_SENSOR_END][HOST_MAX];
#ifdef CONFIG_SUPPORT_EXTEND_TAGS
int double_host_extend_backup[EXTEND_TAGS_NUM][HOST_MAX];
#endif
...
{
	if (memset_s(&ipc_debug_info, sizeof(ipc_debug_info), 0, sizeof(ipc_debug_info)))
		hwlog_err("clean_ipc_debug_info memset failed\n");
}
...
#ifdef CONFIG_SYS_PM_ULSR
static atomic_t is_ulsr_sensor_status_back = ATOMIC_INIT(0);
int get_ulsr_sensor_status(void)
{
	return atomic_read(&is_ulsr_sensor_status_back);
}

void set_ulsr_sensor_status(int flag)
{
	atomic_set(&is_ulsr_sensor_status_back, flag);
}
#endif

static void inputhub_sensor_enable_double_host(int tag, bool enable)
{
	hwlog_info("tag %d : host_default: %d host_lake: %d",
		tag, double_host_backup[tag][HOST_DEFAULT], double_host_backup[tag][HOST_LAKE]);
	if (double_host_backup[tag][HOST_DEFAULT])
		inputhub_sensor_enable(tag, enable, HOST_DEFAULT);
	if (double_host_backup[tag][HOST_LAKE])
		inputhub_sensor_enable(tag, enable, HOST_LAKE);
}

#ifdef CONFIG_SUPPORT_EXTEND_TAGS
static void inputhub_sensor_enable_double_host_extend(int tag, bool enable)
{
	hwlog_info("tag %d : host_default: %d host_lake: %d", tag,
		double_host_extend_backup[tag - TAG_EXTEND_BEGIN][HOST_DEFAULT],
		double_host_extend_backup[tag - TAG_EXTEND_BEGIN][HOST_LAKE]);
	if (double_host_extend_backup[tag - TAG_EXTEND_BEGIN][HOST_DEFAULT])
		inputhub_sensor_enable(tag, enable, HOST_DEFAULT);
	if (double_host_extend_backup[tag - TAG_EXTEND_BEGIN][HOST_LAKE])
		inputhub_sensor_enable(tag, enable, HOST_LAKE);
}
#endif

void static double_host_backup_handler(void)
{
	int len;
	len = sizeof(int) * TAG_SENSOR_END * HOST_MAX;
	if (memset_s(&double_host_backup, len, 0, len))
		hwlog_err("double_host_backup memset failed\n");
	if (memcpy_s(&double_host_backup, len,
		&double_host, len))
		hwlog_err("double_host_backup memcpy failed\n");
#ifdef CONFIG_SUPPORT_EXTEND_TAGS
	len = sizeof(int) * EXTEND_TAGS_NUM * HOST_MAX;
	if (memset_s(&double_host_extend_backup, len, 0, len))
		hwlog_err("double_host_extend_backup memset failed\n");
	if (memcpy_s(&double_host_extend_backup, len,
		&double_host_extend, len))
		hwlog_err("double_host_extend_backup memcpy failed\n");
#endif
}
...
void disable_sensors_when_suspend(void)
{
...
#ifdef CONFIG_SYS_PM_ULSR
	if (is_ulsr())
		hwlog_info("sensorhub_ulsr_suspend++\n");
#endif

	double_host_backup_handler();
	if (memset_s(&sensor_status_backup, sizeof(sensor_status_backup), 0, sizeof(sensor_status_backup)))
		hwlog_err("sensor_status_backup memset failed\n");
	if (memcpy_s(&sensor_status_backup, sizeof(sensor_status_backup),
		&sensor_status, sizeof(sensor_status_backup)))
		hwlog_err("sensor_status_backup memcpy failed\n");
	for (tag = TAG_SENSOR_BEGIN; tag < TAG_SENSOR_END; ++tag) {
		if ((sensor_status_backup.status[tag] ||
			...
			hwlog_info("disable_sensors_when_suspend:sensor_status_backup.opened[%d]:%d, sensor_status_backup.status[%d]:%d\n",
					tag, sensor_status_backup.opened[tag], tag, sensor_status_backup.status[tag]);
			if (should_be_processed_when_sr(tag))
				inputhub_sensor_enable_double_host(tag, false);
		}
	}
#ifdef CONFIG_SUPPORT_EXTEND_TAGS
	int i = 0;
	for (i = 0; i < EXTEND_TAGS_NUM; ++i) {
		...
		if ((sensor_status_backup.status_extend[i] ||
			...
			if (should_be_processed_when_sr(tag))
				inputhub_sensor_enable_double_host_extend(tag, false);
		}
	}
#endif
	...
	for (int i = 0; i < EXTEND_TAGS_NUM; i++) {
		...
		if ((sensor_status_backup.status_extend[i] ||
			...
			if (sensor_status_backup.opened_extend[i] &&
				...
				inputhub_sensor_enable_double_host_extend(tag, true);
...
#ifdef CONFIG_SYS_PM_ULSR
static void iom3_ulsr_recovery(void)
{
	hwlog_info("sensorhub_ulsr_resume++\n");
	iom3_need_recovery(SENSORHUB_MODID, SH_FAULT_ULSR_RESUME);
	hwlog_info("sensorhub_ulsr_resume--\n");
}
#endif
...
void enable_sensors_when_resume(void)
{
	...
	for (tag = TAG_SENSOR_BEGIN;
		...
		if ((sensor_status_backup.status[tag] || sensor_status_backup.opened[tag]) &&
			!(get_hifi_supported() == 1 && (sensor_status.batch_cnt[tag] > 1))) {
			if (should_be_processed_when_sr(tag)) {
				hwlog_info("enable_sensors_when_resume:sensor_status_backup.opened[%d]:%d, sensor_status_backup.status[%d]:%d\n",
					tag, sensor_status_backup.opened[tag], tag, sensor_status_backup.status[tag]);
				if (sensor_status_backup.opened[tag] && (sensor_status.opened[tag] == 0))
					inputhub_sensor_enable_double_host(tag, true);
				if (sensor_status_backup.status[tag]) {
					delay_param.period =
...
static void key_fb_notifier_action(int enable)
{
	...
	if (enable) {
		ret = inputhub_sensor_enable(TAG_KEY, true, HOST_DEFAULT);
		if (ret) {
			hwlog_err("write open err.\n");
			return;
		}
		if (memset_s(&param, sizeof(param), 0, sizeof(param)))
			hwlog_err("key_fb_notifier_action memset failed\n");
		param.period = 20;
		ret = inputhub_sensor_setdelay(TAG_KEY, &param, HOST_DEFAULT);
		if (ret)
			hwlog_err("write interval err.\n");
		else
			key_state = 1;
	} else {
		ret = inputhub_sensor_enable(TAG_KEY, false, HOST_DEFAULT);
		if (ret)
...
static bool spi_nor_is_suspend = false;
bool get_spi_nor_suspend_status(void)
{
	return spi_nor_is_suspend;
}
#endif
	...
	unsigned long mode, void *_unused)
{
...
#ifdef CONFIG_SPI_NOR_AGENT
		spi_nor_is_suspend = true;
		hwlog_info("suspend in spi_nor_is_suspend:%d\n", spi_nor_is_suspend);
#endif
		break;
...
#ifdef CONFIG_SPI_NOR_AGENT
		spi_nor_is_suspend = false;
		hwlog_info("resume in spi_nor_is_suspend:%d\n", spi_nor_is_suspend);
#endif
		break;

	case PM_HIBERNATION_PREPARE: /* Going to hibernate */
		hwlog_info("Going to hibernate %s\n", __func__);
#ifdef CONFIG_SPI_NOR_AGENT
		disable_sensors_when_suspend();
#endif
		break;
	case PM_POST_HIBERNATION: /* Hibernation finished */
		hwlog_info("Hibernation finished %s\n", __func__);
#ifndef CONFIG_SENSORHUB_S4_RECOVERY
		reset_calibrate_data();
		enable_sensors_when_recovery_iom3();
		config_sensors_when_recovery_iom3();
		enable_motions_when_recovery_iom3();
#else
		iom3_need_recovery(SENSORHUB_MODID, SH_S4_RECOVERY);
#endif
#ifdef CONFIG_SPI_NOR_AGENT
		enable_sensors_when_resume();
#endif
	...
	case PM_RESTORE_PREPARE: /* Going to restore a saved image */
		hwlog_info("Going to restore a saved image %s\n", __func__);
		break;
	case PM_POST_RESTORE: /* Restore failed */
		hwlog_info("Restore failed %s\n", __func__);
		break;
