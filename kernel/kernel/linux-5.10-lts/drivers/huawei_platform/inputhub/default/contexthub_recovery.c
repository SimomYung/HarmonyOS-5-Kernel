...
static void get_dump_config_from_dts(void)
{
	...
	if (of_property_read_string(dump_node, "dump_dir", (const char **)&p_str)) {
		hwlog_err("%s failed to find property dump_dir use default\n",
			__func__);
		(void)memset_s(g_dump_dir, sizeof(g_dump_dir), 0, sizeof(g_dump_dir));
		(void)strncpy_s(g_dump_dir, PATH_MAXLEN, SH_DMP_DIR, PATH_MAXLEN - 1);
	}

	if (of_property_read_string(dump_node, "dump_fs", (const char **)&p_str)) {
		hwlog_err("%s failed to find property dump_dir use default\n",
			__func__);
		(void)memset_s(g_dump_fs, sizeof(g_dump_fs), 0, sizeof(g_dump_fs));
		(void)strncpy_s(g_dump_fs, PATH_MAXLEN, SH_DMP_FS, PATH_MAXLEN - 1);
	}
...
static void sh_reset(u32 modid, u32 etype, u64 coreid)
{
	...
	if (dump_vir_addr)
		(void)memset_s(dump_vir_addr, current_sh_info.log_len, 0, current_sh_info.log_len);
}
...
static int clean_rdr_memory(struct rdr_register_module_result rdr_info)
{
	...
	(void)memset_s(dump_vir_addr, rdr_info.log_len, 0, rdr_info.log_len);
	return 0;
}
...
static void fill_in_einfo(struct rdr_exception_info_s *einfo)
{
...
static int sh_rdr_register_exception(void)
{
	...
	hwlog_info("%s start\n", __func__);
	(void)memset_s(&einfo, sizeof(struct rdr_exception_info_s), 0, sizeof(struct rdr_exception_info_s));
	fill_in_einfo(&einfo);

	int cpy_lenth = (sizeof(sh_module_name) > MODULE_NAME_LEN) ? MODULE_NAME_LEN : sizeof(sh_module_name);
	(void)memcpy_s(einfo.e_from_module, cpy_lenth, sh_module_name, cpy_lenth);
	cpy_lenth = (sizeof(sh_module_desc) > STR_EXCEPTIONDESC_MAXLEN) ? STR_EXCEPTIONDESC_MAXLEN : sizeof(sh_module_desc);
	(void)memcpy_s(einfo.e_desc, cpy_lenth, sh_module_desc, cpy_lenth);
	...
	cpy_lenth = (sizeof(sh_module_user_desc) > STR_EXCEPTIONDESC_MAXLEN) ?
		STR_EXCEPTIONDESC_MAXLEN : sizeof(sh_module_user_desc);
	(void)memcpy_s(einfo.e_desc, cpy_lenth, sh_module_user_desc, cpy_lenth);
    ret = rdr_register_exception(&einfo);
	if (ret == 0) {
...
#endif
	int cpy_lenth1 = (sizeof(sh_module_fdul_desc) > STR_EXCEPTIONDESC_MAXLEN) ?
		STR_EXCEPTIONDESC_MAXLEN : sizeof(sh_module_fdul_desc);
	(void)memcpy_s(einfo.e_desc, cpy_lenth1, sh_module_fdul_desc, cpy_lenth1);
	ret = rdr_register_exception(&einfo);
	if (ret == 0) {
	...
	u32 is_append)
{
	...
	int print_result = 0;
	print_result = snprintf_s(path, PATH_MAXLEN, PATH_MAXLEN - 1, "%s/%s", logpath, filename);
	if (print_result < 0)
		hwlog_err("snprintf_s failed ,failed code:%d\n", print_result);
...
static int sh_create_dir(const char *path)
{
	...
	(void)memset_s(cur_path, CUR_PATH_LEN, 0, CUR_PATH_LEN); /* 64: path len */
	if (*path != '/')
		return -1;
	cur_path[index++] = *path++;
	while (*path != '\0') {
...
static int write_sh_dump_history(void)
{
	...
	print_result = snprintf_s(date, DATATIME_MAXLEN, DATATIME_MAXLEN - 1, "%s-%08lld", rdr_get_timestamp(),
		rdr_get_tick());
	if (print_result < 0)
		hwlog_err("snprintf_s failed ,failed code:%d\n", print_result);
	...
	print_result = 0;
	print_result = snprintf_s(local_path, PATH_MAXLEN, PATH_MAXLEN - 1, "%s/%s", g_dump_dir, "history.log");
	if (print_result < 0)
		hwlog_err("snprintf_s failed ,failed code:%d\n", print_result);
...
static int write_sh_dump_file(void)
{
	...
	print_result = 0;
	print_result = snprintf_s(path, PATH_MAXLEN, PATH_MAXLEN - 1, "sensorhub-%02d.dmp", g_dump_index);
	if (print_result < 0)
		hwlog_err("snprintf_s failed ,failed code:%d\n", print_result);
...
int iom3_rec_sys_callback(const struct pkt_header *head)
{
	...
	if (get_iom3_rec_state() == IOM3_RECOVERY_MINISYS) {
		...
		if (((pkt_sys_statuschange_req_t *)head)->status == ST_MINSYSREADY) {
			...
			ret = ar_set_gps_config();
			if (ret < 0)
				hwlog_err("ar set gps config failed, ret = %d\n", ret);
...
void enable_key_when_recovery_iom3(void)
{
	...
	(void)memset_s(&interval_param, sizeof(interval_param), 0, sizeof(interval_param));
	if (strlen(get_sensor_chip_info_address(KEY)) == 0) {
		hwlog_err("no key\n");
		return;
	}
	...
	if (get_key_state()) { /* open */
		ret = inputhub_sensor_enable_nolock(TAG_KEY, true, HOST_DEFAULT);
		if (ret) {
		...
		ret = inputhub_sensor_setdelay_nolock(TAG_KEY, &interval_param, HOST_DEFAULT);
		if (ret)
			hwlog_err("write interval cmd err\n");
	} else { /* close */
		ret = inputhub_sensor_enable_nolock(TAG_KEY, false, HOST_DEFAULT);
		if (ret < 0)
			hwlog_err("write close cmd err\n");
	}
	hwlog_info("enable_key_when_recovery_iom3 --\n");
...
static void disable_sensors_when_sysreboot(void)
{
	...
	for (tag = TAG_SENSOR_BEGIN; tag < TAG_SENSOR_END; ++tag) {
		if (sensor_status.status[tag] && tag == TAG_STEP_COUNTER) {
		...
		} else if (sensor_status.status[tag]) {
			if (double_host[tag][HOST_DEFAULT])
	...
	for (int i = 0; i < EXTEND_TAGS_NUM; i++) {
		...
		if (sensor_status.status_extend[i]) {
			if (double_host_extend[i][HOST_DEFAULT])
				inputhub_sensor_enable(tag, false, HOST_DEFAULT);
			if (double_host_extend[i][HOST_LAKE])
				inputhub_sensor_enable(tag, false, HOST_LAKE);
			msleep(50); /* 50:wait 50ms after disable sensor */
			hwlog_info("disable sensor - %d before reboot\n", tag);
		}
...
static void iom3_recovery_work(struct work_struct *work)
{
	...
	bool recovery_iom3_flag = true;
	while (recovery_iom3_flag) {
		recovery_iom3_flag = false;
...
#endif
	(void)memcpy_s(&pmsg->hd, sizeof(struct pkt_header), head, sizeof(struct pkt_header));
	(void)memcpy_s(pmsg->info, sizeof(char) * len, info, sizeof(char) * len);
...
#else
		hwlog_err("%s [clk: %5lu.%06lus], dir=%d, tag=%d, "
			"cmd=%d, resp=%d, hw_trans_mode=%d,"
			"rsv=%d, partial_order=%d, tranid=%d, app_tag=%d, length=%d, %s\n",
			__func__, (unsigned long)ts, rem_nsec / 1000, g_iom3_msg_buffer.msg[i].dir, /* 1000:accuracy */
			g_iom3_msg_buffer.msg[i].hd.tag, g_iom3_msg_buffer.msg[i].hd.cmd,
...
int recovery_init(void)
{
	...
	register_iom3_recovery_notifier(REC_USR_DUMP, &recovery_notify);
		...
		unsigned long foo, void *bar)
{
	...
	case IOM3_RECOVERY_BOXREADY:
		hwlog_info("box ready\n");
		ret = motion_set_cfg_data();
		if (ret < 0)
			hwlog_err("box_ready: motion set cfg data err ret=%d\n", ret);
		ret = sensor_set_cfg_data();
		if (ret < 0)
			hwlog_err("box_ready: sensor_set_cfg_data ret=%d\n", ret);
		break;
#endif
	default:
		hwlog_err("%s -unknow state %ld\n", __func__, foo);
