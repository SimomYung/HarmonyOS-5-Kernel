	...
	u32 is_append)
{
	...
	int print_result = 0;
	print_result = snprintf(path, PATH_MAXLEN, "%s/%s", logpath, filename);
	if (print_result < 0)
		hwlog_err("snprintf failed ,failed code:%d\n", print_result);
...
static int write_sh_dump_history(void)
{
	...
	print_result = snprintf(date, DATATIME_MAXLEN, "%s-%08lld", rdr_get_timestamp(),
		rdr_get_tick());
	if (print_result < 0)
		hwlog_err("snprintf failed ,failed code:%d\n", print_result);
	...
	print_result = 0;
	print_result = snprintf(local_path, PATH_MAXLEN, "%s/%s", g_dump_dir, "history.log");
	if (print_result < 0)
		hwlog_err("snprintf failed ,failed code:%d\n", print_result);
...
static int write_sh_dump_file(void)
{
	...
	print_result = 0;
	print_result = snprintf(path, PATH_MAXLEN, "sensorhub-%02d.dmp", g_dump_index);
	if (print_result < 0)
		hwlog_err("snprintf failed ,failed code:%d\n", print_result);
