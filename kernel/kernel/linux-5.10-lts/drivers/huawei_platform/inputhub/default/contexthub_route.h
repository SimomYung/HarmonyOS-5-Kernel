...
#define SUPPORT_HEAT_DISSIPATE 0x8
#define HEADT_DISSIPATE_SHELL_NUM 1
#define BRACKET_SHELL_NUM 2
#define TAB_SENSOR_CTL_MAX 2
...
#define offset_of_end_mem(struct_t, member) \
    ((unsigned long)(offsetof(struct_t, member) + sizeof(((struct_t *)0)->member)))
...
struct sensor_status {
	...
	/* record latest delay time for double host */
	struct double_host_delay delays[TAG_SENSOR_END];
	...
	/* record latest delay time for double host */
	struct double_host_delay delays_extend[TAG_SENSOR_END];
	/* record whether sensor was opened */
...
extern struct dkmd_alsc hisi_dkmd_alsc_init;
extern struct dkmd_alsc hisi_dkmd_alsc1_init;
#endif

extern int double_host[TAG_SENSOR_END][HOST_MAX];
#ifdef CONFIG_SUPPORT_EXTEND_TAGS
extern int double_host_extend[EXTEND_TAGS_NUM][HOST_MAX];
#endif
...
extern int inputhub_sensor_enable(int tag, bool enable, int host);
extern int inputhub_sensor_enable_stepcounter(bool enable,
		type_step_counter_t steptype, int host);
extern int inputhub_sensor_setdelay(int tag, interval_param_t *interval_param, int host);
extern int inputhub_sensor_enable_nolock(int tag, bool enable, int host);
extern int inputhub_sensor_setdelay_nolock(int tag,
		interval_param_t *interval_param, int host);
#ifndef CONFIG_INPUTHUB_30
...
enum {
	FOLD_TYPE_INVALID = 0x0, // straight phone
	FOLD_TYPE_LIKE_PALAU, // out fold along the y axis
	FOLD_TYPE_LIKE_ALTAB, // infold along the y axis
	FOLD_TYPE_LIKE_LEM, // infold along the x axis
	FOLD_TYPE_LIKE_GRL, // trifold phone
};
