...
#define AOD_HUGE_DATA_SIZE 30000
...
static bool g_displayengine_ioctl_lock_init = false;
static struct mutex g_displayengine_ioctl_lock;
...
#if (defined CONFIG_DKMD_DPU_AOD) || (defined CONFIG_DPU_FB_V346)
static int huge_data_send_to_sensorhub(int cmd, const void *buf, uint32_t length, bool is_lock)
{
	int ret = 0;
	struct write_info pkg_ap;
	struct read_info pkg_rply;
	ret = memset_s(&pkg_ap, sizeof(pkg_ap), 0, sizeof(pkg_ap));
	if (ret != EOK) {
		hwlog_err("%s memset_s failed, ret = %d\n", __func__, ret);
		return ret;
	}
    // since brightness lut and alpha lut is too huge and will recovery failed,
	// we use specific address to pass data
	uint8_t* g_vaddr = (uint8_t *)ioremap_wc(DDR_AOD_BASE_AP, DDR_AOD_SIZE);
	if (g_vaddr == NULL) {
		hwlog_err("%s cmd:%d g_vaddr is null\n", __func__, cmd);
		iounmap(g_vaddr);
		return -1;
	}
	ret = memcpy_s((void*)g_vaddr, length, buf, length);
	if (ret != EOK) {
		hwlog_err("%s memcpy fail,ret=%d\n", __func__, ret);
		iounmap(g_vaddr);
		return ret;
	}
	...
	pkg_ap.cmd = cmd;
	pkg_ap.wr_buf = buf;
    // sizeof(size)+sizeof(cmd_type)+sizeof(panel_id)
	pkg_ap.wr_len = sizeof(uint32_t) + sizeof(uint32_t) + sizeof(int32_t);
	hwlog_info("%s +\n", __func__);
	ret = display_engine_send_cmd_to_sensorhub(&pkg_ap, &pkg_rply, is_lock);
	hwlog_info("%s -\n", __func__);
	iounmap(g_vaddr);
	if (ret)
		hwlog_err("%s notify fail,ret=%d\n", __func__, ret);
	return ret;
}
#endif

static int display_effect_send_to_sensorhub(int cmd, const void *buf, uint32_t length, bool is_lock)
{
	if (length > AOD_HUGE_DATA_SIZE) {
#if (defined CONFIG_DKMD_DPU_AOD) || (defined CONFIG_DPU_FB_V346)
	    return huge_data_send_to_sensorhub(cmd, buf, length, is_lock);
#endif
	}
	int ret = 0;
	struct write_info pkg_ap;
	...
	ret = display_engine_send_cmd_to_sensorhub(&pkg_ap, NULL, is_lock);
	if (ret)
...
static int display_engine_set_status_param(unsigned int cmd, const void __user *argp)
{
	...
	if (ret)
		return -EINVAL;
	ret = display_effect_send_to_sensorhub(CMD_CMN_CONFIG_REQ, g_status_data, length, true);
	return ret;
}
...
static int display_engine_set_effect_param(unsigned int cmd, const void __user *argp)
{
	...
	ret = display_effect_send_to_sensorhub(CMD_CMN_CONFIG_REQ,
		...
		g_data[head[DATA_IDX_CMD_TYPE]][head[DATA_IDX_PANEL_ID]]->size, true);
...
static long display_engine_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	hwlog_info("display_engine: enter %s\n", __func__);
	if (!g_displayengine_ioctl_lock_init) {
		hwlog_info("display_engine g_displayengine_ioctl_lock_init:%d\n", g_displayengine_ioctl_lock_init);
		return -EINVAL;
	}
	hwlog_info("display_engine cmd: %u\n", cmd);
	mutex_lock(&g_displayengine_ioctl_lock);
	switch (cmd) {
	...
	mutex_unlock(&g_displayengine_ioctl_lock);
	return ret;
}
...
static int process_de_data_from_mcu(const struct pkt_header *head)
{
	...
	if (report_t->hd.length <= 0 || report_t->hd.length > DE_MAX_MALLOC_SIZE) {
		hwlog_err("input length %u invalid, expected at least: %u, max: %u\n", report_t->hd.length,
			sizeof(pkt_display_engine_report_t), DE_MAX_MALLOC_SIZE);
		return -EINVAL;
	}
	...
	if (report_t->hd.length != g_data_size) {
		hwlog_err("%s: invalid data size des %d src %d\n", __func__, g_data_size, report_t->hd.length);
		return -EINVAL;
	}
...
static int resend_effect_when_recovery_iom3()
{
	...
	if (g_data[IOCFG_TYPE_APLHA_PARAM][0]) {
		ret = display_effect_send_to_sensorhub(
			CMD_CMN_CONFIG_REQ, g_data[IOCFG_TYPE_APLHA_PARAM][0], g_data[IOCFG_TYPE_APLHA_PARAM][0]->size, false);
...
static int __init display_engine_hub_init(void)
{
	...
	mutex_init(&g_displayengine_ioctl_lock);
	g_displayengine_ioctl_lock_init = true;
	...
	register_iom3_recovery_notifier(REC_USR_AOD, &display_engine_recovery_notify);
	hwlog_info("%s ok\n", __func__);
...
static void __exit display_engine_hub_exit(void)
{
	...
	mutex_destroy(&g_displayengine_ioctl_lock);
	hwlog_info("exit %s\n", __func__);
}
