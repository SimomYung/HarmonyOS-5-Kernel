...
#if defined(CONFIG_HUAWEI_OHOS_DSM) || defined(CONFIG_HUAWEI_DSM)
#define GYRO_CALI_DATA_IS_ERROR  -2
static void gyro_check_six_side_cali_data(int *gyro_cali_data, int tag)
{
	int i;
	int32_t update = 0;
	struct dsm_client *client = inputhub_get_shb_dclient();

	for (i = MATRIX_XX; i < ONLINE_X; ++i) {
		if (gyro_cali_data[i] != 0)
			return;
	}
#ifndef SENSOR_IN_FACTORY_MODE
	if (get_dsm_client_result(client)) {
		dsm_client_record(client, "dmd_case = %d, dmd_tag = %d", GYRO_CALI_DATA_IS_ERROR, tag);
		dsm_client_notify(client, DSM_SHB_ERR_IOM7_DETECT_FAIL);
		hwlog_info("gyro %d six cali is all zero\n", tag);
	}
#endif
}
#endif
...
#if defined(CONFIG_HUAWEI_OHOS_DSM) || defined(CONFIG_HUAWEI_DSM)
	gyro_check_six_side_cali_data(gyro2_sensor_offset, TAG_GYRO2);
#endif
...
#if defined(CONFIG_HUAWEI_OHOS_DSM) || defined(CONFIG_HUAWEI_DSM)
	gyro_check_six_side_cali_data(gyro_sensor_offset, TAG_GYRO);
#endif
...
#define I2C_ERROR_RESET_TIME_24H    (24 * 3600 * 1000000000LL) // 1h = 3600s; 1s = 10^9 ns

void sensor_i2c_nack_recovery_mechanism(const char *temp, int i2c_error_code)
{
	uint64_t total_time;
	...
	hwlog_err("i2c continuous read error times:%d,i2c_error:%d\n",
		reset_sensorhub_times, i2c_error_code);
...
int write_gyro1_sensor_offset_to_nv(const char *temp, int length)
{
	...
	if (gyro1_sensor_calibrate_data[MCU_RESET_IDX] == CONTINUES_I2C_ERROR) {
		sensor_i2c_nack_recovery_mechanism(temp,
			gyro1_sensor_calibrate_data[MCU_RESET_IDX]);
		return 0;
	}
...
static int gyro_read_nv_befor_write(const char *temp, int length)
{
	char nv_buff[GYRO_CALIDATA_NV_SIZE] = {0};

	if (read_calibrate_data_from_nv(GYRO_CALIDATA_NV_NUM,
		GYRO_CALIDATA_NV_SIZE, "GYRO")) {
		hwlog_err("read_gyro_sensor_offset_from_nv fail\n");
		return -1;
	}

	if (memcpy_s(nv_buff, sizeof(nv_buff), user_info.nv_data,
		sizeof(nv_buff)) != EOK) {
		hwlog_err("copy fail %d size:%d\n", __LINE__, sizeof(nv_buff));
		return -1;
	}
	if (memcpy_s(nv_buff, sizeof(nv_buff), temp, length) != EOK) {
		hwlog_err("copy fail %d size:%d\n", __LINE__, length);
		return -1;
	}

	if (write_calibrate_data_to_nv(GYRO_CALIDATA_NV_NUM, sizeof(nv_buff),
		"GYRO", (const char *)(&nv_buff)))
		return -1;
...
int write_gyro_sensor_offset_to_nv(const char *temp, int length)
{
	...
	ret = gyro_read_nv_befor_write(temp, length);
	if (ret == -1)
		return ret;
