...
static uint32_t g_gyro_angular_rate_sensitivity[ANGULAR_RATE_SENSITIVITY_ARR_LEN] = {0};
...
const uint32_t *get_gyro_angular_rate_sensitivity_val(void)
{
    return g_gyro_angular_rate_sensitivity;
}
...
static void read_gyro_config5_from_dts(struct device_node *dn,
	struct sensor_detect_manager *sm)
{
	struct gyro_platform_data *pf_data =
		(struct gyro_platform_data *)sm->spara;
	...
	if (!of_property_read_u32(dn, "multi_device_enable", &temp)) {
		if (of_property_read_string(dn, "compatible", (const char **)&chip_info)) {
			...
			if (strncpy_s(pf_data->gyro_extend_data, SENSOR_PLATFORM_EXTEND_DATA_SIZE, chip_info,
...
static void set_gyro_angular_rate_sensitivity_val(struct device_node *dn,
	struct sensor_detect_manager *sm)
{
	struct gyro_platform_data *pf_data =
		(struct gyro_platform_data *)sm->spara;
	...
	uint32_t temp[ANGULAR_RATE_SENSITIVITY_ARR_LEN] = {0};
	if (of_property_read_u32_array(dn, "sensitivity", temp, ANGULAR_RATE_SENSITIVITY_ARR_LEN))
		hwlog_info("%s:read %s gyro angular rate sensitivity fail\n", __func__, sm->sensor_name_str);
	else
		hwlog_info("%s:read gyro angular rate sensitivity %u %u\n", __func__, temp[0], temp[1]);
	pf_data->angular_rate_sensitivity[0] = (uint32_t)temp[0];
	pf_data->angular_rate_sensitivity[1] = (uint32_t)temp[1];
    (void)memcpy_s(g_gyro_angular_rate_sensitivity, sizeof(g_gyro_angular_rate_sensitivity),
		pf_data->angular_rate_sensitivity, sizeof(g_gyro_angular_rate_sensitivity));
    return;
}
...
#endif
	read_gyro_config5_from_dts(dn, sm);
	read_sensorlist_info(dn, sm->sensor_id);
	set_gyro_axis_data_val(sm);
	set_gyro_angular_rate_sensitivity_val(dn, sm);
}
