	...
	struct gyro_device_info *dev_info, unsigned long val)
{
	...
	int32_t i;
	if (dev_info == NULL) {
		hwlog_err("%s error, dev_info is NULL\n", __func__);
		return -1;
	}
	...
	struct gyro_device_info *dev_info, unsigned long val)
{
	...
	int32_t i;
	if (dev_info == NULL) {
		hwlog_err("%s gyro_device_info is null\n", __func__);
		return -1;
	}
	/* send calibrate command, need set delay first */
...
static int gyro2_calibrate_on_enter(struct gyro_device_info *dev_info)
{
	...
	if (sensor_status.opened[TAG_GYRO2] == 0) {
		...
		ret = inputhub_sensor_enable(TAG_GYRO2, true, HOST_DEFAULT);
		if (ret) {
	...
	if ((sensor_status.delay[TAG_GYRO2] == 0) ||
		...
		ret = inputhub_sensor_setdelay(TAG_GYRO2, &param, HOST_DEFAULT);
		if (ret) {
			dev_info->gyro_calibration_res = COMMU_FAIL;
...
static int gyro1_calibrate_on_enter(struct gyro_device_info *dev_info)
{
	...
	if (sensor_status.opened[TAG_GYRO1] == 0) {
		...
		ret = inputhub_sensor_enable(TAG_GYRO1, true, HOST_DEFAULT);
		if (ret) {
			dev_info->gyro_calibration_res = COMMU_FAIL;
			hwlog_err("send gyro open cmd(during calibrate) to mcu fail,ret=%d\n",
				ret);
	...
	if ((sensor_status.delay[TAG_GYRO1] == 0) ||
		...
		param.period = GYRO_PERIOD_MAX;
		ret = inputhub_sensor_setdelay(TAG_GYRO1, &param, HOST_DEFAULT);
		if (ret) {
			dev_info->gyro_calibration_res = COMMU_FAIL;
...
static void gyro2_calibrate_on_exit(struct gyro_device_info *dev_info)
{
	...
	if (dev_info->gyro_opened_before_calibrate == 0) {
		...
		ret = inputhub_sensor_enable(TAG_GYRO2, false, HOST_DEFAULT);
		if (ret) {
			dev_info->gyro_calibration_res = COMMU_FAIL;
...
static void gyro1_calibrate_on_exit(struct gyro_device_info *dev_info)
{
	...
	if (dev_info->gyro_opened_before_calibrate == 0) {
		dev_info->gyro_opened_before_calibrate = 1;
		hwlog_info("send gyro close cmd(during calibrate) to mcu.\n");
		ret = inputhub_sensor_enable(TAG_GYRO1, false, HOST_DEFAULT);
		if (ret) {
			dev_info->gyro_calibration_res = COMMU_FAIL;
			hwlog_err("send gyro close cmd(during calibrate) to mcu fail,ret=%d\n",
				ret);
	...
	struct gyro_device_info *dev_info, unsigned long val)
{
	...
	if (dev_info->gyro_calibration_res == COMMU_FAIL) {
		...
		if (val == 1) {
			for (i = 0; i < GYRO_CALIB_DATA_LEN; i++)
				dev_info->gyro_calib_data[i] = *(gyro_cali_data + i);
		...
		for (int i = 0;i < ARRAY_SIZE(dev_info->gyro_calib_data); i++) {
			hwlog_info("gyro calibrate success, val=%lu index=%d data=%d len=%d\n", val, i,
				dev_info->gyro_calib_data[i], read_pkg.data_length);
		}
	}
...
static int gyro_calibrate_on_enter(struct gyro_device_info *dev_info)
{
	...
	if (sensor_status.opened[TAG_GYRO] == 0) {
		...
		ret = inputhub_sensor_enable(TAG_GYRO, true, HOST_DEFAULT);
		if (ret) {
			dev_info->gyro_calibration_res = COMMU_FAIL;
			hwlog_err("send gyro open cmd(during calibrate) to mcu fail,ret=%d\n",
				ret);
	...
	if ((sensor_status.delay[TAG_GYRO] == 0) ||
		...
		param.period = GYRO_PERIOD_MAX;
		ret = inputhub_sensor_setdelay(TAG_GYRO, &param, HOST_DEFAULT);
		if (ret) {
			dev_info->gyro_calibration_res = COMMU_FAIL;
...
static void gyro_calibrate_on_exit(struct gyro_device_info *dev_info)
{
	...
	if (dev_info->gyro_opened_before_calibrate == 0) {
		dev_info->gyro_opened_before_calibrate = 1;
		hwlog_info("send gyro close cmd(during calibrate) to mcu\n");
		ret = inputhub_sensor_enable(TAG_GYRO, false, HOST_DEFAULT);
		if (ret) {
			dev_info->gyro_calibration_res = COMMU_FAIL;
			hwlog_err("send gyro close cmd(during calibrate) to mcu fail,ret=%d\n",
				ret);
