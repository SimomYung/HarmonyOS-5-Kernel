	...
	enum obj_sub_cmd subcmd, enum kb_type_t type, bool use_lock)
{
	...
	memset_s(&interval_param, sizeof(interval_param), 0, sizeof(interval_param));
...
int kernel_send_kb_report_event(unsigned int cmd, void *buffer, int size)
{
	...
	memset_s(&outreport, sizeof(outreport), 0, sizeof(outreport));

	pkg_ap.cmd = CMD_CMN_CONFIG_REQ;
	pkg_ap.tag = kb_cmd_map_tab[i].tag;
	outreport.sub_cmd = kb_cmd_map_tab[i].subcmd;
	outreport.report_len = size;
	memcpy_s(outreport.report_data, sizeof(outreport.report_data), buffer, size);
...
static int __init kbhub_init(void)
{
	...
	register_iom3_recovery_notifier(REC_USR_KB, &kb_recovery_notify);
	hwlog_info("%s ok\n", __func__);
	return 0;
}
