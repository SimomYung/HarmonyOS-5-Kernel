...
{
	struct mag_device_info *dev_info = NULL;
	static bool mag_charge_notify_open_flag = false;

	dev_info = mag_get_device_info(TAG_MAG);
	if (dev_info == NULL)
		return 0;
#if defined(CHA_MAG_DETACH) || defined(BBK_MAG_DETACH) || defined(XIAN_MAG_DETACH)
	hwlog_info("%s get charger action: %d\n", __func__, action);
	if (action < CHARGER_TYPE_MAG_NONE) {
		mag_charge_notify_open();
		mag_charge_notify_open_flag = true;
	} else if (action >= CHARGER_TYPE_MAG_NONE && mag_charge_notify_open_flag) {
		mag_charge_notify_close();
		mag_charge_notify_open_flag = false;
	}
#else
