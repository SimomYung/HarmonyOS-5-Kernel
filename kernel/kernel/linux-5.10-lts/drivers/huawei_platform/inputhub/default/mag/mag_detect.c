...
static struct compass_platform_data mag_data[MAG_DEV_COUNT_MAX] = {
	...
	.calibrate_method = 1,
	.charger_trigger = 0,
	.product_type = 0, /* default set up 0, 1:bruce+ */
	},
};
static struct mag_device_info g_mag_dev_info[MAG_DEV_COUNT_MAX];
#if defined(CHA_MAG_DETACH) || defined(BBK_MAG_DETACH) || defined(XIAN_MAG_DETACH)
	...
	struct sensor_detect_manager *sm)
{
	...
	if (sm->spara == NULL) {
		hwlog_err("%s: %s sm->spara is null\n", __func__, sm->sensor_name_str);
		return;
	}
	...
	if (of_property_read_u32(dn, "product_type", &temp))
		hwlog_err("%s:read %s mag product_type fail\n",
			__func__, sm->sensor_name_str);
	else
		pf_data->product_type = (int16_t)temp;
}
	...
	struct sensor_detect_manager *sm)
{
	...
	pf_data->hall_gpio = (uint16_t)temp;

	if (of_property_read_u32(dn, "hall1_gpio", &temp)) {
		hwlog_err("%s:read %s mag hall1_gpio fail\n",
			__func__, sm->sensor_name_str);
		temp = HALL1_STATE_GPIO;
	}
	pf_data->hall1_gpio = (uint16_t)temp;

	if (of_property_read_u32(dn, "phone_type", &temp)) {
		hwlog_err("%s:read %s mag phone_type fail\n",
			__func__, sm->sensor_name_str);
		temp = 0;
	}
	pf_data->phone_type = (uint16_t)temp;
}
#endif
