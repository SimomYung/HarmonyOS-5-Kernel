...
int inputhub_process_motion_report(const struct pkt_header *head)
{
	...
	if (motion_data[MOTION_TYPE] == MOTIONHUB_TYPE_SHAKE && camera_flag) {
		...
		hwlog_info("%s : shake msg upload to camera\n", __func__);
		sprintf_s(shake_res, sizeof(shake_res), "%d", SHAKE_RESULT);
		write_buf_to_node(shake_res, sizeof(shake_res));
	}

	if (motion_data[MOTION_TYPE] == MOTIONHUB_TYPE_SHAKE && framework_flag == false) {
		hwlog_info("%s : shake msg no upload to framework\n", __func__);
		return 0;
	}
