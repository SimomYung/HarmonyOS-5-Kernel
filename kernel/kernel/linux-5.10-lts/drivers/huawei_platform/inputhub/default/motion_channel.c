...
#define MOTION_CONFIG_LEN 16
#define SHAKE_CMD_FLAG 2
...
static bool g_camera_use_shake = false;
static bool g_framework_use_shake = false;
...
static char *motion_type_str[] = {
	...
	[MOTION_TYPE_COVER] = "cover",
	[MOTION_TYPE_CAMERA_FLIP] = "camera_flip",
	[MOTION_TYPE_PS_IN_CALL] = "ps_in_call",
	[MOTION_TYPE_POCKET_MOVE] = "pocket_move",
	[MOTION_TYPE_COLLISION] = "collision",
	[MOTION_TYPE_FALLDOWN] = "falldown",
	[MOTION_TYPE_END] = "end",
};
...
static int extend_step_counter_process(bool enable)
{
	...
	inputhub_sensor_enable_stepcounter(enable, TYPE_EXTEND, HOST_DEFAULT);
	if (enable) {
		app_config[0] = enable;
		app_config[1] = TYPE_EXTEND;

		inputhub_sensor_setdelay(TAG_STEP_COUNTER, &extend_open_param, HOST_DEFAULT);
...
static int extend_step_counter_process_nolock(bool enable)
{
	...
	if (really_do_enable_disable(get_step_ref_cnt(), enable, TYPE_EXTEND))
		inputhub_sensor_enable_nolock(TAG_STEP_COUNTER, enable, HOST_DEFAULT);
	if (enable) {
		interval_param_t extend_open_param = {
			.period = 20, /* default delay_ms */
			.batch_count = 1,
		...
		inputhub_sensor_setdelay_nolock(TAG_STEP_COUNTER,
			&extend_open_param, HOST_DEFAULT);
...
static int send_motion_cmd_internal(int tag, enum obj_cmd cmd,
	int flag, motion_type_t type, bool use_lock)
{
	uint8_t app_config[MOTION_CONFIG_LEN] = { 0 };
	interval_param_t interval_param;
	bool cmd_status = false;

	if (get_stop_auto_motion() == 1) {
		hwlog_info("%s stop_auto_motion: %d", __func__, get_stop_auto_motion());
		return 0;
	}
	...
	if (type == MOTION_TYPE_SHAKE)
		app_config[SHAKE_CMD_FLAG] = flag;
	(void)memset_s(&interval_param, sizeof(interval_param), 0, sizeof(interval_param));

	if ((type == MOTIONHUB_TYPE_HW_STEP_COUNTER) && (cmd == CMD_CMN_OPEN_REQ || cmd == CMD_CMN_CLOSE_REQ)) {
...
#ifdef CONFIG_SYS_PM_ULSR
	if (get_ulsr_sensor_status()) {
		hwlog_err("in ulsr, cmd:%d motion: %s motion_ref_cnt= %x", cmd, motion_type_str[type], motion_ref_cnt);
		return 0;
	}
#endif

	if (cmd == CMD_CMN_OPEN_REQ) { /* send open motion cmd when open first sub type */
		if (really_do_enable_disable(&motion_ref_cnt, true, type)) {
			if (use_lock) {
				...
				inputhub_sensor_setdelay_nolock(tag, &interval_param, HOST_DEFAULT);
			}
		}
		/* send config cmd to open motion type */
		send_app_config_cmd(TAG_MOTION, app_config, use_lock);
		hwlog_info("open config cmd:%d motion: %s motion_ref_cnt= %x", cmd, motion_type_str[type], motion_ref_cnt);
	} else if (cmd == CMD_CMN_CLOSE_REQ) { /* send config cmd to close motion type */
		send_app_config_cmd(TAG_MOTION, app_config, use_lock);
		hwlog_info("close config cmd:%d motion: %s motion_ref_cnt= %x", cmd, motion_type_str[type], motion_ref_cnt);
...
{
	hwlog_info("send_motion_shake_cmd type %d cmd %d flag %d\n", arg, cmd, flag);

	if (cmd == CMD_CMN_OPEN_REQ) {
		g_camera_use_shake = flag == IS_CAMERA_USE_SHAKE_FLAG ? true : g_camera_use_shake;
		update_motion_info(cmd, arg);
		return send_motion_cmd_internal(motions_cmd_map_tab[0].tag,
			motions_cmd_map_tab[0].cmd, flag,
			arg, true);
	}
	if (cmd == CMD_CMN_CLOSE_REQ) {
		g_camera_use_shake = flag == IS_CAMERA_USE_SHAKE_FLAG ? false : g_camera_use_shake;
		if (g_framework_use_shake) {
			hwlog_err("%s: shake no close, used by framework\n", __func__);
			return 0;
		}
		update_motion_info(cmd, arg);
		return send_motion_cmd_internal(motions_cmd_map_tab[1].tag,
			motions_cmd_map_tab[1].cmd, flag,
			arg, true);
	}
	hwlog_err("%s: invalid cmd = %d\n", __func__, cmd);
	return 0;
}

int get_shake_state_signs(void)
{
	return (g_camera_use_shake << 1) | g_framework_use_shake;
}

static int send_motion_cmd_except_ble_range(int motion_type, int cmd_map_idx, enum obj_cmd cmd)
{
	if (motion_type != MOTION_TYPE_SHAKE) {
		update_motion_info(motions_cmd_map_tab[cmd_map_idx].cmd, motion_type);
		return send_motion_cmd_internal(motions_cmd_map_tab[cmd_map_idx].tag,
			motions_cmd_map_tab[cmd_map_idx].cmd, motions_cmd_map_tab[cmd_map_idx].subcmd,
			motion_type, true);
	}

	if (cmd == CMD_CMN_OPEN_REQ) {
		g_framework_use_shake = true;
	}
	if (cmd == CMD_CMN_CLOSE_REQ) {
		g_framework_use_shake = false;
		if (g_camera_use_shake) {
			hwlog_err("%s: shake no close, used by camera\n", __func__);
			return 0;
		}
	}
	return send_motion_shake_cmd(cmd, motion_type, IS_FRAMEWORK_USE_SHAKE_FLAG);
}
...
static int send_motion_cmd(unsigned int cmd, unsigned long arg)
{
	...
	if (argvalue != MOTION_TYPE_BLE_RANGE)
		return send_motion_cmd_except_ble_range(argvalue, i, motions_cmd_map_tab[i].cmd);
...
static int __init motionhub_init(void)
{
	...
	register_iom3_recovery_notifier(REC_USR_MOTION, &motion_recovery_notify);
