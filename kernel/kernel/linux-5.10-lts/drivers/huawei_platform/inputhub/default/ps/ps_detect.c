...
static int get_device_index(enum sensor_detect_list sname)
{
	...
	hwlog_err("%s: get_device_index fail\n", __func__);
	return PS_DEVICE_ID_0;
}
...
static void read_ps_chip_info(enum sensor_detect_list sname)
{
	...
	if (!strncmp(chip_info, "huawei,txc-pa224",
		...
		dev_info->chip_type = PS_CHIP_TCS3701;
	} else if (!strncmp(chip_info, "huawei,ps_s001003",
		sizeof("huawei,ps_s001003"))) {
		dev_info->chip_type = PS_CHIP_TCS3718;
	} else if (!strncmp(chip_info, "huawei,ps_s006001",
		sizeof("huawei,ps_s006001"))) {
		dev_info->chip_type = PS_CHIP_SIP3623;
	} else {
		return;
	}
...
#if defined(CONFIG_INPUTHUB_PS_EXTEND_DATA)
static void read_ps_config8_from_dts(struct device_node *dn,
	struct sensor_detect_manager *sm)
{
	struct ps_platform_data *pf_data;
	u32 temp = 0;
	...
	if (of_property_read_u32(dn, "pwindows_max_threshold", &temp))
		hwlog_err("%s: read pwindows_max_threshold fail\n", __func__);
	else
		pf_data->pwindows_max_threshold = (uint16_t)temp;

	if (of_property_read_u32(dn, "pwindows_min_threshold", &temp))
		hwlog_err("%s: read pwindows_min_threshold fail\n", __func__);
	else
		pf_data->pwindows_min_threshold = (uint16_t)temp;

	if (of_property_read_u32(dn, "pwave_max_threshold", &temp))
		hwlog_err("%s: read pwave_max_threshold fail\n", __func__);
	else
		pf_data->pwave_max_threshold = (uint16_t)temp;

	if (of_property_read_u32(dn, "pwave_min_threshold", &temp))
		hwlog_err("%s: read pwave_min_threshold fail\n", __func__);
	else
		pf_data->pwave_min_threshold = (uint16_t)temp;

	if (of_property_read_u32(dn, "ps_calib_20cm_min_threshold", &temp))
		hwlog_err("%s: read ps_calib_20cm_min_threshold fail\n", __func__);
	else
		pf_data->ps_calib_20cm_min_threshold = (uint16_t)temp;

	if (of_property_read_u32(dn, "ps_calib_5cm_min_threshold", &temp))
		hwlog_err("%s: read ps_calib_5cm_min_threshold fail\n", __func__);
	else
		pf_data->ps_calib_5cm_min_threshold = (uint16_t)temp;

	if (of_property_read_u32(dn, "ps_calib_3cm_min_threshold", &temp))
		hwlog_err("%s: read ps_calib_3cm_min_threshold fail\n", __func__);
	else
		pf_data->ps_calib_3cm_min_threshold = (uint16_t)temp;

	if (of_property_read_u32(dn, "default_xtalk", &temp))
		hwlog_err("%s: read default_xtalk fail\n", __func__);
	else
		pf_data->default_xtalk = (uint16_t)temp;

	if (of_property_read_u32(dn, "offset_min_map", &temp))
		hwlog_err("%s: read offset_min_map fail\n", __func__);
	else
		pf_data->offset_min_map = (uint16_t)temp;
}
#endif
...
#if defined(CONFIG_INPUTHUB_PS_EXTEND_DATA)
	read_ps_config8_from_dts(dn, sm);
#endif
