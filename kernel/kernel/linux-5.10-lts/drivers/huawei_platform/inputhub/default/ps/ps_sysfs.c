...
#define PHONE_LEM                    7
#define DEFAULT_PS_MIN_THRESHOLD     10
#endif
...
ssize_t attr_ps1_calibrate_show(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	struct ps_device_info *dev_info = NULL;
	...
	if (dev_info == NULL)
		return -1;
	hwlog_info("attr_ps1_calibrate_show result=%d\n",
		(int32_t)dev_info->ps_calibration_res);
	return snprintf_s(buf, PAGE_SIZE, PAGE_SIZE - 1, "%d\n",
		(int32_t)dev_info->ps_calibration_res);
}
...
static int ps_calibrate_save(int tag, const void *buf, int length)
{
	struct ps_device_info *dev_info = NULL;
	...
	if (ret) {
		ret = mcu_save_calidata_to_nv(tag, temp_buf);
		if (ret != 0) {
...
static void do_ps_calibrate(int tag, unsigned long val)
{
	struct ps_device_info *dev_info = NULL;
	...
	if ((val == PS_5CM_CALIBRATE) || (val == PS_MINPDATA_MODE) ||
		...
		ps_calibrate_save(tag, ps_sensor_offset, sizeof(ps_sensor_offset));
}

#ifdef SENSOR_DATA_ACQUISITION
static void init_threshold_ps(struct ps_platform_data *pf_data,
	int32_t *min_threshold_ps, int32_t *max_threshold_ps)
{
	...
	struct ps_device_info *dev_info, unsigned long val)
{
	...
	int32_t min_threshold_ps[PS_CAL_NUM] = { DEFAULT_PS_MIN_THRESHOLD, DEFAULT_PS_MIN_THRESHOLD,
		DEFAULT_PS_MIN_THRESHOLD, -pf_data->offset_min, 0, 0, 0, -pf_data->offset_min };
	...
	init_threshold_ps(pf_data, min_threshold_ps, max_threshold_ps);
	...
	if (val == PS_5CM_CALIBRATE || val == PS_SCREEN_ON_XTALK_CALIBRATE ||
		(pf_data->oily_count_size != PHONE_LEM && val == PS_XTALK_CALIBRATE))
		return;
...
{
	return attr_ps_calibrate_write_by_tag(dev, TAG_PS, attr, buf, count);
}

ssize_t attr_ps1_calibrate_write(struct device *dev,
	struct device_attribute *attr, const char *buf, size_t count)
{
	return attr_ps_calibrate_write_by_tag(dev, TAG_PS1, attr, buf, count);
}
ssize_t attr_ps_calibrate_write_by_tag(struct device *dev, int tag,
	struct device_attribute *attr, const char *buf, size_t count)
{
	struct ps_platform_data *pf_data = NULL;
	struct ps_device_info *dev_info = NULL;
	...
	pf_data = ps_get_platform_data(tag);
	dev_info = ps_get_device_info(tag);
