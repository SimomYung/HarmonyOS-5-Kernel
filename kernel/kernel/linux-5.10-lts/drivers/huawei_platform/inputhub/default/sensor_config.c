...
__attribute__((weak)) int hisi_nve_direct_access(
	struct hisi_nve_info_user *user_info_in)
{
	if (!user_info_in)
		hwlog_info("%s: user_info_in is null\n", __func__);
	...
	const char *nv_name)
{
	...
	if (strncpy_s(user_info.nv_name, sizeof(user_info.nv_name),
		nv_name, sizeof(user_info.nv_name)))
		hwlog_err("nv_cali data cpy err\n");
	...
	const char *nv_name, const char *temp)
{
	...
	(void)memset_s(&local_user_info, sizeof(local_user_info),
		0, sizeof(local_user_info));
	local_user_info.nv_operation = NV_WRITE_TAG;
	local_user_info.nv_number = nv_number;
	local_user_info.valid_size = nv_size;
	strncpy_s(local_user_info.nv_name, sizeof(local_user_info.nv_name), nv_name,
	...
	unsigned int len =
		...
		sizeof(local_user_info.nv_data) : local_user_info.valid_size;
	if (memcpy_s(local_user_info.nv_data, len, temp, len))
		hwlog_err("local_user_info cpy err\n");
	...
	int length, bool is_recovery)
{
	...
	if (memcpy_s(cpkt.para, length, data, length))
		hwlog_err("calibrate_data cpy err\n");
...
int mcu_save_calidata_to_nv(int tag, const int *para)
{
	...
	if (tag == TAG_ALS || tag == TAG_ALS1 || tag == TAG_ALS2) {
		...
		(void)memcpy_s(spkt.para, sizeof(val), &val, sizeof(val));
	} else {
		(void)memcpy_s(spkt.para, MAX_PS_CALIBRATE_DATA_LENGTH,
			para, MAX_PS_CALIBRATE_DATA_LENGTH);
	}
	ret = write_customize_cmd(&pkg_ap, &pkg_mcu, true);
	if (ret) {
...
int send_tof_calibrate_data_to_mcu(void)
{
	...
	if (memcpy_s(tof_sensor_offset, sizeof(tof_sensor_offset),
		user_info.nv_data, sizeof(tof_sensor_offset)))
		hwlog_err("nv_data cpy err\n");
	...
	unsigned int len =
		...
		sizeof(tof_sensor_calibrate_data) : TOF_CALIDATA_NV_SIZE;
	if (memcpy_s(&tof_sensor_calibrate_data, len, tof_sensor_offset, len))
		hwlog_err("tof_calibrate_data cpy err\n");
...
int send_handpress_calibrate_data_to_mcu(void)
{
	...
	if (memcpy_s(hp_offset, HANDPRESS_CALIDATA_NV_SIZE,
		user_info.nv_data, HANDPRESS_CALIDATA_NV_SIZE))
		hwlog_err("hp_offset cpy err\n");

	unsigned int len =
		(sizeof(handpress_calibrate_data) < sizeof(hp_offset)) ?
		sizeof(handpress_calibrate_data) : sizeof(hp_offset);
	if (memcpy_s(&handpress_calibrate_data, len, hp_offset, len))
		hwlog_err("handpress_calibrate_data cpy err\n");
	...
	uint32_t tx_len, uint8_t *rx_out, uint32_t rx_len)
{
	...
	if (real_tx_len && tx)
		(void)memcpy_s((void *)pkt_combo_trans->tx, (unsigned long)real_tx_len,
			(void *)tx, (unsigned long)real_tx_len);
	...
	if (ret) {
		...
		if (pkg_mcu.errno != 0) {
			...
			if (rx_out && rx_len)
				(void)memcpy_s((void *)rx_out, (unsigned long)rx_len,
					(void *)pkg_mcu.data, (unsigned long)rx_len);
	...
	unsigned char *dest, size_t max_size, int flag)
{
	...
	if (flag == EXTEND_DATA_TYPE_IN_DTS_BYTE) {
		...
		for (i = 0; i < len; i++) {
			(void)memcpy_s(pdest, EXTEND_DATA_MAX_LEN, pbuf, 2); /* copy length is 2 */
			pdest += 2;
			pbuf++;
		}
	} else if (flag == EXTEND_DATA_TYPE_IN_DTS_WORD) {
		if (memcpy_s(dest, len * sizeof(int), buf, len * sizeof(int)))
			hwlog_err("fill_extend cpy err\n");
	}
...
int huawei_set_key_backlight(void *param_t)
{
	...
	if (memcpy_s(cpkt.para, sizeof(key_brightness),
		&key_brightness, sizeof(key_brightness)))
		hwlog_err("key_brightness cpy err\n");
...
static int light_sensor_update_fastboot_info(void)
{
	...
	if (memcpy_s(tmp, tmp_len, pstr, tmp_len))
		hwlog_err("light_data cpy err\n");
	tmp[dstr - pstr] = '\0';
