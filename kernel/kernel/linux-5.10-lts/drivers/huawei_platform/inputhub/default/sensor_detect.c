...
#define SENSOR_PHONE_VERSION_MAX_LEN 28
...
static uint32_t g_phone_fold_type = 0;  // 0:invalid 1:palau 2:altaB 3:lem 4:GRL
static uint32_t g_ar_not_support_gps = 0;
...
uint32_t get_phone_fold_type(void)
{
	return g_phone_fold_type;
}
...
static int read_rpc_data_from_dts(struct device_node *dn)
{
	...
	memset_s(&rpc_data, sizeof(rpc_data), 0, sizeof(rpc_data));
	...
	memset_s(wia, sizeof(wia), 0, sizeof(wia));
...
static int get_ar_config_from_dts(void)
{
	unsigned int i = 0;
	struct device_node *sensorhub_node = NULL;
	...
	if (!of_property_read_u32(sensorhub_node, "ar_not_support_gps", &i)) {
		g_ar_not_support_gps = (uint32_t)i;
		hwlog_info("sensor get ar_not_support_gps: %u\n", g_ar_not_support_gps);
	}
...
static int get_sensors_id_from_dts(void)
{
	...
	if (get_ar_config_from_dts())
		hwlog_err("get ar config from dts fail\n");
...
int send_fileid_to_mcu(void)
{
	...
	memset_s(&dynload_req, sizeof(pkt_sys_dynload_req_t), 0, sizeof(pkt_sys_dynload_req_t));
...
static void parse_tp_ud_algo_conf(struct tp_ud_platform_data *tp_data_config)
{
	...
	if (!dn) {
		...
		memset_s(&tp_data_config->algo_conf, sizeof(struct tp_ud_algo_config), 0,
	...
	struct tp_ud_platform_data *tp_data_config)
{
	...
	if (of_property_read_u32(dn, "fw_addr_7", &temp)) {
		...
		hwlog_err("%s:read fw_addr_7 = %d\n", __func__, tp_data_config->fw_addr_7);
	}
}
	...
	struct tp_ud_platform_data *tp_data_config)
{
	...
	if (of_property_read_u32(dn, "support_head_down_lowpower", &temp)) {
		tp_data_config->support_head_down_lowpower = 0;
	} else {
		tp_data_config->support_head_down_lowpower = (uint16_t)temp;
		hwlog_info("%s:read support_head_down_lowpower = %u\n",
			__func__, tp_data_config->support_head_down_lowpower);
	}
}
...
static void parse_thp_algo_conf(enum sensor_detect_list sensor_id)
{
	...
	if (!dn) {
		hwlog_err("%s:no config, skip\n", __func__);
		memset_s(&thp_config->algo_conf, sizeof(struct tp_ud_algo_config), 0, sizeof(struct tp_ud_algo_config));
		return;
	}
...
static void read_thp_features_config_from_dts(struct device_node *dn, struct thp_platform_data *thp_config)
{
	...
	thp_config->thp_shb_timeout_hard_reset_delay = (uint16_t)tp_parse_property_u32(dn, "thp_shb_timeout_hard_reset_delay");
...
static void read_thp_bus_config_from_dts(struct device_node *dn, struct thp_platform_data *thp_config)
{
	...
	thp_config->multi_spi = tp_parse_property_u32(dn, "multi_spi");
}
...
int get_combo_prop(struct device_node *dn, struct detect_word *p_det_wd)
{
	int ret;
	struct property *prop = NULL;
	const char *bus_type = NULL;
...
static int detect_i2c_device(struct device_node *dn, char *device_name)
{
	...
	/* 4 bytes */
	ret = memcpy_s(&device_id, sizeof(device_id), detected_device_id, 4);
	if (ret != EOK) {
	...
	struct detect_word *p_det_wd)
{
	...
	uint32_t pc_virtual_sensor = 0;

	if (!of_property_read_u32(dn, "pc_virtual_sensor", &pc_virtual_sensor)) {
		if (of_property_read_u32(dn, "bus_number", &i2c_bus_num))
			return -1;

		p_det_wd->cfg.bus_type = TAG_I2C;
		p_det_wd->cfg.bus_num = (uint8_t)i2c_bus_num;
		hwlog_info("virtual %s sensor detect success\n", sensor_manager[index].sensor_name_str);
		return 0;
	}
#endif
	...
	struct sensor_combo_cfg *p_succ_ret)
{
	...
	if (prop) {
		...
		hwlog_info("%s: combo detect bus type %d; num %d; data %d; txlen %d;"
			"tx[0] 0x%x; rxLen %d; rxmsk[0] 0x%x; n %d; rxexp[0] 0x%x",
...
static int device_detect_ex(struct device_node *dn, int index)
{
	...
	if (sensor_manager[index].sensor_id == HANDPRESS) {
		...
		return inputhub_fingerprint_sensor_detect(dn, sensor_manager, index);
	} else if (sensor_manager[index].sensor_id == FINGERPRINT_UD) {
		return inputhub_fingerprint_ud_sensor_detect(dn, sensor_manager, index);
	...
	} else if (sensor_manager[index].sensor_id == ALS) {
		ret = cam_als_detect(dn, sensor_manager, index);
		if (!ret)
			return ret;
	} else if ((sensor_manager[index].sensor_id == ALS2) || (sensor_manager[index].sensor_id == ALS1)) {
		...
		return tof_sensor_detect(dn, sensor_manager, index);
	} else if (sensor_manager[index].sensor_id == ACC) {
		hwlog_info("[%s]:acc sensor_id:%d sensor detect start\n", __func__, sensor_manager[index].sensor_id);
		acc_sensor_pre_detect(dn, sensor_manager, index);
	}
	
	ret = _device_detect(dn, index, &cfg);
	if (!ret) {
		if (memcpy_s((void *)sensor_manager[index].spara,
			sizeof(struct sensor_combo_cfg), (void *)&cfg, sizeof(struct sensor_combo_cfg)) != EOK)
			ret = RET_FAIL;
	}
	return ret;
}
...
static void light_sensor_config_rst(struct device_node *dn, int index)
{
	int i;

	if (sensor_manager[index].sensor_id != ALS &&
		sensor_manager[index].sensor_id != PS)
		return;

	if (sensor_manager[ALS].detect_result == DET_SUCC ||
		sensor_manager[PS].detect_result == DET_SUCC) {
		hwlog_err("%s: als ps detect res %d %d\n", __func__,
			sensor_manager[PS].detect_result, sensor_manager[PS].detect_result);
		return;
	}
	light_sensor_set_rst_gpio(dn);
	hwlog_err("%s: detect sensor %d\n", __func__, sensor_manager[index].sensor_id);
}
...
static int device_detect(struct device_node *dn, int index)
{
	...
	// config light sensor rst gpio
	light_sensor_config_rst(dn, index);
...
/* detect current device successful, set the flag as present */
	...
	case ACC:
		...
		break;
	case ACC1:
		set_hw_dev_flag(DEV_I2C_G_SENSOR_1);
		break;
	case ACC2:
		set_hw_dev_flag(DEV_I2C_G_SENSOR_2);
		break;
	case MAG:
	case MAG1:
	...
	case GYRO:
		...
		break;
	case GYRO1:
		set_hw_dev_flag(DEV_I2C_GYROSCOPE1);
		break;
	case GYRO2:
		set_hw_dev_flag(DEV_I2C_GYROSCOPE2);
		break;
	case ALS:
	case ALS2:
	case PS:
	case PS1:
		...
		break;
	case ALS1:
		set_hw_dev_flag(DEV_I2C_L_SENSOR_1);
		break;
	case AIRPRESS:
	...
	for (i = 0; i < SENSOR_MAX; i++) {
		if (sensor_manager[i].detect_result == DET_FAIL) {
			...
			if (total_len < DSM_MAX_IC_NAME_LEN)
				strcat_s(sensor_name, DSM_MAX_IC_NAME_LEN,
	...
	uint32_t size)
{
	...
	if (get_dsm_client_result(client)) {
...
#define ACCEL_DETECT_RESULT_IS_FAIL  -1
static void acc_detect_fail_event_report(int tag)
{
	static int need_report_flag = 1;
	struct dsm_client *client = inputhub_get_shb_dclient();

	if (need_report_flag != 1)
		return;

	if (get_dsm_client_result(client)) {
		dsm_client_record(client, "dmd_case = %d, dmd_tag %d", ACCEL_DETECT_RESULT_IS_FAIL, tag);
		dsm_client_notify(client, DSM_SHB_ERR_IOM7_DETECT_FAIL);
		need_report_flag = 0;
		hwlog_info("acc %d check detect result is fail\n", tag);
	}
}
#endif
...
static uint8_t check_detect_result(enum detect_mode mode)
{
	...
	for (i = 0; i < SENSOR_MAX; i++) {
		...
		if (result == DET_FAIL) {
			...
			if (total_len < MAX_STR_SIZE)
				strcat_ret = strcat_s(detect_result, MAX_STR_SIZE, sensor_manager[i].sensor_name_str) ||
							 strcat_s(detect_result, MAX_STR_SIZE, "  ");
			if (strcat_ret)
				hwlog_err("strcat_s err\n");
			hwlog_info("%s: %s detect fail\n", __func__, sensor_manager[i].sensor_name_str);
#if defined(CONFIG_HUAWEI_OHOS_DSM) || defined(CONFIG_HUAWEI_DSM)
			if (mode == REDETECT_LATER && sensor_manager[i].tag == TAG_ACCEL)
				acc_detect_fail_event_report(sensor_manager[i].tag);
#endif
	...
	if (detect_fail_num > 0) {
		...
		total_len += strlen(sf);
		if (total_len < MAX_STR_SIZE)
			strcat_s(detect_result, MAX_STR_SIZE, sf);

#if defined(CONFIG_HUAWEI_OHOS_DSM) || defined(CONFIG_HUAWEI_DSM)
		if (mode == BOOT_DETECT_END)
			boot_detect_fail_dsm((char *)__func__, detect_result, MAX_STR_SIZE);
#endif
	} else {
...
static uint8_t read_vendor_country_cmdline(void)
{
	char phone_version_buf[SENSOR_PHONE_VERSION_MAX_LEN] = { 0 };
	char *pstr = NULL;
	char *dstr = NULL;
	bool checklen = false;
	uint8_t phone_version = 0; // 1:demo 0:Normal
	int ret;

	pstr = strstr(saved_command_line, "vendorcountry=");
	if (!pstr) {
		hwlog_err(" No fastboot vendorcountry info\n");
		return 0;
	}
	pstr += strlen("vendorcountry=");
	dstr = strstr(pstr, " ");
	if (!dstr) {
		hwlog_err(" No find the vendorcountry end\n");
		return 0;
	}
	checklen = SENSOR_PHONE_VERSION_MAX_LEN > (unsigned long)(dstr - pstr);
	if (!checklen) {
		hwlog_err(" overrun vendorcountry\n");
		return 0;
	}
	ret = memcpy_s(phone_version_buf, sizeof(phone_version_buf),
		pstr, (unsigned long)(dstr - pstr));
	if (ret != EOK) {
		hwlog_err("phone_version_buf: memcpy_s fail!\n");
		return 0;
	}
	phone_version_buf[dstr - pstr] = '\0';

	hwlog_info("get vendorcountry:%s length:%d\n",
		phone_version_buf, (int)strlen(phone_version_buf));
	if (strstr(phone_version_buf, "demo") != NULL) {
		phone_version = DEMO_VERSION;
		hwlog_info("phone_version is demo version\n");
	} else {
		phone_version = NORMAL_VERSION;
		hwlog_info("phone_version is normal version\n");
	}
...
static int get_phone_type_send(void)
{
	...
	phone_version = read_vendor_country_cmdline();
	struct motion_config *motion = iomcu_config_get(IOMCU_CONFIG_USR_MOTION, sizeof(struct motion_config));
	motion->phone_type_info[0] = temp[0];
	motion->phone_type_info[1] = temp[1];
	motion->phone_is_demo = phone_version;
	hwlog_info("%s: phone type is %d %d phone_version:%d\n",
		__func__, motion->phone_type_info[0],
		motion->phone_type_info[1], motion->phone_is_demo);

	struct drop_config *drop = iomcu_config_get(IOMCU_CONFIG_USR_DROP, sizeof(struct drop_config));
	drop->phone_type_info[0] = temp[0];
	drop->phone_type_info[1] = temp[1];
	hwlog_info("%s: drop phone type is %d %d\n", __func__, drop->phone_type_info[0], drop->phone_type_info[1]);

	u32 phone_fold_type = 0;
	if (of_property_read_u32(sensorhub_node, "phone_fold_type", &phone_fold_type) != 0) {
		hwlog_err("read phone_fold_type from dts fail\n");
		return -1;
	}
	motion->phone_fold_type = phone_fold_type;
	hwlog_info("%s: phone fold type is %d\n", __func__, motion->phone_fold_type);
	g_phone_fold_type = phone_fold_type;

	return 0;
}

static int get_phone_version_mode(void)
{
	struct als_config *als = iomcu_config_get(IOMCU_CONFIG_USR_ALS, sizeof(struct als_config));
	struct motion_config *motion = iomcu_config_get(IOMCU_CONFIG_USR_MOTION, sizeof(struct motion_config));
	als->version_mode = 0; // target version: user mode
	motion->version_mode = 0;
#ifdef SENSOR_IN_FACTORY_MODE
	als->version_mode = 1; // target version: factory mode
	motion->version_mode = 1;
#endif
	hwlog_info("%s: is %d\n", __func__, als->version_mode);
...
static int get_hall_gpio_id_from_dts(void)
{
	...
	if (of_property_read_u32(sensorhub_node, "phone_fold_type", &temp) != 0) {
		hwlog_err("read phone_fold_type from dts fail\n");
		return -1;
	}

	if (temp == 4) { // 4: phone_fold_type is like GRL
...
int init_sensors_cfg_data_from_dts(void)
{
	...
	get_phone_version_mode(); // Obtain whether phone is user or factory version

	memset_s(&sensorlist_info, SENSOR_MAX * sizeof(struct sensorlist_info),
		0, SENSOR_MAX * sizeof(struct sensorlist_info));
...
uint32_t get_spa_manufacture(void)
{
	return spa_manufacture;
}
void send_parameter_to_mcu(enum sensor_detect_list s_id, int cmd)
{
	...
	struct write_info pkg_ap = { 0 };
	struct read_info pkg_mcu = { 0 };
	pkt_sensors_parameter_t sensor_cpkt = {0};
	char buf[50] = { 0 }; /* buf size 50 */

	pkg_ap.tag = sensor_manager[s_id].tag;
	pkg_ap.cmd = CMD_CMN_CONFIG_REQ;
	sensor_cpkt.sub_cmd = cmd;
	pkg_ap.wr_buf = &sensor_cpkt;
	pkg_ap.wr_len = sensor_manager[s_id].cfg_data_length + SUBCMD_LEN;
	if (memcpy_s(sensor_cpkt.data, sizeof(sensor_cpkt.data), sensor_manager[s_id].spara,
		sensor_manager[s_id].cfg_data_length) != EOK) {
		hwlog_err("%s: memcpy_s fail %d %d \n", __func__,
		sizeof(sensor_cpkt.data), sizeof(sensor_manager[s_id].cfg_data_length));
		return;
	}
...
static void register_priv_notifier(enum sensor_detect_list s_id)
{
	...
	case THERMOMETER:
		...
		break;
	case CAP_PROX:
		register_mcu_event_notifier(TAG_CAP_PROX, CMD_CMN_CONFIG_REQ,
			cap_prox_data_from_mcu);
		break;
	default:
		break;
	}
...
int ar_set_gps_config(void)
{
	int ret = 0;
	unsigned int sub_cmd = 0;
#ifdef CONFIG_INPUTHUB_30
	ar_hdr_t *ptr_hdr = (ar_hdr_t *)&sub_cmd;
	ptr_hdr->sub_cmd = (unsigned char)SUB_CMD_AR_NOT_SUPPORT_GPS_REQ;
#endif
	if (!g_ar_not_support_gps) {
		 hwlog_info("%s: not suport!\n", __func__);
		 return ret;
	}

#ifdef CONFIG_INPUTHUB_30
	hwlog_info("%s: send cmd to ar!\n", __func__);
	if ((get_iom3_state() == IOM3_ST_RECOVERY) ||
			(get_iomcu_power_state() == ST_SLEEP))
		ret = send_cmd_from_kernel_nolock(TAG_AR, CMD_CMN_CONFIG_REQ, sub_cmd, NULL, 0);
	else
		ret = send_cmd_from_kernel(TAG_AR, CMD_CMN_CONFIG_REQ, sub_cmd, NULL, 0);
	if (ret)
		hwlog_info("%s cmd failed, info = %d\n", __func__, ret);
#endif
	return ret;
}
	...
	for (s_id = ACC; s_id < SENSOR_MAX; s_id++) {
		if (strlen(get_sensor_chip_info_address(s_id)) != 0) {
			if (need_download_fw(sensor_manager[s_id].tag)) {
				...
				memcpy_s(cpkt.para, sizeof(val), &val, sizeof(val));
				ret = write_customize_cmd(&pkg_ap, NULL, false);
...
{
	memset_s(&s_redetect_state, sizeof(s_redetect_state), 0, sizeof(s_redetect_state));
