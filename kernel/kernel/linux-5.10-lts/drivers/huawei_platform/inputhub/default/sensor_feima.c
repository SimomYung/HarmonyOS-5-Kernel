...
#define SENSOR_DO_SELFTEST_CMD  1
#define MAX_COPY_STR        1000
...
struct sensor_ag_enable_flag {
	int tag;
	bool flag;
};

static struct sensor_ag_enable_flag target[] = {
	{TAG_ACCEL, false},
	{TAG_GYRO, false},
	{TAG_ACC1, false},
	{TAG_GYRO1, false},
...
static bool split_motion_param(char *buf, unsigned long *cmd, unsigned long *type)
{
	char *temp;
	char *p;
	temp = strtok_s(buf, " ", &p);
	if (temp == NULL) {
		hwlog_err("%s: temp1 is null\n", __func__);
		return false;
	}
	...
	struct device_attribute *attr, const char *buf, size_t size)
{
	...
	if (size > MOTION_CTRL_PAR) {
		...
		unsigned long cmd = 0;
		ret = split_motion_param(temp, &cmd, &type);
		if (ret == false)
			return size;
...
static int send_rpc_get_sleep_mode_cmd_to_mcu(unsigned long sar_info)
{
	struct write_info pkg_ap = { 0 };
	rpc_ioctl_t pkg_ioctl = { 0 };
	int ret;
	pkg_ap.tag = TAG_RPC;
	pkg_ap.cmd = CMD_CMN_CONFIG_REQ;
	pkg_ioctl.sub_cmd = SUB_CMD_RPC_GET_SLEEP_MODE_REQ;
	pkg_ioctl.sar_info = sar_info;
	pkg_ap.wr_buf = &pkg_ioctl;
	pkg_ap.wr_len = sizeof(pkg_ioctl);
...
static int send_sar_dynamic_scan_cmd_to_mcu(int val)
{
	struct write_info pkg_ap = { 0 };
	rpc_ioctl_t pkg_ioctl = { 0 };
	int ret;
	pkg_ap.tag = TAG_RPC;
	pkg_ap.cmd = CMD_CMN_CONFIG_REQ;
	pkg_ioctl.sub_cmd = SUB_CMD_RPC_SET_DYNAMIC_SCAN_REQ;
	pkg_ioctl.sar_info = val;
	pkg_ap.wr_buf = &pkg_ioctl;
	pkg_ap.wr_len = sizeof(pkg_ioctl);
	...
	struct device_attribute *attr, const char *buf, size_t size)
{
	...
	int sarinfo_bit8 = 0;
	int sarinfo_bit9 = 0;
	static int pre_sarinfo_bit8 = 0;
	static int pre_sarinfo_bit9 = 0;
	...
	rpc_sar_service_req_ex_info = (rpc_sar_service_req_ex_info & ~BIT(9)) | /* clean bit 9 */
		(1 << 9); /* set bit 9 */
	sarinfo_bit8 = (rpc_sar_service_req_ex & BIT_8_MASK) >> BIT_8;
	sarinfo_bit9 = (rpc_sar_service_req_ex & BIT_9_MASK) >> BIT_9;
	hwlog_info("store_rpc_sar_service_req_ex  rpc_get_sleep_mode_info is %lu\n", rpc_sar_service_req_ex_info);

	if (sarinfo_bit9 == 1 && sarinfo_bit9 != pre_sarinfo_bit9) {
		ret = send_rpc_get_sleep_mode_cmd_to_mcu(rpc_sar_service_req_ex_info);
		if (ret) {
			hwlog_err("store_rpc_sar_service_req_ex get sleep mode fail,ret=%d\n", ret);
		} else {
			pre_sarinfo_bit9 = sarinfo_bit9;
			hwlog_info("store_rpc_sar_service_req_ex get sleep mode succ\n");
		}
	}

	if (sarinfo_bit8 != pre_sarinfo_bit8) {
		ret = send_sar_dynamic_scan_cmd_to_mcu(sarinfo_bit8);
		if (ret) {
			hwlog_err("store_rpc_sar_service_req_ex set rpc dynamic scan fail,ret=%d\n", ret);
		} else {
			pre_sarinfo_bit8 = sarinfo_bit8;
			hwlog_info("store_rpc_sar_service_req_ex set rpc dynamic scan succ\n");
		}
	}
	...
	struct device_attribute *attr, const char *buf, size_t size)
{
	...
	CHECK_SENSOR_COOKIE(data);
	memset_s(&cpkt, sizeof(cpkt), 0, sizeof(cpkt));
	if (kstrtoul(buf, TO_DECIMALISM, &val))
		return -EINVAL;
...
static ssize_t show_get_data(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	if (wait_for_completion_interruptible(&get_data->complete) == 0) {
		...
		for (; i < mem_num; i++) {
			...
			offset += (ssize_t)sprintf_s(buf + offset, MAX_COPY_STR, "%d\t",
				get_data->data.value[i]);
		}
		offset += (ssize_t)sprintf_s(buf + offset, MAX_COPY_STR, "\n");
	}
	return offset;
}
...
#define ACC_GYRO_PERIOD_10	10
#define MAX_AG_TIMES 50
static int g_rt_times = 0;

static void enable_ag_sensor(int val, struct sensor_cookie *data)
{
	int res;
	int i;

	if (val == SENSOR_DO_SELFTEST_CMD && g_rt_times == 0 && (unsigned char)data->tag == TAG_ACCEL) {
		for (i = 0; i < ARRAYSIZE(target); ++i) {
			if (target[i].flag == false) {
				interval_param_t param = {0};
				res = inputhub_sensor_enable(target[i].tag, true, HOST_DEFAULT);
				if (res != 0) {
					hwlog_err("send open cmd(during calibrate) to mcu fail,tag=%d,res=%d\n", data->tag, res);
					continue;
				}
				param.period = ACC_GYRO_PERIOD_10;
				res = inputhub_sensor_setdelay(target[i].tag, &param, HOST_DEFAULT);
				if (res != 0) {
					hwlog_err("send set delay cmd(during calibrate) to mcu fail,tag=%d,res=%d\n", data->tag, res);
					continue;
				}
			}
			target[i].flag = true;
		}
		hwlog_info("store_selftest enable type %d\n", data->tag);
	}
	if ((unsigned char)data->tag == TAG_ACCEL)
		++g_rt_times;
}

static void judge_ag_sensor_stop_respond_or_count_reach_max(int val, struct sensor_cookie *data)
{
	int i;
	// close the corresponding device when device exception
	if (get_sensor_unchange_flag(data->tag)) {
		for (i = 0; i < ARRAYSIZE(target); ++i) {
			if ((target[i].flag == true) && (data->tag == target[i].tag)) {
				hwlog_err("%s selftest fail a+g\n", data->name);
				memcpy_s(sensor_status.selftest_result[data->tag],
					sizeof(sensor_status.selftest_result[data->tag]), "1", 2);
				inputhub_sensor_enable(data->tag, false, HOST_DEFAULT);
				target[i].flag = false;
				return;
			}
		}
	}
	// close the corresponding device when device finishes detection
	if (val == SENSOR_DO_SELFTEST_CMD && g_rt_times == MAX_AG_TIMES) {
		for (i = 0; i < ARRAYSIZE(target); ++i) {
			if (target[i].flag == true) {
				inputhub_sensor_enable(target[i].tag, false, HOST_DEFAULT);
				target[i].flag = false;
				hwlog_info("store_selftest close type %d\n", data->tag);
			}
		}
	}
}
	...
	struct device_attribute *attr, const char *buf, size_t size)
{
	...
	memcpy_s(sensor_status.selftest_result[data->tag], sizeof(sensor_status.selftest_result[data->tag]), "1", 2);
	if (kstrtoul(buf, TO_DECIMALISM, &val))
		return -EINVAL;
	hwlog_err("%s selftest code %d\n", __func__, val);
	if (val <= MAX_SELFTEST_CMD) {
		enable_ag_sensor(val, data);
		cpkt.hd.tag = (unsigned char)data->tag;
		cpkt.hd.cmd = CMD_CMN_CONFIG_REQ;
...
#ifndef CONFIG_INPUTHUB_30
		{
			...
			ret = wait_for_mcu_resp_data_after_send(&cpkt, inputhub_mcu_write_cmd(&cpkt, sizeof(cpkt)),
		...
		if (ret == 0) {
			...
			if (errno != 0) {
				...
				memcpy_s(sensor_status.selftest_result[data->tag],
					sizeof(sensor_status.selftest_result[data->tag]), "1", 2); /* src size is 2 */
			} else {
				hwlog_info("%s selftest success\n", data->name);
				/* flyhorse k : SUC-->"0", OTHERS-->"1" */
				memcpy_s(sensor_status.selftest_result[data->tag],
					sizeof(sensor_status.selftest_result[data->tag]), "0", 2); /* src size is 2 */
			}
			judge_ag_sensor_stop_respond_or_count_reach_max(val, data);
		}
	}
	return size;
}
	...
	struct device_attribute *attr, const char *buf, size_t size)
{
	...
	if ((vibrator_shake == 1) ||
		((HALL_COVERD & (unsigned int)get_hall_value()) &&
		(get_hall_sen_type() == 0))) {
	...
	struct device_attribute *attr, const char *buf, size_t size)
{
	...
	hwlog_info("%s:ois source %d, sensor_feima\n", __func__, source);
	if (ois_ctrl_lock(source) == false)
		return size;
...
static DEVICE_ATTR(sar_cover_status, 0660, show_sar_cover_status, NULL);
...
int posture_sensor_enable(void)
{
	...
	inputhub_sensor_enable(TAG_POSTURE, true, HOST_DEFAULT);

	ret = inputhub_sensor_setdelay(TAG_POSTURE, &param, HOST_DEFAULT);
	if (ret)
	...
	struct device_attribute *attr, char *buf)
{
	...
	int print_result = 0;
	print_result = snprintf_s(buf, MAX_STR_SIZE, MAX_STR_SIZE - 1, "%d\n", flag);
	if (print_result < 0)
		hwlog_err("snprintf_s failed,result%d\n", print_result);
	return flag;
}

ssize_t show_gyro_axis_data(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	uint64_t gyro_axis_data = 0;
	int print_result = 0;
	gyro_axis_data = get_gyro_axis_data_val();
	hwlog_info("gyro_axis_data_val:%llu\n", gyro_axis_data);
	print_result = snprintf_s(
		buf, MAX_STR_SIZE, MAX_STR_SIZE - 1, "%llu\n", gyro_axis_data);
	if (print_result < 0)
		hwlog_err("snprintf_s failed,result%d\n", print_result);
	return print_result;
}

ssize_t show_gyro_angular_rate_sensitivity(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	const uint32_t *gyro_angular_rate_sensitivity = NULL;
	int print_result = 0;
	gyro_angular_rate_sensitivity = get_gyro_angular_rate_sensitivity_val();
	if (gyro_angular_rate_sensitivity == NULL) {
        hwlog_err("get_gyro_angular_rate_sensitivity_val returned NULL\n");
		return 0;
    }
	hwlog_info("gyro_angular_rate_sensitivity_val:%u %u\n", gyro_angular_rate_sensitivity[0],
        gyro_angular_rate_sensitivity[1]);
	print_result = snprintf_s(
		buf, MAX_STR_SIZE, MAX_STR_SIZE - 1, "%u %u\n", gyro_angular_rate_sensitivity[0],
        gyro_angular_rate_sensitivity[1]);
	if (print_result < 0) {
		hwlog_err("snprintf_s failed,result%d\n", print_result);
		return 0;
...
static DEVICE_ATTR(gyro_axis_data, 0444, show_gyro_axis_data, NULL);
static DEVICE_ATTR(gyro_angular_rate_sensitivity, 0444, show_gyro_angular_rate_sensitivity, NULL);
...
static ssize_t attr_als_dbv_ratio_store(struct device *dev,
	struct device_attribute *attr, const char *buf, size_t count)
{
	struct sensor_cookie *data =
		(struct sensor_cookie *)dev_get_drvdata(dev);
	...
	CHECK_SENSOR_COOKIE(data);
	return als_dbv_ratio_store(data->tag, dev, attr, buf, count);
}

static ssize_t attr_als_dbv_ratio_show(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	struct sensor_cookie *data =
		(struct sensor_cookie *)dev_get_drvdata(dev);
	...
	CHECK_SENSOR_COOKIE(data);
	return als_dbv_ratio_show(data->tag, dev, attr, buf);
}

static DEVICE_ATTR(als_dbv_ratio, 0660,
	attr_als_dbv_ratio_show, attr_als_dbv_ratio_store);

static ssize_t attr_als_screen_off_and_cct_manual_store(struct device *dev,
	struct device_attribute *attr, const char *buf, size_t count)
{
	struct sensor_cookie *data =
		(struct sensor_cookie *)dev_get_drvdata(dev);
	...
	CHECK_SENSOR_COOKIE(data);
	return als_screen_off_and_cct_manual_store(data->tag, dev, attr, buf, count);
}

static ssize_t attr_als_screen_off_and_cct_manual_show(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	struct sensor_cookie *data =
		(struct sensor_cookie *)dev_get_drvdata(dev);
	...
	CHECK_SENSOR_COOKIE(data);
	return als_screen_off_and_cct_manual_show(data->tag, dev, attr, buf);
}

static DEVICE_ATTR(als_screen_off_and_cct_manual, 0660,
	attr_als_screen_off_and_cct_manual_show, attr_als_screen_off_and_cct_manual_store);

static ssize_t attr_als_manual_gamma_store(struct device *dev,
	struct device_attribute *attr, const char *buf, size_t count)
{
	struct sensor_cookie *data =
		(struct sensor_cookie *)dev_get_drvdata(dev);
	...
	CHECK_SENSOR_COOKIE(data);
	return als_manual_gamma_store(data->tag, dev, attr, buf, count);
}

static ssize_t attr_als_manual_gamma_show(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	struct sensor_cookie *data =
		(struct sensor_cookie *)dev_get_drvdata(dev);
	...
	CHECK_SENSOR_COOKIE(data);
	return als_manual_gamma_show(data->tag, dev, attr, buf);
}

static DEVICE_ATTR(als_manual_gamma, 0660,
	attr_als_manual_gamma_show, attr_als_manual_gamma_store);
...
static void send_get_offset_req_to_mcu(int tag)
{
	int ret;
	struct write_info pkg_ap = {0};
	...
	pkg_ap.tag = tag;
	pkg_ap.cmd = CMD_CMN_CONFIG_REQ;
	int buf = SUB_CMD_GET_AG_OFFSET_REQ;
	pkg_ap.wr_buf = (const void *)(&buf);
	pkg_ap.wr_len = sizeof(int);
	ret = write_customize_cmd(&pkg_ap, NULL, false);
	if (ret)
		hwlog_err("send_get_offset_req_to_mcu fail\n");
	hwlog_info("send_get_offset_req_to_mcu tag %d success\n", tag);
}

static void send_acc_req_to_mcu(int tag)
{
	struct acc_device_info *dev_info = acc_get_device_info(tag);
	enum ret_type res;
	if (dev_info == NULL)
		return;

	res = dev_info->acc_calibration_res;
	hwlog_info("get tag %d calibrate result %d\n", tag, res);
	// 当返回是写nv失败时，通过sensorhub进行写nv
	if (res == NV_FAIL) {
		send_get_offset_req_to_mcu(tag);
		dev_info->acc_calibration_res = SUC;
	}
}

static void send_gyro_req_to_mcu(int tag)
{
	struct gyro_device_info *dev_info = gyro_get_device_info(tag);
	enum ret_type res;
	if (dev_info == NULL)
		return;

	res = dev_info->gyro_calibration_res;
	hwlog_info("get tag %d calibrate result %d\n", tag, res);
	// 当返回是写nv失败时，通过sensorhub进行写nv
	if (res == NV_FAIL) {
		send_get_offset_req_to_mcu(tag);
...
static ssize_t store_calibrate_after_sale(struct device *dev,
	struct device_attribute *attr, const char *buf, size_t size)
{
	struct sensor_cookie *data = (struct sensor_cookie *)dev_get_drvdata(dev);

	CHECK_SENSOR_COOKIE(data);
	sensors_calibrate_store(data->tag, dev, attr, buf, size);

	if (data->tag == TAG_ACCEL ||
		data->tag == TAG_ACC1 || data->tag == TAG_ACC2)
		send_acc_req_to_mcu(data->tag);
	else if (data->tag == TAG_GYRO ||
		data->tag == TAG_GYRO1 || data->tag == TAG_GYRO2)
		send_gyro_req_to_mcu(data->tag);

	return size;
}

static DEVICE_ATTR(calibrate_after_sale, 0660, NULL, store_calibrate_after_sale);
...
static struct attribute *acc_sensor_attrs[] = {
	...
	&dev_attr_calibrate_after_sale.attr,
	NULL,
};
...
#endif
	&dev_attr_als_dbv_ratio.attr,

	&dev_attr_als_manual_gamma.attr,
	&dev_attr_als_screen_off_and_cct_manual.attr,
	NULL,
};
...
static struct attribute *gyro_sensor_attrs[] = {
	...
	&dev_attr_gyro_axis_data.attr,
	&dev_attr_calibrate_after_sale.attr,
	&dev_attr_gyro_angular_rate_sensitivity.attr,
	NULL,
};
...
static struct attribute *cap_prox_sensor_attrs[] = {
	...
	&dev_attr_sar_cover_status.attr,
	NULL,
};
...
{
	char temp[MOTION_CTRL_NUM] = {0};
	if (memcpy_s(temp, sizeof(temp), buf, size)) {
		hwlog_err("%s:memcpy_s failed\n", __func__);
		return -EINVAL;
	}
	unsigned long type = 0;
	...
	hwlog_info("shake cmd send cmd = %d, type = %d\n", cmd, type);

	send_motion_shake_cmd(cmd, type, IS_CAMERA_USE_SHAKE_FLAG);
	return size;
}
...
static int split_spi_param(const char *buf, struct spi_rw_info *rw_info)
{
	int ret;
	int temp[SPI_PARAMETER_COUNT] = {0, };

	ret = sscanf_s(buf, "%d %d %d %d %d %d %d %d %d %d %d", &temp[SPI_BUS_NUM_IDX], &temp[SPI_DATA_IDX_1],
		&temp[SPI_DATA_IDX_2], &temp[SPI_REG_WIDTH_IDX], &temp[SPI_REG_ADDR_IDX_1], &temp[SPI_REG_ADDR_IDX_2],
		&temp[SPI_DUMMY_EXIST_IDX], &temp[SPI_RW_DATA_LEN_IDX], &temp[SPI_RW_TAG], &temp[SPI_RW_DATA_IDX_1],
		&temp[SPI_RW_DATA_IDX_2]);
	if (ret < SPI_PARAMETER_COUNT) {
		hwlog_err("sscanf_s fail error_code=%d\n", ret);
		return ret;
	}

	rw_info->bus_num = temp[SPI_BUS_NUM_IDX];
	rw_info->ctrl.data = (temp[SPI_DATA_IDX_1] << BIT_MOVE_HIGH) + temp[SPI_DATA_IDX_2];
	rw_info->reg_width = temp[SPI_REG_WIDTH_IDX];
	rw_info->reg[0] = temp[SPI_REG_ADDR_IDX_1];
	rw_info->reg[1] = temp[SPI_REG_ADDR_IDX_2];
	rw_info->dummy_exist = temp[SPI_DUMMY_EXIST_IDX];
	rw_info->len = temp[SPI_RW_DATA_LEN_IDX];
	if (rw_info->len > SPI_DATA_LENGTH) {
		hwlog_err("len exceed %d\n", rw_info->len);
		rw_info->len = SPI_DATA_LENGTH;
	}
	rw_info->data_buf[0] = temp[SPI_RW_DATA_IDX_1];
	rw_info->data_buf[1] = temp[SPI_RW_DATA_IDX_2];
	rw_info->rw_tag = temp[SPI_RW_TAG];

	return ret;
}

static ssize_t spi_rw_store(struct device *dev, struct device_attribute *attr,
	const char *buf, size_t count)
{
	int ret;
	struct spi_rw_info rw_info;
	uint8_t spi_reg[SPI_REG_LENGTH] = {0, };

	ret = split_spi_param(buf, &rw_info);
	if (ret < SPI_PARAMETER_COUNT)
		return -EINVAL;

	spi_reg[0] = rw_info.reg[0];
	spi_reg[1] = rw_info.reg[1];
	hwlog_info("%s: rw_tag = %d\n", __func__, rw_info.rw_tag);
	if (rw_info.rw_tag == SPI_READ) {
		if (rw_info.dummy_exist)
			rw_info.len =  rw_info.len + 1;

		if (rw_info.reg_width == SPI_REG_LENGTH) {
			rw_info.reg[0] = rw_info.reg[0] | TOP_BIT_SET_1;
			ret = mcu_spi_rw(rw_info.bus_num, rw_info.ctrl,
				&rw_info.reg[0], rw_info.reg_width, &rw_info.data_buf[0], rw_info.len);
		} else {
			rw_info.reg[1] = rw_info.reg[1] | TOP_BIT_SET_1;
			ret = mcu_spi_rw(rw_info.bus_num, rw_info.ctrl,
				&rw_info.reg[1], rw_info.reg_width, &rw_info.data_buf[0], rw_info.len);
		}
	} else {
		if (rw_info.reg_width == SPI_REG_LENGTH) {
			ret = mcu_spi_rw(rw_info.bus_num, rw_info.ctrl,
				&rw_info.reg[0], rw_info.reg_width + rw_info.len, NULL, 0);
		} else {
			ret = mcu_spi_rw(rw_info.bus_num, rw_info.ctrl,
				&rw_info.reg[1], rw_info.reg_width + rw_info.len, NULL, 0);
		}
	}

	if (ret < 0)
		hwlog_err("oper %d(1/32:w 0/31:r) spi reg fail\n", rw_info.data_buf[0]);
	if (rw_info.rw_tag == SPI_READ) {
		if (rw_info.dummy_exist) {
			hwlog_info("%s: dummy_exist spi reg %x value %x\n", __func__, spi_reg[1],
				rw_info.data_buf[1]);
		} else {
			hwlog_info("%s: spi reg %x value %x\n", __func__, spi_reg[1],
				rw_info.data_buf[0]);
		}
	}
	return count;
...
static DEVICE_ATTR(spi_rw, 0664, NULL, spi_rw_store);

static struct attribute *spi_attrs[] = {
	&dev_attr_spi_rw.attr,
	NULL,
};

static const struct attribute_group spi_attrs_grp = {
	.attrs = spi_attrs,
};
