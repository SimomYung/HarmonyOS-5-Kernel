	...
	struct device_attribute *attr, const char *buf, size_t size)
{
	...
	ret = inputhub_sensor_enable(TAG_CONNECTIVITY, enable, HOST_DEFAULT);
	if (ret) {
	...
	if (enable == 1) {
		ret = inputhub_sensor_setdelay(TAG_CONNECTIVITY, &delay_param, HOST_DEFAULT);
		if (ret) {
...
static int tell_cts_test_to_mcu(int status)
{
	...
	if (status == 1) {
		inputhub_sensor_enable(TAG_AR, false, HOST_DEFAULT);
	...
	struct device_attribute *attr, const char *buf, size_t count)
{
	...
	case TAG_PS:
	case TAG_PS1:
		return attr_ps_calibrate_write_by_tag(dev, tag, attr, buf, count);
	case TAG_ALS:
	case TAG_ALS1:
	case TAG_ALS2:
