...
static spinlock_t gyro_ois_lock;
...
static int init_msg_for_enq(struct message *msg)
{
	...
	memset_s(msg, sizeof(struct message), 0, sizeof(struct message));
	...
	struct sensor_eng_cal_test sensor_test)
{
	if (!events)
		return -1;
	memset_s(events, sizeof(struct event), 0, sizeof(struct event));
	events->error_code = 0;
	events->cycle = 0;
	memcpy_s(events->station, sizeof(NA), NA, sizeof(NA));
	memcpy_s(events->device_name, sizeof(sensor_test.name), sensor_test.name, sizeof(sensor_test.name));
	memcpy_s(events->bsn, sizeof(NA), NA, sizeof(NA));
	memcpy_s(events->min_threshold, sizeof(NA), NA, sizeof(NA));
	memcpy_s(events->max_threshold, sizeof(NA), NA, sizeof(NA));
	memcpy_s(events->result, sizeof(sensor_test.result), sensor_test.result, sizeof(sensor_test.result));
	memcpy_s(events->firmware, sizeof(NA), NA, sizeof(NA));
	memcpy_s(events->description, sizeof(NA), NA, sizeof(NA));
	return 0;
}
...
int enq_msg_data_in_sensorhub_single(struct event events)
{
	...
	memcpy_s(events.firmware, sizeof(NA), NA, sizeof(NA));
	memcpy_s(events.description, sizeof(NA), NA, sizeof(NA));
	memcpy_s(&(msg->events[0]), sizeof(events), &events, sizeof(events));
	...
	struct device_attribute *attr, char *buf)
{
	...
	for (i = 0; i < DEBUG_DATA_LENGTH; i++) {
		/* len 6 enough */
		int print_result = 0;
		print_result = snprintf_s(p, MAX_STR_SIZE, 6, "0x%x,", debug_read_data_buf[i]);
		if (print_result < 0)
	...
	const char *buf, size_t count)
{
	...
	if (rw) {
		...
		/* copy 2 bytes */
		memcpy_s(i2c_rw16_data_buf, sizeof(i2c_rw16_data_buf), &buf_temp[1], 2);
	}
	return count;
}
	...
	struct device_attribute *attr, char *buf)
{
	...
	/* 8 bytes */
	int print_result = 0;
	print_result = snprintf_s(p, MAX_STR_SIZE, 8, "0x%02x%02x\n",
	...
	struct device_attribute *attr, char *buf)
{
	...
	for (i = 0; i < DEBUG_DATA_LENGTH; i++) {
		/* len 12 enough */
		print_result = snprintf_s(p, MAX_STR_SIZE, 12, "0x%x,", i2c_rw_16bit_reg_data_buf[i]);
		if (print_result < 0)
			hwlog_err("snprintf_s failed,result%d\n", print_result);
	...
	enum ret_type *rtype)
{
	...
	memcpy_s(spkt.para, sizeof(val), &val, sizeof(val));
...
int write_tof_offset_to_nv(uint8_t *temp, uint16_t length)
{
	...
	hwlog_info("write_ps_offset_to_nv temp:"
		"temp[0]=%d,temp[9]=%d,temp[19]=%d temp[27]=%d temp[29]=%d temp[44]=%d temp[45]=%d temp[46]=%d\n",
	...
	uint8_t calibrate_order)
{
	...
	memcpy_s(temp_buf, length, buf, length);
...
static DEVICE_ATTR(ps1_calibrate, 0664,
	attr_ps1_calibrate_show, attr_ps1_calibrate_write);
	...
	unsigned int responsed, bool is_subcmd)
{
	...
	memset_s(&cpkt_interval, sizeof(cpkt_interval), 0, sizeof(cpkt_interval));
	...
	if (is_subcmd) {
		...
		memcpy_s(cpkt.para, sizeof(pare), &pare, sizeof(pare));
	} else {
		pkg_ap.cmd = cmd;
...
int write_handpress_offset_to_nv(const void *offset)
{
	...
	if (!offset) {
	 ...
	 if (memcpy_s(offset2, sizeof(offset2), offset, HANDPRESS_CALIDATA_NV_SIZE))
		return -1;
	...
	if (read_calibrate_data_from_nv(HANDPRESS_CALIDATA_NV_NUM,
		...
		return -1;
	if (memcpy_s(offset2, sizeof(offset2), user_info.nv_data, sizeof(offset2)))
		return -1;
	...
	struct device_attribute *attr, const char *buf, size_t count)
{
	...
	data_len = (pkg_mcu.data_length > data_len) ? data_len :
		pkg_mcu.data_length;
	memcpy_s(hand_calidata, data_len, pkg_mcu.data, data_len);
	...
	if (val == 1) {
		...
		memcpy_s(hand_metal, data_len, hand_calidata, data_len);
...
static void store_ois_commu(long source)
{
	int ret;
	unsigned int cmd;
...
void set_hall_value(int new_value)
{
	hall_value = new_value;
}
static int g_ois_cnt = 0;

void ois_ctrl_reset_ref_cnt(void)
{
	unsigned long flags = 0;
	spin_lock_irqsave(&gyro_ois_lock, flags);
	g_ois_cnt = 0;
	hwlog_info("%s:refcnt %d\n", __func__, g_ois_cnt);
	spin_unlock_irqrestore(&gyro_ois_lock, flags);
}

bool ois_ctrl_lock(long source)
{
	unsigned long flags = 0;
	bool ret = false;

	spin_lock_irqsave(&gyro_ois_lock, flags);
	hwlog_info("%s:source, refcnt %d, %d\n", __func__, source, g_ois_cnt);
	if (source == 1) {
		if (g_ois_cnt == 0)
			ret = true;
		g_ois_cnt++;
	...
	if (source == 0) {
		if (g_ois_cnt == 1)
			ret = true;
		g_ois_cnt = ((g_ois_cnt - 1) > 0 ? (g_ois_cnt - 1) : 0);
	}
	spin_unlock_irqrestore(&gyro_ois_lock, flags);
	return ret;
}
	...
	struct device_attribute *attr, const char *buf, size_t size)
{
	...
	hwlog_info("%s:ois source %d, sensor_sysfs\n", __func__, source);
	if (source == 3)
		source = 1;
	
	if (ois_ctrl_lock(source))
		store_ois_commu(source);
	return size;
}
...
static int send_offset_cmd(uint8_t tag, uint32_t subcmd, int value)
{
	...
	memcpy_s(cpkt.para, sizeof(value), &value, sizeof(value));
	ret = write_customize_cmd(&pkg_ap, &pkg_mcu, true);
	if (ret < 0) {
