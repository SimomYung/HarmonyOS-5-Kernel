...
static time64_t vib_cold_check_timestamp;
...
int write_vibrator_calib_value_to_nv(const char *temp, uint16_t length)
{
	...
	if (memset_s(&user_info, sizeof(user_info), 0, sizeof(user_info)))
		hwlog_err("write_vibrator_calib_value_to_nv memset failed\n");
	user_info.nv_operation = NV_WRITE_TAG;
	user_info.nv_number = VIB_CALIDATA_NV_NUM;
	user_info.valid_size = VIB_CALIDATA_NV_SIZE;
	strncpy_s(user_info.nv_name, sizeof(user_info.nv_name), "VIBCAL", sizeof(user_info.nv_name));
	...
	enum ret_type *rtype)
{
	...
	if (memset_s(&pkg_ap, sizeof(pkg_ap), 0, sizeof(pkg_ap)))
		hwlog_err("vibrator_send_cali_test_cmd pkg_ap memset failed\n");
	if (memset_s(&pkg_mcu, sizeof(pkg_mcu), 0, sizeof(pkg_mcu)))
		hwlog_err("vibrator_send_cali_test_cmd pkg_mcu memset failed\n");
	pkg_ap.tag = TAG_VIBRATOR;
	pkg_ap.cmd = CMD_CMN_CONFIG_REQ;
	spkt.subcmd = SUB_CMD_SELFCALI_REQ;
	pkg_ap.wr_buf = &shd[1];
	pkg_ap.wr_len = len + SUBCMD_LEN;
	if (memcpy_s(spkt.para, len, cmd, len))
		hwlog_err("vibrator_send_cali_test_cmd memcpy failed\n");
		...
		unsigned int len, enum ret_type *rtype)
{
	...
	if (memset_s(&pkg_mcu, sizeof(pkg_mcu), 0, sizeof(pkg_mcu)))
		hwlog_err("vibrator_send_cfg_cmd pkg_mcu memset failed\n");
	if ((sub_cmd > SUB_CMD_MAX) || (!cmd_para)) {
		*rtype = COMMU_FAIL;
		return pkg_mcu;
	}
	if (memset_s(&pkg_ap, sizeof(pkg_ap), 0, sizeof(pkg_ap)))
		hwlog_err("vibrator_send_cfg_cmd pkg_ap memset failed\n");
	if (memset_s(&pkg_mcu, sizeof(pkg_mcu), 0, sizeof(pkg_mcu)))
		hwlog_err("vibrator_send_cfg_cmd pkg_mcu memset failed\n");
	pkg_ap.tag = TAG_VIBRATOR;
	pkg_ap.cmd = CMD_CMN_CONFIG_REQ;
	...
	pkg_ap.wr_buf = &shd[1];
	pkg_ap.wr_len = len + SUBCMD_LEN;
	if (memcpy_s(spkt.para, len, cmd_para, len))
		hwlog_err("vibrator_send_cfg_cmd memcpy failed\n");
...
static void haptics_play_effect(struct work_struct *work)
{
	...
	if (memset_s(&read_pkg, sizeof(read_pkg), 0, sizeof(read_pkg)))
		hwlog_err("haptics_play_effect memset failed\n");
				       ...
				       const char *buf, size_t count)
{
	...
	hwlog_info("%s\n", __func__);
	if (kstrtoull(buf, CONVERT_TO_16, &value)) {
	...
	if (memset_s(&read_pkg, sizeof(read_pkg), 0, sizeof(read_pkg)))
		hwlog_err("vibrator_dbc_test_store memset failed\n");
				    ...
				    const char *buf, size_t count)
{
	...
	if (memset_s(&read_pkg, sizeof(read_pkg), 0, sizeof(read_pkg)))
		hwlog_err("vibrator_calib_store memset failed\n");
				   ...
				   struct device_attribute *attr, char *buf)
{
	...
	if (memset_s(&read_pkg, sizeof(read_pkg), 0, sizeof(read_pkg)))
		hwlog_err("vibrator_calib_show memset failed\n");
	...
	return snprintf_s(buf, PAGE_SIZE, PAGE_SIZE - 1, "%d", val);
}
				     ...
				     struct device_attribute *attr, char *buf)
{
	...
	return snprintf_s(buf, PAGE_SIZE, PAGE_SIZE - 1, "%x\n", reg_value);
}
	...
	struct device_attribute *attr, const char *buf, size_t count)
{
	...
	vibrator_off();
	if (memcpy_s(&g_drv2605_data->vib_cmd, sizeof(unsigned int), haptic_value, sizeof(unsigned int)))
		hwlog_err("haptic_test_store memcpy failed\n");
	...
	struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (memset_s(&pkg_ap, sizeof(pkg_ap), 0, sizeof(pkg_ap)))
		hwlog_err("set_amplitude_store pkg_ap memset failed\n");
	if (memset_s(&pkg_mcu, sizeof(pkg_mcu), 0, sizeof(pkg_mcu)))
		hwlog_err("set_amplitude_store pkg_mcu memset failed\n");
	pkg_ap.tag = TAG_VIBRATOR;
	pkg_ap.cmd = CMD_CMN_CONFIG_REQ;
	spkt.subcmd = SUB_CMD_VIBRATOR_SET_AMPLITUDE_REQ;
	pkg_ap.wr_buf = &shd[1];
	pkg_ap.wr_len = sizeof(val)+SUBCMD_LEN;
	if (memcpy_s(spkt.para, sizeof(val), &val, sizeof(val)))
		hwlog_err("set_amplitude_store memcpy failed\n");
	ret = write_customize_cmd(&pkg_ap, &pkg_mcu, true);
	if (ret)
...
static void vibrator_set_time(int val){
	...
	if (memset_s(&pkg_ap, sizeof(pkg_ap), 0, sizeof(pkg_ap)))
		hwlog_err("vibrator_set_time pkg_ap memset failed\n");
	if (memset_s(&pkg_mcu, sizeof(pkg_mcu), 0, sizeof(pkg_mcu)))
		hwlog_err("vibrator_set_time pkg_mcu memset failed\n");
	pkg_ap.tag = TAG_VIBRATOR;
	pkg_ap.cmd = CMD_CMN_CONFIG_REQ;
	...
	pkg_ap.wr_len = sizeof(val) + SUBCMD_LEN;
	if (memcpy_s(spkt.para, sizeof(val), &val, sizeof(val)))
		hwlog_err("vibrator_set_time memcpy failed\n");
...
static ssize_t haptics_write(struct file* filp, const char* buff, size_t len, loff_t* off)
{
	...
	if (kstrtoull(write_buf, CONVERT_TO_10, &type)) {
...
static ssize_t haptics_read(struct file *filp, char *buff, size_t len, loff_t *off)
{
	...
	data = (struct drv2605_data *)filp->private_data;
	count = snprintf_s(read_buf, MAX_BUF_RD_LEGTH, PAGE_SIZE, "%d", data->vib_cmd);
...
static int haptics_probe(struct drv2605_data *data)
{
	...
	vib_wl = wakeup_source_register(data->device, "haptic_vib_wakelock");
#else
	vib_wl = wakeup_source_register("haptic_vib_wakelock");
#endif
	return 0;
