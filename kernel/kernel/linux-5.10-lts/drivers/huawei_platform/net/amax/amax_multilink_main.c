...
static uint8_t g_bssid_assist[ETHER_ADDR_LEN] = {0};
...
static amax_roam_chr_stat *g_amax_roam_stat = NULL;
static amax_roam_chr_err *g_amax_roam_err = NULL;
...
static int32_t amax_sta_pair_init(void)
{
	...
	(void)memset_s(g_sta_pair, sizeof(amax_sta_pair), 0, sizeof(amax_sta_pair));
	return SUCC;
}
...
/* 双频漫游大数据统计结构体所有数据 */
static void amax_roam_stat_reset(void)
{
	amax_print(PRINT_DEBUG, "[roam]reset stat\n");
	(void)memset_s(g_amax_roam_stat, sizeof(amax_roam_chr_stat), 0, sizeof(amax_roam_chr_stat));
}

static void amax_roam_err_reset(void)
{
	amax_print(PRINT_DEBUG, "[roam]reset err\n");
	(void)memset_s(g_amax_roam_err, sizeof(amax_roam_chr_err), 0, sizeof(amax_roam_chr_err));
}
...
static int32_t amax_rx_thread(void *arg)
{
	...
	while (!kthread_should_stop()) {
		...
		if (g_amax_rx_timer_trigger_flag == true) {
			...
			if (is_router_support_low_power() && timer_counter % AMAX_LOW_POWER_PERIOD == 0) {
				amax_low_power_process();
			}
		}
...
static int32_t amax_rx_frame_reassemble(struct sk_buff *net_buf, recv_frame *info)
{
	...
	if (info->fn == g_fn_assist) {
		set_rx_assit_pkt_flag(1);
		amax_low_power_assist_link_pkt_process();
	}
...
static void amax_rx_time_process(uint8_t tid)
{
	...
	if (get_dualband_send_on() && g_amax_roam_stat != NULL) {
		g_amax_roam_stat->rx_pkt_loss = 
			g_amax_roam_stat->rx_pkt_loss + ((cur_id + MAX_ITEM_NUM - recv_array_hdl->rx_lowedge) & MAX_ITEM_MASK);
	}
...
uint8_t* amax_get_assist_bssid(void)
{
	amax_mac_vap_stru *pst_mac_vap = NULL;
	if (g_netdev_assist == NULL) {
		amax_print(PRINT_ERROR, "g_netdev_assist is NULL\n");
		goto exit;
	}
	pst_mac_vap = g_netdev_assist->ml_priv;
	if (pst_mac_vap == NULL) {
		amax_print(PRINT_ERROR, "pst_mac_vap is NULL\n");
		goto exit;
	}
	if (memcpy_s(g_bssid_assist, ETHER_ADDR_LEN, pst_mac_vap->auc_bssid, ETHER_ADDR_LEN) != EOK)
		goto exit;
	return g_bssid_assist;
exit:
	return NULL;
}
...
static void amax_tx_frame_dispatch(struct sk_buff *netbuf, send_frame *info, uint8_t tid)
{
	...
	if (info->fn == g_fn_assist || info->fn == FN_DOUBLE) {
		set_tx_assit_pkt_flag(1);
		amax_low_power_assist_link_pkt_process();
	}
}
...
int32_t amax_tx_handle_hook(struct sk_buff *netbuf, struct net_device **net_dev)
{
	...
	if (packet_type == PACKET_OUTBOUND) {
		...
		if (ret != SUCC) {
			...
			if(g_amax_roam_stat != NULL) {
				g_amax_roam_stat->tx_pkt_loss[FN_2] = get_2g_pkt_fail();
				g_amax_roam_stat->tx_pkt_loss[FN_5] = get_5g_pkt_fail();
			}
			return ret;
		}
...
int32_t amax_roam_sta_init(void)
{   
	if (g_amax_roam_stat != NULL) {
		amax_print(PRINT_ERROR, "[roam]g_amax_roam_stat is not NULL\n");
		kfree(g_amax_roam_stat);
		g_amax_roam_stat = NULL;
	}
	g_amax_roam_stat = (amax_roam_chr_stat *)kmalloc(sizeof(amax_roam_chr_stat), GFP_KERNEL);
	if (g_amax_roam_stat == NULL) {
		amax_print(PRINT_ERROR, "[roam]init failed!\n");
		return FAIL;
	}
	amax_roam_stat_reset();
	return SUCC;
}

void amax_roam_sta_exit(void)
{
	if (g_amax_roam_stat != NULL) {
		kfree(g_amax_roam_stat);
		g_amax_roam_stat = NULL;
...
int32_t amax_roam_err_init(void)
{   
	if (g_amax_roam_err != NULL) {
		amax_print(PRINT_ERROR, "[roam]g_amax_roam_err is not NULL\n");
		kfree(g_amax_roam_err);
		g_amax_roam_err = NULL;
	}
	g_amax_roam_err = (amax_roam_chr_err *)kmalloc(sizeof(amax_roam_chr_err), GFP_KERNEL);
	if (g_amax_roam_err == NULL) {
		amax_print(PRINT_ERROR, "[roam]init failed!\n");
		return FAIL;
	}
...
void set_send_mode_param(uint32_t sendmode)
{
	amax_print(PRINT_INFO, "{set_send_mode_param} sendmode [%d]\n", sendmode);

	if (sendmode == FN_2) {
		g_send_mode_param = AMAX_FORCE_FN_2;
		set_dualband_send_on(0);
	} else if (sendmode == FN_5) {
		g_send_mode_param = AMAX_FORCE_FN_5;
		set_dualband_send_on(0);
	} else if (sendmode == FN_DOUBLE) {
		g_send_mode_param = AMAX_FORCE_FN_DUAL;
		set_dualband_send_on(1);
	} else {
		g_send_mode_param = 0;
		set_dualband_send_on(0);
		amax_print(PRINT_ERROR, "{set_send_mode_param} out range, sendmode [%d]\n", sendmode);
	}
}

struct net_device* get_main_link_dev(void)
{
    g_netdev_main = dev_get_by_name(&init_net, g_dev_name_main);
    if(g_netdev_main != NULL) 
        return g_netdev_main;
...
uint8_t get_main_link_band(void)
{
	return g_fn_main;
}

uint8_t get_assist_link_band(void)
{
	return g_fn_assist;
}

uint8_t* get_amax_roam_stat(void)
{
	if(g_amax_roam_stat != NULL) 
		return (uint8_t*)g_amax_roam_stat;
	return NULL;
}

uint8_t* get_amax_roam_err(void)
{
	if(g_amax_roam_err != NULL) 
		return (uint8_t*)g_amax_roam_err;
	return NULL;
}

int32_t amax_ml_roam_info_report_hook(amax_dualband_roam_info_stru *dualband_roam_info)
{
	int32_t ret = SUCC;
	...
	if (g_is_dev_info_get == false || g_enable_amax_uplink == false) {
		amax_print(PRINT_ERROR, "{amax_ml_roam_info_report_hook} multilink is disable\n");
		return NOT_PROCESS;
	}
	amax_chan_info_struct amax_info;
	amax_info.fn_main = g_fn_main;
	amax_info.fn_assist = g_fn_assist;
	amax_info.force_send_mode = 0;
	ret = amax_ml_roam_info_report(dualband_roam_info, &amax_info);
...
static void amax_game_foregound_process(void)
{
...
static void amax_non_game_foregound_process(void)
{
...
static void amax_enable_plus_low_process(void)
{
	amax_non_game_foregound_process();
	low_power_set_proc();
}

static void amax_disable_process(void)
{
	amax_print(PRINT_INFO, "receive amax disable msg");
	/* clear caps when multilink disconnected */
	reset_low_power_control_info();
	if (g_enable_amax_uplink) {
	...
	if (amax_ml_get_roam_state() != INIT_STATUS) {
		amax_print(PRINT_INFO, "clear dualband roam info");
		dps_report_amax_msg_to_userspace(NULL, 0, TRANSITION_AMAX_ROAM_DISABLE);
		amax_chr_update_dualband_roam_fail(AMAX_ROAM_ML_END, 0);
...
int amax_ioctl_reset_tx_status(uint8_t enable_amax)
{
	...
	switch (enable_amax) {
		case GAME_FOREGROUND:
			amax_game_foregound_process();
			break;
		case NON_GAME_FOREGROUND:
			amax_non_game_foregound_process();
			break;
		case AMAX_ENABLE_PLUS_LOW_POWER:
			amax_enable_plus_low_process();
			break;
		default:
			amax_disable_process();
			break;
	}
...
void amax_module_init(void)
{
	...
	if (amax_try_init() == SUCC) {
		...
		amax_roam_hook_reg(amax_ml_roam_info_report_hook);
