...
/* 双频漫游单次信息统计结构体 */
typedef struct {
	uint64_t roam_start_timestamp;          /* 双频漫游开始时间戳 */
	uint64_t send_start_timestamp;          /* 双发冗余开始时间戳 */
	uint32_t send_link_dura_ns[RATIO_NUM];  /* 双频漫游冗余发送平均持续时长 */
} amax_roam_single_stat;

/* 双频漫游统计相关信息结构体 */
typedef struct {
	uint32_t roam_trigger;				  /* 双频漫游触发计数 */
	uint32_t roam_success;				  /* 双频漫游成功计数 */
	uint32_t roam_main_first;			  /* 双频漫游主链先漫游计数 */
	uint32_t duration_total;              /* 双频漫游总持续时长 */
	uint32_t send_duration_total;         /* 双频漫游冗余发送总持续时长 */
	uint32_t duration_ave;                /* 双频漫游平均持续时长 */
	uint32_t send_duration_ave;           /* 双频漫游冗余发送平均持续时长 */
	uint64_t rx_pkt_loss;                 /* 双频漫游平均rx丢包统计 */
	uint64_t tx_pkt_loss[RATIO_NUM];      /* 双频漫游平均tx丢包统计,区分频段 */
	amax_roam_single_stat single_time;
	uint64_t last_upload_tstamp; /* 双频漫游上报时间 */
	uint64_t duration; /* 双频漫游上报时间 */
} amax_roam_chr_stat;

/* 双频漫游异常结构体 */
typedef struct {
	uint32_t ml_reasoncode;       /* 双频漫游异常失败原因值 */
	uint32_t ml_disconnect;       /* 双频漫游导致ML断链 */
} amax_roam_chr_err;
...
typedef struct amax_stat {
	...
	amax_roam_chr_stat roam_chr_stat;
