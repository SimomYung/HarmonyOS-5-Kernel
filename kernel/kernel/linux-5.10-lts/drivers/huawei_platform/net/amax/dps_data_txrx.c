...
void dps_report_amax_msg_to_userspace(uint8_t *msg_info, unsigned int msg_len, int32_t msg_type)
{
	...
	dps_swap_uint16(buf.header.operation, OPERATION_AMAX_LINK_OPS);
...
void dps_ioctl_msg_proc(uint8_t *msg_info, unsigned int msg_len)
{
	...
	if (dps_action_info->operation[1] == OPERATION_AMAX_ROAM_START) {
		...
		return;
	} else if (dps_action_info->operation[1] == OPERATION_AMAX_ROAM_END) {
		...
		return;
	} else if (dps_action_info->operation[1] == OPERATION_AMAX_LINK_OPS) {
