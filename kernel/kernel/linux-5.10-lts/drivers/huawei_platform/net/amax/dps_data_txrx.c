...
#include "low_power/amax_low_power.h"
...
static uint8_t g_bssid_main[ETHER_ADDR_LEN] = {0};
dps_action_info g_amax_roam_start_info;
dps_action_info g_amax_roam_end_info;
/* 端管管理报文使用广播ip地址发送 */
...
void devicepipe_rx_data_report(uint8_t *buf, size_t len)
{
	...
	if (unlikely(buf == NULL || len > DPS_BUFFER_MAX_SIZE))
		return;
...
static bool is_need_process_in_amax(uint8_t *buf, int32_t len)
{
	dps_head *head = NULL;
	dps_capabilities *caps = NULL;
	uint16_t type;

	if (unlikely(buf == NULL || len < sizeof(dps_head) || len > DPS_BUFFER_MAX_SIZE)) {
		printk("[DPS] buf is NULL or len %d is invalid", len);
		return false;
	}

	head = (dps_head *)buf;
	type = htons(head->operation);
	/* get router caps from announcement, then don't need process in amax */
	if (type == DPS_OPERATION_ANNOUNCEMENT) {
		caps = (dps_capabilities *)(head + 1);
		set_router_support_low_power(caps->supportLowPower);
		printk("[DPS] current router low power capability:%hhu", caps->supportLowPower);
		return false;
	}

	/* low power req or rsp should process in amax, so return true */
...
static void process_dps_msg(uint8_t *buf, int32_t len)
{
	dps_head *head = NULL;
	uint16_t type;
	...
	switch (type) {
		case DPS_OPERATION_AMAX_LOW_POWER_REQ:
			process_low_power_req(buf, len);
			break;
		case DPS_OPERATION_AMAX_LOW_POWER_RSP:
			process_low_power_rsp(buf, len);
			break;
		default:
			printk("[DPS] type %hu is invalid", type);
			break;
	}
}
...
void dps_report_data_to_userspace(struct sk_buff *netbuf)
{
	...
	/* 消息传递至用户态 */
	if (is_need_process_in_amax(msg, msg_len)) {
		process_dps_msg(msg, msg_len);
	} else {
...
static uint16_t dps_swap_uint8(uint8_t *src, uint8_t len) {
	if (src == NULL || (src + 1) == NULL || len != DPS_MSG_TWO_BYTE_SIZE)
		return 0;
...
{
	uint32_t ret = SUCC;
	dps_action_frame buf;
	int32_t buf_len = sizeof(dps_action_frame);
	(void)memset_s(&buf, sizeof(buf), 0, sizeof(buf));
	printk("{dps_report_amax_msg_to_userspace} msg_type=%d", msg_type);
	if (msg_type == TRANSITION_AMAX_MAIN_LINK_INFO_REQ || msg_type == TRANSITION_AMAX_ASSIST_LINK_INFO_REQ) {
		if (unlikely(msg_info == NULL)) {
			printk("[DPS] No info in report");
			return;
        }
		dps_swap_uint16(buf.tlv_length, ETHER_ADDR_LEN);
        ret = memcpy_s(buf.link_bssid, ETHER_ADDR_LEN, msg_info, msg_len);
		if (ret != SUCC) {
			amax_print(PRINT_ERROR, "{dps_report_amax_msg_to_userspace} memcpy_s err.\n");
			return;
		}
	}
	buf.header.version = DPS_VERSION_TYPE;
	dps_swap_uint16(buf.header.operation, DPS_OPERATION_AMAX_LINK_OPS);
	dps_swap_uint16(buf.header.transactionID, msg_type);
	devicepipe_rx_data_report((uint8_t *)&buf, buf_len);
}

uint8_t *dps_get_sta_mac_addr(uint8_t dev)
{
	return dps_get_dev_mac_addr(dev);
}
...
void dps_alloc_and_send_frame(dps_mgmt_msg *dps_data_info, unsigned int msg_len)
{
	...
	if (dps_data_info == NULL || msg_len > DPS_BUFFER_MAX_SIZE ||
		(msg_len < sizeof(dps_data_info->send_dev) + sizeof(dps_data_info->bssid))) {
		printk("[DPS] invalid msg length: %u\n", msg_len);
		return;
	}
...
static void dps_handle_getlinkinfo_msg(dps_mgmt_msg *dps_data_info, unsigned int msg_len)
{
	if (dps_data_info == NULL || msg_len < sizeof(dps_mgmt_msg)) {
		printk("[DPS] dps_handle_getlinkinfo_msg check in error %u\n", msg_len);
		return;
	}
	dps_action_tlv *dps_tlv = (dps_action_tlv *)(dps_data_info->data + sizeof(dps_action_header));
	uint16_t tlv_length = 0;
    if (dps_tlv == NULL || dps_tlv->link_num > MAX_LINK_NUM) {
        printk("[DPS] dps_handle_getlinkinfo_msg dps_tlv is NULL\n");
        return;
    }
	tlv_length = dps_swap_uint8(dps_tlv->tlv_length, sizeof(dps_tlv->tlv_length));
	if (tlv_length != DPS_MSG_GET_INFO_TOTAL_LENGTH) {
        printk("[DPS] dps_handle_getlinkinfo_msg tlv_length error[%d]\n", tlv_length);
        return;
    }
	for (int i = 0; i < dps_tlv->link_num; i++) {
		if (dps_tlv->link_info[i].link_name == get_first_link_band()) {
			set_link_bssid((uint8_t *)dps_tlv->link_info[i].link_bssid, 0);
		} else {
			set_link_bssid((uint8_t *)dps_tlv->link_info[i].link_bssid, 1);
		}
	}
}

static void dps_handle_announcement(dps_mgmt_msg *dps_data_info, unsigned int msg_len)
{
	int32_t ret;

	if (dps_data_info == NULL || msg_len != DPS_ANNOUNCEMENT_TOTAL_LEN) {
		printk("[DPS] dps_handle_announcement check in error %u\n", msg_len);
		return;
	}

	if (dps_data_info->send_dev == DPS_DEV_MAIN) {
		ret = memcpy_s(g_bssid_main, ETHER_ADDR_LEN, dps_data_info->bssid, ETHER_ADDR_LEN);
		if (ret == EOK)
			printk("[DPS] get main bssid " BLURMACSTR "\n", BLURMAC2STR(g_bssid_main));
	}
}
...
void dps_ioctl_msg_proc(uint8_t *msg_info, unsigned int msg_len)
{
	...
	dps_action_header *dps_action_info = NULL;
	...
	if (msg_len < sizeof(dps_mgmt_msg) || msg_len > DPS_BUFFER_MAX_SIZE) {
		printk("[DPS] error msg_len %u\n", msg_len);
		return;
	}

	printk("[DPS] dps ioctl msg. len[%u]\n", msg_len);
	dps_action_info = (dps_action_header *)dps_data_info->data;
	if (dps_action_info == NULL) {
		printk("[DPS] dps_action_info is NULL\n");
		return;
	}
	
	if (dps_action_info->operation[1] == DPS_OPERATION_ZERO_ROAM_START) {
		/* multilink重新建链，双频漫游状态机初始化 */
		if (amax_ml_get_roam_state() != INIT_STATUS)
			amax_ml_roam_init_process();
		printk("[DPS] dps_action_info dual_band roam start msg store, len[%u]\n", msg_len);
		if (memcpy_s(g_amax_roam_start_info.param_buf, sizeof(char) * DPS_BUFFER_MAX_SIZE, msg_info, msg_len) != EOK)
			return;
		g_amax_roam_start_info.param_size = msg_len;
		return;
	} else if (dps_action_info->operation[1] == DPS_OPERATION_ZERO_ROAM_END) {
		printk("[DPS] dps_action_info dual_band roam end msg store, len[%u]\n", msg_len);
		if (memcpy_s(g_amax_roam_end_info.param_buf, sizeof(char) * DPS_BUFFER_MAX_SIZE, msg_info, msg_len) != EOK)
			return;
		g_amax_roam_end_info.param_size = msg_len;
		return;
	} else if (dps_action_info->operation[1] == DPS_OPERATION_AMAX_LINK_OPS) {
		printk("[DPS] dps_action_info dual_band roam link bssid info get, len[%u]\n", msg_len);
		dps_handle_getlinkinfo_msg(dps_data_info, msg_len);
		return;
	} else if (dps_action_info->operation[1] == DPS_OPERATION_ANNOUNCEMENT) {
		dps_handle_announcement(dps_data_info, msg_len);
	}
...
{
	int i;
	if (unlikely(bssid == NULL || len != ETHER_ADDR_LEN))
		return false;
...
{
	dps_common_part *common_part = NULL;

	if (unlikely(buf == NULL || len < COMMON_PART_LEN)) {
		printk("[DPS] buf is null or len %u is invalid\n", len);
		return -1;
	}

	common_part = (dps_common_part *)buf;
	common_part->send_dev = DPS_DEV_MAIN;
	if (!is_bssid_valid(g_bssid_main, sizeof(g_bssid_main))) {
		printk("[DPS] build common part: main bssid is invalid\n");
		return -1;
	}
	if (memcpy_s(common_part->bssid, sizeof(common_part->bssid), g_bssid_main, sizeof(g_bssid_main)) != EOK) {
		printk("[DPS] build common part: bssid copy failed\n");
		return -1;
	}
	return 0;
}
...
void dps_build_data_header(uint8_t *buf, uint16_t type, uint8_t version)
{
	...
	if (unlikely(buf == NULL)) {
		printk("[DPS] dps_build_data_header buf is null\n");
		return;
	}

	head = (dps_head *)buf;
	head->version = version;
	head->operation = htons(type);
	/* transaction_id当前默认填1，并无实际作用 */
	head->transactionId = htons(1);
}
