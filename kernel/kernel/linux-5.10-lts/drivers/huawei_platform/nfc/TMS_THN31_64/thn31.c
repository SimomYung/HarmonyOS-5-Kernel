...
#define BL_RECV_BUFFER_LEN 40
#define BL_RESET_CMD_LEN 4
#define BL_FW_CMD_LEN 9
#define NCI_HEADER 3
#define NCI_LEN_INDEX 2
#define T1_HEAD 0x5A
...
static int g_nfc_bootloader_status = NFC_IN_NORMAL_STATE;
static int g_nfc_dwl_volt_status = 0;
static int g_nfc_fw_status = 0;
...
static bool get_nfc_chip_only_ese(void)
{
    int ret = -1;
    static char chip_mode[MAX_CONFIG_NAME_SIZE] = {0};
    if (memset_s(chip_mode, sizeof(chip_mode), 0x00, MAX_CONFIG_NAME_SIZE) != 0) {
        hwlog_err("%s: memset_s failed\n");
    }
    ret = nfc_get_dts_config_string("nfc_chip_only_ese", "nfc_chip_only_ese", chip_mode, sizeof(chip_mode));
    if (ret != 0) {
        hwlog_info("%s: has no chip only ese config!\n", __func__);
        return false;
    }
    if (!strcasecmp(chip_mode, "true")) {
        return true;
    }
    return false;
}

static int get_nfc_chip_gpio_ven(void)
{
    int nfc_chip_gpio_ven = -1;
    if (nfc_get_dts_config_u32("nfc_chip_gpio_ven", "nfc_chip_gpio_ven", &nfc_chip_gpio_ven) != 0) {
        hwlog_err("%s: has no chip gpio ven!\n", __func__);
        return -1;
    }
    hwlog_info("%s: nfc_chip_gpio_ven: %d\n", __func__, nfc_chip_gpio_ven);
    return nfc_chip_gpio_ven;
}
		...
		size_t count, loff_t *offset)
{
	...
	/* thn31 download max size is 4096 */
	if (count > MAX_BUFFER_SIZE) {
		if (count > MAX_DL_BUFFER_SIZE) {
			hwlog_err("%s : Max count = %d\n", __func__, count);
...
static int thn31_ese_recover(struct  thn31_dev *pdev)
{
	int ret = NFC_OK;
	int ret_len;
	char buffer[MAX_NFC_DMD_INFO_LEN];
	...
	if (g_is_first_time ||
		...
		} else if (g_nfc_open_status == NFC_OPEN_STATUS) {
			ret = ese_recover_with_openstatus(pdev);
		} else {
			ret = NFC_OPENING_NO_DO_RECOVER;
		}
		ret_len = snprintf_s(buffer, MAX_NFC_DMD_INFO_LEN, MAX_NFC_DMD_INFO_LEN - 1,
			"%s nfc_status = %d, dwl_volt_value = 0x%x, fw_status = %d, ret = 0x%x.",
			__func__, g_nfc_open_status, g_nfc_dwl_volt_status, g_nfc_fw_status, ret);
		if (ret_len < 0) {
			hwlog_err("%s: snprintf_s is failed\n", __func__);
	    } else {
	    	hwlog_info("%s %s\n", __func__, buffer);
    ...
    int tmpFwStatue = (g_nfc_fw_status == -1) ? (0xF1) : g_nfc_fw_status;
    int retVal = ((ret << 24) | (g_nfc_open_status << 16) | (g_nfc_dwl_volt_status << 8) | tmpFwStatue);
    hwlog_info("%s retVal=0x%X", __func__, retVal);
    return retVal;
}
...
static int check_sim_status(struct i2c_client *client, struct  thn31_dev *pdev)
{
	...
	if (ret < 0 || recv_buf[NCI_LEN_INDEX] > sizeof(recv_buf)) {
		hwlog_err("%s: CORE_RESET_RSP HEAD nfc_bj_i2c_read failed, ret:%d\n", __func__, ret);
		goto failed;
	}
	ret = thn31_i2c_read(pdev, recv_buf, recv_buf[NCI_LEN_INDEX]);    // read rsp payload recv_buf[2]
	if (ret < 0) {
	...
	ret = thn31_i2c_read(pdev, recv_buf, recv_buf[NCI_LEN_INDEX]);    // read ntf payload recv_buf[2]
	if (ret < 0) {
	...
	mutex_unlock(&pdev->common_mutex);
	return status;
failed:
	pdev->sim_status = ret;
	mutex_unlock(&pdev->common_mutex);
...
static ssize_t nfc_bootloader_status_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	return (ssize_t)(snprintf_s(buf, MAX_ATTRIBUTE_BUFFER_SIZE, MAX_ATTRIBUTE_BUFFER_SIZE - 1, "%d", g_nfc_bootloader_status));
}

static ssize_t nfc_bootloader_status_store(struct device *dev, struct device_attribute *attr,
			 const char *buf, size_t count)
{
	int val = 0;
	if (sscanf_s(buf, "%d", &val) == 1) {
		g_nfc_bootloader_status = val;
	} else {
		hwlog_err("%s: set g_nfc_bootloader_status error\n", __func__);
		g_nfc_bootloader_status = NFC_IN_NORMAL_STATE;
	}
	hwlog_info("%s: g_nfc_bootloader_status:%d\n", __func__, g_nfc_bootloader_status);
	return (ssize_t)count;
}
			 ...
			 const char *buf, size_t count)
{
	...
	if (sscanf_s(buf, "%1d", &val) == 1) {
		...
		hwlog_err("%s: set g_nfc_need_reset error\n", __func__);
		g_nfc_need_reset = NFC_NONEED_RESET;
	}
	hwlog_info("%s: g_nfc_need_reset:%d\n", __func__, g_nfc_need_reset);
	return (ssize_t)count;
}
...
static struct device_attribute thn31_attr[] = {
	...
	__ATTR(nfc_ese_num_dts, 0664, nfc_ese_num_dts_show, NULL),
	__ATTR(spi_bus, 0664, spi_bus_show, NULL),
	__ATTR(gpio_spi_cs, 0664, gpio_spi_cs_show, NULL),
	...
	__ATTR(nfc_bootloader_status, 0664, nfc_bootloader_status_show, nfc_bootloader_status_store),
};
/* lint -restore */
/* lint -save -e* */
...
/* lint -restore */
/* lint -save -e* */
static int send_bl_cmd(struct thn31_dev *pdev, const char *cmd, int cmd_len, unsigned char *recv, int max_recv_len)
{
	int ret = -1;

	if (pdev == NULL || cmd == NULL || recv == NULL) {
		hwlog_err("%s: is NULL\n", __func__);
		return ret;
    }
	hwlog_info("%s: enter \n", __func__);
	...
	/* write T1_RESET_CMD */
	ret = thn31_i2c_write(pdev, cmd, cmd_len);
	if (ret < 0) {
		hwlog_err("%s: nfc_bj_i2c_write failed, ret:%d\n", __func__, ret);
		return ret;
	}
	...
	msleep(10);
	hwlog_info("%s: sleep 10ms:\n", __func__);
	/* read response */
	ret = thn31_i2c_read(pdev, recv, NCI_HEADER);
	if (ret < 0) {
		hwlog_err("%s: HEAD nfc_bj_i2c_read failed, ret:%d\n", __func__, ret);
		return ret;
	}
	if (recv[NCI_LEN_INDEX] + 1 + NCI_HEADER > max_recv_len) {
		hwlog_err("%s: len error:\n", __func__);
		return ret;
	}
	ret = thn31_i2c_read(pdev, &recv[NCI_HEADER], recv[NCI_LEN_INDEX] + 1); // read rsp payload recv_buf[2] + 1: NAD + PCB + LEN + 1
	if (ret < 0) {
		hwlog_err("%s: PAYLOAD nfc_bj_i2c_read failed, ret:%d\n", __func__, ret);
		return ret;
	}
...
{
	int ret = -1;
	unsigned char recv_buf[BL_RECV_BUFFER_LEN] = {0};
	const char bl_reset_cmd[BL_RESET_CMD_LEN] = {0x5A, 0xC4, 0x00, 0x9E};
	const char bl_fw_status_cmd[BL_FW_CMD_LEN] = {0x5A, 0x00, 0x05, 0x00, 0x4A, 0xFE, 0x00, 0x00, 0xEB};

	hwlog_info("%s: enter send_bl_reset\n", __func__);

	ret = send_bl_cmd(pdev, bl_reset_cmd, sizeof(bl_reset_cmd), recv_buf, sizeof(recv_buf));
	if (ret != NFC_OK || ((recv_buf[0] != 0xA5) || (recv_buf[1] != 0xE4))) {
		hwlog_err("%s: PAYLOAD nfc_bj_i2c_read failed, ret:%d\n", __func__, ret);
		return -1;        
	}
	memset_s(recv_buf, sizeof(recv_buf), 0x00, sizeof(recv_buf));
	ret = send_bl_cmd(pdev, bl_fw_status_cmd, sizeof(bl_fw_status_cmd), recv_buf, sizeof(recv_buf));
	if (ret != NFC_OK) {
		hwlog_err("%s: PAYLOAD nfc_bj_i2c_read failed, ret:%d\n", __func__, ret);
		return -1;        
	}
	if (recv_buf[NCI_LEN_INDEX] > 5) { // buf 2 means payload len
		g_nfc_dwl_volt_status = recv_buf[3];
		if ((recv_buf[4] == 0x67) && (recv_buf[5] == 0x45) &&
 			(recv_buf[6] == 0x23) && (recv_buf[7] == 0x01)) { // 67452301 means fw Ok
 			g_nfc_fw_status = 0;
 			hwlog_err("%s: PAYLOAD fw is normal\n", __func__);
		} else if ((recv_buf[4] == 0x98) && (recv_buf[5] == 0xBA) &&
 			(recv_buf[6] == 0xDC) && (recv_buf[7] == 0xFE)) { // 98BADCFE means fw is not exist
 			g_nfc_fw_status = 1;
 			hwlog_err("%s: PAYLOAD fw is not exist\n", __func__);
		} else { // other error
 			g_nfc_fw_status = -1;
 			hwlog_err("%s: PAYLOAD fw Other error\n", __func__);
		}
	}
	return 0;
}
...
static int check_bl_reset(struct i2c_client *client, struct  thn31_dev *pdev)
{
	...
	int count = 0;
	int retLen = 0;
	char dmd_record_info[MAX_NFC_DMD_INFO_LEN] = {'\0'};

	ret = send_bl_reset(client, pdev);
	while (ret != NFC_OK && count < NFC_TRY_TWO) {
		ret = send_bl_reset(client, pdev);
		if (ret != NFC_OK) {
	...
	if (ret != NFC_OK) {
		hwlog_err("%s: send_bl_reset failed, ret:%d\n", __func__, ret);
		return ret;
	}
	retLen = snprintf_s(dmd_record_info, MAX_NFC_DMD_INFO_LEN, MAX_NFC_DMD_INFO_LEN - 1,
        "%s, dwl_volt_value=0x%x, fw_status = %d.",
		"THN31 in bootloader state1", g_nfc_dwl_volt_status, g_nfc_fw_status);
	if (retLen > 0) {
		tms_nfc_record_dmd_info(NFC_DMD_I2C_READ_ERROR_NO, dmd_record_info);
	}
	msleep(10);
	ret = thn31_enable_nfc(pdev);
	if (ret != NFC_OK)
...
static int check_bl_download_and_reset(struct i2c_client *client, struct  thn31_dev *pdev)
{
	...
	for (count = 0; count < NFC_TRY_TWO; count++) {
		...
		g_nfc_bootloader_status = NFC_IN_BOOTLOADER_STATE;
...
static int thn31_only_ese_probe(struct i2c_client *client)
{
    int ret = -1;
    int nfc_chip_gpio_ven = -1;
    if (client == NULL) {
    ...
    nfc_chip_gpio_ven = get_nfc_chip_gpio_ven();
    if (nfc_chip_gpio_ven < 0) {
        return -ENODEV;
    }
    ret = ese_ca_parse_dt();
    ...
    hwlog_info("Nfc on by ven_gpio:%d\n", nfc_chip_gpio_ven);
    if (!gpio_is_valid(nfc_chip_gpio_ven)) {
        hwlog_err("failed to check nfc_chip_gpio_ven");
        return -ENODEV;
    }
    ret = gpio_request(nfc_chip_gpio_ven, NULL);
    if (ret < 0) {
        hwlog_err("%s: gpio_request failed, ret:%d.\n", __func__, nfc_chip_gpio_ven);
        return -ENODEV;
    }

    ret = gpio_direction_output(nfc_chip_gpio_ven, 0);
    if (ret < 0) {
        hwlog_err("%s: Fail set gpio as output, nfc_chip_gpio_ven:%d.\n", __func__, nfc_chip_gpio_ven);
        return -ENODEV;
    }
    msleep(TIME_CHIP_POWER_UP);
    ret = gpio_direction_output(nfc_chip_gpio_ven, 1);
    if (ret < 0) {
        hwlog_err("%s: Fail set gpio as output, nfc_chip_gpio_ven:%d.\n", __func__, nfc_chip_gpio_ven);
        return -ENODEV;
    }
		...
		const struct i2c_device_id *id)
{
    ...
    if (get_nfc_chip_only_ese()) {
        return thn31_only_ese_probe(client);
    }
	...
	mutex_init(&thn31_dev->common_mutex);
	/* Initialize wakelock */
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(5, 10, 0))
...
#endif
	mutex_destroy(&thn31_dev->common_mutex);
...
static int thn31_only_ese_remove(struct i2c_client *client)
{
	if (client == NULL) {
		hwlog_err("%s:	client == NULL!\n", __func__);
		return -ENODEV;
	}
	remove_sysfs_interfaces(&client->dev);
	return 0;
...
static int thn31_remove(struct i2c_client *client)
{
    ...
    if (get_nfc_chip_only_ese()) {
        return thn31_only_ese_remove(client);
    }
