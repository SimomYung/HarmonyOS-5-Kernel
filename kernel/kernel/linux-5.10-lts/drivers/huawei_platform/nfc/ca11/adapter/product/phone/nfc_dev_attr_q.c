...
static ssize_t nfc_activated_se_info_show(struct device *dev, struct device_attribute *attr, char *buf)
{
    unused(dev);
    unused(attr);
    PS_PRINT_INFO("nfc_activated_se_info = %d\n", g_nfc_activated_se_info);
    return (ssize_t)(snprintf_s(buf, MAX_ATTRIBUTE_BUFFER_SIZE, MAX_ATTRIBUTE_BUFFER_SIZE - 1,
...
static struct device_attribute caxx_only_ese_attr[] = {
	__ATTR(nfc_chip_type, 0444, nfc_chip_type_show, NULL),
	__ATTR(nfc_at_result, 0664, nfc_at_result_show, nfc_at_result_store),
};
...
int caxx_only_ese_create_attribute_node(struct i2c_client *client)
{
	...
	for (i = 0 ; i < ARRAY_SIZE(caxx_only_ese_attr); i++) {
		if (device_create_file(&client->dev, caxx_only_ese_attr + i)) {
			i--;
			PS_PRINT_ERR("caxx only ese unable to create attribute sysfs interface.\n");
	...
	ret = sysfs_create_link(NULL, &client->dev.kobj, "nfc");
	if (ret < 0) {
		PS_PRINT_ERR("caxx only ese create_attribute_node failed to sysfs_create_link.\n");
		goto create_file_error;
	}
	return 0;
...
create_file_error:
	for (; i >= 0; i--)
		device_remove_file(&client->dev, caxx_only_ese_attr + i);
...
int caxx_only_ese_remove_attribute_sysfs(struct device *dev)
{
	int i;
	for (i = 0; i < ARRAY_SIZE(caxx_only_ese_attr); i++)
		device_remove_file(dev, caxx_only_ese_attr + i);
