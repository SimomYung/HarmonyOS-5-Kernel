							...
							unsigned int cmd, unsigned long arg)
{
	...
	switch (arg) {
		...
		case NFC_POWER_ON:
			...
			break;
#ifdef CONFIG_CA11_NFC
		case NFC_VOTE_POWER_OFF:
			/* Vote for NFC power-off to SE */
			PS_PRINT_INFO("nfc ioctl: Vote for nfc poweroff to SE.\n");
			ret  = nfc_ven_set(false);
			break;
		case NFC_VOTE_POWER_ON:
			/* Vote for NFC power-on to SE */
			PS_PRINT_INFO("nfc ioctl: Vote for nfc poweron to SE.\n");
			ret  = nfc_ven_set(true);
			break;
#endif
		default:
...
#ifdef CONFIG_CA11_NFC
static int caxx_only_ese_probe(struct i2c_client *client)
{
	PS_PRINT_INFO("caxx_only_ese_probe begin!\n");
	/* create interface node */
	if (caxx_only_ese_create_attribute_node(client) < 0) {
		PS_PRINT_ERR("Failed to caxx only ese create_attribute_node\n");
		return -ENODEV;
	}
	PS_PRINT_INFO("caxx_only_ese_probe sucess!\n");
	return 0;
}
#endif

static inline void caxx_dsm_register_client(void)
{
#ifdef CONFIG_HUAWEI_DSM
	if (!nfc_dclient)
...
#ifdef CONFIG_CA11_NFC
	if (caxx_chip_only_ese()) {
		return caxx_only_ese_probe(client);
	}
#endif

	PS_PRINT_INFO("caxx_probe begin!\n");

	caxx_dsm_register_client();
...
#ifdef CONFIG_CA11_NFC
	if (caxx_chip_only_ese()) {
		caxx_only_ese_remove_attribute_sysfs(&client->dev);
		return 0;
	}
#endif
