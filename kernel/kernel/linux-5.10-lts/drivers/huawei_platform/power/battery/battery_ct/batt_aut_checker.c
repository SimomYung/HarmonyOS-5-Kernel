...
static int get_batt_storage_info_wrapper(struct batt_chk_data *drv_data,
	struct batt_storage_info_data *data, enum batt_storage_info_type type)
{
	...
	if (!drv_data || !data)
		return ret;
	if (drv_data->ic_ops.get_batt_storage_info) {
		bat_type_apply_mode(BAT_ID_SN);
		ret = drv_data->ic_ops.get_batt_storage_info(drv_data->ic,
			data, type);
		bat_type_release_mode(true);
	}
...
static int set_batt_storage_info_wrapper(struct batt_chk_data *drv_data,
	struct batt_storage_info_data *data, enum batt_storage_info_type type)
{
	int ret = -EINVAL;
	...
	if (!drv_data || !data)
		return ret;
	if (drv_data->ic_ops.set_batt_storage_info) {
		bat_type_apply_mode(BAT_ID_SN);
		ret = drv_data->ic_ops.set_batt_storage_info(drv_data->ic,
			data, type);
		bat_type_release_mode(true);
	}
...
static int fetch_batt_type_from_sn(struct batt_chk_data *drv_data)
{
	int index, i;

	if (!drv_data->bco.get_batt_sn) {
		hwlog_err("%s get_batt_sn is NULL\n", __func__);
		return -ENOTSUPP;
	}

	if (!drv_data->sn) {
		if (drv_data->bco.get_batt_sn(drv_data, 0,
	...
	for (i = 0; i < drv_data->batt_type_len; i++) {
		index = drv_data->batt_type_index[i];
		if(index >= MAX_RAW_SN_LEN) {
			hwlog_err("%s index is out of range\n", __func__);
			return -EINVAL;
		}
		drv_data->batt_type_extend[i] = drv_data->sn[index];
	}

	return 0;
}

static int get_batt_type_extend(struct batt_chk_data *drv_data,
	const char **type, unsigned int *type_len)
{
	int ret;
	...
	ret = fetch_batt_type_from_sn(drv_data);
	if (ret != EOK) {
		hwlog_err("%s fetch_batt_type_from_sn failed\n", __func__);
		return ret;
	}

	*type = drv_data->batt_type_extend;
	*type_len = (unsigned int)drv_data->batt_type_len;

	return 0;
}

static int get_certification_retry_count_wrapper(struct batt_chk_data *drv_data)
{
	int ret = -1;
	...
	if (drv_data->ic_ops.get_certification_retry_count) {
		bat_type_apply_mode(BAT_ID_SN);
		ret = drv_data->ic_ops.get_certification_retry_count(drv_data->ic);
		bat_type_release_mode(true);
	}
	return ret;
}
...
static void init_batt_storage_info_func(struct work_struct *work)
{
	...
	drv_data = container_of(work, struct batt_chk_data, init_storage_info_work);
	if (!drv_data || !drv_data->bco.get_batt_storage_info) {
		hwlog_err("[%s] drv_data is NULL\n", __func__);
		return;
	}

	drv_data->storage_info.data.activation = &(drv_data->storage_info.activation.data);
	drv_data->storage_info.data.cycle = &(drv_data->storage_info.cycle.data);
	drv_data->storage_info.data.real_soh = &(drv_data->storage_info.real_soh.data);
	drv_data->storage_info.data.display_soh = &(drv_data->storage_info.display_soh.data);

	mutex_init(&(drv_data->storage_info.cycle.update_lock));
	mutex_init(&(drv_data->storage_info.real_soh.update_lock));
	mutex_init(&(drv_data->storage_info.display_soh.update_lock));

	if (drv_data->bco.get_batt_storage_info(drv_data, &(drv_data->storage_info.data),
		BATT_ALL_STORAGE_INFO)) {
		hwlog_err("[%s] get all batt storage info failed\n", __func__);
		return;
	}

	drv_data->storage_info.activation.initialized = true;
	drv_data->storage_info.cycle.initialized = true;
	drv_data->storage_info.real_soh.initialized = true;
	drv_data->storage_info.display_soh.initialized = true;
}
...
static void init_wrapper_ops(struct batt_chk_data *drv_data)
{
	...
	drv_data->bco.get_batt_storage_info = get_batt_storage_info_wrapper;
	drv_data->bco.set_batt_storage_info = set_batt_storage_info_wrapper;
	drv_data->bco.get_batt_type_extend = get_batt_type_extend;
	drv_data->bco.get_certification_retry_count = get_certification_retry_count_wrapper;
}
...
static int battery_all_work_init(struct batt_chk_data *drv_data)
{
	...
	/* init a work for read battery storage info from IC */
	INIT_WORK(&(drv_data->init_storage_info_work), init_batt_storage_info_func);
...
static void parse_batt_type_dts_info(struct platform_device *pdev, struct batt_chk_data *drv_data)
{
	int ret;
	if (!drv_data)
		return;

	drv_data->batt_type_len = power_dts_read_u32_count(power_dts_tag(HWLOG_TAG),
		pdev->dev.of_node, "batt-type-index", BATT_TYPE_INDEX_ROW, BATT_TYPE_INDEX_COL);
	if (drv_data->batt_type_len < 0) {
		hwlog_err("%s power_dts_read_u32_count failed\n", __func__);
		goto use_default;
	}
	if (drv_data->batt_type_len > MAX_BATT_TYPE_EXTEND_LEN) {
		hwlog_err("%s batt_type_len is out of range\n", __func__);
		goto use_default;
	}

	ret = power_dts_read_u32_array(power_dts_tag(HWLOG_TAG), pdev->dev.of_node,
		"batt-type-index", drv_data->batt_type_index, drv_data->batt_type_len);
	if (ret != EOK) {
		hwlog_err("%s power_dts_read_u32_array\n", __func__);
		goto use_default;
	}
	return;

use_default:
	hwlog_err("[%s] get battery type failed and use default\n", __func__);
	drv_data->batt_type_len = DEFAULT_BATT_TYPE_LEN;
	drv_data->batt_type_index[0] = DEFAULT_BATT_TYPE_FISRT_INDEX;
	drv_data->batt_type_index[1] = DEFAULT_BATT_TYPE_SECOND_INDEX;
	return;
}
...
static struct batt_chk_data *batt_chk_data_init(struct platform_device *pdev)
{
	...
	parse_batt_type_dts_info(pdev, drv_data);
	...
	drv_data->batt_check_lock = power_wakeup_source_register(&pdev->dev, "battcheck_wakelock");
...
{
	/* update:multiline parameter */
	int row, len;
	int backup_id_para[BACKUP_ID_CHECK_MAX * BACKUP_ID_END] = { 0 };

	data->backup_id_check_level = 0;
	len = power_dts_read_string_array(power_dts_tag(HWLOG_TAG), np,
		"backup_id_check", backup_id_para, BACKUP_ID_CHECK_MAX, BACKUP_ID_END);
	if (len < 0) {
		hwlog_err("backup_id_check parse failed\n");
		return;
	}
	len /= BACKUP_ID_END;
	data->backup_id_check_level = len;

	for (row = 0; row < len; row++) {
		data->backup_check_item[row].sn_backup_id =
			backup_id_para[row * BACKUP_ID_END + SN_BACKUP_ID_OFFSET];
		data->backup_check_item[row].content =
			backup_id_para[row * BACKUP_ID_END + CONTENT_OFFSET];

		hwlog_info("backup_id_check[%d].sn_backup_id=%d content=%d\n", row,
			data->backup_check_item[row].sn_backup_id,
			data->backup_check_item[row].content);
	}
}
...
static int battery_aut_checker_probe(struct platform_device *pdev)
{
	...
	/* init batt storage info */
	schedule_work(&(drv_data->init_storage_info_work));
...
{
	struct batt_chk_data *drv_data = platform_get_drvdata(pdev);

	if (!drv_data)
		return -ENODEV;
	power_wakeup_source_unregister(drv_data->batt_check_lock);
	return BATTERY_DRIVER_SUCCESS;
}
