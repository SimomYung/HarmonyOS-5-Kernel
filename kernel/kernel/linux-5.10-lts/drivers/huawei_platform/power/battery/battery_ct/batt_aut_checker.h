...
struct batt_storage_activation {
	struct activation_info data;
	bool initialized;
};

struct batt_storage_cycle {
	u16 data;
	u16 update_data;
	bool update_flag;
	bool initialized;
...
struct batt_storage_display_soh {
	u8 data;
	u8 update_data;
	bool update_flag;
	bool initialized;
...
struct batt_storage_info_data {
	struct activation_info *activation;
	u16 *cycle;
	u8 *real_soh;
	u8 *display_soh;
};

struct batt_storage_info {
	struct batt_storage_activation activation;
	struct batt_storage_cycle cycle;
	struct batt_storage_real_soh real_soh;
	struct batt_storage_display_soh display_soh;
	struct batt_storage_info_data data;
};
...
struct batt_ct_ops {
	...
	int (*get_batt_storage_info)(struct platform_device *pdev,
		struct batt_storage_info_data *data, enum batt_storage_info_type type);
	int (*set_batt_storage_info)(struct platform_device *pdev,
		struct batt_storage_info_data *data, enum batt_storage_info_type type);
	int (*get_certification_retry_count)(struct platform_device *pdev);
};
...
enum backup_id_offset {
	SN_BACKUP_ID_OFFSET = 0,
	CONTENT_OFFSET,
	BACKUP_ID_END,
};
...
#define BACKUP_ID_CHECK_MAX                         3

/* Battery type */
#define MAX_BATT_TYPE_EXTEND_LEN                    5
#define DEFAULT_BATT_TYPE_LEN                       2
#define BATT_TYPE_INDEX_COL                         1
#define BATT_TYPE_INDEX_ROW                         5
#define DEFAULT_BATT_TYPE_FISRT_INDEX               5
#define DEFAULT_BATT_TYPE_SECOND_INDEX              4
...
/* batt storage info */
#define BATT_ACTIVATION_LEN                          2
#define BATT_CYCLE_LEN                               2
#define BATT_REAL_SOH_LEN                            1
#define BATT_DISPLAY_SOH_LEN                         1
#define BATT_STORAGE_INFO_LEN                        6
#define BATT_ACTIVATION_OFFSET                       0
#define BATT_CYCLE_OFFSET                            (BATT_ACTIVATION_OFFSET + BATT_ACTIVATION_LEN)
#define BATT_REAL_SOH_OFFSET                         (BATT_CYCLE_OFFSET + BATT_CYCLE_LEN)
#define BATT_DISPLAY_SOH_OFFSET                      (BATT_REAL_SOH_OFFSET + BATT_REAL_SOH_LEN)

/* bit operation of batt storage info */
#define BITS_PER_BYTE                                8
#define BYTE_MASK                                    0xFF
#define BUF_FIRST_INDEX                              0
#define BUF_SECOND_INDEX                             1
#define BUF_THIRD_INDEX                              2
...
struct batt_ct_wrapper_ops {
	...
	int (*get_batt_type_extend)(struct batt_chk_data *drv_data,
		const char **type, unsigned int *type_len);
	int (*get_batt_storage_info)(struct batt_chk_data *drv_data,
		struct batt_storage_info_data *data, enum batt_storage_info_type type);
	int (*set_batt_storage_info)(struct batt_chk_data *drv_data,
		struct batt_storage_info_data *data, enum batt_storage_info_type type);
	int (*get_certification_retry_count)(struct batt_chk_data *data);
};
...
struct batt_chk_data {
	...
	int backup_id_check_level;
	struct backup_id_check backup_check_item[BACKUP_ID_CHECK_MAX];
	enum sn_type check_sn_type;
	struct batt_thread_work key_twork;
	struct batt_thread_work sn_twork;
	int batt_type_len;
	u32 batt_type_index[MAX_BATT_TYPE_EXTEND_LEN];
	char batt_type_extend[MAX_BATT_TYPE_EXTEND_LEN];
	struct wakeup_source *batt_check_lock;
	struct work_struct init_storage_info_work;
