...
struct batt_info *get_batt_info(void)
{
	if (!g_batt_info)
		hwlog_err("g_batt_info is null\n");

	return g_batt_info;
}

struct list_head *get_batt_checkers_head(void)
{
	return &batt_checkers_head;
}

int get_checker_data_by_id(unsigned int index, struct batt_chk_data **checker_data)
{
	struct batt_checker_entry *temp = NULL;
	struct batt_chk_data *checker_data_temp = NULL;

	list_for_each_entry(temp, &batt_checkers_head, node) {
		checker_data_temp = platform_get_drvdata(temp->pdev);
		if ((!checker_data_temp) || (index == (unsigned int)checker_data_temp->id_in_grp))
			break;
	}

	if (!checker_data_temp || (index != (unsigned int)checker_data_temp->id_in_grp)) {
		hwlog_err("[%s] get checker_data@%u fail\n", __func__, index);
		return -ENOENT;
	}

	*checker_data = checker_data_temp;
	return 0;
}
...
int check_certification_retry_count(struct batt_chk_data *checker_data)
{
	int ret = 0;

	if (checker_data && checker_data->bco.get_certification_retry_count)
		ret = checker_data->bco.get_certification_retry_count(checker_data);
	return ret;
}
	...
	unsigned char subcmd, const void *data, int len)
{
	...
	case ACT_SIGN:
		...
		return;
	default:
		hwlog_err("unkown subcmd for checker(NO:%hhu Group:%hhu)\n",
...
static int batt_check_battery_cells_match(void)
{
	...
	list_for_each_entry(temp, &batt_checkers_head, node) {
		...
		if (!checker_data->bco.get_batt_type) {
			hwlog_err("[%s] checker(NO:%hhu Group:%hhu) get_batt_type is null\n",
		...
		if (checker_data->bco.get_batt_type(checker_data,
			...
			hwlog_err("[%s] checker(NO:%hhu Group:%hhu) get battery type error\n",
				__func__, checker_data->id_in_grp,
				checker_data->id_of_grp);
			return -1;
		}
	...
	int out_max_len)
{
	...
	list_for_each_entry(temp, &batt_checkers_head, node) {
		...
		if (!checker_data->bco.get_batt_safe_info) {
			hwlog_err("[%s] checker(NO:%hhu Group:%hhu) get_batt_safe_info is null\n",
				__func__, checker_data->id_in_grp,
				checker_data->id_of_grp);
			return -1;
		}
...
static int check_ic_func(struct batt_info *drv_data, struct batt_chk_rslt *final_result)
{
	...
	list_for_each_entry(temp, &batt_checkers_head, node) {
		...
		if (!ret) {
			...
			hwlog_info("checker(NO:%hhu Group:%hhu) status illegal\n",
	...
	list_for_each_entry(temp, &batt_checkers_head, node) {
		...
		if (!ret) {
			*final_result = checker_data->result;
			hwlog_info("batt(NO:%hhu Group:%hhu) key status illegal\n",
				checker_data->id_in_grp,
				checker_data->id_of_grp);
			return -1;
		}
...
static int check_sn_func(struct batt_chk_rslt *final_result)
{
	...
	list_for_each_entry(temp, &batt_checkers_head, node) {
		...
		if (!ret) {
			hwlog_info("batt(NO:%hhu Group:%hhu) sn status illegal\n",
				checker_data->id_in_grp,
				checker_data->id_of_grp);
			return -1;
		}
...
static bool check_backup_id_validate(struct batt_chk_data *checker_data, char *val)
{
	int i;

	if (checker_data->backup_id_check_level <= 0)
		return true;

	for (i = 0; i < checker_data->backup_id_check_level; i++) {
		if ((int)(*val) == checker_data->backup_check_item[i].content)
			return true;
		hwlog_err("%s sn backup id content is %c not equal to required para %c.\n",
			__func__, *val, checker_data->backup_check_item[i].content);
	}
	return false;
}
...
static int check_backup_id_func(struct batt_chk_rslt *result)
{
	...
	list_for_each_entry(temp, &batt_checkers_head, node) {
		checker_data = platform_get_drvdata(temp->pdev);
		if (!flag_ptr || !checker_data) {
			hwlog_err("%s ptr is null\n", __func__);
			return BACKUP_ID_CHECK_ERROR;
		}
		val = strsep(&flag_ptr, "\n");
		if (*val == '\0') {
		...
		if (!check_backup_id_validate(checker_data, val)) {
			checker_data->result.sn_status = SN_SNS_UNMATCH;
			*result = checker_data->result;
			return BACKUP_ID_CHECK_ERROR;
		}
...
static void batt_storage_delayed_update_func(struct work_struct *work)
{
	struct batt_checker_entry *temp = NULL;
	struct batt_chk_data *checker_data = NULL;
	...
	if (!dw) {
		hwlog_err("[%s]: dw is NULL\n", __func__);
		return;
	}

	drv_data = container_of(dw, struct batt_info, delayed_update_work);
	if (!drv_data) {
	...
	list_for_each_entry(temp, batt_checkers_head, node) {
		checker_data = platform_get_drvdata(temp->pdev);
		if (drv_data->panel_blank && checker_data->storage_info.cycle.update_flag) {
			hwlog_info("[%s] update battery cycle\n", __func__);
			update_battery_cycle(checker_data);
		}
		if (drv_data->panel_blank && checker_data->storage_info.real_soh.update_flag) {
			hwlog_info("[%s] update battery real soh\n", __func__);
			update_battery_real_soh(checker_data);
		}
		if (drv_data->panel_blank && checker_data->storage_info.display_soh.update_flag) {
			hwlog_info("[%s] update battery display soh\n", __func__);
			update_battery_display_soh(checker_data);
		}
	}
}

static int panel_blank_notifier_call(struct notifier_block *nb,
	unsigned long event, void *data)
{
	struct batt_info *drv_data = NULL;
	drv_data = container_of(nb, struct batt_info, panel_blank_listener);
	if (!drv_data)
		return -EINVAL;

	switch (event) {
	case POWER_NE_PANEL_BLANK:
		hwlog_info("received panel blank notifier\n");
		drv_data->panel_blank = true;
		schedule_delayed_work(&drv_data->delayed_update_work,
			msecs_to_jiffies(BLANK_UPDATE_WAIT_INTERVAL));
		break;
	case POWER_NE_PANEL_UNBLANK:
		hwlog_info("received panel unblank notifier\n");
		drv_data->panel_blank = false;
		cancel_delayed_work(&drv_data->delayed_update_work);
		break;
	default:
		return NOTIFY_OK;
	}
...
static void init_batt_storage_delayed_update(struct batt_info *drv_data)
{
	int ret;
	...
	if (!drv_data->delayed_update_enabled) {
		hwlog_info("[%s] batt_storage delayed update is disabled\n", __func__);
		return;
	}

	INIT_DELAYED_WORK(&drv_data->delayed_update_work, batt_storage_delayed_update_func);

	drv_data->panel_blank_listener.notifier_call = panel_blank_notifier_call;

	ret = power_event_bnc_register(POWER_BNT_PANEL_EVENT, &drv_data->panel_blank_listener);
	if (ret)
		(void)power_event_bnc_unregister(POWER_BNT_PANEL_EVENT, &drv_data->panel_blank_listener);
}
	...
	char *buf)
{
	...
	return snprintf(buf, PAGE_SIZE, "%u", drv_data->total_checkers);
}
...
static ssize_t battery_ct_shield_show(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	return snprintf(buf, PAGE_SIZE, "%u", shield_ct_sign);
}
...
static ssize_t batt_type_extend_show(struct device *dev, struct device_attribute *attr,
	char *buf)
{
	struct batt_checker_entry *temp = NULL;
	struct batt_chk_data *checker_data = NULL;
	int ret;
	int count = 0;
	unsigned int name_size = MAX_BATT_TYPE_EXTEND_LEN;
	char name[MAX_BATT_TYPE_EXTEND_LEN] = { 0 };

	list_for_each_entry(temp, &batt_checkers_head, node) {
		checker_data = platform_get_drvdata(temp->pdev);
		if ((!checker_data)) {
			count += snprintf_s(buf + count, PAGE_SIZE,
				PAGE_SIZE - count, "checker_data is NULL\n");
			return count;
		}
		ret = get_battery_type_by_id((unsigned int)checker_data->id_in_grp, name, name_size);
		if (ret != EOK) {
			count += snprintf_s(buf + count, PAGE_SIZE,
				PAGE_SIZE - count, "get_battery_type_by_id fail\n");
			return count;
		} else {
			count += snprintf_s(buf + count, PAGE_SIZE,
				PAGE_SIZE - count, "batt_sn: %s\n", checker_data->sn);
			count += snprintf_s(buf + count, PAGE_SIZE,
				PAGE_SIZE - count, "batt_index: %hhu  batt_type: %s\n",
				checker_data->id_in_grp, name);
		}
	}
	return count;
}
...
static DEVICE_ATTR_RO(batt_type_extend);
#endif /* BATTERY_LIMIT_DEBUG */
...
static const struct attribute *batt_info_attrs[] = {
	...
	&dev_attr_batt_type_extend.attr,
#endif
...
static int battct_generate_result_str(char *buff)
{
	...
	list_for_each_entry(temp, &batt_checkers_head, node) {
		...
		count += snprintf(buff + count, DMD_BUF_SIZE - count,
			...
			checker_data->id_of_grp);
		count += snprintf(buff + count, DMD_BUF_SIZE - count,
			"rlt:ic %d, key %d, sn %d Read:%s, retry_count %d\n",
			checker_data->result.ic_status,
			checker_data->result.key_status,
			checker_data->result.sn_status,
			is_sn_read(checker_data),
			check_certification_retry_count(checker_data));
	}
	return count;
}
...
static int batt_info_dts_init(struct platform_device *pdev)
{
	...
	bool delayed_update_enabled = false;
	int ret;
	...
	if (power_dts_read_u32(power_dts_tag(HWLOG_TAG), pdev->dev.of_node,
		"delayed-update-enabled", (u32 *)&delayed_update_enabled, 0)) {
		/* default disable storage_info delayed update */
		drv_data->delayed_update_enabled = false;
		hwlog_info("set default delayed_update_enabled false\n");
	} else {
		drv_data->delayed_update_enabled = delayed_update_enabled;
	}
...
static int battery_info_probe(struct platform_device *pdev)
{
	...
	init_batt_storage_delayed_update(drv_data);
