...
#define CURR_UUID_BIND_VER                   RAW_UUID_BOARD_BIND_VER
#define MAX_BATT_BIND_UUID_NUM               (MAX_BATT_BIND_NUM - 1)
...
static void batt_binder_init(void)
{
	...
	g_bind_info.bind_info.record.version = CURR_UUID_BIND_VER;
...
int batt_uuid_read_cb(unsigned char version, void *data, int len)
{
	...
	hwlog_info("battery bind uuid version is %u\n", g_bind_info.bind_info.record.version);
	if (memcmp(g_bind_info.bind_info.record.uuid[MAX_BATT_BIND_NUM - 1],
			board_info_cb_mesg[NEW_BOARD_MESG_INDEX], strlen(board_info_cb_mesg[NEW_BOARD_MESG_INDEX])) &&
		memcmp(g_bind_info.bind_info.record.uuid[MAX_BATT_BIND_NUM - 1],
			board_info_cb_mesg[OLD_BOARD_MESG_INDEX], strlen(board_info_cb_mesg[OLD_BOARD_MESG_INDEX]))) {
		g_bind_info.update_sign = 1;
		g_raw_record.version = 0; // force write bind_info to sec_stroage
		hwlog_info("invalid uuid[9], set update_sign to 1\n");
	}
	mutex_unlock(&g_bind_info.lock);
...
int batt_read_bind_mesg(void)
{
	int i;
	int left;
	int bind_ver = CURR_UUID_BIND_VER;
	...
	const char *uuid, int len)
{
	...
	if (memset_s(uuid_record->uuid[MAX_BATT_BIND_UUID_NUM], MAX_BATT_UUID_LEN, 0, MAX_BATT_UUID_LEN) != 0)
		hwlog_info("memset_s failed\n");
	uuid_record->version = CURR_UUID_BIND_VER;
	/* 1:enable update sign to refresh tee */
	g_bind_info.update_sign = 1;
	...
	const char *uuid, int len)
{
	...
	if ((drv_data->result.sn_status == SN_OBT_REMATCH) ||
		(drv_data->result.sn_status == SN_OBD_REMATCH) ||
		(drv_data->result.sn_status == SN_NN_REMATCH)) {
		for (i = MAX_BATT_BIND_UUID_NUM - 1; i > 0; i--)
			memcpy(uuid_record->uuid[i], uuid_record->uuid[i - 1],
				MAX_BATT_UUID_LEN);
