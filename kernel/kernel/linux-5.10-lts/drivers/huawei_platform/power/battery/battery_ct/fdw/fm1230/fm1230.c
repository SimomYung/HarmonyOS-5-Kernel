...
static int fm1230_cyclk_set(struct fm1230_dev *di)
{
	...
	struct fm1230_file_operation file_op = { FILE16, LOCK, 0 };

	if (!di)
		return -ENODEV;

	ret = fm1230_operate_file(di, &file_op, NULL, 0);
	if (ret) {
		hwlog_err("[%s] lock fail or is locked, ic_index:%u\n", __func__, di->ic_index);
		return -EAGAIN;
	}

	file_op.type = LOCK_STATUS;
	ret = fm1230_operate_file(di, &file_op, buf, LOCK_BUF_LEN);
	if (ret) {
...
{
	struct fm1230_dev *di = NULL;
	struct fm1230_file_operation file_op = { FILE12, READ, 0 };
	...
	if (fm1230_operate_file(di, &file_op, type_temp, FM1230_BATTTYP_LEN)) {
...
{
	struct fm1230_dev *di = NULL;
	struct fm1230_file_operation file_op = { FILE13, READ, 0 };
	int ret;
	...
	if (!di->mem.sn_ready) {
		...
		ret = fm1230_operate_file(di, &file_op, di->mem.sn, FM1230_SN_ASC_LEN);
		if (ret) {
...
static int fm1230_act_read(struct fm1230_dev *di)
{
	...
	struct fm1230_file_operation file_op = { FILE11, READ, 0 };

	if (fm1230_operate_file(di, &file_op, di->mem.act_sign, FM1230_ACT_LEN)) {
	...
	enum res_type type, const struct power_genl_attr *res)
{
	...
	struct fm1230_dev *di = NULL;
	struct fm1230_file_operation file_op = { FILE11, LOCK_STATUS, 0 };
	...
	case RES_ACT:
		file_op.type = WRITE;
		ret = fm1230_operate_file(di, &file_op, act, FM1230_ACT_LEN);
		if (ret) {
...
static int fm1230_write_group_sn(struct fm1230_dev *di, void *value)
{
	...
	struct fm1230_file_operation file_op = { FILE14, LOCK_STATUS, 0 };
	int ret;
	...
	ret = fm1230_operate_file(di, &file_op, arr, FM1230_IC_GROUP_SN_LENGTH);
	if (ret) {
...
static int fm1230_set_batt_org(struct fm1230_dev *di, void *value)
{
	int ret;
	unsigned char buf[LOCK_BUF_LEN] = { 0 };
	struct fm1230_file_operation file_op = { FILE16, LOCK_STATUS, 0 };
...
static int get_batt_match_info(struct fm1230_dev *di, void *value)
{
	...
	struct fm1230_file_operation file_op = { FILE14, READ, 0 };

	ret = fm1230_operate_file(di, &file_op, byte_group_sn, FM1230_IC_GROUP_SN_LENGTH);
	if (ret) {
...
static int get_batt_activation(struct fm1230_dev *di, struct batt_storage_info_data *data)
{
	int ret;
	unsigned char buf[BATT_ACTIVATION_LEN] = { 0 };
	struct fm1230_file_operation file_op = { FILE17, READ, BATT_ACTIVATION_OFFSET };
	...
	ret = fm1230_operate_file(di, &file_op, buf, BATT_ACTIVATION_LEN);
	if (ret) {
...
static int get_batt_cycle(struct fm1230_dev *di, struct batt_storage_info_data *data)
{
	int ret;
	unsigned char buf[BATT_CYCLE_LEN] = { 0 };
	struct fm1230_file_operation file_op = { FILE17, READ, BATT_CYCLE_OFFSET };
	...
	ret = fm1230_operate_file(di, &file_op, buf, BATT_CYCLE_LEN);
	if (ret) {
...
static int get_batt_real_soh(struct fm1230_dev *di, struct batt_storage_info_data *data)
{
	int ret;
	unsigned char buf[BATT_REAL_SOH_LEN] = { 0 };
	struct fm1230_file_operation file_op = { FILE17, READ, BATT_REAL_SOH_OFFSET };
	...
	ret = fm1230_operate_file(di, &file_op, buf, BATT_REAL_SOH_LEN);
	if (ret) {
...
static int get_batt_display_soh(struct fm1230_dev *di, struct batt_storage_info_data *data)
{
	int ret;
	unsigned char buf[BATT_DISPLAY_SOH_LEN] = { 0 };
	struct fm1230_file_operation file_op = { FILE17, READ, BATT_DISPLAY_SOH_OFFSET };
	...
	ret = fm1230_operate_file(di, &file_op, buf, BATT_DISPLAY_SOH_LEN);
	if (ret) {
...
static int get_batt_all_storage_info(struct fm1230_dev *di, struct batt_storage_info_data *data)
{
	int ret;
	int cycle_first_index;
	int cycle_second_index;
	unsigned char buf[BATT_STORAGE_INFO_LEN] = { 0 };
	struct fm1230_file_operation file_op = { FILE17, READ, 0 };
	...
	ret = fm1230_operate_file(di, &file_op, buf, BATT_STORAGE_INFO_LEN);
	if (ret) {
...
static int fm1230_get_batt_storage_info(struct platform_device *pdev,
...
static int set_batt_activation(struct fm1230_dev *di, struct batt_storage_info_data *data)
{
	int ret;
	unsigned char buf[BATT_ACTIVATION_LEN] = { 0 };
	struct fm1230_file_operation file_op = { FILE17, WRITE, BATT_ACTIVATION_OFFSET };
...
static int set_batt_cycle(struct fm1230_dev *di, struct batt_storage_info_data *data)
{
	int ret;
	unsigned char buf[BATT_CYCLE_LEN] = { 0 };
	struct fm1230_file_operation file_op = { FILE17, WRITE, BATT_CYCLE_OFFSET };
...
static int set_batt_real_soh(struct fm1230_dev *di, struct batt_storage_info_data *data)
{
	int ret;
	unsigned char buf[BATT_REAL_SOH_LEN] = { 0 };
	struct fm1230_file_operation file_op = { FILE17, WRITE, BATT_REAL_SOH_OFFSET };
...
static int set_batt_display_soh(struct fm1230_dev *di, struct batt_storage_info_data *data)
{
	int ret;
	unsigned char buf[BATT_DISPLAY_SOH_LEN] = { 0 };
	struct fm1230_file_operation file_op = { FILE17, WRITE, BATT_DISPLAY_SOH_OFFSET };
...
static int fm1230_set_batt_storage_info(struct platform_device *pdev,
	struct batt_storage_info_data *data, enum batt_storage_info_type type)
{
	struct fm1230_dev *di = NULL;
...
static int fm1230_ecc_ex_check(struct platform_device *pdev,
	struct power_genl_attr *key_res)
{
	hwlog_info("[%s] user mode prohibit this ops\n", __func__);
	return 0;
}
#endif /* BATTBD_FORCE_MATCH */
	...
	struct batt_ct_ops *bco)
{
	...
	bco->set_batt_storage_info = fm1230_set_batt_storage_info;
	bco->get_batt_storage_info = fm1230_get_batt_storage_info;
	...
	uint8_t *group_sn)
{
	int ret;
	struct fm1230_dev *di = platform_get_drvdata(pdev);
	struct fm1230_file_operation file_op = { FILE14, WRITE, 0 };
	...
	ret = fm1230_operate_file(di, &file_op, buf, FM1230_IC_GROUP_SN_LENGTH);
	if (ret) {
		hwlog_err("[%s] write group sn fail, ic_index:%u\n", __func__, di->ic_index);
		return -EINVAL;
	}
		...
		char *buf)
{
	...
	bat_type_apply_mode(BAT_ID_SN);
	ret = fm1230_operate_file(di, &file_op, type_temp, FM1230_BATTTYP_LEN);
	bat_type_release_mode(true);
	if (ret) {
		hwlog_err("[%s] read battery type err\n", __func__);
		...
		char *buf)
{
	...
	bat_type_apply_mode(BAT_ID_SN);
	ret = fm1230_operate_file(di, &file_op, sn, FM1230_SN_ASC_LEN);
	bat_type_release_mode(true);
	if (ret) {
	...
	struct device_attribute *attr, const char *buf, size_t count)
{
	...
	ret = memcpy_s(group_sn, FM1230_SN_ASC_LEN, buf, count);
	if (ret != EOK) {
		...
		char *buf)
{
	...
	bat_type_apply_mode(BAT_ID_SN);
	ret = fm1230_operate_file(di, &file_op, cert_status_buf, LOCK_BUF_LEN);
	bat_type_release_mode(true);
	if (ret) {
		hwlog_err("[%s] check lock state fail, ic_index:%u\n", __func__, di->ic_index);
