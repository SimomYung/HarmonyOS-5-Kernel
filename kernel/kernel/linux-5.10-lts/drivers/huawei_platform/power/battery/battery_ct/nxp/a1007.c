	...
	const uint8_t *buf)
{
	...
	cpu_latency_qos_add_request(&drv_data->pm_qos, 0);
	kick_all_cpus_sync();
	...
	cpu_latency_qos_remove_request(&drv_data->pm_qos);
...
static void power_down(struct platform_device *pdev)
{
	...
	power_wakeup_unlock(drv_data->wlock, true);
	mutex_unlock(&drv_data->ops_mutex);
}
...
static struct a1007_des *a1007_init_drive_data(struct platform_device *pdev)
{
	...
	drv_data->wlock = power_wakeup_source_register(&pdev->dev, pdev->name);
