...
{
	struct t91407_dev *di = NULL;
	struct t91407_file_operation file_op = { T91407_FILE_BATTTYPE, T91407_READ, 0 };
	...
	if (!di->mem.batt_type_ready) {
		if (t91407_operate_file(di, &file_op, type_temp, T91407_BATTTYP_LEN)) {
...
{
	struct t91407_dev *di = NULL;
	struct t91407_file_operation file_op = { T91407_FILE_SN, T91407_READ, 0 };
	int ret;
	...
	if (!di->mem.sn_ready) {
		...
		ret = t91407_operate_file(di, &file_op, di->mem.sn, T91407_SN_ASC_LEN);
		if (ret) {
...
static int t91407_get_certification_retry_count(struct platform_device *pdev)
{
	int ret = 0;
	struct t91407_dev *di = NULL;
	...
	di = platform_get_drvdata(pdev);
	if (di->mem.retry_count_max)
		ret = di->mem.retry_count_max;
	return ret;
}
...
static int t91407_act_read_retry(struct t91407_dev *di)
{
	int ret = 0;
	uint16_t crc_act_read = 0;
	uint16_t crc_act_cal = 0;
	struct t91407_file_operation file_op = { T91407_FILE_ACT, T91407_READ, 0 };

	if (di->mem.act_sign_ready) {
		hwlog_info("[%s] act_sign_ready, crc ret succ\n", __func__);
		return ret;
	}

	if (t91407_operate_file(di, &file_op, di->mem.act_sign, T91407_ACT_LEN)) {
...
static int t91407_act_read(struct t91407_dev *di)
{
	...
	for (uint8_t retry = 0; retry < T91407_ACT_READ_RETRY_COUNT; retry++) {
		ret = t91407_act_read_retry(di);
		if (!ret)
			break;
	}
	return ret;
	...
	enum res_type type, const struct power_genl_attr *res)
{
	...
	struct t91407_dev *di = NULL;
	struct t91407_file_operation file_op = { T91407_FILE_ACT, T91407_LOCK_STATUS, 0 };
	...
	ret = t91407_operate_file(di, &file_op, buf, T91407_LOCK_BUF_LEN);
	if (ret)
	...
	switch (type) {
	case RES_ACT:
		file_op.type = T91407_WRITE;
		ret = t91407_operate_file(di, &file_op, act, T91407_ACT_LEN);
		if (ret)
			goto act_sig_set_fail;
...
static int t91407_cyclk_set(struct t91407_dev *di)
{
	int ret;
	unsigned char buf[T91407_LOCK_BUF_LEN] = { 0 };
	struct t91407_file_operation file_op = { T91407_FILE_LOCK, T91407_LOCK, 0 };

	if (!di)
		return -ENODEV;

	ret = t91407_operate_file(di, &file_op, NULL, 0);
	if (ret) {
		hwlog_err("[%s] lock fail or is locked, ic_index:%u\n", __func__, di->ic_index);
		return -EAGAIN;
	}

	file_op.type = T91407_LOCK_STATUS;
	ret = t91407_operate_file(di, &file_op, buf, T91407_LOCK_BUF_LEN);
	if (ret) {
...
static int t91407_set_batt_org(struct t91407_dev *di, void *value)
{
	int ret;
	unsigned char buf[T91407_LOCK_BUF_LEN] = { 0 };
	struct t91407_file_operation file_op = { T91407_FILE_LOCK, T91407_LOCK_STATUS, 0 };
...
static int t91407_write_group_sn(struct t91407_dev *di, void *value)
{
	...
	struct t91407_file_operation file_op = { T91407_FILE_GSN, T91407_LOCK_STATUS, 0 };
	int ret;
	...
	ret = t91407_operate_file(di, &file_op, arr, T91407_IC_GROUP_SN_LENGTH);
	if (ret) {
...
static int t91407_get_batt_match_info(struct t91407_dev *di, void *value)
{
	...
	struct t91407_file_operation file_op = { T91407_FILE_GSN, T91407_READ, 0 };

	if (!di->mem.group_sn_ready) {
		ret = t91407_operate_file(di, &file_op, byte_group_sn, T91407_IC_GROUP_SN_LENGTH);
		if (ret) {
...
static int t91407_get_batt_activation(struct t91407_dev *di, struct batt_storage_info_data *data)
{
	int ret;
	u8 buf[BATT_ACTIVATION_LEN] = { 0 };
	struct t91407_file_operation file_op = { T91407_FILE_STORAGE, T91407_READ, BATT_ACTIVATION_OFFSET };
	...
	ret = t91407_operate_file(di, &file_op, buf, BATT_ACTIVATION_LEN);
	if (ret != EOK) {
		hwlog_err("[%s] read activation info from ic fail, ic_index:%u\n", __func__, di->ic_index);
		return -EINVAL;
	}
...
static int t91407_get_batt_cycle(struct t91407_dev *di, struct batt_storage_info_data *data)
{
	int ret;
	u8 buf[BATT_CYCLE_LEN] = { 0 };
	struct t91407_file_operation file_op = { T91407_FILE_STORAGE, T91407_READ, BATT_CYCLE_OFFSET };
	...
	ret = t91407_operate_file(di, &file_op, buf, BATT_CYCLE_LEN);
	if (ret != EOK) {
		hwlog_err("[%s] read cycle from ic fail, ic_index:%u\n", __func__, di->ic_index);
		return -EINVAL;
	}

	*(data->cycle) = ((u16)buf[BUF_FIRST_INDEX] << BITS_PER_BYTE) | buf[BUF_SECOND_INDEX];

	return 0;
}

static int t91407_get_batt_real_soh(struct t91407_dev *di, struct batt_storage_info_data *data)
{
	int ret;
	u8 buf[BATT_REAL_SOH_LEN] = { 0 };
	struct t91407_file_operation file_op = { T91407_FILE_STORAGE, T91407_READ, BATT_REAL_SOH_OFFSET };
	...
	ret = t91407_operate_file(di, &file_op, buf, BATT_REAL_SOH_LEN);
	if (ret != EOK) {
		hwlog_err("[%s] read batt real soh from ic fail, ic_index:%u\n", __func__, di->ic_index);
		return -EINVAL;
	}

	*(data->real_soh) = buf[BUF_FIRST_INDEX];

	return 0;
}

static int t91407_get_batt_display_soh(struct t91407_dev *di, struct batt_storage_info_data *data)
{
	int ret;
	u8 buf[BATT_DISPLAY_SOH_LEN] = { 0 };
	struct t91407_file_operation file_op = { T91407_FILE_STORAGE, T91407_READ, BATT_DISPLAY_SOH_OFFSET };
	...
	ret = t91407_operate_file(di, &file_op, buf, BATT_DISPLAY_SOH_LEN);
	if (ret != EOK) {
		hwlog_err("[%s] read batt display soh from ic fail, ic_index:%u\n", __func__, di->ic_index);
		return -EINVAL;
	}

	*(data->display_soh) = buf[BUF_FIRST_INDEX];

	return 0;
}

static int t91407_get_batt_all_storage_info(struct t91407_dev *di, struct batt_storage_info_data *data)
{
	int ret;
	...
	ret = t91407_operate_file(di, &file_op, buf, BATT_STORAGE_INFO_LEN);
	if (ret != EOK) {
...
static int t91407_get_batt_storage_info(struct platform_device *pdev,
	struct batt_storage_info_data *data, enum batt_storage_info_type type)
{
	...
	case BATT_ACTIVATION:
		if (t91407_get_batt_activation(di, data)) {
			hwlog_err("[%s] read batt activation fail, ic_index:%u\n", __func__, di->ic_index);
			return -EINVAL;
		}
		break;
	case BATT_CYCLE:
		if (t91407_get_batt_cycle(di, data)) {
			hwlog_err("[%s] read batt cycle fail, ic_index:%u\n", __func__, di->ic_index);
			return -EINVAL;
		}
		break;
	case BATT_REAL_SOH:
		if (t91407_get_batt_real_soh(di, data)) {
			hwlog_err("[%s] read batt real soh fail, ic_index:%u\n", __func__, di->ic_index);
			return -EINVAL;
		}
		break;
	case BATT_DISPLAY_SOH:
		if (t91407_get_batt_display_soh(di, data)) {
			hwlog_err("[%s] read batt display soh fail, ic_index:%u\n", __func__, di->ic_index);
			return -EINVAL;
		}
		break;
	case BATT_ALL_STORAGE_INFO:
		if (t91407_get_batt_all_storage_info(di, data)) {
			hwlog_err("[%s] read all batt storage info fail, ic_index:%u\n", __func__, di->ic_index);
			return -EINVAL;
		}
		break;
	default:
...
static int t91407_set_batt_activation(struct t91407_dev *di, struct batt_storage_info_data *data)
{
	int ret;
	u8 buf[BATT_ACTIVATION_LEN] = { 0 };
	struct t91407_file_operation file_op = { T91407_FILE_STORAGE, T91407_WRITE, BATT_ACTIVATION_OFFSET };
...
static int t91407_set_batt_cycle(struct t91407_dev *di, struct batt_storage_info_data *data)
{
	int ret;
	u8 buf[BATT_CYCLE_LEN] = { 0 };
	struct t91407_file_operation file_op = { T91407_FILE_STORAGE, T91407_WRITE, BATT_CYCLE_OFFSET };
	...
	buf[BUF_SECOND_INDEX] = (u8)(*(data->cycle) & BYTE_MASK);
	buf[BUF_FIRST_INDEX] = (u8)((*(data->cycle) >> BITS_PER_BYTE) & BYTE_MASK);
...
static int t91407_set_batt_real_soh(struct t91407_dev *di, struct batt_storage_info_data *data)
{
	int ret;
	u8 buf[BATT_REAL_SOH_LEN] = { 0 };
	struct t91407_file_operation file_op = { T91407_FILE_STORAGE, T91407_WRITE, BATT_REAL_SOH_OFFSET };
	...
	buf[BUF_FIRST_INDEX] = *(data->real_soh);
...
static int t91407_set_batt_display_soh(struct t91407_dev *di, struct batt_storage_info_data *data)
{
	int ret;
	u8 buf[BATT_DISPLAY_SOH_LEN] = { 0 };
	struct t91407_file_operation file_op = { T91407_FILE_STORAGE, T91407_WRITE, BATT_DISPLAY_SOH_OFFSET };
	...
	buf[BUF_FIRST_INDEX] = *(data->display_soh);
...
static int t91407_set_batt_storage_info(struct platform_device *pdev,
	struct batt_storage_info_data *data, enum batt_storage_info_type type)
{
	struct t91407_dev *di = NULL;
	...
	switch (type) {
	case BATT_ACTIVATION:
		if (t91407_set_batt_activation(di, data)) {
			hwlog_err("[%s] write batt activation fail, ic_index:%u\n", __func__, di->ic_index);
			return -EINVAL;
		}
		break;
	case BATT_CYCLE:
		if (t91407_set_batt_cycle(di, data)) {
			hwlog_err("[%s] write batt cycle fail, ic_index:%u\n", __func__, di->ic_index);
			return -EINVAL;
		}
		break;
	case BATT_REAL_SOH:
		if (t91407_set_batt_real_soh(di, data)) {
			hwlog_err("[%s] write batt real soh fail, ic_index:%u\n", __func__, di->ic_index);
			return -EINVAL;
		}
		break;
	case BATT_DISPLAY_SOH:
		if (t91407_set_batt_display_soh(di, data)) {
	...
	struct batt_ct_ops *bco)
{
	...
	bco->set_batt_storage_info = t91407_set_batt_storage_info;
	bco->get_batt_storage_info = t91407_get_batt_storage_info;
	bco->set_ic_status = t91407_set_ic_status;
	bco->power_down = NULL;
	bco->get_certification_retry_count = t91407_get_certification_retry_count;
	return 0;
}
	...
	uint8_t *group_sn)
{
	int ret;
	struct t91407_dev *di = platform_get_drvdata(pdev);
	struct t91407_file_operation file_op = { T91407_FILE_GSN, T91407_WRITE, 0 };
	...
	ret = t91407_operate_file(di, &file_op, buf, T91407_IC_GROUP_SN_LENGTH);
	if (ret) {
		hwlog_err("[%s] write group sn fail, ic_index:%u\n", __func__, di->ic_index);
		return -EINVAL;
	}
	...
	char *buf)
{
	...
	if (!di->mem.batt_type_ready) {
		bat_type_apply_mode(BAT_ID_SN);
		ret = t91407_operate_file(di, &file_op, type_temp, T91407_BATTTYP_LEN);
		bat_type_release_mode(true);
		if (ret) {
			hwlog_err("[%s] read battery type err\n", __func__);
	...
	char *buf)
{
	...
	bat_type_apply_mode(BAT_ID_SN);
	ret = t91407_operate_file(di, &file_op, cert_status_buf, T91407_LOCK_BUF_LEN);
	bat_type_release_mode(true);
	if (ret) {
