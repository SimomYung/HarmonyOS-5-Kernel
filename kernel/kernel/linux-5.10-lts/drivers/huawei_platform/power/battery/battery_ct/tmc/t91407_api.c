...
static const struct t91407_file_start_page g_page_no[] = {
	...
	{T91407_FILE_STORAGE, T91407_FILE_STORAGE_PAGE, T91407_FILE_STORAGE_NUM},
};
...
static bool need_transmit_continue(int ret_code, uint8_t retry_count)
{
	if (((ret_code == T91407_ERR_READ_FAILED) || (ret_code == T91407_ERR_SWI_FORMAT) ||
		(ret_code == T91407_ERR_CRC_CHIP) || (ret_code == T91407_ERR_CRC_VERIGY)) &&
		(retry_count < (T91407_MAX_PACKAGE_RETRY_COUNT - 1))) {
	...
	unsigned char send_length, int receive_length)
{
	...
	for (i = 0; i < send_length; i++) {
		...
		for (j = 0, counter = 0; j < T91407_LEN_OF_DATA; j++) {
			counter += (data & 0x01);
			data >>= 1;
		}
		data = send[i];
...
static void t91407_power_on(struct t91407_dev *di)
{
	...
	t91407_udelay(T91407_SWI_DELAY_POWERUP_450US);
...
static int t91407_cmd_receive(struct t91407_dev *di, unsigned char *receive, int receive_length)
{
	...
	response_length = (unsigned int)(receive_length) + 1;
	...
	unsigned char *cmd_header, int data_length, int receive_length)
{
	...
	send_length = (unsigned int)(data_length) + T91407_LEN_OF_SENDING_FORMAT; /* 发送格式分包指令0xAA和1位CRC校验 */
	...
	unsigned short address, uint8_t length)
{
	...
	for (retry = 0; retry < T91407_MAX_PACKAGE_RETRY_COUNT; retry++) {
		local_irq_save(di->irq_flags);
...
{
	uint8_t i;
	uint8_t page_count = (uint8_t)(ARRAY_SIZE(g_page_no));

	for (i = 0; i < ARRAY_SIZE(g_page_no); i++) {
...
static int t91407_read_file(struct t91407_dev *di, int fid, unsigned char *buf, int size, int offset)
{
	int ret;
	uint8_t i;
	...
	if (size % T91407_BYTE_OF_ONE_PAGE)
		page_count += 1;
	for (i = 0; i < T91407_MAX_COMMAND_RETRY_COUNT; i++) {
		for (uint8_t page_no = start_page; page_no < (start_page + page_count); page_no++) {
			if (page_no < (start_page + (size / T91407_BYTE_OF_ONE_PAGE)))
				ret = t91407_read_page_data(di, &rbuf[(page_no - start_page) * T91407_BYTE_OF_ONE_PAGE],
					page_no, offset, T91407_BYTE_OF_ONE_PAGE);
			else
				ret = t91407_read_page_data(di, &rbuf[(page_no - start_page) * T91407_BYTE_OF_ONE_PAGE],
					page_no, offset, (size % T91407_BYTE_OF_ONE_PAGE));
			if (ret)
				break;
		}
		if (!ret) {
			hwlog_err("[%s] T91407_MAX_COMMAND_RETRY_COUNT : %d\n", __func__, i);
			break;
		}
		set_sched_affinity_to_all();
...
static int t91407_write_file(struct t91407_dev *di, int fid, unsigned char *buf, int size, int offset)
{
	int ret;
	uint8_t i;
	...
	if (size % T91407_BYTE_OF_ONE_PAGE)
		page_count += 1;
	for (i = 0; i < T91407_MAX_COMMAND_RETRY_COUNT; i++) {
		for (page_no = start_page; page_no < (start_page + page_count); page_no++) {
			if (page_no < (start_page + (size / T91407_BYTE_OF_ONE_PAGE)))
...
static int t91407_get_file_lock_state(struct t91407_dev *di, int fid, unsigned char *buf, int buf_len)
{
	...
	for (k = start_page; k < (uint8_t)(start_page + page_num); k++) {
...
static int t91407_lock_file(struct t91407_dev *di, int fid)
{
	...
	ret = find_fid(&start_page, &page_num, fid);
	end_page = (uint8_t)(start_page + page_num);
	if (ret)
		return T91407_ERR_WRONG_PARAMETER;
...
int t91407_operate_file(struct t91407_dev *di, struct t91407_file_operation *file_op,
	unsigned char *buf, int buf_len)
{
	...
	switch (file_op->type) {
	case T91407_READ:
		ret = t91407_read_file(di, file_op->file, buf, buf_len, file_op->offset);
		break;
	case T91407_WRITE:
		ret = t91407_write_file(di, file_op->file, buf, buf_len, file_op->offset);
		break;
	case T91407_LOCK:
		ret = t91407_lock_file(di, file_op->file);
		break;
	case T91407_LOCK_STATUS:
		ret = t91407_get_file_lock_state(di, file_op->file, buf, buf_len);
		break;
	default:
...
static int t91407_read_certification(struct t91407_dev *di, unsigned char* certification, int length)
{
	...
	if (length != T91407_CERTIFICATION_LEN) {
		hwlog_err("[%s] Params error\n", __func__);
		return T91407_ERR_WRONG_PARAMETER;
	}
	...
	const unsigned char *cert_public_key_256, int pubkey_length)
{
	...
	if ((key_length != T91407_PUBKEY_LEN) || (cert_length != T91407_CERTIFICATION_LEN) ||
...
static int t91407_send_random(struct t91407_dev *di, unsigned char* random, int length)
{
	...
	page_count = length / T91407_BYTE_OF_ONE_PAGE;
	for (page_no = 0; page_no < page_count; page_no++) {
		for (retry = 0; retry < T91407_MAX_PACKAGE_RETRY_COUNT; retry++) {
			local_irq_save(di->irq_flags);
			...
			if (!ret)
				break;
			if (need_transmit_continue(ret, retry)) {
				hwlog_err("[%s] t91407_write_data_by_address failed continue: %d\n", __func__, ret);
				continue;
			}
			hwlog_err("[%s] t91407_write_data_by_address failed: %d\n", __func__, ret);
			return ret;
		}
	}
...
static int t91407_get_sign(struct t91407_dev *di, unsigned char *sign, int length)
{
	...
	uint8_t page_no;
	uint8_t retry;
	uint8_t rbuf[T91407_READ_RECV_MAX_LEN] = { 0 };
	...
	for (page_no = 0; page_no < page_count; page_no++) {
		for (retry = 0; retry < T91407_MAX_PACKAGE_RETRY_COUNT; retry++) {
			...
			if (!ret)
				break;
			if (need_transmit_continue(ret, retry)) {
				hwlog_err("[%s] t91407_read_data_by_address failed continue: %d\n", __func__, ret);
				continue;
			}
	...
	unsigned char *sign_rand)
{
	...
	t91407_dev_on(di);
	for (i = 0; i < T91407_MAX_PACKAGE_RETRY_COUNT; i++) {
		if (i) {
			local_irq_save(di->irq_flags);
			t91407_power_on(di);
			local_irq_restore(di->irq_flags);
...
{
	int ret;
	unsigned char certification[T91407_CERTIFICATION_LEN] = { 0 };
	...
	t91407_dev_on(di);
	ret = t91407_read_certification(di, certification, T91407_CERTIFICATION_LEN);
	t91407_dev_off(di);
	if (ret)
		return -EAGAIN;
	ret = t91407_verify_certification(T91407_PUBKEY_LEN, certification, T91407_CERTIFICATION_LEN,
...
int t91407_do_authentication(struct t91407_dev *di, struct power_genl_attr *key_res)
{
	...
	for (uint8_t retry = 0; retry < T91407_MAX_AUTHENTICATION_RETRY_COUNT; retry++) {
		...
		if (!ret) {
			di->mem.retry_count_max = retry;
			hwlog_info("[%s] t91407_do_authentication success, %d\n", __func__, retry);
			break;
		}
		msleep(T91407_SWI_DELAY_COMMAND_RETRY_MS);
	}
