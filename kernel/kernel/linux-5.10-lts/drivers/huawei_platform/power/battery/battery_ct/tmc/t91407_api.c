	...
	unsigned char data, int receive_length)
{
	...
	/* Bus Command Whitelist */
	const unsigned char bus_cmd[] = {T91407_SWI_SFCD, T91407_SWI_BRES, T91407_SWI_SPDN, T91407_SWI_ECCE};
	size_t bus_cmd_length = sizeof(bus_cmd) / sizeof(bus_cmd[0]);
	size_t i = 0;
	int ret;
	...
	/* check bus cmd */
	for (i = 0; i < bus_cmd_length; i++) {
		if (data == bus_cmd[i])
			break;
	}
	if (i >= bus_cmd_length) {
		hwlog_err("[%s]  check Bus CMD data fail, data (hex) is 0x%02x\n", __func__, data);
		return T91407_ERR_WRONG_PARAMETER;
	}
