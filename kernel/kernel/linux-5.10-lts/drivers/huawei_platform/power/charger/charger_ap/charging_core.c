...
static void charge_core_iterm_handler(int temp,
	struct charge_iterm_data iterm_para[], struct charge_core_data *data)
{
	int i;

	for (i = 0; i < TEMP_PARA_LEVEL; i++) {
		if ((temp >= iterm_para[i].itemp_min) &&
			(temp < iterm_para[i].itemp_max))
			data->iterm = iterm_para[i].iterm_temp;
	}
}
...
static void charge_core_soh_handler(struct charge_core_data *data)
{
	...
	if (data->vterm_change_with_temp_en && (data->vterm_dec > 0)) {
		data->vterm -= data->vterm_dec;
		hwlog_info("%s after vterm dec, vterm=%u, vterm_dec=%u\n", __func__, data->vterm,
			data->vterm_dec);
	}
...
struct charge_core_data *charge_core_get_params(void)
{
	...
	if (di->iterm_table_parse_ok)
		charge_core_iterm_handler(tbatt, di->iterm_para, &di->data);
...
static void charge_core_parse_dts_iterm_para(struct charge_core_info *di)
{
	int i, row, col, len;
	int tdata[TEMP_PARA_LEVEL * ITERM_PARA_TOTAL] = { 0 };

	len = power_dts_read_string_array(power_dts_tag(HWLOG_TAG), di->dev->of_node,
		"iterm_table", tdata, TEMP_PARA_LEVEL, ITERM_PARA_TOTAL);
	if (len < 0) {
		hwlog_err("%s, parse_iterm_para fail\n", __func__);
		return;
	}

	for (row = 0; row < len / ITERM_PARA_TOTAL; row++) {
		col = row * ITERM_PARA_TOTAL + ITERM_PARA_TEMP_MIN;
		di->iterm_para[row].itemp_min = tdata[col];
		col = row * ITERM_PARA_TOTAL + ITERM_PARA_TEMP_MAX;
		di->iterm_para[row].itemp_max = tdata[col];
		col = row * ITERM_PARA_TOTAL + ITERM_PARA_TEMP_ICHG;
		di->iterm_para[row].iterm_temp = tdata[col];
	}

	for (i = 0; i < len / ITERM_PARA_TOTAL; i++)
		hwlog_info("dts_iterm_para[%d] %d %d %d\n",
			i, di->iterm_para[i].itemp_min, di->iterm_para[i].itemp_max,
			di->iterm_para[i].iterm_temp);
	di->iterm_table_parse_ok = 1;
}
...
static int charge_core_parse_battery_data(struct charge_core_info *di)
{
	...
	charge_core_parse_dts_iterm_para(di);
...
static int charge_core_parse_vdpm_para(struct device_node *np, struct charge_core_info *di)
{
	...
	if ((array_len <= 0) || (array_len % VDPM_PARA_TOTAL != 0)) {
		hwlog_err("vdpm_para is invalid,please check vdpm_para number\n");
		return -EINVAL;
	}
...
static int charging_core_parse_iterm(struct charge_core_info *di, struct device_node *np)
{
	...
	if ((array_len <= 0) || (array_len % CHARGE_ITERM_PARA_TOTAL != 0)) {
		hwlog_err("iterm_para is invalid, not necessay, please check\n");
		return 0;
	}
...
static int charge_core_parse_basic_para(struct device_node *np, struct charge_core_info *di)
{
	/* default iin_ac 2000mA */
	if (power_dts_read_u32(power_dts_tag(HWLOG_TAG), np, "iin_ac", &(di->data.iin_ac), 2000))
		return -EINVAL;

	/* default ichg_ac 2000mA */
	if (power_dts_read_u32(power_dts_tag(HWLOG_TAG), np, "ichg_ac", &(di->data.ichg_ac), 2000))
		return -EINVAL;

	/* default battery cell num: 1 */
	...
	power_dts_read_u32(power_dts_tag(HWLOG_TAG), np, "iin_fcp", &(di->data.iin_fcp),
		di->data.iin_ac);
	power_dts_read_u32(power_dts_tag(HWLOG_TAG), np, "ichg_fcp", &(di->data.ichg_fcp),
		di->data.ichg_ac);

	/* default iin_usb 500mA */
	if (power_dts_read_u32(power_dts_tag(HWLOG_TAG), np, "iin_usb", &(di->data.iin_usb), 500))
		return -EINVAL;

	/* default ichg_usb 500mA */
	if (power_dts_read_u32(power_dts_tag(HWLOG_TAG), np, "ichg_usb", &(di->data.ichg_usb), 500))
		return -EINVAL;

	/* default iin_nonstd 1200mA */
	if (power_dts_read_u32(power_dts_tag(HWLOG_TAG), np, "iin_nonstd",
		&(di->data.iin_nonstd), 1200))
		return -EINVAL;

	/* default ichg_nonstd 1000mA */
	if (power_dts_read_u32(power_dts_tag(HWLOG_TAG), np, "ichg_nonstd",
		&(di->data.ichg_nonstd), 1000))
		return -EINVAL;

	power_dts_read_u32(power_dts_tag(HWLOG_TAG), np, "iin_nor_scp",
		&(di->data.iin_nor_scp), di->data.iin_ac);
	power_dts_read_u32(power_dts_tag(HWLOG_TAG), np, "ichg_nor_scp",
		&(di->data.ichg_nor_scp), di->data.ichg_ac);

	/* default iin_bc_usb 1500mA */
	if (power_dts_read_u32(power_dts_tag(HWLOG_TAG), np, "iin_bc_usb",
		&(di->data.iin_bc_usb), 1500))
		return -EINVAL;

	/* default ichg_bc_usb 1400mA */
	if (power_dts_read_u32(power_dts_tag(HWLOG_TAG), np, "ichg_bc_usb",
		&(di->data.ichg_bc_usb), 1400))
		return -EINVAL;

	/* default iin_vr 1000mA */
	if (power_dts_read_u32(power_dts_tag(HWLOG_TAG), np, "iin_vr", &(di->data.iin_vr), 1000))
		return -EINVAL;

	/* default ichg_vr 1000mA */
	if (power_dts_read_u32(power_dts_tag(HWLOG_TAG), np, "ichg_vr", &(di->data.ichg_vr), 1000))
		return -EINVAL;

	power_dts_read_u32(power_dts_tag(HWLOG_TAG), np, "iin_weaksource",
		&(di->data.iin_weaksource), INVALID_CURRENT_SET);
	power_dts_read_u32(power_dts_tag(HWLOG_TAG), np, "vterm_change_with_temp_en",
		&(di->data.vterm_change_with_temp_en), 0);
	return 0;
}
	...
	struct charge_core_info *di)
{
	...
	if (charge_core_parse_basic_para(np, di))
		return -EINVAL;
	...
	if ((array_len <= 0) || (array_len % INDUCTANCE_PARA_TOTAL != 0)) {
		hwlog_err("inductance_para is invalid,please check inductance_para number\n");
		return -EINVAL;
	}
