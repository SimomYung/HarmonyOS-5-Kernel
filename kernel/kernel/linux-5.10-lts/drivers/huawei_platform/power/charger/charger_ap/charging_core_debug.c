...
#define IIN_BC_USB_CURR_MIN 0
#define IIN_BC_USB_CURR_MAX 1500
...
static ssize_t charging_core_dbg_iin_bc_usb_store(void *dev_data,
	const char *buf, size_t size)
{
	int iin_bc_usb = 0;
	int ret;
	...
	ret = kstrtoint(buf, 0, &iin_bc_usb);
	if (ret) {
		hwlog_err("unable to parse input:%s\n", buf);
		return -EINVAL;
	}
	if ((iin_bc_usb < IIN_BC_USB_CURR_MIN) || (iin_bc_usb > IIN_BC_USB_CURR_MAX)) {
		hwlog_err("invalid input value:%d\n", iin_bc_usb);
		return -EINVAL;
	}
	di->data.iin_bc_usb = iin_bc_usb;

	return size;
}

static ssize_t charging_core_dbg_iin_bc_usb_show(void *dev_data,
	char *buf, size_t size)
{
	struct charge_core_info *di = dev_data;
	...
	return scnprintf(buf, size, "iin_bc_usb is %d\n", di->data.iin_bc_usb);
}
...
void charging_core_dbg_register(struct charge_core_info *di)
{
	...
	power_dbg_ops_register("charging_core", "iin_bc_usb", (void *)di,
		charging_core_dbg_iin_bc_usb_show,
		charging_core_dbg_iin_bc_usb_store);
}
