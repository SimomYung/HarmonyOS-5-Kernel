...
#include <huawei_platform/power/huawei_charger_check.h>
...
#define CHARGE_EUROPEAN_RESTORE_SOC         95
#define CHARGE_EUROPEAN_RESTORE_VBAT        4000
...
struct eco_design_para {
	u32 is_support_eco_design;
	u32 eco_design_restore_soc_dts;
	u32 eco_design_restore_vbat_dts;
	int last_soc;
	int has_discharged;
	struct notifier_block soc_change_nb;
	struct work_struct soc_change_work;
};
...
static struct eco_design_para g_eco_design_para;
...
void huawei_charger_set_iin(unsigned int chg_type, unsigned int iin, bool is_mod_work)
{
	struct charge_device_info *di = g_di;

	if (!di) {
		hwlog_err("set_sdp_iin di is null\n");
		return;
	}

	switch (chg_type) {
	case CHARGER_TYPE_SDP:
		di->iin_sdp_config = iin;
		hwlog_info("[%s] iin_sdp_config=%dmA\n", __func__, di->iin_sdp_config);
		break;
	default:
		break;
	}
...
{
	if (charge_context(init_data).charger_type == CHARGER_TYPE_WIRELESS)
		return (charge_context(init_data).vbus == ADAPTER_5V) ?
...
static void huawei_charge_force_into_sleep(void)
{
	...
	hwlog_info("force charge wake unlock\n");
}
...
void huawei_charger_recv_pd_src_cap(enum charge_pd_src_cap src_cap)
{
	const char *buck_dev_name = NULL;

	switch (src_cap) {
	case CHARGE_PD_SRC_CAP_CCH_COMP_MAYBE_LOW:
		adpt_intfr_recv_cch_comp_maybe_low();
		break;
	case CHARGE_PD_SRC_CAP_MAYBE_DISABLE_9V:
		buck_dev_name = charge_get_dev_name();
		if (buck_dev_name && power_strsame(buck_dev_name, CHARGE_DEV_SCHARGERV800))
			charge_set_reset_adapter_source(RESET_ADAPTER_SET_MODE, RESET_ADAPTER_SOURCE_DISABLE_9V);
		break;
	default:
		break;
	}
...
static int support_fcp_detect(void)
{
...
/* disable pd for using typeC adapter to do fcp charge mode test */
static void fcp_disable_pd(void)
{
	if (!pd_dpm_get_ctc_cable_flag() || !power_cmdline_is_factory_mode())
		return;

	adapter_set_usbpd_enable(ADAPTER_PROTOCOL_FCP, 0, false);
}
...
static bool fcp_charge_precheck(unsigned int type)
{
	...
	if (charge_context(wait_pd_init)) {
	...
	if (!charge_pd_bat_vol_support_9v()) {
		hwlog_info("can not enter fcp 9v beacuse vbat is low\n");
		return false;
	}
...
static void fcp_exit_hv_mode(void)
{
...
static void fcp_charge_check(struct charge_device_info *di, unsigned int type)
{
	...
	if (!di->charge_enable) {
		hwlog_err("enable false, no in fcp check\n");
		return;
	}
#endif
	...
	if (!hvdcp_check_charger_type(type) ||
		...
		hwlog_info("charger type not dcp/fcp, can not enter fcp\n");
		return;
	}
...
static void charge_select_charging_current(struct charge_device_info *di)
{
	...
	case CHARGER_TYPE_USB:
		di->input_current = min(di->core_data->iin_usb, di->iin_sdp_config);
	...
	if (power_cmdline_is_factory_mode() && !power_platform_is_battery_exit()) {
		...
		if (charge_sysfs_get_sysfs_value(CHARGE_SYSFS_LIMIT_CHARGING) == TRUE) {
			...
			di->input_current = min(di->input_current,
				(unsigned int)(charge_sysfs_get_sysfs_value(CHARGE_SYSFS_IIN_THERMAL)));
			di->input_current = min(di->input_current,
				(unsigned int)(charge_sysfs_get_sysfs_value(CHARGE_SYSFS_IIN_RUNNINGTEST)));
			di->charge_current = min(di->charge_current, di->core_data->ichg);
			di->charge_current = min(di->charge_current,
				(unsigned int)(charge_sysfs_get_sysfs_value(CHARGE_SYSFS_ICHG_THERMAL)));
			di->charge_current = min(di->charge_current,
				(unsigned int)(charge_sysfs_get_sysfs_value(CHARGE_SYSFS_ICHG_RUNNINGTEST)));
		}
...
static void charge_start_charging(struct charge_device_info *di)
{
	...
	if (g_eco_design_para.is_support_eco_design)
		power_supply_reg_notifier(&g_eco_design_para.soc_change_nb);
}
...
static void charge_stop_charging(struct charge_device_info *di)
{
...
#endif
	di->iin_sdp_config = di->core_data->iin_usb;
	...
	charger_type_update = FALSE;
	charge_check_reset_vbus_check_info();
	hvdcp_reset_flags();
	...
	if (g_eco_design_para.is_support_eco_design) {
		power_supply_unreg_notifier(&g_eco_design_para.soc_change_nb);
		g_eco_design_para.has_discharged = 0;
		g_eco_design_para.last_soc = -1;
	}
}
...
static bool charge_recharge_precheck(struct charge_device_info *di,
	unsigned int *state)
{
	int ret;

	if (!g_sw_recharge_dts || !di->charge_enable || di->core_data->warm_triggered ||
...
static void charge_recharge_check(struct charge_device_info *di)
{
	...
	if (!charge_recharge_precheck(di, &state))
		return;
	...
	if (state & CHAGRE_STATE_CHRG_DONE) {
		...
		power_supply_get_int_prop(POWER_PLATFORM_BAT_PSY_NAME, POWER_SUPPLY_PROP_CAPACITY,
			&uisoc, POWER_SUPPLY_DEFAULT_CAPACITY, 1);

		hwlog_info("[%s] vbat %d, cv %d, uf_soc %d uisoc %d, cc %d, fcc %d\n",
			__func__, vbat, cv, unfilter_soc, uisoc, cc, fcc);
		...
		if (!first_done) {
			...
			hwlog_info("first done, cv_real=%d, cv_set=%d\n", cv_real, cv_set);
			return;
		}

		if ((cv_set != cv) && !di->recharge_para[RECHARGE_IGNORE_CV_CHANGE]) {
			first_done = false;
			return;
		}

		if ((unfilter_soc > (int)di->recharge_para[RECHARGE_SOC_TH]) ||
			(uisoc > (int)di->recharge_para[RECHARGE_UI_SOC_TH]) ||
		...
		/* 100 is full soc */
		if (fcc && ((discharge_cc * 100 / fcc) < (int)di->recharge_para[RECHARGE_CC_TH])) {
			hwlog_info("can not recharge because cc, discharge_cc=%d\n", discharge_cc);
			return;
		}
...
static void charge_done_uevent(void)
{
	struct power_event_notify_data n_data;
 
	n_data.event = "BMS_EVT=CHARGE_DONE";
	n_data.event_len = 19; /* length of BMS_EVT=CHARGE_DONE */
	power_event_report_uevent(&n_data);
	hwlog_info("charge done report uevent\n");
}
...
static void charge_select_terminal_voltage(struct charge_device_info *di)
{
	...
	if (buck_charge_check_batfet_single_status() == 1) {
		hwlog_info("terminal voltage increased %dmV for batfet_single_ctrl\n", di->increase_volt_for_batfet_single);
		vterm += di->increase_volt_for_batfet_single;
	}
...
/*
 * SchargerV800 can not enter 9v buck when the battery voltage is low,
 * we need to switch 5v buck when vbat decrease to 3.3v during 9v buck charging.
 */
static void charge_9v_buck_safe_protect(void)
{
	int vbat = 0;
	unsigned int type = RESET_ADAPTER_SOURCE_BUCK_CHARGER;

	if (charge_context(buck_9v_enter_th) <= BUCK_9V_VBAT_TH_HYSTER)
		return;

	power_supply_get_int_prop(POWER_PLATFORM_BAT_PSY_NAME,
		POWER_SUPPLY_PROP_VOLTAGE_NOW, &vbat, 0, POWER_UV_PER_MV);

	if (vbat < charge_context(buck_9v_enter_th) - BUCK_9V_VBAT_TH_HYSTER) {
		if ((hvdcp_get_charging_stage() == HVDCP_STAGE_SUCCESS)) {
			hwlog_info("%s vbat=%d, need set fcp to 5v\n", __func__, vbat);
			fcp_exit_hv_mode();
			return;
		}

		if ((charge_get_charger_type() == CHARGER_TYPE_PD) &&
			(charge_get_vusb() > VBUS_VOLTAGE_7000_MV)) {
			hwlog_info("%s vbat=%d, need set pd to 5v\n", __func__, vbat);
			charge_set_adapter_voltage(ADAPTER_5V, type, 0);
			return;
		}
		return;
	}
	...
	if ((vbat > charge_context(buck_9v_enter_th)) &&
		(charge_get_reset_adapter_source() & BIT(type)))
		charge_set_reset_adapter_source(RESET_ADAPTER_CLEAR_MODE, type);
}
...
static void set_charge_work_interval(struct charge_device_info *di)
{
	...
	else if (charge_context(wait_pd_init))
...
static void battery_overheat_handle(void)
{
	if (strstr(saved_command_line, "over_temp_mode=true"))
		huawei_charge_force_into_sleep();
}

static void charge_check_eco_design_state()
{
	if (!g_eco_design_para.is_support_eco_design)
		return;

	if (work_busy(&g_eco_design_para.soc_change_work)) {
		hwlog_info("soc_change_work is scheduled, ignore\n");
		return;
	}

	schedule_work(&g_eco_design_para.soc_change_work);
}

static void charge_soc_work(struct work_struct *work)
{
	int vbat = 0;
	int uiSoc = 0;

	power_supply_get_int_prop(POWER_PLATFORM_BAT_PSY_NAME,
		POWER_SUPPLY_PROP_CAPACITY, &uiSoc,
		POWER_SUPPLY_DEFAULT_CAPACITY, 1);
	if (g_eco_design_para.has_discharged) {
		power_supply_get_int_prop(POWER_PLATFORM_BAT_PSY_NAME,
			POWER_SUPPLY_PROP_VOLTAGE_NOW, &vbat,
			POWER_SUPPLY_DEFAULT_VOLTAGE_NOW, POWER_UV_PER_MV);
		if ((vbat <= g_eco_design_para.eco_design_restore_vbat_dts) ||
			(uiSoc <= g_eco_design_para.eco_design_restore_soc_dts)) {
			power_if_kernel_sysfs_set(POWER_IF_OP_TYPE_ALL,
				POWER_IF_SYSFS_ENABLE_CHARGER, 1);
			g_eco_design_para.has_discharged = 0;
			hwlog_info("recharge because uisoc=%d vbat=%d\n", uiSoc, vbat);
		}
	} else {
		if ((g_eco_design_para.last_soc == CHARGE_FULL_SOC) &&
			(uiSoc < CHARGE_FULL_SOC)) {
			power_if_kernel_sysfs_set(POWER_IF_OP_TYPE_ALL,
				POWER_IF_SYSFS_ENABLE_CHARGER, 0);
			g_eco_design_para.has_discharged = 1;
			hwlog_info("charge stop because uisoc=%d\n", uiSoc);
		}
	}
	g_eco_design_para.last_soc = uiSoc;
}

static int charge_psy_change_cb(
	struct notifier_block *nb, unsigned long event, void *data)
{
	...
	charge_check_eco_design_state();
	return NOTIFY_OK;
}

static void huawei_charger_init_eco_design_info(struct charge_device_info *di,
	struct device_node *np)
{
	(void)power_dts_read_u32(power_dts_tag(HWLOG_TAG), np,
		"is_support_eco_design", &g_eco_design_para.is_support_eco_design, 0);
	if (g_eco_design_para.is_support_eco_design) {
		(void)power_dts_read_u32(power_dts_tag(HWLOG_TAG), np, "eco_design_restore_soc",
			&g_eco_design_para.eco_design_restore_soc_dts, CHARGE_EUROPEAN_RESTORE_SOC);
		(void)power_dts_read_u32(power_dts_tag(HWLOG_TAG), np, "eco_design_restore_vbat",
			&g_eco_design_para.eco_design_restore_vbat_dts, CHARGE_EUROPEAN_RESTORE_VBAT);
		INIT_WORK(&g_eco_design_para.soc_change_work, charge_soc_work);
		g_eco_design_para.soc_change_nb.notifier_call = charge_psy_change_cb;
	}
}
...
static void charge_monitor_work(struct work_struct *work)
{
	...
	/* if support both SCP/UFCS/PPS and PD, try to control adapter by SCP/UFCS/PPS */
	charge_pd_try_to_dc();

	/* For uem:if support both FCP and PD, try to control adapter by FCP */
	charge_pd_try_to_fcp();
...
reschedule_work:
	...
	ret = queue_delayed_work(system_power_efficient_wq, &di->charge_work,
		msecs_to_jiffies(di->monitor_interval));
	if (!ret)
		hwlog_err("%s queue_delay_work fail\n", __func__);
}
...
void charger_vbus_init_handler(int vbus)
{
	...
	if (vbus >= ADAPTER_12V * POWER_MV_PER_V)
		charge_context(init_data).vbus = ADAPTER_12V;
	...
	(void)charge_init_chip(&charge_context(init_data));
	(void)charge_set_vbus_vset(charge_context(init_data).vbus);
	if (charge_get_charger_type() != CHARGER_REMOVED)
		mod_delayed_work(system_power_efficient_wq, &di->charge_work, 0);
}
...
static void charge_pd_to_dc_check(void)
{
	if ((hw_pd_get_bc12_type() != CHARGER_TYPE_DCP) || charge_context(pd_to_dc_flag)) {
		hwlog_info("bc12=%d pd_to_dc_flag=%u\n", hw_pd_get_bc12_type(),
			charge_context(pd_to_dc_flag));
		return;
	}

	charge_context(pd_to_dc_flag) = true;
	charge_context(try_pd_to_fcp_cnt) = PD_TO_DC_MAX_COUNT;
	charge_context(try_pd_to_dc_cnt) = PD_TO_DC_MAX_COUNT;
	hwlog_info("%s set try_pd_to_dc_cnt to max\n", __func__);
}

static bool charge_pd_need_ignore_event(unsigned int charger_type)
{
	if (charge_context(support_new_pd_process) && (charger_type == CHARGER_TYPE_STANDARD) &&
		!charge_context(wait_pd_init)) {
		hwlog_info("%s ignore pd event cause now is dcp\n", __func__);
		return true;
	}
	...
	if (!charge_context(support_new_pd_process) && (charger_type == CHARGER_TYPE_STANDARD) &&
		...
		hwlog_info("%s ignore pd event when dcp and ufcs\n", __func__);
		return true;
	}
...
static void charge_pd_send_icon_uevent(void)
{
	if (!charge_context(support_new_pd_process) || !adapter_support_9v(ADAPTER_PROTOCOL_PD) ||
		(power_icon_inquire() != ICON_TYPE_NORMAL))
		return;

	power_icon_notify(ICON_TYPE_QUICK);
	hwlog_info("%s pd support 9v, report quick icon\n", __func__);
}
...
static void charger_type_handler(unsigned long type, void *data)
{
	...
	case PD_DPM_VBUS_TYPE_TYPEC:
		...
		return;
	case PD_DPM_VBUS_TYPE_PD:
		if (charge_pd_need_ignore_event(charger_type))
			break;
		...
		charge_pd_send_icon_uevent();
		charge_pd_to_dc_check();
		need_resched_work = 1;
...
static bool charge_pd_handle_event(unsigned long event, struct charge_device_info *di)
{
	if ((charge_detecion_get_sinksource_type() == POWER_SUPPLY_START_SINK) &&
		(event == CHARGER_TYPE_DCP) && (charge_get_charger_type() == CHARGER_TYPE_PD)) {
		charge_context(pd_to_dc_flag) = true;
		charge_context(try_pd_to_fcp_cnt) = PD_TO_DC_MAX_COUNT;
		charge_context(try_pd_to_dc_cnt) = PD_TO_DC_MAX_COUNT;
	...
	if ((event == CHARGER_TYPE_SDP) || (event == CHARGER_TYPE_CDP)) {
		charge_context(pd_adapter_support_dc) = false;
		hwlog_info("%s event=%d, not support dc\n", __func__, event);
	}

	if (event == PD_DPM_EMARK_FINISHED) {
		if (charge_context(support_new_pd_process) &&
			(charge_detecion_get_sinksource_type() == POWER_SUPPLY_START_SINK)) {
			charge_context(pd_done) = true;
			mod_delayed_work(system_power_efficient_wq, &di->charge_work, 0);
		}
		hwlog_info("%s receive pd done info\n", __func__);
		return true;
	}
...
static int pd_dpm_notifier_call(struct notifier_block *tcpc_nb, unsigned long event, void *data)
{
	...
	if (charge_pd_handle_event(event, di))
		return NOTIFY_OK;

	if ((event == PD_DPM_VBUS_TYPE_PD) && !charge_context(support_new_pd_process))
...
static void charger_dts_read_u32(struct charge_device_info *di,
	struct device_node *np)
{
	u32 support_new_pd_process = 0;
	u32 pd_ovp_enable = 0;

	(void)power_dts_read_u32(power_dts_tag(HWLOG_TAG), np,
		"support_new_pd_process", &support_new_pd_process, 0);
	charge_context(support_new_pd_process) = support_new_pd_process ? true : false;
	hwlog_info("support_new_pd_process=%u\n", charge_context(support_new_pd_process));

	(void)power_dts_read_u32(power_dts_tag(HWLOG_TAG), np,
		"pd_ovp_enable", &pd_ovp_enable, 0);
	charge_context(pd_ovp_enable) = pd_ovp_enable ? true : false;
	hwlog_info("pd_ovp_enable=%u\n", charge_context(pd_ovp_enable));

	(void)power_dts_read_u32(power_dts_tag(HWLOG_TAG), np,
		"buck_9v_enter_th", (u32 *)&charge_context(buck_9v_enter_th), 0);
	(void)power_dts_read_u32(power_dts_tag(HWLOG_TAG), np,
	...
	(void)power_dts_read_u32(power_dts_tag(HWLOG_TAG), np,
		"increase_volt_for_batfet_single", &di->increase_volt_for_batfet_single, INCREASE_VOLT_DEFAULT);
}
	...
	struct device_node *np)
{
	if (power_dts_read_u32_array(power_dts_tag(HWLOG_TAG), np,
		...
		di->recharge_para[RECHARGE_IGNORE_CV_CHANGE] = 0;
		di->recharge_para[RECHARGE_UI_SOC_TH] = UISOC_RECHARGE_TH;
	}
}
...
int charge_otg_mode_with_volt_enable(int enable, int volt, unsigned int user)
{
	if (enable)
		vbus_ch_open_with_volt(user, VBUS_CH_TYPE_CHARGER, false, volt);
	else
		vbus_ch_close(user, VBUS_CH_TYPE_CHARGER, false, false);

	hwlog_info("charge_otg_mode_enable=%d,user=%d,volt=%d\n", enable, user, volt);
	return 0;
}
...
static int charge_probe(struct platform_device *pdev)
{
	...
	charge_pd_init(&di->pd_voltage_change_work);
	...
	if (!pmic_vbus_irq_enabled) {
		charge_context(wait_pd_init) = true;
	...
	huawei_charger_init_eco_design_info(di, np);
