...
static bool charge_check_pd_ovp_flag(void)
{
	time64_t delta_time;
	unsigned int vbus_vol = 0;
	unsigned int state = CHAGRE_STATE_NORMAL;
	...
	charge_check_get_vbus_voltage(&vbus_vol);
	(void)charge_get_charging_state(&state);

	if ((vbus_vol > VBUS_VOLTAGE_7000_MV) && (state & CHAGRE_STATE_NOT_PG)) {
		if (!charge_context(pd_ovp_time)) {
			charge_context(pd_ovp_time) = power_get_monotonic_boottime();
			hwlog_err("%s pd charging abnormal, ovp begin time=%lld\n",
				__func__, charge_context(pd_ovp_time));
		} else {
			delta_time = power_get_monotonic_boottime() - charge_context(pd_ovp_time);
		}
	} else {
		charge_context(pd_ovp_time) = 0;
		return false;
	}

	if (delta_time > PD_VBUS_OVP_TH) {
		hwlog_err("pd charging abnormal and over 120s, delta_time=%d\n", delta_time);
		return true;
	}
	return false;
}
...
void charge_check_vbus_voltage(void)
{
	...
	if (charge_context(pd_ovp_enable)) {
		if (charge_check_pd_ovp_flag()) {
			charge_set_reset_adapter_source(RESET_ADAPTER_SET_MODE, RESET_ADAPTER_SOURCE_OVP_5V);
			hwlog_err("[%s]pd ovp more than 2 minutes, change 9V to 5V\n", __func__);
		}
	}
...
void charge_check_reset_vbus_check_info(void)
{
	...
	charge_context(pd_time) = 0;
	charge_context(pd_ovp_time) = 0;
	charge_set_reset_adapter_source(RESET_ADAPTER_CLEAR_MODE, RESET_ADAPTER_SOURCE_OVP_5V);
}
