...
static void wltx_aux_dsm_report_tx_err(int err_type)
{
	int ret;
	struct wltx_aux_dev_info *di = g_wltx_aux_di;
	struct hiview_info info[TX_ERR_HIVIEW_INFO_LEN];
	char dev_no[WL_TX_DEV_NO_BUFF] = { 0 };
	...
	info[0].key = "ProductType";
	ret = snprintf_s(dev_no, WL_TX_DEV_NO_BUFF, WL_TX_DEV_NO_BUFF - 1, "%d", di->wireless_tx_acc->dev_no);
	if (ret < 0) {
	...
	info[1].key = "ChargeStatus";
	switch (err_type) {
	case WL_TX_DSM_TX_ERR_FOD:
		info[1].value = "FOD";
		break;
	case WL_TX_DSM_TX_ERR_PING_TIMEOUT:
		info[1].value = "PING_TIMEOUT";
		break;
	default:
		hwlog_err("%s: err_type wrong\n", __func__);
		return;
	}

	hwlog_info("%s: %s\n", __func__, info[1].value);
	power_dsm_report_multi_hiview(POWER_DSM_ERROR_WIRELESS_TX_FOD_PING_TIMEOUT, info, TX_ERR_HIVIEW_INFO_LEN);
}
...
static void wltx_aux_para_init(struct wltx_aux_dev_info *di)
{
	...
	power_supply_get_int_prop(POWER_PLATFORM_BAT_PSY_NAME, POWER_SUPPLY_PROP_TEMP, &g_init_tbatt,
...
static int wltx_aux_chip_init(struct wltx_aux_dev_info *di)
{
	...
	wltx_ic_set_rx_type(WLTRX_IC_AUX, di->tx_open_type);
...
static int wltx_aux_can_do_reverse_charging(void)
{
	...
	power_supply_get_int_prop(POWER_PLATFORM_BAT_PSY_NAME, POWER_SUPPLY_PROP_TEMP, &batt_temp,
		POWER_SUPPLY_DEFAULT_TEMP / POWER_SUPPLY_BAT_TEMP_UNIT, POWER_SUPPLY_BAT_TEMP_UNIT);
...
static void wltx_aux_notify_uevent(struct wltx_acc_dev *di)
{
	...
	if (!di || (di->dev_no < ACC_DEV_NO_BEGIN) ||
		...
		hwlog_err("input invalid, wltx aux not notify uevent\n");
		return;
	}
	switch (di->dev_state) {
	...
	snprintf(g_wl_acc_aux_info_tab[WL_TX_ACC_INFO_NO].value,
		ACC_VALUE_MAX_LEN, "%hhu", di->dev_no);
...
static int wltx_aux_ping_rx(struct wltx_aux_dev_info *di)
{
	...
	wltx_aux_dsm_report_tx_err(WL_TX_DSM_TX_ERR_PING_TIMEOUT);
...
void wltx_aux_report_acc_info(int dev_no, int state)
{
	...
	if (support_switch) {
		...
		wltx_aux_notify_uevent(acc_dev);
	} else {
		...
		if (ret)
			return;
		wltx_aux_notify_uevent(di->wireless_tx_acc);
	}
}
...
static void wltx_aux_event_work(struct work_struct *work)
{
	...
	case POWER_NE_WLTX_HANDSHAKE_SUCC:
		...
		di->standard_rx = true;
		if (di->support_keyboard_slot &&
			(di->tx_event_data == HWQI_HANDSHAKE_ID_HW_SLOT_SLE)) {
			/* 0x8855 Handshake succeeds, enabling 13/14 modem */
			wltx_ic_switch_modem(WLTRX_IC_AUX);
			di->tx_event_data = WL_TX_CLEAR_EVENT_DATA;
		}
		break;
	...
	case POWER_NE_WLTX_TX_FOD:
		...
		wltx_aux_report_acc_info(di->wireless_tx_acc->dev_no,
			WL_ACC_DEV_STATE_CHARGE_ERROR);
		wltx_aux_dsm_report_tx_err(WL_TX_DSM_TX_ERR_FOD);
	...
	struct wltx_aux_dev_info *di)
{
	...
	(void)power_dts_read_u32(power_dts_tag(HWLOG_TAG), np,
		"support_keyboard_slot", &di->support_keyboard_slot, 0);
}
...
static struct power_sysfs_attr_info wltx_aux_sysfs_field_tbl[] = {
	...
	power_sysfs_attr_ro(wltx_aux, 0444, WL_TX_SYSFS_TX_FOD_GAP_STATE, tx_fod_gap_state),
};
	...
	struct device_attribute *attr, char *buf)
{
	...
	case WL_TX_SYSFS_TX_FOD_GAP_STATE:
		wltx_ic_get_fod_gap_state(WLTRX_IC_AUX, &temp_data);
...
static int wltx_aux_freeze(struct device *dev)
{
	struct wltx_aux_dev_info *di = g_wltx_aux_di;
	...
	if (wltx_aux_get_tx_open_flag()) {
		g_hall_state = false;
		di->pen_approaching = false;
		queue_delayed_work(g_wltx_aux_di->aux_tx_wq,
			&g_wltx_aux_di->hall_away_work, msecs_to_jiffies(0));
		flush_delayed_work(&g_wltx_aux_di->hall_away_work);
		flush_delayed_work(&di->wltx_aux_monitor_work);
	}
	hwlog_info("wltx_aux_freeze\n");
	return 0;
}
