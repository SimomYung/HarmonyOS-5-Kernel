...
static void wireless_charge_para_init(struct wlrx_dev_info *di)
{
	...
	di->monitor_interval = WLRX_MON_WORK_INTERVAL;
	di->ctrl_interval = CONTROL_INTERVAL_NORMAL;
	...
	wlrx_mon_reset_para(di->drv_type, WLTRX_RST_IN_INIT);
	...
	unsigned long event, void *data)
{
	...
	case POWER_NE_SCREEN_FOLD:
		if (!dts->support_fold_status_check || power_cmdline_is_factory_mode())
			return NOTIFY_OK;
		if (!wlrx_get_screen_stop_charging_state())
			break;
...
static void wlrx_module_deinit(unsigned int drv_type)
{
	...
	wlrx_pwr_eff_deinit(drv_type);
	wlrx_mon_deinit(drv_type);
}
...
static int wlrx_module_init(unsigned int drv_type, struct device *dev)
{
	...
	if (ret)
		goto exit;
	ret = wlrx_pwr_eff_init(drv_type);
	if (ret)
		goto exit;
	ret = wlrx_mon_init(drv_type, dev);
	if (ret)
		goto exit;
