...
static int wltx_check_abnormal_vbatt(struct wltx_dev_info *di)
{
	int i;
	int vbatt;

	if (wltx_get_client() == WLTX_CLIENT_BAT_HEATING)
		return 0;

	/* vbatt_th typically 3450mV, 20ms*3=60ms timeout for vbatt debouncing */
	for (i = 0; i < 3; i++) {
		vbatt = coul_drv_battery_voltage();
		if (tx_open_flag && (vbatt < di->abnormal_vbat)) {
			hwlog_err("abnormal vbatt=%d, vbat_min_th=%d\n", vbatt, di->abnormal_vbat);
			msleep(20);
		} else {
			break;
		}
...
static int wltx_check_idischrg(struct wltx_dev_info *di, int idischrg_max, bool high_power_charge)
{
	int idischrg;
	int i;
...
static int wltx_check_abnormal_ibatt(struct wltx_dev_info *di)
{
	int idischrg_max;
	...
	if ((tx_cap_type == WLACC_ADP_FCP) ||
		...
		((dts->pwr_type == WL_TX_PWR_5VBST_RVSSC_CP) &&
		(di->cur_pwr_src == PWR_SRC_RVS_SC_CP))) {
		idischrg_max = WLTX_ABNORMAL_IBAT_2;
	} else {
		idischrg_max = WLTX_ABNORMAL_IBAT;
	}

	return wltx_check_idischrg(di, idischrg_max, high_power_charge);
}

static void wltx_chk_abnormal_tx_vin(struct wltx_dev_info *di)
{
	int i;
	u16 tx_vin = 0;
	const char *src = NULL;
...
static void wltx_check_abnormal_power(struct wltx_dev_info *di)
{
	if (wltx_check_abnormal_vbatt(di))
		return;

	if (wltx_check_abnormal_ibatt(di))
		return;

	wltx_chk_abnormal_tx_vin(di);
}
...
static void wltx_select_vset_para(struct wltx_dev_info *di)
{
	...
	for (i = 0; i < di->pwr_src_vset_level; i++) {
		if (di->cur_pwr_src == di->pwr_src_vset[i].pwr_src) {
			di->tx_vset.v_ps = di->pwr_src_vset[i].v_ps;
			di->tx_vset.v_ping = di->pwr_src_vset[i].v_ping;
			di->tx_vset.v_hs = di->pwr_src_vset[i].v_hs;
			di->tx_vset.v_dflt = di->pwr_src_vset[i].v_dflt;
			return;
		}
	}
	...
	} else if (di->cur_pwr_src == PWR_SRC_RVS_SC_CP) {
		di->tx_vset.v_ps = di->sc12_dts_vset.v_ps;
		di->tx_vset.v_ping = di->sc12_dts_vset.v_ping;
		di->tx_vset.v_hs = di->sc12_dts_vset.v_hs;
		di->tx_vset.v_dflt = di->sc12_dts_vset.v_dflt;
	} else {
...
static void wltx_get_ilim(struct wltx_dev_info *di, u16 *ilim, u16 *imax)
{
	...
	if (!di->prot_id_hwrx && di->tx_third_party_ilim)
		tx_ilim = di->tx_third_party_ilim;
...
static void wltx_stop_wlcase_working(void)
{
	if (wirelesscase_get_online_state() == WLCASE_ONLINE_COOLINGCASE) {
		hwlog_info("[wltx_stop_wlcase_working] send wlcase stop working msg\n");
...
static void wireless_tx_fault_event_handler(struct wltx_dev_info *di)
{
	...
	wltx_stop_wlcase_working();
	wireless_tx_send_event();
	hwlog_info("[%s] tx_status = 0x%02x\n", __func__, tx_status);
	wltx_stop_charging();
	di->can_switch_pwr_src = false;
...
static int wltx_update_vset_by_ilim(struct wltx_dev_info *di)
{
	u16 tx_ilim = 0;
	u16 tx_imax = 0;

	(void)wltx_get_ilim(di, &tx_ilim, &tx_imax);
	if ((di->tx_vset.para[di->tx_vset.cur].vset >= WLTX_RX_HIGH_VOUT) &&
		(tx_ilim <= WL_TX_IIN_LOW) && (tx_imax <= WL_TX_IIN_LOW))
		return ADAPTER_5V * POWER_MV_PER_V;
...
static void wltx_ps_tx_vset_check(struct wltx_dev_info *di)
{
	...
	tx_vset_by_ilim = wltx_update_vset_by_ilim(di);
	if (tx_vset > tx_vset_by_ilim)
		tx_vset = tx_vset_by_ilim;
...
static void wltx_ps_tx_volt_check(struct wltx_dev_info *di)
{
	...
	if ((dts->pwr_type != WL_TX_PWR_5VBST_VBUS_OTG_CP) &&
		(dts->pwr_type != WL_TX_PWR_5VBST_VBUS_OTG_CP2) &&
...
/* This function is used to determine whether the current mode is 7.5w or higher */
static bool wltx_is_in_high_power_mode(struct wltx_dev_info *di)
{
	if (((wltx_cap_get_cur_id(WLTRX_DRV_MAIN) == WLTX_HIGH_PWR_CAP) || 
		(wltx_cap_get_cur_id(WLTRX_DRV_MAIN) == WLTX_HIGH_PWR2_CAP)) &&
		(wltx_get_tx_pwr(WLTRX_DRV_MAIN) >= WLTX_CAP_7_5W))
		return true;
	return false;
}
...
{
	if (di->new_hp_timeout) {
		if (wltx_is_in_high_power_mode(di))
			return true;
		return false;
	}
...
static bool wltx_coolingcase_handler_valid(enum wltx_client type, bool enable)
{
	...
	if (!enable && (type == WLTX_CLIENT_UI)) {
		wltx_stop_wlcase_working();
		hwlog_info("[coolingcase_handler_valid] user close wltx, stop wlcase working\n");
	}
...
static void wltx_handle_ask_rx_event(struct wltx_dev_info *di)
{
	...
	case RX_STATR_CHARGING:
		...
		if (di->new_hp_timeout && wltx_is_in_high_power_mode(di)) {
			hwlog_info("wltx_handle_ask_rx_event: hp_time_out\n");
...
static void wltx_handle_ask_set_vtx(struct wltx_dev_info *di)
{
	...
	if (!di->new_hp_timeout && (wltx_cap_get_cur_id(WLTRX_DRV_MAIN) == WLTX_HIGH_PWR_CAP)) {
		if (di->tx_vout.v_ask < WLTX_RX_HIGH_VOUT)
			return;
		di->hp_time_out = jiffies + msecs_to_jiffies(di->hp_timeout_dflt);
	}
...
static struct wltx_prevfod_cfg *wltx_select_prevfod_cfg(struct wltx_dev_info *di)
{
	...
	if (!dts) {
		hwlog_err("dts is null\n");
		return &di->prevfod_cfg2;
	}

	hwlog_info("select prevfod config, product_id=%d, enable=%d\n",
		di->rx_product_id, dts->tx_3c_enable);

	if (wltx_rxid_in_prevfod1_list(di))
		return &di->prevfod_cfg1;

	if ((di->rx_product_id == WIRELESS_DEFAULT_PROCUCT_ID) && dts->tx_3c_enable)
		return &di->prevfod_cfg1;
...
static void wltx_parse_sc12_stage_vset_para(struct device_node *np, struct wltx_dev_info *di)
{
	int ret;

	if (of_property_count_u32_elems(np, "tx_sc2_stage_vset") < 0)
		ret = __wltx_parse_stage_vset_para("tx_stage_vset", &di->sc12_dts_vset, np, di);
	else
		ret = __wltx_parse_stage_vset_para("tx_sc2_stage_vset", &di->sc12_dts_vset, np, di);
	if (ret < 0)
		goto parse_err;
	goto print_para;

parse_err:
	di->sc12_dts_vset.v_ps = 5000; /* mV */
	di->sc12_dts_vset.v_ping = 5000; /* mV */
	di->sc12_dts_vset.v_hs = 5000; /* mV */
	di->sc12_dts_vset.v_dflt = 5000; /* mV */

print_para:
	hwlog_info("[%s] sc2_pwr_supply:%d ping:%d handshake:%d default:%d\n",
		__func__, di->sc12_dts_vset.v_ps, di->sc12_dts_vset.v_ping,
		di->sc12_dts_vset.v_hs, di->sc12_dts_vset.v_dflt);
}
...
static void wltx_parse_pwr_src_vset_para(struct device_node *np, struct wltx_dev_info *di)
{
	int len;

	len = power_dts_read_u32_count(power_dts_tag(HWLOG_TAG), np, "tx_pwr_src_vset",
		WLTX_PWR_SRC_VSET_ROW, WLTX_PWR_SRC_VSET_COL);
	if (len <= 0)
		return;

	if (power_dts_read_u32_array(power_dts_tag(HWLOG_TAG), np,
		"tx_pwr_src_vset", (u32 *)di->pwr_src_vset, len))
		return;

	di->pwr_src_vset_level = len / WLTX_PWR_SRC_VSET_COL;
}
...
static void wltx_parse_basic_dts(struct device_node *np, struct wltx_dev_info *di)
{
	(void)power_dts_read_u32(power_dts_tag(HWLOG_TAG), np,
	...
	(void)power_dts_read_u32(power_dts_tag(HWLOG_TAG), np,
		"rxsw_ctrl_ovp", &di->rxsw_ctrl_ovp, 1);
	(void)power_dts_read_u32(power_dts_tag(HWLOG_TAG), np,
		"new_hp_timeout", &di->new_hp_timeout, 0);
	(void)power_dts_read_u32(power_dts_tag(HWLOG_TAG), np,
		"abnormal_vbat", &di->abnormal_vbat, WLTX_ABNORMAL_VBAT);
	(void)power_dts_read_u32(power_dts_tag(HWLOG_TAG), np,
		"tx_third_party_ilim", &di->tx_third_party_ilim, 0);
}
...
static void wireless_tx_parse_dts(struct device_node *np, struct wltx_dev_info *di)
{
	...
	wltx_parse_sc12_stage_vset_para(np, di);
	wltx_parse_sc14_stage_vset_para(np, di);
	wltx_parse_pwr_src_vset_para(np, di);
