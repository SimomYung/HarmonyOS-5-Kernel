 ...
 * Description: dubai gpu frequency stats
 */
...
static int dubai_get_gpu_info(struct dubai_gpu_freq_info *data, int num)
{
	...
	length = num * sizeof(struct dubai_gpu_freq_info);
	mutex_lock(&stats_lock);
	if (freq_stats)
		memcpy_s(data, length, freq_stats, freq_num * sizeof(struct dubai_gpu_freq_info));
	mutex_unlock(&stats_lock);
