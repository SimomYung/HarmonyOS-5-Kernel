/*
 * Copyright (c) Huawei Technologies Co., Ltd. 2012-2024. All rights reserved.
 * Description: dubai wakeup stats
 */

#include <linux/suspend.h>
#include <securec.h>

#include <chipset_common/dubai/dubai_plat.h>
#include <huawei_platform/power/dubai/dubai_wakeup_stats.h>

#define WAKEUP_SOURCE_SIZE						32
#define WAKEUP_RTC_VALUE 	(0xA5A5)
#define WAKEUP_STATE_BYTE_SIZE	(DUBAI_WAKEUP_NUM_MAX * sizeof(struct dubai_wakeup_info))

static atomic_t wakeup_enable = ATOMIC_INIT(0);
static bool wakeup_state = false;
static size_t wakeup_num = 0;
static struct dubai_wakeup_info wakeup_stats[DUBAI_WAKEUP_NUM_MAX];
static const char *rtc_wakeup_source[] = {"RTC0", "rtc0", "intr_rtc0", "INTR_RTC0"};
#ifdef CONFIG_HM_SR_PHONE
static const char *pending_wakeup_source[] = {"PERI_IOC_TO_LPMCU_0", "IRQ_PERI_DSI_IOC_TO_LPMCU", 
	"IPC_GIC_INT_COMB", "pie15_gpio_lpmcu", "IRQ_AO_IOC_TO_LPMCU_0", "IRQ_PERI_IOC_TO_LPMCU"};
#endif
static int g_dubai_wakeup_gpio = 0;

static void dubai_reset_wakeup_stats(void)
{
	wakeup_state = false;
	wakeup_num = 0;
	g_dubai_wakeup_gpio = 0;
	memset_s(wakeup_stats, WAKEUP_STATE_BYTE_SIZE, 0, DUBAI_WAKEUP_NUM_MAX * sizeof(struct dubai_wakeup_info));
}

static bool is_rtc_wakeup(const char *irq)
{
	size_t i;

	for (i = 0; i < sizeof(rtc_wakeup_source) / sizeof(const char *); i++) {
		if (strstr(irq, rtc_wakeup_source[i]))
			return true;
	}
	return false;
}

#ifdef CONFIG_HM_SR_PHONE
static bool is_need_pending_wakeup(const char *irq)
{
	size_t i;
 
	for (i = 0; i < sizeof(pending_wakeup_source) / sizeof(const char *); i++) {
		if (strstr(irq, pending_wakeup_source[i]))
			return true;
	}
	return false;
}
#endif

static void set_wakeup_info(enum dubai_wakeup_type type, const char *tag, const char *msg)
{
	if (wakeup_num >= DUBAI_WAKEUP_NUM_MAX) {
		dubai_err("Invalid wakeup num: %d", wakeup_num);
		return;
	}

	strncpy_s(wakeup_stats[wakeup_num].tag, DUBAI_WAKEUP_TAG_LENGTH, tag, DUBAI_WAKEUP_TAG_LENGTH - 1);
	strncpy_s(wakeup_stats[wakeup_num].msg, DUBAI_WAKEUP_MSG_LENGTH, msg, DUBAI_WAKEUP_MSG_LENGTH - 1);
	wakeup_stats[wakeup_num].type = type;
	wakeup_num++;
}

/*
 * Caution: It's dangerous to use HWDUBAI_LOG in this function,
 * because it's in the SR process, and the HWDUBAI_LOG will wake up the kworker thread that will open irq
 */
void dubai_log_wakeup_info(const char *tag, const char *fmt, ...)
{
	va_list args;
	char msg[DUBAI_WAKEUP_MSG_LENGTH] = {0};

	if (atomic_read(&wakeup_enable) == 0) {
		return;
	}

	if (!tag || (strlen(tag) >= DUBAI_WAKEUP_TAG_LENGTH)) {
		dubai_err("Invalid parameter");
		return;
	}

	va_start(args, fmt);
	vscnprintf(msg, DUBAI_WAKEUP_MSG_LENGTH, fmt, args);
	set_wakeup_info(DUBAI_WAKEUP_TYPE_OTHERS, tag, msg);
	va_end(args);
}
EXPORT_SYMBOL(dubai_log_wakeup_info); //lint !e580

int dubai_get_wakeup_gpio(void)
{
	return g_dubai_wakeup_gpio;
}
EXPORT_SYMBOL(dubai_get_wakeup_gpio);
/*
 * Caution: It's dangerous to use HWDUBAI_LOG in this function,
 * because it's in the SR process, and the HWDUBAI_LOG will wake up the kworker thread that will open irq
 */
void dubai_log_irq_wakeup(enum dubai_irq_wakeup_type type, const char *source, int gpio)
{
	int ret;
	char wakeup_source[WAKEUP_SOURCE_SIZE] = {0};
	char msg[DUBAI_WAKEUP_MSG_LENGTH] = {0};
	enum dubai_wakeup_type wakeup_type = DUBAI_WAKEUP_TYPE_INVALID;

	if (atomic_read(&wakeup_enable) == 0) {
		return;
	}

	if (!source) {
		dubai_err("Invalid parameter");
		return;
	}

	g_dubai_wakeup_gpio = gpio;
	switch (type) {
	case DUBAI_IRQ_WAKEUP_TYPE_AP:
		if (gpio >= 0) {
			ret = snprintf_s(wakeup_source, WAKEUP_SOURCE_SIZE, WAKEUP_SOURCE_SIZE - 1, "GPIO%d", gpio);
			wakeup_type = DUBAI_WAKEUP_TYPE_GPIO;
		} else if (is_rtc_wakeup(source)) {
			ret = snprintf_s(wakeup_source, WAKEUP_SOURCE_SIZE, WAKEUP_SOURCE_SIZE - 1, "RTC0");
            wakeup_type = DUBAI_WAKEUP_TYPE_RTC;
		} else {
			ret = snprintf_s(wakeup_source, WAKEUP_SOURCE_SIZE, WAKEUP_SOURCE_SIZE - 1, "%s", source);
			wakeup_type = DUBAI_WAKEUP_TYPE_OTHERS;
		}
		if (ret < 0)
			dubai_err("%s: snprintf_s wakeup_source failed\n", __func__);

		if (!wakeup_state) {
			dubai_wakeup_notify(wakeup_source);
			wakeup_state = true;
		}
		break;
	default:
		return;
	}

	ret = snprintf_s(msg, DUBAI_WAKEUP_MSG_LENGTH, DUBAI_WAKEUP_MSG_LENGTH - 1, "name=%s", wakeup_source);
	if (ret < 0)
		dubai_err("%s: snprintf_s wakeup_msg failed\n", __func__);
	set_wakeup_info(wakeup_type, "DUBAI_TAG_AP_WAKE_IRQ", msg);
}
EXPORT_SYMBOL(dubai_log_irq_wakeup); //lint !e580

#ifdef CONFIG_HM_SR_PHONE
void dubai_log_irq_wakeup_isrtc(const char *source)
{
	int ret;
	char wakeup_source[WAKEUP_SOURCE_SIZE] = {0};
	char msg[DUBAI_WAKEUP_MSG_LENGTH] = {0};
	enum dubai_wakeup_type wakeup_type = DUBAI_WAKEUP_TYPE_INVALID;
	if (atomic_read(&wakeup_enable) == 0) {
		return;
	}

	if (!source) {
		dubai_err("Invalid parameter");
		return;
	}

	if (!is_need_pending_wakeup(source)) {
		if(is_rtc_wakeup(source)){
			ret = snprintf_s(wakeup_source, WAKEUP_SOURCE_SIZE, WAKEUP_SOURCE_SIZE - 1, "RTC0");
            wakeup_type = DUBAI_WAKEUP_TYPE_RTC;
            g_dubai_wakeup_gpio = WAKEUP_RTC_VALUE;
        } else {
            ret = snprintf_s(wakeup_source, WAKEUP_SOURCE_SIZE, WAKEUP_SOURCE_SIZE - 1, "%s", source);
			wakeup_type = DUBAI_WAKEUP_TYPE_OTHERS;
		}
		if (ret < 0)
			dubai_err("%s: snprintf_s wakeup_source failed\n", __func__);
 
		if (!wakeup_state) {
			dubai_wakeup_notify(wakeup_source);
			wakeup_state = true;
		}
		ret = snprintf_s(msg, DUBAI_WAKEUP_MSG_LENGTH, DUBAI_WAKEUP_MSG_LENGTH - 1, "name=%s", wakeup_source);
		if (ret < 0)
			dubai_err("%s: snprintf_s wakeup_msg failed\n", __func__);
		set_wakeup_info(wakeup_type, "DUBAI_TAG_AP_WAKE_IRQ", msg);
	}
}
EXPORT_SYMBOL(dubai_log_irq_wakeup_isrtc); //lint !e580
#endif

static int dubai_get_wakeup_info(struct dubai_wakeup_info *data, size_t num)
{
	size_t length;
	if (!data || (num < wakeup_num) || (num > DUBAI_WAKEUP_NUM_MAX)) {
		dubai_err("Invalid parameter");
		return -1;
	}

	length = num * sizeof(struct dubai_wakeup_info);
	if (wakeup_num > 0)
		memcpy_s(data, length, wakeup_stats, wakeup_num * sizeof(struct dubai_wakeup_info));
	return 0;
}

static int dubai_suspend_notify(unsigned long mode)
{
	switch (mode) {
	case PM_SUSPEND_PREPARE:
		dubai_reset_wakeup_stats();
		atomic_set(&wakeup_enable, 1);
		break;
	case PM_POST_SUSPEND:
		atomic_set(&wakeup_enable, 0);
		break;
	default:
		break;
	}
	return 0;
}

static struct dubai_wakeup_stats_ops wakeup_ops = {
	.suspend_notify = dubai_suspend_notify,
	.get_stats = dubai_get_wakeup_info,
};

int dubai_wakeup_stats_init(void)
{
	dubai_register_module_ops(DUBAI_MODULE_WAKEUP, &wakeup_ops);
	return 0;
}

void dubai_wakeup_stats_exit(void)
{
	dubai_unregister_module_ops(DUBAI_MODULE_WAKEUP);
	dubai_reset_wakeup_stats();
}
