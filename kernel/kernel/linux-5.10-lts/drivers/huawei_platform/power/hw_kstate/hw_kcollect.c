...
int hw_freeze_binder_info(struct kstate_binder_info *info)
{
...
#ifdef CONFIG_HW_BINDER_RECORD
	/* process in container no need report async binder */
	if (level > 0 && type == TRANSAC_ASYNC && calling_pid > 0) {
		pr_debug("hw_kcollect %s: %d->%d async call no need report.\n", __func__, calling_pid, called_pid);
		return 0;
	}
	if (!is_freeze) {
	...
	return kcollect(KCOLLECT_BINDERCALL_MASK, "[TPID %d CALLED][PID %d][%u][%u][%u][%u][%u]", called_pid,
		get_level1_pid(calling_pid), is_freeze, type, flags, level, code);
#else
