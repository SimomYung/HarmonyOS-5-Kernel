 ...
 *
 * Copyright (c) 2018-2025 Huawei Technologies Co., Ltd.
 *
...
typedef enum {
	...
	DEVICE_IDLE_CHANGED = 13,
	SLEEP_STATE_CHANGED = 14,
#endif
	ANCO_STATUS_CHANGED = 101,
	ANCO_BINDER_CALL = 102
} pg_command_order;
...
static int handle_anco_pg_cmd(pg_command pg_cmd)
{
	int ret = 0;
	switch (pg_cmd.cmd) {
	case ANCO_STATUS_CHANGED:
		dispatch_anco_status(pg_cmd.lock_timeout);
		break;
	case ANCO_BINDER_CALL:
		dispatch_binder_call(pg_cmd.name);
		break;
#ifdef CONFIG_DOZE_FILTER
	case WIFI_STATE_CHANGED: {
		int wifi_state = simple_strtol(pg_cmd.name, NULL, BASE);
		hw_set_net_filter_re_enable(wifi_state);
		break;
	}
	case DEVICE_IDLE_CHANGED: {
		int device_idle = simple_strtol(pg_cmd.name, NULL, BASE);
		pr_info("device idle:%d", device_idle);
		hw_set_net_filter_enable(device_idle);
		break;
	}
	case SLEEP_STATE_CHANGED: {
		int sleep = simple_strtol(pg_cmd.name, NULL, BASE);
		hw_set_net_filter_user_sleep(sleep);
		break;
	}
#endif
	default:
