...
static bool check_kstate_condition(struct kstate_sock *kstate_sk, struct kstate_msg *kmsg)
{
	if (kstate_sk->user_pid <= 0) {
		pr_debug("hw_kstate %s kstate_sk->user_pid:%d",
			__func__, kstate_sk->user_pid);
		return false;
	}

	if (kmsg->pid_level != kstate_sk->net_level) {
		pr_debug("hw_kstate %s kmsg->pid_level:%d, kstate_sk->net_level:%d",
			__func__, kmsg->pid_level, kstate_sk->net_level);
		return false;
	}
	return true;
}
...
static int send_to_user(struct kstate_msg *kmsg)
{
	...
	list_for_each_entry(kstate_sk, &kstate_socks, list) {
		if (!check_kstate_condition(kstate_sk, kmsg)) {
			continue;
		}

		skb = alloc_skb(NLMSG_SPACE(len), GFP_ATOMIC);
		if (IS_ERR_OR_NULL(skb)) {
			pr_err("hw_kstate %s: GFP_ATOMIC alloc skb failed! len:%d\n", __func__, NLMSG_SPACE(len));

			skb = alloc_skb(NLMSG_SPACE(len), GFP_ATOMIC);
			if (IS_ERR_OR_NULL(skb)) {
				pr_err("hw_kstate %s: GFP_ATOMIC retry alloc skb failed! len:%d\n", __func__, NLMSG_SPACE(len));
				continue;
			}
		}
		...
		if (!is_anco_exit()) {
			...
			netlink_broadcast(kstate_sk->sk, skb, 0, 1, GFP_ATOMIC);
		}
		pr_debug("hw_kstate %s netlink_unicast:%s",
			__func__, kmsg->msg.buffer + sizeof(u32) * 4);
		ret = netlink_unicast(kstate_sk->sk, skb, kstate_sk->user_pid, MSG_DONTWAIT);
		if ((ret < 0) && (kstate_sk->user_pid >= 0))
			pr_info("hw_kstate %s unicast fail:%d, user_pid:%d, %s",
				__func__, ret, kstate_sk->user_pid, kmsg->msg.buffer + sizeof(u32) * 4);
	}
	...
	while (1) {
		...
		while (kfifo_out(&kstate_fifo, &msg, sizeof(struct kstate_msg*)) != 0) {
			...
			if (send_to_user(msg) < 0)
				pr_debug("hw_kstate %s: send msg to user failed!\n", __func__);
			kfree(msg);
		}
	}
	return 0;
...
static int fifo_in(struct kstate_msg *msg)
{
	...
	if (kfifo_in(&kstate_fifo, &msg, sizeof(struct kstate_msg*)) == 0) {
		...
		if (kfifo_is_full(&kstate_fifo)) {
			...
			kfree(msg);
#ifdef CONFIG_HW_BINDER_RECORD
			pr_err("hw_kstate %s: kfifo_is_full!\n", __func__);
 ...
 * @return -1 for failed, 0 for successed
 */
int kstate(channel_id dst, packet_tag tag, const char *data, size_t len, int level)
{
