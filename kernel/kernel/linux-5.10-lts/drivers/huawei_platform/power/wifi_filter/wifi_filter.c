...
static bool handle_doze_info(struct sk_buff *skb)
{
    ...
    memcpy_s(&g_item_info.p_items_data[g_item_info.scrnoff_item_cnt+g_item_info.doze_index],
        sizeof(hw_wifi_filter_item), &item, sizeof(hw_wifi_filter_item));
...
static void add_echo_info(void)
{
	...
	if (!success)
		return;
    if (g_item_info.p_items_data != NULL)
		hw_add_filter_items(&g_item_info.p_items_data[0], g_item_info.scrnoff_item_cnt + g_item_info.doze_item_cnt);
}
