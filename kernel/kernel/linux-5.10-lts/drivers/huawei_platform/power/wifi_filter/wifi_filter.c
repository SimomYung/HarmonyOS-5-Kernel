/*
* Copyright (c) 2017-2025 Huawei Device Co., Ltd.
* Licensed under the Apache License, Version 2.0 (the "License");
* you may not use this file except in compliance with the License.
* You may obtain a copy of the License at
*
*     http://www.apache.org/licenses/LICENSE-2.0
*
* Unless required by applicable law or agreed to in writing, software
* distributed under the License is distributed on an "AS IS" BASIS,
* WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
* See the License for the specific language governing permissions and
* limitations under the License.
 */



/*****************************************************************************
  1.Header File Include
*****************************************************************************/
#include <linux/kernel.h>
#include <linux/module.h>
#include <linux/spinlock.h>
#include <linux/file.h>
#include <linux/list.h>
#include <linux/types.h>
#include <linux/err.h>
#include <linux/fs.h>
#include <linux/fdtable.h>
#include <linux/tcp.h>
#include <securec.h>

#include <net/sock.h>

#include <net/tcp.h>
#include <net/inet_sock.h>

#include <huawei_platform/power/wifi_filter/wifi_filter.h>
#include <linux/kthread.h>

#include <huawei_platform/connectivity/hw_connectivity.h>
#ifdef CONFIG_DKMD_DPU_OHOS
#include <platform_include/display/dkmd/dkmd_notify.h>
#endif
#include <linux/fb.h>

#include <linux/time.h>
#include <linux/rtc.h>
/******************************************************************************
   2.Macro Definition
******************************************************************************/
#define DOZABLE_NAME    "fw_dozable"
#define WLAN_NAME       "wlan0"
#define ITEM_COUNT_MAX_BRODCOM    (1<<5) //32
#define ITEM_COUNT_MAX_HISI       (1<<6) //64

#define NETBIOS_PORT              (137)
#define SPECIAL_PORT_NUM          (0)
#define CHIP_TYPE_HI110X          "hisi"
#define LOG_TAG                   "wifi_filter"

#define DEBUG                     1
#define INFO                      1
#define ECHO_PORT                 7
#define WIFI_OPEN                 3
#define DNS_PORT                  5353

#define FILTER_LOGD(fmt, ...) \
    do { \
        if (DEBUG) { \
            printk(KERN_DEBUG "["LOG_TAG"] %s: "fmt"\n", __func__, ##__VA_ARGS__); \
        } \
    } while (0)

#define FILTER_LOGI(fmt, ...) \
    do { \
        if (INFO) { \
            printk(KERN_INFO "["LOG_TAG"] %s: "fmt"\n", __func__, ##__VA_ARGS__); \
        } \
    } while (0)

#define FILTER_LOGE(fmt, ...) \
    do { \
        printk(KERN_ERR "["LOG_TAG"] %s: "fmt"\n", __func__, ##__VA_ARGS__); \
    } while (0)
/*****************************************************************************
  3.Function Declaration
*****************************************************************************/
static int hw_add_filter_items(hw_wifi_filter_item *items, int count);
static int hw_clear_all_item(void);

static inline struct ipt_entry *
get_entry(const void *base, unsigned int offset)
{
    return (struct ipt_entry *)(base + offset);
}

static inline const struct xt_entry_target *
ipt_get_target_c(const struct ipt_entry *e)
{
    return ipt_get_target((struct ipt_entry *)e);
}

static bool add_to_screenoff_table(hw_wifi_filter_item *pItem);

static void add_echo_info(void);
static void add_dns_info(void);

/******************************************************************************
   4.Private Definition
******************************************************************************/
enum WakeCondition{
    WAKE_INIT = 0,
    WAKE_ADD,
    WAKE_SCRN_ON,
    WAKE_SCRN_OFF,
    WAKE_EXIT_NIGHT,
    WAKE_ENTER_NIGHT,
    WAKE_WIFI_REENABLE
};
enum FilterType{
    TYPE_HEAD = 0,
    TYPE_ICMP = 1,
    TYPE_TAIL
};
static struct task_struct *WifiFilterThread = NULL;
static wait_queue_head_t mythread_wq;
static int wake_up_condition = WAKE_INIT;
static bool bIsSupportWifiFilter = false;
static atomic_t bScreenoff = ATOMIC_INIT(0);
static struct hw_wlan_filter_ops gWlanFilterOps;

static int g_item_cnt_max = 0;

typedef struct {
    unsigned short scrnoff_item_cnt;
    unsigned short scrnoff_index;
    hw_wifi_filter_item *p_items_data;
}wifi_filter_item_info;

static wifi_filter_item_info g_item_info;
static bool bIsNight = false;
DEFINE_SPINLOCK(item_lock);

/******************************************************************************
   5.Global Variable Definition
******************************************************************************/

/******************************************************************************
   6.Function Implementation
******************************************************************************/
#ifdef CONFIG_DKMD_DPU_OHOS
static int wifi_filter_dkmd_notifier_cb(struct notifier_block *self,
    unsigned long event, void *data)
{
    struct dkmd_event *dkmd_event = (struct dkmd_event *)data;
    int blank;

    if (!bIsSupportWifiFilter) {
        return NOTIFY_DONE;
    }
    if (data == NULL) {
        return NOTIFY_DONE;
    }
    if (event != DKMD_EVENT_BLANK || dkmd_event == NULL) {
        return NOTIFY_DONE;
    }
    blank = dkmd_event->value;
    if (event == DKMD_EVENT_BLANK) {
        if (blank == FB_BLANK_UNBLANK) {
            FILTER_LOGI("screenon");
            atomic_set(&bScreenoff, 0);
            wake_up_condition = WAKE_SCRN_ON;
            wake_up_interruptible(&mythread_wq);
        } else if (blank == FB_BLANK_POWERDOWN) {
            FILTER_LOGI("screenoff");
            atomic_set(&bScreenoff, 1);
            wake_up_condition = WAKE_SCRN_OFF;
            wake_up_interruptible(&mythread_wq);
        } else {
            FILTER_LOGI("other dkmd blank event value=%d\n", blank);
        }
    }
    return NOTIFY_DONE;
}
#else
static int wifi_filter_fb_notifier_cb(struct notifier_block *self,
    unsigned long event, void *data)
{
    struct fb_event *fb_event = NULL;
    int *blank = NULL;

    if (!bIsSupportWifiFilter) {
        return NOTIFY_DONE;
    }
    if (NULL == data) {
        return NOTIFY_DONE;
    }

    fb_event = data;
    blank = fb_event->data;

    if (NULL == blank) {
        return NOTIFY_DONE;
    }

    if (event == FB_EVENT_BLANK)
    {
        if (*blank == FB_BLANK_UNBLANK){            //screenon
            FILTER_LOGI("screenon");
            atomic_set(&bScreenoff, 0);
            wake_up_condition = WAKE_SCRN_ON;
            wake_up_interruptible(&mythread_wq);
        }else if (*blank == FB_BLANK_POWERDOWN){    //screenoff
            FILTER_LOGI("screenoff");
            atomic_set(&bScreenoff, 1);
            wake_up_condition = WAKE_SCRN_OFF;
            wake_up_interruptible(&mythread_wq);
        }else{
            FILTER_LOGI("other, blank value: %d", *blank);
        }
    }

    return NOTIFY_DONE;
}
#endif

static struct notifier_block wifi_filter_fb_notifier = {
#ifdef CONFIG_DKMD_DPU_OHOS
    .notifier_call = wifi_filter_dkmd_notifier_cb,
#else
    .notifier_call = wifi_filter_fb_notifier_cb,
#endif
};

static void reg_fb_notification(void)
{
#ifdef CONFIG_DKMD_DPU_OHOS
    dkmd_register_client(&wifi_filter_fb_notifier);
#else
    fb_register_client(&wifi_filter_fb_notifier);
#endif
}

static void unreg_fb_notification(void)
{
#ifdef CONFIG_DKMD_DPU_OHOS
    dkmd_unregister_client(&wifi_filter_fb_notifier);
#else
 	fb_unregister_client(&wifi_filter_fb_notifier);
#endif
}

int hw_register_wlan_filter(struct hw_wlan_filter_ops *ops)
{
    int interval;
    struct rtc_time tm;
    if ( NULL == ops ) {
        return -1;
    }
    
    gWlanFilterOps.add_filter_items     = ops->add_filter_items;
    gWlanFilterOps.clear_filters        = ops->clear_filters;
    gWlanFilterOps.get_filter_pkg_stat  = ops->get_filter_pkg_stat;
    gWlanFilterOps.set_filter_enable    = ops->set_filter_enable;
    gWlanFilterOps.set_filter_enable_ex = ops->set_filter_enable_ex;
    /*
     * get wifi chip type
     */
    if (isMyConnectivityChip(CHIP_TYPE_HI110X)) {
        FILTER_LOGD("this is hisi chip");
        g_item_cnt_max = ITEM_COUNT_MAX_HISI;
    } else {
        g_item_cnt_max = ITEM_COUNT_MAX_BRODCOM;
    }
    spin_lock_bh(&item_lock);
    g_item_info.p_items_data = (hw_wifi_filter_item *)kzalloc(
            sizeof(hw_wifi_filter_item)*g_item_cnt_max, GFP_ATOMIC);
    if (NULL == g_item_info.p_items_data) {
        FILTER_LOGE("malloc failed!");
        spin_unlock_bh(&item_lock);
        return -1;
    }
    spin_unlock_bh(&item_lock);
    bIsSupportWifiFilter = true;
    return 0;
}
EXPORT_SYMBOL(hw_register_wlan_filter);

int hw_unregister_wlan_filter(void)
{
    gWlanFilterOps.add_filter_items     = NULL;
    gWlanFilterOps.clear_filters        = NULL;
    gWlanFilterOps.get_filter_pkg_stat  = NULL;
    gWlanFilterOps.set_filter_enable    = NULL;
    gWlanFilterOps.set_filter_enable_ex = NULL;
    FILTER_LOGI("hw_unregister_wlan_filter");
    spin_lock_bh(&item_lock);
    if (g_item_info.p_items_data != NULL) {
        kfree(g_item_info.p_items_data);
        g_item_info.p_items_data = NULL;
    }
    bIsSupportWifiFilter = false;
    spin_unlock_bh(&item_lock);
    return 0;
}
EXPORT_SYMBOL(hw_unregister_wlan_filter);

void handle_ipv4_info(struct sk_buff *skb, hw_wifi_filter_item *p_item)
{
    const struct iphdr *ip = NULL;
    ip = ip_hdr(skb);
    if (ip == NULL) {
		FILTER_LOGE("ipv4 is null");
        return;
    }

    if (ip->protocol == IPPROTO_TCP) {
		FILTER_LOGI("tcp_ipv4 dest=%d, source=%d", ntohs(tcp_hdr(skb)->dest), ntohs(tcp_hdr(skb)->source));
        p_item->port = tcp_hdr(skb)->dest;
        p_item->protocol = IPPROTO_TCP;
        p_item->filter_cnt = ntohs(ETH_P_IP);
		return;
    }

    if (ip->protocol == IPPROTO_UDP) {
		FILTER_LOGI("udp_ipv4 dest=%d, source=%d", ntohs(udp_hdr(skb)->dest), ntohs(udp_hdr(skb)->source));
        p_item->port = udp_hdr(skb)->dest;
        p_item->protocol = IPPROTO_UDP;
        p_item->filter_cnt = ntohs(ETH_P_IP);
		return;
    }
}

void handle_ipv6_info(struct sk_buff *skb, hw_wifi_filter_item *p_item)
{
	const struct ipv6hdr *ip = NULL;
	ip = ipv6_hdr(skb);
	if (ip == NULL) {
		FILTER_LOGE("ipv6 is null");
		return;
	}

	if (ip->nexthdr == NEXTHDR_TCP) {
		FILTER_LOGI("tcp_ipv6 dest=%d, source=%d", ntohs(tcp_hdr(skb)->dest), ntohs(tcp_hdr(skb)->source));
		p_item->port = tcp_hdr(skb)->dest;
		p_item->protocol = IPPROTO_TCP;
		p_item->filter_cnt = ntohs(ETH_P_IPV6);
		return;
	}

	if (ip->nexthdr == NEXTHDR_UDP) {
		FILTER_LOGI("udp_ipv6 dest=%d, source=%d", ntohs(udp_hdr(skb)->dest), ntohs(udp_hdr(skb)->source));
		p_item->port = udp_hdr(skb)->dest;
		p_item->protocol = IPPROTO_UDP;
		p_item->filter_cnt = ntohs(ETH_P_IPV6);
		return;
	}
}

void get_wifi_filter_info(struct sk_buff *skb, hw_wifi_filter_item *p_item)
{
	if (skb == NULL) {
		FILTER_LOGE("skb is null");
		return;
	}
	if (p_item == NULL) {
		FILTER_LOGE("p_item is null");
		return;
	}

	unsigned short protocol_ip = skb->protocol;
	FILTER_LOGI("protocol_ip=%d", skb->protocol);
	if (protocol_ip == ntohs(ETH_P_IP)) {
		handle_ipv4_info(skb, p_item);
	} else if (protocol_ip == ntohs(ETH_P_IPV6)) {
		handle_ipv6_info(skb, p_item);
	}
}

static bool handle_screenoff_info(struct sk_buff *skb)
{
	hw_wifi_filter_item item;
	bool ret = false;

	memset_s(&item, sizeof(hw_wifi_filter_item), 0, sizeof(hw_wifi_filter_item));
	get_wifi_filter_info(skb, &item);
	ret = add_to_screenoff_table(&item);

	return ret;
}

static bool add_to_screenoff_table(hw_wifi_filter_item *p_item)
{
	if (g_item_info.p_items_data == NULL) {
		return false;
	}

	FILTER_LOGI("scrnoff_index=%d",g_item_info.scrnoff_index);

	if (g_item_info.scrnoff_index >= g_item_cnt_max) {
		FILTER_LOGE("add_to_screenoff_table too much!");
		return false;
	}

	memcpy_s(&g_item_info.p_items_data[g_item_info.scrnoff_index++],
	    sizeof(hw_wifi_filter_item), p_item, sizeof(hw_wifi_filter_item));
	if (g_item_info.scrnoff_index < g_item_cnt_max) {
		g_item_info.scrnoff_item_cnt = g_item_info.scrnoff_index;
	} else {
		g_item_info.scrnoff_item_cnt = g_item_cnt_max;
		g_item_info.scrnoff_index = 0;
	}
	return true;
}

void get_filter_info(
    struct sk_buff *skb,
    const struct nf_hook_state *state,
    unsigned int hook,
    const struct xt_table_info *private,
    const struct ipt_entry *e)
{
	bool success = false;
	spin_lock_bh(&item_lock);

	if (!bIsSupportWifiFilter) {
		FILTER_LOGD("wifi filter is not support");
		spin_unlock_bh(&item_lock);
		return;
	}
	if (NULL == state) {
		FILTER_LOGE("state is null");
		spin_unlock_bh(&item_lock);
		return;
	}
	if (hook != NF_INET_LOCAL_IN) {
		spin_unlock_bh(&item_lock);
		return;
	}

	success = handle_screenoff_info(skb);
	if (!success) {
		spin_unlock_bh(&item_lock);
		return;
	}

	spin_unlock_bh(&item_lock);

	wake_up_condition = WAKE_ADD;
	wake_up_interruptible(&mythread_wq);
}

void get_filter_infoEx(struct sk_buff *skb)
{
	bool success = false;
	spin_lock_bh(&item_lock);

	if (!bIsSupportWifiFilter) {
		FILTER_LOGD("wifi filter is not support");
		spin_unlock_bh(&item_lock);
		return;
	}
	if (skb->dev != NULL) {
		if (strncmp(skb->dev->name, WLAN_NAME, strlen(WLAN_NAME)) != 0) {
			spin_unlock_bh(&item_lock);
			return;
		}
	}
	if (atomic_read(&bScreenoff) == 0) {
		spin_unlock_bh(&item_lock);
		return;
	}
	
    FILTER_LOGD("screen if off");
    // add to screenoff table
    success = handle_screenoff_info(skb);
	if (!success) {
		spin_unlock_bh(&item_lock);
		return;
	}
	spin_unlock_bh(&item_lock);
	wake_up_condition = WAKE_ADD;
	wake_up_interruptible(&mythread_wq);
}

void get_pg_app_info(int port, int protocol)
{
	hw_wifi_filter_item item;
	bool success = false;

	spin_lock_bh(&item_lock);

	if (!bIsSupportWifiFilter) {
		FILTER_LOGD("wifi filter is not support");
		spin_unlock_bh(&item_lock);
		return;
	}
	if (atomic_read(&bScreenoff) == 0) {
		spin_unlock_bh(&item_lock);
		return;
	}

	item.port = port;
	item.protocol = protocol;
	success = add_to_screenoff_table(&item);
	if (!success) {
		spin_unlock_bh(&item_lock);
		return;
	}
	spin_unlock_bh(&item_lock);
	wake_up_condition = WAKE_ADD;
	wake_up_interruptible(&mythread_wq);
}

static void add_echo_info(void)
{
	hw_wifi_filter_item item;
	bool success = false;

	spin_lock_bh(&item_lock);
	item.port = ntohs(ECHO_PORT);
	item.protocol = IPPROTO_TCP;
	success = add_to_screenoff_table(&item);
	spin_unlock_bh(&item_lock);
	if (!success)
		return;
    if (g_item_info.p_items_data != NULL)
		hw_add_filter_items(&g_item_info.p_items_data[0], g_item_info.scrnoff_item_cnt);
}

static void add_dns_info(void)
{
    FILTER_LOGI("add_dns_info ");
	hw_wifi_filter_item item;
	bool success = false;

	spin_lock_bh(&item_lock);
	item.port = ntohs(DNS_PORT);
	item.protocol = IPPROTO_UDP;
	success = add_to_screenoff_table(&item);
	spin_unlock_bh(&item_lock);
	if (!success) {
		return;
    }
    if (g_item_info.p_items_data != NULL) {
		hw_add_filter_items(&g_item_info.p_items_data[0], g_item_info.scrnoff_item_cnt);
    }
}

static int hw_add_filter_items(hw_wifi_filter_item *items, int count)
{
    if ( NULL == gWlanFilterOps.add_filter_items )
    {
        return -1;
    }
	FILTER_LOGI("hw_add_filter_items count=%d", count);

    return gWlanFilterOps.add_filter_items(items, count);
}

static int hw_clear_all_item(void)
{
    if ( NULL == gWlanFilterOps.clear_filters )
    {
        return -1;
    }

    spin_lock_bh(&item_lock);

    g_item_info.scrnoff_item_cnt = 0;
    g_item_info.scrnoff_index = 0;
    if (g_item_info.p_items_data != NULL) {
        memset_s(&g_item_info.p_items_data[0], sizeof(hw_wifi_filter_item)*g_item_cnt_max, 0,
            sizeof(hw_wifi_filter_item)*g_item_cnt_max);
    }

    spin_unlock_bh(&item_lock);

    FILTER_LOGD("hw_clear_filter_item");
    return gWlanFilterOps.clear_filters();
}

int hw_set_net_filter_enable(int enable)
{
    FILTER_LOGD("doze enable=%d", enable);
    return 0;
}
EXPORT_SYMBOL(hw_set_net_filter_enable);

int hw_set_net_filter_re_enable(int state)
{
    if (atomic_read(&bScreenoff) == 1) {
        if (state == WIFI_OPEN) {
            FILTER_LOGI("wake wifi reanable");
            wake_up_condition = WAKE_WIFI_REENABLE;
            wake_up_interruptible(&mythread_wq);
            return 0;
        } else {
            FILTER_LOGD("WiFi not open already");
            return -1;
        }
    }
    FILTER_LOGI("not in screen off");
    return -1;
}
EXPORT_SYMBOL(hw_set_net_filter_re_enable);

int hw_set_net_filter_user_sleep(int sleep)
{
    FILTER_LOGI("handle sleep state:%d", sleep);
    if (sleep) {
        wake_up_condition = WAKE_ENTER_NIGHT;
    } else {
        wake_up_condition = WAKE_EXIT_NIGHT;
    }
    wake_up_interruptible(&mythread_wq);
    return 0;
}
EXPORT_SYMBOL(hw_set_net_filter_user_sleep);

static void handle_wake_scrn_off(void)
{
	if (gWlanFilterOps.set_filter_enable != NULL) {
		gWlanFilterOps.set_filter_enable(true);
		add_echo_info();
	}
}

static void handle_wake_screen_on(void)
{
    if (gWlanFilterOps.set_filter_enable != NULL) {
        gWlanFilterOps.set_filter_enable(false);
    }
    hw_clear_all_item();
}

static void handle_wake_exit_night(void)
{
    bIsNight = false;
	if (gWlanFilterOps.set_filter_enable_ex != NULL) {
		FILTER_LOGD("night set_filter_enable_ex false");
		gWlanFilterOps.set_filter_enable_ex(TYPE_ICMP,false);
	}
}

static void handle_wake_enter_night(void)
{
    bIsNight = true;
    add_dns_info();
    if (gWlanFilterOps.set_filter_enable_ex != NULL) {
        FILTER_LOGD("night set_filter_enable_ex true");
        gWlanFilterOps.set_filter_enable_ex(TYPE_ICMP,true);
    }
}

static void handle_wifi_re_enable(void)
{
	if (gWlanFilterOps.set_filter_enable != NULL) {
        FILTER_LOGI("set_filter_enable");
		gWlanFilterOps.set_filter_enable(true);
		add_echo_info();
        if (bIsNight) {
            add_dns_info();
        }
	}
}

static int wifi_filter_threadfn(void *data)
{
    while(1)
    {
        wait_event_interruptible(mythread_wq, wake_up_condition);
		FILTER_LOGI("filter thread wake_up_condition=%d",wake_up_condition);
        switch(wake_up_condition) {
        case WAKE_ADD:
            if (g_item_info.p_items_data != NULL) {
                hw_add_filter_items(&g_item_info.p_items_data[0],
                    g_item_info.scrnoff_item_cnt);
            }
            break;
        case WAKE_SCRN_ON:
            handle_wake_screen_on();
            break;
        case WAKE_SCRN_OFF:
			handle_wake_scrn_off();
            break;
        case WAKE_ENTER_NIGHT:
            handle_wake_enter_night();
            break;
        case WAKE_EXIT_NIGHT:
			handle_wake_exit_night();
            break;
        case WAKE_WIFI_REENABLE:
            handle_wifi_re_enable();
            break;
        default:
            break;
        }
        wake_up_condition = WAKE_INIT;
    }
    return 0;
}

static int __init init_kthread(void)
{
    int err;
    init_waitqueue_head(&mythread_wq);
    WifiFilterThread = kthread_run(wifi_filter_threadfn,NULL,"wifi_filter_thread");
    if (IS_ERR(WifiFilterThread))
    {
        FILTER_LOGE("create new kernel thread failed");
        err = PTR_ERR(WifiFilterThread);
        WifiFilterThread = NULL;
        return err;
    }
    reg_fb_notification();
    return 0;
}

static void __exit exit_kthread(void)
{
    unreg_fb_notification();
    if(WifiFilterThread)
    {
        FILTER_LOGI("stop MyThread");
        kthread_stop(WifiFilterThread);
    }
}

late_initcall(init_kthread);
module_exit(exit_kthread);


MODULE_AUTHOR("z00220931");
MODULE_LICENSE("GPL");
