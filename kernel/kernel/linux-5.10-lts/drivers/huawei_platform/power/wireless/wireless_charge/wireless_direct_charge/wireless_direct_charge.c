...
static bool wldc_check_decrease_vout(struct wldc_dev_info *di, struct wldc_regulation_data *data)
{
	...
	if ((data->rx_iout > data->rx_imax + WLDC_DFT_IMAX_ERR_TH) ||
		...
		di->rx_vout_set = power_min_positive(data->rx_vout, di->rx_vout_set) - vstep * data->cp_ratio;
...
static bool wldc_check_increase_vout(struct wldc_dev_info *di, struct wldc_regulation_data *data)
{
	int vstep = di->vstep / SC_DEFAULT_VOLT_RATIO * di->dc_ratio;
	int cp_iout_err_lth = di->cp_iout_err_lth * SC_DEFAULT_VOLT_RATIO / di->dc_ratio;

	if ((data->cp_iout_avg > di->cur_cp_iout_hth - cp_iout_err_lth) ||
		(data->cp_iout > di->cur_cp_iout_hth - cp_iout_err_lth))
		return false;
	...
	if (data->cp_iout_avg < di->cur_cp_iout_hth - cp_iout_err_lth) {
		...
		hwlog_info("[%s] cp_iout_avg:%dmA<%dmA, increase rx_vout\n",
			__func__, data->cp_iout_avg,
			di->cur_cp_iout_hth - cp_iout_err_lth);
	}
...
static void wldc_charge_regulation(struct wldc_dev_info *di)
{
	...
	if (!wldc_check_decrease_vout(di, &data) && !wldc_check_increase_vout(di, &data))
		return;

	wldc_rx_vset_volt_check(di, data.rx_vout, data.cp_ratio, data.rx_vmax);
	ret = wlrx_pmode_set_vrx(WLTRX_DRV_MAIN, di->rx_vout_set);
	if (ret)
...
static void wldc_check_recharge_path(struct wldc_dev_info *di)
{
	...
	dc_rechg_set_enable(rechg_en, rechg_ibat);
	if (rechg_en) {
...
static void wldc_check_exit_condition(struct wldc_dev_info *di)
{
	...
	exp_pid = wlrx_pmode_get_curr_int_cfg(WLTRX_DRV_MAIN, WLRX_PMODE_EXP_MODE);
...
static void wldc_select_charge_param(struct wldc_dev_info *di)
{
	...
	pctrl_irx = wlrx_pctrl_get_para(WLTRX_DRV_MAIN, WLRX_PCTRL_IRX_LIM);
...
static void wldc_stop_limit_output_current(struct wldc_dev_info *di)
{
	...
	for (i = 0; i < 8; i++) { /* 8: limit current cnt */
		if (wlrx_get_wired_channel_state() == WIRED_CHANNEL_ON)
			break;
		rx_vout -= rx_vstep;
		if (wlrx_pmode_set_vrx(WLTRX_DRV_MAIN, rx_vout))
			break;
...
static void wldc_stop_output_check(struct wldc_dev_info *di)
{
	...
	wlrx_pmode_set_vrx(WLTRX_DRV_MAIN, vfc_reg); /* in charging plugin otg ovp */
...
void wldc_stop_charging(struct wldc_dev_info *di)
{
	...
	wldc_cp_reset(di);
	power_event_bnc_notify(POWER_BNT_WLC, POWER_NE_WLC_DC_STOP_CHARGING, NULL);
	...
	struct wldc_dev_info *di)
{
	...
	(void)power_dts_read_u32(power_dts_tag(HWLOG_TAG), np,
		"cp_iout_err_lth", &di->cp_iout_err_lth,
		di->cp_iout_err_hth);
