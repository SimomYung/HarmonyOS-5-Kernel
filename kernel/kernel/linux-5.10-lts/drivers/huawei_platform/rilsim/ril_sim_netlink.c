...
static int process_sim_update_msg(const struct nlmsghdr *nlh)
{
	...
	int ret = memcpy_s(&sim_pin_info[slot_id], sizeof(knl_sim_pin_info), pin_info, sizeof(*pin_info));
	if (ret != EOK) {
		hwlog_err("%s: memcpy_s error: %d\n", __func__, ret);
		return KNL_FAILED;
	}
...
static int send_get_pin_info_cnf(u32 pid, int slot_id)
{
	...
	/* fill user data */
	int result = memcpy_s(nlmsg_data(nlh), (size_t)size, &sim_pin_info[slot_id], (size_t)size);
	if (result != EOK) {
		hwlog_err("%s: memcpy_s error: %d\n", __func__, result);
		ret = KNL_FAILED;
		goto end;
...
static int send_query_verified_cnf(u32 pid)
{
	...
	int result = memcpy_s(nlmsg_data(nlh), (size_t)size, &is_verified, (size_t)size);
	if (result != EOK) {
		hwlog_err("%s: memcpy_s error: %d\n", __func__, result);
		ret = KNL_FAILED;
		goto end;
...
static void sim_netlink_init(void)
{
	...
	int ret = memset_s(sim_pin_info, sizeof(sim_pin_info), 0, sizeof(sim_pin_info));
	if (ret != EOK) {
		hwlog_err("%s: memset_s error: %d\n", __func__, ret);
		return;
	}
