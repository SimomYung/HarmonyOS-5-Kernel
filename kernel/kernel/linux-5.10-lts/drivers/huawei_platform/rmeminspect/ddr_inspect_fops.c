...
#elif defined(DDR_NASHVILLE_PLATFORM)
#define DDR_INFO_ADDR 0xFDB23428
#elif defined(DDR_XIAN_PLATFORM)
#define DDR_INFO_ADDR 0xFDB23428
#elif defined(DDR_CHARLOTTE_PRO_PLATFORM)
#define DDR_INFO_ADDR 0xEC91B428
#else
				...
				unsigned long start_pfn, unsigned long nr_pages)
{
	...
	if (z == NULL) {
		pr_err("%s: z is invaild.", __func__);
		return false;
	}
	...
	size_t count, loff_t *f_pos)
{
	...
	if ((buf == NULL) || (filp == NULL) || (f_pos == NULL)) {
		pr_err("%s: buf or filp or f_pos is invaild.", __func__);
		return -EINVAL;
	}
	...
	if (copy_to_user(buf, (void *)(uintptr_t)ddr_insp_dev->req_vma + *f_pos, count)) {
		mutex_unlock(&ddr_insp_dev->ddr_inspect_mutex);
		return -EFAULT;
	}
	*f_pos += count;
...
static int ddr_inspect_mmap(struct file *filp, struct vm_area_struct *vma)
{
	...
	if (!filp || !vma) {
		pr_err("%s: filp or vma is invaild.", __func__);
		return -EINVAL;
	}
	...
	if (vma->vm_end - vma->vm_start > max_size) {
		pr_err("mmap error:  %lx > %lx", vma->vm_end - vma->vm_start, max_size);
		mutex_unlock(&ddr_insp_dev->ddr_inspect_mutex);
		return -EINVAL;
	}
	...
	for (req_idx = 0; req_idx < ddr_insp_dev->mem_req_num; req_idx++) {
		...
		if (insp_vma->req_pfn != 0 && valid_mmap_phys_addr_range(
			...
			if (ret) {
				pr_err("%s %u error %d", __func__, insp_vma->req_pfn, ret);
				mutex_unlock(&ddr_insp_dev->ddr_inspect_mutex);
				return ret;
			}
		}
