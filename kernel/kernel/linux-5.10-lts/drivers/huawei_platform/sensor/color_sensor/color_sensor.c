...
static void color_i2c_transfer_failure(void)
{
#if defined(CONFIG_HUAWEI_OHOS_DSM) || defined(CONFIG_HUAWEI_DSM)
	if (dsm_client_ocuppy(shb_dclient))
		return;
	dsm_client_record(shb_dclient, "I2C transfer fail\n");
	dsm_client_notify(shb_dclient, DSM_AP_ERR_COLORSENSOR_I2C_ERR);
	hwlog_err("%s I2C transfer fail\n", __func__);
#endif
}

static void rgb_i2c_err_dmd(void)
{
#if defined(CONFIG_HUAWEI_OHOS_DSM) || defined(CONFIG_HUAWEI_DSM)
	static int i2c_err_dmd_cnt;
	static int dmd_report_cnt;

	if (!rgb_probe_succ)
		return;

	i2c_err_dmd_cnt++;
	if (i2c_err_dmd_cnt >= RGB_I2C_ERR_DMD_PRINT_CNT_MAX)
		i2c_err_dmd_cnt = 0;
	else
		return;
	dmd_report_cnt++;
	if (dmd_report_cnt >= RGB_DMD_REPORT_MAX_CNT) {
		dmd_report_cnt = RGB_DMD_REPORT_MAX_CNT;
		return;
	}
	color_i2c_transfer_failure();
#endif
}
	...
	struct device_attribute *attr, char *buf)
{
	...
	if (chip->support_flk_size_adj)
		max_flk_data_len = chip->flk_data_len * CONVERT_INT_TO_CHAR;
	else
		max_flk_data_len = DEFAULT_FLk_DATA_SIZE;
	return max_flk_data_len;
}

static ssize_t flicker_data_len_show(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	struct color_chip *chip = NULL;
	int flk_data_length;
	...
	if (chip->support_flk_size_adj)
		flk_data_length = chip->flk_data_len;
	else
		flk_data_length = DEFAULT_FLK_DATA_LEN;
	return sprintf_s(buf, MAX_ALGO_TYPE_STR_LEN, "%d", flk_data_length);
}
...
DEVICE_ATTR(flicker_data_len, 0660, flicker_data_len_show, NULL);
...
static struct attribute *color_sensor_attributes[] = {
	...
	&dev_attr_flicker_data_len.attr,
