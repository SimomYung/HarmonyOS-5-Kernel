...
unsigned int ir_factor[10] = { 170, 100, 100, 50, 50, 70, 70, 100, 100, 100 }; // default ir factor expand 100
static bool als_enabled = false;

static bool sy3132cs_check_ic_poweroff(struct sy3132cs_ctx *ctx);
static bool sy3132cs_software_reset(struct sy3132cs_ctx *ctx);
...
static int sy3132cs_get_flk_autogain_limit(int data, uint16_t *low_limit, uint16_t *high_limit)
{
    int data_offset = FLK_DATA_OFFSET;
	int flk_count_for_down_max = MAX_FLK_COUNT_FOR_DOWN;
	int index = 0;
	int over_flow_count_1 = 0;
	int i;
	int flk_fifo_data_index = 0;

	if (data == 0) { // 0.25 fullcount = 2159
		*low_limit = LOW_LIMIT_300;
		*high_limit = FLK_AUTO_GAIN_DATA + data_offset;
		flk_count_for_down_max /= 4;
	} else if (data == 1) { // 0.5 fullcount = 4207
		*low_limit = LOW_LIMIT_800;
		*high_limit = FLK_AUTO_GAIN_DATA * 2 + data_offset;
		flk_count_for_down_max /= 2;
	} else if (data == 2) { // 1ms fullcount = 8303
		*low_limit = LOW_LIMIT_1000;
		*high_limit = FLK_AUTO_GAIN_DATA * 4 + data_offset;
	}

	index = sy3132cs_algo.fifo_counter / 2 - sy3132cs_algo.fifo_level;
	if (index > 0) {
		for (i = 0; i < sy3132cs_algo.fifo_level; i++)
			flk_fifo_data_index = index + i;
			if (flk_fifo_data_index >= SY3132_FIFO_BYTE_DATA_NUM)
				return over_flow_count_1;
			if (sy3132cs_algo.flk_fifo_data[index + i] > flk_count_for_down_max)
				over_flow_count_1++;
	}
	return over_flow_count_1;
}
...
static bool sy3132cs_flk_auto_gain(struct sy3132cs_ctx *ctx, uint16_t *gain, uint16_t *it)
{
	uint16_t data_low_limit;
	uint16_t current_gain = sy3132cs_algo.flk_gain;
	uint16_t data_high_limit = 0;
	int over_flow_count;
	int index;

	index = sy3132cs_get_ic_func(ctx->handle, sy3132cs_ic_func.FLK_IT);
	over_flow_count = sy3132cs_get_flk_autogain_limit(index, &data_low_limit, &data_high_limit);

	if (data_high_limit > FLK_AUTO_GAIN_HIGH_LIMIT_DATA)
		data_high_limit = FLK_AUTO_GAIN_HIGH_LIMIT_DATA;

	if (over_flow_count > FLK_AUTO_GAIN_OVER_FLOW_LIMIT_DATA) {
		if (current_gain > FLK_GAIN_1) {
			*gain = current_gain / 2;
			current_gain = current_gain / 2;
...
static void sy3132cs_calcu_flk_fifo_data(const struct sy3132cs_ctx *ctx)
{
	...
	sy3132cs_algo.fifo_level = fifo_level;
	if (fifo_level != 0) {
...
static int sy3132cs_flk_data_check(struct sy3132cs_ctx *ctx)
{
	...
	if (sy3132cs_algo.flk_delay_counter) {
		...
		if (als_valid_flag && !cali_flag) {
			if (sy3132cs_flk_auto_gain(ctx, &flk_gain, &flk_it) == true) {
...
static void read_c8_nir_darktrim(const struct sy3132cs_ctx *ctx)
{
	...
	if (value & mask)
		sy3132cs_algo.dark_data.dark_ratio[CH7_VALUE] =
			DATA_EXPAND_100 - (full_trimdata - index) * trimration / DATA_EXPAND_100;
	else
...
static uint8_t read_channel_darktrim_code(const struct sy3132cs_ctx *ctx, int *ration)
{
	...
	hwlog_info("%s, reg[0x%x] = 0x%x, reg[0x%x] = 0x%x, reg[0x%x] = 0x%x,, reg[0x%x] = 0x%x,\n", __func__,
		SY3132CS_IC_VERSION_REG1, factory_trimcode[0], SY3132CS_IC_VERSION_REG2, factory_trimcode[1],
		SY3132CS_IC_VERSION_REG3, factory_trimcode[2], 0x3D, factory_trimcode[3]);
...
static void sy3132cs_get_default_dark(const struct sy3132cs_ctx *ctx)
{
	...
	if ((cal_temp & FLK_TRIM_MASK) > 0)
		flk_trim_ration = (cal_temp & FLK_BIT5_MASK) > 0 ?
			(int)(FLK_TRIM_HIGHSTEP_FULL_BITS - (cal_temp & FLK_TRIM_STEP_MASK)) * (-FLK_TRIM_STEP) :
			(int)((cal_temp & FLK_TRIM_STEP_MASK) + FLK_TRIM_HIGHSTEP_BASE_DATA) * (FLK_TRIM_STEP);
	else
		flk_trim_ration = (cal_temp & FLK_BIT5_MASK) > 0 ?
			(int)(FLK_TRIM_STEP_FULL_BITS - (cal_temp & FLK_TRIM_STEP_MASK)) * (-FLK_TRIM_STEP) :
			(int)(cal_temp & FLK_TRIM_STEP_MASK) * (FLK_TRIM_STEP);
...
void sy3132cs_dummy_avg_proc(void)
{
	...
	ration = sy3132cs_algo.als_trans_gain * SY3132CS_ALS_IT_ST * DATA_EXPAND_100 *
		DATA_EXPAND_100 / dummy_it / ADCTIME_DIV_RATION;
...
bool sy3132cs_dark_ir_algo(void)
{
	int nor_ch5data = sy3132cs_algo.avg_data[4] * sy3132cs_algo.als_trans_it *
		sy3132cs_algo.als_trans_gain / DATA_EXPAND_100;
	int i;
	int tempdark[10] = { 0 };
	const int adc_ration[10] = { 100, 50, 50, 25, 25, 25, 25, 100, 100, 100 };
	int offset = sy3132cs_algo.dummyc_avg * sy3132cs_algo.dark_data.dark_ratio[CH8_VALUE] * 2 /
		DATA_EXPAND_100 - IR_DARK_OFFSET;
	int32_t ir_ration = 0;
	if (!sy3132cs_algo.avg_data[8])
		return false;
	ir_ration = sy3132cs_algo.avg_data[4] / sy3132cs_algo.avg_data[8];

	if (nor_ch5data > CH5_IR_CHECK_LIMIT && ir_ration < CH5_IR_RATION_LIMIT) { // c5/nir
		for (i = 0; i < ALL_CHANNEL_NUM; i++) {
			if (sy3132cs_algo.dummya_avg < sy3132cs_algo.dark_data.dummya) {
				tempdark[i] = 0;
				continue;
			}
			if (i < CH7_VALUE || i == CH9_VALUE) // c1-c7, flk
				tempdark[i] = (sy3132cs_algo.dummya_avg - sy3132cs_algo.dark_data.dummya) *
					ir_factor[i] / DATA_EXPAND_100;
			else
				tempdark[i] = 0; // c8, nir
		}
		for (i = 0; i < (ALL_CHANNEL_NUM - 1); i++) {
			if (i < CH7_VALUE) { // c1-c7
				tempdark[i] += (sy3132cs_algo.dark_data.dummya * sy3132cs_algo.dark_data.dark_ratio[i] *
					adc_ration[i] / DATA_EXPAND_10000);
			} else if (i == CH7_VALUE) { // c8
				tempdark[i] += (sy3132cs_algo.dummyc_avg * sy3132cs_algo.dark_data.dark_ratio[CH7_VALUE] * 2 /
					DATA_EXPAND_100);
			} else {
				tempdark[i] += (offset > 0) ? offset : 0;
			}
			dark_value[i] = tempdark[i];
		}
		tempdark[CH9_VALUE] += (sy3132cs_algo.dark_data.dummya * sy3132cs_algo.dark_data.flk_dark_ration /
			DATA_EXPAND_100);
		dark_value[CH9_VALUE] = tempdark[CH9_VALUE] / (cali_flag ? CALI_GAIN_VALUE : CLEAR_GAIN_RATION);

		hwlog_info("ir dark_value, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d,\n",
			dark_value[0], dark_value[1], dark_value[2], dark_value[3], dark_value[4],
			dark_value[5], dark_value[6], dark_value[7], dark_value[8], dark_value[9]);
		return true;
	}
	return false;
}
...
void sy3132cs_first_selfclosing(void)
{
	struct color_chip *chip = p_chip;
	...
	if (startup_flag) {
		count++;
		if (count >= MODE3_SWITCH_MODE2_TIME && !cali_flag) {
			if (!als_enabled)
				sy3132cs_set_als_power_status(chip, CLOSE_ALS);
			else
				hwlog_info("%s: rgb enabled by hal, do not need close\n", __func__);
			count = MODE3_SWITCH_MODE2_TIME;
			startup_flag = false;
		}
	}
}
...
bool sy3132cs_sw_update_alsvalid(bool flag)
{
	int i;
	bool ret = false;
	static int count = 0;
	if (flag) {
		count = 0;
		for (i = 0; i < (SY3132CS_CHANNEL_NUMBER - 1); i++)
			sy3132cs_algo.last_raw_data[i] = sy3132cs_algo.ch_raw_data[i];
		return flag;
	}

	count++;
	if (count <= NO_UPDATA_ALSVALID_MAX_CNT)
		return flag;
	count = 0;

	if (sy3132cs_algo.last_raw_data[CH1_VALUE] == sy3132cs_algo.ch_raw_data[CH1_VALUE] &&
		sy3132cs_algo.last_raw_data[CH4_VALUE] == sy3132cs_algo.ch_raw_data[CH4_VALUE] &&
		sy3132cs_algo.last_raw_data[CH5_VALUE] == sy3132cs_algo.ch_raw_data[CH5_VALUE])
			ret = false;
	else
			ret = true;
...
void sy3132cs_data_algo_process(struct sy3132cs_ctx *ctx)
{
	...
	als_valid_flag = sy3132cs_sw_update_alsvalid(als_valid_flag);

	if (sy3132cs_software_reset(ctx)) {
		hwlog_info("%s sy3132cs_software_reset is ture", __func__);
		return;
	}

	if (!als_valid_flag) {
		hwlog_info("%s als_valid_flag is false", __func__);
		return;
	}
...
static void sy3132cs_ic_poweron(struct sy3132cs_ctx *ctx)
{
	struct color_chip *chip = p_chip;
...
static bool sy3132cs_software_reset(struct sy3132cs_ctx *ctx)
{
	...
	static uint16_t temp_lastrawdata[ALL_CHANNEL_NUM] = { 0 };
	int i;
	int count = 0;
	uint8_t buf[CHECK_WHT_NUM_OF_REG] = { 0 };

	buf[0] = sy3132cs_get_ic_func(ctx->handle, sy3132cs_ic_func.ALS_IT); // IT_INDEX
	buf[1] = sy3132cs_get_ic_func(ctx->handle, sy3132cs_ic_func.ALS_GAIN); // GAIN_INDEX
	color_sensor_get_byte(ctx->handle, SY3132CS_REG_ALS_CON5, &buf[2]); // 0x05
	color_sensor_get_byte(ctx->handle, SY3132CS_REG_AINT_CON, &buf[3]); // 0x07
	color_sensor_get_byte(ctx->handle, SY3132CS_REG_FINT_CON, &buf[4]); // 0x08
	color_sensor_get_byte(ctx->handle, SY3132CS_REG_ALS_LTL, &buf[5]); // 0x0B
	color_sensor_get_byte(ctx->handle, SY3132CS_REG_ALS_LTH, &buf[6]); // 0x0C
	if ((buf[0] > ALS_IT_INDEX_MAX) || (buf[1] > ALS_GAIN_INDEX_MAX) || (buf[2] != SY3132CS_REG_ALS_CON5_DEF_DATA) ||
		(buf[3] != REG_DEFAULT_DATA) || (buf[4] != REG_DEFAULT_DATA) ||
		(buf[5] != REG_DEFAULT_DATA) || (buf[6] != REG_DEFAULT_DATA)) {
		color_sensor_set_byte(ctx->handle, SY3133CS_COMMAND_SET_REG_DEFAULT, 0x00);
		sy3132cs_ic_poweron(ctx);
		color_sensor_set_byte(ctx->handle, SY3133CS_COMMAND_REST_ADC, 0x00);
		hwlog_info("%s, register change software reset init\n", __func__);
		return true;
	}

	for (i = 0; i < ALL_CHANNEL_NUM; i++) {
		if (temp_lastrawdata[i] == sy3132cs_algo.ch_raw_data[i])
			count++;
		else
			count = 0;
	}
	if (count == ALL_CHANNEL_NUM) {
		if ((sy3132cs_algo.ch_raw_data[0] == CANNEL_MAX_CONT) || (sy3132cs_algo.ch_raw_data[0] == 0))
			report_count = 0;
		else
			report_count++;
	} else {
	...
	for (i = 0; i < ALL_CHANNEL_NUM; i++)
		temp_lastrawdata[i] = sy3132cs_algo.ch_raw_data[i];
	if (report_count >= CHECK_NO_DATA_MAX_CNT) {
		report_count = 0;
		color_sensor_set_byte(ctx->handle, SY3133CS_COMMAND_SET_REG_DEFAULT, 0x00);
		sy3132cs_ic_poweron(ctx);
		color_sensor_set_byte(ctx->handle, SY3133CS_COMMAND_REST_ADC, 0x00);
		hwlog_info("%s, data no change software reset init\n", __func__);
		return true;
	}
	return false;
}
...
static void sy3132cs_get_dts_parameter(const struct device *dev)
{
	...
	rc = of_property_read_u32_array(dev->of_node, "ir_factor_array", ir_factor, ARRAY_SIZE(ir_factor));
	if (rc < 0)
		hwlog_err("%s, get ir_factor_array failed\n", __func__);
	for (i = 0; i < DATA_EXPAND_10; i++)
		hwlog_info("%s ir_factor[%d] = %d\n", __func__, i, ir_factor[i]);
