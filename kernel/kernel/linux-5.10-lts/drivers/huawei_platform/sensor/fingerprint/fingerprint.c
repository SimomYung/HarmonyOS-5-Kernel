...
{
	unsigned int events[] = {
		EVENT_UP, EVENT_DOWN, EVENT_LEFT, EVENT_RIGHT, EVENT_HOLD, EVENT_CLICK,
		EVENT_DCLICK, EVENT_FINGER_UP, EVENT_FINGER_DOWN, EVENT_FINGER_IDENTIFY,
		EVENT_IDENTIFY_END, EVENT_FINGER_ENROLL, EVENT_FINGER_NAV_DOWN,
		EVENT_FINGER_NAV_UP, EVENT_FINGER_NAV_CILCK, EVENT_FINGER_NAV_DOWN_SECOND,
		EVENT_FINGER_NAV_FINISH, EVENT_FINGER_NAV_HOLD, EVENT_FINGER_NAV_TOUCH,
		EVENT_FINGER_OPEN_AFT, EVENT_FINGER_CLOSE_AFT
	};
	unsigned int i;

	for (i = 0; i < ARRAY_SIZE(events); i++) {
		input_set_capability(fp->input_dev, EV_KEY, events[i]);
		set_bit(events[i], fp->input_dev->evbit);
	}

	if (fp->cts_home > 0) {
		hwlog_info("%s fingerprint->cts_home=%d\n", __func__, fp->cts_home);
