...
static unsigned long g_finger_down_time;
#if defined(CONFIG_FB) || defined(CONFIG_DKMD_DKSM)
static struct fp_lcd_status g_lcd = { -1, FP_LCD_STATE_NULL, 0 };
#endif
...
/*
 * since HZ is 100, so the unit of jiffies is 10ms
 * therefore, suggest use the function below instead of jiffies to measure elapsed time. */
static unsigned long get_monotonic_timestamp_ms(void)
{
...
static ssize_t empty_store(struct device *device,
...
static DEFINE_MUTEX(g_lcd_mutex);

static ssize_t lcd_status_show(struct device *device,
	struct device_attribute *attribute, char *buffer)
{
#if defined(CONFIG_FB) || defined(CONFIG_DKMD_DKSM)
	ssize_t ret;
	struct fp_lcd_status *data = &g_lcd;
	if (buffer == NULL)
		return -EINVAL;
	mutex_lock(&g_lcd_mutex);
	ret = scnprintf(buffer, PAGE_SIZE, "{\"panel_id\": %d, \"state\": %d, \"offset_ms\": %lu}",
		data->panel_id, data->state, (get_monotonic_timestamp_ms() - data->time_stamp));
	mutex_unlock(&g_lcd_mutex);
	return ret;
#else
	if (buffer == NULL)
		return -EINVAL;
	return scnprintf(buffer, PAGE_SIZE, "{\"panel_id\": -1, \"state\": -1, \"offset_ms\": -1}");
#endif
}

static bool is_s4_mode = false;

static DEVICE_ATTR(irq, 0600, irq_get, empty_store);
...
static DEVICE_ATTR(low_temperature, 0660, low_temperature_show, empty_store);
static DEVICE_ATTR(lcd_status, 0660, lcd_status_show, empty_store);

static void noitfy_lcd_state_change(int panel_id, int state)
{
	unsigned long time_stamp = get_monotonic_timestamp_ms();
	mutex_lock(&g_lcd_mutex);
	g_lcd.panel_id = panel_id;
	g_lcd.state = state;
	g_lcd.time_stamp = time_stamp;
	sysfs_notify(&g_fingerprint->pf_dev->dev.kobj, NULL, dev_attr_lcd_status.attr.name);
	mutex_unlock(&g_lcd_mutex);
}
...
void fp_on_lcd_backlight_change(int panel_id, int value)
{
	...
	if ((last_value == 0) && (value > 0)) {
		hwlog_info("%s: panel_id: %d, backlight on\n", __func__, panel_id);
		noitfy_lcd_state_change(panel_id, FP_LCD_BACKLIGHT_ON);
	} else if ((last_value > 0) && (value == 0)) {
		hwlog_info("%s: panel_id: %d, backlight off\n", __func__, panel_id);
		noitfy_lcd_state_change(panel_id, FP_LCD_BACKLIGHT_OFF);
	}
	last_value = value;
}
...
static irqreturn_t fingerprint_irq_handler(int irq, void *handle)
{
	...
	g_finger_down_time = get_monotonic_timestamp_ms();
...
int fingerprint_get_dts_data(struct device *dev, struct fp_data *fp_data)
{
	...
	fp_data->pc_product = of_property_read_bool(np, "fingerprint,pc_product");
...
{
	int value = get_monotonic_timestamp_ms() - g_finger_down_time;
	int ret = copy_to_user(argp, &value, sizeof(value));
	if (ret) {
		hwlog_err("copy_to_user failed, ret=%d\n", ret);
		return -EFAULT;
	}
	return FP_RETURN_SUCCESS;
}
...
static int fp_ioc_send_screen_name(struct fp_data *fp, const void __user *argp)
{
	...
	hwlog_info("fp_ioctl_cmd info, info.data_len = %u, sizeof(fp->screen_name) = %u\n",
		info.data_len, sizeof(fp->screen_name));
	if (info.data_len >= sizeof(fp->screen_name)) {
		hwlog_err("data_len is invalid\n");
		return -EFAULT;
	}
...
static int fp_ioc_finger_position_event(struct fp_data *fp, const void __user *argp)
{
	struct fingerprint_position_info  finger_pos_info = { 0 };
 
	if (copy_from_user(&finger_pos_info, argp, sizeof(finger_pos_info))) {
		hwlog_err("%s copy_from_user failed\n", __func__);
		return -EFAULT;
	}
 
	input_report_rel(fp->input_dev_mouse, REL_X, finger_pos_info.finger_rel_x);
	input_report_rel(fp->input_dev_mouse, REL_Y, finger_pos_info.finger_rel_y);
	input_sync(fp->input_dev_mouse);
	hwlog_err("%s position report succ\n", __func__);
	return FP_RETURN_SUCCESS;
}
	...
	unsigned long arg)
{
	...
	case FP_IOC_CMD_GET_TOUCH_DURATION:
		hwlog_info("%s FP_IOC_CMD_GET_TOUCH_DURATION\n", __func__);
		ret = fp_ioc_get_touch_duration(argp);
		break;
	...
	case FP_IOC_CMD_WAIT_BACKLIGHT_STATUS:
		hwlog_info("%s FP_IOC_CMD_WAIT_BACKLIGHT_STATUS, it is obsoleted\n", __func__);
		break;
	case FP_IOC_CMD_MMI_TEST_STATUS:
		hwlog_info("%s FP_IOC_CMD_FP_MMI_TEST_STATUS\n", __func__);
		ret = fp_ioc_mmi_test_status(argp);
		break;
	case FP_IOC_CMD_FINGER_POSITION_EVENT:
		hwlog_info("%s FP_IOC_CMD_FINGER_POSITION_EVENT\n", __func__);
		ret = fp_ioc_finger_position_event(fp, argp);
		break;
...
static int fingerprint_device_create(struct fp_data *fp, struct device *dev)
{
	...
	fp->input_dev_mouse = devm_input_allocate_device(dev);
	if (!fp->input_dev_mouse) {
		hwlog_err("%s failed to allocate input dev mouse\n", __func__);
		return -ENOMEM;
	}
	fp->input_dev_mouse->name = "hw_fingerprint_mouse";
...
static void fingerprint_device_set(struct fp_data *fp)
{
	...
	input_set_capability(fp->input_dev, EV_KEY, EVENT_FINGER_NAV_DOWN);
	input_set_capability(fp->input_dev, EV_KEY, EVENT_FINGER_NAV_UP);
	input_set_capability(fp->input_dev, EV_KEY, EVENT_FINGER_NAV_CILCK);
	input_set_capability(fp->input_dev, EV_KEY, EVENT_FINGER_NAV_DOWN_SECOND);
	input_set_capability(fp->input_dev, EV_KEY, EVENT_FINGER_NAV_FINISH);
	input_set_capability(fp->input_dev, EV_KEY, EVENT_FINGER_NAV_HOLD);
	if (fp->cts_home > 0) {
	...
	set_bit(EVENT_FINGER_NAV_DOWN, fp->input_dev->evbit);
	set_bit(EVENT_FINGER_NAV_UP, fp->input_dev->evbit);
	set_bit(EVENT_FINGER_NAV_CILCK, fp->input_dev->evbit);
	set_bit(EVENT_FINGER_NAV_DOWN_SECOND, fp->input_dev->evbit);
	set_bit(EVENT_FINGER_NAV_FINISH, fp->input_dev->evbit);
	set_bit(EVENT_FINGER_NAV_HOLD, fp->input_dev->evbit);
}

static void fingerprint_device_mouse_set(struct fp_data *fp)
{
	fp->input_dev_mouse->evbit[0] |= BIT_MASK(EV_KEY) | BIT_MASK(EV_REL);
	fp->input_dev_mouse->relbit[0] |= BIT_MASK(REL_X) | BIT_MASK(REL_Y);
	input_set_capability(fp->input_dev_mouse, EV_REL, REL_X);
	input_set_capability(fp->input_dev_mouse, EV_REL, REL_Y);
	input_set_capability(fp->input_dev_mouse, EV_KEY, BTN_LEFT);
	input_set_capability(fp->input_dev_mouse, EV_KEY, BTN_RIGHT);
}
...
static int fingerprint_device_register(struct fp_data *fp)
{
	...
	ret = input_register_device(fp->input_dev_mouse);
	if (ret) {
		hwlog_err("%s input_register_dev_mouse failed ret=%d\n",
			__func__, ret);
		return ret;
	}
	...
	unsigned long action, void *data)
{
	...
	smp_rmb();
	hwlog_info("%s, action:%lu, blank:%d\n", __func__, action, *blank);
	if (action == FB_EVENT_BLANK && (event_data->info == registered_fb[0])) {
	...
	if (action == FB_EVENT_BLANK && (*blank == FB_BLANK_UNBLANK || *blank == FB_BLANK_POWERDOWN))
		noitfy_lcd_state_change(-1, (*blank == FB_BLANK_UNBLANK) ? FP_LCD_POWER_ON : FP_LCD_POWER_OFF);
...
#if defined(CONFIG_DKMD_DKSM)
static int fp_dkmd_notifier_callback(struct notifier_block *nb,
	unsigned long action, void *data)
{
	struct fp_data *fp = g_fingerprint;
	struct dkmd_event *event = data;
	if (!fp || !event)
		return NOTIFY_OK;

	hwlog_info("%s, action:%lu, panel_id:%d, value:%d\n", __func__, action, event->panel_id, event->value);
	if (action != DKMD_EVENT_BLANK ||
		(event->value != FB_BLANK_UNBLANK && event->value != FB_BLANK_POWERDOWN)) {
		return NOTIFY_OK;
	}
	noitfy_lcd_state_change(event->panel_id, (event->value == FB_BLANK_UNBLANK) ? FP_LCD_POWER_ON : FP_LCD_POWER_OFF);
	return NOTIFY_OK;
}

static struct notifier_block fp_dkmd_notifier_block = {
	.notifier_call = fp_dkmd_notifier_callback,
};
#endif
...
static int fingerprint_probe(struct platform_device *pdev)
{
	...
	hwlog_info("fingerprint driver kernel probe begin\n");
	...
	fingerprint_device_mouse_set(fp);
	...
	if (fp->notify_enable == FP_UNBLANK_NOTIFY_ENABLE) {
		...
		fp->fb_fp_notify.notifier_call = fp_fb_notifier_callback;
...
#endif
#if defined(CONFIG_DKMD_DKSM)
	dkmd_register_client(&fp_dkmd_notifier_block);
#endif
...
static int fingerprint_remove(struct platform_device *pdev)
{
	...
	input_free_device(fp->input_dev_mouse);
...
static int fingerprint_resume(struct device *dev)
{
	hwlog_info("%s\n", __func__);
	if (is_ulsr()) {
		hwlog_info("%s in ulsr\n ", __func__);
		struct fp_data *fp = dev_get_drvdata(dev);
		if (fp == NULL)
			return -EINVAL;
		int ret = irq_set_irq_type(gpio_to_irq(fp->irq_gpio), IRQF_TRIGGER_RISING |
			IRQF_ONESHOT | IRQF_NO_SUSPEND);
		if (ret) {
			hwlog_err("irq_set_irq_type error");
		}
	}
	return FP_RETURN_SUCCESS;
}
