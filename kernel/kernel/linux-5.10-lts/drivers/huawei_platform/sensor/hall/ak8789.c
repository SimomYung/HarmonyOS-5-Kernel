...
static unsigned int is_support_case_shield_report = 0;
int hinge_status = 0;
...
static struct error_no_map hall_error_no_map[] = {
	{ 926002000, "HALL_ERROR_NO", 1 },
};

int hall_errorno_to_str(int error_no, char *str, int bufflen)
{
	size_t i;
	size_t error_no_map_len = sizeof(hall_error_no_map) / sizeof(hall_error_no_map[0]);

    if (!str)
        return -1;

	for (i = 0; i < error_no_map_len; i++) {
		if (hall_error_no_map[i].error_no == error_no) {
			if (strlen(hall_error_no_map[i].name) >= bufflen) {
				hwlog_err("%s hall_error_no_map name too long\n", __func__);
				return -1;
			}
			if (strncpy_s(str, bufflen, hall_error_no_map[i].name, strlen(hall_error_no_map[i].name)) != 0) {
				hwlog_err("%s hall_error_no_map strncpy fail\n", __func__);
				return -1;
			}
			str[bufflen - 1] = '\0';
			hwlog_info("%s hall_error_no_map:%s\n", __func__, str);
			return 0;
		}
	}
	return -1;
...
static int hall_errorno_get_running_test_switch(int error_no)
{
	size_t i;
	size_t hall_error_no_map_len = sizeof(hall_error_no_map) / sizeof(hall_error_no_map[0]);

	for (i = 0; i < hall_error_no_map_len; i++) {
		if (hall_error_no_map[i].error_no == error_no)
			return hall_error_no_map[i].running_test_switch;
	}
	return 0;
}
#endif
...
struct dsm_client_ops hall_ops = {
	.poll_state = NULL,
	.dump_func = NULL,
#ifdef CONFIG_HISYSEVENT
	.errorno_to_str = hall_errorno_to_str,
	.errorno_get_running_test_switch = hall_errorno_get_running_test_switch,
#endif
};
#endif
...
static bool ak8789_lightstrap_get_hall_status(void)
{
	...
	hwlog_info("%s: hall value is %d, status is %u\n",
...
static void ak8789_report_delayed_work(struct work_struct *work)
{
	...
	if (is_support_case_shield_report)
		pdata = ((pdata & HALL_LEATHER_CASE_GPIO) && (pdata & HALL_FITTINGS_GPIO)) ? pdata : (pdata & ~1);
				     ...
				     struct device_node *node)
{
	...
	hall_dev->is_hi9030_ao_pin = of_property_read_bool(node, "is_hi9030_ao_pin");
	hwlog_info("is_hi9030_ao_pin = %u\n", hall_dev->is_hi9030_ao_pin);
...
#ifdef CONFIG_GPIO_HI9030
static void ak8789_hi9030_event_handler(int gpio, void *hall_dev)
{
	struct ak8789_data *data = NULL;
	struct hall_device *h_dev = hall_dev;

	if (!hall_dev) {
		hwlog_err("%s: hall dev null\n", __func__);
		return;
	}
	data = platform_get_drvdata(h_dev->pdev);
	if (data == NULL) {
		hwlog_err("%s: data pdev null\n", __func__);
		return;
	}
	hwlog_info("%s: gpio-%d irq occurred\n", __func__, gpio);
				   ...
				   enum hall_type type)
{
	...
	hwlog_info("%s gpio = %d, is_hi9030_ao_pin = %u\n", __func__, gpio, hall_dev->is_hi9030_ao_pin);
#ifdef CONFIG_GPIO_HI9030
	if (hall_dev->is_hi9030_ao_pin) {
		ret = hi9030_gpio_irq_request(gpio, (void *)&ak8789_hi9030_event_handler,
			IRQF_TRIGGER_FALLING | IRQF_TRIGGER_RISING, hall_dev);
		hwlog_info("use is_hi9030_ao_pin, gpio = %d, hahaha ret = %d\n", gpio, ret);
		hall_dev->h_info.irq[type] = gpio;
...
static void ak8789_prases_hall_workaround_param(struct device_node *node)
{
	...
	if (!of_property_read_u32(node, "is_support_case_shield_report", &tmp)) {
		is_support_case_shield_report = tmp;
		hwlog_info("is_support_case_shield_report = %u\n", is_support_case_shield_report);
	}
...
static void hall_exit_s4_report_delayed_work(struct work_struct *work)
{
	struct hall_device *temp_dev = NULL;
	struct ak8789_data *data = data_ak8789;
	packet_data temp_data;
...
static void exit_s4_hall_report_data(void)
{
	struct ak8789_data *data = data_ak8789;

	if (data == NULL) {
		hwlog_err("none ak8789 data is null\n");
		return;
	}

	queue_delayed_work(data->hall_wq,
			   &data->s4_delayed_work,
			   HZ / HALL_REPORT_MIN_PERIOD);
}

static int hall_pm_notifier(struct notifier_block *nb,
	unsigned long foo, void *bar)
{
	switch (foo) {
	case PM_HIBERNATION_PREPARE: /* Going to hibernate */
		hwlog_info("going to hibernate\n");
		break;
	case PM_POST_HIBERNATION: /* Hibernation finished */
		exit_s4_hall_report_data();
		hwlog_info("exit s4 hall report data done\n");
		break;
...
static struct notifier_block hall_pm_notify = {
	.notifier_call = hall_pm_notifier,
};

static void hall_s4_init(void)
{
	struct ak8789_data *data = data_ak8789;
	struct device_node *node = NULL;
	unsigned int is_support_s4 = 0;
	...
	if (node == NULL) {
		hwlog_err("ak8789 data node is null\n");
		return;
	}

	if (!of_property_read_u32(node, "hall_support_s4", &is_support_s4))
		hwlog_info("is_support_s4 = %u.\n", is_support_s4);

	if (is_support_s4) {
		register_pm_notifier(&hall_pm_notify);
		INIT_DELAYED_WORK(&data->s4_delayed_work, hall_exit_s4_report_delayed_work);
	}
}
#endif
...
#define SUSPEND 0
#define RESUME 1
static void hall_gpio_config(struct platform_device *pdev, int pm_state)
{
	int irq = 0;
	unsigned int gpio = 0;
	struct hall_device *temp_dev = NULL;
	struct ak8789_data *data = data_ak8789;
	if (data == NULL) {
		hwlog_err("%s: data_ak8789 is null\n", __func__);
		return;
	}
	list_for_each_entry(temp_dev, &data->head, list) {
		if (support_type_north(temp_dev->hall_type)) {
			gpio = temp_dev->h_info.gpio[NORTH];
			irq = gpio_to_irq(gpio);
			if (pm_state == SUSPEND) {
				disable_irq(irq);
			} else if (pm_state == RESUME) {
				irq_set_irq_type(irq, IRQF_TRIGGER_FALLING | IRQF_TRIGGER_RISING);
				enable_irq(irq);
			}
			hwlog_info("%s: pm_state:%d, gpio:%d, irq:%d\n", __func__, pm_state, gpio, irq);
		}
		if (support_type_south(temp_dev->hall_type)) {
			gpio = temp_dev->h_info.gpio[SOUTH];
			irq = gpio_to_irq(gpio);
			if (pm_state == SUSPEND) {
				disable_irq(irq);
			} else if (pm_state == RESUME) {
...
#ifdef CONFIG_PM
static int hall_gpio_suspend(struct platform_device *pdev, pm_message_t state)
{
#ifdef CONFIG_SYS_PM_ULSR
	if (is_ulsr())
		hall_gpio_config(pdev, SUSPEND);
#endif
	return 0;
}

static int hall_gpio_resume(struct platform_device *pdev)
{
#ifdef CONFIG_SYS_PM_ULSR
	if (is_ulsr()) {
		hall_gpio_config(pdev, RESUME);
		hall_first_report(true);
	}
#endif
	return 0;
}
...
#ifdef CONFIG_PM
	.suspend = hall_gpio_suspend,
	.resume = hall_gpio_resume,
#endif
};
...
static int __init ak8789_init(void)
{
	...
	hwlog_info("init\n");
	ret = platform_driver_register(&ak8789_driver);
	if (ret) {
		hwlog_err("%s: init fail, %d\n", ret);
		return ret;
	}
#ifdef CONFIG_HIBERNATION
	hall_s4_init();
