...
static bool g_enter_recovery = false;
static bool g_update_in_air_state = false;
struct regulator *g_update_in_air_state_ctrl = NULL;

#define DTS_UPDATE_IN_AIR_CTRL "116x_update_in_air_ctrl"
#define arry_size(a) (sizeof(a) / sizeof((a)[0]))

#ifdef CONFIG_HUAWEI_OHOS_DSM
static bool hi116x_check_report_status(u8 entry_type)
{
	switch (entry_type) {
		case HI1162_ROMLOADER_TYPE:
			for (int i = HI1162_SSB_FW_INFO; i <= HI1162_APP_FW_INFO; i++) {
				if (g_hi1162_info->need_update[i])
					return true;
			}
			break;
		case HI1162_CODELOADER_TYPE:
			return g_hi1162_info->need_update[HI1162_OTA_FW_INFO] ? true : false;
		default:
			break;
	}
	return false;
...
static void hi116x_report_update_event(char *version_msg, char *fwupdate_result_msg)
{
	int ret = 0;
	struct hiview_hisysevent *event = NULL;
	event = hisysevent_create("ACCESSORY", "HI1162_UPDATE", STATISTIC);
	if (event == NULL) {
        sparklink_errmsg("create hisysevent failed in %s", __func__);
        return;
    }
	ret |= hisysevent_put_string(event, "VERSION", version_msg);
	ret |= hisysevent_put_string(event, "RESULT", fwupdate_result_msg);
	if (ret < 0) {
		sparklink_errmsg("hisysevent_put_result_string failed, ret = %d\n", ret);
	...
	if (ret < 0) {
		sparklink_errmsg("hisysevent_write failed, ret = %d\n", ret);
	}
	hisysevent_destroy(&event);
	sparklink_infomsg("hi116x_report_update_event success.\n");
	return;
}

void hi116x_fwupdate_msg_report(struct hi1162_device_info *di,  u8 entry_type, int fwupdate_result)
{
	int  ret = 0;
	char version_msg[UPDATE_EVT_MAX] = { 0 };
	char fwupdate_result_msg[UPDATE_EVT_MAX] = { 0 };
	
	sparklink_infomsg("enter hi116x_fwupdate_msg_report!\n");

	if (!di || fwupdate_result > 0) {
		sparklink_errmsg("hi116x_ota_fwupdate_msg_report failed.\n");
		return;
	}

	if (!hi116x_check_report_status(entry_type)) {
		sparklink_errmsg("no need to update firmware, don't report.\n");
		return;
	}

	if (fwupdate_result == 0) {
		ret |= snprintf_s(version_msg, UPDATE_EVT_MAX, UPDATE_EVT_MAX - 1,
			"current_app_version[0x%x] ", di->version_app_reg.value);
		ret |= snprintf_s(fwupdate_result_msg, UPDATE_EVT_MAX, UPDATE_EVT_MAX - 1,
			"hi116x fwupdate status: success, recovery_mode: %d, ", g_enter_recovery);
	} else {
		ret |= snprintf_s(version_msg, UPDATE_EVT_MAX, UPDATE_EVT_MAX - 1,
			"current_app_version[0x%x] ", di->current_app_version);
		ret |= snprintf_s(fwupdate_result_msg, UPDATE_EVT_MAX, UPDATE_EVT_MAX - 1,
			"hi116x fwupdate status: fail, recovery_mode: %d, ", g_enter_recovery);
	}

    switch (entry_type) {
        case HI1162_ROMLOADER_TYPE:
            ret |= strncat_s(fwupdate_result_msg, UPDATE_EVT_MAX,
                "type: ROMLOADER ", UPDATE_EVT_APPEND_MAX);
            break;
        case HI1162_CODELOADER_TYPE:
            ret |= strncat_s(fwupdate_result_msg, UPDATE_EVT_MAX,
                "type: CODELOADER ", UPDATE_EVT_APPEND_MAX);
            break;
        default:
            break;
	}
	if (ret < 0) {
		sparklink_errmsg("sprintf_s or strncat_s failed when report hi116x fwupdate message.\n");
	} else {
		hi116x_report_update_event(version_msg, fwupdate_result_msg);
	}
	return;
}
#endif
...
{
	if (p) {
		g_enter_recovery = (strncmp(p, "1", 1) == 0) ? true : false; // strlen"1" is 1
		sparklink_errmsg("enter_mode = %s\n", g_enter_recovery ? "recovery" : "normal");
	}
...
bool hi116x_get_recovery_status(void)
{
	return g_enter_recovery;
}


static void wakeup_116x_before_i2c_ops(struct hi1162_device_info *di)
{
	if (!di || !gpio_is_valid(di->wakeup_gpio))
		return;
	...
	if (!di->support_gpio_reverse) {
		...
		gpio_set_value(di->wakeup_gpio, 0);
		msleep(WAKEUP_GPIO_REVERSE_DELAY_TIME);
		gpio_set_value(di->wakeup_gpio, 1);
		msleep(WAKEUP_DELAY_TIME);
		sparklink_infomsg("trigger gpio_reverse wakeup_gpio\n");
	}
}
...
int hi1162_check_id(struct hi1162_device_info *di)
{
	...
	sparklink_infomsg("hi116x read chipid ok\n");
	return 0;
}

static int hi116x_get_app_version_from_fw(struct hi1162_device_info *di)
{
	int ret = 0;
	const struct firmware *ota_fw = NULL;
	const u8 version_tag[HI1162_SPARKLINK_STR_LEN] = {'S', 'p', 'a', 'r', 'k', 'l', 'i', 'n', 'k'};

	if (!di)
		return -EINVAL;

	ret = request_firmware(&ota_fw, HI1162_OTA_PATH, di->dev);
	if (ret < 0) {
		sparklink_errmsg("request OTA firmware fail %d.\n", ret);
		return -EINVAL;
	}

	ret = memcmp(ota_fw->data + HI1162_OTA_FW_BASE, version_tag, HI1162_SPARKLINK_STR_LEN);
	if (ret != 0) {
		sparklink_errmsg("check OTA tag failed.\n");
		ret = -EINVAL;
		goto HI1162_OTA_FW_FREE;
	}
	
	ret = (int)ota_fw->data[HI1162_OTA_FW_OFFSET];
	if (ret <= 0x0 || ret >= 0xFF) {
		sparklink_errmsg("app version is invalid when ota, version:0x%x.\n", ret);
		ret = -EINVAL;
		goto HI1162_OTA_FW_FREE;
	}
	di->version_app_reg.value = (u32)ret;
	sparklink_infomsg("read app version from fw, version:0x%x\n", di->version_app_reg.value);

HI1162_OTA_FW_FREE:
	release_firmware(ota_fw);
	return ret;
}

static void hi1162_set_firmware_update_flag(struct hi1162_device_info *di)
{
	int ret;
	...
	g_hi1162_info->current_app_version = ret < 0 ? 0 : ret; // read version failed, report 0
...
static int hi1162_clear_irq(struct hi1162_device_info *di, u16 irq_val)
{
	...
	irq[0] = (u8)((irq_val >> BYTE_LEN) & 0xff);
...
static void hi116x_irq_handler(struct hi1162_device_info *di)
{
	...
	if (di->irq_val & HI1162_IRQ_RINGING_REPORT) {
		di->irq_val &= ~HI1162_IRQ_RINGING_REPORT;
		sparklink_infomsg("ringing irq: report ringing cmd\n");
		hi116x_report_ringing_cmd(di);
	}
}
...
static irqreturn_t hi1162_interrupt(int irq, void *dev)
{
	...
	if (di->irq_active && (di->support_wltx || di->support_ringing)) {
...
#ifdef CONFIG_FACTORY_MODE
static int hi1162_i2c_retry_check(struct hi1162_device_info *di)
 {
	int ret, i;
	...
	msleep(I2C_DELAY_TIME_5S);

	for (i = 0; i < I2C_CHECK_RETRY; i++) {
		ret = hi1162_check_id(di);
			if (ret == 0){
				sparklink_errmsg("i2c is normal after fwupdata\n");
				return 0;
			}
		msleep(I2C_DELAY_TIME_1S);
	}
	return ret;
}

static int hi1162_set_rf_power(struct hi1162_device_info *di)
{
	int ret;
	u8 rf_power[RF_POWER_LEN] = {0};

	if (!di)
		return -EINVAL;

	rf_power[0] = di->gfsk_power;
	rf_power[1] = di->qpsk_power;

	ret = sparklink_i2c_write_block(di->client, RF_POWER_ADDR, RF_POWER_LEN, rf_power);
	if (ret < 0) {
		sparklink_errmsg("hi116x set rf power : write faild\n");
		return ret;
	}
	sparklink_infomsg("hi116x set rf power sucess\n");
	return 0;
}
#endif
...
static int hi1162_update_time(struct hi1162_device_info *di)
{
	...
	for (i = 0; i < sizeof(time_buf); i++) {
		time_buf[i] = (tv.tv_sec >> (BYTE_LEN * i)) & 0xff;
...
static void hi116x_fwupdate_common(struct hi1162_device_info *di)
{
	int ret;
	int update_in_air_mode = UPDATE_IN_AIR_CLOSE;
	struct sparklink_uart_ops *ops = NULL;
	if (!di || !g_hi1162_info || !g_hi1162_info->client) {
		sparklink_errmsg("hi116x_fwupdate_common failed.\n");
		return;
	}

	if (di->support_update_in_air) {
		update_in_air_mode = sparklink_i2c_read_byte(g_hi1162_info->client, UPDATE_IN_AIR_STATE_REG_ADDR);
		if (update_in_air_mode == UPDATE_IN_AIR_OPEN) {
			sparklink_errmsg("it cannot update firmware when OTA by update in air \n");
			goto HI1162_SKIP_FW_UPDATE;
		}
		sparklink_infomsg("update_in_air_mode:%d \n", update_in_air_mode);
	}

	ops = sparklink_get_uart_ops();
	if (ops == NULL) {
...
#ifdef CONFIG_FACTORY_MODE
	ret = hi1162_i2c_retry_check(di);
	if (ret < 0) {
		sparklink_errmsg("More than 20s, i2 is abnormal\n");
	}
	msleep(10);
	hi1162_set_rf_power(di);
	sparklink_infomsg("hi1162_i2c_retry_check and hi1162_set_rf_power only in factory\n");
#endif

HI1162_SKIP_FW_UPDATE:
	g_hi1162_state = SPARKLINK_STATE_POWERON;
	msleep(1000); /* delay 1s,then do i2c ops */
	hi1162_update_time(di);
}

static void hi116x_update_in_air_get_time(void)
{
    int32_t ret;

    if (g_update_in_air_state_ctrl == NULL) {
        sparklink_errmsg("g_update_in_air_state_ctrl is NULL\n");
        return;
    }
    /* Reuse regulator interface for time get */
    ret = regulator_get_voltage(g_update_in_air_state_ctrl);
	if (ret < 0)
		sparklink_errmsg("get time_minute error ret:%d\n", ret);
	else
		sparklink_errmsg("ret:%d Converted hours:ret %d\n", ret, ret/MINUTES_PER_HOUR);
}

static int32_t hi116x_update_in_air_set_time(uint32_t time_hour)
{
    int32_t ret;
    uint32_t time_minute = time_hour * MINUTES_PER_HOUR ;

    if (g_update_in_air_state_ctrl == NULL) {
        sparklink_errmsg("g_update_in_air_state_ctrl is NULL\n");
        return -EINVAL;
    }
    /* To modify pmu reg value with reuse regulator interface for time set */
    ret = regulator_set_voltage(g_update_in_air_state_ctrl, time_minute, time_minute);
    sparklink_infomsg("time_minute %d ret %d\n", time_minute, ret);
    hi116x_update_in_air_get_time();
    return ret;
}

static int hi116x_notify_power_onoff_state(u8 value)
{
	int ret = 0;

	if (!g_hi1162_info || !g_hi1162_info->client)
		return -EINVAL;
	/* reg 0x1F can only write */
	ret = sparklink_i2c_write_byte(g_hi1162_info->client, SPARKLINK_POWER_STATE_ADDR, value);
	if (ret < 0) {
		sparklink_errmsg("write state:0x%x fail ret:%d \n", value, ret);
		return -EINVAL;
	}
	sparklink_infomsg("write state:0x%x success ret:%d \n", value, ret);
	return 0;
}

static void hi116x_set_firmware_update_common(struct hi1162_device_info *di)
{
	int ret, i;
	u8 value_nv[NV_LEN] = {0};
	if (!di) {
		sparklink_errmsg("hi116x_set_firmware_update_common failed.\n");
		return;
	}

	sparklink_infomsg("hi116x_set_firmware_update_common enter\n");

#ifndef CONFIG_FACTORY_MODE
	if (g_hi1162_info->support_update_in_air)
		hi116x_notify_power_onoff_state(SPARKLINK_POWER_ON);
#endif
	...
	if (ret < 0) {
		...
		if ((ret == 0) && (((value_nv[1] << BYTE_LEN) | value_nv[0]) == NV_MAGIC)) {
		...
		hi1162_set_firmware_update_flag(di);
	}
...
static int hi1162_request_wakeup_gpio(struct hi1162_device_info *di)
{
	...
	if (ret < 0) {
		sparklink_errmsg("request wakeup_gpio fail\n");
		di->wakeup_gpio = -EINVAL;
		return ret;
	}
	...
	if (!di->support_gpio_reverse)
		...
		ret = gpio_direction_output(di->wakeup_gpio, 1);
	if (ret < 0) {
...
static int hi1162_request_buck_boost_en_gpio(struct hi1162_device_info *di)
{
	...
	if (ret < 0) {
		sparklink_errmsg("set buck_boost_en_gpio high fail\n");
		gpio_free(di->buck_boost_en_gpio);
		di->buck_boost_en_gpio = -EINVAL;
		return ret;
	} else if (gpio_get_value(di->buck_boost_en_gpio) != 1) {
		sparklink_errmsg("readback buck_boost_en_gpio fail\n");
	}
...
static int hi1162_request_i2c_switch_gpio(struct hi1162_device_info *di)
{
	...
	if (ret < 0) {
		sparklink_errmsg("set i2c_switch_gpio output fail\n");
...
static int hi116x_update_in_air_ldo_init(struct hi1162_device_info *di)
{
	if (!di || !di->dev)
		return -EINVAL;

	g_update_in_air_state_ctrl = regulator_get(di->dev, DTS_UPDATE_IN_AIR_CTRL);
	if (IS_ERR_OR_NULL(g_update_in_air_state_ctrl)) {
		g_update_in_air_state_ctrl = NULL;
		sparklink_errmsg("regulator_get error\n");
		return -EINVAL;
	}

	hi116x_update_in_air_set_time(UPDATE_IN_AIR_TIME_DEFAULT);

	return 0;
}

static int hi1162_set_pmu_116x_pwr_gpio(struct hi1162_device_info *di)
{
	int ret;

	if (!di || (di->pmu_116x_pwr_gpio < 0) || (di->pmu_shutdown_alive_gpio < 0))
		return -EINVAL;

	ret = gpio_request(di->pmu_116x_pwr_gpio, "hi1162_pmu_116x_pwr_gpio");
	if (ret < 0) {
		sparklink_errmsg("request fail\n");
		di->pmu_116x_pwr_gpio = -EINVAL; /* invalid gpio */
		return ret;
	}

	/* the pmu_116x_pwr_gpio set high to enable sparklink */
	ret = gpio_direction_output(di->pmu_116x_pwr_gpio, 1);
	if (ret < 0) {
		sparklink_errmsg("set pmu_116x_pwr_gpio high fail\n");
		gpio_free(di->pmu_116x_pwr_gpio);
		di->buck_boost_en_gpio = -EINVAL;
		return ret;
	} else if (gpio_get_value(di->pmu_116x_pwr_gpio) != 1) {
		sparklink_errmsg("readback pmu_116x_pwr_gpio fail\n");
	}

	ret = gpio_request(di->pmu_shutdown_alive_gpio, "hi1162_pmu_shutdown_alive_gpio");
	if (ret < 0) {
		sparklink_errmsg("request fail\n");
		di->pmu_shutdown_alive_gpio =  -EINVAL; /* invalid gpio */
		return ret;
	}

	/* close update in air default */
	ret = gpio_direction_output(di->pmu_shutdown_alive_gpio, 0);
	if (ret < 0) {
		sparklink_errmsg("set output fail\n");
		gpio_free(di->pmu_shutdown_alive_gpio);
		di->pmu_shutdown_alive_gpio =  -EINVAL; /* invalid gpio */
		return ret;
	}
...
static int hi1162_request_irq_gpio(struct hi1162_device_info *di)
{
	...
	if (!di || !gpio_is_valid(di->irq_gpio)) {
		sparklink_infomsg("di is null or irq_gpio is invalid\n");
		return -EINVAL;
	}
...
static int hi1162_power_enable(struct hi1162_device_info *di)
{
	int ret;
	...
	if (di->power_always_on) {
		sparklink_errmsg("power_always_on\n");
		return 0;
	}
	...
	if (di->support_update_in_air) {
		ret = hi1162_set_pmu_116x_pwr_gpio(di);
		if (ret < 0)
			sparklink_errmsg("request pmu_116x_pwr_gpio error\n");
		/* return error when pmu_116x_pwr_gpio and buck_boost_en_gpio both is invalid */
		if ((di->pmu_116x_pwr_gpio < 0) && (di->buck_boost_en_gpio < 0)) {
			sparklink_errmsg("request buck_boost and pmu_116x_pwr error\n");
			goto HI1162_PMU_POWER_ENALBE_FAIL;
		}

		ret = hi116x_update_in_air_ldo_init(di);
		if (ret < 0)
			sparklink_errmsg("hi116x_update_in_air_ldo_init failed\n");
	} else {
		if (di->buck_boost_en_gpio < 0)
			goto HI1162_BUCK_BOOST_ENALBE_FAIL;
	}
	return 0;

HI1162_PMU_POWER_ENALBE_FAIL:
	if (gpio_is_valid(di->pmu_116x_pwr_gpio))
		gpio_free(di->pmu_116x_pwr_gpio);
	if (gpio_is_valid(di->pmu_shutdown_alive_gpio))
		gpio_free(di->pmu_shutdown_alive_gpio);
HI1162_BUCK_BOOST_ENALBE_FAIL:
	if (gpio_is_valid(di->buck_boost_en_gpio))
		gpio_free(di->buck_boost_en_gpio);
	return -1;
}
...
static int hi1162_gpio_init(struct hi1162_device_info *di)
{
	...
	ret = hi1162_power_enable(di);
	if (ret < 0)
		goto HI1162_POWER_ENALBE_FAIL;

	ret = hi1162_request_i2c_switch_gpio(di);
	if (ret < 0)
		sparklink_errmsg("request i2c_switch_gpio error\n");
	return 0;

HI1162_POWER_ENALBE_FAIL:
	if (di->irq >= 0)
		free_irq(di->irq, di);
	if (gpio_is_valid(di->irq_gpio))
		gpio_free(di->irq_gpio);
...
static void hi1162_gpio_deinit(struct hi1162_device_info *di)
{
	...
	if (di->support_update_in_air && (di->pmu_116x_pwr_gpio > 0))
		gpio_free(di->pmu_116x_pwr_gpio);

	if (di->support_update_in_air && (di->pmu_shutdown_alive_gpio > 0))
		gpio_free(di->pmu_shutdown_alive_gpio);
}
...
static int hi1162_parse_common_gpio(struct hi1162_device_info *di)
{
	int i;
	struct device_node *np = NULL;
	const char *gpio_name[] = {"wakeup_gpio", "reset_gpio"};
	unsigned int gpio_value[arry_size(gpio_name)] = {-1, -1}; /* invlaid gpio */

	if (!di || !di->client || !di->client->dev.of_node)
		return -EINVAL;
...
static int hi116x_parse_optional_gpio(struct hi1162_device_info *di)
{
	int i;
	struct device_node *np = NULL;

	const char *gpio_name[] = {"irq_gpio", "buck_boost_en_gpio", "i2c_switch_gpio"};
	int gpio_value[arry_size(gpio_name)] = {-1, -1, -1}; /* invalid gpio */
...
static int hi116x_parse_power_config(struct hi1162_device_info *di)
{
	int ret;
	struct device_node *np = NULL;
	...
	ret = of_property_read_u32(np, "power_always_on", &di->power_always_on);
	if (ret < 0) {
		sparklink_errmsg("read power_always_on error\n");
		di->power_always_on = 0;
	}

	ret = of_property_read_u32(np, "support_update_in_air", &di->support_update_in_air);
	if (ret < 0) {
		sparklink_errmsg("read support_update_in_air error\n");
		di->support_update_in_air = 0;
	}

	ret = of_property_read_u32(np, "pmu_116x_pwr_gpio", &di->pmu_116x_pwr_gpio);
	if (ret < 0) {
		sparklink_errmsg("read pmu_116x_pwr_gpio error\n");
		di->pmu_116x_pwr_gpio = -EINVAL;
	}

	ret = of_property_read_u32(np, "pmu_shutdown_alive_gpio", &di->pmu_shutdown_alive_gpio);
	if (ret < 0) {
		sparklink_errmsg("read pmu_shutdown_alive_gpio error\n");
		di->pmu_shutdown_alive_gpio = -EINVAL;
	}

	return 0;
}

static int hi1162_parse_reg_dts(struct hi1162_device_info *di)
{
	int ret, i;
	struct device_node *np = NULL;
	const char *reg_name[] = {"chip_id_reg", "version_ssb_reg",
		...
		"bt_fw_info", "app_fw_info", "ota_fw_info"}; // fw_info [index ramsize flashsize]
...
static int hi1162_parse_fw_path(struct hi1162_device_info *di)
{
	int ret, i;
	struct device_node *np = NULL;
...
static int hi1162_parse_uart_info(struct hi1162_device_info *di)
{
	int ret;
	struct device_node *np = NULL;
...
static int hi1162_parse_wltx_info(struct hi1162_device_info *di)
{
	int ret;
	struct device_node *np = NULL;
...
static int hi116x_parse_gpio_reverse_info(struct hi1162_device_info *di)
{
	int ret;
	struct device_node *np = NULL;
	...
	np = di->client->dev.of_node;
	ret = of_property_read_u32(np, "support_gpio_reverse", &di->support_gpio_reverse);
	if (ret < 0) {
		sparklink_errmsg("support_gpio_reverse is not set, config 0\n");
		di->support_gpio_reverse = 0;
	}
	return 0;
}

static int hi116x_parse_ringing_cmd(struct hi1162_device_info *di)
{
	int ret;
	struct device_node *np = NULL;
	...
	np = di->client->dev.of_node;
	ret = of_property_read_u32(np, "support_ringing", &di->support_ringing);
	if (ret < 0) {
		sparklink_errmsg("support_ringing is not set, config 0\n");
		di->support_ringing = 0;
	}
	return 0;
}
...
u32 hi116x_get_uart_desc(void)
{
	if (!g_hi1162_info)
		return 0;
	return g_hi1162_info->uart_desc;
}

static int hi116x_parse_uart_desc_info(struct hi1162_device_info *di)
{
	int ret;
	struct device_node *np = NULL;
	...
	np = di->client->dev.of_node;
	ret = of_property_read_u32(np, "uart_desc", &di->uart_desc);
	if (ret < 0) {
		sparklink_errmsg("uart_desc is not reset, config default SPARKLINK_UART\n");
		di->uart_desc = SPARKLINK_UART;
	}
	return 0;
}

static int hi116x_parse_attach_event_info(struct hi1162_device_info *di)
{
	int ret;
	struct device_node *np = NULL;
...
static int hi1162_parse_switch_channel_info(struct hi1162_device_info *di)
{
	int ret;
	struct device_node *np = NULL;
...
static int hi1162_parse_rf_power_info(struct hi1162_device_info *di)
 {
	int ret;
	struct device_node *np = NULL;
	...
	np = di->client->dev.of_node;
	ret = of_property_read_u8(np, "gfsk_power", &di->gfsk_power);
	if (ret != 0) {
		sparklink_errmsg("gfsk_power is not find, config 0\n");
		di->gfsk_power = 0;
	}

	ret = of_property_read_u8(np, "qpsk_power", &di->qpsk_power);
	if (ret != 0) {
		sparklink_errmsg("qpsk_power is not find, config 0\n");
		di->qpsk_power = 0;
	}
	sparklink_infomsg("rf power is 0x%x and 0x%x\n", di->gfsk_power, di->qpsk_power);
 
	return 0;
}

static int hi1162_parse_dts(struct hi1162_device_info *di)
{
	int ret;
	...
	ret = hi1162_parse_common_gpio(di);
	if (ret < 0)
		return ret;

	ret = hi1162_parse_reg_dts(di);
	if (ret < 0)
		return ret;

	ret = hi1162_parse_fw_path(di);
	if (ret < 0)
		return ret;

	ret = hi1162_parse_uart_info(di);
	if (ret < 0)
		return ret;
	/* necessary config, must be here, end */

	/* customize config, must be here, start */
	ret = hi1162_parse_wltx_info(di);
	if (ret < 0)
		return ret;

	ret = hi116x_parse_attach_event_info(di);
	if (ret < 0)
		return ret;

	ret = hi1162_parse_switch_channel_info(di);
	if (ret < 0)
		return ret;

	ret = hi116x_parse_optional_gpio(di);
	if (ret < 0)
		return ret;

	ret = hi116x_parse_power_config(di);
	if (ret < 0)
		return ret;

	ret = hi116x_parse_gpio_reverse_info(di);
	if (ret < 0)
		return ret;

	ret = hi116x_parse_ringing_cmd(di);
	if (ret < 0)
		return ret;

	ret = hi116x_parse_uart_desc_info(di);
	if (ret < 0)
		return ret;

	ret = hi1162_parse_rf_power_info(di);
	if (ret < 0)
		return ret;
...
void hi116x_i2c_operate_start(void)
{
	...
	if (gpio_is_valid(g_hi1162_info->i2c_switch_gpio)) {
		...
		sparklink_infomsg("i2c_switch_gpio value: %d\n", ret);
...
static int hi1162_uart_switch_channel(int uart_channel)
{
	int ret;
...
static int hi116x_ringing_ack_reply(u32 ringing_ack_value)
{
	int ret;
	u8 buf[HI116X_RINGING_REG_LEN] = { 0 };

	if (ringing_ack_value == 0 || ringing_ack_value > HI116X_RINGING_ACK_VAL_MAX) {
		sparklink_errmsg("ringing_ack_value is invalid\n");
		return -EINVAL;
	}

	buf[HI116X_RINGING_ACK_TYPE_LOC] = HI116X_RINGING_ACK_FLAG;
	buf[HI116X_RINGING_ACK_LEN_LOC] = HI116X_RINGING_DATA_LEN;
	sparklink_infomsg("receive ringing_ack_value:%x\n", ringing_ack_value);
	switch (ringing_ack_value) {
	case RINGING_OPEN:
		buf[HI116X_RINGING_ACK_VAL_LOC] = HI116X_RINGING_OPEN_FLAG;
		break;
	case RINGING_CLOSE:
		buf[HI116X_RINGING_ACK_VAL_LOC] = HI116X_RINGING_CLOSE_FLAG;
		break;
	case RINGING_END:
		buf[HI116X_RINGING_ACK_VAL_LOC] = HI116X_RINGING_END_FLAG;
		break;
	case RINGING_ULSR_RESUME:
		buf[HI116X_RINGING_ACK_VAL_LOC] = HI116X_RINGING_ULSR_RESUME_FLAG;
		break;
	default:
		sparklink_errmsg("ringing_ack_value is invalid\n");
		return -EINVAL;
	...
	ret = sparklink_i2c_write_block(g_hi1162_info->client,
		HI116X_RINGING_ACK_REG, HI116X_RINGING_REG_LEN, buf);
	if (ret < 0) {
		sparklink_errmsg("write ringing_ack reply failed\n");
		return ret;
	}
...
static int hi116x_ota_fwupdate(void)
{
	struct hi1162_device_info *di = g_hi1162_info;
	if (!di) {
		return -EINVAL;
	...
	if (!g_enter_recovery) {
		sparklink_errmsg("hi116x_ota_fwupdate trigger failed, not recovery mode.\n");
		return -EINVAL;
	}

	if (g_enter_recovery) {
		if (hi116x_get_app_version_from_fw(di) < 0)
			di->version_app_reg.value  = HI1162_FAULT_APP_VERSION;
	}
	
	sparklink_infomsg("hi116x_ota_fwupdate enter.\n");
	msleep(HI1162_RESET_WAIT_TIME); // wait for 1162 reset done in probe to avoid read chipid failure
	hi116x_set_firmware_update_common(di);
	hi116x_fwupdate_common(di);
 
	return 0;
}

static int hi116x_update_in_air_set_mode(int state)
{
	if (!g_hi1162_info || state >= UPDATE_IN_AIR_PARAM_MAX || g_hi1162_info->pmu_shutdown_alive_gpio < 0)
		return -EINVAL;

	if (!g_hi1162_info->support_update_in_air) {
		sparklink_errmsg("not support update in air\n");
		return -EINVAL;
	}
	sparklink_infomsg("state is %d\n", state);

	if (state == 0) {
		gpio_set_value(g_hi1162_info->pmu_shutdown_alive_gpio, 0);
		g_update_in_air_state = false;
		hi116x_update_in_air_set_time(UPDATE_IN_AIR_TIME_DEFAULT);
	} else {
		gpio_set_value(g_hi1162_info->pmu_shutdown_alive_gpio, 1);
		g_update_in_air_state = true;
		hi116x_update_in_air_set_time(state);
	}

	return 0;
}

static int hi116x_get_update_in_air_state(u32 *val)
{
	u32 gpio_state, time;

	if (!g_hi1162_info || !g_update_in_air_state_ctrl || (g_hi1162_info->pmu_shutdown_alive_gpio < 0))
		return -EINVAL;
	...
	gpio_state = gpio_get_value(g_hi1162_info->pmu_shutdown_alive_gpio);
	time = regulator_get_voltage(g_update_in_air_state_ctrl);
	sparklink_infomsg("gpio_state:%d time:%d\n", gpio_state, time);
	if ((gpio_state < 0) || (time < 0)) {
		sparklink_errmsg("get gpio_state or time error\n");
		return -1;
	}

	*val = gpio_state*SHIFTING_RATIO + time/MINUTES_PER_HOUR;

	return 0;
}

static int hi116x_record_power_state(int power_onoff_state)
{
	if (!g_hi1162_info || !g_hi1162_info->support_update_in_air) {
		sparklink_errmsg("null or not support_update_in_air");
		return -EINVAL;
	}

	switch (power_onoff_state) {
	case SPARKLINK_NOTIFY_POWER_OFF:
		hi116x_notify_power_onoff_state(SPARKLINK_POWER_OFF);
		break;
	case SPARKLINK_NOTIFY_POWER_ON:
		hi116x_notify_power_onoff_state(SPARKLINK_POWER_ON);
		break;
	default:
		sparklink_errmsg("power onoff state invalid\n");
		return -EINVAL;
	}
	return 0;
}
...
struct sparklink_driver_ops hi1162_ops = {
	...
	.device_ringing_ack = hi116x_ringing_ack_reply,
	.ota_fwupdate = hi116x_ota_fwupdate,
	.device_update_in_air_set_mode = hi116x_update_in_air_set_mode,
	.device_get_update_in_air_state = hi116x_get_update_in_air_state,
	.device_record_power_state = hi116x_record_power_state,
};
				...
				unsigned long code, void *unused)
{
...
#ifndef CONFIG_FACTORY_MODE
	if (g_hi1162_info->support_update_in_air && g_update_in_air_state == true)
		hi116x_notify_power_onoff_state(SPARKLINK_POWER_OFF);
#endif
	...
	/* make sure 1162 power off safety */
	gpio_set_value(g_hi1162_info->reset_gpio, 1);
	msleep(SHUTDOWN_DELAY_TIME); /* delay 5ms wait reset done */
	gpio_set_value(g_hi1162_info->buck_boost_en_gpio, 0);

	if (g_hi1162_info->support_update_in_air && g_update_in_air_state == false) {
		if(g_hi1162_info->pmu_116x_pwr_gpio > 0)
			gpio_set_value(g_hi1162_info->pmu_116x_pwr_gpio, 0);
		if (g_hi1162_info->pmu_shutdown_alive_gpio > 0)
			gpio_set_value(g_hi1162_info->pmu_shutdown_alive_gpio, 0);
	}
...
static int hi1162_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	if (ret < 0) {
		sparklink_errmsg("register_notifier error\n");
		goto FREE_GPIO;
	}

	sparklink_driver_ops_register(&hi1162_ops);

	if (g_enter_recovery == true) {
...
#ifdef CONFIG_PM
static int hi116x_pm_suspend(struct device *pdev)
{
	sparklink_infomsg("enter sparklink pm_suspend\n");
	if (!g_hi1162_info) {
		return -ENODEV;
	}
	if (!g_hi1162_info->irq_active) {
		return 0;
	}

	if (is_ulsr() && (g_hi1162_info->support_wltx || g_hi1162_info->support_ringing)) {
		disable_irq(g_hi1162_info->irq);
		g_hi1162_info->irq_active = false;
	}

	return 0;
}

static int hi116x_pm_resume(struct device *pdev)
{
	int ret = 0;
	sparklink_infomsg("enter sparklink pm_resume\n");
	if (!g_hi1162_info)
		return -ENODEV;

	if (is_ulsr() && (g_hi1162_info->support_wltx || g_hi1162_info->support_ringing)) {
		ret = irq_set_irq_type(g_hi1162_info->irq, IRQF_TRIGGER_FALLING | IRQF_TRIGGER_RISING);
		if (ret < 0) {
			sparklink_infomsg("set irq type failed.\n");
			return -EFAULT;
		}
		g_hi1162_info->irq_active = true;
		enable_irq(g_hi1162_info->irq);
	}

	return 0;
}

static const struct dev_pm_ops hi116x_pm_ops = {
	.suspend = hi116x_pm_suspend,
	.resume = hi116x_pm_resume,
};
#endif
