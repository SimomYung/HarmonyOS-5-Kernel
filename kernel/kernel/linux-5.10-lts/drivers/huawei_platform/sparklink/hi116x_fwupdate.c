...
#define WAIT_IMG_COPY_TIME 10000
	...
	struct hi1162_device_info *di, int index)
{
	...
	if (index == HI1162_OTA_CORE_TYPE)
		/* wating for copying OTA.bin from the backup area to main area & restart the 1162 A core */
		msleep(WAIT_IMG_COPY_TIME);
...
static int codeloader_type_fwupdate(struct sparklink_uart_ops *ops, struct hi1162_device_info *di)
{
	...
	if (di->need_update[HI1162_OTA_FW_INFO] == true) {
		...
		sparklink_errmsg("codeloader and no need to update\n");
	}
...
{
	int ret = 0;
	g_enter_recovery = hi116x_get_recovery_status();
...
int hi1162_fwupdate(struct sparklink_uart_ops *ops, struct hi1162_device_info *di)
{
	...
	if (hi1162_device_get_chipid_state() == SPARKLINK_CHIPID_SUCCESS) {
		...
		hi1162_uart_deinit(ops);
		if (ret < 0) {
			sparklink_errmsg("hi1162_device_get_mode failed.\n");
			return ret;
		}
    ...
    hi116x_fwupdate_msg_report(di, entry_type, ret);
#endif
	if (ret < 0)
		return ret;

	di->current_app_version = di->version_app_reg.value;
