...
static void fxfer_aoa_hid_refresh(struct aoa_context *ctx)
{
	...
	if (dev && (dev->desc_offset == dev->desc_len)) {
		found = true;
		spin_unlock_irqrestore(&ctx->lock, flags);
		pr_info("%s: try to start device %u\n", __func__, dev->id);

		if (fxfer_aoa_init_hid_device(dev)) {
			pr_err("can't add hid device %u\n", dev->id);
			dev->state = HID_STATE_STOP;
		} else {
	...
	if (dev) {
		found = true;
		pr_info("%s: found stopped device %u\n", __func__, dev->id);
		list_del(&dev->list);
	}
	spin_unlock_irqrestore(&ctx->lock, flags);

	if (dev && (dev->state == HID_STATE_STOP)) {
		pr_info("%s: delete stopped device %u\n", __func__, dev->id);
	...
	struct usb_ep *ep, struct usb_request *req)
{
	...
	pr_info("%s: hid=%u, length=%u, offset=%u, total=%u\n",
		__func__, dev->id, length, dev->desc_offset, dev->desc_len);
	...
	struct usb_ep *ep, struct usb_request *req)
{
	...
	pr_info("%s: hid=%u, length=%u, ret=%d\n", __func__, dev->id, req->actual, ret);
}
