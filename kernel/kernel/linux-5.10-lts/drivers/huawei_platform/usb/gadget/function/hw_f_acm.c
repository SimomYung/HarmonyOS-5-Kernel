...
static int hw_acm_dbg_level[HW_MAX_U_SERIAL_PORTS] = {0};

static int g_acm_is_single_interface = HW_ACM_IS_SINGLE;

static inline unsigned char acm_get_type(const struct f_acm *acm)
{
...
int hw_acm_get_dbg_level(int port_num)
{
	if (port_num < HW_MAX_U_SERIAL_PORTS)
		return hw_acm_dbg_level[port_num];
	return 0;
}
	...
	const struct usb_ctrlrequest *ctrl)
{
	 ...
	 */
	switch ((ctrl->bRequestType << ACM_ONE_BYTE) | ctrl->bRequest) {
	case (((USB_DIR_OUT | USB_TYPE_CLASS | USB_RECIP_INTERFACE) << ACM_ONE_BYTE) |
	...
	case (((USB_DIR_OUT | USB_TYPE_VENDOR | USB_RECIP_INTERFACE) << ACM_ONE_BYTE) |
	...
	case (((USB_DIR_IN | USB_TYPE_CLASS | USB_RECIP_INTERFACE) << ACM_ONE_BYTE) |
...
static ssize_t f_acm_dbg_set_show(struct config_item *item, char *page)
{
	struct f_serial_opts *opts = to_f_serial_opts(item);
	int dbg = -1;

	if (opts->port_num < HW_MAX_U_SERIAL_PORTS)
		dbg = hw_acm_dbg_level[opts->port_num];

	return scnprintf(page, PAGE_SIZE, "dbg_level=%d\n", dbg);
}

static ssize_t f_acm_dbg_set_store(struct config_item *item,
	const char *page, size_t count)
{
	struct f_serial_opts *opts = to_f_serial_opts(item);
	long val = 0;

	if ((kstrtol(page, 10, &val) < 0) || (val < 0)) {
		pr_err("%s: invalid param\n", __func__);
		return -EINVAL;
	}

	if (opts->port_num < HW_MAX_U_SERIAL_PORTS) {
		pr_info("%s: set dbg level to %d for port %d\n", __func__, val, opts->port_num);
		hw_acm_dbg_level[opts->port_num] = (int)val;
	}

	return count;
}

CONFIGFS_ATTR(f_acm_, dbg_set);
