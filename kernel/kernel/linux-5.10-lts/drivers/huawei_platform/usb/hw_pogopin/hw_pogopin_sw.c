...
static void hw_pogopin_sw_vcc_ctrl(int enable)
{
	...
	if (di->pogopin_chg_support && gpio_is_valid(di->kb_power_switch_gpio))
		gpio_set_value(di->kb_power_switch_gpio, enable);
...
static bool hw_pogopin_sw_need_drop_msg(const unsigned char *cp, int count)
{
	...
	if ((count == HW_POGO_SW_CALL_INFO_LEN) &&
		((cp[ACC_BT_MAC_CP_FRONT] == g_msg_bt_mac_addr[ACC_BT_MAC_CP_FRONT]) &&
		(cp[ACC_BT_MAC_CP_LATER] == g_msg_bt_mac_addr[ACC_BT_MAC_CP_LATER])))
		return true;

	if ((count == ARRAY_SIZE(g_msg_require_type)) &&
		((cp[ACC_SUPPORT_SLE_CP_FRONT] == g_msg_require_type[ACC_SUPPORT_SLE_CP_FRONT]) &&
...
static void hw_pogopin_sw_sysfs_create_group(struct device *dev)
{
	...
	struct device_attribute *attr, const char *buf, size_t count)
{
	...
	info = hw_pogopin_sw_sysfs_field_lookup(attr->attr.name);
	if (!info || !di || !buf) {
		hwlog_err("%s info or di or buf is null\n", __func__);
		return -EINVAL;
	}
...
static void pogopin_detect_acc_info_work(struct work_struct *work)
{
	...
	if (wait_for_completion_timeout(&hw_pogopin_sw_ldisc_data.completion,
        ...
        if (!(avgvalue >= KB_ONLINE_MIN_ADC_LIMIT && avgvalue <= di->kb_max_adc_limit)) {
...
static void hw_pogopin_sw_disconnect_handler(struct hw_pogopin_sw_info *di)
{
...
static void pogopin_detect_id_irq_work(struct work_struct *work)
{
	...
	msleep(di->pogo_detect_debounce_msecs);
	hw_pogopin_sw_gpio_value = gpio_get_value(di->hw_pogopin_sw_int_gpio);
	...
	if ((hw_pogopin_sw_gpio_value == LOW) && di->pogopin_chg_support && 
		gpio_is_valid(di->pogopin_int_gpio)) {
		if (gpio_get_value(di->pogopin_int_gpio) == LOW) {
			hwlog_info("pogopin charger insert, ignore kb insert event\n");
			goto exit;
		}
	}
	...
	if (hw_pogopin_sw_gpio_value == HIGH) {
		hw_pogopin_sw_disconnect_handler(di);
	} else {
		avgvalue = hw_pogopin_sw_adc_sampling(di);
		if (avgvalue >= KB_ONLINE_MIN_ADC_LIMIT && avgvalue <= di->kb_max_adc_limit) {
			hw_pogopin_sw_clear_acc_uevent_data();
	...
	struct device_node *np)
{
	...
	di->pogopin_chg_support = of_property_read_bool(np, "pogopin_chg_support");
	if (di->pogopin_chg_support) {
		pogopin_chg_np = of_find_node_by_name(NULL, "huawei_pogopin_charger");
		if (pogopin_chg_np) {
			di->kb_power_switch_gpio = of_get_named_gpio(pogopin_chg_np, "power_switch_gpio", 0);
			if (!gpio_is_valid(di->kb_power_switch_gpio))
				hwlog_err("get power_switch_gpio valid\n");
			di->pogopin_int_gpio = of_get_named_gpio(pogopin_chg_np, "pogopin_int_gpio", 0);
			if (!gpio_is_valid(di->pogopin_int_gpio))
				hwlog_err("get pogopin_int_gpio valid\n");
		}
	}
...
static int hw_pogopin_sw_parse_dts(struct hw_pogopin_sw_info *di, struct device_node *np)
{
	...
	(void)power_dts_read_u32(power_dts_tag(HWLOG_TAG), np,
		"kb_max_adc_limit", &di->kb_max_adc_limit, KB_ONLINE_MAX_ADC_LIMIT);
	(void)power_dts_read_u32(power_dts_tag(HWLOG_TAG), np,
		"charge_timer_enable", &di->charge_timer_enable, 0);
	(void)power_dts_read_u32(power_dts_tag(HWLOG_TAG), np,
		"pogo_detect_debounce_msecs", &di->pogo_detect_debounce_msecs, HW_POGO_SW_DETECT_DEBOUNCE_MSECS);
...
static int hw_pogopin_notify_and_sys_tty(struct device *dev, struct hw_pogopin_sw_info *di, struct device_node *np)
{
	...
	hw_pogopin_sw_sysfs_create_group(dev);
