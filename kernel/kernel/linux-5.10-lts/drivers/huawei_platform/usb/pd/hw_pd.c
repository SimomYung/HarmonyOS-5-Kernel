...
static int g_support_pd_certification;
static unsigned int g_config_delay;
...
static int g_pd_support_pps = 0;
static int g_forbidden_pd_9v = 0;
static bool pd_certification_set_hiz = false;
#ifdef CONFIG_DKMD_DKSM
static int g_support_fold_screen = 0;
#endif
...
#define REVERSE_SC_DEF_DEV_CLS          (-2)
#endif /* CONFIG_REVERSE_CHARGE */
...
#define STANDBY_CURR_MAX_MA             100
#define USB_ENUM_TIMEOUT                10000

#define TEM_VOL_VALUE                   5000
#define TEM_CUR_VAL                     2000

struct pd_dpm_cable_state {
	int cable_fail;                                   /* cable authentication fail */
	int succ_flag;                                    /* cable authentication succ flag */
	int support_cable_auth;                           /* cable authentication is supported */
	...
	bool bc12_dcp_cmp;
	bool emark_detect_running;
	struct completion bc12_dcp;
...
struct hisysevent_error_no_map usb_event_map[] = {
	...
	/* report on single only. use temporary dmd no */
	{ 920080000, POWER_DSM_LEVEL_STATISTIC, "DSM_USB",  "CONTINUOUS_STOP_CHARGING", "CONTENT", 0 },
};
#endif /* CONFIG_HUAWEI_OHOS_DSM */
...
#ifdef CONFIG_DKMD_DKSM
static int pd_dpm_handle_dkmd_event(struct notifier_block *self,
	unsigned long event, void *data)
{
	struct dkmd_event *dkmd_event = (struct dkmd_event *)data;
	int blank;

	if (!g_pd_di || !dkmd_event)
		return NOTIFY_DONE;

	blank = dkmd_event->value;

	hwlog_info("%s: event=%lu, blank=%d\n", __func__, event, blank);
...
#ifdef CONFIG_DKMD_DKSM
static struct notifier_block pd_dpm_handle_dkmd_notifier = {
	.notifier_call = pd_dpm_handle_dkmd_event,
};
#endif
...
{
#ifdef CONFIG_DKMD_DKSM
	if (g_support_fold_screen != 0)
		dkmd_register_client(&pd_dpm_handle_dkmd_notifier);
	else
...
void pd_dpm_set_source_sink_state(enum power_supply_sinksource_type type)
{
	...
	if (g_pd_support_pps && (type == POWER_SUPPLY_STOP_SINK))
		hw_pd_pps_notify_stop();
	power_supply_set_int_property_value("charger_detection",
		POWER_SUPPLY_PROP_CHG_PLUGIN, type);
}

static int hw_pd_pps_get_apdo_caps(struct pps_apdo_cap *caps, int num, struct pps_pdo_raw *raw)
{
	int ret = -EACCES;

	if (g_pd_support_pps == 0) {
		hwlog_info("%s not support\n", __func__);
		return ret;
	}

	if (g_ops && g_ops->pd_dpm_pps_get_apdo_caps)
		ret = g_ops->pd_dpm_pps_get_apdo_caps(g_client, caps, num, raw);
	hwlog_info("%s ret %d\n", __func__, ret);
	return ret;
}

static int hw_pd_pps_set_state(enum pps_set_state state, int mv, int ma)
{
	int ret = -EACCES;
	...
	if (g_ops && g_ops->pd_dpm_pps_set_state)
		ret = g_ops->pd_dpm_pps_set_state(g_client, state, mv, ma);
	hwlog_info("%s: state=%d, mv=%d, ma=%d ret=%d\n",
		__func__, (int)state, mv, ma, ret);
	return ret;
}

static int hw_pd_pps_get_status(int *mv, int *ma)
{
	int ret = -EACCES;
	...
	if (g_ops && g_ops->pd_dpm_pps_get_status)
		ret = g_ops->pd_dpm_pps_get_status(g_client, mv, ma);
	hwlog_info("%s: mv=%d, ma=%d ret=%d\n", __func__, *mv, *ma, ret);
	return ret;
}

struct hw_pd_pps_ops g_hw_pd_pps_ops = {
	.get_apdo_caps = hw_pd_pps_get_apdo_caps,
	.set_state = hw_pd_pps_set_state,
	.get_status = hw_pd_pps_get_status,
};
...
void pd_dpm_detect_emark_cable_finish(void)
{
	...
	if (!g_ops || !g_ops->data_role_swap || !g_client || !g_ops->pd_dpm_inquire_usb_comm)
		return;
	/* rt1715 is not support dr_swap after emark */
	if (external_pd_flag)
		return;
...
int pd_dpm_notify_cable_uvdm(uint32_t *data, uint8_t cnt)
{
	if (g_ops && g_ops->pd_dpm_notify_cable_uvdm) {
		hwlog_info("%s start send uvdm", __func__);
		return g_ops->pd_dpm_notify_cable_uvdm(g_client, data, cnt);
	}
	hwlog_err("%s uvdm ops is null", __func__);
	return false;
}

void pd_dpm_notify_9v2a_enable(void)
{
	if (g_forbidden_pd_9v)
		return;
	if (!external_pd_flag)
		return;
	if (!g_pd_hv_en)
		return;
	if (g_ops && g_ops->pd_dpm_notify_9v2a_enable) {
		hwlog_info("call pd_charge_voltage_up for thirdparty pd\n");
		g_ops->pd_dpm_notify_9v2a_enable(g_client);
	}
}
	...
	struct device_attribute *attr, char *buf)
{
	...
	if (g_reverse_sc.reverse_sc_device_class == USB_CLASS_HUB)
...
static ssize_t hw_pd_status_show(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	int ret;
	int volt = 0;
	...
	if (ret) {
		hwlog_info("%s: get volt ret %d\n", __func__, ret);
		volt = 0;
	}

	ret = dcm_get_ic_ibus(SC_MODE, CHARGE_MULTI_IC, &cur);
	if (ret) {
		hwlog_info("%s: get current ret %d\n", __func__, ret);
		cur = 0;
	}

	if (!g_pd_di) {
		hwlog_info("%s: get g_pd_di failed\n", __func__);
		return -EINVAL;
	}

	/*
	di->bc12_event   PLEASE_PROVIDE_POWER
	pd_dpm_typec_state   PD_DPM_USB_TYPEC_NONE
	pd_finish_flag   pd_dpm_get_pd_finish_flag
	sink_source_type   POWER_SUPPLY_STOP_SINK
	*/
	return scnprintf(buf, PAGE_SIZE, "bc12=%d;typec=%d;pdflag=%d;snksrc=%d;pwr=%d,%d\n",
		g_pd_di->bc12_event, pd_dpm_typec_state, g_pd_di->pd_finish_flag, sink_source_type, volt, cur);
}

static ssize_t power_check_show(struct device *dev,
	struct device_attribute *attr, char *buf)
{
   int value;
	if (!g_ops) {
		hwlog_info("%s: get g_ops failed\n", __func__);
		return -EINVAL;
	}
   value = (g_ops->pd_dpm_inquire_usb_comm(g_client));
   return scnprintf(buf, PAGE_SIZE, "%d\n", value);
}

static ssize_t hw_pd_status_store(struct device *dev,
...
#ifdef CONFIG_REVERSE_CHARGE
static DEVICE_ATTR(reverse_sc_support, S_IRUSR  | S_IRGRP | S_IROTH, reverse_sc_support_show, NULL);
static DEVICE_ATTR(reverse_sc_mode, S_IRUSR | S_IWUSR | S_IRGRP | S_IROTH,
	reverse_sc_mode_show, reverse_sc_mode_store);
#endif /* CONFIG_REVERSE_CHARGE */
static DEVICE_ATTR(hw_pd_status, S_IRWXU | S_IRWXG | S_IROTH, hw_pd_status_show, hw_pd_status_store);
static DEVICE_ATTR(power_check, S_IRWXU | S_IRWXG | S_IROTH, power_check_show, NULL);
...
#endif /* CONFIG_REVERSE_CHARGE */
	&dev_attr_hw_pd_status.attr,
	&dev_attr_power_check.attr,
	NULL,
};
...
bool hw_pd_is_certification_hiz_en(void)
{
	return pd_certification_set_hiz;
}
...
static int pd_dpm_hw_update_src_cap(struct otg_cap *ext, int send)
{
	...
	hwlog_info("%s: max_otg_vol=%d, max_otg_cur=%d, send=%d\n",
		__func__, ext->otg_vol, ext->otg_cur, send);
...
void pd_dpm_report_pd_source_vbus(struct pd_dpm_info *di, void *data)
{
	...
	if (vbus_state->mv == 0) {
		...
		if (g_support_pd_certification) {
			if (pd_certification_set_hiz) {
				pd_certification_set_hiz = false;
...
static bool hw_pd_filter_pps_sink(struct pd_dpm_info *di, struct pd_dpm_vbus_state *state)
{
	int ret;
	...
	if ((!di->pd_finish_flag) || (di->pd_source_vbus))
		return false;

	if (state->mv == 0)
		return false;

	ret = hw_pd_pps_filter_sink(state->mv, state->ma);
...
void pd_dpm_report_pd_sink_vbus(struct pd_dpm_info *di, void *data)
{
	...
	if (g_pd_support_pps && hw_pd_filter_pps_sink(di, vbus_state)) {
		hwlog_info("%s: Ignore pps sink message\n", __func__);
		return;
	}
	...
	if (vbus_state->vbus_type & TCP_VBUS_CTRL_PD_DETECT) {
		...
		di->pd_finish_flag = true;
		di->ctc_cable_flag = true;
		g_cable_state.emark_detect_running = true;
		if (g_support_pd_certification)
			pd_reset_bMaxPower(true);
	}
	...
	if (vbus_state->mv == 0) {
		...
		if (g_support_pd_certification) {
			if (vbus_state->ma < STANDBY_CURR_MAX_MA) {
				pd_certification_set_hiz = true;
				charge_set_hiz_enable(HIZ_MODE_ENABLE);
			} else if (pd_certification_set_hiz) {
				pd_certification_set_hiz = false;
				charge_set_hiz_enable(HIZ_MODE_DISABLE);
			}
		}
...
int hw_pd_get_bc12_type(void)
{
	if (!g_pd_di)
		return CHARGER_TYPE_NONE;

	hwlog_info("%s : %lu\n", __func__, g_pd_di->bc12_event);
	return g_pd_di->bc12_event;
}

int hw_pd_get_src_cap(void)
{
	if (!g_ops || !g_ops->pd_dpm_get_src_cap)
		return false;
 
	return g_ops->pd_dpm_get_src_cap(g_client);
}
...
int pd_dpm_report_bc12(struct notifier_block *usb_nb, unsigned long event, void *data)
{
	...
	if ((event == CHARGER_TYPE_DCP) && external_pd_flag) {
		hwlog_info("dcp event is succ");
		complete(&g_cable_state.bc12_dcp);
		g_cable_state.bc12_dcp_cmp = true;
		while (i < PD_DPM_CABLE_WAIT_EMARK_CNT) {
			if (!g_pd_di->ctc_cable_flag || 
				!g_cable_state.support_cable_auth || 
				!g_cable_state.emark_detect_running)
				break;
			/* sleep 20ms to wait emark finish */
			msleep(20);
			hwlog_info("sleep wait for emark");
			i++;
		}
	}
	...
	if (g_slow_down_bc12) {
		msleep(g_slow_down_bc12); // Slow down the rate at which BC12 recognizes DCP g_slow_down_bc12 ms
		hwlog_info("%s : slow down %dMS\n", __func__, g_slow_down_bc12);
	}
	if ((event == CHARGER_TYPE_DCP) && !di->pd_finish_flag && hw_pd_get_src_cap()) {
		vbus_state.mv = TEM_VOL_VALUE;
		vbus_state.ma = TEM_CUR_VAL;
		vbus_state.vbus_type = 1;
		vbus_state.ext_power = 1;
		vbus_state.remote_rp_level = PD_DPM_CC_VOLT_SNK_3_0;
		hwlog_info("%s : report PD_TYPE\n", __func__);
		pd_dpm_vbus_notifier_call(di, PD_DPM_VBUS_TYPE_PD, &vbus_state);
	}
	...
	if ((!ignore_bc12_event_when_vbusoff && (event == CHARGER_TYPE_NONE)) ||
		...
		if ((!di->pd_finish_flag && (last_event != event)) ||
			...
			(event == CHARGER_TYPE_SDP) || (event == CHARGER_TYPE_CDP)))) {
...
void pd_dpm_report_device_attach(void)
{
	...
	hwlog_info("%s \r\n", __func__);
	usb_config_set_delay(g_config_delay);
	if (g_support_pd_certification) {
		huawei_charger_set_iin(CHARGER_TYPE_SDP, CHARGE_CURRENT_0100_MA);
		if (delayed_work_pending(&g_pd_di->usb_enum_timeout_work))
			cancel_delayed_work_sync(&g_pd_di->usb_enum_timeout_work);
		schedule_delayed_work(&g_pd_di->usb_enum_timeout_work, msecs_to_jiffies(USB_ENUM_TIMEOUT));
	}
...
static int get_usb_enum_device(struct usb_device *udev)
{
	int nintf;
	...
	hwlog_info("%s\n", __func__);
	/* most usb device only one configuration */
	if (!udev || udev->descriptor.bNumConfigurations != 1)
		return -EINVAL;
	...
	if (!alt)
		return -EINVAL;
	return alt->desc.bInterfaceClass;
}
				...
				unsigned long action, void *data)
{
	...
	if (!udev || (action != USB_DEVICE_ADD)
		|| (g_reverse_sc.reverse_sc_device_class != REVERSE_SC_DEF_DEV_CLS)
		|| !pd_dpm_is_rev_sc_plugin())
		return NOTIFY_OK;

	if ((udev->parent != NULL) && (udev->parent->parent == NULL)) {
		g_reverse_sc.reverse_sc_device_class = get_usb_enum_device(udev);
		hwlog_info("%s reverse_sc_device_class:%d\n", __func__,
			g_reverse_sc.reverse_sc_device_class);
	}
...
void pd_dpm_report_device_detach(void)
{
	...
	if (g_support_pd_certification) {
		...
		if (delayed_work_pending(&g_pd_di->usb_enum_timeout_work))
			cancel_delayed_work_sync(&g_pd_di->usb_enum_timeout_work);
		huawei_charger_set_iin(CHARGER_TYPE_SDP, CHARGE_CURRENT_0500_MA);
	}
...
static bool pd_dpm_support_host_attach(void)
{
	if (!g_ops || !g_ops->pd_dpm_inquire_usb_comm)
		return true;

	if (!g_ops->pd_dpm_inquire_usb_comm(g_client) && g_pd_di->ctc_cable_flag)
		return false;
	return true;
}
...
static void pd_dpm_usb_update_state(struct work_struct *work)
{
	...
	case PD_DPM_USB_TYPEC_HOST_ATTACHED:
		if (last_ev != PD_DPM_USB_TYPEC_DETACHED)
			pd_dpm_report_detach(last_ev);
		if (new_ev == PD_DPM_USB_TYPEC_HOST_ATTACHED) {
			support_host_attach = pd_dpm_support_host_attach();
			if (!support_host_attach) {
				hwlog_info("not support host attach event");
				break;
			}
		}
...
enum cur_cap pd_dpm_get_cvdo_cur_cap(void)
{
	...
	if (!g_cable_state.usb_port_start && g_cable_state.support_cable_auth) {
...
static void pd_dpm_emark_detect_finish(void)
{
	hwlog_info("%s start\n", __func__);
	emark_detect_complete();
	g_cable_state.emark_detect_running = false;
}
...
void pd_dpm_set_cable_vdo(void *data)
{
	...
	if (g_pd_di->cable_vdo.cable_vdo_ext == 0)
		g_cable_state.succ_flag = PD_DPM_RESULT_THIRDPARTY;
}

int pd_dpm_support_cable_auth(void)
{
	return g_cable_state.support_cable_auth;
}

int pd_dpm_uvdm_send_data(uint32_t *uvdm_data, uint8_t len)
{
	int ret;

	ret = pd_dpm_notify_cable_uvdm(uvdm_data, len);
	if (ret)
		return 0;
	return PD_DPM_CABLE_ERR_CODE;
}
...
static void pd_dpm_start_cable_auth(void)
{
	...
	reinit_completion(&g_cable_state.bc12_dcp);

	if (external_pd_flag == 0) {
		...
		ret = pd_dpm_uvdm_send_data(uvdm_data, len);
		schedule_delayed_work(&di->hw_pd_get_cable_data_work, msecs_to_jiffies(0));
	}
	if (ret)
		hwlog_info("send random is fail");
}
...
static void pd_dpm_set_cable_auth_cur(void)
{
	...
	pd_dpm_notify_9v2a_enable();
	pd_dpm_emark_detect_finish();
...
int pd_dpm_handle_pe_event(unsigned long event, void *data)
{
	...
	case PD_DPM_PE_EVT_TYPEC_STATE:
		{
			...
			case PD_DPM_TYPEC_ATTACHED_SNK:
				...
				g_cable_state.bc12_dcp_cmp = false;
				g_9v2a_forbid = 0;
			...
			case PD_DPM_TYPEC_UNATTACHED:
				...
				if (pd_dpm_analog_hs_state == 1) {
					...
					g_pd_di->bc12_event = CHARGER_TYPE_NONE;
					hwlog_info("%s UNATTACHED\r\n", __func__);
				}
#ifdef CONFIG_TYPEC_CAP_CUSTOM_SRC2
				if (support_smart_holder) {
					if (g_pd_smart_holder)
						hishow_notify_dev_uevent(HISHOW_DEVICE_OFFLINE,
							HISHOW_USB_DEVICE);
			...
			case PD_DPM_TYPEC_ATTACHED_CUSTOM_SRC2:
				if (support_smart_holder) {
					...
					if (g_cable_vdo_ops && g_cable_vdo_ops->is_cust_src2_cable()) {
						if (!g_pd_smart_holder)
							hishow_notify_dev_uevent(HISHOW_DEVICE_ONLINE,
								HISHOW_USB_DEVICE);
	...
	case PD_DPM_PE_EVT_DR_SWAP:
		{
			...
			if (g_reverse_sc.support_reverse_sc && pd_dpm_is_reverse_charge_enable())
				return 0;
#endif /* CONFIG_REVERSE_CHARGE */
			if (!pd_dpm_support_host_attach()) {
				hwlog_info("%s datarole	notify break", __func__);
				break;
			}
	...
	case PD_DPM_PE_CABLE_VDO:
		if (g_pd_support_pps)
			hw_pd_pps_notify_vdo();
		if (g_cable_state.support_cable_auth == 1) {
			g_cable_state.cable_fail = PD_DPM_CABLE_INIT;
		...
		pd_dpm_emark_detect_finish();
		pd_dpm_vbus_notifier_call(g_pd_di, PD_DPM_EMARK_FINISHED, NULL);
		break;

	case PD_DPM_PE_CABLE_DATA:
		complete(&g_cable_state.emark_detect_cable_data);
		hwlog_info("PD_DPM_PE_CABLE_DATA enter, time out is %d", g_cable_state.cable_ntimeout);
...
static void pd_misc_dts_parser(struct pd_dpm_info *di, struct device_node *np)
{
	...
	(void)power_dts_read_u32(power_dts_tag(HWLOG_TAG), np,
		"support_cable_auth", &g_cable_state.support_cable_auth, 0);
	(void)power_dts_read_u32(power_dts_tag(HWLOG_TAG), np,
...
#endif /* CONFIG_REVERSE_CHARGE */
	(void)power_dts_read_u32(power_dts_tag(HWLOG_TAG), np,
		"pd_support_pps", &g_pd_support_pps, 0);
	(void)power_dts_read_u32(power_dts_tag(HWLOG_TAG), np,
		"forbidden_pd_9v", &g_forbidden_pd_9v, 0);
	(void)power_dts_read_u32(power_dts_tag(HWLOG_TAG), np,
		"support_pd_certification", &g_support_pd_certification, 0);
#ifdef CONFIG_DKMD_DKSM
	(void)power_dts_read_u32(power_dts_tag(HWLOG_TAG), np,
		"support_fold_screen", &g_support_fold_screen, 0);
#endif
	(void)power_dts_read_u32(power_dts_tag(HWLOG_TAG), np,
		"config_delay", &g_config_delay, 0);
	(void)power_dts_read_u32(power_dts_tag(HWLOG_TAG), np, 
		"slow_down_bc12", &g_slow_down_bc12, 0);	
}
...
static void pd_dpm_parse_dts(struct pd_dpm_info *di, struct device_node *np)
{
	...
	g_cable_state.support_cable_auth = 0;
	hwlog_info("support cable flag is %d\n", g_cable_state.support_cable_auth);
#endif
...
static void usb_enum_timeout_work(struct work_struct *work)
{
	/* usb enum timeout, set sdp current to 500mA */
	huawei_charger_set_iin(CHARGER_TYPE_SDP, CHARGE_CURRENT_0500_MA);
	hwlog_info("%s\n", __func__);
}
...
{
	/* usb suspend, set charger hiz when the port type is sdp */
	if (chip_get_charger_type() == CHARGER_TYPE_SDP) {
		pd_certification_set_hiz = true;
		charge_set_hiz_enable(HIZ_MODE_ENABLE);
		hwlog_info("%s set charger hiz\n", __func__);
	}
	hwlog_info("%s\n", __func__);
}

static int gadget_event_notifier_fn(struct notifier_block *nb, unsigned long event, void *para)
{
	struct pd_dpm_info *di = container_of(nb, struct pd_dpm_info, gadget_event_nb);
	...
	case DEVICE_EVENT_SETCONFIG:
		/* usb eunm success, set sdp current to 500mA */
		huawei_charger_set_iin(CHARGER_TYPE_SDP, CHARGE_CURRENT_0500_MA);
		if (delayed_work_pending(&di->usb_enum_timeout_work))
			cancel_delayed_work(&di->usb_enum_timeout_work);
		hwlog_info("%s set_config\n", __func__);
		break;
	case DEVICE_EVENT_SUSPEND:
		hwlog_info("%s usb suspend\n", __func__);
		usb_state = *(enum usb_device_state *)para;
		if (usb_state < USB_STATE_CONFIGURED) {
			schedule_work(&di->usb_suspend_work);
			hwlog_info("%s usb_suspend_work\n", __func__);
		}
		break;
	default:
		break;
...
static void pd_dpm_init_resource(struct pd_dpm_info *di)
{
	...
	if (g_support_pd_certification) {
		INIT_DELAYED_WORK(&di->usb_enum_timeout_work, usb_enum_timeout_work);
		INIT_WORK(&di->usb_suspend_work, usb_suspend_work);
		di->gadget_event_nb.notifier_call = gadget_event_notifier_fn;
#ifndef CONFIG_USB_HIUSB
		ret = dwc3_device_event_notifier_register(&di->gadget_event_nb);
#endif
		if (ret)
			hwlog_err("gadget device event notifier register failed\n");
	}
...
static int pd_dpm_probe(struct platform_device *pdev)
{
	...
	init_completion(&g_cable_state.bc12_dcp);
	...
	if (g_pd_support_pps)
		hw_pd_pps_init(&g_hw_pd_pps_ops);
...
#ifdef CONFIG_REVERSE_CHARGE
	if (g_reverse_sc.support_reverse_sc)
		usb_unregister_notify(&usb_reverse_nb);
#endif /* CONFIG_REVERSE_CHARGE */
