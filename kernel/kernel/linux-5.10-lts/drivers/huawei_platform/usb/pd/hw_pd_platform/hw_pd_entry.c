...
static void pd_cc_protection_dts_parser(struct device_node *np)
{
	...
	(void)power_dts_read_u32(power_dts_tag(HWLOG_TAG), np,
		"v700_cc_abnormal", &pd_state(v700_cc_abnormal), 0);
}
...
static void pd_misc_dts_parser(struct device_node *np)
{
	...
	(void)power_dts_read_u32(power_dts_tag(HWLOG_TAG), np,
		"support_pd_certification", &pd_state(support_pd_certification), 0);
#ifdef CONFIG_DKMD_DKSM
	(void)power_dts_read_u32(power_dts_tag(HWLOG_TAG), np,
		"support_fold_screen", &pd_state(support_fold_screen), 0);
#endif
}
...
static void pd_accessory_dts_parser(struct device_node *np)
{
	...
	(void)power_dts_read_u32(power_dts_tag(HWLOG_TAG), np,
		"pd_support_pps", &pd_state(pd_support_pps), 0);
	(void)power_dts_read_u32(power_dts_tag(HWLOG_TAG), np,
		"forbidden_pd_9v", &pd_state(forbidden_pd_9v), 0);
	(void)power_dts_read_u32(power_dts_tag(HWLOG_TAG), np,
	    "wired_rchg_9v", &pd_state(wired_rchg_9v), 0);
#ifdef CONFIG_REVERSE_CHARGE
	(void)power_dts_read_u32(power_dts_tag(HWLOG_TAG), np,
		"support_reverse_sc", &pd_state(support_reverse_sc), 0);
#endif /* CONFIG_REVERSE_CHARGE */
	(void)power_dts_read_u32(power_dts_tag(HWLOG_TAG), np, "force_report_port",
		&pd_state(force_report_port), 0);
	(void)power_dts_read_u32(power_dts_tag(HWLOG_TAG), np, "slow_down_bc12",
		&pd_state(slow_down_bc12), 0);
}
