...
bool hw_pd_support_voltage_up(void)
{
	if (pd_state(forbidden_pd_9v)) {
		hwlog_info("%s : forbidden PD 9V\n", __func__);
		return false;
	}
...
void hw_pd_send_otg_state (bool enable)
{
	return rchg_pd_otg_enable(enable);
}

void pd_wired_rchg_cb (unsigned int type, char *event_data)
{
	unsigned long event;
	struct otg_cap *ext_src_cap = NULL;
	u32 otg_vol;
	u32 otg_cur;
	int retry = 100;  /* the maximum retry is 100 times */
	...
	if (pd_state(cur_usb_event) != PD_DPM_TYPEC_ATTACHED_SRC) {
		hwlog_err("%s : is not source, ignore\n", __func__);
		return;
	}

	event = ((struct wired_rchg_event *)event_data)->event;
	ext_src_cap = ((struct wired_rchg_event *)event_data)->data;
	hwlog_info("%s : received event = %lu\n", __func__, event);

	otg_vol = ext_src_cap->otg_vol;
	otg_cur = ext_src_cap->otg_cur;

	if ((otg_vol != SRC_VBUS_DEFAULT_VOL) && (otg_vol != SRC_VBUS_HIGH_VOL)) {
	...
	if (!hw_pd_ops_check_src_cap()) {
		hwlog_err("%s not support\n", __func__);
		return;
	}

	/*
	 * waiting 800ms for DP communication, If the DP mode is entered,
	 * the reverse charging of large power is not supported.
	 */
	msleep(800); 
	if (hw_pd_get_dp_state()) {
		hwlog_err("%s in DP mode, not support\n", __func__);
		return;
	}

	hw_pd_ops_update_src_cap(otg_vol, otg_cur, 1);

	if (otg_vol == SRC_VBUS_DEFAULT_VOL) {
		/*
		 * After the 5V negotiation is complete,
		 * turn off the 9V power supply. The minimum waiting
		 * time granularity is 2ms, and the maximum retry is 100 times.
		 */
		while (retry > 0) {
			retry--;
			msleep(2);
			if (hw_pd_get_source_vbus() == SRC_VBUS_DEFAULT_VOL) {
				hwlog_info("%s: 5V negotiation completed\n", __func__);
				break;
			}
		}
