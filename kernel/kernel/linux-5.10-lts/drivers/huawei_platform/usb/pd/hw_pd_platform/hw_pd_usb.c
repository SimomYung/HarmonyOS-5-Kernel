...
int hw_pd_handle_combphy_event(struct pd_dpm_combphy_event event)
{
	...
	if ((g_last_combphy_notify_event.dev_type == TCA_DP_IN) ||
		...
		if ((event.dev_type == TCA_CHARGER_CONNECT_EVENT) ||
			...
			if (pd_state(cur_usb_event) != PD_DPM_TYPEC_UNATTACHED) {
...
#ifdef CONFIG_REVERSE_CHARGE
	if (pd_state(support_reverse_sc) && reverse_sc_device_detect()) {
		hwlog_info("%s reverse charge is enabled, skip usb enumeration\n", __func__);
		return;
	}
#endif /* CONFIG_REVERSE_CHARGE */
...
void hw_pd_usb_connect_init(void)
{
	...
	if (g_pd_combphy_wq == NULL) {
		hwlog_err("%s create_workqueue fail\n", __func__);
	}
