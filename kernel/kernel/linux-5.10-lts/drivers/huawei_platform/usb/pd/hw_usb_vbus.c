...
#ifdef CONFIG_HUAWEI_OHOS_DSM
static int g_icon_abnormal_cnt;
#define ICON_ABNORMAL_REPORT_CNT      5
#define ERR_CONTINUOUS_STOP_CHARGING  920080000 /* report on single only. use temporary dmd no */
void vbus_disconnect_abnormal_check()
{
	int state;
	int ret;
	char dsm_buf[PD_DPM_CC_DMD_BUF_SIZE] = {0};

	pd_dpm_get_typec_state(&state);
	if (state == PD_DPM_USB_TYPEC_DEVICE_ATTACHED)
		g_icon_abnormal_cnt++;

	if (state == PD_DPM_USB_TYPEC_DETACHED)
		g_icon_abnormal_cnt = 0;

	if (g_icon_abnormal_cnt == ICON_ABNORMAL_REPORT_CNT) {
		ret = snprintf_s(dsm_buf, PD_DPM_CC_DMD_BUF_SIZE, PD_DPM_CC_DMD_BUF_SIZE - 1,
				"disconnect abnormal when cc is connected");
		if (ret < 0)
			return;
		power_dsm_report_dmd(POWER_DSM_USB, ERR_CONTINUOUS_STOP_CHARGING, dsm_buf);
	}
}
#endif /* CONFIG_HUAWEI_OHOS_DSM */
...
#ifdef CONFIG_HUAWEI_OHOS_DSM
	vbus_disconnect_abnormal_check();
#endif /* CONFIG_HUAWEI_OHOS_DSM */
}
