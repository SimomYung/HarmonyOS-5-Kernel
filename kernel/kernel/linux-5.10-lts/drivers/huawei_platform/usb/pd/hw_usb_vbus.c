...
#define ICON_ABNORMAL_REPORT_CNT      3
...
void vbus_disconnect_abnormal_check(void)
{
	int state;
	int ret;
	...
	if (g_icon_abnormal_cnt == ICON_ABNORMAL_REPORT_CNT) {
		...
		if (ret < 0)
			return;
		power_dsm_report_dmd(POWER_DSM_USB, POWER_DSM_CONTINUOUS_STOP_CHARGING, dsm_buf);
	}
}
