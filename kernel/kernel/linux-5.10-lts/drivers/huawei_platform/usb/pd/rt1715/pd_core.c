	...
	struct pd_battery_info *bat_info)
{
	...
	ret = snprintf_s(mfrs_info->mfrs_string, sizeof(mfrs_info->mfrs_string),
	...
	struct pd_port *pd_port, struct device_node *np)
{
	...
	for (i = 0; i < pd_port->bat_nr; i++) {
		(void)snprintf_s(temp_string, 26, 26, "bat-info%d", i);
	...
	struct pd_port *pd_port, struct device_node *np)
{
	...
	for (i = 0; i < pd_port->country_nr; i++) {
		(void)snprintf_s(temp_string, 26, 26, "country%d", i);
		sub = of_find_node_by_name(np, temp_string);
		if (!sub) {
		...
		uint8_t chunk_nr, uint8_t size, const uint8_t *data)
{
	...
	*ext_hdr = PD_EXT_HEADER_CK(size, request, chunk_nr, true);
	(void)memcpy_s(&ext_hdr[1], (sizeof(uint32_t) * PD_DATA_OBJ_SIZE - sizeof(uint16_t)), data, size);
		...
		uint32_t timer_id)
{
	...
	(void)memcpy_s(&payload[1], sizeof(uint32_t) * (PD_DATA_OBJ_SIZE - 1), data_obj, sizeof(uint32_t) * cnt);
