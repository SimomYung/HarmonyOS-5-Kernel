...
{
	size_t ts_size = 0, msg_size = 0;
	struct msg_node *mn = NULL;
	u64 ts = 0;
	unsigned long rem_msec = 0;
	va_list args;
	char buf[PD_DBG_INFO_BUF] = { 0 };
	...
	ts_size = snprintf_s(buf, sizeof(buf), (sizeof(buf) - 1), "<%5lu.%03lu>", (unsigned long)ts, rem_msec);
	va_start(args, fmt);
	msg_size = vsnprintf_s(buf + ts_size, (sizeof(buf) - ts_size), (sizeof(buf) - ts_size - 1), fmt, args);
	va_end(args);

	if ((ts_size <= 0) || (msg_size <= 0))
		return -EPERM;
	...
	(void)memcpy_s(mn->msg, (ts_size + msg_size), buf, (ts_size + msg_size));
