...
#define SRC_CAP_9V_MASK      0xFFC00
#define SRC_CAP_9V           0x2D000
#define POWER_CAPS_PDO_MAX_NR 7
...
void pd_dpm_inform_cable_id(struct pd_port *pd_port, bool ack, bool src_startup)
{
	...
	if (ack && payload) {
		buf[0] = '\0';
		(void)snprintf_s(buf + strlen(buf), sizeof(buf) - strlen(buf), sizeof(buf) - strlen(buf),
			 "InformCableID");
		for (i = 0; i < cnt; i++)
			(void)snprintf_s(buf + strlen(buf), sizeof(buf) - strlen(buf), sizeof(buf) - strlen(buf),
				 ", 0x%08x", payload[i]);
		...
		rc = memcpy_s(pe_data->cable_vdos, sizeof(uint32_t) * VDO_MAX_NR, payload, sizeof(uint32_t) * cnt);
		if (rc != EOK)
...
{
	(void)memset_s(sink, sizeof(struct dpm_pdo_info_t), 0, sizeof(*sink));
...
#ifdef CONFIG_TCPC_RT1715
bool tcpci_support_9v_src_cap(struct pd_port *pd_port)
{
    int i;
    struct pd_port_power_caps *src_cap = &pd_port->pe_data.remote_src_cap;

    for (i = 0; i < src_cap->nr; i++) {
        if (SRC_CAP_9V == (src_cap->pdos[i] & SRC_CAP_9V_MASK)) {
            DPM_INFO("SrcCap include 9v\n");
            return true;
        }
    }
		...
		struct pd_port *pd_port, struct dpm_rdo_info_t *req_info)
{
	...
	(void)memset_s(req_info, sizeof(struct dpm_rdo_info_t), 0, sizeof(struct dpm_rdo_info_t));
			...
			struct pd_port *pd_port, uint32_t *payload)
{
	...
	rc = memcpy_s(pd_port->pe_data.partner_vdos, sizeof(uint32_t) * VDO_MAX_NR, payload, size);
	if (rc != EOK)
		pr_err("%s: memcpy_s fail = %d\n", __func__, rc);
	(void)memset_s((char *)pd_port->pe_data.partner_vdos + size, sizeof(uint32_t) * VDO_MAX_NR - size, 0,
	...
	uint16_t svid, uint32_t *mode_list, uint8_t count)
{
	...
	if (count != 0) {
		rc = memcpy_s(svid_data->remote_mode.mode_vdo, sizeof(uint32_t) * VDO_MAX_NR,
...
void pd_dpm_ufp_recv_cvdm(struct pd_port *pd_port)
{
	...
	rc = memcpy_s(pd_port->cvdm_data, sizeof(uint32_t) * PD_DATA_OBJ_SIZE,
...
void pd_dpm_dr_inform_sink_cap(struct pd_port *pd_port)
{
	...
	rc = memcpy_s(snk_cap->pdos, sizeof(uint32_t) * POWER_CAPS_PDO_MAX_NR,
		pd_get_msg_data_payload(pd_port),
		pd_get_msg_data_size(pd_port));
	if (rc != EOK)
		pr_err("%s: memcpy_s fail = %d\n", __func__, rc);
...
void pd_dpm_dr_inform_source_cap(struct pd_port *pd_port)
{
	...
	rc = memcpy_s(src_cap->pdos, sizeof(uint32_t) * POWER_CAPS_PDO_MAX_NR,
		pd_get_msg_data_payload(pd_port),
		pd_get_msg_data_size(pd_port));
	if (rc != EOK)
		pr_err("%s: memcpy_s fail = %d\n", __func__, rc);
...
int pd_dpm_send_country_info(struct pd_port *pd_port)
{
	...
	for (i = 0; i < pd_port->country_nr; i++) {
		if (ca[i].code == cc) {
			cidb_size += ca[i].len;
			rc = memcpy_s(cidb.country_special_data, PD_CIDB_DYNAMIC_SIZE,
					ca[i].data, ca[i].len);
			if (rc != EOK)
				pr_err("%s: memcpy_s fail = %d\n", __func__, rc);
			break;
...
int pd_dpm_send_status(struct pd_port *pd_port)
{
	...
	(void)memset_s(&sdb, PD_SDB_SIZE, 0, PD_SDB_SIZE);
