				 ...
				 int bytes, const void *src, bool asyn)
{
	...
	if (rd->props.io_log_en) {
		j = 0;
		for (i = 0; i < count; i++) {
			ret = snprintf_s(wri_data + j, sizeof(wri_data) - j, sizeof(wri_data) - j - 1,
				       "%02x,", wdata[i]);
			...
			int bytes, void *dest)
{
	...
	(void)memcpy_s(dest, bytes, &rd->cache_data[rio.index][rio.offset], bytes);
...
void rt_cache_getlasterror(struct rt_regmap_device *rd, char *buf, size_t size)
{
	...
	down(&rd->semaphore);
	ret = snprintf_s(buf, size, size - 1, "%s", rd->err_msg);
	up(&rd->semaphore);
	if ((ret < 0) || (ret >= size))
...
do {									\
	...
	ret = snprintf_s(rd->err_msg + len, ERR_MSG_SIZE - len,		\
		       ERR_MSG_SIZE - len - 1, fmt, ##__VA_ARGS__);				\
	rd->error_occurred = 1;						\
	up(&rd->semaphore);						\
			     ...
			     size_t count, loff_t *ppos)
{
	...
	*ppos = 0;
	...
	case RT_DBG_DATA:
		...
		(void)memset_s(rd->regval, sizeof(rd->regval), 0, sizeof(rd->regval));
			    ...
			    size_t count, loff_t *ppos)
{
	...
	ret = snprintf_s(lbuf + j, lbuf_size - j, lbuf_size - j - 1, "reg0x%02x:0x", rm->addr);
	...
	j += ret;
	for (i = 0; i < rm->size; i++) {
		ret = snprintf_s(lbuf + j, lbuf_size - j, lbuf_size - j - 1, "%02x,", rd->regval[i]);
		if ((ret < 0) || (ret >= lbuf_size - j)) {
			ret = -EINVAL;
			goto out;
		}
	...
	ret = snprintf_s(lbuf + j, lbuf_size - j, lbuf_size - j - 1, "\n");
	if ((ret < 0) || (ret >= lbuf_size - j)) {
		ret = -EINVAL;
		goto out;
	}
			 ...
			 void *client, int dev_addr, void *drvdata)
{
	...
	(void)memcpy_s(&rd->props, sizeof(rd->props), props, sizeof(rd->props));
	rd->rops = rops;
