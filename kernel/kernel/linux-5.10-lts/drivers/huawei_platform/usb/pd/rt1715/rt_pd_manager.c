...
#define IBUS_3000MA	                3000
#define IBUS_1000MA	                1000
...
struct rt_pd_manager_data {
	...
	int ctc_ocp_flag;
	int bypassboost;
#ifdef CONFIG_TCPC_RT1715_ROLE_DEF_TEST
	struct delayed_work test_role_work;
#endif
#endif
...
#ifdef CONFIG_TCPC_RT1715
void pd_dpm_send_cable_data(struct pd_dpm_cable_data data)
{
	int i;
	for (i = 0; i < data.cnt; i++)
		pr_info("pd_dpm_send_cable_data send data is %x", data.msg[i]);
	pd_dpm_handle_pe_event(PD_DPM_PE_CABLE_DATA, (void *)(&data));
	return;
}
#endif

#ifdef CONFIG_TCPC_RT1715_ROLE_DEF_TEST
static bool test_role_work = false;
static void pd_tcp_test_role(struct rt_pd_manager_data *rpmd, int snk)
{
	struct tcpc_device *tcpc = rpmd->tcpc;

	if (!(rpmd->tcpc->test_role_def))
		return;

	pr_info("%s: snk=%d, pd=%d, work=%d\n", __func__, snk,
		(tcpc ? (int)tcpc->pd_port.pe_data.pd_prev_connected : (-EIO)), test_role_work);

	if (test_role_work)
		return;

	if (snk < 0) {
		if (delayed_work_pending(&rpmd->test_role_work))
			cancel_delayed_work(&rpmd->test_role_work);
		return;
	}
	...
	if (!(delayed_work_pending(&rpmd->test_role_work))) {
		pr_info("%s: start test_role_work\n", __func__);
		/* 30s: The role switchover starts 30 seconds later */
		schedule_delayed_work(&rpmd->test_role_work, msecs_to_jiffies(30 * 1000));
	}
}

static void test_role_handler(struct work_struct *work)
{
	struct delayed_work *dwork = to_delayed_work(work);
	struct rt_pd_manager_data *rpmd =
		container_of(dwork, struct rt_pd_manager_data, test_role_work);
	int ret;

	test_role_work = true;
	if (rpmd->tcpc->desc.role_def == TYPEC_ROLE_SNK)
		rpmd->tcpc->desc.role_def = TYPEC_ROLE_TRY_SRC;
	else if (rpmd->tcpc->desc.role_def == TYPEC_ROLE_TRY_SRC)
		rpmd->tcpc->desc.role_def = TYPEC_ROLE_SNK;

	ret = tcpm_typec_change_role(rpmd->tcpc, (uint8_t)rpmd->tcpc->desc.role_def);
	pr_err("%s: set role to %d, ret=%d\n", __func__, rpmd->tcpc->desc.role_def, ret);

	/* 5s: The role switchover is complete in 5 seconds, and the flag bit is restored */
	msleep(5000);
	test_role_work = false;
}
#endif

#ifdef CONFIG_HW_PD_COMMON
static void pd_tcp_send_vconn_state(bool en)
{
	int val = en ? 1 : 0;
	(void)pd_dpm_handle_pe_event(PD_DPM_PE_EVT_SOURCE_VCONN, (void *)(&val));
}
#endif /* CONFIG_HW_PD_COMMON */
				...
				unsigned long event, void *data)
{
	...
	case TCP_NOTIFY_SINK_VBUS:
		...
		if ((tcpm_inquire_usb_comm(rpmd->tcpc)) && (noti->vbus_state.ma >= IBUS_3000MA)) {
			noti->vbus_state.ma = IBUS_1000MA;
			dev_info(rpmd->dev, "%s: connect is phone, force ma = 1000\n", __func__);
		}
#ifdef CONFIG_HW_PD_COMMON
		pd_dpm_handle_pe_event(PD_DPM_PE_EVT_SINK_VBUS, (void *)(&noti->vbus_state));
#else
...
#ifdef CONFIG_TCPC_RT1715_ROLE_DEF_TEST
			if (noti->vbus_state.type & TCP_VBUS_CTRL_PD_DETECT)
				pd_tcp_test_role(rpmd, 0);
#endif
		/* enable/disable OTG power output */
#ifdef CONFIG_TCPC_RT1715
#ifdef CONFIG_HW_PD_COMMON
		pd_dpm_handle_pe_event(PD_DPM_PE_EVT_SOURCE_VBUS, (void *)(&noti->vbus_state));
#else
...
#ifdef CONFIG_HW_PD_COMMON
		pd_tcp_send_vconn_state(noti->en_state.en);
#else
	...
	case TCP_NOTIFY_TYPEC_STATE:
...
#ifdef CONFIG_TCPC_RT1715_ROLE_DEF_TEST
			if (tc_state.new_state == PD_DPM_TYPEC_UNATTACHED)
				pd_tcp_test_role(rpmd, -EIO);
#endif
			pd_dpm_handle_pe_event(PD_DPM_PE_EVT_TYPEC_STATE, (void*)&tc_state);
#ifdef NOT_USED_CODE
#ifdef CONFIG_TCPC_RT1715
		...
		if (new_state == TYPEC_UNATTACHED) {
			...
			(void)memset_s(&rpmd->partner_identity, sizeof(rpmd->partner_identity), 0,
...
#ifdef CONFIG_HW_PD_COMMON
		pd_tcp_send_vconn_state(vconn_swap_flag);
#else
	...
	case TCP_NOTIFY_PD_STATE:
		...
		case PD_CONNECT_PE_READY_SRC_PD30:
			ret = tcpm_inquire_cable_inform(rpmd->tcpc, vdos);
			pr_info("vdo is %x, %x, %x, %x, %x", vdos[0], vdos[1], vdos[2], vdos[3], vdos[0] & 0xFFFF);
			cable_vdo.cable_vdo = vdos[3];                   /* vdos[3] is cable vdo data */
			if ((vdos[0] & 0xFFFF) == 0x12D1)                /* vdos[0] is cable ID Header vdo */
				cable_vdo.cable_vdo_ext = vdos[2];           /* vdos[2] is Product vdo */
			else
				cable_vdo.cable_vdo_ext = 0;
			if (pd_dpm_support_voltage_up() && !pd_dpm_support_cable_auth())
...
#ifdef CONFIG_USB_POWER_DELIVERY
#ifdef CONFIG_RECV_BAT_ABSENT_NOTIFY
	struct notifier_block *fg_bat_nb = NULL;
	int ret = 0;
#endif /* CONFIG_RECV_BAT_ABSENT_NOTIFY */
#endif /* CONFIG_USB_POWER_DELIVERY */
...
static void rt_pd_manager_parse_dts(struct rt_pd_manager_data *di, struct device_node *np)
{
	...
	(void)power_dts_read_u32(power_dts_tag(HWLOG_TAG), np,
		"enable_bypassboost", &di->bypassboost, 0);
...
#ifdef CONFIG_TCPC_RT1715_ROLE_DEF_TEST
	INIT_DELAYED_WORK(&rpmd->test_role_work, test_role_handler);
#endif
	rpmd->usb_dr = DR_IDLE_DEVICE;
	rpmd->usb_type_polling_cnt = 0;
