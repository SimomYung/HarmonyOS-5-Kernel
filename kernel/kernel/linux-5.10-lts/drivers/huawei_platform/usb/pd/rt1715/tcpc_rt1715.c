...
#if defined(CONFIG_TCPC_RT1715) && defined(CONFIG_HUAWEI_TCPC_HUSB311)
#define TCPC_NORMAL_RP_DUTY_HUSB311	(330)		/* 33% */
...
#define SHIFT_16	16
#define TCPC_TRANSMIT_PACKET_MAX 7
...
#ifdef CONFIG_TCPC_RT1715
bool chip_id_is_et7303_did(void)
{
...
static int rt1711_regmap_init(struct rt1711_chip *chip)
{
	...
	(void)snprintf_s(name, sizeof(name), sizeof(name), "rt1711-%02x", chip->client->addr);
...
static int rt1711_tcpc_init(struct tcpc_device *tcpc, bool sw_reset)
{
	...
	if (chip_id_is_husb311_did()) {
		rt1711_i2c_write16(tcpc,
			RT1711H_REG_DRP_DUTY_CTRL, TCPC_NORMAL_RP_DUTY_HUSB311);
	} else {
		rt1711_i2c_write16(tcpc,
...
static int rt1711_tcpc_deinit(struct tcpc_device *tcpc)
{
	...
	if (chip_id_is_et7303_did()) {
		/*
		 * The 200 ms delay is added to ensure that
		 * the CC_OPNE status is not filtered by the charger.
		 */
		usleep_range(200000, 200000);
		rt1711_set_cc(tcpc, TYPEC_CC_RD);
		rt1711_i2c_write8(tcpc, TCPC_V10_REG_ALERT, 0x01);
		rt1711_i2c_write8(tcpc, RT1711H_REG_BMC_CTRL, 0x0e);
	}
#endif /* CONFIG_TCPC_RT1715 */
	...
	enum tcpm_transmit_type type, uint16_t header, const uint32_t *data)
{
	...
	if (type < TCPC_TX_HARD_RESET) {
		...
		if (data_cnt > 0) {
			rc = memcpy_s(packet.data, sizeof(uint32_t) * TCPC_TRANSMIT_PACKET_MAX, (uint8_t *) data, data_cnt);
			if (rc != EOK)
