				  ...
				  struct device_attribute *attr, char *buf)
{
	...
	case TCPC_DESC_CAP_INFO:
		...
		(void)snprintf_s(buf+strlen(buf), CHAR_BUF_MAX, 256, "selected_cap = %d\n", pe_data->selected_cap);
		(void)snprintf_s(buf+strlen(buf), CHAR_BUF_MAX, 256, "%s\n",
		...
		for (i = 0; i < pd_port->local_src_cap.nr; i++) {
			tcpm_extract_power_cap_val(
				...
				&cap);
			(void)snprintf_s(buf+strlen(buf), CHAR_BUF_MAX, 256, "%d %d %d %d\n",
	...
	case TCPC_DESC_ROLE_DEF:
		(void)snprintf_s(buf, CHAR_BUF_MAX, 256, "%s\n", role_text[tcpc->desc.role_def]);
		break;
	...
	case TCPC_DESC_PD_TEST:
		...
		break;
	case TCPC_DESC_INFO:
		i += snprintf_s(buf + i, CHAR_BUF_MAX,
			256, "|^|==( %s info )==|^|\n", tcpc->desc.name);
		i += snprintf_s(buf + i, CHAR_BUF_MAX,
			256, "role = %s\n", role_text[tcpc->desc.role_def]);
		if (tcpc->typec_local_rp_level == TYPEC_RP_DFT)
			i += snprintf_s(buf + i, CHAR_BUF_MAX, 256,  "rplvl = %s\n", "Default");
		else if (tcpc->typec_local_rp_level == TYPEC_RP_1_5)
			i += snprintf_s(buf + i, CHAR_BUF_MAX, 256, "rplvl = %s\n", "1.5");
		else if (tcpc->typec_local_rp_level == TYPEC_RP_3_0)
			i += snprintf_s(buf + i, CHAR_BUF_MAX, 256, "rplvl = %s\n", "3.0");
	...
	case TCPC_TCPM_VBUS_LEVEL:
		ret = snprintf_s(buf, CHAR_BUF_MAX, 256, "%d\n", tcpm_inquire_vbus_level(tcpc, true));
		if (ret < 0)
			return ret;
		break;
	default:
				   ...
				   const char *buf, size_t count)
{
	...
	case TCPC_DESC_ROLE_DEF:
		...
		if (val >= TYPEC_ROLE_NR) {
			dev_err(dev, "role_def test start\n");
			tcpc->desc.role_def = TYPEC_ROLE_SNK;
			tcpc->test_role_def = true;
			val = TYPEC_ROLE_SNK;
		}
#endif
