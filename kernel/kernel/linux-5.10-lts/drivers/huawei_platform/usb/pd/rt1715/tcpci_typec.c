...
#ifdef CONFIG_TCPC_RT1715
		if (tcpc->typec_attach_new < ARRAY_SIZE(typec_attach_names))
...
int tcpc_typec_handle_timeout(struct tcpc_device *tcpc, uint32_t timer_id)
{
	...
	if (chip_id_is_et7303_did() && timer_id == TYPEC_TIMER_START_ID && 
		tcpc_is_timer_active(tcpc, TYPEC_TIMER_START_ID, PD_TIMER_NR)) {
		TYPEC_INFO("%s: timer_id=%d is active\n", __func__, timer_id);
		/* start timer is active, wait for 20-30ms and check again */
		usleep_range(20000, 30000);
	}

	if (timer_id >= TYPEC_TIMER_START_ID &&
	    tcpc_is_timer_active(tcpc, TYPEC_TIMER_START_ID, PD_TIMER_NR)) {
		TYPEC_INFO("[Type-C] Ignore timer_evt\n");
		return 0;
	}
...
#ifdef CONFIG_TCPC_RT1715
void tcpm_enable_9v2a(void *client)
{
	...
	TYPEC_INFO("up pd charge voltage\n");
	tcpm_dpm_pd_request(tcpc, VBUS_9000MV, IBUS_2000MA, NULL);
}

int tcpm_get_dp_state(void *client)
{
	struct tcpc_device *tcpc = (struct tcpc_device *)client;
...
static bool tcpc_inquire_usb_comm(void *client)
{
	struct tcpc_device *tcpc = (struct tcpc_device *)client;
 
	return tcpm_inquire_usb_comm(tcpc);
}

static int tcpm_pps_set_state(void *client, enum pps_set_state state, int mv, int ma)
{
	int ret = -EINVAL;
	struct tcpc_device *tcpc = (struct tcpc_device *)client;
	...
	if (state == PPS_SET_STATE_START)
		ret = tcpm_set_apdo_charging_policy_ex(tcpc, DPM_CHARGING_POLICY_PPS, mv, ma, NULL);
	else if (state == PPS_SET_STATE_STOP)
		ret = tcpm_reset_pd_charging_policy(tcpc, NULL);
	else if (state == PPS_SET_STATE_ADJUST)
		ret = tcpm_dpm_pd_request(tcpc, mv, ma, NULL);

	TYPEC_INFO("set pps=%d, mv=%d, ma=%d, ret=%d\n", (int)state, mv, ma, ret);
	if ((ret > 0) && (state == PPS_SET_STATE_ADJUST) && (ma == 0))
		ret = 0;

	return ret;
}

static int tcpm_pps_get_apdo_caps(void *client,
	struct pps_apdo_cap *caps, int num, struct pps_pdo_raw *raw)
{
	struct tcpm_power_cap_val cap_val = {0};
	struct tcpm_power_cap cap = {0};
	uint8_t cap_i = 0;
	int i;
	int ret = TCPM_SUCCESS;

	if ((num <= 0) || (num > PDO_MAX_NR) || !client || !caps || !raw)
		return -EINVAL;

	ret = tcpm_inquire_pd_source_cap((struct tcpc_device *)client, &cap);
	if (ret != TCPM_SUCCESS) {
		TYPEC_INFO("Get source cap for pps failed\n");
		return -EIO;
	}

	for (i = 0; i < PD_PDO_MAX_COUNT; i++) {
		if ((i < cap.cnt) && (i < PDO_MAX_NR))
			raw->data[i] = cap.pdos[i];
		else
			raw->data[i] = 0;
	}

	for (i = 0; i < num; i++) {
		ret = tcpm_inquire_pd_source_apdo_ex((struct tcpc_device *)client,
			DPM_APDO_TYPE_PPS, &cap_i, &cap_val);
		if (ret != TCPM_SUCCESS)
			break;

		caps[i].cur = cap_val.ma;
		caps[i].vol_max = cap_val.max_mv;
		caps[i].vol_min = cap_val.min_mv;
	}

	return i;
}

static int tcpm_pps_get_status(void *client, int *mv, int *ma)
{
	struct pd_pps_status pps_status = {0};
	int ret;

	if (!mv || !ma || !client)
		return -EINVAL;

	ret = tcpm_dpm_pd_get_pps_status_ex((struct tcpc_device *)client, NULL, &pps_status);
	TYPEC_INFO("pps status:mv=%d, ma=%d, flag=%d, ret=%d\n",
		pps_status.output_mv, pps_status.output_ma, pps_status.real_time_flags, ret);
...
static bool tcpc_pd_dpm_support_9V_src_cap(void *client)
{
	...
	return tcpci_support_9v_src_cap(&tcpc_dev->pd_port);
}
...
static void tcpc_pd_dpm_set_voltage(int set_voltage, void *client)
{
	...
	ret = tcpm_dpm_pd_request((struct tcpc_device *)client, set_voltage, IBUS_2000MA, NULL);
	if (ret != TCPM_SUCCESS) {
...
static int tcpm_get_src_cap(void *client)
{
	struct tcpc_device* tcpc_dev = ((struct tcpc_device*)client);
	...
	return tcpc_dev->pd_port.pe_data.remote_src_cap.nr;
}

static struct pd_dpm_ops tcpc_device_pd_dpm_ops = {
	.pd_dpm_notify_direct_charge_status = tcpm_set_direct_charge_enable,
	.pd_dpm_notify_cable_uvdm = tcpm_get_cable_uvdm,
	.pd_dpm_notify_9v2a_enable = tcpm_enable_9v2a,
	...
	.pd_dpm_inquire_usb_comm = tcpc_inquire_usb_comm,
	.pd_dpm_pps_set_state = tcpm_pps_set_state,
	.pd_dpm_pps_get_apdo_caps = tcpm_pps_get_apdo_caps,
	.pd_dpm_pps_get_status = tcpm_pps_get_status,
	.pd_dpm_set_cc_mode = tcpm_set_cc_mode,
	.pd_dpm_get_dp_state = tcpm_get_dp_state,
	.pd_dpm_get_src_cap = tcpm_get_src_cap,
};

 static struct hwpd_ops tcpc_device_pd_protocol_ops = {
	.chip_name = "rt1715",
	.support_9v_src_cap = tcpc_pd_dpm_support_9V_src_cap,
