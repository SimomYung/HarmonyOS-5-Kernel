...
int tcpm_inquire_cable_inform(struct tcpc_device *tcpc, uint32_t *vdos)
{
	...
	mutex_lock(&pd_port->pd_lock);
	if (pd_port->pe_data.cable_discovered_state) {
		rc = memcpy_s(vdos, sizeof(uint32_t) * VDO_MAX_NR, pd_port->pe_data.cable_vdos,
...
int tcpm_inquire_pd_partner_inform(struct tcpc_device *tcpc, uint32_t *vdos)
{
	...
	if (pd_port->pe_data.partner_id_present) {
		rc = memcpy_s(vdos, sizeof(uint32_t) * VDO_MAX_NR, pd_port->pe_data.partner_vdos,
			sizeof(uint32_t) * VDO_MAX_NR);
		if (rc != EOK)
			pr_err("%s: memcpy_s fail = %d\n", __func__, rc);
	} else
	...
	struct tcpc_device *tcpc, struct tcpm_svid_list *list)
{
	...
	if (svdm_list->cnt) {
		...
		rc = memcpy_s(list->svids, sizeof(uint16_t) * VDO_MAX_SVID_NR, svdm_list->svids,
	...
	struct tcpc_device *tcpc, uint16_t svid, struct tcpm_mode_list *list)
{
	...
	if (svid_data->remote_mode.mode_cnt) {
		...
		rc = memcpy_s(list->modes, sizeof(uint32_t) * VDO_MAX_NR, svid_data->remote_mode.mode_vdo,
	...
	struct tcpc_device *tcpc, struct tcpm_power_cap *cap)
{
	...
	if (pd_port->pe_data.remote_src_cap.nr) {
		...
		rc = memcpy_s(cap->pdos, sizeof(uint32_t) * PDO_MAX_NR, pd_port->pe_data.remote_src_cap.pdos,
	...
	struct tcpc_device *tcpc, struct tcpm_power_cap *cap)
{
	...
	if (pd_port->pe_data.remote_snk_cap.nr) {
		...
		rc = memcpy_s(cap->pdos, sizeof(uint32_t) * PDO_MAX_NR, pd_port->pe_data.remote_snk_cap.pdos,
			sizeof(uint32_t) * cap->cnt);
		if (rc != EOK)
			pr_err("%s: memcpy_s fail = %d\n", __func__, rc);
	} else
	...
	struct tcpc_device *tcpc, struct tcpm_power_cap *cap)
{
	...
	rc = memcpy_s(cap->pdos, sizeof(uint32_t) * PDO_MAX_NR, pd_port->local_src_cap_default.pdos,
	       sizeof(uint32_t) * cap->cnt);
	if (rc != EOK)
		pr_err("%s: memcpy_s fail = %d\n", __func__, rc);
	mutex_unlock(&pd_port->pd_lock);
	...
	struct tcpc_device *tcpc, struct tcpm_power_cap *cap)
{
	...
	rc = memcpy_s(pd_port->local_src_cap_default.pdos, sizeof(uint32_t) * PDO_MAX_NR, cap->pdos,
	       sizeof(uint32_t) * cap->cnt);
	if (rc != EOK)
		pr_err("%s: memcpy_s fail = %d\n", __func__, rc);
	mutex_unlock(&pd_port->pd_lock);
	...
	const struct tcp_dpm_event_cb_data *cb_data)
{
	...
	rc = memcpy_s(&tcp_event.tcp_dpm_data.cvdm_data, sizeof(struct tcp_dpm_custom_vdm_data), cvdm_data,
		sizeof(*cvdm_data));
	if (rc != EOK)
		pr_err("%s: memcpy_s tcp_event.tcp_dpm_data.cvdm_data fail = %d\n", __func__, rc);
	...
	if (cb_data == NULL && ret == TCP_DPM_RET_SUCCESS &&
		cvdm_data->wait_resp) {
			...
			rc = memcpy_s(cvdm_data->data, sizeof(cvdm_data->data[0]) * PD_DATA_OBJ_SIZE, pd_port->cvdm_data,
				sizeof(cvdm_data->data[0]) * cvdm_data->cnt);
			if (rc != EOK)
				pr_err("%s: memcpy_s cvdm_data->data fail = %d\n", __func__, rc);
		mutex_unlock(&pd_port->pd_lock);
	}
	...
	struct pd_port *pd_port, struct tcp_dpm_event *event)
{
	...
	rc = memcpy_s(&pd_port->tcp_event, sizeof(struct tcp_dpm_event), event, sizeof(struct tcp_dpm_event));
	if (rc != EOK)
		pr_err("%s: memcpy_s fail = %d\n", __func__, rc);
	mutex_unlock(&pd_port->pd_lock);
}
...
#ifdef CONFIG_TCPC_RT1715
int tcpm_get_cable_uvdm(void *client, uint32_t *data, uint8_t cnt)
{
	int ret;
	int i;
	struct tcp_dpm_custom_vdm_data cvdm_data;
	...
	if (tcpm_inquire_usb_comm(tcpc)) {
		pr_err("%s fail tcpm_inquire_usb_comm", __func__);
		return PD_AUTH_COMM_ERR;
	}
	ret = tcpm_dpm_pd_data_swap(tcpc, PD_ROLE_DFP, NULL);
	pr_info("%s tcpm_dpm_pd_data_swap is %d", __func__, ret);
	if (ret != TCP_DPM_RET_SUCCESS && ret != TCP_DPM_RET_DENIED_SAME_ROLE) {
		pr_err("%s fail tcpm_dpm_pd_data_swap", __func__);
		return PD_AUTH_SWAP_ERR;
	}
 
	if (!chip_id_is_et7303_did()) {
		ret = tcpm_dpm_pd_vconn_swap(tcpc, PD_ROLE_VCONN_ON, NULL);
		if (ret != TCP_DPM_RET_SUCCESS && ret != TCP_DPM_RET_DENIED_SAME_ROLE) {
			pr_err("%s tcpm_dpm_pd_vconn_swap", __func__);
			return PD_AUTH_VCONN_ERR;
		}
	}
 
	cvdm_data.wait_resp = true;
	cvdm_data.cable = true;
	cvdm_data.cnt = cnt;
	for (i = 0; i < cvdm_data.cnt; i++)
		cvdm_data.data[i] = data[i];
	ret = tcpm_dpm_send_custom_vdm(tcpc, &cvdm_data, NULL);
	if (ret != TCP_DPM_RET_SUCCESS) {
	  pr_err("%s rt1715 is no tcpm_dpm_send_custom_vdm", __func__);
	  return PD_AUTH_SEND_ERR;
	}
	for (i = 0; i < cvdm_data.cnt; i++)
		pr_info("data[%d] = 0x%08X\n", i, cvdm_data.data[i]);
 
	dpm_uvdm_data.cnt = cvdm_data.cnt;
	for (i = 0; i < cvdm_data.cnt; i++)
		dpm_uvdm_data.msg[i] = cvdm_data.data[i];
	pd_dpm_send_cable_data(dpm_uvdm_data);
...
static inline void tcpm_dpm_bk_copy_data(struct pd_port *pd_port)
{
	...
	if (pd_port->tcpm_bk_cb_data != NULL) {
		rc = memcpy_s(pd_port->tcpm_bk_cb_data, size,
