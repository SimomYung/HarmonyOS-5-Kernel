...
static int rszn_uwb_pmureg_operate_by_byte(int pmuid, int regaddr, int byte)
{
	int value = 0;
...
static int rszn_uwb_ldo_enable1v2(rszn_uwb_power_supply uwbldo)
{
	int status = UWB_FAIL;
	pr_info("UWB: %s, in\n", __func__);
	/* 1v2 adjust */
	if (uwbldo.ldo25_1v25_supply) {
		status = regulator_set_voltage(uwbldo.ldo_reg25, 1250000, 1250000);
	} else {
...
int rszn_uwb_ldo_enable2v85(rszn_uwb_power_supply uwbldo)
{
	int status = UWB_FAIL;
	pr_info("UWB: %s, in\n", __func__);
	/* 2v85 adjust */
...
static int rszn_uwb_ldo_enable(rszn_uwb_power_supply uwbldo)
{
	int status = UWB_FAIL;

	/* 1v2 */
	status = rszn_uwb_ldo_enable1v2(uwbldo);
	if (status) {
		pr_err("UWB:%s, rszn_uwb_ldo_enable1v2 fail, error: 0x%x\n", __func__, status);
		return status;
	}

	/* 2v85 */
	status = rszn_uwb_ldo_enable2v85(uwbldo);
	if (status) {
		pr_err("UWB:%s, rszn_uwb_ldo_enable2v85 fail, error: 0x%x\n", __func__, status);
		return status;
	}
...
static int rszn_uwb_ldo_disable(rszn_uwb_power_supply uwbldo)
{
	int status = UWB_FAIL;
...
static int rszn_uwb_extra_clk32k_disable(rszn_uwb_power_supply uwbldo)
{
	pr_info("UWB: %s, in\n", __func__);
...
static int rszn_uwb_extra_clk38m4_disable(rszn_uwb_power_supply uwbldo)
{
	int ret = UWB_FAIL;
...
static int rszn_uwb_ldo_extra_clk_disable(rszn_uwb_power_supply uwbldo)
{
	int status = UWB_FAIL;
...
static int rszn_uwb_chip_poweron(struct rszn_data *spidev, rszn_uwb_power_supply uwbldo)
{
	int ret = UWB_FAIL;

	pr_info("UWB: %s, rszn in\n", __func__);

	if (uwbldo.ldo_reg25 == NULL  || uwbldo.ldo_reg54 == NULL) {
		pr_err("UWB:%s, input error\n", __func__);
		return UWB_FAIL;
	}
	if ((!uwbldo.ldo53_not_supply) && (uwbldo.ldo_reg53 == NULL)) {
		pr_err("UWB:%s, input error 2\n", __func__);
		return UWB_FAIL;
	}
...
static int rszn_uwb_chip_poweroff_clk_disable(struct rszn_data *spidev, rszn_uwb_power_supply uwbldo)
{
	int ret = UWB_FAIL;
...
int rszn_uwb_extra_clk_enable(rszn_uwb_power_supply uwbldo)
{
	int ret = UWB_FAIL;
	int value;
	...
	if (uwbldo.freq2div38m == 1) {
		ret = rszn_uwb_pmureg_operate_by_byte(PMU_HI_6421, g_uwb_38m_clk_freq_by_hi6421.addr,
			g_uwb_38m_clk_freq_by_hi6421.value);
		pr_info("UWB:%s, 78.6M need 2 div\n", __func__);
	}
	ret = rszn_uwb_pmureg_operate_by_byte(PMU_HI_6421, g_uwb_38m_clk_vpp_adj_by_hi6421.addr,
		g_uwb_38m_clk_vpp_adj_by_hi6421.value);
	if (ret != UWB_SUCC) {
...
static void rszn_uwb_gpio_init(struct rszn_data *spidev)
{
	pr_info("UWB: %s, in\n", __func__);
...
static void rszn_uwb_gpio_deinit(struct rszn_data *spidev)
{
	pr_info("UWB: %s, in\n", __func__);
...
int rszn_uwb_close_deinit(struct rszn_data *spidev, rszn_uwb_power_supply uwbldo)
{
	...
	if (!uwbldo.extra_clk_supply) {
		...
		ret = rszn_uwb_ldo_disable(uwbldo);
		if (ret != UWB_SUCC) {
			pr_err("UWB:%s, rszn_uwb_disable_enable fail, error: 0x%x\n", __func__, ret);
			return UWB_FAIL;
		}
	}
...
static int rszn_request_gpio_irq(struct rszn_data *rszn)
{
	int				 	status  = -1;
...
static int rszn_request_gpio_reset(struct rszn_data *rszn)
{
	int				 	status  = -1;
	struct spi_device   *spi	= rszn->spi;
...
static int rszn_request_gpio_wakeup(struct rszn_data *rszn)
{
	int				 	status  = -1;
	struct spi_device   *spi	= rszn->spi;
...
static int rszn_request_gpio_cs(struct rszn_data *rszn)
{
	int status = -1;
...
static irqreturn_t rszn_interrupt(int irq, void *dev_id)
{
