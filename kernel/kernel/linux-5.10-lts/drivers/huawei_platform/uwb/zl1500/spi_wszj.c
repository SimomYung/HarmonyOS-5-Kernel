...
uwb_power_supply g_uwb_pmu = {NULL, NULL, NULL, NULL, NULL, 0, 0, 0, 0};
...
static unsigned int g_rfpower_flag = 0;
...
static ssize_t wszj_sync(struct wszj_data *wszj, struct spi_message *message)
{
	int status;
...
static ssize_t wszj_sync_write(struct wszj_data *wszj, size_t len)
{
...
static ssize_t wszj_sync_read(struct wszj_data *wszj, size_t len)
{
	struct spi_transfer	t = {
...
static ssize_t wszj_read(struct file *filp, char __user *buf, size_t count, loff_t *f_pos)
{
...
static ssize_t wszj_write(struct file *filp, const char __user *buf, size_t count, loff_t *f_pos)
{
	struct wszj_data	*wszj;
	ssize_t			status;
	unsigned long		missing;
...
static int wszj_message_copy_buff(struct spi_transfer *k_tmp, struct spi_ioc_transfer *u_tmp,
...
static void wszj_message_copy_trans(struct wszj_data *wszj, struct spi_transfer *k_tmp,
...
#ifdef VERBOSE
static void wszj_message_verbose(struct wszj_data *wszj, struct spi_transfer *k_tmp)
{
...
static int wszj_message_copy_data(struct spi_transfer *k_xfers, struct spi_ioc_transfer *u_xfers,
...
static int wszj_message(struct wszj_data *wszj, struct spi_ioc_transfer *u_xfers, unsigned int n_xfers)
{
...
static struct spi_ioc_transfer *wszj_get_ioc_message(unsigned int cmd, struct spi_ioc_transfer __user *u_ioc,
	unsigned int *n_ioc)
{
	u32 tmp;
...
static int wszj_ioctl_rd_wait(struct wszj_data *wszj, unsigned long arg)
{
	int retval;
	int gpio_value = 0;
	unsigned long flags;
...
static int wszj_ioctl_wr_mode(struct wszj_data *wszj, struct spi_device *spi,
...
static int wszj_ioctl_wr_lsb_first(struct wszj_data *wszj, struct spi_device *spi, unsigned long arg)
{
	int retval;
	u32 tmp;
...
static int wszj_ioctl_wr_bits_per_word(struct wszj_data *wszj, struct spi_device *spi, unsigned long arg)
{
	int retval;
	u32 tmp;
...
static int wszj_ioctl_wr_max_speed(struct wszj_data *wszj, struct spi_device *spi, unsigned long arg)
{
	int retval;
	u32 tmp;
...
static int wszj_ioctl_wr_power(struct wszj_data *wszj, unsigned long arg)
{
	int retval;
	u32 tmp;
...
static int wszj_ioctl_wr_wait(struct wszj_data *wszj, unsigned long arg)
{
	int retval;
	u32 tmp;
	unsigned long flags;
...
static int wszj_ioctl_wr_reset(struct wszj_data *wszj, unsigned long arg)
{
	int retval;
	u32 tmp;
...
static int wszj_ioctl_wr_wakeup(struct wszj_data *wszj, unsigned long arg)
{
	int retval;
	u32 tmp;
...
static int wszj_ioctl_default(struct wszj_data *wszj, struct spi_device *spi,
				unsigned int cmd, unsigned long arg)
{
	int retval;
	unsigned int n_ioc;
...
static int wszj_ioctl_switch_extend(struct wszj_data *wszj, struct spi_device *spi,
				unsigned int cmd, unsigned long arg)
{
	int retval;
	switch (cmd) {
...
static int wszj_ioctl_switch(struct wszj_data *wszj, struct spi_device *spi,
				unsigned int cmd, unsigned long arg)
{
	int retval;
...
static int wszj_get_dts_config(const char *node_name, const char *prop_name, char *out_str, int out_str_len)
{
...
static ssize_t wszj_chip_type_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	return snprintf_s(buf, sizeof(wszj_chip_type), sizeof(wszj_chip_type) - 1, "%s", wszj_chip_type);
}

static ssize_t wszj_chip_type_store(struct device *dev, struct device_attribute *attr,
...
static void wszj_set_rf_power_flag(void)
{
	int ret = -1;
	ret = wszj_get_dts_config_u32("rfpower-supply", "rfpower-supply", &g_rfpower_flag);
	if (ret != 0) {
		g_rfpower_flag = 0;
		hwlog_err("%s: can't rfpower-supply\n", __func__);
	}
	hwlog_info("%s: rfpower-supply:%d\n", __func__, g_rfpower_flag);
	return;
}

static void wszj_set_38m4_clk_enable(void)
{
	int ret = -1;
	ret = wszj_get_dts_config_u32("38m4clk_enable_reg", "38m4clk_enable_reg", &g_clk_38m4_enable_reg);
	if (ret != 0) {
		g_clk_38m4_enable_reg = 0;
		hwlog_err("%s: can't read 38m4clk_enable_reg, use default reg.\n", __func__);
		return;
	}
	hwlog_info("%s: 38m4clk_enable_reg:%d\n", __func__, g_clk_38m4_enable_reg);
	return;
}
...
static int wszj_create_device_attr(struct wszj_data *wszj)
{
	...
	wszj_set_rf_power_flag();
	wszj_set_pmu_32k_clk_name();
	wszj_set_38m4_clk_enable();
...
static int uwb_power_manage(struct wszj_data *spidev, uwb_power_supply uwbldo, char sw)
{
	...
	uwbldo.gpio_enable_rfpower = g_rfpower_flag;
