...
static int uwb_ldo_enable(uwb_power_supply uwbldo)
{
	...
	/* 1v8 adjust */
	if (uwbldo.gpio_enable_rfpower != 0) {
...
static int uwb_chip_poweron(struct wszj_data *spidev, uwb_power_supply uwbldo)
{
	...
	if (uwbldo.ldo_reg25 == NULL || (uwbldo.ldo_reg3 == NULL && uwbldo.gpio_enable_rfpower != 0) ||
...
int uwb_extra_clk_enable(uwb_power_supply uwbldo)
{
	...
	if (uwbldo.freq2div38m == 1) {
		ret = uwb_pmureg_operate_by_byte(PMU_HI_6421, g_uwb_38m_clk_freq_by_hi6421.addr,
			g_uwb_38m_clk_freq_by_hi6421.value);
		hwlog_info("UWB:%s, 78.6M need 2 div\n", __func__);
	}
	ret = uwb_pmureg_operate_by_byte(PMU_HI_6421, g_uwb_38m_clk_vpp_adj_by_hi6421.addr,
