...
static int aw869xx_ram_work_init(struct aw869xx *aw869xx)
{
	...
	if (vibrator_get_enter_recovery_mode()) {
		pr_info("recovery mode do not need load firmware");
		return 0;
	}
...
static void aw869xx_set_100_preset_long(struct aw869xx *aw869xx, uint64_t type)
{
	aw869xx->effect_mode = LONG_VIB_RAM_MODE;
	aw869xx->index = AW869XX_LONG_VIB_EFFECT_ID;
	aw869xx->duration = (type - FINE_GRAINED_LONG_BASE) / FINE_GRAINED_AMP_DIV_COFF;
	...
	pr_info("long index = %d, amp = %d, dura = %d", aw869xx->index, aw869xx->amplitude, aw869xx->duration);
	schedule_work(&aw869xx->long_vibrate_work);
}
...
static void aw869xx_play_effect_id(struct aw869xx *aw869xx, uint64_t type)
{
	...
	if (type >= FINE_GRAINED_LONG_BASE) {
		aw869xx_set_100_preset_long(aw869xx, type);
		...
		aw869xx->state = 1;
		aw869xx_fine_grained_adapt_amp_gain(aw869xx);
		queue_work(system_highpri_wq, &aw869xx->long_vibrate_work);
		...
		len += snprintf(aw869xx_rtp_idx_name, AW869XX_RTP_NAME_MAX - 1,
			"pre_170_%d.bin", aw869xx->rtp_idx);
...
static void richtap_update_fifo_data(struct aw869xx *aw_haptic, uint32_t fifo_len)
{
	...
	__pm_wakeup_event(lra_lock, (pos + RTP_AEI_LENGTH) / RTP_LEN_TO_PLAY_TIME_COFF);
...
// 20K 频率播放
	...
	__pm_wakeup_event(lra_lock, (tmp_len + RTP_AEI_LENGTH) / RTP_LEN_TO_PLAY_TIME_COFF);
