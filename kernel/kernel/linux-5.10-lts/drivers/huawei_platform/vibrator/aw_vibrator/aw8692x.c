...
static uint16_t aw_i2c_err_dmd_cnt = 0;
static uint8_t aw_dmd_report_cnt = 0;

static void aw_i2c_err_dmd(struct aw_haptic *aw_haptic)
{
#if defined(CONFIG_HUAWEI_OHOS_DSM) || defined(CONFIG_HUAWEI_DSM)
	if (aw_haptic->probe_succ) {
		aw_i2c_err_dmd_cnt++;
		if (aw_i2c_err_dmd_cnt > 1 && aw_i2c_err_dmd_cnt < I2C_ERR_DMD_PRINT_CNT_MAX)
			return;
		if (aw_i2c_err_dmd_cnt == I2C_ERR_DMD_PRINT_CNT_MAX)
			aw_i2c_err_dmd_cnt = 0;
		aw_dmd_report_cnt++;
		if (aw_dmd_report_cnt > AW_DMD_REPORT_MAX_CNT) {
			aw_dmd_report_cnt = AW_DMD_REPORT_MAX_CNT;
			return;
		}
		aw_vibrator_dmd_work(DSM_VIBRATOR_ABNORMAL_ERROR);
	}
#endif
}
...
static void aw8692x_set_bst_vol(struct aw_haptic *aw_haptic, uint8_t bst_vol)
{
	...
	aw_info("bst_vol = 0x%02x", bst_vol);
}
...
static void aw8692x_play_mode(struct aw_haptic *aw_haptic, uint8_t play_mode)
{
	...
	case AW_RAM_MODE:
		...
		if (aw_haptic->index >= HAPTIC_RAM_BRK_DISABLE_ID_1 || aw_haptic->is_charging_status)
			hv_i2c_w_bits(aw_haptic, AW8692X_REG_PLAYCFG3,
...
static int aw8692x_get_f0(struct aw_haptic *aw_haptic)
{
	...
	reg_array[0] = aw_haptic->info.cont_drv2_lvl;
	reg_array[1] = aw_haptic->info.cont_drv1_time;
	reg_array[2] = aw_haptic->info.cont_drv2_time;
...
static void aw8692x_short_ram_brk_cfg_set(struct aw_haptic *aw_haptic)
{
	if (!aw_haptic) {
	...
	hv_i2c_w_bytes(aw_haptic, AW8692X_REG_CONTCFG10, &aw_haptic->info.cont_brk_time, AW_I2C_BYTE_ONE);
	hv_i2c_w_bits(aw_haptic, AW8692X_REG_DETCFG2, AW8692X_BIT_DETCFG2_D2S_GAIN_MASK, aw_haptic->info.d2s_gain);
	hv_i2c_w_bits(aw_haptic, AW8692X_REG_CONTCFG5, AW8692X_BIT_CONTCFG5_BST_BRK_GAIN_MASK &
		AW8692X_BIT_CONTCFG5_BRK_GAIN_MASK, (aw_haptic->info.cont_bst_brk_gain << 4) | aw_haptic->info.cont_brk_gain);
}

static void aw8692x_long_ram_brk_cfg_set(struct aw_haptic *aw_haptic)
{
	if (!aw_haptic) {
		aw_err("aw_haptic or np is null");
		return;
	...
	hv_i2c_w_bytes(aw_haptic, AW8692X_REG_CONTCFG10, &aw_haptic->info.long_cont_brk_time, AW_I2C_BYTE_ONE);
	hv_i2c_w_bits(aw_haptic, AW8692X_REG_DETCFG2, AW8692X_BIT_DETCFG2_D2S_GAIN_MASK, aw_haptic->info.long_d2s_gain);
	hv_i2c_w_bits(aw_haptic, AW8692X_REG_CONTCFG5, AW8692X_BIT_CONTCFG5_BST_BRK_GAIN_MASK &
		AW8692X_BIT_CONTCFG5_BRK_GAIN_MASK, (aw_haptic->info.cont_bst_brk_gain << 4) |
		aw_haptic->info.long_cont_brk_gain);
}

static void aw8692x_ram_brk_cfg(struct device *dev, struct aw_haptic *aw_haptic,
							struct device_node *np)
{
	uint32_t val;

	aw_haptic->info.support_long_brk_cfg = of_property_read_bool(np, "aw8692x_support_long_brk_cfg");
	aw_info("aw_haptic->info.aw8692x_support_long_brk_cfg = %u", aw_haptic->info.support_long_brk_cfg);
	...
	val = of_property_read_u8(np, "aw8692x_long_cont_brk_time", &aw_haptic->info.long_cont_brk_time);
	if (val != 0)
		aw_info("long_vib_cont_brk_time not found");
	val = of_property_read_u8(np, "aw8692x_long_d2s_gain", &aw_haptic->info.long_d2s_gain);
	if (val != 0)
		aw_info("long_vib_d2s_gain not found");
	val = of_property_read_u8(np, "aw8692x_long_cont_brk_gain", &aw_haptic->info.long_cont_brk_gain);
	if (val != 0)
		aw_info("long_vib_cont_brk_gain not found");
}
							...
							struct device_node *np)
{
	...
	val = of_property_read_u8(np, "aw8692x_max_bst_vol", &aw_haptic->info.max_bst_vol);
	if (val != 0)
	...
	val = of_property_read_u8(np, "f0_cali_percent", &aw_haptic->info.f0_cali_percent);
	if (val != 0)
	...
	val = of_property_read_u8(np, "aw8692x_cont_drv_width", &aw_haptic->info.cont_drv_width);
	if (val != 0)
	...
	val = of_property_read_u8_array(np, "aw8692x_duration_time", duration_time, ARRAY_SIZE(duration_time));
	if (val != 0)
		aw_info("aw8692x_duration_time not found");
	else
		memcpy(aw_haptic->info.duration_time, duration_time, sizeof(duration_time));
	val = of_property_read_u8_array(np, "aw8692x_trig_config", trig_config_temp, ARRAY_SIZE(trig_config_temp));
	if (val != 0)
		aw_info("vib_trig_config not found");
	else
		memcpy(aw_haptic->info.trig_cfg, trig_config_temp, sizeof(trig_config_temp));
	val = of_property_read_u8(np, "aw8692x_bst_vol_default", &aw_haptic->info.bst_vol_default);
	if (val != 0)
		aw_info("vib_bst_vol_default not found");
	val = of_property_read_u8(np, "aw8692x_bst_vol_ram", &aw_haptic->info.bst_vol_ram);
	if (val != 0)
		aw_info("vib_bst_vol_ram not found");
	val = of_property_read_u8(np, "aw8692x_bst_vol_rtp", &aw_haptic->info.bst_vol_rtp);
	if (val != 0)
	...
	aw_haptic->info.is_enabled_lowpower_bst_config = of_property_read_bool(np, "aw8692x_vib_lowpower_bst_config");
	aw_info("aw_haptic->info.is_enabled_lowpower_bst_config = %d", aw_haptic->info.is_enabled_lowpower_bst_config);
	aw_haptic->info.is_enabled_richtap_core = of_property_read_bool(np, "aw8692x_is_enabled_richtap_core");
	aw_info("aw_haptic->info.aw8692x_is_enabled_richtap_core = %u", aw_haptic->info.is_enabled_richtap_core);
	aw_haptic->info.is_support_charging_notify = of_property_read_bool(np, "support_charging_notify");
	aw_info("aw_haptic->info.is_support_charging_notify = %u", aw_haptic->info.is_support_charging_notify);
	val = of_property_read_u8_array(np, "silicon_battery_cfg_dts",
		silicon_battery_cfg_temp, ARRAY_SIZE(silicon_battery_cfg_temp));
	if (val != 0)
		aw_info("silicon_battery_cfg_dts not found");
	else
		memcpy(aw_haptic->info.silicon_battery_cfg, silicon_battery_cfg_temp, sizeof(silicon_battery_cfg_temp));
	aw8692x_ram_brk_cfg(dev, aw_haptic, np);
}

static void bst_11v_config(struct aw_haptic *aw_haptic)
{
	uint8_t reg_val = 0;
	...
	reg_val = AW8692X_REG_PLAYCFG1_MAX_BOOST;
	hv_i2c_w_bytes(aw_haptic, AW8692X_REG_PLAYCFG1, &reg_val, AW_I2C_BYTE_ONE);
	reg_val = AW8692X_TMCFG_TRIM_REDUCE_VALUE;
	hv_i2c_w_bytes(aw_haptic, AW8692X_REG_TMCFG, &reg_val, AW_I2C_BYTE_ONE);
	reg_val = AW8692X_ANACFG13_BST_BURST_IN_OUT_DELAY_4US;
	hv_i2c_w_bytes(aw_haptic, AW8692X_REG_ANACFG13, &reg_val, AW_I2C_BYTE_ONE);
	reg_val = AW8692X_ANACFG8_CP_LDO_5_3V_OFFSET_50MV;
	hv_i2c_w_bytes(aw_haptic, AW8692X_REG_ANACFG8, &reg_val, AW_I2C_BYTE_ONE);
	reg_val = AW8692X_TMCFG_TRIM_NONE_VALUE;
	hv_i2c_w_bytes(aw_haptic, AW8692X_REG_TMCFG, &reg_val, AW_I2C_BYTE_ONE);
}
...
static void aw8692x_misc_para_init(struct aw_haptic *aw_haptic)
{
	...
	if (aw_haptic->info.bst_vol_default == AW8692X_REG_PLAYCFG1_MAX_BOOST)
		bst_11v_config(aw_haptic);
	aw8692x_short_ram_brk_cfg_set(aw_haptic);
}
...
struct aw_haptic_func aw8692x_func_list = {
	...
	.short_ram_brk_cfg_set = aw8692x_short_ram_brk_cfg_set,
	.long_ram_brk_cfg_set = aw8692x_long_ram_brk_cfg_set,
};
