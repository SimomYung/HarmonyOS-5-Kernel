...
static int vibrator_notifier_call(struct notifier_block *nb, unsigned long event, void *data)
{
...
static void vibrator_notifier_call_work_handler(struct work_struct *work) {
...
static int vibrator_init(struct aw_haptic *aw_haptic)
{
	...
	INIT_WORK(&aw_haptic->notifier_call_work, vibrator_notifier_call_work_handler);
...
static int aw_i2c_probe(struct i2c_client *i2c, const struct i2c_device_id *id)
{
	...
	if (aw_haptic->boot_power_set == SUPPORT_BOOST_5V) {
		aw_haptic->nb.notifier_call = vibrator_notifier_call;
		ret = power_event_bnc_register(POWER_BNT_BYPASSBOOST, &aw_haptic->nb);
		if (ret)
			aw_err("%s: Failed to register notifier\n", __func__);
	}
