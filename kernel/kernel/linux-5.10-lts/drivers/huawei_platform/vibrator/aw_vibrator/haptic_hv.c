 /*
  * haptic_hv.c
  *
  * code for vibrator
  *
  * Copyright (c) 2020 Huawei Technologies Co., Ltd.
  *
  * This software is licensed under the terms of the GNU General Public
  * License version 2, as published by the Free Software Foundation, and
  * may be copied, distributed, and modified under those terms.
  *
  * This program is distributed in the hope that it will be useful,
  * but WITHOUT ANY WARRANTY; without even the implied warranty of
  * MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE. See the
  * GNU General Public License for more details.
  *
  */

#include <linux/module.h>
#include <linux/kernel.h>
#include <linux/i2c.h>
#include <linux/of_gpio.h>
#include <linux/delay.h>
#include <linux/device.h>
#include <linux/firmware.h>
#include <linux/slab.h>
#include <linux/version.h>
#include <linux/input.h>
#include <linux/interrupt.h>
#include <linux/debugfs.h>
#include <linux/miscdevice.h>
#include <linux/uaccess.h>
#include <linux/syscalls.h>
#include <linux/power_supply.h>
#include <linux/vmalloc.h>
#include <linux/pm_qos.h>
#include <sound/core.h>
#include <sound/pcm.h>
#include <sound/pcm_params.h>
#include <sound/control.h>
#include <sound/soc.h>
#include <securec.h>
#ifdef CONFIG_HIBERNATION
#include <linux/suspend.h>
#endif

#include "haptic_hv.h"
#include "haptic_hv_reg.h"
#include "aw8692x_reg.h"
#include "../vibrator_common/vibrator_common.h"
#include <chipset_common/hwpower/hardware_ic/boost_5v.h>
#include <chipset_common/hwpower/hardware_ic/buck_boost.h>
#include <chipset_common/hwpower/common_module/power_event_ne.h>

#ifdef AAC_RICHTAP_SUPPORT
#include <linux/mman.h>
#endif

#ifdef CONFIG_HUAWEI_POWER_EMBEDDED_ISOLATION
#include <linux/power_supply.h>
#include <chipset_common/hwpower/common_module/power_supply_interface.h>
#endif

#ifdef CONFIG_HUAWEI_DUBAI
#include <huawei_platform/log/hwlog_kernel.h>
#endif

#if defined(CONFIG_HUAWEI_OHOS_DSM) || defined(CONFIG_HUAWEI_DSM)
#include <dsm/dsm_pub.h>
#endif

#ifdef CONFIG_SYS_PM_ULSR
#include <platform_include/cee/linux/ulsr.h>
#endif

#define HAPTIC_HV_DRIVER_VERSION	"v0.5.1.0"

#ifdef CONFIG_HUAWEI_POWER_EMBEDDED_ISOLATION
static void aw_set_bst_vol(struct aw_haptic *aw_haptic);
#endif
static int ram_update(struct aw_haptic *aw_haptic);
static void check_chip_status(struct aw_haptic *aw_haptic);

char *aw8692x_ram_name = "aw8692x_ram.bin";
char aw_rtp_name[][AW_RTP_NAME_MAX] = {
	{"haptic_rtp_osc_24K_5s.bin"},
	{"haptic_rtp.bin"},
	{"haptic_rtp_lighthouse.bin"},
	{"haptic_rtp_silk.bin"},
};

static bool m_common_reset_gpio_reseted = false;
static bool is_charging_status_g = false;
static char aw_rtp_idx_name[AW_RTP_NAME_MAX] = {0};
// static struct pm_qos_request aw_pm_qos_req_vb;
struct aw_haptic_container *aw_rtp;
static struct aw_haptic *aw_haptic_file;
static struct aw_haptic *aw_haptic1_file;
static struct timespec64 pre_ts;
static int64_t gap_time_ms = 0;
static unsigned int bypassboost_vol = BYPASSBOOST_4500MV;

static const unsigned char long_amp_again_val[AW_LONG_MAX_AMP_CFG] =
	{ 0x80, 0x80, 0x70, 0x60, 0x50, 0x40, 0x30, 0x20, 0x10 };
static const unsigned char short_amp_again_val[AW_SHORT_MAX_AMP_CFG] =
	{ 0x80, 0x80, 0x66, 0x4c, 0x33, 0x19 };

#ifdef CONFIG_HISYSEVENT
struct error_no_map {
	int error_no;
	char *name;
	int running_test_switch;
};

static struct error_no_map aw_vibrator_error_no_map[] = {
	{ 926012015, "HAPTIC_VIBRATOR_ABNORMAL_ERROR", 1 },
	{ 926012016, "HAPTIC_VIBRATOR_OCP_ERROR", 1 },
	{ 926012017, "HAPTIC_VIBRATOR_UVP_ERROR", 1 },
	{ 926012018, "HAPTIC_VIBRATOR_POWER_DOWN_ERROR", 1 },
	{ 926012019, "HAPTIC_VIBRATOR_OTP_ERROR", 1 },
	{ 926012020, "HAPTIC_VIBRATOR_F0_ERROR", 1 },
};

int aw_vibrator_errorno_to_str(int error_no, char *str, int bufflen)
{
	size_t i;
	struct error_no_map aw_vibrator_error_no_map[] = {
		{ 926012015, "HAPTIC_VIBRATOR_ABNORMAL_ERROR" },
		{ 926012016, "HAPTIC_VIBRATOR_OCP_ERROR" },
		{ 926012017, "HAPTIC_VIBRATOR_UVP_ERROR" },
		{ 926012018, "HAPTIC_VIBRATOR_POWER_DOWN_ERROR" },
		{ 926012019, "HAPTIC_VIBRATOR_OTP_ERROR" },
		{ 926012020, "HAPTIC_VIBRATOR_F0_ERROR" },
	};
	size_t error_no_map_len = sizeof(aw_vibrator_error_no_map) / sizeof(aw_vibrator_error_no_map[0]);

    if (!str)
        return -1;

	for (i = 0; i < error_no_map_len; i++) {
		if (aw_vibrator_error_no_map[i].error_no == error_no) {
			if (strlen(aw_vibrator_error_no_map[i].name) >= bufflen) {
				aw_err("%s aw_vibrator_error_no_map name too long\n", __func__);
				return -1;
			}
			if (strncpy_s(str, bufflen, aw_vibrator_error_no_map[i].name, strlen(aw_vibrator_error_no_map[i].name)) != 0) {
				aw_err("%s aw_vibrator_error_no_map strncpy fail\n", __func__);
				return -1;
			}
			str[bufflen - 1] = '\0';
			aw_info("%s aw_vibrator_error_no_map:%s\n", __func__, str);
			return 0;
		}
	}
	return -1;
}

static int vib_errorno_get_running_test_switch(int error_no)
{
	size_t i;
	size_t aw_vibrator_error_no_map_len = sizeof(aw_vibrator_error_no_map) / sizeof(aw_vibrator_error_no_map[0]);

	for (i = 0; i < aw_vibrator_error_no_map_len; i++) {
		if (aw_vibrator_error_no_map[i].error_no == error_no)
			return aw_vibrator_error_no_map[i].running_test_switch;
	}
	return 0;
}
#endif

#if defined(CONFIG_HUAWEI_OHOS_DSM) || defined(CONFIG_HUAWEI_DSM)
struct dsm_client_ops aw_vibrator_ops = {
	.poll_state = NULL,
	.dump_func = NULL,
#ifdef CONFIG_HISYSEVENT
	.errorno_to_str = aw_vibrator_errorno_to_str,
	.errorno_get_running_test_switch = vib_errorno_get_running_test_switch,
#endif
};
#endif

#if defined(CONFIG_HUAWEI_OHOS_DSM) || defined(CONFIG_HUAWEI_DSM)
static struct dsm_dev dsm_vibrator = {
#ifdef CONFIG_HISYSEVENT
	.name = "VIBRATOR",
#else
	.name = "dsm_vibrator",
#endif
	.device_name = "aw86927",
	.ic_name = NULL,
	.module_name = NULL,
	.fops = &aw_vibrator_ops,
	.buff_size = 1024, // vibrator dsm buf size
};
static struct dsm_client *vib_dclient;
#endif

void aw_vibrator_dmd_work(uint8_t num)
{
#if defined(CONFIG_HUAWEI_OHOS_DSM) || defined(CONFIG_HUAWEI_DSM)
	if (dsm_client_ocuppy(vib_dclient))
		return;
	switch (num) {
	case DSM_VIBRATOR_ABNORMAL_ERROR:
		dsm_client_record(vib_dclient, "LRA driver i2c transfer fail\n");
		dsm_client_notify(vib_dclient, DSM_SHB_VIBRATOR_ABNORMAL_ERROR);
		aw_err("%s LRA driver i2c transfer fail\n", __func__);
		break;
	case DSM_VIBRATOR_OCP_ERROR:
		dsm_client_record(vib_dclient, "LRA driver OCP\n");
		dsm_client_notify(vib_dclient, DSM_SHB_VIBRATOR_OCP_ERROR);
		aw_err("%s LRA driver OCP\n", __func__);
		break;
	case DSM_VIBRATOR_UVP_ERROR:
		dsm_client_record(vib_dclient, "LRA driver UVP\n");
		dsm_client_notify(vib_dclient, DSM_SHB_VIBRATOR_UVP_ERROR);
		aw_err("%s LRA driver UVP\n", __func__);
		break;
	case DSM_VIBRATOR_POWER_DONE:
		dsm_client_record(vib_dclient, "LRA driver power done\n");
		dsm_client_notify(vib_dclient, DSM_SHB_VIBRATOR_OVP_ERROR);
		aw_err("%s LRA driver power done\n", __func__);
		break;
	case DSM_VIBRATOR_OTP_ERROR:
		dsm_client_record(vib_dclient, "LRA driver OTP\n");
		dsm_client_notify(vib_dclient, DSM_SHB_VIBRATOR_OTP_ERROR);
		aw_err("%s LRA driver OTP\n", __func__);
		break;
	case DSM_VIBRATOR_F0_ERROR:
		dsm_client_record(vib_dclient, "LRA driver F0 error\n");
		dsm_client_notify(vib_dclient, DSM_SHB_VIBRATOR_F0_ERROR);
		aw_err("%s LRA driver F0 error\n", __func__);
		break;
	default:
		break;
	}
#endif
}

/*********************************************************
 *
 * I2C Read/Write
 *
 *********************************************************/
int i2c_r_bytes(struct aw_haptic *aw_haptic, uint8_t reg_addr, uint8_t *buf,
		uint32_t len)
{
	int ret;

	if (!aw_haptic) {
		aw_err ("aw_haptic is null");
		return -EINVAL;
	}
	struct i2c_msg msg[] = {
		[0] = {
			.addr = aw_haptic->i2c->addr,
			.flags = 0,
			.len = sizeof(uint8_t),
			.buf = &reg_addr,
			},
		[1] = {
			.addr = aw_haptic->i2c->addr,
			.flags = I2C_M_RD,
			.len = len,
			.buf = buf,
			},
	};

	ret = i2c_transfer(aw_haptic->i2c->adapter, msg, ARRAY_SIZE(msg));
	if (ret < 0) {
		aw_err("transfer failed.");
		return ret;
	} else if (ret != AW_I2C_READ_MSG_NUM) {
		aw_err("transfer failed(size error).");
		return -ENXIO;
	}
	return ret;
}
EXPORT_SYMBOL(i2c_r_bytes);

int i2c_w_bytes(struct aw_haptic *aw_haptic, uint8_t reg_addr, uint8_t *buf,
		uint32_t len)
{
	uint8_t *data = NULL;
	int ret = -1;

	if (!aw_haptic || !buf) {
		aw_err ("aw_haptic or buf is null");
		return -EINVAL;
	}
	data = kmalloc(len + 1, GFP_KERNEL);
	if (!data)
		return -ENOMEM; // Handling Memory Allocation Failures

	data[0] = reg_addr;
	memcpy(&data[1], buf, len);

	if (!aw_haptic->i2c) {
		aw_err ("i2c is null");
		kfree(data);
		return -EINVAL;
	}
	ret = i2c_master_send(aw_haptic->i2c, data, len + 1);
	if (ret < 0)
		aw_err("i2c master send 0x%02x err", reg_addr);
	kfree(data);
	return ret;
}
EXPORT_SYMBOL(i2c_w_bytes);

int i2c_w_bits(struct aw_haptic *aw_haptic, uint8_t reg_addr, uint32_t mask,
			uint8_t reg_data)
{
	uint8_t reg_val = 0;
	int ret = -1;

	if (!aw_haptic) {
		aw_err ("aw_haptic is null");
		return -EINVAL;
	}
	ret = i2c_r_bytes(aw_haptic, reg_addr, &reg_val, AW_I2C_BYTE_ONE);
	if (ret < 0) {
		aw_err("i2c read error, ret=%d", ret);
		return ret;
	}
	reg_val &= mask;
	reg_val |= reg_data;
	ret = i2c_w_bytes(aw_haptic, reg_addr, &reg_val, AW_I2C_BYTE_ONE);
	if (ret < 0) {
		aw_err("i2c write error, ret=%d", ret);
		return ret;
	}
	return 0;
}
EXPORT_SYMBOL(i2c_w_bits);

static int parse_trig_dt_config( struct aw_haptic *aw_haptic, struct device_node *np)
{
	for (int i = 0; i < AW_TRIG_NUM; i++)
		aw_haptic->trig_gpios[i] = -1;

	if (of_property_read_u32(np, "trig_enable", &aw_haptic->trig_enable)) {
		aw_info("aw_haptic->trig_enable = %d", aw_haptic->trig_enable);
		return 0;
	}
	aw_haptic->trig_gpios[TRIG_MODE0] = of_get_named_gpio(np, "trig_gpio0", 0);
	if (aw_haptic->trig_gpios[TRIG_MODE0] < 0) {
		aw_err("no trig_gpio0 provide");
		return -EINVAL;
	}
	aw_haptic->trig_gpios[TRIG_MODE1] = of_get_named_gpio(np, "trig_gpio1", 0);
	if (aw_haptic->trig_gpios[TRIG_MODE1] < 0) {
		aw_err("no trig_gpio1 provide");
		return -EINVAL;
	}

	aw_haptic->trig_gpios[TRIG_MODE2] = of_get_named_gpio(np, "trig_gpio2", 0);
	if (aw_haptic->trig_gpios[TRIG_MODE2] < 0) {
		aw_err("no trig_gpio2 provide");
		return -EINVAL;
	}
	return 0;
}

static int parse_sub_haptic_dt_config( struct aw_haptic *aw_haptic, struct device_node *np)
{
	int val = 0;

	if (!aw_haptic || !np) {
		aw_err ("aw_haptic or np is null");
		return -EINVAL;
	}
	if (of_property_read_u32(np, "common-reset-pin", &aw_haptic->common_reset_pin))
		aw_info("aw_haptic->common-reset-pin not found");
	else
		aw_info("aw_haptic->common_reset_pin = %d", aw_haptic->common_reset_pin);

	if (of_device_is_compatible(np, "awinic,haptic1_hv")) {
		aw_info("compatible haptic1_hv.");
		val = memcpy_s(aw_haptic->name, sizeof(aw_haptic->name), "vibrator1", sizeof("vibrator1"));
		if (val != 0) {
			aw_info("memcpy_s copy failed.");
			return -EPERM;
		}
	}
	return 0;
}

static int parse_dt_gpio(struct device *dev, struct aw_haptic *aw_haptic,
						struct device_node *np)
{
	int val = 0;

	if (!aw_haptic || !np) {
		aw_err ("aw_haptic or np is null");
		return -EINVAL;
	}
	// hardware or software power enable contrl
	val = of_property_read_u32(np, "boost_fw", &aw_haptic->info.boost_fw);
	if (val != 0) {
		aw_info("boost_fw not found");
	}
	// hardware enable when boost_fw is ture
	if (aw_haptic->info.boost_fw) {
		aw_info("boost_en gpio software control %d", aw_haptic->info.boost_en);
		aw_haptic->info.boost_en = of_get_named_gpio(np, "boost_en", 0);
		if (aw_haptic->info.boost_en >= 0) {
			aw_info("boost en-%d ok\n", aw_haptic->info.boost_en);
		} else {
			aw_err("get boost en gpio fail\n");
		}
		aw_info("boost_en gpio provide ok %d", aw_haptic->info.boost_en);
	} else {
		aw_info("boost_en gpio hardware control %d", aw_haptic->info.boost_en);
	}
	aw_haptic->reset_gpio = of_get_named_gpio(np, "reset-gpio", 0);
	if (aw_haptic->reset_gpio < 0) {
		aw_err("no reset gpio provide");
		return -EPERM;
	}
	aw_info("reset gpio provide ok %d", aw_haptic->reset_gpio);
	aw_haptic->irq_gpio = of_get_named_gpio(np, "irq-gpio", 0);
	if (aw_haptic->irq_gpio < 0)
		aw_err("no irq gpio provided.");
	else
		aw_info("irq gpio provide ok irq = %d.", aw_haptic->irq_gpio);

	val = of_property_read_u8(np, "mode", &aw_haptic->info.mode);
	if (val != 0)
		aw_info("mode not found");

	val = of_property_read_u32(np, "boost_power_on", &aw_haptic->boot_power_set);
	if (val != 0)
		aw_info("aw_haptic->boot_power_set not found");

	// boost enable or unable contrl
	val = of_property_read_u32(np, "bst_enable", &aw_haptic->bst_enable);
	if (val != 0)
		aw_info("bst_enable not found");
	aw_info("aw_haptic->bst_enable = %d", aw_haptic->bst_enable);

	// i2c addr update
	val = of_property_read_u32(np, "i2c_real_addr", &aw_haptic->real_i2c_addr);
	if (val != 0)
		aw_info("i2c_real_addr is not found");
	if (aw_haptic->real_i2c_addr != 0) {
		aw_info("i2c_real_addr is %d", aw_haptic->real_i2c_addr);
		aw_haptic->i2c->addr = (uint16_t) aw_haptic->real_i2c_addr;
	}
	return 0;
}

static void hw_reset(struct aw_haptic *aw_haptic)
{
	aw_info("enter");
	if (!aw_haptic) {
		aw_err ("aw_haptic is null");
		return;
	}

	if (aw_haptic->common_reset_pin && m_common_reset_gpio_reseted)
		return;

	if (aw_haptic && gpio_is_valid(aw_haptic->reset_gpio)) {
		gpio_set_value_cansleep(aw_haptic->reset_gpio, 0);
		usleep_range(1000, 2000);
		gpio_set_value_cansleep(aw_haptic->reset_gpio, 1);
		usleep_range(8000, 8500);
		m_common_reset_gpio_reseted = true;
	} else {
		aw_err("failed");
	}
}

static void sw_reset(struct aw_haptic *aw_haptic)
{
	uint8_t reset = AW_BIT_RESET;

	if (!aw_haptic) {
		aw_err ("aw_haptic is null");
		return;
	}
	aw_info("enter");
	i2c_w_bytes(aw_haptic, AW_REG_CHIPID, &reset, AW_I2C_BYTE_ONE);
	usleep_range(3000, 3500);
}

static int judge_value(uint8_t reg)
{
	int ret = 0;

	if (!reg)
		return -ERANGE;
	switch (reg) {
	case AW86925_BIT_RSTCFG_PRE_VAL:
	case AW86926_BIT_RSTCFG_PRE_VAL:
	case AW86927_BIT_RSTCFG_PRE_VAL:
	case AW86928_BIT_RSTCFG_PRE_VAL:
	case AW86925_BIT_RSTCFG_VAL:
	case AW86926_BIT_RSTCFG_VAL:
	case AW86927_BIT_RSTCFG_VAL:
	case AW86928_BIT_RSTCFG_VAL:
		ret = -ERANGE;
		break;
	default:
		break;
	}
	return ret;
}

static int read_chipid(struct aw_haptic *aw_haptic, uint32_t *reg_val)
{
	uint8_t value[2] = {0};
	int ret = 0;

	if (!aw_haptic || !reg_val) {
		aw_err ("aw_haptic or reg_val is null");
		return -EINVAL;
	}
	/* try the old way of read chip id */
	ret = i2c_r_bytes(aw_haptic, AW_REG_CHIPID, &value[0], AW_I2C_BYTE_ONE);
	if (ret < 0) {
		aw_info ("old way fail");
		return ret;
	}

	ret = judge_value(value[0]);
	if (!ret) {
		aw_info ("judge_value fail");
		*reg_val = value[0];
		return ret;
	}
	/* try the new way of read chip id */
	ret = i2c_r_bytes(aw_haptic, AW_REG_CHIPIDH, value, AW_I2C_BYTE_TWO);
	if (ret < 0) {
		aw_info ("new way fail");
		return ret;
	}
	*reg_val = value[0] << 8 | value[1];
	aw_info ("reg_val = 0x%02X", *reg_val);
	return ret;
}

static int parse_chipid(struct aw_haptic *aw_haptic)
{
	int ret = -1;
	uint32_t reg = 0;
	uint8_t cnt = 0;

	if (!aw_haptic) {
		aw_err ("aw_haptic is null");
		return -EINVAL;
	}
	for (cnt = 0; cnt < AW_READ_CHIPID_RETRIES; cnt++) {
		/* hardware reset */
		hw_reset(aw_haptic);
		ret = read_chipid(aw_haptic, &reg);
		aw_info("reg_val = 0x%02X", reg);
		if (ret < 0)
			aw_err("read chip id fail: %d", ret);
		switch (reg) {
		case AW8695_CHIPID:
			aw_haptic->chipid = AW8695_CHIPID;
			aw_haptic->bst_pc = AW_BST_PC_L1;
			aw_haptic->i2s_config = false;
			aw_info("detected aw8695.");
			return 0;
		case AW8697_CHIPID:
			aw_haptic->chipid = AW8697_CHIPID;
			aw_haptic->bst_pc = AW_BST_PC_L2;
			aw_haptic->i2s_config = false;
			aw_info("detected aw8697.");
			return 0;
#ifdef AW869XX_DRIVER_ENABLE
		case AW86905_CHIPID:
			aw_haptic->chipid = AW86905_CHIPID;
			aw_haptic->bst_pc = AW_BST_PC_L1;
			aw_haptic->i2s_config = false;
			aw_info("detected aw86905.");
			return 0;
		case AW86907_CHIPID:
			aw_haptic->chipid = AW86907_CHIPID;
			aw_haptic->bst_pc = AW_BST_PC_L2;
			aw_haptic->i2s_config = false;
			aw_info("detected aw86907.");
			return 0;
#endif
		case AW86915_CHIPID:
			aw_haptic->chipid = AW86915_CHIPID;
			aw_haptic->bst_pc = AW_BST_PC_L1;
			aw_haptic->i2s_config = true;
			aw_info("detected aw86915.");
			return 0;
		case AW86917_CHIPID:
			aw_haptic->chipid = AW86917_CHIPID;
			aw_haptic->bst_pc = AW_BST_PC_L2;
			aw_haptic->i2s_config = true;
			aw_info("detected aw86917.");
			return 0;
		case AW86715_CHIPID:
			aw_haptic->chipid = AW86715_CHIPID;
			aw_haptic->i2s_config = false;
			aw_info("detected aw86715.");
			return 0;
		case AW86716_CHIPID:
			aw_haptic->chipid = AW86716_CHIPID;
			aw_haptic->i2s_config = true;
			aw_info("detected aw86716.");
			return 0;
		case AW86717_CHIPID:
			aw_haptic->chipid = AW86717_CHIPID;
			aw_haptic->i2s_config = true;
			aw_info("detected aw86717.");
			return 0;
		case AW86718_CHIPID:
			aw_haptic->chipid = AW86718_CHIPID;
			aw_haptic->i2s_config = true;
			aw_info("detected aw86718.");
			return 0;
		case AW86925_CHIPID:
			aw_haptic->chipid = AW86925_CHIPID;
			aw_haptic->bst_pc = AW_BST_PC_L1;
			aw_info("detected aw86925.");
			return 0;
		case AW86926_CHIPID:
			aw_haptic->chipid = AW86926_CHIPID;
			aw_haptic->bst_pc = AW_BST_PC_L1;
			aw_info("detected aw86926.");
			return 0;
		case AW86927_CHIPID:
			aw_haptic->chipid = AW86927_CHIPID;
			aw_haptic->bst_pc = AW_BST_PC_L1;
			aw_info("detected aw86927.");
			return 0;
		case AW86928_CHIPID:
			aw_haptic->chipid = AW86928_CHIPID;
			aw_haptic->bst_pc = AW_BST_PC_L1;
			aw_info("detected aw86928.");
			return 0;
		default:
			aw_info("unsupport device revision (0x%02X)", reg);
			break;
		}
		usleep_range(AW_READ_CHIPID_RETRY_DELAY * 1000,
					AW_READ_CHIPID_RETRY_DELAY * 1000 + 500);
	}
	return -EINVAL;
}

static int ctrl_init(struct aw_haptic *aw_haptic, struct device *dev)
{
	if (aw_haptic->chipid <= 0) {
		aw_info("wrong chipid!");
		return -EINVAL;
	}
	switch (aw_haptic->chipid) {
#ifdef AW869X_DRIVER_ENABLE
	case AW8695_CHIPID:
	case AW8697_CHIPID:
		aw_haptic->func = aw869x_func();
		break;
#endif
#ifdef AW869XX_DRIVER_ENABLE
	case AW86905_CHIPID:
	case AW86907_CHIPID:
	case AW86915_CHIPID:
	case AW86917_CHIPID:
		aw_haptic->func = aw869xx_func();
		break;
#endif
#ifdef AW8671X_DRIVER_ENABLE
	case AW86715_CHIPID:
	case AW86716_CHIPID:
	case AW86717_CHIPID:
	case AW86718_CHIPID:
		aw_haptic->func = aw8671x_func();
		break;
#endif
#ifdef AW8692X_DRIVER_ENABLE
	case AW86925_CHIPID:
	case AW86926_CHIPID:
	case AW86927_CHIPID:
	case AW86928_CHIPID:
		aw_haptic->func = aw8692x_func();
		break;
#endif
	default:
		aw_info("unexpected chipid!");
		return -EINVAL;
	}
	return 0;
}

static void ram_play(struct aw_haptic *aw_haptic, uint8_t mode)
{
	if (!aw_haptic) {
		aw_err ("aw_haptic is null");
		return;
	}
	aw_haptic->func->play_mode(aw_haptic, mode);
	aw_haptic->func->play_go(aw_haptic, true);
}

static int get_ram_num(struct aw_haptic *aw_haptic)
{
	uint8_t wave_addr[2] = {0};
	uint32_t first_wave_addr = 0;

	if (!aw_haptic->ram_init) {
		aw_err("ram init faild, ram_num = 0!");
		return -EPERM;
	}
	mutex_lock(&aw_haptic->lock);
	/* RAMINIT Enable */
	aw_haptic->func->ram_init(aw_haptic, true);
	aw_haptic->func->play_stop(aw_haptic);
	aw_haptic->func->set_ram_addr(aw_haptic);
	aw_haptic->func->get_first_wave_addr(aw_haptic, wave_addr);
	first_wave_addr = (wave_addr[0] << 8 | wave_addr[1]);
	aw_haptic->ram.ram_num =
			(first_wave_addr - aw_haptic->ram.base_addr - 1) / 4;
	aw_info("first wave addr = 0x%04x", first_wave_addr);
	aw_info("ram_num = %d", aw_haptic->ram.ram_num);
	/* RAMINIT Disable */
	aw_haptic->func->ram_init(aw_haptic, false);
	mutex_unlock(&aw_haptic->lock);
	return 0;
}

static void ram_vbat_comp(struct aw_haptic *aw_haptic, bool flag)
{
	int temp_gain = 0;

	if (!aw_haptic) {
		aw_err ("aw_haptic is null");
		return;
	}
	if (flag) {
		if (aw_haptic->ram_vbat_comp == AW_RAM_VBAT_COMP_ENABLE) {
			aw_haptic->func->get_vbat(aw_haptic);
			temp_gain = aw_haptic->gain * AW_VBAT_REFER /
						aw_haptic->vbat;
			if (temp_gain > (128 * AW_VBAT_REFER / AW_VBAT_MIN)) {
				temp_gain = 128 * AW_VBAT_REFER / AW_VBAT_MIN;
				aw_dbg("gain limit=%d", temp_gain);
			}
			aw_haptic->func->set_gain(aw_haptic, temp_gain);
			aw_info("ram vbat comp open");
		} else {
			aw_haptic->func->set_gain(aw_haptic, aw_haptic->gain);
			aw_info("ram vbat comp close(AW_RAM_VBAT_COMP_ENABLE failed)");
		}
	} else {
		aw_haptic->func->set_gain(aw_haptic, aw_haptic->gain);
		aw_info("ram vbat comp close(flag false)");
	}
}

static int f0_cali(struct aw_haptic *aw_haptic)
{
	char f0_cali_lra = 0;
	uint32_t f0_limit = 0;
	uint32_t f0_cali_min = 0;
	uint32_t f0_cali_max = 0;
	int ret = 0;
	int f0_cali_step = 0;

	if (!aw_haptic) {
		aw_err ("aw_haptic is null");
		return -EINVAL;
	}
	f0_cali_min = aw_haptic->info.f0_pre * (100 - aw_haptic->info.f0_cali_percent) / 100;
	f0_cali_max = aw_haptic->info.f0_pre * (100 + aw_haptic->info.f0_cali_percent) / 100;
	aw_haptic->func->upload_lra(aw_haptic, AW_WRITE_ZERO);
	if (aw_haptic->func->get_f0(aw_haptic)) {
		aw_err("get f0 error, user defafult f0");
		aw_vibrator_dmd_work(DSM_VIBRATOR_F0_ERROR);
	} else {
		/* max and min limit */
		f0_limit = aw_haptic->f0;
		aw_info("f0_pre = %d, f0_cali_min = %d, f0_cali_max = %d, f0 = %d",
			aw_haptic->info.f0_pre,
			f0_cali_min, f0_cali_max, aw_haptic->f0);

		if ((aw_haptic->f0 < f0_cali_min) ||
			aw_haptic->f0 > f0_cali_max) {
			aw_err("f0 calibration out of range = %d!",
				aw_haptic->f0);
			f0_limit = aw_haptic->info.f0_pre;
			return -ERANGE;
		}
		aw_info("f0_limit = %d", (int)f0_limit);
		/* calculate cali step */
		f0_cali_step = 100000 * ((int)f0_limit -
					(int)aw_haptic->info.f0_pre) /
					((int)f0_limit * AW_OSC_CALI_ACCURACY);
		aw_info("f0_cali_step = %d", f0_cali_step);
		if (f0_cali_step >= 0) {
			if (f0_cali_step % 10 >= 5)
				f0_cali_step = 32 + (f0_cali_step / 10 + 1);
			else
				f0_cali_step = 32 + f0_cali_step / 10;
		} else {
			if (f0_cali_step % 10 <= -5)
				f0_cali_step = 32 + (f0_cali_step / 10 - 1);
			else
				f0_cali_step = 32 + f0_cali_step / 10;
		}
		if (f0_cali_step > 31)
			f0_cali_lra = (char)f0_cali_step - 32;
		else
			f0_cali_lra = (char)f0_cali_step + 32;
		/* update cali step */
		aw_haptic->f0_cali_data = (int)f0_cali_lra;

		aw_info("f0_cali_data = 0x%02X", aw_haptic->f0_cali_data);
	}
	aw_haptic->func->upload_lra(aw_haptic, AW_F0_CALI_LRA);
	/* restore standby work mode */
	aw_haptic->func->play_stop(aw_haptic);
	return ret;
}

static int rtp_osc_cali(struct aw_haptic *aw_haptic)
{
	uint32_t buf_len = 0;
	int ret = -1;
	const struct firmware *rtp_file;

	if (!aw_haptic) {
		aw_err ("aw_haptic is null");
		return -EINVAL;
	}
	aw_haptic->rtp_cnt = 0;
	aw_haptic->timeval_flags = 1;

	/* fw loaded */
	ret = request_firmware(&rtp_file, aw_rtp_name[0], aw_haptic->dev);
	if (ret < 0) {
		aw_err("failed to read %s", aw_rtp_name[0]);
		return ret;
	}
	/* aw_haptic add stop,for irq interrupt during calibrate */
	aw_haptic->func->play_stop(aw_haptic);
	aw_haptic->rtp_init = false;
	mutex_lock(&aw_haptic->rtp_lock);
	vfree(aw_rtp);
	aw_rtp = vmalloc(rtp_file->size + sizeof(int));
	if (!aw_rtp) {
		release_firmware(rtp_file);
		mutex_unlock(&aw_haptic->rtp_lock);
		aw_err("error allocating memory");
		return -ENOMEM;
	}
	aw_rtp->len = rtp_file->size;
	aw_haptic->rtp_len = rtp_file->size;
	aw_info("rtp file:[%s] size = %dbytes", aw_rtp_name[0], aw_rtp->len);
	memcpy(aw_rtp->data, rtp_file->data, rtp_file->size);
	release_firmware(rtp_file);
	mutex_unlock(&aw_haptic->rtp_lock);
	/* gain */
	ram_vbat_comp(aw_haptic, false);
	/* rtp mode config */
	aw_haptic->func->play_mode(aw_haptic, AW_RTP_MODE);
	/* bst mode */
	aw_haptic->func->bst_mode_config(aw_haptic, AW_BST_BYPASS_MODE);
	disable_irq(gpio_to_irq(aw_haptic->irq_gpio));
	/* haptic go */
	aw_haptic->func->play_go(aw_haptic, true);

	while (1) {
		if (!aw_haptic->func->rtp_get_fifo_afs(aw_haptic)) {
#ifdef AW_ENABLE_RTP_PRINT_LOG
			aw_info("not almost_full, aw_haptic->rtp_cnt=%d",
				aw_haptic->rtp_cnt);
#endif
			mutex_lock(&aw_haptic->rtp_lock);
			if ((aw_rtp->len - aw_haptic->rtp_cnt) <
				(aw_haptic->ram.base_addr >> 2))
				buf_len = aw_rtp->len - aw_haptic->rtp_cnt;
			else
				buf_len = (aw_haptic->ram.base_addr >> 2);
			if (aw_haptic->rtp_cnt != aw_rtp->len) {
				if (aw_haptic->timeval_flags == 1) {
					aw_haptic->kstart = ktime_get();
					aw_haptic->timeval_flags = 0;
				}
				aw_haptic->func->set_rtp_data(
						aw_haptic, &aw_rtp->data
						[aw_haptic->rtp_cnt], buf_len);
				aw_haptic->rtp_cnt += buf_len;
			}
			mutex_unlock(&aw_haptic->rtp_lock);
		}
		if (aw_haptic->func->get_osc_status(aw_haptic)) {
			aw_haptic->kend = ktime_get();
			aw_info("osc trim playback done aw_haptic->rtp_cnt= %d",
				aw_haptic->rtp_cnt);
			break;
		}
		aw_haptic->kend = ktime_get();
		aw_haptic->microsecond = ktime_to_us(ktime_sub(aw_haptic->kend,
								aw_haptic->kstart));
		if (aw_haptic->microsecond > AW_OSC_CALI_MAX_LENGTH) {
			aw_info("osc trim time out! aw_haptic->rtp_cnt %d",
				aw_haptic->rtp_cnt);
			break;
		}
	}
	enable_irq(gpio_to_irq(aw_haptic->irq_gpio));
	aw_haptic->microsecond = ktime_to_us(ktime_sub(aw_haptic->kend,
							aw_haptic->kstart));
	/* calibration osc */
	aw_info("microsecond: %llu", aw_haptic->microsecond);
	return 0;
}

static void rtp_trim_lra_cali(struct aw_haptic *aw_haptic)
{
#ifdef AW_OSC_MULTI_CALI
	uint8_t last_trim_code = 0;
	int temp = 0;
	int count = 5;
#endif
	uint32_t lra_trim_code = 0;
	/* 0.1 percent below no need to calibrate */
	uint32_t osc_cali_threshold = 10;
	uint32_t real_code = 0;
	uint32_t theory_time = 0;
	uint32_t real_time = 0;

	if (!aw_haptic) {
		aw_err ("aw_haptic is null");
		return;
	}
	aw_haptic->func->upload_lra(aw_haptic, AW_WRITE_ZERO);
#ifdef AW_OSC_MULTI_CALI
	while (count) {
#endif
	rtp_osc_cali(aw_haptic);
	real_time = aw_haptic->microsecond;
	theory_time = aw_haptic->func->get_theory_time(aw_haptic);
	if (theory_time == real_time) {
		aw_info("theory_time == real_time: %d, no need to calibrate!",
			real_time);
		return;
	} else if (theory_time < real_time) {
		if ((real_time - theory_time) >
			(theory_time / AW_OSC_TRIM_PARAM)) {
			aw_info("(real_time - theory_time) > (theory_time/50), can't calibrate!");
			return;
		}

		if ((real_time - theory_time) <
			(osc_cali_threshold * theory_time / 10000)) {
			aw_info("real_time: %d, theory_time: %d, no need to calibrate!",
				real_time, theory_time);
			return;
		}

		real_code = 100000 * ((real_time - theory_time)) /
					(theory_time * AW_OSC_CALI_ACCURACY);
		real_code = ((real_code % 10 < 5) ? 0 : 1) + real_code / 10;
		real_code = 32 + real_code;
	} else if (theory_time > real_time) {
		if ((theory_time - real_time) >
			(theory_time / AW_OSC_TRIM_PARAM)) {
			aw_info("(theory_time - real_time) > (theory_time / 50), can't calibrate!");
			return;
		}
		if ((theory_time - real_time) <
			(osc_cali_threshold * theory_time / 10000)) {
			aw_info("real_time: %d, theory_time: %d, no need to calibrate!",
				real_time, theory_time);
			return;
		}

		real_code = (theory_time - real_time) / (theory_time / 100000) / AW_OSC_CALI_ACCURACY;
		real_code = ((real_code % 10 < 5) ? 0 : 1) + real_code / 10;
		real_code = 32 - real_code;
	}
	if (real_code > 31)
		lra_trim_code = real_code - 32;
	else
		lra_trim_code = real_code + 32;
#ifdef AW_OSC_MULTI_CALI
	last_trim_code = aw_haptic->func->get_trim_lra(aw_haptic);
	if (last_trim_code) {
		if (lra_trim_code >= 32) {
			temp = last_trim_code - (64 - lra_trim_code);
			if (temp < 32 && temp > 0 && last_trim_code >= 32)
				temp = 32;
			if (temp < 0)
				temp = lra_trim_code + last_trim_code;
		} else if (lra_trim_code > 0 && lra_trim_code < 32) {
			temp = (last_trim_code + lra_trim_code) & 0x3f;
			if (temp >= 32 && last_trim_code <= 31)
				temp = 31;
			if ((64 - last_trim_code) <= lra_trim_code)
				temp = last_trim_code + lra_trim_code - 64;
		} else {
			temp = last_trim_code;
		}
		aw_haptic->osc_cali_data = temp;
	} else {
		aw_haptic->osc_cali_data = lra_trim_code;
	}
#else
	aw_haptic->osc_cali_data = lra_trim_code;
#endif
	aw_haptic->func->upload_lra(aw_haptic, AW_OSC_CALI_LRA);
	aw_info("real_time: %d, theory_time: %d", real_time, theory_time);
	aw_info("real_code: %d, trim_lra: 0x%02X", real_code, lra_trim_code);
#ifdef AW_OSC_MULTI_CALI
	count--;
	}
#endif
}

static int aw_file_open(struct inode *inode, struct file *file)
{
	if (!file)
		return 0;

	if (!try_module_get(THIS_MODULE))
		return -ENODEV;

	file->private_data = (void *)aw_haptic_file;

	return 0;
}

static int sub_aw_file_open(struct inode *inode, struct file *file)
{
	if (!file)
		return 0;

	if (!try_module_get(THIS_MODULE))
		return -ENODEV;

	file->private_data = (void *)aw_haptic1_file;

	return 0;
}
static int aw_file_release(struct inode *inode, struct file *file)
{
	if (!file)
		return 0;

	module_put(THIS_MODULE);
	return 0;
}

static long aw_file_unlocked_ioctl(struct file *file,
	unsigned int cmd, unsigned long arg)
{
	struct aw_haptic *aw_haptic = NULL;

	if (!file)
		return 0;

	aw_haptic = (struct aw_haptic *)file->private_data;

	if (!aw_haptic)
		return 0;
	aw_info("cmd=0x%x, arg=0x%lx\n", cmd, arg);

	mutex_lock(&aw_haptic->lock);
	if (_IOC_TYPE(cmd) != AW_HAPTIC_IOCTL_MAGIC) {
		aw_err("%s: cmd magic err\n", __func__);
		mutex_unlock(&aw_haptic->lock);
		return -EINVAL;
	}

	switch (cmd) {
	default:
		aw_err("%s, unknown cmd\n", __func__);
		break;
	}

	mutex_unlock(&aw_haptic->lock);

	return 0;
}

static ssize_t aw_file_read(struct file *filp,
	char *buff, size_t len, loff_t *offset)
{
	return len;
}

static void aw_adapt_amp_again(struct aw_haptic *aw_haptic, int haptic_type)
{
	if (!aw_haptic) {
		aw_err ("aw_haptic is null");
		return;
	}
	switch (haptic_type) {
	case LONG_VIB_RAM_MODE:
		if (aw_haptic->amplitude >= AW_LONG_MAX_AMP_CFG || aw_haptic->amplitude < 0)
			return;
		aw_haptic->gain = long_amp_again_val[aw_haptic->amplitude];
		aw_info("long gain = %u\n", aw_haptic->gain);
		break;
	case SHORT_VIB_RAM_MODE:
		if (aw_haptic->amplitude >= AW_SHORT_MAX_AMP_CFG || aw_haptic->amplitude < 0)
			return;
		aw_haptic->gain = short_amp_again_val[aw_haptic->amplitude];
		aw_info("short gain = %u\n", aw_haptic->gain);
		break;
	case RTP_VIB_MODE:
		aw_haptic->gain = 0x80; // define max amp
		break;
	default:
		break;
	}
}

static void aw_special_type_process(struct aw_haptic *aw_haptic,
	uint64_t type)
{
	switch (type) {
	case HAPTIC_CHARGING_CALIB_ID:
		aw_haptic->f0_is_cali = HAPTIC_CHARGING_IS_CALIB;
		schedule_work(&aw_haptic->vibrator_work);
		break;
	case HAPTIC_NULL_WAVEFORM_ID:
		aw_info("null wave id");
		break;
	case HAPTIC_TRIG_MODE1_ID:
	case HAPTIC_TRIG_MODE2_ID:
	case HAPTIC_TRIG_MODE3_ID:
		aw_info("triger wave id");
		aw_haptic->trig_mode = type - HAPTIC_TRIG_MODE1_ID;
		schedule_work(&aw_haptic->trig_work);
		break;
	default:
		break;
	}
}

static void aw_fine_grained_adapt_amp_gain(struct aw_haptic *aw_haptic)
{
	if (aw_haptic->amplitude > HAPTIC_GAIN_MAX_ADAPT_LEVELS)
		aw_haptic->amplitude = HAPTIC_GAIN_MAX_ADAPT_LEVELS;

	if (aw_haptic->amplitude < 0)
		return;

	aw_haptic->gain = HAPTIC_MAX_GAIN_REG * aw_haptic->amplitude / HAPTIC_GAIN_MAX_ADAPT_LEVELS;
	aw_info("gain = %u", aw_haptic->gain);
}

static uint32_t aw_get_inter_f0(uint32_t f0)
{
	aw_info("%s f0 = %u", __func__, f0);
	if (f0 < AW_F0_LOW_VAL || f0 > AW_F0_HIGH_VAL)
		return AW_MID_F0_VAL;

	return (((f0 - AW_F0_LOW_VAL) / AW_F0_INTERVAL_BIAS) * AW_F0_INTERVAL_BIAS + AW_F0_LOW_VAL + AW_CAL_INTE_F0_BIAS);
}

static void aw_set_100_preset_long(struct aw_haptic *aw_haptic, uint64_t type)
{
	aw_haptic->effect_mode = LONG_VIB_RAM_MODE;
	aw_haptic->index = LONG_VIB_EFFECT_ID;
	aw_haptic->duration = (type - FINE_GRAINED_LONG_BASE) / FINE_GRAINED_AMP_DIV_COFF;
	aw_haptic->amplitude = type % FINE_GRAINED_AMP_DIV_COFF;
	aw_haptic->state = 1;
	aw_fine_grained_adapt_amp_gain(aw_haptic);
	__pm_wakeup_event(aw_haptic->ws, aw_haptic->duration + HAPTIC_WAKE_LOCK_GAP);
	aw_info("long index = %d, amp = %d, dura = %d", aw_haptic->index, aw_haptic->amplitude, aw_haptic->duration);
	schedule_work(&aw_haptic->vibrator_work);
}

static void aw_play_effect_id(struct aw_haptic *aw_haptic, uint64_t type)
{
	int rtp_len = 0;

	aw_haptic->duration = 0;
	aw_haptic->ignore_stop_cmd = false;
	if (type >= FINE_GRAINED_LONG_BASE) {
		aw_set_100_preset_long(aw_haptic, type);
	} else if (type > FINE_GRAINED_RAM_BASE && type <= FINE_GRAINED_LONG_BASE) { // transient fine grained, amp is 0-99
		aw_haptic->effect_mode = SHORT_VIB_RAM_MODE;
		aw_haptic->amplitude = type % FINE_GRAINED_AMP_DIV_COFF;
		aw_haptic->index = type / FINE_GRAINED_AMP_DIV_COFF - FINE_GRAINED_RAM_ID_BASE_VAL;
		aw_info("fine grained index = %d, amp = %d", aw_haptic->index, aw_haptic->amplitude);
		aw_haptic->state = 1;
		aw_fine_grained_adapt_amp_gain(aw_haptic);
		queue_work(system_highpri_wq, &aw_haptic->vibrator_work);
	} else if (type > LONG_HAPTIC_RTP_MAX_ID && type < FINE_GRAINED_RAM_BASE) { // long time
		aw_haptic->effect_mode = LONG_VIB_RAM_MODE;
		aw_haptic->index = LONG_VIB_EFFECT_ID;
		aw_haptic->duration = type / LONG_TIME_AMP_DIV_COFF;
		aw_haptic->amplitude = type % LONG_TIME_AMP_DIV_COFF;
		aw_haptic->state = 1;
		aw_adapt_amp_again(aw_haptic, LONG_VIB_RAM_MODE);
		__pm_wakeup_event(aw_haptic->ws, aw_haptic->duration + HAPTIC_WAKE_LOCK_GAP);
		aw_info("long index = %d, amp = %d", aw_haptic->index, aw_haptic->amplitude);
		schedule_work(&aw_haptic->vibrator_work);
	} else if ((type > 0) && (type <= SHORT_HAPTIC_RAM_MAX_ID)) { // short ram haptic
		aw_haptic->effect_mode = SHORT_VIB_RAM_MODE;
		aw_haptic->amplitude = type % SHORT_HAPTIC_AMP_DIV_COFF;
		aw_haptic->index = type / SHORT_HAPTIC_AMP_DIV_COFF;
		aw_adapt_amp_again(aw_haptic, SHORT_VIB_RAM_MODE);
		aw_info("short index = %d, amp = %d", aw_haptic->index, aw_haptic->amplitude);
		aw_haptic->state = 1;
		schedule_work(&aw_haptic->vibrator_work);
	} else { // long and short rtp haptic
		aw_haptic->effect_mode = RTP_VIB_MODE;
		aw_haptic->amplitude = type % SHORT_HAPTIC_AMP_DIV_COFF;
		aw_haptic->index = type / SHORT_HAPTIC_AMP_DIV_COFF;
		aw_haptic->func->play_stop(aw_haptic);
		aw_haptic->func->set_rtp_aei(aw_haptic, false);
		aw_haptic->func->irq_clear(aw_haptic);
		aw_adapt_amp_again(aw_haptic, RTP_VIB_MODE);
		aw_haptic->state = 1;
		aw_haptic->rtp_idx = aw_haptic->index - BASE_INDEX;
		rtp_len += snprintf(aw_rtp_idx_name, AW_RTP_NAME_MAX - 1,
			"pre_%u_%d.bin", aw_get_inter_f0((aw_haptic->f0 / AW_F0_DATA_COFF)), aw_haptic->rtp_idx);
		aw_info("get rtp name = %s, index = %d, len = %d",
			aw_rtp_idx_name, aw_haptic->rtp_idx, rtp_len);
		schedule_work(&aw_haptic->rtp_work);
	}
}

static int bypbst_notify_call(struct notifier_block *nb, unsigned long event, void *data)
{
	struct buck_boost_vout_msg *vout_msg = NULL;
	struct aw_haptic *aw_haptic = NULL;

	if (!nb || !data) {
		aw_err("nb or data is NULL");
		return NOTIFY_OK;
	}
	aw_haptic = container_of(nb, struct aw_haptic, nb);
	vout_msg = (struct buck_boost_vout_msg *)data;
	if (!aw_haptic || !vout_msg)
		return NOTIFY_OK;

	switch (event) {
	case POWER_NE_BYPASSBOOST_VOUT_CHANGE:
		bypassboost_vol = vout_msg->vol;
		aw_info("bypassboost_vol = %u", bypassboost_vol);
		schedule_work(&aw_haptic->notifier_call_work);
		break;
	default:
		break;
	}
	return NOTIFY_OK;
}

static int charging_notify_call(struct notifier_block *nb, unsigned long event, void *data)
{
	if (!nb) {
		aw_err("nb is NULL");
		return NOTIFY_OK;
	}

	switch (event) {
	case POWER_NE_USB_CONNECT:
	case POWER_NE_WIRELESS_CONNECT:
	case POWER_NE_WIRELESS_TX_START:
		is_charging_status_g = true;
		break;
	case POWER_NE_USB_DISCONNECT:
	case POWER_NE_WIRELESS_TX_STOP:
	case POWER_NE_WIRELESS_DISCONNECT:
		is_charging_status_g = false;
		break;
	default:
		break;
	}

	aw_info("is_charging_status_g = %d", is_charging_status_g);
	return NOTIFY_OK;
}

static void bypbst_notify_call_work_handler(struct work_struct *work) {
	struct aw_haptic *aw_haptic = container_of(work, struct aw_haptic, notifier_call_work);

#ifdef CONFIG_HUAWEI_POWER_EMBEDDED_ISOLATION
		aw_set_bst_vol(aw_haptic);
#endif
}

static ssize_t aw_file_write(struct file *filp,
	const char *buff, size_t len, loff_t *off)
{
	struct aw_haptic *aw_haptic = NULL;
	char write_buf[MAX_WRITE_BUF_LEN] = {0};
	uint64_t type = 0;

	if (!buff || !filp || (len > (MAX_WRITE_BUF_LEN - 1)))
		return len;

	aw_haptic = (struct aw_haptic *)filp->private_data;
	if (!aw_haptic)
		return len;
	if (!aw_haptic->ram_init) {
		aw_err("ram init failed, not allow to play");
		return len;
	}
	mutex_lock(&aw_haptic->lock);
	if (copy_from_user(write_buf, buff, len)) {
		aw_err("[haptics_write] copy_from_user failed\n");
		mutex_unlock(&aw_haptic->lock);
		return len;
	}
	if (kstrtoull(write_buf, 10, &type)) {
		aw_err("[haptics_write] read value error\n");
		mutex_unlock(&aw_haptic->lock);
		return len;
	}
	aw_info("get haptic id = %llu", type);

	aw_haptic->emergency_mode_flag = false;
	aw_haptic->cust_boost_on = AW_SEL_BOOST_CFG_ON;
	if ((type < HAPTIC_MAX_SPEC_CMD_ID) && (type > HAPTIC_MIN_SPEC_CMD_ID)) {
		aw_special_type_process(aw_haptic, type);
		mutex_unlock(&aw_haptic->lock);
		return len;
	}
	aw_play_effect_id(aw_haptic, type);
	mutex_unlock(&aw_haptic->lock);
	return len;
}

#ifdef AAC_RICHTAP_SUPPORT
static void richtap_clean_buf(struct aw_haptic *aw_haptic, int status)
{
	struct mmap_buf_format *opbuf = aw_haptic->start_buf;
	int i = 0;

	for (i = 0; i < RICHTAP_MMAP_BUF_SUM; i++) {
		memset(opbuf->data, 0, RICHTAP_MMAP_BUF_SIZE);
		opbuf->status = status;
		opbuf = opbuf->kernel_next;
	}
}

static void richtap_update_fifo_data(struct aw_haptic *aw_haptic, uint32_t fifo_len)
{
	int32_t samples_left;
	int pos = 0;
	int retry = 3; // set max retry count 3

	do {
		if (aw_haptic->curr_buf->status == MMAP_BUF_DATA_VALID) {
			samples_left = aw_haptic->curr_buf->length - aw_haptic->pos;
			if (samples_left < fifo_len) {
				memcpy(&aw_haptic->rtp_ptr[pos],
					&aw_haptic->curr_buf->data[aw_haptic->pos],
					samples_left);
				pos += samples_left;
				fifo_len -= samples_left;
				aw_haptic->curr_buf->status = MMAP_BUF_DATA_INVALID;
				aw_haptic->curr_buf->length = 0;
				aw_haptic->curr_buf = aw_haptic->curr_buf->kernel_next;
				aw_haptic->pos = 0;
			} else {
				memcpy(&aw_haptic->rtp_ptr[pos],
					&aw_haptic->curr_buf->data[aw_haptic->pos],
					fifo_len);
				aw_haptic->pos += fifo_len;
				pos += fifo_len;
				fifo_len = 0;
			}
		} else if (aw_haptic->curr_buf->status == MMAP_BUF_DATA_FINISHED) {
			break;
		} else {
			if (retry-- <= 0) {
				pr_info("invalid data\n");
				break;
			} else {
				usleep_range(1000, 1000);
			}
		}
	} while (fifo_len > 0 && atomic_read(&aw_haptic->richtap_rtp_mode));
	aw_info("update fifo len %d\n", pos);
	__pm_wakeup_event(aw_haptic->ws, (pos + RTP_AEI_LENGTH) / RTP_LEN_TO_PLAY_TIME_COFF);
	aw_haptic->func->set_rtp_data(aw_haptic, aw_haptic->rtp_ptr, pos);
}

static bool richtap_rtp_start(struct aw_haptic *aw_haptic)
{
	int cnt = 200; // max wait gls state time
	bool rtp_work_flag = false;
	uint8_t reg_val = 0;

	mutex_lock(&aw_haptic->lock);
	aw_haptic->func->play_mode(aw_haptic, AW_RTP_MODE);
	aw_haptic->func->play_go(aw_haptic, true);
	udelay(2000);

	while (cnt) {
		reg_val = aw_haptic->func->get_glb_state(aw_haptic);
		if ((reg_val & AW_GLBRD_STATE_MASK) == AW_STATE_RTP) {
			cnt = 0;
			rtp_work_flag = true;
			pr_info("%s: RTP_GO! glb_state=0x08\n", __func__);
		} else {
			usleep_range(2000, 2200);
			cnt--;
			pr_debug("%s: wait for RTP_GO, glb_state=0x%02X\n",
				__func__, reg_val);
		}
	}

	if (rtp_work_flag == false)
		aw_haptic->func->play_stop(aw_haptic);

	mutex_unlock(&aw_haptic->lock);
	return rtp_work_flag;
}

static void update_richtap_play_on_time(int data_len)
{
	if (data_len > MAX_TRANSIENT_RTP_LEN)
		return;
	ktime_get_real_ts64(&pre_ts);
	gap_time_ms = data_len / RTP_LEN_TO_PLAY_TIME_COFF + RTP_PLAY_END_STABLE_TIME; // calc play time
}

static void richtap_rtp_work(struct work_struct *work)
{
	struct aw_haptic *aw_haptic = container_of(work, struct aw_haptic, richtap_rtp_work);
	uint32_t retry = 0;
	uint32_t tmp_len = 0;

	if (!aw_haptic)
		return;
	aw_haptic->curr_buf = aw_haptic->start_buf;

	check_chip_status(aw_haptic);
	if (aw_haptic->is_dev_resetting) {
		aw_info("vib resetting");
		return;
	}
	do {
		if (aw_haptic->curr_buf->status == MMAP_BUF_DATA_VALID) {
			if ((tmp_len + aw_haptic->curr_buf->length) >
				aw_haptic->ram.base_addr) {
				memcpy(&aw_haptic->rtp_ptr[tmp_len],
					aw_haptic->curr_buf->data,
					(aw_haptic->ram.base_addr - tmp_len));
				aw_haptic->pos = aw_haptic->ram.base_addr - tmp_len;
				tmp_len = aw_haptic->ram.base_addr;
			} else {
				memcpy(&aw_haptic->rtp_ptr[tmp_len],
					aw_haptic->curr_buf->data,
					aw_haptic->curr_buf->length);
				tmp_len += aw_haptic->curr_buf->length;
				aw_haptic->curr_buf->status = MMAP_BUF_DATA_INVALID;
				aw_haptic->curr_buf->length = 0;
				aw_haptic->pos = 0;
				aw_haptic->curr_buf = aw_haptic->curr_buf->kernel_next;
			}
		} else if (aw_haptic->curr_buf->status == MMAP_BUF_DATA_FINISHED) {
			break;
		} else {
			usleep_range(1000, 1200);
		}
	} while (tmp_len < aw_haptic->ram.base_addr && retry++ < 30); // try cont

	aw_info("rtp tm_len = %u, retry = %u, aw_haptic->ram.base_addr = %u", tmp_len, retry, aw_haptic->ram.base_addr);
	__pm_wakeup_event(aw_haptic->ws, (tmp_len + RTP_AEI_LENGTH) / RTP_LEN_TO_PLAY_TIME_COFF);
	if (richtap_rtp_start(aw_haptic)) {
		aw_haptic->func->set_rtp_data(aw_haptic, aw_haptic->rtp_ptr, tmp_len);
		update_richtap_play_on_time(tmp_len);
		aw_info("set rtp data end");
		aw_haptic->func->set_rtp_aei(aw_haptic, true);
		aw_haptic->func->irq_clear(aw_haptic);
		atomic_set(&aw_haptic->richtap_rtp_mode, true);
	}
}

static void richtap_config_stream_mode(struct aw_haptic *aw_haptic)
{
	richtap_clean_buf(aw_haptic, MMAP_BUF_DATA_INVALID);
	mutex_lock(&aw_haptic->lock);
	aw_haptic->func->irq_clear(aw_haptic);
	aw_haptic->func->play_stop(aw_haptic);
	mutex_unlock(&aw_haptic->lock);
	aw_haptic->func->set_rtp_aei(aw_haptic, false);
	atomic_set(&aw_haptic->richtap_rtp_mode, false);
	aw_haptic->func->upload_lra(aw_haptic, AW_OSC_CALI_LRA);
	aw_haptic->func->set_bst_vol(aw_haptic, aw_haptic->vmax);
#ifdef CONFIG_HUAWEI_POWER_EMBEDDED_ISOLATION
	if (aw_haptic->info.silicon_battery_cfg[IS_SILICON_BATTERY])
		aw_set_bst_vol(aw_haptic);
	else
		aw_haptic->func->set_bst_vol(aw_haptic, aw_haptic->vmax);
#else
	aw_haptic->func->set_bst_vol(aw_haptic, aw_haptic->vmax);
#endif
	schedule_work(&aw_haptic->richtap_rtp_work);
}

static void richtap_config_stop_mode(struct aw_haptic *aw_haptic)
{
	richtap_clean_buf(aw_haptic, MMAP_BUF_DATA_FINISHED);
	mutex_lock(&aw_haptic->lock);
	aw_haptic->func->irq_clear(aw_haptic);
	aw_haptic->func->play_stop(aw_haptic);
	mutex_unlock(&aw_haptic->lock);
	aw_haptic->func->enable_gain(aw_haptic, 0);
	aw_haptic->func->set_rtp_aei(aw_haptic, false);
	atomic_set(&aw_haptic->richtap_rtp_mode, false);
}

static int richtap_running_status_check()
{
	struct timespec64 now_ts;
	struct timespec64 sub_ts;
	int64_t ns;

	ktime_get_real_ts64(&now_ts);
	sub_ts = timespec64_sub(now_ts, pre_ts);
	ns = timespec64_to_ns(&sub_ts);
	pr_info("%s: gap pre play time = %lu, gap_time_ms = %lu\n", __func__, ns, gap_time_ms);
	if (ns < (gap_time_ms * MS_TIME_TO_NS_TIME))
		return -1;

	return 0;
}

static long richtap_file_unlocked_ioctl(struct file *filp,
	unsigned int cmd, unsigned long arg)
{
	struct aw_haptic *aw_haptic = NULL;
	int ret = 0;
	int tmp;

	if (!filp)
		return -EFAULT;
	aw_haptic = (struct aw_haptic *)filp->private_data;
	if (!aw_haptic)
		return -EFAULT;
	pr_info("%s: cmd=0x%x, arg=0x%lx\n", __func__, cmd, arg);
	if (aw_haptic->info.is_enabled_richtap_core != true) {
		pr_info("do not support richtap core feature");
		return -EFAULT;
	}
	switch (cmd) {
	case RICHTAP_GET_HWINFO:
		tmp = RICHTAP_AW_8697;
		if (copy_to_user((void __user *)arg, &tmp, sizeof(int)))
			ret = -EFAULT;
		break;
	case RICHTAP_RTP_MODE:
		break;
	case RICHTAP_OFF_MODE:
		break;
	case RICHTAP_GET_F0:
		tmp = aw_haptic->f0;
		if (copy_to_user((void __user *)arg, &tmp, sizeof(int)))
			ret = -EFAULT;
		break;
	case RICHTAP_SETTING_GAIN:
		if (arg > 0x80)
			arg = 0x80;
		aw_haptic->gain = (int)arg;
		aw_haptic->func->enable_gain(aw_haptic, 1);
		aw_haptic->func->set_gain(aw_haptic, (uint8_t)arg);
		break;
	case RICHTAP_STREAM_MODE:
		__pm_wakeup_event(aw_haptic->ws, HAPTIC_RTP_WAKE_TIME);
		ret = richtap_running_status_check();
		if (!ret)
			richtap_config_stream_mode(aw_haptic);
		break;
	case RICHTAP_STOP_MODE:
		aw_haptic->ignore_stop_cmd = false;
		ret = richtap_running_status_check();
		if (!ret)
			richtap_config_stop_mode(aw_haptic);
		break;
	default:
		aw_info("%s, unknown cmd\n", __func__);
		break;
	}
	return ret;
}

static int richtap_file_mmap(struct file *filp, struct vm_area_struct *vma)
{
	unsigned long phys;
	struct aw_haptic *aw_haptic = NULL;
	int ret;

	if (!filp || !vma)
		return -EPERM;
	aw_haptic = (struct aw_haptic *)filp->private_data;
	if (!aw_haptic)
		return -EPERM;
#if LINUX_VERSION_CODE > KERNEL_VERSION(4,7,0)
	// only accept PROT_READ, PROT_WRITE and MAP_SHARED from the API of mmap
	vm_flags_t vm_flags = calc_vm_prot_bits(PROT_READ|PROT_WRITE, 0) |
		calc_vm_flag_bits(MAP_SHARED);
	vm_flags |= current->mm->def_flags | VM_MAYREAD | VM_MAYWRITE |
		VM_MAYEXEC| VM_SHARED | VM_MAYSHARE;
	if (vma && (pgprot_val(vma->vm_page_prot) !=
		pgprot_val(vm_get_page_prot(vm_flags))))
		return -EPERM;

	if (vma && ((vma->vm_end - vma->vm_start) !=
		(PAGE_SIZE << RICHTAP_MMAP_PAGE_ORDER)))
		return -ENOMEM;
#endif
	phys = virt_to_phys(aw_haptic->start_buf);
	ret = remap_pfn_range(vma, vma->vm_start, (phys >> PAGE_SHIFT),
		(vma->vm_end - vma->vm_start), vma->vm_page_prot);
	if (ret) {
		dev_err(aw_haptic->dev, "Error mmap failed\n");
		return ret;
	}

	return ret;
}

static int richtap_rtp_config_init(struct aw_haptic *aw_haptic)
{
	uint32_t i;
	struct mmap_buf_format *temp;

	aw_haptic->rtp_ptr = kmalloc(RICHTAP_MMAP_BUF_SIZE * RICHTAP_MMAP_BUF_SUM,
		GFP_KERNEL);
	if (aw_haptic->rtp_ptr == NULL) {
		aw_err("malloc rtp memory failed\n");
		return -EINVAL;
	}

	aw_haptic->start_buf = (struct mmap_buf_format *)__get_free_pages(GFP_KERNEL,
		RICHTAP_MMAP_PAGE_ORDER);
	if (aw_haptic->start_buf == NULL) {
		kfree(aw_haptic->rtp_ptr);
		aw_err("Error __get_free_pages failed\n");
		return -EINVAL;
	}
	SetPageReserved(virt_to_page(aw_haptic->start_buf));
	temp = aw_haptic->start_buf;
	for (i = 1; i < RICHTAP_MMAP_BUF_SUM; i++) { // from page 1 start
		temp->kernel_next = (aw_haptic->start_buf + i);
		temp = temp->kernel_next;
	}
	temp->kernel_next = aw_haptic->start_buf;

	INIT_WORK(&aw_haptic->richtap_rtp_work, richtap_rtp_work);
	atomic_set(&aw_haptic->richtap_rtp_mode, false);
	return 0;
}
#endif

static const struct file_operations fops = {
	.owner = THIS_MODULE,
	.read = aw_file_read,
	.write = aw_file_write,
#ifdef AAC_RICHTAP_SUPPORT
	.unlocked_ioctl = richtap_file_unlocked_ioctl,
	.mmap = richtap_file_mmap,
#endif
	.open = aw_file_open,
	.release = aw_file_release,
};

static const struct file_operations fops1 = {
	.owner = THIS_MODULE,
	.read = aw_file_read,
	.write = aw_file_write,
#ifdef AAC_RICHTAP_SUPPORT
	.unlocked_ioctl = richtap_file_unlocked_ioctl,
	.mmap = richtap_file_mmap,
#endif
	.open = sub_aw_file_open,
	.release = aw_file_release,
};

static struct miscdevice aw_haptic1_misc = {
	.minor = MISC_DYNAMIC_MINOR,
	.name = AW_HAPTIC1_NAME,
	.fops = &fops1,
};

static struct miscdevice aw_haptic_misc = {
	.minor = MISC_DYNAMIC_MINOR,
	.name = AW_HAPTIC_NAME,
	.fops = &fops,
};

static enum hrtimer_restart vibrator_timer_func(struct hrtimer *timer)
{
	struct aw_haptic *aw_haptic = NULL;

	if (!timer) {
		return 0;
	}
	aw_haptic = container_of(timer, struct aw_haptic, timer);
	aw_haptic->state = 0;
	aw_haptic->ignore_stop_cmd = false;
	schedule_work(&aw_haptic->vibrator_work);
	return HRTIMER_NORESTART;
}

#ifdef AW_DURATION_DECIDE_WAVEFORM
static int ram_select_waveform(struct aw_haptic *aw_haptic)
{
	uint8_t wavseq = 0;
	uint8_t wavloop = 0;

	if (!aw_haptic) {
		aw_err ("aw_haptic is null");
		return -EINVAL;
	}
	if (aw_haptic->duration <= 0) {
		aw_err("duration time %d error", aw_haptic->duration);
		return -ERANGE;
	}
	aw_haptic->activate_mode = AW_RAM_MODE;
	if ((aw_haptic->duration > 0) &&
		(aw_haptic->duration < aw_haptic->info.duration_time[0])) {
		wavseq = 3;
	} else if ((aw_haptic->duration >= aw_haptic->info.duration_time[0]) &&
				(aw_haptic->duration < aw_haptic->info.duration_time[1])) {
		wavseq = 2;
	} else if ((aw_haptic->duration >= aw_haptic->info.duration_time[1]) &&
				(aw_haptic->duration < aw_haptic->info.duration_time[2])) {
		wavseq = 1;
	} else if (aw_haptic->duration >= aw_haptic->info.duration_time[2]) {
		wavseq = 4;
		wavloop = 15;
		aw_haptic->activate_mode = AW_RAM_LOOP_MODE;
	}
	aw_info("duration %d, select index %d", aw_haptic->duration, wavseq);
	aw_haptic->func->set_wav_seq(aw_haptic, 0, wavseq);
	aw_haptic->func->set_wav_loop(aw_haptic, 0, wavloop);
	aw_haptic->func->set_wav_seq(aw_haptic, 1, 0);
	aw_haptic->func->set_wav_loop(aw_haptic, 1, 0);
	return 0;
}
#endif

static void aw_long_ram_mode_boost_config(struct aw_haptic *aw_haptic,
	unsigned char wavseq)
{
	if (aw_haptic->cust_boost_on == AW_SEL_BOOST_CFG_ON) {
		aw_haptic->func->set_wav_seq(aw_haptic, 0, 0x14);
		aw_haptic->func->set_wav_loop(aw_haptic, 0, 0); // loop mode
		aw_haptic->func->set_wav_seq(aw_haptic, 1, wavseq);
		aw_haptic->func->set_wav_loop(aw_haptic, 1, 0x0f);
		aw_haptic->func->set_wav_seq(aw_haptic, 2, 0);
		aw_haptic->func->set_wav_loop(aw_haptic, 2, 0);
	} else {
		aw_haptic->func->set_wav_seq(aw_haptic, 0, wavseq);
		aw_haptic->func->set_wav_loop(aw_haptic, 0, 0x0f);
		aw_haptic->func->set_wav_seq(aw_haptic, 1, 0);
		aw_haptic->func->set_wav_loop(aw_haptic, 1, 0);
	}
}

static int aw_haptic_ram_config(struct aw_haptic *aw_haptic)
{
	unsigned char wavseq = 0;
	unsigned char wavloop = 0;
	unsigned char seq_idx = 0;
	// unsigned int total_wavloop = 0;

	if (!aw_haptic) {
		aw_err ("aw_haptic is null");
		return -EINVAL;
	}
	aw_info("duration=%d,index = %d\n", aw_haptic->duration, aw_haptic->index);
	if (aw_haptic->effect_mode == SHORT_VIB_RAM_MODE) {
		wavseq = aw_haptic->index;
		wavloop = 0;
		aw_haptic->func->set_wav_seq(aw_haptic, 0, wavseq);
		aw_haptic->func->set_wav_loop(aw_haptic, 0, wavloop);
		aw_haptic->func->set_wav_seq(aw_haptic, 1, 0);
		aw_haptic->func->set_wav_loop(aw_haptic, 1, 0);
		aw_info ("short ram waveseq = %u\n", wavseq);
		aw_haptic->func->short_ram_brk_cfg_set(aw_haptic);
		aw_haptic->is_charging_status = is_charging_status_g;
	} else {
		wavseq = aw_haptic->index;
		if (aw_haptic->info.support_long_brk_cfg)
			aw_haptic->func->long_ram_brk_cfg_set(aw_haptic);
		aw_long_ram_mode_boost_config(aw_haptic, wavseq);
		aw_info ("long ram waveseq = %u\n", wavseq);
		aw_haptic->is_charging_status = false;
	}
	return 0;
}

static int aw_haptic_play_repeat_seq(struct aw_haptic *aw_haptic,
									unsigned char flag)
{
	aw_info("enter\n");

	if (!aw_haptic) {
		aw_err ("aw_haptic is null");
		return -EINVAL;
	}
	if (flag) {
		aw_haptic->func->play_mode(aw_haptic, AW_RAM_LOOP_MODE);
		aw_haptic->func->play_go(aw_haptic, true);
	}
	return 0;
}

#ifdef CONFIG_HUAWEI_POWER_EMBEDDED_ISOLATION
static void aw_haptic_emergency_mode(struct aw_haptic *aw_haptic)
{
	int ret;
	int ele = 0;

	ret = power_supply_get_int_property_value("Battery",
		POWER_SUPPLY_PROP_CAPACITY, &ele);
	if (ret < 0) {
		aw_err("%s: get capacity failed", __func__);
		return;
	}
	if (ele <= 1) {
		aw_info("%s: enter!", __func__);
		aw_haptic->emergency_mode_flag = true;
	}
}

static void adjust_haptic_gain_and_bst_by_vol(struct aw_haptic *aw_haptic, int bat_power,
	int specified_power, int bat_temp, int specified_temp)
{
	if (bypassboost_vol == BYPASSBOOST_5000MV) {
		if (bat_power < specified_power)
			aw_haptic->func->set_bst_vol(aw_haptic, aw_haptic->info.silicon_battery_cfg[LOWPOWER_AND_LOWTEMP_BST]);
		else
			aw_haptic->func->set_bst_vol(aw_haptic, aw_haptic->info.silicon_battery_cfg[LOWPOWER_OR_LOWTEMP_BST]);
	} else if (bypassboost_vol == BYPASSBOOST_4500MV) {
		if (bat_power < specified_power && bat_temp < specified_temp)
			aw_haptic->func->set_bst_vol(aw_haptic, aw_haptic->info.silicon_battery_cfg[LOWPOWER_AND_LOWTEMP_BST]);
		else if ((bat_power > specified_power && bat_temp < specified_temp) ||
			(bat_power < specified_power && bat_temp > specified_temp))
			aw_haptic->func->set_bst_vol(aw_haptic, aw_haptic->info.silicon_battery_cfg[LOWPOWER_OR_LOWTEMP_BST]);
		else
			aw_haptic->func->set_bst_vol(aw_haptic, aw_haptic->vmax);
	}
}

static void adjust_gain_and_bst_by_power_and_temp(struct aw_haptic *aw_haptic, int bat_power,
	int specified_power, int bat_temp, int specified_temp)
{
	if (bat_power < specified_power && bat_temp < specified_temp)
		aw_haptic->func->set_bst_vol(aw_haptic, aw_haptic->info.silicon_battery_cfg[LOWPOWER_AND_LOWTEMP_BST]);
	else if (bat_power < specified_power || bat_temp < specified_temp)
		aw_haptic->func->set_bst_vol(aw_haptic, aw_haptic->info.silicon_battery_cfg[LOWPOWER_OR_LOWTEMP_BST]);
	else
		aw_haptic->func->set_bst_vol(aw_haptic, aw_haptic->vmax);
}

static void aw_set_bst_vol(struct aw_haptic *aw_haptic)
{
	int bat_power = 0;
	int bat_temp = 0;
	int specified_temp = 0;
	int specified_power = 0;
 
	power_supply_get_int_property_value("Battery", POWER_SUPPLY_PROP_CAPACITY, &bat_power);
	power_supply_get_int_property_value("Battery", POWER_SUPPLY_PROP_TEMP, &bat_temp);
	bat_temp /= 10;
	aw_info("bat_power = %d bat_temp = %d!", bat_power, bat_temp);
	specified_power = aw_haptic->info.silicon_battery_cfg[SPECIFIED_BAT_POWER];
	if (aw_haptic->info.silicon_battery_cfg[IS_TEMP_BELOW_ZERO])
		specified_temp -= aw_haptic->info.silicon_battery_cfg[SPECIFIED_BAT_TEMP];
	else
		specified_temp = aw_haptic->info.silicon_battery_cfg[SPECIFIED_BAT_TEMP];

	if (aw_haptic->boot_power_set == SUPPORT_BOOST_5V) {
		adjust_haptic_gain_and_bst_by_vol(aw_haptic, bat_power, specified_power, bat_temp, specified_temp);
	} else {
		adjust_gain_and_bst_by_power_and_temp(aw_haptic, bat_power, specified_power, bat_temp, specified_temp);
	}
}
#endif

static void aw_boost_config(struct aw_haptic *aw_haptic)
{
	if (!aw_haptic->info.is_enabled_lowpower_bst_config) {
		aw_haptic->func->bst_mode_config(aw_haptic, AW_BST_BOOST_MODE);
		return;
	}
	if (aw_haptic->emergency_mode_flag)
		aw_haptic->func->bst_mode_config(aw_haptic, AW_BST_BYPASS_MODE);
	else
		aw_haptic->func->bst_mode_config(aw_haptic, AW_BST_BOOST_MODE);
}

static void ram_mode_sel_boost_play_cfg(struct aw_haptic *aw_haptic)
{
#ifdef CONFIG_HUAWEI_POWER_EMBEDDED_ISOLATION
	if (aw_haptic->info.is_enabled_lowpower_bst_config)
		aw_haptic_emergency_mode(aw_haptic);
	if (aw_haptic->info.silicon_battery_cfg[IS_SILICON_BATTERY])
		aw_set_bst_vol(aw_haptic);
#endif

	if (aw_haptic->cust_boost_on == AW_SEL_BOOST_CFG_ON) {
		ram_vbat_comp(aw_haptic, false);
		aw_boost_config(aw_haptic);
		aw_haptic->func->play_mode(aw_haptic, AW_RAM_MODE);
		aw_haptic->func->play_go(aw_haptic, true);
		return;
	}
	if (aw_haptic->effect_mode == LONG_VIB_RAM_MODE) {
		ram_vbat_comp(aw_haptic, true);
		aw_haptic->func->bst_mode_config(aw_haptic, AW_BST_BYPASS_MODE);
		aw_haptic_play_repeat_seq(aw_haptic, true);
	} else {
		ram_vbat_comp(aw_haptic, false);
		aw_haptic->func->bst_mode_config(aw_haptic, AW_BST_BOOST_MODE);
		aw_haptic->func->play_mode(aw_haptic, AW_RAM_MODE);
		aw_haptic->func->play_go(aw_haptic, true);
	}
}

static void haptic_base_reg_init(struct aw_haptic *aw_haptic)
{
	aw_haptic->func->play_mode(aw_haptic, AW_STANDBY_MODE);
	aw_haptic->func->set_pwm(aw_haptic, AW_PWM_12K);
	/* misc value init */
	aw_haptic->func->misc_para_init(aw_haptic);
	aw_haptic->func->protect_config(aw_haptic, 0x01, 0x00);
	aw_haptic->func->auto_bst_enable(aw_haptic, aw_haptic->info.is_enabled_auto_bst);

	/* vbat compensation */
	aw_haptic->func->vbat_mode_config(aw_haptic, AW_CONT_VBAT_HW_COMP_MODE);
	aw_haptic->func->upload_lra(aw_haptic, AW_F0_CALI_LRA);
}

static void chip_reset(struct aw_haptic *aw_haptic)
{
	aw_info("power done and reset");
	hw_reset(aw_haptic);
	haptic_base_reg_init(aw_haptic);
	aw_haptic->func->interrupt_setup(aw_haptic);
	ram_update(aw_haptic);
	aw_info("reset end");
}

static void check_chip_status(struct aw_haptic *aw_haptic)
{
	static bool print_reg_flag = true;
	int i = 0;
	uint8_t reg_val = 0;

	if (aw_haptic->is_dev_resetting) {
		aw_info("vib resetting");
		return;
	}

	i2c_r_bytes(aw_haptic, AW8692X_REG_SYSINTM, &reg_val, AW_I2C_BYTE_ONE);
	if (reg_val == AW8692X_REG_SYSINTM_RESET) {
		aw_haptic->is_dev_resetting = true;
		if (print_reg_flag) {
			for (i = 0; i < AW8692X_ALL_REG; i++) {
				i2c_r_bytes(aw_haptic, i, &reg_val, AW_I2C_BYTE_ONE);
				aw_info("reg: 0x%02x = 0x%02x", i, reg_val);
			}
			print_reg_flag = false;
		}
		aw_vibrator_dmd_work(DSM_VIBRATOR_POWER_DONE);
		chip_reset(aw_haptic);
	}
}

static void vibrator_work_routine(struct work_struct *work)
{
	struct aw_haptic *aw_haptic = NULL;

	if (!work) {
		aw_err ("work is null");
		return;
	}
	aw_haptic = container_of(work, struct aw_haptic, vibrator_work);

	check_chip_status(aw_haptic);
	if (aw_haptic->is_dev_resetting) {
		aw_info("vib reseting");
		return;
	}
	/* charging mode */
	mutex_lock(&aw_haptic->lock);
	if (aw_haptic->f0_is_cali == HAPTIC_CHARGING_IS_CALIB) {
		aw_haptic->func->upload_lra(aw_haptic, AW_F0_CALI_LRA);
		f0_cali(aw_haptic);
		aw_haptic->f0_is_cali = 0;
		mutex_unlock(&aw_haptic->lock);
		return;
	}

	aw_haptic->func->upload_lra(aw_haptic, AW_F0_CALI_LRA);
	/* Enter standby mode */
	aw_haptic->func->play_stop(aw_haptic);

	if (aw_haptic->state) {
		aw_info("state is true");
#ifdef AW_DURATION_DECIDE_WAVEFORM
		ram_select_waveform(aw_haptic);
#endif
		if (aw_haptic->activate_mode == AW_HAPTIC_ACTIVATE_RAM_MODE) {
			aw_info("activate_mode == AW_HAPTIC_ACTIVATE_RAM_MODE");
			aw_haptic_ram_config(aw_haptic);
			if (aw_haptic->duration <= AW_LONG_HAPTIC_RUNNING) { // max long vibra config
				aw_info("duration <= AW_LONG_HAPTIC_RUNNINGe");
				ram_mode_sel_boost_play_cfg(aw_haptic);
			} else {
				ram_vbat_comp(aw_haptic, true);
				aw_haptic_play_repeat_seq(aw_haptic, true);
			}
		} else if (aw_haptic->activate_mode == AW_HAPTIC_ACTIVATE_CONT_MODE) {
			aw_info("activate_mode == AW_HAPTIC_ACTIVATE_CONT_MODE");
			aw_haptic->func->cont_config(aw_haptic);
		} else {
			/* other mode */
			aw_err("set other mode\n");
		}
		/* run ms timer */
		if (aw_haptic->effect_mode == LONG_VIB_RAM_MODE ||
			(aw_haptic->activate_mode == AW_HAPTIC_ACTIVATE_CONT_MODE)) {
			aw_info("run ms timer");
			hrtimer_start(&aw_haptic->timer, ktime_set(aw_haptic->duration / 1000,
				(aw_haptic->duration % 1000) * 1000000), HRTIMER_MODE_REL);
			__pm_wakeup_event(aw_haptic->ws, aw_haptic->duration + HAPTIC_WAKE_LOCK_GAP);
			aw_info("%s gain = 0x%02X\n",  __func__, aw_haptic->gain);
#ifdef CONFIG_HUAWEI_DUBAI
			HWDUBAI_LOGE("DUBAI_TAG_HAPTIC_DURATION", "duration=%d index=%d",
				aw_haptic->duration, aw_haptic->index);
#endif
		}
	} else {
		aw_info("state is flase");
	}
	mutex_unlock(&aw_haptic->lock);
}

static void rtp_play(struct aw_haptic *aw_haptic)
{
	uint8_t glb_state_val = 0;
	uint32_t real_len = 10;

	aw_info("enter!");
	if (!aw_haptic) {
		aw_err ("aw_haptic is null");
		return;
	}

	mutex_lock(&aw_haptic->rtp_lock);
	while ((!aw_haptic->func->rtp_get_fifo_afs(aw_haptic))
			&& (aw_haptic->play_mode == AW_RTP_MODE)) {
#ifdef AW_ENABLE_RTP_PRINT_LOG
		aw_info("rtp cnt = %d", aw_haptic->rtp_cnt);
#endif
		if (!aw_rtp) {
			aw_info("aw_rtp is null, break!");
			break;
		}
		if (aw_haptic->rtp_cnt < (aw_haptic->ram.base_addr)) {
			if ((aw_rtp->len - aw_haptic->rtp_cnt) < (aw_haptic->ram.base_addr))
				real_len = aw_rtp->len - aw_haptic->rtp_cnt;
			else
				real_len = aw_haptic->ram.base_addr;
		} else if ((aw_rtp->len - aw_haptic->rtp_cnt) < (aw_haptic->ram.base_addr >> 2)) {
			real_len = aw_rtp->len - aw_haptic->rtp_cnt;
		} else {
			real_len = aw_haptic->ram.base_addr >> 2; // 512 data len
		}
		aw_haptic->func->set_rtp_data(aw_haptic, &aw_rtp->data[aw_haptic->rtp_cnt], real_len);
		aw_haptic->rtp_cnt += real_len; // 2048
		glb_state_val = aw_haptic->func->get_glb_state(aw_haptic);
		if ((aw_haptic->rtp_cnt == aw_rtp->len) || ((glb_state_val & AW_GLBRD_STATE_MASK) == AW_STATE_STANDBY)) {
			if (aw_haptic->rtp_cnt != aw_rtp->len)
				aw_err("rtp play suspend!");
			else
				aw_info("rtp update complete!");
			aw_haptic->rtp_cnt = 0;
			aw_haptic->rtp_init = false;
			break;
		}
	}
	if (aw_haptic->play_mode == AW_RTP_MODE)
		aw_haptic->func->set_rtp_aei(aw_haptic, true);
	mutex_unlock(&aw_haptic->rtp_lock);
}

static int rtp_work_init(struct aw_haptic *aw_haptic)
{
	int ret = -1;
	const struct firmware *rtp_file;

	mutex_lock(&aw_haptic->rtp_lock);
	aw_haptic->rtp_routine_on = 1;
	/* fw loaded */
	ret = request_firmware(&rtp_file, aw_rtp_idx_name, aw_haptic->dev);
	if (ret < 0) {
		aw_err("failed to read %s", aw_rtp_idx_name);
		aw_haptic->rtp_routine_on = 0;
		mutex_unlock(&aw_haptic->rtp_lock);
		return ret;
	}
	aw_haptic->rtp_init = false;
	vfree(aw_rtp);
	aw_rtp = vmalloc(rtp_file->size + sizeof(int));
	if (!aw_rtp) {
		release_firmware(rtp_file);
		aw_err("error allocating memory");
		aw_haptic->rtp_routine_on = 0;
		mutex_unlock(&aw_haptic->rtp_lock);
		return -1;
	}
	aw_rtp->len = rtp_file->size;
	aw_info("rtp file:[%s] size = %dbytes", aw_rtp_idx_name, aw_rtp->len);
	memcpy(aw_rtp->data, rtp_file->data, rtp_file->size);
	mutex_unlock(&aw_haptic->rtp_lock);
	release_firmware(rtp_file);
	aw_haptic->rtp_init = true;
	aw_haptic->func->upload_lra(aw_haptic, AW_OSC_CALI_LRA);
	aw_haptic->func->set_rtp_aei(aw_haptic, false);
	aw_haptic->func->irq_clear(aw_haptic);
	aw_haptic->func->play_stop(aw_haptic);
	/* gain */
	ram_vbat_comp(aw_haptic, false);
	/* boost voltage */
	if (aw_haptic->info.bst_vol_rtp <= aw_haptic->vmax && aw_haptic->info.bst_vol_rtp > 0)
		aw_haptic->func->set_bst_vol(aw_haptic, aw_haptic->info.bst_vol_rtp);
	else
		aw_haptic->func->set_bst_vol(aw_haptic, aw_haptic->vmax);
	return 0;
}

static void rtp_work_routine(struct work_struct *work)
{
	bool rtp_work_flag = false;
	uint8_t reg_val = 0;
	int cnt = 200;
	int ret = -1;
	struct aw_haptic *aw_haptic = NULL;

	if (!work) {
		aw_err("work is null");
		return;
	}
	aw_haptic = container_of(work, struct aw_haptic, rtp_work);

	check_chip_status(aw_haptic);
	if (aw_haptic->is_dev_resetting) {
		aw_info("vib resetting");
		return;
	}
	ret = rtp_work_init(aw_haptic);
	if (ret < 0) {
		aw_err("rtp_work_init failed");
		return;
	}
	mutex_lock(&aw_haptic->lock);
	/* rtp mode config */
	aw_haptic->func->play_mode(aw_haptic, AW_RTP_MODE);
	/* haptic go */
	aw_haptic->func->play_go(aw_haptic, true);
	usleep_range(2000, 2500);
	while (cnt) {
		reg_val = aw_haptic->func->get_glb_state(aw_haptic);
		if ((reg_val & AW_GLBRD_STATE_MASK) == AW_STATE_RTP) {
			cnt = 0;
			rtp_work_flag = true;
			aw_info("RTP_GO! glb_state=0x08");
		} else {
			cnt--;
			aw_info("wait for RTP_GO, glb_state=0x%02X", reg_val);
		}
		usleep_range(2000, 2500);
	}
	if (rtp_work_flag) {
		aw_info("rtp_work_flag is ture");
		rtp_play(aw_haptic);
		aw_haptic->rtp_cnt = 0;
	} else {
		/* enter standby mode */
		aw_haptic->func->play_stop(aw_haptic);
		aw_err("failed to enter RTP_GO status!");
	}
	aw_haptic->rtp_routine_on = 0;
	mutex_unlock(&aw_haptic->lock);
}

static void trig_work_routine(struct work_struct *work)
{
	struct aw_haptic *aw_haptic = NULL;
	int gpio = -1;

	if (!work) {
		aw_err("work is null");
		return;
	}
	aw_haptic = container_of(work, struct aw_haptic, trig_work);

	if (aw_haptic->trig_mode >= AW_TRIG_NUM) {
		aw_err("input wrong trig_mode");
		return;
	}
	mutex_lock(&aw_haptic->lock);
	gpio = aw_haptic->trig_gpios[aw_haptic->trig_mode];
	aw_info("trig_mode = %d, gpio = %d", aw_haptic->trig_mode, gpio);

	if (!gpio_is_valid(gpio)) {
		aw_err("invalue gpio");
		mutex_unlock(&aw_haptic->lock);
		return;
	}
	if (gpio_get_value(gpio))
		gpio_set_value(gpio, 0);
	else
		gpio_set_value(gpio, 1);
	mutex_unlock(&aw_haptic->lock);
}

static irqreturn_t irq_richtap_aei_handle(struct aw_haptic *aw_haptic)
{
	if (!aw_haptic->func->get_irq_state(aw_haptic)) {
		pr_info("%s: aw_haptic rtp fifo almost empty\n", __func__);
		/* modify next line second parameter according to ram.base_addr */
		richtap_update_fifo_data(aw_haptic,
			(aw_haptic->ram.base_addr >> 1));
		while (!aw_haptic->func->rtp_get_fifo_afi(aw_haptic) &&
			atomic_read(&aw_haptic->richtap_rtp_mode) &&
			(aw_haptic->curr_buf->status == MMAP_BUF_DATA_VALID))
			richtap_update_fifo_data(aw_haptic,
				(aw_haptic->ram.base_addr >> 2));
	}
	if (aw_haptic->curr_buf->status == MMAP_BUF_DATA_INVALID) {
		aw_haptic->func->enable_gain(aw_haptic, 0);
		aw_haptic->func->set_rtp_aei(aw_haptic, false);
		atomic_set(&aw_haptic->richtap_rtp_mode, false);
	}

	atomic_set(&aw_haptic->is_process_irq_work, false);
	return IRQ_HANDLED;
}

static irqreturn_t irq_handle(int irq, void *data)
{
	uint8_t glb_state_val = 0;
	uint32_t buf_len = 0;
	struct aw_haptic *aw_haptic = NULL;

	if (!data) {
		aw_err ("data is null");
		return -EINVAL;
	}
	aw_haptic = data;

	aw_info("irq process work");
	atomic_set(&aw_haptic->is_process_irq_work, true);
#ifdef AAC_RICHTAP_SUPPORT
	if (atomic_read(&aw_haptic->richtap_rtp_mode) == true)
		return irq_richtap_aei_handle(aw_haptic);
#endif

	if (!aw_haptic->func->get_irq_state(aw_haptic)) {
		aw_info("aw_haptic rtp fifo almost empty");
		if (aw_haptic->rtp_init) {
			rtp_play(aw_haptic);
		} else {
			aw_info("rtp already play end");
		}
	}
	if (aw_haptic->play_mode != AW_RTP_MODE)
		aw_haptic->func->set_rtp_aei(aw_haptic, false);
	atomic_set(&aw_haptic->is_process_irq_work, false);
	return IRQ_HANDLED;
}

static int irq_config(struct device *dev, struct aw_haptic *aw_haptic)
{
	int ret = -1;
	int irq_flags = 0;

	if (!dev || !aw_haptic) {
		aw_err ("dev or aw_haptic is null");
		return -EINVAL;
	}
	if (gpio_is_valid(aw_haptic->irq_gpio) &&
		!(aw_haptic->flags & AW_FLAG_SKIP_INTERRUPTS)) {
		/* register irq handler */
		aw_haptic->func->interrupt_setup(aw_haptic);
		irq_flags = IRQF_TRIGGER_FALLING | IRQF_ONESHOT;
		ret = devm_request_threaded_irq(dev,
							gpio_to_irq(aw_haptic->irq_gpio),
							NULL, irq_handle, irq_flags,
							"aw_haptic", aw_haptic);
		if (ret != 0) {
			aw_err("failed to request IRQ %d: %d",
					gpio_to_irq(aw_haptic->irq_gpio), ret);
			return ret;
		}
	} else {
		dev_info(dev, "skipping IRQ registration");
		/* disable feature support if gpio was invalid */
		aw_haptic->flags |= AW_FLAG_SKIP_INTERRUPTS;
	}
	return 0;
}

static void ram_load(const struct firmware *cont, void *context)
{
	uint16_t check_sum = 0;
	int i = 0;
	int ret = 0;
	struct aw_haptic *aw_haptic = NULL;
	struct aw_haptic_container *aw_fw;
	static uint8_t retry_cnt = 0;

#ifdef AW_READ_BIN_FLEXBALLY
	static uint8_t load_cont = 0;
	int ram_timer_val = 1000;

	load_cont++;
#endif
	if (!context) {
		aw_err ("context is null");
		return;
	}
	aw_haptic = context;
	if (!cont) {
		aw_err("failed to read %s", aw8692x_ram_name);
		release_firmware(cont);
#ifdef AW_READ_BIN_FLEXBALLY
		if (load_cont <= MAX_LOAD_CNT) {
			schedule_delayed_work(&aw_haptic->ram_work,
								msecs_to_jiffies(ram_timer_val));
			aw_info("start hrtimer:load_cont%d", load_cont);
		}
#endif
		return;
	}
	aw_info("loaded %s - size: %zu", aw8692x_ram_name, cont ? cont->size : 0);
	/* check sum */
	for (i = 2; i < cont->size; i++)
		check_sum += cont->data[i];
	if (check_sum != (uint16_t)((cont->data[0] << 8) | (cont->data[1]))) {
		aw_err("check sum err: check_sum=0x%04x", check_sum);
		release_firmware(cont);
		return;
	}
	aw_info("check sum pass : 0x%04x", check_sum);
	aw_haptic->ram.check_sum = check_sum;

	/* aw ram update */
	aw_info("befor kzalloc");
	aw_fw = kzalloc(cont->size + sizeof(int), GFP_KERNEL);
	if (!aw_fw) {
		release_firmware(cont);
		aw_err("Error allocating memory");
		return;
	}
	aw_info("kzalloc success");
	aw_fw->len = cont->size;
	aw_info("ready to memcpy");
	memcpy(aw_fw->data, cont->data, cont->size);
	aw_info("ready to release_firmware");
	release_firmware(cont);
	aw_info("ready to container_update");
	ret = aw_haptic->func->container_update(aw_haptic, aw_fw);
	if (ret) {
		if (retry_cnt < MAX_RETRY_CNT) {
			retry_cnt++;
			schedule_delayed_work(&aw_haptic->ram_work, msecs_to_jiffies(8000));
		}
		aw_err("%s: ram firmware update failed retry_cnt = %u!", __func__, retry_cnt);
	} else {
		retry_cnt = 0;
		aw_haptic->ram_init = true;
		aw_haptic->ram.len = aw_fw->len - aw_haptic->ram.ram_shift;
		aw_haptic->func->trig_init(aw_haptic);
		aw_info("ram firmware update complete!");
		get_ram_num(aw_haptic);
	}
	kfree(aw_fw);

#ifdef AW_BOOT_OSC_CALI
	rtp_trim_lra_cali(aw_haptic);
#endif
}

static int ram_update(struct aw_haptic *aw_haptic)
{
	int ret;
	if (!aw_haptic) {
		aw_err ("aw_haptic is null");
		return -EINVAL;
	}
	aw_haptic->ram_init = false;
	aw_haptic->rtp_init = false;
	ret = request_firmware_nowait(THIS_MODULE, FW_ACTION_HOTPLUG,
							aw8692x_ram_name, aw_haptic->dev, GFP_KERNEL,
							aw_haptic, ram_load);
	if (ret < 0) {
		aw_info ("request_firmware_nowait failed, ret = %d\n", ret);
		return ret;
	}
	aw_haptic->is_dev_resetting = false;
	return ret;
}

static void ram_work_routine(struct work_struct *work)
{
	struct aw_haptic *aw_haptic = NULL;
	if (!work) {
		aw_err ("work is null");
		return;
	}
	aw_haptic = container_of(work, struct aw_haptic, ram_work.work);

	ram_update(aw_haptic);
}

static void ram_work_init(struct aw_haptic *aw_haptic)
{
	int ram_timer_val = AW_RAM_WORK_DELAY_INTERVAL;

	if (!aw_haptic) {
		aw_err ("aw_haptic is null");
		return;
	}
	aw_haptic->ram_init = false;
	aw_haptic->rtp_init = false;
	INIT_DELAYED_WORK(&aw_haptic->ram_work, ram_work_routine);
	if (vibrator_get_enter_recovery_mode()) {
		aw_info("recovery mode do not need load firmware");
		return;
	}
	schedule_delayed_work(&aw_haptic->ram_work,
						msecs_to_jiffies(ram_timer_val));
}

/*****************************************************
 *
 * haptic audio
 *
 *****************************************************/
static int audio_ctrl_list_ins(struct aw_haptic_audio *haptic_audio,
							struct aw_haptic_ctr *haptic_ctr)
{
	struct aw_haptic_ctr *p_new = NULL;

	if (!haptic_audio || !haptic_ctr) {
		aw_err ("haptic_audio or haptic_ctr is null");
		return -EINVAL;
	}
	p_new = (struct aw_haptic_ctr *)kzalloc(
		sizeof(struct aw_haptic_ctr), GFP_KERNEL);
	if (p_new == NULL) {
		aw_err("kzalloc memory fail");
		return -ENOMEM;
	}
	/* update new list info */
	p_new->cnt = haptic_ctr->cnt;
	p_new->cmd = haptic_ctr->cmd;
	p_new->play = haptic_ctr->play;
	p_new->wavseq = haptic_ctr->wavseq;
	p_new->loop = haptic_ctr->loop;
	p_new->gain = haptic_ctr->gain;
	INIT_LIST_HEAD(&(p_new->list));
	list_add(&(p_new->list), &(haptic_audio->ctr_list));
	return 0;
}

static void audio_ctrl_list_clr(struct aw_haptic_audio *haptic_audio)
{
	struct aw_haptic_ctr *p_ctr = NULL;
	struct aw_haptic_ctr *p_ctr_bak = NULL;

	if (!haptic_audio) {
		aw_err ("haptic_audio is null");
		return;
	}
	list_for_each_entry_safe_reverse(p_ctr, p_ctr_bak,
						&(haptic_audio->ctr_list), list) {
		list_del(&p_ctr->list);
		kfree(p_ctr);
	}
}

static void audio_init(struct aw_haptic *aw_haptic)
{
	aw_dbg("enter!");
	if (!aw_haptic) {
		aw_err ("aw_haptic is null");
		return;
	}
	aw_haptic->func->set_wav_seq(aw_haptic, 0x01, 0x00);
}

static void audio_off(struct aw_haptic *aw_haptic)
{
	aw_info("enter");

	if (!aw_haptic) {
		aw_err ("aw_haptic is null");
		return;
	}
	mutex_lock(&aw_haptic->lock);
	aw_haptic->func->set_gain(aw_haptic, 0x80);
	aw_haptic->func->play_stop(aw_haptic);
	audio_ctrl_list_clr(&aw_haptic->haptic_audio);
	mutex_unlock(&aw_haptic->lock);
}

static enum hrtimer_restart audio_timer_func(struct hrtimer *timer)
{
	struct aw_haptic *aw_haptic = NULL;

	aw_dbg("enter");

	if (!timer) {
		aw_err ("timer is null");
		return 0;
	}
	aw_haptic = container_of(timer, struct aw_haptic, haptic_audio.timer);
	schedule_work(&aw_haptic->haptic_audio.work);

	hrtimer_start(&aw_haptic->haptic_audio.timer,
				ktime_set(aw_haptic->haptic_audio.timer_val / 1000000,
				(aw_haptic->haptic_audio.timer_val % 1000000) *
				1000), HRTIMER_MODE_REL);
	return HRTIMER_NORESTART;
}

static void audio_work_routine(struct work_struct *work)
{
	struct aw_haptic *aw_haptic = NULL;
	struct aw_haptic_audio *haptic_audio = NULL;
	struct aw_haptic_ctr *p_ctr = NULL;
	struct aw_haptic_ctr *p_ctr_bak = NULL;
	uint32_t ctr_list_flag = 0;
	uint32_t ctr_list_input_cnt = 0;
	uint32_t ctr_list_output_cnt = 0;
	uint32_t ctr_list_diff_cnt = 0;
	uint32_t ctr_list_del_cnt = 0;
	int rtp_is_going_on = 0;

	aw_dbg("enter");
	if (!work) {
		aw_err ("work is null");
		return;
	}
	aw_haptic = container_of(work, struct aw_haptic, haptic_audio.work);
	haptic_audio = &(aw_haptic->haptic_audio);
	mutex_lock(&aw_haptic->haptic_audio.lock);
	memset(&aw_haptic->haptic_audio.ctr, 0,
			sizeof(struct aw_haptic_ctr));
	ctr_list_flag = 0;
	list_for_each_entry_safe_reverse(p_ctr, p_ctr_bak,
					&(haptic_audio->ctr_list), list) {
		ctr_list_flag = 1;
		break;
	}
	if (ctr_list_flag == 0)
		aw_info("ctr list empty");
	if (ctr_list_flag == 1) {
		list_for_each_entry_safe(p_ctr, p_ctr_bak,
					&(haptic_audio->ctr_list), list) {
			ctr_list_input_cnt = p_ctr->cnt;
			break;
		}
		list_for_each_entry_safe_reverse(p_ctr, p_ctr_bak,
						&(haptic_audio->ctr_list),
						list) {
			ctr_list_output_cnt = p_ctr->cnt;
			break;
		}
		if (ctr_list_input_cnt > ctr_list_output_cnt) {
			ctr_list_diff_cnt =
				ctr_list_input_cnt - ctr_list_output_cnt;
		}
		if (ctr_list_input_cnt < ctr_list_output_cnt) {
			ctr_list_diff_cnt =
				32 + ctr_list_input_cnt - ctr_list_output_cnt;
		}
		if (ctr_list_diff_cnt > 2) {
			list_for_each_entry_safe_reverse(p_ctr, p_ctr_bak,
								&(haptic_audio->
								ctr_list), list) {
				if ((p_ctr->play == 0)
					&& (AW_CMD_ENABLE ==
					(AW_CMD_HAPTIC & p_ctr->
						cmd))) {
					list_del(&p_ctr->list);
					kfree(p_ctr);
					ctr_list_del_cnt++;
				}
				if (ctr_list_del_cnt == ctr_list_diff_cnt)
					break;
			}
		}
	}
	/* get the last data from list */
	list_for_each_entry_safe_reverse(p_ctr, p_ctr_bak,
					 &(haptic_audio->ctr_list), list) {
		aw_haptic->haptic_audio.ctr.cnt = p_ctr->cnt;
		aw_haptic->haptic_audio.ctr.cmd = p_ctr->cmd;
		aw_haptic->haptic_audio.ctr.play = p_ctr->play;
		aw_haptic->haptic_audio.ctr.wavseq = p_ctr->wavseq;
		aw_haptic->haptic_audio.ctr.loop = p_ctr->loop;
		aw_haptic->haptic_audio.ctr.gain = p_ctr->gain;
		list_del(&p_ctr->list);
		kfree(p_ctr);
		break;
	}
	if (aw_haptic->haptic_audio.ctr.play) {
		aw_info
		("cnt=%d, cmd=%d, play=%d, wavseq=%d, loop=%d, gain=%d",
			aw_haptic->haptic_audio.ctr.cnt,
			aw_haptic->haptic_audio.ctr.cmd,
			aw_haptic->haptic_audio.ctr.play,
			aw_haptic->haptic_audio.ctr.wavseq,
			aw_haptic->haptic_audio.ctr.loop,
			aw_haptic->haptic_audio.ctr.gain);
	}
	rtp_is_going_on = aw_haptic->func->juge_rtp_going(aw_haptic);
	if (rtp_is_going_on) {
		mutex_unlock(&aw_haptic->haptic_audio.lock);
		return;
	}
	mutex_unlock(&aw_haptic->haptic_audio.lock);
	if (aw_haptic->haptic_audio.ctr.cmd == AW_CMD_ENABLE) {
		if (aw_haptic->haptic_audio.ctr.play ==
			AW_PLAY_ENABLE) {
			aw_info("haptic_audio_play_start");
			mutex_lock(&aw_haptic->lock);
			aw_haptic->func->play_stop(aw_haptic);
			aw_haptic->func->play_mode(aw_haptic, AW_RAM_MODE);
			aw_haptic->func->set_wav_seq(aw_haptic, 0x00,
						aw_haptic->haptic_audio.ctr.wavseq);
			aw_haptic->func->set_wav_seq(aw_haptic, 0x01, 0x00);
			aw_haptic->func->set_wav_loop(aw_haptic, 0x00,
						aw_haptic->haptic_audio.ctr.loop);
			aw_haptic->func->set_gain(aw_haptic,
						aw_haptic->haptic_audio.ctr.gain);
			aw_haptic->func->play_go(aw_haptic, true);
			mutex_unlock(&aw_haptic->lock);
		} else if (AW_PLAY_STOP ==
				aw_haptic->haptic_audio.ctr.play) {
			mutex_lock(&aw_haptic->lock);
			aw_haptic->func->play_stop(aw_haptic);
			mutex_unlock(&aw_haptic->lock);
		} else if (AW_PLAY_GAIN ==
				aw_haptic->haptic_audio.ctr.play) {
			mutex_lock(&aw_haptic->lock);
			aw_haptic->func->set_gain(aw_haptic,
						aw_haptic->haptic_audio.ctr.gain);
			mutex_unlock(&aw_haptic->lock);
		}
	}
}

/*****************************************************
 *
 * node
 *
 *****************************************************/
#ifdef TIMED_OUTPUT
static int vibrator_get_time(struct timed_output_dev *dev)
{
	struct aw_haptic *aw_haptic = NULL;

	if (!dev) {
		aw_err ("dev is null");
		return -EINVAL;
	}
	aw_haptic = container_of(dev, struct aw_haptic, vib_dev);
	if (hrtimer_active(&aw_haptic->timer)) {
		ktime_t r = hrtimer_get_remaining(&aw_haptic->timer);

		return ktime_to_ms(r);
	}
	return 0;
}

static void vibrator_enable(struct timed_output_dev *dev, int value)
{
	struct aw_haptic *aw_haptic = NULL;

	aw_info("enter");
	if (!dev) {
		aw_err ("dev is null");
		return;
	}
	aw_haptic = container_of(dev, struct aw_haptic, vib_dev);
	if (!aw_haptic->ram_init) {
		aw_err("ram init failed, not allow to play!");
		return;
	}
	mutex_lock(&aw_haptic->lock);

	aw_haptic->func->upload_lra(aw_haptic, AW_F0_CALI_LRA);
	aw_haptic->func->play_stop(aw_haptic);
	if (value > 0) {
		ram_vbat_comp(aw_haptic, false);
		ram_play(aw_haptic, AW_RAM_MODE);
	}
	mutex_unlock(&aw_haptic->lock);
	aw_info("exit");
}
#else
static enum led_brightness brightness_get(struct led_classdev *cdev)
{
	struct aw_haptic *aw_haptic = NULL;

	if (!cdev) {
		aw_err ("cdev is null");
		return 0;
	}
	aw_haptic = container_of(cdev, struct aw_haptic, vib_dev);
	return aw_haptic->amplitude;
}

static void brightness_set(struct led_classdev *cdev, enum led_brightness level)
{
	struct aw_haptic *aw_haptic = NULL;

	aw_info("enter");
	if (!cdev) {
		aw_err ("cdev is null");
		return;
	}
	aw_haptic = container_of(cdev, struct aw_haptic, vib_dev);
	if (!aw_haptic->ram_init) {
		aw_err("ram init failed, not allow to play!");
		return;
	}
	aw_haptic->amplitude = level;
	mutex_lock(&aw_haptic->lock);
	aw_haptic->func->play_stop(aw_haptic);
	if (aw_haptic->amplitude > 0) {
		aw_haptic->func->upload_lra(aw_haptic, AW_F0_CALI_LRA);
		ram_vbat_comp(aw_haptic, false);
		ram_play(aw_haptic, AW_RAM_MODE);
	}
	mutex_unlock(&aw_haptic->lock);
}
#endif

static ssize_t state_show(struct device *dev, struct device_attribute *attr,
						char *buf)
{
	cdev_t *cdev = NULL;
	struct aw_haptic *aw_haptic = NULL;

	if (!dev || !buf) {
		aw_err ("dev or buf is null");
		return -EINVAL;
	}
	cdev = dev_get_drvdata(dev);
	if (!cdev) {
		aw_err ("cdev is null");
		return -EINVAL;
	}
	aw_haptic = container_of(cdev, struct aw_haptic, vib_dev);

	return snprintf(buf, PAGE_SIZE, "state = %d\n", aw_haptic->state);
}

static ssize_t state_store(struct device *dev, struct device_attribute *attr,
						const char *buf, size_t count)
{
	return count;
}

static ssize_t duration_show(struct device *dev, struct device_attribute *attr,
							char *buf)
{
	cdev_t *cdev = NULL;
	struct aw_haptic *aw_haptic = NULL;
	ktime_t time_rem;
	s64 time_ms = 0;

	if (!dev || !buf) {
		aw_err ("dev or buf is null");
		return -EINVAL;
	}
	cdev = dev_get_drvdata(dev);
	if (!cdev) {
		aw_err ("cdev is null");
		return -EINVAL;
	}
	aw_haptic = container_of(cdev, struct aw_haptic, vib_dev);

	if (hrtimer_active(&aw_haptic->timer)) {
		time_rem = hrtimer_get_remaining(&aw_haptic->timer);
		time_ms = ktime_to_ms(time_rem);
	}
	return snprintf(buf, PAGE_SIZE, "duration = %lldms\n", time_ms);
}

static ssize_t duration_store(struct device *dev, struct device_attribute *attr,
							const char *buf, size_t count)
{
	cdev_t *cdev = NULL;
	struct aw_haptic *aw_haptic = NULL;
	uint32_t val = 0;
	int rc = 0;

	if (!dev || !buf) {
		aw_err ("dev or buf is null");
		return -EINVAL;
	}
	cdev = dev_get_drvdata(dev);
	if (!cdev) {
		aw_err ("cdev is null");
		return -EINVAL;
	}
	aw_haptic = container_of(cdev, struct aw_haptic, vib_dev);

	rc = kstrtouint(buf, 0, &val);
	if (rc < 0)
		return rc;
	/* setting 0 on duration is NOP for now */
	if (val <= 0)
		return count;
	aw_haptic->duration = val;
	aw_haptic->cust_boost_on = 0;
	aw_info("long vibrator duration time = %d", aw_haptic->duration);
	aw_haptic->ignore_stop_cmd =
		(aw_haptic->duration <= AW_IGNORE_STOP_CMD_TIME) ? true : false;

	__pm_wakeup_event(aw_haptic->ws, aw_haptic->duration + HAPTIC_WAKE_LOCK_GAP);
	return count;
}

static ssize_t activate_show(struct device *dev, struct device_attribute *attr,
							char *buf)
{
	cdev_t *cdev = NULL;
	struct aw_haptic *aw_haptic = NULL;

	if (!dev || !buf) {
		aw_err ("dev or buf is null");
		return -EINVAL;
	}
	cdev = dev_get_drvdata(dev);
	if (!cdev) {
		aw_err ("cdev is null");
		return -EINVAL;
	}
	aw_haptic = container_of(cdev, struct aw_haptic, vib_dev);

	return snprintf(buf, PAGE_SIZE, "activate = %d\n", aw_haptic->state);
}

static ssize_t activate_store(struct device *dev, struct device_attribute *attr,
							const char *buf, size_t count)
{
	cdev_t *cdev = NULL;
	struct aw_haptic *aw_haptic = NULL;
	uint32_t val = 0;
	int rc = 0;

	if (!dev || !buf) {
		aw_err ("dev or buf is null");
		return -EINVAL;
	}
	cdev = dev_get_drvdata(dev);
	if (!cdev) {
		aw_err ("cdev is null");
		return -EINVAL;
	}
	aw_haptic = container_of(cdev, struct aw_haptic, vib_dev);

	rc = kstrtouint(buf, 0, &val);
	if (rc < 0)
		return rc;
	aw_info("value=%d", val);
	if (!aw_haptic->ram_init) {
		aw_err("ram init failed, not allow to play!");
		return count;
	}
	if (val == 0 && aw_haptic->ignore_stop_cmd == true) {
		aw_info("playing is short orig play, no need stop");
		return count;
	}
	mutex_lock(&aw_haptic->lock);
	if (hrtimer_active(&aw_haptic->timer))
		hrtimer_cancel(&aw_haptic->timer);
	aw_haptic->state = val;
	aw_haptic->effect_mode = LONG_VIB_RAM_MODE;
	aw_haptic->index = LONG_VIB_BOOST_EFFECT_ID;
	aw_haptic->gain = HAPTIC_MAX_GAIN_REG;
	mutex_unlock(&aw_haptic->lock);
	schedule_work(&aw_haptic->vibrator_work);

	return count;
}

static ssize_t activate_mode_show(struct device *dev,
				struct device_attribute *attr, char *buf)
{
	cdev_t *cdev = NULL;
	struct aw_haptic *aw_haptic = NULL;

	if (!dev || !buf) {
		aw_err ("dev or buf is null");
		return -EINVAL;
	}
	cdev = dev_get_drvdata(dev);
	if (!cdev) {
		aw_err ("cdev is null");
		return -EINVAL;
	}
	aw_haptic = container_of(cdev, struct aw_haptic, vib_dev);

	return snprintf(buf, PAGE_SIZE, "activate_mode = %d\n",
			aw_haptic->activate_mode);
}

static ssize_t activate_mode_store(struct device *dev,
						struct device_attribute *attr,
						const char *buf, size_t count)
{
	cdev_t *cdev = NULL;
	struct aw_haptic *aw_haptic = NULL;
	uint32_t val = 0;
	int rc = 0;

	if (!dev || !buf) {
		aw_err ("dev or buf is null");
		return -EINVAL;
	}
	cdev = dev_get_drvdata(dev);
	if (!cdev) {
		aw_err ("cdev is null");
		return -EINVAL;
	}
	aw_haptic = container_of(cdev, struct aw_haptic, vib_dev);

	rc = kstrtouint(buf, 0, &val);
	aw_info("activate_mode = %d", val);
	if (rc < 0)
		return rc;
	mutex_lock(&aw_haptic->lock);
	aw_haptic->activate_mode = val;
	mutex_unlock(&aw_haptic->lock);
	return count;
}

static ssize_t index_show(struct device *dev, struct device_attribute *attr,
						char *buf)
{
	cdev_t *cdev = NULL;
	struct aw_haptic *aw_haptic = NULL;

	if (!dev || !buf) {
		aw_err ("dev or buf is null");
		return -EINVAL;
	}
	cdev = dev_get_drvdata(dev);
	if (!cdev) {
		aw_err ("cdev is null");
		return -EINVAL;
	}
	aw_haptic = container_of(cdev, struct aw_haptic, vib_dev);

	aw_haptic->func->get_wav_seq(aw_haptic, 1);
	aw_haptic->index = aw_haptic->seq[0];
	return snprintf(buf, PAGE_SIZE, "index = %d\n", aw_haptic->index);
}

static ssize_t index_store(struct device *dev, struct device_attribute *attr,
						const char *buf, size_t count)
{
	cdev_t *cdev = NULL;
	struct aw_haptic *aw_haptic = NULL;
	uint32_t val = 0;
	int rc = 0;

	if (!dev || !buf) {
		aw_err ("dev or buf is null");
		return -EINVAL;
	}
	cdev = dev_get_drvdata(dev);
	if (!cdev) {
		aw_err ("cdev is null");
		return -EINVAL;
	}
	aw_haptic = container_of(cdev, struct aw_haptic, vib_dev);

	rc = kstrtouint(buf, 0, &val);
	if (rc < 0)
		return rc;
	if (val > aw_haptic->ram.ram_num) {
		aw_err("input value out of range!");
		return count;
	}
	aw_info("value=%d", val);
	mutex_lock(&aw_haptic->lock);
	aw_haptic->index = val;
	aw_haptic->func->set_repeat_seq(aw_haptic, aw_haptic->index);
	mutex_unlock(&aw_haptic->lock);
	return count;
}

static ssize_t vmax_show(struct device *dev, struct device_attribute *attr,
						char *buf)
{
	cdev_t *cdev = NULL;
	struct aw_haptic *aw_haptic = NULL;

	if (!dev || !buf) {
		aw_err ("dev or buf is null");
		return -EINVAL;
	}
	cdev = dev_get_drvdata(dev);
	if (!cdev) {
		aw_err ("cdev is null");
		return -EINVAL;
	}
	aw_haptic = container_of(cdev, struct aw_haptic, vib_dev);

	return snprintf(buf, PAGE_SIZE, "vmax = 0x%02X\n", aw_haptic->vmax);
}

static ssize_t vmax_store(struct device *dev, struct device_attribute *attr,
						const char *buf, size_t count)
{
	cdev_t *cdev = NULL;
	struct aw_haptic *aw_haptic = NULL;
	uint32_t val = 0;
	int rc = 0;

	if (!dev || !buf) {
		aw_err ("dev or buf is null");
		return -EINVAL;
	}
	cdev = dev_get_drvdata(dev);
	if (!cdev) {
		aw_err ("cdev is null");
		return -EINVAL;
	}
	aw_haptic = container_of(cdev, struct aw_haptic, vib_dev);

	rc = kstrtouint(buf, 0, &val);
	if (rc < 0)
		return rc;
	aw_info("value=%d", val);
	mutex_lock(&aw_haptic->lock);
	aw_haptic->vmax = val;
	aw_haptic->func->set_bst_vol(aw_haptic, aw_haptic->vmax);
	mutex_unlock(&aw_haptic->lock);
	return count;
}

static ssize_t gain_show(struct device *dev, struct device_attribute *attr,
						char *buf)
{
	cdev_t *cdev = NULL;
	struct aw_haptic *aw_haptic = NULL;

	if (!dev || !buf) {
		aw_err ("dev or buf is null");
		return -EINVAL;
	}
	cdev = dev_get_drvdata(dev);
	if (!cdev) {
		aw_err ("cdev is null");
		return -EINVAL;
	}
	aw_haptic = container_of(cdev, struct aw_haptic, vib_dev);

	return snprintf(buf, PAGE_SIZE, "gain = 0x%02X\n", aw_haptic->gain);
}

static ssize_t gain_store(struct device *dev, struct device_attribute *attr,
						const char *buf, size_t count)
{
	cdev_t *cdev = NULL;
	struct aw_haptic *aw_haptic = container_of(cdev, struct aw_haptic,
											vib_dev);
	uint32_t val = 0;
	int rc = 0;

	if (!dev || !buf) {
		aw_err ("dev or buf is null");
		return -EINVAL;
	}
	cdev = dev_get_drvdata(dev);
	if (!cdev) {
		aw_err ("cdev is null");
		return -EINVAL;
	}
	aw_haptic = container_of(cdev, struct aw_haptic, vib_dev);

	rc = kstrtouint(buf, 0, &val);
	if (rc < 0)
		return rc;
	aw_info("value=0x%02x", val);
	mutex_lock(&aw_haptic->lock);
	aw_haptic->gain = val;
	aw_haptic->func->set_gain(aw_haptic, aw_haptic->gain);
	mutex_unlock(&aw_haptic->lock);
	return count;
}

static ssize_t seq_show(struct device *dev, struct device_attribute *attr,
			char *buf)
{
	uint8_t i = 0;
	size_t count = 0;
	cdev_t *cdev = NULL;
	struct aw_haptic *aw_haptic = NULL;

	if (!dev || !buf) {
		aw_err ("dev or buf is null");
		return -EINVAL;
	}
	cdev = dev_get_drvdata(dev);
	if (!cdev) {
		aw_err ("cdev is null");
		return -EINVAL;
	}
	aw_haptic = container_of(cdev, struct aw_haptic, vib_dev);

	aw_haptic->func->get_wav_seq(aw_haptic, AW_SEQUENCER_SIZE);
	for (i = 0; i < AW_SEQUENCER_SIZE; i++)
		count += snprintf(buf + count, PAGE_SIZE - count,
						"seq%d = %d\n", i + 1, aw_haptic->seq[i]);
	return count;
}

static ssize_t seq_store(struct device *dev, struct device_attribute *attr,
			 const char *buf, size_t count)
{
	cdev_t *cdev = NULL;
	struct aw_haptic *aw_haptic = NULL;
	uint32_t databuf[2] = { 0, 0 };

	if (!dev || !buf) {
		aw_err ("dev or buf is null");
		return -EINVAL;
	}
	cdev = dev_get_drvdata(dev);
	if (!cdev) {
		aw_err ("cdev is null");
		return -EINVAL;
	}
	aw_haptic = container_of(cdev, struct aw_haptic, vib_dev);

	if (sscanf(buf, "%x %x", &databuf[0], &databuf[1]) == 2) {
		if (databuf[0] >= AW_SEQUENCER_SIZE ||
			databuf[1] > aw_haptic->ram.ram_num) {
			aw_err("input value out of range!");
			return count;
		}
		aw_info("seq%d=0x%02X", databuf[0], databuf[1]);
		mutex_lock(&aw_haptic->lock);
		aw_haptic->seq[databuf[0]] = (uint8_t)databuf[1];
		aw_haptic->func->set_wav_seq(aw_haptic, (uint8_t)databuf[0],
									aw_haptic->seq[databuf[0]]);
		mutex_unlock(&aw_haptic->lock);
	}
	return count;
}

static ssize_t reg_show(struct device *dev, struct device_attribute *attr,
			char *buf)
{
	cdev_t *cdev = NULL;
	struct aw_haptic *aw_haptic = NULL;
	ssize_t len = 0;

	if (!dev || !buf) {
		aw_err ("dev or buf is null");
		return -EINVAL;
	}
	cdev = dev_get_drvdata(dev);
	if (!cdev) {
		aw_err ("cdev is null");
		return -EINVAL;
	}
	aw_haptic = container_of(cdev, struct aw_haptic, vib_dev);

	len = aw_haptic->func->get_reg(aw_haptic, len, buf);
	return len;
}

static ssize_t reg_store(struct device *dev, struct device_attribute *attr,
						const char *buf, size_t count)
{
	uint8_t val = 0;
	cdev_t *cdev = NULL;
	struct aw_haptic *aw_haptic = NULL;
	uint32_t databuf[2] = { 0, 0 };

	if (!dev || !buf) {
		aw_err ("dev or buf is null");
		return -EINVAL;
	}
	cdev = dev_get_drvdata(dev);
	if (!cdev) {
		aw_err ("cdev is null");
		return -EINVAL;
	}
	aw_haptic = container_of(cdev, struct aw_haptic, vib_dev);

	if (sscanf(buf, "%x %x", &databuf[0], &databuf[1]) == 2) {
		val = (uint8_t)databuf[1];
#ifdef AW8692X_DRIVER_ENABLE
		if (aw_haptic->func == aw8692x_func() &&
			(uint8_t)databuf[0] == AW8692X_REG_PLAYCFG1 &&
			val < AW8692X_BIT_PLAYCFG1_BST_VOUT_6V)
			val = AW8692X_BIT_PLAYCFG1_BST_VOUT_6V;
#endif
		i2c_w_bytes(aw_haptic, (uint8_t)databuf[0], &val,
					AW_I2C_BYTE_ONE);
	}
	return count;
}

static ssize_t rtp_show(struct device *dev, struct device_attribute *attr,
			char *buf)
{
	cdev_t *cdev = NULL;
	struct aw_haptic *aw_haptic = NULL;
	ssize_t len = 0;

	if (!dev || !buf) {
		aw_err ("dev or buf is null");
		return -EINVAL;
	}
	cdev = dev_get_drvdata(dev);
	if (!cdev) {
		aw_err ("cdev is null");
		return -EINVAL;
	}
	aw_haptic = container_of(cdev, struct aw_haptic, vib_dev);

	len += snprintf(buf + len, PAGE_SIZE - len, "rtp_cnt = %d\n",
			aw_haptic->rtp_cnt);
	return len;
}

static ssize_t aw_rtp_store(struct device *dev, struct device_attribute *attr,
							const char *buf, size_t count)
{
	cdev_t *cdev = NULL;
	struct aw_haptic *aw_haptic = NULL;
	uint32_t val = 0;
	//uint32_t rtp_num_max = sizeof(aw_rtp_name) / AW_RTP_NAME_MAX;
	int rc = 0;
	int len = 0;

	if (!dev || !buf) {
		aw_err ("dev or buf is null");
		return -EINVAL;
	}
	cdev = dev_get_drvdata(dev);
	if (!cdev) {
		aw_err ("cdev is null");
		return -EINVAL;
	}
	aw_haptic = container_of(cdev, struct aw_haptic, vib_dev);

	rc = kstrtouint(buf, 0, &val);
	if (rc < 0) {
		aw_err("kstrtouint fail");
		return rc;
	}
	mutex_lock(&aw_haptic->lock);
	aw_haptic->func->play_stop(aw_haptic);
	aw_haptic->func->set_rtp_aei(aw_haptic, false);
	aw_haptic->func->irq_clear(aw_haptic);

	len += snprintf(aw_rtp_idx_name, AW_RTP_NAME_MAX - 1, "aw_%u.bin", val);
	aw_info(" get rtp name = %s, len = %d\n", aw_rtp_idx_name, len);
	schedule_work(&aw_haptic->rtp_work);
	mutex_unlock(&aw_haptic->lock);
	return count;
}

static ssize_t ram_update_show(struct device *dev,
			struct device_attribute *attr, char *buf)
{
	cdev_t *cdev = NULL;
	struct aw_haptic *aw_haptic = NULL;
	ssize_t len = 0;

	if (!dev || !buf) {
		aw_err ("dev or buf is null");
		return -EINVAL;
	}
	cdev = dev_get_drvdata(dev);
	if (!cdev) {
		aw_err ("cdev is null");
		return -EINVAL;
	}
	aw_haptic = container_of(cdev, struct aw_haptic, vib_dev);

	/* RAMINIT Enable */
	aw_haptic->func->ram_init(aw_haptic, true);
	aw_haptic->func->play_stop(aw_haptic);
	aw_haptic->func->set_ram_addr(aw_haptic);
	len += snprintf(buf + len, PAGE_SIZE - len, "aw_haptic_ram:\n");
	len += aw_haptic->func->get_ram_data(aw_haptic, buf);
	len += snprintf(buf + len, PAGE_SIZE - len, "\n");
	/* RAMINIT Disable */
	aw_haptic->func->ram_init(aw_haptic, false);
	return len;
}

static ssize_t ram_update_store(struct device *dev,
				struct device_attribute *attr,
				const char *buf, size_t count)
{
	cdev_t *cdev = NULL;
	struct aw_haptic *aw_haptic = NULL;
	uint32_t val = 0;
	int rc = 0;

	if (!dev || !buf) {
		aw_err ("dev or buf is null");
		return -EINVAL;
	}
	cdev = dev_get_drvdata(dev);
	if (!cdev) {
		aw_err ("cdev is null");
		return -EINVAL;
	}
	aw_haptic = container_of(cdev, struct aw_haptic, vib_dev);

	rc = kstrtouint(buf, 0, &val);
	if (rc < 0)
		return rc;
	if (val)
		ram_update(aw_haptic);
	return count;
}

static ssize_t ram_num_show(struct device *dev, struct device_attribute *attr,
							char *buf)
{
	cdev_t *cdev = NULL;
	struct aw_haptic *aw_haptic = NULL;
	ssize_t len = 0;

	if (!dev || !buf) {
		aw_err ("dev or buf is null");
		return -EINVAL;
	}
	cdev = dev_get_drvdata(dev);
	if (!cdev) {
		aw_err ("cdev is null");
		return -EINVAL;
	}
	aw_haptic = container_of(cdev, struct aw_haptic, vib_dev);

	get_ram_num(aw_haptic);
	len += snprintf(buf + len, PAGE_SIZE - len,
			"ram_num = %d\n", aw_haptic->ram.ram_num);
	return len;
}

static ssize_t f0_show(struct device *dev, struct device_attribute *attr,
						char *buf)
{
	cdev_t *cdev = NULL;
	struct aw_haptic *aw_haptic = NULL;
	ssize_t len = 0;

	if (!dev || !buf) {
		aw_err ("dev or buf is null");
		return -EINVAL;
	}
	cdev = dev_get_drvdata(dev);
	if (!cdev) {
		aw_err ("cdev is null");
		return -EINVAL;
	}
	aw_haptic = container_of(cdev, struct aw_haptic, vib_dev);

	mutex_lock(&aw_haptic->lock);
	aw_haptic->func->upload_lra(aw_haptic, AW_WRITE_ZERO);
	aw_haptic->func->get_f0(aw_haptic);
	aw_haptic->func->upload_lra(aw_haptic, AW_F0_CALI_LRA);
	mutex_unlock(&aw_haptic->lock);
	len += snprintf(buf + len, PAGE_SIZE - len,
			"lra_f0 = %d cont_f0 = %d\n", aw_haptic->f0,
			aw_haptic->cont_f0);
	return len;
}

static ssize_t cali_val_show(struct device *dev,
				struct device_attribute *attr, char *buf)
{
	struct aw_haptic *aw_haptic = NULL;
	ssize_t len = 0;

	cdev_t *cdev = dev_get_drvdata(dev);
	aw_haptic = container_of(cdev, struct aw_haptic, vib_dev);

	if (!buf || !aw_haptic)
		return 0;
	pr_info("%s f0 val = %u\n", __func__, aw_haptic->f0);

	if (aw_haptic->f0_val_err < 0)
		aw_haptic->f0 = 0;
	pr_info("%s cal f0 val = %u\n", __func__, aw_haptic->f0);

#ifdef F0_CONT_F0
	len += snprintf(buf + len, PAGE_SIZE - len, "aw_haptic f0 = %d\n", aw_haptic->f0);
#endif
	len += snprintf(buf + len, PAGE_SIZE - len, "%d", aw_haptic->f0);
	return len;
}

static ssize_t cali_show(struct device *dev, struct device_attribute *attr,
						char *buf)
{
	cdev_t *cdev = NULL;
	struct aw_haptic *aw_haptic = NULL;
	ssize_t len = 0;
	int cal_rst;

	if (!dev || !buf) {
		aw_err ("dev or buf is null");
		return -EINVAL;
	}
	cdev = dev_get_drvdata(dev);
	if (!cdev) {
		aw_err ("cdev is null");
		return -EINVAL;
	}
	aw_haptic = container_of(cdev, struct aw_haptic, vib_dev);

#ifdef F0_CONT_F0
	len += snprintf(buf + len, PAGE_SIZE - len, "aw_haptic f0 = %d\n", aw_haptic->f0);
#endif
	if (aw_haptic->f0 > AW_F0_CALI_THER_H ||
		aw_haptic->f0 < AW_F0_CALI_THER_L)
		cal_rst = 0;
	else
		cal_rst = 1;

	if (aw_haptic->f0_val_err < 0) {
		cal_rst = 0;
		pr_info("%s cal err\n", __func__);
	}
	len += snprintf(buf + len, PAGE_SIZE - len, "%d", cal_rst);
	return len;
}

static ssize_t cali_store(struct device *dev, struct device_attribute *attr,
						const char *buf, size_t count)
{
	cdev_t *cdev = NULL;
	struct aw_haptic *aw_haptic = NULL;
	uint32_t val = 0;
	int rc = 0;

	if (!dev || !buf) {
		aw_err ("dev or buf is null");
		return -EINVAL;
	}
	cdev = dev_get_drvdata(dev);
	if (!cdev) {
		aw_err ("cdev is null");
		return -EINVAL;
	}
	aw_haptic = container_of(cdev, struct aw_haptic, vib_dev);

	rc = kstrtouint(buf, 0, &val);
	if (rc < 0)
		return rc;
	if (val) {
		aw_haptic->f0_val_err = 0;
		mutex_lock(&aw_haptic->lock);
		f0_cali(aw_haptic);
		mutex_unlock(&aw_haptic->lock);
	}
	return count;
}

static ssize_t cont_show(struct device *dev, struct device_attribute *attr,
						char *buf)
{
	cdev_t *cdev = NULL;
	struct aw_haptic *aw_haptic = NULL;
	ssize_t len = 0;

	if (!dev || !buf) {
		aw_err ("dev or buf is null");
		return -EINVAL;
	}
	cdev = dev_get_drvdata(dev);
	if (!cdev) {
		aw_err ("cdev is null");
		return -EINVAL;
	}
	aw_haptic = container_of(cdev, struct aw_haptic, vib_dev);

	aw_haptic->func->read_f0(aw_haptic);
	len += snprintf(buf + len, PAGE_SIZE - len,
			"cont_f0 = %d\n", aw_haptic->cont_f0);
	return len;
}

static ssize_t cont_store(struct device *dev, struct device_attribute *attr,
						const char *buf, size_t count)
{
	cdev_t *cdev = NULL;
	struct aw_haptic *aw_haptic = NULL;
	uint32_t val = 0;
	int rc = 0;

	if (!dev || !buf) {
		aw_err ("dev or buf is null");
		return -EINVAL;
	}
	cdev = dev_get_drvdata(dev);
	if (!cdev) {
		aw_err ("cdev is null");
		return -EINVAL;
	}
	aw_haptic = container_of(cdev, struct aw_haptic, vib_dev);

	rc = kstrtouint(buf, 0, &val);
	if (rc < 0)
		return rc;

	aw_haptic->func->play_stop(aw_haptic);
	if (val) {
		aw_haptic->func->upload_lra(aw_haptic, AW_F0_CALI_LRA);
		aw_haptic->func->cont_config(aw_haptic);
	}
	return count;
}

static ssize_t vbat_monitor_show(struct device *dev,
			struct device_attribute *attr, char *buf)
{
	cdev_t *cdev = NULL;
	struct aw_haptic *aw_haptic = NULL;
	ssize_t len = 0;

	if (!dev || !buf) {
		aw_err ("dev or buf is null");
		return -EINVAL;
	}
	cdev = dev_get_drvdata(dev);
	if (!cdev) {
		aw_err ("cdev is null");
		return -EINVAL;
	}
	aw_haptic = container_of(cdev, struct aw_haptic, vib_dev);

	mutex_lock(&aw_haptic->lock);
	aw_haptic->func->get_vbat(aw_haptic);
	len += snprintf(buf + len, PAGE_SIZE - len, "vbat_monitor = %d\n",
			aw_haptic->vbat);
	mutex_unlock(&aw_haptic->lock);

	return len;
}

static ssize_t lra_resistance_show(struct device *dev,
				struct device_attribute *attr, char *buf)
{
	cdev_t *cdev = NULL;
	struct aw_haptic *aw_haptic = NULL;
	ssize_t len = 0;

	if (!dev || !buf) {
		aw_err ("dev or buf is null");
		return -EINVAL;
	}
	cdev = dev_get_drvdata(dev);
	if (!cdev) {
		aw_err ("cdev is null");
		return -EINVAL;
	}
	aw_haptic = container_of(cdev, struct aw_haptic, vib_dev);

	aw_haptic->func->get_lra_resistance(aw_haptic);
	len += snprintf(buf + len, PAGE_SIZE - len, "lra_resistance = %d\n",
			aw_haptic->lra);
	return len;
}

static ssize_t auto_boost_show(struct device *dev,
			struct device_attribute *attr, char *buf)
{
	cdev_t *cdev = NULL;
	struct aw_haptic *aw_haptic = NULL;
	ssize_t len = 0;

	if (!dev || !buf) {
		aw_err ("dev or buf is null");
		return -EINVAL;
	}
	cdev = dev_get_drvdata(dev);
	if (!cdev) {
		aw_err ("cdev is null");
		return -EINVAL;
	}
	aw_haptic = container_of(cdev, struct aw_haptic, vib_dev);

	len += snprintf(buf + len, PAGE_SIZE - len, "auto_boost = %d\n",
			aw_haptic->auto_boost);

	return len;
}

static ssize_t auto_boost_store(struct device *dev,
				struct device_attribute *attr,
				const char *buf, size_t count)
{
	cdev_t *cdev = NULL;
	struct aw_haptic *aw_haptic = NULL;
	uint32_t val = 0;
	int rc = 0;

	if (!dev || !buf) {
		aw_err ("dev or buf is null");
		return -EINVAL;
	}
	cdev = dev_get_drvdata(dev);
	if (!cdev) {
		aw_err ("cdev is null");
		return -EINVAL;
	}
	aw_haptic = container_of(cdev, struct aw_haptic, vib_dev);

	rc = kstrtouint(buf, 0, &val);
	if (rc < 0)
		return rc;

	mutex_lock(&aw_haptic->lock);
	aw_haptic->func->play_stop(aw_haptic);
	aw_haptic->func->auto_bst_enable(aw_haptic, val);
	mutex_unlock(&aw_haptic->lock);

	return count;
}

static ssize_t prct_mode_show(struct device *dev, struct device_attribute *attr,
							char *buf)
{
	cdev_t *cdev = NULL;
	struct aw_haptic *aw_haptic = NULL;
	ssize_t len = 0;
	uint8_t reg_val = 0;

	if (!dev || !buf) {
		aw_err ("dev or buf is null");
		return -EINVAL;
	}
	cdev = dev_get_drvdata(dev);
	if (!cdev) {
		aw_err ("cdev is null");
		return -EINVAL;
	}
	aw_haptic = container_of(cdev, struct aw_haptic, vib_dev);

	reg_val = aw_haptic->func->get_prctmode(aw_haptic);
	len += snprintf(buf + len, PAGE_SIZE - len, "prctmode = %d\n", reg_val);
	return len;
}

static ssize_t prct_mode_store(struct device *dev,
					struct device_attribute *attr,
					const char *buf, size_t count)
{
	cdev_t *cdev = NULL;
	struct aw_haptic *aw_haptic = NULL;
	uint32_t databuf[2] = { 0, 0 };
	uint32_t addr = 0;
	uint32_t val = 0;

	if (!dev || !buf) {
		aw_err ("dev or buf is null");
		return -EINVAL;
	}
	cdev = dev_get_drvdata(dev);
	if (!cdev) {
		aw_err ("cdev is null");
		return -EINVAL;
	}
	aw_haptic = container_of(cdev, struct aw_haptic, vib_dev);

	if (sscanf(buf, "%x %x", &databuf[0], &databuf[1]) == 2) {
		addr = databuf[0];
		val = databuf[1];
		mutex_lock(&aw_haptic->lock);
		aw_haptic->func->protect_config(aw_haptic, addr, val);
		mutex_unlock(&aw_haptic->lock);
	}
	return count;
}

static ssize_t ram_vbat_comp_show(struct device *dev,
			struct device_attribute *attr, char *buf)
{
	cdev_t *cdev = NULL;
	struct aw_haptic *aw_haptic = NULL;
	ssize_t len = 0;

	if (!dev || !buf) {
		aw_err ("dev or buf is null");
		return -EINVAL;
	}
	cdev = dev_get_drvdata(dev);
	if (!cdev) {
		aw_err ("cdev is null");
		return -EINVAL;
	}
	aw_haptic = container_of(cdev, struct aw_haptic, vib_dev);

	len += snprintf(buf + len, PAGE_SIZE - len,
			"ram_vbat_comp = %d\n",
			aw_haptic->ram_vbat_comp);

	return len;
}

static ssize_t ram_vbat_comp_store(struct device *dev,
						struct device_attribute *attr,
						const char *buf, size_t count)
{
	cdev_t *cdev = NULL;
	struct aw_haptic *aw_haptic = NULL;
	uint32_t val = 0;
	int rc = 0;

	if (!dev || !buf) {
		aw_err ("dev or buf is null");
		return -EINVAL;
	}
	cdev = dev_get_drvdata(dev);
	if (!cdev) {
		aw_err ("cdev is null");
		return -EINVAL;
	}
	aw_haptic = container_of(cdev, struct aw_haptic, vib_dev);

	rc = kstrtouint(buf, 0, &val);
	if (rc < 0)
		return rc;
	mutex_lock(&aw_haptic->lock);
	if (val)
		aw_haptic->ram_vbat_comp = AW_RAM_VBAT_COMP_ENABLE;
	else
		aw_haptic->ram_vbat_comp = AW_RAM_VBAT_COMP_DISABLE;
	mutex_unlock(&aw_haptic->lock);

	return count;
}

static ssize_t osc_cali_show(struct device *dev, struct device_attribute *attr,
							char *buf)
{
	cdev_t *cdev = NULL;
	struct aw_haptic *aw_haptic = NULL;
	ssize_t len = 0;

	if (!dev || !buf) {
		aw_err ("dev or buf is null");
		return -EINVAL;
	}
	cdev = dev_get_drvdata(dev);
	if (!cdev) {
		aw_err ("cdev is null");
		return -EINVAL;
	}
	aw_haptic = container_of(cdev, struct aw_haptic, vib_dev);

	len += snprintf(buf + len, PAGE_SIZE - len, "osc_cali_data = 0x%02X\n",
			aw_haptic->osc_cali_data);

	return len;
}

static ssize_t osc_cali_store(struct device *dev, struct device_attribute *attr,
							const char *buf, size_t count)
{
	cdev_t *cdev = NULL;
	struct aw_haptic *aw_haptic = NULL;
	uint32_t val = 0;
	int rc = 0;

	if (!dev || !buf) {
		aw_err ("dev or buf is null");
		return -EINVAL;
	}
	cdev = dev_get_drvdata(dev);
	if (!cdev) {
		aw_err ("cdev is null");
		return -EINVAL;
	}
	aw_haptic = container_of(cdev, struct aw_haptic, vib_dev);

	rc = kstrtouint(buf, 0, &val);
	if (rc < 0)
		return rc;
	mutex_lock(&aw_haptic->lock);
	if (val == 1) {
		rtp_trim_lra_cali(aw_haptic);
	} else if (val == 2) {
		aw_haptic->func->upload_lra(aw_haptic, AW_OSC_CALI_LRA);
		rtp_osc_cali(aw_haptic);
	}
	mutex_unlock(&aw_haptic->lock);

	return count;
}

static ssize_t haptic_audio_show(struct device *dev,
			struct device_attribute *attr, char *buf)
{
	cdev_t *cdev = NULL;
	struct aw_haptic *aw_haptic = NULL;

	ssize_t len = 0;

	if (!dev || !buf) {
		aw_err ("dev or buf is null");
		return -EINVAL;
	}
	cdev = dev_get_drvdata(dev);
	if (!cdev) {
		aw_err ("cdev is null");
		return -EINVAL;
	}
	aw_haptic = container_of(cdev, struct aw_haptic, vib_dev);

	len += snprintf(buf + len, PAGE_SIZE - len, "%d\n",
			aw_haptic->haptic_audio.ctr.cnt);
	return len;
}

static ssize_t haptic_audio_store(struct device *dev,
						struct device_attribute *attr,
						const char *buf, size_t count)
{
	cdev_t *cdev = NULL;
	struct aw_haptic *aw_haptic = NULL;

	uint32_t databuf[6] = { 0 };
	struct aw_haptic_ctr *hap_ctr = NULL;

	if (!dev || !buf) {
		aw_err ("dev or buf is null");
		return -EINVAL;
	}
	cdev = dev_get_drvdata(dev);
	if (!cdev) {
		aw_err ("cdev is null");
		return -EINVAL;
	}
	aw_haptic = container_of(cdev, struct aw_haptic, vib_dev);

	if (!aw_haptic->ram_init) {
		aw_err("ram init failed, not allow to play!");
		return count;
	}
	if (sscanf(buf, "%d %d %d %d %d %d", &databuf[0], &databuf[1],
			&databuf[2], &databuf[3], &databuf[4], &databuf[5]) == 6) {
		if (databuf[2]) {
			aw_info("cnt=%d, cmd=%d, play=%d, wavseq=%d, loop=%d, gain=%d",
				databuf[0], databuf[1], databuf[2], databuf[3],
				databuf[4], databuf[5]);
		}

		hap_ctr = (struct aw_haptic_ctr *)kzalloc(
			sizeof(struct aw_haptic_ctr), GFP_KERNEL);
		if (hap_ctr == NULL) {
			aw_err("kzalloc memory fail");
			return count;
		}
		mutex_lock(&aw_haptic->haptic_audio.lock);
		hap_ctr->cnt = (uint8_t)databuf[0];
		hap_ctr->cmd = (uint8_t)databuf[1];
		hap_ctr->play = (uint8_t)databuf[2];
		hap_ctr->wavseq = (uint8_t)databuf[3];
		hap_ctr->loop = (uint8_t)databuf[4];
		hap_ctr->gain = (uint8_t)databuf[5];
		audio_ctrl_list_ins(&aw_haptic->haptic_audio, hap_ctr);
		if (hap_ctr->cmd == 0xff) {
			aw_info("haptic_audio stop");
			if (hrtimer_active(&aw_haptic->haptic_audio.timer)) {
				aw_info("cancel haptic_audio_timer");
				hrtimer_cancel(&aw_haptic->haptic_audio.timer);
				aw_haptic->haptic_audio.ctr.cnt = 0;
				audio_off(aw_haptic);
			}
		} else {
			if (hrtimer_active(&aw_haptic->haptic_audio.timer)) {
				/* */
			} else {
				aw_info("start haptic_audio_timer");
				audio_init(aw_haptic);
				hrtimer_start(&aw_haptic->haptic_audio.timer,
							ktime_set(aw_haptic->haptic_audio.
							delay_val / 1000000,
							(aw_haptic->haptic_audio.
								delay_val % 1000000) *
							1000),
							HRTIMER_MODE_REL);
			}
		}
		mutex_unlock(&aw_haptic->haptic_audio.lock);
		kfree(hap_ctr);
	}
	return count;
}

static ssize_t haptic_audio_time_show(struct device *dev,
				struct device_attribute *attr, char *buf)
{
	cdev_t *cdev = NULL;
	struct aw_haptic *aw_haptic = NULL;

	ssize_t len = 0;

	if (!dev || !buf) {
		aw_err ("dev or buf is null");
		return -EINVAL;
	}
	cdev = dev_get_drvdata(dev);
	if (!cdev) {
		aw_err ("cdev is null");
		return -EINVAL;
	}
	aw_haptic = container_of(cdev, struct aw_haptic, vib_dev);

	len += snprintf(buf + len, PAGE_SIZE - len,
			"haptic_audio.delay_val=%dus\n",
			aw_haptic->haptic_audio.delay_val);
	len += snprintf(buf + len, PAGE_SIZE - len,
			"haptic_audio.timer_val=%dus\n",
			aw_haptic->haptic_audio.timer_val);
	return len;
}

static ssize_t haptic_audio_time_store(struct device *dev,
							struct device_attribute *attr,
							const char *buf, size_t count)
{
	cdev_t *cdev = NULL;
	struct aw_haptic *aw_haptic = NULL;

	uint32_t databuf[2] = { 0 };

	if (!dev || !buf) {
		aw_err ("dev or buf is null");
		return -EINVAL;
	}
	cdev = dev_get_drvdata(dev);
	if (!cdev) {
		aw_err ("cdev is null");
		return -EINVAL;
	}
	aw_haptic = container_of(cdev, struct aw_haptic, vib_dev);

	if (sscanf(buf, "%d %d", &databuf[0], &databuf[1]) == 2) {
		aw_haptic->haptic_audio.delay_val = databuf[0];
		aw_haptic->haptic_audio.timer_val = databuf[1];
	}
	return count;
}

static ssize_t gun_type_show(struct device *dev, struct device_attribute *attr,
							char *buf)
{
	cdev_t *cdev = NULL;
	struct aw_haptic *aw_haptic = NULL;

	if (!dev || !buf) {
		aw_err ("dev or buf is null");
		return -EINVAL;
	}
	cdev = dev_get_drvdata(dev);
	if (!cdev) {
		aw_err ("cdev is null");
		return -EINVAL;
	}
	aw_haptic = container_of(cdev, struct aw_haptic, vib_dev);

	return snprintf(buf, PAGE_SIZE, "0x%02x\n", aw_haptic->gun_type);
}

static ssize_t gun_type_store(struct device *dev, struct device_attribute *attr,
							const char *buf, size_t count)
{
	cdev_t *cdev = NULL;
	struct aw_haptic *aw_haptic = NULL;

	uint32_t val = 0;
	int rc = 0;

	if (!dev || !buf) {
		aw_err ("dev or buf is null");
		return -EINVAL;
	}
	cdev = dev_get_drvdata(dev);
	if (!cdev) {
		aw_err ("cdev is null");
		return -EINVAL;
	}
	aw_haptic = container_of(cdev, struct aw_haptic, vib_dev);

	rc = kstrtouint(buf, 0, &val);
	if (rc < 0)
		return rc;
	aw_dbg("value=%d", val);
	mutex_lock(&aw_haptic->lock);
	aw_haptic->gun_type = val;
	mutex_unlock(&aw_haptic->lock);
	return count;
}

static ssize_t bullet_nr_show(struct device *dev, struct device_attribute *attr,
							char *buf)
{
	cdev_t *cdev = NULL;
	struct aw_haptic *aw_haptic = NULL;

	if (!dev || !buf) {
		aw_err ("dev or buf is null");
		return -EINVAL;
	}
	cdev = dev_get_drvdata(dev);
	if (!cdev) {
		aw_err ("cdev is null");
		return -EINVAL;
	}
	aw_haptic = container_of(cdev, struct aw_haptic, vib_dev);

	return snprintf(buf, PAGE_SIZE, "0x%02x\n", aw_haptic->bullet_nr);
}

static ssize_t bullet_nr_store(struct device *dev,
					struct device_attribute *attr,
					const char *buf, size_t count)
{
	cdev_t *cdev = NULL;
	struct aw_haptic *aw_haptic = NULL;

	uint32_t val = 0;
	int rc = 0;

	if (!dev || !buf) {
		aw_err ("dev or buf is null");
		return -EINVAL;
	}
	cdev = dev_get_drvdata(dev);
	if (!cdev) {
		aw_err ("cdev is null");
		return -EINVAL;
	}
	aw_haptic = container_of(cdev, struct aw_haptic, vib_dev);

	rc = kstrtouint(buf, 0, &val);
	if (rc < 0)
		return rc;
	aw_dbg("value=%d", val);
	mutex_lock(&aw_haptic->lock);
	aw_haptic->bullet_nr = val;
	mutex_unlock(&aw_haptic->lock);
	return count;
}

static ssize_t f0_save_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	ssize_t len = 0;
	cdev_t *cdev = NULL;
	struct aw_haptic *aw_haptic = NULL;

	if (!dev || !buf) {
		aw_err ("dev or buf is null");
		return -EINVAL;
	}
	cdev = dev_get_drvdata(dev);
	if (!cdev) {
		aw_err ("cdev is null");
		return -EINVAL;
	}
	aw_haptic = container_of(cdev, struct aw_haptic, vib_dev);

	len += snprintf(buf + len, PAGE_SIZE - len, "f0_cali_data = 0x%02X\n",
			aw_haptic->f0_cali_data);

	return len;
}

static ssize_t f0_save_store(struct device *dev, struct device_attribute *attr,
							const char *buf, size_t count)
{
	uint32_t val = 0;
	int rc = 0;
	cdev_t *cdev = NULL;
	struct aw_haptic *aw_haptic = NULL;

	if (!dev || !buf) {
		aw_err ("dev or buf is null");
		return -EINVAL;
	}
	cdev = dev_get_drvdata(dev);
	if (!cdev) {
		aw_err ("cdev is null");
		return -EINVAL;
	}
	aw_haptic = container_of(cdev, struct aw_haptic, vib_dev);

	rc = kstrtouint(buf, 0, &val);
	if (rc < 0)
		return rc;
	aw_haptic->f0_cali_data = val;
	return count;
}


static ssize_t osc_save_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	ssize_t len = 0;
	cdev_t *cdev = NULL;
	struct aw_haptic *aw_haptic = NULL;

	if (!dev || !buf) {
		aw_err ("dev or buf is null");
		return -EINVAL;
	}
	cdev = dev_get_drvdata(dev);
	if (!cdev) {
		aw_err ("cdev is null");
		return -EINVAL;
	}
	aw_haptic = container_of(cdev, struct aw_haptic, vib_dev);

	len += snprintf(buf + len, PAGE_SIZE - len, "osc_cali_data = 0x%02X\n",
			aw_haptic->osc_cali_data);

	return len;
}

static ssize_t osc_save_store(struct device *dev,
					struct device_attribute *attr,
					const char *buf, size_t count)
{
	uint32_t val = 0;
	int rc = 0;
	cdev_t *cdev = NULL;
	struct aw_haptic *aw_haptic = NULL;

	if (!dev || !buf) {
		aw_err ("dev or buf is null");
		return -EINVAL;
	}
	cdev = dev_get_drvdata(dev);
	if (!cdev) {
		aw_err ("cdev is null");
		return -EINVAL;
	}
	aw_haptic = container_of(cdev, struct aw_haptic, vib_dev);

	rc = kstrtouint(buf, 0, &val);
	if (rc < 0)
		return rc;
	aw_haptic->osc_cali_data = val;
	return count;
}

static ssize_t trig_active_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	cdev_t *cdev = NULL;
	struct aw_haptic *aw_haptic = NULL;
	int gpio = -1;
	uint32_t index = 0;
	int rc = 0;

	if (!dev || !buf) {
		aw_err("dev or buf is null");
		return -EINVAL;
	}
	cdev = dev_get_drvdata(dev);
	if (!cdev) {
		aw_err("cdev is null");
		return -EINVAL;
	}
	aw_haptic = container_of(cdev, struct aw_haptic, vib_dev);

	rc = kstrtouint(buf, 0, &index);
	if (rc < 0)
		return rc;

	if (index >= AW_TRIG_NUM) {
		aw_err("input wrong index");
		return -ERANGE;
	}
	gpio = aw_haptic->trig_gpios[index];
	aw_info("index = %d, gpio = %d", index, gpio);

	if (!gpio_is_valid(gpio)) {
		aw_err("invalue gpio");
		return -EINVAL;
	}
	if (gpio_get_value(gpio))
		gpio_set_value(gpio, 0);
	else
		gpio_set_value(gpio, 1);
	return count;
}
static DEVICE_ATTR(f0, S_IWUSR | S_IRUGO, f0_show, NULL);
static DEVICE_ATTR(seq, S_IWUSR | S_IRUGO, seq_show, seq_store);
static DEVICE_ATTR(reg, S_IWUSR | S_IRUGO, reg_show, reg_store);
static DEVICE_ATTR(vmax, S_IWUSR | S_IRUGO, vmax_show, vmax_store);
static DEVICE_ATTR(gain, S_IWUSR | S_IRUGO, gain_show, gain_store);
static DEVICE_ATTR(rtp, S_IWUSR | S_IRUGO, rtp_show, aw_rtp_store);
static DEVICE_ATTR(vibrator_calib, S_IWUSR | S_IRUGO, cali_show, cali_store);
static DEVICE_ATTR(cali_val, S_IWUSR | S_IRUGO, cali_val_show, NULL);
static DEVICE_ATTR(cont, S_IWUSR | S_IRUGO, cont_show, cont_store);
static DEVICE_ATTR(state, S_IWUSR | S_IRUGO, state_show, state_store);
static DEVICE_ATTR(index, S_IWUSR | S_IRUGO, index_show, index_store);
static DEVICE_ATTR(ram_num, S_IWUSR | S_IRUGO, ram_num_show, NULL);
static DEVICE_ATTR(duration, S_IWUSR | S_IRUGO, duration_show, duration_store);
static DEVICE_ATTR(activate, S_IWUSR | S_IRUGO, activate_show, activate_store);
static DEVICE_ATTR(osc_cali, S_IWUSR | S_IRUGO, osc_cali_show, osc_cali_store);
static DEVICE_ATTR(gun_type, S_IWUSR | S_IRUGO, gun_type_show, gun_type_store);
static DEVICE_ATTR(prctmode, S_IWUSR | S_IRUGO, prct_mode_show,
				prct_mode_store);
static DEVICE_ATTR(bullet_nr, S_IWUSR | S_IRUGO, bullet_nr_show,
				bullet_nr_store);
static DEVICE_ATTR(auto_boost, S_IWUSR | S_IRUGO, auto_boost_show,
				auto_boost_store);
static DEVICE_ATTR(ram_update, S_IWUSR | S_IRUGO, ram_update_show,
				ram_update_store);
static DEVICE_ATTR(haptic_audio, S_IWUSR | S_IRUGO, haptic_audio_show,
				haptic_audio_store);
static DEVICE_ATTR(vbat_monitor, S_IWUSR | S_IRUGO, vbat_monitor_show, NULL);
static DEVICE_ATTR(activate_mode, S_IWUSR | S_IRUGO, activate_mode_show,
				activate_mode_store);
static DEVICE_ATTR(ram_vbat_comp, S_IWUSR | S_IRUGO, ram_vbat_comp_show,
				ram_vbat_comp_store);
static DEVICE_ATTR(lra_resistance, S_IWUSR | S_IRUGO, lra_resistance_show,
				NULL);
static DEVICE_ATTR(haptic_audio_time, S_IWUSR | S_IRUGO, haptic_audio_time_show,
				haptic_audio_time_store);
static DEVICE_ATTR(osc_save, S_IWUSR | S_IRUGO, osc_save_show, osc_save_store);
static DEVICE_ATTR(f0_save, S_IWUSR | S_IRUGO, f0_save_show, f0_save_store);
static DEVICE_ATTR(trig_active, S_IWUSR, NULL, trig_active_store);

static struct attribute *vibrator_attributes[] = {
	&dev_attr_state.attr,
	&dev_attr_duration.attr,
	&dev_attr_activate.attr,
	&dev_attr_activate_mode.attr,
	&dev_attr_index.attr,
	&dev_attr_vmax.attr,
	&dev_attr_gain.attr,
	&dev_attr_seq.attr,
	&dev_attr_reg.attr,
	&dev_attr_rtp.attr,
	&dev_attr_ram_update.attr,
	&dev_attr_ram_num.attr,
	&dev_attr_f0.attr,
	&dev_attr_f0_save.attr,
	&dev_attr_vibrator_calib.attr,
	&dev_attr_cali_val.attr,
	&dev_attr_cont.attr,
	&dev_attr_vbat_monitor.attr,
	&dev_attr_lra_resistance.attr,
	&dev_attr_auto_boost.attr,
	&dev_attr_prctmode.attr,
	&dev_attr_ram_vbat_comp.attr,
	&dev_attr_osc_cali.attr,
	&dev_attr_osc_save.attr,
	&dev_attr_haptic_audio.attr,
	&dev_attr_haptic_audio_time.attr,
	&dev_attr_gun_type.attr,
	&dev_attr_bullet_nr.attr,
	&dev_attr_trig_active.attr,
	NULL
};

static struct attribute_group vibrator_attribute_group = {
	.attrs = vibrator_attributes
};

static void notify_register(struct aw_haptic *aw_haptic)
{
	int ret = 0;

	if (aw_haptic->boot_power_set == SUPPORT_BOOST_5V) {
		aw_haptic->nb.notifier_call = bypbst_notify_call;
		ret = power_event_bnc_register(POWER_BNT_BYPASSBOOST, &aw_haptic->nb);
		if (ret)
			aw_err("%s: Failed to register POWER_BNT_BYPASSBOOST notifier\n", __func__);
	}

	if (aw_haptic->info.is_support_charging_notify) {
		aw_haptic->event_connect_nb.notifier_call = charging_notify_call;
		ret = power_event_bnc_register(POWER_BNT_CONNECT, &aw_haptic->event_connect_nb);
		if (ret)
			aw_err("%s: power_event_bnc_register fail\n", __func__);
	}
}

static int vibrator_init(struct aw_haptic *aw_haptic)
{
	int ret = 0;

	aw_info("enter");
	if (!aw_haptic) {
		aw_err ("aw_haptic is null");
		return -EINVAL;
	}
#ifdef TIMED_OUTPUT
	aw_info("TIMED_OUT FRAMEWORK!");
	aw_haptic->vib_dev.name = "vibrator";
	ret = strncmp(aw_haptic->name, "vibrator1", sizeof("vibrator1"));
	if (!ret)
		aw_haptic->vib_dev.name = "vibrator1";

	aw_haptic->vib_dev.get_time = vibrator_get_time;
	aw_haptic->vib_dev.enable = vibrator_enable;

	ret = timed_output_dev_register(&(aw_haptic->vib_dev));
	if (ret < 0) {
		aw_err("fail to create timed output dev");
		return ret;
	}
	ret = sysfs_create_group(&aw_haptic->vib_dev.dev->kobj,
				 &vibrator_attribute_group);
	if (ret < 0) {
		aw_err("error creating sysfs attr files");
		return ret;
	}
#else
	aw_info("loaded in leds_cdev framework!");
	aw_haptic->vib_dev.name = "vibrator";
	ret = strncmp(aw_haptic->name, "vibrator1", sizeof("vibrator1"));
	if (!ret)
		aw_haptic->vib_dev.name = "vibrator1";

	aw_haptic->vib_dev.brightness_get = brightness_get;
	aw_haptic->vib_dev.brightness_set = brightness_set;
	aw_haptic->vib_dev.default_trigger = "hw_vb_trigger";
	ret = devm_led_classdev_register(&aw_haptic->i2c->dev, &aw_haptic->vib_dev);
	if (ret < 0) {
		aw_err("fail to create led dev");
		return ret;
	}
	ret = sysfs_create_group(&aw_haptic->vib_dev.dev->kobj,
				&vibrator_attribute_group);
	if (ret < 0) {
		aw_err("error creating sysfs attr files");
		return ret;
	}
#endif
	hrtimer_init(&aw_haptic->timer, CLOCK_MONOTONIC, HRTIMER_MODE_REL);
	aw_haptic->timer.function = vibrator_timer_func;
	INIT_WORK(&aw_haptic->vibrator_work, vibrator_work_routine);
	INIT_WORK(&aw_haptic->rtp_work, rtp_work_routine);
	INIT_WORK(&aw_haptic->trig_work, trig_work_routine);
	INIT_WORK(&aw_haptic->notifier_call_work, bypbst_notify_call_work_handler);
	mutex_init(&aw_haptic->lock);
	mutex_init(&aw_haptic->rtp_lock);

	return 0;
}

#if defined(CONFIG_HIBERNATION) || defined(CONFIG_SYS_PM_ULSR)
static void s4_hw_reset(struct aw_haptic *aw_haptic)
{
	aw_info("enter");
	if (!aw_haptic) {
		aw_err ("aw_haptic is null");
		return;
	}
	if (!(strncmp(aw_haptic->vib_dev.name, "vibrator1", sizeof("vibrator1"))) &&
		gpio_is_valid(aw_haptic->reset_gpio)) {
		gpio_set_value_cansleep(aw_haptic->reset_gpio, 0);
		usleep_range(AW_DELAY_1000US, AW_DELAY_2000US);
		gpio_set_value_cansleep(aw_haptic->reset_gpio, 1);
		usleep_range(AW_DELAY_8000US, AW_DELAY_8500US);
	}
}

static void haptic_reconfig_delayed_work(struct work_struct *work)
{
	int ret = 0;
	uint32_t reg = 0;
	struct aw_haptic *aw_haptic = NULL;

	aw_info("enter");
	if (!work) {
		aw_err ("work is null");
		return;
	}
	aw_haptic = container_of(work, struct aw_haptic, pm_work.work);
	if (!aw_haptic)
		return;
	s4_hw_reset(aw_haptic);
	ret = read_chipid(aw_haptic, &reg);
	if (ret < 0)
		aw_err("read chip id fail: %d", ret);
	sw_reset(aw_haptic);
	/* haptic init */
	mutex_lock(&aw_haptic->lock);
	haptic_base_reg_init(aw_haptic);
	aw_haptic->func->set_bst_peak_cur(aw_haptic);
	aw_haptic->func->set_bst_vol(aw_haptic, aw_haptic->vmax);
	aw_haptic->func->offset_cali(aw_haptic);
	mutex_unlock(&aw_haptic->lock);
	ram_update(aw_haptic);
}

static void haptic_pm_notifier_sub(struct aw_haptic *aw_haptic)
{
	if (!aw_haptic) {
		aw_err ("aw_haptic is null");
		return;
	}
	if (!strncmp(aw_haptic->vib_dev.name, "vibrator1", sizeof("vibrator1")))
		schedule_delayed_work(&aw_haptic->pm_work, msecs_to_jiffies(AW_WORK_DELAY_500MS));
	else if (!strncmp(aw_haptic->vib_dev.name, "vibrator", sizeof("vibrator")))
		schedule_delayed_work(&aw_haptic->pm_work, msecs_to_jiffies(AW_WORK_DELAY_1S));
}
#endif

#ifdef CONFIG_HIBERNATION
static int haptic_pm_notifier(struct notifier_block *nb,
	unsigned long foo, void *bar)
{
	struct aw_haptic *aw_haptic = container_of(nb, struct aw_haptic, pm_notify);
	if (!aw_haptic) {
		aw_err ("aw_haptic is null");
		return 0;
	}
	switch (foo) {
	case PM_HIBERNATION_PREPARE: /* Going to hibernate */
		aw_info("going to hibernate\n");
		break;
	case PM_POST_HIBERNATION: /* Hibernation finished */
		aw_info("exit s4 haptic init begin\n");
		haptic_pm_notifier_sub(aw_haptic);
		aw_info("exit s4 haptic init end\n");
		break;
	case PM_RESTORE_PREPARE: /* Going to restore a saved image */
	case PM_POST_RESTORE: /* Restore failed */
		break;
	default:
		break;
	}
	return 0;
}

static void haptic_trig_s4_init(struct aw_haptic *aw_haptic)
{
	if (!aw_haptic) {
		aw_err ("aw_haptic is null");
		return;
	}
	aw_haptic->pm_notify.notifier_call = haptic_pm_notifier;
	register_pm_notifier(&aw_haptic->pm_notify);
}
#endif

static void haptic_init(struct aw_haptic *aw_haptic)
{
	int ret;
	aw_info("enter");
	if (!aw_haptic) {
		aw_err ("aw_haptic is null");
		return;
	}

	if (!strncmp(aw_haptic->name, "vibrator1", sizeof("vibrator1")))
		ret = misc_register(&aw_haptic1_misc);
	else
		ret = misc_register(&aw_haptic_misc);

	if (ret) {
		aw_err("misc fail\n");
		return;
	}

	/* haptic audio */
	aw_haptic->haptic_audio.delay_val = 1;
	aw_haptic->haptic_audio.timer_val = 21318;
	INIT_LIST_HEAD(&(aw_haptic->haptic_audio.ctr_list));
	hrtimer_init(&aw_haptic->haptic_audio.timer, CLOCK_MONOTONIC,
				HRTIMER_MODE_REL);
	aw_haptic->haptic_audio.timer.function = audio_timer_func;
	INIT_WORK(&aw_haptic->haptic_audio.work, audio_work_routine);
	mutex_init(&aw_haptic->haptic_audio.lock);
	INIT_LIST_HEAD(&(aw_haptic->haptic_audio.list));

	/* haptic init */
	mutex_lock(&aw_haptic->lock);
	aw_haptic->bullet_nr = 0;
	aw_haptic->gun_type = 0xff;
	aw_haptic->rtp_routine_on = 0;
	aw_haptic->activate_mode = aw_haptic->info.mode;
	aw_haptic->func->play_mode(aw_haptic, AW_STANDBY_MODE);
	aw_haptic->func->set_pwm(aw_haptic, AW_PWM_12K);
	/* misc value init */
	aw_haptic->func->misc_para_init(aw_haptic);

	aw_haptic->func->set_bst_peak_cur(aw_haptic);
	aw_haptic->func->set_bst_vol(aw_haptic, aw_haptic->vmax);
	aw_haptic->func->protect_config(aw_haptic, 0x01, 0x00);
	aw_haptic->func->auto_bst_enable(aw_haptic,
					aw_haptic->info.is_enabled_auto_bst);
	aw_haptic->func->offset_cali(aw_haptic);
	/* vbat compensation */
	aw_haptic->func->vbat_mode_config(aw_haptic, AW_CONT_VBAT_HW_COMP_MODE);
	aw_haptic->ram_vbat_comp = AW_RAM_VBAT_COMP_ENABLE;

	mutex_unlock(&aw_haptic->lock);

	/* f0 calibration */
	mutex_lock(&aw_haptic->lock);
	if (aw_haptic->common_reset_pin == 0)
		f0_cali(aw_haptic);
	mutex_unlock(&aw_haptic->lock);
}
static int trig_gpio_request(struct device *dev, struct aw_haptic *aw_haptic)
{
	int ret;
	char buf[MAX_STR_SIZE] = {0};

	for (int i = 0; i < AW_TRIG_NUM; i++) {
		ret = snprintf_s(buf, MAX_STR_SIZE, MAX_STR_SIZE - 1, "aw_trig_gpio_%d", i);
		if (ret < 0)
			aw_err("snprintf_s error:%d\n", ret);
		ret = devm_gpio_request_one(dev, aw_haptic->trig_gpios[i], GPIOF_OUT_INIT_LOW, buf);
		if (ret) {
			aw_err("trig_gpio %d request failed", aw_haptic->trig_gpios[i]);
			break;
		}
	}
	return ret;
}

static int aw_i2c_probe(struct i2c_client *i2c, const struct i2c_device_id *id)
{
	int ret = 0;
	struct aw_haptic *aw_haptic;
	struct device_node *np = NULL;

	aw_info("enter");
	if (!i2c) {
		aw_err ("i2c error");
		return -EINVAL;
	}
	np = i2c->dev.of_node;
	if (!i2c_check_functionality(i2c->adapter, I2C_FUNC_I2C)) {
		aw_err("check_functionality failed");
		return -EIO;
	}

	aw_haptic = devm_kzalloc(&i2c->dev, sizeof(struct aw_haptic),
				GFP_KERNEL);
	if (aw_haptic == NULL)
		return -ENOMEM;

	aw_haptic->probe_succ = false;
	aw_haptic->dev = &i2c->dev;
	aw_haptic->i2c = i2c;

	i2c_set_clientdata(i2c, aw_haptic);
	dev_set_drvdata(&i2c->dev, aw_haptic);

	/* aw_haptic rst & int */
	if (np) {
		ret = parse_dt_gpio(&i2c->dev, aw_haptic, np);
		if (ret) {
			aw_err("failed to parse gpio");
			goto err_parse_dt;
		}
		ret = parse_trig_dt_config(aw_haptic, np);
		if (ret) {
			aw_err("failed to parse trig gpio");
			goto err_parse_trig_dt;
		}
	} else {
		aw_haptic->reset_gpio = -1;
		aw_haptic->irq_gpio = -1;
		for (int i = 0; i < AW_TRIG_NUM; i++)
			aw_haptic->trig_gpios[i] = -1;
	}
	ret = parse_sub_haptic_dt_config(aw_haptic, np);
	if (ret) {
		aw_err("failed to parse sub haptic config");
		goto err_parse_sub_haptic_dt;
	}
	if (aw_haptic->info.boost_fw) {
		if (gpio_is_valid(aw_haptic->info.boost_en)) {
			ret = devm_gpio_request_one(&i2c->dev, aw_haptic->info.boost_en,
									GPIOF_OUT_INIT_HIGH, "aw869xx_boost_en");
			if (ret) {
				aw_err("boost en gpio request failed");
				goto err_boost_en_request;
			}
			gpio_set_value_cansleep(aw_haptic->info.boost_en, 1);
			usleep_range(3500, 4000);
		}
	}

	if (!(aw_haptic->common_reset_pin && m_common_reset_gpio_reseted) &&
		gpio_is_valid(aw_haptic->reset_gpio)) {
		ret = devm_gpio_request_one(&i2c->dev, aw_haptic->reset_gpio,
									GPIOF_OUT_INIT_LOW, "aw_rst");
		if (ret) {
			aw_err("rst request failed");
			goto err_reset_gpio_request;
		}
	}

#ifdef AW_ENABLE_PIN_CONTROL
	aw_haptic->pinctrl = devm_pinctrl_get(&i2c->dev);
	if (IS_ERR(aw_haptic->pinctrl)) {
		if (PTR_ERR(aw_haptic->pinctrl) == -EPROBE_DEFER) {
			aw_err("pinctrl not ready");
			ret = -EPROBE_DEFER;
			goto err_reset_gpio_request;
		}
		aw_err("Target does not use pinctrl");
		aw_haptic->pinctrl = NULL;
		ret = -EINVAL;
		goto err_reset_gpio_request;
	}
	aw_haptic->pinctrl_state = pinctrl_lookup_state(aw_haptic->pinctrl,
							"irq_active");
	if (IS_ERR(aw_haptic->pinctrl_state)) {
		aw_err("cannot find pinctrl state");
		ret = -EINVAL;
		goto err_reset_gpio_request;
	} else {
		pinctrl_select_state(aw_haptic->pinctrl,
							aw_haptic->pinctrl_state);
	}
#endif

	if (gpio_is_valid(aw_haptic->irq_gpio)) {
		ret = devm_gpio_request_one(&i2c->dev, aw_haptic->irq_gpio,
									GPIOF_DIR_IN, "aw_int");
		if (ret) {
			aw_err("int request failed");
			goto err_irq_gpio_request;
		}
	}
	if (aw_haptic->trig_enable) {
		ret = trig_gpio_request(&i2c->dev, aw_haptic);
		if (ret) {
			aw_err("int request failed");
			goto err_trig_gpio_request;
		}
	}

	/* aw_haptic chip id */
	ret = parse_chipid(aw_haptic);
	if (ret < 0) {
		aw_err("read_chipid failed ret=%d", ret);
		goto err_id;
	}
	sw_reset(aw_haptic);

	aw_haptic->ws = wakeup_source_register(aw_haptic->dev, "aw_haptic");
	if (!aw_haptic->ws) {
		aw_err("failed to wakeup_source_register: %d\n", ret);
		goto err_wakeup_source_register;
	}

	ret = ctrl_init(aw_haptic, &i2c->dev);
	if (ret < 0) {
		aw_err("ctrl_init failed ret=%d", ret);
		goto err_ctrl_init;
	}

#ifdef AW_CHECK_QUALIFY
	ret = aw_haptic->func->check_qualify(aw_haptic);
	if (ret < 0) {
		aw_err("qualify check failed ret=%d", ret);
		goto err_ctrl_init;
	}
#endif
	aw_haptic->func->parse_dt(&i2c->dev, aw_haptic, np);

#ifdef AW_SND_SOC_CODEC
	aw_haptic->func->snd_soc_init(&i2c->dev);
#endif

	/* aw_haptic irq */
	ret = irq_config(&i2c->dev, aw_haptic);
	if (ret != 0) {
		aw_err("irq_config failed ret=%d", ret);
		goto err_irq_config;
	}
	atomic_set(&aw_haptic->is_process_irq_work, false);
#ifdef AAC_RICHTAP_SUPPORT
	ret = richtap_rtp_config_init(aw_haptic);
	if (ret < 0)
		goto richtap_err;
#endif
	if (of_device_is_compatible(aw_haptic->i2c->dev.of_node, "awinic,haptic1_hv"))
		aw_haptic1_file = aw_haptic;
	else
		aw_haptic_file = aw_haptic;
	notify_register(aw_haptic);
	vibrator_init(aw_haptic);
	haptic_init(aw_haptic);
	aw_haptic->func->creat_node(aw_haptic);
	ram_work_init(aw_haptic);
#ifdef CONFIG_HIBERNATION
	haptic_trig_s4_init(aw_haptic);
#endif
#if defined(CONFIG_HIBERNATION) || defined(CONFIG_SYS_PM_ULSR)
	INIT_DELAYED_WORK(&aw_haptic->pm_work, haptic_reconfig_delayed_work);
#endif
	aw_haptic->probe_succ = true;
#if defined(CONFIG_HUAWEI_OHOS_DSM) || defined(CONFIG_HUAWEI_DSM)
	if (!vib_dclient) {
		vib_dclient = dsm_register_client(&dsm_vibrator);
		if (vib_dclient == NULL)
			aw_err("vib_dclient is NULL\n");
		else
			aw_info("drv2605 dsm register success\n");
	}
#endif
	aw_info("probe completed successfully!");
	return 0;

#ifdef AAC_RICHTAP_SUPPORT
richtap_err:
#endif
err_id:
err_wakeup_source_register:
err_ctrl_init:
	wakeup_source_unregister(aw_haptic->ws);
err_irq_config:
	if (gpio_is_valid(aw_haptic->irq_gpio))
		devm_gpio_free(&i2c->dev, aw_haptic->irq_gpio);
err_trig_gpio_request:
err_irq_gpio_request:
	if (gpio_is_valid(aw_haptic->reset_gpio))
		devm_gpio_free(&i2c->dev, aw_haptic->reset_gpio);

err_boost_en_request:
	if (gpio_is_valid(aw_haptic->info.boost_en))
		devm_gpio_free(&i2c->dev, aw_haptic->info.boost_en);

err_parse_sub_haptic_dt:
err_parse_trig_dt:
err_parse_dt:
err_reset_gpio_request:
	devm_kfree(&i2c->dev, aw_haptic);
	aw_haptic = NULL;
	return ret;
}

static int aw_remove(struct i2c_client *i2c)
{
	struct aw_haptic *aw_haptic = i2c_get_clientdata(i2c);

	aw_info("enter");
	cancel_delayed_work_sync(&aw_haptic->ram_work);
	cancel_work_sync(&aw_haptic->haptic_audio.work);
	hrtimer_cancel(&aw_haptic->haptic_audio.timer);
	cancel_work_sync(&aw_haptic->rtp_work);
	cancel_work_sync(&aw_haptic->vibrator_work);
	cancel_work_sync(&aw_haptic->trig_work);
	cancel_work_sync(&aw_haptic->notifier_call_work);
	hrtimer_cancel(&aw_haptic->timer);
	mutex_destroy(&aw_haptic->lock);
	mutex_destroy(&aw_haptic->rtp_lock);
	mutex_destroy(&aw_haptic->haptic_audio.lock);
#ifdef TIMED_OUTPUT
	timed_output_dev_unregister(&aw_haptic->vib_dev);
#endif
	devm_free_irq(&i2c->dev, gpio_to_irq(aw_haptic->irq_gpio), aw_haptic);
	if (gpio_is_valid(aw_haptic->irq_gpio))
		devm_gpio_free(&i2c->dev, aw_haptic->irq_gpio);
#ifdef AW_SND_SOC_CODEC
	snd_soc_unregister_codec(&i2c->dev);
#endif
	if (gpio_is_valid(aw_haptic->reset_gpio))
		devm_gpio_free(&i2c->dev, aw_haptic->reset_gpio);

	if (gpio_is_valid(aw_haptic->info.boost_en))
		devm_gpio_free(&i2c->dev, aw_haptic->info.boost_en);

	if (!strncmp(aw_haptic->name, "vibrator1", sizeof("vibrator1")))
		misc_deregister(&aw_haptic1_misc);
	else
		misc_deregister(&aw_haptic_misc);

	return 0;
}

static int aw_i2c_suspend(struct device *dev)
{
	int ret = 0;
	struct aw_haptic *aw_haptic = dev_get_drvdata(dev);
	unsigned int cnt = HAPTIC_SR_MAX_WAIT_CNT;

	aw_info("enter");
#ifdef AAC_RICHTAP_SUPPORT
	while (cnt && atomic_read(&aw_haptic->is_process_irq_work)) {
		usleep_range(2000, 2500);
		cnt--;
	}
	pr_info("%s wait 2*%u suspend time enter\n", __func__, cnt);
	if (atomic_read(&aw_haptic->richtap_rtp_mode)) {
		richtap_config_stop_mode(aw_haptic);
		pr_info("%s stop richtap rtp mode", __func__);
		return 0;
	}
#endif

	mutex_lock(&aw_haptic->lock);
	aw_haptic->func->play_stop(aw_haptic);
	mutex_unlock(&aw_haptic->lock);

	return ret;
}

static int aw_i2c_resume(struct device *dev)
{
	int ret = 0;
	aw_info("enter");
#ifdef CONFIG_SYS_PM_ULSR
	if (!dev) {
		aw_err("dev is null");
		return -EINVAL;
	}
	struct aw_haptic *aw_haptic = dev_get_drvdata(dev);
	if (!aw_haptic) {
		aw_err ("aw_haptic is null");
		return -EINVAL;
	}
	if (is_ulsr()) {
		aw_info("exit ulsr haptic init begin\n");
		haptic_pm_notifier_sub(aw_haptic);
		aw_info("exit ulsr haptic init end\n");
	}
#endif
	return ret;
}

static SIMPLE_DEV_PM_OPS(aw_pm_ops, aw_i2c_suspend, aw_i2c_resume);

static const struct i2c_device_id aw_i2c_id[] = {
	{AW_I2C_NAME, 0},
	{}
};

MODULE_DEVICE_TABLE(i2c, aw_i2c_id);

static const struct of_device_id aw_dt_match[] = {
	{.compatible = "awinic,haptic1_hv"},
	{.compatible = "awinic,haptic_hv"},
	{},
};

static struct i2c_driver aw_i2c_driver = {
	.driver = {
			.name = AW_I2C_NAME,
			.owner = THIS_MODULE,
			.of_match_table = of_match_ptr(aw_dt_match),
#ifdef CONFIG_PM_SLEEP
			.pm = &aw_pm_ops,
#endif
			},
	.probe = aw_i2c_probe,
	.remove = aw_remove,
	.id_table = aw_i2c_id,
};

static int __init aw_i2c_init(void)
{
	int ret = 0;

	aw_info("aw_haptic driver version %s", HAPTIC_HV_DRIVER_VERSION);
	ret = i2c_add_driver(&aw_i2c_driver);
	if (ret) {
		aw_err("fail to add aw_haptic device into i2c");
		return ret;
	}
	return 0;
}
module_init(aw_i2c_init);

static void __exit aw_i2c_exit(void)
{
	i2c_del_driver(&aw_i2c_driver);
}
module_exit(aw_i2c_exit);

MODULE_DESCRIPTION("AWINIC Haptic Driver");
MODULE_LICENSE("GPL v2");
