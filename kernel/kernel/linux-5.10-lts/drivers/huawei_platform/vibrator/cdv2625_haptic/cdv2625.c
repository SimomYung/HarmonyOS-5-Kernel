...
uint16_t cdv2625_adc_average_get(struct haptic *cdv2625)
{
	...
	cs_i2c_byte_read(cdv2625, CDV2625_DET_CTRL3, buf, CS_I2C_BYTE_TWO);
...
void cdv2625_playmode(struct haptic *cdv2625, uint8_t playmode)
{
	...
	case CS_RAM_MODE:
		...
		if (cdv2625->index >= HAPTIC_RAM_BRK_DISABLE_ID_1 || cdv2625->is_charging_status)
			cdv2625_extra_brk_enable_ctrl(cdv2625, CS_DISABLE);
		else
			cdv2625_extra_brk_enable_ctrl(cdv2625, CS_ENABLE);
		cs_i2c_one_byte_write(cdv2625, CDV2625_CONT_CTRL10, cdv2625->info.long_ram_max_brk_vol);
		break;
...
{
	uint8_t edge_wave0[] = { 0x07, 0x0E, 0x15, 0x1C, 0x23, 0x2A, 0x31, 0x37, 0x3D, 0x44, 0x49, 0x4F, 0x55, 0x5A, 0x5F, 0x63, 0x68, 0x6C, 0x6F, 0x72, 0x75,
				0x78, 0x7A, 0x7C, 0x7D, 0x7E, 0x7F, 0x7F };
	uint8_t edge_wave1[] = { 0x04, 0x09, 0x0d, 0x12, 0x16, 0x1a, 0x1f, 0x23, 0x28, 0x2c, 0x30, 0x34, 0x38, 0x3c, 0x40, 0x44, 0x48, 0x4c, 0x4f, 0x53, 0x56,
				0x59, 0x5c, 0x5f, 0x62, 0x65, 0x68, 0x6a, 0x6d, 0x6f, 0x71, 0x73, 0x75, 0x76, 0x78, 0x79, 0x7b, 0x7c, 0x7c, 0x7d, 0x7e, 0x7e, 0x7e, 0x7f };
	uint16_t t_len = 0;
	edge_pwm_config_t edge_pwm_config_struct;
	uint8_t* edge_wave_data;
	int i = 0;

	if (cdv2625->info.edge_wave_type == 1) { // use edge wave1
		edge_wave_data = edge_wave1;
		t_len = sizeof(edge_wave1);
	} else {
		edge_wave_data = edge_wave0;
		t_len = sizeof(edge_wave0);
	}
	cdv2625_active_mode_set(cdv2625, CDV_MODE_ACTIVE);
	for (i = 0; i < CS_I2C_RETRY_TIMES; i++) {
		if (cdv2625_wave_addr_set(cdv2625, cdv2625->ram_id - t_len) >= 0) {
			if (cdv2625_wave_data_set(cdv2625, edge_wave_data, t_len) >= 0)
				break;
		}
	}
	cdv2625_active_mode_set(cdv2625, CDV_MODE_STANDBY);
...
static int cdv2625_ram_data_write(struct haptic *cdv2625, struct haptic_container *cs_cont)
{
	...
	while (i < cs_cont->len) {
		...
		if (cs_i2c_byte_read(cdv2625, CDV2625_DATA_CTRL, ram_data, 4) >= 0) {
			ram_data[0] &= NOT_CHAECK_DATA_CTRL_REG_BIT0;
...
static void cdv2625_deglitch_number_config(struct haptic *cdv2625, uint8_t num)
{
	uint8_t alg_sel_val = 0;

	cs_i2c_byte_read(cdv2625,CDV2625_DET_CTRL2, &alg_sel_val, 0x01);
	alg_sel_val |= CDV2625_DET_CTRL2_ALG_SEL;
	alg_sel_val &= ~CDV2625_DET_CTRL2_ADC_FLT_COE;
	alg_sel_val |= (num & CDV2625_DET_CTRL2_ADC_FLT_COE);
	cs_i2c_byte_write(cdv2625, CDV2625_DET_CTRL2, &alg_sel_val, 0x01);
}

static void cdv2625_short_ram_brk_cfg_set(struct haptic *cdv2625)
{
	if (!cdv2625) {
		log_err("cdv2625 is null");
		return;
	}

	cs_i2c_one_byte_write(cdv2625, CDV2625_CONT_CTRL4,
	(cdv2625->info.cont_bst_brk_gain << 4) | cdv2625->info.cont_brk_gain);
	cdv2625_pga_gain_config(cdv2625, cdv2625->info.d2s_gain);
	cdv2625_max_brake_cycle_set(cdv2625, cdv2625->info.cont_brk_time);
	cs_i2c_one_byte_write(cdv2625, CDV2625_CONT_CTRL18, cdv2625->info.cont_brk_thr);
	cs_i2c_one_byte_write(cdv2625, CDV2625_CONT_CTRL17, cdv2625->info.cont_track_time);
}

static void cdv2625_long_ram_brk_cfg_set(struct haptic *cdv2625)
{
	if (!cdv2625) {
		log_err("cdv2625 is null");
		return;
	...
	cs_i2c_one_byte_write(cdv2625, CDV2625_CONT_CTRL4,
	(cdv2625->info.cont_bst_brk_gain << 4) | cdv2625->info.long_cont_brk_gain);
	cdv2625_pga_gain_config(cdv2625, cdv2625->info.long_d2s_gain);
	cdv2625_max_brake_cycle_set(cdv2625, cdv2625->info.long_cont_brk_time);
	cs_i2c_one_byte_write(cdv2625, CDV2625_CONT_CTRL18, cdv2625->info.long_cont_brk_thr);
	cs_i2c_one_byte_write(cdv2625, CDV2625_CONT_CTRL17, cdv2625->info.long_cont_track_time);
}
...
static void cdv2625_misc_para_init(struct haptic *cdv2625)
{
	...
	if (cdv2625->info.boot_power_set == SUPPORT_BOOST_5V) {
		cs_i2c_one_byte_write(cdv2625, CDV2625_ANA_CTRL1, SW_CUR_ADJ);
		cs_i2c_one_byte_write(cdv2625, CDV2625_ANA_CTRL2, LOW_POWER_CFG);
	} else {
		cs_i2c_one_byte_write(cdv2625, CDV2625_ANA_CTRL2, LOW_POWER_CFG2);
	}
	...
	cs_i2c_one_byte_write(cdv2625, CDV2625_ANA_CTRL7, OCP_HICCUP_TIME);
	cs_i2c_one_byte_write(cdv2625, CDV2625_ANA_CTRL8, OCP_FILTER_TIME);
	cs_i2c_one_byte_write(cdv2625, CDV2625_CONT_CTRL10, cdv2625->info.long_ram_max_brk_vol);
	cs_i2c_one_byte_write(cdv2625, CDV2625_CONT_CTRL, cdv2625->info.cont_bemf_det_time);
	cs_i2c_one_byte_write(cdv2625, CDV2625_CONT_CTRL19, cdv2625->info.cont_bemf_setup);
	cdv2625_over_dri_volt_set(cdv2625, cdv2625->info.cont_drv1_lvl);
	cdv2625_norm_dri_volt_set(cdv2625, cdv2625->info.cont_drv2_lvl);
	cdv2625_f0_track_config(cdv2625, cdv2625->info.cont_drv1_time, CS_DISABLE, 0);
	cdv2625_f0_close_cyc_set(cdv2625, cdv2625->info.cont_drv2_time);
	...
	cdv2625_short_ram_brk_cfg_set(cdv2625);
}
...
static int cdv2625_free_f0_detect(struct haptic *cdv2625)
{
	...
	cdv2625_deglitch_number_config(cdv2625, DEGLITCH_NUM_1);
	cdv2625_adc_clk_config(cdv2625, ADC_CLK_FRQ_0HZ768);
	...
	if (cdv2625_check_status_wait(cdv2625, CDV_FLAG_DONES, CS_SET, F0_WAIT_TIME_MS)) {
		...
		cdv2625->f0 = 0;
		cdv2625_soft_rst(cdv2625);
		cs_vibrator_dmd_work(DSM_VIBRATOR_F0_ERROR);
		log_info("get f0 err");
	} else {
...
static void cdv2625_interrupt_setup(struct haptic *cdv2625)
{
	...
	cdv2625_interrupt_enable_ctrl(cdv2625, CDV_INTR_UVLOS, CS_ENABLE);
	cdv2625_interrupt_enable_ctrl(cdv2625, CDV_INTR_OCS, CS_ENABLE);
	cdv2625_interrupt_enable_ctrl(cdv2625, CDV_INTR_OTS, CS_ENABLE);
}
 ...
 *
 *****************************************************/
static void cdv2625_ram_brk_cfg(struct device *dev, struct haptic *cs_haptic, struct device_node *np)
{
	int32_t val;
	uint8_t edge_wave_type_temp = 0;
	uint8_t long_ram_max_brk_vol_temp = 0;

	cs_haptic->info.support_long_brk_cfg = of_property_read_bool(np, "cdv2625_support_long_brk_cfg");
	log_info("cs_haptic->info.support_long_brk_cfg = %u", cs_haptic->info.support_long_brk_cfg);
	val = of_property_read_u8(np, "d2s_gain", &cs_haptic->info.d2s_gain);
	if (val != 0)
		log_info("d2s_gain not found");
	val = of_property_read_u8(np, "cont_brk_gain", &cs_haptic->info.cont_brk_gain);
	if (val != 0)
		log_info("cont_brk_gain not found");
	val = of_property_read_u8(np, "cont_brk_time", &cs_haptic->info.cont_brk_time);
	if (val != 0)
		log_info("cont_brk_time not found");
	val = of_property_read_u8(np, "cont_brk_thr", &cs_haptic->info.cont_brk_thr);
	if (val != 0)
		log_info("cont_brk_thr not found");
	val = of_property_read_u8(np, "cont_track_time", &cs_haptic->info.cont_track_time);
	if (val != 0)
		log_info("cont_track_time not found");

	val = of_property_read_u8(np, "long_d2s_gain", &cs_haptic->info.long_d2s_gain);
	if (val != 0)
		log_info("long_d2s_gain not found");
	val = of_property_read_u8(np, "long_cont_brk_gain", &cs_haptic->info.long_cont_brk_gain);
	if (val != 0)
		log_info("long_cont_brk_gain not found");
	val = of_property_read_u8(np, "long_cont_brk_time", &cs_haptic->info.long_cont_brk_time);
	if (val != 0)
		log_info("long_cont_brk_time not found");
	val = of_property_read_u8(np, "long_cont_brk_thr", &cs_haptic->info.long_cont_brk_thr);
	if (val != 0)
		log_info("cont_brk_thr not found");
	val = of_property_read_u8(np, "long_cont_track_time", &cs_haptic->info.long_cont_track_time);
	if (val != 0)
		log_info("cont_track_time not found");
	val = of_property_read_u8(np, "edge_wave_type", &edge_wave_type_temp);
	if (val != 0)
		cs_haptic->info.edge_wave_type = 0;
	else
		cs_haptic->info.edge_wave_type = edge_wave_type_temp;
	val = of_property_read_u8(np, "long_ram_max_brk_vol", &long_ram_max_brk_vol_temp);
	if (val != 0)
		cs_haptic->info.long_ram_max_brk_vol = LONG_RAM_MAX_BRK_VOL;
	else
		cs_haptic->info.long_ram_max_brk_vol = long_ram_max_brk_vol_temp;
}

static void cdv2625_cont_cfg(struct device *dev, struct haptic *cs_haptic, struct device_node *np)
{
	int32_t val = 0;
	val = of_property_read_u8(np, "cont_bemf_det_time", &cs_haptic->info.cont_bemf_det_time);
	if (val != 0)
		log_info("cont_bemf_det_time not found");
	val = of_property_read_u8(np, "cont_drv1_lvl", &cs_haptic->info.cont_drv1_lvl);
	if (val != 0)
		log_info("cont_drv1_lvl not found");
	val = of_property_read_u8(np, "cont_drv2_lvl", &cs_haptic->info.cont_drv2_lvl);
	if (val != 0)
		log_info("cont_drv2_lvl not found");
	val = of_property_read_u8(np, "cont_drv1_time", &cs_haptic->info.cont_drv1_time);
	if (val != 0)
		log_info("cont_drv1_time not found");
	val = of_property_read_u8(np, "cont_drv2_time", &cs_haptic->info.cont_drv2_time);
	if (val != 0)
		log_info("cont_drv2_time not found");
	val = of_property_read_u8(np, "cont_bst_brk_gain", &cs_haptic->info.cont_bst_brk_gain);
	if (val != 0)
		log_info("cont_bst_brk_gain not found");
	val = of_property_read_u8(np, "cont_bemf_setup", &cs_haptic->info.cont_bemf_setup);
	if (val != 0)
		log_info("cont_bemf_setup not found");
}
...
static void cdv2625_parse_dt(struct device *dev, struct haptic *cs_haptic, struct device_node *np)
{
	...
	int32_t val = 0;
	uint8_t silicon_battery_cfg_temp[6];
	...
	val = of_property_read_u8_array(np, "silicon_battery_cfg_dts",
		silicon_battery_cfg_temp, ARRAY_SIZE(silicon_battery_cfg_temp));
	if (val != 0)
		log_info("silicon_battery_cfg_dts not found");
	else
		memcpy(cs_haptic->info.silicon_battery_cfg, silicon_battery_cfg_temp, sizeof(silicon_battery_cfg_temp));
	...
	cs_haptic->info.is_enabled_richtap_core = of_property_read_bool(np, "cdv2625_is_enabled_richtap_core");
	log_info("cs_haptic->info.cdv2625_is_enabled_richtap_core = %u", cs_haptic->info.is_enabled_richtap_core);
	cs_haptic->info.is_support_charging_notify = of_property_read_bool(np, "support_charging_notify");
	log_info("cs_haptic->info.is_support_charging_notify = %u", cs_haptic->info.is_support_charging_notify);
	cdv2625_cont_cfg(dev, cs_haptic, np);
	cdv2625_ram_brk_cfg(dev, cs_haptic, np);
}
...
static void cdv2625_trig_param_init(struct haptic *cs_haptic)
{
	for (int i = 0; i < TRIG_MODE_MAX; i++) {
		cs_haptic->trig[i].trig_level = cs_haptic->info.trig_cfg[i * MAX_IDX + LEVEL_IDX];
		cs_haptic->trig[i].trig_polar = cs_haptic->info.trig_cfg[i * MAX_IDX + POLAR_IDX];
		cs_haptic->trig[i].rise_trigger_en = cs_haptic->info.trig_cfg[i * MAX_IDX + RISE_EN_IDX];
		cs_haptic->trig[i].pos_wave = cs_haptic->info.trig_cfg[i * MAX_IDX + POS_WAVE_IDX];
		cs_haptic->trig[i].fall_trigger_en = cs_haptic->info.trig_cfg[i * MAX_IDX + FALL_EN_IDX];
		cs_haptic->trig[i].neg_wave = cs_haptic->info.trig_cfg[i * MAX_IDX + NEG_WAVE_IDX];
		cs_haptic->trig[i].trig_brk = cs_haptic->info.trig_cfg[i * MAX_IDX + BRK_IDX];
		cs_haptic->trig[i].trig_bst = cs_haptic->info.trig_cfg[i * MAX_IDX + BST_IDX];
	}
}

static void cdv2625_trig_param_config(struct haptic *cs_haptic)
{
	for (int i = 1; i <= TRIG_MODE_MAX; i++) {
		/* set trig boost  */
		if (cs_haptic->trig[i-1].trig_bst)
			cdv2625_triger_bst_enable_ctrl(cs_haptic, i, CS_ENABLE);
		else
			cdv2625_triger_bst_enable_ctrl(cs_haptic, i, CS_DISABLE);

		/* set trig break */
		if (cs_haptic->trig[i-1].trig_brk)
			cdv2625_triger_brk_enable_ctrl(cs_haptic, i, CS_ENABLE);
		else
			cdv2625_triger_brk_enable_ctrl(cs_haptic, i, CS_DISABLE);

		if (!cs_haptic->trig[i-1].trig_level) {
			/* pos enable */
			if (cs_haptic->trig[i-1].rise_trigger_en) {
				cdv2625_triger_mode_config(cs_haptic, i, TRIG_RASING, CS_ENABLE);
				cdv2625_edge_triger_waveid_set(cs_haptic, i, TRIG_RASING, cs_haptic->trig[i-1].pos_wave);
			}

			/* neg enable */
			if (cs_haptic->trig[i-1].fall_trigger_en) {
				cdv2625_triger_mode_config(cs_haptic, i, TRIG_FALLING, CS_ENABLE);
				cdv2625_edge_triger_waveid_set(cs_haptic, i, TRIG_FALLING, cs_haptic->trig[i-1].neg_wave);
			}
		} else {
			if (!cs_haptic->trig[i-1].trig_polar) {
				cdv2625_triger_mode_config(cs_haptic, i, TRIG_HIGH_LEVEL, CS_ENABLE);
				cdv2625_edge_triger_waveid_set(cs_haptic, i, TRIG_HIGH_LEVEL, cs_haptic->trig[i-1].pos_wave);
			} else {
				cdv2625_triger_mode_config(cs_haptic, i, TRIG_LOW_LEVEL, CS_ENABLE);
				cdv2625_edge_triger_waveid_set(cs_haptic, i, TRIG_LOW_LEVEL, cs_haptic->trig[i-1].pos_wave);
			}
		}
	}
}

static void cdv2625_trig_init(struct haptic *cs_haptic)
{
	log_info("enter!");
	if (!cs_haptic) {
	...
	cdv2625_trig_param_init(cs_haptic);
	cdv2625_trig_param_config(cs_haptic);
}
...
struct haptic_func cdv2625_func_list = {
	...
	.trig_init = cdv2625_trig_init,
	.short_ram_brk_cfg_set = cdv2625_short_ram_brk_cfg_set,
	.long_ram_brk_cfg_set = cdv2625_long_ram_brk_cfg_set,
};
