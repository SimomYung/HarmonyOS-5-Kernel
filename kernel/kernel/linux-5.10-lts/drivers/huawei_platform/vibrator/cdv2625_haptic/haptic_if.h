...
#define CS_HAPTIC1_NAME                     "haptics1"
...
#define CS_WORK_DELAY_500MS                 (500)
#define CS_WORK_DELAY_1S                    (1000)
#define CS_DELAY_1000US                     (1000)
#define CS_DELAY_2000US                     (2000)
#define CS_DELAY_8000US                     (8000)
#define CS_DELAY_8500US                     (8500)
#define CS_BASE_ADDR                        0x0800
#define CS_TRIG_NUM                         3
...
#define HAPTIC_GAIN_FOR_F0                  0x80
...
#define HAPTIC_TRIG_MODE1_ID                123452 // index 0
#define HAPTIC_TRIG_MODE2_ID                123453 // index 1
#define HAPTIC_TRIG_MODE3_ID                123454 // index 2
...
#define LONG_HAPTIC_RTP_MAX_ID              399
...
#define FINE_GRAINED_LONG_BASE              6000000
#define CS_IGNORE_STOP_CMD_TIME             30
...
#define LOW_POWER_CFG2               0x2c
#define BST_CFG                      0x43
#define NO_PAD_DEGLITCH              0x00
#define OCP_HICCUP_TIME              0x46
#define OCP_FILTER_TIME              0x57
#define SW_CUR_ADJ                   0x6b
...
#define SUPPORT_BOOST_5V       2
#define BYPASSBOOST_5000MV     5000
#define BYPASSBOOST_4500MV     4500
...
#define LONG_RAM_GAIN_EXPAND_100       100
#define LONG_RAM_GAIN_CONTRACT_60      60
#define LONG_RAM_GAIN_CONTRACT_80      80

#define HAPTIC_RAM_BRK_DISABLE_ID_1    32
...
#define NOT_CHAECK_DATA_CTRL_REG_BIT0           0xfe
#define MAX_RETRY_CNT                           5

#ifdef CONFIG_SPI_NOR_AGENT
#define MAX_LOAD_CNT             20
#else
#define MAX_LOAD_CNT             2
#endif

// silicon battery config
#define IS_SILICON_BATTERY        0
#define IS_TEMP_BELOW_ZERO        1
#define SPECIFIED_BAT_POWER       2
#define SPECIFIED_BAT_TEMP        3
#define LOWPOWER_OR_LOWTEMP_BST   4
#define LOWPOWER_AND_LOWTEMP_BST  5

// vibrator dmd
#define DSM_VIBRATOR_ABNORMAL_ERROR         1
#define DSM_VIBRATOR_OCP_ERROR              2
#define DSM_VIBRATOR_UVP_ERROR              3
#define DSM_VIBRATOR_POWER_DONE             4
#define DSM_VIBRATOR_OTP_ERROR              5
#define DSM_VIBRATOR_F0_ERROR               6
#define I2C_ERR_DMD_PRINT_CNT_MAX           500
#define CS_DMD_REPORT_MAX_CNT               20
#define CS_NAME_MAX                         15

#define CDV2625_REG_SYSINTM                 3
#define CDV2625_REG_SYSINTM_RESET           0xff
...
#define DEGLITCH_NUM_1              0x00 /*!< Glitch num is 1. */
#define DEGLITCH_NUM_3              0x08 /*!< Glitch num is 3. */
#define DEGLITCH_NUM_5              0x10 /*!< Glitch num is 5. */
#define DEGLITCH_NUM_7              0x18 /*!< Glitch num is 7. */
...
struct trig {
	uint8_t trig_level;
	uint8_t trig_polar;
	uint8_t rise_trigger_en;
	uint8_t pos_wave;
	uint8_t fall_trigger_en;
	uint8_t neg_wave;
	uint8_t trig_brk;
	uint8_t trig_bst;
};

enum cs_haptic_trig_val {
	LEVEL_IDX = 0,
	POLAR_IDX = 1,
	RISE_EN_IDX = 2,
	POS_WAVE_IDX = 3,
...
struct haptic_dts_info {
	bool support_long_brk_cfg;
	uint8_t cont_brk_time;
	uint8_t cont_brk_gain;
	uint8_t d2s_gain;
	uint8_t long_cont_brk_time;
	uint8_t long_cont_brk_gain;
	uint8_t long_cont_brk_thr;
	uint8_t long_cont_track_time;
	uint8_t long_d2s_gain;
	uint8_t mode;
	...
	uint8_t cont_bemf_det_time;
	uint8_t cont_drv1_time;
	uint8_t cont_drv2_time;
	uint8_t cont_drv_width;
	uint8_t cont_track_time;
	uint8_t cont_bst_brk_gain;
	uint8_t cont_brk_thr;
	uint8_t cont_bemf_setup;
	uint8_t edge_wave_type;
	uint8_t long_ram_max_brk_vol;
	uint8_t bstcfg[5];
	uint8_t prctmode[3];
	uint8_t sine_array[4];
	uint8_t trig_cfg[24];
	uint8_t silicon_battery_cfg[6];
	uint32_t boot_power_set;
	...
	bool is_support_charging_notify;
};
...
struct haptic {
	...
	/* COMMON */
	uint32_t common_reset_pin;
	uint8_t name[CS_NAME_MAX];
	uint8_t flags;
	uint8_t play_mode;
	uint8_t auto_boost;
	...
	uint8_t trig_mode;
	uint32_t trig_enable;
	uint8_t seq[CS_MAX_SEQ_SIZE];
	uint8_t loop[CS_MAX_SEQ_SIZE];
	uint8_t f0_is_cali;
	int vmax;
	int gain;
	int lowpower_and_lowtemp_gain;
	int lowpower_or_lowtemp_gain;
	int state;
	int index;
	int chipid;
	int irq_gpio;
	...
	bool probe_succ;
	int trig_gpios[CS_TRIG_NUM];
	bool ignore_stop_cmd;
	bool is_dev_resetting;
	bool is_charging_status;
	...
	struct work_struct notifier_call_work;
	struct work_struct trig_work;
	struct notifier_block nb;
	struct notifier_block event_connect_nb;
...
struct haptic_func {
	...
	void (*short_ram_brk_cfg_set)(struct haptic *);
	void (*long_ram_brk_cfg_set)(struct haptic *);
#ifdef SND_SOC_CODEC
