...
/* ---- local macroes ---- */
#define LOG_TIMER_TIMEOUT_VALUE (1000 * 60 * 1)
...
#ifdef CONFIG_BOOT_DETECTOR_SINGLE_FRAME
static void set_callback_save(struct bootfail_proc_param *pparam)
{
	if (pparam == NULL) {
		return;
	}
	...
	if (strlen(log_path) <= 0) {
		print_err("log_path == NULL");
		return;
	}
	print_info("log_path : %s", log_path);

	for (int i = 0; i < sizeof(pparam->log_path) / sizeof(pparam->log_path[0]); ++i) {
		if (strlen(pparam->log_path[i]) > 0) {
			continue;
		}
		int ret = strncpy_s(pparam->log_path[i], sizeof(pparam->log_path[i]) - 1, log_path,
			min(sizeof(pparam->log_path[i]) - 1, strlen(log_path))); //lint !e666
		if (ret != 0) {
...
#ifdef CONFIG_BOOT_DETECTOR_SINGLE_FRAME
		if (atimer->callback != NULL)
			atimer->callback(TIMING_SAVE_LOG);
		if (atimer->is_save_log) {
...
#ifdef CONFIG_BOOT_DETECTOR_SINGLE_FRAME
	static u32 log_timer_timing_count = 0;
	log_timer_timing_count += ptimer_info->count_step_value;
	if (log_timer_timing_count >= LOG_TIMER_TIMEOUT_VALUE) {
		char log_path[BF_SIZE_256];
		bootfail_callback(TIMING_SAVE_LOG, log_path, BF_SIZE_256);
		log_timer_timing_count = 0;
	}
#endif
 ...
 * return: 0 - success, -1 - failed.
 *
 * note: if it is OHOS, we need to capture log when time out
 */
#ifdef CONFIG_BOOT_DETECTOR_SINGLE_FRAME
int action_timer_start(char *act_name, unsigned int timeout_value)
{
	return action_start_timer(act_name, timeout_value, NULL, false);
}

int action_start_timer(char *act_name, unsigned int timeout_value,
	void (*callback)(unsigned int), bool is_save_log)
#else
int action_timer_start(char *act_name, unsigned int timeout_value)
#endif
{
...
#ifdef CONFIG_BOOT_DETECTOR_SINGLE_FRAME
	atimer->is_save_log = is_save_log;
	atimer->callback = callback;
#endif
	...
	if (likely(timer_info)) {
		if (timer_info->boot_timer_count_enable) {
			unsigned long short_time =
				timer_info->short_timer_last_start_time;
...
static void timer_info_init(struct boot_timer_info *ptimer_info)
{
    unsigned long delay_time = 0;
...
int start_boot_timer(void)
{
	...
	if (unlikely(timer_info != NULL)) {
		print_err("boot timer thread already started!");
		return ret;
	}
