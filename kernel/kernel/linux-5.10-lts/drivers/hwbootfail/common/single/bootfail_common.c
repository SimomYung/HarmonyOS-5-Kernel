...
static int __process_upper_bootfail(void *pparam)
{
	...
	if (strlen(log_path) > 0) {
		...
		for (int i = 0; i < sizeof(local_pparam->log_path) / sizeof(local_pparam->log_path[0]); ++i) {
...
static int bootfail_notify_work(struct file *file, unsigned long arg)
{
	...
	if (notify_handle_data.state == TIMING_SAVE_LOG) {
		char* log_temp_path = notify_handle_data.log_temp_path;
		if (strlen(log_temp_path) > 0) {
			save_kmsg_to_templog(log_temp_path, notify_handle_data.count);
		}
	}
