...
/* ----includes ---- */
#if defined(CONFIG_LIBLINUX) && defined(CONFIG_PARTITION_PC)
...
#ifdef CONFIG_BOOT_DETECTOR_SINGLE_FRAME
#define INVALID_VERSION_INFO "invalid version string"
#endif
...
void get_kernel_log(char *pbuf, unsigned int size)
{
...
{
	run_param.adp.pyhs_mem_info.ops.read((unsigned long)version, BF_SIZE_64,
		(void *)(SUB_RESERVED_UNUSED_PHYMEM_BASE + BF_SIZE_1K), BF_SIZE_64);
	if (run_param.adp.sec_funcs.strlen(version) == 0)
		run_param.adp.sec_funcs.strncpy_s(version, BF_SIZE_64, INVALID_VERSION_INFO, BF_SIZE_64);
}
#endif
...
{
#ifdef CONFIG_BOOT_DETECTOR_SINGLE_FRAME
	get_version((char *)&(plog->hdr.version));
#endif
...
static void save_bi_code(void)
{
	union bfi_part_header hdr;
	run_param.adp.sec_funcs.memset_s(&hdr, (unsigned long)sizeof(hdr), 0, (unsigned long)sizeof(hdr));
	hdr.misc_msg.bi_code = BI_BL_BOOT_FAIL;
	int ret = run_param.adp.bfi_part.ops.write(run_param.adp.bfi_part.dev_path, 0, (char *)&hdr, sizeof(hdr));
	run_param.adp.print("save_bi_code result is %d\n", ret);
}
...
enum bootfail_errorcode set_boot_stage(enum boot_stage stage)
{
	...
	errorcode = get_boot_stage(&curr_stage);
	if (errorcode != BF_OK)
		return errorcode;
	...
	struct bootfail_proc_param *pparam)
{
	...
	if (pparam != NULL && is_valid_magic_number(pparam->magic) == 0)
		pparam->magic = BF_SW_MAGIC_NUM;
...
{
	if ((run_param.is_init_succ == 0) || (run_param.adp.print == NULL))
		return;
