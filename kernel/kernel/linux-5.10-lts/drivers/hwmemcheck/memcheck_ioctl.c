...
static int process_do_command(const void *arg)
{
	...
	if ((!(cmd.type & MTYPE_PSS)) && (!(cmd.type & MTYPE_FD))) {
		memcheck_err("invalid memtype %d\n", cmd.type);
		return -EINVAL;
	}
	if (cmd.id <= 0) {
	...
	if (cmd.type & MTYPE_FD)
		return memcheck_fd_do_command(&cmd);
...
static int process_detail_write(const void *arg)
{
	...
	memcheck_info("IOCTL FD STACK WRITE type=%u,size=%llu\n", info.type,
		      info.size);
	if ((!info.size) || (info.size > MEMCHECK_DETAILINFO_MAXSIZE)) {
		memcheck_err("wrong size=%zu\n", info.size);
		return -EINVAL;
...
static int process_fd_stack_read(void *arg)
{
	struct fd_stack_info info;
	...
	if (!(info.type & MTYPE_FD)) {
		memcheck_err("do not support type %d\n", info.type);
		return -EINVAL;
	}

	return memcheck_fd_stack_read(arg, &info);
}

static int process_fd_stack_write(const void *arg)
{
	struct fd_stack_info info;
	...
	return memcheck_fd_stack_write(arg, &info);
}
...
long memcheck_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
...
#ifndef CONFIG_DFX_OHOS
	if(get_log_usertype() != USERTYPE_BETA && cmd != LOGGER_MEMCHECK_PSS_READ) {
		memcheck_err("memcheck ioctl cmd %d not permitted", cmd);
		return -EPERM;
	}
#endif
	...
	case LOGGER_MEMCHECK_DETAIL_WRITE:
		...
		break;
	case LOGGER_MEMCHECK_FD_STACK_READ:
		ret = process_fd_stack_read((void *)arg);
		break;

	case LOGGER_MEMCHECK_FD_STACK_WRITE:
		ret = process_fd_stack_write((void *)arg);
		break;
