...
#ifdef CONFIG_DFX_LIBLINUX
extern int liblinux_send_memcheck_signal(int pid, int signum, struct kernel_siginfo *info);
static inline int send_memcheck_signal(int pid, int signum, struct kernel_siginfo *info)
{
	int ret = -ENOSYS;

	ret = liblinux_send_memcheck_signal(pid, signum, info);
	return ret;
}
#endif
