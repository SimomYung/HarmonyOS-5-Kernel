...
void hton24(u8 *p, int len)
{
	if (unlikely(len < NUM_3))
		return;
	u8 tmp = p[0];
	p[0] = p[2];
	p[2] = tmp;
...
static int match_packet_tx_mode(struct sk_buff *skb)
{
	...
	return skb_uid_match(sk->sk_uid.val);
}
	...
	if ((iph->frag_off & htons(IP_DF)) == 0 && (iph->frag_off & htons(IP_OFFSET))) {
		...
		if (tmp_tuple_hash != NULL) {
			...
			if (skb->sk == NULL)
				return NULL;
			sk = sk_to_full_sk(skb->sk);
			ss = master_session_add_get(&tmp_nf_tuple, skb, sk->sk_uid.val, origin_ss);
			if (ss == NULL)
				return NULL;
			tmp_tuple_hash = ss->session_hash;
		}
