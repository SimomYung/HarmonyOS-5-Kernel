 ...
 * Description: This file collect top10 apps abnormal stats, saved in global variables.
 *              Stats sent to the ap when a message is received from the ap.
 * Author: z30047862
 * Create: 2023-9-13
 */
...
void data_dubai_handle_top_app_statics(void)
{
	...
	temp_top_abn_statistics.avg_rtt = g_data_dubai_stats.app_statics_top.total_rtt /
		g_data_dubai_stats.app_statics_top.total_tcp_in_pkt;
...
{
	if (msg == NULL) {
		pr_err("[DUBAI]: update_top_app_uids fail, msg is null");
		return;
	}
	if (msg->len != TOP_APP_UID_LIST_MSG_LEN) {
