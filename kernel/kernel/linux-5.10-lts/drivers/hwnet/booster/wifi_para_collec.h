/*
 * Copyright (c) Huawei Technologies Co., Ltd. 2020-2020. All rights reserved.
 * Description: This module is to collect wifi chip parameters.
 * Author: tongxilin@huawei.com
 * Create: 2020-03-30
 */

#ifndef _WIFI_PARA_COLLEC_H
#define _WIFI_PARA_COLLEC_H

#include <linux/list.h>
#include <linux/spinlock.h>
#include <linux/timer.h>
#include <net/cfg80211.h>

#include "netlink_handle.h"

#define MAX_RSSI (-1)
#define MIN_RSSI (-100)
#ifdef CONFIG_HW_BOOSTER_QCOM
#define QCOM_24G_DELTA_RSSI 0
#define QCOM_5G_DELTA_RSSI 0
#endif
#define MAX_NOISE 0
#define MIN_NOISE (-100)
#define MAX_SNR 50
#define MIN_SNR (-10)
#define MAX_UL_DELAY 60000
#define MIN_UL_DELAY 0
#define MAX_CHLOAD 1000
#define MIN_CHLOAD 0
#define MAX_WIFI_NUM 3
#define DEV_INDEX_OFFSET 2 /* wlan0:devid = 2, wlan1:devid = 3 */
#define INVALID_NUM1 0xFFFF
#define INVALID_NUM2 0
#define MAX_U32 0xFFFFFFFF
#define BEACON_RSSI_LEN 10
#define UL_DELAY_ALLAY_LEN 4
#define NUM_32_BYTES 32

typedef enum {
    /* HI110X project uses 0 ~ 999 */
    SUB_CHIP_HI1102 = 0,
    SUB_CHIP_HI1103 = 1,
    SUB_CHIP_HI1102A = 2,
    SUB_CHIP_HI1105 = 3,
    SUB_CHIP_SHENKUO = 4,
    SUB_CHIP_BISHENG = 5,
    SUB_CHIP_HI1161 = 6,
    SUB_CHIP_MP17C = 7,
    SUB_CHIP_MP12 = 8,

    /* HI113X project uses 1000 ~ 2000 */
    SUB_CHIP_HI1131K = 1000,
    SUB_CHIP_HI1132,
    SUB_CHIP_HI1135,
    SUB_CHIP_SUZHE,

    SUB_CHIP_BUT,
} sub_chiptype_enum;

struct wifi_req_msg {
	u16 type;
	u16 len;
	u32 report_expires;
	u32 wifi_gateway[MAX_WIFI_NUM];
	u32 wifi_freq[MAX_WIFI_NUM];
};

struct timer_msg {
	u16 type;
	u16 len;
	u32 sync;
};

struct wifi_res {
	s32 rx_bytes;
	s32 tx_bytes;
	s32 rssi;
	u32 phy_tx_rate;
	u32 phy_rx_rate;
	s32 rx_packets;
	s32 tx_packets;
	s32 tx_failed;
	s32 noise;
	s32 snr;
	s32 chload;
	s32 ul_delay;
	u16 ul_delay_array[UL_DELAY_ALLAY_LEN];
	u16 tx_time[UL_DELAY_ALLAY_LEN];
	s8 beacon_rssi[BEACON_RSSI_LEN]; // the latest 10 rssi, 1 rssi per 100ms.
	u16 chload_self;
	u8 tx_mcs;
	u8 tx_nss;
	u8 tx_bw;
	u8 tx_flags;
	u32 arp_state;
};

struct wifi_para {
	u32 rx_bytes;
	u32 tx_bytes;
	u32 phy_tx_rate;
	u32 phy_rx_rate;
	u32 rx_packets;
	u32 tx_packets;
	u32 tx_failed;
	u32 arp_state;
	s32 rssi;
	s32 noise;
	s32 snr;
	s32 chload;
	s32 ul_delay;
	u16 ul_delay_array[UL_DELAY_ALLAY_LEN];
	u16 tx_time[UL_DELAY_ALLAY_LEN];
	s8 beacon_rssi[BEACON_RSSI_LEN]; // the latest 10 rssi, 1 rssi per 100ms.
	u16 chload_self; // Note the four-byte alignment
	u8 tx_mcs;
	u8 tx_nss;
	u8 tx_bw;
	u8 tx_flags;
	u32 tx_ppdu_cnt;
	u32 tx_ppdu_retry_cnt;
	u8 ppdu_per;
	u8 tx_mcs_flag3;
	u8 cw_min;
	u8 cw_max;
	u16 uldelay_cdf[NUM_32_BYTES];
	u16 txtime_cdf[NUM_32_BYTES];
};

/* Statistical list head of wifi stat */
struct wifi_stat {
	u16 dev_id;
	u16 flag;
	struct wifi_para info;
};

/* wifi paramter collect context struct */
struct wifi_ctx {
	u32 expires; // timer cycle by jiffies, set by upper.
	struct wifi_stat *past; // cpus private statistics context parameter
	notify_event *fn; // recv notification interface callback function.
	struct timer_list timer; // report timer for statistics
};

struct wifi_res_msg {
	u16 type; // Event enumeration values
	u16 len; // The length behind this field, the limit lower 2048
	struct wifi_stat wifi_res[0];
};

enum wifi_index {
	WLAN0_IDX,
	WLAN1_IDX,
	WLAN2_IDX,
};

struct wifi_sub_chip_ntfy_msg {
    int16_t type; // Event enumeration values
    int16_t len; // The length behind this field, the limit lower 2048
    int32_t subChipType;
};

msg_process *wifi_para_collec_init(notify_event *fn);
void wifi_para_collec_exit(void);
#ifdef CONFIG_HW_BOOSTER_QCOM
int cfg80211_get_station_register(int(*p)(struct net_device *dev, const u8 *mac_addr,
	struct station_info *sinfo));
int cfg80211_calculate_bitrate_register(u32(*p)(struct rate_info *rate));
#endif
#endif
