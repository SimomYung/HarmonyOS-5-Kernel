...
#endif
{
	if (g_chr_timer_state == CHR_TIMER_60) {
		 ...
		 */
		spin_lock_bh(&g_dest_addr_timer_lock);
		memset_s(g_chr_net_if_struct, DIFF_SRC_IP_ADDR_MAX *
			sizeof(struct chr_net_iface_struct), 0, DIFF_SRC_IP_ADDR_MAX * sizeof(struct chr_net_iface_struct));
		spin_unlock_bh(&g_dest_addr_timer_lock);
...
static void chr_netlink_init(void)
{
	...
	add_timer(&g_chr_netlink_timer);
	g_chr_timer_state = CHR_TIMER_3;
	memset_s(g_chr_net_if_struct, DIFF_SRC_IP_ADDR_MAX * sizeof(struct chr_net_iface_struct), 0,
	...
	unsigned int src_addr, struct http_return *prtn)
{
	...
	memset_s(packet, sizeof(struct chr_nl_packet_msg), 0, sizeof(struct chr_nl_packet_msg));
	if (event == CHR_SPEED_SLOW_EVENT) {
		packet->chr_event = event;
		packet->src_addr = src_addr;
		memset_s(&(packet->rtn_stat), sizeof(struct http_return), 0, sizeof(struct http_return));
	} else if (event == CHR_WEB_STAT_EVENT) {
		packet->chr_event = event;
		packet->src_addr = 0;
		memcpy_s(&(packet->rtn_stat), RNT_SIZE * sizeof(struct http_return), prtn,
			RNT_SIZE * sizeof(struct http_return));
	} else if (event == TCP_RECOVER_HIVIEW_ID) {
		packet->chr_event = event;
		packet->src_addr = 0;
		memcpy_s(&(packet->rtn_stat), sizeof(struct http_return), prtn, sizeof(struct http_return));
	}
