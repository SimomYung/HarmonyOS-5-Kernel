...
static int mplk_interrupt_data_transfer(struct sock *sk, unsigned int strategy)
{
	...
	if (sk->sk_protocol == IPPROTO_UDP) {
		WIFIPRO_DEBUG("port:%d,ptl:%d,fwmak:%d,devif:%d,addrbind:%d,portbind:%d,sk:%px, sk->sk_born_stamp:%u, \
			uid_entry.bind_stamp: %u, uid:%u\n", sk->sk_num, sk->sk_protocol, sk->sk_mark & 0xFFFF,
			sk->sk_bound_dev_if,(sk->sk_userlocks & SOCK_BINDADDR_LOCK) > 0,
			(sk->sk_userlocks & SOCK_BINDPORT_LOCK) > 0, sk, sk->sk_born_stamp, uid_entry.bind_stamp, uid);
	}
	if (uid_entry.need_delete == MPLK_UID_NEED_DELETE) {
		mplk_trigger_socket_reroute(sk, MPLK_UNBIND_TO_CELL,
			uid_entry.netid);
	} else {
		if ((uid_entry.strategy & strategy) && (time_before(sk->sk_born_stamp, uid_entry.bind_stamp)
			||(sk != NULL && (sk->sk_userlocks & SOCK_BINDADDR_LOCK) > 0) ))
			mplk_trigger_socket_reroute(sk, MPLK_BIND_TO_CELL, uid_entry.netid);
	}
