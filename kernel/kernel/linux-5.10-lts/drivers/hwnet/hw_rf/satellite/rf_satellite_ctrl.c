...
static int32_t rsmc_on_proc_other(struct ps_core_s *cd)
{
	...
	if (gpio_is_valid(cd->sate_rsmc_gps_spdt)) {
		hwlog_info("%s, sate_rsmc_gps_spdt", __func__);
		gpio_set_value(cd->sate_rsmc_gps_spdt, GPIO_LOW);
	}
...
static int32_t rsmc_off_proc_other(struct ps_core_s *cd)
{
	...
	if (gpio_is_valid(cd->sate_rsmc_gps_spdt)) {
		hwlog_info("%s, sate_rsmc_gps_spdt", __func__);
		gpio_set_value(cd->sate_rsmc_gps_spdt, GPIO_HIGH);
	}
...
int32_t rf_satelite_config_init(struct ps_core_s *cd)
{
	...
	rc = rf_init_gpio(cd->sate_rsmc_gps_spdt, "sate_rsmc_gps_spdt", GPIO_HIGH, GPIO_DIRECTION_OUTPUT);
	if (rc != 0) {
		hwlog_err("%s: init gpio %d failed %d\n",
			__func__, cd->sate_rsmc_gps_spdt, rc);
		return rc;
	}
