...
#define BBIC_RESET_TIME_MS_X1 20
#define BBIC_PWR_ON_RESET_TIME_MS_X1 500
#define SIM_LDO_SWITCH_TIME_MS 5
#define PA_BST_VOL_4V8 4800
#define PA_BST_VOL_4V5 4500
#define PA_BST_IDX_X1 1
#define PA_BST_VOL_4V7_X1 4600
#define PA_BST_VOL_3V4 3400
...
static struct regulator *lna_ldo_regulator = NULL;
...
{
	if ((type != BBIC_CHIP_TYPE_HLCT) && (type != BBIC_CHIP_TYPE_WS) && (type != BBIC_CHIP_TYPE_WS_X1))
		return false;
...
static int parse_boot_mode_config(struct device_node *dev_node, struct ps_core_s *cd)
{
	...
	if (get_bbic_chip_type() != BBIC_CHIP_TYPE_WS_X1) {
...
static int parse_ls_core_supply_config_ws_x1(struct device_node *dev_node,
	struct ps_core_s *cd)
{
	int ret;
	int32_t value;

	ret = of_property_read_u32(dev_node, "core_en_flag", (unsigned int *)(&cd->core_en_flag));
	if (ret != 0) {
		cd->core_en_flag = -1;
		hwlog_info("%s: no need core_ldo\n", __func__);
	}
	ret = of_property_read_u32(dev_node, "bbic,core_ldo_num",
		(unsigned int *)(&cd->core_ldo_num));
	if (ret != 0) {
		cd->core_ldo_num = -1;
		hwlog_err("%s: failed to get core_ldo_num,err:%d\n", __func__, ret);
		return -1;
	}
	...
	if (ret != 0) {
		cd->core_ldo_vol = -1;
		hwlog_err("%s: failed to get core_ldo_vol,err:%d\n", __func__, ret);
		return -1;
	}
	value = of_get_named_gpio(dev_node, "bbic,core_en_gpio", 0);
	hwlog_info("%s, core_en_gpio:%d\n", __func__, value);
	cd->core_en_gpio = value;

	ret = of_property_read_u32(dev_node, "bbic,lna_ldo_num",
		(unsigned int *)(&cd->lna_ldo_num));
	if (ret != 0) {
		cd->lna_ldo_num = -1;
		hwlog_err("%s: failed to get lna_ldo_num,err:%d\n", __func__, ret);
		return -1;
	}
	ret = of_property_read_u32(dev_node, "bbic,lna_ldo_vol",
		(unsigned int *)(&cd->lna_ldo_vol));
	if (ret != 0) {
		cd->lna_ldo_vol = -1;
		hwlog_err("%s: failed to get lna_ldo_vol,err:%d\n", __func__, ret);
		return -1;
	}
	value = of_get_named_gpio(dev_node, "bbic,bb_radio_avdd_en", 0);
	hwlog_info("%s, bb_radio_avdd_en:%d\n", __func__, value);
	cd->radio_avdd_en = value;
	return ret;
}
	...
	struct ps_core_s *cd)
{
	...
	ret = of_property_read_u32(dev_node, "bbic,usim_ldo_vol_1v8", (unsigned int *)(&cd->usim0_ldo_vol_1v8));
	if (ret != 0) {
		cd->usim0_ldo_vol_1v8 = -1;
		hwlog_info("%s: fail to get usim0_ldo_vol_1v8,default value:%d\n", __func__, cd->usim0_ldo_vol_1v8);
	} else {
	...
	struct ps_core_s *cd)
{
	...
	if (ret != 0) {
		...
		hwlog_info("%s: fail to get usim0_card_ldo_vol,default value:%d\n", __func__, cd->usim0_card_ldo_vol);
	} else {
		hwlog_info("%s: succ to get usim0_card_ldo_vol = %d!\n", __func__, cd->usim0_card_ldo_vol);
	}

	ret = of_property_read_u32(dev_node, "bbic,sim1_ldo_vol", (unsigned int *)(&cd->usim1_card_ldo_vol));
	if (ret != 0) {
		cd->usim1_card_ldo_vol = -1;
		hwlog_info("%s: fail to get usim1_card_ldo_vol,default value:%d\n", __func__, cd->usim1_card_ldo_vol);
	} else {
	...
	if (get_bbic_chip_type() == BBIC_CHIP_TYPE_WS_X1) {
		ret = parse_ls_core_supply_config_ws_x1(dev_node, cd);
	} else {
...
static int32_t boot_mode_gpio_ctrl(int boot_mode0, int boot_mode3)
{
	...
	if ((!gpio_is_valid(cd->bb_boot_mode0)) ||
		((bbic_chip_type != BBIC_CHIP_TYPE_WS_X1) && (!gpio_is_valid(cd->bb_boot_mode3)))) {
		hwlog_err("%s: invalid boot mode gpio", __func__);
		return -1;
	}
	gpio_set_value(cd->bb_boot_mode0, boot_mode0);
	if ((bbic_chip_type != BBIC_CHIP_TYPE_WS_X1))
...
static void set_sim_switch_x1(int sim_slot, int gpio_status)
{
	int usim0_gpio_status;
	int usim1_gpio_status;
	struct ps_core_s *cd = get_core_data();
	...
	if ((!gpio_is_valid(cd->usim0_switch)) || (!gpio_is_valid(cd->usim1_switch))) {
		hwlog_err("%s: invalid sim switch", __func__);
		return;
	}
	if (gpio_status == GPIO_LOW) {
		usim0_gpio_status = GPIO_LOW;
		usim1_gpio_status = GPIO_LOW;
	} else {
		if (sim_slot == SIM_SLOT_0) {
			usim0_gpio_status = GPIO_HIGH;
			usim1_gpio_status = GPIO_LOW;
		} else {
			usim0_gpio_status = GPIO_LOW;
			usim1_gpio_status = GPIO_HIGH;
		}
	}
	hwlog_info("%s: sim0 switch status:%d, sim1 switch status:%d", __func__, usim0_gpio_status, usim1_gpio_status);
	gpio_set_value(cd->usim0_switch, usim0_gpio_status);
	gpio_set_value(cd->usim1_switch, usim1_gpio_status);
}
...
int32_t hw_set_usim_switch(int sim_slot, int gpio_status)
{
	...
	if (get_bbic_chip_type() == BBIC_CHIP_TYPE_WS_X1) {
		set_sim_switch_x1(sim_slot, gpio_status);
		return 0;
	}
	if (sim_slot == SIM_SLOT_0) {
...
static void sim_ldo_regulator_deinit(int sim_idx)
{
	...
	if (sim_idx == SIM_SLOT_0) {
		...
		if ((get_bbic_chip_type() != BBIC_CHIP_TYPE_WS_X1) && (sim1_card_back_ldo_regulator != NULL)) {
...
static void rf_pwr_off_proc_x1(struct ps_core_s *cd)
{
	int ret;
	bool bst_rst;

	if (lna_ldo_regulator != NULL) {
		ret = regulator_disable(lna_ldo_regulator);
		hwlog_info("%s:lna ldo regulator disable,ret = %d\n", __func__, ret);
		devm_regulator_put(lna_ldo_regulator);
		lna_ldo_regulator = NULL;
	}
	bst_rst = buckboost_set_enable(GPIO_LOW, BBST_USER_BOOST_HW_SATE, PA_BST_IDX_X1);
	if (!bst_rst)
		hwlog_err("%s: boost set disable fail", __func__);
}

static int bbic_pwr_off_proc_x1(struct ps_core_s *cd)
{
	int ret;

	if (gpio_is_valid(cd->bb_reset)) { // GPIO219
		hwlog_info("%s, bbic reset", __func__);
		gpio_set_value(cd->bb_reset, GPIO_LOW);
	}
	msleep(BBIC_RESET_TIME_MS_X1);
	if ((cd->core_en_flag == -1) && (core_ldo_regulator != NULL)) {
	...
	if (gpio_is_valid(cd->core_en_gpio)) { // GPIO226
		hwlog_info("%s, set core en gpio:%d", __func__, cd->core_en_gpio);
		gpio_set_value(cd->core_en_gpio, GPIO_LOW);
	}
	if (usim_ldo_regulator != NULL) {
		ret = regulator_disable(usim_ldo_regulator);
		hwlog_info("%s:usim ldo regulator disable,ret = %d\n", __func__, ret);
...
static int32_t lna_ldo_power_on(struct ps_core_s *cd)
{
	int32_t ret;

	hwlog_info("%s: enter", __func__);
	if (!ldo_is_valid(cd->lna_ldo_num) || !(ldo_is_valid(cd->lna_ldo_vol))) {
		hwlog_err("%s:invalid lna ldo info\n", __func__);
		return -1;
	}
	lna_ldo_regulator = devm_regulator_get(cd->dev, "bbic_lna_ldo");
	if (IS_ERR(lna_ldo_regulator)) {
		hwlog_err("%s:No pmu ldo found for lna_ldo\n", __func__);
		return -1;
	}
	ret = regulator_set_voltage(lna_ldo_regulator, cd->lna_ldo_vol, cd->lna_ldo_vol);
	if (ret != 0)
		hwlog_err("%s:regulator_set_voltage lna_ldo_regulator fail,ret = %d\n", __func__, ret);

	ret = regulator_set_mode(lna_ldo_regulator, REGULATOR_MODE_NORMAL);
	if (ret != 0)
		hwlog_err("%s:regulator_set_mode lna_ldo_regulator fail,ret = %d\n", __func__, ret);

	ret = regulator_enable(lna_ldo_regulator);
	if (ret != 0)
		hwlog_err("%s:regulator_enable,ret = %d\n", __func__, ret);
	return ret;
}

static void rf_pwr_on_proc_x1(struct ps_core_s *cd)
{
	int ret;
	bool bst_rst;

	ret = lna_ldo_power_on(cd);
	if (ret != 0) {
		hwlog_err("%s: lna pwr on fail, ret:%d", __func__, ret);
		return;
	}
	bst_rst = buckboost_set_enable(GPIO_HIGH, BBST_USER_BOOST_HW_SATE, PA_BST_IDX_X1);
	if (!bst_rst) {
		hwlog_err("%s: boost set enable fail", __func__);
		return;
	}
	bst_rst = buckboost_set_vout(PA_BST_VOL_4V7_X1, BBST_USER_BOOST_HW_SATE, PA_BST_IDX_X1);
	if (bst_rst != 0) {
		hwlog_err("%s: boost set vout fail", __func__);
		return;
	}
}

static int bbic_pwr_on_proc_x1(struct ps_core_s *cd)
{
	int ret;

	if (gpio_is_valid(cd->core_en_gpio)) { // GPIO226
		hwlog_info("%s, set core en gpio:%d", __func__, cd->core_en_gpio);
		gpio_set_value(cd->core_en_gpio, GPIO_HIGH);
	}
	hwlog_info("%s, core en flag:%d", __func__, cd->core_en_flag);
	if (cd->core_en_flag == -1) {
		ret = core_ldo_power_on(cd);
			if (ret != 0) {
				hwlog_err("%s: core pwr on fail, ret:%d", __func__, ret);
				return -1;
	...
	msleep(BBIC_PWR_ON_RESET_TIME_MS_X1);
	if (gpio_is_valid(cd->bb_reset)) { // GPIO219
		hwlog_info("%s, bbic reset", __func__);
		gpio_set_value(cd->bb_reset, GPIO_HIGH);
	}
	rf_pwr_on_proc_x1(cd);
	return ret;
}
...
int pwr_gpio_ctrl_x1(bool status, bool is_need_set_bootmode)
{
	struct ps_core_s *cd = get_core_data();
	int32_t set_value = status ? GPIO_HIGH : GPIO_LOW;
	int ret;

	hwlog_info("%s: enter, set_value %d", __func__, set_value);
	if (cd == NULL) {
		hwlog_err("%s: core data is null", __func__);
		return -1;
	}
	if (status) {
		...
		if (gpio_is_valid(cd->ap_wakeup_bb))
			gpio_set_value(cd->ap_wakeup_bb, GPIO_HIGH);
		ret = bbic_pwr_on_proc_x1(cd);
		hwlog_info("%s, bbic_pwr_on_proc done", __func__);
		set_bbic_pwr_status(true);
	} else {
		ret = bbic_pwr_off_proc_x1(cd);
		if (gpio_is_valid(cd->bb_boot_mode0))
			gpio_set_value(cd->bb_boot_mode0, GPIO_LOW);
		if (gpio_is_valid(cd->ap_wakeup_bb))
...
static int32_t pwr_config_init_ws_x1(struct ps_core_s *cd)
{
	int32_t rc;

	rc = init_gpio(cd->radio_avdd_en, "bbic_radio_avdd_en", GPIO_HIGH, GPIO_DIRECTION_OUTPUT);
	if (rc != 0) {
		hwlog_err("%s: init gpio %d failed %d\n",
			__func__, cd->radio_avdd_en, rc);
		return rc;
	}
	if (gpio_is_valid(cd->radio_avdd_en)) { // GPIO81
		gpio_set_value(cd->radio_avdd_en, GPIO_HIGH);
		hwlog_info("%s, set radio_avdd_en:%d", __func__, cd->radio_avdd_en);
	}
	rc = init_gpio(cd->core_en_gpio, "bbic_core_en", GPIO_LOW, GPIO_DIRECTION_OUTPUT);
	if (rc != 0) {
		hwlog_err("%s: init gpio %d failed %d\n",
			__func__, cd->core_en_gpio, rc);
		return rc;
	}
	return 0;
}
...
static int32_t hw_bbic_driver_init(void)
{
	...
	} else if (get_bbic_chip_type() == BBIC_CHIP_TYPE_WS_X1) {
		ret = pwr_config_init_ws_x1(cd);
	} else {
