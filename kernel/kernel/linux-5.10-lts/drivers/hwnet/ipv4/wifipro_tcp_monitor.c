...
#ifdef CONFIG_HW_WIFIPRO_OPENHARMONY
	SNMP_MIB_ITEM("IPV6 InSegs", WIFIPRO_IPV6_INSEGS),
	SNMP_MIB_ITEM("IPV6 OutSegs", WIFIPRO_IPV6_OUTSEGS),
#endif
	SNMP_MIB_SENTINEL
};
#endif
...
char *wifipro_ntoa(int addr)
{
	...
	if (memset_s(buf, sizeof(buf), 0, sizeof(buf)) != EOK) {
		WIFIPRO_ERROR("wifipro_ntoa memset_s error");
	}
	/* 24 high addr */
	ret = sprintf_s(buf + len, sizeof(buf) - len, "%d.", (addr>>24) & 0xff);
	if (ret < 0) {
		WIFIPRO_ERROR("wifipro_ntoa sprintf_s_1 error");
		return buf;
	}
	len += ret;
	/* 16 mid addr */
	ret = sprintf_s(buf + len, sizeof(buf) - len, "%d.", (addr>>16) & 0xff);
	if (ret < 0) {
		WIFIPRO_ERROR("wifipro_ntoa sprintf_s_2 error");
		return buf;
	}
	len += ret;
	/* 8 low addr */
	ret = sprintf_s(buf + len, sizeof(buf) - len, "%d.", (addr>>8) & 0xff);
	if (ret < 0) {
		WIFIPRO_ERROR("wifipro_ntoa sprintf_s_3 error");
		return buf;
	}
	len += ret;
	ret = sprintf_s(buf + len, sizeof(buf) - len, "%d", addr & 0xff);
	if (ret < 0) {
		WIFIPRO_ERROR("wifipro_ntoa sprintf_s_4 error");
	}
	return buf;
}
	...
	if ((1 << sk->sk_state) & TCPF_SYN_SENT) {
		if (skb->dev != NULL) {
			wifipro_dev_max_len = strnlen(skb->dev->name,
				IFNAMSIZ - 1);
			if (strncpy_s(sk->wifipro_dev_name, IFNAMSIZ, skb->dev->name,
				wifipro_dev_max_len) != EOK) {
				WIFIPRO_ERROR("print_wifipro_dev_name strncpy_s error");
				return;
			}
...
static void print_wifipro_info(struct sock *sk, const struct sk_buff *skb)
{
	...
	int uid = 0;
	int uid_limited = 10000;
	struct inet_sock *inet = inet_sk(sk);
	
	if (inet == NULL)
		return;
	
	if (!uid_valid(sk->sk_uid))
		return;
	
	uid = (int)sk->sk_uid.val;
	if (uid > uid_limited) {
		source_port = htons(inet->inet_sport);
		if (tcp_hdr(skb)->fin == 1) {
			WIFIPRO_DEBUG("FIN uid=%d, port=%d, wifipro_dev_name is %s", uid, source_port, sk->wifipro_dev_name);
		} else if (tcp_hdr(skb)->syn == 1) {
			WIFIPRO_DEBUG("SYN uid=%d, port=%d, wifipro_dev_name is %s", uid, source_port, sk->wifipro_dev_name);
		} else if (tcp_hdr(skb)->rst == 1) {
			WIFIPRO_DEBUG("RESET uid=%d, port=%d, wifipro_dev_name is %s", uid, source_port, sk->wifipro_dev_name);
		}
	}
}
...
#ifdef CONFIG_HW_WIFIPRO_OPENHARMONY
static inline bool wifipro_is_not_local_or_lan_sock6(const struct in6_addr *addr)
{
    if (addr == NULL) {
        return false;
	}
    ...
    if (ipv6_addr_loopback(addr)) {
 	...
 	// 3. 手动检查链路本地地址 (fe80::/10)
    // 链路本地地址的前10位是 1111111010 (0xFE80)
    if ((addr->s6_addr32[0] & htonl(0xFFC00000)) == htonl(0xFE800000)) {
        return false;
    }

    // 4. 检查唯一本地地址 ULA (fc00::/7)
    if ((addr->s6_addr32[0] & htonl(0xFE000000)) == htonl(0xFC000000)) {
        return false;
    }
    ...
    // 6. 可选：检查站点本地地址 (已弃用但需兼容，fec0::/10)
    if ((addr->s6_addr32[0] & htonl(0xFFC00000)) == htonl(0xFEC00000)) {
        return false;
    }

    // 7. 可选：检查 IPv4 映射地址 (::ffff:0:0/96)
    if (ipv6_addr_v4mapped(addr)) {
        // 如果内部是私有 IPv4，则视为局域网地址
        u32 ipv4 = ntohl(addr->s6_addr32[3]);
        return !(ipv4 >= 0x0A000000 && ipv4 <= 0x0AFFFFFF) && // 10.0.0.0/8
...
static inline bool wifipro_is_wlan(struct sock *sk, const struct sk_buff *skb)
{
	print_wifipro_dev_name(sk, skb);
	bool is_full_sk = sk_fullsock(sk);
	if (!is_full_sk || strncmp(sk->wifipro_dev_name, DEV_NAME_WLAN, WLAN_LENGTH) != 0) {
		WIFIPRO_DEBUG("not wlan %{public}s", sk->wifipro_dev_name);
		return false;
	}
	return true;
}
...
#ifdef CONFIG_HW_WIFIPRO_OPENHARMONY
void wifipro_update_ipv6_statistics(int mib_type, const struct sk_buff *skb,
	struct sock *from_sk)
{
	if (!is_wifipro_on ) {
		return;
	...
	if (skb == NULL || skb->protocol != htons(ETH_P_IPV6)) {
		WIFIPRO_DEBUG("wifipro_update_ipv6_statistics skb == null or is not ipv6");
		return;
	}

    ip6h = ipv6_hdr(skb);
    if (ip6h == NULL || (u8 *)ip6h < skb->head ||
 	    (skb_network_header(skb) + sizeof(*ip6h)) > skb_tail_pointer(skb)) {
        WIFIPRO_DEBUG("wifipro_update_ipv6_statistics ipv6_hdr(skb) failed or is not tcp");
        return;
    }

	if (ip6h->nexthdr != IPPROTO_TCP) {
        WIFIPRO_DEBUG("wifipro_update_ipv6_statistics ipv6_hdr(skb) is not tcp");
        return;
	}

	sk = (from_sk == NULL) ? skb->sk : from_sk;
 	// 检查是否为IPv6套接字
	if (sk == NULL || sk->sk_family != AF_INET6) {
		WIFIPRO_DEBUG("wifipro_update_ipv6_statistics sk=%p", sk);
		return;
	}

	inet = inet_sk(sk);
	if (inet == NULL) {
		WIFIPRO_DEBUG("wifipro_update_ipv6_statistics inet == NULL");
		return;
	}
	...
	// 使用正确的IPv6地址判断函数
	if (!wifipro_is_not_local_or_lan_sock6(&daddrv6)) {
		WIFIPRO_DEBUG("Local/LAN IPv6 address, skipping statistics update");
		return;
	}

	if (!wifipro_is_wlan(sk, skb)) {
		return;
	}

	if (mib_type == WIFIPRO_IPV6_INSEGS) {
		WIFIPRO_TCP_INC_STATS_BH(&init_net, WIFIPRO_IPV6_INSEGS);
	} else if (mib_type == WIFIPRO_IPV6_OUTSEGS) {
		WIFIPRO_TCP_INC_STATS_BH(&init_net, WIFIPRO_IPV6_OUTSEGS);
	} else {
		unknown_segs_count++;
	}
}
...
#ifdef CONFIG_HW_WIFIPRO_OPENHARMONY
static void update_packet_IPV6_data(struct wifipro_nl_packet_msg *packet)
{
	...
	unsigned int quality)
{
	...
	if (memset_s(packet, sizeof(struct wifipro_nl_packet_msg), 0, sizeof(struct wifipro_nl_packet_msg)) != EOK) {
		WIFIPRO_ERROR("alloc skb fail");
		kfree_skb(skb);
		skb = NULL;
		ret = -1;
...
static int wifipro_notify_rtt_msg(int pid, unsigned int msg_from)
{
	...
	packet = nlmsg_data(nlh);
	if (memset_s(packet, sizeof(struct wifipro_nl_packet_msg), 0, sizeof(struct wifipro_nl_packet_msg)) != EOK) {
		WIFIPRO_ERROR("wifipro_notify_rtt_msg memset_s error");
		kfree_skb(skb);
		skb = NULL;
		ret = -1;
		goto end;
...
#ifdef CONFIG_HW_WIFIPRO_OPENHARMONY
static int wifipro_notify_ipv6_msg(int pid, unsigned int msg_from)
{
	int ret;
	int size;
	struct sk_buff *skb = NULL;
	...
	nlh = nlmsg_put(skb, 0, 0, NETLINK_WIFIPRO_GET_IPV6_INFO, size, 0);
	if (nlh == NULL) {
		kfree_skb(skb);
		skb = NULL;
		ret = -1;
	...
	packet->msg_from = msg_from;
	update_packet_IPV6_data(packet);
	ret = netlink_unicast(g_wifipro_nlfd, skb, pid, MSG_DONTWAIT);
...
static void solve_nlmsg_type(struct nlmsghdr *nlh)
{
	...
	case NETLINK_HIDATA_GET_TCP_INFO:
		...
		break;
#ifdef CONFIG_HW_WIFIPRO_OPENHARMONY
	case NETLINK_WIFIPRO_GET_IPV6_INFO:
		wifipro_notify_ipv6_msg(nlh->nlmsg_pid, WIFIPRO_IPV6_QUERY);
		break;
#endif
	default:
	...
	unsigned char offset, const char *name)
{
	...
	if (strncpy_s(dst->state_name, WIFIPRO_MAX_CA_NAME, name, WIFIPRO_MAX_CA_NAME - 1) != EOK) {
		WIFIPRO_ERROR("wifipro_cong_stat_init strncpy_s error");
		return;
	}
...
#else
	if (sscanf_s(buffer, "%s", &log_level, sizeof(log_level)) != 1) {
#endif
...
static void solve_uid_state_print(struct seq_file *seq)
{
	...
	if (memset_s(s_uid_wlan_tcp_stat, sizeof(s_uid_wlan_tcp_stat), 0, sizeof(s_uid_wlan_tcp_stat)) != EOK) {
		WIFIPRO_ERROR("solve_uid_state_print s_uid_wlan_tcp_stat memset_s error");
		return;
	}
	s_wlan_tcp_stat_index = 0;
	...
	if (memset_s(s_uid_rmnet_tcp_stat, sizeof(s_uid_rmnet_tcp_stat), 0, sizeof(s_uid_rmnet_tcp_stat)) != EOK) {
		WIFIPRO_ERROR("solve_uid_state_print s_uid_rmnet_tcp_stat memset_s error");
		return;
	}
	s_rmnet_tcp_stat_index = 0;
}
...
static void update_hidata_tcp_stat_by_uid(kuid_t uid, int index, unsigned int count)
{
	...
	if (result == INVALID_INDEX) {
		...
		if (memset_s(&s_uid_hidata_tcp_stat[temp], sizeof(hidata_ui_dtcp_stat), 0, sizeof(hidata_ui_dtcp_stat)) != EOK) {
			WIFIPRO_ERROR("update_hidata_tcp_stat_by_uid memset_s error");
			return;
		}
...
static int hidata_tcp_info_report(int pid, kuid_t uid)
{
	...
	packet = nlmsg_data(nlh);
	if (memset_s(packet, sizeof(struct wifipro_nl_packet_msg), 0, sizeof(struct wifipro_nl_packet_msg)) != EOK) {
		WIFIPRO_ERROR("hidata_tcp_info_report memset_s error");
		ret = -1;
		goto end;
	}
