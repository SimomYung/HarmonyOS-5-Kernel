...
#ifndef CONFIG_SPI_MSM_GENI_MODULE 
...
static void rsmc_stop_wait_sta_timer(void)
{
	...
	if (g_device_ctx.recv_sta) {
		...
		mutex_lock(&g_device_ctx.recv_mutex);
		atomic_inc(&g_rsmc_spi_ctx.spi_irq);
		rsmc_spi_recv();
		mutex_unlock(&g_device_ctx.recv_mutex);
		chip_power_on(false);
		g_device_ctx.is_chip_on = false;
...
static void rsmc_disable(bool send_ack)
{
	...
	chip_power_on(false);
	g_device_ctx.is_chip_on = false;
	spin_lock_dn_inter_lock(flags);
	rsmc_clear_dn_inter_msg_list();
	spin_unlock_dn_inter_lock(flags);
	hwlog_info("%s: clear dn inter msg list done", __func__);
	spin_lock_dn_spi_lock(flags);
	rsmc_clear_dn_spi_msg_list();
	spin_unlock_dn_spi_lock(flags);
	hwlog_info("%s: clear dn spi msg list done", __func__);
...
msg_process *rsmc_device_reg_x801(notify_event *fun)
{
	...
	mutex_init(&g_device_ctx.recv_mutex);
...
void rsmc_device_unreg_x801(int reason)
{
	...
	mutex_destroy(&g_device_ctx.recv_mutex);
	rsmc_spi_exit_x801();
	rsmc_driver_exit();
}
