...
int rsmc_spi_read_x801(unsigned char *data, unsigned int length)
{
	...
	retval = memset_s(cd->rx_buf, sizeof(cd->rx_buf), 0, sizeof(cd->rx_buf));
	if (retval != EOK)
		hwlog_err("%s: memset_s rx_buf fail", __func__);
	spi_message_init(&cd->msg);
	cd->t.len = length;
	cd->t.tx_buf = cd->tx_buf;
	cd->t.rx_buf = cd->rx_buf;
#ifdef X801_SPI_CS
	cd->t.cs_change = 0;
#else
	cd->t.cs_change = 1;
	...
	memcpy_s(data, length, cd->rx_buf, length);
