...
#define HALL_BRACKETSHELL_DATE  2
HWLOG_REGIST();
...
static void wireless_cool_stop(void)
{
	struct wirelesscase_di *di = g_wirelesscase_di;

	if (di->product_id == WIRELESSCASE_VER_THREE_ID)
		wirelesscase_send_stop_working_msg();

	wltx_open_tx(WLTX_CLIENT_COOLINGCASE, false);
	hwlog_info("%s close coolingcase\n", __FUNCTION__);
}

static int wireless_get_cur_soc(void)
{
	int bat_cap = 0;

	power_supply_get_int_prop(POWER_PLATFORM_BAT_PSY_NAME,
		POWER_SUPPLY_PROP_CAPACITY, &bat_cap, POWER_SUPPLY_DEFAULT_CAPACITY, 1);
	hwlog_info("%s get bat_cap: %d \n", __FUNCTION__, bat_cap);
...
enum wirelesscase_online_state wirelesscase_get_online_state_new(void)
{
	...
	if (di->product_type == BRACKETSHELL_PRODUCT_TYPE)
		return WLCASE_ONLINE_BRACKETSHELL;
	return WLCASE_OFFLINE;
}
...
static void coolingcase_monitor_voltage_work(struct work_struct *work)
{
	...
	if ((g_coolingcase_bst5v_flag && (tx_vin < LV_THRESHOLD)) ||
		...
		wltx_open_tx(WLTX_CLIENT_COOLINGCASE, false);
		di->vin_change_flag = false;
		di->sys_change_flag = false;
...
static int wirelesscase_check_stop_watchdog(struct wirelesscase_di *di)
{
	return (di->case_cap_min < di->coolingcase_cap.no_cable_min);
}

static void coolingcase_watchdog_work(struct work_struct *work)
{
	struct wirelesscase_di *di = g_wirelesscase_di;

	if (!di) {
		hwlog_info("g_wirelesscase_di is null\n");
		return;
	}

	hwlog_info("coolingcase watchdog ++\n");
	if (wirelesscase_check_stop_watchdog(di)) {
		if (!wireless_tx_get_tx_open_flag())
				wltx_open_tx(WLTX_CLIENT_COOLINGCASE, true);

		hwlog_info("not enough power, watchdog exit\n");
		return;
	}

	if (di->sysfs_val == WIRELESSCASE_POWER_OFF || di->sysfs_val == COOLINGCASE_STOP_FLOW) {
		hwlog_info("sysfs_val = %d, watchdog exit\n", di->sysfs_val);
		return;
	}

	wltx_open_tx(WLTX_CLIENT_COOLINGCASE, true);
	msleep(COOLINGCASE_THREE_WATCHDOG_DELAY);

	if (wirelesscase_check_stop_watchdog(di))
		return;

	wltx_open_tx(WLTX_CLIENT_COOLINGCASE, false);
	schedule_delayed_work(&di->watchdog_work,
		msecs_to_jiffies(COOLINGCASE_WATCHDOG_DELAY));
}

static int wirelesscase_check_need_charge_with_cable(struct wirelesscase_di *di)
{
	return (coolingcase_online_state() && (wireless_get_cur_soc() == (COOLINGCASE_THREE_CAP_HIGH + 1)) &&
			(di->product_id == WIRELESSCASE_VER_THREE_ID));
}

static void coolingcase_get_vin(struct work_struct *work)
{
	struct wirelesscase_di *di = g_wirelesscase_di;

	if (wirelesscase_check_need_charge_with_cable(di) && (di->usb_cable)) {
		hwlog_info("coolingcase wired recharge\n");
		if (!wireless_tx_get_tx_open_flag())
			wltx_open_tx(WLTX_CLIENT_COOLINGCASE, true);
	}
}
...
void coolingcase_reinit_tx_chip(void)
{
	...
	hwlog_info("coolingcase ping freq : %d, coolingcase ping duty : %d\n",
		di->ping_freq_cool, di->ping_duty_cool);
	(void)wltx_ic_set_ping_freq(WLTRX_IC_MAIN, di->ping_freq_cool);
	(void)wltx_ic_set_ping_duty(WLTRX_IC_MAIN, di->ping_duty_cool);
...
void wirelesscase_send_stop_working_msg(void)
{
	unsigned char msg = HWQI_PARA_WIRELESSCASE_STOP_MSG;
	int ret;
 
	ret = wireless_send_lightstrap_ctrl_msg(WLTRX_IC_MAIN,
		WIRELESS_PROTOCOL_QI, &msg, WIRELESSCASE_STOP_MSG_LEN);
 
	if (ret)
		hwlog_err("[wirelesscase_send_stop_working_msg] send stop msg fail\n");
}
...
static void lightstrap_control_with_three_coolingcase(struct wirelesscase_di *di)
{
	di->vin_change_flag = false;
	di->sys_change_flag = false;
	wltx_open_tx(WLTX_CLIENT_COOLINGCASE, true);
	msleep(COOLINGCASE_THREE_OPEN_DELAY);
	if (delayed_work_pending(&di->watchdog_work))
		cancel_delayed_work(&di->watchdog_work);
}
...
static void lightstrap_control_work(struct work_struct *work)
{
	...
	case LIGHTSTRAP_LIGHT_UP:
		lightstrap_light_up();
		break;
	case COOLINGCASE_STOP_FLOW:
		hwlog_info("disable coolingcase\n");
		if (di->product_id == WIRELESSCASE_VER_THREE_ID)
				lightstrap_control_with_three_coolingcase(di);
			wireless_cool_stop();
	default:
		break;
	}
...
static int coolingcase_check_version(u8 product_id)
{
	...
	case WIRELESSCASE_VER_THREE_ID:
		return COOLINGCASE_VERSION_TWO;
	default:
		return WIRELESSCASE_OFFLINE;
	}
}
...
static void wirelesscase_send_onoff_uevent(struct wirelesscase_di *di, char *state_buf, u8 product_id)
{
	int len;
	char *single_envp[LIGHTSTRAP_MAX_RX_SIZE_SINGLE] = { NULL, NULL };

	single_envp[LIGHTSTRAP_ENVP_OFFSET0] = kzalloc(LIGHTSTRAP_INFO_SINGLE_LEN, GFP_KERNEL);
	if (!single_envp[LIGHTSTRAP_ENVP_OFFSET0]) {
		hwlog_err("%s :kzalloc failed\n", __FUNCTION__);
		return;
	}

	len = snprintf_s(single_envp[LIGHTSTRAP_ENVP_OFFSET0], LIGHTSTRAP_INFO_SINGLE_LEN, LIGHTSTRAP_INFO_SINGLE_LEN - 1,
			"WIRELESSCASE=%s,PRODUCTTYPE=%02x,MODELID=%s,SUBMODELID=0,PRODUCTID=%02x", state_buf, di->product_type, di->model_id, product_id);
	if (len <= 0) {
		hwlog_err("%s :snprintf_s failed\n", __FUNCTION__);
		goto alloc_free;
	}
	kobject_uevent_env(&di->dev->kobj, KOBJ_CHANGE, single_envp);
	hwlog_info("%s :wirelesscase send case=%s uevent_single\n", __FUNCTION__, state_buf);

alloc_free:
	kfree(single_envp[LIGHTSTRAP_ENVP_OFFSET0]);
}

static void wirelesscase_send_on_uevent(struct wirelesscase_di *di)
{
	char *state_buf = "ON";
	return wirelesscase_send_onoff_uevent(di, state_buf, di->product_id);
}

static void wirelesscase_send_off_uevent(struct wirelesscase_di *di)
{
	char *state_buf = "OFF";
	u8 product_id = LIGHTSTRAP_OFF;
	return wirelesscase_send_onoff_uevent(di, state_buf, product_id);
}
...
static void wirelesscase_send_on_uevent(struct wirelesscase_di *di)
{
	...
	ret = snprintf_s(envp[LIGHTSTRAP_ENVP_OFFSET1], LIGHTSTRAP_INFO_LEN, LIGHTSTRAP_INFO_LEN - 1,
		"PRODUCTTYPE=%02x", di->product_type);
	if (ret < 0) {
		hwlog_err("(%s)snprintf_s OFFSET1 fail ret = %d\n", __func__, ret);
		goto alloc_fail1;
	}
	ret = snprintf_s(envp[LIGHTSTRAP_ENVP_OFFSET2], LIGHTSTRAP_INFO_LEN, LIGHTSTRAP_INFO_LEN - 1,
		"MODELID=%s", di->model_id);
	if (ret < 0) {
		hwlog_err("(%s)snprintf_s OFFSET2 fail ret = %d\n", __func__, ret);
		goto alloc_fail1;
	}
	ret = snprintf_s(envp[LIGHTSTRAP_ENVP_OFFSET4], LIGHTSTRAP_INFO_LEN, LIGHTSTRAP_INFO_LEN - 1,
		"PRODUCTID=%02x", di->product_id);
	if (ret < 0) {
		hwlog_err("(%s)snprintf_s OFFSET4 fail ret = %d\n", __func__, ret);
		goto alloc_fail1;
	}
...
alloc_fail2:
	kfree(envp[LIGHTSTRAP_ENVP_OFFSET1]);
}
...
static int  wirelesscase_parse_product_info(struct wirelesscase_di *di, void *data)
{
	...
	hwlog_info("product_type=%02x, product_id=%02x\n", di->product_type,
		di->product_id);
	if (di->product_type == COOLINGCASE_PRODUCT_TYPE) {
		if (di->product_id == WIRELESSCASE_VER_ZERO_ID)
		...
		if (wlrx_plim_get_src(WLTRX_DRV_MAIN) & BIT(WLRX_PLIM_SRC_WINDOW_SHELL))
			wlrx_plim_clear_src(WLTRX_DRV_MAIN, WLRX_PLIM_SRC_WINDOW_SHELL);
	}
...
static int wirelesscase_check_without_opened_by_sys(struct wirelesscase_di *di)
{
	return ((di->is_opened_by_hall) || (wltx_get_client() == WLTX_CLIENT_UI));
}

static int wirelesscase_check_need_charger(struct wirelesscase_di *di)
{
	return ((di->usb_cable) &&
		(wireless_get_cur_soc() > COOLINGCASE_THREE_CAP_HIGH) && di->vin_change_flag);
}

void wirelesscase_set_charge_flsg(struct wirelesscase_di *di)
{
	if (di->case_cap_min < di->coolingcase_cap.no_cable_min)
		di->sys_change_flag = true;
	else if (di->case_cap_min == di->coolingcase_cap.no_cable_max)
		di->sys_change_flag = false;
	else if ((di->case_cap_min >= di->coolingcase_cap.no_cable_min) && (!(di->sys_change_flag)))
		di->sys_change_flag = false;

	if (di->usb_cable) {
		if (di->case_cap_min < di->coolingcase_cap.cable_min)
			di->vin_change_flag = true;
		else if (di->case_cap_min == di->coolingcase_cap.cable_max)
			di->vin_change_flag = false;
		else if ((di->case_cap_min >= di->coolingcase_cap.cable_min) && (!(di->vin_change_flag)))
			di->vin_change_flag = false;
	} else {
		if (di->case_cap_min < di->coolingcase_cap.no_cable_min)
			di->vin_change_flag = true;
			...
			di->vin_change_flag = false;
		else if ((di->case_cap_min >= di->coolingcase_cap.no_cable_min) && (!(di->vin_change_flag)))
			di->vin_change_flag = false;
	}
}

static void wirelesscase_parse_cap_info_work(struct wirelesscase_di *di)
{
	int cur_soc = 0;
	int client = 0;

	wirelesscase_set_charge_flsg(di);

	cur_soc = wireless_get_cur_soc();
	client = wltx_get_client();

	hwlog_info("%s usb_cable:%d  sysfs_val:%d soc:%d case_cap_max:%d case_cap_min:%d low_cap_flag:%d\n",
		__FUNCTION__, di->usb_cable, di->sysfs_val, cur_soc,
		di->case_cap_max, di->case_cap_min, di->vin_change_flag);

	if (wirelesscase_check_without_opened_by_sys(di) || wirelesscase_check_need_charger(di) ||
		di->case_cap_min >= COOLINGCASE_THREE_ERROR_CAP) {
		hwlog_info("coolingcase opened by hall or UI hall:%d ui:%d",
			di->is_opened_by_hall, client);
		return;
	} else if (di->sysfs_val == WIRELESSCASE_POWER_ON) {
		if (!di->sys_change_flag) {
			msleep(COOLINGCASE_THREE_CAP_DELAY);
			wltx_open_tx(WLTX_CLIENT_COOLINGCASE, false);
			if (!delayed_work_pending(&di->watchdog_work))
				schedule_delayed_work(&di->watchdog_work,
					msecs_to_jiffies(COOLINGCASE_WATCHDOG_DELAY));
		} else {
			hwlog_info("coolingcase not enough power, continue charger\n");
			return;
		}
	} else {
		wireless_cool_stop();
...
static void wirelesscase_parse_cap_info(struct wirelesscase_di *di, u8 *data, int len)
{
	u8 case_cap_min;
	u8 case_cap_max;

	if (!di || !data || len < COOLINGCASE_THREE_HWQI_PKT_DATA)
		return;

	case_cap_min = data[0];
	case_cap_max = data[1];

	di-> case_cap_min = case_cap_min;
	di-> case_cap_max = case_cap_max;
	wirelesscase_parse_cap_info_work(di);
}
...
static void set_duty_low_power_work(struct work_struct *work)
{
	...
	int low_power_duty_min;
	int low_power_duty_max;
	...
	if (di->product_id == WIRELESSCASE_VER_THREE_ID && di->usb_cable) {
		low_power_duty_min = (int)di->coolingcase_duty.cable_min;
		low_power_duty_max = (int)di->coolingcase_duty.cable_max;
	} else {
		low_power_duty_min = (int)di->coolingcase_duty.hv_lp_min;
		low_power_duty_max = (int)di->coolingcase_duty.hv_lp_max;
	}

	hwlog_info("set working duty--min:%d, max:%d\n",
			low_power_duty_min, low_power_duty_max);
	(void)wltx_ic_set_min_duty(WLTRX_IC_MAIN, low_power_duty_min);
	(void)wltx_ic_set_max_duty(WLTRX_IC_MAIN, low_power_duty_max);
}
...
static void lightstrap_process_hall_off_event(struct wirelesscase_di *di)
{
	...
	if (!di->startup_identify_timeout) {
...
static int wirelesscase_check_need_charger_with_cable(struct wirelesscase_di *di)
{
	return (coolingcase_online_state() && (wireless_get_cur_soc() > COOLINGCASE_THREE_CAP_HIGH) &&
			(di->product_id == WIRELESSCASE_VER_THREE_ID));
}

static int wirelesscase_check_need_stop_without_cable(struct wirelesscase_di *di)
{
	return (coolingcase_online_state() && (di->sysfs_val != WIRELESSCASE_POWER_ON) && (di->product_id == WIRELESSCASE_VER_THREE_ID) &&
...
static int coolingcase_wired_recharge_notifier_call(struct notifier_block *nb,
	unsigned long event, void *data)
{
	...
	if (!di || !psy) {
		hwlog_info("wirelesscase or psy is null\n");
		return NOTIFY_OK;
	}

	if (!strcmp(psy->desc->name, POWER_PLATFORM_BAT_PSY_NAME))
		schedule_delayed_work(&di->get_vin, msecs_to_jiffies(0));
	...
	unsigned long event, void *data)
{
	...
	case POWER_NE_LIGHTSTRAP_ON:
		...
		if (di->bracket_shell) {
			if (!val)
				return NOTIFY_OK;
			if (*val == HALL_BRACKETSHELL_DATE) {
				di->product_type = BRACKETSHELL_PRODUCT_TYPE;
				return NOTIFY_OK;
			}
		}
		break;
	...
	case POWER_NE_LIGHTSTRAP_EPT:
		break;
	case POWER_NE_WLCASE_SOC:
		wirelesscase_parse_cap_info(di, (u8 *)data, COOLINGCASE_THREE_HWQI_PKT_DATA);
		break;
	case POWER_NE_USB_CONNECT:
		di->usb_cable = true;
		if (wirelesscase_check_need_charger_with_cable(di))
			wltx_open_tx(WLTX_CLIENT_COOLINGCASE, true);
		break;
	case POWER_NE_USB_DISCONNECT:
		di->usb_cable = false;
		if (wirelesscase_check_need_stop_without_cable(di)) {
			di->vin_change_flag = false;
			di->sys_change_flag = false;
			wireless_cool_stop();
			hwlog_info("coolingcase stop by usb cable disconnect\n");
		}
		break;
	default:
		return NOTIFY_OK;
...
static struct power_sysfs_attr_info lightstrap_sysfs_field_tbl[] = {
	power_sysfs_attr_rw(lightstrap, 0660, LIGHTSTRAP_SYSFS_CONTROL, control),
	...
	struct wirelesscase_di *di)
{
	...
	(void)power_dts_read_u32(power_dts_tag(HWLOG_TAG), np, "tx_ping_duty_cool",
		&di->ping_duty_cool, COOLINGCASE_PING_DUTY_DEFAULT);
	...
	(void)power_dts_read_u32(power_dts_tag(HWLOG_TAG), np, "bracket_shell",
		&di->bracket_shell, 0);
	...
	if (power_dts_read_u32_array(power_dts_tag(HWLOG_TAG), np, "coolingcase_duty",
		...
		di->coolingcase_duty.cable_min = COOLINGCASE_DUTY_DEFAULT;
		di->coolingcase_duty.cable_max = COOLINGCASE_DUTY_DEFAULT;
	}
	if (power_dts_read_u32_array(power_dts_tag(HWLOG_TAG), np, "coolingcase_cap",
		(u32 *)&di->coolingcase_cap, COOLINGCASE_CAP_LEN)) {
			di->coolingcase_cap.cable_min = COOLINGCASE_CAP_MIN_DEFAULT;
			di->coolingcase_cap.cable_max = COOLINGCASE_CAP_MAX_DEFAULT;
			di->coolingcase_cap.no_cable_min = COOLINGCASE_CAP_MIN_DEFAULT;
			di->coolingcase_cap.no_cable_max = COOLINGCASE_CAP_MAX_DEFAULT;
	}
...
static void wirelesscase_init_delayed_work(struct wirelesscase_di *di)
{
	...
	INIT_DELAYED_WORK(&di->watchdog_work, coolingcase_watchdog_work);
	INIT_DELAYED_WORK(&di->get_vin, coolingcase_get_vin);
}
...
static int lightstrap_probe(struct platform_device *pdev)
{
	...
	di->soc_nb.notifier_call = lightstrap_event_notifier_call;
	ret = power_event_bnc_register(POWER_BNT_LIGHTSTRAP, &di->soc_nb);
	if (ret) {
		hwlog_err("power_event_bnc_register reg fail\n");
		goto capcity_notifier_regist_fail;
	}

	di->wired_nb.notifier_call = coolingcase_wired_recharge_notifier_call;
	ret = power_supply_reg_notifier(&di->wired_nb);
	if (ret) {
		hwlog_err("power_supply_reg_notifier reg fail\n");
...
logic_ops_regist_fail:
	(void)power_event_bnc_unregister(POWER_BNT_LIGHTSTRAP, &di->soc_nb);
power_supply_reg_fail:
	(void)power_supply_unreg_notifier(&di->wired_nb);
capcity_notifier_regist_fail:
