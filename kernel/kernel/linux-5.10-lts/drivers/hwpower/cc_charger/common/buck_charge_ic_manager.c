...
int charge_get_vbus_vset(int *volt)
{
	int ret;
	struct charge_device_ops *temp_ops = bc_ic_get_ic_ops();

	if (!temp_ops || !temp_ops->get_vbus_vset) {
		hwlog_err("ops or get_vbus_vset is null\n");
		return -EPERM;
	}

	ret = temp_ops->get_vbus_vset(volt);
	hwlog_info("get_vbus_vset: volt=%d ret=%d\n", *volt, ret);
	return ret;
}
...
bool charge_need_vusb_replace_vbus(void)
{
	struct charge_device_ops *temp_ops = bc_ic_get_ic_ops();

	if (!temp_ops || !temp_ops->need_vusb_replace_vbus)
		return false;

	return temp_ops->need_vusb_replace_vbus();
}
...
int charge_set_otg_switch_mode_enable(int enable)
{
	struct charge_device_ops *temp_ops = bc_ic_get_ic_ops();

	if (!temp_ops || !temp_ops->set_otg_switch_mode_enable) {
		hwlog_err("set_otg_switch_mode_enable is null\n");
		return -EPERM;
	}

	return temp_ops->set_otg_switch_mode_enable(enable);
}
...
const char *charge_get_dev_name(void)
{
	struct charge_device_ops *temp_ops = bc_ic_get_ic_ops();

	if (!temp_ops || !temp_ops->get_dev_name) {
		hwlog_err("get_dev_name is null!\n");
		return NULL;
	}

	return temp_ops->get_dev_name();
}

int charge_get_vsys_mos_enable(bool *is_enable)
{
	int ret;
	struct charge_device_ops *temp_ops = bc_ic_get_ic_ops();

	if (!temp_ops || !temp_ops->get_vsys_mos_enable)
		return -EPERM;

	ret = temp_ops->get_vsys_mos_enable(is_enable);
	return ret;
}

int charge_set_vsys_mos_enable(bool enable)
{
	int ret;
	struct charge_device_ops *temp_ops = bc_ic_get_ic_ops();

	if (!temp_ops || !temp_ops->set_vsys_mos_enable)
		return -EPERM;

	ret = temp_ops->set_vsys_mos_enable(enable);
	return ret;
}

int charge_get_curr_balance_mos_enable(bool *is_enable)
{
	int ret;
	struct charge_device_ops *temp_ops = bc_ic_get_ic_ops();

	if (!temp_ops || !temp_ops->get_curr_balance_mos_enable)
		return -EPERM;

	ret = temp_ops->get_curr_balance_mos_enable(is_enable);
	return ret;
}

int charge_set_curr_balance_mos_enable(bool enable)
{
	int ret;
	struct charge_device_ops *temp_ops = bc_ic_get_ic_ops();

	if (!temp_ops || !temp_ops->set_curr_balance_mos_enable)
		return -EPERM;

	ret = temp_ops->set_curr_balance_mos_enable(enable);
	return ret;
}
