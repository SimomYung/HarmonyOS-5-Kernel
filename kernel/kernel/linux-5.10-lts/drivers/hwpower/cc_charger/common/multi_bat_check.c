...
#define BAT_SOC_LOW_CNT             30
#define BAT_CONN_PATH_CNT_MAX       2
...
enum {
	BAT_CONN_PATH_CHG_MODE,
	BAT_CONN_PATH_INDEX,
	BAT_CONN_PATH_THRE_NAME,
	BAT_CONN_PATH_DIFF_NAME,
	BAT_CONN_PATH_DIS_INDEX,
	BAT_CONN_PATH_MAX,
};

struct bat_connect_path_para {
	int chg_mode;
	int path_index;
	int thre_para_len;
	int diff_para_len;
	int disable_path_index;
	struct bat_thre_para thre_para[BAT_CNT_MAX];
	struct bat_diff_para diff_para[BAT_CNT_MAX];
};
...
struct multi_bat_ck_dev {
	...
	struct bat_connect_path_para conn_path_para[BAT_CONN_PATH_CNT_MAX];
};
...
static unsigned int multi_bat_ck_get_vterm_dec(void)
{
	unsigned int vterm_dec = 0;

	vterm_dec = buck_charge_get_charge_core_data(BUCK_CHARGE_CORE_VTERM_DEC);
	if (vterm_dec > BUCK_CHARGE_CORE_VTERM_DEC_MAX)
		vterm_dec = BUCK_CHARGE_CORE_VTERM_DEC_MAX;
...
static bool multi_bat_check_thre(struct bat_thre_para *para, int len, struct bat_rt_para *info, bool cur_flag)
{
	...
	vterm_dec = multi_bat_ck_get_vterm_dec();
	...
	for (i = 0; i < len; i++) {
		...
		vol_hth = para[i].en_vol_hys ? (para[i].vol_hth_hysteresis - vterm_dec) : (para[i].vol_hth - vterm_dec);
		...
		if ((para[i].soc_lth > 0) && (info[id].soc < para[i].soc_lth)) {
			hwlog_info("[%d]soc=%d is smaller than %d\n", id, info[id].soc, para[i].soc_lth);
			return false;
		}
...
static bool multi_bat_check_path_thre(struct bat_thre_para *para, int len,
	struct bat_rt_para *info, bool is_same_mode)
{
	...
	for (i = 0; i < len; i++) {
		...
		if (para[i].soh_cnt < BAT_SOC_LOW_CNT) {
			hwlog_info("[%d]soc=%d is greater than %d\n", id, info[id].cur, para[i].soc_lth);
			return false;
		}
		...
		if ((para[i].cur_lth >= 0) && (info[id].cur > para[i].cur_lth))
			para[i].cur_cnt++;

		if (para[i].cur_cnt >= BAT_CUR_LOW_CNT) {
			dc_ctrl_fill_eh_buf_and_set_err_flag_true(DC_EH_LVC3_ABNORMAL, NULL);
			hwlog_info("[%d]cur=%d is greater than %d\n", id, info[id].cur, para[i].cur_lth);
			return false;
		}
	}
...
typedef bool (*multi_bat_check_compare)(int, int);
static bool multi_bat_check_greater(int first, int second)
{
	return first > second;
}

static bool multi_bat_check_less(int first, int second)
{
	return first < second;
}

static bool multi_bat_check_diff(struct bat_diff_para *para, int len,
	struct bat_rt_para *info, bool is_same_mode, multi_bat_check_compare compare)
{
	...
	for (i = 0; i < len; i++) {
		...
		if (compare(diff_vol, diff_vol_th)) {
			hwlog_info("%d %d diff_vol of %d %d, diff is %d\n", id0, id1, info[id0].vol,
				info[id1].vol, diff_vol_th);
			return false;
		}

		diff_soc_th = is_same_mode ? para[i].exit_diff_soc : para[i].enter_diff_soc;
		if (compare(diff_soc, diff_soc_th)) {
			hwlog_info("is_same_mode %u %d %d diff_soc of %d %d, diff is %d\n",
	...
	bool cur_flag, bool is_same_mode)
{
	...
	if (!multi_bat_check_diff(conn_para[i].diff_para, conn_para[i].diff_para_len, di->rt_info,
		is_same_mode, multi_bat_check_greater)) {
...
u32 multi_batchk_get_support_path(int cur_mode, int cur_path)
{
	int i;
	bool is_same_mode;
	u32 suppor_path = CHARGE_MULTI_IC;
	struct multi_bat_ck_dev *di = multi_bat_ck_get_di();
	...
	for (i = 0; i < di->bat_conn_path_len; i++) {
		if (cur_mode != di->conn_path_para[i].chg_mode)
			continue;

		if (!(suppor_path & BIT(di->conn_path_para[i].path_index)))
			continue;

		is_same_mode = (u32)cur_path & BIT(di->conn_path_para[i].path_index);
		if (!multi_bat_check_path_thre(di->conn_path_para[i].thre_para,
			di->conn_path_para[i].thre_para_len, di->rt_info, is_same_mode)) {
			suppor_path &= ~BIT(di->conn_path_para[i].path_index);
			continue;
		}

		if (!multi_bat_check_diff(di->conn_path_para[i].diff_para,
			di->conn_path_para[i].diff_para_len, di->rt_info, is_same_mode,
			multi_bat_check_less)) {
			suppor_path &= ~BIT(di->conn_path_para[i].path_index);
			continue;
		}

		if (di->conn_path_para[i].disable_path_index >= 0)
			suppor_path &= ~BIT(di->conn_path_para[i].disable_path_index);
	}
	...
	struct bat_thre_para *info, int *info_len, const char *name)
{
	...
	*info_len = len / BAT_THRE_MAX;
	for (row = 0; row < *info_len; row++) {
		...
		col = row * BAT_THRE_MAX + BAT_THRE_SOC_LTH;
		info[row].soc_lth = data[col];
		...
		hwlog_info("%s[%d]=%d %d %d %d %d %d %d\n", name, row,
			info[row].bat_id, info[row].vol_lth,
			info[row].vol_hth, info[row].soc_hth, info[row].soc_lth,
...
static void __multi_bat_ck_parse_conn_path_para(struct device_node *np, const char *prop,
	int conn_cnt_max, struct bat_connect_path_para *conn_path_para, int *bat_conn_path_len)
{
	int i, row, col, len;
	int idata = 0;
	const char *tmp_string = NULL;
	...
	len = power_dts_read_count_strings(power_dts_tag(HWLOG_TAG), np,
		prop, conn_cnt_max, BAT_CONN_PATH_MAX);
	if (len < 0)
		return;
	...
	for (i = 0; i < len; i++) {
		...
		row = i / BAT_CONN_PATH_MAX;
		col = i % BAT_CONN_PATH_MAX;
		switch (col) {
		case BAT_CONN_PATH_CHG_MODE:
			if (kstrtoint(tmp_string, POWER_BASE_DEC, &conn_path_para[row].chg_mode))
				return;
			break;
		case BAT_CONN_PATH_INDEX:
			if (kstrtoint(tmp_string, POWER_BASE_DEC, &conn_path_para[row].path_index))
				return;
			break;
		case BAT_CONN_PATH_THRE_NAME:
			multi_bat_ck_parse_thre_para(np, conn_path_para[row].thre_para,
				&conn_path_para[row].thre_para_len, tmp_string);
			break;
		case BAT_CONN_PATH_DIFF_NAME:
			multi_bat_ck_parse_diff_para(np, conn_path_para[row].diff_para,
				&conn_path_para[row].diff_para_len, tmp_string);
			break;
		case BAT_CONN_PATH_DIS_INDEX:
			if (kstrtoint(tmp_string, POWER_BASE_DEC, &conn_path_para[row].disable_path_index))
				return;
			break;
		default:
			break;
	...
	*bat_conn_path_len = len / BAT_CONN_PATH_MAX;
	for (i = 0; i < *bat_conn_path_len; i++)
		hwlog_info("%s[%d]=%d %d\n", prop, i, conn_path_para[i].chg_mode, conn_path_para[i].path_index);
}

static void multi_bat_ck_parse_conn_path_para(struct device_node *np, struct multi_bat_ck_dev *di)
{
	__multi_bat_ck_parse_conn_path_para(np, "bat_connect_path_para", BAT_CONN_PATH_CNT_MAX,
		di->conn_path_para, &di->bat_conn_path_len);
}
	...
	struct multi_bat_ck_dev *di)
{
	...
	multi_bat_ck_parse_conn_path_para(np, di);
	return 0;
}
