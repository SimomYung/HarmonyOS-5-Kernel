...
static int multi_ic_check_select_charge_path(struct multi_ic_check_mode_para *para,
	int *mode, u32 support_path)
{
	u32 i;
	u32 min_i = 0;
	...
	default:
		for (i = 0; i < CHARGE_PATH_MAX_NUM; i++) {
			if ((BIT(i) & support_path) && (para->ic_error_cnt[i] < MULTI_IC_CHECK_ERR_CNT_MAX)) {
				*mode = BIT(i);
				hwlog_info("select init mode=%d\n", *mode);
				return 0;
			}
	...
	int *mode, u32 support_path)
{
	...
	if (!para->support_multi_ic && !para->support_single_ic_switch) {
	...
	return multi_ic_check_select_charge_path(para, mode, support_path);
}
