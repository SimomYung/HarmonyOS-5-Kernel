...
int dc_get_gain_ibus(void)
{
	...
	if (dc_rechg_is_en()) {
		hwlog_info("add buck ibus\n");
		gain_ibus += charge_get_ibus();
	}
...
int direct_charge_get_device_vbus(int *value, bool flag)
{
	int ret;
	struct direct_charge_device *l_di = direct_charge_get_di();
	...
	ret = flag ? dcm_get_ic_vusb(l_di->working_mode, l_di->cur_mode, value) :
		dcm_get_ic_vbus(l_di->working_mode, l_di->cur_mode, value);
	if (ret) {
		*value = 0;
...
int direct_charge_get_vbus_vbat(int *vbus, int *vbat)
{
	...
	direct_charge_get_device_vbus(vbus, charge_need_vusb_replace_vbus());
...
unsigned int direct_charge_update_local_mode(unsigned int mode)
{
	...
	if (lvc_di) {
		if ((lvc_di->sysfs_enable_charger == 0) &&
			!(lvc_di->sysfs_mainsc_enable_charger ^ lvc_di->sysfs_auxsc_enable_charger)) {
			local_mode &= ~LVC_MODE;
