...
int dc_sysfs_set_ichg_limit(int local_mode, int val)
{
	...
	if (val == 0)
		l_dev->ichg_limit = di->iin_thermal_default;
	else if (val < cur_low)
		l_dev->ichg_limit = cur_low;
	else
		l_dev->ichg_limit = val;

	hwlog_info("set ichg_limit: %u, limit current: %d\n", val, l_dev->ichg_limit);
	return 0;
}
...
int dc_sysfs_get_ichg_battery_care(int local_mode)
{
	struct dc_sysfs_dev *di = dc_sysfs_get_di(local_mode);
	int ichg_cur;
	...
	ichg_cur = di->ichg_limit;

	hwlog_info("battery care ichg limit cur=%d\n", ichg_cur);
	return ichg_cur;
}
...
static int dc_sysfs_get_max_iin_thermal(int mode)
{
	struct dc_sysfs_dev *di = dc_sysfs_get_di(mode);
	...
	return (l_dev->multi_ic_mode_para.support_multi_ic ?
		power_get_max_value(di->iin_thermal_array, DC_SYSFS_CHANNEL_TYPE_END) :
		di->iin_thermal_array[DC_SYSFS_SINGLE_CHANNEL]);
}

int dc_sysfs_get_optimal_mode(void)
{
	int lvc_iin, sc_iin, sc4_iin;

	lvc_iin = dc_sysfs_get_max_iin_thermal(LVC_MODE);
	sc_iin = dc_sysfs_get_max_iin_thermal(SC_MODE);
	sc4_iin = dc_sysfs_get_max_iin_thermal(SC4_MODE);
	hwlog_info("lvc_iin=%d sc_iin=%d sc4_iin=%d\n", lvc_iin, sc_iin, sc4_iin);

	if ((sc4_iin >= sc_iin) && (sc4_iin >= lvc_iin))
		return SC4_MODE;
	else if ((sc_iin >= sc4_iin) && (sc_iin >= lvc_iin))
		return SC_MODE;
	else
		return LVC_MODE;
}
...
static struct power_sysfs_attr_info g_dc_sysfs_field_tbl[] = {
	...
	power_sysfs_attr_ro(dc, 0444, DC_SYSFS_MAX_POWER, max_power),
	...
	power_sysfs_attr_ro(dc, 0444, DC_SYSFS_ICHG_LIMIT, ichg_limit),
};
...
static int dc_sysfs_get_multi_cur(struct dc_sysfs_dev *l_dev, int local_mode, char *buf, size_t offset)
{
	...
	while ((ret > 0) && (i < di->curr_info.channel_num)) {
		...
		ret += snprintf(temp_buf, DC_SC_CUR_LEN,
			...
			di->curr_info.tbat[i]);
		if (strncat_s(buf, PAGE_SIZE, temp_buf, strlen(temp_buf)))
			return 0;
		i++;
	}
...
static int dc_sysfs_thermal_reason_show(struct dc_sysfs_dev *l_dev, int local_mode, char *buf, size_t offset)
{
	...
	client = dc_vote_get_effective_client_locked(VOTE_OBJ_CURRENT, false);
	if (!client)
		client = "None";
...
static int dc_sysfs_ichg_limit_show(struct dc_sysfs_dev *l_dev, int local_mode, char *buf, size_t offset)
{
...
static int dc_sysfs_max_power_show(struct dc_sysfs_dev *l_dev, int local_mode, char *buf, size_t offset)
{
	return snprintf(buf, PAGE_SIZE, "%d\n", dc_pmode_get_optimal_mode_max_pwr(QUICK_CHECK));
}

static int dc_sysfs_cable_type_info_show(struct dc_sysfs_dev *l_dev, int local_mode, char *buf, size_t offset)
{
	unsigned int cable_type;
	unsigned int ctc_cable;

	ctc_cable = dc_get_cable_inherent_info(DC_IS_CTC_CABLE);
	cable_type = dc_get_cable_inherent_info(DC_CABLE_TYPE);
	/* distinguishing ATC and CTC cables */
	if (ctc_cable && cable_type)
		cable_type += CTC_CABLE_OFFSET;
...
static void dc_sysfs_para_init(struct dc_sysfs_dev *di, int iin_thermal_default)
{
	...
	di->ichg_limit = iin_thermal_default;
