...
static bool dc_check_support_path(struct direct_charge_device *di)
{
	u32 support_path;
	...
	support_path = multi_batchk_get_support_path(di->working_mode, di->cur_mode);
	if (!(support_path & di->cur_mode)) {
...
void dc_control_work(struct work_struct *work)
{
	...
	if (di->local_mode != dc_pmode_get_optimal_mode(NORMAL_CHECK, di->local_mode, true) ||
		dc_is_power_redistributed()) {
		dc_comm_set_need_recheck_flag(true);
		goto out;
	}

	if (!dc_check_support_path(di))
		goto out;
