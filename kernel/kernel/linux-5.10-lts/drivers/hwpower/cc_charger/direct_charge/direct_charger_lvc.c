...
static int mainlvc_set_enable_charger(unsigned int val)
{
	struct direct_charge_device *di = g_lvc_di;
...
static int mainlvc_get_enable_charger(unsigned int *val)
{
	struct direct_charge_device *di = g_lvc_di;
...
static int auxlvc_set_enable_charger(unsigned int val)
{
	struct direct_charge_device *di = g_lvc_di;
...
static int auxlvc_get_enable_charger(unsigned int *val)
{
	struct direct_charge_device *di = g_lvc_di;
...
static int lvc_set_ichg_limit(unsigned int val)
{
	return dc_sysfs_set_ichg_limit(LVC_MODE, (int)val);
}
...
static int lvc_set_iin_thermal(unsigned int index, unsigned int iin_thermal_value)
{
	...
	return dc_sysfs_set_iin_thermal_array(LVC_MODE, index, iin_thermal_value);
}

static int lvc_set_iin_thermal_all(unsigned int value)
{
	int i;

	for (i = DC_CHANNEL_TYPE_BEGIN; i < DC_CHANNEL_TYPE_END; i++) {
		if (dc_sysfs_set_iin_thermal_array(LVC_MODE, i, value))
			return -EPERM;
	}
	return 0;
}
...
static struct power_if_ops lvc_if_ops = {
	...
	.set_iin_thermal_all = lvc_set_iin_thermal_all,
	.set_ichg_limit = lvc_set_ichg_limit,
...
static struct power_if_ops mainlvc_if_ops = {
	.set_enable_charger = mainlvc_set_enable_charger,
	.get_enable_charger = mainlvc_get_enable_charger,
	.get_rt_test_prot_type = lvc_get_rt_test_prot_type,
	.type_name = "mainlvc",
};

static struct power_if_ops auxlvc_if_ops = {
	.set_enable_charger = auxlvc_set_enable_charger,
	.get_enable_charger = auxlvc_get_enable_charger,
	.get_rt_test_prot_type = lvc_get_rt_test_prot_type,
	.type_name = "auxlvc",
};

static void lvc_power_if_ops_register(struct direct_charge_device *di)
{
	power_if_ops_register(&lvc_if_ops);
	if (di->multi_ic_mode_para.support_single_ic_switch) {
		power_if_ops_register(&mainlvc_if_ops);
		power_if_ops_register(&auxlvc_if_ops);
	}
}
...
static void lvc_fault_work(struct work_struct *work)
{
	...
	case POWER_NE_DC_FAULT_CONV_OCP:
		...
		break;
	case POWER_NE_DC_FAULT_CFLY_SHORT:
		hwlog_err("cfly short or open happened\n");
		snprintf(buf, sizeof(buf), "cfly short or open happened\n");
		strncat(buf, reg_info, strlen(reg_info));
		power_dsm_report_dmd(POWER_DSM_DIRECT_CHARGE_SC,
			POWER_DSM_DIRECT_CHARGE_SC_FAULT_CFLY_SHORT, buf);
		break;
	default:
