...
static int dc_mmi_parse_dts(struct device_node *np, struct dc_mmi_data *di)
{
	...
	(void)power_dts_read_u32(power_dts_tag(HWLOG_TAG), np,
		"batt_bal_test", (u32 *)&di->batt_bal_test, 0);
	...
	unsigned long event, void *data)
{
	...
	case POWER_NE_DC_CHECK_SUCC:
		...
		break;
	case POWER_NE_BATT_BAL_ABNORMAL:
		di->batt_bal_check_result = false;
		hwlog_info("battery balance is abnormal\n");
		break;
	default:
		break;
	}
...
static int dc_mmi_get_btb_ck_result(int type)
{
	u32 result = 0;

	btb_ck_get_result_now(type, &result);
	hwlog_info("dc mmi btb check begin, type is %d, result is %u\n", type, result);
...
static int dc_mmi_sc_result(struct dc_mmi_data *di, int mode)
{
	...
	if (dc_mmi_get_btb_ck_result(BTB_VOLT_CHECK) || dc_mmi_get_btb_ck_result(BTB_TEMP_CHECK))
	...
	if (di->batt_bal_test) {
		dc_mmi_set_succ_flag(mode, DC_ERROR_BATT_BAL_ABNORMAL);
		if (!di->batt_bal_check_result)
			return dc_mmi_get_sc_succ_flag(di, mode);
	}
