...
static struct charge_mode_map g_mode_tbl[] = {
	...
	{ "lvc_main", CHARGE_MODE_TYPE_MAIN_LVC },
	{ "lvc_aux",  CHARGE_MODE_TYPE_AUX_LVC },
...
static struct charge_mode_action g_action_tbl[] = {
	...
	{ POWER_IF_OP_TYPE_MAINLVC, POWER_IF_OP_TYPE_LVC },
	{ POWER_IF_OP_TYPE_AUXLVC,  POWER_IF_OP_TYPE_LVC },
...
static int charge_mode_multi_sc_test(struct charge_mode_dev *di)
{
	struct direct_charge_device *l_di = direct_charge_get_di();

	if (!(di->mode_para[di->mode_idx].test_flag & MULTI_SC_CHECK_FLAG))
		return 0;
	...
	if (l_di->cur_mode != CHARGE_MULTI_IC) {
		hwlog_info("now is not multi sc\n");
		return -EPERM;
	}
...
static bool charge_mode_caculate(struct charge_mode_dev *di)
{
	...
	if (charge_mode_multi_sc_test(di))
		return false;
...
static void charge_mode_update_result(struct charge_mode_dev *di)
{
	...
	if ((di->adp_mode != 0 && (di->adp_mode & g_adp_mode_map[index]) == 0) &&
		((di->mode_para[di->mode_idx].test_flag & FORCE_TEST_FLAG) == DISABLE)) {
