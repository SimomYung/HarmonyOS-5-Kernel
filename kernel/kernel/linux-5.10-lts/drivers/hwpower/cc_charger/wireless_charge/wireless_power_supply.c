...
static void wlps_gpio_val_check(unsigned int gpio_type, int gpio_no, int set_val)
{
	...
	if ((gpio_no <= 0) || (gpio_type >= WLPS_GPIO_END))
		return;
...
static void wlps_vpsw_ctrl(unsigned int ic_type, bool flag)
{
	struct wlps_dev_info *di = wlps_get_dev_info(ic_type);

	if (!di || (di->vpsw_index < 0))
		return;

	hwlog_info("[vpsw_ctrl] %s\n", flag ? "on" : "off");
	if (flag)
		power_sw_set_output(di->vpsw_index, POWER_SW_ON);
	else
		power_sw_set_output(di->vpsw_index, POWER_SW_OFF);
}
...
static void wlps_txsw_ctrl(unsigned int ic_type, bool flag)
{
	...
	if (flag) {
		...
		wlps_vpsw_ctrl(ic_type, false);
		gpio_set_value(di->gpio_txsw, set_val);
	} else {
		set_val = !di->gpio_txsw_valid_val;
		gpio_set_value(di->gpio_txsw, set_val);
		wlps_vpsw_ctrl(ic_type, true);
	}
...
static void wlps_rx_ext_pwr_ctrl(unsigned int ic_type, bool flag)
{
	...
	if (flag) {
		...
		power_msleep(DT_MSLEEP_15MS, 0, NULL); /* wait for ldo5V stability */
	} else {
...
static int wlps_request_psw_ovp(struct device_node *np, struct wlps_dev_info *di)
{
	if ((di->gpio_need_request == 0) ||
		!test_bit(WLPS_GPIO_VPSW, &di->gpio_need_request)) {
		di->vpsw_index = -1;
		return 0;
	}

	if (power_dts_read_u32(power_dts_tag(HWLOG_TAG), np,
		"vpsw_index", (u32 *)&di->vpsw_index, 0))
		return -EINVAL;

	set_bit(WLPS_GPIO_VPSW, &di->gpio_requested);
	return 0;
}
