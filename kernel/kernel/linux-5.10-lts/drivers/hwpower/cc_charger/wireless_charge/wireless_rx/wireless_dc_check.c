...
static bool can_err_cnt_do_wldc_charge(struct wldc_dev_info *di, int mode)
{
	...
	if (wlrx_evt_is_pwr_good(WLTRX_DRV_MAIN))
	...
	cur_mode_name = di->mode_para[mode].init_para.dc_name;
	pmode_id = wlrx_pmode_get_pid_by_name(WLTRX_DRV_MAIN, cur_mode_name);
	wlrx_pmode_clear_icon_flag(WLTRX_DRV_MAIN, pmode_id);
	wlrx_pmode_icon_display(WLTRX_DRV_MAIN, WLRX_PMODE_NORMAL_JUDGE);
...
{
	u8 rx_support_mode = wlrx_pmode_get_support_mode(WLTRX_DRV_MAIN);
...
static int wldc_open_dc_path_regulate_vrx(struct wldc_dev_info *di)
{
	...
	if (wlrx_pmode_set_vrx(WLTRX_DRV_MAIN, di->rx_vout_set)) {
...
static int wldc_set_rx_init_vout(struct wldc_dev_info *di)
{
	...
	ret = wlrx_pmode_set_vtrx(WLTRX_DRV_MAIN, vtx, vrx);
	if (ret) {
...
static int wldc_check_prev_init(struct wldc_dev_info *di)
{
	...
	ret = multi_ic_check_select_init_mode(&di->ic_mode_para, &di->ic_data.cur_type, CHARGE_MULTI_IC);
	if (ret) {
		di->stop_flag_error = true;
		hwlog_err("prev_init: multi ic check failed\n");
	...
	wlrx_chip_init(di->drv_type, WILREESS_SC_CHIP_INIT);
