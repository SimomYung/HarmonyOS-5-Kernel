...
#define WLRX_ACC_TRX_PLIM_ROW          30
#define WLRX_ACC_TRX_PLIM_COL          8
#define WLRX_ACC_STHU_FEATURE_TH_ROW   4
#define WLRX_ACC_STHU_FEATURE_TH_DFT   0
...
enum wlrx_acc_tx_fan_speed_cfg {
	WLRX_ACC_TX_FAN_SPEED_BEGIN = 0,
	WLRX_ACC_TX_FAN_SPEED_TX_TYPE  = WLRX_ACC_TX_FAN_SPEED_BEGIN,
	WLRX_ACC_TX_FAN_SPEED_LEVEL_MAX,
	WLRX_ACC_TX_FAN_SPEED_LEVEL,
	WLRX_ACC_TX_FAN_SPEED_END,
};

enum wlrx_acc_trx_plim_cfg_index {
	WLRX_ACC_TRX_PLIM_BEGIN = 0,
	WLRX_ACC_TRX_PLIM_VTX = WLRX_ACC_TRX_PLIM_BEGIN,
	WLRX_ACC_TRX_PLIM_VRX,
	WLRX_ACC_TRX_PLIM_IRX,
	WLRX_ACC_TRX_PLIM_EXTRA,
	WLRX_ACC_TRX_PLIM_TX_TYPE,
	WLRX_ACC_TRX_PLIM_ADAP_TYPE,
	WLRX_ACC_TRX_PLIM_SCN_ID,
	WLRX_ACC_TRX_PLIM_EPT_TYPE,
	WLRX_ACC_TRX_PLIM_END,
};
...
struct wlrx_acc_trx_plim {
	struct wlrx_acc_trx_plim_cfg *cfg;
	int level;
};
...
struct wlrx_acc_sthu_feature_th {
	int fop_hth;
	int fop_lth;
	int vrect_hth;
	int vrect_lth;
...
struct wlrx_acc_dts {
	...
	struct wlrx_acc_sthu_feature_th sthu_feature_th;
	struct wlrx_acc_trx_plim trx_plim;
	struct wlrx_acc_tx_vpa_lim *tx_vpa_lim;
	struct wlrx_acc_fix_tx_fop fix_tx_fop;
	struct wlrx_acc_tx_fan_speed *tx_fan_speed;
	int tx_ilim_len;
	int tx_vpa_lim_len;
	int tx_fan_speed_len;
	int kid;
...
struct wlrx_acc_adaptor {
	u16 id;
	u8 manufacturer; /* 0:unknown 1:huawei 2:third-party */
	u16 err_code;
};
...
enum wlrx_acc_trx_plim_extra_type {
	WLRX_ACC_EXTRA_MIN = 0,
	WLRX_ACC_IS_STHU_TX = WLRX_ACC_EXTRA_MIN,
	WLRX_ACC_EXTRA_MAX,
};

struct wlrx_acc_trx_plim_extra_para {
	enum wlrx_acc_trx_plim_extra_type type;
	bool (*func)(unsigned int drv_type);
};
...
static int wlrx_acc_get_tx_ilim(struct wlrx_acc_dev *di, unsigned int ic_type)
{
	...
	for (i = 0; i < di->dts->tx_ilim_len; i++) {
		if (tx_ilim[i].tx_type == di->tx_type) {
			if ((tx_ilim[i].ocp > 0) && (di->ept_type == WLRX_ACC_EPT_OCP))
				ilim = tx_ilim[i].ocp;
			else
				ilim = tx_ilim[i].dflt;
			break;
...
static void wlrx_acc_uevent_report_work(struct work_struct *work)
{
	int i;
...
static void wlrx_acc_get_info(struct wlrx_acc_dev *di, unsigned int ic_type)
{
	...
	if (di->tx_type == WLACC_TX_UPGRADE) {
		wireless_get_tx_sn(ic_type, WIRELESS_PROTOCOL_QI);
		schedule_delayed_work(&di->uevent_report_work, msecs_to_jiffies(0));
	}
...
static void wlrx_acc_update_icon_display(unsigned int drv_type)
{
	...
	if (di->cap->no_need_cert)
		wlrx_pmode_icon_display(drv_type, WLRX_PMODE_NORMAL_JUDGE);
	else
		wlrx_pmode_icon_display(drv_type, WLRX_PMODE_QUICK_JUDGE);
}
...
static void wlrx_acc_start_prevfod_check(struct wlrx_acc_dev *di)
{
	...
	wlrx_chip_init(WLTRX_DRV_MAIN, WIRELESS_FODCHK_CHIP_INIT);
	...
	wlrx_chip_init(WLTRX_DRV_MAIN, WIRELESS_CHIP_INIT);
	(void)power_msleep(DT_MSLEEP_5S, DT_MSLEEP_50MS, wlrx_msleep_exit);
}
...
static void wlrx_acc_get_adp_info(struct wlrx_acc_dev *di, unsigned int drv_type)
{
	unsigned int ic_type = wlrx_acc_get_ic_type(drv_type);
	struct wlprot_acc_cap *cap = wlrx_acc_get_cap(drv_type);

	if (!cap)
		return;

	if (!cap->support_adp_info) {
		hwlog_info("[wlrx_acc_get_adp_info] not support\n");
		return;
	}

	(void)wireless_get_tx_adapter_id(ic_type, WIRELESS_PROTOCOL_QI, &di->adp.id, &di->adp.manufacturer);
	(void)wireless_get_tx_adapter_err_code(ic_type, WIRELESS_PROTOCOL_QI, &di->adp.err_code);
}

static void wlrx_acc_get_fod_result_info(struct wlrx_acc_dev *di, unsigned int drv_type)
{
	struct wlprot_acc_cap *cap = wlrx_acc_get_cap(drv_type);
	...
	if (!cap->support_fod_info) {
		hwlog_info("[wlrx_acc_get_fod_result_info] not support\n");
		return;
	}

	wlrx_get_fod_result_info(drv_type);
}
...
static void wlrx_acc_cap_post_handler(unsigned int drv_type)
{
	...
	wlrx_acc_get_adp_info(di, drv_type);
	wlrx_acc_get_fod_result_info(di, drv_type);
}
...
static void wlrx_acc_set_dflt_adaptor(struct wlrx_acc_dev *di)
{
	di->adp.id = 0;
	di->adp.manufacturer = 0;
	di->adp.err_code = 0;
}
...
void wlrx_acc_reset_para(unsigned int drv_type)
{
	...
	di->ept_type = WLRX_ACC_EPT_DEFAULT;
	wlrx_acc_notify_match_status(WLRX_ACC_DET_DEFAULT);
}

struct wlrx_acc_tx_fan_speed *wlrx_acc_get_tx_fan_speed(unsigned int drv_type)
{
	int i;
	struct wlrx_acc_dev *di = wlrx_acc_get_di(drv_type);
	...
	for (i = 0; i < di->dts->tx_fan_speed_len; i++) {
		if ((di->dts->tx_fan_speed[i].tx_type < 0) ||
			(di->dts->tx_fan_speed[i].tx_type == di->tx_type))
			return &di->dts->tx_fan_speed[i];
	}
...
static bool wlrx_is_dc_pwr_tx(unsigned int drv_type)
{
	struct wlrx_acc_dev *di = wlrx_acc_get_di(drv_type);
	...
	return di->cap->dc_pwr ? true : false;
}

static bool wlrx_is_sthu_tx(unsigned int drv_type)
{
	int ret;
	int fop = 0; 
	int vrect = 0;
	...
	if (!di || di->tx_type != WLACC_TX_CK30_LP || !wlrx_is_dc_pwr_tx(drv_type))
		return false;

	ret = wlrx_ic_get_ping_fop(WLTRX_IC_MAIN, &fop);
	ret += wlrx_ic_get_ping_vrect(WLTRX_IC_MAIN, &vrect);
	if (ret) {
		hwlog_info("[is_sthu_tx] parse failed");
		return false;
	}

	if ((fop < di->dts->sthu_feature_th.fop_hth) &&
		(fop > di->dts->sthu_feature_th.fop_lth) &&
		(vrect < di->dts->sthu_feature_th.vrect_hth) &&
		(vrect > di->dts->sthu_feature_th.vrect_lth))
		return true;

	return false;
}

static bool wlrx_acc_trx_plim_extra_valid(unsigned int drv_type, u32 type)
{
	u32 i;

	static struct wlrx_acc_trx_plim_extra_para extra[WLRX_ACC_EXTRA_MAX] = {
		 { WLRX_ACC_IS_STHU_TX, wlrx_is_sthu_tx },
	};

	for (i = 0; i < ARRAY_SIZE(extra); i++) {
		if (!(BIT(i) & type))
			continue;
		if (!extra[i].func(drv_type))
			return false;
	}
...
void wlrx_acc_update_trx_plim_para(unsigned int drv_type, struct wlrx_pctrl *pctrl)
{
	int i;
	enum wlrx_scene scn_id;
	struct wlprot_acc_cap *cap = wlrx_acc_get_cap(drv_type);
	struct wlrx_acc_dev *di = wlrx_acc_get_di(drv_type);
	struct wlrx_acc_trx_plim_cfg *para = NULL;

	if (!di || !di->dts || !di->dts->trx_plim.cfg ||
		(di->dts->trx_plim.level <= 0) || !cap)
		return;

	scn_id = wlrx_get_scene();

	for (i = 0; i < di->dts->trx_plim.level; i++) {
		para = &di->dts->trx_plim.cfg[i];
		if (!para)
			continue;
		if ((para->extra > 0) && !wlrx_acc_trx_plim_extra_valid(drv_type, para->extra))
			continue;
		if ((para->tx_type[0] >= 0) && (!power_array_contains_int(di->tx_type, para->tx_type, para->tx_type_size)))
			continue;
		if ((para->adap_type[0] >= 0) && (!power_array_contains_int(cap->adp_type, para->adap_type, para->adap_type_size)))
			continue;
		if ((para->scn_id[0] >= 0) && (!power_array_contains_int(scn_id, para->scn_id, para->scn_id_size)))
			continue;
		if ((para->ept_type[0] >= 0) && (!power_array_contains_int(di->ept_type, para->ept_type, para->ept_type_size)))
			continue;
...
static void wlrx_acc_parse_trx_plim_info_print(struct wlrx_acc_dts *dts)
{
	int i;

	for (i = 0; i < dts->trx_plim.level; i++) {
		hwlog_info("[parse_trx_plim] level: %d, tx_vout: %d, rx_vout: %d, rx_ilim: %d, extra: %d\n",
			i, dts->trx_plim.cfg[i].vtx, dts->trx_plim.cfg[i].vrx, dts->trx_plim.cfg[i].irx, dts->trx_plim.cfg[i].extra);
		power_print_int_array("tx_type", dts->trx_plim.cfg[i].tx_type, dts->trx_plim.cfg[i].tx_type_size);
		power_print_int_array("adap_type", dts->trx_plim.cfg[i].adap_type, dts->trx_plim.cfg[i].adap_type_size);
		power_print_int_array("scn_id", dts->trx_plim.cfg[i].scn_id, dts->trx_plim.cfg[i].scn_id_size);
		power_print_int_array("ept_type", dts->trx_plim.cfg[i].ept_type, dts->trx_plim.cfg[i].ept_type_size);
	}
}

static int wlrx_acc_parse_trx_plim_int_array(const char *tmp_str,
	struct wlrx_acc_trx_plim_cfg *trx_plim_cfg, int index, int col)
{
	int delimiter_cnt;

	if (!trx_plim_cfg || !tmp_str)
		return -EINVAL;

	delimiter_cnt = power_count_char_in_str(tmp_str, '|');
	if (delimiter_cnt < 0)
		return -EINVAL;

	switch (col) {
	case WLRX_ACC_TRX_PLIM_TX_TYPE:
		trx_plim_cfg[index].tx_type = kcalloc(delimiter_cnt + 1, sizeof(int), GFP_KERNEL);
		if (!(trx_plim_cfg[index].tx_type) || 
			power_str2intArray(tmp_str, '|', trx_plim_cfg[index].tx_type, delimiter_cnt + 1))
			return -EINVAL;
		trx_plim_cfg[index].tx_type_size = delimiter_cnt + 1;
		break;
	case WLRX_ACC_TRX_PLIM_ADAP_TYPE:
		trx_plim_cfg[index].adap_type = kcalloc(delimiter_cnt + 1, sizeof(int), GFP_KERNEL);
		if (!(trx_plim_cfg[index].adap_type) || 
			power_str2intArray(tmp_str, '|', trx_plim_cfg[index].adap_type, delimiter_cnt + 1))
			return -EINVAL;
		trx_plim_cfg[index].adap_type_size = delimiter_cnt + 1;
		break;
	case WLRX_ACC_TRX_PLIM_SCN_ID:
		trx_plim_cfg[index].scn_id = kcalloc(delimiter_cnt + 1, sizeof(int), GFP_KERNEL);
		if (!(trx_plim_cfg[index].scn_id) || 
			power_str2intArray(tmp_str, '|', trx_plim_cfg[index].scn_id, delimiter_cnt + 1))
			return -EINVAL;
		trx_plim_cfg[index].scn_id_size = delimiter_cnt + 1;
		break;
	case WLRX_ACC_TRX_PLIM_EPT_TYPE:
		trx_plim_cfg[index].ept_type = kcalloc(delimiter_cnt + 1, sizeof(int), GFP_KERNEL);
		if (!(trx_plim_cfg[index].ept_type) || 
			power_str2intArray(tmp_str, '|', trx_plim_cfg[index].ept_type, delimiter_cnt + 1))
			return -EINVAL;
		trx_plim_cfg[index].ept_type_size = delimiter_cnt + 1;
		break;
	default:
		break;
	}
...
static int wlrx_acc_parse_trx_plim_int(const char *tmp_str,
	struct wlrx_acc_trx_plim_cfg *trx_plim_cfg, int index, int col)
{
	int tmp = 0;
	...
	if (kstrtoint(tmp_str, 0, &tmp))
		return -EINVAL;

	switch (col) {
	case WLRX_ACC_TRX_PLIM_VTX:
		trx_plim_cfg[index].vtx = tmp;
		break; 
	case WLRX_ACC_TRX_PLIM_VRX:
		trx_plim_cfg[index].vrx = tmp;
		break;
	case WLRX_ACC_TRX_PLIM_IRX:
		trx_plim_cfg[index].irx = tmp;
		break;
	case WLRX_ACC_TRX_PLIM_EXTRA:
		trx_plim_cfg[index].extra = tmp;
		break;
	default:
		break;
	}
...
static void wlrx_acc_parse_trx_plim(const struct device_node *np, struct wlrx_acc_dts *dts)
{
	int i, len, size, ret;
	const char *tmp_str = NULL;
	struct wlrx_acc_trx_plim_cfg *trx_plim_cfg = NULL;

	if (dts->tx_ilim_len > 0)
		return;

	len = power_dts_read_count_strings(power_dts_tag(HWLOG_TAG), np,
		"trx_plim", WLRX_ACC_TRX_PLIM_ROW, WLRX_ACC_TRX_PLIM_COL);
	if (len <= 0)
		return;

	size = len / WLRX_ACC_TRX_PLIM_COL;
	trx_plim_cfg = kcalloc(size, sizeof(struct wlrx_acc_trx_plim_cfg), GFP_KERNEL);
	if (!trx_plim_cfg)
		goto kcalloc_fail;
	...
	for (i = 0; i < len; i++) {
		...
		if (i % WLRX_ACC_TRX_PLIM_COL <= WLRX_ACC_TRX_PLIM_EXTRA)
			ret = wlrx_acc_parse_trx_plim_int(tmp_str, trx_plim_cfg, i / WLRX_ACC_TRX_PLIM_COL, i % WLRX_ACC_TRX_PLIM_COL);
		else
			ret = wlrx_acc_parse_trx_plim_int_array(tmp_str, trx_plim_cfg, i / WLRX_ACC_TRX_PLIM_COL, i % WLRX_ACC_TRX_PLIM_COL);

		if (ret)
			goto error;
	}

	hwlog_info("[parse_tx_ilimit] parse succ\n");
	dts->trx_plim.cfg = trx_plim_cfg;
	dts->trx_plim.level = size;
	wlrx_acc_parse_trx_plim_info_print(dts);
	return;

error:
	for (i = 0; i < size;i++) {
		kfree(trx_plim_cfg[i].tx_type);
		kfree(trx_plim_cfg[i].adap_type);
		kfree(trx_plim_cfg[i].scn_id);
		kfree(trx_plim_cfg[i].ept_type);
	}
	kfree(trx_plim_cfg);

kcalloc_fail:
	hwlog_err("parse_tx_ilimit: failed\n");
}
...
static void wlrx_acc_parse_tx_fan_speed(const struct device_node *np, struct wlrx_acc_dts *dts)
{
	int len, i;
	int row;

	len = power_dts_read_count_strings(power_dts_tag(HWLOG_TAG), np,
		"tx_fan_speed", WLACC_TX_ERROR, WLRX_ACC_TX_FAN_SPEED_END);
	if (len <= 0)
		return;

	row = len / WLRX_ACC_TX_FAN_SPEED_END;
	dts->tx_fan_speed = kcalloc(row, sizeof(struct wlrx_acc_tx_fan_speed), GFP_KERNEL);
	if (!dts->tx_fan_speed)
		goto kcalloc_fail;

	len = power_dts_read_string_array(power_dts_tag(HWLOG_TAG), np,
		"tx_fan_speed", (u32 *)dts->tx_fan_speed, row, WLRX_ACC_TX_FAN_SPEED_END);
	if (len <= 0)
		goto parse_tx_fan_speed_fail;

	for (i = 0; i < row; i++)
		hwlog_info("[parse_tx_fan_speed] tx_fan_speed[%d]: %d %d %d\n",
			i, dts->tx_fan_speed[i].tx_type, dts->tx_fan_speed[i].level_max,
			dts->tx_fan_speed[i].level);
	dts->tx_fan_speed_len = row;
	return;

parse_tx_fan_speed_fail:
	kfree(dts->tx_fan_speed);
	dts->tx_fan_speed = NULL;
kcalloc_fail:
	hwlog_err("parse_tx_fan_speed: failed\n");
}
...
static void wlrx_acc_parse_sthu_feature_th(struct wlrx_acc_dts *dts)
{
	struct device_node *np = wlrx_ic_get_dev_node(WLTRX_IC_MAIN);

	if (!np)
		return;

	if (power_dts_read_u32_array(power_dts_tag(HWLOG_TAG), np,
		"sthu_feature_th", (u32 *)&dts->sthu_feature_th, WLRX_ACC_STHU_FEATURE_TH_ROW)) {
		dts->sthu_feature_th.fop_hth = WLRX_ACC_STHU_FEATURE_TH_DFT;
		dts->sthu_feature_th.fop_lth = WLRX_ACC_STHU_FEATURE_TH_DFT;
		dts->sthu_feature_th.vrect_hth = WLRX_ACC_STHU_FEATURE_TH_DFT;
		dts->sthu_feature_th.vrect_lth = WLRX_ACC_STHU_FEATURE_TH_DFT;
	}
	hwlog_info("[parse_rx_sthu_feature_th] fop_hth=%u fop_lth=%u\t"
		"vrect_hth=%u vrect_lth=%u\n", dts->sthu_feature_th.fop_hth,
		dts->sthu_feature_th.fop_lth, dts->sthu_feature_th.vrect_hth,
		dts->sthu_feature_th.vrect_lth);
}
...
static int wlrx_acc_parse_dts(const struct device_node *np, struct wlrx_acc_dev *di)
{
	...
	wlrx_acc_parse_tx_fan_speed(np, di->dts);
	wlrx_acc_parse_fix_tx_fop(np, di->dts);
	wlrx_acc_parse_sthu_feature_th(di->dts);
	wlrx_acc_parse_trx_plim(np, di->dts);
	return 0;
}
...
static void wlrx_acc_kfree_dev(struct wlrx_acc_dev *di)
{
	...
	kfree(di->cap);
	if (di->dts) {
		if (di->dts->tx_fan_speed)
			kfree(di->dts->tx_fan_speed);
		kfree(di->dts->qval_err_tx);
		kfree(di->dts->sc_err_tx);
		kfree(di->dts->tx_ilim);
		kfree(di->dts->trx_plim.cfg);
	}
	kfree(di->dts);
	kfree(di->dbg);
	kfree(di);
...
int wlrx_acc_init(unsigned int drv_type, struct device *dev)
{
	...
	INIT_DELAYED_WORK(&di->uevent_report_work, wlrx_acc_uevent_report_work);
