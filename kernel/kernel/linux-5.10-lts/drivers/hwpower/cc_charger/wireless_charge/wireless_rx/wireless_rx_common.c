...
struct wlrx_common_dev {
	...
	bool pwroff_reset_flag;
	int discon_delay;
	int device_direction;
	int last_device_direction;
};
...
{
	if (wlrx_ic_chip_init(wlrx_common_get_ic_type(drv_type), vtx,
		wlrx_acc_get_tx_type(drv_type)))
...
enum wlrx_scene wlrx_get_scene(void)
{
	...
	case WLCASE_ONLINE_BRACKETSHELL:
		return WLRX_SCN_BRACKETSHELL;
	default:
		break;
	}
...
void wlrx_common_reset_para(unsigned int drv_type, int rst_type)
{
	...
	case WLTRX_RST_IN_INIT:
		di->pwroff_reset_flag = false;
		di->device_direction = di->last_device_direction;
		break;
	case WLTRX_RST_IN_STOP:
		di->device_direction = WLRX_DEVICE_DIRECTION_DFLT;
		di->last_device_direction = WLRX_DEVICE_DIRECTION_DFLT;
		break;
	default:
		break;
	}
...
void wlrx_set_device_direction(unsigned int drv_type, int device_direction)
{
	...
	if (!di || (device_direction < WLRX_DEVICE_DIRECTION_BEGIN) ||
		(device_direction >= WLRX_DEVICE_DIRECTION_END))
		return;

	di->last_device_direction = device_direction;
	if (di->device_direction == WLRX_DEVICE_DIRECTION_DFLT)
		di->device_direction = device_direction;
}
