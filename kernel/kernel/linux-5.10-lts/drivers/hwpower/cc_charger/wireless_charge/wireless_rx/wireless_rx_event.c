...
bool wlrx_evt_is_pwr_good(unsigned int drv_type)
{
	struct wlrx_evt_dev *di = wlrx_evt_get_di(drv_type);
	...
	return di->evt_data.pwr_good == RX_PWR_ON_GOOD;
}

static void wlrx_evt_handle_rx_ocp(struct wlrx_evt_dev *di)
{
	char dsm_buff[POWER_DSM_BUF_SIZE_0512] = { 0 };

	hwlog_info("[handle_rx_ocp] drv_type:%u\n", di->drv_type);
...
static void wlrx_evt_handle_rx_ovp(struct wlrx_evt_dev *di)
{
	char dsm_buff[POWER_DSM_BUF_SIZE_0512] = { 0 };

	hwlog_info("[handle_rx_ovp] drv_type:%u\n", di->drv_type);
	if (wlrx_get_charge_stage() < WLRX_STAGE_REGULATION)
		return;
...
static void wlrx_evt_handle_rx_otp(struct wlrx_evt_dev *di)
{
	char dsm_buff[POWER_DSM_BUF_SIZE_0512] = { 0 };

	hwlog_info("[handle_rx_otp] drv_type:%u\n", di->drv_type);
	if (wlrx_get_charge_stage() < WLRX_STAGE_REGULATION)
		return;
...
static void wlrx_evt_handle_rx_opp(struct wlrx_evt_dev *di)
{
	char dsm_buff[POWER_DSM_BUF_SIZE_0512] = { 0 };

	hwlog_info("[handle_rx_opp] drv_type:%u\n", di->drv_type);
	if (wlrx_get_charge_stage() < WLRX_STAGE_REGULATION)
		return;
...
static void wlrx_evt_handle_rx_cr_temp_err(struct wlrx_evt_dev *di)
{
	char dsm_buff[POWER_DSM_BUF_SIZE_0512] = { 0 };

	hwlog_info("[handle_rx_cr_temp_err] drv_type:%u\n", di->drv_type);
	wlrx_dsm_report(di->drv_type, POWER_DSM_ERROR_WIRELESS_RX_CR_TEMP_ERR,
		dsm_buff, sizeof(dsm_buff));
}
...
static void wlrx_evt_rxic_work(struct work_struct *work)
{
	...
	case POWER_NE_WLRX_OCP:
		wlrx_evt_handle_rx_ocp(di);
		break;
	case POWER_NE_WLRX_OVP:
		wlrx_evt_handle_rx_ovp(di);
		break;
	case POWER_NE_WLRX_OTP:
		wlrx_evt_handle_rx_otp(di);
		break;
	case POWER_NE_WLRX_OPP_H:
		wlrx_evt_handle_rx_opp(di);
		break;
	case POWER_NE_WLRX_CR_TEMP_ERR:
		wlrx_evt_handle_rx_cr_temp_err(di);
		break;
	default:
		break;
	}
