...
void wlrx_plim_update_pctrl(unsigned int drv_type, struct wlrx_pctrl *pctrl)
{
	...
	for (i = WLRX_PLIM_SRC_BEGIN; i < WLRX_PLIM_SRC_END; i++) {
		if (!test_bit(i, &di->cur_src))
			continue;
		pctrl->vtx = power_min_positive(di->tbl[i].vtx, pctrl->vtx);
		pctrl->vrx = power_min_positive(di->tbl[i].vrx, pctrl->vrx);
		pctrl->irx = power_min_positive(di->tbl[i].irx, pctrl->irx);
	}
}
