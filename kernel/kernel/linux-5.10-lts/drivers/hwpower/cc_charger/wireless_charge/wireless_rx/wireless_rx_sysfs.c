...
static struct power_sysfs_attr_info wlrx_sysfs_field_tbl[] = {
	...
	power_sysfs_attr_rw(wlrx, 0644, WLRX_SYSFS_CALI, cali),
	power_sysfs_attr_wo(wlrx, 0220, WLRX_SYSFS_SEND_EPT, send_ept),
	power_sysfs_attr_wo(wlrx, 0220, WLRX_SYSFS_DEVICE_DIRECTION, device_direction),
};
...
static ssize_t wlrx_syfs_show_fod_cali(struct wlrx_sysfs_dev *di, char *buf)
{
	u8 cali = 0;
...
static ssize_t wlrx_syfs_store_cali(struct wlrx_sysfs_dev *di,
	const char *buf, size_t count)
{
	int ret;
	u8 cali = 0;
	...
	/* 1: cali */
	if (!buf || (sscanf_s(buf, "%hhu", &cali) != 1))
		return -EINVAL;

	hwlog_info("[store_cali] cali=%u\n", cali);
	ret = power_common_nv_write(POWER_NV_COMMON_CALI, (void *)&cali);
	if (ret)
		return ret;
...
static ssize_t wlrx_syfs_store_send_ept(struct wlrx_sysfs_dev *di,
	const char *buf, size_t count)
{
	unsigned int ic_type;

	hwlog_info("[store_send_ept]\n");
	ic_type = wlrx_sysfs_get_ic_type(di->drv_type);
	wlrx_ic_send_ept(ic_type, HWQI_EPT_CHRG_COMPLETE);

	return count;
}

static ssize_t wlrx_syfs_store_device_direction(struct wlrx_sysfs_dev *di,
	const char *buf, size_t count)
{
	int device_direction = 0;

	/* 1: device_direction */
	if (!buf || (sscanf_s(buf, "%d", &device_direction) != 1))
		return -EINVAL;

	hwlog_info("[store_device_direction] device_direction=%d\n", device_direction);
	wlrx_set_device_direction(di->drv_type, device_direction);
	...
	struct wlrx_sysfs_dev *di, char *buf)
{
	...
	case WLRX_SYSFS_CALI:
		return wlrx_syfs_show_fod_cali(di, buf);
	default:
		break;
	}
...
static struct wlrx_sysfs_store_handler {
	u8 type;
	ssize_t (*handler)(struct wlrx_sysfs_dev *, const char *, size_t);
} const g_rx_sysfs_store_handler[] = {
	{ WLRX_SYSFS_EN_ENABLE, wlrx_syfs_store_en_enable },
	{ WLRX_SYSFS_FOD_COEF, wlrx_syfs_store_fod_coef },
	{ WLRX_SYSFS_INTERFERENCE_SETTING, wlrx_syfs_store_interference_setting },
	{ WLRX_SYSFS_RX_SUPPORT_MODE, wlrx_syfs_store_rx_support_mode },
	{ WLRX_SYSFS_THERMAL_CTRL, wlrx_syfs_store_thermal_ctrl },
	{ WLRX_SYSFS_IGNORE_FAN_CTRL, wlrx_syfs_store_ingnore_fan_ctrl },
	{ WLRX_SYSFS_CM_CTRL, wlrx_syfs_store_cm_ctrl },
	{ WLRX_SYSFS_QVAL_CALI, wlrx_syfs_store_qval },
	{ WLRX_SYSFS_CALI, wlrx_syfs_store_cali },
	{ WLRX_SYSFS_SEND_EPT, wlrx_syfs_store_send_ept },
	{ WLRX_SYSFS_DEVICE_DIRECTION, wlrx_syfs_store_device_direction },
};
	...
	struct device_attribute *attr, const char *buf, size_t count)
{
	...
	for (i = 0; i < ARRAY_SIZE(g_rx_sysfs_store_handler); i++) {
		if (info->name == g_rx_sysfs_store_handler[i].type) {
			ret = g_rx_sysfs_store_handler[i].handler(di, buf, count);
			break;
		}
	}
