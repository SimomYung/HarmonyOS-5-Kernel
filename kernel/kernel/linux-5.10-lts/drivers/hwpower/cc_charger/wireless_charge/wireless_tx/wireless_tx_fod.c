...
static void wltx_prevfod_act(struct wltx_fod_dev_info *di, int *cnt)
{
	...
	for (i = di->prevfod_cfg.level - 1; i >= 0 ; i--) {
		...
		if (para->ilim < 0) {
			...
			if (wltx_alarm_can_swtich_pwr_src(&para->alarm) &&
				!wltx_pwr_prevfod_switch_pwr_handler())
				return;
...
static int wltx_avg_iin_compensation(int *tx_iin_avg)
{
	int ret;
	struct wltx_fod_dev_info *di = g_tx_fod_di;
	...
	if (di->tx_iin_avg_nv <= 0) {
		ret = power_common_nv_read(POWER_NV_COMMON_TX_AVG_IIN, (void *)&di->tx_iin_avg_nv);
		hwlog_info("[%s]:tx_iin_avg_nv:%u\n", __func__, di->tx_iin_avg_nv);
		if (ret)
			return ret;
	}

	if ((di->tx_iin_avg_nv <= 0) || (di->tx_iin_cpk <= 0)) {
		hwlog_err("%s: compensation params is ilegal, tx_iin_avg_nv:%u tx_iin_cpk:%u\n",
			__func__, di->tx_iin_avg_nv, di->tx_iin_cpk);
		return -EINVAL;
	}
	*tx_iin_avg = *tx_iin_avg * di->tx_iin_cpk / di->tx_iin_avg_nv;
...
static void wltx_ping_current_fod_check(struct wltx_current_fod_chk *curfod_chk, int tx_iin)
{
	...
	hwlog_info("[%s]:tx_iin_avg:%d\n", __func__, tx_iin_avg);

	if (!wltx_avg_iin_compensation(&tx_iin_avg))
		hwlog_info("[%s]: after compensation, tx_iin_avg:%d\n", __func__, tx_iin_avg);
...
static void wltx_fod_parse_dts(struct wltx_fod_dev_info *di)
{
	struct device_node *np = wltx_ic_get_dev_node(WLTRX_IC_MAIN);
	...
	(void)power_dts_read_u16(power_dts_tag(HWLOG_TAG), np,
		"tx_fod_iin_cpk", &di->tx_iin_cpk, 0);
