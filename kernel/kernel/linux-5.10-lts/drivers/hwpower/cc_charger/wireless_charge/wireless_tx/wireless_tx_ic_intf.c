...
int wltx_ic_set_ping_duty(unsigned int type, u16 ping_duty)
{
	...
	if (ops && ops->set_ping_duty)
		return ops->set_ping_duty(ping_duty, ops->dev_data);
...
int wltx_ic_switch_modem(unsigned int type)
{
	...
	if (ops && ops->switch_modem)
		return ops->switch_modem(ops->dev_data);

	return -EPERM;
}

/*
 * Function:wltx_ic_set_rx_type
 * Description:determine what device is being charged
 * Parameters:rx_type: type of device being charged
...
int wltx_ic_set_rx_type(unsigned int type, int rx_type)
{
	...
	if (ops && ops->set_rx_type)
		return ops->set_rx_type(ops->dev_data, rx_type);
...
int wltx_ic_get_fod_gap_state(unsigned int type, u16 *fod_gap_state)
{
	...
	if (ops && ops->get_fod_gap_state)
		return ops->get_fod_gap_state(ops->dev_data, fod_gap_state);
