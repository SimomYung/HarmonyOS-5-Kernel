...
bool power_array_contains_int(int num, int *array, int array_size)
{
	int i;
...
int power_count_char_in_str(const char *str, const char c)
{
	int i;
	int count = 0;
...
int power_str2intArray(const char *str, const char delimiter, int *target, int size)
{
	int i;
	int tmp_char_index = 0;
	int tmp_int_index = 0;
	int tmp_int = 0;
	char tmp_char[POWER_BITS_PER_BYTE];
	const char *ptr = NULL;
	...
	for (i = 0; str[i] != '\0'; i++) {
		if (str[i] == delimiter) {
			tmp_char[tmp_char_index] = '\0';
			ptr = tmp_char;
			if (kstrtoint(ptr, 0, &tmp_int) || tmp_int_index >= size)
				return -EINVAL;
			target[tmp_int_index++] = tmp_int;
			tmp_char_index = 0;
		} else if (str[i + 1] == '\0') {
			tmp_char[tmp_char_index++] = str[i];
			tmp_char[tmp_char_index] = '\0';
			ptr = tmp_char;
			if (kstrtoint(ptr, 0, &tmp_int) || tmp_int_index >= size)
				return -EINVAL;
