...
int power_i2c_write_block_with_crc(struct i2c_client *client, u8 *buf, u8 buf_len)
{
	...
	ret = memcpy_s(&buf_crc[LEN_WR_ADDR], buf_len + LEN_U8_PEC, buf, buf_len);
	if (ret) {
		hwlog_err("memcpy_s fail in power_i2c_write_block_with_crc\n");
		kfree(buf_crc);
		return ret;
	}
	...
	u8 *buf, u8 buf_len)
{
	...
	buf_crc[0] = (client->addr << 1);
	ret = memcpy_s(&buf_crc[LEN_WR_ADDR], buf_crc_len - LEN_WR_ADDR, cmd, cmd_len);
	if (ret) {
		kfree(buf_crc);
		hwlog_err("memcpy_s fail in power_i2c_read_block_with_crc\n");
		return ret;
	}
