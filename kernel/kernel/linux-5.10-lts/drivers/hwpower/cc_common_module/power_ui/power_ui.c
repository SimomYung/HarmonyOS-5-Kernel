...
static bool is_report_count_limited(int info, int *count, int data)
{
	if (info != data) {
		*count = 1;
		return false;
	...
	(*count)++;
	return *count > REPORT_COUNT_LIMIT;
}

static void handle_inner_max_power_event(struct power_ui_dev *l_dev, int value)
{
	if (is_report_count_limited(l_dev->info.inner_max_power,
		&l_dev->info.turbo_event_report_count, value))
		return;
	l_dev->info.inner_max_power = value;
...
void power_ui_event_notify(unsigned long event, const void *data)
{
	...
	case POWER_UI_NE_INNER_MAX_POWER:
		handle_inner_max_power_event(l_dev, *(int *)data);
		break;
	default:
		return;
	}
