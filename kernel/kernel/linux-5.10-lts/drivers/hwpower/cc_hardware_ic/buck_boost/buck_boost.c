...
enum {
	BUCK_COOST_CTRL_MODE_BEGIN = 0,
	BUCK_BOOST_AP_CTRL = BUCK_COOST_CTRL_MODE_BEGIN,
	BUCK_BOOST_CP_CTRL,
};
...
int buckboost_set_vout(unsigned int vol, unsigned int user, unsigned int index)
{
	...
	if (!context || (context->ctrl_mode == BUCK_BOOST_CP_CTRL) ||
		(user < BBST_USER_BEGIN) || (user >= BBST_USER_END))
		return -EPERM;
...
static void buckboost_monitor_work(struct work_struct *work)
{
	...
	for (i = 0; i < di->path_num; i++) {
		...
		if (!context || !context->ops || !context->ops->monitor_state) {
			hwlog_err("index=%d, context or ops or monitor_state is null\n", i);
			continue;
		}
		if (context->ctrl_mode == BUCK_BOOST_CP_CTRL) {
			hwlog_info("index=%d, cp ctrl, skip monitor\n", i);
			continue;
		}
	...
	void *data, int result, const char *client_str)
{
	struct buck_boost_context *context = data;
	struct buck_boost_ops *l_ops = NULL;
	struct buck_boost_vout_msg vout_msg = { 0 };
	...
	if (context->vout_change_notify) {
		vout_msg.index = context->ops->index;
		vout_msg.vol = result;
		power_event_bnc_notify(POWER_BNT_BYPASSBOOST, POWER_NE_BYPASSBOOST_VOUT_CHANGE, (void *)&vout_msg);
	}
	...
	struct buck_boost_dev *di)
{
	...
	for (i = 0; i < di->path_num; i++) {
		...
		(void)power_dts_read_u32_index_dflt(power_dts_tag(HWLOG_TAG), np,
			"cp_ctrl", i, &context->cp_ctrl, 0);
		(void)power_dts_read_u32_index_dflt(power_dts_tag(HWLOG_TAG), np,
			"vout_change_notify", i, &context->vout_change_notify, 0);
		context->ctrl_mode = BUCK_BOOST_AP_CTRL;
...
static void buck_boost_set_ctrl_mode(int index, int mode)
{
	struct buck_boost_context *context = buck_boost_get_context(index);
	int vout;
	...
	if (!context->cp_ctrl || context->ctrl_mode == mode)
		return;
	
	context->ctrl_mode = mode;
	if (mode == BUCK_BOOST_AP_CTRL) {
		if (!context->ops || !context->ops->set_vout)
			return;
		vout = power_vote_get_effective_result_locked(context->vout_vote_object, true);
		hwlog_info("reset vote vout=%d\n", vout);
		vout = vout < 0 ? context->const_vout : vout;
		context->ops->set_vout(vout, context->ops->dev_data);
	}
}

static int buck_boost_ctrl_mode_notifier_call(struct notifier_block *nb, unsigned long event, void *data)
{
	int index;
	...
	switch (event) {
 	case POWER_NE_BYPASSBOOST_CP_CTRL:
 		buck_boost_set_ctrl_mode(index, BUCK_BOOST_CP_CTRL);
 		break;
 	case POWER_NE_BYPASSBOOST_AP_CTRL:
 		buck_boost_set_ctrl_mode(index, BUCK_BOOST_AP_CTRL);
 		break;
 	default:
 		break;
 	}
...
static int buck_boost_probe(struct platform_device *pdev)
{
	...
	di->ctrl_mode_nb.notifier_call = buck_boost_ctrl_mode_notifier_call;
	power_event_bnc_register(POWER_BNT_BYPASSBOOST, &di->ctrl_mode_nb);
...
fail:
	power_event_bnc_unregister(POWER_BNT_BYPASSBOOST, &di->ctrl_mode_nb);
...
static int buck_boost_remove(struct platform_device *pdev)
{
	...
	power_event_bnc_unregister(POWER_BNT_BYPASSBOOST, &l_dev->ctrl_mode_nb);
	buck_boost_release_resources(pdev);
	return 0;
}
