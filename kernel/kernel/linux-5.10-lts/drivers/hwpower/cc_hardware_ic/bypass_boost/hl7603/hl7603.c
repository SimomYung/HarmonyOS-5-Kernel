...
#define HL7603_DBG_MAX_REG_ADDR                      0X05
...
static int hl7603_set_vout(struct hl7603_device_info *di, unsigned int vset)
{
	...
	if (hl7603_write_byte(di, HL7603_VOUT_SEL_ADDR, vset_reg)) {
		hwlog_info("hl7603 %d fail to set vout to %d\n", di->ic_role, vset);
		return -EIO;
	}

	hwlog_info("hl7603 %d set vout to %d\n", di->ic_role, vset);
	return 0;
}
...
static int hl7603_set_mode(struct hl7603_device_info *di, unsigned int mode)
{
	...
	if (mode >= HL7603_MODE_END) {
		hwlog_err("mode is not valid\n");
		return -EINVAL;
	}

	/* 
	 * to fix a hardware issue that device will leak current when set to forcebypass,
	 * before setting forcebypass mode, need to boost vout to 4750mv and delay 5ms
	 */
	if ((di->dev_id == HL7603_DEVID_HL) && (mode == HL7603_MODE_FORCE_BYPASS)) {
		hl7603_set_vout(di, 4750);
		power_msleep(5, 0, NULL);
	}

	if (hl7603_write_mask(di, HL7603_CONFIG_ADDR,
		HL7603_MODE_MASK, HL7603_MODE_SHIFT, mode)) {
		hwlog_err("hl7603 %d fail to set device mode=%d\n", di->ic_role, mode);
		return -EIO;
	}

	hwlog_info("hl7603 %d set device mode=%d\n", di->ic_role, mode);
	return 0;
}
...
static int hl7603_set_ilimt(struct hl7603_device_info *di, unsigned int ilimit)
{
	u8 vset_reg;

	if (ilimit < HL7603_ILIM_SEL_MIN)
		ilimit = HL7603_ILIM_SEL_MIN;

	if (ilimit > HL7603_ILIM_SEL_MAX)
		ilimit = HL7603_ILIM_SEL_MAX;

	vset_reg = (ilimit - HL7603_ILIM_SEL_BASE) / HL7603_ILIM_SEL_STEP;
	if (hl7603_write_mask(di, HL7603_ILIM_SEL_ADDR, HL7603_ILIM_SEL_MASK,
		HL7603_ILIM_SEL_SHIFT, vset_reg)) {
		hwlog_info("hl7603 %d fail to set ilimt to %u\n", di->ic_role, ilimit);
		return -EIO;
	}

	hwlog_info("hl7603 %d set ilimit to %u\n", di->ic_role, ilimit);
	return 0;
}
...
static void hl7603_init_reset(struct hl7603_device_info *di)
{
	if (!di->init_reset_en)
		return;

	hl7603_write_byte(di, HL7603_CONFIG_ADDR, HL7603_CONFIG_DFLT);

	switch (di->dev_id) {
	case HL7603_DEVID_HL:
	case HL7603_DEVID_HLR:
		hl7603_write_byte(di, HL7603_ILIM_SEL_ADDR, HL7603_ILIM_SEL_HL_DFLT);
		hl7603_write_byte(di, HL7603_CONFIG2_ADDR_HL, HL7603_CONFIG2_HL_DFLT);
		break;
	case HL7603_DEVID_JWT:
		hl7603_write_byte(di, HL7603_ILIM_SEL_ADDR, HL7603_ILIM_SEL_DFLT);
		break;
	case HL7603_DEVID_SBW:
		hl7603_write_mask(di, HL7603_VOUT_SEL_ADDR, HL7603_VOUT_PLIM_OFF_MASK,
			HL7603_VOUT_PLIM_OFF_SHIFT, 0);
		hl7603_write_mask(di, HL7603_VOUT_SEL_ADDR, HL7603_VOUT_LOOP_MODE_MASK,
			HL7603_VOUT_LOOP_MODE_SHIFT, 0);
		hl7603_write_byte(di, HL7603_ILIM_SEL_ADDR, HL7603_ILIM_SEL_DFLT);
		break;
	case HL7603_DEVID_SBW1:
		hl7603_write_mask(di, HL7603_VOUT_SEL_ADDR, HL7603_VOUT_PLIM_OFF_MASK,
		...
		hl7603_write_mask(di, HL7603_VOUT_SEL_ADDR, HL7603_VOUT_LOOP_MODE_MASK,
			HL7603_VOUT_LOOP_MODE_SHIFT, 0);
		if (di->bypass_ilim2_off)
			hl7603_write_byte(di, HL7603_ILIM_SEL_ADDR, HL7603_ILIM_SEL_SBW1);
		else
			hl7603_write_byte(di, HL7603_ILIM_SEL_ADDR, HL7603_ILIM_SEL_DFLT);
		break;
	default:
...
static void hl7603_init_ilimit(struct hl7603_device_info *di)
{
	if (di->ilimit == 0) {
		hwlog_info("ilimt is not configured\n");
		return;
	}

	hwlog_info("init ilimit is %d\n", di->ilimit);
	hl7603_set_ilimt(di, di->ilimit);
}
...
{
	return state & HL7603_STATE_POWER_GOOD_MASK;
}

static bool hl7603_is_fault(struct hl7603_device_info *di, u8 state)
{
	u8 fault_mask;
	...
	if (di->dev_id == HL7603_DEVID_JWT)
		fault_mask = HL7603_ABNORMAL_STATE_MASK_JWT;
	else
		fault_mask = HL7603_ABNORMAL_STATE_MASK;

	return state & fault_mask;
}
...
static void hl7603_monitor_dev_en(struct hl7603_device_info *di)
{
	u8 reg_val = 0;
	char buf[HL7603_DSM_BUFF_SIZE] = { 0 };
	...
	if (hl7603_read_byte(di, HL7603_CONFIG_ADDR, &reg_val)) {
		hwlog_err("fail to read reg 0x01\n");
		return;
	}

	if (!(reg_val & HL7603_DEV_EN_MASK)) {
		hwlog_info("%s dev en close, need enable\n", __func__);
		hl7603_write_byte(di, HL7603_CONFIG_ADDR, HL7603_CONFIG_DFLT);
		if (snprintf_s(buf, sizeof(buf), sizeof(buf) - 1,
			"deviceID=0x%x, index=%d, reg[0x01]=0x%x\n",
			di->dev_id, di->ic_role, reg_val) < 0)
			return;
		hwlog_info("dmd content: %s\n", buf);
		power_dsm_report_dmd(POWER_DSM_BATTERY, POWER_DSM_BYPASS_BOOST_I2C_TRANS_FAIL, buf);
	}
...
static void hl7603_monitor_state(void *dev_data)
{
	...
	hl7603_monitor_dev_en(di);
	hl7603_get_vout_adc(&vout_adc, dev_data);
	if (snprintf_s(buf, sizeof(buf), sizeof(buf) - 1,
		"deviceID=0x%x, index=%d, state=0x%x, vout=%d\n",
		di->dev_id, di->ic_role, state, vout_adc) < 0)
		hwlog_err("snprintf_s failed\n");
	hwlog_info("%s:%s", __func__, buf);
	if (hl7603_is_fault(di, state)) {
	...
	struct hl7603_device_info *di)
{
	...
	(void)power_dts_read_u32(power_dts_tag(HWLOG_TAG), np,
		"shutdown_mode", &di->shutdown_mode, HL7603_MODE_FORCE_BYPASS);
	(void)power_dts_read_u32(power_dts_tag(HWLOG_TAG), np,
		"shutdown_voltage", &di->shutdown_voltage, 0);
	(void)power_dts_read_u32(power_dts_tag(HWLOG_TAG), np,
	...
	(void)power_dts_read_u32(power_dts_tag(HWLOG_TAG), np,
		"ilimit", &di->ilimit, 0);
	(void)power_dts_read_u32(power_dts_tag(HWLOG_TAG), np,
		"init_reset_en", &di->init_reset_en, 0);
	(void)power_dts_read_u32(power_dts_tag(HWLOG_TAG), np,
		"bypass_ilim2_off", &di->bypass_ilim2_off, 0);
	(void)power_gpio_config_output(np,
		"gpio_enable", "gpio_enable", &di->gpio_no, 1);
}

static void hl7603_parse_shutdown_cfg_dts(struct device_node *np,
	struct hl7603_device_info *di)
{
	int row, col, len;
	int idata[HL7603_SHUTDOWN_CFG_GROUP * HL7603_SHUTDOWN_CFG_LEN] = { 0 };
 
	len = power_dts_read_string_array(power_dts_tag(HWLOG_TAG), np,
		"shutdown_cfg", idata, HL7603_SHUTDOWN_CFG_GROUP, HL7603_SHUTDOWN_CFG_LEN);
	if (len < 0)
		return;
 
	for (row = 0; row < len / HL7603_SHUTDOWN_CFG_LEN; row++) {
		col = row * HL7603_SHUTDOWN_CFG_LEN + HL7603_DEVID;
		if (idata[col] == di->dev_id) {
			di->shutdown_mode = idata[col + HL7603_SHUTDOWN_MODE];
			di->shutdown_voltage = idata[col + HL7603_SHUTDOWN_VOLTAGE];
			hwlog_info("set shutdown_mode=%d, shutdown_voltage=%d\n", di->shutdown_mode, di->shutdown_voltage);
			return;
		}
	}
}
...
static ssize_t hl7603_dbg_dump_show(void *dev_data, char *buf, size_t size)
{
	...
	for (addr = 0; addr <= HL7603_DBG_MAX_REG_ADDR; addr++) {
...
static void hl7603_register_power_debug(struct hl7603_device_info *di)
{
	...
	if (snprintf_s(name, HL7603_DBG_NAME_LEN, HL7603_DBG_NAME_LEN - 1,
		"hl7603_%d", di->ic_role) < 0)
		hwlog_err("snprintf_s failed\n");
...
static void hl7603_shutdown(struct i2c_client *client)
{
	...
	if (di->shutdown_voltage)
		hl7603_bbst_set_vout(di->shutdown_voltage, di);

	hl7603_set_mode(di, di->shutdown_mode);
}
	...
	const struct i2c_device_id *id)
{
	...
	if (ret != BYPASS_BST_IC_GOOD)
		goto get_devid_fail;
	hl7603_parse_shutdown_cfg_dts(np, di);

	/* 
	 * fix a hardware issue, state will report fault when shutdown and
	 * remains, so clear the state register by reading when probing
	 */
	hl7603_get_state(di, &state);
	if (di->ic_role >= 0) {
	...
	hl7603_init_reset(di);
	hl7603_init_mode(di);
	hl7603_init_vout(di);
	hl7603_init_ilimit(di);
