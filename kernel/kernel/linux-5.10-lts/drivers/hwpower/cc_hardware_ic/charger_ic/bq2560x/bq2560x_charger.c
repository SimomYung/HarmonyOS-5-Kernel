...
static int bq2560x_get_input_current_set(void)
{
	u8 reg = 0;
	int value;
 
	(void)bq2560x_read_mask(BQ2560X_REG_ISC, BQ2560X_REG_ISC_IINLIM_MASK,
		BQ2560X_REG_ISC_IINLIM_SHIFT, &reg);
	value = (int)reg * REG00_IINLIM_LSB + REG00_IINLIM_BASE;
	if (value > AC_IIN_CURRENT_THRESHOLD)
		value -= AC_IIN_CURRENT_OFFSET;
...
static int sc18601_set_cv(int value)
{
	...
	if (g_bq2560x_dev->is_support_vrechg_shift) {
		ret = bq2560x_get_charge_state(&state);
		if (ret < 0)
			hwlog_err("get_charge_state fail, ret=%d\n", ret);
		if (state & CHAGRE_STATE_CHRG_DONE)
			value = value - REG04_VRECHG_100MV_SHIFT;
	}
	
	value = (value < REG04_VREG_BASE_SC18601) ? REG04_VREG_BASE_SC18601 : value;
	value = (value > REG04_VREG_MAX_VOLT_SC18601) ? REG04_VREG_MAX_VOLT_SC18601 : value;
...
static int sc18601_get_cv(void)
{
	int ret, offset, val, mod;
	u8 reg = 0;
	u8 ft_val = 0;

	ret = bq2560x_read_mask(BQ2560X_REG_CVC, BQ2560X_REG_CVC_VREG_MASK,
		BQ2560X_REG_CVC_VREG_SHIFT, &reg);
	ret += bq2560x_read_mask(SC18601_REG_CTL4, SC18601_REG_CTL4_VBAT_REG_FT_MASK,
		SC18601_REG_CTL4_VBAT_REG_FT_SHIFT, &ft_val);
	if (ret < 0)
		return ret;

	mod = (int)ft_val * SC18601_REG_CTL4_VBAT_REG_FT_STEP;
	offset = (int)reg * REG04_VREG_LSB;
	val = REG04_VREG_BASE_SC18601 + mod + offset;
	hwlog_info("sc18601_get_cv [%x]=0x%x, ft_val=%d, val=%d\n",
		BQ2560X_REG_CVC, reg, ft_val, val);
	return val;
}
...
static int sy6974k_get_cv(void)
{
	u8 reg = 0;
	int val, ret;

	ret = bq2560x_read_mask(BQ2560X_REG_CVC, BQ2560X_REG_CVC_VREG_MASK_SY6974K,
		BQ2560X_REG_CVC_VREG_SHIFT_SY6974K, &reg);
	if (ret < 0)
		return ret;

	if (reg < REG04_VREG_BASE1_REG_SY6974K)
		val = (int)(reg) * REG04_VREG_LSB0_SY6974K + REG04_VREG_BASE0_SY6974K;
	else
		val = (int)(reg - REG04_VREG_BASE1_REG_SY6974K) * REG04_VREG_LSB1_SY6974K +
			REG04_VREG_BASE1_SY6974K;
	hwlog_info("sy6974k_get_cv [%x]=0x%x, val=%d\n", BQ2560X_REG_CVC, reg, val);
	return val;
}
...
static int bq2560x_get_terminal_voltage(void)
{
	u8 reg = 0;
	int value, ret;
	struct bq2560x_device_info *di = g_bq2560x_dev;
	...
	if (di->vendor_id == SC18601_VENDOR_ID)
		return sc18601_get_cv();

	if ((di->vendor_id == SY6974B_VENDOR_ID) && di->use_sy_new_cv)
		return sy6974k_get_cv();

	ret = bq2560x_read_mask(BQ2560X_REG_CVC, BQ2560X_REG_CVC_VREG_MASK,
		BQ2560X_REG_CVC_VREG_SHIFT, &reg);
	if (ret < 0)
		return ret;
	value = (int)reg * REG04_VREG_LSB + REG04_VREG_BASE;
	hwlog_info("reused_get_cv [%x]=0x%x, value=%d\n", BQ2560X_REG_CVC, reg, value);
	return value;
}
...
static int sgm41513d_convert_to_iterm_reg_val(int value)
{
	 ...
	 */
	if (value >= 240) {
		ret = bq2560x_write_mask(SC18601_REG_CTL4, AH0072_OTGF_ITREMR_MASK,
			AH0072_OTGF_ITREMR_SHIFT, AH0072_OTGF_ITREMR_DISABLE);
		if (ret) {
			hwlog_err("set 6*iterm flag fail\n");
			return -EIO;
		}
		value = value / 6;
    }
...
static int bq2560x_set_vrechg(int val)
{
	hwlog_info("%s val=%d\n", __func__, val);
	return bq2560x_write_mask(BQ2560X_REG_CVC, BQ2560X_REG_CVC_VRECHG_MASK,
		BQ2560X_REG_CVC_VRECHG_SHIFT, val);
}
...
static int bq2560x_set_term_enable(int enable)
{
	...
	if (di->skip_term_enable && di->skip_term_status)
		return 0;

	di->skip_term_status = true;
...
static int bq2560x_dump_register(char *reg_value, int size, void *dev_data)
{
	...
	for (i = 0; i < reg_num; i++) {
		...
		if (strncat_s(reg_value, size, buff, strlen(buff)))
			return 0;
	}
...
static int bq2560x_get_register_head(char *reg_head, int size, void *dev_data)
{
	...
	for (i = 0; i < reg_num; i++) {
		...
		if (strncat_s(reg_head, size, buff, strlen(buff)))
			return 0;
	}
...
static int bq2560x_set_vbus_vset(u32 volt)
{
	return 0;
}
...
static struct charge_device_ops bq2560x_ops = {
	...
	.set_vbus_vset = bq2560x_set_vbus_vset,
	...
	.get_terminal_voltage = bq2560x_get_terminal_voltage,
	.get_iin_set = bq2560x_get_input_current_set,
...
static int bq2560x_charge_enable(int enable, void *dev_data)
{
	...
	if (di->vendor_id == SY6974B_VENDOR_ID) {
...
static void bq2560x_init_ops_dev_data(struct bq2560x_device_info *di)
{
	...
	if (!di->ic_role) {
		...
		snprintf(di->name, CHIP_DEV_NAME_LEN, "bq2560x_%u", di->ic_role);
...
static void bq2560x_parse_dts(struct device_node *np, struct bq2560x_device_info *di)
{
	...
	(void)power_dts_read_u32(power_dts_tag(HWLOG_TAG), np,
		"skip_term_enable", &di->skip_term_enable, 0);
	(void)power_dts_read_u32(power_dts_tag(HWLOG_TAG), np,
		"vrechg", &di->vrechg, 0);
	(void)power_dts_read_u32(power_dts_tag(HWLOG_TAG), np,
		"is_support_vrechg_shift", &di->is_support_vrechg_shift, 0);
}
...
static void bq2560x_reg_init(struct bq2560x_device_info *di)
{
	...
	if (di->vrechg) {
		ret = bq2560x_set_vrechg(REG04_VRECHG_200MV);
		if (ret)
			hwlog_err("set vrechg fail\n");
	}
}
	...
	const struct i2c_device_id *id)
{
	...
	di->skip_term_status = false;
	if (di->skip_term_enable)
		bq2560x_set_term_enable(1);
