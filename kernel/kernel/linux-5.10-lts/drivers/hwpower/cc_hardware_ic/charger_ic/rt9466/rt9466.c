...
static int rt9466_parse_dt_request_gpio(struct rt9466_info *info)
{
	char *ceb_name = NULL;
	int len, ret;
	...
	if (ret < 0) {
		...
		devm_kfree(info->dev, ceb_name);
	}
	return ret;
}

static void rt9466_parse_dt_basic_parse(struct rt9466_desc *desc, struct device_node *np)
{
	if (!desc || !np)
		return;
...
static void rt9466_parse_dt_extra_parse(struct device_node *np, struct rt9466_info *info,
		struct rt9466_desc *desc)
{
	int ret;
...
static int rt9466_parse_dt(struct rt9466_info *info, struct device *dev)
{
	...
	ret = rt9466_parse_dt_request_gpio(info);
	if (ret < 0) {
		if (desc != NULL)
			devm_kfree(dev, desc);
		return ret;
	}
	rt9466_parse_dt_basic_parse(desc, np);
	rt9466_parse_dt_extra_parse(np, info, desc);
