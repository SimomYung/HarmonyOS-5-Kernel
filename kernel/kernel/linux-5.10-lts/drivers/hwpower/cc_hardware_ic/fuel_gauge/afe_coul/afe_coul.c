...
static void afe_coul_save_fcc_flag_reg(int batt, int invalid_fcc_up_flag)
{
	int ret;
	unsigned short regval = 0;
	...
	hwlog_info("%s coul%d ret=%d, regval=0x%x\n", __func__, batt, ret, regval);
	return;
}

static void afe_coul_save_ocv(int batt, int ocv)
{
	int ret;
...
static void afe_coul_enter_eco(int batt)
{
}

static void afe_coul_exit_eco(int batt)
{
}
...
static struct coul_device_ops afe_coul_ops = {
	...
	.save_fcc_invalid_up_flag = afe_coul_save_fcc_flag_reg,
	.set_low_low_int_val = afe_coul_set_low_volt_int,
	.save_ocv = afe_coul_save_ocv,
	.get_ocvreg = afe_coul_get_ocv,
