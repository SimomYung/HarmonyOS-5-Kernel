...
#define CW2217_FAULT_CURR_FOR_INIT       5000
#define CW2217_VOLTAGE_4000_MV           4000
...
static int cw2217_parse_dt(struct device *dev, struct cw2217_dev *di)
{
	...
	(void)power_dts_read_u32(power_dts_tag(HWLOG_TAG), np, "fault_state_check",
		(u32 *)&di->fault_state_check, 0);
...
static void cw2217_basp_soc_mapping_strategy(struct cw2217_dev *di, int vterm)
{
	...
	for (row = 0; row < di->soc_mapping_len; row++) {
		if (vterm <= (int)(di->soc_mapping[row][CW2217_SOC_MAPPING_VTERM])) {
			di->ocv_idx = row;
			hwlog_info("set ocv_idx = %d\n", di->ocv_idx);
			return;
		}
	}
	if (row >= (int)(di->soc_mapping_len))
...
static void cw2217_soft_reset(struct cw2217_dev *di)
{
	...
	if (di->index >= CW2217_RECORD_NUM) {
		...
		if ((soc_u > 0) && (abs(soc_u - di->ui_soc) > CW2217_MAX_SOC_DIFF))
			need_reset = 1;
		di->index = 0;
	}
	if (need_reset ||
		...
		cw2217_reg_reset(di);
		if (di->fault_state_check) {
#ifdef CONFIG_COUL_DRV
#ifdef CONFIG_COUL_ALGORITHM_CORE
			other_coul_cycle = coul_merge_drv_battery_cycle_count(CW2217_IC_TYPE_MAIN);
#else
			other_coul_cycle = 0;
#endif /* CONFIG_COUL_ALGORITHM_CORE */
#else
			other_coul_cycle = coul_interface_get_battery_cycle(CW2217_IC_TYPE_MAIN);
#endif /* CONFIG_COUL_DRV */
			hwlog_info("%s other_coul_cycle = %d\n", __func__, other_coul_cycle);
			history_cycle = other_coul_cycle / CW2217_CYCLE_BASE_CONVERT;
		}
		...
		snprintf(buf, POWER_DSM_BUF_SIZE_0128 - 1,
			"cw2217 soft reset: workaround %hhu\n", need_reset + 1);
...
static int cw2217_is_battery_replaced(void)
{
	return 0; /* When the gauge is normal, it cannot be replaced */
}

static struct coul_merge_drv_ops cw2217_ops = {
	.is_replaced =  cw2217_is_battery_replaced,
...
static void cw2217_init_func(struct cw2217_dev *di)
{
	...
	if (di->fault_state_check && (abs(di->curr) > CW2217_FAULT_CURR_FOR_INIT ||
		(di->voltage < CW2217_VOLTAGE_4000_MV && di->ui_soc == CW2217_UI_FULL))) {
		hwlog_err("fault state check fail\n");
		return;
	}
