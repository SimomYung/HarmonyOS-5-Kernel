...
#define HL7136_SYS_CLK_RETRY_TIMES          3
...
static void hl7136_lvc_opt_regs(struct hl7136_device_info *di)
{
	...
	hl7136_config_vbus_ovp_ref_mv(di->lvc_vbus_ovp, di);
...
static int hl7136_lvc_charge_enable(int enable, void *dev_data)
{
	...
	if (!enable) {
		...
		(void)hl7136_write_mask(di, HL7136_CTRL3_REG, HL7136_CTRL3_DEV_MODE_MASK,
...
static struct dc_batinfo_ops hl7136_batinfo_ops = {
	...
	.get_ic_vusb = hl7136_get_vbus_mv,
};
...
static void hl7136_init_ops_dev_data(struct hl7136_device_info *di)
{
	...
	if (!di->ic_role) {
		...
		if (snprintf_s(di->name, HL7136_DEV_NAME_LEN, HL7136_DEV_NAME_LEN - 1, "hl7136_%u", di->ic_role) < 0)
			hwlog_err("dev name set fail\n");
...
static void hl7136_parse_dts(struct device_node *np, struct hl7136_device_info *di)
{
	...
	(void)power_dts_read_u32(power_dts_tag(HWLOG_TAG), np, "sys_clk_off",
		(u32 *)&(di->sys_clk_off), 0);
	(void)power_dts_read_u32(power_dts_tag(HWLOG_TAG), np,
		"manual_otg_mode", &di->manual_otg_mode, 0);
	(void)power_dts_read_u32(power_dts_tag(HWLOG_TAG), np,
		"lvc_vbus_ovp", &di->lvc_vbus_ovp, HL7136_VBUS_OVP_INIT);
}
...
static void hl7136_switch_addr_to_dflt(struct hl7136_device_info *di)
{
	int ret;
	u8 val = 0;

	ret = hl7136_read_byte(di, HL7136_SWITCH_ADDR_REG, &val);
	if (ret || (val != HL7136_SWITCH_ADDR_TO_DFLT))
		(void)hl7136_write_byte(di, HL7136_SWITCH_ADDR_REG, HL7136_SWITCH_ADDR_TO_DFLT);
}

/*
 * hl7136_sys_clk_enable - disable sys_clk when shutdown for less leakage, and need enable sys_clk when probe
 * @di: device info
 * @enable: enable or disable sys_clk
 *
 * enable or disable sys_clk need switch i2c addr to 0x55
 */
static void hl7136_sys_clk_enable(struct hl7136_device_info *di, bool enable)
{
	int i, ret;
	u8 val = 0;
	u_short addr_dflt = di->client->addr;
	u8 sys_clk_val = enable ? HL7136_SYS_CLK_EN : HL7136_SYS_CLK_DIS;

	hwlog_info("hl7136_sys_clk_enable enable=%d \n", enable);
	hl7136_switch_addr_to_dflt(di);
	(void)hl7136_read_byte(di, HL7136_DEVICE_ID_REG, &val);
	if (val != HL7136_DEVICE_ID_DFLT)
		goto func_end;

	for (i = 0; i < HL7136_SYS_CLK_RETRY_TIMES; i++) {
		di->client->addr = addr_dflt;
		(void)hl7136_write_byte(di, HL7136_SWITCH_ADDR_REG, HL7136_SWITCH_ADDR_TO_55_CTL1);
		(void)hl7136_write_byte(di, HL7136_SWITCH_ADDR_REG, HL7136_SWITCH_ADDR_TO_55_CTL2);
		di->client->addr = HL7136_SWITCH_ADDR;
		(void)hl7136_write_byte(di, HL7136_SYS_CLK_REG, sys_clk_val);
		power_usleep(DT_USLEEP_1MS);
		val = 0;
		ret = hl7136_read_byte(di, HL7136_SYS_CLK_REG, &val);
		hwlog_info("reg [%x]=0x%x, ret=%d\n", HL7136_SYS_CLK_REG, val, ret);
		if (!ret && (val == sys_clk_val))
			break;
	}
...
func_end:
	...
	for (i = 0; i < HL7136_SYS_CLK_RETRY_TIMES; i++) {
		...
		val = 0;
		ret = hl7136_read_byte(di, HL7136_SWITCH_ADDR_REG, &val);
		hwlog_info("reg [%x]=0x%x, ret=%d\n", HL7136_SWITCH_ADDR_REG, val, ret);
		if (!ret && (val == HL7136_SWITCH_ADDR_TO_DFLT))
			break;
	}

	if (enable) {
		(void)hl7136_write_byte(di, HL7136_ADC_CTRL0_REG, HL7136_ADC_CTRL0_ADC_FORCE_ENABLE);
		(void)hl7136_write_byte(di, HL7136_CTRL2_REG, HL7136_CTRL2_SFT_RST);
		power_usleep(DT_USLEEP_10MS);
	}
}
...
static int hl7136_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	hl7136_parse_dts(di->dev->of_node, di);

	if (di->sys_clk_off)
		hl7136_sys_clk_enable(di, true);

	ret = hl7136_get_device_id(di);
	if ((ret < 0) || (ret == DC_DEVICE_ID_END))
		goto hl7136_fail_0;

	hl7136_parse_mode_para_dts(di->dev->of_node, &di->hl7136_sc_para, "sc_para", di->device_id);
	...
	ret = hl7136_irq_init(di, di->dev->of_node);
	if (ret)
		goto hl7136_fail_2;
...
static void hl7136_shutdown(struct i2c_client *client)
{
	...
	if (di->sys_clk_off)
		hl7136_sys_clk_enable(di, false);
