...
static void hl7139_dump_register(void *dev_data)
{
	...
	ret = hl7139_read_word(di, HL7139_ADC_VIN_0_REG, &data);
	if (ret)
		hwlog_err("dump_register read fail\n");
	else
		hwlog_info("ADC_VIN=0x%x\n", data);
	ret = hl7139_read_word(di, HL7139_ADC_IBAT_0_REG, &data);
	if (ret)
		hwlog_err("dump_register read fail\n");
	else
		hwlog_info("ADC_IBAT=0x%x\n", data);
	ret = hl7139_read_word(di, HL7139_ADC_IIN_0_REG, &data);
	if (ret)
		hwlog_err("dump_register read fail\n");
	else
		hwlog_info("ADC_IIN=0x%x\n", data);
	ret = hl7139_read_word(di, HL7139_ADC_VBAT_0_REG, &data);
	if (ret)
		hwlog_err("dump_register read fail\n");
	else
...
static int hl7139_get_bat_curr_ma(void *dev_data)
{
	int batt_curr = 0;
	int ret;
	struct hl7139_device_info *di = dev_data;
	...
	ret = hl7139_get_ibat_ma(&batt_curr, di);
	if (ret) {
		hwlog_err("get batt_curr error\n");
		return 0;
	}
...
static int hl7139_config_vin_ovp_mv(int vin_value, int chg_mode, void *dev_data)
{
	u8 value;
	int ret;
	struct hl7139_device_info *di = dev_data;
	...
	if (chg_mode == LVC_MODE) {
		if (vin_value < HL7139_VIN_OVP_BP_MIN)
			vin_value = HL7139_VIN_OVP_BP_MIN;

		if (vin_value > HL7139_VIN_OVP_BP_MAX)
			vin_value = HL7139_VIN_OVP_BP_MAX;

		value = (u8)((vin_value - HL7139_VIN_OVP_BP_MIN) /
			HL7139_VIN_OVP_BP_STEP);
	} else if (chg_mode == SC_MODE) {
		if (vin_value < HL7139_VIN_OVP_CP_MIN)
			vin_value = HL7139_VIN_OVP_CP_MIN;

		if (vin_value > HL7139_VIN_OVP_CP_MAX)
			vin_value = HL7139_VIN_OVP_CP_MAX;
		value = (u8)((vin_value - HL7139_VIN_OVP_CP_MIN) /
			HL7139_VIN_OVP_CP_STEP);
	} else {
		hwlog_err("chg mode error:chg_mode=%d\n", chg_mode);
		return -EPERM;
	}

	ret = hl7139_write_mask(di, HL7139_VIN_OVP_REG, HL7139_VIN_OVP_VIN_OVP_MASK,
		HL7139_VIN_OVP_VIN_OVP_SHIFT, value);
	if (ret)
		return -EPERM;

	hwlog_info("hl7139_config_vin_ovp_mv [%x]=0x%x\n",
		HL7139_VIN_OVP_REG, value);
	return 0;
}
...
static int hl7139_config_switching_frequency(int data, void *dev_data)
{
	...
	if (data < HL7139_SW_FREQ_MIN)
		data = HL7139_SW_FREQ_MIN;

	if (data > HL7139_SW_FREQ_MAX)
		data = HL7139_SW_FREQ_MAX;

	freq = (u8)((data - HL7139_SW_FREQ_MIN) / HL7139_SW_FREQ_STEP);

	if ((di->rev_id == 0) && (data >= HL7139_SW_FREQ_1300KHZ)) {
...
static void hl7139_lvc_opt_regs(void *dev_data)
{
	...
	hl7139_config_vbuscon_ovp_ref_mv(di->vgs_voltage, di);
	...
	hl7139_config_vin_ovp_mv(di->sc_bp_vin_ovp, LVC_MODE, di);
}
...
static void hl7139_sc_opt_regs(void *dev_data)
{
	...
	hl7139_config_vin_ovp_mv(di->sc_cp_vin_ovp, SC_MODE, di);
}
...
static int hl7139_reg_init(void *dev_data)
{
	...
	if (di->rev_id == 0)
		...
		ret += hl7139_config_watchdog_ms(di->watchdog_time, di);
	...
	ret += hl7139_config_vbuscon_ovp_ref_mv(di->vgs_voltage, di);
	...
	if (di->close_vbat_ovp)
		ret += hl7139_write_mask(di, HL7139_VBAT_REG_REG, HL7139_VBAT_REG_VBAT_OVP_DIS_MASK,
			HL7139_VBAT_REG_VBAT_OVP_DIS_SHIFT, 1);
...
static void hl7139_init_ops_dev_data(struct hl7139_device_info *di)
{
	...
	if (!di->ic_role) {
		...
		snprintf(di->name, HL7139_DEV_NAME_LEN, "hl7139_%u", di->ic_role);
...
static void hl7139_ops_register(struct hl7139_device_info *di)
{
	...
	if (di->adc_sample_type & BIT(BATT_CURRENT))
		batt_adc_monitor_ops_register(di->ic_role, BATT_CURRENT, hl7139_get_bat_curr_ma, di);
}
...
static void hl7139_parse_dts(struct device_node *np, struct hl7139_device_info *di)
{
	...
	(void)power_dts_read_u32(power_dts_tag(HWLOG_TAG), np,
		"watchdog_time", &di->watchdog_time, HL7139_WD_TMR_1000MS);
	(void)power_dts_read_u32(power_dts_tag(HWLOG_TAG), np,
		"vgs_voltage", &di->vgs_voltage, HL7139_VGS_SEL_MIN);
	...
	(void)power_dts_read_u32(power_dts_tag(HWLOG_TAG), np,
		"adc_sample_type", &di->adc_sample_type, 0);
	(void)power_dts_read_u32(power_dts_tag(HWLOG_TAG), np,
		"close_vbat_ovp", &di->close_vbat_ovp, 0);
	(void)power_dts_read_u32(power_dts_tag(HWLOG_TAG), np,
		"sc_cp_vin_ovp", &di->sc_cp_vin_ovp, HL7139_VIN_OVP_CP_INIT);
	(void)power_dts_read_u32(power_dts_tag(HWLOG_TAG), np,
		"sc_bp_vin_ovp", &di->sc_bp_vin_ovp, HL7139_VIN_OVP_BP_INIT);
}
