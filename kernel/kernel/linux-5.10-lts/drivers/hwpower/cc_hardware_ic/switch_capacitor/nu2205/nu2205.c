...
static void nu2205_init_ops_dev_data(struct nu2205_device_info *di)
{
	if (memcpy_s(&di->lvc_ops, sizeof(struct dc_ic_ops),
		&g_nu2205_lvc_ops, sizeof(struct dc_ic_ops)) == EOK)
		di->lvc_ops.dev_data = (void *)di;
	if (memcpy_s(&di->sc_ops, sizeof(struct dc_ic_ops),
		&g_nu2205_sc_ops, sizeof(struct dc_ic_ops)) == EOK)
		di->sc_ops.dev_data = (void *)di;
	if (memcpy_s(&di->batinfo_ops, sizeof(struct dc_batinfo_ops),
		&g_nu2205_batinfo_ops, sizeof(struct dc_batinfo_ops)) == EOK)
		di->batinfo_ops.dev_data = (void *)di;
	if (memcpy_s(&di->log_ops, sizeof(struct power_log_ops),
		&g_nu2205_log_ops, sizeof(struct power_log_ops)) == EOK)
		di->log_ops.dev_data = (void *)di;

	if (!di->ic_role) {
		if (snprintf_s(di->name, CHIP_DEV_NAME_LEN, CHIP_DEV_NAME_LEN - 1, "nu2205") < 0)
			hwlog_err("dev name set fail\n");
	} else {
		if (snprintf_s(di->name, CHIP_DEV_NAME_LEN, CHIP_DEV_NAME_LEN - 1, "nu2205_%u", di->ic_role) < 0)
			hwlog_err("dev name set fail\n");
