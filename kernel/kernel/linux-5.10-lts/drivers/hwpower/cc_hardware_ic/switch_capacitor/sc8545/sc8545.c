...
static int sc8545_get_bat_curr_ma(void *dev_data)
{
	int batt_curr = 0;
	int ret;
	struct sc8545_device_info *di = dev_data;
	...
	ret = sc8545_get_ibat_ma(&batt_curr, di);
	if (ret) {
...
static void sc8545_init_ops_dev_data(struct sc8545_device_info *di)
{
	...
	if (!di->ic_role) {
		...
		snprintf(di->name, CHIP_DEV_NAME_LEN, "sc8545_%u", di->ic_role);
...
static void sc8545_ops_register(struct sc8545_device_info *di)
{
	...
	if (di->adc_sample_type & BIT(BATT_CURRENT))
		batt_adc_monitor_ops_register(di->ic_role, BATT_CURRENT, sc8545_get_bat_curr_ma, di);
}
