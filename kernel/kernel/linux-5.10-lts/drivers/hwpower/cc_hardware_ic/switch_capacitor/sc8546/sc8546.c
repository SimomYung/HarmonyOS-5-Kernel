...
static int sc8546_threshold_reg_init(struct sc8546_device_info *di, int mode)
{
	...
	if (mode == SC8546_CHG_MODE_BYPASS)
		vac_ovp = di->lvc_vac_ovp;
	else
		vac_ovp = SC8546_VAC_OVP_TH_INIT;

	ret = sc8546_config_vac_ovp_th_mv(di, vac_ovp);
...
static int sc8546_reg_init(struct sc8546_device_info *di)
{
	...
	ret = sc8546_config_watchdog_ms(SC8546_WTD_CONFIG_TIMING_DIS, di);
	ret += sc8546_threshold_reg_init(di, SC8546_CHG_MODE_BUCK);
...
static void sc8546_init_ops_dev_data(struct sc8546_device_info *di)
{
	...
	if (!di->ic_role) {
		...
		snprintf(di->name, CHIP_DEV_NAME_LEN, "sc8546_%u", di->ic_role);
	...
	struct sc8546_device_info *di)
{
	...
	(void)power_dts_read_u32(power_dts_tag(HWLOG_TAG), np,
		"lvc_vac_ovp", &di->lvc_vac_ovp, SC8546_VAC_OVP_TH_INIT);
}
