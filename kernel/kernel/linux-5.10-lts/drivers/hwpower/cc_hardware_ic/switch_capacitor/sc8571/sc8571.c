...
static int sc8571_disable_nu2205_adc(void *dev_data)
{
	int ret;
	u8 reg = 0;
	u8 value = NU2205_ADC_DISABLE;
	struct sc8571_device_info *di = dev_data;

	if (!di)
		return -ENODEV;

	ret = sc8571_write_mask(di, NU2205_ADC_CTRL_REG, NU2205_ADC_CTRL_EN_MASK,
		NU2205_ADC_CTRL_EN_SHIFT, value);
	if (ret)
		return -EPERM;

	ret = sc8571_read_byte(di, NU2205_ADC_CTRL_REG, &reg);
	if (ret)
		return -EPERM;
...
static int sc8571_get_device_id(void *dev_data)
{
	...
	if (sc8571_disable_nu2205_adc(di))
		hwlog_err("disable adc fail\n");
...
static int sc8571_fault_clear(struct sc8571_device_info *di)
{
	int ret;
	u8 reg;
	u8 addr;

	for (addr = SC8571_FLAG1_REG; addr <= SC8571_FLAG5_REG; addr++) {
		reg = 0;
		ret = sc8571_read_byte(di, addr, &reg);
		if (ret)
			return -EPERM;
		if (reg)
...
static int sc8571_register_head(char *buffer, int size, void *dev_data)
{
	...
	if (snprintf_s(buffer, size, size - 1,
		"dev       mode   Ibus   Vbus   Vac1   Vac2   Ibat   Vbat   Temp   ") < 0)
		return -ENOMEM;
...
static int sc8571_dump_reg(char *buffer, int size, void *dev_data)
{
	...
	len = snprintf_s(buffer, size, size - 1, "%-10s", di->name);
	if (len < 0)
		return -ENOMEM;
	...
	len += strlen(buff);
	if ((len < size) && (strncat_s(buffer, size - 1, buff, strlen(buff)) != EOK))
		return -ENOMEM;
	ret = snprintf_s(buff, sizeof(buff), sizeof(buff) - 1, "%-7d%-7d%-7d%-7d%-7d%-7d%-7d",
...
{
	if (memcpy_s(&di->lvc_ops, sizeof(struct dc_ic_ops),
		&g_sc8571_lvc_ops, sizeof(struct dc_ic_ops)) == EOK)
		di->lvc_ops.dev_data = (void *)di;
	if (memcpy_s(&di->sc_ops, sizeof(struct dc_ic_ops),
		&g_sc8571_sc_ops, sizeof(struct dc_ic_ops)) == EOK)
		di->sc_ops.dev_data = (void *)di;
	if (memcpy_s(&di->batinfo_ops, sizeof(struct dc_batinfo_ops),
		&g_sc8571_batinfo_ops, sizeof(struct dc_batinfo_ops)) == EOK)
		di->batinfo_ops.dev_data = (void *)di;
	if (memcpy_s(&di->log_ops, sizeof(struct power_log_ops),
		&g_sc8571_log_ops, sizeof(struct power_log_ops)) == EOK)
		di->log_ops.dev_data = (void *)di;

	if (!di->ic_role) {
		if (snprintf_s(di->name, CHIP_DEV_NAME_LEN, CHIP_DEV_NAME_LEN - 1, "sc8571") < 0)
			hwlog_err("dev name set fail\n");
	} else {
		if (snprintf_s(di->name, CHIP_DEV_NAME_LEN, CHIP_DEV_NAME_LEN - 1, "sc8571_%u", di->ic_role) < 0)
			hwlog_err("dev name set fail\n");
	...
	struct sc8571_mode_para *data, const char *name, int id)
{
	...
	if (index >= 0) {
		for (i = SC8571_INFO_MAX_IBAT; i < SC8571_INFO_TOTAL; i++) {
			...
			(void)kstrtoint(tmp_string, POWER_BASE_DEC, &idata[i]);
		}
