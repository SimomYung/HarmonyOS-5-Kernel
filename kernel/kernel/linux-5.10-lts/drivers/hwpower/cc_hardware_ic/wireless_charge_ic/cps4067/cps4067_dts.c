	...
	struct cps4067_dev_info *di)
{
	...
	if (len > CPS4067_DUMMY_ILOAD_END) {
		hwlog_err("parse_dummy_iload: len out range len =%d\n", len);
		return;
	}

	if ((len <= 0) || power_dts_read_u32_array(power_dts_tag(HWLOG_TAG), np,
		"dummy_iload_new", (u32 *)&di->dummy_iload_new, len)) {
...
static int cps4067_parse_tx_basic_para(struct device_node *np, struct cps4067_dev_info *di)
{
	(void)power_dts_read_u32(power_dts_tag(HWLOG_TAG), np,
	...
	(void)power_dts_read_u32(power_dts_tag(HWLOG_TAG), np,
		"tx_low_power_ocp_th", &di->tx_low_power_ocp_th,
		CPS4067_TX_LOW_POWER_ILIM_VALUE);
	(void)power_dts_read_u32(power_dts_tag(HWLOG_TAG), np,
		"tx_opt_gain", &di->tx_opt_gain, 0);
	(void)power_dts_read_u32(power_dts_tag(HWLOG_TAG), np,
	...
	(void)power_dts_read_u32(power_dts_tag(HWLOG_TAG), np,
		"coil_test_ping_duty", &di->coil_test_ping_duty, CPS4067_TX_PING_DUTY);
	(void)power_dts_read_u32(power_dts_tag(HWLOG_TAG), np,
	...
	(void)power_dts_read_u8(power_dts_tag(HWLOG_TAG), np,
		"force_full_bridge", &di->force_full_bridge, CPS4067_TX_FORCE_FULL_BRIDGE);
	return 0;
}

static int cps4067_parse_rx_basic_para(struct device_node *np, struct cps4067_dev_info *di)
{
	(void)power_dts_read_u32(power_dts_tag(HWLOG_TAG), np,
	...
	(void)power_dts_read_u32(power_dts_tag(HWLOG_TAG), np,
		"rx_prevfod_offset", &di->rx_prevfod_offset,
		CPS4067_RX_PREVFOD_OFFSET);
	(void)power_dts_read_u32(power_dts_tag(HWLOG_TAG), np,
		"rx_prevfod_offset_unit", &di->rx_prevfod_offset_unit,
		CPS4067_RX_PREVFOD_OFFSET);
	(void)power_dts_read_u32(power_dts_tag(HWLOG_TAG), np,
		"rx_ss_low_thr", &di->rx_ss_low_thr, 0);
	(void)power_dts_read_u32(power_dts_tag(HWLOG_TAG), np,
		"rx_ldo_opp_cnt", &di->rx_ldo_opp_cnt, 0);
	return 0;
}
...
static int cps4067_parse_basic_para(struct device_node *np, struct cps4067_dev_info *di)
{
	...
	ret += cps4067_parse_rx_basic_para(np, di);
	ret += cps4067_parse_tx_basic_para(np, di);
	return ret;
}
