...
static int cps8601_fw_check_mtp_version(struct cps8601_dev_info *di)
{
	...
	if (info.mtp_ver != di->firm_info.mtp_ver) {
		hwlog_err("[check_mtp_version] ver=0x%04x\n", di->firm_info.mtp_ver);
		return -EINVAL;
	}
	...
	const u8 *data, int len)
{
	...
	u32 addr = cur_addr;
	int remaining = len;
	u8 wr_buff[CPS8601_SRAM_PAGE_SIZE] = { 0 };

	while (remaining > 0) {
		size_to_wr = remaining > CPS8601_SRAM_PAGE_SIZE ? CPS8601_SRAM_PAGE_SIZE : remaining;
...
static int cps8601_fw_check_bootloader(struct cps8601_dev_info *di)
{
	...
	if (!di || !di->firm_info.btl)
		return -EINVAL;

	crc_cal = cps8601_fw_crc_cal(di->firm_info.btl, di->firm_info.btl_size);

	ret = cps8601_hw_write_dword(di, CPS8601_BOOTLOADER_ADDR_BUFFER0, di->firm_info.btl_size);
	power_usleep(DT_USLEEP_2MS);
	ret += cps8601_hw_read_dword(di, CPS8601_BOOTLOADER_ADDR_BUFFER0, &data);
	if (ret || (data != di->firm_info.btl_size)) {
	...
	for (i = 0; i < CPS8601_FW_REG_CFG_SIZE; i++) {
		if (cps8601_fw_boot_cfg[i].addr != CPS8601_BOOTLOADER_BOOT_ADD) {
			...
			ret = cps8601_fw_write_sram_data(di, CPS8601_BOOTLOADER_BOOT_ADD,
				di->firm_info.btl, di->firm_info.btl_size);
			if (ret) {
...
static int cps8601_fw_write_mtp_data(struct cps8601_dev_info *di)
{
	...
	if (!di || !di->firm_info.mtp)
		return -EINVAL;

	remaining = di->firm_info.mtp_size;
	while (remaining > 0) {
		...
		ret = cps8601_fw_write_sram_data(di, CPS8601_BOOTLOADER_ADDR_BUFFER0,
			di->firm_info.mtp + offset, wr_size);
		if (ret) {
...
static int cps8601_fw_check_mtp_crc_cal(struct cps8601_dev_info *di)
{
	...
	crc_cal = cps8601_fw_crc_cal(di->firm_info.mtp, di->firm_info.mtp_size);

	ret = cps8601_hw_write_dword(di, CPS8601_BOOTLOADER_ADDR_BUFFER0, di->firm_info.mtp_size);
	power_usleep(DT_USLEEP_2MS);
	ret += cps8601_hw_read_dword(di, CPS8601_BOOTLOADER_ADDR_BUFFER0, &data);
	if (ret || (data != di->firm_info.mtp_size)) {
...
static int cps8601_fw_write_q_threshold(struct cps8601_dev_info *di)
{
	int ret;

	if ((di->chip_id != CPS8602_CHIP_ID) ||
		(di->default_psy_type != CPS8601_DEFAULT_LOWPOWER) || !di->tx_q_threshold_en)
		return 0;

	di->tx_q_th.tx_q_threshold.tx_q_crc =
		cps8601_fw_crc_cal(&di->tx_q_th.tx_q_config[CPS8601_Q_THRESHOLD_CRC_LEN],
		CPS8601_Q_THRESHOLD_LEN - CPS8601_Q_THRESHOLD_CRC_LEN);
	hwlog_info("[write_q_threshold] cnt: %d ~ %d width: %d ~ %d crc: 0x%x\n",
	...
	ret = cps8601_fw_write_sram_data(di, CPS8601_BOOTLOADER_ADDR_BUFFER0,
		di->tx_q_th.tx_q_config, CPS8601_Q_THRESHOLD_LEN);
	if (ret) {
		hwlog_err("write_q_threshold: write q_threshold failed\n");
		return ret;
	}

	ret = cps8601_fw_status_check(di, CPS8601_CMD_PGM_BUFFER0);
	if (ret) {
		hwlog_err("write_q_threshold: check crc failed\n");
		return ret;
	}
...
static int cps8601_fw_load_mtp(struct cps8601_dev_info *di)
{
	...
	ret = cps8601_fw_write_q_threshold(di);
	if (ret) {
		hwlog_err("load_mtp: q threshold failed\n");
		return ret;
	}
...
static int cps8601_fw_check_q_threshold(struct cps8601_dev_info *di)
{
	s8 cnt_low = 0;
	s8 cnt_high = 0;
	u16 width_low = 0;
	...
	ret = cps8601_read_word(di, CPS8601_TX_Q_TH_CRC_ADDR, &crc);
	ret += cps8601_read_byte(di, CPS8601_TX_Q_TH_CNT_LOW_ADDR, &cnt_low);
	ret += cps8601_read_byte(di, CPS8601_TX_Q_TH_CNT_HIGH_ADDR, &cnt_high);
	ret += cps8601_read_word(di, CPS8601_TX_Q_TH_WIDTH_LOW_ADDR, &width_low);
	ret += cps8601_read_word(di, CPS8601_TX_Q_TH_WIDTH_HIGH_ADDR, &width_high);
	if (ret) {
		hwlog_err("check_q_threshold: i2c failed\n");
		return -EINVAL;
	}

	hwlog_info("[check_q_threshold] cnt: %d ~ %d, width: %d ~ %d, crc: 0x%x\n",
		cnt_low, cnt_high, width_low, width_high, crc);

	if ((cnt_low == di->tx_q_th.tx_q_threshold.tx_q_cnt_low) &&
		(cnt_high == di->tx_q_th.tx_q_threshold.tx_q_cnt_high) &&
		(width_low == di->tx_q_th.tx_q_threshold.tx_q_width_low) &&
		(width_high == di->tx_q_th.tx_q_threshold.tx_q_width_high))
		return 0;
...
static int cps8601_fw_progam_post_process(struct cps8601_dev_info *di)
{
	...
	ret = cps8601_fw_check_q_threshold(di);
	if (ret) {
		hwlog_err("progam_post_process: q_threshold mismatch\n");
		return ret;
	}
...
static int cps8601_fw_check_mtp(void *dev_data)
{
	...
	if (cps8601_fw_check_q_threshold(di)) {
		hwlog_err("check_mtp: q_threshold mismatch\n");
		goto check_fail;
	}
