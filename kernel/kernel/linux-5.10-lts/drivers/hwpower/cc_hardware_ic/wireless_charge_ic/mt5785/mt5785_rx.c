...
static bool mt5785_rx_is_fc_succ(struct mt5785_dev_info *di, int vset)
{
	...
	for (i = 0; i < cnt; i++) {
		if ((di->g_val.rx_stop_chrg && (vset > WLRX_IC_DFLT_VOUT)) || !mt5785_rx_is_tx_exist(di))
			return false;
...
static bool mt5785_rx_is_vmldo_set_succ(struct mt5785_dev_info *di, int vfc)
{
	...
	for (i = 0; i < cnt; i++) {
		if ((di->g_val.rx_stop_chrg && (vfc > WLRX_IC_DFLT_VOUT)) ||
			!mt5785_rx_is_tx_exist(di) || mt5785_is_sleep_enable(di))
			return false;
...
static int mt5785_rx_set_vfc(int vfc, void *dev_data)
{
	...
	for (i = 0; i < MT5785_RX_FC_VPA_RETRY_CNT; i++) {
		if ((di->g_val.rx_stop_chrg && (vfc > WLRX_IC_DFLT_VOUT)) || !mt5785_rx_is_tx_exist(di)) {
			ret = -EPERM;
			goto out;
		}
...
static void mt5785_rx_power_on_handler(struct mt5785_dev_info *di)
{
	...
	mt5785_rx_get_vfc_reg(&di->last_vfc, di);
...
static ssize_t mt5785_rx_cm_store(void *dev_data, const char *buf, size_t size)
{
	int tmp[MT5785_RX_CM_LEN] = { 0 };
	struct mt5785_dev_info *di = NULL;

	di = (struct mt5785_dev_info *)dev_data;
	if (!di)
		return -ENODEV;

	/* 0~2 index */
	if (sscanf_s(buf, "%x %x %x", &tmp[0], &tmp[1], &tmp[2]) != MT5785_RX_CM_LEN) {
...
static ssize_t mt5785_rx_cm_show(void *dev_data, char *buf, size_t size)
{
	struct mt5785_dev_info *di = NULL;
...
static void mt5785_rx_debug_register(struct mt5785_dev_info *di)
{
	...
	power_dbg_ops_register(dir_name, "cm_cfg", di, mt5785_rx_cm_show, mt5785_rx_cm_store);
}
