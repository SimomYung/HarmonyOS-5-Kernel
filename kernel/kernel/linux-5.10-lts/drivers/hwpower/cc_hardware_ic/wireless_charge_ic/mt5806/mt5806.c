...
static int mt5806_event_notifier_call(struct notifier_block *nb, unsigned long event, void *data)
{
	struct mt5806_dev_info *di = container_of(nb, struct mt5806_dev_info, event_nb);
	...
	case POWER_NE_WLTX_AUX_PEN_HALL_AWAY_FROM:
		di->fod_gap = MT5806_CLEAR_FOD_GAP;
		break;
	default:
		return NOTIFY_OK;
	}
...
static void mt5806_s4_restore_work(struct work_struct *work)
{
	struct mt5806_dev_info *di = NULL;

	if (!work)
		return;
	di = container_of(work, struct mt5806_dev_info, s4_work);
	if (!di)
		return;
...
static int mt5806_request_dev_resources(struct mt5806_dev_info *di, struct device_node *np)
{
	...
	di->system_wq = alloc_workqueue("mt5806_highpri_unbound_wq",
	...
	di->event_nb.notifier_call = mt5806_event_notifier_call;
	ret = power_event_bnc_register(POWER_BNT_WLTX_AUX, &di->event_nb);
	if (ret)
		goto register_notify_fail;

#ifdef CONFIG_HIBERNATION
	INIT_WORK(&di->s4_work, mt5806_s4_restore_work);
#endif /* CONFIG_HIBERNATION */
...
static int mt5806_resume(struct device *dev)
{
	...
	if (is_ulsr()) {
		...
		if (di->default_psy_type != MT5806_DEFAULT_LOWPOWER)
			return 0;
		gpio_direction_output(di->gpio_lowpower, MT5806_Q_SHORT_PERIOD);
...
#ifdef CONFIG_HIBERNATION
static int mt5806_freeze(struct device *dev)
{
	struct i2c_client *client = to_i2c_client(dev);
	struct mt5806_dev_info *di = NULL;
...
static int mt5806_restore(struct device *dev)
{
	struct i2c_client *client = to_i2c_client(dev);
	struct mt5806_dev_info *di = NULL;
...
#ifdef CONFIG_HIBERNATION
	.freeze = mt5806_freeze,
	.restore = mt5806_restore,
#endif /* CONFIG_HIBERNATION */
};
