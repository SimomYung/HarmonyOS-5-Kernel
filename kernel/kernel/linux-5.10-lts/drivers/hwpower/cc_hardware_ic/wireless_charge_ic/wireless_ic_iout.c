...
#define RX_CR_TEMP_PARA_ROW     1
#define RX_CR_TEMP_PARA_COL     2
...
struct wlic_iout_dev {
	...
	struct wlirx_cr_temp_para cr_temp_para[RX_CR_TEMP_PARA_ROW];
	bool rx_stop_chrg_flag;
	bool cr_temp_err_report_flag;
...
static void wlic_cr_temp_err_report_dmd(struct wlic_iout_dev *di, struct wlirx_cr_temp_para cr_temp_para)
{
	if (di->cr_temp_err_report_flag)
		return;
	hwlog_info("cr_temp error, cr_temp is %d, rx_imax is %d",
		cr_temp_para.cr_temp, cr_temp_para.rx_imax);
	power_event_bnc_notify(POWER_BNT_WLRX, POWER_NE_WLRX_CR_TEMP_ERR, NULL);
	di->cr_temp_err_report_flag = true;
}

static void wlic_iout_get_cr_temp_imax(struct wlic_iout_dev *di, int *imax_cr_temp)
{
	int i;
	int cr_temp = 0;

	if (di->cr_temp_para[0].cr_temp <= 0) {
		*imax_cr_temp = WLIC_DEFAULT_RX_IMAX_IC;
		return;
	}

	cr_temp = power_temp_get_raw_value(POWER_TEMP_RX_CR);
	for (i = 0; i < RX_CR_TEMP_PARA_ROW; i++) {
		if (cr_temp > di->cr_temp_para[i].cr_temp) {
			hwlog_info("wlrx_cr_temp is %d", cr_temp);
			wlic_cr_temp_err_report_dmd(di, di->cr_temp_para[i]);
			*imax_cr_temp = di->cr_temp_para[i].rx_imax;
			return;
		}
	}
...
static void wlic_calc_rx_imax(struct wlic_iout_dev *di)
{
	...
	wlic_iout_get_cr_temp_imax(di, &imax_cr_temp);

	rx_imax = min(rx_imax, imax_temp);
	rx_imax = min(rx_imax, imax_time);
	rx_imax = min(rx_imax, imax_cr_temp);
...
void wlic_iout_set_cur_pmode_name(unsigned int ic_type, const char *cur_pmode_name)
{
	...
	ret = strncpy_s(di->cur_pmode_name, RX_MODE_NAME_LEN - 1,
		cur_pmode_name, strlen(cur_pmode_name));
	if (ret)
		hwlog_err("strncpy_s fail in wlic_iout_set_cur_pmode_name\n");
...
void wlic_iout_start_sample(unsigned int ic_type)
{
	...
	di->cr_temp_err_report_flag = false;
	if (di->rx_imax_flag != RX_IMAX_NO_NEED_MON)
...
static int wlic_iout_power_hall_event_notifier_call(struct notifier_block *power_hall_nb,
	unsigned long event, void *data)
{
	struct wlic_iout_dev *di = container_of(power_hall_nb, struct wlic_iout_dev, power_hall_nb);
	...
	case POWER_NE_SCREEN_FOLD:
		wlic_iout_select_para(di->ic_type, WLICI_FACTOR_SCREEN_FOLD, true);
		break;
	case POWER_NE_SCREEN_UNFOLD:
		wlic_iout_select_para(di->ic_type, WLICI_FACTOR_SCREEN_FOLD, false);
		break;
	default:
		break;
	}
...
static int wlic_iout_fb_notifier_call(struct notifier_block *fb_nb, unsigned long event, void *data)
{
	struct wlic_iout_dev *di = container_of(fb_nb, struct wlic_iout_dev, fb_nb);
	...
	case POWER_NE_PANEL_BLANK:
		wlic_iout_select_para(di->ic_type, WLICI_FACTOR_SCREEN_ON, false);
		break;
	case POWER_NE_PANEL_UNBLANK:
		wlic_iout_select_para(di->ic_type, WLICI_FACTOR_SCREEN_ON, true);
		break;
	default:
		break;
	}
...
static ssize_t wlic_iout_time_para_store(void *dev_data, const char *buf, size_t size)
{
	...
	for (i = 0; i < RX_TIME_PARA_ROW; i++) {
		...
		if (strncat_s(out_buf, OUT_BUF_SIZE, tmp_buf, strlen(tmp_buf)))
			return -EINVAL;
	}
...
static void wlic_parse_rx_time_para_group(const struct device_node *np, struct wlic_iout_dev *di)
{
	...
	for (i = 0; i < len; i++) {
		...
		case WLIRX_TIMEG_MODE_NAME:
			if (strncpy_s(di->time_para_group[row].pmode_name, RX_MODE_NAME_LEN - 1,
				tmp_str, strlen(tmp_str)))
				hwlog_err("strncpy_s fail in wlic_parse_rx_time_para_group\n");
			break;
		case WLIRX_TIMEG_FACTOR_MODE:
			if (kstrtouint(tmp_str, 0, &di->time_para_group[row].factor_mode))
				goto error;
			break;
...
static void wlic_parse_rx_cr_temp_para(const struct device_node *np, struct wlic_iout_dev *di)
{
	int len;

	len = power_dts_read_u32_count(power_dts_tag(HWLOG_TAG), np, "cr_temp_para",
		RX_CR_TEMP_PARA_ROW, RX_CR_TEMP_PARA_COL);
	if (len <= 0)
		return;

	if (power_dts_read_u32_array(power_dts_tag(HWLOG_TAG), np,
		"cr_temp_para", (u32 *)di->cr_temp_para, len))
		return;
...
static int wlic_rx_imax_init(struct wlic_iout_dev *di, const struct device_node *np, int *rx_ic_imax)
{
	...
	wlic_parse_rx_cr_temp_para(np, di);
	wlic_iout_dbg_register(di);

	return 0;
}

static void wlic_event_register(struct wlic_iout_dev *di)
{
	di->power_hall_nb.notifier_call = wlic_iout_power_hall_event_notifier_call;
	(void)power_event_bnc_register(POWER_BNT_HALL_EVENT, &di->power_hall_nb);
	di->fb_nb.notifier_call = wlic_iout_fb_notifier_call;
	(void)power_event_bnc_register(POWER_BNT_PANEL_EVENT, &di->fb_nb);
}

static void wlic_event_unregister(struct wlic_iout_dev *di)
{
	(void)power_event_bnc_unregister(POWER_BNT_HALL_EVENT, &di->power_hall_nb);
	(void)power_event_bnc_unregister(POWER_BNT_PANEL_EVENT, &di->fb_nb);
}

static void wlic_kfree_dev(unsigned int ic_type)
{
	wlic_event_unregister(g_ic_iout_di[ic_type]);
