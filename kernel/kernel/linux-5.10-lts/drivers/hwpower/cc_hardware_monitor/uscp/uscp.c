...
#define CHECK_EXIT_DC_COMP_TIEMS   20
...
#ifdef CONFIG_DIRECT_CHARGER
static void uscp_stop_dc_charging(struct uscp_device_info *di)
{
	int ret;
	int state;
	int i;
	...
	for (i = 0; i < CHECK_EXIT_DC_COMP_TIEMS; i++) {
	...
	if (i == CHECK_EXIT_DC_COMP_TIEMS)
		hwlog_info("stop dc charging failed\n");
	dc_set_stop_charging_flag(false);

	if (di->first_in) {
		if ((state == DC_STAGE_DEFAULT) && direct_charge_detect_adapter_again())
			uscp_set_dc_adapter(di, false);
		else if ((state == DC_STAGE_DEFAULT) || (state == DC_STAGE_CHARGE_DONE))
	...
	if (uscp_get_dc_adapter(di)) {
		...
		ret = dc_set_adapter_output_disable();
		if (ret) {
