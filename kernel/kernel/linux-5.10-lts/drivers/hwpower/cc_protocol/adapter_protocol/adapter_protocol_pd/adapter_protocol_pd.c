...
static bool hwpd_support_9v_src_cap(void)
{
	struct hwpd_ops *l_ops = NULL;
	...
	if (!l_ops->support_9v_src_cap) {
		hwlog_err("support_9v_src_cap is null\n");
		return false;
	}

	return l_ops->support_9v_src_cap(l_ops->dev_data);
}
...
static struct adapter_protocol_ops adapter_protocol_hwpd_ops = {
	.type_name = "hw_pd",
	.support_9v  = hwpd_support_9v_src_cap,
