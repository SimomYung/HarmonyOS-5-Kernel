...
#define HWSCP_REG_FUNC_SN                  0x01
#define HWSCP_REG_FUNC_POWER_BANK_CAP      0x02

#define SN_READ_FIRST_SECTION              0x07
#define SN_READ_SECOND_SECTION             0x17
#define SN_READ_THIRD_SECTION              0x27
#define SN_READ_COMMAND_NUM                4
#define SN_READ_TIMES                      3
#define SN_ONE_SECTION_NUM                 8
#define HWSCP_FWVER_2P0                    0x200
...
static const int g_hwscp_read_special_reg[] = {
	HWSCP_ADP_TYPE0,
	HWSCP_POWER_CURVE_NUM,
	HWSCP_TERMINAL_ID,
	HWSCP_POWER_ALLOCATE,
	HWSCP_REG_F2,
};

static const int g_hwscp_multi_read_special_reg[] = {
	HWSCP_ADP_TYPE0,
	HWSCP_REG_F2,
};

static const int g_hwscp_write_special_reg[] = {
	HWSCP_ADP_TYPE0,
	HWSCP_POWER_ALLOCATE,
	HWSCP_REG_F0,
	HWSCP_REG_F1,
...
static bool hwscp_is_special_reg(int reg, const int *special_reg, int size)
{
	int i;

	if (!special_reg || (size <= 0))
		return false;

	for (i = 0; i < size; i++) {
		if (special_reg[i] == reg)
			return true;
	}
...
static int hwscp_reg_read(int reg, int *val, int num)
{
	...
	if (ret < 0) {
		if (!hwscp_is_special_reg(reg, g_hwscp_read_special_reg,
			ARRAY_SIZE(g_hwscp_read_special_reg)))
...
static int hwscp_reg_multi_read(int reg, int *val, int num)
{
	...
	if (ret) {
		if (!hwscp_is_special_reg(reg, g_hwscp_multi_read_special_reg,
			ARRAY_SIZE(g_hwscp_multi_read_special_reg)))
			hwscp_set_rw_error_flag(RW_ERROR_FLAG);
...
static int hwscp_reg_write(int reg, const int *val, int num)
{
	...
	if (ret < 0) {
		if (!hwscp_is_special_reg(reg, g_hwscp_write_special_reg,
			ARRAY_SIZE(g_hwscp_write_special_reg)))
			hwscp_set_rw_error_flag(RW_ERROR_FLAG);
...
static int hwscp_get_terminal_id(int *terminal_id)
{
	int value[BYTE_ONE] = { 0 };
	struct hwscp_dev *l_dev = NULL;
	...
	if (l_dev->info.terminal_id_rd_flag == HAS_READ_FLAG) {
		*terminal_id = l_dev->info.terminal_id;
		hwlog_info("get_terminal_id_a: %x\n", l_dev->info.terminal_id);
		return 0;
	}

	/* reg: 0x8c */
	if (hwscp_reg_read(HWSCP_TERMINAL_ID, value, BYTE_ONE))
		return -EPERM;

	l_dev->info.terminal_id_rd_flag = HAS_READ_FLAG;
	l_dev->info.terminal_id = value[0];
	*terminal_id = value[0];
	hwlog_info("get_terminal_id_f: %x\n", value[0]);
	return 0;
}

static int hwscp_get_support_power_allocate(void)
{
	int value[BYTE_ONE] = { 0 };
	struct hwscp_dev *l_dev = NULL;
	...
	if (l_dev->info.pwr_allocate_support_rd_flag == HAS_READ_FLAG) {
		hwlog_info("get_support_power_allocate_a: %x\n", l_dev->info.pwr_allocate_support);
		return 0;
	}

	/* reg: 0xb6 */
	if (hwscp_reg_read(HWSCP_POWER_ALLOCATE, value, BYTE_ONE))
		return -EPERM;

	l_dev->info.pwr_allocate_support_rd_flag = HAS_READ_FLAG;
	l_dev->info.pwr_allocate_support = (value[0] & HWSCP_SLAVE_SUPPORT_PA_MASK) >> HWSCP_SLAVE_SUPPORT_PA_SHIFT;

	hwlog_info("get_support_power_allocate_f: %d\n", l_dev->info.pwr_allocate_support);
	return 0;
}
...
static int hwscp_get_device_info(struct adapter_device_info *info)
{
	...
	hwscp_get_terminal_id(&info->terminal_id);
	(void)hwscp_get_support_power_allocate();
	...
	info->prot_type = ADAPTER_PROTOCOL_SCP;
...
static int hwscp_set_default_state(void)
{
	...
	/*
	 * Wait for adapter voltage drop to 5.5v, otherwise some 100w typeA adapters will ovp
	 * when set scp out mode disable, resulting in charge disconnect.
	 */
	power_msleep(DT_MSLEEP_80MS, 0, NULL);
...
static int hwscp_check_config_value(void)
{
...
static int hwscp_set_master_power_allocate(int enable)
{
	int value[BYTE_ONE] = { 0 };
	int tmp_value;
	...
	tmp_value = enable ? HWSCP_MASTER_PA_ENABLE : HWSCP_MASTER_PA_DISABLE;
	value[0] &= ~HWSCP_MASTER_SUPPORT_PA_MASK;
	value[0] |= ((int)(tmp_value << HWSCP_MASTER_SUPPORT_PA_SHIFT) & HWSCP_MASTER_SUPPORT_PA_MASK);

	/* reg: 0xb6 */
	if (hwscp_reg_write(HWSCP_POWER_ALLOCATE, value, BYTE_ONE))
		return -EPERM;

	hwlog_info("set_master_power_allocate: %d,%x\n", enable, value[0]);
	return 0;
}

static int hwscp_get_power_redistributed(int *state)
{
	int value[BYTE_ONE] = { 0 };
	...
	*state = (value[0] & HWSCP_SSTS_ALLOCATE_MASK) >> HWSCP_SSTS_ALLOCATE_SHIFT;
	hwlog_info("get_power_redistributed: %d\n", *state);
	return 0;
}

static bool hwscp_is_power_redistributed(void)
{
	struct hwscp_dev *l_dev = hwscp_get_dev();
	int ret;
	int pwr_state = HWSCP_ALLOCATE_DEFAULE;

	if (!l_dev)
		return false;

	if ((l_dev->info.terminal_id != HWSCP_TERMINAL_ID_HW) ||
		(l_dev->info.pwr_allocate_support != HWSCP_SLAVE_SUPPORT_PA))
		return false;

	ret = hwscp_get_power_redistributed(&pwr_state);
	if (ret < 0 || pwr_state != HWSCP_ALLOCATE_ENABLE)
		return false;

	ret = hwscp_set_master_power_allocate(HWSCP_MASTER_PA_ENABLE);
	ret += hwscp_get_power_redistributed(&pwr_state);
	if (ret < 0 || pwr_state != HWSCP_ALLOCATE_DEFAULE)
		return false;

	hwscp_reset_power_curve(l_dev);
	hwlog_info("adapter power redistributed happen, need exit dc\n");
	return true;
}

static int hwscp_get_adp_sn(u8 *sn_info, u8 size)
{
	int i;
	int terminal_id = 0;
	int scp_ver_id = 0;
	struct hwscp_dev *l_dev = hwscp_get_dev();
	int value[SN_READ_COMMAND_NUM] = { HWSCP_REG_FUNC_SN, SN_READ_FIRST_SECTION,
		SN_READ_SECOND_SECTION, SN_READ_THIRD_SECTION };
	int sn[ADAPTER_SN_MAX_SIZE] = { 0 };

	if (!l_dev || !sn_info || (size != ADAPTER_SN_MAX_SIZE))
		return -EINVAL;

	if (l_dev->info.adp_sn_rd_flag == HAS_READ_FLAG) {
	...
	if (hwscp_get_terminal_id(&terminal_id))
		return -EPERM;

	if (terminal_id != HWSCP_TERMINAL_ID_HW)
		return -EPERM;

	if (hwscp_get_fw_version_id(&scp_ver_id))
		return -EPERM;

	if (scp_ver_id <= HWSCP_FWVER_2P0)
		return -EPERM;

	if (hwscp_reg_write(HWSCP_REG_F0, &value[0], BYTE_ONE)) {
		hwlog_err("%s write 0xf0 failed\n", __func__);
		return -EPERM;
	}

	for (i = 0; i < SN_READ_TIMES; i++) {
		if (hwscp_reg_write(HWSCP_REG_F1, &value[i + 1], BYTE_ONE))
			return -EPERM;

		if (hwscp_reg_multi_read(HWSCP_REG_F2, &sn[i * SN_ONE_SECTION_NUM], SN_ONE_SECTION_NUM))
			return -EPERM;
	}

	for (i = 0; i < ADAPTER_SN_MAX_SIZE; i++)
		sn_info[i] = (u8)sn[i];

	l_dev->info.adp_sn_rd_flag = HAS_READ_FLAG;
	return 0;
}

static int hwscp_get_power_bank_capacity(int *cap)
{
	struct hwscp_dev *l_dev = hwscp_get_dev();
	int value[BYTE_ONE] = { HWSCP_REG_FUNC_POWER_BANK_CAP };

	if (!l_dev || !cap)
		return -EINVAL;

	if (hwscp_reg_write(HWSCP_REG_F0, value, BYTE_ONE)) {
		hwlog_err("%s write 0xf0 failed\n", __func__);
		return -EPERM;
	}

	return hwscp_reg_read(HWSCP_REG_F1, cap, BYTE_ONE);
}
...
static struct adapter_protocol_ops adapter_protocol_scp_ops = {
	...
	.is_power_redistributed = hwscp_is_power_redistributed,
	.get_adp_sn = hwscp_get_adp_sn,
	.get_power_bank_cap = hwscp_get_power_bank_capacity,
};
