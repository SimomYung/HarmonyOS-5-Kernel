...
#define UFCS_AUTH_ENCRYPT_CMDS_LEN             2
#define UFCS_ADAPTER_CURRENT_ACCURACY_ERR      200

#define NAPOLEON_ADAPTER_A1_SOURCE_ID          0xb
#define NAPOLEON_ADAPTER_C1_SOURCE_ID          0xc
#define NAPOLEON_ADAPTER_C2_SOURCE_ID          0xd
...
enum UFCS_CMD_SET {
	...
	UFCS_CMD_VDM_SET_EXPAND_CURRENT,
	UFCS_CMD_VDM_GET_ADP_SN,
	UFCS_CMD_VDM_GET_POWER_BANK_CAP,
	UFCS_CMD_END,
};

static bool g_in_ufcs_mode;
...
static int hwufcs_set_default_state(void)
{
	...
	hwufcs_reset_ufcs_mode_flag();
	ret = hwufcs_send_control_msg(HWUFCS_CTL_MSG_EXIT_HWUFCS_MODE, true);
	ret += hwufcs_request_communication(false);
	/*
	 * Need wait for adapter exit ufcs mode, otherwise adapter will
	 * mistaken hard reset after soft reset master.
	 */
	...
	/*
	 * The Napoleon adapter has no load capacity within 100ms after set adapter default.
	 * We need wait for the adapter to stabilize for 100ms, then we can cancle buck hiz.
	 * Otherwise there is low probability of charge disconnect.
	 */
	if ((l_dev->info.source_id == NAPOLEON_ADAPTER_A1_SOURCE_ID) ||
		(l_dev->info.source_id == NAPOLEON_ADAPTER_C1_SOURCE_ID) ||
		(l_dev->info.source_id == NAPOLEON_ADAPTER_C2_SOURCE_ID)) {
		power_msleep(DT_MSLEEP_100MS, 0, NULL);
		hwlog_info("use napolun adapter 0x%x, need wait for voltage decrease to 5v\n", l_dev->info.source_id);
	}
...
static int hwufcs_set_default_param(void)
{
	...
	(void)hwufcs_handle_get_power_redistributed();
	return 0;
}

static int hwufcs_soft_reset_cmd(void)
{
	int ret;
...
{
	int ret;
	bool need_ignore_accept = hwufcs_is_ignore_accept_in_auth_encrypt();
	u8 cmds[UFCS_AUTH_ENCRYPT_CMDS_LEN] = {
		HWUFCS_CTL_MSG_ACCEPT, HWUFCS_CTL_MSG_GET_DEVICE_INFO };
	u8 rcv_cmd = 0;
	struct hwufcs_dev *l_dev = hwufcs_get_dev();
	...
	/*
	 * After send verify request, V600 reply ack quickly. Then the adapter will
	 * send accept message, V600 loss accept message with high probability, result in
	 * auth encrypt fail. So we need ignore accept message here.
	 */
	if (need_ignore_accept)
		ret = hwufcs_receive_predictable_control_msgs(cmds, UFCS_AUTH_ENCRYPT_CMDS_LEN,
			&rcv_cmd, false);
	else
		ret = hwufcs_receive_control_msg(HWUFCS_CTL_MSG_ACCEPT, false);
	hwlog_info("need_ignore_accept=%u, rcv_cmd=%u, ret=%d\n", need_ignore_accept, rcv_cmd, ret);
	ret = hwufcs_check_communication_state(ret);
	if (ret)
		goto end;

	if (!need_ignore_accept || (rcv_cmd == HWUFCS_CTL_MSG_ACCEPT)) {
...
static int hwufcs_vdm_set_expand_current(void *p)
{
	int ret;
	...
	ret = hwufcs_send_vdm_control_msg(HWUFCS_VDM_CTL_MSG_GET_EXPAND_CURRENT, true);
	ret = hwufcs_check_communication_state(ret);
	if (ret)
		goto end;
...
static int hwufcs_vdm_get_adp_sn(void *p)
{
	int ret;
	struct hwufcs_dev *l_dev = p;
	...
	ret = hwufcs_send_vdm_control_msg(HWUFCS_VDM_CTL_MSG_GET_ADP_SN, true);
	ret = hwufcs_check_communication_state(ret);
	if (ret)
		goto end;

	ret = hwufcs_receive_vdm_adp_sn_data_msg(l_dev->info.adp_sn, sizeof(l_dev->info.adp_sn));
	ret = hwufcs_check_communication_state(ret);
...
static int hwufcs_vdm_get_power_bank_cap(void *p)
{
	int ret;
	...
	ret = hwufcs_send_vdm_control_msg(HWUFCS_VDM_CTL_MSG_GET_POWER_BANK_CAP, true);
	ret = hwufcs_check_communication_state(ret);
	if (ret)
		goto end;

	ret = hwufcs_receive_vdm_power_bank_cap_data_msg(p);
	ret = hwufcs_check_communication_state(ret);
...
static struct ufcs_cmd_info g_ufcs_cmd_data[] = {
	...
	{ UFCS_CMD_VDM_SET_EXPAND_CURRENT, hwufcs_vdm_set_expand_current },
	{ UFCS_CMD_VDM_GET_ADP_SN, hwufcs_vdm_get_adp_sn },
	{ UFCS_CMD_VDM_GET_POWER_BANK_CAP, hwufcs_vdm_get_power_bank_cap },
};
...
static int hwufcs_soft_reset_slave(void)
{
	...
	ret = hwufcs_soft_reset_cmd();
	/*
	 * Adapter soft reset will reset watchdog time to default 1s,
	 * so we need reconfig watchdog after send soft reset.
	 */
	if (!ret) {
		wtg.time = di->info.wtg_time;
		ret1 = hwufcs_config_watchdog_cmd(&wtg);
		hwlog_info("%s wtg_time=%u, ret1=%d\n", __func__, wtg.time, ret1);
	}
...
static int hwufcs_get_source_info(struct adapter_source_info *p)
{
	...
	p->dev_temp = data.dev_temp;
	(void)memcpy_s(&l_dev->info.source_info_data, sizeof(struct hwufcs_source_info_data),
		&data, sizeof(struct hwufcs_source_info_data));
	return 0;
}
...
static int hwufcs_send_ping(void)
{
	...
	if (g_in_ufcs_mode) {
		hwlog_info("already in ufcs mode\n");
		return 0;
	}
...
static int hwufcs_detect_adapter_support_mode(int *mode)
{
	...
	/* skip ping if already in test mode */
	if (l_dev->info.support_mode == ADAPTER_TEST_MODE) {
		hwlog_info("adapter mode is test mode,ignore ping\n");
...
static int hwufcs_set_expand_current(void)
{
	int ret;

	ret = hwufcs_cmd_retry(UFCS_CMD_VDM_SET_EXPAND_CURRENT, NULL);
	return ret ? -EPERM : 0;
}
...
static int hwufcs_set_init_data(struct adapter_init_data *data)
{
	...
	if (!hwufcs_set_expand_current())
		hwlog_info("set expand current succ\n");
...
static int hwufcs_request_output_data(struct hwufcs_request_data req)
{
	...
	(void)hwufcs_get_power_drop_current(&cur);
	if ((cur > 0) && (l_dev->info.source_info_data.output_curr <= cur) &&
		(req.output_curr > cur)) {
		req.output_curr = cur;
		hwlog_info("request output curr larger than power change curr, need limit\n");
	}
...
static int hwufcs_set_output_current(int cur)
{
	...
	cur += UFCS_ADAPTER_CURRENT_ACCURACY_ERR;
...
static int hwufcs_get_precise_power_curve(struct adp_precise_pwr_curve_data *val, int *num, int max_size)
{
	unsigned int i;
	struct hwufcs_dev *l_dev = hwufcs_get_dev();
	...
	*num = l_dev->info.mode_quantity;
	...
	for (i = 0; i < *num; i++) {
		val[i].max_volt = l_dev->info.cap[i].max_volt;
		val[i].max_curr = l_dev->info.cap[i].max_curr;
		val[i].min_volt = l_dev->info.cap[i].min_volt;
		val[i].min_curr = l_dev->info.cap[i].min_curr;
	}
...
static int hwufcs_get_min_current(int *cur)
{
	...
	min_curr = l_dev->info.cap[0].min_curr;
	if (hwufcs_get_output_capabilities(l_dev))
		return -EPERM;
...
static int hwufcs_get_max_current(int *cur)
{
	...
	max_curr = l_dev->info.cap[0].max_curr;
	if (hwufcs_get_output_capabilities(l_dev))
		return -EPERM;
...
static int hwufcs_auth_encrypt_start(int key)
{
	...
	if (ret) {
		if (hwufcs_retry_get_out_capabilities())
...
static int hwufcs_get_vdm_source_id(int *source_id)
{
	...
	if (ret) {
		hwlog_info("get source_id failed\n");
		return -EPERM;
	}
...
static int hwufcs_get_device_info(struct adapter_device_info *info)
{
	...
	if (!hwufcs_get_vdm_source_id(&l_dev->info.source_id)) {
		info->adp_code = l_dev->info.source_id;
		hwlog_info("get_vdm_source_id succ, source_id=0x%x\n", l_dev->info.source_id);
	}

	info->prot_type = ADAPTER_PROTOCOL_UFCS;
...
static int hwufcs_get_vdm_adp_sn(u8 *sn_info, u8 size)
{
	int i, ret;
	struct hwufcs_dev *l_dev = hwufcs_get_dev();

	if (!l_dev || !sn_info || (size != ADAPTER_SN_MAX_SIZE))
		return -EPERM;

	if (l_dev->info.sn_info_rd_flag == HAS_READ_FLAG) {
		hwlog_info("already get adp sn\n");
		if (memcpy_s(sn_info, size, l_dev->info.adp_sn, sizeof(l_dev->info.adp_sn)) != EOK)
			return -EPERM;
		return 0;
	}

	ret = hwufcs_cmd_retry(UFCS_CMD_VDM_GET_ADP_SN, (void *)l_dev);
	if (ret) {
		hwlog_err("get adp sn failed\n");
		return -EPERM;
	}

	if (memcpy_s(sn_info, size, l_dev->info.adp_sn, sizeof(l_dev->info.adp_sn)) != EOK) {
		hwlog_err("memcpy_s adp sn failed\n");
		return -EPERM;
	}

	l_dev->info.sn_info_rd_flag = HAS_READ_FLAG;
	return 0;
}

static int hwufcs_get_power_bank_capacity(int *cap)
{
	int ret;
	struct hwufcs_dev *l_dev = hwufcs_get_dev();
	...
	ret = hwufcs_cmd_retry(UFCS_CMD_VDM_GET_POWER_BANK_CAP, (void *)cap);
	if (ret) {
		hwlog_err("get power bank capacity failed\n");
		return -EPERM;
	}

	hwlog_info("get power bank capacity=%d\n", *cap);
	return 0;
}
...
static int hwufcs_get_vdm_scpb_pwr(struct adp_scpb_pwr_data *scpb_pwr, int *size, int max_size)
{
	...
	*size = l_dev->info.scpb_pwr_quantity;
	memcpy_s(scpb_pwr, *size * sizeof(struct adp_scpb_pwr_data),
		l_dev->info.scpb_pwr_data, *size * sizeof(struct adp_scpb_pwr_data));

	return 0;
}

static bool hwufcs_is_power_redistributed(void)
{
	return hwufcs_handle_get_power_redistributed();
}
...
static struct adapter_protocol_ops adapter_protocol_hwufcs_ops = {
	...
	.get_precise_power_curve = hwufcs_get_precise_power_curve,
	.is_power_redistributed = hwufcs_is_power_redistributed,
	.get_adp_sn = hwufcs_get_vdm_adp_sn,
	.get_power_bank_cap = hwufcs_get_power_bank_capacity,
};
	...
	unsigned long event, void *data)
{
	...
	case POWER_NE_USB_DISCONNECT:
		...
		hwufcs_soft_reset_master();
		hwufcs_reset_ufcs_mode_flag();
		di->info.support_mode = ADAPTER_SUPPORT_UNDEFINED;
		break;
...
void hwufcs_reset_ufcs_mode_flag(void)
{
	g_in_ufcs_mode = false;
}
