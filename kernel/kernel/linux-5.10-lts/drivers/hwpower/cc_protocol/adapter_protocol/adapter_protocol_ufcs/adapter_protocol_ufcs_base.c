...
static atomic_t g_power_redistribute = ATOMIC_INIT(0);
...
int hwufcs_updata_power_change_data(struct hwufcs_package_data *pkt)
{
	...
	if ((pkt->len % HWUFCS_POWER_CHANGE_DATA_SIZE != 0) ||
		(num > HWUFCS_POWER_CHANGE_MAX_OUTPUT_MODE)) {
...
static bool hwufcs_check_exception_cmd(u8 *cmds, int len, u8 rcv_cmd)
{
	int i;

	for (i = 0; i < len; i++) {
		if (cmds[i] == rcv_cmd) {
			hwlog_info("%s receive target cmd %u\n", __func__, cmds[i]);
			return false;
		}
	}
...
int hwufcs_receive_predictable_control_msgs(u8 *cmds, int len, u8 *rcv_cmd, bool check_msg_num)
{
	int ret;
	struct hwufcs_package_data pkt = { 0 };
	u8 msg_number = hwufcs_get_msg_number();

	if (!cmds || (len <= 0) || !rcv_cmd)
		return HWUFCS_ERR_UNKNOWN;
	...
	*rcv_cmd = pkt.cmd;
	...
	if ((pkt.msg_type != HWUFCS_MSG_TYPE_CONTROL) ||
		hwufcs_check_exception_cmd(cmds, len, pkt.cmd)) {
		hwlog_err("receive_control_msg msg_type=%u cmd=%u unexpect\n", pkt.msg_type, pkt.cmd);
		return hwufcs_handle_recevice_exception_msg(&pkt);
	}
...
void hwufcs_handle_power_redistribute(void)
{
	atomic_inc(&g_power_redistribute);
	hwlog_info("set g_power_redistribute = %d\n", atomic_read(&g_power_redistribute));
	hwufcs_send_control_msg(HWUFCS_CTL_MSG_ACCEPT, true);
}

bool hwufcs_handle_get_power_redistributed(void)
{
	if (atomic_read(&g_power_redistribute) > 0) {
		atomic_set(&g_power_redistribute, 0);
...
static void hwufcs_parse_vdm_scpb_pwr_curve_data(u64 data, struct adp_precise_pwr_curve_data *p)
{
...
/* vdm data message from source to sink: 0x06 adapter_sn */
int hwufcs_receive_vdm_adp_sn_data_msg(u8 *sn_info, u8 size)
{
	int ret;
	struct hwufcs_package_data pkt = { 0 };

	if (!sn_info || (size < ADAPTER_SN_MAX_SIZE))
		return HWUFCS_ERR_UNKNOWN;
	...
	/* check data msg */
	ret = hwufcs_check_receive_vdm_data_msg(&pkt, HWUFCS_VDM_DATA_MSG_ADP_SN);
	if (ret)
		goto end;

	if (pkt.len != size) {
		hwlog_err("adapter_sn length=%u,%u invalid\n", pkt.len, size);
		return HWUFCS_ERR_ILLEGAL_DATA;
	}

	if (memcpy_s(sn_info, size, pkt.data, pkt.len) != EOK) {
...
/* vdm data message from source to sink: 0x07 power_bank_capacity */
int hwufcs_receive_vdm_power_bank_cap_data_msg(int *cap)
{
	int ret;
	u8 len = sizeof(u8);
	struct hwufcs_package_data pkt = { 0 };
	...
	/* check data msg */
	ret = hwufcs_check_receive_vdm_data_msg(&pkt, HWUFCS_VDM_DATA_MSG_POWER_BANK_CAP);
	if (ret)
		goto end;

	if (pkt.len != len) {
		hwlog_err("power bank cap length=%u,%u invalid\n", pkt.len, len);
		return HWUFCS_ERR_ILLEGAL_DATA;
	}
