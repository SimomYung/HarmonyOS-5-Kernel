...
static const struct wireless_protocol_device_data g_hwqi_dev_data[] = {
	...
	{ WIRELESS_DEVICE_ID_MT5788, "mt5788" },
	{ WIRELESS_DEVICE_ID_BK96285, "bk96285" },
};
...
enum hwqi_op_type {
	HWQI_OPS_TYPE_BEGIN = 0,
	HWQI_SEND_MSG_OP,
	HWQI_SEND_MSG_ACK_OP,
	HWQI_SEND_MSG_ONLY_OP,
	HWQI_SEND_FSK_MSG_OP,
	HWQI_AUTO_SEND_FSK_MSG_OP,
	HWQI_SEND_FSK_ACK_MSG_OP,
	HWQI_GET_ASK_PACKET_OP,
	HWQI_OPS_TYPE_END,
};
...
/* Ensure that only one msg can be sent at the same time. */
static int hwqi_send_msg_exclusive(unsigned int ic_type, struct hwqi_msg_package *msg_pkg,
	enum hwqi_op_type op)
{
	int ret;

	if (!msg_pkg || (ic_type >= WLTRX_IC_MAX))
		return -EINVAL;

	if (g_hwqi_dev[ic_type]->send_occupied) {
		hwlog_info("hwqi_send_msg_exclusive: is occupied\n");
		return -EBUSY;
	}
	g_hwqi_dev[ic_type]->send_occupied = true;

	switch (op) {
	case HWQI_SEND_MSG_OP:
		ret = hwqi_send_msg(ic_type, msg_pkg->cmd, msg_pkg->para, msg_pkg->para_len,
			msg_pkg->data, msg_pkg->data_len, msg_pkg->retrys);
		break;
	case HWQI_SEND_MSG_ACK_OP:
		ret = hwqi_send_msg_ack(ic_type, msg_pkg->cmd, msg_pkg->para,
			msg_pkg->para_len, msg_pkg->retrys);
		break;
	case HWQI_SEND_MSG_ONLY_OP:
		ret = hwqi_send_msg_only(ic_type, msg_pkg->cmd, msg_pkg->para,
			msg_pkg->para_len, msg_pkg->retrys);
		break;
	default:
		ret = -EINVAL;
		hwlog_err("hwqi_send_msg_exclusive: no operetion\n");
		break;
	}

	g_hwqi_dev[ic_type]->send_occupied = false;
	return ret;
}
...
static int hwqi_set_fan_speed_limit(unsigned int ic_type, u8 limit)
{
	int ret;
	struct hwqi_dev *l_dev = hwqi_get_dev(ic_type);
	struct hwqi_msg_package msg_pkg = {
		HWQI_CMD_SET_FAN_SPEED_LIMIT,
		&limit, HWQI_PARA_FAN_SPEED_LIMIT_LEN,
		NULL, 0, WIRELESS_RETRY_ONE
	};
	...
	ret = hwqi_send_msg_exclusive(ic_type, &msg_pkg, HWQI_SEND_MSG_ACK_OP);
	if (ret) {
		hwlog_err("hwqi_set_fan_speed_limit: fail, ret=%d\n", ret);
		return ret;
	}
...
static int hwqi_convert_tx_type(u16 qi_type)
{
	...
	static struct {
		...
		{ HWQI_TX_TYPE_P0010, WLACC_TX_P0010 },
		{ HWQI_TX_TYPE_UPGRADE, WLACC_TX_UPGRADE },
		{ HWQI_TX_TYPE_UPGRADE_FSK, WLACC_TX_UPGRADE_FSK }
	};
...
static int hwqi_get_tx_type(unsigned int ic_type, int *type, char *ver, int size)
{
	int i;
	...
	*type = hwqi_convert_tx_type(data);
	...
	hwlog_info("[get_tx_type] tx_fw_ver: %s tx_type=%d\n", ver, *type);
	return 0;
}

static int hwqi_get_tx_sn(unsigned int ic_type, char *sn, int size)
{
	int i;
	u8 tx_sn[HWQI_ACK_TX_SN_LEN] = { 0 };
	int retry = WIRELESS_RETRY_TWO;
	struct hwqi_dev *l_dev = hwqi_get_dev(ic_type);
	...
	memset_s(l_dev->info.tx_sn, sizeof(l_dev->info.tx_sn), 0, sizeof(l_dev->info.tx_sn));
	if (hwqi_send_msg(ic_type, HWQI_CMD_GET_TX_SN,
		NULL, 0, tx_sn, HWQI_ACK_TX_SN_LEN, retry))
		return -ENOTSUPP;

	for (i = HWQI_ACK_TX_SN_S; i <= HWQI_ACK_TX_SN_E; i++) {
		l_dev->info.tx_sn[i - HWQI_ACK_TX_SN_S] = tx_sn[i];
		if (i != HWQI_ACK_TX_SN_E)
			snprintf_s(sn + strlen(sn), size,
				size - strlen(sn) - 1, "0x%02x ", tx_sn[i]);
		else
			snprintf_s(sn + strlen(sn), size,
				size - strlen(sn) - 1, "0x%02x", tx_sn[i]);
	}

	hwlog_info("[get_tx_sn] tx_sn: %s\n", sn);
	return 0;
}
...
static int hwqi_get_tx_adapter_id(unsigned int ic_type, u16 *adp_id, u8 *adp_manufacturer)
{
	int retry = WIRELESS_RETRY_TWO;
	u8 data[HWQI_PARA_GET_ADAPTER_ID_LEN] = { 0 };
	struct hwqi_dev *l_dev = hwqi_get_dev(ic_type);
 
	if (!l_dev || !adp_id || !adp_manufacturer)
		return -EINVAL;
 
	if (hwqi_send_msg(ic_type, HWQI_CMD_GET_TX_ADAPTER_ID, NULL, 0,
		data, HWQI_PARA_GET_ADAPTER_ID_LEN, retry))
		return -EINVAL;
	
	*adp_id = (data[HWQI_PARA_ADAPTER_ID_E] << POWER_BITS_PER_BYTE) | data[HWQI_PARA_ADAPTER_ID_S];
	*adp_manufacturer = data[HWQI_PARA_ADAPTER_TYPE_OFFSET] & HWQI_PARA_ADAPTER_TYPE_MASK;

	hwlog_info("[get_tx_adapter_id] adp_id=%hu adp_manufacturer=%hhu\n", *adp_id, *adp_manufacturer);
	return 0;
}
 
static int hwqi_get_tx_adapter_err_code(unsigned int ic_type, u16 *err_code)
{
	int retry = WIRELESS_RETRY_TWO;
	u8 data[HWQI_PARA_GET_ADAPTER_ERR_CODE_LEN] = { 0 };
	struct hwqi_dev *l_dev = hwqi_get_dev(ic_type);
 
	if (!l_dev || !err_code)
		return -EINVAL;
 
	if (hwqi_send_msg(ic_type, HWQI_CMD_GET_TX_ADAPTER_ERR_CODE, NULL, 0,
		data, HWQI_PARA_GET_ADAPTER_ERR_CODE_LEN, retry))
		return -EINVAL;
	
	*err_code = (data[HWQI_PARA_ADAPTER_ERR_CODE_E] << POWER_BITS_PER_BYTE) |
		data[HWQI_PARA_ADAPTER_ERR_CODE_S];
 
	hwlog_info("[get_tx_adapter_err_code] tx adapter err code=%hu\n", *err_code);
	return 0;
}
...
static void hwqi_parse_tx_extra_capability(struct wlprot_acc_cap *cap, u8 *data)
{
	...
	cap->support_qval_cali = data[HWQI_TX_EXTRA_CAP_ATTR2] &
		HWQI_PARA_TX_EXT_CAP_SUPPORT_QVAL_CALI_MASK;
	cap->support_adp_info = data[HWQI_TX_EXTRA_CAP_ATTR3] &
		HWQI_PARA_TX_EXT_ADAPTER_INFO_MASK;
	cap->support_fod_info = data[HWQI_TX_EXTRA_CAP_ATTR3] &
		HWQI_PARA_TX_EXT_FOD_INFO_MASK;
...
static int hwqi_get_tx_capability(unsigned int ic_type, struct wlprot_acc_cap *cap)
{
	...
	hwlog_info("tx_extra_cap: [support] fan=%d tec=%d fod_status=%d fop_range=%d vpa_max=%u km=%d support_qval_cali=%d type=%u\n",
		cap->support_fan, cap->support_tec, cap->support_fod_status,
		cap->support_fop_range, cap->vpa_max, cap->support_fod_km, cap->support_qval_cali, ic_type);
	return 0;
}

static int hwqi_get_tx_actual_power(unsigned int ic_type, int *tx_pwr)
{
	int ret;
	u8 data[HWQI_PARA_GET_PTX_LEN] = { 0 };
	struct hwqi_dev *l_dev = hwqi_get_dev(ic_type);
	struct hwqi_msg_package msg_pkg = {
		HWQI_CMD_TX_POWER, NULL, 0,
		data, HWQI_PARA_GET_PTX_LEN,
		WIRELESS_RETRY_TWO
	};
	...
	ret = hwqi_send_msg_exclusive(ic_type, &msg_pkg, HWQI_SEND_MSG_OP);
	if (ret) {
		hwlog_err("hwqi_get_tx_actual_power: fail, ret=%d\n", ret);
		return ret;
	}

	*tx_pwr = (data[HWQI_PARA_PTX_E] << POWER_BITS_PER_BYTE) | data[HWQI_PARA_PTX_S];

	hwlog_info("[hwqi_get_tx_actual_power] tx power=%hu\n", *tx_pwr);
	return 0;
}
...
static int hwqi_get_tx_max_power(unsigned int ic_type, int *power)
{
	...
	char tx_fwver[WIRELESS_TX_FWVER_LEN] = { 0 };

	if (!power)
		return -EINVAL;

	if (!hwqi_get_tx_type(ic_type, &tx_type, tx_fwver, WIRELESS_TX_FWVER_LEN) &&
...
static int hwqi_get_fod_result_info(unsigned int ic_type, struct wltx_fod_result_info *fod_result_info)
{
	int retry = WIRELESS_RETRY_TWO;
	u8 data[HWQI_PARA_GET_TX_FOD_RESULT_LEN] = { 0 };
	struct hwqi_dev *l_dev = hwqi_get_dev(ic_type);
 
	if (!l_dev || !fod_result_info)
		return -EINVAL;
 
	if (hwqi_send_msg(ic_type, HWQI_CMD_GET_FOD_RESULT, NULL, 0,
		data, HWQI_PARA_GET_TX_FOD_RESULT_LEN, retry))
		return -EINVAL;
	
	fod_result_info->q_qp_ratio = data[HWQI_PARA_TX_FOD_Q_QP_RATIO_OFFSET];
	fod_result_info->limit_pwr = data[HWQI_PARA_TX_FOD_LIMIT_POWER_OFFSET];
	fod_result_info->qs = data[HWQI_PARA_TX_FOD_QS_OFFSET];
	fod_result_info->fs = data[HWQI_PARA_TX_FOD_FS_OFFSET];
 
	hwlog_info("[get_fod_result_info] q_qp_ratio=%u limit_pwr=%u qs=%u fs=%u\n",
		fod_result_info->q_qp_ratio, fod_result_info->limit_pwr,
		fod_result_info->qs, fod_result_info->fs);

	return 0;
}

static int hwqi_get_fod_status(unsigned int ic_type, u8 *qr, u8 *fr)
{
	u8 data[HWQI_PARA_GET_FOD_STATUS_LEN] = { 0 };

	if (!qr || !fr)
		return -EINVAL;

	if (hwqi_send_msg(ic_type, HWQI_CMD_GET_FOD_STATUS, NULL, 0, data,
		HWQI_PARA_GET_FOD_STATUS_LEN, WIRELESS_RETRY_THREE))
		return -EINVAL;

	*fr = data[HWQI_PARA_GET_FOD_FR];
	*qr = data[HWQI_PARA_GET_FOD_QR];

	hwlog_info("get_fod_status: qr=%u fr=%u\n", *qr, *fr);

	return 0;
}

static int hwqi_get_fod_km_kz(unsigned int ic_type, u16 *km, u16 *kz)
{
	u16 err_code;
	u8 data[HWQI_PARA_GET_FOD_KM_KZ_LEN] = { 0 };

	if (!km || !kz)
		return -EINVAL;

	if (hwqi_send_msg(ic_type, HWQI_CMD_GET_FOD_KM_KZ, NULL, 0, data,
		HWQI_PARA_GET_FOD_KM_KZ_LEN, WIRELESS_RETRY_THREE))
		return -EINVAL;

	*km = (data[HWQI_PARA_GET_FOD_KM_H] << POWER_BITS_PER_BYTE) | data[HWQI_PARA_GET_FOD_KM_L];
	*kz = (data[HWQI_PARA_GET_FOD_KZ_H] & HWQI_PARA_GET_FOD_CALI_SUCC_MASK) >>
		HWQI_PARA_GET_FOD_CALI_SUCC_SHIFT;
	err_code = (data[HWQI_PARA_GET_FOD_KZ_H] << POWER_BITS_PER_BYTE) | data[HWQI_PARA_GET_FOD_KZ_L];

	hwlog_info("get_fod_status: km=%u kz=%u errCode=0x%x\n", *km, *kz, err_code);
...
static struct wireless_protocol_ops wireless_protocol_hwqi_ops = {
	...
	.get_tx_sn = hwqi_get_tx_sn,
	.get_tx_adapter_type = hwqi_get_tx_adapter_type,
	.get_tx_adapter_id = hwqi_get_tx_adapter_id,
	.get_tx_adapter_err_code = hwqi_get_tx_adapter_err_code,
	.get_tx_capability = hwqi_get_tx_capability,
	.get_tx_actual_power = hwqi_get_tx_actual_power,
	...
	.get_fod_result_info = hwqi_get_fod_result_info,
	.get_fod_status = hwqi_get_fod_status,
	.get_fod_km_kz = hwqi_get_fod_km_kz,
...
static int hwqi_handle_ask_packet_cmd_0x3b(unsigned int ic_type, u8 *data)
{
	...
	if ((tx_id == HWQI_HANDSHAKE_ID_HW) || (tx_id == HWQI_HANDSHAKE_ID_HW_SLOT)) {
		if (((l_dev->dev_attr & HWQI_TX_SUPPORT_SLE) == HWQI_TX_SUPPORT_SLE) &&
			l_dev->acc_info.support_sle)
			if (tx_id == HWQI_HANDSHAKE_ID_HW_SLOT)
				data[HWQI_ASK_PACKET_DAT2] = HWQI_HANDSHAKE_ID_HW_SLOT_SLE;
			else
				data[HWQI_ASK_PACKET_DAT2] = HWQI_HANDSHAKE_ID_HW_SLE;
		hwqi_send_tx_id(ic_type, &data[HWQI_ASK_PACKET_DAT1],
			HWQI_PARA_TX_ID_LEN);
		power_event_bnc_notify(hwqi_get_bnt_wltx_type(ic_type),
			POWER_NE_WLTX_HANDSHAKE_SUCC, &tx_id);
	}
...
static int parse_wlcase_soc_info(u8 *data, u8 *wlcase_soc_info)
{
	static struct {
		u8 lth;
		u8 hth;
	} const soc_table[] = {
		/* format: [soc_level] = {soc_lower, soc_upper} */
		[1] = { 0, 9 },
		[2] = { 10, 19 },
		[3] = { 20, 29 },
		[4] = { 30, 39 },
	...
	soc_level = data[HWQI_ASK_PACKET_DAT1] >> HWQI_WLCASE_SOC_SHIFT_BITS;
	soc_level = soc_level & HWQI_WLCASE_SOC_MASK;

	if (soc_level < HWQI_WLCASE_SOC_MIN_LEVEL || soc_level > HWQI_WLCASE_SOC_MAX_LEVEL)
		return -ENOTSUPP;

	/* soc_info[0]: lower soc, soc_info[1]: upper soc */
	wlcase_soc_info[0] = soc_table[soc_level].lth;
	wlcase_soc_info[1] = soc_table[soc_level].hth;

	return 0;
}

/* 0x28 + 0x43 + charger_state/wlcase_soc + checksum */
static int hwqi_handle_ask_packet_cmd_0x43(unsigned int ic_type, u8 *data)
{
	u8 chrg_state;
	u8 wlcase_soc_info[HWQI_WLCASE_SOC_INFO_NUM] = { 0, 0 };

	chrg_state = data[HWQI_ASK_PACKET_DAT1] & HWQI_CHRG_STATE_MASK;
	hwqi_send_fsk_ack_msg(ic_type);

	if (chrg_state & HWQI_CHRG_STATE_DONE) {
		hwlog_info("tx received rx charge-done event, charger_state=0x%x\n", chrg_state);
		power_event_bnc_notify(hwqi_get_bnt_wltx_type(ic_type),
			POWER_NE_WLTX_CHARGEDONE, NULL);
	}

	if (!parse_wlcase_soc_info(data, wlcase_soc_info)) {
		hwlog_info("tx received rx wlcase soc event, soc_info:[%hhu, %hhu]\n",
			wlcase_soc_info[0], wlcase_soc_info[1]);
		power_event_bnc_notify(POWER_BNT_LIGHTSTRAP, POWER_NE_WLCASE_SOC, wlcase_soc_info);
	}
...
static int hwqi_handle_ask_packet_hdr_0x71(unsigned int ic_type, u8 *data)
{
	...
	hwlog_info("wireless rx man_id = 0x%x, acc_id = 0x%x\n", manufacturer_id, acc_id);
...
static void hwqi_handle_fsk_tx_version(unsigned int ic_type, u8 *pkt, int len)
{
	u16 data;
	u8 header;
	unsigned int i;
	int type = WLACC_TX_UNKNOWN;
	char tx_fwver[WIRELESS_TX_FWVER_LEN] = { 0 };
	...
	header = pkt[HWQI_PKT_HDR];
	if (header != HWQI_FSK_PKT_CMD_MSG_SIZE_5_BYTE) {
		hwlog_err("cmd is not 5 bytes.\n");
		return;
	}

	for (i = HWQI_ACK_TX_SN_E; i >= HWQI_ACK_TX_FWVER_S; i--) {
		if (i != HWQI_ACK_TX_FWVER_S)
			snprintf_s(tx_fwver + strlen(tx_fwver), WIRELESS_TX_FWVER_LEN,
				WIRELESS_TX_FWVER_LEN - strlen(tx_fwver) - 1, "0x%02x ", pkt[HWQI_PKT_DATA + i - 1]);
		else
			snprintf_s(tx_fwver + strlen(tx_fwver), WIRELESS_TX_FWVER_LEN,
				WIRELESS_TX_FWVER_LEN - strlen(tx_fwver) - 1, "0x%02x", pkt[HWQI_PKT_DATA + i - 1]);
	}

	data = (pkt[HWQI_ACK_TX_FWVER_E + 1] << POWER_BITS_PER_BYTE) |
		pkt[HWQI_ACK_TX_FWVER_E];
	type = hwqi_convert_tx_type(data);
	if ((type == WLACC_TX_UPGRADE) || (type == WLACC_TX_UPGRADE_FSK))
		power_event_bnc_notify(POWER_BNT_WLRX, POWER_NE_WLRX_TX_VERSION, tx_fwver);
	hwlog_info("[fsk_tx_version] tx_fwver:%s, tx_type:%d\n", tx_fwver, type);
}

static void hwqi_handle_fsk_tx_sn(unsigned int ic_type, u8 *pkt, int len)
{
	u8 header;
	unsigned int i;
	char tx_sn[WIRELESS_TX_SN_LEN] = { 0 };
	...
	for (i = 0; i < HWQI_ACK_TX_SN_E; i++) {
		if (i != (HWQI_ACK_TX_SN_E - HWQI_ACK_TX_SN_S))
			snprintf_s(tx_sn + strlen(tx_sn), WIRELESS_TX_SN_LEN,
				WIRELESS_TX_SN_LEN - strlen(tx_sn) - 1, "0x%02x ", pkt[HWQI_PKT_DATA + i]);
		else
			snprintf_s(tx_sn + strlen(tx_sn), WIRELESS_TX_SN_LEN,
				WIRELESS_TX_SN_LEN - strlen(tx_sn) - 1, "0x%02x", pkt[HWQI_PKT_DATA + i]);
	}

	hwlog_info("[fsk_tx_sn] tx_sn:%s\n", tx_sn);
}
...
static void hwqi_handle_fsk_packet(u8 *pkt, int len, void *dev_data)
{
	...
	case HWQI_CMD_NAK:
		...
		break;
	case HWQI_CMD_GET_TX_VERSION:
		hwlog_info("received cmd with tx version\n");
		hwqi_handle_fsk_tx_version(ic_type, pkt, len);
		break;
	case HWQI_CMD_GET_TX_SN:
		hwlog_info("received cmd with tx sn\n");
		hwqi_handle_fsk_tx_sn(ic_type, pkt, len);
		break;
	default:
		break;
	}
