...
static struct usb_ctrlrequest gadget_ctrl_request;
	...
	const struct usb_ctrlrequest *ctrl)
{
	...
	case (USB_DIR_OUT | USB_TYPE_VENDOR):
		...
		case (USB_REQ_SEND_HOST_TIME):
			 ...
			 */
			(void)memcpy_s((void *)&gadget_ctrl_request, sizeof(struct usb_ctrlrequest), (void *)ctrl,
					sizeof(struct usb_ctrlrequest));
			req->context = &gadget_ctrl_request;
	...
	case (USB_DIR_IN | USB_TYPE_VENDOR | USB_RECIP_DEVICE):
		...
		case USB_REQ_VENDOR_SWITCH_MODE:
			...
			(void)memcpy_s(req->buf, value, &state, value / 2);
			(void)memcpy_s(req->buf + value / 2, value, &mode, value / 2);
			break;
		default:
			break;
		}
...
#ifdef CONFIG_USB_HW_F_ACM_MUSL
	if (value < 0)
		value = fxfer_aoa_ctrlrequest(cdev, ctrl, false);
#endif
...
void hw_gadget_disconnect(void)
{
#ifdef CONFIG_USB_HW_F_ACM_MUSL
	fxfer_aoa_disconnect();
#endif
}
