...
{
	(void)memcpy_s(g_pcinfo_data + *offset, (PCINFO_SIZE - *offset), buf, len);
	*offset += len;
}
...
static void assemble_pcinfo(struct pcinfo_st *pcinfo)
{
	...
	(void)memset_s(g_pcinfo_data, PCINFO_SIZE, 0x0, PCINFO_SIZE);
...
static int parse_pcinfo(u16 w_value, u16 w_index, u16 w_length)
{
	...
	(void)memset_s(&pcinfo, sizeof(pcinfo), 0x0, sizeof(pcinfo));
