...
static int usb_port_switch(int usb_switch)
{
	...
	snprintf_s(event_buf, sizeof(event_buf), sizeof(event_buf) - 1,
...
void hw_usb_handle_scsi_command(unsigned char *cmd, unsigned int len)
{
	/*
	 * when rework in manufacture, if the phone is in ports mode,
	 * we need to switch it to multi-ports mode for using the diag.
	 */
	u8 cmnd[SCSI_MAX_LEN] = {0};

	/* SCSI cmd has 16 bytes at least */
	if (!cmd || (len < SCSI_MAX_LEN)) {
		pr_err("%s: invalid length %u\n", __func__, len);
		return;
	}

	cmnd[0] = SCSI_COMMAND_REWIND;
	cmnd[1] = 0x06;    /* 0x06: switch port*/
	if (memcmp(cmd, cmnd, SCSI_MAX_LEN) == 0)
		hw_usb_port_switch_request(INDEX_ENDUSER_SWITCH); /* enduser pnp switch such as pc modem */

	cmnd[9] = 0x11;    /* 0x11: switch to factory mode */
	if (memcmp(cmd, cmnd, SCSI_MAX_LEN) == 0)
		hw_usb_port_switch_request(INDEX_FACTORY_REWORK); /* manufactory rework */
}
EXPORT_SYMBOL(hw_usb_handle_scsi_command);
	...
	struct device_attribute *attr, const char *buf, size_t size)
{
	...
	if (sscanf_s(buf, "%2d", &port_mode) == 1)
		return strlen(buf);
