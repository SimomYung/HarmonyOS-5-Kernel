...
#define GET_DP_STATE_RETRY_CNT  10
#define WAIT_DP_STATE_TIME      10
#define CHG_65W_PDO_DATA_NUM    6
#define CHG_CAR_PDO_DATA_NUM    3

#define TEM_VOL_VALUE                   5000
#define TEM_CUR_VAL                     2000

/* 1.8m cable */
#define SPECIFIED_CABLE_MASK 0x1E000
#define SPECIFIED_CABLE      0x4000
...
void hw_pd_set_source_sink(enum power_supply_sinksource_type type)
{
	...
	if (pd_state(pd_support_pps) && (type == POWER_SUPPLY_STOP_SINK))
...
bool hw_pd_check_car_pdo(uint32_t *raw_pdo, size_t size)
{
	int num;
	/* M5 Extended Range Vehicle Wired Charging PDO */
	uint32_t charger_car_pdo_data[CHG_CAR_PDO_DATA_NUM] = {0x801912C, 0x2D12C, 0xC0DC2131};

	for (num = 0; num < size; num++) {
		if (raw_pdo[num] != charger_car_pdo_data[num])
			return false;
	}

	hwlog_info("%s: is specific charger, not rise voltage\n", __func__);
	return true;
}

void hw_pd_get_raw_pdo(uint32_t *raw_pdo, size_t size)
{
	bool specific_charger_flag = false;
	int num;
	uint32_t charger_65W_pdo_data[CHG_65W_PDO_DATA_NUM] = {0xA0190C8, 0x2D0C8, 0x3C0C8, 0x4B12C, 0x64145, 0xC190373C};

	if ((!raw_pdo) || ((size != CHG_65W_PDO_DATA_NUM) && (size != CHG_CAR_PDO_DATA_NUM))) {
		hwlog_info("%s: pdo data is NULL or num error:%d\n", __func__, size);
		return;
	}

	specific_charger_flag = hw_pd_check_car_pdo(raw_pdo, size);
	if (specific_charger_flag) {
		huawei_charger_recv_pd_src_cap(CHARGE_PD_SRC_CAP_MAYBE_DISABLE_9V);
		return;
	}

	for (num = 0; num < size; num++) {
		if (raw_pdo[num] != charger_65W_pdo_data[num])
			return;
	}

	hwlog_info("%s: is specific charger, need send uevent to charge\n", __func__);
	huawei_charger_recv_pd_src_cap(CHARGE_PD_SRC_CAP_CCH_COMP_MAYBE_LOW);
}
...
int hw_pd_get_source_vbus(void)
{
	if (!g_pd_di)
		return 0;

	return g_pd_di->source_vbus;
}
...
static void pd_source_vbus_ok(void *data)
{
	...
	g_pd_di->source_vbus = vbus_state->mv;
	if (vbus_state->mv == SRC_VBUS_HIGH_VOL) {
		/* enable ps_rdy delay */
#ifdef CONFIG_REVERSE_CHARGE
		if (pd_state(support_reverse_sc) &&
			hw_pd_get_reverse_charge_state())
			return;
#endif /* CONFIG_REVERSE_CHARGE */
		hw_pd_ops_set_src_psrdelay(1);
		rchg_pd_9v_enable(true);
		hwlog_info("%s: open rchg 9v\n", __func__);
	} else if (pd_state(otg_channel)) {
		hw_pd_power_set_state(PD_POWER_BOOST, 1);
		if (pd_state(wired_rchg_9v)) {
			hw_pd_send_otg_state(true);
			hwlog_info("%s:notify charger enable wired rchg\n", __func__);
		}
#ifdef CONFIG_POGO_PIN
	} else if (pogopin_is_support() && (pogopin_otg_from_buckboost())) {
...
static void pd_report_source_vbus(struct pd_dpm_info *di, void *data)
{
	...
	if (vbus_state->mv == 0) {
		...
		if (pd_state(wired_rchg_9v)) {
			hw_pd_send_otg_state(false);
			hwlog_info("%s:notify charger disable wired rchg\n", __func__);
		}
...
static void pd_report_sink_vbus(struct pd_dpm_info *di, void *data)
{
	...
	if (pd_state(pd_support_pps) && hw_pd_filter_pps_sink(di, vbus_state)) {
	...
	if (vbus_state->vbus_type & TCP_VBUS_CTRL_PD_DETECT) {
		...
		pd_state(pd_finish_flag) = true;
		pd_state(ctc_cable_flag) = true;
		if (pd_state(support_pd_certification))
...
void hw_pd_check_specified_cable(unsigned int cable_vdo)
{
#ifdef CONFIG_ADAPTER_PROTOCOL
	if ((cable_vdo & SPECIFIED_CABLE_MASK) == SPECIFIED_CABLE) {
		hwlog_info("%s is specified cable\n", __func__);
		adpt_intfr_set_cable_type(ADPT_INTRF_CABLE_TYPE_1M8);
	}
#endif
}
...
void hw_pd_set_cable_vdo(void *data)
{
	...
	hw_pd_check_specified_cable(pd_state(cable_info).cable_vdo);
}
	...
	*audio_event = hw_pd_misc_handle_analog(ANALOG_DETTACH);
	...
	if (pd_state(support_smart_holder)) {
		if (pd_state(smart_holder))
			hishow_notify_dev_uevent(HISHOW_DEVICE_OFFLINE, HISHOW_USB_DEVICE);
	...
	pd_state(bc12_event) = CHARGER_TYPE_NONE;
...
static int hw_pd_set_default_source_cap(void)
{
	u32 otg_vol = SRC_VBUS_DEFAULT_VOL;
	u32 otg_cur = SRC_VBUS_DEFAULT_CUR;

	return hw_pd_ops_update_src_cap(otg_vol, otg_cur, 0);
}

static void hw_pd_wired_rchg_cb (unsigned int type, char *event_data)
{
	if (!event_data) {
	...
	pd_wired_rchg_cb(type, event_data);
}
...
int hw_pd_handle_pe_event(unsigned long event, void *data)
{
	...
	case PD_DPM_PE_ABNORMAL_CC_CHANGE_HANDLER:
		if (pd_state(abnormal_cc_detection) &&
			...
			hw_pd_abnomal_cc_change(PD_DPM_ABNORMAL_CC_CHANGE);
		return 0;
...
#ifdef CONFIG_REVERSE_CHARGE
			/* ignore dr swap to avoid rscp conflict when in revese charging*/
			if (pd_state(support_reverse_sc) && hw_pd_get_reverse_charge_state()) {
				hwlog_info("%s reverse charge is enabled, ignore dr swap\n", __func__);
				return 0;
			}
#endif /* CONFIG_REVERSE_CHARGE */
			if (pd_dpm_support_dp() && !hw_pd_support_host_attach()) {
				while ((retry < GET_DP_STATE_RETRY_CNT) && !hw_pd_get_dp_state()) {
					retry++;
					msleep(WAIT_DP_STATE_TIME);
				}

				if (!hw_pd_get_dp_state()) {
	...
	case PD_DPM_PE_CABLE_VDO:
		if (pd_state(pd_support_pps))
			hw_pd_pps_notify_vdo();
		hw_pd_set_cable_vdo(data);
		emark_detect_complete();
		hw_pd_vbus_event_notify(PD_DPM_EMARK_FINISHED, NULL);
		break;
	default:
...
static int ignore_bc12_charge_type(unsigned long event, unsigned long *last_event, void *data)
{
	if (pd_state(pd_source_vbus)) {
		hwlog_info("%s : line %d\n", __func__, __LINE__);
		return HW_PD_INVALID_STATE;
	...
	if (!pmic_vbus_irq_is_enabled() &&
		...
		if (pd_state(force_report_port)) {
			*last_event = event;
			hw_pd_vbus_event_notify(event, data);
			hwlog_info("%s: force_report_port\n", __func__);
		}
		return HW_PD_INVALID_STATE;
	}
...
static void pd_report_bc12_cb(unsigned int type, char *event_data)
{
	...
	if ((event == CHARGER_TYPE_NONE) && !pd_state(pd_source_vbus)) {
		hwlog_info("%s : PD_WAKE_UNLOCK_DFX_DEBUG_FS \n", __func__);
		hw_pd_wakelock_ctrl(PD_WAKE_UNLOCK);
	}
#else
#ifdef CONFIG_FACTORY_MODE
	if ((event == CHARGER_TYPE_NONE) && !pd_state(pd_source_vbus)) {
		hwlog_info("%s : PD_WAKE_UNLOCK_FACTORY \n", __func__);
		hw_pd_wakelock_ctrl(PD_WAKE_UNLOCK);
	}
#else
	...
	ret = ignore_bc12_charge_type(event, &last_event, data);
	if (ret != 0)
		goto End;

	if (pd_state(slow_down_bc12)) {
		msleep(pd_state(slow_down_bc12)); // Slow down the rate at which BC12 recognizes DCP 200ms
		hwlog_info("%s : slow down %dMS\n", __func__, pd_state(slow_down_bc12));
	}

	if ((event == CHARGER_TYPE_DCP) && !pd_state(pd_finish_flag) && hw_pd_get_src_cap()) {
		...
		hw_pd_vbus_event_notify(PD_DPM_VBUS_TYPE_PD, &vbus_state);
	}
...
static void pd_init_resource(struct pd_dpm_info *di)
{
    ...
    /* Register calback function for wired_rchg notification */
	if (pd_state(wired_rchg_9v))
		hw_pd_event_reg(HW_PD_EVENT_TYPE_RCHG, NULL, hw_pd_wired_rchg_cb);

#ifdef CONFIG_REVERSE_CHARGE
	if (pd_state(support_reverse_sc))
		hw_pd_reverse_sc_init();
#endif /* CONFIG_REVERSE_CHARGE */

	if (pd_state(abnormal_cc_detection))
		hw_pd_abnormal_cc_detection_init();
}

static int pps_get_apdo_caps(struct pps_apdo_cap *caps, int num, struct pps_pdo_raw *raw)
{
	return hw_pd_pps_get_apdo_caps(caps, num, raw);
}

static int pps_set_state(enum pps_set_state state, int mv, int ma)
{
	return hw_pd_pps_set_state(state, mv, ma);
}

static int pps_get_status(int *mv, int *ma)
{
	return hw_pd_pps_get_status(mv, ma);
}

static struct hw_pd_pps_ops g_hw_pd_pps_ops = {
	.get_apdo_caps = pps_get_apdo_caps,
	.set_state = pps_set_state,
	.get_status = pps_get_status,
};
