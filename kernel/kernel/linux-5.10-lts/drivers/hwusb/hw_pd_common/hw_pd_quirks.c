...
static struct delayed_work con_work;
...
static void hw_pd_usb_con_work(struct work_struct *work)
{
	hwlog_info("[force report port]: %s enter\n", __func__);
	hw_pd_report_device_attach();
}

void hw_pd_force_report_port(void)
{
	INIT_DELAYED_WORK(&con_work, hw_pd_usb_con_work);
	schedule_delayed_work(&con_work, msecs_to_jiffies(pd_state(force_report_port) * MS_PER_SEC));
	hwlog_info("%s force report port delay_time %d s\n", __func__, pd_state(force_report_port));
}
