	...
	struct device_attribute *attr, char *buf)
{
	...
	int volt = 0;
	int cur = 0;
	int pd_dpm_typec_state = 0;
	enum power_supply_sinksource_type sink_source_type = POWER_SUPPLY_STOP_SINK;
	...
	hw_pd_get_typec_state(&pd_dpm_typec_state);
	sink_source_type = pd_dpm_get_source_sink_state();
	/*
	...
	*/
	return scnprintf(buf, PAGE_SIZE, "bc12=%d;typec=%d;pdflag=%d;snksrc=%d;pwr=%d,%d\n",
		pd_state(bc12_event), pd_dpm_typec_state, pd_state(pd_finish_flag), sink_source_type, volt, cur);
}
...
{
   int value;
   value = hw_pd_power_check();
   return scnprintf(buf, PAGE_SIZE, "%d\n", value);
}

static ssize_t usb_param_change_show(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	return 0;
}

static ssize_t usb_param_change_store(struct device *dev,
	struct device_attribute *attr, const char *buf, size_t count)
{
	if (buf == NULL) {
...
#endif /* CONFIG_REVERSE_CHARGE */
static DEVICE_ATTR(usb_param_change, S_IRWXU | S_IRWXG | S_IROTH, usb_param_change_show, usb_param_change_store);
...
#endif /* CONFIG_REVERSE_CHARGE */
	&dev_attr_usb_param_change.attr,
	NULL,
};
