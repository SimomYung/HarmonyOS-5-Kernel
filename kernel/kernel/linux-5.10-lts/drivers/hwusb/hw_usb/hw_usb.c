...
#if (defined(CONFIG_HISI_USB_STORAGE_QUIRK) || defined(CONFIG_CHIP_USB_CONFIGFS))
#include <linux/hisi/usb/chip_usb_interface.h>
#endif

#ifndef CONFIG_USB_HW_F_ACM_MUSL
#include <chipset_common/hwusb/hw_usb_input.h>
#endif
...
static uint32_t g_no_power_event;
#ifndef CONFIG_USB_HW_F_ACM_MUSL
static struct device *g_input_dev = NULL;
#endif
	...
	void *data, int result, const char *client_str)
{
	...
	if (result) {
		...
		if (ret != 0)
			hwlog_info("%s:regulator_disable,ret = %d\n", __func__, ret);
...
#ifndef CONFIG_USB_HW_F_ACM_MUSL
static void hw_usb_send_event_input(enum hw_usb_cable_event_type event)
{
	...
	if (g_input_dev) {
		switch (event) {
		case USB_INPUT_PLUG_IN :
			snprintf_s(event_buf, sizeof(event_buf), (sizeof(event_buf) - 1), "USB_INPUT_STATE=ON");
			break;
		case USB_INPUT_PLUG_OUT :
			snprintf_s(event_buf, sizeof(event_buf), (sizeof(event_buf) - 1), "USB_INPUT_STATE=OFF");
			break;
		default:
			return;
		}
	...
	ret = kobject_uevent_env(&g_input_dev->kobj, KOBJ_CHANGE, envp);
	if (ret < 0)
...
void hw_usb_send_event(enum hw_usb_cable_event_type event)
{
	...
	case USB31_CABLE_IN_EVENT:
		snprintf_s(event_buf, sizeof(event_buf), (sizeof(event_buf) - 1), "USB3_STATE=ON");
		break;
	case USB31_CABLE_OUT_EVENT:
		snprintf_s(event_buf, sizeof(event_buf), (sizeof(event_buf) - 1), "USB3_STATE=OFF");
		break;
	case DP_CABLE_IN_EVENT:
		snprintf_s(event_buf, sizeof(event_buf), (sizeof(event_buf) - 1), "DP_STATE=ON");
		break;
	case DP_CABLE_OUT_EVENT:
		snprintf_s(event_buf, sizeof(event_buf), (sizeof(event_buf) - 1), "DP_STATE=OFF");
		break;
	case ANA_AUDIO_IN_EVENT:
		snprintf_s(event_buf, sizeof(event_buf), (sizeof(event_buf) - 1), "ANA_AUDIO_STATE=ON");
		break;
	case ANA_AUDIO_OUT_EVENT:
		snprintf_s(event_buf, sizeof(event_buf), (sizeof(event_buf) - 1), "ANA_AUDIO_STATE=OFF");
		break;
#ifndef CONFIG_USB_HW_F_ACM_MUSL
	case USB_INPUT_PLUG_IN :
		snprintf_s(event_buf, sizeof(event_buf), (sizeof(event_buf) - 1), "USB_INPUT_STATE=ON");
...
#ifndef CONFIG_USB_HW_F_ACM_MUSL
static ssize_t hw_usb_input_status_show(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	return input_hw_usb_status_show(buf);
}

static ssize_t hw_usb_input_status_store(struct device *dev,
	struct device_attribute *attr, const char *buf, size_t size)
{
	return input_hw_usb_status_store(buf, size);
}

static ssize_t hw_usb_input_show(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	return input_hw_usb_show(buf);
}

static ssize_t hw_usb_input_store(struct device *dev,
	struct device_attribute *attr, const char *buf, size_t size)
{
	return input_hw_usb_store(buf, size);
}

#endif

#if defined(CONFIG_CHIP_USB_CONFIGFS)
...
#endif
#ifndef CONFIG_USB_HW_F_ACM_MUSL
static DEVICE_ATTR(usb_input, 0660, hw_usb_input_show, hw_usb_input_store);
static DEVICE_ATTR(input_status, 0664, hw_usb_input_status_show, hw_usb_input_status_store);
#endif
...
#ifndef CONFIG_USB_HW_F_ACM_MUSL
static struct attribute *hw_usb_input_attributes[] = {
	&dev_attr_usb_input.attr,
	&dev_attr_input_status.attr,
	NULL,
};

static const struct attribute_group hw_usb_input_attr_group = {
	.attrs = hw_usb_input_attributes,
};
#endif
...
#ifndef CONFIG_USB_HW_F_ACM_MUSL
static struct device *hw_usb_create_group_input(void)
{
	return power_sysfs_create_group("hw_usb", "usb", &hw_usb_input_attr_group);
}

static void hw_usb_remove_group_input(struct device *dev)
{
	power_sysfs_remove_group(dev, &hw_usb_input_attr_group);
}
#endif
...
static int hw_usb_probe(struct platform_device *pdev)
{
	...
	power_dts_read_u32(power_dts_tag(HWLOG_TAG),
		(pdev->dev).of_node, "no_power_event", &g_no_power_event, 0);
	hwlog_info("%s no_power_event=%u\n", __func__, g_no_power_event);
	if (g_no_power_event) {
#ifndef CONFIG_USB_HW_F_ACM_MUSL
		g_input_dev = hw_usb_create_group_input();
		hw_usb_input_init();
#endif
		return 0;
	}
...
#ifndef CONFIG_USB_HW_F_ACM_MUSL
		if (g_input_dev)
			hw_usb_remove_group_input(g_input_dev);
#endif
		return 0;
	}
