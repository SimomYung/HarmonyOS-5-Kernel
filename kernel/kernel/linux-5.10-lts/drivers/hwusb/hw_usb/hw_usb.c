...
static uint32_t no_power_event;
#ifndef CONFIG_USB_HW_F_ACM_MUSL
	...
	if (g_input_dev) {
		switch (event) {
			case USB_INPUT_PLUG_IN :
				snprintf(event_buf, sizeof(event_buf), "USB_INPUT_STATE=ON");
				break;
			case USB_INPUT_PLUG_OUT :
				snprintf(event_buf, sizeof(event_buf), "USB_INPUT_STATE=OFF");
				break;
			default:
				return;
		}
...
static int hw_usb_probe(struct platform_device *pdev)
{
	...
	power_dts_read_u32(power_dts_tag(HWLOG_TAG),
		(pdev->dev).of_node, "no_power_event", &no_power_event, 0);
	hwlog_info("%s no_power_event=%u\n", __func__, no_power_event);
	if (no_power_event) {
#ifndef CONFIG_USB_HW_F_ACM_MUSL
