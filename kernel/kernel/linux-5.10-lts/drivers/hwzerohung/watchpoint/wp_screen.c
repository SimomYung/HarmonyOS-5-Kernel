 ...
 *
 * Copyright (c) 2017-2021 Huawei Device Co., Ltd.
 *
...
#define POWER_OFF_IGNORE_WAIT 1
...
struct hung_wp_screen_data {
	...
	struct timer_list power_off_ignore_timer;
#if (!defined(CONFIG_KEYBOARD_GPIO_VOLUME_KEY)) && (!defined(CONFIG_KEYBOARD_PMIC_VOLUME_KEY))
...
static int g_check_off;
static int g_is_in_power_off;
...
static int g_lid_closed = 0; /* initial lid status, open */
#endif
...
void hung_wp_screen_setbl(int level)
{
	...
	if (level == 0) {
		g_data.power_off_ignore_timer.expires = jiffies +
				msecs_to_jiffies(POWER_OFF_IGNORE_WAIT * TIME_CONVERT_UNIT);
		if (!timer_pending(&g_data.power_off_ignore_timer)) {
			add_timer(&g_data.power_off_ignore_timer);
			g_is_in_power_off = 1;
			pr_info("%s: start one sec, level=%d\n", __func__, g_data.bl_level);
		}
	}
	spin_unlock_irqrestore(&(g_data.lock), flags);
}
...
#ifdef CONFIG_ARMPC_ECHUB_LID
/* Called from echub when lid status is changed */
void hung_wp_screen_set_lid_status(int status)
{
	g_lid_closed = status;
}
#endif
...
#if (KERNEL_VERSION(4, 15, 0) <= LINUX_VERSION_CODE)
static void hung_wp_power_off_ignore(struct timer_list *t)
#else
static void hung_wp_power_off_ignore(uintptr_t pdata)
#endif
{
	pr_info("%s: ignore poweroff end\n", __func__);
	del_timer(&g_data.power_off_ignore_timer);
	g_is_in_power_off = 0;
}
...
void hung_wp_screen_powerkey_ncb(unsigned long event)
{
...
#ifdef CONFIG_ARMPC_ECHUB_LID
	if (g_lid_closed != 0) {
		pr_err("%s: lid is closed, skip check!", __func__);
		return;
	}
#endif	
#ifdef CONFIG_KEYBOARD_PMIC_VOLUME_KEY
	...
	if (event == WP_SCREEN_PWK_PRESS ||
		...
		g_check_off = 0;
		if ((g_data.bl_level == 0) && (g_is_in_power_off == 0)) {
			pr_info("%s: start screen on\n", __func__);
...
#if (KERNEL_VERSION(4, 15, 0) <= LINUX_VERSION_CODE)
			check_off_point = &g_check_off;
#else
			g_data.long_press_timer.data = (unsigned long)&g_check_off;
#endif
...
static int __init hung_wp_screen_init(void)
{
	...
	timer_setup(&g_data.power_off_ignore_timer, hung_wp_power_off_ignore, 0);
#else
	...
	init_timer(&g_data.power_off_ignore_timer);
	g_data.power_off_ignore_timer.function = hung_wp_power_off_ignore;
	g_data.power_off_ignore_timer.data = 0;
#endif
#if (!defined(CONFIG_KEYBOARD_GPIO_VOLUME_KEY)) && (!defined(CONFIG_KEYBOARD_PMIC_VOLUME_KEY))
#if (KERNEL_VERSION(4, 15, 0) <= LINUX_VERSION_CODE)
