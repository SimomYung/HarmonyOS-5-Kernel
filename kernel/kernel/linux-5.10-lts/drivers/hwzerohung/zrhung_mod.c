...
static void hook_hung_wp_screen_set_lid_status(void *ignore, int status)
{
	hung_wp_screen_set_lid_status(status);
}
#endif
...
#ifdef CONFIG_ARMPC_ECHUB_LID
	REGISTER_HCK_VNH(hung_wp_screen_set_lid_status, hook_hung_wp_screen_set_lid_status, NULL);
#endif	
#if (!defined(CONFIG_KEYBOARD_GPIO_VOLUME_KEY)) && (!defined(CONFIG_KEYBOARD_PMIC_VOLUME_KEY))
