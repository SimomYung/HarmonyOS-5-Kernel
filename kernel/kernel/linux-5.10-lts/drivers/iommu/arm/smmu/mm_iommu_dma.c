				...
				unsigned long attrs)
{
	...
	if (!coherent && (attrs & DMA_ATTR_SKIP_CPU_SYNC) == 0)
		...
		liblinux_dma_map_area(dev, phys_to_virt(paddr + offset), size, dir);
#else
...
static int mm_iommu_dma_map_page_batch(struct device *dev, struct scatterlist *sg,
	int count, enum dma_data_direction dir, unsigned long attrs)
{
	int mapped_count;
	bool coherent = dev_is_dma_coherent_common(dev);
	int prot = mm_dma_info_to_prot(dir, coherent, attrs);
	bool is_dcache_flush = (dir == DMA_FROM_DEVICE && dev->user_inval_cache);

	mapped_count = mm_iommu_map_page_batch(dev, sg, prot, coherent, is_dcache_flush);
	if (!mapped_count) {
		dev_err(dev, "%s map_batch:nelems:%d,dir:%d,attrs:%lx\n",
			__func__, count, dir, attrs);
	}

	return mapped_count;
}

static void mm_iommu_dma_unmap_page_batch(struct device *dev, struct scatterlist *sgl,
	int count, enum dma_data_direction dir, unsigned long attrs)
{
	int ret;

	ret = mm_iommu_unmap_page_batch(dev, sgl, dev_is_dma_coherent_common(dev),
		(dir == DMA_TO_DEVICE));
	if (ret)
		dev_err(dev, "%s:sg:%pK,nelems:%d,dir:%d,attrs:%lx,ret:%d\n",
			__func__, sgl, count, dir, attrs, ret);
}
#endif
...
#ifdef CONFIG_DMA_MAP_BATCH
	.map_page_batch = mm_iommu_dma_map_page_batch,
	.unmap_page_batch = mm_iommu_dma_unmap_page_batch,
#endif
