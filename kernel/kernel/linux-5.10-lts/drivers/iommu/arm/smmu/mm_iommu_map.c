...
#ifdef CONFIG_DMABUF_RECLAIM
static struct iova_dom *mm_iommu_get_iova_dom(struct iommu_domain *domain, unsigned long iova);
#endif
...
struct gen_pool *iova_pool_setup(unsigned long start, unsigned long size,
				 unsigned long align, unsigned long granularity)
{
	...
	if (granularity != 0) {
		for (i = 0; i < size / SZ_4G; ++i) {
			ret = gen_pool_add(pool, start, granularity, -1);
			if (ret) {
				pr_err("gen pool add by granularity failed!\n");
				...
				struct iova_dom *iova_dom)
{
	...
	while (*p) {
		...
		if (iova_dom->iova < entry->iova) {
			p = &(*p)->rb_left;
		} else if (iova_dom->iova > entry->iova) {
			p = &(*p)->rb_right;
		} else {
				...
				unsigned long *out_size)
{
	...
	if ((iova / IOVA_ALGINED_BOUNDARY) != ((iova + iova_size) / IOVA_ALGINED_BOUNDARY))
		dev_err(dev, "smmu iova_addr(0x%lx) size(0x%zx), not aligned with 4G\n", iova, iova_size);
...
#ifdef CONFIG_DMABUF_RECLAIM
static bool is_iova_domain_reclaim(struct iommu_domain *domain,
			unsigned long iova, bool *is_down_read, struct dma_buf **buffer)
{
	struct iova_dom *iova_dom = NULL;
	struct dma_buf *dmabuf = NULL;

	iova_dom = mm_iommu_get_iova_dom(domain, iova);
	if (!iova_dom)
		return false;

	dmabuf = (struct dma_buf *)iova_dom->key;
	if (!is_direct_reclaim_dmabuf(dmabuf))
		return false;

	*buffer = dmabuf;
	*is_down_read = buffer_reclaim_down_read(dmabuf);

	return buffer_reclaim_status_get(dmabuf->priv);
}
#endif
...
#ifdef CONFIG_DMABUF_RECLAIM
	if (is_iova_domain_reclaim(domain, iova, &is_down_read, &dmabuf))
		goto done;
#endif
#if (LINUX_VERSION_CODE < KERNEL_VERSION(5, 10, 0))
...
#ifdef CONFIG_DMABUF_RECLAIM
int mm_iommu_reclaim_flush_tlb(struct device *dev, struct iommu_domain *domain, unsigned long iova, size_t size)
{
	struct iommu_fwspec *fwspec = NULL;
	...
	if (!fwspec->ops->reclaim_flush_tlb) {
		dev_err(dev, "%s ops reclaim_flush_tlb is null\n", __func__);
		return 0;
	}

	fwspec->ops->reclaim_flush_tlb(dev, domain, iova, size);

	return 0;
}
#endif

void mm_iommu_flush_ttwc(struct device *dev, struct iommu_domain *domain)
{
	if (!dev || !domain) {
		pr_err("%s input dev or domain err!\n", __func__);
        return;
    }
					...
					struct sg_table *table)
{
	...
	if (dmabuf->from_udk)
		prot = ((unsigned int)prot | dma_buf_call_helper_mk_prot(dmabuf));
	else
#endif
					     ...
					     struct sg_table *sgt)
{
	...
	*out_size = iova_dom->size;
	iova_dom->dev = dev;
	iova_dom->prot = prot | (IOMMU_READ | IOMMU_WRITE);
	...
	if (is_dma_heap_dmabuf(dmabuf))
		record_buffer_iova_info(dmabuf->priv, dev, iova, iova_dom->size, prot);
#endif
	return iova;
...
#ifdef CONFIG_DMABUF_RECLAIM
static unsigned long __mm_iommu_map_reclaim_dmabuf(struct device *dev,
	struct dma_buf *dmabuf, int prot, unsigned long *out_size, struct sg_table *sgt)
{
	unsigned long iova;
	bool is_down_read = buffer_reclaim_down_read(dmabuf);

	if (buffer_reclaim_status_get(dmabuf->priv)) {
		if (is_down_read)
			buffer_reclaim_up_read(dmabuf);
		dev_err(dev, "%s buffer has been already reclaim\n", __func__);
		return 0;
	}
...
#elif defined(CONFIG_DMABUF_RECLAIM)
	if (is_direct_reclaim_dmabuf(dmabuf))
		return __mm_iommu_map_reclaim_dmabuf(dev, dmabuf, prot, out_size, NULL);
	else
		return __mm_iommu_map_dmabuf(dev, dmabuf, prot, out_size, NULL);
#else
	return __mm_iommu_map_dmabuf(dev, dmabuf, prot, out_size, NULL);
...
#ifdef CONFIG_DMABUF_RECLAIM
	if (is_dma_heap_dmabuf(dmabuf))
		delete_buffer_iova_info(dmabuf->priv, dev, iova, true);
#endif
	kfree(iova_dom);
	return ret;
}
...
static struct iova_dom *mm_iommu_get_iova_dom(struct iommu_domain *domain, unsigned long iova)
{
	...
	cookie = domain->iova_cookie;
	if (!cookie) {
		pr_err("%s, iova cookie is null!\n", __func__);
		return NULL;
	}
	spin_lock(&cookie->iova_lock);
	rbroot = &cookie->iova_root;
	node = rbroot->rb_node;
	while (node) {
		iova_dom = rb_entry(node, struct iova_dom, node);
		if ((iova_dom->iova <= iova) && (iova < iova_dom->iova + iova_dom->size)) {
			spin_unlock(&cookie->iova_lock);
			return iova_dom;
		}
		else if (iova_dom->iova < iova)
			node = node->rb_right;
		else
			node = node->rb_left;
	}
...
int mm_iommu_handle_translation_fault(struct device *dev, unsigned long iova)
{
	struct iova_dom *iova_dom = NULL;
	struct dma_buf *dmabuf = NULL;
	struct iommu_domain *domain = NULL;
	...
	iova_dom = mm_iommu_get_iova_dom(domain, iova);
	if (!iova_dom) {
		pr_err("%s, iova[0x%lx] can not find domain\n", __func__, iova);
		return -EINVAL;
	}

	dmabuf = (struct dma_buf *)iova_dom->key;
	if (IS_ERR_OR_NULL(dmabuf)) {
		pr_err("%s, iova[0x%lx] can not find dmabuf\n", __func__, iova);
		return -EINVAL;
	}

	offset = (iova - iova_dom->iova) >> PAGE_SHIFT;
	ret = iommu_handle_translation_fault(domain, (void *)dmabuf);
	if (ret)
		pr_err("%s, map fail! iova[0x%lx], offset[0x%lx]\n",
			__func__, iova, offset);
	else
		mm_iommu_flush_ttwc(dev, domain);

	return ret;
}
EXPORT_SYMBOL(mm_iommu_handle_translation_fault);
#endif

#ifdef CONFIG_DMABUF_RECLAIM
int mm_iommu_map_dmabuf_page(struct device *dev, struct dma_buf *dmabuf, unsigned long iova,
			unsigned long size, int prot)
{
	...
	if (!dev || !dmabuf) {
		pr_err("%s, input Err! dev[%pK], dmabuf[%pK]\n", __func__, dev, dmabuf);
		return -EINVAL;
	}
	...
	sgt = buffer_sgtable_get(dmabuf->priv);
	if (!sgt) {
		dev_err(dev, "buffer_sgtable_get fail!\n");
		return -EINVAL;
	}

	prot |= IOMMU_READ | IOMMU_WRITE;
	map_size = iommu_map_sg(domain, iova, sgt->sgl, sgt->nents, prot);
	if (map_size != size) {
		pr_err("%s, map fail! iova[0x%lx], size[0x%lx], map_size[0x%lx]\n",	__func__, iova, size, map_size);
		return -EINVAL;
	}

	return 0;
}
EXPORT_SYMBOL(mm_iommu_map_dmabuf_page);

int mm_iommu_unmap_dmabuf_page(struct device *dev, unsigned long iova, unsigned long iova_size)
{
	struct iommu_domain *domain = NULL;
	unsigned long unmap_size = 0;

	if (!dev || !iova) {
		pr_err("%s, input err! dev[%pK], iova[0x%lx]\n",
			__func__, dev, iova);
		return -EINVAL;
	}
	...
	unmap_size = iommu_unmap(domain, iova, iova_size);
	if (unmap_size != iova_size) {
		pr_err("%s, Dev(%s) unmap fail! iova[0x%lx], size[0x%lx], unmap_size[0x%lx]",
			__func__, dev_name(dev), iova, iova_size, unmap_size);
		return -EINVAL;
	}

	mm_iommu_reclaim_flush_tlb(dev, domain, iova, iova_size);

	return 0;
}
EXPORT_SYMBOL(mm_iommu_unmap_dmabuf_page);
...
unsigned long mm_iommu_identity_map(struct device *dev,  phys_addr_t paddr,
		size_t size, int prot)
{
	struct iommu_domain *domain = NULL;
	...
	align_paddr = ALIGN_DOWN(paddr, PAGE_SIZE);
	offset = paddr - align_paddr;
	iova = (unsigned long)align_paddr;
	iova_size = PAGE_ALIGN(size + offset);
	prot |= IOMMU_READ | IOMMU_WRITE;

	if (prot & NO_SLEEP) {
		prot &= ~NO_SLEEP;
		ret = iommu_map_atomic(domain, iova, align_paddr, iova_size, prot);
	} else {
		ret = iommu_map(domain, iova, align_paddr, iova_size, prot);
	}

	if (ret) {
		pr_err("map fail! address 0x%llx,  size 0x%zx\n", align_paddr, iova_size);
		return 0;
	} else {
		mm_iommu_flush_ttwc(dev, domain);
	}
...
EXPORT_SYMBOL(mm_iommu_identity_map);

int mm_iommu_identity_unmap(struct device *dev, unsigned long iova, size_t size)
{
	struct iommu_domain *domain = NULL;
	unsigned long align_iova;
	size_t offset;
	size_t iova_size;
	size_t unmap_size;
	...
	mm_iommu_flush_tlb(dev, domain);
	align_iova = ALIGN_DOWN(iova, PAGE_SIZE);
	offset = iova - align_iova;
	iova_size = PAGE_ALIGN(size + offset);

	unmap_size = iommu_unmap(domain, align_iova, iova_size);
	if (unmap_size != iova_size) {
		pr_err("unmap fail! size 0x%lx, unmap_size 0x%zx\n", size, unmap_size);
		return -EINVAL;
	}
...
EXPORT_SYMBOL(mm_iommu_identity_unmap);

#ifdef CONFIG_DMA_MAP_BATCH
static int mm_iommu_map_batch_alloc_iova(struct device *dev, struct iommu_domain *domain,
	struct mm_dom_cookie *cookie, struct smmu_map_batch_paras *paras,
	struct scatterlist *sgl)
{
	...
	iova_pool = cookie->iova_pool;
	if (!iova_pool) {
		pr_err("smmu iommu domain iova pool is null\n");
		return -EINVAL;
	}

	for_each_sg(sgl, sg, paras->count, i) {
		paras->length[i] = ALIGN(sg->length + sg->offset, PAGE_SIZE);
		paras->phys[i] = page_to_phys(sg_page(sg));
		if (!paras->length[i] || !paras->phys[i]) {
			pr_err("smmu size is %u! addr=0x%lx\n", paras->length[i], paras->phys[i]);
			break;
		}
		paras->iova[i] = mm_iommu_alloc_iova(iova_pool, paras->length[i],
			cookie->iova.iova_align);
		if (!paras->iova[i] && (cookie->iova.iova_free == LAZY_FREE)) {
            paras->iova[i] = mm_iommu_realloc_iova(dev, domain, paras->length[i]);
			dev_err(dev, "smmu %s iova 0x%lx size 0x%u %d addr=0x%lx\n",
				__func__, paras->iova[i], paras->length[i], i, paras->phys[i]);
		}
		if (!paras->iova[i]) {
			dev_err(dev, "smmu %s alloc iova failed! size 0x%u %d addr=0x%lx\n",
				__func__, paras->length[i], i, paras->phys[i]);
			break;
		}
		sg->dma_address = paras->iova[i] + sg->offset;
	}
	if (i < paras->count) {
		for_each_sg(sgl, sg, i, j) {
			mm_iommu_free_iova(iova_pool, paras->iova[j], paras->length[j]);
		}
		return -EINVAL;
	}
	return 0;
...
int mm_iommu_map_page_batch(struct device *dev, struct scatterlist *sgl, int prot,
	bool coherent, bool is_dcache_flush)
{
	struct iommu_domain *domain = NULL;
	struct mm_dom_cookie *cookie = NULL;
	struct smmu_map_batch_paras paras = {};
	int i;
	struct scatterlist *sg = NULL;

	if (!dev || !sgl) {
		pr_err("smmu dev %pK, sgl %pK is null\n", dev, sgl);
		return 0;
	}

	domain = iommu_get_domain_for_dev(dev);
	if (!domain) {
		pr_err("smmu Dev has no iommu domain!\n");
		return 0;
	}

	cookie = domain->iova_cookie;
	if (!cookie) {
		dev_err(dev, "smmu %s, cookie %p!\n", __func__, cookie);
		return 0;
	}

	paras.count = sg_nents(sgl);
	if (paras.count > MAX_SMMU_MAP_PAGE_BATCH_NUMS || paras.count == 0) {
		pr_err("smmu sg count %d invalid\n", paras.count);
		return 0;
	}

	if (mm_iommu_map_batch_alloc_iova(dev, domain, cookie, &paras, sgl))
		return 0;
	paras.sid = to_smmu_domain(domain)->sid;
	paras.is_coherent = coherent;
	paras.is_dcache_flush = is_dcache_flush;

	if (!iommu_map_batch(domain, &paras, (prot | IOMMU_READ | IOMMU_WRITE))) {
		for_each_sg(sgl, sg, paras.count, i) {
			mm_iommu_free_iova(cookie->iova_pool, paras.iova[i], paras.length[i]);
		}
		return 0;
	}
	return paras.count;
}
EXPORT_SYMBOL(mm_iommu_map_page_batch);

static int mm_iommu_unmap_batch_init_paras(struct smmu_unmap_batch_paras *paras,
	struct scatterlist *sgl, struct device *dev)
{
	struct scatterlist *sg = NULL;
	unsigned long sg_iova;
	int i;
	unsigned int offset;

	paras->count = sg_nents(sgl);
	if (paras->count > MAX_SMMU_MAP_PAGE_BATCH_NUMS || paras->count == 0) {
		pr_err("smmu %s, sg count %d invalid\n", __func__, paras->count);
		return -EINVAL;
	}

	for_each_sg(sgl, sg, paras->count, i) {
		sg_iova = sg->dma_address;
		paras->iova[i] = ALIGN_DOWN(sg_iova, PAGE_SIZE);
		offset = sg_iova - paras->iova[i];
		paras->length[i] = ALIGN(sg->length + offset, PAGE_SIZE);
		if (!paras->length[i] || !paras->iova[i]) {
			pr_err("smmu %s, input err! dev %pK, iova 0x%lx, size is %x\n",
				__func__, dev, paras->iova[i], paras->length[i]);
			return -EINVAL;
		}
	}
	return 0;
...
static void mm_iommu_unmap_batch_free_iova(struct smmu_unmap_batch_paras *paras,
	struct device *dev, struct mm_dom_cookie *cookie, struct iommu_domain *domain)
{
	int i;
	if (cookie->iova.iova_free == LAZY_FREE) {
		for (i = 0; i < paras->count; i++) {
			mm_iommu_lazy_free_iova(cookie, paras->iova[i], paras->length[i]);
		}
		return;
	}
	mm_iommu_flush_tlb(dev, domain);
...
int mm_iommu_unmap_page_batch(struct device *dev, struct scatterlist *sgl,
	bool is_coherent, bool is_dma_2_dev)
{
	struct iommu_domain *domain = NULL;
	struct mm_dom_cookie *cookie = NULL;
	struct smmu_unmap_batch_paras paras = {};

	if (!dev || !sgl) {
		pr_err("dev %pK, sgl %pK is null\n", dev, sgl);
		return -EINVAL;
	}
	...
	cookie = domain->iova_cookie;
	if (!cookie) {
		dev_err(dev, "smmu %s, iommu domain cookie is null!\n", __func__);
		return -EINVAL;
	}

	if (!cookie->iova_pool) {
		dev_err(dev, "smmu %s, iova_pool %p!\n", __func__, cookie->iova_pool);
		return -EINVAL;
	}

	if (mm_iommu_unmap_batch_init_paras(&paras, sgl, dev)) {
		return -EINVAL;
	}
	/*
	 * flush tlb to prevent the problem that the tlb is hit
	...
	paras.sid = to_smmu_domain(domain)->sid;
	paras.is_coherent = is_coherent;
	paras.is_dma_2_dev = is_dma_2_dev;

	if (!iommu_unmap_batch(domain, &paras)) {
		return -EINVAL;
	}
	mm_iommu_unmap_batch_free_iova(&paras, dev, cookie, domain);
	return 0;
}
EXPORT_SYMBOL(mm_iommu_unmap_page_batch);
#endif /* CONFIG_DMA_MAP_BATCH */
...
#if defined(CONFIG_MM_IOMMU_TEST) && defined(CONFIG_MM_SMMU_V3)
void mm_smmuv3_show_pte(struct device *dev, unsigned long iova,
				unsigned long size)
{
	phys_addr_t ttbr = 0;
	pgd_t *pgdp = NULL;
	...
	ttbr = kernel_domain_get_ttbr(dev);
	if (!ttbr) {
		pr_err("ttbr 0!\n");
		return;
	}

	pgdp = (pgd_t *)phys_to_virt(ttbr);
	if (!pgdp) {
		pr_err("pgdp NULL!\n");
		return;
	}

	smmu = test_arm_smmu_get_dev_smmu(dev);
	if (smmu)
		test_arm_smmu_dump_pgtbl(smmu, pgdp, iova, size, NULL, 0);
}
#endif
...
#ifdef CONFIG_DMABUF_RECLAIM
	if (is_dma_heap_dmabuf(dmabuf))
		delete_buffer_iova_info(dmabuf->priv, NULL, iova, true);
#endif
	kfree(iova_dom);
}
