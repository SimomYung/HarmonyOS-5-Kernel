 ...
 * SMMU_MEDIA1 & SMMU_MEDIA2 used in driver and can't modify casually
 */
enum smmu_type_id {
	SMMU_MEDIA1,
	SMMU_MEDIA2,
...
#else
static inline bool mm_get_smmu_init_info(enum smmu_type_id smmuid, struct smmu_init_info *info)
{
	unused(smmuid);
	unused(info);
	return false;
...
#else
static inline void test_arm_smmu_dump_pgtbl(struct arm_smmu_device *smmu, pgd_t *base_pgd, unsigned long va,
				size_t pgtbl_size, u64 *dst, size_t buf_size)
{
	return;
}

static inline struct arm_smmu_device *test_arm_smmu_get_dev_smmu(struct device *dev)
{
	return NULL;
}
#endif
