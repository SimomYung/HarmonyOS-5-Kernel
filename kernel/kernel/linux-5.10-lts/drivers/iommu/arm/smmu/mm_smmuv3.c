...
static int mm_svm_flag_set(u32 pid)
{
	struct list_head *pos = NULL;
...
static int mm_svm_flag_unset(struct mm_svm *svm)
{
	struct list_head *pos = NULL;
	struct list_head *tmp = NULL;
	struct svm_pid *svm_pid_ent = NULL;
	...
	mutex_unlock(&svm_pid_list_lock);
	mm_svm_print(SVM_INFO, "%s success, pid %d\n",__func__, svm->pid);
	return 0;
}
#else
static int mm_svm_flag_unset(struct mm_svm *svm)
{
	...
	clear_bit(MMF_SVM, &svm->mm->flags);
				...
				u32 pid, u16 ssid)
{
	...
	list_for_each(pos, &mm_svm_list) {
		...
		if ((u32)tmp_svm->pid == pid || cfg->cd.ssid == ssid) {
			mm_svm_print(SVM_ERROR, "%s: pre svm also bind this mm struct! pid : %d, ssid: %d\n",
				     __func__, tmp_svm->pid, cfg->cd.ssid);
			goto out;
		}
	}

	if (mm_svm_flag_set(pid) != 0) {
...
void mm_svm_unbind_task(struct mm_svm *svm)
{
	...
	ret = mm_svm_flag_unset(svm);
	if (ret != 0) {
		mm_svm_print(SVM_ERROR, "%s svm flag unset failed\n", __func__);
		return;
	}
...
int mm_svm_flush_cache(u32 pid, unsigned long addr, size_t size)
{
	...
	if (WARN(!svm, "do not find svm device"))
		return -ENXIO;

	liblinux_pal_flush_pgtbl_cache((current->tgid == pid ? 0 : pid), addr, (addr + size));
	arm_smmu_svm_tlb_inv_context(svm); /* flush tlb when mmap */
