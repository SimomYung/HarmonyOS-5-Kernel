...
static struct fault_impl *args2fault(int args, char *argv[])
{
	...
	if (type == FAULT_NONE)
		return NULL;
	if (type >= FAULT_READ_DELAY && args >= 2) { /* argv at least 2 */
...
int fail_req_err(struct mmc_request *mrq)
{
	...
	if (fault) {
		if ((read && fault->type == FAULT_READ_FAILED) ||
			...
			data->bytes_xfered =
			(prandom_u32() % (data->bytes_xfered >> 9)) << 9; /* eMMC sector size is 512(2^9) bytes. */
			return 1;
		}
	}
...
static int rasprobe_handler_entry(mmc_request_done)
	...
	if ((read_delay_noblk && (data->flags & MMC_DATA_READ)) ||
	    (write_delay_noblk && (data->flags & MMC_DATA_WRITE))) {
		/* delay noblocked */
...
static int tool_init(void)
{
	...
	data_done_fun = (replace_fun)kallsyms_lookup_name("mmc_wait_for_req_done");
