...
static int check_conflict_record_offset(struct fault_impl *fault)
{
	...
	if (!area_node)
		return -EINVAL;
	(void)memcpy_s(&(area_node->fault), sizeof(struct fault_impl), fault, sizeof(struct fault_impl));
...
static int args2fault(int args, char *argv[])
{
	...
	(void)memset_s(&fault_args, sizeof(struct fault_impl), 0, sizeof(struct fault_impl));
			       ...
			       unsigned int length)
{
	...
	if (fault->type == FAULT_ALL_0) {
		(void)memset_s(buf + fault->offset, fault->size, 0x0, fault->size);
	} else if (fault->type == FAULT_ALL_1) {
		(void)memset_s(buf + fault->offset, fault->size, 0xff, fault->size);
	} else {
				  ...
				  block_t *relative_index, unsigned int *size)
{
	...
	if (disk_index >= cp_blkaddr && disk_index < cp_blkaddr + (cp_blocks >> 1)) {
		...
		*size = sizeof(struct f2fs_summary_block);
		...
		pr_err("%s: can't find area by block=%d\n", __func__, (int)disk_index);
		return -EINVAL;
	}
...
static int debug = 1;
module_param(debug, int, 0444); /* 0444 S_IRUGO */
