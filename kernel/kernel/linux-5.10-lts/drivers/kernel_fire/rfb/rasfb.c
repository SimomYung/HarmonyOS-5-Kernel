...
struct fault_list {
...
#ifdef CONFIG_DFX_LIBLINUX
	const struct fb_ops *fbops[FB_MAX];
	...
	struct fb_ops replace_fbops[FB_MAX];
#endif /* CONFIG_DFX_LIBLINUX */
...
#elif defined(CONFIG_DFX_LIBLINUX)
			fault_injected.fbinfo[i]->fbops = fault_injected.fbops[fault_injected.res[i]];
#endif
		}
	}
	...
	if (fault_args->type != FAULT_FRAME_FAILED) {
		...
		if (fault_args->input_type != FAULT_FRAME_DELAY) {
			fault_args->delay =
			    ...
			    ((fault_args->lost * RFB_FRAME_RATE_USEC) / 1000); /* 1ms = 1000us */
			fault_args->udelay =
			    (fault_args->lost * RFB_FRAME_RATE_USEC % 1000); /* 1ms = 1000us */
		} else {
			fault_args->lost = 1;
...
static void rfb_ifdebug(const char *format, ...)
{
	va_list args;
	char buf[RFB_IF_DEBUG_BUF_SIZE];

	if (rfb_dbg) {
		va_start(args, format);
		if (vsprintf_s(buf, sizeof(buf), format, args) < 0)
			ras_warn("vsnprintf failed");
		va_end(args);
		pr_err("%s", buf);
	}
}
	...
	for (i = 0; i < FB_MAX; i++) {
		...
		for (j = 0; j < ARRAY_SIZE(fault_injected.fbops); j++) {
			...
			if (fault_injected.fbops[j] != NULL &&
			    registered_fb[i]->fbops == fault_injected.fbops[j]) {
				bFind = 1;
				fault_injected.fb_ioctl[i] = fault_injected.fb_ioctl[fault_injected.res[j]];
				rfb_ifdebug("rfb_dbg:i=%d,j=%d,%d,%p,node=%d\n",
		...
		if (registered_fb[i]->fbops->fb_ioctl) {
			rfb_ifdebug("rfb_dbg:%s,i=%d,%d,%p-%p,%d\n",
				    __func__, i, index, rasfb_ioctl,
			    registered_fb[i]->fbops->fb_ioctl,
			    registered_fb[i]->node);
#if (KERNEL_VERSION(5, 10, 0) > LINUX_VERSION_CODE) || defined(CONFIG_DFX_LIBLINUX)
			fault_injected.fb_ioctl[i] = registered_fb[i]->fbops->fb_ioctl;
			fault_injected.fbops[index] = registered_fb[i]->fbops;
			fault_injected.res[index] = i;
#ifdef CONFIG_DFX_LIBLINUX
			fault_injected.fbinfo[index] = registered_fb[i];
			(void)memcpy_s(&fault_injected.replace_fbops[index], sizeof(struct fb_ops),
				registered_fb[i]->fbops, sizeof(struct fb_ops));
			fault_injected.replace_fbops[index].fb_ioctl = rasfb_ioctl;

			registered_fb[i]->fbops = &fault_injected.replace_fbops[index];
#else /* CONFIG_DFX_LIBLINUX */
			registered_fb[i]->fbops->fb_ioctl = rasfb_ioctl;
#endif /* CONFIG_DFX_LIBLINUX */
#endif /* (KERNEL_VERSION(5, 10, 0) > LINUX_VERSION_CODE) || defined(CONFIG_DFX_LIBLINUX) */
		}
	}
	write_unlock(&fault_injected.rwk);
}
...
static struct fault_impl *args2fault(int args, char *argv[])
{
	...
	(void)memset_s(&fault_args, sizeof(fault_args), 0, sizeof(fault_args));
	...
	if (fault_cur)
		(void)memcpy_s(fault_cur, sizeof(struct fault_impl), &fault_args, sizeof(struct fault_impl));
	write_unlock(&fault_injected.rwk);
	...
	if (fault) {
		if (fault->input_type == FAULT_FRAME_LOST_ASC) {
			delay =
			    ...
			    (fault->frame * RFB_FRAME_RATE_USEC) / 1000; /* 1ms = 1000us */
			udelay = (fault->frame * RFB_FRAME_RATE_USEC) % 1000; /* 1ms = 1000us */
