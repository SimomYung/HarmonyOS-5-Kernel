...
static loff_t inject_time;
static void *g_fileaddress;
...
static int handle_restore_func(void *data)
{
	...
	ras_sleep(ci->size * 1000); /* 1000ms */
	write_lock(&chfil.rwk);
	(void)memset_s(ci, sizeof(struct ChfilImpl), 0, sizeof(struct ChfilImpl));
...
static int handle_restore(struct ChfilImpl *ci)
{
	...
	inject_time = ras_ms();
	inject_time = inject_time + ci->size * millisecond;
#endif
	return 0;
}
			      ...
			      struct dentry *den, int ct)
{
	...
	for (i = 0; i < len; i++) {
		if (!check_used(0, &chf->impl[i])) {
			if (ci == NULL)
				ci = &chf->impl[i];
		} else if (ct == CHFILETYPE_HUNMOUNTALL &&
			   ct == chf->impl[i].ct) {
			ci = &chf->impl[i];
			break;
	...
	case CHFILETYPE_UNMOUNT:
		ras_retn_if(ras_malloc((void **)&ci->path, strlen(name) + 1), -ENOMEM);
		ras_retn_iferr(strcpy_s(ci->path, strlen(name) + 1, name));
		break;
...
static inline int chfilimpl_dtor(struct ChfilImpl *impl)
{
	...
	if (impl->path)
		ras_free(impl->path);
	(void)memset_s(impl, sizeof(struct ChfilImpl), 0, sizeof(struct ChfilImpl));
	return 0;
}
	...
	const char *what, enum ChfilType why)
{
	...
	logger_unblocked(who->pid, who->comm,
			 path, what,
			 type2name(why));
#else /* just printk */
...
static unsigned long long chfil_injected_size(struct super_block *sb, int mode)
{
	...
	for (i = 0, len = ARRAY_SIZE(chfil.impl); i < len; i++) {
		if (chfil.impl[i].den == 0 ||
			...
			continue;
		if (chfil.impl[i].ct == CHFILETYPE_FULL &&
...
static unsigned long long get_block_free_size(struct super_block *sb)
{
	...
	if (sb == 0)
		return 0;
	(void)memset_s(&fs_state, sizeof(struct kstatfs), 0, sizeof(struct kstatfs));
...
{
	if (impl->ct == CHFILETYPE_MISSED)
		ras_retn(chfilimpl_geturn(impl));
	return 0;
}

#if KERNEL_VERSION(4, 1, 0) > LINUX_VERSION_CODE || KERNEL_VERSION(5, 10, 90) < LINUX_VERSION_CODE
...
{
	if (impl->ct == CHFILETYPE_UNWRITE ||
	    impl->ct == CHFILETYPE_UNRW ||
...
static long long chfil_rebuild(void *data, const struct ChfilImpl *impl)
{
	if (impl->ct == CHFILETYPE_ZERO || impl->ct == CHFILETYPE_MISSED ||
		...
		(void)memset_s((void *)impl, sizeof(struct ChfilImpl), 0x00, sizeof(struct ChfilImpl));
		return 1;
	}
	return 0;
}
...
static int rasprobe_handler(rw_verify_area) (struct rasprobe_instance *ri,
	...
	if (WRITE == (long)rd->args[0]) {
		...
		if (size != 0 && sb->s_op->statfs) {
			struct kstatfs buf;

			(void)memset_s(&buf, sizeof(buf), 0, sizeof(buf));
			if (sb->s_op->statfs(sb->s_root, &buf) == 0 &&
			    size >= buf.f_bfree * buf.f_bsize) {
				rasprobe_seturn(regs, -ENOSPC);
				return 0;
			}
		}
		if (pos == 0) { /* record write from head */
		...
		if (pos == 0) /* record read from head */
...
{
	if (impl->ct == CHFILETYPE_UNRENAME ||
	    impl->ct == CHFILETYPE_READONLY) {
		ras_retn(chfilimpl_geturn(impl));
	}
	return 0;
...
static long long fail_umount(void *data, const struct ChfilImpl *impl)
{
	...
	if (impl->ct == CHFILETYPE_UNUMOUNT && mnt == impl->mnt)
		return chfilimpl_geturn(impl);

	if (impl->ct == CHFILETYPE_HUNMOUNTALL ||
	    (impl->ct == CHFILETYPE_HUNMOUNT && mnt == impl->mnt)) {
		while (impl->ct)
			ras_delay(20);
	}
	return 0;
...
{
	return (impl->ct == CHFILETYPE_UNMOUNT &&
		impl->path != NULL &&
...
{
	loff_t current_time = 0;
	struct ChfilImpl *ci = data;
#if KERNEL_VERSION(5, 10, 0) < LINUX_VERSION_CODE
	current_time = ras_ms();
#endif
	if (impl->ct == CHFILETYPE_DEPLENISH) {
		if (current_time > inject_time) {
			write_lock(&chfil.rwk);
			(void)memset_s(ci, sizeof(struct ChfilImpl), 0, sizeof(struct ChfilImpl));
			write_unlock(&chfil.rwk);
			return 0;
...
#endif
{
	...
	if (IS_ERR(fp))
		return 0;
	if (should_fail(fail_deplenish_check, fail_deplenish, g_fileaddress)) {
		/* file handler deplenished */
	...
	if (ret != 0) {
		rasprobe_seturn(regs, ret); /* can't open file */
		return 0;
	}
	...
	if (fp->f_mode & FMODE_WRITE) {
		...
		if (ret != 0) {
			rasprobe_seturn(regs, ret); /* can't write */
			return 0;
		}
	}
...
static int cmd_main(void *data, int argc, char *args[])
{
	...
	cmd = args[0];
	name = args[1];
	ras_retn_if(cmd == 0 || name == 0, -EINVAL);
	len = strlen(cmd);
	...
	ras_retn_if((len == 3) && (strncmp("log", cmd, len) == 0),
		    chfil_log(name));
	ras_retn_if((len == 6) && (strncmp("cancel", cmd, len) == 0),
		    chfil_cancel(chf, name));
	ras_retn_if((len == 7) && (strncmp("restore", cmd, len) == 0),
...
static ssize_t proc_ops_write(rfile) (struct file *filp,
	...
	g_fileaddress = PDE_DATA(FILE_NODE(filp));
...
static int rfile_init(void)
{
	...
	(void)memset_s(&chfil, sizeof(chfil), 0, sizeof(chfil));
