...
static int fault_restore(struct fault_impl *fault)
{
	...
	(void)memset_s(fault, sizeof(struct fault_impl), 0, sizeof(struct fault_impl));
	return 0;
}
			    ...
			    char *argv[])
{
	...
	if (fault_args->percent == 0)
		fault_args->percent = 100; /* default 100% occur */
	return 0;
}
	...
	for (i = 0; i < ARRAY_SIZE(fault_injected.impl); i++) {
		...
		if (fault_injected.impl[i].type == fault_args.type &&
		    (fault_injected.impl[i].pid == fault_args.pid ||
		     (fault_args.pid == 0 || fault_injected.impl[i].pid == 0)) &&
		    ((strcmp(fault_args.disk, fault_injected.impl[i].disk) == 0))) {
			/* fault alread exist. */
...
struct fault_impl *fault_include(enum fault_type type, pid_t pid)
{
	...
	for (i = 0; i < ARRAY_SIZE(fault_injected.impl); i++) {
		if (fault_injected.impl[i].type == type &&
		    (fault_injected.impl[i].pid == pid || fault_injected.impl[i].pid == 0)) {
...
static int fault_restore_one(int args, char *argv[])
{
	...
	/* 1. check the args */
	(void)memset_s(&fault_args, sizeof(fault_args), 0, sizeof(fault_args));
	ras_retn_if((args < 2), -EINVAL); /* size of argv >= 2 */
						  ...
						  bool isRw)
{
	...
	if (fault->percent <= random_val % 100) /* random_val mod 100: [0-99] */
		return NULL;
	return fault;
}
...
static int rasprobe_handler(mas_blk_generic_make_request_check) (struct
							  rasprobe_instance *
							  ri,
							  struct pt_regs *
							  regs) {
...
#endif
{
	...
	fault = rasio_check_fault_match(bio, FAULT_R_DELAY, (bio_data_dir(bio) == READ));
	if (!fault)
	...
	if (!fault) {
		...
		for (i = 0; i < ARRAY_SIZE(fault_injected.arr_queue_disk); i++) {
			...
			if (fault_injected.arr_queue_disk[i].gendisk == bdisk) {
				request_fn = fault_injected.arr_queue_disk[i].make_request_fn;
				find = 1;
				break;
			}
		}
...
proc_ops_define(rio);
rasprobe_define(mas_blk_generic_make_request_check);
static struct rasprobe *probes[] = {
	&rasprobe_name(mas_blk_generic_make_request_check),
};
