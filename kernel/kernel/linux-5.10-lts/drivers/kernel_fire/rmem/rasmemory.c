...
static int rasmemele_set(struct RasMemEle *ele, struct RasMemArgs *args)
{
	...
	if (ele->args.total == 0) {
		(void)memcpy_s(&ele->args, sizeof(struct RasMemArgs), args, sizeof(struct RasMemArgs));
		if (idx == 0)
			idx++;
...
static int rasmemele_func(void *data)
{
	...
	while (!kthread_should_stop()) {
		...
		if (ele->len > len &&
		    rasmemele_touch(ele, ele->len - len, rasmemele_addmm) == 0)
			ele->tch += (ele->len - len);
...
static int rasmemele_start(struct RasMemEle *ele)
{
	...
	ele->dtor = 1;
	(void)snprintf_s(name, sizeof(name), sizeof(name) - 1, "rasmem%lu", ele->idx);
...
static int rasmem_main(void *mem, int argc, char *args[])
{
	...
	(void)memset_s(&rma, sizeof(rma), 0, sizeof(rma));
	for (i = 0; i < argc; i++)
...
static int rasmem_ctor(struct RasMem *mem)
{
	...
	(void)memset_s(mem, sizeof(struct RasMem), 0, sizeof(struct RasMem));
	...
	for (i = 0; i < len; i++) {
		...
		ele->next = ele;
		ele->tail = &ele->head;
		(void)memset_s(&ele->head, sizeof(struct RasMemory), 0, sizeof(struct RasMemory));
	}
...
static int proc_ops_show(rmem) (struct seq_file *m, void *v)
{
	...
	for (i = 0; i < len; i++) {
		...
		read_lock(&ele->rwk);
		if (ele->args.total != 0) {
			int ret = snprintf_s(buf, sizeof(buf), sizeof(buf) - 1, rasfmt, ele->idx,
...
static ssize_t proc_ops_write(rmem) (struct file *filp,
	...
	ras_retn_iferr(ras_args(buf_cmd, count, rasmem_main, &gRasMem));
