...
struct RInject {
	unsigned long idx;
	const char *cmd;
	int (*missile)(struct RMissile *rmissile);
	char *dsc;
};
...
static int rmissile_awdt(struct RMissile *prm)
{
#ifdef CONFIG_AP_CP_DECOUPL
...
static struct RInject *rmissile_get(char *cmd)
{
	...
	static struct RInject sr[] = {
		...
		{13, "awdt", rmissile(awdt), "Injection single core stuck"}
	};
...
static int cmd_main(void *data, int argc, char *args[])
{
	...
	rm.para = UINT_MAX;
	if (strcmp(cmd, "irqnum") == 0 && argc < 2) /* if args are less than 2 */
		return -EINVAL;
	if (argc >= 2) { /* checking the 2nd args */
	...
	rm.para1 = UINT_MAX;
	if (argc >= 3) { /* checking the 3rd args */
		ras_retn_iferr(ras_atoll(&val, args[2], strlen(args[2]), 0)); /* 2: parse the 3rd args */
		if (val < 0)
			return -EINVAL;
