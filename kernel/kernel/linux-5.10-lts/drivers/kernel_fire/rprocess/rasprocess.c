...
static struct FaultImpl *find_fault(struct FaultImpl *ft)
{
	...
	for (i = 0; i < FAULTS_MAX; i++) {
		if ((fault_list.impl[i].pid != 0 && fault_list.impl[i].pid == ft->pid) ||
		    (fault_list.impl[i].tid != 0 && fault_list.impl[i].tid == ft->tid)) {
			j = i;
			break;
		}
...
static int fill_fault(struct FaultImpl *ft)
{
	...
	if (impl == 0)
		return -ENOENT;
	(void)memcpy_s(impl, sizeof(struct FaultImpl), ft, sizeof(struct FaultImpl));
	return 0;
}
...
static int resume_fault(struct FaultImpl *ft)
{
	...
	(void)memset_s(ft, sizeof(struct FaultImpl), 0, sizeof(struct FaultImpl));
	return 0;
}
...
static int tool_init(void)
{
	...
	(void)memset_s(&fault_list, sizeof(fault_list), 0, sizeof(fault_list));
