...
#define rasbase_setstr(ins, fld, arg) do { \
	...
	if (strncmp(#fld"=", arg, len) == 0) { \
		...
		(void)strcpy_s((ins)->fld, sizeof((ins)->fld), cval); \
		return 0; \
	} \
} while (0)

#define STATUS_BYTE_SHIFT 1
#define MSG_BYTE_SHIFT	8
#define HOST_BYTE_SHIFT   16
#define DRIVER_BYTE_SHIFT 24
enum ScsiResultByte {
	STATUS_BYTE = 0,
	MSG_BYTE,
	HOST_BYTE,
...
static void fail_done_return(struct fault_args *pfa, struct scsi_cmnd *scmd)
{
	...
	v[STATUS_BYTE] = status_byte(scmd->result);
	v[MSG_BYTE] = msg_byte(scmd->result);
	v[HOST_BYTE] = host_byte(scmd->result);
	v[DRIVER_BYTE] = driver_byte(scmd->result);
	if (pfa->ret_status)
		v[STATUS_BYTE] = pfa->ret_status;
	if (pfa->ret_msg)
		v[MSG_BYTE] = pfa->ret_msg;
	if (pfa->ret_host)
		v[HOST_BYTE] = pfa->ret_host;
	if (pfa->ret_driver)
		v[DRIVER_BYTE] = pfa->ret_driver;
	result = (v[STATUS_BYTE] << STATUS_BYTE_SHIFT) | (v[MSG_BYTE] << MSG_BYTE_SHIFT) |
		 (v[HOST_BYTE] << HOST_BYTE_SHIFT) | (v[DRIVER_BYTE] << DRIVER_BYTE_SHIFT);
...
static int should_fail_attr(struct fault_attribute *pattr)
{
	...
	if (pattr->percent)
		return (prandom_u32() % 100 >= pattr->percent) ? 0 : 1; /* max percent 100 */
...
int fail_queuecommand(struct Scsi_Host *shost, struct scsi_cmnd *scmd)
{
	...
	if (need_bak)
		add_entry_scsi(scmd); /* backup the command */
	spin_unlock_irqrestore(shost->host_lock, flags);
...
int fill_fault(struct fault_args *pa)
{
	...
	if (j == -1) /* max the faults */
		return -1;
	fa = &ft->fts[j];
	(void)memcpy_s(fa, sizeof(struct fault_args), pa, sizeof(struct fault_args));
...
int cmd_main(void *data, int argc, char *args[])
{
	...
	if (argc < 1)
		return -1;
	(void)memset_s(&ft_args, sizeof(ft_args), 0, sizeof(ft_args));
...
static ssize_t proc_ops_write(tool) (struct file *filp,
	...
	ras_retn_iferr(ras_args(buf_cmd, count, cmd_main, filp));
