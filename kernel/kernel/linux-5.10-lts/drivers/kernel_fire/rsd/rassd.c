...
struct fault {
	...
	int offset_bit;
	int offset_byte;
	/* SD scr information modify SD_SCR
...
struct sd_work {
	...
	void (*done_orig)(struct mmc_request *mrq);
};
...
{
	ras_retn_if((fault_temp->type == FAULT_NA && fault_temp->cmd == -1), -EINVAL);
	ras_retn_if((fault_temp->type != FAULT_NA && fault_temp->cmd != -1), -EINVAL);
	if (fault_temp->cmd != -1) {
...
static int fill_fault(struct fault *fault_temp)
{
	...
	for (i = 0; i < ARRAY_SIZE(fault_list.faults); i++) {
		...
		if (fault_temp->type != FAULT_NA) {
			...
			/* Error: sd commands */
			if ((fault_list.faults[i].offset_bit == fault_temp->offset_bit &&
			     fault_temp->offset_bit >= 0) ||
			    (fault_list.faults[i].offset_byte == fault_temp->offset_byte &&
	...
	if (fault_pos != NULL) {
		(void)memcpy_s(fault_pos, sizeof(struct fault), fault_temp, sizeof(struct fault));
		return 0;
	}
	return -EINVAL;
}
...
#define MMC_CMD_BYTE_SIZE (4 * sizeof(u32))
#define MMC_CMD_BIT_SIZE (8 * 4 * sizeof(u32))
#ifdef CONFIG_MMC_DW
...
void replace_done_generic(struct mmc_request *mrq, replace_fun done_callback)
{
...
int cmd_inject(const char *name, struct mmc_command *cmd)
{
	...
	for (i = 0; i < ARRAY_SIZE(fault_list.faults); i++) {
		...
		if (fault_temp->cmd == cmd->opcode) {
			buf_byte_inject(cmd->resp, MMC_CMD_BYTE_SIZE,
					fault_temp->offset_byte,
					fault_temp->byte);
			buf_bit_inject(cmd->resp, MMC_CMD_BIT_SIZE,
...
static int tool_init(void)
{
	...
	data_done_fun =
		(replace_fun)kallsyms_lookup_name("mmc_wait_for_req_done");
	...
	ras_retn_iferr(proc_init(MODULE_NAME,
				 &proc_ops_name(rSd),
				 &fault_list));
	return 0;
}
