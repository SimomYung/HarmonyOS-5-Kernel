...
static int fail(const char *type, int temp)
{
	...
	ras_retn_iferr(strcpy_s(flt.ft[j].thermal_type, strlen(type) + 1, type));
...
static void restore(struct Fault *ft)
{
	...
	(void)memset_s(ft, sizeof(struct Fault), 0, sizeof(struct Fault));
}
...
static void update_temperature(struct thermal_zone_device *pzone, unsigned int *temperature)
{
	int temp = should_fail(pzone->type);

	if (temp != VALID_TEMPREATURE) {
	...
	(struct rasprobe_instance *ri, struct pt_regs *regs)
{
	...
	update_temperature(pzone, temperature);
	return 0;
}
#endif

#ifdef CONFIG_HISI_THERMAL_TSENSOR
static int rasprobe_handler(tsens_tz_get_temp)
	(struct rasprobe_instance *ri, struct pt_regs *regs)
{
	int temp;
	unsigned int *temperature = NULL;
...
static int cmd_main(void *data, int argc, char *argv[])
{
	...
	ras_retn_if(argc < 2, -EINVAL); /* if args are less than 2 */
...
static int tool_init(void)
{
	...
	(void)memset_s(&flt, sizeof(flt), 0, sizeof(flt));
	rwlock_init(&flt.rwk);
