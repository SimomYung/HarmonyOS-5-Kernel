...
#ifdef CONFIG_LIBLINUX
	/* only used for HM-Kernel LDK to avoid order=2 page alloc failed */
	stream = kvzalloc(sizeof(*stream), GFP_KERNEL);
#else
	stream = kzalloc(sizeof(*stream), GFP_KERNEL);
#endif
	if (stream == NULL) {
		uvc_printk(KERN_ERR, "uvc_stream_new failed");
		return NULL;
	} else {
		uvc_printk(KERN_INFO, "uvc_stream_new ok");
	}
