...
static int mmc_blk_check_part(struct mmc_card *card,
				      unsigned int part_type)
{
	struct mmc_blk_data *main_md = dev_get_drvdata(&card->dev);
	int part_flag = mmc_status_check(EMMC_PART_FLAG);
 
	if (part_flag && (main_md->part_curr == part_type)) {
		return NO_NEED_PART_SWITCH;
	} else if ((!part_flag) && (main_md->part_curr == part_type) &&
		(part_type == EXT_CSD_PART_CONFIG_ACC_DATA)) {
		return NO_NEED_PART_SWITCH;
	} else {
		return NEED_PART_SWITCH;
	}
}
...
#ifdef CONFIG_ZODIAC_SHARE_MMC
	ret = mmc_blk_check_part(card, part_type);
	if (ret == NO_NEED_PART_SWITCH)
		return 0;
#else
