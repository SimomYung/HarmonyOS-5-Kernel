		     ...
		     atomic_t *abort)
{
	...
	bool pm = false;
#ifdef CONFIG_ZODIAC_SHARE_MMC
	struct hwspinlock *emmc_hwlock = host->emmc_hwlock;
#endif
	might_sleep();
...
retry:
			...
			if (host->dont_get_hwspink || emmc_hwspin_lock_timeout(emmc_hwlock, EMMC_LOCK_TIMEOUT)) {
				pr_debug("AP mmc try request hwspin lock\n");
				host->dont_get_hwspink = 0;
				WARN_ON((host->hwspink_cnt != 1));
                host->hwspink_cnt--;
				spin_unlock_irqrestore(&host->lock, flags);
				schedule();
				goto retry;
			} else {
				pr_debug("%s get lock success cnt %d claim_cnt:%d\n", __func__, host->hwspink_cnt, host->claim_cnt);
				if (unlikely(!qic_mmc_actrl_reg)) {
					qic_mmc_actrl_reg = ioremap(0xFF586000, 0x1000);
				}
				writel(0x1, qic_mmc_actrl_reg + 0x40C);
				host->hwspink_start = ktime_get();
				host->dont_get_hwspink = 0;
				host->emmc_hwlock_state = 1;
				mmc_status_mark(EMMC_STATUS_ATTRIBUTE, AP_USE_FLAG);
				mmc_status_mark(EMMC_APPLY_FLAG, AP_DONT_WANT_USE_FLAG);
			}
			spin_unlock_irqrestore(&host->lock, flags);
		}
#endif
...
void mmc_release_host(struct mmc_host *host)
{
	...
	if (--host->claim_cnt) {
		...
		host->claimer = NULL;
#ifdef CONFIG_ZODIAC_SHARE_MMC
		if (host->emmc_hwlock && host->emmc_hwlock_state) {
			host->hwspink_cur = ktime_get();
			host->emmc_hwlock_state = 0;
			mmc_status_mark(EMMC_STATUS_ATTRIBUTE, AP_DONT_USE_FLAG);
			if (ktime_to_ms(ktime_sub(host->hwspink_cur, host->hwspink_start)) > 5000)
				host->dont_get_hwspink = 1;
			emmc_hwspin_unlock(host->emmc_hwlock);
			WARN_ON((host->hwspink_cnt != 1));
			host->hwspink_cnt--;
			pr_debug("%s release lock success cnt %d claim_cnt:%d\n", __func__, host->hwspink_cnt, host->claim_cnt);
		}
#endif
		spin_unlock_irqrestore(&host->lock, flags);
		wake_up(&host->wq);
...
int mmc_set_uhs_voltage(struct mmc_host *host, u32 ocr)
{
	...
	if (err) {
		if (!strcmp(mmc_hostname(host), "mmc1")) {
			pr_info("%s:send cmd11 fail, err = %d\n",
			       mmc_hostname(host), err);
		}
		return -EAGAIN;
	}
...
power_cycle:
	if (err) {
		pr_info("%s: Signal voltage switch failed, power cycling card\n", mmc_hostname(host));
		mmc_power_cycle(host, ocr);
	} else {
...
#endif
#ifdef CONFIG_ZODIAC_SHARE_MMC
	mmc_status_mark(EMMC_INATIAL_ATTRIBUTE, EMMC_INI_COMOLETE_FLAG);
#endif
	mmc_release_host(host);
	pr_err("%s, %d end\n", __func__, __LINE__);
 out:
#ifdef CONFIG_ZODIAC_MMC
	if (extend_wakelock)
		__pm_wakeup_event(host->ws, jiffies_to_msecs(HZ / 2));
	else
		__pm_relax(host->ws);
#endif
