...
int mmc_send_op_cond(struct mmc_host *host, u32 ocr, u32 *rocr)
{
	...
	pr_info("%s: return err=%d and cmd.resp[0]=0x%x\n", __func__, err, cmd.resp[0]);
#endif
	return err;
}
...
out:
	...
	if ((err == 0) && (index == EXT_CSD_PART_CONFIG)) {
		if((value & EXT_CSD_PART_CONFIG_ACC_MASK) != EXT_CSD_PART_CONFIG_ACC_DATA)
			mmc_status_mark(EMMC_PART_FLAG, EMMC_APRT_RPMB_FLAG);
		else
			mmc_status_mark(EMMC_PART_FLAG, EMMC_APRT_DATA_FLAG);
	}
#endif
	return err;
}
