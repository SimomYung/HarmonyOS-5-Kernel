...
out:
	...
	/*
	 * if clr MMC_PM_KEEP_POWER, sdio resume will restore power and reinitialize
	 * the card, After receiving cmd5, some devices will response a sd memory card,
	 * that will cause sdio host state-machine busy or card busy.
	 */
...
static int mmc_sdio_hw_reset(struct mmc_host *host)
{
	...
	if (host->ops->need_retuning && host->bus_ops->mmc_retuning && (host->retuning_count == 0)) {
