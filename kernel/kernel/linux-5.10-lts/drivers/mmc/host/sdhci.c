...
void sdhci_enable_clk(struct sdhci_host *host, u16 clk)
{
	...
	pr_err("sdhci_enable_clk clk =0x%x . SDHCI_CLOCK_CONTROL = 0x%x\n", clk, 
			       sdhci_readw(host, SDHCI_CLOCK_CONTROL));
	/* Wait max 150 ms */
...
#ifdef CONFIG_MMC_SDHCI_ZODIAC_APUS
	host->bootack_timeoupt_counter = sdhci_readw(host, SDHCI_TIMEOUT_CONTROL);
	host->cfg_data_timeout_step = sdhci_readw(host, SDEMMC_TIMING_CTRL_2);
    sdhci_writew(host, BOOTACK_TIMEOPUT_COUNTER, SDHCI_TIMEOUT_CONTROL);
    sdhci_writew(host, CFG_DATA_TIMEOUT_STEP, SDEMMC_TIMING_CTRL_2);
#endif
}
...
#ifdef CONFIG_MMC_SDHCI_ZODIAC_APUS
    sdhci_writew(host, host->bootack_timeoupt_counter, SDHCI_TIMEOUT_CONTROL);
    sdhci_writew(host, host->cfg_data_timeout_step, SDEMMC_TIMING_CTRL_2);
#endif
}
...
#ifdef CONFIG_MMC_SDHCI_ZODIAC_APUS
	if ((host->send_tuning) && (host->mmc->index == 2)) {
		pr_err("rwx_log sdhci_irq in\n");
	}
#endif
	spin_lock(&host->lock);
...
#ifdef CONFIG_MMC_SDHCI_ZODIAC_APUS
		if (host->mmc->index == 2)
			pr_err("rwx_log sdhci_irq runtime_suspend\n");
#endif
...
#if defined CONFIG_MMC_SDHCI_ZODIAC_APUS
...
#if defined CONFIG_MMC_SDHCI_ZODIAC
...
#ifdef CONFIG_ZODIAC_SHARE_MMC
	if (mmc->index == 0) {
		mmc->emmc_hwlock = hwspin_lock_request_specific(EMMC_HWLOCK_ID);
		if (!mmc->emmc_hwlock)
			pr_err("%s Request emmc hwspin lock failed !\n", __func__);
		else
			pr_err("%s Request emmc hwspin lock success !\n", __func__);
	}
#endif
	return 0;
