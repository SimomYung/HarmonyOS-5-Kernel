...
static struct _ax_buikin_setting AX88279_BULKIN_SIZE[] = {
...
static const struct ethtool_ops ax88279_ethtool_ops = {
#if KERNEL_VERSION(5, 7, 0) < LINUX_VERSION_CODE
	.supported_coalesce_params = ETHTOOL_COALESCE_RX_USECS,
#endif
	.get_drvinfo	= ax_get_drvinfo,
			  ...
			  struct _ax_ioctl_command *info)
{
	...
	netdev_info(axdev->netdev, "[AX_DEBUG]%s - Start", __func__);
			   ...
			   struct _ax_ioctl_command *info)
{
	...
	netdev_info(axdev->netdev, "[AX_DEBUG]%s - Start offset: %d (0x%x)", __func__, offset, offset);
			    ...
			    void __user *udata, int cmd)
{
	...
	case AX_PRIVATE:
		...
		if (!axdev)
			return -EFAULT;
		netdev_info(axdev->netdev, "[AX_DEBUG]ioctl_cmd: %d", info.ioctl_cmd);
...
static int ax88179a_set_phy_power(struct ax_device *axdev, bool on)
{
	...
	if (on) {
		...
		if (!axdev)
			return -EFAULT;
		netdev_info(axdev->netdev, "[AX_DEBUG]ENABLE_INT_AGGRESSIVE");
