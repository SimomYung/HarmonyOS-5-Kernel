...
int ax_set_s5_wol(struct ax_device *axdev, u8 enable) 
{
	...
	if (axdev->chip_version == AX_VERSION_AX88279) {
		ret = ax_write_cmd(axdev, AX8179A_WAKEUP_SETTING, 8,
			...
			(S5_WOL_EN | S5_WOL_LOW_POWER | GMII_CONTROL_RESET) : GMII_CONTROL_RESET), 0, NULL);
	}
...
static void __int_polling_work(struct work_struct *work)
{
	...
	if (axdev->link) {
		u16 medium_mode = 0;
		ret = ax_read_cmd_nopm(axdev, AX_ACCESS_MAC, AX_MEDIUM_STATUS_MODE,
					2, 2, &medium_mode, 1);
		if (ret < 0) {
			netdev_err(axdev->netdev, "[AX_DEBUG]%s ax_read_cmd_nopm error\n", __func__);
		}
		if (!netif_carrier_ok(axdev->netdev)) {
			...
			netdev_info(axdev->netdev, "[AX_DEBUG] link but rx diable\n");
			if (axdev->driver_info && axdev->driver_info->link_reset)
...
static int ax_open(struct net_device *netdev)
{
	...
	if (res < 0) {
        dev_err(&axdev->intf->dev, 
				"usb get interface failed! Error=%d\n", res);
        dev_err(&axdev->intf->dev, 
				"Post-error: usage_count=%d, runtime_status=%d, disable_depth=%d\n",
                 atomic_read(&axdev->intf->dev.power.usage_count),
                 axdev->intf->dev.power.runtime_status,
                 axdev->intf->dev.power.disable_depth);
		goto out_free;
	}
...
static int ax_probe(struct usb_interface *intf, const struct usb_device_id *id)
{
	...
	if (udev->actconfig->desc.bConfigurationValue != 1) {
		...
		if (ret) {
			dev_err(&intf->dev, "[AX_DEBUG]%s usb_driver_set_configuration error\n", __func__);
		}
		return -ENODEV;
	}
	...
	for (i = 0; i < alt->desc.bNumEndpoints; i++) {
		...
		if (epnum == 1) {
			comp = &ep->ss_ep_comp;
			dev_info(&intf->dev, "[AX_DEBUG]Found EP1: wBytesPerInterval=%u\n",
					le16_to_cpu(comp->wBytesPerInterval));
			break;
		}
	}
	...
	axdev->int_polling_enable = (axdev->ep1_bytes_per_interval == 0);
	dev_info(&intf->dev, "[AX_DEBUG]int_polling_enable=%d\n",
			axdev->int_polling_enable);
...
#endif
	netif_wake_queue(netdev);
	ret = ax_usb_submit_intr_urb(axdev->intr_urb, GFP_KERNEL);
	if (ret) {
		pr_err("[AX_DEBUG]%s ax_usb_submit_intr_urb error\n", __func__);
	}
	ax_schedule_delayed_work(&axdev->int_polling_work,
			      msecs_to_jiffies(INT_POLLING_TIMER));
...
static int ax_system_resume(struct ax_device *axdev)
{
	...
	netdev_info(netdev, "[AX_DEBUG]%s - entry\n", __func__);
	...
	if (netif_running(netdev) && (netdev->flags & IFF_UP)) {
		...
		set_bit(AX_ENABLE, &axdev->flags);
		ret = ax_usb_submit_intr_urb(axdev->intr_urb, GFP_NOIO);
		if (ret) {
			netdev_err(netdev, "[AX_DEBUG]%s ax_usb_submit_intr_urb error\n", __func__);
		}
	...
	netdev_info(netdev, "[AX_DEBUG]%s - exit\n", __func__);
...
static int ax_runtime_resume(struct ax_device *axdev)
{
	...
	if (netif_running(netdev) && (netdev->flags & IFF_UP)) {
		...
		ret = ax_usb_submit_intr_urb(axdev->intr_urb, GFP_NOIO);
		if (ret) {
			netdev_err(netdev, "[AX_DEBUG]%s usb_driver_set_configuration error\n", __func__);
		}
	} else {
		clear_bit(AX_SELECTIVE_SUSPEND, &axdev->flags);
	}
...
static int ax_suspend(struct usb_interface *intf, pm_message_t message)
{
	...
	if (!axdev)
		return -EFAULT;
	netdev_info(axdev->netdev, "[AX_DEBUG]%s - entry\n", __func__);
	mutex_lock(&axdev->control);
	...
	mutex_unlock(&axdev->control);
	netdev_info(axdev->netdev, "[AX_DEBUG]%s - exit\n", __func__);
	return ret;
}
...
static int ax_reset_resume(struct usb_interface *intf)
{
	...
	if (!axdev)
		return -EFAULT;
	netdev_info(axdev->netdev, "[AX_DEBUG] %s\n", __func__);
	clear_bit(AX_SELECTIVE_SUSPEND, &axdev->flags);
...
static const struct usb_device_id ax_usb_table[] = {
	...
	ASIX_USB_DEVICE(USB_VENDOR_ID_ASIX, AX_DEVICE_ID_179X, 0,
			AX_BCDDEVICE_ID_279A, ax88279_info),
	{/*END*/}
};
...
#else
static int ax_cfgselector_probe(struct usb_device *udev)
{
	if (usb_set_configuration(udev, 1)) {
		dev_err(&udev->dev, "Failed to set ax configuration\n");
		return -ENODEV;
	}
...
static void ax_cfgselector_disconnect(struct usb_device *udev)
{
	usb_generic_driver_disconnect(udev);
}

#ifdef CONFIG_PM
static int ax_cfgselector_suspend(struct usb_device *udev, pm_message_t msg)
{
	int status = usb_generic_driver_suspend(udev, msg);
	dev_info(&udev->dev, "%s: status %d\n", __func__, status);
	return status;
}

static int ax_cfgselector_resume(struct usb_device *udev, pm_message_t msg)
{
	int status = usb_generic_driver_resume(udev, msg);
	dev_info(&udev->dev, "%s: status %d\n", __func__, status);
	return status;
}
#endif

static struct usb_device_driver ax_cfgselector_driver = {
	.name =		MODULENAME "-cfgselector",
	.probe =	ax_cfgselector_probe,
	.id_table =	ax_usb_table,
	.generic_subclass = 0,
	.supports_autosuspend = 1,
	.disconnect = ax_cfgselector_disconnect,
#ifdef CONFIG_PM
	.suspend = ax_cfgselector_suspend,
	.resume = ax_cfgselector_resume,
#endif
};

static int __init ax_driver_init(void)
{
	int ret;

	ret = usb_register_device_driver(&ax_cfgselector_driver, THIS_MODULE);
	if (ret)
		return ret;
...
static void __exit ax_driver_exit(void)
{
	usb_deregister(&ax_usb_driver);
	usb_deregister_device_driver(&ax_cfgselector_driver);
}

module_init(ax_driver_init);
module_exit(ax_driver_exit);
