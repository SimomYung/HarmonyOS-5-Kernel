...
#define R8152_CTRL_GET_TIMEOUT		2000
#define R8152_CTRL_SET_TIMEOUT		2000
...
#define INTBUFSIZE		2
#define R8152_MAX_BOTTOM_LOOP    100
...
int get_registers(struct r8152 *tp, u16 value, u16 index, u16 size, void *data)
{
	...
	ret = usb_control_msg(tp->udev, tp->pipe_ctrl_in,
			      ...
			      value, index, tmp, size, R8152_CTRL_GET_TIMEOUT);
	if (ret < 0)
...
int set_registers(struct r8152 *tp, u16 value, u16 index, u16 size, void *data)
{
	...
	ret = usb_control_msg(tp->udev, tp->pipe_ctrl_out,
			      ...
			      value, index, tmp, size, R8152_CTRL_SET_TIMEOUT);
...
static void intr_callback(struct urb *urb)
{
	...
	if (INTR_LINK & __le16_to_cpu(d[0])) {
		if (!netif_carrier_ok(tp->netdev)) {
			dev_info(&tp->intf->dev, "link with no carrier, __le16_to_cpu(d[0]) = %#x, tp->netdev->state = %#x\n", __le16_to_cpu(d[0]), tp->netdev->state);
			set_bit(RTL8152_LINK_CHG, &tp->flags);
			schedule_delayed_work(&tp->schedule, 0);
		}
	} else {
		if (netif_carrier_ok(tp->netdev)) {
			dev_info(&tp->intf->dev, "unlink with carrier, __le16_to_cpu(d[0]) = %#x, tp->netdev->state = %#x\n", __le16_to_cpu(d[0]), tp->netdev->state);
...
{
	int res;
	int max_loop = R8152_MAX_BOTTOM_LOOP;
	...
	do {
		...
		if (!res) {
			max_loop--;
			dev_err(&tp->intf->dev, "tx_bottom continue res:%d max_loop:%d\n", res, max_loop);
			continue;
		}
...
free_skb:
    ...
    if (skb->len > DQL_MAX_OBJECT) {
		dev_err(&tp->intf->dev, "skb->len is too large! %u\n", skb->len);
		dev_kfree_skb_any(skb);
...
{
	dev_info(&tp->intf->dev, "rtl_can_wakeup false\n");
	return false;
}
	...
	if (res < 0) {
        ...
        dev_err(&tp->intf->dev, 
				"Post-error: usage_count=%d, runtime_status=%d, disable_depth=%d\n",
                 atomic_read(&tp->intf->dev.power.usage_count),
                 tp->intf->dev.power.runtime_status,
                 tp->intf->dev.power.disable_depth);
		goto out_free;
	}
...
static int rtl8152_resume(struct usb_interface *intf)
{
	...
	dev_info(&tp->intf->dev, "rtl8152_resume +\n");
