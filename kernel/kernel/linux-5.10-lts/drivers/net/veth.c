...
static int veth_newlink(struct net *src_net, struct net_device *dev,
			...
			struct netlink_ext_ack *extack)
{
	pr_info("%s invoked and dev->name=%s\n", __func__, dev->name);
	int err;
	struct net_device *peer;
	...
	if (data != NULL && data[VETH_INFO_PEER] != NULL) {
		...
		err = rtnl_nla_parse_ifla(peer_tb,
					  ...
					  NULL);
		if (err < 0) {
			pr_err("%s rtnl_nla_parse_ifla fail, err=%d\n", __func__, err);
			return err;
		}

		err = veth_validate(peer_tb, NULL, extack);
		if (err < 0) {
			pr_err("%s veth_validate fail, err=%d\n", __func__, err);
			return err;
		}
	...
	if (IS_ERR(net)) {
		pr_err("%s rtnl_link_get_net fail, err=%d\n", __func__, PTR_ERR(net));
		return PTR_ERR(net);
	}
	...
	if (IS_ERR(peer)) {
		put_net(net);
		pr_err("%s rtnl_create_link fail, err=%d\n", __func__, PTR_ERR(peer));
		return PTR_ERR(peer);
	}
	...
	net = NULL;
	if (err < 0) {
		pr_err("%s register_netdevice fail, err=%d\n", __func__, err);
	...
	if (err < 0) {
		pr_err("%s rtnl_configure_link fail, err=%d\n", __func__, err);
