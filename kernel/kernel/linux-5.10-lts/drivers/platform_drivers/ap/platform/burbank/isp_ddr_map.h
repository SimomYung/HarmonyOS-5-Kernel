/*
 * hisilicon isp_ddr_map.h
 *
 * Copyright (c) 2016 Hisilicon Technologies CO., Ltd.
 */

#ifndef __ISP_DDR_MAP_H__
#define __ISP_DDR_MAP_H__

#include <global_ddr_map.h>

#ifdef __cplusplus
#if __cplusplus
extern "C" {
#endif
#endif
/* FPGA INFO */
#define ISP_PCTRL_PERI_STAT_ADDR    (0x000000BC)/*  offset   */
#define ISP_PCTRL_PERI_FLAG         (1 << 4)    /*  BIT 4 : ISP   */
/* FW IMAGE SIZE*/
#define MEM_ISPFW_SIZE              (0x00E00000)
#define MEM_PMD_ADDR_OFFSET         (0x00002000)
#define MEM_PTE_ADDR_OFFSET         (MEM_ISPFW_SIZE + 0x10000)

#define MEM_RDR_RESERVED_BASE       RESERVED_MNTN_PHYMEM_BASE
#define MEM_ISP_RDR_OFFSET          (0x6BE000)
#define MEM_SMMU_ERRRD_ADDR_OFFSET  (0x0000F000)
#define MEM_SMMU_ERRWR_ADDR_OFFSET  (0x0000F800)
#define MEM_ISP_ERRRD_ADDR_OFFSET   (0x00000500)
#define MEM_ISP_ERRWR_ADDR_OFFSET   (0x00000600)
#define MEM_ISP_ERR_ADDR_BASE       (MEM_RDR_RESERVED_BASE + MEM_ISP_RDR_OFFSET)
/* DEBUG MEM */
#define MEM_RSCTABLE_ADDR_OFFSET    (0x00014000)
#define MEM_RSCTABLE_SIZE           (0x00004000)
#define MEM_ISPDTS_SIZE             (0x02000000)
/* BOTSON CS */
#define MEM_MDC_DA                  (0xC1400000) /*  the mdc iova addr */
#define MEM_MDC_SIZE                (0x00100000) /*  the mdc iova size */
/* coredump and rwa2yuv info */
#define ISPCPU_COREDUMP_ADDR        (0xC4000000)
#define ISPCPU_COREDUMP_SIZE        (0x01500000) /*should equal DEFAULT_ROTATE_SIZE*/
#define MEM_RAW2YUV_DA              (0xC8000000) /*  the raw2yuv iova addr */
#define MEM_RAW2YUV_SIZE            (0x04000000) /*  the raw2yuv size */
/* *end* */
#define SHAREDMEM_BASE              (0xc2000000)
#define VQ_BASE                     (0xc2020000)
#define DEVICE_BASE                 (0xE0000000)
#define TEXT_BASE                   (0xc0200000)
#define DATA_BASE                   (0xc0800000)
#define ISP_TEXT_SIZE               (0x00600000)
#define ISP_DATA_SIZE               (0x00800000)
#define ISP_BIN_DATA_SIZE           (0x00600000)
#define ISP_FW_SIZE                 (ISP_TEXT_SIZE + ISP_DATA_SIZE)
#define ISP_BIN_FW_SIZE             (ISP_TEXT_SIZE + ISP_BIN_DATA_SIZE)
#define ITCM_BASE                   (0xFFFF0000)
#define DTCM_BASE                   (0xE83D0000)
#define ISP_CORE_CFG_BASE_ADDR      (0xE8200000)
#define ISP_PMC_BASE_ADDR           (0xFFF01000)
#define ISP_RPMSG_VR_SIZE           (0x3000)
#define MAX_REGION_NUM              (24)
#define STATIC_REGION_NSEC_NUM      (14)
#define STATIC_REGION_SEC_NUM       (15)
#define ISP_NOC_ENABLE 0x00
#define ISP_TARGETFLOW 0x05
#define SOC_ISPCPU_MID 0x6B  /*SOC_ISP_ISP_A7_CFG_MID*/
#define SOC_ISPCORE_MID 0x40

/* defined for compile, actually not use on current platform */
#define ISP_RPMSG_VQ_BASE           0x1E00000
#define ISP_RPMSG_VQ_SIZE           0x40000
#define ISP_RPMSG_VR0_BASE          (ISP_RPMSG_VQ_BASE + ISP_RPMSG_VQ_SIZE)
#define ISP_RPMSG_VR_SIZE           (0x3000)
#define ISP_RPMSG_VR1_BASE          (ISP_RPMSG_VR0_BASE + ISP_RPMSG_VR_SIZE)

/* hisp mem_pool  */
#define ISP_MEM_POOL_NUM            (7)
#define ISP_MEM_POOL_ALIGN          (0x1000)
#define SEC_MEM_MAP_ALIGEN          (0x200000)

#define ISP_MAX_IOVA_MAGIC          (0xE0000000)
#define ISP_MAP_MAX_SIZE            (0x20000000)

/* isp iomap reg */
#define ISP_CRGPERI      0
#define ISP_ISPCORE      1
#define ISP_PMCTRL       2
#define ISP_PCTRL        3
#define ISP_SCTRL        4
#define ISP_VIVOBUS      5
#define ISP_CSSYS        6
#define ISP_SUBCTRL      7
#define ISP_WDT          8
#define ISP_MEDIA        9
#define ISP_ACTRL        10
#define ISP_NOCDPM       11
#define ISP_MEDIA_CTRL   12
#define ISP_MAX_REGTYPE  13

/* isp clk type */
#define ISPCPU_CLK       0
#define ISPFUNC_CLK      1
#define ISPI2C_CLK       2
#define ISPFUNC2_CLK     3
#define ISPFUNC3_CLK     4
#define ISP_SYS_CLK      5
#define ISP_CLK_MAX      6
/* unused isp clk type at current platform */
#define ISPFUNC4_CLK     ISP_CLK_MAX
#define ISPFUNC5_CLK     ISP_CLK_MAX
#define VIVOBUS_CLK      ISP_CLK_MAX
#define ISPI3C_CLK       ISP_CLK_MAX

/*PLL6 usage num*/
#define PLL6_USAGE_NUM          0

#ifdef __cplusplus
#if __cplusplus
}
#endif
#endif

#endif /* __ISP_DDR_MAP_H__ */
