...
/* 寄存器说明：中断屏蔽寄存器。
   位域定义UNION结构:  SOC_GPIO_GPIOIE4_UNION */
...
/* 寄存器说明：屏蔽后中断状态寄存器。
   位域定义UNION结构:  SOC_GPIO_GPIOMIS4_UNION */
...
/*****************************************************************************
 结构名    : SOC_GPIO_GPIOIE4_UNION
 结构说明  : GPIOIE4 寄存器结构定义。地址偏移量:0x508，初值:0x00000000，宽度:32
 寄存器说明: 中断屏蔽寄存器。
*****************************************************************************/
#ifndef __SOC_H_FOR_ASM__
typedef union
{
    ...
    struct
    {
        unsigned int  interrupt4_mask : 8;  /* bit[0-7] : 中断屏蔽。
                                                          0：屏蔽相应管脚的中断；
                                                          1：使能相应管脚的中断。 */
        unsigned int  reserved        : 24; /* bit[8-31]: 保留。 */
    } reg;
...
/*****************************************************************************
 结构名    : SOC_GPIO_GPIOMIS4_UNION
 结构说明  : GPIOMIS4 寄存器结构定义。地址偏移量:0x538，初值:0x00000000，宽度:32
 寄存器说明: 屏蔽后中断状态寄存器。
*****************************************************************************/
#ifndef __SOC_H_FOR_ASM__
typedef union
{
    ...
    struct
    {
        unsigned int  masked_interrupt4_status : 8;  /* bit[0-7] : 屏蔽后中断状态。
                                                                   0：该位对应的管脚没有触发中断；
                                                                   1：该位对应的管脚已经触发中断。 */
        unsigned int  reserved                 : 24; /* bit[8-31]: 保留。 */
    } reg;
