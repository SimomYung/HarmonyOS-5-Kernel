...
static inline unsigned long bsp_sc_alloc_page(unsigned int pgid, gfp_t gfp_mask)
{
    return __get_free_page(gfp_mask);
}

static inline void bsp_sc_free_page(unsigned int pgid, unsigned long addr)
{
    free_page(addr);
}

static inline int bsp_sc_region_config(unsigned long long phy_addr, unsigned int size)
{
    return 0;
}
#endif
...
#else
static inline void bsp_sc_invalid_cache(unsigned long long phy_addr, unsigned int size) {;}
#endif
