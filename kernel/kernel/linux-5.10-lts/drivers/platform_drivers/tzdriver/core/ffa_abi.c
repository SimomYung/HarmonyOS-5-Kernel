 ...
 * Description: functions for ffa settings
 *
...
{
	(void)ffa_unregister(&tz_ffa_driver);
}

void smc_req(struct smc_in_params *in, struct smc_out_params *out, uint8_t wait)
{
	(void)ffa_forward_call(in, out, wait);
}

static void convert_smc_param_to_ffa_param(const struct smc_in_params *in_param, struct ffa_send_direct_data *ffa_param)
{
...
static void convert_ffa_param_to_smc_param(const struct ffa_send_direct_data *ffa_param, 
...
static uint32_t g_smc_send_fail_count = 0;
#define FFA_LOG_FRQ 100000

int ffa_forward_call(const struct smc_in_params *in_param, struct smc_out_params *out_param, uint8_t wait)
{
	int ret;
	struct ffa_send_direct_data ffa_param = {0};
	...
	if (ret != 0) {
		g_smc_send_fail_count++;
		if (g_smc_send_fail_count % FFA_LOG_FRQ == 1) {
			tloge("failed to call! ret %d oret %lu reason %lu count %u\n", ret,
				out_param->ret, out_param->exit_reason, g_smc_send_fail_count);
		}
	}
	return ret;
}
