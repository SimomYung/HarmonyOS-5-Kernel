...
{
	if (param_type == TEEC_MEMREF_PARTIAL_INPUT || param_type == TEEC_MEMREF_PARTIAL_OUTPUT ||
	...
	uint32_t buffer_size, unsigned int index, struct tc_op_params *op_params)
{
	if (shared_mem->mem_type == RESERVED_TYPE) {
		...
		void *tmp_buffer_addr = NULL;

		sync_user_cache_to_mem((uint64_t)(uintptr_t)buffer_addr,
			(uint64_t)(uintptr_t)buffer_addr + buffer_size);

		tmp_buffer_addr = mailbox_copy_alloc(buffer_addr, buffer_size);
	...
	uint32_t param_type, unsigned int index)
{
	...
	if (fill_shared_mem_info((uint64_t)start_vaddr, pages_no, offset, buffer_size, (uint64_t)buff) != 0) {
		mailbox_free(buff);
		return -EFAULT;
	}
	...
	struct tc_op_params *op_params)
{
	...
	op_params->mb_pack->operation.paramtypes =
		...
		op_params->trans_paramtype[1], op_params->trans_paramtype[2],
	...
	const struct tc_op_params *op_params, unsigned int index)
{
	...
	if (operation->mb_buffer[index] && orig_size >= buffer_size) {
		void *buffer_addr =
			...
			client_param->memref.offset);
		invalid_user_cache((uint64_t)(uintptr_t)buffer_addr,
			(uint64_t)(uintptr_t)buffer_addr + buffer_size);
	...
	struct tc_op_params *op_params)
{
	...
	smc_cmd->ca_pid = (uint32_t)CURRENT_PID;
	smc_cmd->pid = (uint32_t)CURRENT_TGID;
	...
	/* if smc from container CA, set login_method to TEEC_LOGIN_CONTAINER_IDENTIFY */
	if (call_params->dev->kernel_api == TEE_REQ_FROM_CONTAINER_USER_MODE)
		smc_cmd->login_method = TEEC_LOGIN_CONTAINER_IDENTIFY;
	...
	const struct tc_op_params *op_params)
{
	if (call_params->dev->pub_key_len == sizeof(uint32_t) &&
		...
		CURRENT_MM && ((call_params->flags & TC_CALL_GLOBAL) != 0))
		return true;
...
#define TIME_M 1000
#define LOG_TIME_TH 50
uint64_t g_last_print_time = 0;
static bool time_exceeds_log_threshold(void)
{
	struct time_spec now_time = {0};

	get_time_spec(&now_time);
	uint64_t curr_time = now_time.ts.tv_sec * TIME_M + now_time.ts.tv_nsec / TIME_M / TIME_M;
	if (curr_time - g_last_print_time > LOG_TIME_TH) {
		g_last_print_time = curr_time;
		return true;
	}
	return false;
}

static void ca_auth_dfx_dump(const struct tc_ns_dev_file *dev_file, int ret)
{
	if (ret != (int)TEE_ERROR_CA_CALLER_ACCESS_DENIED && ret != (int)TEEC_ERROR_ACCESS_DENIED)
		return;
	uint32_t pkg_len;
	uint32_t pub_len;
#ifdef CONFIG_AUTH_HASH
	uint32_t i;
	if (dev_file->pkg_name_len != SHA256_DIGEST_LENGTH) {
		tloge("invaild pkg_name len=0x%x\n", dev_file->pkg_name_len);
		return;
	}
	for (i = 0; i < dev_file->pkg_name_len; i++)
		tloge("auth info hash: [%d]=0x%x\n", i, dev_file->pkg_name[i]);
	(void)pkg_len;
	(void)pub_len;
#elif defined(CONFIG_AUTH_SUPPORT_UNAME)
	pkg_len = strnlen((const char *)dev_file->pkg_name, sizeof(dev_file->pkg_name));
	pub_len = strnlen((const char *)dev_file->pub_name, sizeof(dev_file->pub_key));
	if (pkg_len == 0 || pkg_len >= sizeof(dev_file->pkg_name) ||
		pub_len == 0 || pub_len >= sizeof(dev_file->pub_key)) {
		tloge("invalid pkg or pub len, 0x%x, 0x%x\n", pkg_len, pub_len);
		return;
	}
	tloge("auth info: pkg=%s, len=0x%x, pub=%s, len=0x%x\n",
		dev_file->pkg_name, dev_file->pkg_name_len, dev_file->pub_key, dev_file->pub_key_len);
#else
	pkg_len = strnlen((const char *)dev_file->pkg_name, sizeof(dev_file->pkg_name));
	if (pkg_len == 0 || pkg_len >= sizeof(dev_file->pkg_name)) {
		tloge("invalid pkg_name len =0x%x\n", pkg_len);
		return;
	}
	tloge("auth info: pkg=%s, len=0x%x, pub=%d, len=0x%x\n",
		dev_file->pkg_name, dev_file->pkg_name_len, *((uint32_t *)(dev_file->pub_key)), dev_file->pub_key_len);
	(void)pub_len;
#endif
}
	...
	struct tc_op_params *op_params, int tee_ret)
{
	...
	if (tee_ret != 0) {
		...
		ca_auth_dfx_dump(call_params->dev, tee_ret);
		ret = EFAULT;
...
int tc_client_call(const struct tc_call_params *call_params)
{
	...
	op_params.smc_cmd->uid = current_uid().val;
	if (call_params->context->param_types != 0) {
