	...
	for (k = 0; k < g_mb_count; k++) {
		...
		for (i = (unsigned int)order; i <= (unsigned int)g_max_oder; i++) {
			...
			for (j = (unsigned int)order; j < i; j++) {
				...
				list_add_tail(&new_page->node, &g_m_zone[k]->free_areas[j].page_list);
			}
			list_del(&pos->node);
			page_addr = mailbox_page_address(pos->page);
			addr = (void *)page_addr;
			tag = true;
			break;
		}
...
/* mailbox_free has checked g_m_zone */
...
void mailbox_free(const void *ptr)
{
	...
	struct mb_page_t *self = NULL, *buddy = NULL;
...
#define MAILBOX_DBG_ENTRY_COUNT_MAX 1000
...
static unsigned int mb_dbg_add_entry(void *ptr)
{
	...
	if (g_mb_dbg_entry_count >= MAILBOX_DBG_ENTRY_COUNT_MAX) {
		tloge("dbg entry count is reaching the max\n");
		return 0;
	}
...
static int mailbox_register(const void *mb_pool, unsigned int size)
{
	...
	operation->params[0].value.a = mailbox_alloc_virt_to_phys((uintptr_t)mb_pool);
	operation->params[0].value.b =
		(uint64_t)mailbox_alloc_virt_to_phys((uintptr_t)mb_pool) >> ADDR_TRANS_NUM;
	smc_cmd->operation_phys = mailbox_alloc_virt_to_phys((uintptr_t)operation);
	smc_cmd->operation_h_phys =
		(uint64_t)mailbox_alloc_virt_to_phys((uintptr_t)operation) >> ADDR_TRANS_NUM;
...
int re_register_mailbox(void)
{
	...
	for (; i < g_mb_count; i++) {
		page_addr = mailbox_page_address(g_m_zone[i]->all_pages);
		(void)memset_s((void *)page_addr, MAILBOX_POOL_SIZE, 0, MAILBOX_POOL_SIZE);
...
#if defined(CONFIG_TZ_RESERVED_MAILBOX)
	// if UEFI exists, mailbox buffer has been registered during UEFI
	if (check_uefi_enable()) {
		g_mb_count = 1;
		goto zone_init;
	}
#endif
...
static mailbox_page_t **g_all_pages = NULL;

int mailbox_mempool_init(void)
{
	uint32_t pool_count, i;
	int ret = 0;

	if (MAILBOX_POOL_COUNT < 1 || MAILBOX_POOL_COUNT > MAILBOX_POOL_MAX) {
		tloge("MAILBOX_POOL_COUNT define %u is invalid, cannot be greater than %u\n",
			(uint32_t)MAILBOX_POOL_COUNT, MAILBOX_POOL_MAX);
		return -EINVAL;
	}
	...
	g_all_pages = kzalloc(sizeof(mailbox_page_t **) * (uint32_t)MAILBOX_POOL_COUNT, GFP_KERNEL);
	if (ZERO_OR_NULL_PTR((unsigned long)(uintptr_t)g_all_pages))
		return -ENOMEM;

	for (pool_count = 0; pool_count < (uint32_t)MAILBOX_POOL_COUNT; pool_count++) {
		g_all_pages[pool_count] = mailbox_alloc_pages(g_max_oder);
		if (!g_all_pages[pool_count]) {
	...
	ret = mailbox_init(pool_count, g_all_pages);
	if (ret != 0) {
		tloge("mailbox init failed\n");
		goto clear2;
	}

	// free unregistered mailbox
	for (i = g_mb_count; i < pool_count; i++) {
		mailbox_free_pages(g_all_pages[i], g_max_oder);
		g_all_pages[i] = NULL;
	}
