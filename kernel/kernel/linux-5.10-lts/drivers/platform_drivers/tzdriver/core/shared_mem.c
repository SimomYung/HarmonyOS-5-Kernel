...
static bool g_reserved_mailbox_buffer = false;
bool get_reserved_mailbox_buffer(void)
{
	return g_reserved_mailbox_buffer;
}
...
{
	int rc;
	const struct device_node *np = nodep;

	rc = of_property_read_u64(np, "tz_shmem_cmd_addr", (u64 *)&g_cmd_mem_paddr);
	if (rc != 0) {
		tloge("read tz_shmem_cmd_addr failed\n");
		return -ENODEV;
	}

	rc = of_property_read_u64(np, "tz_shmem_cmd_size", (u64 *)&g_cmd_mem_size);
	if (rc != 0 || g_cmd_mem_size < CMD_MEM_MIN_SIZE) {
		tloge("read tz_shmem_cmd_size failed or size too short\n");
		return -ENODEV;
	}

	rc = of_property_read_u64(np, "tz_shmem_mailbox_addr", (u64 *)&g_mailbox_paddr);
	if (rc != 0) {
		tloge("read tz_shmem_mailbox_addr failed\n");
		return -ENODEV;
	}

	rc = of_property_read_u64(np, "tz_shmem_mailbox_size", (u64 *)&g_mailbox_size);
	if (rc != 0 || g_mailbox_size < MAILBOX_POOL_SIZE + OPERATION_MEM_MIN_SIZE) {
		tloge("read tz_shmem_mailbox_size failed or size too short\n");
		return -ENODEV;
	}

	rc = of_property_read_u64(np, "tz_shmem_spi_addr", (u64 *)&g_spi_mem_paddr);
	if (rc != 0) {
		tloge("read tz_shmem_spi_addr failed\n");
		return -ENODEV;
	}

	rc = of_property_read_u64(np, "tz_shmem_spi_size", (u64 *)&g_spi_mem_size);
	if (rc != 0 || g_spi_mem_size < SPI_MEM_MIN_SIZE) {
		tloge("read tz_shmem_spi_size failed or size too short\n");
		return -ENODEV;
	}

	rc = of_property_read_u64(np, "tz_shmem_log_addr", (u64 *)&g_log_mem_paddr);
	if (rc != 0) {
		tloge("read tz_shmem_log_addr failed\n");
		return -ENODEV;
	}

	rc = of_property_read_u64(np, "tz_shmem_log_size", (u64 *)&g_log_mem_size);
...
mailbox_page_t *mailbox_alloc_pages(int order)
{
	...
	remap_addr = ioremap_cache(g_mailbox_paddr, g_mailbox_size);
	g_shmem_start_virt = (uintptr_t)remap_addr;
...
{
	void *remap_addr = ioremap_cache(g_log_mem_paddr, g_log_mem_size);
	uint64_t log_vaddr = (uint64_t)(uintptr_t)remap_addr;
	if (log_vaddr == 0) {
...
{
	void *remap_addr = ioremap_cache(g_spi_mem_paddr, g_spi_mem_size);
	uint64_t spi_vaddr = (uint64_t)(uintptr_t)remap_addr;
	if (spi_vaddr == 0) {
...
uint64_t g_mailbox_size;
static mailbox_page_t *g_mailbox_page;
static uintptr_t g_mailbox_vaddr;
static uintptr_t g_page_offset;
...
mailbox_page_t *mailbox_alloc_pages(int order)
{
	...
	g_mailbox_paddr = RESERVED_SECOS_TZDRV_BASE;
	g_mailbox_size = RESERVED_SECOS_TZDRV_SIZE;
	if (g_mailbox_paddr == 0 || g_mailbox_size == 0) {
		tloge("mailbox phy or mailbox size is error\n");
		goto clear;
	}
	g_reserved_mailbox_buffer = true;
	map_addr = ioremap_cache(g_mailbox_paddr, g_mailbox_size);
	g_mailbox_vaddr = (uintptr_t)map_addr;
	if (g_mailbox_vaddr == 0) {
		tloge("io remap for reserved mailbox buffer failed\n");
		goto clear;
	}
	if (memset_s((void *)g_mailbox_vaddr, g_mailbox_size, 0, g_mailbox_size) != 0)
		tloge("Caution, reserved mailbox memset 0 fail\n");
	g_mailbox_page[0] = (mailbox_page_t)g_mailbox_vaddr;
...
void mailbox_free_pages(mailbox_page_t *pages, int order)
{
	...
	(void)order;
	iounmap((void __iomem*)(uintptr_t)g_mailbox_vaddr);
	kfree(pages);
	g_mailbox_page = NULL;
	g_mailbox_vaddr = NULL;
}
...
uintptr_t mailbox_virt_to_phys(uintptr_t addr)
{
	if (!addr || addr < g_mailbox_vaddr || addr > g_mailbox_vaddr + g_mailbox_size)
		return 0;
	return g_mailbox_paddr + (addr - g_mailbox_vaddr);
}

mailbox_page_t *mailbox_virt_to_page(uint64_t ptr)
{
	if (!ptr || ptr < g_mailbox_vaddr || ptr > g_mailbox_vaddr + g_mailbox_size)
		return NULL;

	return &g_mailbox_page[(ptr - g_mailbox_vaddr) / g_page_offset];
}

struct tc_ns_smc_queue *get_reserved_mailbox_vaddr(void)
{
	return (struct tc_ns_smc_queue *)g_mailbox_vaddr;
}

uintptr_t mailbox_alloc_virt_to_phys(uintptr_t addr)
{
	if (!addr)
		return 0;
