...
#if defined(DYNAMIC_DIR_CONFIGURABLE)
#if defined(DYNAMIC_DRV_DIR) || defined(DYNAMIC_CRYPTO_DRV_DIR) || defined(DYNAMIC_SRV_DIR)
/* Append Dynamic Drive Path */
static char *dynamic_drv_dir = NULL;
static char *dynamic_srv_dir = NULL;
static char *dynamic_crypto_drv_dir = NULL;
module_param(dynamic_drv_dir, charp, 0664);
module_param(dynamic_srv_dir, charp, 0664);
module_param(dynamic_crypto_drv_dir, charp, 0664);
#endif
#endif
	...
	uint8_t **cert_buffer, unsigned int *cert_buffer_size)
{
	...
	*cert_buffer_size += CA_USERID_SIZE;
#endif
...
#ifdef CONFIG_TA_GET_CA_USERID
	uint32_t user_id = *(uint32_t *)cert_buffer;
	cert_buffer += CA_USERID_SIZE;
#endif
...
#ifdef CONFIG_TA_GET_CA_USERID
	uint8_t *user_id_pos = (uint8_t *)dev_file->pkg_name + dev_file->pkg_name_len;
	*(uint32_t *)user_id_pos = user_id;
...
/* set mm when register and check when unregister, to avoid PUAF */
static int agent_mm_valid(struct tc_ns_dev_file *dev_file)
{
	if (dev_file->mm != CURRENT_MM) {
		tloge("check agent mm not valid\n");
		return -EPERM;
	}
	return 0;
}
...
int public_ioctl(const struct file *file, unsigned int cmd, unsigned long arg, bool is_from_client_node)
{
	...
	case TC_NS_CLIENT_IOCTL_UNREGISTER_AGENT:
		if (ioctl_check_agent_owner(dev_file, (unsigned int)arg, nsid) != 0 || agent_mm_valid(dev_file) != 0)
			return -EPERM;
...
#if !defined(CONFIG_DISABLE_SVC) && !defined(CONFIG_LIBLINUX_CDC_RDA)
...
#if defined(DYNAMIC_DRV_DIR) || defined(DYNAMIC_CRYPTO_DRV_DIR) || defined(DYNAMIC_SRV_DIR)
#if defined(DYNAMIC_DIR_CONFIGURABLE)
	tz_load_dynamic_dir(dynamic_drv_dir, dynamic_srv_dir, dynamic_crypto_drv_dir);
#else
	tz_load_dynamic_dir();
#endif /* DYNAMIC_DIR_CONFIGURABLE */
#endif
