...
#define USB_SCSI_MAX_QUEUE_TIME 10
#endif
...
static void scsi_softirq_done(struct request *rq)
{
...
#endif
#if defined(CONFIG_MAS_BLK) || defined(CONFIG_USB_CONFIG_PRODUCT_PC)
	...
	case ADD_TO_MLQUEUE:
#ifdef CONFIG_USB_CONFIG_PRODUCT_PC
        if (cmd_age >= USB_SCSI_MAX_QUEUE_TIME && shost != NULL &&
            strstr(shost->hostt->name, "usb")) {
            pr_err("usb storage device hardware err\n");
            scsi_finish_command(cmd);
            break;
        }
#endif
...
extern int sd_proxy_block_cmd(bool is_blocked);
#endif
...
#ifdef CONFIG_LIBLINUX_UDK_UFS
	if (shost->hostt->name == "ufshcd")
		sd_proxy_block_cmd(true);
#endif
}
