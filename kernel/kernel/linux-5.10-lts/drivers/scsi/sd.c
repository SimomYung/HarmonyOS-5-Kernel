...
static int sd_revalidate_disk(struct gendisk *disk)
{
	...
	if (sdkp->media_present) {
		...
		if (sdp->host->hostt->name && strcmp(sdp->host->hostt->name, "ufshcd") == 0) {
			blk_queue_flag_set(QUEUE_FLAG_NONROT, q);
			blk_queue_flag_clear(QUEUE_FLAG_ADD_RANDOM, q);
		}
#endif
...
static int sd_probe(struct device *dev)
{
	...
	trace_ldk_rvh_sd_ida_alloc(&index);
#else
 ...
 out_free_index:
#ifdef CONFIG_LIBLINUX_UDK_UFS
	trace_ldk_rvh_sd_ida_free(index);
#else
...
#ifdef CONFIG_LIBLINUX_UDK_UFS
	trace_ldk_rvh_sd_ida_free(sdkp->index);
#else
...
static int sd_suspend_common(struct device *dev, bool ignore_stop_errors)
{
	...
	if (sdkp->WCE && sdkp->media_present) {
		...
		sd_printk(KERN_NOTICE, sdkp, "Synchronizing SCSI cache finish\n");
		if (ret) {
	...
	if (sdkp->device->manage_start_stop) {
		...
		sd_printk(KERN_NOTICE, sdkp, "Stopping disk finish\n");
