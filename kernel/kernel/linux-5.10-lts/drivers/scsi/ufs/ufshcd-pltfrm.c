...
static int ufshcd_pltfrm_restore(struct device *dev)
{
	...
	if (is_mcq_enabled(hba)) {
		for (i = 0; i < hba->nr_hw_queues; i++) {
			...
			hwq->sq_tp_slot = hwq->sq_hp_slot;
			hwq->cq_hp_slot = hwq->cq_tp_slot;
			dev_err(hba->dev, "----------- ufs mcq dump start id 0x%08x -------\n", hwq->id);
			...
			dev_err(hba->dev, "REG_SQHP: 0x%08x\n", readl(hba->mmio_base + MCQ_HW_SQD_BASE + i * MCQ_HW_SQD_SCALE + REG_SQHP));
			dev_err(hba->dev, "REG_SQTP: 0x%08x\n", readl(hba->mmio_base + MCQ_HW_SQD_BASE + i * MCQ_HW_SQD_SCALE + REG_SQTP));
			dev_err(hba->dev, "REG_CQHP: 0x%08x\n", readl(hba->mmio_base + MCQ_HW_CQD_BASE + i * MCQ_HW_CQD_SCALE + REG_CQHP));
			dev_err(hba->dev, "REG_CQTP: 0x%08x\n", readl(hba->mmio_base + MCQ_HW_CQD_BASE + i * MCQ_HW_CQD_SCALE + REG_CQTP));
			dev_err(hba->dev, "REG_CQIE: 0x%08x\n", readl(hba->mmio_base + MCQ_HW_CQIS_BASE + i * MCQ_HW_CQIS_SCALE + REG_CQIE));
			dev_err(hba->dev, "REG_CQIS: 0x%08x\n", readl(hba->mmio_base + MCQ_HW_CQIS_BASE + i * MCQ_HW_CQIS_SCALE + REG_CQIS));
			dev_err(hba->dev, "----------- ufs mcq dump queue  end --------\n");
		}
...
void ufshcd_pltfrm_shutdown(struct platform_device *pdev)
{
	...
	if (is_mcq_enabled(hba)) {
		(void)ufsmcq_check_inflt_reqs(hba, SHUTDOWN_TIMEOUT);
	}
#endif
