...
inline void ufshcd_send_command(struct ufs_hba *hba, unsigned int task_tag)
{
	...
	if (UFS_UPIU_RPMB_WLUN == lrbp->lun)
...
#ifdef CONFIG_DFA_MCQ
	if (is_dfa_enabled(hba_camera_tmp)) {
		dev_err(hba_camera_tmp->dev, "%s:DFA tele close autoh8\n", __func__);
...
#ifdef CONFIG_DFA_MCQ
	if (is_dfa_enabled(hba_camera_tmp)) {
		dev_err(hba_camera_tmp->dev, "%s:DFA tele open autoh8\n", __func__);
