...
#include "platform_include/basicplatform/linux/ufs/hufs_bkops_enhanced.h"
...
#endif
#ifdef CONFIG_ARMPC_UFS_LED
#include "ufs_led_dev.h"
#endif
...
static DEFINE_MUTEX(g_tmp_hba_lock);
static struct ufs_hba *hba_tmp = NULL;
...
#if defined(CONFIG_SCSI_HUFS) && defined(CONFIG_LIBLINUX_UFS_ACE_HOST)
		...
		unsigned int tag, const char *str)
{
	...
	if (cmd && trace_ufshcd_upiu_enabled())
...
static void __ufshcd_release(struct ufs_hba *hba)
{
	...
	if (hba->clk_gating.active_reqs || hba->clk_gating.is_suspended ||
	    hba->ufshcd_state != UFSHCD_STATE_OPERATIONAL ||
#ifdef CONFIG_UFS_4_0_HC
	(is_mcq_enabled(hba) && atomic_read(&hba->mcq_inflt_reqs)) ||
#endif
#ifdef CONFIG_SCSI_HUFS
		hba->lrb_in_use || hba->outstanding_tasks ||
#else
...
static inline int ufshcd_hba_capabilities(struct ufs_hba *hba)
{
	...
	atomic_set(&hba->mcq_inflt_reqs, 0);
#ifdef CONFIG_UFS_4_0_HC
	if (hba->mcq_sup)
		hba->nutrs = MCQ_TAG_NUM;
#endif
...
#ifdef CONFIG_DFA_MCQ
	if (is_dfa_enabled_backdata(hba))
		return 0;
#endif
...
#endif
#if defined(CONFIG_MAS_BLK) || defined(CONFIG_MAS_VIRTIO_BLK_BE)
...
void ufshcd_prepare_utp_verify_upiu(struct ufshcd_lrb *lrbp)
{
	struct utp_upiu_req *ucd_req_ptr = lrbp->ucd_req_ptr;
	static int start_lba = 0;
	...
	if (start_lba > 40960)
		start_lba = 0;
	/* command descriptor fields */
	ucd_req_ptr->header.dword_0 = UPIU_HEADER_DWORD(UPIU_TRANSACTION_COMMAND, 0x42, 3, lrbp->task_tag);
	/* clear rest of the fields of basic header */
	ucd_req_ptr->header.dword_1 = 0;
	ucd_req_ptr->header.dword_2 = 0;

	ucd_req_ptr->sc.cdb[0] = VERIFY;
	ucd_req_ptr->sc.cdb[2] = (unsigned char)((start_lba & 0xff000000) >> 24);
	ucd_req_ptr->sc.cdb[3] = (unsigned char)((start_lba & 0x00ff0000) >> 16);
	ucd_req_ptr->sc.cdb[4] = (unsigned char)((start_lba & 0x0000ff00) >> 8);
	ucd_req_ptr->sc.cdb[5] = (unsigned char)(start_lba & 0xff);
	ucd_req_ptr->sc.cdb[7] = 0;
	ucd_req_ptr->sc.cdb[8] = 1;
 ...
 * @lrbp: pointer to local reference block
 */
int ufshcd_compose_devman_upiu(struct ufs_hba *hba,
				      struct ufshcd_lrb *lrbp)
{
	u8 upiu_flags;
	int ret = 0;
	...
	else if (hba->dev_cmd.type == DEV_CMD_TYPE_VERIFY)
		ufshcd_prepare_utp_verify_upiu(lrbp);
	else
		ret = -EINVAL;
...
void ufshcd_disable_dev_tmt_cnt_no_irq(struct ufs_hba *hba)
{
...
static int ufshcd_queuecommand(struct Scsi_Host *host, struct scsi_cmnd *cmd)
{
	...
	if (is_mcq_enabled(hba))
		return ufsmcq_queuecommand(host, cmd);
#endif
 ...
 * @lrbp: pointer to local reference block
 */
int ufshcd_dev_cmd_completion(struct ufs_hba *hba, struct ufshcd_lrb *lrbp)
{
	int resp;
	int err = 0;
	...
	case UPIU_TRANSACTION_RESPONSE:
		err = lrbp->ucd_rsp_ptr->header.dword_1 & 0xff;
		if (err)
			dev_err(hba->dev, "%s: dword_1[3]=0x%x (2-error, 0-succ), dword_1=0x%x\n",
					 __func__, err, lrbp->ucd_rsp_ptr->header.dword_1);
		break;
	default:
		err = -EINVAL;
...
#ifdef CONFIG_DFA_MCQ
	if (is_dfa_enabled(hba))
		return dfa_exec_dev_cmd(hba, cmd_type, timeout);
#endif

#ifdef CONFIG_UFS_4_0_HC
	if (is_mcq_enabled(hba))
		return ufsmcq_exec_dev_cmd(hba, cmd_type, timeout);
#endif
...
#if defined(CONFIG_SCSI_UFS_VELA61_VCMD) && defined(CONFIG_HUFS_HC)
...
#if defined(CONFIG_LIBLINUX_UDK_UFS) && defined(CONFIG_HUFS_HC_CORE_UTR)
	utrdl_size = (sizeof(struct hufs_utp_transfer_req_desc) *
		      ((unsigned int)hba->nutrs + CORE_SLOT_RESERVED_NUM));
#else
	utrdl_size = (sizeof(struct hufs_utp_transfer_req_desc) *
...
{
	ktime_t current_time = ktime_get() / NANOSECOND_TO_MILLISECOND;

	mutex_lock(&g_tmp_hba_lock);

	if (hba_tmp == NULL)
		goto out;

#ifdef CONFIG_DFA_MCQ
	if (is_dfa_enabled(hba_tmp)) {
		dev_err(hba_tmp->dev, "%s:DFA tele close autoh8\n", __func__);
		dfa_auto_h8_switch(false);
		HWDUBAI_LOGE("DUBAI_TAG_TELE_CALL_UFS_AH8", "HAPPENTIME=%lld TAG=0", current_time);
		goto out;
	}
#endif

	if (ufshcd_is_auto_hibern8_allowed(hba_tmp)) {
		dev_err(hba_tmp->dev, "%s:tele close autoh8\n", __func__);
		ufshcd_disable_auto_hibern8(hba_tmp);
		HWDUBAI_LOGE("DUBAI_TAG_TELE_CALL_UFS_AH8", "HAPPENTIME=%lld TAG=0", current_time);
	}

out:
	mutex_unlock(&g_tmp_hba_lock);
	return;
}
...
#ifdef CONFIG_DFA_MCQ
	if (is_dfa_enabled(hba_tmp)) {
		dev_err(hba_tmp->dev, "%s:DFA tele open autoh8\n", __func__);
		dfa_auto_h8_switch(true);
		HWDUBAI_LOGE("DUBAI_TAG_TELE_CALL_UFS_AH8", "HAPPENTIME=%lld TAG=1", current_time);
		goto out;
	}
#endif

	if (ufshcd_is_auto_hibern8_allowed(hba_tmp)) {
		dev_err(hba_tmp->dev, "%s:tele open autoh8\n", __func__);
		ufshcd_enable_auto_hibern8(hba_tmp);
		HWDUBAI_LOGE("DUBAI_TAG_TELE_CALL_UFS_AH8", "HAPPENTIME=%lld TAG=1", current_time);
	}
...
#ifdef CONFIG_DFA_MCQ
	if (is_dfa_enabled(hba_tmp)) {
		dfa_mode = hba_tmp->dhba.workmode;
		mutex_unlock(&g_tmp_hba_lock);
		return dfa_mode;
	} else {
		dev_err(hba_tmp->dev, "%s: dfa bypass\n", __func__);
		goto out;
	}
#endif
...
int ufshcd_vcmd_send_data(struct dfa_vcmd_vars *vars)
{
	int ret;
...
static void ufshcd_iolimit_update_h8(u32 ahit)
{
	...
	if (ufshcd_is_auto_hibern8_allowed(hba_iolimit_tmp)) {
		...
		ufshcd_auto_hibern8_update(hba_iolimit_tmp, ahit);
	}
...
{
	ufshcd_iolimit_update_h8(UFS_ACGROUP_IOLIMIT_HIBERN8_OPT_TIMER);
}
EXPORT_SYMBOL_GPL(ufshcd_iolimit_wakeup_and_update_h8);

void ufshcd_iolimit_reset_h8(void)
{
	ufshcd_iolimit_update_h8(UFS_AHIT_AUTOH8_TIMER);
}
...
static int ufshcd_get_max_pwr_mode(struct ufs_hba *hba)
{
	...
	if (!pwr_info->lane_rx || !pwr_info->lane_tx) {
		...
		__func__,
		(hba->ufs_unipro_base + (PA_ACTIVETXDATALANES * 4)), readl(hba->ufs_unipro_base + PA_ACTIVETXDATALANES * 4),
		(hba->ufs_unipro_base + (PA_ACTIVERXDATALANES * 4)), readl(hba->ufs_unipro_base + PA_ACTIVERXDATALANES * 4),
		(hba->ufs_unipro_base + (PA_CONNECTEDTXDATALANES * 4)), readl(hba->ufs_unipro_base + PA_CONNECTEDTXDATALANES * 4),
		(hba->ufs_unipro_base + (PA_CONNECTEDRXDATALANES * 4)), readl(hba->ufs_unipro_base + PA_CONNECTEDRXDATALANES * 4),
		(hba->ufs_unipro_base + (PA_PWRMODE * 4)), readl(hba->ufs_unipro_base + PA_PWRMODE * 4));
		return -EINVAL;
	}
...
#ifdef CONFIG_DFA_MCQ
	/*
	 * Dfa doesn't reinit ufs device in kernel. Dfa core and device
	 * have been running for some time in bootloader, may not response
	 * nop out in 200ms. Acore already try to commuicate with Dfa core
	 * in dfa_queue_disable, so NOP OUT can be skipped in dfa_init.
	 */
	if (is_dfa_enabled(hba)) {
		dev_err(hba->dev, "%s: NOP OUT skip\n", __func__);
		return 0;
	}
#endif
...
static int ufshcd_slave_configure(struct scsi_device *sdev)
{
	...
	if (strncmp(hba->model, UFS_PRODUCT_NAME_THOR950_DFA, UFS_PRODUCT_NAME_THOR950_LEN) == 0) {
		hufs_enhanced_manual_bkops_config(sdev);
	} else {
	...
	ufshcd_device_support_cp(sdev);
#endif
	ufstt_set_sdev(sdev);
#ifdef CONFIG_SCSI_UFS_VELA61_VCMD
	ufsfsr_set_sdev(sdev, hba);
#endif
#ifdef CONFIG_DFA_MCQ
	dfa_set_sdev(sdev);
#endif
...
void ufshcd_compl_one_task(struct ufs_hba *hba, int task_tag)
{
	...
	if (cmd && lrbp->command_type != UTP_CMD_TYPE_DEV_MANAGE &&
		...
		if (is_mcq_enabled(hba))
			spin_lock_irqsave(hba->host->host_lock, flags);
		if ((opcode == READ_10) || (opcode == WRITE_10))
			hba->ufs_inflight_byte -= cmd->sdb.length;
		if (is_mcq_enabled(hba))
			spin_unlock_irqrestore(hba->host->host_lock, flags);
#endif
#ifdef CONFIG_HUAWEI_DSM_IOMT_UFS_HOST
...
#endif
		spin_unlock_irqrestore(hba->host->host_lock, flags);
#ifdef CONFIG_SCSI_UFS_RESET_FLUSH
		hba->ufshcd_reset_flush = 1;
		err = ufshcd_send_scsi_sync_cache(hba, hba->sdev_ufs_device, UFS_ACTIVE_PWR_MODE);
		if (err) {
			dev_err(hba->dev, "%s flush cache fail\n", __func__);
		} else {
			dev_err(hba->dev, "%s flush cache succ\n", __func__);
		}
		hba->ufshcd_reset_flush = 0;
#endif
...
static int ufshcd_eh_device_reset_handler(struct scsi_cmnd *cmd)
{
	...
	for_each_set_bit(pos, ufshcd_outstanding_reqs(hba), hba->nutrs) {
...
#ifdef CONFIG_DFA_MCQ
	if (is_dfa_enabled(hba))
		return dfa_abort(cmd);
#endif

#ifdef CONFIG_UFS_4_0_HC
	if (is_mcq_enabled(hba))
		return ufsmcq_abort(cmd);
#endif
...
#ifdef CONFIG_UFS_4_0_HC
static void ufs_update_lmt_ratio(struct ufs_hba *hba)
{
#ifdef CONFIG_MAS_BLK
	struct request_queue *q = hba->cmd_queue;

	if (q->mas_queue_ops && q->mas_queue_ops->blk_lmt_ratio_update)
		q->mas_queue_ops->blk_lmt_ratio_update(q, hba->dev_info.bqueuedepth / DEFAULT_ACTIVE_CMDS);
#endif
}
#endif
...
static int ufshcd_probe_hba(struct ufs_hba *hba, bool async)
{
	...
	hba->ufshcd_state = UFSHCD_STATE_RESET;
#if defined(CONFIG_SCSI_UFS_LOW_POWER_VOTE)
	hba->ufs_inflight_byte = 0;
	hba->ufs_ccbus_threshold = UFS_CCBUS_LIMIT_BUSY;
#elif defined(CONFIG_UFS_LOW_POWER_VOTE_STATUS_SYNC)
	ufshcd_clear_status_sync_info(hba);
#endif

	if (dsm_ufs_disable_uic_err())
		dev_err(hba->dev, "dsm uic disable bit already be set.\n");
#if defined(CONFIG_SCSI_UFS_PICTOR) || defined(CONFIG_SCSI_UFS_AURIGA)
	ret = ufs_fpga_spi_driver_probe(hba);
	if (ret)
		dev_err(hba->dev, "ufs_fpga_spi_driver_probe error. maybe already be set.\n");
#endif
...
#ifdef CONFIG_DFA_MCQ
	hba->dhba.need_report_bit_error = dfa_is_need_report_bit_error(hba);
#endif
}
#else
static void ufshcd_async_scan(void *data, async_cookie_t cookie)
...
#ifdef CONFIG_LIBLINUX_UDK_UFS
static struct scsi_device *ufshcd_find_sdev_by_lun(struct Scsi_Host *shost, int lun)
{
	struct scsi_device *sdev;
...
static int ufshcd_sync_cache(struct scsi_device *sdev, struct scsi_cmnd *scmd)
{
	...
	sdev_printk(KERN_INFO, sdev, "Synchronizing SCSI cache(%d)\n", scmd->tag);

	cmd[0] = SYNCHRONIZE_CACHE;
	res = scsi_send_eh_cmnd(scmd, cmd, 10, SCSI_DEFAULT_EH_TIMEOUT, 0);
	if (res != SUCCESS) {
		sdev_printk(KERN_INFO, sdev, "Synchronize Cache(10) failed(%d)", res);
	}
	msleep(100);
	return res;
}

static int ufshcd_stu_and_tur(struct scsi_device *sdev)
{
	struct scsi_cmnd *cmd;
	struct request *req;
	int ret;
	...
	ret = scsi_eh_try_stu(cmd);
	pr_info("scsi: stu returned %d\n", ret);

	ret = scsi_eh_tur(cmd);
	pr_info("scsi: tur returned %d\n", ret);

	blk_put_request(req);
	return SUCCESS;
}

static int ufshcd_remote_lun_reset(struct Scsi_Host *shost, int lun, struct scsi_cmnd *dummy)
{
	struct scsi_device *sdev;
	struct scsi_cmnd *cmd;
	struct request *req;
	bool sense_alloc = false;
	int ret;

	sdev = ufshcd_find_sdev_by_lun(shost, lun);
	if (!sdev) {
		/* No such device, we return SUCCESS here */
		return SUCCESS;
	}

	/* For host reset if needed */
	dummy->device = sdev;
	...
	ufshcd_sync_cache(sdev, cmd);

	ret = shost->hostt->eh_device_reset_handler(cmd);
	pr_info("scsi: reseting device, lun=%d, ret=%d\n", lun, ret);
	if (ret == SUCCESS) {
		if (scsi_eh_tur(cmd) != 0) {
			pr_info("scsi: tur failed\n");
			ret = FAILED;
		}
	}
	if (shost->force_hostreset) {
		/* force to do host reset, so assume device reset failed */
		ret = FAILED;
	}
...
static int ufshcd_test_devices(struct Scsi_Host *shost, int lun, struct scsi_cmnd *dummy)
{
	struct scsi_device *sdev;
	struct scsi_cmnd *cmd;
	struct request *req;
	...
	return ufshcd_stu_and_tur(sdev);
}

typedef int (*lun_handler)(struct Scsi_Host *shost, int lun, struct scsi_cmnd *dummy);

static int ufshcd_iter_lun(struct Scsi_Host *shost, unsigned long lun,
			   lun_handler handler, struct scsi_cmnd *dummy)
{
	int offset = -1;
	int failed = 0;
	int ret;
	...
	while (true) {
		offset = find_next_bit(&lun, sizeof(shost->remote_lun), offset + 1);
		if (offset >= sizeof(lun)) {
			break;
		}
		ret = handler(shost, offset, dummy);
		if (ret != SUCCESS) {
			failed++;
		}
	}
...
/*
 * ufshcd_remote_error_handler - handle udk side error
 */
static void ufshcd_remote_error_handler(struct Scsi_Host *shost)
{
	struct scsi_cmnd scmd = {0};
	int failed = 0;
	int ret;

	if (!shost->remote_error_handler) {
		return;
	}

	pr_info("scsi: handling remote error handler, lun=%lx force_hostreset=%u",
		shost->remote_lun, shost->force_hostreset);

	failed = ufshcd_iter_lun(shost, shost->remote_lun, ufshcd_remote_lun_reset, &scmd);
	if (failed == 0) {
		return;
	}

	ret = shost->hostt->eh_host_reset_handler(&scmd);
	pr_info("scsi: reseting host %u, ret=%d\n", shost->host_no, ret);
	if (ret == SUCCESS) {
		shost->host_reseted = true;
		failed = ufshcd_iter_lun(shost, shost->remote_lun, ufshcd_test_devices, &scmd);
	}
}

static void ufshcd_scsi_error(struct Scsi_Host *shost)
{
	unsigned long flags;
	LIST_HEAD(eh_work_q);
	LIST_HEAD(eh_done_q);

	pr_info("scsi: ufshcd_scsi_error, remote=%u", shost->remote_error_handler);

	ufshcd_remote_error_handler(shost);
	...
	shost->remote_error_handler = false;

	pr_info("scsi: ufshcd_scsi_error exit");
}

struct scsi_transport_template ufshcd_transport_template = {
	.eh_strategy_handler = ufshcd_scsi_error
};
#endif
...
#if defined(CONFIG_LIBLINUX_UDK_UFS) && defined(CONFIG_UFS_4_0_HC)
	/* Reserve 2 tags for synccache and dev_cmd */
	host->can_queue = MCQ_TAG_NUM - 2;
#endif
...
#ifdef CONFIG_RPMB_UFS
#ifdef CONFIG_UFS_LOW_POWER_VOTE_STATUS_SYNC
		atomic_set(&hba->rpmb_lp_delay_extend, 0);
		atomic_set(&hba->rpmb_lp_delayed, 0);
		atomic_set(&hba->rpmb_lp_voted, 0);
#endif
...
#endif
#ifdef CONFIG_UFS_LOW_POWER_VOTE_STATUS_SYNC
    err = ufshcd_init_status_sync_info(hba);
    if (err)
		goto out_disable;
#endif
