...
#define MCQ_CORE_QUEUE_NUM 5
#define SYNCCACHE_QUEUE 3
#define DEV_CMD_QUEUE 4
#else
#define SYNCCACHE_QUEUE 27
#define REQ_ASYNC_QUEUE 28
#define REQ_HIGH_PRIO_QUEUE 29
#define REQ_VIP_QUEUE 30
#define DEV_CMD_QUEUE 31
#endif
#define MCQ_TAG_RECORD 32
...
#if defined(CONFIG_SCSI_UFS_PICTOR) || defined(CONFIG_SCSI_UFS_AURIGA)
	int (*fpga_spi_driver_probe)(struct ufs_hba *hba);
#endif
};
...
#define CORE_IRQ_NAME_LEN	32
	...
	struct delayed_work ufs_restore_key_worker;
	int restore_key_error_code;
#endif
	...
	atomic_t rpmb_lp_delay_extend;
	atomic_t rpmb_lp_delayed;
	atomic_t rpmb_lp_voted;
#endif
...
#ifdef CONFIG_UFS_4_0_HC
#define UFSHCD_IDLE_TIMEOUT_DEFAULT_VAL	8000
#else
...
#define UFSHCD_IDLE_INTR_CHECK_INTERVAL	3600000
	...
	bool esi_sup;
	bool ext_iid_sup;
	unsigned int mcq_irq[32];
	struct hlist_node mcq_node;
	atomic_t mcq_inflt_reqs;
#ifdef CONFIG_LIBLINUX_UDK_UFS
#define MCQ_TAG_NUM 16
#else
#define MCQ_TAG_NUM 256
#endif
#define BITS_PER_ITEM 64
#define MCQ_OUTSTANDING_ITEMS (MCQ_TAG_NUM + BITS_PER_ITEM - 1) / BITS_PER_ITEM
#define mcq_tag_idx(tag) ((tag) / BITS_PER_ITEM)
#define mcq_tag_off(tag) ((tag) % BITS_PER_ITEM)
	unsigned long mcq_outstanding_reqs[MCQ_OUTSTANDING_ITEMS];
#ifdef CONFIG_DFA_MCQ
	struct dfa_hba_info dhba;
#endif
#if defined(CONFIG_SCSI_UFS_LOW_POWER_VOTE) || defined(CONFIG_UFS_LOW_POWER_VOTE_STATUS_SYNC)
	bool ufs_dfa_lp_voted;
	bool ufs_dfa_ccbus_voted;
#endif
#if defined(CONFIG_SCSI_UFS_LOW_POWER_VOTE)
	u64 ufs_inflight_byte;
	u64 ufs_ccbus_threshold;
...
int ufshcd_compose_devman_upiu(struct ufs_hba *hba, struct ufshcd_lrb *lrbp);
...
/**
 * struct dfa_vcmd_vars - sendata vcmd variables
 * @type: currently set to 0, reserved for future expansion.
 * @length: length of the transmitted data.
 * @buffer_phy_addr: physical address of pa_offset.
 */
struct dfa_vcmd_vars {
	u8 type;
	u16 length;
	u64 buffer_phy_addr;
};
