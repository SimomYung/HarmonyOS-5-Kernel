...
static u32 sim_read_bridge_config(struct hw_sim_hardware_info *info, struct device_node *np)
{
	...
	hwsim_logi("sim_%u, transfer_type is %u, ret is 0x%x\n", info->sim_id, info->bridge_ctrl.type, ret);
...
static u32 sim_io_mux_init(struct hw_sim_hardware_info *info, struct device_node *np)
{
	...
	hwsim_logi("sim_%u sim_io_cfg.phy_addr: 0x%8X, pad_iomg_phy_addr: 0x%8X, pad_iocg_phy_addr: 0x%8X\n", info->sim_id,
		info->sim_io_cfg.phy_addr, info->sim_io_cfg.pad_iomg_phy_addr, info->sim_io_cfg.pad_iocg_phy_addr);
	...
	hwsim_logi("sim_%u usim1_func_switch: [0x%02x, 0x%02x], esim_func_switch: [0x%02x, 0x%02x]\n",
		info->sim_id, info->sim1_func[0], info->sim1_func[1], info->esim_func[0], info->esim_func[1]);
	...
	hwsim_logi("sim_%u iomg_usim_offset: [0x%02X, 0x%02X, 0x%02X], iocg_usim_offset: [0x%02X, 0x%02X, 0x%02X]\n",
		info->sim_id,
		info->sim_io_cfg.iomg_usim_offset[0],
		info->sim_io_cfg.iomg_usim_offset[1],
		info->sim_io_cfg.iomg_usim_offset[2], // 2 is index
		info->sim_io_cfg.iocg_usim_offset[0],
		info->sim_io_cfg.iocg_usim_offset[1],
		info->sim_io_cfg.iocg_usim_offset[2]); // 2 is index
	return 0;
}
...
static u32 sim_sd_reg_init(struct hw_sim_hardware_info *info, struct device_node *np)
{
	...
	hwsim_logi("sim_%u read sd sim reg begin\n", info->sim_id);
...
static void hw_sim_read_esim_class_support(struct hw_sim_hardware_info *info, const struct device_node *np)
{
	...
	hwsim_logi("sim_%u esim_class_only value is: %d\n", info->sim_id, esim_classc_only);
...
static void hw_sim_read_lvshift_switch(struct hw_sim_hardware_info *info, struct device_node *np)
{
	int ret;
	u32 lv_shift_switch = 1; // default enabled for eSIM

	ret = of_property_read_u32(np, "lv_shift_switch", &lv_shift_switch);
	if (ret != 0) {
		hwsim_loge("sim_%u fail to read lv_shift_switch, %d\n", info->sim_id, ret);
		info->lv_shift_switch = 0;
	} else {
		hwsim_logi("sim_%u lv_shift_switch is %u\n", info->sim_id, lv_shift_switch);
		info->lv_shift_switch = lv_shift_switch;
	}
}
...
static int hw_sim_read_dts(struct hw_sim_hardware_info *info, struct device_node *np)
{
	...
	hwsim_logi("sim_%u, begin to read hw sim dts\n", info->sim_id);
	...
	info->lv_shift_en_gpio = of_get_named_gpio(np, "lv_shift_en", 0);
	hwsim_logi("sim_%u info->lv_shift_en_gpio: %u\n", info->sim_id, info->lv_shift_en_gpio);
	...
	hwsim_logi("sim_%u power_type: %u, lv_shift_ctrl.func: %u, nvt_shift.func: %u, extral_ldo_out_type: %d, ret: %d\n",
		info->sim_id, info->sim_pmu_hardware.sim_pmu_hardware_type, info->sim_pmu_hardware.lv_shift_ctrl.func,
	...
	hw_sim_read_lvshift_switch(info, np);
	return ret;
}
...
static int hw_sim_read_regulator_by_type(const struct hw_sim_hardware_info *info)
{
	...
	hwsim_logi("sim_%u sim_pmu_hardware_type: %d\n", info->sim_id, info->sim_pmu_hardware.sim_pmu_hardware_type);
	...
	case SIM_POWER_TYPE_SINGLE:
		...
		if (info->lv_shift_switch != 0) {
			ret += hw_sim_read_regulator(sim_id, LVSHIFT_VCC_FLAG, &info->sim_pmu_hardware.hard_cfg.single_sim_pmu.sim_protectionlvshift_io,
				info->spmidev, "lvshift_vcc");
		}
        break;
...
static int set_nanosd_io_sdfun(const struct hw_sim_hardware_info *info)
{
	...
	hwsim_logi("sim_%u set sd_io_cfg gpio func to be GPIO\n", info->sim_id);
	return HW_SIM_OK;
}
...
static int set_nanosd_io_simfun(const struct hw_sim_hardware_info *info)
{
	...
	hwsim_logi("sim_%u current 0x%02X, set sd_io_cfg gpio fun to be USIM\n", info->sim_id, cur);
	return HW_SIM_OK;
}
...
static s32 set_sim_io_simfun(struct hw_sim_hardware_info *info)
{
	...
	for (i = 0; i < USIM_OFFSET_SIZE; ++i) {
		hwsim_logi("sim_io_cfg.iomg_usim_offset[%u] is 0x%02X\n", i, info->sim_io_cfg.iomg_usim_offset[i]);
		writel(USIM_IO_MUX,
	...
	hwsim_logi("sim_%u set sim_io_cfg gpio func to be USIM\n", info->sim_id);
	return HW_SIM_OK;
}
...
static s32 set_sim1_func(struct hw_sim_hardware_info *info, u32 io_func)
{
	...
	hwsim_logi("set sim1 pins to Function[%u]\n", func_id);
	...
	for (i = 0; i < USIM_OFFSET_SIZE; ++i) {
		...
		hwsim_logi("set_sim1_func usim1_iocfg_offset[%u] is 0x%02X\n", i, info->sim_io_cfg.usim1_iocfg_offset[i]);
	}
	...
	if (io_func == GPIO_IO_FUNC) {
		...
		hwsim_logi("special set sim1 clk Function\n");
	}
...
static s32 set_esim_func(struct hw_sim_hardware_info *info, u32 io_func)
{
	...
	hwsim_logi("set esim pins to Function[%u]\n", func_id);
	...
	for (i = 0; i < USIM_OFFSET_SIZE; ++i) {
		...
		hwsim_logi("set_esim_func esim_iocfg_offset[%u] is 0x%02X\n", i, info->sim_io_cfg.esim_iocfg_offset[i]);
	}
...
static s32 set_sim_io_gpiofun(struct hw_sim_hardware_info *info)
{
	...
	hwsim_logi("sim_%u set sim_io_cfg gpio fun to be GPIO\n", info->sim_id);
	return HW_SIM_OK;
}
...
static s32 set_hw_sim_io_mux(struct hw_sim_hardware_info *info, set_sim_io_type set_io_type)
{
	...
	hwsim_logi("sim_%u sim_io_mux, type is %d, result 0x%x\n", info->sim_id, set_io_type, ret);
	return (s32)ret;
}
...
static sci_hw_op_result_e sim_inquire_card_type(u32 sim_id, struct hw_sim_hardware_info *info)
{
	...
	hwsim_logi("sim_id: %u, sim_status: %u, sim_esim_type: %d, power_type: %u, esim_no_sd_3to2: %u\n",
		sim_id, info->sim_status, info->sim_esim_type, info->sim_pmu_hardware.sim_pmu_hardware_type,
		info->esim_no_sd_3to2_hardware);
...
static struct regulator* get_protectionlvshift_vcc_regulator(const sim_pmu_hardware_ctrl *sim_pmu_hardware)   //获取protectionlvshift电压
{
	return sim_pmu_hardware->hard_cfg.single_sim_pmu.sim_protectionlvshift_io;
}
...
static int hw_sim_protectionlvshift_enable_regulator(struct hw_sim_hardware_info *info,
													const struct regulator *sim_regulator,
													const struct regulator *protectionlvshift_regulator,
													s32 vol_sim, s32 vol_protectionlvshift)
{
	if (sim_regulator == NULL) {
		hwsim_loge("sim_regulator is null\n");
		return HW_SIM_ERR;
	}
	if (protectionlvshift_regulator == NULL) {
		hwsim_loge("protectionlvshift_regulator is null\n");
		return HW_SIM_ERR;
	}

	hwsim_logi("vol_sim is %d\n", vol_sim);
	// 这里需要判断levelshift 电压是否开启，开启则不用管，没有开启则需要开启电压
	if (regulator_is_enabled(protectionlvshift_regulator) != 1) { // 判断电源是否使能
		hwsim_logi("levelshift enable first\n");   // levelshift 电源未使能
		if (regulator_set_voltage(protectionlvshift_regulator, vol_protectionlvshift, vol_protectionlvshift) != HW_SIM_OK) {  // 设置电压
			hwsim_loge("levelshift regulator_set_voltage fail\n");
			return HW_SIM_ERR;
		}
		if (regulator_enable(protectionlvshift_regulator) != HW_SIM_OK) {  // 使能电源
			hwsim_loge("levelshift regulator_enable fail\n");
			return HW_SIM_ERR;
		}
	}
	if (gpio_is_valid(info->lv_shift_en_gpio)) {
		if (gpio_get_value(info->lv_shift_en_gpio) == 0) {
			gpio_set_value(info->lv_shift_en_gpio, 1);
			udelay(1000); // 1ms
		}
	}
	hwsim_logi("sim_%u levelshift enable success, lv_shift_en_gpio value: %u\n", info->sim_id, gpio_get_value(info->lv_shift_en_gpio));

	if (regulator_is_enabled(sim_regulator)) { // 判断电源是否使能
		hwsim_logi("sim disable first\n");
		if (regulator_disable(sim_regulator) == 0)
			hwsim_logi("sim regulator_disable ok\n");
		else
			hwsim_loge("sim regulator_disable fail\n");
	}
	if (regulator_set_voltage(sim_regulator, vol_sim, vol_sim) != HW_SIM_OK) {
		hwsim_loge("sim regulator_set_voltage fail\n");
		return HW_SIM_ERR;
	}

	if (regulator_enable(sim_regulator) != HW_SIM_OK) {  // 使能电源
		hwsim_loge("sim regulator_enable fail\n");
		return HW_SIM_ERR;
	}
	hwsim_logi("hw_sim_protectionlvshift_enable_regulator success\n"); 
	return HW_SIM_OK;
}
...
static int power_on_esim(struct hw_sim_hardware_info *info, s32 vol)
{
	...
	hwsim_logi("sim_%u esim pmu power on\n", info->sim_id);
	...
	hwsim_logi("sim_%u hw_sim_enable_regulator\n", info->sim_id);
	ret = hw_sim_enable_regulator(info->sim_pmu_hardware.esim_ctrl.esim_vcc, vol);
...
static int hw_sim_pmu_power_on(u32 sim_id, struct hw_sim_hardware_info *info, s32 vol)
{
	...
	struct regulator *levelshfit_vcc_pmu = NULL;
	...
	levelshfit_vcc_pmu = get_protectionlvshift_vcc_regulator(&info->sim_pmu_hardware);
	switch (info->sim_pmu_hardware.sim_pmu_hardware_type) {
	case SIM_POWER_TYPE_SINGLE:
		if (info->lv_shift_switch == 0) {
			ret = hw_sim_enable_regulator(sim_vcc_pmu, vol);
		} else {
			//这里使用重新定义的hw_sim_protectionlvshift_enable_regulator 表示同时给SIM和防漏电装置protectionlvshift上电（1.2v）
			ret = hw_sim_protectionlvshift_enable_regulator(info, sim_vcc_pmu, levelshfit_vcc_pmu, vol, VCC_1_2V);
		}
		break;
...
sci_hw_op_result_e sim_hwadp_handler(u32 sim_id, sci_hw_op_type_e msg_id)
{
	...
	hwsim_logi("sim_%u receive msg id: 0x%02x(%s)\n", sim_id, msg_id, msg_id_to_string(msg_id));
	...
	case SIM_HW_CARD_DET_WHILE_ESIM_INQUIRE:
		rslt = SIM_HW_CARD_NO_EXIST_WHILE_ESIM;
		break;
...
int sim_hwadp_init(u32 sim_id, struct device *dev)
{
	...
	hwsim_logi("sim_%u, hw sim hardware done, res: 0x%x\n", sim_id, res);
	return HW_SIM_OK;
}
