...
static sci_hw_op_result_e phy_card_one_det(struct hw_sim_hardware_info *info)
{
	int ret;
...
{
	sci_hw_op_result_e ret = phy_card_one_det(info);
	if (ret == SIM_HW_CARD_NO_EXIST_WHILE_ESIM) {
		hwsim_loge("second detection start\n");
		ret = phy_card_one_det(info);
	}
	return ret;
}
