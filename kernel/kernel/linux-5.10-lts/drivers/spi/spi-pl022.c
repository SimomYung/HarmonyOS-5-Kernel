...
#define SPI_WAIT_FOR_COMPLETION  (HZ * 10)
u32 g_spi_step = 0;
u32 g_spi_rx_chan = 0;
u32 g_spi_tx_chan = 0;
#endif
...
struct pl022 {
	...
	int sync_locked;
	unsigned int dma_for_send_only;
#ifdef CONFIG_SPI_V400_TIMEOUT_LOG
	struct completion trans_complete;
#endif
#endif
	int cur_cs;
	int *chipselects;
...
static void giveback(struct pl022 *pl022)
{
	 ...
	 */
#ifdef CONFIG_SPI_V400
#ifdef CONFIG_SPI_V400_TIMEOUT_LOG
		g_spi_step = 19;
		complete(&pl022->trans_complete);
#endif
...
static void pump_transfers(unsigned long data)
{
	...
	if (pl022->cur_chip->enable_dma) {
#if defined CONFIG_SPI_V400
		if (pl022->dma_for_send_only) {
			if (spi_v400_configure_dma_tx(pl022)) {
				dev_dbg(&pl022->adev->dev,
					"spi_v400_configure_dma_tx, fall back to interrupt mode\n");
				goto err_config_dma;
			}
		} else {
#if defined CONFIG_SPI_V400_TIMEOUT_LOG
...
{
	u32 read_cr0;
	u16 read_cr1, read_dmacr, read_sr, read_ris;
	...
	read_ris = readw(SSP_RIS(pl022->virtbase));

	dev_err(&pl022->adev->dev, "spi-pl022 CR0: %x\n", read_cr0);
	dev_err(&pl022->adev->dev, "spi-pl022 CR1: %x\n", read_cr1);
	dev_err(&pl022->adev->dev, "spi-pl022 DMACR: %x\n", read_dmacr);
	dev_err(&pl022->adev->dev, "spi-pl022 SR: %x\n", read_sr);
	dev_err(&pl022->adev->dev, "spi-pl022 RIS: %x\n", read_ris);
	dev_err(&pl022->adev->dev,
...
#ifdef CONFIG_SPI_V400_TIMEOUT_LOG
#define REG_DMAC_BASE 0xfa000000
...
#define DMA_MAX_CH 15
#define DMA_MIN_CH 2
#define INT_TC1 0x600
#define INT_ERR1 0x608
#define INT_ERR2 0x610

static void dump_dma_config(void)
{
	uintptr_t va;
	unsigned int stat, config, count, cur_cnt;
	unsigned int src_l, src_h, dst_l, dst_h;
	unsigned int i;

	va = __ioremap(REG_DMAC_BASE, REG_SIZE, PAGE_KERNEL_RO, MODE_DEVICE);
	stat = readl(va + CH_STAT);
	printk(KERN_ERR "spi dma stat 0x%x\n", stat);
	stat = readl(va + INT_TC1);
	printk(KERN_ERR "spi dma int tc1 stat 0x%x\n", stat);
	stat = readl(va + INT_ERR1);
	printk(KERN_ERR "spi dma int err1 0x%x\n", stat);
	stat = readl(va + INT_ERR2);
	printk(KERN_ERR "spi dma int err2 0x%x\n", stat);

	for (i = DMA_MIN_CH; i <= DMA_MAX_CH; i++) {
		src_l = (unsigned int)readl(va + CX_SRC_L + (i * CH_OFFSET));
		src_h = (unsigned int)readl(va + CX_SRC_H + (i * CH_OFFSET));
		dst_l = (unsigned int)readl(va + CX_DST_L + (i * CH_OFFSET));
		dst_h = (unsigned int)readl(va + CX_DST_H + (i * CH_OFFSET));
		config = (unsigned int)readl(va + CX_CONFIG + (i * CH_OFFSET));
		count = (unsigned int)readl(va + CX_CNT + (i * CH_OFFSET));
		cur_cnt = (unsigned int)readl(va + CX_CUR_CNT + (i * (CH_OFFSET - 0x20)));
		printk(KERN_ERR "spi dma chan %d, src_l = 0x%x, src_h = 0x%x, dst_l = 0x%x, dst_h = 0x%x\n",
				i, src_l, src_h, dst_l, dst_h);
		printk(KERN_ERR "spi dma chan %d, config = 0x%x, count = 0x%x, cur_cnt = 0x%x\n",
				i, config, count, cur_cnt);
	}
	iounmap(va);
	return;
}
...
#ifdef CONFIG_SPI_V400_TIMEOUT_LOG
	int ret;
	reinit_completion(&pl022->trans_complete);
	g_spi_step = 0;
	g_spi_rx_chan = 0;
	g_spi_tx_chan = 0;
#endif
...
#ifdef CONFIG_SPI_V400_TIMEOUT_LOG
	ret = wait_for_completion_timeout(&pl022->trans_complete,
		SPI_WAIT_FOR_COMPLETION);
	if (ret == 0) {
		dev_err(&pl022->adev->dev, "spi trans timeout, step %d! tx %d rx %d\n",
				g_spi_step, g_spi_tx_chan, g_spi_rx_chan);
		if (pl022->cur_transfer)
			dev_err(&pl022->adev->dev,"spi cur tans len %d", pl022->cur_transfer->len);
		dump_dma_config();
		print_current_status(pl022);
	};
#endif
...
static int pl022_probe(struct amba_device *adev, const struct amba_id *id)
{
...
#if defined CONFIG_SPI_V400
#ifdef CONFIG_SPI_V400_TIMEOUT_LOG
	init_completion(&pl022->trans_complete);
#endif
	...
	if (of_property_read_u32(np, "dma-for-send-only", &pl022->dma_for_send_only))
		pl022->dma_for_send_only = 0;
	dev_info(dev, "dma for send only is %u\n", pl022->dma_for_send_only);
#endif
	/*
