					     ...
					     struct logger_entry *scratch)
{
	...
	if (len != sizeof(*scratch)) {
		if (memcpy_s(((void *)scratch), sizeof(*scratch), log->buffer + off, len) != EOK)
			pr_err("memcpy_s log off failed: %s\n",  __func__);
		if (memcpy_s(((void *)scratch) + len, sizeof(*scratch) - len, log->buffer, sizeof(*scratch) - len) != EOK)
			pr_err("memcpy_s log buffer failed: %s\n",  __func__);
		return scratch;
	}
...
static void do_write_log(struct logger_log *log, const void *buf, size_t count)
{
	...
	if (memcpy_s(log->buffer + log->w_off, log->size - log->w_off, buf, len) != EOK)
		pr_err("memcpy_s log w_off failed: %s\n",  __func__);

	if (count != len && memcpy_s(log->buffer, log->size, buf + len, count - len) != EOK)
		pr_err("memcpy_s log buf failed: %s\n",  __func__);
...
static ssize_t hw_logger_write_iter(struct kiocb *iocb, struct iov_iter *from)
{
	...
	while (nr_segs-- > 0 && header.len >= ret) {
		size_t len;
		ssize_t nr;
...
static int check_tag_level_to_show(char *srcbuff, const char *name, int level)
{
	...
	if (snprintf_s(checkbuff, sizeof(checkbuff), MAX_NAME_LEN + 1, "%s", name) < 0) {
			    ...
			    loff_t *pos)
{
	...
	if (memset_s(log_tag->tag_save_buff, TAG_BUFF_SIZE, 0, TAG_BUFF_SIZE) != EOK) 
		pr_err("memset_s failed: %s\n",  __func__);
	...
	for (t = &__start_hwlog_tag; t < &__stop_hwlog_tag; t++) {
		...
		if (checkresult == 1) {
			...
			len = snprintf_s(str, MAX_NAME_AND_LEVEL_BUFF_SIZE + 1, MAX_NAME_AND_LEVEL_BUFF_SIZE,
            	"%.*s 0X%0*X\n", MAX_NAME_LEN, t->name, LEVEL_LEN, t->level);
			if (len < 0) {
				pr_err("snprintf_s failed: %s\n",  __func__);
				return 0;
			}
			str += min(len, MAX_NAME_AND_LEVEL_BUFF_SIZE);
		}
	}
	str = NULL;
			     ...
			     size_t count, loff_t *pos)
{
	...
	ret = sscanf_s(kernelbuf, "%s 0X%X", name, sizeof(name), &val);
	if (ret != 2) {
