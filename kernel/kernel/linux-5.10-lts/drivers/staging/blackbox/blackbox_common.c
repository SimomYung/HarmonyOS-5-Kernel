...
int get_sub_string(const char *event_name, const char split, char *report_event_name)
{
	int index = 0;
	...
	for (index = 0; index < strlen(event_name); index++) {
		if (event_name[index] == split) {
			break;
		}
		report_event_name[index] = event_name[index];
	}
	report_event_name[index] = '\0';
	return 0;
}

int send_vendor_event(const char *event_name, const char *module, const char *reason, const char *log_path,
		      const char *sub_log_path, const char *summary)
{
#ifdef CONFIG_HISYSEVENT
	struct hiview_hisysevent *event = NULL;
	char report_event_name[CATEGORY_MAX_LEN] = {0};
	int ret = 0;

	ret = get_sub_string(event_name, ':', report_event_name);
	if (ret != 0) {
		bbox_print_err("failed to get sub string from event_name.\n");
		return -EINVAL;
	}

	event = hisysevent_create(KERNEL_VENDOR_DOMAIN, report_event_name, FAULT);
	if (!event) {
		bbox_print_err("failed to create event.\n");
		return -EINVAL;
	}
	ret = hisysevent_put_string(event, "MODULE", module);
	ret += hisysevent_put_string(event, "REASON", reason);
	ret += hisysevent_put_string(event, "LOG_PATH", log_path);
	ret += hisysevent_put_string(event, "SUB_LOG_PATH", sub_log_path);
	ret += hisysevent_put_string(event, "SUMMARY", summary);
	if (ret != 0) {
		bbox_print_err("failed to put sting to event, ret=%d.\n", ret);
		goto hisysevent_end;
	}
...
hisysevent_end:
	...
	return ret;
#else
	bbox_print_err("HISYSEVENT is not configured.\n");
	return 0;
#endif
}
