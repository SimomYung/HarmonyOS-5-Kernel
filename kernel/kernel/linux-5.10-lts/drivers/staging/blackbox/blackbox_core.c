...
#define TOP_CATEGORY_ADAPTER		"ADAPTER"
...
static struct error_info_to_category error_info_categories[] = {
    ...
    {
        MODULE_BOOTFAIL,
        {EVENT_BOOTFAIL, CATEGORY_SYSTEM_BOOTFAIL, TOP_CATEGORY_SYSTEM_RESET}
    },
...
static void format_error_info_from_adapter(struct error_info *info, const char event[EVENT_MAX_LEN],
...
static void format_error_info_from_adapter(struct error_info *info, const char event[EVENT_MAX_LEN],
					   const void *error_info)
{
	if (unlikely(!info || !event || !error_info)) {
		bbox_print_err("info: %p, event: %p, error_info: %p\n",
				info, event, error_info);
		return;
	}

	struct error_info *perror_info = (struct error_info *)error_info;
	memset(info, 0, sizeof(*info));
	strncpy(info->event, perror_info->event, min(strlen(perror_info->event),
				sizeof(info->event) - 1));
	strncpy(info->module, perror_info->module, min(strlen(perror_info->module),
				sizeof(info->module) - 1));
	strncpy(info->category, perror_info->category, min(strlen(perror_info->category),
				sizeof(info->category) - 1));
	strncpy(info->error_time, perror_info->error_time, min(strlen(perror_info->error_time),
				sizeof(info->error_time) - 1));
	strncpy(info->log_path, perror_info->log_path, min(strlen(perror_info->log_path),
				sizeof(info->log_path) - 1));
	info->sysreboot = perror_info->sysreboot;
	strncpy(info->top_category, perror_info->top_category, min(strlen(perror_info->top_category),
				sizeof(info->top_category) - 1));
	strncpy(info->error_desc, perror_info->error_desc, min(strlen(perror_info->error_desc),
				sizeof(info->error_desc) - 1));
}
			     ...
			     const char *timestamp, int need_sys_reset)
{
	...
	scnprintf(buf, HISTORY_LOG_MAX_LEN, HISTORY_LOG_FORMAT,
			strlen(info->top_category) ? info->top_category : get_top_category(info->module, info->event),
			info->module, info->category, info->event, timestamp,
			need_sys_reset ? "true" : "false", info->error_desc,
			strlen(info->log_path) ? info->log_path : log_root_dir);
	send_vendor_event(info->category, info->module, info->event, strlen(info->log_path) ? info->log_path : log_root_dir,
			  timestamp, info->error_desc);
...
static bool find_module_ops(struct error_info *info, struct bbox_ops **ops)
{
	...
	if (is_adapter_system) {
		list_for_each_entry(cur, &ops_list, list) {
			if (!strcmp(cur->ops.module, ADAPTER_SYSTEM)) {
				*ops = cur;
				find_module = true;
				break;
			}
...
static void save_log_without_reset(struct error_info *info)
{
	...
	if (strlen(info->error_time))
		strncpy(timestamp, info->error_time, TIMESTAMP_MAX_LEN - 1);
	else
		get_timestamp(timestamp, sizeof(timestamp));
	...
	if (ops->ops.dump) {
		...
		memset(log_dir, 0, PATH_MAX_LEN);
		if (log_dir) {
			if (!strlen(info->log_path)) {
				format_log_dir(log_dir, PATH_MAX_LEN,
						CONFIG_BLACKBOX_LOG_ROOT_PATH, timestamp);
				create_log_dir(log_dir);
			} else {
				strncpy(log_dir, info->log_path, min(strlen(info->log_path),
					PATH_MAX_LEN - 1));
				(void)strncat(log_dir, timestamp, (PATH_MAX_LEN - 1 - strlen(timestamp)));
			}
		} else
			bbox_print_err("kmalloc failed!\n");
	}
	invoke_module_ops(log_dir, info, ops);
	save_history_log(CONFIG_BLACKBOX_LOG_ROOT_PATH, info, timestamp, info->sysreboot);
	kfree(log_dir);
}
				...
				const char error_desc[ERROR_DESC_MAX_LEN], int need_sys_reset)
{
	...
	if (!strcmp(event, EVENT_BOOTFAIL)) {
		format_error_info_from_adapter(info, event, (const void *)error_desc);
		is_adapter_system = false;
	} else if (strcmp(event, EVENT_ADAPTER)) {
		format_error_info(info, event, module, error_desc);
		show_stack(current, NULL, KERN_DEFAULT);
		is_adapter_system = false;
	} else {
		format_error_info_from_adapter(info, event, (const void *)error_desc);
		is_adapter_system = true;
	}
	if (!need_sys_reset) {
