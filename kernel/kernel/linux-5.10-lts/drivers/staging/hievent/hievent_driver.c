...
#ifdef CONFIG_BOOT_DETECTOR_SINGLE_FRAME
	bool is_bootfail;
	unsigned int ret = boot_detector_poll(filep, wait, &is_bootfail);
	if (is_bootfail) {
		return mask | ret;
	}
#endif
...
static long hievent_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	long ret = -EINVAL;
#ifdef CONFIG_BOOT_DETECTOR_SINGLE_FRAME
	unsigned int is_used = 0;
	ret = boot_detector_ioctl(file, cmd, arg, &is_used);
	if (is_used != 0) {
		pr_err("boot_detector_ioctl is used");
		return ret;
	}
#endif
...
int hievent_release(struct inode* inodep, struct file* filep)
{
#ifdef CONFIG_BOOT_DETECTOR_SINGLE_FRAME
	boot_detector_release(filep);
#endif
	return 0;
}

static const struct file_operations hievent_fops = {
	.release = hievent_release,
	.read = hievent_read,   /* read */
	.poll = hievent_poll,   /* poll */
	.write_iter = hievent_write_iter, /* write_iter */
	.unlocked_ioctl = hievent_ioctl,
#ifdef CONFIG_COMPAT
	.compat_ioctl = hievent_ioctl,
#endif
};
