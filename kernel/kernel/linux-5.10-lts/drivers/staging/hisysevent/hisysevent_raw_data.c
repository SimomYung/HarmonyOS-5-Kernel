...
static int raw_data_init(struct hisysevent_raw_data *raw_data)
{
	...
	raw_data->data = kzalloc(EXPAND_BUF_SIZE, hisysevent_gfp_flag());
	if (!(raw_data->data)) {
		    ...
		    unsigned int pos)
{
	...
	if ((pos + len) > dest->capacity) {
		...
		resize_data = kmalloc(dest->capacity + expanded_size, hisysevent_gfp_flag());
		if (!resize_data) {
...
raw_data_create(void)
{
	...
	raw_data = kzalloc(sizeof(*raw_data), hisysevent_gfp_flag());
	if (!raw_data)
		return NULL;
