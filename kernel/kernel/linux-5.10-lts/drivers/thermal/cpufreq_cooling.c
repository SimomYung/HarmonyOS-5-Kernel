				       ...
				       u32 *power)
{
...
#else
	freq = cpufreq_quick_get(cpufreq_cdev->cpuid_start);
#endif
#ifdef CONFIG_IPA_THERMAL
...
#else
		u32 ncpus = cpufreq_cdev->cpu_num;
#endif
	...
	for (i = 0; i < cpufreq_cdev->cpu_num; i++) {
#endif
		u32 load;
	...
	*power = dynamic_power;
	...
	if (load_cpu) {
		...
		trace_thermal_power_cpu_get_power(policy->related_cpus, freq,
						  load_cpu, i, *power);
#else
		trace_thermal_power_cluster_get_power(cpufreq_cdev->cluster, freq,
						  load_cpu, cpufreq_cdev->cpu_num, dynamic_power, static_power, *power);
#endif
		kfree(load_cpu);
	}
#ifdef CONFIG_IPA_THERMAL
...
static struct cpufreq_policy g_policy[MAX_CPU_CLUSTER];
static struct thermal_cooling_device *
__cpufreq_cooling_register_ldk(struct device_node *np, int cluster)
{
	struct thermal_cooling_device *cdev;
	struct cpufreq_cooling_device *cpufreq_cdev;
	char dev_name[THERMAL_NAME_LENGTH];
	...
	cpufreq_cdev->policy = &g_policy[cluster];
	cpufreq_cdev->cluster = cluster;

	ret = cdev_get_cpu_info(cpufreq_cdev);
	if (ret != 0) {
		pr_err("cdev_get_cpu_info failed \n");
		goto free_cdev;
	}
	ret = get_opp_freq_and_volt(cpufreq_cdev);
	if (ret < 0) {
		pr_err("get_opp_freq_and_volt cluster error: %d\n", cluster);
		goto free_cdev;
	}

	cpufreq_cdev->idle_time = kcalloc(cpufreq_cdev->cpu_num,
		sizeof(*cpufreq_cdev->idle_time), GFP_KERNEL);
	if (!cpufreq_cdev->idle_time) {
		cdev = ERR_PTR(-ENOMEM);
...
#ifdef CONFIG_SMT_MODE_GOV
	cpufreq_cdev->busy_time = kcalloc(cpufreq_cdev->cpu_num,
		sizeof(*cpufreq_cdev->busy_time), GFP_KERNEL);
	if (!cpufreq_cdev->busy_time) {
		cdev = ERR_PTR(-ENOMEM);
		goto free_idle_time;
	}
	...
	if (1) {
#else
	if (em_is_sane(cpufreq_cdev, NULL)) {
#endif
...
#endif
	freq_constraints_init(&(cpufreq_cdev->policy->constraints));
 
	cdev = thermal_of_cooling_device_register(np, dev_name, cpufreq_cdev, cooling_ops);
	if (IS_ERR(cdev)) {
		pr_err("__cpufreq_cooling_register_ldk: cdev is null\n");
		goto remove_qos_req;
	}
...
#ifdef CONFIG_LDK_THERMAL
struct thermal_cooling_device *
of_cpufreq_cooling_register_ldk(int cluster)
{
	struct device_node *np = NULL;
	char node[16] = {0};
	int ret;
...
#ifdef CONFIG_THERMAL_PERCORE
	int i = cluster;
	// Require cdev to be registered in order
	static int last_registed_cdev = -1;
	if (cluster <= last_registed_cdev)
		i = last_registed_cdev + 1;
	for (; i < MAX_THERMAL_CLUSTER_NUM; i++) {
		ret = snprintf_s(node, sizeof(node), (sizeof(node) - 1),
			 "cluster%d", i);
		if (ret < 0) {
			pr_err("snprintf_s error: %s\n", node);
		...
		// If there are no cores in the cluster, use the next cluster
		if (0 == of_get_child_count(np)) {
			pr_err("there is no core0 in cluster%d\n", cluster);
			of_node_put(np);
			continue;
		}
		pr_err("cdev cluster:%d use np node:cluster%d\n", cluster, i);
		last_registed_cdev = i;
		break;
	}
#else
	ret = snprintf_s(node, sizeof(node), (sizeof(node) - 1),
...
#endif
	if (of_find_property(np, "#cooling-cells", NULL)) {
		cdev = __cpufreq_cooling_register_ldk(np, cluster);
		if (IS_ERR(cdev)) {
			pr_err("cpufreq_cooling: cluster%d failed to register as cooling device: %ld\n",
			       cluster, PTR_ERR(cdev));
			cdev = NULL;
		}
	}
...
EXPORT_SYMBOL_GPL(of_cpufreq_cooling_register_ldk);
#endif
