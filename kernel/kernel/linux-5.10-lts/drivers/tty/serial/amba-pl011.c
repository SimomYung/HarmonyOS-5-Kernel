...
#include <linux/io.h>
#ifdef CONFIG_BUS_AMBA_PL011
#include <linux/cpumask.h>
#include "bus_amba_pl011.h"
#else
...
static int pl011_dma_tx_refill(struct uart_amba_port *uap)
{
...
#ifndef CONFIG_BUS_UART_OVERSAMPLING
	/*
	...
	if (dma_map_sg(dma_dev->dev, &dmatx->sg, 1, DMA_TO_DEVICE) != 1) {
		uap->dmatx.queued = false;
		dev_err(uap->port.dev, "unable to map TX DMA\n");
		return -EBUSY;
	}
...
static inline bool pl011_dma_tx_start(struct uart_amba_port *uap)
{
	...
	if (!uap->port.x_char) {
		...
		if (!uap->dmatx.queued) {
			...
			printk(KERN_ERR "pl011_dma_tx_start dmatx busy\n");
		}
		return ret;
	}
			    ...
			    struct resource *mmiobase, int index)
{
...
#ifdef CONFIG_BUS_AMBA_PL011
	ret = bus_pl011_pinctrl(pdev, uap);
	if (ret) {
...
#ifdef CONFIG_BUS_AMBA_PL011
	ret = bus_pl011_probe_reset_func_enable(pdev, uap);
	if (ret)
	...
	bus_pl011_probe_console_enable(pdev, uap, amba_console.name);
#endif
...
#ifdef CONFIG_BUS_AMBA_PL011
	ret = bus_pl011_probe_get_clk_freq(dev, uap, portnr);
	if (ret)
...
#endif
#ifdef CONFIG_BUS_AMBA_PL011
static SIMPLE_DEV_PM_OPS(pl011_dev_pm_ops, bus_pl011_suspend, bus_pl011_resume);
#else
