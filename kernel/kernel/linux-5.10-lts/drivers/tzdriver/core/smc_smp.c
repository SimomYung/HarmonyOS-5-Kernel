...
#ifndef CONFIG_NO_IPIHELPER_THREAD
...
static atomic_t g_freeze_flag;
inline bool is_in_freeze(void)
{
	return atomic_read(&g_freeze_flag) == 1;
}

#ifdef CONFIG_BLOCK_SMC_WHEN_SUSPEND
#define TIME_M 1000
#define MAX_WAIT 3000
static int tz_prepare_freeze(struct kprobe *p, struct pt_regs *regs)
{
	int i;
	uint64_t start_time, curr_time;
	struct time_spec now_time;
	uint64_t smc_not_return;
	(void)p;
	(void)regs;

	tlogi("prepare freeze");
	atomic_set(&g_freeze_flag, 1);
	get_time_spec(&now_time);
	start_time = now_time.ts.tv_sec * TIME_M + now_time.ts.tv_nsec / TIME_M / TIME_M;
	/*
	 * Wait smc return REE, max wait time = 3s.
	 * We can NOT freeze when smc is handling, so if smc not return REE, freeze will fail.
	 * The code below can make almost smc return REE in prepare.
	 * After prepare, there will be just several smc when freezing. (include svc thread)
	 */
	while (true) {
		get_time_spec(&now_time);
		curr_time = now_time.ts.tv_sec * TIME_M + now_time.ts.tv_nsec / TIME_M / TIME_M;
		if (curr_time - start_time >= MAX_WAIT) { // MAX wait 3s
			break;
		}
		smc_not_return = 0;
		for (i = 0; i < MAX_SMC_CMD; i++) {
			if (test_bit(i, (unsigned long *)g_cmd_data->in_bitmap) != 0 &&
				test_bit(i, (unsigned long *)g_cmd_data->out_bitmap) == 0) {
				tlogd("smc in bitmap and not out index %d\n", i);
				smc_not_return++;
			}
		}
		tlogd("smc not return count %d\n", smc_not_return);
		if (smc_not_return <= 1) // allow the svc cmd not return
			break;
		cond_resched();
	}
	return 0;
...
static void tz_complete_freeze(struct kprobe *p, struct pt_regs *regs, unsigned long flags)
{
	(void)p;
	(void)regs;
	(void)flags;

	tlogi("complete all freeze");
	atomic_set(&g_freeze_flag, 0);
}

static struct kprobe kp_freeze_proc = {
	.symbol_name   = "freeze_processes",
	.pre_handler   = tz_prepare_freeze,
	.post_handler  = NULL,
	.fault_handler = NULL,
};

static struct kprobe kp_thaw_proc = {
	.symbol_name   = "thaw_processes",
	.pre_handler   = NULL,
	.post_handler  = tz_complete_freeze,
	.fault_handler = NULL,
};
...
static int kprobe_init(void)
{
	...
	ret = register_kprobe(&kp_freeze_proc);
	if (ret < 0) {
		tloge("register_kprobe freeze failed, returned %d\n", ret);
		return ret;
	}
	ret = register_kprobe(&kp_thaw_proc);
	if (ret < 0) {
		tloge("register_kprobe thaw failed, returned %d\n", ret);
		return ret;
	}
	return 0;
}
...
{
	unregister_kprobe(&kp_freeze_proc);
	unregister_kprobe(&kp_thaw_proc);
	tlogi("unregister kprobe");
}
#endif
...
#if defined(CONFIG_TZDRIVER_MODULE)
...
{
#ifdef CONFIG_TZ_S4_SUPPORT
	hibernation_down_read_sem();
#endif
	int ret;
	static bool set_preempt_permission = false;

	if (!set_preempt_permission)
		set_preempt_permission = set_preempt_call_permission();
...
#ifdef CONFIG_LIBLINUX
#define CURRENT_SIGNAL_BIT_KILL  1
#define CURRENT_SIGNAL_BIT_FREEZ 3
#define CURRENT_SIGNAL_BIT_POSIX 10
bool sigkill_pending(struct task_struct *tsk)
{
	uint64_t mask = liblinux_pal_get_current_signals();
	if ((mask & (1ull << CURRENT_SIGNAL_BIT_KILL)) != 0)
		return true;

	if ((mask & (1ull << CURRENT_SIGNAL_BIT_POSIX)) != 0) {
...
static bool is_ready_to_kill(bool need_kill, uint32_t cmd_id)
{
	...
	if (cmd_id == GLOBAL_CMD_ID_PORTAL_WORK) {
		return (need_kill && current_sigkill_pending());
	} else {
		return (need_kill && current_sigkill_pending() && is_thread_reported(CURRENT_PID));
	}
#else
	(void)cmd_id;
	return (need_kill && current_sigkill_pending() && is_thread_reported(CURRENT_PID));
...
{
	unsigned long ret = raw_smc_send(TSP_REE_SIQ, mode, 0, false);
	if (ret == TSP_CRASH) {
		tloge("TEEOS has crashed!\n");
		g_sys_crash = true;
		cmd_monitor_fault_crash(TYPE_CRASH_TEE, NULL, 0);
	}

	if (siq_mode == SIQ_DUMP_TIMEOUT)
		tz_log_write();
	if (siq_mode == SIQ_DUMP_SHELL) {
...
static void cmd_result_check(const struct tc_ns_smc_cmd *cmd, int cmd_index)
{
	...
	if (cmd->ret_val == (int)TEE_ERROR_TAGET_DEAD) {
		...
		cmd_monitor_fault_crash((ta_killed == true) ? TYPE_KILLED_TA : TYPE_CRASH_TA,
	...
	const struct smc_out_params *out_params, int *n_idled)
{
	if (out_params->exit_reason == SMC_EXIT_PREEMPTED) {
		...
		in_params->x2 = (unsigned long)CURRENT_PID;
	...
	int *n_idled)
{
	struct smc_in_params in_params = {
		TSP_REQUEST, SMC_OPS_START_SHADOW, CURRENT_PID, 0, *(const unsigned long *)arg
	};
	...
	send_smc_cmd(&in_params, out_params, 0);
}
	...
	int *n_preempted, int *n_idled, int *ret_val)
{
	...
	*n_preempted = 0;
	...
	timeout = HZ * (long)(HZ_COUNT + ((uint8_t)CURRENT_PID & LOW_BYTE));
#ifdef CONFIG_LIBLINUX
	rc = (int)wait_event_freezable_timeout(pe->wq,
		atomic_read(&pe->run) || liblinux_pal_thread_freezing(), (long)timeout);
	int freeze_ret;
	trace_ldk_rvh_thread_try_to_freeze(&freeze_ret);
#else
	rc = (int)wait_event_freezable_timeout(pe->wq,
		atomic_read(&pe->run), (long)timeout);
#endif
	clear_freezable_wait_pending(rc);
	if (rc == 0)
		(*n_idled)++;
...
{
	int n_preempted = 0, ret = 0, n_idled = 0;
	struct smc_out_params params = { 0, SMC_EXIT_MAX, 0, 0 };
	struct pending_entry *pe = NULL;

#ifdef CONFIG_LIBLINUX
	trace_ldk_rvh_thread_set_freezable(&ret);
#else
	set_freezable();
#endif
...
struct task_struct *shadow_work_func(struct kthread_work *work)
{
	...
	struct shadow_work *s_work = NULL;
	uint64_t *target_arg = NULL;

	if (!work) {
		tloge("%s: param is NULL\n", __func__);
		return NULL;
	}

	s_work = container_of(work, struct shadow_work, kthwork);
	target_arg = kzalloc(sizeof(uint64_t), GFP_KERNEL);
	...
	*target_arg = s_work->target;
	shadow_thread = kthread_create(shadow_thread_fn,
		(void *)(uintptr_t)target_arg, "$%s", s_work->name);
	...
	tlogi("%s: create shadow thread %s: for target 0x%" PRIx64 "\n",
		__func__, s_work->name, *target_arg);
#if (CONFIG_CPU_AFF_NR != 0)
...
void fiq_shadow_work_func(uint64_t target)
{
	...
	struct smc_send_param param = { .cmd = TSP_REQUEST, .ops = (unsigned long)SMC_OPS_START_FIQSHD,
									.ca = (unsigned long)(uint32_t)CURRENT_PID };
	...
	struct tc_ns_smc_cmd *in)
{
	...
	case TEEC_PENDING2: {
		 ...
		 */
		cmd_monitor_set_agent_status(AGENT_RECV_TEE_CMD);
		if (agent_process_work(cmd, agent_id, nsid) != 0)
			tloge("agent process work failed\n");
		cmd_monitor_set_agent_status(AGENT_RET_TO_TEE);
...
static void print_crash_msg(union crash_inf *crash_info)
{
	...
	if (app_index == KERNEL_INDEX) {
		tloge("====crash app:%s\n", crash_app_name);
		length = strnlen(crash_info->crash_msg.sym_name, SYM_NAME_LEN_1);
		if (length != 0 && length < SYM_NAME_LEN_1)
			tloge("====crash user sym:%s kernel crash off/size: <0x%x/0x%x>\n",
					crash_info->crash_msg.sym_name, off, crash_info->crash_msg.size);
		tloge("====crash halt reason: 0x%x far:0x%x fault:0x%x "
			"elr:0x%x (ret_ip: 0x%" PRIx64 ")\n", halt_reason, crash_info->crash_msg.far,
			crash_info->crash_msg.fault, crash_info->crash_msg.elr, crash_info->crash_reg[2]);
	} else {
		...
		if (memcpy_s(syms + SYM_NAME_LEN_1, SYM_NAME_LEN_MAX - SYM_NAME_LEN_1,
			crash_info->crash_msg.sym_name_append, SYM_NAME_LEN_2) != EOK)
			tloge("memcpy sym_name_append failed!\n");
		tloge("====crash app:%s\n", crash_app_name);
		if (strnlen(syms, SYM_NAME_LEN_MAX) != 0)
			tloge("====crash user_sym:%s + <0x%x/0x%x>\n", syms, off, crash_info->crash_msg.size);
		/* For ccos, when drvmgr crashes, exception information is encapsulated in gtask, 
		 * and register information cannot be obtained.
		 */
		if ((!is_ccos()) || (app_index != TEE_DRV_SERVER_INDEX))
			tloge("====crash far:0x%x fault:0x%x\n", crash_info->crash_msg.far, crash_info->crash_msg.fault);
	}
}
	...
	struct smc_cmd_ret *cmd_ret, int cmd_index)
{
	...
	ca = is_ccos() ? ((unsigned long)cmd_index) : ((unsigned long)(uint32_t)CURRENT_PID);
	...
	bool reuse)
{
	...
	*pe = init_pending_entry();
	...
	in->ca_pid = (unsigned int)CURRENT_PID;
#ifdef CONFIG_CONFIDENTIAL_CONTAINER
	if (in->nsid == 0)
...
static u64 process_abort_cmd(int index, const struct pending_entry *pe)
{
	(void)pe;
	if (set_abort_cmd(index) == 0)
		return (u64)SMC_OPS_ABORT_TASK;

	return (u64)SMC_OPS_SCHEDTO;
}
...
#define TIMER_IRQ_OFFSET 32
#ifdef CONFIG_WAIT_FREEZABLE_WHEN_BLOCKED
#define BLOCK_TA_OFFSET 33
#endif
static long wait_event_internal_done(struct pending_entry *pe, struct timeout_step_t *step, uint64_t pending_args)
{
	if (!CURRENT_MM) {
		/*
		 ...
		 */
#ifdef CONFIG_LIBLINUX
		long ret = wait_event_freezable_timeout(pe->wq, atomic_read(&pe->run) ||
			liblinux_pal_thread_freezing(), step->steps[step->cur]);
		int freeze_ret;
		trace_ldk_rvh_thread_try_to_freeze(&freeze_ret);
		return ret;
#else
		return wait_event_freezable_timeout(pe->wq, atomic_read(&pe->run), step->steps[step->cur]);
#endif
	} else {
#ifdef CONFIG_WAIT_FREEZABLE_WHEN_BLOCKED
		long ret;
		bool is_ta_blocked = (bool)(pending_args & ((uint64_t)1 << BLOCK_TA_OFFSET));
		if (is_ta_blocked) {
retry_wait:
			ret = wait_event_freezable_timeout(pe->wq, atomic_read(&pe->run), step->steps[step->cur]);
			if (ret == -ERESTARTSYS && sigisemptyset(&current->pending.signal)) {
				clear_thread_flag(TIF_SIGPENDING);
				goto retry_wait;
			}
			return ret;
		} else {
			return wait_event_timeout(pe->wq, atomic_read(&pe->run), step->steps[step->cur]);
		}
#else
		(void)pending_args;
		return (long)wait_event_timeout(pe->wq, atomic_read(&pe->run), step->steps[step->cur]);
#endif
	}
}
	...
	u64 *ops)
{
	...
	bool timer_no_irq = (bool)(pending_args & ((uint64_t)1 << TIMER_IRQ_OFFSET));
...
resleep:
	...
	if (wait_event_internal_done(pe, step, pending_args) == 0) {
		...
		if (is_ca_killed((int32_t)cmd_index)) {
			*ops = process_abort_cmd((int32_t)cmd_index, pe);
			return PD_WAKEUP;
		}
	} else {
		woke_up = true;
...
static void set_cmd_reuse_info(struct cmd_reuse_info *info, const struct tc_ns_smc_cmd *in)
{
...
int smp_smc_send_func(struct tc_ns_smc_cmd *in, bool reuse)
{
	...
	struct tc_ns_smc_cmd cmd = { {0}, 0 };
	struct pending_entry *pe = NULL;
	struct timeout_step_t timeout_step = {{0, 0, 0, 0}, TO_STEP_SIZE, -1, false};
	u64 ops = SMC_OPS_NORMAL;
...
retry:
	...
	if (!is_cmd_working_done((uint32_t)info.cmd_index)) {
		if (cmd_ret.exit == SMC_EXIT_NORMAL) {
			...
			ops = process_abort_cmd(info.cmd_index, pe);
			goto retry;
		} else {
			tloge("invalid cmd work state: %ld\n", cmd_ret.exit);
			cmd.ret_val = TEEC_ERROR_GENERIC;
			goto clean;
		}
	}
...
static int smc_svc_thread_fn(void *arg)
{
	...
	while (!kthread_should_stop()) {
		...
		if (smp_smc_send_func(&smc_cmd, false) == (int)TEE_ERROR_IS_DEAD &&
			!is_tee_rebooting())
			break;
...
void wakeup_tc_siq(uint32_t siq_mode)
{
	...
	if (siq_mode == 0 || is_in_freeze())
		return;

	mutex_lock(&g_siq_lock);
	i = get_free_siq_index();
	if (i >= MAX_SIQ_NUM) {
		tlogd("dump is too frequent\n");
		mutex_unlock(&g_siq_lock);
		return;
	}
...
void smc_set_cmd_buffer(void)
{
	 ...
	 */
#ifdef CONFIG_TEE_RESET
#if defined(CONFIG_TZ_RESERVED_MAILBOX) && !defined(CONFIG_LIBLINUX_CDC_RDA)
...
static int alloc_cmd_buffer(void)
{
	...
	g_cmd_phys = RESERVED_SECOS_CMDMEM_BASE;
	g_cmd_size = RESERVED_SECOS_CMDMEM_SIZE;
	g_reserved_cmd_buffer = true;
#endif

	if (g_reserved_cmd_buffer) {
		tlogi("use reserved cmd buffer\n");
		reserved_addr = get_reserved_cmd_vaddr_of(g_cmd_phys, (uint64_t)g_cmd_size);
		g_cmd_data = (struct tc_ns_smc_queue *)(uintptr_t)reserved_addr;
		if (!g_cmd_data)
			return -ENOMEM;
...
static int init_smc_related_rsrc(const struct device *class_dev)
{
	...
	init_kthread_worker(&g_ipi_helper_worker);
#else
...
{
	if (event_reason == NULL || uuid == NULL || ta_version == NULL) {
		tloge("Bad TA_ABNORMAL event info!\n");
		return;
	}
	int ret;
	struct hiview_hisysevent *event = NULL;

	event = hisysevent_create("RELIABILITY", "TA_ABNORMAL", FAULT);
	if (!event) {
		tloge("[ta_abnormal]failed to create event");
		return;
	}
	ret = hisysevent_put_string(event, "REASON", event_reason);
	if (ret != 0) {
		tloge("[ta_abnormal]failed to put reason string to event, ret = %d", ret);
		goto exit;
	}
	ret = hisysevent_put_string(event, "UUID", uuid);
	if (ret != 0) {
		tloge("[ta_abnormal]failed to put uuid string to event, ret = %d", ret);
		goto exit;
	}
	ret = hisysevent_put_string(event, "VERSION", ta_version);
	if (ret != 0) {
		tloge("[ta_abnormal]failed to put version string to event, ret = %d", ret);
		goto exit;
	}
	ret = hisysevent_write(event);
	if (ret < 0)
		tloge("[ta_abnormal]failed to send event, ret = %d", ret);
	else
		tlogi("[ta_abnormal]send event successfully.");
exit:
...
#endif
	release_kthread_worker(&g_ipi_helper_worker);
#ifndef CONFIG_NO_IPIHELPER_THREAD
	if (!IS_ERR_OR_NULL(g_ipi_helper_thread)) {
