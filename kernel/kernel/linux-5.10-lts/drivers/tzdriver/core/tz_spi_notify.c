...
struct notify_context_shadow {
	uint64_t target_tcb;
	char shadow_thread_name[SHADOW_NAME_LEN];
};
...
#ifdef CONFIG_SET_SHADOW_PRIORITY
struct notify_context_set_shadow_priority {
    uint32_t ca_pid;
    uint32_t priority;
};
#endif
...
#endif
#ifdef CONFIG_SET_SHADOW_PRIORITY
    struct notify_context_set_shadow_priority shadow_priority;
#endif
...
#endif
#ifdef CONFIG_SET_SHADOW_PRIORITY
	NOTIFY_DATA_ENTRY_SET_SHADOW_PRIORITY,
#endif
...
static noinline int notify_data_entry_type_handle(const uint32_t entry_type)
{
	switch (entry_type) {
...
#endif
#ifdef CONFIG_SET_SHADOW_PRIORITY
	case NOTIFY_DATA_ENTRY_SET_SHADOW_PRIORITY:
		g_notify_data->meta.context.meta.recv_sp++;
		break;
#endif
	default:
...
static noinline int get_notify_data_entry(struct notify_data_entry *copy)
{
	...
	for (i = NOTIFY_DATA_ENTRY_UNUSED; i < NOTIFY_DATA_ENTRY_COUNT; i++) {
		...
		if (filled == 0)
			continue;
		if (notify_data_entry_type_handle(e->entry_type) != 0)
			goto exit;
...
static void tc_notify_shadow_fn(const struct notify_data_entry *entry)
{
	...
	smc_queue_shadow_worker(tc_notify_shadow->target_tcb, tc_notify_shadow->shadow_thread_name);
}
...
#define MAX_SHADOW_PRIORITY_ALLOWED 41
#define MIN_SHADOW_PRIORITY_ALLOWED 20
#define NORMAL_DEFAULT_PRIORITY 20
 
#ifdef CONFIG_LIBLINUX
int set_thread_priority_by_tid(uint32_t tid, uint32_t priority)
{
	int ret = -1;
	int nice = (int)priority;
 
	if (priority == MAX_SHADOW_PRIORITY_ALLOWED) {
		ret = liblinux_pal_thread_set_vip_prio_by_tid(tid, 1);
	} else if (priority >= MIN_SHADOW_PRIORITY_ALLOWED &&
			   priority <= MAX_SHADOW_PRIORITY_ALLOWED - 1) {
		nice = NORMAL_DEFAULT_PRIORITY - priority;
		ret = liblinux_pal_thread_set_vip_prio_by_tid(tid, 0);
		if (ret != 0) {
			tloge("canceling thread vip prio fails, ret=%d\n", ret);
		} else {
			ret = liblinux_pal_set_user_nice(tid, nice);
		}
	} else {
		tloge("invalid shadow priority: prio=%u\n", priority);
	}
	tlogi("set shadow priority: tid=%u, prio=%u, nice=%d, ret=%d\n", tid, priority, nice, ret);
...
int set_thread_priority_by_tid(uint32_t tid, uint32_t priority)
{
	...
	if (priority == MAX_SHADOW_PRIORITY_ALLOWED) {
		...
		ret = set_vip_prio(task, 1);
#else
		tloge("setting vip priority is not supported\n");
#endif
	} else if (priority >= MIN_SHADOW_PRIORITY_ALLOWED &&
			   priority <= MAX_SHADOW_PRIORITY_ALLOWED - 1) {
		nice = NORMAL_DEFAULT_PRIORITY - priority;
...
static void tc_notify_set_shadow_priority(struct notify_data_entry *entry)
{
	int ret;
	struct notify_context_set_shadow_priority *tc_notify_shadow_priority = NULL;
 
	tc_notify_shadow_priority = &(entry->context.shadow_priority);
 
	ret = set_thread_priority_by_tid(tc_notify_shadow_priority->ca_pid, tc_notify_shadow_priority->priority);
	if (ret != 0)
		tloge("tz set_thread_priority_by_tid fails, err=%d", ret);
}
#endif
...
#ifdef CONFIG_SET_SHADOW_PRIORITY
		case NOTIFY_DATA_ENTRY_SET_SHADOW_PRIORITY:
			tc_notify_set_shadow_priority(&copy);
			break;
#endif
		default:
...
{
	struct tc_ns_callback *data = NULL;
	struct tc_ns_callback *temp = NULL;
	...
	mutex_lock(&g_ta_callback_func_list.callback_list_lock);
	list_for_each_entry_safe(data, temp, &g_ta_callback_func_list.callback_list, head) {
		list_del(&data->head);
		kfree(data);
	}
	mutex_unlock(&g_ta_callback_func_list.callback_list_lock);
