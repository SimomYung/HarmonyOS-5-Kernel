...
#if defined(CONFIG_FAULT_MODE_REGHOOK) || defined(CONFIG_FAULT_MODE_NETLINK)
#include "fault_manager.h"
#endif

#if !defined(CONFIG_FAULT_MODE_REGHOOK) && !defined(CONFIG_FAULT_MODE_NETLINK)
static inline void fault_monitor_init(void)
{
	return;
}

static inline void fault_monitor_start(int32_t type, const uint8_t *ta_uuid)
{
	(void)type;
	(void)ta_uuid;
	return;
...
static inline void fault_monitor_fini(void)
{
	return;
}
#endif
