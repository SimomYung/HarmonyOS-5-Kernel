...
#define MAX_ION_NUM 16
#define UUID_MEMBER_NUM 11
static DEFINE_MUTEX(dynamic_mem_lock);
struct dynamic_mem_list {
	struct list_head list;
};

static struct dynamic_mem_config *g_dyn_mem_config = NULL;
static struct dynamic_mem_list g_dynamic_mem_list;
static uint32_t g_dyn_mem_config_num = 0;

void free_ion_whitelist_mem(void)
{
	if (g_dyn_mem_config != NULL) {
		kfree(g_dyn_mem_config);
		g_dyn_mem_config = NULL;
		g_dyn_mem_config_num = 0;
	}
}

#define UUID_TIME_LOW 0
#define UUID_TIME_MID 1
#define UUID_TIMEHI_AND_VERSION 2
#define UUID_CLOCKSEQ_AND_NODE_BEGIN 3
static int parse_ion_node(struct device_node *node, struct dynamic_mem_config *ion_info)
{
	int ret;
	unsigned ion_uuid[UUID_MEMBER_NUM] = {0};
	if (node == NULL || ion_info == NULL) {
		tloge("the params is NULL");
		return -1;
	}
	ret = of_property_read_variable_u32_array(node, "uuid", ion_uuid, UUID_MEMBER_NUM, UUID_MEMBER_NUM);
	if (ret != UUID_MEMBER_NUM) {
		tloge("the uuid is invalid\n");
		return -1;
	}

	(ion_info->uuid).time_low = ion_uuid[UUID_TIME_LOW];
	(ion_info->uuid).time_mid = ion_uuid[UUID_TIME_MID];
	(ion_info->uuid).timehi_and_version = ion_uuid[UUID_TIMEHI_AND_VERSION];
	/* Determine whether it is truncated. */
	if ((ion_info->uuid).timehi_and_version != ion_uuid[UUID_TIMEHI_AND_VERSION] ||
		(ion_info->uuid).time_mid != ion_uuid[UUID_TIME_MID]) {
		tloge("the uuid is invalid timehi_and_version = 0x%x, time_mid = 0x%x\n",
			  ion_uuid[UUID_TIMEHI_AND_VERSION], ion_uuid[UUID_TIME_MID]);
		return -1;
	}

	int i = UUID_CLOCKSEQ_AND_NODE_BEGIN;
	for (; i < UUID_MEMBER_NUM; i++) {
		(ion_info->uuid).clockseq_and_node[i - UUID_CLOCKSEQ_AND_NODE_BEGIN] = ion_uuid[i];
		if((ion_info->uuid).clockseq_and_node[i - UUID_CLOCKSEQ_AND_NODE_BEGIN] != ion_uuid[i]) {
			tloge("the uuid is invalid 0x%x\n", ion_uuid[i]);
			return -1;
		}
	}

	if (of_property_read_u32(node, "ddr_sec_region", &(ion_info->ddr_sec_region)) != 0) {
		tloge("get the ddr_sec_region failed\n");
		return -1;
	}
	return 0;
}

static int parse_whitelist(unsigned int *ion_whitelist, int ion_num, int *index)
{
	int i;
	int result;
	for (i = 0; i < ion_num; i++) {
		struct device_node *entry_node = of_find_node_by_phandle(ion_whitelist[i]);
		result = parse_ion_node(entry_node, &g_dyn_mem_config[*index]);
		if (result != 0) {
			tloge("parse ion node failed index is %d\n", i);
			return -1;
		}
		(*index)++;
	}
...
{
	int num;
	num = of_property_read_variable_u32_array(dev_node, item_name, whitelist, 0, whitelist_len);
	if (num >= 0)
		return num;
	if (num == -EINVAL || num == -ENODATA) {
		tlogi("the ion item is empty or not exist\n");
		return 0;
	}
...
int load_ion_whitelist(struct device_node *dev_node)
{
	int result;
	int ion_num;
	if (dev_node == NULL)
	...
	unsigned ion_whitelist[MAX_ION_NUM] = {0};
	ion_num = get_whitelist_item_num(dev_node, "ion_whitelist", ion_whitelist, MAX_ION_NUM);
	if (ion_num < 0)
		return -1;
	g_dyn_mem_config_num += ion_num;
#ifdef DEF_ENG
	int eng_ion_num;
	unsigned eng_ion_whitelist[MAX_ION_NUM] = {0};
	eng_ion_num = get_whitelist_item_num(dev_node, "ion_whitelist_eng", eng_ion_whitelist, MAX_ION_NUM);
	if (eng_ion_num < 0)
		return -1;
	g_dyn_mem_config_num += eng_ion_num;
#endif

	if (g_dyn_mem_config_num > 0) {
		int buf_len = g_dyn_mem_config_num * sizeof(struct dynamic_mem_config);
		g_dyn_mem_config = kmalloc(buf_len, GFP_KERNEL);
		if (ZERO_OR_NULL_PTR((unsigned long)(uintptr_t)g_dyn_mem_config)) {
			tloge("alloc mem failed\n");
			return -1;
		}
		memset_s(g_dyn_mem_config, buf_len, 0, buf_len);
		int index = 0;

		result = parse_whitelist(ion_whitelist, ion_num, &index);
		if (result != 0) {
...
#ifdef DEF_ENG
		result = parse_whitelist(eng_ion_whitelist, eng_ion_num, &index);
		if (result != 0) {
			free_ion_whitelist_mem();
			return -1;
		}
...
static int proc_alloc_dyn_mem(struct dynamic_mem_item *mem_item)
{
	...
	if (UINT32_MAX - mem_item->size < BLOCK_64KB_SIZE_MASK) {
