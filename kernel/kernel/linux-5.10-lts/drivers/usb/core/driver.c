...
#include <usb/smart_usb/hw_smart_usb.h>
#endif
...
#ifdef CONFIG_HUAWEI_ARMPC_SMART_USB
	if (check_storage_rejects_status() && (intf->cur_altsetting != NULL) &&
		(intf->cur_altsetting->desc.bInterfaceClass == USB_CLASS_MASS_STORAGE)) {
		pr_info("%s: devpath %s Interface %d is not authorized to load the driver.",
			__func__, udev->devpath, intf->cur_altsetting->desc.bInterfaceNumber);
		return error;
	}
#endif
...
#ifdef CONFIG_PM
	if (!status && udev->descriptor.bDeviceClass == USB_CLASS_HUB)
...
static void choose_wakeup(struct usb_device *udev, pm_message_t msg)
{
	...
	if (udev->state == USB_STATE_SUSPENDED && w != udev->do_remote_wakeup) {
		...
		dev_err(&udev->dev, "%s: pm_runtime_resume\n", __func__);
	}
...
int usb_runtime_suspend(struct device *dev)
{
	...
	dev_err(&udev->dev, "%s status 0x%x -\n", __func__, status);
	return status;
}
