...
#ifdef CONFIG_USB_CONFIG_PRODUCT_PC
#define USB_MAX_TIME_OUT_S        3  /* s */
#else
#define USB_MAX_TIME_OUT_S        2  /* s */
#endif

#define USB_MAX_TIME_OUT_MS    (USB_MAX_TIME_OUT_S * 1000)  /* ms */
...
int usb_new_device(struct usb_device *udev)
{
	...
	if (err < 0)
		goto fail;
	dev_err(&udev->dev, "udev %d, busnum %d, minor = %d\n",
	...
	dev_err(&udev->dev, "udev %d, busnum %d\n",	udev->devnum, udev->bus->busnum);
	return err;
...
u32 g_port_init_tries = PORT_INIT_TRIES;

#define GET_DES_TRIES_RESUME 2
...
void usb_set_port_init_tries(u32 port_init_tries)
{
    g_port_init_tries = min(PORT_INIT_TRIES, max(port_init_tries, 1));
    pr_info("usb_set_port_init_tries: set %d real %d\n", port_init_tries, g_port_init_tries);
}
			   ...
			   struct usb_port *port_dev)
{
	 ...
	 */
	if (use_both_schemes && retry >= (g_port_init_tries + 1) / 2)
	...
	/* Is the device still present? */
		...
		if (retries--) {
			...
			status = hub_port_status(hub, port1, &portstatus,
							     &portchange);
#ifdef CONFIG_USB_CONFIG_PRODUCT_PC
			dev_err(&port_dev->dev, "usb resume portstatus 0x%x\n", portstatus);
			if (hub_port_warm_reset_required(hub, port1, portstatus)) {
				dev_err(&port_dev->dev, "usb port resume do warm reset\n");
...
#ifdef CONFIG_USB_HIUSB
	unsigned remote_wakeup_back = udev->do_remote_wakeup;
	pr_info("%s: do_remote_wakeup = 0x%x, s3_power_down_flag = 0x%x",
		__func__, udev->do_remote_wakeup, hiusb_get_s3_power_down_flag());
	if (udev->do_remote_wakeup && !PMSG_IS_AUTO(msg)) {
		udev->do_remote_wakeup = !hiusb_get_s3_power_down_flag();
		pr_info("%s: set do_remote_wakeup = 0x%x", __func__, udev->do_remote_wakeup);
	}
#endif
...
#ifdef CONFIG_USB_HIUSB
	udev->do_remote_wakeup = remote_wakeup_back;
#endif
...
static int finish_port_resume(struct usb_device *udev)
{
	...
	dev_info(&udev->dev, "%s, idVendor=%04x, idProduct=%04x\n",
		 udev->reset_resume ? "finish reset-resume" : "finish resume",
		 le16_to_cpu(udev->descriptor.idVendor),
		 le16_to_cpu(udev->descriptor.idProduct));
	if (udev->product)
		dev_info(&udev->dev, "Product: %s\n", udev->product);
	if (udev->manufacturer)
		dev_info(&udev->dev, "Manufacturer: %s\n", udev->manufacturer);
		...
		u16 *portchange, u16 *portstatus)
{
	...
	unsigned long jiffies_expire = jiffies + USB_MAX_TIME_OUT_S * HZ;
#endif
	int status = 0, delay_ms = 0;

#ifdef CONFIG_USB_WAIT_FOR_CONNECTED_ACCURATELY
    while (delay_ms < USB_MAX_TIME_OUT_MS && time_before(jiffies, jiffies_expire)) {
#else
	while (delay_ms < 2000) {
#endif
		...
		int retry_counter)
{
	...
	if ((udev->do_dpm_resume) && (GET_MAXPACKET0_TRIES > GET_DES_TRIES_RESUME))
		retry_cnt = GET_DES_TRIES_RESUME;
	else
		retry_cnt = GET_MAXPACKET0_TRIES; 
...
#define GET_DESCRIPTOR_BUFSIZE	64
			 ...
			 * 512 (WUSB1.0[4.8.1]).
			 */
			for (operations = 0; operations < retry_cnt;
					++operations) {
...
static int usb_reset_and_verify_device(struct usb_device *udev)
{
	...
	int				i, j, ret = 0;
	int				port1 = udev->portnum;
	bool wakeup_backup = udev->dev.power.can_wakeup;
	...
	if (descriptors_changed(udev, &descriptor, bos)) {
		...
		dev_info(&udev->dev, "dev wakeup 0x%x and wakeup_back 0x%x\n", udev->dev.power.can_wakeup, wakeup_backup);
		udev->dev.power.can_wakeup = wakeup_backup;
		goto re_enumerate;
	}
