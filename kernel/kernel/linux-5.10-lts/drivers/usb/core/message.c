...
#define USB_SG_COMPLETION_TIMEOUT_TIMES 15000
#else
#define USB_SG_COMPLETION_TIMEOUT_TIMES 10000
#endif
...
void usb_sg_wait(struct usb_sg_request *io)
{
	...
	if (!wait_for_completion_timeout(&io->complete, msecs_to_jiffies(USB_SG_COMPLETION_TIMEOUT_TIMES))) {
		usb_sg_cancel(io);
		if (!wait_for_completion_timeout(&io->complete, msecs_to_jiffies(2000)))
			dev_err(&io->dev->dev,
					"%s, submit urbs fails and cancel all urbs fail again\n",
					__func__);
		else
			dev_err(&io->dev->dev,
					"%s, submit urbs fails and cancel all urbs\n",
					__func__);
	}
	sg_clean(io);
}
