...
{
	if (usb_gadget_device && !IS_ERR(usb_gadget_device))
		return device_create(usb_gadget_class, usb_gadget_device,
...
void (*usb_gadget_setup_complete)(struct usb_ep *ep,
...
#ifdef CONFIG_CHIP_USB_CONFIGFS
	usb_gadget_setup_complete = cdev->req->complete;
#endif
...
#ifdef CONFIG_USB_CONFIGFS_UEVENT
static void usb_gadget_work(struct work_struct *data)
{
	...
	if (status[0]) {
		...
		pr_info("%s: current usb state is CONNECTED\n", __func__);
		uevent_sent = true;
	}
	...
	if (status[1]) {
		...
		pr_info("%s: current usb state is CONFIGURED\n", __func__);
		uevent_sent = true;
	}

	if (status[2]) {
		kobject_uevent_env(&gi->dev->kobj, KOBJ_CHANGE, disconnected);
		pr_info("%s: current usb state is DISCONNECTED\n", __func__);
		uevent_sent = true;
	}
...
static void mirrorlink_work(struct work_struct *data)
{
	...
	kobject_uevent_env(&usb_gadget_device->kobj, KOBJ_CHANGE,
			mirrorlink_event);
}
#endif
#endif
...
#ifdef CONFIG_USB_CONFIGFS_UEVENT
static int usb_gadget_setup(struct usb_gadget *gadget,
			const struct usb_ctrlrequest *c)
{
	...
	if (!usb_gadget_device)
		return 0;

	gi = dev_get_drvdata(usb_gadget_device);
	spin_lock_irqsave(&gi->spinlock, flags);
	cdev = get_gadget_data(gadget);
	if (!cdev || gi->unbind) {
		spin_unlock_irqrestore(&gi->spinlock, flags);
	...
	cdev->req->zero = 0;
	cdev->req->length = 0;
	cdev->req->complete = usb_gadget_setup_complete;
...
static struct device_attribute *usb_gadget_usb_attributes[] = {
	&dev_attr_state,
	NULL
};
...
static int usb_gadget_device_create(struct gadget_info *gi)
{
	struct device_attribute **attrs;
	struct device_attribute *attr;
	int ret;

	INIT_WORK(&gi->work, usb_gadget_work);
...
#endif
	gi->dev = device_create(usb_gadget_class, NULL,
			MKDEV(0, 0), NULL, "gadget%d", gadget_index++);
	if (IS_ERR(gi->dev))
		return PTR_ERR(gi->dev);

	dev_set_drvdata(gi->dev, gi);
	if (!usb_gadget_device)
		usb_gadget_device = gi->dev;

	attrs = usb_gadget_usb_attributes;
	while ((attr = *attrs++)) {
		int err;
	...
	hw_rwswitch_create_device(usb_gadget_device, usb_gadget_class);
	hw_usb_get_device(usb_gadget_device);
	hw_pcinfo_create_device(usb_gadget_device, usb_gadget_class);
#endif

	return 0;
}

static void usb_gadget_device_destroy(struct gadget_info *gi)
{
	struct device_attribute **attrs;
	struct device_attribute *attr;
	int ret;
...
#else
static inline int usb_gadget_device_create(struct gadget_info *gi)
{
	return 0;
}

static inline void usb_gadget_device_destroy(struct gadget_info *gi)
{
}
#endif
		...
		const char *name)
{
	...
	if (usb_gadget_device_create(gi) < 0)
		goto err;
...
#ifdef CONFIG_USB_CONFIGFS_UEVENT
	usb_gadget_class = class_create(THIS_MODULE, "gadget_usb");
