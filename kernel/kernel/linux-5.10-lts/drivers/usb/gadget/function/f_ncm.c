...
static inline unsigned ncm_bitrate(struct usb_gadget *g)
{
	if (!g)
		return 0;
	else if (gadget_is_superspeed(g) && ((g->speed == USB_SPEED_SUPER) || (g->speed == USB_SPEED_SUPER_PLUS)))
		return 4250000000U;
...
#define NTB_LOW_MEM_OUT_SIZE	(4096 - 512 - 16)
#define USB_LOW_MEM_OUT_BUFFER_SIZE	(4096 - 512)
#define USB_LOW_MEM_OUT_CFG 4096
#endif
...
#ifdef CONFIG_LIBLINUX
static void ncm_set_out_len(struct f_ncm *ncm)
{
	if (ntb_parameters.dwNtbOutMaxSize == cpu_to_le32(NTB_LOW_MEM_OUT_SIZE)) {
		ncm->port.fixed_out_len = USB_LOW_MEM_OUT_BUFFER_SIZE;
	} else {
		ncm->port.fixed_out_len = USB_OUT_BUFFER_SIZE;
	}
}
 
static void ncm_set_ntb_param_size(struct f_ncm_opts *ncm_opts)
{
	if (ncm_opts->ntb_size == USB_LOW_MEM_OUT_CFG) {
		ntb_parameters.dwNtbOutMaxSize = cpu_to_le32(NTB_LOW_MEM_OUT_SIZE);
	} else {
		ntb_parameters.dwNtbOutMaxSize = cpu_to_le32(NTB_OUT_SIZE);
	}
}
#endif
...
#ifdef CONFIG_LIBLINUX
	/* fixed out len is controlled by dwNtbOutMaxSize */
	ncm_set_out_len(ncm);
#else
	ncm->port.fixed_out_len = USB_OUT_BUFFER_SIZE;
#endif
...
static int ncm_setup(struct usb_function *f, const struct usb_ctrlrequest *ctrl)
{
	...
	case ((USB_DIR_IN | USB_TYPE_CLASS | USB_RECIP_INTERFACE) << 8)
		...
		INFO(cdev, "Host asked NTB parameters dwNtbOutMaxSize=%u\n", ntb_parameters.dwNtbOutMaxSize);
		break;
...
static int ncm_set_alt(struct usb_function *f, unsigned intf, unsigned alt)
{
	...
	/* Data interface has two altsettings, 0 and 1 */
		...
		if (alt == 1) {
			...
			INFO(cdev, "ncm gether_connect success");
		}
...
#ifdef CONFIG_LIBLINUX
/* f_ncm_opts_ntb_size */
USB_ETHER_CONFIGFS_ITEM_ATTR_INT_RW(ncm, ntb_size);
#endif
...
#ifdef CONFIG_LIBLINUX
	// reset ntb_size, write to ConfigFS
	opts->ntb_size = USB_OUT_BUFFER_SIZE;
	pr_info("reset ntb_size to %d\n", USB_OUT_BUFFER_SIZE);
#endif
...
#ifdef CONFIG_LIBLINUX
	// initial ntb_size value, write to ConfigFs
	opts->ntb_size = USB_OUT_BUFFER_SIZE;
	pr_info("ncm_alloc_inst ntb_size=%u\n", opts->ntb_size);
#endif
