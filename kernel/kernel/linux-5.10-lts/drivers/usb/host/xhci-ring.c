...
#define USB_MAX_PORTID  5
#define XHCI_DIED_NOTIFY_ACTION 0
/*
...
static int xhci_abort_cmd_ring(struct xhci_hcd *xhci, unsigned long flags)
{
	...
	xhci_err(xhci, "Abort command ring\n");
	...
	if (xhci->quirks & XHCI_CACMD_TIMEOUT_QUIRK) {
		...
		if (ret < 0) {
			...
			xhci_err(xhci, "xhci num_ports is 0x%x\n", num_ports);
			if (ret == -ETIMEDOUT && num_ports >= 2) {
				u32 portsc;
...
static void xhci_kill_ep_urbs(struct xhci_hcd *xhci, struct xhci_virt_ep *ep)
{
	struct xhci_td *cur_td;
	struct xhci_td *tmp;
	struct xhci_ring *ring;
...
void xhci_stop_endpoint_command_watchdog(struct timer_list *t)
{
	...
	xhci_warn(xhci, "hc not set died\n");

	ep->ep_state &= ~EP_STOP_CMD_PENDING;

	if (xhci->quirks & XHCI_CACMD_TIMEOUT_QUIRK && !hiusb_port_is_suspend_state()) {
		/* stop_ep_cmd watchdog dont set xhci died, cmd timeout will do U3 warm reset to recovery.*/
		xhci_warn(xhci, "s0 stop ep cmd timeout, kill urb and do warm reset\n");
		xhci_kill_ep_urbs(xhci, ep);
	} else {
		xhci_halt(xhci);
	...
	if (hiusb_port_is_suspend_state()) {
		pr_err("xhci port id 0x%x", xhci->portid);
		if (xhci->portid > USB_MAX_PORTID)
			return;
		usb_mode_call_notifiers(XHCI_DIED_NOTIFY_ACTION, &xhci->portid);
	}
#endif
}
