...
int xhci_suspend(struct xhci_hcd *xhci, bool do_wakeup)
{
	...
	if (!hcd->msi_enabled) {
		...
		if ((irq_pending & IMAN_IP)) {
			xhci_warn(xhci, "WARN: irq happened befor xhci stop\n");
			irq_pending |= IMAN_IP;
...
{
	xhci_info(xhci, "%s hibernated(%s)\n", __func__, hibernated ? "true" : "false");
	u32			command, temp = 0;
	struct usb_hcd		*hcd = xhci_to_hcd(xhci);
...
int xhci_disable_slot(struct xhci_hcd *xhci, u32 slot_id)
{
	...
	timeleft = wait_for_completion_timeout(command->completion, XHCI_CMD_DEFAULT_TIMEOUT);
	if (timeleft == 0) {
		xhci_err(xhci, "Timeout while waiting for disable slot command\n");
		spin_lock_irqsave(&xhci->lock, flags);
		list_del(&command->cmd_list);
		spin_unlock_irqrestore(&xhci->lock, flags);
	}
		...
		struct usb_host_endpoint *ep)
{
	...
	if (!udev) {
		xhci_info(xhci, "ep already disabaled, uedv is null\n");
		return;
	}
	...
	ep_index = xhci_get_endpoint_index(&ep->desc);
	if (!xhci->devs[slot_id]) {
		xhci_err(xhci, "xhci->devs[slot_id] is null, ep has been freed\n");
		spin_unlock_irqrestore(&xhci->lock, flags);
		return;
	}
