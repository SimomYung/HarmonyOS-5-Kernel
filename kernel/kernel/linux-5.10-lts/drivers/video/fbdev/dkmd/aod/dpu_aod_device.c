...
static void g_render_dmabuf_put(void)
{
	...
	int ret = dpu_aod_template_req_with_resp((void *)(&post_handle.sub_cmd), sizeof(uint32_t));
	if (ret)
		DPU_AOD_ERR("dpu_aod_post_handle fail\n");

	for (uint32_t i = 0; i < BUFFER_TYPE_MAX && i < g_buffer_list->buffer_list.count; i++) {
...
static int dynamic_fb_dma_check(void)
{
	 ...
	 */
#if defined(CONFIG_DKMD_DPU_AOD)
	for (int i = 0; i < g_ion_fb_config->dynamic_fb_count; i++) {
		if (g_ion_fb_config->fb[i].is_via_smmu) {
...
static int dpu_aod_render_buffer_phys(aod_render_buffer_data_t *aod_render_buffer)
{
	...
	for (int i = 0; i < MIN(BUFFER_TYPE_MAX, g_buffer_list->buffer_list.count); i++) {
...
int dpu_aod_panel_handle(int blank_mode)
{
	int ret;
	if (!g_aod_data)
		return -1;
