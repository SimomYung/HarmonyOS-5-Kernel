...
int dptx_video_mode_change(struct dp_ctrl *dptx, uint8_t vmode, int stream)
{
	...
	if (dptx->user_timing == NULL) {
		...
		dpu_pr_info("[DP] The mode is changed by PC user");
		if (!convert_code_to_dtd_by_user(dptx, &mdtd, &vparams->m_fps)) {
			dpu_pr_err("[DP] Invalid user mode value, user fps:[%d]", dptx->user_timing->fps);
			return -EINVAL;
		}
	}
	...
	dpu_pr_info("[DP] The mode is changed as [%d * %d @%d Hz], ret = %d\n", mdtd.h_active, mdtd.v_active,
