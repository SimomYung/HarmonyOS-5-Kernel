...
struct dtd {
	...
	uint16_t interlaced; /* 1 for interlaced, 0 progressive */
...
struct dp_ctrl {
	...
	*/
	...
	uint32_t crc_count;
	uint32_t bit_error_rate_poll;
	uint32_t error_type;
	uint32_t screen_ratio_size;
	...
	struct edid_support_timing *user_timing;
	...
	int (*dptx_get_user_edid_timing_id)(struct dp_ctrl *dptx, struct edid_support_timing *user_timing);
	void (*delete_invalid_timing_node)(struct dp_ctrl *dptx);
