...
void dptx_read_phy_reg(struct dp_ctrl *dptx, uint32_t reg, bool is_info)
{
	...
	if (dptx_is_edp_panel(dptx) && dptx->dptx_show_phy_status_handler)
		dptx->dptx_show_phy_status_handler(dptx);

	aux_sts = inp32(dptx->combo_phy_base + 0x68c);
	mode_sts = inp32(dptx->combo_phy_base + 0x1c);
	if (is_info)
		dpu_pr_info("[DP] 0x68c:0x%x, 0x1c:0x%x, reg:0x%x", aux_sts, mode_sts, reg);
	else
		dpu_pr_debug("[DP] 0x68c:0x%x, 0x1c:0x%x, reg:0x%x", aux_sts, mode_sts, reg);
}
...
static bool dp_link_status_changed(struct dp_ctrl *dptx)
{
	...
	if (dptx->is_edid_change)
		return true;
	if (dptx->is_pixel_changed && dptx->video_transfer_enable)
		return true;
...
void dp_send_cable_notification(struct dp_ctrl *dptx, int val)
{
	...
	if (dptx->video_transfer_enable && val == HOT_PLUG_IN && dp_link_status_changed(dptx)
		...
		dpu_pr_warn("[DP] is_edid_change is %d, is_pixel_changed is %d", dptx->is_edid_change, dptx->is_pixel_changed);
...
static int dp_ctrl_on(struct dkmd_connector_info *pinfo)
{
	...
	for (i = 0; (i < connector_cnt) && (i < MAX_DPTX_COUNT); i++) {
		...
		* is greater than dp_relink_overtime seconds, the connection needs to be reestablished. */
		if (dp_priv->dp_relink_overtime > 0 && dptx->dptx_enable && !dptx->power_saving_mode &&
				dptx->plugin_jiffies > 0 && jiffies > (dptx->plugin_jiffies + HZ * dp_priv->dp_relink_overtime)) {
		...
		if (dptx->dptx_enable && dptx->power_saving_mode && dptx->dptx_power_handler) {
			...
			if (ret) {
				...
				dptx->power_saving_mode = true;
				dp_add_error_log(dptx, "powerhandler failed");
				dp_dmd_print_dfx(dptx, DMD_DP_FAILED, dptx->error_type);
			} else {
				dp_dmd_print_dfx(dptx, DMD_DP_CONNECT, dptx->error_type);
...
static int panel_power_off_no_need_dp_off(struct dkmd_connector_info *pinfo)
{
	struct dp_ctrl *dptx = NULL;
	dptx = dp_get_dptx(pinfo);
	if (dptx == NULL) {
		dpu_pr_info("[DP] dptx is null!\n");
		return 0;
	}

	if (dptx->dp_panel_mgr && dptx->dp_panel_mgr->panel_power_no_need_dp_off) {
...
static int dp_ctrl_off(struct dkmd_connector_info *pinfo)
{
	...
	for (i = 0; (i < connector_cnt) && (i < MAX_DPTX_COUNT); i++) {
		...
		dpu_pr_info("[DP] dp%d (portid:%d) enter!\n", i, dptx->port_id);
		mutex_lock(&dptx->dptx_mutex);
		if (panel_power_off_no_need_dp_off(pinfo) == NEED_SKIP_DP_OFF) {
            mutex_unlock(&dptx->dptx_mutex);
            return 0;
        }
		dptx->is_power_offing = true;
...
static int32_t set_dptx_display_timing(struct dkmd_connector_info *pinfo, struct edid_support_timing *user_timing)
{
	struct dp_private *dp_priv = NULL;
	struct dp_ctrl *dptx = NULL;

	dpu_check_and_return(!user_timing, -EINVAL, err, "[DP] get user_timing failed!");
	dpu_check_and_return(!pinfo, -EINVAL, err, "[DP] get connector info failed!");
	dp_priv = to_dp_private(pinfo);
	dpu_check_and_return(!dp_priv, -EINVAL, err, "[DP] dp_priv is null pointer\n");
	dptx = &dp_priv->dp[MASTER_DPTX_IDX];
	...
	if (user_timing->hactive == pinfo->base.xres && user_timing->vactive == pinfo->base.yres) {
		dpu_pr_warn("[DP] User set resolution is equal to current resolution[%u * %u]",
			pinfo->base.xres, pinfo->base.yres);
		return 0;
	}

	if (dptx->dptx_get_user_edid_timing_id(dptx, user_timing) < 0)
		return -EINVAL;

	dpu_pr_info("[DP] dptx set display resolution [%u * %u@%uHz] start!", user_timing->hactive, user_timing->vactive,
		user_timing->fps);

	mutex_lock(&dptx->dptx_mutex);
	dptx->user_timing = user_timing;
	dptx->is_ldi_enable = false;
	/* DP plug out */
	dpu_pr_info("[DP] dptx set display resolution start!");
	dptx_connect_status_mutex(pinfo, DP_DISCONNECTED);
	if (dptx->handle_hotunplug)
		dptx->handle_hotunplug(dptx);
	mutex_unlock(&dptx->dptx_mutex);
	...
	dpu_pr_info("[DP] The DP has been plugout and is plugin");
	...
	if (dptx->dptx_core_on)
		dptx->dptx_core_on(dptx);
	/* connect_status need update before dp set uevent to hwc */
	dptx_connect_status_mutex(pinfo, DP_CONNECTED);
	dptx->user_mode = 1;
	if (dptx->handle_hotplug && dptx->handle_hotplug(dptx) != 0) {
		dpu_pr_err("[DP] dptx set display timing failed!\n");
		mutex_unlock(&dptx->dptx_mutex);
		dptx->user_mode = 0;
		return -EINVAL;
	}
...
void dp_ctrl_default_setup(struct dpu_connector *connector)
{
	...
	connector->conn_info->set_device_display_timing = set_dptx_display_timing;
