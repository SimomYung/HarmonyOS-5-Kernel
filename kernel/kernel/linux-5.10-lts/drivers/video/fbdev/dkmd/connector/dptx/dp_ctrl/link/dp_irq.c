...
void dptx_show_err_count(struct dp_ctrl *dptx)
{
	uint8_t bytes[8] = {0};
	int retval = 0;

	dpu_check_and_no_retval(!dptx, err, "[DP] dptx NULL Pointer\n");
	if (dptx_is_edp_panel(dptx))
		return;

	dptx->bit_error_rate_poll++;
	if (dptx->bit_error_rate_poll % 900 != 0)
		return;

	retval = dptx_read_bytes_from_dpcd(dptx, DP_SYMBOL_ERROR_COUNT_LANE0, bytes, sizeof(bytes));
	dpu_check_and_no_retval(retval, err, "[DP] Read dptx_read_bytes_from_dpcd error\n");

	if (dptx->link.lanes == 4 && bytes[1] == 0x80 && bytes[0] == 0x0 && bytes[3] == 0x80 && bytes[2] == 0 &&
		bytes[5] == 0x80 && bytes[4] == 0x0 && bytes[7] == 0x80 && bytes[6] == 0x0)
		return;
	else if (dptx->link.lanes == 2 && bytes[1] == 0x80 && bytes[0] == 0x0 && bytes[3] == 0x80 && bytes[2] == 0)
		return;
	else if (dptx->link.lanes == 1 && bytes[1] == 0x80 && bytes[0] == 0x0)
		return;

	dpu_pr_info("[DP] port id %d bit_error_rate, 0:[0x%x-0x%x] 1:[0x%x-0x%x] 2:[0x%x-0x%x] 3[0x%x-0x%x] (%d)",
		dptx->port_id, bytes[1], bytes[0], bytes[3], bytes[2], bytes[5],
		bytes[4], bytes[7], bytes[6], dptx->bit_error_rate_poll);
	return;
}
...
int handle_hotunplug(struct dp_ctrl *dptx)
{
	...
	dptx->filter_resolution_enable = false;
...
static bool dptx_edid_one_block_check(struct dp_ctrl *dptx, uint8_t* edid)
{
	...
	if (checksum & 0xFF) {
		...
		dp_add_error_log(dptx, "EDIDchecksumfailed");
		return false;
	}
	return true;
}
...
static int dptx_read_edid_block(struct dp_ctrl *dptx, unsigned int block)
{
	...
	while (retry <= EDID_READ_RETRY) {
		...
		if (retval || !dptx_edid_one_block_check(dptx, &dptx->edid[block * EDID_BLOCK_LENGTH])) {
			...
			dp_add_error_log(dptx, "i2c_or_sum_failed");
			retry++;
			mdelay(3);
			continue;
		}
...
static int dptx_read_edid(struct dp_ctrl *dptx)
{
	...
	for (i = 1; i <= ext_blocks; i++) {
		if (dptx_read_edid_block(dptx, i) != 0)
			goto fail;
	}
	...
	struct timing_info *per_timing_info, uint32_t fps, uint32_t fps_default)
{
	...
	uint32_t target_screen_ratio = 0;
	...
	if ((g_dp_debug_mode_enable != 0) && (g_dp_vactive_pixels_debug != 0) &&
		...
		dpu_pr_info("[DP] timing is filtered, vactive_pixels %hu, vactive_pixels_debug %hu, hactive_pixels %hu, \
	...
	/* If the timing node is same, so it need not be update the current perfered timing */
	if (dptx_timing_node->hactive_pixels == per_timing_info->hactive_pixels &&
			...
			(VALUE_DIFF(target_fps, fps) > VALUE_DIFF(target_fps, fps_default) ||
			VALUE_DIFF(fps, fps_default) <= 1)) {
		dpu_pr_info("[DP] current timing[%u * %u @ %u] is same as current preferred timing[%u * %u @ %u]",
	...
	/* Select the default and proper resolution that should be closer to the target screen ratio */
	cur_timing_ratio = (dptx_timing_node->hactive_pixels >= dptx_timing_node->vactive_pixels ?
						IMAGE_RATIO(dptx_timing_node->hactive_pixels, dptx_timing_node->vactive_pixels) :
						IMAGE_RATIO(dptx_timing_node->vactive_pixels, dptx_timing_node->hactive_pixels));
	cur_perf_timing_ratio = (per_timing_info->hactive_pixels >= per_timing_info->vactive_pixels ? 
							IMAGE_RATIO(per_timing_info->hactive_pixels, per_timing_info->vactive_pixels) :
							IMAGE_RATIO(per_timing_info->vactive_pixels, per_timing_info->hactive_pixels));
	target_screen_ratio = dptx->screen_ratio_size;
	if ((VALUE_DIFF(target_screen_ratio, cur_timing_ratio) > VALUE_DIFF(target_screen_ratio, cur_perf_timing_ratio)) &&
			target_screen_ratio > 0) {
		dpu_pr_info("[DP] current perfered ratio[%u] is closer to target screen ratio[%u] than current ratio[%u]",
			cur_perf_timing_ratio, target_screen_ratio, cur_timing_ratio);
		return false;
	} else if ((VALUE_DIFF(target_screen_ratio, cur_timing_ratio) < VALUE_DIFF(target_screen_ratio, cur_perf_timing_ratio)) &&
			target_screen_ratio > 0) {
		dpu_pr_info("[DP] current perfered ratio[%u] is not closer to target screen ratio[%u] than current ratio[%u]",
			cur_perf_timing_ratio, target_screen_ratio, cur_timing_ratio);
		return true;
	}

	/* Select the higher resolution within the bandwidth limit */
	if ((dptx_timing_node->hactive_pixels < per_timing_info->hactive_pixels) ||
			(dptx_timing_node->vactive_pixels < per_timing_info->vactive_pixels)) {
		dpu_pr_info("[DP] current timing[%u * %u @ %u] is lower than current preferred timing[%u * %u @ %u]",
			dptx_timing_node->hactive_pixels, dptx_timing_node->vactive_pixels, fps,
			per_timing_info->hactive_pixels, per_timing_info->vactive_pixels, fps_default);
		return false;
	}
...
static bool dptx_check_priority_timing(struct dp_ctrl *dptx, struct timing_info *dptx_timing_node, uint32_t fps)
{
	struct edid_video *vid_info = NULL;
	bool valid_timing = false;
	uint8_t i = 0;

	/*
	The priority policy would be ignored when using HUASHAN monitor, thus the display can select the desired
	resolution when it's connected with some Docks.
	*/
	if (dptx->hook_ops && dptx->hook_ops->is_valid_timing_info) {
		if (dptx->hook_ops->is_valid_timing_info (
			dptx_timing_node->hactive_pixels, dptx_timing_node->vactive_pixels, fps, (uint8_t)dptx->port_id))
			return true;
	}

	vid_info = &(dptx->edid_info.video);
	for (i = 0; i < vid_info->pri_vcount; i++) {
		if (dptx_timing_node->hactive_pixels <= vid_info->pri_timing[i].pri_hpixels &&
				dptx_timing_node->vactive_pixels <= vid_info->pri_timing[i].pri_vpixels)
			valid_timing = true;
	}
	if (!valid_timing)
		dpu_pr_info("[DP] Current timing > priority timing (hactive:%u, vactive:%u), skip this timing node",
			vid_info->pri_timing[i].pri_hpixels, vid_info->pri_timing[i].pri_vpixels);
...
static int dptx_choose_edid_timing(struct dp_ctrl *dptx, uint8_t *bsafe_mode)
{
	...
	list_for_each_entry_safe(dptx_timing_node, _node_, dptx->edid_info.video.dptx_timing_list, list_node) {
		...
		dpu_pr_info("[DP] timing start active_pixels %d, hactive_pixels %d, pc: %d, mpc: %d",
			dptx_timing_node->vactive_pixels, dptx_timing_node->hactive_pixels, dptx_timing_node->pixel_clock,
			dptx->edid_info.video.max_pixel_clock);
		...
		if ((dptx_timing_node->interlaced != 1) && (dptx_timing_node->vblanking <= VBLANKING_MAX) &&
			...
			if (!dptx_check_priority_timing(dptx, dptx_timing_node, fps))
				continue;
...
void dptx_link_params_reset(struct dp_ctrl *dptx)
{
	...
	dptx->filter_resolution_enable = (bool)pinfo->filter_resolution_enable;
	dptx->is_new_frame_update = false;
...
static int dptx_link_get_device_caps(struct dp_ctrl *dptx)
{
	...
	if (retval != 0) {
		dp_add_error_log(dptx, "failed_write_POWER");
	...
	*bsafe_mode = SAFE_MODE_NO;
	...
	if (!dptx->mst) {
		...
		if (retval < EDID_BLOCK_LENGTH) {
			...
			*edid_info_size = 0;
			dp_add_error_log(dptx, "failed rd edid safe");
			dptx->error_type = DMD_DP_FAILED_SAFEMODE;
			*bsafe_mode = SAFE_MODE_YES;
			return retval;
		} else {
		...
		if (retval) {
			...
			dp_add_error_log(dptx, "EDID fail safe mode");
			dptx->error_type = DMD_DP_FAILED_SAFEMODE;
			*bsafe_mode = SAFE_MODE_YES;
			return retval;
		}
...
int handle_hotplug(struct dp_ctrl *dptx)
{
	...
	do {
		...
		if ((retry > GET_DEVICE_CAPS_TIMES) && (retval != 0)) {
			...
			dp_add_error_log(dptx, "failed getdevicecaps");
	...
	if (retval) {
		...
		dp_add_error_log(dptx, "failed linktraining");
	...
	if (retval) {
		...
		dp_add_error_log(dptx, "Failed to init psr");
