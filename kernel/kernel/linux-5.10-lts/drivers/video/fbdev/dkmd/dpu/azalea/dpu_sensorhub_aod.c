...
extern int dpu_aod_panel_handle(int blank_mode);
#endif
...
int dpu_sensorhub_aod_dvfs_vote(uint32_t msg_no)
{
	void_unused(msg_no);
	return 0;
}
...
static bool dpu_sh_need_start_aod(struct dpu_fb_data_type *dpufd)
{
#ifdef CONFIG_LIBLINUX
	struct dpu_fb_data_type *dpufd_other = NULL;

	/* If another screen is power on, the current screen will not enter AOD when it is powered off */
	if (dpufd->panel_info.product_type == PANEL_DUAL_PANEL_TYPE && IS_BUILTIN_PANEL) {
		if (dpufd->index == PRIMARY_PANEL_IDX)
			dpufd_other = dpufd_list[BUILT_IN_PANEL_INDEX];
		else
			dpufd_other = dpufd_list[PRIMARY_PANEL_IDX];

		if (dpufd_other->panel_power_on)
			return false;
	}
#endif
	return true;
...
int dpu_fb_aod_blank(struct dpu_fb_data_type *dpufd, int blank_mode)
{
	...
	if (blank_mode == FB_BLANK_UNBLANK) {
		...
		if (!dpu_sh_need_start_aod(dpufd)) {
			DPU_FB_INFO("current panel is %d, other panel is power on\n", dpufd->index);
			return ret;
		}
...
int dpu_blank_peri_handle(struct dpu_fb_data_type *dpufd, const void __user *argp)
{
	int ret = 0;
    ...
    int blank_mode = (int)(uintptr_t)argp;
    DPU_FB_INFO("blank_mode:%d", blank_mode);

	switch (blank_mode) {
	case FB_BLANK_PERI_UNBLANK:
	case FB_BLANK_PERI_POWERDOWN:
	    ret = dpu_aod_panel_handle(blank_mode);
	    break;
	default:
	    DPU_FB_INFO("dpu aod panel handle mode:%d", blank_mode);
		break;
	}
