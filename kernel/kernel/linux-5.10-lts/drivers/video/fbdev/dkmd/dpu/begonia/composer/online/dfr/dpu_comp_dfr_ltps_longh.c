...
static void dfr_te_skip_unregister_te_isr(struct dpu_comp_dfr_ctrl *dfr_ctrl)
{
	...
	isr_ctrl = &dfr_ctrl->dpu_comp->isr_ctrl;
	dsi_te_id = pinfo->base.lcd_te_idx == 0 ? DSI_INT_LCD_TE0 : DSI_INT_LCD_TE1;
	ukmd_isr_unregister_listener(isr_ctrl, &g_te_isr_notifier, dsi_te_id);
}
...
static void dfr_longh_unregister_vactive_start_isr(struct dpu_comp_dfr_ctrl *dfr_ctrl)
{
	...
	isr_ctrl = &dfr_ctrl->dpu_comp->isr_ctrl;
	ukmd_isr_unregister_listener(isr_ctrl, &g_vactive_start_isr_notifier,
		DSI_INT_VACT0_START);
}
...
int32_t dfr_ltps_longh_send_dcs_cmds(struct dpu_comp_dfr_ctrl *dfr_ctrl, struct disp_effect_params *effect_params)
{
	struct dkmd_connector_info *pinfo = NULL;
...
static struct dfr_ctrl_ops g_dfr_ctrl_ops = {
	...
	.setup_data = dfr_ltps_longh_setup_priv_data,
	.release_data = dfr_ltps_longh_release_priv_data,
	.commit = dfr_ltps_longh_commit,
