...
struct f2fs_sb_info {
	...
	struct f2fs_block_alloc_info *ba_info;      /* block alloc statistics */
#endif
...
static inline struct f2fs_block_alloc_info *F2FS_BA_STAT(struct f2fs_sb_info *sbi)
{
	return (struct f2fs_block_alloc_info *)sbi->ba_info;
}
#define inc_ba_val(sbi, member, val) do {			\
	struct f2fs_block_alloc_info *ba = F2FS_BA_STAT(sbi);	\
	if (ba)							\
		ba->member += (val);				\
} while (0)
#define inc_ba_array_val(sbi, member, idx, val) do {		\
	struct f2fs_block_alloc_info *ba = F2FS_BA_STAT(sbi);	\
	if (ba)							\
		ba->member[(idx)] += (val);			\
} while (0)
#define ba_mutex_lock(mutex) mutex_lock((mutex))
#define ba_mutex_unlock(mutex) mutex_unlock((mutex))
#else
...
#define inc_ba_val(sbi, member, val)
#define inc_ba_array_val(sbi, member, idx, val)
#define ba_mutex_lock(mutex)
#define ba_mutex_unlock(mutex)
#endif
