					     ...
					     unsigned flags)
{
	...
	spin_lock(&kernfs_idr_lock);
#ifdef CONFIG_LIBLINUX
	ret = idr_alloc_cyclic(&root->ino_idr, kn, 1, LIBLINUX_SYSFS_MAX, GFP_ATOMIC);
#else
...
#ifdef CONFIG_LIBLINUX
	trace_ldk_vh_kernfs_add_one(kn, kernfs_next_descendant_post,
						kernfs_activate_one, __kernfs_remove, &ret);
	/* Node activation will be done inside trace so skip kernfs_activate() */
	goto out_unlock;
#endif
...
#ifdef CONFIG_LIBLINUX
		int ret = 0;
		trace_ldk_vh_kernfs_put(pos, &ret);
...
#ifdef CONFIG_LIBLINUX
	trace_ldk_rvh_kernfs_rename_ns(kn, new_parent, new_name, new_ns, &error);
	if (error < 0) {
		if (new_name)
