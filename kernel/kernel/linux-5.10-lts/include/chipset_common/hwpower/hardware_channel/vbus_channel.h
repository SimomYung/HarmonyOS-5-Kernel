...
struct vbus_ch_ops {
	...
	int (*open_with_volt)(unsigned int user, int flag, int volt);
...
static inline int vbus_ch_open_with_volt(unsigned int user, unsigned int type, int flag, int volt)
{
	return -EPERM;
}
...
static inline int vbus_ch_close_switch(unsigned int user, unsigned int type, bool flag)
{
	return -EPERM;
}
