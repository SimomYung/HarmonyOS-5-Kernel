...
#define ADAPTER_SN_MAX_SIZE                   24
#define ADAPTER_INFO_MAX_SIZE                 16
#define ADAPTER_SN_PRODUCT_TYPE               1
#define ADAPTER_SN_PRODUCT_TYPE_SHIFT         2
#define ADAPTER_SN_POWER_BANK_PRODUCT         2
...
enum adapter_support_vol {
	ADAPTER_SUPPORT_5V = 5000,
	ADAPTER_SUPPORT_10V = 10000,
	ADAPTER_SUPPORT_15V = 15000,
	ADAPTER_SUPPORT_20V = 20000,
};
...
struct adp_precise_pwr_curve_data {
	u8 min_curr;
	u16 max_curr;
	u16 min_volt;
	u16 max_volt;
...
struct adp_scpb_pwr_data {
	...
	struct adp_precise_pwr_curve_data curve_data;
};
...
struct adapter_protocol_ops {
	...
	int (*get_precise_power_curve)(struct adp_precise_pwr_curve_data *val, int *size, int max_size);
	...
	bool (*is_power_redistributed)(void);
	int (*set_send_data)(u32 *data, u8 cnt, bool wait_rsp, int type);
	bool (*support_9v)(void);
	int (*get_adp_sn)(u8 *sn_info, u8 size);
	int (*get_power_bank_cap)(int *cap);
};
...
int adapter_get_precise_power_curve(int prot, struct adp_precise_pwr_curve_data *val, int *size, int max_size);
...
bool adapter_is_power_redistributed(int prot);
int adapter_uvdm_send_data(int prot, u32 *data, u8 cnt, bool wait_rsp, int type);
bool adapter_support_9v(int prot);
int adapter_get_adp_sn(int prot, u8 *sn_info, u8 len);
int adapter_get_power_bank_cap(int prot, int *cap);
#else
...
static inline int adapter_get_precise_power_curve(int prot,
	struct adp_precise_pwr_curve_data *val, int *size, int max_size)
{
	return -EPERM;
}
...
static inline bool adapter_is_power_redistributed(int prot)
{
	return false;
}
...
static inline bool adapter_support_9v(int prot)
{
	return false;
}

static inline int adapter_get_adp_sn(int prot, u8 *sn_info, u8 len)
{
	return -EPERM;
}

static inline int adapter_get_power_bank_cap(int prot, int *cap)
{
	return -EPERM;
}
