...
enum wireless_protocol_device_id {
	...
	WIRELESS_DEVICE_ID_MT5788,
	WIRELESS_DEVICE_ID_BK96285,
...
struct wlprot_acc_cap {
	...
	bool support_fod_km;
	bool dc_pwr;
	bool support_qval_cali;
	bool support_adp_info;
	bool support_fod_info;
...
struct wireless_device_info {
	...
	char tx_serial_num[WIRELESS_TX_SN_LEN];
	int tx_id;
	int tx_type;
	int tx_adp_type;
};

struct wltx_fod_result_info {
	u8 q_qp_ratio;
	u8 limit_pwr;
	u8 qs;
	u8 fs;
...
struct wireless_protocol_ops {
	...
	int (*get_tx_type)(unsigned int ic_type, int *type, char *ver, int len);
	int (*get_tx_sn)(unsigned int ic_type, char *ver, int len);
	int (*get_tx_adapter_type)(unsigned int ic_type, int *type);
	int (*get_tx_adapter_id)(unsigned int ic_type, u16 *adp_id, u8 *adp_manufacturer);
	int (*get_tx_adapter_err_code)(unsigned int ic_type, u16 *err_code);
	int (*get_tx_capability)(unsigned int ic_type, struct wlprot_acc_cap *cap);
	int (*get_tx_actual_power)(unsigned int ic_type, int *tx_pwr);
	...
	int (*get_fod_result_info)(unsigned int ic_type, struct wltx_fod_result_info *fod_result_info);
	int (*get_fod_status)(unsigned int ic_type, u8 *qr, u8 *fr);
	int (*get_fod_km_kz)(unsigned int ic_type, u16 *km, u16 *kz);
...
int wireless_get_tx_sn(unsigned int ic_type, int prot);
int wireless_get_tx_adapter_type(unsigned int ic_type, int prot);
int wireless_get_tx_adapter_id(unsigned int ic_type, int prot, u16 *adp_id, u8 *adp_manufacturer);
int wireless_get_tx_adapter_err_code(unsigned int ic_type, int prot, u16 *err_code);
int wireless_get_tx_capability(unsigned int ic_type, int prot, struct wlprot_acc_cap *cap);
int wireless_get_tx_actual_power(unsigned int ic_type, int prot, int *tx_pwr);
...
int wireless_get_fod_result_info(unsigned int ic_type, int prot,
	struct wltx_fod_result_info *fod_result_info);
int wireless_get_fod_status(unsigned int ic_type, int prot, u8 *qr, u8 *fr);
int wireless_get_fod_km_kz(unsigned int ic_type, int prot, u16 *km, u16 *kz);
...
void wireless_protocol_reset_para(void);
#else
...
static inline int wireless_get_tx_sn(unsigned int ic_type, int prot)
{
	return -ENOTSUPP;
}
...
static inline int wireless_get_tx_adapter_id(unsigned int ic_type, int prot, u16 *adp_id, u8 *adp_manufacturer)
{
	return -ENOTSUPP;
}

static inline int wireless_get_tx_adapter_err_code(unsigned int ic_type, int prot, u16 *err_code)
{
	return -ENOTSUPP;
}
...
static inline int wireless_get_tx_actual_power(unsigned int ic_type, int prot, int *tx_pwr)
{
	return -ENOTSUPP;
}
...
static inline int wireless_get_fod_result_info(unsigned int ic_type, int prot,
	struct wltx_fod_result_info *fod_result_info)
{
	return -ENOTSUPP;
}

static inline int wireless_get_fod_status(unsigned int ic_type, int prot, u8 *qr, u8 *fr)
{
	return -ENOTSUPP;
}

static inline int wireless_get_fod_km_kz(unsigned int ic_type, int prot, u16 *km, u16 *kz)
{
	return -ENOTSUPP;
}
...
static inline void wireless_protocol_reset_para(void)
{
