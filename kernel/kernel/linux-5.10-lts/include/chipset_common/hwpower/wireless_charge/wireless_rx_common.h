...
#define WLRX_DEVICE_DIRECTION_DFLT          (-1)
...
enum wlrx_scene {
	...
	WLRX_SCN_HALL_ONLY,
	WLRX_SCN_BRACKETSHELL,
	/* new scene must be appended */
	WLRX_SCN_END,
};

enum wlrx_device_direction {
	WLRX_DEVICE_DIRECTION_BEGIN = 0,
	WLRX_DEVICE_DIRECTION_TOP = WLRX_DEVICE_DIRECTION_BEGIN,
	WLRX_DEVICE_DIRECTION_LEFT,
	WLRX_DEVICE_DIRECTION_BOTTOM,
	WLRX_DEVICE_DIRECTION_RIGHT,
	WLRX_DEVICE_DIRECTION_END,
};
...
void wlrx_chip_init(unsigned int drv_type, int vtx);
...
void wlrx_set_pwroff_reset_flag(unsigned int drv_type, bool flag);
bool wlrx_in_pwroff_reset(unsigned int drv_type);
void wlrx_common_reset_para(unsigned int drv_type, int rst_type);
void wlrx_wait_fop_fix_to_default(int vtx);
void wlrx_set_device_direction(unsigned int drv_type, int device_direction);
int wlrx_get_device_direction(unsigned int drv_type);
#else
...
static inline void wlrx_chip_init(unsigned int drv_type, int vtx)
{
}
...
static inline void wlrx_set_iout_min(void)
{
}

static inline void wlrx_set_pwroff_reset_flag(unsigned int drv_type, bool flag)
{
}

static inline bool wlrx_in_pwroff_reset(unsigned int drv_type)
{
	return false;
}

static inline void wlrx_common_reset_para(unsigned int drv_type, int rst_type)
{
}

static inline void wlrx_wait_fop_fix_to_default(int vtx)
{
}

static inline void wlrx_set_device_direction(unsigned int drv_type, int device_direction)
{
}

static inline int wlrx_get_device_direction(unsigned int drv_type)
{
	return WLRX_DEVICE_DIRECTION_DFLT;
}
