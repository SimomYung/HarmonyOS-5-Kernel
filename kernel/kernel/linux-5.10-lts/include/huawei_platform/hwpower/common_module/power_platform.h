...
static inline int power_platform_support_cable_auth(void)
{
	return pd_dpm_support_cable_auth();
}

static inline bool power_platform_support_new_pd_process(void)
{
	return charge_context(support_new_pd_process);
}
...
static inline void power_platform_start_acr_calc(bool is_eis_replacing_acr)
{
	if (is_eis_replacing_acr) {
		eis_high_freq_cal_start();
	}
	soh_acr_low_precision_cal_start();
}

static inline int power_platform_get_acr_resistance(int *eis_value, bool is_eis_replacing_acr)
{
	int ret;
	struct bat_acr_info acr_data;
        struct eis_high_freq_info eis_data = {0};

        if (!eis_value)
                return -EPERM;

	if (is_eis_replacing_acr) {
		ret = eis_get_high_freq_resistance(&eis_data);
                if (!ret)
			*eis_value = eis_data.batt_acr;
		return ret;
	} 

	memset(&acr_data, 0, sizeof(acr_data));
	ret = soh_get_acr_resistance(&acr_data, ACR_L_PRECISION);
	*eis_value = acr_data.batt_acr;
	return ret;
}
