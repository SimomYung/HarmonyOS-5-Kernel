...
#define ML_ROAM_TRIGGER       1
#define ML_ROAM_NO_CANDIDATE  2
#define ML_ROAM_START         3
#define ML_ROAM_INFO_REPORT   4
#define ML_ROAM_FAIL          5
#define DUALBAND_ROAM_SUPPORT 1

typedef struct{
	uint32_t ml_roam_msg_type;
	uint8_t ml_roam_band;
	uint8_t support_zero_roaming;
	uint8_t ml_auc_bssid[ETH_ALEN];
} amax_dualband_roam_info_stru;
...
typedef int32_t (*amax_ml_roam_func)(amax_dualband_roam_info_stru *dualband_roam_info);
...
extern int32_t amax_hook_roam_info_handle(amax_dualband_roam_info_stru *dualband_roam_info);
extern void amax_roam_hook_reg(amax_ml_roam_func ml_roam_info_func);
extern void amax_roam_hook_unreg(void);
/* hmac驱动调用 */
