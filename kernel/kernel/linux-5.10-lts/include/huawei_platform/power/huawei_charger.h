...
#include <huawei_platform/power/huawei_charger_pd.h>
...
#define INCREASE_VOLT_DEFAULT               150
...
enum soft_recharge_para {
	...
	RECHARGE_IGNORE_CV_CHANGE,
	RECHARGE_UI_SOC_TH,
	RECHARGE_PARA_TOTAL,
};

enum charge_pd_src_cap {
	CHARGE_PD_SRC_CAP_CCH_COMP_MAYBE_LOW = 0,
	CHARGE_PD_SRC_CAP_MAYBE_DISABLE_9V,
	CHARGE_PD_SRC_CAP_END,
};
...
struct charge_core_data {
	...
	unsigned int vterm_change_with_temp_en;
...
struct charge_device_info {
	...
	u32 increase_volt_for_batfet_single;
...
static inline int charge_otg_volt_mode_enable(int enable, int volt, unsigned int user)
{
	return 0;
}
...
static inline void huawei_charger_set_iin(unsigned int chg_type, unsigned int iin, bool is_mod_work)
{
}

static inline void huawei_charger_recv_pd_src_cap(enum charge_pd_src_cap src_cap)
{
}
#endif /* CONFIG_HUAWEI_CHARGER_AP */
