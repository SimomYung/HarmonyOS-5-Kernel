...
struct pd_dpm_ops {
	...
	/* get direct charge status(in or out) */
	int (*pd_dpm_pps_set_state)(void *client, enum pps_set_state state, int mv, int ma);
	...
	void (*pd_dpm_set_pd_certification)(void *client, bool flag);
};
...
enum pd_dpm_vbus_type {
	...
	/* other messages */
	PD_DPM_EMARK_FINISHED, /* emark finished */
};
...
struct pd_dpm_info {
	...
	struct delayed_work pd_certification_work;

	struct delayed_work usb_enum_timeout_work;
	struct work_struct usb_suspend_work;
	struct notifier_block gadget_event_nb;
...
#if defined(CONFIG_HW_GADGET) && !defined(CONFIG_USB_HW_F_ACM_MUSL)
void usb_config_set_delay(unsigned int delay);
#else
static inline void usb_config_set_delay(unsigned int delay)
{
}
#endif /* CONFIG_HW_GADGET */

#ifdef CONFIG_HW_GADGET
/* dynamically adjusts the USB enum request current */
void pd_reset_bMaxPower(bool state);
#else
static inline void pd_reset_bMaxPower(bool state)
{
}
#endif /* CONFIG_HW_GADGET */
...
static inline int pd_dpm_notify_cable_uvdm(uint32_t *data, uint8_t cnt)
{
	return 0;
}

static inline int pd_dpm_notify_9v2a_enable(void)
{
	return 0;
}
...
static inline bool hw_pd_is_rev_sc_plugin(void)
{
	return false;
}
...
static inline bool hw_pd_is_certification_hiz_en(void)
{
	return false;
}
#endif /* CONFIG_TCPC_CLASS */
