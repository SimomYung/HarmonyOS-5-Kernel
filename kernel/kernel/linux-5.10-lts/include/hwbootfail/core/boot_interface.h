...
struct shutfail_log_header {
	...
	char log_path[3][128]; /* max count and size of upper layer logs */
	int kernel_log_size;
	char reserved[108]; /* reserved count */
} __packed;

struct shutfail_info {
	char time[BF_SIZE_32];
...
enum boot_stage {
	...
	KERNEL_STAGE = (((unsigned int)STAGE_NATIVE) << 28) - 4,
	KERNEL_STAGE_S4_START = (((unsigned int)STAGE_NATIVE) << 28) - 3,
	KERNEL_STAGE_S4_END = (((unsigned int)STAGE_NATIVE) << 28) - 2,
#else
