...
void rx_window_reset(struct session *ss, int id)
{
	...
	log_info("rx window reset id %d", id);
...
void all_window_reset(struct session *ss)
{
	...
	log_info("all window reset id %d", id);
	(void)memset_s(ss->ss_window[id], sizeof(ss->ss_window[id]),
					0x00, sizeof(ss->ss_window[id]));
	ss->begin[id] = 0;
	ss->end[id] = SLIDE_WIN_SIZE;
...
static void slid_half_window(struct session *ss, int id)
{
	...
	log_info("id %d begin %llu end %llu offset %d", id, ss->begin[id], ss->end[id], offset);
...
static bool seq_valid_check(struct session *ss, struct honhdr *honh, int id)
{
	u32 seq;

	if (unlikely(!honh || !ss || (id < 0) || (id >= RX_WIN_NUM))) {
		log_err("input param is invalid, error");
		return false;
	}
	...
	seq = ntohl(honh->seq);
	if (seq < ss->begin[id]) {
		log_err("seq is out of data, error");
		return false;
	}
