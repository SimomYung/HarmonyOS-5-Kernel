...
#define WIFI_CURE_ENABLE true

#define MAX_SERVICE_UID 10000
...
enum rnic_type {
	// no cellular rnic and wifi rnic
	RNIC_OTHER = 0,
	RNIC_CELLULAR = 1,
	RNIC_WIFI = 2,
};
...
{
	memset_s(info, sizeof(struct uid_info), 0, sizeof(struct uid_info));
	info->uid = uid;
...
static void notify_drop_event(int uid, struct packet_drop_stats *stats)
{
	...
	// 24 eq len(2B type + 2B len + 4B uid + 4B hook + 4B pkts + 4B dns_drop_count + 4B pkt_type)
	assign_short(p, 24);
	skip_byte(p, sizeof(s16));
	// uid
	assign_int(p, uid);
	...
	skip_byte(p, sizeof(int));
	// wifi_pkts
	assign_int(p, stats->wifi_pkts);
	skip_byte(p, sizeof(int));

	notifier((struct res_msg_head *)event);
}

static void notify_recovery_event(int uid, struct packet_drop_stats *stats, int pkt_type)
{
	char event[NOTIFY_BUF_LEN] = {0};
	char *p = event;
	...
	skip_byte(p, sizeof(s16));
	// 16 eq len((2B type + 2B len + 4B uid + 4B hook + 4B pkt_type))
	assign_short(p, 16);
	skip_byte(p, sizeof(s16));
	// uid
	assign_int(p, uid);
	...
	skip_byte(p, sizeof(int));
	// pkt_type
	assign_int(p, pkt_type);
	notifier((struct res_msg_head *)event);
}

static void update_packet_count(int uid, struct packet_drop_stats *stats, bool is_dns_drop, int pkt_type)
{
	 ...
	 */
	if (pkt_type == RNIC_WIFI) {
		stats->wifi_pkts = stats->wifi_pkts + 1;
	} else if(pkt_type == RNIC_CELLULAR) {
		stats->pkts = stats->pkts + 1;
	}

	if (is_dns_drop)
		stats->dns_drop_count = stats->dns_drop_count + 1;
	if ((stats->pkts >= threshold || stats->wifi_pkts >= threshold) && (!stats->reported_drop)) {
...
static int update_filter_pkts(struct uid_info *info,
	struct packet_drop_stats *stats, int pass, bool is_dns_drop, int pkt_type)
{
	int ret = 0;
	...
	if (pass == PASS) {
		...
		if (stats->reported_drop && !stats->reported_recovery) {
			notify_recovery_event(info->uid, stats, pkt_type);
			stats->reported_recovery = true;
			stats->reported_drop = false;
			stats->ts_reset = 0;
		}
	} else {
		update_packet_count(info->uid, stats, is_dns_drop, pkt_type);
		ret = info->bypass;
	}
	return ret;
}
...
static int update_ip_out_pkts(struct uid_info *info,
	struct packet_drop_stats *stats, int pass, int pkt_type)
{
	if (pass != PASS)
		return 0;
...
static int hw_pfb_hooks_info_judge(int i, int hook, int pass, bool is_dns_drop, int pkt_type)
{
	int ret = 0;
	if (i < listen_uids.nums) {
		...
		case HW_PFB_INET6_LOCAL_OUT:
			ret = update_filter_pkts(&listen_uids.infos[i],
				&listen_uids.infos[i].local_out_drop_stats, pass, is_dns_drop, pkt_type);
			break;
		case HW_PFB_INET_BPF_EGRESS:
		case HW_PFB_INET6_BPF_EGRESS:
			ret = update_filter_pkts(&listen_uids.infos[i],
				&listen_uids.infos[i].bpf_egress_drop_stats, pass, is_dns_drop, pkt_type);
			break;
		case HW_PFB_INET_IP_XMIT:
		case HW_PFB_INET6_IP_XMIT:
			ret = update_ip_in_pkts(&listen_uids.infos[i],
				&listen_uids.infos[i].ip_in_stats, pass, is_dns_drop, pkt_type);
			break;
		case HW_PFB_INET_DEV_XMIT:
		case HW_PFB_INET6_DEV_XMIT:
			ret = update_ip_out_pkts(&listen_uids.infos[i],
				&listen_uids.infos[i].ip_in_stats, pass, pkt_type);
			break;
		case HW_PFB_INET_LOCAL_IN:
		case HW_PFB_INET6_LOCAL_IN:
			ret = update_filter_pkts(&listen_uids.infos[i],
				&listen_uids.infos[i].local_in_drop_stats, pass, is_dns_drop, pkt_type);
			break;
		case HW_PFB_INET_BPF_INGRESS:
		case HW_PFB_INET6_BPF_INGRESS:
			ret = update_filter_pkts(&listen_uids.infos[i],
				&listen_uids.infos[i].bpf_ingress_drop_stats, pass, is_dns_drop, pkt_type);
			break;
	...
	struct net_device *out, int pass)
{
	...
	if (is_ds_rnic(in) || is_ds_rnic(out))
		pkt_type = RNIC_CELLULAR;
	if (WIFI_CURE_ENABLE && (is_wifi_rnic(in) || is_wifi_rnic(out)))
		pkt_type = RNIC_WIFI;
	if (pkt_type == RNIC_OTHER)
		return ret;
	...
	ret = hw_pfb_hooks_info_judge(i, hook, pass, is_dns_drop, pkt_type);
	spin_unlock_bh(&listen_uids.lock);
...
static void reset_socket_close_info(void)
{
	...
	(void)memset_s(&g_socket_close_uids_info, sizeof(struct chr_socket_close_info),
        0, sizeof(struct chr_socket_close_info));
...
{
	uid_t uid = hw_get_sock_uid((struct sock *)sk);

	if (uid < MAX_SERVICE_UID)
		return true;

	return is_foreground_uid(uid);
}
#endif
...
static bool is_netd_process()
{
	...
	if (current != NULL && current->group_leader != NULL) {
		...
		rcu_read_unlock();
		if (task->active_mm != NULL && task->active_mm->exe_file != NULL &&
            task->active_mm->exe_file->f_path.dentry != NULL) {
