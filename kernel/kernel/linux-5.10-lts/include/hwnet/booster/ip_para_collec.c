...
static DEFINE_SPINLOCK(g_tcp_recover_qoe_state_lock);
static u32 g_foreground_uid = 0;
static u32 g_tcp_recover_qoe_state = 0;
...
struct fg_custom_info g_st_fg_custom_info;
 ...
 */
static void notify_traffic_info_process(char *event, u16 len)
{
	if (len < sizeof(struct res_msg_head))
		return;
...
static void update_traffic_info(struct sk_buff *skb, struct sock *sk, int len, u32 uid)
{
	...
	if (match_net_dev(skb, sk, &dev_id) != RTN_SUCC) {
		pr_debug("[IP_PARA]update traffic info get dev id failed");
		return;
	}
	...
	if (dev_id < 0 || dev_id > WIFI_NET_ID) {
		pr_debug("[IP_PARA]update traffic info dev id error");
		return;
	}
	if (g_dev_id_network_type[dev_id] < 0 || g_dev_id_network_type[dev_id] >= RAT_NUM_MAX) {
		pr_debug("[IP_PARA]update traffic info dev id network type error");
		return;
	}
	if (uid < SINGLE_UID_MASK) {
		// only for double frame , App Multi-Opening, another app uid will > UID_MASK
		uid = uid % UID_MASK;
	}
	...
	if (match_traffic_info_ptr == NULL) {
		pr_debug("[IP_PARA]match node is not found");
		return;
	}
...
static void statistics_traffic(u8 protocol, struct sk_buff *skb, struct sock *sk, u8 direction)
{
	...
	notify_traffic_info_process(event, notify_len);
	if (event != NULL)
		kfree(event);
}
...
static void tcp_recover_hiview_report(struct sock *sk, struct sk_buff *ooo_first_skb, u32 estimate_time)
{
	...
	u32 unreceived_pkg = (TCP_SKB_CB(ooo_first_skb)->end_seq - tp->rcv_nxt)/advmss;
	...
	tcp_info.tcp_seq = TCP_SKB_CB(ooo_first_skb)->seq;
...
static bool tcp_recover_pre_check(struct tcp_sock *tp)
{
	return tp != NULL
...
{
	spin_lock_bh(&g_tcp_recover_qoe_state_lock);
	if (!g_tcp_recover_qoe_state) {
		spin_unlock_bh(&g_tcp_recover_qoe_state_lock);
		return;
	}
	spin_unlock_bh(&g_tcp_recover_qoe_state_lock);
	struct tcp_sock *tp = NULL;
	...
	if (tcp_recover_pre_check(tp)) {
		...
		if (jiffies_to_msecs(tcp_jiffies32 - tp->ofo_tstamp) > DURATION_ONE_SEC) {
			u64 estimated_loss_pack = (TCP_SKB_CB(ooo_first_skb)->seq - tp->rcv_nxt) *
					jiffies_to_msecs(tcp_jiffies32 - tp->ofo_tstamp);
			u64 packet_loss_threshold =(tp->rcv_nxt - tp->last_rcv_nxt) *
					TCP_RECOVER_ESTIMATE_TIME_THR;
			if (estimated_loss_pack > packet_loss_threshold) {
				u32 estimate_time = (TCP_SKB_CB(ooo_first_skb)->seq - tp->rcv_nxt) /
						(tp->rcv_nxt - tp->last_rcv_nxt) * jiffies_to_msecs(tcp_jiffies32 - tp->ofo_tstamp);
                pr_err("tcp_recover:ooo_first_skb seq:%u ooo_last_skb seq:%u rcv_nxt:%u last_rcv_nxt:%u usedtime:%u"
                       "estimated_loss_pack:%u packet_loss_threshold:%u \n", TCP_SKB_CB(ooo_first_skb)->seq,
                       TCP_SKB_CB(tp->ooo_last_skb)->seq, tp->rcv_nxt, tp->last_rcv_nxt,
                       jiffies_to_msecs(tcp_jiffies32 - tp->ofo_tstamp), estimated_loss_pack, packet_loss_threshold);
				tp->should_recover = 1;
				spin_lock_bh(&g_tcp_recover_lock);
				g_last_recover_time = jiffies;
				spin_unlock_bh(&g_tcp_recover_lock);
				tcp_recover_hiview_report(sk, ooo_first_skb, estimate_time);
				return;
			} else {
				tp->should_recover = 0;
				tp->should_enter_recover = 0;
...
static void set_dev_name(struct sock *sk, const struct sk_buff *skb)
{
	...
	if ((1 << sk->sk_state) & TCPF_SYN_SENT) {
		dev_max_len = strnlen(skb->dev->name,
			IFNAMSIZ - 1);
		strncpy_s(sk->wifipro_dev_name, dev_max_len + 1, skb->dev->name,
			dev_max_len);
	...
	const struct nf_hook_state *state, u8 protocol, u16 payload_len)
{
	...
	app = &g_app_ctx->cur[cpu];
	if (app == NULL) {
	    pr_debug("stat_process, app is null");
	    return;
	}
	...
	if (state->hook >= NF_INET_TCP_DUPACK_IN_HOOK) {
		...
		if (protocol == IPPROTO_TCP)
			...
			update_udp_para(&pos_selec->stat.info[dev_id], skb,
				state->hook);
			model_get_udp_para(sk, skb, state->hook);
	}
	...
	unsigned int hook)
{
	...
	memset_s(&state, sizeof(struct nf_hook_state), 0, sizeof(struct nf_hook_state));
	state.hook = hook;
...
typedef void (*combine_ip_para)(struct res_msg *res);
static combine_ip_para g_combine_func = NULL;

void dpa_combine_func_register(combine_ip_para func)
{
	pr_info("[IP_PARA]: dpa_combine_func_register");
	g_combine_func = func;
}
EXPORT_SYMBOL(dpa_combine_func_register);

typedef void (*sync_cmd)(struct req_msg_head *msg);
static sync_cmd g_sync_cmd_func = NULL;

void dpa_sync_config_register(sync_cmd func)
{
	pr_info("[IP_PARA]: dpa_sync_config_register");
    g_sync_cmd_func = func;
}
EXPORT_SYMBOL(dpa_sync_config_register);
#endif
	...
	struct app_stat *stat)
{
	...
	for (i = 0; i < cnt; i++) {
		...
		if (need_new) {
			...
			if (pos == NULL)
				goto list_end;
			memset_s(pos, sizeof(struct app_list), 0, sizeof(struct app_list));
			pos->stat.pid = id[i].pid;
...
static void process_sync(struct req_msg *msg)
{
	...
	if (flag & RESTART_SYNC) {
		...
		mod_timer(&g_app_ctx->timer, jiffies + g_app_ctx->jcycle + ms_convert_jiffies(IP_COLLECT_DELAY));
	} else {
...
static void set_tcp_recover_qoe_state(const struct tcp_recover_qoe_msg *msg) {
	if (msg == NULL || msg->len != sizeof(struct tcp_recover_qoe_msg)){
		return;
	}
	spin_lock_bh(&g_tcp_recover_qoe_state_lock);
	g_tcp_recover_qoe_state = msg->qoe_state;
	spin_unlock_bh(&g_tcp_recover_qoe_state_lock);
}
...
#ifdef CONFIG_HW_HIGH_PRIORITY_LABEL
static void save_acc_state_info(struct fg_custom_msg *msg)
{
	if (msg->len != sizeof(struct fg_custom_msg)) {
		pr_err("[IP_PARA] msg->len: %d, sizeof(struct fg_custom_info): %d, sizeof(struct fg_custom_msg): %d\n",
			msg->len, sizeof(struct fg_custom_info), sizeof(struct fg_custom_msg));
		return;
	}
	struct fg_custom_info *fg_custom_info_tmp = &msg->fg_custom_info_msg;
	g_st_fg_custom_info.uid = fg_custom_info_tmp->uid;
	g_st_fg_custom_info.protocol_bitmap = fg_custom_info_tmp->protocol_bitmap;
	g_st_fg_custom_info.tcp_retran_discard = fg_custom_info_tmp->tcp_retran_discard;
	g_st_fg_custom_info.discard_timer = fg_custom_info_tmp->discard_timer;;
	g_st_fg_custom_info.app_type = fg_custom_info_tmp->app_type;
}
 
static bool detect_sock_state(struct sock *pst_sock)
{
	if ((!sk_fullsock(pst_sock)) || sock_flag(pst_sock, SOCK_DEAD) || (pst_sock->sk_state == TCP_TIME_WAIT))
		return false;
	return true;
}
 
uint8_t fg_encode_discard_timer(unsigned long ul_timer)
{
	uint8_t discard_timer;
 
	if ((ul_timer >= 0) && (ul_timer <= CON_UL_TIMER_THR_1))
		discard_timer = 0x00 | ((ul_timer +
			CON_UL_TIMER_VAL_10 - 1) / CON_UL_TIMER_VAL_10);
	else if ((ul_timer > CON_UL_TIMER_THR_1) && (ul_timer <= CON_UL_TIMER_THR_2))
		discard_timer = 0x40 | (((ul_timer - CON_UL_TIMER_VAL_640) +
			CON_UL_TIMER_VAL_40 - 1) / CON_UL_TIMER_VAL_40);
	else if ((ul_timer > CON_UL_TIMER_THR_2) && (ul_timer <= CON_UL_TIMER_THR_3))
		discard_timer = 0x80 | (((ul_timer - CON_UL_TIMER_VAL_3200) +
			CON_UL_TIMER_VAL_100 - 1) / CON_UL_TIMER_VAL_100);
	else if ((ul_timer > CON_UL_TIMER_THR_3) && (ul_timer <= CON_UL_TIMER_THR_4))
		discard_timer = 0xC0 | (((ul_timer - CON_UL_TIMER_VAL_9600) +
			CON_UL_TIMER_VAL_400 - 1) / CON_UL_TIMER_VAL_400);
	else
		discard_timer = 0;
	return discard_timer;
}
 
static void handle_discard_timer(struct fg_custom_info *past_custom_info,
	...
	struct inet_connection_sock *p_inet_sock)
{
	...
	if ((timeout != 0) && (HZ != 0)) {
		timeout = ((timeout * BST_FG_US_MS) / HZ) +
			(BST_FG_TWENTY_TIME *
			(past_custom_info->tcp_retran_discard & 0x7F));
		*uc_discard_timer = fg_encode_discard_timer(timeout);
		fg_set_discard_timer(pst_sock, *uc_discard_timer);
	}
}
 
void fg_custom_process(struct sock *pst_sock, struct msghdr *msg, uint8_t uc_protocol_bitmap)
{
	uid_t l_sock_uid;
	uint8_t idx;
	uint8_t uc_app_num;
	uint8_t uc_discard_timer = 0;
	struct fg_custom_info *past_custom_info = NULL;
	struct inet_connection_sock *p_inet_sock = NULL;
 
	if (!detect_sock_state(pst_sock))
		return;
	l_sock_uid = sock_i_uid(pst_sock).val;
	fg_set_discard_timer(pst_sock, 0);
#ifdef CONFIG_HUAWEI_SMARTCOM
	fg_init_app_type(pst_sock);
#endif

	if (l_sock_uid <= 0)
		return;
	if (l_sock_uid != g_st_fg_custom_info.uid)
		return;
	past_custom_info = &g_st_fg_custom_info;
	if (!past_custom_info)
		return;
	if (!(uc_protocol_bitmap & past_custom_info->protocol_bitmap))
		return;

	if ((past_custom_info->tcp_retran_discard & 0x80) &&
		(uc_protocol_bitmap == FG_TCP_BITMAP)) {
		p_inet_sock = inet_csk(pst_sock);
		handle_discard_timer(past_custom_info, &uc_discard_timer, pst_sock, p_inet_sock);
	} else {
		if (uc_protocol_bitmap == FG_UDP_BITMAP) {
			uc_discard_timer = past_custom_info->discard_timer;
			fg_set_discard_timer(pst_sock, uc_discard_timer);
		}
	}
#if defined(CONFIG_HW_DPIMARK_MODULE) || defined(CONFIG_HW_HIGH_PRIORITY_LABEL)
	fg_set_app_type(pst_sock, past_custom_info->app_type);
#endif
#if defined(CONFIG_HW_DPIMARK_MODULE) || defined(CONFIG_HW_HIGH_PRIORITY_LABEL)
	pr_debug("[IP_PARA] new DPI_MARK is 0x%x, discard_timer is %d",
		pst_sock->__sk_common.skc_hwdpi_mark, pst_sock->discard_duration);
#endif
}
#endif
...
static void cmd_process(struct req_msg_head *msg, u32 len)
{
	...
	} else if (msg->type == INFER_MODEL_COLLECT_START) {
		model_data_collect_start((struct model_key_flow_req *)msg);
	} else if (msg->type == INFER_MODEL_COLLECT_STOP) {
		model_data_collect_stop((struct model_key_flow_req *)msg);
	} else if (msg->type == LABEL_HIGH_PRIORITY_CMD) {
#ifdef CONFIG_HW_HIGH_PRIORITY_LABEL
		save_acc_state_info((struct fg_custom_msg *) msg);
#endif
	} else if (msg->type == TCP_RECOVERY_BASE_ON_APP_CMD) {
		set_tcp_recover_qoe_state((struct tcp_recover_qoe_msg *)msg);
	} else {
...
{
	pr_info("[IP_PARA] ip_para_collec_init");
	int i;
	int ret;
	...
	if (g_app_ctx == NULL)
		return NULL;
	memset_s(g_app_ctx, sizeof(struct app_ctx), 0, sizeof(struct app_ctx));
	g_app_ctx->fn = fn;
	g_app_ctx->cur = kmalloc(sizeof(struct app_stat) * CONFIG_NR_CPUS,
				 GFP_KERNEL);
	if (g_app_ctx->cur == NULL)
		goto init_error;
	memset_s(g_app_ctx->cur, sizeof(struct app_stat) * CONFIG_NR_CPUS, 0, sizeof(struct app_stat) * CONFIG_NR_CPUS);
	for (i = 0; i < CONFIG_NR_CPUS; i++) {
