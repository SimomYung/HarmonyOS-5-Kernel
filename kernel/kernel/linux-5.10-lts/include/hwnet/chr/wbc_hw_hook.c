...
/* report data to netmanager_enhanced */
static tcp_zero_window_handler *g_tcp_zero_window_handler = NULL;
...
DEFINE_SPINLOCK(g_tcp_zero_window_lock);
...
u32 get_report_top_app(int index)
{
    if (index < 0 || index >= CHR_MAX_REPORT_APP_COUNT) {
        return 0;
    }
    return g_s_report_app_uid_lst[index];
}
...
void web_stat_no_ack_report(u8 interface_type, u8 hash_cnt)
{
	...
	if (time_after(jiffies, g_abn_stamp_web_no_ack) &&
		...
		memset_s(&g_rtn_abn, sizeof(g_rtn_abn), 0, sizeof(g_rtn_abn));
...
void chr_no_ack_report(u32 hash_cnt, u8 if_type)
{
	...
	if (type == HTTP_GET && time_after(jiffies,
		...
		pr_info("chr: web no ack report uid:%d-port:%u\n", g_stream_list[hash_cnt].uid,
			htons(g_stream_list[hash_cnt].tcp_port));
...
#endif
{
	...
	if (time_after(jiffies, g_rpt_stamp + REPORT_TIME)) {
		...
		spin_lock_bh(&g_web_stat_lock);
		memset_s(&g_rtn_stat, sizeof(g_rtn_stat), 0, sizeof(g_rtn_stat));
...
void out_proc(void)
{
	...
	if (g_stream_list[hash_cnt].is_valid == IS_UNUSE) {
		if (g_stream_list[hash_cnt].type == HTTP_GET &&
			g_http_para_out.type == HTTP_GET) {
			g_stream_list[hash_cnt].time_stamp = g_http_para_out.time_stamp;
		}
		if (g_http_para_out.type == HTTP_SYN)
			memcpy_s(&g_stream_list[hash_cnt], sizeof(g_http_para_out), &g_http_para_out,
				sizeof(g_http_para_out));
	} else if (g_stream_list[hash_cnt].type == HTTP_SYN &&
		g_http_para_out.type == HTTP_GET) {
		http_para_out_http_get(hash_cnt);
		g_stream_list[hash_cnt].time_stamp = g_http_para_out.time_stamp;
	} else if (g_stream_list[hash_cnt].type == HTTP_GET && 
		g_http_para_out.type == HTTP_GET &&
...
{
	pr_info("%s:\n", __func__);
	memset_s(&g_rtn_stat_wifi, sizeof(g_rtn_stat_wifi), 0, sizeof(g_rtn_stat_wifi));
	if (delay_chr == NULL)
		return;
	...
	g_rtn_stat_wifi[WLAN_INTERFACE].ohos_uid = delay_chr->ohos_uid;
	chr_notify_event(CHR_WEB_STAT_EVENT, get_user_space_pid(), 0,
		g_rtn_stat_wifi);
}
...
void tcp_window_check(struct sk_buff *skb, struct tcphdr *tcph)
{
	...
	if (tcph->window == 0 && tcph->syn == 0 && tcph->rst == 0 && tcph->ack == 1) {
        ...
        if (g_tcp_zero_window_handler != NULL) {
            g_tcp_zero_window_handler(skb);
        }
...
void tcp_sock_win_report(struct tcphdr *th, struct sock *sk)
{
	...
	memset_s(rtn_stat_sock, sizeof(struct http_return) * RNT_STAT_SIZE, 0, sizeof(struct http_return) * RNT_STAT_SIZE);
	sock = tcp_sk(sk);
...
static bool check_is_http_out(bool is_ipv6_pkt, struct sk_buff *skb, struct ipv6hdr *ip6h,
    struct iphdr *iph, struct tcphdr *tcph, bool *is_http_out)
{
	u32 dlen;
#ifdef CONFIG_HW_BOOSTER
	u8 *vaddr = NULL;
...
static bool is_loopback_addr(bool is_ipv6_pkt, struct ipv6hdr *ip6h,
	struct iphdr *iph)
{
    if ((is_ipv6_pkt && ip6h == NULL) || (!is_ipv6_pkt && iph == NULL)) {
        return false;
    ...
    if (is_ipv6_pkt) {
        return ipv6_address_equal(ip6h->daddr, ip6h->saddr);
	} else {
        return iph->daddr == iph->saddr;
	}
}
...
static unsigned int hook_local_out(void *ops, struct sk_buff *skb, const struct nf_hook_state *state)
{
	...
	if (get_uid_from_sock(skb->sk) != g_app_top_uid || is_loopback_addr(is_ipv6_pkt, ip6h, iph)) {
		spin_unlock_bh(&g_web_para_out_lock);
		return NF_ACCEPT;
	}
...
void variable_initialization(void)
{
	...
	memset_s(&g_rtn_stat_wifi, sizeof(g_rtn_stat_wifi), 0, sizeof(g_rtn_stat_wifi));
	memset_s(&g_rtn_abn, sizeof(g_rtn_abn), 0, sizeof(g_rtn_abn));
	memset_s(&g_http_para_in, sizeof(g_http_para_in), 0, sizeof(g_http_para_in));
	memset_s(&g_http_para_out, sizeof(g_http_para_out), 0, sizeof(g_http_para_out));
	memset_s(&g_stack_rtt, sizeof(g_stack_rtt), 0, sizeof(g_stack_rtt));
}
...
{
	memset_s(g_abn_stamp_list_syn_no_ack, sizeof(unsigned long) * SYN_NO_ACK_MAX, 0,
		sizeof(unsigned long) * SYN_NO_ACK_MAX);
	memset_s(g_abn_stamp_list_web_no_ack, sizeof(unsigned long) * WEB_NO_ACK_MAX, 0,
		sizeof(unsigned long) * WEB_NO_ACK_MAX);
	memset_s(g_abn_stamp_list_web_fail, sizeof(unsigned long) * WEB_FAIL_MAX, 0,
		sizeof(unsigned long) * WEB_FAIL_MAX);
	memset_s(g_abn_stamp_list_web_delay, sizeof(unsigned long) * WEB_DELAY_MAX, 0,
		sizeof(unsigned long) * WEB_DELAY_MAX);
	memset_s(g_abn_stamp_list_tcp_rtt_large, sizeof(unsigned long) * TCP_RTT_LARGE_MAX, 0,
...
int web_chr_init(void)
{
	...
	if (g_stream_list == NULL)
		return -1;
	memset_s(g_stream_list,  sizeof(struct http_stream) * HASH_MAX, 0, sizeof(struct http_stream) * HASH_MAX);
...
void upload_tcp_recover_info(struct tcp_recover_info *tcp_info)
{
	...
	if (g_tcp_recover.tcp_recover.app_uid != 0) {
		chr_notify_event(TCP_RECOVER_HIVIEW_ID, get_user_space_pid(), 0, &g_tcp_recover);
		pr_info("upload_tcp_recover_info");
	}
}
#endif

int get_data_reg_tech(void)
{
	return g_data_reg_tech;
}
EXPORT_SYMBOL(get_data_reg_tech);

struct http_stream * get_stream_list(u8 hash_cnt)
{
	return &g_stream_list[hash_cnt];
}
EXPORT_SYMBOL(get_stream_list);

void set_stream_list(u8 hash_cnt)
{
	g_stream_list[hash_cnt].is_valid = IS_UNUSE;
}
EXPORT_SYMBOL(set_stream_list);

void set_tcp_for_dpa(void)
{
	g_http_para_out.tcp_for_dpa = 1;
}
EXPORT_SYMBOL(set_tcp_for_dpa);

void set_tcp_zero_window_handler(tcp_zero_window_handler *handler)
{
	spin_lock_bh(&g_tcp_zero_window_lock);
	g_tcp_zero_window_handler = handler;
	spin_unlock_bh(&g_tcp_zero_window_lock);
}
