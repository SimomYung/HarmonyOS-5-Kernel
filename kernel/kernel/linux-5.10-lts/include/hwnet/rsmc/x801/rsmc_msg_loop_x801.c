...
const static u16 cmd_module_map[][MAP_ENTITY_NUM] = {
	{CMD_DN_INIT_REQ, RSMC_DEVICE, sizeof(struct enable_msg)},
	{CMD_DN_ANY_REQ, RSMC_DEVICE, sizeof(struct any_msg)},
	{CMD_DN_DTX_REQ, RSMC_DEVICE, sizeof(struct dtx_msg)},
	{CMD_INTER_INIT_REQ, RSMC_DEVICE, sizeof(struct enable_msg)},
	{CMD_INTER_HB_TIMER_REQ, RSMC_DEVICE, 0},
	{CMD_INTER_START_SPI_TIMER, RSMC_DEVICE, 0},
	{CMD_INTER_STOP_SPI_TIMER, RSMC_DEVICE, 0},
	{CMD_INTER_WAIT_STA_TIMEOUT, RSMC_DEVICE, 0},
};

void spin_lock_dn_inter_lock(unsigned long flags)
{
	spin_lock_irqsave(&g_nl_ctx.dn_inter_lock, flags);
}

void spin_unlock_dn_inter_lock(unsigned long flags)
{
	spin_unlock_irqrestore(&g_nl_ctx.dn_inter_lock, flags);
}

void spin_lock_dn_spi_lock(unsigned long flags)
{
	spin_lock_irqsave(&g_nl_ctx.dn_spi_lock, flags);
}

void spin_unlock_dn_spi_lock(unsigned long flags)
{
...
void rsmc_nl_dn_msg_dipatch(struct msg_head *msg, u16 skb_buff_len)
{
	if (msg == NULL) {
		hwlog_err("%s: nl msg null", __func__);
		return;
	}
	if ((msg->len + sizeof(struct msg_head) + SKB_HEAD_LEN > skb_buff_len) || (msg->len > NETLINK_BUFFER_MAX_SIZE)) {
		hwlog_err("%s: invalid msg len", __func__);
		return;
	}
	if ((msg->type == CMD_DN_ANY_REQ) || (msg->type == CMD_DN_DTX_REQ))
		rsmc_nl_dn_msg_notify_spi(msg);
	else if (msg->type == CMD_DN_INIT_REQ)
		rsmc_nl_dn_msg_notify_inter(msg);
	else
		hwlog_err("%s: invalid msg type:%d", __func__, msg->type);
}
...
void rsmc_process_cmd_x801(struct msg_head *cmd)
{
	...
	for (i = 0; i < sizeof(cmd_module_map) / (sizeof(u16) * MAP_ENTITY_NUM); i++) {
		...
		if (g_nl_ctx.mod_cb[cmd_module_map[i][MAP_VALUE_INDEX]] == NULL)
			break;
		if (cmd->len < cmd_module_map[i][MAP_MSG_LEN_INDEX]) {
			hwlog_err("%s,invalid cmd len:%d", __func__, cmd->len);
			break;
		}
...
void rsmc_netlink_recv(struct sk_buff *__skb)
{
	...
	case NL_MSG_HEARTBEAT:
		...
		break;
	case NL_MSG_REQ:
		rsmc_nl_dn_msg_dipatch((struct msg_head *)NLMSG_DATA(nlh), skb->len);
		break;
	default:
		break;
	}
...
static struct msg_entity *rsmc_nl_up_get_msg(void)
{
	...
	spin_lock_irqsave(&g_nl_ctx.up_lock, flags);
	if (g_nl_ctx.up_end == g_nl_ctx.up_start) {
		spin_unlock_irqrestore(&g_nl_ctx.up_lock, flags);
		return NULL;
	}
