...
static int g_sysfs_prot_type_table[SYSFS_PROTOCOL_END] = {
	...
	[SYSFS_PROTOCOL_PPS] = ADAPTER_PROTOCOL_PPS,
};
...
static int adapter_detect_ping_pps_type(int *adp_mode)
{
	int prot_type = ADAPTER_PROTOCOL_PPS;

	return adapter_detect_adapter_support_mode(prot_type, adp_mode);
}

static int adapter_detect_ping_ufcs_type(int *adp_mode)
{
	int ret;
...
static int adapter_detect_ping_scp_type(int *adp_mode)
{
	int ret;
	int i;
...
static struct adapter_detect_priority_data adapter_detect_priority_data[] = {
	...
	{ ADAPTER_PROTOCOL_SCP, "scp", adapter_detect_ping_scp_type, false },
	{ ADAPTER_PROTOCOL_PPS, "pps", adapter_detect_ping_pps_type, false },
};
