...
static bool bat_chg_balance_check_cur_limit(struct bat_chg_bal_device *di)
{
	int i;
	int cur = 0;

	if (!di->is_support_limit_cur)
		return false;

	for (i = 0; i < di->check_batt_num; i++) {
		power_supply_get_int_prop(g_battery_psy[i],
			POWER_SUPPLY_PROP_CURRENT_NOW, &cur, 0, 1);
		if (cur == 0) {
			if (di->zero_count < BAT_CUR_ERR_COUNT)
				di->zero_count++;
			break;
		}
	}
	if (i >= di->check_batt_num)
		di->zero_count = 0;
	hwlog_info("%s zero_count=%d\n", __func__, di->zero_count);
	return di->zero_count >= BAT_CUR_ERR_COUNT;
}
	...
	u32 info_len, struct bat_chg_balance_cur *result, int mode)
{
	...
	if (bat_chg_balance_check_cur_limit(di)) {
		result->total_cur = di->limit_current;
		return 0;
	}
...
static int bat_chg_bal_parse_dts(struct bat_chg_bal_device *di)
{
	...
	(void)power_dts_read_u32(power_dts_tag(HWLOG_TAG), np,
		"is_support_limit_cur", &di->is_support_limit_cur, 0);
	(void)power_dts_read_u32(power_dts_tag(HWLOG_TAG), np,
		"check_batt_num", &di->check_batt_num, BAT_BALANCE_COUNT);
	(void)power_dts_read_u32(power_dts_tag(HWLOG_TAG), np,
		"limit_current", &di->limit_current, 0);
	...
	power_dts_read_u32(power_dts_tag(HWLOG_TAG), np,
		"bal_detect_cur_th", &di->bal_detect_cur_th, BAL_DETECT_CUR_TH);
