...
#define UPDATE_SUBSTITUTED_TEMP_DAY      (24 * 60 * 60 * 1000) /* 1 day */
#define UPDATE_BATT_TEMP_OUTRANG_TIME    (10 * 60 * 1000) /* 10 min */

#define MIN_LOG_SHOW_LEN                 2

#define INVALID_BAT_INDEX                (-1)
#define MULTI_BTB_DIFF_INFO_LEN          5
#define TARGET_BAT_TEMP_COMP_DFLT        5000 /* 5 degree */
...
#define SEND_SUBSTITUTED_MAX_NUM         30
...
enum temp_abnormal_check {
	RAW_SENSOR_NAME = 0,
	SUBSTITUTED_SENSOR_NAME,
	RAW_ABNORMAL_TEMP_TH,
	SUBSTITUTED_LOW_TEMP_TH,
	SUBSTITUTED_HIGH_TEMP_TH,
	TEMP_ABNORMAL_CHECK_TOTAL,
};

struct multi_btb_temp_check_para {
	int target_index;
	int refer_index;
	int btb_temp_th;
	int low_temp_diff;
...
struct temp_abnormal_check_para {
	char raw_sensor_name[THERMAL_NAME_LENGTH];
	char substituted_sensor_name[THERMAL_NAME_LENGTH];
	int raw_abnormal_temp_th;
	int substituted_low_temp_th;
	int substituted_high_temp_th;
};
...
struct multi_btb_temp_info {
	...
	struct delayed_work multi_btb_dmd_delay_work;
	struct notifier_block connect_nb;
	struct notifier_block charge_channel_nb;
	struct sensor_para sensor_tab[SENSOR_MAX_NUM];
	struct temp_abnormal_check_para check_para[SENSOR_MAX_NUM];
	struct multi_btb_temp_check_para diff_check_para;
...
static int g_substituted_temp_num = 0;
...
static int multi_btb_temp_valid_check(struct multi_btb_temp_info *di, int temp_with_comp)
{
	int i = di->diff_check_para.target_index;
	int j = di->diff_check_para.refer_index;

	/* skip check when temp not init */
	if (!di->temp_already_init)
		return temp_with_comp;

	if ((i < 0) || (i >= di->sensor_num) || (j < 0) || (j >= di->sensor_num))
		return temp_with_comp;

	if (di->sensor_tab[i].temp < di->diff_check_para.btb_temp_th)
		return di->sensor_tab[j].temp;
 
	if (di->sensor_tab[j].temp - di->sensor_tab[i].temp > di->diff_check_para.low_temp_diff)
		return di->sensor_tab[j].temp;

	if (di->sensor_tab[i].temp - di->sensor_tab[j].temp > di->diff_check_para.high_temp_diff)
		return di->sensor_tab[j].temp + TARGET_BAT_TEMP_COMP_DFLT;
...
static int multi_btb_get_temp_with_comp(int index, int temp_without_comp)
{
	...
	if (btf_temp_fitted_eqr()) {
		hwlog_info("bat temp fitting, need not comp\n");
		return temp_without_comp;
	}
#endif /* CONFIG_HUAWEI_BATTERY_TEMP_FITTING */
	...
	if (di->diff_check_para.target_index == index)
		temp_with_comp = multi_btb_temp_valid_check(di, temp_with_comp);
...
static int multi_btb_get_check_para_num(const char *sensor_name, struct multi_btb_temp_info *di)
{
	int i;

	for (i = 0; i < di->sensor_check_num; i++) {
		if (strlen(sensor_name) != strlen(di->check_para[i].raw_sensor_name))
			continue;
		if (!strncmp(sensor_name, di->check_para[i].raw_sensor_name, strlen(sensor_name)))
			return i;
	}
...
/*
 * fix lem large battery temp abnorml issue.
 * if the battery temp ntc broken, the large battery temp is abnormal,
 * need select the other ntc to substituted.
 */
static int multi_btb_get_raw_temp(const char *sensor_name)
{
	int num, raw_temp, substituted_temp;
	struct multi_btb_temp_info *di = g_info;

	raw_temp = power_temp_get_average_value(sensor_name);
	if (!di || !di->sensor_check_num)
		return raw_temp;

	num = multi_btb_get_check_para_num(sensor_name, di);
	if (num < 0)
		return raw_temp;

	if (raw_temp > di->check_para[num].raw_abnormal_temp_th)
		return raw_temp;

	substituted_temp = power_temp_get_average_value(di->check_para[num].substituted_sensor_name);
	if ((substituted_temp >= di->check_para[num].substituted_low_temp_th) &&
		(substituted_temp <= di->check_para[num].substituted_high_temp_th)) {
		g_substituted_temp_num++;
		return substituted_temp;
	}
	return raw_temp;
}
...
static int multi_btb_get_temp_sync(const int index, const char *sensor_name)
{
	...
	raw = multi_btb_get_raw_temp(sensor_name);
...
static void multi_btb_update_temp(struct multi_btb_temp_info *di)
{
	...
	for (i = 0; i < di->sensor_num; i++) {
		raw = multi_btb_get_raw_temp(di->sensor_tab[i].name);
...
static void multi_btb_dmd_delay_work(struct work_struct *work)
{
	struct multi_btb_temp_info *di = g_info;
	...
	if (g_substituted_temp_num >= SEND_SUBSTITUTED_MAX_NUM) {
		ret = snprintf_s(dsm_buf, POWER_DSM_BUF_SIZE_0128, POWER_DSM_BUF_SIZE_0128 - 1,
			"batt temp diff,num=%d",g_substituted_temp_num);
		if (ret < 0 || ret >= POWER_DSM_BUF_SIZE_0128 - 1) {
			hwlog_info("snprintf_s return error,ret=%d\n",ret);
			schedule_delayed_work(&di->multi_btb_dmd_delay_work,
				msecs_to_jiffies(UPDATE_BATT_TEMP_OUTRANG_TIME));
			return;
		}
		hwlog_info("batt_temp out,report dmd message\n");
		power_dsm_report_dmd(POWER_DSM_BATTERY, POWER_DSM_BATTERY_TEMP_OUT_RANGE, dsm_buf);
		g_substituted_temp_num = 0;
	}
	schedule_delayed_work(&di->multi_btb_dmd_delay_work,
		msecs_to_jiffies(UPDATE_SUBSTITUTED_TEMP_DAY));
}
...
static void multi_btb_temp_print_check_para(struct multi_btb_temp_info *di)
{
	int i;

	for (i = 0; i < di->sensor_check_num; i++)
		hwlog_info("abnormal_check_para[%d] %s %s %d %d %d\n", i,
			di->check_para[i].raw_sensor_name,
			di->check_para[i].substituted_sensor_name,
			di->check_para[i].raw_abnormal_temp_th,
			di->check_para[i].substituted_low_temp_th,
			di->check_para[i].substituted_high_temp_th);
}

static void multi_btb_temp_parse_abnormal_check_para(struct device_node *np,
	struct multi_btb_temp_info *di)
{
	int array_len, i, row, col;
	int value = 0;
	...
	array_len = power_dts_read_count_strings(power_dts_tag(HWLOG_TAG), np,
		"temp_abnormal_check", SENSOR_MAX_NUM, TEMP_ABNORMAL_CHECK_TOTAL);
	if (array_len <= 0)
		goto err_out;

	di->sensor_check_num = array_len / TEMP_ABNORMAL_CHECK_TOTAL;
	for (i = 0; i < array_len; i++) {
		if (power_dts_read_string_index(power_dts_tag(HWLOG_TAG),
			np, "temp_abnormal_check", i, &str))
			goto err_out;

		row = i / TEMP_ABNORMAL_CHECK_TOTAL;
		col = i % TEMP_ABNORMAL_CHECK_TOTAL;
		switch (col) {
		case RAW_SENSOR_NAME:
			if (snprintf_s(di->check_para[row].raw_sensor_name,
				THERMAL_NAME_LENGTH, THERMAL_NAME_LENGTH - 1, "%s", str) < 0)
				goto err_out;
			break;
		case SUBSTITUTED_SENSOR_NAME:
			if (snprintf_s(di->check_para[row].substituted_sensor_name,
				THERMAL_NAME_LENGTH, THERMAL_NAME_LENGTH - 1, "%s", str) < 0)
				goto err_out;
			break;
		case RAW_ABNORMAL_TEMP_TH:
			if (kstrtoint(str, POWER_BASE_DEC, &value))
				goto err_out;
			di->check_para[row].raw_abnormal_temp_th = value;
			break;
		case SUBSTITUTED_LOW_TEMP_TH:
			if (kstrtoint(str, POWER_BASE_DEC, &value))
				goto err_out;
			di->check_para[row].substituted_low_temp_th = value;
			break;
		case SUBSTITUTED_HIGH_TEMP_TH:
			if (kstrtoint(str, POWER_BASE_DEC, &value))
				goto err_out;
			di->check_para[row].substituted_high_temp_th = value;
			break;
		default:
			break;
		}
	...
	multi_btb_temp_print_check_para(di);
	return;

err_out:
	di->sensor_check_num = 0;
}
...
static void multi_btb_temp_parse_diff_para(struct device_node *np,
	struct multi_btb_temp_info *di)
{
	if (power_dts_read_u32_array(power_dts_tag(HWLOG_TAG), np, "diff_check_para",
		(u32 *)&di->diff_check_para, MULTI_BTB_DIFF_INFO_LEN)) {
		di->diff_check_para.target_index = INVALID_BAT_INDEX;
		di->diff_check_para.refer_index = INVALID_BAT_INDEX;
		di->diff_check_para.btb_temp_th = INVALID_TEMP_LOW;
		di->diff_check_para.low_temp_diff = INVALID_TEMP_LOW;
		di->diff_check_para.high_temp_diff = INVALID_TEMP_HIGH;
	}

	hwlog_info("diff_check_para %d %d %d %d %d\n", di->diff_check_para.target_index,
		di->diff_check_para.refer_index, di->diff_check_para.btb_temp_th,
		di->diff_check_para.low_temp_diff, di->diff_check_para.high_temp_diff);
}
	...
	struct multi_btb_temp_info *di)
{
	...
	multi_btb_temp_parse_abnormal_check_para(np, di);
	multi_btb_temp_parse_group_comp_para(np, di);
	multi_btb_temp_parse_diff_para(np, di);
	return 0;
}
...
static int multi_btb_temp_probe(struct platform_device *pdev)
{
	...
	INIT_DELAYED_WORK(&di->multi_btb_dmd_delay_work, multi_btb_dmd_delay_work);
	schedule_delayed_work(&di->multi_btb_dmd_delay_work, msecs_to_jiffies(UPDATE_BATT_TEMP_OUTRANG_TIME));
	return 0;
