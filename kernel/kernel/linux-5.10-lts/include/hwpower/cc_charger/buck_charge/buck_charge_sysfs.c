...
static struct power_sysfs_attr_info charge_sysfs_field_tbl[] = {
	...
	charge_sysfs_field_rw(vsys_mos_enable,          VSYS_MOS_ENABLE),
	charge_sysfs_field_rw(curr_balance_mos_enable,  CURR_BALANCE_MOS_ENABLE),
...
{
	if (di->vterm < 0 || (unsigned int)di->vterm < di->vterm_rt)
...
static int charge_sysfs_vsys_mos_enable_show(struct charge_sysfs_dev *di, char *buf, size_t offset)
{
	int ret;
	bool enable = false;

	ret = buck_charge_get_vsys_mos_enable(&enable);
	if (ret)
		return scnprintf(buf, PAGE_SIZE, "get vsys_mos_enable fail\n");

	return scnprintf(buf, PAGE_SIZE, "%u\n", enable);
}

static int charge_sysfs_curr_balance_mos_enable_show(struct charge_sysfs_dev *di, char *buf, size_t offset)
{
	int ret;
	bool enable = false;

	ret = buck_charge_get_curr_balance_mos_enable(&enable);
	if (ret)
		return scnprintf(buf, PAGE_SIZE, "get curr_balance_mos_enable fail\n");
...
static int charge_sysfs_vsys_mos_enable_store(struct charge_sysfs_dev *di, const char *buf)
{
	int ret;
	unsigned int val = 0;
	bool enable = false;

	if ((kstrtouint(buf, POWER_BASE_DEC, &val) < 0) || (val > 1)) {
		hwlog_err("vsys_mos_enable:invaild para val=%u\n", val);
		return -EINVAL;
	}

	enable = (val != 0);
	ret = buck_charge_set_vsys_mos_enable(enable);
	if (ret) {
		hwlog_err("buck_charge_set_vsys_mos_enable fali, val=%u, ret=%d\n", val, ret);
		return ret;
	}
...
static int charge_sysfs_curr_balance_mos_enable_store(struct charge_sysfs_dev *di, const char *buf)
{
	int ret;
	unsigned int val = 0;
	bool enable = false;

	if ((kstrtouint(buf, POWER_BASE_DEC, &val) < 0) || (val < 0) || (val > 1)) {
		hwlog_err("curr_balance_mos_enable:invaild para val=%u\n", val);
		return -EINVAL;
	}

	enable = (val != 0);
	ret = buck_charge_set_curr_balance_mos_enable(enable);
	if (ret) {
		hwlog_err("buck_charge_set_curr_balance_mos_enable fail, val=%u, ret=%d\n", val, ret);
		return ret;
	}
...
static struct charge_sysfs_show_struct g_charge_sysfs_show_tbl[] = {
	...
	[CHARGE_SYSFS_VSYS_MOS_ENABLE] = { charge_sysfs_vsys_mos_enable_show, 0 },
	[CHARGE_SYSFS_CURR_BALANCE_MOS_ENABLE] = { charge_sysfs_curr_balance_mos_enable_show, 0 },
};
...
static struct charge_sysfs_store_struct g_charge_sysfs_store_tbl[] = {
	...
	[CHARGE_SYSFS_VSYS_MOS_ENABLE] = { charge_sysfs_vsys_mos_enable_store },
	[CHARGE_SYSFS_CURR_BALANCE_MOS_ENABLE] = { charge_sysfs_curr_balance_mos_enable_store },
};
...
static void charge_sysfs_params_init(struct charge_sysfs_dev *di)
{
	...
	if (snprintf_s(di->thermal_reason, CHARGE_SYSFS_THERMAL_REASON_SIZE,
		CHARGE_SYSFS_THERMAL_REASON_SIZE - 1, "0#0#0#0") < 0)
		hwlog_err("thermal_reason init failed\n");
}
