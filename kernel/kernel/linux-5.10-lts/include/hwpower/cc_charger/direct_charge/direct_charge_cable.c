...
void dc_update_ui_cable_type(void)
{
	...
	if (!para->is_ctc_cable) {
		...
		/* use cola or fenda adapter, ui cable type change to std */
		if ((para->cable_type == DC_NONSTD_CABLE) && dc_is_cola_or_fenda_adpter()) {
			hwlog_info("use cola or fenda adapter, ui cable type change to std\n");
	...
	if (!power_platform_support_cable_auth()) {
		para->ui_cable_type = DC_UI_STD_CABLE;
...
void dc_update_cable_max_current(int *cable_max_cur, bool cable_unlimit_processing)
{
	int max_cur;
	struct dc_cable_inherent_para *para = dc_get_cable_inherent_para();
	...
	/* use cola or fenda adapter, no current processing restrictions */
	if (cable_unlimit_processing)
		*cable_max_cur = 0;
}
...
static int dc_get_path_resistance_info(struct dc_resist_data *data, int working_mode)
{
	...
	if (direct_charge_get_device_vbus(&(data->vbus), false))
		return -EPERM;
...
int dc_resist_handler(int mode, int value)
{
	...
	adpt_intfr_check_cable(value, l_di->dts_info.intfr_resist);
	return ret;
}
...
void dc_send_cable_type_uevent(int mode)
{
	...
	if (!l_di || l_di->dts_info.is_send_cable_type == 0)
		return;
	...
	struct dc_dts_cable_info *info)
{
	...
	(void)power_dts_read_u32(power_dts_tag(HWLOG_TAG), np,
		"intfr_resist", (u32 *)&info->intfr_resist, 0);
