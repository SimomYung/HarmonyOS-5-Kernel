...
static enum fsm_event dc_get_charging_event(void)
{
	...
	if (charge_get_charger_type() == CHARGER_REMOVED) {
		hwlog_info("charger plug out, exit dc\n");
		return EVT_NOT_CHARGING;
	}
...
static void dc_exceed_err_cnt_handler(struct direct_charge_device *di)
{
	...
	if ((di->dc_err_report_flag == FALSE) &&
		...
		if (snprintf_s(di->dsm_buff + strlen(di->dsm_buff),
			sizeof(di->dsm_buff) - strlen(di->dsm_buff),
			sizeof(di->dsm_buff) - strlen(di->dsm_buff) - 1,
			"mode=%d path=%d vbat=%d ibat=%d ls_vbus=%d\n", di->working_mode,
			di->cur_mode, di->vbat, di->ibat, di->ls_vbus) <= 0)
			return;
...
static int st_anomaly_chk_entry(void *p)
{
	...
	support_path = multi_batchk_get_support_path(di->working_mode, 0);
	if (multi_ic_check_select_init_mode(&di->multi_ic_mode_para, &di->cur_mode, support_path)) {
		dc_mmi_set_succ_flag(di->working_mode, DC_ERROR_BTB_CHECK);
		di->error_cnt += 1;
		hwlog_err("after multi_ic_check error_cnt = %d\n", di->error_cnt);
		return -EPERM;
	}
...
static int st_evt_pre_rept_entry(void *p)
{
	...
	if (adapter_detect_get_runtime_protocol_type() != BIT(ADAPTER_PROTOCOL_SCP))
		return 0;
	
	max_pwr = dc_pmode_get_optimal_mode_max_pwr(QUICK_CHECK);
	if (dc_get_cable_inherent_info(DC_CABLE_DETECT_OK) == CABLE_DETECT_OK)
...
static int st_prot_chk_entry(void *p)
{
	...
	if (dc_is_scp_superior(di->dc_volt_ratio)) {
...
static void st_evt_rept_uevent(struct direct_charge_device *di)
{
	if (dc_comm_get_uevent_send_flag()) {
		hwlog_info("st_evt_rept_uevent already report\n");
		return;
	}
	...
	dc_send_adapter_info_uevent();
	dc_comm_set_uevent_send_flag(true);
}
...
static int st_antifake_rechk_entry(void *p)
{
	...
	dc_recheck_adapter_antifake(di->local_mode);
...
static void direct_charge_check_exit_ufcs_mode(void)
{
...
static void direct_charge_report_adp_code(void)
{
	int len;
	...
	adp_code = dc_get_adapter_code();
	adpt_intfr_check_adpt_code(adp_code);
	len = snprintf_s(buf, POWER_EVENT_NOTIFY_SIZE, POWER_EVENT_NOTIFY_SIZE - 1,
		"POWER_ADP_CODE=%s:%d", dc_get_protocol_str(), adp_code);
	if (len <= 0)
		return;
...
void direct_charge_check(void)
{
	...
	if (ret == MODE_NEED_RECHK)
		goto retry;
exit:
	direct_charge_check_exit_ufcs_mode();
	return;
retry:
	/* need stay in ufcs mode when test mode */
...
int direct_charge_pre_check(void)
{
	...
	if (!dc_adpt_detect_ping()) {
		...
		if (!power_platform_support_new_pd_process() &&
			(adapter_detect_get_runtime_protocol_type() == BIT(ADAPTER_PROTOCOL_UFCS))) {
			dc_set_adapter_default();
