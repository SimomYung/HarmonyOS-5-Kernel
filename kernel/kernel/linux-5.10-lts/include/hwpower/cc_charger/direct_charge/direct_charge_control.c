...
static void dc_cccv_regulation_by_ibat(struct direct_charge_device *di, int ibat)
{
	...
	int max_adap_cur;
	int vstep = direct_charge_get_vstep(di->local_mode);
	int delta_err = dc_adpt_update_delta_err(di->local_mode,
		di->cur_ibat_th_high / di->dc_volt_ratio, di->dc_volt_ratio, di->delta_err);
...
static void dc_update_regulation_data(struct direct_charge_device *di, int ibat)
{
	...
	direct_charge_get_device_vbus(&di->ls_vbus, false);
	direct_charge_get_device_temp(&di->tls);
	dc_adpt_update_power_bank_cap();
...
static int dc_check_device_and_adapter_temp(struct direct_charge_device *di)
{
	...
	if (di->tadapt > max_tadap) {
		di->adp_otp_cnt++;
		hwlog_info("adapter over temp, cnt=%d\n", di->adp_otp_cnt);
	} else {
		di->adp_otp_cnt = 0;
	}

	if (di->adp_otp_cnt >= ADP_OTP_CNT) {
		snprintf(tmp_buf, sizeof(tmp_buf), "tadp=%d > %d, stop direct charge\n",
			di->tadapt, max_tadap);
		dc_fill_eh_buf(di->dsm_buff, sizeof(di->dsm_buff),
			DC_EH_TADP_ABNORMAL, tmp_buf);
...
static void dc_ctrl_set_start_time(void)
{
	...
	if (g_dc_ctrl_base_data.dc_first_start_time == 0)
		g_dc_ctrl_base_data.dc_first_start_time = cur_time;
}

void dc_ctrl_clear_first_start_time(void)
{
	g_dc_ctrl_base_data.dc_first_start_time = 0;
}
...
time64_t dc_ctrl_get_first_start_time(void)
{
	return g_dc_ctrl_base_data.dc_first_start_time;
}
