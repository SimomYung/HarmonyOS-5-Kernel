...
static int dc_check_multi_ic_current(struct direct_charge_device *di)
{
	...
	ibus_th = (di->working_mode == LVC_MODE) ?
		LVC_MIN_CURRENT_FOR_MULTI_IC : MIN_CURRENT_FOR_MULTI_IC;
	while (count) {
		...
		if ((main_ibus > ibus_th) && (aux_ibus > ibus_th))
			return 0;
...
void dc_check_recharge_path(void)
{
	...
	/* judge soc level */
	rechg_en = rechg_en || dc_rechg_check_soc();
