...
#define DC_PMODE_VBAT_HYSTERESIS                      100
...
void dc_pmode_disable_mode(unsigned int reason, unsigned int mode)
{
	...
	if (reason & (DC_DIS_BY_IBAT_TH_LOW | DC_DIS_BY_SYSFS_DISABLE |
		DC_DIS_BY_PRIORITY | DC_DIS_BY_THERMAL))
		tbl[idx].dis_time_begin = power_get_current_kernel_time().tv_sec;
	hwlog_info("mode=0x%x disable_reason=0x%x dis_flag=0x%x\n", mode, reason, tbl[idx].dis_flag);
}
...
void dc_pmode_update_mode_power(void)
{
	...
	bool cable_no_limit = false;
	struct pmode_table *tbl = dc_pmode_get_table();

	dc_update_cable_type();
	if (!dc_get_cable_inherent_info(DC_IS_CTC_CABLE) &&
		(dc_get_cable_inherent_info(DC_CABLE_TYPE) == DC_NONSTD_CABLE) &&
		dc_is_cola_or_fenda_adpter())
		cable_no_limit = true;
	for (i = 0; i < DC_MODE_TOTAL; i++) {
		...
		dc_update_cable_max_current(&tbl[i].max_cur_cable, cable_no_limit);
...
int dc_pmode_get_max_pwr_adp(void)
{
	int i;
	int max_pwr = 0;
	struct pmode_table *tbl = dc_pmode_get_table();
	...
	for (i = 0; i < DC_MODE_TOTAL; i++) {
		if ((tbl[i].max_cur_adp * tbl[i].base_volt / POWER_UW_PER_MW) > max_pwr)
			max_pwr = tbl[i].max_cur_adp * tbl[i].base_volt / POWER_UW_PER_MW;
	}

	hwlog_info("get max pwr adp=%d\n", max_pwr);
	return max_pwr;
}
...
static bool dc_pmode_check_thermal_optimal_mode(int idx, int cur_mode)
{
	int i, voltage;
	int target_mode;
	struct pmode_table *tbl = dc_pmode_get_table();
	struct direct_charge_device *l_di = direct_charge_get_di_by_mode(tbl[idx].local_mode);

	if (power_cmdline_is_factory_mode() || !l_di || !l_di->thermal_switch_en)
		return false;

	if (dc_get_cable_inherent_info(DC_CABLE_TYPE) != DC_STD_CABLE) {
		hwlog_info("not standard cable, disable thermal mode switch\n");
		return false;
	}

	target_mode = dc_sysfs_get_optimal_mode();
	if (tbl[idx].local_mode != target_mode)
		return false;

	if (target_mode == cur_mode) {
		hwlog_info("cur mode is target mode, keep going\n");
		return true;
	}

	voltage = dcm_get_ic_vbtb(tbl[idx].local_mode, CHARGE_IC_MAIN);
	if ((voltage <= l_di->min_dc_bat_vol + DC_PMODE_VBAT_HYSTERESIS) ||
		(voltage >= l_di->max_dc_bat_vol)) {
		hwlog_err("%s voltage=%d, can not enter target mode\n", __func__, voltage);
		return false;
	}
	...
	for (i = 0; i < DC_MODE_TOTAL; i++) {
		if (idx == i)
			continue;
		if (tbl[i].dis_flag & DC_DIS_BY_THERMAL)
			continue;
		dc_pmode_disable_mode(DC_DIS_BY_THERMAL, tbl[i].local_mode);
	}

	hwlog_info("%s can enter target_mode=%d\n", __func__, target_mode);
	return true;
}

static void dc_pmode_clear_thermal_dis_flag(struct pmode_table *tbl)
{
	int i;

	for (i = 0; i < DC_MODE_TOTAL; i++)
		dc_pmode_enable_mode(DC_DIS_BY_THERMAL, tbl[i].local_mode);
}
...
static bool dc_pmode_dis_flag_check_failed(struct pmode_table *tbl)
{
	...
	if (tbl->dis_flag & (DC_DIS_BY_IBAT_TH_LOW | DC_DIS_BY_SYSFS_DISABLE |
		...
		time_th = (tbl->dis_flag & DC_DIS_BY_SYSFS_DISABLE) ?
			DC_PMODE_SWITCH_SYSFS_HYSTERESIS_SEC : DC_PMODE_SWITCH_HYSTERESIS_SEC;
...
static unsigned int dc_pmode_get_normal_optimal_mode(int mode, bool cur_flag)
{
	...
	for (i = 0; i < DC_MODE_TOTAL; i++) {
		...
		if (dc_pmode_check_thermal_optimal_mode(i, mode)) {
			idx = i;
			break;
		}
	...
	if (idx < 0) {
		...
		dc_pmode_clear_thermal_dis_flag(tbl);
		return UNDEFINED_MODE;
	}
