...
bool dc_rechg_check_soc()
{
	...
	return batt_soc <= g_rechg_di->dts.rechg_soc_th;
}
...
void dc_rechg_set_enable(int enable, int target_ibat)
{
...
void dc_rechg_regulation(int lim_ibus, int target_ibat)
{
	...
	if (!g_rechg_di->info.enable || g_rechg_di->info.target_cur <= 0) {
		dc_rechg_set_enable(DC_RECHARGE_DISABLE, DC_RECHARGE_TARGET_IBAT_DFT);
		dc_rechg_set_enable(DC_RECHARGE_ENABLE, target_ibat);
	}
...
void dc_rechg_enforce_en()
{
	...
	dc_rechg_set_enable(DC_RECHARGE_ENABLE, g_rechg_di->dts.enforce_ibat);
...
static struct power_sysfs_attr_info dc_rechg_sysfs_field_tbl[] = {
	...
	power_sysfs_attr_rw(dc_rechg, 0644,
		DC_RECHG_SYSFS_SOC_TH, rechg_soc_th),
};
...
static void dc_rechg_parse_dts(struct device_node *np, struct dc_rechg_dev *di)
{
	...
	(void)power_dts_read_u32(power_dts_tag(HWLOG_TAG), np,
		"rechg_soc_th", &di->dts.rechg_soc_th, DC_RECHG_SOC_TH_DFT);
