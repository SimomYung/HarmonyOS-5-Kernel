	...
	struct direct_charge_device *di)
{
	...
	while (ibus < MIN_CURRENT_FOR_RES_DETECT) {
		...
		if (dc_need_exit() || dc_get_stop_charging_flag())
			return false;
		/* sc open succ need 200+ ms, need wait */
...
static void direct_charge_open_recharge_path_before_charging(void)
{
	struct direct_charge_device *l_di = direct_charge_get_di();
	...
	if (dc_rechg_is_en()) {
		hwlog_info("revise adapter current\n");
		dc_set_adapter_current(l_di->local_mode,
			l_di->adaptor_iset + DC_RECHARGE_ADAPTER_CURRENT_COM);
...
int direct_charge_open_charging_path(void)
{
	...
	direct_charge_open_recharge_path_before_charging();
...
static void direct_charge_reset_para_in_stop(int mode)
{
	...
	l_di->cc_safe = true;
	dc_ctrl_clear_first_start_time();
}
...
void direct_charge_exit(void)
{
	...
	dc_comm_set_uevent_send_flag(false);
}
...
bool direct_charge_check_timeout(void)
{
	...
	if ((adapter_detect_get_runtime_protocol_type() == BIT(ADAPTER_PROTOCOL_PPS)) && (l_di->working_mode == SC4_MODE)) {
		time = dc_adpt_get_time_sc4_pps();
	} else {
...
static int direct_charge_get_imax_by_time(struct direct_charge_device *di)
{
	...
	if (dc_turbo_get_time_para(di->working_mode, &para, &size, di->adapter_type) ||
		...
		if (di->time_para_parse_ok)
			para = di->time_para;
		else if (di->time_para_p_parse_ok)
...
static void direct_charge_update_vote_on_condition(int client, bool en, int val)
{
	if (val > 0)
		dc_vote_set(VOTE_OBJ_CURRENT, client, en, val);
	else
		dc_vote_set(VOTE_OBJ_CURRENT, client, false, 0); /* cancel vote */
}

void direct_charge_vote_current_by_static_cccv(struct direct_charge_device *di)
{
	dc_vote_set(VOTE_OBJ_CURRENT, VOTE_CLIENT_CURR_STATIC_CCCV,
		true, direct_charge_get_cccv_max_current(di));
}

void direct_charge_vote_current_by_tbat(struct direct_charge_device *di)
{
	/* step-1: get max current with specified temp */
	dc_vote_set(VOTE_OBJ_CURRENT, VOTE_CLIENT_CURR_TBAT,
		true, direct_charge_get_temp_max_current(di));
}

void direct_charge_vote_current_by_adpt(struct direct_charge_device *di)
{
	/* step-2: get max current with adapter */
	direct_charge_update_vote_on_condition(VOTE_CLIENT_CURR_ADPT,
		true, (dc_adpt_get_curr(di->local_mode, di->adaptor_vset) * di->dc_volt_ratio));
}

void direct_charge_vote_current_by_cable(struct direct_charge_device *di)
{
	/* step-3: get max current with cable type */
	direct_charge_update_vote_on_condition(VOTE_CLIENT_CURR_CABLE,
		true, dc_get_cable_curr());
	hwlog_info("cc_cable=%d\n", dc_get_cable_inherent_info(DC_CABLE_DETECT_OK));
}

void direct_charge_vote_current_by_cur_mode(struct direct_charge_device *di)
{
	/* step-5: get max current with cur_mode */
	direct_charge_update_vote_on_condition(VOTE_CLIENT_CURR_CUR_MODE,
		true, direct_charge_get_imax_with_cur_mode(di));
}

void direct_charge_vote_current_by_time(struct direct_charge_device *di)
{
	/* step-6: get max current with time limit */
	direct_charge_update_vote_on_condition(VOTE_CLIENT_CURR_TIME,
		true, direct_charge_get_imax_by_time(di));
}

void direct_charge_vote_current_by_multi_ic(struct direct_charge_device *di)
{
	/* step-7: error check limit current, only multi mode effected */
	if (di->multi_ic_check_info.limit_current > 0 && di->cur_mode == CHARGE_MULTI_IC)
		direct_charge_update_vote_on_condition(VOTE_CLIENT_CURR_MULTI_IC,
			true, di->multi_ic_check_info.limit_current);
}

void direct_charge_vote_current_by_thermal(struct direct_charge_device *di)
{
	/* step-8: get max current with thermal */
	if (!dc_mmi_get_test_flag())
		direct_charge_update_vote_on_condition(VOTE_CLIENT_CURR_THERMAL,
			true, dc_sysfs_get_ichg_thermal(di->local_mode, di->cur_mode));
}

void direct_charge_vote_current_by_battery_care(struct direct_charge_device *di)
{
	/* step-9: get max current with batteryCare */
	if (!dc_mmi_get_test_flag())
		direct_charge_update_vote_on_condition(VOTE_CLIENT_CURR_BATTERY_CARE,
			true, dc_sysfs_get_ichg_battery_care(di->local_mode));
}

void direct_charge_vote_current_by_dynamic_cccv(struct direct_charge_device *di)
{
	direct_charge_update_vote_on_condition(VOTE_CLIENT_CURR_DYNAMIC_CCCV,
		true, bat_cccv_get_ichg(di->vbat));
}

void direct_charge_vote_current_by_balance(struct direct_charge_device *di)
{
	direct_charge_update_vote_on_condition(VOTE_CLIENT_CURR_BALANCE,
		true, direct_charge_get_imax_by_batt_balance(di));
}

void direct_charge_vote_current_by_cc_unsafe(struct direct_charge_device *di)
{
	/* enable sc and limit cur to 3000mA even when cc is unsafe when cc_unsafe_sc_enable is true */
	if (di->cc_unsafe_sc_enable && !di->cc_safe)
		direct_charge_update_vote_on_condition(VOTE_CLIENT_CURR_CC_UNSAFE,
			true, CC_UNSAFE_MAX_CURRENT);
}

typedef void (*direct_charge_vote_current_fn)(struct direct_charge_device *di);

static direct_charge_vote_current_fn vote_current_fns[] = {
	direct_charge_vote_current_by_static_cccv,
	direct_charge_vote_current_by_tbat,
	direct_charge_vote_current_by_adpt,
	direct_charge_vote_current_by_cable,
	direct_charge_vote_current_by_cur_mode,
	direct_charge_vote_current_by_time,
	direct_charge_vote_current_by_multi_ic,
	direct_charge_vote_current_by_thermal,
	direct_charge_vote_current_by_battery_care,
	direct_charge_vote_current_by_dynamic_cccv,
	direct_charge_vote_current_by_balance,
	direct_charge_vote_current_by_cc_unsafe,
};
...
void direct_charge_select_charging_param(void)
{
	...
	for (i = 0; i < ARRAY_SIZE(vote_current_fns); ++i) {
		if (vote_current_fns[i])
			vote_current_fns[i](l_di);
	}

	l_di->cur_ibat_th_high = dc_vote_get_effective_result_with_lock(VOTE_OBJ_CURRENT);
}
...
void direct_charge_stop_charging(void)
{
	...
	dc_rechg_set_enable(DC_RECHARGE_DISABLE, DC_RECHARGE_TARGET_IBAT_DFT);
	...
	dc_vote_clear(VOTE_OBJ_CURRENT);
