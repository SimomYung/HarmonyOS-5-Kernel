// SPDX-License-Identifier: GPL-2.0
/*
 * wireless_rx_fod.c
 *
 * rx fod for wireless charging
 *
 * Copyright (c) 2021-2021 Huawei Technologies Co., Ltd.
 *
 * This software is licensed under the terms of the GNU General Public
 * License version 2, as published by the Free Software Foundation, and
 * may be copied, distributed, and modified under those terms.
 *
 * This program is distributed in the hope that it will be useful,
 * but WITHOUT ANY WARRANTY; without even the implied warranty of
 * MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE. See the
 * GNU General Public License for more details.
 *
 */

#include <chipset_common/hwpower/protocol/wireless_protocol.h>
#include <chipset_common/hwpower/wireless_charge/wireless_rx_acc.h>
#include <chipset_common/hwpower/wireless_charge/wireless_rx_common.h>
#include <chipset_common/hwpower/wireless_charge/wireless_rx_fod.h>
#include <chipset_common/hwpower/wireless_charge/wireless_trx_ic_intf.h>
#include <chipset_common/hwpower/wireless_charge/wireless_rx_pmode.h>
#include <chipset_common/hwpower/wireless_charge/wireless_rx_status.h>
#include <chipset_common/hwpower/common_module/power_cmdline.h>
#include <chipset_common/hwpower/common_module/power_debug.h>
#include <chipset_common/hwpower/common_module/power_delay.h>
#include <chipset_common/hwpower/common_module/power_dts.h>
#include <chipset_common/hwpower/common_module/power_printk.h>
#include <chipset_common/hwpower/common_module/power_supply_interface.h>
#include <chipset_common/hwpower/common_module/power_extra_event.h>
#include <huawei_platform/hwpower/common_module/power_platform_macro.h>
#include <securec.h>

#define HWLOG_TAG wireless_rx_fod
HWLOG_REGIST();

#define WLRX_FOD_STATUS_CFG_ROW           30
#define WLRX_FOD_STATUS_CFG_COL           4
#define WLRX_FOD_STATUS_FMT_OLD           0
#define WLRX_FOD_STATUS_FMT_NEW           1
#define WLRX_FOD_QR_FR_MIN                1
#define WLRX_FOD_QR_FR_MAX                255
#define WLRX_FOD_QR_FR_LEGAL_MIN          60
#define WLRX_FOD_QR_FR_LEGAL_MAX          80
#define WLRX_FOD_KM_MIN                   1
#define WLRX_FOD_KM_MAX                   65535
#define WLRX_FOD_QVAL_CALI_ROW            20
#define WLRX_FOD_QVAL_CALI_COL            2

#define WLRX_FOD_INGNORE_QVAL_TIMEOUT     30000 /* 30s */

enum wlrx_status_cond {
	WLRX_FOD_STATUS_BEGIN = 0,
	WLRX_FOD_STATUS_ID = WLRX_FOD_STATUS_BEGIN,
	WLRX_FOD_STATUS_SCN,
	WLRX_FOD_STATUS_TX_TYPE,
	WLRX_FOD_STATUS_TH,
	WLRX_FOD_STATUS_FOLD_STATUS,
	WLRX_FOD_STATUS_DIRECTION,
	WLRX_FOD_STATUS_END,
};

struct wlrx_status_cfg {
	int id;
	int scn;
	int tx_type;
	int status_th;
	u32 fold_status;
	u32 device_direction;
};

struct wlrx_fod_calibrated_qval_para {
	u32 fold_status;
	u32 device_direction;
};

struct wlrx_fod_dts {
	int ignore_qval;
	unsigned int qval_delayed_time;
	unsigned int cfg_fmt_type;
	int support_qval_cali;
	int qval_err_bat_cycle;
	/* old format */
	unsigned int status_cfg_len;
	u32 status_cfg_old[WLRX_SCN_END];
	/* new format */
	unsigned int status_cfg_row;
	struct wlrx_status_cfg *status_cfg_new;
	unsigned int calibrated_qval_para_row;
	struct wlrx_fod_calibrated_qval_para *calibrated_qval_para;
};

struct wlrx_fod_dev {
	u8 qval_supp_pmode;
	int fod_status;
	unsigned int drv_type;
	struct wlrx_fod_dts *dts;
	struct wltx_fod_result_info fod_result_info;
	struct delayed_work ignore_qval_work;
	struct delayed_work qval_delayed_work;
};

static struct wlrx_fod_dev *g_rx_fod_di[WLTRX_DRV_MAX];

static struct wlrx_fod_dev *wlrx_fod_get_di(unsigned int drv_type)
{
	if (!wltrx_is_drv_type_valid(drv_type)) {
		hwlog_err("get_di: drv_type=%u err\n", drv_type);
		return NULL;
	}

	if (!g_rx_fod_di[drv_type] || !g_rx_fod_di[drv_type]->dts) {
		hwlog_err("get_di: drv_type=%u para null\n", drv_type);
		return NULL;
	}

	return g_rx_fod_di[drv_type];
}

static unsigned int wlrx_fod_get_ic_type(unsigned int drv_type)
{
	return wltrx_get_dflt_ic_type(drv_type);
}

static bool wlrx_fod_status_match(int cur_status, unsigned long status)
{
	if (status == 0)
		return true;

	if (cur_status < 0)
		return false;

	return test_bit(cur_status, &status);
}

static int wlrx_fod_select_new_status_para(struct wlrx_fod_dev *di, enum wlrx_scene scn_id,
	unsigned int tx_type)
{
	int i;
	int fold_status = power_hall_get_status();
	int device_direction = wlrx_get_device_direction(di->drv_type);

	for (i = 0; i < di->dts->status_cfg_row; i++) {
		if ((di->dts->status_cfg_new[i].scn >= 0) &&
			(di->dts->status_cfg_new[i].scn != scn_id))
			continue;
		if ((di->dts->status_cfg_new[i].tx_type >= 0) &&
			(di->dts->status_cfg_new[i].tx_type != tx_type))
			continue;
		if (!wlrx_fod_status_match(fold_status, di->dts->status_cfg_new[i].fold_status))
			continue;
		if (!wlrx_fod_status_match(device_direction, di->dts->status_cfg_new[i].device_direction))
			continue;
		break;
	}
	if (i >= di->dts->status_cfg_row) {
		hwlog_err("select_new_status_para: status_th mismatch\n");
		return -EINVAL;
	}
	hwlog_info("[select_new_status_para] id=%d scn_id=%u tx_type=%u fold_status=%d "
		"device_direction=%d status_th=0x%x\n", i, scn_id, tx_type, fold_status,
		device_direction, di->dts->status_cfg_new[i].status_th);
	di->fod_status = di->dts->status_cfg_new[i].status_th;
	return 0;
}

static int wlrx_fod_select_old_status_para(struct wlrx_fod_dev *di, enum wlrx_scene scn_id)
{
	if (di->dts->status_cfg_old[scn_id] <= 0)
		return -EINVAL;

	di->fod_status = di->dts->status_cfg_old[scn_id];
	return 0;
}

static int wlrx_fod_select_status_para(struct wlrx_fod_dev *di, unsigned int tx_type)
{
	enum wlrx_scene scn_id = wlrx_get_scene();

	if ((scn_id < 0) || (scn_id >= WLRX_SCN_END))
		return -EINVAL;

	if (di->dts->cfg_fmt_type == WLRX_FOD_STATUS_FMT_NEW)
		return wlrx_fod_select_new_status_para(di, scn_id, tx_type);
	else if (di->dts->cfg_fmt_type == WLRX_FOD_STATUS_FMT_OLD)
		return wlrx_fod_select_old_status_para(di, scn_id);
	else
		return -EINVAL;
}

static bool wlrx_ignore_fod_status(struct wlrx_fod_dev *di)
{
	if (di->dts->ignore_qval <= 0)
		return false;

	return wlrx_is_qval_err_tx(di->drv_type);
}

static bool wlrx_need_delayed_fod_status(struct wlrx_fod_dev *di)
{
	int i;

	if (di->dts->qval_delayed_time == 0)
		return false;

	for (i = 0; i < di->dts->qval_delayed_time / DT_MSLEEP_100MS; i++) {
		power_msleep(DT_MSLEEP_100MS, 0, NULL);
		if (di->dts->qval_delayed_time == 0)
			return false;
		if (wlrx_get_wireless_channel_state() == WIRELESS_CHANNEL_OFF) {
			di->dts->qval_delayed_time = 0;
			return true;
		}
	}

	return false;
}

static void wlrx_fod_ignore_qval_work(struct work_struct *work)
{
	struct wlrx_fod_dev *di = container_of(work,
		struct wlrx_fod_dev, ignore_qval_work.work);

	if (!di)
		return;

	hwlog_info("[ignore_qval_work] timeout, permit SC mode\n");
	di->qval_supp_pmode = WLRX_SUPP_PMODE_ALL;
}

static void wlrx_fod_preproccess_status(struct wlrx_fod_dev *di)
{
	hwlog_err("preproccess_status: tx_fod_err, forbid SC mode\n");
	di->qval_supp_pmode = WLRX_SUPP_PMODE_BUCK;
	schedule_delayed_work(&di->ignore_qval_work,
		msecs_to_jiffies(WLRX_FOD_INGNORE_QVAL_TIMEOUT));
}

u8 wlrx_fod_get_support_mode(unsigned int drv_type)
{
	struct wlrx_fod_dev *di = wlrx_fod_get_di(drv_type);

	if (!di)
		return WLRX_SUPP_PMODE_BUCK;

	return di->qval_supp_pmode;
}

static void wlrx_send_noncali_qval_para(struct wlrx_fod_dev *di, unsigned int tx_type)
{
	int ret;

	if ((di->dts->cfg_fmt_type == WLRX_FOD_STATUS_FMT_NEW) &&
		(di->dts->status_cfg_row == 0))
		return;

	if ((di->dts->cfg_fmt_type == WLRX_FOD_STATUS_FMT_OLD) &&
		(di->dts->status_cfg_len == 0))
		return;

	ret = wlrx_fod_select_status_para(di, tx_type);
	if (ret)
		return;

	ret = wireless_send_fod_status(wlrx_fod_get_ic_type(di->drv_type),
		WIRELESS_PROTOCOL_QI, di->fod_status);
	if (ret) {
		hwlog_err("send_fod_status: val=0x%x, failed\n", di->fod_status);
		return;
	}

	hwlog_info("[send_fod_status] val=0x%x, succ\n", di->fod_status);
}

static bool wlrx_fod_support_calibrated_qval(struct wlrx_fod_dev *di)
{
	int i;
	int fold_status = power_hall_get_status();
	int device_direction = wlrx_get_device_direction(di->drv_type);

	if (di->dts->calibrated_qval_para_row == 0)
		return true;

	for (i = 0; i < di->dts->calibrated_qval_para_row; i++) {
		if (!wlrx_fod_status_match(fold_status,
			di->dts->calibrated_qval_para[i].fold_status))
			continue;
		if (!wlrx_fod_status_match(device_direction,
			di->dts->calibrated_qval_para[i].device_direction))
			continue;

		return true;
	}

	return false;
}

static bool wlrx_fod_use_calibrated_qval(struct wlrx_fod_dev *di)
{
	enum wlrx_scene scn_id;
	struct wlprot_acc_cap *acc_cap;

	if (!di->dts->support_qval_cali)
		return false;

	if (wlrx_is_fac_tx(di->drv_type))
		return true;

	scn_id = wlrx_get_scene();
	if (scn_id != WLRX_SCN_NORMAL) {
		hwlog_err("wlrx_fod_use_calibrated_qval: scn_id=%d\n", scn_id);
		return false;
	}

	acc_cap = wlrx_acc_get_cap(di->drv_type);
	if (!acc_cap || !acc_cap->support_fod_km) {
		hwlog_err("wlrx_fod_use_calibrated_qval: no support_fod_km\n");
		return false;
	}

	if (!wlrx_fod_support_calibrated_qval(di)) {
		hwlog_err("wlrx_fod_use_calibrated_qval: fold status not support\n");
		return false;
	}

	return true;
}

static bool wlrx_fod_recheck_cali_val(struct wlrx_fod_dev *di, struct power_nv_fod_qval qval)
{
	int batt_cycle = 0;

	if (di->dts->qval_err_bat_cycle == 0)
		return true;
	(void)power_supply_get_int_property_value(POWER_PLATFORM_BAT_PSY_NAME,
		POWER_SUPPLY_PROP_CYCLE_COUNT, &batt_cycle);
	if ((batt_cycle >= di->dts->qval_err_bat_cycle) &&
		((qval.qr < WLRX_FOD_QR_FR_LEGAL_MIN) || (qval.qr > WLRX_FOD_QR_FR_LEGAL_MAX)))
		return false;

	return true;
}

static bool wlrx_fod_check_qval(struct wlrx_fod_dev *di, struct power_nv_fod_qval qval)
{
	if ((qval.km < WLRX_FOD_KM_MIN) || (qval.km > WLRX_FOD_KM_MAX))
		return false;
	if ((qval.qr < WLRX_FOD_QR_FR_MIN) || (qval.qr > WLRX_FOD_QR_FR_MAX))
		return false;
	if ((qval.fr < WLRX_FOD_QR_FR_MIN) || (qval.fr > WLRX_FOD_QR_FR_MAX))
		return false;
	if (!wlrx_fod_recheck_cali_val(di, qval))
		return false;

	return true;
}

static int wlrx_send_cali_qval_para(struct wlrx_fod_dev *di)
{
	int ret;
	u32 fod_status;
	struct power_nv_fod_qval qval = { 0 };

	if (!wlrx_fod_use_calibrated_qval(di))
		return -ENOTSUPP;

	ret = wlrx_fod_get_cali_para(di->drv_type, &qval);
	hwlog_info("[get_fod_qval] km=%u, qr=%u, fr=%u, ret=%d\n",
		qval.km, qval.qr, qval.fr, ret);
	if (ret || !wlrx_fod_check_qval(di, qval))
		return -EINVAL;

	ret = wireless_send_fod_km(wlrx_fod_get_ic_type(di->drv_type),
		WIRELESS_PROTOCOL_QI, (int)qval.km);
	if (ret) {
		hwlog_err("send_fod_km: failed\n");
		return ret;
	}
	hwlog_info("[send_fod_km] succ\n");

	/* fod_status: qr[8bits] fr[8bits] */
	fod_status = (qval.qr << 8) + qval.fr;
	ret = wireless_send_fod_status(wlrx_fod_get_ic_type(di->drv_type),
		WIRELESS_PROTOCOL_QI, (int)fod_status);
	hwlog_info("[send_fod_status] val=0x%x, ret=%d\n", fod_status, ret);

	return ret;
}

int wlrx_fod_get_cali_para(unsigned int drv_type, struct power_nv_fod_qval *qval)
{
	struct wlrx_fod_dev *di = wlrx_fod_get_di(drv_type);

	if (!di)
		return -EINVAL;

	return power_common_nv_read(POWER_NV_COMMON_QVAL, (void *)qval);
}

int wlrx_fod_set_cali_para(unsigned int drv_type, struct power_nv_fod_qval qval)
{
	struct wlrx_fod_dev *di = wlrx_fod_get_di(drv_type);

	if (!di)
		return -EINVAL;

	return power_common_nv_write(POWER_NV_COMMON_QVAL, (void *)&qval);
}

static void wlrx_fod_reset_cali_para(unsigned int drv_type)
{
	struct wlrx_fod_dev *di = wlrx_fod_get_di(drv_type);
	struct power_nv_fod_qval qval = { 0 };

	if (!di)
		return;

	power_common_nv_write(POWER_NV_COMMON_QVAL, (void *)&qval);
}

static void wlrx_fod_get_cali_qval(unsigned int drv_type, struct wlrx_fod_dev *di)
{
	int ret;
	u8 cali = 0;
	struct wlprot_acc_cap *acc_cap = wlrx_acc_get_cap(drv_type);
	struct power_nv_fod_qval qval = { 0 };

	(void)power_common_nv_read(POWER_NV_COMMON_CALI, (void *)&cali);
	if (!di->dts->support_qval_cali || !wlrx_is_fac_tx(drv_type) ||
		!acc_cap || !acc_cap->support_qval_cali || cali)
		return;

	ret = wireless_get_fod_status(wlrx_fod_get_ic_type(drv_type),
		WIRELESS_PROTOCOL_QI, (u8 *)&qval.qr, (u8 *)&qval.fr);
	if (ret) {
		wlrx_fod_reset_cali_para(drv_type);
		return;
	}

	ret = wireless_get_fod_km_kz(wlrx_fod_get_ic_type(drv_type),
		WIRELESS_PROTOCOL_QI, (u16 *)&qval.km, (u16 *)&qval.kz);
	if (ret || !qval.kz) {
		wlrx_fod_reset_cali_para(drv_type);
		return;
	}

	(void)wlrx_fod_set_cali_para(drv_type, qval);
}

void wlrx_send_fod_status(unsigned int drv_type, unsigned int tx_type)
{
	struct wlrx_fod_dev *di = wlrx_fod_get_di(drv_type);

	if (!di || !di->dts)
		return;

	if (wlrx_ignore_fod_status(di)) {
		wlrx_fod_preproccess_status(di);
		return;
	}

	if (wlrx_need_delayed_fod_status(di))
		return;

	if (power_cmdline_is_factory_mode())
		wlrx_fod_get_cali_qval(drv_type, di);

	if (wlrx_send_cali_qval_para(di))
		wlrx_send_noncali_qval_para(di, tx_type);
}

void wlrx_get_fod_result_info(unsigned int drv_type)
{
	int ret;
	struct wlrx_fod_dev *di = wlrx_fod_get_di(drv_type);

	if (!di)
		return;

	ret = wireless_get_fod_result_info(wlrx_fod_get_ic_type(drv_type),
		WIRELESS_PROTOCOL_QI, &(di->fod_result_info));
	if (ret)
		hwlog_err("wlrx_get_fod_result_info: failed\n");
}

static void wlrx_qval_delayed_timeout_work(struct work_struct *work)
{
	struct wlrx_fod_dev *di = container_of(work,
		struct wlrx_fod_dev, qval_delayed_work.work);

	if (!di) {
		hwlog_err("qval_delayed_timeout_work: para null\n");
		return;
	}

	di->dts->qval_delayed_time = 0;
	hwlog_info("[qval_delayed_timeout_work] qval_delayed_time cleared\n");
}

static int wlrx_fod_set_status_cfg(const char *tmp_str, struct wlrx_status_cfg *status_cfg, int offset)
{
	if (!tmp_str || !status_cfg)
		return -EINVAL;

	switch (offset) {
	case WLRX_FOD_STATUS_ID:
		return kstrtoint(tmp_str, 0, &status_cfg->id);
	case WLRX_FOD_STATUS_SCN:
		return kstrtoint(tmp_str, 0, &status_cfg->scn);
	case WLRX_FOD_STATUS_TX_TYPE:
		return kstrtoint(tmp_str, 0, &status_cfg->tx_type);
	case WLRX_FOD_STATUS_TH:
		return kstrtoint(tmp_str, 0, &status_cfg->status_th);
	case WLRX_FOD_STATUS_FOLD_STATUS:
		return kstrtouint(tmp_str, 0, &status_cfg->fold_status);
	case WLRX_FOD_STATUS_DIRECTION:
		return kstrtouint(tmp_str, 0, &status_cfg->device_direction);
	default:
		break;
	}

	return -EINVAL;
}

static int wlrx_parse_new_fod_status(const struct device_node *np, struct wlrx_fod_dts *dts,
	int len, u32 status_cfg_col)
{
	int i, row, col;
	const char *tmp_str = NULL;

	if ((dts->status_cfg_row == 0) || (status_cfg_col == 0))
		goto parse_fod_status_fail;

	dts->status_cfg_new = kcalloc(dts->status_cfg_row, sizeof(*(dts->status_cfg_new)), GFP_KERNEL);
	if (!dts->status_cfg_new)
		goto parse_fod_status_fail;

	for (i = 0; i < len; i++) {
		if (power_dts_read_string_index(power_dts_tag(HWLOG_TAG), np, "fod_status_new", i, &tmp_str))
			goto parse_status_cfg_fail;

		row = i / status_cfg_col;
		col = i % status_cfg_col;
		if (wlrx_fod_set_status_cfg(tmp_str, &dts->status_cfg_new[row], col))
			goto parse_status_cfg_fail;
	}

	for (i = 0; i < dts->status_cfg_row; i++)
		hwlog_info("rx_status_cond[%d] id=%d scn=%d tx_type=%d fold_status=0x%x "
			"device_direction=0x%x status_th=0x%x\n",
			i, dts->status_cfg_new[i].id, dts->status_cfg_new[i].scn,
			dts->status_cfg_new[i].tx_type, dts->status_cfg_new[i].fold_status,
			dts->status_cfg_new[i].device_direction, dts->status_cfg_new[i].status_th);

	return 0;

parse_status_cfg_fail:
	kfree(dts->status_cfg_new);
parse_fod_status_fail:
	dts->status_cfg_row = 0;
	hwlog_err("parse_new_fod_status: failed\n");
	return -EINVAL;
}

static int wlrx_parse_old_fod_status(const struct device_node *np, struct wlrx_fod_dts *dts)
{
	int i;

	if (power_dts_read_u32_array(power_dts_tag(HWLOG_TAG), np,
		"fod_status", (u32 *)dts->status_cfg_old, dts->status_cfg_len)) {
		dts->status_cfg_len = 0;
		return -EINVAL;
	}
	for (i = 0; i < dts->status_cfg_len; i++)
		hwlog_info("fod_status[%d]=0x%x\n", i, dts->status_cfg_old[i]);

	return 0;
}

static int wlrx_parse_fod_status(const struct device_node *np, struct wlrx_fod_dts *dts)
{
	int len;
	u32 status_cfg_col = 0;

	(void)power_dts_read_u32(power_dts_tag(HWLOG_TAG), np,
		"fod_status_new_col", &status_cfg_col, WLRX_FOD_STATUS_CFG_COL);
	len = power_dts_read_count_strings(power_dts_tag(HWLOG_TAG), np,
		"fod_status_new", WLRX_FOD_STATUS_CFG_ROW, status_cfg_col);
	if ((len > 0) && (status_cfg_col > 0)) {
		dts->status_cfg_row = len / status_cfg_col;
		dts->cfg_fmt_type = WLRX_FOD_STATUS_FMT_NEW;
		return wlrx_parse_new_fod_status(np, dts, len, status_cfg_col);
	}

	len = of_property_count_u32_elems(np, "fod_status");
	if ((len <= 0) || (len > WLRX_SCN_END)) {
		dts->status_cfg_len = 0;
		return 0;
	}
	dts->status_cfg_len = len;
	dts->cfg_fmt_type = WLRX_FOD_STATUS_FMT_OLD;
	return wlrx_parse_old_fod_status(np, dts);
}

static void wlrx_parse_qval_cali_para(const struct device_node *np, struct wlrx_fod_dts *dts)
{
	int len, row;

	len = power_dts_read_u32_count(power_dts_tag(HWLOG_TAG), np,
		"calibrated_qval_para", WLRX_FOD_QVAL_CALI_ROW, WLRX_FOD_QVAL_CALI_COL);
	if (len <= 0)
		return;

	row = len / WLRX_FOD_QVAL_CALI_COL;
	dts->calibrated_qval_para = kcalloc(row, sizeof(struct wlrx_fod_calibrated_qval_para), GFP_KERNEL);
	if (!dts->calibrated_qval_para)
		return;

	if (power_dts_read_u32_array(power_dts_tag(HWLOG_TAG), np,
		"calibrated_qval_para", (u32 *)dts->calibrated_qval_para, len)) {
		kfree(dts->calibrated_qval_para);
		dts->calibrated_qval_para = NULL;
		return;
	}

	dts->calibrated_qval_para_row = row;
}

static int wlrx_fod_parse_dts(const struct device_node *np, struct wlrx_fod_dts **dts)
{
	int ret;

	*dts = kzalloc(sizeof(**dts), GFP_KERNEL);
	if (!*dts)
		return -ENOMEM;

	ret = wlrx_parse_fod_status(np, *dts);
	if (ret)
		return ret;

	(void)power_dts_read_u32(power_dts_tag(HWLOG_TAG), np,
		"ignore_qval", &(*dts)->ignore_qval, 0);

	(void)power_dts_read_u32(power_dts_tag(HWLOG_TAG), np,
		"qval_delayed_time", &(*dts)->qval_delayed_time, 0);

	(void)power_dts_read_u32(power_dts_tag(HWLOG_TAG), np,
		"support_qval_cali", &(*dts)->support_qval_cali, 0);

	(void)power_dts_read_u32(power_dts_tag(HWLOG_TAG), np,
		"qval_err_bat_cycle", &(*dts)->qval_err_bat_cycle, 0);

	wlrx_parse_qval_cali_para(np, *dts);

	return 0;
}

static void wlrx_fod_kfree_dev(struct wlrx_fod_dev *di)
{
	if (!di)
		return;

	if (di->dts->status_cfg_new)
		kfree(di->dts->status_cfg_new);
	if (di->dts->calibrated_qval_para)
		kfree(di->dts->calibrated_qval_para);
	kfree(di->dts);
	kfree(di);
}

static void wlrx_fod_free_resources(struct wlrx_fod_dev *di)
{
	if (!di)
		return;

	if (di->dts->qval_delayed_time)
		cancel_delayed_work_sync(&di->qval_delayed_work);
}

static int wlrx_fod_debug_set(struct wlrx_fod_dev *di, int fod_status)
{
	int i;

	if ((di->dts->cfg_fmt_type == WLRX_FOD_STATUS_FMT_NEW) &&
		(di->dts->status_cfg_row > 0)) {
		di->dts->status_cfg_new[0].scn = -1;
		di->dts->status_cfg_new[0].tx_type = -1;
		di->dts->status_cfg_new[0].fold_status = 0;
		di->dts->status_cfg_new[0].device_direction = 0;
		di->dts->status_cfg_new[0].status_th = fod_status;
		return 0;
	}

	di->dts->cfg_fmt_type = WLRX_FOD_STATUS_FMT_OLD;
	for (i = WLRX_SCN_BEGIN; i < WLRX_SCN_END; i++)
		di->dts->status_cfg_old[i] = fod_status;

	hwlog_info("[fod_debug_set] fod_status=0x%x\n", fod_status);
	return 0;
}

static int wlrx_fod_debug_get(struct wlrx_fod_dev *di, char *buf, size_t size)
{
	int i;
	int len = 0;

	if (di->dts->cfg_fmt_type == WLRX_FOD_STATUS_FMT_NEW) {
		len += snprintf_s(buf + len, size - len, size - len - 1,
			"id scn tx_type fold device_direction status_th\n");
		for (i = 0; i < di->dts->status_cfg_row; i++)
			len += snprintf_s(buf + len, size - len, size - len - 1, "%d %d %d 0x%x 0x%x 0x%x\n",
				di->dts->status_cfg_new[i].id, di->dts->status_cfg_new[i].scn,
				di->dts->status_cfg_new[i].tx_type, di->dts->status_cfg_new[i].fold_status,
				di->dts->status_cfg_new[i].device_direction, di->dts->status_cfg_new[i].status_th);
		return len;
	}

	len += snprintf_s(buf + len, size - len, size - len - 1, "scn val\n");
	for (i = WLRX_SCN_BEGIN; i < WLRX_SCN_END; i++)
		len += snprintf_s(buf + len, size - len, size - len - 1, "%d 0x%x\n",
			i, di->dts->status_cfg_old[i]);

	return len;
}

static ssize_t wlrx_fod_debug_store(void *dev_data, const char *buf, size_t size)
{
	int ret;
	int fod_status = 0;
	struct wlrx_fod_dev *di = NULL;

	hwlog_info("debug_store\n");
	di = (struct wlrx_fod_dev *)dev_data;
	if (!di || !di->dts)
		return -EINVAL;

	/* 1: input para num */
	if (sscanf_s(buf, "%x", &fod_status) != 1) {
		hwlog_err("fod_status_store: unable to parse input %s\n", buf);
		return -EINVAL;
	}

	ret = wlrx_fod_debug_set(di, fod_status);
	if (ret)
		hwlog_err("fod_status_store: set fail\n");
	return (ssize_t)size;
}

static ssize_t wlrx_fod_debug_show(void *dev_data, char *buf, size_t size)
{
	struct wlrx_fod_dev *di = NULL;

	hwlog_info("debug_show\n");
	di = (struct wlrx_fod_dev *)dev_data;
	if (!di || !di->dts)
		return -EINVAL;

	return wlrx_fod_debug_get(di, buf, size);
}

static void wlrx_fod_set_dflt_fod_result(struct wlrx_fod_dev *di)
{
	di->fod_result_info.q_qp_ratio = 0;
	di->fod_result_info.limit_pwr = 0;
	di->fod_result_info.qs = 0;
	di->fod_result_info.fs = 0;
}

void wlrx_fod_reset_para(unsigned int drv_type, int rst_type)
{
	struct wlrx_fod_dev *di = wlrx_fod_get_di(drv_type);

	if (!di)
		return;

	switch (rst_type) {
	case WLTRX_RST_IN_INIT:
		di->qval_supp_pmode = WLRX_SUPP_PMODE_ALL;
		wlrx_fod_set_dflt_fod_result(di);
		break;
	case WLTRX_RST_IN_STOP:
		cancel_delayed_work_sync(&di->ignore_qval_work);
		break;
	case WLTRX_RST_IN_SHUTDWON:
		cancel_delayed_work(&di->ignore_qval_work);
		break;
	default:
		hwlog_err("reset_para: input=%d invalid\n", rst_type);
		break;
	}
}

int wlrx_fod_init(unsigned int drv_type, struct device *dev)
{
	int ret;
	struct wlrx_fod_dev *di = NULL;

	if (!dev || !wltrx_is_drv_type_valid(drv_type))
		return -EINVAL;

	di = kzalloc(sizeof(*di), GFP_KERNEL);
	if (!di)
		return -ENOMEM;

	ret = wlrx_fod_parse_dts(dev->of_node, &di->dts);
	if (ret)
		goto exit;

	di->drv_type = drv_type;
	di->qval_supp_pmode = WLRX_SUPP_PMODE_ALL;
	INIT_DELAYED_WORK(&di->ignore_qval_work, wlrx_fod_ignore_qval_work);
	if (di->dts->qval_delayed_time) {
		INIT_DELAYED_WORK(&di->qval_delayed_work, wlrx_qval_delayed_timeout_work);
		schedule_delayed_work(&di->qval_delayed_work,
			msecs_to_jiffies(di->dts->qval_delayed_time));
	}

	power_dbg_ops_register("wlrx", "fod_status", di, wlrx_fod_debug_show, wlrx_fod_debug_store);
	g_rx_fod_di[drv_type] = di;
	return 0;

exit:
	wlrx_fod_kfree_dev(di);
	return ret;
}

void wlrx_fod_deinit(unsigned int drv_type)
{
	if (!wltrx_is_drv_type_valid(drv_type))
		return;

	wlrx_fod_free_resources(g_rx_fod_di[drv_type]);
	wlrx_fod_kfree_dev(g_rx_fod_di[drv_type]);
	g_rx_fod_di[drv_type] = NULL;
}
