...
static void wlrx_ui_prepare_ui_para(struct wlrx_ui_dev *di)
{
	...
	cur_tx_max_pwr = di->ui_para->tx_max_pwr;
	...
	if (cur_tx_max_pwr != di->ui_para->tx_max_pwr) {
		di->ui_para->max_pwr_update = true;
		return;
	}
	di->ui_para->max_pwr_update = false;
}
...
void wlrx_ui_send_charge_uevent(unsigned int drv_type, unsigned int icon_type)
{
	...
	struct wlrx_ui_dev *di = wlrx_ui_get_di(drv_type);
	bool icon_changed = (di->curr_icon_type != icon_type);
	...
	if (di->icon_flag.super_charge) {
		...
		if (icon_changed || di->ui_para->max_pwr_update) {
