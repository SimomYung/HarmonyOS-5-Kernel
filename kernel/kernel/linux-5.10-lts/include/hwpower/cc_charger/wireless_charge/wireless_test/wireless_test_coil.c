...
#define WIRELESS_COIL_TEST_NV_RETRY           3
 ...
 *
 */
static int wireless_coil_test_get_avg_iin(struct wireless_coil_dev *di, u16 *tx_avg_iin)
{
	int i;
	u16 tx_iin = 0;
	u16 iin_sum = 0;
	...
	for (i = 0; i < 20; i++) {
		...
		if ((tx_iin < di->wl_coil_iin_thr) && (cnt > 10))
			cnt--;
		else
			iin_sum += tx_iin;
	...
	*tx_avg_iin = iin_sum / cnt;
	hwlog_info("[get_avg_iin] tx_avg_iin=%humA,cnt=%d\n", *tx_avg_iin, cnt);
	return 0;
}

static int wireless_coil_test_store_avg_iin(u16 tx_avg_iin)
{
	int retry_times;
	int ret;

	for (retry_times = 0; retry_times < WIRELESS_COIL_TEST_NV_RETRY; retry_times++) {
		ret = power_common_nv_write(POWER_NV_COMMON_TX_AVG_IIN, (void *)&tx_avg_iin);
		if (!ret) {
			hwlog_info("[contact_test] write nv succ, tx_avg_iin=%u\n", tx_avg_iin);
			break;
		}
	}
...
static int wireless_coil_contact_test(struct wireless_coil_dev *di)
{
	int ret;
	u16 tx_avg_iin = 0;

	ret = wireless_coil_test_get_avg_iin(di, &tx_avg_iin);
	if (ret || (tx_avg_iin < di->wl_coil_iin_thr)) {
		hwlog_err("contact_test: failed\n");
		ret = -EINVAL;
		goto close_tx;
	}

	ret = wireless_coil_test_store_avg_iin(tx_avg_iin);
	if (ret) {
		hwlog_err("contact_test: write tx_avg_iin to NV failed\n");
		goto close_tx;
	}

	di->tx_avg_iin = tx_avg_iin;
...
static void wireless_coil_test_work(struct work_struct *work)
{
	...
	if (wireless_coil_contact_test(di))
...
int wireless_get_tx_avg_iin(void)
{
	struct wireless_coil_dev *di = g_wireless_coil_di;
...
static int __init wireless_coil_test_init(void)
{
	...
	(void)power_dts_read_u32_compatible(power_dts_tag(HWLOG_TAG),
		"huawei,power_test", "wl_coil_iin_thr", &di->wl_coil_iin_thr,
		WIRELESS_COIL_TEST_IIN_MIN);
