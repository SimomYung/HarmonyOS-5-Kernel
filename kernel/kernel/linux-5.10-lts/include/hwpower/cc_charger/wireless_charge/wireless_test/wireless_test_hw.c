...
int wireless_hw_test_ops_register(const struct wireless_hw_test_ops *ops)
{
	...
	if (ops->dev_data) {
		hwlog_info("register dev_data ops\n");
		g_hw_chk_ops.dev_data = ops->dev_data;
	}
...
static void wireless_hw_test_pwr_good_gpio(void)
{
	...
	if (di->hw_test_module & WIRELESS_HW_TEST_PWR_GOOD_GPIO &&
		!di->ops->chk_pwr_good_gpio(di->ops->dev_data))
...
static struct power_sysfs_attr_info wireless_hw_sysfs_field_tbl[] = {
	...
	power_sysfs_attr_rw(wireless_hw, 0640, WIRELESS_TX_AVG_IIN, tx_avg_iin),
};
	...
	struct device_attribute *attr, char *buf)
{
	...
	case WIRELESS_HW_TEST:
		if (di->test_busy)
			return snprintf_s(buf, PAGE_SIZE, PAGE_SIZE - 1, "test running\n");
		return snprintf_s(buf, PAGE_SIZE, PAGE_SIZE - 1, "%u\n", di->hw_test_result);
	case WIRELESS_COIL_TEST:
		return snprintf_s(buf, PAGE_SIZE, PAGE_SIZE - 1, "%d\n",
			wirless_coil_test_result());
	case WIRELESS_TX_AVG_IIN:
		return snprintf_s(buf, PAGE_SIZE, PAGE_SIZE - 1, "%d\n",
			wireless_get_tx_avg_iin());
	default:
		break;
	}
