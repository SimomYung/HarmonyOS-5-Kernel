...
static bool wltx_cap_need_revise_imax(void)
{
	...
	if (((pwr_type == WL_TX_PWR_5VBST_VBUS_OTG_CP) ||
		(pwr_type == WL_TX_PWR_5VBST_VBUS_OTG_CP2)) &&
...
unsigned int wltx_get_tx_pwr(unsigned int drv_type)
{
	struct wltx_cap_dev *di = wltx_cap_get_di(drv_type);
...
void wltx_send_tx_cap(unsigned int drv_type)
{
	...
	di->tx_pwr = tx_cap[WLTX_CAP_IMAX] * tx_cap[WLTX_CAP_VMAX] * POWER_TENTH;
}
