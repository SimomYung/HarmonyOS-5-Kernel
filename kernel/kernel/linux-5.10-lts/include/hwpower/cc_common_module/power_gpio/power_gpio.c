...
static unsigned int g_power_gpio_enable;
...
int power_gpio_request_index(struct device_node *np,
	const char *prop, const char *label, int index, unsigned int *gpio)
{
	if (!np || !prop || !label || !gpio) {
		hwlog_err("np or prop or label or gpio is null\n");
		return -EINVAL;
	...
	*gpio = of_get_named_gpio(np, prop, index);
	hwlog_info("%s: %s=%d\n", label, prop, *gpio);
...
int power_gpio_config_input_index(struct device_node *np,
	const char *prop, const char *label, int index, unsigned int *gpio)
{
	if (power_gpio_request_index(np, prop, label, index, gpio))
		return -EINVAL;
...
EXPORT_SYMBOL_GPL(power_gpio_config_input_index);
...
int power_gpio_config_output_index(struct device_node *np,
	const char *prop, const char *label, int index, unsigned int *gpio, int value)
{
	if (power_gpio_request_index(np, prop, label, index, gpio))
		return -EINVAL;
...
EXPORT_SYMBOL_GPL(power_gpio_config_output_index);
