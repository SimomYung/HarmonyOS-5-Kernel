...
#define POWER_PRINTK_INT_STR_LEN        12
...
void power_print_int_array(const char *name, const int *array, unsigned int size)
{
	int i, str_len;
	char *print_str = NULL;
	char tmp_str[POWER_PRINTK_INT_STR_LEN] = {0};

	if (!name || !array)
		return;

	str_len = size * POWER_PRINTK_INT_STR_LEN * sizeof(char);
	print_str = kzalloc(str_len, GFP_KERNEL);
	if (!print_str)
		return;

	for (i = 0; i < size; i++) {
		if (snprintf_s(tmp_str, POWER_PRINTK_INT_STR_LEN + 1, POWER_PRINTK_INT_STR_LEN, "%d ", array[i]) < 0)
			goto end;
		if (strncat_s(print_str, str_len - strlen(print_str), tmp_str, strlen(tmp_str)))
			goto end;
	}
