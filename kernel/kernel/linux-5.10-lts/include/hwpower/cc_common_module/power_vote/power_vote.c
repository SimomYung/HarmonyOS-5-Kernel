...
static const char *power_vote_get_effective_client(struct power_vote_object *obj)
{
	const char *eff_client_name = NULL;
	...
	return eff_client_name ? eff_client_name : "None";
}
	...
	const char *client_name, int client_id, bool enabled, int value)
{
	...
	similar_vote = (obj->clients[client_id].enabled == enabled) &&
		(obj->clients[client_id].value == value);

	obj->clients[client_id].enabled = enabled;
	obj->clients[client_id].value = value;

	return similar_vote && obj->voted_on;
}
	...
	const char *client_name, bool enabled, int value)
{
	...
	hwlog_info("%s: [%s,%d] vote %s of value=%d eff_result={%d:%d} last_eff_result={%d:%d}\n",
		obj->name, client_name, client_id, enabled ? "on" : "off", value,
		eff_id, eff_result, obj->eff_client_id, obj->eff_result);
...
bool power_vote_has_vote_object(const char *name)
{
	return name && power_vote_find_object(name);
}

const char *power_vote_get_effective_client_locked(const char *name, bool lock_flag)
{
	const char *client_name;
	struct power_vote_object *obj = NULL;
	...
	client_name = power_vote_get_effective_client(obj);

	if (lock_flag)
		power_vote_unlock(obj);

	hwlog_info("%s: get vote eff_client=%s\n", obj->name, client_name);
	return client_name;
}
...
void power_vote_print_object(const char *name)
{
	char *buf = NULL;
	struct power_vote_object *obj = NULL;
	...
	obj = power_vote_find_object(name);
	if (!obj) {
		hwlog_err("vote object %s not exist\n", name);
		return;
	}
	...
	if(!buf)
		return;
	power_vote_show_client_info(obj, buf, BUF_LEN - 1, 0);
	hwlog_info("all clients information: %s\n", buf);
	kfree(buf);
}
