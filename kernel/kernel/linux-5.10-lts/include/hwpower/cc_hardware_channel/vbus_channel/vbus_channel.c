...
int vbus_ch_open_with_volt(unsigned int user, unsigned int type, int flag, int volt)
{
	struct vbus_ch_ops *l_ops = vbus_ch_get_ops(user, type);
	...
	if (!l_ops->open_with_volt) {
		hwlog_err("open_with_volt is null\n");
		return -EINVAL;
	}

	hwlog_info("user=%s type=%s open\n", vbus_ch_get_op_user_name(user), vbus_ch_get_op_type_name(type));

	return l_ops->open_with_volt(user, flag, volt);
}
...
int vbus_ch_close_switch(unsigned int user, unsigned int type, bool flag)
{
	struct vbus_ch_ops *l_ops = vbus_ch_get_ops(user, type);
	...
	if (!l_ops->close_switch) {
		hwlog_err("close_switch is null\n");
		return -EINVAL;
	}

	hwlog_info("user=%s type=%s close_switch\n", vbus_ch_get_op_user_name(user),
		vbus_ch_get_op_type_name(type));

	return l_ops->close_switch(flag);
}
