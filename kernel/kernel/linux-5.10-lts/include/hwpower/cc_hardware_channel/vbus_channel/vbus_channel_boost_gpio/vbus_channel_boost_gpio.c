...
#define OCP_CHECK_RETRY_TIMES                  5
#define BOOST_GPIO_HIGH                        1
...
static void boost_gpio_otg_ocp_work(struct work_struct *work)
{
	...
	if (l_dev->otg_ocp_deglitch_enable) {
		for (i = 0; i < OCP_CHECK_RETRY_TIMES; i++) {
			power_usleep(DT_USLEEP_100US);
			gpio_value = gpio_get_value(l_dev->otg_ocp_int);
			if (gpio_value == BOOST_GPIO_HIGH) {
				hwlog_info("otg ocp deglitch succ\n");
				return;
			}
		}
	}
...
static void boost_gpio_set_gpio_switch(struct boost_gpio_dev *l_dev, int value)
{
	hwlog_info("%s start value=%d\n", __func__, value);
	mutex_lock(&l_dev->gpio_switch_mutex);
	l_dev->gpio_switch_flag = !!value;
	gpio_set_value(l_dev->gpio_en, value);
	mutex_unlock(&l_dev->gpio_switch_mutex);
	hwlog_info("%s end value=%d\n", __func__, value);
}

static int boost_gpio_close_gpio_switch(bool flag)
{
	struct boost_gpio_dev *l_dev = boost_gpio_get_dev();
	int value = flag ? BOOST_GPIO_SWITCH_DISABLE : BOOST_GPIO_SWITCH_ENABLE;

	if (!l_dev)
		return -EINVAL;

	hwlog_info("%s start flag=%u\n", __func__, flag);
	mutex_lock(&l_dev->gpio_switch_mutex);
	if (l_dev->gpio_switch_flag)
		gpio_set_value(l_dev->gpio_en, value);
	mutex_unlock(&l_dev->gpio_switch_mutex);
	hwlog_info("%s end flag=%u\n", __func__, flag);
	return 0;
}
...
static int boost_gpio_start_config(int flag)
{
	...
	power_sw_set_output(l_dev->vusbsw_index, POWER_SW_OFF);
	boost_5v_enable(BOOST_5V_ENABLE, BOOST_CTRL_BOOST_GPIO_OTG);
	power_usleep(DT_USLEEP_10MS);
	boost_gpio_set_gpio_switch(l_dev, BOOST_GPIO_SWITCH_ENABLE);
...
static int boost_gpio_stop_config(int flag)
{
	...
	boost_gpio_set_gpio_switch(l_dev, BOOST_GPIO_SWITCH_DISABLE);
	power_usleep(DT_USLEEP_10MS);
	boost_5v_enable(BOOST_5V_DISABLE, BOOST_CTRL_BOOST_GPIO_OTG);
	power_sw_set_output(l_dev->vusbsw_index, POWER_SW_ON);
...
static struct vbus_ch_ops boost_gpio_ops = {
	...
	.set_voltage = NULL,
	.get_voltage = NULL,
	.close_switch = boost_gpio_close_gpio_switch,
};
	...
	struct boost_gpio_dev *l_dev)
{
	...
	(void)power_dts_read_str2int(power_dts_tag(HWLOG_TAG), np,
		"vusbsw_index", &l_dev->vusbsw_index, -1);
	...
	(void)power_dts_read_u32(power_dts_tag(HWLOG_TAG), np,
		"otg_ocp_deglitch_enable", &l_dev->otg_ocp_deglitch_enable, 0);
...
static int boost_gpio_probe(struct platform_device *pdev)
{
	...
	mutex_init(&l_dev->gpio_switch_mutex);
...
fail_register_ops:
	mutex_destroy(&l_dev->gpio_switch_mutex);
