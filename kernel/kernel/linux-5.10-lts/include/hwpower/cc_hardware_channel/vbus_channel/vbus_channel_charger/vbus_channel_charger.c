...
static const char * const g_charger_otg_device_id_table[] = {
	...
	[OTG_DEVICE_ID_SCHARGERV800] = "schargerv800",
};
...
{
	int ret;
	int status = enable ? l_dev->volt : 0;

	if (l_dev->ops && l_dev->ops->otg_set_enable) {
		ret = l_dev->ops->otg_set_enable(status);
		if (ret)
...
static int charger_otg_start_config(int flag, int volt)
{
	struct charger_otg_dev *l_dev = charger_otg_get_dev();
...
static int charger_otg_open(unsigned int user, int flag)
{
	...
	if (charger_otg_start_config(flag, OTG_VOLTAGE_5V))
		return -EINVAL;
...
static int charger_otg_open_with_volt(unsigned int user, int flag, int volt)
{
	struct charger_otg_dev *l_dev = charger_otg_get_dev();
	...
	if (charger_otg_start_config(flag, volt))
		return -EINVAL;

	l_dev->user |= (1 << user);

	hwlog_info("user=%x volt=%d open ok\n", l_dev->user, volt);
	return 0;
}
...
static struct vbus_ch_ops charger_otg_ops = {
	...
	.open_with_volt = charger_otg_open_with_volt,
