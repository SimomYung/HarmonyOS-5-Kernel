...
static int bq2560x_set_otg_enable(int enable)
{
	...
	if (di->change_otg_limit_reg)
		bq2560x_write_mask(BQ2560X_REG_POC, BQ2560X_REG_POC_MIN_VBAT_SEL_MASK,
			BQ2560X_REG_POC_MIN_VBAT_SEL_SHIFT, REG01_MIN_VBAT_2P5V);
...
static void bq2560x_parse_dts(struct device_node *np, struct bq2560x_device_info *di)
{
	...
	(void)power_dts_read_u32(power_dts_tag(HWLOG_TAG), np,
		"change_otg_limit_reg", &di->change_otg_limit_reg, 0);
}
