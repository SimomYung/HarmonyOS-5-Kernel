...
struct rt9471_desc {
	...
	u32 skip_term_enable;
	u32 vrechg;
	bool skip_term_status;
	bool en_safe_tmr;
	bool en_te;
	bool en_jeita;
	bool ceb_invert;
...
static int __rt9471_set_vrechg(struct rt9471_chip *chip, u32 val)
{
	hwlog_info("%s val = %d\n", __func__, val);
	return (val ? rt9471_set_bit : rt9471_clr_bit)
		(chip, RT9471_REG_VCHG, RT9471_VRECHG_200);
}
...
static int rt9471_get_input_current_set(void)
{
	u32 aicr = 0;
	...
	(void)__rt9471_get_aicr(chip, &aicr);
	return (aicr > RT9471_AICR_MAX ? RT9471_AICR_MAX : (int)aicr);
}
...
static void rt9471_charger_parameter_parse_dt(struct device_node *np, struct rt9471_desc *desc)
{
	...
	if (of_property_read_u32(np, "skip_term_enable", &desc->skip_term_enable) < 0)
		hwlog_info("no skip_term_enable\n");

	if (of_property_read_u32(np, "vrechg", &desc->vrechg) < 0)
		hwlog_info("no vrechg\n");
...
static void rt9471_init_reg_top(struct rt9471_chip *chip)
{
	int ret;
...
static void rt9471_init_reg_ibus(struct rt9471_chip *chip)
{
	int ret;
	struct rt9471_desc *desc = chip->desc;
...
static void rt9471_init_reg_vbus(struct rt9471_chip *chip)
{
	int ret;
	struct rt9471_desc *desc = chip->desc;
...
static void rt9471_init_reg_eoc(struct rt9471_chip *chip)
{
	int ret;
	struct rt9471_desc *desc = chip->desc;
...
static int rt9471_init_setting(struct rt9471_chip *chip)
{
	...
	rt9471_init_reg_vbus(chip);
	rt9471_init_reg_ibus(chip);
	rt9471_init_reg_eoc(chip);
	rt9471_init_reg_top(chip);
	...
	ret = __rt9471_set_vrechg(chip, desc->vrechg);
	if (ret < 0)
		hwlog_err("set vrechg fail %d\n", ret);
...
static int rt9471_get_cv(void)
{
	...
	ret = __rt9471_get_cv(chip, &cv);
	if (ret)
		hwlog_err("rt9471_get_cv fail %d\n", ret);
	hwlog_info("rt9471_get_cv CV = %dmV\n", cv);
	return (int)cv;
}
...
static int rt9471_enable_te(int en)
{
	...
	if ((chip->desc != NULL) && chip->desc->skip_term_enable &&
		chip->desc->skip_term_status)
		return 0;
	if (chip->desc != NULL)
		chip->desc->skip_term_status = true;
...
static int rt9471_set_vbus_vset(u32 volt)
{
	return 0;
}
...
static struct charge_device_ops rt9471_ops = {
	...
	.set_vbus_vset = rt9471_set_vbus_vset,
	.set_term_enable = rt9471_enable_te,
	.set_watchdog_timer = rt9471_set_wdt,
	.get_charge_state = rt9471_get_charge_state,
	.get_iin_set = rt9471_get_input_current_set,
	.get_terminal_voltage = rt9471_get_cv,
	...
	const struct i2c_device_id *id)
{
	...
	chip->desc->skip_term_status = false;
	if (chip->desc->skip_term_enable)
		rt9471_enable_te(1);
