...
static void rstm32g031_fw_update_work(struct work_struct *work)
{
	struct rstm32g031_device_info *di = NULL;
	static int cnt = 0;

	di = container_of(work, struct rstm32g031_device_info, fw_upgrade_work.work);
	if (!di || !di->client)
		return;

	if (cnt < RHC32L110_FW_UPDATE_RETRY_COUNT && rhc32l110_get_firmware(di)) {
		cnt++;
		schedule_delayed_work(&di->fw_upgrade_work,
			msecs_to_jiffies(RHC32L110_FW_UPDATE_DELAY_TIME));
		return;
	}

	/* need update mtp when mtp is empty */
	if (rhc32l110_fw_update_empty_mtp(di))
		hwlog_err("rhc32l110 empty mtp update fail\n");
	/* need update mtp when mtp is latest */
	if (rhc32l110_fw_update_latest_mtp(di))
		hwlog_err("rhc32l110 fw update fail\n");
...
static void rstm32g031_enable_irq(struct rstm32g031_device_info *di)
{
	unsigned long flags;

	spin_lock_irqsave(&di->irq_status_lock, flags);
	if (!di->irq_active) {
		di->irq_active = 1;
		enable_irq(di->irq_int);
	}
	spin_unlock_irqrestore(&di->irq_status_lock, flags);
}
	...
	struct device_node *np)
{
	...
	spin_lock_init(&di->irq_status_lock);
	di->irq_active = 1;
	...
	const struct i2c_device_id *id)
{
	...
	if (rstm32g031_pre_init_check(di) && rhc32l110_fw_enter_bootloader_mode(di))
		goto fail_free_gpio;
	...
	INIT_DELAYED_WORK(&di->fw_upgrade_work, rstm32g031_fw_update_work);
	(void)schedule_delayed_work(&di->fw_upgrade_work,
		msecs_to_jiffies(RHC32L110_FW_UPDATE_DELAY_TIME));
...
fail_free_gpio:
	...
	gpio_free(di->gpio_reset);
fail_free_mem:
	if (di->param_dts.use_ldo && di->ldo_regulator && !IS_ERR(di->ldo_regulator)) {
		if (regulator_is_enabled(di->ldo_regulator))
			regulator_disable(di->ldo_regulator);
		devm_regulator_put(di->ldo_regulator);
	}
...
static int rstm32g031_remove(struct i2c_client *client)
{
	...
	cancel_delayed_work(&di->fw_upgrade_work);
	if (!di->fw_data) {
		kfree(di->fw_data);
		di->fw_data = NULL;
	}
