	...
	unsigned int num_bytes)
{
	...
	if (di->in_fw_update) {
		hwlog_info("mcu is updating fw, and write block is not allowed\n");
		return -EINVAL;
	}
	...
	buf[0] = reg;
	ret = memcpy_s(&buf[1], num_bytes, value, num_bytes);
	if (ret) {
	...
	unsigned int num_bytes)
{
	...
	if (di->in_fw_update) {
		hwlog_info("mcu is updating fw, and read block is not allowed\n");
		return -EINVAL;
	}
...
int rstm32g031_write_byte(struct rstm32g031_device_info *di, u8 reg, u8 value)
{
	...
	if (di->in_fw_update) {
		hwlog_info("mcu is updating fw, and write byte is not allowed\n");
		return -EINVAL;
	}
...
int rstm32g031_read_byte(struct rstm32g031_device_info *di, u8 reg, u8 *value)
{
	...
	if (di->in_fw_update) {
		hwlog_info("mcu is updating fw, and read byte is not allowed\n");
		return -EINVAL;
	}
	...
	u8 shift, u8 value)
{
	...
	if (di->in_fw_update) {
		hwlog_info("mcu is updating fw, and write mask is not allowed\n");
		return -EINVAL;
	}
