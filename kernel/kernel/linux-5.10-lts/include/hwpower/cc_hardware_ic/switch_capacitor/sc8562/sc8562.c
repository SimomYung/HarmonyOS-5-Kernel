...
#define SC8562_REG_DUMP_BUF_LEN  512
...
static int sc8562_get_device_id(void *dev_data)
{
	...
	if (part_info == SC8562_DEVICE_ID_SC8562) {
		...
		ret = sc8562_read_byte(di, SC8562_DEVICE_VER_REG, &device_ver);
		if (ret) {
			hwlog_err("ic_%u get_device_ver read failed\n", di->ic_role);
			return SC8562_DEVICE_ID_GET_FAIL;
		}
		if (device_ver == BK1865_DEVICE_VER)
			di->device_id = SWITCHCAP_BK1865;
		else
	...
	hwlog_info("ic_%u get_device_id [%x]=0x%x, device_id: 0x%x, device_ver: 0x%x\n",
		di->ic_role, SC8562_DEVICE_ID_REG, part_info, di->device_id, device_ver);
...
static int sc8562_get_ibat_ma(int *ibat, void *dev_data)
{
	...
	if (di->device_id == SWITCHCAP_SC8565 || di->device_id == SWITCHCAP_BK1865)
		/* IBAT ADC LBS: 3.75mA */
...
static int sc8562_set_dpdm_pre_state(struct sc8562_device_info *di, bool value)
{
	...
	if (value) {
		if (di->device_id == SWITCHCAP_BK1865) {
			ret = sc8562_write_mask(di, BK1865_DPDM_CTRL_MASK_REG, BK1865_DP_DRIVE_MASK,
				BK1865_DP_DRIVE_SHIFT, BK1865_DP_0P6V_EN);
			ret += sc8562_write_mask(di, BK1865_DPDM_CTRL2_MASK_REG,
				BK1865_DPDM_EN_MASK, BK1865_DPDM_EN_SHIFT, 1);
		} else {
		...
		if (di->device_id == SWITCHCAP_BK1865) {
			ret = sc8562_write_mask(di, BK1865_DPDM_CTRL2_MASK_REG,
				BK1865_DPDM_EN_MASK, BK1865_DPDM_EN_SHIFT, 0);
		} else {
			ret = sc8562_write_mask(di, SC8562_DPDM_CTRL1_REG,
...
static const int g_bk1865_dpdm[] = {
		[BK1865_DPDM_HIGH_THAN_0P0V]   = 0,
		[BK1865_DPDM_HIGH_THAN_0P325V] = 325,
		[BK1865_DPDM_HIGH_THAN_1P0V]   = 1000,
		[BK1865_DPDM_HIGH_THAN_1P35V]  = 1350,
		[BK1865_DPDM_HIGH_THAN_2P2V]   = 2200,
		[BK1865_DPDM_HIGH_THAN_3P0V]   = 3000,
};
...
static int sc8562_get_dpdm_state(int *dp_mv, int *dm_mv, void *dev_data)
{
	...
	if (di->device_id == SWITCHCAP_BK1865) {
		ret = sc8562_read_byte(di, BK1865_DP_STAT_MASK_REG, &data);
		dp_stat = (int)data;
		ret += sc8562_read_byte(di, BK1865_DM_STAT_MASK_REG, &data);
		dm_stat = (int)data;
		if (ret)
			return -EIO;
		sc8562_set_dpdm_pre_state(di, false);
		hwlog_info("ic_%u VDP_RD=0x%d, VDM_RD=0x%d\n", di->ic_role, dp_stat, dm_stat);
		*dp_mv = g_bk1865_dpdm[dp_stat];
		*dm_mv = g_bk1865_dpdm[dm_stat];
		return 0;
	}
...
static int sc8562_lvc_charge_init(void *dev_data)
{
	...
	ret += sc8562_config_switching_frequency(di->lvc_switching_frequency, di);
	if (ret)
		return -EIO;
...
static int sc8562_sc_charge_init(void *dev_data)
{
	...
	ret = sc8562_reg_init(di);
	ret += sc8562_config_switching_frequency(di->sc_switching_frequency, di);
	if (ret)
		return -EIO;
...
static int sc8562_sc4_charge_init(void *dev_data)
{
	...
	ret = sc8562_reg_init(di);
	ret += sc8562_config_switching_frequency(di->sc4_switching_frequency, di);
	if (ret)
		return -EIO;
...
static int sc8562_value_dump(char *buffer, int size, void *dev_data)
{
	...
	ret = snprintf_s(device_name, SC8562_BUF_LEN, SC8562_BUF_LEN - 1, "%s_%u",
		dc_get_device_name_without_mode(di->device_id), di->ic_role);
	if (ret < 0)
		return -EPERM;
	snprintf(buff, sizeof(buff), "%-10s", device_name);
	if (strncat_s(buffer, size, buff, strlen(buff)))
		return -EPERM;
...
static void sc8562_fault_event_notify_judge(struct sc8562_device_info *di, int val, int flag, void *data)
{
	switch (flag) {
	case SC8562_IRQ_VBAT_OVP:
		if (val >= SC8562_VBAT_OVP_INIT)
			sc8562_fault_event_notify(POWER_NE_DC_FAULT_VBAT_OVP, data);
		break;
	case SC8562_IRQ_IBAT_OCP:
	...
	struct nty_data *data, u8 *flag)
{
	...
	if (flag[SC8562_IRQ_FLT_FLAG] & SC8562_CONV_OCP_FLAG_MASK) {
		hwlog_info("%s ic_%u CONV OCP happened\n", __func__, di->ic_role);
		sc8562_fault_event_notify(POWER_NE_DC_FAULT_CONV_OCP, data);
	} else if (flag[SC8562_IRQ_PIN_FLAG] & SC8562_PIN_DIAG_FAIL_FLAG_MASK) {
		hwlog_info("%s ic_%u CFLY SHORT or OPEN happened\n", __func__, di->ic_role);
		sc8562_fault_event_notify(POWER_NE_DC_FAULT_CFLY_SHORT, data);
	} else if (flag[SC8562_IRQ_VUSB_OVP] & SC8562_VUSB_OVP_FLAG_MASK) {
		...
		sc8562_fault_event_notify_judge(di, val, SC8562_IRQ_VBAT_OVP, data);
		...
		sc8562_fault_event_notify_judge(di, val, SC8562_IRQ_IBAT_OCP, data);
		...
		sc8562_fault_event_notify_judge(di, val, SC8562_IRQ_FLT_FLAG, data);
		...
		sc8562_fault_event_notify_judge(di, val, SC8562_IRQ_IBUS_OCP, data);
...
static void sc8562_interrupt_work(struct work_struct *work)
{
	...
	(void)sc8562_read_byte(di, SC8562_CONVERTER_STATE_REG, &flag[SC8562_IRQ_PIN_FLAG]);
	...
	hwlog_info("ic_%u FLAG_VBAT_OVP[0x%x]=0x%x FLAG_FLT_FLAG[0x%x]=0x%x FLAG_VUSB_OVP[0x%x]=0x%x " \
		...
		"FLAG_SCP_FLAG[0x%x]=0x%x FLAG_INT_FLAG[0x%x]=0x%x SC8562_IRQ_PIN_FLAG[0x%x]=0x%x\n", di->ic_role,
		...
		SC8562_CONVERTER_STATE_REG, flag[SC8562_IRQ_PIN_FLAG]);
	...
	struct sc8562_device_info *di)
{
	(void)power_dts_read_u32(power_dts_tag(HWLOG_TAG), np,
		"lvc_switching_frequency", &di->lvc_switching_frequency,
		SC8562_SW_FREQ_MIN);
	(void)power_dts_read_u32(power_dts_tag(HWLOG_TAG), np,
		"sc_switching_frequency", &di->sc_switching_frequency,
		SC8562_SW_FREQ_MIN);
	(void)power_dts_read_u32(power_dts_tag(HWLOG_TAG), np,
		"sc4_switching_frequency", &di->sc4_switching_frequency,
		SC8562_SW_FREQ_600KHZ);
	(void)power_dts_read_u32(power_dts_tag(HWLOG_TAG), np,
	...
	(void)power_dts_read_u32(power_dts_tag(HWLOG_TAG), np,
		"sc_vusb_ovp", &di->sc_vusb_ovp, SC8562_VUSB_VWPC_OVP_F21SC_INIT);
	(void)power_dts_read_u32(power_dts_tag(HWLOG_TAG), np,
