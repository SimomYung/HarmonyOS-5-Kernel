...
static int cps4067_qi_wait_fsk_ack(struct cps4067_dev_info *di)
{
	int j;
	u8 val = 0;
	...
	for (j = 0; j < 10; j++) {
		...
		if (di->irq_val & CPS4067_RX_IRQ_FSK_PKT) {
			di->irq_val &= ~CPS4067_RX_IRQ_FSK_PKT;
			(void)cps4067_read_byte(di, CPS4067_RCVD_MSG_CMD_ADDR, &val);
			if (val == HWQI_CMD_ND)
				return 0;
		}
...
static int cps4067_qi_send_ask_msg_ack(u8 cmd, u8 *data, int data_len, void *dev_data)
{
	...
	for (i = 0; i < 2; i++) { /* retry twice */
		...
		if (ret)
			continue;
		ret = cps4067_qi_wait_fsk_ack(di);
		if (ret != CPS4067_QI_RETRY)
			return ret;
