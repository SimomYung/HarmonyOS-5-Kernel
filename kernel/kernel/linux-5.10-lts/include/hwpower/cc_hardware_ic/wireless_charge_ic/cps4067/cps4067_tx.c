...
static int cps4067_tx_set_bridge(unsigned int v_ask, unsigned int type, void *dev_data)
{
	...
	if (type == WLTX_PT_HALF_BRIDGE && !di->force_full_bridge) {
		ret += cps4067_write_dword_mask(di, CPS4067_TX_FUNC_EN_ADDR,
			CPS4067_TX_SWITCH_BRI_CTL_MASK, CPS4067_TX_SWITCH_BRI_CTL_SHIFT,
			CPS4067_TX_FUNC_EN);
		power_usleep(DT_USLEEP_5MS); /*delay 5ms */
...
static int cps4067_tx_get_curfod_iin(u16 *tx_iin, void *dev_data)
{
	return cps4067_read_word(dev_data, CPS4067_TX_CURFOD_IIN_ADDR, tx_iin);
}
...
static int cps4067_tx_set_init_para(struct cps4067_dev_info *di)
{
	...
	ret += cps4067_write_word(di, CPS4067_TX_LOW_POWER_ILIM_ADDR, di->tx_low_power_ocp_th);
	ret += cps4067_write_dword_mask(di, CPS4067_TX_FUNC_EN_ADDR,
		CPS4067_TX_SWITCH_BRI_CTL_MASK, CPS4067_TX_SWITCH_BRI_CTL_SHIFT,
		CPS4067_TX_FUNC_DIS);
	if (di->tx_opt_gain)
		ret += cps4067_write_dword_mask(di, CPS4067_TX_FUNC_EN_ADDR,
			CPS4067_TX_OPT_GAIN_EN_MASK, CPS4067_TX_OPT_GAIN_EN_SHIFT,
			CPS4067_TX_FUNC_EN);
...
static void cps4067_tx_ept_handler(struct cps4067_dev_info *di)
{
	...
	case CPS4067_TX_EPT_SRC_RPP_TIMEOUT:
		di->tx_ept_type &= ~CPS4067_TX_EPT_SRC_CEP_TIMEOUT;
...
static struct wltx_ic_ops g_cps4067_tx_ic_ops = {
	...
	.get_curfod_iin         = cps4067_tx_get_curfod_iin,
	...
	.set_ping_duty          = cps4067_tx_set_ping_duty,
};
