...
static int cps8601_tx_set_para_for_kb(struct cps8601_dev_info *di)
{
	int ret;

	ret = cps8601_write_word(di, CPS8601_TX_PLOSS_TH0_ADDR, di->tx_fod.ploss_th0_kb);
	ret += cps8601_write_word(di, CPS8601_TX_PLOSS_TH1_ADDR, di->tx_fod.ploss_th1_kb);
	ret += cps8601_write_word(di, CPS8601_TX_PLOSS_TH2_ADDR, di->tx_fod.ploss_th2_kb);
	...
	if (di->tx_limit_power_en) {
		ret += cps8601_write_byte(di, CPS8601_TX_LIMIT_POWER_CTL_ADDR, CPS8601_TX_LIMIT_POWER_EN);
		ret += cps8601_write_word(di, CPS8601_TX_CURR_MAX_ADDR, di->tx_fod.tx_curr_max_kb);
		ret += cps8601_write_byte(di, CPS8601_TX_CURR_STEP_ADDR, di->tx_fod.curr_step_kb);
		ret += cps8601_write_byte(di, CPS8601_TX_CURR_CNT_MAX_ADDR, di->tx_fod.curr_cnt_max);
		ret += cps8601_write_word(di, CPS8601_TX_PLOSS_DIFF_MIN_ADDR, di->tx_fod.ploss_diff_min_kb);
	}

	return ret;
}

static int cps8601_tx_set_para_for_pen(struct cps8601_dev_info *di)
{
	int ret;
	...
	if (di->tx_limit_power_en) {
		ret += cps8601_write_byte(di, CPS8601_TX_LIMIT_POWER_CTL_ADDR, CPS8601_TX_LIMIT_POWER_EN);
		ret += cps8601_write_byte(di, CPS8601_TX_CURR_CNT_MAX_ADDR, di->tx_fod.curr_cnt_max);
		ret += cps8601_write_word(di, CPS8601_TX_CURR_MAX_ADDR, di->tx_fod.tx_curr_max);
		ret += cps8601_write_byte(di, CPS8601_TX_CURR_STEP_ADDR, di->tx_fod.curr_step);
		ret += cps8601_write_word(di, CPS8601_TX_PLOSS_DIFF_MIN_ADDR, di->tx_fod.ploss_diff_min);
	}
...
static int cps8601_tx_init_fod_coef(struct cps8601_dev_info *di)
{
	...
	if (di->rx_type == WLTX_AUX_OPEN_BY_KB) {
		hwlog_info("[init_fod_coef] kb\n");
		ret = cps8601_tx_set_para_for_kb(di);
	} else {
		hwlog_info("[init_fod_coef] pen\n");
		ret = cps8601_tx_set_para_for_pen(di);
	}
	if (ret) {
...
static void cps8601_tx_report_calibration_info(struct cps8601_dev_info *di)
{
	struct hiview_info info[CPS8601_Q_HIVIEW_MAX];
	char hiview_str[CPS8601_Q_HIVIEW_MAX][POWER_DSM_BUF_SIZE_0016] = { 0 };
	u16 hiview_value[CPS8601_Q_HIVIEW_MAX];
	int i;
	int ret;

	info[CPS8601_Q_HIVIEW_FAC_WIDTH].key = "FACTORYWIDTH";
	info[CPS8601_Q_HIVIEW_DYN_WIDTH].key = "DYNAMICWIDTH";
	info[CPS8601_Q_HIVIEW_SPL_WIDTH].key = "SAMPLEWIDTH";
	info[CPS8601_Q_HIVIEW_FLAG].key = "CALIBRATEFLAG";

	hiview_value[CPS8601_Q_HIVIEW_FAC_WIDTH] = di->tx_q_factor.tx_q_fac_width;
	hiview_value[CPS8601_Q_HIVIEW_DYN_WIDTH] = di->tx_q_factor.tx_q_dyn_width;
	hiview_value[CPS8601_Q_HIVIEW_SPL_WIDTH] = di->tx_q_factor.tx_q_spl_width;
	hiview_value[CPS8601_Q_HIVIEW_FLAG] = di->tx_q_factor.tx_q_flag;
	for (i = 0; i < CPS8601_Q_HIVIEW_MAX; i++) {
		ret = snprintf_s(hiview_str[i], POWER_DSM_BUF_SIZE_0016,
			POWER_DSM_BUF_SIZE_0016 - 1, "%u", hiview_value[i]);
		if (ret < 0) {
			hwlog_err("report_calibration_info: snprintf_s failed\n");
			return;
		}
		info[i].value = hiview_str[i];
	}

	power_dsm_report_multi_hiview(SMARTCHARGE_Q_OFFSET_WLC, info, CPS8601_Q_HIVIEW_MAX);
}
...
static void cps8601_tx_q_cali_handler(struct cps8601_dev_info *di)
{
	...
	if (((di->tx_q_factor.tx_q_fac_width < CPS8601_Q_FACTORY_WIDTH_LTH) ||
		...
		if ((di->tx_q_factor.tx_q_width >= di->q_sample_width_lth) &&
			(di->tx_q_factor.tx_q_width <= di->q_sample_width_hth)) {
		...
		if (di->q_cali_dynamic_en &&
			(di->tx_q_factor.tx_q_width >= di->q_sample_width_lth) &&
			(di->tx_q_factor.tx_q_width <= di->q_sample_width_hth) &&
	...
	cps8601_tx_report_calibration_info(di);
}
...
static void cps8601_tx_get_fod_gap_handler(struct cps8601_dev_info *di)
{
	int ret;
	u16 fod_gap = 0;

	ret = cps8601_read_word(di, CPS8601_TX_FOD_GAP_ADDR, &fod_gap);
	if (ret) {
		hwlog_err("get fod gap fail\n");
		return;
	}
	di->fod_gap = fod_gap;

	hwlog_info("fod_gap=%d\n", di->fod_gap);
}
...
void cps8601_tx_mode_irq_handler(struct cps8601_dev_info *di)
{
	...
	if (di->irq_val & CPS8601_TX_IRQ_FOD_GAP) {
		di->irq_val &= ~CPS8601_TX_IRQ_FOD_GAP;
		cps8601_tx_get_fod_gap_handler(di);
	}
...
static int cps8601_tx_switch_modem(void *dev_data)
{
	int ret;
	struct cps8601_dev_info *di = dev_data;
	...
	ret = cps8601_write_word_mask(di, CPS8601_TX_CMD_ADDR, CPS8601_TX_CMD_SWITCH_MODEM,
		CPS8601_TX_CMD_SWITCH_MODEM_SHIFT, CPS8601_TX_CMD_VAL);
	if (ret) {
		hwlog_err("switch modem fail\n");
		return ret;
	}

	hwlog_info("switch 13/14 modem\n");
	return 0;
}

 static int cps8601_tx_set_rx_type(void *dev_data, int type)
{
	struct cps8601_dev_info *di = dev_data;
...
static int cps8601_tx_get_fod_gap_state(void *dev_data, u16 *fod_gap_state)
{
	struct cps8601_dev_info *di = dev_data;
	...
	*fod_gap_state = CPS8601_FOD_GAP_FAIL;
	if (di->fod_gap > di->tx_fod_gap_th)
		*fod_gap_state = CPS8601_FOD_GAP_SUCC;
...
static struct wltx_ic_ops g_cps8601_tx_ops = {
	...
	.switch_modem           = cps8601_tx_switch_modem,
	.set_rx_type            = cps8601_tx_set_rx_type,
	.get_fod_gap_state      = cps8601_tx_get_fod_gap_state,
};
