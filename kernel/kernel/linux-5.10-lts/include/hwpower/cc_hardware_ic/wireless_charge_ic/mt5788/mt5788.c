...
#define HWLOG_TAG wireless_mt5788
HWLOG_REGIST();

#define MT5788_PINCTRL_LEN                   2
...
bool mt5788_is_pwr_good(struct mt5788_dev_info *di)
{
	...
	if (gpio_val == MT5788_GPIO_PWR_GOOD_VAL)
		return true;
...
static int mt5788_i2c_read(struct mt5788_dev_info *di, u8 *cmd, int cmd_len,
	u8 *buf, int buf_len)
{
	int i;
...
static int mt5788_i2c_write(struct mt5788_dev_info *di, u8 *buf, int buf_len)
{
	int i;
...
int mt5788_read_block(struct mt5788_dev_info *di, u16 reg, u8 *data, u8 len)
{
	...
	return mt5788_i2c_read(di, cmd, MT5788_ADDR_LEN, data, len);
}
...
int mt5788_write_block(struct mt5788_dev_info *di, u16 reg, u8 *data, u8 data_len)
{
	...
	cmd = kzalloc(sizeof(u8) * (MT5788_ADDR_LEN + data_len), GFP_KERNEL);
	if (!cmd)
		return -ENOMEM;

	cmd[0] = reg >> POWER_BITS_PER_BYTE;
	cmd[1] = reg & POWER_MASK_BYTE;
	(void)memcpy_s(&cmd[MT5788_ADDR_LEN], data_len, data, data_len);
	ret = mt5788_i2c_write(di, cmd, MT5788_ADDR_LEN + data_len);
	kfree(cmd);
	return ret;
}
...
{
	return mt5788_read_block(di, reg, data, POWER_BYTE_LEN);
}
...
int mt5788_read_word(struct mt5788_dev_info *di, u16 reg, u16 *data)
{
	...
	if (!data || mt5788_read_block(di, reg, buff, POWER_WORD_LEN))
		return -EIO;
...
int mt5788_read_dword(struct mt5788_dev_info *di, u16 reg, u32 *data)
{
	...
	if (!data || mt5788_read_block(di, reg, buff, POWER_DWORD_LEN))
		return -EIO;
...
{
	return mt5788_write_block(di, reg, &data, POWER_BYTE_LEN);
}

int mt5788_write_word(struct mt5788_dev_info *di, u16 reg, u16 data)
{
	return mt5788_write_block(di, reg, (u8 *)&data, POWER_WORD_LEN);
}

int mt5788_write_dword(struct mt5788_dev_info *di, u16 reg, u32 data)
{
	return mt5788_write_block(di, reg, (u8 *)&data, POWER_DWORD_LEN);
}
...
int mt5788_read_byte_mask(struct mt5788_dev_info *di, u16 reg, u8 mask, u8 shift, u8 *data)
{
	...
	ret = mt5788_read_byte(di, reg, &val);
	if (ret)
		return ret;
...
int mt5788_write_byte_mask(struct mt5788_dev_info *di, u16 reg, u8 mask, u8 shift, u8 data)
{
	...
	return mt5788_write_byte(di, reg, val);
}
...
int mt5788_write_word_mask(struct mt5788_dev_info *di, u16 reg, u16 mask, u16 shift, u16 data)
{
	...
	ret = mt5788_read_word(di, reg, &val);
	if (ret)
		return ret;
	...
	return mt5788_write_word(di, reg, val);
}
...
int mt5788_read_dword_mask(struct mt5788_dev_info *di, u16 reg, u32 mask, u32 shift, u32 *data)
{
	...
	ret = mt5788_read_dword(di, reg, &val);
	if (ret)
		return ret;
...
int mt5788_write_dword_mask(struct mt5788_dev_info *di, u16 reg, u32 mask, u32 shift, u32 data)
{
	...
	return mt5788_write_dword(di, reg, val);
}

int mt5788_get_chip_id(struct mt5788_dev_info *di, u16 *chip_id)
{
	return mt5788_read_word(di, MT5788_CHIP_ID_ADDR, chip_id);
}
...
int mt5788_get_chip_info(struct mt5788_dev_info *di, struct mt5788_chip_info *info)
{
	...
	ret = mt5788_get_chip_id(di, &info->chip_id);
	ret += mt5788_fw_get_mtp_ver(di, &info->mtp_ver);
	if (ret) {
...
int mt5788_get_chip_info_str(char *info_str, int len, void *dev_data)
{
	...
	ret = mt5788_get_chip_info(dev_data, &info);
	if (ret)
		return -EIO;
...
int mt5788_get_mode(struct mt5788_dev_info *di, u32 *mode)
{
	...
	return mt5788_read_dword(di, MT5788_RX_SYS_MODE_ADDR, mode);
}
...
static void mt5788_irq_work(struct work_struct *work)
{
	int ret;
	int gpio_val;
	u32 mode = 0;
	struct mt5788_dev_info *di = container_of(work, struct mt5788_dev_info, irq_work);
	...
	if (mode & MT5788_RX_SYS_MODE_RX)
		mt5788_rx_mode_irq_handler(di);
	else
		mt5788_tx_mode_irq_handler(di);
exit:
...
static irqreturn_t mt5788_interrupt(int irq, void *_di)
{
	struct mt5788_dev_info *di = _di;
...
static void mt5788_register_pwr_dev_info(struct mt5788_dev_info *di)
{
...
static void mt5788_ops_unregister(struct wltrx_ic_ops *ops)
{
	if (!ops)
		return;
...
static int mt5788_ops_register(struct wltrx_ic_ops *ops, struct mt5788_dev_info *di)
{
	int ret;

	ret = mt5788_fw_ops_register(ops, di);
	if (ret) {
		hwlog_err("ops_register: register fw_ops failed\n");
		return ret;
	}

	ret = mt5788_rx_ops_register(ops, di);
	if (ret) {
		hwlog_err("ops_register: register rx_ops failed\n");
		return ret;
	}

	ret = mt5788_tx_ops_register(ops, di);
	if (ret) {
		hwlog_err("ops_register: register tx_ops failed\n");
		return ret;
	}

	ret = mt5788_qi_ops_register(ops, di);
	if (ret) {
	...
	ret = mt5788_hw_test_ops_register(ops, di);
	if (ret) {
...
static int mt5788_gpio_init(struct mt5788_dev_info *di, struct device_node *np)
{
	if (power_gpio_config_output(np, "gpio_en", "mt5788_en",
		&di->gpio_en, di->gpio_en_valid_val))
		goto gpio_en_fail;

	if (power_gpio_config_output(np, "gpio_sleep_en", "mt5788_sleep_en",
		&di->gpio_sleep_en, WLTRX_IC_SLEEP_DISABLE))
		goto gpio_sleep_en_fail;

	if (power_gpio_config_input(np, "gpio_pwr_good", "mt5788_pwr_good",
		&di->gpio_pwr_good))
...
static int mt5788_irq_init(struct mt5788_dev_info *di,
	struct device_node *np)
{
	INIT_WORK(&di->irq_work, mt5788_irq_work);

	if (power_gpio_config_interrupt(np, "gpio_int", "mt5788_int",
		&di->gpio_int, &di->irq_int))
		return -EINVAL;

	if (request_irq(di->irq_int, mt5788_interrupt,
		IRQF_TRIGGER_FALLING, "mt5788_irq", di)) {
...
static void mt5788_free_dev_resources(struct mt5788_dev_info *di)
{
...
static int mt5788_request_dev_resources(struct mt5788_dev_info *di, struct device_node *np)
{
	int ret;

	ret = mt5788_parse_dts(np, di);
	if (ret)
		goto parse_dts_fail;

	ret = power_pinctrl_config(di->dev, "pinctrl-names", MT5788_PINCTRL_LEN);
	if (ret)
		return ret;
	ret = mt5788_gpio_init(di, np);
	if (ret)
		goto gpio_init_fail;
	ret = mt5788_irq_init(di, np);
	if (ret)
		goto irq_init_fail;
	di->wakelock = power_wakeup_source_register(di->dev, "mt5788_wakelock");
...
static int mt5788_dev_check(struct mt5788_dev_info *di)
{
	int ret;
	u16 chip_id = 0;
...
static int mt5788_probe(struct i2c_client *client,
	const struct i2c_device_id *id)
{
	int ret;
	struct mt5788_dev_info *di = NULL;
	...
	ret = mt5788_dev_check(di);
	if (ret) {
		ret = -EPROBE_DEFER;
		goto dev_ck_fail;
	}

	ret = mt5788_request_dev_resources(di, np);
	if (ret)
		goto req_dev_res_fail;

	ret = mt5788_ops_register(ops, di);
	if (ret)
		goto ops_regist_fail;

	wlic_iout_init(di->ic_type, np, NULL);
	mt5788_fw_mtp_check(di);
	mt5788_rx_probe_check_tx_exist(di);
	mt5788_register_pwr_dev_info(di);

	hwlog_info("[probe] ok\n");
	return 0;

ops_regist_fail:
	mt5788_free_dev_resources(di);
req_dev_res_fail:
dev_ck_fail:
	mt5788_ops_unregister(ops);
	devm_kfree(&client->dev, di);
	return ret;
}

static void mt5788_shutdown(struct i2c_client *client)
{
	struct mt5788_dev_info *di = i2c_get_clientdata(client);

	if (!di)
		return;

	mt5788_rx_shutdown_handler(di);
}

static const struct of_device_id mt5788_of_match[] = {
	{
		.compatible = "mt,mt5788",
	},
	{},
};
MODULE_DEVICE_TABLE(of, mt5788_of_match);

static const struct i2c_device_id mt5788_i2c_id[] = {
	{ "mt5788", WLTRX_IC_MAIN }, {}
};
MODULE_DEVICE_TABLE(i2c, mt5788_i2c_id);

static struct i2c_driver mt5788_driver = {
	.probe = mt5788_probe,
	.shutdown = mt5788_shutdown,
	.id_table = mt5788_i2c_id,
	.driver = {
		.owner = THIS_MODULE,
		.name = "wireless_mt5788",
		.of_match_table = of_match_ptr(mt5788_of_match),
	},
};

static int __init mt5788_init(void)
{
	return i2c_add_driver(&mt5788_driver);
}

static void __exit mt5788_exit(void)
{
	i2c_del_driver(&mt5788_driver);
}

device_initcall(mt5788_init);
module_exit(mt5788_exit);

MODULE_LICENSE("GPL v2");
MODULE_DESCRIPTION("mt5788 module driver");
