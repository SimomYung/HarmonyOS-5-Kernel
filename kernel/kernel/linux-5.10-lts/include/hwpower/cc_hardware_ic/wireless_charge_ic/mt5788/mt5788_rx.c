...
static int mt5788_rx_get_prx(int *prx, void *dev_data)
{
	return mt5788_read_word(dev_data, MT5788_RX_PRX_ADDR, (u16 *)prx);
}
...
static int mt5788_rx_chip_init(unsigned int init_type, unsigned int tx_type, void *dev_data)
{
	...
	(void)mt5788_write_word(di, MT5788_RX_ILOAD_ASK_TH_ADDR,
		di->rx_iload_ask_th);
	(void)mt5788_write_word(di, MT5788_RX_ILOAD_NON_ASK_TH_ADDR,
		di->rx_iload_non_ask_th);
...
static ssize_t mt5788_rx_cm_store(void *dev_data, const char *buf, size_t size)
{
	int tmp[MT5788_RX_CM_LEN] = { 0 };
	struct mt5788_dev_info *di = dev_data;
	...
	/* 0~2 index */
	if (sscanf_s(buf, "%x %x %x", &tmp[0], &tmp[1], &tmp[2]) != MT5788_RX_CM_LEN) {
...
static ssize_t mt5788_rx_cm_show(void *dev_data, char *buf, size_t size)
{
	struct mt5788_dev_info *di = dev_data;
...
static ssize_t mt5788_rx_dummy_iload_store(void *dev_data, const char *buf, size_t size)
{
	u32 tmp[MT5788_DUMMY_ILOAD_END] = { 0 };
	struct mt5788_dev_info *di = dev_data;
	...
	if (sscanf_s(buf, "%u %u %u %u %u %u %u %u",
		&tmp[MT5788_DUMMY_ILOAD_5V_NO_MOD], &tmp[MT5788_DUMMY_ILOAD_5V_MOD],
		&tmp[MT5788_DUMMY_ILOAD_9V_NO_MOD], &tmp[MT5788_DUMMY_ILOAD_9V_MOD],
		&tmp[MT5788_DUMMY_ILOAD_SC_NO_MOD], &tmp[MT5788_DUMMY_ILOAD_SC_MOD],
		&tmp[MT5788_DUMMY_ILOAD_FODCHK_NO_MOD], &tmp[MT5788_DUMMY_ILOAD_FODCHK_MOD])
		!= MT5788_DUMMY_ILOAD_END) {
		hwlog_err("dummy_iload_store: unable to parse input %s\n", buf);
		return -EINVAL;
	}

	if (memcpy_s(di->dummy_iload, sizeof(di->dummy_iload), tmp, sizeof(tmp)) != EOK)
		hwlog_err("[dummy_iload_store] copy failed\n");

	return (ssize_t)size;
}

static ssize_t mt5788_rx_dummy_iload_show(void *dev_data, char *buf, size_t size)
{
	struct mt5788_dev_info *di = dev_data;
	...
	return snprintf_s(buf, size, size - 1, "dummy_iload = %u %u %u %u %u %u %u %u\n",
		di->dummy_iload[MT5788_DUMMY_ILOAD_5V_NO_MOD], di->dummy_iload[MT5788_DUMMY_ILOAD_5V_MOD],
		di->dummy_iload[MT5788_DUMMY_ILOAD_9V_NO_MOD], di->dummy_iload[MT5788_DUMMY_ILOAD_9V_MOD],
		di->dummy_iload[MT5788_DUMMY_ILOAD_SC_NO_MOD], di->dummy_iload[MT5788_DUMMY_ILOAD_SC_MOD],
		di->dummy_iload[MT5788_DUMMY_ILOAD_FODCHK_NO_MOD], di->dummy_iload[MT5788_DUMMY_ILOAD_FODCHK_MOD]);
}

static void mt5788_rx_debug_register(struct mt5788_dev_info *di)
{
	...
	power_dbg_ops_register(dir_name, "cm_cfg", di, mt5788_rx_cm_show, mt5788_rx_cm_store);
	power_dbg_ops_register(dir_name, "dummy_iload_cfg", di, mt5788_rx_dummy_iload_show,
		mt5788_rx_dummy_iload_store);
}
