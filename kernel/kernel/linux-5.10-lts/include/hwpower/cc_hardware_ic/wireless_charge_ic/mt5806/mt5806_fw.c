...
static u16 mt5806_fw_crc_cal(const u8 *start, u16 len)
{
	u16 i, j;
	u16 crc_in = MT5806_FW_CRC_INIT;
	u16 crc_poly = MT5806_FW_CRC_SEED;

	for (i = 0; i < len; i++) {
		crc_in ^= (*(start + i) << POWER_BITS_PER_BYTE);
		for (j = 0; j < POWER_BITS_PER_BYTE; j++) {
			if (crc_in & MT5806_FW_CRC_HIGHEST_BIT)
...
static int mt5806_fw_load_q_threshold(struct mt5806_dev_info *di)
{
	int ret;

	if ((di->default_psy_type != MT5806_DEFAULT_LOWPOWER) || !di->tx_q_threshold_en)
		return 0;

	di->tx_q_th.tx_q_threshold.tx_q_crc = mt5806_fw_crc_cal(di->tx_q_th.tx_q_config,
		MT5806_Q_THRESHOLD_LEN - MT5806_Q_THRESHOLD_CRC_LEN);
	hwlog_info("[load_q_threshold] cnt: %d ~ %d, width: %d ~ %d, crc: 0x%x\n",
	...
	ret = mt5806_mtp_load_fw(di, MT5806_BOOT_Q_THRESHOLD_ADDR,
		di->tx_q_th.tx_q_config, MT5806_Q_THRESHOLD_LEN);
	if (ret) {
		hwlog_err("load_q_threshold: load q threshold failed\n");
		return ret;
	}
...
static int mt5806_fw_load_mtp(struct mt5806_dev_info *di)
{
	...
	ret = mt5806_fw_load_q_threshold(di);
	if (ret) {
		hwlog_err("fw_load_mtp: load fw q_threshold failed\n");
		goto exit;
	}
...
static int mt5806_check_q_threshold(struct mt5806_dev_info *di)
{
	int ret;
	union mt5806_tx_q_threshold_data data = { 0 };

	if ((di->default_psy_type != MT5806_DEFAULT_LOWPOWER) || !di->tx_q_threshold_en)
		return 0;

	ret = mt5806_read_block(di, MT5806_TX_Q_THRESHOLD_ADDR, data.tx_q_config, MT5806_TX_Q_THRESHOLD_LEN);
	if (ret) {
		hwlog_err("get_q_threshold: i2c failed\n");
		return -EINVAL;
	}

	hwlog_info("[get_q_threshold] cnt: %d ~ %d, width: %d ~ %d, crc: 0x%x\n",
		data.tx_q_threshold.tx_q_cnt_low, data.tx_q_threshold.tx_q_cnt_high,
		data.tx_q_threshold.tx_q_width_low, data.tx_q_threshold.tx_q_width_high,
		data.tx_q_threshold.tx_q_crc);

	if ((data.tx_q_threshold.tx_q_cnt_low == di->tx_q_th.tx_q_threshold.tx_q_cnt_low) &&
		(data.tx_q_threshold.tx_q_cnt_high == di->tx_q_th.tx_q_threshold.tx_q_cnt_high) &&
		(data.tx_q_threshold.tx_q_width_low == di->tx_q_th.tx_q_threshold.tx_q_width_low) &&
		(data.tx_q_threshold.tx_q_width_high == di->tx_q_th.tx_q_threshold.tx_q_width_high))
		return 0;
...
static int mt5806_fw_program_mtp(struct mt5806_dev_info *di)
{
	...
	ret = mt5806_check_q_threshold(di);
	if (ret) {
		hwlog_err("program_mtp: q_threshold mismatch\n");
		goto exit;
	}
