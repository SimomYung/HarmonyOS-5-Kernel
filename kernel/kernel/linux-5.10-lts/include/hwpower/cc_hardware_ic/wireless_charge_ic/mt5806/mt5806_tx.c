...
static int mt5806_tx_set_para_for_kb(struct mt5806_dev_info *di)
{
	int ret;

	ret = mt5806_write_word(di, MT5806_TX_PLOSS_TH0_ADDR, di->tx_fod.ploss_th0_kb);
	ret += mt5806_write_word(di, MT5806_TX_PLOSS_TH1_ADDR, di->tx_fod.ploss_th1_kb);
	ret += mt5806_write_word(di, MT5806_TX_PLOSS_TH2_ADDR, di->tx_fod.ploss_th2_kb);
	...
	ret += mt5806_write_word(di, MT5806_TX_PLOSS_RPP_TH1_ADDR, di->tx_fod.ploss_rpp_th1);
	if (di->tx_limit_power_en) {
		ret += mt5806_write_byte(di, MT5806_TX_LIMIT_POWER_CTL_ADDR, MT5806_TX_LIMIT_POWER_EN);
		ret += mt5806_write_word(di, MT5806_TX_CURR_MAX_ADDR, di->tx_fod.tx_curr_max_kb);
		ret += mt5806_write_word(di, MT5806_TX_CURR_STEP_ADDR, di->tx_fod.curr_step_kb);
		ret += mt5806_write_word(di, MT5806_TX_CURR_CNT_MAX_ADDR, di->tx_fod.curr_cnt_max);
		ret += mt5806_write_word(di, MT5806_TX_PLOSS_DIFF_MIN_ADDR, di->tx_fod.ploss_diff_min_kb);
	}

	return ret;
}

static int mt5806_tx_set_para_for_pen(struct mt5806_dev_info *di)
{
	int ret;
	...
	if (di->tx_limit_power_en) {
		ret += mt5806_write_byte(di, MT5806_TX_LIMIT_POWER_CTL_ADDR, MT5806_TX_LIMIT_POWER_EN);
		ret += mt5806_write_word(di, MT5806_TX_CURR_CNT_MAX_ADDR, di->tx_fod.curr_cnt_max);
		ret += mt5806_write_word(di, MT5806_TX_CURR_MAX_ADDR, di->tx_fod.tx_curr_max);
		ret += mt5806_write_word(di, MT5806_TX_CURR_STEP_ADDR, di->tx_fod.curr_step);
		ret += mt5806_write_word(di, MT5806_TX_PLOSS_DIFF_MIN_ADDR, di->tx_fod.ploss_diff_min);
	}
...
static int mt5806_tx_init_fod_coef(struct mt5806_dev_info *di)
{
	...
	if (di->rx_type == WLTX_AUX_OPEN_BY_KB) {
		...
		ret = mt5806_tx_set_para_for_kb(di);
	} else {
		hwlog_info("[init_fod_coef] pen\n");
		ret = mt5806_tx_set_para_for_pen(di);
	}
	if (ret) {
...
static void mt5806_tx_report_calibration_info(struct mt5806_dev_info *di)
{
	struct hiview_info info[MT5806_Q_HIVIEW_MAX];
	char hiview_str[MT5806_Q_HIVIEW_MAX][POWER_DSM_BUF_SIZE_0016] = { 0 };
	u16 hiview_value[MT5806_Q_HIVIEW_MAX];
	int i;
	int ret;

	info[MT5806_Q_HIVIEW_FAC_WIDTH].key = "FACTORYWIDTH";
	info[MT5806_Q_HIVIEW_DYN_WIDTH].key = "DYNAMICWIDTH";
	info[MT5806_Q_HIVIEW_SPL_WIDTH].key = "SAMPLEWIDTH";
	info[MT5806_Q_HIVIEW_FLAG].key = "CALIBRATEFLAG";

	hiview_value[MT5806_Q_HIVIEW_FAC_WIDTH] = di->tx_q_factor.tx_q_fac_width;
	hiview_value[MT5806_Q_HIVIEW_DYN_WIDTH] = di->tx_q_factor.tx_q_dyn_width;
	hiview_value[MT5806_Q_HIVIEW_SPL_WIDTH] = di->tx_q_factor.tx_q_spl_width;
	hiview_value[MT5806_Q_HIVIEW_FLAG] = di->tx_q_factor.tx_q_flag;
	for (i = 0; i < MT5806_Q_HIVIEW_MAX; i++) {
	...
	power_dsm_report_multi_hiview(SMARTCHARGE_Q_OFFSET_WLC, info, MT5806_Q_HIVIEW_MAX);
}
...
static void mt5806_tx_q_cali_handler(struct mt5806_dev_info *di)
{
	...
	mt5806_tx_report_calibration_info(di);
}
...
static int mt5806_tx_switch_modem(void *dev_data)
{
	int ret;
	struct mt5806_dev_info *di = dev_data;
	...
	ret = mt5806_write_word(di, MT5806_TX_SWITCH_MODEM_ADDR, MT5806_TX_SWITCH_MODEM);
	if (ret) {
		hwlog_err("switch_modem: write failed\n");
		return ret;
	}
...
static void mt5806_tx_get_fod_gap_handler(struct mt5806_dev_info *di)
{
	int ret;
	u16 fod_gap = 0;

	ret = mt5806_read_word(di, MT5806_TX_FOD_GAP_ADDR, &fod_gap);
	if (ret) {
...
void mt5806_tx_mode_irq_handler(struct mt5806_dev_info *di)
{
	...
	if (di->irq_val & MT5806_TX_IRQ_FOD_GAP_RCVD) {
		di->irq_val &= ~MT5806_TX_IRQ_FOD_GAP_RCVD;
		mt5806_tx_get_fod_gap_handler(di);
	}
...
static int mt5806_set_rx_type(void *dev_data, int type)
{
	struct mt5806_dev_info *di = dev_data;
...
static int mt5806_tx_get_fod_gap_state(void *dev_data, u16 *fod_gap_state)
{
	struct mt5806_dev_info *di = dev_data;
	...
	*fod_gap_state = MT5806_FOD_GAP_FAIL;
	if (di->fod_gap > di->tx_fod_gap_th)
		*fod_gap_state = MT5806_FOD_GAP_SUCC;
...
static struct wltx_ic_ops g_mt5806_tx_ic_ops = {
	...
	.switch_modem           = mt5806_tx_switch_modem,
	.set_rx_type            = mt5806_set_rx_type,
	.get_fod_gap_state      = mt5806_tx_get_fod_gap_state,
};
