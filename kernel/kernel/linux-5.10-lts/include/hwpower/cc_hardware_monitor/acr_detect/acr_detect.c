...
static int acr_det_start_detect(struct acr_det_dev *l_dev)
{
	int ret;
	int acr_value = 0;
	bool is_eis_replacing_acr = l_dev->is_eis_replacing_acr;

	/* first: calculate acr */
	power_platform_start_acr_calc(is_eis_replacing_acr);

	/* second: sleep 1000ms, wait for acr calculate finish */
	msleep(1000);

	/* third: get acr resistance */
	ret = power_platform_get_acr_resistance(&acr_value, is_eis_replacing_acr);
	if (ret == 0) {
...
{
	l_dev->is_eis_replacing_acr = of_property_read_bool(np, "eis_replacing_acr");
