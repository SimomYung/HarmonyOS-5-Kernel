...
#define LPM_VBAT_VALUE_MIN           2000
#define LPM_UNDER_VOL_MAX_TIMES      3
...
{
	if (bst_flag) {
		buckboost_set_enable(LPM_ENABLE, BBST_USER_LPM, BBST_INDEX_DEFAULT);
		buckboost_set_vout(l_dev->lpm_bbst_vout[LPM_BBST_ENTER_VOL],
			BBST_USER_LPM, BBST_INDEX_DEFAULT);
		...
		(void)power_msleep(l_dev->ecm.delay_time, 0, NULL);
		buckboost_set_vout(l_dev->lpm_bbst_vout[LPM_BBST_EXIT_VOL],
			BBST_USER_LPM, BBST_INDEX_DEFAULT);
		buckboost_set_enable(LPM_DISABLE, BBST_USER_LPM, BBST_INDEX_DEFAULT);
	}
}
...
static void low_power_boost_vsys_enable(struct low_power_dev *l_dev, bool enable)
{
	...
	hwlog_info("[boost_vsys_enable] enable:%d\n", enable);
...
static bool low_power_is_night_time(void)
{
	/* night time: 22:00-4:00 */
	return power_is_within_time_interval(22, 0, 4, 0);
}

static bool low_power_need_skip_ecm(struct low_power_dev *l_dev)
{
	int ibat_dischg;

	if (!l_dev->night_ecm_shutdown_check)
		return false;

	ibat_dischg = -charge_get_battery_current_avg();
	if (low_power_is_night_time() && !l_dev->shutdown_status && (ibat_dischg < LPM_SHUTDOWN_CUR_TH) && (l_dev->screen_state == LPM_SCREEN_OFF)) {
		hwlog_info("device is shutdown, shutdown_status=%d, ibat_dischg=%d\n", l_dev->shutdown_status, ibat_dischg);
		return true;
	}
...
static void low_power_ecm_status_check(struct low_power_dev *l_dev)
{
	...
	need_shutdown = low_power_need_skip_ecm(l_dev);
	if (need_shutdown) {
	...
	if ((vbat >= LPM_VBAT_VALUE_MIN) &&
		((vbat <= shutdown_th) || ((vbat <= l_dev->ecm.vbat_gsm)
		&& (l_dev->ecm.gsm_status != ECM_GSM_IDLE)))) {
		low_vol_cnt++;
		if (low_vol_cnt >= LPM_UNDER_VOL_MAX_TIMES) {
			low_vol_cnt = 0;
			l_dev->ecm.bst_vsys = false;
			l_dev->ecm.event_type = LPM_EVENT_UNDER_VOLT;
		}
	...
	if ((vbat >= LPM_VBAT_VALUE_MIN) && (vbat > l_dev->ecm.vbat_bst_th)) {
		l_dev->ecm.bst_vsys = false;
		return;
	}
...
{
	if (l_dev->support_ecm && (l_dev->ecm.trigger_status != ECM_TRIGGER_IDLE))
		l_dev->work_interval = (l_dev->boost_type == LPM_BST_TYPE_NOOP) ?
			LPM_NOOP_WORK_INTERVAL : LPM_ECM_WORK_INTERVAL;
	else
	...
	unsigned long event, void *data)
{
	...
	case POWER_NE_PANEL_UNBLANK:
		...
		if (l_dev->soc_battery <= LPM_SHUTDOWN_SOC_TH)
			l_dev->shutdown_status = true;
...
static int low_power_soc_battery_notifier_call(struct notifier_block *nb,
	unsigned long event, void *data)
{
	struct low_power_dev *l_dev = low_power_get_dev();
	...
	if (!strcmp(psy->desc->name, POWER_PLATFORM_BAT_PSY_NAME)) {
		power_supply_get_int_property_value(POWER_PLATFORM_BAT_PSY_NAME,
			POWER_SUPPLY_PROP_CAPACITY, &l_dev->soc_battery);
		hwlog_info("psy %s is target, soc_battery=%d\n", POWER_PLATFORM_BAT_PSY_NAME, l_dev->soc_battery);
	}

	if (l_dev->soc_battery > LPM_SHUTDOWN_SOC_TH)
		l_dev->shutdown_status = false;
...
static struct power_sysfs_attr_info low_power_sysfs_field_tbl[] = {
	...
	power_sysfs_attr_ro(low_power, 0440, LPM_SYSFS_ECM_STATUS, ecm_status),
};
	...
	struct device_attribute *attr, const char *buf, size_t count)
{
	...
	case LPM_SYSFS_TRIGGER_ECM:
		if ((val < ECM_TRIGGER_BEGIN) || (val >= ECM_TRIGGER_END))
			return -EINVAL;
		l_dev->ecm.trigger_status = val;
		l_dev->ecm_status = val;
	...
	struct device *dev, struct low_power_dev *l_dev)
{
	...
	(void)power_dts_read_u32(power_dts_tag(HWLOG_TAG), np,
		"night_ecm_shutdown_check", &l_dev->night_ecm_shutdown_check, 0);
...
static int low_power_nb_register(struct low_power_dev *l_dev)
{
	...
	if (l_dev->night_ecm_shutdown_check) {
		l_dev->soc_battery_nb.notifier_call = low_power_soc_battery_notifier_call;
		ret = power_supply_reg_notifier(&l_dev->soc_battery_nb);
		if (ret < 0)
			hwlog_err("register soc_battery_nb failed, ret:%d\n", ret);
	}
...
static int low_power_remove(struct platform_device *pdev)
{
	...
	if (l_dev->night_ecm_shutdown_check) 
		power_supply_unreg_notifier(&l_dev->soc_battery_nb);
