...
int adapter_show_device_info(int prot)
{
	...
	case ADAPTER_PROTOCOL_UFCS:
		...
		break;
	case ADAPTER_PROTOCOL_PPS:
		hwlog_info("use pps protocol now\n");
		break;
	default:
		break;
	}
...
int adapter_get_precise_power_curve(int prot, struct adp_precise_pwr_curve_data *val,
	int *size, int max_size)
{
	struct adapter_protocol_ops *l_ops = adapter_get_protocol_ops(prot);
	...
	if (!l_ops->get_precise_power_curve) {
		hwlog_err("get_power_curve is null\n");
		return -EPERM;
	}

	return l_ops->get_precise_power_curve(val, size, max_size);
}
...
int adapter_get_adp_sn(int prot, u8 *sn_info, u8 size)
{
	int ret;
	struct adapter_protocol_ops *l_ops = adapter_get_protocol_ops(prot);
	struct adapter_protocol_dev *l_dev = adapter_get_protocol_dev();

	if (!l_ops ||!l_dev || !sn_info)
		return -EPERM;

	if (!l_ops->get_adp_sn) {
		hwlog_err("get_adp_sn is null\n");
		return -EPERM;
	}

	ret = l_ops->get_adp_sn(sn_info, size);
	if (ret) {
		hwlog_err("get_adp_sn failed\n");
		return -EPERM;
	}

	ret = memcpy_s(l_dev->info.adp_sn, sizeof(l_dev->info.adp_sn), sn_info, size);
	if (ret != EOK) {
		hwlog_err("get_adp_sn memcpy_s failed\n");
		return -EPERM;
	}
...
int adapter_get_power_bank_cap(int prot, int *cap)
{
	...
	if (!l_ops->get_power_bank_cap) {
		hwlog_err("get_power_bank_cap is null\n");
		return -EPERM;
	}

	ret = l_ops->get_power_bank_cap(cap);
	if (ret) {
		hwlog_err("get_power_bank_cap failed\n");
		return -EPERM;
	}

	l_dev->info.power_bank_cap = *cap;
	return 0;
}
...
bool adapter_is_power_redistributed(int prot)
{
	...
	if (!l_ops || !l_ops->is_power_redistributed)
		return false;

	return l_ops->is_power_redistributed();
}
...
bool adapter_support_9v(int prot)
{
	...
	if (!l_ops || !l_ops->support_9v) {
		hwlog_err("support_9v is null\n");
		return false;
	}

	return l_ops->support_9v();
}
