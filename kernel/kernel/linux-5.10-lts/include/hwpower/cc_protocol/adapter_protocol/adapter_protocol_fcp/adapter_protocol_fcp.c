...
static int hwfcp_set_usbpd_enable(int enable, bool flag)
{
	unsigned int value[BYTE_ONE] = { 0 };
	unsigned int tmp_value;

	/* reg: 0xcf */
	if (hwfcp_reg_read(HWFCP_USBPD_INFO, (int *)value, BYTE_ONE))
		return -EPERM;

	tmp_value = enable ? HWFCP_USBPD_ENABLE : HWFCP_USBPD_DISABLE;
	value[0] &= ~HWFCP_USBPD_DISABLE_MASK;
	value[0] |= (tmp_value << HWFCP_USBPD_DISABLE_SHIFT) & HWFCP_USBPD_DISABLE_MASK;

	/* reg: 0xcf */
	if (hwfcp_reg_write(HWFCP_USBPD_INFO, (const int *)value, BYTE_ONE))
		return -EPERM;

	hwlog_info("fcp set_usbpd_enable:%d, value=0x%x, flag=%u\n", enable, value[0], flag);
	return 0;
}
...
static struct adapter_protocol_ops adapter_protocol_hwfcp_ops = {
	...
	.set_usbpd_enable = hwfcp_set_usbpd_enable,
};
