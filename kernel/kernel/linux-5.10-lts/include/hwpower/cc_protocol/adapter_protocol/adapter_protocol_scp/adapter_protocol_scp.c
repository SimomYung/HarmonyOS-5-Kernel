...
static bool hwscp_is_read_special_reg(int reg)
{
	unsigned int i;

	for (i = 0; i < ARRAY_SIZE(g_hwscp_read_special_reg); i++) {
		if (g_hwscp_read_special_reg[i] == reg)
			return true;
	}
...
static int hwscp_reg_read(int reg, int *val, int num)
{
	...
	if (ret < 0) {
		if (!hwscp_is_read_special_reg(reg))
...
static int hwscp_get_terminal_id(void)
{
	int value[BYTE_ONE] = { 0 };
	struct hwscp_dev *l_dev = NULL;
