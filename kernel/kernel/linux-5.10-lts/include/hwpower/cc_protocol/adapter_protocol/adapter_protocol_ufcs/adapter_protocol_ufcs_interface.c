...
bool hwufcs_is_ignore_accept_in_auth_encrypt(void)
{
	struct hwufcs_ops *l_ops = hwufcs_get_ops();
	...
	if (!l_ops->is_ignore_accept_in_auth_encrypt) {
		hwlog_info("is_ignore_accept_in_auth_encrypt is null\n");
		return false;
	}

	return l_ops->is_ignore_accept_in_auth_encrypt(l_ops->dev_data);
}