...
int wireless_get_tx_type(unsigned int ic_type, int prot)
{
	...
	if (l_ops->get_tx_type(ic_type, &l_dev->info.tx_type, l_dev->info.tx_fwver,
...
int wireless_get_tx_sn(unsigned int ic_type, int prot)
{
	...
	if (!l_ops->get_tx_sn) {
		hwlog_err("get_tx_sn is null\n");
		return -ENOTSUPP;
	}

	memset_s(l_dev->info.tx_serial_num, WIRELESS_TX_SN_LEN, 0, WIRELESS_TX_SN_LEN);
	return l_ops->get_tx_sn(ic_type, l_dev->info.tx_serial_num, WIRELESS_TX_SN_LEN);
}
...
int wireless_get_tx_adapter_id(unsigned int ic_type, int prot, u16 *adp_id, u8 *adp_manufacturer)
{
	...
	if (!l_ops || !adp_id || !adp_manufacturer)
		return -EINVAL;
 
	if (!l_ops->get_tx_adapter_id) {
		hwlog_err("get_tx_adapter_id is null\n");
		return -ENOTSUPP;
	}
 
	return l_ops->get_tx_adapter_id(ic_type, adp_id, adp_manufacturer);
}
...
int wireless_get_tx_adapter_err_code(unsigned int ic_type, int prot, u16 *err_code)
{
	...
	if (!l_ops->get_tx_adapter_err_code) {
		hwlog_err("get_tx_adapter_err_code is null\n");
		return -ENOTSUPP;
	}
 
	return l_ops->get_tx_adapter_err_code(ic_type, err_code);
}
...
int wireless_get_tx_actual_power(unsigned int ic_type, int prot, int *tx_pwr)
{
	...
	if (!l_ops->get_tx_actual_power) {
		hwlog_err("get_tx_actual_power is null\n");
		return -ENOTSUPP;
	}
 
	return l_ops->get_tx_actual_power(ic_type, tx_pwr);
}
	...
	struct wltx_fod_result_info *fod_result_info)
{
	...
	if (!l_ops || !fod_result_info)
		return -EINVAL;
 
	if (!l_ops->get_fod_result_info) {
		hwlog_err("get_fod_result_info is null\n");
		return -ENOTSUPP;
	}
 
	return l_ops->get_fod_result_info(ic_type, fod_result_info);
}
...
int wireless_get_fod_status(unsigned int ic_type, int prot, u8 *qr, u8 *fr)
{
	...
	if (!l_ops->get_fod_status) {
		hwlog_err("get_fod_status is null\n");
		return -ENOTSUPP;
	}

	return l_ops->get_fod_status(ic_type, qr, fr);
}
...
int wireless_get_fod_km_kz(unsigned int ic_type, int prot, u16 *km, u16 *kz)
{
	...
	if (!l_ops->get_fod_km_kz) {
		hwlog_err("get_fod_km_kz is null\n");
		return -ENOTSUPP;
	}

	return l_ops->get_fod_km_kz(ic_type, km, kz);
}
...
void wireless_protocol_reset_para(void)
{
	...
	l_dev->info.tx_type = WLACC_TX_UNKNOWN;
	l_dev->info.tx_adp_type = WLACC_ADP_UNKOWN;
	memset_s(l_dev->info.tx_fwver, WIRELESS_TX_FWVER_LEN, 0, WIRELESS_TX_FWVER_LEN);
	memset_s(l_dev->info.tx_serial_num, WIRELESS_TX_SN_LEN, 0, WIRELESS_TX_SN_LEN);
}
