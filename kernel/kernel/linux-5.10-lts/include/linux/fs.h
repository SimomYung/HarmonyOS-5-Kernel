...
#ifdef CONFIG_ARM64_PTR_AUTH_DATA_PTR
...
#define FMODE_MAP_DOWNGRADE	((__force fmode_t)0x80000000)
#endif
...
#define S_DECRYPT	(1 << 25) /* code decrypt */
...
#ifdef CONFIG_LIBLINUX_UDK_MISC
static inline int register_misc_chrdev(unsigned int major, const char *name,
				       const struct file_operations *fops)
{
	int ret;

	/* Reserve misc minor 236, 237 for udk */
	ret = __register_chrdev(major, 0, 236, name, fops);
	if (ret == 0) {
		__register_chrdev(major, 238, 18, name, fops);
	}
