...
#if defined(CONFIG_USB_PD_CUSTOM_DBGACC_SUPPORT) || defined(CONFIG_KIRIN_USB_PD_CUSTOM_DBGACC_SUPPORT)
...
#endif
#if defined(CONFIG_USB_PD_UVDM_SUPPORT) || defined(CONFIG_KIRIN_USB_PD_UVDM_SUPPORT)
	TCP_NOTIFY_UVDM,
#endif
...
#define DPM_CAP_SNK_EMARK_DISCOVER		(1 << 26)
...
/* Vendor Defined Functions Flags */

#define VDF_FLAGS_TX_DISCARD_MASK		(1 << 0)
#define VDF_FLAGS_ATC_CC_OPEN_MASK		(1 << 1)
#define VDF_FLAGS_SINK_PRS_QUICK_MASK		(1 << 2)
#define VDF_FLAGS_PD_31_CERTIFICATION_MASK	(1 << 3)
...
#if defined(CONFIG_VENDOR_USB_TYPEC) || defined(CONFIG_KIRIN_TYPEC)
...
#if defined(CONFIG_USB_POWER_DELIVERY_SUPPORT) || defined(CONFIG_KIRIN_POWER_DELIVERY_SUPPORT)
...
bool hisi_tcpm_support_snk_detect_emark(struct tcpc_device *tcpc);
...
int hisi_tcpm_discover_cable(struct tcpc_device *tcpc_dev, uint32_t *vdos,
		unsigned max_size);
bool hisi_tcpm_support_src_cap(struct tcpc_device *tcpc_dev, uint32_t src_cap_vol, uint32_t src_cap_mask);
int hisi_tcpm_is_during_dp(struct tcpc_device *tcpc_dev);
int hisi_tcpm_receive_src_cap(struct tcpc_device *tcpc_dev);
...
#ifdef CONFIG_KIRIN_USB_PD_ALT_MODE
	TCP_DPM_EVT_DP_ATTENTION,
#ifdef CONFIG_KIRIN_USB_PD_ALT_MODE_DFP
	TCP_DPM_EVT_DP_STATUS_UPDATE,
	TCP_DPM_EVT_DP_CONFIG,
#endif
#endif

#ifdef CONFIG_KIRIN_USB_PD_CUSTOM_VDM
	TCP_DPM_EVT_UVDM,
#endif
...
struct pd_source_cap_ext {
	...
	uint8_t batteries;
	uint8_t srs_source_pdp;
	uint8_t erp_source_pdp;
};
...
int tcpm_adjust_apdo_charging_policy(struct tcpc_device *tcpc, int mv, int ma,
