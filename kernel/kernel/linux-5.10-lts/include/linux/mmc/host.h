...
#define MMC_CAP2_FULL_PWR_CYCLE (1 << 2)        /* Can do full power cycle in suspend */
...
#define MMC_CAP2_CMD_QUEUE     	(1 << 27)       /* support eMMC command queue */
	...
	u32			caps3;		/* More host capabilities */
#define MMC_CAP3_ENHANCED_STROBE		(1 << 23)
#define MMC_CAP3_CACHE_FLUSH_BARRIER		(1 << 24)
#define MMC_CAP3_BKOPS_AUTO_CTRL		(1 << 25)	/* Allow background operations auto enable control */
#define MMC_CAP3_SUPPORT_WIFI        		(1 << 26)	/* host is connected by wifi through sdio */
#define MMC_CAP3_SUPPORT_WIFI_CMD11		(1 << 27)	/* host is connected to 1102 wifi */
#define MMC_CAP3_WIFI_NO_LOWPWR			(1 << 28)	/* host do not support low power for wifi*/
	...
	unsigned int		bus_suspend_done:1;
#endif
	...
	unsigned int		bus_suspend:1;	/* bus has been suspend */
#endif
	...
	*/
	...
	int (*card_hwlocked)(struct mmc_host *host);
	struct hwspinlock *emmc_hwlock;
	#define EMMC_LOCK_TIMEOUT 3
	int emmc_hwlock_state;
	ktime_t hwspink_start;
	ktime_t hwspink_cur;
	ktime_t hwspink_end;
	unsigned int hwspink_cnt;
	unsigned int dont_get_hwspink;
#endif
...
{
	return host->caps3 & MMC_CAP3_SUPPORT_WIFI_CMD11;
}

static inline int mmc_host_wifi_support_lowpwr(struct mmc_host *host)
{
	return !(host->caps3 & MMC_CAP3_WIFI_NO_LOWPWR);
}
