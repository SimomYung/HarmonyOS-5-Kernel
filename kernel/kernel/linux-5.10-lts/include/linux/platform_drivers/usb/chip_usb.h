...
#define DEVICE_EVENT_SUSPEND	4

/* usb pmu vbus event */
#define USB_VBUS_CONNECT		0
#define USB_VBUS_DISCONNECT		1
...
#define PORDUCT_LEN 256
#define SERIAL_LEN 256
#define DEV_PATH_LEN 16
...
struct usb_device_info {
	...
	char product[PORDUCT_LEN];
	char manufacturer[MANUFEATURER_LEN];
...
void usb_vbus_register_notify(struct notifier_block *nb);
void usb_vbus_unregister_notify(struct notifier_block *nb);
#else
static inline void usb_vbus_register_notify(struct notifier_block *nb)
{
}
static inline void usb_vbus_unregister_notify(struct notifier_block *nb)
{
}
