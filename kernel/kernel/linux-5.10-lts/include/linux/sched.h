...
#ifdef CONFIG_WORKER_MONITOR
#include "basicplatform/linux/worker_monitor.h"
#endif
...
enum vip_boost_type {
	...
	VIP_BOOST_CGROUP_RWSEM_DEFERRED,
	VIP_BOOST_TYPE_MAX,
};
#endif
...
struct task_struct {
	...
	unsigned int			flags;
#if defined(CONFIG_HW_CGROUP_WORKINGSET) || \
	defined(CONFIG_USER_HANDLE_FREEZING)
	...
	atomic_t trans_qos[DYNAMIC_QOS_TYPE_MAX];
	spinlock_t qos_lock;
#endif
	/*
	...
	pid_t				frozen_procs_by;
	pid_t				frozen_stat_by;
#ifdef CONFIG_SCHED_LATENCY_NICE
	int				latency_prio;
#endif
	...
	/*
	 * 'soft_throttle_list' represents list head to hang task on rt_rq's
	 * soft_throttle_head. rt_rq can perceive downgraded tasks
	 *
	 * 'attached_rt_rq' represents orig rt_rq downgrade happened. when
	 * tasks downgrade to cfs, load balance will make them run anywere
	 * use attached rt_rq to mark rt_rq we downgrade from, easy for info
	 * clear on exit routine. should accompy with list_del.
	 *
	 * 'orig_prio' represents prio before downgraded
	 *
	 * 'orig policy' represents policy before downgraded
	 */
	struct list_head		soft_throttle_list;
	struct rt_rq			*attached_rt_rq;
	int orig_prio;
	int orig_policy;
#endif
...
#ifdef CONFIG_HW_CGROUP_WORKINGSET_FAULT_AROUND
#define PF_EXT_WSCG_FAULT_AROUND 0x00000004	 /* I am in a workingset do exact fault around */
#endif

#ifdef CONFIG_USER_HANDLE_FREEZING
#define PF_EXT_IN_USERHANDLE_FLAG	0x00000010	/* This means already have thread back to userspace */
#define PF_EXT_IN_USERHANDLE	0x00000020	/* This means thread is working in userspace */
#define PF_EXT_REIN_USERHANDLE	0x00000040
#define PF_EXT_REIN_USERHANDLE_FLAG	0x00000080
#endif
...
#ifdef CONFIG_SCHED_RTG
extern struct related_thread_group *task_related_thread_group(struct task_struct *p);
#endif
...
#ifdef CONFIG_RT_SOFT_BANDWIDTH
void mark_rt_restricted(struct task_struct* p);
void unmark_rt_restricted(struct task_struct* p);
#endif
