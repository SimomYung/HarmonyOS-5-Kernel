...
/* tmp_seq for __print_flags() and __print_symbolic() etc. */
typedef int (*hmtrace_print_event)(void *entry, struct seq_buf *seq, struct trace_seq *tmp_seq);
#endif
...
#ifdef CONFIG_LIBLINUX_HMTRACE
struct hmtrace_event_functions {
	hmtrace_print_event	print_event;
};
#endif
...
#ifdef CONFIG_LIBLINUX_HMTRACE
	struct hmtrace_event_functions	*hmfuncs;
...
extern uintptr_t hmtracefs_record_shm_addr_of(void);
extern uint64_t hmtracefs_rmq_enqueue_get_entry(uintptr_t rmq_addr, uint64_t len,
						void *ppe);
extern void hmtracefs_rmq_enqueue_confirm_entry(void *ppe);
extern void hmtracefs_set_taskinfo_save(int enable);

extern int hmtracefs_tid_of(void);
extern int hmtracefs_rmq_is_enabled(void);
extern uintptr_t hmtracefs_record_shm_addr_of(void);
extern void hmtracefs_rmq_dequeue_futex_wake(void);
#endif
