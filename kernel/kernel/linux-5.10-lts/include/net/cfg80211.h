 ...
 * @ASSOC_REQ_FAST_CONNECT: User space indicates fast connect.
 * @ASSOC_REQ_NO_ENCRPTY: User space indicates no encrpty connect.
 */
...
#endif
	ASSOC_REQ_FAST_CONNECT			= BIT(30),
	ASSOC_REQ_NO_ENCRPTY                    = BIT(31),
};
...
#ifdef CONFIG_HISI_WIFI7_MLO
/**
 * cfg80211_inform_mlo_assoc_link_id - notify kernel mlo connection assoc link id
 *
 * @dev: the network device
 * @assoc_link_id: mlo connection current work link id
 */
void cfg80211_inform_mlo_assoc_link_id(struct net_device *dev, u8 assoc_link_id);

/**
 * cfg80211_inform_link_switch - notify upper layer link switch event
 *
 * @dev: the network device
 * @link_id: after link switch, netdevice work link id
 */
void cfg80211_inform_link_switch(struct net_device *dev, u8 link_id);
#endif

/**
 * cfg80211_inform_cofeature_state - notify upper layer mlo/wur working state event
 *
 * @dev: the network device
 * @feature: cofeature, such as mlo or wur, and so on
 * @state: working state
 * @reason_code: cofeature switch reason
 */
