...
#ifdef CONFIG_DPA_PROTO
static inline bool dpa_add_backlog(struct sock *sk, struct sk_buff *skb)
{
	volatile u32 *dpa_status = (volatile u32 *)inet_sk(sk)->dpa_ctx;

	if (dpa_status && *dpa_status != 0) {
		CALL_HCK_VH(dpa_tcp_backlog, sk, skb);
		return true;
	}
