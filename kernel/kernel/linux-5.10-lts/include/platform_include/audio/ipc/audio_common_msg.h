...
enum total_log_saving_state {
    CLOSE_TOTAL_LOG_SAVE,
    OPEN_TOTAL_LOG_SAVE
};
...
typedef enum {
    ...
    SAMPLERATE_96K = 96000,
    SAMPLERATE_192K = 192000
} sample_rate_t;

struct dsp_route_write_msg {
    unsigned int msg_id;
    sample_rate_t sample_rate;
};

struct dsp_route_write_ack_msg {
    unsigned int msg_id;
    int result;
};
...
#define SINGLE_DSP_TIME_INFO_STR_LEN 60
#define MAX_DSP_TIME_INFO_STR_LEN (SINGLE_DSP_TIME_INFO_STR_LEN * STAMP_POS_BUTT)

enum audio_pcm_device_enum {
    AUDIO_PCM_DEVICE_PRIMARY = 0,
    AUDIO_PCM_DEVICE_BT = 1,
    AUDIO_PCM_DEVICE_FM_AP = 2,
    AUDIO_PCM_DEVICE_OFFLOAD = 3,
    AUDIO_PCM_DEVICE_DIRECT = 4, /* according to pcm driver */
    AUDIO_PCM_DEVICE_LOW_LATENCY = 5,
    AUDIO_PCM_DEVICE_MMAP = 6,
    AUDIO_PCM_DEVICE_MULTI_CHANNEL = 7,
    AUDIO_PCM_DEVICE_KARAOKE_RECORD = 8,
    AUDIO_PCM_DEVICE_XCALL = 9,
    AUDIO_PCM_DEVICE_EC = 10,
    AUDIO_PCM_DEVICE_NOISE_REDUCTION = 11,
    AUDIO_PCM_DEVICE_SOUNDTRIGGER = 12,
    AUDIO_PCM_DEVICE_BT_SLAVE_PLAY_LOOP = 13,
    /* The following pcm devices do not use pcm card */
    AUDIO_PCM_DEVICE_WAKEUP = 20,
    AUDIO_PCM_DEVICE_WAKEUP_LP = 21,
    AUDIO_PCM_DEVICE_ULTRASONIC = 22,
    AUDIO_PCM_DEVICE_MMAP_EXTEND = 23,
    AUDIO_PCM_DEVICE_MMAP_MAX = AUDIO_PCM_DEVICE_MMAP_EXTEND + MAX_MMAP_SINGLE_NUM - 2,
    AUDIO_PCM_DEVICE_HAPTIC,
...
enum pcm_type_enum {
    ...
    PCM_TYPE_FAST_MMAP_VOIP, /* OH mmap voip */
    PCM_TYPE_BUTT
};
...
enum audio_cmd_vol_type_enum {
    ...
    HIFI_VOL_TYPE_GT_LOCAL_PLAYER,
    HIFI_VOL_TYPE_GT_NOTIFICATION,
    HIFI_VOL_TYPE_AUDIO_LOOP,
    HIFI_VOL_TYPE_STR_AUDIO_LOOP,
    HIFI_VOL_TYPE_AUDIO_AVLPP,
    HIFI_VOL_TYPE_CNT,
};

enum audio_avlpp_start_status_enum {
    AVLPP_STOP_READY,
    AVLPP_START_READY,
};

enum audio_cmd_volume_mode_enum {
    AI_VOICE_TTS_RX = 0,
    AI_VOICE_TTS_TX = 1,
    AI_VOICE_SPKOUT = 2,
    AI_VOICE_MICIN = 3,
...
enum audio_stream_status_enum {
    STREAM_STATUS_NONE = 0,
    STREAM_STATUS_PLAY_OPEN,
    STREAM_STATUS_CAPTURE_OPEN,
    STREAM_STATUS_ALL_OPEN,
};

enum audio_cmd_blow_cognition_mode_enum {
    BLOW_COGNITION_WALLPAPER = 0,
    BLOW_COGNITION_AOD = 1,
    BLOW_COGNITION_CNT,
};

enum audio_cognition_type {
    BLOW_COGNITION,
    EMOTION_COGNITION,
    ENV_SOUND_COGNITION,
};
...
typedef struct {
    ...
    unsigned int                left_step;      /* 左声道音量阶数  */
    unsigned int                right_step;     /* 右声道音量阶数  */
...
typedef struct {
    ...
    unsigned int                resv;
} avlpp_start_ready_cmd;

struct voip_config_direction {
    unsigned short              msg_id;        /* 0xFF19 */
...
struct audio_cmd_mute_msg {
    ...
    unsigned int stream_type;
    unsigned int is_bt_slave_voice;
};
...
struct audio_cmd_set_io_sample_rate_msg {
    unsigned short msg_id;
    unsigned short reserved;
    unsigned int io_sample_rate;
    unsigned int is_bt_slave_voice; /* 0: bt_master, 1: bt_slave */
};
...
typedef struct {
    unsigned short msg_id;
    unsigned short combine_noise_mode;
} voice_algo_combine_noise_para;
...
struct sensorhub_recovery_start_msg {
    unsigned short msg_id;
    unsigned short reserve;
    unsigned int resv; /* no need data */
};
...
enum syscache_mode {
    SYSCACHE_AUDIO_BUFFER_MODE = 0,
    SYSCACHE_VOICE_CACHE_MODE,
    SYSCACHE_AUDIO_CACHE_MODE,
    SYSCACHE_MODE_CNT,
};

struct nn_model_change_recive_type {
    unsigned int type;
};

enum voice_model_type {
    HANDESET_MODEL_TYPE = 0,
    HANDFREE_MODEL_TYPE,
    AIAEC_MODEL_TYPE,
    PNR_MODEL_TYPE,
    DOWNLINK_MODEL_TYPE,
    SINGLE_NET_MODEL_TYPE,
    BT_COMB_NR_MODEL_TYPE,
    AISED_MODEL_TYPE,
    RELEASE_MODEL_TYPE,
    RELEASE_MODEL_PLAY_AISED_TYPE,
    MAX_MODEL_TYPE,
};

static const char * const VOICE_NN_MODEL[MAX_MODEL_TYPE] = {
    "HandsetModel",
    "HandfreeModel",
    "AIAECModel",
    "PNRModel",
...
typedef enum multi_channel_stream_type {
    STREAM_3DA,
    STREAM_PCM,
    STREAM_TYPE_BUTT
} multi_channel_stream_type;

struct multi_channel_stream_info {
    unsigned short msg_id;
    unsigned short stream_type;
    unsigned int channel_mask;
};

struct ec_channel_info {
    unsigned short msg_id;
...
struct ap_hifi_chime_start_msg {
    ...
    unsigned int player_id; /* 0：common, 1：special */
};
...
struct a2dp_offload_event_info {
    ...
    unsigned int flowctrl_begin_timestamp[BT_EVENT_MAX_RECORD_CNT];
    unsigned int flowctrl_end_timestamp[BT_EVENT_MAX_RECORD_CNT];
    unsigned int flowctrl_begin_cnt;
    unsigned int flowctrl_end_cnt;
};
...
enum audio_dsp_info_params_type {
    ...
    AUDIO_DSP_INFO_PARAMS_TYPE_DSP_TIME,
    AUDIO_DSP_INFO_PARAMS_TYPE_STREAM_CHECK,
...
struct audio_nn_init_req_msg {
    unsigned short msg_id;
    unsigned short uid;
    unsigned int offset;
};
struct audio_nn_init_cnf_msg {
    unsigned short msg_id;
    unsigned short reserved;
    unsigned int result;
};
struct audio_nn_deinit_req_msg {
    unsigned short msg_id;
    unsigned short reserved;
};
struct audio_nn_deinit_cnf_msg {
    unsigned short msg_id;
    unsigned short reserved;
    unsigned int result;
};
struct audio_nn_info_set_req {
    unsigned short msg_id;
    unsigned short model_id;
    unsigned short model_status;
    unsigned short reserved;
};
struct audio_nn_info_set_cnf {
    unsigned short msg_id;
    unsigned short result;
};
struct notify_ap_save_total_log {
    unsigned short msg_id;
    unsigned short mode;
    unsigned int share_mem_size;
};

struct notify_dsp_set_total_log_saving_state {
    unsigned short msg_id;
    unsigned short is_open_total_log_saving;
};

#ifdef BT_COMBINE_DENOISE
struct combined_denoise {
    unsigned short msg_id;
    unsigned short reserved;
    unsigned int enable;
};

struct combined_denoise_update_switch {
    unsigned short msg_id;
    unsigned short reserved;
};
#endif

struct combined_denoise_cnf {
    unsigned short msg_id;
    unsigned short result;
};

struct audio_cmd_set_volume_msg {
    unsigned short msg_id;
    unsigned short volume_mode;  // 0:TTS_RX; 1:TTS_TX; 2:SPKOUT; 3:MICIN; else:invalid
    unsigned int volume; // range: 0-32767
};

#ifdef AUDIO_DATA_INJECTION_SUPPORT
enum audio_data_inject_cmd_type {
    DATA_INJECT_CMD_QUERY,
    DATA_INJECT_CMD_START,
    DATA_INJECT_CMD_STOP,
    DATA_INJECT_CMD_BUTT
};
...
} audio_data_inject_cnf;

typedef struct {
    unsigned short msg_id;
    unsigned short reserved;
    unsigned int read_size;
} audio_data_inject_notify;
#endif

struct notify_dsp_str_mode_change {
    unsigned short msg_id;
    unsigned short mode;
};

struct audio_cmd_blow_cognition_update_msg {
    unsigned short msg_id;
    unsigned short reserved;
    unsigned short sink;  // 0:wallpaper; 1:aod
    unsigned short enable; // 0:disable; 1:enable
};
...
#ifdef AUDIO_SLE_OFFLOAD_SUPPORT
/* used for HAL update params to nearlink module */
struct sle_update_info {
    unsigned short              msg_id;         /* 0xFF80 */
    unsigned short              reserved;
    unsigned short              info_size;
    unsigned char               info_body[];
};
#endif // AUDIO_SLE_OFFLOAD_SUPPORT

struct audio_haptic_sync_msg {
    unsigned short msg_id;
    unsigned short reserve;
...
struct dsp_iodie_stop_msg {
    unsigned short msg_id;
    unsigned short status;
    unsigned int error_code;
};

struct dsp_iodie_stop_msg_ack {
    unsigned short msg_id;
    unsigned short reserved;
    unsigned int error_code;
};
...
enum env_snd_type {
    ...
    ENV_ALARM = 6,
    ENV_VALID_MAX = 7,
    ENV_WATER = ENV_VALID_MAX, /* reserved */
    ENV_CARALARM = 8, /* reserved */
    ENV_SIREN = 9, /* reserved */
    ENV_SND_TYPE_MAX = 10
};

struct env_sound_res_msg {
    unsigned int match_env_snd_type;
};
...
#define AUDIO_COGNITION_DATA_MAX_LEN 64
struct audio_cog_data {
    enum audio_cognition_type type;
    unsigned int size;
    char data[0];
};
