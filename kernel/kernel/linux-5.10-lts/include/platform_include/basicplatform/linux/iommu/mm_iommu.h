...
unsigned long mm_iommu_identity_map(struct device *dev,  phys_addr_t paddr,
		size_t size, int prot);
int mm_iommu_identity_unmap(struct device *dev, unsigned long iova,
		size_t size);
...
extern struct platform_device *test_pdev, *test_pdev2;
#endif
...
void mm_smmuv3_show_pte(struct device *dev, unsigned long iova,
				unsigned long size);
#else
static inline void mm_smmuv3_show_pte(struct device *dev, unsigned long iova,
				unsigned long size)
{
}
#endif
...
#ifdef CONFIG_DMA_MAP_BATCH
static inline int mm_iommu_map_page_batch(struct device *dev, struct scatterlist *sgl, int prot,
	bool coherent, bool is_dcache_flush)
{
	return 0;
}

static inline int mm_iommu_unmap_page_batch(struct device *dev, struct scatterlist *sgl
	bool is_coherent, bool is_dma_2_dev)
{
	return -EINVAL;
}
...
#ifdef CONFIG_MM_IOMMU_FAULT
static inline int mm_iommu_handle_translation_fault(struct device *dev, unsigned long iova)
{
	return -EINVAL;
}
#endif
...
#ifdef CONFIG_DMABUF_RECLAIM
static inline int mm_iommu_map_dmabuf_page(struct device *dev, struct dma_buf *dmabuf, unsigned long iova,
			unsigned long size, int prot)
{
	return -EINVAL;
}

static inline int mm_iommu_unmap_dmabuf_page(struct device *dev, unsigned long iova, unsigned long iova_size)
{
	return -EINVAL;
}
#endif
