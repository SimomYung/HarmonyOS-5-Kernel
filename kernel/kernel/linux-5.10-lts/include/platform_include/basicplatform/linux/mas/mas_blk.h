...
#include <platform_include/basicplatform/linux/hck/ufs/hck_dfa_mas_mcq.h>
...
#define BLK_LLD_IOSCHED_DFA_EN (1ULL << __BLK_LLD_IOSCHED_DFA_EN)
...
struct blk_queue_ops {
	...
	/* Block request limit update */
	void (*blk_lmt_ratio_update)(struct request_queue *, unsigned int);
	void *scheduler_priv;
...
void ufs_mq_dump_work_tm(const struct request_queue *q);
