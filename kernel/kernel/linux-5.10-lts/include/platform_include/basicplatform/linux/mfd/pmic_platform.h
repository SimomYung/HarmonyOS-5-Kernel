...
#define SPMI_IODIE_PMIC_COMP	"iodie-pmic-spmi"
#define SPMI_CAM_PMIC_COMP	"cam-pmic-spmi"
...
struct vendor_pmic {
	...
	int *irq_mask_val_arry;
	int *irq_addr_arry;
	int opt_reg_num;
	unsigned int *opt_reg_addrs;
	unsigned int *opt_reg_vals;
	char irq_name[PMIC_IRQ_NAME_SIZE];
	char chip_irq_name[PMIC_IRQ_NAME_SIZE];
	struct irq_chip irq_chip;
	int powerkey_irq_down_up;
	unsigned int spmi_slv_addr;
	unsigned char spmi_slv_mode;
	struct cfg_regs_info cfg_regs;
	bool handle_irq;
};
...
static inline int sub1_pmu_pmic_dieid(char *dieid, unsigned int len)
{
	int ret = 0;
#if defined(CONFIG_PMIC_SUB_PMU_SPMI)
	ret = pmic_subpmu1_get_dieid(dieid, len);
#endif
	return ret;
}
static inline int iodiev500_pmu_pmic_dieid(char *dieid, unsigned int len)
{
    int ret = 0;
    ret = pmic_iodiev500_get_dieid(dieid, len);
    return ret;
}
static inline int camv500_pmu_pmic_dieid(char *dieid, unsigned int len)
{
    int ret = 0;
    ret = pmic_camv500_get_dieid(dieid, len);
    return ret;
}
#else
