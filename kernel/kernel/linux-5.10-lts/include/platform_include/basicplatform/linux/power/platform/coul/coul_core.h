...
#define CHARGING_IAVG_MA_RATIO  200
...
#define REPORTCOUL_PARA_MS		(10 * 1000)
...
#define UUC_UPDATE_INTERVAL             20                 /* 20s */
...
#define SOC_FULL        100
#define BATT_SN_LEN     20
#define UUC_SOC_TH      50
#define DMD_DATA_LEN    150
...
#define LIMIT_DRC_SOC           40
#define ABNORMAL_SHUTDOWN_SOC   2
#define INVALID_FCC_CYCLE_FOR_AGING  10
#define LAST_FCC_CYCLE_MASK          0x0000ffff
#define AGING_CYCLE_MASK             0xffff0000
#define SIXTEEN_BIT_SHEFT            16
#define INVALID_AGING_CYCLE          32767
#define UUC_SMOOTH_TEMP_LEVEL        3
#define NORMAL_CUTOFF_VOLT_MIN       2500 /* 2.5V */
#define NORMAL_CUTOFF_VOLT_MAX       3500 /* 3.5V */
...
enum coul_fault_type {
	...
	COUL_FAULT_I_OUT,
	COUL_FAULT_I_IN,
	COUL_FAULT_VBAT_DROP,
	COUL_FAULT_TOTAL,
};

typedef enum {
	ERROR_COUL_UNFILTERED_SOC = 0,
	ERROR_COUL_LIMIT_FCC,
	ERROR_COUL_UUC,
	ERROR_COUL_DELTA_RC,
	ERROR_COUL_ADJUST_SOC_FAST,
	ERROR_COUL_REPORT_PARA,
} error_soc_type;
...
struct soc_change_check_info {
        int check_flag;
        int ratio;
        struct coul_batt_dsm_info start_info;
        struct coul_batt_dsm_info end_info;
};
...
enum ocv_cali_type {
	...
	POWER_ON_SAVED_OCV,
	NORMAL_SR_OCV,
	ABNORMAL_SR_OCV, /* dRC > 10% */
	CHARGE_DONE_OCV,
	CHARGE_DONE_RESUME_OCV,
	FORCED_OCV,
};

enum soe_charge_state {
	SOE_START = 0,
	SOE_CHARGE,
	SOE_DISCHARGE,
	SOE_UNKNOW
...
struct soe_information {
	bool first_in;
	int soe_adjust_flag; /* cap is between soc and soe */
	u64 last_calc_time;
	enum soe_charge_state curr_charge_status;
	enum soe_charge_state last_charge_status;
	struct coul_coul_soe_para soe_para;
};

enum uuc_smooth_temp_para_info {
	TEMPERATURE_PARA_MIN = 0,
	TEMPERATURE_PARA_MAX,
	TEMPERATURE_PARA_SOC,
	TEMPERATURE_PARA_TOTAL,
};

struct uuc_smooth_range {
	int temp_min;
	int temp_max;
	int temp_soc;
};
...
struct smartstar_coul_device {
	...
	int batt_ocv_dsm_type;
	int batt_ocv_temp;
	enum ocv_table_type batt_ocv_type; /* 0: discharge ocv, 1: charge ocv */
	...
	int last_fifo_vavg_mv;
	int iterm;
	int vterm;
	int fcc_real_mah;
	int external_compensation_is;
	...
	struct single_row_lut aged_fcc_sf_lut;
	struct delayed_work calculate_soc_delayed_work;
	struct delayed_work battery_check_delayed_work;
	struct delayed_work read_temperature_delayed_work;
	struct delayed_work nvme_library_work;
	struct delayed_work coul_para_report_delay_work;
	struct work_struct fault_work;
	struct work_struct coul_drop_work;
	...
	struct soe_information soe_info;
	...
	int discharge_cc_base_uah; /* dischagre_cc = discharge_cc_base + CC_OUT */
	int uuc_zero_vol;
	u32 is_board_type;
	u32 adc_batt_id;
	...
	unsigned int last_fcc_cycle;
	int aging_delta_percent;
	int aging_flag;
	int aging_cycle;
	int invalid_fcc_cycle;
	int enable_aging_factor;

	int g_do_save_offset_ret;
	int g_curr_cal_temp;
	unsigned int batt_indentify_fcc;
	unsigned int use_nvme_manager;
	int use_nvme_manager_init_flag;
	...
	int ui_soc;
	unsigned int emer_uuc_uah; /* reserved cap for emergency mode */
	unsigned int off_find_uuc_uah; /* reserved cap for off fined mode */
	unsigned int safety_uuc_uah; /* reserved cap for low volt batt safety */
	int basp_zero_ocv;
	int basp_zero_pc; /* tenth */
	int off_find_flag;
	int enable_basp_zero_ocv; /* whether to raise zero vol from basp */
	int enable_charge_stop_limit_drc; /* ignore drc for Si bat when charge stop */
	int charge_stop_limit_drc_soc;
	char sn[BATT_SN_LEN];
	int uuc_smooth_vol_mv;
	struct uuc_smooth_range uuc_smooth_para_range[UUC_SMOOTH_TEMP_LEVEL];

	struct soc_change_check_info soc_change_check;
	int adjust_soc_check_flag; /* dsm check when flag 1 */
	error_soc_type coul_error_soc_type;
};
