...
struct coul_merge_drv_ops {
	...
	int (*battery_discharge_cc)(void *data);
	...
	int (*battery_capacity)(void *data); /* the only capacity interface to upper level, return soe or soc */
	int (*battery_soc)(void *data); /* only for charge-log */
	int (*battery_soe)(void *data); /* only for charge-log */
	...
	int (*battery_uuc_avg_cur)(void *data);
	...
	s64 (*iscd_full_update_cc)(void *data);
	...
	int (*get_nondc_vol_dec)(void *data);
	int (*get_basp_zero_ocv)(void *data);
	...
	int (*set_off_find)(void *data, int flag);
	int (*get_off_find)(void *data);
	int (*get_battery_aging_cycle)(void *data);
	int (*set_battery_aging_flag)(void *data, int flag);
	int (*set_battery_aging_cycle)(void *data, int cycle);
	int (*set_battery_aging_factor)(void *data, int *factor, size_t size);
	int (*battery_ocv_interval)(void *data); /* only for charge-log */
};
...
int coul_merge_drv_get_discharge_cc(int batt);
...
int coul_merge_drv_battery_soc(int batt);
int coul_merge_drv_battery_soe(int batt);
...
int coul_merge_drv_battery_uuc_average_current(int batt);
...
s64 coul_merge_drv_iscd_full_update_cc(int batt);
...
int coul_merge_drv_get_battery_aging_cycle(int batt);
int coul_merge_drv_set_battery_aging_flag(int flag, int batt);
int coul_merge_drv_set_battery_aging_cycle(int cycle, int batt);
int coul_merge_drv_set_battery_aging_factor(int *factor, int batt, size_t size);
