...
struct coul_device_ops {
	...
	void (*save_fcc_invalid_up_flag)(int batt, int invalid_fcc_up_flag);
	void (*set_low_low_int_val)(int vol_mv);
	void (*save_ocv)(int batt, int ocv);
	void (*save_ocv_and_type)(int batt, int ocv, unsigned int ocv_type);
	short (*get_ocvreg)(int batt);
	...
	int (*get_battery_vol_uv_drop_from_fifo)(int batt, short fifo_order);
	int (*get_coul_drop_th)(int batt);
	void (*coul_drop_enable)(int batt, int en);
	...
	unsigned int (*get_ocv_type)(int batt);
};
