/*
 * Copyright (c) Hisilicon Technologies Co., Ltd. 2012-2019. All rights reserved.
 * Description: RPMB head file for external
 * Create: 2012-05-01
 * History: 2019-03-18 structure optimization
 */

#ifndef __RPMB_H__
#define __RPMB_H__

#include <platform_include/basicplatform/linux/mmc/ioctl.h>
#include <linux/fs.h>
#include <uapi/linux/bsg.h>

#define MAX_CDB_CMD_LENGTH 16
#define UFS_IOC_MAX_RPMB_CMD 3
#define STORAGE_IOC_MAX_RPMB_CMD 3
#define MAX_IOC_RPMB_BYTES (32 * 512)  /* max 32 blks, 1 blk = 512bytes */

enum rpmb_op_type {
	RPMB_OP_RD,
	RPMB_OP_WR_DATA,
	RPMB_OP_WR_CNT,
	RPMB_OP_WR_PROTECT = 4,
	RPMB_OP_RD_PROTECT = 5,
};

enum func_id {
	RPMB_FUNC_ID_RESERVED,
	RPMB_FUNC_ID_SE,
	RPMB_FUNC_ID_SECURE_OS,
	RPMB_FUNC_ID_MAX,
};

enum rpmb_version {
	RPMB_VER_INVALID = 0,
	RPMB_VER_MMC_51,
	RPMB_VER_UFS_20,
	RPMB_VER_UFS_21,
	RPMB_VER_UFS_30,
	RPMB_VER_UFS_31,
	RPMB_VER_UFS_40,
	RPMB_VER_MAX,
};

struct storage_blk_ioc_data {
	unsigned char *buf;
	u64 buf_bytes;
	u32 blocks;
};

struct mmc_blk_ioc_data {
	struct mmc_ioc_cmd ic;
	unsigned char *buf;
	u64 buf_bytes;
};

struct ufs_blk_ioc_data {
	/* struct sg_io_hdr sih; */
	struct sg_io_v4 siv;
	unsigned char *buf;
	u64 buf_bytes;
};

struct storage_blk_ioc_rpmb_data {
	struct storage_blk_ioc_data data[STORAGE_IOC_MAX_RPMB_CMD];
};

struct mmc_blk_ioc_rpmb_data {
	struct mmc_blk_ioc_data data[MMC_IOC_MAX_RPMB_CMD];
};

struct ufs_blk_ioc_rpmb_data {
	struct ufs_blk_ioc_data data[UFS_IOC_MAX_RPMB_CMD];
	u8 sdb_command[UFS_IOC_MAX_RPMB_CMD][MAX_CDB_CMD_LENGTH];
};

struct mmc_rpmb_ioc_data {
	u8 buf[MAX_IOC_RPMB_BYTES];
	u64 buf_bytes;
	u32 blocks;
};

struct mmc_rpmb_ioc_cmd_sub_pakage {
	struct mmc_rpmb_ioc_data data[MMC_IOC_MAX_RPMB_CMD];
};

/* emmc rpmb cmd pakage from ldk to udk */
struct mmc_rpmb_ioc_cmd_pakage {
	u32 id;
	u32 operation;
	struct mmc_rpmb_ioc_cmd_sub_pakage sub_pakage;
};

extern u64 g_rpmb_ufs_start_time;
extern u64 g_rpmb_ufs_end_time;
extern struct mutex rpmb_counter_lock;
extern void vendor_rpmb_active(void);
extern int rpmb_get_dev_ver(enum rpmb_version *ver);
extern int wait_mspc_rpmb_request_is_finished(void);
extern int check_mspc_rpmb_request_is_cleaned(void);
void rpmb_dump_io_latency(void);
#ifdef CONFIG_RPMB_UFS
int rpmb_ufs_init(int is_emulator);
#endif
#ifdef CONFIG_RPMB_MMC
int rpmb_mmc_init(void);
int mmc_blk_ioctl_rpmb_cmd(enum func_id id, struct block_device *bdev,
			   struct mmc_blk_ioc_rpmb_data *idata);
#endif
#if defined(CONFIG_PRODUCT_CDC) || defined(CONFIG_PRODUCT_CDC_ACE)
bool rpmb_get_wake_lock_status(void);
#endif

#ifdef CONFIG_VENDOR_RPMB
extern int get_rpmb_key_status(void);
extern u32 get_rpmb_support_key_num(void);
extern int get_rpmb_init_status(void);
extern int32_t mspc_exception_to_reset_rpmb(void);
void mspc_report_rpmb(void);
#else
static inline int get_rpmb_key_status(void) {return 0;}
static inline u32 get_rpmb_support_key_num(void) {return 0;}
static inline int32_t mspc_exception_to_reset_rpmb(void) {return 0;}
static inline void mspc_report_rpmb(void) {}
static inline int get_rpmb_init_status(void) {return 0;}
#endif

#endif /* __RPMB_H__ */
