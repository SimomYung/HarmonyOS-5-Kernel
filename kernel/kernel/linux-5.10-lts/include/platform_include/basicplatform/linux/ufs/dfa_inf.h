...
struct dfa_debugfs_files {
	...
	struct dentry *debug_index;
	struct dentry *sync_rtc;
	struct dentry *dfa_inject;
	struct dentry *dfa_test_cases;
};
#endif

struct uic_trans_err_para {
	u32 ov_flag;
	u32 counter;
};
...
#define DFA_RESET_DELAY_MS 2000
#define DFA_USED_IRQ_NUMS 80
struct dfa_hba_info {
	struct ufs_hba *hba;
	bool dfa_enabled;
	...
	int dfa_acore_irq;
	u8 workmode;
	unsigned int sram_config;
	int error_code;
	bool need_report_bit_error;

#ifdef CONFIG_DFX_DEBUG_FS
	struct dfa_debugfs_files debugfs_files;
#endif
	void *dfa_log;
	void *dfa_log_virtual;
	atomic64_t dfa_log_offset;
	struct task_struct *eh_worker_task;
	struct kthread_worker eh_worker;
	struct kthread_work eh_work;
	atomic_t need_reset;
	atomic_t dfa_blocked;
	struct mutex dfa_lock;
	struct workqueue_struct * dfa_reset_workqueue;
	struct delayed_work dfa_reset_work;
	atomic_t dfa_reset_status;
	atomic_t dfa_reset_cnt;
#ifdef CONFIG_DFA_MCQ_OHOS
	spinlock_t dfa_spinlock;
#endif
#ifdef CONFIG_SCSI_UFS_LOW_POWER_VOTE
	unsigned long dfa_inflight_byte;
	spinlock_t dfa_lp_lock;

	atomic64_t last_vote_idle;
	struct task_struct *ppll_worker_task;
	struct kthread_worker ppll_worker;
	struct kthread_work ppll_work;
#endif
	struct uic_trans_err_para uic_trans_err;
	unsigned int dfa_irq[DFA_USED_IRQ_NUMS];
};
