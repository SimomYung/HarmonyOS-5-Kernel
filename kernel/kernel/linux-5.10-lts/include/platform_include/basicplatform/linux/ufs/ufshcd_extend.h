...
#define THR950_DFA_FSR_INFO_SIZE 8192

#if defined(CONFIG_SCSI_UFS_VELA61_VCMD) && defined(CONFIG_PLATFORM_DIEID)
...
#define UFS_AR_UID_TABLE_SIZE 96
#define UFS_ST_UID_MAP_SIZE 64
#define UFS_DCIP_CRACK_NOW_SIZE 1
#define UFS_NAND_CHIP_VER_SIZE 8
#define UFS_DIEID_TOTAL_SIZE 337
...
#define UFS_DIEID_AR_UID_OFFSET 175
#define UFS_DIEID_ST_UID_MAP_OFFSET 271
...
#define UFS_AR_UID_STEP_NUN 4
#define UFS_AR_UID_STEP_SIZE (UFS_AR_UID_STEP_NUN * sizeof(u32))
#define UFS_ST_UID_STEP_NUN 2
#define UFS_ST_UID_STEP_SIZE (UFS_ST_UID_STEP_NUN * sizeof(u16))
#define UFS_FLASH_DIEID_STEP_NUN 4
#define UFS_FLASH_DIEID_STEP_SIZE (UFS_FLASH_DIEID_STEP_NUN * sizeof(u32))
#define UFS_DIEID_ARRAY_IDX_1 1
#define UFS_DIEID_ARRAY_IDX_2 2
#define UFS_DIEID_ARRAY_IDX_3 3
#define UFS_DIEID_SNPRINTF_ERR -2
#endif
...
static inline unsigned long *ufshcd_outstanding_reqs(struct ufs_hba *hba)
{
#ifdef CONFIG_UFS_4_0_HC
	if (is_mcq_enabled(hba))
		return &hba->mcq_outstanding_reqs[0];
#endif
	return &hba->outstanding_reqs;
}
...
void ufsmcq_pack_assemble_fail_rollback(void *q);
...
static inline void ufshcd_mcq_send_queue(struct ufs_hba *hba, unsigned int task_tag, struct ufshcd_lrb *lrbp)
{
}
static inline void ufshcd_mcq_transfer_req_compl_all(struct ufs_hba *hba, bool force_compl)
{
}
...
static inline int ufsmcq_check_inflt_reqs(struct ufs_hba *hba, unsigned long timeout)
{
	return 0;
}
static inline void ufsmcq_inflt_req_clear(struct ufs_hba *hba, int tag)
{
}
static inline int ufsmcq_queuecommand(struct Scsi_Host *host, struct scsi_cmnd *cmd)
{
	return 0;
}
static inline int ufshcd_mcq_config_utrdlp(struct ufs_hba *hba, unsigned int task_tag, struct ufshcd_lrb *lrbp)
{
	return 0;
}
static inline int ufsmcq_abort(struct scsi_cmnd *cmd)
{
	return 0;
}
static inline bool ufsmcq_get_dev_cmd_tag(struct ufs_hba *hba, int *tag_out)
{
	return 0;
}
static inline void ufsmcq_put_dev_cmd_tag(struct ufs_hba *hba, int tag)
{
}
static inline int ufsmcq_exec_dev_cmd(struct ufs_hba *hba, enum dev_cmd_type cmd_type, int timeout)
{
	return 0;
}
static inline int __ufsmcq_queuecommand_directly(struct ufs_hba *hba, struct scsi_cmnd *cmd, unsigned int timeout)
{
	return 0;
}
static inline void ufsmcq_pack_assemble_fail_rollback(void* q)
{
}
static inline void ufsmcq_send_command(struct ufs_hba *hba, unsigned int task_tag)
{
}
static inline void ufsmcq_irq_cpuhp_remove(struct ufs_hba *hba)
{
}
static inline int __ufsmcq_wait_for_reqs_clr(struct ufs_hba *hba)
{
	return 0;
}
#endif
