...
typedef enum _tag_cam_device_id_constants {
	...
	CAM_SUBDEV_MIPI_EXTISP   = 0x8180,
} cam_device_id_constants_t;

typedef enum _tag_cam_chip_type {
	CAM_CT_ES = 0,
	CAM_CT_ES2,
	CAM_CT_CS,
	CAM_CT_UNKNOWN
...
#define CAM_V4L2_IOCTL_ISP_THERMAL_GUARD \
	_IOWR('A', BASE_VIDIOC_PRIVATE + 0x24, struct v4l2_event)
#define CAM_V4L2_IOCTL_DFX_IXC \
    _IOWR('A', BASE_VIDIOC_PRIVATE + 0x25, struct v4l2_event)
...
typedef enum _tag_cam_cfgreq2dev_kind {
	...
	CAM_CFGDEV_REQ_ISP_THERMAL,
	CAM_CFGDEV_REQ_DDR_THERMAL,
	CAM_CFGDEV_REQ_VBAT_LIMIT,
	CAM_CFGDEV_REQ_IODIE_STATUS,
	CAM_CFGDEV_REQ_LCD_MIPI_SWITCH,
	CAM_CFGDEV_REQ_LCD_START_SWITCH_MIPI,
	CAM_CFGDEV_REQ_IXC_DFX,
	CAM_CFGDEV_REQ_MAX,
} cam_cfgreq2dev_kind_t;

typedef enum _tag_cam_lcd_status {
	CAM_LCD_STATUS_EXPAND,
	CAM_LCD_STATUS_FLOD,
} cam_lcd_status;
...
typedef struct _tag_cam_cfgreq2dev {
	...
	union { /* can ONLY place 10 int fields here. */
		...
		unsigned int ispThermalLevel;
		unsigned int ddrThermalLevel;
		unsigned int vbatLimitLevel;
		unsigned int iodieStatus; /* 0 normal, 1 abnormal */
		struct {
			cam_lcd_status lcd_status;
		} lcd_mipi_switch;
	};
} cam_cfgreq2dev_t;
