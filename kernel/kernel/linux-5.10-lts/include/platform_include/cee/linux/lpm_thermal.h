...
struct __thermal_zone {
	...
	int target_trips_num;
	int *target_trips;
	int control_num;
	int *control_levels;
	int target_num;
	int *target_levels;
	int boost_ipa_trip_0_temp;
	int boost_ipa_trip_1_temp;
#endif
#ifdef CONFIG_THERMAL_PERCORE
	bool is_tdp_hotplug;
	u32 cpu_tdp_cluster_num;
	int *cpu_tdp_hotplug_num_low;
	int *cpu_tdp_hotplug_num_high;
	int pg_cpu_big_cluster_num;
	int gpu_tdp_hotplug_num_low;
	int gpu_tdp_hotplug_num_high;
#endif
};
...
int cluster_get_static_power_by_temp(cpumask_t *cpumask, unsigned long u_volt, int temp, u32 *static_power);
...
void ipa_trip_and_tdp_percore_refresh(struct thermal_zone_device *tz);
...
int get_npu_ex_sustain_power(void);
#endif

#ifdef CONFIG_THERMAL_IPA_OTHER_POWER
int  parse_ipa_target_levels(struct device_node *np, struct __thermal_zone *tzp);
void update_pid_target(struct thermal_zone_device *tz);
void update_boost_ipa_trips(struct thermal_zone_device *tz, bool is_boost);
#endif

#ifdef CONFIG_THERMAL_PERCORE
enum thermal_cdev_type_id ipa_get_cpu_type_id(int cluster_id);
int ipa_set_cdev_type_id(struct thermal_cooling_device *cdev);
void ipa_divide_cpu_power_equally(struct thermal_zone_device *tz, u32 *granted_power,
				int num_actors, enum thermal_cdev_type_id type_id);
void ipa_query_offline_cluster(struct thermal_zone_device *tz, int *offline_clusters, int num_actors);
void ipa_cpu_percore(struct thermal_zone_device *tz, u32 *req_power, u32 *max_power, 
				u32 *granted_power, int num_actors);
void ipa_percore_num_refresh(struct thermal_zone_device *tz);
void ipa_cpu_percore_check_recovery(struct thermal_zone_device *tz);
bool thermal_is_over_hotplug_temp(void);
int get_gpu_hotplug_status(void);
int get_cpu_hotplug_status(void);
int parse_tdp_hotplug(struct device_node *np, struct __thermal_zone *tzp);
void save_tdp_hotplug_config(struct __thermal_zone *tzp);
void update_tdp_hotplug(struct thermal_zone_device *tz);
bool is_tdp_hotplug_support(u32 *tdp_level);
bool get_ag_status(void);
void set_tdp_ag_chip(bool is_ag_chip);
void set_tdp_support(bool support);
void tdp_gpu_hotplug_config(u32 cores);
u32 get_cpu_big_cluster_num(void);
#endif
#ifdef CONFIG_LDK_THERMAL
...
int cal_extra_attr_group_size(struct thermal_zone_device *tz);
#endif
#ifdef CONFIG_THERMAL_GOV_AC_CTRL
int cluster_get_static_power_ldk_by_temp(u32 nr_cpus, int cluster, int temp,
					 unsigned long u_volt, u32 *static_power);
#endif
#ifdef CONFIG_OPT_IPA_THERMAL
int opt_ipa_params_init(struct thermal_zone_device *tz);
void opt_ipa_params_destroy(struct thermal_zone_device *tz);
void opt_ipa_cal_cdevs_weight(struct thermal_zone_device *tz);
int opt_ipa_get_weight(struct thermal_zone_device *tz,
		       struct thermal_cooling_device *cdev);
#endif /* CONFIG_OPT_IPA_THERMAL */
#endif
#ifdef CONFIG_IPA_THERMAL
bool check_tzd_list_lock(void);
#endif
...
void thermal_update_level(struct thermal_zone_device *tz, int *temperature);
void print_temp_trace(struct thermal_zone_device *tz, int temp);
#endif

#define MODE_ENABLED			"enabled"
#define MODE_DISABLED			"disabled"
#define SOC_THERMAL_NAME		"soc_thermal"
#define SOC_THERMAL_NAME_LEN		(sizeof(SOC_THERMAL_NAME) - 1)
#define BOARD_THERMAL_NAME		"board_thermal"
#define NPU_THERMAL_NAME		"npu_thermal"
#define FAN_THERMAL_NAME		"fan_thermal"
#define MARGIN_THERMAL_NAME		"margin_thermal"
...
#define CDEV_CPU_CLUSTER3_NAME		"thermal-cpufreq-3"
#define CDEV_CPU_CLUSTER4_NAME		"thermal-cpufreq-4"
#define CDEV_CPU_CLUSTER5_NAME		"thermal-cpufreq-5"
#define THERMAL_ACTOR_NAME_LEN		17
#define	THERMAL_ZONE_NAME_LENGTH	32
...
#define IPA_CLUSTER3_WEIGHT_NAME	"cdev4_weight"
#define IPA_CLUSTER4_WEIGHT_NAME	"cdev5_weight"
#define IPA_CLUSTER5_WEIGHT_NAME	"cdev6_weight"
#define IPA_GPU_WEIGHT_NAME		"cdev0_weight"

#define MAX_THERMAL_CLUSTER_NUM		10
...
int ipa_get_actor_name_by_cdev(char *cdev_name, char *actor_name);
struct thermal_cooling_device *ipa_get_cdev_by_name(const char *cdev_name);
void ipa_freq_limit_init(void);
void ipa_freq_limit_reset(struct thermal_zone_device *tz);
