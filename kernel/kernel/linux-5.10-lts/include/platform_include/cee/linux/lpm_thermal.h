...
struct soc_shell_thermal_params;

enum thermal_polling_type {
	THERMAL_DISABLED_POLLING,
	THERMAL_NORMAL_POLLING,
	THERMAL_BOOST_POLLING,
	THERMAL_MONITOR_POLLING,
};
...
#ifdef CONFIG_THERMAL_SOC_SHELL
	struct soc_shell_thermal_params *sst_params;
#endif /* CONFIG_THERMAL_SOC_SHELL */
#ifdef CONFIG_OPT_IPA_THERMAL
	int opt_cdev_num;
	int opt_param_length;
	int opt_kp_length;
	int *opt_cdev_weight;
	int *opt_control_temp;
	int *opt_total_power;
	int *opt_dynamic_kp;
...
#if defined(CONFIG_OPT_IPA_THERMAL) || defined(CONFIG_THERMAL_SOC_SHELL) || defined(CONFIG_THERMAL_GOV_DYNAMIC_INTERVAL_CTRL)
...
s64 opt_ipa_estimate_pid(struct thermal_zone_device *tz, int control_temp,
			 u32 sustainable_power, u32 max_allocatable_power);
void opt_ipa_divvy_up_power(struct thermal_zone_device *tz,
			    u32 *req_power, int num_actors, u32 total_req_power,
			    u32 total_weighted_req_power, u32 power_range);
#endif /* CONFIG_OPT_IPA_THERMAL */
#endif
#ifdef CONFIG_IPA_THERMAL
...
#endif
#ifdef CONFIG_THERMAL_SOC_SHELL
int soc_shell_thermal_init(struct thermal_zone_device *tz,
			   struct power_allocator_params *params);
void soc_shell_thermal_destroy(struct thermal_zone_device *tz,
			       struct power_allocator_params *params);
void soc_shell_thermal_notify_boost(struct thermal_zone_device *tz);
void soc_shell_thermal_notify_recover(struct thermal_zone_device *tz);
bool soc_shell_thermal_check_state(struct thermal_zone_device *tz,
			     struct power_allocator_params *params);
int soc_shell_thermal_throttle(struct thermal_zone_device *tz,
			       struct power_allocator_params *params,
			       int (*ipa_throttle)(struct thermal_zone_device *, int, int));
bool check_polling_count_down(struct thermal_zone_device *tz);
bool optimal_should_stop_polling(struct thermal_zone_device *tz);
void optimal_thermal_zone_device_set_polling(struct thermal_zone_device *tz, int delay,
					     enum thermal_polling_type polling_type);
#endif /* CONFIG_THERMAL_SOC_SHELL */
