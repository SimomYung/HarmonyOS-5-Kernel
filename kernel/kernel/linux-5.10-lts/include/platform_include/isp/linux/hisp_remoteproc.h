...
#define ISP_MUDP_RESMEM     RESERVED_CAMERA_MUDP_PHYMEM_BASE
...
enum hisp_fw_resource_type {
	...
	RSC_VINRG1_QUERY_ID,
	RSC_VQUEUE_QUERY_ID,
	RSC_ISP_NPU_IPC_QUERY_ID,
	RSC_ISP_MUDP_QUERY_ID,
	RSC_ISP_IXC_QUERY_ID,
	RSC_QUERY_ID_MAX,
...
extern int hisp_memecc_use(void);
#else
static inline void hisp_memecc_probe(struct platform_device *pdev) {(void)pdev;};
static inline void hisp_memecc_remove(void) {};
static inline int hisp_memecc_use(void) { return 0; };
#endif
#ifdef ISP_DMA_SPLITTED_SID
extern int hisp_ispdma_device_attach(struct platform_device *pdev);
extern int hisp_ispdma_device_remove(struct platform_device *pdev);
#endif
#ifdef CONFIG_ISP_BUF_PRIVATIZE
extern struct sg_table *hisp_alloc_dynmem_sg_table(unsigned long length, void **sg_info);
extern void hisp_free_dynmem_sg_table(void **sg_info);
...
extern int hisp_parse_ipcbuffer_resmem_desc(uint64_t *ipcbuffer_base_addr, uint32_t *ipcbuffer_size);
