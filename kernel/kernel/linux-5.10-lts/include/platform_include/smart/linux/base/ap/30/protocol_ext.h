...
#define MAX_CAP_PROX_CALIBRATE_DATA_LENGTH   64
#define MAX_CAP_PROX_CALI_CHANNEL_NUM        2
...
#define MAX_COVER_PARAMET_LENGTH             104
...
#define MAX_SENSORS_PARA_LEN             256
...
typedef enum {
	...
	DUBAI_EVENT_SHAKE_ORIENT_TIME = 21,
	DUBAI_EVENT_DDR_REQ_STATISTICS = 22,
	DUBAI_EVENT_PCIE = 23,
	DUBAI_EVENT_LCD_STATUS_NUMBER = 24,
	DUBAI_EVENT_END
} dubai_event_type_t;
...
typedef enum {
	...
	BIG_DATA_EVENT_ATHENA_STASTIC = 936005014,
	BIG_DATA_EVENT_COVER_OFFSET_INFO = 936005020,
} big_data_event_id_t;
...
typedef struct {
	struct pkt_header hd;
	unsigned int subcmd;
	char calibrate_data[MAX_CAP_PROX_CALIBRATE_DATA_LENGTH];
} pkt_cap_prox_calibrate_data_req_t;

typedef struct {
	struct pkt_header hd;
	unsigned int subcmd;
	char calibrate_data[MAX_COVER_PARAMET_LENGTH];
} pkt_cover_calibrate_data_req_t;
...
typedef struct {
	uint32_t sub_cmd;
	unsigned char data[MAX_SENSORS_PARA_LEN]; // 256: sensors para len
} pkt_sensors_parameter_t;
...
typedef enum additional_info_type {
	 ...
	 * corresponds to os sensor definitions.
	 * float[12]: 3x4 matrix in row major order
	 */
...
typedef enum {
	...
	MOTION_TYPE_MULTIMODAL_AWARENESS,
	MOTION_TYPE_COVER,
	MOTION_TYPE_LF_END, /* 100hz sample type end */
	/* 500hz sample type start */
	MOTION_TYPE_TOUCH_LINK = 32,
	MOTION_TYPE_BACK_TAP,
	MOTION_TYPE_CAMERA_FLIP,
	MOTION_TYPE_CLIP = 35,
	MOTION_TYPE_PS_IN_CALL = 36,
	MOTION_TYPE_POCKET_MOVE = 37,
	MOTION_TYPE_MOVE_DETECTOR = 38,
	MOTION_TYPE_STEADY_STAND = 39, /* use 100hz sample */
	MOTION_TYPE_COLLISION = 40,
	MOTION_TYPE_FALLDOWN = 41,
