...
TRACE_EVENT(devfreq_gpu_scene_aware,
	...
	TP_ARGS(scene_id, util, util_sum, window_counter,
        utilisation, target_load, target_freq),
	...
	TP_STRUCT__entry(
		...
		__field(unsigned long, utilisation	)
...
TRACE_EVENT(devfreq_gpu_scene_aware_apply_limits,
	...
	TP_fast_assign(
		...
		__entry->user_set_util = user_set_util;
		__entry->target_freq = target_freq;
	),

	TP_printk("boost=%d boost_freq=%lu user_set_freq=%lu user_set_util=%lu target_freq=%lu",
		  __entry->cl_boost, __entry->cl_boost_freq, __entry->user_set_freq,
		  __entry->user_set_util, __entry->target_freq)
);
