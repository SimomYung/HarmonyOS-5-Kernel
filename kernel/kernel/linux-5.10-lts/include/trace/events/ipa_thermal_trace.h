...
TRACE_EVENT(IPA_tdp_percore,
	TP_PROTO(s32 temp, u32 percore_num, u32 shell_percore_num, s32 is_tdp_hotplug, s32 is_ag_chip, u32 cpu_big_down,
		 s32 soc_temp_up, s32 soc_temp_down),
	TP_ARGS(temp, percore_num, shell_percore_num, is_tdp_hotplug, is_ag_chip, cpu_big_down, soc_temp_up, soc_temp_down),
	TP_STRUCT__entry(
		__field(s32, temp               )
		__field(u32, percore_num        )
		__field(u32, shell_percore_num  )
		__field(s32, is_tdp_hotplug     )
		__field(s32, is_ag_chip         )
		__field(u32, cpu_big_down       )
		__field(s32, soc_temp_up        )
		__field(s32, soc_temp_down      )
	),
	TP_fast_assign(
		__entry->temp = temp;
		__entry->percore_num = percore_num;
		__entry->shell_percore_num = shell_percore_num;
		__entry->is_tdp_hotplug = is_tdp_hotplug;
		__entry->is_ag_chip = is_ag_chip;
		__entry->cpu_big_down = cpu_big_down;
		__entry->soc_temp_up = soc_temp_up;
		__entry->soc_temp_down = soc_temp_down;
	),

	TP_printk("%d, %u,%u, %d,%d, %u, %d,%d",
		__entry->temp, __entry->percore_num, __entry->shell_percore_num, __entry->is_tdp_hotplug,
		__entry->is_ag_chip, __entry->cpu_big_down, __entry->soc_temp_up, __entry->soc_temp_down)
);
