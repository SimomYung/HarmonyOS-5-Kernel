...
TRACE_EVENT(get_frame_window_size,

	TP_PROTO(unsigned int frame_rate, unsigned int frame_window),

	TP_ARGS(frame_rate, frame_window),

	TP_STRUCT__entry(
		__field(unsigned int, frame_rate)
		__field(unsigned int, frame_window)
	),

	TP_fast_assign(
		__entry->frame_rate   	  = frame_rate;
		__entry->frame_window 	  = frame_window;
	),

	TP_printk("frame_rate=%u, frame_window=%u\n", __entry->frame_rate, __entry->frame_window)
);
 
TRACE_EVENT(gpufreq_change_polling,
 
	TP_PROTO(unsigned int old_polling_ms, unsigned int new_polling_ms),
 
	TP_ARGS(old_polling_ms, new_polling_ms),
 
	TP_STRUCT__entry(
		__field(unsigned int, old_polling_ms)
		__field(unsigned int, new_polling_ms)
	),
 
	TP_fast_assign(
		__entry->old_polling_ms = old_polling_ms;
		__entry->new_polling_ms = new_polling_ms;
	),
 
	TP_printk("gpufreq change polling, old=%u, new=%u\n", __entry->old_polling_ms, __entry->new_polling_ms)
);
