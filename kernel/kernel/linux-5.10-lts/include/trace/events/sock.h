...
TRACE_EVENT(netlink_attachskb,

	TP_PROTO(struct sock *sk, unsigned long nlk_state),

	TP_ARGS(sk, nlk_state),

	TP_STRUCT__entry(
		__field(int, rmem_alloc)
		__field(int, sk_rcvbuf)
		__field(unsigned long, nlk_state)
	),

	TP_fast_assign(
		__entry->rmem_alloc = atomic_read(&sk->sk_rmem_alloc);
		__entry->sk_rcvbuf  = READ_ONCE(sk->sk_rcvbuf);
		__entry->nlk_state  = nlk_state;
	),

	TP_printk("rmem_alloc=%d sk_rcvbuf=%d nlk_state=%lu",
		__entry->rmem_alloc, __entry->sk_rcvbuf, __entry->nlk_state)
);
