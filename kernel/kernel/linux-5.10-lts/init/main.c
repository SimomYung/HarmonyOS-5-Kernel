...
#include <security/device_threat_detection_interface.h>
...
static void __init replace_str(const char *tmp_cmdline, const char *str)
{
	char *cmd_prefix = NULL;
	char *cmd_suffix = NULL;
	int len = 0;

	cmd_prefix = strstr(tmp_cmdline, str);
	if (cmd_prefix == NULL) {
		pr_notice("Kernel command line: %s\n", tmp_cmdline);
		return;
	}
	cmd_suffix = strstr(cmd_prefix, " ");
	len = (cmd_suffix != NULL) ? (cmd_suffix - cmd_prefix) : (tmp_cmdline + strlen(tmp_cmdline) - cmd_prefix);
...
static void __init print_filtered_cmdline(char *cmdline)
{
	...
	replace_str(tmp_cmdline, "androidboot.serialno");
	replace_str(tmp_cmdline, "sn");
...
#ifndef CONFIG_ARM64_PTR_AUTH_EXT
...
#ifdef CONFIG_LIBLINUX
	int ret = false;
	if (trace_ldk_rvh_initcall_blacklist_enabled()) {
		trace_ldk_rvh_initcall_blacklist(fn, &ret);
		if (ret == true)
			return ret;
	}
#endif
...
#ifdef CONFIG_LIBLINUX
		if (level == DEVICE_INITCALL_LEVEL)
...
static int __ref kernel_init(void *unused)
{
	...
	/* once marked ro data, kernel DTD will create baseline for ro data */
	dtd_init_rodata();
...
#ifdef CONFIG_LIBLINUX
	trace_ldk_rvh_kernel_init_done(system_state);
