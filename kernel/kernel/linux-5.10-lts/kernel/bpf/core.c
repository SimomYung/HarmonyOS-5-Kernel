...
#undef ALU
	...
	{
		typedef u64 (*target_fn_t)(u64, u64, u64, u64, u64);
		target_fn_t call_pac;
		uintptr_t addr;
		addr = (uintptr_t)__bpf_call_base + (s32)(insn->imm);
		call_pac =
			(target_fn_t)pauth_pacia((void *)addr, __builtin_get_modifier_bytype(target_fn_t));
		BPF_R0 = call_pac(BPF_R1, BPF_R2, BPF_R3,
						       BPF_R4, BPF_R5);
		CONT;
	}
	...
	{
		typedef u64 (*target_fn_t)(u64, u64, u64, u64, u64);
		volatile target_fn_t call_pac;
		call_pac = (uintptr_t)__bpf_call_base + (s32)(insn->imm);
		PAC_FECFI_XPAC_SIGN(call_pac, target_fn_t);
		BPF_R0 = call_pac(BPF_R1, BPF_R2, BPF_R3,
						       BPF_R4, BPF_R5);
		CONT;
	}
...
#if defined(CONFIG_ARM64_PTR_AUTH_FWD_CFI) || defined(PAC_FWCFI_SYSSERVICE) 
	{
		typedef u64 (*target_fn_t)(u64, u64, u64, u64, u64, const struct bpf_insn *);
		target_fn_t call_pac;
		uintptr_t addr;
		addr = (uintptr_t)__bpf_call_base + (s32)(insn->imm);
		call_pac =
			(target_fn_t)pauth_pacia((void *)addr, __builtin_get_modifier_bytype(target_fn_t));
		BPF_R0 = call_pac(BPF_R1, BPF_R2,
							    BPF_R3, BPF_R4,
							    BPF_R5,
							    insn + insn->off + 1);
		CONT;
