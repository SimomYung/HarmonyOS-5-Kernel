...
#include <hwqos_sched/hwqos_mutex.h>
#endif
		    ...
		    struct ww_acquire_ctx *ww_ctx, const bool use_ww_ctx)
{
	...
	if (!use_ww_ctx) {
		...
		debug_mutex_add_waiter(lock, &waiter, current);
#ifdef CONFIG_HW_QOS_SCHED
		mutex_qos_list_add(current, &waiter.list, &lock->wait_list);
#else
	...
	for (;;) {
...
#ifdef CONFIG_SCHED_RTG_TRANS
		rtg_add_trans_thread(current, __mutex_owner(lock), MUTEX_TRANS_RTG);
#endif
#ifdef CONFIG_HW_QOS_SCHED
		rcu_read_lock();
		tsk = __mutex_owner(lock);
		if (tsk != NULL)
			get_task_struct(tsk);
		rcu_read_unlock();
		if (tsk != NULL) {
			mutex_qos_enqueue(tsk, current);
			put_task_struct(tsk);
		}
#endif
		...
		current->thread_info.owner = (uintptr_t)&lock->owner;
#endif
		schedule_preempt_disabled();
...
#endif
#ifdef CONFIG_SCHED_RTG_TRANS
	rtg_remove_trans_thread(current, MUTEX_TRANS_RTG);
#endif
}
