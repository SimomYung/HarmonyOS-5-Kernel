...
#include <hwqos_sched/hwqos_rwsem.h>
#endif
...
queue:
...
#ifdef CONFIG_HW_QOS_SCHED
	rwsem_qos_list_add(current, &waiter.list, &sem->wait_list);
#else
	...
	for (;;) {
		...
		raw_spin_lock_irq(&sem->wait_lock);
		rcu_read_lock();
		tsk = rwsem_owner_flags(sem, &flags);
		if (tsk != NULL && !(flags & RWSEM_READER_OWNED))
			get_task_struct(tsk);
		rcu_read_unlock();
		if (tsk != NULL && !(flags & RWSEM_READER_OWNED)) {
			rwsem_qos_enqueue(tsk, waiter.task);
			put_task_struct(tsk);
		}
		raw_spin_unlock_irq(&sem->wait_lock);
#endif
#ifdef CONFIG_LIBLINUX
		current->thread_info.owner = (uintptr_t)&sem->owner;
#endif
		schedule();
...
#ifdef CONFIG_HW_QOS_SCHED
	rwsem_qos_list_add(waiter.task, &waiter.list, &sem->wait_list);
#else
	list_add_tail(&waiter.list, &sem->wait_list);
#endif
...
wait:
	...
	for (;;) {
		...
		for (;;) {
			...
			tsk = rwsem_owner_flags(sem, &flags);
			if (tsk != NULL && !(flags & RWSEM_READER_OWNED))
				rtg_add_trans_thread(waiter.task, tsk, RWSEM_TRANS_RTG);
			rcu_read_unlock();
			raw_spin_unlock_irq(&sem->wait_lock);
#endif
#ifdef CONFIG_LIBLINUX
...
#ifdef CONFIG_HW_QOS_SCHED
	rwsem_qos_dequeue(current);
#endif

#ifdef CONFIG_SCHED_RTG_TRANS
	rtg_remove_trans_thread(current, RWSEM_TRANS_RTG);
#endif
