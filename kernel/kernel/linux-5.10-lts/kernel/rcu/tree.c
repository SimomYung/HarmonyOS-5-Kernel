...
#if defined(CONFIG_LIBLINUX) && defined(CONFIG_NO_HZ_IDLE)
		trace_ldk_rvh_rcu_restart_tick(rdp->cpu);
#endif
	}
...
#if defined(CONFIG_LIBLINUX) && defined(CONFIG_NO_HZ_IDLE)
	/* Restart tick to avoid RCU stalls */
	trace_ldk_rvh_rcu_restart_tick(rdp->cpu);
#endif
...
static bool __note_gp_changes(struct rcu_node *rnp, struct rcu_data *rdp)
{
	...
	trace_ldk_rvh_rcu_notify_qs(rdp->cpu, rdp->core_needs_qs);
#endif
...
#if defined(CONFIG_LIBLINUX) && defined(CONFIG_NO_HZ_IDLE)
	trace_ldk_rvh_rcu_restart_tick(smp_processor_id());
#endif
...
#ifdef CONFIG_LIBLINUX
	if (!rcu_gp_in_progress()) {
		trace_ldk_rvh_rcu_notify_qs(rdp->cpu, true);
	}
#endif
}
...
#if defined(CONFIG_LIBLINUX) && defined(CONFIG_NO_HZ_IDLE)
		trace_ldk_rvh_rcu_restart_tick(cpu);
#endif
	} else {
...
void rcu_barrier(void)
{
	...
	/* Enable tick to accelerate RCU barrier. */
...
#ifdef CONFIG_LIBLINUX_NO_HZ
	/* RCU barrier completed, so disable tick. */
	tick_dep_clear(TICK_DEP_BIT_RCU);
#endif
...
int rcutree_offline_cpu(unsigned int cpu)
{
	...
	tick_dep_set(TICK_DEP_BIT_RCU);
#if defined(CONFIG_LIBLINUX) && defined(CONFIG_NO_HZ_IDLE)
	// restart tick on all cpus
	trace_ldk_rvh_rcu_restart_tick(nr_cpu_ids);
#endif
	return 0;
}
