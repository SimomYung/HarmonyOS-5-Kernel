...
{
/*
 * If CONFIG_PROVE_RCU is selected, rcu_test_sync_prims() will be
 * called in liblinux_kernel_init(), the call of wait_rcu_gp()
 * depends on the clock however is not available during devhost
 * initialization. So rcu_test_sync_prims() should not be called
 * when CONFIG_LIBLINUX is selected.
 */
