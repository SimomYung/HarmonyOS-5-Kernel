...
static int rvh_func_add(struct tracepoint *tp, struct tracepoint_func *func)
{
	...
	if (!static_key_enabled(&tp->key)) {
		...
		tp->funcs = rvh_zalloc_funcs(HCK_RVH_NR_PROBES_MAX + 1);
		if (!tp->funcs)
			return ENOMEM;
	}

	for (i = 0; i < HCK_RVH_NR_PROBES_MAX; i++) {
		if (!tp->funcs[i].func) {
			if (!static_key_enabled(&tp->key))
...
static int hck_rvh_add_func(struct tracepoint *tp, struct tracepoint_func *func)
{
	int ret;
...
int hck_rvh_probe_register(struct tracepoint *tp, void *probe, void *data)
{
	...
	tp_func.func = probe;
	tp_func.data = data;
	ret = hck_rvh_add_func(tp, &tp_func);
	mutex_unlock(&tracepoints_mutex);

	return ret;
}
EXPORT_SYMBOL_GPL(hck_rvh_probe_register);
#endif
