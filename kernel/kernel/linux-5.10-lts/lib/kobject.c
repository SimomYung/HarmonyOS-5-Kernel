...
#ifdef PAC_FWCFI_SYSSERVICE
	typedef ssize_t (*target_fn_t)(struct kobject *kobj, struct kobj_attribute *attr,
			char *buf);
	target_fn_t call_pac;
	call_pac = kattr->show;
	if (call_pac) {
		PAC_FECFI_XPAC_SIGN(call_pac, target_fn_t);
		ret = call_pac(kobj, kattr, buf);
	}
#else
	if (kattr->show)
			       ...
			       const char *buf, size_t count)
{
	...
	kattr = container_of(attr, struct kobj_attribute, attr);
#ifdef PAC_FWCFI_SYSSERVICE
	typedef ssize_t (*target_fn_t)(struct kobject *kobj,
					struct kobj_attribute *attr, const char *buf, size_t count);
	target_fn_t call_pac;
	call_pac = kattr->store;
	if (call_pac) {
		PAC_FECFI_XPAC_SIGN(call_pac, target_fn_t);
		ret = call_pac(kobj, kattr, buf, count);
	}
#else
	if (kattr->store)
