...
#if defined (CONFIG_HW_LINUX_KERNEL_MM) && defined(CONFIG_HISI_LOWMEM_DBG)
#include <linux/mm/lowmem_killer.h>
#endif
...
{
#ifndef CONFIG_LIBLINUX_VMEMMAP_DEBUG
...
#else
	if (!pfn_valid_within(page_to_pfn(page))) {
		pr_err("pfn 0x%lx is invalid.\n", page_to_pfn(page));
		return 0;
	}
	if (zone != page_zone(page)) {
		pr_err("page 0x%lx zone%s isn't same with zone%s\n",
		       page_to_pfn(page), page_zone(page)->name, zone->name);
		return 0;
	}
#endif
...
#ifdef CONFIG_LIBLINUX_BUDDY_TRACKER
		liblinux_sub_caller_use_cnt(page, order);
#endif
		return false;
	}
			    ...
			    fpi_t fpi_flags)
{
	...
	if (test_bit(PG_rsv, &page->flags)) {
		__free_pages_internal(page, order);
		return;
	}
#endif /* CONFIG_LIBLINUX */
...
void __init page_alloc_init_late(void)
{
	...
	trace_ldk_vh_zone_pageset_init_late(0);
...
#ifdef CONFIG_LIBLINUX_PCP_FREE_FACTOR
...
#ifdef CONFIG_LIBLINUX_PCP_FREE_FACTOR
		free_pcppages_bulk(zone, nr_pcp_free(pcp, pcp->high, batch), pcp);
#else
...
static bool should_reclaim_filecache_retry(int order, int *retry_num)
{
	...
	/*
	* Retry allocation 3 times at most for page with order 2.
	*/
	if (order != 2 || *retry_num >= 3)
		return false;
							...
							nodemask_t *nodemask)
{
	...
	if (gfp_mask & LIBLINUX_PAL_GFP_RESERVE) {
		page = __alloc_pages_internal(gfp_mask, order);
		if(!page)
			pr_err("%s: alloc reserve mem order:%d failed\n", __func__, order);
#ifdef CONFIG_LIBLINUX_BUDDY_TRACKER
		liblinux_add_caller_use_cnt(page, order, _RET_IP_);
#endif
		return page;
	}
	unsigned long start = liblinux_get_time_us();
#endif
...
#endif
#ifdef CONFIG_LIBLINUX
	trace_ldk_rvh_alloc_pages_time_stat(order, gfp_mask, (liblinux_get_time_us() - start), page);
#endif
	return page;
}
...
long si_mem_available(void)
{
	...
	/* add dmaheap pool pages */
	available += global_zone_page_state(NR_DMAHEAPCACHE_PAGES);
	/* add mail pool pages */
...
#endif
		" free:%lu free_pcp:%lu free_cma:%lu dmaheap_cache:%lu"
		" dmaheap_used:%lu(bytes)\n",
...
#endif
#ifdef CONFIG_ZSMALLOC
			K(node_page_state(pgdat, NR_ZSPAGES)),
#endif
...
#ifdef CONFIG_LIBLINUX
		if (step_free_kbytes != -1) {
			tmp = (step_free_kbytes >> (PAGE_SHIFT - 10));
		} else {
			/*
...
#ifdef CONFIG_LIBLINUX
/*
 * step_free_kbytes_sysctl_handler - just a wrapper around proc_dointvec() so
 *	that we can call two helper functions whenever step_free_kbytes
 *	changes.
 */
int step_free_kbytes_sysctl_handler(struct ctl_table *table, int write,
		void *buffer, size_t *length, loff_t *ppos)
{
	int rc;
			...
			struct page **page_array)
{
	...
	while (nr_populated < nr_pages) {
		...
		page = __rmqueue_pcplist(zone, ac.migratetype, alloc_flags, pcp,
					 &pcp->lists[ac.migratetype]);
		if (unlikely(!page)) {
