...
#if defined(CONFIG_LIBLINUX) && !defined(CONFIG_PREEMPTION)
...
/* show_slab in emergency scene, only can be called before oom panic */
void show_slab_emergency(void)
{
	struct kmem_cache *cachep = NULL;
	...
	if (!mutex_trylock(&slab_mutex)) {
		pr_warn("slab_mutex busy, fail to show_slab\n");
		return;
	}
	list_for_each_entry(cachep, &slab_caches, list) {
		struct slabinfo sinfo;
