...
extern void print_v4_rule_add_ret_info(const struct fib_rule *rule, int ret);
extern void print_v6_rule_add_ret_info(const struct fib_rule *rule, int ret);
extern void print_v4_rule_del_ret_info(const struct fib_rule *rule, int ret);
extern void print_v6_rule_del_ret_info(const struct fib_rule *rule, int ret);

void print_rule_add_ret_info(int fam, const struct fib_rule *rule, int ret)
{
    if (fam == AF_INET)
        print_v4_rule_add_ret_info(rule, ret);
    else if (fam == AF_INET6)
        print_v6_rule_add_ret_info(rule, ret);
}

void print_rule_del_ret_info(int fam, const struct fib_rule *rule, int ret)
{
    if (fam == AF_INET)
        print_v4_rule_del_ret_info(rule, ret);
    else if (fam == AF_INET6)
        print_v6_rule_del_ret_info(rule, ret);
}
		   ...
		   struct netlink_ext_ack *extack)
{
	...
	rules_ops_put(ops);
	print_rule_add_ret_info(frh->family, rule, 0);
	return 0;

errout_free:
	print_rule_add_ret_info(frh->family, rule, err);
	kfree(rule);
errout:
		   ...
		   struct netlink_ext_ack *extack)
{
	...
	print_rule_del_ret_info(frh->family, rule, err);
	fib_rule_put(rule);
	flush_route_cache(ops);
	rules_ops_put(ops);
	kfree(nlrule);
