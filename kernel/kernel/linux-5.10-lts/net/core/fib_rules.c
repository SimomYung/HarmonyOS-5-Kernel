...
extern void print_v4_rule_add_ret_info(const struct fib_rule *rule, int ret, bool is_host);
extern void print_v6_rule_add_ret_info(const struct fib_rule *rule, int ret, bool is_host);
extern void print_v4_rule_del_ret_info(const struct fib_rule *rule, int ret, bool is_host);
extern void print_v6_rule_del_ret_info(const struct fib_rule *rule, int ret, bool is_host);

void print_rule_add_ret_info(int fam, const struct fib_rule *rule, int ret, bool is_host)
{
    if (fam == AF_INET)
        print_v4_rule_add_ret_info(rule, ret, is_host);
    else if (fam == AF_INET6)
        print_v6_rule_add_ret_info(rule, ret, is_host);
}

void print_rule_del_ret_info(int fam, const struct fib_rule *rule, int ret, bool is_host)
{
    if (fam == AF_INET)
        print_v4_rule_del_ret_info(rule, ret, is_host);
    else if (fam == AF_INET6)
        print_v6_rule_del_ret_info(rule, ret, is_host);
}
		   ...
		   struct netlink_ext_ack *extack)
{
	...
	rules_ops_put(ops);
	print_rule_add_ret_info(frh->family, rule, 0, net_eq(net, &init_net));
	return 0;

errout_free:
	print_rule_add_ret_info(frh->family, rule, err, net_eq(net, &init_net));
	kfree(rule);
errout:
	pr_info("ip%s rule add fail, may not generate rule\n",
		frh->family == AF_INET ? "" : (frh->family == AF_INET6 ? "6" : "?"));
	rules_ops_put(ops);
	return err;
}
		   ...
		   struct netlink_ext_ack *extack)
{
	...
	print_rule_del_ret_info(frh->family, rule, err, net_eq(net, &init_net));
	fib_rule_put(rule);
	flush_route_cache(ops);
	rules_ops_put(ops);
	kfree(nlrule);
