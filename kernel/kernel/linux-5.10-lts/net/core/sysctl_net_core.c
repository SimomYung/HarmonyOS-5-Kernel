...
#endif
struct ctl_table net_core_table[] = {
#ifdef CONFIG_NET
	{
		.procname	= "wmem_max",
...
EXPORT_SYMBOL_GPL(net_core_table);
...
#ifdef CONFIG_NETLINK_ROUTE_NLMSG_READPRIV
	{
		.procname	= "getlink_disable",
		.data		= &init_net.core.sysctl_getlink_disable,
		.maxlen		= sizeof(int),
		.mode		= 0644,
		.extra1		= SYSCTL_ZERO,
		.proc_handler	= proc_dointvec_minmax
...
#ifdef CONFIG_NETLINK_ROUTE_NLMSG_READPRIV
		trace_ldk_vh_sysctl_core_net_init(net, tbl, &init_user_ns);
#endif
	}
