...
extern void try_change_to_uniform_sockaddr_in(struct sockaddr_in *sin, struct sock *sk);
#endif
...
#ifdef CONFIG_HUAWEI_NAT_TRAV
	correct_v4_saddr(net, &addr->sin_addr.s_addr);
#endif
...
out:
	if (err)
		pr_info("__inet_bind port:%u, addr:%d..%d, return err:%d\n", ntohs(addr->sin_port),
			((unsigned char*)&(addr->sin_addr.s_addr))[0], ((unsigned char*)&(addr->sin_addr.s_addr))[3], err);
	return err;
}
...
#ifdef CONFIG_DPA_PROTO
	if (!err)
		CALL_HCK_VH(dpa_try_offload, sk);
#endif
	return err;
}
			...
			int addr_len, int flags)
{
	...
	trace_ldk_vh_inet_stream_connect(sock, flags);
		...
		bool kern)
{
	...
	err = 0;
#ifdef CONFIG_DPA_PROTO
	CALL_HCK_VH(dpa_try_offload, sk2);
#endif
	release_sock(sk2);
	trace_ldk_vh_inet_accept(sock);
do_err:
	return err;
}
...
#ifdef CONFIG_HUAWEI_NAT_TRAV
	try_change_to_uniform_sockaddr_in(sin, sk);
#endif
...
int inet_ioctl(struct socket *sock, unsigned int cmd, unsigned long arg)
{
	...
	case SIOCDELRT:
		...
		pr_info("inet_ioctl route from user. cmd:%u\n", cmd == SIOCADDRT ? "add" : "del" );
		break;
	case SIOCRTMSG:
		err = -EINVAL;
		break;
