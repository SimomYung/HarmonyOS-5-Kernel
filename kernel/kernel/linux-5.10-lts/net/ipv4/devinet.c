...
extern bool can_change_to_uniform_in_ifa(const struct in_ifaddr *ifa);
extern bool try_change_to_uniform_in_ifaddr(struct net_device *dev, struct in_ifaddr *new_ifa);
#endif
...
int devinet_ioctl(struct net *net, unsigned int cmd, struct ifreq *ifr)
{
...
#ifdef CONFIG_HUAWEI_NAT_TRAV
	if ((cmd == SIOCGIFADDR || cmd == SIOCGIFDSTADDR || cmd == SIOCGIFNETMASK)
			&& can_change_to_uniform_in_ifa(ifa))
		need_change = try_change_to_uniform_in_ifaddr(ifa->ifa_dev->dev, &unif_ifa);
#endif
	...
	case SIOCGIFADDR:	/* Get interface address */
		...
		if (need_change) sin->sin_addr.s_addr = unif_ifa.ifa_local;
		break;
	...
	case SIOCGIFDSTADDR:	/* Get the destination address */
		...
		if (need_change) sin->sin_addr.s_addr = unif_ifa.ifa_address;
		break;

	case SIOCGIFNETMASK:	/* Get the netmask for the interface */
		ret = 0;
		sin->sin_addr.s_addr = ifa->ifa_mask;
		if (need_change) sin->sin_addr.s_addr = unif_ifa.ifa_mask;
		break;
...
#ifdef CONFIG_HUAWEI_NAT_TRAV
		if (can_change_to_uniform_in_ifa(ifa) && try_change_to_uniform_in_ifaddr(dev, &unif_ifa))
			(*(struct sockaddr_in *)&ifr.ifr_addr).sin_addr.s_addr = unif_ifa.ifa_local;
#endif
			    ...
			    struct inet_fill_args *args)
{
...
#ifdef CONFIG_HUAWEI_NAT_TRAV
	if (can_change_to_uniform_in_ifa(ifa))
		need_change = try_change_to_uniform_in_ifaddr(ifa->ifa_dev->dev, &ifa_host);

	if (need_change) {
		ifm->ifa_prefixlen = ifa_host.ifa_prefixlen;
		ifm->ifa_flags = ifa_host.ifa_flags;
		ifm->ifa_scope = ifa_host.ifa_scope;
	}
#endif
	...
	if ((ifa->ifa_address &&
	     nla_put_in_addr(skb, IFA_ADDRESS, need_change ? ifa_host.ifa_address : ifa->ifa_address)) ||
	    (ifa->ifa_local &&
	     nla_put_in_addr(skb, IFA_LOCAL, need_change ? ifa_host.ifa_local : ifa->ifa_local)) ||
	    (ifa->ifa_broadcast &&
	     nla_put_in_addr(skb, IFA_BROADCAST, ifa->ifa_broadcast)) ||
	    (ifa->ifa_label[0] &&
	     nla_put_string(skb, IFA_LABEL, ifa->ifa_label)) ||
	    nla_put_u32(skb, IFA_FLAGS, need_change ? ifa_host.ifa_flags : ifa->ifa_flags) ||
	    (ifa->ifa_rt_priority &&
...
static void devinet_sysctl_unregister(struct in_device *idev)
{
	...
	trace_ldk_vh_devinet_sysctl_unregister(net, idev->dev->name);
