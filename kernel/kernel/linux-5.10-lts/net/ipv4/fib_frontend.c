...
static void print_ip_route_add_ret_info(const struct fib_config *cfg, int err)
{
	if (!cfg) {
		pr_info("ip route add fail, err:%d, not generate cfg\n", err);
		return;
	}
	__be32 dst = cfg->fc_dst;
	pr_info("ip route add %s, err:%d. table:%u, pri:%u. type:%u, dst:%u..%u/%u, tos:%u. oif:%d, "
		"gw:%#x(%u). pref_src:%u..%u. ptl:%u, scope:%u, flags:%#x\n",
		err ? "fail" : "ok", err, cfg->fc_table, cfg->fc_priority,
...
static void print_ip_route_del_ret_info(const struct fib_config *cfg, int err)
{
	if (!cfg) {
		pr_info("ip route del fail, err:%d, not generate cfg\n", err);
		return;
	}
	__be32 dst = cfg->fc_dst;
	pr_info("ip route del %s, err:%d. table:%u, pri:%u. cfg->fc_type:%u, dst:%u..%u/%u, tos:%u. oif:%d, "
		"gw:%#x(%u). pref_src:%u..%u. ptl:%u, scope:%u, flags:%#x\n",
		err ? "fail" : "ok", err, cfg->fc_table, cfg->fc_priority,
		cfg->fc_type, dst & 0xFF, (dst >> 24) & 0xFF,
		cfg->fc_dst_len, cfg->fc_tos, cfg->fc_oif, ntohl(cfg->fc_gw4),
...
int ip_rt_ioctl(struct net *net, unsigned int cmd, struct rtentry *rt)
{
	...
	case SIOCDELRT:		/* Delete a route */
		...
		pr_info("ip route %s %s for generate cfg. table:%u\n", cmd == SIOCADDRT ? "add" : "del",
			err ? "fail" : "ok", cfg.fc_table);
		if (err == 0) {
			struct fib_table *tb;
			...
			if (cmd == SIOCDELRT) {
				...
				if (tb)
					...
					err = -ESRCH;
				print_ip_route_del_ret_info(&cfg, err);
			} else {
				...
				if (tb)
					...
					err = -ENOBUFS;
				print_ip_route_add_ret_info(&cfg, err);
			}
