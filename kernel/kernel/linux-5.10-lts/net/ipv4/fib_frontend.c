...
static void print_ip_route_add_ret_info(const struct fib_config *cfg, int err, bool is_host)
{
	if (!cfg) {
		pr_info("ip route add fail, err:%d, is_host:%d, not generate cfg\n", err, is_host);
		return;
	}
	__be32 dst = cfg->fc_dst;
	pr_info("ip route add %s, err:%d. is_host:%d, table:%u, pri:%u. type:%u, dst:%u..%u/%u, tos:%u. oif:%d, "
		"gw:%#x(%u). pref_src:%u..%u. ptl:%u, scope:%u, flags:%#x\n",
		err ? "fail" : "ok", err, is_host, cfg->fc_table, cfg->fc_priority,
		cfg->fc_type, dst & 0xFF, (dst >> 24) & 0xFF,
		cfg->fc_dst_len, cfg->fc_tos,cfg->fc_oif, ntohl(cfg->fc_gw4),
		cfg->fc_gw_family, cfg->fc_prefsrc & 0xFF, (cfg->fc_prefsrc >> 24) & 0xFF,
		cfg->fc_protocol, cfg->fc_scope, cfg->fc_flags);
}

static void print_ip_route_del_ret_info(const struct fib_config *cfg, int err, bool is_host)
{
	if (!cfg) {
		pr_info("ip route del fail, err:%d, is_host:%d, not generate cfg\n", err, is_host);
		return;
	}
	__be32 dst = cfg->fc_dst;
	pr_info("ip route del %s, err:%d. is_host:%d, table:%u, pri:%u. cfg->fc_type:%u, dst:%u..%u/%u, tos:%u. oif:%d, "
		"gw:%#x(%u). pref_src:%u..%u. ptl:%u, scope:%u, flags:%#x\n",
		err ? "fail" : "ok", err, is_host, cfg->fc_table, cfg->fc_priority,
		cfg->fc_type, dst & 0xFF, (dst >> 24) & 0xFF,
		cfg->fc_dst_len, cfg->fc_tos, cfg->fc_oif, ntohl(cfg->fc_gw4),
...
int ip_rt_ioctl(struct net *net, unsigned int cmd, struct rtentry *rt)
{
	...
	case SIOCDELRT:		/* Delete a route */
		...
		pr_info("ip route %s %s for generate cfg. table:%u, is_host:%d\n", cmd == SIOCADDRT ? "add" : "del",
			err ? "fail" : "ok", cfg.fc_table, net_eq(net, &init_net));
		if (err == 0) {
			struct fib_table *tb;
			...
			if (cmd == SIOCDELRT) {
				...
				if (tb)
					...
					err = -ESRCH;
				print_ip_route_del_ret_info(&cfg, err, net_eq(net, &init_net));
			} else {
				...
				if (tb)
					...
					err = -ENOBUFS;
				print_ip_route_add_ret_info(&cfg, err, net_eq(net, &init_net));
			}
...
errout:
	print_ip_route_del_ret_info(&cfg, err, net_eq(sock_net(skb->sk), &init_net));
	return err;
}
...
errout:
	print_ip_route_add_ret_info(&cfg, err, net_eq(sock_net(skb->sk), &init_net));
	return err;
}
