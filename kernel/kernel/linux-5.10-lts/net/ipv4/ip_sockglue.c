...
extern void correct_v4_saddr(struct net *net, __be32 *saddr);
...
#ifdef CONFIG_HUAWEI_NAT_TRAV
	correct_v4_saddr(sock_net(sk), &mreqs.imr_sourceaddr);
#endif
		...
		sockptr_t optval, unsigned int optlen)
{
...
#ifdef CONFIG_HUAWEI_NAT_TRAV
		correct_v4_saddr(sock_net(sk), &mreq.imr_address.s_addr);
#endif
		if (!mreq.imr_ifindex) {
...
#ifdef CONFIG_HUAWEI_NAT_TRAV
		correct_v4_saddr(sock_net(sk), &mreqs.imr_interface);
		correct_v4_saddr(sock_net(sk), &mreqs.imr_sourceaddr);
#endif
	...
	if (err != 0 && err != -ENOPROTOOPT)
		pr_info("do_ip_setsockopt optname:%d err:%d host_net?:%d\n", optname, err, net == &init_net);
	release_sock(sk);
	if (needs_rtnl)
		rtnl_unlock();
	return err;
...
e_inval:
	pr_info("do_ip_setsockopt optname:%d err:EINVAL host_net?:%d\n", optname, net == &init_net);
	release_sock(sk);
	if (needs_rtnl)
		rtnl_unlock();
	return -EINVAL;
			    ...
			    char __user *optval, int __user *optlen)
{
...
#ifdef CONFIG_HUAWEI_NAT_TRAV
		try_change_to_uniform_ipv4_addr(&addr.s_addr, sk);
#endif

		release_sock(sk);
		pr_info("%s IP_MULTICAST_IF: isHost=%d, level=%d, optname=%d\n",
				__func__, (sock_net(sk) == &init_net), level, optname);
		if (put_user(len, optlen)) {
			pr_err("%s IP_MULTICAST_IF: put_user fail\n", __func__);
			return -EFAULT;
		}
		if (copy_to_user(optval, &addr, len)) {
			pr_err("%s IP_MULTICAST_IF: copy_to_user fail\n", __func__);
			return -EFAULT;
		}
		pr_info("%s IP_MULTICAST_IF: success\n", __func__);
		return 0;
	}
	case IP_MSFILTER:
	{
		struct ip_msfilter msf;
		pr_info("%s IP_MSFILTER: isHost=%d, level=%d, optname=%d\n",
				__func__, (sock_net(sk) == &init_net), level, optname);
		if (len < IP_MSFILTER_SIZE(0)) {
			pr_err("%s IP_MSFILTER: len error\n", __func__);
			err = -EINVAL;
			goto out;
		}
		if (copy_from_user(&msf, optval, IP_MSFILTER_SIZE(0))) {
			pr_err("%s IP_MSFILTER: copy_from_user fail\n", __func__);
			err = -EFAULT;
			goto out;
		}

#ifdef CONFIG_HUAWEI_NAT_TRAV
		correct_v4_saddr(sock_net(sk), &msf.imsf_interface);
#endif
		err = ip_mc_msfget(sk, &msf,
				   (struct ip_msfilter __user *)optval, optlen);
		pr_info("%s IP_MSFILTER: err=%d\n", __func__, err);
		goto out;
	}
	case MCAST_MSFILTER:
		if (in_compat_syscall())
	...
	case IP_PKTOPTIONS:
	{
		...
		release_sock(sk);
		pr_info("%s IP_PKTOPTIONS: isHost=%d, level=%d, optname=%d\n",
				__func__, (sock_net(sk) == &init_net), level, optname);
		if (sk->sk_type != SOCK_STREAM)
			return -ENOPROTOOPT;
...
#ifdef CONFIG_HUAWEI_NAT_TRAV
			try_change_to_uniform_ipv4_addr(&info.ipi_addr.s_addr, sk);
			try_change_to_uniform_ipv4_addr(&info.ipi_spec_dst.s_addr, sk);
#endif
