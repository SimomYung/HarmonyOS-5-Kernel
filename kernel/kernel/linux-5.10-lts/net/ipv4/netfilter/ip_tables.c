	     ...
	     struct xt_table *table)
{
 ...
 no_match:
			...
			if (v < 0) {
				...
				if (v != XT_RETURN) {
					...
					if ((verdict & NF_VERDICT_MASK) == NF_DROP)
						trace_ldk_rvh_trace_iptable_drop(state, table->name,
										 private, e, skb);
#endif
					break;
				}
		...
		if ((verdict & NF_VERDICT_MASK) == NF_DROP || acpar.hotdrop)
			trace_ldk_rvh_trace_iptable_drop(state, table->name, private, e, skb);
#endif
...
do_replace(struct net *net, sockptr_t arg, unsigned int len)
{
	...
	pr_info("v4 iptable do_replace for name:%s, entry num:%u, valid hooks:%#x\n",
...
do_ipt_set_ctl(struct sock *sk, int cmd, sockptr_t arg, unsigned int len)
{
	...
	pr_info("do_ipt_set_ctl X cmd:%d, host net?:%d, return ret:%d\n", cmd, sock_net(sk) == &init_net, ret);
	return ret;
}
