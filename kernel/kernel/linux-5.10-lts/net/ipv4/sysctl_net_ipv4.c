...
#else
		table = kvmalloc(sizeof(ipv4_net_table), GFP_KERNEL);
#endif
		if (!table)
			goto err_alloc;
...
#else
	net->ipv4.sysctl_local_reserved_ports = kvzalloc(65536 / 8, GFP_KERNEL);
#endif
...
#else
	kvfree(net->ipv4.sysctl_local_reserved_ports);
#endif
