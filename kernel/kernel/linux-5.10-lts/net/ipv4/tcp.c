...
int tcp_sendmsg_locked(struct sock *sk, struct msghdr *msg, size_t size)
{
	...
	sock_stat_bg_network_flow(sk, false, size);
#endif
	...
	fg_custom_process(sk, msg, (uint8_t)FG_TCP_BITMAP);
#endif
...
out_nopush:
	...
	trace_ldk_vh_tcp_sendmsg(sk);
#ifdef CONFIG_LIBLINUX
	trace_ldk_rvh_tcp_event_update(sk, NULL, TCP_WRITE, copied + copied_syn);
#endif
...
int tcp_sendmsg(struct sock *sk, struct msghdr *msg, size_t size)
{
	...
	lock_sock(sk);
#ifdef CONFIG_TASK_MANAGER_COLLECTOR
	taskmanager_update_process_netinfo(0, size);
#endif
...
#ifdef CONFIG_TASK_MANAGER_COLLECTOR
	if (copied > 0) {
		taskmanager_update_process_netinfo(copied, 0);
	}
#endif
	struct tcp_sock *tp = tcp_sk(sk);
...
#ifdef CONFIG_LIBLINUX
	trace_ldk_devhost_current_mm_set(current);
	if (current->mm == NULL) {
		pr_err("current mm set failed in tcp zerocopy recv\n");
		return -ENOMEM;
	}
#endif
...
#ifdef CONFIG_LIBLINUX
		trace_ldk_devhost_current_mm_clear(current);
#endif
		return -EINVAL;
	}
...
found_fin_ok:
	...
	trace_ldk_vh_tcp_recvmsg(sk);
#ifdef CONFIG_HW_NETWORK_AWARE
	sock_stat_bg_network_flow(sk, true, copied);
#endif
#ifdef CONFIG_HW_CLOUD_NETWORK
	tcp_rx_packet_infor_update(sk, len);
#endif
#ifdef CONFIG_LIBLINUX
	trace_ldk_rvh_tcp_event_update(sk, NULL, TCP_READ, copied);
#endif
	release_sock(sk);
...
#ifdef CONFIG_HUAWEI_SMARTCOM
	smartcom_tcp_fallback(sk, SMARTCOM_TCP_FALLBACK_NOPAYLOAD, state);
#endif
...
#ifdef CONFIG_TCP_SOCK_DESTROY
	/* BPF context ensures sock locking. */
	if (has_current_bpf_ctx() && spin_is_locked(&sk->sk_lock.slock))
		locked = false;
	else
#endif  /* CONFIG_TCP_SOCK_DESTROY */
	bh_lock_sock(sk);
