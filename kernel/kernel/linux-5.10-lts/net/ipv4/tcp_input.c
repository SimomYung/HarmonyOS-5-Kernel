...
void tcp_cwnd_reduction(struct sock *sk, int newly_acked_sacked, int flag)
{
	...
	trace_ldk_rvh_tcp_event_update(sk, NULL, TCP_CWND_DEC, 0);
#endif
}
...
#ifdef CONFIG_LIBLINUX
			trace_ldk_rvh_skb_drop_reason(sk, skb, SKB_TCP_RCV_ZEROWIN);
#endif
			goto out_of_window;
		}
...
queue_and_out:
		if (skb_queue_len(&sk->sk_receive_queue) == 0)
			...
			NET_INC_STATS(sock_net(sk), LINUX_MIB_TCPRCVQDROP);
#ifdef CONFIG_LIBLINUX
			trace_ldk_rvh_skb_drop_reason(sk, skb, SKB_TCP_RCV_NOBUF);
#endif
			sk->sk_data_ready(sk);
			goto drop;
		}
...
#ifdef CONFIG_LIBLINUX
		trace_ldk_rvh_skb_drop_reason(sk, skb, SKB_TCP_RCV_RST);
#endif
		goto discard;
	}
...
void tcp_init_transfer(struct sock *sk, int bpf_op, struct sk_buff *skb)
{
	...
	trace_ldk_rvh_tcp_init_transfer(sk, bpf_op, skb);
					 ...
					 const struct tcphdr *th)
{
	...
	if (th->ack) {
		...
		if (th->rst) {
			tcp_reset(sk);
#ifdef CONFIG_HUAWEI_SMARTCOM
			smartcom_tcp_fallback(sk, SMARTCOM_TCP_FALLBACK_SYN_RST, 0);
#endif
			goto discard;
		}
...
#ifdef CONFIG_LIBLINUX
		/* Queued equal to 0 indicates that tcp_drop has occurred and skb has been released. */
		if (queued)
			trace_ldk_vh_tcp_connect_rcv(sk, th);
#endif
		if (queued >= 0)
			return queued;
