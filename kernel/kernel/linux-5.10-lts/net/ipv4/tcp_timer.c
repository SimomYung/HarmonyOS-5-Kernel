...
#ifdef CONFIG_HUAWEI_SMARTCOM
	smartcom_tcp_fallback(sk, SMARTCOM_TCP_FALLBACK_RETRANS, 0);
#endif

	if (expired) {
#ifdef CONFIG_HUAWEI_SMARTCOM
		smartcom_tcp_fallback(sk, SMARTCOM_TCP_FALLBACK_SYN_TOUT, 0);
#endif
...
void tcp_retransmit_timer(struct sock *sk)
{
	...
	if (!tp->snd_wnd && !sock_flag(sk, SOCK_DEAD) &&
		...
		trace_ldk_rvh_tcp_event_update(sk, NULL, TCP_PROBE0, 0);
#endif
...
#ifdef CONFIG_LIBLINUX
	trace_ldk_rvh_tcp_event_update(sk, NULL, TCP_RETRANS, 0);
#endif
...
#ifdef CONFIG_HUAWEI_SCHEDULE_DRIVER
		smartcom_schedule_driver_fastsyn(sk);
#endif
	}
	inet_csk_reset_xmit_timer(sk, ICSK_TIME_RETRANS,
