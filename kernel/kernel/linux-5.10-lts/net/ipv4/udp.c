...
#ifdef CONFIG_TASK_MANAGER_COLLECTOR
#include <huawei_platform/taskmanager_collector/netinfo_collector.h>
#endif
...
#if (defined(CONFIG_HW_NETWORK_QOE) || defined(CONFIG_HW_HIGH_PRIORITY_LABEL))
...
#ifdef CONFIG_HW_NETWORK_SLICE_NETMGR
#include <hwnet/booster/netmanager_ext/netmanager_ext_ns_route.h>
#endif
...
#ifdef CONFIG_HUAWEI_NAT_TRAV
extern void try_change_to_uniform_ipv4_addr(__be32 *ipv4_addr, struct sock *sk);
#endif
...
#ifdef CONFIG_IP_RSVD_PORT_BIND_CTRL
...
send:
	...
	if (trace_ldk_rvh_udp_send_skb_enabled()) {
		trace_ldk_rvh_udp_send_skb(skb, fl4, &err);
		if (!err)
			return 0;
	}
#endif
...
int udp_sendmsg(struct sock *sk, struct msghdr *msg, size_t len)
{
...
#ifdef CONFIG_TASK_MANAGER_COLLECTOR
	taskmanager_update_process_netinfo(0, len);
#endif

#ifdef CONFIG_HW_NETWORK_AWARE
	sock_network_aware(sk, false);
#endif

#ifdef CONFIG_HW_HIGH_PRIORITY_LABEL
	fg_custom_process(sk, msg, (uint8_t)FG_UDP_BITMAP);
#endif
...
#ifdef CONFIG_HUAWEI_SMARTCOM
	struct sockaddr_in smartcom_usin = {0};
	smartcom_usin.sin_addr.s_addr = daddr;
	smartcom_usin.sin_port = dport;
	smartcom_usin.sin_family = AF_INET;
	smartcom_bind2device(sk, (struct sockaddr *)(&smartcom_usin));
#endif
	...
	if (cgroup_bpf_enabled && !connected) {
		...
		if (err) {
#ifdef CONFIG_LIBLINUX
			trace_ldk_rvh_bpf_drop_reason((void *)sk, BPF_DROP_SOCK);
#endif
			goto out_free;
		}
		if (usin) {
...
{
#ifdef CONFIG_TASK_MANAGER_COLLECTOR
	taskmanager_update_process_netinfo(len, 0);
#endif
	struct inet_sock *inet = inet_sk(sk);
...
int udp_pre_connect(struct sock *sk, struct sockaddr *uaddr, int addr_len)
{
	...
	if (addr_len < sizeof(struct sockaddr_in))
		return -EINVAL;
#ifdef CONFIG_LIBLINUX
	int ret = BPF_CGROUP_RUN_PROG_INET4_CONNECT_LOCK(sk, uaddr);
	if (ret)
		trace_ldk_rvh_bpf_drop_reason((void *)sk, BPF_DROP_SOCK);
	return ret;
#endif
...
#ifdef CONFIG_DPA_PROTO
	CALL_HCK_VH(dpa_udp_unicast_rcv, sk, skb);
#endif
		       ...
		       int (*push_pending_frames)(struct sock *))
{
	...
	if (!err)
		pr_err("%s: isHost=%d, level=%d, optname=%d err=%d\n",
				__func__, (sock_net(sk) == &init_net), level, optname, err);
	return err;
}
