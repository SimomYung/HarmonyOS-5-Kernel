...
extern bool can_change_to_uniform_in6_ifa(const struct inet6_ifaddr *ifa);
extern bool try_change_to_uniform_in6_ifaddr(struct net_device *dev, struct inet6_ifaddr *in6_ifaddr);
#endif
			     ...
			     struct inet6_fill_args *args)
{
	...
	u32 preferred, valid;
	int need_change = 0;
	struct inet6_ifaddr unif_ifaddr = {0};
...
#ifdef CONFIG_HUAWEI_NAT_TRAV
	if (can_change_to_uniform_in6_ifa(ifa))
		need_change = try_change_to_uniform_in6_ifaddr(ifa->idev->dev, &unif_ifaddr);
#endif

	if (!ipv6_addr_any(&ifa->peer_addr)) {
		if (nla_put_in6_addr(skb, IFA_LOCAL, need_change ? &unif_ifaddr.addr : &ifa->addr) < 0 ||
		    nla_put_in6_addr(skb, IFA_ADDRESS, &ifa->peer_addr) < 0)
			goto error;
	} else
		if (nla_put_in6_addr(skb, IFA_ADDRESS, need_change ? &unif_ifaddr.addr : &ifa->addr) < 0)
			goto error;
	...
	if (nla_put_u32(skb, IFA_FLAGS, need_change ? unif_ifaddr.flags : ifa->flags) < 0)
		goto error;
...
const struct ctl_table addrconf_sysctl[] = {
	{
		.procname	= "forwarding",
...
EXPORT_SYMBOL_GPL(addrconf_sysctl);
...
static void addrconf_sysctl_unregister(struct inet6_dev *idev)
{
	...
	trace_ldk_vh_addrconf_sysctl_unregister(net, idev->dev->name);
