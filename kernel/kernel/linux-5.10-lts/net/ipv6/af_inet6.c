...
extern void correct_v6_saddr(struct net *net, struct in6_addr *sin6_addr);
...
#ifdef CONFIG_HUAWEI_NAT_TRAV
		correct_v4_saddr(net, &(addr->sin6_addr.s6_addr32[3])); // 3: v4addr
#endif
...
#ifdef CONFIG_HUAWEI_NAT_TRAV
			correct_v6_saddr(net, &addr->sin6_addr);
#endif
...
out:
	if (err) {
		if (addr_type == IPV6_ADDR_MAPPED)
			pr_info("__inet6_bind mapped port:%u, addr:%#x..%#x, return err:%d\n", ntohs(addr->sin6_port),
				addr->sin6_addr.s6_addr[12], addr->sin6_addr.s6_addr[15], err); // 12: v4addr head 15: v4addr tail
		else
			pr_info("__inet6_bind port:%u, addr:%#x..%#x, return err:%d\n", ntohs(addr->sin6_port),
				ntohl(addr->sin6_addr.s6_addr32[0]), ntohl(addr->sin6_addr.s6_addr32[3]), err); // 3: v6addr tail
	}
	if (flags & BIND_WITH_LOCK)
		release_sock(sk);
	return err;
...
#ifdef CONFIG_HUAWEI_NAT_TRAV
	try_change_to_uniform_sockaddr_in6(sin, sk);
#endif
