...
void print_v6_rule_add_ret_info(const struct fib_rule *rule, int ret)
{
    char *result = !ret ? "ok" : "fail";
    if (rule) {
        struct fib6_rule *v6rule = (struct fib6_rule *)rule;
        pr_info("ip6 rule add %s. ret:%d pref:%u, proto:%u. "
        ...
        pr_info("ip6 rule add %s ret:%d,\n", result, ret);
    }
}

void print_v6_rule_del_ret_info(const struct fib_rule *rule, int ret)
{
    char *result = !ret ? "ok" : "fail";
    if (rule) {
        struct fib6_rule *v6rule = (struct fib6_rule *)rule;
        pr_info("ip6 rule del %s. ret:%d pref:%u, proto:%u. "
            "iif:%d, oif:%d, uid:%u-%u, mark:%#x/%#x, ip_proto:%u, "
            "from-to:%#x..%#x/%d - %#x..%#x/%d, tc:%u. "
            "action:%u, table:%u, target:%u, flags:%#x\n",
            result, ret, rule->pref, rule->proto,rule->iifindex, rule->oifindex, rule->uid_range.start,
        ...
        pr_info("ip6 rule del %s ret:%d, not find rule\n", result, ret);
    }
}
