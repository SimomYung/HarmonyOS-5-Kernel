...
void print_v6_rule_add_ret_info(const struct fib_rule *rule, int ret, bool is_host)
{
    char *result = !ret ? "ok" : "fail";
    if (rule) {
        struct fib6_rule *v6rule = (struct fib6_rule *)rule;
        pr_info("ip6 rule add %s. ret:%d is_host:%d, pref:%u, proto:%u. "
            "iif:%d, oif:%d, uid:%u-%u, mark:%#x/%#x, ip_proto:%u, "
            "from-to:%#x..%#x/%d - %#x..%#x/%d, tc:%u. "
            ...
            rule->uid_range.end, rule->mark, rule->mark_mask, rule->ip_proto,ntohl(v6rule->src.addr.s6_addr32[0]),
            ntohl(v6rule->src.addr.s6_addr32[3]), v6rule->src.plen,ntohl(v6rule->dst.addr.s6_addr32[0]),
            ntohl(v6rule->dst.addr.s6_addr32[3]), v6rule->dst.plen, v6rule->tclass,
            rule->action, rule->table, rule->target, rule->flags);
    } else {
        pr_info("ip6 rule add %s ret:%d, is_host:%d\n", result, ret, is_host);
    }
}

void print_v6_rule_del_ret_info(const struct fib_rule *rule, int ret, bool is_host)
{
    char *result = !ret ? "ok" : "fail";
    if (rule) {
        struct fib6_rule *v6rule = (struct fib6_rule *)rule;
        pr_info("ip6 rule del %s. ret:%d is_host:%d, pref:%u, proto:%u. "
            "iif:%d, oif:%d, uid:%u-%u, mark:%#x/%#x, ip_proto:%u, "
            "from-to:%#x..%#x/%d - %#x..%#x/%d, tc:%u. "
            "action:%u, table:%u, target:%u, flags:%#x\n",
            result, ret, is_host, rule->pref, rule->proto,rule->iifindex, rule->oifindex, rule->uid_range.start,
        ...
        pr_info("ip6 rule del %s ret:%d, is_host:%d, not find rule\n", result, ret, is_host);
    }
}
