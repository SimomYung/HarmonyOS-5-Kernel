			...
			if (v < 0) {
				...
				if (v != XT_RETURN) {
					...
					if ((verdict & NF_VERDICT_MASK) == NF_DROP)
						trace_ldk_rvh_trace_iptable6_drop(state, table->name,
										  private, e, skb);
#endif
					break;
				}
...
#ifdef CONFIG_LIBLINUX
		if ((verdict & NF_VERDICT_MASK) == NF_DROP || acpar.hotdrop)
			trace_ldk_rvh_trace_iptable6_drop(state, table->name, private, e, skb);
#endif
...
do_replace(struct net *net, sockptr_t arg, unsigned int len)
{
	...
	pr_info("v6 iptable do_replace for name:%s, entry num:%u, valid hooks:%#x\n",
		tmp.name, tmp.num_entries, tmp.valid_hooks);
	/* overflow check */
...
do_ip6t_set_ctl(struct sock *sk, int cmd, sockptr_t arg, unsigned int len)
{
	...
	pr_info("do_ip6t_set_ctl X cmd:%d,  host net?:%d, return ret:%d\n", cmd, sock_net(sk) == &init_net, ret);
	return ret;
}
