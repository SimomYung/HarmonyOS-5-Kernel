...
static void print_ip6_route_add_ret_info(const struct fib6_config *cfg, int err, bool is_host)
{
	if (!cfg) {
		pr_info("ip6 route add fail, err:%d, is_host:%d, not generate cfg\n", err, is_host);
		return;
	}
	pr_info("ip6 route add %s, err:%d. is_host:%d, table:%u. type:%u, dst:%#x..%#x/%d, src:%#x..%#x/%d. "
		"oif:%d, gw:%#x..%#x. prefsrc:%#x..%#x. ptl:%u, flags:%#x\n",
		err ? "fail" : "ok", err, is_host, cfg->fc_table,cfg->fc_type,
		ntohl(cfg->fc_dst.s6_addr32[0]), ntohl(cfg->fc_dst.s6_addr32[3]), cfg->fc_dst_len,
		ntohl(cfg->fc_src.s6_addr32[0]), ntohl(cfg->fc_src.s6_addr32[3]), cfg->fc_src_len,
		cfg->fc_ifindex, ntohl(cfg->fc_gateway.s6_addr32[0]), ntohl(cfg->fc_gateway.s6_addr32[3]),
		ntohl(cfg->fc_prefsrc.s6_addr32[0]), ntohl(cfg->fc_prefsrc.s6_addr32[3]),
		cfg->fc_protocol, cfg->fc_flags);
}

static void print_ip6_route_del_ret_info(const struct fib6_config *cfg, int err, bool is_host)
{
	if (!cfg) {
		pr_info("ip6 route del fail, err:%d, is_host:%d, not generate cfg\n", err, is_host);
		return;
	}
	pr_info("ip6 route del %s, err:%d. is_host:%d, table:%u. dst:%#x..%#x/%d, src:%#x..%#x/%d, prefsrc:%#x..%#x. "
		"oif:%d, gw:%#x..%#x. ptl:%u, type:%u, flags:%#x\n",
		err ? "fail" : "ok", err, is_host, cfg->fc_table,
		ntohl(cfg->fc_dst.s6_addr32[0]), ntohl(cfg->fc_dst.s6_addr32[3]), cfg->fc_dst_len,
		ntohl(cfg->fc_src.s6_addr32[0]), ntohl(cfg->fc_src.s6_addr32[3]), cfg->fc_src_len,
		ntohl(cfg->fc_prefsrc.s6_addr32[0]), ntohl(cfg->fc_prefsrc.s6_addr32[3]),
		cfg->fc_ifindex, ntohl(cfg->fc_gateway.s6_addr32[0]), ntohl(cfg->fc_gateway.s6_addr32[3]),
		cfg->fc_protocol, cfg->fc_type, cfg->fc_flags);
}
...
int ipv6_route_ioctl(struct net *net, unsigned int cmd, struct in6_rtmsg *rtmsg)
{
	...
	case SIOCADDRT:
		...
		print_ip6_route_add_ret_info(&cfg, err, net_eq(net, &init_net));
		break;
	case SIOCDELRT:
		err = ip6_route_del(&cfg, NULL);
		print_ip6_route_del_ret_info(&cfg, err, net_eq(net, &init_net));
		break;
	}
	rtnl_unlock();
	return err;
			      ...
			      struct netlink_ext_ack *extack)
{
	...
	if (err < 0) {
		print_ip6_route_del_ret_info(NULL, err, net_eq(sock_net(skb->sk), &init_net));
		return err;
	}
	...
	if (cfg.fc_mp)
		...
		print_ip6_route_del_ret_info(&cfg, err, net_eq(sock_net(skb->sk), &init_net));
		return err;
	}
}
			      ...
			      struct netlink_ext_ack *extack)
{
	...
	err = rtm_to_fib6_config(skb, nlh, &cfg, extack);
	if (err < 0) {
		print_ip6_route_add_ret_info(NULL, err, net_eq(sock_net(skb->sk), &init_net));
		return err;
	}
	...
	if (cfg.fc_mp) {
		...
		print_ip6_route_add_ret_info(&cfg, err, net_eq(sock_net(skb->sk), &init_net));
		return err;
 	}
}
