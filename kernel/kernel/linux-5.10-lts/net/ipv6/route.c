...
static void print_ip6_route_add_ret_info(const struct fib6_config *cfg, int err)
{
	if (!cfg) {
		pr_info("ip6 route add fail, err:%d, not generate cfg\n", err);
		return;
	}
	pr_info("ip6 route add %s, err:%d. table:%u. type:%u, dst:%#x..%#x/%d, src:%#x..%#x/%d. "
		"oif:%d, gw:%#x..%#x. prefsrc:%#x..%#x. ptl:%u, flags:%#x\n",
		err ? "fail" : "ok", err, cfg->fc_table,cfg->fc_type,
...
static void print_ip6_route_del_ret_info(const struct fib6_config *cfg, int err)
{
	if (!cfg) {
		pr_info("ip6 route del fail, err:%d, not generate cfg\n", err);
		return;
	}
	pr_info("ip6 route del %s, err:%d. table:%u. dst:%#x..%#x/%d, src:%#x..%#x/%d, prefsrc:%#x..%#x. "
		"oif:%d, gw:%#x..%#x. ptl:%u, type:%u, flags:%#x\n",
		err ? "fail" : "ok", err, cfg->fc_table,
		ntohl(cfg->fc_dst.s6_addr32[0]), ntohl(cfg->fc_dst.s6_addr32[3]), cfg->fc_dst_len,
		ntohl(cfg->fc_src.s6_addr32[0]), ntohl(cfg->fc_src.s6_addr32[3]), cfg->fc_src_len,
		ntohl(cfg->fc_prefsrc.s6_addr32[0]), ntohl(cfg->fc_prefsrc.s6_addr32[3]),
		cfg->fc_ifindex, ntohl(cfg->fc_gateway.s6_addr32[0]), ntohl(cfg->fc_gateway.s6_addr32[3]),
...
int ipv6_route_ioctl(struct net *net, unsigned int cmd, struct in6_rtmsg *rtmsg)
{
	...
	case SIOCADDRT:
		...
		print_ip6_route_add_ret_info(&cfg,err);
		break;
	case SIOCDELRT:
		err = ip6_route_del(&cfg, NULL);
		print_ip6_route_del_ret_info(&cfg, err);
		break;
	}
	rtnl_unlock();
	return err;
			      ...
			      struct netlink_ext_ack *extack)
{
	...
	if (err < 0) {
		print_ip6_route_del_ret_info(NULL, err);
		return err;
	}
			      ...
			      struct netlink_ext_ack *extack)
{
	...
	err = rtm_to_fib6_config(skb, nlh, &cfg, extack);
	if (err < 0) {
		print_ip6_route_add_ret_info(NULL, err);
		return err;
	}
