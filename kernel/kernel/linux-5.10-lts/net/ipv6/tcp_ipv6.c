			      ...
			      int addr_len)
{
	...
	int ret = BPF_CGROUP_RUN_PROG_INET6_CONNECT(sk, uaddr);
	if (ret)
...
static void tcp_v6_mtu_reduced(struct sock *sk)
{
	...
	if (!dst)
		return;
#ifdef CONFIG_DPA_PROTO
	CALL_HCK_VH(dpa_tcp_mtu_reduced, sk, &skip_rtx);
#endif
...
#ifdef CONFIG_LIBLINUX
	trace_ldk_rvh_skb_drop_reason(sk, skb, SKB_TCP_SEND_RST);
#endif
...
csum_err:
#ifdef CONFIG_LIBLINUX
	trace_ldk_rvh_skb_drop_reason(sk, skb, SKB_TCP_INVALID_CSUM);
#endif
...
process:
#ifdef CONFIG_LIBLINUX
	trace_ldk_rvh_tcp_event_update(sk, skb, TCP_RCV, 0);
#endif
	if (sk->sk_state == TCP_TIME_WAIT)
		goto do_time_wait;
	...
	wifipro_update_ipv6_statistics(WIFIPRO_IPV6_INSEGS, skb, sk);
#endif
#ifdef CONFIG_DPA_PROTO
	if (dpa_add_backlog(sk, skb)) {
		skb_to_free = NULL;
	} else
#endif
	if (!sock_owned_by_user(sk)) {
