...
extern void try_change_to_uniform_sockaddr_in6(struct sockaddr_in6 *sin, struct sock *sk);
#endif
...
#ifdef CONFIG_HUAWEI_NAT_TRAV
		try_change_to_uniform_sockaddr_in6(sin6, sk);
#endif
			     ...
			     int addr_len)
{
	...
	int ret = BPF_CGROUP_RUN_PROG_INET6_CONNECT_LOCK(sk, uaddr);
	if (ret)
...
#ifdef CONFIG_HUAWEI_SMARTCOM
	if (sin6) {
		smartcom_bind2device(sk, (struct sockaddr *)sin6);
	} else { // dst addr is provided in udp sk connect()
		struct sockaddr_in6 smartcom_usin = {0};
		smartcom_usin.sin6_addr = *daddr;
		smartcom_usin.sin6_port = inet->inet_dport;
		smartcom_usin.sin6_family = AF_INET6;
