...
static void network_aware(int cur_uid, int cur_pid, bool is_recving)
{
...
void tcp_network_aware(bool is_recving)
{
	...
	return network_aware(current_uid().val, current->tgid, is_recving);
}

void sock_network_aware(struct sock *sk, bool is_recving)
{
	if (aware_net_ctrl.enable == 0)
		return;

	return network_aware(sock_i_uid(sk).val, current->tgid, is_recving);
}
...
void stat_bg_network_flow_x(bool is_recving, int len)
{
	...
	if (period == 0) {
		...
		for (i = 1; i < period; i++) {
			...
			if (memset_s(temp_net_stat_x, sizeof(*temp_net_stat_x), 0, sizeof(*temp_net_stat_x)) != EOK) {
				return;
			}
		}
		/* update cur data */
...
static void __stat_bg_network_flow(int cur_uid, int cur_pid, bool is_recving, int len)
{
	if (!is_bg(cur_uid)) {
...
{
	return __stat_bg_network_flow(current_uid().val, current->tgid, is_recving, len);
}

void sock_stat_bg_network_flow(struct sock *sk, bool is_recving, int len)
{
	return __stat_bg_network_flow(sock_i_uid(sk).val, current->tgid, is_recving, len);
}
	...
	size_t count, loff_t *ppos)
{
	...
	netinfo.bg_num = i;
	if (memset_s(&netinfo.bg_net_stat, sizeof(netinfo.bg_net_stat), 0, sizeof(netinfo.bg_net_stat)) != EOK) {
		err = -EFAULT;
		goto out;
	}
	reinit_bg_stats();
...
static void aware_net_exit(struct net *net)
{
	if (net == NULL)
		return;
	char awareBgUids[AWARE_PATH_MAX_LEN] = "";
	char awareBgPids[AWARE_PATH_MAX_LEN] = "";
	char awareFgUids[AWARE_PATH_MAX_LEN] = "";
	char awareFsCtrl[AWARE_PATH_MAX_LEN] = "";
	if (sprintf_s(awareBgUids, AWARE_PATH_MAX_LEN, "%s%s%s", AWARE_FS_NET_PATH, "/", AWARE_FS_BG_UIDS) < 0) {
		return;
	}
	if (sprintf_s(awareBgPids, AWARE_PATH_MAX_LEN, "%s%s%s", AWARE_FS_NET_PATH, "/", AWARE_FS_BG_PIDS) < 0) {
		return;
	}
	if (sprintf_s(awareFgUids, AWARE_PATH_MAX_LEN, "%s%s%s", AWARE_FS_NET_PATH, "/", AWARE_FS_FG_UIDS) < 0) {
		return;
	}
	if (sprintf_s(awareFsCtrl, AWARE_PATH_MAX_LEN, "%s%s%s", AWARE_FS_NET_PATH, "/", AWARE_FS_CTRL) < 0) {
		return;
	}
	remove_proc_entry(awareBgUids, net->proc_net);
	remove_proc_entry(awareBgPids, net->proc_net);
	remove_proc_entry(awareFgUids, net->proc_net);
	remove_proc_entry(awareFsCtrl, net->proc_net);
