...
#ifdef CONFIG_DPA_PROTO
int is_packet_rcv(void *fn)
{
	if (fn == (void *)tpacket_rcv || fn == (void *)packet_rcv)
		return 1;

	return 0;
}
EXPORT_SYMBOL(is_packet_rcv);
#endif
...
#ifdef CONFIG_DPA_PROTO
	CALL_HCK_VH(dpa_tcpdump_ctrl, false);
#endif
...
#else
	buffer = vmalloc_user(array_size((1 << order), PAGE_SIZE));
#endif
	if (buffer)
		return buffer;
		...
		int closing, int tx_ring)
{
	...
	if (was_running) {
		register_prot_hook(sk);
#ifdef CONFIG_DPA_PROTO
		CALL_HCK_VH(dpa_tcpdump_ctrl, true);
#endif
	}
	spin_unlock(&po->bind_lock);
	...
	for (rb = &po->rx_ring; rb <= &po->tx_ring; rb++) {
		...
		for (i = 0; i < rb->pg_vec_len; i++) {
			...
			for (pg_num = 0; pg_num < rb->pg_vec_pages; pg_num++) {
				...
				if (trace_ldk_rvh_sk_vm_insert_page_enabled())
					trace_ldk_rvh_sk_vm_insert_page(vma, start, kaddr, page, &err);
#endif
				if (unlikely(err))
					goto out;
				start += PAGE_SIZE;
