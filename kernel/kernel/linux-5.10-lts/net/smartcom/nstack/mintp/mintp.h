...
#define pr_sk_fmt(sk, fmt) "[nstack_mtp]%s[%d]%d:%d : " fmt, __func__, __LINE__, (sk)->src_port, (sk)->dst_port
...
struct mtp_skb_cb {
	...
	struct {
		...
		__u32 in_flight : 30, /* Bytes in flight at transmit */
		      ...
		      need_cache : 1; /* cache packets for hi1106 batch sending */
...
struct mtp_sock {
	...
	u64 last_rate_sample_mstamp; /* timestamp of previous valid rate sample */
	u64	burst_end_skb_mstamp;
	u64	burst_end_mstamp;
	u64	rate_stat_ignore_mstamp;
	...
	atomic_t msq_limit; /* limit mintp bytes in qdisc and NIC queue */
...
static inline void mtp_state_store(struct sock *sk, unsigned char newstate)
{
	smp_store_release(&sk->sk_state, newstate);
}

static inline unsigned char mtp_state_load(const struct sock *sk)
{
...
static inline bool mtp_is_writeable(struct sock *sk)
{
	return (sk->sk_sndbuf > sk->sk_wmem_queued) && sk_stream_is_writeable(sk);
}

#ifdef CONFIG_LIBLINUX
static inline void mtp_hm_wake_poll(struct sock *sk, bool was_writeable)
{
	if (!(sk->sk_shutdown & SEND_SHUTDOWN) && was_writeable && !mtp_is_writeable(sk))
...
extern int mtp_slow_start_after_idle;
