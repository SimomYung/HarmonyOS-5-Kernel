...
static void mtp_debug_undo(struct sock *sk, const char *msg)
{
	...
	mtp_sk_info(msk, "Undo %s c%u l%u ss%u/%u p%u\n", msg,
...
void mtp_init_cwnd_reduction(struct sock *sk)
{
	...
	mtp_sk_info(msk, "snd_cwnd %u snd_ssthresh %u\n", msk->snd_cwnd, msk->snd_ssthresh);
}
...
void mtp_cwnd_reduction(struct sock *sk, int newly_acked_sacked, u32 flag)
{
	...
	mtp_sk_debug(msk, "snd_cwnd %u snd_ssthresh %u prr_delivered %u prr_out %u\n",
		msk->snd_cwnd, msk->snd_ssthresh, msk->prr_delivered, msk->prr_out);
}
...
void mtp_enter_cwr(struct sock *sk)
{
	struct mtp_sock *msk = mtp_sk(sk);
...
void mtp_enter_recovery(struct sock *sk)
{
	...
	mtp_sk_info(msk, "set prior_ssthresh to %u cwnd %u in_acks %u out_msq_limit %u\n",
...
void mtp_update_reordering(struct sock *sk, const int metric)
{
	...
	if (metric > msk->reordering) {
		...
		mtp_sk_info(msk, "Disorder %s %u f%u s%u undo%u\n",
...
static void mtp_update_pacing_rate(struct sock *sk)
{
	...
	mtp_sk_debug(msk, "sk_pacing_rate %lu\n", rate);
}
...
void mtp_init_congestion_control(struct sock *sk)
{
	...
	msk->snd_cwnd_stamp = MTP_JIFFIERS32;
	mtp_sk_info(msk, "snd_cwnd init %u\n", msk->snd_cwnd);
	msk->snd_cwnd_cnt = 0;
	msk->prior_ssthresh = 0;
