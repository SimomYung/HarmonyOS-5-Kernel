...
static void mtp_rtt_estimator(struct sock *sk, long mrtt_us)
{
	...
	mtp_sk_debug(msk, "skb rtt_seq %u get srtt_us %u mdev_us %u snd_una %u\n",
...
static void mtp_skb_acked(struct sock *sk, struct sk_buff *skb)
{
...
static u32 mtp_write_queue_skb_acked(struct sock *sk, struct sk_buff *skb,
	u64 *first_ackt, u64 *last_ackt, struct mtp_sacktag_state *sack)
{
	...
	mtp_sk_debug(msk, "skb seq %u acked\n", dcb->seq);
	msk->packets_out--;
	mtp_rate_skb_delivered(sk, skb, sack->rate);
	...
	u32 prior_snd_una, int *acked, struct mtp_sacktag_state *sack)
{
	...
	while ((skb = mtp_write_queue_head(sk)) && skb != mtp_send_head(sk)) {
		...
		flag |= mtp_write_queue_skb_acked(sk, skb, &first_ackt, &last_ackt, sack);
...
check_next:
		...
		fack_count += 1;
		mtp_sk_debug(msk, "%u~%u MTPCB_SACKED_ACKED fack_count %d sacked_out %d\n",
			start_seq, end_seq, fack_count, msk->sacked_out);
					...
					u32 start_seq, u32 end_seq)
{
	...
	mtp_for_write_queue_from(skb, sk) {
		...
		if (in_sack) {
			...
			mtp_rate_skb_delivered(sk, skb, state->rate);
	...
	struct mtp_sack_block *sp, const int sp_len)
{
	...
	for (i = 0; i < num_sacks; i++) {
		...
		mtp_sk_debug(msk, "recv sack block %d %u~%u\n", used_sacks,
	...
	for (this_sack = 0; this_sack < num_sacks;) {
		if (!before(msk->rcv_nxt, sp->start_seq)) {
			...
			if (before(msk->rcv_nxt, sp->end_seq))
				mtp_sk_debug(msk, "rcv_nxt %u must cover end_seq %u\n",
					msk->rcv_nxt, sp->end_seq);
...
static void mtp_mark_head_lost(struct sock *sk, int packets, int mark_head)
{
	...
	mtp_for_write_queue_from(skb, sk) {
		...
		if (!(MTP_SKB_CB(skb)->sacked & (MTPCB_LOST | MTPCB_SACKED_ACKED))) {
			...
			mtp_sk_debug(msk, "mark %u~%u lost cnt %d\n", MTP_SKB_CB(skb)->seq,
				MTP_SKB_CB(skb)->end_seq, cnt);
		}
	...
	if (msk->lost_out > origin_lost_out)
		mtp_sk_info(msk, "origin lost_out %u now %u\n", origin_lost_out, msk->lost_out);
}
...
static void mtp_fin(struct sock *sk)
{
	...
	case TCP_FIN_WAIT2:
		...
		break;
	default:
		mtp_sk_err(msk, "Impossible, sk->sk_state=%d\n", sk->sk_state);
		break;
	}
...
static int mtp_prior_ack_recv(struct sock *sk, const struct sk_buff *skb, struct mtp_sacktag_state *sack_state)
{
	struct mtp_sock *msk = mtp_sk(sk);
...
static int mtp_ack(struct sock *sk, const struct sk_buff *skb)
{
	...
	int acked = 0; /* Number of packets newly acked */
	int rexmit = REXMIT_NONE; /* Flag to (re)transmit to recover losses */
	struct mtp_rate_sample rs = { .prior_delivered = 0 };
	...
	if (before(MTP_SKB_CB(skb)->ack_seq, msk->snd_una))
		return mtp_prior_ack_recv(sk, skb, &sack_state);
	...
	mtp_sk_debug(msk, "flag %x rexmit %d\n", flag, rexmit);
	delivered = msk->delivered - delivered; /* freshly ACKed or SACKed */
	mtp_rate_gen(sk, delivered, sack_state.rate);
...
static int mtp_receive_queue(struct sock *sk, struct sk_buff *skb)
{
	...
	if (unlikely(sock_flag(sk, SOCK_DEAD) && skb->len)) {
		mtp_sk_err(msk, "sock is dead, can't recv data\n");
		return -1;
	}

	msk->cur_rcv_msg_size += skb->len;
	if (unlikely(msk->cur_rcv_msg_size > msk->max_msg_size)) {
		mtp_sk_err(msk, "cur_msg_size %u larger than %u\n",
	...
	if (!sock_flag(sk, SOCK_DEAD) && (mcb->type == MTPHDR_TYPE_MSG_END ||
		...
		sk->sk_data_ready(sk);
		mtp_sk_debug(msk, "sk_data_ready msg_size %u\n", msk->cur_rcv_msg_size);
...
static int mtp_ofo_queue(struct sock *sk)
{
	...
	while (p) {
		...
		if (unlikely(!after(MTP_SKB_CB(skb)->end_seq, msk->rcv_nxt))) {
			mtp_sk_debug(msk, "ofo packet was already received\n");
			mtp_drop(sk, skb);
			continue;
		}
		mtp_sk_debug(msk, "ofo requeuing: rcv_next %X seq %X - %X\n",
		...
		*parent = **p;
		...
		if (before(seq, MTP_SKB_CB(skb1)->end_seq)) {
			if (!after(end_seq, MTP_SKB_CB(skb1)->end_seq)) {
				...
				mtp_sk_debug(msk, "already recevied\n");
				mtp_drop(sk, skb);
				return MTP_DATA_OFO_RETURN;
			}
			if (after(seq, MTP_SKB_CB(skb1)->seq)) {
				/* Partial overlap. */
				mtp_sk_debug(msk, "Partial overlap\n");
			} else {
				mtp_rtx(msk, skb);
...
static void mtp_data_queue_ofo(struct sock *sk, struct sk_buff *skb)
{
	...
	seq = MTP_SKB_CB(skb)->seq;
	end_seq = MTP_SKB_CB(skb)->end_seq;
	mtp_sk_debug(msk, "out of order segment: rcv_next %X seq %X - %X\n",
		   msk->rcv_nxt, seq, end_seq);
...
insert:
	...
	mtp_sk_debug(msk, "Insert out_of_order_queue\n");
	mtp_rtx(msk, skb);
...
coalesce_done:
	mtp_sk_debug(msk, "mtp_try_coalesce\n");
	mtp_rtx(msk, skb);
...
static void mtp_reset(struct sock *sk)
{
	...
	mtp_sk_err(msk, "recv reset on state %u\n", sk->sk_state);
	switch (sk->sk_state) {
	case TCP_SYN_SENT:
...
static int mtp_syn_sent_rcv_check(struct sock *sk, const struct sk_buff *skb)
{
	...
	if (!after(ack, msk->snd_una) || after(ack, msk->snd_nxt)) {
		mtp_sk_info(msk, "ack_seq invalid\n");
		return -1;
	}

	if (mcb->type == MTPHDR_TYPE_RST) {
		mtp_sk_info(msk, "reset is set\n");
		mtp_reset(sk);
		return -1;
	}

	if (mcb->type != MTPHDR_TYPE_HANDSHAKE) {
		mtp_sk_info(msk, "recv unexpect type %u\n", mcb->type);
		return -1;
	}

	if (recv_mss < MTP_MIN_MSS) {
		mtp_sk_info(msk, "mss should at least %d\n", MTP_MIN_MSS);
		mtp_reset(sk);
		return -1;
	}

	if (!recv_msg_size) {
		mtp_sk_info(msk, "recv_msg_size should be set\n");
		mtp_reset(sk);
		return -1;
	}
	if (msk->mss > recv_mss)
	...
	mtp_sk_debug(msk, "max_msg_size %u sk_sndbuf %u\n", msk->max_msg_size, sk->sk_sndbuf);
	return 0;
}
...
static int mtp_syn_sent_rcv(struct sock *sk, struct sk_buff *skb)
{
	...
	msk->snd_cwnd = msk->cong_cfg.init_cwnd ? : MTP_INIT_CWND;
...
static void mtp_check_write(struct sock *sk)
{
	struct mtp_sock *msk = mtp_sk(sk);
...
static int do_rcv_seq(struct sock *sk, struct sk_buff *skb, u32 seq)
{
	...
	if (before(MTP_SKB_CB(skb)->ack_seq, msk->snd_una) &&
	    before(MTP_SKB_CB(skb)->ack_seq, msk->snd_una - sk->sk_sndbuf))
		goto drop;
...
disconnect:
		...
		if (after(seq, msk->rcv_nxt + msk->mss * msk->rcv_wnd))
			return -1;
		mtp_sk_debug(msk, "ofo seq %u rcv_nxt %u\n", seq, msk->rcv_nxt);
		mtp_ack(sk, skb);
		if (len || is_fin) {
...
int mtp_l2_do_rcv(struct sock *sk, struct sk_buff *skb)
{
	...
	if (after(ack, msk->snd_nxt)) {
		mtp_sk_err(msk, "ack is invalid, ack %u snd_nxt %u\n", ack, msk->snd_nxt);
		goto discard;
	}
...
static struct sock *mtp_accept_queue_add(struct sock *sk, struct sock *nsk)
{
	...
	if (unlikely(sk->sk_state != TCP_LISTEN)) {
		...
		sk_acceptq_added(sk);
		mtp_sk_info(msk, "sk_ack_backlog %d\n", sk->sk_ack_backlog);
	}
	...
	const struct mtphdr *mtph)
{
	...
	if (sk_acceptq_is_full(sk)) {
		mtp_sk_err(msk, "sk_acceptq_is_full %u\n", sk->sk_ack_backlog);
		mtp_reply_reset(skb);
		mtp_drop(sk, skb);
		return;
	}

	if (memcmp(msk->src_mac, eth->h_source, ETH_ALEN) == 0)
		is_loopback = true;
...
static void mtp_listen_rcv_ack(struct sock *sk, struct sk_buff *skb)
{
	...
	if (recv_mss < MTP_MIN_MSS) {
		mtp_sk_err(mtp_sk(sk), "mss should at least %d\n", MTP_MIN_MSS);
		goto discard;
	}

	if (!recv_msg_size) {
		mtp_sk_err(mtp_sk(sk), "recv_msg_size should set\n");
		goto discard;
	}
...
drop:
		mtp_sk_err(mtp_sk(sk), "recv invalid packet\n");
		mtp_drop(sk, skb);
	}
}
	       ...
	       struct packet_type *pt, struct net_device *orig_dev)
{
	...
	if (sock_owned_by_user(sk)) {
		if (mtp_add_backlog(sk, skb)) {
			mtp_sk_err(mtp_sk(sk), "mtp_add_backlog fail\n");
