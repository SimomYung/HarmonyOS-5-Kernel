...
void mtp_msq_handler(struct sock *sk)
{
	...
	if (mtp_state_load(sk) == TCP_CLOSE)
		return;
...
static void mtp_tasklet_func(unsigned long data)
{
	...
	list_for_each_safe(q, n, &list) {
		...
		sk = (struct sock *)msk;
		mtp_sk_debug(msk, "tasklet called\n");
				  ...
				  unsigned int factor, struct net_device *dev)
{
	...
	limit = max_t(unsigned long, MTP_BATCH_SENDING_MAX * skb->truesize, atomic_read(&msk->msq_limit));
	limit = min_t(unsigned long, limit, mtp_limit_output_bytes);
	limit <<= factor;
...
static void mtp_free_cache_list(struct sock *sk, struct sk_buff *skb)
{
	/* skip first skb */
	skb = GET_NEXT_MTP_FRAME(skb);

	while (skb) {
		struct sk_buff *tmp_skb = GET_NEXT_MTP_FRAME(skb);
		RESET_MTP_FRAME_PTR(skb);
		skb->destructor = NULL;
		skb->sk = NULL;
		(void)refcount_sub_and_test(skb->truesize, &sk->sk_wmem_alloc);
		kfree_skb(skb);
		skb = tmp_skb;
	}
}

#define PACKET_TO_BYTES_SHIFT 11  /* 2048 byte per packet */

static void mtp_update_burst_info(struct mtp_sock *msk, struct sk_buff *skb)
{
	u64 cur_us = mtp_clock_us();
	u64 skb_us = mtp_skb_timestamp_us(skb);
	int msk_msq_limit = atomic_read(&msk->msq_limit);
	u32 app_limited = READ_ONCE(msk->app_limited);
	u32 snd_cwnd = READ_ONCE(msk->snd_cwnd);
	if (cur_us - msk->burst_end_mstamp > MAX_BURST_INTERVAL && skb_us > msk->burst_end_skb_mstamp &&
		(!app_limited) && ((u64)snd_cwnd  << PACKET_TO_BYTES_SHIFT > msk_msq_limit)) {
		atomic_add(msk_msq_limit >> 1, &msk->msq_limit);
		msk->rate_stat_ignore_mstamp = cur_us;
	}
	msk->burst_end_mstamp = cur_us;
	msk->burst_end_skb_mstamp = skb_us;
}
...
static void mtp_wfree(struct sk_buff *skb)
{
	...
	bool empty;
	mtp_update_burst_info(msk, skb);

	mtp_free_cache_list(sk, skb);
	RESET_MTP_FRAME_PTR(skb);
	if (refcount_sub_and_test(skb->truesize - 1, &sk->sk_wmem_alloc))
		mtp_sk_err(msk, "sk_wmem_alloc should not 0\n");

	limit = atomic_read(&msk->msq_limit);
	limit = max_t(unsigned long, MTP_BATCH_SENDING_MAX * skb->truesize, limit);
	limit = min_t(unsigned long, limit, mtp_limit_output_bytes);
	if (refcount_read(&sk->sk_wmem_alloc) >= (limit >> 1))
		goto out;
	...
	if (empty) {
		mtp_sk_debug(msk, "tasklet_schedule msq\n");
		...
		*ptr++ = htonl((MTPOPT_NOP  << MTP_OPT_OFFSET_24) |
		...
		for (this_sack = 0; this_sack < opts->num_sack_blocks; ++this_sack) {
			...
			*ptr++ = htonl(sp[this_sack].end_seq);
			mtp_sk_debug(msk, "send sack block %d %u~%u\n", this_sack,
...
{
	/* send packet at once */
	if (MTP_SKB_CB(skb)->tx.need_cache == MTP_NOCACHE && msk->tx_tmp_list == NULL)
		return msk->xmit(skb);

	/* caches packet in the queue */
	mtp_cache_pkt(msk, skb);
	/* need send packets in the queue? */
	if ((MTP_SKB_CB(skb)->tx.need_cache == MTP_NOCACHE) || (msk->tx_tmp_list_len >= MTP_BATCH_SENDING_MAX))
...
static int mtp_xmit_skb(struct mtp_sock *msk, struct sk_buff *skb)
{
	...
	if (unlikely(((struct sock *)msk)->sk_state == TCP_SYN_SENT && mtp_mac_any(msk->dst_mac, ETH_ALEN))) {
		...
		u32 nexthop4 = (__force u32)msk->inet.inet_daddr;
		struct in6_addr *nexthop6 = &msk->inet.sk.sk_v6_daddr;
		rcu_read_lock_bh();
		neigh = nexthop4 ? __ipv4_neigh_lookup_noref(dev, nexthop4) :
			__ipv6_neigh_lookup_noref(dev, nexthop6);
		if (!IS_ERR_OR_NULL(neigh)) {
			...
			neigh = __neigh_create(nexthop4 ? &arp_tbl : &nd_tbl, nexthop4 ? (void *)&nexthop4 : (void *)nexthop6,
				dev, false);
		}
...
static struct sk_buff *mtp_clone_skb_for_transmit(struct sk_buff *skb, int clone_it,
	gfp_t gfp_mask, struct mtp_sock *msk, struct sk_buff **oskb)
{
	*oskb = NULL;
	if (clone_it) {
		*oskb = skb;
				...
				struct mtp_sock *msk, u32 rcv_nxt)
{
	...
	if (dcb->type == MTPHDR_TYPE_SYN || dcb->type == MTPHDR_TYPE_HANDSHAKE || dcb->type == MTPHDR_TYPE_FIN)
		data_len -= 1;
	if (msk->ca_recovery) {
		mtp_sk_debug(msk, "set MTPOPT_FLAG_REC\n");
...
static void mtp_skb_priority(struct sock *sk, struct sk_buff *skb)
{
	struct mtp_skb_cb *dcb = MTP_SKB_CB(skb);

	skb->priority = sk->sk_priority;
	if (skb->priority != TC_PRIO_BULK)
		return;

	if (dcb->type == MTPHDR_TYPE_SYN || dcb->type == MTPHDR_TYPE_HANDSHAKE || (dcb->flags & MTPHDR_RTX))
				...
				int clone_it, gfp_t gfp_mask, u32 rcv_nxt)
{
	...
	skb = mtp_clone_skb_for_transmit(skb, clone_it, gfp_mask, msk, &oskb);
	if (skb == NULL)
		return -ENOBUFS;
...
int mtp_retransmit_skb(struct sock *sk, struct sk_buff *skb)
{
	...
	if (unlikely(skb_fclone_busy(sk, skb))) {
		if (msk->ca_state != MTP_CA_LOSS) {
			return -EBUSY;
		} else {
			struct sk_buff_fclones *fclones = container_of(skb, struct sk_buff_fclones, skb1);
			struct sk_buff *skb2 = &fclones->skb2;
			mtp_sk_warn(msk, "retransmits %u skb_fclone_busy fclone %u:%u fclone_ref %d user %d\n",
				msk->rtx.retransmits, skb->fclone, skb2->fclone,
				refcount_read(&fclones->fclone_ref),
				refcount_read(&skb2->users));
		}
	}

	MTP_SKB_CB(skb)->flags |= MTPHDR_RTX;
	MTP_SKB_CB(skb)->tx.need_cache = MTP_NOCACHE;
...
static void mtp_write_queue(struct sock *sk, struct sk_buff *skb, struct net_device *dev)
{
	...
	mtp_for_write_queue_from(skb, sk) {
		...
		if (mtp_small_queue_check(sk, skb, 1, dev)) {
			mtp_sk_info(msk, "mtp_small_queue_check fail\n");
			break;
		}
		skb->dev = dev;
		...
		if (skb == mtp_write_queue_head(sk)) {
			...
			mtp_sk_info(msk, "seq %u rto %u timeout %u\n",
...
void mtp_rearm_rto(struct sock *sk)
{
	...
	if (!msk->packets_out) {
		...
		mtp_sk_debug(msk, "rto %u timeout %u\n", msk->rtx.rto, msk->rtx.timeout);
		sk_reset_timer(sk, &msk->rtx.timer, msk->rtx.timeout);
	}
}
...
static void mtp_event_new_data_sent(struct sock *sk, const struct sk_buff *skb)
{
	...
	if (!prior_packets || msk->rtx.pend == MTP_RTX_TIMER_LOSS_PROBE) {
		mtp_rearm_rto(sk);
		mtp_sk_debug(msk, "mtp_rearm_rto send_head %d\n", sk->sk_send_head ? 1 : 0);
	}
}
 ...
 */
static void mtp_cwnd_application_limited(struct sock *sk)
{
	struct mtp_sock *msk = mtp_sk(sk);
	...
	if (msk->ca_state == MTP_CA_OPEN &&
		...
		u32 init_win = MTP_INIT_CWND;
		u32 win_used = max(msk->snd_cwnd_used, init_win);
		if (win_used < msk->snd_cwnd) {
			msk->snd_ssthresh = mtp_current_ssthresh(sk);
			msk->snd_cwnd = (msk->snd_cwnd + win_used) >> 1;
			if (msk->is_hi1106 && !msk->batch_send)
				msk->batch_send = 1;
			mtp_sk_debug(msk, "set cwnd %u ssthresh %u batch_send %u\n",
				msk->snd_cwnd, msk->snd_ssthresh, msk->batch_send);
		}
		msk->snd_cwnd_used = 0;
	}
...
static void mtp_cwnd_validate(struct sock *sk, bool ca_cwnd_limited)
{
	...
	if (!before(msk->snd_una, msk->max_packets_seq) ||
		...
		mtp_sk_debug(msk, "ca_cwnd_limited %d\n", ca_cwnd_limited);
	}
	...
	if (mtp_is_cwnd_limited(sk)) {
		...
		/* Network starves. */
		if (msk->packets_out > msk->snd_cwnd_used)
			msk->snd_cwnd_used = msk->packets_out;

		if (mtp_slow_start_after_idle &&
		    (s32)(MTP_JIFFIERS32 - msk->snd_cwnd_stamp) >= msk->rtx.rto)
			mtp_cwnd_application_limited(sk);
	}
}
...
void mtp_init_probe_timer(struct sock *sk)
{
	struct mtp_sock *msk = mtp_sk(sk);
	...
	msk->rtx.timeout = jiffies + msk->rtx.rto;
	mtp_sk_debug(msk, "MTP_RTX_TIMER_PROBE timer %u\n", msk->rtx.rto);
	sk_reset_timer(sk, &msk->rtx.timer, msk->rtx.timeout);
}
...
void mtp_write_xmit(struct sock *sk, int push_one, gfp_t gfp)
{
	...
	while ((skb = mtp_send_head(sk))) {
		...
		skb->dev = dev;
		MTP_SKB_CB(skb)->tx.need_cache = (push_one ? MTP_NOCACHE : MTP_CACHE);
		if (unlikely(mtp_transmit_skb(sk, skb, 1, gfp, msk->rcv_nxt))) {
			mtp_sk_info(msk, "mtp_transmit_skb fail\n");
			break;
		}
	...
	if (!push_one && msk->batch_send)
		mtp_xmit_cache(msk);

	if (!mtp_send_head(sk) && !push_one)
		msk->stats.out_app_limit++;
	dev_put(dev);
	is_cwnd_limited = (mtp_packets_in_flight(msk) >= msk->snd_cwnd);
	if (likely(sent_pkts || is_cwnd_limited))
		mtp_cwnd_validate(sk, is_cwnd_limited);
	if (likely(sent_pkts)) {
		if (mtp_in_cwnd_reduction(sk))
...
bool mtp_schedule_loss_probe(struct sock *sk, bool advancing_rto)
{
	 ...
	 */
	if (!msk->packets_out || (msk->ca_state != MTP_CA_OPEN && msk->ca_state != MTP_CA_CWR))
		return false;
	...
	mtp_sk_debug(msk, "TLP timeout %u\n", timeout);
	sk_reset_timer(sk, &msk->rtx.timer, msk->rtx.timeout);
	return true;
}
...
void mtp_send_ack(struct sock *sk, u8 probe)
{
	...
	mtp_sk_debug(msk, "send ack sk_state %u rcv_nxt %u\n", sk->sk_state, msk->rcv_nxt);
	...
	if (unlikely(!skb)) {
		mtp_sk_err(mtp_sk(sk), "alloc_skb failed\n");
...
void mtp_send_synack(struct sock *sk, u32 seq, u32 ack_seq)
{
	...
	mtp_sk_debug(msk, "send synack\n");
	if (sock_flag(sk, SOCK_DEAD)) {
		mtp_sk_info(msk, "SOCK_DEAD\n");
		return;
	}
	...
	if (mtp_state_load(sk) == TCP_SYN_SENT) {
		...
		skb = alloc_skb(MAX_MTP_HEADER, sk_gfp_mask(sk, GFP_ATOMIC | __GFP_NOWARN));
		if (unlikely(!skb)) {
			mtp_sk_err(msk, "alloc_skb failed\n");
			return;
		}
...
void mtp_release_cb(struct sock *sk)
{
	...
	mtp_sk_debug(mtp_sk(sk), "flags %x\n", flags);
	...
	if (flags & MTPF_WRITE_TIMER_DEFERRED) {
		...
		mtp_sk_debug(mtp_sk(sk), "MTPF_WRITE_TIMER_DEFERRED\n");
		__sock_put(sk);
	}

	if (flags & MTPF_DELACK_TIMER_DEFERRED) {
		mtp_delack_timer_handler(sk);
		mtp_sk_debug(mtp_sk(sk), "MTP_DELACK_TIMER_DEFERRED\n");
		__sock_put(sk);
	}
}
