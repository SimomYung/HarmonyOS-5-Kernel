...
static int mtp_tls_set_prot(struct mtp_tls_info *prot, struct tls_crypto_info *crypto_info, u16 mss)
{
	if (!mss) {
		mtp_info("mss %hu invalid\n", mss);
		return -EINVAL;
	}
	...
	if (mss >= TLS_MAX_PAYLOAD_SIZE + prot->overhead_size) {
		prot->record_size = TLS_MAX_PAYLOAD_SIZE + prot->overhead_size;
	} else {
...
static int mtp_tls_set_sw_offload(struct sock *sk, struct mtp_tls_context *ctx, struct mtp_tls_aead *tls_aead)
{
	...
	rc = mtp_tls_set_prot(prot, crypto_info, mtp_sk(sk)->mss);
	if (rc)
		return rc;
...
static int mtp_setsockopt_tls_conf(struct sock *sk, sockptr_t optval, unsigned int optlen, int tx)
{
	...
	mtp_sk_info(msk, "mtp_tls_set_sw_offload tx %d ret %d\n", tx, rc);
	if (!rc) {
	...
	int required_size, char **prepend_buf)
{
	...
	while (required_size) {
		...
		mtp_sk_debug(msk, "i %d: skb_size %d len %d\n", i, skb_size, skb->len);
		if (!required_size)
	...
	struct mtp_tls_rec *rec, struct mtp_tls_info *prot_info)
{
	...
	if (err) {
		...
		mtp_sk_debug(mtp_sk(sk), "mtp_sg_from_iter err %d\n", err);
		if (skb_copy_err) {
			mtp_sk_err(mtp_sk(sk), "skb_copy_err %d\n", skb_copy_err);
			sg_init_table(rec->sg_en, TLS_SG_MAX);
			__skb_queue_purge(&rec->skb_queue);
			return skb_copy_err;
		}
	...
	int i, struct sk_buff *skb)
{
	...
	if (!i) {
		...
		if (offset + prot_info->prepend_size > skb->len) {
			mtp_sk_err(msk, "prepend_buf error offset %u len %u data_len %u\n",
		...
		if (ret < 0) {
			mtp_sk_err(msk, "skb_copy_bits error offset %u len %u data_len %u\n",
				offset, skb->len, skb->data_len);
			return -EBADMSG;
		}
...
static s16 mtp_recv_get_record(struct sock *sk, bool *read_finish, u16 *copied_skb, int *seg)
{
	...
	skb_queue_walk_safe(&sk->sk_receive_queue, skb, tmp) {
		...
		i += sg_used;
		mtp_sk_debug(msk, "i %d record_size %hu sg_used %d record_unread %hu\n",
			i, record_size, sg_used, record_unread);
		if ((msk->copied_seq - MTP_SKB_CB(skb)->seq) < skb->len) {
			mtp_sk_debug(msk, "skb is partially read");
			break;
		}
		*copied_skb += 1;
		...
		if (type == MTPHDR_TYPE_MSG_END || type == MTPHDR_TYPE_FIN) {
			*read_finish = true;
			if (record_unread) {
				mtp_sk_err(msk, "record_unread left %u type %u record_size %u\n",
	...
	if (record_unread) {
		mtp_sk_err(msk, "record_unread left %u\n", record_unread);
read_failure:
...
static int mtp_decrypt(struct mtp_tls_aead *rx, struct mtp_tls_rec *rec, struct mtp_tls_info *prot_info, u16 user_size)
{
	...
	if (err < 0) {
		if (err != -EINPROGRESS) {
			mtp_err("fail err %d sg_pl_frag_used %d sg_pl_frag_iter %d sg_pl_len %d user_size%hu\n",
				err, rec->sg_pl_frag_used, rec->sg_pl_frag_iter, rec->sg_pl_len, user_size);
...
static int mtp_recv_sgout_skb_append(struct sock *sk, struct mtp_tls_rec *rec, s16 *sg_out_idx, int len)
{
	...
	skb_queue_walk(&rec->skb_queue, skb) {
		...
		if (sg_used < 0) {
			mtp_sk_err(mtp_sk(sk), "skb_to_sgvec_nomark error %d\n", sg_used);
			mtp_tls_err_recv(sk, rec);
			return sg_used;
		}
	...
	if (len) {
		...
		tls_cb = skb_tls_cb(skb);
		if (len >= tls_cb->len) {
			mtp_sk_err(mtp_sk(sk), "len %d bigger than tls_cb->len %d\n", len, tls_cb->len);
			mtp_tls_err_recv(sk, rec);
			return -EBADMSG;
		}
	...
	mtp_sk_debug(mtp_sk(sk), "user_data_len %d sg_out_idx %d\n", rec->user_data_len, *sg_out_idx);
exit:
...
static int mtp_recv_decrypted_skb(struct sock *sk, struct msghdr *msg, struct mtp_tls_rec *rec, size_t len)
{
	...
	if (copy_len) {
		...
		if (copy_len >= tls_cb->len) {
			mtp_sk_err(mtp_sk(sk), "copy_len %d tls_cb->len %d offset %d\n", copy_len, tls_cb->len, tls_cb->offset);
			mtp_tls_err_recv(sk, rec);
			return -EBADMSG;
		}

		err = skb_copy_datagram_msg(skb, tls_cb->offset + rec->user_data_offset, msg, copy_len);
		if (err < 0)
			return err;
		copied += copy_len;
...
static int copy_iter_from_decrypted_skb(struct sock *sk, struct msghdr *msg,
	struct mtp_tls_rec *rec, size_t len, s16 user_size)
{
	int decrypted_size = mtp_recv_decrypted_skb(sk, msg, rec, len);
	mtp_sk_debug(mtp_sk(sk), "len %d decrypted_size %d\n", len, decrypted_size);
	if (decrypted_size < 0) {
		mtp_sk_debug(mtp_sk(sk), "mtp_recv_decrypted_skb failed %d\n", decrypted_size);
		return -EBADMSG;
	}
...
static s16 mtp_tls_recv_record(struct sock *sk, struct msghdr *msg, size_t len, u16 *copied_skb, bool *read_finish)
{
	...
	if (user_size > recv_size) {
		...
		if (err) {
			mtp_sk_err(mtp_sk(sk), "mtp_recv_sgout_skb_append err %d\n", err);
			mtp_sg_put_page(rec->sg_pl, rec->sg_pl_frag_iter);
			sg_init_table(rec->sg_pl, TLS_SG_MAX);
			mtp_tls_err_recv(sk, rec);
			return -EBADMSG;
	...
	if (user_size <= recv_size)
		__skb_queue_purge(&rec->skb_queue);
	if (err) {
		mtp_sk_err(mtp_sk(sk), "record_size %d sg_en_cnt %d recv_size %d\n",
			record_size, sg_en_cnt, recv_size);
		iov_iter_revert(&msg->msg_iter, recv_size);
		mtp_tls_err_recv(sk, rec);
		return -EBADMSG;
	}
	if (recv_size == 0)
...
int mtp_tls_recv_queue(struct sock *sk, struct msghdr *msg, size_t len, u16 *copied_skb)
{
	...
	if (rec->user_data_len) {
		...
		if (copy_len < 0)
			return copy_len;
		mtp_sk_debug(msk, "mtp_recv_decrypted_skb %d\n", copy_len);
		len -= copy_len;
		copied += copy_len;
	...
	while (len > 0 && !read_finish) {
		...
		if (copied > mtp_sk(sk)->max_msg_size) {
			mtp_sk_err(msk, "msg size too big, copied %d max_msg_size %u\n", copied, msk->max_msg_size);
			break;
		}
		len -= recv_size;
	}
...
void mtp_tls_send_pending_page(struct sock *sk)
{
	...
	if (err) {
		mtp_sk_err(msk, "mtp_send_skb_alloc failed\n");
		sg_chain(rec->sg_aead_in, 2, rec->sg_pl);
		goto free_pages;
	}
	...
	if (err) {
		mtp_sk_err(msk, "mtp_encrypt failed\n");
		__skb_queue_purge(&rec->skb_queue);
		goto free_pages;
	}
...
static void mtp_sg_pl_page_add(struct mtp_tls_rec *rec, struct page *page, size_t size)
{
	get_page(page);
	rec->sg_pl_frag_used++;
...
static void mtp_sg_pl_page_dec(struct mtp_tls_rec *rec, struct page *page, size_t size)
{
	rec->sg_pl_len -= size;
	rec->sg_pl_frag_used--;
	mtp_put_page(page);
...
static size_t mtp_tls_sendpage_size_adj(size_t size, struct mtp_tls_info *prot_info, struct mtp_tls_rec *rec)
{
	size_t adj_size = size;
	if (adj_size > (prot_info->record_size - prot_info->overhead_size - rec->sg_pl_len))
		adj_size = prot_info->record_size - prot_info->overhead_size - rec->sg_pl_len;
	return adj_size;
}
...
int mtp_tls_sendpage_locked(struct sock *sk, struct page *page, int offset, size_t size, int flags)
{
	...
	if (err) {
		...
		__skb_queue_purge(&rec->skb_queue);
error:
		mtp_sg_pl_page_dec(rec, page, size);
		return err;
	}
	...
	mtp_sk_debug(msk, "send segments %d ret %zu\n", seg, size);
#ifdef CONFIG_LIBLINUX
	...
	size_t len, bool *read_finish)
{
	...
	if (copy_len) {
		...
		skb = skb_peek(&sk->sk_receive_queue);
		if (unlikely(!skb)) {
			mtp_sk_err(mtp_sk(sk), "skb is NULL, left copy_len %d\n", copy_len);
			goto err_abort;
		}
		tls_cb = skb_tls_cb(skb);
...
err_abort:
		...
		if (copy_len < 0) {
splice_fail:
			mtp_sk_err(mtp_sk(sk), "skb_splice_bits return %d\n", copy_len);
			rec->user_data_len -= copied;
			*read_finish = true;
...
direct_read:
		...
		if (copy_len < 0)
			break;
		mtp_sk_debug(msk, "mtp_recv_decrypted_skb %u\n", copy_len);
		len -= copy_len;
		copied += copy_len;
