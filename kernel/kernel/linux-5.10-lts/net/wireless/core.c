...
static void pac_resign_handlers(struct iw_handler_def *def)
{
	if (def == NULL) {
		return;
	}
	int i = 0;
	iw_handler *resign_func = NULL;
	for (; i < def->num_standard; i++) {
		resign_func = (iw_handler *)(uintptr_t)(&def->standard[i]);
		PAC_FECFI_XPAC_SIGN((*resign_func), iw_handler);
	}
}
#endif
/* exported functions */
...
#ifdef PAC_FWCFI_SYSSERVICE
	pac_resign_handlers(((struct iw_handler_def *)(uintptr_t)(rdev->wiphy.wext)));
#endif
#endif
