...
#include "reg.h"
#include "rdev-ops.h"
#if (defined(CONFIG_HW_WIFI_EVENT) && defined(CONFIG_HW_WIFI_AMAX))
#include <huawei_platform/net/amax/amax_multilink_interface.h>
#endif

#ifdef CONFIG_WIFI_AUTH_REQ_RETRY
#define AUTH_REQ_MAX_RETRY_NUM 3
#define AUTH_REQ_RETRY_TIME 10
#endif
...
#endif
	[NL80211_ATTR_FAST_CONNECT] = { .type = NLA_FLAG },
	[NL80211_ATTR_NO_ENCRPTY] = { .type = NLA_FLAG },
};
			      ...
			      enum nl80211_commands cmd)
{
	...
	case NL80211_IFTYPE_ADHOC: {
		...
		if (!wdev->connected)
			break;
		if (wdev->ssid_len &&
		    nla_put(msg, NL80211_ATTR_SSID, wdev->ssid_len,
			    wdev->ssid))
            goto nla_put_failure_locked;
		break;
		}
...
#define PUT_SINFO_U64(attr, memb) do {					\
	...
	if (sinfo->filled & BIT_ULL(NL80211_STA_INFO_UL_DELAY_ARRAY)) {
		if (nla_put(msg, NL80211_STA_INFO_UL_DELAY_ARRAY,
			sizeof(sinfo->ul_delay_array), sinfo->ul_delay_array))
		goto nla_put_failure;
	}

	if (sinfo->filled & BIT_ULL(NL80211_STA_INFO_TX_TIME)) {
		if (nla_put(msg, NL80211_STA_INFO_TX_TIME,
			sizeof(sinfo->tx_time), sinfo->tx_time))
		goto nla_put_failure;
	}

	if (sinfo->filled & BIT_ULL(NL80211_STA_INFO_BEACON_RSSI)) {
		if (nla_put(msg, NL80211_STA_INFO_BEACON_RSSI,
			sizeof(sinfo->beacon_rssi), sinfo->beacon_rssi))
		goto nla_put_failure;
	}

	if (sinfo->filled & BIT(NL80211_STA_INFO_CHLOAD_SELF))
		if (nla_put_u16(msg, NL80211_STA_INFO_CHLOAD_SELF, sinfo->chload_self))
			goto nla_put_failure;

	if (sinfo->filled & BIT_ULL(NL80211_STA_INFO_SIGNAL_DUAL)) {
		if (nla_put(msg, NL80211_STA_INFO_SIGNAL_DUAL,
			sizeof(sinfo->rpt_signal), sinfo->rpt_signal))
		goto nla_put_failure;
	}
#endif
	if (sinfo->filled & BIT_ULL(NL80211_STA_INFO_TX_PPDU_CNT)) {
		if (nla_put_u32(msg, NL80211_STA_INFO_TX_PPDU_CNT,
			(u32)sinfo->tx_ppdu_cnt))
		goto nla_put_failure;
	}
	if (sinfo->filled & BIT_ULL(NL80211_STA_INFO_TX_PPDU_RETRY_CNT)) {
		if (nla_put_u32(msg, NL80211_STA_INFO_TX_PPDU_RETRY_CNT,
			(u32)sinfo->tx_ppdu_retry_cnt))
		goto nla_put_failure;
	}
	if (sinfo->filled & BIT_ULL(NL80211_STA_INFO_PPDU_PER)) {
		if (nla_put_u8(msg, NL80211_STA_INFO_PPDU_PER,
			(u32)sinfo->ppdu_per))
		goto nla_put_failure;
	}
	if (sinfo->filled & BIT_ULL(NL80211_STA_INFO_TX_MCS)) {
		if (nla_put_u8(msg, NL80211_STA_INFO_TX_MCS,
			(u32)sinfo->tx_mcs))
		goto nla_put_failure;
	}
	if (sinfo->filled & BIT_ULL(NL80211_STA_INFO_CWMIN)) {
		if (nla_put_u8(msg, NL80211_STA_INFO_CWMIN,
			(u32)sinfo->cw_min))
		goto nla_put_failure;
	}
	if (sinfo->filled & BIT_ULL(NL80211_STA_INFO_CWMAX)) {
		if (nla_put_s8(msg, NL80211_STA_INFO_CWMAX,
			(u32)sinfo->cw_max))
		goto nla_put_failure;
	}
	if (sinfo->filled & BIT_ULL(NL80211_STA_INFO_ULDELAY_CDF)) {
		if (nla_put(msg, NL80211_STA_INFO_ULDELAY_CDF,
			sizeof(sinfo->uldelay_cdf), sinfo->uldelay_cdf)) {
				goto nla_put_failure;
			}
	}

	if (sinfo->filled & BIT_ULL(NL80211_STA_INFO_TX_TIME_CDF)) {
		if (nla_put(msg, NL80211_STA_INFO_TX_TIME_CDF,
			sizeof(sinfo->txtime_cdf), sinfo->txtime_cdf)) {
				goto nla_put_failure;
			}
	}
	...
	case NL80211_IFTYPE_STATION:
		...
		for_each_valid_link(wdev, link_id) {
			if (intbss == wdev->links[link_id].client.current_bss &&
				(!wdev->valid_links || link_id == wdev->assoc_link_id || (wdev->cofeature == WIFI7_MLO_COFEATURE &&
				wdev->cofeature_state == WIFI7_MLO_COFEATURE_STATE_EMLSR)) &&
...
#if defined (CONFIG_CONNECTIVITY_HI110X)
/* WAPI */
				  NL80211_WPA_VERSION_1 |  /* lint !e655 */
#endif
...
static int nl80211_connect(struct sk_buff *skb, struct genl_info *info)
{
	...
	if (nla_get_flag(info->attrs[NL80211_ATTR_FAST_CONNECT])) {
		connect.flags |= ASSOC_REQ_FAST_CONNECT;
	}

	if (nla_get_flag(info->attrs[NL80211_ATTR_NO_ENCRPTY])) {
        connect.flags |= ASSOC_REQ_NO_ENCRPTY;
    }
#ifdef CONFIG_HISI_WIFI7_MLO
...
#ifdef CONFIG_HISI_WIFI7_MLO
	if (memcmp(pmk_conf.aa, wdev->connected_addr, ETH_ALEN)) {
#else
...
#if (defined(CONFIG_HW_WIFI_EVENT) && defined(CONFIG_HW_WIFI_AMAX))
void cfg80211_dualband_roam_process(struct cfg80211_event *ev)
{
	if (ev == NULL)
		return;
	uint32_t ret = 0;
	amax_dualband_roam_info_stru *dualband_roam_info = NULL;
	dualband_roam_info = (amax_dualband_roam_info_stru *) kmalloc(sizeof(amax_dualband_roam_info_stru), GFP_KERNEL);
	if (dualband_roam_info == NULL) {
		pr_err("[dualband_roam_info]init failed!\n");
		goto roam_done;
	}
 
	(void)memset_s(dualband_roam_info, sizeof(amax_dualband_roam_info_stru), 0, sizeof(amax_dualband_roam_info_stru));
	if (ev->type == EVENT_DRV_WIFI_EVENT) {
		if(!ev->dc.ie || ev->dc.ie_len != sizeof(amax_dualband_roam_info_stru)) {
			pr_err("cfg80211_dualband_roam_process ie is null\n");
			goto roam_done;
		}
		ret = memcpy_s(dualband_roam_info, sizeof(amax_dualband_roam_info_stru), ev->dc.ie, ev->dc.ie_len);
		if (ret)
			goto roam_done;
		if (dualband_roam_info->support_zero_roaming != DUALBAND_ROAM_SUPPORT) {
			pr_err("cfg80211_dualband_roam_process msg_type error or do not support dualband roam\n");
            goto roam_done;
		}
	} else {
		goto roam_done;
	}
	amax_hook_roam_info_handle(dualband_roam_info);
 
roam_done:
	if (dualband_roam_info != NULL) {
		kfree(dualband_roam_info);
		dualband_roam_info = NULL;
	}
	return;
}
EXPORT_SYMBOL(cfg80211_dualband_roam_process);
#endif
...
#ifdef CONFIG_HISI_WIFI7_MLO
static void nl80211_inform_link_switch(struct cfg80211_registered_device *rdev,
				    struct net_device *netdev,
				    unsigned int link_id, gfp_t gtp,
				    enum nl80211_commands notif)
{
	struct wireless_dev *wdev = netdev->ieee80211_ptr;
	struct sk_buff *msg;
	void *hdr;
	msg = nlmsg_new(NLMSG_DEFAULT_SIZE, gtp);
	if (!msg)
		return;
	...
	if (nla_put_u32(msg, NL80211_ATTR_WIPHY, rdev->wiphy_idx) ||
		nla_put_u32(msg, NL80211_ATTR_IFINDEX, netdev->ifindex))
		goto nla_put_failure;
	if (nla_put_u8(msg, NL80211_ATTR_MLO_LINK_SWITCH_LINK_ID, link_id))
		goto nla_put_failure;
	if (!wdev->links[link_id].client.current_bss)
		goto nla_put_failure;
	if (wdev->links[link_id].client.current_bss->pub.bssid == NULL ||
		nla_put(msg, NL80211_ATTR_MLO_LINK_SWITCH_WORK_BSSID, ETH_ALEN,
		wdev->links[link_id].client.current_bss->pub.bssid))
		goto nla_put_failure;
	genlmsg_end(msg, hdr);
	genlmsg_multicast_netns(&nl80211_fam, wiphy_net(&rdev->wiphy), msg, 0,
				NL80211_MCGRP_MLME, gtp);
	return;
nla_put_failure:
	nlmsg_free(msg);
}
void cfg80211_inform_link_switch(struct net_device *dev, u8 link_id)
{
	struct wireless_dev *wdev = dev->ieee80211_ptr;
	struct wiphy *wiphy = wdev->wiphy;
	struct cfg80211_registered_device *rdev = wiphy_to_rdev(wiphy);
	...
	/* only wifi7 mlo connect, link_id match valid link inform */
	if (wdev->valid_links && (wdev->valid_links & BIT(link_id))) {
		wdev->assoc_link_id = link_id;
		nl80211_inform_link_switch(rdev, dev, link_id, GFP_KERNEL, NL80211_CMD_LINK_SWITCH_EVENT);
	}
}
EXPORT_SYMBOL(cfg80211_inform_link_switch);
			       ...
			       unsigned int link_id, u16 punct_bitmap)
{
	...
	if (wdev->iftype == NL80211_IFTYPE_STATION &&
	    !WARN_ON(!wdev->links[link_id].client.current_bss))
	cfg80211_update_assoc_bss_entry(wdev, link_id, chandef->chan);
	cfg80211_sched_dfs_chan_update(rdev);
				       ...
				       bool quiet, u16 punct_bitmap)
{
	...
	trace_cfg80211_ch_switch_started_notify(dev, chandef);//todo

	nl80211_ch_switch_notify(rdev, dev, link_id, chandef, GFP_KERNEL,
				 NL80211_CMD_CH_SWITCH_STARTED_NOTIFY, count, quiet, punct_bitmap);
}
...
static void nl80211_inform_cofeature_state(struct cfg80211_registered_device *rdev,
				    struct net_device *netdev, u8 feature,
				    u8 state, u16 reason_code, gfp_t gtp,
				    enum nl80211_commands notif)
{
	struct sk_buff *msg;
	void *hdr;
	...
	if (nla_put_u32(msg, NL80211_ATTR_WIPHY, rdev->wiphy_idx) ||
		nla_put_u32(msg, NL80211_ATTR_IFINDEX, netdev->ifindex))
		goto nla_put_failure;
	if (nla_put_u8(msg, NL80211_ATTR_COFEATURE, feature))
		goto nla_put_failure;
	if (nla_put_u8(msg, NL80211_ATTR_COFEATURE_STATE, state))
		goto nla_put_failure;
	if (nla_put_u16(msg, NL80211_ATTR_COFEATURE_SWITCH_REASON, reason_code))
		goto nla_put_failure;
	genlmsg_end(msg, hdr);
	genlmsg_multicast_netns(&nl80211_fam, wiphy_net(&rdev->wiphy), msg, 0,
				NL80211_MCGRP_MLME, gtp);
...
void cfg80211_inform_cofeature_state(struct net_device *dev, u8 feature, u8 state, u16 reason_code)
{
	struct wireless_dev *wdev = dev->ieee80211_ptr;
	struct wiphy *wiphy = wdev->wiphy;
	struct cfg80211_registered_device *rdev = wiphy_to_rdev(wiphy);
	ASSERT_WDEV_LOCK(wdev);
	wdev->cofeature = feature;
	wdev->cofeature_state = state;
	wdev->reason_code = reason_code;
	nl80211_inform_cofeature_state(rdev, dev, feature, state, reason_code, GFP_KERNEL, NL80211_CMD_MLO_WORK_STATE_EVENT);
}
EXPORT_SYMBOL(cfg80211_inform_cofeature_state);
...
#ifdef CONFIG_WIFI_AUTH_REQ_RETRY
	for (int i = 0; i < AUTH_REQ_MAX_RETRY_NUM; i++) {
		if (!wdev->conn_owner_nlportid) {
			msleep(AUTH_REQ_RETRY_TIME);
		} else {
			break;
		}
	}
...
#ifdef CONFIG_HISI_WIFI7_MLO
			params->ssid.ssid) || (!is_zero_ether_addr(params->mld_addr) &&
