				...
				gfp_t gfp)
{
	...
	if (data->bss_source == BSS_SOURCE_MBSSID) {
		 ...
		 */
		spin_lock_bh(&rdev->bss_lock);
		if (cfg80211_add_nontrans_list(data->source_bss,
					       &res->pub)) {
...
/* hisi platform don't use this one function */
struct cfg80211_bss *
			       ...
			       gfp_t gfp)
{
	...
	if (!res || !wiphy->support_mbssid ||
			!cfg80211_find_ie(WLAN_EID_MULTIPLE_BSSID, inform_data.ie, inform_data.ielen))
			return res;
	if (wiphy->support_only_he_mbssid &&
		!cfg80211_find_ext_ie(WLAN_EID_EXT_HE_CAPABILITY, inform_data.ie, inform_data.ielen))
		return res;
