...
#define WIRE_LESS_BSSID_SIZE 6
#define WIRE_LESS_BSSID_ARRAY_SIZE (WIRE_LESS_BSSID_SIZE * 3)
#define WIRE_LESS_MAX_ANONYMIZE_SSID_SIZE 4
			       ...
			       bool wextev)
{
	...
	for_each_valid_link(cr, link) {
		...
		wdev->current_bss = bss_from_pub(cr->links[link].bss);
	}
	wdev->connected = true;
	ether_addr_copy(wdev->connected_addr, connected_addr);
	if (cr->valid_links) {
		for_each_valid_link(cr, link)
	...
	rcu_read_lock();
	for_each_valid_link(cr, link) {
		country_elem = ieee80211_bss_get_elem(cr->links[link].bss, WLAN_EID_COUNTRY);
		if (country_elem)
			break;
	}
	if (!country_elem) {
	...
	country_data = kmemdup(country_elem->data, 2 + country_datalen, GFP_ATOMIC);
	rcu_read_unlock();
	...
	if (!wdev->ssid_len) {
		...
		for_each_valid_link(cr, link) {
			...
			memcpy(wdev->ssid, ssid->data, ssid->datalen);
			wdev->ssid_len = ssid->datalen;
			break;
		}
		rcu_read_unlock();
	}
					...
					struct cfg80211_bss **bss)
{
	struct cfg80211_registered_device *rdev = wiphy_to_rdev(wdev->wiphy);
   	...
   	if (list_empty(&ibss->list)) {
		...
		found = cfg80211_get_bss(wdev->wiphy, NULL, (*bss)->bssid,
					   wdev->ssid, wdev->ssid_len,
					   wdev->conn_bss_type,
					   IEEE80211_PRIVACY_ANY); //todo
	   	if (found) {
...
#endif
#ifdef CONFIG_HISI_WIFI7_MLO
/* inform current assoc link id to kernel */
void cfg80211_inform_mlo_assoc_link_id(struct net_device *dev, u8 assoc_link_id)
{
	struct wireless_dev *wdev = dev->ieee80211_ptr;
	wdev->assoc_link_id = assoc_link_id;
}
EXPORT_SYMBOL(cfg80211_inform_mlo_assoc_link_id);
			   ...
			   gfp_t gfp)
{
	...
	if (!ev) {
		for_each_valid_link(params, link)
			cfg80211_put_bss(wdev->wiphy, params->links[link].bss);
		return;
	}
	...
	if (params->ap_mld_addr) {
		...
		memcpy((void *)ev->cr.ap_mld_addr, params->ap_mld_addr, ETH_ALEN);
		next += ETH_ALEN;
	}
		       ...
		       struct cfg80211_roam_info *info)
{
	...
	for_each_valid_link(info, link) {
		...
		wdev->current_bss = bss_from_pub(info->links[link].bss);
	}
...
static void anonymize_ssid(const char *str, char *newStr, int len)
{
	if (str == NULL || *str == '\0')
		return;
...
static void anonymize_bssid(const u8 *bssid, char *newStr, int len)
{
	if (bssid == NULL || strlen(bssid) != WIRE_LESS_BSSID_SIZE)
		return;

	memset(newStr, 0, len);
	snprintf(newStr, len, "%02x:%02x:**:**:**:%02x", bssid[0], bssid[1], bssid[WIRE_LESS_BSSID_SIZE-1]);
}
		     ...
		     gfp_t gfp)
{
	...
	for_each_valid_link(info, link) {
		...
		if (info->links[link].channel != NULL) {
			pr_info("cfg80211_roamed: channel band:%d", info->links[link].channel->band);
		}
		char anonymizeBssid[WIRE_LESS_BSSID_ARRAY_SIZE] = { 0 };
		char anonymizeSsid[WIRE_LESS_MAX_ANONYMIZE_SSID_SIZE] = { 0 };
		anonymize_bssid(info->links[link].bssid, anonymizeBssid, WIRE_LESS_BSSID_ARRAY_SIZE);
		anonymize_ssid(wdev->ssid, anonymizeSsid, WIRE_LESS_MAX_ANONYMIZE_SSID_SIZE);
		pr_info("cfg80211_roamed: bssid:%s, ssid:%s, ssid_len:%u, conn_bss_type:%d",
			anonymizeBssid, anonymizeSsid, wdev->ssid_len, wdev->conn_bss_type);
		info->links[link].bss =
		     ...
		     gfp_t gfp)
{
	...
	if (info->channel != NULL) {
		pr_info("cfg80211_roamed: channel band:%d", info->channel->band);
	}
	char anonymizeBssid[WIRE_LESS_BSSID_ARRAY_SIZE] = { 0 };
	char anonymizeSsid[WIRE_LESS_MAX_ANONYMIZE_SSID_SIZE] = { 0 };
	anonymize_bssid(info->bssid, anonymizeBssid, WIRE_LESS_BSSID_ARRAY_SIZE);
	anonymize_ssid(wdev->ssid, anonymizeSsid, WIRE_LESS_MAX_ANONYMIZE_SSID_SIZE);
	pr_info("cfg80211_roamed: bssid:%s, ssid:%s, ssid_len:%u, conn_bss_type:%d",
			anonymizeBssid, anonymizeSsid, wdev->ssid_len, wdev->conn_bss_type);
	if (!info->bss) {
...
#ifdef CONFIG_HISI_WIFI7_MLO
	if (WARN_ON(!wdev->connected) || WARN_ON(!wdev->links[wdev->assoc_link_id].client.current_bss) ||
	    WARN_ON(!ether_addr_equal(wdev->links[wdev->assoc_link_id].client.current_bss->pub.bssid, bssid)))
#else
	if (WARN_ON(!wdev->current_bss) ||
