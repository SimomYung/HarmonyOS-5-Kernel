	...
	struct miniisp_msg_t *p_msg_ack)
{
	...
	case SUB_CMD_SWING_CAM_MATCH_ID:
		...
		camera_index = p_msg_ack->u.ack_query_igs_cam.camid >> 16;
		sensor_index = p_msg_ack->u.ack_query_igs_cam.camid & 0xFFFF;
		igs = iomcu_config_get(IOMCU_CONFIG_USR_IGS, sizeof(struct igs_config));
		if ((p_msg_ack->u.ack_query_igs_cam.status == 0) &&
			(camera_index < 3) && (sensor_index < 4) && (igs != NULL)) {
			igs->camera_match_flag[camera_index][sensor_index] = 1;
			swing_cam_log_info("[%s] camera_match_flag[%u][%u]=1", __func__,
				camera_index, sensor_index);
		}
		break;
...
static int __init swing_cam_dev_init(struct platform_device *pdev)
{
	...
	ret = register_iom3_recovery_notifier(REC_USR_IGS, &g_miniisp_reboot_notify);
	if (ret < 0) {
...
static int swing_v4l2_ioctl_cam_match_id(void *arg)
{
	...
	swing_cam_log_info("[%s] camid:%d, pos:%d, i2c:%d, csi:%d", __func__,
...
static int __init swing_cam_main_init(void)
{
	...
	igs = iomcu_config_get(IOMCU_CONFIG_USR_IGS, sizeof(struct igs_config));
	if (igs != NULL) {
		if (memset_s(igs->camera_match_flag, (sizeof(unsigned char) * IGS_CAM_NUM * IGS_SENSOR_NUM),
			0, (sizeof(unsigned char) * IGS_CAM_NUM * IGS_SENSOR_NUM)) != 0) {
			swing_cam_log_info("[%s] set igs fail ", __func__);
		}
	}
...
module_init(swing_cam_main_init);
