...
static struct file * load_npu_open_partition(const char *partition_name)
{
	...
	if (IS_ERR(fp)) {
		npu_drv_err("[load_npu]open ts partition filp in read-only mode failed path=%s\n", partition_path);
		fp = NULL;
	}
...
static int32_t load_npu_read(const char *partition_name, uintptr_t *cert_addr, uintptr_t *data_addr)
{
	...
	npu_drv_debug("[load_npu]load_npu_read cert and data succ ret=%d\n", ret);
	filp_close(fp, NULL);
...
int load_npu_verify_image(void)
{
	...
	atomic_set(&g_load_status, (ret == 0) ? NPU_TSFW_LOADED_SUCC : NPU_TSFW_UNLOAD);
...
EXPORT_SYMBOL(load_npu_verify_image);
