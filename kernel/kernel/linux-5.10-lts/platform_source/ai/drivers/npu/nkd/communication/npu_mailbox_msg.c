	...
	struct npu_stream_msg *stream_msg)
{
	...
	stream_msg->sync_type = NPU_MAILBOX_SYNC;
	stream_msg->result = 0;
	...
	if (stream_info->strategy & STREAM_STRATEGY_ASYNC)
		stream_msg->flag = 1;

	stream_msg->sq_index = (u16)sq_index;
	stream_msg->cq0_index = (u16)cq_index;
	stream_msg->pid = stream_info->server_tid;
	...
	npu_drv_debug("create alloc stream msg :"
		"stream_msg->valid = %d  stream_msg->cmd_type = %d stream_msg->sync_type = %d "
		...
		stream_msg->valid, stream_msg->cmd_type, stream_msg->sync_type,
	...
	struct npu_stream_msg *stream_msg)
{
	...
	npu_drv_debug("create free stream msg : stream_msg->valid = %d stream_msg->cmd_type = %d "
		"stream_msg->sync_type = %d stream_msg->result = %d stream_msg->sq_index = %d\n"
		"stream_msg->sq_addr = %pK stream_msg->cq0_index = %d "
		"stream_msg->cq0_addr = %pK stream_msg->stream_id = %d\n"
		"stream_msg->plat_type = %d stream_msg->cq_slot_size = %d\n",
		stream_msg->valid, stream_msg->cmd_type, stream_msg->sync_type,
...
int npu_send_cancel_mailbox(u8 dev_id, pid_t server_pid)
{
	int ret;
	struct npu_cancel_msg *cancel_msg = NULL;
	int mbx_send_result = -1;
	size_t msg_len;
 
	// call mailbox to info ts to cancel task
	cancel_msg = (struct npu_cancel_msg *)kzalloc(
		sizeof(struct npu_cancel_msg), GFP_KERNEL);
	if (cancel_msg == NULL) {
		ret = -ENOMEM;
		npu_drv_err("kzalloc cancel_msg failed, ret = %d\n", ret);
		goto alloc_cancel_msg_failed;
	}
 
	/* add message header */
	cancel_msg->valid = NPU_MAILBOX_MESSAGE_VALID;
	cancel_msg->cmd_type = NPU_MAILBOX_CANCEL_TASK;
	cancel_msg->sync_type = NPU_MAILBOX_BOTTOM_SYNC;
	cancel_msg->result = 0;
 
	/* add payload */
	cancel_msg->cancel_type = NPU_CANCEL_SERVER_CMD;
	cancel_msg->pid = (u32)server_pid;
 
	msg_len = sizeof(struct npu_cancel_msg);
	ret = npu_mailbox_message_send_for_res(dev_id,
		(u8 *)cancel_msg, (u32)msg_len, &mbx_send_result);
	if (ret != 0) {
		npu_drv_err("cancel task mailbox_message_send failed"
			" mbx_send_result = %d ret = %d\n", mbx_send_result, ret);
		goto message_send_for_res_failed;
	}
 
	npu_drv_warn("cancel task mailbox_message_send success"
		" mbx_send_result = %d ret = %d\n", mbx_send_result, ret);
	kfree(cancel_msg);
	cancel_msg = NULL;
	return 0;
