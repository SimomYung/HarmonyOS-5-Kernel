...
struct npu_stream_msg {
	...
	u8 sync_type;       /* TS return hw_exp_irq ack or not */
...
enum cancel_cmd_type {
	NPU_CANCEL_SERVER_CMD = 0x01,
};

struct npu_cancel_msg {
	u16 valid;       /* validity judgement, 0x5a5a is valid */
	u8 cmd_type;       /* command type */
	u8 sync_type;       /* TS return hw_exp_irq ack or not */
	u32 result;       /* TS's process result succ or fail: no error: 0, error: not 0 */
	u32 cancel_type;       /* invalid addr: 0x0 */
	u32 pid;
	u32 model_id;
	u32 task_id;
	u8 resv[40];
...
struct npu_mailbox_message_header {
	u16 valid; /* validity judgement, 0x5a5a is valid */
	u8 cmd_type; /* identify command or operation */
	u8 sync_type; /* TS return hw_exp_irq ack or not */
