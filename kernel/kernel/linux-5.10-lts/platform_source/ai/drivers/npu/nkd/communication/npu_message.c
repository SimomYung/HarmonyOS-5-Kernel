	...
	struct npu_receive_response_info *report_info, struct npu_dev_ctx *dev_ctx)
{
	...
	if (unlikely(count > (u32)atomic_read(&cq_info->count_report)))
		npu_drv_err("devid: %u, too much report, cq id: %d, "
	...
	struct npu_ts_cq_info *cq_info, struct npu_cq_sub_info *cq_sub_info, u32 count)
{
	...
	struct npu_exec_task_info task_info = {0};
#ifdef NPU_ARCH_V100
...
#endif
		if (npu_task_set_remove(dev_ctx, proc_ctx,
			p_report->stream_id, p_report->task_id, &task_info) != 0)
			npu_drv_err("npu taskset remove fail, stream_id = %u, task_id = %u, sq_id = %u, sq_head = %u, payload = 0x%llx",
				p_report->stream_id, p_report->task_id, p_report->sq_id, p_report->sq_head, p_report->pay_load);
		else if (task_info.work_mode >= NPU_WORKMODE_MAX)
			npu_drv_err("work_mode %u invalid", task_info.work_mode);
		else if (nonsec_workmode_map[task_info.work_mode] < NPU_NOSEC_WORKMODE_NUM)
			task_release_cnt[nonsec_workmode_map[task_info.work_mode]]++;
	...
	struct npu_receive_response_info *report_info)
{
	...
	npu_release_report(dev_ctx, proc_ctx, cq_info, cq_sub_info, report_info->response_count);

#ifdef CONFIG_HIBERNATION
	if (dev_ctx->s4_pending_report_count != 0) {
		dev_ctx->s4_pending_report_count = (dev_ctx->s4_pending_report_count >= report_info->response_count) ?
			(dev_ctx->s4_pending_report_count - report_info->response_count) : 0;
		npu_drv_warn("after S4, receieve pending reports count: %u", report_info->response_count);
	}
#endif
...
static void npu_proc_cq_info_print(struct npu_dev_ctx *dev_ctx, struct npu_ts_cq_info *cq_info)
{
	...
	cq_sub_info = npu_get_cq_sub_addr(dev_ctx, cq_info->index);
	if (cq_sub_info == NULL) {
		npu_drv_err("get cq %u sub info failed", cq_info->index);
		return;
	}
	report = __npu_get_report(cq_sub_info->virt_addr,
		cq_info->slot_size, cq_info->tail);
...
static int npu_complete_comm_sqe(struct npu_proc_ctx *proc_ctx, npu_rt_task_t *comm_task)
{
	...
	if (comm_task->type == NPU_TASK_MODEL_EXECUTE ||
		...
		ret = npu_proc_get_model_info(proc_ctx, comm_task->u.model_execute_task.model_id, &model_desc_sub);
		cond_return_error(ret != 0, ret, "npu_proc_get_model_info error, ret= %d\n", ret);

		ret = npu_get_ssid_bypid(proc_ctx->devid, current->tgid,
			model_desc_sub.execute_pid,
			&ssid, &ttbr, &tcr);
...
/* 1. Supported from v310, the previous platform can ignore this log. */
/* 2. We do not distinguish the previous platform to avoid having to adapt to each new platform. */
#if defined(CONFIG_NPU_PM_SMC) && defined(CONFIG_NPU_DEBUG) 
	if (comm_task->type == NPU_TASK_MODEL_EXECUTE ||
		comm_task->type == NPU_TASK_MODEL_LOOP_EXECUTE)
		npu_drv_warn("get current npu voltage: %d\n", npu_pm_get_voltage_smc_request());
#endif

	/* update doorbell */
	mutex_lock(&cur_dev_ctx->pm.npu_power_mutex);
	if (cur_dev_ctx->pm.power_stage == NPU_PM_UP) {
		(void)npu_write_doorbell_val(DOORBELL_RES_CAL_SQ, sq_index, sq_info->tail);
	} else {
...
static int npu_verify_model_task(struct npu_proc_ctx *proc_ctx, npu_rt_task_t *comm_task)
{
	uint16_t model_id = NPU_MAX_MODEL_ID;
	struct npu_id_allocator *id_allocator = NULL;
	struct npu_id_entity *id_entity = NULL;
	npu_model_desc_sub_t *model_desc_sub;

	if (comm_task->type == NPU_TASK_MODEL_LOOP_EXECUTE || comm_task->type == NPU_TASK_MODEL_EXECUTE)
		model_id = comm_task->u.model_execute_task.model_id;
	else if (comm_task->type == NPU_TASK_MODEL_STOP)
		model_id = comm_task->u.model_stop_task.model_id;
	else
		return 0;
	...
	model_desc_sub = (npu_model_desc_sub_t*)id_entity->data;

	if (model_desc_sub->execute_enable != NPU_MODEL_EXECUTE_ENABLE) {
		npu_drv_err("model_id = %u forbidden execute\n", model_id);
		mutex_unlock(&id_allocator->lock);
		return -EPERM;
	}

	if (proc_ctx->dev_type == DIRECT && model_desc_sub->execute_pid != (u32)(current->tgid)) {
		...
		struct npu_proc_ctx *proc_ctx, npu_rt_task_t *comm_task)
{
	...
	ret = npu_verify_model_task(proc_ctx, comm_task);
	cond_goto_error(ret != 0, NPU_EXCEPTION, ret, ret, "npu model task verify failed, ret= %d\n", ret);

	ret = npu_plat_get_work_mode(proc_ctx, comm_task, &work_mode);
	cond_goto_error(ret != 0, NPU_EXCEPTION, ret, ret, "npu plat get work mode failed, ret= %d\n", ret);
	...
	npu_drv_debug("sq_sub_info, index= %u, "
		"ref_by_streams= %u, phy_addr= 0x%pK, virt_addr= 0x%pK\n",
...
#ifndef NPU_ARCH_V100
	if (comm_task->type == NPU_TASK_MODEL_EXECUTE || comm_task->type == NPU_TASK_MODEL_LOOP_EXECUTE) {
		model_desc_info = npu_calc_model_desc_info(proc_ctx->devid, comm_task->u.model_execute_task.model_id);
		cond_return_error(model_desc_info == NULL, -1,
			"get model desc info failed, model_id = %u", comm_task->u.model_execute_task.model_id);
		model_desc_info->priority = comm_task->u.model_execute_task.priority;
	}
#endif
...
#endif
	if (ret == 0 && (comm_task->type == NPU_TASK_MODEL_EXECUTE ||
		comm_task->type == NPU_TASK_MODEL_LOOP_EXECUTE))
...
#ifdef CONFIG_HIBERNATION
static u32 npu_calc_proc_except_task_count(struct npu_dev_ctx *dev_ctx, struct npu_proc_ctx *proc_ctx)
{
	struct npu_task_info *curr_task = NULL;
	struct npu_task_info *next_task = NULL;
	u32 except_count = 0;
...
static u32 npu_calc_proc_report_count(struct npu_dev_ctx *dev_ctx, struct npu_proc_ctx *proc_ctx)
{
	struct npu_cq_sub_info *cq_sub_info = NULL;
	...
	list_for_each_entry_safe(cq_sub_info, next_cq_sub_info, &proc_ctx->cq_list, list) {
		...
		report_count += (cq_info->tail + NPU_MAX_CQ_DEPTH - cq_info->head) % NPU_MAX_CQ_DEPTH;
	}

	return report_count;
}

int npu_wait_pending_task_done(struct npu_dev_ctx *dev_ctx)
{
	struct npu_proc_ctx *proc_ctx = NULL;
	struct npu_proc_ctx *next_proc_ctx = NULL;
	u32 timeout = 300;
	u32 pending_task_count = 0;
	u32 receieve_task_count;
	u32 report_count;
	...
	// 1. calculate the number of pending tasks
	list_for_each_entry_safe(proc_ctx, next_proc_ctx, &dev_ctx->proc_ctx_list, dev_ctx_list)
		pending_task_count += proc_ctx->task_cnt[NPU_NONSEC];
	npu_drv_warn("pending task count: %u", pending_task_count);
	cond_return_warn(pending_task_count == 0, 0, "no pending task\n");

	while (timeout > 0) {
		receieve_task_count = 0;
		dev_ctx->s4_pending_report_count = 0;
		list_for_each_entry_safe(proc_ctx, next_proc_ctx, &dev_ctx->proc_ctx_list, dev_ctx_list) {
			// 2. calculate the number of abnormal tasks caused by global exception
			receieve_task_count += npu_calc_proc_except_task_count(dev_ctx, proc_ctx);
			// 3. calculate the number of receieved reports
			report_count = npu_calc_proc_report_count(dev_ctx, proc_ctx);
			receieve_task_count += report_count;
			dev_ctx->s4_pending_report_count += report_count;
		}
		npu_drv_warn("receieve_task_count:%u, s4_pending_report_count:%u",
			receieve_task_count, dev_ctx->s4_pending_report_count);
		if (receieve_task_count >= pending_task_count) {
			npu_drv_warn("wait all pending tasks done success!");
			return 0;
		}
		msleep(10);
		timeout--;
...
void npu_proc_pending_task_except(struct npu_dev_ctx *dev_ctx)
{
	struct npu_proc_ctx *proc_ctx = NULL;
	struct npu_proc_ctx *next_proc_ctx = NULL;
	...
	list_for_each_entry_safe(proc_ctx, next_proc_ctx, &dev_ctx->proc_ctx_list,
		dev_ctx_list) {
		mutex_lock(&dev_ctx->pm.task_set_lock);
		if (list_empty(&proc_ctx->task_set) == 0)
