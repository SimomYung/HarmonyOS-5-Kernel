	...
	struct npu_ts_cq_info *cq_info, struct npu_cq_sub_info *cq_sub_info, u32 count)
{
	...
	struct npu_exec_task_info task_info;
#ifdef NPU_ARCH_V100
	...
	for (i = 0; i < count; i++) {
		...
		npu_task_set_remove(dev_ctx, proc_ctx,
			p_report->stream_id, p_report->task_id, &task_info);
		if (nonsec_workmode_map[task_info.work_mode] < NPU_NOSEC_WORKMODE_NUM)
...
#endif
			if (receive_out_time.tv_sec >=
                dev_ctx->npu_live_stamp.tv_sec + ((int64_t)dev_ctx->npu_task_time_out / MSEC_PER_SEC)) {
		...
		struct npu_proc_ctx *proc_ctx, npu_rt_task_t *comm_task)
{
	...
	cond_return_error(ret == -1, ret, "npu plat get work mode failed\n");
	...
	cond_goto_error(ret != 0, NPU_EXCEPTION, ret, ret, "model execute disable, ret= %d\n", ret);
	
	ret = npu_pm_proc_send_task_enter_wm(proc_ctx, cur_dev_ctx, work_mode);
	cond_goto_error(ret != 0, NPU_ENTER_WM_EXCEPTION, ret, ret,
