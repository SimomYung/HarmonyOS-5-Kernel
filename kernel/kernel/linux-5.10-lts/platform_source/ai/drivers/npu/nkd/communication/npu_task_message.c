	...
	struct npu_proc_ctx *proc_ctx, npu_rt_task_t *comm_task, u32 work_mode)
{
	...
	task->task_id = comm_task->task_id;
	task->stream_id = comm_task->stream_id;
	task->type = comm_task->type;
	if (comm_task->type == NPU_TASK_MODEL_EXECUTE ||
		comm_task->type == NPU_TASK_MODEL_LOOP_EXECUTE)
		task->model_id = comm_task->u.model_execute_task.model_id;
	else if (comm_task->type == NPU_TASK_MODEL_STOP)
		task->model_id = comm_task->u.model_stop_task.model_id;
	else
...
int npu_task_set_remove(struct npu_dev_ctx *dev_ctx, struct npu_proc_ctx *proc_ctx,
	u32 stream_id, u32 task_id, struct npu_exec_task_info *task_info)
{
	struct npu_task_info *task = NULL;
	...
	list_for_each_safe(pos, n, &proc_ctx->task_set) {
		...
		if (task->task_id == task_id) {
			...
			if (task_info != NULL) {
				task_info->model_id = task->model_id;
				task_info->work_mode = task->work_mode;
	...
	mutex_unlock(&dev_ctx->pm.task_set_lock);
	npu_drv_err("task is not existed, taskid = %u, stream = %u\n", task_id, stream_id);
	return -1;
}
