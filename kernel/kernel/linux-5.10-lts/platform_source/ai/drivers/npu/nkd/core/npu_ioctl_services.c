	...
	npu_model_desc_t *model_desc)
{
	...
	for (stream_idx = model_desc->stream_cnt - 1; stream_idx >= 0; stream_idx--) {
		...
		if (ret != 0) {
			vfree(stream_buf_addr);
			npu_drv_err("npu_plat_save_sw_sqe fail, length= %u, ret= %d\n",
				stream_len, ret);
			return -1;
		}
...
#ifdef CONFIG_LIBLINUX
	model_desc->execute_pid = (uint32_t)liblinux_pal_thread_vtid_to_root_tid(model_desc->execute_pid);
#endif
	return 0;
}
	...
	unsigned long arg)
{
	...
	cond_return_error(para.model_id >= NPU_MAX_MODEL_ID, -EINVAL,
		"invalid model id(%u)\n", para.model_id);
	cond_return_error(para.flag > NPU_MODEL_EXECUTE_ENABLE, -EINVAL,
		"invalid flag(%u)\n", para.flag);

#ifdef CONFIG_NPU_FFRT
	ret = npu_ffrt_get_sched_path(proc_ctx, para.model_id, &sched_path);
	cond_return_error(ret != 0, -EFAULT,
		"get sched path error, ret= %d\n", ret);

	if (sched_path == NPU_SCHED_FFTS) {
		ret = npu_ffrt_set_ssid_by_enable_state(proc_ctx, para.model_id, para.flag);
		cond_return_error(ret != 0, -EFAULT,
			"set npu ffrt id error, ret= %d\n", ret);
