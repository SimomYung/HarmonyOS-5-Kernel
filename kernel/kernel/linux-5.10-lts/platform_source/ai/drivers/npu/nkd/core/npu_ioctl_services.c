	...
	unsigned long arg)
{
	...
	ret = npu_proc_alloc_stream(proc_ctx, &stream_info,
		(u32)para_1.strategy, para_1.priority);
	if (ret != 0) {
...
int npu_ioctl_smartisp_get_svm_ssid(struct npu_proc_ctx *proc_ctx, unsigned long arg)
{
	int ret;
	struct process_info info = {0};
#ifdef CONFIG_NPU_ISPNN_INFO
	struct hiail_para_info *hiail_para = (struct hiail_para_info *)(uintptr_t)get_shm_virt_addr(0, NPU_ISPNN_INFO_MEM);
	struct hiail_svm_info *hiail_svm = &(hiail_para->svm_info);
#else
	uint64_t ttbr;
	uint64_t tcr;
	u16 ssid = 0;
...
#ifdef CONFIG_NPU_ISPNN_INFO
#ifdef CONFIG_LIBLINUX
	info.vpid = liblinux_pal_thread_vtid_to_root_tid(info.vpid);
#endif
	ret = npu_get_ssid_bypid(proc_ctx->devid, current->tgid,
		info.vpid, &(hiail_svm->ssid), &(hiail_svm->ttbr), &(hiail_svm->tcr));
	if (ret != 0) {
		npu_drv_err("fail to get svm_id, ret = %d\n", ret);
		return ret;
	}
	info.pasid = UINT16_MAX;
	npu_drv_debug("pid=%d get svm_id 0x%x\n", info.vpid, hiail_svm->ssid);
#else
	ret = npu_get_ssid_bypid(proc_ctx->devid, current->tgid,
		current->tgid, &ssid, &ttbr, &tcr);
	if (ret != 0) {
		npu_drv_err("fail to get svm_id, ret = %d\n", ret);
		return ret;
	}
	...
	struct npu_entity_info *hwts_sq_sub_info, u32 *core_type)
{
	...
	// verify hwts_sqe
	ret = npu_plat_verify_hwts_sqe(proc_ctx, stream_buf_addr, stream_len, core_type);
	if (ret != 0) {
		vfree(stream_buf_addr);
		npu_drv_err("npu_plat_verify_hwts_sqe fail, length= %u, ret= %d\n",
			stream_len, ret);
		return -1;
	}
	...
	sqe_num = npu_plat_save_hwts_sqe(model_desc_info, stream_buf_addr, hwts_sq_sub_info, stream_len);
...
#ifdef CONFIG_NPU_FFRT
static int npu_ffrt_load_model(struct npu_proc_ctx *proc_ctx, npu_model_desc_t *model_desc)
{
	...
	load_model_desc_buf_info.ssid = uid;

	ret = npu_plat_ffrt_load_model_buf(&load_model_desc_buf_info);
	cond_return_error(ret != 0, -1, "ffrt load model buf failed");
		...
		npu_model_desc_info_t *model_desc_info, u32 *pt_core_type)
{
	...
	for (idx = 0; idx < model_desc->stream_cnt; idx++) {
		...
		cond_return_error(stream_info == NULL, -1,
			"stream_id= %hu stream_info is null\n", model_desc->stream_id[idx]);
	...
	npu_model_desc_t *model_desc)
{
	...
	model_desc_info->server_id = (uint32_t)(current->tgid);
	npu_save_model_info(proc_ctx, model_desc);
	npu_drv_debug("model_id:%d stream_cnt:%d compute_type:%d aiv_csw_buff_addr:0x%pK, model_desc->scheduling_path:%u",
		model_desc_info->model_id, model_desc_info->stream_cnt,
		model_desc_info->compute_type, model_desc_info->aiv_csw_buff_addr, model_desc->scheduling_path);
	...
	model_desc_info->core_type = core_type;
	...
	npu_model_desc_t *model_desc)
{
	...
	for (stream_idx = model_desc->stream_cnt - 1; stream_idx >= 0; stream_idx--) {
		...
		stream_buf_addr = vmalloc(stream_len);
		cond_return_error(stream_buf_addr == NULL, -ENOMEM,
			"vmalloc stream_buf memory size= %lu failed\n", stream_len);
		...
		if (ret != 0) {
			vfree(stream_buf_addr);
			npu_drv_err("npu_plat_verify_hwts_sqe fail, length= %lu, ret= %d\n",
				stream_len, ret);
			return -1;
		}

		npu_drv_debug("stream_cnt %u, stream_idx = %d\n", model_desc->stream_cnt, stream_idx);
		ret = npu_plat_save_sw_sqe(model_desc, stream_buf_addr, &first_task_id, devid, stream_idx);
		if (ret != 0) {
			vfree(stream_buf_addr);
			npu_drv_err("npu_plat_save_sw_sqe fail, length= %lu, ret= %d\n",
				stream_len, ret);
			return -1;
		}
...
#ifdef CONFIG_LIBLINUX
	model_desc->execute_pid = (u32)liblinux_pal_thread_vtid_to_root_tid(model_desc->execute_pid);
#endif
	return 0;
}
...
#ifndef NPU_ARCH_V100
	model_desc_info = npu_calc_model_desc_info(proc_ctx->devid, model_param_info.model_id);
	cond_return_error(model_desc_info == NULL, -1,
		"get model desc info failed, model_id = %u", model_param_info.model_id);
	model_desc_info->priority = (uint8_t)model_prio_info.priority;
#endif
	return  ret;
}
	...
	unsigned long arg)
{
	...
	npu_rdr_exception_report((uint32_t)exception_code, uid);
...
{
	int ret;
	struct npu_execute_info excute_info = {0};

	if (copy_from_user_safe(&excute_info, (void __user *)(uintptr_t)arg, sizeof(struct npu_execute_info))) {
		npu_drv_err("copy from user safe error\n");
		return -EFAULT;
	}

	if (excute_info.model_cnt == 0) {
		npu_drv_err("model cnt invalid\n");
		return -EFAULT;
	}

	// get model execute times and NPU reboot times.
	ret = npu_proc_get_execute_info(proc_ctx, &excute_info);
	cond_return_error(ret != 0, -EFAULT,
		"get model info failed, ret= %d\n", ret);

	if (copy_to_user_safe((void __user *)(uintptr_t)arg, &excute_info, sizeof(struct npu_execute_info))) {
		npu_drv_err("copy to user safe error size = %u\n", sizeof(struct npu_execute_info));
		return -EFAULT;
	}
	...
	unsigned long arg)
{
	...
	cond_return_error(proc_ctx->devid >= NPU_DEV_NUM, -EINVAL, "invalid dev_ctx_id\n");
	cond_return_error(para.flag > NPU_MODEL_EXECUTE_ENABLE, -EINVAL, "invalid flag(%u)\n", para.flag);

	mutex_lock(&proc_ctx->model_mutex);
	ret = npu_proc_check_model_id(proc_ctx, para.model_id);
	cond_goto_error(ret != 0, out, ret, -EFAULT, "invalid model id(%u)\n", para.model_id);

#ifdef CONFIG_NPU_FFRT
	ret = npu_ffrt_get_sched_path(proc_ctx, para.model_id, &sched_path);
	cond_goto_error(ret != 0, out, ret, -EFAULT, "get sched path error, ret= %d\n", ret);

	if (sched_path == NPU_SCHED_FFTS) {
		ret = npu_ffrt_set_ssid_by_enable_state(proc_ctx, para.model_id, para.flag);
		cond_goto_error(ret != 0, out, ret, -EFAULT, "set npu ffrt id error, ret= %d\n", ret);
		npu_drv_warn("set ffts model execute enable %u success\n", para.flag);
		mutex_unlock(&proc_ctx->model_mutex);
		return ret;
	}
#endif
	ret = npu_proc_set_execute_enable(proc_ctx, &para);
	cond_goto_error(ret != 0, out, ret, -EFAULT, "set execute enable error, ret= %d\n", ret);
	npu_drv_warn("set model execute enable %u success\n", para.flag);
out:
	mutex_unlock(&proc_ctx->model_mutex);
	return ret;
}
...
int npu_ioctl_get_err_task_info(struct npu_proc_ctx *proc_ctx, unsigned long arg)
{
	struct npu_task_dump_info para = {0};
	int ret = 0;
	...
	/* get err task info from bbox */
	ret = npu_bbox_get_err_task_info(&para);
	cond_return_error(ret != 0, -EFAULT,
		"get err task info fail, ret= %d\n", ret);

	if (copy_to_user_safe((void __user *)(uintptr_t)arg, &para, sizeof(struct npu_task_dump_info))) {
		npu_drv_err("copy to user safe error size = %u\n", sizeof(struct npu_task_dump_info));
		return -EFAULT;
	}
...
#ifndef NPU_ARCH_V100
int npu_ioctl_set_model_bandwidth(struct npu_proc_ctx *proc_ctx, unsigned long arg)
{
	struct npu_set_model_bandwidth_info para = {0};
	npu_model_desc_info_t *model_desc_info = NULL;
	int ret = 0;

	cond_return_error(proc_ctx->devid >= NPU_DEV_NUM, -1, "invalid device");

	if (copy_from_user_safe(&para, (void __user *)(uintptr_t)arg, sizeof(struct npu_set_model_bandwidth_info))) {
		npu_drv_err("copy from user safe error\n");
		return -EFAULT;
	}

	mutex_lock(&proc_ctx->model_mutex);
	ret = npu_proc_check_model_id(proc_ctx, (u32)para.model_id);
	if (ret != 0) {
		npu_drv_err("invalid model id(%u)", para.model_id);
		mutex_unlock(&proc_ctx->model_mutex);
		return -EFAULT;
	}

	model_desc_info = npu_calc_model_desc_info(proc_ctx->devid, (u32)para.model_id);
	if (model_desc_info == NULL) {
		npu_drv_err("model_desc_info is NULL");
		mutex_unlock(&proc_ctx->model_mutex);
		return -EFAULT;
	}
	model_desc_info->bandwidth_reduct_info.flag = 1;
	model_desc_info->bandwidth_reduct_info.mode = para.mode;
	mutex_unlock(&proc_ctx->model_mutex);

	ret = npu_plat_send_ipc_with_power_check(IPCDRV_TS_SET_MODEL_BANDWIDTH, IPCDRV_MSG_ASYNC,
		(u8 *)&para.model_id, (u32)sizeof(para.model_id), IPCDRV_SEND_WITHOUT_ACK);
...
/* svm flush cache */
int npu_ioctl_svm_flush_cache(struct npu_proc_ctx *proc_ctx,
	unsigned long arg)
{
	int ret;
	pid_t pid;
	...
	pid = liblinux_pal_thread_vtid_to_root_tid(pid);
#endif
	ret = npu_svm_flushcache_bypid(cur_dev_ctx, pid);
	cond_return_error(ret != 0, -EINVAL,
		"npu svm flush cache fail, ret = %d\n", ret);
	return ret;
}
...
static int (*npu_ioctl_call[NPU_MAX_CMD])(struct npu_proc_ctx *proc_ctx, unsigned long arg) = {
	...
	[_IOC_NR(NPU_GET_SVM_SSID)] = npu_ioctl_smartisp_get_svm_ssid,
	...
	[_IOC_NR(NPU_GET_ERROR_TASK_INFO)] = npu_ioctl_get_err_task_info,
#ifndef NPU_ARCH_V100
	[_IOC_NR(NPU_MODEL_BANDWIDTH_SET)] = npu_ioctl_set_model_bandwidth,
#endif
	[_IOC_NR(NPU_SVM_FLUSH_CACHE)] = npu_ioctl_svm_flush_cache,
#ifdef CONFIG_NPU_SYSCACHE
