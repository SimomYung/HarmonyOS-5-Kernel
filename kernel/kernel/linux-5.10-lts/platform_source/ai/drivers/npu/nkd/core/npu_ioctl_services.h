...
#define NPU_MODEL_BANDWIDTH_SET        _IO(NPU_ID_MAGIC, 8)
...
#define NPU_SVM_FLUSH_CACHE           _IO(NPU_ID_MAGIC, 49)
#define NPU_GET_ERROR_TASK_INFO       _IO(NPU_ID_MAGIC, 50)
...
int npu_ioctl_get_model_execute_info(struct npu_proc_ctx *proc_ctx, unsigned long arg);

int npu_ioctl_set_model_execute_enable(struct npu_proc_ctx *proc_ctx, unsigned long arg);
...
int npu_ioctl_svm_flush_cache(struct npu_proc_ctx *proc_ctx, unsigned long arg);
