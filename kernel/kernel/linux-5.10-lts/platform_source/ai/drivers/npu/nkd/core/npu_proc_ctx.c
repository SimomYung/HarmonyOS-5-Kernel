...
void npu_proc_ctx_destroy(struct npu_proc_ctx **proc_ctx_ptr)
{
	...
	mutex_destroy(&((*proc_ctx_ptr)->resp_lock));
...
static int npu_get_proc_mm(struct npu_proc_ctx *proc_ctx)
{
	int ret = 0;
#ifndef CONFIG_NPU_OHOS
	...
	pid_struct = find_get_pid(proc_ctx->pid);
	cond_goto_error(pid_struct == NULL, last_deal, ret, -1, "get pid failed\n");

	task = get_pid_task(pid_struct, PIDTYPE_PID);
	cond_goto_error(task == NULL, free_pid, ret, -1, "get pid task failed\n");

	mm = get_task_mm(task);
	cond_goto_error(mm == NULL, free_task, ret, -1, "get task mm failed\n");
	npu_drv_warn("pid: %d\n", proc_ctx->pid);
...
static void npu_put_proc_mm(struct npu_proc_ctx *proc_ctx)
{
	u32 work_mode;
	struct npu_dev_ctx *cur_dev_ctx = NULL;
	
	cur_dev_ctx = get_dev_ctx_by_id(proc_ctx->devid);
	cond_return_void(cur_dev_ctx == NULL, "invalid dev id %u\n", proc_ctx->devid);
	work_mode = cur_dev_ctx->pm.work_mode;
	if ((proc_ctx->dev_type == DIRECT) && ((npu_bitmap_get_ffts(work_mode) != 0)
		|| (npu_bitmap_get_ispnn(work_mode) != 0))) {
		npu_drv_warn("wait for ts to recycle resources: %u\n", work_mode);
		mdelay(100);
	}
#ifndef CONFIG_NPU_OHOS
	if (proc_ctx->mm != NULL) {
...
static void npu_release_proc_ctx(struct npu_proc_ctx *proc_ctx)
{
	int ret = 0;
	struct npu_platform_info *plat_info = npu_plat_get_info();
...
#if (defined NPU_ARCH_V200) || (defined NPU_ARCH_V210) || (defined NPU_ARCH_V300)
		(void)npu_svm_unbind(proc_ctx->devid, proc_ctx->pid);
#else
		npu_drv_warn("direct svm can't unbind here, pid = %d\n", current->tgid);
#endif
	}
...
struct npu_ts_cq_info *npu_proc_alloc_cq(struct npu_proc_ctx *proc_ctx)
{
	...
	if (cq_info == NULL) {
		npu_drv_err("npu dev %u cq_info is null\n", dev_id);
		return NULL;
	}
...
int npu_proc_send_alloc_stream_mailbox(struct npu_proc_ctx *proc_ctx)
{
	...
	list_for_each_safe(pos, n, &proc_ctx->stream_list) {
		...
		if (ret != 0) {
			npu_drv_err("invalid param, cur_dev_id %u, cq_id %d", cur_dev_id, stream_info->cq_index);
			return ret;
		}
	...
	u32 sq_id)
{
	...
	cond_return_error(sq_sub_info == NULL, -1, "sq_sub_info ptr is NULL\n");
		...
		u32 *stream_sq_id, u32 *stream_sq_used_round)
{
	...
	cond_return_error(sq_id >= NPU_MAX_SQ_NUM, -1, "alloc sq from dev %u failed\n",
		proc_ctx->devid);
...
EXPORT_SYMBOL(npu_proc_check_notify_id);
...
int npu_proc_alloc_stream(struct npu_proc_ctx *proc_ctx,
	struct npu_stream_info **stream_info, u32 strategy, u16 priority)
{
	int ret;
	u32 stream_cq_id = 0;
	u32 stream_sq_id = 0;
	...
	cond_return_error(ret != 0, -1, "get stream_cq_id failed, strategy:%u\n",
		strategy);
	ret = npu_proc_get_stream_sq_id(proc_ctx, strategy, &stream_sq_id,
		&stream_sq_used_round);
	cond_return_error(ret != 0, -1, "get stream_sq_id failed, strategy:%u\n",
		strategy);

	npu_drv_debug("stream sq_id:%d cq_id:%d, strategy:%u\n", stream_sq_id,
...
#ifdef CONFIG_NPU_SWTS
		struct schedule_stream *sched_stream = sched_stream_mngr_get_stream((int32_t)(*stream_info)->id);
		cond_return_error(sched_stream == NULL, -1, "sched_stream_mngr_get_stream null\n");
		sched_stream_init(sched_stream, (int32_t)(*stream_info)->id, proc_ctx->resp_queue);
#endif
...
#ifdef CONFIG_NPU_FFRT
	ret = npu_ffrt_get_sched_path(proc_ctx, model_id, &sched_path);
	cond_goto_error(ret != 0, ffrt_out, ret, -EFAULT, "get sched path error, ret= %d\n", ret);

	if (sched_path == NPU_SCHED_FFTS) {
		ret = npu_ffrt_set_ssid_by_enable_state(proc_ctx, model_id, 0);
		cond_goto_error(ret != 0, ffrt_out, ret, -EFAULT, "set npu ffrt id error, ret= %d\n", ret);
...
EXPORT_SYMBOL(npu_proc_get_model_info);
	...
	struct npu_platform_info *plat_info)
{
	...
	ret = npu_get_proc_mm(proc_ctx);
	if (ret != 0) {
		npu_drv_err("get proc mm failed\n");
...
int npu_open_common(struct inode *inode, struct file *filep, npu_dev_type dev_type)
{
	...
	npu_drv_warn("start. minor = %u, dev_type = %u\n", minor, dev_type);
...
int npu_release(struct inode *inode, struct file *filep)
{
	...
	if (npu_is_proc_resource_leaks(proc_ctx) == true) {
		...
		if (proc_ctx->dev_type == NPU)
			npu_raise_server_released_exception(cur_dev_ctx, proc_ctx);
		else
...
void npu_execute_err_cnt(uint8_t dev_id, uint16_t model_id)
{
	...
	++(id_entity->data[MODEL_PARAM_REBOOT_TIMES]);
	mutex_unlock(&id_allocator->lock);
}

int npu_proc_get_execute_info(struct npu_proc_ctx *proc_ctx, struct npu_execute_info *excute_info)
{
	...
	u32 execute_times = 0;
	u32 reboot_times = 0;
	struct npu_model_execute_info *model_execute_info = NULL;
	int ret = 0;

	id_allocator = npu_get_model_entity(dev_id);
	cond_return_error(id_allocator == NULL, -EINVAL, "id allocator handel is null\n");
	model_execute_info = kzalloc(sizeof(struct npu_model_execute_info) * NPU_MAX_MODEL_ID, GFP_KERNEL);
	cond_return_error(model_execute_info == NULL, -ENOMEM, "alloc memory for model_execute_info failed\n");
	...
	for (model = g_curr_model; model < NPU_MAX_MODEL_ID; model++) {
		...
		execute_times = id_entity->data[MODEL_PARAM_EXECUTE_TIMES];
		reboot_times = id_entity->data[MODEL_PARAM_REBOOT_TIMES];
		if (execute_times == 0 && reboot_times == 0) {
			continue;
		}

		model_execute_info[num].model_id = id_entity->id;
		model_execute_info[num].uid = id_entity->data[MODEL_PARAM_UID];
		model_execute_info[num].execute_times = execute_times;
		model_execute_info[num].reboot_times = reboot_times;

		id_entity->data[MODEL_PARAM_EXECUTE_TIMES] = 0;
		id_entity->data[MODEL_PARAM_REBOOT_TIMES] = 0;
		num++;
		// can get model_cnt info max, if not all, will get agian
		if (num >= excute_info->model_cnt) {
			g_curr_model = model;
			mutex_unlock(&id_allocator->lock);
			excute_info->is_all_data = NPU_GET_PART_EXEC_INFO;
			excute_info->size = num;
			ret = copy_to_user_safe((void *)(uintptr_t)excute_info->execute_info_addr, model_execute_info,
							  num * sizeof(struct npu_model_execute_info));
			kfree(model_execute_info);
			return ret;
		}
	}
	...
	g_curr_model = 0;
	mutex_unlock(&id_allocator->lock);
	excute_info->is_all_data = NPU_GET_ALL_EXEC_INFO;
	excute_info->size = num;
	if (num != 0) {
		ret = copy_to_user_safe((void *)(uintptr_t)excute_info->execute_info_addr, model_execute_info,
					  num * sizeof(struct npu_model_execute_info));
...
int npu_proc_set_execute_enable(struct npu_proc_ctx *proc_ctx, struct npu_enable_info *enable_info)
{
	...
	if (id_entity->data[MODEL_PARAM_EXECUTE_ENABLE] != enable_info->flag) {
		npu_drv_warn("set model(%u) execute flag from %u to %u\n",
			enable_info->model_id, id_entity->data[MODEL_PARAM_EXECUTE_ENABLE], enable_info->flag);
		id_entity->data[MODEL_PARAM_EXECUTE_ENABLE] = enable_info->flag;
	}
...
#ifdef CONFIG_NPU_FFRT
static int npu_ffrt_get_model_execute_pid(struct npu_proc_ctx *proc_ctx, uint32_t model_id, uint32_t *execute_pid)
{
	int ret;
	...
	*execute_pid = model_desc_sub.execute_pid;

	return 0;
}

static int npu_ffrt_get_mdc_ssid(struct npu_proc_ctx *proc_ctx, uint32_t model_id, uint16_t *ssid)
{
	int ret;
	uint32_t execute_pid = 0;
	uint64_t ttbr;
	uint64_t tcr;
	
	ret = npu_ffrt_get_model_execute_pid(proc_ctx, model_id, &execute_pid);
	cond_return_error(ret != 0, ret,
		"npu_ffrt_get_model_execute_pid error, ret= %d\n", ret);
	ret = npu_get_ssid_bypid(proc_ctx->devid, current->tgid, (int)execute_pid, ssid, &ttbr, &tcr);
	cond_return_error(ret != 0, ret,
		"npu_get_id_bypid error, ret= %d\n", ret);

	return 0;
}

int npu_ffrt_get_sched_path(struct npu_proc_ctx *proc_ctx, uint32_t model_id, uint32_t *sched_path)
{
	int ret;
	npu_model_desc_sub_t model_desc_sub = {0};
	...
	cond_return_error(ret != 0, ret,
		"npu_proc_get_model_info error, ret= %d\n", ret);
	*sched_path = model_desc_sub.sched_path;

	return 0;
}

int npu_ffrt_set_ssid_by_enable_state(struct npu_proc_ctx *proc_ctx,
	uint32_t model_id, uint32_t enable_state)
{
	int ret = 0;
	uint16_t ssid = NPU_INVALID_SSID;

	if (enable_state == 1) {
		ret = npu_ffrt_get_mdc_ssid(proc_ctx, model_id, &ssid);
		cond_return_error(ret != 0, ret,
			"npu_ffrt_get_mdc_id error, ret= %d\n", ret);
	}

	ret = npu_plat_ffrt_set_ssid(model_id, ssid);
	cond_return_error(ret != 0, ret,
			"npu_ffrt_set_id error, ret= %d\n", ret);
