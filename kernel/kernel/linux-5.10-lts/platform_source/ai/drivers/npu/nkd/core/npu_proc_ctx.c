...
#ifdef NPU_ARCH_V100
	npu_drv_info("cur_dev_ctx->pm.pm_bitmap:0x%x", cur_dev_ctx->pm.pm_bitmap);
	proc_ctx->pm_bitmap_index = alloc_pm_bitmap_index(cur_dev_ctx);
	if (proc_ctx->pm_bitmap_index >= PROC_INDEX_NUM) {
		npu_drv_err("alloc pm_bitmap_index failed, pm_bitmap_index=%u \n", proc_ctx->pm_bitmap_index);
		proc_ctx->pm_bitmap_index = 0xff;
	}
#endif
...
int npu_proc_get_execute_info(struct npu_proc_ctx *proc_ctx, struct npu_execute_info *excute_info)
{
	...
	for (model = g_curr_model; model < NPU_MAX_MODEL_ID; model++) {
		...
		model_execute_info[num].uid = id_entity->data[6];  // // 6 is uid
