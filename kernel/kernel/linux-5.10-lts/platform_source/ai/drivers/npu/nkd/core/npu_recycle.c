...
static void npu_recycle_model_id(struct npu_proc_ctx *proc_ctx)
{
	...
	list_for_each_safe(pos, n, &proc_ctx->model_list) {
		...
		if (model_info != NULL) {
#ifdef CONFIG_NPU_SWTS
			destory_sched_model((uint16_t)model_info->id);
#endif
...
static bool npu_task_has_remain(struct npu_proc_ctx *proc_ctx)
{
	if (proc_ctx->task_cnt[NPU_NONSEC_EFF] != 0 || proc_ctx->task_cnt[NPU_NONSEC_GE] != 0 ||
		proc_ctx->task_cnt[NPU_NONSEC] != 0 || proc_ctx->task_cnt[NPU_HTS] != 0 ||
		proc_ctx->task_cnt[NPU_HTS_GE] != 0 || proc_ctx->task_cnt[NPU_HTS_EFF] != 0
		) {
		return true;
	}
	return false;
...
void npu_recycle_npu_resources(struct npu_proc_ctx *proc_ctx)
{
	...
	if (proc_ctx->dev_type == DIRECT) {
		if (npu_task_has_remain(proc_ctx)) {
