	...
	unsigned long arg)
{
	...
	if (!npu_plat_query_sku_value(NPU_SKU_VAR_SUPPORT_SC)) {
	...
	ret = npu_dma_buf_attach_lb(msg.fd, pid, msg.offset, (size_t)msg.size);
	cond_return_error(ret != 0, -EINVAL,
...
void npu_sc_disable(struct npu_dev_ctx *dev_ctx)
{
	...
	if (g_syscache_state == NPU_SYSCACHE_DISABLE) {
		npu_drv_warn("npu syscache is already disabled\n");
		mutex_unlock(&dev_ctx->syscache_mutex);
		return;
	}
