...
#if defined(CONFIG_NPU_PM_SMC) && defined(CONFIG_NPU_DEBUG)
#include <platform_include/cee/linux/npu_pm.h>
#endif

#define NPU_DUMP_BBOXINFO_TIMEOUT_MSEC 500
#define NPU_MAX_SELF_UID 10000
 ...
 */
#ifndef CONFIG_NPU_SWTS
static void npu_mntn_dump_tsch_log(void)
{
	char *tsch_log_title = "\r\n[TSCH log dump]:";
	char *dump_buf = NULL;
	char log_buf[NPU_BUF_LEN_MAX + 3];
	uint32_t pos = 0;
	...
	while (pos < pos_max) {
		...
		if (dump_buf[pos] == 0xCD && dump_buf[pos + 1] == 0xAB) {
			...
			if (tslog_header->length <= NPU_BUF_LEN_MAX && tslog_header->length > 0) {
				...
				log_buf[tslog_header->length + 2] = '\0'; /* to avoid if no \0 in original tslog */
...
static void npu_mntn_query_mem_region(uint8_t wptr)
{
	...
	const int max_mem_id = (int)(sizeof(struct mem_region_info) / sizeof(struct buffer_info));
...
{
	uint8_t wptr = g_npu_task_dump_manager.wptr % NPU_TASK_DUMP_NUM;
	char log_buf[NPU_BUF_LEN_MAX + 1] = {0};
	int try_count;
	int ret;
	int count = 0;
	...
	do {
		...
		if (g_npu_task_dump_manager.flag[wptr] == NPU_TASK_DUMP_IDLE)
			return;
		...
		while (try_count-- > 0 && g_npu_task_dump_manager.flag[wptr] == NPU_TASK_DUMP_INCOMPLETE)
			msleep(1);
		if (g_npu_task_dump_manager.flag[wptr] == NPU_TASK_DUMP_INCOMPLETE)
			g_npu_task_dump_manager.flag[wptr] = NPU_TASK_DUMP_TIMEOUT;
		...
		ret = snprintf_s(log_buf, NPU_BUF_LEN_MAX + 1, NPU_BUF_LEN_MAX,
				"%d-%d-%d %d:%d:%d\r\n", g_npu_task_dump_manager.tm[wptr].tm_year + 1900,
				g_npu_task_dump_manager.tm[wptr].tm_mon + 1, g_npu_task_dump_manager.tm[wptr].tm_mday,
				g_npu_task_dump_manager.tm[wptr].tm_hour, g_npu_task_dump_manager.tm[wptr].tm_min,
				g_npu_task_dump_manager.tm[wptr].tm_sec);
		cond_return_void(ret < 0, "snprintf_s failed, ret = %d\n", ret);
		(void)npu_mntn_copy_reg_to_bbox(log_buf, (unsigned int)strlen(log_buf));
		if (g_npu_task_dump_manager.info[wptr].func_name[0] != 0) {
			...
			ret = snprintf_s(log_buf, NPU_BUF_LEN_MAX + 1, NPU_BUF_LEN_MAX,
				...
				g_npu_task_dump_manager.info[wptr].model_id, g_npu_task_dump_manager.info[wptr].strm_id,
				g_npu_task_dump_manager.info[wptr].task_id);
			cond_return_void(ret < 0, "snprintf_s failed, ret = %d\n", ret);
			(void)npu_mntn_copy_reg_to_bbox(log_buf, (unsigned int)strlen(log_buf));
		}
		if (count == 0)
...
#ifndef CONFIG_NPU_SWTS
static void npu_mntn_dump_ts_bbox(void)
{
	exce_module_info_t *module_info = NULL;
	vir_addr_t bbox_ts_core_dump_log_addr_vir;
	size_t addr_vir_ofs;
	char *tsch_bbox_title = "\r\n[TSCH core dump]:\r\n";
	...
	if (module_info->exp_message_len > module_info->buffer_len ||
		...
		bbox_ts_core_dump_log_addr_vir = g_bbox_addr_vir + addr_vir_ofs;
		if (*(u64 *)(uintptr_t)bbox_ts_core_dump_log_addr_vir != 0) {
			npu_mntn_copy_reg_to_bbox(tsch_bbox_title, (unsigned int)strlen(tsch_bbox_title));
			npu_mntn_copy_reg_to_bbox((void *)((uintptr_t)bbox_ts_core_dump_log_addr_vir),
				module_info->exp_message_len);
			(void)memset_s((void *)((uintptr_t)bbox_ts_core_dump_log_addr_vir), module_info->exp_message_len,
				0, module_info->exp_message_len);
		}
	}
}
#endif
...
#ifndef CONFIG_NPU_SWTS
	/* dump tsch log */
	npu_mntn_dump_tsch_log();

	npu_mntn_dump_ts_bbox();
#endif
...
{
	(void)npu_plat_notify(NULL, NPD_NOTICE_MNTN_RESET, 0, 0);
}
 ...
 */
static int npu_rdr_register_exception(void)
{
	size_t size;
	unsigned int ret;
	unsigned long index;
	...
	for (index = 0; index < size; index++) {
		...
		if (ret == 0) {
			npu_drv_err("register rdr exception is failed! index = %lu, ret = %u",
				index, ret);
			return -EINTR;
		}
	}
...
static int npu_rdr_unregister_exception(void)
{
	...
	for (index = 0; index < size; index++) {
		...
		if (ret < 0) {
			npu_drv_err("unregister rdr exception is failed! index = %lu, ret = %d",
				index, ret);
			return -EINTR;
		}
	}
...
static void npu_mntn_rdr_reset(u32 modid, u32 etype, u64 coreid)
{
	...
	if ((modid >= RDR_EXC_TYPE_NOC_NPU0) && (modid <= RDR_EXC_TYPE_NOC_NPU7))
		queue_work(system_wq, &g_npu_mntn_private->reset_work);
}
 ...
 */
static int npu_rdr_register_core(void)
{
	int ret;
 ...
 * return : int:  0 is OK. other value is Error No.
 */
static int npu_rdr_addr_map(void)
{
	int ret;
	if (g_npu_mntn_private == NULL) {
...
static int npu_blackbox_addr_release(void)
{
	if (g_bbox_addr_vir != 0) {
		iounmap((void *)((uintptr_t)g_bbox_addr_vir));
		g_bbox_addr_vir = 0;
...
static struct rdr_exception_info g_rdr_exception_info[] = {
		...
		{RDR_TYPE_HWTS_TIMEOUT, 1, 0},
		/* FFTS/HTS exception code 0xc0000900-0xc00009ff */
		{EXC_TYPE_WAIT_FFTS_EXCEPTION, 1, 0}
};
...
int npu_get_uid_by_model_id(struct npu_dev_ctx *dev_ctx, u16 model_id, u32 *uid)
{
	struct npu_id_allocator *id_allocator = NULL;
	struct npu_id_entity *id_entity = NULL;

	cond_return_error(model_id >= NPU_MAX_MODEL_ID, -1, "model_id error, model_id is %u.\n", model_id);

	id_allocator = &(dev_ctx->id_allocator[NPU_ID_TYPE_MODEL]);
	cond_return_error(id_allocator == NULL, -1, "get id allocator error, model_id is %u.\n", model_id);
	...
	if (id_entity == NULL || id_entity->id != model_id) {
		npu_drv_err("fatal error, can not find id entity id = %u\n", model_id);
		mutex_unlock(&id_allocator->lock);
		return -1;
	}
	*uid = id_entity->data[MODEL_PARAM_UID];
	mutex_unlock(&id_allocator->lock);

	return 0;
}

void npu_rdr_exception_report(uint32_t error_code, uint32_t uid)
{
	cond_return_void(uid > NPU_MAX_SELF_UID, "Third party do not need reported.\n");
...
#ifdef CONFIG_NPU_FFRT
static int npu_get_persist_task_info(u8 devid, struct hwts_exception_report_info *report)
{
	...
	struct hwts_sqe_head *hwts_sqe = NULL;

	model_desc_info = npu_calc_model_desc_info(devid, report->model_id);
	cond_return_error(model_desc_info == NULL, -1, "model_desc_info is NULL\n");

	cond_return_error(report->rtsq_idx >= NPU_MODEL_STREAM_NUM, -1, "invalid rtsq idx %u", report->rtsq_idx);
	report->persist_stream_id = model_desc_info->stream_id[report->rtsq_idx];

	stream_info = npu_calc_stream_info(devid, report->persist_stream_id);
	cond_return_error(stream_info == NULL, -1,
		"stream_id= %u stream_info is null\n", report->persist_stream_id);
	...
	cond_return_error(hwts_sq_info->iova_addr == 0, -1,
		"iova_addr is empty: stream= %d, sq_index= %u\n",
		stream_info->id, stream_info->sq_index);
	
	entity_info = (struct npu_entity_info *)(uintptr_t)hwts_sq_info->hwts_sq_sub;
	hwts_sqe = (struct hwts_sqe_head *)(uintptr_t)
		(entity_info->vir_addr + report->excp_sq_head * NPU_HWTS_SQ_SLOT_SIZE);
	report->persist_task_id = hwts_sqe->task_id;
	return 0;
}
#endif

void npu_rdr_taskerr_exception_report(struct npu_dev_ctx *dev_ctx, uint32_t error_code,
	struct hwts_exception_report_info *report)
{ 
	u32 uid = 0;
	u16 model_id = 0;
	...
	if (report != NULL &&
		(report->service_type == NPU_SERVICE_TYPE_NONSEC || report->service_type == NPU_SERVICE_TYPE_FFRT)) {
#ifdef CONFIG_NPU_FFRT
		if (report->service_type == NPU_SERVICE_TYPE_FFRT)
			// get persist stream + persist task
			npu_get_persist_task_info(dev_ctx->devid, report);
#endif
		npu_drv_warn("model_id = %u, stream_id = %u, task_id = %u, service_type = %u\n",
			report->model_id, report->persist_stream_id, report->persist_task_id, report->service_type);
		model_id = report->model_id;
		(void)npu_get_uid_by_model_id(dev_ctx, model_id, &uid);

		ktime_get_real_ts64(&tv);
		tv.tv_sec -= (long)sys_tz.tz_minuteswest * 60;
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(5,10,0))
		rtc_time64_to_tm(tv.tv_sec, &g_npu_task_dump_manager.tm[g_npu_task_dump_manager.wptr]);
#else
		rtc_time_to_tm(tv.tv_sec, &g_npu_task_dump_manager.tm[g_npu_task_dump_manager.wptr]);
#endif
	...
	npu_rdr_exception_report(error_code, uid);
}
...
static int npu_bbox_find_err_task(uint16_t model_id)
{
	uint8_t wptr = g_npu_task_dump_manager.wptr % NPU_TASK_DUMP_NUM;
	uint8_t index = wptr == 0 ? (NPU_TASK_DUMP_NUM - 1) : wptr - 1;

	while (index != wptr) {
		if (g_npu_task_dump_manager.info[index].model_id == model_id &&
			g_npu_task_dump_manager.flag[index] == NPU_TASK_DUMP_INCOMPLETE)
			return index;
		index = (index == 0 ? (NPU_TASK_DUMP_NUM - 1) : index - 1);
	}

	return NPU_TASK_DUMP_NUM;
}

int npu_bbox_get_err_task_info(struct npu_task_dump_info *error_task)
{
	int index;
	int trycount = 1000;

	if (error_task == NULL || error_task->model_id >= NPU_MAX_MODEL_ID) {
		npu_drv_err("invalid param\n");
		return -1;
	}

	while (trycount-- > 0) {
		index = npu_bbox_find_err_task(error_task->model_id);
		if (index >= NPU_TASK_DUMP_NUM) {
			// maybe exception hb has not record, wait a while
			mdelay(1);
			continue;
		}
		error_task->strm_id = g_npu_task_dump_manager.info[index].strm_id;
		error_task->task_id = g_npu_task_dump_manager.info[index].task_id;
		npu_drv_warn("get err task info succ, model_id = %u, stream_id = %u, task_id = %u",
			error_task->model_id, error_task->strm_id, error_task->task_id);
		return 0;
	}

	npu_drv_err("get err task info timeout, maybe its a global excp");
	return -1;
}
...
void npu_dump_current_voltage(void)
{
#if defined(CONFIG_NPU_PM_SMC) && defined(CONFIG_NPU_DEBUG)
	char log_buf[NPU_BUF_LEN_MAX + 1] = {0};
	int cur_volt;
	...
	cur_volt = npu_pm_get_voltage_smc_request();
	ret = snprintf_s(log_buf, NPU_BUF_LEN_MAX + 1, NPU_BUF_LEN_MAX,
		"\r\ncurrent npu voltage: %d\r\n", cur_volt);
	cond_return_void(ret < 0, "snprintf_s failed, ret = %d\n", ret);
	(void)npu_mntn_copy_reg_to_bbox(log_buf, (unsigned int)strlen(log_buf));
#endif
}