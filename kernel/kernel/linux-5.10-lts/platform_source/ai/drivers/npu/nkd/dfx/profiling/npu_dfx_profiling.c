	...
	u32 circle_len, uint64_t src_addr, uint64_t dest)
{
	...
	if (src_addr == 0 || ring_buff == NULL || dest == 0 ||
		...
		npu_drv_err("addr is null. src_addr:0x%pK, ring_buff:0x%pK, dest:0x%pK, circle_len:%u\n",
			(void *)(uintptr_t)src_addr, (void *)(uintptr_t)ring_buff, (void *)(uintptr_t)dest, circle_len);
		return PROF_ERROR;
	}
	...
	if (ring_buff->read < ring_buff->write) {
		...
		if (ret > 0) {
			npu_drv_err("copy_to_user fail, src_addr:0x%pK, dest:0x%pK, write:0x%x, read:0x%x\n",
				(void *)(uintptr_t)src_addr, (void *)(uintptr_t)dest, ring_buff->write, ring_buff->read);
			return PROF_ERROR;
		}
		return PROF_OK;
...
static int npu_prof_buff_init(void)
{
	...
	cond_return_error(plat_info == NULL, PROF_ERROR, "get plat info fail\n");

	profile_addr = plat_info->resmem_info.resmem_desc[NPU_PROF_MEM_IDX].base;
	profile_size = plat_info->resmem_info.resmem_desc[NPU_PROF_MEM_IDX].len;
	...
	ret = memset_s(g_prof_buff, profile_size, 0, profile_size);
