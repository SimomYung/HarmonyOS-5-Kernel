...
int prof_buffer_init(struct prof_buff_desc *prof_buff)
{
	...
	unsigned long iova;
#endif
	vir_addr_t addr[PROF_CHANNEL_MAX] = {0};
	
	cond_return_error(prof_buff == NULL, PROF_ERROR, "prof buf nullptr\n");

#ifndef CONFIG_NPU_PHY_MEM
	/* init tscpu data buffer */
	ret = prof_buff_alloc(PROF_TSCPU_DATA_SIZE, &addr[PROF_CHANNEL_TSCPU], &iova);
	if (ret != 0) {
		npu_drv_err("prof_buff_alloc fail,goal = tscpu_data,size = 0x%x\n",
			PROF_TSCPU_DATA_SIZE);
		return PROF_ERROR;
	}
	prof_buff->head.manager.ring_buff[PROF_CHANNEL_TSCPU].iova_addr = (unsigned)iova;

	/* init hwts log data buffer */
	ret = prof_buff_alloc(PROF_HWTS_LOG_SIZE, &addr[PROF_CHANNEL_HWTS_LOG], &iova);
	if (ret != 0) {
	...
	prof_buff->head.manager.ring_buff[PROF_CHANNEL_HWTS_LOG].iova_addr = (unsigned)iova;

	/* init hwts profiling data buffer */
	ret = prof_buff_alloc(PROF_HWTS_PROFILING_SIZE, &addr[PROF_CHANNEL_HWTS_PROFILING], &iova);
	if (ret != 0) {
		prof_buff_free(addr[PROF_CHANNEL_TSCPU],
			prof_buff->head.manager.ring_buff[PROF_CHANNEL_TSCPU].iova_addr);
	...
	prof_buff->head.manager.ring_buff[PROF_CHANNEL_HWTS_PROFILING].iova_addr = (unsigned)iova;
#else
