	...
	npu_rt_task_t *ts_task)
{
	...
	ts_task->u.model_maintenance_task.server_pid = current->tgid;
#ifndef NPU_ARCH_V100
	if (ts_task->u.model_maintenance_task.operation_type ==
		NPU_MMT_STREAM_UNBIND) {
		model_desc_info = npu_calc_model_desc_info(proc_ctx->devid, ts_task->u.model_maintenance_task.model_id);
		cond_return_error(model_desc_info == NULL, -1,
			"model_desc_info is NULL\n");
		model_desc_info->stream_cnt = 0;
	}
#endif
...
#ifdef NPU_ARCH_V100
	if (ts_task->u.model_execute_task.pid != current->tgid) {
		ts_task->u.model_execute_task.pid = current->tgid;
	}
#endif
#endif
#ifndef NPU_ARCH_V100
	model_desc_info = npu_calc_model_desc_info(proc_ctx->devid, ts_task->u.model_execute_task.model_id);
	cond_return_error(model_desc_info == NULL, -1,
		"model_desc_info is NULL\n");
	if (model_desc_info->stream_cnt == 0)
		return -1;
#endif
	/* no need to verify ts_task->u.model_maintenance_task.first_task_id,
	 * since it's not used after the first edition
