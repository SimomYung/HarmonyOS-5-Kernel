...
int npu_cq_list_init(u8 dev_id)
{
	...
	npu_drv_debug("cur dev %u own %d calc cq\n", dev_id, cur_dev_ctx->cq_num);
...
static u32 npu_alloc_cq_id(u8 dev_id)
{
	...
	if (list_empty_careful(&cur_dev_ctx->cq_available_list)) {
		spin_unlock_irqrestore(&cur_dev_ctx->spinlock, flags);
		npu_drv_err(
			"cur dev %u available cq list empty, left cq_num = %d !!!\n",
			dev_id, cur_dev_ctx->cq_num);
		return NPU_MAX_CQ_NUM;
	}
	...
	cur_dev_ctx->cq_num--;
	spin_unlock_irqrestore(&cur_dev_ctx->spinlock, flags);
	npu_drv_debug("cur dev %u alloc:%d, left %d cq\n", dev_id, cq_sub->index,
		cur_dev_ctx->cq_num);
...
static int npu_free_cq_id(u8 dev_id, u32 cq_id)
{
	...
	cq_info->slot_size = NPU_CQ_SLOT_SIZE;
	spin_unlock_irqrestore(&cur_dev_ctx->spinlock, flags);
	npu_drv_warn("cur dev %u own %d cq\n", dev_id, cur_dev_ctx->cq_num);
...
static int npu_alloc_cq_mem(u8 dev_id, u32 cq_id, int pid)
{
	...
	if (cq_virt_addr == 0) {
		npu_drv_err("cur_dev_ctx %u calc cq ioremap_wc failed\n", dev_id);
		return -1;
	}
	...
	npu_drv_debug("dev %u cur cq %d phy_addr = %pK "
...
static int npu_free_cq_mem(u8 dev_id, u32 cq_id)
{
	...
	cq_sub->phy_addr = 0;
	spin_unlock_irqrestore(&cur_dev_ctx->spinlock, flags);
	npu_drv_debug("free dev %u cur cq %d memory success\n", dev_id, cq_id);
...
struct npu_ts_cq_info *npu_alloc_cq(u8 dev_id, int pid)
{
	...
	if (cq_id >= NPU_MAX_CQ_NUM) {
		npu_drv_err("alloc cq_id from dev %u failed\n", dev_id);
		return NULL;
	}
	...
	if (ret != 0) {
		npu_free_cq_id(dev_id, cq_id);
		npu_drv_err("alloc cq mem from dev %u cq %d failed\n", dev_id, cq_id);
		return NULL;
	}
	...
	if (ret != 0) {
		...
		npu_free_cq_id(dev_id, cq_id);
		npu_drv_err("clear cq from dev %u cq %d failed\n", dev_id, cq_id);
		return NULL;
	}
