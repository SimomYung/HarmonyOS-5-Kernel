...
int npu_sq_list_init(u8 dev_id)
{
	...
	npu_drv_debug("cur dev %u own %d calc sq\n", dev_id, cur_dev_ctx->sq_num);
...
static u32 npu_alloc_sq_id(u8 dev_id)
{
	...
	if (list_empty_careful(&cur_dev_ctx->sq_available_list)) {
		...
		npu_drv_warn("cur dev %u available sq list empty,"
			"left sq_num = %d !!!\n", dev_id, cur_dev_ctx->sq_num);
		return NPU_MAX_SQ_NUM;
	}
	sq_sub = list_first_entry(&cur_dev_ctx->sq_available_list,
		struct npu_sq_sub_info, list);
	if (((u64)(uintptr_t)sq_sub >> 32) == (u64)0xdead0000) {
		spin_unlock_irqrestore(&cur_dev_ctx->spinlock, flags);
	...
	cur_dev_ctx->sq_num--;
	spin_unlock_irqrestore(&cur_dev_ctx->spinlock, flags);
	npu_drv_debug("cur dev %u alloc:%d, left %d sq\n", dev_id, sq_sub->index,
		cur_dev_ctx->sq_num);
...
static int npu_alloc_sq_mem(u8 dev_id, u32 sq_id)
{
	...
	npu_drv_debug("dev %u cur sq %d phy_addr = %pK\n", dev_id, sq_id,
...
int npu_get_sq_ref_by_stream(u8 dev_id, u32 sq_id)
{
	...
	npu_drv_debug("sq calc channel %u for ref_by_streams = %d\n",
...
static int npu_free_sq_id(u8 dev_id, u32 sq_id)
{
	...
	if (sq_sub->ref_by_streams != 0) {
		spin_unlock_irqrestore(&cur_dev_ctx->spinlock, flags);
		npu_drv_debug("can't release cur_dev_ctx %u "
	...
	sq_info->send_count = 0;
	spin_unlock_irqrestore(&cur_dev_ctx->spinlock, flags);
	npu_drv_debug("cur dev %u own %d sq\n", dev_id, cur_dev_ctx->sq_num);
