...
int npu_hwts_sq_init(u8 dev_id)
{
	...
	for (i = 0; i < NPU_MAX_HWTS_SQ_NUM; i++) {
		...
		hwts_sq_info->head = 0;
		hwts_sq_info->tail = 0;
		if (NPU_MAX_LONG_HWTS_SQ_NUM == 0) {
			hwts_sq_info->credit = NPU_MAX_HWTS_SQ_DEPTH - 1;
		} else {
...
int npu_free_hwts_sq(u8 dev_id, u32 hwts_sq_id)
{
	...
	if (NPU_MAX_LONG_HWTS_SQ_NUM == 0) {
		ret = npu_free_entity(dev_id, NPU_MEM_POOL_TYPE_HWTS_SQ, hwts_sq_id);
	} else {
