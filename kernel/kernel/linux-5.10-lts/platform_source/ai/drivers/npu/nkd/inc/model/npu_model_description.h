...
#else
#define NPU_MAX_HWTS_SQ_POOL_NUM      1
#endif
...
/* Max value for all version,  the number of streams bound to a model */
#define NPU_MODEL_STREAM_NUM  3
#define NPU_MODEL_EVENT_NUM   128
/* Max value for all version,  the size of notify_info bit map */
#define NPU_MODEL_NOTIFY_NUM  12
...
struct npu_ffrt_load_model_buf_info {
	...
	uint16_t stream_tasks[NPU_MODEL_STREAM_NUM];
	uint64_t aiv_csw_buff_addr[NPU_MODEL_STREAM_NUM];
	uint32_t priority;
	uint16_t ssid;
};
...
typedef struct npu_model_bandwidth_reduct_info {
	uint16_t flag;
	uint16_t mode;
} npu_model_bandwidth_reduct_info_t;
...
typedef struct npu_model_desc_info {
	...
	npu_model_bandwidth_reduct_info_t bandwidth_reduct_info;
	uint32_t server_id;
	uint8_t priority;
	uint8_t reserve2[39]; // 64 bytes cache line align
