...
#define npu_bitmap_get_ffts(work_mode_set) (((u64)(work_mode_set)) & 0x0700U)
#define npu_bitmap_get_ispnn(work_mode_set) (((u64)(work_mode_set)) & 0x000CU)
...
struct ffts_exception_report_info {
	u16 model_id;
	u16 channel_id;
	u32 hwts_sq_id;
	u16 rtsq_idx;
...
struct npu_interframe_idle_manager {
	...
	u32 idle_subip;
	u32 idle_time_out;
	struct delayed_work work[NPU_WORKMODE_MAX];
	atomic_t wq_processing[NPU_WORKMODE_MAX];
};
...
struct npu_power_mgr {
	u32 work_mode;  /* indicates npu current work mode power up */
	u32 work_mode_idle; /* indicates npu work mode which powerup but no tasks */ 
...
#ifdef NPU_ARCH_V100
	u8 pm_bitmap; // v100 record power state for multi-process
	unsigned long proc_index_bitmap;
	struct mutex proc_index_mutex;
#else
	u32 cur_subip_set;
...
#ifdef CONFIG_NPU_FFRT
	struct mutex ffrt_entry_mutex;
#endif
#ifdef CONFIG_HIBERNATION
	struct mutex npu_s4_mutex;
#endif
...
enum npu_exception_type {
	...
	NPU_EXCEPTION_TYPE_HWTS_TASK_TIMEOUT,
	...
	NPU_EXCEPTION_TYPE_HWTS_HTS_ERROR,
	NPU_EXCEPTION_TYPE_HWTS_FFRT_ERROR,
	NPU_EXCEPTION_TYPE_HWTS_FFRT_DRAIN,
	NPU_EXCEPTION_TYPE_HWTS_POWER_TIMEOUT,
	NPU_EXCEPTION_TYPE_HWTS_TIMEOUT, /* The hardware may be faulty. The register cannot be polled. */
