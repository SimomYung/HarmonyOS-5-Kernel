...
#define npu_drv_info(fmt, ...) \
	drv_printk(KERN_INFO, MODULE_DEVDRV"] [I",  fmt, ## __VA_ARGS__)
#define npu_drv_debug(fmt, ...) \
	drv_printk(KERN_DEBUG, MODULE_DEVDRV"] [D",  fmt, ## __VA_ARGS__)
#else
#define npu_drv_info(fmt, ...) do {} while (0)
#define npu_drv_debug(fmt, ...) do {} while (0)
#endif
