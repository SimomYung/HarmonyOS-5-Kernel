...
static int ai_errorno_to_str(int error_no, char *str, int bufflen) {
	size_t i;
	cond_return_error(bufflen <= 0, -1, "bufflen less 0 error.\n");
	struct error_no_map ai_error_no_map[] = {
		{KERN_POWER_UP_ERR, "KERN_POWER_UP_ERR"},
		{KERN_WTD_TIMEOUT_ERR, "KERN_WTD_TIMEOUT_ERR"},
	};
	size_t ai_error_no_map_len = sizeof(ai_error_no_map) / sizeof(ai_error_no_map[0]);
	errno_t ret = 0;

	for (i = 0; i < ai_error_no_map_len; i++) {
		if (ai_error_no_map[i].error_no == error_no) {
			if (strlen(ai_error_no_map[i].name) >= (size_t)bufflen) return -1;
			ret = strncpy_s(str, (size_t)bufflen, ai_error_no_map[i].name, (size_t)(bufflen - 1));
...
struct dsm_client_ops ai_dsm_ops = {
	.poll_state = NULL,
	.dump_func = NULL,
	.errorno_to_str = ai_errorno_to_str,
};
...
int npu_manager_init(void)
{
	...
	npu_drv_warn("npu dev %u drv manager init start\n", dev_id);
	plat_info = npu_plat_get_info();
	cond_return_error(plat_info == NULL, -ENODEV,
		"get plat info failed\n");

#if (!defined COMPILE_ST)
	/* bbox black box init */
	...
	cond_goto_error(ret != 0, npu_devinit_failed, ret, -ENODEV,
		"npu dev %u npu devinit failed\n", dev_id);

#ifdef CONFIG_NPU_DIRECT
	ret = npu_dev_direct_init();
	cond_goto_error(ret != 0, npu_devinit_failed, ret, -ENODEV,
		"npu dev %u npu devinit failed\n", dev_id);
#endif
	...
	cond_goto_error(ret != 0, npu_direct_init_failed, ret, ret,
