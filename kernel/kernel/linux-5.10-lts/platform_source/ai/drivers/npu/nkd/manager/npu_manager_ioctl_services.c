...
static int npu_manager_get_devids(unsigned long arg)
{
	...
	if (copy_to_user_safe((void __user *)(uintptr_t)arg,
		...
		npu_drv_err("copy to user failed\n");
		return -EINVAL;
	}
	...
	unsigned int cmd, unsigned long arg)
{
	...
	mutex_lock(&dev_ctx->npu_iova_map_mutex);
	npu_ioctl_check_ionfd(&check_ion);
	cond_goto_error(check_ion.result != NPU_VALID_ION, unlock_return, ret, -EFAULT, "invalid ion\n");

	/* map iova memory */
	ret = npu_iova_map_ion(msg.fd, msg.prot, (unsigned long *)&vaddr, msg.type);
	cond_goto_error(ret != 0, unlock_return, ret, -EFAULT, "npu iova map ion fail, ret = %d\n", ret);
...
unlock_return:
	mutex_unlock(&dev_ctx->npu_iova_map_mutex);
#endif
	return ret;
}
	...
	unsigned int cmd, unsigned long arg)
{
	...
	cond_goto_error(ret != 0, unlock_return, ret, -EFAULT, "npu iova unmap ion fail, ret = %d\n", ret);
