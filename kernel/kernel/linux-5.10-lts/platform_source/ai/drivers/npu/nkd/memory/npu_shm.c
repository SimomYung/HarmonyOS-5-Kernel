...
EXPORT_SYMBOL(get_shm_phy_addr);
...
EXPORT_SYMBOL(get_shm_virt_addr);
...
EXPORT_SYMBOL(npu_calc_stream_info);
...
int npu_shm_init(u8 dev_id)
{
	...
	npu_drv_debug("sq mem: phy_addr = 0x%pK, size = %lu\n",
		g_shm_desc[dev_id][NPU_SQ_MEM].phy_addr,
		g_shm_desc[dev_id][NPU_SQ_MEM].size);

	npu_drv_debug("info mem: virt_addr = 0x%pK, order = %u, size = %lu\n",
...
static int npu_shm_v200_init_sq_mem(u8 dev_id)
{
	...
	g_shm_desc[dev_id][NPU_SQ_MEM].phy_addr = g_sq_desc->base +
		CHIP_BASEADDR_PA_OFFSET * dev_id;
	g_shm_desc[dev_id][NPU_SQ_MEM].size = g_sq_desc->len;
	cond_return_error(g_shm_desc[dev_id][NPU_SQ_MEM].phy_addr == 0, -1, "shm sq phy addr is NULL!");
	...
	npu_drv_debug("sqcq mem: phy_addr = 0x%pK, size = %lu, virt_addr = 0x%pK, "
		"sq phy_addr = 0x%pK, cq phy_addr = 0x%pK, "
		"dfx sq phy_addr = 0x%pK, dfx cq phy_addr = 0x%pK\n",
		g_shm_desc[dev_id][NPU_SQ_MEM].phy_addr,
		g_shm_desc[dev_id][NPU_SQ_MEM].size,
...
static int npu_shm_v200_init_ffrt_buff(u8 dev_id)
{
	vir_addr_t virt_addr;
	struct npu_platform_info *plat_info = NULL;
	int ret = 0;
	...
	plat_info = npu_plat_get_info();
	cond_return_error(plat_info == NULL, -EFAULT, "npu_plat_get_info error");
	// mdc init
	...
	cond_return_error(virt_addr == 0, -EFAULT, "mdc mem ioremap_wc failed");

	g_shm_desc[dev_id][NPU_FFRT_MDC_MEM].virt_addr = virt_addr;
	memset_s((void *)(uintptr_t)virt_addr, g_shm_desc[dev_id][NPU_FFRT_MDC_MEM].size, 0,
		g_shm_desc[dev_id][NPU_FFRT_MDC_MEM].size);
	npu_drv_debug("mdc buff mem: phy_addr = 0x%pK, size = %lu, virt_addr = 0x%pK\n",
		g_shm_desc[dev_id][NPU_FFRT_MDC_MEM].phy_addr,
		g_shm_desc[dev_id][NPU_FFRT_MDC_MEM].size,
	...
	cond_goto_error(virt_addr == 0, init_mdc_mem_failed, ret, -1, "ioremap_wc failed!");

	g_shm_desc[dev_id][NPU_FFRT_SWAP_MEM].virt_addr = virt_addr;
	memset_s((void *)(uintptr_t)virt_addr, g_shm_desc[dev_id][NPU_FFRT_SWAP_MEM].size, 0,
		g_shm_desc[dev_id][NPU_FFRT_SWAP_MEM].size);
	npu_drv_debug("ffrt swap buff mem: phy_addr = 0x%pK, size = %lu, virt_addr = 0x%pK\n",
		g_shm_desc[dev_id][NPU_FFRT_SWAP_MEM].phy_addr,
		g_shm_desc[dev_id][NPU_FFRT_SWAP_MEM].size,
...
int npu_shm_v200_init_swap_buff(u8 dev_id)
{
	...
	plat_info = npu_plat_get_info();
	cond_return_error(plat_info == NULL, -1, "npu_plat_get_info error");

	hwts_swap_buf = plat_info->resmem_info.hwts_swap_buf;
	npu_drv_debug("hwts_swap_buf.base = 0x%pK, hwts_swap_buf.len = 0x%llx\n", hwts_swap_buf->base, hwts_swap_buf->len);
	...
	npu_drv_debug("swap buff mem: phy_addr = 0x%pK, size = %lu, virt_addr = 0x%pK\n",
		g_shm_desc[dev_id][NPU_SWAP_BUFF_MEM].phy_addr,
		g_shm_desc[dev_id][NPU_SWAP_BUFF_MEM].size,
...
static int npu_shm_v200_init_ispnn_info_buff(u8 dev_id)
{
	vir_addr_t virt_addr;
	struct npu_platform_info *plat_info = NULL;
	...
	memset_s((void *)(uintptr_t)virt_addr, g_shm_desc[dev_id][NPU_ISPNN_INFO_MEM].size, 0,
		g_shm_desc[dev_id][NPU_ISPNN_INFO_MEM].size);
	...
	u8 dev_id)
{
	...
	npu_drv_debug("info mem: virt_addr = 0x%pK, size = %lu, "
		"sqinfo phy_addr = 0x%pK, cqinfo phy_addr = 0x%pK, "
		"streaminfo phy_addr = 0x%pK, hwtssqinfo phy_addr = 0x%pK, "
		"hwtscqinfo phy_addr = 0x%pK, modelinfo phy_addr = 0x%pK\n",
		g_shm_desc[dev_id][NPU_INFO_MEM].virt_addr,
		g_shm_desc[dev_id][NPU_INFO_MEM].size,
		g_shm_desc[dev_id][NPU_INFO_MEM].phy_addr,
		g_shm_desc[dev_id][NPU_INFO_MEM].phy_addr +
...
static int npu_shm_v200_init_pad_mem(u8 dev_id)
{
	...
	npu_drv_debug("pad buf mem: phy_addr = 0x%pK, size = %lu\n",
...
int npu_shm_v200_init(u8 dev_id)
{
	...
	if (ret != 0)
		return ret;
#ifdef CONFIG_NPU_PHY_MEM
#if !defined(CONFIG_NPU_SWTS) || (defined(CONFIG_NPU_SWTS) && !defined(CONFIG_NPU_SINK_UDK))
	...
	unsigned long size, unsigned long align_size, pid_t process_id)
{
	...
	npu_drv_debug(
		"zap_vma_ptes.devmem_base=0x%pK, vma_start_aligned=0x%lx, size=0x%lx, align_size=0x%lx\n",
