...
struct npd_registry {
	...
	int (*npd_send_ipc_with_power_check)(u32 cmd_type, u32 msg_sync_type, const u8 *send_msg,
										u32 send_len, enum ipc_send_ack_type ack_type);
#ifdef CONFIG_NPU_FFRT
	int (*npd_ffts_entry_is_clear)(void);
	int (*npd_ffts_clear_entry_by_taskidx)(uint16_t task_idx);
	int (*npd_proc_block_resource_clear)(u16 uid);
	int (*npd_ffrt_load_model_buf)(struct npu_ffrt_load_model_buf_info *model_desc);
	int (*npd_ffrt_set_ssid)(uint32_t model_id, uint16_t ssid);
#endif
};
