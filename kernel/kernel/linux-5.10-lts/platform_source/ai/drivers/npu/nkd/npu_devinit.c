...
static int npu_resource_list_init(struct npu_dev_ctx *dev_ctx)
{
	...
	ret = npu_id_allocator_create(id_allocator, NPU_NOTIFY_ID_START, NPU_NOTIFY_ID_CNT, 0);
...
static int npu_synchronous_resource_init(struct npu_dev_ctx *dev_ctx)
{
	...
	mutex_init(&dev_ctx->npu_iova_map_mutex);
...
#ifdef CONFIG_NPU_FFRT
	mutex_init(&dev_ctx->ffrt_entry_mutex);
#endif
#ifdef CONFIG_HIBERNATION
	mutex_init(&dev_ctx->npu_s4_mutex);
#endif
	// except manager
...
#if defined(CONFIG_NPU_FFRT) && defined(CONFIG_NPU_FFRT_V100) && !defined(CONFIG_NPU_SINK_UDK)
static void npu_register_ffts_proc(void)
{
	ffts_hw_proc_block_regist(npu_ffap1_status_hold, npu_plat_proc_block_resource_clear, FFTS_HW_NPU);
	ffts_ffap1_clear_entry_regist(npu_plat_ffts_clear_entry_by_taskidx, FFTS_HW_NPU);
}

static void npu_unregister_ffts_proc(void)
{
	ffts_hw_proc_block_unregist(FFTS_HW_NPU);
	ffts_ffap1_clear_entry_unregist(FFTS_HW_NPU);
}
#endif

static void npu_dev_ctx_init(struct npu_dev_ctx *dev_ctx, u8 dev_id)
{
	cond_return_void(dev_ctx == NULL, "dev_ctx is null\n");
	...
	const struct file_operations *npu_fops)
{
	...
	cond_return_error(dev_id >= NPU_DEV_NUM, -1, "illegal npu dev id = %hhu\n", dev_id);
	dev_ctx = kzalloc(sizeof(struct npu_dev_ctx), GFP_KERNEL);
	cond_return_error(dev_ctx == NULL, -ENOMEM, "kmalloc devid = %hhu dev_ctx failed\n", dev_id);

	npu_dev_ctx_init(dev_ctx, dev_id);

	if (plat_info->dts_info.feature_switch[NPU_FEATURE_HWTS] == 1)
		ret = npu_shm_v200_init(dev_id);
	else
		ret = npu_shm_init(dev_id);
	cond_goto_error(ret != 0, resource_list_init, ret, -ENODEV, "dev %hhu shm init failed\n", dev_id);
	...
	cond_goto_error(npu_synchronous_resource_init(dev_ctx), npu_synchronous_resource_failed, ret,
		-ENODEV, "npu_synchronous_resource_init fail\n");

#ifndef CONFIG_NPU_SWTS
	cond_goto_error(npu_heart_beat_resource_init(dev_ctx), npu_heart_beat_init_failed, ret,
...
#else
	ret = task_sched_init();
	cond_goto_error(ret != 0, task_sched_init_failed, ret, -ENODEV, "task_sched_init fail!\n");
#endif

	ret = npu_easc_init(dev_ctx);
	cond_goto_error(ret != 0, npu_easc_init_failed, ret, -ENODEV, "npu_easc_excp_int fail!\n");

#ifdef CONFIG_NPU_ECC
	ret = npu_ecc_init(dev_ctx);
	cond_goto_error(ret != 0, npu_ecc_init_failed, ret, -ENODEV, "npu_ecc_init fail!\n");
#endif
...
#ifdef CONFIG_NPU_SINK_UDK
	npu_ldk_register_callback();
#endif
#if defined(CONFIG_NPU_FFRT) && defined(CONFIG_NPU_FFRT_V100) && !defined(CONFIG_NPU_SINK_UDK)
	npu_register_ffts_proc();
#endif
#ifdef CONFIG_NPU_FFRT
