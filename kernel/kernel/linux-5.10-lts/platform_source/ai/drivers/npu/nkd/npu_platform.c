...
/* new platform need to consider ffrt/ispnn/nonsec_ge/nonsec_eff mode */
static int npu_s4_power_down(void)
{
	...
	// user process is frozen, sec mode cannot powerdown by self
	if (npu_bitmap_get(curr_work_mode, NPU_SEC) != 0 || cur_dev_ctx->pm.wm_cnt[NPU_SEC] != 0) {
		npu_drv_err("current device is busy in sec mode");
