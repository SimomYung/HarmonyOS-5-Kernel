...
u8 alloc_pm_bitmap_index(struct npu_dev_ctx *dev_ctx)
{
	mutex_lock(&dev_ctx->pm.proc_index_mutex);
	u8 index = find_first_zero_bit(&dev_ctx->pm.proc_index_bitmap, PROC_INDEX_NUM);
	dev_ctx->pm.proc_index_bitmap = npu_bitmap_set(dev_ctx->pm.proc_index_bitmap, index);
	npu_drv_info("alloc pm_bitmap_index %u, proc_index_bitmap = %u\n", index, dev_ctx->pm.proc_index_bitmap);
	mutex_unlock(&dev_ctx->pm.proc_index_mutex);
	return index;
}

static void free_pm_bitmap_index(struct npu_dev_ctx *dev_ctx, u8 index)
{
	mutex_lock(&dev_ctx->pm.proc_index_mutex);
	dev_ctx->pm.proc_index_bitmap = npu_bitmap_clear(dev_ctx->pm.proc_index_bitmap, index);
	npu_drv_info("free pm_bitmap_index %u, proc_index_bitmap = %u\n", index, dev_ctx->pm.proc_index_bitmap);
	mutex_unlock(&dev_ctx->pm.proc_index_mutex);
}
	...
	struct npu_dev_ctx *dev_ctx, u32 workmode)
{
	...
	if (ret != 0) {
		if (ret == 1) {
			...
			dev_ctx->pm.pm_bitmap = npu_bitmap_set(dev_ctx->pm.pm_bitmap, proc_ctx->pm_bitmap_index);
			npu_drv_info("after vote power up, pm_bitmap %u, pm_bitmap_index %u\n",
				dev_ctx->pm.pm_bitmap, proc_ctx->pm_bitmap_index);
		}
	...
	struct npu_dev_ctx *dev_ctx, u32 work_mode)
{
	...
	dev_ctx->pm.pm_bitmap = npu_bitmap_set(dev_ctx->pm.pm_bitmap, proc_ctx->pm_bitmap_index);
	npu_drv_warn("after set pm_bitmap_index: %u, pm_bitmap: %u", proc_ctx->pm_bitmap_index, dev_ctx->pm.pm_bitmap);
	...
	struct npu_dev_ctx *dev_ctx)
{
	...
	if (dev_ctx->pm.power_stage == NPU_PM_DOWN) {
		npu_drv_warn("npu dev %u has already powerdown!\n", dev_ctx->devid);
		dev_ctx->pm.work_mode = 0;
		return 0;
	}

	cond_return_error(proc_ctx->pm_bitmap_index >= PROC_INDEX_NUM, 0, "pm_bitmap_index invalid: %u\n", proc_ctx->pm_bitmap_index);
	dev_ctx->pm.pm_bitmap = npu_bitmap_clear(dev_ctx->pm.pm_bitmap, proc_ctx->pm_bitmap_index);
	npu_drv_warn("pm_bitmap_index: %u, after clear pm_bitmap: %u", proc_ctx->pm_bitmap_index, dev_ctx->pm.pm_bitmap);
	if (dev_ctx->pm.pm_bitmap != 0) {
		return 0;
	}
	...
	if (ret != 0) {
		...
		npu_drv_warn("npu dev %u powerdown success!\n", dev_ctx->devid);
	}
	npu_proc_clear_sqcq_info(proc_ctx);
	...
	struct npu_dev_ctx *dev_ctx)
{
	...
	mutex_lock(&dev_ctx->pm.npu_power_mutex);
	ret = npu_powerdown(proc_ctx, dev_ctx);
	free_pm_bitmap_index(dev_ctx, proc_ctx->pm_bitmap_index);
...
int npu_pm_handle_exception(struct npu_dev_ctx *dev_ctx, struct npu_exception_record *er)
{
	int ret;
	struct npu_platform_info *plat_info = NULL;
	u32 work_mode = NPU_NONSEC;
	struct npu_proc_ctx *proc_ctx = NULL;
	struct npu_proc_ctx *next_proc_ctx = NULL;
	unsigned long flags;
	plat_info = npu_plat_get_info();
	cond_return_error(plat_info == NULL, -1, "get plat_ops faile\n");
	mutex_lock(&dev_ctx->pm.npu_power_mutex);
	if (dev_ctx->pm.power_stage == NPU_PM_DOWN) {
		npu_drv_warn("npu dev %u has already powerdown!\n", dev_ctx->devid);
		dev_ctx->pm.work_mode = 0;
	...
	cond_goto_error(npu_bitmap_get(dev_ctx->pm.work_mode, NPU_NONSEC) == 0, pm_handle_out, ret, 0,
		"sec workmode, no need to reset\n");
	npu_heart_beat_exit(dev_ctx);
#if defined (CONFIG_DPM_HWMON) && defined (CONFIG_NPU_DPM_ENABLED)
	npu_dpm_exit();
#endif
	...
	if (ret != 0) {
		npu_drv_err("plat_ops npu_power_down failed\n");
		/* bbox : npu power down falied */
		npu_rdr_exception_report(RDR_EXC_TYPE_NPU_POWERDOWN_FAIL, 0);
		mutex_unlock(&dev_ctx->pm.npu_power_mutex);
		return -1;
	} else {
		npu_drv_warn("npu dev %u powerdown success!\n", dev_ctx->devid);
	...
	if (ret != 0) {
		npu_drv_err("plat_ops npu_power_up failed\n");
		/* bbox : npu power up falied */
		npu_rdr_exception_report(RDR_EXC_TYPE_NPU_POWERUP_FAIL, 0);
		mutex_unlock(&dev_ctx->pm.npu_power_mutex);
		return ret;
	}
	spin_lock_irqsave(&dev_ctx->ts_spinlock, flags);
