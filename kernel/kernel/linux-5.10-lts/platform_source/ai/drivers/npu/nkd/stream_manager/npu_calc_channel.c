	...
	u32 sq_id, u32 cq_id)
{
	...
	if (stream_id < NPU_MAX_NON_SINK_STREAM_ID) {
		npu_drv_err("alloc sink stream_id from dev %u failed\n", dev_id);
		return NULL;
	}
	...
	if (plat_info->dts_info.feature_switch[NPU_FEATURE_HWTS] == 1) {
		...
		if (ret != 0) {
			npu_drv_err("alloc hwts_sq_mem from dev %u failed\n", dev_id);
...
static struct npu_stream_info *npu_alloc_non_sink_stream(u8 cur_dev_id, u32 strategy,
	u32 sq_id, u32 cq_id)
{
	int ret = 0;
	pid_t server_id = 0;
	...
	stream_id = (u32)npu_alloc_stream_id(cur_dev_id);

	cond_return_error(stream_id < 0 ||
		stream_id >= NPU_MAX_NON_SINK_STREAM_ID, NULL,
		"alloc stream_id from dev %u failed\n", cur_dev_id);
	...
	if (npu_get_server_pid(cur_dev_ctx, current->tgid, &server_id) != 0) {
		npu_drv_err("get server_id fail, process id  %u\n", current->tgid);
		server_id = 0;
	}
	stream_info->server_tid = server_id;

	// bind stream with sq_id
	cond_goto_error(npu_bind_stream_with_sq(cur_dev_id, stream_id, sq_id),
		bind_stream_with_sq_failed,
		ret, ret, "bind stream = %d with sq_id = %d from dev %u failed\n",
	...
	cond_goto_error(npu_bind_stream_with_cq(cur_dev_id, stream_id, cq_id),
		...
		ret, ret, "bind stream = %d with cq_id = %d from dev %u failed\n",
	...
	if (inform_ts == NPU_NEED_TO_INFORM) {
		...
		if (ret != 0) {
			...
			npu_drv_err("invalid param, cur_dev_id %u, cq_id %d", cur_dev_id, cq_id);
	...
	u32 cq_id)
{
	...
	stream_info = npu_alloc_non_sink_stream(cur_dev_id, strategy, sq_id, cq_id);
...
static int npu_free_sink_stream(u8 dev_id, u32 stream_id)
{
	...
	if (ret != 0) {
		npu_drv_err("free sink stream id:%d, dev:%u failed\n",
			stream_id, dev_id);
		return -1;
	}
	...
	if (plat_info->dts_info.feature_switch[NPU_FEATURE_HWTS] == 1) {
		stream_info = npu_calc_stream_info(dev_id, stream_id);
		if (stream_info == NULL) {
			npu_drv_err("calc stream info failed, stream_id:%d, dev:%u\n",
				stream_id, dev_id);
			return -1;
		}
	}
