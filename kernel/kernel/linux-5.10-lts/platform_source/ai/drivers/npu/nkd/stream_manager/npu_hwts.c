...
int npu_proc_alloc_proc_ctx_sub(struct npu_proc_ctx *proc_ctx)
{
	...
	if (cur_dev_ctx == NULL) {
		npu_drv_err("cur_dev_ctx of dev: %u is null\n", proc_ctx->devid);
		return -1;
	}
	cur_dev_ctx_sub = cur_dev_ctx->dev_ctx_sub;
	if (cur_dev_ctx_sub == NULL) {
		npu_drv_err("cur_dev_ctx_sub of dev: %u is null\n", proc_ctx->devid);
		return -1;
	}
	...
	u32 *hwts_cq_id)
{
	...
	if (proc_ctx_sub == NULL) {
		npu_drv_err("cur proc_ctx_sub of dev:%u is null\n", proc_ctx->devid);
		return -1;
	}
	...
	struct npu_proc_ctx *proc_ctx)
{
	...
	if (hwts_cq_info == NULL) {
		npu_drv_err("npu dev %u hwts_cq_info is null\n", proc_ctx->devid);
		return NULL;
	}
