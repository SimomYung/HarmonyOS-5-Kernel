...
int __attribute__((weak)) npu_ffts_clear_entry_by_taskidx(uint16_t task_idx)
{
	unused(task_idx);
	return 0;
}

int __attribute__((weak)) npu_proc_block_resource_clear(u16 uid)
{
	unused(uid);
	return 0;
}

int __attribute__((weak)) npu_ffrt_load_model_buf(struct npu_ffrt_load_model_buf_info *model_desc)
{
	unused(model_desc);
	return 0;
}

int __attribute__((weak)) npu_ffrt_set_ssid(uint32_t model_id, uint16_t ssid)
{
	unused(model_id);
	unused(ssid);
	return 0;
