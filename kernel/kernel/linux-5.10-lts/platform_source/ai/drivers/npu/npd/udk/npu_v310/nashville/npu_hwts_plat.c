...
{
	SOC_NPU_HWTS_ACSQ_TO_RTSQ_MAP0_UNION map0 = {0};
	u64 hwts_base = npu_hwts_get_base_addr();

	cond_return_error(hwts_base == 0ULL, MAX_UINT16_NUM, "hwts_base is NULL\n");

	if (channel_id >= HWTS_ACSQ_NUM) {
		npu_drv_err("channel_id: %u\n", channel_id);
		return MAX_UINT16_NUM;
	}
	map0.value = readl((const volatile void *)(uintptr_t)SOC_NPU_HWTS_ACSQ_TO_RTSQ_MAP0_ADDR(hwts_base, channel_id));
	npu_drv_warn("hwts_get_rtsq_id_by_acsq_id end, value:%d\n", map0.reg.acsq_to_rtsqid_map);
	return map0.reg.acsq_to_rtsqid_map;
}
...
int npu_hwts_query_aic_eff_pool_status(
	u8 index, u8 *reg_val)
{
	SOC_NPU_HWTS_HWTS_EFF_AIC_POOL_STATUS_UNION eff_aic_pool_status = {0};
	...
	eff_aic_pool_status.value = readl(
		(const volatile void *)(uintptr_t)SOC_NPU_HWTS_HWTS_EFF_AIC_POOL_STATUS_ADDR(hwts_base, 0));
	*reg_val = eff_aic_pool_status.reg.eff_aic_enabled_status;

	npu_drv_warn("HWTS_AIC_EFF_POOL_STATUS%u_NS = 0x%x\n", index, *reg_val);
...
int npu_hwts_query_sqe_type(u16 channel_id, u8 *reg_val)
{
	...
	cond_return_warn(acsq_id == UINT16_MAX, 0, "invalid acsq id\n");
...
int npu_hwts_query_aic_task_config(void)
{
	...
	aic_fsm0_dfx0.value = readl((const volatile void *)(uintptr_t)SOC_NPU_HWTS_HWTS_AIC_FSM0_DFX0_ADDR(hwts_base, 0));
	npu_drv_warn("HWTS_AIC_BLK_FSM_STATE = 0x%llx\n", aic_fsm0_dfx0.value);

	return aic_fsm0_dfx0.reg.aic_fsm0_state;
}
...
int npu_hwts_query_aiv_task_config(void)
{
	...
	aiv_fsm0_dfx0.value = readl((const volatile void *)(uintptr_t)SOC_NPU_HWTS_HWTS_AIV_FSM0_DFX0_ADDR(hwts_base, 0));
	npu_drv_warn("HWTS_AIV_BLK_FSM_STATE = 0x%llx\n", aiv_fsm0_dfx0.value);

	return aiv_fsm0_dfx0.reg.aiv_fsm0_state;
}

int npu_hwts_query_eff_aic_task_config(void)
{
	SOC_NPU_HWTS_HWTS_EFF_AIC_FSM0_DFX0_UNION eff_aic_fsm0_dfx0 = {0};
	u64 hwts_base = npu_hwts_get_base_addr();

	cond_return_error(hwts_base == 0, -EINVAL, "hwts_base is NULL\n");

	eff_aic_fsm0_dfx0.value = readl((const volatile void *)(uintptr_t)SOC_NPU_HWTS_HWTS_EFF_AIC_FSM0_DFX0_ADDR(hwts_base, 0));
	npu_drv_warn("HWTS_EFF_AIC_BLK_FSM_STATE = 0x%llx\n", eff_aic_fsm0_dfx0.value);
	return eff_aic_fsm0_dfx0.reg.eff_aic_fsm0_state;
}
...
static void npu_hwts_query_acsq_info(u64 hwts_base, struct sq_exception_info *sq_info)
{
...
int npu_hwts_polling_eff_aic_fsm_state(u64 timeout)
{
	...
	do {
		eff_aic_fsm0_dfx0.value = readl(
			(const volatile void *)(uintptr_t)SOC_NPU_HWTS_HWTS_EFF_AIC_FSM0_DFX0_ADDR(hwts_base, 0));
		if (eff_aic_fsm0_dfx0.reg.eff_aic_fsm0_state == 0x4 && // 4'h4: WAIT_EXCEPTION_HANDLE
			eff_aic_fsm0_dfx0.reg.eff_aic_fsm0_rd_ost_cnt == 0 &&
			eff_aic_fsm0_dfx0.reg.eff_aic_fsm0_pmu_ost_cnt == 0 &&
			eff_aic_fsm0_dfx0.reg.eff_aic_fsm0_log_ost_cnt == 0 &&
			eff_aic_fsm0_dfx0.reg.eff_aic_fsm0_cfg_ost_cnt == 0)
			return 0;
...
static int npu_hwts_eff_aic_fsm_poll_proc(u16 acsq_id, u32 bitmap, u64 timeout)
{
	uint32_t index = 0;
	u64 timeout_tmp;
	SOC_NPU_HWTS_HWTS_EFF_AIC_FSM0_DFX0_UNION hwts_eff_aic_fsm0_dfx0 = {0};
	u64 hwts_base = npu_hwts_get_base_addr();
	cond_return_error(hwts_base == 0, -1, "hwts_base is NULL\n");
 
	for (; index < EFF_AIC_NUM; index++) {
		if ((bitmap & (0x1 << index)) == 0)
			continue;
		timeout_tmp = timeout;
		do {
			hwts_eff_aic_fsm0_dfx0.value = readl(
				(const volatile void *)(uintptr_t)SOC_NPU_HWTS_HWTS_EFF_AIC_FSM0_DFX0_ADDR(hwts_base, index));
			if ((hwts_eff_aic_fsm0_dfx0.reg.eff_aic_fsm0_rd_ost_cnt == 0) &&
				(hwts_eff_aic_fsm0_dfx0.reg.eff_aic_fsm0_pmu_ost_cnt == 0) &&
				(hwts_eff_aic_fsm0_dfx0.reg.eff_aic_fsm0_log_ost_cnt == 0) &&
				(hwts_eff_aic_fsm0_dfx0.reg.eff_aic_fsm0_cfg_ost_cnt == 0)) {
				npu_drv_warn("eff aic suspend, acsq_id = %u core_id = %u state = %x",
					acsq_id, index, hwts_eff_aic_fsm0_dfx0.value);
				break;
			}
			usleep_range(1, 1);
			timeout_tmp--;
...
int npu_hwts_polling_acsq_ip_own_state(u16 acsq_id, u64 timeout, uint32_t *need_reset)
{
	...
	switch(acsq_fsm_dfx_ctrl0.reg.sqe_type) {
		...
		case NPU_HWTS_SQE_AICORE_EFF:
			ret = npu_hwts_eff_aic_fsm_poll_proc(acsq_id, acsq_fsm_ip_own_state.reg.ip_own_bitmap, timeout);
			if (ret != 0) {
				npu_drv_err("aiv fsm polling timeout");
				return -1;
			}
