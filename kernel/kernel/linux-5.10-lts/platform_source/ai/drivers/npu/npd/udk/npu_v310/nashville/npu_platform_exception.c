...
#define MAX_ERROR_PC  0x40000
#define ERROR_PC_MASK 0x3FFFC
...
static void npu_exception_bbox_dump_aicore(u8 aic_id,
	...
	struct aicore_exception_error_info *aic_error_info)
{
	...
	/* valid when mte bus error */
	if ((aic_info->aic_error1 & 0xFFFFFFFFULL) != 0) {
		if (aic_error_info->mte_error_pc < (u32)(aic_info->ifu_start & ERROR_PC_MASK)) {
			error_pc_offset = aic_error_info->mte_error_pc + MAX_ERROR_PC - (u32)(aic_info->ifu_start & ERROR_PC_MASK);
		} else {
			error_pc_offset = aic_error_info->mte_error_pc - (u32)(aic_info->ifu_start & ERROR_PC_MASK);
		}
		(void)snprintf_s(log_buf, sizeof(log_buf), sizeof(log_buf) - 1,
			"mte error pc=0x%lx\n", error_pc_offset);
		(void)npu_mntn_copy_reg_to_bbox(log_buf, (unsigned int)strlen(log_buf));
	...
	/* valid when su illegal instr */
	if ((aic_info->aic_error & 0xFFFFFFFF00000000ULL) != 0) {
		if (aic_error_info->su_error_pc < (u32)(aic_info->ifu_start & ERROR_PC_MASK)) {
			error_pc_offset = aic_error_info->su_error_pc + MAX_ERROR_PC - (u32)(aic_info->ifu_start & ERROR_PC_MASK);
		} else {
			error_pc_offset = aic_error_info->su_error_pc - (u32)(aic_info->ifu_start & ERROR_PC_MASK);
		}
		(void)snprintf_s(log_buf, sizeof(log_buf), sizeof(log_buf) - 1,
			"su error pc=0x%lx, illegal instr=0x%lx\n", error_pc_offset, aic_error_info->illegal_instr);
		(void)npu_mntn_copy_reg_to_bbox(log_buf, (unsigned int)strlen(log_buf));
	}
}
...
static void npu_exception_query_aicore_info(u16 channel_id, u32 is_timeout)
{
	...
	npu_exception_bbox_dump_aicore(core_id, &aicore_info, &aicore_dbg_info, &aicore_error_info);
}
...
int npu_timeout_exception_dump(void)
{
	...
	uint64_t nonsec_bitmap = NPU_HWTS_RTSQ_LITE_UNSEC_BITMAP | NPU_HWTS_RTSQ_SMARTISP_BITMAP |
