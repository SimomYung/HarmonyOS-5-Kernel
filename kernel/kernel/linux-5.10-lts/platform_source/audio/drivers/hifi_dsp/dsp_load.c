...
static int create_dsp_load_thread(void)
{
	...
	if (IS_ERR_OR_NULL(dsp_load_khandler_task)) {
		loge("create dsp load handler thread fail\n");
		return -EBUSY;
	}

	logi("create dsp load handler thread succ\n");
...
void dsp_load_init(unsigned char *unsec_virt_addr)
{
	...
	if (strstr(saved_command_line, "recovery_update=1")) {
		logi("recovery_update, do not load hifi\n");
		return;
	}
