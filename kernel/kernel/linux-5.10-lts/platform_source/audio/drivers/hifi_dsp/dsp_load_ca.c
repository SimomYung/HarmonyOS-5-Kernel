...
static int dsp_read_bin(unsigned int offset, unsigned int length, char *buffer)
{
	...
	__flush_dcache_area(buffer, length);
...
static int load_dsp_image(void)
{
	int ret;
...
{
	int ret;
	unsigned int load_times = 0;

	while (load_times < DSP_LOAD_TIMES) {
		ret = load_dsp_image();
		if (ret == 0)
			break;

		load_times++;
		loge("load dsp image failed %u times, 0x%x\n", load_times, ret);
	}
