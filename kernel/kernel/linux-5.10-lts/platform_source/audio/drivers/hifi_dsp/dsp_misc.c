...
#define UEVENT_STRING_LENGTH (32)
...
#endif
static bool g_request_flag[SYSCACHE_MODE_CNT] = { false, false, false };
...
static unsigned int get_syscache_pid(unsigned int mode)
{
	if (mode == SYSCACHE_AUDIO_BUFFER_MODE)
		return PID_AUDIO;

	if (mode == SYSCACHE_AUDIO_CACHE_MODE)
		return PID_HIFI;
...
static void send_audio_syscache_quota_ack(unsigned int mode, unsigned int msg_type)
{
	uint32_t ret = 0;
	struct syscache_quota_ack_msg msg = { 0 };

	if ((mode != SYSCACHE_AUDIO_CACHE_MODE) && (mode != SYSCACHE_VOICE_CACHE_MODE))
		return;
...
static void set_audio_syscache_quota(unsigned int msg_type, unsigned int mode)
{
	int ret;
	unsigned int pid;

	logi("syscache quota msg info type %u mode %u\n",
		msg_type, mode);

	pid = get_syscache_pid(mode);

	if (msg_type == SYSCACHE_QUOTA_REQUEST) {
		ret = dsp_request_quota(pid);
		if (ret != 0) {
			loge("request syscache fail ret %d pid %u\n", ret, pid);
			return;
		}
		g_request_flag[mode] = true;
	} else {
		ret = dsp_release_quota(pid);
		if (ret != 0) {
		...
		g_request_flag[mode] = false;
	}

	send_audio_syscache_quota_ack(mode, msg_type);
	logi("set syscache quota success\n");
}


static void audio_syscache_quota(enum socdsp_om_work_id work_id,
	const unsigned char *data, unsigned int len)
{
	...
	if (msg->mode >= SYSCACHE_MODE_CNT ||
		msg->msg_type > SYSCACHE_QUOTA_REQUEST) {
		loge("msg info error, type %u, mode %u\n",
			msg->msg_type, msg->mode);
		return;
	}

	set_audio_syscache_quota(msg->msg_type, msg->mode);
}
...
static void dsp_device_change_send_event(struct work_struct* work)
{
	int ret;
	unsigned int i;
	unsigned int type;
	char uevent_string[UEVENT_STRING_LENGTH] = {0};
	char *envp[2] = { uevent_string, NULL };
	struct audio_nn_work* data = NULL;
	if (work == NULL) {
		loge("work queue is null\n");
		return;
	}
	data = container_of(work, struct audio_nn_work, work);
	type = data->msg_type;
	for (i = 0; i < MAX_MODEL_TYPE; ++i) {
		if (i == type) {
			snprintf_s(uevent_string, sizeof(uevent_string), sizeof(uevent_string) - 1, "%s", VOICE_NN_MODEL[i]);
			logi("model type = %s\n", uevent_string);
		}
	}
	if (uevent_string[0] == '\0') {
		logi("not devcie = %u model\n", type);
		kfree(data);
		data = NULL;
		return;
	}
	...
	kfree(data);
	data = NULL;
	logi("uvent send succ\n");
	return;
}

void rcv_dsp_nn_model_change_msg(enum socdsp_om_work_id work_id, const uint8_t *data, uint32_t len)
{
	struct audio_nn_work* work_data;
	struct nn_model_change_recive_type* msg_rcv_type;
	uint32_t msg_type;
	if ((data == NULL) || (len < sizeof(struct nn_model_change_recive_type))) {
		loge("audio nn data is null\n");
		return;
	}
	msg_rcv_type = (struct nn_model_change_recive_type *)data;
	msg_type = msg_rcv_type->type;
	logi("type = %u", msg_type);
	work_data = kmalloc(sizeof(struct audio_nn_work), GFP_ATOMIC);
	if (!work_data) {
		loge("kmalloc audio nn work failed\n");
		return;
	}

	work_data->msg_type = msg_type;

	INIT_WORK(&(work_data->work), dsp_device_change_send_event);
...
#endif
	{ ID_HIFI_AP_SYSCACHE_QUOTA_CMD, 0, audio_syscache_quota },
#ifdef CONFIG_HIFI_NPU_ROUTE
	{ ID_DSP_AP_NN_ROUTE_WRITE_COMPLETE, 0, rcv_nn_dsp_route_msg },
#endif
#ifdef CONFIG_HIFI_AUDIO_NN_SUPPORT
	{ ID_DSP_AP_NN_ROUTE_VOICE_MODULE_CHANGE, 0, rcv_dsp_nn_model_change_msg },
#endif
#ifdef HIFI_BLOW_RES_UPDATE_CDEV
	{ ID_DSP_AP_LPC_BLOW_COGNITION_RES_CMD, 0, rcv_dsp_blow_cognition_res_msg },
#endif
};
...
#ifdef CONFIG_PM
static void get_socdsp_service_status(void)
{
	unsigned int active_status;
	unsigned int wlp_status;

	if (g_misc_data.base_virt == NULL) {
		logw("base virt is null\n");
		return;
	}

	active_status = readl(g_misc_data.base_virt +
		(DRV_DSP_SERVICE_ACTIVE_ADDR - DSP_UNSEC_BASE_ADDR));

	wlp_status = readl(g_misc_data.base_virt +
		(DRV_DSP_WLP_STATUS_ADDR - DSP_UNSEC_BASE_ADDR));

	logi("socdsp active status: 0x%x, wlp status: 0x%x\n", active_status, wlp_status);
}
...
#ifdef CONFIG_HIFI_NPU_ROUTE
	init_nn_dsp_route(g_misc_data.base_virt);
#endif
...
#ifdef HIFI_BLOW_RES_UPDATE_CDEV
	ret = blow_res_update_init();
	/* for other function with "return ERROR"，need to add blow_res_update_deinit() */
	if (ret != 0)
		pr_err("blow res update init fail\n");
#endif
	return 0;
