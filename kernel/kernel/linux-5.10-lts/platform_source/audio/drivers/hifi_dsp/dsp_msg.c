...
static const struct misc_msg_info g_msg_info[] = {
	...
	{ ID_AP_DSP_LPC_BLOW_COGNITION_UPDATE_CMD, "ID_AP_DSP_LPC_BLOW_COGNITION_UPDATE_CMD"},
#endif
};
...
static int get_dp_info(struct dp_edid_info *edid_info)
{
	...
	ret = dpu_dptx_get_spec(edid_info->spec, sizeof(edid_info->spec),
		&edid_info->ext_acount, (int)edid_info->port_id);
	if (ret) {
...
static int set_dp_info(const struct dp_edid_info *edid_info)
{
	...
	ret = dpu_dptx_set_aparam(edid_info->aparam.channel_num,
		edid_info->aparam.data_width, edid_info->aparam.sample_rate, (int)edid_info->port_id);
	if (ret) {
