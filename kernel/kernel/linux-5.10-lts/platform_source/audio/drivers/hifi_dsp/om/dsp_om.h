...
do { \
	if (om_get_print_level() >= 3) { \
		printk(LOG_TAG" audio[D][%u]:%s:%d: "fmt, DSP_STAMP, __func__, __LINE__, ##__VA_ARGS__); \
	} \
} while (0)
...
do { \
	if (om_get_print_level() >= 2) { \
		printk(LOG_TAG" audio[I][%u]:%s:%d: "fmt, DSP_STAMP, __func__, __LINE__, ##__VA_ARGS__); \
	} \
} while (0)
...
do { \
	if (om_get_print_level() >= 1) { \
		printk(LOG_TAG" audio[W][%u]:%s:%d: "fmt, DSP_STAMP, __func__, __LINE__, ##__VA_ARGS__); \
	} \
} while (0)

#define loge(fmt, ...) \
		printk(LOG_TAG" audio[E][%u]:%s:%d: "fmt, DSP_STAMP, __func__, __LINE__, ##__VA_ARGS__)
/*lint +e683 */
