...
#define HIFI_MUSIC_DATA_DUMP_PATH "data/music_data.bin"
#define SOCDSP_OCRAM_MAD_IF1_ADDR (SOCDSP_OCRAM_BASE_ADDR + 0x10000)
#define MAD_32K_BUFFER_SIZE       0x8000
#define DSP_TIME_LEN 18

#define SOCDSP_DMSG_CLEAR 1
#define SOCDSP_DMSG_WITHOUTSORT 2
...
extern void vdec_betaclub_dump_mculog(void);
...
static DEFINE_MUTEX(g_dsp_time_info_lock);
static char g_dsp_time_info[STAMP_POS_BUTT][DSP_TIME_LEN + 1] = {0};


static int get_dsp_time_info(int pos, char *str, size_t str_len) {
	size_t dsp_time_len;

	if (pos < 0 || pos >= STAMP_POS_BUTT) {
		loge("Invalid argument, pos is %d\n", pos);
		return -EINVAL;
	}

	if (str == NULL) {
		loge("Invalid argument, str is NULL\n");
		return -EINVAL;
	}

	dsp_time_len = strlen(g_dsp_time_info[pos]);
	if (str_len == 0 || dsp_time_len > DSP_TIME_LEN || str_len <= dsp_time_len) {
		loge("Invalid argument, str_len is %zu, dsp_time_len is %zu\n", str_len, dsp_time_len);
		return -EINVAL;
	}

	mutex_lock(&g_dsp_time_info_lock);
	if (strncpy_s(str, str_len, g_dsp_time_info[pos], dsp_time_len) != 0) {
		loge("strncpy_s failed!\n");
		mutex_unlock(&g_dsp_time_info_lock);
		return -EINVAL;
	}
	str[dsp_time_len] = '\0';
	mutex_unlock(&g_dsp_time_info_lock);
	logi("get_dsp_time_info str is : %s\n", str);

	return 0;
}

int get_dsp_time_info_str(char *str, size_t str_len)
{
	int i = 0;
	int ret = 0;
	char dsp_time[DSP_TIME_LEN] = {0};
	char single_time_info[SINGLE_DSP_TIME_INFO_STR_LEN] = {0};
	...
	if (str_len == 0 || str_len > MAX_DSP_TIME_INFO_STR_LEN) {
		loge("Invalid argument, str len is %zu", str_len);
		return -EINVAL;
	}

	for (i = 0; i < STAMP_POS_BUTT; i++) {
		get_dsp_time_info(i, dsp_time, DSP_TIME_LEN);
		ret = sprintf_s(single_time_info, SINGLE_DSP_TIME_INFO_STR_LEN,
			"time_stamp_pos:%d,dsp_time:%s,", i, dsp_time);
		if (ret <= 0) {
			loge("time info str: %s too long\n", dsp_time);
			return ret;
		}

		ret = strncat_s(str, str_len, single_time_info, strlen(single_time_info));
		if (ret) {
			loge("strncat_s failed");
			return ret;
		}
		(void)memset_s(dsp_time, DSP_TIME_LEN, 0, DSP_TIME_LEN);
		(void)memset_s(single_time_info, SINGLE_DSP_TIME_INFO_STR_LEN, 0, SINGLE_DSP_TIME_INFO_STR_LEN);
	}

	logi("dsp time info str is : %s\n", str);
	return 0;
}
...
void __attribute__((weak)) vdec_betaclub_dump_mculog(void)
{
}
...
static void set_om_data(const unsigned char *unsec_virt_addr)
{
	...
	g_priv_debug.dsp_log_len_addr = (unsigned int *)(unsec_virt_addr +
		(DRV_DSP_UART_TO_MEM_LEN_ADDR - DSP_UNSEC_BASE_ADDR));
	g_priv_debug.dsp_log_addr = (char *)(unsec_virt_addr +
		(DRV_DSP_UART_TO_MEM - DSP_UNSEC_BASE_ADDR));
	(void)memset_s(g_priv_debug.dsp_log_addr, DRV_DSP_UART_TO_MEM_SIZE, 0, DRV_DSP_UART_TO_MEM_SIZE);
...
int om_debug_init(unsigned char *unsec_virt_addr)
{
	...
	logi("om debug init\n");
	(void)memset_s(&g_priv_debug, sizeof(g_priv_debug), 0, sizeof(g_priv_debug));
