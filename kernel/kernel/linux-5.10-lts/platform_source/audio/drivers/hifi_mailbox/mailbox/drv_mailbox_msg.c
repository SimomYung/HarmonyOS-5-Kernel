...
#define MB_FULL_MAX_CONTINUOUS_TIMES 10
#endif
	...
	uint32_t length)
{
	...
	int try_times = 0;
#ifdef CONFIG_HIFI_BB
	static uint32_t mb_full_continuous_times = 0;
#endif
...
#ifdef CONFIG_HIFI_BB
	if (ret == MAILBOX_FULL) {
		mb_full_continuous_times++;
		if (mb_full_continuous_times == MB_FULL_MAX_CONTINUOUS_TIMES && !is_dsp_resetting()) {
			AUDIO_LOGE("mailbox full, dump dsp log");
			socdsp_dump_panic_log();
		}
	}

	if (ret == 0)
		mb_full_continuous_times = 0;
#endif
