...
#ifdef CONFIG_HISI_MODEM_OHOS
static unsigned int dsp_reset_poll(struct file *filp, poll_table *wait)
{
	struct dsp_reset_node_info *node = NULL;
	unsigned int mask = 0;
	...
	AUDIO_LOGE("dsp reset wait before");
	poll_wait(filp, &node->wait, wait);
	AUDIO_LOGE("dsp reset wait after");

	if (node->flag == NOTIFY_NODEM_BEGIN) {
		mask = POLLIN | POLLRDNORM;
		AUDIO_LOGI("dsp reset read ready");
	}
