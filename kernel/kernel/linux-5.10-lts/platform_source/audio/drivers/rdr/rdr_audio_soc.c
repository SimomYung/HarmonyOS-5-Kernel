...
#endif
#include "asp_codec_gt_proxy_common.h"
#include "audio_log.h"
#include "dsp_misc.h"
#include "dsp_om.h"
...
#define INTR_ASP_HTS_MST_ERR BIT(3)
#define INTR_ASP_HTS_SLV_ERR BIT(4)
...
void trigger_dsp_reset(void)
{
	g_soc_des.dsp_resetting = true;
...
bool is_dsp_resetting(void)
{
	return g_soc_des.dsp_resetting;
}
...
static void mask_asp_hts_mst_irq(void)
{
	writel(INTR_ASP_HTS_MST_ERR, (g_soc_des.intr_hub_ao_addr + INTR_HUB_AO_MASK_CLEAR));
}

static void mask_asp_hts_slv_irq(void)
{
	writel(INTR_ASP_HTS_SLV_ERR, (g_soc_des.intr_hub_ao_addr + INTR_HUB_AO_MASK_CLEAR));
}
...
static irqreturn_t asp_hts_mst_err_irq_handler(int irq, void *data)
{
	AUDIO_LOGI("asp hts mst err irq arrival");

	mask_asp_hts_mst_irq();

	return IRQ_HANDLED;
}

static irqreturn_t asp_hts_slv_err_irq_handler(int irq, void *data)
{
	AUDIO_LOGI("asp hts slv err irq arrival");

	mask_asp_hts_slv_irq();
...
static irqreturn_t soc_irq_handler(int irq, void *data)
{
	...
	if (int_state & INTR_ASP_WATCHDOG)
		return soc_wtd_irq_handler(irq, data);
	else if (int_state & INTR_ASP_DWAXI_DLOCK)
	...
	else if (int_state & INTR_ASP_HTS_MST_ERR)
		return asp_hts_mst_err_irq_handler(irq, data);
	else if (int_state & INTR_ASP_HTS_SLV_ERR)
		return asp_hts_slv_err_irq_handler(irq, data);

	AUDIO_LOGE("rdr: unrecognition irq, socdsp irq state = 0x%x", int_state);
	return IRQ_NONE;
#else
	return soc_wtd_irq_handler(irq, data);
#endif
...
static int create_rdr_soc_thread(void)
{
	...
	if (IS_ERR_OR_NULL(g_soc_des.kdump_task)) {
	...
	if (IS_ERR_OR_NULL(g_soc_des.khandler_task)) {
