...
int soc_mad_request_pinctrl_state(unsigned int mode)
{
	...
	AUDIO_LOGI("soc_mad_request_pinctrl_state mad_mode: %lu", mad_data.pinctrl_state);

	int ret = select_mode();
	if (ret) {
		AUDIO_LOGE("soc_mad_request_pinctrl_state, select_mode error, ret: %d", ret);
		return ret;
	}
...
int soc_mad_release_pinctrl_state(unsigned int mode)
{
	...
	AUDIO_LOGI("soc_mad_release_pinctrl_state mad_mode: %lu", mad_data.pinctrl_state);

	int ret = select_mode();
	if (ret) {
		AUDIO_LOGE("soc_mad_release_pinctrl_state, select_mode error, ret: %d", ret);
		return ret;
	}
