...
int soc_mad_request_pinctrl_state(unsigned int mode)
{
	...
	AUDIO_LOGI("soc mad request gpio, vote mode(high-0/low-1): %lu, vote status: %lu", mode, mad_data.pinctrl_state);

	int ret = select_mode();
	if (ret) {
		AUDIO_LOGE("soc mad request gpio vote, select_mode error, ret: %d", ret);
		return ret;
	}

#ifdef CONFIG_MAD_EXTENSION
	ret = mad_ex_request_pinctrl_state(mode);
#endif
	return ret;
}
...
int soc_mad_release_pinctrl_state(unsigned int mode)
{
	...
	AUDIO_LOGI("soc mad release gpio, vote mode(high-0/low-1):%lu, vote status: %lu", mode, mad_data.pinctrl_state);

	int ret = select_mode();
	if (ret) {
		AUDIO_LOGE("soc mad release gpio vote, select_mode error, ret: %d", ret);
		return ret;
	}

#ifdef CONFIG_MAD_EXTENSION
	ret = mad_ex_release_pinctrl_state(mode);
#endif
	return ret;
}
...
void soc_single_path_mad_with_dmic_select_din(unsigned int mode)
{
	unsigned int i;
	for (i = MAD_REG; i < REG_TYPE_NUM; i++) {
		if (!mad_data.reg_base_addr[i]) {
	...
	if (mode == LOW_FREQ_MODE)
		...
		writel(0xA, mad_data.reg_base_addr[MAD_REG] + MAD_MUX_SEL);
	else
		AUDIO_LOGE("invalid mode: %d", mode);
}

void soc_single_path_mad_with_dmic_hpf_config(unsigned int mode, unsigned int hpf_cfg)
{
	unsigned int i;
	for (i = MAD_REG; i < REG_TYPE_NUM; i++) {
		if (!mad_data.reg_base_addr[i]) {
	...
	if (hpf_cfg == HPF_ENABLE) {
		if (mode == HIGH_FREQ_MODE) {
			writel(0x800, mad_data.reg_base_addr[MAD_REG] + ADC_CTRL);
			return;
		}
		writel(0x0, mad_data.reg_base_addr[MAD_REG] + HPF1_768K_CTRL);
	} else {
		if (mode == HIGH_FREQ_MODE) {
			writel(0x80F, mad_data.reg_base_addr[MAD_REG] + ADC_CTRL);
			return;
		}
		writel(0x1, mad_data.reg_base_addr[MAD_REG] + HPF1_768K_CTRL);
	}
