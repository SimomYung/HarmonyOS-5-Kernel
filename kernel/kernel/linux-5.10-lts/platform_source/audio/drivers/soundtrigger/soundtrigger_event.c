...
void hw_soundtrigger_event_uevent(uint32_t soundtrigger_event, int vendor)
{
	...
	case AUDIO_LP_SOUNDTRIGGER_TRIGGER:
		(void)snprintf_s(envp_ext0, ENVP_LENTH, (unsigned int)(ENVP_LENTH - 1), "hotword_detected_lp=%d", vendor);
		break;
	case AUDIO_NORMAL_SOUNDTRIGGER_TRIGGER:
		(void)snprintf_s(envp_ext0, ENVP_LENTH, (unsigned int)(ENVP_LENTH - 1), "hotword_detected_normal=%d", vendor);
		break;
	default:
		(void)snprintf_s(envp_ext0, ENVP_LENTH, (unsigned int)(ENVP_LENTH - 1), "soundtrigger=%u", soundtrigger_event);
		break;
	}

	AUDIO_LOGI("soundtrigger_uevent %s\n", envp_ext0);
...
MODULE_DEVICE_TABLE(of, codec_vad_of_match);
