...
static void ring_buffer_reset(struct ring_buffer_itef *itef)
{
	struct ring_buffer *rb = (struct ring_buffer *)itef;
	rb->start = 0;
	rb->count = 0;
	(void)memset_s(rb->buffer, rb->element_size * rb->size, 0, rb->element_size * rb->size);
}
...
struct ring_buffer_itef *st_ring_buffer_init(uint32_t element_size, uint32_t element_cnt)
{
	...
	rb->itef.reset = ring_buffer_reset;
