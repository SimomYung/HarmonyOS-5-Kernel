...
static uint32_t g_socdsp_vendor[WAKEUP_MODE_BUTT];

static void soundtrigger_lp_hotword_detected_work(struct work_struct *wk)
{
	AUDIO_LOGI("soundtrigger_lp_hotword_detected vendor:%u\n", g_socdsp_vendor[LP_WAKEUP_MODE]);
	hw_soundtrigger_event_uevent(AUDIO_LP_SOUNDTRIGGER_TRIGGER, g_socdsp_vendor[LP_WAKEUP_MODE]);
}

static void soundtrigger_set_mute_work(struct work_struct *wk)
{
	hw_soundtrigger_event_uevent(AUDIO_SOUNDTRIGGER_SETMUTE, VENDOR_NULL);
}

static void soundtrigger_elapsed_timeout_work(struct work_struct *wk)
{
	hw_soundtrigger_event_uevent(AUDIO_SOUNDTRIGGER_TIMEOUT, VENDOR_NULL);
}

static void soundtrigger_normal_hotword_detected_work(struct work_struct *wk)
{
	AUDIO_LOGI("soundtrigger_normal_hotword_detected vendor:%u\n", g_socdsp_vendor[NORMAL_WAKEUP_MODE]);
	hw_soundtrigger_event_uevent(AUDIO_NORMAL_SOUNDTRIGGER_TRIGGER, g_socdsp_vendor[NORMAL_WAKEUP_MODE]);
}

static void soundtrigger_wakeup_elapsed_timeout_work(struct work_struct *wk)
{
	hw_soundtrigger_event_uevent(AUDIO_SOUNDTRIGGER_NORMAL_TIMEOUT, VENDOR_NULL);
}
...
static void wakeup_hotword_ack_proc(struct soundtrigger_rcv_msg *rcv_msg, struct completion *msg_complete)
{
	...
	g_socdsp_vendor[NORMAL_WAKEUP_MODE] = rcv_msg->vendor;
...
static void wakeup_lp_hotword_ack_proc(struct soundtrigger_rcv_msg *rcv_msg, struct completion *msg_complete)
{
	...
	g_socdsp_vendor[LP_WAKEUP_MODE] = rcv_msg->vendor;
...
int parameter_set_msg(uint16_t msg_type, int module_id, const struct parameter_set *set_val, uint32_t para_size)
{
	...
	AUDIO_LOGI("parameter_set_msg, type:%u, module_id:%d, key:%d", msg_type, module_id, set_msg.para.key);
...
int soundtrigger_mailbox_init(void)
{
	...
	AUDIO_LOGI("soundtrigger mailbox init in");
