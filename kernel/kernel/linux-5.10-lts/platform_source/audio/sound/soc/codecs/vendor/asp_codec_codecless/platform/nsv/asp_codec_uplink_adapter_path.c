...
#define UP_PGA_GAIN_KCONTROLS \
	SOC_SINGLE("MIC7_UP_PGA_GAIN", \
		MIC7_UP_PGA_CTRL_REG, MIC7_UP_PGA_GAIN_OFFSET, \
		max_val_on_bit(MIC7_UP_PGA_GAIN_LEN), 0), \
	SOC_SINGLE("MIC7_UP_PGA_BYPASS", \
		MIC7_UP_PGA_CTRL_REG, MIC7_UP_PGA_BYPASS_OFFSET, \
		max_val_on_bit(MIC7_UP_PGA_BYPASS_LEN), 0), \
	SOC_SINGLE("MIC8_UP_PGA_GAIN", \
		MIC8_UP_PGA_CTRL_REG, MIC8_UP_PGA_GAIN_OFFSET, \
		max_val_on_bit(MIC8_UP_PGA_GAIN_LEN), 0), \
	SOC_SINGLE("MIC8_UP_PGA_BYPASS", \
		MIC8_UP_PGA_CTRL_REG, MIC8_UP_PGA_BYPASS_OFFSET, \
		max_val_on_bit(MIC8_UP_PGA_BYPASS_LEN), 0), \
	SOC_SINGLE("ULTR_ADC_CIC_GAIN", \
		ULTR_ADC_CTRL0_REG, ULTR_ADC_CIC_GAIN_OFFSET, \
		max_val_on_bit(ULTR_ADC_CIC_GAIN_LEN), 0) \
...
#define UP_SRC_KCONTROLS \
	...
	SOC_SINGLE("MIC7_UP_SRC_CLKEN", \
		CODEC_CLK_EN1_REG, MIC7_UP_SRCDN_CLKEN_OFFSET, \
		max_val_on_bit(MIC7_UP_SRCDN_CLKEN_LEN), 0), \
	SOC_SINGLE("MIC7_UP_SRC_DOUT", \
		FS_CTRL5_REG, FS_MIC7_SRCDN_DOUT_OFFSET, \
		max_val_on_bit(FS_MIC7_SRCDN_DOUT_LEN), 0), \
	SOC_SINGLE("MIC7_UP_SRC_MODE", \
		SRCDN_CTRL1_REG, MIC7_UP_SRCDN_SRC_MODE_OFFSET, \
		max_val_on_bit(MIC7_UP_SRCDN_SRC_MODE_LEN), 0), \
	SOC_SINGLE("MIC8_UP_SRC_CLKEN", \
		CODEC_CLK_EN1_REG, MIC8_UP_SRCDN_CLKEN_OFFSET, \
		max_val_on_bit(MIC8_UP_SRCDN_CLKEN_LEN), 0), \
	SOC_SINGLE("MIC8_UP_SRC_DOUT", \
		FS_CTRL5_REG, FS_MIC8_SRCDN_DOUT_OFFSET, \
		max_val_on_bit(FS_MIC8_SRCDN_DOUT_LEN), 0), \
	SOC_SINGLE("MIC8_UP_SRC_MODE", \
		SRCDN_CTRL1_REG, MIC8_UP_SRCDN_SRC_MODE_OFFSET, \
		max_val_on_bit(MIC8_UP_SRCDN_SRC_MODE_LEN), 0) \

static const struct snd_kcontrol_new uplink_adapter_snd_kcontrols[] = {
	UP_PGA_GAIN_KCONTROLS,
	UP_SRC_KCONTROLS,
};
...
static const char * const mic7_mux_texts[] = {
	"MIC1_ADC",
	"MIC3_ADC",
	"MIC5_ADC",
	"MIC7_ADC",
...
static const struct soc_enum mic7_mux_enum =
	SOC_ENUM_SINGLE(SIF_CTRL_REG, MIC7_DIN_SEL_OFFSET,
		ARRAY_SIZE(mic7_mux_texts), mic7_mux_texts);

static const struct snd_kcontrol_new dapm_mic7_mux_controls =
	SOC_DAPM_ENUM("Mux", mic7_mux_enum);

static const char * const mic8_mux_texts[] = {
	"MIC2_ADC",
	"MIC4_ADC",
	"MIC6_ADC",
	"MIC8_ADC",
...
static const struct soc_enum mic8_mux_enum =
	SOC_ENUM_SINGLE(SIF_CTRL_REG, MIC8_DIN_SEL_OFFSET,
		ARRAY_SIZE(mic8_mux_texts), mic8_mux_texts);

static const struct snd_kcontrol_new dapm_mic8_mux_controls =
	SOC_DAPM_ENUM("Mux", mic8_mux_enum);
...
static const struct soc_enum ultr_up_mux_enum =
	SOC_ENUM_SINGLE(CODEC_DIN_MUX1_REG, ADC_ULTR_CIC_DIN_SEL_OFFSET,
...
#define UP_MUX_WIDGET \
	...
	SND_SOC_DAPM_MUX("MIC7_MUX", \
		SND_SOC_NOPM, 0, 0, &dapm_mic7_mux_controls), \
	SND_SOC_DAPM_MUX("MIC8_MUX", \
		SND_SOC_NOPM, 0, 0, &dapm_mic8_mux_controls), \
	...
	struct snd_kcontrol *kcontrol, int event)
{
	...
	case SND_SOC_DAPM_PRE_PMU:
		...
		snd_soc_component_update_bits_with_lock(codec, ULTR_ADC_CTRL0_REG,
			BIT(ULTR_ADC_HBFVD_BYPASS_EN_OFFSET), BIT(ULTR_ADC_HBFVD_BYPASS_EN_OFFSET));
		break;
...
#define MIC78_INPUT_OUTPUT_WIDGET \
	SND_SOC_DAPM_INPUT("AUDIO_MIC78_INPUT"), \
	SND_SOC_DAPM_OUTPUT("AUDIO_MIC78_OUTPUT")

static const struct snd_kcontrol_new dapm_wakeup_main_mic78_16k_switch_controls =
	SOC_DAPM_SINGLE("ENABLE", CODEC_VIR2_REG_ADDR, WAKEUP_MAIN_MIC_SWITCH_BIT, 1, 0);

static void mic78_fifo_clk_clear(struct snd_soc_component *codec)
{
...
static void mic78_fifo_clk_set(struct snd_soc_component *codec)
{
	snd_soc_component_update_bits_with_lock(codec, MIC78_UP_AFIFO_CLKEN_REG,
...
static void mic78_up_16k_set(struct snd_soc_component *codec)
{
	snd_soc_component_update_bits(codec, SRCDN_CTRL1_REG,
		mask_on_bit(MIC7_UP_SRCDN_SRC_MODE_LEN, MIC7_UP_SRCDN_SRC_MODE_OFFSET) |
		mask_on_bit(MIC8_UP_SRCDN_SRC_MODE_LEN, MIC8_UP_SRCDN_SRC_MODE_OFFSET),
		0x2 << MIC7_UP_SRCDN_SRC_MODE_OFFSET | 0x2 << MIC8_UP_SRCDN_SRC_MODE_OFFSET);
	snd_soc_component_update_bits(codec, FS_CTRL1_REG,
		mask_on_bit(FS_MIC7_UP_AFIFO_LEN, FS_MIC7_UP_AFIFO_OFFSET) |
		mask_on_bit(FS_MIC8_UP_AFIFO_LEN, FS_MIC8_UP_AFIFO_OFFSET),
		BIT(FS_MIC7_UP_AFIFO_OFFSET) | BIT(FS_MIC8_UP_AFIFO_OFFSET));
	snd_soc_component_update_bits(codec, FS_CTRL0_REG,
		mask_on_bit(FS_MIC5_UP_PGA_LEN, FS_MIC8_UP_PGA_OFFSET),
		BIT(FS_MIC8_UP_PGA_OFFSET));
	snd_soc_component_update_bits(codec, FS_CTRL1_REG,
		mask_on_bit(FS_MIC5_UP_PGA_LEN, FS_MIC7_UP_PGA_OFFSET),
		BIT(FS_MIC7_UP_PGA_OFFSET));
	snd_soc_component_update_bits(codec, FS_CTRL5_REG,
		mask_on_bit(FS_MIC5_SRCDN_DOUT_LEN, FS_MIC7_SRCDN_DOUT_OFFSET) |
		mask_on_bit(FS_MIC6_SRCDN_DOUT_LEN, FS_MIC8_SRCDN_DOUT_OFFSET),
		BIT(FS_MIC7_SRCDN_DOUT_OFFSET) | BIT(FS_MIC8_SRCDN_DOUT_OFFSET));
}

static void mic78_src_and_pga_set(struct snd_soc_component *codec)
{
...
static void mic78_src_and_pga_clear(struct snd_soc_component *codec)
{
	snd_soc_component_update_bits(codec, MIC78_UP_PGA_CLKEN_REG,
...
static int wakeup_main_mic78_16k_power_event(struct snd_soc_dapm_widget *w,
	struct snd_kcontrol *kcontrol, int event)
{
	struct snd_soc_component *codec = snd_soc_dapm_to_component(w->dapm);

	IN_FUNCTION;
	AUDIO_LOGI("wakeup main mic78 16k power event %d", event);

	switch (event) {
	case SND_SOC_DAPM_PRE_PMU:
		mic78_fifo_clk_clear(codec);
		mic78_up_16k_set(codec);
		mic78_src_and_pga_set(codec);
		udelay(CLEAR_FIFO_DELAY_LEN_US);
		mic78_fifo_clk_set(codec);
		break;
	case SND_SOC_DAPM_POST_PMD:
		mic78_fifo_clk_clear(codec);
		mic78_src_and_pga_clear(codec);
		break;
	default:
		AUDIO_LOGW("wakeup main mic78 16k power event err: %d", event);
		break;
	}
...
#define WAKEUP_MIC78_SWITCH_WIDGET \
	SND_SOC_DAPM_SWITCH_E("WAKEUP_MIC78_O_SWITCH", SND_SOC_NOPM, 0, 0, \
		&dapm_wakeup_main_mic78_16k_switch_controls, wakeup_main_mic78_16k_power_event, \
		(SND_SOC_DAPM_PRE_PMU | SND_SOC_DAPM_POST_PMD)) \
...
static const struct snd_kcontrol_new dapm_bt_noise_reduction_32k_src_controls =
	SOC_DAPM_SINGLE("ENABLE", CODEC_VIR1_REG_ADDR, BT_NOISE_REDUCTION_32K_BIT, 1, 0);
static const struct snd_kcontrol_new dapm_bt_noise_reduction_voip_src_controls =
	SOC_DAPM_SINGLE("ENABLE", CODEC_VIR1_REG_ADDR, BT_NOISE_REDUCTION_VOIP_BIT, 1, 0);
...
static int bt_noise_reduction_32k_power_event(struct snd_soc_dapm_widget *w, struct snd_kcontrol *kcontrol, int event)
{
	(void)kcontrol;
	struct snd_soc_component *codec = snd_soc_dapm_to_component(w->dapm);

	IN_FUNCTION;
	AUDIO_LOGI("CollaborativeNoiseReduction power event %d", event);
	...
	case SND_SOC_DAPM_POST_PMD:
		...
		break;
	default:
		AUDIO_LOGW("CollaborativeNoiseReduction power event err: %d", event);
		break;
	}
...
#define BT_UP_SRC_WIDGET \
	...
	SND_SOC_DAPM_SWITCH_E("BT_VOICE_UP_32K_SWITCH", SND_SOC_NOPM, 0, 0, \
		...
		(SND_SOC_DAPM_PRE_PMU | SND_SOC_DAPM_POST_PMD)), \
	SND_SOC_DAPM_SWITCH_E("BT_NOISE_REDUCTION_32K_SWITCH", SND_SOC_NOPM, 0, 0, \
		&dapm_bt_noise_reduction_32k_src_controls, bt_noise_reduction_32k_power_event, \
		(SND_SOC_DAPM_PRE_PMU | SND_SOC_DAPM_POST_PMD)) \

static const struct snd_soc_dapm_widget uplink_adapter_dapm_widgets[] = {
	UP_MUX_WIDGET,
	ULTR_UP_SWITCH_WIDGET,
	MIC78_INPUT_OUTPUT_WIDGET,
	WAKEUP_MIC78_SWITCH_WIDGET,
	BT_UP_SRC_WIDGET,
};
