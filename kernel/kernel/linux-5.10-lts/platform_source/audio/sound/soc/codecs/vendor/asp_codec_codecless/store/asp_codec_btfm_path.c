...
static int i2s2_supply_power_event(struct snd_soc_dapm_widget *w,
	struct snd_kcontrol *kcontrol, int event)
{
	unused_parameter(w);
...
static const struct snd_kcontrol_new dapm_uart_route_switch_controls =
	SOC_DAPM_SINGLE("ENABLE", CODEC_VIR1_REG_ADDR, A2DP_OFFLOAD_BIT, 1, 0);

#define BT_UART_ROUTE_WIDGET \
	SND_SOC_DAPM_INPUT("UART_INPUT"), \
	SND_SOC_DAPM_OUTPUT("UART_OUTPUT"), \
	SND_SOC_DAPM_SWITCH_E("UART_ROUTE_SWITCH", SND_SOC_NOPM, 0, 0, \
		&dapm_uart_route_switch_controls, i2s2_supply_power_event, \
		(SND_SOC_DAPM_PRE_PMU | SND_SOC_DAPM_POST_PMD)) \
