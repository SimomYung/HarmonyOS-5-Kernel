...
#define AU_PA_INPUT_OUTPUT_WIDGET \
	...
	SND_SOC_DAPM_INPUT("AUDIO_STEREO_INPUT"), \
	SND_SOC_DAPM_INPUT("RCV_INPUT"), \
	...
	SND_SOC_DAPM_OUTPUT("AUDIO_STEREO_OUTPUT"), \
	SND_SOC_DAPM_OUTPUT("RCV_OUTPUT"), \
...
static int pa2_stereo_fifo_power_event(struct snd_soc_dapm_widget *w,
	struct snd_kcontrol *kcontrol, int event)
{
	struct snd_soc_component *codec = snd_soc_dapm_to_component(w->dapm);
	...
	case SND_SOC_DAPM_PRE_PMU:
		/* clear fifo */
		da_separate_update_bits(codec, CODEC_CLK_EN0_REG,
			BIT(VOICE_L_DN_AFIFO_CLKEN_OFFSET) | BIT(AUDIO_L_DN_AFIFO_CLKEN_OFFSET), 0);

		udelay(CLEAR_FIFO_DELAY_LEN_MS);

		/* fifo clk enable */
		da_separate_update_bits(codec, CODEC_CLK_EN0_REG,
			BIT(VOICE_L_DN_AFIFO_CLKEN_OFFSET) | BIT(AUDIO_L_DN_AFIFO_CLKEN_OFFSET),
			BIT(VOICE_L_DN_AFIFO_CLKEN_OFFSET) | BIT(AUDIO_L_DN_AFIFO_CLKEN_OFFSET));
		break;
...
static int rcv_fifo_power_event(struct snd_soc_dapm_widget *w,
	struct snd_kcontrol *kcontrol, int event)
{
	struct snd_soc_component *codec = snd_soc_dapm_to_component(w->dapm);
	AUDIO_LOGI("power event enter %d", event);
...
static const struct snd_kcontrol_new pa2_stereo_au_virtual_switch[] = {
	SOC_DAPM_SINGLE("ENABLE", VIR1_ADDR, REG_BIT27, 1, 0),
};

static const struct snd_kcontrol_new pa2_stereo_dac_switch[] = {
	SOC_DAPM_SINGLE("ENABLE", AU2_3_CTRL_REG, AU2_SPA_DAC_EN_OFFSET, 1, 0),
};

static const struct snd_kcontrol_new dapm_pa2_stereo_fifo_switch_controls =
	SOC_DAPM_SINGLE("ENABLE", VIR1_ADDR, REG_BIT28, 1, 0);

static const struct snd_kcontrol_new rcv_fifo_switch_controls =
	SOC_DAPM_SINGLE("ENABLE", VIR1_ADDR, REG_BIT29, 1, 0);
...
#define AU_PA_SWITCH_WIDGET \
	...
	SND_SOC_DAPM_SWITCH("PA2_STEREO_AU_SWITCH", SND_SOC_NOPM, 0, 0, pa2_stereo_au_virtual_switch), \
	SND_SOC_DAPM_SWITCH("PA2_STEREO_DAC_SWITCH", SND_SOC_NOPM, 0, 0, pa2_stereo_dac_switch), \
	SND_SOC_DAPM_SWITCH_E("PA2_STEREO_FIFO_SWITCH", SND_SOC_NOPM, 0, 0, \
		&dapm_pa2_stereo_fifo_switch_controls, pa2_stereo_fifo_power_event, \
		(SND_SOC_DAPM_PRE_PMU | SND_SOC_DAPM_POST_PMD)), \
	SND_SOC_DAPM_SWITCH_E("RCV_FIFO_SWITCH", SND_SOC_NOPM, 0, 0, \
		&rcv_fifo_switch_controls, rcv_fifo_power_event, \
		(SND_SOC_DAPM_PRE_PMU | SND_SOC_DAPM_POST_PMD)), \
...
// for burbank single pa + rcv play scene
#define AUDIO_STEREO_DOWNLINK \
	{ "VOICE_DL_PA2_STEREO_PGA",  NULL,                "AU_PA2_DLINK INPUT" }, \
	{ "VOICE_DR PGA",             NULL,                "AU_PA3_DLINK INPUT" }, \
	{ "VOICE_DL_PA2_STEREO_PGA",  NULL,                "PLL" }, \
	{ "VOICE_DR PGA",             NULL,                "PLL" }, \
	{ "PA2_STEREO_AU_SWITCH",     "ENABLE",            "VOICE_DL_PA2_STEREO_PGA" }, \
	{ "AU_PA3 SWITCH",            "AU_PA3_EN",         "VOICE_DR PGA" }, \
	{ "AU_PA2 OUTPUT",            NULL,                "PA2_STEREO_AU_SWITCH" }, \
	{ "AU_PA3 OUTPUT",            NULL,                "AU_PA3 SWITCH" }, \
	{ "AU_PA_DL MIXER",           "IN1_VOICE_DL",      "VOICE_DL_PA2_STEREO_PGA" }, \
	{ "AU_PA_DR MIXER",           "IN1_VOICE_DR",      "VOICE_DR PGA" }, \
	{ "AU_PA_DL MIXER",           "IN2_FM_L",          "S2_SRC_RX SWITCH" }, \
	{ "AU_PA_DR MIXER",           "IN2_FM_R",          "S2_SRC_RX SWITCH" }, \
	{ "AU_PA_DL MIXER",           "IN3_ULTR_DN",       "ULTR_DLINK_EN MIXER" }, \
	...
	{ "AU_PA2_DACL_FILTER",       NULL,                "AU_PA_DL MIXER" }, \
	{ "AU_PA3_DACR_FILTER",       NULL,                "AU_PA_DR MIXER" }, \
	{ "PA2_STEREO_DAC_SWITCH",    "ENABLE",            "AU_PA2_DACL_FILTER" }, \
	{ "DAC_SPA3 SWITCH",          "DAC_SPA3_EN",       "AU_PA3_DACR_FILTER" }, \
	{ "AU_PA2_EN MIXER",          "AU_PA2_DAC_EN",     "PA2_STEREO_DAC_SWITCH" }, \
	{ "AU_PA3_EN MIXER",          "AU_PA3_DAC_EN",     "DAC_SPA3 SWITCH" }, \
	{ "DACL_SPA2 OUTPUT",         NULL,                "AU_PA2_EN MIXER" }, \
	{ "DACR_SPA3 OUTPUT",         NULL,                "AU_PA3_EN MIXER" }, \

#define PA2_STEREO_FIFO_DOWNLINK \
	{ "PA2_STEREO_FIFO_SWITCH",       "ENABLE",        "AUDIO_STEREO_INPUT" }, \
	{ "AUDIO_STEREO_OUTPUT",          NULL,            "PA2_STEREO_FIFO_SWITCH" }, \

#define RCV_FIFO_DOWNLINK \
	{ "RCV_FIFO_SWITCH",              "ENABLE",        "RCV_INPUT" }, \
	{ "RCV_OUTPUT",                   NULL,            "RCV_FIFO_SWITCH" }, \
...
static const struct snd_soc_dapm_route au_pa_route_map[] = {
	AU_PA_DOWNLINK
	AU_PA_UPLINK
	AUDIO_STEREO_DOWNLINK
	PA2_STEREO_FIFO_DOWNLINK
	RCV_FIFO_DOWNLINK
};
#else
static const struct snd_soc_dapm_route au_pa_route_map[] = {
