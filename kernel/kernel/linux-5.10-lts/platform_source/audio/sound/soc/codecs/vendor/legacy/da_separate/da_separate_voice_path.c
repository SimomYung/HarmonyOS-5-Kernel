...
static int voice_pa2_stereo_pga(struct snd_soc_dapm_widget *w,
	struct snd_kcontrol *kcontrol, int event)
{
	struct snd_soc_component *codec = snd_soc_dapm_to_component(w->dapm);
	...
	case SND_SOC_DAPM_POST_PMU:
		da_separate_update_bits(codec, CODEC_CLK_EN0_REG,
			BIT(VOICE_L_DN_PGA_CLKEN_OFFSET) | BIT(VOICE_L_DN_SRCUP_CLKEN_OFFSET),
			BIT(VOICE_L_DN_PGA_CLKEN_OFFSET) | BIT(VOICE_L_DN_SRCUP_CLKEN_OFFSET));
		break;
	case SND_SOC_DAPM_PRE_PMD:
		/* clk disable */
		da_separate_update_bits(codec, CODEC_CLK_EN0_REG,
			BIT(VOICE_L_DN_PGA_CLKEN_OFFSET) | BIT(VOICE_L_DN_SRCUP_CLKEN_OFFSET), 0);
		break;
	default:
		AUDIO_LOGE("power mode event err: %d", event);
		break;
...
static const struct snd_soc_dapm_widget voice_dapm_widgets[] = {
	...
	SND_SOC_DAPM_PGA_E("VOICE_DL_PA2_STEREO_PGA", SND_SOC_NOPM, 0, 0, NULL, 0,
		voice_pa2_stereo_pga, (SND_SOC_DAPM_POST_PMU | SND_SOC_DAPM_PRE_PMD)),
