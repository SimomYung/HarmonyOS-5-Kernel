...
int usb_h2x_on(void)
{
	...
	if (asp_subsys_clk) {
		...
		if (ret != 0) {
			AUDIO_LOGE("asp subsys clk enable failed: %d", ret);
...
static void asp_cfg_dp_module_enable_i2s(void)
{
	/* enable clk */
...
static void asp_cfg_dp_module_disable_i2s(void)
{
	/* disable clk */
...
static void asp_cfg_dp_module_enable_spdif(void)
{
	/* enable clk */
...
static void asp_cfg_dp_module_disable_spdif(void)
{
	/* disable clk */
...
void asp_cfg_dp_module_enable(enum asp_hdmi_interface type)
{
	switch (type) {
	case ASP_HDMI_I2S:
		asp_cfg_dp_module_enable_i2s();
		break;
	default:
		asp_cfg_dp_module_enable_spdif();
		break;
	}
}
...
void asp_cfg_dp_module_disable(enum asp_hdmi_interface type)
{
	switch (type) {
	case ASP_HDMI_I2S:
		asp_cfg_dp_module_disable_i2s();
		break;
	default:
		asp_cfg_dp_module_disable_spdif();
		break;
	}
}
