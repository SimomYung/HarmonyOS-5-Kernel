...
struct asp_hdmi_dma_priv {
	...
	spinlock_t lock;
	struct resource *res;
	void __iomem *asp_hdmi_dma_reg_base_addr;
};
...
static unsigned int dmac_reg_read(unsigned int reg)
{
	...
	ret = readl(priv->asp_hdmi_dma_reg_base_addr + reg);
...
static void dmac_reg_write(unsigned int reg, unsigned int value)
{
	...
	spin_lock_irqsave(&priv->lock, flag_sft);
	writel(value, priv->asp_hdmi_dma_reg_base_addr + reg);
	spin_unlock_irqrestore(&priv->lock, flag_sft);
}
...
static void dmac_reg_set_bit(unsigned int reg, unsigned int offset)
{
	...
	value = readl(priv->asp_hdmi_dma_reg_base_addr + reg);
	value |= BIT(offset);
	writel(value, priv->asp_hdmi_dma_reg_base_addr + reg);
...
static int asp_hdmi_dma_probe(struct platform_device *pdev)
{
	...
	priv->asp_hdmi_dma_reg_base_addr =
		devm_ioremap(dev, priv->res->start, resource_size(priv->res));
	if (!priv->asp_hdmi_dma_reg_base_addr) {
