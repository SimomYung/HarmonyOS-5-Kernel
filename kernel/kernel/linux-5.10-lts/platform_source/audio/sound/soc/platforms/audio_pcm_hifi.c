...
static int pcm_notify_set_buf(struct snd_pcm_substream *substream)
{
	...
	if (pcm_device == (int)priv_cust->dev_mmap) {
	...
	if (prtd->using_thread_flag)
		...
		ret = pcm_mailbox_send_data(&msg_body, sizeof(msg_body));
	...
	struct mb_queue *mail_handle, unsigned int mail_len)
{
	...
	case HI_CHN_MSG_PCM_PERIOD_STOP:
		if (prtd->status == STATUS_STOPPING) {
			...
			AUDIO_LOGI("device %d mode %hu stop now",
	...
	struct snd_pcm_hw_params *params)
{
	...
	ret = pcm_mailbox_send_data(&open_msg, sizeof(open_msg));
	...
	struct snd_pcm_hw_params *params)
{
	...
	if (device == (int)priv_cust->dev_mmap) {
	...
	if (ret < 0) {
		...
		if (device != (int)priv_cust->dev_mmap) {
			...
			int result = pcm_free_mmap_share_buf(substream);
			if (result != 0)
...
static int audio_pcm_open(struct snd_pcm_substream *substream)
{
	...
	AUDIO_LOGI("device %d, mode %d open", pcm_device, pcm_mode);
	...
	if (WARN_ON(pcm_device >= (int)priv_cust->dev_num || pcm_mode >= PCM_STREAM_MAX))
		return -EINVAL;
	...
	prtd->is_support_low_power = (pdata->low_power_support &&
		pcm_mode == SNDRV_PCM_STREAM_PLAYBACK && pcm_device == (int)priv_cust->dev_normal);
...
static int preallocate_dma_buffer(struct snd_pcm *pcm, int stream)
{
	...
	if (pcm->device == (int)priv_cust->dev_mmap)
		return 0;
