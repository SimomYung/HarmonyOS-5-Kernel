...
bool is_typec_support_suspend(void)
{
	struct usbaudio_dsp *pdata = get_usbaudio_priv_data();
	if (!pdata)
		return false;
	return pdata->typec_support_suspend;
}

static bool is_allow_use_hifiusb(struct usb_device *dev)
{
	struct usbaudio_dsp *pdata = get_usbaudio_priv_data();
	if (!pdata || !dev) {
		AUDIO_LOGE("pdata or dev is null");
		return false;
	}

	if (pdata->usbaudio_connect_state == USBAUDIO_NONE) {
		AUDIO_LOGI("not yet use hifiusb, allow to try switch");
		return true;
	}

	if (pdata->usbaudio_connect_state == USBAUDIO_AP) {
		AUDIO_LOGI("switching hifiusb, only allow hifiusb switch");
		if (usb_using_hifi_usb(dev))
			return true;
		return false;
	}

	if (pdata->usbaudio_connect_state == USBAUDIO_DSP) {
		AUDIO_LOGI("using hifiusb, other forbid use hifiusb");
		return false;
	}
	return false;
}

static bool is_hifiusb(struct usb_device *dev)
{
	struct usbaudio_dsp *pdata = get_usbaudio_priv_data();
	if (!pdata || !dev) {
		AUDIO_LOGE("pdata or dev is null");
...
static enum usbaudio_controller_status query_multi_usb_location(struct usbaudio_dsp *pdata, unsigned int card_index)
{
	if (pdata->chip->card->number != (int)card_index)
		return ACPU_CONTROL;
...
static enum usbaudio_controller_status query_single_usb_location(struct usbaudio_dsp *pdata)
{
	if (pdata->ops->controller_query(pdata->chip))
		return DSP_CONTROL;
	return ACPU_CONTROL;
...
void usbaudio_ctrl_query_info(unsigned int card_index, struct usbaudio_info *usbinfo)
{
	struct usbaudio_dsp *pdata = get_usbaudio_priv_data();
	...
	if (!pdata->chip) {
		...
		AUDIO_LOGI("usb card number is %d, card_index is :%x", pdata->chip->card->number, card_index);
		if (card_index >= CARD_DECODE_MASK)
			usbinfo->controller_location = query_multi_usb_location(pdata, card_index - CARD_DECODE_MASK);
		else
			usbinfo->controller_location = query_single_usb_location(pdata);
		AUDIO_LOGI("controller location %d", usbinfo->controller_location);
	}
...
static void usbaudio_ctrl_set_chip(struct snd_usb_audio *chip)
{
	...
	mutex_lock(&pdata->connect_mutex);
	if (!is_hifiusb(chip->dev)) {
		AUDIO_LOGI("arm usb not save chip infomation");
		mutex_unlock(&pdata->connect_mutex);
		return;
	}
	pdata->wake_up = true;
	...
	if (chip->card) {
		...
		AUDIO_LOGI("usb card number is %d",chip->card->number);	
#ifdef CONFIG_HUAWEI_AUDIO
...
static int usbaudio_dsp_probe(struct platform_device *pdev)
{
	...
	if (of_property_read_bool(node, "typec_support_suspend"))
		pdata->typec_support_suspend = true;
	else
		pdata->typec_support_suspend = false;
	return 0;
}
