...
#endif
#ifdef CONFIG_STORAGE_LOW_POWER_INTERFACE
#include <linux/mutex.h>
#ifdef CONFIG_SCSI_UFSHCD
#include <scsi/ufs/ufshcd.h>
#endif
#ifdef CONFIG_HIPCIE_CORE
#include <basicplatform/drivers/pci/controller/hi_pcie_external_api.h>
#ifdef CONFIG_LIBLINUX
#include <liblinux/pal.h>
#endif
#endif
#endif /* CONFIG_STORAGE_LOW_POWER_INTERFACE */
...
struct __bootdevice {
	...
	void (*get_dfa_rev)(const struct device *, char *);
#endif
	char name[MAX_NAME_LENTH + 1];
	char rev[MAX_REV_LENTH + 1];
...
void set_bootdevice_type(u32 type)
{
	bootdevice.type = type;
}

u32 get_bootdevice_type(void)
{
...
void set_bootdevice_rev(char *fr, u32 len)
{
	if (!fr || (len == 0))
		return;

	(void)memcpy_s(bootdevice.rev, sizeof(bootdevice.rev), fr, len);
}
...
static int rev_proc_show(struct seq_file *m, void *v)
{
	...
	if ((bootdevice.get_rev) && (bootdevice.dev)) {
...
#ifdef CONFIG_DFA_MCQ
void set_bootdevice_dfa_rev_handler(void (*get_dfa_rev_func)(const struct device *, char *))
{
	bootdevice.get_dfa_rev = get_dfa_rev_func;
}

static int dfa_rev_proc_show(struct seq_file *m, void *v)
{
	char dfa_rev[2 * MAX_REV_LEN + 1] = { 0 };

	if ((bootdevice.get_dfa_rev) && (bootdevice.dev)) {
		bootdevice.get_dfa_rev(bootdevice.dev, dfa_rev);
		seq_printf(m, "%s\n", dfa_rev);
	}
	return 0;
}

static int dfa_rev_proc_open(struct inode *inode, struct file *file)
{
	return single_open(file, dfa_rev_proc_show, NULL);
}

#if (LINUX_VERSION_CODE >= KERNEL_VERSION(5,10,0))
static const struct proc_ops dfa_rev_proc_fops = {
	.proc_open = dfa_rev_proc_open,
	.proc_read = seq_read,
	.proc_lseek = seq_lseek,
	.proc_release = single_release,
};
...
#ifdef CONFIG_STORAGE_LOW_POWER_INTERFACE
static DEFINE_MUTEX(storage_low_power_control_lock);
/* normally low power is enabled.
   to make low level interface happy, should call in the order: disable->enable */
static bool storage_low_power_enable = true;
static void *storage_low_power_writer;

#ifdef CONFIG_HIPCIE_CORE
#define SSD_PCIE_CDX 	2
#ifdef CONFIG_LIBLINUX
#define NVME_IOCTL_LOWPOWER_CMD	_IOW('N', 0x42, char)
#define NVME_DEVICE 	"/dev/nvme0"

static int nvme_low_power_ctrl(bool enable)
{
	int fd;
	int ret;
	char ioc_arg = enable;

	fd = liblinux_pal_vfs_open(NVME_DEVICE, O_RDONLY, 0);
	if (fd < 0) {
		pr_err("%s open %s failed, ret=%d\n", __func__, NVME_DEVICE, fd);
		return -EINVAL;
	}

	ret = liblinux_pal_vfs_ioctl(fd, NVME_IOCTL_LOWPOWER_CMD, (uintptr_t)&ioc_arg, sizeof(ioc_arg));
	if (ret)
		pr_err("%s ioctl %s failed: ret=%d\n", __func__, NVME_DEVICE, ret);

	(void)liblinux_pal_vfs_close(fd);
	return ret;
}
#endif /* CONFIG_LIBLINUX */
#endif /* CONFIG_HIPCIE_CORE */

static int storage_low_power_ctrl(bool enable)
{
	int ret = -EINVAL;
	u32 dev_type = get_bootdevice_type();

	if (storage_low_power_enable != enable) {
		if (dev_type == BOOT_DEVICE_UFS) {
#ifdef CONFIG_SCSI_UFSHCD
			if (enable) {
		...
		} else if (dev_type & (1 <<  DEVICE_TYPE_SSD)) {
#ifdef CONFIG_HIPCIE_CORE
			ret = hi_pcie_lp_ctrl(SSD_PCIE_CDX, enable);
			if (ret) {
				pr_err("%s hi_pcie_lp_ctrl fail: ret=%d\n", __func__, ret);
				ret = -EINVAL;
			} else {
#ifdef CONFIG_LIBLINUX
				ret = nvme_low_power_ctrl(enable);
#endif
			}
#endif /* CONFIG_HIPCIE_CORE */
		}

		pr_info("%s set auto enable=%d, ret=%d\n", __func__, enable, ret);
		if (ret == 0)
			storage_low_power_enable = enable;
	} else {
		pr_info("%s unchanged auto enable=%d\n", __func__, enable);
		ret = 0;
	}
	return ret;
}

static ssize_t storage_low_power_proc_write(struct file *file, const char __user *buf,
				   size_t count, loff_t *ppos)
{
	...
	if (ret) {
		pr_err("%s Invalid bool argument", __func__);
		return ret;
	}

	/* only one writer, and writer can write again */
	mutex_lock(&storage_low_power_control_lock);
	if (storage_low_power_writer == NULL) {
		storage_low_power_writer = file;
	}

	if (storage_low_power_writer == file) {
		ret = storage_low_power_ctrl(val);
	} else {
		pr_err("%s auto enable unchanged: another writer in progress\n", __func__);
		ret = -EBUSY;
	}
	mutex_unlock(&storage_low_power_control_lock);
...
static int storage_low_power_proc_show(struct seq_file *m, void *v)
{
	seq_printf(m, "%d\n", storage_low_power_enable);
	return 0;
}

static int storage_low_power_proc_open(struct inode *inode, struct file *file)
{
	return single_open(file, storage_low_power_proc_show, NULL);
}

static int storage_low_power_proc_release(struct inode *inode, struct file *file)
{
	/* when writer leave or crash, keep low power enable */
	mutex_lock(&storage_low_power_control_lock);
	if (storage_low_power_writer == file) {
		if (!storage_low_power_enable) {
			pr_info("%s restore auto enable in release\n", __func__);
			storage_low_power_ctrl(true);
		}
		storage_low_power_writer = NULL;
	}
	mutex_unlock(&storage_low_power_control_lock);
...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(5,10,0))
static const struct proc_ops storage_low_power_proc_ops = {
	.proc_open	= storage_low_power_proc_open,
	.proc_read	= seq_read,
	.proc_lseek	= seq_lseek,
	.proc_write	= storage_low_power_proc_write,
	.proc_release = storage_low_power_proc_release,
};
#else
static const struct file_operations storage_low_power_proc_ops = {
	.open = storage_low_power_proc_open,
	.read = seq_read,
	.llseek = seq_lseek,
	.write = storage_low_power_proc_write,
	.release = storage_low_power_proc_release,
};
#endif
#endif /* CONFIG_STORAGE_LOW_POWER_INTERFACE */

#ifdef CONFIG_SPI_NOR_AGENT
void set_bootdevice_jedec_id(u32 jedec_id)
{
	bootdevice.jedec_id = jedec_id;
}

static int jedec_id_proc_show(struct seq_file *m, void *v)
{
	seq_printf(m, "0x%x\n", bootdevice.jedec_id);
	return 0;
}

static int jedec_id_proc_open(struct inode *inode, struct file *file)
{
	return single_open(file, jedec_id_proc_show, NULL);
}

static const struct proc_ops jedec_id_proc_fops = {
	.proc_open = jedec_id_proc_open,
	.proc_read = seq_read,
	.proc_lseek = seq_lseek,
	.proc_release = single_release,
};
...
#ifdef CONFIG_DFA_MCQ
	proc_create("bootdevice/dfa_rev", 0000, NULL, &dfa_rev_proc_fops);
#endif
...
#ifdef CONFIG_STORAGE_LOW_POWER_INTERFACE
	proc_create("bootdevice/low_power", 0660, NULL, &storage_low_power_proc_ops);
#endif
#ifdef CONFIG_SPI_NOR_AGENT
	proc_create("bootdevice/jedec_id", 0000, NULL, &jedec_id_proc_fops);
#endif
	return 0;
}
