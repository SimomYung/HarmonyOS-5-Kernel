...
static unsigned long get_pll_rate(struct media_pll_info *pll_info)
{
	unsigned long pll_rate = 0;
	struct clk *pll = NULL;

	if (pll_info->pll_type == ALTER_PLL) {
		pll = __clk_lookup(pll_info->pll_name);
		if (IS_ERR_OR_NULL(pll)) {
			pr_err("[%s] %s get failed!\n", __func__, pll_info->pll_name);
			return 0;
		}
		pll_rate = __clk_get_rate(pll);
	} else {
		pll_rate = ((unsigned long)pll_info->pll_profile) * FREQ_CONVERSION;
	}

	return pll_rate;
}

static unsigned long get_pll_profile(struct hi3xxx_fastclk *fclk, u32 pll_id, int level)
{
	struct media_pll_info *pll_info = fclk->pll_info[pll_id];
	unsigned long pll_profile = 0;
	unsigned int mul_ret;

	if (pll_info->pll_type == ALTER_PLL)
		pll_profile = ((unsigned long)fclk->profile_value[level]) * fclk->div_val[level];
	else
		pll_profile = (unsigned long)pll_info->pll_profile;

	mul_ret = mul_valid_cal(pll_profile, FREQ_CONVERSION);
	if (!mul_ret) {
		pr_err("[%s] profile:%lu is invalid!\n", __func__, pll_profile);
		return 0;
	}

	return pll_profile * FREQ_CONVERSION;
}

static int pll_set_rate(struct clk *pll, unsigned int pll_type,
	struct hi3xxx_fastclk *fclk, int level)
{
	unsigned long pll_rate = 0;
	unsigned int mul_ret;

	/* only alter pll can set rate */
	if (pll_type != ALTER_PLL)
		return 0;

	pll_rate = ((unsigned long)fclk->profile_value[level]) * fclk->div_val[level];
	mul_ret = mul_valid_cal(pll_rate, FREQ_CONVERSION);
	if (!mul_ret) {
		pr_err("[%s] profile:%lu div:%u is invalid!\n",
			__func__, fclk->profile_value[level], fclk->div_val[level]);
		return -EINVAL;
	}

	pll_rate = pll_rate * FREQ_CONVERSION + ADD_FREQ_OFFSET;
	return clk_set_rate_nolock(pll, pll_rate);
}
	...
	unsigned long parent_rate)
{
	...
	sw_value = ((sw_value & fclk->clksw_offset[CFG_MASK]) | (fclk->clksw_offset[CFG_MASK] << HIMASKEN_SHIFT));

	for (i = 0; i <= fclk->profile_level; i++)
		if (sw_value == fclk->p_sw_cfg[i])
			break;

	if ((i > fclk->profile_level) || (fclk->profile_pll_id[i] >= fclk->pll_num)) {
	...
	pll_info = fclk->pll_info[fclk->profile_pll_id[i]];
	fclk->rate = get_pll_rate(pll_info) / div_value;
	...
	int cur_level, int tar_level)
{
	...
	/* only alter pll can set rate */
	ret = pll_set_rate(tar_pll, tar_pll_info->pll_type, fclk, tar_level);
	if (ret) {
		pr_err("[%s] tar_pll %s set rate fail, ret = %d!\n",
	...
	unsigned long tar_rate, int cur_level, int tar_level)
{
	...
	cur_pll_profile = get_pll_profile(fclk, pll_id, cur_level);

	pll_id = fclk->profile_pll_id[tar_level];
	if (pll_id >= fclk->pll_num)
		goto err_ret;

	tar_pll_profile = get_pll_profile(fclk, pll_id, tar_level);
...
static inline void sw_div_done_check(void __iomem *base, unsigned int offset,
	unsigned int cfg, unsigned int mask)
{
	unsigned int reg_value;

	reg_value = (readl(base + offset) & mask);
	if ((cfg & mask) != reg_value)
        pr_err("[%s] sw-div check fail! addr:0x%x cfg:0x%x reg_val:0x%x!\n",
			__func__, offset, cfg, reg_value);
}
	...
	unsigned long rate, unsigned long parent_rate)
{
	...
	sw_div_done_check(fclk->base_addr, fclk->clkdiv_offset[CFG_OFFSET],
		inter_div_cfg, fclk->clkdiv_offset[CFG_MASK]);

	/* target sw pll choose set */
	writel(fclk->p_sw_cfg[tar_level], sw_addr);
	sw_div_done_check(fclk->base_addr, fclk->clksw_offset[CFG_OFFSET],
		fclk->p_sw_cfg[tar_level], fclk->clksw_offset[CFG_MASK]);

	/* target div set */
	writel(fclk->p_div_cfg[tar_level], div_addr);
	sw_div_done_check(fclk->base_addr, fclk->clkdiv_offset[CFG_OFFSET],
		fclk->p_div_cfg[tar_level], fclk->clkdiv_offset[CFG_MASK]);

	spin_unlock_irqrestore(&fast_dvfs_lock, flags);

	clk_log_dbg("%s level:[%d->%d] rate:[%lu->%lu] inter_div:0x%x target_div:0x%x sw_val:0x%x\n",
		clk_hw_get_name(hw), cur_level, tar_level, cur_rate, tar_rate,
		inter_div_cfg, fclk->p_div_cfg[tar_level], fclk->p_sw_cfg[tar_level]);
