...
#define FSM_INTR_NO         1
#define PLL_UNLOCK_INTR_NO  0
#define FSM_INTER_BP_FLAG   0
...
static irqreturn_t pll_unlock_interrupt(int irq, void *p)
{
	...
	if (pll->fsm_flag != FSM_INTER_BP_FLAG) {
		pll_fsm_stat = (unsigned int)readl(pll->fsm_dump + pll->base_addr);
		hipll_err("%s FSM UnLock Status : 0x%x!\n", __func__, pll_fsm_stat);
		disable_irq_nosync(pll->fsm_irq);
	}
...
#else
	bbox_diaginfo_record(CLOCK_PLL_AP, NULL, "pll unlock status : 0x%x, 0x%x",
		pll_lock_stat, pll_fsm_stat);
#endif
	return IRQ_HANDLED;
}
...
static int pll_init(struct device_node *node, struct hipll_device *pll)
{
	...
	ret = of_property_read_u32(node, "pll_fsm_intr_flag", (u32 *)&pll->fsm_flag);
	if (ret)
		pll->fsm_flag = FSM_INTER_BP_FLAG;

	if (pll->fsm_flag != FSM_INTER_BP_FLAG) {
		ret = of_property_read_u32(node, "pll_fsm_intr_dump", (u32 *)&pll->fsm_dump);
		if (ret) {
			hipll_err("no fsm_dump resources\n");
			return -ENODEV;
		}
	}
...
static int fsm_intr_register(struct device_node *node, struct hipll_device *pll)
{
	int ret;
	...
	pll->fsm_irq = irq_of_parse_and_map(node, FSM_INTR_NO);
	hipll_debug("fsm_irq is [%u]\n", pll->fsm_irq);

	ret = request_irq(pll->fsm_irq, pll_unlock_interrupt, 0,
		"pll_fsm_intr", (void *)pll);
	if (ret) {
...
static int pll_unlock_probe(struct platform_device *pdev)
{
	...
	pll->pll_irq = irq_of_parse_and_map(node, PLL_UNLOCK_INTR_NO);
	...
	ret = fsm_intr_register(node, pll);
	if (ret) {
		hipll_err("request fsm irq failed, ret = %d\n", ret);
		return ret;
	}
