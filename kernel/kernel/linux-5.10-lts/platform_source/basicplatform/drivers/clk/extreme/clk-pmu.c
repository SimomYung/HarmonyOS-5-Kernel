...
static int hi3xxx_multicore_clkgate_is_enabled(struct clk_hw *hw)
{
	return 0;
}
...
static const struct clk_ops hi3xxx_pmu_clkgate_ops = {
	...
	.is_enabled = hi3xxx_multicore_clkgate_is_enabled,
};

static void __hi3xxx_abb_clk_hwunlock(struct hi3xxx_clkpmu *pclk, unsigned long *intr_state)
{
	hwspin_unlock_irqrestore(pclk->clk_hwlock, intr_state);
}

static int __hi3xxx_abb_clk_hwlock(struct hi3xxx_clkpmu *pclk, unsigned long *intr_state)
{
	if ((clk_hwlock_9 == NULL) && (pclk->hwlock_id != INVALID_HWSPINLOCK_ID)) {
		clk_hwlock_9 = hwspin_lock_request_specific(pclk->hwlock_id);
	...
	if (hwspin_lock_timeout_irqsave(pclk->clk_hwlock, CLK_HWLOCK_TIMEOUT, intr_state)) {
...
static int hi3xxx_multicore_abb_clkgate_prepare(struct clk_hw *hw)
{
	...
	u64 tm_begin, tm_end;
	unsigned long intr_state = 0;
	u32 val;
	int ret;

	ret = __hi3xxx_abb_clk_hwlock(pclk, &intr_state);
	if (ret)
		return ret;

	tm_begin = get_32k_abs_timer_value();
	...
	tm_end = get_32k_abs_timer_value();
	__hi3xxx_abb_clk_hwunlock(pclk, &intr_state);

	pr_info("clk abb prepare hold hwlock begin:%lu, end:%lu\n",
		tm_begin, tm_end);
...
static void hi3xxx_multicore_abb_clkgate_unprepare(struct clk_hw *hw)
{
	...
	if (__hi3xxx_abb_clk_hwlock(pclk, &intr_state)) {
	...
	val = readl(pclk->sctrl);
	if (!pclk->always_on) {
		if (((val & BIT(AP_ABB_EN)) == 1) || (pclk->disable_unused)) {
	...
	tm_end = get_32k_abs_timer_value();
	__hi3xxx_abb_clk_hwunlock(pclk, &intr_state);
	pclk->disable_unused = 0;

	pr_info("clk abb unprepare hold hwlock begin:%lu, end:%lu\n",
		tm_begin, tm_end);
#endif
}

static int hi3xxx_multicore_abb_clkgate_is_prepared(struct clk_hw *hw)
{
	struct hi3xxx_clkpmu *pclk = container_of(hw, struct hi3xxx_clkpmu, hw);
	u32 val;

	if (!(pclk->disable_unused))
		return 0;

	val = readl(pclk->sctrl);
	pr_info("abb_scbakdata=0x%x\n", val & ABB_EN_MASK);

	if ((val & ABB_EN_MASK) == 0) {
		val = (*pclk->clk_pmic_read)(pclk->pmu_clk_enable);
		if (val & pclk->ebits) {
			pr_info("clk abb is enabled before kernel, need disable for power\n");
			return 1;
		}
	}
...
static const struct clk_ops hi3xxx_abb_clkgate_ops = {
	...
	.is_prepared = hi3xxx_multicore_abb_clkgate_is_prepared,
};
	...
	struct clock_data *data)
{
	...
	pclk->hw.init = &init;
	pclk->hwlock_id = pmu_clk_hwspinlock_id(clkpmu->flags);
	pclk->disable_unused = pmu_clk_disable_unuse(clkpmu->flags);
