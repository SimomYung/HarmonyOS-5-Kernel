...
int get_clk_chip_type(void)
{
	...
	struct device_node *np = NULL;
	static int type = CHIP_TYPE_MAX;
	int32_t ret;

	if ((type >= CHIP_TYPE_ES) && (type < CHIP_TYPE_MAX))
		return type;

	np = of_find_node_by_name(NULL, "clk_chip_type");
	if (!np) {
		pr_info("[%s] not find clk_chip_type node!\n", __func__);
		type = CHIP_TYPE_COMMON;
		return type;
	}

	if (of_property_read_string(np, "type", &chip_type)) {
		pr_info("[%s] not find type!\n", __func__);
		type = CHIP_TYPE_COMMON;
		return type;
	}

	if (strcmp(chip_type, "es") == 0)
		type = CHIP_TYPE_ES;
	else if (strcmp(chip_type, "cs") == 0)
		type = CHIP_TYPE_CS;
	else if (strcmp(chip_type, "cs_v2") == 0)
		type = CHIP_TYPE_CS_V2;
	else
		type = CHIP_TYPE_COMMON;
	return type;
}
