...
#define PMU_MASK_LEN    8
#define pmu_clk_disable_unuse(pmu_clk_flag) (((pmu_clk_flag) >> PMU_MASK_LEN) & 0xFF)
#define pmu_clk_hwspinlock_id(pmu_clk_flag) ((pmu_clk_flag) & 0xFF)
#define pmu_clk_flag(disable_unuse_flag, hwlock_id) \
	((((disable_unuse_flag) & 0xFF) << PMU_MASK_LEN) | ((hwlock_id) & 0xFF))

#define CLKGATE_COUNT		2

/* fast dvfs clk define */
#define PROFILE_CNT		14
#define GATE_CFG_CNT		2
#define SW_DIV_CFG_CNT		3

/* comb dvfs clk define */
#define MAX_CLK_NUM  3
...
enum {
	VOTE_XPU_CLK_INIT,
	VOTE_XPU_CLK_ENABLE,
	VOTE_XPU_CLK_DISABLE,
	VOTE_XPU_CLK_DUMP,
	VOTE_XPU_CLK_ALL_DUMP,
	VOTE_XPU_CLK_OPS_MAX
};
...
struct alter_fsm_pll_clock {
	...
	u32 ctrl0_addr;
	u32 ctrl1_addr;
	const struct pll_freq_info *freq_tables;
	u32 freq_nums;
	const char	*alias;
};
...
struct pmu_clock {
	...
	u32	bit_mask;
	u32	flags; /* byte[1]: disable_unused flag, byte[0]: hwlock-id */
	u32	always_on;
	const char *alias;
};
...
#define ALTER_PLL_INFO(name, profile, type) \
	{\
		.pll_name = (name), \
		.pll_profile = (profile), \
		.pll_type = (type), \
...
struct comb_clock {
	const char *clk_name;
	u32 pwoff_freq;
	u32 sensitive_freqs[MAX_LEVEL_NUM];
};
...
struct comb_dvfs {
	...
	u8	num_clks;
	const struct level_info *level_infos;
	u8 num_levels;
};

struct freq_vote_reg {
	u32 addr;
	u8  freq_shift;
	u8  freq_width;
	u8  mask_bit;
...
struct freq_intr_stat {
	u32 addr;
	u8 stat_bit;
};

struct vote_xpu_clock {
	u32 id;
	const char *name;
	u32 xpu_clk_id;
	struct freq_vote_reg vote_reg;
	struct freq_stat_reg stat_reg;
	u32 result_freq_reg;
	struct freq_intr_stat intr_reg;
	u32 low_temperature_freq;
