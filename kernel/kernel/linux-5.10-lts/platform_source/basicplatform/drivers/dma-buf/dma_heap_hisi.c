				   ...
				   unsigned long length, unsigned int cmd)
{
	...
	ret = dma_heap_do_cache_op(dmabuf, uaddr, length, cmd);
...
int mm_dmabuf_page_rmap_user(struct dma_buf *dmabuf, struct vm_area_struct *vma)
{
	...
	array = vmalloc(sgt->nents * sizeof(*array));
	if (!array) {
		pr_err("alloc array fail!\n");
		return -ENOMEM;
	}
...
static int mm_dma_heap_clear_pages(struct page **pages, int num, pgprot_t pgprot)
{
	...
	if (!addr)
		return -ENOMEM;
	memset_s(addr, PAGE_SIZE * num, 0, PAGE_SIZE * num);
	vunmap(addr);
					    ...
					    pgprot_t pgprot)
{
	...
	pages = kvmalloc(sizeof(struct page *) * PAGES_MAX, GFP_KERNEL);
	if (!pages) {
