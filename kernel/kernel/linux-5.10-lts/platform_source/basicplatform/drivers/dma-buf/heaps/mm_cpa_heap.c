...
#ifdef CONFIG_MM_EBIT_SUPPORT
/* CPA HEAP support allocting 4K page for EBIT */
static unsigned int orders[] = {9, 6, 3, 0};
				...
				unsigned int order, unsigned int is_nofail)
{
	...
	if (is_nofail && (order <= orders[CPA_NUM_ORDERS - 1]))
				...
				struct cpa_heap *cpa_heap)
{
	...
	dma_ci_cache_by_sg(buffer->sg_table, DMA_FROM_DEVICE);
#else
			...
			unsigned long heap_flags)
{
	...
	unsigned long reclaimed_page = 0;
	/*lint -e666 */
	unsigned long size_remaining = ALIGN(size, order_to_size(orders[CPA_NUM_ORDERS - 1]));
	/*lint +e666 */
	unsigned int i = 0;
	int ret = 0;
