...
#define DMA_POOL_INVALID_SIZE 0xffffffff
...
static int static_dma_pool_heap_create(struct dma_pool_heap *dma_pool_heap)
{
	...
	/*
	 * When the initialization size of dma_pool_heap is set
	 * to 0xffffffff, the default pool size is the size of CMA.
	 * Otherwise, it is created according to heap_size.
	 */
	if (dma_pool_heap->size == DMA_POOL_INVALID_SIZE)
		size = cma_get_size(mm_camera_pool);
	else
		size = dma_pool_heap->size;
						...
						struct device_node *np)
{
	...
	if (ret < 0){
		pr_err("%s, can not find heap-size reset size 0xffffffff\n", __func__);
		heap_size = DMA_POOL_INVALID_SIZE;
	}
	dma_pool_heap->size = heap_size;
