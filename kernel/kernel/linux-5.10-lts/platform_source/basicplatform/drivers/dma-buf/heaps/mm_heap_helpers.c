...
static void mm_dma_heap_buffer_destroy(struct mm_heap_helper_buffer *buffer)
{
			   ...
			   struct dma_buf_attachment *attachment)
{
	...
	if (is_direct_reclaim_dmabuf(dmabuf)) {
		is_lock_recursive = mm_heap_mutex_lock_recursive(&buffer->lock);
		if (buffer->is_reclaim) {
			pr_err("%s buf has already released\n", __func__);
			mm_heap_mutex_unlock_recursive(&buffer->lock, is_lock_recursive);
			kfree(a);
			return -EINVAL;
		}
