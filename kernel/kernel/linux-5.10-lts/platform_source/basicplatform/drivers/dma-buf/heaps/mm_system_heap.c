...
static long system_get_pool_size_total_page_nums(struct dma_heap *heap)
{
	int i;
	long num_pages = 0;
...
static int dma_sys_pool_count(struct system_heap *heap)
{
	return system_get_pool_size_total_page_nums((struct dma_heap*)heap);
}
...
static void system_heap_show_pool(struct seq_file *s, struct dmabuf_page_pool **pools)
{
	...
	for (i = 0; i < SYS_NUM_ORDERS; i++) {
		...
		if (pool->count[POOL_HIGHPAGE] > 0)
			seq_printf(s, "order %u highmem pages %8d total %8lu kB\n",
					pool->order, pool->count[POOL_HIGHPAGE],
					pool->count[POOL_HIGHPAGE] << pool->order << (PAGE_SHIFT - KB_SHIFT));

		seq_printf(s, "order %u lowmem pages %8d total %8lu kB\n",
...
static void system_heap_add_pool_large_block_cnt(struct system_heap *system_heap, unsigned long len)
{
	int i = 0;

	if (len > DMABUF_LARGE_BUF_SIZE) {
		for (i = 0; i < SYS_NUM_ORDERS; i++) 
			atomic64_inc_return(&system_heap->pools[i]->large_size_cnt);
	}

	return;
}

static void system_heap_minus_pool_large_block_cnt(struct system_heap *system_heap, unsigned long len)
{
	int i = 0;

	if (len > DMABUF_LARGE_BUF_SIZE) {
		for (i = 0; i < SYS_NUM_ORDERS; i++) {
			if (atomic64_read(&system_heap->pools[i]->large_size_cnt) == 0) {
				pr_err("%s: large size cnt is already zero\n", __func__);
				continue;
			}
			atomic64_dec_return(&system_heap->pools[i]->large_size_cnt);
		}
	}
					       ...
					       unsigned long heap_flags)
{
	...
	i = 0;
	system_heap_add_pool_large_block_cnt(system_heap, len);
	while (size_remaining > 0) {
		...
		if (fatal_signal_pending(current)) {
			system_heap_minus_pool_large_block_cnt(system_heap, len);
			goto free_buffer;
		}
	...
	if (dma_buf_is_direct_reclaim(buffer->flags)) {
		...
		buffer->extid_table = kvzalloc(sizeof(int) * buffer->extid_num, GFP_KERNEL);
...
static long system_get_pool_size(struct dma_heap *heap)
{
	...
	num_pages = system_get_pool_size_total_page_nums(heap);
...
static struct kobject *dma_kobject;
...
static int system_heap_create(struct device_node *np)
{
...
#ifdef CONFIG_DMABUF_NPU_HUGE_POOL
	for (k = 0; k < SYS_NUM_ORDERS; k++) {
		system_heap->huge_pools[k] = dmabuf_page_pool_create(order_flags[k], orders[k]);
		if (!system_heap->huge_pools[k]) {
			pr_err("%s: page huge pool creation failed!\n", __func__);
			goto destroy_huge_pools;
		}
		system_heap->huge_pools[k]->reclaim_flag = DMABUF_PAGE_POOL_RECLAME_ALL;
	}
#endif
	...
	dma_kobject = kobject_create_and_add("dma_pool", kernel_kobj);
	if (!dma_kobject)
		pr_err("kobject_create_and_add fail!\n");

	err = sysfs_create_group(dma_kobject, &dma_pool_attr_group);
	if (err)
...
destroy_huge_pools:
	for (j = 0; j < k; j++)
		dmabuf_page_pool_destroy(system_heap->huge_pools[k]);
#endif

destroy_pools:
	for (j = 0; j < i; j++)
		dmabuf_page_pool_destroy(system_heap->pools[j]);
free_heap:
	kfree(system_heap);
