...
{
	return (struct mm_heap_helper_buffer *)dmabuf->priv;
}
...
void dma_ci_cache_by_pa(dma_addr_t addr, size_t size, enum dma_data_direction dir)
{
...
void dma_ci_cache_by_sg(struct sg_table *sgt, enum dma_data_direction dir)
{
	...
	for_each_sg(sgl, sg, nents, i) {
		...
		dma_ci_cache_by_pa(paddr, sg->length, dir);
	}
}
#endif
...
#else
#ifndef CONFIG_DISABLE_FLUSH_ALL_CPUS_CACHES
...
bool is_direct_reclaim_buffer(void *buf)
{
	...
	if (!is_system_heap(buffer->heap) ||
		dma_buf_is_secure(buffer->flags) ||
		dma_buf_is_mm_lb(buffer->flags) ||
		!dma_buf_is_direct_reclaim(buffer->flags))
		return false;
	else
		return true;
}
...
static int dmaheap_memtrack_show(struct seq_file *s, void *d)
{
	...
	seq_printf(s, "task_comm	pid		buf_size	magic		attach_comm\n");
	mutex_lock(&dmaheap_buffers_lock);
	for (n = rb_first(&dmaheap_buffers_rb); n; n = rb_next(n)) {
		...
		seq_printf(s, "%-16.s %-16d %-16zu %-16llu %-16.s\n", buffer->task_comm,
				buffer->pid, buffer->size, buffer->magic, buffer->dmabuf->attach_process_comm);
	}
	mutex_unlock(&dmaheap_buffers_lock);
...
void memtrack_show(void)
{
	...
	struct rb_node *n = NULL;
	char t_name[TASK_COMM_LEN] = { 0 };
	char t_type[TASK_COMM_LEN] = { 0 };
	pr_info("memtrack_show start\n");
	pr_info("fd               buf_size         magic            buf->tgid        buf->pid         \
		buf->task_comm       buf->attach_comm       buf_name       buf_type\n");
	mutex_lock(&dmaheap_buffers_lock);
	for (n = rb_first(&dmaheap_buffers_rb); n; n = rb_next(n)) {
		struct mm_heap_helper_buffer *buffer = rb_entry(n, struct mm_heap_helper_buffer, node);
		struct dma_buf *dmabuf = buffer->dmabuf;

		memset_s(t_name, TASK_COMM_LEN, 0, TASK_COMM_LEN);
		memset_s(t_type, TASK_COMM_LEN, 0, TASK_COMM_LEN);
		spin_lock(&dmabuf->name_lock);
		if (dmabuf->name)
			strlcpy(t_name, dmabuf->name, TASK_COMM_LEN);
		if (dmabuf->type)	
			strlcpy(t_type, dmabuf->type, TASK_COMM_LEN);
		spin_unlock(&dmabuf->name_lock);

		pr_info("%-16u %-16zu %-16llu %-16u %-16u %-.16s-%-.16s %-.16s %-.16s %-.16s\n",
				dmabuf->fd, buffer->size, buffer->magic, dmabuf->tgid,
				dmabuf->pid, dmabuf->exp_task_comm, dmabuf->exp_thread_comm, dmabuf->attach_process_comm,
				t_name, t_type);
					...
					struct file *f, unsigned int fd)
{
	...
	*args->total_dmaheap_alloc_size += dbuf->size;
	seq_printf(args->seq,
			"%16s %16d %16u %16zu %16llu %16d %16s %016lx %16s\n",
			tsk->comm, tsk->pid,
...
static int mm_dmaheap_debug_heap_show(struct seq_file *s, void *d)
{
	...
	for (n = rb_first(&dmaheap_buffers_rb); n; n = rb_next(n)) {
		...
		seq_printf(s, "%16s %10d %16zu %4d %8llu %16lld %16lld\n",
			   buffer->task_comm, buffer->pid,
			   buffer->size, buffer->vmap_cnt, buffer->magic, buffer->alloc_time_start, buffer->alloc_time_end);
	}
	mutex_unlock(&dmaheap_buffers_lock);
