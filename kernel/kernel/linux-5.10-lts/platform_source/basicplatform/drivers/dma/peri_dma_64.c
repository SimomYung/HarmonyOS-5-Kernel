...
#ifdef CONFIG_LIBLINUX_PERI_DMA_USE_HRTIMER
static void peri_dma_pending_config(struct peri_dma_dev *d)
{
	struct peri_dma_phy *p = NULL;
	struct peri_dma_chan *c = NULL;
	struct peri_dma_chan *cn = NULL;
...
static enum hrtimer_restart peri_dma_issue_timer_func(struct hrtimer *timer)
{
    struct peri_dma_dev *d = container_of(timer, struct peri_dma_dev, dma_timer);
    peri_dma_pending_config(d);
...
#ifdef CONFIG_LIBLINUX_PERI_DMA_USE_HRTIMER
	hrtimer_init(&d->dma_timer, CLOCK_MONOTONIC, HRTIMER_MODE_REL);
	d->dma_timer.function = peri_dma_issue_timer_func;
	dev_info(&op->dev, "initialized. hrtimer_init ok!\n");
#endif
