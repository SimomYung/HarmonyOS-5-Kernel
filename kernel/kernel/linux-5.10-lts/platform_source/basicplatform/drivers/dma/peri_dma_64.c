...
#include <linux/io.h>
#define DMA_REG_BASE_LEN 0x1000

static inline void peri_dma_write(u32 val, const volatile void __iomem *addr, u32 offset)
{
	struct peri_dma_write_info info = {0};

	info.offset[info.count] = ((u32)addr + offset) & 0xFFF;
	info.val[info.count] = val;
	info.count++;
	liblinux_pal_peri_dma_write(&info);
}
#else
static inline void peri_dma_write(u32 val, volatile void __iomem *addr, u32 offset)
{
...
#if defined CONFIG_SPI_V400_TIMEOUT_LOG
extern u32 g_spi_step;
extern u32 g_spi_rx_chan;
extern u32 g_spi_tx_chan;
#endif
	...
	struct peri_dma_dev *d, bool on)
{
	...
	if (on) {
		...
		val |= CCFG_EN;
		peri_dma_write(val, phy->base, CX_CONFIG);
	...
	struct peri_dma_dev *d)
{
	...
	info.val[info.count] = val;
	info.offset[info.count] = ((u32)d->base + INT_TC1_RAW) & 0xFFF;
    info.count++;
	info.val[info.count] = val;
	info.offset[info.count] = ((u32)d->base + INT_ERR1_RAW) & 0xFFF;
    info.count++;
	info.val[info.count] = val;
	info.offset[info.count] = ((u32)d->base + INT_ERR2_RAW) & 0xFFF;
    info.count++;
	liblinux_pal_peri_dma_write(&info);
}
...
static void peri_dma_set_desc(struct peri_dma_phy *phy, struct peri_dma_desc_hw *hw)
{
	...
	info.val[info.count] = GET_HIGH_32BIT(hw->lli);
	info.offset[info.count] = ((u32)phy->base + CX_LLI_H) & 0xFFF;
    info.count++;
	info.val[info.count] = GET_LOW_32BIT(hw->lli);
	info.offset[info.count] = ((u32)phy->base + CX_LLI_L) & 0xFFF;
    info.count++;
	info.val[info.count] = hw->count;
	info.offset[info.count] = ((u32)phy->base + CX_CNT) & 0xFFF;
    info.count++;
	info.val[info.count] = GET_HIGH_32BIT(hw->saddr);
	info.offset[info.count] = ((u32)phy->base + CX_SRC_H) & 0xFFF;
    info.count++;
	info.val[info.count] = GET_LOW_32BIT(hw->saddr);
	info.offset[info.count] = ((u32)phy->base + CX_SRC_L) & 0xFFF;
    info.count++;
	info.val[info.count] = GET_HIGH_32BIT(hw->daddr);
	info.offset[info.count] = ((u32)phy->base + CX_DST_H) & 0xFFF;
    info.count++;
	info.val[info.count] = GET_LOW_32BIT(hw->daddr);
	info.offset[info.count] = ((u32)phy->base + CX_DST_L) & 0xFFF;
    info.count++;
	info.val[info.count] = hw->config;
	info.offset[info.count] = ((u32)phy->base + CX_CONFIG) & 0xFFF;
    info.count++;
	liblinux_pal_peri_dma_write(&info);
}
#else
static void peri_dma_set_desc(struct peri_dma_phy *phy, struct peri_dma_desc_hw *hw)
{
	peri_dma_write(GET_HIGH_32BIT(hw->lli), phy->base, CX_LLI_H);
	peri_dma_write(GET_LOW_32BIT(hw->lli), phy->base, CX_LLI_L);
	peri_dma_write(hw->count, phy->base, CX_CNT);
	peri_dma_write(GET_HIGH_32BIT(hw->saddr), phy->base, CX_SRC_H);
	peri_dma_write(GET_LOW_32BIT(hw->saddr), phy->base, CX_SRC_L);
	peri_dma_write(GET_HIGH_32BIT(hw->daddr), phy->base, CX_DST_H);
	peri_dma_write(GET_LOW_32BIT(hw->daddr), phy->base, CX_DST_L);
	peri_dma_write(hw->config, phy->base, CX_CONFIG);
}
	...
	struct peri_dma_dev *d)
{
	...
	/* phy->idx must be 0~15 */
	val = (u32)0x1 << phy->idx;
	peri_dma_write(val, d->base, INT_TC1_RAW);
	peri_dma_write(val, d->base, INT_ERR1_RAW);
	peri_dma_write(val, d->base, INT_ERR2_RAW);
}
#endif
...
static void peri_dma_enable_dma(struct peri_dma_dev *d, bool on)
{
	...
	if (on) {
		/* set same priority */
		info.val[info.count] = 0x0;
		info.offset[info.count] = ((u32)d->base + CH_PRI) & 0xFFF;
        info.count++;
		/* unmask irq */
		info.val[info.count] = d->dma_used_chans;
		info.offset[info.count] = ((u32)d->base + INT_TC1_MASK) & 0xFFF;
        info.count++;
		info.val[info.count] = d->dma_used_chans;
		info.offset[info.count] = ((u32)d->base + INT_ERR1_MASK) & 0xFFF;
        info.count++;
		info.val[info.count] = d->dma_used_chans;
		info.offset[info.count] = ((u32)d->base + INT_ERR2_MASK) & 0xFFF;
        info.count++;
		liblinux_pal_peri_dma_write(&info);
	} else {
		/* mask irq */
...
static void peri_dma_enable_dma(struct peri_dma_dev *d, bool on)
{
	if (on) {
		/* set same priority */
		peri_dma_write(0x0, d->base, CH_PRI);

		/* unmask irq */
		peri_dma_write(d->dma_used_chans, d->base, INT_TC1_MASK);
		peri_dma_write(d->dma_used_chans, d->base, INT_ERR1_MASK);
		peri_dma_write(d->dma_used_chans, d->base, INT_ERR2_MASK);
	} else {
		/* mask irq */
		peri_dma_write(0x0, d->base, INT_TC1_MASK);
		peri_dma_write(0x0, d->base, INT_ERR1_MASK);
		peri_dma_write(0x0, d->base, INT_ERR2_MASK);
	}
}
#endif
...
#ifdef CONFIG_SPI_V400_TIMEOUT_LOG
	spin_lock_irqsave(&d->lock, flags);
#endif
	p = &d->phy[i];
	c = p->vchan;
	if (c) {
#ifndef CONFIG_SPI_V400_TIMEOUT_LOG
...
#ifdef CONFIG_SPI_V400_TIMEOUT_LOG
			if ((c->vc.chan.chan_id == 12) || (c->vc.chan.chan_id == 13))
				g_spi_step = 15;
#endif
		}
		p->ds_done = p->ds_run;
...
#ifdef CONFIG_LIBLINUX_PERI_DMA
static void peri_dma_clr_int(struct peri_dma_dev *d, u32 tc1, u32 err1, u32 err2)
{
	struct peri_dma_write_info info = {0};

	info.val[info.count] = tc1;
	info.offset[info.count] = ((u32)d->base + INT_TC1_RAW) & 0xFFF;
    info.count++;
	info.val[info.count] = err1;
	info.offset[info.count] = ((u32)d->base + INT_ERR1_RAW) & 0xFFF;
    info.count++;
	info.val[info.count] = err2;
	info.offset[info.count] = ((u32)d->base + INT_ERR2_RAW) & 0xFFF;
    info.count++;
	liblinux_pal_peri_dma_write(&info);  
}
...
static void peri_dma_clr_int(struct peri_dma_dev *d, u32 tc1, u32 err1, u32 err2)
{
	peri_dma_write(tc1, d->base, INT_TC1_RAW);
	peri_dma_write(err1, d->base, INT_ERR1_RAW);
	peri_dma_write(err2, d->base, INT_ERR2_RAW); 
}
#endif
...
static irqreturn_t peri_dma_int_handler(int irq, void *dev_id)
{
	...
	if (d->dma_irq_flag == 0) {
		peri_dma_clr_int(d, tc1, err1, err2);
	...
	peri_dma_clr_int(d, tc1_irq, err1_irq, err2_irq);
...
static int peri_dma_start_txd(struct peri_dma_chan *c)
{
	...
	if (!c->phy) {
		dev_err(d->slave.dev, "%s: dma chan phy is NULL\n", __func__);
		return -ENODEV;
	}
	...
	if (vd) {
		...
		c->phy->ds_done = NULL;
#if defined CONFIG_SPI_V400_TIMEOUT_LOG
		if (c->vc.chan.chan_id == 12) {
			g_spi_rx_chan = c->phy->idx;
			g_spi_step = 14;
		}
		if (c->vc.chan.chan_id == 13) {
			g_spi_tx_chan = c->phy->idx;
			g_spi_step = 14;
		}
#endif
		/* start dma */
	...
	if (vchan_issue_pending(&c->vc)) {
		...
		if (!c->phy)
			if (list_empty(&c->node)) {
				...
				/* check in tasklet */
#if defined CONFIG_SPI_V400_TIMEOUT_LOG
				if ((chan->chan_id == 12) || (chan->chan_id == 13))
					g_spi_step = 12;
#endif
				tasklet_schedule(&d->task);
		...
		dev_err(d->slave.dev, "vchan %pK: nothing to issue\n", &c->vc);
	}
...
void show_dma64_reg(struct dma_chan *chan)
{
	...
	for (i = d->dma_min_chan; i < d->dma_channels; i++) {
		...
		dev_info(d->slave.dev,
			...
			readl(p->base + CX_CUR_CNT - i * CURRENT_COUNT_OFFSET));
	}
...
static void peri_set_dma_ctrl_pipeline(struct peri_dma_dev *d)
{
	...
	if (d->dma_pipeline_en == 0) {
		...
		peri_dma_write(dma_ctrl_cfg, d->base, DMA_CTRL);
	}
}
...
#ifdef CONFIG_LIBLINUX_PERI_DMA
	d->base = __ioremap(iores->start, DMA_REG_BASE_LEN, PAGE_KERNEL_RO, MODE_DEVICE);
#else
