	...
	struct pps_apdo_cap *apdo_caps, int num, struct pps_pdo_raw *pdo_cap)
{
	...
	struct tcpm_power_cap pdo_raw = {0};
	uint8_t i;
	...
	ret =  hisi_tcpm_get_source_cap(typec->tcpc_dev, &pdo_raw);
	/* check */
	if (!ret) {
		for (i = 0; i < TCPM_PDO_MAX_SIZE; i++)
			pdo_cap->data[i] = pdo_raw.pdos[i];
	}
...
static bool hisi_usb_support_9v_src_cap(void *dev_data)
{
	...
	uint32_t src_cap_9v_mask;
	uint32_t src_cap_9v;
	src_cap_9v_mask = 0xFFC00;
	src_cap_9v = 0x2D000;
