...
#ifdef CONFIG_KIRIN_TCPC_VBUS_IRQ_ASSIST
	bool vbus_irq;
	int vbus_status;
#endif
	int poll_cc_status;
...
static int hisi_tcpc_read8(struct tcpc_device *tcpc, u8 reg)
{
	...
	return hisi_tcpc_regmap_read8(chip->regmap, reg);
}
...
static int hisi_tcpc_write8(struct tcpc_device *tcpc, u8 reg, u8 data)
{
	...
	return hisi_tcpc_regmap_write8(chip->regmap, reg, data);
}
...
static void hisi_tcpc_da_vbus_en_status(struct tcpc_device *tcpc)
{
	...
	if (chip->pd_dbg_rdata_cfg != PD_DBG_RDATA_VBUS_STATUS) {
		(void)hisi_tcpc_regmap_write8(chip->regmap, REG_PD_DBG_RDATA_CFG,
				PD_DBG_RDATA_VBUS_STATUS);
		chip->pd_dbg_rdata_cfg = PD_DBG_RDATA_VBUS_STATUS;
	}

	ret = hisi_tcpc_regmap_read8(chip->regmap, REG_PD_DBG_RDATA);
	if (ret < 0) {
		E("hisi_tcpc_regmap_read8 error return %d\n", ret);
		return;
	}
...
static void bist_mode_2_stop_work_fn(struct work_struct *work)
{
	...
	(void)hisi_tcpc_regmap_write8(chip->regmap, REG_PD_VDM_ENABLE, 0);
	(void)hisi_tcpc_regmap_write8(chip->regmap, REG_PD_VDM_ENABLE, 1);
}
#endif

static void hisi_tcpc_enable_vbus(struct regmap *regmap)
{
	uint8_t reg;
	int ret;

	/* dis force disch */
	ret = hisi_tcpc_regmap_read8(regmap, TCPC_V10_REG_POWER_CTRL);
	if (ret < 0) {
		E("hisi_tcpc_regmap_read8 error return %d\n", ret);
		return;
	}
	...
	(void)hisi_tcpc_regmap_write8(regmap, TCPC_V10_REG_POWER_CTRL, reg);
}

static void hisi_tcpc_disable_vbus(struct regmap *regmap)
{
	uint8_t reg;
	int ret;
...
static void hisi_tcpc_ext_otg_valid(struct hisi_tcpc_chip *chip, int valid, unsigned int msecs)
{
	...
	if (chip->chip_version != TCPC_CHIP_VERSION_V600 && chip->otg_vbst_ori) {
		msleep(msecs);
		(void)hisi_tcpc_regmap_write8(chip->regmap, PD_EXT_OTG_EN_REG, val);
	}
}
		...
		int mv, int ma)
{
	...
	if (type == TCP_VBUS_CTRL_REMOVE) {
		hisi_tcpc_disable_vbus(chip->regmap);
	...
	if (type == TCP_VBUS_CTRL_TYPEC) {
		if (mv != TCPC_VBUS_SOURCE_0V)
			hisi_tcpc_enable_vbus(chip->regmap);
		else
			hisi_tcpc_disable_vbus(chip->regmap);
...
static void hisi_tcpc_set_vbus_detect(struct tcpc_device *tcpc, bool enable)
{
	...
	ret = hisi_tcpc_regmap_read8(chip->regmap,
	...
	(void)hisi_tcpc_regmap_write8(chip->regmap, TCPC_V10_REG_POWER_CTRL, reg);
}

static int __hisi_tcpc_pd_fsm_reset(struct regmap *regmap)
{
	uint8_t reg;
	int ret;

	ret = hisi_tcpc_regmap_read8(regmap, REG_PD_VDM_CFG_1);
	if (ret < 0) {
		E("hisi_tcpc_regmap_read8 error return %d\n", ret);
		return ret;
	}
	...
	reg = (uint8_t)ret;
	reg |= PD_TC_ALL_RESET;
	(void)hisi_tcpc_regmap_write8(regmap, REG_PD_VDM_CFG_1, reg);
...
/* PD_RX_PHY_SOFT_RESET & PD_TX_PHY_SOFT_RESET & PD_FSM_RESET & TC_FSM_RESET */
...
#endif
	(void)__hisi_tcpc_pd_fsm_reset(chip->regmap);
}

/* PD_FSM_RESET */
static void hisi_tcpc_only_pd_fsm_reset(struct tcpc_device *tcpc)
{
	struct hisi_tcpc_chip *chip = hisi_tcpc_get_dev_data(tcpc);

	(void)hisi_tcpc_regmap_update_bits(chip->regmap, REG_PD_VDM_CFG_1, PD_FSM_RESET, PD_FSM_RESET_VAL);
	(void)hisi_tcpc_regmap_update_bits(chip->regmap, REG_PD_VDM_CFG_1, PD_FSM_RESET, PD_FSM_UNRESET_VAL);
}
...
static void hisi_tcpc_pd_fsm_state(struct tcpc_device *tcpc)
{
	...
	if (chip->pd_dbg_rdata_cfg != PD_DBG_RDATA_MACHINE_STATUS) {
		(void)hisi_tcpc_regmap_write8(chip->regmap,
...
static void hisi_tcpc_init_alert_mask(struct tcpc_device *tcpc)
{
	...
	(void)hisi_tcpc_regmap_write16(chip->regmap,
...
static int hisi_tcpc_tcpc_init(struct tcpc_device *tcpc, bool sw_reset)
{
	...
	/* Except V600, add pd_en_look4con_alert */
	if (chip->chip_version != TCPC_CHIP_VERSION_V600)
		(void)hisi_tcpc_write8(tcpc, TCPC_V10_REG_TCPC_CTRL,
			TCPC_V10_REG_TCPC_CTRL_EN_LOOK4CON_ALERT);
...
static void hisi_tcpc_fr_swap(struct tcpc_device *tcpc, bool enable)
{
	...
	if (!hisi_tcpc_support_reverse_sc(tcpc) && chip->otg_vbst_ori == 0) {
		D("Inside Otg\n");
		return;
	}
	...
	/* alert mask: fr_swap */
	ret = hisi_tcpc_regmap_read8(chip->regmap, TCPC_V10_REG_ALERT_MASK);
	if (ret < 0) {
		E("i2c read8 error ret %d\n", ret);
		return;
	}
	...
	/* Switch */
	ret = hisi_tcpc_regmap_read8(chip->regmap, REG_PD_VDM_CFG_0);
	if (ret < 0) {
		E("i2c read8 error ret %d\n", ret);
		return;
	}
	...
	if (enable)
		...
		rdata &= ~PD_DA_FRS_ENABLE;
	(void)hisi_tcpc_regmap_write8(chip->regmap, REG_PD_VDM_CFG_0, rdata);

	/* FR_Swap Detect en */
	ret = hisi_tcpc_regmap_read8(chip->regmap, chip->reg_info.cfg_reg_1);
	if (ret < 0) {
		E("i2c read8 error ret %d\n", ret);
		return;
	}
	...
	if (enable)
		...
		rdata &= ~PD_FRS_DETECT;
	(void)hisi_tcpc_regmap_write8(chip->regmap, chip->reg_info.cfg_reg_1, rdata);
}
...
static void hisi_tcpc_sc_buck_ctrl(struct tcpc_device *tcpc, bool en)
{
	...
	ret = hisi_tcpc_regmap_read8(chip->regmap, chip->reg_info.sc_buck_en);
	if (ret < 0) {
		E("i2c read8 error ret %d\n", ret);
		return;
	}
	...
	if (en)
		...
		rdata |= BIT_SC_BUCK_EN;
	(void)hisi_tcpc_regmap_write8(chip->regmap, chip->reg_info.sc_buck_en, rdata);
}
...
static int hisi_tcpc_get_alert_status(struct tcpc_device *tcpc, uint32_t *alert)
{
	...
	ret = hisi_tcpc_regmap_read16(chip->regmap, TCPC_V10_REG_ALERT);
	if (ret < 0) {
...
static void enable_auto_discharge_disconnect(struct hisi_tcpc_chip *chip)
{
	...
	ret = hisi_tcpc_regmap_read8(chip->regmap, TCPC_V10_REG_POWER_CTRL);
	if (ret < 0) {
		E("hisi_tcpc_regmap_read8 error ret %d\n", ret);
		return;
	}
...
static int hisi_tcpc_get_version(struct tcpc_device *tcpc, uint8_t *version, uint8_t *revision)
{
	int ret;
	struct hisi_tcpc_chip *chip = hisi_tcpc_get_dev_data(tcpc);
	...
	*revision = 0;
	...
	ret = hisi_tcpc_regmap_read8(chip->regmap, TCPC_V10_REG_PD_VER);
	if (ret < 0)
		return ret;

	*version = (uint8_t)(unsigned int)ret;

	ret = hisi_tcpc_regmap_read8(chip->regmap, TCPC_V10_REG_PD_REVER);
	if (ret < 0)
		return ret;

	*revision = (uint8_t)(unsigned int)ret;

	if (chip->chip_version < TCPC_CHIP_VERSION_V800) {
		*version = 0x10;
		*revision = 0x20;
	}
...
int hisi_tcpc_set_dp_aux(struct tcpc_device *tcpc, uint8_t polarity)
{
	...
	if (polarity == 0 || polarity == 1) {
		...
		(void)hisi_tcpc_regmap_write8(chip->regmap, chip->reg_info.aux_sbu_en, polarity ? 1 : 0);
		(void)hisi_tcpc_regmap_write8(chip->regmap, chip->reg_info.jtag_sbu_en, polarity ? 0 : 1);
	} else  {
		I("disconnect\n");
		(void)hisi_tcpc_regmap_write8(chip->regmap, chip->reg_info.aux_sbu_en, 0);
		(void)hisi_tcpc_regmap_write8(chip->regmap, chip->reg_info.jtag_sbu_en, 0);
	}

	D("aux_sbu, readback 0x%x\n", hisi_tcpc_regmap_read8(chip->regmap,chip->reg_info.aux_sbu_en));
	D("jtag_sbu, readback 0x%x\n", hisi_tcpc_regmap_read8(chip->regmap,chip->reg_info.jtag_sbu_en));
						  ...
						  bool en)
{
	...
	int ret = hisi_tcpc_regmap_read8(chip->regmap, chip->reg_info.cdr_cfg_0);
	if (ret < 0) {
		E("hisi_tcpc_regmap_read8 error ret %d\n", ret);
		return;
	}
	...
	(void)hisi_tcpc_regmap_write8(chip->regmap, chip->reg_info.cdr_cfg_0, ret);
}
...
{
	uint8_t msg_header =
		TCPC_V10_REG_MSG_HDR_INFO_SET(((unsigned int)data_role) & 0x1,
			((unsigned int)power_role) & 0x1, rev & 0x3);
...
bool hisi_tcpc_support_reverse_sc(struct tcpc_device *tcpc)
{
	if (!tcpc)
		return false;

	#ifdef CONFIG_REVERSE_CHARGE
	D("cur_support_reverse_sc: %d\n", tcpc->support_reverse_sc);
	return tcpc->support_reverse_sc;
	#else
	return false;
	#endif
}
...
static int hisi_tcpc_set_rx_enable(struct tcpc_device *tcpc, uint8_t enable)
{
	...
	ret = hisi_tcpc_regmap_read8(chip->regmap, TCPC_V10_REG_RX_DETECT);
	if (ret < 0) {
		E("hisi_tcpc_regmap_read8 error ret %d\n", ret);
		return ret;
	}
	...
	reg = enable;
	ret = hisi_tcpc_regmap_write8(chip->regmap, TCPC_V10_REG_RX_DETECT, reg);
			...
			uint16_t *msg_head, enum tcpm_transmit_type *frame_type)
{
	...
	rv = hisi_tcpc_block_read(chip->regmap, TCPC_V10_REG_RX_BYTE_CNT, 8, buf); /* 8: filp */
	if (rv) {
	...
	*msg_head = *(uint16_t *)&buf[2]; /* 3rid&4th Byte: Header */
	...
	I("cnt: %d, type: 0x%x, header: 0x%x\n", cnt, type, *msg_head);

	if (cnt > 7) { /* 7: MSG_HDR */
		cnt -= 7; /* 7: MSG_HDR */
		/* First DW payload Read with Header: the last 4 of 8Byte */
		rv = hisi_tcpc_block_read(chip->regmap, TCPC_V10_REG_RX_DATA + 4, cnt,
				(uint8_t *)(payload + 1));
		if (rv) {
			E("hisi_tcpc_block_read error rv %d\n", rv);
			return rv;
	...
	enum tcpm_transmit_type type, uint16_t header, const uint32_t *data)
{
	...
	if (type < TCPC_TX_HARD_RESET) {
		...
		rv = hisi_tcpc_block_write(chip->regmap, TCPC_V10_REG_TX_BYTE_CNT,
	...
	(void)hisi_tcpc_write8(tcpc, TCPC_V10_REG_TRANSMIT,
			TCPC_V10_REG_TRANSMIT_SET(tcpc->retry_cnt, (unsigned int)type));
...
{
	(void)hisi_tcpc_write8(tcpc, TCPC_V10_REG_TRANSMIT,
			TCPC_V10_REG_TRANSMIT_SET(tcpc->retry_cnt, TCPC_TX_SOP));
	return 0;
}
#endif

#endif /* CONFIG_KIRIN_POWER_DELIVERY_SUPPORT */

static struct tcpc_ops hisi_tcpc_ops = {
	.init = hisi_tcpc_tcpc_init,
	.get_version = hisi_tcpc_get_version,
	...
	.reset_only_pd_fsm = hisi_tcpc_only_pd_fsm_reset,
...
{
	int ret = hisi_tcpc_regmap_read8(chip->regmap, chip->reg_info.dbg_cfg_1);
	uint8_t reg_val;
	...
	if (ret < 0) {
		...
		(void)hisi_tcpc_regmap_write8(chip->regmap, chip->reg_info.dbg_cfg_1, reg_val);
	}

	ret = hisi_tcpc_regmap_read8(chip->regmap, chip->reg_info.dbg_cfg_1);
	if (ret < 0)
		 E("hisi_tcpc_regmap_read8 error again ret %d\n", ret);
}
...
static void hisi_tcpc_cc_abnormal_irq_handler(struct hisi_tcpc_chip *chip)
{
	...
	ret = hisi_tcpc_regmap_read8(chip->regmap, chip->reg_info.abnormal_irq_sts);
	if (ret < 0) {
		E("hisi_tcpc_regmap_read8 error ret %d\n", ret);
		return;
	}
	...
	if (reg_val & chip->reg_info.cc_ocp_irq) {
		E("CC OCP\n");
	}
 
	if (reg_val & chip->reg_info.cc_uvp_irq) {
		E("CC UVP\n");
	}
 
	if (reg_val & chip->reg_info.sbu1_ovp_irq) {
		E("sbu1_ovp_irq\n");
	}
 
	if (reg_val & chip->reg_info.sbu2_ovp_irq) {
		E("sbu2_ovp_irq\n");
	}

	(void)hisi_tcpc_regmap_write8(chip->regmap, chip->reg_info.abnormal_irq_sts,
		   reg_val);
}
...
static void hisi_tcpc_irq_work_handler(struct kthread_work *work)
{
	...
	if ((desc->irq_data.chip->irq_unmask != NULL) && (chip->chip_version > TCPC_CHIP_VERSION_V700))
...
static irqreturn_t hisi_tcpc_intr_handler(int irq, void *data)
{
	...
	if ((desc->irq_data.chip->irq_mask != NULL) && (chip->chip_version > TCPC_CHIP_VERSION_V700))
...
static int hisi_tcpc_get_irq(struct hisi_tcpc_chip *chip)
{
	...
	return of_irq_get_byname(chip->dev->of_node, "tcpc_irq");
}
...
static void __hisi_tcpc_init_abnormal_irq(struct hisi_tcpc_chip *chip)
{
	...
	ret = hisi_tcpc_regmap_read8(chip->regmap, chip->reg_info.abnormal_irq_msk);
	if (ret < 0) {
		E("hisi_tcpc_regmap_read8 error ret %d\n", ret);
		return;
	}
	...
	(void)hisi_tcpc_regmap_write8(chip->regmap, chip->reg_info.abnormal_irq_msk, reg_val);
}
...
static void __hisi_tcpc_init_alert(struct hisi_tcpc_chip *chip)
{
	...
	(void)hisi_tcpc_regmap_write16(chip->regmap, TCPC_V10_REG_ALERT_MASK, 0);
	(void)hisi_tcpc_regmap_write16(chip->regmap, TCPC_V10_REG_ALERT, 0xffff);

	ret = hisi_tcpc_regmap_read8(chip->regmap, chip->reg_info.irq_gbl_msk);
	if (ret < 0) {
		E("hisi_tcpc_regmap_read8 error ret %d\n", ret);
		return;
	}
	...
	if (reg_u8 & chip->reg_info.irq_msk_pd_bit) {
		...
		(void)hisi_tcpc_regmap_write8(chip->regmap, chip->reg_info.irq_gbl_msk, reg_u8);
	}
}
...
static void hisi_tcpc_get_otg_vbst_type(struct hisi_tcpc_chip *chip)
{
	...
	/* Default: Inner OTG */
	if (chip->chip_version != TCPC_CHIP_VERSION_V600 && chip->otg_vbst_ori)
		(void)hisi_tcpc_regmap_write8(chip->regmap, PD_EXT_OTG_SEL_REG, EXT_OTG_BIT);
}
		...
		struct device *dev)
{
	...
	if (of_property_read_u32(np, "support_reverse_sc", &val) >= 0) {
		if (val) 
			chip->tcpc->support_reverse_sc = true;
		else 
			chip->tcpc->support_reverse_sc = false;
	}
	D("support_reverse_sc : %d\n", chip->tcpc->support_reverse_sc);
#endif
...
{
	int ret;
	struct device_node *device = chip->dev->parent->parent->of_node;

	ret = hisi_tcpc_regmap_read16(chip->regmap, TCPC_V10_REG_VID);
	if (ret < 0) {
		dev_err(chip->dev, "read chip ID fail\n");
		return -EIO;
	}
	...
	if (of_property_read_u32(device, "chip_version", &chip->chip_version)) {
		I("%s Chip_version fail, default Chip V600", device->name);
		chip->chip_version = TCPC_CHIP_VERSION_V600;
	}

	if (hisi_tcpc_get_reg_info(chip->dev->of_node, &(chip->reg_info))) {
		E("Get reg info fail\n");
		return -1;
	}
...
static int hisi_tcpc_probe(struct platform_device *pdev)
{
	...
	regmap = dev_get_regmap(pdev->dev.parent->parent, NULL);
	if (!regmap) {
		E("no regmap\n");
		ret = -ENODEV;
		goto fail_free;
	...
	if (ret < 0) {
		dev_err(chip->dev, "hisi tcpc dev init fail\n");
		goto fail_free;
	}
...
{
	struct hisi_tcpc_chip *chip = dev_get_drvdata(dev);
...
static int hisi_tcpc_restore(struct device *dev)
{
	struct hisi_tcpc_chip *chip = dev_get_drvdata(dev);
	int ret;
	uint8_t reg_u8;
	...
	(void)hisi_tcpc_resume(dev);

	ret = hisi_tcpc_regmap_read8(chip->regmap, TCPC_V10_REG_ROLE_CTRL);
	if (ret < 0) {
		E("hisi_tcpc_i2c_read8 error ret %d\n", ret);
		return 0;
	}
	...
	reg_u8 = (uint8_t)(unsigned int)ret;
	if ((reg_u8 & TCPC_V10_REG_ROLE_CTRL_DRP) == 0) {
		I("EXIT HIBERNATION\n");
		hisi_tcpc_set_cc(chip->tcpc, TYPEC_CC_DRP);
	}
...
#if defined(CONFIG_HIBERNATION)
	.suspend = hisi_tcpc_suspend,
	.resume = hisi_tcpc_resume,
	.poweroff = hisi_tcpc_suspend,
	.restore = hisi_tcpc_restore,
#else
...
#ifdef CONFIG_REVERSE_CHARGE
	void set_desc_role_def(struct tcpc_device *tcpc_dev, uint8_t tar_role)
{
	struct tcpc_desc *desc_role_def = &tcpc_dev->desc;
