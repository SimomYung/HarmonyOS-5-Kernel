...
#define PD_HEADER_SOP(msg_type, pd_rev, prole, drole, id, cnt) \
		((msg_type & 0x1F) | ((pd_rev & 0x3) << 6) | \
		 ((drole & 0x1) << 5) | ((prole & 0x1) << 8) | \
		 ((id & 0x7) << 9) | ((cnt & 0x7) << 12))
...
#define VDO_S(svid, cmd_type, cmd, obj, ver)	\
	VDO((svid), 1, VDO_CMDT(cmd_type) | VDO_OPOS(obj) | VDO_SVDM_VER(ver) | VDO_SVDM_VER_MIN(ver) | (cmd))
