...
void set_desc_role_def(struct tcpc_device *tcpc, uint8_t tar_role);
void hisi_rchg_timeout_alert(struct tcpc_device *tcpc);
#endif

int tcpci_get_cc_res(struct tcpc_device *tcpc);
...
{
	if (tcpc->pd_port.vdf_flags & VDF_FLAGS_PD_31_CERTIFICATION_MASK)
...
static inline int tcpci_get_ext_status(struct tcpc_device *tcpc,
		uint8_t *ext_status)
{
	if (tcpc->ops->get_ext_status)
		return tcpc->ops->get_ext_status(tcpc, ext_status);
	*ext_status = 0;
	return 0;
}
...
static inline int tcpci_get_pd_version(struct tcpc_device *tcpc,
		uint8_t *version, uint8_t *revision)
{
	if (tcpc->ops->get_version)
		return tcpc->ops->get_version(tcpc, version, revision);
	return 0;
}
...
static inline void tcpci_reset_only_pd_fsm(struct tcpc_device *tcpc)
{
	if (tcpc->ops->reset_only_pd_fsm)
		tcpc->ops->reset_only_pd_fsm(tcpc);
}
...
static inline void tcpci_mutex_lock(struct tcpc_device *tcpc_dev)
{
	mutex_lock(&tcpc_dev->access_lock);
}

static inline void tcpci_mutex_unlock(struct tcpc_device *tcpc_dev)
{
	mutex_unlock(&tcpc_dev->access_lock);
}

static inline bool pd_check_rev30(pd_port_t *pd_port)
{
...
static inline int tcpci_set_msg_header(
		struct tcpc_device *tcpc, int power_role, int data_role, uint8_t rev)
{
	return tcpc->ops->set_msg_header(tcpc, power_role, data_role, rev);
}
...
static inline int tcpci_notify_status(struct tcpc_device *tcpc, struct pd_status *sdb)
{
	struct tcp_notify tcp_noti;

	tcp_noti.status_msg.sdb= sdb;
	srcu_notifier_call_chain(&tcpc->evt_nh, TCP_NOTIFY_STATUS, &tcp_noti);
	return 0;
}

static inline int tcpci_notify_request_bat_info(
