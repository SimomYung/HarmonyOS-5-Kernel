...
struct tcpc_ops {
	...
	int (*get_version)(struct tcpc_device *tcpc, uint8_t *vision, uint8_t *revision);
	...
	void (*reset_only_pd_fsm)(struct tcpc_device *tcpc);
...
static inline uint8_t hisi_pd_get_rev(pd_port_t *pd_port, uint8_t sop_type)
{
	uint8_t pd_rev = PD_REV30;
	if (sop_type == TCPC_TX_SOP) {
		pd_rev = pd_port->pd_revision[0];
	} else {
		if (pd_port->cable_communicated)
	...
	I("pd rev: 0x%x, retry_cnt: 0x%x\n", pd_rev, pd_port->tcpc_dev->retry_cnt);
