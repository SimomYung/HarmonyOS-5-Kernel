...
#define SRC_CAP_HIGH_CUR              3000
#define SRC_CAP_LOW_CUR               2000
...
int hisi_pd_dpm_send_source_caps(pd_port_t *pd_port)
{
	...
	if ((pd_port->ext_src_cap.otg_vol == SRC_CAP_HIGH_VOL &&
		pd_port->ext_src_cap.otg_cur == SRC_CAP_LOW_CUR) ||
		hisi_usb_is_reverse_charge_enable()) {
			src_cap1->nr = SRC_PDO_TWO;
	} else if (pd_port->ext_src_cap.otg_vol == SRC_CAP_HIGH_VOL &&
	...
	if (src_cap1->nr == SRC_PDO_THREE) {
			src_cap1->pdos[1] = src_cap1->pdos[2];
			src_cap1->nr = SRC_PDO_TWO;
	}
#endif
		...
		struct dpm_rdo_info_t *req_info)
{
	...
	hisi_usb_typec_get_raw_pdo(src_cap->pdos, src_cap->nr);
	return max_uw > 0;
}
...
void hisi_pd_dpm_snk_standby_power(pd_port_t *pd_port, pd_event_t *pd_event)
{
	...
	/*
	 * pSnkStdby : Maximum power consumption while in Sink Standby. (25mW)
	 * I1 = (pSnkStdby/VBUS)
	...
	uint8_t type;
	int ma = -1;
	int standby_curr = 25000 / max(pd_port->request_v,
	...
	if (pd_port->request_v_new > pd_port->request_v) {
		...
		/* Case9 v_new == v_old and increasing current, for TEST.PD.PS.SNK.01 */
		ma = standby_curr;
		type = TCP_VBUS_CTRL_STANDBY;
	}

	if (ma >= 0) {
		I("set sink vbus %dV  %dmA\n", pd_port->request_v, ma);
...
#ifdef CONFIG_KIRIN_PD_REV30_PPS_SINK
void hisi_pd_dpm_pps_request_heart_beat(pd_port_t *pd_port)
{
	if(!pd_port)
		return ;
	if(pd_port->request_apdo)
...
void hisi_pd_dpm_src_evaluate_request(pd_port_t *pd_port, pd_event_t *pd_event)
{
	...
	if (accept_request) {
#ifdef CONFIG_REVERSE_CHARGE
			if (hisi_tcpc_support_reverse_sc(pd_port->tcpc_dev)) {
...
{
	size_t size = sizeof(uint32_t) * VDO_MAX_SIZE;
	errno_t errno;
...
{
	uint8_t *cnt = &pd_port->pd_response_id30_cnt;
	...
	return hisi_pd_reply_svdm_request(pd_port, pd_event, CMDT_RSP_ACK,
			rev30 ? pd_port->id_vdo_nr : PD_RESPONSE_ID30_COUNT, pd_port->id_vdos);  // TEST.PD.VDM.SNK.01
}
...
#endif /* CONFIG_KIRIN_USB_PD_UVDM_SUPPORT */
...
#ifdef CONFIG_KIRIN_PD_REV30
void hisi_pd_dpm_inform_sink_cap_ext(pd_port_t *pd_port, pd_event_t *pd_event)
{
	struct pd_sink_cap_ext *skedb;

	if (dpm_check_ext_msg_event(pd_event, PD_EXT_SINK_CAP_EXT)) {
		skedb = pd_get_msg_data_payload(pd_port);
		if (skedb == NULL)
			return;
		I("vid=0x%04x, pid=0x%04x\n", skedb->vid, skedb->pid);
		I("fw_ver=0x%02x, hw_ver=0x%02x\n",
			skedb->fw_ver, skedb->hw_ver);
		I("skedb_ver=0x%02x\n", skedb->skedb_ver);
		I("min_pdp=%d, max_pdp=%d\n",
			skedb->min_pdp, skedb->max_pdp);
	}
}

static struct pd_sink_cap_ext g_snk_cap_ext = {0};
int hisi_pd_dpm_send_sink_cap_ext(pd_port_t *pd_port)
{
	g_snk_cap_ext.vid = 0x12D1;
	g_snk_cap_ext.pid = 0x6526;
	g_snk_cap_ext.xid = 0x0;
	g_snk_cap_ext.fw_ver = 0x30;
	g_snk_cap_ext.hw_ver = 0x70;
	g_snk_cap_ext.skedb_ver = 0x1;
	g_snk_cap_ext.load_step = 0x1;

	g_snk_cap_ext.battery_info = 0x1;
	g_snk_cap_ext.min_pdp = 0xA;
	g_snk_cap_ext.max_pdp = 0x50;
	g_snk_cap_ext.epr_oper_pdp = 0x10;
	g_snk_cap_ext.sink_modes = 0xB;

	return hisi_pd_send_ext_msg(pd_port, TCPC_TX_SOP, PD_EXT_SINK_CAP_EXT,
		false, 0, PD_SKEDB_SIZE, (uint8_t *)&pd_port->snk_cap_ext);
}

#ifdef CONFIG_KIRIN_PD_REV30_BAT_CAP_LOCAL
static struct pd_battery_capabilities c_invalid_bcdb = {
	0xFFFF, 0x0000, 0x0000,
	0x0000, 0x0001
};

int hisi_pd_dpm_send_battery_cap(pd_port_t *pd_port)
{
	struct pd_battery_capabilities *bcdb = &c_invalid_bcdb;
	struct pd_battery_info *bat_info;
	struct pd_get_battery_capabilities *gbcdb = pd_get_msg_data_payload(pd_port);

	if (gbcdb) {
		bat_info = hisi_pd_get_battery_info(pd_port, gbcdb->bat_cap_ref);
		if (bat_info != NULL) {
	...
	return hisi_pd_send_ext_msg(pd_port, TCPC_TX_SOP, PD_EXT_BAT_CAP,
		false, 0, PD_BCDB_SIZE, (uint8_t *)bcdb);
}
#endif

#ifdef CONFIG_KIRIN_PD_REV30_BAT_CAP_REMOTE
void hisi_pd_dpm_inform_battery_cap(pd_port_t *pd_port, pd_event_t *pd_event)
{
	struct pd_battery_capabilities *bcdb;

	if (dpm_check_ext_msg_event(pd_event, PD_EXT_BAT_CAP)) {
...
static uint32_t c_invalid_bsdo =
	BSDO(0xBB80, BSDO_BAT_INFO_INVALID_REF); // TD.PD.SRC3.E9 Battery Status Fields Checks

int hisi_pd_dpm_send_battery_status(pd_port_t *pd_port)
{
	uint32_t *bsdo = &c_invalid_bsdo;
	struct pd_battery_info *bat_info;
	...
	if (gbsdb) {
		...
		bat_info = hisi_pd_get_battery_info(pd_port, gbsdb->bat_status_ref);
		if (bat_info != NULL) {
			tcpci_notify_request_bat_info(
	...
	return hisi_pd_send_data_msg(pd_port, TCPC_TX_SOP,
		PD_DATA_BAT_STATUS, PD_BSDO_SIZE, bsdo);
}
#endif

#ifdef CONFIG_KIRIN_PD_REV30_BAT_STATUS_REMOTE
void hisi_pd_dpm_inform_battery_status(pd_port_t *pd_port, pd_event_t *pd_event)
{
	uint32_t *payload;

	if (dpm_check_data_msg_event(pd_event, PD_DATA_BAT_STATUS)) {
...
struct pd_manufacturer_info g_hisi_mfrs = {
	.vid = 0xFFFF, .pid = 0x0000,
	.mfrs_string = "Not Supported",
};
int hisi_pd_dpm_send_mfrs_info(pd_port_t *pd_port)
{
	uint8_t len = 0;
	struct pd_battery_info *bat_info;
	...
	if (gmidb) {
		...
		if (gmidb->info_target == PD_GMIDB_TARGET_BATTRY) {
			bat_info = hisi_pd_get_battery_info(pd_port, gmidb->info_ref);
			if (bat_info)
	...
	if (len < sizeof(midb->mfrs_string))
		len++;
	return hisi_pd_send_ext_msg(pd_port, TCPC_TX_SOP, PD_EXT_MFR_INFO,
		false, 0, PD_MIDB_MIN_SIZE + len,
		(uint8_t *)midb);
}
#endif

#ifdef CONFIG_KIRIN_PD_REV30_MFRS_INFO_REMOTE
void hisi_pd_dpm_inform_mfrs_info(pd_port_t *pd_port, pd_event_t *pd_event)
{
	struct pd_manufacturer_info *midb;

	if (dpm_check_ext_msg_event(pd_event, PD_EXT_MFR_INFO)) {
...
void hisi_pd_dpm_inform_country_codes(pd_port_t *pd_port, pd_event_t *pd_event)
{
	struct pd_country_codes *ccdb;

	if (dpm_check_ext_msg_event(pd_event, PD_EXT_COUNTRY_CODES)) {
		ccdb = pd_get_msg_data_payload(pd_port);
		if (ccdb == NULL)
			return;
		I("len=%d, country_code[0]=0x%04x\n",
...
int hisi_pd_dpm_send_country_codes(pd_port_t *pd_port, pd_event_t *pd_event)
{
	uint8_t i;
	struct pd_country_codes ccdb;
	...
	return hisi_pd_send_ext_msg(pd_port, TCPC_TX_SOP, PD_EXT_COUNTRY_CODES,
		false, 0, 2 + ccdb.length*2, (uint8_t *)&ccdb);
}
#endif

#ifdef CONFIG_KIRIN_PD_REV30_COUNTRY_INFO_REMOTE
void hisi_pd_dpm_inform_country_info(pd_port_t *pd_port, pd_event_t *pd_event)
{
	struct pd_country_info *cidb;

	if (dpm_check_ext_msg_event(pd_event, PD_EXT_COUNTRY_INFO)) {
...
int hisi_pd_dpm_send_country_info(pd_port_t *pd_port)
{
	uint8_t i, cidb_size;
	struct pd_country_info cidb;
	struct pd_country_authority *ca = pd_port->country_info;
	...
	for (i = 0; i < pd_port->country_nr; i++) {
		if (ca[i].code == cc) {
			cidb_size += ca[i].len;
			memcpy_s(cidb.country_special_data, sizeof(cidb.country_special_data),
					ca[i].data, ca[i].len);
			break;
		}
	}

	return hisi_pd_send_ext_msg(pd_port, TCPC_TX_SOP, PD_EXT_COUNTRY_INFO, false,
			0, cidb_size, (uint8_t *)&cidb);
}
#endif

void hisi_pd_dpm_inform_alert(pd_port_t *pd_port, pd_event_t *pd_event)
{
...
int hisi_pd_dpm_send_alert(pd_port_t *pd_port, pd_event_t *pd_event)
{
	...
	return pd_send_message(pd_port, TCPC_TX_SOP, PD_DATA_ALERT, false,
			PD_ADO_SIZE, &ado);
}
#endif

#ifdef CONFIG_KIRIN_PD_REV30_STATUS_REMOTE
void hisi_pd_dpm_inform_status(pd_port_t *pd_port, pd_event_t *pd_event)
{
	/* Todo */
	I("\n");
}
...
int hisi_pd_dpm_send_status(pd_port_t *pd_port)
{
	struct pd_status sdb = {0};
//	struct pe_data *pe_data = &pd_port->pe_data;

	memset_s(&sdb, sizeof(sdb), 0, PD_SDB_SIZE);
...
//	pe_data->pd_status_event &= ~PD_STASUS_EVENT_READ_CLEAR;
...
#ifdef CONFIG_KIRIN_PD_REV30_STATUS_LOCAL_TEMP
	...
	return hisi_pd_send_ext_msg(pd_port, TCPC_TX_SOP, PD_EXT_STATUS,
			false, 0, PD_SDB_SIZE, (uint8_t *)&sdb);
}
#endif

#ifdef CONFIG_KIRIN_PD_REV30_PPS_SINK
void hisi_pd_dpm_inform_pps_status(pd_port_t *pd_port, pd_event_t *pd_event)
{
	struct pd_pps_status_raw *ppssdb;

	if (dpm_check_ext_msg_event(pd_event, PD_EXT_PPS_STATUS)) {
...
void hisi_pd_dpm_inform_not_support(pd_port_t *pd_port)
{
	I("not support\n");
	/* TODO */
}

void hisi_pd_dpm_inform_revision(pd_port_t *pd_port, pd_event_t *pd_event)
{
	uint32_t *payload;
	uint32_t rmdo;

	if (dpm_check_data_msg_event(pd_event, PD_DATA_REVISION)) {
		payload = pd_get_msg_data_payload(pd_port);
		if (payload == NULL)
			return;
		rmdo = payload[0];
		I("Revision=%d.%d, Version=%d.%d\n",
			  RMDO_REV_MAJ(rmdo), RMDO_REV_MIN(rmdo),
			  RMDO_VER_MAJ(rmdo), RMDO_VER_MIN(rmdo));
	}
}

static uint32_t c_rmdo = RMDO(3, 1, 1, 6);

int hisi_pd_dpm_send_revision(pd_port_t *pd_port)
{
	return hisi_pd_send_data_msg(pd_port, TCPC_TX_SOP,
...
struct pd_source_cap_ext g_src_cap_ext = {0};
int hisi_pd_dpm_send_source_cap_ext(pd_port_t *pd_port)
{
	g_src_cap_ext.vid = 0x12d1;
	g_src_cap_ext.pid = 0x6526;
	g_src_cap_ext.batteries = 0x1 << 1;
	g_src_cap_ext.xid = 0x0800;
	g_src_cap_ext.fw_ver = 0x30;
	g_src_cap_ext.hw_ver = 0x70;
	g_src_cap_ext.hold_time_ms = 0x14;
	g_src_cap_ext.source_inputs = 0x4;

	pd_port->local_alert = 0;
	D("send_source cap ext\n");

	return hisi_pd_send_ext_msg(pd_port, TCPC_TX_SOP, PD_EXT_SOURCE_CAP_EXT,
			false, 0, PD_SCEDB_SIZE, (uint8_t *)&pd_port->src_cap_ext);
}
#endif

#ifdef CONFIG_KIRIN_PD_REV30_SRC_CAP_EXT_REMOTE
void hisi_pd_dpm_inform_source_cap_ext(pd_port_t *pd_port, pd_event_t *pd_event)
{
	struct pd_source_cap_ext *scedb;

	if (dpm_check_ext_msg_event(pd_event, PD_EXT_SOURCE_CAP_EXT)) {
...
/* PE : Notify DPM */
static bool pd_dpm_ready_power_role_swap(pd_port_t *pd_port, pd_event_t *pd_event)
{
	bool do_swap = false;
...
static int pd_dpm_ready_data_role_swap(pd_port_t *pd_port, pd_event_t *pd_event)
{
	bool do_swap = false;
	uint32_t prefer_role;
...
static int pd_dpm_ready_get_sink_cap(pd_port_t *pd_port, pd_event_t *pd_event)
{
...
static int pd_dpm_ready_get_source_cap(pd_port_t *pd_port, pd_event_t *pd_event)
{
...
static int pd_dpm_ready_attempt_get_flags(pd_port_t *pd_port, pd_event_t *pd_event)
{
	uint32_t prefer_role;
...
{
	D("\n");
	return pd_dpm_ready_attempt_get_flags(pd_port, pd_event);
}
...
static int __pd_dpm_ready_attempt_discover_cable(pd_port_t *pd_port, pd_event_t *pd_event)
{
...
static int __pd_dpm_ufp_attempt_discover_cable(pd_port_t *pd_port, pd_event_t *pd_event)
{
	if (!(pd_port->dpm_caps & DPM_CAP_SNK_EMARK_DISCOVER) && (pd_port->data_role != PD_ROLE_UFP)) {
		D("check_cable_id_dfp not support\n");
		return 0;
	}
 
	D("check_cable_id_ufp\n");
	if (!pd_is_auto_discover_cable_id(pd_port))
		return PD_DPM_RTN_VCONN;
 
	if (pd_port->dpm_flags & DPM_FLAGS_PARTNER_USB_COMM) {
	...
	if (pd_port->vdf_flags &VDF_FLAGS_PD_31_CERTIFICATION_MASK &&
		!pd_check_rev30(pd_port) && pd_port->data_role == PD_ROLE_UFP)
		return hisi_tcpm_data_role_swap(pd_port->tcpc_dev);
	...
	msleep(20); // To avoid SAM charger bug 
...
int pd_dpm_ufp_attempt_discover_cable(pd_port_t *pd_port,
		pd_event_t *pd_event)
{
	int ret;
 
	D("+\n");
	ret = __pd_dpm_ufp_attempt_discover_cable(pd_port, pd_event);
	if (ret != PD_DPM_RTN_VCONN)
		return ret;
...
void hisi_pd_dpm_dfp_inform_cable_vdo(pd_port_t *pd_port, pd_event_t *pd_event)
{
	...
	if (pd_port->vconn_source) {
		D("vconn swap after sent vdm\n");
		pd_port->cable_vconn_back = true;
		pd_dpm_attempt_vswap(pd_port, pd_event);
	}
#endif
...
static int pd_dpm_notify_pe_ufp_ready(pd_port_t *pd_port, pd_event_t *pd_event)
{
	D("+\n");
 
#ifdef CONFIG_USB_PD_UFP_FLOW_DELAY
	if (!pd_port->dpm_ufp_flow_delay_done) {
		D("Delay UFP Flow\n");
...
#ifdef CONFIG_USB_PD_DFP_READY_DISCOVER_ID
	if (!pd_port->cable_vdo_set && (pd_port->dpm_caps & DPM_CAP_SNK_EMARK_DISCOVER))
		return pd_dpm_ufp_attempt_discover_cable(pd_port, pd_event);
#endif
...
int hisi_pd_dpm_notify_pe_startup(pd_port_t *pd_port, pd_event_t *pd_event)
{
	uint32_t caps;
	uint32_t flags = 0;
...
int hisi_pd_dpm_notify_pe_ready(pd_port_t *pd_port, pd_event_t *pd_event)
{
...
#endif
	if (pd_dpm_ready_get_source_cap(pd_port, pd_event))
		return 1;

	if (pd_dpm_ready_get_sink_cap(pd_port, pd_event))
		return 1;

	if (pd_dpm_ready_power_role_swap(pd_port, pd_event))
		return 1;
	...
	if (pd_dpm_ready_data_role_swap(pd_port, pd_event))
		return 1;
	...
	if (pd_port->data_role == PD_ROLE_UFP)
		ret = pd_dpm_notify_pe_ufp_ready(pd_port, pd_event);
