...
void hisi_pe_snk_startup_entry(pd_port_t *pd_port, pd_event_t *pd_event)
{
	...
	hisi_pd_cable_flag_set(pd_port);
...
void hisi_pe_snk_ready_entry(pd_port_t *pd_port, pd_event_t *pd_event)
{
	...
	hisi_pd_notify_pe_snk_explicit_contract(pd_port->tcpc_dev);

	if (!pd_port->cable_vdo_set && (pd_port->dpm_caps & DPM_CAP_SNK_EMARK_DISCOVER)) {
		pd_port->msg_id_tx[TCPC_TX_SOP_PRIME] = 0;
		pd_dpm_ufp_attempt_discover_cable(pd_port, PD_TIMER_DISCOVER_ID);
	}
}
...
{
	D("+\n");
	pd_port->dpm_charging_policy = DPM_CHARGING_POLICY_PPS;
	hisi_pd_send_ctrl_msg(pd_port, TCPC_TX_SOP, PD_CTRL_GET_SOURCE_CAP_EXT);
	pd_enable_timer(pd_port, PD_TIMER_SRC_CAP_EXT_TOUT);
	D("-\n");
}
...
void hisi_pe_snk_get_source_cap_ext_exit(pd_port_t *pd_port, pd_event_t *pd_event)
{
	...
	pd_disable_timer(pd_port, PD_TIMER_SRC_CAP_EXT_TOUT);
...
{
	I("+\n");
	hisi_pd_dpm_inform_pps_status(pd_port, pd_event);
	I("-\n");
}
#endif
