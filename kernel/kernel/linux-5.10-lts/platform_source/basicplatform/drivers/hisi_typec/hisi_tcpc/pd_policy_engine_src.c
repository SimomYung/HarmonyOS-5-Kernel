		...
		pd_port_t *pd_port, pd_event_t *pd_event)
{
	...
	hisi_pd_sync_sop_spec_revision(pd_port,
...
{
	pd_port->state_machine = PE_STATE_MACHINE_SOURCE;
	hisi_pd_notify_pe_src_explicit_contract(pd_port->tcpc_dev);
...
void hisi_pe_src_vdm_identity_naked_entry(pd_port_t *pd_port, pd_event_t *pd_event)
{
	pd_disable_timer(pd_port, PD_TIMER_VDM_RESPONSE);
#ifdef CONFIG_KIRIN_PD_REV30
	hisi_pd_sync_sop_prime_spec_revision(pd_port, PD_REV20);
#endif
	hisi_pd_dpm_src_inform_cable_vdo(pd_port, pd_event);
	pd_free_pd_event(pd_port, pd_event);
}
...
#ifdef CONFIG_KIRIN_PD_REV30
void hisi_pe_src_send_not_supported_entry(pd_port_t *pd_port, pd_event_t *pd_event)
{
	D("+\n");
	hisi_pd_send_ctrl_msg(pd_port, TCPC_TX_SOP, PD_CTRL_NOT_SUPPORTED);
}

void hisi_pe_src_not_supported_received_entry(pd_port_t *pd_port, pd_event_t *pd_event)
{
	D("+\n");
	hisi_pd_dpm_inform_not_support(pd_port);
}

void hisi_pe_src_chunk_received_entry(pd_port_t *pd_port, pd_event_t *pd_event)
{
	D("+\n");
...
void hisi_pe_src_send_source_alert_entry(pd_port_t *pd_port, pd_event_t *pd_event)
{
	D("+\n");
	hisi_pd_dpm_send_alert(pd_port, pd_event);
}
#endif

#ifdef CONFIG_KIRIN_PD_REV30_ALERT_REMOTE
void hisi_pe_src_sink_alert_received_entry(pd_port_t *pd_port, pd_event_t *pd_event)
{
	D("+\n");
	hisi_pd_dpm_inform_alert(pd_port, pd_event);
}
#endif

#ifdef CONFIG_KIRIN_PD_REV30_SRC_CAP_EXT_LOCAL
void hisi_pe_src_give_source_cap_ext_entry(pd_port_t *pd_port, pd_event_t *pd_event)
{
	D("+\n");
...
void hisi_pe_src_give_source_status_entry(pd_port_t *pd_port, pd_event_t *pd_event)
{
	D("+\n");
	hisi_pd_dpm_send_status(pd_port);
}
#endif
...
{
	D("+\n");
	hisi_pd_send_ctrl_msg(pd_port, TCPC_TX_SOP, PD_CTRL_GET_STATUS);
}

void hisi_pe_src_get_sink_status_exit(pd_port_t *pd_port, pd_event_t *pd_event)
{
	D("+\n");
	hisi_pd_dpm_inform_status(pd_port, pd_event);
}
#endif

#ifdef CONFIG_KIRIN_PD_REV30_PPS_SOURCE
void hisi_pe_src_give_pps_status_entry(pd_port_t *pd_port, pd_event_t *pd_event)
{
	D("+\n");
	/* TODO */
	hisi_pd_send_ctrl_msg(pd_port, TCPC_TX_SOP, PD_CTRL_NOT_SUPPORTED);
...
void hisi_pe_src_get_sink_cap_ext_entry(pd_port_t *pd_port, pd_event_t *pd_event)
{
	D("+\n");
	hisi_pd_send_ctrl_msg(pd_port, TCPC_TX_SOP, PD_CTRL_GET_SINK_CAP_EXT);
}

void hisi_pe_src_get_sink_cap_ext_exit(pd_port_t *pd_port, pd_event_t *pd_event)
{
