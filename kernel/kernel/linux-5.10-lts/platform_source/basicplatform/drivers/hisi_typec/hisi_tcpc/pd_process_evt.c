...
bool hisi_pd_is_multi_chunk_msg(pd_port_t *pd_port)
{
#ifdef CONFIG_KIRIN_PD_REV30
	uint16_t size;
...
static bool hisi_pd_process_ready_protocol_error(pd_port_t *pd_port)
{
#ifdef CONFIG_KIRIN_PD_REV30
	bool multi_chunk;
#endif
...
#ifdef CONFIG_KIRIN_PD_REV30
	multi_chunk = hisi_pd_is_multi_chunk_msg(pd_port);
...
#endif
		if (hisi_pd_process_ready_protocol_error(pd_port))
			return true;;
...
static bool hisi_pd_process_cable_ctrl_msg_accept(pd_port_t *pd_port)
{
...
static bool hisi_pd_process_event_cable(pd_port_t *pd_port, pd_event_t *pd_event)
{
	...
	if (pd_event->msg == PD_CTRL_ACCEPT)
		ret = hisi_pd_process_cable_ctrl_msg_accept(pd_port);
#endif
	return ret;
}
...
D("+  %d\n", pd_event->msg_sec);
	...
	case PD_DPM_PD_REQUEST_GET_SINK_CAP:
		ret = hisi_pd_process_dpm_msg_get_sink_cap(pd_port);
		break;
...
static bool pe_is_valid_pd_msg(pd_port_t *pd_port, pd_event_t *pd_event)
{
	...
	if (pd_port->vdf_flags & VDF_FLAGS_PD_31_CERTIFICATION_MASK) {
		if (pd_port->msg_id_rx[sop_type] == msg_id) {
			D("Repeat msg id, event_type = %u, sop_type = %u, msg_type = %u, msg_id = %u\n", event_type, sop_type, msg_type, msg_id);
		...
		pd_event_t *pd_event)
{
	...
	if (pd_msg->frame_type == TCPC_TX_SOP_PRIME)
		hisi_pd_sync_sop_prime_spec_revision(
...
static bool pe_exit_idle_state(pd_port_t *pd_port, pd_event_t *pd_event)
{
	...
	hisi_pd_dpm_notify_pe_startup(pd_port, pd_event);
