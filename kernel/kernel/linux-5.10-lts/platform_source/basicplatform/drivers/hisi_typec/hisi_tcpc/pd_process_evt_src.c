...
static bool pd_process_ctrl_msg_accept_check_src_ready(pd_port_t *pd_port)
{
#ifdef CONFIG_KIRIN_PD_REV30
...

{
	...
	case PD_CTRL_ACCEPT:
		ret = pd_process_ctrl_msg_accept_check_src_ready(pd_port);
		break;
	...
	case PD_CTRL_GET_SINK_CAP:
		if (pd_port->vdf_flags & VDF_FLAGS_PD_31_CERTIFICATION_MASK)
			pd_port->pd_source_get_sink_cap_cnt++;
		...
		pd_port_t *pd_port, pd_event_t *pd_event)
{
	...
	if (pd_port->pe_state_curr == PE_SRC_SEND_CAPABILITIES) {
		if (!pd_port->pd_connected || !pd_port->explicit_contract) {
			PE_TRANSIT_STATE(pd_port, PE_SRC_DISCOVERY);
			return true;
		}
	}
 
	if (pd_port->pe_state_curr == PE_DFP_CBL_SEND_SOFT_RESET) {
		PE_TRANSIT_STATE(pd_port, PE_SRC_SEND_CAPABILITIES);
		return true;
	}
 
	return PE_MAKE_STATE_TRANSIT_FORCE(PD_HW_MSG_TX_FAILED,
			PE_SRC_SEND_SOFT_RESET); // TEST.PD.PROT.SRC3.06 Soft_Reset Sent when SinkTxOK
}
