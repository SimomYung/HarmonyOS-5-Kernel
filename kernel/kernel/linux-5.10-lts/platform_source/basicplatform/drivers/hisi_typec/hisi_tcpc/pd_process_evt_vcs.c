...
DECL_PE_STATE_TRANSITION(PD_TIMER_VCONN_ON) = {
	{ PE_VCS_WAIT_FOR_VCONN, PE_SRC_HARD_RESET},
};
		...
		pd_port_t *pd_port, pd_event_t *pd_event)
{
	...
	case PD_CTRL_NOT_SUPPORTED:
		...
		if (pd_port->cable_vconn_back && pd_port->vconn_source) {
			hisi_pd_dpm_vcs_enable_vconn(pd_port, false);
			pd_port->cable_vconn_back = false;
		}
#endif
#endif
		...
		pd_port_t *pd_port, pd_event_t *pd_event)
{
	...
	case PD_TIMER_VCONN_ON:
		ret = PE_MAKE_STATE_TRANSIT(PD_TIMER_VCONN_ON);
		break;
