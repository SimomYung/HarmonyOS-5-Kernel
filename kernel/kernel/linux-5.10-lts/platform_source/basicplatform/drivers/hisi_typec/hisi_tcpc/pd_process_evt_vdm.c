		...
		pd_port_t *pd_port, pd_event_t *pd_event)
{
	...
	// case PE_UFP_VDM_SEND_SVIDS:   // TEST.PD.VDM.SNK.02 & TEST.PD.VDM.SNK.05
...
#define LECROY_VID 1535
static bool pd_check_svid_invalid_uvdm(pd_port_t *pd_port, uint16_t svid)
{
	switch(svid) {
	case LECROY_VID:
		D("Lecroy vid, return a not_supported message\n");
		return true;
	default:
		return false;
	}
...
static bool pd_process_uvdm(pd_port_t *pd_port, pd_event_t *pd_event)
{
	...
	uint16_t svid = PD_VDO_VID(vdm_hdr);
	...
	if (pd_port->data_role == PD_ROLE_UFP) {
		if (pd_check_pe_state_ready(pd_port)) {
			if ((pd_port->vdf_flags & VDF_FLAGS_PD_31_CERTIFICATION_MASK) &&
					pd_check_svid_invalid_uvdm(pd_port, svid))
...
static bool pd_process_dfp_vdm(pd_port_t *pd_port, pd_event_t *pd_event)
{
	...
	D("pe_state_curr: %s\n",  pd_pe_state_name(pd_port->pe_state_curr));
	switch (pd_port->pe_state_curr) {
	...
	default:
		if (pd_check_vdm_state_ready(pd_port)) {
			if (pd_check_rev30(pd_port))
				PE_TRANSIT_STATE(pd_port, PE_DFP_VDM_UNKNOWN);
			else
				PE_TRANSIT_STATE(pd_port, PE_UFP_VDM_GET_IDENTITY_NAK);
			return true;
		}
		break;
	}
...
static inline bool pd_check_svid_valid(pd_port_t *pd_port, uint16_t svid)
{
	switch (svid) {
	case USB_SID_PD:
		return true;
...
static bool pd_process_data_msg(pd_port_t *pd_port, pd_event_t *pd_event)
{
	...
	if ((pd_port->vdf_flags & VDF_FLAGS_PD_31_CERTIFICATION_MASK) && 
			!pd_check_svid_valid(pd_port, svid)) { // TEST.PD.VDM.SRC.02
		PE_TRANSIT_STATE(pd_port, PE_UFP_VDM_GET_IDENTITY_NAK);
		ret = true;
	} else {
...
{
	D("\n");
	if(pd_port->pe_state_curr == PE_DFP_CBL_VDM_IDENTITY_ACKED ||
		pd_port->pe_state_curr == PE_DFP_CBL_VDM_IDENTITY_NAKED){
			PE_TRANSIT_READY_STATE(pd_port);
			return true;
	}
