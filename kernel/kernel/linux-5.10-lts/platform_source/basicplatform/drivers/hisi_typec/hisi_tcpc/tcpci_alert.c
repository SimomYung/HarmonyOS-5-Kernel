		...
		uint16_t power_status)
{
	...
	tcpci_get_ext_status(tcpc_dev, &ext_status);
	D("ext_status: %d\n", ext_status);
	if ((tcpc_dev->pd_port.vdf_flags & VDF_FLAGS_PD_31_CERTIFICATION_MASK) && ext_status) {
...
#ifdef CONFIG_KIRIN_TCPC_QUIRK_V100
...
static int tcpci_alert_tx_discard(struct tcpc_device *tcpc_dev)
{
	...
	if(tcpc_dev->pd_port.vdf_flags & VDF_FLAGS_TX_DISCARD_MASK)
		return 0;
...
static int tcpci_set_wake_lock_pd(struct tcpc_device *tcpc_dev, bool pd_lock)
{
	uint8_t wake_lock_pd;

	tcpci_mutex_lock(tcpc_dev);
	wake_lock_pd = tcpc_dev->wake_lock_pd;
	...
	if (wake_lock_pd == 0)
		__pm_wakeup_event(tcpc_dev->dettach_temp_wake_lock,
				jiffies_to_msecs(5 * HZ));

	__tcpci_set_wake_lock_pd(tcpc_dev, wake_lock_pd, tcpc_dev->wake_lock_user);

	if (wake_lock_pd == 1)
		__pm_relax(tcpc_dev->dettach_temp_wake_lock);

	tcpc_dev->wake_lock_pd = wake_lock_pd;
	tcpci_mutex_unlock(tcpc_dev);

	return 0;
}

static int tcpci_report_usb_port_attached(struct tcpc_device *tcpc_dev)
{
	I("usb_port_attached\n");

	if (tcpc_dev->typec_attach_new != TYPEC_ATTACHED_AUDIO)
		tcpci_set_wake_lock_pd(tcpc_dev, true);

#ifdef CONFIG_KIRIN_POWER_DELIVERY_SUPPORT
	pd_put_cc_attached_event(tcpc_dev, tcpc_dev->typec_attach_new);
#endif
...
if (hisi_tcpc_support_reverse_sc(tcpc))
		set_desc_role_def(tcpc, TYPEC_ROLE_DRP);
#endif
...
static void report_usb_port_changed_handle_unattached(struct tcpc_device *tcpc)
{
	...
	if (!hisi_tcpc_support_reverse_sc(tcpc) || (transfer_path == 0 || tcpc->pd_port.prev_path == transfer_path ||
...
#ifdef CONFIG_KIRIN_TYPEC_CAP_DBGACC_SNK_SUPPORT
...
#ifdef CONFIG_KIRIN_TYPEC_CAP_CUSTOM_SRC_SUPPORT
