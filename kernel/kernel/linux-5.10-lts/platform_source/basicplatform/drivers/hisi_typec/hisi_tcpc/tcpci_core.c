		...
		struct tcpc_ops *ops, void *drv_data)
{
	...
	tcpc->attach_wake_lock = wakeup_source_register(&tcpc->dev,
			"hisi_tcpc_attach_wakelock");
	tcpc->dettach_temp_wake_lock = wakeup_source_register(&tcpc->dev,
	...
	if (tcpc->pd_port.vdf_flags & VDF_FLAGS_PD_31_CERTIFICATION_MASK)
		hisi_tcpc_certification_timer_pre_init(tcpc);
