		...
		const pd_event_t *pd_event, bool from_port_partner)
{
	...
	D("event_type %s: %d, from partner %u\n",
		pd_event_type_name[pd_event->event_type], pd_event->msg, from_port_partner);
...
bool pd_check_vdm_state_ready(pd_port_t *pd_port)
{
...
bool hisi_pd_get_deferred_tcp_event_ex(struct tcpc_device *tcpc_dev, struct tcp_dpm_event *tcp_event)
{
	bool ret;
	I("\n");
	tcpci_mutex_lock(tcpc_dev);
	...
	if (tcpc_dev->tcp_event_count)
		hisi_tcpc_restart_timer(tcpc_dev, PD_TIMER_DEFERRED_EVT);
	else
		hisi_tcpc_disable_timer(tcpc_dev, PD_TIMER_DEFERRED_EVT);
...
bool hisi_pd_put_deferred_tcp_event_ex(
	struct tcpc_device *tcpc_dev, const struct tcp_dpm_event *tcp_event)
{
	bool ret = true;
	pd_port_t *pd_port = &tcpc_dev->pd_port;
	...
	if (tcpc_dev->tcp_event_count == 1)
		hisi_tcpc_enable_timer(tcpc_dev, PD_TIMER_DEFERRED_EVT);
#endif
#endif
...
void hisi_pd_notify_pe_snk_explicit_contract(struct tcpc_device *tcpc_dev)
{
#ifdef CONFIG_KIRIN_PD_REV30_COLLISION_AVOID
	pd_port_t *pd_port = &tcpc_dev->pd_port;
	I("\n");
	if (pd_port->explicit_contract || !pd_check_rev30(pd_port))
		return;
	...
	if (pd_port->pd_traffic_control == PD_SINK_TX_OK) {
		pd_port->pd_traffic_control = PD_SINK_TX_START;
...
void hisi_pd_notify_pe_src_explicit_contract(struct tcpc_device *tcpc_dev)
{
	pd_port_t *pd_port = &tcpc_dev->pd_port;
	...
	if (pd_port->explicit_contract) {
		...
		if (pd_check_rev30(pd_port) &&
			(pd_port->pd_traffic_control == PD_SOURCE_TX_START))
			pd_restart_timer(pd_port, PD_TIMER_SINK_TX);
#endif	/* CONFIG_KIRIN_PD_REV30_SRC_FLOW_DELAY_STARTUP */
#endif	/* CONFIG_KIRIN_PD_REV30_COLLISION_AVOID */
		return;
	}
	...
	struct tcpc_device *tcpc_dev, const struct tcp_dpm_event *tcp_event)
{
	bool ret = false;
	if (!tcpc_dev->pd_pe_running || tcpc_dev->pd_wait_pe_idle) {
		E("failed !pd_pe_running || pd_wait_pe_idle\n");
		return false;
	}

	if (tcpc_dev->tcp_event_count >= TCP_EVENT_BUF_SIZE) {
		E("failed tcp_event_count overflow\n");
		return false;
	}

	if (tcpc_dev->pd_wait_hard_reset_complete) {
		E("failed pd_wait_hard_reset_complete\n");
		return false;
	}
...
#ifdef CONFIG_REVERSE_CHARGE
	if (hisi_tcpc_support_reverse_sc(tcpc_dev))
		set_desc_role_def(tcpc_dev, TYPEC_ROLE_DRP);
#endif
...
bool hisi_pd_put_pd_msg_event(struct tcpc_device *tcpc_dev, pd_msg_t *pd_msg)
{
	...
	cnt = PD_HEADER_CNT(pd_msg->msg_hdr);
	cmd = PD_HEADER_TYPE(pd_msg->msg_hdr);
	ext = PD_HEADER_EXT(pd_msg->msg_hdr);
	...
	if ((cnt != 0) && ((enum pd_data_msg_type)cmd == PD_DATA_BIST) && (ext == 0)) {
		D("set pd_bist_mode\n");
...
void hisi_pd_notify_pe_execute_pr_swap(pd_port_t *pd_port, bool start_swap)
{
	...
	tcpci_snk_unattach_by_vbus_bypass(tcpc_dev, true);
#endif
}
