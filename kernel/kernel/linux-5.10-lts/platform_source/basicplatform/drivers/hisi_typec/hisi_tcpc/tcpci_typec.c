...
int tcpci_get_cc_res(struct tcpc_device *tcpc_dev)
{
...
static inline void typec_unattached_cc_entry(struct tcpc_device *tcpc_dev)
{
	...
	tcpc_dev->pd_port.cable_vdo_set = false;
	tcpc_dev->pd_port.remote_src_cap.nr = 0;

	if (tcpc_dev->pd_port.vdf_flags & VDF_FLAGS_ATC_CC_OPEN_MASK) {
...
static void typec_unattached_entry(struct tcpc_device *tcpc_dev)
{
	...
	tcpc_dev->pd_port.cable_vconn_back = false;
#endif	
	tcpc_dev->pd_port.cable_vdo_set = false;
...
#ifndef CONFIG_KIRIN_TCPC_VSAFE0V_DETECT_SUPPORT
...
#ifdef CONFIG_REVERSE_CHARGE
if (hisi_tcpc_support_reverse_sc(tcpc_dev) || tcpc_dev->typec_attach_new == TYPEC_UNATTACHED) {
		switch (tcpc_dev->typec_role) {
		case TYPEC_ROLE_SNK:
			tcpci_set_cc(tcpc_dev, TYPEC_CC_RD);
		default:
...
void hisi_usb_rpd_set_src_cap(struct tcpc_device *tcpc_dev, struct otg_cap *allow_ext, int send)
{
	...
	if (tcpc_dev->pd_port.ext_src_cap.otg_vol == allow_ext->otg_vol &&
		...
		D("aleardy set ext_cap : vol=%d, cur=%d\n", allow_ext->otg_vol, allow_ext->otg_cur);
		return;
	} 
