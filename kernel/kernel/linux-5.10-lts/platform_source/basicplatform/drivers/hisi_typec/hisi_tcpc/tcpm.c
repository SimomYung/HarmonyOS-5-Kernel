...
{
	return pd_put_cc_re_enable_pd_event(tcpc, tcpc->typec_attach_new);
}
...
bool hisi_tcpm_support_snk_detect_emark(struct tcpc_device *tcpc)
{
	...
	return (pd_port->dpm_caps & DPM_CAP_SNK_EMARK_DISCOVER);
}
...
bool hisi_tcpm_support_src_cap(struct tcpc_device *tcpc_dev, uint32_t src_cap_vol, uint32_t src_cap_mask)
{
	...
	for (i = 0; i < src_cap->nr; i++) {
		if (src_cap_vol == (src_cap->pdos[i] & src_cap_mask)) {
		    D("SrcCap include %x\n", src_cap_vol);
		    return true;
		}
	}
	D("SrcCap do not include %x\n", src_cap_vol);
	return false;
}
...
{
	pd_port_t *pd_port = &tcpc_dev->pd_port;
	return hisi_pd_dpm_is_during_dp(pd_port);
}
