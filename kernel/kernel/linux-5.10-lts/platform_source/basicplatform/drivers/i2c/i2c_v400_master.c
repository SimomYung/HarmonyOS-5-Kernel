...
static void hs_i2c_xfer_init(struct hs_i2c_dev *dev)
{
	...
	dev->slv_addr = msgs[dev->msg_write_idx].addr;
...
static void hs_i2c_xfer_msg(struct hs_i2c_dev *dev)
{
    ...
    if (!dev->msgs) {
        dev_err(dev->dev, "%s: invalid message\n", __func__);
        return;
    }
	...
	for (; dev->msg_write_idx < dev->msgs_num; dev->msg_write_idx++) {
		...
		while (buf_len > 0 && tx_limit > 0 && rx_limit > 0) {
			...
			times++;
			if ((dev->wriet_buf_len_crc > 0) && (buf_len > dev->wriet_buf_len_crc)) {
				dev_err(dev->dev, "%s err buf len %d, times %d\n", __func__, buf_len, times);
				regmap_write(dev->map, INT_MASK, 0);
				dev->msg_err = -EINVAL;
...
static void hs_i2c_read(struct hs_i2c_dev *dev)
{
	...
	for (; dev->msg_read_idx < dev->msgs_num; dev->msg_read_idx++) {
		...
		if ((dev->status & STATUS_READ_IN_PROGRESS) == 0) {
			dev->rx_buf_len = 0;
			buf = msgs[dev->msg_read_idx].buf;
			dev->msgs_read_buf = msgs[dev->msg_read_idx].buf;
		} else {
			buf = dev->rx_buf;
		}

		for (; msgs[dev->msg_read_idx].len - dev->rx_buf_len > 0; dev->rx_buf_len++) {
			if (dev->msgs_read_buf != buf) {
				dev_err(dev->dev, "%s msgs is not correct\n", __func__);
				return;
			}

			u32 flags = msgs[dev->msg_read_idx].flags;

            stat = readl(dev->base + FIFO_STATE);
			if (stat & I2C_V400_RX_EMPTY)
				break;
            tmp = readl(dev->base + RXDATA);
			*buf = tmp & 0xff;
#ifdef CONFIG_I2C_V400_MASTER_RECV_LEN
            /* Ensure length byte is a valid value */
            if (flags & I2C_M_RECV_LEN)
				dev->rx_buf_len = hs_i2c_recv_len(dev, *buf);
#endif
			buf++;
			dev->msgs_read_buf++;
			dev->rx_outstanding--;
		...
		if (msgs[dev->msg_read_idx].len - dev->rx_buf_len > 0) {
...
static int hs_i2c_wait_for_completion(struct hs_i2c_dev *dev,
	int totallen)
{
	int ret, trans_stat, irq_stat;

	ret = wait_for_completion_timeout(&dev->cmd_complete,
		WAIT_FOR_COMPLETION);
	if (ret == 0) {
		disable_irq(dev->irq);
		regmap_read(dev->map, INT_MSTAT, &irq_stat);
		regmap_read(dev->map, TRANS_STATE, &trans_stat);
		dev_err(dev->dev, "dev timed out, irq stat[0x%x], trans stat[0x%x]\n",
			irq_stat, trans_stat);
		regmap_write(dev->map, INT_MASK, 0);
...
static void release_trans_resource(struct hs_i2c_dev *dev)
{
	dev->msgs = NULL;
    dev->msgs_num = 0;
    dev->tx_buf = NULL;
...
static int hs_i2c_xfer(struct i2c_adapter *adap, struct i2c_msg msgs[], int num)
{
	...
	if ((dev->use_srflag) && (dev->suspend_flag)) {
		pr_err("%s:i2c ctrler doesn't resume\n", __func__);
		return -ENODEV;
	}
	mutex_lock(&dev->lock);
	...
	struct platform_device *pdev)
{
	...
	ret = of_property_read_u32(dev->of_node, "use-srflag", &use_srflag);
	if (ret)
		use_srflag = 0;
	pr_info("%s use_srflag=%u\n", __func__, use_srflag);
	d->use_srflag = use_srflag;

	ret = of_property_read_u32(dev->of_node, "write-len-crc", &write_len_crc);
	if (ret)
		write_len_crc = 0;
	pr_info("%s write_len_crc=%u\n", __func__, write_len_crc);
	d->wriet_buf_len_crc = write_len_crc;
...
static irqreturn_t i2c_isr(int this_irq, void *dev_id)
{
	...
	if (!dev || !dev->msgs) {
		pr_err("%s: i2c contrller do not be init or msgs is null.\n", __func__);
		return IRQ_HANDLED;
	}
