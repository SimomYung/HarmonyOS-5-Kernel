...
#define PMIC_KEYPAD_UP_DATA 0x2
#define PMIC_KEYPAD_DOWN_DATA 0x4
#define PMIC_GPIO_KEY_INT 0x1A3

#define PMIC_KEYUP_INT_CLEAR 0
#define PMIC_KEYPAD_UP_DATA_BIT 1
#define PMIC_KEYPAD_DOWN_DATA_BIT 2
#define PMIC_KEYPAD_UP_UP_INT (1 << 1)
#define PMIC_KEYPAD_UP_DOWN_INT (1 << 0)
#define PMIC_KEYPAD_DOWN_UP_INT (1 << 3)
#define PMIC_KEYPAD_DOWN_DOWN_INT (1 << 2)
#else
...
#if defined(CONFIG_PMIC_21V900_PMU) || defined(CONFIG_PMIC_55V500_PMU) || defined(CONFIG_PMIC_26V100_PMU)\
	|| defined(CONFIG_PMIC_21BV100_PMU) || (defined CONFIG_PMIC_55V800_PMU)
...
static irqreturn_t keypad_pmic_key_irq_handler(int irq, void *dev_id)
{
	struct keypad_pmic_key *gpio_key =
		(struct keypad_pmic_key *)dev_id;
	u32 key_event, key_value;
 
	if (!gpio_key)
		return IRQ_NONE;
 
	key_value = keypad_pmic_key_read(PMIC_GPIO_KEY_INT);
	keypad_pmic_key_write(key_value, PMIC_GPIO_KEY_INT);
	if ((key_value & PMIC_KEYPAD_UP_UP_INT) || (key_value & PMIC_KEYPAD_UP_DOWN_INT)) {
		key_event = keypad_pmic_key_read(PMIC_GPIO_KEYUP_DATA);
		if ((key_event & PMIC_KEYPAD_UP) == GPIO_LOW_VOLTAGE)
			gpio_key_vol_updown_press_set_bit(VOL_UP_BIT);
		else
		...
		__pm_wakeup_event(gpio_key->volume_up_key_lock,
			jiffies_to_msecs(KEY_IRQ_WAKEUP_INTERVAL));
	} else if ((key_value & PMIC_KEYPAD_DOWN_UP_INT) || (key_value & PMIC_KEYPAD_DOWN_DOWN_INT)) {
		key_event = keypad_pmic_key_read(PMIC_GPIO_KEYDOWN_DATA);
		if ((key_event & PMIC_KEYPAD_DOWN) == GPIO_LOW_VOLTAGE)
			gpio_key_vol_updown_press_set_bit(VOL_DOWN_BIT);
		else
...
static int pmic_key_irq_init(struct platform_device *pdev,
	struct keypad_pmic_key *gpio_key)
{
	int irq, ret;

	irq = platform_get_irq_byname(pdev, "volumekey");
	if (irq < 0)
		return -1;
 
	ret = devm_request_irq(&pdev->dev, irq, keypad_pmic_key_irq_handler,
		IRQF_SHARED | IRQF_NO_SUSPEND, "volumekey", gpio_key);
	if (ret < 0) {
		pr_err("%s: request volumekey irq err, ret %d\n", __func__, ret);
		return -ENOENT;
	}

	pr_info("volumekey irq request, ret %d\n", ret);
	return ret;
}
#endif
...
#if  (defined CONFIG_PMIC_21BV100_PMU) || (defined CONFIG_PMIC_55V800_PMU)
	err = pmic_key_irq_init(pdev, gpio_key);
	if (err) {
