...
#elif defined(CONFIG_PMIC_21BV100_PMU) || defined(CONFIG_PMIC_55V800_PMU)
#include "linear_vibrator_pmic21.h"
#else
...
struct linear_vibrator_dev {
	...
	unsigned int *strength_data;
	unsigned int *strength_addr;
	...
	u32 linear_vibrator_type;
	u32 linear_vibrator_freq_max;
	u32 linear_vibrator_freq_min;
	s8 name[32];
	u32 strength;
	u32 vib_type;
#ifdef CONFIG_BOOST_5V
...
static void linear_vibrator_freq_dmd(struct linear_vibrator_dev *vdev)
{
	...
	if ((vdev->linear_vibrator_freq > g_vdev->linear_vibrator_freq_max) ||
		(vdev->linear_vibrator_freq < g_vdev->linear_vibrator_freq_min)) {
		dev_info(vdev->dev,
...
static s32 linear_vibrator_set_strength_cfg(struct linear_vibrator_dev *vdev)
{
	u32 i, ladd_reg;
    if(vdev->vib_type == LINEAR_VIBRATOR_MIATYEP) {
        for (i = 0; i < LINEAR_VIBRATOR_REG_CFG_NUM; i++) {
            ladd_reg = LINEAR_VIBRATOR_LADD_PARA_REG + i;
            linear_vibrator_write_u8(
                vdev->haptics_lib[5].cfg[i].ladd_para, ladd_reg);
	    }
    }

	for(i = 0; i < vdev->strength; i++){
		linear_vibrator_write_u8(vdev->strength_data[i], vdev->strength_addr[i]);
	}
...
static void linear_vibrator_enable(struct linear_vibrator_dev *vdev)
{
	...
	if (vdev->state) {
		...
		if (vdev->linear_erm_vibrator) {
			...
			if((vdev->vib_type == LINEAR_VIBRATOR_MIATYEP) || (vdev->vib_type == LINEAR_VIBRATOR_MRTTYEP)) {
				linear_vibrator_set_strength_cfg(vdev);
			} else {
...
static void haptics_spel_time1(void)
{
	linear_vibrator_write_u8(
			LINEAR_VIBRATOR_SPEL_TIME_VALUE_0,
			LINEAR_VIBRATOR_SPEL_TIME_REG0);
	linear_vibrator_write_u8(
			LINEAR_VIBRATOR_SPEL_TIME_VALUE_1,
			LINEAR_VIBRATOR_SPEL_TIME_REG1);
	linear_vibrator_write_u8(
			LINEAR_VIBRATOR_SPEL_TIME_VALUE_2,
			LINEAR_VIBRATOR_SPEL_TIME_REG2);
	linear_vibrator_write_u8(
			LINEAR_VIBRATOR_SPEL_TIME_VALUE_3,
			LINEAR_VIBRATOR_SPEL_TIME_REG3);
	linear_vibrator_write_u8(
			LINEAR_VIBRATOR_SPEL_TIME_VALUE_4,
			LINEAR_VIBRATOR_SPEL_TIME_REG4);
	linear_vibrator_write_u8(
			LINEAR_VIBRATOR_SPEL_TIME_VALUE_5,
			LINEAR_VIBRATOR_SPEL_TIME_REG5);
}

static void haptics_spel_time2(void)
{
	linear_vibrator_write_u8(
			LINEAR_VIBRATOR_SPEL_TIME_VALUE0_1,
			LINEAR_VIBRATOR_SPEL_TIME_REG0);
	linear_vibrator_write_u8(
			LINEAR_VIBRATOR_SPEL_TIME_VALUE_1,
			LINEAR_VIBRATOR_SPEL_TIME_REG1);
	...
	linear_vibrator_write_u8(
			...
			LINEAR_VIBRATOR_SPEL_TIME_REG4);
	linear_vibrator_write_u8(
			LINEAR_VIBRATOR_SPEL_TIME_VALUE5_1,
			LINEAR_VIBRATOR_SPEL_TIME_REG5);
}

static void haptics_spel_time_set(u32 type)
{
	int i;
	...
	for(i = 0; i < 4; i++)
	{
		linear_vibrator_write_u8(0, LINEAR_VIBRATOR_SPEL_TIME_REG5 + i + 1);
	}
}
	...
	struct device_attribute *attr, char *buf)
{
	...
	if ((g_vdev->linear_vibrator_freq > g_vdev->linear_vibrator_freq_max) ||
		(g_vdev->linear_vibrator_freq < g_vdev->linear_vibrator_freq_min))
		cal_rst = 0;
	else
		cal_rst = 1;
...
#if  defined(CONFIG_PMIC_21BV100_PMU) ||  defined(CONFIG_PMIC_55V800_PMU)
static irqreturn_t linear_vibrator_pmu_handler(int irq, void *data)
{
	struct linear_vibrator_dev *vdev =
		(struct linear_vibrator_dev *)data;
	u32 i, vib_value;
	char* irq_name = "unknown";

	if (!data)
		return IRQ_NONE;

	vib_value = linear_vibrator_read_u8(LINEAR_IRQ_L3_LRA_RPT);
	linear_vibrator_write_u8(vib_value, LINEAR_IRQ_L3_LRA_RPT);
	switch  (vib_value) {
	case LINEAR_VIBRATOR_EVENTOCP:
		irq_name = "ocp";
		break;
	case LINEAR_VIBRATOR_EVENTOUT:
		irq_name = "out";
		break;
	case LINEAR_VIBRATOR_EVENTUND:
		irq_name = "undervol";
		break;
	case LINEAR_VIBRATOR_EVENTOVE:
		irq_name = "overvol";
		break;
	case LINEAR_VIBRATOR_EVENTADC:
		irq_name = "adc";
		break;
	default:
	...
	dev_err(g_vdev->dev,"linear vibrator interrupt happend[%s] vib_value = %d\n", irq_name, vib_value);
#ifdef CONFIG_HUAWEI_DSM
	linear_vibrator_irq_dmd_match(irq_name);
#endif
...
static int vibrator_irq_request(struct spmi_device *pdev)
{
	struct linear_vibrator_dev *vdev = NULL;
	int irq, ret;
	...
	irq = spmi_get_irq_byname(pdev, NULL, "vibrator");
	if (irq < 0)
		return -1;

	ret = devm_request_irq(&pdev->dev, irq, linear_vibrator_pmu_handler,
		IRQF_SHARED | IRQF_NO_SUSPEND, "vibrator", vdev);
	if (ret < 0) {
		pr_err("%s: request vibrator_connect irq err, ret %d\n", __func__, ret);
		return -ENOENT;
	}
	return ret;
}
...
/* Write the value of the (normal duty 、ovdr duty、 boost、brake duty、gate1~gate3) register */
static s32 linear_vibrator_reg_strength(struct linear_vibrator_dev *vdev)
{
	struct device *dev = vdev->dev;
	unsigned int *strength_data = NULL;
	unsigned int *strength_addr = NULL;
	int size = 0;
	int ret;

	ret = of_property_read_u32(dev->of_node, "vibrator-reg-strength", &size);
	if (ret < 0) {
		dev_info(dev, "get reg-strength fail,not uesed!\n");
		return ret;
	}

	strength_data = devm_kzalloc(dev, sizeof(unsigned int) * size, GFP_KERNEL);
	if (strength_data == NULL) {
		dev_err(dev, "failed to allocate strength cfg \n");
		return -ENOMEM;
	}

	strength_addr = devm_kzalloc(dev, sizeof(unsigned int) * size, GFP_KERNEL);
	if (strength_addr == NULL) {
		dev_err(dev, "failed to allocate strength addr cfg \n");
		return -ENOMEM;
	}

	ret = of_property_read_u32_array(
		dev->of_node, "vibrator-reg-strength-val", strength_data, size);
	if (ret) {
		dev_err(dev, "could not read vibrator-reg-strength-val table\n");
		return ret;
	}

	ret = of_property_read_u32_array(
		dev->of_node, "vibrator-reg-strength-addr", strength_addr, size);
	if (ret) {
		dev_err(dev, "could not read vibrator-reg-strength-addr table\n");
		return ret;
	}

	vdev->strength_data = strength_data;
	vdev->strength_addr = strength_addr;
	vdev->strength = size;
...
static s32 linear_vibrator_parse_dt(struct linear_vibrator_dev *vdev)
{
	...
	ret = of_property_read_u32(dev->of_node, "vib_type", &temp);
	if (ret < 0) {
		dev_info(dev, "get vib_type fail\n");
		vdev->vib_type = NONE_VIB;
	} else {
		vdev->vib_type = temp;
		dev_info(dev, "vib_type is %d\n",vdev->vib_type);
	}

	if (vdev->vib_type == LINEAR_VIBRATOR_MIATYEP) {
		vdev->linear_vibrator_freq_min = LINEAR_XVIBRATOR_FREQ_MIN;
		vdev->linear_vibrator_freq_max = LINEAR_XVIBRATOR_FREQ_MAX;
	} else {
		vdev->linear_vibrator_freq_min = LINEAR_VIBRATOR_FREQ_MIN;
		vdev->linear_vibrator_freq_max = LINEAR_VIBRATOR_FREQ_MAX;
	}
	...
	ret = linear_vibrator_reg_strength(vdev);
	if (ret)
		dev_info(dev, "vibrator reg strength fail\n");
