...
#include <asm/irq.h>
#if !defined(CONFIG_PRODUCT_CDC_ACE) && defined(CONFIG_SYS_PM_ULSR)
...
#if defined(CONFIG_PMIC_21BV100_PMU) || defined(CONFIG_PMIC_55BV800_PWRKEY)
#include <platform_include/basicplatform/linux/mfd/pmic_platform.h>
...
static irqreturn_t powerkey_down_hdl(int irq, void *data)
{
	...
	if (info->type == POWER_KEY_ARMPC && info->is_suspended)
		return IRQ_HANDLED;
...
#if !defined(CONFIG_PRODUCT_CDC_ACE) && defined(CONFIG_SYS_PM_ULSR)
	if (is_ulsr()) {
		pr_err("[%s]power key press interrupt in ulsr!\n", __func__);
		call_powerkey_notifiers(PRESS_KEY_DOWN, data);
		input_report_key(info->idev, KEY_POWER, POWER_KEY_PRESS);
		input_sync(info->idev);
	}
...
#if defined(CONFIG_PMIC_21BV100_PMU) || defined(CONFIG_PMIC_55BV800_PWRKEY)
static inline void clear_l3_irq(void)
{
	unsigned int key_value;
	key_value = pmic_read_reg(IRQ_L3_PWRON_RPT);
	pmic_write_reg(IRQ_L3_PWRON_RPT, key_value);
