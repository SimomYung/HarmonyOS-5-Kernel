...
#include <platform_include/basicplatform/linux/leds_pmic.h>
...
#ifdef CONFIG_LEDS_ARMPC
#define T_FULL_ON	2000
#define T_FULL_OFF	3000
#endif
	...
	struct spmi_device *pdev, int index)
{
	...
	ret = of_property_read_string(root, "dr_ctrl", &led->name);
	if (ret < 0) {
	...
	ret = of_property_read_u32(root, "dr_start_del",
		&led->dr_start_del);
	if (ret < 0) {
	...
	ret = of_property_read_u32(root, "dr_iset", &led->dr_iset);
	if (ret < 0) {
		dev_err(&pdev->dev, "led%d's dr_iset fail!\n", index);
		return ret;
	}

	ret = of_property_read_u32(root, "dr_time_config0",
		&led->dr_time_config0);
	if (ret < 0) {
	...
	ret = of_property_read_u32(root, "dr_time_config1",
		&led->dr_time_config1);
	if (ret < 0) {
...
static int led_basic_dt_cfg(struct spmi_device *pdev)
{
	...
	ret = of_property_read_u32(pdev->dev.of_node, "dr_led_ctrl",
	...
	ret = of_property_read_u32(pdev->dev.of_node, "dr_out_ctrl",
...
#ifdef CONFIG_LEDS_ARMPC
	/* set power led shine in s0 mode */
	ret = led_set(&data->leds[LED2], LED_FULL);
	if (ret < 0)
		dev_err(&pdev->dev, "%s:set led brightness failed\n", __func__);
#endif

	atomic_notifier_chain_register(&panic_notifier_list, &panic_led);
	dev_err(&pdev->dev, "leds init success\n");
...
static void led_spmi_remove(struct spmi_device *pdev)
{
	int i;
	struct led_spmi_platform_data *pdata = NULL;
	const struct of_device_id *match = NULL;
...
static int led_spmi_suspend(struct spmi_device *pdev, pm_message_t state)
{
	struct led_spmi_drv_data *data = dev_get_drvdata(&pdev->dev);
#ifdef CONFIG_LEDS_ARMPC
	struct led_spmi_data *blue_led_data = &data->leds[LED2];
	unsigned long delay_on = T_FULL_ON;
	unsigned long delay_off = T_FULL_OFF;
	int ret;
#endif
...
#ifdef CONFIG_LEDS_ARMPC
	/* set power led blink in s3 mode */
	ret = led_set(blue_led_data, LED_FULL);
	if (ret < 0)
		dev_err(&pdev->dev, "%s:set led brightness failed\n", __func__);

	ret = led_set_blink(&blue_led_data->ldev, &delay_on, &delay_off);
	if (ret < 0)
		dev_err(&pdev->dev, "%s:set led blink failed\n", __func__);
#endif
...
#ifdef CONFIG_LEDS_ARMPC
	/* set power led shine recover from s3 mode to s0 mode */
	if(led_set(&data->leds[LED2], LED_FULL) < 0)
		dev_err(&pdev->dev, "%s:set led brightness failed\n", __func__);
#endif
