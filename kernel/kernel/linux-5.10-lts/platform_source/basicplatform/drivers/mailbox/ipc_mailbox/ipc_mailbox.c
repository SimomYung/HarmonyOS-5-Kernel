...
#if defined(CONFIG_NPU_PLATFORM_DRIVER) || defined(CONFIG_NPU_PLATFORM_DRIVER_MODULE)
...
static void ipc_recv_rx_task(struct ipc_mbox_device *mdev)
{
	...
	if(mdev->ops->read_erecord(mdev)) {
		mbox_pr_err("mdev %s erecord error\n",mdev->name);
		goto out;
	}
#endif
...
static irqreturn_t ipc_mbox_interrupt(int irq, void *p)
{
	...
	if (mdev->ops->get_vm_flag(mdev) || mdev->ops->is_stm(mdev, DESTINATION_STATUS)) {
		...
		if(mdev->rx_bh.func == NULL) {
			mbox_pr_err("%s--%d before schedule", mdev->name, irq);
			WARN_ON(1);
		}
...
#ifdef CONFIG_IPC_MAILBOX_REMAP_MID	
		if(mdev->ops->read_erecord(mdev)) {
			mbox_pr_err("mdev %s ack erecord error\n",mdev->name);
			mdev->ops->set_eclear(mdev);
		}
#endif
	...
	struct ipc_mbox_device *mdev, enum mbox_mail_type_t mail_type)
{
	...
	if (!--mdev->configured) {
		...
		unused_thread = mdev->ops->get_unused_thread(mdev);
...
static int ipc_tx_mbox_startup(struct ipc_mbox_device *mdev)
{
	...
	if(unused_thread == 0) {
		...
		mbox_pr_err("ipc_tx_mbox_startup ===== mbx name = %s\n",mdev->name);
