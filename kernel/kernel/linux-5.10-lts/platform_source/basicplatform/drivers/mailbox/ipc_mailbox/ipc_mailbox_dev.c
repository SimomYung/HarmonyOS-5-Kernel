...
#ifdef CONFIG_EXT_INPUTHUB_IPC_COMMU
...
#ifdef CONFIG_IPC_MAILBOX_REMAP_MID
	unsigned int *remap_id;
#endif
...
#ifdef CONFIG_IPC_MAILBOX_REMAP_MID
static void mailbox_remap_init(void)
{
	int ret;
	struct ffa_send_direct_data args = {0};

	args.data0 = FID_MAILBOX_REMAPID_INIT;
	ret = ffa_platdrv_send_msg(&args);
	if (ret != 0)
		pr_err("%s failed to send_msg, ret:%d!\n", __func__, ret);
}

static void ipc_mdev_set_mid2(struct ipc_mbox_device *mdev, int dex)
{
	struct ipc_mbox_device_priv *priv = mdev->priv;
  	int temp = 0;

	if (!strncmp("ao-mailbox-0", mdev->name, strlen("ao-mailbox-0"))) {
		temp = dex | IPC_AOMBX0_TO_LPMCU;
	} else {
		temp = dex | (dex << 8);
	}
	__ipc_writel(priv->idev, (unsigned int)temp,
		(unsigned int)ipc_mbx_mid2(priv->mbox_channel, priv->idev->reg_spc));
}

static int ipc_mdev_read_erecord(struct ipc_mbox_device *mdev)
{
	struct ipc_mbox_device_priv *priv = mdev->priv;
	unsigned int erecord1,erecord2;
 
	erecord1 = __ipc_readl(priv->idev, (unsigned int)ipc_mbx_erecord1(priv->mbox_channel, priv->idev->reg_spc));
	erecord2 = __ipc_readl(priv->idev, (unsigned int)ipc_mbx_erecord2(priv->mbox_channel, priv->idev->reg_spc));
	if(erecord1 || erecord2){
		mdev_err("%s erecord error erecord1 = 0x%x  erecord2 = 0x%x \n", mdev->name, erecord1, erecord2);
		return EPERM;
	}
	return 0;
}
 
static void ipc_mdev_set_eclear(struct ipc_mbox_device *mdev)
{
	struct ipc_mbox_device_priv *priv = mdev->priv;

	__ipc_writel(priv->idev, IPC_SET_ECLEAR,
		(unsigned int)ipc_mbx_eclear(priv->mbox_channel, priv->idev->reg_spc));
}
#endif
...
#ifdef CONFIG_IPC_MAILBOX_REMAP_MID
	ipc_mdev_set_mid2(mdev, idev->remap_id[(unsigned int)priv->des]);
#endif
	/* des config */
	__ipc_set_des(priv);
...
static unsigned int ipc_mdev_unused_thread(struct ipc_mbox_device *mdev)
{
	struct ipc_mbox_device_priv *priv = mdev->priv;
	return priv->unused_thread;
}
...
struct ipc_mbox_dev_ops ipc_mdev_ops = {
	...
	.get_unused_thread =ipc_mdev_unused_thread,
#ifdef CONFIG_IPC_MAILBOX_REMAP_MID
	.read_erecord = ipc_mdev_read_erecord,
	.set_eclear = ipc_mdev_set_eclear,
#endif
};
...
#ifdef CONFIG_IPC_MAILBOX_REMAP_MID
	remap_id = kcalloc(rproc_num, sizeof(unsigned int), GFP_KERNEL);
	if (remap_id == NULL) {
		mdev_err("failed to alloc remap_id \n");
		return -ENOMEM;
	}
	ret = of_property_read_u32_array(node, "remap_id", remap_id, rproc_num);
	if (ret) {
		mdev_err("failed to read remap_id\n");
        kfree(idev->remap_id);
		return -ENODEV;
	}
 
	idev->remap_id = remap_id;
#endif
	for (i = 0; i < RPROC_NUMBER; i++)
	...
	unsigned int used)
{
	...
	ret = of_property_read_u32(son, "unused_thread", &(priv->unused_thread));
	if (ret)
		priv->unused_thread = 0;
...
#ifdef CONFIG_EXT_INPUTHUB_IPC_COMMU
	if (!strncmp("ao-mailbox", idev->mdev_res[0]->name, strlen("ao-mailbox"))) {
		mdev_info("%s: suspend ao-mailbox", __func__);
		commu_suspend(IPC_DRIVER);
	}
#endif
	if (idev)
...
#ifdef CONFIG_EXT_INPUTHUB_IPC_COMMU
	if (!strncmp("ao-mailbox", idev->mdev_res[0]->name, strlen("ao-mailbox"))) {
		mdev_info("%s: resume ao-mailbox", __func__);
		commu_resume(IPC_DRIVER);
	}
#endif
	mdev_info("%s: resume -", __func__);
	return 0;
