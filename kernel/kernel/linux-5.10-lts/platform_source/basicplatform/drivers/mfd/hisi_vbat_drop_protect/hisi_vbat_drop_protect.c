...
static unsigned int cluster_max_perf[ALL];
#endif

static unsigned int g_limit_perf_en;
...
static int hisi_get_perf_lower_limit(struct hisi_vbat_drop_protect_dev *di) {
	int soc, index, ret;
	struct perf_limit_info *perf_limit = NULL;
	struct power_supply *psy = NULL;
	union power_supply_propval val = {0};

	if (di == NULL || di->perf_limit == NULL)
	...
	soc = val.intval;
	pr_info("%s batt_cap:%d\n", __func__, soc);

	for (index = 0; index < di->perf_limit_info_num; index++) {
		perf_limit = &di->perf_limit[index];
		if (soc >= perf_limit->soc_l && soc <= perf_limit->soc_h)
			return perf_limit->perf_lower_limit;
	}
...
static void hisi_vbat_drop_limit_cluster_freq(struct hvdev *hvdev, int freq)
{
	int ret;
...
static int hisi_get_cluster_max_freq(void)
{
	...
	for_each_possible_cpu(cpu) {
		...
		if (!cluster_max_perf[cluster]) {
			cluster_max_perf[cluster] = cpufreq_get_hw_max_freq(cpu);
			if (unlikely(!cluster_max_perf[cluster]))
				goto get_freq_err;
		}
	}

	devfreq = find_devfreq_by_name("gpufreq");
	if (IS_ERR_OR_NULL(devfreq)) {
		goto get_freq_err;
	} else {
		cluster_max_perf[GPU_CPU] = find_available_max_freq(devfreq) / PERMILLAGE;
		if (!cluster_max_perf[GPU_CPU])
			goto get_freq_err;
	}
	return 0;

get_freq_err:
	pr_err("[%s]: cluster max freq is invalid!\n", __func__);
	return -1;
}

static void hisi_vbat_drop_cluster_freq_set(enum vbat_drop_freq freq_type, int perf_lower_limit)
{
	...
	} else if (freq_type == LIMIT_FREQ) {
		pr_err("[%s]limit perf %d of percent\n", __func__, perf_lower_limit);
		if (hisi_get_cluster_max_freq()) {
			pr_err("[%s]hisi_get_cluster_max_freq fail\n", __func__);
			hisi_vbat_drop_restore_cluster_freq(limit_bigfreq_hvdev);
			hisi_vbat_drop_restore_cluster_freq(limit_middlefreq_hvdev);
			hisi_vbat_drop_restore_cluster_freq(limit_littlefreq_hvdev);
			hisi_vbat_drop_restore_cluster_freq(limit_l3freq_hvdev);
			...
			hisi_vbat_drop_restore_cluster_freq(limit_littlefreq_hvdev);
			hisi_vbat_drop_restore_cluster_freq(limit_l3freq_hvdev);
			hisi_vbat_drop_limit_cluster_freq(limit_bigfreq_hvdev,
							cluster_max_perf[BIG_CPU] / FULL_OF_PERCENT * perf_lower_limit);
			hisi_vbat_drop_limit_cluster_freq(limit_middlefreq_hvdev,
							cluster_max_perf[MIDDLE_CPU] / FULL_OF_PERCENT * perf_lower_limit);
			hisi_vbat_drop_limit_cluster_freq(limit_gpufreq_hvdev,
							cluster_max_perf[GPU_CPU] / FULL_OF_PERCENT * perf_lower_limit);
		}
	} else {
...
#ifdef CONFIG_HW_VOTE
			if (g_limit_perf_en == 0)
				hisi_vbat_drop_cluster_freq_set(RESTOR_FREQ, FULL_OF_PERCENT);
			else {
				perf_lower_limt = hisi_get_perf_lower_limit(di);
				hisi_vbat_drop_cluster_freq_set(LIMIT_FREQ, perf_lower_limt);
			}
#endif
...
#endif
#ifdef CONFIG_HW_VOTE
	hisi_vbat_drop_cluster_freq_set(MIN_FREQ, 0);
#endif
...
static int hisi_vbat_drop_restore_freq(
	struct notifier_block *nb, unsigned long event, void *data)
{
	...
	case VCHRG_START_CHARGING_EVENT:
		if (g_limit_perf_en == 1)
			hisi_vbat_drop_cluster_freq_set(RESTOR_FREQ, FULL_OF_PERCENT);
		break;
	default:
		pr_err("%s default run.\n", __func__);
		break;
	}
...
static int hisi_vbat_drop_parse_dts(struct hisi_vbat_drop_protect_dev *di)
{
	...
	if (of_find_property(np, "soc_perf_lower_limit", NULL)) {
		count = of_property_count_elems_of_size(np, "soc_perf_lower_limit", sizeof(int));
		if (count > 0 && !(count % SOC_PERF_LIMIT_CNT)) {
			g_limit_perf_en = 1;
			di->perf_limit_info_num = count / SOC_PERF_LIMIT_CNT;
			di->perf_limit = devm_kzalloc(di->dev, di->perf_limit_info_num * sizeof(struct perf_limit_info), GFP_KERNEL);
			if(!di->perf_limit)
				return -ENOMEM;
 
			for (index = 0; index < di->perf_limit_info_num; index++) {
				if (of_property_read_u32_index(np, "soc_perf_lower_limit",
								index * SOC_PERF_LIMIT_CNT,
								&perf_info[0]))
					return -ENODEV;
				if (of_property_read_u32_index(np, "soc_perf_lower_limit",
								index * SOC_PERF_LIMIT_CNT + 1,
								&perf_info[1]))
					return -ENODEV;
				if (of_property_read_u32_index(np, "soc_perf_lower_limit",
								index * SOC_PERF_LIMIT_CNT + 2,
								&perf_info[2]))
					return -ENODEV;
				di->perf_limit[index].soc_l = perf_info[0];
				di->perf_limit[index].soc_h = perf_info[1];
				di->perf_limit[index].perf_lower_limit = perf_info[2];
			}
		}
	}
...
static void hisi_vbat_drop_vol_set(int mv)
{
	...
	reg_val = _hisi_vbat_drop_vol_set(mv);
...
static int hisi_vbat_drop_protect_probe(struct platform_device *pdev)
{
	...
	di->nb.notifier_call = hisi_vbat_drop_restore_freq;
