		...
		int value)
{
	...
	regmap_update_bits(pmic_gpio->regmap, pmic_gpio->regs.data_reg, BIT(pmic_gpio->regs.data_shift), value << pmic_gpio->regs.data_shift);
	...
	const struct device_node *np)
{
	...
	ret = of_property_read_u32(np, "data-shift", &pmic_gpio->regs.data_shift);
	if (ret)
		pmic_gpio->regs.data_shift = 0;
