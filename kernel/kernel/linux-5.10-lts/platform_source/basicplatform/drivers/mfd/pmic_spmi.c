...
#define SCTRL_IOMAP_SIZE 0x1000

static struct vendor_pmic *g_pmic = NULL;
static struct vendor_pmic *g_mmw_pmic = NULL;
static struct vendor_pmic *g_sub_pmic = NULL;
static struct vendor_pmic *g_sub1_pmic = NULL;
static struct vendor_pmic *g_comm_pmic = NULL;
static struct vendor_pmic *g_iodiev500_pmic = NULL;
static struct vendor_pmic *g_camv500_pmic = NULL;
...
static u32 iodiev500_pmic_read(struct vendor_pmic *pmic, int reg)
{
    return _spmi_pmic_read(g_iodiev500_pmic, reg);
}

static void iodiev500_pmic_write(struct vendor_pmic *pmic, int reg, u32 val)
{
    _spmi_pmic_write(g_iodiev500_pmic, reg, val);
}

static u32 camv500_pmic_read(struct vendor_pmic *pmic, int reg)
{
    return _spmi_pmic_read(g_camv500_pmic, reg);
}

static void camv500_pmic_write(struct vendor_pmic *pmic, int reg, u32 val)
{
    _spmi_pmic_write(g_camv500_pmic, reg, val);
}
...
int pmic_subpmu1_get_dieid(char *dieid, unsigned int len)
{
	int ret;

	ret = _pmic_get_dieid(g_sub1_pmic, dieid, len);
	if (ret)
		pr_err("%s:fail!\n", __func__);
	return ret;
}

int pmic_iodiev500_get_dieid(char *dieid, unsigned int len)
{
    int ret;

    ret = _pmic_get_dieid(g_iodiev500_pmic, dieid, len);
    if (ret)
        pr_err("%s:fail!\n", __func__);
    return ret;
}

int pmic_camv500_get_dieid(char *dieid, unsigned int len)
{
    int ret;

    ret = _pmic_get_dieid(g_camv500_pmic, dieid, len);
    if (ret)
        pr_err("%s:fail!\n", __func__);
    return ret;
}
...
static void _pmic_irq_ipc_mask(
	struct irq_data *d, int maskflag)
{
	struct vendor_pmic *pmic = irq_data_get_irq_chip_data(d);
	u32 data, offset;
	...
	if (pmic == NULL) {
		pr_err(" irq_ipc_mask pmic is NULL\n");
		return;
	}
	/* Convert interrupt data to interrupt offset */
	offset = (irqd_to_hwirq(d) / ONE_IRQ_GROUP_NUM);
	spin_lock_irqsave(&pmic->lock, flags);
	data = pmic->irq_mask_val_arry[offset];
	if (maskflag == PMIC_IRQ_MASK)
		data |= (1 << (irqd_to_hwirq(d) & 0x07)); /* offset mask 3 */
	else
		data &= ~(u32)(1 << (irqd_to_hwirq(d) & 0x07)); /* offset mask 3 */

	pmic->irq_mask_val_arry[offset] = data;
	spin_unlock_irqrestore(&pmic->lock, flags);
}
...
{
	struct vendor_pmic *pmic = irq_data_get_irq_chip_data(d);
	if (pmic->irq_type == IRQ_IPC)	
		_pmic_irq_ipc_mask(d, PMIC_IRQ_MASK);
	else
...
static void pmic_irq_unmask(struct irq_data *d)
{
	struct vendor_pmic *pmic = irq_data_get_irq_chip_data(d);
	if (pmic->irq_type == IRQ_IPC)	
		_pmic_irq_ipc_mask(d, PMIC_IRQ_UNMASK);
	else
...
static int get_pmic_device_tree_opt_data(
	struct spmi_device *pdev, struct vendor_pmic *pmic)
{
	...
	if (of_property_read_bool(np, "pmic-shutdown-opt-reg-ctrl")) {
		/* get pmic optimize register */
		pmic->opt_reg_num = of_property_count_elems_of_size(np,
			"pmic-shutdown-opt-reg-addrs", sizeof(u32));
		if (pmic->opt_reg_num > 0) {
			pmic->opt_reg_addrs = (int *)devm_kzalloc(
				dev, sizeof(int) * pmic->opt_reg_num, GFP_KERNEL);
			if (!pmic->opt_reg_addrs)
				return -ENOMEM;

			ret = of_property_read_u32_array(np,
				"pmic-shutdown-opt-reg-addrs",
				(int *)pmic->opt_reg_addrs, pmic->opt_reg_num);
			if (ret) {
				pr_err("no pmic-shutdown-opt-reg-addrs property\n");
				return -ENODEV;
			}
			pmic->opt_reg_vals = (int *)devm_kzalloc(
				dev, sizeof(int) * pmic->opt_reg_num, GFP_KERNEL);
			if (!pmic->opt_reg_vals)
				return -ENOMEM;
			ret = of_property_read_u32_array(np,
				"pmic-shutdown-opt-reg-vals",
				(int *)pmic->opt_reg_vals, pmic->opt_reg_num);
			if (ret) {
				pr_err("no pmic-shutdown-opt-reg-vals property\n");
				return -ENODEV;
			}
		}
	}
	...
	struct spmi_device *pdev, struct vendor_pmic *pmic)
{
	...
	pmic->handle_irq = of_property_read_bool(np, "handle_irq");
	if (!pmic->handle_irq) {
	...
	pmic->irq_mask_val_arry = (int *)devm_kzalloc(
		dev, sizeof(int) * pmic->irqarray, GFP_KERNEL);
	if (!pmic->irq_mask_val_arry)
		return -ENOMEM;

	for (i = 0; i < pmic->irqarray; i++)
		pmic->irq_mask_val_arry[i] = PMIC_MASK_STATE;
	...
	ret = get_pmic_device_tree_opt_data(pdev, pmic);
	if (ret) {
		pr_err("no pmic_opt_data property set\n");
		return ret;
	}
	return ret;
}
...
static bool is_recovery_mode(void)
{
	...
	g_sctrl_base = ioremap((phys_addr_t)SOC_ACPU_SCTRL_BASE_ADDR, SCTRL_IOMAP_SIZE);
	if (g_sctrl_base == NULL)
		return false;

	if (readl(RECOVERY_MODE_FLAG_REG(g_sctrl_base)) & RECOVERY_MODE_FLAG)
		recovery_mode_flag = true;

	iounmap(g_sctrl_base);
	g_sctrl_base = NULL;
	pr_info("%s: recovery_mode_flag %d\n", __func__, recovery_mode_flag);
	return recovery_mode_flag;
}

static bool is_pmic_irq_ipc_handle(struct vendor_pmic *pmic, uint32_t irq_num)
{
	u32 data, offset;

	if (pmic == NULL || irq_num >= pmic->irqnum)
		return false;

	offset = (irq_num / ONE_IRQ_GROUP_NUM);
	data = pmic->irq_mask_val_arry[offset];

	if (test_bit(irq_num & 0x07, (unsigned long *)&data) == 0) {
		if (irq_num == IRQ_SIM1_UP || irq_num == IRQ_SIM1_DOWN)
			sim_hpd_proc(pmic);
		return true;
	}	
	return false;
...
static int pmic_irq_ipc_handler(const struct pkt_header *pkt)
{
	unsigned long flags;
	struct pmu_ipc_msg *data = (struct pmu_ipc_msg *)pkt;
	...
	pr_info("%s: tag:%u, recv pmu irq id:%u\n", __func__, pkt->tag, data->irq_id);

	if (pkt->tag == TAG_PMU && g_pmic != NULL &&
		is_pmic_irq_ipc_handle(g_pmic, data->irq_id)) {
			local_irq_save(flags);
			generic_handle_irq(g_pmic->irqs[data->irq_id]);
	...
	struct spmi_device *pdev, struct vendor_pmic *pmic, const char *name)
{
	...
	if (pmic->spmi_slv_mode == SPMI_SLV_MODE_TO_2) {
	...
	} else if (pmic->irq_type == IRQ_IPC && !is_recovery_mode()) {
		if (of_device_is_compatible(np, SPMI_PMIC_COMP))
			register_mcu_event_notifier(TAG_PMU, PMU_IPC_NOTIFY, pmic_irq_ipc_handler);
	} else {
		ret = of_irq_get_bygpio(np, pmic, name);
		if (ret < 0)
			return ret;
		pmic->irq_type = IRQ_GPIO;
...
free_gpio:
	if (pmic->irq_type == IRQ_GPIO) {
		gpio_free(pmic->gpio);
	} else if (pmic->irq_type == IRQ_IPC) {
		if (of_device_is_compatible(np, SPMI_PMIC_COMP))
			unregister_mcu_event_notifier(TAG_PMU, PMU_IPC_NOTIFY, pmic_irq_ipc_handler);
	}
	return -ENODEV;
}
...
static void pmic_get_spmi_slv_mode(struct spmi_device *pdev, struct vendor_pmic *pmic)
{
	struct device *dev = &pdev->dev;
	struct device_node *np = dev->of_node;
	int ret;
	...
	ret = of_property_read_u32(np, "spmi_slv_addr",
		(u32 *)&pmic->spmi_slv_addr);
	if (!ret) {
		spmi_slv_mode = _spmi_pmic_read(pmic, pmic->spmi_slv_addr);
		if ((spmi_slv_mode & BIT(0)) == BIT(0))
			pmic->spmi_slv_mode = SPMI_SLV_MODE_TO_2;
	}
}
...
static int cfg_regs_dts_parse(struct spmi_device *pdev, struct vendor_pmic *pmic)
{
	int i, ret;
	struct device *dev = &pdev->dev;
	struct device_node *np = dev->of_node;

	/* Get the number of pmic register */
	ret = of_property_read_u32(np, "pmic-init-reg-num", &pmic->cfg_regs.reg_num);
	if (ret) {
		return -ENODEV;
	}
	if (pmic->cfg_regs.reg_num == 0)
		return 0;

	pmic->cfg_regs.reg_addr = (int *)devm_kzalloc(
		dev, sizeof(int) * (pmic->cfg_regs.reg_num), GFP_KERNEL);
	if (!pmic->cfg_regs.reg_addr)
		return -ENOMEM;

	pmic->cfg_regs.reg_val = (int *)devm_kzalloc(
		dev, sizeof(int) * (pmic->cfg_regs.reg_num), GFP_KERNEL);
	if (!pmic->cfg_regs.reg_val)
		return -ENOMEM;

	ret = of_property_read_u32_array(np, "pmic-init-reg-addrs",
					(u32 *)pmic->cfg_regs.reg_addr, pmic->cfg_regs.reg_num);
	if (ret) {
		pr_info("no pmic-init-reg-addrs property\n");
		return -ENODEV;
	}

	ret = of_property_read_u32_array(np, "pmic-init-reg-vals",
					(u32 *)pmic->cfg_regs.reg_val, pmic->cfg_regs.reg_num);
	if (ret) {
		pr_info("no pmic-init-reg-vals property\n");
		return -ENODEV;
	}
	for (i = 0; i < pmic->cfg_regs.reg_num; i++)
		pr_info("%s pmic-init-reg-addr is 0x%x, pmic-init-reg-val is 0x%x\n",
			__func__, pmic->cfg_regs.reg_addr[i], pmic->cfg_regs.reg_val[i]);
...
static int pmic_probe(struct spmi_device *pdev)
{
	...
	ret = cfg_regs_dts_parse(pdev, pmic);
	...
	ret = of_property_read_u32(np, "irq_type", (u32 *)&(pmic->irq_type));
	if (ret)
		dev_info(&pdev->dev, "irq_type is not set, default is gpio_irq\n");
	else
		dev_info(&pdev->dev, "irq_type is %d\n", pmic->irq_type);

	pmic_get_spmi_slv_mode(pdev, pmic);
...
success:
	if (of_device_is_compatible(np, SPMI_MMW_PMIC_COMP))
		...
		g_sub1_pmic = pmic;
	else if (of_device_is_compatible(np, SPMI_IODIE_PMIC_COMP))
        g_iodiev500_pmic = pmic;
    else if (of_device_is_compatible(np, SPMI_CAM_PMIC_COMP))
        g_camv500_pmic = pmic;
...
static void pmic_shutdown(struct spmi_device *pdev)
{
	...
	for (i = 0; i < pmic->opt_reg_num; i++) {
		_spmi_pmic_write(pmic, pmic->opt_reg_addrs[i],
			pmic->opt_reg_vals[i]);
		pmic->opt_reg_vals[i] = _spmi_pmic_read(pmic, pmic->opt_reg_addrs[i]);
		pr_info("%s i = %d, opt_reg = 0x%x, opt_val = 0x%x\n",
			__func__, i,
			pmic->opt_reg_addrs[i], pmic->opt_reg_vals[i]);
	}
}
...
static int pmic_suspend(struct spmi_device *pdev, pm_message_t state)
{
	int i;
	struct vendor_pmic *pmic = dev_get_drvdata(&pdev->dev);
	...
	if (is_ulsr()) {
		...
		if (pmic->handle_irq) {
			for (i = 0; i < pmic->irqarray; i++) {
				pmic->irq_mask_val_arry[i] = _spmi_pmic_read(pmic, pmic->irq_mask_addr_arry[i]);
				pr_err("PMU sid %d IRQ address value:irq[0x%x] = 0x%x\n", pdev->usid,
					pmic->irq_mask_addr_arry[i], pmic->irq_mask_val_arry[i]);
			}
...
static int pmic_resume(struct spmi_device *pdev)
{
	int i, ret;
	struct vendor_pmic *pmic = dev_get_drvdata(&pdev->dev);
	...
	if (is_ulsr()) {
		...
		for (i = 0; i < pmic->cfg_regs.reg_num; i++) {
			ret = regmap_write(pmic->regmap, pmic->cfg_regs.reg_addr[i], pmic->cfg_regs.reg_val[i]);
			if (ret < 0)
				pr_err("%s regmap write 0x%x failed\n", __func__, pmic->cfg_regs.reg_addr[i]);
			pr_info("%s regmap write 0x%x, value 0x%x\n", __func__, pmic->cfg_regs.reg_addr[i], pmic->cfg_regs.reg_val[i]);
		}
		if (pmic->handle_irq) {
			for (i = 0; i < pmic->irqarray; i++) {
				_spmi_pmic_write(pmic, pmic->irq_mask_addr_arry[i], pmic->irq_mask_val_arry[i]);
				pr_err("PMU sid %d IRQ address value:irq[0x%x] = 0x%x\n", pdev->usid,
					pmic->irq_mask_addr_arry[i], pmic->irq_mask_val_arry[i]);
			}
			enable_irq(pmic->irq);
...
static const struct of_device_id of_spmi_pmic_match_tbl[] = {
	...
	{ .compatible = SPMI_IODIE_PMIC_COMP, },
	{ .compatible = SPMI_CAM_PMIC_COMP, },
	{  }   /* end */
};
