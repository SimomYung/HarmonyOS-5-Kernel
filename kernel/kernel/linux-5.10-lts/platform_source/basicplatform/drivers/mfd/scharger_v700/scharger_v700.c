...
{
	struct scharger_v700_priv *scharger = context;

	rt_mutex_lock(&scharger->i2c_lock);
}
...
static void regmap_unlock_mutex(void *context)
{
	...
	rt_mutex_unlock(&scharger->i2c_lock);
}

static struct regmap_config scharger_regmap_config = {
	.reg_bits = 16,
	.val_bits = 8,
	.max_register = 0x7fff,
	.can_sleep = true,
	...
	const struct i2c_device_id *id)
{
	...
	g_scharger = scharger;
	scharger_regmap_config.lock_arg = scharger;
	...
	(void)hisi_tcpc_irq_gpio_register(scharger->gpio);
