...
EXPORT_SYMBOL(lb_get_available_size);
...
EXPORT_SYMBOL(lb_up_policy_prio);

int lb_down_policy_prio(u32 pid)
{
	return 0;
}
EXPORT_SYMBOL(lb_down_policy_prio);
...
u64 lb_pid_to_gidphys(u32 pgid)
{
	...
	return PTE_LB(pgid & PGID_LOW_BITS_MASK) | PBHA_GID_HIGH_BIT((pgid & PGID_HIGH_BITS_MASK) >> 4);
#else
	return PTE_LB(pgid);
#endif
}
...
int lb_pages_detach(u32 pgid, struct page *pages, size_t count)
{
	...
	if (!cpu_p_info->page_count)
		lb_gid_bypass(cpu_pgid);

	if (p_info && !p_info->page_count)
		lb_gid_bypass(pgid);
