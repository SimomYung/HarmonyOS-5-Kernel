...
#ifdef CONFIG_ZODIAC_SHARE_MMC
	if(mmc_status_check(EMMC_USE_FLAG) == SENSOR_USE_FLAG) {
		pr_err("%s: sensor is using emmc , cant reset mmc\n",
				mmc_hostname(card->host));
		pr_err("%s out\n", __func__);
		return 0;
	}
...
#ifdef CONFIG_ZODIAC_SHARE_MMC
	unsigned int hwlock_try_count = 2000; /* wait hwlock host 2s */
#endif
	int ret = 0;
...
#ifdef CONFIG_ZODIAC_SHARE_MMC
	if (card->host->index == 0 && card->host->emmc_hwlock) {
		if (emmc_hwspin_lock_timeout(card->host->emmc_hwlock, hwlock_try_count)) {
			pr_err("%s claim host try to emmc_hwlock failed\n", __func__);
			ret = -EIO;
			goto release_host;
		} else
			card->host->emmc_hwlock_state = 1;
...
void *mmc_sctrl_reg;
void *mmc_actrl_reg;
void mmc_status_mark(u32 attribute_flag, u32 use_flag)
{
	u32 reg_val;
	if (unlikely(!mmc_sctrl_reg))
		mmc_sctrl_reg = ioremap(0xFF717000, 0x1000);

	if (unlikely(!mmc_actrl_reg))
		mmc_actrl_reg = ioremap(0xFF586000, 0x1000);

	reg_val = readl(mmc_sctrl_reg + 0x4C4);

	switch(attribute_flag) {
	case EMMC_AP_POWER_ATTRIBUTE:
		reg_val &= ~0x7;
		break;
	case EMMC_SENSOR_POWER_ATTRIBUTE:
		reg_val &= ~0x38;
		break;
	case EMMC_STATUS_ATTRIBUTE:
		reg_val &= ~0xE00;
		if (use_flag == AP_USE_FLAG)
			writel(0x1, mmc_actrl_reg + 0x40C);
		break;
	case EMMC_INATIAL_ATTRIBUTE:
		reg_val &= ~(0x1000);
		break;
	case EMMC_APPLY_FLAG:
		reg_val &= ~(0x6000);
		break;
	case EMMC_PART_FLAG:
		reg_val &= ~(0x8000);
	...
	reg_val |= use_flag;
	writel((reg_val | (0xFFFF << 16)), (mmc_sctrl_reg + 0x4C4));
}

u32 mmc_status_check(u32 flag)
{
	u32 reg_val;
