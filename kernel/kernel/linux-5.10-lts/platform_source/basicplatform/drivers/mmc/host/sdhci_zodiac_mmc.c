...
#include <platform_include/basicplatform/linux/mmc/sdhci_zodiac_mmc_orion.h>
...
#include <platform_include/cee/linux/str.h>
#endif
...
void sdhci_zodiac_dumpregs(struct sdhci_host *host)
{
	...
	SDHCI_DUMP("EMMC_PHY_PHYINITCTRL:0x%08x | EMMC_PHY_DATENA_DLY:0x%08x \n",
		sdhci_mphy_readl(host, EMMC_PHY_PHYINITCTRL), sdhci_mphy_readl(host, EMMC_PHY_DATENA_DLY));
	SDHCI_DUMP("EMMC_PHY_PHYCTRL2:0x%08x | EMMC_PHY_DLY_CTL:0x%08x \n",
		sdhci_mphy_readl(host, EMMC_PHY_PHYCTRL2), sdhci_mphy_readl(host, EMMC_PHY_DLY_CTL));
	SDHCI_DUMP("EMMC_PHY_DLY_CTL1:0x%08x | EMMC_PHY_DLY_CTL2:0x%08x \n",
		sdhci_mphy_readl(host, EMMC_PHY_DLY_CTL1), sdhci_mphy_readl(host, EMMC_PHY_DLY_CTL2));
    SDHCI_DUMP("EMMC_PHY_TUNING_CONFIG:0x%08x\n",
		sdhci_mphy_readl(host, EMMC_PHY_TUNING_CONFIG));
    SDHCI_DUMP("SC_BAKDATA5_MASK:0x%08x\n",
		readl(host->sctrl + SC_BAKDATA5_MASK));
...
void sdhci_zodiac_gpio_value_set(int value, int gpio_num)
{
	...
	pr_err("%s mmc gpio num: %d, lever_value: %d\n", __func__, gpio_num, value);
...
#ifdef CONFIG_MMC_SDHCI_ZODIAC_APUS
	pr_err("%s: sdhci_zodiac_disable_clk litecrg 0x340 =0x%x\n", __func__, sdhci_litecrg_readl(host, 0x340));
#endif
...
#ifdef CONFIG_MMC_SDHCI_ZODIAC_APUS
	pr_err("%s: sdhci_enable_clk litecrg 0x340 =0x%x\n", __func__, sdhci_litecrg_readl(host, 0x340));
#endif
...
#ifdef CONFIG_MMC_SDHCI_ZODIAC_APUS
#define SOC_ACPU_GIC600_BASE_ADDR                               (0xEE000000)
static void __iomem *sd_intr_gic_addr = NULL;
static void print_gic_addr_sd_intr(void)
{
	u32 p, end;

	pr_err("cpu online mask:%llx\n", cpumask_bits(cpu_online_mask)[0]);
	if (!sd_intr_gic_addr)
		sd_intr_gic_addr = ioremap(SOC_ACPU_GIC600_BASE_ADDR, 0x300);
	...
	for (p = 0x100, end = 0x180; p < end; p+=32)
		pr_err("%08x: %08x %08x %08x %08x %08x %08x %08x %08x\n",
				...
				*((u32*)(sd_intr_gic_addr + p + 0)),
				*((u32*)(sd_intr_gic_addr + p + 4)),
				*((u32*)(sd_intr_gic_addr + p + 8)),
				*((u32*)(sd_intr_gic_addr + p + 12)),
				*((u32*)(sd_intr_gic_addr + p + 16)),
				*((u32*)(sd_intr_gic_addr + p + 20)),
				*((u32*)(sd_intr_gic_addr + p + 24)),
				*((u32*)(sd_intr_gic_addr + p + 28))
				);
...
static int sdhci_zodiac_execute_hwtuning(struct sdhci_host *host, u32 opcode)
{
	...
	for (i = 0; i < MAX_TUNING_LOOP; i++) {
		...
		if (!host->tuning_done) {
			pr_err("%s: Tuning timeout, falling back to fixed sampling clock i = 0x%x\n",
					mmc_hostname(host->mmc), i);
			sdhci_dumpregs(host);
#ifdef CONFIG_MMC_SDHCI_ZODIAC_APUS
			print_gic_addr_sd_intr();
#endif
...
static void sdhci_zodiac_power_off(struct sdhci_host *host)
{
	...
	if (sdhci_zodiac->lever_shift) {
#ifdef CONFIG_MMC_SDHCI_ZODIAC_ARA
		sdhci_zodiac_gpio_value_set(LEVER_SHIFT_3_0V, sdhci_zodiac->lever_shift_ionumb);
#else
		sdhci_zodiac_gpio_value_set(LEVER_SHIFT_1_8V, sdhci_zodiac->lever_shift_ionumb);
#endif
	}
}
...
#endif
#ifdef CONFIG_MMC_SDHCI_ZODIAC_APUS
	.delay_measurement = sdhci_zodiac_mphy_delay_line,
#endif
};
...
static int sdhci_zodiac_suspend(struct device *dev)
{
	...
	dev_err(dev, "%s: suspend +\n", __func__);
#ifdef CONFIG_SYS_PM_STR
	if (is_str() && (sdhci_zodiac->sdhci_host_id == SDHCI_SDIO_ID)) {
		pr_info("%s:bypass sdio suspend when mode is str\n", __func__);
		return 0;
	}
#endif

	if (host && host->mmc && host->mmc->card) {
		dev_info(dev, "%s: bus_completion_done = 0x%x\n", __func__,
			host->mmc->card->dev.power.completion.done);
		if (!(mmc->bus_suspend_done)) {
			dev_info(dev, "%s: mmc_bus is not suspended\n", __func__);
			return -1;
		}
	}

#ifdef CONFIG_ZODIAC_SHARE_MMC
	if (host->mmc->emmc_hwlock) {
		mmc_status_mark(EMMC_AP_POWER_ATTRIBUTE, 0x2);
		dev_err(dev, "%s: dont nedd suspend -\n", __func__);
		return 0;
	}
#endif
...
#ifdef CONFIG_SYS_PM_STR
	if (is_str() && (sdhci_zodiac->sdhci_host_id == SDHCI_SDIO_ID)) {
		pr_info("%s:bypass sdio resume when mode is str\n", __func__);
		return 0;
	}
#endif
...
retry:
	if (host->mmc->emmc_hwlock) {
		if (emmc_hwspin_lock_timeout(host->mmc->emmc_hwlock, EMMC_LOCK_TIMEOUT)) {
			pr_err("mmc request hwspin lock failed, AP mmc try request hwspin lock\n");
			mmc_status_mark(EMMC_APPLY_FLAG, AP_WANT_USE_FLAG);
			goto retry;
		} else {
			pr_info("%s get lock success cnt %d claim_cnt:%d\n", __func__, host->mmc->hwspink_cnt, host->mmc->claim_cnt);
			host->mmc->emmc_hwlock_state = 1;
			mmc_status_mark(EMMC_STATUS_ATTRIBUTE, AP_USE_FLAG);
			status = mmc_status_check(EMMC_INATIAL_ATTRIBUTE);
			if (status == 0x1000) {
				/* sensor in user,AP no need open clock */
				mmc_status_mark(EMMC_AP_POWER_ATTRIBUTE, 0x1);
				pr_err("ap mmc dont need resume\n");
				goto out;
			}
		}
	}
...
#ifdef CONFIG_MMC_SDHCI_ZODIAC_APUS
	sdhci_zodiac_mphy_clk_init(host);
#endif
...
#ifdef CONFIG_MMC_SDHCI_ZODIAC_APUS
	if (sdhci_zodiac->sdhci_host_id == 2) {
		cpumask_setall(&new_mask);
		cpumask_clear_cpu(0, &new_mask);
		irq_set_affinity_hint(host->irq, &new_mask);
	}
#endif
...
out:
	mmc_status_mark(EMMC_AP_POWER_ATTRIBUTE, 0x1);
	if (host->mmc->emmc_hwlock && host->mmc->emmc_hwlock_state) {
		emmc_hwspin_unlock(host->mmc->emmc_hwlock);
		host->mmc->emmc_hwlock_state = 0;
		mmc_status_mark(EMMC_STATUS_ATTRIBUTE, AP_DONT_USE_FLAG);
		pr_err("%s release lock success cnt %d claim_cnt:%d\n", __func__, host->mmc->hwspink_cnt, host->mmc->claim_cnt);
	}
#endif
	dev_info(dev, "%s: resume -\n", __func__);
...
#ifdef CONFIG_SYS_PM_STR
static int sdchi_zodiac_sdio_str_suspend(void)
{
	int ret;
	unsigned int clk;
	struct sdhci_host *host = sdhci_sdio_host;
	struct sdhci_pltfm_host *pltfm_host = sdhci_priv(host);
	struct sdhci_zodiac_data *sdhci_zodiac = pltfm_host->priv;

	pr_info("%s:  sdio str suspend ++\n", __func__);
	pm_runtime_get_sync(mmc_dev(host->mmc));

	sdhci_zodiac_hardware_disreset(host);

	clk = readl(host->pericrg + PERI_CRG_PERSTAT6);
	clk |= GT_CLK_SDIO_HF_SD;
	writel(clk, (host->pericrg + PERI_CRG_PEREN6));
 
	clk = readl(host->hsdtcrg + HSDT_CRG_PERSTAT0);
	clk |= GT_ACLK_SDIO;
	writel(clk, host->hsdtcrg);
	ret = sdhci_suspend_host(host);
	if (ret)
		return ret;
	...
	sdhci_zodiac->str_enter_end = 1;
	pm_runtime_mark_last_busy(mmc_dev(host->mmc));
	pm_runtime_put_autosuspend(mmc_dev(host->mmc));

	writel(SDHCI_ZODIAC_SDIO_AP_SUSPEND, (host->sctrl + SC_BAKDATA5_MASK));
	pr_info("%s:  sdio str suspend --\n", __func__);
	return 0;
}
static int sdchi_zodiac_sdio_str_resume(void)
{
	struct sdhci_host *host = sdhci_sdio_host;
	struct sdhci_pltfm_host *pltfm_host = sdhci_priv(host);
	struct sdhci_zodiac_data *sdhci_zodiac = pltfm_host->priv;
	...
	sdhci_zodiac_clock_enable(mmc_dev(host->mmc), sdhci_zodiac);

	pwr = sdhci_readb(host, SDHCI_POWER_CONTROL);
	pwr |= SDHCI_POWER_ON;
	sdhci_writeb(host, pwr, SDHCI_POWER_CONTROL);
	sdhci_zodiac_enable_clk(host);
	pr_info("%s: host->mmc->ios.clock=%d, timing=%d.\n", __func__, host->mmc->ios.clock, host->mmc->ios.timing);
	...
	enable_irq(host->irq);
	sdhci_zodiac->str_enter_end = 0;
	pm_runtime_mark_last_busy(mmc_dev(host->mmc));
	pm_runtime_put_autosuspend(mmc_dev(host->mmc));

	writel(SDHCI_ZODIAC_SDIO_AP_RESUME, (host->sctrl + SC_BAKDATA5_MASK));
	pr_info("%s: sdio str resume --\n", __func__);
	return ret;
}

static int sdchi_zodiac_sdio_str_enter_notify(struct notifier_block *notify_block, unsigned long mode, void *unused)
{
	int ret;

	pr_info("%s: sdio_str_enter_notify ++\n", __func__);
	if (mode != PM_STR_ENTER) {
		pr_err("%s, sdio enter error \n", __func__);
		return -1;
	}
 
	/* enter str mode */
	ret = sdchi_zodiac_sdio_str_suspend();
	if (ret) {
		pr_err("%s, sdio enter str mode fail \n", __func__);
		return ret;
	}
...
static int sdchi_zodiac_sdio_str_exit_notify(struct notifier_block *notify_block, unsigned long mode, void *unused)
{
	int ret;

	pr_info("%s: sdio_str_exit_notify ++\n", __func__);
	if (mode == PM_STR_ENTER) {
		pr_err("%s, sdio exit error \n", __func__);
		return -1;
	}
 
	/* exit str mode */
	ret = sdchi_zodiac_sdio_str_resume();
	if (ret) {
		pr_err("%s, sdio str exit mode fail \n", __func__);
		return ret;
	}
...
static struct notifier_block sdchi_zodiac_str_enter_notify= {
	.notifier_call = sdchi_zodiac_sdio_str_enter_notify,
	.priority = 7,
};
 
static struct notifier_block sdchi_zodiac_str_exit_notify= {
	.notifier_call = sdchi_zodiac_sdio_str_exit_notify,
	.priority = 10,
};

static void sdchi_zodiac_sdio_str_init(struct sdhci_host *host)
{
	struct sdhci_pltfm_host *pltfm_host = sdhci_priv(host);
	struct sdhci_zodiac_data *sdhci_zodiac = pltfm_host->priv;
	...
	if (register_pm_str_notifier(PM_STR_ENTER, &sdchi_zodiac_str_enter_notify) < 0)
		pr_err("register sdio pm_enter_notifier failed!");
 
	if (register_pm_str_notifier(PM_STR_EXIT, &sdchi_zodiac_str_exit_notify) < 0)
		pr_err("register sdio pm_exit_notifier failed!");
}
#endif
...
static void sdhci_zodiac_populate_dt(struct platform_device *pdev)
{
	...
	sdhci_zodiac->lever_shift_ionumb = of_get_named_gpio(np, "lever-shift-ionumb", 0);
	dev_err(&pdev->dev, "%s: %s lever_shift gpio = %d\n", dev_name(sdhci_zodiac->dev), __func__, 
		sdhci_zodiac->lever_shift_ionumb);

#if defined(CONFIG_MMC_SDHCI_MUX_SDSIM)
	if (sdhci_zodiac->mux_sdsim) {
	...
	if (of_find_property(np, "sdio_support_max_sdr50", NULL)) {
		dev_info(&pdev->dev, "sdhci:%d %s find sdio_support_max_sdr50 in dts\n", sdhci_zodiac->sdhci_host_id,  __func__);
...
static int sdhci_zodiac_clock_init(struct sdhci_host *host, struct platform_device *pdev, struct sdhci_zodiac_data *sdhci_zodiac)
{
	...
	ret = sdhci_zodiac_get_clock_gt(pdev, sdhci_zodiac);
	if (ret)
		return ret;
...
#ifdef CONFIG_MMC_SDHCI_ZODIAC_APUS
	dev_err(&pdev->dev, "%s: after clk_enable litecrg 0x340 =0x%x 0x18 =0x%x 0x1C =0x%x\n", __func__,
		sdhci_litecrg_readl(host, 0x340), sdhci_litecrg_readl(host, 0x18), sdhci_litecrg_readl(host, 0x1C));
#endif
	dev_err(&pdev->dev, "%s: ciu_clk=%lu\n", __func__, clk_get_rate(sdhci_zodiac->ciu_clk));
...
static int sdhci_zodiac_probe(struct platform_device *pdev)
{
	...
	sdhci_zodiac->data = (void *)host;
	INIT_WORK(&sdhci_zodiac->dsm_work, sdhci_dsm_work);
	spin_lock_init(&sdhci_zodiac->sdhci_dsm_lock);
#endif
#ifdef CONFIG_RPMB_MMC
	(void)rpmb_mmc_init();
#endif
#ifdef CONFIG_SYS_PM_STR
	(void)sdchi_zodiac_sdio_str_init(host);
#endif
...
static const struct of_device_id sdhci_zodiac_of_match[] = {
	...
	{.compatible = "hisilicon,xian-sdhci"},
	{.compatible = "hisilicon,orion-sdhci"},
	{}
};
