...
static void set_bl31_excepion_info(struct rdr_exception_info_s *einfo)
{
	/* register rdr exception type */
	if (memset_s((void *)einfo, sizeof(struct rdr_exception_info_s), 0, sizeof(struct rdr_exception_info_s)) != EOK)
	...
	einfo->e_notify_core_mask = RDR_RDA_AP;
	einfo->e_reset_core_mask = RDR_RDA_AP;
	einfo->e_from_core = RDR_RDA_AP;
#else
	...
	if (memcpy_s((void *)einfo->e_from_module, sizeof(einfo->e_from_module), (const void *)"RDR BL31 PANIC", sizeof("RDR BL31 PANIC")) != EOK)
		BB_PRINT_ERR("%s():%d:memcpy_s fail!\n", __func__, __LINE__);
	if (memcpy_s((void *)einfo->e_desc, sizeof(einfo->e_desc), (const void *)"RDR BL31 PANIC", sizeof("RDR BL31 PANIC")) != EOK)
		BB_PRINT_ERR("%s():%d:memcpy_s fail!\n", __func__, __LINE__);
}
