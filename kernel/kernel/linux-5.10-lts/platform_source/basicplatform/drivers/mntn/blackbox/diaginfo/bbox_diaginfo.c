...
#define BBOX_DIAGINFO_FILE      BBOX_CONCAT_TO_PATH_ROOT(diaginfo.log)
#define BBOX_DIAGINFO_FILE_BACK BBOX_CONCAT_TO_PATH_ROOT(diaginfo_1.log)
#else
...
static struct wait_queue_head g_bbox_diaginfo_wq;
static DECLARE_WAIT_QUEUE_HEAD(g_bbox_diaginfo_wq);
...
#ifdef CONFIG_DFX_DIAGINFO_RT_SWITCH
#define __DIAGINFO_MAP(x, y, z, k, i)  { x, #x, y, z, k, 0 , i},
#else
...
static int bbox_diaginfo_save_to_buffer(const struct bbox_diaginfo_param_s *pinfo)
{
	 ...
	 * if support rt_switch, 
	 * * Time [], Module [], Error_ID [Name:NO], Rt_Switch [], Log_level [], Fault_Type [], Data []
	 */
#ifdef CONFIG_DFX_DIAGINFO_RT_SWITCH
	ret = sprintf_s(pmsg, DIAGINFO_STRING_MAX_LEN,
			"Time [%s], Module [%s], Error_ID [%s:%u], Rt_Switch [%u], Log_Level [%s], Fault_Type [%s], Data [%s]\n",
			pinfo->diaginfo.date,
			p_diaginfo.module,
			p_diaginfo_map->error_id_name,
			p_diaginfo_map->errid,
			p_diaginfo_map->rt_switch,
			p_diaginfo.loglevel,
			p_diaginfo.type,
			pinfo->diaginfo.msg);
#else
...
static int bbox_diaginfo_main_thread(void *arg)
{
	...
	while (!kthread_should_stop()) {
		wait_event_freezable(g_bbox_diaginfo_wq,
				!list_empty(&g_bbox_diaginfo_list) ||
				kthread_should_stop());
...
#ifndef CONFIG_MNTN_RDA_DIAGINFO
