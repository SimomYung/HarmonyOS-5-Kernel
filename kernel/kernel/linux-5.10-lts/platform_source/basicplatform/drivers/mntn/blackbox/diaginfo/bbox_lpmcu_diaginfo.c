...
#define PSCI_MSG_TYPE_AP_DIAGINFO_READDY IPC_CMD(OBJ_AP, OBJ_LPM3, CMD_INQUIRY, 5)
#define DIAGINFO_MBX_MSG_LEN  2
#define DIAGINFO_AP_READY  0
#define DIAGINFO_AP_NOTREADY  (-1)
static int g_diaginfo_ap_ready = DIAGINFO_AP_NOTREADY;

void reset_diaginfo_ap_state(void)
{
	BB_PRINT_ERR("%s: entry \n", __func__);
	g_diaginfo_ap_ready = DIAGINFO_AP_NOTREADY;
	up(&g_bbox_telemntn_sem);
}

static void notify_lpmcu_diaginfo_ap_ready(void)
{
	while (get_bbox_diaginfo_write_async_flag() == 0)
		msleep(30);

	if (g_diaginfo_ap_ready != DIAGINFO_AP_READY) {
		rproc_msg_t msg[DIAGINFO_MBX_MSG_LEN] = {0};
		msg[0] = PSCI_MSG_TYPE_AP_DIAGINFO_READDY;
		/* notify lpm3 success */
		if (RPROC_ASYNC_SEND(IPC_ACPU_LPM3_MBX_5, msg, DIAGINFO_MBX_MSG_LEN) == 0)
			g_diaginfo_ap_ready = DIAGINFO_AP_READY;
		BB_PRINT_ERR("notify_lpmcu_diaginfo_ap_ready return %d\n", g_diaginfo_ap_ready);
	}
	return;
}
...
static int bbox_lpmcu_diaginfo_main_thread(void *arg)
{
	...
	while (!kthread_should_stop()) {
		notify_lpmcu_diaginfo_ap_ready();
