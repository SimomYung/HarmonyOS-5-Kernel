...
#include <platform_include/basicplatform/linux/mntn_factory_rt_flag.h>
#endif
...
static int get_reboot_reason_str(const void *rdr_memory, size_t size, char *out_buf, size_t buf_size)
{
	int ret;
	u32 reboot_type_value;
	u32 reboot_subtype_value;
	char *reboot_type_name = NULL;
	char *reboot_subtype_name = NULL;
	(void)(rdr_memory);
	(void)(size);

	reboot_type_value = rdr_get_reboot_type();
	reboot_subtype_value = rdr_get_exec_subtype_value();
	reboot_type_name = rdr_get_exception_type(reboot_type_value);
	reboot_subtype_name = rdr_get_subtype_name(reboot_type_value, reboot_subtype_value);

	ret = sprintf_s(out_buf, buf_size, "%s-%s;", reboot_type_name, reboot_subtype_name);
	if (ret < 0) {
		pr_err("%s():%d sprintf_s ERROR\n", __func__, __LINE__);
		return -1;
	}
	return ret;
}
...
int dfx_diaginfo_get_base_info(char *buf, size_t size)
{
	...
	if (ret != EOK) {
		pr_err("bbox diaginfo get base info failed, copy string return %d\n", ret);
		return -1;
	}
