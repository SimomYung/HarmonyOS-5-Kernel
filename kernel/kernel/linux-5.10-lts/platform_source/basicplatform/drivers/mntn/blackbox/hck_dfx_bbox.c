...
#define DFX_MBOX_MSG_LEN  2
static void hck_dfx_ap_notify_lpm3_vm_off(bool state)
{
	int ret = 0;
	rproc_msg_t msg[DFX_MBOX_MSG_LEN] = {PSCI_MSG_TYPE_M3_CTXSAVE, 0};
	if (likely(state)) {
		msg[1] = DFX_BB_MOD_VMSOLO_START;
		ret = RPROC_ASYNC_SEND(IPC_ACPU_LPM3_MBX_5, msg, (rproc_msg_len_t)DFX_MBOX_MSG_LEN);
		pr_err("%s: ret is %d", __func__, ret);
	}
}
#else
static inline void hck_dfx_ap_notify_lpm3_vm_off(bool state) {}
#endif
...
static int __init hck_dfx_bbox_register(void)
{
	...
	REGISTER_HCK_VH(hck_dfx_ap_notify_lpm3_vm_off, hck_dfx_ap_notify_lpm3_vm_off);
