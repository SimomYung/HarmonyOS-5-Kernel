...
#elif defined(CONFIG_LIBLINUX_CDC_RDA)
...
#endif
#ifdef CONFIG_DFX_EXC_INFO
static dfx_exc_info_t g_dfx_exc_info_get_fn = NULL;
static struct dfx_exc_info_s g_dfx_exc_info_rec;
#endif
...
void set_exception_info(struct dfx_exc_info_func_s *info)
{
#ifdef CONFIG_DFX_EXC_INFO
	struct timespec64 temp_time;
	ktime_get_real_ts64(&temp_time);
	if (info->flag == DFX_EXC_INFO_ESR) {
		g_dfx_exc_info_rec.esr = info->esr;
		g_dfx_exc_info_rec.far = info->far;
		BB_PRINT_PN("[%s], esr = 0x%llx, far = 0x%llx\n",
			__func__, 
			g_dfx_exc_info_rec.esr, 
			g_dfx_exc_info_rec.far);
	}
	if (info->flag == DFX_EXC_INFO_REGS) {
		g_dfx_exc_info_rec.cpuid = smp_processor_id();
		g_dfx_exc_info_rec.lr = (info->regs->regs[30]) - kaslr_offset();
		g_dfx_exc_info_rec.sp = (info->regs->sp) - kaslr_offset();
		g_dfx_exc_info_rec.pc = (info->regs->pc) - kaslr_offset();
		g_dfx_exc_info_rec.ptime = (u64)temp_time.tv_sec;
		BB_PRINT_PN("[%s], cpuid = %u, lr = 0x%llx, sp = 0x%llx, pc = 0x%llx, ptime = %llu\n", 
			__func__, 
			g_dfx_exc_info_rec.cpuid, 
			g_dfx_exc_info_rec.lr, 
			g_dfx_exc_info_rec.sp, 
			g_dfx_exc_info_rec.pc, 
			g_dfx_exc_info_rec.ptime);
		if (g_dfx_exc_info_rec.esr == 0) {
			g_dfx_exc_info_rec.esr = info->esr;
			g_dfx_exc_info_rec.far = info->far;
			BB_PRINT_PN("[%s], esr = 0x%llx, far = 0x%llx\n",
				__func__, 
	...
	exception_buf_len = (unsigned long)sprint_symbol(exception_buf, (unsigned long)instruction_pointer(info->regs));
}
#else
...
int save_mntndump_log(void *arg)
{
	...
	if (vfs_stat(SRC_LB_MASTER_STATE_MEMORY, &mem_stat) == 0)
		save_lb_master_state_memory();
#endif

#ifdef CONFIG_CLOCK_DFX_LOG_SAVE
	if (vfs_stat(CLOCK_MNTN_LOG_FILE, &mem_stat) == 0)
		save_clock_logbuff();
#endif
...
void save_dfx_ap_log(char *log_path, u32 modid)
{
	...
	int ret;
	unsigned int path_root_len = (unsigned int)strlen(PATH_ROOT);
...
#else
	temp.e_from_core = RDR_RDA_AP;
	temp.e_reset_core_mask = RDR_RDA_AP;
#endif
	...
	ret = save_mntndump_log(log_path);
	if (ret)
...
void rdr_ap_reset(u32 modid, u32 etype, u64 coreid)
{
	...
	if ((modid == MODID_AP_S_COMBINATIONKEY) && (etype == AP_S_COMBINATIONKEY)) {
		preempt_enable();
		liblinux_reboot("AP_S_COMBINATIONKEY", true);
		BB_PRINT_PN("%s end\n", __func__);
		return;
	}
#endif
...
enum ap_panic_id {
	INSTRUCTION_ABORT0 = 0x20,
	INSTRUCTION_ABORT1 = 0x21,
	DATA_ABORT0 = 0x24,
	DATA_ABORT1 = 0x25,
	SERROR = 0x2F,
};

static struct ap_panic_trans g_ap_panic_trans[] = {
	{ INSTRUCTION_ABORT0, MODID_AP_S_PANIC_INSTRUCTION_ABORT },
	{ INSTRUCTION_ABORT1, MODID_AP_S_PANIC_INSTRUCTION_ABORT },
	{ DATA_ABORT0, MODID_AP_S_PANIC_DATA_ABORT },
	{ DATA_ABORT1, MODID_AP_S_PANIC_DATA_ABORT },
	{ SERROR, MODID_AP_S_PANIC_SERROR },
};
#endif

static unsigned int translate_ap_panic(void)
{
	u32 module_id = MODID_AP_S_PANIC;
#if defined (CONFIG_DFX_EXC_INFO) && defined(CONFIG_DFX_EXPAND_AP_PANIC)
	u32 i;
	u64 esr;

	esr = g_dfx_exc_info_rec.esr;
	for (i = 0; i < ARRAY_SIZE(g_ap_panic_trans); i++)
		if (ESR_ELx_EC(esr) == g_ap_panic_trans[i].ap_panic_id) {
			module_id = g_ap_panic_trans[i].module_value;
			break;
		}
#endif
	return module_id;
				...
				unsigned long event, void *buf)
{
...
#ifdef CONFIG_DFX_EXC_INFO
	if (g_dfx_exc_info_get_fn != NULL) {
		BB_PRINT_PN("[%s]: g_dfx_exc_info is using\n", __func__);
		BB_PRINT_PN("[%s], g_dfx_exc_info_rec cpuid = %u, esr = 0x%llx, far = 0x%llx\n",
			__func__, 
			g_dfx_exc_info_rec.cpuid, 
			g_dfx_exc_info_rec.esr, 
			g_dfx_exc_info_rec.far);
		BB_PRINT_PN("lr = 0x%llx, sp = 0x%llx, pc = 0x%llx, ptime = %llu\n", 
			g_dfx_exc_info_rec.lr, 
			g_dfx_exc_info_rec.sp, 
			g_dfx_exc_info_rec.pc, 
			g_dfx_exc_info_rec.ptime);
		g_dfx_exc_info_get_fn(g_dfx_exc_info_rec);
	}
#endif
...
#ifdef CONFIG_HOTPLUG_CPU_INFO
	hwdiag_apanic_info.cpu_info.hpcpu_map = get_cpuhp_map();
#endif
	...
	if (watchdog_softlockup_happen()) {
...
#ifdef CONFIG_DFX_EXPAND_AP_PANIC
	} else if (watchdog_hungtask_happen()) {
		rdr_syserr_process_for_ap(MODID_AP_S_PANIC_HUNGTASK, 0, 0);
#endif
	} else {
#ifdef PMU_SAVE_CPU_ID
		...
		if (is_modem_exception())
			...
			rdr_syserr_process_for_ap(translate_ap_panic(), 0, 0);
	}
...
#ifdef CONFIG_DFX_EXC_INFO
void register_dfx_exc_info(dfx_exc_info_t func) 
{
	if (func == NULL) {
		BB_PRINT_PN("[%s]: the func of dfx_exc_info is null\n", __func__);
		return;
	}
	g_dfx_exc_info_get_fn = func;
	BB_PRINT_PN("[%s]: the func of dfx_exc_info is registed\n", __func__);
	return;
}
#endif
