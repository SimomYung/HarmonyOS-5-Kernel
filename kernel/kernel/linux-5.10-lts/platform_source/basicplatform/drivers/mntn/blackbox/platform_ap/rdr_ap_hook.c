...
static const char *g_hook_trace_pattern[HK_MAX] = {
	...
	"task_switch::ktime,stack,pid,priority,comm", /* TASK */
...
#else
static const char *g_hook_trace_pattern[HK_MAX] = {
	[HK_DIAGINFO] = "diaginfo::Time,Module,Error_ID,Log_Level,Fault_Type,Data",
};
#endif
...
#if (defined(CONFIG_PM) && defined(CONFIG_DFX_MNTN_S4))
...
void worker_recent_search(u8 cpu, u64 basetime, u64 delta_time)
{
	...
	for (i = begin_worker; i <= last_worker; i++) {
		cur_worker_info = (struct worker_info *)&q->data[(i % q->max_num) * q->field_count];
		BB_PRINT_PN("worker cpu:%u, pid:%u, ktime:%llu, action:%ps, dir:%d\n",
...
int __init dfx_ap_hook_init(void)
{
