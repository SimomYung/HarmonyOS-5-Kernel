...
static int dfx_pm_notify_gt_watch(unsigned char power_status)
{
	...
	if (g_gt_watch_type == GT_WATCH_C) {
		send_gt_watch.send_buffer = pay_load;
		send_gt_watch.send_buffer_len = PAYLOAD_LEN;
	} else {
...
static void iomcu_shutdown_state(void)
{
	int count = 0;
	struct iomcu_shutdown_config *shutdown = NULL;
	int ship_mode = 0;
	int ret;

	while (count <= TIMEOUT_POWEROFF) {
		shutdown = iomcu_config_get(IOMCU_CONFIG_USR_IOMCU_SHUTDOWN,
			sizeof(struct iomcu_shutdown_config));
		BB_PRINT_ERR("[%s],end times%d state %d\n", __func__, count,
			shutdown->shutdown_finish);
		if (shutdown->shutdown_finish)
			break;
    ...
    ret = ship_mode_get_work_mode(&ship_mode);
	if (ret < 0) {
		BB_PRINT_ERR("[%s],ship_mode_get_work_mode failed %d\n", __func__, ret);
        return;
    }
	if (ship_mode == SHIP_MODE_IN_SHUTDOWN_SHIP && shutdown->shutdown_finish == 3)
		ship_mode_set_work_mode(0);
}
...
static int dfx_pm_notify_mode_choose(void)
{
	int ret = 0;
	int ship_mode = 0;
	int ret_notify = -1;
	...
	BB_PRINT_ERR("[%s],notify mcu poweroff ship_mode %d\n", __func__, ship_mode);

	if (ship_mode == SHIP_MODE_IN_SHUTDOWN_SHIP)
		ret_notify = dfx_pm_notify_gt_watch(GT_WATCH_SHIPMODE);
	else
...
void dfx_pm_gt_watch_poweroff_process(void)
{
	...
	while (count < AP_MAX_NOTIFY_COUNT) {
		if (g_gt_watch_type == GT_WATCH_C)
			ret_notify = dfx_pm_notify_mode_choose();
		else
			ret_notify = dfx_pm_notify_gt_watch(GT_WATCH_POWER_OFF);
...
void dfx_pm_get_gt_watch_support_state(void)
{
	...
	else if (strncmp("GT_WATCH_C", gt_watch_type, sizeof("GT_WATCH_C")) == 0)
		g_gt_watch_type = GT_WATCH_C;
	else
...
#ifdef CONFIG_KEYBOARD_PMIC_FUNC_KEY
int rdr_press_funckey_to_fastboot(unsigned long event)
{
	if (event != PRESS_KEY_6S)
		return -1;
 
	if (check_mntn_switch(MNTN_PRESS_KEY_TO_FASTBOOT)) {
		if ((unsigned int)func_key_press_get() == 1) {
			func_key_press_set_zero();
			if (is_pmu_func_key_pressed()) {
				BB_PRINT_PN("[%s]Powerkey+Func_key\n", __func__);
#ifdef CONFIG_DFX_BB
...
#ifdef CONFIG_KEYBOARD_PMIC_FUNC_KEY
	int ret = 0;
	ret = rdr_press_funckey_to_fastboot(event);
	if(!ret)
        return 0;
#endif
...
void rdr_long_press_powerkey(void)
{
	...
	flush_cache_all();
	if (save_log_to_dfx_tempbuffer(AP_S_PRESS6S) < 0)
		BB_PRINT_PN("press6s save log to dfx failed\n");
	SYS_SYNC();
