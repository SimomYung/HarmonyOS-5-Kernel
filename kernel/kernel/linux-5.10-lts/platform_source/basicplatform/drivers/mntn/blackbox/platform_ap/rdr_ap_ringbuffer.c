...
void dfx_ap_ringbuffer_write(struct dfx_ap_ringbuffer_s *ptr, u8 *element)
{
...
#endif
	static DEFINE_RATELIMIT_STATE(memcpy_log_ratelmt_state, 60 * HZ, 10);
	...
	ret = memcpy_s((void *)&ptr->data[q->rear * q->field_count],
		   (q->max_num - q->rear) * q->field_count, (void *)element, q->field_count * sizeof(*element));
	if (ret != EOK) {
		if (__ratelimit(&memcpy_log_ratelmt_state))
 ...
 * fail = other
 */
int dfx_ap_ringbuffer_read(struct dfx_ap_ringbuffer_s *q, u8 *element, u32 len, bool dequeue)
{
	u32 ridx;
