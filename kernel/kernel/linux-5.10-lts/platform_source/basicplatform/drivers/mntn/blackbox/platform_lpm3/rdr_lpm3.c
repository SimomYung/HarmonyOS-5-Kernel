...
#ifdef CONFIG_SUPPORT_GET_PERI_DATA
#include <peri_data_def.h>
...
#define RECORD_KASLR_MAX_LENGTH  20
static u64 g_kaslr;
static u64 g_pc_nokaslr[CPU_CORE_NUM] = {0};
...
#ifdef CONFIG_SUPPORT_GET_PERI_DATA
static struct semaphore g_peri_data_sem;
#endif
...
#define DFX_M3_FORMAT "LPM3_RSN:0x%x,0x%x,0x%x,0x%x,0x%x,0x%x,0x%x,0x%x,0x%x,0x%x,0x%x,0x%x,0x%x."
#endif
...
#ifdef PMU_REG_BACKUP_OFFSET
	{
		"PMU_REGS",
		M3_RDR_SYS_CONTEXT_EXPOSE_STORAGE_OFFSET + PMU_REG_BACKUP_OFFSET,
		M3_RDR_PMU_REG_SIZE,
		system_reg_parse
	},
#endif
...
#endif
#ifdef DMA_REG_BACKUP_OFFSET
	{
		"DMA_REG",
		M3_RDR_SYS_CONTEXT_EXPOSE_STORAGE_OFFSET + DMA_REG_BACKUP_OFFSET,
		DMA_REG_BACKUP_SIZE,
		system_reg_parse
	},
#endif
...
#endif
#ifdef M3_RDR_DDR_MNTN_USER_SAVE_SIZE
	{
		"DDR_USER_REGS",
		M3_RDR_DDR_MNTN_USER_SAVE_OFFSET,
		M3_RDR_DDR_MNTN_USER_SAVE_SIZE,
		system_reg_parse
	},
#endif
};
...
static bool check_kaslr_cmdline_valid(const char *kaslr_cmdline, size_t len)
{
	unsigned int i;
	if (len <= 2)
		return false;
	if (kaslr_cmdline[0] != '0' || (kaslr_cmdline[1] != 'x' && kaslr_cmdline[1] != 'X'))
		return false;
	for (i = 0; i < len; i++) {
		if ((kaslr_cmdline[i] >= '0' && kaslr_cmdline[i] <= '9') ||
			(kaslr_cmdline[i] >= 'a' && kaslr_cmdline[i] <= 'f') ||
			(kaslr_cmdline[i] >= 'A' && kaslr_cmdline[i] <= 'F'))
			continue;
		else
			return false;
...
static int early_parse_get_kaslr_cmdline(char * kaslr_cmdline)
{
	int ret;
	size_t len;

	if (kaslr_cmdline == NULL) {
		BB_PRINT_ERR("[%s:%d]: kaslr_cmdline is null\n", __func__, __LINE__);
		return -1;
	}
	len = strnlen(kaslr_cmdline, RECORD_KASLR_MAX_LENGTH);
	if (len >= RECORD_KASLR_MAX_LENGTH) {
		BB_PRINT_ERR("[%s:%d]: kaslr cmdline len is invalid\n", __func__, __LINE__);
		return -1;
	}
	if (!check_kaslr_cmdline_valid(kaslr_cmdline, len)) {
		BB_PRINT_ERR("invalid kaslr cmdline %s\n", kaslr_cmdline);
		return -1;
	}

	ret = kstrtoull(kaslr_cmdline, 16, &g_kaslr);
	if(ret < 0){
		BB_PRINT_ERR("kstrtoull err");
		return -1;
	}

	return 0;
}
early_param("kaslr", early_parse_get_kaslr_cmdline);
			   ...
			   u64 log_addr, u32 log_len)
{
	...
	for (cpu_idx = 0; cpu_idx < CPU_CORE_NUM; cpu_idx++) {
		...
		pc = ((u64)data_high << 32) + (u64)data_low;
		g_pc_nokaslr[cpu_idx] = (pc > g_kaslr) ? pc - g_kaslr : 0x0;
		BB_PRINT_ERR("[%s] kaslr %llx, pc %llx, pc_nokaslr 0x%llx\n",
				__func__, g_kaslr, pc, g_pc_nokaslr[cpu_idx]);
	}
...
int get_pc_nokaslr_str(char *buf, size_t size)
{
	int ret;
	char pc_nokaslr_str[PC_NOKASLR_STR_MAX_LEN] = {0};
	size_t len;
	u32 cpu_idx;
	int offset;
	...
	for(cpu_idx = 0; cpu_idx < CPU_CORE_NUM; cpu_idx++){
		if (cpu_idx == 0)
			ret = sprintf_s(pc_nokaslr_str + offset, PC_NOKASLR_STR_MAX_LEN - offset,
				"0x%lx", g_pc_nokaslr[cpu_idx]);
		else
			ret = sprintf_s(pc_nokaslr_str + offset, PC_NOKASLR_STR_MAX_LEN - offset,
				",0x%lx", g_pc_nokaslr[cpu_idx]);
		if (ret <= 0) {
			BB_PRINT_ERR("get pc nokaslr string failed, pc_nokaslr[%u] 0x%llx\n",
			cpu_idx, g_pc_nokaslr[cpu_idx]);
			break;
		}
		offset += ret;
	}

	len = strnlen(pc_nokaslr_str, PC_NOKASLR_STR_MAX_LEN);
	if (size < len + 1) {
		BB_PRINT_ERR("get pc nokaslr string failed, buffer too small, len %lu\n", len);
		return -1;
	}

	ret = strncpy_s(buf, size, pc_nokaslr_str, len);
	if (ret != EOK) {
		BB_PRINT_ERR("get pc nokaslr string failed, copy string return %d\n", ret);
		return -1;
	}
...
#ifdef CONFIG_DFX_DIAGINFO_BASEINFO
#ifdef CONFIG_LPMCU_MNTN_RISCV
	if ((u8)p_exc->reset_reason[0] > PSCI_REASON_WDT_END)
		bbox_diaginfo_record(LPM3_EXC_INFO, NULL, DFX_M3_FORMAT,
			p_exc->reset_reason[0], p_exc->slice, p_exc->rtc, p_exc->lr, p_exc->pc, p_exc->mcause, p_exc->exc_trace,
			p_exc->ddr_exc, p_exc->irq_id, p_exc->task_id, p_exc->pc_async, p_exc->lr_async, p_exc->mtval);
#else
	if ((u8)p_exc->reset_reason[0] > PSCI_REASON_WDT_END)
		bbox_diaginfo_record(LPM3_EXC_INFO, NULL, DFX_M3_FORMAT,
			p_exc->reset_reason[0], p_exc->slice, p_exc->rtc, p_exc->r13, p_exc->lr1, p_exc->pc, p_exc->xpsr,
			p_exc->cfsr, p_exc->hfsr, p_exc->bfar, p_exc->exc_trace, p_exc->ddr_exc, p_exc->task_id);
#endif
#endif
...
static void rdr_addr_send_lpm3(void)
{
	...
	if (g_lpmcu_rdr_ddr_addr != NULL && g_lpmcu_rdr_flag == false) {
		...
		ret = memset_s(g_lpmcu_rdr_ddr_addr + M3_RDR_SYS_CONTEXT_RDR_MEM_MAP_INDEX_SIZE,
			       (size_t)g_rdr_lpm3_buf_len - M3_RDR_SYS_CONTEXT_RDR_MEM_MAP_INDEX_SIZE, 0,
			       (size_t)g_rdr_lpm3_buf_len - M3_RDR_SYS_CONTEXT_RDR_MEM_MAP_INDEX_SIZE);
		if (ret != EOK) {
...
#if defined (CONFIG_PLATDRV_SUPPORT_FFA) && defined (CONFIG_SUPPORT_GET_PERI_DATA)
int get_peri_data(struct peri_data *data)
{
	int ret;
	struct ffa_send_direct_data args = {
		.data0 = FID_REAL_TIME_PERI_DATA,
	};

	if (data == NULL) {
		BB_PRINT_ERR("%s:%d, NULL pointer volt\n", __func__, __LINE__);
		return -EINVAL;
	}

	down(&g_peri_data_sem);
...
int parse_peri_info_from_rdr(const void *bbox_base, size_t size, char *out_buf, size_t buf_size)
{
	int ret;
	struct ffa_send_direct_data args = {
		.data0 = FID_RDR_PERI_DATA,
		.data1 = buf_size,
	};
	...
	if (out_buf == NULL) {
		BB_PRINT_ERR("%s:%d, NULL pointer out_buf\n", __func__, __LINE__);
		return -EINVAL;
	}

	down(&g_peri_data_sem);
	ret = ffa_platdrv_send_msg_with_shmem(&args, (u8 *)out_buf, SHMEM_OUT);
	up(&g_peri_data_sem);
	if ((ret != 0) || (args.data0 != 0)) {
		pr_err("%s %d, send ffa msg to tzsp with shared memory fail\n", __func__, __LINE__);
		return -EINVAL;
