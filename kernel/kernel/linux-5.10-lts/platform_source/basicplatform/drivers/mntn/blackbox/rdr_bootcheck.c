...
static int rdr_check_exceptionboot(struct bootcheck *info)
{
	...
	info->core = RDR_AP;
#else
	info->mask = RDR_RDA_AP;
	info->core = RDR_RDA_AP;
#endif
...
#if defined(CONFIG_PRODUCT_CDC_ACE) || defined(CONFIG_LIBLINUX_CDC_RDA)
...
static void set_rdr_excepion_info(struct rdr_exception_info_s *exception_info)
{
	exception_info->e_from_core = RDR_RDA_AP;
	exception_info->e_reset_core_mask = RDR_RDA_AP;
	exception_info->e_exce_type = rdr_get_reboot_type();
	exception_info->e_exce_subtype = rdr_get_exec_subtype_value();
}
#endif
...
int rdr_bootcheck_thread_body(void *arg)
{
	...
	unsigned int enter_recovery_flag = get_boot_into_recovery_flag();
...
int powerkey1s_save_log(void)
{
	...
	ret = snprintf_s(date, DATATIME_MAXLEN, DATATIME_MAXLEN, "%s-%08llu",
		  rdr_get_timestamp(), rdr_get_tick());
	if (unlikely(ret < 0)) {
		BB_PRINT_ERR("[%s], sprintf date return %d!\n", __func__, ret);
		goto end;
