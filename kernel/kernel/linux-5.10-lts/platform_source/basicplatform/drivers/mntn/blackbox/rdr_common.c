...
/* RECORD_PC_STR_MAX_LENGTH for panic_pc content's maxlen. */
#define RECORD_PC_STR_MAX_LENGTH 0x48
/* This field is used to store the pc pointer in the cmdline from the UEFI. */
static char g_panic_pc[RECORD_PC_STR_MAX_LENGTH] = "undef";
...
struct blackbox_modid_list g_modid_list[] = {
	...
	{ (unsigned int)DFX_BB_MOD_FFTS_START, (unsigned int)DFX_BB_MOD_FFTS_END, "FFTS" },
	{ (unsigned int)DFX_BB_MOD_SILS_START, (unsigned int)DFX_BB_MOD_SILS_END, "sils"},
	{ (unsigned int)DFX_BB_MOD_DPA_START, (unsigned int)DFX_BB_MOD_DPA_END, "DPA" },
...
char *rdr_get_exception_core(u64 coreid)
{
	...
	case RDR_GPU:
		...
		break;
	case RDR_FFTS:
		core = exception_core[EXCEPTION_CORE_FFTS];
		break;
	case RDR_DFA:
		core = exception_core[EXCEPTION_CORE_DFA];
		break;
	case RDR_VDEC:
		core = exception_core[EXCEPTION_CORE_VDEC];
		break;
	case RDR_DDR_SEC:
		core = exception_core[EXCEPTION_CORE_DDR_SEC];
		break;
	case RDR_DPA:
		core = exception_core[EXCEPTION_CORE_DPA];
		break;
	default:
...
#if (defined(CONFIG_DFX_MNTN_S4) && !defined(CONFIG_DFX_MNTN_PC))
		break;
#endif
		while (times--) {
...
/**
 * Provides an interface for reading the PC pointer information during panic.
 */
static int __init early_parse_panic_pc_cmdline(char * panic_pc_cmdline)
{
	size_t len;

	if(panic_pc_cmdline == NULL) {
		BB_PRINT_ERR("[%s:%d]: panic_pc_cmdline is null\n", __func__, __LINE__);
		return -1;
	}

	len = strnlen(panic_pc_cmdline, RECORD_PC_STR_MAX_LENGTH);
	if (len == 0 || len >= RECORD_PC_STR_MAX_LENGTH) {
		pr_err("parse panic pc failed, string length %lu error\n", len);
		return -1;
	}

	if(memset_s(g_panic_pc, RECORD_PC_STR_MAX_LENGTH, 0, RECORD_PC_STR_MAX_LENGTH) != EOK) {
		BB_PRINT_ERR("[%s:%d]: memset_s err\n", __func__, __LINE__);
		return -1;
	}

	if(strncpy_s(g_panic_pc, RECORD_PC_STR_MAX_LENGTH, panic_pc_cmdline, len)) {
		BB_PRINT_ERR("[%s:%d]: failed to strncpy_s panic_pc\n", __func__, __LINE__);
		return -1;
	}

	return 0;
}
early_param("panic_pc", early_parse_panic_pc_cmdline);

/**
 * This interface is used to read the PC information get from UEFI through cmdline.
 */
int rdr_get_panic_pc(char * out_buf, size_t buf_size)
{
	size_t len;

	if (out_buf == NULL) {
		pr_err("get panic pc failed, invalid param\n");
		return -1;
	}

	len = strnlen(g_panic_pc, RECORD_PC_STR_MAX_LENGTH);
	if (buf_size < len + 1) {
		BB_PRINT_ERR("buffer size %lu is not enough for panic pc string %lu\n", buf_size, len);
		return -1;
	}

	if (strncpy_s(out_buf, buf_size, g_panic_pc, len)) {
		BB_PRINT_ERR("[%s:%d]: failed to strncpy_s panic_pc\n", __func__, __LINE__);
		return -1;
	}
...
void panic_show_regs(void)
{
	struct pt_regs regs;
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(5,10,0))
	struct dfx_exc_info_func_s info;
#endif
...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(5,10,0))
		info.regs = &regs;
		info.flag = DFX_EXC_INFO_REGS;
		info.esr = read_sysreg(esr_el1);
		info.far = read_sysreg(far_el1);
		CALL_HCK_VH(hck_dfx_bbox_set_exception_info, &info);
#endif
		console_verbose();
		show_regs(&regs);
	}
...
#define REBOOT_TYPE_LEN 32
static char g_reboot_type_str[REBOOT_TYPE_LEN] = {0};
static int __init early_parse_reboot_type_cmdline(char *cmdline)
{
	if (cmdline == NULL) {
		BB_PRINT_PN("[%s:%d]: reboot_type_cmdline is null\n", __func__, __LINE__);
		return 0;
	}

	if(strncpy_s(g_reboot_type_str, REBOOT_TYPE_LEN, cmdline, strlen(cmdline)) != EOK) {
		BB_PRINT_ERR("[%s:%d]: reboot_type_cmdline copy fail\n", __func__, __LINE__);
		return -1;
	}

	return 0;
}
early_param("reboot_type", early_parse_reboot_type_cmdline);

static int reboot_type_proc_show(struct seq_file *m, void *v)
{
	seq_puts(m, g_reboot_type_str);
	seq_putc(m, '\n');
	return 0;
}

static int __init proc_reboot_type_init(void)
{
	if (g_reboot_type_str[0] == 0) {
		BB_PRINT_PN("[%s:%d]: no need to create\n", __func__, __LINE__);
		return 0;
	}

	proc_create_single("reboot_type", 0, NULL, reboot_type_proc_show);
	return 0;
}
fs_initcall(proc_reboot_type_init);
