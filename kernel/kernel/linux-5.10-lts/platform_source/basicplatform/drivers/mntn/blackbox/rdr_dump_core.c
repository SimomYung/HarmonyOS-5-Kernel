				...
				bool is_save_done, u32 bootup_keypoint)
{
	...
	if ((p->e_reset_core_mask & RDR_AP) || (p->e_reset_core_mask & RDR_RDA_AP))
		reboot_from_ap = "true";
	else
...
#ifdef CONFIG_LIBLINUX
	if ((save_flags & RDR_SAVE_TRACK_INFO) && check_mntn_switch(MNTN_KERNEL_DUMP_ENABLE)) {
		BB_PRINT_PN("%s(): %s track_info msg saving\n", __func__, ap_path);
		(void)liblinux_hm_track_info_set(true);
		liblinux_dump_hm_track_info(ap_path, "kernel_logbuff.bin", true);
		(void)liblinux_hm_track_info_set(false);
	}
#endif
}
...
/*
 * Description: reduce file size to max_size by drop head bytes
 */
int rdr_keep_file_size(const char *logpath, const char *filename, u32 max_size)
{
	int ret;
	struct kstat stat;
	...
	if (logpath ==  NULL || filename == NULL || max_size > RDR_ELOG_METADATA_MAX_SIZE) {
		BB_PRINT_ERR("%s(): invalid parameter\n", __func__);
		return -1;
	}

	ret = snprintf_s(path, PATH_MAXLEN, PATH_MAXLEN - 1, "%s/%s", logpath, filename);
	if (ret < 0) {
		BB_PRINT_ERR("%s(): snprintf_s fail, ret = %d\n", __func__, ret);
		return -1;
	}
	...
	if (ret != 0) {
		BB_PRINT_ERR("%s(): vfs_stat fail, ret = %d\n", __func__, ret);
		return -1;
	}
	...
	buf = vmalloc(max_size);
	if (buf == NULL) {
		BB_PRINT_ERR("%s(): malloc fail.\n", __func__);
		return -1;
	}

	/* read old file tail content */
	fp = filp_open(path, O_RDONLY, FILE_LIMIT);
	if (IS_ERR(fp)) {
		BB_PRINT_ERR("%s(): open file %s fail. fp=0x%pK\n", __func__, path, fp);
		ret = -1;
		goto free;
	}

	pos = stat.size - (loff_t)max_size;
	ret = vfs_read(fp, buf, max_size, &pos);
	if (ret != max_size) {
		BB_PRINT_ERR("%s(): read fail, ret=%d.\n", __func__, ret);
		ret = -1;
		goto close;
	}
	...
	/* rewrite */
	fp = filp_open(path, O_WRONLY | O_TRUNC, FILE_LIMIT);
	if (IS_ERR(fp)) {
		BB_PRINT_ERR("%s(): open file %s fail. fp=0x%pK\n", __func__, path, fp);
		ret = -1;
		goto free;
	...
	pos = 0;
	ret = vfs_write(fp, buf, max_size, &pos);
	if (ret != max_size) {
		BB_PRINT_ERR("%s(): write file fail, ret=%d\n", __func__, ret);
		ret = -1;
		goto close;
	}
...
{
#if defined(CONFIG_PARTITION_TABLE) && !defined(CONFIG_LIBLINUX_CDC_RDA)
	char *buf = NULL;
	int ret, cnt;
	int fd_dfx;
...
static int save_dfxbuffer_to_file(struct dfx_head_info *dfx_head_info)
{
	...
	while (dfx_head_info->need_save_number != 0) {
		...
		if (!need_save_dfxbuffer2file(every_number_info->reboot_type, every_number_info->bootup_keypoint)) {
			...
			BB_PRINT_ERR("%s continue here, dfx_head_info->need_save_number is %u\n",
...
static int check_dfx_head_info(const char *tmp_buf)
{
	...
	if (dfx_head_info->cur_number < 0 || dfx_head_info->cur_number >= TOTAL_NUMBER) {
		BB_PRINT_ERR("%s():%d:dfx_head_info->cur_number=%u error\n", __func__, __LINE__,
 ...
 * Input:          every_number_info:the data that need to write
 */
static int save_buffer_to_dfx_tempbuffer(struct every_number_info *every_number_info)
{
	int ret = -1;
#ifdef CONFIG_PARTITION_TABLE
	void *buf = NULL;
 ...
 */
int save_log_to_dfx_tempbuffer(u32 reboot_type)
{
	void *buf = NULL;
	void *fastbootlog_addr = NULL;
	void *last_kmsg_addr = NULL;
	...
	(void)(last_kmsg_addr);
	every_number_info->last_kmsg_size = LAST_KMSG_SIZE;
	liblinux_save_hm_log((char *)every_number_info + every_number_info->last_kmsg_start_addr, LAST_KMSG_SIZE);
#endif
	ret = save_buffer_to_dfx_tempbuffer(every_number_info);
	BB_PRINT_PN("%s, save buffer to dfx tempbuf return %d\n", __func__, ret);
