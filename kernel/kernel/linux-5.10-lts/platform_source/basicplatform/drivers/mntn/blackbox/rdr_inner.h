...
#elif (NR_CPUS == 14)
#include "rdr_dfx_ap_hook_ratio_fourteen_core.h"
#elif (NR_CPUS == 16)
#include "rdr_dfx_ap_hook_ratio_sixteen_core.h"
#elif (NR_CPUS == 20)
...
#define RDR_ELOG_METADATA_MAX_SIZE (50 * 1024)
...
int rdr_create_epath_bc(char *date, size_t date_len, char *path, u32 path_len);
...
int rdr_hmlog_save_klog(char **ret_klog_buf, size_t *ret_klog_buflen);
void rdr_hmlog_dump_post(const char *klog_buf);
int rdr_hmlog_dump_ekbox(const char* dump_path, const char *file_name);
#else
static inline void rdr_hmlog_dump_on_exception(const char *path, const char *klog_buf, size_t klog_buflen)
{
	unused(path);
	unused(klog_buf);
	unused(klog_buflen);
...
static inline int rdr_hmlog_save_klog(char **ret_klog_buf, size_t *ret_klog_buflen)
{
	unused(ret_klog_buf);
	unused(ret_klog_buflen);
	return 0;
}

static inline void rdr_hmlog_dump_post(const char *klog_buf)
{
	unused(klog_buf);
}
static inline int rdr_hmlog_dump_ekbox(const char* dump_path, const char *file_name)
{
	unused(dump_path);
	unused(file_name);
	return 0;
