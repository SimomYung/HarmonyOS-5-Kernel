...
#define DIR_FILE_MAX_SIZE 0x2300000
...
int rdr_create_epath_bc(char *date, size_t date_len, char *path, u32 path_len)
{
	...
	if (path_len < PATH_MAXLEN || date_len >= DATATIME_MAXLEN) {
...
int bbox_chown(const char *path, uid_t user, gid_t group, bool recursion)
{
	...
	if (strncmp(path, PATH_ROOT, strlen(PATH_ROOT)) != 0 &&
		strncmp(path, RECOVERY_LOG_PATH, strlen(RECOVERY_LOG_PATH)) != 0) {
...
#endif
{
	...
	if (recursion) {
		/* cppcheck-suppress */
		dir_size = (u32)rdr_dir_size(fullname, PATH_MAXLEN, recursion);
		(*size) += dir_size;
		if (dir_size > (u32)DIR_FILE_MAX_SIZE)
			BB_PRINT_ERR("[%s]: %s file size exceeds the specified threshold,size is %u!\n", __func__, fullname, dir_size);
	}
	else
	...
	while (true) {
		...
		for (buf_attr.bpos = 0; buf_attr.bpos < (unsigned int)buf_attr.bufsize; buf_attr.bpos += d->d_reclen) {
			...
			if ((d_type == DT_DIR) && __rdr_dir_size(d, recursion, &size, &stat, fullname))
				...
				if (stat.size > (u32)DIR_FILE_MAX_SIZE)
					BB_PRINT_ERR("[%s]: %s file size exceeds the specified threshold,size is %lld!\n", __func__, fullname, stat.size);
				size += (u32)stat.size;
			}
		}
	}
