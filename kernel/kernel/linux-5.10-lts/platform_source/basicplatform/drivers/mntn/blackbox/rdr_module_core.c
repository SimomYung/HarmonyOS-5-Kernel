...
static DEFINE_MUTEX(g_rdr_module_curr_mask_mutex);
...
static int rdr_get_module_info(u64 coreid, struct rdr_register_module_result *retinfo)
{
	...
	case RDR_RDA_AP:
		...
		break;
	case RDR_FFTS:
		ret = rdr_get_areainfo(RDR_AREA_FFTS, retinfo);
		break;
	case RDR_DFA:
		ret = rdr_get_areainfo(RDR_AREA_DFA, retinfo);
		break;
	case RDR_VDEC:
		ret = rdr_get_areainfo(RDR_AREA_VDEC, retinfo);
		break;
	case RDR_DDR_SEC:
		ret = rdr_get_areainfo(RDR_AREA_DDR_SEC, retinfo);
		break;
	case RDR_DPA:
		ret = rdr_get_areainfo(RDR_AREA_DPA, retinfo);
		break;
	default:
		ret = -1;
	}
...
static void rdr_dump_done(u32 modid, u64 coreid)
{
	...
	mutex_lock(&g_rdr_module_curr_mask_mutex);
	current_mask |= coreid;
	BB_PRINT_PN("%s current mask:[0x%llx]\n", __func__, current_mask);
	mutex_unlock(&g_rdr_module_curr_mask_mutex);
...
void rdr_notify_module_reset(u32 modid, struct rdr_exception_info_s *e_info)
{
	...
	list_for_each_safe(cur, next, &__rdr_module_ops_list) {
		...
		if (p_module_ops->s_core_id == RDR_AP || p_module_ops->s_core_id == RDR_RDA_AP) {
