...
static void rdr_updater_bbox_chown(char *path)
{
	int ret;
...
#ifdef CONFIG_LIBLINUX
	if (ret == 0)
		rdr_updater_bbox_chown(path);
#endif
	BB_PRINT_END();
	return ret;
}

static void rdr_save_history_log_to_recovery_log(struct rdr_exception_info_s *p, char *date, u32 datelen,
				u32 bootup_keypoint)
{
	int ret;
...
#ifdef CONFIG_LIBLINUX
static void rdr_save_pstore_to_updater_log(void)
{
	int ret;
	char log_path[PATH_MAXLEN];
	char log_path_dmesg[PATH_MAXLEN];
	char date[DATATIME_MAXLEN];
	struct rdr_exception_info_s temp;

	if (rdr_wait_partition(RECOVERY_LOG_WAIT_PATH, RDR_WAIT_PARTITION_TIME) != 0) {
		BB_PRINT_PN("%s():%d:RECOVERY_LOG_WAIT_PATH  is not ready\n", __func__, __LINE__);
		return;
	}

	(void)memset_s(log_path, PATH_MAXLEN, 0, PATH_MAXLEN);
	(void)memset_s(log_path_dmesg, PATH_MAXLEN, 0, PATH_MAXLEN);
	(void)memset_s(date, DATATIME_MAXLEN, 0, DATATIME_MAXLEN);
	...
	rdr_save_history_log_to_recovery_log(&temp, &date[0], DATATIME_MAXLEN, get_boot_keypoint());
	save_pstore_info(log_path);
	rdr_hmlog_dump_ekbox(log_path, "ekbox.txt");

	ret = snprintf_s(log_path_dmesg, PATH_MAXLEN, PATH_MAXLEN - 1, "%s%s", log_path, "dmesg-ramoops-0");
	if (ret < 0) {
		BB_PRINT_ERR("%s():%d:snprintf_s fail!\n", __func__, __LINE__);
		return;
	}
	ret = SYS_ACCESS(log_path_dmesg, 0);
	if (ret < 0) {
		BB_PRINT_ERR("%s():%d:updater dmesg access fail!\n", __func__, __LINE__);
		return;
	}
	rdr_updater_bbox_chown(log_path_dmesg);
}

static void rdr_mkdir_updater_bbox(void)
{
	int ret;
	char log_path[PATH_MAXLEN];

	BB_PRINT_PN("============wait for RECOVERY_LOG_WAIT_PATH ready start =============\n");
	while (rdr_wait_partition(RECOVERY_LOG_WAIT_PATH, RDR_TIME_OUT) != 0);
	BB_PRINT_PN("============wait for RECOVERY_LOG_WAIT_PATH ready e n d =============\n");
	...
	ret = rdr_create_dir(log_path);
	if (ret < 0) {
		BB_PRINT_ERR("%s():%d:creat updater_bbox fail[%d]\n", __func__, __LINE__, ret);
		return;
	}
	rdr_updater_bbox_chown(log_path);
}
#else
...
/*
 * Description:    copy file from /proc/rdr/pstore/ to dst_dir_str
 */
static void rdr_save_pstore_to_recovery_log(void)
{
	int ret, tmp_cnt;
	char *pbuff = NULL;
	char log_path[PATH_MAXLEN];
	...
	if (ret)
		BB_PRINT_ERR("rdr_save_pstore_to_recovery_log fail\n");
...
static int recovery_save_dfxbuffer_to_file(struct dfx_head_info *dfx_head_info)
{
	...
	while (dfx_head_info->need_save_number != 0) {
		...
		if (every_number_info->last_kmsg_size != 0) {
			...
			rdr_save_history_log_to_recovery_log(&temp, &dfx_save_file.date[0], DATATIME_MAXLEN,
 ...
 */
static void rdr_save_dfxlog_to_recovery_log(void)
{
	...
	str_len = (int)(strlen(p_name) - strlen(PART_DFX));
...
#else
static inline void rdr_save_dfxlog_to_recovery_log(void)
{
	return;
}
#endif

static int is_need_save_pstore(int reboot_type) {
	if (reboot_type >= REBOOT_REASON_LABEL1 && reboot_type < REBOOT_REASON_LABEL3)
		return NEED_SAVE_PSTORE;
#ifdef CONFIG_LIBLINUX
	if (reboot_type == HM_PANIC)
		return NEED_SAVE_PSTORE;
#endif
	return NOT_SAVE_PSTORE;
}
...
#ifdef CONFIG_LIBLINUX
	rdr_mkdir_updater_bbox();
#endif

	reboot_type = rdr_get_reboot_type();
	BB_PRINT_PN("%s reboot_type = [0x%x]\n", __func__, reboot_type);

	rdr_save_dfxlog_to_recovery_log();
	BB_PRINT_PN("%s rdr_save_dfxlog_to_recovery_log end\n", __func__);

	if (is_need_save_pstore(reboot_type) == NEED_SAVE_PSTORE) {
#ifdef CONFIG_LIBLINUX
		rdr_save_pstore_to_updater_log();
#else
		rdr_save_pstore_to_recovery_log();
#endif
		BB_PRINT_PN("%s rdr_save_pstore_to_recovery_log end\n", __func__);
	}
	/* at most 5 logs */
