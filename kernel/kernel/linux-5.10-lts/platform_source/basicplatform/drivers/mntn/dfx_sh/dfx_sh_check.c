...
bool is_sh_gpu_pim_exc(void)
{
	...
	return (reboot_type == AP_S_NOC && excep_sub_type == NOC_GPU);
#else
	return false;
#endif
}

#define QIC_MEDIA1_EXC_BUSKEY	0x2
...
#endif
	qice_intr = check_qice_intr();
	pr_info("dfx_sh check vdec exc, qic_buskey 0x%x, qice_intr %u\n",
			qic_buskey, qice_intr);
	return (qice_intr && qic_buskey == QIC_MEDIA1_EXC_BUSKEY);
}
...
{
	unsigned int reboot_type = 0;
	unsigned int sr_point = 0;

	reboot_type = rdr_get_reboot_type();
	if (reboot_type != LPM3_S_LPMCURST)
		return false;

	get_last_sr_info(&sr_point, NULL);
	pr_info("dfx_sh check hfrc panic, reboot_type 0x%x,sr p:%u\n", reboot_type, sr_point);

	if (sr_point >= SH_HFRC_SR_PONIT_MIN && sr_point <= SH_HFRC_SR_PONIT_MAX)
		return true;
