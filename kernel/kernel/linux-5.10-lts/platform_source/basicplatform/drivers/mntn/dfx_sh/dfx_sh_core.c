...
bool g_sh_need_update_main_nve = false;
struct mutex g_sh_nv_access_mutex;
...
static bool is_dfv_enable(enum sh_type type)
{
	if (type > sh_type_cluster_max)
		return false;

	if (!g_sh_cfg_info_main.dfv_cfg.enable || !g_sh_cfg_info_main.sw_cnt[type].enable)
		return false;
...
static void set_dfv_flag_after_cnt(enum sh_type type)
{
	if (type > sh_type_cluster_max)
		return;
	...
	if (g_sh_cfg_info_main.dfv_cfg.threshold > 0 &&
...
static void sh_diaginfo_record_cnt(enum sh_type type)
{
	...
	if (type <= sh_type_cluster_max) {
		bbox_diaginfo_record(g_sh_cluster_dmd_cnt[type], NULL,
				"acpu cluster %u sh cnt, ver %u, cnt %d, threshold %d, reason %s:%s",
...
static void sh_diaginfo_record_pv(enum sh_type type)
{
	...
	if (type <= sh_type_cluster_max) {
		bbox_diaginfo_record(g_sh_cluster_dmd_cnt[type], NULL,
				"acpu cluster %u pv, ver %u, cnt %d, threshold %d",
				type, g_sh_cfg_info_main.version,
				g_sh_cfg_info_main.sw_cnt[type].count,
				g_sh_cfg_info_main.sw_cnt[type].threshold);
		return;
...
static void sh_diaginfo_record_dfv(enum sh_type type)
{
	...
	bbox_diaginfo_record(g_dfv_sh_cluster_dmd_cnt[type], NULL,
			"acpu cluster %u close dfv, ver %u, cnt %d, dfv threshold %d",
			type, g_sh_cfg_info_main.version,
			g_sh_cfg_info_main.sw_cnt[type].count,
...
static int do_isp_sh_core(u32 rtc_sec)
{
	bool need_clear = false;
 
	g_sh_cfg_info_main.sw_cnt[sh_type_isp].count++;
	pr_info("sh type sh_type_isp count add\n");
	if (g_sh_cfg_info_main.panic_time[sh_type_isp] != 0) {
		need_clear = sh_need_clear(sh_type_isp, rtc_sec);
		if (need_clear) {
			pr_info("sh type sh_type_isp need clear cnt to 1\n");
			g_sh_cfg_info_main.sw_cnt[sh_type_isp].count = 1;
		}
	}
	g_sh_cfg_info_main.panic_time[sh_type_isp] = rtc_sec;
 
	if (!is_sh_threshold_match(sh_type_isp))
		return SH_STATE_NO_CHANGE;
 
	/* if isp cnt match threshold check isp pv or cancel pv and panic rtc set to 0 */
	pr_info("sh type sh_type_isp change pv flag\n");
	g_sh_cfg_info_main.sw_cnt[sh_type_isp].pv_flag = ~g_sh_cfg_info_main.sw_cnt[sh_type_isp].pv_flag;
	g_sh_cfg_info_main.sw_cnt[sh_type_isp].count = 0;
	g_sh_cfg_info_main.panic_time[sh_type_isp] = 0;
...
static void do_sh(enum sh_type type, u32 rtc_sec)
{
	...
	if (is_dfv_enable(type) && is_dfv_match(type))
...
static void isp_sh_wk(struct work_struct *wk)
{
	...
	change_isp_state = do_isp_sh_core(tv.tv_sec);
...
void ecc_ue_do_sh(int cluster)
{
	...
	if (g_sh_cfg_info_main.sw_cnt[type].enable && !is_sh_threshold_match(type)) {
