...
#define SEPALT_TPM_ENABLE_STATUS  "okay"
...
#define TRACE_FLAG                      0xae85ae85
#define SE_FAULT_VALUE_PIN_ERR          0xc98a3157

enum {
	MSPC_EXCEPTION = 0,
	MSPC_EXCEPTION_NORST = 1,
	MSPC_EXCEPTION_IRQ = 2,
};
...
#ifdef CONFIG_GENERAL_SEE_MNTN_REPORT_FILTER
...
#define HISEE_MNTN_STATE_ENABLE  0x5A
...
static spinlock_t g_mntn_trigger_spinlock;
static struct ipc_msg g_msg;
union mspc_mntn_state g_hisee_mntn_state = { .state_val = 0 };
#ifdef CONFIG_CRYPTO_CORE_APM_KCA_NOTIFY
static struct task_struct *g_apm_thread = NULL;
static u64 g_sr_status_flag = PM_POST_SUSPEND;
#endif
#define GENERAL_SEE_LOGBUF_HEADER_LEN sizeof(struct general_see_logbuf_header)

static void set_mntn_state_flag(u32 state)
{
	switch (state) {
	case HISEE_STATE_INVALID:
		g_hisee_mntn_state.state_val = 0;
		break;
	case HISEE_STATE_READY:
		g_hisee_mntn_state._state.common = HISEE_MNTN_STATE_ENABLE;
		break;
	case HISEE_STATE_LOG_OUT:
		g_hisee_mntn_state._state.log_out = HISEE_MNTN_STATE_ENABLE;
		break;
	case HISEE_STATE_HISEE_EXC:
		g_hisee_mntn_state._state.hisee_exc = HISEE_MNTN_STATE_ENABLE;
		break;
	case HISEE_STATE_PRINT_VERINFO:
		g_hisee_mntn_state._state.print_verinfo = HISEE_MNTN_STATE_ENABLE;
		break;
	default:
		break;
	}
...
static void clear_mntn_state_flag(u32 state)
{
	unsigned long flags;

	spin_lock_irqsave(&g_mntn_trigger_spinlock, flags);
	switch (state) {
	case HISEE_STATE_LOG_OUT:
		g_hisee_mntn_state._state.log_out = 0x0;
		break;
	case HISEE_STATE_HISEE_EXC:
		g_hisee_mntn_state._state.hisee_exc = 0x0;
		break;
	case HISEE_STATE_PRINT_VERINFO:
		g_hisee_mntn_state._state.print_verinfo = 0x0;
		break;
	default:
		break;
	}
	spin_unlock_irqrestore(&g_mntn_trigger_spinlock, flags);
}
...
void rdr_hisee_call_to_record_exc(int data)
{
	...
	spin_lock_irqsave(&g_mntn_trigger_spinlock, flags);
	set_mntn_state_flag(HISEE_STATE_HISEE_EXC);
	// set completion first to make rdr_hisee_thread to busy wait
...
#ifdef CONFIG_CRYPTO_CORE_V2
		if ((int_id & g_hisee_exc_trans[i].int_id_value) != 0) {
			module_id = g_hisee_exc_trans[i].module_value;
			if (g_hisee_exc_trans[i].int_id_value == HISEE_RST_INT_ID) /* lower the priority of rst */
				continue;
			break;
		}
#else
...
static noinline int copy_logdata_to_log_phy(u64 source_addr, u64 size, u32 trace_flag)
{
	u64 recv_size;
	...
	while (size > 0) {
		...
		size -= recv_size;
		g_logbuf_info.header->real_size += recv_size;
	}
	return HISEE_OK;
}
...
static noinline u64 tzsp_hisi_service_hisee_mntn_ffa(u64 _function_id, u64 _arg0, u64 _arg1, u64 _arg2)
{
...
#ifdef CONFIG_GENERAL_SEE_MNTN_ENHANCE
		ret = copy_logdata_to_log_phy(g_mntn.trace_addr,
					      g_mntn.trace_size, TRACE_FLAG);
		if(ret != HISEE_OK) {
			pr_err("copy_logdata trace fail\n");
			return ret;
		}
#endif
		ret = copy_logdata_to_log_phy(g_mntn.base_addr,
					       g_mntn.base_size, 0);
		if(ret != HISEE_OK) {
			pr_err("copy_logdata base fail\n");
			return ret;
		}
		return ret;
	case HISEE_SMC_LOG_OUT:
		...
		g_logbuf_info.header->real_size = 0;
		ret = copy_logdata_to_log_phy(args.data0, args.data1, 0);
		if(ret != HISEE_OK)
			return ret;

		if (args.data2 == NEED_COPY_SECOND_BLOCK) {
			ret = copy_logdata_to_log_phy(args.data3, args.data4, 0);
			if(ret != HISEE_OK)
				return ret;
		}
		return ret;
#ifdef CONFIG_GENERAL_SEE_MNTN_SEPLAT
	case HISEE_SMC_SET_SEPLAT_STATUS:
		ret = ffa_rpmbmspc_send_msg_with_args(&args);
		if (ret) {
			pr_err("ffa send fail:%d\n", ret);
			return ret;
...
#ifdef CONFIG_GENERAL_SEE_MNTN_SEPLAT
static bool is_seplat_enable(void)
{
	int ret;
	struct device_node *np = NULL;
	const char *status = NULL;

	np = of_find_compatible_node(NULL, NULL, "hisilicon,seplat");
	if (!np) {
		pr_err("%s: no hisilicon,seplat compatible node found.\n",
		       __func__);
		return false;
	}
	...
	if (ret || !status) {
		pr_err("%s: get seplat status failed!\n", __func__);
		return false;
	}

	if (strncmp(status, SEPALT_TPM_ENABLE_STATUS, strlen(SEPALT_TPM_ENABLE_STATUS)) != 0) {
		pr_err("%s: seplat not enable\n", __func__);
		return false;
	}

	pr_err("%s: seplat enable\n", __func__);

	return true;
}

static void hisee_mntn_save_seplat_status(void)
{
    int flag = is_seplat_enable() ? 1 : 0;
    (void)hisee_mntn_svc(HISEE_MNTN_ID, (u64)HISEE_SMC_SET_SEPLAT_STATUS,
					       (u64)flag, (u64)0x0);
}
#endif
...
void rdr_hisee_save_print_log(void)
{
	...
	ret = mntn_filesys_write_log(path,
				     ...
				     HISEE_FILE_PERMISSION);
	if (ret <= 0)
		pr_err("save hisee log err, ret = %d\n", ret);
...
exit_handle:
	clear_mntn_state_flag(HISEE_STATE_LOG_OUT);
}
...
#ifdef CONFIG_CRYPTO_CORE
static void rdr_hisee_mntn_exc(void)
{
	u32 mod_id;
	struct mspc_diaginfo diaginfo;
	unsigned int rcv_exc_data[MAX_MAIL_SIZE] = {0};
	unsigned long flags;

	// busy wait exception info ready
	spin_lock_irqsave(&g_mntn_trigger_spinlock, flags);
	mod_id = g_hisee_exception_modid;
	(void)memcpy_s(rcv_exc_data, sizeof(rcv_exc_data), &g_msg.data[0], sizeof(g_msg.data));
	diaginfo.exception_type = (u32)g_hisee_exception_type,
	diaginfo.mod_id = g_hisee_diaginfo_modid;
	...
	if (mod_id == (u32)MODID_GENERAL_SEE_EXC_BUS)
		mspc_report_rpmb();
	/*
	...
	pr_err("vote lpm3[0x%x]vote atf[0x%x] vote TEE[0x%llx]\n",
		rcv_exc_data[HISEE_MNTN_IPC_DATA6], g_vote_val_atf, g_vote_val_tee);
	pr_err("msg.data[0] = %u msg.data[1] = %u\n",
		rcv_exc_data[HISEE_MNTN_IPC_DATA0], rcv_exc_data[HISEE_MNTN_IPC_DATA1]);
	/* not care record result */
	(void)mspc_record_mspc_diaginfo(&diaginfo);

	rdr_system_error(mod_id, 0, 0);
	clear_mntn_state_flag(HISEE_STATE_HISEE_EXC);
}
#else
static void rdr_hisee_mntn_exc(void)
{
	 ...
	 * MODID_SIMULATE_EXC_RPMB_KO
	 */
	if (mod_id != HISEE_MNTN_IPC_MSG_0)
		g_rpmb_status_flag = mspc_exception_to_reset_rpmb();
	...
	if (rdr_hisee_judge_new_misc() == HISEE_OK) {
		...
		pr_err("fi[0x%x] fv[0x%x] ss[0x%x] sc[0x%x] vote lpm3[0x%x] vote atf[0x%x] rpmb %d\n",
			rcv_exc_data[HISEE_MNTN_IPC_DATA2], rcv_exc_data[HISEE_MNTN_IPC_DATA3],
			rcv_exc_data[HISEE_MNTN_IPC_DATA4], rcv_exc_data[HISEE_MNTN_IPC_DATA5],
			rcv_exc_data[HISEE_MNTN_IPC_DATA6], g_vote_val_atf,
			g_rpmb_status_flag);
		pr_err("msg.data[0] = %u msg.data[1] = %u\n",
			rcv_exc_data[HISEE_MNTN_IPC_DATA0], rcv_exc_data[HISEE_MNTN_IPC_DATA1]);
		...
		(void)bbox_diaginfo_record(MSPC_DIAG_INFO, NULL,
			...
			mod_id, rcv_exc_data[HISEE_MNTN_IPC_DATA2],
			rcv_exc_data[HISEE_MNTN_IPC_DATA3], rcv_exc_data[HISEE_MNTN_IPC_DATA4],
			rcv_exc_data[HISEE_MNTN_IPC_DATA5]);
#endif
		rdr_system_error(mod_id, 0, 0);
	} else {
 ...
 */
static void rdr_hisee_print_verinfo(void)
{
	struct mspc_access_info info = {0};
	static unsigned int sleep_cnt = 0;
	static unsigned int try_cnt = 0;

	if (sleep_cnt > HISEE_MNTN_PRINT_COS_VER_MS) {
		sleep_cnt = 0;
		...
		if (mspc_access_partition((char *)&g_curr_ver_mntn,
			...
			hisee_mntn_print_cos_info();
			try_cnt = 0;
			clear_mntn_state_flag(HISEE_STATE_PRINT_VERINFO);
			return;
		} else {
			try_cnt++;
			if (try_cnt > HISEE_MNTN_PRINT_COS_VER_MAXTRY) {
				pr_err("%s:fail to print verinfo after all try!\n", __func__);
				try_cnt = 0;
				clear_mntn_state_flag(HISEE_STATE_PRINT_VERINFO);
				return;
			}
...
int rdr_hisee_thread(void *arg)
{
	...
	while (!kthread_should_stop()) {
		...
		/* If same mntn event occured twice in a short time, the 2nd will be discard */
		if (g_hisee_mntn_state._state.hisee_exc == HISEE_MNTN_STATE_ENABLE) {
			pr_err("%s, entry hisee mntn hisee_exc.", __func__);
			rdr_hisee_mntn_exc();
		}
		if (g_hisee_mntn_state._state.log_out == HISEE_MNTN_STATE_ENABLE) {
			pr_err("%s, entry hisee mntn log_out.", __func__);
			rdr_hisee_save_print_log();
		}
		if (g_hisee_mntn_state._state.print_verinfo == HISEE_MNTN_STATE_ENABLE)
			rdr_hisee_print_verinfo();
		/* according to spin lock, when state_val is 0, there must be no completion */
		spin_lock_irqsave(&g_mntn_trigger_spinlock, flags);
		if ((g_hisee_mntn_state.state_val >> 8) == 0) { /* nocare low 8 bit(state_common) */
			reinit_completion(&g_hisee_mntn_complete);
		}
		spin_unlock_irqrestore(&g_mntn_trigger_spinlock, flags);
	}
	return 0;
...
{
	int ret;
	unsigned int err_id = MSPC_DIAG_INFO;
#ifdef CONFIG_GENERAL_SEE_MNTN_REPORT_FILTER
	ret = save_mspc_diaginfo(diaginfo);
	if (ret != 0)
		return ret; 
#endif
	if (diaginfo->exception_type == MSPC_EXCEPTION_NORST)
		err_id = diaginfo->mod_id;
#ifdef CONFIG_GENERAL_SEE_MNTN_SEPLAT
	if (diaginfo->fv == SE_FAULT_VALUE_PIN_ERR)
		err_id = MSPC_PIN_ERROR;
#endif
			  ...
			  void *msg)
{
	...
	case HISEE_LOG_OUT:
		spin_lock_irqsave(&g_mntn_trigger_spinlock, flags);
		set_mntn_state_flag(HISEE_STATE_LOG_OUT);
		// set completion first to make rdr_hisee_thread to busy wait
		complete(&g_hisee_mntn_complete);
	...
	case HISEE_EXCEPTION:
		...
		if ((p_ipcmsg->data[HISEE_EXC_TYPE_ID] &
			...
			g_hisee_diaginfo_modid = g_hisee_exception_modid;
#ifdef CONFIG_CRYPTO_CORE
			g_hisee_exception_type = MSPC_EXCEPTION;
#endif
	...
	case HISEE_IRQ:
		...
		g_hisee_diaginfo_modid = p_ipcmsg->data[1];
#ifdef CONFIG_CRYPTO_CORE
		g_hisee_exception_type = MSPC_EXCEPTION_IRQ;
#endif
		g_vote_val_lpm3 = p_ipcmsg->data[HISEE_MNTN_IPC_DATA6];
		g_msg = *p_ipcmsg;
		spin_unlock_irqrestore(&g_mntn_trigger_spinlock, flags);
	...
	case MSPC_APM_KCA_NOTIFY:
		pr_err("apm_debug: %s  %d\n", __func__, __LINE__);
		mspc_apm_kca_notify();
		break;
#endif
#ifdef CONFIG_GENERAL_SEE_MNTN_ENHANCE
	case MSPC_POWEROFF_ERR:
		spin_lock_irqsave(&g_mntn_trigger_spinlock, flags);
		set_mntn_state_flag(HISEE_STATE_HISEE_EXC);
		// set completion first to make rdr_hisee_thread to busy wait
		complete(&g_hisee_mntn_complete);
		g_hisee_exception_modid = (u32)MODID_GENERAL_SEE_EXC_POWEROFF_FAIL;
		g_hisee_diaginfo_modid = g_hisee_exception_modid;
		spin_unlock_irqrestore(&g_mntn_trigger_spinlock, flags);
		break;
	case MSPC_TEE_ERR_NOTIFY:
		spin_lock_irqsave(&g_mntn_trigger_spinlock, flags);
		set_mntn_state_flag(HISEE_STATE_HISEE_EXC);
		// set completion first to make rdr_hisee_thread to busy wait
		complete(&g_hisee_mntn_complete);
		g_hisee_exception_modid = (u32)MODID_GENERAL_SEE_EXC_TEE_ERR_NOTIFY;
		g_hisee_diaginfo_modid = g_hisee_exception_modid;
		spin_unlock_irqrestore(&g_mntn_trigger_spinlock, flags);
		break;
#endif
...
void hisee_mntn_debug_dump(void)
{
	...
	if (g_hisee_mntn_state._state.common != HISEE_MNTN_STATE_ENABLE) {
...
#ifdef CONFIG_CRYPTO_CORE_APM_KCA_NOTIFY
u64 mspc_apm_get_sr_status_flag(void)
{
	return g_sr_status_flag;
}
#endif
...
static int get_mspc_mntn_memory_data_from_dts(void)
{
	 ...
	 * data[2] is the mbx base address
	 * data[3] is the size of mbx
	 */
	u32 data[4] = { 0 };
...
#ifdef CONFIG_GENERAL_SEE_MNTN_ENHANCE
	ret = of_property_read_u32_array(np, "reg", &data[0], 4);
#else
...
#ifdef CONFIG_GENERAL_SEE_MNTN_ENHANCE
	g_mntn.trace_addr = data[2];
	g_mntn.trace_size = data[3];
#endif
...
#endif
#ifdef CONFIG_CRYPTO_CORE_APM_KCA_NOTIFY
static int mspc_apm_pm_callback(struct notifier_block *nb,
	unsigned long event, void *ptr)
{
	...
	case PM_HIBERNATION_PREPARE:
		pr_info("%s: S4 suspend +\n", __func__);
		g_sr_status_flag = PM_SUSPEND_PREPARE;
		pr_info("%s: S4 suspend -\n", __func__);
		break;
	case PM_SUSPEND_PREPARE: /* call before suspend */
		pr_info("%s: suspend +\n", __func__);
		if (mspc_apm_get_ca_ta_status())
			return NOTIFY_BAD;
		g_sr_status_flag = PM_SUSPEND_PREPARE;
		pr_info("%s: suspend -\n", __func__);
		break;
	case PM_POST_HIBERNATION: /* call after resume */
		pr_info("%s: S4 resume +\n", __func__);
		g_sr_status_flag = PM_POST_SUSPEND;
		pr_info("%s: S4 resume -\n", __func__);
		break;
	case PM_POST_SUSPEND: /* call after resume */
		pr_info("%s: resume +\n", __func__);
		g_sr_status_flag = PM_POST_SUSPEND;
		if (mspc_apm_get_process_interrupt_flag())
			mspc_apm_kca_notify();
		pr_info("%s: resume -\n", __func__);
		break;
	default:
...
static struct notifier_block mspc_apm_pm_notify_block = {
	.notifier_call = mspc_apm_pm_callback,
};
#endif
/*
...
static int hisee_mntn_probe(struct platform_device *pdev)
{
	...
	register_pm_notifier(&mspc_apm_pm_notify_block);
#endif
...
#ifdef CONFIG_GENERAL_SEE_MNTN_SEPLAT
	/* register seplat status save func */
	hisee_mntn_save_seplat_status();
#endif
	...
	spin_lock_init(&g_mntn_trigger_spinlock);

	spin_lock_irqsave(&g_mntn_trigger_spinlock, flags);
	set_mntn_state_flag(HISEE_STATE_READY);
	set_mntn_state_flag(HISEE_STATE_PRINT_VERINFO);
	complete(&g_hisee_mntn_complete);
	spin_unlock_irqrestore(&g_mntn_trigger_spinlock, flags);
	pr_err("exit %s\n", __func__);
	return 0;
