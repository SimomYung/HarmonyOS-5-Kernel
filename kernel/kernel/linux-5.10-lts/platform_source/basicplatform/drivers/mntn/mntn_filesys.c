...
int mntn_filesys_rm_dir(const char *dirname)
{
	...
	/* count is used to prevent dirname without end char '\0' */
	while ((*pdst != '\0') && count < MNTN_FILESYS_FNAME_LEN) {
		pdst++;
		count++;
	}

	if (count == MNTN_FILESYS_FNAME_LEN && (*pdst != '\0')) {
		MNTN_FILESYS_PRINT
		    ("mntn_err: dir is err, end without end char\n");
		return -1;
	}
