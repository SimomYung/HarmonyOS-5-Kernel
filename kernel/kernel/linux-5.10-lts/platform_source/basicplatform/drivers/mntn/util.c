...
static bool is_mntn_switch_valid(const char *mntn_switch, size_t len)
{
	unsigned int i;

	for (i = 0; i < len; i++) {
		if (mntn_switch[i] != '0' && mntn_switch[i] != '1')
			return false;
	}
...
static int __init early_parse_mntn_switch_cmdline(char *mntn_switch_cmdline)
{
	...
	len = strnlen(mntn_switch_cmdline, MNTN_SWITCH_VALID_SIZE);
	if (len == 0 || len >= MNTN_SWITCH_VALID_SIZE) {
		BB_PRINT_ERR("error: invalid himn cmdline size!\n");
		return -1;
	}
	if (!is_mntn_switch_valid(mntn_switch_cmdline, len)) {
		BB_PRINT_ERR("error: invalid himn cmdline: %s\n", mntn_switch_cmdline);
		return -1;
	}
	ret = strncpy_s(mntn_switch, MNTN_SWITCH_VALID_SIZE + 1, mntn_switch_cmdline, len);
	if (ret != EOK)
		return -1;
