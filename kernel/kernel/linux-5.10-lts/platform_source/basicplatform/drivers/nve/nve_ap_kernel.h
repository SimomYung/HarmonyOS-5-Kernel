...
#include <platform_include/see/hicrypto_udk_for_ldk.h>
#endif
...
#define NVE_NV_SIZE 128
#define NV_DEVICE_NAME "/dev/block/by-name/nvme"
#define NV_DEVICE_BAK_NAME "/dev/block/by-name/nvme_bak"

#define NV_INFO_LEN 1024
#define NVE_ERROR (-1)
#define NVE_OK (0)
#define NV_DEVICE_MAX_PATH_LEN 64
#define NVE_TIMEOUT (-2)
#define NVE_UNSUPPORT_MAC (-3)
#define NVE_SEM_TIMEOUT 1000

#ifdef CONFIG_NVE_SHARE_ACCESS
#define NVE_HW_LOCK_TIME    3 /* ms */
#define NVE_HW_LOCK_TIMEOUT    10000 /* ms */
#define HWLOCK_RESOURCE_TYPE_ACCESS_NVE         69

/* for record nve init status */
#define SCTRL_SCBAKDATA2_MSK (SOC_ACPU_SCTRL_BASE_ADDR + 0x4C8)
#endif
/*
...
/* NV IDs used in xloader */
static uint32_t nv_num_xloader[] = { 1, 2, 262, 272, 273, 277, 278, 279, 280, 281, 282, 297, 515 };
#endif
...
struct nve_partition_header_struct {
	...
	unsigned char reserved[64 - NVE_MAC_LENTH];
#else
	unsigned char reserved[68 - NVE_MAC_LENTH];
#endif
    unsigned char nve_mac[NVE_MAC_LENTH];
	/* changed by run-time image */
	unsigned int nve_age;
};

enum efuse_hicrypto_status {
	STATUS_NOT_GET = 0,
	UNSUPPORT_HICRYPTO,
	SUPPORT_HICRYPTO,
};
