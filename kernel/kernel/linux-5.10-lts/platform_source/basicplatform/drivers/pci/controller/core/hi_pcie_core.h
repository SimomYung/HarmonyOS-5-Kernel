...
struct hi_pcie_transfer_ops {
	...
	int (*read_by_cpu)(struct hi_pcie_port *port, void *buf, u32 size, u32 offset);
	int (*write_by_cpu)(struct hi_pcie_port *port, void *buf, u32 size, u32 offset);
...
struct hi_pcie_platform_ops {
	...
	int (*start_link)(struct hi_pcie *pcie);
	...
	int (*fixup_cfg)(struct hi_pcie *pcie);
	int (*flush_work)(struct hi_pcie_port *port);
};
