					      ...
					      struct hi_pcie_ctrl *ctrl)
{
	...
	if (irq <= 0) {
		hi_pcie_pr_e("get local irq-%dfailed", irq);
		return -1;
	}
...
static int pcie_ctrl_setup(struct hi_pcie *pcie)
{
	...
	if (atomic_read(&pcie->is_ctrl_setup)) {
		hi_pcie_pr_i("ctrl[%u] is already setup", pcie->ctrl_id);
		return ret;
	}
#endif
...
#ifdef CONFIG_HIPCIE_OCT
	atomic_set(&(pcie->is_ctrl_setup), 1);
#endif
...
static int pcie_ctrl_cfg_ecam_atu(struct hi_pcie_port *port)
{
	struct hi_pcie_ctrl_port *ctrl_port = port->priv;
	struct hi_pcie_ctrl *ctrl = ctrl_port->ctrl;
	u64 tar = ((u64)ctrl->rc_dma_addr->message.addr_h << 32) | ctrl->rc_dma_addr->message.addr_l;
	pcie_setup_tx_atu(ctrl, 0, ctrl->ecam_phy_base, tar, ctrl->rc_dma_addr->message.size);
	return 0;
}

static int pcie_ctrl_read_by_cpu(struct hi_pcie_port *port, void *buf, u32 size, u32 offset)
{
	struct hi_pcie_ctrl_port *ctrl_port = port->priv;
	struct hi_pcie_ctrl *ctrl = ctrl_port->ctrl;
	int i;
	for (i = 0; i < (int)(size / sizeof(u32)); i++)
		((u32 *)buf)[i] = ((u32 *)ctrl->ecam_base + offset)[i];
	return 0;
}

static int pcie_ctrl_write_by_cpu(struct hi_pcie_port *port, void *buf, u32 size, u32 offset)
{
	struct hi_pcie_ctrl_port *ctrl_port = port->priv;
	struct hi_pcie_ctrl *ctrl = ctrl_port->ctrl;
	int i;
	for (i = 0; i < (int)size / ((int)sizeof(u32)); i++)
		((u32 *)ctrl->ecam_base + offset)[i] = ((u32 *)buf)[i];
	return 0;
}
