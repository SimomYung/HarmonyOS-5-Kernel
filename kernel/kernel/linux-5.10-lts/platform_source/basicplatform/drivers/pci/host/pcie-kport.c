					...
					struct platform_device *pdev)
{
	...
	pcie->dtsinfo.skip_s5 = of_property_read_bool(np, "skip_s5");
}
...
static void pcie_get_eq_preset(struct pcie_kport *pcie, struct platform_device *pdev)
{
	struct device_node *np = pdev->dev.of_node;
	struct pcie_dtsinfo *dtsinfo = &pcie->dtsinfo;

	if (of_property_read_u32(np, "eq_preset", &dtsinfo->eq_preset))
		dtsinfo->eq_preset = 0xFF;
}
...
#if defined CONFIG_PCIE_KPORT_PC && defined CONFIG_PCIE_KPORT_JUN
	if ((pcie->dtsinfo.eq_preset != 0xFF) && pcie_phy_equalization_fixed_preset(pcie->rc_id, pcie->dtsinfo.eq_preset))
		PCIE_PR_E("Failed to fix pcie rc %d equalization to level %d", pcie->rc_id, pcie->dtsinfo.eq_preset);
#endif
...
static void pcie_shutdown(struct platform_device *pdev)
{
	...
	if (pcie->dtsinfo.skip_s5) {
		PCIE_PR_I("pcie shutdown skip");
		return;
	}
...
static void pcie_mask_irq(struct pcie_kport *pcie)
{
#ifdef CONFIG_PCIE_KPORT_JUN
	u32 val;

	val = pcie_apb_ctrl_readl(pcie, SOC_PCIECTRL_CTRL10_ADDR);
	val |= (BIT(11) | BIT(12) | BIT(22));
...
int pcie_kport_enumerate(u32 rc_idx)
{
	...
	if (pcie->ep_dev != NULL) {
		...
		PCIE_PR_E("%s pcie->ep_dev is NULL!", __func__);
	}
#endif
