...
struct bc_detect_cfg {
	...
	struct chip_usb_reg_cfg **bc12_dpdm_up;
	int bc12_dpdm_up_num;
	struct chip_usb_reg_cfg **bc12_dpdm_down;
	int bc12_dpdm_down_num;
	bool bc12_dpdm_dis;
};
...
close_bc12_detect:
	...
	if (!priv->bc_detect_cfg.bc12_dpdm_dis &&
		priv->bc_detect_cfg.bc12_dpdm_up && priv->bc_detect_cfg.bc12_dpdm_down &&
		(charger_type == CHARGER_TYPE_CDP || charger_type == CHARGER_TYPE_SDP)) {
		USB2_T5_INFO("dp_dm pull up_down\n");
		msleep(10);
		chip_usb_reg_write_array(priv->bc_detect_cfg.bc12_dpdm_up,
							priv->bc_detect_cfg.bc12_dpdm_up_num);
		msleep(10);
		chip_usb_reg_write_array(priv->bc_detect_cfg.bc12_dpdm_down,
							priv->bc_detect_cfg.bc12_dpdm_down_num);
	}
	return charger_type;
}
...
static int get_bc12_ops_resource(struct phy_priv *priv)
{
	...
	ret = get_chip_usb_reg_cfg_array(dev, "bc12-dpdm-up",
			&(priv->bc_detect_cfg.bc12_dpdm_up), &(priv->bc_detect_cfg.bc12_dpdm_up_num));
	if (ret)
		USB2_T5_INFO("get bc12-dpdm-up fail or no node %d\n", ret);

	ret = get_chip_usb_reg_cfg_array(dev, "bc12-dpdm-down",
			&(priv->bc_detect_cfg.bc12_dpdm_down), &(priv->bc_detect_cfg.bc12_dpdm_down_num));
	if (ret)
		USB2_T5_INFO("get bc12-dpdm-down fail or no node %d\n", ret);

	priv->bc_detect_cfg.bc12_dpdm_dis = of_property_read_bool(dev->of_node, "bc12_dpdm_dis");
...
exit_bc12_detect_dcd:
	free_chip_usb_reg_cfg_array(priv->bc_detect_cfg.bc12_dpdm_up, priv->bc_detect_cfg.bc12_dpdm_up_num);
	free_chip_usb_reg_cfg_array(priv->bc_detect_cfg.bc12_dpdm_down, priv->bc_detect_cfg.bc12_dpdm_down_num);
...
static void put_bc12_ops_resource(struct phy_priv *priv)
{
	...
	free_chip_usb_reg_cfg_array(priv->bc_detect_cfg.bc12_detect_dcd,
					priv->bc_detect_cfg.bc12_detect_dcd_num);
	free_chip_usb_reg_cfg_array(priv->bc_detect_cfg.bc12_dpdm_up,
					priv->bc_detect_cfg.bc12_dpdm_up_num);
	free_chip_usb_reg_cfg_array(priv->bc_detect_cfg.bc12_dpdm_down,
					priv->bc_detect_cfg.bc12_dpdm_down_num);
	return;
}
...
static const char * const chip_ec_version_str[] = {
	[CHIP_NO_EC] = "chip_no_ec",
	[CHIP_EC] = "chip_ec",
};

static int get_chip_ec_version(struct phy_priv *priv)
{
	const char *chip_ec_version = NULL;
	size_t actual = 0;
	int err = 0, i = 0;
	/* chip_ec_version is set by default to CHIP_EC */
	priv->chip_ec_version = CHIP_EC;

	err = device_property_read_string(priv->dev, "chip_ec_version", &chip_ec_version);
	if (err < 0){
		USB2_T5_ERR("get chip_ec_version failed, chip_ec_version is set by default to CHIP_EC\n");
		return err;
	}

	for (i = 0; i < ARRAY_SIZE(chip_ec_version_str); i++) {
		actual = strlen(chip_ec_version_str[i]) < strlen(chip_ec_version) ? strlen(chip_ec_version_str[i]) : strlen(chip_ec_version);
		if (strncmp(chip_ec_version, chip_ec_version_str[i], actual) == 0) {
			priv->chip_ec_version = (enum chip_ec_version)i;
			break;
		}
	}
	USB2_T5_INFO("read %s, chip_ec_version is set to %s\n", chip_ec_version, chip_ec_version_str[priv->chip_ec_version]);
	return 0;
}
...
static int phy_enhance_utmi_clk(struct hiusb_usb2phy *usb2_phy)
{
	...
	if (utmi_freq_exam_is_good(usb2_phy) && phy_hs_bist_is_good(usb2_phy)) {
		pr_info("[usb_utmi_clk][first check good]max support retry %d times\n", ENHANCE_UTMI_CLK_RETRY_MAX);
	...
	for (count = 1; count <= ENHANCE_UTMI_CLK_RETRY_MAX; ++count) {
		...
		if (ret) {
			pr_err("enhance t5 phy clk fail, max support retry %d times\n", ENHANCE_UTMI_CLK_RETRY_MAX);
			return ret;
		}
		if(utmi_freq_exam_is_good(usb2_phy) && phy_hs_bist_is_good(usb2_phy)) {
			pr_info("[usb_utmi_clk][retry check good]after retry %d times, max %d times, utmi clk is good!!!!\n", count, ENHANCE_UTMI_CLK_RETRY_MAX);
			return 0;
		}
	}
...
static int usb2_phy_t5_probe(struct platform_device *pdev)
{
	...
	priv->usb_phy.phy_enhance = NULL;

	if (!get_chip_ec_version(priv)) {
		if (priv->chip_ec_version == CHIP_NO_EC) {
			USB2_T5_ERR("[chip_ec][kernel] chip is no ec, do usb_enhance_clk.\n");
			priv->usb_phy.phy_enhance = usb2_phy_enhance;
		} else {
			USB2_T5_ERR("[chip_ec][kernel] chip is ec, not do usb_enhance_clk.\n");
		}
	}
