...
enum {
	PHY_PARA_PLL0SSCGCTRL = 0,
	PHY_PARA_PLL0SSCGINITIALR0,
	PHY_PARA_PLL0SSCGCNT2R0,
	PHY_PARA_PLL0RESERVED,
	PHY_PARA_PLL1RESERVED,
	PHY_PARA_RXTOPBIASSEL,
	PHY_PARA_STATICTERMCTRL,
	PHY_PARA_RX_PMA_DIG_DEBUG,
	PHY_PARA_USB_PCS_STATUS,
};
...
#define USB_TCA_PLL0_RESERVED(base)	((base) + 0xAC0)
#define USB_TCA_PLL1_RESERVED(base)	((base) + 0xCC0)
...
#define AUXCLK_STATIC_TERM_CTRL(base)	((base) + 0x1B44)
...
#define PERLANE_RXDETCTRL_LANE0(base)	(USB_TCA_PERLANE_BASE(base) + 0x31C)
#define PERLANE_RXDETCTRL_LANE1(base)	(USB_TCA_PERLANE_BASE(base) + 0x71C)
#define PERLANE_RXOFSTUPDTCTRL_LANE0(base) (USB_TCA_PERLANE_BASE(base) + 0x320)
#define PERLANE_RX_TOP_BIAS_SEL_LANE0(base) (USB_TCA_PERLANE_BASE(base) + 0x38c)
#define PERLANE_RX_TOP_BIAS_SEL_LANE1(base) (USB_TCA_PERLANE_BASE(base) + 0x78c)
...
#define PERLANE_RX_PMA_DIG_DEBUG_LANE0(base)	(USB_TCA_PERLANE_BASE(base) + 0x388)
#define PERLANE_RX_PMA_DIG_DEBUG_LANE1(base)	(USB_TCA_PERLANE_BASE(base) + 0x788)
#define PERLANE_PCS_STATUS_LANE0(base)	(USB_TCA_PERLANE_BASE(base) + 0x200)
#define PERLANE_PCS_STATUS_LANE1(base)	(USB_TCA_PERLANE_BASE(base) + 0x600)
...
static int combophy_set_dpmode(struct hiusb_usb3phy *combophy,
			     enum hiusb_phy_dp_mode dpmode, uint32_t usb_role)
{
	unused(usb_role);
	struct phy_priv *priv = chip_combophy_to_phy_priv(combophy);
...
#define config_phy_para_by_bitmap(para, bitmap, addr) \
do { \
	uint32_t bitmask = 1 << PHY_PARA_##bitmap; \
	if ((priv->phy_para_bitmap & bitmask) == bitmask) \
		writel(priv->para, addr); \
} while (0)
...
static void combophy_config_phy_para(struct phy_priv *priv)
{
	...
	config_phy_para_by_bitmap(pll0sscgctrl, PLL0SSCGCTRL, USB_TCA_PLL0SSCG_CTRL(priv->usb_tca));
	config_phy_para_by_bitmap(pll0sscginitialr0, PLL0SSCGINITIALR0, USB_TCA_PLL0SSCG_INITIAL_R0(priv->usb_tca));
	config_phy_para_by_bitmap(pll0sscgcnt2r0, PLL0SSCGCNT2R0, USB_TCA_PLL0SSCG_CNT2_R0(priv->usb_tca));
	config_phy_para_by_bitmap(statictermctrl, STATICTERMCTRL, AUXCLK_STATIC_TERM_CTRL(priv->usb_tca));
	config_phy_para_by_bitmap(pll0_reserved, PLL0RESERVED, USB_TCA_PLL0_RESERVED(priv->usb_tca));
	config_phy_para_by_bitmap(pll1_reserved, PLL1RESERVED, USB_TCA_PLL1_RESERVED(priv->usb_tca));
	config_phy_para_by_bitmap(rx_top_bias_sel, RXTOPBIASSEL, PERLANE_RX_TOP_BIAS_SEL_LANE0(priv->usb_tca));
	config_phy_para_by_bitmap(rx_top_bias_sel, RXTOPBIASSEL, PERLANE_RX_TOP_BIAS_SEL_LANE1(priv->usb_tca));
	config_phy_para_by_bitmap(rx_pma_dig_debug, RX_PMA_DIG_DEBUG, PERLANE_RX_PMA_DIG_DEBUG_LANE0(priv->usb_tca));
	config_phy_para_by_bitmap(rx_pma_dig_debug, RX_PMA_DIG_DEBUG, PERLANE_RX_PMA_DIG_DEBUG_LANE1(priv->usb_tca));
	config_phy_para_by_bitmap(pcs_status, USB_PCS_STATUS, PERLANE_PCS_STATUS_LANE0(priv->usb_tca));
	config_phy_para_by_bitmap(pcs_status, USB_PCS_STATUS, PERLANE_PCS_STATUS_LANE1(priv->usb_tca));
}
...
static void combophy_reg_dump(struct phy_priv *priv)
{
	uint32_t reg;

	reg = readl(USB_TCA_PHY_MODE(priv->usb_tca));
	pr_err("combophy_dump_reg phymode 0x%x", reg);
	reg = readl(PERLANE_RXDETCTRL_LANE0(priv->usb_tca));
	pr_err("combophy_dump_reg rxdet lan0 0x%x", reg);
	reg = readl(PERLANE_RXDETCTRL_LANE1(priv->usb_tca));
	pr_err("combophy_dump_reg rxdet lan1 0x%x", reg);
}
...
test_stub:
	...
	priv->debug_flag = of_property_read_bool(priv->dev->of_node, "debug-flag");
	pr_info("debug_flag %s\n", priv->debug_flag ? "true" : "false");
...
#define of_get_combophy_param_bitmap(para, bitmap) \
do { \
	int ret; \
	ret = of_property_read_u32(np, #para, &priv->para); \
	if (!ret) \
		priv->phy_para_bitmap |= (1 << PHY_PARA_##bitmap); \
} while (0)
...
static void get_phy_param(struct device_node *np, struct phy_priv *priv)
{
	...
	of_get_combophy_param_bitmap(pll0sscgctrl, PLL0SSCGCTRL);
	of_get_combophy_param_bitmap(pll0sscginitialr0, PLL0SSCGINITIALR0);
	of_get_combophy_param_bitmap(pll0sscgcnt2r0, PLL0SSCGCNT2R0);
	of_get_combophy_param_bitmap(statictermctrl, STATICTERMCTRL);
	of_get_combophy_param_bitmap(pll0_reserved, PLL0RESERVED);
	of_get_combophy_param_bitmap(pll1_reserved, PLL1RESERVED);
	of_get_combophy_param_bitmap(rx_top_bias_sel, RXTOPBIASSEL);
	of_get_combophy_param_bitmap(rx_pma_dig_debug, RX_PMA_DIG_DEBUG);
	of_get_combophy_param_bitmap(pcs_status, USB_PCS_STATUS);
}
