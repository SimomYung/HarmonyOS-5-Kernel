...
#define ACCESS_FLAG 31
#define IOMCU_PW_STATUS 30
#define MAP_SIZE 0x1000

static void __iomem *g_sctrl_base = NULL;

static bool check_iomcu_status(struct pch_device *pch)
{
	if ((pch->iomcu_ioc_flag == 0) || (g_sctrl_base == NULL))
		return false;

	/* set bit */
	writel(readl(g_sctrl_base + ACCESS_STATUS) | BIT(ACCESS_FLAG), g_sctrl_base + ACCESS_STATUS);
	if (iomcu_check_dump_status(false) || \
		((readl(g_sctrl_base + ACCESS_STATUS) & BIT(IOMCU_PW_STATUS)) >> IOMCU_PW_STATUS)) {
		pr_info("[%s] iomcu is deepsleep or reset, can't access ioc!\n", __func__);
		/* clr bit */
		writel(readl(g_sctrl_base + ACCESS_STATUS) & ~BIT(ACCESS_FLAG), g_sctrl_base + ACCESS_STATUS);
		return true;
	}
...
static void clr_status(struct pch_device *pch)
{
	if ((pch->iomcu_ioc_flag == 0) || (g_sctrl_base == NULL))
		return;
