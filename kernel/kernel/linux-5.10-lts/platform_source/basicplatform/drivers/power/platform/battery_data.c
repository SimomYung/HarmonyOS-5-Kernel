...
static int g_temp_points_chg_ocv[] = { 0, 15, 25, 40 };
...
#ifdef CONFIG_HUAWEI_BATTERY_INFORMATION
	ret = get_battery_type_by_id(batt_index, id_sn, ID_SN_SIZE);
	if (ret) {
		bat_data_err("get id_sn from ic fail!\n");
	...
	struct coul_battery_data *pdat)
{
	...
	for (i = 0; i < pdat->fcc_temp_lut->cols; i++) {
		...
		(void)get_decress_fcc_temp(np, pdat, i);
	...
	struct coul_battery_data *pdat)
{
	...
	for (i = 0; i < pdat->pc_temp_ocv_lut0->cols; i++) { /* 6 */
		for (j = 0; j < pdat->pc_temp_ocv_lut0->rows; j++) { /* 29 */
			...
			bat_info("pc_temp_ocv_lut0[%d][%d] is %d\n",
				j, i, pdat->pc_temp_ocv_lut0->ocv[j][i]);
		}
	}
	return 0;
}
...
static int get_dat_pc_temp_chg_ocv(struct device_node *np,
	struct coul_battery_data *pdat)
{
	int ret, i, j;

	/* chg ocv */
	ret = of_property_read_u32(np, "pc_temp_ocv_chg_rows", /* 29 */
		(unsigned int *)(&(pdat->pc_temp_ocv_chg_lut->rows)));
	ret += of_property_read_u32(np, "pc_temp_ocv_chg_cols", /* 4 */
		(unsigned int *)(&(pdat->pc_temp_ocv_chg_lut->cols)));
	if (ret || pdat->pc_temp_ocv_chg_lut->cols > TEMP_SAMPLING_POINTS_CHG_OCV ||
		pdat->pc_temp_ocv_chg_lut->rows != pdat->pc_temp_ocv_lut0->rows) {
		bat_data_err("get pc_temp_ocv_chg failed\n");
		return -1;
	}
	bat_info("pc_temp_ocv_chg rows=%d, cols=%d\n",
		pdat->pc_temp_ocv_chg_lut->rows, pdat->pc_temp_ocv_chg_lut->cols);

	for (i = 0; i < pdat->pc_temp_ocv_chg_lut->cols; i++)
		pdat->pc_temp_ocv_chg_lut->temp[i] = g_temp_points_chg_ocv[i];
	for (i = 0; i < pdat->pc_temp_ocv_chg_lut->rows - 1; i++)
		pdat->pc_temp_ocv_chg_lut->percent[i] = pdat->pc_temp_ocv_lut0->percent[i];

	for (i = 0; i < pdat->pc_temp_ocv_chg_lut->cols; i++) { /* 4 */
		for (j = 0; j < pdat->pc_temp_ocv_chg_lut->rows; j++) { /* 29 */
			ret = of_property_read_u32_index(np, "pc_temp_ocv_chg",
			  j * pdat->pc_temp_ocv_chg_lut->cols + i, (unsigned int *)(
			  &(pdat->pc_temp_ocv_chg_lut->ocv[j][i]))); /* cols */
			if (ret) {
				bat_data_err("get pc_temp_ocv_chg_lut[%d] failed\n",
					j * pdat->pc_temp_ocv_chg_lut->cols + i);
				return -1;
			}
			bat_info("pc_temp_ocv_chg[%d][%d] is %d\n",
				j, i,
				pdat->pc_temp_ocv_chg_lut->ocv[j][i]);
		}
	}
	return 0;
}

static int get_dat_cur_temp_uuc(struct device_node *np,
	struct coul_battery_data *pdat)
{
	int ret, i, j;

	ret = of_property_read_u32(np, "cur_temp_uuc_rows",
		(unsigned int *)(&(pdat->cur_temp_uuc->rows)));
	if (ret) {
		pdat->cur_temp_uuc->rows = 0;
		return 0;
	}
	bat_data_info("get cur_temp_uuc from dts!, rows=%d\n", pdat->cur_temp_uuc->rows);

	ret = of_property_read_u32_array(np, "cur_temp_uuc_current",
			(u32 *)(pdat->cur_temp_uuc->cur), pdat->cur_temp_uuc->rows);
	if (ret) {
		bat_data_err("get cur_temp_uuc_current failed\n");
		return -EINVAL;
	}

	for (i = 0; i < TEMP_SAMPLING_POINTS; i++) { /* 6 */
		pdat->cur_temp_uuc->temp[i] = g_temp_points[i];
		for (j = 0; j < pdat->cur_temp_uuc->rows; j++) {
			ret = of_property_read_u32_index(np, "cur_temp_uuc",
			  j * TEMP_SAMPLING_POINTS + i, (unsigned int *)(
			  &(pdat->cur_temp_uuc->uuc[j][i]))); /* cols */
			if (ret) {
				bat_data_err("get cur_temp_uuc_data[%d] failed\n",
					j * TEMP_SAMPLING_POINTS + i);
				return -EINVAL;
			}
		}
...
static int get_dat_cap_dense_area(struct device_node *np,
	struct coul_battery_data *pdat)
{
	int ret, i, j;

	ret = of_property_read_u32(np, "cap_dense_area_rows",
		(unsigned int *)(&(pdat->cap_dense_area->rows)));
	if (ret) {
		pdat->cap_dense_area->rows = 0;
		return 0;
	}
	if (pdat->cap_dense_area->rows > MAX_CAP_DENSE_AREA_ROWS) {
		bat_data_err("cap_dense_area error: rows=%d too long!\n", pdat->cap_dense_area->rows);
		return -EINVAL;
	}
	bat_data_info("get cap_dense_area from dts!, rows=%d\n", pdat->cap_dense_area->rows);
	for (j = 0; j < pdat->cap_dense_area->rows; j++) {
		ret = 0;
		ret += of_property_read_u32_index(np, "cap_dense_area",
			j * CAP_DENSE_AREA_COLS, (unsigned int *)(
			&(pdat->cap_dense_area->start[j]))); /* start */
		ret += of_property_read_u32_index(np, "cap_dense_area",
			j * CAP_DENSE_AREA_COLS + 1, (unsigned int *)(
			&(pdat->cap_dense_area->end[j]))); /* end */
		if (ret) {
			bat_data_err("get cap_dense_area[%d] failed\n", j);
			return -EINVAL;
		}
		bat_data_info("cap_dense_area start=%d, end=%d\n",
			pdat->cap_dense_area->start[j], pdat->cap_dense_area->end[j]);
	}
	return 0;
}

static int get_dat_soe_params(struct device_node *np,
	struct coul_battery_data *pdat)
{
	int ret, i, array_len;
	const char *soe_data_string = NULL;

	array_len = of_property_count_strings(np, "soe_params_abcd");
	if (array_len != sizeof(*pdat->soe_params) / sizeof(int)) {
		bat_data_err("soe_param is invaild, use default val, array_len=%d!\n", array_len);
		return 0;
	}
	for (i = 0; i < array_len; i++) {
		ret = of_property_read_string_index(np,
			"soe_params_abcd", i, &soe_data_string);
		if (ret) {
			bat_data_err("get soe_param failed\n");
			return -EINVAL;
		}
		ret = kstrtoint(soe_data_string, DEC, (&(pdat->soe_params->a) + i));
		if (ret < 0) {
			bat_data_err("convert soe_param data failed\n");
			return -EINVAL;
		}
	}
	bat_data_info("get soe_params a=%d, b=%d, c=%d, d=%d\n",
		pdat->soe_params->a, pdat->soe_params->b, pdat->soe_params->c, pdat->soe_params->d);
	return 0;
}
...
static int get_dat(struct device_node *np, struct coul_battery_data *pdat)
{
	...
	if (ret)
		return -EINVAL;
	ret = get_dat_pc_temp_chg_ocv(np, pdat);
	if (ret) {
		kfree(pdat->pc_temp_ocv_chg_lut); /* if no chg data, free it, but program is normal */
		pdat->pc_temp_ocv_chg_lut = NULL;
	}

	/* cur_temp_uuc (optional) */
	ret = get_dat_cur_temp_uuc(np, pdat);
	if (ret)
		return -EINVAL;
	
	ret = get_dat_cap_dense_area(np, pdat);
	if (ret)
		return ret;

	ret = get_dat_soe_params(np, pdat);
	return ret;
}
...
static int get_mem(struct coul_battery_data **p)
{
	...
	pdat->cur_temp_uuc = kzalloc(sizeof(*pdat->cur_temp_uuc), GFP_KERNEL);
	if (!pdat->cur_temp_uuc)
		goto get_mem_fail_3;

	pdat->cap_dense_area = kzalloc(sizeof(*pdat->cap_dense_area), GFP_KERNEL);
	if (!pdat->cap_dense_area)
		goto get_mem_fail_4;

	pdat->soe_params = kzalloc(sizeof(*pdat->soe_params), GFP_KERNEL);
	if (!pdat->soe_params)
		goto get_mem_fail_5;

	pdat->pc_temp_ocv_chg_lut = kzalloc(sizeof(*pdat->pc_temp_ocv_chg_lut), GFP_KERNEL);
	if (!pdat->pc_temp_ocv_chg_lut)
		goto get_mem_fail_6;
	...
	*p = pdat;
	return 0;
get_mem_fail_7:
	kfree(pdat->pc_temp_ocv_chg_lut);
	pdat->pc_temp_ocv_chg_lut = NULL;
get_mem_fail_6:
	kfree(pdat->soe_params);	
	pdat->soe_params = NULL;
get_mem_fail_5:
	kfree(pdat->cap_dense_area);
	pdat->cap_dense_area = NULL;
get_mem_fail_4:
	kfree(pdat->cur_temp_uuc);
	pdat->cur_temp_uuc = NULL;
get_mem_fail_3:
