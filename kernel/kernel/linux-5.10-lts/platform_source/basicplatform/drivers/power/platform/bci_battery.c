...
struct bci_device_info {
	...
	struct cap_lock_para vth_correct_data_low_temp[CAP_PARA_LOW_TEMP_LEVEL];
	unsigned int vth_correct_en;
	unsigned int vth_correct_low_temp_en;
	u32 disable_pre_vol_check;
	u32 sc_uisoc_update_speed;
	u32 battery_ov_th;
	u32 psy_use_cache;
	u32 bci_work_dc_interval;
	u32 hold_full_soc_enable;
	int last_cap_enable;
...
static enum power_supply_property bci_battery_props[] = {
	...
	POWER_SUPPLY_PROP_OFF_FIND,
	POWER_SUPPLY_PROP_DISCHARGE_CC,
};
...
static void bci_get_ocv_corresponds_cap_low_temp_by_basp(
	struct bci_device_info *di)
{
	int voltage_1 = 0;
	int voltage_2 = 0;
	...
	ret = bci_get_get_ocv_by_cap(CAPACITY_MINUS_ONE, &voltage_1);
	ret += bci_get_get_ocv_by_cap(CAPACITY_ZERO, &voltage_2);
	if (ret) {
		bci_err("%s %d, get voltage by cap failed\n",
			__func__, __LINE__);
		unused(di);
	...
	bci_info("%s, voltage_-1 = %d, voltage_0 = %d\n",  __func__, voltage_1, voltage_2);

	for (i = 0; i < CAP_PARA_LOW_TEMP_LEVEL; i++) {
		if (di->vth_correct_data_low_temp[i].cap == CAPACITY_ONE) {
			di->vth_correct_data_low_temp[i].level_vol = voltage_1;
			bci_err("%s %d, di->vth_correct_data_low_temp[%d].level_vol = %d \n",
				__func__, __LINE__,
				i, di->vth_correct_data_low_temp[i].level_vol);
		}
		if (di->vth_correct_data_low_temp[i].cap == CAPACITY_TWO) {
			di->vth_correct_data_low_temp[i].level_vol = voltage_2;
			bci_err("%s %d, di->vth_correct_data_low_temp[%d].level_vol = %d \n",
				__func__, __LINE__,
				i, di->vth_correct_data_low_temp[i].level_vol);
		}
...
static int calc_lock_capacity(int curr_capacity, int prev_capacity,
	struct cap_lock_para *vth_correct_data, int i)
{
	int battery_volt = 0;
	int count = DEBOUNCE_TIMES;
	int bat_vot;
	...
	if (battery_volt >= vth_correct_data[i].level_vol) {
		bci_info("low capacity reported, battery_vol = %d mv, prev_capacity = %d, capacity = %d, lock_cap:%d\n",
			battery_volt, prev_capacity, curr_capacity,
			vth_correct_data[i].cap);
		return vth_correct_data[i].cap;
	}
	return curr_capacity;
}

static int check_curr_low_capacity(int curr_capacity, struct bci_device_info *di)
{
	int i;
	for (i = 0; i < CAP_LOCK_PARA_LEVEL; i++) {
		if ((curr_capacity < di->vth_correct_data[i].cap) &&
			...
			return calc_lock_capacity(curr_capacity, di->prev_capacity, di->vth_correct_data, i);
		}
	}
	return curr_capacity;
}

static int check_capacity_low_temp(int curr_capacity, struct bci_device_info *di)
{
	int i;
	if (!di->vth_correct_low_temp_en)
		return curr_capacity;

	for (i = 0; i < CAP_PARA_LOW_TEMP_LEVEL; i++) {
		if ((curr_capacity < di->vth_correct_data_low_temp[i].cap) &&
			(di->prev_capacity >= (unsigned int)di->vth_correct_data_low_temp[i].cap)) {
			bci_get_ocv_corresponds_cap_low_temp_by_basp(di);
			curr_capacity = calc_lock_capacity(curr_capacity, di->prev_capacity, di->vth_correct_data_low_temp, i);
		}
	}
	return curr_capacity;
}
		...
		if(bat_volt <= di->cap_one_vth) {
			...
			if (count == 0) {
				curr_capacity =
					...
					(cap - 1) :
					(CAPACITY_ONE);
				bci_info("%s bat_volt=%d, inp cap=%d, out cap=%d\n",
					__func__, bat_volt, cap, curr_capacity);
				blocking_notifier_call_chain(&g_notifier_list, BATTERY_CAP_ONE_BY_VTH, curr_capacity);
			}
		}
	}
	return curr_capacity;
...
static int vth_correct_soc(int curr_capacity, struct bci_device_info *di)
{
	...
	if ((di->charge_status == POWER_SUPPLY_STATUS_DISCHARGING) ||
		...
		if (di->bat_temperature > 0)
			return check_curr_low_capacity(curr_capacity, di);
		else
			return check_capacity_low_temp(curr_capacity, di);
	}
	return curr_capacity;
}
	...
	struct bci_device_info *di)
{
	...
	if ((!di->bat_exist) &&
		(strstr(saved_command_line, "ohos.boot.swtype=factory") || 
...
static void report_ui_capacity_jump(int prev_capacity, int correct_capacity)
{
	int ret, delta_cap;
#if defined(CONFIG_HUAWEI_DSM) || defined(CONFIG_HUAWEI_OHOS_DSM)
	char buf[DSM_BATTERY_MAX_SIZE] = { 0 };
#endif

	delta_cap = abs(prev_capacity - correct_capacity);
	bci_info("%s delta_soc = %d, prev_soc = %d, cur_soc = %d\n",
		__func__, delta_cap, prev_capacity, correct_capacity);
	if (delta_cap < CHG_ODD_CAPACITY)
		return;
#if defined(CONFIG_HUAWEI_DSM) || defined(CONFIG_HUAWEI_OHOS_DSM)
	ret = snprintf_s(buf, DSM_BATTERY_MAX_SIZE, DSM_BATTERY_MAX_SIZE - 1,
		"[ui_capacity_jump]delta_soc = %d, prev_soc = %d, curr_soc = %d\n",
		delta_cap, prev_capacity, correct_capacity);
	if (ret < 0) {
		bci_err("%s snprintf_s failed\n", __func__);
		return;
	}
	power_dsm_report_dmd(POWER_DSM_BATTERY, ERROR_BATT_SOC_JUMP_CHANGE, buf);
#endif
	bci_info("%s delta_soc = %d, prev_soc = %d, cur_soc = %d\n",
		__func__, delta_cap, prev_capacity, correct_capacity);
}
	...
	struct bci_device_info *di)
{
	...
	report_ui_capacity_jump(di->prev_capacity, correct_capacity);
...
#endif
			/* if multi battery, get MAX voltage */
			if (di->hold_full_soc_enable && (battery_volt >=
...
static int capacity_changed(struct bci_device_info *di)
{
	...
	if (di->basp_map_en == BASP_MAP_ON)
		basp_capacity = bci_get_capacity_map_by_basp(curr_capacity);
	else
		basp_capacity = curr_capacity;
...
static int check_batt_terminate_too_early(char *buf)
{
	...
	if (di->chargedone_stat && capacity <= CHG_TE_EARLY_SOC)
		return 1;
	...
	char *buf, int i)
{
	if (batt_dsm_array[i].check_error(buf)) {
		...
		power_dsm_report_dmd(POWER_DSM_BATTERY, di->bat_err, buf);
	...
	union power_supply_propval *val, struct bci_device_info *di)
{
	...
	case POWER_SUPPLY_PROP_CHARGE_COUNTER:
		if (di->psy_use_cache)
			...
			val->intval = coul_drv_battery_cc_uah();
	...
	case POWER_SUPPLY_PROP_CATHODE_TYPE:
		...
		break;
	case POWER_SUPPLY_PROP_OFF_FIND:
		val->intval = coul_drv_battery_get_off_find();
		break;
...
#endif
		break;
	case POWER_SUPPLY_PROP_DISCHARGE_CC:
		val->intval = coul_drv_battery_discharge_cc();
		bci_info("discharge_cc_uah:%d \n", val->intval);
		break;
	default:
		return -EINVAL;
	}
...
static int bci_battery_set_property(struct power_supply *psy,
	enum power_supply_property psp, const union power_supply_propval *val)
{
	...
	if (!di) {
		bci_err("NULL point in [%s]\n", __func__);
		return -EINVAL;
	}

	switch (psp) {
	case POWER_SUPPLY_PROP_OFF_FIND:
		bci_info("[%s] set off_find flag=%d\n", __func__, val->intval);
		ret = coul_drv_battery_set_off_find(val->intval);
		break;
	default:
		return -EINVAL;
	}
...
static const struct power_supply_desc bci_battery_desc = {
	...
	.set_property = bci_battery_set_property,
};
...
void coul_clear_last_soc_flag(void)
{
	reg_spmi_write(COUL_HARDWARE_SAVE_LAST_SOC, 0);
}

/* smooth first capacit to avoid capacit jump in startup step, clear flag after use */
	...
	struct bci_device_info *di)
{
	...
	size_t array_size_low_temp = sizeof(di->vth_correct_data_low_temp);
	...
	/* vth_correct_data */
	count = of_property_count_elems_of_size(np, "vth_correct_para", sizeof(int));
	...
	/* vth_correct_data_low_temp */
	if (of_property_read_u32(np,
		"vth_correct_low_temp_en", (u32 *)&di->vth_correct_low_temp_en)) {
		di->vth_correct_low_temp_en = 0;
		bci_err("error:get vth_correct_low_temp_en value failed, used default value!\n");
	}

	count = of_property_count_elems_of_size(np, "vth_correct_para_low_temp", sizeof(int));
	if ((count <= 0) || ((count % CAP_PARA_MEM_NUM) != 0) ||
		(count > (array_size_low_temp / int_size))) {
		di->vth_correct_low_temp_en = 0;
		bci_err("Uncorrect vth_correct_para_low_temp size %d\n", count);
		return 0;
	}

	if (of_property_read_u32_array(np, "vth_correct_para_low_temp",
			(u32 *)&di->vth_correct_data_low_temp[0], count)) {
		di->vth_correct_low_temp_en = 0;
		bci_err("get vth_correct_para_low_temp fail!!\n");
	}

	for (i = 0; i < CAP_PARA_LOW_TEMP_LEVEL; i++)
		bci_info("di->vth_correct_data_low_temp[%d]:%d,%d\n", i,
			di->vth_correct_data_low_temp[i].cap,
			di->vth_correct_data_low_temp[i].level_vol);
	...
	struct bci_device_info *di)
{
	...
	if (of_property_read_u32(np, "hold_full_soc_enable", &di->hold_full_soc_enable))
		di->hold_full_soc_enable = 1;
}
	...
	struct bci_device_info *di)
{
	...
	if (of_property_read_u32(np, "basp_map_en", (u32 *)&di->basp_map_en))
		di->basp_map_en = BASP_MAP_ON;
