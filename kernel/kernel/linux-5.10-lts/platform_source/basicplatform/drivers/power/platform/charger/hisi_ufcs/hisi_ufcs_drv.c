...
#define ufcs_debug(fmt, args...) pr_debug("[hi_ufcs] %s " fmt, __func__, ## args)
...
static int g_dmd_cnt;
#ifdef SCHARGER_DPDM_OVP
struct ufcs_driver_info *g_ufcs = NULL;
#endif
...
#if defined(CONFIG_HUAWEI_DSM) || defined(CONFIG_HUAWEI_OHOS_DSM)
	if ((irq_flag_val != 0) && (g_dmd_cnt == 0)) {
		g_dmd_cnt++;
		length = snprintf_s(buf, sizeof(buf), sizeof(buf) - 1,
			"ufcs irq non zero before ufcs en!, irq_flag_val:0x%x\n", irq_flag_val);
		if (length < 0)
			scharger_err("%s, snprintf_s error %d\n", __func__, length);
		power_dsm_report_dmd(POWER_DSM_BATTERY, DSM_SC0_INFO, buf);
	}
#endif
...
static bool hisi_ufcs_is_ignore_accept_in_auth_encrypt(void *dev_data)
{
	struct ufcs_driver_info *ufcs = dev_data;
	...
	return (ufcs->chip_version == SCHARGER_VERSION_V600);
}
...
static struct hwufcs_ops ufcs_drv_ops = {
	...
	.is_ignore_accept_in_auth_encrypt = hisi_ufcs_is_ignore_accept_in_auth_encrypt,
};
...
#ifdef SCHARGER_DPDM_OVP
		regs->ufcs_irq_flg_11 =       CHG_IRQ_FLAG_11_REG;
		regs->ufcs_dpdm_ovp_en =      CHG_SYS_DET_CFG_REG_7_REG;
		regs->ufcs_dpdm_ovp_en_msk =  CHG_DPDM_OVP_EN_MSK;
		regs->ufcs_dpdm_ovp_en_shift = CHG_DPDM_OVP_EN_SHIFT;
#endif
	} else {
...
#ifdef SCHARGER_DPDM_OVP
static int hisi_ufcs_irq_dpdm_ovp_enable(struct ufcs_driver_info *ufcs, unsigned int enable)
{
	ufcs_info("%s en:%u\n", __func__, enable);
	return regmap_update_bits(ufcs->regmap, ufcs->regs.ufcs_dpdm_ovp_en,
		ufcs->regs.ufcs_dpdm_ovp_en_msk, !!enable << ufcs->regs.ufcs_dpdm_ovp_en_shift);
}

static void irq_ufcs_dpdm_ovp_handle(struct ufcs_driver_info *ufcs)
{
	if (gpio_is_valid(ufcs->dpdm_ovp_gpio)) {
		gpio_set_value(ufcs->dpdm_ovp_gpio, 1);
		ufcs_err("%s gpio %u set output 1\n", __func__, ufcs->dpdm_ovp_gpio);
	}
}

void reset_dpdm_ovp_gpio(void)
{
	if (!g_ufcs || g_ufcs->dpdm_ovp_handle_en != 1) {
		ufcs_err("%s g_ufcs is null or dpdm_ovp_handle_en is 0\n", __func__);
		return;
	}
	ufcs_info("%s\n", __func__);

	if (gpio_is_valid(g_ufcs->dpdm_ovp_gpio))
		gpio_set_value(g_ufcs->dpdm_ovp_gpio, 0);
}
EXPORT_SYMBOL(reset_dpdm_ovp_gpio);
#endif
...
#ifdef SCHARGER_DPDM_OVP
	if (ufcs->dpdm_ovp_handle_en == 1) {
		regmap_read(ufcs->regmap, ufcs->regs.ufcs_irq_flg_11, &irq_flg_11);
		if ((irq_flg_11 & IRQ_UFCS_DP_OVP) || (irq_flg_11 & IRQ_UFCS_DM_OVP)) {
			irq_ufcs_dpdm_ovp_handle(ufcs);
			ufcs_err("IRQ_UFCS_DP_OVP or IRQ_UFCS_DM_OVP, irq_flg_11:0x%x\n", irq_flg_11);
		}
		regmap_write(ufcs->regmap, ufcs->regs.ufcs_irq_flg_11, irq_flg_11);
	}
#endif
...
#ifdef SCHARGER_DPDM_OVP
static void dpdm_ovp_gpio_init(struct ufcs_driver_info *ufcs)
{
	if (of_property_read_u32(ufcs->dev->of_node, "dpdm_ovp_handle_en", &(ufcs->dpdm_ovp_handle_en))) {
		ufcs->dpdm_ovp_handle_en = 0;
		ufcs_err("%s, get dpdm_ovp_handle_en failed, use default val 0\n", __func__);
		return;
	}
	if (ufcs->dpdm_ovp_handle_en == 1) {
		ufcs->dpdm_ovp_gpio = of_get_named_gpio(ufcs->dev->of_node, "dpdm_ovp_gpio", 0);
		if (!gpio_is_valid(ufcs->dpdm_ovp_gpio)) {
			ufcs->dpdm_ovp_handle_en = 0;
			ufcs_err("%s, dpdm_ovp_gpio %u is not valid\n", __func__, ufcs->dpdm_ovp_gpio);
			return;
		}
		if (gpio_request(ufcs->dpdm_ovp_gpio, "dpdm_ovp_gpio")) {
			ufcs->dpdm_ovp_handle_en = 0;
			ufcs_err("%s, dpdm_ovp_gpio %u request fail\n", __func__, ufcs->dpdm_ovp_gpio);
			return;
		}
		if (gpio_direction_output(ufcs->dpdm_ovp_gpio, 0)) {
			ufcs->dpdm_ovp_handle_en = 0;
			ufcs_err("%s, gpio %u set output fail\n", __func__, ufcs->dpdm_ovp_gpio);
			gpio_free(ufcs->dpdm_ovp_gpio);
			return;
		}
		hisi_ufcs_irq_dpdm_ovp_enable(ufcs, 1);
	}
}
#endif
