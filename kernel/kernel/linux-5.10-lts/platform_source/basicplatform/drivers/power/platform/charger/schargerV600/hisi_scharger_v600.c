...
static int hi6526_get_vbus_vset(int *value)
{
	struct hi6526_device_info *di = g_hi6526_dev;
...
static bool is_factory_mode(char *saved_command_line)
{
	return strstr(saved_command_line, "ohos.boot.swtype=factory") ||
...
int hi6526_set_input_current(int cin_limit)
{
	...
	if (di->input_limit_flag) {
		if (di->buck_vbus_set < VBUS_VSET_9V) {
			if (is_factory_mode(saved_command_line))
				max = CHG_ILIMIT_1500;
			else
				max = CHG_ILIMIT_1100;
		} else if (di->buck_vbus_set < VBUS_VSET_12V) {
	...
	if (di->param_dts.i_in_limit_calibration && (i_in_limit > 1))
		i_in_limit -= 1;
	iin_set = cin_limit;
...
static void hi6526_overtemp_fault_hander(void)
{
	...
	(void)power_supply_get_property(psy, POWER_SUPPLY_PROP_VOLTAGE_NOW, &val);
	vbat = val.intval / 1000;

	(void)power_supply_get_property(psy, POWER_SUPPLY_PROP_TEMP, &val);
...
struct charge_device_ops hi6526_ops = {
	...
	.get_vbus_vset = hi6526_get_vbus_vset,
...
static void hi6526_ops_register(struct hi6526_device_info *di)
{
	...
	(void)hisi_tcpc_reg_ops_register(&hi6526_tcpc_reg_ops);
	(void)hisi_tcpc_irq_gpio_register(di->gpio_int);
...
static void regmap_unlock_mutex(void *context)
{
	...
	rt_mutex_unlock(&di->regmap_lock);
}
