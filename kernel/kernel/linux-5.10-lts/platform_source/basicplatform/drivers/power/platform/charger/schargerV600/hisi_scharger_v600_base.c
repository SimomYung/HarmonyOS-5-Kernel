...
int hi6526_get_adc_value(u32 chan, u32 *data)
{
	...
	if ((di->param_dts.adc_single_mode_en == 0) && (lvc_mode || sc_mode)) {
		...
		if (ret)
			scharger_err("[%s] hi6526_read CHG_ADC_DATA fail,ret:%d\n",
				__func__, ret);
...
void parse_dts(struct device_node *np, struct hi6526_device_info *di)
{
	...
	ret = of_property_read_u32(np, "i_in_limit_calibration",
				   (u32 *)&(di->param_dts.i_in_limit_calibration));
	if (ret)
		di->param_dts.i_in_limit_calibration = 0;
	scharger_inf("prase_dts i_in_limit_calibration=%d\n",
		di->param_dts.i_in_limit_calibration);
