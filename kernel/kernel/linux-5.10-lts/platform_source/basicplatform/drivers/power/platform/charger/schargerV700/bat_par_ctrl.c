...
#define BUCK_FFC_CHARGE_VOTER           "BUCK_FFC_CHARGE"
...
void bat_par_ctrl_set_lvc_charge_flag(bool flag)
{
	struct bat_par_ctrl *di = g_bat_par_ctrl_dev;
	...
	if (flag) {
		di->lvc_charge_flag = true;
		power_vote_set(BAT_PAR_CTRL_VOTE_OBJECT, BUCK_CV_CHARGE_VOTER, false, false);
	} else {
		di->lvc_charge_flag = false;
	}
	hwlog_info("%s flag=%u\n", __func__, flag);
}
...
static void bat_par_ctrl_batfet(void)
{
	...
	for (i = 0; i < di->info_num; i++) {
		...
		enable = enable && (socs[i] >= di->ctrl_info[i].soc_th);
	}

	if (enable && (socs[0] > socs[1])) {
		power_vote_set(BAT_PAR_CTRL_VOTE_OBJECT, BUCK_FFC_CHARGE_VOTER, true, true);
		set_batfet_single_ctrl(1, DBFET_BAT_H_OFF);
	} else {
		set_batfet_single_ctrl(0, DBFET_BAT_H_OFF);
		power_vote_set(BAT_PAR_CTRL_VOTE_OBJECT, BUCK_FFC_CHARGE_VOTER, false, false);
	}
}
...
static void bat_par_ctrl_work(struct work_struct *work)
{
	...
	hwlog_info("%s on_cnt=%d off_cnt=%d lvc_charge=%u\n", __func__,
		switch_on_cnt, switch_off_cnt, di->lvc_charge_flag);
	if ((switch_off_cnt == di->info_num) && !di->lvc_charge_flag &&
		...
		hwlog_info("%s set switch off\n", __func__);
	}

	if (switch_off_flag && (switch_on_cnt || di->lvc_charge_flag)) {
		power_vote_set(BAT_PAR_CTRL_VOTE_OBJECT, BUCK_CV_CHARGE_VOTER,
			false, false);
		switch_off_flag = false;
		hwlog_info("%s set switch on\n", __func__);
	}
	...
	struct bat_par_ctrl *di)
{
	...
	if (power_cmdline_is_factory_mode())
		(void)power_dts_read_u32(power_dts_tag(HWLOG_TAG), np,
			"fac_bat_temp_th", (u32 *)&di->bat_temp_th, FAC_BAT_TEMP_TH);
	else
		(void)power_dts_read_u32(power_dts_tag(HWLOG_TAG), np,
