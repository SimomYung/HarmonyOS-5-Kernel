...
{
	int ret = 0;
	struct scharger_buck *buck = scharger_v700_get_buck_device();

	if (dc == NULL || buck == NULL) {
		scharger_err("%s scharger_dc or buck is NULL!\n", __func__);
		return -EINVAL;
	}
	...
	/* usb plugin enable */
	(void)regmap_update_bits(dc->regmap, REG_USB_OVP_CFG_REG_2, USB_PLUGIN_EN_MSK,
			 USB_PLUGIN_EN << USB_PLUGIN_EN_SHIFT);
	/* set usb ovp 11v */
	(void)set_vusb_ovp(USB_OVP_11V);
...
{
	if (!dc->lvc_init_flag || dc->iwatt_lvc_adap || (adapter_detect_get_runtime_protocol_type() == BIT(ADAPTER_PROTOCOL_PPS)) ||
...
static int dc_get_ibat_ma(int *ibat, void *dev_data)
{
	...
	(void)dc_get_coul_ibat(dc->batt_l_name, &ibat_l);
	(void)dc_get_coul_ibat(dc->batt_h_name, &ibat_h);
	*ibat = (ibat_l + ibat_h);
...
static int dc_get_low_batt_vbat_mv(void *dev_data)
{
	int ret = 0;
	int vbat_l = 0;
	struct scharger_dc *dc = dev_data;
...
static int sc_charge_init(void *dev_data)
{
	...
	ret += regmap_update_bits(dc->regmap, PSW_OVP_CFG_REG_1, OVP_CLK_MODE_MSK, OVP_CLK_16M);
	ret += regmap_update_bits(dc->regmap, PSW_OVP_CFG_REG_3, OVP_DELAY_MSK,
			OVP_DELAY_2US_1MS << OVP_DELAY_SHIFT);
	/* set usb ovp 11v */
...
static int lvc_charge_init(void *dev_data)
{
	...
	bat_par_ctrl_set_lvc_charge_flag(true);
	return ret;
}
...
static int dc_charge_exit(void *dev_data)
{
	...
	ret += regmap_update_bits(dc->regmap, PSW_OVP_CFG_REG_1, OVP_CLK_MODE_MSK, OVP_CLK_6M);
	ret += regmap_update_bits(dc->regmap, PSW_OVP_CFG_REG_3, OVP_DELAY_MSK,
			OVP_DELAY_32US_128US << OVP_DELAY_SHIFT);
	dc->dc_chg_mode = NONE;
	dc->iwatt_lvc_adap = false;
	dc->lvc_init_flag = false;
	bat_par_ctrl_set_lvc_charge_flag(false);
	...
	if (dc->dc_chg_mode == SC) {
		...
		if (enable) {
			...
			regmap_update_bits(dc->regmap, PSW_OVP_CFG_REG_3, OVP2_DROPOVP_EN_MSK,
				OVP2_DROPOVP_DISABLE << OVP2_DROPOVP_EN_SHIFT);
			if (dc->ovp3_lvc)
				regmap_update_bits(dc->regmap, PSW_OVP_CFG_REG_1, OVP3_DROPOVP_MIN_EN_MSK,
					0 << OVP3_DROPOVP_MIN_EN_SHIFT);
			else
				regmap_update_bits(dc->regmap, PSW_OVP_CFG_REG_1, OVP2_DROPOVP_MIN_EN_MSK,
					OVP2_DROPOVP_DISABLE << OVP2_DROPOVP_MIN_EN_SHIFT);
		}
...
static int lvc_charge_enable(int enable, void *dev_data)
{
	...
	if (en) {
		dc->dc_chg_mode = LVC;
		if (dc->ovp3_lvc) {
			bat_par_ctrl_set_switch_off(true);
			mdelay(10); /* mdelay 10ms */
			scharger_ovp3_enable(en);
			mdelay(50); /* mdelay 50ms */
		...
		if (dc->ovp3_lvc) {
			scharger_ovp3_enable(en);
			mdelay(50); /* mdelay 50ms */
			bat_par_ctrl_set_switch_off(false);
			mdelay(10); /* mdelay 10ms */
		} else {
			scharger_psw_enable(en);
			scharger_wldc_enable(dc, en);
...
static int ovp2_lvc_charge_enable(int enable, void *dev_data)
{
	struct scharger_dc *dc = dev_data;
	...
	dc->ovp3_lvc = false;
	return lvc_charge_enable(enable, dev_data);
}

static int ovp3_lvc_charge_enable(int enable, void *dev_data)
{
	struct scharger_dc *dc = dev_data;
...
int scharger_lvc_enable(int enable)
{
	...
	return lvc_charge_enable(en, g_scharger_dc);
}
...
static int ovp3_lvc_charge_is_close(void *dev_data)
{
	struct scharger_dc *dc = dev_data;
	u8 ovp3_state = 0;
	int ret = 0;
	...
	if (ret) {
		scharger_err("%s: check ovp3 state fail\n", __func__);
		return -1;
	}

	if (ovp3_state == PSW_ENABLE)
		return 0;
...
static void scharger_dc_vdrop_ovp_delay_work(struct work_struct *work)
{
	...
	if (dc->dc_chg_mode == SC) {
		...
		if (ibat_h + ibat_l >= CHG_CURRENT_1300MA) {
			scharger_inf("%s enable ovp2 or ovp3 drop min!\n", __func__);
			if (dc->ovp3_lvc)
				regmap_update_bits(dc->regmap, PSW_OVP_CFG_REG_1, OVP3_DROPOVP_MIN_EN_MSK,
					1 << OVP3_DROPOVP_MIN_EN_SHIFT);
			else
				regmap_update_bits(dc->regmap, PSW_OVP_CFG_REG_1, OVP2_DROPOVP_MIN_EN_MSK,
					OVP2_DROPOVP_ENABLE << OVP2_DROPOVP_MIN_EN_SHIFT);
		} else {
...
static struct dc_ic_ops lvc_ops = {
	...
	.ic_enable = ovp2_lvc_charge_enable,
...
static struct dc_ic_ops ovp3_lvc_ops = {
	.dev_name = "hi6526",
	.ic_init = lvc_charge_init,
	.ic_exit = dc_charge_exit,
	.ic_enable = ovp3_lvc_charge_enable,
	.ic_discharge = dc_discharge,
	.is_ic_close = ovp3_lvc_charge_is_close,
	.get_ic_id = get_loadswitch_id,
	.config_ic_watchdog = dc_config_watchdog_ms,
	.kick_ic_watchdog = dc_kick_watchdog_timer,
};
...
static struct dc_batinfo_ops low_batt_batinfo_ops = {
	.init = dc_batinfo_init,
	.exit = dc_batinfo_exit,
	.get_bat_btb_voltage = dc_get_low_batt_vbat_mv,
	.get_bat_package_voltage = dc_get_low_batt_vbat_mv,
	.get_vbus_voltage = dc_get_vbus_mv,
	.get_bat_current = dc_get_ibat_ma,
	.get_ic_ibus = dc_get_ibus_ma,
	.get_ic_temp = dc_get_chip_temp,
...
static void scp_ops_register(struct scharger_dc *dc)
{
	...
	if (dc->support_ovp3_lvc) {
		ovp3_lvc_ops.dev_data = (void *)dc;
		low_batt_batinfo_ops.dev_data = (void *)dc;
		(void)dc_ic_ops_register(LVC_MODE, IC_TWO, &ovp3_lvc_ops);
		(void)dc_batinfo_ops_register(IC_TWO, &low_batt_batinfo_ops, SWITCHCAP_SCHARGERV700);
	}
...
{
	if (of_property_read_u32(np, "support_ovp3_lvc", &dc->support_ovp3_lvc) != 0)
		dc->support_ovp3_lvc = 0;

	scharger_inf("prase_dts support_ovp3_lvc=%u\n", dc->support_ovp3_lvc);
}
...
static void get_battery_name(struct scharger_dc *dc)
{
	...
	coul_np = of_find_compatible_node(NULL, NULL, "hisilicon,scharger_coul");
	if (coul_np == NULL) {
