...
static unsigned int g_scharger_debug_regs[] = { 0x362, 0x7A, 0x900, 0x901, 0x902, 
	0x903, 0x904, 0x905, 0x906, 0x908, 0x90B, 0x90C, 0x90F, 0x91B, 0x91C, 0x91E, 0x91F,
	0x252, 0x253, 0x50C, 0x52C, 0x52D, 0x510};
...
static void show_reg_head(char *buf, size_t str_len)
{
	...
	for (i = 0; i < ARRAY_SIZE(g_scharger_debug_regs); i++) {
		len = strlen(buf);
		ret = snprintf_s(buf + len, str_len - len, str_len - len -1, "0x%-3x  ", g_scharger_debug_regs[i]);
		if (ret < 0) {
...
static void show_reg_value_schargerlog(char *buf, int str_len, struct scharger_buck *di)
{
	...
	for (i = 0; i < ARRAY_SIZE(g_scharger_debug_regs); i++) {
		regmap_read(di->regmap, g_scharger_debug_regs[i], &val);
		len = strlen(buf);
