...
int scharger_reset_watchdog_timer(void)
{
	...
	if ((g_wdg->scharger_version == CHIP_ID_V700) ||
		(g_wdg->scharger_version == CHIP_ID_V710)) {
...
int scharger_set_watchdog_timer(int value)
{
	...
	if ((g_wdg->scharger_version == CHIP_ID_V700) ||
		 ...
		 */
		if (value >= WATCHDOG_TIMER_05_S)
			val = 3;
		else if (value >= WATCHDOG_TIMER_02_S)
			val = 2;
		else if (value >= WATCHDOG_TIMER_01_S)
			val = 1;
		else
			val = 0;

		(void)scharger_reset_watchdog_timer();
		ret += regmap_update_bits(g_wdg->regmap, WATCHDOG_CTRL_REG,
			WATCHDOG_TIMER_MSK_V700, val);
	} else {
		if (value == 0) {
			ret = scharger_set_watchdog_enable(0);
			return ret;
		}

		if (value >= WATCHDOG_TIMER_100_S)
			val = 6; /* 6: watchdog timer 100s */
		else if (value >= WATCHDOG_TIMER_80_S)
			val = 5; /* 5: watchdog timer 80s */
		else if (value >= WATCHDOG_TIMER_50_S)
			val = 4; /* 4: watchdog timer 50s */
...
static int scharger_watchdog_probe(struct platform_device *pdev)
{
	...
	wdg->scharger_version = scharger_get_device_version();
