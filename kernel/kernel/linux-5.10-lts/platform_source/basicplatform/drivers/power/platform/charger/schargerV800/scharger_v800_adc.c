...
int scharger_v800_get_sbu1_res(void)
{
	 ...
	 */
	hisi_ldo33_enable(SCHARGER_V800_IC, HKADC_DET, 1);
	...
	hisi_ldo33_enable(SCHARGER_V800_IC, HKADC_DET, 0);
...
static void scharger_v800_disable_sbu_ovp(struct scharger_adc *adc)
{
	regmap_update_bits(adc->regmap, CHG_TCPC_CFG_REG_3_REG,
			 CHG_SBU_OVP_EN_MSK, 1 << CHG_SBU_OVP_EN_SHIFT);
}
...
static int scharger_adc_probe(struct platform_device *pdev)
{
	...
	scharger_v800_disable_sbu_ovp(adc);
