...
int scharger_v800_set_vsys_mos_enable(bool enable);
...
static struct opt_regs buck_common_v810_option_regs[] = {
	/* reg, mask, shift, val */
	reg_cfg(0x3b0, 0xff, 0, 0x01),
	reg_cfg(0x9b6, 0xff, 0, 0x90),
	reg_cfg(0x2035, 0xff, 0, 0x40),
	reg_cfg(0x94e, 0xff, 0, 0x00),
	reg_cfg(0x9f4, 0xff, 0, 0x1a),
	reg_cfg(0x948, 0xff, 0, 0x48),
	reg_cfg(0x966, 0xff, 0, 0x0d),
	reg_cfg(0x95f, 0xff, 0, 0x0e),
	reg_cfg(0x95c, 0xff, 0, 0x9d),
	reg_cfg(0x94e, 0xff, 0, 0x08),
...
static struct opt_regs buck_v810_9v_extra_opt_regs[] = {
	/* reg, mask, shift, val */
	reg_cfg(0x967, 0xff, 0, 0x55),
	reg_cfg(0x378, 0xff, 0, 0x34),
	reg_cfg(0x95D, 0xff, 0, 0x0C),
	reg_cfg(0x377, 0xff, 0, 0x31),
	reg_cfg(0x376, 0xff, 0, 0x31),
	reg_cfg(0x375, 0xff, 0, 0x09),
...
static struct opt_regs v810_otg_5v_option_regs[] = {
	/* reg, mask, shift, val */
	reg_cfg( 0x330, 0xFF, 0, 0x1  ),
	reg_cfg( 0x935, 0xFF, 0, 0x06 ),
	reg_cfg( 0x37D, 0xFF, 0, 0x1  ),
	...
	reg_cfg( 0x37B, 0xFF, 0, 0xB0 ),
	reg_cfg( 0x371, 0xFF, 0, 0x7  ),
	reg_cfg( 0x37F, 0xFF, 0, 0x10 ),
	reg_cfg( 0x974, 0xFF, 0, 0xA  ),
	reg_cfg( 0x96E, 0xFF, 0, 0xBD ),
	reg_cfg( 0x992, 0xFF, 0, 0x07 ),
	reg_cfg( 0X936, 0xFF, 0, 0X10 ),
	reg_cfg( 0x973, 0xFF, 0, 0XB6 ),
...
static struct opt_regs v810_otg_9v_option_regs[] = {
	/* reg, mask, shift, val */
	reg_cfg( 0x330, 0xFF, 0, 0x1  ),
	reg_cfg( 0x935, 0xFF, 0, 0x06 ),
	reg_cfg( 0x37D, 0xFF, 0, 0x1  ),
	...
	reg_cfg( 0x37B, 0xFF, 0, 0xD0 ),
	reg_cfg( 0x371, 0xFF, 0, 0x1  ),
	reg_cfg( 0x37F, 0xFF, 0, 0x14 ),
	reg_cfg( 0x974, 0xFF, 0, 0xB  ),
	reg_cfg( 0x96E, 0xFF, 0, 0xAD ),
	reg_cfg( 0x992, 0xFF, 0, 0x27 ),
	reg_cfg( 0X936, 0xFF, 0, 0X50 ),
	reg_cfg( 0X938, 0xFF, 0, 0x20 ),
...
static struct opt_regs v810_otg_9v_exit_option_regs[] = {
	/* reg, mask, shift, val */
	reg_cfg( 0x935, 0xFF, 0, 0x00 ),
	reg_cfg( 0x37D, 0xFF, 0, 0x04 ),
	reg_cfg( 0x37E, 0xFF, 0, 0x1C ),
...
bool scharger_chip_version_is_v80x(void)
{
	...
	if (buck->scharger_version == CHIP_ID_V800 || buck->scharger_version == CHIP_ID_V801 ||
		buck->scharger_version == CHIP_ID_V802 || buck->scharger_version == CHIP_ID_V803)
		return TRUE;
	return FALSE;
}
...
static void scharger_buck_common_opt_param(struct scharger_buck *buck)
{
	if (scharger_chip_version_is_v80x()) {
		...
		opt_regs_set(buck, buck_common_v810_option_regs,
			ARRAY_SIZE(buck_common_v810_option_regs));
	}
}
...
static void scharger_buck_otg_opt_param(enum charger_mode mode)
{
	...
	if (scharger_chip_version_is_v80x()) {
		...
		case BUCK5V:
			...
			break;
		case BUCK9V:
			opt_regs_set(buck, buck_v810_9v_extra_opt_regs,
					ARRAY_SIZE(buck_v810_9v_extra_opt_regs));
			break;
		case OTG5V:
			opt_regs_set(buck, v810_otg_5v_option_regs,
					ARRAY_SIZE(v810_otg_5v_option_regs));
			break;
		case OTG9V:
			opt_regs_set(buck, v810_otg_9v_option_regs,
					ARRAY_SIZE(v810_otg_9v_option_regs));
			break;
		default:
			scharger_err("%s mode not match!\n", __func__);
			break;
...
static void scharger_buck_otg_exit_opt_param(enum charger_mode mode)
{
	...
	case OTG9V:
		if (scharger_chip_version_is_v80x())
			...
			opt_regs_set(buck, v810_otg_9v_exit_option_regs,
					ARRAY_SIZE(v810_otg_9v_exit_option_regs));
		break;
	default:
		scharger_err("%s mode not match!\n", __func__);
		break;
...
static void scharger_parse_dts(struct device_node *np, struct scharger_buck *buck)
{
	...
	ret = of_property_read_u32(np, "lvc_mos", &(buck->param_dts.lvc_mos));
	if (ret != 0) {
		scharger_err("get lvc_mos failed\n");
		buck->param_dts.lvc_mos = FUNC_UNSUPPORTED;
	}
	scharger_inf("prase_dts lvc_mos=%u\n", buck->param_dts.lvc_mos);

	ret = of_property_read_u32(np, "vsys_mos", &(buck->param_dts.vsys_mos));
	if (ret != 0) {
		scharger_err("get vsys_mos failed\n");
		buck->param_dts.vsys_mos = FUNC_UNSUPPORTED;
	}
	scharger_inf("prase_dts vsys_mos=%u\n", buck->param_dts.vsys_mos);

	ret = of_property_read_u32(np, "cur_balance_mos", &(buck->param_dts.cur_balance_mos));
	if (ret != 0) {
		scharger_err("get cur_balance_mos failed\n");
		buck->param_dts.cur_balance_mos = FUNC_UNSUPPORTED;
	}
	scharger_inf("prase_dts cur_balance_mos=%u\n", buck->param_dts.cur_balance_mos);

	ret = of_property_read_u32(np, "gate1_power_mode", &(buck->param_dts.gate1_power_mode));
	if (ret != 0) {
		scharger_err("get gate1_power_mode failed\n");
		buck->param_dts.gate1_power_mode = POWERED_BY_BAT;
	}
	scharger_inf("prase_dts gate1_power_mode=%u\n", buck->param_dts.gate1_power_mode);

	ret = of_property_read_u32(np, "gate2_power_mode", &(buck->param_dts.gate2_power_mode));
	if (ret != 0) {
		scharger_err("get gate2_power_mode failed\n");
		buck->param_dts.gate2_power_mode = POWERED_BY_BAT;
	}
	scharger_inf("prase_dts gate2_power_mode=%u\n", buck->param_dts.gate2_power_mode);

	ret = of_property_read_u32(np, "i_in_limit_calibration", (u32 *)&(buck->param_dts.i_in_limit_calibration));
	if (ret != 0) {
		scharger_err("get i_in_limit_calibration failed\n");
		buck->param_dts.i_in_limit_calibration = 0;
	}
	scharger_inf("prase_dts i_in_limit_calibration=%d\n",
		buck->param_dts.i_in_limit_calibration);
...
int scharger_v800_set_charge_enable(int enable)
{
	...
	if (enable == CHG_ENABLE) {
		scharger_v800_set_vsys_mos_enable(0);
		...
		scharger_v800_set_vsys_mos_enable(1);
	}
...
int scharger_v800_set_vusb_sleep_mask(int mask)
{
	struct scharger_buck *buck = g_buck;
	...
	return regmap_update_bits(buck->regmap, CHG_SYS_DET_CFG_REG_6_REG,
				 CHG_VUSB_SLEEP_MSK, mask << CHG_VUSB_SLEEP_SHIFT);
}

int scharger_v800_set_vbus_uvp_mv(unsigned int uv_vol)
{
	int ret = 0;
	struct scharger_buck *buck = g_buck;
	unsigned int i;
	unsigned int vbus_uv_sel[VBUS_UV_NUM] = {3800, 7400, 2500, 3000, 5000, 6000, 10000, 12000};

	for(i = 0; i < VBUS_UV_NUM; i++)
		if (uv_vol == vbus_uv_sel[i]) {
			ret = regmap_update_bits(buck->regmap, CHG_REF_TOP_CFG_REG_3_REG, CHG_VBUS_UV_SET_MSK,
				i << CHG_VBUS_UV_SET_SHIFT);
			break;
		}
	if (i == VBUS_UV_NUM) {
		scharger_err("%s uv_vol %u is error !\n", __func__, uv_vol);
		ret = -1;
	}
	return ret;
}
...
static int scharger_v800_set_vbus_uvp(enum charger_mode mode)
{
	...
	ret = scharger_v800_set_vbus_uvp_mv(uv_vol);
	return ret;
}
...
static int scharger_v800_fast_uv_en(struct scharger_buck *buck, int enable)
{
	scharger_inf("%s, enable:%d\n", __func__, enable);

	return regmap_update_bits(buck->regmap, CHG_USB_OVP_CFG_REG_0_REG,
				 CHG_FAST_USB_UV_EN_MSK, !enable << CHG_FAST_USB_UV_EN_SHIFT);
}

static int scharger_v800_set_usb_uv(struct scharger_buck *buck, enum charger_mode mode)
{
	unsigned int reg_val;
	unsigned int usb_uv_sel;

	switch(mode) {
	case BUCK5V:
		/* 0: set usb uv to 3000mV */
		reg_val = 0;
		usb_uv_sel = 3000;
		break;
	case BUCK9V:
		/* 1: set usb uv to 7000mV */
		reg_val = 1;
		usb_uv_sel = 7000;
		break;
	default:
	...
	scharger_inf("%s, usb uv:%u\n", __func__, usb_uv_sel);

	return regmap_update_bits(buck->regmap, CHG_USB_OVP_CFG_REG_1_REG,
				 CHG_USB_UV_SET_MSK, reg_val << CHG_USB_UV_SET_SHIFT);
}
...
static int buck_init_5v(void)
{
	...
	if (scharger_chip_version_is_v80x() == FALSE) {
		ret += scharger_v800_fast_uv_en(buck, 0);
		ret += scharger_v800_set_usb_uv(buck, BUCK5V);
	}
	...
	if (scharger_v800_flash_state() != ENTIRE_IC_STANDBY_MODE)
...
static int buck_init_9v(void)
{
	...
	if (scharger_chip_version_is_v80x() == FALSE) {
		if (charge_get_charger_type() != CHARGER_TYPE_WIRELESS) {
			scharger_v800_fast_uv_en(buck, 1);
			scharger_v800_set_usb_uv(buck, BUCK9V);
		}
	}
...
int __set_input_current(int cin_limit)
{
	...
	if (buck->param_dts.i_in_limit_calibration && (i_in_limit > 1) &&
...
int scharger_get_flash_limit_input_current(void)
{
	...
	if (buck->buck_vbus_set == VBUS_VSET_5V) {
		vbus_coefficient = FLASH_BUCK5V_IACL_COEFFICIENT;
		max = CHG_FLASH_ILIMIT_2200;
	} else {
		vbus_coefficient = FLASH_BUCK9V_IACL_COEFFICIENT;
		max = CHG_FLASH_ILIMIT_1200;
	}

	/* 5V BUCK:I_ACL=I_CHG*0.8, 9V BUCK:I_ACL=I_CHG*0.4 */
	ilimit = (buck->charge_current * vbus_coefficient) / 10;
	if (ilimit > buck->input_current)
		ilimit = buck->input_current;

	if (ilimit > max) {
		scharger_inf("%s ilimit=%d, max %d, vbus set is %u\n",
			      __func__, ilimit, max, buck->buck_vbus_set);
		ilimit = max;
	}
	scharger_inf("%s vbus_vset:%d, charge_current:%d, input_current:%d, ilimit:%d\n", __func__,
		      buck->buck_vbus_set, buck->charge_current, buck->input_current, ilimit);
...
int scharger_flash_restore_input_current(void)
{
	...
	scharger_inf("%s +\n", __func__);
	return __set_input_current(buck->input_current);
}
...
static int scharger_v800_set_input_current(int cin_limit)
{
	...
	if (scharger_v800_flash_state() != ENTIRE_IC_STANDBY_MODE)
		cin_limit = scharger_get_flash_limit_input_current();

	return __set_input_current(cin_limit);
}
...
static int scharger_v800_set_charge_current(int charge_current)
{
	...
	buck->charge_current = charge_current;
	if (scharger_v800_flash_state() != ENTIRE_IC_STANDBY_MODE)
		__set_input_current(scharger_get_flash_limit_input_current());
...
static void scharger_v800_gate1_vdropovp_enable(struct scharger_buck *buck, unsigned int enable)
{
	regmap_update_bits(buck->regmap, CHG_USB_OVP_CFG_REG_1_REG,
		CHG_GATE1_VDROPOVP_SET_MSK, 1 << CHG_GATE1_VDROPOVP_SET_SHIFT);
	regmap_update_bits(buck->regmap, CHG_USB_OVP_CFG_REG_3_REG,
		CHG_OVP_GATE1_VDROPOVP_EN_MSK, enable << CHG_OVP_GATE1_VDROPOVP_EN_SHIFT);
}

static void scharger_v800_gate2_vdropovp_enable(struct scharger_buck *buck, unsigned int enable)
{
	regmap_update_bits(buck->regmap, CHG_PSW_OVP_CFG_REG_1_REG,
		CHG_GATE2_VDROPOVP_SET_MSK, 1 << CHG_GATE2_VDROPOVP_SET_SHIFT);
	regmap_update_bits(buck->regmap, CHG_SYS_LOGIC_CFG_REG_9_REG,
		CHG_OVP_GATE2_VDROPOVP_EN_MSK, enable << CHG_OVP_GATE2_VDROPOVP_EN_SHIFT);
}

static int scharger_v800_gate1_enable(struct scharger_buck *buck, bool enable)
{
	int ret = 0;
...
#ifdef CONFIG_BOOST_5V
	if (buck->param_dts.gate1_power_mode == POWERED_BY_BOOST5V) {
		if (enable == TRUE)
			boost_5v_enable(BOOST_5V_ENABLE, BOOST_CTRL_SCHARGER_GATE1);
		else
			boost_5v_enable(BOOST_5V_DISABLE, BOOST_CTRL_SCHARGER_GATE1);
	}
#endif

	if (enable) {
		hisi_ldo33_enable(SCHARGER_V800_IC, GATE1_CTRL, 1);
		...
		hisi_ldo33_enable(SCHARGER_V800_IC, GATE1_CTRL, 0);
	}

	return ret;
}

static int scharger_v800_get_gate1_status(struct scharger_buck *buck, bool *status)
{
	unsigned int reg_val = 0;
	int ret;

	ret = regmap_read(buck->regmap, CHG_USB_OVP_CFG_REG_3_REG, &reg_val);
	*status = reg_val & CHG_GATE1_EN_MSK;

	return ret;
}

static int scharger_v800_gate2_enable(struct scharger_buck *buck, bool enable)
{
	int ret = 0;
...
#ifdef CONFIG_BOOST_5V
	if (buck->param_dts.gate2_power_mode == POWERED_BY_BOOST5V) {
		if (enable == TRUE)
			boost_5v_enable(BOOST_5V_ENABLE, BOOST_CTRL_SCHARGER_GATE2);
		else
			boost_5v_enable(BOOST_5V_DISABLE, BOOST_CTRL_SCHARGER_GATE2);
	}
#endif

	if (enable) {
		hisi_ldo33_enable(SCHARGER_V800_IC, GATE2_CTRL, 1);
		ret += regmap_update_bits(buck->regmap, CHG_SYS_LOGIC_CFG_REG_9_REG,
			CHG_GATE2_EN_MSK, 1 << CHG_GATE2_EN_SHIFT);
	} else {
		ret += regmap_update_bits(buck->regmap, CHG_SYS_LOGIC_CFG_REG_9_REG,
			CHG_GATE2_EN_MSK, 0 << CHG_GATE2_EN_SHIFT);
		hisi_ldo33_enable(SCHARGER_V800_IC, GATE2_CTRL, 0);
	}

	return ret;
}

static int scharger_v800_get_gate2_status(struct scharger_buck *buck, bool *status)
{
	unsigned int reg_val = 0;
	int ret;

	ret = regmap_read(buck->regmap, CHG_SYS_LOGIC_CFG_REG_9_REG, &reg_val);
	*status = reg_val & CHG_GATE2_EN_MSK;

	return ret;
}

int scharger_v800_lvc_mos_enable(bool enable, void *dev_data)
{
	struct scharger_buck *buck = g_buck;
	int ret = 0;
	...
	if (buck->param_dts.lvc_mos == FUNC_UNSUPPORTED) {
		scharger_err("%s lvc_mos is unsupported!\n", __func__);
		return -1;
	} else if (buck->param_dts.lvc_mos == GATE_1) {
		scharger_v800_gate1_vdropovp_enable(buck, 1);
		ret = scharger_v800_gate1_enable(buck, enable);
	} else if (buck->param_dts.lvc_mos == GATE_2) {
		scharger_v800_gate2_vdropovp_enable(buck, 1);
		ret = scharger_v800_gate2_enable(buck, enable);
	}

	return ret;
}

static int __set_cur_balance_mos_open(unsigned int value)
{
	int ret = 0;
	struct scharger_buck *buck = g_buck;
	...
	if (buck->param_dts.cur_balance_mos == FUNC_UNSUPPORTED) {
		scharger_err("%s cur_balance_mos is unsupported!\n", __func__);
		return -1;
	} else if (buck->param_dts.cur_balance_mos == GATE_1) {
		ret = scharger_v800_gate1_enable(buck, value);
	} else if (buck->param_dts.cur_balance_mos == GATE_2) {
		ret = scharger_v800_gate2_enable(buck, value);
	}

	return ret;
}

static int scharger_get_cur_balance_mos_status(bool *is_enable)
{
	int ret = 0;
	struct scharger_buck *buck = g_buck;
	bool status = FALSE;
	...
	if (buck->param_dts.cur_balance_mos == FUNC_UNSUPPORTED) {
		...
		ret = scharger_v800_get_gate1_status(buck, &status);
	} else if (buck->param_dts.cur_balance_mos == GATE_2) {
		ret = scharger_v800_get_gate2_status(buck, &status);
	}
...
/* This interface can be used only for fatctory tests. */
static int scharger_factory_set_cur_balance_mos_open(bool value)
{
	struct scharger_buck *buck = g_buck;
	...
	if (!(strstr(saved_command_line, "ohos.boot.swtype=factory") ||
		...
		scharger_inf("%s not factory mode\n", __func__);
		return -1;
	}

	buck->factory_cur_balance_mos_flag = value;
	scharger_inf("%s value:%u\n", __func__, value);

	return __set_cur_balance_mos_open(value);
}

int scharger_v800_set_cur_balance_mos_open(unsigned int value)
{
	struct scharger_buck *buck = g_buck;
	...
	if (buck->factory_cur_balance_mos_flag == 1) {
		scharger_err("%s factory_cur_balance_mos_flag is 1\n", __func__);
		return -1;
	}
...
static int __set_vsys_mos_enable(bool enable)
{
	struct scharger_buck *buck = g_buck;
	int ret = 0;
	...
	if (buck->param_dts.vsys_mos == FUNC_UNSUPPORTED) {
		scharger_err("%s vsys_mos is unsupported!\n", __func__);
		return -1;
	} else if (buck->param_dts.vsys_mos == GATE_1) {
		ret = scharger_v800_gate1_enable(buck, enable);
	} else if (buck->param_dts.vsys_mos == GATE_2) {
		ret = scharger_v800_gate2_enable(buck, enable);
	}
...
static int scharger_get_vsys_mos_status(bool *is_enable)
{
	int ret = 0;
	struct scharger_buck *buck = g_buck;
	bool status = FALSE;
...
/* This interface can be used only for fatctory tests. */
static int scharger_factory_set_vsys_mos_enable(bool enable)
{
	struct scharger_buck *buck = g_buck;
	...
	buck->factory_vsys_mos_flag = enable;
	scharger_inf("%s enable:%u\n", __func__, enable);

	return __set_vsys_mos_enable(enable);
}
...
int scharger_v800_set_vsys_mos_enable(bool enable)
{
	...
	if (buck->factory_vsys_mos_flag == 1) {
		scharger_err("%s factory_vsys_mos_flag is 1\n", __func__);
		return -1;
	}
...
static int set_otg_disable(void)
{
	...
	if (charger_mode->chg_mode == OTG9V) {
		ret = set_9v_otg_enable(0);
	} else if (charger_mode->chg_mode == OTG5V) {
		ret = set_5v_otg_enable(0);
	} else {
		scharger_inf("%s current mode is not OTG, chg_mode=%u\n",
...
int scharger_efuse_read(u8 addr, u8 *value)
{
	...
	hisi_ldo33_enable(SCHARGER_V800_IC, EFUSE_WR, 1);
	...
	*value = (u8)val;
	...
	hisi_ldo33_enable(SCHARGER_V800_IC, EFUSE_WR, 0);
	mutex_unlock(&buck->efuse_lock);
...
static struct charge_device_ops scharger_ops = {
	...
	.get_curr_balance_mos_enable = scharger_get_cur_balance_mos_status,
	.set_curr_balance_mos_enable = scharger_factory_set_cur_balance_mos_open,
	.get_vsys_mos_enable = scharger_get_vsys_mos_status,
	.set_vsys_mos_enable = scharger_factory_set_vsys_mos_enable,
};
...
static void scharger_buck_usb_ovp_status_in_shutdown(void)
{
	struct scharger_buck *buck = g_buck;
	unsigned int vusb_ovp = 0;
	...
	regmap_read(buck->regmap, CHG_IRQ_STATUS_0_REG, &vusb_ovp);
	if ((vusb_ovp & CHG_IRQ_ST_VUSB_OVP_MSK) == CHG_IRQ_ST_VUSB_OVP_MSK) {
		scharger_inf("%s, vusb_ovp = 0x%x!\n", __func__, vusb_ovp);
...
static void do_ldo33_enable(unsigned int enable, void *dev_data)
{
	struct scharger_buck *buck = dev_data;
	...
	scharger_inf("%s enable=%u\n", __func__, enable);
	(void)regmap_update_bits(buck->regmap, CHG_SYS_LOGIC_CFG_REG_4_REG, CHG_LDO33_EN_MSK,
...
static struct hisi_ldo33_ops ldo33_ops = {
	.ldo33_enable = do_ldo33_enable,
};

static void scharger_buck_ldo33_register(struct scharger_buck *buck)
{
	unsigned int i;

	memcpy_s(&buck->ldo33_ops, sizeof(struct hisi_ldo33_ops), &ldo33_ops, sizeof(struct hisi_ldo33_ops));
	buck->ldo33_ops.dev_data = (void *)buck;
	buck->ldo33_ops.ldo33_en = 0;
	for (i = 0; i < LDO33_USER_MAX; i++)
		buck->ldo33_ops.ldo33_state[i] = 0;

	hisi_ldo33_register(SCHARGER_V800_IC, &buck->ldo33_ops);
}

static int scharger_v800_connect_notifier_call(struct notifier_block *nb,
	unsigned long event, void *data)
{
	struct scharger_buck *buck = container_of(nb, struct scharger_buck, connect_nb);
...
static int scharger_buck_probe(struct platform_device *pdev)
{
	...
	buck->connect_nb.notifier_call = scharger_v800_connect_notifier_call;
	if (power_event_bnc_register(POWER_BNT_CONNECT, &buck->connect_nb))
		scharger_err("%s notifier_register failed\n", __func__);
	...
	scharger_buck_ldo33_register(buck);

	g_buck = buck;
	scharger_buck_usb_ovp_status_in_shutdown();
	...
	scharger_cur_balance_mos_init(buck);
...
static int scharger_buck_suspend(struct platform_device *pdev, pm_message_t state)
{
	...
	scharger_v800_set_cur_balance_mos_state(buck, STATE_SUSPEND);
	if (vbat < buck->param_dts.vsys_mos_suspend_vbat_thre || cap <  buck->param_dts.vsys_mos_suspend_cap_thre) {
		scharger_v800_set_vsys_mos_enable(1);
		scharger_inf("%s vbat=%d, cap=%d, set vsys mos open\n", __func__, vbat, cap);
	} else {
		scharger_v800_set_vsys_mos_enable(0);
	}
...
static int scharger_buck_resume(struct platform_device *pdev)
{
	...
	scharger_v800_set_cur_balance_mos_state(buck, STATE_DISCHARGING_BELOW_THRE);
	if (scharger_get_charge_enable(buck) == CHG_DISABLE)
		scharger_v800_set_vsys_mos_enable(1);
	else
		scharger_v800_set_vsys_mos_enable(0);
