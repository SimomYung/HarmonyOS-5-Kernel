...
#include "../hisi_ldo33/hisi_ldo33_ctrl.h"
...
struct param {
	...
	unsigned int lvc_mos;
	unsigned int vsys_mos;
	unsigned int cur_balance_mos;
	unsigned int gate1_power_mode;
	unsigned int gate2_power_mode;
	unsigned int vsys_mos_suspend_vbat_thre;
	unsigned int vsys_mos_suspend_cap_thre;
	int i_in_limit_calibration;
};
...
struct scharger_buck {
	...
	bool dc_charging;
	struct hisi_ldo33_ops ldo33_ops;
	unsigned int factory_cur_balance_mos_flag;
	unsigned int factory_vsys_mos_flag;
};
...
extern void reset_dpdm_ovp_gpio(void);
#endif
