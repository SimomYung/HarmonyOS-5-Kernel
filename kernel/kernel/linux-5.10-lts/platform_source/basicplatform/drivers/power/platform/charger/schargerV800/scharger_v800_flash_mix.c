...
static int scharger_v800_flash_get_dt_data(struct hw_flash_ctrl_t *flash_ctrl)
{
	...
	pdata->id_list_count = of_property_count_elems_of_size(of_node, "vendor,led-id", sizeof(u32));
	if (pdata->id_list_count > 0 && pdata->id_list_count <= HWFLASH_POSITION_MAX) {
		scharger_flash_inf("%s:%d vendor,led-id set id_list_count:%d", __func__, __LINE__, pdata->id_list_count);
		ret = of_property_read_u32_array(of_node, "vendor,led-id",
			pdata->id_list, pdata->id_list_count);
		if (ret < 0) {
			scharger_flash_inf("%s get led-id fail %d\n",
				__func__, __LINE__);
			ret = 0;
		}
	}
	...
	struct device_attribute *attr, char *buf)
{
	...
	if (!pdata) {
		scharger_flash_err("%s invalid pdata", __func__);
		return -EINVAL;
	}
 
	if (!buf) {
		scharger_flash_err("%s invalid params", __func__);
		return -EINVAL;
	}
