...
#define REG_FLASH_BASE_ADDR   0x1000
/*
 * REG_FLASH range: 0x00-0xA, 0x40-0x49 */
static const int reg_flash_range[][2] = {{0x00, 0x0A}, {0x40, 0x49}};
...
static void show_reg_head(char *buf, size_t str_len)
{
	...
	num = sizeof(reg_flash_range) / sizeof(reg_flash_range[0]);
	for (i = 0; i < num; i++) {
		start = reg_flash_range[i][0];
		end = reg_flash_range[i][1];
		for (j = start; j <= end; j++) {
			len = strlen(buf);
			ret = snprintf_s(buf + len, str_len - len, str_len - len -1, "0x%x  ",
					 REG_FLASH_BASE_ADDR + j);
			if (ret < 0) {
				scharger_err("(%s)snprintf_s fail ret = %d\n", __func__, ret);
				return;
			}
...
static void show_reg_value_schargerlog(char *buf, int str_len, struct scharger_buck *di)
{
	...
	for (i = 0; i < num; i++) {
		...
		for (j = start; j <= end; j++) {
			regmap_read(di->regmap, REG_FLASH_BASE_ADDR + j, &val);
			len = strlen(buf);
			ret = snprintf_s(buf + len, str_len - len, str_len - len -1, "0x%-4x  ", val);
			if (ret < 0) {
				scharger_err("(%s)snprintf_s fail ret = %d\n", __func__, ret);
				return;
			}
