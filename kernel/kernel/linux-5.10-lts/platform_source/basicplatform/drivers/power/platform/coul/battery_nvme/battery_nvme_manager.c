...
void save_batt_nv_library(int batt_index, const char *batt_sn, struct battery_nvme_info *from_coul_core)
{
	...
	batt_nvme->cleared = 0;
	cycle = (u16)batt_nvme->cycle / BAT_CYCLE_RATIO;
	nvme_write(batt_index, &di->library_nvme);
	set_battery_cycle_by_id(batt_index, cycle);
...
static int batt_nv_cycle_clear(int batt_index)
{
	...
	if (is_battery_without_sn(library->batteries[library->index].sn) == 0) {
		library->batteries[library->index].cycle = 0;
		coul_err("%s library[%d].sn %s cycle clear\n",
...
int quary_batt_nv_library(int batt_index, const char *batt_sn, struct battery_nvme_info *to_coul_core)
{
	...
	// If the cycle in NVMe is 0, set the value in the battery protection board as the new cycle value.
	if (batt_nvme->cycle == 0) {
		ret = get_battery_cycle_by_id(batt_index, &cycle);
		coul_info("%s batt %d batt_nvme->cycle %d, cycle %d, ret %d\n", __func__,
			batt_index, batt_nvme->cycle, cycle, ret);
		if (!ret)
			batt_nvme->cycle = cycle * BAT_CYCLE_RATIO;
	}
