...
#include "coul_dsm.h"
#endif
#define INVALID_AGING_DELTA_PERCENT 20

static int calculate_real_fcc_uah(struct smartstar_coul_device *di,
	int *ret_fcc_uah);
static int coul_get_rm(struct smartstar_coul_device *di);

static ATOMIC_NOTIFIER_HEAD(g_coul_fault_notifier_list_0);
static ATOMIC_NOTIFIER_HEAD(g_coul_fault_notifier_list_1);
static ATOMIC_NOTIFIER_HEAD(g_coul_fault_notifier_list_2);


static int register_coul_fault_notifier(struct notifier_block *nb, int batt)
{
	if (batt == BATT_0)
		return atomic_notifier_chain_register(&g_coul_fault_notifier_list_0, nb);
	else if (batt == BATT_1)
		return atomic_notifier_chain_register(&g_coul_fault_notifier_list_1, nb);
	else
		return atomic_notifier_chain_register(&g_coul_fault_notifier_list_2, nb);
}

int coul_core_notify_fault(int batt, unsigned long val, void *v)
{
	if (batt == BATT_0)
		return atomic_notifier_call_chain(&g_coul_fault_notifier_list_0, val, v);
	else if (batt == BATT_1)
		return atomic_notifier_call_chain(&g_coul_fault_notifier_list_1, val, v);
	else
		return atomic_notifier_call_chain(&g_coul_fault_notifier_list_2, val, v);
}

static int unregister_coul_fault_notifier(struct notifier_block *nb, int batt)
{
	if (batt == BATT_0)
		return atomic_notifier_chain_unregister(&g_coul_fault_notifier_list_0, nb);
	else if (batt == BATT_1)
		return atomic_notifier_chain_unregister(&g_coul_fault_notifier_list_1, nb);
	else
		return atomic_notifier_chain_unregister(&g_coul_fault_notifier_list_2, nb);
}
	...
	int ratio, int i_ma)
{
	...
	for (i = 0; i <= SOC_FULL; i++) {
		ocv_uv = coul_ocv_cal_tbl_ocv(di->batt_index, i * TENTH, OCV_DISCHG,
			di->batt_temp) * PERMILLAGE;
	...
	return coul_ocv_cal_tbl_pc(di->batt_index, unusable_uv, OCV_DISCHG,
			di->batt_temp);
}
...
static int calc_rm_chg_uah(struct smartstar_coul_device *di, int fcc_uah)
{
	...
	pc = coul_ocv_cal_tbl_pc(di->batt_index, di->batt_ocv, di->batt_ocv_type,
		di->batt_ocv_temp);
...
static int get_batt_data_uuc_uah(
	struct smartstar_coul_device *di, int cur_ma)
{
	int uuc;

	uuc = coul_interpolate_cur_temp_uuc(di->batt_data->cur_temp_uuc,
		di->batt_temp, cur_ma);
	if (uuc < 0)
		return 0;
	else
...
static bool check_uuc_smooth_temp(struct smartstar_coul_device *di)
{
	int i;
	int temp = di->batt_temp / TENTH;
	int capcity = di->batt_capacity_1000x / PERMILLAGE;
	for (i = 0; i < UUC_SMOOTH_TEMP_LEVEL; i++) {
		if ((temp > di->uuc_smooth_para_range[i].temp_min) &&
			(temp < di->uuc_smooth_para_range[i].temp_max) &&
				(capcity > di->uuc_smooth_para_range[i].temp_soc))
		return true;
	}
...
static int calculate_unusable_charge_uah_curr(struct smartstar_coul_device *di, int iavg_ma)
{
	int uuc_smooth_volt_max;

	if ((di->basp_zero_ocv <= NORMAL_CUTOFF_VOLT_MIN) || (di->basp_zero_ocv >= NORMAL_CUTOFF_VOLT_MAX))
		uuc_smooth_volt_max = di->uuc_zero_vol + di->uuc_smooth_vol_mv;
	else
		uuc_smooth_volt_max = di->basp_zero_ocv + di->uuc_smooth_vol_mv;

	if (di->uuc_smooth_vol_mv && check_uuc_smooth_temp(di) &&
		(coul_core_get_battery_voltage_mv(di) > uuc_smooth_volt_max) &&
			(di->last_iavg_ma != IMPOSSIBLE_IAVG) && (di->i_cc_avg_ua >= 0))
		return abs(di->last_iavg_ma);
...
{
#ifdef CONFIG_COUL_UUC
	int i;
	int uuc_uah_iavg, uuc1;
	int iavg_ma = iavg_ua / PERMILLAGE;
	int pc_unusable;
	unsigned int basp_zero_uuc, off_find_uuc;
	int uuc_type; /* 0: use calc uuc, 1: use tbl uuc */

	if (di->batt_data->cur_temp_uuc && di->batt_data->cur_temp_uuc->rows > 0) {
		uuc_type = 1;
		iavg_ma = (iavg_ma < 0) ? (fcc_uah / CHARGING_IAVG_MA_RATIO / UA_PER_MA) : iavg_ma;
	} else {
		uuc_type = 0;
	...
	if (uuc_type == 1) {
		iavg_ma = calculate_unusable_charge_uah_curr(di, iavg_ma);
		uuc_uah_iavg = get_batt_data_uuc_uah(di, iavg_ma);
		pc_unusable = uuc_uah_iavg * PERCENT * TENTH / fcc_uah;
	} else {
		pc_unusable = calculate_termination_uuc_pc(di, iavg_ma);
		uuc_uah_iavg = pc_unusable * (fcc_uah / PERMILLAGE);
	}
	di->uuc_iavg_ma = iavg_ma;
	coul_core_info("RBATT_ADJ: batt %d, iavg_ma=%d, UUC= %d uAh, pc= %d.%d, uuc_type=%d (1:tbl)\n",
		di->batt_index, iavg_ma, uuc_uah_iavg, pc_unusable / TENTH, pc_unusable % TENTH, uuc_type);
	...
	/* while discharging */
	if ((iavg_ua >= 0) && (uuc_type == 0))
		uuc_uah_iavg += fcc_uah / PERCENT;
	di->batt_uuc = uuc_uah_iavg;
	coul_core_debug("%s, batt %d, batt_uuc %d\n", __func__,
		di->batt_index, di->batt_uuc);

	off_find_uuc = (di->off_find_flag == OFF_FIND_FLAG_ON) ? di->off_find_uuc_uah : 0;
	basp_zero_uuc = di->basp_zero_pc * fcc_uah / TENTH / PERCENT;

	if (uuc_type == 1) {
		if ((di->ui_soc > SOC_ONE) || (di->charging_state == CHARGING_STATE_CHARGE_START)) {
			uuc1 = (int)(di->emer_uuc_uah) * (UUC_SOC_TH - di->batt_soc_real / TENTH) / UUC_SOC_TH
				+ (int)(di->emer_uuc_uah) / UUC_SOC_TH;
			uuc1 = clamp(uuc1, 0, (int)(di->emer_uuc_uah));
			di->batt_uuc = max(uuc1 + (int)basp_zero_uuc, uuc_uah_iavg);
		} else {
			di->batt_uuc = basp_zero_uuc + di->safety_uuc_uah + off_find_uuc;
		}
	} else {
		di->batt_uuc = uuc_uah_iavg;
	}

	coul_core_info("%s batt %d, uuc_tbl=%d uAh, emer_uuc=%d uAh, off_find_uuc=%d, "
		"basp_zero_uuc=%d uAh, safety_uuc=%d uAh, batt_uuc=%d uAh\n",
		__func__, di->batt_index, uuc_uah_iavg, di->emer_uuc_uah, off_find_uuc,
		basp_zero_uuc, di->safety_uuc_uah, di->batt_uuc);
#endif
	return di->batt_uuc;
}
...
{
	di->qmax_start_pc = coul_ocv_cal_tbl_pc(di->batt_index, di->batt_ocv,
		OCV_DISCHG, di->batt_ocv_temp) / TENTH;
...
static int get_high_pre_qmax(struct smartstar_coul_device *di)
{
	...
	if (di->qmax_refresh_flag) {
		di->qmax_end_pc = coul_ocv_cal_tbl_pc(di->batt_index, di->batt_ocv,
		OCV_DISCHG, di->batt_ocv_temp) / TENTH;
...
void coul_get_ocv_by_vol(struct smartstar_coul_device *di, int ocv_dsm_type)
{
	int voltage_uv, rm;
	int ocv_type;
	...
	ocv_type = coul_ocv_select_type(di->batt_index, voltage_uv, di->batt_soc_real);
	if (ocv_type == OCV_MIX) {
	...
	di->batt_ocv_dsm_type = ocv_dsm_type;
	di->batt_ocv_type = ocv_type;
	di->coul_dev_ops->save_ocv_temp(di->batt_index, (short)di->batt_ocv_temp);
	coul_set_fcc_flag(di, IS_UPDATE_FCC);
	coul_record_ocv_cali_info(di);
	coul_core_clear_cc_register(di);
	coul_core_clear_coul_time(di);

	di->coul_dev_ops->save_ocv_and_type(di->batt_index, voltage_uv, ocv_type);
	coul_core_info("batt %d, awake from deep sleep, new OCV = %d, ocv_type=%d, fcc_flag=%d\n",
		di->batt_index, di->batt_ocv, ocv_type, di->batt_ocv_valid_to_refresh_fcc);
	di->dbg_ocv_cng_0++;
...
static int limit_delta_rc_when_charge_stop(struct smartstar_coul_device *di,
	int delta_rc)
{
	int prev_delta_rc = di->batt_delta_rc;

	if ((di->batt_soc_real / TENTH <= di->charge_stop_limit_drc_soc) &&
		!coul_ocv_is_dischg_state(di->batt_index, di->batt_soc_real) &&
		(delta_rc > prev_delta_rc)) {
		coul_core_err("%s batt %d, delta_rc %d, prev_delta_rc %d, use prev value\n",
			__func__, di->batt_index, delta_rc, prev_delta_rc);
		return prev_delta_rc;
	}
	...
	int rbatt_tbl, int pc_new, int avg_c)
{
	...
	if (di->rbatt_ratio <= 0) {
		...
		pc_new_100 = coul_ocv_cal_tbl_pc(di->batt_index, (ocv - delta_ocv_100) * PERMILLAGE,
			OCV_DISCHG, di->batt_ocv_temp);
	...
	if (di->enable_charge_stop_limit_drc)
		delta_rc_final = limit_delta_rc_when_charge_stop(di, delta_rc_final);
	...
	int rbatt_calc, int ocv, int avg_c)
{
	...
	pc_new = coul_ocv_cal_tbl_pc(di->batt_index, ocv_new * PERMILLAGE,
		OCV_DISCHG, di->batt_temp);
	...
	int rbatt_tbl, int fcc_uah, struct vcdata *vc, int *soc_new)
{
	...
	ocv = coul_ocv_cal_tbl_ocv(di->batt_index, di->batt_soc_real, OCV_MIX, di->batt_temp);
...
#if defined(CONFIG_HUAWEI_OHOS_DSM) || defined(CONFIG_HUAWEI_DSM)
	coul_report_adjust_soc_fast_info(di, soc_est_avg, soc);
#endif
...
static int limit_soc(struct smartstar_coul_device *di, int input_soc)
{
	...
	if (di->hold_full_soc_enable && (di->charging_state == CHARGING_STATE_CHARGE_DONE)) {
...
static short coul_core_get_last_capacity(struct smartstar_coul_device *di)
{
	short capacity = 0;
	short batt_real_soc = (short)(di->batt_soc_real / TENTH);

	coul_hardware_get_last_soc(&capacity);
	capacity = abs(capacity - batt_real_soc) > TENTH ? batt_real_soc : capacity;
	return capacity;
}
	...
	struct soc_param_data *soc_params)
{
	...
	int delta_cc, delta_time;
	u64 time_now;
	int uuc_update_flag = 0;

	time_now = dfx_getcurtime() / NSEC_PER_SEC;
	...
	/* get ui_soc */
	di->ui_soc = bci_show_capacity();
	if (di->ui_soc < SOC_ZERO)
		di->ui_soc = coul_core_get_last_capacity(di);

	coul_core_info("%s batt %d, uiSOC= %d, SOC real = %d\n",
		__func__, di->batt_index, di->ui_soc, di->batt_soc_real);
	...
	if (di->first_in) {
		di->i_cc_avg_ua = di->coul_dev_ops->get_battery_current_ua(di->batt_index);
		di->first_in = 0;
	} else {
		...
		if (delta_time > 0)
			di->i_cc_avg_ua = div_s64((s64)delta_cc * SEC_PER_HOUR, delta_time);
		else
			di->i_cc_avg_ua = di->coul_dev_ops->get_battery_current_ua(di->batt_index);

		coul_core_info("batt %d, delta_time=%d, i_ua=%d\n", di->batt_index, delta_time, di->i_cc_avg_ua);
	}
	di->last_cc = di->cc_end_value;
	di->last_time = di->get_cc_end_time;

	coul_calculate_iavg_ma(di, di->i_cc_avg_ua);

	if (uuc_update_flag)
		soc_params->unusable_charge_uah =
			calculate_unusable_charge_uah(di, soc_params->fcc_uah, di->i_cc_avg_ua);
	else
...
static bool check_ocv_valid(struct smartstar_coul_device *di)
{
	...
	if ((di->batt_ocv < OCV_3900_UV) && !coul_ocv_check_dense_area(di->batt_index, di->batt_ocv))
		return true;
	else
		return false;
}
...
static void coul_check_drained_battery_flag(struct smartstar_coul_device *di)
{
	...
	if (!(strstr(saved_command_line, "ohos.boot.mode=normal") ||
		strstr(saved_command_line, "androidboot.mode=normal")) || 
		!(strstr(saved_command_line, "ohos.boot.swtype=normal") ||
	...
	struct smartstar_coul_device *di, int sleep_cc, int sleep_time)
{
	...
	if (sleep_time <= 0) {
		...
		if (sleep_current < CHG_DONE_SLEEP_CUR_UPPER) {
			...
			di->coul_dev_ops->save_ocv_level(di->batt_index, di->last_ocv_level);
			coul_get_ocv_by_vol(di, CHARGE_DONE_RESUME_OCV);
		}
	}
}
...
void coul_get_basp_zero_ocv(struct smartstar_coul_device *di)
{
	int ret, zero_pc;
	int ocv = 0;

	ret = battery_ocv_get_ocv_by_cap(SOC_ZERO, &ocv);
	if (ret || ocv < OCV_BY_MAP_MIN || ocv > OCV_BY_MAP_MAX || ocv < di->uuc_zero_vol) {
		coul_core_err("%s, get voltage by cap failed, ocv = %d\n", __func__, ocv);
		di->basp_zero_pc = SOC_ZERO;
		return;
	}
	di->basp_zero_ocv = ocv;
	zero_pc = coul_ocv_cal_tbl_pc(di->batt_index, ocv * PERMILLAGE, OCV_DISCHG,
			di->batt_temp);
	di->basp_zero_pc = (zero_pc >= SOC_ZERO) ? zero_pc : SOC_ZERO;
}

static void coul_get_calculate_soe_para(struct smartstar_coul_device *di)
{
	di->soe_info.soe_para.index = di->batt_index;
	di->soe_info.soe_para.soc = di->batt_soc;
	di->soe_info.soe_para.capacity_1000x = di->batt_capacity_1000x;
	di->soe_info.soe_para.ui_soc = di->ui_soc;
	di->soe_info.soe_para.current_ua = di->i_cc_avg_ua;
	di->soe_info.soe_para.fcc_uah = di->batt_fcc;
	di->soe_info.soe_para.uuc_uah = di->batt_uuc;
	di->soe_info.soe_para.rm_uah = di->batt_ruc;
	di->soe_info.soe_para.charging_state = di->charging_state;
}

static void coul_calculate_cap(struct smartstar_coul_device *di)
{
	if (di->soe_enable) {
		coul_get_calculate_soe_para(di);
		di->batt_capacity_1000x = cou_calc_soe_cap(di->batt_data->soe_params, &(di->soe_info), &(di->batt_soe));
	} else {
		di->batt_soe = 0;
		di->batt_capacity_1000x = di->batt_soc * PERMILLAGE;
	}
	coul_core_info("batt %d, Capacity is %d SOC is %d SOE is %d\n",
		di->batt_index, di->batt_capacity_1000x, di->batt_soc, di->batt_soe);
}
...
void calculate_soc_work(void *data)
{
	...
	cal_soc(di);
	coul_calculate_cap(di);
	coul_record_charge_term_voltage_and_current(di);
	...
	if (di->batt_soc <= BATTERY_CC_LOW_LEV) {
		...
		blocking_notifier_call_chain(notifier_list,
			BATTERY_LOW_WARNING, NULL);
	}

#ifdef CONFIG_DIRECT_CHARGER
	if ((direct_charge_in_charging_stage() == DC_IN_CHARGING_STAGE) ||
		(wldc_get_charge_stage() == WLDC_STAGE_CHARGING) ||
		(di->batt_soc < TENTH) || (di->batt_temp / TENTH < TENTH))
		di->coul_dev_ops->coul_drop_enable(di->batt_index, DIS);
	else
		di->coul_dev_ops->coul_drop_enable(di->batt_index, EN);
#endif
#if defined(CONFIG_HUAWEI_OHOS_DSM) || defined(CONFIG_HUAWEI_DSM)
	coul_algo_dsm_check(di);
#endif
	coul_core_info("%s -\n", __func__);
}
...
static void report_coul_para_work(struct work_struct *work)
{
	struct smartstar_coul_device *di =
		container_of(work, struct smartstar_coul_device,
			coul_para_report_delay_work.work);
#if defined(CONFIG_HUAWEI_OHOS_DSM) || defined(CONFIG_HUAWEI_DSM)
	coul_para_report(di);
#endif
	cancel_delayed_work(&di->coul_para_report_delay_work);
}
...
void coul_core_smart_charge_stop(struct smartstar_coul_device *di)
{
	coul_core_info("%s +\n", __func__);
	if (di == NULL) {
	...
	int new_fcc_uah, int delta_fcc_uah, int design_fcc_mah, int fcc_uah)
{
	...
	ret = sprintf_s(buff, (size_t)DSM_BUFF_SIZE_MAX, " [refresh fcc warning] batt %d, old_fcc:%dmAh, "
		"last_fcc_cycle:%u, new_fcc:%dmAh, delta_fcc:%dmAh, charging_beging_soc:%d, "
		"charging_begin_cc:%dmAh, charging_end_cc:%dmAh, temp:%d, vterm=%d, iterm=%d, basplevel:%u",
		di->batt_index, di->fcc_real_mah, di->last_fcc_cycle & LAST_FCC_CYCLE_MASK,
		new_fcc_uah / UA_PER_MA, delta_fcc_uah / UA_PER_MA, di->charging_begin_soc,
		di->charging_begin_cc / UA_PER_MA,
		di->coul_dev_ops->calculate_cc_uah(di->batt_index) / UA_PER_MA,
		di->batt_temp, di->vterm, di->iterm, di->basp_level);
	if (ret < 0)
...
/* last_fcc_cycle_nv high 16bits is aging_cycle, low 16bits is last_fcc_cycle */
static int get_last_fcc_cycle(struct smartstar_coul_device *di)
{
	/* Prevents exceptions caused by single and double frame version switchover */
	if (!di->enable_aging_factor && (di->last_fcc_cycle < LAST_FCC_CYCLE_MASK))
		return (int)di->last_fcc_cycle;

	return (int)(di->last_fcc_cycle & LAST_FCC_CYCLE_MASK);
}

static bool check_aging_delta_percent_valid(struct smartstar_coul_device *di)
{
	return (di->aging_delta_percent > 0) && (di->aging_delta_percent < INVALID_AGING_DELTA_PERCENT);
}

static bool check_saved_cycle_valid(int cur_cycle, int saved_cycle)
{
	return (saved_cycle > 0) && (saved_cycle < cur_cycle);
}
...
static void limit_max_fcc_by_sf_tbl(struct smartstar_coul_device *di)
{
	...
	int last_fcc_cycle = get_last_fcc_cycle(di);
	/* invalid fcc cycle or fcc self study within 10 or 50 cycles, do not follow fcc_sf_tbl */
	if (!check_saved_cycle_valid(cur_cycle, last_fcc_cycle)
			|| (cur_cycle - last_fcc_cycle <= di->invalid_fcc_cycle)) {
		coul_core_info("%s batt%d last_fcc_cycle = %d, cur_cycle = %d, use study fcc %d\n",
			__func__, di->batt_index, last_fcc_cycle, cur_cycle,
			di->batt_fcc / PERMILLAGE);
		return;
	}

	designed_fcc_mah = coul_interpolate_fcc(di, di->batt_temp);
	sf_percent = coul_interpolate_scalingfactor_fcc(di, cur_cycle);
	if (di->enable_aging_factor && (di->aging_flag == 1) && check_aging_delta_percent_valid(di))
		sf_fcc_mah = (designed_fcc_mah * (sf_percent - di->aging_delta_percent)) / PERCENT;
	else
	...
	readjust_fcc_table(di);
	coul_core_info("%s batt%d have not fresh fcc in %d cycles, last_fcc = %d, adjusted_fcc=%d\n",
		__func__, di->batt_index, di->invalid_fcc_cycle, cur_fcc_mah, di->fcc_real_mah);

#if defined(CONFIG_HUAWEI_OHOS_DSM) || defined(CONFIG_HUAWEI_DSM)
	if (di->enable_aging_factor && di->aging_flag == 1)
		coul_report_abnomal_aging_info(di, sf_percent, cur_fcc_mah);
#endif
}

static bool is_normal_aged_battery(struct smartstar_coul_device *di)
{
	return di->aging_cycle == INVALID_AGING_CYCLE;
}
 
static void save_aging_cycle(struct smartstar_coul_device *di)
{
	unsigned int last_fcc_cycle, aging_cycle;
	
	last_fcc_cycle = di->last_fcc_cycle & LAST_FCC_CYCLE_MASK;
	aging_cycle = (unsigned int)di->aging_cycle << SIXTEEN_BIT_SHEFT;
	
	di->last_fcc_cycle = last_fcc_cycle | aging_cycle;
}

static void calc_aging_scalingfactor(struct smartstar_coul_device *di)
{
	int sf_percent, af_percent;

	if (is_normal_aged_battery(di)) {
		save_aging_cycle(di);
		di->aging_delta_percent = 0;
		return;
	}

	if (di->aging_flag == 1 && check_saved_cycle_valid(di->batt_chargecycles / PERCENT, di->aging_cycle)) {
		save_aging_cycle(di);
		af_percent = coul_interpolate_agingfactor_fcc(di, di->aging_cycle);
		sf_percent = coul_interpolate_scalingfactor_fcc(di, di->aging_cycle);
		if (af_percent <= 0 || af_percent >= sf_percent || sf_percent == INVALID_FCC_UAH) {
			di->aging_delta_percent = 0;
		} else {
			di->aging_delta_percent = sf_percent - af_percent;
			di->invalid_fcc_cycle = INVALID_FCC_CYCLE_FOR_AGING;
		}
	}

	coul_core_info("%s batt%d flag=%d aging_cycle=%d delta_pc=%d last_fcc_cycle_nv=0x%x\n", __func__,
		di->batt_index, di->aging_flag, di->aging_cycle, di->aging_delta_percent, di->last_fcc_cycle);
}
...
static void check_refresh_fcc_valid(struct smartstar_coul_device *di)
{
	...
	if (di->enable_aging_factor)
		calc_aging_scalingfactor(di);

	limit_max_fcc_by_sf_tbl(di);
}
/* last_fcc_cycle_nv high 16bits is aging_cycle, low 16bits is last_fcc_cycle */
static void save_last_fcc_cycle(struct smartstar_coul_device *di)
{
	unsigned int cur_cycle = di->batt_chargecycles / PERCENT;

	if (di->enable_aging_factor)
		cur_cycle |= (di->last_fcc_cycle & AGING_CYCLE_MASK);

	di->last_fcc_cycle = cur_cycle;
}
/*
...
static void get_batt_sn(struct smartstar_coul_device *di)
{
	unsigned int  buf_len = BATT_SN_LEN;
	unsigned int  sn_len = 0;
	int ret;

	ret = get_battery_sn_by_id(di->batt_index, di->sn, buf_len, &sn_len);
	if (ret) {
		memcpy_s(di->sn, BATT_SN_LEN, WITHOUT_SN, strlen(WITHOUT_SN) + 1);
		coul_core_err("%s batt index %d, error, ret %d\n", __func__, di->batt_index, ret);
	}
	coul_core_info("%s batt index %d, %s, len %d\n", __func__, di->batt_index, di->sn, sn_len);
}
...
static int coul_work_register(struct smartstar_coul_device *di)
{
	...
	INIT_WORK(&di->coul_drop_work, report_coul_drop_dsm_second_work);
	INIT_DELAYED_WORK(&di->read_temperature_delayed_work,
		read_temperature_work);
	INIT_DELAYED_WORK(&di->coul_para_report_delay_work,
		report_coul_para_work);
	if (di->use_nvme_manager)
		INIT_DELAYED_WORK(&di->nvme_library_work, nvme_library_to_coul_core);
	di->work_init_flag = 1;
...
int coul_core_register(struct coul_device_ops *dev_ops, const struct device_node *cfg)
{
	...
	retval = coul_ocv_register(di->batt_index, di->batt_data, cfg);
	if (retval < 0)
		coul_core_info("use discharge ocv!\n");
	...
	coul_soe_initial(di->batt_index, di->soe_enable, di->batt_data->soe_params, &(di->soe_info));
	get_initial_soc(di);
	...
	queue_delayed_work(system_power_efficient_wq,
		&di->coul_para_report_delay_work,
		round_jiffies_relative(msecs_to_jiffies(REPORTCOUL_PARA_MS + di->batt_index * PERMILLAGE)));
	if (di->use_nvme_manager)
		queue_delayed_work(system_power_efficient_wq, &di->nvme_library_work, 0);
...
coul_no_battery:
	...
	coul_ocv_chg_event_process(di->batt_index, VCHRG_START_CHARGING_EVENT);
	coul_ocv_chg_event_process(di->batt_index, VCHRG_STOP_CHARGING_EVENT);
	coul_calibration_adc(di, 0);
...
void coul_core_shutdown(void *data)
{
	...
	if (di->coul_dev_ops->set_bootocv_sample != NULL) {
		if (coul_ocv_is_dischg_state(di->batt_index, di->batt_soc_real) == TRUE)
