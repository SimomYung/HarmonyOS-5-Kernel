...
static int calculate_unusable_charge_uah_curr(struct smartstar_coul_device *di, int iavg_ma, int iavg_ua)
{
	...
	if (di->uuc_smooth_vol_mv && check_uuc_smooth_temp(di) &&
		(coul_core_get_battery_voltage_mv(di) > uuc_smooth_volt_max) &&
			(di->last_iavg_ma != IMPOSSIBLE_IAVG) && (iavg_ua >= 0))
	...
	struct smartstar_coul_device *di, int fcc_uah, int iavg_ua)
{
	...
	if (uuc_type == 1) {
		iavg_ma = calculate_unusable_charge_uah_curr(di, iavg_ma, iavg_ua);
...
void coul_get_ocv_by_vol(struct smartstar_coul_device *di, int ocv_type)
{
	int voltage_uv, rm;
	...
	di->coul_dev_ops->save_ocv(di->batt_index, voltage_uv);
	...
	struct soc_param_data *soc_params)
{
	...
	if (di->ui_soc < SOC_ZERO)
		di->ui_soc = di->batt_soc_real / TENTH;
...
static bool check_ocv_valid(struct smartstar_coul_device *di)
{
	...
	if ((di->batt_ocv < OCV_3900_UV) && !check_ocv_dense_area(di, di->batt_ocv))
		return true;
	else
		return false;
}
...
void coul_get_basp_zero_ocv(struct smartstar_coul_device *di)
{
	...
	zero_pc = coul_calculate_pc(di, ocv * PERMILLAGE, di->batt_temp,
		di->batt_chargecycles / PERCENT);
...
static int coul_work_register(struct smartstar_coul_device *di)
{
	...
	INIT_DELAYED_WORK(&di->uefi_coul_state_check_delayed_work, uefi_coul_state_check_work); /* worker for V750 UEFI coul DMD report */
	queue_delayed_work(system_power_efficient_wq,
		&di->uefi_coul_state_check_delayed_work,
		msecs_to_jiffies(DMD_UEFI_WAIT));
	di->work_init_flag = 1;
