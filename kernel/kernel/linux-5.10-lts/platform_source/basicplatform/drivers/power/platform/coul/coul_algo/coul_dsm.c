...
static void soc_change_fast_dump(struct smartstar_coul_device *di, char *buf)
{
	...
	ret = snprintf_s(buf, DSM_BUFF_SIZE_MAX, DSM_BUFF_SIZE_MAX - 1,
		"[soc_change_fast]batt%d, cycle = %d, fcc = %d, soc0 = %d, soc_real0 = %d, ocv0 = %d, ocv_lv0 = %d, "
		"uuc0 = %dmAh, drc0 = %dmAh, iavg0 = %dmA, vavg0 = %dmA, temp0 = %d, "
...
static int record_coul_config_para(struct smartstar_coul_device *di, char *buff, int buff_size)
{
	...
	tmp_len = snprintf_s(buff, buff_size, buff_size - 1,
		...
		"enable_basp_zero_v:%d, basp_zero_v:%d, emer_uuc:%d, "
		...
		di->enable_basp_zero_ocv, di->basp_zero_ocv, di->emer_uuc_uah / PERMILLAGE,
		di->off_find_uuc_uah / PERMILLAGE, di->safety_uuc_uah / PERMILLAGE,
...
void coul_para_report(struct smartstar_coul_device *di)
{
	...
	tmp_len = snprintf_s(dsm_buf, DSM_BUFF_SIZE_MAX, DSM_BUFF_SIZE_MAX - 1,
		"[coul_para_report]error_type = %d, batt%d, cyc:%u, v:%d, i:%d, temp:%d, ufsoc:%d, soc:%d, "
		...
		cur / PERMILLAGE, di->batt_temp, di->batt_soc_real, di->batt_soc, last_soc, di->batt_fcc / PERMILLAGE,
...
{
	int abnormal_delta_rc = di->batt_fcc * ABNORMAL_UUC_PERCENT / PERCENT;
...
{
	int abnormal_uuc = di->batt_fcc * ABNORMAL_DRC_PERCENT / PERCENT;
