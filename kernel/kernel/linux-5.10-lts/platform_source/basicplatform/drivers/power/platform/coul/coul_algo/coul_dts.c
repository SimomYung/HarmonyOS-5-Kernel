...
static void get_hold_full_soc_enable(struct smartstar_coul_device *di,
	const struct device_node *np)
{
	int ret;
 
	ret = of_property_read_u32(np, "hold_full_soc_enable", &di->hold_full_soc_enable);
	if (ret) {
		coul_core_err("%s hold_full_soc_enable, default 1\n", __func__);
		di->hold_full_soc_enable = 1;
	} else {
		coul_core_info("%s hold_full_soc_enable %d\n", __func__, di->hold_full_soc_enable);
	}
}
	...
	const struct device_node *np)
{
	...
	get_hold_full_soc_enable(di, np);
}
...
static void get_batt_soe_enable(struct smartstar_coul_device *di,
	const struct device_node *np)
{
	int ret;
 
	ret = of_property_read_u32(np, "soe_enable", &di->soe_enable);
	if (ret) {
		coul_core_err("%s soe_enable not set, default 0\n", __func__);
		di->soe_enable = 0;
	} else {
		coul_core_info("%s soe_enable %d\n", __func__, di->soe_enable);
	}
}
...
static int get_uuc_smooth_paras_range(struct smartstar_coul_device *di, const struct device_node *np)
{
	...
	for (i = 0; i < array_len; i++) {
		...
		coul_core_info("di->uuc_smooth_temp[%d][%d][%d] = %d\n",
			i / (TEMPERATURE_PARA_TOTAL), i % (TEMPERATURE_PARA_TOTAL),
				i % (TEMPERATURE_PARA_TOTAL), idata);
	}
