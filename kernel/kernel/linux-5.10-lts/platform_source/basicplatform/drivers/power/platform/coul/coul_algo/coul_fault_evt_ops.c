...
/* handle uefi coul state check */
void uefi_coul_state_check_work(struct work_struct *work)
{
	struct smartstar_coul_device *di =
		container_of(work, struct smartstar_coul_device, uefi_coul_state_check_delayed_work.work);

	/* if shared memory starts with null char: do nothing, else read memory and report */
	if(strlen(di->boot_info.dmd_data) == 0) {
		coul_core_info("[%s] normal uefi coul state \n", __func__);
	} else {
		coul_core_err("[%s] abnormal uefi coul state, send dsm report. %s \n", __func__, di->boot_info.dmd_data);
		power_dsm_report_dmd(POWER_DSM_BATTERY, DSM_SC0_ERROR, di->boot_info.dmd_data);
	}
#if defined(CONFIG_HUAWEI_OHOS_DSM) || defined(CONFIG_HUAWEI_DSM)
