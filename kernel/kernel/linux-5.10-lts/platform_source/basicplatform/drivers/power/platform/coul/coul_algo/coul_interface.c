...
static int coul_get_discharge_cc(void *data)
{
	int discharge_cc_uah, cc_uah;
	int charging_state;
	...
	charging_state = di->charging_state;

	if(charging_state == CHARGING_STATE_CHARGE_START) {
		return 0;
	}

	cc_uah = di->coul_dev_ops->calculate_cc_uah(di->batt_index);
	discharge_cc_uah = cc_uah + di->discharge_cc_base_uah;

	coul_core_info("[%s] batt_index: %d, discharge_cc:%d, cc: %d \n", __func__,
		di->batt_index, discharge_cc_uah, cc_uah);

	return discharge_cc_uah;
}
...
static int coul_get_battery_aging_cycle(void *data)
{
	struct smartstar_coul_device *di = (struct smartstar_coul_device *)data;
	...
	return (int)(di->last_fcc_cycle >> SIXTEEN_BIT_SHEFT);
}

static int coul_set_battery_aging_flag(void *data, int aging_flag)
{
	struct smartstar_coul_device *di = (struct smartstar_coul_device *)data;
	...
	di->aging_flag = aging_flag;

	return 0;
}

static int coul_set_battery_aging_cycle(void *data, int aging_cycle)
{
	int cur_cycle;
	struct smartstar_coul_device *di = (struct smartstar_coul_device *)data;
	...
	cur_cycle = di->batt_chargecycles / PERCENT;
	if (aging_cycle < 0 || (aging_cycle > cur_cycle && aging_cycle != INVALID_AGING_CYCLE))
		return -EPERM;

	di->aging_cycle = aging_cycle;
	return 0;
}

static int coul_set_battery_aging_factor(void *data, int *aging_factor, size_t af_size)
{
	int ret, i;
	struct single_row_lut lut = { 0 };
	struct smartstar_coul_device *di = (struct smartstar_coul_device *)data;

	if (!di || !di->batt_data|| !di->batt_data->fcc_sf_lut)
		return -EPERM;

	if (!aging_factor || di->batt_data->fcc_sf_lut->cols != af_size)
		return -EPERM;

	for (i = 0; i < af_size; i++) {
		lut.x[i] = di->batt_data->fcc_sf_lut->x[i];
		lut.y[i] = aging_factor[i];
	}
	lut.cols = af_size;
	if (!check_aged_fcc_sf_lut_valid(&lut))
		return -EPERM;

	ret = memcpy_s(&di->aged_fcc_sf_lut, sizeof(struct single_row_lut),
		&lut, sizeof(struct single_row_lut));
	if (ret != EOK) {
	...
	unsigned int event)
{
	...
	case VCHRG_NOT_CHARGING_EVENT:
		di_lock();
		coul_core_smart_charge_stop(di);
...
static int coul_battery_charger_event_rcv(void *data, unsigned int evt)
{
	...
	coul_ocv_chg_event_process(di->batt_index, evt);
	return 0;
}
...
static int coul_get_ocv_by_soc(void *data, int temp, int soc)
{
	...
	return coul_ocv_cal_tbl_ocv(di->batt_index, soc, OCV_DISCHG, temp * TENTH);
}
...
static int coul_get_soc_by_ocv(void *data, int temp, int ocv)
{
	...
	return coul_ocv_cal_tbl_pc(di->batt_index, ocv * PERMILLAGE, OCV_DISCHG, temp);
}
...
int coul_get_basp_nondc_vol_dec(void *data)
{
...
int coul_core_get_basp_zero_ocv(void *data)
{
	struct smartstar_coul_device *di = data;
	...
	return di->basp_zero_ocv;
}


s64 coul_core_get_iscd_full_update_cc(void *data)
{
	struct smartstar_coul_device *di = data;
...
int coul_core_get_battery_uuc_avg_cur(void *data)
{
	struct smartstar_coul_device *di = data;
...
static int coul_set_off_find(void *data, int flag)
{
	struct smartstar_coul_device *di = (struct smartstar_coul_device *)data;
	...
	di->off_find_flag = flag;

	return 0;
}

static int coul_get_off_find(void *data)
{
	struct smartstar_coul_device *di = (struct smartstar_coul_device *)data;
...
static struct coul_merge_drv_ops g_coulometer_ops = {
	...
	.battery_discharge_cc = coul_get_discharge_cc,
	...
	.battery_soc = coul_core_get_battery_soc,
	.battery_soe = coul_core_get_battery_soe,
	...
	.battery_uuc_avg_cur = coul_core_get_battery_uuc_avg_cur,
	...
	.iscd_full_update_cc = coul_core_get_iscd_full_update_cc,
	...
	.get_nondc_vol_dec = coul_get_basp_nondc_vol_dec,
	.get_basp_zero_ocv = coul_core_get_basp_zero_ocv,
	...
	.set_off_find = coul_set_off_find,
	.get_off_find = coul_get_off_find,
	.get_battery_aging_cycle = coul_get_battery_aging_cycle,
	.set_battery_aging_flag = coul_set_battery_aging_flag,
	.set_battery_aging_cycle = coul_set_battery_aging_cycle,
	.set_battery_aging_factor = coul_set_battery_aging_factor,
	.battery_ocv_interval = coul_core_get_ocv_interval,
};
