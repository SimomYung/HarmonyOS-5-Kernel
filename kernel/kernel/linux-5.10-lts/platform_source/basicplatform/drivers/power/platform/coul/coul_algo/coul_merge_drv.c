...
int coul_merge_drv_get_basp_nondc_volt_dec(int batt_index)
{
	struct coulometer_drv *coul_merge_drv = NULL;
	struct coul_merge_drv_ops *ops = NULL;
	...
	/* execute the operation of coul module */
	if (ops && ops->get_nondc_vol_dec)
		return ops->get_nondc_vol_dec(coul_merge_drv->data);

	return 0;
}

int coul_merge_drv_get_basp_zero_ocv(int batt_index)
{
	struct coulometer_drv *coul_merge_drv = NULL;
	struct coul_merge_drv_ops *ops = NULL;
	...
	/* execute the operation of coul module */
	if (ops && ops->get_basp_zero_ocv)
		return ops->get_basp_zero_ocv(coul_merge_drv->data);
...
int coul_merge_drv_get_discharge_cc(int batt_index)
{
	struct coulometer_drv *coul_merge_drv = NULL;
	struct coul_merge_drv_ops *ops = NULL;
	...
	if (ops && ops->battery_discharge_cc)
		return ops->battery_discharge_cc(coul_merge_drv->data);
...
int coul_merge_drv_battery_soc(int batt_index)
{
	struct coulometer_drv *coul_merge_drv = NULL;
	struct coul_merge_drv_ops *ops = NULL;
	...
	if (ops && ops->battery_soc)
		return ops->battery_soc(coul_merge_drv->data);

	return -EPERM;
}

int coul_merge_drv_battery_soe(int batt_index)
{
	struct coulometer_drv *coul_merge_drv = NULL;
	struct coul_merge_drv_ops *ops = NULL;
	...
	if (ops && ops->battery_soe)
		return ops->battery_soe(coul_merge_drv->data);
...
int coul_merge_drv_get_battery_aging_cycle(int batt_index)
{
	struct coulometer_drv *coul_merge_drv = NULL;
	struct coul_merge_drv_ops *ops = NULL;
	...
	if (ops && ops->get_battery_aging_cycle)
		return ops->get_battery_aging_cycle(coul_merge_drv->data);

	return -EPERM;
}

int coul_merge_drv_set_battery_aging_flag(int aging_flag, int batt_index)
{
	struct coulometer_drv *coul_merge_drv = NULL;
	struct coul_merge_drv_ops *ops = NULL;
	...
	if (ops && ops->set_battery_aging_flag)
		return ops->set_battery_aging_flag(coul_merge_drv->data, aging_flag);

	return -EPERM;
}

int coul_merge_drv_set_battery_aging_cycle(int aging_cycle, int batt_index)
{
	struct coulometer_drv *coul_merge_drv = NULL;
	struct coul_merge_drv_ops *ops = NULL;
	...
	if (ops && ops->set_battery_aging_cycle)
		return ops->set_battery_aging_cycle(coul_merge_drv->data, aging_cycle);

	return -EPERM;
}

int coul_merge_drv_set_battery_aging_factor(int *aging_factor, int batt_index, size_t af_size)
{
	struct coulometer_drv *coul_merge_drv = NULL;
	struct coul_merge_drv_ops *ops = NULL;
	...
	if (ops && ops->set_battery_aging_factor)
		return ops->set_battery_aging_factor(coul_merge_drv->data, aging_factor, af_size);
...
s64 coul_merge_drv_iscd_full_update_cc(int batt_index)
{
	struct coulometer_drv *coul_merge_drv = NULL;
	struct coul_merge_drv_ops *ops = NULL;
	...
	if (ops && ops->iscd_full_update_cc)
		return ops->iscd_full_update_cc(coul_merge_drv->data);
...
static bool coul_be_replaced(int batt_index)
{
	struct coul_merge_drv_ops *coul_ops = g_coul_merge_drv_list[batt_index].g_coul_ops;

	if (coul_ops)
		return ((g_coul_merge_drv_list[batt_index].init_flag == 1) &&
			coul_ops->is_replaced && coul_ops->is_replaced());
	return false;
}
	...
	void *data, int batt_index, const char *batt_name)
{
	...
	if (g_coul_merge_drv_list[batt_index].init_flag != 1)
		g_batt_cnt++;

	if((g_coul_merge_drv_list[batt_index].init_flag != 1) ||
		coul_be_replaced(batt_index)) {
...
int coul_merge_drv_battery_uuc_average_current(int batt_index)
{
	struct coulometer_drv *coul_merge_drv = NULL;
	struct coul_merge_drv_ops *ops = NULL;
	...
	if (ops && ops->battery_uuc_avg_cur)
		return ops->battery_uuc_avg_cur(coul_merge_drv->data);
...
int coul_merge_drv_set_off_find(int batt, int flag)
{
	struct coulometer_drv *coul_merge_drv = NULL;
	struct coul_merge_drv_ops *ops = NULL;
	...
	if (ops && ops->set_off_find)
		return ops->set_off_find(coul_merge_drv->data, flag);

	return -1;
}

int coul_merge_drv_get_off_find(int batt)
{
	struct coulometer_drv *coul_merge_drv = NULL;
	struct coul_merge_drv_ops *ops = NULL;
	...
	if (ops && ops->set_off_find)
		return ops->get_off_find(coul_merge_drv->data);

	return -1;
}

int coul_merge_drv_battery_ocv_interval(int batt_index)
{
	struct coulometer_drv *coul_merge_drv = NULL;
	struct coul_merge_drv_ops *ops = NULL;
	...
	if (ops && ops->battery_ocv_interval)
		return ops->battery_ocv_interval(coul_merge_drv->data);
