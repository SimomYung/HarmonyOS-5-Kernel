...
void coul_record_ocv_cali_info(struct smartstar_coul_device *di)
{
	...
	if ((di == NULL) || (di->batt_ocv_dsm_type == POWER_ON_SAVED_OCV))
		return;

	coul_core_info("[%s], batt_ocv_dsm_type=%d\n", __func__, di->batt_ocv_dsm_type);
	/* clean buff */
	...
	di->g_coul_ocv_cali_info[di->g_ocv_cali_temp_index].cali_ocv_type = di->batt_ocv_dsm_type;
	...
	int err_num, const char *buff)
{
	...
	for (i = 0; i < INDEX_MAX; i++) {
		...
		tmp_len += sprintf_s(dsm_buf + tmp_len,
			DSM_BUFF_SIZE_MAX - tmp_len, "ocv_type:%d\n",
			di->g_coul_ocv_cali_info[i].cali_ocv_type);
	}
...
void coul_core_clear_cc_register(struct smartstar_coul_device *di)
{
	...
	di->discharge_cc_base_uah += cc_temp;
...
static int get_calc_ocv(struct smartstar_coul_device *di)
{
	...
	soc_rbatt = coul_ocv_cal_tbl_pc(di->batt_index, vbatt_uv, OCV_DISCHG, batt_temp);
...
void coul_core_get_initial_ocv(struct smartstar_coul_device *di)
{
	...
	ocvreg = (unsigned short)di->coul_dev_ops->get_ocvreg(di->batt_index);
	...
	if (ocvreg == (unsigned short)FLAG_USE_CLAC_OCV) {
		...
		di->coul_dev_ops->save_ocv_and_type(di->batt_index, ocv_uv, OCV_DISCHG);
		...
		di->batt_ocv_dsm_type = POWER_ON_CALC_OCV;
		di->batt_ocv_type = OCV_DISCHG;
		...
		di->batt_ocv_dsm_type = POWER_ON_SAVED_OCV;
		di->batt_ocv_type = di->coul_dev_ops->get_ocv_type(di->batt_index);
		...
		ocv_uv = di->coul_dev_ops->convert_ocv_regval2uv(di->batt_index, ocvreg);
		di->is_nv_need_save = 0;
		di->batt_ocv_dsm_type = POWER_ON_PMU_OCV;
		di->batt_ocv_type = di->coul_dev_ops->get_ocv_type(di->batt_index);
...
#endif
	coul_core_info("initial OCV = %d, OCV_temp=%d, OCV_type=%d, fcc_flag= %d, ocv_level: %d\n",
		di->batt_ocv, di->batt_ocv_temp, di->batt_ocv_type,
...
void coul_get_ocv_by_fcc(struct smartstar_coul_device *di)
{
	...
	new_ocv = coul_ocv_cal_tbl_ocv(di->batt_index,
		PC_UPPER_LIMIT, batt_ocv_type, di->batt_temp);
	if (new_ocv < 0) {
		coul_core_err("coul_interpolate_ocv err\n");
		return;
	} else if (new_ocv > (CAPACITY_INVALID_AREA_5000 / UVOLT_PER_MVOLT)) {
		new_ocv = CAPACITY_INVALID_AREA_5000;
	} else {
		new_ocv = new_ocv * UVOLT_PER_MVOLT;
	}

	if ((new_ocv - di->batt_ocv) > 0) {
		di->batt_ocv_type = batt_ocv_type;
		di->dbg_ocv_cng_1++;
		coul_core_info("full charged, and OCV change, "
			"new_ocv = %d, old_ocv = %d, new_ocv_type=%d\n",
			new_ocv, di->batt_ocv, di->batt_ocv_type);
		di->batt_ocv = new_ocv;
		di->batt_ocv_temp = di->batt_temp;
		di->batt_ocv_dsm_type = CHARGE_DONE_OCV;
		di->coul_dev_ops->save_ocv_temp(di->batt_index, (short)di->batt_ocv_temp);
		coul_set_fcc_flag(di, IS_UPDATE_FCC);
		di->coul_dev_ops->save_ocv_and_type(di->batt_index, new_ocv, di->batt_ocv_type);
...
int coul_get_ocv_vol_from_fifo(struct smartstar_coul_device *di)
{
	...
	for (i = 0; i < di->coul_dev_ops->get_fifo_depth(); i++) {
		...
		if ((voltage_uv >= CAPACITY_INVALID_AREA_5000) ||
...
static int save_multi_ocv_and_level(struct smartstar_coul_device *di)
{
	...
	di->coul_dev_ops->save_ocv_temp(di->batt_index, (short)di->batt_ocv_temp);
	ocv_type = coul_ocv_select_type(di->batt_index, di->batt_ocv, di->batt_soc_real);
	if (di->g_ocv_level_para[di->last_ocv_level].allow_fcc_update) {
		coul_set_fcc_flag(di, IS_UPDATE_FCC);
		di->coul_dev_ops->save_ocv_and_type(di->batt_index, di->batt_ocv, ocv_type);
	} else {
		coul_set_fcc_flag(di, NOT_UPDATE_FCC);
		di->coul_dev_ops->save_ocv_and_type(di->batt_index, di->batt_ocv, ocv_type);
	}
	...
	coul_core_clear_cc_register(di);
	coul_core_clear_coul_time(di);
	cc = di->g_eco_leak_uah;
	di->coul_dev_ops->save_cc_uah(di->batt_index, cc);
	di->coul_dev_ops->save_ocv_level(di->batt_index, di->last_ocv_level);
	coul_batt_notify(di, BATT_OCV_UPDATE);
	coul_core_info("awake from deep sleep, batt %d, new OCV = %d, ocv_type= %d, ocv level = %u, fcc_flag = %d\n",
		       di->batt_index, di->batt_ocv, ocv_type, di->last_ocv_level,
...
static int is_ocv_reach_renew_threshold(
	...
	int time_now, int batt_temp, int ocv_type)
{
	int pc_now;
	u8 last_ocv_level;
	...
	if (current_ocv_level <= last_ocv_level)
		return 1;
	pc_now = coul_ocv_cal_tbl_pc(di->batt_index, ocv_now, ocv_type, batt_temp);
	...
	int time_now, int sleep_time, int wake_time, int sleep_current)
{
	...
	int ocv_now, big_current_10min, temp, duty_ratio, ocv_type;
	...
	ocv_type = coul_ocv_select_type(di->batt_index, ocv_now, di->batt_soc_real);
	if ((cur_level < LOW_PRECISION_OCV_LEVEL) ||
			((cur_level < OCV_LEVEL_MAX) && is_ocv_reach_renew_threshold(
				di, cur_level, ocv_now, time_now, temp, ocv_type))) {
		...
		di->batt_ocv_dsm_type = NORMAL_SR_OCV;
...
static void get_ocv_resume(struct smartstar_coul_device *di)
{
	...
	di->g_eco_leak_uah = di->coul_dev_ops->calculate_eco_leak_uah(di->batt_index);
	coul_get_ocv_by_vol(di, ABNORMAL_SR_OCV);
...
{
	int old_ocv, ocv_type;

	if ((di == NULL) || (di->coul_dev_ops == NULL))
		return;
	coul_core_info("sr_resume_time=%d, charging_stop_time=%d\n", di->sr_resume_time, di->charging_stop_time);
