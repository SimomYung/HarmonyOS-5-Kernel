...
#define ABNORMAL_BATT_TEMPERATURE_HIGH  120
...
#define SCALING_FACTOR_LOWER_LIMIT      50
...
int coul_core_is_battery_exist(void *data)
{
	...
	if ((temp <= ABNORMAL_BATT_TEMPERATURE_LOW) ||
		...
		di->batt_exist = 0;
		coul_core_err("[%s] abnormal battery temperature, temp: %d\n", __func__, temp);
	} else {
		di->batt_exist = 1;
	}
...
/* 
	the only capacity interface to coul_merge,
	return soe or soc, and the caller is not aware of it.
 */
int coul_core_get_battery_capacity(void *data)
{
	...
	return DIV_ROUND_CLOSEST(di->batt_capacity_1000x, PERMILLAGE);
}

int coul_core_get_battery_soc(void *data)
{
	struct smartstar_coul_device *di = data;
...
int coul_core_get_battery_soe(void *data)
{
	struct smartstar_coul_device *di = data;
...
void coul_report_abnomal_aging_info(struct smartstar_coul_device *di, int sf_percent, int last_fcc)
{
	int ret;
	...
	ret = snprintf_s(dsm_buf, DSM_BUFF_SIZE_MAX, DSM_BUFF_SIZE_MAX - 1,
		"[abnomal_aging_study_info] batt%d, curr_cycle = %d, aging_cycle = %d, "
		"nomal_sf_percent = %d, aging_delta_pc = %d, last_fcc = %d, curr_fcc = %d",
		di->batt_index, di->batt_chargecycles / PERCENT, di->last_fcc_cycle >> SIXTEEN_BIT_SHEFT,
		sf_percent, di->aging_delta_percent, last_fcc, di->fcc_real_mah);
	if (ret < 0) {
		coul_core_err("%s snprintf_s failed\n", __func__);
	} else {
#if defined(CONFIG_HUAWEI_OHOS_DSM) || defined(CONFIG_HUAWEI_DSM)
		power_dsm_report_dmd(POWER_DSM_BATTERY, ERROR_BATT_AGING_ABNOMAL, dsm_buf);
#endif
	}
}
...
void coul_record_charge_term_voltage_and_current(struct smartstar_coul_device *di)
{
	int vbat_uv = 0;
	int ibat_ua = 0;
	...
	if (di->charging_state == CHARGING_STATE_CHARGE_START) {
		coul_core_get_battery_voltage_and_current(di, &ibat_ua, &vbat_uv);
		if (vbat_uv > 0 && ibat_ua < CHARGING_CURRENT_OFFSET) {
			di->vterm = vbat_uv / PERMILLAGE;
			di->iterm = ibat_ua / PERMILLAGE;
		}
	}
}
...
int coul_linear_interpolate(int y0, int x0, int y1, int x1, int x)
{
	...
	return y0 + (int)((s64)(y1 - y0) * (s64)(x - x0) / (s64)(x1 - x0));
}
...
int check_aged_fcc_sf_lut_valid(struct single_row_lut *lut)
{
	int i;

	if (!lut || (lut->y[0] != DEFAULT_SCALING_FACTOR) || (lut->y[lut->cols - 1] < SCALING_FACTOR_LOWER_LIMIT))
		return 0;

	for (i = 0; i < lut->cols - 1; i++) {
		if (lut->y[i] < lut->y[i + 1])
			return 0;
	}
...
int coul_interpolate_agingfactor_fcc(struct smartstar_coul_device *di, int cycle)
{
	struct single_row_lut *lut = NULL;

	if (!di)
		return -1;

	lut = &di->aged_fcc_sf_lut;
	if (!check_aged_fcc_sf_lut_valid(lut))
		return coul_interpolate_single_lut(di->batt_data->fcc_sf_lut, cycle);

	return coul_interpolate_single_lut(lut, cycle);
}
...
int coul_calculate_fcc_uah(struct smartstar_coul_device *di)
{
	...
	if (di->adjusted_fcc_temp_lut == NULL) {
		...
		result = PERMILLAGE * coul_interpolate_fcc_adjusted(di, di->batt_temp);
	}
	if (result <= 0) {
		coul_core_info("[%s] invalid fcc=%d\n", __func__, result);
		return -1;
	}
	return result;
}
...
/**
 * coul_interpolate_cur_temp_uuc - 2d interpolate uuc by uuc-table from batt data
 * @cur_temp_uuc: include 2D current-temp-uuc_mah, temp, currents_ma from batt_data
 * @temp: temperature in degree * 10
 * @cur_ma: batt current ma
 * Returns: uuc_uah
 */
int coul_interpolate_cur_temp_uuc(struct cur_temp_uuc *cur_temp_uuc, int temp, int cur_ma)
{
	int temp0 = 0;
	int cur0 = 0;
	int i, temp1, cur1;
	...
	int *curs = NULL;
	int *temps = NULL;
	int temps_len = TEMP_SAMPLING_POINTS;

	if (cur_temp_uuc == NULL) {
		coul_core_err("[%s] NULL point\n", __func__);
		return 0;
	}

	temp1 = temps_len;
	cur1 = cur_temp_uuc->rows;
	curs = (int*)cur_temp_uuc->cur;
	temps = (int*)cur_temp_uuc->temp;

	cur_ma = clamp(cur_ma, curs[0], curs[cur1 - 1]);
	temp = clamp(temp, temps[0] * TENTH, temps[temp1 - 1] * TENTH);

	for (i = 0; i < cur_temp_uuc->rows; i++) {
		if (cur_ma < curs[i]) {
			cur1 = i;
			break;
		}
	...
	if (cur1 == 0) {
		cur0 = 0;
		cur1 = 1;
	} else if (cur0 == cur_temp_uuc->rows - 1) {
		cur0 = cur_temp_uuc->rows - 2;
		cur1 = cur_temp_uuc->rows - 1;
	}

	for (i = 0; i < temps_len; i++) {
		if (temp < temps[i] * TENTH) {
			temp1 = i;
			break;
		}
		temp0 = i;
	...
	if (temp1 == 0) {
		temp0 = 0;
		temp1 = 1;
	} else if (temp0 == temps_len - 1) {
		temp0 = temps_len - 2;
		temp1 = temps_len - 1;
	}

	uuc0 = coul_linear_interpolate(cur_temp_uuc->uuc[cur0][temp0] * PERMILLAGE, curs[cur0],
		cur_temp_uuc->uuc[cur1][temp0] * PERMILLAGE, curs[cur1], cur_ma);

	uuc1 = coul_linear_interpolate(cur_temp_uuc->uuc[cur0][temp1] * PERMILLAGE, curs[cur0],
		cur_temp_uuc->uuc[cur1][temp1] * PERMILLAGE, curs[cur1], cur_ma);

	return coul_linear_interpolate(uuc0, temps[temp0] * TENTH,
		uuc1, temps[temp1] * TENTH, temp);
}
...
int coul_basp_full_pc_by_voltage(struct smartstar_coul_device *di)
{
	...
	if (delta_ocv) {
		batt_fcc_ocv = coul_ocv_cal_tbl_ocv(di->batt_index, 
			TENTH * SOC_FULL, OCV_DISCHG, di->batt_temp);
		pc = coul_ocv_cal_tbl_pc(di->batt_index, (batt_fcc_ocv - delta_ocv) * PERMILLAGE,
...
void coul_force_ocv_update(struct smartstar_coul_device *di)
{
	...
	coul_get_ocv_by_vol(di, FORCED_OCV);
	...
	enum batt_coul_event event)
{
	...
	case BATT_OCV_UPDATE:
		...
		data.pc = coul_ocv_cal_tbl_pc(di->batt_index, di->batt_ocv,
			di->batt_ocv_type, di->batt_temp) * PERCENT;
		...
		break;
	case BATT_COUL_DROP:
		data2.batt_index = di->batt_index;
		data2.volt = coul_core_get_battery_voltage_mv(di);
		data2.cur = coul_core_get_battery_current_ma(di);
		data2.temp = di->batt_temp / TENTH;
		data2.soc = di->batt_soc;
		data2.fcc = di->batt_fcc / PERMILLAGE;
		for (j = 0; j < V_DROP_LEN; j++)
			data2.vol_mv_drop[j] =
				di->coul_dev_ops->get_battery_vol_uv_drop_from_fifo(di->batt_index, j) / PERMILLAGE;
		ret = call_coul_blocking_notifiers(BATT_COUL_DROP, (void *)&data2);
		if ((ret != NOTIFY_DONE) && (ret != NOTIFY_OK))
			coul_core_err("vbat drop notify failed, ret %d\n", ret);
		break;
	default:
		break;
	}
...
static int coul_dev_ops_check_fail_part_two(struct smartstar_coul_device *di)
{
	...
	check_coul_dev_ops(di, get_battery_vol_uv_drop_from_fifo);
	check_coul_dev_ops(di, get_coul_drop_th);
	check_coul_dev_ops(di, coul_drop_enable);
	return 0;
}
...
static int coul_dev_ops_check_fail(struct smartstar_coul_device *di)
{
	...
	check_coul_dev_ops(di, get_ocvreg);
	...
	check_coul_dev_ops(di, save_fcc_invalid_up_flag);
...
void coul_core_set_low_vol_int(struct smartstar_coul_device *di, int state)
{
	...
	coul_core_info("%s low_vol=%d, delt=%d\n", __func__, vol, delt);
...
void coul_clear_moved_battery_data(struct smartstar_coul_device *di, const struct device_node *cfg)
{
	...
	if (di->use_nvme_manager) {
		coul_core_info("use_nvme_manager\n");
		return;
	}
...
void coul_set_fcc_flag(struct smartstar_coul_device *di, int fcc_flag)
{
	switch(fcc_flag) {
	case IS_UPDATE_FCC:
		di->coul_dev_ops->save_fcc_invalid_up_flag(di->batt_index, IS_UPDATE_FCC);
		di->batt_ocv_valid_to_refresh_fcc = 1;
		break;
	case NOT_UPDATE_FCC:
		di->coul_dev_ops->save_fcc_invalid_up_flag(di->batt_index, NOT_UPDATE_FCC);
...
int coul_core_get_ocv_interval(void *data)
{
	struct smartstar_coul_device *di = data;
	...
	return di->get_cc_end_time / SEC_PER_MIN;
}