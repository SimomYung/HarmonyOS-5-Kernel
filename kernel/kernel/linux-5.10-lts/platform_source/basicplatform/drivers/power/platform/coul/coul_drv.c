...
static BLOCKING_NOTIFIER_HEAD(batt_mode_notifier_list);

int batt_mode_register_notifier(struct notifier_block *nb)
{
	return blocking_notifier_chain_register(&batt_mode_notifier_list, nb);
}

int batt_mode_unregister_notifier(struct notifier_block *nb)
{
	return blocking_notifier_chain_unregister(&batt_mode_notifier_list, nb);
}

int send_batt_mode_change_event(unsigned long events)
{
	return blocking_notifier_call_chain(&batt_mode_notifier_list, events, NULL);
}
...
int coul_drv_battery_discharge_cc(void)
{
	struct coulometer_ops *ops = g_coul_ops;

	if (ops && ops->battery_discharge_cc)
		return ops->battery_discharge_cc();
...
/* 0: disable 1: enable */
int coul_drv_battery_set_off_find(int flag)
{
	/* declare the local variable of struct coulometer_ops  */
	struct coulometer_ops *ops = g_coul_ops;
	...
	if (ops && ops->set_off_find)
		return ops->set_off_find(flag);

	return 0;
}

/* 0: disable 1: enable */
int coul_drv_battery_get_off_find(void)
{
	/* declare the local variable of struct coulometer_ops  */
	struct coulometer_ops *ops = g_coul_ops;

	/* execute the operation of coul module */
	if (ops && ops->get_off_find)
		return ops->get_off_find();
