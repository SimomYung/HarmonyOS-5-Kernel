...
static struct coul_data_cache data_cache[CACHE_TOTAL];
...
static void coul_cache_mutex_init(void)
{
	int i;
	for (i = 0; i < CACHE_TOTAL; ++i)
...
static void coul_save_cc_uah(int batt, int cc_uah)
{
	...
	coul_regs_read(COUL_HARDWARE_CHG_TIMER_BASE, &cl_in_time, 4); /* 4: regs_num */
	coul_regs_read(COUL_HARDWARE_LOAD_TIMER_BASE, &cl_out_time, 4); /* 4: regs_num */
	cc_uah -= ((int)cl_in_time * c_offset_b / SECONDS_PER_HOUR)
		+ ((int)cl_out_time * c_offset_b / SECONDS_PER_HOUR);
	...
	data_cache[CC_CACHE].last_time = 0; /* reforce cc cache refresh */
	coul_hardware_inf("%s cl_in_time=0x%x, cl_out_time=0x%x, cc_saved=%d\n",
		__func__, cl_in_time, cl_out_time, cc_uah);
}

#ifdef CONFIG_DFX_DEBUG_FS
void pmic_save_cc_mah(int batt_index, int cc_mah)
{
 ...
 */
void coul_save_ocv(int batt, int ocv, unsigned int ocv_type)
{
...
static int coul_calculate_cc_uah_cache(int batt_index)
{
	...
	mutex_lock(&data_cache[CC_CACHE].lock);
	current_time = dfx_getcurtime() / NSEC_PER_MSEC;
	if (current_time - data_cache[CC_CACHE].last_time > COUL_REFRESH_TIME_MS) {
		data_cache[CC_CACHE].data = coul_calculate_cc_uah(batt_index);
		data_cache[CC_CACHE].last_time = current_time;
	}

	mutex_unlock(&data_cache[CC_CACHE].lock);
	return data_cache[CC_CACHE].data;
}
...
static int coul_hardware_get_battery_voltage_uv_cache(int batt_index)
{
	...
	mutex_lock(&data_cache[VOLT_CACHE].lock);
	current_time = dfx_getcurtime() / NSEC_PER_MSEC;
	if (current_time - data_cache[VOLT_CACHE].last_time > COUL_REFRESH_TIME_MS) {
		data_cache[VOLT_CACHE].data = coul_hardware_get_battery_voltage_uv(batt_index);
		data_cache[VOLT_CACHE].last_time = current_time;
	}
	mutex_unlock(&data_cache[VOLT_CACHE].lock);

	return data_cache[VOLT_CACHE].data;
}
...
static int coul_get_battery_current_ua_cache(int batt_index)
{
	...
	mutex_lock(&data_cache[CURRENT_CACHE].lock);
	current_time = dfx_getcurtime() / NSEC_PER_MSEC;
	if (current_time - data_cache[CURRENT_CACHE].last_time > COUL_REFRESH_TIME_MS) {
		data_cache[CURRENT_CACHE].data = coul_get_battery_current_ua(batt_index);
		data_cache[CURRENT_CACHE].last_time = current_time;
	}
	mutex_unlock(&data_cache[CURRENT_CACHE].lock);

	return data_cache[CURRENT_CACHE].data;
}
