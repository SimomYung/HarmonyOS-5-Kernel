...
static struct coul_data_cache data_cache[BATT_MAX * CACHE_TOTAL];

#define BATT_H_NODE   "batt_h"
#define BATT_L_NODE   "batt_l"

static void coul_cc_mutex_init(int max_count)
{
	int i;
	for (i = 0; i < max_count; ++i)
		mutex_init(&data_cache[i].lock);
}
...
static void coul_save_cc_uah(int batt_index, int cc_uah)
{
	...
	cc_uah -= ((int)cl_in_time_ro * c_offset_b / SECONDS_PER_HOUR)
		+ ((int)cl_out_time_ro * c_offset_b / SECONDS_PER_HOUR);
	...
	coul_cc_write_pro(LOCK);
	data_cache[batt_index * CACHE_TOTAL + CC_CACHE].last_time = 0; /* reforce cc cache refresh */
	coul_hardware_inf("%s batt_index=%d, cl_in_time=0x%x, cl_out_time=0x%x, cc_saved=%d\n",
		__func__, batt_index, cl_in_time_ro, cl_out_time_ro, cc_uah);
}

#ifdef CONFIG_DFX_DEBUG_FS
void scharger_save_cc_mah(int batt_index, int cc_mah)
{
	coul_save_cc_uah(batt_index, (cc_mah - NEGATIVE_CC_OFFSET) * PERMILLAGE);
}
#endif
 ...
 */
static void coul_save_ocv(int batt, int ocv, unsigned int ocv_type)
{
	unsigned short ocvreg;

	ocvreg = coul_convert_ocv_uv2regval(batt, ocv);
	coul_save_ocvreg(batt, ocvreg);
	coul_save_ocvtype(batt, ocv_type);
...
static int coul_hardware_get_battery_voltage_uv(int batt_index)
{
	...
	if (log_cnt >= BASE_INFO_PRINT_INTER) {
...
static int coul_get_tbat(int batt_index)
{
	...
	coul_hardware_inf("%s batt_index %d, tbat %d\n", __func__, batt_index, temp);
	return temp;
}

static int coul_calculate_cc_uah_cache(int batt_index)
{
	u64 current_time;
	...
	mutex_lock(&data_cache[batt_index * CACHE_TOTAL + CC_CACHE].lock);
	current_time = dfx_getcurtime() / NSEC_PER_MSEC;
	if (current_time - data_cache[batt_index * CACHE_TOTAL + CC_CACHE].last_time > COUL_REFRESH_TIME_MS) {
		data_cache[batt_index * CACHE_TOTAL + CC_CACHE].data = coul_calculate_cc_uah(batt_index);
		data_cache[batt_index * CACHE_TOTAL + CC_CACHE].last_time = current_time;
	}
	mutex_unlock(&data_cache[batt_index * CACHE_TOTAL + CC_CACHE].lock);

	return data_cache[batt_index * CACHE_TOTAL + CC_CACHE].data;
}

static int coul_hardware_get_battery_voltage_uv_cache(int batt_index)
{
	u64 current_time = 0;
	...
	mutex_lock(&data_cache[batt_index * CACHE_TOTAL + VOLT_CACHE].lock);
	current_time = dfx_getcurtime() / NSEC_PER_MSEC;
	if (current_time - data_cache[batt_index * CACHE_TOTAL + VOLT_CACHE].last_time > COUL_REFRESH_TIME_MS) {
		data_cache[batt_index * CACHE_TOTAL + VOLT_CACHE].data =
			coul_hardware_get_battery_voltage_uv(batt_index);
		data_cache[batt_index * CACHE_TOTAL + VOLT_CACHE].last_time = current_time;
	}
	mutex_unlock(&data_cache[batt_index * CACHE_TOTAL + VOLT_CACHE].lock);

	return data_cache[batt_index * CACHE_TOTAL + VOLT_CACHE].data;
}

static int coul_get_battery_current_ua_cache(int batt_index)
{
	u64 current_time = 0;
	...
	mutex_lock(&data_cache[batt_index * CACHE_TOTAL + CURRENT_CACHE].lock);
	current_time = dfx_getcurtime() / NSEC_PER_MSEC;
	if (current_time - data_cache[batt_index * CACHE_TOTAL + CURRENT_CACHE].last_time > COUL_REFRESH_TIME_MS) {
		data_cache[batt_index * CACHE_TOTAL + CURRENT_CACHE].data = coul_get_battery_current_ua(batt_index);
		data_cache[batt_index * CACHE_TOTAL + CURRENT_CACHE].last_time;
	}
	mutex_unlock(&data_cache[batt_index * CACHE_TOTAL + CURRENT_CACHE].lock);

	return data_cache[batt_index * CACHE_TOTAL + CURRENT_CACHE].data;
}
...
struct coul_device_ops scharger_coul_ops = {
	...
	.calculate_cc_uah = coul_calculate_cc_uah_cache,
	...
	.save_ocv_and_type = coul_save_ocv,
	...
	.get_battery_voltage_uv = coul_hardware_get_battery_voltage_uv_cache,
	.get_battery_current_ua = coul_get_battery_current_ua_cache,
	...
	.get_ocv_type = coul_get_ocv_type,
};
...
static int coul_hardware_probe(struct platform_device *pdev)
{
	...
	coul_cc_mutex_init(BATT_MAX * CACHE_TOTAL);
