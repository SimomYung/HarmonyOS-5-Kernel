...
static struct mutex g_tab_lock;
...
static int series_charge_handle(struct tab_health_monitor_device *di)
{
	...
	int resistance0 = hisi_eis_get_resistance(BATT_0);
	int resistance1 = hisi_eis_get_resistance(BATT_1);
	...
	struct notifier_block *nb, unsigned long event, void *data)
{
	...
	case CHARGE_DONE:
		...
		break;
	default:
		tab_monitor_info("[%s] Not charging event, start time = 0 \n", __func__);
		di->charge_done_data.start_time = 0;
		break;
	}
