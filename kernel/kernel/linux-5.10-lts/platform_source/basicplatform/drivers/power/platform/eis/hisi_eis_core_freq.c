...
static int eis_drv_freq_ops_check(struct hisi_eis_device *di)
{
	...
	check_eis_freq_ops(di, get_freq_num);
	if (di->eacr_support)
	...
	struct hisi_eis_device *di, int idx, unsigned int n, long long prd_ms, int periods)
{
	...
	di->eis_ops->eis_enable(true, false);
	...
	struct eis_freq_device *di, int freq, int idx)
{
	...
	if (eis_di->sample_mod == FIFO_MOD) {
		for (p = 0; p < FREQ_SAMP_PRDS_10; p++) {
			...
			eis_di->eis_ops->eis_enable(true, false);
			...
			if (ret == _STOP) {
				eis_di->eis_ops->eis_enable(false, true);
				return _STOP;
			}

			eis_di->eis_ops->eis_enable(false, false);
...
static int hisi_eis_high_freq_start_sampling_func(
	struct eis_freq_device *di, int idx)
{
	...
	eis_di = container_of(di, struct hisi_eis_device, eis_freq_dev);
	if (!eis_di || !eis_di->eis_freq_dev.freq_ops->eis_sampling_init)
		return _ERROR;
...
static void hisi_eis_set_cur_threshold(struct hisi_eis_device *di, enum eis_mode mode)
{
	...
	if (wait_eis_working_stop(di, tv, WAIT_EIS_END) == _ERROR) {
		di->eis_ops->eis_enable(false, true);
		hisi_eis_err("wait working stop time out!\n");
		return;
	}
	...
	hisi_eis_debug("eis_detect_flag = %d!\n", eis_detect_flag);
	di->eis_ops->eis_enable(false, false);
...
static int hisi_eis_high_freq_sampling_work_exe(struct eis_freq_device *di)
{
	int err_cnt = 0;
	int idx = 0;
	...
	di->high_freq_info.batt_cycle = bat_cycle_now;
	ret = hisi_eis_high_freq_start_sampling_func(di, idx);
	if (ret == _STOP)
		return ret;
	...
	hisi_eis_debug("%dth sample_prds = %d!\n", idx,
		di->freq_sample_info[idx].sample_prds);
	ret = check_and_adjust_v_i_gain_for_freq(di, idx);
	if (ret == V_I_GAIN_ADJUSTED) {
		gain_err_freq[err_cnt] = (int)t_freq[idx];
		err_cnt++;
	}
	...
	if (err_cnt > 0) {
		...
		err_idx = di->freq_ops->locate_in_ary(
			t_freq, LEN_T_FREQ, gain_err_freq[idx]);
		di->freq_ops->set_i_gain(di->freq_sample_info[err_idx].i_gain);
		di->freq_ops->set_v_gain(di->freq_sample_info[err_idx].v_gain);
		ret = hisi_eis_high_freq_start_sampling_func(di, err_idx);
		if (ret == _STOP)
			return ret;
		hisi_eis_err("%dth, freq = %d resample ended!\n",
			idx, gain_err_freq[idx]);
	}
...
static int hisi_eis_freq_sampling_work_exe(struct eis_freq_device *di)
{
	...
	for (i = 0; i < di->freq_ops->get_freq_num(); i++) {
...
static bool hisi_eis_cal_high_freq_intermediate_data_check(
	struct eis_freq_device *di, int idx)
{
	...
	if (!data_judge) {
		di->high_freq_info.batt_acr = CAL_ERR;
...
static void hisi_eis_cal_high_freq_avg_val(struct eis_freq_device *di, int idx)
{
	...
	di->high_freq_info.batt_acr = get_averaged_freq_eis_real(
		di->freq_sample_info[idx].raw_eis_r, FREQ_SAMP_PRDS_10);
	hisi_eis_inf("freq = 1000/%d, eis_real = %lld!\n",
		 t_freq[idx], di->high_freq_info.batt_acr);
	di->high_freq_info.imag = get_averaged_freq_eis_imag(
		di->freq_sample_info[idx].raw_eis_i, FREQ_SAMP_PRDS_10);
	hisi_eis_inf("freq = 1000/%d, eis_imag = %lld!\n",
		 t_freq[idx], di->high_freq_info.imag);
}
...
static void hisi_eis_calculate_high_freq_func(struct eis_freq_device *di, int idx)
{
	unsigned int i;
	int v_gain;
	int ret;
	...
	if (hisi_eis_cal_high_freq_intermediate_data_check(di, idx) == false) {
		di->high_freq_info.imag = CAL_ERR;
...
static int eis_check_high_freq_valid(struct hisi_eis_device *eis_di, struct eis_freq_device *di)
{
	int sampling_valid_ret;
	int fail_cnt = 0;
	int mod_valid_ret = _SUCC;
	int idx = 0;

	hisi_eis_sampling_data_print(EIS_FREQ, eis_di, idx);
	sampling_valid_ret = hisi_eis_is_freq_sampling_valid(t_freq[idx], di);
	if (sampling_valid_ret == _SUCC) {
		hisi_eis_calculate_high_freq_func(di, idx);
		if (di->avg_freq_check)
			mod_valid_ret = hisi_eis_is_freq_mod_valid(di, idx);
	}
	if ((sampling_valid_ret == _SUCC) && (mod_valid_ret == _SUCC)) {
		hisi_eis_cal_high_freq_avg_val(di, idx);
	} else {
...
void eis_high_freq_cal_start(void)
{
	int ret;
	errno_t err;
	struct eis_freq_infos infos;
	const char *name;
	struct hisi_eis_device *eis_di = g_eis_di[0];
	if (!eis_di) {
		hisi_eis_err("hisi_eis_device is null\n");
		return;
	}
	struct eis_freq_device *di = &(eis_di->eis_freq_dev);
	if (!di) {
		hisi_eis_err("eis_freq_device is null\n");
		return;
	}
	hisi_eis_inf("eis_high_freq_test start!");
	...
	if (hisi_eis_cur_temp_soc_check(eis_di) == false) {
		hisi_eis_err("freq sampling condition not satisfied!\n");
		goto EXIT_CAL;
	}
	ret = hisi_eis_high_freq_sampling_work_exe(di);
	if (ret != _SUCC) {
		hisi_eis_err("freq sampling execution failed!\n");
		goto EXIT_CAL;
	}
	if (eis_check_high_freq_valid(eis_di, di) == _ERROR)
		goto EXIT_CAL;

	infos.batt_index = eis_di->batt_index;
	infos.num_of_info = 1;
	err = memcpy_s(infos.freq_infos, sizeof(struct eis_high_freq_info) * infos.num_of_info,
		&di->high_freq_info, sizeof(di->high_freq_info));
	if (err != EOK)
		hisi_eis_err("eis freq memcpy_s failed!\n");
	hisi_eis_inf("the result of batt_acr =  %llu !\n", di->high_freq_info.batt_acr);
EXIT_CAL:
	if (eis_di->psy)
		power_supply_put(eis_di->psy);
	eis_wake_unlock(eis_di);
...
int eis_get_high_freq_resistance(struct eis_high_freq_info *r)
{
	struct hisi_eis_device *di = g_eis_di[0];
	errno_t err;

	if (!r) {
		hisi_eis_err("%s pointer null!\n", __func__);
		return -1;
	}

	if (!di) {
		hisi_eis_err("%s g_eis_high_di null!\n", __func__);
		return -1;
	}

	err = memcpy_s(r, sizeof(*r),
		&di->eis_freq_dev.high_freq_info,
		sizeof(di->eis_freq_dev.high_freq_info));
	if (err != EOK) {
		hisi_eis_err("[%s]memcpy_s fail, err=%d\n", __func__, err);
		return -1;
	}

	hisi_eis_inf("[%s]eis = %d, cycle = %d, vol = %d,"
		"batt_temp = %d, imag = %d!\n",
		__func__, r->batt_acr, r->batt_cycle, r->batt_vol, r->batt_temp, r->imag);
	return 0;
}
