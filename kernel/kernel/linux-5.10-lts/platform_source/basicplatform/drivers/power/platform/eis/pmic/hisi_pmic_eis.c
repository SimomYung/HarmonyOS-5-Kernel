...
struct eis_pmic_device *g_eis_pmic_di;
...
_STATIC void eis_pmic_freq_set_curt_sample_thres(int batt, int cur_offset)
{
	...
	if (g_eis_pmic_di && g_eis_pmic_di->cur_thres_err > 0) {
		get_calibrated_ana_cur(g_eis_pmic_di->cur_thres_err, &high_err);
		get_calibrated_ana_cur(g_eis_pmic_di->cur_thres_err, &low_err);
	} else {
		if (r_uohm == R_OHM_1M) {
...
_STATIC void eis_pmic_eis_enable(bool enable, bool is_interrupt)
{
	u8 reg_val;
	unused(is_interrupt);
...
static int eis_pmic_get_pure_vol_digit2ana_data(
	const int *raw_sample_vol_data, int len_raw,
...
static int eis_pmic_get_pure_cur_digit2ana_data(
	...
	int v_gain, const int *raw_sample_vol_data, unsigned int len)
{
	...
	(void)eis_pmic_get_pure_vol_digit_data(
...
static int eis_pmic_get_freq_num(void)
{
	return LEN_FREQ;
}
	...
	int *v_gain, int periods, int (*raw_sample_vol_data)[V_BAT_FIFO_DEPTH])
{
	...
	for (i = 0; i < periods; i++) {
		ret = eis_pmic_get_pure_vol_digit2ana_data(
			&raw_sample_vol_data[i][0], V_BAT_FIFO_DEPTH,
			pure_vol_ana, V_BAT_FIFO_DEPTH);
		if(ret){
			eis_err("Error in function 'eis_pmic_get_pure_vol_digit2ana_data'\n");
			return -1;
		}
	...
	int *i_gain, int periods, int (*raw_sample_cur_data)[I_BAT_FIFO_DEPTH])
{
	...
	for (i = 0; i < periods; i++) {
		ret = eis_pmic_get_pure_cur_digit2ana_data(
			&raw_sample_cur_data[i][0], I_BAT_FIFO_DEPTH,
			pure_cur_ana, I_BAT_FIFO_DEPTH);
		if(ret){
			eis_err("Error in function 'eis_pmic_get_pure_cur_digit2ana_data'\n");
			return -1;
		}
...
static void parse_pmic_freq_dts(struct eis_pmic_device *di)
{
	...
	ret = of_property_read_u32(np, "cur_thres_err",
		(u32 *)&di->cur_thres_err);
	if (ret)
		di->cur_thres_err = -1;
	eis_inf("cur_thres_err = %d\n", di->cur_thres_err);
}
...
static struct eis_freq_device_ops hisi_pmic_freq_ops = {
	...
	.get_freq_num = eis_pmic_get_freq_num,
};
