...
static void eis_scharger_eis_enable(bool enable, bool is_interrupt)
{
	u8 reg_val;
	int flag;
	...
	if (enable == false && is_interrupt == true) {
		eis_reg_write(EIS_SOFT_RST_CTRL, SOFT_RST_ON);
		mdelay(RST_DELAY_MS);
		eis_reg_write(EIS_SOFT_RST_CTRL, SOFT_RST_OFF);
	}
