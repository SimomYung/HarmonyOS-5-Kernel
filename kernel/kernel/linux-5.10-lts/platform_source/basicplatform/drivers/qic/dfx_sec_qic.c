...
#ifdef CONFIG_DFX_SEC_QIC_V300_CHIP_ERRLOG_MODIFY
static int dfx_sec_qic_ddr_handler(int irq)
{
	struct arm_smccc_res res = {0};

	arm_smccc_smc((u64)FID_QIC_TBD_SEC_INT_HANDLER, irq, 0, 0, 0, 0, 0, 0, &res);
	pr_err("[%s] get mtype %d and reason %u from smc result\n", __func__, (int)res.a0, (unsigned int)res.a1);
...
static irqreturn_t dfx_sec_qic_irq_handler(int irq, void *data)
{
...
#ifdef CONFIG_DFX_SEC_QIC_V300_CHIP_ERRLOG_MODIFY
	int master_type __maybe_unused = -1;

	master_type = dfx_sec_qic_ddr_handler(irq);
	pr_err("[%s] get master_type %d from dfx_sec_qic_ddr_handler\n", __func__, master_type);
	/* tbd err, not qic err, should be handled above and return */
	if (master_type != 0) {
		if (master_type == DMSS_MASTER_TYPE_MODEM)
			rdr_system_error(RDR_MODEM_DMSS_MOD_ID, 0, 0);
		else if (master_type > DMSS_MASTER_TYPE_MODEM)
	...
	dfx_sec_qic_reset_handler(qic_dev);
#else
