...
#include "maligpu/drivers/gpu/arm/valhall-r42p0-01eac0/platform/hisilicon/mali_kbase_platform.h"
#endif
...
static int regulator_enable_host(struct regulator_ip *sreg, uint64_t cmd)
{
	...
	void *name_buffer = kmalloc(NAME_MAX_SIZE, GFP_KERNEL);
	if (name_buffer == NULL) {
...
static int ip_regulator_cmd_send(struct regulator_dev *dev, int cmd)
{
	...
	if (cmd == REGULATOR_ENABLE) {
		...
		sreg->lpm_enable_value[1] = kbase_sh_runtime_check();
#endif
