 ...
 *
 * Copyright (c) 2024-2040 Huawei Technologies Co., Ltd.
 *
 ...
 * bit[0]: AP           media1 vote, unused
 * bit[1]: Sensorhub    media1 vote
 * bit[2]: AP           vivobus vote, unused
...
/* sctrl 0x4CC : power state vote
 * bit[0]:		AP tcu vote
 * bit[1]:		Sensorhub tcu vote
 * bit[2]:		AP isp vote
 * bit[3]:		Sensorhub isp vote
 * bit[4]:		AP ipp tbu vote
 * bit[5]:		Sensorhub ipp tbu vote
 */
#define AP_ISP_VOTE		2
#define IOMCU_ISP_VOTE	3
#define ispsubsys_vote_state(val) (((val) & BIT(IOMCU_ISP_VOTE)) == 0)
...
static int aod_check_bus_pwstat_vote(struct regulator_ip *sreg)
{
	...
	case DSSSUBSYS_ID:
		...
		break;
	case ISPSUBSYS_ID:
		vote_val = ispsubsys_vote_state(val);
		ret = vote_state(vote_val);
		break;
	case IPP_ID:
		vote_val = ispsubsys_vote_state(val);
	...
	u32 control_flag)
{
	...
	if (sreg->regulator_id == DSSSUBSYS_ID)
		bit_shift = AP_DSS_VOTE;
	else if (sreg->regulator_id == ISPSUBSYS_ID)
		bit_shift = AP_ISP_VOTE;
	else if (sreg->regulator_id == IPP_ID)
		bit_shift = AP_ISP_VOTE;
	else
		return -1;

	/* himask register needs to set mask, otherwise, need to read value */
	if (sreg->sc_himask_flag)
		val = BIT_MSK(bit_shift);
	else
		val = readl(sreg->sctrl_reg + sreg->aod_sc_offset);

	if (control_flag == REGULATOR_ENABLE) /* vote AP media/vivobus/dss/isp power up */
		val |= (BIT(AP_AOD_EN) << bit_shift);
	else /* vote AP media/vivobus/dss/isp power off */
		val &= ~(BIT(AP_AOD_EN) << bit_shift);
...
static int get_softreset_state(struct regulator_ip_core *pmic, struct regulator_ip *sreg)
{
	int ret;
	unsigned int val;
	...
	if (sreg->hwlock_seq > 0) {
		if (sreg->hwlock == NULL) {
			pr_err("[%s]regulator hwlock_%d hwspinlock is null!\n",
				__func__, sreg->hwlock_seq);
			return -ENOENT;
		}

		if (hwspin_lock_timeout(sreg->hwlock,
...
static int ip_power_set(struct regulator_ip *sreg, u32 control_flag)
{
	...
	if (sreg->hwlock_seq > 0) {
		...
		if (hwspin_lock_timeout(sreg->hwlock,
			REGULATOR_HWLOCK_TIMEOUT)) {
			pr_err("IP regulator-%d enable hwspinlock timout!\n", sreg->regulator_id);
			return -ENOENT;
		}
	...
	if (ret) {
		pr_err("%s: ldo[%s] control-%u failled, ret-%d!\n",
			__func__, sreg->name, control_flag, ret);
		ret = -1;
	}
	return ret;
}
...
static int dt_parse_aod_atf(struct regulator_ip *sreg,
	struct device_node *np)
{
	u32 aod_resource[3] = {0}; /* aod hwlock resource and SC offset */
	...
	sreg->sc_himask_flag = aod_resource[2];

	if (sreg->hwlock_seq > 0) {
		sreg->hwlock = hwspin_lock_request_specific(sreg->hwlock_seq);
		if (sreg->hwlock == NULL) {
			pr_err("IP regulator-%d request hwspin lock failed!\n", sreg->regulator_id);
			return -ENODEV;
		}
	}
	return 0;
...
static void regulator_dump_dt_parse(struct regulator_ip *sreg,
	struct device_node *np)
{
	int ret;

	ret = of_property_read_u32(np, "dump-reg-num", &sreg->dump_reg_num);
	if (ret) {
		pr_info("%s parse \"dump-reg-num\" fail.\n", __func__);
		sreg->dump_reg_num = 0;
		return;
	}

	pr_info("%s dump-reg-num: %u.\n", __func__, sreg->dump_reg_num);
	if (sreg->dump_reg_num > DUMP_REG_MAX)
		sreg->dump_reg_num = DUMP_REG_MAX;

	ret = of_property_read_u32_array(np, "dump-regs", sreg->dump_regs, sreg->dump_reg_num);
	if (ret) {
		pr_err("%s parse \"dump-regs\" fail.\n", __func__);
		sreg->dump_reg_num = 0;
		return;
	}
}
	...
	struct platform_device *pdev)
{
	...
	regulator_dump_dt_parse(sreg, np);
...
static void media2_bus_onoff(void)
{
	...
	if (ret)
		pr_err("media2 ip power on fail, ret-%d\n", ret);

	ret = atfd_service_ip_regulator_smc(IP_REGULATOR_REGISTER_FN_ID,
			MEDIA2_SUBSYS_ID, REGULATOR_DISABLE, 0);
	if (ret)
		pr_err("media2 ip power off fail, ret-%d\n", ret);
}

static bool media1_poweroff_check(struct regulator_dev *dev)
{
	struct regulator_ip *sreg = rdev_get_drvdata(dev);
	struct regulator_ip_core *pmic = rdev_to_ip_core(dev);

	if (!of_find_node_by_name(NULL, "media1_power_off"))
		return false;

	if (sreg->regulator_id == DSSSUBSYS_ID)
		if (check_dss_is_power_on(sreg, pmic) == 0) {
			pr_err("%s dss is power off \n", __func__);
			return true;
		}
...
static void media1_bus_onoff(void)
{
	int ret;

	ret = atfd_service_ip_regulator_smc(IP_REGULATOR_REGISTER_FN_ID,
		MEDIA1_SUBSYS_ID, REGULATOR_ENABLE, 0);
	if (ret)
		pr_err("%s media1 ip power on fail, ret-%d\n", __func__, ret);

	ret = atfd_service_ip_regulator_smc(IP_REGULATOR_REGISTER_FN_ID,
		VIVOBUS_ID, REGULATOR_ENABLE, 0);
	if (ret)
		pr_err("%s vivobus ip power on fail, ret-%d\n", __func__, ret);

	ret = atfd_service_ip_regulator_smc(IP_REGULATOR_REGISTER_FN_ID,
		VIVOBUS_ID, REGULATOR_DISABLE, 0);
	if (ret)
		pr_err("%s vivobus ip power off fail, ret-%d\n", __func__, ret);

	ret = atfd_service_ip_regulator_smc(IP_REGULATOR_REGISTER_FN_ID,
		MEDIA1_SUBSYS_ID, REGULATOR_DISABLE, 0);
	if (ret)
		pr_err("%s media1 ip power off fail, ret-%d\n", __func__, ret);

	ip_regulator_debug("media1 power off when uefi no dss power on\n");
}

void regulator_dump_regs(struct regulator *regulator)
{
	struct regulator_ip *sreg = NULL;
	u32 i;

	if (IS_ERR_OR_NULL(regulator)) {
		pr_err("%s dev NULL point!\n", __func__);
		return;
	}

	sreg = regulator_get_drvdata(regulator);
	if (IS_ERR_OR_NULL(sreg->sctrl_reg)) {
		pr_err("%s sctrl_reg NULL point!\n", __func__);
		return;
	}

	for (i = 0; i < sreg->dump_reg_num; i++)
		pr_err("%s 0x%x: 0x%x!\n", __func__, sreg->dump_regs[i],
			readl(sreg->sctrl_reg + sreg->dump_regs[i]));
}
...
static int regulator_ip_probe(struct platform_device *pdev)
{
	...
	/* Power off media1 & vivobus when no dss on (dp screen) */
	if (media1_poweroff_check(rdev) == true)
		media1_bus_onoff();
