...
#include "../mntn/blackbox/rdr_field.h"
#include <m3_rdr_ddr_map.h>
#endif

#ifdef CONFIG_DFX_DIAGINFO_BASEINFO
#define MEDIA_MNT_MAGIC_NUM	0x4D454441 /* MEDA ascii */
...
static int ip_regulator_state_show(struct device *dev, void *data)
{
	...
	if ((strstr(rdev->constraints->name, "ldo") != NULL) ||
		...
		(strstr(rdev->constraints->name, "xvs") != NULL))
		return 0;
	ops = rdev->desc->ops;
	if (ops->is_enabled) {
		enabled = ops->is_enabled(rdev);
...
#ifdef CONFIG_DFX_DIAGINFO_BASEINFO
static int get_ip_regulator_mntn_offset(u64 *offset)
{
	int ret;
	...
	if (ret != 0) {
		pr_err("%s: rdr_get_areainfo of RDR_AREA_AP failed.\n", __func__);
		return -EINVAL;
	}

	ret = rdr_get_areainfo(RDR_AREA_LPM3, &rdr_area_lpm3_info);
	if (ret != 0) {
		pr_err("%s: rdr_get_areainfo of RDR_AREA_LPM3 failed.\n", __func__);
		return -EINVAL;
	}

	if (rdr_area_lpm3_info.log_addr < rdr_area_ap_info.log_addr) {
		pr_err("%s: log_addr is invalid\n", __func__);
		return -EINVAL;
	}

	*offset = rdr_area_lpm3_info.log_addr - rdr_area_ap_info.log_addr +
		RDR_BASEINFO_SIZE + M3_RDR_DDR_MNTN_OFFSET;
	return 0;
}

int ip_regulator_mntn_parse(const void *bbox_base, size_t size, char *out_buf, size_t buf_size)
{
	u64 offset;
	u32 *data;
	u32 end_offset, len = 0;
	bool flag = false;
	int ret;

	if (!bbox_base || !out_buf) {
		pr_err("%s: NULL point\n", __func__);
		return -1;
	}
	...
	/* get ddr mntn begin offset */
	ret = get_ip_regulator_mntn_offset(&offset);
	if ((ret != 0) || (offset > (u64)size)) {
		pr_err("%s: get offset failed, offset:%lu size:%u\n", __func__, offset, size);
		return -1;
	}

	/* get ddr mntn data end offset */
	end_offset = offset + M3_RDR_DDR_MNTN_SIZE;
	if (end_offset > (u64)size)
		end_offset = size;

	if (end_offset < ALIGN_WIDTH) {
		pr_err("%s: size-%u offset-%u is invalid\n", __func__, size, offset);
		return -1;
	}

	end_offset -= ALIGN_WIDTH;
	while (offset <= end_offset) {
		data = (u32*)(bbox_base + offset);
		offset += ALIGN_WIDTH;

		/* end magic number check */
		if (flag && (*data == MEDIA_MNT_MAGIC_NUM))
			break;

		if (!flag) {
			/* begin magic number check */
			if (*data == MEDIA_MNT_MAGIC_NUM)
				flag = true;
			continue;
		}

		ret = snprintf_s(out_buf + len, buf_size - len, buf_size - len - 1, "%x,", *data);
		if (ret < 0) {
			pr_err("%s: format err, offset:%lu buf_size:%u, len:%u\n",
				__func__, offset, buf_size, len);
			break;
		}
		len += ret;
	}
