...
static u64 g_get_counter_comb = 0;
#endif
...
static void ufs_rpmb_time_stamp_dump(void)
{
	int i, j;
	int state;
	char *sourcename[RPMB_FROM_MAX] = {"SECURE_OS", "MSPC", "AUTH_KEY"};
	char *mspc_state[] = {"nothing to do", "set key", "read", "get counter(atf)", "get counter", "write"};
	char *ioctl_opt[] = {"read", "write", "get counter", "no opt", "writr protect", "read protect"};

#ifdef CONFIG_RPMB_TIME_DEBUG
	if (g_get_counter_comb > 0)
		pr_err("rpmb get counter comb begin time [%llu]\n", g_get_counter_comb);
#endif
	for (i = 0; i < RPMB_FROM_MAX; i++) {
		if (rpmb_time_point[i].begin_combinate > 0) {
			state = rpmb_time_point[i].state;
#ifdef CONFIG_RPMB_TIME_DEBUG
			pr_err("rpmb req source[%s]: rpmb opt[%s], rpmb begin comb time[%llu]ns\n",
				sourcename[i], (i == RPMB_FROM_SECURE_OS ? ioctl_opt[state] :
				 (i == RPMB_FROM_MSPC ? mspc_state[state] : "rpmb set key")), rpmb_time_point[i].begin_combinate);
			pr_err("lock time[%llu]ns\n", rpmb_time_point[i].cmd_lock_done);
			pr_err("rpmb ufs cmd1 time: ufs start time1[%llu]ns, ufs end time1[%llu]ns, cost[%llu]ns\n",
				rpmb_time_point[i].rpmb_ufs_start_time[0], rpmb_time_point[i].rpmb_ufs_end_time[0],
				rpmb_time_point[i].rpmb_ufs_end_time[0] - rpmb_time_point[i].rpmb_ufs_start_time[0]);
			pr_err("rpmb ufs cmd2 time: ufs start time2[%llu]ns, ufs end time2[%llu]ns, cost[%llu]ns\n",
				rpmb_time_point[i].rpmb_ufs_start_time[1], rpmb_time_point[i].rpmb_ufs_end_time[1],
				rpmb_time_point[i].rpmb_ufs_end_time[1] - rpmb_time_point[i].rpmb_ufs_start_time[1]);

			if (((i == RPMB_FROM_SECURE_OS) && ((state == RPMB_OP_WR_DATA) || (state == RPMB_OP_WR_PROTECT))) ||
				((i == RPMB_FROM_MSPC) && (state == RPMB_STATE_WR_DATA)))
				pr_err("rpmb ufs cmd3 time: ufs start time3[%llu]ns, ufs end time3[%llu]ns, cost[%llu]ns\n",
				rpmb_time_point[i].rpmb_ufs_start_time[2], rpmb_time_point[i].rpmb_ufs_end_time[2],
				rpmb_time_point[i].rpmb_ufs_end_time[2] - rpmb_time_point[i].rpmb_ufs_start_time[2]);

			pr_err("unlock time[%llu]ns\n", rpmb_time_point[i].cmd_unlock);
#endif
			pr_err("rpmb req source[%s]: rpmb opt[%s], (rpmb begin combinate to io end)cost time[%llu]ns,"
				" (blk + scsi + ufs device)cost time[%llu]ns\n", sourcename[i],
				(i == RPMB_FROM_SECURE_OS ? ioctl_opt[rpmb_time_point[i].state] :
				(i == RPMB_FROM_MSPC ? mspc_state[rpmb_time_point[i].state] : "rpmb set key")),
				rpmb_time_point[i].cmd_unlock - rpmb_time_point[i].begin_combinate,
				rpmb_time_point[i].cmd_unlock - rpmb_time_point[i].cmd_lock_done);
			pr_err("ufs req1 dev cost time[%llu]ns\n",
					rpmb_time_point[i].rpmb_ufs_end_time[0] - rpmb_time_point[i].rpmb_ufs_start_time[0]);
			pr_err("ufs req2 dev cost time[%llu]ns\n",
					rpmb_time_point[i].rpmb_ufs_end_time[1] - rpmb_time_point[i].rpmb_ufs_start_time[1]);
			if (((i == RPMB_FROM_SECURE_OS) && ((state == RPMB_OP_WR_DATA) || (state == RPMB_OP_WR_PROTECT))) ||
				((i == RPMB_FROM_MSPC) && (state == RPMB_STATE_WR_DATA)))
				pr_err("ufs req3 dev cost time[%llu]ns\n",
						rpmb_time_point[i].rpmb_ufs_end_time[2] - rpmb_time_point[i].rpmb_ufs_start_time[2]);
		}
	}

	for (i = 0; i < RPMB_FROM_MAX; i++) {
		rpmb_time_point[i].begin_combinate = 0;
		rpmb_time_point[i].cmd_lock_done = 0;
		rpmb_time_point[i].cmd_unlock = 0;
		for (j = 0; j < UFS_IOC_MAX_RPMB_CMD; j++) {
			rpmb_time_point[i].rpmb_ufs_start_time[j] = 0;
			rpmb_time_point[i].rpmb_ufs_end_time[j] = 0;
		}
	}

#ifdef CONFIG_RPMB_TIME_DEBUG
	g_get_counter_comb = 0;
#endif
#ifdef CONFIG_RPMB_BLK_TIME_DEBUG
...
#ifndef CONFIG_RPMB_REQ_LOCK_DISABLE
	mutex_unlock(&rpmb_ufs_cmd_lock);
	rpmb_time_point[source].cmd_unlock = dfx_getcurtime();
#endif
	return ret;
}
#else
static int rpmb_send_ufs_verify(int32_t fd)
{
...
#else
		transfer_frame[i] = kzalloc(
			MAX_RPMB_FRAME * sizeof(struct rpmb_frame),
			GFP_KERNEL | LIBLINUX_PAL_GFP_RESERVE);
#endif
		if (!transfer_frame[i]) {
...
static int ufs_rpmb_work(struct rpmb_request *request)
{
	...
	rpmb_time_point[RPMB_FROM_MSPC].begin_combinate = dfx_getcurtime();
...
#ifdef CONFIG_RPMB_TIME_DEBUG
		g_get_counter_comb = dfx_getcurtime();
#endif
#endif
		ufs_rpmb_get_counter(request, rpmb_data, sense_buffer,
				     transfer_frame);
			    ...
			    struct storage_blk_ioc_rpmb_data *storage_data)
{
	...
	rpmb_time_point[RPMB_FROM_SECURE_OS].begin_combinate = dfx_getcurtime();
