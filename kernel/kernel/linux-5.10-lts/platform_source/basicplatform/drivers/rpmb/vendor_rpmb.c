...
#define UFS_VENDOR_VELA61 0x8B6
...
static enum bootdevice_type rpmb_support_device = BOOT_DEVICE_MAX;
...
{
	/* rpmb time record */
	pr_err("[%s]: tzsp(atf) start %llu, rpmb ap thread start %llu,"
			" rpmb ap thread end %llu , tzsp(atf) end %llu\n",
	       __func__, g_atf_start_time, g_mspc_start_time,
	       g_mspc_end_time, g_atf_end_time);
...
static void rpmb_mspc_time_stamp(enum rpmb_state state, unsigned int region, unsigned int blks,
			  struct rpmb_operation *rpmb_ops)
{
	char *mspc_rpmb_state[] = {"nothing to do", "set key", "read", "get counter(atf)", "get counter", "write"};
	u64 mspc_start = 0;

	u64 temp_aft_start = (g_atf_start_time * RPMB_TIME_UNIT * RPMB_TIME_UNIT / RPMB_TZSP_TIME) * RPMB_TIME_UNIT;
	u64 temp_aft_end = (g_atf_end_time * RPMB_TIME_UNIT * RPMB_TIME_UNIT / RPMB_TZSP_TIME) * RPMB_TIME_UNIT;

	if (state == RPMB_STATE_WR_CNT || state == RPMB_STATE_CNT)
		mspc_start = g_mspc_get_cnt_start_time;
	else
		mspc_start = g_mspc_start_time;

	if ((g_mspc_end_time - mspc_start) < RPMB_TIMEOUT_PRINT_IN_KERNEL)
		return;

	pr_err("[%s]rpmb work:opt=%s, region=%u, blk count=%u\n", __func__, mspc_rpmb_state[state], region, blks);
	pr_err("[%s]rpmb ap thread start[%llu]ns, rpmb ap thread end[%llu]ns, ap issue cost time[%llu]ns\n",
			__func__, mspc_start, g_mspc_end_time, g_mspc_end_time - mspc_start);
	pr_err("[%s]tzsp(atf) start[%llu]ns, tzsp(atf) end[%llu]ns, tzsp(atf) issue cost time[%llu]ns\n",
			__func__, temp_aft_start, temp_aft_end, temp_aft_end - temp_aft_start);
...
static void rpmb_work_routine(void)
{
	...
	int ret;
	enum rpmb_state temp_state = RPMB_STATE_IDLE;
	struct rpmb_request *request = vendor_rpmb.rpmb_request;
	struct rpmb_operation *rpmb_ops = vendor_rpmb.ops;
	...
	temp_state = request->info.state;
	if (temp_state == RPMB_STATE_WR_CNT || temp_state == RPMB_STATE_CNT)
		g_mspc_get_cnt_start_time = g_mspc_start_time;
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(5,4,0))
...
out:
	...
	rpmb_mspc_time_stamp(temp_state, request->info.rpmb_region_num,
...
int get_rpmb_key_status(void)
{
	...
	if (get_rpmb_support_key_num() == 1) {
		...
		result = atfd_rpmb_smc((u64)RPMB_SVC_MULTI_KEY_STATUS,
					    (u64)KEY_STATUS_ALL_MSK, (u64)0x0, (u64)0x0);
		if (result == KEY_STATUS_ALL_MSK) {
			return 0;
		} else {
			pr_err("[%s]:rpmb key status failed, result=%d\n", __func__, result);
...
__attribute__((weak)) int wait_rpmb_device_ready(void)
{
	...
	if (i == WAIT_INIT_TIMES) {
		pr_err("wait for device init timeout, rpmb_support_device = %d, ret = %d\n", rpmb_support_device, ret);
...
{
	if (rpmb_support_device == BOOT_DEVICE_NORFLASH) {
	...
	if (rpmb_drivers_init_status == RPMB_DRIVER_IS_READY &&
	    rpmb_key_status == KEY_NOT_READY) {
		rpmb_key_status_check();
		pr_err("[%s]:rpmb key status[%s]\n", __func__,
			rpmb_key_status == KEY_NOT_READY ? "not ready" : "ready");
	}
...
static void rpmb_os_time_stamp(enum rpmb_op_type operation,
			       struct rpmb_operation *rpmb_ops, struct rpmb_storage_data_debug *storage_data_debug)
{
	int i = 0;
	char *rpmb_op_type_buf[] = {"read", "write", "get counter", "no opt", "writr protect", "read protect"};
	char *rpmb_func_id_buf[] = {"RESERVED", "SE", "SECURE_OS"};

	g_os_end_time = dfx_getcurtime();
	if ((g_os_end_time - g_os_start_time) < RPMB_TIMEOUT_PRINT_IN_KERNEL)
		return;

	pr_err("[%s]rpmb opt[%s], req start[%llu]ns, req end[%llu]ns, req in ap cost time[%llu]ns\n",
			__func__, rpmb_op_type_buf[operation], g_os_start_time, g_os_end_time, g_os_end_time - g_os_start_time);
	for (i = 0; i < STORAGE_IOC_MAX_RPMB_CMD; i++) {
		if ((i == 2) && (operation != RPMB_OP_WR_DATA) && (operation != RPMB_OP_WR_PROTECT))
			break;
		pr_err("[%s]:req frame[%d]: func id[%s], opt[%s], blk count=%u, buf bytes=%u\n", __func__, i,
		rpmb_func_id_buf[storage_data_debug[i].func_id], rpmb_op_type_buf[storage_data_debug[i].operation],
		storage_data_debug[i].block_count, storage_data_debug[i].buf_bytes);
	}
			...
			struct storage_blk_ioc_rpmb_data *storage_data)
{
	...
	struct rpmb_operation *rpmb_ops = vendor_rpmb.ops;
	int i = 0;
	struct rpmb_storage_data_debug storage_data_debug[STORAGE_IOC_MAX_RPMB_CMD] = {0};
	...
	for (i = 0; i < STORAGE_IOC_MAX_RPMB_CMD; i++) {
		storage_data_debug[i].func_id = id;
		storage_data_debug[i].operation = operation;
		storage_data_debug[i].block_count = storage_data->data[i].blocks;
		storage_data_debug[i].buf_bytes = storage_data->data[i].buf_bytes;
	}
...
out:
	...
	rpmb_os_time_stamp(operation, rpmb_ops, storage_data_debug);
	return ret;
}
#endif
...
{
	return rpmb_device_init_status;
}
#endif
...
static int __init rpmb_init(void)
{
	...
	u32 type = get_bootdevice_type();
	if ((type & (0x1 << DEVICE_TYPE_NORFLASH)))
		rpmb_support_device = BOOT_DEVICE_NORFLASH;
	else
		rpmb_support_device = type;
...
#ifndef CONFIG_MMC_SDHCI_ZODIAC_APUS
