...
#define KEY_STATUS_ALL_MSK ((1 << MAX_KEY_STATUS_NUM) - 1)
...
extern char g_rpmb_ufs_model[MAX_MODEL_LEN + 1];
...
#define RPMB_TIME_UNIT 1000
#define RPMB_TZSP_TIME 32768
#define RPMB_TIMEOUT_PRINT_IN_KERNEL 100000000 /* Printing MT Logs If the Time Exceeds 100ms */

struct rpmb_storage_data_debug {
	uint32_t func_id;
	uint32_t operation;
	u32 block_count;
	u64 buf_bytes;
...
struct rpmb_time_record {
	...
	u64 begin_combinate;
	u64 cmd_lock_done;
	u64 rpmb_ufs_start_time[UFS_IOC_MAX_RPMB_CMD];
	u64 rpmb_ufs_end_time[UFS_IOC_MAX_RPMB_CMD];
	u64 cmd_unlock;
};
...
extern long blk_scsi_kern_ioctl(void *bd, unsigned long arg, bool need_order,
					 unsigned int source_id, unsigned int io_cnt);
#else
extern long blk_scsi_kern_ioctl(unsigned int fd, unsigned long arg, bool need_order,
			 unsigned int source_id, unsigned int io_cnt);
#endif
...
#endif
int norflash_rpmb_key_status(void);
