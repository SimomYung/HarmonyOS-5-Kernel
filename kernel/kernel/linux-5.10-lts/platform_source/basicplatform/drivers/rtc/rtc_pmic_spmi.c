...
static struct wakeup_source *alarm_guarantee_lock;
...
static int pmu_rtc_synctime_by_pmu(time64_t rtc_offset)
{
	...
	unsigned long pmu_curr_time = pmu_rtc_read_bulk(ldata->pmu_rtc_baseaddr + PMU_RTC_DR);
	unsigned long pmu_sync_time = (time64_t)pmu_curr_time + (time64_t)rtc_offset;
...
static int virt_soc_rtc_read_time(struct device *dev, struct rtc_time *tm)
{
	int ret;
	unsigned long sec = 0;
...
static int virt_soc_rtc_set_time(struct device *dev, struct rtc_time *tm)
{
	unsigned long sec;
...
static int virt_soc_rtc_read_alarm(struct device *dev, struct rtc_wkalrm *alarm)
{
	int ret;
	unsigned long sec = 0;
...
static int virt_soc_rtc_set_alarm(struct device *dev, struct rtc_wkalrm *alarm)
{
	unsigned long sec;
...
static int virt_soc_rtc_alarm_irq_enable(struct device *dev,
	unsigned int enabled)
{
	int ret = 0;
...
static const struct rtc_class_ops virt_rtc_ops = {
	.read_time = virt_soc_rtc_read_time,
	.set_time = virt_soc_rtc_set_time,
	.read_alarm = virt_soc_rtc_read_alarm,
	.set_alarm = virt_soc_rtc_set_alarm,
	.alarm_irq_enable = virt_soc_rtc_alarm_irq_enable,
};
#endif /* CONFIG_LIBLINUX */
...
static void rtc_alarm_notify_work(struct work_struct *work)
{
	...
	__pm_stay_awake(alarm_guarantee_lock);
#ifdef CONFIG_LIBLINUX
	...
	msleep(WAKEUP_RELAX_WAIT);
	__pm_relax(alarm_guarantee_lock);
}
...
#ifndef CONFIG_LIBLINUX
	if (unlikely(get_pd_charge_flag()))
		alarm_guarantee_lock = wakeup_source_register(dev, "alarm_guarantee_lock");
...
#elif (defined CONFIG_LIBLINUX_WATCHES_RTC)
static int rtc_suspend(struct device *dev)
{
	pr_err("%s: suspend \n", __func__);
	return 0;
...
static int rtc_resume(struct device *dev)
{
	...
	rtime = pmu_rtc_read_bulk(
			ldata->pmu_rtc_baseaddr + PMU_RTC_DR);
 
	rtc_time64_to_tm((offset + rtime), &soctime);
	
	virt_soc_rtc_set_time(dev, &soctime);
 
	tv64.tv_sec = offset + rtime;
	do_settimeofday64(&tv64);
 
	pr_err("%s: setting system clock to UTC =%lld\n", __func__, (long long)tv64.tv_sec);
