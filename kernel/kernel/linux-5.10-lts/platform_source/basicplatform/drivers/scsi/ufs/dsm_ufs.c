...
struct error_no_ufs_map isp_error_no_ufs_map[] = {
	{ 928008000, "UFS_POWER_MODE_FAIL", 1 },
	{ 928008001, "UFS_RW_FAIL", 1 },
	{ 928008002, "UFS_SYS_BUS_ERR", 1 },
	{ 928008003, "UFS_UIC_TRAN_ERR", 1 },
	{ 928008004, "UFS_UIC_CMD_ERR", 1 },
	{ 928008005, "UFS_CONTROLLER_ERR", 1 },
	{ 928008006, "UFS_DEVICE_ERR", 1 },
	{ 928008007, "UFS_TIMEOUT_ERR", 1 },
	{ 928008008, "UFS_UTP_ERR", 1 },
	{ 928008009, "UFS_ASSERT_NUM_ERR", 1 },
	{ 928008010, "UFS_VOLT_ERR", 1 },
	{ 928008011, "UFS_LINKUP_ERR", 1 },
	{ 928008012, "UFS_ENTER_OR_EXIT_H8_ERR", 1 },
	{ 928008013, "UFS_LIFETIME_EXCCED_ERR", 1 },
	{ 928008014, "UFS_HARDWARE_RESET", 1 },
	{ 928008015, "UFS_MEDIUM_ERR", 1 },
	{ 928008016, "UFS_PA_INIT_ERR", 0 },
	{ 928008017, "UFS_INLINE_CRYPTO_ERR", 1 },
	{ 928008018, "UFS_HARDWARE_ERR", 1 },
	{ 928008019, "UFS_LINE_RESET_ERR", 0 },
	{ 928008021, "UFS_HI186X_FSR", 0 },
	{ 928008022, "UFS_DEVICE_FATAL_ERROR", 1 },
	{ 928008023, "UFS_RTBB_ABNORMAL", 1 },
	{ 928008024, "UFS_HINFO_ABNORMAL", 1 },
	{ 928008025, "UFS_PROGRAM_FAIL_CNT", 1 },
	{ 928008026, "UFS_ERASE_FAIL_CNT", 1 },
	{ 928008027, "UFS_IO_TIMEOUT", 0 },
	{ 928008028, "UFS_PWRMODE_SWITCH", 1 },
	{ 928008029, "UFS_UIC_TRANS_BIT_ERR_EXCCED", 1 },
	{ 928008030, "UFS_CPORT_TIMEOUT_ERR", 1 },
	{ 928008031, "UFS_DFA_TIMEOUT_ERR", 1 },
	{ 925205300, "UFS_POWER_DOWN_FLUSH_ERR", 0 },
	{ 925205510, "UFS_MPHY_CALIBRATION_INFO", 0 },
};
...
static int ufs_errorno_get_running_test_switch(int error_no)
{
	int i;
	int isp_error_no_map_len = sizeof(isp_error_no_ufs_map) / sizeof(struct error_no_ufs_map);
	...
	for (i = 0; i < isp_error_no_map_len; i++) {
		if (isp_error_no_ufs_map[i].error_no == error_no) {
			return isp_error_no_ufs_map[i].running_test_switch;
		}
	}
...
static int ufs_errorno_get_event_type(int error_no)
{
	if (error_no == DSM_UFS_MPHY_CALIBRATION_INFO) {
		return STATISTIC;
	} else {
		return FAULT;
	}
...
struct dsm_client_ops hisp_dsm_ops_ufs = {
	...
	.errorno_get_event_type = ufs_errorno_get_event_type,
	.errorno_get_running_test_switch = ufs_errorno_get_running_test_switch,
#endif
};
#endif
...
#endif
	{UFS_UIC_TRANS_BIT_ERR_EXCCED, DSM_UFS_UIC_TRANS_BIT_ERR_EXCCED, "UIC trans bit error in [10^-12, 10^-10]"},
	{UFS_CPORT_TIMEOUT_ERR, DSM_UFS_CPORT_TIMEOUT_ERR, "UFS Cport TimeOut Error"},
	{UFS_DFA_TIMEOUT_ERR, DSM_UFS_DFA_TIMEOUT_ERR, "UFS DFA TimeOut Error"},
};

EXPORT_SYMBOL(ufs_dclient);

unsigned long dsm_ufs_if_err(struct ufs_hba *hba)
{
	unsigned long ret = g_ufs_dsm_adaptor.err_type;
#ifdef CONFIG_DFA_MCQ
	struct dfa_hba_info *dhba = &hba->dhba;
...
int dsm_ufs_update_upiu_info(struct ufs_hba *hba, int tag, int err_code)
{
	...
	if (err_code == DSM_UFS_TIMEOUT_ERR) {
		if (test_and_set_bit_lock(UFS_TIMEOUT_ERR,
			...
			return -1;
	} else if (err_code == DSM_UFS_CPORT_TIMEOUT_ERR) {
		if (test_and_set_bit_lock(UFS_CPORT_TIMEOUT_ERR,
			&g_ufs_dsm_adaptor.err_type))
			return -1;
	} else if (err_code == DSM_UFS_DFA_TIMEOUT_ERR) {
		if (test_and_set_bit_lock(UFS_DFA_TIMEOUT_ERR,
			&g_ufs_dsm_adaptor.err_type))
			return -1;
...
int dsm_ufs_if_uic_err_disable(struct ufs_hba *hba)
{
#ifdef CONFIG_DFA_MCQ
		if (is_dfa_enabled(hba))
			return 0;
...
int dsm_ufs_update_uic_info(struct ufs_hba *hba, int err_code)
{
	...
	if (dsm_ufs_if_uic_err_disable(hba))
		return 0;

	if (test_and_set_bit_lock(UFS_UIC_TRANS_ERR, &g_ufs_dsm_adaptor.err_type))
		return -1;

	if ((hba->ufs_stats.event[UFS_EVT_PA_ERR].pos < 1) ||
		(hba->ufs_stats.event[UFS_EVT_DL_ERR].pos < 1) ||
		(hba->ufs_stats.event[UFS_EVT_NL_ERR].pos < 1) ||
		(hba->ufs_stats.event[UFS_EVT_TL_ERR].pos < 1) ||
		(hba->ufs_stats.event[UFS_EVT_DME_ERR].pos < 1))
		return -1;
...
int dsm_ufs_update_error_info(struct ufs_hba *hba, int code)
{
	...
	case DSM_UFS_SYSBUS_ERR:
		(void)test_and_set_bit_lock(UFS_SYSBUS_ERR,
			&g_ufs_dsm_adaptor.err_type);
		break;
	...
	unsigned long code)
{
	...
	if (!(g_ufs_dsm_adaptor.err_type & ((1<<UFS_UIC_TRANS_ERR) |
		...
		(1 << UFS_UIC_TRANS_BIT_ERR_EXCCED)))) {
	...
	unsigned long code)
{
	...
	if (!(g_ufs_dsm_adaptor.err_type & ((1 << UFS_TIMEOUT_ERR) |
		(1 << UFS_HARDWARE_RESET) | (1 << UFS_CPORT_TIMEOUT_ERR) |
		(1 << UFS_DFA_TIMEOUT_ERR)))) {
		pr_err("UFS DSM Error! Err Num: %lu, %ld\n",
			code, g_ufs_dsm_adaptor.err_type);
		return;
	}
...
int dsm_ufs_update_host_reset_info(struct ufs_hba *hba, int err_code)
{
	...
	int *size, unsigned long code)
{
	...
	case DSM_UFS_UIC_TRANS_BIT_ERR_EXCCED:
		/* fall through */
	...
	case DSM_UFS_CPORT_TIMEOUT_ERR:
	case DSM_UFS_DFA_TIMEOUT_ERR:
...
int dsm_ufs_uic_err_need_report(struct ufs_hba *hba, unsigned long *sub_err_type)
{
	u32 ov_flag = 0;
	u32 counter = 0;
	bool dfa_enable = false;
	...
	if (is_dfa_enabled(hba)) {
		struct dfa_hba_info *dhba = &hba->dhba;
		ov_flag = dhba->uic_trans_err.ov_flag;
		counter = dhba->uic_trans_err.counter;
		dfa_enable = true;
	}
#endif
	...
	if ((ten_billon_bits_cnt >= MIN_BER_SAMPLE_BITS) &&
		...
		*sub_err_type = UFS_UIC_TRANS_ERR;
		return 1;
	}
	if (hba->ufs_device_spec_version == UFS_VERSION_4_0 && (ten_billon_bits_cnt >= MIN_BER_SAMPLE_BITS) &&
		(uic_err_history.delta_err_cnt * UIC_TRANS_BITS_ERR_FACTOR > uic_err_history.delta_bit_cnt) &&
		(((ov_flag & 0x1) != 0) || (counter != 0))) {
		*sub_err_type = UFS_UIC_TRANS_BIT_ERR_EXCCED;
		return 1;
	}
...
{
	unsigned long flags;
	unsigned long sub_err_type = 0xff;

	if (g_ufs_dsm_adaptor.err_type & (1 << UFS_UIC_TRANS_ERR)) {
		if (dsm_ufs_uic_err_need_report(hba, &sub_err_type)) {
			if (sub_err_type == UFS_UIC_TRANS_ERR) {
				dsm_ufs_log(hba, DSM_UFS_UIC_TRANS_ERR, "UIC No Fatal Error");
			} else if(sub_err_type == UFS_UIC_TRANS_BIT_ERR_EXCCED) {
				dsm_ufs_log(hba, DSM_UFS_UIC_TRANS_BIT_ERR_EXCCED, "UIC trans bit error in [10^-12, 10^-10]");
			} else {
				pr_err("%s: sub_err_type  = %d err!\n", __func__, sub_err_type);
			}
		}
...
{
	if (g_ufs_dsm_adaptor.err_type & ((1 << UFS_TIMEOUT_ERR) | (1 << UFS_CPORT_TIMEOUT_ERR) |
		(1 << UFS_DFA_TIMEOUT_ERR))) {
		...
		if (g_ufs_dsm_adaptor.err_type & (1 << UFS_CPORT_TIMEOUT_ERR)) {
			dsm_ufs_log(hba, DSM_UFS_CPORT_TIMEOUT_ERR, "UFS Cport TimeOut Error");
			clear_bit_unlock(UFS_CPORT_TIMEOUT_ERR, &g_ufs_dsm_adaptor.err_type);
		}
		if (g_ufs_dsm_adaptor.err_type & (1 << UFS_DFA_TIMEOUT_ERR)) {
			dsm_ufs_log(hba, DSM_UFS_DFA_TIMEOUT_ERR, "UFS DFA TimeOut Error");
			clear_bit_unlock(UFS_DFA_TIMEOUT_ERR, &g_ufs_dsm_adaptor.err_type);
		}
	}
}
...
static bool dsm_hisysevent_is_ready()
{
	int count = 0;
	bool ret;
	struct file *filp;

	while (count < UFS_DSM_RETRY_NUM) {
		filp = filp_open("/dev/bbox", O_WRONLY, 0);
		if (!filp || IS_ERR(filp)) {
			msleep(DELAY_MS_100);
			count++;
		} else {
	...
	ret = (count < UFS_DSM_RETRY_NUM)? true : false;
	return ret;
}
#endif
#endif
...
void dsm_ufs_handle_work(struct work_struct *work)
{
...
#ifdef CONFIG_HISYSEVENT
#ifndef CONFIG_LIBLINUX_UDK_UFS
	if (!dsm_hisysevent_is_ready()) {
		pr_err("hisysevent is not ready after retry\n");
		return;
	}
#endif
#endif
	...
	for (i = 0; i < len; i++) {
		...
		case UFS_CPORT_TIMEOUT_ERR:
		case UFS_DFA_TIMEOUT_ERR:
...
{
	if ((dsm_ufs_enabled()) && (dsm_ufs_if_err(hba)))
		queue_work(system_freezable_wq, &hba->dsm_work);
}
