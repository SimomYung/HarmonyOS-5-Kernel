...
void ufshcd_hufs_update_upiu_prdt_offset(
...
int ufshcd_hufs_disable_auto_hibern8(struct ufs_hba *hba)
{
	...
	ufs_disable_auto_h8_status_sync(hba);
#endif
	...
	struct ufs_hba *hba, struct ufs_pa_layer_attr *dev_req_params)
{
	...
	dev_info(hba->dev, "%s ++\n", __func__);
#ifdef CONFIG_DFX_DEBUG_FS
	dev_info(hba->dev, "device manufacturer_id is 0x%x\n", hba->manufacturer_id);
#endif
...
irqreturn_t ufshcd_ufs_intr_hub_intr(int ufs_intr_hub_irq, void *__hba)
{
	...
	if (host->host_id == 1) {
		intr_stat = readl(host->hsdt1_intr_hub +
				INTR_HUB_LEVEL2_INTR_STATUS_NS_N(HSDT1_INTR_HUB_UFS_GROUP));
		if (intr_stat & BIT(HSDT1_INTR_UNIPRO_GIC_BIT))
			retval = _ufshcd_hufs_unipro_intr(ufs_intr_hub_irq, hba);

		if (intr_stat & BIT(HSDT1_INTR_UFSHC_GIC_BIT))
...
static void ufshcd_timeout_handler(struct ufs_hba *hba, u32 intr_status)
{
	...
	if (intr_status & UFS_RX_CPORT_TIMEOUT_INTR)
		dsm_ufs_update_upiu_info(hba, -1, DSM_UFS_CPORT_TIMEOUT_ERR);
#ifdef CONFIG_HUFS_HC_CORE_UTR
	if ((intr_status & MASK_CORE_IE_IO_TIMEOUT) ||
	    (intr_status & VS_IS_IO_TIMEOUT))
#else
	if (intr_status & VS_IS_IO_TIMEOUT)
#endif
...
static void ufshcd_sl_vs_intr(struct ufs_hba *hba, u32 vs_intr_status)
{
	...
	if ((vs_intr_status & VS_AH8_ENTER_INTR) && (ufshcd_readl(hba, UFS_AUTO_H8_STATE_OFF) == 0x20) &&
		(atomic_read(&hba->mcq_inflt_reqs) == 0)) {
		ufs_low_power_vote(hba, true);
		ufs_dfa_ccbus_vote(hba, true);
		if (hba->ufs_dfa_lp_voted)
			dev_err(hba->dev, "err: ufs_dfa_lp_voted true is not expect\n");
		hba->ufs_dfa_lp_voted = true;
	}
	spin_unlock_irqrestore(hba->host->host_lock, flags);
#elif defined(CONFIG_UFS_LOW_POWER_VOTE_STATUS_SYNC)
	if ((vs_intr_status & VS_AH8_ENTER_INTR) && (ufshcd_readl(hba, UFS_AUTO_H8_STATE_OFF) == 0x20))
		ufs_auto_h8_entry_status_sync(hba);
#endif
...
#endif
		ufshcd_timeout_handler(hba, vs_intr_status);
	}
...
int request_irq_for_core_irq(struct ufs_hba *hba, struct device *dev)
{
	...
	for (i = 0; i < UTR_CORE_NUM; i++) {
		...
		err = irq_set_affinity((u32)hba->core_irq[i], cpumask_of(UFS_IRQ_DEFAULT_CORE));
#else
				...
				unsigned int queue_num)
{
	if (unlikely(intr_status & MASK_CORE_IE_IO_TIMEOUT)) {
		...
		hufs_key_reg_dump(hba);
		ufshcd_timeout_handler(hba, intr_status);
	}
...
#ifdef CONFIG_UFS_LOW_POWER_VOTE_STATUS_SYNC
void ufs_set_ldk_cmd_exist(struct ufs_hba *hba, bool enable)
{
	struct hufs_host *host = NULL;
	unsigned int reg_val;

	host = (struct hufs_host *)hba->priv;
	reg_val = UFS_LDK_CMD_EXIST_MASK;
	if (enable)
		reg_val |= UFS_LDK_CMD_EXIST_BIT;

	writel(reg_val, SOC_SCTRL_SCBAKDATA1_MSK_ADDR(host->sysctrl));
}

static bool ufs_check_status_exist(struct ufs_hba *hba, unsigned int val)
{
	struct hufs_host *host = NULL;
	unsigned int reg_val;

	host = (struct hufs_host *)hba->priv;
	reg_val = readl(SOC_SCTRL_SCBAKDATA1_MSK_ADDR(host->sysctrl));
	if (reg_val & val)
		return true;
...
static void ufs_set_low_power_vote_flag(struct ufs_hba *hba, bool enable)
{
	struct hufs_host *host = NULL;
	unsigned int reg_val;

	host = (struct hufs_host *)hba->priv;
	reg_val = UFS_LOW_POWER_VOTE_MASK;
	if (enable)
		reg_val |= UFS_LOW_POWER_VOTE_BIT;

	writel(reg_val, SOC_SCTRL_SCBAKDATA1_MSK_ADDR(host->sysctrl));
}

static void ufs_decrease_freq(struct ufs_hba *hba)
{
	if (is_dfa_enabled(hba)) {
		if (dfa_sram_deep_sleep_vote(hba, true)) {
			dev_err(hba->dev, "%s: skip vote sram\n", __func__);
			return;
		}
		dfa_cpu_clk_vote(hba, true);
	}
	ufs_low_power_vote(hba, true);
	if (ufs_check_status_exist(hba, UFS_LOW_POWER_VOTE_BIT))
		pr_err("%s: have low power vote is not expect\n", __func__);

	ufs_set_low_power_vote_flag(hba, true);
}

static void ufs_increase_freq(struct ufs_hba *hba)
{
	if (ufs_check_status_exist(hba, UFS_LOW_POWER_VOTE_BIT)) {
		if (is_dfa_enabled(hba)) {
			dfa_sram_deep_sleep_vote(hba, false);
			dfa_cpu_clk_vote(hba, false);
		}
		ufs_low_power_vote(hba, false);
		ufs_set_low_power_vote_flag(hba, false);
	}
}

void ufs_auto_h8_entry_status_sync(struct ufs_hba *hba)
{
	int ret;
	unsigned long flags;

	if (atomic_read(&hba->rpmb_lp_delayed))
		return;

	spin_lock_irqsave(&hba->sync_lock, flags);
	ret = hwspin_lock_timeout(hba->hwlock, UFS_HWLOCK_TIMEOUT);
	if (ret) {
		pr_err("%s: hwlock is busy\n", __func__);
		goto free_sync_lock;
	}

	if (ufs_check_status_exist(hba, UFS_CMD_EXIST_BIT)) {
		pr_debug("%s: cmd exist, can not decrease frequency\n", __func__);
		goto free_hw_lock;
	}

	ufs_decrease_freq(hba);

free_hw_lock:
	hwspin_unlock(hba->hwlock);
free_sync_lock:
	spin_unlock_irqrestore(&hba->sync_lock, flags);
}

void ufs_send_cmd_status_sync(struct ufs_hba *hba)
{
	int ret;
	unsigned long flags;

	atomic_set(&hba->rpmb_lp_delay_extend, 1);
	...
	if (atomic_read(&hba->rpmb_lp_delayed) && atomic_read(&hba->rpmb_lp_voted))
		goto free_sync_lock;

	ret = hwspin_lock_timeout(hba->hwlock, UFS_HWLOCK_TIMEOUT);
	if (ret) {
		pr_err("%s: hwlock is busy\n", __func__);
		/* force open mphy clk if hwspinlock timeout, avoid ufs&dfa cmd error */
		ufs_mphy_clk_vote(hba, false);
		goto free_sync_lock;
	}

	ufs_set_ldk_cmd_exist(hba, true);
	ufs_increase_freq(hba);
	hwspin_unlock(hba->hwlock);

	if (atomic_read(&hba->rpmb_lp_delayed))
		atomic_set(&hba->rpmb_lp_voted, 1);

free_sync_lock:
	spin_unlock_irqrestore(&hba->sync_lock, flags);
}

void ufs_cmd_compl_status_sync(struct ufs_hba *hba)
{
	int ret;
	unsigned long flags;
	...
	ufs_set_ldk_cmd_exist(hba, false);
	hwspin_unlock(hba->hwlock);
...
void ufs_disable_auto_h8_status_sync(struct ufs_hba *hba)
{
	int ret;
	unsigned long flags;
	...
	if (ufs_check_status_exist(hba, UFS_CMD_EXIST_BIT)) {
		pr_info("%s: cmd exist, no need increase_freq\n", __func__);
		goto free_hw_lock;
	}

	pr_debug("%s: increase_freq\n", __func__);
	ufs_increase_freq(hba);
...
static void ufs_clear_all_bit_flag(struct ufs_hba *hba)
{
	struct hufs_host *host = NULL;
	unsigned int reg_val;

	host = (struct hufs_host *)hba->priv;
	reg_val = UFS_CMD_EXIST_MASK | UFS_LOW_POWER_VOTE_MASK;
	writel(reg_val, SOC_SCTRL_SCBAKDATA1_MSK_ADDR(host->sysctrl));
}

void ufshcd_clear_status_sync_info(struct ufs_hba *hba)
{
	unsigned long flags;
	int ret;
...
int ufshcd_init_status_sync_info(struct ufs_hba *hba)
{
	spin_lock_init(&hba->sync_lock);
	hba->hwlock = hwspin_lock_request_specific(UFS_HWLOCK_ID);
	if (!hba->hwlock) {
		pr_err("%s: request ufs hwspin lock fail\n", __func__);
		return -EINVAL;
	}
