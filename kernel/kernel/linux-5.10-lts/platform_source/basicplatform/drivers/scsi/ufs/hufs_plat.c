...
static void ufs_restore_key_upload_bigdata(struct work_struct *work)
{
	struct ufs_hba *hba = container_of(work, struct ufs_hba, ufs_restore_key_worker.work);
	...
	stat_data.hufs_err_code = hba->restore_key_error_code;
...
#ifdef CONFIG_SCSI_UFS_ENHANCED_INLINE_CRYPTO_V3
#ifdef CONFIG_SCSI_UFS_ENHANCED_INLINE_CRYPTO_KM
#ifndef CONFIG_DFA_MCQ
static void ufshcd_fbex_gen_sk(struct work_struct *work)
#else
void ufshcd_fbex_gen_sk(struct work_struct *work)
#endif
{
	int err = 0;
	int stat_rpmb = 0;
...
static void hufs_read_scbakdata(void __iomem * sctrl_addr)
{
	...
	do {
		...
		if (reg_value & BIT_SCTRL_SCBAKDATA1_MSPC_READY)
			return;
		mdelay(2);
		if ((timeout--) == 0) {
			timeout = 100;
	...
	}while(!(reg_value & BIT_SCTRL_SCBAKDATA1_MSPC_READY));
}
...
int hufs_genkey_and_restorekey(struct ufs_hba *hba)
{
	...
	if (ufshcd_eh_in_progress(hba)) { /* ufs reset */
		...
		gufs_sk_state = UFSHCD_SK_STATE_AVABLE;
#ifdef CONFIG_LOG_EXCEPTION
		hba->restore_key_error_code = err;
		schedule_delayed_work(&hba->ufs_restore_key_worker, 0);
#endif
		if (err)
			BUG();
	} else { /* ufs init */
...
{
	if (host->device_4_0 && (host->caps & USE_HS_GEAR5)) {
		hufs_cap->hs_rx_gear = UFS_HS_G5;
		hufs_cap->hs_tx_gear = UFS_HS_G5;
		hufs_cap->desired_working_mode = FAST;
	} else if (host->caps & USE_HS_GEAR4) {
	...
	struct device_node *np, struct hufs_host *host)
{
	...
	struct st_caps_map caps_arry[] = {
		...
		{ "hufs-use-HS-GEAR5", USE_HS_GEAR5 },
	...
	struct hufs_host *host, struct device_node *np)
{
...
#ifdef CONFIG_DFA_MCQ
	if (is_dfa_enabled(host->hba))
		host->hba->caps &= (u32)(~UFSHCD_CAP_AUTO_HIBERN8);
#endif
	...
	if (of_find_property(np, "hufs_standard_uic", NULL)) {
		...
		dev_err(host->hba->dev, "hufs use standard unipro command\n");
	} else {
	...
	dev_err(host->hba->dev, "hufs use hostid:%d\n", host->host_id);
}
...
#if defined(CONFIG_SCSI_UFS_LOW_POWER_VOTE) || defined(CONFIG_UFS_LOW_POWER_VOTE_STATUS_SYNC)
#define UFS_CCBUS_MASK ((0x1UL) << 31)
#define UFS_CCBUS_OFFSET 16
#define DFA_MEM_VOTE_OFFSET 0x3
#define DFA_MEM_VOTE_MASK (0x7 << 19)

void ufs_vddc_mem_volt_vote(struct ufs_hba *hba, bool idle)
{
	u32 val;
	int retry;
	struct hufs_host *host = (struct hufs_host *)hba->priv;
	/* 1. set vote ip profile */
	if (idle)
		writel((PERI_MEM_VOL_LEVEL0 << DFA_MEM_VOTE_OFFSET) | DFA_MEM_VOTE_MASK,
			SOC_PMCTRL_VDDC_MEM_VOTE_VOL_RANK_SFT_1_ADDR(host->pmc_reg));
	else
		writel((PERI_MEM_VOL_LEVEL2 << DFA_MEM_VOTE_OFFSET) | DFA_MEM_VOTE_MASK,
			SOC_PMCTRL_VDDC_MEM_VOTE_VOL_RANK_SFT_1_ADDR(host->pmc_reg));

	/* 2. set vote ip valid with highbit(16) mask */
	writel(BIT(PERI_MEM_DFA) | BIT(PERI_MEM_DFA + 16),
		SOC_PMCTRL_VDDC_MEM_VOTE_VOL_VALID_ADDR(host->pmc_reg));

	/* 3. issue volt request */
	writel(BIT(PERI_MEM_DFA),
		SOC_PMCTRL_VDDC_MEM_VOTE_SFT_CHG_ADDR(host->pmc_reg));

	/* 4. wait atmost 50us and clear volt request status */
	retry = 50;
	while(retry-- > 0) {
		val = readl(SOC_PMCTRL_VDDC_MEM_VOL_STABLE_ADDR(host->pmc_reg));
		if ((val & BIT(PERI_MEM_DFA)) != 0) {
			writel(BIT(PERI_MEM_DFA), SOC_PMCTRL_VDDC_MEM_VOL_SFT_CLR_ADDR(host->pmc_reg));
			break;
		}
		udelay(1);
	}

	/* 5. clear vote ip valid with highbit(16) mask */
	writel(((~BIT(PERI_MEM_DFA)) & 0xFF) | BIT(PERI_MEM_DFA + 16),
		SOC_PMCTRL_VDDC_MEM_VOTE_VOL_VALID_ADDR(host->pmc_reg));

	if (retry <= 0)
		dev_err(hba->dev, "%s, [DFA] wait volt request %u timeout\n", __func__, idle);
}

inline void ufs_dfa_ccbus_vote(struct ufs_hba *hba, bool idle)
{
	struct hufs_host *host = (struct hufs_host *)hba->priv;
	u32 val;

	if (hba->ufs_dfa_ccbus_voted == idle)
		return;

	hba->ufs_dfa_ccbus_voted = idle;
	/* 0->151M, 1->208 2->334 3->557 */
	val = idle ? 0x00 : (is_dfa_enabled(hba) ? 0x03 : 0x02);
	writel((val << UFS_CCBUS_OFFSET) | UFS_CCBUS_MASK, SOC_PMCTRL_FREQ_VOTE_G0_ADDR(host->pmc_reg));
}

void ufs_mphy_clk_vote(struct ufs_hba *hba, bool idle)
{
	struct hufs_host *host = (struct hufs_host *)hba->priv;

	/* ufs idle close mphy clk, busy open mphy clk */
	if (host->host_id == 1) {
		if (idle)
			writel(MPHY_SYS_CLK_EN, host->hsdt1_crg + HSDT1_CRG_PERDIS1_OFFSET);
		else
			writel(MPHY_SYS_CLK_EN, host->hsdt1_crg + HSDT1_CRG_PEREN1_OFFSET);
	} else {
		if (idle)
...
void ufs_low_power_vote(struct ufs_hba *hba, bool idle)
{
	ufs_vddc_mem_volt_vote(hba, idle);
	ufs_mphy_clk_vote(hba, idle);
}
#endif
...
#endif
#if defined(CONFIG_SCSI_UFS_ENHANCED_INLINE_CRYPTO) &&                         \
	defined(CONFIG_SCSI_UFS_ENHANCED_INLINE_CRYPTO_V3) &&                      \
	defined(CONFIG_LOG_EXCEPTION)
	INIT_DELAYED_WORK(&hba->ufs_restore_key_worker, ufs_restore_key_upload_bigdata);
#endif
	goto out;
...
#ifdef CONFIG_DFA_MCQ
	if (is_dfa_enabled(hba)) {
		dfa_irq_cpuhp_remove(hba);
		return;
	}
#endif
#if defined(CONFIG_UFS_4_0_HC) && !defined(CONFIG_MCQ_OHOS)
	ufsmcq_irq_cpuhp_remove(hba);
#endif
}
...
#if defined(CONFIG_SCSI_UFS_PICTOR) || defined(CONFIG_SCSI_UFS_AURIGA)
int ufs_fpga_spi_driver_probe(struct ufs_hba *hba)
{
	if (hba && hba->vops && hba->vops->fpga_spi_driver_probe)
		return hba->vops->fpga_spi_driver_probe(hba);
...
#if defined(CONFIG_SCSI_UFS_PICTOR) || defined(CONFIG_SCSI_UFS_AURIGA)
	.fpga_spi_driver_probe = hufs_fpga_spi_driver_probe,
#endif
};
