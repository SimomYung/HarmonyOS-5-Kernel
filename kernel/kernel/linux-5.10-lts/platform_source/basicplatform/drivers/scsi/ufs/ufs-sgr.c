...
static void hufs_rg_pll_config(struct ufs_hba *hba)
{
#ifdef CONFIG_DFX_DEBUG_FS
	dev_err(hba->dev, "%s:config 1865 rg_pll\n", __func__);
#endif
	/* this para affect ufs signal quality by chip hard test */
	...
	hufs_set_each_cfg_attr(hba, cfg, ARRAY_SIZE(cfg));
}
...
void v300_gears_config(struct ufs_hba *hba, u32 gear_rx)
{
	...
	if (strstarts(hba->model, UFS_MODEL_THOR950) || strstarts(hba->model, UFS_MODEL_THOR850) ||
		strstarts(hba->model, UFS_MODEL_THOR955))
		hufs_rg_pll_config(hba);
}
