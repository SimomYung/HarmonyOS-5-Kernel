	...
	struct ufs_read_buffer_vcmd *vcmd)
{
	...
	int data_direction = DMA_FROM_DEVICE;
	unsigned char cmd[R_BUFFER_CDB_LEN] = { 0 };

	cmd[READ_BUFFER_OPCODE_OFFSET] = vcmd->opcode;
	cmd[READ_BUFFER_MODE_OFFSET] = READ_BUFFER_MODE;
	cmd[READ_BUFFER_BUFFER_ID_OFFSET] = vcmd->buffer_id;
	for (i = 0; i < R_BUFFER_1ST_RESERVED_LEN; ++i)
		cmd[READ_BUFFER_1ST_RESERVED_OFFSET + i] = vcmd->reserved_1st[i];
	put_unaligned_be16(vcmd->buffer_len, cmd + READ_BUFFER_LEN_OFFSET);
	for (i = 0; i < R_BUFFER_2ND_RESERVED_LEN; ++i)
		cmd[READ_BUFFER_2ND_RESERVED_OFFSET + i] = vcmd->reserved_2nd[i];

	for (i = 0; i < vcmd->retries; ++i) {
		ret = scsi_execute(dev, cmd, data_direction,
			vcmd->buffer, vcmd->buffer_len, NULL, &sshdr, VCMD_REQ_TIMEOUT,
			vcmd->retries, 0, RQF_PM, NULL);
		if (!ret)
			break;
	}
	return ret;
