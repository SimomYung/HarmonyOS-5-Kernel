...
void ufshcd_hufs_mq_init(struct Scsi_Host *host)
{
	...
	if (host->queue_quirk_flag &
		 ...
		 * 15: Dev cmd
		 */
		host->mq_queue_depth = 14;
		host->mq_reserved_queue_depth = 3;
		host->mq_high_prio_queue_depth = 3;
#else
		/* Tag dispatch:
		 * 0 - 127: Sync
		 * 128 - 189: Async
		 ...
		 * 255: Dev cmd
		 */
		host->mq_queue_depth = 254;
		host->mq_reserved_queue_depth = 62;
...
#ifdef CONFIG_UFS_4_0_HC
#define UFS_TRP_SQ_QOS_DFX1 0x5000
#define UFS_TRP_SQ_QOS_DFX4 0x500C
#define UFS_TRP_SQ_QOS_DFX1_MASK 0xFFFFFFFF
#define UFS_TRP_SQ_NUM_OUT_MASK 0xFF
static int ufshcd_mcq_sync_cache_irq_safe(struct ufs_hba *hba,
					struct scsi_cmnd *cmd,	unsigned int query_intr_timeout)
{
	...
	dev_err(hba->dev, "%s start inflt_reqs: %d\n", __func__, atomic_read(&hba->mcq_inflt_reqs));
	pm_runtime_get_sync(hba->dev);
	spin_lock_irqsave(hba->host->host_lock, flags);
	spin_unlock_irqrestore(hba->host->host_lock, flags);

	/* wait mcq io all complete */
	retries = 1000;
	while (retries) {
		if ((ufshcd_readl(hba, UFS_TRP_SQ_QOS_DFX1) == UFS_TRP_SQ_QOS_DFX1_MASK) &&
			((ufshcd_readl(hba, UFS_TRP_SQ_QOS_DFX4) & UFS_TRP_SQ_NUM_OUT_MASK) == 0))
			break;
		retries--;
		udelay(10);
	}
	if (retries <= 0) {
		dev_err(hba->dev, "mcq clear timeout dfx1:0x%x dfx4: %x\n",
				ufshcd_readl(hba, UFS_TRP_SQ_QOS_DFX1), ufshcd_readl(hba, UFS_TRP_SQ_QOS_DFX4));
		for (i = 0; i < MCQ_OUTSTANDING_ITEMS; i++)
			dev_err(hba->dev, "hba->mcq_outstanding_reqs: idx[%d]-[%llx]\n", i, hba->mcq_outstanding_reqs[i]);
	}

	if (readl(hba->uhq[SYNCCACHE_QUEUE].mcq_base_cqis + REG_CQIS)) {
		dev_err(hba->dev, "mcq hw synccache queue err intr\n");
		writel(UFSHCD_MCQ_CQIS_TEPS, hba->uhq[SYNCCACHE_QUEUE].mcq_base_cqis + REG_CQIS);
	}

	tag = SYNCCACHE_CMD_TAG;
	lrbp = &hba->lrb[tag];
	ufshcd_sync_cache_setup_lrbp(lrbp, cmd, tag);
	lrbp->intr_cmd = true;
	if (ufshcd_mcq_config_utrdlp(hba, tag, lrbp))
		goto unlock;
	if (ufshcd_compose_upiu(hba, lrbp))
		goto unlock;
	writel(0, hba->uhq[SYNCCACHE_QUEUE].mcq_base_cqis + REG_CQIE);

	ufshcd_mcq_send_queue(hba, tag, lrbp);
	/* Make sure descriptors are ready before ringing the doorbell */
	wmb();

	hwq = (struct ufs_hw_queue *)(lrbp->hwq);
	while (--query_intr_timeout > 0) {
		events = readl(hwq->mcq_base_cqis + REG_CQIS);
		if (events & UFSHCD_MCQ_CQIS_TEPS) {
			spin_lock(&(hwq->cq_lock));
			writel(events, hwq->mcq_base_cqis + REG_CQIS);
	...
	if (query_intr_timeout == 0)
		dev_err(hba->dev, "sync cache queue wait int timeout IE:%x CQHP:%x CQTP:%x\n", readl(hwq->mcq_base_cqis + REG_CQIE),
...
out:
	...
	dev_err(hba->dev, "%s end  inflt_reqs: %d\n", __func__, atomic_read(&hba->mcq_inflt_reqs));
...
static int ufshcd_send_scsi_sync_cache_on_specified_disk(struct ufs_hba *hba, struct scsi_device *sdp)
{
	...
	if (is_mcq_enabled(hba))
		ret = ufshcd_mcq_sync_cache_irq_safe(hba, &cmnd, (unsigned int)100000);
	else
#endif
...
int ufshcd_direct_flush(struct scsi_device *sdev)
{
	...
	if (is_dfa_enabled(hba))
		return dfa_direct_flush(sdev, hba, sync_cache_buffer);
#endif
...
#ifdef CONFIG_DFA_MCQ
	if (is_dfa_enabled(hba))
		return dfa_dump_status(hba, dump_type);
#endif
...
#define DFA_ISP_REV_OFFSET     4104
static void ufshcd_set_dfa_rev(char *dfa_rev, u8 *fbuf)
{
    dfa_rev[0] = fbuf[DFA_ISP_REV_OFFSET + 3];
    dfa_rev[1] = fbuf[DFA_ISP_REV_OFFSET + 2];
    dfa_rev[2] = fbuf[DFA_ISP_REV_OFFSET + 1];
    dfa_rev[3] = fbuf[DFA_ISP_REV_OFFSET];
    dfa_rev[4] = '\0';
}

#define DFA_REV_LENGTH 17
static void ufshcd_device_get_dfa_rev(const struct device *dev, char *dfa_rev)
{
	int ret;
	u16 fsr_buf_len = THR950_DFA_FSR_INFO_SIZE;
	u8 *fbuf = NULL;
	struct ufs_hba *hba = NULL;
	...
	if (!is_dfa_enabled(hba)) {
		if (snprintf_s(dfa_rev, DFA_REV_LENGTH, DFA_REV_LENGTH - 1, "%s", "Not Dfa") < 0) {
			dev_err(hba->dev, "%s: snprintf_s Not Dfa error.\n", __func__);
		}
		return;
	}
	...
	if (ret) {
		if (snprintf_s(dfa_rev, DFA_REV_LENGTH, DFA_REV_LENGTH - 1, "%s", "Failed") < 0) {
			dev_err(hba->dev, "%s: snprintf_s Failed error.\n", __func__);
		}
		dev_err(hba->dev, "%s: ufshcd_read_buffer_vcmd_retry fail %d\n", __func__, ret);
	} else {
		ufshcd_set_dfa_rev(dfa_rev, fbuf);
	}
...
/**
 * ufshcd_high_temp_config - config TOO_HIGH_TEMP enable for VELA63
 */
void ufshcd_high_temp_config(struct ufs_hba *hba)
{
	...
	if ((strstarts(hba->model, UFS_MODEL_THOR935) && (hba->dev_info.fw_version % 100 >= HIGH_TEMP_SUPPORT_FW_REV)) ||
		...
		(hba->ufs_device_spec_version == UFS_DEVICE_SPEC_4_0)) {
...
void ufs_dev_too_high_temp_event_handler(struct ufs_hba *hba)
{
	...
	if (hba->ufs_device_spec_version == UFS_DEVICE_SPEC_4_0) {
		ret = ufshcd_query_attr(hba, UPIU_QUERY_OPCODE_READ_ATTR,
				QUERY_ATTR_IDN_DEVICE_ROUGH_TEMP, 0, 0, &attr);
		dev_err(hba->dev, "%s:ufs device temperature[%d], ret: %d\n", __func__, (attr & 0xff) - 80, ret);
		ret = ufshcd_disable_ee(hba, MASK_EE_TOO_HIGH_TEMP);
		if (ret)
			dev_err(hba->dev, "%s:enable high temp ee fail:%d\n", __func__, ret);
	} else {
...
#ifdef CONFIG_DFA_MCQ
		set_bootdevice_dfa_rev_handler(ufshcd_device_get_dfa_rev);
#endif
...
void ufshcd_device_not_support_autoh8(struct ufs_hba *hba)
{
	...
	if((hba->manufacturer_id == UFS_VENDOR_SANDISK) &&
		(hba->ufs_device_spec_version == UFS_DEVICE_SPEC_2_1)) {
		hba->caps &= (u32)(~UFSHCD_CAP_AUTO_HIBERN8);
		dev_err(hba->dev, "disable autoH8\n");
	}
#endif
	if ((hba->manufacturer_id == UFS_VENDOR_VELA61) &&
		(strstarts(hba->model, UFS_MODEL_SS6100))) {
...
void ufshcd_config_adapt(struct ufs_hba *hba, struct ufs_pa_layer_attr *pwr_mode)
{
	...
	if ((hba->manufacturer_id == UFS_VENDOR_TOSHIBA) ||
		...
		(strstarts(hba->model, UFS_MODEL_THOR950))) {
...
#ifdef CONFIG_UFS_4_0_HC
	if (is_mcq_enabled(hba))
		return __ufsmcq_wait_for_reqs_clr(hba);
#endif
...
enum blk_eh_timer_return ufshcd_eh_timed_out(struct scsi_cmnd *scmd)
{
	...
	for_each_set_bit(index, ufshcd_outstanding_reqs(hba), hba->nutrs) {
...
int __ufshcd_uic_hibern8_op_irq_safe(struct ufs_hba *hba, bool h8_op)
{
	...
	ufshcd_writel(hba, 0, REG_INTERRUPT_ENABLE);
#ifdef CONFIG_UFS_4_0_HC
	vs_ie_value = ufshcd_readl(hba, UFS_VS_IE);
	ufshcd_writel(hba, 0, UFS_VS_IE);
#endif
	...
	if (ret) {
		...
		if ((mask == UIC_HIBERNATE_ENTER) && ((value & MASK_DEBUG_UNIPRO_STATE) == LINK_HIBERN)) {
			dev_err(hba->dev, "enter h8 uhes timeout,but h8 enter sucess\n");
			ret = 0;
		} else if ((mask == UIC_HIBERNATE_EXIT) && ((value & MASK_DEBUG_UNIPRO_STATE) == LINK_UP)){
			dev_err(hba->dev, "exit h8 uhes timeout,but h8 exit sucess\n");
			ret = 0;
		}
	}
#endif
...
out:
	...
	ufshcd_writel(hba, ie_value, REG_INTERRUPT_ENABLE);
#ifdef CONFIG_UFS_4_0_HC
	ufshcd_writel(hba, vs_ie_value, UFS_VS_IE);
#endif
