...
static int get_board_id_from_dts(void)
{
	...
	ret = of_property_read_u32_array(root, "hisi,boardid", board_id, BOARD_ID_SIZE);
	if (ret < 0) {
		pr_err("Failed : of_property_read_u32 %d\n", ret);
		return 0;
	}
	...
	for (i = 0; i < BOARD_ID_SIZE; i++)
		result = result * 10 + board_id[i];
	pr_err("%s: result is %d\n", __func__, result);

	return result;
}

#define UFS_SKHYNIX_SPECIAL_NAME         "HN8T274EJKX130"
#define UFS_SKHYNIX_SPECIAL_NAME_LEN     (14)
/* Note: this function is for N platform PI+V4C2(boardid 21495) and ICEV4A1(boardid 21405) */
static bool is_special_skhynix_product(struct ufs_hba *hba)
{
	int boardid = 0;

	if (strncmp(hba->model, UFS_SKHYNIX_SPECIAL_NAME, UFS_SKHYNIX_SPECIAL_NAME_LEN) != 0) {
		return false;
	}
	boardid = get_board_id_from_dts();
	/* note: use old cid on N platform PI+V4C2(boardid 21495) and ICEV4A1(boardid 21405). */
	if (boardid == 21495 || boardid == 21405)
		return true;
	else
		return false;
}

static void ufs_get_sec_unique_number_skhynix(struct ufs_hba *hba,
				const uint8_t *desc_buf, uint8_t *snum_buf)
{
	int i;

	if (hba->ufs_device_spec_version < 0x400 || is_special_skhynix_product(hba)) {
		/* hynix only have 6Byte, add a 0x00 before every byte */
		for (i = 0; i < 6; i++) {
			snum_buf[i * 2] = 0x0;
			snum_buf[i * 2 + 1] = desc_buf[QUERY_DESC_HDR_SIZE + i];
		}
	} else if (hba->ufs_device_spec_version == 0x400) {
		for (i = 0; i < SERIAL_NUM_SIZE; i++) {
			snum_buf[i] = desc_buf[QUERY_DESC_HDR_SIZE + 2 * i + 1];
		}
	}
}
				      ...
				      const char *product_name)
{
	...
	case UFS_VENDOR_SKHYNIX:
		ufs_get_sec_unique_number_skhynix(hba, desc_buf, snum_buf);
		break;
...
static void ufs_scramble_config(struct ufs_hba *hba)
{
	if (strstarts(hba->model, UFS_MODEL_THOR935) || strstarts(hba->model, UFS_MODEL_THOR850) ||
		strstarts(hba->model, UFS_MODEL_THOR610) || strstarts(hba->model, UFS_MODEL_THOR950) ||
		strstarts(hba->model, UFS_MODEL_THOR955))
		hba->scramble = true;
}
...
u64 read_utr_doorbell(struct ufs_hba *hba)
{
	...
	if (is_dfa_enabled(hba))
		return atomic_read(&hba->dhba.dfa_inflts);
#endif
...
static int ufshcd_ioctl_query(struct ufs_hba *hba, u8 lun, void __user *buffer)
{
	...
	spin_lock_irqsave(hba->host->host_lock, flags);
	/* if error handling is in progress, don't issue query */
	if (hba->ufshcd_state == UFSHCD_STATE_EH_SCHEDULED ||
		hba->ufshcd_state == UFSHCD_STATE_RESET ||
		hba->force_reset) {
		spin_unlock_irqrestore(hba->host->host_lock, flags);
		dev_err(hba->dev, "%s: error handling is in progress, don't issue query\n", __func__);
		return -ENOMEM;
	}
	spin_unlock_irqrestore(hba->host->host_lock, flags);
