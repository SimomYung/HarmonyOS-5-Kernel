...
int ufsmcqtt_do_scsi_cmd(struct ufs_hba *hba, struct scsi_cmnd *cmd, unsigned int timeout, ktime_t start_time)
{
	int err;
	int tag;
	...
	time_left = wait_for_completion_timeout(hba->dev_cmd.complete, msecs_to_jiffies(timeout));
	...
	struct ufs_hba *hba, struct scsi_cmnd *cmd, unsigned int timeout, ktime_t start_time)
{
	...
	if (is_dfa_enabled(hba))
		return dfa_tt_do_scsi_cmd(hba, cmd, timeout, start_time);
#endif

#ifdef CONFIG_UFS_4_0_HC
	if (is_mcq_enabled(hba))
		return ufsmcqtt_do_scsi_cmd(hba, cmd, timeout, start_time);
#endif
