...
int mm_sec_cma_reserve(struct reserved_mem *rmem)
{
	...
	if (!strcmp(cma_name, "mm-cma-pool")) {
		...
		mm_cma = cma;
		dma_pool_heap_register_dma_camera_cma((void *)cma);
	}

	return 0;
}
RESERVEDMEM_OF_DECLARE(mm_cma_pool,
		       "mm-cma-pool",
		       mm_sec_cma_reserve);
...
RESERVEDMEM_OF_DECLARE(mm_smemheap_cma,
		       "smemheap-cma",
		       mm_sec_cma_reserve);
RESERVEDMEM_OF_DECLARE(mm_drm_redraw_cma,
		       "mm-drm-redraw-cma-pool",
		       mm_sec_cma_reserve);
					    ...
					    unsigned long align)
{
	...
	dma_ci_cache_by_pa(page_to_phys(page), size, DMA_FROM_DEVICE);
#else
...
void sec_alloc_init(struct device_node *np)
{
	u32 alloc_prot = 0;
	u32 i;
	int ret;

	ret = of_property_read_u32(np, "sec_alloc_sc_prot",
				   &alloc_prot);
	if (ret) {
