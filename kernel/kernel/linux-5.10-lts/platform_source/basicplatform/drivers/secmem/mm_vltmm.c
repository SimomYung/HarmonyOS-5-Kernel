 ...
 *
 */
#define pr_fmt(fmt) "[secmem-vltmm]" fmt
...
struct dma_heap_sec_tee g_smem_tee = {0};
...
unsigned long smem_minfree[SMEM_MAX_ADJ] = {
	SMEM_MINFREE_MIN,
	SMEM_MINFREE_LOW,
	SMEM_MINFREE_HIGH,
};
...
static void smem_send_reclaim_msg(void)
{
	...
	ret = secmem_tee_exec_cmd(session, &mcl, DMA_HEAP_SEC_CMD_VLTMM);
...
static struct kobject *smem_kobject;

void smem_sysfs_init(void)
{
	int ret;

	smem_kobject = kobject_create_and_add("smem_vltmm", kernel_kobj);
	if (!smem_kobject)
		return;

	ret = sysfs_create_group(smem_kobject, &smem_pool_attr_group);
	if (ret) {
		kobject_put(smem_kobject);
...
static int smem_ta_session_init(void)
{
	...
	if (!ta_init) {
		ret = sec_tee_init(&g_smem_tee, DMA_HEAP_SESSIONS_VLTMM);
		if (ret) {
			pr_err("TA session init failed\n");
			return -1;
		}
		context = g_smem_tee.context;
		session = g_smem_tee.session;
		ta_init = 1;
	}
