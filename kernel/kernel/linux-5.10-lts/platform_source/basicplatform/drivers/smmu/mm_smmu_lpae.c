		...
		struct device_node *np, struct mm_dom_cookie *cookie)
{
	...
	cookie->vm_iova_pool =
		iova_pool_setup(cookie->vm_iova.iova_start, cookie->vm_iova.iova_size,
				cookie->vm_iova.iova_align, cookie->vm_iova.iova_seg_grain);
...
int of_get_iova_info(struct device_node *np, const char *name,
		     ...
		     unsigned long *iova_align, unsigned long *iova_seg_grain)
{
	...
	if (ret)
		*iova_align = SZ_256K;
	ret = of_property_read_u64(node, "iova-segment-granularity", (u64 *)iova_seg_grain);
	if (ret)
		pr_err("iova_seg_grain is no needed\n");
 
	pr_err("%s:start_addr 0x%lx, size 0x%lx align 0x%lx granularity 0x%lx\n", __func__,
	       *iova_start, *iova_size, *iova_align, *iova_seg_grain);
...
static int mm_attach_dev_lpae(struct iommu_domain *domain, struct device *dev)
{
	...
	cookie->iova_pool =
		iova_pool_setup(cookie->iova.iova_start, cookie->iova.iova_size,
				cookie->iova.iova_align, cookie->iova.iova_seg_grain);
	if (!cookie->iova_pool) {
