...
static int configure_dma(struct hs_spi_controller *spi_control)
{
	...
	if ((spi_control->sel_dma) && (spi_control->actrlbase != NULL))
		writel(SPI_SET_PERI_DMA, spi_control->actrlbase + SPI_DMA_SEL_OFFSET);
...
static irqreturn_t hs_spi_interrupt_handler(int irq, void *dev_id)
{
	...
	if (irq_status) {
		complete(&spi_control->dma_complete);
...
static void do_exclusive_dma_transfer(struct spi_master *master)
{
	...
	reinit_completion(&spi_control->dma_complete);
	...
	ret = wait_for_completion_timeout(&spi_control->dma_complete,
		WAIT_FOR_COMPLETION);
	if (ret == 0) {
		dev_err(&spi_control->pdev->dev,
			"exclusive_dma timeout, cur_transfer len %u, irq stat 0x%x, trans stat 0x%x\n",
			spi_control->cur_transfer->len,
			readl(INTR_SPI_DMA(spi_control->virtbase)),
			readl(SPI_DMA_TRANS_STAT(spi_control->virtbase)));
	};
					...
					struct spi_message *msg)
{
	...
	if (!spi_control->hardware_locked) {
		dev_err(&spi_control->pdev->dev, "spi hardware not ready\n");
		return -EIO;
	}
...
#ifndef CONFIG_SPI_V500_VM
	if (spi_control->spi_sleep_use_doze)
		vote_disable_doze(spi_control->doze_id);
#endif
	ret = hs_spi_runtime_resume(master->dev.parent);
	spi_control->hardware_locked = true;
...
static int hs_spi_unprepare_transfer_hardware(struct spi_master *master)
{
	...
	spi_control->hardware_locked = false;
#ifndef CONFIG_SPI_V500_VM
	if (spi_control->spi_sleep_use_doze)
		vote_enable_doze(spi_control->doze_id);
#endif
	return 0;
}
...
hs_spi_platform_data_dt_get(struct device *dev)
{
	...
	if (of_property_read_u32(np, "enable-dma", &tmp)) {
		dev_info(dev, "no enable-dma defined\n");
		pd->enable_dma = 1;
	} else {
		pd->enable_dma = tmp;
		dev_info(dev, "enable-dma %d\n", pd->enable_dma);
	}
...
static void hs_spi_sel_dma(struct platform_device *pdev,
			struct hs_spi_controller *spi_ctrl)
{
	struct device_node *np = pdev->dev.of_node;
	struct device_node *actrl_np = NULL;

	if (of_property_read_u32(np, "spi-sel-dma", &spi_ctrl->sel_dma)) {
		dev_err(&pdev->dev, "no need sel dma\n");
		spi_ctrl->sel_dma = 0;
		return;
	}

	actrl_np = of_find_compatible_node(NULL, NULL, "hisilicon,actrl");
	if (actrl_np == NULL) {
		dev_err(&pdev->dev, "fail to find Actrl node\n");
		spi_ctrl->sel_dma = 0;
		return;
	}

	spi_ctrl->actrlbase = of_iomap(actrl_np, 0);
	if (spi_ctrl->actrlbase == NULL) {
		dev_err(&pdev->dev, "failed to iomap actrl\n");
		spi_ctrl->sel_dma = 0;
		return;
	}
}

static void hs_spi_get_doze_id(struct platform_device *pdev,
	struct hs_spi_controller *spi_control)
{
	if (of_property_read_u32(pdev->dev.of_node, "use-doze-to-sleep",
		&spi_control->spi_sleep_use_doze))
		spi_control->spi_sleep_use_doze = 0;

	if (spi_control->spi_sleep_use_doze) {
		if (of_property_read_u32(pdev->dev.of_node, "doze-id",
			&spi_control->doze_id)) {
			pr_err("%s: spi doze id get err\n", __func__);
			spi_control->spi_sleep_use_doze = 0;
		}
	}
}
			...
			u64 *clk_rate)
{
	...
	if (of_property_read_u32(np, "pinctrl-not-select",
		&spi_control->pinctrl_not_select))
		spi_control->pinctrl_not_select = 0;
	hs_spi_parse_iodio_cfg(pdev, spi_control);
	hs_spi_get_doze_id(pdev, spi_control);
			...
			struct hs_spi_controller *spi_control)
{
	...
	init_completion(&spi_control->dma_complete);
	...
	if (platform_data->enable_dma == 0) {
		dev_err(&pdev->dev, "no need dma\n");
		return status;
	}
#ifndef CONFIG_SPI_V500_VM
...
static int hs_spi_master_probe(struct platform_device *pdev)
{
	...
	if (status)
		dev_err(&pdev->dev, "probe - can not get pins data\n");
	...
	hs_spi_sel_dma(pdev, spi_control);
	...
	if (iores == NULL) {
		dev_err(dev, "get the resource failed\n");
		goto hwspin_lock_err0;
	}
...
err_no_ioremap:
	...
	if (spi_control->actrlbase != NULL) {
		iounmap(spi_control->actrlbase);
		spi_control->actrlbase = NULL;
	}
...
static int hs_spi_runtime_suspend(struct device *dev)
{
	...
	clk_disable_unprepare(spi_control->clk);
	if (spi_control->pinctrl_not_select == 0)
