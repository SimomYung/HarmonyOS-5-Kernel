...
static void uart_v500_dma_shutdown(struct uart_v500_port *uap)
{
	...
	if (count == TXFF_TIMEOUT)
		dev_err(uap->port.dev, "Tx busy timeout, uart stat 0x%x\n",
			uart_v500_read(uap, REG_UART_STAT));
...
struct uart_v500_port uap_uart0;
static int uart_v500_probe(struct platform_device *pdev)
{
	...
	uart_nu = uart_probe_dt_alias(uart_nu, &pdev->dev);
	if (uart_nu == 0)
		uap = &uap_uart0;
	else
		uap = devm_kzalloc(&pdev->dev, sizeof(struct uart_v500_port), GFP_KERNEL);
