...
static int usb_mask_int_for_asp(struct chip_usb_misc_ctrl *misc_ctrl)
{
	int ret = 0;

	if (misc_ctrl->asp_int_mask) {
		misc_ctrl_err("mask event_ring_1's int (second event ring) for asp\n");
		ret = chip_usb_reg_write(misc_ctrl->asp_int_mask);
		if (ret)
			misc_ctrl_err("mask int for asp failed\n");
	}

	return ret;
}

static int bp_mem_ds_cfg(struct chip_usb_misc_ctrl *misc_ctrl)
{
	int ret = 0;

	if (!misc_ctrl->bp_mem_ds_set || !misc_ctrl->bp_mem_ds_clr) {
		return ret;
	}

	misc_ctrl_info("config bp mem ds\n");
	ret = chip_usb_reg_write(misc_ctrl->bp_mem_ds_set);
	if (ret) {
		misc_ctrl_err("bp mem ds set failed\n");
		return ret;
	}

	udelay(50);

	ret = chip_usb_reg_write(misc_ctrl->bp_mem_ds_clr);
	if (ret) {
		misc_ctrl_err("bp mem ds clr failed\n");
		return ret;
	}
...
int multi_usb_misc_ctrl_init(struct fwnode_handle *fwnode)
{
	...
	ret = usb_mask_int_for_asp(usb_misc_ctrl);
	if (ret) {
		misc_ctrl_err("mask int for asp failed\n");
		goto enter_power_idle;
	}

	ret = bp_mem_ds_cfg(usb_misc_ctrl);
	if (ret) {
		misc_ctrl_err("bp_mem_ds_cfg failed\n");
		goto enter_power_idle;
	}
...
static int get_reg_cfgs(struct chip_usb_misc_ctrl *misc_ctrl)
{
	...
	get_reg_cfg(asp_int_mask);
	get_reg_cfg(bp_mem_ds_set);
	get_reg_cfg(bp_mem_ds_clr);
...
static void put_bp_mem_cfgs(struct chip_usb_misc_ctrl *misc_ctrl)
{
	if (misc_ctrl->bp_mem_ds_set)
		of_remove_chip_usb_reg_cfg(misc_ctrl->bp_mem_ds_set);
	if (misc_ctrl->bp_mem_ds_clr)
		of_remove_chip_usb_reg_cfg(misc_ctrl->bp_mem_ds_clr);
}
...
static void put_reg_cfgs(struct chip_usb_misc_ctrl *misc_ctrl)
{
	...
	if (misc_ctrl->asp_int_mask)
		of_remove_chip_usb_reg_cfg(misc_ctrl->asp_int_mask);

	put_bp_mem_cfgs(misc_ctrl);
}
