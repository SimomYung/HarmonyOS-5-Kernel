...
static int g_wireless_charger;
static int g_device_mode;
static int detect_config_device()
{
	int value = 0;

	g_device_mode = 1;
	if (g_wireless_charger < 0) {
		pr_err("%s: gpio is invalide, default usb device mode\n", __func__);
		return 0;
	}

	mdelay(50);
	value = gpio_get_value(g_wireless_charger);
	pr_err("%s:The usb connet is %s\n", __func__, value ? "wireless charger" : "device mode");
...
static int is_device_mode()
{
	if (g_wireless_charger < 0)
		return 1;

	pr_err("%s:The usb connet is %d\n", __func__, g_device_mode);
	if (g_device_mode)
		return 1;
...
static int get_wireless_charger_gpio(struct spmi_device *dev,
	struct spmi_resource *node, const char *name)
{
	...
	if (of_property_read_u32(node->of_node, name, &gpio)) {
		dev_err(&dev->dev, "%s read fail\n", __func__);
		return -1;
	}
...
static int hisi_usb_spmi_vbus_probe(struct spmi_device *pdev)
{
	...
	/* get the gpio of wireless charger state */
	g_wireless_charger = get_wireless_charger_gpio(pdev,
		&pdev->res, "wireless_charger");
...
static void hisi_usb_spmi_vbus_remove(struct spmi_device *pdev)
{
