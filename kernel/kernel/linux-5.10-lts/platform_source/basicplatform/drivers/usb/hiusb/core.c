...
#include <linux/platform_drivers/usb/tcp.h>
#endif
...
static bool g_usb_port_suspend = false;

bool hiusb_port_is_suspend_state(void)
{
	return g_usb_port_suspend;
}
...
enum usb_state  hiusb_get_state_of_default_port(void)
{
	int portid;
	struct hiusb_port *port = NULL;
...
EXPORT_SYMBOL_GPL(hiusb_get_state_of_default_port);
...
int hiusb_usb20_phy_calibrate(struct usb_device *udev)
{
	...
	pr_info("hiusb_usb20_phy_calibrate portid = %d calibrate complete !\n", portid);
	return 0;
}
...
int hiusb_host_start(struct hiusb_port *port)
{
	...
	state.prop.state.lane_num = port->usb_lane_num;
...
static int hiusb_prepare(struct device *dev)
{
	pr_err(" g_usb_port_suspend true\n");
	g_usb_port_suspend = true;
	return 0;
}

static void hiusb_complete(struct device *dev)
{
	pr_err(" g_usb_port_suspend false\n");
	g_usb_port_suspend = false;
}
...
#ifdef CONFIG_PM_SLEEP
	.prepare = hiusb_prepare,
	.complete = hiusb_complete,
