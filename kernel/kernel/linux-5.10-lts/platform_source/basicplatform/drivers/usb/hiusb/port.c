...
static int hiusb_port_unreset_phy_if_fpga(struct hiusb_port *port)
{
	int ret = 0;
	unsigned int gpio;

	if (port->fpga_xctrl_phy_unreset && port->fpga_xctrl_phy_reset) {
		ret = chip_usb_reg_write(port->fpga_xctrl_phy_reset);
		if (ret) {
			pr_err("fpga reset phy failed\n");
			return ret;
		}
		udelay(200);

		ret = chip_usb_reg_write(port->fpga_xctrl_phy_unreset);
		if (ret) {
			pr_err("fpga unreset phy failed\n");
			return ret;
		}
		udelay(200);

		pr_info("fpga unreset phy success\n");
		return ret;
	}
...
int hiusb_port_usb2_init(struct hiusb_port *port)
{
	...
	if (port->is_fpga_flag) {
		pr_info("fpga unreset u2 phy by reset_gpio or fpga_xctrl\n");
		return hiusb_port_unreset_phy_if_fpga(port);
	}
...
static void hiusb_port_get_resource_for_fpga(struct hiusb_port *port)
{
	...
	port->fpga_xctrl_phy_reset = of_get_chip_usb_reg_cfg(dev->of_node,
			"fpga_xctrl_phy_reset");
	if (port->fpga_xctrl_phy_reset) {
		port->fpga_xctrl_phy_unreset = of_get_chip_usb_reg_cfg(dev->of_node,
			"fpga_xctrl_phy_unreset");
		if (!port->fpga_xctrl_phy_unreset) {
			of_remove_chip_usb_reg_cfg(port->fpga_xctrl_phy_reset);
		}
	}
	if ((!port->fpga_xctrl_phy_unreset) || (!port->fpga_xctrl_phy_unreset)) {
		port->fpga_phy_reset_gpio = of_get_named_gpio(dev->of_node, "fpga_phy_reset_gpio", 0);
		if (port->fpga_phy_reset_gpio < 0)
			pr_err("error: fpga_xctrl and fpga_phy_reset_gpio not found, cann't unreset fgpa.\n");
		else
			pr_info("fpga_xctrl not found, will unreset fgpa by gpio.\n");
	}
...
static int hiusb_port_get_res_form_fwnode(struct hiusb_port *port)
{
	...
	port->usb_init_flag_switch = of_property_read_bool(port->dev.of_node, "usb_init_flag_switch");
	pr_info("usb_init_flag_switch %d\n", port->usb_init_flag_switch);
	...
	if (of_property_read_u32(port->dev.of_node, "usb_lane_num", &(port->usb_lane_num)))
		port->usb_lane_num = HIUSB_PROP_DP_2LANE;
	pr_info("usb_lane_num %d\n", port->usb_lane_num);
...
static void hiusb_port_put_res_form_fwnode(struct hiusb_port *port)
{
	...
	if (port->fpga_xctrl_phy_unreset) {
		of_remove_chip_usb_reg_cfg(port->fpga_xctrl_phy_unreset);
	}
...
static int hiusb_port_common_resume(struct device *dev, bool is_freeze)
{
	...
	if (port->usb_state == USB_STATE_DEVICE) {
		...
		if (ret) {
			...
			pr_info("hiusb_port_resume_process success and device enable tcp irq\n");
			tcp_set_port_irq_status(port->portid, true);
		}
#else
		if (ret)
			pr_err("hiusb_port_resume_process failed in device state\n");
		...
		if (ret) {
			...
			if (port->usb_state != USB_STATE_HIFI_USB) {
				tcp_set_port_irq_status(port->portid, true);
				pr_info("hiusb_port_resume_process success and host enable tcp irq\n");
			}
		}
#else
		if (ret)
