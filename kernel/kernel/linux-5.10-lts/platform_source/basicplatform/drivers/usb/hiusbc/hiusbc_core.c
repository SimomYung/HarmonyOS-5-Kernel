...
static void hiusbc_host_reg_init(struct hiusbc *hiusbc)
{
	...
	hiusbc_writel(HIUSBC_PPOFF_PHY_LP_DISABLE, hiusbc->u2_piu_regs,
		MPI_U2_PM_UIU_REG_CSR_HST_POWERED_OFF_PHY_LP_EN_OFFSET);
	dev_info(hiusbc->dev, "usb2 pm uiu suspend close when pp off\n");

	/* Config HS LPM_TIME to 100us(Minimum SI 125us) */
	reg = hiusbc_readl(hiusbc->host_regs, MPI_APP_HOST_REG_LPM_TIME_OFFSET);
	reg &= (~MPI_APP_HOST_REG_U2_LPM_ENTRY_TIME_MASK);
	reg |= (HIUSBC_U2_LPM_ENTRY_TIME << MPI_APP_HOST_REG_U2_LPM_ENTRY_TIME_SHIFT);
	hiusbc_writel(reg, hiusbc->host_regs, MPI_APP_HOST_REG_LPM_TIME_OFFSET);
	dev_info(hiusbc->dev, "usb2 lpm_entry_time 0x%X\n",
		hiusbc_readl(hiusbc->host_regs, MPI_APP_HOST_REG_LPM_TIME_OFFSET));
