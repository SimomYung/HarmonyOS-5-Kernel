			...
			const struct hiusbc_evt_trb *event, int status)
{
	...
	while (!list_empty(&ring->queue)) {
		if (!hep->ep.desc)
			return;
		hreq = list_first_entry_or_null(&ring->queue, struct hiusbc_req, queue);
	...
	if (usb_endpoint_xfer_control(hep->ep.desc) == 0)
		return;
				...
				const struct hiusbc_xfer_ring *ring, u32 code)
{
	...
	if (usb_endpoint_xfer_isoc(hep->ep.desc) == 0)
	...
	case HIUSBC_CMPLT_CODE_BABBLE_ERR:
		...
		if (usb_endpoint_xfer_control(hep->ep.desc))
			hiusbc_dbg(HIUSBC_DEBUG_ERR, "This is a %s ctrl_babble event!\n",
				hiusbc->babble_expected != 0 ? "expected" : "unexpected");
	...
	if (list_empty(&ring->queue) ||
			...
			hiusbc_xfer_trb_vrt_to_dma(
				ring->enqueue_seg, ring->enqueue) ==
					short_trb_addr) {
		dev_dbg(hiusbc->dev, "Can't find TRB on the ring! "
			"cmplt_code = %u, EP = %u.\n",
...
static void state_check_ctrl_xfer(struct hiusbc *hiusbc)
{
	...
	if ((hiusbc->next_stage != HIUSBC_SETUP_STAGE) ||
		(hiusbc->eps[0]->stalled != 0) || (hiusbc->eps[1]->stalled != 0)) {
		/*
		...
		struct hiusbc_ep *hep, const struct hiusbc_evt_trb *event)
{
	...
	hiusbc->ctrl_num = (u8)hiusbc_get_trb_ctrl_num(event->info);
	hiusbc->babble_expected = false;
			...
			const struct hiusbc_evt_trb *event)
{
	...
	} else if (hiusbc->need_data_stage == 0) {
			...
			struct hiusbc *hiusbc, struct hiusbc_ep *hep)
{
	...
	ring = hep->ring;
	if (list_empty(&ring->queue) &&
		usb_endpoint_xfer_isoc(hep->ep.desc) != 0) {
		cur_mf = hiusbc_get_frame_id(hiusbc);
		start_mf = (u32)cur_mf + HIUSBC_DEV_IST;
...
static void set_bulk_u2_lpm_mode(struct hiusbc *hiusbc)
{
	u32 reg;
	reg = hiusbc_readl(hiusbc->dev_regs,
		MPI_APP_DEVICE_REG_TRPU_DEV_USB_CTRL_OFFSET);
	if (hiusbc->dis_gadget_u2_lpm_bulk_always_idle) {
		reg |= MPI_APP_DEVICE_REG_TRPU_DEV_U2_LP_MOD_MASK;
	} else {
...
static void hiusbc_dev_event_connect(struct hiusbc *hiusbc)
{
	 ...
	 */
	if ((old_speed != USB_SPEED_UNKNOWN) && (hiusbc->last_is_connect_event == true))
		hiusbc_dbg(HIUSBC_DEBUG_ERR, "two connect events come together "
			"with different speed.\n");

	hiusbc_dbg(HIUSBC_DEBUG_ERR, "Connected at speed = %d\n", speed);
...
static int hiusbc_wakeup_active_bulk_eps(struct hiusbc *hiusbc)
{
	...
	for (epnum = 2; epnum < HIUSBC_EP_NUMS; epnum++) {
		hep = hiusbc->eps[epnum];
		if ((hep->enabled != 0) && (hep->flow_control != 0) &&
			!list_empty(&hep->ring->queue) &&
			(usb_endpoint_xfer_bulk(hep->ep.desc) != 0)) {
			hiusbc_dbg(HIUSBC_DEBUG_XFER,
...
{
	hiusbc_dbg(HIUSBC_DEBUG_ERR, "+\n");
	hiusb_atomic_notifier_call_chain(DEVICE_EVENT_SUSPEND,
		(void *)&hiusbc->gadget.state);
	if (hiusbc->gadget.state >= USB_STATE_CONFIGURED)
		hiusbc_gadget_suspend(hiusbc);

	if (hiusbc->disconnect_in_suspend) {
		hiusbc_dbg(HIUSBC_DEBUG_ERR, "call disconnect in suspend event!!!\n");
...
void hiusbc_intr_handle_evt_ring(unsigned long param)
{
	...
	hiusbc_dbg(HIUSBC_DEBUG_EVT_RING,
		"Start to handle interrupt at Deq = %pK (0x%llx dma).\n",
			event_ring->dequeue,
	...
	while (hiusbc_handle_event(event_ring) > 0) {
		event_total++;
		event_counter++;
		if (event_counter >= (int)(event_ring->size / 4)) {
			/* update erdq */
			dequeue = hiusbc_evt_trb_vrt_to_dma(event_ring, event_ring->dequeue);
			if (dequeue == 0)
				pr_err("1/4 Bad sw event ring dequeue ptr!\n");

			dequeue &= HIUSBC_EBDP_MASK;
	...
	hiusbc_dbg(HIUSBC_DEBUG_EVT_RING,
		"Handled %d evt, from:%pK (0x%llx dma), to:%pK (0x%llx dma)\n",
			event_total, dequeue_backup,
			hiusbc_evt_trb_vrt_to_dma(event_ring, dequeue_backup),
			event_ring->dequeue,
			hiusbc_evt_trb_vrt_to_dma(event_ring, event_ring->dequeue));
	...
	dequeue &= HIUSBC_EBDP_MASK;
	dequeue |= MPI_APP_DEVICE_REG_DEV_EVENT_HANDLER_BUSY_0_MASK;
	reg = lower_32_bits(dequeue);
	hiusbc_writel(reg, hiusbc->dev_regs,
		hiusbc_dev_ebdp0_offset(event_ring->int_idx));
	reg = upper_32_bits(dequeue);
...
irqreturn_t hiusbc_interrupt(int irq, void *_hiusbc)
{
	...
	if ((reg & MPI_APP_DEVICE_REG_DEV_EVENT_INTERRUPT_MASK) == 0) {
		dev_err(hiusbc->dev,
	...
	if ((reg & MPI_APP_DEVICE_REG_DEV_INT_PENDING_0_MASK) == 0) {
		dev_err(hiusbc->dev,
			"Interrupt comes but IMAN_IP is 0, IMAN = 0x%x!\n",
			reg);
		goto error;
	}
	reg |= MPI_APP_DEVICE_REG_DEV_INT_PENDING_0_MASK;
	hiusbc_writel(reg, hiusbc->dev_regs,
		hiusbc_dev_iman_offset(event_ring->int_idx));
