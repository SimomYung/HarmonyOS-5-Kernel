...
static void hiusbc_enable_intr(struct hiusbc_evt_ring *event_ring)
{
	...
	reg |= MPI_APP_DEVICE_REG_DEV_INT_ENABLE_0_MASK;
...
void hiusbc_set_ux_exit(struct hiusbc *hiusbc)
{
	...
	if (hiusbc->usb3_dev_lpm_ux_exit == 0)
		return;
	...
	for (epnum = 0; epnum < HIUSBC_EP_NUMS; epnum++) {
		...
		if (hep->si != 0 && hep->enabled != 0) {
			if ((ux_exit_interval == 0) ||
	...
	if (ux_exit_interval == 0) {
		...
		hiusbc_dbg(HIUSBC_DEBUG_SYS,
			...
			hiusbc->u1_enable != 0 ? "Enabled" : "Disabled",
			hiusbc->u2_enable != 0 ? "Enabled" : "Disabled");
...
{
	struct hiusbc *hiusbc = event_ring->hiusbc;
	u8 index = (u8)(event_ring->int_idx);
	u32 reg;

	hiusbc_dbg(HIUSBC_DEBUG_SYS, "+\n");

	reg = hiusbc_readl(hiusbc->dev_regs, hiusbc_dev_imod_offset(index));
	reg &= ~MPI_APP_DEVICE_REG_DEV_INT_MOD_INTERVAL_0_MASK;
	...
	do {
		...
		if (reg & MPI_APP_DEVICE_REG_DEV_CMD_DONE_MASK) {
			...
			case HIUSBC_CMD_ABORT:
				pr_debug("Cmd done info: Aborted.\n");
				ret = 0;
				break;
			...
			case HIUSBC_CMD_FRM_EXPIRED:
				pr_err("Cmd done info: Frame expired.\n");
				ret = -EINVAL;
				break;
...
int hiusbc_send_cmd_set_addr(struct hiusbc *hiusbc, u8 addr)
{
	...
	hiusbc_dbg(HIUSBC_DEBUG_CMD,
		"Queuing a Set Location cmd, location=%u.\n", addr);
...
#else
	parm.word2 |= hiusbc_cmd_max_packet_size((unsigned int)usb_endpoint_maxp(ep_desc));
#endif
	return hiusbc_send_cmd(hiusbc, &parm);
}
	...
	if (ret == 0) {
		...
		if (!list_empty(&hep->queue)) {
			...
			hiusbc_dbg(HIUSBC_DEBUG_XFER | HIUSBC_DEBUG_BULK,
				"Restart ep%u from trb_dma after clear stall. init = %u\n",
				hep->epnum, init);
...
int hiusbc_set_ep_stall(struct hiusbc_ep *hep, bool set)
{
	...
	if (hep->ep.desc && usb_endpoint_xfer_isoc(hep->ep.desc)) {
...
static int hiusbc_gadget_pullup(struct usb_gadget *g, int is_on)
{
	...
	hiusbc_dbg(HIUSBC_DEBUG_ERR, "is_on = %d\n", is_on);
...
#ifndef CONFIG_USB_CONFIG_PRODUCT_PC
	if (!is_on)
		udelay(15000); // delay 15ms for aviod recovery status
#endif
...
error:
	...
	if (!is_on) {
		msleep(15); // delay 15ms for aviod recovery status
	}
#endif
	hiusbc_dbg(HIUSBC_DEBUG_ERR, "-\n");
...
int hiusbc_run_stop(struct hiusbc *hiusbc, int is_on)
{
	...
	if (is_on != 0 && !(reg & MPI_APP_DEVICE_REG_DEV_RUN_STOP_MASK)) {
	...
	/* dfx set if ltssm_dump is true */
	reg_dfx = hiusbc_readl(hiusbc->u3_link_regs,
		MPI_U3_LINK_REG_CSR_DFX_STATE_CFG_OFFSET);
	if (hiusbc->ltssm_dump)
		reg_dfx |= MPI_U3_LINK_REG_CSR_DFX_STATE_WR_EN_MASK;
	else
		reg_dfx &= ~MPI_U3_LINK_REG_CSR_DFX_STATE_WR_EN_MASK;
...
static int hiusbc_gadget_init_interrupt(struct hiusbc *hiusbc)
{
	...
	hiusbc_dbg(HIUSBC_DEBUG_SYS,
		...
		"start TRB = %pK (0x%llx dma), size = %uTRBs\n",
