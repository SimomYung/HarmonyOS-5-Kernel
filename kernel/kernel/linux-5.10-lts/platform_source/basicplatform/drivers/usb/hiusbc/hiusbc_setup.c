			...
			const struct usb_ctrlrequest *setup, u16 *status)
{
	...
	*status = 0;
	epnum = (u8)((setup->wIndex & USB_ENDPOINT_NUMBER_MASK) << 1);
		...
		const struct usb_ctrlrequest *setup, bool set)
{
	...
	if (hiusbc->usb3_dev_lpm_capable == 0) {
				...
				const struct usb_ctrlrequest *setup)
{
	...
	hiusbc_dbg(HIUSBC_DEBUG_CTRL,
		"Start handling std req: Set Location=%u\n", setup->wValue);

	if (setup->wValue > 127) {
		dev_err(hiusbc->dev, "Location % is greater than 127!\n",
			setup->wValue);
		return -EINVAL;
	}
	...
	if (state != USB_STATE_CONFIGURED) {
		dev_err(hiusbc->dev,
			"will set Location to 0x%x in Status Stage.\n", addr);
		return 0;
	}

	dev_err(hiusbc->dev,
		"Set Location: wrong device state = %i!\n", state);
	return -EINVAL;
}
...
static void hiusbc_enable_accept_lpm(struct hiusbc *hiusbc)
{
	...
	if ((hiusbc->gadget.speed >= USB_SPEED_SUPER) &&
		...
		hiusbc_writel(reg, hiusbc->dev_regs,
			MPI_APP_DEVICE_REG_DEV_CTL_OFFSET);
		hiusbc_dbg(HIUSBC_DEBUG_ERR, "Enable %s %s Accept.\n",
			hiusbc->usb3_dev_lpm_u1_accept != 0 ? "U1" : " ",
			hiusbc->usb3_dev_lpm_u2_accept != 0 ? "U2" : " ");
	}
}
		...
		const struct usb_ctrlrequest *setup)
{
	...
	if (config != 0 && (ret == 0 || ret == USB_GADGET_DELAYED_STATUS)) {
		if (ret == 0) {
			usb_gadget_set_state(&hiusbc->gadget,
					USB_STATE_CONFIGURED);
			hiusb_atomic_notifier_call_chain(DEVICE_EVENT_SETCONFIG,
					(void *)&hiusbc->gadget.speed);
			hiusbc_dbg(HIUSBC_DEBUG_ERR,
