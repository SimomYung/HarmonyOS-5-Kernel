...
#endif
#else
#define MAIN_PANEL_ID_MIN 0
#define MAIN_PANEL_ID_MAX 1
static int hifi_usb_dkmd_notifier_callback(struct notifier_block *self,
			unsigned long event, void *data)
{
	int blank = 0;
	int panel_id = -1;
	struct dkmd_event *evdata = (struct dkmd_event *)data;
	struct hifi_usb_proxy *proxy = get_hifi_usb_proxy_handle();
	...
	blank = evdata->value;
	panel_id = evdata->panel_id;

	info("event 0x%lx, blank 0x%x, panel_id %d\n", event, blank, panel_id);

	if (panel_id >= MAIN_PANEL_ID_MIN && panel_id <= MAIN_PANEL_ID_MAX) {
		if (event == DKMD_EARLY_EVENT_BLANK && blank == FB_BLANK_UNBLANK) {
			info("main panel %d display on, fb forbid hifiusb hibernate\n", panel_id);
			hifi_usb_hibernation_ctrl(FB_CTRL, false);
		} else if (event == DKMD_EVENT_BLANK && blank == FB_BLANK_POWERDOWN) {
			info("main panel %d display off, fb allow hifiusb hibernate\n", panel_id);
			hifi_usb_hibernation_ctrl(FB_CTRL, true);
		}
	}
...
err_hw_revive:
	...
	usb_stop_hifi_usb_by_portid(proxy->current_port, false);
...
void hifi_usb_hibernation_init(struct hifi_usb_proxy *hifiusb)
{
	...
	hifiusb->fb_notify.notifier_call = hifi_usb_dkmd_notifier_callback;
	ret = dkmd_register_client(&hifiusb->fb_notify);
	if (ret)
		err("register hifi_usb_fb_notifier_callback fail\n");
#endif
...
void hifi_usb_hibernation_exit(struct hifi_usb_proxy *hifiusb)
{
	...
	ret = dkmd_unregister_client(&hifiusb->fb_notify);
	if (ret)
		err("unregister fb_notify fail\n");
#endif
}
