...
#ifdef CONFIG_USB_HIUSB
	if (is_hifi_usb_in_used()) {
		pr_info("hifi is used, using ap audio\n");
		return -EBUSY;
	}
...
#ifdef CONFIG_USB_HIUSB
	/* in single port product, if port0 state is off, unable to start hifi */
	if (hiusb_check_default_port_state_is_off()) {
		pr_info("port start is off, do not start hifi");
		return -ENODEV;
	}
#endif
...
int usb_start_hifi_usb_reset_power(struct usb_device *udev)
{
	...
	/* in single port product, if port0 state is off, unable to start hifi */
	if (hiusb_check_default_port_state_is_off()) {
		pr_info("port start is off, do not start hifi reset power");
		return -ENODEV;
	}
#endif
	portid = usb_find_portid_by_udev(udev);
...
void usb_stop_hifi_usb_by_portid(int portid, bool reset_power)
{
...
#ifdef CONFIG_USB_HIUSB
	hiusb_stop_hifi_usb(portid, reset_power);
#else
	(void)portid;
	if(reset_power)
		event_type = STOP_HIFI_USB_RESET_VBUS;
	else
		event_type = STOP_HIFI_USB;
	if (chip_usb_otg_event(event_type))
...
void usb_stop_hifi_usb(struct usb_device *udev)
{
	...
	if (portid < 0)
		return;
	usb_stop_hifi_usb_by_portid(portid, false);
}
...
static int hifi_usb_proxy_urb_dequeue(struct proxy_hcd_client *client, struct urb *urb, struct urb_msg *urb_msg)
{
	struct hifi_usb_proxy *proxy = client->client_priv;
	struct hifi_usb_op_msg __op_msg = {0};
	struct hifi_usb_op_msg *op_msg = &__op_msg;
	unsigned long phcd_lock_flags;
	...
	dbg("+\n");
	spin_lock_irqsave(&phcd->lock, phcd_lock_flags);
	if (urb_msg == NULL) {
		spin_unlock_irqrestore(&phcd->lock, phcd_lock_flags);
	...
	if(!urb || !urb->hcpriv) {
		spin_unlock_irqrestore(&phcd->lock, phcd_lock_flags);
		pr_err("usb urb is already released");
		return 0;
	}
#ifdef CONFIG_USB_PROXY_HCD_HIBERNATE
...
static int hifi_usb_notify_hifi_run(struct hifi_usb_proxy *proxy)
{
	...
	if (ret) {
		...
		while (retry_count > 0) {
			ret_stop += hifi_usb_runstop_and_wait(proxy, false);
			retry_count--;
		}
		if (ret_stop) {
			err("hifi_usb_run failed or timeout\n");
#if defined CONFIG_USB_HIUSB
			hiusb_config_tzpc_by_portid(proxy->current_port);
#endif
		}
...
static void hifi_usb_dts_config(struct hifi_usb_proxy *hifiusb)
{
	...
	bool *p_usb_reset_power = get_ptr_usb_reset_power();
	bool *p_usb_support_more_hifi = get_ptr_usb_support_more_hifi();
	...
	*p_usb_reset_power = of_property_read_bool(dev->of_node, "hifi-usb-reset-power");
	info("reset_power  %d\n", *p_usb_reset_power);
	*p_usb_support_more_hifi = of_property_read_bool(dev->of_node, "usb-support-more-hifi");
	info("usb-support-more-hifi  %d\n", *p_usb_support_more_hifi);
