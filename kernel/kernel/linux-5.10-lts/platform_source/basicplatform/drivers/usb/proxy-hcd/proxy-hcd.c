...
int phcd_urb_complete(struct proxy_hcd *phcd, struct urb_msg *urb_msg)
{
	...
	if (phcd_urb->urb_msg->is_vm_urb)
		vfree(phcd_urb->urb_msg);
	else
...
static int phcd_urb_enqueue(struct usb_hcd *hcd, struct urb *urb, gfp_t mem_flags)
{
	...
	if (phcd_urb->urb_msg == NULL) {
		err("kmalloc urb_msg failed, try use vmalloc\n");
		if (in_interrupt()) {
			/* call by interrupt context */
			err("kmalloc urb_msg failed, and interrupt context, return fail\n");
			kfree(phcd_urb);
			return -ENOMEM;
		} else {
			/* call by process context */
			phcd_urb->urb_msg = vmalloc(sizeof(*phcd_urb->urb_msg) + PHCD_MAX_XFER_LEN);
			if (phcd_urb->urb_msg == NULL) {
				/* vmalloc fail */
				err("vmalloc urb_msg failed, return fail\n");
				kfree(phcd_urb);
				return -ENOMEM;
			} else {
				/* vmalloc sucess */
				(void)memset_s(phcd_urb->urb_msg, sizeof(*phcd_urb->urb_msg) + PHCD_MAX_XFER_LEN,
								0, sizeof(*phcd_urb->urb_msg) + PHCD_MAX_XFER_LEN);
				phcd_urb->urb_msg->is_vm_urb = 1;
			}
		}
	} else {
		phcd_urb->urb_msg->is_vm_urb = 0;  /* kmalloc sucess */
	}
...
wait_enqueue_complete:
	...
	ret = proxy_urb_dequeue(phcd, urb, phcd_urb->urb_msg);
	if (unlikely(ret)) {
		err("usb proxy_urb_dequeue failed %d\n", ret);
		spin_lock_irqsave(&phcd->lock, flags);
