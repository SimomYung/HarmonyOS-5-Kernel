...
#define VID_JMICRON 0x152d
#define PID_JMICRON 0x0579
...
bool hifi_usb_reset_power;
bool usb_support_more_hifi = false;

bool *get_ptr_usb_support_more_hifi(void)
{
	return &usb_support_more_hifi;
}

bool *get_ptr_usb_reset_power(void)
{
	return &hifi_usb_reset_power;
}
...
static bool is_usb_black_list_devices(struct usb_device *udev)
{
	if (((le16_to_cpu(udev->descriptor.idVendor) == VID_HUAWEI_ST310) &&
		(le16_to_cpu(udev->descriptor.idProduct) == PID_HUAWEI_ST310)) ||
		((le16_to_cpu(udev->descriptor.idVendor) == VID_JMICRON) &&
		(le16_to_cpu(udev->descriptor.idProduct) == PID_JMICRON)))
		return true;
	return false;
}
static bool is_u_disk_device(struct usb_device *udev)
{
	int    nintf;
	...
	if(is_usb_black_list_devices(udev)) {
		info("is black list devices\n");
		return false;
	}
...
static bool match_reset_power_devices(struct usb_device *udev)
{
	...
	if (hifi_usb_reset_power && is_u_disk_device(udev))
		return true;
...
static bool is_usbaudio_device(struct usb_device *udev, int configuration)
{
	...
	if ((audio_intf_num == 1) &&
	    (hid_intf_num <= 1 || usb_support_more_hifi) &&
	    (other_intf_num == 0)) {
