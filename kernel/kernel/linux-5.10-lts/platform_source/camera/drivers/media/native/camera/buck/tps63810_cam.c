...
static bool tps63810_set_enable(struct hw_buck_ctrl_t *buck_ctrl, int state)
{
	u32 ctrl_val;
...
static int tps63810_set_vout(struct hw_buck_ctrl_t *buck_ctrl,
	u32 voltage)
{
	u8 vset;
...
static int tps63810_state_control_by_enpin(struct hw_buck_ctrl_t *buck_ctrl,
	int state)
{
	int ret;
	...
	tps63810_set_enable(buck_ctrl, POWER_ON);

	return 0;
}

static int tps63810_state_control_by_reg(struct hw_buck_ctrl_t *buck_ctrl,
	int state)
{
	tps63810_set_enable(buck_ctrl, state);

	return 0;
}

static int tps63810_state_control(struct hw_buck_ctrl_t *buck_ctrl, int state)
{
	int ret;
	if (buck_ctrl->buck_info.control_type == 0) {
		ret = tps63810_state_control_by_enpin(buck_ctrl, state);
	} else {
		ret = tps63810_state_control_by_reg(buck_ctrl, state);
	}
				...
				int state)
{
	...
	ret = tps63810_state_control(buck_ctrl, state);
	if (ret < 0) {
		cam_err("%s state control fail %d", __func__, ret);
		return -1;
	}

	return tps63810_set_vout(buck_ctrl, voltage);
}
...
static int tps63810_init(struct hw_buck_ctrl_t *buck_ctrl)
{
	...
	ret = tps63810_state_control(buck_ctrl, POWER_ON);
	if (ret < 0) {
		cam_err("%s:state control failed, ret = %d", __func__, ret);
		return ret;
	}
...
EXIT:
	tps63810_state_control(buck_ctrl, POWER_OFF);
