...
int cam_internal_sc_attach(int fd, struct systemcache_format *fmt)
{
	int rc;

	cam_info("%s: fd %d for pid %u offset %u size %u", __func__, fd, fmt->pid, fmt->offset, fmt->size);

	rc = dma_buf_attach_lb(fd, fmt->pid, (unsigned long)fmt->offset, (size_t)fmt->size);
	if (rc < 0)
		cam_err("%s: failed", __func__);
...
int cam_internal_sc_detach(int fd)
{
	int rc;

	cam_info("%s: fd %d ", __func__, fd);

	rc = dma_buf_detach_lb(fd);
	if (rc < 0)
		cam_err("%s: failed", __func__);
