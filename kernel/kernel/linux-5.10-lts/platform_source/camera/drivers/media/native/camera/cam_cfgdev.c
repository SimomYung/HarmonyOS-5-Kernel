...
#include "smart/drivers/io_die/iodie_hiphyio_hal.h"
...
static unsigned int g_isp_thermal_level = 0;
static int cam_cfgdev_isp_thermal_guard(struct v4l2_event *ev);
#ifdef CONFIG_THERMAL_ISP_HOTPLUG
extern void isp_hotplug_callback_register(int (*isp_hotplug_handler)(unsigned int));
extern void isp_hotplug_callback_unregister(void);
static int handle_isp_thermal(unsigned int level);
extern void ddr_to_isp_callback_register(int (*ddr_to_isp_handler)(unsigned int));
extern void ddr_to_isp_callback_unregister(void);
static int handle_ddr_thermal(unsigned int level);
int (*g_block_isp_cb)(bool blockf) = NULL;

static void ddr_thermal_work_handler(struct work_struct * work);
DECLARE_WORK(ddr_thermal_work, ddr_thermal_work_handler);
#define DDR_THERMAL_LEVEL_FIFO 1024
DEFINE_KFIFO(ddr_thermal_level_fifo, unsigned int, DDR_THERMAL_LEVEL_FIFO);
#endif
#ifdef CONFIG_HISI_VBAT_DROP_PRT_V2
extern void vbat_to_isp_callback_register(int (*vbat_to_isp_handler)(unsigned int));
extern void vbat_to_isp_callback_unregister(void);
#endif
...
static ssize_t ixc_data_show(struct device_driver *drv, char *buf);
static ssize_t ixc_data_store(struct device_driver *drv,
		const char *buf, size_t count);
static DRIVER_ATTR_RW(ixc_data);
#endif
...
static int cam_cfgdev_ixc_store(void)
{
	struct v4l2_event ev = {
		.type = CAM_V4L2_EVENT_TYPE,
		.id = CAM_CFGDEV_REQUEST,
	...
	req->kind = CAM_CFGDEV_REQ_IXC_DFX;
	return cam_cfgdev_send_req(&ev, &s_cfgdev.rq);
}

static int cam_cfgdev_lcd_mipi_switch(void)
{
	struct v4l2_event ev = {
		.type = CAM_V4L2_EVENT_TYPE,
		.id = CAM_CFGDEV_REQUEST,
	...
	req->kind = CAM_CFGDEV_REQ_LCD_MIPI_SWITCH;
	return cam_cfgdev_send_req(&ev, &s_cfgdev.rq);
}

void camera_lcd_mipi_switch_status_store(int panel_id)
{
	if (cam_cfgdev_lcd_mipi_switch()) {
		cam_err("%s notify err", __func__);
	}

	return;
}
EXPORT_SYMBOL(camera_lcd_mipi_switch_status_store);

static int cam_cfgdev_lcd_start_switch_mipi(int panel_id)
{
	struct v4l2_event ev = {
		.type = CAM_V4L2_EVENT_TYPE,
		.id = CAM_CFGDEV_REQUEST,
	...
	req->kind = CAM_CFGDEV_REQ_LCD_START_SWITCH_MIPI;
	req->lcd_mipi_switch.lcd_status = panel_id;
	return cam_cfgdev_send_req(&ev, &s_cfgdev.rq);
}

void camera_lcd_start_switch_mipi_store(int panel_id)
{
	if (cam_cfgdev_lcd_start_switch_mipi(panel_id)) {
		cam_err("%s notify err", __func__);
	}
...
static int handle_ddr_thermal(unsigned int level)
{
    if (kfifo_in(&ddr_thermal_level_fifo, &level, 1) != 1) {
        cam_warn("%s, discard ddr_level = %d", __func__, level);
        return 0;
    }
    schedule_work(&ddr_thermal_work);
    return 0;
}
...
static void ddr_thermal_work_handler(struct work_struct * work)
{
	...
	cam_cfgreq2dev_t *req = (cam_cfgreq2dev_t *)ev.u.data;
	req->kind = CAM_CFGDEV_REQ_DDR_THERMAL;

	while (kfifo_out(&ddr_thermal_level_fifo, &level, 1) > 0) {
		cam_cfg_info("%s, ddr_level = %d, fifo len = %d", __func__, level, kfifo_len(&ddr_thermal_level_fifo));
		req->ddrThermalLevel = level;
		cam_cfgdev_send_req(&ev, &s_cfgdev.rq);
	}
}
#endif

static int handle_vbat_limit(unsigned int level)
{
	cam_cfg_info("%s, vbat_level = %d", __func__, level);
	struct v4l2_event ev = {
		.type = CAM_V4L2_EVENT_TYPE,
		.id = CAM_CFGDEV_REQUEST,
	};
	...
	req->kind = CAM_CFGDEV_REQ_VBAT_LIMIT;
	req->vbatLimitLevel = level;
	return cam_cfgdev_send_req(&ev, &s_cfgdev.rq);
}
...
static ssize_t ixc_data_show(struct device_driver *drv, char *buf)
{
	...
	cam_cfg_info("%s enter", __func__);
	ret = snprintf_s(buf, PAGE_SIZE, PAGE_SIZE - 1,
			"ixc notify:[%s], ret:[%u]\n", s_ixc_data.sbuf, s_ixc_data.status);
	if (ret < 0) {
		cam_warn("%s snprintf_s fail", __func__);
		return 0;
	}
				   ...
				   const char *buf, size_t count)
{
	errno_t err;
	size_t size_to_copy;
	const size_t sbuf_size = sizeof(s_ixc_data.sbuf);
	...
	cam_cfg_info("%s enter", __func__);
	err = memset_s(s_ixc_data.sbuf, sbuf_size, 0, sbuf_size);
	if (err != EOK)
		cam_warn("%s memset_s fail", __func__);
	...
	if (count > size_to_copy)
		cam_warn("%s count[%zu] is beyond sbuf size[%zu]",
			__func__, count, sizeof(s_ixc_data.sbuf));

	err = memcpy_s(s_ixc_data.sbuf, sbuf_size - 1, buf, size_to_copy);
	if (err != EOK)
		cam_warn("%s memcpy_s fail", __func__);
	s_ixc_data.sbuf[sbuf_size - 1] = '\0';
	s_ixc_data.size = size_to_copy;
	(void)cam_cfgdev_ixc_store();
	return count;
}
#endif
...
static int cam_cfgdev_dfx_ixc(struct v4l2_event *ev)
{
	errno_t err = EOK;
	if (ev->id == CAM_CFGDEV_REQUEST) {
		err = memcpy_s(ev->u.data, sizeof(ev->u.data),
				s_ixc_data.sbuf, s_ixc_data.size);
		if (err != EOK)
			cam_warn("%s memcpy_s fail", __func__);
	} else if (ev->id == CAM_NOTIFY_USER) {
		const size_t sbuf_size = sizeof(s_ixc_data.sbuf);
	    err = memset_s(s_ixc_data.sbuf, sbuf_size, 0, sbuf_size);
		const size_t len = ev->u.data[sizeof(ev->u.data) - 2];
		err = memcpy_s(s_ixc_data.sbuf, sizeof(s_ixc_data.sbuf), ev->u.data, len);
		s_ixc_data.status = ev->u.data[sizeof(ev->u.data) - 1];
		if (err != EOK)
			cam_warn("%s memcpy_s fail", __func__);
	}
	return err;
...
EXPORT_SYMBOL(init_subdev_media_entity);
...
EXPORT_SYMBOL(cam_cfgdev_register_subdev);
...
EXPORT_SYMBOL(cam_cfgdev_unregister_subdev);
...
static long cam_cfgdev_vo_do_ioctl(struct file *filep, unsigned int cmd, void *arg)
{
	...
	case CAM_V4L2_IOCTL_MOTION_DATA:
		...
		break;
	case CAM_V4L2_IOCTL_ISP_THERMAL_GUARD:
		rc = cam_cfgdev_isp_thermal_guard((struct v4l2_event *)arg);
		break;
	case CAM_V4L2_IOCTL_DFX_IXC:
	    rc = cam_cfgdev_dfx_ixc((struct v4l2_event *)arg);
		break;
	default:
...
#ifdef CONFIG_THERMAL_ISP_HOTPLUG
	isp_hotplug_callback_register(handle_isp_thermal);
	ddr_to_isp_callback_register(handle_ddr_thermal);
#endif
#ifdef CONFIG_HISI_VBAT_DROP_PRT_V2
    vbat_to_isp_callback_register(handle_vbat_limit);
#endif
	goto probe_end;
...
static int cam_cfgdev_vo_remove(struct platform_device *pdev)
{
	...
	s_cfgdev.mdev = NULL;
#ifdef CONFIG_THERMAL_ISP_HOTPLUG
	isp_hotplug_callback_unregister();
	ddr_to_isp_callback_unregister();
#endif
#ifdef CONFIG_HISI_VBAT_DROP_PRT_V2
	vbat_to_isp_callback_unregister();
#endif
...
#ifdef CONFIG_CONTEXTHUB_IO_DIE_HIPHYIO
static void iodie_notify_handler(enum iodie_sys_status sys_st, enum iodie_err_status err_st)
{
	mutex_lock(&g_iodie_st_lock);
	enum iodie_err_status m_iodie_err_st = err_st;
	cam_cfg_info("%s: iodie sys_st:%u err_st:%u\n", __func__, sys_st, m_iodie_err_st);
	...
	req->kind = CAM_CFGDEV_REQ_IODIE_STATUS;
    req->iodieStatus = m_iodie_err_st == IODIE_ERR_IODIE ? 1 : 0;
	cam_cfgdev_send_req(&ev, &s_cfgdev.rq);
...
static int __init cam_cfgdev_vo_init(void)
{
	...
	if (ret == 0) {
		...
		if (driver_create_file(&s_cfgdev_driver.driver,
					&driver_attr_ixc_data) != 0)
			cam_warn("%s create driver attr ixc_data failed",
					__func__);
#endif
#ifdef CONFIG_CONTEXTHUB_IO_DIE_HIPHYIO
	if (iodie_notify_register(IODIE_USER_AP_CAM, iodie_notify_handler)) {
		pr_err("%s: regist iodie notify fail\n", __func__);
	}
#endif
	}
	return ret;
...
{
	if (ev) {
		*((unsigned int*)ev->u.data) = g_isp_thermal_level;
	}
	return 0;
}
...
static int handle_isp_thermal(unsigned int level)
{
	cam_cfg_info("%s, level = %d", __func__, level);
	g_isp_thermal_level = level;
	if (g_block_isp_cb != NULL) {
		if (level >= 3) { /* 3, max isp thermal level */
			g_block_isp_cb(true);
		} else {
			g_block_isp_cb(false);
		}
	}
	struct v4l2_event ev = {
		.type = CAM_V4L2_EVENT_TYPE,
	...
	req->kind = CAM_CFGDEV_REQ_ISP_THERMAL;
	req->ispThermalLevel = level;
	return cam_cfgdev_send_req(&ev, &s_cfgdev.rq);
}

void set_block_isp_cb(int (*block_isp_cb)(bool))
{
	if (block_isp_cb == NULL) {
		cam_cfg_info("block_isp_cb is NULL!\n");
		return;
	}
	g_block_isp_cb = block_isp_cb;
}
EXPORT_SYMBOL(set_block_isp_cb);
#endif
