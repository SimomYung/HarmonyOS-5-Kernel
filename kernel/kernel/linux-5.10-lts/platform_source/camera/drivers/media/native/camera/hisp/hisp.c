/*
 * Copyright (c) Huawei Technologies Co., Ltd. 2014-2020. All rights reserved.
 * Description:Implement of hisp
 * Create: 2014-11-11
 *
 * This program is free software; you can redistribute it and/or modify
 * it under the terms of the GNU General Public License as published by
 * the Free Software Foundation; either version 2 of the License, or
 * (at your option) any later version.
 *
 * This program is distributed in the hope that it will be useful,
 * but WITHOUT ANY WARRANTY; without even the implied warranty of
 * MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE. See the
 * GNU General Public License for more details.
 */

#if defined(HISP150_CAMERA)
#include <platform_include/camera/native/hisp150_cfg.h>
#elif defined(HISP160_CAMERA)
#include <platform_include/camera/native/hisp160_cfg.h>
#elif defined(HISP120_CAMERA)
#include <platform_include/camera/native/hisp120_cfg.h>
#elif defined(HISP200_CAMERA)
#include <platform_include/camera/native/hisp200_cfg.h>
#elif defined(HISP250_CAMERA)
#include <platform_include/camera/native/hisp250_cfg.h>
#elif defined(HISP210_CAMERA)
#include <platform_include/camera/native/hisp210_cfg.h>
#elif defined(HISP310_CAMERA)
#include <platform_include/camera/native/hisp250_cfg.h>
#elif defined(HISP300_CAMERA)
#include <platform_include/camera/native/hisp300_cfg.h>
#elif defined(HISP350_CAMERA)
#include <platform_include/camera/native/hisp350_cfg.h>
#elif defined(HISP500_CAMERA)
#include <platform_include/camera/native/hisp500_cfg.h>
#elif defined(HISP600_CAMERA)
#include <platform_include/camera/native/hisp600_cfg.h>
#elif defined(HISP220_CAMERA)
#include <platform_include/camera/native/hisp210_cfg.h>
#elif defined(HISP230_CAMERA)
#include <platform_include/camera/native/hisp210_cfg.h>
#else
#include <platform_include/camera/native/hisp_cfg.h>
#endif

#include <linux/compiler.h>
#include <linux/fs.h>
#include <linux/gpio.h>
#include <linux/module.h>
#include <linux/of_device.h>
#include <linux/of_gpio.h>
#include <linux/of.h>
#include <linux/platform_device.h>
#include <linux/sched.h>
#include <linux/slab.h>
#include <linux/types.h>
#include <linux/videodev2.h>
#include <media/v4l2-event.h>
#include <media/v4l2-fh.h>
#include <media/v4l2-subdev.h>
#include <media/videobuf2-core.h>
#include <linux/string.h>
#include <platform_include/isp/linux/hisp_remoteproc.h>
#include <linux/sched/mm.h>
#include <linux/sched/task.h>
#include <linux/pid.h>

#include <dsm/dsm_pub.h>
#include <securec.h>
#include "cam_buf.h"
#include "hisp_intf.h"
#include "cam_log.h"

#define DSM_DEV_BUFF_SIZE 1024
#define CREATE_TRACE_POINTS

static struct dsm_dev g_dev_hisp = {
									.name = "dsm_hisp",
									.device_name = NULL,
									.ic_name = NULL,
									.module_name = NULL,
									.fops = NULL,
									.buff_size = DSM_DEV_BUFF_SIZE,
};

static struct dsm_client *g_client_hisp = NULL;
int hisp_dev_setpinctl(struct pinctrl *isp_pinctrl,
						 struct pinctrl_state *pinctrl_def, struct pinctrl_state *pinctrl_idle);
int hisp_dev_setclkdepend(struct clk *aclk_dss, struct clk *pclk_dss);
int hisp_secmem_pa_set(u64 sec_boot_phymem_addr, unsigned int trusted_mem_size);
int hisp_secmem_pa_release(void);
int hisp_mdcmem_pa_set(u64 mdc_phymem_addr, unsigned int mdc_mem_size,
					   int shared_fd);
int hisp_mdcmem_pa_release(void);

static bool g_hisp_mdcmem_set;
static bool g_hisp_secmem_set;
static unsigned int g_system_couter_rate = 0;
static timestamp_mode g_timestamp_mode;
#define SD2HISP(sd) container_of(sd, hisp_t, subdev)

extern int memset_s(void *dest, size_t destMax, int c, size_t count);


#define MAX_DAEMON_REF_COUNT 10
struct daemon_ref_count {
	pid_t pid;
	struct v4l2_subdev_fh* fh[MAX_DAEMON_REF_COUNT];
	volatile int ref_counts;
	struct mm_struct *mm;
};

static DEFINE_MUTEX(client_maps_mtx);
static struct daemon_ref_count hisp_client_id_to_daemon_pid[MAX_HISP_CLIENT];

static void isp_get_proc_mm(int cid)
{
	int ret = 0;
	struct pid *pid_struct = NULL;
	struct task_struct *task = NULL;
	struct mm_struct *mm = NULL;

	pid_struct = find_get_pid(current->tgid);
	if(pid_struct == NULL) {
		ret = -1;
		goto last_deal;
	}

	task = get_pid_task(pid_struct, PIDTYPE_PID);
	if(task == NULL) {
		cam_warn("get pid task null: %d", current->tgid);
		ret = -1;
		goto free_pid;
	}

	mm = get_task_mm(task);
	if(mm == NULL) {
		cam_warn("get task mm null: %d", current->tgid);
		ret = -1;
		goto free_task;
	}

free_task:
	put_task_struct(task);
free_pid:
	put_pid(pid_struct);
last_deal:
	if (ret == 0)
		hisp_client_id_to_daemon_pid[cid].mm = mm;
	else
		hisp_client_id_to_daemon_pid[cid].mm = NULL;
}

static void isp_put_proc_mm(int cid)
{
	if (hisp_client_id_to_daemon_pid[cid].mm != NULL) {
		mmput(hisp_client_id_to_daemon_pid[cid].mm);
		hisp_client_id_to_daemon_pid[cid].mm = NULL;
	}
}

static int hisp_allocate_client_id(pid_t pid, struct v4l2_subdev_fh* fh)
{
	int cid;
	int rc = -EBUSY;
	mutex_lock(&client_maps_mtx);
	for (cid = 0; cid < MAX_HISP_CLIENT; ++cid)
		if (hisp_client_id_to_daemon_pid[cid].pid == pid) {
			rc = 0;
			goto out;
		}

	for (cid = 0; cid < MAX_HISP_CLIENT; ++cid)
		if (hisp_client_id_to_daemon_pid[cid].pid == 0) {
			hisp_client_id_to_daemon_pid[cid].pid = pid;
			rc = 0;
			isp_get_proc_mm(cid);
			goto out;
		}

out:
	if (rc == 0 && hisp_client_id_to_daemon_pid[cid].ref_counts < MAX_DAEMON_REF_COUNT) {
		hisp_client_id_to_daemon_pid[cid].fh[hisp_client_id_to_daemon_pid[cid].ref_counts] = fh;
		++hisp_client_id_to_daemon_pid[cid].ref_counts;
		cam_info("%s: allocate daemon %d client_id: %d refs: %d", __func__, pid, cid,
				hisp_client_id_to_daemon_pid[cid].ref_counts);
	} else {
		rc = -EBUSY;
		cam_warn("%s: allocate daemon %d client_id failed, client full", __func__, pid);
	}
	mutex_unlock(&client_maps_mtx);
	return rc;
}

static pid_t hisp_get_pid_by_fh(struct v4l2_subdev_fh* fh)
{
	int cid;
	int fhid;
    pid_t pid = 0;
	mutex_lock(&client_maps_mtx);
	for (cid = 0; cid < MAX_HISP_CLIENT; ++cid) {
		for (fhid = 0; fhid < hisp_client_id_to_daemon_pid[cid].ref_counts; ++fhid) {
			if (hisp_client_id_to_daemon_pid[cid].fh[fhid] == fh) {
				pid = hisp_client_id_to_daemon_pid[cid].pid;
				break;
			}
		}
	}
    mutex_unlock(&client_maps_mtx);
    return pid;
}

static void hisp_free_client_id(struct v4l2_subdev_fh* fh)
{
	int cid;
	int fhid;
	bool found = false;
	mutex_lock(&client_maps_mtx);
	for (cid = 0; cid < MAX_HISP_CLIENT; ++cid) {
		for (fhid = 0; fhid < hisp_client_id_to_daemon_pid[cid].ref_counts; ++fhid) {
			if (hisp_client_id_to_daemon_pid[cid].fh[fhid] == fh) {
				found = true;
				break;
			}
		}
		if (found)
			break;
	}

	if (!found) {
		mutex_unlock(&client_maps_mtx);
		cam_warn("%s: free daemon failed", __func__);
		return;
	}

	hisp_client_id_to_daemon_pid[cid].ref_counts--;
	if (hisp_client_id_to_daemon_pid[cid].ref_counts != fhid)
		swap(hisp_client_id_to_daemon_pid[cid].fh[fhid],
				hisp_client_id_to_daemon_pid[cid].fh[hisp_client_id_to_daemon_pid[cid].ref_counts]);

	cam_info("%s: free daemon %d client_id: %d refs: %d", __func__,
			hisp_client_id_to_daemon_pid[cid].pid, cid, hisp_client_id_to_daemon_pid[cid].ref_counts);

	if (hisp_client_id_to_daemon_pid[cid].ref_counts == 0) {
		hisp_client_id_to_daemon_pid[cid].pid = 0;
		isp_put_proc_mm(cid);
	}
	mutex_unlock(&client_maps_mtx);
}

int hisp_get_client_id(pid_t pid)
{
	int cid;
	mutex_lock(&client_maps_mtx);
	for (cid = 0; cid < MAX_HISP_CLIENT; ++cid)
		if (hisp_client_id_to_daemon_pid[cid].pid == pid) {
			mutex_unlock(&client_maps_mtx);
			return cid;
		}
	mutex_unlock(&client_maps_mtx);
	return -EBUSY;
}

int hisp_get_client_refs(pid_t pid)
{
	int cid;
	int refs;
	mutex_lock(&client_maps_mtx);
	for (cid = 0; cid < MAX_HISP_CLIENT; ++cid)
		if (hisp_client_id_to_daemon_pid[cid].pid == pid) {
			refs = hisp_client_id_to_daemon_pid[cid].ref_counts;
			mutex_unlock(&client_maps_mtx);
			return refs;
		}
	mutex_unlock(&client_maps_mtx);
	return -EBUSY;
}

static bool hisp_is_valid_daemon(pid_t pid)
{
#ifdef CONFIG_HISP_FUZZ
	(void)pid;
	return true;
#else
	int cid;
	mutex_lock(&client_maps_mtx);
	for (cid = 0; cid < MAX_HISP_CLIENT; ++cid)
		if (hisp_client_id_to_daemon_pid[cid].pid == pid) {
			mutex_unlock(&client_maps_mtx);
			return true;
		}
	mutex_unlock(&client_maps_mtx);
	return false;
#endif
}

int hisp_get_current_client_ids(int* client_ids)
{
	int num = 0;
	int cid;
	mutex_lock(&client_maps_mtx);
	for (cid = 0; cid < MAX_HISP_CLIENT; ++cid)
		if (hisp_client_id_to_daemon_pid[cid].pid != 0)
			client_ids[num++] = cid;
	mutex_unlock(&client_maps_mtx);
	return num;
}

/*
 * when get call back from rpmsg, notify
 * camera daemon throuth v4l2 event
 * i: hisp_notify_intf_t to find hisp_t object
 * hisp_ev: hisp event with specific rpmsg type
 */
static void hisp_notify_rpmsg_cb(hisp_notify_intf_t *i, hisp_event_t *hisp_ev)
{
	hisp_t *isp = NULL;
	uint8_t client_id;
	struct v4l2_event ev;
	struct video_device *vdev = NULL;
	hisp_event_t *req = (hisp_event_t *)ev.u.data;
	errno_t err = memset_s(&ev, sizeof(struct v4l2_event), 0,
						   sizeof(struct v4l2_event));
	if (err != EOK)
		cam_err("func %s: memset_s fail", __func__);

	if (!hisp_ev || !i) {
		cam_err("func %s: hisp_ev or i is NULL", __func__);
		return;
	}
	client_id = hisp_ev->kind >> HISP_MESSAGE_ID_CLIENT_BIT;
	cam_debug("enter %s, client_id: %d", __func__, client_id);

	isp = notify2hisp(i);
	if (!isp) {
		cam_err("func %s: isp is NULL", __func__);
		return;
	}
	hisp_assert(isp);
	vdev = isp->subdev.devnode;

	ev.type = HISP_V4L2_EVENT_TYPE;
	ev.id = HISP_HIGH_PRIO_EVENT | (client_id << HISP_MESSAGE_ID_CLIENT_BIT);

	req->kind = hisp_ev->kind & ((1 << HISP_MESSAGE_ID_CLIENT_BIT) - 1);

	v4l2_event_queue(vdev, &ev);
}

static hisp_notify_vtbl_t s_notify_hisp = {
										   .rpmsg_cb = hisp_notify_rpmsg_cb,
};

static void *hisp_subdev_get_info(hisp_t *isp, hisp_info_t *info)
{
	char *name = NULL;
	size_t min_len;

	if (!isp || !info) {
		cam_err("func %s: isp or info is NULL", __func__);
		return NULL;
	}
	name = hisp_intf_get_name(isp->hw);
	if (name) {
		errno_t err = memset_s(info->name, HISP_NAME_SIZE, 0,
							   HISP_NAME_SIZE);
		if (err != EOK)
			cam_err("func %s: memset_s fail", __func__);

		min_len = strlen(name);
		if (min_len > HISP_NAME_SIZE - 1)
			min_len = HISP_NAME_SIZE - 1;

		err = memcpy_s(info->name, HISP_NAME_SIZE, name, min_len);
		if (err != EOK)
			return NULL;

		return info->name;
	} else {
		cam_err("%s: hisp_intf_get_name() is NULL", __func__);
		return NULL;
	}
}

bool __weak modify_ack_timestamp_in_user(void)
{
	return false;
}

static void hisp_subdev_get_system_timestamp(hisp_system_time_t *isp_systime)
{
#if defined(HISP500_CAMERA)
	unsigned long long system_cntpct = __arch_counter_get_cntpct();
#else
	unsigned long long system_cntpct = 0;
#endif

	if (!isp_systime)
		return;

	g_timestamp_mode = isp_systime->mode;

	isp_systime->s_system_counter = __arch_counter_get_cntvct();
	isp_systime->s_system_us = modify_ack_timestamp_in_user() ?
		(g_timestamp_mode == TIMESTAMP_USE_BOOTTIME ?
		ktime_to_us(ktime_get_boottime()) : ktime_get_real_ns() / 1000) : 0;
	isp_systime->s_system_couter_rate = arch_timer_get_rate();

	cam_info("syscnt:%llu, syscntpct:%llu, sysus:%llu, syscnt_rate:%u, timestamp_mode:%d",
		isp_systime->s_system_counter,
		system_cntpct,
		isp_systime->s_system_us,
		isp_systime->s_system_couter_rate,
		g_timestamp_mode);

	if (g_system_couter_rate != 0) {
		isp_systime->s_system_counter =
			mult_frac(isp_systime->s_system_counter, g_system_couter_rate, isp_systime->s_system_couter_rate);
		isp_systime->s_system_couter_rate = g_system_couter_rate;
		cam_info("update syscnt:%llu, syscnt_rate:%u",
			isp_systime->s_system_counter, isp_systime->s_system_couter_rate);
	}
}

static long hisp_ioctl_select(unsigned int cmd, hisp_t *isp, void *arg)
{
	long rc = 0;
	size_t len = _IOC_SIZE(cmd);

	switch (cmd) {
	case HISP_IOCTL_CFG_ISP:
		cam_debug("Enter HISP_IOCTL_CFG_ISP");
		if (isp && isp->hw && isp->hw->vtbl)
			rc = isp->hw->vtbl->config(isp->hw, arg);
		break;
	case HISP_IOCTL_GET_INFO:
		cam_debug("Enter HISP_IOCTL_GET_INFO");
		if (!hisp_subdev_get_info(isp, (hisp_info_t *)arg)) {
			rc = -EFAULT;
			cam_err("func %s, line %d: ret = %ld",
					__func__, __LINE__, rc);
		}
		break;
	case HISP_IOCTL_POWER_ON:
		break;
	case HISP_IOCTL_GET_SYSTEM_TIME:
		cam_debug("Enter HISP_IOCTL_GET_SYSTEM_TIME");
		hisp_subdev_get_system_timestamp((hisp_system_time_t *)arg);
		break;
	case HISP_IOCTL_POWER_OFF:
		break;
	case HISP_IOCTL_SEND_RPMSG:
		cam_debug("Enter HISP_IOCTL_SEND_RPMSG");
		if (isp && isp->hw && isp->hw->vtbl) {
			if (!hisp_is_valid_daemon(current->tgid)) {
				cam_err("invalid SEND_RPMSG usr(cur %d)", current->tgid);
				return -EINVAL;
			}
			rc = isp->hw->vtbl->send_rpmsg(isp->hw, arg, len);
		}
		break;
	case HISP_IOCTL_RECV_RPMSG:
		cam_debug("Enter HISP_IOCTL_RECV_RPMSG");
		if (isp && isp->hw && isp->hw->vtbl) {
			if (!hisp_is_valid_daemon(current->tgid)) {
				cam_err("invalid RECV_RPMSG usr(cur %d)", current->tgid);
				return -EINVAL;
			}
			rc = isp->hw->vtbl->recv_rpmsg(isp->hw, arg, len);
		}
		break;
	default:
		cam_err("invalid IOCTL CMD(0x%x)", cmd);
		rc = -EINVAL;
		break;
	}

	return rc;
}

/* ioctl function for v4l2 subdev */
static long hisp_vo_subdev_ioctl(struct v4l2_subdev *sd,
	unsigned int cmd, void *arg)
{
	hisp_t *isp = NULL;

	if (!arg || !sd) {
		cam_err("func %s: arg or sd is NULL", __func__);
		return -1;
	}
	isp = SD2HISP(sd);
	if (!isp) {
		cam_err("func %s: isp is NULL", __func__);
		return -1;
	}
	cam_debug("cmd is (0x%x), arg(%pK)", cmd, arg);

	return hisp_ioctl_select(cmd, isp, arg);
}

#ifdef CONFIG_COMPAT
static long hisp_usercopy(
	struct v4l2_subdev *sd,
	unsigned int cmd,
	void *kp,
	void __user *up)
{
	long rc;

	rc = memset_s(kp, _IOC_SIZE(cmd), 0, _IOC_SIZE(cmd));
	if (rc != 0) {
		cam_err("%s memset_s return fail", __func__);
		return -EINVAL;
	}

	if (!up || !kp) {
		cam_err("func %s: up or kp is NULL", __func__);
		return -1;
	}

	if (_IOC_DIR(cmd) & _IOC_WRITE) {
		if (copy_from_user(kp, up, _IOC_SIZE(cmd))) {
			cam_err("%s: copy in arg failed", __func__);
			return -EFAULT;
		}
	}

	rc = hisp_vo_subdev_ioctl(sd, cmd, kp);
	if (rc < 0) {
		cam_err("%s subdev_ioctl failed", __func__);
		return -EFAULT;
	}

	switch (_IOC_DIR(cmd)) {
	case _IOC_READ:
	case (_IOC_WRITE | _IOC_READ):
		if (copy_to_user(up, kp, _IOC_SIZE(cmd))) {
			cam_err("%s: copy back arg failed", __func__);
			return -EFAULT;
		}
		break;
	default:
		break;
	}

	return rc;
}

static long hisp_ioctl_usercopy_data(void *vpdata, unsigned int size_data,
									 struct v4l2_subdev *sd, unsigned int cmd, unsigned long arg)
{
	long rc;

	if (!vpdata || !sd) {
		cam_err("func %s: up or kp is NULL", __func__);
		return -1;
	}

	if (_IOC_SIZE(cmd) > size_data) {
		cam_err("%s: cmd size too large", __func__);
		return -EINVAL;
	}

	rc = hisp_usercopy(sd, cmd, vpdata, (void __user *)arg);
	if (rc < 0)
		cam_err("%s: hisp_usercopy fail cmd = %d", __func__, cmd);

	return rc;
}

static long hisp_vo_subdev_compat_ioctl32(struct v4l2_subdev *sd,
										  unsigned int cmd, unsigned long arg)
{
	long rc = -EFAULT;

	switch (cmd) {
	case HISP_IOCTL_CFG_ISP: {
		struct hisp_cfg_data data;

		rc = hisp_ioctl_usercopy_data((void *)&data, sizeof(data),
									  sd, cmd, arg);
		break;
	}
	case HISP_IOCTL_GET_INFO: {
		hisp_info_t data;

		rc = hisp_ioctl_usercopy_data((void *)&data, sizeof(data),
									  sd, cmd, arg);
		break;
	}
	case HISP_IOCTL_GET_SYSTEM_TIME: {
		hisp_system_time_t data;

		rc = hisp_ioctl_usercopy_data((void *)&data, sizeof(data),
									  sd, cmd, arg);
		break;
	}
	case HISP_IOCTL_SEND_RPMSG: {
		struct hisp_cfg_data data;

		rc = hisp_ioctl_usercopy_data((void *)&data, sizeof(data),
									  sd, cmd, arg);
		break;
	}
	case HISP_IOCTL_RECV_RPMSG: {
		hisp_msg_t data;

		rc = hisp_ioctl_usercopy_data((void *)&data, sizeof(data),
									  sd, cmd, arg);
		break;
	}
	default:
		cam_err("invalid IOCTL CMD(0x%x)", cmd);
		break;
	}

	return rc;
}
#endif


__weak void hisp_clear_msgs(int client_id)
{
	(void)client_id;
}

static int hisp_subdev_subscribe_event(struct v4l2_subdev *sd,
									   struct v4l2_fh *fh, struct v4l2_event_subscription *sub)
{
	(void)sd;
	int client_id = hisp_get_client_id(current->tgid);
	if (client_id < 0 || client_id >= MAX_HISP_CLIENT)
		return -EBUSY;

	cam_info("enter %s, client_id: %d", __func__, client_id);
	sub->id |= (client_id << HISP_MESSAGE_ID_CLIENT_BIT);
	hisp_clear_msgs(client_id);
	/* 128:size */
	return v4l2_event_subscribe(fh, sub, 128, NULL);
}

static int hisp_subdev_unsubscribe_event(struct v4l2_subdev *sd,
										 struct v4l2_fh *fh, struct v4l2_event_subscription *sub)
{
	hisp_t* isp = SD2HISP(sd);
	int client_id = hisp_get_client_id(current->tgid);
	if (client_id < 0 || client_id >= MAX_HISP_CLIENT)
		return -EBUSY;

	cam_info("enter %s, client_id: %d", __func__, client_id);
	hisp_event_t isp_ev;
	if (!isp) {
		cam_err("func %s: isp is NULL", __func__);
		return -1;
	}
	isp_ev.kind = HISP_RPMSG_CB | (client_id << HISP_MESSAGE_ID_CLIENT_BIT);
	hisp_notify_intf_rpmsg_cb(&isp->notify, &isp_ev);
	return v4l2_event_unsubscribe(fh, sub);
}

static struct v4l2_subdev_core_ops s_subdev_core_ops_hisp = {
															 .ioctl = hisp_vo_subdev_ioctl,
#ifdef CONFIG_COMPAT
															 .compat_ioctl32 = hisp_vo_subdev_compat_ioctl32,
#endif
															 .subscribe_event = hisp_subdev_subscribe_event,
															 .unsubscribe_event = hisp_subdev_unsubscribe_event,
};

static int hisp_subdev_open(struct v4l2_subdev *sd, struct v4l2_subdev_fh *fh)
{
	hisp_t *isp = NULL;
	int rc = 0;

	if (!sd) {
		cam_err("func %s: sd is NULL", __func__);
		return -1;
	}
	isp = SD2HISP(sd);
	if (!isp) {
		cam_err("func %s: isp is NULL", __func__);
		return -1;
	}

	if (isp->hw && isp->hw->vtbl && isp->hw->vtbl->open)
		rc = isp->hw->vtbl->open(isp->hw);

	if (!rc)
		rc = hisp_allocate_client_id(current->tgid, fh);

	if (rc && isp->hw && isp->hw->vtbl) {
		if (isp->hw->vtbl->close)
			isp->hw->vtbl->close(isp->hw);
		else if (isp->hw->vtbl->close_at_orig_pid)
			isp->hw->vtbl->close_at_orig_pid(isp->hw, current->tgid);
	}

	return rc;
}

static int hisp_subdev_close(struct v4l2_subdev *sd, struct v4l2_subdev_fh *fh)
{
	hisp_t *isp = NULL;
	int rc = 0;

	if (!sd) {
		cam_err("func %s: sd is NULL", __func__);
		return -1;
	}
	isp = SD2HISP(sd);
	if (!isp) {
		cam_err("func %s: isp is NULL", __func__);
		return -1;
	}

	if (isp->hw && isp->hw->vtbl) {
		if (isp->hw->vtbl->close) {
			rc = isp->hw->vtbl->close(isp->hw);
		} else if (isp->hw->vtbl->close_at_orig_pid) {
			pid_t orig_pid = hisp_get_pid_by_fh(fh);
			cam_info("%s: close by orig_pid: %d", __func__, orig_pid);
			rc = isp->hw->vtbl->close_at_orig_pid(isp->hw, orig_pid);
		}
	}

	hisp_free_client_id(fh);

	return rc;
}

static struct v4l2_subdev_internal_ops s_subdev_internal_ops_hisp = {
																	 .open = hisp_subdev_open,
																	 .close = hisp_subdev_close,
};

static struct v4l2_subdev_ops s_subdev_ops_hisp = {
												   .core = &s_subdev_core_ops_hisp,
};

static int get_isp_dt_aclk(struct device_node *dev_node, hisp_dt_data_t *dt,
					struct device *dev)
{
	int ret;
	const char *clk_name = NULL;

	ret = of_property_read_string_index(dev_node, "clock-names",
										0, &clk_name);
	cam_info("%s: clk(%s)", __func__, clk_name);
	if (ret < 0) {
		cam_err("%s: could not get aclk name", __func__);
		return ret;
	}

	dt->aclk = devm_clk_get(dev, clk_name);
	if (IS_ERR_OR_NULL(dt->aclk)) {
		cam_err("%s: could not get aclk", __func__);
		ret = PTR_ERR(dt->aclk);
		return ret;
	}
	return 0;
}

static int get_isp_dt_aclk_dss(struct device_node *dev_node, hisp_dt_data_t *dt,
						struct device *dev)
{
	int ret;
	const char *clk_name = NULL;

	ret = of_property_read_string_index(dev_node,
										"clock-names", 1, &clk_name);
	cam_info("%s: clk(%s)", __func__, clk_name);
	if (ret < 0) {
		cam_err("%s: could not get aclk_dss name", __func__);
		return ret;
	}

	dt->aclk_dss = devm_clk_get(dev, clk_name);
	if (IS_ERR_OR_NULL(dt->aclk_dss)) {
		cam_err("%s: could not get aclk_dss", __func__);
		ret = PTR_ERR(dt->aclk_dss);
		return ret;
	}
	return 0;
}

static int get_isp_dt_pclk_dss(struct device_node *dev_node, hisp_dt_data_t *dt,
						struct device *dev)
{
	int ret;
	const char *clk_name = NULL;
	/* 2: Stores the specified string. */
	ret = of_property_read_string_index(dev_node,
										"clock-names", 2, &clk_name);
	cam_info("%s: clk(%s)", __func__, clk_name);
	if (ret < 0) {
		cam_err("%s: could not get pclk_dss name", __func__);
		return ret;
	}

	dt->pclk_dss = devm_clk_get(dev, clk_name);
	if (IS_ERR_OR_NULL(dt->pclk_dss)) {
		cam_err("%s: could not get pclk_dss", __func__);
		ret = PTR_ERR(dt->pclk_dss);
		return ret;
	} else {
		/* 120000000UL: size of clk */
		ret = clk_set_rate(dt->pclk_dss, 120000000UL);
		if (ret < 0)
			cam_err("%s: could not set pclk_dss rate",
					__func__);
	}
	return 0;
}

#define MAX_PINCTRL_NAME 32
#define PINCTRL_EXTRA_PAIRS "pinctrl-extra-pairs"
static int get_extra_pinctrl(struct device *dev, hisp_dt_data_t *dt)
{
	int idx;
	int ret;
	int extra_pairs;
	struct device_node *np = dev->of_node;
	char fullname[MAX_PINCTRL_NAME] = {0}; // store xx-default/xx-idle
	struct extra_pinctrl ep = {
		.name = NULL,
		.pinctrl_default = NULL,
		.pinctrl_idle = NULL,
	};

	extra_pairs = of_property_count_strings(np, PINCTRL_EXTRA_PAIRS);
	if (extra_pairs < 0) // not support extra pinctrl, it's ok
		return 0;
	if ((size_t)extra_pairs > ARRAY_SIZE(dt->extra_pinctrl)) {
		cam_err("%s count: %d exceeds max: %d", PINCTRL_EXTRA_PAIRS,
			extra_pairs, MAX_EXTRA_PINCTRL_PAIRS);
		return -EFAULT;
	}

	for (idx = 0; idx < extra_pairs; ++idx) {
		ret = of_property_read_string_index(
			np, PINCTRL_EXTRA_PAIRS, idx, &ep.name);
		if (ret < 0) {
			cam_err("fail to get index %d of %s", idx, PINCTRL_EXTRA_PAIRS);
			return ret;
		}

		ret = sprintf_s(fullname, sizeof(fullname), "%s-default", ep.name);
		if (ret < 0) {
			cam_err("extra pinctrl name: %s too long", ep.name);
			return ret;
		}
		ep.pinctrl_default = pinctrl_lookup_state(dt->pinctrl, fullname);
		if (IS_ERR_OR_NULL(ep.pinctrl_default)) {
			cam_err("fail to get pinctrl state: %s", fullname);
			return -ENODATA;
		}

		ret = sprintf_s(fullname, sizeof(fullname), "%s-idle", ep.name);
		if (ret < 0) {
			cam_err("extra pinctrl name: %s too long", ep.name);
			return ret;
		}
		ep.pinctrl_idle = pinctrl_lookup_state(dt->pinctrl, fullname);
		if (IS_ERR_OR_NULL(ep.pinctrl_idle)) {
			cam_err("fail to get pinctrl state: %s", fullname);
			return -ENODATA;
		}

		ret = pinctrl_select_state(dt->pinctrl, ep.pinctrl_idle);
		if (ret != 0) {
			cam_err("could not set pinctrl: %s to idle", ep.name);
			return ret;
		}
		cam_info("parsed extra pinctrl: %s, idx: %d", ep.name, idx);
		dt->extra_pinctrl[idx] = ep;
	}
	return 0;
}

__weak int hisp_extra_pinctrl_index(const char *name)
{
	(void)name;
	return -1;
}

__weak int hisp_extra_pinctrl_select_state(int idx, const char *state)
{
	(void)idx;
	(void)state;
	return -1;
}

static int set_isp_dt_parameter(struct device *dev, hisp_dt_data_t *dt)
{
	int ret = 0;

	dt->pinctrl = devm_pinctrl_get(dev);
	if (IS_ERR_OR_NULL(dt->pinctrl)) {
		cam_err("%s: could not get pinctrl", __func__);
		ret = PTR_ERR(dt->pinctrl);
		return ret;
	}

	dt->pinctrl_default = pinctrl_lookup_state(dt->pinctrl, PINCTRL_STATE_DEFAULT);
	if (IS_ERR_OR_NULL(dt->pinctrl_default)) {
		cam_err("%s: could not get default pinstate", __func__);
		return -1;
	}

	dt->pinctrl_idle = pinctrl_lookup_state(dt->pinctrl, PINCTRL_STATE_IDLE);
	if (IS_ERR_OR_NULL(dt->pinctrl_idle)) {
		cam_err("%s: could not get idle pinstate", __func__);
		return -1;
	}
	ret = pinctrl_select_state(dt->pinctrl, dt->pinctrl_idle);
	if (ret != 0) {
		cam_err("%s: could not set idle pinstate",
				__func__);
		return -1;
	}
	return get_extra_pinctrl(dev, dt);
}

int hisp_get_dt_data(struct platform_device *pdev, hisp_dt_data_t *dt)
{
	int ret;
	struct device *dev = NULL;
	struct device_node *dev_node = NULL;
	unsigned int is_fpga = 0;

	if (!pdev || !dt) {
		cam_err("%s: pdev or dt is NULL", __func__);
		return -1;
	}
	dev = &pdev->dev;
	dev_node = dev->of_node;

	if (!dev_node) {
		cam_err("%s: of node NULL", __func__);
		return -1;
	}

	ret = of_property_read_u32(dev_node, "vendor,is_fpga", &is_fpga);
	if (ret < 0)
		cam_err("%s: get FPGA flag failed", __func__);

	if (is_fpga) {
		cam_info("%s: Ignored FPGA", __func__);
		return 0;
	}

	ret = of_property_read_u32(dev_node, "vendor,system_couter_rate", &g_system_couter_rate);
	if (ret < 0)
		cam_err("%s: get system_couter_rate failed", __func__);

	ret = get_isp_dt_aclk(dev_node, dt, dev);
	if (ret < 0 || IS_ERR_OR_NULL(dt->aclk))
		return ret;

#if (!defined(HISP350_CAMERA) && !defined(HISP500_CAMERA) && !defined(HISP600_CAMERA))
	ret = get_isp_dt_aclk_dss(dev_node, dt, dev);
	if (ret < 0 || IS_ERR_OR_NULL(dt->aclk_dss))
		return ret;

	ret = get_isp_dt_pclk_dss(dev_node, dt, dev);
	if (ret < 0 || IS_ERR_OR_NULL(dt->pclk_dss))
		return ret;
#endif

	ret = set_isp_dt_parameter(dev, dt);
	if (ret < 0 || IS_ERR_OR_NULL(dt->pinctrl))
		return ret;

	if (hisp_dev_setpinctl(dt->pinctrl, dt->pinctrl_default,
							 dt->pinctrl_idle) < 0) {
		cam_err("%s Failed: setpinctl", __func__);
		return -1;
	}

#if (!defined(HISP350_CAMERA) && !defined(HISP500_CAMERA) && !defined(HISP600_CAMERA))
	if (hisp_dev_setclkdepend(dt->aclk_dss, dt->pclk_dss) < 0) {
		cam_err("%s Failed: clkdepend", __func__);
		return -1;
	}
#endif

	return 0;
}

/*
 * Initialize the hisp_t structure, called from Histar ISP probe
 * pdev: the rpmsg_channel used in Histar ISP driver
 * hw: to call the hook functions in Histar ISP driver
 * notify: get notified when Histar ISP driver needs
 */
int32_t hisp_register(struct platform_device *pdev, hisp_intf_t *hw,
					  hisp_notify_intf_t **notify)
{
	int rc = 0;
	struct v4l2_subdev *subdev = NULL;
	hisp_t *isp = NULL;
	int ret;

	cam_notice("%s enter", __func__);

	if (!notify) {
		cam_err("func %s: notify is NULL", __func__);
		return -1;
	}

	isp = kzalloc(sizeof(hisp_t), GFP_KERNEL);
	if (!isp) {
		rc = -ENOMEM;
		cam_err("func %s, line %d: out of memory", __func__,
				__LINE__);
		goto alloc_fail;
	}

	hisp_assert(hw);

	subdev = &isp->subdev;
	if (!subdev) {
		cam_err("func %s: subdev is NULL", __func__);
#if (LINUX_VERSION_CODE < KERNEL_VERSION(5, 10, 0))
		kzfree(isp);
#else
		kfree_sensitive(isp);
#endif
		return -1;
	}

	mutex_init(&isp->lock);

	v4l2_subdev_init(subdev, &s_subdev_ops_hisp);
	subdev->internal_ops = &s_subdev_internal_ops_hisp;
	ret = snprintf_s(subdev->name, sizeof(subdev->name),
					 sizeof(subdev->name) - 1, "%s", "cam-cfg-hisp");
	if (ret < 0)
		cam_err("func %s: snprintf_s fail", __func__);
	subdev->flags |= V4L2_SUBDEV_FL_HAS_DEVNODE;
	subdev->flags |= V4L2_SUBDEV_FL_HAS_EVENTS;

	init_subdev_media_entity(subdev, CAM_SUBDEV_HISP);
	cam_cfgdev_register_subdev(subdev, CAM_SUBDEV_HISP);
	subdev->devnode->lock = &isp->lock;

	platform_set_drvdata(pdev, subdev);
	isp->hw = hw;
	isp->notify.vtbl = &s_notify_hisp;
	*notify = &(isp->notify);

	/* register hisp dsm client */
	g_client_hisp = dsm_register_client(&g_dev_hisp);

alloc_fail:
	return rc;
}

void hisp_unregister(struct platform_device *pdev)
{
	struct v4l2_subdev *subdev = NULL;
	hisp_t *isp = NULL;

	if (!pdev)
		return;
	subdev = platform_get_drvdata(pdev);
	if (!subdev) {
		cam_err("%s: subdev is NULL", __func__);
		return;
	}
	isp = SD2HISP(subdev);
	if (!isp) {
		cam_err("%s: isp is NULL", __func__);
		return;
	}

	media_entity_cleanup(&subdev->entity);
	cam_cfgdev_unregister_subdev(subdev);
	mutex_destroy(&isp->lock);
#if (LINUX_VERSION_CODE < KERNEL_VERSION(5, 10, 0))
	kzfree(isp);
#else
	kfree_sensitive(isp);
#endif
}

int hisp_get_sg_table(int fd, struct device *dev, struct dma_buf **buf,
					  struct dma_buf_attachment **attach, struct sg_table **table)
{
	*buf = dma_buf_get(fd);
	if (IS_ERR_OR_NULL(*buf))
		goto err_dma_buf;
	*attach = dma_buf_attach(*buf, dev);
	if (IS_ERR_OR_NULL(*attach))
		goto err_dma_buf_attach;

	*table = dma_buf_map_attachment(*attach, DMA_BIDIRECTIONAL);

	if (IS_ERR_OR_NULL(*table)) {
		cam_err("%s Failed : dma_buf_map_attachment.%d",
				__func__, PTR_ERR(*table));
		goto err_dma_buf_map_attachment;
	}
	cam_info("func %s: dma_buf_map_attachment ok", __func__);
	return 0;
err_dma_buf_map_attachment:
	dma_buf_detach(*buf, *attach);
err_dma_buf_attach:
	dma_buf_put(*buf);
err_dma_buf:
	return -ENODEV;
}

void hisp_free_dma_buf(struct dma_buf **buf, struct dma_buf_attachment **attach,
					   struct sg_table **table)
{
	dma_buf_unmap_attachment(*attach, *table, DMA_BIDIRECTIONAL);
	dma_buf_detach(*buf, *attach);
	dma_buf_put(*buf);
	*table = NULL;
	*attach = NULL;
	*buf = NULL;
}

int hisp_set_sec_fw_buffer(struct hisp_cfg_data *cfg)
{
	int rc;
	uint32_t size;
	phys_addr_t phys;
	struct sg_table *sgt = NULL;

	sgt = cam_buf_get_sgtable(cfg->share_fd);
	if (IS_ERR_OR_NULL(sgt)) {
		cam_err("%s: error get sg_table of share_fd:%d",
				__func__, cfg->share_fd);
		return PTR_ERR(sgt);
	}

	phys = sg_phys(sgt->sgl);
	size = sgt->sgl->length;
	if (size < cfg->buf_size) {
		cam_err("%s: real size [%u] less than claimed [%u]",
				__func__, size, cfg->buf_size);
		rc = -ERANGE;
		goto err_fw_buf_size;
	}

	/*
	 * incase something happens, mdc is not release as expected,
	 * like Tomestone. we release it here before next set.
	 */
	if (g_hisp_secmem_set) {
		cam_info("%s: abnormal case, but do it", __func__);
		(void)hisp_release_sec_fw_buffer();
	}

	rc = hisp_secmem_pa_set(phys, size);
	if (rc < 0)
		cam_err("%s: pa set error:%d", __func__, rc);
	else
		g_hisp_secmem_set = true;

err_fw_buf_size:
	cam_buf_put_sgtable(sgt);
	return rc;
}

int hisp_release_sec_fw_buffer(void)
{
	int rc = hisp_secmem_pa_release();
	if (rc < 0)
		cam_err("%s: fail, rc:%d", __func__, rc);
	else
		g_hisp_secmem_set = false;
	return rc;
}

int hisp_set_mdc_buffer(struct hisp_cfg_data *cfg)
{
	int rc;
	uint32_t size;
	phys_addr_t phys;
	struct sg_table *sgt = NULL;

	if (!cfg) {
		cam_err("%s: input param is null", __func__);
		return -ERANGE;
	}

	sgt = cam_buf_get_sgtable(cfg->share_fd);
	if (IS_ERR_OR_NULL(sgt)) {
		cam_err("%s: error get sg_table of share_fd:%d",
				__func__, cfg->share_fd);
		return PTR_ERR(sgt);
	}

	phys = sg_phys(sgt->sgl);
	size = sgt->sgl->length;

	if (size < cfg->buf_size) { /*lint -e644*/ /*lint -e530*/
		cam_err("%s: real size [%u] less than claimed [%u]",
				__func__,
				size, cfg->buf_size); /*lint -e644*/ /*lint -e530*/

		rc = -ERANGE;
		goto err_buf_size;
	}

	/*
	 * incase something happens, mdc is not release as expected,
	 * like Tomestone. we release it here before next set.
	 */
	if (g_hisp_mdcmem_set) {
		cam_info("%s: abnormal case, but do it", __func__);
		(void)hisp_release_mdc_buffer();
	}

	rc = hisp_mdcmem_pa_set(phys, size, cfg->share_fd); /*lint -e644*/
	if (rc < 0)
		cam_err("%s: pa set error:%d", __func__, rc);
	else
		g_hisp_mdcmem_set = true;

err_buf_size:
	cam_buf_put_sgtable(sgt);
	return rc;
}

int hisp_release_mdc_buffer(void)
{
	int rc = hisp_mdcmem_pa_release();
	if (rc < 0)
		cam_err("%s: fail, rc:%d", __func__, rc);
	else
		g_hisp_mdcmem_set = false;
	return rc;
}

timestamp_mode hisp_get_timestamp_mode(void)
{
	return g_timestamp_mode;
}
