...
struct rpmsg_hisp220_service {
	...
	struct sk_buff_head queue[MAX_HISP_CLIENT];
...
static void hisp220_notify_rpmsg_cb(int client_id);
...
static int hisp220_power_off(hisp_intf_t *i, pid_t orig_pid);

static int hisp220_open(hisp_intf_t *i);
static int hisp220_close(hisp_intf_t *i, pid_t orig_pid);
...
static uint8_t hisp220_remote_processor_up(void)
{
	uint8_t up_count = 0;
	unsigned int i;
	...
	cam_info("func %s: up_count is %u", __func__, up_count);
	return up_count;
}
...
static hisp_vtbl_t s_vtbl_hisp220 = {
	...
	.open = hisp220_open,
	.close_at_orig_pid = hisp220_close,
};
...
static void hisp220_notify_rpmsg_cb(int client_id)
{
	cam_info("%s: notify client_id: %d", __func__, client_id);
	hisp_event_t isp_ev;
...
static void hisp220_save_rpmsg_data(const void *data, int len)
{
	...
	if (!skb) {
		cam_err("%s() %d failed: alloc_skb len is %d",
	...
	int client_id = get_client_id_by_msg_id(((hisp_msg_t *)data)->message_id);

	((hisp_msg_t *)skbdata)->message_id &= (1 << HISP_MESSAGE_ID_CLIENT_BIT) - 1;
	unsigned int api_name = ((hisp_msg_t *)data)->api_name;

	cam_info("%s: client_id: %d, message_id: %#x, message_id: %#x api_name: %#x", __func__, client_id,
		((hisp_msg_t *)data)->message_id, ((hisp_msg_t *)skbdata)->message_id, api_name);

	skb_queue_tail(&kernel_serv->queue[client_id], skb);

	wake_up_interruptible(&kernel_serv->readq);
	hisp220_notify_rpmsg_cb(client_id);

#define MSG_EVENT_SENT 0x3000
	if (api_name == MSG_EVENT_SENT) {
		int client_ids[MAX_HISP_CLIENT];
		int client_num = hisp_get_current_client_ids(client_ids);
		cam_debug("broadcast event to all client client_num:%d", client_num);
		for (i = 0; i < client_num; ++i) {
			if (client_ids[i] != client_id && client_ids[i] >= 0 && client_ids[i] < MAX_HISP_CLIENT) {
				skb_queue_tail(&kernel_serv->queue[client_ids[i]], skb_copy(skb, GFP_KERNEL));
				hisp220_notify_rpmsg_cb(client_ids[i]);
			}
		}
	}
...
static int hisp220_config(hisp_intf_t *i, void *cfg)
{
	...
	case HISP_CONFIG_POWER_ON:
		...
		if (hisp220_remote_processor_up()) {
			cam_warn("%s hisp220 is still on power-on state", __func__);
	...
	case HISP_CONFIG_POWER_OFF:
		...
		if (hisp220_remote_processor_up()) {
			...
			rc = hisp220_power_off(i, current->tgid);
		}
		mutex_unlock(&hisp_power_lock_mutex);
		break;
...
static int hisp220_close(hisp_intf_t *i, pid_t orig_pid)
{
	...
	if (hisp_get_client_refs(orig_pid) == 1 && hisp220_remote_processor_up()) {
		cam_warn("%s hisp220 is still on power-on state, power off it",
			__func__);
		rc = hisp220_power_off(i, orig_pid);
		if (rc != 0)
...
#pragma GCC visibility pop
	...
	if (s_timestamp_state == TIMESTAMP_INTIAL) {
        cam_info("%s timestamp already init", __func__);
        goto init_timestamp;
	}
...
static int hisp220_power_off(hisp_intf_t *i, pid_t orig_pid)
{
	...
	hisp220_t *hi = NULL;
	unsigned long current_jiffies = jiffies;
	int client_id = hisp_get_client_id(orig_pid);
	struct rpmsg_hisp220_service *kernel_serv = NULL;
	if (i == NULL)
		return -1;
...
static void hisp220_rpmsg_remove(struct rpmsg_device *rpdev)
{
	...
	for (i = 0; i < MAX_HISP_CLIENT; ++i)
		skb_queue_purge(&kernel_serv->queue[i]); // release isp msg nodes
...
static int hisp220_power_off_rpmsg(hisp_msg_t *msg, int client_id, size_t len)
{
#define COMMAND_ISP_CPU_POWER_OFF_REQUEST 4141
	struct rpmsg_hisp220_service *kernel_serv = rpmsg_local.kernel_isp_serv;
	struct sk_buff *skb = NULL;
	char *skbdata = NULL;
	int rc;

	if (msg->api_name != COMMAND_ISP_CPU_POWER_OFF_REQUEST) {
		return -EINVAL;
	}
	...
	mutex_lock(&kernel_serv->recv_lock);
	if (hisp220_remote_processor_up() <= 1) {
		cam_info("func %s: client id is %d, num <= 1", __func__, client_id);
		mutex_unlock(&kernel_serv->recv_lock);
		return -1;
	}

	cam_warn("%s() %d: two daemon run, skip power off rpmsg", __func__, __LINE__);

	skb = alloc_skb(len, GFP_KERNEL);
	if (!skb) {
		cam_err("%s() %d failed: alloc_skb len is %u", __func__, __LINE__, len);
		mutex_unlock(&kernel_serv->recv_lock);
		return -1;
	}

	skbdata = skb_put(skb, len);
	rc = memcpy_s(skbdata, len, msg, len);
	if (rc != 0)
		cam_err("%s: memcpy fail", __func__);
...
static int hisp220_send_rpmsg(hisp_intf_t *i, hisp_msg_t *from_user, size_t len)
{
	...
	if (!hisp220_power_off_rpmsg(msg, client_id, len))
		return rc;

	msg->message_id |= (client_id << HISP_MESSAGE_ID_CLIENT_BIT);
	cam_debug("%s: client_id:%d, api_name:%#x, message_id:%#x", __func__,
		client_id, msg->api_name, msg->message_id);

	mutex_lock(&kernel_serv->send_lock);
	/* if the msg is the first msg, let's treat it special */
	if (RPMSG_CONNECTED != kernel_serv->state) {
		cam_debug("%s() %d kernel_serv->state: %d", __func__, __LINE__, kernel_serv->state);
...
static int hisp220_recv_rpmsg(hisp_intf_t *i, hisp_msg_t *user_addr, size_t len)
{
	...
	if (skb_queue_empty(&kernel_serv->queue[client_id])) {
		cam_err("%s() %d skb_queue_empty client_id: %d", __func__, __LINE__, client_id);
		rc = -EIO;
		goto unlock_ret;
	}
	...
	skb = skb_dequeue(&kernel_serv->queue[client_id]);
	if (!skb) {
...
unlock_ret:
	mutex_unlock(&kernel_serv->recv_lock);
ret:
	cam_info("%s() %d end rc: %d", __func__, __LINE__, rc);
	return rc;
}
...
static int32_t hisp220_rpmsg_probe(struct rpmsg_device *rpdev)
{
	...
	for (i = 0; i < MAX_HISP_CLIENT; i++) {
		cam_info("%s init index i: %u", __func__, i);
		skb_queue_head_init(&kernel_serv->queue[i]);
	}
