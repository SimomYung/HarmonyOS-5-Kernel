...
#if defined(HISP370_CAMERA) && defined(CONFIG_SOC_PG_INFO)
...
extern void ois_ctrl_reset_ref_cnt(void);
#endif
...
static int hisp350_power_off(hisp_intf_t *i, pid_t orig_pid);

static int hisp350_open(hisp_intf_t *i);
static int hisp350_close(hisp_intf_t *i, pid_t orig_pid);
...
static hisp_vtbl_t s_vtbl_hisp350 = {
	...
	.open = hisp350_open,
	.close_at_orig_pid = hisp350_close,
};
	...
	void *data, int len, void *priv, u32 src)
{
	...
	if (client_id >= MAX_HISP_CLIENT) {
		cam_err("func %s: client id is %hhu, large than %d", __func__, client_id, MAX_HISP_CLIENT);
		return -EINVAL;
	}
...
static int hisp350_init_imu_buffer(void *cfg)
{
	...
	if ((ret == 0) && mem_info.cpu_addr != 0) {
		...
		imu_map_ref++;
		cam_info("%s: iova = %pK, size = %d", __FUNCTION__, (void *)(uintptr_t)pcfg->imu_param.iova, pcfg->imu_param.size);
	} else {
		ret = -ENOMEM;
...
static int hisp350_config_power_off(hisp_intf_t *i)
{
	...
	if (hisp350_remote_processor_up()) {
		...
		rc = hisp350_power_off(i, current->tgid);
	}
	mutex_unlock(&hisp_power_lock_mutex);
	return rc;
}
...
static int hisp350_close(hisp_intf_t *i, pid_t orig_pid)
{
	...
	if (hisp_get_client_refs(orig_pid) == 1 && hisp350_remote_processor_up()) {
		cam_warn("%s hisp350 is still on power-on state, power off it",
			__func__);

		rc = hisp350_power_off(i, orig_pid);
		if (rc != 0)
			cam_err("failed to hisp350 power off");

		if (!hisp350_remote_processor_up()) {
#ifndef CONFIG_KERNEL_CAMERA_ISPV360
			ois_ctrl_reset_ref_cnt();
#endif
...
{
#define COMMAND_ISP_CPU_POWER_OFF_REQUEST 4128
	int rc;
