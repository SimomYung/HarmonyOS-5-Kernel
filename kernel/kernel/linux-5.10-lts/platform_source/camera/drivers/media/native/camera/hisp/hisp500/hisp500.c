...
static int hisp500_close(hisp_intf_t *i, pid_t orig_pid);
...
static bool hisp500_set_remote_processor_shutdown(hisp_intf_t *i, pid_t orig_pid)
{
...
static hisp_vtbl_t s_vtbl_hisp500 = {
	...
	.open = hisp500_open,
	.close_at_orig_pid = hisp500_close,
};
...
static int hisp500_config_power_off(hisp_intf_t *i, struct hisp_cfg_data *pcfg)
{
	...
	if (hisp500_remote_processor_up()) {
		...
		if (hisp500_set_remote_processor_shutdown(i, current->tgid)) {
...
static int hisp500_close(hisp_intf_t *i, pid_t orig_pid)
{
	...
	if (hisp_get_client_refs(orig_pid) == 1 && hisp500_remote_processor_up()) {
		cam_warn("%s hisp500 is still on power-on state, power off it",
			__func__);

		if (hisp500_set_remote_processor_shutdown(i, orig_pid)) {
			rc = hisp500_power_off(i);
			if (rc != 0)
				cam_err("failed to hisp500 power off");
		}
	...
	struct device_attribute *attr, char *buf)
{
	(void)dev;
	(void)attr;
	cam_info("enter %s, isp power on:%u",
