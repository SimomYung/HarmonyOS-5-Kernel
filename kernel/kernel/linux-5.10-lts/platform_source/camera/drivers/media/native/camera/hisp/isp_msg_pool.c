...
struct isp_msg_node {
	...
	u64 busy_ns; // timestamp queued to busy queue
	uint32_t msg_size;
...
void clear_isp_msg_pool(struct isp_msg_pool *pool)
{
	...
	cam_info("%s: msg nodes: allocated:%d, freed:%u",
...
static struct isp_msg_node *get_idle_msg(struct isp_msg_pool *pool)
{
	...
	if (atomic_read(&pool->total_msg) >= MAX_CACHED_MSGS) {
		cam_err("allocated %d msgs, discard coming msg",
	...
	cam_debug("allocated %d msgs", atomic_read(&pool->total_msg));
	return node;
}
	...
	uint32_t len, struct isp_msg_header *hdr)
{
	...
	elapsed_ns = ktime_get_ns() - msg->busy_ns;
	if (elapsed_ns > 10 * NS_PER_MS) // 10ms
		cam_warn("msg: msg_id: %#x api: %#x waited for %lums",
			msghdr(msg->data)->message_id,
			msghdr(msg->data)->api_name, elapsed_ns / NS_PER_MS);
	put_idle_msg(pool, msg);
	return rc;
}
