...
#include "platform_v500/hjpg_softreset.h"
#elif defined(HISP370_CAMERA)
#include "platform_v350/hjpg_softreset.h"
#endif
...
static int hjpeg_poweroff_ippcomm(hjpeg_base_t *pjpeg_dev, bool disable_smmu_bypass);
...
#if defined(JPEGENC_SOFT_RESET)
static void hjpeg_soft_reset_cfg_reg_sets(void __iomem *temp_base, int i);
#endif
...
static void dump_ffa_info(void)
{
	cam_info("%s enter", __func__);
	...
	if (!s_hjpeg_ipp_comm->ffa_dump) {
		cam_info("%s: ffa_dump is null", __func__);
		return;
	}

	s_hjpeg_ipp_comm->ffa_dump();
}
#endif
...
static int hjpeg_soft_reset(hjpeg_base_t *phjpeg)
{
	...
	cam_info("%s, enter, subctrl=%pK, cvdr=%pK",
			 __func__, jpeg_top_base, cvdr_axi_base);
	for (i = 0; i < (int)ARRAY_SIZE(sr_cfg_reg_sets); ++i) {
		...
		hjpeg_soft_reset_cfg_reg_sets(temp_base, i);
...
static int hjpeg_encode_process(hjpeg_intf_t *i, void *cfg)
{
	...
	cam_debug("input_buffer_y:%pK, input_buffer_uv:%pK, output_buffer:%pK, output_size:%u",
			  (void *)(uintptr_t)pcfg->buffer.input_buffer_y,
			  (void *)(uintptr_t)pcfg->buffer.input_buffer_uv,
			  (void *)(uintptr_t)pcfg->buffer.output_buffer, pcfg->buffer.output_size);
...
static int power_control(hjpeg_base_t *phjpeg, bool on, bool disable_smmu_bypass)
{
	int ret;
	...
	case PC_IPPCOMM:
		if (on)
			...
			ret = hjpeg_poweroff_ippcomm(phjpeg, disable_smmu_bypass);
		break;
	default:
...
static int hjpeg_power_on(hjpeg_intf_t *i)
{
	...
	bool disable_smmu_bypass = false;
	...
	ret = power_control(phjpeg, true, false);
	if (ret != 0) {
	...
	/* config smmu */
	ret = hjpeg_smmu_config(&phjpeg->hw_ctl, &(phjpeg->phy_pgd), &status);
	if (status != 0) {
		disable_smmu_bypass = true;
	}
	if (ret != 0) {
...
powerup_error:
	if (power_control(phjpeg, false, disable_smmu_bypass) != 0)
...
static int hjpeg_power_off(hjpeg_intf_t *i)
{
	...
	ret = power_control(phjpeg, false, false);
	if (ret != 0)
...
static int hjpeg_poweron_ippcomm(hjpeg_base_t *pjpeg_dev)
{
	...
	if (ret != 0) {
		cam_err("%s: power failed %d", __func__, ret);
		goto failed_jpg_poweron;
	}
...
static int hjpeg_poweroff_ippcomm(hjpeg_base_t *pjpeg_dev, bool disable_smmu_bypass)
{
	...
	if (!disable_smmu_bypass) {
		cam_info("%s: do disable_smmu", __func__);
...
{
#if defined(JPEGENC_BYPASS) && defined(CONFIG_SOC_PG_INFO)
	if (platform_is_isp_all_tailor()) {
		cam_info("%s: is pg chip, bypass jpegenc\n", __func__);
		return true;
	}
#else
