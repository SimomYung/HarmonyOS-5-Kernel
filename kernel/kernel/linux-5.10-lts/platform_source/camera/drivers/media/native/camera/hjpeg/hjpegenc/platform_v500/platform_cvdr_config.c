 ...
 * Description: v500 platform cvdr register config
 * Create: 2023-07-03
 *
...
#define ISP_QCP_BASE_ADDR                              (0xE0F00000)
...
static void platform_config_cvdr_vp_wr_if_cfg(hjpeg_hw_ctl_t *hw_ctl, unsigned char *cvdr_base)
{
	...
	if (hw_ctl->smmu_type == ST_SMMUV3) /* v500 is sure SMMUv3, but might be smmu ipp. */
...
{
	if (!hw_ctl || !hw_ctl->cvdr_viraddr || !hw_ctl->subctrl_viraddr) {
	...
	// dump ipp top regs
	for (i = 0; i < 0x800; i += 4) {
		reg_addr = (void __iomem *)((char *)hw_ctl->subctrl_viraddr + i);
		cam_info("%s: IPP TOP reg: Offset:%#08x, Value:%#08x", __func__, i, ioread32(reg_addr));
	}
	
	// dump media2ctrl regs
	...
	uint32_t media2ctrl_offsets[] = {
		...
		0x848, 0x84C, 0x854, 0x858, 0x85c, 0x8a8, 0x8ac, 0x8b0, 0x8b4, 0x8b8
	};
	...
	// ispqic em
	void __iomem *ispqic_viraddr = NULL;
	ispqic_viraddr = ioremap(ISP_QCP_BASE_ADDR, 0x1000);
	if (IS_ERR_OR_NULL(ispqic_viraddr)) {
		cam_err("%s: media2 ctrl addr map failed", __func__);
		return;
	}
	
	uint32_t ispqic_em_offsets[] = { 0x8, 0x20, 0x28, 0x30, 0x34, 0x40 };
	for (i = 0; i < ARRAY_SIZE(ispqic_em_offsets); i++) {
		reg_addr = (void __iomem *)((char *)ispqic_viraddr + ispqic_em_offsets[i]);
		cam_info("%s: ispqic em reg: Offset:%#08x, Value:%#08x", __func__, ispqic_em_offsets[i], ioread32(reg_addr));
	}
	iounmap(ispqic_viraddr);
}
