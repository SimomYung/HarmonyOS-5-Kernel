...
static int do_cfg_smmuv3_ippcomm(hjpeg_hw_ctl_t *hw_ctl, int *status)
{
	int ret;
...
// NSV/CHS IPP UNSEC and ISP SEC Conflict, TBU Regs is SEC Mode, Kernel Code cannot access
#ifndef JPEGENC_SMMU_TBU_DUMP_BYPASS
...
int hjpeg_smmu_config(hjpeg_hw_ctl_t *hw_ctl, void *pgd, int *status)
{
	if (!hw_ctl || !pgd) {
		*status = -1;
	...
	case ST_SMMUV3:
		if (hw_ctl->power_controller == PC_IPPCOMM)
			return do_cfg_smmuv3_ippcomm(hw_ctl, status);
