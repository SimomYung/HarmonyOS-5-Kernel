...
int tof8801_app0_capture(void *tof_chip, int capture)
{
	...
	if (capture) {
		...
		tof8801_infomsg("tof capsettings: Reg0x%x: ", TOF8801_APP0_CMD_START);
		for(i = 0; i < sizeof(chip->app0_app.cap_settings.buf); i++) {
			tof8801_infomsg("%02x ", chip->app0_app.cap_settings.buf[i]);
		}
		tof8801_infomsg("\n");
...
int tof8801_app0_get_v2_results(struct tof_sensor_chip *chip)
{
	...
	if (error == 0) {
		...
		tof8801_infomsg("status=%2x %2x %2x, %2x %2x dist=%4d, xtalk=%u temp=%u\n",
