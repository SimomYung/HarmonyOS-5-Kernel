...
int tof8801_app0_wait_for_idle(void *tof_chip, unsigned long usec_timeout)
{
	...
	for (i = 0; i < (MAX_REGS / 8); ++i) {
		tof8801_dbgmsg("0x%02x: %02x %02x %02x %02x  %02x %02x %02x %02x\n", i * 8,
...
int tof8801_app0_capture(void *tof_chip, int capture)
{
	...
	if (capture) {
		...
		chip->app0_app.cap_settings.buf[3] |= 0x04;
		chip->app0_app.cap_settings.buf[6] |= 0x40;
		tof8801_dbgmsg("tof capsettings: Reg0x%x: ", TOF8801_APP0_CMD_START);
		for(i = 0; i < sizeof(chip->app0_app.cap_settings.buf); i++) {
			tof8801_dbgmsg("%02x ", chip->app0_app.cap_settings.buf[i]);
		}
		tof8801_dbgmsg("\n");
...
int tof8801_app0_clock_skew_correct_results(struct tof_sensor_chip *chip)
{
	...
	if (chip->skip_tolerance_calculate != 1) {
		/* apply filter */
...
int tof8801_app0_get_v2_results(struct tof_sensor_chip *chip)
{
	...
	if (error == 0) {
		...
		tof8801_dbgmsg("status=%2x %2x %2x, %2x %2x dist=%4d, xtalk=%u temp=%u\n",
                        result->status.status, result->status.registerContents, result->status.tid, \
                        result->data.resultNum, result->data.resultInfo.data, result->data.distPeak, \
                        chip->xtalk_peak, result->data.stateData.data[10]);
	}
	...
	chip->distance = result->data.distPeak;
...
int tof8801_app0_read_results(struct tof_sensor_chip *chip)
{
	...
	if (error)
 	...
 	if (((distance >= 400) && (distance != 8000)) || distance == 0) {
 		tof8801_infomsg("distance=%u, set iterations to 900k\n", distance);
 		iterations = 900000;
 	} else {
 		tof8801_infomsg("distance=%u, set iterations to 80k\n", distance);
 		iterations = 80000;
 	}
 	tof8801_app0_set_clk_iterations(chip, iterations);
 	/* chip takes iterations in 1000s */
 	iterations /= 1000;
 	*((__le16 *)chip->app0_app.cap_settings.iterations) = cpu_to_le16(iterations);
 	if (chip->app0_app.cap_settings.cmd != 0) {
 		error = tof8801_app0_capture((void*)chip, 1);
 		tof8801_infomsg("capture stop and start\n");
 	}
...
static int tof8801_app0_issue_gen_cmd(unsigned char cmd, struct tof_sensor_chip *chip)
{
	...
	tof8801_infomsg("tof8801_app0_issue_gen_cmd: %x %x %x %x | %x %x %x %x | %x %x %x\n",
			cmd_byte[0], cmd_byte[1], cmd_byte[2], cmd_byte[3], 
			cmd_byte[4], cmd_byte[5], cmd_byte[6], cmd_byte[7],
			cmd_byte[8], cmd_byte[9], cmd_byte[10]);
	error = tof_i2c_write(chip->client, TOF8801_APP0_CMD_START,
	...
	unsigned char cmd, struct tof_sensor_chip *chip)
{
	...
	error = tof_i2c_write(chip->client, TOF8801_APP0_CMD_START,
		...
		TOF8801_APP0_MAX_CMD_SIZE);
	cmd_byte = chip->app0_app.user_cmd.anon_cmd.buf;
	tof8801_infomsg("tof8801_app0_issue_force_cmd: %x %x %x %x | %x %x %x %x | %x %x %x\n",
		cmd_byte[0], cmd_byte[1], cmd_byte[2], cmd_byte[3], 
		cmd_byte[4], cmd_byte[5], cmd_byte[6], cmd_byte[7],
		cmd_byte[8], cmd_byte[9], cmd_byte[10]);
	return error;
