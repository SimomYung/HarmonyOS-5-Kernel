...
static void tof8801_configure_gpio(struct tof_sensor_chip *chip);
	...
	struct device_attribute *attr, const char *buf, size_t count)
{
	...
	tof8801_get_all_regs(tof_chip);
	for (i = 0; i < (MAX_REGS / 8); ++i) {
		tof8801_infomsg("0x%02x: %02x %02x %02x %02x  %02x %02x %02x %02x\n", i * 8,
			chip->shadow[i * 8 + 0], chip->shadow[i * 8 + 1], chip->shadow[i * 8 + 2], chip->shadow[i * 8 + 3],
			chip->shadow[i * 8 + 4], chip->shadow[i * 8 + 5], chip->shadow[i * 8 + 6], chip->shadow[i * 8 + 7]);
	}
	if (capture) {
...
static int tof_CE_toggle(struct i2c_client *client)
{
	struct tof_sensor_chip *chip = i2c_get_clientdata(client);
	int error;
	if (!chip || !chip->pdata || !chip->pdata->gpiod_enable) {
		tof8801_errmsg("chip is null\n");
...
static irqreturn_t tof_irq_handler(int irq, void *dev_id)
{
	tof8801_infomsg("tof_irq_handler enter");
 ...
 * @tof_chip: tof_sensor_chip pointer
 */
int tof8801_get_all_regs(struct tof_sensor_chip *tof_chip)
{
	int error;
	...
	struct tof_sensor_chip *chip, const char *buf, size_t count)
{
	...
	for (i = 0; i < count; ++i) {
		val |= chip->ext_calib_data.fac_data.data[i];
	}

	if (!val) {
		tof8801_errmsg("%s invalid calibration data\n", __func__);
		return -1;
	}
	...
	struct tof_sensor_chip *chip, hwlaser_calibration_data_t *p)
{
	...
	if (cal_data->is_read) {
		...
		memset(&(cal_data->u.dataL4), 0, sizeof(hwlaser_calibration_data_L2));
...
static int tof8801_laser_get_data(struct tof_sensor_chip *chip, void *p)
{
	...
	udata->u.data_1.confidence = (status == 255 || status == 0) ? 1 : 0;
...
static void Chip_init(struct tof_sensor_chip *chip)
{
	...
	AMS_MUTEX_UNLOCK(&chip->lock);
	tof_standby_operation(chip->client, STANDBY);
	return;
}

static void tof8801_enable_irq(struct tof_sensor_chip *chip)
{
	if(!chip)
		return;

	if(chip->irq_state == TOF8801_IRQ_DISABLED) {
		chip->irq_state = TOF8801_IRQ_ENABLED;
		enable_irq(chip->irq);
	}
	return;
}

static void tof8801_disable_irq(struct tof_sensor_chip *chip)
{
	if(!chip)
		return;

	if(chip->irq_state == TOF8801_IRQ_ENABLED) {
		chip->irq_state = TOF8801_IRQ_DISABLED;
...
long tof_ioctl(void *hw_data, unsigned int cmd, void *p)
{
	...
	case HWLASER_IOCTL_STOP:
		...
		tof8801_disable_irq(tof_chip);
		tof8801_get_register(tof_chip->client, 0xE0, &cpu_stat);
		tof_standby_operation(tof_chip->client, STANDBY);
		msleep(30);
		tof8801_get_register(tof_chip->client, 0xE0, &cpu_stat);
		tof8801_errmsg("tof8801 state 0xE0:0x%x", cpu_stat);
		break;
	case HWLASER_IOCTL_START:
		tof8801_infomsg("HWLASER_IOCTL_START");
		tof_standby_operation(tof_chip->client, WAKEUP);
		tof8801_enable_irq(tof_chip);
		period_store(&tof_chip->client->dev, 0, period, 3);
		capture_store(&tof_chip->client->dev, 0, capture_on, 2);
		if (tof_chip->skip_tolerance_calculate) {
			tof8801_infomsg("HWLASER_IOCTL_START end");
			break;
		}
		msleep(100);
		break;
	...
	case HWLASER_IOCTL_POWERON:
		...
		tof8801_configure_gpio(tof_chip);
		Chip_init(tof_chip);
		break;
	...
	struct tof_sensor_chip *tof_chip)
{
	...
	tof_chip->skip_tolerance_calculate = 0;
	rc = of_property_read_u32(node, "skip-tolerance-calculate", &tof_chip->skip_tolerance_calculate);
	tof8801_infomsg("skip-tolerance-calculate %d, rc %d\n", tof_chip->skip_tolerance_calculate, rc);
	if (rc < 0) {
		tof8801_infomsg("get skip-tolerance-calculate fail, rc %d", rc);
		tof_chip->skip_tolerance_calculate = 0;
	}
...
int tof_probe(struct i2c_client *client, const struct i2c_device_id *idp)
{
	...
	tof_chip->irq_state = TOF8801_IRQ_DISABLED;
	/* ioctrl interface */
