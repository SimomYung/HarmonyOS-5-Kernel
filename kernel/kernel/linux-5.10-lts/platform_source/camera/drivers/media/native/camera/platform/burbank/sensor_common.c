	...
	struct sensor_power_setting *power_setting, int state)
{
	...
	if (POWER_ON == state) {
		...
		if ((power_setting->delay != 0) && (sensor_info->ldo_pwdn_delay == 1))
			hw_camdrv_msleep(power_setting->delay);
		rc = 0;
	}
	...
	sensor_t *sensor)
{
	...
	rc = of_property_read_u32(of_node, "ldo_pwdn_delay", &sensor_info->ldo_pwdn_delay); /*lint !e64 */
	cam_info("%s ldo_pwdn_delay 0x%x, rc %d\n", __func__, sensor_info->ldo_pwdn_delay, rc);
	if (rc < 0) {
		sensor_info->ldo_pwdn_delay = 0;
		cam_warn("%s read ldo_pwdn_delay failed, rc %d, set default value %d\n",
		__func__, rc, sensor_info->ldo_pwdn_delay);
		rc = 0;
	}
	return rc;
fail:
