...
extern void mipi_occupy_notify(int panel_id, bool value);
static int hisensor_do_lcd_mipi_switch(hwsensor_intf_t *si, void *data)
{
	struct sensor_ncs_extend_cmd_info *cdata = data;
	uint32_t mipi_status = cdata->cmd.mipi_switch_info.mipi_status;
	mipi_occupy_notify(0, mipi_status);
	return 0;
}

extern int get_mipi_state(int panel_id);
static int hisensor_get_lcd_mipi_switch_status(hwsensor_intf_t *si, void *data)
{
	struct sensor_ncs_extend_cmd_info *cdata = data;
	cdata->cmd.mipi_switch_info.mipi_status = get_mipi_state(1);
	return 0;
}

static int hisensor_do_nsc_extend_cmd(hwsensor_intf_t *si, struct sensor_ncs_extend_cmd_info *data)
{
	uint32_t ret = 0;
	...
	switch (data->extend_cmd) {
	case NCS_EXTEND_CMD_SET_LCD_MIPI_SWITCH_OCCUPY:
		ret = hisensor_do_lcd_mipi_switch(si, data);
		break;
	case NCS_EXTEND_CMD_GET_LCD_MIPI_SWITCH_STATUS:
		ret = hisensor_get_lcd_mipi_switch_status(si, data);
		break;
	default:
		cam_err("%s. unknow extend cmd %d", __func__, data->extend_cmd);
	}
	return ret;
}
...
static int hisensor_config(hwsensor_intf_t *si, void *argp)
{
	...
	case SEN_CONFIG_POWER_ON:
		sensor->work_stage = data->cfg.work_stage.stage;
		if (!si->vtbl->power_up) {
	...
	case SEN_CONFIG_POWER_OFF:
		if (sensor->work_stage != data->cfg.work_stage.stage) {
			cam_warn("power off stage abnormal: pre work_stage not equal current. stage[pre:current]: %d:%d",
				sensor->work_stage, data->cfg.work_stage.stage);
		}
	...
	case SEN_CONFIG_NCS_EXTEND_CMD:
		ret = hisensor_do_nsc_extend_cmd(si, &(data->cfg.ncs_extend_cmd));
		break;
	default:
