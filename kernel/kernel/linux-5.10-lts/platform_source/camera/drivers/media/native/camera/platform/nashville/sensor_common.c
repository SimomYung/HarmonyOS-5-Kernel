...
static int hw_sensor_power_control(sensor_t *s_ctrl, bool on)
{
	...
	enum power_setting_type ps_type = PS_SENSOR;
	...
	if (s_ctrl->work_stage == SENSOR_WORK_STAGE_MATCHID) {
		cam_info("curr work stage %u", SENSOR_WORK_STAGE_MATCHID);
		ps_type = PS_MATCHID;
		if (pset_group_invalid(s_ctrl->power_group, PS_MATCHID)) {
			ps_type = PS_SENSOR;
		}
		cam_info("ps type %d", ps_type);
	}

	if (on)
		return sensor_enable_power_setting(s_ctrl->power_group, ps_type);
	sensor_disable_power_setting(s_ctrl->power_group, ps_type);
	return 0;
}
