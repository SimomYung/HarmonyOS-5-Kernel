...
static const struct parse_seq_string {
	...
	[PS_MATCHID] = {
		"vendor,matchid-power-seq-type", "vendor,matchid-power-seq-val",
		"vendor,matchid-power-seq-cfg-val", "vendor,matchid-power-seq-cfg-val-max",
		"vendor,matchid-power-seq-cfg-val-off", "vendor,matchid-power-seq-cfg-val-off-max",
		"vendor,matchid-power-seq-delay", "vendor,matchid-power-seq-delay-off"
	},
};
...
{
	if (ps_type != PS_SENSOR && ps_type != PS_MATCHID) {
		return false;
	}
...
int sensor_enable_power_setting(struct power_setting_group *grp, enum power_setting_type ps_type)
{
	...
	++grp->power_ref[ps_type];
	dev_info(grp->dev, "power_up: power-seq-on, ref: %u", grp->power_ref[ps_type]);
err_out:
	if (ret)
...
void sensor_disable_power_setting(struct power_setting_group *grp, enum power_setting_type ps_type)
{
	...
	if (grp->power_ref[ps_type] > 0)
		--grp->power_ref[ps_type];
	dev_info(grp->dev, "power_down: power-seq-off, ref:%u", grp->power_ref[ps_type]);
	mutex_unlock(&grp->power_lock);
}
...
int sensor_ext_vcm_power_on_off(struct power_setting_group *grp, bool on)
{
	...
	if (on)
		return sensor_enable_power_setting(grp, PS_VCM);
	sensor_disable_power_setting(grp, PS_VCM);
	return 0;
}
