...
typedef struct _tag_hwsensor_board_info {
	...
	unsigned int ldo_pwdn_delay; /* for ldo powerdown use */
    unsigned int support_avs; /* denote whether support avs */
    unsigned int avs_k; /* avs calib k */
    unsigned int avs_a; /* avs calib a */
    unsigned int avs_range[2]; /* avs threshold */
	unsigned int delta_v; /* delat v */
...
typedef struct _sensor_t {
	...
	enum sensor_work_stage_e work_stage;
} sensor_t;
