...
static int et5907_regulator_enable(struct regulator_dev *rdev)
{
	...
	int ret;
	rdev->use_count = 1;
	pr_info("%s  id= %d, use_count = %u\n", __func__, id, rdev->use_count);
	if (info == NULL) {
...
static void et5907_soft_reset(struct regmap *regmap)
{
	int ret;
	ret = regmap_write(regmap, ET5907_REG_RESET, ET5907_SOFT_RESET_VAL);
	pr_info("Et5907 softreset ret: %d\n", ret);
	mdelay(5); /* delay 5ms */
}

static bool et5907_support_spare_reg(struct device_node *np)
{
	int ret;
	u32 value;
	ret = of_property_read_u32(np, "support_spare_reg", &value);
	if (ret != 0 || value != 1)
		return false;
...
static int et5907_get_chip_id(struct i2c_client *i2c, struct device *dev,
    struct regmap *regmap, unsigned int *val)
{
	int i, ret;
	const unsigned short et5907_spare_addr[] = {
		ET5907_SPARE_REG1, ET5907_SPARE_REG2, ET5907_SPARE_REG3, ET5907_DEFAULT_REG};
	unsigned short ori_addr;

	ret = regmap_read(regmap, ET5907_REG_CHIPID, val);
	if (ret >= 0) {
		pr_info("Success to read chipId, ret: %d\n", ret);
		return ret;
	}

	if (!et5907_support_spare_reg(dev->of_node))
		return ret;

	ori_addr = i2c->addr;
	for (i = 0; i < ARRAY_SIZE(et5907_spare_addr); i++) {
		if (et5907_spare_addr[i] == ori_addr)
			continue;
		i2c->addr = et5907_spare_addr[i];
		ret = regmap_read(regmap, ET5907_REG_CHIPID, val);
		if (ret >= 0) {
			pr_info("Success to read chipId, addr %u ret: %d\n", i2c->addr, ret);
			et5907_soft_reset(regmap);
			i2c->addr = ET5907_DEFAULT_REG;
			ret = regmap_read(regmap, ET5907_REG_CHIPID, val);
			pr_info("Read after softreset, addr %u ret: %d\n", i2c->addr, ret);
			break;
		}
		pr_err("Failed to read chipId, addr %u ret: %d\n", i2c->addr, ret);
		i2c->addr = ori_addr;
	}
	return ret;
}
...
static int et5907_i2c_probe(struct i2c_client *i2c, const struct i2c_device_id *id)
{
	...
	ret = et5907_get_chip_id(i2c, chip->dev, chip->regmap, &data);
	if (ret < 0)
