...
static const char *ldo_status_str[] = {
	"camera pmic ldo1 have over current fault!!!",
	"camera pmic ldo2 have over current fault!!!",
	"camera pmic ldo3 have over current fault!!!",
	"camera pmic ldo4 have over current fault!!!",
	"camera pmic ldo5 have over current fault!!!",
	"camera pmic ldo6 have over current fault!!!",
	"camera pmic ldo7 have over current fault!!!"
};
...
static int hw_wl2868c_over_current(struct regulator_dev *rdev) {
    int ret, index;
    unsigned int val = 0;
    unsigned int addr, addrs;
	const unsigned int ldo_status_str_len = sizeof(ldo_status_str) / sizeof(ldo_status_str[0]);
    bool ocp_state = false;
	dmd_log_err("#### wl2868c have over current fault occurs!!!");
 
    for (addr = WL2868C_LDO1_STATUS; addr <= WL2868C_LDO7_STATUS; addr += 2) {
		addrs = addr + 1; /* clear faults register */
        ret = regmap_read(rdev->regmap, addr, &val);
        if (ret) {
            dmd_log_err("Failed to read DEVICE_ID reg: %d\n", ret);
            return ret;
        }
 
        if (val & WL2868C_LDO_OCP_MASK) {
            ocp_state = true;
			index = (addr - WL2868C_LDO1_STATUS) / 2;
			if (index >= 0 && index < ldo_status_str_len)
				dmd_log_err("%s", ldo_status_str[index]);
			else
				dmd_log_err("err index of ldo_status_str:%d", index);
			index += 1;
			ret = regmap_update_bits(rdev->regmap, addrs, BIT(1), BIT(1));
			if (ret) {
				dmd_log_err("Failed to set bit 1 of next reg: %d\n", ret);
				return ret;
			}
 
			msleep(10);
			ret = regmap_update_bits(rdev->regmap, addrs, BIT(1), 0);
			if (ret) {
				dmd_log_err("Failed to set bit 1 of next reg to 0: %d\n", ret);
				return ret;
			}
        }
    }
    ...
    if (ocp_state) {
		ret = regmap_update_bits(rdev->regmap, INT_LATCHED_CLR, BIT(4), BIT(4));
		if (ret < 0) {
			dmd_log_err("Failed to clear ocp_latched_sts in 22H!!!\n");
			return ret;
		}

        ret = hw_pmic_dmd(DSM_CAMPMIC_OVER_CURRENT_ERROR_NO, index);
        if (ret) {
            dmd_log_err("pmic over current dmd failed\n");
            return -EPERM;
        }
    }
...
static int hw_wl2868c_under_voltage_lockout(struct regulator_dev *rdev) {
    int ret;
	dmd_log_err("#### wl2868c have under voltage lockout fault occurs!!!");
 
	ret = regmap_update_bits(rdev->regmap, INT_LATCHED_CLR, BIT(0), BIT(0));
	if (ret < 0) {
		dmd_log_err("Failed to clear uvlo_latched_sts in 22H!!!\n");
		return ret;
	}

    ret = hw_pmic_dmd(DSM_CAMPMIC_UNDER_VOLTAGE_LOCKOUT_ERROR_NO, 0);
    if (ret) {
        dmd_log_err("pmic under voltage lockout dmd failed\n");
        return -EPERM;
    }
...
static int hw_wl2868c_thermal_shutdown(struct regulator_dev *rdev) {
	int ret;
	dmd_log_err("#### wl2868c have thermal shutdown fault occurs!!!");
 
	ret = regmap_update_bits(rdev->regmap, INT_LATCHED_CLR, BIT(3), BIT(3));
	if (ret < 0) {
		dmd_log_err("Failed to clear otst3_latched_sts in 22H!!!\n");
		return ret;
	}

    ret = hw_pmic_dmd(DSM_CAMPMIC_TSD_ERROR_NO, 0);
    if (ret) {
        dmd_log_err("pmic thermal shutdown dmd failed\n");
        return -EPERM;
    }
...
static int hw_wl2868c_thermal_warning(struct regulator_dev *rdev) {
	int ret;
	dmd_log_err("#### wl2868c have thermal warning fault occurs!!!");
 
	ret = regmap_update_bits(rdev->regmap, INT_LATCHED_CLR, BIT(2), BIT(2));
	if (ret < 0) {
		dmd_log_err("Failed to clear otst2_latched_sts in 22H!!!\n");
		return ret;
	}
 
    ret = hw_pmic_dmd(DSM_CAMPMIC_TSD_ERROR_NO, 0);
    if (ret) {
        dmd_log_err("pmic thermal warning dmd failed\n");
        return -EPERM;
    }
...
static int wl2868c_regulator_enable(struct regulator_dev *rdev)
{
	...
	pr_info("### wl2868c enable id %d name %s rc %d use_count %u #### .... \n",
		rdev->desc->id, rdev->desc->name, rc, rdev->use_count);
	return rc;
}
...
static int wl2868c_regulator_is_enabled(struct regulator_dev *rdev)
{
	...
	pr_info("wl2868c IS_ENABLED enter id %d %s ret %d use_count %u\n",
		rdev->desc->id, rdev->desc->name, rc, rdev->use_count);
...
static irqreturn_t wl2868c_irq_handler(int irq, void *dev_id)
{
	int ret;
	unsigned int val = 0;
	if (!dev_id) {
		dmd_log_err("wl2868c_irq_handler dev_id is invalid parameter!\n");
		return IRQ_HANDLED;
	}
	if (!((struct wl2868c *)dev_id)->rdev) {
		dmd_log_err("wl2868c_irq_handler dev_id->rdev is invalid parameter!\n");
		return IRQ_HANDLED;
	}
 
	struct wl2868c *chip = (struct wl2868c *)dev_id;
	struct regulator_dev *rdev = chip->rdev;
 
	pr_info("wl2868c_irq_handler happen\n");
 
	ret = regmap_read(rdev->regmap, INT_PENDING_STS, &val);
	if (ret) {
		dmd_log_err("Failed to read  INT_PENDING_STS reg:%d\n", ret);
		return ret;
	} else
		dmd_log_inf("success read INT_PENDING_STS val:0x%02x\n", val);
 
	if (val & WL2868C_UVLO_MASK)
		hw_wl2868c_under_voltage_lockout(rdev);
	if (val & WL2868C_OTST2_MASK)
		hw_wl2868c_thermal_warning(rdev);
	if (val & WL2868C_OTST3_MASK)
		hw_wl2868c_thermal_shutdown(rdev);
	if (val & WL2868C_OCP_MASK)
		hw_wl2868c_over_current(rdev);
 
	return IRQ_HANDLED;
}

static int wl2868c_irq_gpio_request(struct wl2868c *chip)
{
	int rc = 0;
	int irq = 0;
	unsigned int addr;
	...
	if (!chip) {
		dmd_log_err("wl2868c_irq_gpio_request chip is invalied paremeter!\n");
		return -EINVAL;
	}
			
	chip->rdev = wl2868c_rdevs[6];
	
	if (!(chip->rdev)) {
		dmd_log_err("wl2868c_irq_gpio_request chip->rdev is invalied paremeter!\n");
		return -EINVAL;
	}
 
	if (!gpio_is_valid(chip->chip_irq_pin))
		return -ENODEV;
	
	/* set enable corresponding interrupt */
	rc = regmap_write(chip->regmap, INT_EN_SET, 0x1D);
	if (rc < 0) {
		dev_err(chip->dev, "set interrupt failed!!!\n");
		return rc;
	}

	/* set ldo1-7 interrupt */
	for (addr = WL2868C_LDO1_OCP_CTL; addr <= WL2868C_LDO7_OCP_CTL; addr +=2) {
		rc = regmap_update_bits(chip->regmap, addr, BIT(2), BIT(2));
		if (rc) {
			dev_err(chip->dev, "Failed to set bit 2 to 1:%d\n", rc);
			return rc;
		}
	}
	gpio_request(chip->chip_irq_pin, "wl2868c irq gpio");
	gpio_direction_input(chip->chip_irq_pin);
	irq = gpio_to_irq(chip->chip_irq_pin);
	if (irq < 0) {
		pr_err("fail to map GPIO: %d to INT: %d\n", chip->chip_irq_pin, irq);
		rc = -EINVAL;
		goto exit_free_gpio;
	} else {
		pr_info("request irq: %d\n", irq);
		rc = request_threaded_irq(irq, NULL, wl2868c_irq_handler,
			IRQF_TRIGGER_HIGH | IRQF_ONESHOT, "wl2868c_irq", chip);
		if (rc) {
	...
	return rc;
exit_free_gpio:
	gpio_free(chip->chip_irq_pin);
	return rc;
}
 
static int wl2868c_parse_dt(struct device_node *np,
				struct wl2868c *chip)
{
	int ret = 0;
	...
	/* init gpio WL2868_ENr */
	chip->chip_cs_pin = of_get_named_gpio(np, "will,cs-gpios", 0);
	if (chip->chip_cs_pin < 0) {
		pr_err("get cs gpio: %d error\n",chip->chip_cs_pin);
		return -ENODEV;
	}
	ret = xpmic_gpio_ops(chip->chip_cs_pin, XPMIC_GPIO_PULL_HIGH);
	if (ret) {
		pr_err("GPIO%d request failed:%d \n", chip->chip_cs_pin, ret);
		goto error;
	}
	mdelay(10);
	pr_info("#### wl2868c_12c_probe cs_gpio: %d...\n", chip->chip_cs_pin);
 
	chip->chip_irq_pin = of_get_named_gpio(np, "will,irq-gpios", 0);
	if (chip->chip_irq_pin < 0)
		pr_err("get irq gpio: %d error\n", chip->chip_irq_pin);
	else
		pr_info("#### wl2868c_i2c_probe irq_gpio: %d...\n", chip->chip_irq_pin);
				...
				const struct i2c_device_id *id)
{
	...
	chip->dev_name = dev_name(&client->dev);
	chip->client = client;
	chip->dev = dev;
	node = dev->of_node;
 
	ret = wl2868c_parse_dt(node, chip);
	if (ret)
		pr_err("wl2868c parse dt err: %d\n", ret);
	...
	/* set ldo3 ocp */
	ret = regmap_write(chip->regmap, WL2868C_LDO3_OCP_CTL, 0xF0);
	if (ret < 0) {
		dev_err(chip->dev, "Set ldo4 ocp failed!!!\n");
		goto error;
	}
	/* set ldo1 ocp */
	ret = regmap_write(chip->regmap, WL2868C_LDO1_OCP_CTL, 0xF0);
	if (ret < 0) {
		dev_err(chip->dev, "Set ldo4 ocp failed!!!\n");
		goto error;
	}
	/* set ldo2 ocp */
	ret = regmap_write(chip->regmap, WL2868C_LDO2_OCP_CTL, 0xF0);
	if (ret < 0) {
		dev_err(chip->dev, "Set ldo2 ocp failed!!!\n");
		goto error;
	}

	/* set ldo7 ocp */
	ret = regmap_write(chip->regmap, WL2868C_LDO7_OCP_CTL, 0xF0);
	if (ret < 0) {
		dev_err(chip->dev, "Set ldo7 ocp failed!!!\n");
		goto error;
	}
	...
	if (chip->chip_irq_pin > 0) {
		ret = wl2868c_irq_gpio_request(chip);
		if (ret)
		pr_err("wl2868c irq gpio request err: %d\n", ret);
	}
