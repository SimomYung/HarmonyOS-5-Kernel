...
static void cam_vib_get_dt_data(void)
{
	g_cam_vib_notify_dev->open_resonance_time = INVALID_VALUE;
	int rc = of_property_read_u32(g_cam_vib_notify_dev->dev->of_node, "vendor,open_resonance_time",
		&(g_cam_vib_notify_dev->open_resonance_time));
	if (rc < 0)
		cam_info("%s failed", __func__);
}
	...
	unsigned long time, void *bar)
{
	...
	cam_info("Enter %s", __func__);
	int open_resonance_time = VID_SHAKE_MIN_DURATION;
	if(g_cam_vib_notify_dev->open_resonance_time != INVALID_VALUE)
		open_resonance_time = g_cam_vib_notify_dev->open_resonance_time;
	cam_info("open_resonance_time: %d", open_resonance_time);
	if (time >= open_resonance_time) {
