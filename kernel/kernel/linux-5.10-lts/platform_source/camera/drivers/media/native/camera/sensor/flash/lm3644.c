...
define_kernel_flash_mutex(lm3644);
...
static int hw_lm3644_get_dt_data(struct hw_flash_ctrl_t *flash_ctrl)
{
	...
	rc = of_property_read_u32(of_node, "vendor,manual-cfg-flag",
		&g_manual_flag);
	if (rc < 0)
		cam_info("%s vendor,manual-cfg-flag failed", __func__);

	cam_info("%s vendor,manual-cfg-flag %d, rc %d\n", __func__,
		g_manual_flag, rc);
	...
	if (g_manual_flag == 1) {
		...
		rc = of_property_read_u32(of_node, "vendor,flash-pin",
			&pdata->strobe);
		cam_info("%s vendor,flash-pin %d, rc %d\n", __func__,
			pdata->strobe, rc);
		if (rc < 0)
			cam_err("%s failed %d\n", __func__, __LINE__);
	}
	...
	rc = of_property_read_u32(of_node, "vendor,flash-type",
		&pdata->flash_type);
	cam_info("%s vendor,flash_type 0x%x, rc %d\n", __func__,
		pdata->flash_type, rc);
	if (rc < 0) {
		cam_err("%s failed %d\n", __func__, __LINE__);
		return rc;
...
static int hw_lm3644_torch_brightness_set(struct led_classdev *cdev,
	...
	struct device *dev)
{
	...
	flash_ctrl->cdev_torch.brightness_set_blocking = hw_lm3644_torch_brightness_set;
