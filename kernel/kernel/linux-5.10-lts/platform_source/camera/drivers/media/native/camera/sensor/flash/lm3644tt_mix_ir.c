...
static int hw_lm3644tt_mix_ir_get_dt_data(struct hw_flash_ctrl_t *flash_ctrl)
{
	...
	pdata->id_list_count = of_property_count_elems_of_size(node, "vendor,led-id", sizeof(u32));
	if (pdata->id_list_count > 0 && pdata->id_list_count <= HWFLASH_POSITION_MAX) {
		cam_info("%s:%d vendor,led-id set id_list_count:%d", __func__, __LINE__, pdata->id_list_count);
		rc = of_property_read_u32_array(node, "vendor,led-id",
			pdata->id_list, pdata->id_list_count);
		if (rc < 0) {
	...
	struct device_attribute *attr, const char *buf, size_t count)
{
	...
	g_lm3644tt_mix_ir_ctrl.mix_pos = flash_id;
	if (cdata.mode == STANDBY_MODE) {
		...
		if ((g_lm3644tt_mix_ir_ctrl.mix_pos == HWFLASH_POSITION_FORE) ||
			(g_lm3644tt_mix_ir_ctrl.mix_pos == HWFLASH_POSITION_REAR1))
			cdata.data =
...
static int hw_lm3644tt_mix_ir_torch_brightness_set_imp(struct led_classdev *cdev,
...
static int hw_lm3644tt_mix_ir_torch_brightness_set(struct led_classdev *cdev,
	enum led_brightness brightness)
{
	g_lm3644tt_mix_ir_ctrl.mix_pos = HWFLASH_POSITION_REAR;
	return hw_lm3644tt_mix_ir_torch_brightness_set_imp(cdev, brightness);
}

static int hw_lm3644tt_mix_ir_torch_brightness_f_set(struct led_classdev *cdev,
	enum led_brightness brightness)
{
	g_lm3644tt_mix_ir_ctrl.mix_pos = HWFLASH_POSITION_FORE;
	return hw_lm3644tt_mix_ir_torch_brightness_set_imp(cdev, brightness);
...
static ssize_t hw_lm3644tt_mix_ir_flash_get_id_list_show(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	int rc = 0;
	struct hw_lm3644tt_mix_ir_private_data_t *pdata = NULL;
	pdata = g_lm3644tt_mix_ir_ctrl.pdata;
	if (!pdata) {
	...
	if (pdata->id_list_count == LED_COUNT2) {
		rc = snprintf_s(buf, PAGE_SIZE, PAGE_SIZE - 1,"%d %d\n",
			g_lm3644tt_mix_ir_pdata.id_list[0],
			g_lm3644tt_mix_ir_pdata.id_list[1]);
	} else {
		rc = snprintf_s(buf, PAGE_SIZE, PAGE_SIZE - 1,"%d\n",
			g_lm3644tt_mix_ir_pdata.id_list[0]);
	}
...
static ssize_t hw_lm3644tt_mix_ir_flash_get_id_list_store(struct device *dev,
	struct device_attribute *attr,
	const char *buf,
	size_t count)
{
...
static struct device_attribute g_lm3644tt_mix_ir_flash_get_id_list =
	__ATTR(flash_get_id_list, 0660, hw_lm3644tt_mix_ir_flash_get_id_list_show,
	hw_lm3644tt_mix_ir_flash_get_id_list_store); /* 660:-wr-wr--- */
	...
	struct device *dev)
{
	...
	register_camerafs_attr(&g_lm3644tt_mix_ir_flash_get_id_list);
