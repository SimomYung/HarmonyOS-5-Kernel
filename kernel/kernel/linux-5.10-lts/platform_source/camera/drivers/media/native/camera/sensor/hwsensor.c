...
// ref count +1
static bool add_cmd_pid_to_list(struct list_head *cmd_pid_list, enum power_ctrl_type cmd, pid_t pid)
{
    int ref_count = 0;
	...
	cam_info("%s: cmd: %d ref_count: %d", __func__, cmd, ref_count);
	return ref_count == 1;
}

// ref count -1
static bool remove_cmd_pid_from_list(struct list_head *cmd_pid_list, enum power_ctrl_type cmd, pid_t pid, bool force)
{
	int ref_count = query_cmd_count(cmd_pid_list, cmd);
	if (ref_count == 0) {
		cam_info("%s: cmd: %d already removed", __func__, cmd);
		return false;
	}
		...
		pid_t orig_pid)
{
	...
	switch (data->cfgtype) {
		...
		case SEN_CONFIG_POWER_ON:
			power_type = true;
			do_power = add_cmd_pid_to_list(&s->cmd_pid_list, to_power_ctl_type(data->cfgtype), orig_pid);
			break;
		case SEN_CONFIG_UFS_AUTO_CONTROL_ON:
		case SEN_CONFIG_EXT_VCM_POWER_OFF:
		case SEN_CONFIG_POWER_OFF:
			power_type = true;
			do_power = remove_cmd_pid_from_list(&s->cmd_pid_list, to_power_ctl_type(data->cfgtype), orig_pid, force_down);
			break;
...
static long hwsensor_subdev_get_info(struct hwsensor_t *s, hwsensor_info_t *info)
{
	...
	{
		cam_cfg_err("sensor or sensor->board_info is null");
		return -1;
	}
	...
	info->avs_cfg.support_avs = sensor->board_info->support_avs;
	info->avs_cfg.k_val = sensor->board_info->avs_k;
	info->avs_cfg.a_val = sensor->board_info->avs_a;
