			    ...
			    unsigned long len, void *msg)
{
	...
	if (g_hkadc_debug == DEBUG_ON && len < DEBUG_MSG_LEN) {
		for (i = 0; i < len; i++)
...
static int adc_send_wait(int adc_channel, int ipc_header)
{
	...
	__pm_stay_awake(g_adc_dev->wlock);
	ret = adc_send_ipc_to_lpm3(adc_channel, ipc_header);
	if (ret != 0) {
		__pm_relax(g_adc_dev->wlock);
		return ret;
	}
...
static int __init adc_driver_init(void)
{
	...
	g_adc_dev->wlock = wakeup_source_register(NULL, "thermal_adc");
	if (g_adc_dev->wlock == NULL) {
		pr_err("%s wakeup source register failed\n", __func__);
		ret = -EACCES;
		goto err_wakeup_lock;
	}
	...
	return ret;
err_get_mbox:
	wakeup_source_unregister(g_adc_dev->wlock);
	g_adc_dev->wlock = NULL;
err_wakeup_lock:
	kfree(g_adc_dev->nb);
	g_adc_dev->nb = NULL;
err_adc_nb:
