...
#ifdef CONFIG_DDR_DFX_MNTN
bool get_cur_qice_freq_info(unsigned int *value)
{
	unsigned int *freq_addr = NULL;

	if (value == NULL) {
		pr_err("get_qice_freq_info, input value err\n");
		return false;
	}

	freq_addr = ioremap(QICE_FREQ_INFO_ADDR, sizeof(unsigned long));
	if (freq_addr == NULL) {
		pr_err("get_qice_freq_info, addr map failed\n");
		return false;
	}
	*value = (unsigned int)readl(freq_addr);
	iounmap(freq_addr);
	return true;
}
#endif
