...
struct devfreq_gpu_scene_aware_data {
	...
	unsigned int freq_update_cnt;
	ktime_t last_update;
	ktime_t last_suspend;
	bool ignore_util;
#ifdef CONFIG_GPUFREQ_HRTIMER
...
#ifdef CONFIG_GPUFREQ_HRTIMER
static unsigned int get_polling_ms(struct devfreq *devfreq)
{
	if (devfreq->profile->update_polling_ms)
		devfreq->profile->update_polling_ms(devfreq);
	return devfreq->profile->polling_ms;
}

static void gpufreq_hrtimer_start(struct devfreq *devfreq)
{
	...
	polling_ms = get_polling_ms(devfreq);
	if (polling_ms != 0)
		hrtimer_start(&data->timer, ms_to_ktime(polling_ms), HRTIMER_MODE_REL);
}

static enum hrtimer_restart gpufreq_hrtimer_notify(struct hrtimer *hrtimer)
{
	...
	data = container_of(hrtimer, struct devfreq_gpu_scene_aware_data, timer);
	polling_ms = get_polling_ms(data->devfreq);
	kthread_queue_work(&data->worker, &data->work);
	if (polling_ms == 0)
		return HRTIMER_NORESTART;
	hrtimer_forward_now(&data->timer, ms_to_ktime(polling_ms));

	return HRTIMER_RESTART;
}

static void gpufreq_hrtimer_cancel(struct devfreq *devfreq)
{
	struct devfreq_gpu_scene_aware_data *data = devfreq->data;
	...
	kthread_flush_work(&data->work);
}

static void gpufreq_work(struct kthread_work *work)
{
	int ret = 0;
	struct devfreq *devfreq = NULL;
	struct devfreq_gpu_scene_aware_data *data =
		container_of(work, struct devfreq_gpu_scene_aware_data, work);
...
static int gpufreq_hrtimer_work_init(struct devfreq *devfreq)
{
	struct devfreq_gpu_scene_aware_data *data = devfreq->data;
	struct hrtimer *timer = &data->timer;

	data->devfreq = devfreq;
	kthread_init_work(&data->work, gpufreq_work);
	kthread_init_worker(&data->worker);
	data->thread = kthread_run(kthread_worker_fn, &data->worker,
				   "gpufreq");

	if (IS_ERR(data->thread)) {
		pr_err("failed to create gpufreq thread: %ld\n", PTR_ERR(data->thread));
		return PTR_ERR(data->thread);
	}
#ifdef CONFIG_LIBLINUX
	trace_ldk_rvh_set_vip_prio(data->thread, 1);
#endif /* CONFIG_LIBLINUX */
	hrtimer_init(timer, CLOCK_MONOTONIC, HRTIMER_MODE_REL);
	timer->function = gpufreq_hrtimer_notify;

	return 0;
}

static void gpufreq_hrtimer_update_interval(struct devfreq *devfreq, unsigned int *delay)
{
	...
	if (!new_delay) {
		gpufreq_hrtimer_cancel(devfreq);
		return;
	}

	/* if current delay is zero, start polling with new delay */
	if (!cur_delay)
		gpufreq_hrtimer_start(devfreq);
}
#else
static inline int gpufreq_hrtimer_work_init(struct devfreq *devfreq __maybe_unused)
{
	return 0;
}

static inline void gpufreq_hrtimer_start(struct devfreq *devfreq __maybe_unused)
{
}

static inline void gpufreq_hrtimer_cancel(struct devfreq *devfreq __maybe_unused)
{
}
#endif
						 ...
						 unsigned long *freq)
{
	...
	if (data->user_set_util != 0)
		*freq = data->user_set_util / PERCENT_TRANSFER_DIV;
	...
	trace_devfreq_gpu_scene_aware_apply_limits(data->cl_boost,
		data->cl_boost_freq, data->user_set_freq,
		data->user_set_util, *freq);
}
					...
					unsigned long *freq)
{
...
#ifdef CONFIG_HECA_GPU_LTS
	if (heca_gpu_enable()) {
		update_heca_gpu_min_max_freq(gpu_min_freq(df), gpu_max_freq(df));
		return 0;
	}
#endif
	...
	*freq = stat.current_frequency;
	...
	data->utilisation = stat.busy_time;
	util = data->ignore_util ? 0 : stat.busy_time * PERCENT_TRANSFER_DIV / stat.total_time;
#ifdef trace_perf
	...
	if (data->user_set_freq > 0 || data->user_set_util > 0)
		goto check_barrier;
	...
	trace_devfreq_gpu_scene_aware(data->cur_policy->para[INDEX],
		...
		data->normalized_util, targetload, *freq);
check_barrier:
...
static ssize_t store_user_set_util(struct device *dev,
				   struct device_attribute *attr __maybe_unused,
				   const char *buf, size_t count)
{
	unsigned long input;
...
GPU_SCENE_AWARE_ATTR_RW(user_set_util);
...
static int gpu_scene_aware_init(struct devfreq *devfreq)
{
	...
	*(data->cur_policy->target_load) = TARGET_LOAD;
	...
	err = gpufreq_hrtimer_work_init(devfreq);
	if (err != 0) {
		pr_err("%s: hrtimer work init err %d\n", __func__, err);
...
#define IGNORE_UTIL_THRES_MS 4
static bool should_ignore_freq(struct devfreq_gpu_scene_aware_data *data)
{
	unsigned long delta;

	if (data->freq_update_cnt == 0)
		return false;
	delta = ktime_ms_delta(ktime_get(), data->last_update);
	if (delta <= IGNORE_UTIL_THRES_MS)
		return true;
	return false;
}
...
static void gov_suspend(struct devfreq *devfreq)
{
	...
	mutex_lock(&devfreq->lock);
	data->ignore_util = should_ignore_freq(data);
	ret = update_devfreq(devfreq);
	if (ret != 0)
		dev_err(&devfreq->dev, "%s update devfreq fail%d\n", __func__, ret);
	data->ignore_util = false;
	data->last_suspend = ktime_get();
	mutex_unlock(&devfreq->lock);
#else
#ifdef CONFIG_POWER_DUBAI
	struct devfreq_dev_status stat;
...
#endif /* CONFIG_GPU_RT_DEVFREQ */
	devfreq_monitor_suspend(devfreq);
#ifdef trace_perf
	trace_perf(clock_set_rate, "gpu_state", 0, 0);
#endif
}

#define IGNORE_RESUME_UTIL_THRES_MS	32
static void gov_resume(struct devfreq *devfreq)
{
	int ret;
	struct devfreq_gpu_scene_aware_data *data = devfreq->data;
	unsigned int time_delta = IGNORE_RESUME_UTIL_THRES_MS;

#ifdef CONFIG_FRAME_WINDOW_GPUFREQ
	time_delta = get_frame_window_size_ms();
#endif

	mutex_lock(&devfreq->lock);
	if (ktime_ms_delta(ktime_get(), data->last_suspend) >= time_delta) {
		data->ignore_util = true;
		ret = update_devfreq(devfreq);
		if (ret != 0)
			dev_err(&devfreq->dev, "%s update devfreq fail%d\n", __func__, ret);
		data->ignore_util = false;
	}
	data->freq_update_cnt = 0;
	mutex_unlock(&devfreq->lock);

	devfreq_monitor_resume(devfreq);

#ifdef trace_perf
	trace_perf(clock_set_rate, "gpu_state", 1, 0);
#endif
}
...
#ifdef CONFIG_GPUFREQ_HRTIMER
		gpufreq_hrtimer_update_interval(devfreq, (unsigned int *)data);
#else
