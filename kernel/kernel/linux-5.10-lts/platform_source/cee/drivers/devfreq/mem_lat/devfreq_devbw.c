...
static int devbw_target(struct device *dev, unsigned long *freq, u32 flags)
{
	...
	set_dev_freq(d, *freq);
	spin_unlock(&d->vote_spinlock);
	trace_perf(memlat_set_dev_freq, dev_name(dev), "periodic_update",
				  cpumask_any(&d->cpus), 0, 0, *freq);

	return 0;
}

static int devbw_get_dev_status(struct device *dev __maybe_unused,
				struct devfreq_dev_status *stat __maybe_unused)
{
	return 0;
}
				    ...
				    cpumask_t *mask)
{
	...
	for_each_possible_cpu(cpu) {
		...
		for (i = 0; ; i++) {
			...
			tmp_np = of_parse_phandle(mon_np, "target-dev", 0);
			      ...
			      void __iomem *reg_base)
{
	...
	if (of_property_read_u32(dev->of_node, "vote-reg-mask",
		...
		dev_info(dev, "[%s] node %s doesn't have vote-reg mask property!\n",
			 __func__, dev->of_node->name);
	} else {
		hw_vote->mbits = reg_mask;
	}

	if (of_property_read_u32(dev->of_node, "vote-value-rsb",
				 &rsb) != 0) {
		hw_vote->rsb = VOTE_RSB_DEFAULT;
		dev_info(dev, "[%s] node %s doesn't have vote-value-rsb property!\n",
			 __func__, dev->of_node->name);
	} else {
		hw_vote->rsb = rsb;
	}
...
#ifdef CONFIG_LIBLINUX
	trace_ldk_vh_devfreq_add_devbw(&gov_name);
#endif
...
static const struct of_device_id match_table[] = {
	{ .compatible = "devbw", .data = "memlat_c" },
	{ .compatible = "l3-devbw", .data = "l3_memlat_c"},
	{}
};
