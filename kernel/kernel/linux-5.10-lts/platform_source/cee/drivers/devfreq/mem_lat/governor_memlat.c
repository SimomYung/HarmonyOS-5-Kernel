...
static ssize_t show_map(struct device *dev, struct device_attribute *attr __maybe_unused,
			char *buf)
{
				  ...
				  char *buf)
{
	...
	for (i = 0; i < (unsigned int)(n->ntarget_ratios); i++)
			    ...
			    void *data __maybe_unused)
{
	...
	case CPU_PM_ENTER:
		...
		if (cores_pwrdn) {
			...
			trace_perf(memlat_set_dev_freq, dev_name(df->dev.parent),
						  "idle_vote",
						  cpu, d->min_core_freq,
						  0, d->prev_idle_freq);
		}
...
void stop_monitor(struct devfreq *df)
{
	...
	set_dev_votefreq(df->dev.parent, 0);
	trace_perf(memlat_set_dev_freq, dev_name(df->dev.parent), "stop_monitor",
				  0, 0, 0, 0);
				   ...
				   unsigned long *freq)
{
	...
	for (i = 0; i < hw->num_cores; i++) {
		...
		trace_perf(memlat_dev_meas, dev_name(df->dev.parent),
				      hw->core_stats[i].id,
				      (int)(hw->core_stats[i].inst_count),
	...
	if (max_freq != 0) {
		...
		trace_perf(memlat_dev_update, dev_name(df->dev.parent),
