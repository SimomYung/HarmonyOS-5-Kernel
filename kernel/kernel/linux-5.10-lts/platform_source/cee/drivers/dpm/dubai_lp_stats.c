...
#define DDR_VOTE_DATA_MAX	20524
...
#define LPM3_VOTE_DURATION_MAX 131070000
...
#ifdef CONFIG_LOWPM_DDR_STATE
static struct ddr_state_ip g_ddr_lp_stats_table[] = {
...
#ifdef CONFIG_LOWPM_DDR_STATE_ENHANCE
	{ DDR_CLK1X_NUM_NUM, "ddr_clk1x_num" },
	{ DDR_RCMD_NUM, "ddr_rcmd_num" },
	{ DDR_WCMD_NUM, "ddr_wcmd_num" },
	{ DDR_ACT_NUM, "ddr_active_num" },
	{ DDR_RNK_CHG_NUM, "ddr_rankchange_num" },
	{ DDR_RW_CHG_NUM, "ddr_rwchange_num" },
	{ DDR_RW_CHG_NUM, "ddr_rwchange_num" },
	{ DDR_TO_NUM, "ddr_timeout_num" },
	{ DDR_RHIT_NUM, "ddr_rowhit_num" },
#endif
};
#endif
...
static void update_lpm3_vote_info_resume(void)
{
	...
	for (client_id = 0; client_id < CLIENT_BUTT; client_id++) {
		...
		temp = lpm3_vote_data[client_id].duration;
		lpm3_vote_data[client_id].duration +=
			(((int64_t)readl(runtime_reserved_base + CLENT_VOTE_TIME_OFFSET(client_id)) > LPM3_VOTE_DURATION_MAX) ? 0 
				: (int64_t)readl(runtime_reserved_base + CLENT_VOTE_TIME_OFFSET(client_id)));
#ifdef CONFIG_HM_SR_PHONE
		pr_err("vote id:%d src=%ld incre=%ld dest=%ld", client_id, temp,
				(int64_t)readl(runtime_reserved_base + CLENT_VOTE_TIME_OFFSET(client_id)),
				lpm3_vote_data[client_id].duration);
#endif
		/* should clear here */
...
static int malloc_vote_time_data(void)
{
	...
	g_ddr_state_data_size = sizeof(struct dubai_transmit_t) +
			       sizeof(struct ddr_state) * ARRAY_SIZE(g_ddr_lp_stats_table) * MAX_DDR_FREQ_NUM;
