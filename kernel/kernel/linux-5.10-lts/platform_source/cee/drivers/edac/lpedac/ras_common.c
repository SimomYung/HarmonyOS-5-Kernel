			  ...
			  int inst_nr, int block_nr, const char *msg)
{
	...
	if (instance->nr_blocks > 0) {
		...
		if (block->counters.ce_count >= CE_MASK_CNT) {
			edac_device_printk(edac_dev, KERN_CRIT,
				"instance: %s block: %s ce count overflow, mask ce irq.\n",
				instance->name, block ? block->name : "N/A");
			ras_ce_mask(block_nr, pdata->mod_id);
		}
	}
	...
	read_cpu_ecc_dfx_info(&dfx_info, (is_l3_ecc(instance->name) ? L3_CPUID : err_data->cpu));
	...
	ras_ce_sh(edac_dev, err_data->cpu, instance->name);
			  ...
			  int inst_nr, int block_nr, const char *msg)
{
	...
	edac_device_printk(edac_dev, KERN_WARNING,"freq:%u,volt:%u-%u,temp:%d\n",
					   dfx_info.freq_mhz, dfx_info.logic_volt, dfx_info.mem_volt, dfx_info.temp);
	ras_de_sh(edac_dev, err_data->cpu, instance->name);
			  ...
			  int inst_nr, int block_nr, const char *msg)
{
	...
	ras_ue_sh(edac_dev, err_data->cpu, instance->name);
