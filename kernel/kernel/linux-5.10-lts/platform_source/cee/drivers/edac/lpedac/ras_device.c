...
unsigned int g_cpu_pll_source[FREQ_DOMAIN_MAX] = {0};
...
void read_cpu_ecc_dfx_info(struct cpu_ecc_dfx_info *info, unsigned int cpuid)
{
	...
	info->cluster = (cpuid == L3_CPUID) ? (FREQ_DOMAIN_MAX - 1) : topology_physical_package_id(cpuid);
	for (type = RAS_CPU_DFX_FREQ; type < RAS_CPU_DFX_INFO_TYPE_CNT; type++) {
		value = _read_cpu_ecc_dfx_info((u64)type, (u64)(info->cluster));
		if (type == RAS_CPU_DFX_FREQ)
			g_cpu_pll_source[info->cluster] = (unsigned int)((value >> CPU_PLL_SOURCE_SHIFT) & CPU_PLL_SOURCE_MASK);
		*(dfx_info[type]) = (unsigned int)value;
	}
}

void ras_ce_mask(unsigned int node, unsigned int mod_id)
{
	(void)atfd_ras_smc((u64)RAS_CE_MASK, (u64)NULL, node, mod_id);
}
#else
void read_cpu_ecc_dfx_info(struct cpu_ecc_dfx_info *info, unsigned int cpuid)
{
...
static void parse_info(int irq, struct edac_device_ctl_info *edac_dev)
{
	...
	if (!cpumask_test_cpu(smp_processor_id(), &irq_data[i].support_cpus)) {
		edac_device_printk(edac_dev, KERN_CRIT, "ECC interrupt response on unsupported cpu\n");
		return;
	}
