...
static u32 g_loadmonior_on_off_vote = LOADMONITOR_VOTE_OFF;
struct semaphore g_sem_loadmonior_on_off_vote;
...
#ifdef CONFIG_PERI1_LOADMONITOR
...
void sec_loadmonitor_switch_enable(bool open_sr_ao_monitor, enum LOADMONITOR_ON_OFF_VOTE vote)
{
	...
	down(&g_sem_loadmonior_on_off_vote);
	if (g_loadmonior_on_off_vote != LOADMONITOR_VOTE_OFF) {
		g_loadmonior_on_off_vote |= vote;
		up(&g_sem_loadmonior_on_off_vote);
		return;
	}
...
void sec_loadmonitor_switch_disable(u32 en_flags, bool open_sr_ao_monitor, enum LOADMONITOR_ON_OFF_VOTE vote)
{
	...
	down(&g_sem_loadmonior_on_off_vote);
	if (vote == LOADMONITOR_VOTE_BY_SR) {
		g_loadmonior_on_off_vote = LOADMONITOR_VOTE_OFF;
	} else {
		g_loadmonior_on_off_vote &= ~vote;
		if (g_loadmonior_on_off_vote != LOADMONITOR_VOTE_OFF) {
			up(&g_sem_loadmonior_on_off_vote);
