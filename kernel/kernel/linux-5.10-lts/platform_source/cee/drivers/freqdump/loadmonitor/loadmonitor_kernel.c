...
static struct wakeup_source *g_dubai_wake_lock;
...
static bool g_is_doing_sr_flag = false;
...
static int enable_monitor(void)
{
	if (g_loadmonitor_status == LOADMONITOR_OFF) {
		...
		sec_loadmonitor_switch_enable(false, LOADMONITOR_VOTE_BY_DUBAI);
...
static int disable_monitor(void)
{
	...
	if (g_loadmonitor_status == LOADMONITOR_ON) {
		...
		sec_loadmonitor_switch_disable(
			...
			false, LOADMONITOR_VOTE_BY_DUBAI);
...
static struct ioctrl_dpm_ldm_t dpm_ldm_func_table[] = {
	...
	{ DUBAI_IOCTL_GET_LITE_SIZE_INFO, ioctrl_get_dubai_lite_size },
	{ DUBAI_IOCTL_GET_LITE_DATA_INFO, ioctrl_get_dubai_lite_data },
#endif
...
static long dpm_loadmonitor_ioctrl(struct file *file, u_int cmd, u_long arg)
{
	...
	if (g_is_doing_sr_flag) {
		pr_info("doing sr, dubai ioctrl ignore\n");
		return -EFAULT;
	}

	__pm_stay_awake(g_dubai_wake_lock);
...
static int dpmldm_pm_callback(struct notifier_block *nb, unsigned long action, void *ptr)
{
	...
	case PM_HIBERNATION_PREPARE:
		pr_info("loadmonitor pm suspend +:status%u, action:%u\n",
			g_loadmonitor_status, action);
		g_is_doing_sr_flag = true;
		if (g_loadmonitor_status == LOADMONITOR_ON) {
			cancel_delayed_work_sync(&g_dpm_delayed_work);
			sec_loadmonitor_switch_disable(
				g_monitor_delay_value.monitor_enable_flags,
				g_open_sr_ao_monitor, LOADMONITOR_VOTE_BY_SR);
	...
	case PM_POST_HIBERNATION:
		pr_info("loadmonitor pm resume +:status%u, action:%u\n", g_loadmonitor_status, action);
		if (g_loadmonitor_status == LOADMONITOR_SUSPEND) {
			sec_loadmonitor_switch_enable(g_open_sr_ao_monitor, LOADMONITOR_VOTE_BY_SR);
...
#ifdef CONFIG_DUBAI_LITE_VOTE
	format_dubai_lite_data(&g_loadmonitor_data_kernel.lite);
#endif
...
static int __init dpmldm_init(void)
{
	...
	sema_init(&g_sem_loadmonior_on_off_vote, 1);
	g_dubai_wake_lock = wakeup_source_register(NULL, "dubai_wake_lock");
