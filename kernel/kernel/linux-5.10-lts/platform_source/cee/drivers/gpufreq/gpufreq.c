				 ...
				 unsigned int flags)
{
...
#if ((defined CONFIG_HECA) || (defined CONFIG_HECA_GPU_LTS))
	if (heca_gpu_enable())
		return 0;
#endif
	...
	df = gfdev->df;
	if (df == NULL) {
		dev_err(dev, "gpufreq_device df not initialized yet\n");
		return -EPERM;
	}
	...
	if (ret != 0) {
		...
		/*
		 * The only writer of df->last_status.current_frequency:
		 * devfreq_gpu_scene_aware_func() calls gpufreq_get_dev_status()
		 * at the beginning for getting gpu info to calc next freq.
		 * The only reader of df->last_status.current_frequency:
		 * devfreq_cooling_get_requested_power() directly reads this var
		 * since it can't call get_dev_status() due to lock order.
		 * So it's ok if IPA don't care of reading an out-of-date value.
		 * But when IPA update itself in very low interval, that matters
		 * very much otherwise IPA won't be aware of gpufreq limit set
		 * by its last polling.
		 * So, the devfreq framework and devfreq_cooling framework didn't
		 * solve this issue for us. We can implement something like
		 * devfreq_update_stats() in somewhere of gpu driver. But the
		 * simplest way is to keep df->last_status.current_frequency
		 * up-to-date here and it brings absolutely no harm.
		 */
		df->last_status.current_frequency = freq;
	}
...
static void gpufreq_update_polling_ms(struct devfreq *df)
{
	unsigned int new_polling_ms;

	if (df->profile->polling_ms == 0)
		return;

	new_polling_ms = get_frame_window_size_ms();
	if (likely(df->profile->polling_ms == new_polling_ms))
		return;
	trace_perf(gpufreq_change_polling, df->profile->polling_ms, new_polling_ms);
	df->profile->polling_ms = new_polling_ms;
}
#endif

#ifdef CONFIG_GPUFREQ_FREQ_HIDE
static unsigned long gpufreq_freq_hide_convert(unsigned long freq)
{
	if (freq >= CONFIG_FREQ_HIDE_GPU)
		freq = CONFIG_FREQ_SHOW_GPU;
	return freq;
}
static bool gpufreq_freq_hide_check(unsigned long freq)
{
	bool ret = false;
...
#ifdef CONFIG_HECA_GPU_LTS
int gpufreq_get_frequency(struct device *dev, unsigned long *freq)
{
	struct gpufreq_device *gfdev = (struct gpufreq_device *)dev->platform_data;

	if (get_gpu_heca_freq(freq))
		return 0;

	*freq = gfdev->df->previous_freq;
	return 0;
}
#endif
...
#ifdef CONFIG_HECA_GPU_LTS
	.get_cur_freq = gpufreq_get_frequency,
#endif
	.get_dev_status	= gpufreq_get_dev_status,
#ifdef CONFIG_FRAME_WINDOW_GPUFREQ
	.update_polling_ms = gpufreq_update_polling_ms,
#endif
#ifdef CONFIG_GPUFREQ_FREQ_HIDE
	.freq_hide_convert = gpufreq_freq_hide_convert,
	.freq_hide_check = gpufreq_freq_hide_check,
#endif
};
...
#ifdef CONFIG_GPU_REMOVE_FREQ
static bool is_need_remove_freq(void)
{
	struct soc_component_pg_info pg_info = {0};

	get_soc_pg_info_by_index(SOC_COMPONENT_GPU_CORE_INFO, &pg_info);

	return pg_info.tailor_strategy == SOC_COMPONENT_TAILOR_STRATEGY_2;
}

static int remove_freq_if_need(struct device *dev)
{
	struct dev_pm_opp *opp;
    unsigned long target_freq = ULONG_MAX;
	int ret;

	if (!is_need_remove_freq())
		return 0;
	...
	ret = dev_pm_opp_disable(dev, target_freq);
	if (ret == 0)
		dev_err(dev, "The freq %lu has been removed\n", target_freq);
	else
		dev_err(dev, "Failed to remove the freq %lu\n", target_freq);
...
#ifdef CONFIG_GPU_REMOVE_FREQ
	ret = remove_freq_if_need(gpufreq_dev);
	if (ret != 0)
		return ret;
#endif
