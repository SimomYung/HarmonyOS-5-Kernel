			  ...
			  const char *buf, size_t count)
{
	...
	case AI_ENTRY:
		...
		if (!l3e->mode_flag) {
			...
			lb_gid_enable(0);
			trace_perf(l3extension_switch, "enable forcedly");
		}
		spin_unlock(&l3e->enable_lock);
		break;
	case AI_EXIT:
		...
		if (l3e->mode_flag) {
			...
			queue_delayed_work(system_power_efficient_wq, &l3e->work,
					   msecs_to_jiffies(l3e->work_ms));
#endif
			trace_perf(l3extension_switch, "disable forcedly");
		}
		spin_unlock(&l3e->enable_lock);
		break;
	default:
	...
	if (g_dpc_extension_flag && will_enable) {
		...
		if (!l3e->enable_flag) {
			...
			if (try_en_cnt >= l3e->up_interval) {
				trace_perf(l3extension_switch, "enable normally");
				l3e->enable_flag = true;
				lb_gid_enable(0);
				try_en_cnt = 0;
			}
		...
		if (l3e->enable_flag) {
			...
			if (try_dis_cnt >= l3e->down_interval) {
				trace_perf(l3extension_switch, "disable normally");
				l3e->enable_flag = false;
				lb_gid_bypass(0);
				try_dis_cnt = 0;
			}
...
static void l3ex_handle_update(struct work_struct *work)
{
	...
	trace_perf(update_polling_info, algo->miss_bw,
				  algo->lb_hit_rate,
				  algo->l3_hit_rate,
				  delta);
...
#ifndef CONFIG_L3CACHE_WEOE_BLOCK
	spin_lock(&l3e->enable_lock);
#endif	
	if (l3e->enable_flag) {
		l3e->enable_flag = false;
