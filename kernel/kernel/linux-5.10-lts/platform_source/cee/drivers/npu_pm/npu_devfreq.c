...
#elif defined(CONFIG_NPU_PM_PMC)
#include "pmc/npu_pm_private.h"
#else
...
#include "npu_freq_cooling.h"
#endif

#ifdef CONFIG_NPU_PM_REMOVE_FREQ
#include <soc_spec.h>
#endif
...
int npu_kobject_set_name_vargs(struct kobject *kobj, const char *fmt, va_list vargs)
{
	const char *s;
...
int npu_dev_set_name(struct device *dev, const char *fmt, ...) {
	dev_err(dev, "npu_dev_set_name: before dev_name: %s\n", dev_name(dev));

	va_list vargs;
	int err;

	va_start(vargs, fmt);
	err = npu_kobject_set_name_vargs(&dev->kobj, fmt, vargs);
	va_end(vargs);

	dev_err(dev, "npu_dev_set_name: after dev_name: %s\n", dev_name(dev));
...
int npu_pm_devfreq_init(struct npu_pm_device *pmdev)
{
	...
	if (strncmp(NPU_PM_DEVFREQ_DEV_NAME, dev_name(dev), strlen(NPU_PM_DEVFREQ_DEV_NAME)) != 0) {
		...
		rename_ret = npu_dev_set_name(dev, NPU_PM_DEVFREQ_DEV_NAME);
		if (rename_ret != 0)
			dev_err(dev, "npu_dev_set_name fail %d\n", rename_ret);
	}

	dev_err(dev, "final npu dev name: %s\n", dev_name(dev));
...
#if defined(CONFIG_THERMAL_NPU) || defined(CONFIG_THERMAL_NPU_V2)
#ifdef CONFIG_THERMAL_EX_SUSTAIN_POWER
	if (npu_power_ratios_init(pmdev, np) != 0)
		goto err;
#endif /* CONFIG_THERMAL_EX_SUSTAIN_POWER */
...
#endif /* CONFIG_THERMAL_NPU_V2 */
	return 0;
err:
#ifdef CONFIG_THERMAL_EX_SUSTAIN_POWER
	kfree(pmdev->power_ratios);
#endif /* CONFIG_THERMAL_EX_SUSTAIN_POWER */
	return -EFAULT;

#endif /* CONFIG_THERMAL_NPU || CONFIG_THERMAL_NPU_V2 */
