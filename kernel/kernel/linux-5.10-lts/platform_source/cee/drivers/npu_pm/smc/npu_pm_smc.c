...
#define NPU_AVS_VOLT_MAX_LENGTH	10U
...
enum {
	...
	NPU_PM_SMC_INIT_REQ,
	NPU_PM_SMC_SH_VOLT_REQ,
	NPU_PM_SMC_GET_POWER_VOTE_REQ,
};
...
int npu_pm_sh_volt_smc_request(unsigned int profid, unsigned int delta_volt, unsigned int enable)
{
	return npu_pm_smc_request(NPU_PM_SMC_SH_VOLT_REQ,
				       (u64)profid,
				       (u64)delta_volt,
				       (u64)enable);
}
...
EXPORT_SYMBOL(npu_pm_get_voltage_smc_request);

int npu_pm_get_power_vote_smc_request(void)
{
#ifdef CONFIG_NPU_PM_DEBUG
	return npu_pm_smc_request(NPU_PM_SMC_GET_POWER_VOTE_REQ,
				       0,
				       0,
				       0);
#else
...
EXPORT_SYMBOL(npu_pm_get_power_vote_smc_request);

#ifdef CONFIG_NPU_PM_DMD_REPORT
static int npu_pm_get_avs_voltage_smc_request(void)
{
	int ret;
	unsigned int i;
	struct ffa_send_direct_data args = {
		.data0 = NPU_PM_TZSP_KERNEL_RD_AVS,
		.data1 = 0,
		.data2 = 0,
	};
	unsigned short rx_buf[NPU_AVS_VOLT_MAX_LENGTH] = { 0 };

	ret = ffa_platdrv_send_msg_with_shmem(&args, (u8 *)&rx_buf[0], SHMEM_OUT);
	if (ret != 0) {
		pr_err("%s %d, send ffa msg to tzsp with shared memory fail, proc_ret=0x%x\n", __func__, __LINE__, ret);
		return -EINVAL;
	}

	bbox_diaginfo_record(LOWPOWER_NPU_AVS_VAL, NULL, "NPU_AVS:,%#x,%#x,%#x,%#x,%#x", rx_buf[0], rx_buf[1],
					   rx_buf[2], rx_buf[3], rx_buf[4]);
	bbox_diaginfo_record(DMD_NPU_PA_SENSOR, NULL, "NPU_PASENSOR:,%#x,%#x", rx_buf[5], rx_buf[6]);
...
int npu_pm_sh_volt(unsigned int profid, unsigned int delta_volt, unsigned int enable)
{
	...
	ret = npu_pm_sh_volt_smc_request(profid, delta_volt, enable);
	if (ret != 0)
		pr_err("%s %d: npu_pm_sh_volt failed, ret: %d!\n", __func__, __LINE__, ret);
	mutex_unlock(&power_mutex);

	return ret;
}
EXPORT_SYMBOL(npu_pm_sh_volt);
...
EXPORT_SYMBOL(npu_pm_power_off);
...
#ifdef CONFIG_NPU_PM_DMD_REPORT
	(void)npu_pm_get_avs_voltage_smc_request();
#endif
