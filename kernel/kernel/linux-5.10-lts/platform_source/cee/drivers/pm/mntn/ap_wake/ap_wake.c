...
static void show_wake_irq_name(struct seq_file *s)
{
	...
	if (g_lpmcu_irq_name == NULL || wake_irq >= g_lpmcu_irq_num || wake_irq < 0) {
...
#if defined(CONFIG_POWER_DUBAI) && defined(CONFIG_HM_SR_PHONE)
	dubai_log_irq_wakeup_isrtc(irq_name);
#endif
}
