...
struct wake_ipc_info {
	...
	const enum IPC_TYPE ipc_type;
	const int reg_index;
};

static struct wake_ipc_info g_ns_ipc_list[] = {
	{
		/* g_ns_ipc */
		.desc = "nsipc",
		.runtime_offset = AP_NSIPC_IRQ_OFFSET,
		.reg_domain = IPC,
		.ipc_type = PERI_NS_NORMAL_IPC,
		.reg_index = 0,
	},
	{
		/* g_ns_ipc1 */
		.desc = "nsipc1",
		.runtime_offset = AP_NSIPC_IRQ1_OFFSET,
		.reg_domain = IPC,
		.ipc_type = PERI_NS_NORMAL_IPC,
		.reg_index = 1,
	},
...
static void combo_ipc_irq_show(struct seq_file *s, const struct wake_ipc_info *ipc)
{
	...
	for (i = 0; i < ipc_mbx_num; i++) {
		...
		ipc_mbx_info_show(s, ipc->mbxs[i].index + ipc->reg_index * REG_BIT_NUM, ipc);
	}
...
void show_wake_ipc(struct seq_file *s)
{
	...
	for (i = 0; i < ARRAY_SIZE(g_ns_ipc_list); i++)
		combo_ipc_irq_show(s, &g_ns_ipc_list[i]);
}
...
static int read_ipc_mbx_info(struct device_node *ipc_dn, struct wake_ipc_info *ipc_info)
{
	...
	for_each_child_of_node(ipc_dn, mbx_dn) {
		...
		if ((of_property_read_string(mbx_dn, "rproc", &(ipc_info->mbxs[i].name)) != 0) &&
...
{
	int i;
	struct wake_ipc_info *ipc_info = NULL;

	for (i = 0; i < ARRAY_SIZE(g_ns_ipc_list); i++) {
		ipc_info = &g_ns_ipc_list[i];
		if (map_reg_base(ipc_info->reg_domain) != 0) {
			lp_err("map %s reg failed", ipc_info->desc);
			return -EFAULT;
		}
	}
	return 0;
...
int init_ipc_table(const struct device_node *lp_dn)
{
	...
	for_each_matching_node(dn, ipc_dev_match) {
		...
		for (i = 0; i < ARRAY_SIZE(g_ns_ipc_list); i++) {
			ipc_info = &g_ns_ipc_list[i];
			if (ipc_type == ipc_info->ipc_type) {
				ret = init_ns_ipc_table(dn, ipc_info);
				if (ret != 0) {
					lp_err("init %s failed %d", ipc_info->desc, ret);
					return -ENODEV;
				}
				lp_info("init %s success", ipc_info->desc);
			}
		}
	}
