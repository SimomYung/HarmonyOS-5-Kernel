...
#endif
	lp_msg_cont(s, "\n");
#ifdef LPMCU_MAX_WAKEUP_IRQ_OFFSET
	lp_msg(s, "SR:lpmcu max wakeirq:[0x%x].", _runtime_read(LPMCU_MAX_WAKEUP_IRQ_OFFSET));
#endif
#if defined(DDR_CANNOT_ENTER_DEEPSLEEP_CNT_OFFSET) && defined(MODEM_CANNOT_ENTER_DEEPSLEEP_CNT_OFFSET)
	lp_msg(s, "SR:can't enter deepsleep count, ddr:[0x%x] modem:[0x%x].", _runtime_read(DDR_CANNOT_ENTER_DEEPSLEEP_CNT_OFFSET),
		_runtime_read(MODEM_CANNOT_ENTER_DEEPSLEEP_CNT_OFFSET));
#endif
#ifdef SLEEP_FAIL_ITEM_OFFSET
	lp_msg(s, "SR:sleep fail to enter for items 0x%x.", _runtime_read(SLEEP_FAIL_ITEM_OFFSET));
#endif
}

static void show_subsys_sr_times(struct seq_file *s)
{
#ifdef SWITCH_MODE_CNT_OFFSET
	lp_msg(s, "SR:switch str %u times", _runtime_read(SWITCH_MODE_CNT_OFFSET));
#endif
	lp_msg(s, "SR:system sleeped %u times.", _runtime_read(SYS_DSLEEP_CNT_OFFSET));
#ifdef SYS_STANDBY_CNT_OFFSET
	lp_msg(s, "SR:system standbyed %u times.", _runtime_read(SYS_STANDBY_CNT_OFFSET));
#endif
#ifdef SYS_ULSR_CNT_OFFSET
	lp_msg(s, "SR:system ulsr %u times.", _runtime_read(SYS_ULSR_CNT_OFFSET));
#endif
#ifdef SYS_LAST_MODE_DSLEEP_CNT_OFFSET
	lp_msg(s, "SR:last mode system sleeped %u times.", _runtime_read(SYS_LAST_MODE_DSLEEP_CNT_OFFSET));
#endif
