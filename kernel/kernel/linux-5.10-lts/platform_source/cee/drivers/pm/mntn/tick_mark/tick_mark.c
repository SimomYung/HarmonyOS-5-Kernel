...
#include <platform_include/cee/linux/sr.h>

#define DEEPSLEEP_TIME_HIGH_OFFSET 4
#define sr_durtime_read(offset) runtime_read(M3_RDR_SYS_CONTEXT_RUNTIME_VAR_OFFSET + (offset))
...
static void trace_point(unsigned int point)
{
	...
	if (ret != 0)
		lp_err("ffa send msg fail: %d", ret);
}
...
#ifndef LPMCU_TICKMARK_SPECIAL_THRESHOLD_TIME
#define LPMCU_SUSPEND_THRESHOLD_TIME 5000
#define LPMCU_RESUME_MODME_THRESHOLD_TIME 5500
#define LPMCU_RESUME_ALL_THRESHOLD_TIME 6100
#define LPMCU_SLEEP_ENTER_THRESHOLD_TIME 400
#define LPMCU_SLEEP_EXIT_THRESHOLD_TIME 600
#endif

static int g_suspend_timeout = LPMCU_SUSPEND_THRESHOLD_TIME;
static int g_resume_timeout = LPMCU_RESUME_MODME_THRESHOLD_TIME;
static int g_resume_all_timeout = LPMCU_RESUME_ALL_THRESHOLD_TIME;
static int g_sleep_enter_timeout = LPMCU_SLEEP_ENTER_THRESHOLD_TIME;
static int g_sleep_exit_timeout = LPMCU_SLEEP_EXIT_THRESHOLD_TIME;
...
#ifdef LPMCU_TICKMARK_DYNAMIC_CAL
#define TICK_DDR_DPM_THD 100
static int cal_and_update_suspend_timeout(void)
{
	int tick_rec = 0;
	int tick_update = 0;

	tick_rec = (int)(get_tickmark_diff(TICK_DDR_DPM_SUSPEND_ENTRY, TICK_DDR_DPM_SUSPEND_EXIT));
	if (tick_rec >= TICK_DDR_DPM_THD )
		tick_update += tick_rec - TICK_DDR_DPM_THD;
...
static int tickmark_show_smoke_show(struct seq_file *s, void *data)
{
	...
	int sleep_exit_cost;
	int supend_timeout_update = g_suspend_timeout;

	no_used(data);

#ifdef LPMCU_TICKMARK_DYNAMIC_CAL
	supend_timeout_update += cal_and_update_suspend_timeout();
	lp_info("update suspend timeout: g_suspend_timeout[%d]us supend_timeout_update[%d]us",
		g_suspend_timeout, supend_timeout_update);
#endif
	...
	resume_all_cost = (int)(get_tickmark_diff(TICK_SYS_WAKEUP, TICK_SYS_WAKEUP_END));
	sleep_enter_cost = (int)(get_tickmark_diff(TICK_32K_ENTER_ENTRY, TICK_32K_ENTER_END));
	sleep_exit_cost = (int)(get_tickmark_diff(TICK_32K_EXIT_ENTRY + 1, TICK_32K_EXIT_END));

/* sys_resume does not contain the peri power-on time, modem drx need to add extra. */
#ifdef LPMCU_TICKMARK_MODEM_DRX_NEED_PERI_ON_TIME
	resume_cost += (int)(get_tickmark_diff(TICK_DPM_RESUME_PERI_MEM_ENTRY, TICK_DPM_RESUME_PERI_MEM_END)) +
		(int)(get_tickmark_diff(TICK_DPM_RESUME_PERI_ENTRY, TICK_DPM_RESUME_PERI_END));
#endif

	if (suspend_cost > supend_timeout_update )
		smoke_result |= (1 << SUSPEND_TIMEOUT);

	if (resume_cost > g_resume_timeout)
		smoke_result |= (1 << RESUME_TIMEOUT);

	if (resume_all_cost > g_resume_all_timeout)
		smoke_result |= (1 << RESUME_ALL_TIMEOUT);
	...
	lp_msg(s, "%d, %d[%d], %d[%d], %d[%d], %d[%d], %d[%d]", smoke_result,
		sleep_enter_cost, g_sleep_enter_timeout, sleep_exit_cost, g_sleep_exit_timeout,
		suspend_cost, supend_timeout_update, resume_cost, g_resume_timeout,
		resume_all_cost, g_resume_all_timeout);
			     ...
			     size_t count, loff_t *ppos)
{
	int suspend_timeout;
	int resume_timeout;
	int resume_all_timeout;
	int sleep_enter_timeout;
	int sleep_exit_timeout;

	no_used(s);

	const int correct_columns = 5;
	...
	if (sscanf_s(input_buf, "%d, %d, %d, %d, %d", &sleep_enter_timeout, &sleep_exit_timeout,
				&suspend_timeout, &resume_timeout, &resume_all_timeout) != correct_columns) {
	...
	*ppos += count;
	...
	g_resume_all_timeout = resume_all_timeout;
...
{
	u64 deepsleep_dur_time = 0;
	u64 deepsleep_dur_time_l = sr_durtime_read(DEEPSLEEP_MNTN_TIME);
	u64 deepsleep_dur_time_h = sr_durtime_read(DEEPSLEEP_MNTN_TIME + DEEPSLEEP_TIME_HIGH_OFFSET);

	deepsleep_dur_time = (deepsleep_dur_time_h << 32) | deepsleep_dur_time_l;
	return deepsleep_dur_time;
}
#endif