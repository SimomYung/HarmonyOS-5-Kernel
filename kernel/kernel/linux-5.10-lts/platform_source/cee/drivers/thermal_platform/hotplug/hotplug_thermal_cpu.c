...
static int get_critical_hotplug_bitmask(struct device_node *np, u32 *critical_mask)
{
	...
	ret = of_property_read_u32_array(np, "ithermal,critical_cpu_hotplug_cluster", critical_hotplug_cluster, critical_cluster_num);
	if (ret != 0) {
...
{
	int ret, temp = 0;
	ret = get_cpu_temp(&temp);
	if (ret != 0)
		return true;
	if (g_cpuhp_ops.up_threshold == 0)
		return true;
	
#ifdef CONFIG_HOTPLUG_EMULATION
	if (g_cpuhp_ops.emul_temp != 0)
		temp = g_cpuhp_ops.emul_temp;
#endif

	return temp >= g_cpuhp_ops.up_threshold;
}
#endif
