...
static u32 g_gpu_tdp_cores[MAX_TDP_LEVEL] = {0};
#endif
...
void thermal_hotplug_handle_gpu(unsigned int target_cores)
{
...
{
#ifdef CONFIG_THERMAL_PERCORE
	u32 gpu_cores = g_gpuhp_ops.control_val[status];
	u32 tdp_level = 0;
	u32 tdp_cores;
	if (is_tdp_hotplug_support(&tdp_level)) {
		if (tdp_level < MAX_TDP_LEVEL) {
			tdp_cores = g_gpu_tdp_cores[tdp_level];
			gpu_cores = (gpu_cores < tdp_cores) ? gpu_cores : tdp_cores;
		}
		else
			pr_err("gpu_fsm_callback: tdp level not right. %u\n", tdp_level);
	}
	pr_err("gpu_fsm_callback: gpu to %u cores, current_temp:%d\n",
	       gpu_cores, g_gpuhp_ops.current_temp);
	thermal_hotplug_handle_gpu(gpu_cores);
#else
...
{
	return g_gpuhp_ops.status;
}

void tdp_gpu_hotplug_config(u32 cores)
{
	if (cores <= g_gpuhp_ops.control_val[HOTPLUG_NONE])
		thermal_hotplug_handle_gpu(cores);
	else
		pr_err("%s: too many gpu cores. %u %u\n", __func__, cores, g_gpuhp_ops.control_val[HOTPLUG_NONE]);
}
#endif
...
#ifdef CONFIG_THERMAL_PERCORE
	u32 gpu_tdp_cores;
	ret = of_property_read_u32(np, "ithermal,tdp_gpu_hotplug_cores_low", &gpu_tdp_cores);
	if (ret != 0) {
		pr_err("%s tdp_gpu_hotplug_cores_low read err\n", __func__);
		return ret;
	}
	g_gpu_tdp_cores[0] = gpu_tdp_cores;
	ret = of_property_read_u32(np, "ithermal,tdp_gpu_hotplug_cores_high", &gpu_tdp_cores);
	if (ret != 0) {
		pr_err("%s tdp_gpu_hotplug_cores_high read err\n", __func__);
		return ret;
	}
	g_gpu_tdp_cores[1] = gpu_tdp_cores;
#endif
