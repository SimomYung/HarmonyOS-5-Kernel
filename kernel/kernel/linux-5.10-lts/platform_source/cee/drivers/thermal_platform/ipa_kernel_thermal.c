...
#endif
#ifdef CONFIG_SLOW_FREQ_LIMIT
#define MAX_BOOST_COUNT 3
#endif /* CONFIG_SLOW_FREQ_LIMIT */
...
static char g_ipafreq_name[CAPACITY_OF_ARRAY][THERMAL_ACTOR_NAME_LEN] = {0};
...
static void boost_mntn(struct thermal_zone_device *tz)
{
#ifdef trace_perf
	if (tz->is_board_thermal)
		trace_perf(clock_set_rate, "brd_tz_boost", tz->is_boost ? 1 : 0, raw_smp_processor_id());
	if (tz->is_soc_thermal)
		trace_perf(clock_set_rate, "soc_tz_boost", tz->is_boost ? 1 : 0, raw_smp_processor_id());
#endif
	pr_err("thermal_zone: %s boost %s\n", tz->type, tz->is_boost ? "enable" : "disable");
}
...
int thermal_zone_notify_enabled(struct thermal_zone_device *tz)
{
	...
	tz->is_boost = false;
#ifdef CONFIG_SLOW_FREQ_LIMIT
	tz->boost_count = MAX_BOOST_COUNT;
#endif /* CONFIG_SLOW_FREQ_LIMIT */
	boost_mntn(tz);
#ifdef CONFIG_THERMAL_IPA_OTHER_POWER
	update_boost_ipa_trips(tz, false);
	return 0;
#endif

	return thermal_zone_device_enable(tz);
}
...
int thermal_zone_notify_disabled(struct thermal_zone_device *tz)
{
	...
	boost_mntn(tz);
#ifdef CONFIG_THERMAL_IPA_OTHER_POWER
	update_boost_ipa_trips(tz, true);
	return 0;
#endif
	...
	return thermal_zone_device_disable(tz);
}
...
{
	int trips_num, ret, target_num, control_num;
	u32 boost_temp_trip[2] = {0};
	if (np == NULL || tzp == NULL) {
	...
	if(ret != 0) {
		pr_err("%s cannot read ipa-target-level-trips ret:%d .\n", __func__, ret);
		goto err_trips;
	}

	ret = of_property_count_u32_elems(np, "ipa-control-levels");
	if (ret <= 0) {
		pr_err("%s cannot find ipa-control-levels ret:%d.\n", __func__, ret);
		goto err_trips;
	}
	control_num = ret;

	tzp->control_levels = kzalloc(sizeof(int) * control_num, GFP_KERNEL);
	if (tzp->control_levels == NULL)
		goto err_trips;

	ret = of_property_read_u32_array(np, "ipa-control-levels", tzp->control_levels, control_num);
	if(ret != 0) {
		pr_err("%s cannot read ipa-control-levels ret:%d .\n", __func__, ret);
		goto err_control;
	}
	...
	if(ret != 0) {
		pr_err("%s cannot read ipa-target-levels ret:%d .\n", __func__, ret);
		goto err_target;
	}
#ifdef CONFIG_THERMAL_BOOST_BYPASS
	if (tzp->boost_bypass == false) {
		ret = of_property_read_u32_array(np, "ipa-boost-trips", boost_temp_trip, target_num);
		if(ret != 0) {
			pr_err("%s get boost trips temp failed\n", __func__);
			goto err_target;
		}
		if (boost_temp_trip[0] >= boost_temp_trip[1]) {
			pr_err("%s boost temp trip config failed\n", __func__);
			goto err_target;
		}
		tzp->boost_ipa_trip_0_temp = boost_temp_trip[0];
		tzp->boost_ipa_trip_1_temp = boost_temp_trip[1];
	}
#endif
	if (((trips_num + 1) != control_num) || (control_num != target_num)) {
		pr_err("%s config num not right. trips_num=%d, control_num=%d, target_num=%d\n",
		       __func__, trips_num, control_num, target_num);
		goto err_target;
	}

	tzp->target_trips_num = trips_num;
	tzp->control_num = control_num;
...
int parse_tdp_hotplug(struct device_node *np, struct __thermal_zone *tzp)
{
	int ret;
	const char *is_tdp_hotplug = NULL;
	if (np == NULL || tzp == NULL) {
		pr_err("%s cannot find np or tzp empty.\n", __func__);
		return -EINVAL;
	}

	ret = of_property_read_string(np, "hotplug-by-tdp", &is_tdp_hotplug);
	if (ret < 0) {
		tzp->is_tdp_hotplug = false;
		pr_err("%pOFn: missing hotplug-by-tdp property\n", np);
		set_tdp_support(false);
		return ret;
	} else {
		if (!strncmp(is_tdp_hotplug, "enable", strlen(is_tdp_hotplug))) {
			tzp->is_tdp_hotplug = true;
			set_tdp_support(true);
		} else {
			tzp->is_tdp_hotplug = false;
			set_tdp_support(false);
			return 0;
	...
	ret = of_property_count_u32_elems(np, "cpu-tdp-hotplug-num-low");
	if (ret <= 0) {
		pr_err("%s cannot find cpu-tdp-hotplug-num-low ret:%d.\n", __func__, ret);
		return -EINVAL;
	}
	tzp->cpu_tdp_cluster_num = ret;
	tzp->cpu_tdp_hotplug_num_low = kzalloc(sizeof(int) * tzp->cpu_tdp_cluster_num, GFP_KERNEL);
	if (tzp->cpu_tdp_hotplug_num_low == NULL)
		return -EINVAL;
	tzp->cpu_tdp_hotplug_num_high = kzalloc(sizeof(int) * tzp->cpu_tdp_cluster_num, GFP_KERNEL);
	if (tzp->cpu_tdp_hotplug_num_high == NULL)
		goto alloc_failed;
	ret = of_property_read_u32_array(np, "cpu-tdp-hotplug-num-low",
					 tzp->cpu_tdp_hotplug_num_low, tzp->cpu_tdp_cluster_num);
	if(ret != 0) {
		pr_err("%s cannot read cpu-tdp-hotplug-num-low ret:%d .\n", __func__, ret);
		goto property_failed;
	}
	ret = of_property_read_u32_array(np, "cpu-tdp-hotplug-num-high",
					 tzp->cpu_tdp_hotplug_num_high, tzp->cpu_tdp_cluster_num);
	if(ret != 0) {
		pr_err("%s cannot read cpu-tdp-hotplug-num-high ret:%d .\n", __func__, ret);
		goto property_failed;
	}
	ret = of_property_read_u32(np, "pg-cpu-tdp-big-cluster-down", &(tzp->pg_cpu_big_cluster_num));
	if(ret != 0) {
		pr_err("%s cannot read pg-cpu-tdp-big-cluster-down ret:%d .\n", __func__, ret);
		goto property_failed;
	}
	ret = of_property_read_u32(np, "gpu-tdp-hotplug-num-low", &(tzp->gpu_tdp_hotplug_num_low));
	if(ret != 0) {
		pr_err("%s cannot read gpu-tdp-hotplug-num-low ret:%d .\n", __func__, ret);
		goto property_failed;
	}
	ret = of_property_read_u32(np, "gpu-tdp-hotplug-num-high", &(tzp->gpu_tdp_hotplug_num_high));
	if(ret != 0) {
		pr_err("%s cannot read gpu-tdp-hotplug-num-high ret:%d .\n", __func__, ret);
		goto property_failed;
	}
	save_tdp_hotplug_config(tzp);
	return 0;
property_failed:
	kfree(tzp->cpu_tdp_hotplug_num_high);
alloc_failed:
	kfree(tzp->cpu_tdp_hotplug_num_low);
	return -EINVAL;
}
#endif
		      ...
		      u32 capacitance)
{
	...
	ret = snprintf_s(g_ipafreq_name[cpufreq_cdev->id],
			 THERMAL_ACTOR_NAME_LEN, THERMAL_ACTOR_NAME_LEN - 1,
			 "ipafreq_c%d", cpufreq_cdev->id);
	if (ret < 0)
		pr_err("%s: set ipafreq_c%d fail\n", __func__, cpufreq_cdev->id);
...
#endif
	trace_perf(clock_set_rate, g_ipafreq_name[cpufreq_cdev->id], freq, cpufreq_cdev->id);
	pr_err("%s set cpufreq, cluster:%d, freq:%u\n",
			g_ipafreq_name[cpufreq_cdev->id], cpufreq_cdev->id, freq);
	if (ret >= 0)
	...
	unsigned long state)
{
	...
	trace_perf(clock_set_rate, "ipafreq_gpu", freq, 0);
#endif
...
int cdev_get_cpu_info(struct cpufreq_cooling_device *cpufreq_cdev)
{
	...
	pr_debug("%s: cpuid_start=%d, cpu_num=%d\n", __func__, cpufreq_cdev->cpuid_start, cpufreq_cdev->cpu_num);
	return 0;
}
