...
u32 g_little_mid_cluster_num = 2;
#endif
...
u32 ipa_get_cluster_num(void)
{
	return g_cluster_num;
}

char *g_ipa_cdev_name[CAPACITY_OF_ARRAY] = {
	CDEV_GPU_NAME,
...
static char *g_ipa_weight_name[CAPACITY_OF_ARRAY] = {
	IPA_GPU_WEIGHT_NAME, 
...
char *g_ipa_actor[CAPACITY_OF_ARRAY] = {
	"gpu", 
	"cluster0",
	"cluster1",
	"cluster2",
...
int ipa_get_actor_name_by_cdev(char *cdev_name, char *actor_name)
{
	...
	for (i = 0; i < CAPACITY_OF_ARRAY; i++) {
		...
		if (strncmp(cdev_name, g_ipa_cdev_name[i], THERMAL_ACTOR_NAME_LEN) == 0) {
			ret = strncpy_s(actor_name, THERMAL_ACTOR_NAME_LEN,
					g_ipa_actor[i], THERMAL_ACTOR_NAME_LEN);
			break;
		}
	}

	if (ret != 0) {
		pr_err("%s: copy %s to actor_name fail\n", __func__, g_ipa_actor[i]);
		return -ENODEV;
	}
...
void ipa_trip_and_tdp_percore_refresh(struct thermal_zone_device *tz)
{
	...
	if (boost_bypass_check(tz) == true || (boost_bypass_check(tz) == false && tz->is_boost == false))
		update_pid_target(tz);
#else
	update_pid_target(tz);
#endif /* CONFIG_THERMAL_BOOST_BYPASS */
	mutex_unlock(&tz->lock);
#endif /* CONFIG_THERMAL_IPA_OTHER_POWER */
#ifdef CONFIG_THERMAL_PERCORE
	mutex_lock(&tz->lock);
	update_tdp_hotplug(tz);
	mutex_unlock(&tz->lock);
#endif /* CONFIG_THERMAL_PERCORE */
}
...
int cluster_get_static_power_by_temp(cpumask_t *cpumask, unsigned long u_volt, int temp, u32 *static_power)
{
	...
	t_scale = get_temperature_scale(temp);
	v_scale = get_voltage_scale(u_volt);
	*static_power = (u32)(nr_cpus * (cpu_coeff * t_scale * v_scale) / HZ_PER_MHZ);
...
u32 get_cpu_big_cluster_num(void) {
	return (g_cluster_num > g_little_mid_cluster_num) ? (g_cluster_num - g_little_mid_cluster_num) : 0;
}
EXPORT_SYMBOL(get_cpu_big_cluster_num);
#endif
				 ...
				 struct platform_device *pdev)
{
	...
	pr_debug("%s: g_cluster_num=%u, cpuclustercnt=%d, nr_cpu_ids=%u\n",
		 __func__, g_cluster_num, cpuclustercnt, nr_cpu_ids);
	for (i = 0; i < (int)g_cluster_num; i++) {
...
#ifdef CONFIG_THERMAL_PERCORE
	if (cpuclustercnt == g_cluster_num)
		set_tdp_ag_chip(true);
	else
		set_tdp_ag_chip(false);
#endif
	return 0;
...
int cal_extra_attr_group_size(struct thermal_zone_device *tz)
{
	...
	if (strncmp(tz->type, MARGIN_THERMAL_NAME, sizeof(MARGIN_THERMAL_NAME) - 1) == 0)
		ret++;
#endif /* CONFIG_THERMAL_GOV_MARGIN_CTRL */
#ifdef CONFIG_THERMAL_GOV_DYNAMIC_INTERVAL_CTRL
	if (strncmp(tz->type, NPU_THERMAL_NAME, sizeof(NPU_THERMAL_NAME) - 1) == 0)
		ret++;
	if (strncmp(tz->type, FAN_THERMAL_NAME, sizeof(FAN_THERMAL_NAME) - 1) == 0)
		ret++;
