...
int hv_set_ipafreq(int actor_id, unsigned int freq)
{
	int ret = -1;

	if (actor_id < 0 || actor_id >= g_freqset_dev_num){
		pr_err("ipafreq set id %d is invalid\n", actor_id);
		return ret;
	}

	ret = hv_set_freq(g_freqset_dev[actor_id].freqset_hvdev, freq);
	if (ret != 0)
		pr_err("%s set hvdev freq fail\n", g_freqset_dev[actor_id].name);
	
#ifdef trace_perf
	trace_perf(clock_set_rate, g_freqset_dev[actor_id].name, freq, actor_id);
#endif
...
static int ipafreq_set_dev_init(struct device *dev)
{
	...
	for_each_child_of_node(np, child) {
		...
		if (freqset->freqset_hvdev == NULL) {
			pr_err("ipa %s hvdev register fail!\n", trace_name);
			continue;
		}
	}
