			   ...
			   u32 granted_power)
{
	...
	actor_id = ipa_get_actor_id_by_cdev(instance->cdev->type);
	if (actor_id < 0) {
		instance->target = state;
	} else if (tz->is_board_thermal) {
#ifdef CONFIG_SLOW_FREQ_LIMIT
		if (tz->boost_count > 0 && state > g_ipa_board_state[actor_id] + MAX_CHANGE_STATE)
			state = g_ipa_board_state[actor_id] + MAX_CHANGE_STATE;
#endif /* CONFIG_SLOW_FREQ_LIMIT */
		g_ipa_board_state[actor_id] = state;
		instance->target = max(g_ipa_board_state[actor_id], g_ipa_soc_state[actor_id]);
	} else {
#ifdef CONFIG_SLOW_FREQ_LIMIT
		if (tz->boost_count > 0 && state > g_ipa_soc_state[actor_id] + MAX_CHANGE_STATE)
			state = g_ipa_soc_state[actor_id] + MAX_CHANGE_STATE;
#endif /* CONFIG_SLOW_FREQ_LIMIT */
...
void update_pid_target(struct thermal_zone_device *tz)
{
	...
	if (!data)
		return;
	for (i = 0; i < data->target_trips_num; i++) {
		if (tz->tzp->sustainable_power <= data->target_trips[i]) {
			break;
		}
	}
	if (!tz->ops || !tz->ops->set_trip_temp)
		return;
	...
	ret = tz->ops->set_trip_temp(tz, params->trip_switch_on, data->control_levels[i]);
	if (ret)
		pr_err("set_control_trip_temp ret:%d\n", ret);
	ret = tz->ops->set_trip_temp(tz, params->trip_max_desired_temperature, data->target_levels[i]);
	if (ret)
		pr_err("set_target_trip_temp ret:%d\n", ret);
	pr_info("%s: sustainable_power=%u, set control_temp=%d, target_temp=%d\n",
		__func__, tz->tzp->sustainable_power, data->control_levels[i], data->target_levels[i]);
}
...
void update_boost_ipa_trips(struct thermal_zone_device *tz, bool is_boost)
{
	...
	struct __thermal_zone *data = NULL;
	struct power_allocator_params *params = NULL;
	int trip_0_temp, trip_1_temp;
	if (tz == NULL)
		return;
	data = tz->devdata;
	if (data == NULL)
		return;
	if (tz->ops->set_trip_temp == NULL)
		return;
	params = tz->governor_data;
	if (params == NULL)
		return;
	mutex_lock(&tz->lock);
	if (is_boost) {
		trip_0_temp = data->boost_ipa_trip_0_temp;
		trip_1_temp = data->boost_ipa_trip_1_temp;
		pr_err("%s: boost enable, set trip_0_temp=%d, trip_1_temp=%d\n",
			__func__, trip_0_temp, trip_1_temp);
		ret = tz->ops->set_trip_temp(tz, params->trip_switch_on, trip_0_temp);
		if (ret)
			pr_err("set_control_trip_temp ret:%d\n", ret);
		ret = tz->ops->set_trip_temp(tz, params->trip_max_desired_temperature, trip_1_temp);
		if (ret)
			pr_err("set_target_trip_temp ret:%d\n", ret);
	} else {
		update_pid_target(tz);
	}
	mutex_unlock(&tz->lock);
}
#endif
...
#ifdef CONFIG_THERMAL_IPA_OTHER_POWER
				if (tz->tzp->other_power >= 0 && tz->tzp->other_power < tz->tzp->sustainable_power)
					sustainable_power -= tz->tzp->other_power;
#endif
			} else {
...
int gpufreq_update_policies(bool updatepolicy)
{
	...
	list_for_each_entry(cdev, &thermal_cdev_list, node) {
		...
		if (!strncmp(cdev->np->name, "gpufreq", strlen("gpufreq")) ||
			!strncmp(cdev->np->name, "mali", strlen("mali"))) {
			dfc = cdev->devdata;
			if (is_spm_mode_enabled()) {
 ...
 */
#ifdef CONFIG_FREQ_LIMIT_THERMAL
#define CLUSTER0_MIN_FREQ	1000000
#endif
			...
			unsigned int freq)
{
	...
	int cluster = topology_physical_package_id(policy->cpu);
#endif

	for (level = 1; level <= cpufreq_cdev->max_level; level++)
#if (!(defined CONFIG_LDK_THERMAL) && (defined CONFIG_FREQ_LIMIT_THERMAL))
		if (freq > freq_table[level].frequency || (cluster == 0 && freq_table[level].frequency < CLUSTER0_MIN_FREQ))
#else
