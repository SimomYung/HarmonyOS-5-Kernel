...
int npu_thermal_creat_attrs(struct thermal_zone_device *tz)
{
	...
	if (tz->ori_attr_groups_size - 1 < 0) {
		pr_err("%s npu thermal attrs init err for zero length\n", __func__);
		return -ENODEV;
	}

	for (indx = 0; indx < cal_extra_attr_group_size(tz); indx++)
		if (tz->device.groups[tz->ori_attr_groups_size - 1 + indx] == NULL)
			break;
	tz->dyn_interval_attr_index = tz->ori_attr_groups_size - 1 + indx;
