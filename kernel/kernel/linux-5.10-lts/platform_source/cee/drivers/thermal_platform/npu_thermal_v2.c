...
{
	int i, calib_value;
	int shell_temp = get_shell_frame_temp();
	if (shell_temp == 0 || !param->dyn_step_wise_enabled) {
	...
	for (i = 0; i < param->dyn_ref_temp_num; i++) {
		if (shell_temp <= param->dyn_ref_temp[i]) {
			break;
		}
	}
...
static int npu_thermal_probe(struct platform_device *pdev)
{
	...
	dev_info(&pdev->dev, "Thermal zone npu device registered.\n");
