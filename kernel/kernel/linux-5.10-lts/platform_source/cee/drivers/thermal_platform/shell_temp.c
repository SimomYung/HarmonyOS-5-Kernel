...
static void set_shell_frame_temp(char *type, int temp)
{
	if (type == NULL)
		return;

	if (!strncasecmp(type, IPA_SENSOR_SHELL, THERMAL_NAME_LENGTH))
		atomic_set(&g_shell_temp, temp);
}

int get_shell_frame_temp(void)
{
	return atomic_read(&g_shell_temp);
}
...
int get_shell_temp(struct thermal_zone_device *thermal, int *temp)
{
	...
	*temp = shell->temp;
	...
	set_shell_frame_temp(thermal->type, *temp);
...
static int fill_sensor_coef(struct shell_t *shell, struct device_node *np)
{
	...
	for_each_child_of_node(np, child) {
		...
		/* terminal sensor */
		} else if (!IS_ERR(thermal_zone_get_zone_by_name(ptr_type))) {
