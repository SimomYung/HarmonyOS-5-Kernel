			  ...
			  int *temperature)
{
	...
	if (g_tmdev->tsens_level == 0 || !check_temperature_hidden(tz))
		return;
	if (g_tmdev->tsens_level == 2 || check_temperature_no_show(tz)) {
		*temperature = 0;
	} else {
		/* g_tmdev->tsens_level == 1 */
...
static int get_device_tree_data(struct platform_device *pdev)
{
	...
	if (rc != 0 || tsensor_level == 0) {
		g_tmdev->tsens_level = tsensor_level;
	} else if (tsensor_level == 1 || tsensor_level == 2) {
		g_tmdev->tsens_level = tsensor_level;
	} else {
