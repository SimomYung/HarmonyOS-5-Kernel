...
#define THERMAL_LEVEL_NORMAL_WARM_THRESHOLD	70
#define THERMAL_LEVEL_WARM_TEMP_SHOW		75
#define THERMAL_LEVEL_WARM_HIGH_THRESHOLD	80
#define THERMAL_LEVEL_HIGH_TEMP_SHOW		85
#define THERMAL_LEVEL_HIGH_HOT_THRESHOLD	90
#define THERMAL_LEVEL_HOT_TEMP_SHOW		95
#define THERMAL_LEVEL_HOT_CRITICAL_THRESHOLD	105
#define THERMAL_LEVEL_CRITICAL_TEMP_SHOW	110

enum thermal_level {
	THERMAL_LEVEL_COLD = 0,
	THERMAL_LEVEL_LOW,
...
static int g_temp_level_table[THERMAL_LEVEL_MAX + 1] = {
	...
	THERMAL_LEVEL_WARM_TEMP_SHOW,
	THERMAL_LEVEL_HIGH_TEMP_SHOW,
	THERMAL_LEVEL_HOT_TEMP_SHOW,
...
static int g_temp_level_threshold_table[THERMAL_LEVEL_MAX] = {
	...
	THERMAL_LEVEL_NORMAL_WARM_THRESHOLD,
	THERMAL_LEVEL_WARM_HIGH_THRESHOLD,
	THERMAL_LEVEL_HIGH_HOT_THRESHOLD,
	THERMAL_LEVEL_HOT_CRITICAL_THRESHOLD,
};
...
static bool check_temperature_hidden(struct thermal_zone_device *tz)
{
	...
	if (strncmp(tz->type, SOC_THERMAL_NAME, sizeof(SOC_THERMAL_NAME) - 1) == 0)
		ret = true;
	else if (check_tz_type((const char **)g_tsensor_name,
						...
						int temperature)
{
	...
	if (tz->emul_temperature != 0)
		return level;
	/* used for soc_thermal */
	temperature /= CELSIUS_SCALING;
			  ...
			  int *temperature)
{
	...
	if (strncmp(tz->type, SOC_THERMAL_NAME, sizeof(SOC_THERMAL_NAME) - 1) == 0) {
		...
		*temperature = g_temp_level_table[level] * CELSIUS_SCALING;
	} else {
		*temperature = 0;
	}
}
...
{
#ifdef trace_perf
	char reg_name_str[THERMAL_ZONE_NAME_LENGTH] = {0};
	...
	/* ip temp */
	if (check_tz_type((const char **)g_trace_ip_print_list,
			  ARRAY_SIZE(g_trace_ip_print_list),
			  tz->type, &indx)) {
		if (g_tmdev->tsens_level)
			return;
		ret = snprintf_s(reg_name_str, sizeof(reg_name_str), (sizeof(reg_name_str) - 1),
				 "%s_temp", g_trace_ip_print_list[indx]);
#ifdef CONFIG_IPA_THERMAL
	/* is_soc_thermal and is_board_thermal depend on IPA_THERMA */
	} else if (tz->is_soc_thermal) {
		ret = snprintf_s(reg_name_str, sizeof(reg_name_str),
				 (sizeof(reg_name_str) - 1), "soc_temp");
		if (g_tmdev->tsens_level) {
			level = get_temperature_level(tz, temp);
			print_temp = g_temp_level_table[level] * CELSIUS_SCALING;
		}
	} else if (tz->is_board_thermal) {
		ret = snprintf_s(reg_name_str, sizeof(reg_name_str),
				 (sizeof(reg_name_str) - 1), "board_temp");
#endif /* CONFIG_IPA_THERMAL */
#ifdef CONFIG_THERMAL_SHELL
	/* shell temp depends on THERMAL_SHELL */
	...
	if (ret < 0) {
		pr_err("%s: compose reg_name_str fail!\n", __func__);
		return;
	}
	trace_perf(clock_set_rate, reg_name_str, print_temp,
...
int ipa_get_tsensor_id(const char *name)
{
	...
	for (id = 0; id < g_tmdev->tsens_num_sensor; id++) {
		if (g_tsensor_name[id] == NULL) {
			pr_err("%s:g_tsensor_name is NULL!\n", __func__);
			return ret;
		}
...
static int get_device_tree_data(struct platform_device *pdev)
{
	...
	if (rc != 0 || tsensor_level == 0) {
		...
		pr_err("%s tsensor_level read err for invalid value\n", __func__);
		return -EINVAL;
	}
#ifndef CONFIG_THERMAL_TSENSOR_GET_TEMP
