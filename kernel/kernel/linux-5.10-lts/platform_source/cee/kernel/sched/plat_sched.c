...
void get_task_fit_cpus(struct task_struct *p, struct cpumask *fit_cpus)
{
	...
	for_each_sched_cluster(cluster) {
		...
		unsigned long task_util_pred = use_task_pred_load(fcpu, p)
						? task_pred_util(p) : 0;

		if (rtg_target && cap_orig < rtg_target_cap)
			continue;

		/* To save overhead. Assumes capacity_orig >= capacity_of. */
		if (!fit_margin(task_util, cap_orig, margin) ||
		    !fit_margin(task_util_pred, cap_orig, margin))
			continue;
...
bool task_fit_cpu(struct task_struct *p, int cpu)
{
	...
	unsigned long task_util = uclamp_task_util(p);
	unsigned long task_util_pred = use_task_pred_load(cpu, p)
					? task_pred_util(p) : 0;
	...
	if (!rtg_misfit &&
	    fit_margin(task_util, cap, margin) &&
	    fit_margin(task_util_pred, cap_orig, margin))
		return true;
...
#ifdef CONFIG_MICROVISOR_DYNAMIC_SHARED_CPU
	/*
	 * If possible, never select RDA shared cpus for rt task
	 * otherwise it may suffer a latency.
	 * This condition is weaker than above conditions.
	 */
	if (rt_task(p))
		if (!cpumask_subset(cpus, cpu_shared_mask))
			cpumask_andnot(cpus, cpus, cpu_shared_mask);
#endif
}

bool test_cpu_available(int cpu, struct task_struct *p)
{
	cpumask_t avail_cpus = CPU_MASK_NONE;

	cpumask_set_cpu(cpu, &avail_cpus);
	get_avail_cpus(&avail_cpus, p);
	return cpumask_test_cpu(cpu, &avail_cpus);
}
...
void get_max_spare_cap_cpus(struct cpumask *cpus, struct task_struct *p)
{
	...
	for_each_cpu(i, cpus) {
		...
		long cap = task_is_smt_expeller ? capacity_orig_of(i)
						: capacity_of(i);
		long spare_cap = max_t(long, cap - cpu_util_without(i, p), 0);

		/*
		 * No spare cap left. Let the one with more tasks have
		 * less spare cap. When p can preempt, it prevents rt tasks
		 * to cumulate on lower cpu ids and helps to stay away from
		 * an unbalanced cpu due to cpuset. When p can not preempt,
		 * it keeps tasks balanced at waking.
		 */
		if (spare_cap == 0) {
			spare_cap -= cpu_rq(i)->rt.rt_nr_total;
			spare_cap -= !!test_slow_cpu(i);
		}
