...
int32_t jpu_job_decode(struct jpu_data_type *jpu_device, const void __user *argp)
{
	...
	struct semaphore *sem = &jpu_device->blank_sem;
	...
	if(ret != 0){
		jpu_err("the sleep task is interrupted by a signal");
		goto dowm_sem_err_out;
	}
...
case_err_out:
	if (ret != 0 || list_empty(&sem->wait_list)) {
