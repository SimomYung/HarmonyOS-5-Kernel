...
static int32_t dp_panel_parse_dt(struct dp_private *priv, struct device_node *np)
{
	...
	dpu_pr_info("[DP] min_lane=%#x, min_rate=%#x, max_rate=%#x, ssc_flag=%#x, pipe_sw_pre_itfch_idx=%#x, \
		...
		filter_resolution_enable=%d, board_mode:%d, aux_phy=0x%x, jump_wait_dss=0x%x, base id=%u, bit_depth=%u",
		...
		pinfo->filter_resolution_enable, pinfo->dp_board_mode, priv->dp_aux_phy_config,
		priv->dp_jump_wait_dss, pinfo->base.id, dptx0->bit_depth);
...
bool dptx_is_edp_panel(struct dp_ctrl *dptx)
{
	dpu_check_and_return(!dptx, false, err, "[DP] dptx is null pointer!");
	if (dptx->port_id == DPTX_PORT_ID_4 || dptx->port_id == DPTX_PORT_ID_5)
		return true;
	else
		return false;
}
