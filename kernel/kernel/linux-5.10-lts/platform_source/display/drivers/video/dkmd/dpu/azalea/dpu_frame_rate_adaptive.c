	...
	const char *buf, size_t count)
{
	...
	if (dpufd->panel_info.dfr_info.oled_info.oled_type == PANEL_OLED_LTPS) {
		if (!pdata->fps_scence_get || !pdata->lcd_fps_scence_handle)
			return -1;

		if (pdata->fps_scence_get(frame_rate, &scence) != 0)
			return -1;

		ret = pdata->lcd_fps_scence_handle(dpufd->pdev, scence);
...
static ssize_t dfr_average_frame_rate_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	ret = snprintf_s(buf, PAGE_SIZE, PAGE_SIZE - 1,
		"1hz[%u], 5hz[%u], 10hz[%u], 15hz[%u], 20hz[%u], 24hz[%u], 30hz[%u], 36hz[%u], 45hz[%u], 60hz[%u],"
		" 72hz[%u], 90hz[%u], 120hz[%u], 144hz[%u]\n",
		frame_rate_static->refresh_stat_info.stat_info.refresh_cnt[FRM_RATE_1HZ],
		frame_rate_static->refresh_stat_info.stat_info.refresh_cnt[FRM_RATE_5HZ],
		...
		frame_rate_static->refresh_stat_info.stat_info.refresh_cnt[FRM_RATE_20HZ],
		...
		frame_rate_static->refresh_stat_info.stat_info.refresh_cnt[FRM_RATE_36HZ],
		...
		frame_rate_static->refresh_stat_info.stat_info.refresh_cnt[FRM_RATE_72HZ],
		...
		frame_rate_static->refresh_stat_info.stat_info.refresh_cnt[FRM_RATE_144HZ]);
