	...
	int frm_rate, uint32_t porch_ratio, uint32_t notify_type)
{
	...
	snprintf(buf, sizeof(buf) - 1, "dfrNotify=%u, Refresh=1", value);
...
void mipi_dsi_frm_rate_ctrl_init(struct dpu_fb_data_type *dpufd)
{
	...
	if (dpufd->dfr_create_singlethread == false) {
		...
		dpufd->panel_info.fps_updt = tmp_fps;
		dpufd->dfr_notice_wq = alloc_ordered_workqueue("dfr_notice_work", WQ_HIGHPRI | __WQ_LEGACY | WQ_MEM_RECLAIM);
