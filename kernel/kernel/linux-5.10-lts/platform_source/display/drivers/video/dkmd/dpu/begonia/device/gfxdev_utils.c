	...
	const void __user* argp)
{
	...
	if (!ukmd_cmdlist_is_kernel_mode(CMDLIST_DEV_ID_DPU))
		frame.cmdlist_id = CMDLIST_ID_USER_MODE(frame.cmdlist_id);

	ret = comp->present(comp, (void *)&frame);
	if (unlikely(ret != 0)) {
		dpu_pr_warn("%s present fail: scene_id=%d cmdlist_id=%llu layer_count=%u frame_index=%u",
			comp->base.name, frame.scene_id, frame.cmdlist_id, frame.layer_count, frame.frame_index);
		ukmd_cmdlist_present_release_locked(CMDLIST_DEV_ID_DPU,
			(uint32_t)frame.scene_id, frame.cmdlist_id);
	}
...
static int32_t update_hardware_cursor(struct composer *comp, struct hardware_cursor_info *info)
{
	if (unlikely(comp->update_hardware_cursor == NULL)) {
		dpu_pr_err("[hw cursor] update_hardware_cursor is not init");
		return -EINVAL;
	}

	return comp->update_hardware_cursor(comp, info);
}
...
int32_t dpu_gfxdev_update_hardware_cursor(struct composer *comp, void __user *argp)
{
	...
	switch(info.cmd) {
		case HARDWARE_CURSOR_UPDATE:
			ret = update_hardware_cursor(comp, &info);
			break;
...
int32_t dpu_gfxdev_tunnel_present(struct composer *comp, void __user *argp)
{
	...
	if (!comp->power_on) {
		dpu_pr_err("[tunnel] composer is not power on\n");
		return -1;
	}
