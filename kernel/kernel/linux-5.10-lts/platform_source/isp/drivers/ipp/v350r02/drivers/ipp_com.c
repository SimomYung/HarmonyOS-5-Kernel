...
static int hipp_core_powerup(struct hippcomdev_s *dev, unsigned int type)
{
	...
	ret = regulator_enable(dev->media_supply);
	if (ret != 0) {
...
fail_tcu_poweron:
	ret = regulator_disable(dev->media_supply);
	if (ret != 0)
	...
	struct hippcomdev_s *comdev)
{
	...
	comdev->media_supply = devm_regulator_get(dev, "ipp-media");
	if (IS_ERR_OR_NULL(comdev->media_supply)) {
		pr_err("[%s] Failed : MEDIA1 devm_regulator_get.%pK\n",
			__func__, comdev->media_supply);
		return -EINVAL;
	}
