...
struct hippcomdev_s {
	...
	struct clk *ippclk;
	unsigned int ippclk_value;
	unsigned int ippclk_normal;
	unsigned int ippclk_hsvs;
	unsigned int ippclk_svs;
	unsigned int ippclk_off;
...
static int hippsmmu_enable(struct hipp_common_s *drv)
{
	...
	if (drv->tbu_mode >= MAX_HIPP_TBU) {
		pr_err("[%s] Failed : tbu_mode[%u] invalid\n", __func__, drv->tbu_mode);
		return -EINVAL;
	}
	...
	unsigned int sid, unsigned int ssid)
{
	...
	ret = hipp_ffa_cfg_smmu_ssidv((unsigned short)swid, (unsigned short)len, (unsigned short)sid, (unsigned short)ssid);
#else
...
#ifdef CONFIG_HISP_FFA_SUPPORT
	ret = hipp_ffa_cfg_smmu_perf((unsigned short)swid, (unsigned short)len);
#else
...
#ifdef CONFIG_HISP_FFA_SUPPORT
static void hipp_ffa_dump(void)
{
	int ret = hipp_ffa_dump_info();
	if (ret != 0) {
		pr_info("[%s] unspport ffa dump\n", __func__);
	}
}
#endif
...
#else
static unsigned int get_clk_rate(struct hippcomdev_s *dev, unsigned int index)
{
	unsigned int rate = 0;

	switch (index) {
	case HIPPCORE_RATE_TBR:
		rate = dev->ippclk_value;
		break;

	case HIPPCORE_RATE_NOR:
		rate = dev->ippclk_normal;
		break;

	case HIPPCORE_RATE_HSVS:
		rate = dev->ippclk_hsvs;
		break;
...
static int ipp_setclk_enable(struct hippcomdev_s *dev, unsigned int devtype)
{
	...
	for (i = 0; i < MAX_HIPP_COM; i++)
		dev->jpeg_rate[i] = get_clk_rate(dev, CLK_RATE_OFF);

	ret = clk_set_rate(dev->ippclk, dev->ippclk_off);
	if (ret < 0) {
		pr_err("[%s] Failed: clk_set_rate %d.%d\n",
			__func__, dev->ippclk_off, ret);
		return -EINVAL;
	}

	ret = clk_prepare_enable(dev->ippclk);
	if (ret < 0) {
		pr_err("[%s] Failed: clk_prepare_enable.%d\n", __func__, ret);
		return -EINVAL;
	}

	dev->jpeg_current_rate = dev->ippclk_off;
	dev->jpeg_rate[devtype] = dev->ippclk_off;
...
static int ipp_setclk_disable(struct hippcomdev_s *dev, unsigned int devtype)
{
	...
	clk_disable_unprepare(dev->ippclk);
	dev->jpeg_rate[devtype] = dev->ippclk_off;
	dev->jpeg_current_rate = dev->ippclk_off;

	for (i = 0; i < MAX_HIPP_COM; i++) {
		if (dev->jpeg_rate[i] != dev->ippclk_off)
...
static int hipp_core_powerdn(struct hippcomdev_s *dev, unsigned int type)
{
	...
	ret = regulator_disable(dev->media2_supply);
	if (ret != 0)
		pr_err("[%s] Failed: media2 regulator_disable.%d\n", __func__, ret);
	...
	unsigned int devtype, unsigned int clktype)
{
	...
	set_rate = get_clk_rate(dev, clktype);

	return set_rate;
}

static int hipp_set_rate(struct hippcomdev_s *dev, unsigned int devtype, unsigned int clktype)
{
	int ret;
	int i = 0;
	unsigned int set_rate;
	...
	ret = clk_set_rate(dev->ippclk, set_rate);
	if (ret < 0) {
...
static int hippcore_pu(struct hipp_common_s *drv)
{
	int ret = 0;
	struct hippcomdev_s *dev = NULL;
	...
	dev->refs_power++;
	pr_info("[%s] : refs_power.%u\n", __func__, dev->refs_power);
	mutex_unlock(&dev->mutex_power);

	return 0;
}

static int hippcore_pd(struct hipp_common_s *drv)
{
	struct hippcomdev_s *dev = NULL;
	int ret = 0;
	...
	mutex_lock(&dev->mutex_power);
	if (dev->refs_power == 0) {
		pr_err("[%s] : ipp no need to pn,refs_power.%u\n",
			__func__, dev->refs_power);
		mutex_unlock(&dev->mutex_power);
		return 0;
	}
...
static int hippcom_earlydts_init(struct hippcomdev_s *dev)
{
	...
	ret = of_property_read_u32(np, "drv-num", (unsigned int *)(&drvnum));
	if (ret != 0) {
		pr_err("[%s] Failed: drv-num property.%d\n", __func__, ret);
		return -EINVAL;
	}

	if (drvnum > MAX_HIPP_COM) {
		pr_err("[%s] Failed: drvnum.%u\n", __func__, drvnum);
		return -EINVAL;
	}
	...
	ret = of_property_read_string_array(np, "drv-names",
		dev->drvname, drvnum);
	if (ret < (int)drvnum) {
		pr_err("[%s] Failed : drv-names string_array.%d < %u\n",
			__func__, ret, drvnum);
		return -EINVAL;
	}
	...
	struct hippcomdev_s *comdev)
{
	...
	if (IS_ERR_OR_NULL(comdev->media2_supply)) {
		pr_err("[%s] Failed : MEDIA2 devm_regulator_get\n", __func__);
		return -EINVAL;
	}

	comdev->cpe_supply = devm_regulator_get(dev, "ipp-cpe");
	if (IS_ERR_OR_NULL(comdev->cpe_supply)) {
		pr_err("[%s] Failed : CPE devm_regulator_get\n", __func__);
		return -EINVAL;
	}

	comdev->smmu_tcu_supply = devm_regulator_get(dev, "ipp-smmu-tcu");
	if (IS_ERR_OR_NULL(comdev->smmu_tcu_supply)) {
		pr_err("[%s] Failed : smmu tcu devm_regulator_get\n", __func__);
		return -EINVAL;
	}

	comdev->ippclk = devm_clk_get(dev, "clk_ipp");
	if (IS_ERR_OR_NULL(comdev->ippclk)) {
		pr_err("[%s] Failed: get clk ipp", __func__);
		return -EINVAL;
	}
	...
	struct hippcomdev_s *comdev)
{
	...
	ret = of_property_read_u32(np, "clock-value", &comdev->ippclk_value);
	if (ret < 0) {
		pr_err("[%s] Failed: clock-value.%d\n", __func__, ret);
		return -EINVAL;
	}

	ret = of_property_read_u32(np, "clock-value-normal", &comdev->ippclk_normal);
	if (ret < 0) {
		pr_err("[%s] Failed: clock nor.%d\n", __func__, ret);
		return -EINVAL;
	}

	ret = of_property_read_u32(np, "clock-value-hsvs", &comdev->ippclk_hsvs);
	if (ret < 0) {
		pr_err("[%s] Failed: clock hsvs.%d\n", __func__, ret);
		return -EINVAL;
	}

	ret = of_property_read_u32(np, "clock-value-svs", &comdev->ippclk_svs);
	if (ret < 0) {
		pr_err("[%s] Failed: clock svs.%d\n", __func__, ret);
		return -EINVAL;
	}

	ret = of_property_read_u32(np, "clock-value-off", &comdev->ippclk_off);
	if (ret < 0) {
