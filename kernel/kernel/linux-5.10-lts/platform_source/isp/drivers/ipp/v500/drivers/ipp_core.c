...
#if defined(CONFIG_SOC_PG_INFO) && !defined(NSV_IPP_SCOUT)
...
{
#if !defined(NSV_IPP_SCOUT) && defined(CONFIG_SOC_PG_INFO)
	struct soc_component_pg_info pg_info;
#if defined(CONFIG_HISPIPP_FE_PG)
	...
	if (pg_info.tailor_strategy == SOC_COMPONENT_NOT_TAILOR || 
		pg_info.tailor_strategy == SOC_COMPONENT_TAILOR_STRATEGY_1 ||
		pg_info.tailor_strategy == SOC_COMPONENT_NOT_TAILOR_EFUSE_NONE) {
		pr_info("IPP do not enable PG");
		return 0;	
	} else if (pg_info.tailor_strategy == SOC_COMPONENT_TAILOR_STRATEGY_2){
		pr_info("IPP enable PG, do not use IPP");
		return (int)pg_info.tailor_strategy;
	} else {
		pr_info("SOC_COMPONENT_MEDIA2_ISP_FE_INFO tailor_strategy ERROR");
		return (int)pg_info.tailor_strategy;
	}
#else
	get_soc_pg_info_by_index(SOC_COMPONENT_MEDIA2_ISP_INFO, &pg_info);
	pr_info("efuse_value: 0x%08x, tailor_strategy: %d", pg_info.efuse_value, pg_info.tailor_strategy);
	if (pg_info.efuse_value == SOC_SPEC_EFUSE_INVALID_VALUE) {
		pr_err("efuse_value err!");
		return (int)pg_info.efuse_value;
	}
	return (int)pg_info.tailor_strategy;
#endif
#else
...
static void lock_ipp_wakelock(void)
{
	struct hispcpe_s *dev = hispcpe_dev;
...
static void relax_ipp_wakelock(void)
{
	struct hispcpe_s *dev = hispcpe_dev;
...
static int hipp_cmd_pwrup(unsigned long args)
{
	...
	logd("+");
	if (dev->refs_power_up > MAX_HIPP_REFS_CNT) {
		pr_err("Failed: pu times(%d) is greater than threshold\n", dev->refs_power_up);
...
static int hipp_cmd_get_mem_size(unsigned long args)
{
	...
	if (remaining_bytes != 0) {
		pr_err("[%s] copy_from_user failed, remaining_bytes = %lu\n", __func__, remaining_bytes);
		ret = ISP_IPP_ERR;
		goto out;
	}
	...
	if (remaining_bytes != 0) {
		pr_err("[%s] copy_to_user failed, remaining_bytes = %lu\n", __func__, remaining_bytes);
		ret = ISP_IPP_ERR;
		goto out;
	}
	logd("-");
...
static int hipp_cmd_do_req(unsigned long args)
{
	int ret = 0;
	struct hispcpe_s *dev = hispcpe_dev;
	hipp_adapter_s *hipp_dev = get_ipp_adapter();
	...
	mutex_lock(&dev->dev_lock);
	mutex_lock(&hipp_dev->hipp_refs_lock);
	logi("+");
	if (dev->refs_power_up <= 0) {
		pr_err("[%s] Failed : ipp powerdown.%d\n", __func__, dev->refs_power_up);
		mutex_unlock(&hipp_dev->hipp_refs_lock);
		mutex_unlock(&dev->dev_lock);
		return -EINVAL;
	}
	if (hipp_dev->hipp_refs[REFS_TYP_EARCMK] > MAX_HIPP_REFS_CNT) {
		loge("max hipp refs.%d", hipp_dev->hipp_refs[REFS_TYP_EARCMK]);
		mutex_unlock(&hipp_dev->hipp_refs_lock);
		mutex_unlock(&dev->dev_lock);
		return ISP_IPP_ERR;
	}
	hipp_dev->hipp_refs[REFS_TYP_EARCMK]++;
	mutex_unlock(&hipp_dev->hipp_refs_lock);
	mutex_unlock(&dev->dev_lock);
...
static int hispcpe_get_irq(unsigned int index)
{
	...
	irq = (unsigned int)irq_of_parse_and_map(np, (int)index);
	if (irq == 0) {
