...
#define VP_RD_ARF_DU 8
#define VP_RD_ORB_ENH_KLT_SRC_DU 6
#define VP_RD_CMP_RDR_DU 2
enum cvdr_dev_e {
	CVDR_RT  = 0,
	CVDR_SRT = 1,
};
...
static unsigned int calculate_cvdr_allocated_du(unsigned int pclk, unsigned int throughput, int port)
{
	int num_du = VP_RD_ORB_ENH_KLT_SRC_DU;
	(void)pclk;
	switch(port){
		case IPP_VP_RD_ORB_ENH_Y_HIST:
		case IPP_VP_RD_ORB_ENH_Y_IMAGE:
		case IPP_VP_RD_KLT_SRC:
			num_du = VP_RD_ORB_ENH_KLT_SRC_DU;
			break;
		case IPP_VP_RD_ARF_0:
		case IPP_VP_RD_ARF_1:
		case IPP_VP_RD_ARF_2:
		case IPP_VP_RD_ARF_3:
			num_du = VP_RD_ARF_DU;
			break;
		case IPP_VP_RD_RDR:
		case IPP_VP_RD_CMP:
			num_du = VP_RD_CMP_RDR_DU;
		default:
			num_du = VP_RD_ORB_ENH_KLT_SRC_DU;
	}
	if (throughput <= 0) 
							...
							unsigned int line_stride, unsigned int isp_clk, pix_format_e format)
{
	...
	pix_size = (unsigned int)((cfmt->expand == EXP_PIX) ? pix_fmt_info[pix_fmt].expand_size : pix_fmt_info[pix_fmt].compact_size);
	...
	for (i = cfmt->id; i < (cfmt->id + config_num); i++) {
		...
		vp_wr_cfg->fmt.pix_expan   = (unsigned char)cfmt->expand;
		vp_wr_cfg->fmt.line_stride =
			(unsigned short)(align_up(cfmt->full_width * pix_size / BITS_NUM_OF_BYTE, CVDR_ALIGN_BYTES) / CVDR_ALIGN_BYTES - 1);
		...
		if (line_stride != 0) {
			...
			vp_wr_cfg->fmt.line_stride = (unsigned short)align_up(line_stride, CVDR_ALIGN_BYTES) / CVDR_ALIGN_BYTES - 1;
	...
	unsigned int line_stride, unsigned int isp_clk, pix_format_e format)
{
	...
	pix_size = (unsigned int)((cfmt->expand == EXP_PIX) ? pix_fmt_info[pix_fmt].expand_size : pix_fmt_info[pix_fmt].compact_size);
	du = calculate_cvdr_allocated_du(cvdr_vp_rd_bw[id].pclk, cvdr_vp_rd_bw[id].throughput, id);
	p_cvdr_cfg->vp_rd_cfg[id].to_use = 1;
	p_cvdr_cfg->vp_rd_cfg[id].id     = (unsigned char)id;
	p_cvdr_cfg->vp_rd_cfg[id].fmt.fs_addr      = cfmt->addr;
	p_cvdr_cfg->vp_rd_cfg[id].fmt.pix_fmt      = pix_fmt;
	p_cvdr_cfg->vp_rd_cfg[id].fmt.pix_expan    = (unsigned char)cfmt->expand;
	p_cvdr_cfg->vp_rd_cfg[id].fmt.allocated_du = (unsigned short)du;
	p_cvdr_cfg->vp_rd_cfg[id].fmt.line_size    = (unsigned short)(cfmt->line_size - 1);
	p_cvdr_cfg->vp_rd_cfg[id].fmt.vprd_ihleft  = 0; // set 0 in IPP;
	p_cvdr_cfg->vp_rd_cfg[id].fmt.frame_size   = (unsigned short)(cfmt->height - 1);
	...
	if (line_stride != 0) {
		p_cvdr_cfg->vp_rd_cfg[id].fmt.line_stride = (unsigned short)(align_up(line_stride, CVDR_ALIGN_BYTES) / CVDR_ALIGN_BYTES - 1);
		p_cvdr_cfg->vp_rd_cfg[id].fmt.last_page   = (cfmt->addr +
										...
										unsigned int line_stride, unsigned int total_bytes)
{
	...
	p_cvdr_cfg->vp_wr_cfg[id].id = (unsigned char)id;
	...
	p_cvdr_cfg->vp_wr_cfg[id].fmt.line_stride = (unsigned short)line_stride;
										...
										unsigned int total_bytes, unsigned int line_stride)
{
	...
	id = cfmt->id;
	p_cvdr_cfg->vp_rd_cfg[id].to_use = 1;
	p_cvdr_cfg->vp_rd_cfg[id].id = (unsigned char)cfmt->id;
	p_cvdr_cfg->vp_rd_cfg[id].fmt.fs_addr      = cfmt->addr;
	...
	p_cvdr_cfg->vp_rd_cfg[id].fmt.frame_size   = (unsigned short)(cfmt->height - 1);
	p_cvdr_cfg->vp_rd_cfg[id].fmt.line_stride  = (unsigned short)((line_stride > 0) ? (line_stride - 1) : 0);
	p_cvdr_cfg->vp_rd_cfg[id].fmt.line_wrap    = DEFAULT_LINE_WRAP;
	calculate_cvdr_bw_limiter(&(p_cvdr_cfg->vp_rd_cfg[id].bw), cvdr_vp_rd_bw[id].throughput);
