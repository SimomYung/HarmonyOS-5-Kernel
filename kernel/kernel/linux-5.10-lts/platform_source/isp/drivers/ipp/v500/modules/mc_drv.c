...
static void mc_static_config(mc_dev_t *dev, mc_static_cfg2_t *static_cfg)
{
	macro_cmdlst_set_reg(dev->base_addr + MC_STATIC_CFG2_REG, static_cfg->u32);
}
...
static void mc_index_pairs_config(mc_dev_t *dev, cfg_tab_mc_t *tab_mc)
{
	...
	if (tab_mc->mc_cascade_en == CASCADE_ENABLE) {
		for (i = 0; i < (mc_data_size / CMDLST_BURST_READ_MAX_SIZE); i++) {
			...
			offset = MC_INDEX_CFG_0_REG + i * (mc_reg_size * 4);
			macro_cmdlst_set_reg_incr(dev->base_addr + offset, mc_reg_size, INCR_MODE, WR_OPS_MODE);
...
static int mc_coord_pairs_config(mc_dev_t *dev,  cfg_tab_mc_t *tab_mc)
{
	int ret = 0;
	unsigned int i = 0;
	unsigned int offset = 0;
	unsigned int mc_reg_size = 0;
	unsigned int mc_data_size = MC_COOR_KPT_NUM_MAX;
	mc_coord_pairs_cfg_t *coord_pairs_cfg = &(tab_mc->coord_pairs_cfg);

	// Cur and ref should be configured alternately and the cur is configured first; must cfg 1600*2 times;
	macro_cmdlst_set_addr_align(CVDR_ALIGN_BYTES);
	macro_cmdlst_set_reg_burst_data_align(MC_COOR_KPT_NUM_MAX, CVDR_ALIGN_BYTES);
	macro_cmdlst_set_addr_align(CVDR_ALIGN_BYTES);
	tab_mc->mc_coordinate_addr = dev->cmd_buf->start_addr_isp_map + dev->cmd_buf->data_size;
	...
	if (tab_mc->mc_cascade_en == CASCADE_ENABLE) {
		for (i = 0; i < (mc_data_size / CMDLST_BURST_READ_MAX_SIZE); i++) {
			mc_reg_size = ((mc_data_size - i * CMDLST_BURST_READ_MAX_SIZE) > CMDLST_BURST_READ_MAX_SIZE) ?
							CMDLST_BURST_READ_MAX_SIZE :
							(mc_data_size - i * CMDLST_BURST_READ_MAX_SIZE);
			offset = MC_COORDINATE_CFG_0_REG + i * (mc_reg_size * 4);
			macro_cmdlst_set_reg_incr(dev->base_addr + offset, mc_reg_size, INCR_MODE, WR_OPS_MODE);
			macro_cmdlst_set_addr_offset(mc_reg_size);
		}
	} else if (tab_mc->mc_cascade_en == CASCADE_DISABLE) {
		ret = macro_cmdlst_set_reg_burst(dev->base_addr + MC_COORDINATE_CFG_0_REG, MC_COOR_KPT_NUM_MAX, 0,
							(unsigned int*)&(coord_pairs_cfg->coordinate_cur_ref_pairs[0]));
		if (ret) {
...
static int mc_do_config(mc_dev_t *dev, cfg_tab_mc_t *tab_mc)
{
	...
	if (tab_mc->ctrl_cfg.bits.cfg_mode == MC_CFG_MODE_COORD_PAIRS) 
		mc_coord_pairs_config(dev, tab_mc);
