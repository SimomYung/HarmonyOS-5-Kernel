...
channel_map_t g_channel_map[WORK_MODULE_MAX] = {
	...
	[ENH_ARF_KLT_MC] = {
		.enh_arf_channel = CMDLST_CHANNEL_11,
		.rdr_channel = CMDLST_CHANNEL_MAX,
		.cmp_channel = CMDLST_CHANNEL_MAX,
		.mc_channel  = CMDLST_CHANNEL_13,
		.klt_channel = CMDLST_CHANNEL_12,
	},
...
int seg_ipp_set_cmdlst_wr_buf_mc_coor(cmd_buf_t *cmd_buf, cmdlst_wr_cfg_t *wr_cfg)
{
	...
	cmdlst_wr_table.vp_wr_cfg[IPP_VP_WR_CMDLST].fmt.line_start_wstrb = (unsigned short)line_start_wstrb;
	...
	for (i = 0; i < burst_cnt; i++) {
		...
		if (wr_cfg->read_mode == 1) { // only for updata mc's index_cfg reg.
			...
			unsigned int offset = MC_COORDINATE_CFG_0_REG + i * (CMDLST_BURST_READ_MAX_SIZE * 4);
			cmdlst_cmd.bits.addr_range = (JPG_MC_OFFSET + offset) >> 2;
...
int seg_ipp_set_cmdlst_wr_buf(cmd_buf_t *cmd_buf, cmdlst_wr_cfg_t *wr_cfg)
{
	...
	for (i = 0; i < burst_cnt; i++) {
		...
		if (wr_cfg->read_mode == 1) { // only for updata mc's index_cfg reg.
			...
			cmdlst_cmd.u32 = 0;
			cmdlst_cmd.bits.cfg_mode   = WR_OPS_MODE;
			unsigned int offset = MC_INDEX_CFG_0_REG + i * (reg_size * 4);
			cmdlst_cmd.bits.addr_range = (JPG_MC_OFFSET + offset) >> 2;
			cmdlst_cmd.bits.incr_mode  = INCR_MODE; //  e.g: 0x0/0x4/0x8.....
			cmdlst_cmd.bits.burst_size = (reg_size >= 1) ? (reg_size - 1) : 0;
			logd("cmdlst_cmd.u32 = 0x%x", cmdlst_cmd.u32);
