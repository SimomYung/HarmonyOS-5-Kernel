...
hipp_adapter_s *get_ipp_adapter(void)
{
	return g_hipp_adapter;
}
...
int hipp_adapter_register_irq(int irq)
{
	...
	ret = request_irq((unsigned int)irq, hispcpe_irq_handler, 0, "HIPP_ADAPTER_IRQ", (void *)dev);
	if (ret != 0) {
...
unsigned int get_cpe_addr_da(unsigned int work_module)
{
	...
	return (unsigned int)dev->daddr[work_module];
}
...
static int dataflow_cvdr_global_config(void) // liang, need check
{
...
{
	int ret = 0;
	ret = dev->ippdrv->setsid_smmu(dev->ippdrv, IPPCORE_SWID_INDEX0, IPPCORE_SWID_LEN0,
	...
	ret = dev->ippdrv->setsid_smmu(dev->ippdrv, IPPCORE_SWID_INDEX1, IPPCORE_SWID_LEN1,
		(unsigned int)sid, (unsigned int)ssid);
	if (ret != 0) {
		loge("Failed : dev->ippdrv->enable_smmu.%d", ret);
		return ret;
...
static int hipp_orcm_enh_arf_klt_mc_wait(hipp_adapter_s *dev, struct msg_req_ipp_cmd_req_t *req)
{
	work_module_e module = req->work_module;
	...
	unsigned int arf_enable = req->req_enh_arf.req_arf_cur.reg_cfg[0].top_cfg.bits.ar_feature_en;
	unsigned int klt_enable = req->req_klt.reg_cfg.klt_klt_cfg.klt_start.bits.start_klt;
	unsigned int mc_enable = req->req_klt.work_mode;
	...
	if (ret0 < 0 || ret1 < 0 || ret2 < 0) {
		loge("work_module.%d: ret0.%d, ret1.%d, ret2.%d", module, ret0, ret1, ret2);
		return ISP_IPP_ERR;
	}
...
static int hipp_orcm_wait(hipp_adapter_s *dev, struct msg_req_ipp_cmd_req_t *req)
{
	...
	case ENH_ARF_KLT:
		...
		break;
	case ENH_ARF_KLT_MC:
		ret0 = hipp_orcm_enh_arf_klt_mc_wait(dev, req);
		break;
	default:
		loge("Failed : Invilid work_module = %d", module);
		return ISP_IPP_ERR;
...
static int hipp_orcm_eop_handle(hipp_adapter_s *dev, struct msg_req_ipp_cmd_req_t *req)
{
	...
	switch (work_module) {
	case ENH_ONLY:
		dequeue_channel_ready_list(g_channel_map[work_module].enh_arf_channel);
		break;
	case ARFEATURE_ONLY:
		dequeue_channel_ready_list(g_channel_map[work_module].enh_arf_channel);
		break;
	case MATCHER_ONLY:
		dequeue_channel_ready_list(g_channel_map[work_module].rdr_channel);
		dequeue_channel_ready_list(g_channel_map[work_module].cmp_channel);
		break;
	case MC_ONLY:
		dequeue_channel_ready_list(g_channel_map[work_module].mc_channel);
		break;
	case ENH_ARF:
		dequeue_channel_ready_list(g_channel_map[work_module].enh_arf_channel);
		break;
	...
	case ENH_ARF_MATCHER_MC:
		...
		break;
	case KLT_ONLY:
		dequeue_channel_ready_list(g_channel_map[work_module].klt_channel);
		break;
	case ENH_ARF_KLT:
		dequeue_channel_ready_list(g_channel_map[work_module].enh_arf_channel);
		dequeue_channel_ready_list(g_channel_map[work_module].klt_channel);
...
static int fd_to_iova(struct msg_req_ipp_cmd_req_t *req, ipp_stream_t *stream_info, struct list_head *fd_link_head)
{
	...
	if ((iova == 0) || (iova_size < stream_info->total_size)) {
		loge("Failed : prot.0x%d, size.%lu, stream_info->total_size.%d",
...
static void get_enh_kernel_buff_addr(struct msg_req_ipp_cmd_req_t *req, msg_req_enh_t *msg_enh, struct list_head *fd_link_head)
{
	int i = 0;
...
static void get_arf_kernel_buff_addr(struct msg_req_ipp_cmd_req_t *req, msg_req_arf_t *msg_arf, struct list_head *fd_link_head)
{
	int i = 0, j = 0;
...
static void get_mc_kernel_buff_addr(struct msg_req_ipp_cmd_req_t *req, msg_req_mc_t *msg_mc, struct list_head *fd_link_head)
{
	int i = 0;
...
static void get_klt_kernel_buff_addr(struct msg_req_ipp_cmd_req_t *req, msg_req_klt_t *msg_klt, struct list_head *fd_link_head)
{
	int i = 0;
...
static void dump_cmdlst_buff(struct msg_req_ipp_cmd_req_t *req)
{
	...
	unsigned int i;
	int ret = 0;
	cpe_mem_free(MEM_ID_CMDLST_PARA_ENH_ARF, req->work_module);
	cpe_mem_free(MEM_ID_CMDLST_PARA_MC, req->work_module);
	cpe_mem_free(MEM_ID_CMDLST_PARA_REORDER, req->work_module);
	cpe_mem_free(MEM_ID_CMDLST_PARA_COMPARE, req->work_module);
	cpe_mem_free(MEM_ID_CMDLST_PARA_KLT, req->work_module);
	...
	ret = cpe_mem_get(MEM_ID_CMDLST_PARA_ENH_ARF, req->work_module, &va, &da);
	if (ret != 0) {
		loge("Failed : cpe_mem_get %d", MEM_ID_CMDLST_PARA_ENH_ARF);
	} else {
		cmdlst_para_t *cmp_cmdlst_para = (cmdlst_para_t *)(uintptr_t)va;
		schedule_cmdlst_link_t *new_entry = (schedule_cmdlst_link_t *)cmp_cmdlst_para->cmd_entry;
		loge("ENH ARF CMDLST BUFF:");
		for (i = 0; i < cmp_cmdlst_para->stripe_cnt; ++i)
			cmdlst_buff_dump(&(new_entry[i].cmd_buf));
	}
	ret = cpe_mem_get(MEM_ID_CMDLST_PARA_MC, req->work_module, &va, &da);
	if (ret != 0) {
	...
	ret = cpe_mem_get(MEM_ID_CMDLST_PARA_KLT, req->work_module, &va, &da);
	if (ret != 0) {
		loge("Failed : cpe_mem_get %d", MEM_ID_CMDLST_PARA_KLT);
	} else {
		cmdlst_para_t *cmp_cmdlst_para = (cmdlst_para_t *)(uintptr_t)va;
		schedule_cmdlst_link_t *new_entry = (schedule_cmdlst_link_t *)cmp_cmdlst_para->cmd_entry;
		loge("KLT CMDLST BUFF:");
		for (i = 0; i < cmp_cmdlst_para->stripe_cnt; ++i)
			cmdlst_buff_dump(&(new_entry[i].cmd_buf));
	}
}

static void dump_ipp_top(void)
{
	void __iomem *crg_base;
	int i;
	unsigned int value, value1, value3, value2;
	crg_base = hipp_get_regaddr(CPE_TOP);
	...
	loge("enh clk = 0x%08x, enh soft rst = 0x%08x", readl(crg_base + IPP_TOP_ENH_CRG_CFG0_REG),
			readl(crg_base + IPP_TOP_ENH_CRG_CFG0_REG));
	loge("arf clk = 0x%08x, arf soft rst = 0x%08x", readl(crg_base + IPP_TOP_ARF_CRG_CFG0_REG),
			readl(crg_base + IPP_TOP_ARF_CRG_CFG0_REG));
	loge("rdr clk = 0x%08x, rdr soft rst = 0x%08x", readl(crg_base + IPP_TOP_RDR_CRG_CFG0_REG),
			readl(crg_base + IPP_TOP_RDR_CRG_CFG0_REG));
	loge("cmp clk = 0x%08x, cmp soft rst = 0x%08x", readl(crg_base + IPP_TOP_CMP_CRG_CFG0_REG),
			readl(crg_base + IPP_TOP_CMP_CRG_CFG0_REG));
	loge("mc  clk = 0x%08x, mc  soft rst = 0x%08x", readl(crg_base + IPP_TOP_MC_CRG_CFG0_REG),
			readl(crg_base + IPP_TOP_MC_CRG_CFG0_REG));
	loge("klt clk = 0x%08x, klt soft rst = 0x%08x", readl(crg_base + IPP_TOP_KLT_CRG_CFG0_REG),
			readl(crg_base + IPP_TOP_KLT_CRG_CFG0_REG));

	loge("ipp top debug:");
	for (i = IPP_TOP_JPG_DEBUG_0_REG; i <= IPP_TOP_JPG_DEBUG_3_REG; i += 16) {
		value = readl(crg_base + i);
		value1 = readl(crg_base + i + 0x4);
		value2 = readl(crg_base + i + 0x8);
		value3 = readl(crg_base + i + 0xC);
		loge("offset(0x%x - 0x%x): value(0x%08x  0x%08x  0x%08x  0x%08x)", i, i + 0xC, value, value1, value2, value3);
	}
}

static void dump_ipp_cvdr(void)
{
	void __iomem *crg_base;
	loge("cvdr reg:");
	crg_base = hipp_get_regaddr(CVDR_REG);
	loge("offset(0x00 - 0x0C): value(0x%08x  0x%08x  0x%08x  0x%08x)",
		readl(crg_base + 0x0), readl(crg_base + 0x4), readl(crg_base + 0x8), readl(crg_base + 0xC));
	loge("offset(0x10 - 0x1C): value(0x%08x  0x%08x  0x%08x  0x%08x)",
		readl(crg_base + 0x10), readl(crg_base + 0x14), readl(crg_base + 0x18), readl(crg_base + 0x1C));
	loge("debug: ");
	loge("nr rd 2: 0x%08x", readl(crg_base + CVDR_IPP_CVDR_IPP_NR_RD_DEBUG_2_REG));
	loge("nr rd 3: 0x%08x", readl(crg_base + CVDR_IPP_CVDR_IPP_NR_RD_DEBUG_3_REG));
	loge("nr rd 4: 0x%08x", readl(crg_base + CVDR_IPP_CVDR_IPP_NR_RD_DEBUG_4_REG));
	loge("nr rd 5: 0x%08x", readl(crg_base + CVDR_IPP_CVDR_IPP_NR_RD_DEBUG_5_REG));
	loge("nr rd 6: 0x%08x", readl(crg_base + CVDR_IPP_CVDR_IPP_NR_RD_DEBUG_6_REG));
	loge("nr rd 7: 0x%08x", readl(crg_base + CVDR_IPP_CVDR_IPP_NR_RD_DEBUG_7_REG));
	loge("nr rd 8: 0x%08x", readl(crg_base + CVDR_IPP_CVDR_IPP_NR_RD_DEBUG_8_REG));

	loge("nr wr 8: 0x%08x", readl(crg_base + CVDR_IPP_CVDR_IPP_NR_WR_DEBUG_8_REG));

	loge("vp rd 0: 0x%08x", readl(crg_base + CVDR_IPP_CVDR_IPP_VP_RD_DEBUG_0_REG));
	loge("vp rd 1: 0x%08x", readl(crg_base + CVDR_IPP_CVDR_IPP_VP_RD_DEBUG_1_REG));
	loge("vp rd 6: 0x%08x", readl(crg_base + CVDR_IPP_CVDR_IPP_VP_RD_DEBUG_6_REG));
	loge("vp rd 7: 0x%08x", readl(crg_base + CVDR_IPP_CVDR_IPP_VP_RD_DEBUG_7_REG));
	loge("vp rd 9: 0x%08x", readl(crg_base + CVDR_IPP_CVDR_IPP_VP_RD_DEBUG_9_REG));
	loge("vp rd 10: 0x%08x", readl(crg_base + CVDR_IPP_CVDR_IPP_VP_RD_DEBUG_10_REG));
	loge("vp rd 12: 0x%08x", readl(crg_base + CVDR_IPP_CVDR_IPP_VP_RD_DEBUG_12_REG));
	loge("vp rd 13: 0x%08x", readl(crg_base + CVDR_IPP_CVDR_IPP_VP_RD_DEBUG_13_REG));
	loge("vp rd 14: 0x%08x", readl(crg_base + CVDR_IPP_CVDR_IPP_VP_RD_DEBUG_14_REG));
	loge("vp rd 15: 0x%08x", readl(crg_base + CVDR_IPP_CVDR_IPP_VP_RD_DEBUG_15_REG));
	loge("vp rd 22: 0x%08x", readl(crg_base + CVDR_IPP_CVDR_IPP_VP_RD_DEBUG_22_REG));

	loge("vp wr 0: 0x%08x", readl(crg_base + CVDR_IPP_CVDR_IPP_VP_WR_SID_DEBUG_SID0_0_REG));
	loge("vp wr 3: 0x%08x", readl(crg_base + CVDR_IPP_CVDR_IPP_VP_WR_SID_DEBUG_SID0_3_REG));
	loge("vp wr 12: 0x%08x", readl(crg_base + CVDR_IPP_CVDR_IPP_VP_WR_SID_DEBUG_SID0_12_REG));
	loge("vp wr 13: 0x%08x", readl(crg_base + CVDR_IPP_CVDR_IPP_VP_WR_SID_DEBUG_SID0_13_REG));
	loge("vp wr 14: 0x%08x", readl(crg_base + CVDR_IPP_CVDR_IPP_VP_WR_SID_DEBUG_SID0_14_REG));
	loge("vp wr 15: 0x%08x", readl(crg_base + CVDR_IPP_CVDR_IPP_VP_WR_SID_DEBUG_SID0_15_REG));
	loge("vp wr 16: 0x%08x", readl(crg_base + CVDR_IPP_CVDR_IPP_VP_WR_SID_DEBUG_SID0_16_REG));
	loge("vp wr 17: 0x%08x", readl(crg_base + CVDR_IPP_CVDR_IPP_VP_WR_SID_DEBUG_SID0_17_REG));
	loge("vp wr 18: 0x%08x", readl(crg_base + CVDR_IPP_CVDR_IPP_VP_WR_SID_DEBUG_SID0_18_REG));
	loge("vp wr 19: 0x%08x", readl(crg_base + CVDR_IPP_CVDR_IPP_VP_WR_SID_DEBUG_SID0_19_REG));
	loge("vp wr 22: 0x%08x", readl(crg_base + CVDR_IPP_CVDR_IPP_VP_WR_SID_DEBUG_SID0_22_REG));
	loge("vp wr 23: 0x%08x", readl(crg_base + CVDR_IPP_CVDR_IPP_VP_WR_SID_DEBUG_SID0_23_REG));
	loge("vp wr 24: 0x%08x", readl(crg_base + CVDR_IPP_CVDR_IPP_VP_WR_SID_DEBUG_SID0_24_REG));
	loge("vp wr 25: 0x%08x", readl(crg_base + CVDR_IPP_CVDR_IPP_VP_WR_SID_DEBUG_SID0_25_REG));
	loge("vp wr 26: 0x%08x", readl(crg_base + CVDR_IPP_CVDR_IPP_VP_WR_SID_DEBUG_SID0_26_REG));
	loge("vp wr 30: 0x%08x", readl(crg_base + CVDR_IPP_CVDR_IPP_VP_WR_SID_DEBUG_SID0_30_REG));
}

static void dump_smmu_reg(void)
{
	void __iomem *crg_base;

	crg_base = hipp_get_regaddr(SEC_ADPT_REG);
	loge("sec_adapt addr reg 0x%x = 0x%x", 0x4 * IPPCORE_SWID_INDEX0, readl(crg_base + 0x4 * IPPCORE_SWID_INDEX0));
	loge("sec_adapt addr reg 0x%x = 0x%x", 0x4 * IPPCORE_SWID_INDEX1, readl(crg_base + 0x4 * IPPCORE_SWID_INDEX1));
}
...
static void dump_reg(void)
{
	...
	loge("arf reg: total kpt num: offset = 0x%x, value = 0x%x", AR_FEATURE_TOTAL_KPT_NUM_REG, value);
	loge("arf reg:");
	...
	loge("rdr reg:");
	for (i = REORDER_REORDER_CFG_REG; i <= REORDER_EC_1_REG; i += 16) {
		value = readl(crg_base + i);
		value2 = readl(crg_base + i + 0x4);
		value3 = readl(crg_base + i + 0x8);
		value4 = readl(crg_base + i + 0xC);
	...
	loge("klt reg:");
	for (i = IPP_KLT_DS_CONFIG_REG; i <= IPP_KLT_GYRO_GRID_OFFSET_REG; i += 16) {
		value = readl(crg_base + i);
		value2 = readl(crg_base + i + 0x4);
		value3 = readl(crg_base + i + 0x8);
		value4 = readl(crg_base + i + 0xC);
	...
	loge("kp number: %d", readl(crg_base + 0x11d4));
	loge("val kp number: %d", readl(crg_base + 0x311c));
	crg_base = hipp_get_regaddr(CMDLIST_REG);
	loge("cmdlst reg:");
	for (i = 0; i <= 15; i ++) {
		loge("cmdlst ch id %d", i);
		loge("(0x80)0x%08x  (0x94)0x%08x  (0x98)0x%08x  (0x9C)0x%08x, (0xD4)0x%08x, (0xF0)0x%08x",
			readl(crg_base + 0x80 + 0x80 * i), readl(crg_base + 0x94 + 0x80 * i), readl(crg_base + 0x98 + 0x80 * i),
			readl(crg_base + 0x9C + 0x80 * i), readl(crg_base + 0xD4 + 0x80 * i), readl(crg_base + 0xF0 + 0x80 * i));
	}
	dump_ipp_cvdr();
}
...
static int config_cpe_clk_rate(unsigned int req_rate)
{
	...
	dev->cpe_rate_voting[dev->cpe_rate_value]++;
...
int reset_cpe_clk_rate(void)
{
	...
	if (dev->cpe_rate_voting[dev->cpe_rate_value] == 0) {
		pr_err("[%s] cpe_rate_voting[%s] is 0\n", __func__, clk_name[dev->cpe_rate_value]);
		mutex_unlock(&dev->cpe_rate_lock);
	    mutex_unlock(&dev->hipp_refs_lock);
		return ISP_IPP_ERR;
	}
	...
	unsigned int rate = CLK_RATE_SVS;
	for (int i = CLK_RATE_TURBO; i < CLK_RATE_SVS; i++) {
		if (dev->cpe_rate_voting[i] > 0) {
			rate = i;
			break;
		}
	}

	if (rate > dev->cpe_rate_value) {
		dev->cpe_rate_value = rate;
		ret = dev->ippdrv->set_jpgclk_rate(dev->ippdrv, dev->cpe_rate_value);
...
int hipp_adapter_do_request(unsigned long args)
{
	...
	if (remaining_bytes != 0) {
		loge("Failed : copy_from_user failed, remaining_bytes = %lu", remaining_bytes);
		ret = ISP_IPP_ERR;
		goto free_req_vmalloc_memory;
	}
	...
	logi("cur: inv_xsize = %d, inv_ysize = %d, blk_xsize = %d, blk_ysize = %d, blknumx = %d, blknumy = %d",
			req->req_enh_arf.req_enh_cur.reg_cfg.adjust_image_cfg.inv_blk_size_x.bits.inv_xsize,
			req->req_enh_arf.req_enh_cur.reg_cfg.adjust_image_cfg.inv_blk_size_y.bits.inv_ysize,
			req->req_enh_arf.req_enh_cur.reg_cfg.cal_hist_cfg.blk_size.bits.blk_xsize,
			req->req_enh_arf.req_enh_cur.reg_cfg.cal_hist_cfg.blk_size.bits.blk_ysize,
			req->req_enh_arf.req_enh_cur.reg_cfg.cal_hist_cfg.blk_num.bits.blknumx,
			req->req_enh_arf.req_enh_cur.reg_cfg.cal_hist_cfg.blk_num.bits.blknumy);
	logi("ref: inv_xsize = %d, inv_ysize = %d, blk_xsize = %d, blk_ysize = %d, blknumx = %d, blknumy = %d",
			req->req_enh_arf.req_enh_ref.reg_cfg.adjust_image_cfg.inv_blk_size_x.bits.inv_xsize,
			req->req_enh_arf.req_enh_ref.reg_cfg.adjust_image_cfg.inv_blk_size_y.bits.inv_ysize,
			req->req_enh_arf.req_enh_ref.reg_cfg.cal_hist_cfg.blk_size.bits.blk_xsize,
			req->req_enh_arf.req_enh_ref.reg_cfg.cal_hist_cfg.blk_size.bits.blk_ysize,
			req->req_enh_arf.req_enh_ref.reg_cfg.cal_hist_cfg.blk_num.bits.blknumx,
			req->req_enh_arf.req_enh_ref.reg_cfg.cal_hist_cfg.blk_num.bits.blknumy);
...
void hipp_resource_init(void)
{
	...
	for (i = 0; i < WORK_MODULE_MAX;i++) {
		...
		if (ret < 0)
			loge("Failed : cpe_init_memory:%d", i);
		cpe_set_memory(i);
	}
