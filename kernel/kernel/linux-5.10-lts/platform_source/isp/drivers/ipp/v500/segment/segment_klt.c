...
static void seg_klt_dump_req(msg_req_klt_t *klt_req)
{
	...
	logi("klt_kp_number: %d", reg_cfg->klt_klt_cfg.prev_next_kp_coord.klt_kp_number.u32);
	for (i = 0; i < 16; ++i)
		logi("kp_prev_x_y: %d", reg_cfg->klt_klt_cfg.prev_next_kp_coord.kp_prev_xy[i].bits.kp_prev_x_y);
	for (i = 0; i < 16; ++i)
		logi("kp_next_x_y: %d", reg_cfg->klt_klt_cfg.prev_next_kp_coord.kp_next_xy[i].bits.kp_next_x_y);
	...
	seg_klt_cfg_t *seg_cfg_tab)
{
	if (klt_req->work_mode == KLT_ONLY_DS) {
		...
		if (klt_req->streams[KLT_PREV_LAYER0_BUFF].buffer != 0 && klt_req->streams[KLT_NEXT_LAYER0_BUFF].buffer != 0) {
	...
	global_info_ipp_t *global_info)
{
	...
	if (work_mode == KLT_ONLY_KLT || work_mode == KLT_DS_AND_KLT) {
		...
		if (klt_req->klt_stat_buff[KLT_MC_KP_VAL_NUMBER_BUFF].buffer) stripe_cnt++;
		if (klt_req->klt_stat_buff[KLT_MC_KP_NEXT_PREV_BUFF].buffer) stripe_cnt++;
	}
	if (global_info->mc_update_flag && global_info->mc_en) {
		stripe_cnt += 2;  // update matched_num and coor
		logi("add 2 stripe");
	}
	...
	if (global_info->mc_update_flag && global_info->mc_en) {
		...
		cmdlst_stripe[stripe_cnt - 1].ch_link    = global_info->mc_channel;
		cmdlst_stripe[stripe_cnt - 1].ch_link_act_nbr = global_info->mc_stripe_cnt;
	}
...
static int seg_klt_update_mc_cmdlst_buf(schedule_cmdlst_link_t *cmd_link_entry,
				global_info_ipp_t *global_info, unsigned int *i)
{
	cmdlst_wr_cfg_t cmdlst_wr_cfg = {0};
	int ret = 0;
	...
	cmdlst_wr_cfg.reg_rd_addr = IPP_BASE_ADDR_KLT + IPP_KLT_MC_KP_NB_REG;
	cmdlst_wr_cfg.is_wstrb = 1;
	cmdlst_wr_cfg.read_mode = 0;
	cmdlst_wr_cfg.buff_wr_addr = global_info->mc_matched_num_addr;
	ret = seg_ipp_set_cmdlst_wr_buf(&cmd_link_entry[(*i)++].cmd_buf, &cmdlst_wr_cfg);
	if (ret == ISP_IPP_ERR) {
		logi("error: seg_ipp_set_cmdlst_wr_buf fail!");
		return ISP_IPP_ERR;
	}

	cmdlst_wr_cfg.data_size = KLT_KP_NUM_MAX + KLT_KP_NUM_MAX;
	cmdlst_wr_cfg.is_wstrb = 0;
	cmdlst_wr_cfg.reg_rd_addr = IPP_BASE_ADDR_KLT + IPP_KLT_MC_NEXT_KP_0_REG;
	cmdlst_wr_cfg.read_mode = 1;
	cmdlst_wr_cfg.buff_wr_addr = global_info->mc_coordinate_addr;
	ret = seg_ipp_set_cmdlst_wr_buf_mc_coor(&cmd_link_entry[(*i)++].cmd_buf, &cmdlst_wr_cfg);
	if (ret == ISP_IPP_ERR) {
		logi("error: seg_ipp_set_cmdlst_wr_buf_mc_coor fail!");
		return ISP_IPP_ERR;
	}
	...
	int *index)
{
	...
	if (klt_req->klt_stat_buff[KLT_KP_PREV_BUFF].buffer) {
		...
		cmdlst_wr_cfg.is_wstrb  = 0;
		cmdlst_wr_cfg.reg_rd_addr = IPP_BASE_ADDR_KLT + IPP_KLT_KP_PREV_BUFF_X_0_REG;
	...
	if (klt_req->klt_stat_buff[KLT_KP_NEXT_BUFF].buffer) {
		...
		cmdlst_wr_cfg.is_wstrb  = 0;
		cmdlst_wr_cfg.reg_rd_addr = IPP_BASE_ADDR_KLT + IPP_KLT_KP_NEXT_BUFF_X_0_REG;
		cmdlst_wr_cfg.buff_wr_addr = klt_req->klt_stat_buff[KLT_KP_NEXT_BUFF].buffer;
		loge_if_set_ret(seg_ipp_set_cmdlst_wr_buf(&cmd_link_entry[(*index)++].cmd_buf, &cmdlst_wr_cfg), ret);
		if (ret) return ISP_IPP_ERR;
	}

	if (klt_req->klt_stat_buff[KLT_MC_KP_VAL_NUMBER_BUFF].buffer) {
		cmdlst_wr_cfg.data_size = 1; // kp_next_x + kp_next_y
		cmdlst_wr_cfg.is_wstrb  = 0;
		cmdlst_wr_cfg.reg_rd_addr = IPP_BASE_ADDR_KLT + IPP_KLT_MC_KP_NB_REG;
		cmdlst_wr_cfg.buff_wr_addr = klt_req->klt_stat_buff[KLT_MC_KP_VAL_NUMBER_BUFF].buffer;
		loge_if_set_ret(seg_ipp_set_cmdlst_wr_buf(&cmd_link_entry[(*index)++].cmd_buf, &cmdlst_wr_cfg), ret);
		if (ret) return ISP_IPP_ERR;
	}

	if (klt_req->klt_stat_buff[KLT_MC_KP_NEXT_PREV_BUFF].buffer) {
		cmdlst_wr_cfg.data_size = KLT_KP_NUM_MAX + KLT_KP_NUM_MAX + 1; // kp_next_x + kp_next_y
		cmdlst_wr_cfg.is_wstrb  = 0;
		cmdlst_wr_cfg.reg_rd_addr = IPP_BASE_ADDR_KLT + IPP_KLT_MC_NEXT_KP_0_REG;
		cmdlst_wr_cfg.buff_wr_addr = klt_req->klt_stat_buff[KLT_MC_KP_NEXT_PREV_BUFF].buffer;
		loge_if_set_ret(seg_ipp_set_cmdlst_wr_buf(&cmd_link_entry[(*index)++].cmd_buf, &cmdlst_wr_cfg), ret);
		if (ret) return ISP_IPP_ERR;
	}
	...
	global_info_ipp_t *global_info)
{
	...
	case KLT_DS_AND_KLT:
		...
		if (global_info->mc_update_flag && global_info->mc_en) {
			loge_if_set_ret(seg_klt_update_mc_cmdlst_buf(cmd_link_entry, global_info, &i), ret);
			if (ret) return ISP_IPP_ERR;
		}
