...
#define ST_INITED_MSG             0U
#define ERR_HIPHY_MSG             1U
...
static unsigned int hiphyio_state = 0;
...
#else
	if (dev->phy_param_flag) {
		hisp_smc_phy_csi_connect_no_addr(phy_info->phy_id, phy_info->phy_work_mode, csi_index);
	} else {
		...
		if (ret != 0) {
			hisp_err("%s: copy phy info to cis phy info fail\n", __func__);
			return -EINVAL;
		}
...
#ifdef ISP_DMA_SPLITTED_SID
struct device *hisp_get_ispdma_device(void)
{
	struct hisp_boot_device *dev = &hisp_boot_dev;

	if (dev->ispdma_pdev == NULL) {
		pr_err("[%s] Failed: ispfw_pdev is NULL!\n", __func__);
		return NULL;
	}

	return &dev->ispdma_pdev->dev;
}
#endif
...
unsigned int hisp_wait_excflag_timeout(unsigned int flag, unsigned int time)
{
	...
	pr_debug("[%s] exc_flag.0x%x != flag.0x%x, timeout.%u\n",
...
#ifdef CONFIG_FFRT_UNIFORM_ID
static uint16_t hisp_share_get_ffrt_uid(void)
{
	uint16_t uid = 0;
	uid = ffrt_get_uniform_id(current->tgid);
	if (uid == FFRT_INVALID_UNIFORM_ID)
		hisp_err("uid invalid\n");
	return uid;
}
#endif
...
{
#ifdef CONFIG_FFRT_UNIFORM_ID
    struct hisp_shared_para *param = hisp_share_get_para();
	if (!param)
		return;

	if (force || atomic_read(&param->ffrt_uid) == 0)
        atomic_set(&param->ffrt_uid, hisp_share_get_ffrt_uid());
#endif
}
...
u32 hisp_share_get_exc_flag(void)
{
	...
	if (share_para == NULL) {
		hisp_err("Failed : hisp_share_get_para.\n");
...
static void hisp_set_iodie_flag(void)
{
	struct hisp_shared_para *share_temp = NULL;

	hisp_lock_sharedbuf();
	share_temp = hisp_share_get_para();
	if (share_temp == NULL) {
		hisp_unlock_sharedbuf();
		return;
	}
	if (hiphyio_state == ERR_HIPHY_MSG) {
		share_temp->iodie_status_flag = ERR_HIPHY_MSG;
		pr_err("iodie_status_flag: %d\n", share_temp->iodie_status_flag);
	} else if (hiphyio_state == ST_INITED_MSG) {
		share_temp->iodie_status_flag = ST_INITED_MSG;
		pr_err("iodie_status_flag: %d\n", share_temp->iodie_status_flag);
	}
	hisp_unlock_sharedbuf();
}

static void hiphyio_status_callback(enum iodie_sys_status sys_st, enum iodie_err_status err_st)
{
	hiphyio_state = 0;
	pr_err("iodie sys status %u, iodie err status %u\n", (uint32_t)sys_st, (uint32_t)err_st);

	if ((sys_st == IODIE_ST_ERR) && (err_st == IODIE_ERR_HIPHY)) {
		hiphyio_state = ERR_HIPHY_MSG;
		hisp_set_iodie_flag();
	} else if ((sys_st == IODIE_ST_INITED) && (err_st == IODIE_ERR_NO)) {
		hiphyio_state = ST_INITED_MSG;
		hisp_set_iodie_flag();
	}
}

int __attribute__((weak)) iodie_notify_register(enum iodie_user user, iodie_user_func func)
{
	(void)user;
	(void)func;
	return 0;
...
#ifdef ISP_LINX_CPU_SUPPORT
static int hisp_check_ispcpu_reg_avail(unsigned int addr)
{
	if (addr < (unsigned int)DEVICE_BASE || addr >= (unsigned int)(DEVICE_BASE - 1 + DEVICE_SIZE)) {
		pr_err("%s, addr error\n", __func__);
		return -1;
	}
...
int hisp_nsec_reg_space_map_for_isp(struct rproc *rproc)
{
	struct hisp_boot_device *hisp_dev = &hisp_boot_dev;
	struct iommu_domain *domain = hisp_rproc_get_domain(rproc);
	unsigned int i;
	int ret = 0;
	...
	if (domain == NULL) {
		hisp_info("%s get domain is NULL\n", __func__);
		return 0;
	}

	for (i = 0; i < hisp_dev->reg_num_ispcpu; i++) {
		ret = hisp_check_ispcpu_reg_avail(hisp_dev->reg_ispcpu[i].addr);
		if (ret != 0) {
			pr_err("%s, addr[%u] unavail\n", __func__, i);
			return ret;
		}

		ret = iommu_map(domain, hisp_dev->reg_ispcpu[i].addr, (u64)hisp_dev->reg_ispcpu[i].addr,
					hisp_dev->reg_ispcpu[i].size, (IOMMU_WRITE | IOMMU_READ | IOMMU_MMIO | IOMMU_NOEXEC));
		if (ret) {
			pr_err("%s, iommu_map[%d] failed: %d\n", __func__, i, ret);
			return ret;
		}
	}
...
int hisp_nsec_reg_space_unmap_for_isp(void)
{
	struct hisp_boot_device *hisp_dev = &hisp_boot_dev;
	struct device *device = hisp_get_device();
	unsigned int i;
	size_t phy_len = 0;
	int ret = 0;
	...
	for (i = 0; i < hisp_dev->reg_num_ispcpu; i++) {
		...
		phy_len = mm_iommu_unmap_fast(device, (u64)hisp_dev->reg_ispcpu[i].addr, hisp_dev->reg_ispcpu[i].size);
		if (phy_len != hisp_dev->reg_ispcpu[i].size) {
			pr_err("%s, iommu_unmap[%u] failed: %u\n", __func__, i, phy_len);
			return -EINVAL;
		}
	}
	hisp_info("-\n");
...
static int hisp_device_disable(void)
{
	...
	hisp_info("+\n");
	mutex_lock(&dev->ispdev_mutex);
	if (dev->ispcpu_status == 0) {
		pr_err("%s: isp already off\n", __func__);
		mutex_unlock(&dev->ispdev_mutex);
		return 0;
	}
	switch (dev->case_type) {
	case SEC_CASE:
	...
	mutex_unlock(&dev->ispdev_mutex);/*lint !e456 */
	hisp_info("-\n");
...
static int hisp_power_on_work(void *data)
{
	...
	ret = iodie_notify_register(IODIE_USER_ISP_I2C, hiphyio_status_callback);
	if (ret != 0) {
		hisp_err("Failed : iodie notify register: %d\n", ret);
	}
	...
	while (!kthread_should_stop()) {
		ret = wait_event_interruptible(dev->pwronisp_wait,
			dev->pwronisp_wake);
		if (ret < 0) {
			hisp_info("fail to wait for pwronisp_wake.%d\n", ret);
			continue;
		}
...
int hisp_wait_rpmsg_completion(struct rproc *rproc)
{
	...
	ret = (unsigned int)wait_for_completion_timeout(&hisp_dev->rpmsg_boot_comp,
...
static int hisp_nsec_bootimg_init(struct rproc *rproc)
{
	...
	(void)domain;
	if (isp_bw_va == NULL) {
		hisp_err("%s: get early boot image va fail\n", __func__);
		return -ENOMEM;
	}

	ret = memcpy_s(isp_bw_va, ISP_MEM_SIZE, imgva, PAGE_SIZE * 2);
	if (ret != 0) {
		hisp_err("%s: copy text to boot image fail\n", __func__);
		return ret;
	}
	__flush_dcache_area(isp_bw_va, ISP_MEM_SIZE);
#else
...
#ifdef HISP_FUZZ
	hisp_fuzz_kcov_shadow_mem_deinit();
#endif
#ifdef ISP_BUF_PRIVATIZE
	hisp_mem_list_clean();
#else
...
#ifdef ISP_LINX_CPU_SUPPORT
	if (hisp_nsec_boot_mode()) {
		err = hisp_nsec_reg_space_unmap_for_isp();
		if (err < 0) {
			hisp_err("Failed : hisp_nsec_reg_space_unmap_for_isp.0x%x\n", err);
			return err;
		}
	}
#endif
...
static void hisp_dyn_bootaddr_alloc(void)
{
	...
	if (hisp_dev->dyn_bootflag == ISP_DYNBOOT_ENABLE) {
		...
		hisp_dev->dyn_bootaddr = (num & (unsigned int)(GENMASK(15, 12))) | BIT(16);
	} else {
...
bool hisp_ltar_enabled(void)
{
	struct hisp_boot_device *hisp_dev = &hisp_boot_dev;
	return hisp_dev->ltar_enabled;
}
...
#ifdef ISP_LINX_CPU_SUPPORT
	if (hisp_nsec_boot_mode()) {
		err = hisp_nsec_reg_space_map_for_isp(rproc);
		if (err < 0) {
			hisp_err("Failed : hisp_nsec_reg_space_map_for_isp.0x%x\n", err);
			atomic_set(&hisp_dev->rproc_enable_status, 0);
			return err;
		}
	}
...
static int hisp_pwron_task_init(struct hisp_boot_device *hisp_dev)
{
	...
	ret = sched_setscheduler(hisp_dev->pwronisp, SCHED_RR, &param);
	if (ret < 0) {
			...
			struct hisp_boot_device *dev)
{
	...
	ret = of_property_read_u32(np, "isp-ltar-enabled", &dev->ltar_enabled);
	if (ret < 0) {
		dev->ltar_enabled = 0;
		pr_info("[%s]: not supported isp load-time aslr relocator\n", __func__);
	}
	pr_info("isp-ltar-enabled.0x%x of_property_read_u32.%d\n",
			dev->ltar_enabled, ret);
		...
		struct hisp_boot_device *rproc_dev)
{
	...
	for (i = 0; i < min_num; i++) {
		...
		if (rproc_dev->reg[i] == NULL) {
			pr_err("[%s] resource.%u ioremap fail\n", __func__, i);
			goto error;
		}
	}

#ifdef ISP_LINX_CPU_SUPPORT
	ret = of_property_read_u32(np, "reg-num-ispcpu",
		(unsigned int *)(&rproc_dev->reg_num_ispcpu));
	if (ret < 0) {
		pr_err("[%s] Failed: reg-num-ispcpu,ret.%d,maybe not supported\n", __func__, ret);
		rproc_dev->reg_num_ispcpu = 0;
	}

	rproc_dev->reg_num_ispcpu = hisp_min(ISP_MAX_REGTYPE_FOR_ISPCPU, rproc_dev->reg_num_ispcpu);

	if (rproc_dev->reg_num_ispcpu == 0)
		return 0;

	ret = of_property_read_u32_array(np, "reg-ispcpu",
				(unsigned int *)&(rproc_dev->reg_ispcpu), rproc_dev->reg_num_ispcpu * 2);
	if (ret < 0) {
		pr_err("[%s] Failed: reg-ispcpu,ret.%d with reg_num_isppcu.%d\n",
			__func__, ret, rproc_dev->reg_num_ispcpu);
		memset_s(rproc_dev->reg_ispcpu, sizeof(rproc_dev->reg_ispcpu), 0,
				sizeof(rproc_dev->reg_ispcpu));
		goto error;
	}
#endif
...
error:
	for (j = 0; j < i; j++) {//lint !e442
		if (rproc_dev->reg[j] != NULL) {
			iounmap(rproc_dev->reg[j]);
			rproc_dev->reg[j] = NULL;
		}
	}
	return -ENOMEM;
}
		...
		struct hisp_boot_device *dev)
{
	...
	ret = of_property_read_u32(np, "phy-param-flag", &dev->phy_param_flag);/*lint !e64*/
	if (ret) {
		pr_info("%s: no phy-param-flag, set default 0\n", __func__);
		dev->phy_param_flag = 0;
	}
	...
	struct hisp_boot_device *hisp_dev)
{
	...
	mutex_init(&hisp_dev->ispdev_mutex);
...
jpeg_wakelock_fail:
	...
	mutex_destroy(&hisp_dev->ispdev_mutex);
...
#ifdef ISP_DMA_SPLITTED_SID
int hisp_ispdma_device_attach(struct platform_device *pdev)
{
	struct hisp_boot_device *hisp_dev = &hisp_boot_dev;

	hisp_dev->ispdma_pdev = pdev;

	return 0;
}

int hisp_ispdma_device_remove(struct platform_device *pdev)
{
	struct hisp_boot_device *hisp_dev = &hisp_boot_dev;

	(void)pdev;

	hisp_dev->ispdma_pdev = NULL;
...
const char *hisp_get_firmware(void)
{
	struct hisp_boot_device *hisp_dev = &hisp_boot_dev;
	struct rproc *rproc = NULL;
	rproc = platform_get_drvdata(hisp_dev->isp_pdev);
