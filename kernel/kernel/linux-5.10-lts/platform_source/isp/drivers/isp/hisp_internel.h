...
#define ISPMSG_TYPE_KERNL     0x1
#define ISPMSG_TYPE_OTHER     0x0
...
struct hisp_shared_para {
	...
	atomic_t ffrt_uid;
#endif
#ifdef CONFIG_HISP_FE_PG
	unsigned int fe_pg_efuse;
#endif
	unsigned int log_hpl_enable;
	unsigned int log_read_offset;
	unsigned int iodie_status_flag;
};
...
#define ISP_MSG_MAX_PAYLOAD 512
struct isp_msg_content {
	struct hisp_msg_info info;
	char data[ISP_MSG_MAX_PAYLOAD - sizeof(struct rpmsg_hdr) - sizeof(struct hisp_msg_info)];
};
...
typedef struct hisp_prog_blk {
    uint8_t *mem;
    uint8_t *iova;
    size_t size;
} hisp_prog_blk_t;

typedef struct hisp_prog_info {
    hisp_prog_blk_t text;
    hisp_prog_blk_t data;
} hisp_prog_info_t;
...
int hisp_smc_phy_csi_connect_no_addr(u32 phy_id, u32 phy_work_mode, u32 csi_idx);
...
extern int hisp_msg_service_recv(void *data, unsigned int len, unsigned int src);
extern int hisp_msg_service_send(void *data, unsigned int len);
