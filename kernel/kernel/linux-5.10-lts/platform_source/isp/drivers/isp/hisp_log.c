...
#define LOG_READ_OFFSET     _IOWR(ISP_IOCTL_MAGIC, 0x04, int)
...
static int hisplog_ioctl_check(unsigned int cmd, unsigned long args)
{
	...
	if (args == 0) {
		pr_err("[%s] cmd[%u] args NULL", __func__, cmd);
		return -EFAULT;
	}
...
static void hisplog_config_user_para(struct log_user_para *tmp)
{
	...
	tmp->log_hpl_enable = dev->share_para->log_hpl_enable;
		...
		unsigned long args)
{
	...
	case LOG_WR_OFFSET:
		...
		ret = (int)wait_event_timeout(dev->wait_ctl,
	...
	case LOG_VERSION_TYPE:
		...
		ret = (int)copy_from_user(&dev->version_type,
			(void __user *)(uintptr_t)args, (unsigned long)sizeof(int));
		if (ret != 0) {
	...
	case LOG_READ_OFFSET:
		if (_IOC_TYPE(cmd) != ISP_IOCTL_MAGIC) {
			pr_err("[%s] readoffset type is wrong.\n", __func__);
			return -EINVAL;
		}

		if (_IOC_NR(cmd) >= ISP_IOCTL_MAX_NR) {
			pr_err("[%s] readoffset number is wrong.\n", __func__);
			return -EINVAL;
		}

		ret = (int)copy_from_user(&dev->share_para->log_read_offset,
			(void __user *)(uintptr_t)args, sizeof(unsigned int));
		if (ret != 0) {
			pr_err("[%s] readoffset copy_from_user failed.\n", __func__);
			return -EFAULT;
		}
		break;
		...
		struct device_attribute *attr, char *buf)
{
	...
	for (i = 0; i < ARRAY_SIZE(loglevel); i++)
		size += snprintf_s(tmp+size, ISP_LOG_MAX_SIZE,
			...
			(param ?
			(((logx_switch & loglevel[i].level) != 0) ?
			loglevel[i].enable_cmd : loglevel[i].disable_cmd) :
			"ispoffline"),
			(((dev->local_loglevel & loglevel[i].level) != 0) ?
			loglevel[i].enable_cmd : loglevel[i].disable_cmd),
