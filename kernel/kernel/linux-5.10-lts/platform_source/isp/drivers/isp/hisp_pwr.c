...
#define EFUSE_PARTIALGOOD_FLAG8_START_BIT		768
#define EFUSE_DLDO_START_BIT                    (883 - (EFUSE_PARTIALGOOD_FLAG8_START_BIT))
#define EFUSE_DLDO_BIT_SIZE		                10

#define EFUSE_PARTIALGOOD_FLAG1_START_BIT		0
#define EFUSE_FT_START_BIT                      (121 - (EFUSE_PARTIALGOOD_FLAG1_START_BIT))
#define EFUSE_FT_BIT_SIZE	                    1

#define DLDO_EFUSE_MAX_VALUE 					(881)
#define DLDO_EFUSE_MIN_VALUE 					(1)

#define ALL_FE_EFUSE                            (0x7)
#define PG_FE_0_1_EFUSE                         (0x3)
...
enum binded_clock_num_e {
	SOURCE_CLOCK_NUM     = 0,
	TARGET_CLOCK_NUM     = 1,
	MAX_BINED_CLOCK_TYPE = 2
};
...
struct pll6_usage_item {
    unsigned int type;
    unsigned int pll6_freq;
    unsigned int stub_freq;
};
...
struct hisp_pwr {
	...
	unsigned int binded_clock_flag;
	unsigned int ispsmmu_init_byap;
	unsigned int clock_num;
	unsigned int pwstat_num;
	unsigned int pll6_usage_num;
	unsigned int ispcpu_switch_volt_feq;
#ifdef DEBUG_HISP
	...
	unsigned int binded_clock_num[MAX_BINED_CLOCK_TYPE];
	unsigned int binded_vote_level[ISP_CLK_MAX];
	unsigned int binded_source_clock[ISP_CLK_MAX];
	unsigned int binded_target_clock[ISP_CLK_MAX];
	struct pll6_usage_item pll6_usage_tbl[PLL6_USAGE_NUM];

	unsigned int efuse_dldo;
	unsigned int ft_dldo;
};
		...
		unsigned int length, char *buffer)
{
	...
	if (ret != (int)length) {
		pr_err("read ops failed, read ret=%d(length=%u)", ret, length);
		set_fs(fs);
		ret = -1;
		goto err_close;
	}
...
static unsigned int pll6_usage_search(struct pll6_usage_item *arr, unsigned int max_item,
	unsigned int type, unsigned int rate)
{
	unsigned int i;

	for (i = 0; i < max_item; i++)
		if ((arr[i].type == type) && (arr[i].pll6_freq == rate))
			return i;

	return max_item;
}

static int pll6_clk_set(unsigned int type, unsigned int rate)
{
	struct hisp_pwr *dev = &isp_pwr_dev;
	struct pll6_usage_item *pll6_tbl;
	unsigned int item;
	unsigned int tbl_size;
	int ret;
	pll6_tbl = dev->pll6_usage_tbl;

	if (dev->pll6_usage_num == 0 || dev->pll6_usage_num > PLL6_USAGE_NUM) {
		pr_info("[%s] Notice: pll6_usage_num is %u, no need ppll6 setting\n", __func__, dev->pll6_usage_num);
		return 0;
	}

	tbl_size = dev->pll6_usage_num;

	item = pll6_usage_search(pll6_tbl, tbl_size, type, rate);
	if (item >= tbl_size)
		return 0;

	pr_info("[%s]: clk.0x%x.%s switch to sub freq[%u]\n", __func__, type,
		dev->clk_name[type], pll6_tbl[item].stub_freq);
	ret = clk_set_rate(dev->ispclk[type], (unsigned long)pll6_tbl[item].stub_freq);
	if (ret < 0) {
		pr_err("[%s] Failed to switch to sub freq, clk.0x%x.%s freq.%u\n", __func__, type,
		dev->clk_name[type], pll6_tbl[item].stub_freq);
		return ret;
	}
		...
		int clk, unsigned int clkdown)
{
	...
	do {
		...
		ret = clk_set_rate(dev->ispclk[clk], ispclk);
		if (ret < 0) {
			pr_err("[%s] Failed: clk_set_rate.%u > %u\n",
		...
		if (ret < 0) {
			pr_err("[%s] Failed: clk_prepare_enable.%d, %u > %u\n",
...
static int hisp_pwr_clock_enable(struct hisp_pwr *dev, unsigned int clk)
{
	unsigned long ispclock = 0;
	int ret;
#ifdef DEBUG_HISP
	...
	if (ret == 0) {
		ret = clk_prepare_enable(dev->ispclk[clk]);
		if (ret < 0) {
			pr_err("[%s] Failed: %u.%s.clk_prepare_enable.%d\n",
				__func__, clk, dev->clk_name[clk], ret);
			return -EINVAL;
		}
		pr_info("[%s] %u.%s.clk_prepare_enable\n",
	...
	if (pll6_clk_set(clk, (unsigned int)ispclock) < 0) {
		pr_err("[%s] Failed: pll6_clk_set.%u > %ul\n", __func__, clk, ispclock);
		goto try_clock_down;
	}

	ret = clk_set_rate(dev->ispclk[clk], ispclock);
	if (ret < 0) {
		pr_err("[%s] Failed: %d.%d M, %u.%s.clk_set_rate.%d\n",
	...
	ret = clk_prepare_enable(dev->ispclk[clk]);
	if (ret < 0) {
		pr_err("[%s] Failed: %d.%d M, %u.%s.clk_prepare_enable.%d\n",
			__func__, (int)ispclock / HZ_TO_MHZ_DIV,
			(int)ispclock % HZ_TO_MHZ_DIV,
			clk, dev->clk_name[clk], ret);
		goto try_clock_down;
	...
	pr_info("[%s] %u.%s.clk_set_rate.%d.%d M\n", __func__,
...
static int hisp_pwr_get_isppwr_state(unsigned int type)
{
	...
	if (type >= pwsta_num) {
		pr_err("[%s] Failed: type.%u, pwstat_num.%u\n",
			__func__, type, pwsta_num);
		return 0;
	}

	if (dev->pwstat_offset[type] >= ISP_CFG_SIZE) {
		pr_err("[%s] Failed: pwstat_offset.0x%x, type.%u\n",
...
smc_disreset_cpu:
	...
	(void)remap_addr;
	ret = hisp_ffa_disreset_ispcpu(canary, hisp_get_dyn_bootaddr());
#else
...
static unsigned int hisp_switch_rate_to_level(unsigned int type, unsigned int rate)
{
	struct hisp_pwr *dev = &isp_pwr_dev;
	unsigned int level = HISP_CLK_SVS;

	if (rate < dev->clkdn[HISP_CLK_HIGHSVS][type]) {
		level = HISP_CLK_SVS;
	} else if (rate > dev->clkdn[HISP_CLK_HIGHSVS][type] && rate < dev->clkdn[HISP_CLK_NORMINAL][type]) {
		level = HISP_CLK_HIGHSVS;
	} else if (rate > dev->clkdn[HISP_CLK_NORMINAL][type] && rate < dev->clkdn[HISP_CLK_TURBO][type]) {
		level = HISP_CLK_NORMINAL;
	} else if (rate >= dev->clkdn[HISP_CLK_TURBO][type]) {
		level = HISP_CLK_TURBO;
	}

	return level;
}

static unsigned int hisp_refresh_current_level(unsigned int type, unsigned int level)
{
	struct hisp_pwr *dev = &isp_pwr_dev;
	unsigned int target_level = HISP_CLK_SVS;
	unsigned int type_source;
	unsigned int num_source;
	unsigned int i;

	// Store current type clk level
	dev->binded_vote_level[type] = level;

	num_source = min(dev->clock_num, dev->binded_clock_num[SOURCE_CLOCK_NUM]);

	// Check current max level for all source clks
	for (i = 0; i < num_source; i++) {
		type_source = dev->binded_source_clock[i];
		if (type_source >= dev->clock_num) {
			pr_err("[%s] source clock[%u].%u invalid", __func__, i, type_source);
			return HISP_CLK_DVFS_MAX;
		}

		if (dev->binded_vote_level[type_source] < target_level) {
			target_level = dev->binded_vote_level[type_source];
		}
	}
...
static int hisp_set_binded_clock(unsigned int type, unsigned int rate)
{
	struct hisp_pwr *dev = &isp_pwr_dev;
	int ret;
	unsigned int level = HISP_CLK_SVS;
	...
	level = hisp_switch_rate_to_level(type, rate);
	if (level >= HISP_CLK_DVFS_MAX) {
		pr_info("[%s] clk.0x%x.%s switch Rate.%u to level.%u Failed",
			__func__, type, dev->clk_name[type], rate, level);
		return 0;
	}
	level = hisp_refresh_current_level(type, level);
	if (level >= HISP_CLK_DVFS_MAX) {
		pr_info("[%s] clk.0x%x.%s refresh Rate.%u to level.%u Failed",
			__func__, type, dev->clk_name[type], rate, level);
		return 0;
	}

	// Fresh Target Clock Rate
	for (i = 0; i < dev->binded_clock_num[TARGET_CLOCK_NUM]; i++) {
		target_type = dev->binded_target_clock[i];
		if (target_type >= dev->clock_num) {
			pr_err("[%s] target clock[%u].%u invalid", __func__, i, target_type);
			return -EINVAL;
		}

		pr_info("[%s] DVFS Set.0x%x.%s Rate.%d. %d.%08d M\n", __func__, target_type, dev->clk_name[target_type],
			dev->clkdn[level][target_type], dev->clkdn[level][target_type] / HZ_TO_MHZ_DIV,
			dev->clkdn[level][target_type] % HZ_TO_MHZ_DIV);
		ret = pll6_clk_set(target_type, dev->clkdn[level][target_type]);
		if (ret < 0) {
			pr_err("[%s] Failed: pll6_clk_set.%u > %ul\n",
				__func__, target_type, dev->clkdn[level][target_type]);
			return ret;
		}

		ret = clk_set_rate(dev->ispclk[target_type], (unsigned long)dev->clkdn[level][target_type]);
		if (ret < 0) {
			pr_err("[%s] Failed : DVFS Set.0x%x.%s Rate.%d. %d.%08d M\n",
					__func__, target_type, dev->clk_name[target_type],
					dev->clkdn[level][target_type],
					dev->clkdn[level][target_type] / HZ_TO_MHZ_DIV,
					dev->clkdn[level][target_type] % HZ_TO_MHZ_DIV);
			return ret;
		}
...
static unsigned int hisp_get_efuse(struct efuse_desc desc)
{
	unsigned int efuse = 0;
	size_t e_size = 0;

	desc.buf = &efuse;
	e_size = sizeof(efuse) / sizeof(unsigned int);
	desc.buf_size = (unsigned int) e_size;
	if (efuse_read_value_t(&desc) != EFUSE_OK) {
		pr_err("dldo efuse read failed");
		return 0;
	}
...
static void hisp_pwr_get_dldo_efuse(void) {
	struct hisp_pwr *dev = &isp_pwr_dev;
	struct efuse_desc desc = {0};

	desc.start_bit = EFUSE_DLDO_START_BIT;
	desc.bit_cnt = EFUSE_DLDO_BIT_SIZE;
	desc.item_vid = EFUSE_KERNEL_PARTIALGOOD_FLAG8;

	dev->efuse_dldo = hisp_get_efuse(desc);
	if (dev->efuse_dldo > DLDO_EFUSE_MAX_VALUE || dev->efuse_dldo < DLDO_EFUSE_MIN_VALUE) {
		pr_info("Do not support dldo efuse, val = %u", dev->efuse_dldo);
	} else {
		pr_info("dldo efuse val = %u", dev->efuse_dldo);
	}

	// FT version dldo en bit
	desc.start_bit = EFUSE_FT_START_BIT;
	desc.bit_cnt = EFUSE_FT_BIT_SIZE;
	desc.item_vid = EFUSE_KERNEL_PARTIALGOOD_FLAG1;

	dev->ft_dldo = hisp_get_efuse(desc);
	if (dev->ft_dldo == 0) {
		pr_info("FT version do not support dldo volt change, val = %u", dev->ft_dldo);
	} else {
		pr_info("FT version support dldo volt change, val = %u", dev->ft_dldo);
	}
}

static unsigned int hisp_if_dldo_change_en(void) {
	struct hisp_pwr *dev = &isp_pwr_dev;

	if (dev->ispcpu_switch_volt_feq < dev->clkdis_dvfs[ISPCPU_CLK] || dev->ispcpu_switch_volt_feq > dev->ispclk_value[ISPCPU_CLK])
		return 0;
...
#if defined(CONFIG_SOC_PG_INFO) && defined(CONFIG_HISP_FE_PG)
static int hisp_get_fe_pg(void) {
	struct hisp_shared_para *param = NULL;
	struct soc_component_pg_info pg_info;
 
	get_soc_pg_info_by_index(SOC_COMPONENT_MEDIA2_ISP_FE_INFO, &pg_info);
	pr_info("efuse_value: 0x%08x, tailor_strategy: %d", pg_info.efuse_value, pg_info.tailor_strategy);

	if (pg_info.tailor_strategy == SOC_SPEC_EFUSE_INVALID_VALUE ||
		pg_info.tailor_strategy == SOC_COMPONENT_TAILOR_STRATEGY_2)
		return -EINVAL;

	param = hisp_share_get_para();
	if (param == NULL) {
		pr_err("[%s] share param is NULL!\n", __func__);
		return -EINVAL;
	}

	if (pg_info.tailor_strategy == SOC_COMPONENT_NOT_TAILOR ||
		pg_info.tailor_strategy == SOC_COMPONENT_NOT_TAILOR_EFUSE_NONE) {
		param->fe_pg_efuse = ALL_FE_EFUSE;
	} else if (pg_info.tailor_strategy == SOC_COMPONENT_TAILOR_STRATEGY_1 && pg_info.efuse_value == ALL_FE_EFUSE){
        pr_info("NOTICE:SOC_COMPONENT_TAILOR_STRATEGY_1, 3 FE Valiable, Only Use FE 0, FE 1");
		param->fe_pg_efuse = PG_FE_0_1_EFUSE;
	} else {
		param->fe_pg_efuse = pg_info.efuse_value;
	}
...
static int hisp_pwr_setclkrate(unsigned int type, unsigned int rate)
{
	...
	if (ret != 0) {
		pr_err("[%s] Failed : DVFS type.0x%x, rate.%u, dvfsmask.0x%x\n",
				__func__, type, rate, dev->dvfsmask);
		return -EINVAL;
	}

	if ((rate > dev->ispclk_value[type]) || (rate == 0)) {
		pr_err("[%s] Fail: type.0x%x.%s, %u. %d.%08dM > %d. %d.%08dM\n",
...
#ifdef CONFIG_HISP_FFA_SUPPORT
	dldo_change_en = hisp_if_dldo_change_en();
	if (type == ISPCPU_CLK && dldo_change_en > 0 && rate > dev->ispcpu_switch_volt_feq) {
		ret	= hisp_ffa_set_ispcpu_clk(ISPCPU_VOLT_CMOS, dev->efuse_dldo);
		if (ret != 0) {
			pr_err("[%s] Failed : ISPCPU_CLK set DLOD fail ret:0x%x\n", __func__, ret);
			return ret;
		}
	}
#endif
	...
	if (ret < 0) {
		pr_err("[%s] Failed : DVFS Set.0x%x.%s Rate.%u. %d.%08d M\n",
				__func__, type, dev->clk_name[type], rate,
				rate / HZ_TO_MHZ_DIV, rate % HZ_TO_MHZ_DIV);
		return ret;
	}
...
#ifdef CONFIG_HISP_FFA_SUPPORT
	if (type == ISPCPU_CLK && dldo_change_en > 0 && rate <= dev->ispcpu_switch_volt_feq) {
		ret	= hisp_ffa_set_ispcpu_clk(ISPCPU_VOLT_DLDO, dev->efuse_dldo);
		if (ret != 0)
			pr_err("[%s] Failed : ISPCPU_CLK set DLOD fail ret:0x%x\n", __func__, ret);
	}
#endif
	...
	pr_info("[%s] DVFS Set.0x%x.%s Rate.%u. %d.%08d M\n",
		__func__, type, dev->clk_name[type], rate,
		rate / HZ_TO_MHZ_DIV, rate % HZ_TO_MHZ_DIV);

	if (dev->binded_clock_flag != 1)
		return 0;

	ret = hisp_set_binded_clock(type, rate);
	if (ret < 0) {
		pr_err("[%s] Failed : hisp_set_binded_clock %d\n", __func__, ret);
		return ret;
	}
...
static int hisp_pwr_subsys_common_up(void)
{
	...
	for (index = 0; index < dev->clock_num; index++) {
		ret = hisp_pwr_clock_enable(dev, index);
		if (ret < 0) {
			pr_err("[%s] Failed: hisp_pwr_clock_enable.%d, index.%u\n",
					__func__, ret, index);
			goto err_ispclk;
		}
	}
...
err_isptop:
#ifdef DEBUG_HISP
	if (status == ISP_PCIE_MODE)
		err = hisp_pwr_subsys_common_dn_pcie();
	else
#endif
...
#ifdef CONFIG_HISP_FFA_SUPPORT
	u64 early_pa;
	early_pa = hisp_nsec_get_remap_pa();
	if (early_pa == 0ULL) {
		pr_err("%s: get early boot mem pa fail\n", __func__);
		mutex_unlock(&dev->pwrlock);
		return -EINVAL;
	}
	hisp_ffa_ispmmu_init(pgd_base, early_pa);
#else
...
static inline bool in_range(uintptr_t addr, uintptr_t start, uintptr_t size)
{
    return (addr >= start) && (addr < (start + size));
}

uintptr_t OsAslrMemMap(uintptr_t addr)
{
	hisp_prog_info_t *pcb = hisp_prog_info_get();
    if (in_range(addr, (uintptr_t)pcb->text.iova, pcb->text.size)) {
        return (addr - (uintptr_t)pcb->text.iova) + (uintptr_t)pcb->text.mem;
    } else if (in_range(addr, (uintptr_t)pcb->data.iova, pcb->data.size)) {
        return (addr - (uintptr_t)pcb->data.iova) + (uintptr_t)pcb->data.mem;
    } else {
		return 0;
	}
}

static inline void hisp_flush_image_va(void)
{
	hisp_prog_info_t *pcb = hisp_prog_info_get();
	__flush_dcache_area(pcb->text.mem, pcb->text.size);
	__flush_dcache_area(pcb->data.mem, pcb->data.size);
}

#define HISP_FW_NAME_MAX_LEN 			31
#define HISP_FW_PATH_MAX_LEN			(HISP_FW_NAME_MAX_LEN + 32 + 1)
#define HISP_FW_ROOT_DIR_OHOS			"/chip_prod/etc/firmware"
#define HISP_FW_ROOT_DIR				"/odm/etc/firmware"

static bool hisp_firmware_name_ok(const char *firmware)
{
	if (strstr(firmware, "..")) {
		return false;
	}
	if ((strlen(firmware) == 0) || (strlen(firmware) > HISP_FW_NAME_MAX_LEN)) {
		return false;
	}
	return true;
}

static int hisp_get_firmware_fullpath(const char *firmware, char *buf, size_t max_len)
{
	size_t idx;
	int ret;
	...
	const char *dirs[] = {
		HISP_FW_ROOT_DIR,
		HISP_FW_ROOT_DIR_OHOS,
	};

	for (idx = 0; idx < ARRAY_SIZE(dirs); idx++) {
		ret = snprintf_s(buf, max_len, HISP_FW_PATH_MAX_LEN - 1, "%s/%s", dirs[idx], firmware);
		if (ret <= 0) {
			return ret;
		}
		filp = filp_open(buf, O_RDONLY, 0400);
...
static int hisp_nsec_image_aslr(void)
{
	int ret;
	int32_t random;
	ElfFile *elf = NULL;
	...
	char fw_fullpath[HISP_FW_PATH_MAX_LEN];

	if (!hisp_ltar_enabled()) {
		return 0;
	}

	firmware = hisp_get_firmware();
	if (firmware == NULL || !hisp_firmware_name_ok(firmware)) {
		pr_info("[%s] get firmware failed\n", __func__);
		return -1;
	}

	ret = hisp_get_firmware_fullpath(firmware, fw_fullpath, sizeof(fw_fullpath));
	if (ret != 0) {
		pr_info("[%s] get firmware fullpath failed\n", __func__);
		return -1;
	}
	elf = elf_load(fw_fullpath);
	if (elf == NULL) {
		pr_info("[%s] elf_load failed\n", __func__);
		return -1;
	}

	aslr = aslr_cb_create(elf);
	if (aslr == NULL) {
		pr_info("[%s] aslr_cb_create failed\n", __func__);
		goto FREE_ELF;
	}

	random = hisp_get_dyn_bootaddr();

	ret = do_aslr_reloc(aslr, random);
	if (ret != 0) {
		pr_info("[%s] do_aslr_reloc failed\n", __func__);
		goto FREE_ASLR_CB;
	}

	/* do_aslr_reloc changed the content of loaded isp image, 
	 * make sure everything changed flush to memory */
	hisp_flush_image_va();

	aslr_cb_destroy(aslr);
	elf_destroy(elf);
...
int hisp_pwr_cpu_nsec_dst(u64 remap_addr, unsigned int canary)
{
	...
	ret = hisp_nsec_image_aslr();
	if (ret < 0) {
		pr_err("[%s] Failed : hisp_nsec_image_aslr.%d\n", __func__, ret);
		mutex_unlock(&dev->pwrlock);
		return ret;
	}
...
int hisp_pwr_cpu_nsec_rst(void)
{
	...
	if ((hisp_nsec_boot_mode() != 0) || (hisp_ca_boot_mode() == 0)) {
#ifdef CONFIG_HISP_FFA_SUPPORT
		ret = hisp_ffa_reset_ispcpu();
		if (ret != 0)
			pr_err("[%s] Failed : hisp ispcpu power down.%d\n", __func__, ret);
#endif
	}
...
static int hisp_pwr_clock_binded_getdts(struct device_node *np,
			struct hisp_pwr *dev)
{
	int ret;
	unsigned int i;
	...
	ret = of_property_read_u32_array(np, "binded-clock-num", dev->binded_clock_num, MAX_BINED_CLOCK_TYPE);
	if (ret < 0) {
		pr_err("[%s] Failed: binded_clock_num get from dtsi.%d\n", __func__, ret);
		return ret;
	}

	ret = of_property_read_u32_array(np, "binded-source-clock",
			dev->binded_source_clock, dev->binded_clock_num[SOURCE_CLOCK_NUM]);
	if (ret < 0) {
		pr_err("[%s] Failed: binded_source_clock get from dtsi.%d\n", __func__, ret);
		return ret;
	}

	ret = of_property_read_u32_array(np, "binded-target-clock",
			dev->binded_target_clock, dev->binded_clock_num[TARGET_CLOCK_NUM]);
	if (ret < 0) {
		pr_err("[%s] Failed: binded_target_clock get from dtsi.%d\n", __func__, ret);
		return ret;
	}

	for (i = 0; i < dev->binded_clock_num[SOURCE_CLOCK_NUM]; i++) {
		source_clk = dev->binded_source_clock[i];
		if (source_clk >= dev->clock_num) {
			pr_err("[%s] Failed: binded_target_clock[%u].%u invalid\n", __func__, i, source_clk);
			return -EINVAL;
		}
		dev->binded_vote_level[source_clk] = HISP_CLK_SVS;
	}
			...
			struct hisp_pwr *dev)
{
	...
	ret = of_property_read_u32(np, "binded-clock-flag",
				(unsigned int *)(&dev->binded_clock_flag));
	if (ret < 0) {
		pr_err("[%s] Failed: binded_clock_flag get from dtsi.%d\n", __func__, ret);
		dev->binded_clock_flag = 0;
	}
	pr_info("[%s] binded_clock_flag.0x%x\n", __func__, dev->binded_clock_flag);

	ret = of_property_read_u32(np, "ispcpu_switch_volt_feq",
		(unsigned int *)(&dev->ispcpu_switch_volt_feq));
	if (ret < 0) {
		pr_info("[%s] Notice: ispcpu_switch_volt_feq,ret.%d, maybe not supported\n", __func__, ret);
		dev->ispcpu_switch_volt_feq = 0;
	}
	if (dev->ispcpu_switch_volt_feq < dev->clkdis_dvfs[ISPCPU_CLK] ||
		dev->ispcpu_switch_volt_feq > dev->ispclk_value[ISPCPU_CLK]) {
		pr_info("[%s] Notice: ispcpu_switch_volt_feq get from dts invalid, set to 0 %u\n",
				__func__, dev->ispcpu_switch_volt_feq);
		dev->ispcpu_switch_volt_feq = 0;
	}
	pr_info("[%s] ispcpu_switch_volt_feq is.%u\n", __func__, dev->ispcpu_switch_volt_feq);

	ret = of_property_read_u32(np, "pll6-usage-num", (unsigned int *)(&dev->pll6_usage_num));
	if (ret < 0) {
		pr_info("[%s] Notice: pll6-usage-num,ret.%d, maybe not supported\n", __func__, ret);
		dev->pll6_usage_num = 0;
	}
	dev->pll6_usage_num = hisp_min(PLL6_USAGE_NUM, dev->pll6_usage_num);

	ret = of_property_read_u32_array(np, "pll6-usage-tbl", (unsigned int *)(&dev->pll6_usage_tbl),
		dev->pll6_usage_num * (sizeof(struct pll6_usage_item) / sizeof(unsigned int)));
	if (ret < 0) {
		pr_info("[%s] Notice: pll6-usage-tbl, ret %d with pll6-usage-num %d.\n",
			__func__, ret, dev->pll6_usage_num);
		memset_s(dev->pll6_usage_tbl, sizeof(dev->pll6_usage_tbl), 0, sizeof(dev->pll6_usage_tbl));
		dev->pll6_usage_num = 0;
	}

	pr_info("[%s]: dump pll6_usage_tbl[%u] info:\n", __func__, dev->pll6_usage_num);

	if (dev->binded_clock_flag == 0)
		return 0;

	ret = hisp_pwr_clock_binded_getdts(np, dev);
	if (ret < 0) {
		pr_err("[%s] Failed: hisp_pwr_clock_binded_getdts.%d\n", __func__, ret);
		return ret;
	}
...
static int hisp_qos_getdts(struct device_node *np, struct hisp_pwr *dev)
{
	...
	if (ret < 0)
		pr_info("[%s] : get qos-sec-flag.%u, ret.%d\n",
			__func__, secreg, ret);
...
static int hisp_clk_show_ispcore(char *tmp, ssize_t *size)
{
	...
	for (index = ISPCPU_CLK; index < ISP_CLK_MAX; index++) {
		ret = snprintf_s(tmp + *size, NONSEC_MAX_SIZE,
			NONSEC_MAX_SIZE - 1,
			"%x: %lu  Real CLK:%lu\n",
			index, hisp_pwr_get_ispclk_freq(index),
