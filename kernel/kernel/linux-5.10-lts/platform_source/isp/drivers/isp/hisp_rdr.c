...
static bool hisp_rdr_get_wdt_flag(void)
{
	int ret;
	...
	ret = of_property_read_u32(np, "isp-wdt-flag", &wdt_flag);
	if (ret) {
		pr_err("%s: failed to get isp-wdt-flag resource.\n", __func__);
		return false;
	}

	of_node_put(np);

	pr_info("%s: comp.%s, isp-wdt-flag.%u.\n", __func__, name, wdt_flag);
	return (bool)wdt_flag;
}
...
{
	struct hisp_rdr_device *dev = &hisp_rdr_dev;
	struct hisp_rdr_syserr_dump_info *entry = NULL, *temp = NULL;
	unsigned int sync;
	int timeout = 20;

	(void)work;

	list_for_each_entry_safe(entry, temp, &dev->dump_list, node) {
...
static void hisp_rdr_noc_register(void)
{
	...
	for (i = 0; i < ARRAY_SIZE(ispnoc_err); i++) {
		pr_debug("%s: register rdr exception, i = %u, masterid:%d",
...
static int hisp_rdr_exception_register(struct hisp_rdr_device *dev)
{
	...
	for (i = 0; i < ARRAY_SIZE(exc_info); i++) {
		pr_info("%s: register rdr exception, i = %u, type:%d",
		...
		if (ret != exc_info[i].e_modid_end) {
			pr_info("%s: rdr_register_exception failed, ret.%u.\n",
					__func__, ret);
...
static void hisp_rdr_exception_unregister(struct hisp_rdr_device *dev)
{
	...
	for (i = ARRAY_SIZE(exc_info); i > 0; i--) {
		pr_info("%s: register rdr exception, i = %lu, type:%d",
...
static int hisp_rdr_dev_map(struct hisp_rdr_device *dev)
{
	...
	wdt_flag = hisp_rdr_get_wdt_flag();
	if (wdt_flag) {
		...
		int len, struct hisp_shared_para *param)
{
	...
	if (strncmp(buf, "sirq", (int)strlen("sirq")) == 0) {
		...
		ret = memcpy_s(q, (unsigned long)size, buf, len);
		if (ret != 0) {
		...
		struct device_attribute *attr, char *buf)
{
	...
	for (i = 0; i < ARRAY_SIZE(rdrlevel); i++)
		...
		size += snprintf_s(tmp + size, ISP_RDR_MAX_SIZE,
			...
			(param ? (((bool)(rdr_enable_type & rdrlevel[i].level)) ?
			rdrlevel[i].enable_cmd : rdrlevel[i].disable_cmd) :
			"ispoffline"),
			(((bool)(dev->offline_rdrlevel & rdrlevel[i].level)) ?
			rdrlevel[i].enable_cmd : rdrlevel[i].disable_cmd),
	...
	for (i = 1; i < IRQ_NUM; i++)
		size += snprintf_s(tmp + size, ISP_RDR_MAX_SIZE,
			   ...
			   (param ? (((bool)(param->irq[i])) ?
			   "enable":"disable"):"ispoffline"),
			   (((bool)(dev->irq[i])) ? "enable":"disable"), i);
