...
static atomic_t hisp_alloc_pages_num = ATOMIC_INIT(0);
#define ISP_NUM_ORDERS ARRAY_SIZE(orders)

static int hisp_rproc_stop(struct rproc *rproc);

int get_hisp_alloc_pages_num(void)
{
	return atomic_read(&hisp_alloc_pages_num);
}
				...
				unsigned int max_order, unsigned int *order)
{
	...
	for (i = 0; i < ISP_NUM_ORDERS; i++) {
...
#ifdef CONFIG_LIBLINUX
		trace_ldk_rvh_hisp_alloc_largest_pages(orders[i], &gfp_flags);
#endif
		...
		*order = orders[i];
		atomic_add(1 << orders[i], &hisp_alloc_pages_num);
...
#ifdef CONFIG_ISP_OHOS_ALLOC_DMA
struct isp_dma_mem_info {
	struct sg_table *table;
	struct dma_buf *buf;
	struct dma_buf_attachment *attachment;
};

struct sg_table *hisp_alloc_dynmem_sg_table(unsigned long length, void **sg_info)
{
	struct sg_table *sgt = NULL;
	struct device *dev = NULL;
	struct dma_buf *dmabuf = NULL;
	struct dma_buf_attachment *dmabuf_attach = NULL;
	struct isp_dma_mem_info *dma_info = NULL;

	pr_debug("ohos [%s] in len %u\n", __func__, length);

	if (sg_info == NULL) {
		pr_err("[%s] sg_info is NULL\n", __func__);
		return NULL;
	}
	...
	*(struct isp_dma_mem_info **)sg_info = NULL;
	dma_info = kmalloc(sizeof(struct isp_dma_mem_info), GFP_KERNEL);
	if (dma_info == NULL) {
		pr_err("[%s] alloc dma info struct fail\n", __func__);
		return NULL;
	}

	dmabuf = dma_heap_buffer_alloc_kernel("system_heap", length, 0);
	if (IS_ERR_OR_NULL(dmabuf)) {
		pr_err("[%s] failed: dma_heap_buffer_alloc_kernel\n", __func__);
		goto alloc_fail;
	}

	dmabuf_attach = dma_buf_attach(dmabuf, dev);
	...
	dma_info->table = sgt;
	dma_info->buf = dmabuf;
	dma_info->attachment = dmabuf_attach;
	*sg_info = dma_info;
...
void hisp_free_dynmem_sg_table(void **sg_info)
{
	struct isp_dma_mem_info *dma_info = NULL;
	...
	dma_info = *(struct isp_dma_mem_info **)sg_info;
	if (dma_info == NULL) {
		pr_err("%s: dma_info is NULL\n", __func__);
		return;
	}

	if ((dma_info->table == NULL) || (dma_info->buf == NULL) || (dma_info->attachment == NULL)) {
		pr_err("%s: sg_info error\n", __func__);
		return;
	}

	dma_buf_unmap_attachment(dma_info->attachment, dma_info->table, DMA_BIDIRECTIONAL);
	dma_buf_detach(dma_info->buf, dma_info->attachment);
	dma_heap_buffer_free_kernel(dma_info->buf);
	kfree(dma_info);
	*(struct isp_dma_mem_info **)sg_info = NULL;
...
static struct sg_table *hisp_get_sg_table_from_sg_info(void *sg_info)
{
	struct isp_dma_mem_info *dma_info = sg_info;

	return dma_info->table;
}
...
free_pages:
	list_for_each_entry_safe(page_info, tmp_page_info, &pages_list, node) {
		atomic_sub(1 << page_info->order, &hisp_alloc_pages_num);
...
void hisp_free_dynmem_sg_table(void **sg_info)
{
	...
	for_each_sg(table->sgl, sg, table->nents, i) {
		atomic_sub(1 << ((unsigned int)get_order(PAGE_ALIGN(sg->length))), &hisp_alloc_pages_num);
		__free_pages(sg_page(sg), (unsigned int)get_order(PAGE_ALIGN(sg->length)));
	}
	sg_free_table(table);
	kfree(table);
	*(struct sg_table **)sg_info = NULL;
...
static struct sg_table *hisp_get_sg_table_from_sg_info(void *sg_info)
{
	return (struct sg_table *)sg_info;
}
#endif
...
static void *hisp_alloc_dynmem_table(struct fw_rsc_carveout *rsc)
{
	...
	(void)hisp_alloc_dynmem_sg_table(rsc->len, &sg_info);
...
static hisp_prog_info_t g_prog_info = {0};

hisp_prog_info_t *hisp_prog_info_get(void)
{
	return &g_prog_info;
}

static inline void hisp_progblk_set(hisp_prog_blk_t *blk, uint8_t *mem, uint8_t *iova, size_t size)
{
	blk->mem = mem;
    blk->iova = iova;
    blk->size = size;
...
{
	struct hisp_rproc *hproc = rproc->priv;
	struct sg_table *table = hisp_get_sg_table_from_sg_info(mem->priv);
	...
	if (strncmp(mem->name, "ISP_MEM_TEXT", strlen("ISP_MEM_TEXT")) == 0) {
		...
		mem->len -= dyn_off;
		hproc->imgva = mem->va;
		hisp_progblk_set(&hisp_prog_info_get()->text, va, (void *)(uintptr_t)mem->da, mem->len + dyn_off);
	} else if (strncmp(mem->name, "ISP_MEM_DATA", strlen("ISP_MEM_DATA")) == 0) {
		mem->va = va + (unsigned long long)dyn_off;
		mem->len -= dyn_off;
		hisp_progblk_set(&hisp_prog_info_get()->data, va, (void *)(uintptr_t)mem->da, mem->len + dyn_off);
	} else {
		mem->va = va;
	}
...
static int hisp_ixc_alloc_dynmem(struct rproc *rproc,
		struct rproc_mem_entry *mem)
{
	struct hisp_rproc *hproc = rproc->priv;
	struct sg_table *table = hisp_get_sg_table_from_sg_info(mem->priv);
...
static void hisp_clean_rproc_list(struct hisp_rproc *hproc, void *va, u32 len)
{
	struct rproc_cache_entry *tmp = NULL, *entry = NULL;

	list_for_each_entry_safe(entry, tmp, &hproc->caches, node) {
		if (entry->va == va && entry->len == len) {
			list_del(&entry->node);
			kfree(entry);
			break;
		}
...
mem_free:
	hisp_clean_rproc_list(hproc, mem->va, mem->len);
	hisp_mem_unmap_kernel(mem->va);
	hisp_free_dynmem_sg_table(&mem->priv);
	hproc->imgva = NULL;

	return ret;
}

static int hisp_ixc_release_dynmem(struct rproc *rproc,
		struct rproc_mem_entry *mem)
{
	struct device *dev = &rproc->dev;
	struct hisp_rproc *hproc = rproc->priv;
	...
	if ((mem->flags & ISP_MAP_SUCESS_FLAG) != ISP_MAP_SUCESS_FLAG ) {
		pr_err("%s, mem not map\n", __func__);
		goto mem_free;
	}
		...
		void *fw_rsc, int offset)
{
	...
	rsc = (struct fw_rsc_carveout *)fw_rsc;
	sg_info = hisp_alloc_dynmem_table(rsc);
	if (sg_info == NULL) {
	...
	if (mem == NULL) {
		pr_err("%s, Can't allocate memory entry structure\n", __func__);
		hisp_free_dynmem_sg_table(&sg_info);
		return -ENOMEM;
	}
...
/**
 * hisp_handle_isp_npu_ipcmem() - handle the npu ipc memory
 * @rproc: the remote processor
 * @fw_rsc: the carveout resource descriptor
 * @offset: offset of version rsc in resource table
 */
static int hisp_handle_isp_npu_ipcmem(struct rproc *rproc,
		void *fw_rsc, int offset)
{
	struct rproc_mem_entry *mem = NULL;
	struct fw_rsc_carveout *rsc = NULL;
	struct device *dev = &rproc->dev;
	uint64_t isp_npu_ipc_pa = 0;
	uint32_t isp_npu_ipc_buffer_size = 0;
	int ret = 0;
	...
	ret = hisp_parse_ipcbuffer_resmem_desc(&isp_npu_ipc_pa, &isp_npu_ipc_buffer_size);
#else
	ret = -1;
#endif
	if ((ret != 0) || (isp_npu_ipc_buffer_size != rsc->len)) {
		pr_err("%s, get npu ipc buffer pa failed\n", __func__);
		return -EINVAL;
	}

	mem = rproc_mem_entry_init(dev, NULL,
			(dma_addr_t)isp_npu_ipc_pa, rsc->len, rsc->da,
			hisp_alloc_mem_common, hisp_release_mem_common, rsc->name);
	if (mem == NULL) {
		pr_err("%s, Can't allocate memory entry structure\n", __func__);
		return -ENOMEM;
...
/**
 * hisp_handle_ixc_mem() - handle the isp ixc memory
 * @rproc: the remote processor
 * @fw_rsc: the carveout resource descriptor
 * @offset: offset of version rsc in resource table
 */
static int hisp_handle_ixc_mem(struct rproc *rproc,
		void *fw_rsc, int offset)
{
	struct rproc_mem_entry *mem = NULL;
	struct fw_rsc_carveout *rsc = NULL;
	...
	mem = rproc_mem_entry_init(dev, NULL, 0, rsc->len, rsc->da,
			hisp_ixc_alloc_dynmem, hisp_ixc_release_dynmem, rsc->name);
	if (mem == NULL) {
		pr_err("%s, Can't allocate memory entry structure\n", __func__);
		hisp_free_dynmem_sg_table(&sg_info);
		return -ENOMEM;
...
/**
 * hisp_handle_isp_mudp_resmem() - handle the isp mudp resmem
 * @rproc: the remote processor
 * @fw_rsc: the carveout resource descriptor
 * @offset: offset of version rsc in resource table
 */
static int hisp_handle_isp_mudp_resmem(struct rproc *rproc,
		void *fw_rsc, int offset)
{
	struct rproc_mem_entry *mem = NULL;
	struct fw_rsc_carveout *rsc = NULL;
	...
	rsc = (struct fw_rsc_carveout *)fw_rsc;
#ifdef CONFIG_HISP_MUDP_RESMEM_ENABLE
	pa = ISP_MUDP_RESMEM;
#else
	pr_err("%s, get isp mudp resmem pa failed\n", __func__);
	return -EINVAL;
#endif

	mem = rproc_mem_entry_init(dev, NULL, (dma_addr_t)pa, rsc->len, rsc->da,
			hisp_alloc_mem_common, hisp_release_mem_common, rsc->name);
	if (mem == NULL) {
		pr_err("%s, Can't allocate memory entry structure\n", __func__);
		return -ENOMEM;
...
static hisp_handle_resource_t hisp_loading_handlers[RSC_QUERY_ID_MAX] = {
	...
	[RSC_ISP_NPU_IPC_QUERY_ID] = hisp_handle_isp_npu_ipcmem,
	[RSC_ISP_MUDP_QUERY_ID]    = hisp_handle_isp_mudp_resmem,
	[RSC_ISP_IXC_QUERY_ID]	   = hisp_handle_ixc_mem,
};
...
static void hisp_mbox_rx_work(struct rproc *rproc)
{
	...
	while (kfifo_len(&hisp_rx_mbox->rpmsg_rx_fifo)
		...
		if (len == 0) {
			pr_err("Failed : kfifo_out_locked.%u\n", len);
			return;
		}
...
static int hisp_rproc_vring_init(struct hisp_rproc *dev)
{
	...
	for (i = 0; i < HISP_VRING_MAX; i++) {
		...
		vring->va = dma_alloc_coherent(device,
			vring->size, &vring->pa, (gfp_t)GFP_KERNEL);
		if (!vring->va) {
...
static int hisp_rproc_vqueue_init(struct hisp_rproc *dev)
{
	...
	vqueue->va = dma_alloc_coherent(device,
		vqueue->size, &vqueue->pa, (gfp_t)GFP_KERNEL);
	if (!vqueue->va) {
...
static int hisp_ispdma_probe(struct platform_device *pdev)
{
	return hisp_ispdma_device_attach(pdev);
}

static int hisp_ispdma_remove(struct platform_device *pdev)
{
	return hisp_ispdma_device_attach(pdev);
}

static const struct of_device_id hisp_ispdma_of_match[] = {
	{ .compatible = "hisilicon,isp-dma" },
	{ },
};
MODULE_DEVICE_TABLE(of, hisp_ispdma_of_match);

static struct platform_driver hisp_ispdma_driver = {
	.driver = {
		.owner      = THIS_MODULE,
		.name       = "ispdma",
		.of_match_table = of_match_ptr(hisp_ispdma_of_match),
	},
	.probe  = hisp_ispdma_probe,
	.remove = hisp_ispdma_remove,
};

module_platform_driver(hisp_ispdma_driver);
#endif
