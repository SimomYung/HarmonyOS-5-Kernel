...
static int hispdbg_hal_intf_test(unsigned long arg)
{
	...
	if ((void __user *)(uintptr_t)arg == NULL)
		return -EFAULT;
	ret = (int)copy_from_user(&info, (void __user *)(uintptr_t)arg, (unsigned long)sizeof(interface_test_t));
	if (ret != 0) {
					...
					const unsigned int flag_len)
{
	...
	for (index = RPMSG_CAMERA_SEND_MSG; index < len; index++) {
		if (message_flag[index] && message_info[index] < RPMSG_RDR_INFO_MAX_NUM) {
...
static int hispdbg_pool_setup(map_hisp_cfg_data_t *cfg)
{
	...
	rpmsg_info("shared mem addr.0x%llx, size.0x%x, prot.0x%x, pool_num.%d\n",
		cfg->param.mem_info.cpu_addr, map_info.size, cfg->param.prot,
		cfg->param.pool_num);

	type.master = ISP_CPU;
	type.mem_type = ISP_PRIVATE_MEM;
	map_info.size = 0x01000000;
	...
	rpmsg_info("priv mem addr.0x%llx, size.0x%x, prot.0x%x, pool_num.%d\n",
		mem_info.cpu_addr, map_info.size, cfg->param.prot,
		cfg->param.pool_num);
	return 0;
}
		...
		unsigned long arg)
{
	...
	ret1 = copy_from_user(&cfg, (void __user *)(uintptr_t)arg, (unsigned long)sizeof(cfg));
	if (ret1 != 0) {
		...
		unsigned long arg)
{
	...
	ret = (int)copy_from_user(&info,
		(void __user *)(uintptr_t)arg, (unsigned long)sizeof(info));
	if (ret != 0) {
		...
		unsigned long arg)
{
	...
	ret = (int)copy_from_user(&buf, (void __user *)(uintptr_t)arg, (unsigned long)sizeof(struct memory_block_s));
	if (ret != 0) {
		rpmsg_err("copy_from_user.%d\n", ret);
		return -EFAULT;
	}
		...
		const char __user *ubuf, size_t len, loff_t *offp)
{
	...
	if (copy_from_user(msg_buf, ubuf, (unsigned long)use))
		return -EMSGSIZE;
