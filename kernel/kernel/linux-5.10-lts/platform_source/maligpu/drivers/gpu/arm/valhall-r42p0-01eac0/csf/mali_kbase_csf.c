...
#ifdef CONFIG_MALI_GPU_RECLAIM
	if (kctx->kctx_sw.is_background) {
		WARN_ON(true);
		pr_warn("[mali] kbase_csf_queue_kick but kctx is background! kctx = %p tgid = %d\n", kctx, kctx->tgid);
		kbase_switch_to_frontground(kctx, kctx->tgid);
	}
#endif
...
handle_fault_event(struct kbase_queue *const queue, const u32 cs_ack)
{
	...
	if (cs_fault_exception_type == CS_FAULT_EXCEPTION_TYPE_RESOURCE_EVICTION_TIMEOUT ) 
	{
		dev_err(kbdev->dev,"CS_FAULT triggers GPU reset.");
	...
	kbase_gpu_fault_dmd_report(kbdev, cs_fault_exception_type, CS_FAULT);
#if CONFIG_GPU_EWP_HEAL
	if ((kbdev->gpu_dev_data.error_num.cs_fault + 1) % CS_FAULT_DMD_TIMES == 0)
		gpu_dmd_msg_check(kbdev);
#endif
#endif
	...
	if (kbdev->gpu_dev_data.factory_mode && g_report_panic)
		BUG_ON(1);
...
#if defined (CONFIG_HUAWEI_DSM) || defined (CONFIG_HUAWEI_OHOS_DSM)
	kbase_gpu_fault_dmd_report(kbdev, cs_fatal_exception_type, CS_FATAL);
#if CONFIG_GPU_EWP_HEAL
	if ((kbdev->gpu_dev_data.error_num.cs_fatal + 1) % CS_FAULT_DMD_TIMES == 0)
			gpu_dmd_msg_check(kbdev);
#endif
#endif

	kbdev->gpu_dev_data.error_num.cs_fatal++;
	if (kbdev->gpu_dev_data.factory_mode && g_report_panic)
		BUG_ON(1);
...
#if defined (CONFIG_HUAWEI_DSM) || defined (CONFIG_HUAWEI_OHOS_DSM)
	kbase_gpu_fault_dmd_report(kbdev, 0x7A, CS_FAULT);
#endif
