...
static void fence_timeout_callback(struct timer_list *timer)
{
	...
	cmd->timeout_fence_waiting = false;
	if (info.status == 1) {
		queue_work(kcpu_queue->wq, &kcpu_queue->work);
		...
		cmd->timeout_fence_waiting = true;
		queue_work(kcpu_queue->wq, &kcpu_queue->work);
	} else {
		...
		struct kbase_kcpu_command_fence_info *fence_info)
{
	...
	if (kcpu_queue->fence_wait_processed) {
		...
		struct kbase_kcpu_command *cmd = &kcpu_queue->commands[kcpu_queue->start_offset];
		if (cmd->timeout_fence_waiting) {
			fence_status = 1;
			cmd->timeout_fence_waiting = false;
			dev_err(kctx->kbdev->dev, "[fence workaround] seqno = %d\n", fence->seqno);
		}
	} else {
			...
			struct kbase_ioctl_kcpu_queue_new *newq)
{
...
#ifdef CONFIG_LIBLINUX
	queue = kvzalloc(sizeof(*queue), GFP_KERNEL);
#else
	...
	if (!queue) {
		ret = -ENOMEM;
#ifdef CONFIG_LIBLINUX
		dev_err(kctx->kbdev->dev, "queue alloc failed\n");
#endif
		goto out;
	}
