...
#ifdef CONFIG_GPU_AI_FENCE_INFO
static int kbase_api_ai_fence_get(struct kbase_context *kctx,
		struct kbase_ioctl_ai_fence_info *ai_fence_info)
{	
	void __user *user_fence_info = u64_to_user_ptr(ai_fence_info->fence_info_addr);
	return perf_ctrl_get_gpu_fence(user_fence_info);
}

static int kbase_api_ai_buffer_size_get(struct kbase_context *kctx,
		struct kbase_ioctl_ai_buffer_info *ai_buffer_info)
{
	void __user *user_buffer_info = u64_to_user_ptr(ai_buffer_info->buffer_size_addr);
	return perf_ctrl_get_gpu_buffer_size(user_buffer_info);
}
#endif
...
#ifdef CONFIG_MALI_GPU_RECLAIM
static long kbase_ioctl_context_switch(struct kbase_context *kctx, struct kbase_ioctl_para_ctx_sw *para)
{
	...
	switch (cmd) {
	case KBASE_CTX_SW_TO_FRONT:
		dev_info(kctx->kbdev->dev, "[ctx_sw] ftont cmd:%d, tgid:%d\n", cmd, para->in.tgid);
		ret = kbase_switch_to_frontground(kctx, para->in.tgid);
		break;
	case KBASE_CTX_SW_TO_BACKGROUND:
		dev_info(kctx->kbdev->dev, "[ctx_sw] back cmd:%d, tgid:%d ratio:%u\n",
			cmd, para->in.tgid, para->in.ratio);
		ret = kbase_switch_to_background(kctx, para);
		break;
	case KBASE_CTX_SW_INIT_GPU_SIZE:
		dev_info(kctx->kbdev->dev, "[ctx_sw] init gpu size cmd:%d, tgid:%d ratio:%u size:%llu\n",
			cmd, para->in.tgid, para->in.ratio, para->in.size);
		size = (unsigned long)para->in.size;
		ret = mm_gpu_page_swap_space_init(size);
		if (ret != 0)
			dev_err(kctx->kbdev->dev, "[ctx_sw]swap_space_init failed,size:0x%lx\n", size);
		else
			dev_info(kctx->kbdev->dev, "[ctx_sw]swap_space_init success,size:0x%lx\n", size);
		break;
	default:
		dev_info(kctx->kbdev->dev, "invalid sw value:%u,tgid:%u\n", cmd, para->in.tgid);
		break;
	}
	return ret;
}

static long kbase_ioctl_get_ctx_pages(struct kbase_context *kctx, union kbase_ioctl_para_get_ctx_pages *para)
{
	int ret = 0;
	struct kbase_device *kbdev = NULL;
	struct kbase_context *ctx_tmp = NULL;
	...
	// get tgid all pages
	list_for_each_entry(ctx_tmp, &kbdev->kctx_list, kctx_list_link) {
		if (ctx_tmp->tgid == tgid || ctx_tmp->pid == tgid) {
			dev_info(kbdev->dev, "tgid is %lu ctx is %p\n", tgid, ctx_tmp);
			tmp_pages = kbase_switch_get_ctx_pages(ctx_tmp);
	...
	if (ctx_pages == 0) {
		dev_dbg(kbdev->dev, "ctx_pages is 0\n");
		ret = -1;
	}
...
static long kbase_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	kctx = kbase_file_get_kctx_if_setup_complete(kfile);
	if (unlikely(!kctx)) {
		pr_err("kbase_file_get_kctx_if_setup_complete get kctx failed! kfile.setup_state = %d\n",
	...
	case KBASE_IOCTL_SET_LIMITED_CORE_COUNT:
		...
		break;
#ifdef CONFIG_GPU_AI_FENCE_INFO
	case KBASE_IOCTL_AI_FENCE_GET:
		KBASE_HANDLE_IOCTL_IN(KBASE_IOCTL_AI_FENCE_GET,
					kbase_api_ai_fence_get,
					struct kbase_ioctl_ai_fence_info,
					kctx);
		break;
	case KBASE_IOCTL_AI_BUFFER_SIZE_GET:
		KBASE_HANDLE_IOCTL_IN(KBASE_IOCTL_AI_BUFFER_SIZE_GET,
					kbase_api_ai_buffer_size_get,
					struct kbase_ioctl_ai_buffer_info,
					kctx);
		break;
#endif
#ifdef CONFIG_MALI_GPU_RECLAIM
	case KBASE_IOCTL_CONTEXT_SWITCH:
		KBASE_HANDLE_IOCTL_INOUT(KBASE_IOCTL_CONTEXT_SWITCH,
					kbase_ioctl_context_switch,
					struct kbase_ioctl_para_ctx_sw,
					kctx);
	break;
	case KBASE_IOCTL_GET_CTX_PAGES:
		KBASE_HANDLE_IOCTL_INOUT(KBASE_IOCTL_GET_CTX_PAGES,
					kbase_ioctl_get_ctx_pages,
					union kbase_ioctl_para_get_ctx_pages,
					kctx);
	break;
#endif
	}
				  ...
				  struct device_attribute *attr, char *buf)
{
	static const struct gpu_product_id_name {
		...
		{ .id = GPU_ID2_PRODUCT_TGRX >> KBASE_GPU_ID_VERSION_PRODUCT_ID_SHIFT,
		  .name = "Mali-G510" },
		/* Originally G310, modified to G51 due to anonymization requirements */
		...
		const char *buf, size_t count)
{
	...
	kbdev->gpu_dev_data.error_num.cs_fatal = 0;
	kbdev->gpu_dev_data.error_num.mmu_flush_fail = 0;
