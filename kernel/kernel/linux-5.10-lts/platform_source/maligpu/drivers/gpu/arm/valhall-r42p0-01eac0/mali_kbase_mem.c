		...
		u64 start_pfn, size_t nr_pages, int zone)
{
	...
	new_reg->nr_pages = nr_pages;
#ifdef CONFIG_MALI_GPU_RECLAIM
	new_reg->is_free = false;
	new_reg->swap_out = false;
#endif
		...
		unsigned long uaddr, size_t size, u64 *offset)
{
	...
	map = kbasep_find_enclosing_cpu_mapping(kctx, NULL, uaddr, size, offset);
		...
		struct basep_syncset *sset, enum kbase_sync_type sync_fn)
{
	...
	map = kbasep_find_enclosing_cpu_mapping(kctx, reg, start, size, &offset);
	if (!map) {
		dev_warn(kctx->kbdev->dev, "Can't find CPU mapping 0x%016lX/%zu for VA 0x%016llX",
				start, size, sset->mem_handle.basep.handle);
		err = -EINVAL;
		goto out_unlock;
	}
...
#ifdef CONFIG_MALI_GPU_RECLAIM
	if (reg->swap_out) {
		dev_warn(kctx->kbdev->dev, "reg = %p is swap_out need to restore!\n", reg);
...
#ifdef CONFIG_MALI_GPU_RECLAIM
					kbase_mem_phy_alloc_alias_put(aliased[i].alloc);
#else
