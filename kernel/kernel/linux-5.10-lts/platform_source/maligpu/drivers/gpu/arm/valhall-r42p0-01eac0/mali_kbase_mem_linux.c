	...
	for (i = 0; i < nents; i++) {
		if (ai[i].handle.basep.handle < BASE_MEM_FIRST_FREE_ADDRESS) {
			...
			if (ai[i].offset + ai[i].length > alloc->nents)
				goto bad_handle; /* beyond end */
#ifdef CONFIG_MALI_GPU_RECLAIM
			reg->gpu_alloc->imported.alias.aliased[i].alloc = kbase_mem_phy_alloc_alias_get(alloc);
#else
...
static vm_fault_t kbase_cpu_vm_fault(struct vm_fault *vmf)
{
	...
	kbase_gpu_vm_lock(map->kctx);
#ifdef CONFIG_MALI_GPU_RECLAIM
	if (map->kctx->kctx_sw.is_background) {
		pr_info("[mali] kbase_cpu_vm_fault kctx is background! need to switch frontground\n");
		kbase_switch_to_frontground(map->kctx, map->kctx->tgid);
	}
#endif
	...
	if (map->alloc->reg && (map->alloc->reg->flags & KBASE_REG_DONT_NEED))
		goto exit;
#ifdef CONFIG_LIBLINUX
	ret = kbase_mem_fault(vma, map_start_pgoff, nents, pages);
#else
...
#ifdef CONFIG_MALI_GPU_RECLAIM
const struct vm_operations_struct *kbase_mem_get_vm_ops(void)
{
	return &kbase_vm_ops;
}
#endif
