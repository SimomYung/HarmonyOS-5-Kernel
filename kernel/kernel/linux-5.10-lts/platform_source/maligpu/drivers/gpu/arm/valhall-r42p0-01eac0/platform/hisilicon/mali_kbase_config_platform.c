...
static int kbase_set_hi_features_mask(struct kbase_device *kbdev)
{
	...
	case GPU_ID2_MAKE(10, 8, 6, 7, 0, 0, 4):
			...
			break;
	case GPU_ID2_MAKE(10, 12, 7, 4, 0, 0, 1):
			hi_features = kbase_hi_feature_tvax_r0p0;
			break;
		default:
			dev_err(kbdev->dev,
...
#if defined (CONFIG_MALI_VALE) || defined (CONFIG_MALI_VALE_OH)
...
int32_t gpu_errorno_to_str(int32_t error_no, char *str, int32_t buff_len) {
	int32_t i;
	int32_t ret;
	// Define local maps to save memory and reduce code modification conflicts.
	static struct error_no_map gpu_error_no_map[] = {
		{922002000, "DSM_GPU_JOB_FAIL_NO"},
		{922002001, "DSM_GPU_PAGE_FAULT_NO"},
		{922002004, "DSM_GPU_BIT_STUCK_NO"},
		{922002005, "DSM_GPU_HARD_RESET_NO"},
		{922002007, "DSM_GPU_MMU_FLUSH_FAILED"},
	};
	uint32_t gpu_error_no_map_len = sizeof(gpu_error_no_map) / sizeof(error_no_map);
 
	for (i = 0; i < gpu_error_no_map_len; i++) {
		if (gpu_error_no_map[i].error_no == error_no) {
			if (strlen(gpu_error_no_map[i].name) >= buff_len){
				return -1;
			}
			ret = strncpy_s(str, buff_len, gpu_error_no_map[i].name, buff_len - 1);
			if (ret != 0) {
...
struct dsm_client_ops gpu_dsm_ops = {
	.poll_state = NULL,
	.dump_func = NULL,
	.errorno_to_str = gpu_errorno_to_str,
};
