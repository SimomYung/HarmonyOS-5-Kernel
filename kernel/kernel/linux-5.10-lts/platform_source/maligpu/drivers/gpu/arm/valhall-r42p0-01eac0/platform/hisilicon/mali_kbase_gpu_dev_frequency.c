...
int gpu_kobject_set_name_vargs(struct kobject *kobj, const char *fmt, va_list vargs)
{
	const char *s;
...
int gpu_dev_set_name(struct device *dev, const char *fmt, ...) {
	va_list vargs;
	int err;

	va_start(vargs, fmt);
	err = gpu_kobject_set_name_vargs(&dev->kobj, fmt, vargs);
	va_end(vargs);
...
void kbase_gpu_devfreq_init(struct kbase_device *kbdev)
{
	...
	rename_ret = gpu_dev_set_name(dev, "gpufreq");
	if (rename_ret != 0)
		dev_err(dev, "gpu_dev_set_name fail %d\n", rename_ret);
