...
#define MSPC_IS_WRITE_ACCESS(type)    ((type) < MSPC_WRITE_MAX && (type) >= MSPC_FLAG_INFO_WRITE)
				    ...
				    struct mspc_access_info *info)
{
	...
	if (type == MSPC_FLAG_INFO_READ || type == MSPC_FLAG_INFO_WRITE) {
		...
		info->size = sizeof(struct mspc_encos_header);
				...
				type == MSPC_INSTALL_RESULT_WRITE) {
		info->offset = (int64_t)(MSPC_IMG_PARTITION_SIZE - SIZE_1K +
			      ...
			      struct mspc_access_info *access_info)
{
	...
	filp = crypto_core_sys_open(access_info->fullpath, access_info->flags,
				  MSPC_FILESYS_DEFAULT_MODE);
	if (IS_ERR(filp)) {
		pr_err("%s:open file failed! err:%ld\n", __func__, PTR_ERR(filp));
	...
	if (MSPC_IS_WRITE_ACCESS(access_info->access_type)) {
		cnt = crypto_core_sys_write(filp, buf, access_info->size, &access_info->offset);
		ret = crypto_core_sys_fsync(filp, CRYPTO_CORE_SYNC_DATA_METADATA);
		if (ret < 0) {
		...
		cnt = crypto_core_sys_read(filp, buf, access_info->size, &access_info->offset);
	}
...
static int32_t mspc_is_subfile_valid(struct mspc_img_header *header)
{
	...
	for (i = 0; i < header->file_cnt; i++) {
		...
		if (size == 0 || size > MSPC_MAX_IMG_SIZE) {
			pr_err("%s:image:%u size:%u error!\n",
			       __func__, i, size);
			return MSPC_SUB_FILE_SIZE_ERROR;
		}

		if (offset < total_size) {
			pr_err("%s:image:%u offset:%u error!\n",
			       __func__, i, offset);
			return MSPC_SUB_FILE_SIZE_ERROR;
		}
		total_size += size;
		...
		if (!strncmp(header->file[i].name, MSPC_IMG_DCS_NAME,
			...
			if (cos_id >= MSPC_MAX_COS_IMAGE_NUM ||
			    ...
			    MSPC_MAX_DCS_FILES) {
				pr_err("%s:dcs for image:%u cos is:%u error!\n",
				       __func__, i, cos_id);
		...
		if (!strncmp(header->file[i].name,
			...
			if (cos_id >= MSPC_MAX_COS_IMAGE_NUM) {
				pr_err("%s:cos image:%u cos id:%u is error!\n",
				       __func__, i, cos_id);
...
static int32_t mspc_encos_erase(struct file* filp, uint32_t id, struct mspc_encos_header *header)
{
	int64_t cnt;
	loff_t offset;
	...
	offset = 0;
	cnt = crypto_core_sys_write(filp, (const char *)header, sizeof(struct mspc_encos_header), &offset);
	if ((uint64_t)cnt < sizeof(struct mspc_encos_header)) {
		pr_err("%s: write header failed, ret: %lld\n", __func__, cnt);
		return MSPC_ENCOS_WRITE_FILE_ERROR;
	}

	offset = (loff_t)(header->file[id].offset);
	...
	cnt = crypto_core_sys_write(filp, file_buff, MSPC_ENCOS_FILE_LEN, &offset);
	...
	if (crypto_core_sys_fsync(filp, CRYPTO_CORE_SYNC_DATA_METADATA) < 0) {
...
int32_t mspc_remove_flash_cos(void)
{
	...
	filp = crypto_core_sys_open(filename, O_WRONLY, S_IWUSR);
	if (IS_ERR(filp)) {
		pr_err("%s: open %s failed:%ld!\n", __func__, filename, PTR_ERR(filp));
		set_fs(old_fs);
		return MSPC_ENCOS_OPEN_FILE_ERROR;
	}

	/* 3. Do real erase operate */
	ret = mspc_encos_erase(filp, id, header);
	crypto_core_sys_close(filp);
	set_fs(old_fs);
	return ret;
}
...
int32_t mspc_check_hisee_fs_empty(void)
{
	...
	filp = crypto_core_sys_open(fullpath, O_RDONLY, S_IRUSR);
	if (IS_ERR(filp)) {
	...
	for (i = 0; i < MSPC_FS_TOTAL_SIZE / MSPC_FS_ONCE_CHK_SIZE; i++) {
		cnt = crypto_core_sys_read(filp, file_buff, MSPC_FS_ONCE_CHK_SIZE, &offset);
