...
#define CRYPTO_CORE_TZSP_TEST  "mspc_tzsp_test"
static int32_t mspc_tzsp_test(const int8_t *buf, int32_t len);
...
static const struct mspc_driver_func g_mspc_power_func_list[] = {
	...
	{ CRYPTO_CORE_TZSP_TEST, mspc_tzsp_test },
#ifdef CRYPTO_CORE_SUPPORT_UPGRADE
			     ...
			     const char *buf, size_t count)
{
	...
	if (unlikely(i == ARRAY_SIZE(g_mspc_power_func_list) ||
		...
		pr_err("%s: cannot find command, i:%u!\n", __func__, i);
		ret = MSPC_INVALID_PARAM_ERROR;
		goto exit;
	}
...
static int32_t mspc_tzsp_test(const int8_t *buf, int32_t len)
{
	...
	uint64_t cmd_to_atf = (uint64_t)CRYPTO_CORE_SMC_DEV_TEST;
	uint64_t func_id = (uint64_t)MSPC_FN_MAIN_SERVICE_CMD;

	if (len <= sizeof(CRYPTO_CORE_TZSP_TEST) + 1) {
		return MSPC_INVALID_PARAM_ERROR;
	}

	msg[1] = (uint32_t)(buf[1] - '0');
	ret = mspc_send_smc_no_ack(func_id, cmd_to_atf, (const char *)msg, sizeof(msg));
	if (ret != MSPC_OK)
