...
static noinline int32_t mspc_send_tzsp_smc_with_shmem(uint64_t _function_id, uint64_t _arg0,
						char *buf, uint32_t size, uint32_t mem_type)
{
	uint64_t recv_size;
	char *addr = buf;
	...
	while (size > 0) {
		...
		if (ffa_rpmbmspc_send_msg_with_shmem(&args, addr, mem_type) != 0) {
			pr_err("%s: ffa send 0x%llx fail1, size: 0x%x\n", __func__, _arg0, size);
			return MSPC_SMC_CMD_PROCESS_ERROR;
		}
		addr += recv_size;
		size -= recv_size;
	}
...
int32_t mspc_send_smc_with_shmem_no_ack(uint64_t fid, uint64_t smc_cmd,
			    char *buf, uint32_t buf_len, uint32_t mem_type)
{
#ifdef CONFIG_CRYPTO_CORE_FFA_SUPPORT
	return mspc_send_tzsp_smc_with_shmem(fid, smc_cmd, buf, buf_len, mem_type);
#else
	return MSPC_OK;
#endif
}
