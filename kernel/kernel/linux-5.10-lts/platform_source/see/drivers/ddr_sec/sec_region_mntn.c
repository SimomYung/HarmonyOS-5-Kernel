...
static int sec_region_mntn_process(void *arg)
{
	...
	struct qice_subreason_info *sub_info = NULL;
	u64 *gic_flag_addr = NULL;
	u64 *intr_info_addr = NULL;
	u64 *err_addr = NULL;
	u64 intr_id, addr;
	u64 sub_reason = MODID_AP_S_DDRC_SEC;
	...
	iounmap(err_addr);
	/* record info in dmd */
	(void)bbox_diaginfo_record(DMD_DDR_SEC_CNT, NULL,
	                           "[ddr_sec]: err_reason: %d, mid: %d, secure_type: %d, access_type: %d, addr: 0x%lx",
	                           intr_info->err_reason, intr_info->mid, intr_info->ns, intr_info->access_type, addr);

	intr_info_addr = ioremap(QICE_SUBREASON_INFO_ADDR, QICE_SUBREASON_INFO_SIZE);
	if(intr_info_addr == NULL) {
		pr_err("[ddr_sec]: sub_info ioremap fail \n");
		goto err;
	}
	sub_info = (struct qice_subreason_info *)(uintptr_t)intr_info_addr;
	if(sub_info->init_flag != DDR_SEC_INIT_END) {
		pr_err("[ddr_sec]: sub_info flag is not set \n");
		iounmap(intr_info_addr);
		goto err;
	}
	pr_err("[ddr_sec]: sub_reason is 0x%lx \n", sub_info->sub_reason);
	sub_reason = sub_info->sub_reason + MODID_DDR_SEC_BEGIN;
	iounmap(intr_info_addr);
err:
	pr_err("[ddr_sec]: sub_reason is 0x%lx \n", sub_reason);
