...
#include <platform_include/see/sec_auth_lib.h>
...
#define SECINFO_QUEUE_NAME                       "secinfo_queue"
#define EMPOWER_CERT_PARTITION_NAME 			 "xloader"
#define EMPOWER_CERT_IMAGE_NAME 				 "empower"

struct secinfo_work_ctx {
	s32 result;
	u64 *paras;
	size_t size;
...
typedef void (*secinfo_work_fn)(struct work_struct *work);

static s32 dev_secinfo_work_ctx_init(struct secinfo_work_ctx *ctx, u64 *paras, size_t size, secinfo_work_fn fn)
{
	struct workqueue_struct *queue = NULL;

	queue = create_singlethread_workqueue(SECINFO_QUEUE_NAME);
...
static void secinfo_work_ctx_deinit(struct secinfo_work_ctx *ctx)
{
...
{
	u32 ret;
	struct secinfo_work_ctx *ctx = NULL;

	ctx = container_of(work, struct secinfo_work_ctx, work);
...
static void huk2_enhance_update_cnt_work(struct work_struct *work)
{
	u32 ret;
	struct secinfo_work_ctx *ctx = NULL;
	...
	ret = dev_secinfo_huk2_enhance_update_cnt();
	dev_secinfo_err("exit, ret=0x%x!\n", ret);
	ctx->result = (s32)ret;
	complete(&ctx->completion);
}
...
static s32 uapp_set_enable_state_by_queue(u32 state)
{
	...
	ret = dev_secinfo_work_ctx_init(&ctx, paras, size, uapp_set_enable_work);
	if (ret != 0) {
	...
	secinfo_work_ctx_deinit(&ctx);
	dev_secinfo_err("exit, ret=0x%x\n", ret);

	return ret;
}

static void dev_secinfo_update_all_nvcnt_work(struct work_struct *work)
{
	u32 ret;
	struct secinfo_work_ctx *ctx = NULL;

	ctx = container_of(work, struct secinfo_work_ctx, work);
	ret = dev_secinfo_update_all_nvcnt();
	dev_secinfo_err("exit, ret=0x%x!\n", ret);
	ctx->result = (s32)ret;
	complete(&ctx->completion);
}

static s32 dev_secinfo_update_all_nvcnt_by_queue(void)
{
	s32 ret;
	struct secinfo_work_ctx ctx = {};
	u64 paras[] = {};
	size_t size = 0;

	dev_secinfo_err("enter %s\n", __func__);

	ret = dev_secinfo_work_ctx_init(&ctx, paras, size, dev_secinfo_update_all_nvcnt_work);
	if (ret != 0) {
		dev_secinfo_err("error, init uapp work %d!\n", ret);
		return ret;
	}
...
static s32 huk2_enhance_update_cnt_by_queue(void)
{
	s32 ret;
	struct secinfo_work_ctx ctx = {0};
	...
	ret = dev_secinfo_work_ctx_init(&ctx, NULL, 0, huk2_enhance_update_cnt_work);
	if (ret != 0) {
		dev_secinfo_err("error, init huk2_enhance work %d!\n", ret);
		return ret;
	}
...
s32 security_info_update_all_nvcnt(u32 cmd, uintptr_t arg)
{
	s32 ret;

	(void)arg;

	if (cmd != SECURUTY_INFO_UPDATE_ALL_NVCNT) {
		dev_secinfo_err("error, cmd=0x%08x\n", cmd);
		return -EFAULT;
	}

	ret = dev_secinfo_update_all_nvcnt_by_queue();
	if (ret != 0) {
		dev_secinfo_err("error, update_all_nvcnt_by_queue ret=0x%08X\n", ret);
		return -EFAULT;
	}
...
s32 security_info_get_uapp_bindfile_pos(u32 cmd, uintptr_t arg)
{
	u32 ret;
	void __user *argp = (void __user *)arg;
	struct uapp_bindfile_pos bindfile = {};
	uint32_t size_in_cmd = _IOC_SIZE(cmd);
...
s32 security_info_get_empower_cert_pos(u32 cmd, uintptr_t arg)
{
	u32 ret;
	void __user *argp = (void __user *)arg;
	struct dev_file_pos empower_cert = { 0 };
	uint32_t size_in_cmd = _IOC_SIZE(cmd);
	...
	if (cmd != SECURUTY_INFO_GET_EMPOWER_CERT_POS) {
		dev_secinfo_err("error, cmd=0x%08x\n", cmd);
		return -EFAULT;
	}
	...
	ret = dev_secinfo_empower_cert_pos(&empower_cert);
	if (ret != UAPP_OK) {
...
s32 security_info_get_rotpk_hash(u32 cmd, uintptr_t arg)
{
	...
	ret = dev_secinfo_rotpk_hash(&hash);
	if (ret != 0) {
...
s32 security_info_update_huk2_enhance_cnt(u32 cmd, uintptr_t arg)
{
	s32 ret;
	(void)arg;
	dev_secinfo_err("huk2_enhance_cnt info, cmd=0x%08x\n", cmd);
	if (cmd != SECURUTY_INFO_UPDATE_HUK2_ENHANCE_CNT) {
		dev_secinfo_err("error, cmd=0x%08x\n", cmd);
		return -EFAULT;
	}

	ret = huk2_enhance_update_cnt_by_queue();
	if (ret != 0) {
		dev_secinfo_err("error, update huk2 enhance cnt ret=0x%08X\n", ret);
		return -EFAULT;
	}
...
static void verify_empower_cert_work(struct work_struct *work)
{
	u32 ret;
	struct secinfo_work_ctx *ctx = NULL;
	...
	struct auth_node node = {0};
	struct authed_data out = {0};
	uint8_t dummy_data[32] = {0};
	struct image_id_t id = {
		.partition_name = EMPOWER_CERT_PARTITION_NAME,
		.image_name = EMPOWER_CERT_IMAGE_NAME,
	};

	node.cert_addr = (void *)ctx->paras;
	node.cert_size = (u32)ctx->size;
	node.data_addr = (void *)dummy_data;
	node.data_size = (u32)sizeof(dummy_data);
	node.id = id;

	ret = sec_auth_cert(&node, &out);
	if (ret != 0) {
		dev_secinfo_err("%s: verify empower cert failed", __func__);
	} else {
		dev_secinfo_info("%s: verify empower cert succeed", __func__);
	}
...
static s32 verify_empower_cert_by_queue(struct empower_cert *empower)
{
	s32 ret;
	struct secinfo_work_ctx ctx = {0};
	ret = dev_secinfo_work_ctx_init(&ctx, (u64)empower->cert, (size_t)empower->cert_bytes, verify_empower_cert_work);
	if (ret != 0) {
		dev_secinfo_err("error, init verify empower cert work %d!\n", ret);
		return ret;
	}
...
s32 security_info_verify_empower_cert(u32 cmd, uintptr_t arg)
{
	s32 ret;
	struct empower_cert cert_info;
	void __user *argp = (void __user *)arg;

	dev_secinfo_err("%s: enter func\n", __func__);
	...
	if (cmd != SECURUTY_INFO_VERIFY_EMPOWER_CERT) {
		dev_secinfo_err("cmd 0x%08x error\n", cmd);
		return -EFAULT;
	}

	ret = copy_from_user(&cert_info, (struct empower_cert *)argp, sizeof(struct empower_cert));
	if (ret != 0) {
		dev_secinfo_err("copy cert fail from %d\n", ret);
		return -EFAULT;
	}

	ret = verify_empower_cert_by_queue(&cert_info);
	if (ret != UAPP_OK) {
		dev_secinfo_err("error, verify empower cert ret=0x%08X\n", ret);
		return -EFAULT;
	}
