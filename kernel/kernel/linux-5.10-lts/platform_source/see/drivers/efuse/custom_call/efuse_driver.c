...
#define EFUSE_FN_SET_OEMINFO              0xCA00003C
/*
...
static s32 efuse_update_nvcnt(u8 *buf, u32 size)
{
	(void)buf;
	(void)size;
...
s32 set_efuse_modem_slt_flag(u8 *buf, u32 size, u32 timeout)
{
	s32 ret;
...
#ifdef CONFIG_EFUSE_SUPPORT_FFA
	ret = (s32)efuse_custom_write(buf, size, EFUSE_TZSP_KERNEL_WR_MDM_SLTFLG);
#else
	pr_err("%s: func not support.\n", __func__);
	ret = -1;
#endif
...
s32 is_need_soc_heavy_rt(void)
{
#ifdef CONFIG_EFUSE_SUPPORT_FFA
	u32 ret;
	struct efuse_call_args args = { 0 };

	args.arg0 = EFUSE_TZSP_KERNEL_RD_HEAVY_RT;
	ret = efuse_ffa(&args);
	if (ret != EFUSE_OK) {
		pr_err("[%s]: error, ffa ret=0x%08x, id=0x%x\n",\
			  __func__, ret, EFUSE_TZSP_KERNEL_RD_HEAVY_RT);
		return 0;
	}

	return (s32)args.arg1;
#else
	pr_err("%s: func not support.\n", __func__);
	return 0;
#endif
...
#define EFUSE_APP_READ_ACPU_BOOT \
	_IOW(EFUSE_APP_IOCTL_MAGIC, 0xC, struct efuse_buf)
...
#define EFUSE_APP_READ_OEMINFO \
	_IOW(EFUSE_APP_IOCTL_MAGIC, 0x11, struct efuse_buf)

#define EFUSE_APP_WRITE_OEMINFO \
	_IOR(EFUSE_APP_IOCTL_MAGIC, 0x11, struct efuse_buf)
...
static s32 ioctl_read_acpu_boot_enable(uint32_t cmd, void __user *argp)
{
	uint32_t ret;
	struct efuse_buf local = { 0 };
	uint32_t buffer;
	...
	desc.buf = &buffer;
	desc.buf_size = (uint32_t)(sizeof(buffer) / sizeof(uint32_t));
	desc.start_bit = 0;
	desc.bit_cnt = 1;
	desc.item_vid = EFUSE_KERNEL_ACPU_BOOT;
	...
	if ((buffer & 0x1) == 1) {
		local.value[0] = 1;
		pr_err("%s acpu boot eanbled\n", __func__);
	} else
		pr_err("%s acpu boot not eanbled\n", __func__);
...
static s32 ioctl_read_oeminfo(uint32_t cmd, void __user *argp)
{
	return __ioctl_read_efuse(cmd, argp, EFUSE_KERNEL_OEMINFO_ENABLE);
}

static s32 ioctl_write_oeminfo(uint32_t cmd, void __user *argp)
{
	s32 ret;
 	struct efuse_buf local = { 0 };
...
#ifdef CONFIG_EFUSE_SUPPORT_FFA
	ret = (s32)efuse_custom_write(&local.value[0], local.bytes, EFUSE_TZSP_KERNEL_WR_OEM_INFO);
#else
	ret = smc_write_efuse(EFUSE_FN_SET_OEMINFO, &local.value[0], local.bytes, EFUSE_TIMEOUT_SECOND);
#endif
	if (ret != OK) {
		pr_err("%s: write oeminfo enable err, ret=%d\n", __func__, ret);
		return EFUSE_IOCTL_KERNEL_PROC_ERR;
	}
...
#endif
	{EFUSE_APP_READ_ACPU_BOOT, ioctl_read_acpu_boot_enable},
	{EFUSE_APP_READ_OEMINFO, ioctl_read_oeminfo},
	{EFUSE_APP_WRITE_OEMINFO, ioctl_write_oeminfo},
};
