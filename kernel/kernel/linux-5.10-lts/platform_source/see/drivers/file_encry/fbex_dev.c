...
static const struct cmd_table g_cmd_table[] = {
...
#ifdef CONFIG_PRODUCT_ARMPC
	{ HISI_FBEX_REWRAP, SEC_FILE_ENCRY_CMD_ID_REWRAP },
	{ HISI_FBEX_SET_MASTERKEY, SEC_FILE_ENCRY_CMD_ID_SET_MASTERKEY },
	{ HISI_FBEX_DEL_MASTERKEY, SEC_FILE_ENCRY_CMD_ID_DEL_MASTERKEY },
#endif
};
...
static u32 fbex_ioccmd_to_id(u32 cmd)
{
	...
	pr_err("%s, error cmd=0x%08x.\n", __func__, cmd);
	return SEC_FILE_ENCRY_CMD_ID_MAX_SUPPORT;
}

#ifdef CONFIG_PRODUCT_ARMPC
void hisi_fbex_return_data(const u8 *key, u32 key_maxlen, u32 copy_len)
{
	u32 copy_len_tmp = min(key_maxlen, copy_len);
	/* SEC_FILE_ENCRY_CMD_ID_SET_MASTERKEY: hisi_fbex_copy_out desc */
	g_fbex_data.req.len = 0;
	(void)memset_s(g_fbex_data.req.key, sizeof(g_fbex_data.req.key),
			0, sizeof(g_fbex_data.req.key));

	if (key != NULL &&
		memcpy_s(g_fbex_data.req.key, KEY_LEN, key, copy_len_tmp) == EOK)
		g_fbex_data.req.len = copy_len_tmp;
}

#endif
static u32 hisi_fbex_copy_out(u32 cmd, fbex_req *from, void __user *to)
{
	u32 ret;

#ifdef CONFIG_PRODUCT_ARMPC
	if (cmd != SEC_FILE_ENCRY_CMD_ID_SET_MASTERKEY)
		return 0;
#else
	...
	ret = (u32)copy_to_user(to, from, sizeof(struct fbex_req_v0));
	if (ret) {
...
static u32 hisi_fbex_cmd_dispatch(u32 cmd, void __user *argp)
{
	...
	pr_info("%s, cmd 0x%x, user 0x%x, file 0x%x, len 0x%x, flag 0x%x\n",
		...
		g_fbex_data.req.file, g_fbex_data.req.len, g_fbex_data.req.flag);
#ifdef CONFIG_PRODUCT_ARMPC
	ret = file_encry_check_caller_uid(g_fbex_data.cmd, g_fbex_data.req.user);
	if (ret != 0) {
		pr_err("%s, uid error, cmd %u, ret 0x%x\n", __func__, g_fbex_data.cmd, ret);
		return ret;
	}
#endif
	...
	if (wait_for_completion_timeout(&g_fbex_ta_comp,
		...
		file_encry_record_error(g_fbex_data.cmd, g_fbex_data.req.user,
					g_fbex_data.req.file, FBE3_ERROR_CKEY_TIMEER_OUT);
...
#ifndef CONFIG_PRODUCT_ARMPC    /* if NOT def, normal progress */
	...
	if (id == SEC_FILE_ENCRY_CMD_ID_VOLD_ADD_IV) {
		...
		if (index >= FBEX_MAX_UFS_SLOT) {
			...
			file_encry_record_error(g_fbex_data.cmd, g_fbex_data.req.user,
				g_fbex_data.req.file, FBE3_ERROR_INVALID_IV);
			return FBE3_ERROR_INVALID_IV;
		}
	}
#else
	ret = file_encry_masterkey_to_keyring(id, g_fbex_data.req.flag,
			g_fbex_data.req.key, g_fbex_data.req.len);
	if (ret != 0) {
		pr_err("%s, uid error, cmd %u, ret 0x%x\n", __func__, id, ret);
		return ret;
	}
...
static int __init hisi_fbex_module_init(void)
{
	...
	ret = fbex_init_uece_dev();
	if (ret != 0) {
		pr_err("failed to register fbex uece dev\n");
		goto out;
	}

	ret = fbex_init_uece_enhance_dev();
	if (ret != 0) {
		pr_err("failed to register fbex uece enhance dev\n");
		goto out;
	}
...
{
	if (g_fbex_data.req.authtoken_len > AUTHTOKEN_MAXLEN ||
		g_fbex_data.req.authtoken_len == 0) {
		pr_err("%s length: %u\n", __func__, g_fbex_data.req.authtoken_len);
		return FBE3_ERROR_AUTHLEN;
	}
	*authtoken = (void *)g_fbex_data.req.authtoken;
	*authtoken_len = g_fbex_data.req.authtoken_len;
	return 0;
}
#endif
