...
#ifdef CONFIG_FBE_DIRECT_RESTOREKEY
static bool g_restorekey_session_create = true;
static TEEC_Context g_restorekey_context;
static TEEC_Session g_restorekey_session;
#endif
static u32 g_origin;
...
static u32 fbe_ca_create_session(void)
{
	TEEC_Result result;
...
#ifdef CONFIG_FBE_DIRECT_RESTOREKEY
	if (g_restorekey_session_create) {
		result = TEEK_InitializeContext(NULL, &g_restorekey_context);
		if (result != TEEC_SUCCESS) {
			pr_err("%s, TEEC init failed.\n", __func__);
			return result;
		}

		result = TEEK_OpenSession(&g_restorekey_context, &g_restorekey_session, &uuid_id,
					TEEC_LOGIN_IDENTIFY, NULL, &operation, &g_origin);
		if (result != TEEC_SUCCESS) {
			pr_err("%s: open session failed, 0x%x\n", __func__, result);
			TEEK_FinalizeContext(&g_restorekey_context);
			return result;
		}
		g_restorekey_session_create = false;
	}
#endif
	return TEEC_SUCCESS;
}
#ifdef CONFIG_FBE_DIRECT_RESTOREKEY
static u32 fbe_ca_restorekey_invoke_command(u32 cmd, TEEC_Operation *op)
{
	TEEC_Result result;

	if (g_restorekey_session_create) {
		pr_err("%s: never installed key.\n", __func__);
		return 0;
	}

	if (mutex_is_locked(&g_fbex_ta_mutex)) {
		pr_err("%s: bypass.\n", __func__);
	}

	result = TEEK_InvokeCommand(&g_restorekey_session, cmd, op, &g_origin);
	if (result != TEEC_SUCCESS) {
...
#endif
u32 fbe_ca_invoke_command(u32 cmd, TEEC_Operation *op)
{
	TEEC_Result result;
#ifdef CONFIG_FBE_DIRECT_RESTOREKEY
	if (cmd == SEC_FILE_ENCRY_CMD_ID_KEY_RESTORE) {
		return fbe_ca_restorekey_invoke_command(cmd, op);
	}
#endif
	mutex_lock(&g_fbex_ta_mutex);
	result = fbe_ca_create_session();
	if (result != TEEC_SUCCESS)
		goto exit;
...
void file_encry_record_error(u32 cmd, u32 user, u32 file, u32 error)
{
	...
	ret = bbox_diaginfo_record(FBE_DIAG_FAIL_ID, NULL, "FBE: cmd 0x%x fail, "
				   ...
				   cmd, user, file, error);
	pr_err("%s, record finish ret %d. data=(%x,%x,%x,%x)\n", __func__, ret,
					cmd, user, file, error);
}
...
void fbex_try_preloading(u32 user, u32 file)
{
	int status;
	u32 index;
...
u32 file_encry_add_iv(u32 user, u32 file, u8 *iv, u32 iv_len)
{
	...
	if (fbex_fill_authtoken(&operation.params[2].tmpref.buffer,
			&operation.params[2].tmpref.size) == 0) {
		operation.paramTypes = TEEC_PARAM_TYPES(TEEC_MEMREF_TEMP_INOUT,
				TEEC_VALUE_INPUT, TEEC_MEMREF_TEMP_INPUT, TEEC_NONE);
	}
#endif
...
#endif /* ifndef CONFIG_PRODUCT_ARMPC */
...
u32 fbex_restore_key(void)
{
	...
	operation.paramTypes = TEEC_PARAM_TYPES(TEEC_VALUE_OUTPUT, TEEC_NONE,
						TEEC_NONE, TEEC_NONE);
	operation.started = 1;
	ret = fbe_ca_invoke_command(SEC_FILE_ENCRY_CMD_ID_KEY_RESTORE,
				&operation);
	pr_err("%s, finish. ret=%d, cnt=%d.\n", __func__, ret, operation.params[0].value.a);
	return ret;
}
...
u32 fbex_gen_sk(uint32_t type)
{
	...
	pr_err("%s, in\n", __func__);
	if (is_ulsr())
		return generate_session_key(type);
...
u32 get_metadata_sece(struct fbex_sece_param *param)
{
	...
	if (g_sece_ops.ret)
		file_encry_record_error(param->cmd, param->idx, METADATA, g_sece_ops.ret);
