...
static void fbex_sync_ufs_lock(void)
{
	...
	ret = liblinux_vfs_ioctl(fd, FBEX_UDK_CMD_ENCRYPT_SLOT, g_ufs_lock, sizeof(g_ufs_lock));
	if (ret)
		pr_err("%s, sync ufs lock failed, ret: %d\n", __func__, ret);
	(void)liblinux_vfs_close(fd);
#endif
}
...
{
	return (u32)liblinux_vfs_unlock_in(user, file, iv, iv_len);
}
#endif
			   ...
			   u32 iv_len _unused)
{
	...
	if (ret != 0)
		pr_err("%s, lock sece error, ret %x\n", __func__, ret);
