...
static u32 fbex_fill_authtoken_uece(void **authtoken, u32 *authtoken_len)
{
	if (g_uece_req_info.req.authtoken_len > AUTHTOKEN_MAXLEN ||
		g_uece_req_info.req.authtoken_len == 0) {
		pr_err("%s length: %d\n", __func__, g_uece_req_info.req.authtoken_len);
		return FBE3_ERROR_AUTHLEN;
	}
	*authtoken = (void *)g_uece_req_info.req.authtoken;
	*authtoken_len = g_uece_req_info.req.authtoken_len;
	return 0;
}
#endif
...
static u32 file_encry_read_uece(u32 user_id_double, u32 user_id_single, u32 status, u8 *buf, u32 length)
{
	...
	operation.paramTypes = TEEC_PARAM_TYPES(TEEC_MEMREF_TEMP_INOUT,
			TEEC_VALUE_INPUT, TEEC_VALUE_INOUT, TEEC_NONE);
	if (fbex_fill_authtoken_uece(&operation.params[3].tmpref.buffer,
			&operation.params[3].tmpref.size) == 0) {
		operation.paramTypes = TEEC_PARAM_TYPES(TEEC_MEMREF_TEMP_INOUT,
				TEEC_VALUE_INPUT, TEEC_VALUE_INOUT, TEEC_MEMREF_TEMP_INPUT);
	}
#else
	operation.paramTypes = TEEC_PARAM_TYPES(TEEC_MEMREF_TEMP_INOUT,
						TEEC_VALUE_INPUT,
...
#ifdef CONFIG_FILE_ENCRY_ACCESS_CONTROL
		g_uece_req_info.req.length = operation.params[2].value.a;
#else
...
#ifdef CONFIG_FBE_UECE_BACKUP
static u32 file_encry_update_key_context(u32 user_id_double _unused,
										 u32 user_id_single, u32 status _unused,
										 u8 *buf _unused, u32 length _unused)
{
	TEEC_Operation operation = { 0 };
	operation.paramTypes = TEEC_PARAM_TYPES(TEEC_VALUE_INPUT, TEEC_NONE,
											TEEC_NONE, TEEC_NONE);
	operation.params[0].value.a = user_id_single;
	operation.started = 1;
	return fbe_ca_invoke_command(SEC_FILE_ENCRY_CMD_ID_UPDATE_KEY_CONTEXT, &operation);
}
#endif
...
#ifdef CONFIG_FBE_UECE_BACKUP	
	{ HISI_FBEX_UPDATE_KEY_CONTEXT, file_encry_update_key_context },
#endif
#ifdef CONFIG_FBE_DE_DOUBLE_TO_SINGLE
	...
	unsigned long arg)
{
    ...
    if (g_uece_req_info.ret) {
		...
		ret = (uint32_t)copy_to_user(argp, &g_uece_req_info.req, sizeof(struct fbex_uece_req_v0));
		if (ret) {
			pr_err("fbex uece, copy to user failed\n");
			g_uece_req_info.ret = FBE3_ERROR_COPY_FAIL;
		}
		(void)memset_s(&g_uece_req_info.req, sizeof(fbex_uece_req), 0, sizeof(fbex_uece_req));
	}
exit:
