...
#ifdef CONFIG_SECBOOT_BUFFER_FORM_PAGE
#define SECBOOT_BUFFER_ORDER  0x8
...
#ifdef CONFIG_SECBOOT_BUFFER_FORM_PAGE
static s32 bsp_malloc_seboot_buffer(void)
{
	struct page *sec_page = NULL;

	/* 1 page is 4kbytes, 256 pages is 1Mbytes */
	sec_page = alloc_pages(GFP_KERNEL, SECBOOT_BUFFER_ORDER);
	if (!sec_page) {
		sec_print_err("alloc pages error\n");
		return SEC_ERROR;
	}
	g_secboot_buffer = (char *)page_address(sec_page);
	if (!g_secboot_buffer) {
		__free_pages(sec_page, SECBOOT_BUFFER_ORDER);
		sec_page = NULL;
		sec_print_err("g_secboot_buffer is NULL\n");
		return SEC_ERROR;
	}
	return SEC_OK;
}
...
#ifdef CONFIG_SECBOOT_BUFFER_FORM_PAGE
	int ret = SEC_OK;
	ret = bsp_malloc_seboot_buffer();
	if (ret != SEC_OK) {
		sec_print_err("malloc secboot buffer error\n");
		panic("[%s] error! ret 0x%x\n", __func__, ret);
	}
#endif
