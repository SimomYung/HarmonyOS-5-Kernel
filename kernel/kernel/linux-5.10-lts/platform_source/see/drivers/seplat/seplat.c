...
static struct task_struct *g_shutdown_thread;
...
static int seplat_schdule_shutdown_work_sync(void)
{
	int ret;
	...
	ret = wake_up_process(g_shutdown_thread);
	if (ret == 0) {
...
static int seplat_shutdown_thread(void *arg)
{
	int ret;
	unused(arg);
...
static int __init seplat_probe(struct platform_device *pdev)
{
	...
	g_shutdown_thread = kthread_create(seplat_shutdown_thread,
									   NULL, "seplat_shutdown_thread");
	if (IS_ERR((const void *)g_shutdown_thread)) {
		pr_err("kthread_create failed, ret=%ld\n", PTR_ERR((const void *)g_shutdown_thread));
		return PTR_ERR((const void *)g_shutdown_thread);
	}
...
static void seplat_shutdown(struct platform_device *pdev)
{
	int ret;
	unused(pdev);
	...
	ret = seplat_schdule_shutdown_work_sync();
	if (ret)
		pr_err("seplat shutdown fail, ret = %d\n", ret);
