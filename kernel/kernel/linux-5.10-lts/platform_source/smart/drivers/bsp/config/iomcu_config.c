...
int iomcu_config_register(int id, int len, int attr)
{
	...
	pr_info("%s: register success id %x, len %x\n", __func__, id, len);
...
EXPORT_SYMBOL(iomcu_config_get);
...
static void sub_init(void)
{
	...
	(void)iomcu_config_register(IOMCU_CONFIG_USR_DROP, sizeof(struct drop_config), 0);
#ifdef CONFIG_CONTEXTHUB_MNTN_STATUS_SAVELOG
	(void)iomcu_config_register(IOMCU_CONFIG_USR_MNTN_STATUS, sizeof(struct mntn_status_config), 0);
#endif
	(void)iomcu_config_register(IOMCU_CONFIG_USR_UDP_FLAG, sizeof(struct iomcu_udp_flag_node), 0);
	(void)iomcu_config_register(IOMCU_CONFIG_USR_IOMCU_SHUTDOWN, sizeof(struct iomcu_shutdown_config), 0);
	(void)iomcu_config_register(IOMCU_CONFIG_USR_IOMCU_REMOTE_DUMP, sizeof(struct iomcu_remote_dump_config), 0);
	(void)iomcu_config_register(IOMCU_CONFIG_USR_IOMCU_QUICK_ON, sizeof(struct iomcu_quick_on_config), 0);
	(void)iomcu_config_register(IOMCU_CONFIG_USR_IOMCU_MIPI_CLK, sizeof(struct iomcu_mipi_clk_config), 0);
