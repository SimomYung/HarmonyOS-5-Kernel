...
#ifdef CONFIG_CONTEXTHUB_MID_HEADER_FILE
#include "soc_mid_iomcu.h"
#else
...
#elif defined(CONFIG_HEADER_FILE_REPLACE_EXTRACT)
#include "iomcu_mem.h"
#else
...
#endif
#ifdef CONFIG_WATCH_IOMCU_DUMP_TIME
#include <platform_include/cee/linux/lpcpu_idle_sleep.h>
#include <linux/pm_qos.h>
static struct pm_qos_request g_qos_req;
#endif
...
#ifdef CONFIG_DFX_SEC_QIC_V300_MODID_REGISTER
#ifdef QIC_CENTER_CROSSBAR_BUS
#define QIC_IOMCU_RELATED_BUSES (QIC_AO_BUS | QIC_IOMCU_BUS | QIC_CENTER_CROSSBAR_BUS)
#else
#define QIC_IOMCU_RELATED_BUSES (QIC_AO_BUS | QIC_IOMCU_BUS)
#endif
#endif
...
#ifdef CONFIG_RISCV_DUMP_ADAPT
#define RISCV_EXCEPTION_LPMCU_TMOUT      0x9A
#endif

#define BOX_DRV             BIT(0)
#define BOX_APP             BIT(2)
#define BOX_AI              BIT(3)
#define BOX_AOD             BIT(4)
#define BOX_ALL             (0xFFFFFFFFU)
#define BOX_MAX             (5)

const static unsigned int g_usr_boxid_map[REC_USR_MAX] = {
	BOX_DRV,                                // REC_USR_LOG
	BOX_APP | BOX_DRV,                      // REC_USR_DUMP
	BOX_APP | BOX_DRV,                      // REC_USR_AOD
	BOX_APP | BOX_DRV,                      // REC_USR_FP
	BOX_APP | BOX_DRV,                      // REC_USR_FP_UD
	BOX_APP | BOX_DRV,                      // REC_USR_KB
	BOX_APP | BOX_DRV,                      // REC_USR_MLPS
	BOX_APP | BOX_DRV,                      // REC_USR_MOTION
	BOX_APP | BOX_DRV,                      // REC_USR_SENSOR
	BOX_APP | BOX_DRV,                      // REC_USR_THP
	BOX_DRV,                                // REC_USR_IPC
	BOX_APP | BOX_DRV,                      // REC_USR_BLE
	BOX_APP | BOX_DRV,                      // REC_USR_CHRE
	BOX_APP | BOX_AI | BOX_AOD | BOX_DRV,   // REC_USR_UDI
	BOX_APP | BOX_DRV,                      // REC_USR_AR
	BOX_APP | BOX_DRV,                      // REC_USR_FLP
	BOX_AI | BOX_DRV,                       // REC_USR_IGS
	BOX_DRV,                                // REC_USR_IODIE
	BOX_APP | BOX_DRV,                      // REC_USR_PEDOMETER
	BOX_APP | BOX_DRV,                      // REC_USR_BLPWM
	BOX_APP | BOX_DRV,                      // REC_USR_BASTET
	BOX_AI | BOX_DRV,                       // REC_USR_LPC
};

struct iomcu_dump_priv {
	struct list_head head;
	struct rw_semaphore rwsem;
};

struct iomcu_dump_notifier_node {
	struct notifier_block *nb;
	struct list_head entry;
	enum rec_user_e usr;
};

static struct iomcu_dump_priv g_iomcu_dump_priv = {LIST_HEAD_INIT(g_iomcu_dump_priv.head)};
...
u8 g_mcu_dump_recovery_stat = 1;

u8 get_mcu_recovery_stat(void)
{
	return g_mcu_dump_recovery_stat;
}
...
static void set_iom3_power_state(int state)
{
#if defined(CONFIG_SYS_PM_ULSR) || defined(CONFIG_WATCH_IOMCU_DUMP_TIME)
	if (state)
		enable_irq(g_sensorhub_wdt_irq);
	else
		disable_irq(g_sensorhub_wdt_irq);
#else
	(void)state;
#endif
}
...
static void sh_dump(u32 modid, u32 etype, u64 coreid, char *pathname, pfn_cb_dump_done pfn_cb)
{
	...
	if ((modid == SENSORHUB_IODIE_MODID) || (modid == SENSORHUB_IODIE_SYS_MODID)) {
		pr_warn("%s: iodie err, no need to reset sensorhub\n", __func__);
		if (pfn_cb)
			pfn_cb(modid, g_current_sh_core_id);
		return;
	}
#ifdef CONFIG_CONTEXTHUB_IGS_VOLTAGE_CURE
	my_func_t my_func = NULL;

	if (modid == SENSORHUB_TINY_MODID) {
		my_func = symbol_get(igs_tiny_vlotage_cure);
#ifdef PAC_FWCFI_SYSSERVICE
		PAC_FECFI_SIGN(my_func, my_func_t);
#endif
		if (my_func) {
			my_func();
			symbol_put(igs_tiny_vlotage_cure);
		} else {
			printk(KERN_ERR "failed to get symbol\n");
		}
	}
#endif
...
#endif
	{
		.e_modid = SENSORHUB_IODIE_MODID,
		.e_modid_end = SENSORHUB_IODIE_MODID,
		.e_process_priority = RDR_WARN,
		.e_reboot_priority = RDR_REBOOT_NO,
		.e_notify_core_mask = RDR_IOM3 | RDR_AP,
		.e_reset_core_mask = RDR_IOM3,
		.e_reentrant = RDR_REENTRANT_ALLOW,
		.e_exce_type = IODIE_S_EXCEPTION,
		.e_from_core = RDR_IOM3,
		.e_upload_flag = RDR_UPLOAD_YES,
		.e_save_log_flags = RDR_SAVE_LOGBUF,
		.e_from_module = "RDR_IOM7",
		.e_desc = "RDR_IOM7 for IODIE trigger dump."
	},
	{
		.e_modid = SENSORHUB_IODIE_SYS_MODID,
		.e_modid_end = SENSORHUB_IODIE_SYS_MODID,
		.e_process_priority = RDR_ERR,
		.e_reboot_priority = RDR_REBOOT_NOW,
		.e_notify_core_mask = RDR_IOM3 | RDR_AP,
		.e_reset_core_mask = RDR_IOM3 | RDR_AP,
		.e_reentrant = RDR_REENTRANT_ALLOW,
		.e_exce_type = IODIE_S_EXCEPTION,
		.e_from_core = RDR_IOM3,
		.e_upload_flag = RDR_UPLOAD_YES,
		.e_save_log_flags = RDR_SAVE_LOGBUF,
		.e_from_module = "RDR_IOM7",
		.e_desc = "RDR_IOM7 for IODIE_SYS trigger dump."
	},
	{
		.e_modid = SENSORHUB_PANIC_MODID,
		.e_modid_end = SENSORHUB_PANIC_MODID,
		.e_process_priority = RDR_ERR,
		.e_reboot_priority = RDR_REBOOT_NOW,
		.e_notify_core_mask = RDR_IOM3 | RDR_AP | RDR_LPM3,
		.e_reset_core_mask = RDR_AP,
		.e_reentrant = RDR_REENTRANT_ALLOW,
		.e_exce_type = IOM3_S_EXCEPTION_PANIC,
		.e_from_core = RDR_IOM3,
		.e_upload_flag = RDR_UPLOAD_YES,
		.e_save_log_flags = RDR_SAVE_LOGBUF,
		.e_from_module = "RDR_IOM7",
		.e_desc = "RDR_IOM7 for AP_PANIC trigger dump."
	},
};
...
static void sh_qic_register(void)
{
	...
	dfx_qic_modid_register(iomcu_m7_mid, iomcu_m7_mid,
						   QIC_IOMCU_RELATED_BUSES, SENSORHUB_MODID);
#else
	dfx_qic_modid_register(SOC_IOMCU_M7_MID, SOC_IOMCU_M7_MID,
						   QIC_IOMCU_RELATED_BUSES, SENSORHUB_MODID);
#ifdef CONFIG_CONTEXTHUB_MID_HEADER_FILE
	dfx_qic_modid_register(SOC_MID_NPU_TINY_CORE0, SOC_MID_NPU_TINY_CORE0,
						   QIC_NPU_BUS, SENSORHUB_TINY_MODID);
#ifndef CONFIG_HISI_VBAT_DROP_PRT_XIA
	dfx_qic_modid_register(SOC_MID_NPU_TINY_CORE1, SOC_MID_NPU_TINY_CORE1,
						   QIC_NPU_BUS, SENSORHUB_TINY_MODID);
#endif
	dfx_qic_modid_register(SOC_MID_NPU_HWTS_TINY, SOC_MID_NPU_HWTS_TINY,
						   QIC_NPU_BUS, SENSORHUB_TINY_MODID);
	dfx_qic_modid_register(SOC_MID_NPU_HWTS_TINY, SOC_MID_NPU_HWTS_TINY,
						   QIC_NPUTS_BUS, SENSORHUB_TINY_MODID);
#endif
#ifdef CONFIG_CHARPRO_IP_PLATFORM
	dfx_qic_modid_register(SOC_MID_NPU_TINY_CORE0, SOC_MID_NPU_TINY_CORE0,
						   QIC_NPU_BUS, SENSORHUB_TINY_MODID);
	...
	dfx_qic_modid_register(iomcu_dma_mid, iomcu_dma_mid,
						   QIC_IOMCU_RELATED_BUSES, SENSORHUB_IOMCU_DMA_MODID);
#else
...
static bool is_ulsr_recovery(void)
{
	struct dump_config *dump = NULL;
	dump = iomcu_config_get(IOMCU_CONFIG_USR_DUMP, sizeof(struct dump_config));
	if (dump == NULL)
		return false;
	return dump->reason == SH_FAULT_ULSR_RESUME;
}

#ifdef CONFIG_WATCH_IOMCU_DUMP_TIME
__weak void save_mcu_dump_file(void)
{
}
#endif
...
static int rdr_sh_thread(void *arg)
{
	...
	while (1) {
		...
		if (IS_ENABLE_DUMP) {
			...
			if (dump != NULL)
				pr_warn("dump reason idx %d, boxid %x\n", dump->reason, dump->boxid);
...
{
	pr_info(" register_iom3_recovery_notifier: usr %x.\n", usr);

#ifdef CONFIG_BOX_RECOVERY
	struct iomcu_dump_notifier_node *pnode = NULL;

	if (usr >= REC_USR_MAX) {
		pr_err(" register_iom3_recovery_notifier: invalid usr %u.\n", (unsigned int)usr);
		return -EINVAL;
	}

	down_write(&g_iomcu_dump_priv.rwsem);
	list_for_each_entry(pnode, &g_iomcu_dump_priv.head, entry) {
		if ((nb == pnode->nb) && (usr == pnode->usr)) {
			pr_warn("[%s]: nb already registed\n!", __func__);
			up_write(&g_iomcu_dump_priv.rwsem);
			return -EINVAL;
		}
	}

	pnode = kzalloc(sizeof(struct iomcu_dump_notifier_node), GFP_ATOMIC);
	...
	list_add(&pnode->entry, &g_iomcu_dump_priv.head);
	up_write(&g_iomcu_dump_priv.rwsem);
#endif
	return blocking_notifier_chain_register(&iom3_recovery_notifier_list, nb);
}
EXPORT_SYMBOL(register_iom3_recovery_notifier);

int unregister_iom3_recovery_notifier(enum rec_user_e usr, struct notifier_block *nb)
{
	pr_info(" unregister_iom3_recovery_notifier: usr %x.\n", usr);

#ifdef CONFIG_BOX_RECOVERY
	int ret = 0;
	struct iomcu_dump_notifier_node *pos = NULL;
	struct iomcu_dump_notifier_node *n = NULL;

	if (usr >= REC_USR_MAX) {
		pr_err(" unregister_iom3_recovery_notifier: invalid usr %u.\n", (unsigned int)usr);
		return -EINVAL;
	}

	// try to found node and free it.
	down_write(&g_iomcu_dump_priv.rwsem);
	list_for_each_entry_safe(pos, n, &g_iomcu_dump_priv.head, entry) {
		if ((usr == pos->usr) && (nb == pos->nb)) {
			list_del(&pos->entry);
			kfree(pos);
			pos = NULL;
			break;
...
#ifdef CONFIG_WATCH_IOMCU_DUMP_TIME
	/* sensorhub restarts successfully, and the cpu can enter idle */
	lpcpu_idle_sleep_vote(ID_IOMCU, 0);
	cpu_latency_qos_remove_request(&g_qos_req);
#endif
...
#ifdef CONFIG_WATCH_IOMCU_DUMP_TIME
__weak void mcu_dfx_info(void)
{
}

__weak bool is_reset_doing(void)
{
	return false;
}
#endif
...
static void iom3_recovery_work(struct work_struct *work)
{
	...
	u32 tx_buffer;
#ifdef CONFIG_WATCH_IOMCU_DUMP_TIME
	u32 mcuready_timeout = 30 * HZ;
#else
	u32 mcuready_timeout = 5 * HZ;
#endif

	pr_err("%s enter\n", __func__);
	__pm_stay_awake(g_iom3_rec_wl);
#ifdef CONFIG_WATCH_IOMCU_DUMP_TIME
	/* during the sensorhub reset, the cpu idle cannot be entered */
	lpcpu_idle_sleep_vote(ID_IOMCU, 1);
	cpu_latency_qos_add_request(&g_qos_req, PM_QOS_DEFAULT_VALUE);
#endif
...
recovery_iom3:
	...
	if (rec_nest_count++ > IOM3_REC_NEST_MAX) {
		...
		cpu_latency_qos_remove_request(&g_qos_req);
#endif
		set_iom3_power_state(1);
		pr_err("%s exit\n", __func__);
#ifdef CONFIG_WATCH_IOMCU_DUMP_TIME
		union sctrl_sh_boot sh_boot;
		sh_boot = get_boot_stat();
		pr_err("iomcu triggered bug_on\n");
		rdr_syserr_process_for_ap(SENSORHUB_PANIC_MODID, (uint64_t)sh_boot.reg.boot_step, (uint64_t)sh_boot.reg.boot_stat);
#endif
		return;
	}
...
#ifdef CONFIG_CONTEXTHUB_LPMCU_DUMP_LOG
	(void)save_sh_last_log();
#endif
	iomcu_reinit();
	...
	iomcu_load_image();
#ifdef CONFIG_WATCH_IOMCU_DUMP_TIME
	tx_buffer = LOADED_IOM3_IMG_CMD;
	rc = RPROC_ASYNC_SEND(ipc_ap_to_lpm_mbx, &tx_buffer, 1);
	if (rc) {
		pr_err("RPROC load iom3 image failed %d, nest_count %d\n", rc, rec_nest_count);
		goto recovery_iom3;
	}
#endif
#endif
	...
	if (rc) {
#ifdef CONFIG_WATCH_IOMCU_DUMP_TIME
		pr_err("RPROC start iom3 ready wait longer %d, nest_count %d\n", rc, rec_nest_count);
#else
...
#ifndef CONFIG_WATCH_IOMCU_DUMP_TIME
	set_iom3_power_state(1);
#endif
#ifdef CONFIG_DFX_DEBUG_FS
	show_iom3_stat();
...
#endif
	/* dynamic loading */
	if (wait_for_completion_timeout(&iom3_rec_done, mcuready_timeout) == 0) {
...
#endif
#ifdef CONFIG_RISCV_DUMP_ADAPT
		// if dump reason is lpmcu timeout, just set as user dump(only for riscv)
		if ((dump != NULL) && (dump->reason == RISCV_EXCEPTION_LPMCU_TMOUT))
			modid = SENSORHUB_USER_MODID;
#else
		if ((dump != NULL) && (dump->reason == LPMCU_TIMEOUT))
			modid = SENSORHUB_USER_MODID;
#endif
#ifdef CONFIG_CONTEXTHUB_BOOT_STAT
#ifdef CONFIG_DFX_BB
...
int iomcu_check_dump_status(bool is_lock)
{
	...
	if (is_lock == true) {
		if (rec_state != IOM3_RECOVERY_IDLE) {
			pr_warn("%s iomcu is dumping [%d].\n", __func__, rec_state);
			return -ESPIPE;
		}
...
static void iomcu_dump_exit(void)
{
	...
	(void)unregister_iom3_recovery_notifier(REC_USR_DUMP, &recovery_notify);
}
...
#ifdef CONFIG_BOX_RECOVERY
static void iomcu_dump_box_recovery_notify(unsigned int boxid, unsigned long state)
{
	unsigned long flags = 0;
	struct iomcu_dump_notifier_node *pos = NULL;
	struct iomcu_dump_notifier_node *n = NULL;
	unsigned int box_info = 0;

	// try to found node and free it.
	down_read(&g_iomcu_dump_priv.rwsem);
	list_for_each_entry_safe(pos, n, &g_iomcu_dump_priv.head, entry) {
		if (pos->usr >= REC_USR_MAX)
			continue;
		if (pos->nb == NULL)
			continue;
		if (pos->nb->notifier_call == NULL)
			continue;

		box_info = g_usr_boxid_map[pos->usr];
		if ((BIT(boxid) & box_info) == 0)
			continue;

		pr_info("%s: usr %x, map %x, state %x\n", __func__, pos->usr, g_usr_boxid_map[pos->usr], state);

		// call notifier
		pos->nb->notifier_call(pos->nb, state, NULL);
	}
	up_read(&g_iomcu_dump_priv.rwsem);

	return;
}

static int iomcu_dump_box_callback(const struct pkt_header *head)
{
	pr_info("%s: 0 rec state .\n", __func__);

	if (head == NULL) {
		pr_err("%s: invalid head.\n", __func__);
		return 0;
	}

	if (head->cmd != CMD_SYS_BOX_RECOVERY) {
		pr_err("%s: invalid cmd 0x%x.\n", __func__, head->cmd);
		return 0;
	}

	pkt_box_recovery_t *pkt_rec = (pkt_box_recovery_t *)head;
	if (pkt_rec->boxid >= BOX_MAX) {
		pr_err("%s: invalid boxid %u.\n", __func__, pkt_rec->boxid);
		return 0;
	}

	pr_info("%s: rec state %x, boxid %u\n", __func__, get_iom3_rec_state(), pkt_rec->boxid);
	save_sh_dump_file(g_sensorhub_dump_buff, g_dump_info,
				g_iomcu_ao_ns_ipc_base, g_dump_mail_no, IOMCU_DUMP_IPC_MAILNO_COUNT);

	iomcu_dump_box_recovery_notify(pkt_rec->boxid, IOM3_RECOVERY_START);
	iomcu_dump_box_recovery_notify(pkt_rec->boxid, IOM3_RECOVERY_MINISYS);
	iomcu_dump_box_recovery_notify(pkt_rec->boxid, IOM3_RECOVERY_BOXREADY);
	iomcu_dump_box_recovery_notify(pkt_rec->boxid, IOM3_RECOVERY_DOING);
	iomcu_dump_box_recovery_notify(pkt_rec->boxid, IOM3_RECOVERY_3RD_DOING);
	iomcu_dump_box_recovery_notify(pkt_rec->boxid, IOM3_RECOVERY_IDLE);
...
static int iomcu_dump_init(void)
{
	...
	init_rwsem(&g_iomcu_dump_priv.rwsem);
	...
	ret = register_iom3_recovery_notifier(REC_USR_DUMP, &recovery_notify);
	if (ret != 0) {
...
#ifdef CONFIG_BOX_RECOVERY
	ret = register_mcu_event_notifier(TAG_SYS, CMD_SYS_BOX_RECOVERY,
					  iomcu_dump_box_callback);
	if (ret != 0)
		pr_err("[%s], register sys status failed\n", __func__);
#endif
