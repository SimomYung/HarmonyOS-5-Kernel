...
#endif
#ifdef CONFIG_CONTEXTHUB_MNTN_STATUS_SAVELOG
#include <platform_include/smart/linux/mntn_status_savelog.h>
#endif
#include <platform_include/basicplatform/linux/dfx_bbox_diaginfo.h>
#include "bbox_diaginfo_id_def.h"
#ifdef CONFIG_CONTEXTHUB_EXTRACT_DUMP_LOG
...
static char *g_sh_reset_reasons[] = {
	...
	"SH_FAULT_REACT",          "SH_FAULT_IO_DIE",
	"SH_FAULT_ULSR_RESUME",    "BOX_RECOVERY",
	"LPMCU_TIMEOUT",
	"SH_FAULT_EXP_BOTTOM", // also use as unknow dump
};
#else
static char *g_sh_box_name[] = {
	"drvbox",
	"shellbox",
	"appbox",
	"aibox",
...
#ifdef CONFIG_CONTEXTHUB_MNTN_STATUS_SAVELOG
void *get_dump_restore_dir(void)
{
	return (void *)g_dump_restore_dir;
}
#endif
...
static int __sh_create_dir(char *path, unsigned int len)
{
	...
	if (len > CUR_PATH_LEN) {
		pr_err("invalid  parameter. len is %u\n", len);
		return -1;
	}
	if (path == NULL) {
		pr_err("null path\n");
...
static int sh_readfs2buf(char *logpath, char *filename, void *buf, u32 len)
{
	...
	if (read_size != len)
		pr_err("%s:read file %s exception with ret %ld.\n", __func__, path, read_size);
	else
		ret = (int)read_size;
...
static int sh_savebuf2fs(char *logpath, char *filename, void *buf, u32 len, u32 is_append)
{
	...
	if (write_size != len) {
		pr_err("%s:write file exception with ret %ld.\n", __func__, write_size);
		goto FS_CLOSE;
	}
...
static char *get_box_name(unsigned int boxid)
{
#ifndef CONFIG_RISCV_DUMP_ADAPT
	return "unknown";
#else
	if (boxid >= (uint32_t)ARRAY_SIZE(g_sh_box_name))
		return "unknown";

	return g_sh_box_name[boxid];
#endif
}
...
static int get_dump_boxid(void)
{
	struct dump_config *dump = iomcu_config_get(IOMCU_CONFIG_USR_DUMP, sizeof(struct dump_config));
...
static int write_sh_dump_history(union dump_info dump_info)
{
	...
	int boxid = get_dump_boxid();
	box_name = get_box_name(boxid);

	if (g_dp_regs != NULL) {
		print_ret = snprintf_s(buf, sizeof(buf), sizeof(buf) - 1, "[%02u], reason [%s], boxid[0x%x], boxname[%s],"
			"%x, %x, %x, %x, %x, %x, %x, %x, dump_info[0x%x], time [%s]\n",
			g_dump_index, content, boxid, box_name,
			g_dp_regs[EXP_PC], g_dp_regs[EXP_LR], g_dp_regs[CFSR],
			g_dp_regs[HFSR], g_dp_regs[MMFAR], g_dp_regs[BFAR],
			g_dp_regs[PERI_USED], g_dp_regs[AO_CNT], dump_info.value, date);
		if (print_ret < 0)
			pr_err("%s write dp regs history failed\n", __func__);

		(void)bbox_diaginfo_record(IOMCU_EXC_INFO, NULL, "[%02u], reason [%s], boxid[0x%x], boxname[%s]"
			"%x, %x, %x, %x, %x, %x, %x, %x, dump_info[0x%x], time [%s]\n",
			g_dump_index, content, boxid, box_name,
			g_dp_regs[EXP_PC], g_dp_regs[EXP_LR], g_dp_regs[CFSR],
			g_dp_regs[HFSR], g_dp_regs[MMFAR], g_dp_regs[BFAR],
		...
		print_ret = snprintf_s(buf, sizeof(buf), sizeof(buf) - 1,
			"[%02u], reason [%s], , boxid[0x%x], boxname[%s], dump_info[0x%x], time [%s]",
			g_dump_index, content, boxid, box_name, dump_info.value, date);
		if (print_ret < 0)
			pr_err("%s write history failed\n", __func__);
		(void)bbox_diaginfo_record(IOMCU_EXC_INFO, NULL, "[%02u], reason [%s], boxid[0x%x], boxname[%s], dump_info[0x%x], time [%s]",
			g_dump_index, content, boxid, box_name, dump_info.value, date);
	}
...
int __weak load_sh_dict(void)
{
	return 0;
}

int __weak extract_sh_dump_log(uint8_t *sensorhub_dump_buff, union dump_info dump_info, uint32_t dump_index)
{
	return 0;
}
...
#ifdef CONFIG_WATCH_IOMCU_DUMP_TIME
	if (is_recovery_mode()) {
		pr_info("%s skipped in recovery mode!\n", __func__);
		return 0;
	}
#endif
...
#ifdef CONFIG_CONTEXTHUB_EXTRACT_DUMP_LOG
	if (load_sh_dict() == 0) {
		pr_info("load dict succ\n");
		if (extract_sh_dump_log(sensorhub_dump_buff, dump_info, g_dump_index)) {
			pr_err("failed to extract dump_log\n");
		} else {
			pr_info("extract_sh_dump_log succ\n");
		}
	} else {
		pr_err("failed to load dict\n");
	}
#endif
...
#define SH_LAST_LOG_SIZE 0x80000
static int write_sh_last_log_file(uint8_t *iomcu_last_log, uint32_t length)
{
	int ret;
	char path[PATH_MAXLEN];
	struct dump_zone_head *dzh = NULL;
	...
	if (ret != EOK)
		pr_err("[%s]memset_s path fail[%d]\n", __func__, ret);
	ret = sprintf_s(path, PATH_MAXLEN, "sensorhub-log-last-%02u", g_dump_index);
	if (ret < 0)
		pr_err("[%s]sprintf_s PATH_MAXLEN fail[%d]\n", __func__, ret);
	pr_info("%s: file %s\n", __func__, path);
#if (KERNEL_VERSION(4, 4, 0) > LINUX_VERSION_CODE)
	flush_cache_all();
#endif
	(void)iomcu_config_get(IOMCU_CONFIG_USR_DUMP, sizeof(struct dump_config));
	if (iomcu_last_log != NULL)
		sh_savebuf2fs(g_dump_restore_dir, path, iomcu_last_log, length, 0);
	return 0;
}

int save_sh_last_log(void)
{
#ifdef CONFIG_SECURITY_HEADER_FILE_REPLACE
	uint32_t sensorhub_dump_buff_addr = dts_ddr_addr_get(DTSI_DDR_SENSORHUB_DUMP_BUFF_ADDR_AP);
	uint32_t sensorhub_dump_buff_size = dts_ddr_size_get(DTSI_DDR_SENSORHUB_DUMP_BUFF_ADDR_AP);
#else
	uint32_t sensorhub_dump_buff_addr = RESERVED_SENSORHUB_SHARE_MEM_PHYMEM_BASE;
	uint32_t sensorhub_dump_buff_size = SH_LAST_LOG_SIZE;
#endif
	if (sensorhub_dump_buff_addr == 0 || sensorhub_dump_buff_size == 0) {
		pr_err("%s sensorhub_dump_buff_addr or sensorhub_dump_buff_size err\n", __func__);
		return -1;
	}
	uint8_t *iomcu_last_log = (uint8_t *)ioremap_wc(sensorhub_dump_buff_addr, sensorhub_dump_buff_size);
	char dump_fs[PATH_MAXLEN] = SH_DMP_FS;
	if (IS_ENABLE_DUMP == 0) {
		pr_info("%s skipped!\n", __func__);
		return 0;
	...
	if (iomcu_last_log == NULL) {
		pr_err("%s iomcu_last_log is null\n", __func__);
		return -1;
	}

	write_sh_last_log_file(iomcu_last_log, sensorhub_dump_buff_size);
	return 0;
}
#endif