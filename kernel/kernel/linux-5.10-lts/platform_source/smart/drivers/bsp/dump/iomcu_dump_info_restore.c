...
static int write_sh_dump_history(union dump_info dump_info)
{
	...
	if (g_dp_regs != NULL) {
		print_ret = snprintf_s(buf, sizeof(buf), sizeof(buf) - 1, "reason [%s], [%02u], time [%s],"
			"%x, %x, %x, %x, %x, %x, %x, %x, dump_info[0x%x], boxid[0x%x], boxname[%s]\n",
			content, g_dump_index, date,
			g_dp_regs[EXP_PC], g_dp_regs[EXP_LR], g_dp_regs[CFSR],
			g_dp_regs[HFSR], g_dp_regs[MMFAR], g_dp_regs[BFAR],
			g_dp_regs[PERI_USED], g_dp_regs[AO_CNT], dump_info.value, boxid, box_name);
		if (print_ret < 0)
			pr_err("%s write dp regs history failed\n", __func__);

		(void)bbox_diaginfo_record(IOMCU_EXC_INFO, NULL, "reason [%s], [%02u], time [%s],"
			"%x, %x, %x, %x, %x, %x, %x, %x, dump_info[0x%x], boxid[0x%x], boxname[%s]\n",
			content, g_dump_index, date,
			g_dp_regs[EXP_PC], g_dp_regs[EXP_LR], g_dp_regs[CFSR],
			g_dp_regs[HFSR], g_dp_regs[MMFAR], g_dp_regs[BFAR],
		...
		print_ret = snprintf_s(buf, sizeof(buf), sizeof(buf) - 1,
			"reason [%s], [%02u], time [%s],dump_info[0x%x], boxid[0x%x], boxname[%s]",
			content, g_dump_index, date, dump_info.value, boxid, box_name);
		if (print_ret < 0)
			pr_err("%s write history failed\n", __func__);
		(void)bbox_diaginfo_record(IOMCU_EXC_INFO, NULL, "reason [%s], [%02u], time [%s],dump_info[0x%x], boxid[0x%x], boxname[%s]",
			content, g_dump_index, date, dump_info.value, boxid, box_name);
	}
...
static int write_sh_last_log_file(uint8_t *iomcu_last_log)
{
	int ret;
	char path[PATH_MAXLEN];
	struct dump_zone_head *dzh = NULL;
	...
	if (iomcu_last_log != NULL)
		sh_savebuf2fs(g_dump_restore_dir, path, iomcu_last_log, 508 * 1024, 0);
	return 0;
}
...
int save_sh_last_log(void)
{
	...
	uint32_t sensorhub_dump_buff_size = DDR_SHMEM_SIZE - DDR_CONFIG_SIZE;
#endif
	...
	write_sh_last_log_file(iomcu_last_log);
	return 0;
}
#endif