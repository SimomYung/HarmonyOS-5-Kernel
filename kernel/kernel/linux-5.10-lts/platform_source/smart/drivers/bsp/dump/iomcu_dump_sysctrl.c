...
#define IOMCU_STAT_REG_OFFSET SOC_SCTRL_SCBAKDATA2_MSK_ADDR(0)
#endif
#endif
...
#ifdef CONFIG_SYS_PM_ULSR
static int iomcu_stat_reg_offset;
#endif
#endif
...
void show_iom3_stat(void)
{
	...
	for (i = IOMCU_REG_CLK_SEL; i <= IOMCU_REG_DBG_INFO2; i++) {
		...
		arm_smccc_1_1_smc(BL31_IOMCU_READ_RESERVED_REGS, i, 0, 0, 0, 0, 0, 0, &res);
#else
...
static void get_boot_stat_from_iomcu_reg(void)
{
	...
	struct arm_smccc_res res = {0};
#ifdef CONFIG_LIBLINUX
		arm_smccc_1_1_smc(BL31_IOMCU_READ_RESERVED_REGS, IOMCU_REG_RESERVED_REG0, 0, 0, 0, 0, 0, 0, &res);
#else
		arm_smccc_smc(BL31_IOMCU_READ_RESERVED_REGS, IOMCU_REG_RESERVED_REG0, 0, 0, 0, 0, 0, 0, &res);
#endif
	step0 = res.a0;
#ifdef CONFIG_LIBLINUX
		arm_smccc_1_1_smc(BL31_IOMCU_READ_RESERVED_REGS, IOMCU_REG_RESERVED_REG1, 0, 0, 0, 0, 0, 0, &res);
#else
...
#ifdef CONFIG_SYS_PM_ULSR
void reset_iomcu_stat(void)
{
	iomcu_stat_reg_offset = IOMCU_STAT_REG_OFFSET;
	if (g_sysctrl_base != NULL)
		writel(0x20000, g_sysctrl_base + iomcu_stat_reg_offset);
}
#endif
#endif
