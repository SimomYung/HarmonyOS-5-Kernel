...
#ifdef CONFIG_CONTEXTHUB_IOMCU_IPC_SLAB
	static struct kmem_cache *g_iomcu_link_header_cache = NULL;
#endif
...
int iomcu_link_ipc_send(uint8_t prio, enum tcp_ver version, void *data, uint16_t length, bool resp)
{
	...
	if (link_head->shmem)
		pr_info("[%s] [0x%x] [0x%x] tag[0x%x] cmd[0x%x] lvl %d resp %d ver %d tranid %d shmem %d len %d,\
			offset %x\n", __func__, temp[0], *((uint32_t *)data), *((uint8_t *)data), *((uint8_t *)data + 1),\
			link_head->prio, link_head->resp, link_head->version, link_head->transid, link_head->shmem,\
			link_head->length, node->shmem_offset);
	else
		pr_info("[%s] [0x%x] [0x%x] tag[0x%x],cmd[0x%x] lvl %d resp %d ver %d tranid %d shmem %d len %d\n",\
			__func__, temp[0], temp[1], *(uint8_t*)&temp[1], *((uint8_t*)&temp[1]+1),\
			link_head->prio, link_head->resp, link_head->version, link_head->transid, link_head->shmem,\
			link_head->length);
...
unsigned long iomcu_link_ipc_get_buffer(uint16_t size, uint32_t context)
{
	...
	unsigned long flags = 0;
#ifdef CONFIG_CONTEXTHUB_IOMCU_IPC_SLAB
	buffer_node = (struct iomcu_link_buffer_node *)kmem_cache_alloc(
		g_iomcu_link_header_cache,
		GFP_ATOMIC);

	if (buffer_node == NULL) {
		pr_err("%s: alloc buffer node failed \n", __func__);
		return 0;
	}
	if (memset_s(buffer_node, sizeof(*buffer_node), 0, sizeof(*buffer_node)) != EOK) {
		pr_err("%s: memset_s buffer node failed \n", __func__);
		return 0;
	}
#else
...
#ifdef CONFIG_CONTEXTHUB_IOMCU_IPC_SLAB
			kmem_cache_free(g_iomcu_link_header_cache, buffer_node);
#else
			kfree(buffer_node);
#endif
			buffer_node = NULL;
...
void iomcu_link_ipc_put_buffer(unsigned long tcp_addr, bool is_force)
{
	...
	list_del(&node->list);
#ifdef CONFIG_CONTEXTHUB_IOMCU_IPC_SLAB
	kmem_cache_free(g_iomcu_link_header_cache, node);
#else
	kfree(node);
#endif
	node = NULL;
...
#ifdef CONFIG_CONTEXTHUB_IOMCU_IPC_SLAB
	g_iomcu_link_header_cache = kmem_cache_create("g_iomcu_link_header_cache", sizeof(struct iomcu_link_buffer_node), 0, 0, NULL);
	if (g_iomcu_link_header_cache == NULL) {
		pr_err("Unable to create g_iomcu_link_header_cache \n");
		return -EINVAL;
	}
#endif
...
#ifdef CONFIG_CONTEXTHUB_IOMCU_IPC_SLAB
	kmem_cache_destroy(g_iomcu_link_header_cache);
#endif
	pr_info("[%s] done...\n", __func__);
}
