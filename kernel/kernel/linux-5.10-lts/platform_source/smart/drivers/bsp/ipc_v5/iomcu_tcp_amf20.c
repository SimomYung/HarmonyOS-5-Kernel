...
static int iomc_tcp_amf20_recv_handler(const void *tcp_data, const uint16_t tcp_data_len)
{
	...
	list_for_each_entry_safe(pos, n, &notify_list, entry) {
		...
		pos->notify(head->svc_id, head->cmd, (void *)&head[1], tcp_data_len - sizeof(struct tcp_head_amf20));
...
int tcp_send_data_by_amf20(const uint16_t svc_id, const uint16_t cmd, uintptr_t *data, uint16_t data_len)
{
	...
	if (data_len > ipc_shm_get_capacity()) {
		pr_err("[%s] data_len %u  Max Len %u error.\n", __func__, data_len, ipc_shm_get_capacity());
		return -EINVAL;
	}

	ret = iomcu_check_dump_status(false);
	if (ret != 0)
		return ret;
...
static int iomcu_tcp_amf20_init(void)
{
	...
	ret = register_iom3_recovery_notifier(REC_USR_IPC, &tcp_amf20_notify);
	if (ret != 0)
...
static void __exit iomcu_tcp_amf20_exit(void)
{
	...
	unregister_iom3_recovery_notifier(REC_USR_IPC, &tcp_amf20_notify);
