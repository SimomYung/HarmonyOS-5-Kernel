...
#define SH_FAULT_IPC_TX_TIMEOUT_CNT   20
#else
...
static struct mcu_event_wait_list {
	...
	.slock = __SPIN_LOCK_UNLOCKED(mcu_event_wait_list.slock),
	.head = LIST_HEAD_INIT(mcu_event_wait_list.head)
};

static int iomcu_tcp_amsm_recovery_notifier(struct notifier_block *nb, unsigned long foo, void *bar);
static struct iomcu_tcp_amsm_notifier tcp_amsm_notifier = {
	.head = LIST_HEAD_INIT(tcp_amsm_notifier.head),
	.lock = __SPIN_LOCK_UNLOCKED(tcp_amsm_notifier.lock)
};
...
static void iomcu_tcp_amsm_notifier_handler(int *pkt_data, uint16_t pke_len)
{
	...
	// try to remember all matched notify.
	...
	list_for_each_entry_safe(pos, n, &tcp_amsm_notifier.head, entry) {
		...
		if (ZERO_OR_NULL_PTR((unsigned long)(uintptr_t)pnode)) {
			pr_err("[%s] alloc temporary node failed.\n", __func__);
			continue;
		}

		pnode->tag = (uint8_t)pos->tag;
		pnode->cmd = (uint8_t)pos->cmd;
		pnode->notify = pos->notify;

		list_add_tail(&pnode->entry, &notify_list);
	}
	spin_unlock_irqrestore(&tcp_amsm_notifier.lock, flags);

	// try to lookup notify_list, to do notify.
	list_for_each_entry_safe(pos, n, &notify_list, entry) {
...
static int iomc_tcp_amsm_recv_handler(const void *buf, const uint16_t len)
{
	...
	if (head->tag == TAG_SYS)
		pr_info("[%s] 1 tag[0x%x]cmd[0x%x]len[%u]\n", __func__, head->tag, head->cmd, len);
	...
	if (head->tag == TAG_SYS)
		pr_info("[%s] 2 tag[0x%x]cmd[0x%x]len[%u]\n", __func__, head->tag, head->cmd, len);
	...
	if (head->tag == TAG_SYS)
		pr_info("[%s] 3 tag[0x%x]cmd[0x%x]len[%u]\n", __func__, head->tag, head->cmd, len);
...
static int iomcu_tcp_amsm_init(void)
{
	...
	ret = register_iom3_recovery_notifier(REC_USR_IPC, &tcp_amsm_notify);
	if (ret != 0)
...
static void __exit iomcu_tcp_amsm_exit(void)
{
	...
	unregister_iom3_recovery_notifier(REC_USR_IPC, &tcp_amsm_notify);
}
