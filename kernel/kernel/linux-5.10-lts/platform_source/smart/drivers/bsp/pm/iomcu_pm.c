...
#include "dump/iomcu_dump_priv.h"
#endif
...
#endif
static struct timeval ddr_request_tv_end;
static struct timeval ddr_request_tv_begin;
...
void __weak ddr_req_statis_que_work(void)
{
	pr_err("ddr_req_statis_que_work is invalid;\n");
}

void __weak set_resume_time(struct timeval *ddr_req_tv_end)
{
	(void)ddr_req_tv_end;
}

void __weak set_suspend_time(struct timeval *ddr_req_tv_begin)
{
	(void)ddr_req_tv_begin;
}
...
int tell_modem_reset_status_to_mcu(int ap_st)
{
	struct write_info winfo;
	pkt_sys_statuschange_req_t pkt;
	struct read_info pkg_mcu;

	if  (ap_st < ST_MODEM_RESET_END) {
		(void)memset_s(&winfo, sizeof(struct write_info), 0, sizeof(struct write_info));
		(void)memset_s(&pkg_mcu, sizeof(struct read_info), 0, sizeof(struct read_info));
		(void)memset_s(&pkt, sizeof(pkt_sys_statuschange_req_t), 0, sizeof(pkt_sys_statuschange_req_t));
		winfo.tag = TAG_SYS;
		winfo.cmd = CMD_MDM_RESET_STATUS_REQ;
		winfo.wr_len = (int)(sizeof(pkt) - sizeof(pkt.hd));
		pkt.status = (unsigned short)ap_st;
		winfo.wr_buf = &pkt.status;
		ctxhub_info("------------>tell mcu modem reset in status %d\n", ap_st);
...
EXPORT_SYMBOL(tell_modem_reset_status_to_mcu);
...
static inline void do_gettimeofday(struct timeval *tv)
{
	...
	tv->tv_usec = now.tv_nsec / THOUSAND;
}
#endif
...
#ifdef CONFIG_SYS_PM_ULSR
	if (is_ulsr()) {
		ctxhub_info("%s do ulsr\n", __func__);
		clr_iom3_dump_miniready_state();
		reset_iomcu_stat();
		return RET_SUCC; // should not fail
	}
#endif
...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(5, 0, 0))
	do_gettimeofday(&ddr_request_tv_begin);
	set_suspend_time(&ddr_request_tv_begin);
#endif
	ctxhub_info("%s-\n", __func__);
	return ret;
}
...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(5, 0, 0))
	do_gettimeofday(&ddr_request_tv_end);
	set_resume_time(&ddr_request_tv_end);
#endif
	/* wait mini ready */
...
#ifdef CONFIG_CONTEXTHUB_DSS_TRISTATEID
static void sensorhub_recovey_dss_tristateid_to_mcu(void)
{
...
static int sensorhub_recovey_status_to_mcu(struct notifier_block *nb, unsigned long foo, void *bar)
{
	ctxhub_info("%s +\n", __func__);
	switch (foo) {
	case IOM3_RECOVERY_IDLE:
		sensorhub_recovey_dss_tristateid_to_mcu();
		break;
	default: break;
	}
	ctxhub_info("%s -\n", __func__);
...
#if defined(CONFIG_CONTEXTHUB_DSS_TRISTATEID) && defined(CONFIG_DKMD_DKSM)
static struct notifier_block sensorhub_recovey_notify = {
	.notifier_call = sensorhub_recovey_status_to_mcu,
	.priority = -1,
};
#endif
...
#if defined (CONFIG_CONTEXTHUB_DSS_TRISTATEID) && defined(CONFIG_DKMD_DKSM)
	ret = register_iom3_recovery_notifier(REC_USR_DSS_TRISTATEID, &sensorhub_recovey_notify);
	if (ret != 0)
		ctxhub_err("[%s], register_iom3_recovery_notifier failed\n", __func__);
#endif
