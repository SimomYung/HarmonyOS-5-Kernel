...
#define IOMCU_LOGIC_NODE_REG "logic_available_reg"
...
#ifndef CONFIG_INPUTHUB_20_970
static bool check_iomcu_logic_is_available_multibit(struct device_node *node)
{
	void __iomem *addr = NULL;
	u32 expect_val[EXPECT_VAL_MAX_LEN];
	u32 reg;
	u32 msk;
	u32 val;
	int ret;
	...
	ret = of_property_read_u32(node, IOMCU_LOGIC_NODE_REG, &reg);
	if (ret) {
		pr_warn("[%s] read reg addr fail, ret[%d]!\n", __func__, ret);
		return true;
	}

	ret = of_property_read_u32(node, "mask", &msk);
	if (ret) {
		pr_warn("[%s] read msk fail, ret[%d]!\n", __func__, ret);
		return true;
	}

	ret = of_property_read_variable_u32_array(node, "available_val", expect_val,
											EXPECT_VAL_MAX_LEN, EXPECT_VAL_MAX_LEN);
	if (ret < 0 || ret > EXPECT_VAL_MAX_LEN) {
		pr_warn("[%s] read val fail, ret[%d]!\n", __func__, ret);
		return true;
	}

	addr = ioremap(reg, sizeof(unsigned int));
	if (addr == NULL)
		return true;

	val = ioread32(addr) & msk;
	pr_warn("[%s] read val is 0x%x, msk is 0x%x, ret is %d\n", __func__, val, msk, ret);
	for (i = 0; i < ret; i++) {
		pr_warn("%s, val is 0x%x\n", __func__, expect_val[i]);
		if (val == expect_val[i]) {
			iounmap(addr);
			return true;
		}
...
static bool check_iomcu_logic_is_available_singlebit(struct device_node *node)
{
...
static bool ctxhub_is_iomcu_logic_available(void)
{
	...
	ret = of_property_read_u32(node, IOMCU_LOGIC_NODE_REG, &reg);
	if (ret) {
		//  not found "logic_available_reg" node
		pr_warn("[%s] read reg use single bit, ret[%d]!\n", __func__, ret);
		return check_iomcu_logic_is_available_singlebit(node);
	} else {
		// found "logic_available_reg" node
		pr_warn("[%s] read reg use multiple bit, ret[%d]!\n", __func__, ret);
		return check_iomcu_logic_is_available_multibit(node);
	}
}
#endif
...
EXPORT_SYMBOL(get_contexthub_dts_status);
