...
enum {
	...
	SUB_CMD_BLE_COLLABORATION_REPORT_REQ,
};
...
static int get_data_from_mcu(const struct pkt_header *head)
{
	...
	case SUB_CMD_BLE_WEAKUP_AP:
		...
		break;
	case SUB_CMD_BLE_COLLABORATION_REPORT_REQ:
		pr_info("[%s]: get collaboration.\n", __func__);
		status = ble_generate_netlink_packet(data, len, BLE_GENL_CMD_COLLABORATION);
		break;
	default:
...
static int __init ble_channel_register(void)
{
	...
	(void)register_iom3_recovery_notifier(REC_USR_BLE, &sensor_reboot_notify);
	pr_info("[%s]: success.\n", __func__);
	return 0;
err:
