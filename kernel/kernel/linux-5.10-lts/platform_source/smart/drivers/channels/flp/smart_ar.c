...
/*
 * Send get the Parking cmd message to contexthub, and return the status reported by AR to HAL
 */
static int ar_parking_info_cmd(const struct ar_port_t *ar_port, unsigned long arg)
{
	int ret = 0;
	struct context_ar_parking_info parking_cmd;
	struct context_dev_info_t *devinfo = &g_ar_dev.ar_devinfo;

	mutex_lock(&g_ar_dev.lock);
	if (!(FLP_AR_DATA & ar_port->channel_type) || !(FLP_AR_DATA & g_ar_dev.service_type)) {
		ctxhub_err("hismart:[%s]:line[%d] ar_parking_info_cmd[%x][%x]\n", __func__, __LINE__,
				ar_port->channel_type, g_ar_dev.service_type);
		ret =  -EIO;
		goto AR_PARKING_INFO_ERR;
	}

	if (copy_from_user((void *)&parking_cmd, (void __user *)((uintptr_t)arg), sizeof(struct context_ar_parking_info))) {
		ctxhub_err("hismart:[%s]:line[%d] parking_cmd copy_from_user error\n", __func__, __LINE__);
		ret =  -EFAULT;
		goto AR_PARKING_INFO_ERR;
	}

	ctxhub_info("hismart:[%s]:line[%d] parking_cmd[%u]\n interval %lld args %lld", __func__, __LINE__, parking_cmd.cmd, parking_cmd.interval, parking_cmd.args);

	ar_send_cmd_from_kernel(TAG_AR, CMD_CMN_CONFIG_REQ, SUB_CMD_FLP_AR_PARKING_CMD_REQ,
								(char *)&parking_cmd, sizeof(parking_cmd));
...
static int flp_ar_ioctl(struct ar_port_t *ar_port, unsigned int cmd, unsigned long arg)
{
	...
	case flp_ioctl_ar_state_v2(0):
		...
		break;
	case flp_ioctl_ar_parking_info(0):
		ret = ar_parking_info_cmd(ar_port, arg);
		break;
	default:
...
static int __init ar_init(void)
{
	...
	if (g_dts_status == SENSORHUB_DTS_ENABLED) {
		...
		register_iom3_recovery_notifier(REC_USR_AR, &ar_reboot_notify);
	}
	return ret;
