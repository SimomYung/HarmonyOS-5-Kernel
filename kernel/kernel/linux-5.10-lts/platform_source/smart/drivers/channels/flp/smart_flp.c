...
static void flp_resort_cellbatching_data(char *data, unsigned int len)
{
	...
	if (data == NULL || len == 0 || (len % sizeof(struct cell_batching_data_t)) != 0) {
		pr_err("flp:%s invalid data len[%u]\n", __func__, len);
		return;
	}
...
static int flp_open_send_status_flp(struct flp_port_t *flp_port)
{
	...
	pr_info("[%s]: channel_type=%u\n", __func__, flp_port->channel_type);
	if (!(flp_port->channel_type & IOMCU_APP_FLP)) {
		pr_info("[%s]: strat open flp!\n", __func__);
...
static int flp_get_safe_location(struct flp_port_t *flp_port, unsigned long arg)
{
	int ret;
	int mode;

	if (get_user(mode, (int __user *)((uintptr_t)arg)))
		return -EFAULT;
	...
	pr_info("flp:[%s] config mode is [%d].\n", __func__, mode);
	ret = inputhub_wrapper_pack_and_send_cmd(TAG_FLP, CMD_CMN_CONFIG_REQ, SUB_CMD_FLP_GET_SAFE_LOCATION_REQ,
		(char *)&mode, sizeof(mode), NULL);
	if (ret != 0) {
		pr_err("flp:[%s] send cmd failed. ret is [%d].\n", __func__, ret);
		return ret;
	}
	if (mode == 1) {
		flp_port->channel_type |= FLP_SAFE_LOCATION;
		g_flp_dev.service_type |= FLP_SAFE_LOCATION;
	} else if (mode == 0) {
		flp_port->channel_type &= ~FLP_SAFE_LOCATION;
		g_flp_dev.service_type &= ~FLP_SAFE_LOCATION;
	} else {
		pr_err("flp:[%s]unsupport mode[%d]\n", __func__, mode);
		return -EPERM;
	}
	return 0;
}
...
static int flp_geofence_ioctl(struct flp_port_t *flp_port, unsigned int cmd, unsigned long arg)
{
	...
	case FD_IOCTL_GEOFENCE_GET_SIZE:
		...
		break;
	case FD_IOCTL_GET_SAFE_LOCATION:
		ret = flp_get_safe_location(flp_port, arg);
		break;
	default:
...
static int flp_diag_send_cmd(struct flp_port_t *flp_port, unsigned long arg)
{
	...
	if (data_len > FLP_DIAG_MAX_CMD_LEN || !data_len) {
		pr_err("[flperr][%s] invalid data_len=%u\n", __func__, data_len);
		return -EFAULT;
	}
...
static int flp_mmap(struct file *filp, struct vm_area_struct *vma)
{
	...
	if (filp == NULL || vma == NULL) {
		pr_err("flp_mmap param is error\n");
		return -EINVAL;
	}
...
int flp_register(void)
{
	...
	register_iom3_recovery_notifier(REC_USR_FLP, &sensor_reboot_notify);
