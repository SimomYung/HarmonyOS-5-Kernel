...
STATIC void ao_loadmonitor_exit(void)
{
	(void)unregister_mcu_event_notifier(TAG_LOADMONITOR, CMD_READ_AO_MONITOR_SENSOR_RESP, ao_loadmonitor_data_from_mcu);
}
...
int32_t _ao_loadmonitor_read(void *data, uint32_t len)
{
	...
	if (get_contexthub_dts_status() != 0) {
		...
		if (len < sizeof(struct loadmonitor_sigs)) {
			pr_err("[%s], data len is too small %u\n", __func__, len);
			return -EINVAL;
		}
...
late_initcall(ao_loadmonitor_init);
module_exit(ao_loadmonitor_exit);