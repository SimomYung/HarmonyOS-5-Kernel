...
#ifdef CONFIG_SECURITY_HEADER_FILE_REPLACE
struct sdc_sensor_info_map g_sdc_sensor_map[] = {
	{ TAG_ACCEL, 0, 0, 0 },
	{ TAG_MAG, 0, 0, 0 },
...
#ifdef CONFIG_SENSOR_DIRECT_CHANNEL_EXPAND
	{ TAG_GRAVITY, 0, 0, 0 },
	{ TAG_MAG_UNCALIBRATED, 0, 0, 0 },
	{ TAG_GEOMAGNETIC_RV, 0, 0, 0 },
	{ TAG_GYRO_UNCALIBRATED, 0, 0, 0 },
	{ TAG_ACCEL_UNCALIBRATED, 0, 0, 0 },
	{ TAG_ACC1, 0, 0, 0 },
	{ TAG_GYRO1, 0, 0, 0 },
	{ TAG_ACC2, 0, 0, 0 },
	{ TAG_GYRO2, 0, 0, 0 },
#endif
#ifdef CONFIG_SENSOR_DIRECT_CHANNEL_EXPAND_V1
	{ TAG_GAME_RV, 0, 0, 0 },
	{ TAG_PRESSURE, 0, 0, 0 },
#endif
};
...
#ifdef CONFIG_SENSOR_DIRECT_CHANNEL_EXPAND
	{ TAG_GRAVITY, DDR_SHMEM_SDC_GRAVITY_SIZE, DDR_SHMEM_SDC_GRAVITY_ADDR_AP, 0 },
	{ TAG_MAG_UNCALIBRATED, DDR_SHMEM_SDC_MAG_UNCALI_SIZE, DDR_SHMEM_SDC_MAG_UNCALI_ADDR_AP, 0 },
	{ TAG_GEOMAGNETIC_RV, DDR_SHMEM_SDC_GEOMAGNETIC_RV_SIZE, DDR_SHMEM_SDC_GEOMAGNETIC_RV_ADDR_AP, 0 },
	{ TAG_GYRO_UNCALIBRATED, DDR_SHMEM_SDC_GYRO_UNCALI_SIZE, DDR_SHMEM_SDC_GYRO_UNCALI_ADDR_AP, 0 },
	{ TAG_ACCEL_UNCALIBRATED, DDR_SHMEM_SDC_ACC_UNCALI_SIZE, DDR_SHMEM_SDC_ACC_UNCALI_ADDR_AP, 0 },
	{ TAG_ACC1, DDR_SHMEM_SDC_ACC1_SIZE, DDR_SHMEM_SDC_ACC1_ADDR_AP, 0 },
	{ TAG_GYRO1, DDR_SHMEM_SDC_GYRO1_SIZE, DDR_SHMEM_SDC_GYRO1_ADDR_AP, 0 },
	{ TAG_ACC2, DDR_SHMEM_SDC_ACC2_SIZE, DDR_SHMEM_SDC_ACC2_ADDR_AP, 0 },
	{ TAG_GYRO2, DDR_SHMEM_SDC_GYRO2_SIZE, DDR_SHMEM_SDC_GYRO2_ADDR_AP, 0 },
#endif
#ifdef CONFIG_SENSOR_DIRECT_CHANNEL_EXPAND_V1
	{ TAG_GAME_RV, DDR_SHMEM_SDC_GAME_RV_SIZE, DDR_SHMEM_SDC_GAME_RV_ADDR_AP, 0 },
	{ TAG_PRESSURE, DDR_SHMEM_SDC_PRESSURE_SIZE, DDR_SHMEM_SDC_PRESSURE_ADDR_AP, 0 },
#endif
};
#endif
...
static const struct sensors_cmd_map sensors_cmd_map_tab[] = {
	...
	{ SDC_SENSOR_TYPE_ORIENTATION, TAG_ORIENTATION },
	{ SDC_SENSOR_TYPE_ROTATION_VECTOR, TAG_ROTATION_VECTORS },
#ifdef CONFIG_SENSOR_DIRECT_CHANNEL_EXPAND
	{ SDC_SENSOR_TYPE_GRAVITY, TAG_GRAVITY },
	{ SDC_SENSOR_TYPE_MAGNETIC_FIELD_UNCALIBRATED, TAG_MAG_UNCALIBRATED },
	{ SDC_SENSOR_TYPE_GEOMAGNETIC_ROTATION_VECTOR, TAG_GEOMAGNETIC_RV },
	{ SDC_SENSOR_TYPE_GYROSCOPE_UNCALIBRATED, TAG_GYRO_UNCALIBRATED },
	{ SDC_SENSOR_TYPE_ACCELEROMETER_UNCALIBRATED, TAG_ACCEL_UNCALIBRATED },
	{ SDC_SENSOR_TYPE_ACCELEROMETER1, TAG_ACC1 },
	{ SDC_SENSOR_TYPE_GYROSCOPE1, TAG_GYRO1 },
	{ SDC_SENSOR_TYPE_ACCELEROMETER2, TAG_ACC2 },
	{ SDC_SENSOR_TYPE_GYROSCOPE2, TAG_GYRO2 },
#endif
#ifdef CONFIG_SENSOR_DIRECT_CHANNEL_EXPAND_V1
	{ SDC_SENSOR_TYPE_GAME_RV, TAG_GAME_RV },
	{ SDC_SENSOR_TYPE_PRESSURE, TAG_PRESSURE },
#endif
};
...
static int send_sensor_sdc_enable_cmd(int tag, int enable, int mode, bool is_lock)
{
	...
	if (sdc_map->enabled == 1 && mode == BOOT_MODE) {
		pr_info("%s, sensor tag %d sdc double open\n", __func__, tag);
		return 0;
	}
	...
	if (ret != 0 || rd.errno != 0) {
		pr_err("sensor[%d] sdc enable[%d] fail, ret[%d, %d]\n", tag, enable, ret, rd.errno);
		return -EIO;
	}
...
static int sdc_sensor_recover(int tag)
{
	...
	ret = send_sensor_sdc_enable_cmd(tag, 1, RECOVERY_MODE, false);
	if (ret != 0) {
...
static bool is_support_sdc_sensor(int32_t sdc_type)
{
	size_t num = ARRAY_SIZE(sensors_cmd_map_tab);
	size_t i;

	for (i = 0; i < num; i++) {
		if (sdc_type == sensors_cmd_map_tab[i].sdc_sensor_type)
			return true;
	}
	return false;
}
...
static int send_sensor_cmd(unsigned int cmd, unsigned long arg)
{
	...
	if (!is_support_sdc_sensor(para.sdc_type)) {
		pr_err("%s, not suppported sdc sensor type %d\n", __func__, para.sdc_type);
		return -EINVAL;
	}
	...
	case SDC_IOCTL_APP_ENABLE:
		ret = send_sensor_sdc_enable_cmd(tag, 1, BOOT_MODE, true);
		if (ret != 0) {
	...
	case SDC_IOCTL_APP_GET_OFFSET:
		...
		break;
	default:
		pr_err("unknown shb_hal_cmd %u in %s\n", cmd, __func__);
		return -EINVAL;
	}
...
static int sdc_mmap(struct file *filp, struct vm_area_struct *vma)
{
	...
	if (size != sdc_map->size) {
		pr_err("%s: invalid sensor[%d] buffer size[0x%x]\n", __func__, tag, size);
		return -EINVAL;
	}
...
/* get sensorhub  base  for sdc */
	...
	register_iom3_recovery_notifier(REC_USR_SENSOR, &sdc_recovery_notify);
#ifdef CONFIG_HIBERNATION
