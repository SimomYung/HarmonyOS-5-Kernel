...
static int32_t omxvdec_release(struct inode *inode, struct file *file)
{
	...
	if (omxvdec_is_heif_scene(&g_vdec_map_info[index])) {
 	...
 	if (g_vdec_map_info[index].lock_flag) {
		g_vdec_map_info[index].lock_flag = false;
...
static long handle_lock_hw(const struct file *file, unsigned int cmd, omx_vdec_entry *omxvdec)
{
	vcodec_bool x_scene;
	int32_t index;
	...
	g_vdec_map_info[index].lock_flag = true;
	return 0;
}

static long handle_unlock_hw(const struct file *file, unsigned int cmd, omx_vdec_entry *omxvdec)
{
	vcodec_bool x_scene;
	int32_t index;
	index = vdec_get_file_index(file);
	...
	unsigned long arg, compat_type_e type)
{
	...
	case VDEC_IOCTL_LOCK_HW:
		ret = handle_lock_hw(file, cmd, omxvdec);
		break;
	case VDEC_IOCTL_UNLOCK_HW:
		ret = handle_unlock_hw(file, cmd, omxvdec);
		break;
		/*lint +e454 +e456 +e455*/
	default:
...
static int32_t omxvdec_resume(struct platform_device *pltdev)
{
	...
	if (g_is_normal_init != 0) {
		...
		if (ret != 0) {
			(void)vdec_regulator_disable();
			dprint(PRN_FATAL, "%s, vctrl resume failed\n", __func__);
			return VCODEC_FAILURE;
		}
	}
