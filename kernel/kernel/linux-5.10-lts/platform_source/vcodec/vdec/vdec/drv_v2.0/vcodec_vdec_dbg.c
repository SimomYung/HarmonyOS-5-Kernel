...
static ssize_t vdec_fill_setting_option(const struct vdec_setting_option *option, char *buf, ssize_t len)
{
	...
	switch (option->type) {
	case SETTING_OPT_TYPE_BOOL:
		ret = sprintf_s(buf, (size_t)len, " (default %s)\n",
			option->default_value.enable ? "true" : "false");
		break;
	case SETTING_OPT_TYPE_INT:
		ret = sprintf_s(buf, (size_t)len, " (from %d to %d) (default %d)\n",
			(int32_t)(int64_t)option->min_value, (int32_t)(int64_t)option->max_value, option->default_value.i32);
		break;
	case SETTING_OPT_TYPE_UINT:
		ret = sprintf_s(buf, (size_t)len, " (from %u to %u) (default %u)\n",
			(uint32_t)option->min_value, (uint32_t)option->max_value, (uint32_t)option->default_value.i32);
		break;
	case SETTING_OPT_TYPE_INT64:
		ret = sprintf_s(buf, (size_t)len, " (from %lld to %lld) (default %lld)\n",
			(int64_t)option->min_value, (int64_t)option->max_value, (int64_t)option->default_value.i64);
		break;
	case SETTING_OPT_TYPE_UINT64:
		ret = sprintf_s(buf, (size_t)len, " (from %llu to %llu) (default %llu)\n",
			option->min_value, option->max_value, option->default_value.i64);
		break;
	case SETTING_OPT_TYPE_STRING:
		ret = sprintf_s(buf, (size_t)len, " (default %s)\n",
...
static ssize_t vdec_show_setting_option(char *buf, ssize_t start_pos, ssize_t buf_capacity)
{
	...
	ret = sprintf_s(buf + filled_len, (size_t)(buf_capacity - filled_len), "Setting option:\n");
	if (ret < 0)
		return filled_len;
	...
	for (i = 0; i < ARRAY_SIZE(setting_options); i++) {
		const struct vdec_setting_option *option = &setting_options[i];
		ret = sprintf_s(buf + filled_len, (size_t)(buf_capacity - filled_len), "  %-40s", option->name);
		if (ret < 0)
			return filled_len;
		...
		for (j = 0; j < ARRAY_SIZE(vdec_setting_option_type_to_string_table); j++) {
			...
			if (option->type == map_info->type) {
				ret = sprintf_s(buf + filled_len, (size_t)(buf_capacity - filled_len), "%-12s %s", map_info->str, option->help);
				if (ret < 0)
					return filled_len;
...
static ssize_t vdec_show_setting_usage(char *buf, ssize_t start_pos, ssize_t buf_capacity)
{
	...
	ret = sprintf_s(buf + filled_len, (size_t)(buf_capacity - filled_len), "\n\nUsage:\n");
	if (ret < 0)
		return filled_len;
	...
	ret = sprintf_s(buf + filled_len, (size_t)(buf_capacity - filled_len),
					"  echo key1=value1:key2=value2[: ...] > vdec_setting\n");
	if (ret < 0)
		return filled_len;
	...
	const struct vdec_setting_info *setting_info)
{
	...
	for (i = 0; i < ARRAY_SIZE(setting_options); i++) {
		...
		ret = sprintf_s(buf + filled_len, (size_t)(buf_capacity - filled_len), "  %s=", option->name);
		if (ret < 0)
			return filled_len;
		...
		switch (option->type) {
		case SETTING_OPT_TYPE_BOOL:
			ret = sprintf_s(buf + filled_len, (size_t)(buf_capacity - filled_len), "%d\n", *(bool *)(target_addr));
			break;
		case SETTING_OPT_TYPE_INT:
			ret = sprintf_s(buf + filled_len, (size_t)(buf_capacity - filled_len), "%d\n", *(int *)(target_addr));
			break;
		case SETTING_OPT_TYPE_UINT:
			ret = sprintf_s(buf + filled_len, (size_t)(buf_capacity - filled_len), "%u\n", *(unsigned int *)(target_addr));
			break;
		case SETTING_OPT_TYPE_INT64:
			ret = sprintf_s(buf + filled_len, (size_t)(buf_capacity - filled_len), "%lld\n", *(long long *)(target_addr));
			break;
		case SETTING_OPT_TYPE_UINT64:
			ret = sprintf_s(buf + filled_len, (size_t)(buf_capacity - filled_len), "%llu\n",
							*(unsigned long long *)(target_addr));
			break;
		case SETTING_OPT_TYPE_STRING:
			ret = sprintf_s(buf + filled_len, (size_t)(buf_capacity - filled_len), "%s\n", target_addr);
			break;
		default:
			ret = 0;
			break;
...
ssize_t vdec_show_setting_info(char *buf, const size_t count, const struct vdec_setting_info *setting_info)
{
	...
	filled_len = vdec_show_setting_option(buf, filled_len, (ssize_t)count);

	filled_len = vdec_show_setting_usage(buf, filled_len, (ssize_t)count);

	filled_len = vdec_show_setting_value(buf, filled_len, (ssize_t)count, setting_info);
	...
	long long value)
{
	if (value > option->max_value || value < option->min_value) {
		dprint(PRN_ERROR, "Invalid value(%lld) is out of range [%llu, %llu].", value, option->min_value, option->max_value);
		return -1;
	}
...
static void vdec_store_setting_value(const void *start_addr, const char *key, const int key_len, const char *value)
{
	...
	for (i = 0; i < ARRAY_SIZE(setting_options); i++) {
		...
		if (strlen(option->name) == key_len &&
			!strncmp(key, option->name, (size_t)key_len)) {
			switch (option->type) {
			case SETTING_OPT_TYPE_BOOL:
			case SETTING_OPT_TYPE_INT:
			case SETTING_OPT_TYPE_UINT:
