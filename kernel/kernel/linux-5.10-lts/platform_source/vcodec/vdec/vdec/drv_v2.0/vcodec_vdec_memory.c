...
void *vdec_mem_kernel_map(struct dma_buf *dma_buf)
{
	int32_t ret = 0;
	void *virt_addr = NULL;
	vdec_mutex_lock(&g_mem_mutex);
	...
	if (ret) {
		vdec_mutex_unlock(&g_mem_mutex);
		dprint(PRN_ERROR, "dma_buf_begin_cpu_access failed\n");
		return NULL;
	}

	virt_addr = dma_buf_kmap(dma_buf, 0);
	vdec_mutex_unlock(&g_mem_mutex);
	if (!virt_addr) {
		dprint(PRN_FATAL, "kmap failed\n");
		(void)dma_buf_end_cpu_access(dma_buf, DMA_TO_DEVICE);
	}
	return virt_addr;
}

void vdec_mem_kernel_unmap(struct dma_buf *dma_buf)
{
	int32_t ret = 0;
	...
	vdec_mutex_lock(&g_mem_mutex);
	dma_buf_kunmap(dma_buf, 0, NULL);
	ret = dma_buf_end_cpu_access(dma_buf, DMA_TO_DEVICE);
	if (ret)
		dprint(PRN_ERROR, "end cpu access failed\n");
	vdec_mutex_unlock(&g_mem_mutex);
}
#endif