	...
	struct file *file_handle, struct dma_buf **dmabuf)
{
	...
	*dmabuf = dma_buf_get(buf_record->share_fd);
	...
	if (!iova_addr) {
		dprint(PRN_FATAL, "map iova failed, fd:%d", buf_record->share_fd);
		dma_buf_put(*dmabuf);
		vdec_mutex_unlock(&g_mem_mutex);
		return -EFAULT;
	}
	...
	vdec_mutex_unlock(&g_mem_mutex);
	dprint(PRN_DBG, "share_fd:%d, size:%u, virt_addr:%pK", buf_record->share_fd, buf_record->iova_size, buf_record->vir_addr);
	return 0;
}
