...
static int32_t vdec_plat_switch_clk(uint32_t dst_clk_level, uint32_t dst_clk_rate, uint32_t dst_pxpc_clk_rate)
{
	...
	int32_t ret = 0;
	uint32_t current_pxpc_clk_rate;
	vdec_plat *plt = vdec_plat_get_entry();
	...
	if (dst_pxpc_clk_rate == current_pxpc_clk_rate) {
		return 0;
	}
...
int32_t power_up_vdec_cal(void)
{
	...
	if (!ops->is_need_power_up_vdec_cal())
		return 0;
	...
	// v800 need config enable, v900 default enable
...
int32_t vdec_plat_regulator_set_clk_rate(uint32_t dst_clk_level)
{
	...
	dprint(PRN_ALWS, "set dst_pxpc_clk_rate value: %u", dst_pxpc_clk_rate);
