/*
 * vcodec_vdec_refulator.h
 *
 * This is for vdec regulator
 *
 * Copyright (c) 2019-2020 Huawei Technologies CO., Ltd.
 *
 * This software is licensed under the terms of the GNU General Public
 * License version 2, as published by the Free Software Foundation, and
 * may be copied, distributed, and modified under those terms.
 *
 * This program is distributed in the hope that it will be useful,
 * but WITHOUT ANY WARRANTY; without even the implied warranty of
 * MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the
 * GNU General Public License for more details.
 *
 */

#ifndef VCODEC_VDEC_REGULATOR_H
#define VCODEC_VDEC_REGULATOR_H

#include <linux/clk.h>
#include <linux/of.h>
#include <linux/of_irq.h>
#include <linux/of_address.h>
#include <linux/clk.h>
#include <linux/regulator/consumer.h>
#include <linux/mutex.h>
#include <linux/platform_device.h>
#include "vcodec_types.h"
#include "vfmw_ext.h"
#include "smmu_regs.h"

#define VDEC_CLK_LOAD_LIMIT_MAX (VDEC_CLK_RATE_LEVEL_MAX - 1)

enum {
	MEDIA_REGULATOR,
	VDEC_REGULATOR,
	SMMU_TCU_REGULATOR,
	MAX_REGULATOR,
};

typedef struct {
	struct clk *clk_vdec;
	struct clk *clk_vdec_pxpc;
	struct regulator *regulators[MAX_REGULATOR];
	// vdec_clk and pxpc_clk always work on same voltage
	uint32_t clk_values[VDEC_CLK_RATE_LEVEL_MAX];
	uint32_t pxpc_clk_values[VDEC_CLK_RATE_LEVEL_MAX];
	uint64_t clk_load_limit_values[VDEC_CLK_LOAD_LIMIT_MAX];
	uint32_t default_clk_rate;
} vdec_regulator;

typedef struct {
	uint64_t load;
	uint32_t base_clk_level;
	uint32_t is_release;
	int32_t chan_id;
	int32_t is_back_ground;
} performance_params_s;

typedef struct {
	/* The current frequency may be invalid when a power-off operation has been performed.
	   The frequency needs to be reset. */
	int32_t clk_flag;
	uint32_t current_clk_level; /* Current decoder frequency */
	uint32_t dynamic_clk_level; /* Set by the performance node. (/sys/class/vdec_class/hi_vdec/vdec_freq) */
	uint32_t static_clk_level;  /* Set this parameter based on the load in user mode. */
	uint32_t open_in_high_clk;  /* Power up in highest clk level. (/proc/vcodec_vdec) */
} vdec_clk_ctrl;

typedef struct {
	struct mutex vdec_plat_mutex;
	vdec_regulator regulator_info;
	vdec_dts dts_info;
	int32_t power_flag;
	int32_t power_vdec_cal_flag;
	int32_t plt_init;
	vdec_clk_ctrl clk_ctrl;
	struct smmu_tbu_info smmu_info;
	struct device *dev;
} vdec_plat;

vdec_plat *vdec_plat_get_entry(void);

int32_t vdec_plat_init(struct platform_device *plt_dev);
int32_t vdec_plat_deinit(void);

int32_t power_up_vdec_top(void);
void power_down_vdec_top_mcu(void);

int32_t power_up_vdec_cal(void);
void  power_down_vdec_cal(bool retention_img);

void vdec_plat_set_dynamic_clk_rate(uint32_t clk_level);
void vdec_plat_get_dynamic_clk_rate(uint32_t *clk_level);
void vdec_plat_set_static_clk_rate(uint32_t clk_level);
void vdec_plat_get_static_clk_rate(uint32_t *clk_level);
void vdec_plat_get_target_clk_rate(uint32_t *clk_level);
int32_t vdec_plat_regulator_set_clk_rate(uint32_t dst_clk_level);

uint32_t vdec_plat_is_es(void);

#endif

