...
static int32_t smmu_init_reg(struct smmu_entry *entry)
{
	...
	for (reg_index = 0; reg_index < tbu_num; reg_index++) {
		...
		dprint(PRN_DBG, "reg_index %u tbu and tcu connect success\n", reg_index);
...
static void smmu_deinit_reg(struct smmu_entry *entry)
{
	...
	for (tbu_index = 0; tbu_index < tbu_num; tbu_index++) {
		...
		if (ret != SMMU_OK)
			...
			dprint(PRN_ALWS, "reg_index %u tbu and tcu disconnect success\n", tbu_index);
	}
...
int32_t smmu_connect(void)
{
	struct smmu_entry *entry = NULL;

	entry = smmu_get_entry();
	smmu_init_entry(entry);
	...
	entry->smmu_init = 1;
	dprint(PRN_ALWS, "smmu connect ok");
	return SMMU_OK;
}

void smmu_disconnect(void)
{
	struct smmu_entry *entry = NULL;

	entry = smmu_get_entry();
	if (entry->smmu_init != 1) {
		dprint(PRN_ERROR, "smmu not connect");
		return;
	}
