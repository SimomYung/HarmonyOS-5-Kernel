...
static uint32_t get_clk_rate(void)
{
	...
	clk_level_by_loads = VDEC_CLK_RATE_LEVEL_MAX - 1;
	for (i = 0; i < VDEC_CLK_LOAD_LIMIT_MAX; i++) {
		if (total_load <= regulator_info->clk_load_limit_values[i]) {
			clk_level_by_loads = i;
			break;
		}
	}

	clk_level = max_base_clk_level > clk_level_by_loads ? max_base_clk_level : clk_level_by_loads;

	dprint(PRN_ALWS, "chan_num is %u total_load %llu, clk_level_by_loads %u, max_base_clk_level %u, clk_level %u",
		chan_num, total_load, clk_level_by_loads, max_base_clk_level, clk_level);
...
bool vdec_chan_list_get(struct vdec_chan_list *list, uint32_t chanid)
{
	...
	if (!list) {
		dprint(PRN_FATAL, "get chan list nullptr");
		return false;
	}
	...
	list_for_each_entry(pos, &list->chan_head, head) {
		if (pos->chan_id == (int32_t)chanid) {
			if (pos->ref_num == 0) {
				dprint(PRN_ERROR, "The chan %u is not exist error", chanid);
				goto error;
			}
			if (pos->ref_num == CHAN_REF_COUNT_MAX) {
				dprint(PRN_FATAL, "ref num exceed 8 error");
				goto error;
			}
			pos->ref_num++;
...
bool vdec_chan_list_destroy(struct vdec_chan_list *list, uint32_t chanid)
{
	...
	if (!list) {
		dprint(PRN_FATAL, "destroy chan list nullptr");
		return false;
	}
	...
	list_for_each_entry(pos, &list->chan_head, head) {
		if (pos->chan_id == (int32_t)chanid) {
			dprint(PRN_ALWS, "destroy chan %u", chanid);
			while(pos->ref_num != 1) {
				reinit_completion(&pos->chan_ref_num_equal_one);
				mutex_unlock(&list->lock);
				timeout = wait_for_completion_timeout(&pos->chan_ref_num_equal_one, msecs_to_jiffies(1000));
				if (timeout == 0) {
					dprint(PRN_FATAL, "error: wait chan ref num equal one timeout");
					return false;
				}
				mutex_lock(&list->lock);
				if (pos->ref_num == 1) {
...
void vdec_chan_list_put(struct vdec_chan_list *list, uint32_t chanid)
{
	struct vdec_chan_listnode *pos = NULL;
	if (!list) {
		dprint(PRN_FATAL, "put chan list nullptr");
		return;
	}
	...
	list_for_each_entry(pos, &list->chan_head, head) {
		if (pos->chan_id == (int32_t)chanid) {
			pos->ref_num--;
			if (pos->ref_num == 1) {
				dprint(PRN_CTRL, "this channel chanid ref is 1 can destory");
				complete(&pos->chan_ref_num_equal_one);
			} else {
				dprint(PRN_CTRL, "this channel chanid ref not equal 1 can not destroy");
			}
			mutex_unlock(&list->lock);
			return;
		}
...
int32_t vdec_chan_list_add(struct vdec_chan_list *list, int32_t chan_id)
{
	...
	if (!list) {
		dprint(PRN_FATAL, "add chan list nullptr");
		return -EFAULT;
	}
	...
	node->ref_num = 1;
	init_completion(&node->chan_ref_num_equal_one);
...
int32_t vdec_chan_list_remove(struct vdec_chan_list *list, int32_t chan_id)
{
	...
	if (!list) {
		dprint(PRN_FATAL, "remove chan list nullptr");
		return -EFAULT;
	}
...
static void vdec_init_all_file_handle(void)
{
	...
	for (instance_id = 0; instance_id < MAX_OPEN_COUNT; instance_id++) {
		...
		mutex_init(&g_file_handle[instance_id].chan_create_lock);
	}
}
...
static void vdec_close_notify_mcu_destroy(struct dec_file_handle *handle)
{
	...
	list_for_each_entry_safe(chan_pos, chan_node, &chan_list->chan_head, head) {
		...
		// get the segbuf iova address from MCU
		msg.output.iova = 0;
		...
		if (!ret && chan_pos->chan_state == VFMW_CHAN_STATE_CREATE) {
			...
			if (!ret && !msg.output.res && msg.output.iova) {
				if (vdec_free_segbuffer(msg.output.iova))
					dprint(PRN_ERROR, "free seg buffer of chan_id:%d failed", msg.chan_id);
			} else {
				dprint(PRN_ERROR, "mcu destroy chan:%d failed %d %d", msg.chan_id, ret, msg.output.res);
			}
		}
		chan_pos->chan_id = -1;
		chan_pos->chan_state = VFMW_CHAN_STATE_NULL;
...
static void vdec_recv_event_notify(enum vcodec_dev_event_notify event, void * msg)
{
	...
	case VCODEC_DEV_MCU_RESET_GLOBAL:
		...
		break;
	case VCODEC_DEV_MCU_GET_MEDIA1_CRG_INFO:
		ops->get_media1_crg(vdec_get_entry()->media1_crg_vir_addr);
		break;
	default:
		return;
	}
...
static int32_t vdec_close_bsp_pxp(void *arg)
{
	vdec_entry *vdec = (vdec_entry *)arg;
...
static void vdec_sync_open_failed_process(void)
{
	vdec_entry* vdec = vdec_get_entry();
	void *work_node = NULL;

	// no process needed when async open failed
	if (vcodec_work_finish(vdec->async_open_node))
		return;

	// close bsp/pxp when async open success
	work_node = vcodec_work_submit(vdec->work_ctx, vdec_close_bsp_pxp, vdec);
	if (work_node)
		vcodec_work_finish(work_node);
	else
		dprint(PRN_FATAL, "submit close bsp and pxp device work fail");
}
...
static int32_t vdec_close(struct inode *inode, struct file *file_handle)
{
	...
	if (vdec_file->hw_lock_flag) {
		...
		if (ret) {
			dprint(PRN_ERROR, "unlock hw failed");
		}
		vdec->device_locked = VCODEC_FALSE;
	...
	vdec_ioctl_msg *pvdec_msg)
{
	...
	dprint(PRN_CTRL, "start VDEC_IOCTL_SET_CLK_RATE");
	...
	vdec_mutex_unlock(&g_vdec_entry.vdec_mutex);
	dprint(PRN_CTRL, "end VDEC_IOCTL_SET_CLK_RATE");
