...
#define PSCI_MSG_TYPE_AP_VDEC_IY IPC_CMD(OBJ_AP, OBJ_LPM3, CMD_INQUIRY, 9)
/*
...
void vdec_write_nvme(int state)
{
	int ret;
	void *node = NULL;
...
void vdec_sh_init(void)
{
	...
	if (is_sh_vdec_exc()) {
		dprint(PRN_ALWS, "vdec has qice exception, set sh flag");
		vdec_write_nvme(SH_PV_ON);
		bbox_diaginfo_record(DMD_SOC_VDEC_SH_CNT, NULL, "vdec qice exception");
	} else {
		dprint(PRN_ALWS, "vdec no qice exception");
	}
...
static int32_t vdec_read_iy_nvme_task(void *arg)
{
	...
	if (iy_vdec_info == NULL) {
		dprint(PRN_ERROR, "vdec info is NULL");
		return -EINVAL;
	}
	...
	dprint(PRN_ALWS, "vdec read iy nvme info nv_flag:0x%x", iy_vdec_info->nv_flag.value);
	dprint(PRN_ALWS, "vdec read iy nvme info error_flag:0x%x", iy_vdec_info->error_flag.value);
	dprint(PRN_ALWS, "vdec read iy nvme info reserved:0x%x", iy_vdec_info->reserved.value);
	return 0;
}
...
void vdec_init_iy_nvme(void)
{
	...
	if (iy_vdec_info.nv_flag.bits.pv_flag == 1) {
