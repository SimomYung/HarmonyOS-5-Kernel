...
SINT32 scd_drv_check_cfg_address(scd_config_reg_s *sm_ctrl_reg, mem_buffer_s *scd_mem_map)
{
	...
	for (i = SCD_SHAREFD_MESSAGE_POOL; i < max_scd_buf_num; i++) {
		if ((scd_mem_map[i].share_fd == INVALID_SHAREFD) ||
			...
			dprint(PRN_FATAL, "%s parameter error: buffer no map. index: %d, isMapped: %u, shareFd: %d\n",
...
{
	g_scd_state_reg.scd_protocol = (SINT32)(rd_scd_reg(REG_SCD_PROTOCOL));
	g_scd_state_reg.scd_over     = (SINT32)(rd_scd_reg(REG_SCD_OVER));

	g_scd_state_reg.scd_num      = (SINT32)(rd_scd_reg(REG_SCD_NUM));
	g_scd_state_reg.scd_roll_addr = patch_address((rd_scd_reg(REG_ROLL_ADDR_MSB)),
		(rd_scd_reg(REG_ROLL_ADDR)));
	g_scd_state_reg.src_eaten    = (SINT32)(rd_scd_reg(REG_SRC_EATEN));
	g_scd_state_reg.up_len       = (SINT32)(rd_scd_reg(REG_UP_LEN));
}
...
static void print_scd_vtrl_reg(void)
{
	...
	sm_ctrl_reg.up_len          = (SINT32)(rd_scd_reg(REG_UP_LEN));
