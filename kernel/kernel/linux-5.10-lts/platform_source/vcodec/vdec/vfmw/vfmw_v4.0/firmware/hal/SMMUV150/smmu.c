...
#define VCODEC_VDEC_SMMU_COMMON_OFFSET       0x20000
#define VCODEC_VDEC_SMMU_MASTER_OFFSET       0xF000

#define VCODEC_VDEC_SMMU_COMMON_BASE_ADDR \
	(g_vdh_reg_base_addr + VCODEC_VDEC_SMMU_COMMON_OFFSET)
#define VCODEC_VDEC_SMMU_MASTER_BASE_ADDR \
	(g_vdh_reg_base_addr + VCODEC_VDEC_SMMU_MASTER_OFFSET)
...
static void set_common_reg(UADDR addr, SINT32 val, SINT32 bw, SINT32 bs)
{
	...
	rd_smmu_common_vreg(addr, tmp_value);
	tmp_value &= ~(mask << bs); /*lint !e502*/
	wr_smmu_common_vreg(addr, tmp_value | ((val & mask) << bs));
}
...
static void set_master_reg(UADDR addr, SINT32 val, SINT32 bw, SINT32 bs)
{
	...
	rd_smmu_master_vreg(addr, tmp_value);
	tmp_value &= ~(mask << bs); /*lint !e502*/
	wr_smmu_master_vreg(addr, (tmp_value | ((val & mask) << bs)));
}
	...
	SINT32 val, SINT32 bw, SINT32 bs)
{
	...
	switch (master_type) {
	case MFDE:
		rd_smmu_mfde_vreg(addr, tmp_value);
		tmp_value &= ~(mask << bs); /*lint !e502*/
		wr_smmu_mfde_vreg(addr, tmp_value | ((val & mask) << bs));
		break;
	case SCD:
		rd_smmu_scd_vred(addr, tmp_value);
		tmp_value &= ~(mask << bs); /*lint !e502*/
		wr_smmu_scd_vreg(addr, tmp_value | ((val & mask) << bs));
		break;
	default:
		break;
	}
...
void smmu_int_serv_proc(void)
{
	...
	rd_smmu_common_vreg(SMMU_INTSTAT_NS, tmp_value);
	dprint(PRN_ALWS, "SMMU_INTSTAT_NS : 0x%x\n", tmp_value);
	rd_smmu_master_vreg(SMMU_MSTR_DBG_0, tmp_value);
	dprint(PRN_ALWS, "SMMU_MSTR_DBG_0 : 0x%x\n", tmp_value);
	rd_smmu_master_vreg(SMMU_MSTR_DBG_1, tmp_value);
	dprint(PRN_ALWS, "SMMU_MSTR_DBG_1 : 0x%x\n", tmp_value);
	rd_smmu_master_vreg(SMMU_MSTR_DBG_2, tmp_value);
	dprint(PRN_ALWS, "SMMU_MSTR_DBG_2 : 0x%x\n", tmp_value);
	rd_smmu_master_vreg(SMMU_MSTR_DBG_3, tmp_value);
	dprint(PRN_ALWS, "SMMU_MSTR_DBG_3 : 0x%x\n", tmp_value);
	rd_smmu_master_vreg(SMMU_MSTR_DBG_4, tmp_value);
	dprint(PRN_ALWS, "SMMU_MSTR_DBG_4 : 0x%x\n", tmp_value);
	rd_smmu_master_vreg(SMMU_MSTR_DBG_5, tmp_value);
	dprint(PRN_ALWS, "SMMU_MSTR_DBG_5 : 0x%x\n", tmp_value);
	rd_smmu_master_vreg(SMMU_MSTR_DBG_6, tmp_value);
...
SINT32 smmu_init(void)
{
	...
	g_smmu_reg_vir.smmu_common_base_vir_addr =
		(SINT32 *) mem_phy_2_vir(VCODEC_VDEC_SMMU_COMMON_BASE_ADDR);
	...
	g_smmu_reg_vir.smmu_master_base_vir_addr =
		(SINT32 *) mem_phy_2_vir(VCODEC_VDEC_SMMU_MASTER_BASE_ADDR);
