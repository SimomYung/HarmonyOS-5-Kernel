...
void vdmhal_imp_reset_vdm(SINT32 vdh_id)
{
	...
	read_vreg((SINT32)VREG_INT_MASK, tmp, vdh_id);//lint !e578
...
UINT32 vdmhal_imp_check_reg(reg_id_e reg_id)
{
	SINT32 *p32 = NULL;
	UINT32 dat = 0;
	SINT32 reg_type;
	...
	mem_buffer_s *vdh_mem_map)
{
	...
	uint32_t vdh_stream_buf_num;
#ifdef VFMW_HEVC_SUPPORT
	const int32_t msg_slot_size = HEVC_ONE_MSG_SLOT_LEN;
#else
	const int32_t msg_slot_size = MSG_SLOT_SIZE;
#endif
	int32_t is_mapped = 0;
	...
	for (index = VDH_SHAREFD_FRM_BUF; index < VDH_SHAREFD_MAX; index++) {
		...
		if (((USIGN)(vdh_reg_cfg->vdh_yst_addr) >= vdh_mem_map[index].start_phy_addr) &&
			((vdh_reg_cfg->vdh_uv_offset > 0) && ((uint32_t)vdh_reg_cfg->vdh_uv_offset < vdh_mem_map[index].size)) &&
			((USIGN)(vdh_reg_cfg->vdh_yst_addr) < (vdh_mem_map[index].start_phy_addr + vdh_mem_map[index].size -
			(uint32_t)vdh_reg_cfg->vdh_uv_offset))) {
			is_mapped = 1;
			break;
		}
	}
...
SINT32 vdmhal_is_vdm_run(SINT32 vdh_id)
{
	...
	read_vreg((SINT32)VREG_VCTRL_STATE, data32, vdh_id);
...
SINT32 vdmhal_imp_backupinfo(void)
{
	...
	g_vdm_reg_state.state_reg = vdmhal_imp_check_reg(INT_STATE_REG);
...
void vdmhal_imp_starthwrepair(SINT32 vdh_id)
{
	...
	read_vreg((SINT32)VREG_BASIC_CFG0, d32, vdh_id);
	d32 = 0x4000000;
