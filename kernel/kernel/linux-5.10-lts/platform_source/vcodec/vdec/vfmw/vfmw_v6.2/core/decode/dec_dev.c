...
static int32_t dec_dev_block_wait_reg(vdh_reg_info *reg)
{
	uint32_t wait_reg_time_out = 100;
	OS_EVENT_CLEAR((uint32_t)reg->event);
	return OS_EVENT_WAIT((uint32_t)reg->event, (uint64_t)wait_reg_time_out);
}
...
static int32_t dec_dev_get_cfg_reg_id(dec_dev_info *dev, uint16_t *reg_id)
{
	...
	if (reg->used == VCODEC_TRUE && dec_dev_block_wait_reg(reg) != OSAL_OK) {
	    return DEC_ERR;
	}
...
static int32_t dec_dev_release_reg(dec_dev_info *dev, uint16_t reg_id)
{
	...
	OS_EVENT_GIVE((uint32_t)vdh->vdh_reg_pool.vdh_reg[reg_id].event);
...
int32_t dec_dev_config_reg(uint32_t instance_id, void *dev_cfg)
{
	...
	if (VDH_REG_NUM > 1 && get_current_sec_mode() == SEC_MODE) {
...
int32_t dec_dev_reset(dec_dev_info *dev, bool is_timeout)
{
	...
	if (ret != DEC_OK) {
		OS_SPIN_UNLOCK((uint32_t)reg_pool->spin_lock, &lock_flag);
		dprint(PRN_ERROR, "dev_id %hu, reset failed.", dev->dev_id);
		return DEC_ERR;
	}
	OS_SPIN_UNLOCK((uint32_t)reg_pool->spin_lock, &lock_flag);
	...
	while (dec_dev_reglist_get_back_up(back_up, back_up_list) != DEC_OK) {
		if (dec_dev_decode_status(dev, back_up_list) != OSAL_OK) {
			if (dev->reg_dump_enable != 0 || glb_info->is_fpga != 0) {
				dprint(PRN_ERROR, "decoder timeout, dump reg[%hu] now\n", back_up_list->get_index);
...
static void dec_dev_suspend_vdh(dec_dev_info *dec_dev)
{
	...
	do {
		if (wait_count > DEC_VDH_WAIT_COUNT) {
			dprint(PRN_FATAL, "suspend vdh wait timeout, dec reset now\n");
