...
#define dprint(type, fmt, arg...) \
	do { \
		if ((type == PRN_FATAL) || (type == PRN_ERROR)) { \
			tloge(" [vcodec-driver] [%s] " fmt, __func__, ##arg); \
		} else if (type == PRN_WARN) { \
			tlogw(" [vcodec-driver] [%s] " fmt, __func__, ##arg); \
		} else if ((type == PRN_ALWS) || default_print_check(type)) { \
			tlogi(" [vcodec-driver] [%s] " fmt, __func__, ##arg); \
		} \
	} while (0)
#endif
