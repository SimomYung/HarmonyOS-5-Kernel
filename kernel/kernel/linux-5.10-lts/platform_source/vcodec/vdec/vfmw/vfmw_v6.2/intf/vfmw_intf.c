...
static int32_t g_sec_init_state;
static int32_t g_sec_power_state = 0;

#define WAIT_TASK_EXIT_TIMEOUT        100 // ms
#define WAIT_TASK_SEC_DEINIT_TIMEOUT  30 // ms
#define WAIT_TASK_EXIT_INER           1  // ms
#ifdef CONFIG_VCODEC_OHOS
#ifdef VCODEC_PASSTHROUGH_SUPPORT
#define CLIENT_CA_UID 1013
#else
#define CLIENT_CA_UID 3031
#endif
#else
#define CLIENT_CA_UID 1046
#endif
...
void vfmw_tvp_task_init(void)
{
	if (current_uid().val != CLIENT_CA_UID ) {
		dprint(PRN_ALWS, "uid not support\n");
		return;
	}

	if (!IS_ERR_OR_NULL(g_vdec_tvp.task))
		return;
...
int32_t vfmw_sec_power_on(void)
{
	int32_t ret = VDEC_OK;
#ifdef VCODEC_TVP_SUPPORT
	if (current_uid().val != CLIENT_CA_UID) {
	...
	g_sec_power_state = 1;
	dprint(PRN_FATAL, "vfmw_sec_power_on succ\n");
...
void vfmw_sec_power_off(void)
{
	...
	do {
		msleep(WAIT_TASK_EXIT_INER);
		sleep_count++;
		if (g_sec_init_state == 0)
			break;
	} while (sleep_count < (WAIT_TASK_SEC_DEINIT_TIMEOUT / WAIT_TASK_EXIT_INER));
	g_sec_power_state = 0;
	dprint(PRN_ALWS, "sleep_count %d\n",sleep_count);
#endif
}
...
static int32_t vfmw_dec_process(void *fd, void *dev_cfg, void *vdh_state_reg)
{
	...
	ret = dec_dev_config_reg(vdec_file->instance_id, dev_cfg);
	if (ret)
		goto error;

	return VFMW_OK;
error:
	vfmw_del_from_dmabuf_list(vdec_file->instance_id, true);
	return VFMW_ERR;
}
...
int32_t vfmw_query_image(struct file *file_handle, void *dev_cfg, void *read_backup)
{
	...
	vfmw_del_from_dmabuf_list(vdec_file->instance_id, false);
	return ret;
}
#ifdef ENABLE_VDEC_MODULE
...
vcodec_bool vfmw_scene_ident(void)
{
	...
	dprint(PRN_ALWS, "get pid failed ident_info %d, tgid %d\n", ident_info, (uint32_t)current->tgid);
	return VCODEC_FALSE;
}
#ifdef ENABLE_VDEC_MODULE
