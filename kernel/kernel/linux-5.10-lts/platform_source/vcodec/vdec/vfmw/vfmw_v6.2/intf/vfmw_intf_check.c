...
static int32_t vfmw_rollback_dmabuf_list(struct vdec_dmabuf_s *bufs, struct vdec_dmabuf_list *list)
{
	int32_t ret;
	uint16_t insert_index;
	struct vdec_dmabuf_node *buf_node = NULL;
	...
	mutex_lock(&list->buf_mutex);
	insert_index = list->insert_index;
	if (insert_index >= VDH_REG_NUM) {
		dprint(PRN_ERROR, "failed, index = %hu\n", insert_index);
		ret = VFMW_ERR;
		goto exit;
	}
	insert_index = (insert_index <= 0 ? (VDH_REG_NUM - 1) : (insert_index - 1));

	buf_node = &list->dmabuf_list[insert_index];
	if (buf_node->used == false) {
		dprint(PRN_ERROR, "buf is not used faile, index = %hu\n", insert_index);
		ret = VFMW_ERR;
		goto exit;
	}
	...
	buf_node->used = false;
	list->insert_index = insert_index;
	ret = VFMW_OK;
...
void vfmw_del_from_dmabuf_list(uint32_t instance_id, bool is_rollback)
{
	int32_t ret;
	struct vdec_dmabuf_s bufs;
	struct vdec_dmabuf_list *list = vfmw_get_dmabuf_list(instance_id);
	...
	if (is_rollback)
		ret = vfmw_rollback_dmabuf_list(&bufs, list);
	else
...
int32_t vfmw_stm_param_check(void *fd, void *stm_cfg)
{
	...
	vfmw_check_return((reg_config->reg.safe_flag == 0), VFMW_ERR);
...
int32_t vfmw_dec_param_check(void *dev_cfg)
{
	...
	vfmw_check_return((cfg->is_frm_cpy == 0) ||
		((cfg->is_frm_cpy == 1) && ((std_vid == VFMW_VP9) || (std_vid == VFMW_AV1)|| (std_vid == VFMW_MPEG4))),
		VFMW_ERR);
#else
	vfmw_check_return((cfg->is_frm_cpy == 0) ||
