...
static uint64_t linux_get_time_in_ns(void)
{
...
{
	uint64_t sys_time = linux_get_time_in_ns();
...
void linux_clear_event(uint32_t handle)
{
	osal_event *event = NULL;
	int32_t id = handle & 0xffff;
	uint32_t unid = handle >> 16;
...
void osal_intf_init(void)
{
	...
	ops->event_clear = linux_clear_event;
