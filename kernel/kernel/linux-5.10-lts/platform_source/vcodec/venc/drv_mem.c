...
struct mem_pool *drv_mem_create_pool(struct device *dev, uint32_t each_size, uint32_t num, uint32_t align_size)
{
	...
	VCODEC_INFO_VENC("begin to dma_alloc_coherent, size=%u", size);
	...
	VCODEC_INFO_VENC("dma_alloc_coherent succ");
...
int32_t drv_mem_alloc_dma(struct device *dev, uint32_t size, struct venc_dma_mem *mem)
{
	...
	VCODEC_INFO_VENC("begin to dma_heap_buffer_alloc_kernel, size=%u", aligned_size);
	dmabuf = dma_heap_buffer_alloc_kernel(name, aligned_size, 0);
	if (IS_ERR_OR_NULL(dmabuf)) {
		VCODEC_FATAL_VENC("fail to alloc dma buffer");
		return -ENOMEM;
	}
	VCODEC_INFO_VENC("dma_heap_buffer_alloc_kernel succ");

	attach = dma_buf_attach(dmabuf, dev);
	if (IS_ERR_OR_NULL(attach)) {
		VCODEC_FATAL_VENC("fail to attach dma buf");
		goto err1;
	}

	sgt = dma_buf_map_attachment(attach, DMA_BIDIRECTIONAL);
	if (IS_ERR_OR_NULL(sgt)) {
		VCODEC_FATAL_VENC("fail to map attachment");
		goto err2;
	}

	iova = kernel_iommu_map_dmabuf(dev, dmabuf, 0, &iova_size);
	if (!iova) {
		VCODEC_FATAL_VENC("fail to map iova");
		goto err3;
	}
	
	ret = dma_buf_begin_cpu_access(dmabuf, DMA_BIDIRECTIONAL);
	if (ret < 0) {
		VCODEC_FATAL_VENC("fail to begin cpu access");
		goto err4;
	}

	va = dma_buf_kmap(dmabuf, 0);
	if (!va) {
		VCODEC_FATAL_VENC("fail to kmap");
		goto err5;
	}
...
err4:
	kernel_iommu_unmap_dmabuf(dev, dmabuf, iova);
err3:
...
{
	if (!dev || !mem || !mem->virt_addr || !mem->iova_addr ||
	    !mem->attach || !mem->dmabuf || !mem->hal_table) {
		VCODEC_FATAL_VENC("invalid param when free");
		return;
	}
	dma_buf_kunmap(mem->dmabuf, 0, NULL);
	dma_buf_end_cpu_access(mem->dmabuf, DMA_BIDIRECTIONAL);
	kernel_iommu_unmap_dmabuf(dev, mem->dmabuf, mem->iova_addr);
	dma_buf_unmap_attachment(mem->attach, mem->hal_table, DMA_BIDIRECTIONAL);
	dma_buf_detach(mem->dmabuf, mem->attach);
	dma_heap_buffer_free_kernel(mem->dmabuf);
}

#else // CONFIG_VCODEC_OHOS
...
int32_t drv_mem_alloc_dma(struct device *dev, uint32_t size, struct venc_dma_mem *mem)
{
	...
	aligned_size = PAGE_ALIGN(size);
	VCODEC_INFO_VENC("begin to dma_alloc_coherent, size=%u", aligned_size);
...
#endif // CONFIG_VCODEC_OHOS

#ifdef ENABLE_VENC_WIRELESS_LOW_LATENCY
struct dma_buf *venc_kmap_kernel_va(int32_t shared_fd, uint64_t *kernel_va)
{
	struct dma_buf *dmabuf = NULL;
	if (vcodec_venc_down_interruptible(&g_venc_mem_sem)) {
		VCODEC_FATAL_VENC("IommuMap down interruptible failed");
		return NULL;
	}

	dmabuf = dma_buf_get(shared_fd);
	if (IS_ERR(dmabuf)) {
		VCODEC_FATAL_VENC("get dma buf failed");
		vcodec_venc_up_interruptible(&g_venc_mem_sem);
		return NULL;
	}

	*kernel_va = (uint64_t)(uintptr_t)dma_buf_kmap(dmabuf, 0);
	vcodec_venc_up_interruptible(&g_venc_mem_sem);
	return dmabuf;
}

void venc_kunmap_kernel_va(struct dma_buf *dmabuf)
{
	if (!dmabuf) {
