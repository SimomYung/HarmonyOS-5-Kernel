...
__attribute__((weak)) void venc_dump_dfx_info(uint32_t *reg_base)
{
	unused(reg_base);
}
#endif
...
#ifdef VENC_DEBUG_ENABLE
	venc_dump_dfx_info(ctx->reg_base);
#endif
...
static void venc_encode_done(struct venc_context *ctx)
{
	...
	*/
	...
	if (is_encode_done(int_status)) {
		...
		venc_hal_get_pic_stat(&info.picture_info, ctx->reg_base);
#ifdef VENC_DEBUG_ENABLE
		if (venc->debug_flag & (1LL << PRINT_ENC_STATIC_INFO))
			venc_hal_printf_pic_stat(&info.picture_info, ctx->reg_base,
				info.channel_info.id, info.channel_info.frame_number);
#endif
#endif
#ifdef ENABLE_VENC_WIRELESS_LOW_LATENCY
		if (info.channel_info.wireless_ldy_info.is_wireless_lowdelay_enable &&
			info.channel_info.wireless_ldy_info.pack_enable) {
