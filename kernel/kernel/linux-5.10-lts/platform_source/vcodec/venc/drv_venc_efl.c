...
int32_t venc_lock_shared_fd(struct venc_fd_list *list, venc_fd_info_t *info)
{
	...
	ret = venc_shared_fd_list_add(list, info->watermark_shared_fd);
	if (ret != 0)
		return ret;
...
void venc_unlock_shared_fd(struct venc_fd_list *list, venc_fd_info_t *info)
{
	...
	venc_shared_fd_list_remove(list, info->watermark_shared_fd);
}
...
static int32_t get_mem_info(venc_fd_info_t *fd_info, venc_buffer_info_t *mem_info)
{
	...
	if (fd_info->watermark_shared_fd >= 0) {
		ret = get_mem_info_by_fd(fd_info->watermark_shared_fd, &mem_info->watermark_buffer);
		if (ret) {
			VCODEC_FATAL_VENC("watermark buffer map failed");
			return VCODEC_FAILURE;
		}
	}
...
int32_t venc_drv_create_queue(void)
{
	...
	for (i = 0; i < MAX_INSTANCES_COUNT; i++) {
		...
		g_venc_file_private[i].feedback_queue = create_queue(struct venc_feedback_queue);
		if (g_venc_file_private[i].feedback_queue == NULL)
			break;
#endif
	}
...
#ifdef ENABLE_VENC_WIRELESS_LOW_LATENCY
		if (g_venc_file_private[i].feedback_queue != NULL) {
			destory_queue(g_venc_file_private[i].feedback_queue);
			g_venc_file_private[i].feedback_queue = NULL;
		}
#endif
	}
}
...
int32_t venc_drv_alloc_file_private(struct file *file)
{
	...
	for (i = 0; i < MAX_INSTANCES_COUNT; i++) {
        ...
        if (queue_is_null(g_venc_file_private[i].encode_done_info) &&
            queue_is_null(g_venc_file_private[i].feedback_queue))
#else
		if (queue_is_null(g_venc_file_private[i].encode_done_info))
#endif
			break;
...
#ifdef ENABLE_VENC_WIRELESS_LOW_LATENCY
	ret = alloc_queue(g_venc_file_private[i].feedback_queue, MAX_QUEUE_SIZE);
	if (ret != 0) {
		free_queue(g_venc_file_private[i].encode_done_info);
		VCODEC_FATAL_VENC("failed to allocate feedback_queue");
		return VCODEC_FAILURE;
	}
#endif
...
int32_t venc_drv_free_file_private(const struct file *file)
{
	...
	for (i = 0; i < MAX_INSTANCES_COUNT; i++) {
		if (file->private_data == &g_venc_file_private[i]) {
			free_queue(g_venc_file_private[i].encode_done_info);
#ifdef ENABLE_VENC_WIRELESS_LOW_LATENCY
			free_queue(g_venc_file_private[i].feedback_queue);
#endif
...
static int32_t venc_drv_update_channel_id(struct encode_info *encode_info, struct venc_file_private *file_private)
{
	uint32_t i;
	VCODEC_DBG_VENC("enter %s (): User channel ID = %d", __func__, encode_info->channel.id);

	for (i = 0; i < MAX_INSTANCES_COUNT; i++) {
		if (&g_venc_file_private[i] == file_private) {
			file_private->user_channel_id = encode_info->channel.id;
			file_private->venc_perf_loads = encode_info->clock_info.loads;
			file_private->venc_low_power_enable_flag =
...
int32_t venc_drv_encode(struct encode_info *encode_info, struct venc_file_private *file_private)
{
	uint32_t i;
	venc_entry_t *venc = platform_get_drvdata(venc_get_device());
	if (venc_drv_update_channel_id(encode_info, file_private))
		return VCODEC_FAILURE;
	...
	return venc->ops.encode(encode_info, file_private->encode_done_info);
}
