...
#ifdef ENABLE_VENC_WIRELESS_LOW_LATENCY
#include "drv_venc_wireless.h"
#endif

#ifdef ENABLE_VENC_LOW_TEMP
...
#define check_para_size_return(size, para_size, command) \
	do { \
		if ((size) != (para_size)) { \
			VCODEC_ERR_VENC("%s: prarameter_size is error\n", command); \
			return -EINVAL; \
		} \
	} while (0)
...
struct ioctl_fun_param {
	unsigned int cmd;
	void __user *user_arg;
	struct venc_file_private *file_private;
	compat_type_e type;
};

typedef int32_t(*venc_fn_ioctl_handler)(struct ioctl_fun_param *param);

struct venc_ioctl_cmd_node {
	uint32_t cmd;
	venc_fn_ioctl_handler handle;
};
...
static int32_t venc_drv_cmd_encode(unsigned int cmd,
			struct venc_file_private *file_private, void __user *user_arg)
{
	int32_t ret;
	void *kernel_arg = NULL;
	struct encode_info *encode_info = NULL;
#ifdef ENABLE_VENC_WIRELESS_LOW_LATENCY
	int32_t is_wireless_scene;
	struct wireless_lowdelay_info *info = NULL;
#endif
...
#endif
#ifdef ENABLE_VENC_WIRELESS_LOW_LATENCY
	info = &encode_info->channel.wireless_ldy_info;
	is_wireless_scene = venc_drv_wireless_check_file_private(file_private);
	if (info->is_wireless_lowdelay_enable && (is_wireless_scene == EOK)) {
		if (info->is_send_stream_header_to_wifi) {
			venc_drv_wireless_send_head(encode_info);
			vcodec_venc_up_interruptible(&g_encode_sem);
			return 0;
		} else {
			ret = venc_drv_wireless_check_and_map(encode_info);
			if (ret) {
				VCODEC_FATAL_VENC("venc wireless check and map failed");
				vcodec_venc_up_interruptible(&g_encode_sem);
				return VCODEC_FAILURE;
			}
		}
	} else {
		(void)memset_s(info, sizeof(struct wireless_lowdelay_info), 0, sizeof(struct wireless_lowdelay_info));
	}
#endif

	ret = venc_lock_shared_fd(&file_private->fdlist, &encode_info->channel.mem_info);
	if (ret != 0) {
		VCODEC_FATAL_VENC("lock shared fd failed");
		vcodec_venc_up_interruptible(&g_encode_sem);
		return ret;
	}
	ret = venc_drv_encode(encode_info, file_private);
	if (ret != 0) {
	...
	if (encode_info->is_block == 1) {
		...
		venc_unlock_shared_fd(&file_private->fdlist, &encode_info->channel.mem_info);
	}
...
static int32_t ioctl_cmd_encode(struct ioctl_fun_param *param)
{
	int32_t ret;
	if (!param || !param->file_private || !param->user_arg) {
		VCODEC_FATAL_VENC("param private_data or user_arg is null");
		return VCODEC_FAILURE;
	}

	ret = venc_drv_cmd_encode(param->cmd, param->file_private, param->user_arg);
	if (ret != 0) {
		VCODEC_FATAL_VENC("venc_drv_cmd_encode failed");
		return VCODEC_FAILURE;
	}
	return 0;
}

static int32_t ioctl_cmd_encode_done(struct ioctl_fun_param *param)
{
	int32_t ret;
	...
	ret = venc_drv_get_encode_done_info(param->file_private->encode_done_info, &info);
	if (ret != 0) {
		VCODEC_FATAL_VENC("encode done info buffer is empty");
		return VCODEC_FAILURE;
	}
	venc_unlock_shared_fd(&param->file_private->fdlist, &info.channel_info.mem_info);

	if (copy_to_user(param->user_arg, &info, sizeof(info))) {
...
static int32_t ioctl_cmd_iommu_map(struct ioctl_fun_param *param)
{
	int32_t ret;
	...
	ret = venc_compat_get_data(param->type, param->user_arg, &ioctl_msg);
	if (ret != 0) {
		VCODEC_FATAL_VENC("venc_compat_get_data failed");
		return VCODEC_FAILURE;
	}

	check_para_size_return(ioctl_msg.in_size, sizeof(buf_node), "copy_from_user");
...
static int32_t ioctl_cmd_iommu_unmap(struct ioctl_fun_param *param)
{
	int32_t ret;
	venc_ioctl_msg ioctl_msg = {0};
	venc_buffer_record_t buf_node = {0};
...
#ifdef ENABLE_VENC_WIRELESS_LOW_LATENCY
	if (venc_drv_wireless_check_file_private(param->file_private) == EOK)
		venc_drv_unmap_wireless(buf_node.shared_fd);
#endif
...
#ifdef ENABLE_VENC_WIRELESS_LOW_LATENCY
static int32_t ioctl_cmd_wireless_lowdelay_init(struct ioctl_fun_param *param)
{
	int32_t ret;
	if (!param || !param->user_arg || !param->file_private) {
		VCODEC_FATAL_VENC("param private_data or user_arg is null");
		return -EINVAL;
	}
	...
	ret = vcodec_venc_down_interruptible(&g_encode_sem);
	if (ret != 0) {
		VCODEC_FATAL_VENC("wireless_lowdelay_init: down interruptible failed");
		return ret;
	}

	ret = venc_drv_wireless_init(param->cmd, param->file_private);
	if (ret != 0) {
		vcodec_venc_up_interruptible(&g_encode_sem);
		VCODEC_FATAL_VENC("init venc drv wireless_lowdelay failed");
		return ret;
	}
	vcodec_venc_up_interruptible(&g_encode_sem);
	return 0;
...
static int32_t ioctl_cmd_wireless_lowdelay_feedback(struct ioctl_fun_param *param)
{
	int32_t ret;
	struct wifi_feedback_info feedback_info = { 0 };

	if (!param || !param->file_private || !param->user_arg) {
		VCODEC_FATAL_VENC("param or user_arg is null");
		return -EINVAL;
	}
	...
	ret = venc_drv_wireless_get_feedback_info(param->file_private, &feedback_info);
	if (ret) {
		VCODEC_FATAL_VENC("feedback info buffer is empty");
		return ret;
	}

	if (copy_to_user(param->user_arg, &feedback_info, sizeof(struct wifi_feedback_info))) {
		VCODEC_FATAL_VENC("get feedback info: copy to user failed");
		return VCODEC_FAILURE;
	}
	return 0;
}
...
static const struct venc_ioctl_cmd_node g_venc_ioctl_cmd_table[] = {
	{ CMD_VENC_ENCODE, ioctl_cmd_encode },
	{ CMD_VENC_GET_ENCODE_DONE_INFO, ioctl_cmd_encode_done },
	{ CMD_VENC_IOMMU_MAP, ioctl_cmd_iommu_map },
	{ CMD_VENC_IOMMU_UNMAP, ioctl_cmd_iommu_unmap },
#ifdef ENABLE_VENC_WIRELESS_LOW_LATENCY
	{ CMD_VENC_WIRELESS_LOWDELAY_INIT, ioctl_cmd_wireless_lowdelay_init },
	{ CMD_VENC_WIRELESS_LOWDELAY_GET_FEEDBACK_INFO, ioctl_cmd_wireless_lowdelay_feedback },
#endif
};

static venc_fn_ioctl_handler venc_ioctl_get_handler(uint32_t cmd)
{
	size_t index;
	venc_fn_ioctl_handler target_handler = VCODEC_NULL;
	size_t cmd_table_size = ARRAY_SIZE(g_venc_ioctl_cmd_table);

	for (index = 0; index < cmd_table_size; index++) {
		if (cmd == g_venc_ioctl_cmd_table[index].cmd) {
			target_handler = g_venc_ioctl_cmd_table[index].handle;
			break;
		}
	}
...
static long venc_ioctl_common(struct file *file, unsigned int cmd, unsigned long arg, compat_type_e type)
{
	...
	struct ioctl_fun_param fun_params = {0};
	venc_fn_ioctl_handler ioctl_handler;
	...
	fun_params.cmd = cmd;
	fun_params.user_arg = user_arg;
	fun_params.file_private = (struct venc_file_private *)file->private_data;
	fun_params.type = type;

	ioctl_handler = venc_ioctl_get_handler(cmd); 
	if (!ioctl_handler) {
		VCODEC_FATAL_VENC("venc_ioctl_get_handler is null");
		return VCODEC_FAILURE;
	}
	ret = ioctl_handler(&fun_params);
	return ret;
}
...
static int32_t venc_drv_open(struct inode *finode, struct file  *ffile)
{
	...
	VCODEC_INFO_VENC("begin to open");
...
#ifdef ENABLE_VENC_WIRELESS_LOW_LATENCY
	if (venc_drv_wireless_check_file_private(ffile->private_data) == EOK)
...
{
#if defined (SUPPORT_CMDLIST) && !defined (VENC_CMDLIST_DISABLE)
	cmdlist_init_ops();
#else
	venc_init_ops();
#endif
