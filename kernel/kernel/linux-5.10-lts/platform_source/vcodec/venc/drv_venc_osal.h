	...
	while (!(condtion) && (_ret != 0)) { \
		...
		if (_ret < 0) { \
			...
			if (time_period(start_time, cur_time) > (uint64_t)(timeout_in_ms)) { \
				VCODEC_FATAL_VENC("waitevent time out, time : %llu", \
...
#define DECLARE_QUEUE_PTR(element_type) \
	uint32_t max_size; \
	uint32_t cur_size; \
...
#define queue_is_empty(queue) (queue->cur_size == 0)
#define queue_is_full(queue) (queue->cur_size >= queue->max_size)
#define queue_is_null(queue) (queue->buffer == NULL)
#define queue_data_len(queue) (queue->cur_size)

#define create_queue(type) \
({ \
	type *queue = NULL; \
	type *tmp = NULL; \
	tmp = (type *)vmalloc(sizeof(type)); \
	if (!IS_ERR_OR_NULL(tmp)) { \
		(void)memset_s(tmp, sizeof(type), 0, sizeof(type)); \
...
({ \
	...
	do { \
		(queue)->buffer = vmalloc(sizeof((queue)->element[0]) * size); \
		if (queue_is_null(queue)) { \
			VCODEC_WARN_VENC("queue is null"); \
			break; \
		} \
		(queue)->max_size = size; \
		(queue)->front = 0; \
		(queue)->rear = 0; \
		(queue)->element_size = sizeof((queue)->element[0]); \
		_ret = 0; \
	} while (0); \
	_ret; \
})
...
({ \
	...
	do { \
		if (queue_is_null(queue) || queue_is_empty(queue)) { \
			VCODEC_ERR_VENC("pop data failed"); \
			break; \
		} \
        uint8_t* srcData = (queue)->buffer + (queue)->rear * (queue)->element_size; \
        (void)memcpy_s(buf, sizeof(*buf), srcData, sizeof(*buf)); \
...
({ \
	...
	spin_lock_irqsave((queue)->lock, _flag); \
	do { \
		if (queue_is_null(queue) || queue_is_full(queue)) { \
			VCODEC_ERR_VENC("push data failed"); \
			break; \
		} \
        uint8_t* dtsData = (queue)->buffer + (queue)->front * (queue)->element_size; \
        (queue)->front = (++(queue)->front) % (queue)->max_size; \
		(void)memcpy_s(dtsData, sizeof(*buf), buf, sizeof(*buf)); \
        (queue)->cur_size++; \
		_ret = 0; \
	} while (0); \
	spin_unlock_irqrestore((queue)->lock, _flag); \
