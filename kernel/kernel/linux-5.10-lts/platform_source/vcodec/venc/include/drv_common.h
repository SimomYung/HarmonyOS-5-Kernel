/*
 * drv_common.h
 *
 * This is for VENC Driver Common Structure.
 *
 * Copyright (c) 2019-2020 Huawei Technologies CO., Ltd.
 *
 * This software is licensed under the terms of the GNU General Public
 * License version 2, as published by the Free Software Foundation, and
 * may be copied, distributed, and modified under those terms.
 *
 * This program is distributed in the hope that it will be useful,
 * but WITHOUT ANY WARRANTY; without even the implied warranty of
 * MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the
 * GNU General Public License for more details.
 *
 */

#ifndef __DRV_COMMON_H__
#define __DRV_COMMON_H__

#include <linux/platform_device.h>
#include <linux/version.h>
#include "securec.h"
#include "vcodec_type.h"
#include "drv_venc_osal.h"
#include "drv_venc_ioctl.h"
#include "power_manager.h"
#include "drv_mem.h"
#include "hal_capability.h"
#ifdef ENABLE_VENC_WIRELESS_LOW_LATENCY
#include "drv_venc_thread.h"
#endif

#ifdef PCIE_LINK
#define INTERRUPT_TIMEOUT_MS 100000
#define ENCODE_DONE_TIMEOUT_MS 110000
#define WAIT_CORE_IDLE_TIMEOUT_MS 110000
#define FPGA_WAIT_EVENT_TIME_MS 110000
#else
#define INTERRUPT_TIMEOUT_MS 300
#define ENCODE_DONE_TIMEOUT_MS 2000
#define WAIT_CORE_IDLE_TIMEOUT_MS 1000
#define FPGA_WAIT_EVENT_TIME_MS 1000000
#endif

#define MAX_INSTANCES_COUNT 16
#define MAX_QUEUE_SIZE 8
#define MAX_STRM_BUF_SIZE 32

#define align_up(val, align) (((val) + ((align)-1)) & ~((align)-1))
#ifndef unused
#define unused(x) (void)(x)
#endif

#define d_venc_check_cfg_reg_streamaddr_ret(map_addr, cfg_reg_addr, map_buffer_size, shared_fd) \
	do { \
		if ((((cfg_reg_addr) < (map_addr))  || ((cfg_reg_addr) >= ((map_addr) + (map_buffer_size)))) && \
			(((shared_fd) >= 0) && (((cfg_reg_addr) != 0) && ((cfg_reg_addr) != 0xffffffff)))) \
			return VCODEC_FAILURE; \
	} while (0)

#define d_venc_check_cfg_reg_streamendaddr_ret(map_addr, cfg_reg_addr, map_buffer_size, shared_fd) \
	do { \
		if ((((cfg_reg_addr) < ((map_addr) + 1))  || ((cfg_reg_addr) > ((map_addr) + (map_buffer_size)))) && \
			(((shared_fd) >= 0) && (((cfg_reg_addr) != 0) && ((cfg_reg_addr) != 0xffffffff)))) \
			return VCODEC_FAILURE; \
	} while (0)

#define d_venc_check_cfg_reg_addr_ret(map_addr, cfg_reg_addr, map_buffer_size) \
	do { \
		if (((cfg_reg_addr) < (map_addr)) || ((cfg_reg_addr) >= ((map_addr) + (map_buffer_size)))) \
			return VCODEC_FAILURE; \
	} while (0)

#define d_venc_check_cfg_reg_endaddr_ret(map_addr, cfg_reg_addr, map_buffer_size) \
	do { \
		if (((cfg_reg_addr) < ((map_addr) + 1)) || ((cfg_reg_addr) > ((map_addr) + (map_buffer_size)))) \
			return VCODEC_FAILURE; \
	} while (0)

#define d_venc_check_smrx_reg_addr_ret(map_addr, cfg_reg_addr, map_buffer_size) \
	do { \
	if (((cfg_reg_addr) < ((map_addr) >> 15)) || ((cfg_reg_addr) >= (((map_addr) + (map_buffer_size)) >> 15))) \
		return VCODEC_FAILURE; \
	} while (0)

#define d_venc_check_smrx_reg_endaddr_ret(map_addr, cfg_reg_addr, map_buffer_size) \
	do { \
		if (((cfg_reg_addr) < (((map_addr) + 1) >> 15)) || ((cfg_reg_addr) > (((map_addr) + (map_buffer_size)) >> 15))) \
			return VCODEC_FAILURE; \
	} while (0)

enum {
	VEDU_H265 = 0,
	VEDU_H264 = 1
};

typedef enum {
	VENC_CORE_0,
	VENC_CORE_1,
	MAX_SUPPORT_CORE_NUM,
} venc_core_id_t;

typedef enum {
	VENC_POWER_OFF,
	VENC_IDLE,
	VENC_BUSY,
	VENC_TIME_OUT,
} venc_ip_status_t;

enum {
	CMDLIST_DISABLE = 0,
	CMDLIST_DUMP_DATA,
	PRINT_ENC_COST_TIME,
	PRINT_HARDWARE_USAGE,
	LOW_POWER_DISABLE,
	MCORE_DISABLE,
	PRINT_ENC_STATIC_INFO,
};

#ifdef VENC_DPM_ENABLE
enum {
	DP_MONIT_MOUDLE,
	SMMU_PRE_MODULE,
	MAX_INNER_MODULE,
};
#endif

struct venc_fifo_buffer {
	spinlock_t *lock;
	vedu_osal_event_t event;
	DECLARE_QUEUE_PTR(struct encode_done_info);
};

#ifdef ENABLE_VENC_WIRELESS_LOW_LATENCY
struct venc_feedback_queue {
	spinlock_t *lock;
	vedu_osal_event_t event;
	DECLARE_QUEUE_PTR(struct wifi_feedback_info);
};

struct venc_ctrl_queue {
	spinlock_t *lock;
	vedu_osal_event_t event;
	DECLARE_QUEUE_PTR(struct _tx_layer_ctrl*);
};
#endif

struct venc_fd_listnode {
	struct list_head head;
	int32_t refcnt;
	int32_t shared_fd;
	struct dma_buf *dmabuf;
};
struct venc_fd_list {
	struct mutex  lock;
	struct list_head fdhead;
};
struct venc_file_private {
	uint32_t user_channel_id; // store channel id of use space
	bool venc_low_power_enable_flag;
	uint64_t venc_perf_loads;
	struct venc_fifo_buffer *encode_done_info;
#ifdef ENABLE_VENC_WIRELESS_LOW_LATENCY
	struct venc_feedback_queue *feedback_queue; // feedback queue
#endif
	struct venc_fd_list fdlist;
};

struct venc_irq_num {
	uint32_t  normal;
	uint32_t  protect;
	uint32_t  safe;
	uint32_t  mcore_normal;
	uint32_t  mcore_safe;
	uint32_t  ecc_detect;
	uint32_t  ecc_multpl;
};

/* VENC IP Context */
struct venc_context {
	spinlock_t lock;
	venc_ip_status_t status;
	struct venc_fifo_buffer *buffer;
	struct channel_info  channel;
	uint32_t  *reg_base;
	uint8_t *intr_hub_reg_base;
	struct venc_irq_num  irq_num;
	bool first_cfg_flag;
	bool is_block;
	bool is_protected;
	venc_timer_t timer;
	struct power_manager pm;
	uint64_t start_time;
	int32_t core_id;
	uint64_t min_time;
	uint64_t max_time;
	uint64_t total_time;
	uint64_t total_number;
};

struct encode_ops {
	int32_t (*init)(void);
	void (*deinit)(void);
	int32_t (*encode)(struct encode_info*, struct venc_fifo_buffer*);
	void (*encode_done)(struct venc_context*);
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(5, 10, 0))
	void (*encode_timeout)(struct timer_list *);
#else
	void (*encode_timeout)(unsigned long);
#endif
};

struct clock_backup_info {
	struct mutex lock;
	struct clock_info info;
};

struct venc_mcu_entry {
	uint32_t *intr_hub_reg_base;
	struct venc_dma_mem mcu_ddr;
	void *image;
	uint32_t image_size;
};

typedef struct {
	dev_t dev;
	struct device *venc_device;
	struct cdev cdev;
	struct class *venc_class;
	struct semaphore hw_sem; /* hardware lock */
	struct venc_context ctx[MAX_SUPPORT_CORE_NUM];
	vedu_osal_event_t event;
	struct encode_ops ops;
	struct clock_backup_info backup_info;
	uint32_t debug_flag;
	uint32_t popf_switch; /* switch power off per frame */
	struct venc_mcu_entry mcu;
} venc_entry_t;

#ifdef ENABLE_VENC_WIRELESS_LOW_LATENCY
struct stream_buf {
	int32_t shared_fd;
	int32_t status;
	struct dma_buf *dma_buf;
	uint64_t wifi_iova;
	uint64_t kernel_va;
	uint32_t buf_iova;
	uint32_t buf_size;
};

struct venc_wireless_entry {
	struct _tx_layer_ctrl *ctrl_pool[MAX_QUEUE_SIZE];
	struct venc_ctrl_queue *ctrl_queue;  // ctrl buffer queue
	struct venc_fifo_buffer *send_queue;  // send package thread work queue
	struct venc_fifo_buffer *backup_queue; // info backup queue
	struct venc_fifo_buffer *encdone_queue; // encode done queue
	struct venc_kthread pack_thread;
	struct stream_buf stream_buf[MAX_STRM_BUF_SIZE];
	struct venc_file_private *file_private;
	struct encode_done_info curr_xps_info;
	uint32_t channel_id;
	uint32_t retry_send_xps_count;
	bool curr_xps_existence_flag;
	bool last_xps_feedback;
	bool send_xps_success_flag;
	bool reserved;
	volatile int32_t init_status;
};
#endif

struct platform_device *venc_get_device(void);
int32_t venc_check_coreid(int32_t core_id);
void venc_set_device(struct platform_device *dev);
irqreturn_t venc_drv_encode_done(int32_t irq, void *dev_id);
irqreturn_t venc_drv_mcore_irq(int32_t irq, void *dev_id);
int32_t venc_drv_get_encode_done_info(struct venc_fifo_buffer *buffer, struct encode_done_info *encode_done_info);

#endif
