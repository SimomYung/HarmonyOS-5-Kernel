...
#ifdef LOAD_VENC
#include "load_image/load_venc.h"
#endif
...
#ifdef LOAD_VENC
#define MCORE_MAX_IMG_SIZE (VENC_MCORE_ITCM_SIZE + 8 * 1024) // SEC AUTH CERT SIZE 8k
#define VENC_MCORE_IMG "/vendor/firmware/sec_venc_mcore.img"
#else
#define MCORE_MAX_IMG_SIZE VENC_MCORE_ITCM_SIZE
...
static int32_t venc_mcore_load_image(venc_entry_t *venc)
{
	...
	if (size % 4 != 0 || size > MCORE_MAX_IMG_SIZE) { /* 4 Bytes */
...
static int32_t venc_mcore_alloc_and_init_image_ddr(struct platform_device *pdev)
{
	...
	/* read mcu image */
	ret = kernel_read_file_from_path(VENC_MCORE_IMG, 0, &data, MCORE_MAX_IMG_SIZE, &size, READING_FIRMWARE);
	if (ret < 0 || size == 0) {
...
int32_t venc_mcore_open(void)
{
	...
	if (venc_mcore_load_image(venc) != 0)
		goto ERROR;
#else
	if (load_venc_image(venc->mcu.image, venc->mcu.image_size) != 0)
	 	goto ERROR;
#endif
