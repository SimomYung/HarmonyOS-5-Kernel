...
static void venc_hal_cfg_h264_adjust_pmemv(struct encode_info *channel_cfg, uint32_t *reg_base)
{
...
void venc_hal_cfg_platform_diff(struct encode_info *channel_cfg, uint32_t *reg_base)
{
	...
	venc_hal_cfg_h264_adjust_pmemv(channel_cfg, reg_base);
...
void venc_hal_get_reg_mse(struct stream_info *stream_info, uint32_t *reg_base, uint32_t lcu_block_cnt)
{
	...
	uint64_t sum_mse = all_reg->FUNC_SAO_MSE_SUM;
