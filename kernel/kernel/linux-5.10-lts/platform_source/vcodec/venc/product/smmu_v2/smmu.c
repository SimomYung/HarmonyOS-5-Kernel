...
#define VCODEC_VENC_SMMU_COMMON_OFFSET    0x20000
#define VCODEC_VENC_SMMU_MASTER_OFFSET    0x1A000
...
int32_t venc_smmu_init(bool is_protected, int32_t core_id)
{
	...
	g_vencsmmuregvir.smmu_common_base_viraddr   = (uint32_t *)((uint64_t)(uintptr_t)regbase + VCODEC_VENC_SMMU_COMMON_OFFSET);
	if (g_vencsmmuregvir.smmu_common_base_viraddr == NULL) {
		VCODEC_FATAL_VENC("smmu_common_base_viraddr is NULL");
		return SMMU_ERR;
	}

	g_vencsmmuregvir.smmu_master_base_viraddr   = (uint32_t *)((uint64_t)(uintptr_t)regbase + VCODEC_VENC_SMMU_MASTER_OFFSET);
	if (g_vencsmmuregvir.smmu_master_base_viraddr == NULL) {
		VCODEC_FATAL_VENC("smmu_master_base_viraddr is NULL ");
		return SMMU_ERR;
	}
...
int32_t venc_smmu_cfg(struct encode_info *channelcfg, uint32_t *reg_base)
{
	...
	g_vencsmmuregvir.smmu_master_base_viraddr = (uint32_t *)((uint64_t)(uintptr_t)reg_base + VCODEC_VENC_SMMU_MASTER_OFFSET);
	if (g_vencsmmuregvir.smmu_master_base_viraddr == NULL) {
		VCODEC_FATAL_VENC("smmu_master_base_viraddr is NULL ");
		return SMMU_ERR;
	}
