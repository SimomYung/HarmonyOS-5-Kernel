...
void __attribute__((weak)) venc_hal_set_tbu_swid(uint32_t *reg_base)
{
	unused(reg_base);
}
...
#ifdef VENC_SUPPORT_SET_MCU_SSID
	set_pre_reg(SMMU_PRE_SSID, SMMU_PRE_SSID_MCU_VALUE, 8, 12);
	set_pre_reg(SMMU_PRE_SSIDV, 0x3, 4, 0);
#endif
	/* open tbu */
#ifdef VENC_ECC_ENABLE
	venc_smmu_tbu_ecc_enable();
#endif
...
int32_t venc_smmu_tbu_init(int32_t core_id)
{
	...
	venc_hal_set_tbu_swid(reg_base_addr);
...
void venc_smmu_put_tcu_regulator_info(void)
{
	struct smmu_entry *entry = venc_smmu_get_entry();

	if (!IS_ERR_OR_NULL(entry->tcu_regulator))
		devm_regulator_put(entry->tcu_regulator);
