...
{
	int32_t ret;
	uint32_t single_core_mode_value;
...
#ifdef VENC_FREQUENCY_ADJUSTMENT_IN_KERNEL
...
int32_t venc_get_regulator_info(struct platform_device *pdev)
{
	...
	for (i = 0; i < g_venc_config.venc_conf_com.core_num; i++) {
		if (i == 0) {
			...
			ret = sprintf_s(tmp_name, MAX_NAME_LEN, "%s%u", VENC_REGULATOR_NAME, i);
			if (ret < 0) {
				VCODEC_FATAL_VENC("call sprintf_s failed! ");
				goto name_err;
			}
...
venc_err:
	for (i = 0; i < g_venc_config.venc_conf_com.core_num; i++) {
		if (!IS_ERR_OR_NULL(g_venc_regulator.venc_regulator[i]))
			devm_regulator_put(g_venc_regulator.venc_regulator[i]);
		g_venc_regulator.venc_regulator[i] = VCODEC_NULL;
	}
name_err:
#ifdef SMMU_V3
	venc_smmu_put_tcu_regulator_info();
tcu_err:
#endif
	devm_regulator_put(g_venc_regulator.media_regulator);
	g_venc_regulator.media_regulator = VCODEC_NULL;

	return VCODEC_FAILURE;
}

void venc_put_regulator_info(void)
{
	uint32_t i;
...
static int32_t power_on_single_core(int32_t core_id)
{
	...
	if (venc_mcore_open() != 0) {
		VCODEC_FATAL_VENC("open mcore failed");
...
static int32_t set_clk_rate_single_core(venc_clk_t clk_type, int32_t core_id)
{
	...
	if (g_venc_clock_manager[core_id].curr_clk_type == clk_type && g_venc_config.venc_conf_com.low_temp_flag == 0)
		return 0;

	need_clk = g_venc_config.venc_conf_com.clk_rate[clk_type];
	current_clk = (uint32_t)clk_get_rate(g_venc_clock_manager[core_id].venc_clk);
	if (need_clk == current_clk && g_venc_config.venc_conf_com.low_temp_flag == 0) {
		g_venc_clock_manager[core_id].curr_clk_type = clk_type;
		return 0;
	}
	g_venc_config.venc_conf_com.low_temp_flag = 0;
...
void venc_set_low_temp_flag(int32_t low_temp_flag)
{
	if (low_temp_flag != 0)
		g_venc_config.venc_conf_com.low_temp_flag = 1;
}
