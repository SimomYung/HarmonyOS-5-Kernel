...
static void *liblinux_kernfs_open(void *ctx, unsigned int flags, unsigned int mode)
{
	...
	if (unlikely(ret < 0)) {
		kernfs_put(kn);
		pr_err("%s: get sysfs path failed, ret=%d\n", __func__, ret);
		return NULL;
		...
		if (IS_ERR(path)) {
			kernfs_put(kn);
			pr_err("%s: acquire sysfs path failed, ret=%d\n", __func__, PTR_ERR(path));
			return NULL;
		}
	}
	kernfs_put(kn);
	...
	if (IS_ERR(filep)) {
		pr_err("%s: file open root failed, ret=%d\n", __func__, PTR_ERR(filep));
		filep = NULL;
	} else {
...
{
	return ns == NULL ? KERNFS_COMMON_NSID : (ns->nsid & NET_NSID_MASK);
}
...
/*
 * liblinux_kernfs_activate() does the excatly same thing as kernfs_activate()
 * but it is already holding a rwsem write lock which is required in
 * kernfs_add_one(). So we do not need to acquire this lock again.
 */
static void liblinux_kernfs_activate(struct kernfs_node *kn,
...
{
	struct kernfs_root *root = NULL;

	BUG_ON(kn == NULL || next == NULL || activate == NULL || remove == NULL);

	root = kernfs_root(kn);
	/* Activate the node in advance to avoid the node is not activated when it is accessed */
	if (!(root->flags & KERNFS_ROOT_CREATE_DEACTIVATED))
		liblinux_kernfs_activate(kn, next, activate);
