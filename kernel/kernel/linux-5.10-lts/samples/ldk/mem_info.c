...
#endif
#ifdef CONFIG_HISI_GPU
	mem_info->hvgr_total_used = (unsigned long)get_hvgr_alloc_pages_num();
#endif
	return 0;
}
...
#define LINBLINUX_DMABUF_MEMTOTAL_MAX (16ULL * 1024 * 1024 * 1024)
#define LINLINUX_DMABUF_THRESHOLD_MAX (2ULL * 1024 * 1024 * 1024)
static inline unsigned long long liblinux_dmabuf_threshold(unsigned long long mem_total)
{
	return (mem_total > LINBLINUX_DMABUF_MEMTOTAL_MAX ? LINLINUX_DMABUF_THRESHOLD_MAX : mem_total >> 3);
}
#endif
...
#endif
#ifdef CONFIG_LIBLINUX_BUDDY_TRACKER
	liblinux_dump_ldk_buddy_tracker_info();
#endif
}
