...
void liblinux_current_priority_get(void)
{
	int rt_prio;
	int nice;
	struct task_struct *cur = current;
...
EXPORT_SYMBOL(liblinux_current_priority_get);
...
void liblinux_enter_nodac(void)
{
	...
	task_thread_info(tsk)->thread_handle.tcb_cref = liblinux_pal_thread_sched_cref();
	liblinux_current_priority_get();
...
void liblinux_enter_nomm(void)
{
	...
	cur->mm = NULL;
	task_thread_info(cur)->thread_handle.tcb_cref = liblinux_pal_thread_sched_cref();
	liblinux_current_priority_get();
	...
	sigemptyset(&cur->pending.signal);
	__set_current_state(TASK_RUNNING);
}
...
void liblinux_enter_basic(void)
{
	task_thread_info(current)->thread_handle.tcb_cref = liblinux_pal_thread_sched_cref();
}
