...
static void hisysevent_show(const char *fmt, va_list args)
{
	...
	log = kmalloc(MSG_MAX_SIZE, GFP_ATOMIC);
	if (log == NULL)
		return;
	...
	ret = vscnprintf(log, MSG_MAX_SIZE, fmt, args);
	if (ret > 0) {
		...
		pr_err("hisysevent report failed to parse logs.: NETLOG");
...
static void netlog_show(const char *fmt, va_list args)
{
	...
	info = kmalloc(MSG_MAX_SIZE, GFP_ATOMIC);
	if (info == NULL)
		return;
	...
	len = vscnprintf(info, MSG_MAX_SIZE, fmt, args);
	if (len > 0) {
		getnstimeofday64(&now);
		len = snprintf_s(log, MSG_MAX_SIZE, MSG_MAX_SIZE - 1,
