...
#ifdef CONFIG_NET_NS
/*
 * because there is a limit that can not do RPC in spinlock，
 * place the deletion outside the spinlock.
 */
static DEFINE_MUTEX(quota_lock);

static void hook_proc_quota(void *__data, int quota_mutex)
{
	if (quota_mutex == QUOTA_LOCK) {
		mutex_lock(&quota_lock);
	} else {
		mutex_unlock(&quota_lock);
	}
}
INIT_VENDOR_HOOK(ldk_vh_proc_quota, hook_proc_quota);
#endif
