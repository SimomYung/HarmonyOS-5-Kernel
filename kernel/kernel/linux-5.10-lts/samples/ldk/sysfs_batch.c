...
static int kernfs_child_node_traverse_locked(struct kernfs_node *kn, void *ns,
					     struct kernfs_node **kns, int *cnt)
{
	int ret = 0;
	...
	ret = kernfs_child_node_traverse(kn, ns, kns, cnt);
...
static void hook_setup_net(void *__data, void *ns, int *ret)
{
	...
	*ret = kernfs_child_node_traverse_locked(netns->loopback_dev->dev.kobj.sd->parent, (void *)netns, kns, &cnt);
	if (*ret != 0) {
		goto out_undo;
	}
	...
	*ret = kernfs_child_node_traverse_locked(class_kobj->sd, (void *)netns, kns, &cnt);
	if (*ret != 0) {
		goto out_undo;
	}
...
static struct kernfs_node *find_dev_class_kn(struct kernfs_node *kn, char *devname, void *ns)
{
	struct rb_node *rbn = NULL;
	struct rb_node *tmp_rbn = NULL;
	struct kernfs_node *pos = NULL;
	...
	while (pos) {
		if ((kernfs_type(pos) == KERNFS_LINK) &&
		    (strcmp(pos->name, devname) == 0) &&
		    (liblinux_get_ns_info_by_kn(pos) == ns)) {
			return pos;
		}
...
int liblinux_register_netdev(void *data)
{
	...
	/* traverse /sys/devices/virtual/net */
	ret = kernfs_child_node_traverse_locked(dev->dev.kobj.sd, (void *)netns, kns, &cnt);
	if (ret != 0) {
		goto out_undo;
	}
	...
	ret = kernfs_child_node_traverse_locked(class_kn, (void *)netns, kns, &cnt);
	if (ret != 0) {
		goto out_undo;
	}
...
static void hook_kobject_add_uevent(void *__data, struct device *dev, struct kobject *kobj)
{
	...
	if (dev->kobj.batch_create && dev_can_create_async()) {
		ret = liblinux_kobject_add_uevent_async((void *)kobj);
		if (ret) {
			kobject_put(kobj);
		}
		return;
...
#endif
	ret = liblinux_kobject_add_uevent((void *)kobj);
	if (ret)
		pr_err("device=%s kobject_uevent failed, ret=%d\n", dev_name(dev), ret);
}
