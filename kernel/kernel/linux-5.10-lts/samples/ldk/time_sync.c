...
static bool time_keeping_inited = false;
...
static void liblinux_timezone_sync(struct liblinux_time_udata *tdata)
{
...
void liblinux_time_sync(void)
{
	...
	liblinux_timezone_sync(&data);
	if (!data.only_update_timezone) {
		liblinux_timekeeper_sync(&data);
		pmu_rtc_synctime(data.rtc_lastts, data.rtc_offset);
 
#ifdef CONFIG_VM_TIME_SYNC
		CALL_HCK_VH(rtc_time_sync, data.real/1000000000);
#endif
	}
}
