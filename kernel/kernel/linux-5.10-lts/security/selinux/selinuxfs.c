...

{
	...
	if (new_value != old_value) {
		...
		if (length)
			goto out;
		trace_ldk_vh_sel_update_enforce(new_value, &length);
...

{
...
#ifdef CONFIG_LIBLINUX
	if (selinux_initialized(fsi->state)) {
		mutex_unlock(&fsi->state->policy_mutex);
		return -ENOTSUPP;
	}
#endif /* CONFIG_LIBLINUX */
	...
	bool hmsel_initialized = false;
	trace_ldk_vh_sel_load_policy(buf, count, &length);
	if (length) {
		goto out;
	}
	hmsel_initialized = true;
...
out:
#ifdef CONFIG_HM_SEPOL_POLICYDB
	/* length == count means succeed to load policy */
	BUG_ON(hmsel_initialized && length != count);
#endif
	mutex_unlock(&fsi->state->policy_mutex);
	vfree(data);
	return length;
...
#ifdef CONFIG_LIBLINUX
	trace_ldk_vh_sel_update_cache_threshold(new_value, &ret);
	if (ret)
		goto out;
#endif /* CONFIG_LIBLINUX */
