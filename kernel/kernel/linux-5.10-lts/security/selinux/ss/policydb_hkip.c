...
#ifndef CONFIG_HM_SEPOL_POLICYDB
...
void policydb_destroy(struct policydb *p)
{
	...
	trace_ldk_vh_sel_role_trans_destroy(p);
#else
	...
	if (p->type_attr_map_array) {
		...
		nop_free(p->type_attr_map_array);
	}
...
static int filename_trans_read_helper_compat(struct policydb *p, void *fp)
{
	...
	if (!datum) {
		rc = -ENOMEM;
		datum = pmalloc(&selinux_pool, sizeof(*datum), PRMEM_NO_FLAGS);
		if (!datum)
			goto out;

		ebitmap_init(&datum->stypes, HKIP_SELINUX_EBITMAP_RO);
		datum->otype = otype;
		datum->next = NULL;
...
static int filename_trans_read_helper(struct policydb *p, void *fp)
{
	...
	for (i = 0; i < ndatum; i++) {
		...
		*dst = datum;
		...
		rc = ebitmap_read(&datum->stypes, fp, HKIP_SELINUX_EBITMAP_RO);
		if (rc)
			goto out;
	...
	if (IS_ENABLED(CONFIG_HM_SEPOL_POLICYDB))
		return 0;
...
int policydb_read(struct policydb *p, void *fp)
{
...
#ifdef CONFIG_HM_SEPOL_POLICYDB
	trace_ldk_vh_sel_policydb_init(p, &rc);
	if (rc) {
		goto out;
	}
#else
...
#ifdef CONFIG_HM_SEPOL_POLICYDB
	trace_ldk_vh_sel_role_trans_read(p, fp, &rc);
	if (rc)
		goto bad;
#else
	rc = next_entry(buf, fp, sizeof(u32));
	...
	for (i = 0; i < nel; i++) {
		rc = -ENOMEM;
		rtk = pmalloc(&selinux_pool, sizeof(*rtk), PRMEM_NO_FLAGS);
		if (!rtk)
			goto bad;

		rc = -ENOMEM;
		rtd = pmalloc(&selinux_pool, sizeof(*rtd), PRMEM_NO_FLAGS);
		if (!rtd)
			goto bad;
	...
	rc = -ENOMEM;
	p->type_attr_map_array = pmalloc(&selinux_pool,
					 p->p_types.nprim * sizeof(*p->type_attr_map_array),
					 PRMEM_NO_FLAGS);
	if (!p->type_attr_map_array)
		goto bad;
	...
	for (i = 0; i < p->p_types.nprim; i++)
		ebitmap_init(&p->type_attr_map_array[i], HKIP_SELINUX_EBITMAP_RO);

	for (i = 0; i < p->p_types.nprim; i++) {
		struct ebitmap *e = &p->type_attr_map_array[i];

		if (p->policyvers >= POLICYDB_VERSION_AVTAB) {
			rc = ebitmap_read(e, fp, HKIP_SELINUX_EBITMAP_RO);
			if (rc)
				goto bad;
		}
...
#ifdef CONFIG_HM_SEPOL_POLICYDB
	trace_ldk_vh_sel_role_trans_write(p, fp, &rc);
	return rc;
#else
