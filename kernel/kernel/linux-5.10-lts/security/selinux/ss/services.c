...
#ifdef CONFIG_HM_SEPOL_POLICYDB
		trace_ldk_vh_sel_role_trans_search(policydb, &newcontext, scontext->role, tcontext->type, tclass);
#else
...
#ifdef CONFIG_HM_SEPOL_POLICYDB
	trace_ldk_vh_sel_policydb_write(&policy->policydb, &fp, data, len, &rc);
#else
