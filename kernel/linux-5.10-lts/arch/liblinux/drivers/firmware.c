				 ...
				 unsigned int size)
{
	...
	rc = liblinux_pal_vfs_read(fd, buf, size);
	if (rc < 0)
		goto fail;
				...
				size_t fw_path_size, int *ret)
{
	...
	for (i = 0; i < fw_path_size; i++) {
		...
		fw_fd = liblinux_pal_vfs_open(path, O_RDONLY, 0);
		if (fw_fd < 0) {
			dev_warn(device, "attempted to open fw %s, but failed %d\n", path, fw_fd);
			continue;
		}
		rc = liblinux_pal_vfs_fstat(fw_fd, &fstat);
		if (rc < 0) {
			dev_warn(device, "firmware, attempted to fstat %s, but failed %d\n", path, rc);
			rc_close = liblinux_pal_vfs_close(fw_fd);
			if (rc_close)
