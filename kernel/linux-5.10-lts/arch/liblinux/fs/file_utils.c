...
static int __firmware_file_fsync(struct file *file, loff_t start, loff_t end, int datasync)
{
	...
	mutex_lock(&internal_fp->flock);
	liblinux_ctx_leave();
	ret = liblinux_pal_ksys_fsync(internal_fp->fd);
	liblinux_ctx_enter();
	mutex_unlock(&internal_fp->flock);
	return ret;
}
...
static loff_t __firmware_file_llseek(struct file *file, loff_t offset, int whence)
{
	...
	mutex_lock(&internal_fp->flock);
	ret = liblinux_pal_ksys_lseek(internal_fp->fd, offset, whence, &tmp);
	if ((ret == 0) && (tmp >= 0)) {
		file->f_pos = tmp;
	}
	mutex_unlock(&internal_fp->flock);
			      ...
			      const char *name, long size, long ctime_sec)
{
	...
	liblinux_ctx_leave();
	ret = liblinux_pal_ksys_read(fd, cache_info->buf, size);
	liblinux_ctx_enter();
	if (ret < 0) {
...
struct file __override *filp_open(const char *filename, int flags, umode_t mode)
{
	...
	liblinux_ctx_leave();
	fd = liblinux_pal_ksys_open(filename, flags, mode);
	liblinux_ctx_enter();
	if (fd < 0) {
...
static void __filp_priv_close(struct file *filp)
{
	...
	mutex_lock(&internal_fp->flock);
	liblinux_ctx_leave();
	liblinux_pal_ksys_close(internal_fp->fd);
	liblinux_ctx_enter();
	mutex_unlock(&internal_fp->flock);
	kfree(internal_fp);
...
int __override vfs_fchown(struct file *filp, uid_t user, gid_t group)
{
	...
	mutex_lock(&fp->flock);
	ret = liblinux_pal_ksys_fchown(fp->fd, user, group);
			   ...
			   int flags)
{
	...
	ret = liblinux_pal_ksys_fstatat(dfd, filename, &stat, (unsigned int)flags);
	if (ret < 0)
		return -ENOENT;
...
long __override do_unlinkat(int dfd, struct filename *name)
{
	...
	ret = liblinux_pal_ksys_unlink(name->uptr);
exit:
	putname(name);
	return ret;
}
