...
extern int lnxbase_iofast_alloc_pfn_v(unsigned int order, unsigned long long *pa_array,
				      unsigned long num, unsigned int flags,
				      unsigned int *pa_num);
extern int lnxbase_iofast_free_pfn_v(unsigned long long *pa_array, unsigned int pa_num);
			   ...
			   unsigned long pfn, unsigned int order)
{
	...
	if (page_zone(pg) != page_zone(pg + nr_pages - 1)) {
...
__extend_zone(struct zonelist *zonelist, int extend_order, gfp_t gfp_mask)
{
	...
	ret = lnxbase_iofast_alloc_pfn_v(extend_order,
	...
	for (i = 0; i < pa_num; i++) {
		...
		if (__populate_vmemmap(PFN_PHYS(pfn), size, gfp_zone(gfp_mask)) == NULL) {
			pr_err("__extend_zone error 0x%lx 0x%lx\n", pfn, size);
...
static unsigned long zone_do_shrink_order(int order, gfp_t gfp_mask)
{
	...
	if (lnxbase_iofast_free_pfn_v(&pa, 1) != 0) {
