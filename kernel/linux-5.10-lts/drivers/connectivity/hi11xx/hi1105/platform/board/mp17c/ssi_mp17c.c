...
static ssi_reg_info g_sharemem_fix_pos = { 0x04040000, 0x0, 0x1000,  SSI_RW_DWORD_MOD };
...
static ssi_reg_info g_pcie_mac_part0 = { 0x4908000, 0x0, 0x85c,  SSI_RW_DWORD_MOD };   // AP_GLOBAL_REG
static ssi_reg_info g_pcie_mac_part1 = { 0x4940000, 0x0, 0x100,  SSI_RW_DWORD_MOD };   // EPF_CFGSPACE
static ssi_reg_info g_pcie_mac_part2 = { 0x495e000, 0x400, 0x800,  SSI_RW_DWORD_MOD }; // CORE_GLOBAL_CTRL_REG
...
{
    uint32_t data = mp17c_ssi_spmi_read(CHIP_VERSION_2);
...
static void pmu_status_error_dsm_report(void)
{
    ...
    hw_mpxx_dsm_client_notify(SYSTEM_TYPE_PLATFORM, DSM_BUCK_PROTECTED, "get pmu reg:%s", dsm_buf);
#else
...
static int ssi_dump_device_pciectrl_regs(unsigned long long module_set)
{
    ...
    if (module_set & SSI_MODULE_MASK_PCIE_CFG) {
        ...
        if (ssi_check_pcie_status() == OAL_SUCC) {
            ...
            ret = ssi_read_reg_info(&g_pcie_mac_part0, NULL, 0, g_ssi_is_logfile);
            if (ret) {
                return ret;
            }
        } else {
