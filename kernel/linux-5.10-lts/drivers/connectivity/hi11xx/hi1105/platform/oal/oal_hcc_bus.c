...
OAL_STATIC int32_t wait_wlan_wkup_gpio_level(int32_t gpio_level, unsigned long timeout, hcc_bus *hi_bus)
{
    ...
    gpio_type = (hi_bus->dev_id == HCC_EP_GT_DEV) ? GT_WKUP_HOST : W_WKUP_HOST;

    forever_loop() {
        gpio_value = conn_get_gpio_level(gpio_type);
...
OAL_STATIC int32_t panic_status_check_by_gpio(hcc_bus *hi_bus)
{
    ...
    if (((conn_gpio_valid(W_WKUP_HOST) == OAL_FALSE) && (hi_bus->dev_id == HCC_EP_WIFI_DEV)) ||
        ((conn_gpio_valid(GT_WKUP_HOST) == OAL_FALSE) && (hi_bus->dev_id == HCC_EP_GT_DEV))) {
        ret = OAL_FALSE;
    } else {
