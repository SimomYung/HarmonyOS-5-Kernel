...
#include "chip/mp17c/pcie_chip_mp17c.h"
#include "chip/mp12/pcie_chip_mp12.h"
...
static pcie_chip_id_stru g_pcie_chipinfo_id[] = {
    ...
    {PCIE_HISI_DEVICE_ID_MP17CMPW, oal_pcie_chip_info_init_mp16c},
    {PCIE_HISI_DEVICE_ID_MP17C,    oal_pcie_chip_info_init_mp17c},
    {PCIE_HISI_DEVICE_ID_MP12,     oal_pcie_chip_info_init_mp12}
};
...
static board_chip_id_stru g_board_chipinfo_id[] = {
    ...
    {BOARD_VERSION_MP17C, oal_pcie_board_init_mp17c},
    {BOARD_VERSION_MP12, oal_pcie_board_init_mp12},
};
...
{
    if (pst_pci_res->chip_info.cb.pcie_check_gen == NULL) {
        oal_print_mpxx_log(MPXX_LOG_INFO, "pcie_check_gen is null");
        return -OAL_ENODEV;
    }
    return pst_pci_res->chip_info.cb.pcie_check_gen(pst_pci_dev, pst_pci_res);
}

int32_t oal_pcie_check_ltssm(oal_pci_dev_stru *pst_pci_dev, oal_pcie_res *pst_pci_res)
{
    if (pst_pci_res->chip_info.cb.pcie_check_ltssm == NULL) {
        oal_print_mpxx_log(MPXX_LOG_INFO, "pcie_check_ltssm is null");
        return -OAL_ENODEV;
    }
    return pst_pci_res->chip_info.cb.pcie_check_ltssm(pst_pci_dev, pst_pci_res);
}
...
{
    if (pst_pci_res->chip_info.cb.pcie_get_link_spend_sts == NULL) {
        oal_print_mpxx_log(MPXX_LOG_INFO, "pcie_get_link_spend_sts is null");
        return;
    }
    return pst_pci_res->chip_info.cb.pcie_get_link_spend_sts(pst_pci_dev, pst_pci_res, link_speed, link_sts);
}

void* oal_pcie_get_ctrl_address(oal_pci_dev_stru *pst_pci_dev, oal_pcie_res *pst_pci_res)
{
    if (pst_pci_res->chip_info.cb.pcie_get_ctrl_address == NULL) {
        oal_print_mpxx_log(MPXX_LOG_INFO, "pcie_get_ctrl_address is null");
        return NULL;
    }
    return pst_pci_res->chip_info.cb.pcie_get_ctrl_address(pst_pci_dev, pst_pci_res);
}

uintptr_t oal_pcie_get_pcs_paddr(oal_pci_dev_stru *pst_pci_dev, oal_pcie_res *pst_pci_res, uint32_t dump_type)
{
    if (pst_pci_res->chip_info.cb.pcie_get_pcs_paddr == NULL) {
        oal_print_mpxx_log(MPXX_LOG_INFO, "pcie_get_pcs_paddr is null");
        return (uintptr_t)NULL;
    }
    return pst_pci_res->chip_info.cb.pcie_get_pcs_paddr(pst_pci_dev, pst_pci_res, dump_type);
}
