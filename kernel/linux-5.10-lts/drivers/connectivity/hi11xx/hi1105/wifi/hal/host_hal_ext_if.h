...
static inline hal_host_device_stru *hal_host_rx_get_mpdu_link(hal_host_device_stru *hal_device, oal_netbuf_stru *mpdu)
{
    if (g_hal_common_ops->host_rx_get_mpdu_link != NULL) {
        return g_hal_common_ops->host_rx_get_mpdu_link(hal_device, mpdu);
    }
...
static inline  uint16_t hal_tx_rw_smaller_ptr(uint16_t rptr_val1, uint16_t rptr_val2)
{
    un_rw_ptr rptr1 = { .rw_ptr = rptr_val1 };
    un_rw_ptr rptr2 = { .rw_ptr = rptr_val2 };

    if (hal_tx_rw_ptr_compare(rptr1, rptr2) == RING_PTR_GREATER) {
        return rptr_val2;
    } else {
        return rptr_val1;
    }
    ...
    uint64_t *info_ring_devva, uint64_t *addr_ring_devva)
{
    if (g_hal_common_ops->host_tx_get_msdu_info_devva != NULL) {
        return g_hal_common_ops->host_tx_get_msdu_info_devva(msdu_ring, info_ring_devva, addr_ring_devva);
    }
    return OAL_FAIL;
}
...
static inline uint32_t hal_host_tx_init_tid_ring_table(uint8_t lut_idx, uint8_t tid, uint64_t *hostva)
{
    if (g_hal_common_ops->host_tx_init_tid_ring_table != NULL) {
        return g_hal_common_ops->host_tx_init_tid_ring_table(lut_idx, tid, hostva);
    }
    return OAL_FAIL;
}
...
static inline void hal_host_unmask_slave_interrupt(hal_host_device_stru *hal_device)
{
    if (g_hal_common_ops->host_unmask_slave_interrupt != NULL) {
        g_hal_common_ops->host_unmask_slave_interrupt(hal_device);
    }
}
