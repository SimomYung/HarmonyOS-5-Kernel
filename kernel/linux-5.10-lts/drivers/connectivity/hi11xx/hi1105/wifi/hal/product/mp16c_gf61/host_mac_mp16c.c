...
/* 功能描述 : unmask PCIE中断屏蔽bit */
void mp16c_host_soc_int_unmask(hal_host_device_stru *hal_device, uint32_t irq)
{
    uint32_t intr_mask;
    intr_mask = hal_host_readl(hal_device->soc_regs.host_intr_mask_reg);
    if (intr_mask == HAL_HOST_READL_INVALID_VAL) {
...
void mp16c_host_unmask_slave_interrupt(hal_host_device_stru *hal_device)
{
    mp16c_host_soc_int_unmask(hal_device, MP16C_PCIE_CTRL_RB_MAC_N2_INTR_STATUS_OFFSET);
}
...
uint32_t mp16c_host_get_tsf_lo(hal_host_device_stru *hal_device, uint8_t hal_vap_id, uint32_t *tsf)
{
    ...
    reg_addr = hal_device->mac_regs.tsf_lo_reg + hal_get_vap_reg_offset(hal_vap_id);
