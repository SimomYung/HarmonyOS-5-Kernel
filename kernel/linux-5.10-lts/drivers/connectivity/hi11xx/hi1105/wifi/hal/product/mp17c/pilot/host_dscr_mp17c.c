...
oal_netbuf_stru *mp17c_rx_get_next_sub_msdu(hal_host_device_stru *hal_device,
...
void mp17c_host_rx_amsdu_list_build(hal_host_device_stru *hal_device, oal_netbuf_stru *netbuf)
{
    oal_netbuf_stru *next_netbuf = NULL;

    while (netbuf != NULL) {
        next_netbuf = mp17c_rx_get_next_sub_msdu(hal_device, netbuf);
        oal_netbuf_next(netbuf) = next_netbuf;
        netbuf = next_netbuf;
    }
}

uint8_t mp17c_host_get_rx_msdu_status(oal_netbuf_stru *netbuf)
{
    mp17c_rx_msdu_dscr_stru *rx_hw_dscr = NULL;
...
uint8_t mp17c_rx_is_frag_frm(mac_header_frame_control_stru *fc_ctl_hdr, mp17c_rx_msdu_dscr_stru *rx_msdu_hw_dscr)
{
...
static void mp17c_host_rx_cb_set(hal_host_device_stru *hal_dev,
    mac_rx_ctl_stru *rx_ctl, mp17c_rx_msdu_dscr_stru *rx_hw_dscr, uint8_t first_msdu)
{
    ...
    if (first_msdu) {
        ...
        if (mp17c_rx_is_frag_frm(fc_ctl_hdr, rx_hw_dscr)) {
 ...
 *   修改内容  : 新生成函数
 */
static uint32_t mp17c_host_rx_adjust_netbuf_len(hal_host_device_stru *hal_dev,
    oal_netbuf_stru *netbuf, mac_rx_ctl_stru *rx_ctl, mp17c_rx_msdu_dscr_stru *rx_hw_dscr, uint8_t first_msdu)
{
    ...
    if ((rx_hw_dscr->rx_status == HAL_RX_SUCCESS) &&
        (rx_hw_dscr->packet_len == 0 || (rx_hw_dscr->packet_len > (cb_res->len - MP17C_RX_MSDU_DSCR_LEN)))) {
        ...
        oam_report_dscr(BROADCAST_MACADDR, (uint8_t *)rx_hw_dscr, MP17C_RX_MSDU_DSCR_LEN, OAM_OTA_TYPE_RX_MSDU_DSCR);
        return OAL_FAIL;
    }
...
{
    mp17c_rx_msdu_dscr_stru      *rx_msdu_hw_dscr = NULL;
    uint32_t                       ret;

    memset_s(rx_ctl, sizeof(mac_rx_ctl_stru), 0, sizeof(mac_rx_ctl_stru));
    rx_msdu_hw_dscr = (mp17c_rx_msdu_dscr_stru *)oal_netbuf_data(netbuf);
    mp17c_host_rx_cb_set(hal_dev, rx_ctl, rx_msdu_hw_dscr, first_msdu);
    ret = mp17c_host_rx_adjust_netbuf_len(hal_dev, netbuf, rx_ctl, rx_msdu_hw_dscr, first_msdu);
    if (ret != OAL_SUCC) {
        return ret;
    }
...
{
    mp17c_rx_msdu_dscr_stru *msdu_dscr = (mp17c_rx_msdu_dscr_stru *)oal_netbuf_data(mpdu);
    uint8_t link_id;
...
void mp17c_host_rx_tcp_udp_checksum(oal_netbuf_stru *netbuf)
{
...
oal_bool_enum_uint8 mp17c_host_rx_ip_checksum_is_pass(oal_netbuf_stru *netbuf)
{
...
uint32_t mp17c_host_rx_buff_recycle(hal_host_device_stru *hal_device, oal_netbuf_head_stru *netbuf_head)
{
    ...
    while ((netbuf = oal_netbuf_delist_nolock(netbuf_head)) != NULL) {
        ...
        if (netbuf_total_len < LARGE_NETBUF_SIZE + MP17C_RX_MSDU_DSCR_LEN) {
        ...
        oal_netbuf_set_len(netbuf, LARGE_NETBUF_SIZE + MP17C_RX_MSDU_DSCR_LEN);
