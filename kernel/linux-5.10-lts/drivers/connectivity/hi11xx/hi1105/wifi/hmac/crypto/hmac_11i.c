...
static oal_bool_enum_uint8 hmac_11i_is_bigtk_key_id(uint32_t key_index)
{
    /* 6/7 是BIGTK key_id */
    return (key_index == 6 || key_index == 7) ? OAL_TRUE : OAL_FALSE;
}

static wlan_priv_key_param_stru *hmac_11i_get_bigtk(mac_vap_stru *mac_vap, uint32_t key_index)
{
    ...
    if (hmac_11i_is_bigtk_key_id(key_index) == OAL_FALSE) {
        return NULL;
    }
    return &hmac_vap->bigtk_key[key_index - WLAN_BIGTK_START_KEY_ID];
}
    ...
    oal_bool_enum_uint8 en_pairwise, uint8_t uc_key_index, uint16_t *pus_user_idx)
{
    ...
    if (hmac_11i_is_bigtk_key_id(uc_key_index)) {
        return hmac_11i_get_bigtk(pst_mac_vap, uc_key_index);
    } else {
...
static void hmac_11i_print_mld_key_info(hmac_vap_stru *hmac_vap, hmac_mld_key_info *mld_key_infos)
{
    uint32_t i;
    hmac_mld_link_key_info *key_info = NULL;
    oam_warning_log4(hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_WPA,
        "MLD: mld key info. link_num %d, key_len %d, mld_addr %02x:%02x ",
        mld_key_infos->link_num, mld_key_infos->key_len,
        mld_key_infos->mld_addr[MAC_ADDR_4], mld_key_infos->mld_addr[MAC_ADDR_5]);
    for (i = 0; i < mld_key_infos->link_num; i++) {
        if (i >= WLAN_MAX_ML_LINK_NUM) {
            oam_warning_log1(hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_WPA,
                "MLD: invalid link %d", i);
            return;
        }
        key_info = &(mld_key_infos->link_key_info[i]);
        oam_warning_log3(hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_WPA,
            "MLD:%d link %d, key_index %d",
            i, key_info->link_id, key_info->key_index);
        oam_warning_log4(hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_WPA,
            "MLD: %02x %02x %02x %02x",
            key_info->key[0], key_info->key[1], key_info->key[2], key_info->key[3]); /* 输出数组0/1/2/3字节 */
    }
}
#endif

uint32_t hmac_config_sta_set_mld_key_info(mac_vap_stru *mac_vap, uint8_t *param)
{
#ifdef _PRE_WLAN_FEATURE_ML
    hmac_vap_stru *hmac_vap = NULL;
    hmac_mld_manage_stru *mld_mgm = NULL;

    if (oal_any_null_ptr2(mac_vap, param)) {
        oam_error_log0(0, OAM_SF_CFG, "{hmac_config_sta_set_mld_key_info::argument is null!}");
        return OAL_FAIL;
    }

    hmac_vap = mac_res_get_hmac_vap(mac_vap->uc_vap_id);
    if (hmac_vap == NULL) {
        oam_error_log0(mac_vap->uc_vap_id, OAM_SF_CFG, "{hmac_config_sta_set_mld_key_info::can't get hmac_vap!}");
        return OAL_FAIL;
    }

    mld_mgm = hmac_vap->mld_vap_link_info.mld_vap_manage;
    if (mld_mgm == NULL) {
        oam_error_log0(mac_vap->uc_vap_id, OAM_SF_CFG, "{hmac_config_sta_set_mld_key_info::can't get mld_vap_manage!}");
        return OAL_FAIL;
    }

    if (mld_mgm->mld_key_infos != NULL) {
        oal_free(mld_mgm->mld_key_infos);
        mld_mgm->mld_key_infos = NULL;
    }

    mld_mgm->mld_key_infos = (hmac_mld_key_info *)oal_memalloc(sizeof(hmac_mld_key_info));
    if (mld_mgm->mld_key_infos == NULL) {
        oam_error_log0(mac_vap->uc_vap_id, OAM_SF_CFG, "{hmac_config_sta_set_mld_key_info::oal_memalloc fail!}");
        return OAL_FAIL;
    }

    memcpy_s(mld_mgm->mld_key_infos, sizeof(hmac_mld_key_info), (hmac_mld_key_info *)param, sizeof(hmac_mld_key_info));
    hmac_11i_print_mld_key_info(hmac_vap, mld_mgm->mld_key_infos);
#endif
...
{
    hmac_last_mcast_pn_stru *last_mcast_pn = hmac_get_last_mcast_pn(hmac_user);
    hmac_last_ucast_pn_stru *last_ucast_pn = hmac_get_last_ucast_pn(hmac_user);
 ...
 *   修改内容   : 新生成函数
 */
void hmac_user_clear_rx_pn(mac_vap_stru *mac_vap, hmac_user_stru *hmac_user, uint8_t pairwise)
{
    ...
    if (mac_vap_find_user_by_macaddr(mac_vap, mac_vap->auc_bssid, &user_idx) != OAL_SUCC) {
    ...
    mac_addkey_param_stru *addkey_params)
{
    ...
    if ((mac_vap->en_vap_mode == WLAN_VAP_MODE_BSS_STA) && (mac_vap->en_vap_state == MAC_VAP_STATE_ROAMING)) {
        if (hmac_vap_is_legacy_mld_mode (hmac_vap) == OAL_FALSE && (pairwise == OAL_TRUE)) {
            hmac_roam_add_key_done(hmac_vap);
        }
    }
    return ret;
...
static uint32_t hmac_config_11i_close_port(mac_vap_stru *mac_vap, hmac_user_stru *hmac_user)
{
#ifdef _PRE_WLAN_FEATURE_WAPI
    ...
    if (mac_vap->pwl_enable == OAL_TRUE) {
        if ((hmac_pwl_set_port(&hmac_user->pwl_pre, OAL_FALSE) != OAL_SUCC) ||
            (hmac_pwl_set_port(&hmac_user->pwl, OAL_FALSE) != OAL_SUCC)) {
            return OAL_FAIL;
        }
    }
#endif
...
#ifdef _PRE_WLAN_FEATURE_ML
#ifndef CONFIG_HISI_WIFI7_MLO
/* 两个link的gtk/igtk/bigtk都安装完成后，释放mld管理结构体中临时保存的密钥信息 */
OAL_STATIC void hmac_free_mld_key_info(hmac_vap_stru *hmac_vap)
{
    hmac_mld_key_info *mld_key_infos = NULL;

    if (hmac_vap->mld_vap_link_info.mld_vap_manage == NULL) {
        return;
    }

    mld_key_infos = hmac_vap->mld_vap_link_info.mld_vap_manage->mld_key_infos;
    if ((mld_key_infos != NULL) && (mld_key_infos->link_num > 0)) {
        mld_key_infos->link_num--;
        if (mld_key_infos->link_num == 0) {
            oal_free(mld_key_infos);
            hmac_vap->mld_vap_link_info.mld_vap_manage->mld_key_infos = NULL;
        }
    }
}

OAL_STATIC hmac_vap_stru *hmac_get_link_vap_by_key(hmac_vap_stru *hmac_vap, uint8_t *key, int32_t key_len,
    uint8_t key_index)
{
    hmac_mld_key_info *mld_key_infos = NULL;
    uint8_t i;
    ...
    mld_key_infos = hmac_vap->mld_vap_link_info.mld_vap_manage->mld_key_infos;
    /* sta是mld, ap是非mld ap */
    if (mld_key_infos == NULL) {
        return hmac_vap;
    }
    oam_warning_log2(hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_WPA,
        "MLD: hmac_get_link_vap_by_key len %d, idx %d",
        key_len, key_index);
    oam_warning_log4(hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_WPA,
        "MLD: %02x %02x %02x %02x",
        key[0], key[1], key[2], key[3]); /* 输出数组0/1/2/3字节 */
    /* 从mld管理结构体 找到本次add的key 所匹配的link vap，对匹配的link vap配置秘钥 */
    for (i = 0; i < WLAN_MAX_ML_LINK_NUM; i++) {
        if ((mld_key_infos->key_len == key_len) && (mld_key_infos->link_key_info[i].key_index == key_index) &&
            (oal_memcmp(key, mld_key_infos->link_key_info[i].key, key_len) == 0)) {
            return hmac_vap->mld_vap_link_info.mld_vap_manage->link_vap[i];
        }
    }
    return NULL;
}
#endif
OAL_STATIC uint32_t hmac_config_mld_add_ptk(hmac_vap_stru *hmac_vap, uint16_t len, uint8_t *param)
{
    mac_addkey_param_stru tmp_addkey_param;
    uint8_t i;
    uint8_t ret = 0;
    ...
    memcpy_s(&tmp_addkey_param, sizeof(mac_addkey_param_stru), param, sizeof(mac_addkey_param_stru));

    /* 先配置link_vap密钥 */
    for (i = 0; i < WLAN_MAX_ML_LINK_NUM; i++) {
        hmac_link_vap = hmac_vap->mld_vap_link_info.mld_vap_manage->link_vap[i];
        if (hmac_link_vap == NULL) {
            continue;
        }
        if (hmac_vap_is_ml_mode(hmac_link_vap) == OAL_TRUE) {
            continue;
        }
        /* TODO:对于ap，需要增加通过mld user mac地址来找到对应user的接口 */
        if (is_sta(&hmac_link_vap->st_vap_base_info)) {
            memcpy_s(tmp_addkey_param.auc_mac_addr, WLAN_MAC_ADDR_LEN,
                hmac_link_vap->st_vap_base_info.auc_bssid, WLAN_MAC_ADDR_LEN);
        }

        ret += hmac_config_11i_add_key(&hmac_link_vap->st_vap_base_info, len, (uint8_t *)&tmp_addkey_param);
    }

    /* 后配置ml_link vap密钥 */
    hmac_link_vap = hmac_vap->mld_vap_link_info.mld_vap_manage->mld_vap;
    if (hmac_link_vap == NULL) {
        return OAL_FAIL;
    }
    if (is_sta(&hmac_link_vap->st_vap_base_info)) {
...
OAL_STATIC uint32_t hmac_config_mld_add_gtk(hmac_vap_stru *hmac_vap, uint16_t len, uint8_t *param)
{
    hmac_vap_stru *hmac_link_vap = NULL;
    mac_addkey_param_stru *addkey_param = (mac_addkey_param_stru *)param;

#ifdef CONFIG_HISI_WIFI7_MLO
    hmac_link_vap = hmac_mld_get_sta_vap_by_ap_link_id(hmac_vap, addkey_param->link_id);
#else
    hmac_link_vap = hmac_get_link_vap_by_key(hmac_vap, addkey_param->st_key.auc_key,
        addkey_param->st_key.key_len, addkey_param->uc_key_index);
#endif
    if (hmac_link_vap == NULL) {
        oam_error_log0(hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_WPA,
            "{hmac_config_mld_add_gtk::link_vap is NULL}");
        return OAL_FAIL;
    }

    return hmac_config_11i_add_key(&hmac_link_vap->st_vap_base_info, len, param);
}
#endif

/* 功能描述：保存bigtk 到hmac_vap 和透传密钥到device */
static uint32_t hmac_config_11i_add_bigtk(hmac_vap_stru *hmac_vap, mac_addkey_param_stru *addkey_param)
{
    uint32_t ret;
    int32_t l_ret;
    hmac_vap_stru *hmac_vap_tmp = hmac_vap;
    wlan_priv_key_param_stru *bigtk_key = NULL;

    if ((g_expand_feature_switch_bitmap & CUSTOM_BEACON_PROTECTION) == 0) {
        /* 不支持beacon 保护，不用配置BIGTK */
        return OAL_ERR_CODE_SECURITY_INSTALL_BIGTK;
    }

#ifdef _PRE_WLAN_FEATURE_ML
    if (hmac_vap_is_ml_mode(hmac_vap) == OAL_TRUE) {
#ifdef CONFIG_HISI_WIFI7_MLO
        hmac_vap_tmp = hmac_mld_get_sta_vap_by_ap_link_id(hmac_vap, addkey_param->link_id);
#else
        hmac_vap_tmp = hmac_get_link_vap_by_key(hmac_vap, addkey_param->st_key.auc_key,
            addkey_param->st_key.key_len, addkey_param->uc_key_index);
#endif
        if (hmac_vap_tmp == NULL) {
            oam_error_log0(hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_WPA,
                "{hmac_config_11i_add_bigtk::could not find link vap by key");
#ifndef CONFIG_HISI_WIFI7_MLO
            hmac_free_mld_key_info(hmac_vap);
#endif
            return OAL_FAIL;
        }
#ifndef CONFIG_HISI_WIFI7_MLO
...
#endif
    mac_mib_set_BeaconProtectionEnabled(&(hmac_vap_tmp->st_vap_base_info), OAL_TRUE);

    /* 保存bigkt到hmac_vap */
    bigtk_key = &hmac_vap->bigtk_key[addkey_param->uc_key_index - WLAN_BIGTK_START_KEY_ID];
    bigtk_key->cipher = addkey_param->st_key.cipher;
    bigtk_key->key_len = (uint32_t)addkey_param->st_key.key_len;
    bigtk_key->seq_len = (uint32_t)addkey_param->st_key.seq_len;

    l_ret = memcpy_s(bigtk_key->auc_key, WLAN_WPA_KEY_LEN,
        addkey_param->st_key.auc_key, (uint32_t)addkey_param->st_key.key_len);
    l_ret += memcpy_s(bigtk_key->auc_seq, WLAN_WPA_SEQ_LEN,
        addkey_param->st_key.auc_seq, (uint32_t)addkey_param->st_key.seq_len);
    if (l_ret != EOK) {
        oam_warning_log1(hmac_vap_tmp->st_vap_base_info.uc_vap_id, OAM_SF_WPA,
            "{hmac_config_11i_add_bigtk::copy key fail[%d].}", l_ret);
    }

    ret = hmac_config_send_event(&hmac_vap_tmp->st_vap_base_info, WLAN_CFGID_ADD_KEY,
        sizeof(mac_addkey_param_stru), (uint8_t *)addkey_param);
    if (ret != OAL_SUCC) {
        oam_warning_log1(hmac_vap_tmp->st_vap_base_info.uc_vap_id, OAM_SF_WPA,
            "{hmac_config_11i_add_bigtk::send event failed[%d].}", ret);
    }
    return ret;
}

uint32_t hmac_config_add_key(mac_vap_stru *mac_vap, uint16_t len, uint8_t *param)
{
    hmac_vap_stru *hmac_vap = NULL;
    ...
    if (oal_any_null_ptr2(mac_vap, param)) {
        oam_error_log0(0, OAM_SF_WPA, "{hmac_config_add_key::invalid arguments.}");
        return OAL_ERR_CODE_PTR_NULL;
    }

    hmac_vap = (hmac_vap_stru *)mac_res_get_hmac_vap(mac_vap->uc_vap_id);
    if (hmac_vap == NULL) {
        oam_error_log0(mac_vap->uc_vap_id, OAM_SF_WPA, "{hmac_config_add_key::hmac_vap is NULL}");
        return OAL_ERR_CODE_PTR_NULL;
    }

    if (hmac_11i_is_bigtk_key_id(addkey_params->uc_key_index) == OAL_TRUE) {
        return hmac_config_11i_add_bigtk(hmac_vap, addkey_params);
    }
#ifdef _PRE_WLAN_FEATURE_ML
    if ((hmac_vap_is_ml_mode(hmac_vap) == OAL_TRUE) && (addkey_params->en_pairwise == OAL_TRUE)) {
        return hmac_config_mld_add_ptk(hmac_vap, len, param);
    } else if (hmac_vap_is_ml_mode(hmac_vap) == OAL_TRUE) {
        ret = hmac_config_mld_add_gtk(hmac_vap, len, param);
#ifndef CONFIG_HISI_WIFI7_MLO
        hmac_free_mld_key_info(hmac_vap);
#endif
        return ret;
    ...
    return hmac_config_11i_add_key(mac_vap, len, param);
}
...
uint32_t hmac_config_11i_add_key(mac_vap_stru *mac_vap, uint16_t len, uint8_t *param)
{
...
#ifdef _PRE_WLAN_FEATURE_WAPI
    if (oal_unlikely(addkey_params->st_key.cipher == WLAN_CIPHER_SUITE_SMS4)) {
        return hmac_config_wapi_add_key_and_sync(mac_vap, addkey_params);
    }
#endif
#ifdef _PRE_WLAN_FEATURE_PWL
    if ((mac_vap->pwl_enable == OAL_TRUE) && (hmac_is_pwl_cipher(addkey_params) == OAL_TRUE)) {
    ...
    if (hmac_config_11i_close_port(mac_vap, hmac_user) != OAL_SUCC) {
        return OAL_FAIL;
    }
...
OAL_STATIC wlan_priv_key_param_stru *hmac_get_key_11i(mac_vap_stru *mac_vap,
    hmac_getkey_param_stru *payload_getkey_params, uint8_t *mac_addr,
    oal_bool_enum_uint8 pairwise, uint8_t key_index, uint16_t *user_idx)
{
    wlan_priv_key_param_stru *priv_key = NULL;
#if (defined(_PRE_WLAN_FEATURE_ML) && defined(CONFIG_HISI_WIFI7_MLO))
    int32_t link_id;
    hmac_vap_stru *hmac_vap = NULL;
    hmac_vap_stru *hmac_vap_tmp = NULL;
    hmac_vap = (hmac_vap_stru *)mac_res_get_hmac_vap(mac_vap->uc_vap_id);
    if (hmac_vap == NULL) {
        oam_error_log0(mac_vap->uc_vap_id, OAM_SF_CFG, "{hmac_get_key_11i::hmac_vap null.}");
        return NULL;
    }
    link_id = payload_getkey_params->link_id;
    hmac_vap_tmp = hmac_mld_get_sta_vap_by_ap_link_id(hmac_vap, link_id);
    if (hmac_vap_tmp == NULL) {
        oam_error_log0(mac_vap->uc_vap_id, OAM_SF_CFG, "{hmac_get_key_11i::hmac_vap_tmp is null}");
        return NULL;
    }

    priv_key = hmac_get_key_info(&hmac_vap_tmp->st_vap_base_info, mac_addr,
        pairwise, key_index, user_idx);
#else
    priv_key = hmac_get_key_info(mac_vap, mac_addr, pairwise, key_index, user_idx);
#endif
    return priv_key;
}
...
uint32_t hmac_config_11i_get_key(mac_vap_stru *pst_mac_vap, uint16_t us_len, uint8_t *puc_param)
{
    ...
    /* 2.2 索引值最大值检查 */
    max_key_index = ((g_expand_feature_switch_bitmap & CUSTOM_BEACON_PROTECTION) != 0) ?
        (WLAN_NUM_TK + WLAN_NUM_IGTK + WLAN_NUM_BIGTK) :
        (WLAN_NUM_TK + WLAN_NUM_IGTK);
    if (uc_key_index >= max_key_index) {
        oam_error_log1(pst_mac_vap->uc_vap_id, OAM_SF_WPA,
            "{hmac_config_11i_get_key::uc_key_index invalid[%d].}", uc_key_index);
        return OAL_ERR_CODE_SECURITY_KEY_ID;
    }

    pst_priv_key = hmac_get_key_11i(pst_mac_vap, pst_payload_getkey_params, puc_mac_addr,
        en_pairwise, uc_key_index, &us_user_idx);
...
uint32_t hmac_config_11i_remove_key(mac_vap_stru *mac_vap, uint16_t len, uint8_t *param)
{
    ...
    /* 4.1 区分是wep还是wpa */
    if ((priv_key->cipher == WLAN_CIPHER_SUITE_WEP40) || (priv_key->cipher == WLAN_CIPHER_SUITE_WEP104)) {
...
#ifdef _PRE_WLAN_FEATURE_ML
OAL_STATIC uint32_t hmac_mld_config_remove_ptk(hmac_vap_stru *hmac_vap, uint16_t len, uint8_t *param)
{
    uint8_t i;
    uint8_t ret = 0;
    hmac_vap_stru *hmac_link_vap = NULL;
    mac_removekey_param_stru tmp_removekey_param;

    memcpy_s(&tmp_removekey_param, sizeof(mac_removekey_param_stru), param, sizeof(mac_removekey_param_stru));
#ifdef CONFIG_HISI_WIFI7_MLO
    if (tmp_removekey_param.link_id == -1) {
#endif
        for (i = 0; i < WLAN_MAX_ML_LINK_NUM; i++) {
            if (hmac_vap->mld_vap_link_info.mld_vap_manage == NULL) {
                return OAL_FAIL;
        ...
        /* TODO:对于ap，需要增加通过mld user mac地址来找到对应user的接口 */
        if (is_sta(&hmac_link_vap->st_vap_base_info)) {
            memcpy_s(tmp_removekey_param.auc_mac_addr, WLAN_MAC_ADDR_LEN,
                hmac_link_vap->st_vap_base_info.auc_bssid, WLAN_MAC_ADDR_LEN);
        }

            ret += hmac_config_11i_remove_key(&hmac_link_vap->st_vap_base_info, len, (uint8_t *)&tmp_removekey_param);
        }
#ifdef CONFIG_HISI_WIFI7_MLO
    } else {
            hmac_link_vap = hmac_mld_get_sta_vap_by_ap_link_id(hmac_vap, tmp_removekey_param.link_id);
            if (hmac_link_vap == NULL) {
                return OAL_FAIL;
            }
            if (is_sta(&hmac_link_vap->st_vap_base_info)) {
...
OAL_STATIC uint32_t hmac_mld_config_remove_gtk(hmac_vap_stru *hmac_vap, uint16_t len, uint8_t *param)
{
    uint32_t ret = 0;
    uint8_t i;
    hmac_vap_stru *hmac_link_vap = NULL;
    ...
    if (tmp_removekey_param.link_id == -1) {
        ...
        for (i = 0; i < WLAN_MAX_ML_LINK_NUM; i++) {
            ...
            ret += hmac_config_11i_remove_key(&hmac_link_vap->st_vap_base_info, len, param);
        }
#ifdef CONFIG_HISI_WIFI7_MLO
    } else {
        hmac_link_vap = hmac_mld_get_sta_vap_by_ap_link_id(hmac_vap, tmp_removekey_param.link_id);
...
uint32_t hmac_config_remove_key(mac_vap_stru *mac_vap, uint16_t len, uint8_t *param)
{
    hmac_vap_stru *hmac_vap = NULL;
#ifdef _PRE_WLAN_FEATURE_ML
    mac_removekey_param_stru *removekey_params = (mac_removekey_param_stru *)param;
    ...
    if (oal_any_null_ptr2(mac_vap, param)) {
        oam_error_log0(0, OAM_SF_WPA, "{hmac_config_remove_key::invalid arguments.}");
        return OAL_ERR_CODE_PTR_NULL;
    }

    hmac_vap =  (hmac_vap_stru *)mac_res_get_hmac_vap(mac_vap->uc_vap_id);
    if (hmac_vap == NULL) {
        oam_error_log0(mac_vap->uc_vap_id, OAM_SF_WPA, "{hmac_config_remove_key::hmac_vap is NULL}");
        return OAL_ERR_CODE_PTR_NULL;
    }

#ifdef _PRE_WLAN_FEATURE_ML
    if ((hmac_vap_is_ml_mode(hmac_vap) == OAL_TRUE) && (removekey_params->en_pairwise == OAL_TRUE)) {
        return hmac_mld_config_remove_ptk(hmac_vap, len, param);
    } else if (hmac_vap_is_ml_mode(hmac_vap) == OAL_TRUE) {
        return hmac_mld_config_remove_gtk(hmac_vap, len, param);
    }
#endif

    return hmac_config_11i_remove_key(mac_vap, len, param);
}
    ...
    uint16_t us_len, uint8_t *puc_param)
{
    ...
    if (uc_key_index >= WLAN_NUM_TK + WLAN_NUM_IGTK) {
        /* beacon 保护BIGTK default_key_id密钥配置，透传事件到device即可 */
        ret = OAL_SUCC;
    } else if (uc_key_index >= WLAN_NUM_TK) {
                                                  ...
                                                  mac_bss_dscr_stru *pst_bss_dscr)
{
    ...
    mac_vap_config_mib_by_protocol(pst_mac_vap);
...
uint32_t hmac_rx_tkip_mic_failure_process(frw_event_mem_stru *pst_event_mem)
{
    ...
    /* 去关联的STA mac地址 */
    if (memcpy_s((uint8_t *)frw_get_event_payload(pst_hmac_event_mem), sizeof(dmac_to_hmac_mic_event_stru),
        (uint8_t *)pst_mic_event, sizeof(dmac_to_hmac_mic_event_stru)) != EOK) {
