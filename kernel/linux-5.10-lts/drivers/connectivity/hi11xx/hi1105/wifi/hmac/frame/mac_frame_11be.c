...
/*
 * 函 数 名   : mac_parse_eht_ppe_thresholds
 * 功能描述   : 解析 EHT PPE thresholds
 * 修改历史
 * 1.日    期 : 2023年9月18日
 *   修改内容 : 新生成函数
 */
OAL_STATIC uint32_t mac_parse_eht_ppe_thresholds(mac_vap_stru *mac_vap, uint8_t *ppe_buffer,
    uint8_t ppe_len, mac_frame_eht_capabilities_ie_stru *eht_cap)
{
    mac_frame_eht_ppe_thresholds_stru *ppe = (mac_frame_eht_ppe_thresholds_stru *)ppe_buffer;
    uint8_t eht_ppet[MAX_EHT_PPET_LEN] = {0};
    uint8_t nss = ppe->nss;
    uint8_t actual_ppe_len = 0;
    uint8_t count = 0;
    uint8_t local_nss = 0;
    uint8_t ru_bitmask = 0;
    errno_t ret;

    if (ppe->ru_index0_mask)
        count++;
    if (ppe->ru_index1_mask)
        count++;
    if (ppe->ru_index2_mask)
        count++;
    if (ppe->ru_index3_mask)
        count++;
    if (ppe->ru_index4_mask)
        count++;

    actual_ppe_len = ((MAC_EHT_PPET_NSS_FIELD_BIT + MAC_EHT_PPET_RU_BITMASK_FIELD_BIT +
        (BYTE_OFFSET_6 * (nss + 1) * count) + BYTE_OFFSET_7) / BYTE_OFFSET_8);

    if ((actual_ppe_len != ppe_len) || (actual_ppe_len > MAX_EHT_PPET_LEN)) {
        oam_error_log0(0, OAM_SF_ANY, "{mac_parse_eht_ppe_thresholds::invalid ppe length.}");
        return OAL_FAIL;
    }

    ret = memcpy_s(eht_ppet, MAX_EHT_PPET_LEN, ppe, ppe_len);
    if (ret != EOK) {
        oam_error_log1(0, OAM_SF_ANY, "{mac_parse_eht_ppe_thresholds::memcpy_s fail.[%d]}", ret);
        return OAL_FAIL;
    }

    ppe = (mac_frame_eht_ppe_thresholds_stru *)eht_ppet;

    local_nss = mac_mib_get_eht_Nss(mac_vap->pst_mib_info);
    ru_bitmask = mac_mib_get_eht_RuIndexMask(mac_vap->pst_mib_info);

    if (ppe->nss > local_nss)
        ppe->nss = local_nss;

    ppe->ru_index0_mask &= (ru_bitmask & MAC_EHT_PPET_RU_MASK);
    ppe->ru_index1_mask &= ((ru_bitmask >> MAC_EHT_PPET_RU1_INDEX) & MAC_EHT_PPET_RU_MASK);
    ppe->ru_index2_mask &= ((ru_bitmask >> MAC_EHT_PPET_RU2_INDEX) & MAC_EHT_PPET_RU_MASK);
    ppe->ru_index3_mask &= ((ru_bitmask >> MAC_EHT_PPET_RU3_INDEX) & MAC_EHT_PPET_RU_MASK);
    ppe->ru_index4_mask &= ((ru_bitmask >> MAC_EHT_PPET_RU4_INDEX) & MAC_EHT_PPET_RU_MASK);

    ret = memcpy_s((uint8_t *)(&eht_cap->eht_ppe_thresholds), sizeof(mac_frame_eht_ppe_thresholds_stru),
                   eht_ppet, sizeof(mac_frame_eht_ppe_thresholds_stru));
    if (ret != EOK) {
        oam_error_log0(0, OAM_SF_ANY, "{mac_parse_eht_ppe_thresholds::memory copy failed.}");
        return OAL_FAIL;
    }
    return OAL_SUCC;
}
...
OAL_STATIC uint32_t mac_parse_eht_mcs_20mhz_nss(uint8_t band, uint8_t he_channel_width_set,
    mac_frame_eht_capabilities_ie_stru *eht_cap, uint8_t *mcs_nss, uint8_t *size)
{
...
static void mac_sync_eht_mcs_nss(mac_frame_eht_capabilities_ie_stru *eht_cap, const uint8_t *mcs_nss, uint8_t size)
{
...
OAL_STATIC uint32_t mac_parse_eht_mcs_nss(mac_vap_stru *mac_vap,
    mac_frame_eht_phy_capabilities_stru *eht_phy_cap, uint8_t *mcs_nss, uint8_t *he_cap_ie,
    mac_frame_eht_capabilities_ie_stru *eht_cap, uint8_t *mcs_nss_len)
{
    uint8_t band = mac_vap->st_channel.en_band;
    uint8_t vap_mode = mac_vap->en_vap_mode;
    uint8_t support_320mhz = eht_phy_cap->bit_320mhz_6ghz_support;
    uint8_t he_channel_width_set = 0;
    uint8_t size = 0;
    uint32_t ret;

#ifdef _PRE_WLAN_FEATURE_11AX
    mac_frame_he_phy_cap_stru *he_phy_cap = NULL;

    if (he_cap_ie != NULL) {
        he_phy_cap = (mac_frame_he_phy_cap_stru *)(he_cap_ie + MAC_IE_EXT_HDR_LEN + MAC_HE_MAC_CAP_LEN);
        he_channel_width_set = he_phy_cap->bit_channel_width_set;
    }
#endif

    if (vap_mode == WLAN_VAP_MODE_BSS_AP) {
        ret = mac_parse_eht_mcs_20mhz_nss(band, he_channel_width_set, eht_cap, mcs_nss, &size);
        if (ret != OAL_SUCC) {
    ...
    mac_sync_eht_mcs_nss(eht_cap, mcs_nss, size);
    ret = mac_parse_eht_mcs_below_80mhz_nss(band, vap_mode, he_channel_width_set, eht_cap, mcs_nss + size,
        &size);
    if (ret != OAL_SUCC) {
    ...
    ret = mac_parse_eht_mcs_160mhz_nss(he_channel_width_set, eht_cap, mcs_nss + size, &size);
    if (ret != OAL_SUCC) {
        oam_error_log0(0, OAM_SF_ANY, "{mac_parse_eht_mcs_160mhz_nss: fail.}");
        return OAL_FAIL;
    }

    ret = mac_parse_eht_mcs_320mhz_nss(support_320mhz, eht_cap, mcs_nss + size, &size);
    if (ret != OAL_SUCC) {
    ...
    mac_frame_eht_phy_capabilities_stru *phy_cap)
{
    ...
    /* B35: EHT MU PPDU With 4x EHT-LTF And 0.8 μs GI：是否支持接收4x EHT-LTF和0.8 μs GI的EHT MU PPDU */
    phy_cap->bit_eht_mu_ppdu_4x_eht_ltf_08us_gi &=
        mac_mib_get_EHTMUPPDUwith4xEHTLTFand0point8usecGIImplemented(mac_vap->pst_mib_info);
...
* 修改内容 : 新生成函数
 */
uint32_t mac_parse_eht_capabilities_ie(mac_vap_stru *mac_vap, uint8_t *eht_cap_ie,
    uint8_t *he_cap_ie, mac_frame_eht_capabilities_ie_stru *eht_cap)
{
    mac_frame_eht_mac_capabilities_stru mac_cap;
    mac_frame_eht_phy_capabilities_stru phy_cap;
    uint8_t mac_cap_len = sizeof(mac_frame_eht_mac_capabilities_stru);
    ...
    if (oal_unlikely(oal_any_null_ptr3(mac_vap, eht_cap_ie, eht_cap))) {
        oam_error_log0(0, OAM_SF_ANY, "{mac_parse_eht_capabilities_ie::param null}");
        return OAL_ERR_CODE_PTR_NULL;
    }

    memset_s(&mac_cap, sizeof(mac_frame_eht_mac_capabilities_stru), 0, sizeof(mac_frame_eht_mac_capabilities_stru));
    memset_s(&phy_cap, sizeof(mac_frame_eht_phy_capabilities_stru), 0, sizeof(mac_frame_eht_phy_capabilities_stru));
    ...
    buffer = eht_cap_ie + MAC_IE_EXT_HDR_LEN;

    /* 获取 EHT MAC Capabilities Info */
    if (memcpy_s((uint8_t *)(&mac_cap), mac_cap_len, buffer, mac_cap_len) != EOK) {
    ...
    if (memcpy_s((uint8_t *)(&phy_cap), phy_cap_len, buffer, phy_cap_len) != EOK) {
        oam_error_log0(0, OAM_SF_ANY, "{mac_parse_eht_capabilities_ie::memory copy failed.}");
        return OAL_FAIL;
    }
    buffer += MAC_EHT_PHY_CAP_LEN;
    ...
    if (mac_parse_eht_mcs_nss(mac_vap, &phy_cap, buffer, he_cap_ie,
        eht_cap, &mcs_nss_set_len) != OAL_SUCC) {
        return OAL_FAIL;
    }
    buffer += mcs_nss_set_len;

    /* 解析EHT PPE Thresholds field */
    ppe_len = len - MAC_EHT_CAP_MIN_LEN - mcs_nss_set_len;
    if (phy_cap.ppe_thresholds_present) {
        if (mac_parse_eht_ppe_thresholds(mac_vap, buffer, ppe_len, eht_cap) != OAL_SUCC)
            return OAL_FAIL;
    }
...
uint32_t mac_process_eht_capabilities_ie(mac_vap_stru *mac_vap, mac_user_stru *mac_user,
    uint8_t *eht_cap_ie, uint8_t *he_cap_ie)
{
    ...
    if (mac_parse_eht_capabilities_ie(mac_vap, eht_cap_ie, he_cap_ie, &eht_cap_value) != OAL_SUCC) {
        return OAL_FAIL;
    }
    ...
    uint8_t chan_width, uint8_t channel, uint8_t ccfs0, uint8_t ccfs1)
{
    ...
    ******************************************************************/
    if (chan_width == WLAN_EHT_CHANNEL_WIDTH_320) {
        /* 当前设备不支持320MHz带宽 */
        ret = WLAN_BAND_WIDTH_160MINUSMINUSMINUS;
#ifdef _PRE_WLAN_FEATURE_160M
