    ...
    ml_link_info_stru *ml_link_info)
{
    ...
    if (sta_info[0] != offset) {
        oam_error_log0(0, OAM_SF_ANY, "{hmac_sta_parse_basic_ml_sta_info::invalid sta info length.}");
        return OAL_FAIL;
    }
...
OAL_STATIC uint8_t mac_mld_support_eml_capabilities(hmac_vap_stru *hmac_vap)
{
    /* 定制化使能单link，则不支持EML，否则支持 */
    if (hmac_mld_enable_eml_cap() == OAL_FALSE ||
        hmac_mld_enable_single_link() == OAL_TRUE ||
        hmac_get_mld_assoc_mode(hmac_vap) == MLD_ASSOC_MODE_STR) {
        return OAL_FALSE;
    }
...
void mac_set_basic_ml_ie_control(hmac_vap_stru *hmac_vap, mac_ml_control_union *ml_control)
{
    ...
    ml_control->basic_ml_control.ml_eml_capabilities_present = mac_mld_support_eml_capabilities(hmac_vap);
...
uint8_t mac_set_eml_capabilities_field(hmac_mld_vap_info_stru *mld_vap_link_info, uint8_t *buffer)
{
    ...
    eml_cap->emlsr_support = mld_vap_link_info->emlsr_cap.emlsr_support;
    eml_cap->emlsr_padding_delay = mld_vap_link_info->emlsr_cap.emlsr_padding_delay;
    eml_cap->emlsr_transition_delay = mld_vap_link_info->emlsr_cap.emlsr_transition_delay;
