...
{
    if (mac_mib_get_ft_trainsistion(mac_vap) != OAL_TRUE) {
        *puc_ie_len = 0;
        return;
    }
    ...
    *(puc_buffer + BYTE_OFFSET_4) = 0;
    ...
    /***************************************************************************
             ...
             Bits:  |1         | 1              | 6          |
                    ------------------------------------------
    ***************************************************************************/
    if (mac_mib_get_ft_over_ds(mac_vap) == OAL_TRUE) {
        *(puc_buffer + BYTE_OFFSET_4) += 1;
    }
    if (mac_mib_get_ft_resource_req(mac_vap) == OAL_TRUE) {
...
void mac_set_ft_ie(void *pst_vap, uint8_t *puc_buffer, uint16_t *pus_ie_len)
{
    ...
    if (mac_mib_get_ft_trainsistion(pst_mac_vap) != OAL_TRUE) {
        *pus_ie_len = 0;
        return;
    }
...
void hmac_config_11r_cap(hmac_vap_stru *hmac_vap, mac_conn_security_stru *conn_sec, mac_bss_dscr_stru *bss_dscr)
{
    ...
    if (hmac_vap->bit_11r_enable == OAL_TRUE) {
        mde = mac_find_ie(MAC_EID_MOBILITY_DOMAIN, &ies_info);
        if (mde != NULL) {
    ...
    uint32_t remain_len)
{
    ...
    if (cowork_ie->capa_bitmap.mdid_flag != 0) {
        md_ie.mdid = cowork_ie->capa_bitmap.dyn_data.mdid;
    }
...
uint8_t *hmac_scan_find_cowork_ie(uint8_t *mgmt_frame, uint16_t frame_len)
{
    ...
    ies_info.frame_ies = mgmt_frame + offset;
...
int32_t hmac_scan_attach_akm_suite_to_rsn_ie(uint8_t *mgmt_frame, uint16_t *frame_len, uint8_t ext_len)
{
    ...
    rsn_ie = mac_find_ie(MAC_EID_RSN, &ies_info);
...
uint8_t hmac_scan_extend_mgmt_len_needed(oal_netbuf_stru *netbuf, uint16_t netbuf_len)
{
    ...
    ies_info.frame_ies = tagged_para;
    ies_info.frame_len = tagged_para_len;
