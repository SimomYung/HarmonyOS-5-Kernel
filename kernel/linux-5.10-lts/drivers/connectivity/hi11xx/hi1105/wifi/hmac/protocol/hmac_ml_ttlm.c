...
{
    uint8_t                 tid, ttlm_field_len, len;
    uint8_t                 offset = MAC_IE_EXT_HDR_LEN;
    mac_frame_ie_ext_hdr    *ie_ext_hdr = NULL;
    ttlm_control            *ttlm_ctrl = NULL;
    /* 解析TID-To-Link Mapping IE */
    ie_ext_hdr = (mac_frame_ie_ext_hdr *)ttlm_ie;
    if (ie_ext_hdr->length < TTLM_IE_MIN_LEN || ie_ext_hdr->length > TTLM_IE_MAX_LEN) {
        oam_warning_log1(0, OAM_SF_ANY, "{hmac_parse_beacon_ttlm_ie::invalid ie len[%u]}", ie_ext_hdr->length);
        return OAL_FAIL;
    }
    ttlm_ctrl = (ttlm_control *)(ttlm_ie + offset);
    if (ttlm_ctrl->direction != WLAN_DOWNUPLINK || ttlm_ctrl->default_link_mapping != 0 ||
        ttlm_ctrl->expected_duration_present == 0 || ttlm_ctrl->link_mapping_size > 1 ||
        ttlm_ctrl->link_mapping_presence_bitmap == 0) {
        oam_warning_log3(0, OAM_SF_ANY, "{hmac_parse_beacon_ttlm_ie::invalid ttlm param:dir[%u],dflkmp[%u],mpswtp[%u],\
            epdp[%u]}", ttlm_ctrl->direction, ttlm_ctrl->default_link_mapping, ttlm_ctrl->expected_duration_present);
        oam_warning_log2(0, OAM_SF_ANY, "{hmac_parse_beacon_ttlm_ie::invalid ttlm param:lkmps[%u],lkmppb[%u]}",
            ttlm_ctrl->link_mapping_size, ttlm_ctrl->link_mapping_presence_bitmap);
        return OAL_FAIL;
    }
    ttlm_field_len = (ttlm_ctrl->link_mapping_size == 0 ? 2 : 1); /* 2 表示Link Mapping Of TID字段的长度 */
    if (memcpy_s(ttlm_cfg_ie, sizeof(mac_cfg_ttlm_ie), ttlm_ctrl, sizeof(ttlm_control)) != EOK) {
        return OAL_FAIL;
    }
    offset += sizeof(ttlm_control);
    if (ttlm_ctrl->mapping_switch_time_present != 0) {
        len = TTLM_MAPPING_SWITCH_TIME_LEN + TTLM_EXPECTED_DURATION_LEN;
        if (memcpy_s(&(ttlm_cfg_ie->mapping_switch_time), len, ttlm_ie + offset, len) != EOK) {
            return OAL_FAIL;
        }
        offset += len;
    } else {
        len = TTLM_EXPECTED_DURATION_LEN;
        if (memcpy_s(ttlm_cfg_ie->expected_duration, len, ttlm_ie + offset, len) != EOK) {
            return OAL_FAIL;
        }
        offset += len;
    }
...
OAL_STATIC uint8_t hmac_ttlm_check_need_link_switch(mac_vap_stru *mac_vap, mac_ttlm_msg_stru *ttlm_msg)
{
    hmac_vap_stru *hmac_vap = NULL;
    hmac_user_stru *disable_user = NULL;
    hmac_vap_stru *disable_vap = NULL;
    hmac_vap_stru *enable_vap = NULL;
    uint8_t ret = OAL_SUCC;
    uint16_t idx;

    hmac_vap = (hmac_vap_stru *)mac_res_get_hmac_vap(mac_vap->uc_vap_id);
    if (hmac_vap == NULL ||
        ttlm_msg->count != WLAN_TIDNO_BUTT ||
    ...
    if (disable_vap == NULL || disable_vap->mld_vap_link_info.mld_mode != WLAN_ML_LINK_MODE) {
        return ret;
    }

    enable_vap = hmac_mld_get_another_vap(disable_vap);
    if (enable_vap == NULL || enable_vap->mld_vap_link_info.mld_mode != WLAN_LINK_MODE) {
        return ret;
    }
...
uint32_t hmac_ttlm_update_status(mac_vap_stru *mac_vap, hmac_user_stru *hmac_user)
{
    ...
    ret = hmac_ttlm_check_need_link_switch(mac_vap, &ttlm_msg);
    if (ret != OAL_SUCC) {
        return ret;
    }
