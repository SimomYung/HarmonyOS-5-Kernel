...
void hmac_user_change_tid_info_user_id(hmac_user_stru *cur_work_user, hmac_user_stru *next_work_user)
{
    uint8_t tid;
    ...
    for (tid = 0; tid < WLAN_TID_MAX_NUM; tid++) {
        tid_info = hmac_get_tx_tid_info(cur_work_user, tid);
        tid_ba = hmac_get_tid_ba_info(cur_work_user, tid);
        if (tid_info == NULL || tid_ba == NULL) {
            continue;
        }
        tid_info->user_index = next_work_user->st_user_base_info.us_assoc_id;
        if (tid_ba->pst_ba_rx_info == NULL) {
            continue;
        }
        tid_ba->pst_ba_rx_info->st_alarm_data.us_mac_user_idx = next_work_user->st_user_base_info.us_assoc_id;
        tid_ba->pst_ba_rx_info->st_alarm_data.uc_vap_id = next_work_user->st_user_base_info.uc_vap_id;
    }
}

hmac_tid_res_stru g_hmac_tid_res[WLAN_ASSOC_USER_MAX_NUM];
...
{
    memset_s(g_hmac_tid_res, sizeof(hmac_tid_res_stru) * WLAN_ASSOC_USER_MAX_NUM,
        0, sizeof(hmac_tid_res_stru) * WLAN_ASSOC_USER_MAX_NUM);
}

void hmac_user_tid_res_deinit(hmac_user_stru *hmac_user)
{
    if (hmac_user->tid_res == NULL) {
        return;
    }

    if (hmac_user->tid_res->work_flag == 0) {
        oam_error_log1(0, 0, "hmac_user_tid_res_deinit:: work flag[%d] error!", hmac_user->tid_res->work_flag);
        return;
    }
    hmac_user->tid_res->work_flag--;
    /* 所有user都已释放该资源，对应bitmap可置为0 */
    if (hmac_user->tid_res->work_flag == 0) {
        oal_bit_clear_bit_four_byte(&g_hmac_tid_res_bitmap, (oal_bitops)hmac_user->tid_res->tid_res_id);
    }
}

hmac_tid_res_stru *hmac_ml_user_alloc_tid_res(hmac_user_stru *hmac_user)
{
#ifdef _PRE_WLAN_FEATURE_ML
    ...
    if (master_user->tid_res != NULL) {
        return master_user->tid_res;
    }
#endif
    return NULL;
}

hmac_tid_res_stru *hmac_alloc_tid_res(hmac_user_stru *hmac_user)
{
    uint8_t tid_res_id;
    uint8_t loop;
    hmac_tid_res_stru *tid_res = NULL;

    tid_res = hmac_ml_user_alloc_tid_res(hmac_user);
    if (tid_res != NULL) {
        oam_warning_log3(0, 0, "hmac_user_tid_init::res_id %d user_id %d bitmap 0x%x",
            tid_res->tid_res_id, hmac_user->st_user_base_info.us_assoc_id, g_hmac_tid_res_bitmap);
        return tid_res;
    }

    tid_res_id = oal_bit_find_first_zero_four_byte(g_hmac_tid_res_bitmap);
    if (tid_res_id >= WLAN_ASSOC_USER_MAX_NUM) {
        oam_error_log1(0, 0, "hmac_alloc_tid_res:: tid_info_res_id[%d] error", tid_res_id);
        return NULL;
    }
    if (g_hmac_tid_res[tid_res_id].work_flag != 0) {
        oam_error_log2(0, 0, "hmac_alloc_tid_res:: res_id[%d] work_flag[%d] error",
            tid_res_id, g_hmac_tid_res[tid_res_id].work_flag);
    }
    tid_res = &g_hmac_tid_res[tid_res_id];
    for (loop = 0; loop < WLAN_TID_MAX_NUM; loop++) {
        if (tid_res->tid_ba[loop].st_ba_tx_info.st_addba_timer.en_is_registerd == OAL_TRUE) {
            frw_timer_immediate_destroy_timer_m(&(tid_res->tid_ba[loop].st_ba_tx_info.st_addba_timer));
        }
    ...
    memset_s(&g_hmac_tid_res[tid_res_id], sizeof(hmac_tid_res_stru), 0, sizeof(hmac_tid_res_stru));
    oal_bit_set_bit_four_byte(&g_hmac_tid_res_bitmap, (oal_bitops)tid_res_id);
    g_hmac_tid_res[tid_res_id].tid_res_id = tid_res_id;

    oam_warning_log3(0, 0, "hmac_user_tid_init::res_id %d user_id %d bitmap 0x%x",
        tid_res_id, hmac_user->st_user_base_info.us_assoc_id, g_hmac_tid_res_bitmap);
    return &g_hmac_tid_res[tid_res_id];
}

void hmac_tid_ba_info_init(hmac_user_stru *hmac_user, uint8_t tid)
{
    hmac_ba_tx_stru *tx_ba = NULL;
    hmac_tid_ba_stru *tid_ba = NULL;

    tid_ba = hmac_get_tid_ba_info(hmac_user, tid);
    if (tid_ba == NULL) {
        return;
    }
...
OAL_STATIC void hmac_user_tid_init(hmac_user_stru *hmac_user)
{
    ...
    hmac_user->tid_res = hmac_alloc_tid_res(hmac_user);
    if (oal_unlikely(hmac_user->tid_res == NULL)) {
        oam_error_log1(0, 0, "hmac_user_tid_init::user[%d] tid res null",
            hmac_user->st_user_base_info.us_assoc_id);
        return;
    }

    if (hmac_user->tid_res->work_flag != 0) { // 说明已初始化
        hmac_user->tid_res->work_flag++;
        return;
    }
    hmac_user->tid_res->work_flag++;
    ...
    for (tid = 0; tid < WLAN_TID_MAX_NUM; tid++) {
        hmac_tid_ba_info_init(hmac_user, tid);
...
void hmac_device_rx_list_init(hmac_user_stru *hmac_user)
{
    uint8_t tid;
    for (tid = 0; tid < WLAN_TID_MAX_NUM; tid++) {
...
void hmac_mcast_ampdu_rx_ba_init(hmac_user_stru *hmac_user, uint8_t tid)
{
    ...
    if (ba_rx_stru == NULL) {
        oam_error_log1(0, OAM_SF_BA,
            "{hmac_mcast_ampdu_rx_ba_init::alloc fail.tid[%d]}", tid);
        return;
    }

    tid_ba = hmac_get_tid_ba_info(hmac_user, tid);
    if (tid_ba == NULL) {
        oal_mem_free_m(ba_rx_stru, OAL_TRUE);
        return;
    }
    tid_ba->pst_ba_rx_info = ba_rx_stru;
    /* Ba会话参数初始化 */
...
uint32_t hmac_user_init(hmac_user_stru *hmac_user)
{
    ...
    hmac_device_rx_list_init(hmac_user);
    if (hmac_vap == NULL) {
        return OAL_ERR_CODE_PTR_NULL;
    }
#ifdef _PRE_WLAN_FEATURE_MCAST_AMPDU
    ...
    uint8_t *puc_mac_addr)
{
    ...
    if ((pst_bss_dscr != NULL) && (pst_bss_dscr->st_channel.en_band == WLAN_BAND_5G) &&
...
static void hmac_user_del_vap_init(hmac_vap_stru *hmac_vap, mac_device_stru *mac_device, hmac_user_stru *hmac_user)
{
    if (hmac_vap->st_vap_base_info.en_vap_mode == WLAN_VAP_MODE_BSS_STA) {
        ...
        hmac_mld_destroy_mlsr_link_switch_timer(hmac_vap);
...
uint32_t hmac_user_del(mac_vap_stru *mac_vap, hmac_user_stru *hmac_user)
{
    ...
    /* 删用户重置ba时只允许只有1个user使用tid资源，在mld场景和mld漫游非mld场景生效 防止删用户和建ba异步导致的状态错误 */
    if (hmac_user->tid_res != NULL && hmac_user->tid_res->work_flag == 1) {
...
#endif
    hmac_user_tid_res_deinit(hmac_user);
#ifdef _PRE_WLAN_FEATURE_ML
