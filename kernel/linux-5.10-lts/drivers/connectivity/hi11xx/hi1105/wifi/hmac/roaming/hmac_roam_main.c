...
uint32_t hmac_roam_start(hmac_vap_stru *hmac_vap, hmac_roam_start_params *roam_start_params)
{
    ...
    if ((hmac_vap == NULL) || (roam_start_params == NULL)) {
        oam_error_log0(0, OAM_SF_ROAM, "{hmac_roam_start::hmac_vap or roam_start_paramsnull.}");
        return OAL_ERR_CODE_PTR_NULL;
    }
...
static uint32_t hmac_roam_connecting_fail_post_do(hmac_roam_info_stru *roam_info)
{
    ...
    if (roam_info->pst_hmac_vap->bit_11r_auth_fail == OAL_TRUE) {
        oam_warning_log0(0, OAM_SF_ROAM,
            "{hmac_roam_connecting_fail_post_do:: 11r auth fail, don't notify wpa_s!}");
        return OAL_SUCC;
    }
...
OAL_STATIC uint32_t hmac_roam_connecting_fail(hmac_roam_info_stru *roam_info, void *param)
{
    ...
    if (hmac_vap_is_link_mode(roam_info->pst_hmac_vap) == OAL_FALSE) {
        ret = hmac_roam_connecting_fail_post_do(roam_info);
    }
    return ret;
}
...
static uint32_t hmac_roam_handle_fail_handshake_phase_post_do(hmac_roam_info_stru *roam_info)
{
    ...
    if (roam_info->st_alg.c_current_rssi > ROAM_RSSI_MAX_TYPE) {
         ...
         */
        return hmac_roam_trigger_handle(roam_info->pst_hmac_vap, roam_info->st_alg.c_current_rssi - 1,
                                        OAL_TRUE, ROAM_SAME_MLD_LINK_REASSOC);
    }
...
#ifdef _PRE_WLAN_FEATURE_WUR
    hmac_wur_roam_connect_exit_wur(hmac_vap, hmac_user);
#endif
