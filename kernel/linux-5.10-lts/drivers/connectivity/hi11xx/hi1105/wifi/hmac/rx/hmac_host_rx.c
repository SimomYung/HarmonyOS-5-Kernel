...
OAL_STATIC uint32_t hmac_rx_get_da_sa_addr(dmac_msdu_stru *rx_msdu,
    mac_ieee80211_frame_stru *frame_hdr, hmac_host_rx_context_stru *rx_context)
{
    uint8_t *addr = NULL;
    ...
    if (!hmac_vap_is_legacy_mld_mode(hmac_vap)) {
        ...
        mld_addr = hmac_vap->mld_vap_link_info.mld_vap_manage->mld_addr;
        addr = mld_addr;
    }
#endif
...
OAL_STATIC uint32_t hmac_rx_proc_80211_to_eth(oal_netbuf_stru *netbuf,
    mac_rx_ctl_stru *rx_ctrl, mac_ieee80211_frame_stru *frame_hdr, hmac_host_rx_context_stru *rx_context)
{
    ...
    if (mac_get_rx_cb_is_first_sub_msdu(rx_ctrl)) {
        /* 聚合首帧处理，获取源地址和目的地址 */
        if (hmac_rx_get_da_sa_addr(&rx_msdu, frame_hdr, rx_context) != OAL_SUCC) {
            return OAL_FAIL;
        }
...
OAL_STATIC oal_bool_enum_uint8 hmac_rx_qos_data_need_drop(hmac_host_rx_context_stru *rx_context)
{
    ...
    /* qos 已建ba返回 */
    hmac_tid = hmac_get_tid_ba_info(hmac_user, rx_tid);
    if (hmac_tid == NULL) {
        return OAL_TRUE;
    }
...
static uint32_t hmac_is_hal_vap_valid(hal_host_device_stru *hal_device, mac_rx_ctl_stru *rx_ctl)
{
    ...
    if (oal_unlikely(hal_vap_id >= mac_chip_get_hal_max_vap_num()) && (hal_vap_id != g_wlan_spec_cfg->other_bss_id)) {
        oam_error_log1(0, OAM_SF_RX, "{hmac_is_hal_vap_valid::invalid hal vap id[%d].}", hal_vap_id);
        return OAL_FAIL;
    }

    if ((hal_vap_id < mac_chip_get_hal_max_vap_num()) && (!hal_device->hal_vap_sts_info[hal_vap_id].hal_vap_valid)) {
    ...
    oal_netbuf_head_stru *rpt_list, uint8_t *buff_is_valid)
{
    ...
    hmac_tid = hmac_get_tid_ba_info(hmac_user, mac_get_rx_cb_tid(rx_ctl));
    if (hmac_tid == NULL) {
        return OAL_SUCC;
    }
    ba_rx_hdl = hmac_tid->pst_ba_rx_info;
...
void hmac_rx_netbuf_update_rxctl_data_type_mp16(oal_netbuf_stru *netbuf, mac_rx_ctl_stru *rx_ctl)
{
    ...
    if (frame_format == MAC_FRAME_TYPE_80211) {
        ...
        frame_hdr = oal_netbuf_data(netbuf);
        eth_type = mac_get_eth_data_subtype(netbuf, frame_len);
        if (eth_type == MAC_FRAME_TYPE_8023) {
            /* 跳过DA/SA/length 14字节 */
            frame_hdr += ETHER_HDR_LEN;
            datatype = mac_get_data_type_from_8023(netbuf, frame_hdr, MAC_FRAME_TYPE_8023,
                frame_len - ETHER_HDR_LEN, OAM_OTA_FRAME_DIRECTION_TYPE_RX);
        } else {
            datatype = mac_get_data_type_from_8023(netbuf, frame_hdr, MAC_FRAME_TYPE_RTH, frame_len,
                OAM_OTA_FRAME_DIRECTION_TYPE_RX);
        }
    } else {
        oam_warning_log1(0, OAM_SF_RX, "hmac_rx_netbuf_update_rxctl_data_type:frame format[%d]", frame_format);
...
void hmac_rx_netbuf_update_rxctl_data_type_mp17c(oal_netbuf_stru *netbuf, mac_rx_ctl_stru *rx_ctl)
{
    mac_data_type_enum_uint8 datatype = MAC_DATA_NUM;
    uint16_t frame_format = rx_ctl->bit_frame_format;
    uint32_t frame_len = rx_ctl->us_frame_len;
                                                     ...
                                                     hal_host_device_stru *hal_dev, mac_rx_ctl_stru *cb)
{
    ...
    if ((usr_lut < HAL_MAX_LUT) && (hal_chip->user_sts_info[usr_lut].user_valid)) {
        hmac_user = (hmac_user_stru *)mac_res_get_hmac_user(hal_chip->user_sts_info[usr_lut].user_id);
    }
...
{
    hal_host_rx_proc_msdu_dscr(hal_dev, netbuf);
...
OAL_INLINE uint16_t hmac_rx_netbuf_get_ether_type(oal_netbuf_stru *netbuf, mac_rx_ctl_stru *rx_ctl)
{
    ...
    mac_vap_stru *vap, uint32_t len)
{
    ...
    oam_warning_log3(vap->uc_vap_id, OAM_SF_MLOG, "{hmac_rx_vip_info_proc::EAPOL type=%u, len==%u, \
    ...
    mac_vap_stru *vap, uint32_t payload_len)
{
    ...
    oam_warning_log3(vap->uc_vap_id, OAM_SF_MLOG, "{hmac_rx_vip_info_proc::vap_id=%d DHCP type=%d len=%d [1:discovery \
        2:offer 3:request 4:decline 5:ack 6:nack 7:release 8:inform.]", vap->uc_vap_id, dhcp_type, payload_len);
    oam_warning_log4(vap->uc_vap_id, OAM_SF_MLOG, "{hmac_rx_vip_info_proc:: DHCP sip: %d.%d, dip: %d.%d.",
...
static inline void hmac_rx_vip_info_proc_rx_ctl_print(mac_vap_stru *vap, mac_rx_ctl_stru *rx_ctrl)
{
    oam_warning_log4(vap->uc_vap_id, OAM_SF_RX, "{hmac_rx_vip_info_proc::VIP data process_flag[%d], tid[%d], \
        seq[%d], start_seq[%d]",
        rx_ctrl->bit_process_flag, rx_ctrl->bit_rx_tid, rx_ctrl->us_seq_num, rx_ctrl->bit_start_seqnum);
    oam_warning_log2(vap->uc_vap_id, OAM_SF_RX, "{hmac_rx_vip_info_proc:: VIP data release_start[%d], release_end[%d]",
    ...
    uint8_t data_type, oal_netbuf_stru *netbuf, mac_rx_ctl_stru *rx_ctrl)
{
    ...
    oam_warning_log3(vap->uc_vap_id, OAM_SF_MLOG, "{hmac_rx_vip_info_proc::send to wlan smac: %02X:XX:XX:XX:%02X:%02X]",
        ether_hdr->auc_ether_shost[MAC_ADDR_0],
        ether_hdr->auc_ether_shost[MAC_ADDR_4], ether_hdr->auc_ether_shost[MAC_ADDR_5]);
    oam_warning_log4(vap->uc_vap_id, OAM_SF_MLOG, "{hmac_rx_vip_info_proc::user_id[%d] dmac:%02X:XX:XX:XX:%02X:%02X]",
    ...
    hmac_rx_vip_info_proc_rx_ctl_print(vap, rx_ctrl);
...
static uint32_t hmac_rx_context_prepare_msdu_list_to_eth(hmac_host_rx_context_stru *rx_context)
{
    ...
    while (netbuf != NULL) {
        ...
        if (rx_ctrl->bit_frame_format == MAC_FRAME_TYPE_80211) {
            /* 80211转换成8023 */
            if (hmac_rx_proc_80211_to_eth(netbuf, rx_ctrl, frame_header, rx_context) != OAL_SUCC) {
...
static uint32_t hmac_host_rx_mpdu_proc(
    hal_host_device_stru *origin_dev, oal_netbuf_stru *mpdu, oal_netbuf_head_stru *rpt_list)
{
    hal_host_device_stru *hal_device = hal_host_rx_get_mpdu_link(origin_dev, mpdu);
    hmac_host_rx_context_stru rx_context = { 0 };

    /* 外层异常处理基于链表的形式释放netbuf, 此处需要优先建链 */
    hal_host_rx_amsdu_list_build(origin_dev, mpdu);

    if (oal_unlikely(!hal_device)) {
        oam_error_log0(0, 0, "{hmac_host_rx_mpdu_proc::get mpdu link failed}");
        return OAL_FAIL;
    }

    if (hmac_host_rx_proc_dev_routine(hal_device, mpdu, &rx_context) != OAL_SUCC) {
        return OAL_FAIL;
    }
#ifdef _PRE_WLAN_FEATURE_VSP
    if (hmac_vsp_rx_proc(&rx_context, mpdu) == VSP_RX_ACCEPT) {
        /* vsp帧不采用上报协议栈方式，单独处理 */
        return OAL_SUCC;
    }
#endif
...
static void hmac_host_rx_mpdu_list_proc(hal_host_device_stru *origin_dev, oal_netbuf_head_stru *mpdu_list)
{
...
uint32_t hmac_host_rx_ring_data_event(frw_event_mem_stru *event_mem)
{
    ...
    hmac_host_rx_mpdu_list_proc(wlan_rx_event->hal_dev, &mpdu_list);
