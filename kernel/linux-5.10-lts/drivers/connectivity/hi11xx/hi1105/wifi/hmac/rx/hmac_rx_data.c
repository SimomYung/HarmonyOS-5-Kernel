...
void hmac_rx_report_eth_frame(mac_vap_stru *mac_vap, oal_netbuf_stru *netbuf)
{
    ...
    if (mac_vap->en_vap_mode == WLAN_VAP_MODE_BSS_AP) {
        ...
        if (oal_unlikely(pst_ether_hdr == NULL)) {
            oam_error_log0(mac_vap->uc_vap_id, OAM_SF_RX, "{hmac_rx_report_eth_frame::pst_ether_hdr null.}");
            oal_netbuf_pull(netbuf, ETHER_HDR_LEN);
            return;
        }

        ret = mac_vap_find_user_by_macaddr(mac_vap, pst_ether_hdr->auc_ether_shost, &us_user_idx);
        if (ret == OAL_ERR_CODE_PTR_NULL) {
            oam_error_log0(mac_vap->uc_vap_id, OAM_SF_RX, "{hmac_rx_report_eth_frame::ul_ret null.}");
            oal_netbuf_pull(netbuf, ETHER_HDR_LEN);
            return;
        }
        ...
        us_user_idx = mac_vap->us_assoc_vap_id;
        oal_set_mac_addr(auc_user_macaddr, mac_vap->auc_bssid);
    }
    ...
    if (ret != OAL_SUCC) {
        oam_warning_log1(mac_vap->uc_vap_id, OAM_SF_RX,
    ...
    oal_netbuf_stru *ori_netbuf, mac_rx_ctl_stru *ori_rx_ctrl, mac_ieee80211_frame_stru *frame_hdr)
{
    ...
    hmac_user_stru *hmac_user = (hmac_user_stru *)mac_res_get_hmac_user(ori_rx_ctrl->bit_rx_user_id);
    ...
    oal_netbuf_head_stru *netbuf_header, oal_netbuf_stru *netbuf, mac_ieee80211_frame_stru *pst_frame_hdr)
{
    ...
    if (pst_rx_ctrl == NULL) {
        oam_warning_log0(pst_vap->st_vap_base_info.uc_vap_id, OAM_SF_RX,
                         "{hmac_rx_prepare_msdu_list_to_wlan::pst_rx_ctrl is NULL.}");
        return;
    }
    ...
    hmac_vap_stru *pst_vap, hmac_user_stru *hmac_user, oal_netbuf_stru *netbuf, mac_ether_header_stru *pst_ether_hdr)
{
    ...
    if ((pst_mac_vap->en_vap_mode == WLAN_VAP_MODE_BSS_AP) && (pst_vap->uc_edca_opt_flag_ap == OAL_TRUE)) {
        if (pst_ether_hdr->us_ether_type == oal_host2net_short(ETHER_TYPE_IP)) {
...
OAL_STATIC void hmac_rx_transmit_msdu_to_lan(hmac_vap_stru *pst_vap, hmac_user_stru *hmac_user, dmac_msdu_stru *msdu)
{
    ...
    if (wlan_pm_wkup_src_debug_get() == OAL_TRUE) {
        ...
        hmac_parse_packet(&pst_vap->st_vap_base_info, netbuf);
    ...
    if (hmac_11i_ether_type_filter(pst_vap, hmac_user, pst_ether_hdr->us_ether_type) != OAL_SUCC) {
        /* 接收安全数据过滤 */
...
static void hmac_rx_lan_frame_no_user_log(hmac_vap_stru *hmac_vap, oal_netbuf_stru *netbuf, mac_rx_ctl_stru *rx_ctrl)
{
    ...
    oam_error_log1(hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_RX,
        "{hmac_rx_lan_frame_classify::user_idx=%d}", rx_ctrl->bit_rx_user_id);
#ifdef _PRE_WLAN_DFT_DUMP_FRAME
...
void hmac_rx_lan_frame_classify(hmac_vap_stru *vap, oal_netbuf_stru *netbuf, mac_ieee80211_frame_stru *pst_frame_hdr)
{
    ...
    hmac_user_stru *pst_hmac_user = (hmac_user_stru *)mac_res_get_hmac_user(pst_rx_ctrl->bit_rx_user_id);
...
void hmac_rx_process_data_filter(oal_netbuf_head_stru *netbuf_header, oal_netbuf_stru *temp_netbuf, uint16_t netbuf_num)
{
    ...
    while (netbuf_num != 0) {
        ...
        if (oal_board_get_service_vap_start_id() > uc_vap_id || uc_vap_id >= mac_chip_get_vap_support_max_num_limit()) {
 ...
 */
void hmac_rx_process_data_ap_tcp_ack_opt(hmac_vap_stru *hmac_vap, oal_netbuf_head_stru *netbuf_header)
{
    ...
    oal_netbuf_list_head_init(&st_w2w_netbuf_hdr);
    hmac_rx_set_hdr_info(hmac_vap, netbuf_header, &pst_temp_netbuf, &us_netbuf_num, &event_hdr);
    ...
    while (us_netbuf_num != 0) {
        ...
        if (oal_unlikely(hmac_vap == NULL)) {
            oam_warning_log0(rx_ctrl->bit_vap_id, OAM_SF_RX, "{hmac_rx_process_data_ap::hmac_vap null.}");
            hmac_rx_free_netbuf(netbuf, (uint16_t)uc_buf_nums);
            continue;
        }
        ...
        if (ether_is_multicast(puc_da)) {
            ...
            oam_stat_vap_incr(hmac_vap->st_vap_base_info.uc_vap_id, rx_mcast_cnt, 1);
            ...
            if (hmac_rx_copy_netbuff(&netbuf_copy, netbuf, rx_ctrl->uc_mac_vap_id, &pst_copy_frame_hdr) != OAL_SUCC) {
                ...
                oam_stat_vap_incr(hmac_vap->st_vap_base_info.uc_vap_id, rx_no_buff_dropped, 1);
                hmac_rx_free_netbuf(netbuf, (uint16_t)uc_buf_nums);
                continue;
            }

            hmac_rx_lan_frame_classify(hmac_vap, netbuf, pst_frame_hdr);  // 上报网络层

            /* 将MPDU解析成单个MSDU，把所有的MSDU组成一个netbuf链 */
            hmac_rx_prepare_msdu_list_to_wlan(hmac_vap, &st_w2w_netbuf_hdr, netbuf_copy, pst_copy_frame_hdr);
            continue;
        }

        /* 用户级别处理 */
        if (hmac_rx_data_ap_designated_user_process(hmac_vap, pst_frame_hdr, netbuf, uc_buf_nums) == OAL_RETURN) {
            continue;
        }

        /* 将MPDU解析成单个MSDU，把所有的MSDU组成一个netbuf链 */
        hmac_rx_prepare_msdu_list_to_wlan(hmac_vap, &st_w2w_netbuf_hdr, netbuf, pst_frame_hdr);
    }
 ...
 *   修改内容  : 新生成函数
 */
uint32_t hmac_rx_process_data_sta_tcp_ack_opt(hmac_vap_stru *hmac_vap, oal_netbuf_head_stru *netbuf_header)
{
    /* 将需要上报的帧逐一出队处理 */
...
static void __hmac_hal_vap_info_dump(hal_host_device_stru *hal_device)
{
    ...
    for (hal_vap_id = 0; hal_vap_id < mac_chip_get_hal_max_vap_num(); hal_vap_id++) {
...
static void hmac_mac_vap_info_dump(hmac_vap_stru *hmac_vap)
{
    ...
    oam_warning_log3(0, 0, "{hmac_mac_vap_info_dump::mac_vap_id[%d] device_id[%d] init_flag[%d]}",
