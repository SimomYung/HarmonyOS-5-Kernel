...
OAL_STATIC OAL_INLINE oal_bool_enum_uint8 hmac_tid_ba_is_setup(hmac_user_stru *pst_hmac_user, uint8_t uc_tidno)
{
    ...
    hmac_tid = hmac_get_tid_ba_info(pst_hmac_user, uc_tidno);
    if (oal_unlikely(hmac_tid == NULL)) {
        return OAL_FALSE;
    }
...
{
    hmac_tid_ba_stru *pst_hmac_tid_info = hmac_get_tid_ba_info(hmac_user, tidno);
    if (pst_hmac_tid_info == NULL) {
        return OAL_FALSE;
    }
    /* 对tid对应的tx BA会话状态加锁 */
...
{
    /* 该tid下不允许建BA，配置命令需求 */
    hmac_tid_ba_stru *hmac_tid = hmac_get_tid_ba_info(pst_hmac_user, uc_tidno);
    if (hmac_tid == NULL) {
        return OAL_FALSE;
    }

    if (hmac_tid->en_ba_handle_tx_enable == OAL_FALSE) {
        return OAL_FALSE;
    }
