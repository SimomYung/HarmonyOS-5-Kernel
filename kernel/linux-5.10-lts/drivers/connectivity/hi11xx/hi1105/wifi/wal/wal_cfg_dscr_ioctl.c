    ...
    uint8_t uc_value)
{
    ...
    if (wal_get_cmd_one_arg(pc_param, ac_arg, WAL_HIPRIV_CMD_NAME_MAX_LEN, &off_set) != OAL_SUCC) {
        oam_error_log0(0, OAM_SF_ANY,
            "{wal_tx_pow_param_tas_pow_ctrl_case::TAS pwr ctrl core or needimproved lost return err_code!}\r\n");
...
OAL_STATIC void wal_hipriv_tx_pow_get_next_para(mac_cfg_set_tx_pow_param_stru *pow_param, int8_t *param)
{
    ...
    if (wal_get_cmd_one_arg(param, ac_arg, WAL_HIPRIV_CMD_NAME_MAX_LEN, &off_set) == OAL_SUCC) {
        pow_param->auc_value[1] = (uint8_t)oal_atoi(ac_arg);
    }
}

#ifdef _PRE_WLAN_FEATURE_TAS_ANT_SWITCH
OAL_STATIC uint32_t wal_hipriv_tx_pow_tas_hanlde(oal_net_device_stru *net_dev,
    mac_cfg_set_tx_pow_param_stru *pow_param, int8_t *param, uint8_t value, wal_tx_pow_param_enum param_index)
{
    switch (param_index) {
        ...
        case WAL_TX_POW_PARAM_TAS_ANT_SWITCH:
            if (g_wlan_chip_ops->set_tas_ant_switch != NULL) {
                return g_wlan_chip_ops->set_tas_ant_switch(net_dev, value);
            }
            return OAL_FAIL;
        default:
            break;
...
OAL_STATIC uint32_t wal_hipriv_check_param(oal_net_device_stru *pst_net_dev, mac_cfg_set_tx_pow_param_stru *pow_param,
    int8_t *param, wal_tx_pow_param_enum param_index)
{
    ...
    switch (param_index) {
        ...
        case WAL_TX_POW_PARAM_TAS_RSSI_MEASURE:
        case WAL_TX_POW_PARAM_TAS_ANT_SWITCH:
            ret = wal_hipriv_tx_pow_tas_hanlde(pst_net_dev, pow_param, param, value, param_index);
            if (ret != OAL_SUCC) {
                return ret;
            }
            break;
...
uint32_t wal_hipriv_set_tx_pow_param(oal_net_device_stru *pst_net_dev, int8_t *pc_param)
{
    ...
    ***************************************************************************/
    ...
    /* 参数校验 */
    ret = wal_hipriv_check_param(pst_net_dev, pst_set_tx_pow_param, pc_param, en_param_index);
    if (ret != OAL_SUCC) {
...
{
    return wal_hipriv_set_mcs_by_cfgid(net_dev, param, WLAN_CFGID_SET_MCS,
...
{
    return wal_hipriv_set_mcs_by_cfgid(net_dev, param, WLAN_CFGID_SET_MCSAC,
...
{
    return wal_hipriv_set_mcs_by_cfgid(net_dev, param, WLAN_CFGID_SET_MCSAX,
...
{
    return wal_hipriv_set_mcs_by_cfgid(net_dev, param, WLAN_CFGID_SET_MCSBE,
                                       WAL_HIPRIV_EHT_MCS_MIN, WAL_HIPRIV_EHT_MCS_MAX);
}
#endif
