 ...
 * 1.日    期  : 2016年5月25日
  *   修改内容  : 新生成函数
 */
uint32_t wal_cfgvendor_get_current_channel_list(oal_net_device_stru *pst_netdev,
    uint32_t band, int32_t *pl_channel_list, uint32_t *pul_num_channels)
{
    int32_t l_ret;
    ...
    if (oal_unlikely(oal_net_dev_priv(pst_netdev) == NULL)) {
        ...
        switch (type) {
            case ANDR_WIFI_ATTRIBUTE_COUNTRY:
                ...
                if (memcpy_s(auc_country_code, sizeof(auc_country_code), oal_nla_data(iter),
                    oal_min(oal_nla_len(iter), sizeof(auc_country_code))) != EOK) {
    ...
    uint8_t *auc_random_mac_oui, uint8_t auc_random_mac_oui_len)
{
    ...
    ***************************************************************************/
    ...
    if (memcpy_s(st_write_msg.auc_value, sizeof(st_write_msg.auc_value),
        auc_random_mac_oui, auc_random_mac_oui_len) != EOK) {
    ...
    const void *p_data, int32_t l_len)
{
    ...
    if (l_type == ANDR_WIFI_ATTRIBUTE_RANDOM_MAC_OUI) {
        ...
        if (memcpy_s(auc_random_mac_oui, WLAN_RANDOM_MAC_OUI_LEN,
            oal_nla_data(p_data), oal_nla_len(p_data)) != EOK) {
    ...
    uint8_t *puc_mac_addr, uint8_t puc_mac_addr_len)
{
    ...
    ***************************************************************************/
    ...
    if (memcpy_s(st_write_msg.auc_value, sizeof(st_write_msg.auc_value), puc_mac_addr, puc_mac_addr_len) != EOK) {
...
OAL_STATIC int32_t wal_cfgvendor_set_bssid_blacklist_info(
    ...
    wal_wifi_bssid_params *bssid_info, int32_t remaining_len_input)
{
    int32_t type;
    uint8_t i;
    int32_t remaining_len = remaining_len_input;
    ...
    oam_warning_log3(0, OAM_SF_ANY, "{wal_cfgvendor_set_bssid_blacklist_info::bssid[0]=%02x:xx:xx:xx:%02x:%02x}\r\n",
        bssid_info->bssids[0][MAC_ADDR_0], bssid_info->bssids[0][MAC_ADDR_4], bssid_info->bssids[0][MAC_ADDR_5]);
    oam_warning_log3(0, OAM_SF_ANY, "{wal_cfgvendor_set_bssid_blacklist_info::bssid[1]=%02x:xx:xx:xx:%02x:%02x}\r\n",
        bssid_info->bssids[1][MAC_ADDR_0], bssid_info->bssids[1][MAC_ADDR_4], bssid_info->bssids[1][MAC_ADDR_5]);
    oam_warning_log3(0, OAM_SF_ANY, "{wal_cfgvendor_set_bssid_blacklist_info::bssid[2]=%02x:xx:xx:xx:%02x:%02x}\r\n",
        bssid_info->bssids[BYTE_OFFSET_2][MAC_ADDR_0],
        bssid_info->bssids[BYTE_OFFSET_2][MAC_ADDR_4], bssid_info->bssids[BYTE_OFFSET_2][MAC_ADDR_5]);
                                                         ...
                                                         uint8_t *pc_keep_alive_info, uint8_t uc_msg_len)
{
    ...
    ***************************************************************************/
    ...
    if (memcpy_s(st_write_msg.auc_value, sizeof(st_write_msg.auc_value), pc_keep_alive_info, uc_msg_len) != EOK) {
 ...
 *   修改内容  : 新生成函数
 */
int32_t wl_cfgvendor_start_vowifi_nat_keep_alive(oal_wiphy_stru *wiphy, oal_wireless_dev_stru *wdev,
    const void *data, int32_t len)
{
    uint16_t ip_pkt_len = 0;
    uint16_t msg_len;
    ...
    /* 入参检查 */
    if (oal_any_null_ptr2(wdev, data) || (len <= 0)) {
        oam_error_log1(0, OAM_SF_ANY, "{wl_cfgvendor_start_vowifi_nat_keep_alive::invalid para len=%d.}", len);
        return -OAL_EINVAL;
    }

    /* nla长度检查 */
    ret = wl_cfgvendor_vowifi_check_len_valid(wiphy, wdev, data, len);
    if (ret != OAL_SUCC) {
        return ret;
    }

    /* nla参数有效性检查 */
    ret = wl_cfgvendor_vowifi_check_para_valid(wiphy, wdev, data, len);
    if (ret != OAL_SUCC) {
        return ret;
    }

    /* 获取pkt_len用于申请内存: idata + 消息头(mac_vowifi_nat_keep_alive_start_info_stru) + ether_type字段(uint16_t) */
    ip_pkt_len = wl_cfgvendor_vowifi_get_pkt_len(wiphy, wdev, data, len);
    ...
    wal_send_vowifi_nat_keep_alive_params(wdev->netdev, (uint8_t *)start_info, msg_len);
    ...
    hmac_vap_stru *hmac_vap, uint8_t *p_mac)
{
    ...
    /* 填写 msg 消息体 */
    if (memcpy_s(st_write_msg.auc_value, sizeof(st_write_msg.auc_value),
        pst_query_station_info, sizeof(dmac_query_station_info_request_event)) != EOK) {
...
OAL_STATIC int32_t wal_cfgvendor_lstats_get_event_timeout_valid(hmac_vap_stru *hmac_vap)
{
    ...
    i_leftime = oal_wait_event_interruptible_timeout_m(hmac_vap->query_wait_q,
        (hmac_vap->station_info_query_completed_flag == OAL_TRUE), QUERY_STATION_INFO_TIME_INTER);
    if (i_leftime <= 0) {
    ...
    wal_wifi_iface_stat_stru *iface_stat)
{
    ...
    /* 固定时间内最多更新一次RSSI */
    if (wal_cfg80211_get_station_filter(&hmac_vap->st_vap_base_info, p_mac) == OAL_FALSE) {
...
/*
 * 函 数 名  : wal_cfgvendor_pkt_filter_get_capabilities
 * 功能描述  : 获取pkt_filter能力
 * 1.日    期  : 2018年3月17日
 *   作    者  : wifi
 *   修改内容  : 新生成函数
 */
int32_t wal_cfgvendor_pkt_filter_get_capabilities(oal_wiphy_stru *wiphy, oal_wireless_dev_stru *wdev,
    const void *data, int32_t len)
{
    oal_netbuf_stru *skb = NULL;
    ...
    if (oal_any_null_ptr3(wdev, wdev->netdev, wiphy)) {
        oam_error_log0(0, OAM_SF_ANY, "{wal_cfgvendor_pkt_filter_get_capabilities:input params null.}");
        return -OAL_EFAUL;
    }
    skb = oal_cfg80211_vendor_cmd_alloc_reply_skb(wiphy, l_mem_needed);
    if (oal_unlikely(!skb)) {
        oam_warning_log1(0, OAM_SF_ANY, "wal_cfgvendor_pkt_filter_get_capabilities::skb alloc failed, len %d",
            l_mem_needed);
        return -OAL_ENOMEM;
    }

    if (oal_strcmp(oal_netdevice_name(wdev->netdev), "wlan0") == 0) {
        max_len = PKT_FILTER_PROGRAM_MAX_LEN;
    } else {
        max_len = 0;
    }

    oal_nla_put_u32(skb, PKT_FILTER_ATTRIBUTE_VERSION, PKT_FILTER_VERSION);
    oal_nla_put_u32(skb, PKT_FILTER_ATTRIBUTE_MAX_LEN, max_len);

    l_ret = oal_cfg80211_vendor_cmd_reply(skb);
    if (oal_unlikely(l_ret)) {
        oam_error_log1(0, OAM_SF_ANY,
            "wal_cfgvendor_pkt_filter_get_capabilities::Vendor Command reply failed, ret:%d.", l_ret);
    }
    return l_ret;
}
static int32_t wal_cfgvendor_pkt_filter_get_program_len(mac_pkt_filter_cmd_stru *pkt_filter_cmd,
    const oal_nlattr_stru *iter)
{
    uint32_t program_len;
    if (oal_nla_len(iter) < (int)sizeof(uint32_t)) {
        oam_error_log1(0, OAM_SF_ANY, "wal_cfgvendor_pkt_filter_get_program_len:nla_len=%d", oal_nla_len(iter));
        return -OAL_EINVAL;
    }
    program_len = oal_nla_get_u32(iter);
    if (oal_unlikely(!program_len || program_len > PKT_FILTER_PROGRAM_MAX_LEN)) {
        oam_error_log1(0, OAM_SF_ANY, "wal_cfgvendor_pkt_filter_get_program_len::program len[%d] invalid", program_len);
        return -OAL_EINVAL;
    }
    pkt_filter_cmd->us_program_len = (uint16_t)program_len;
    return OAL_SUCC;
}
static int32_t wal_cfgvendor_pkt_filter_program_proc(mac_pkt_filter_cmd_stru *pkt_filter_cmd,
    const oal_nlattr_stru *iter)
{
    if (oal_unlikely(!pkt_filter_cmd->us_program_len)) {
        oam_error_log0(0, OAM_SF_ANY, "wal_cfgvendor_pkt_filter_program_proc::program len not set");
        return -OAL_EINVAL;
    }
    if (oal_nla_len(iter) < (int)sizeof(uint8_t)) {
        oam_error_log1(0, OAM_SF_ANY, "wal_cfgvendor_pkt_filter_program_proc:nla_len=%d", oal_nla_len(iter));
        return -OAL_EINVAL;
    }
    pkt_filter_cmd->puc_program = (uint8_t *)oal_nla_data(iter);
    pkt_filter_cmd->en_cmd_type = PKT_FILTER_SET_FILTER_CMD;
    return OAL_SUCC;
}
/*
 * 函 数 名  : wal_cfgvendor_pkt_filter_set_filter
 * 功能描述  : 设置pkt filter
 * 1.日    期  : 2018年3月17日
 *   作    者  : wifi
 *   修改内容  : 新生成函数
 */
int32_t wal_cfgvendor_pkt_filter_set_filter(oal_wiphy_stru *wiphy, oal_wireless_dev_stru *wdev,
    const void *data, int32_t len)
{
    const oal_nlattr_stru *iter = NULL;
    ...
    if (oal_any_null_ptr2(wdev, wdev->netdev) || (data == NULL) || (len <= 0)) {
        oam_error_log0(0, OAM_SF_ANY, "{wal_cfgvendor_pkt_filter_set_filter:input params null.}");
        return -OAL_EFAUL;
    }

    st_pkt_filter_cmd.us_program_len = 0;

    /* assumption: length attribute must come first */
    oal_nla_for_each_attr(iter, data, len, l_tmp) {
        l_type = oal_nla_type(iter);
        switch (l_type) {
            case PKT_FILTER_ATTRIBUTE_PROGRAM_LEN:
                ret = wal_cfgvendor_pkt_filter_get_program_len(&st_pkt_filter_cmd, iter);
                if (ret != OAL_SUCC) {
                    return ret;
                }
                break;
            case PKT_FILTER_ATTRIBUTE_PROGRAM:
                ret = wal_cfgvendor_pkt_filter_program_proc(&st_pkt_filter_cmd, iter);
                if (ret != OAL_SUCC) {
                    return ret;
                }
                /***************************************************************************
                    抛事件到wal层处理
                ***************************************************************************/
                wal_write_msg_hdr_init(&st_write_msg, WLAN_CFGID_SET_PKT_FILTER, sizeof(st_pkt_filter_cmd));
                if (memcpy_s(st_write_msg.auc_value, sizeof(st_write_msg.auc_value),
                    &st_pkt_filter_cmd, sizeof(st_pkt_filter_cmd)) != EOK) {
                    return -OAL_EFAIL;
                }

                /* 发送消息 */
                /* 需要将发送该函数设置为同步，否则hmac处理时会使用已释放的内存 */
                if (wal_send_cfg_event(wdev->netdev, WAL_MSG_TYPE_WRITE, WAL_MSG_WRITE_MSG_HDR_LENGTH +
                    sizeof(st_pkt_filter_cmd), (uint8_t *)&st_write_msg, OAL_TRUE, &pst_rsp_msg) != OAL_SUCC) {
                    oam_warning_log0(0, OAM_SF_ANY, "{wal_cfgvendor_pkt_filter_set_filter::wal_send_cfg_event fail!}");
                    return -OAL_EFAIL;
                }
                if (wal_check_and_release_msg_resp(pst_rsp_msg) != OAL_SUCC) {
                    oam_warning_log0(0, OAM_SF_ANY, "{wal_cfgvendor_pkt_filter_set_filter::msg_resp fail!}");
                    return -OAL_EFAIL;
                }
                break;
            default:
                oam_error_log1(0, OAM_SF_ANY, "wal_cfgvendor_pkt_filter_set_filter::no such attribute %d", l_type);
                return -OAL_EINVAL;
        }
    }

    oam_warning_log1(0, 0, "wal_cfgvendor_pkt_filter_set_filter: program len %d", st_pkt_filter_cmd.us_program_len);
    return OAL_SUCC;
}
#endif
...
{
    oam_warning_log3(0, OAM_SF_FTM, "wal_cfgvendor_rtt_params_print:bssid[%X:XX:XX:XX:%X:%X]",
...
/* 功能描述: 解析上层下发chba连接私有app ie */
OAL_STATIC int32_t wal_cfgvendor_fill_chba_conn_notify_assoc_priv_ie_msg(hmac_chba_conn_notify_stru *ntf_info,
    const oal_nlattr_stru *iter)
{
    uint8_t *assoc_priv_ie = NULL;
    int32_t ret;

    ntf_info->assoc_priv_ie_len = (uint16_t)oal_nla_get_u8(iter);
    if (ntf_info->assoc_priv_ie_len == 0) {
        return OAL_SUCC;
    }
    if (ntf_info->assoc_priv_ie_len > HMAC_CHBA_USER_ASSOC_PRIV_IE_MAX_LEN) {
        oam_error_log1(0, OAM_SF_ANY, "wal_cfgvendor_fill_chba_conn_notify_assoc_priv_ie_msg:priv_ie_len[%d]",
            ntf_info->assoc_priv_ie_len);
        return -OAL_EFAIL;
    }

    assoc_priv_ie = (uint8_t *)oal_nla_data(iter);
    ret = memcpy_s(ntf_info->assoc_priv_ie, HMAC_CHBA_USER_ASSOC_PRIV_IE_MAX_LEN, assoc_priv_ie + 1,
        ntf_info->assoc_priv_ie_len);
    if (ret != EOK) {
        oam_error_log0(0, OAM_SF_ANY,
            "wal_cfgvendor_fill_chba_conn_notify_assoc_priv_ie_msg:mac addr memcpy fail!");
        return -OAL_EFAIL;
    }
    return OAL_SUCC;
}
...
{
    hmac_chba_conn_notify_stru *ntf = (hmac_chba_conn_notify_stru*)msg->auc_value;
    const oal_nlattr_stru *iter = NULL;
    int32_t tmp, attribute;
    oal_nla_for_each_attr(iter, data, len, tmp) {
        attribute = oal_nla_type(iter);
        ...
        switch (attribute) {
            case MAC_CHBA_CONN_NTF_ATTR_ID:
                ntf->id = oal_nla_get_u8(iter);
                break;
            case MAC_CHBA_CONN_NTF_ATTR_MAC_ADDR:
                if (memcpy_s(ntf->peer_mac_addr, WLAN_MAC_ADDR_LEN, oal_nla_data(iter), oal_nla_len(iter)) != EOK) {
                    oam_error_log0(0, 0, "wal_cfgvendor_chba_conn_notify::mac addr memcpy fail!");
                    return -OAL_EFAIL;
                }
                break;
            case MAC_CHBA_CONN_NTF_ATTR_CENTER_FREQ_20M:
                ntf->channel.center_freq_20m = oal_nla_get_u32(iter);
                break;
            case MAC_CHBA_CONN_NTF_ATTR_CENTER_FREQ_1:
                ntf->channel.center_freq1 = oal_nla_get_u32(iter);
                break;
            case MAC_CHBA_CONN_NTF_ATTR_CENTER_FREQ_2:
                ntf->channel.center_freq2 = oal_nla_get_u32(iter);
                break;
            case MAC_CHBA_CONN_NTF_ATTR_CENTER_BANDWIDTH:
                ntf->channel.bandwidth = oal_nla_get_u8(iter);
                break;
            case MAC_CHBA_CONN_NTF_ATTR_EXPIRE_TIME:
                ntf->expire_time = oal_nla_get_u16(iter);
                break;
            case MAC_CHBA_CONN_NTF_ATTR_CONNT_FLAG:
                ntf->conn_flag = oal_nla_get_u8(iter);
                break;
            case MAC_CHBA_CONN_NTF_ATTR_ASSOC_PRIV_IE:
                if (wal_cfgvendor_fill_chba_conn_notify_assoc_priv_ie_msg(ntf, iter) != OAL_SUCC) {
                    return -OAL_EFAIL;
                }
                break;
            default:
    ...
    oam_warning_log2(0, 0, "CHBA: rcv conn notify. len %d, ntf id %d", len, ntf->id);
    return OAL_SUCC;
}

/* 功能描述: connect notify命令下发时打断已有扫描 */
static void wal_cfgvendor_chba_conn_notify_abort_scan(oal_wireless_dev_stru *wdev)
{
    ...
    /* 非扫描状态, 无需处理 */
    if (scan_mgmt->en_is_scanning == OAL_FALSE) {
        return;
    }

    /* chba场景p2p listen为高优先级事件, 暂不打断 */
    if (scan_mgmt->st_scan_record_mgmt.en_scan_mode == WLAN_SCAN_MODE_P2P_LISTEN) {
        return;
    }

    oam_warning_log2(mac_vap->uc_vap_id, OAM_SF_SCAN,
        "{wal_cfgvendor_chba_conn_notify_abort_scan:abort vap[%d] scan, scan_mode[%d]}",
        scan_mgmt->st_scan_record_mgmt.uc_vap_id, scan_mgmt->st_scan_record_mgmt.en_scan_mode);
    /* connect_notify命令下发前打断正在执行的扫描 */
    wal_force_abort_device_scan_and_wait_compelte(mac_vap->uc_device_id);
}
...

{
    ...
    wal_cfgvendor_chba_conn_notify_abort_scan(wdev);
...
OAL_STATIC int32_t wal_cfgvendor_fill_cancel_chba_conn_notify_msg(wal_msg_write_stru *msg, const void *data,
    int32_t len)
{
    hmac_chba_cancel_conn_notify_stru *ntf_info = (hmac_chba_cancel_conn_notify_stru*)msg->auc_value;
    const oal_nlattr_stru *iter = NULL;
    int32_t tmp;
    int32_t attribute;
    oal_nla_for_each_attr(iter, data, len, tmp) {
        attribute = oal_nla_type(iter);
        switch (attribute) {
            case MAC_CHBA_CANCEL_CONN_NTF_ATTR_ID:
                if (oal_nla_len(iter) < (int)sizeof(uint8_t)) {
                    oam_error_log2(0, OAM_SF_ANY, "wal_cfgvendor_fill_cancel_chba_conn_notify_msg:attr=%d, nla_len=%d",
                        attribute, oal_nla_len(iter));
                    return -OAL_EFAIL;
                }
                ntf_info->op_id = oal_nla_get_u8(iter);
                break;
            case MAC_CHBA_CANCEL_CONN_NTF_ATTR_MAC_ADDR:
                if (memcpy_s(ntf_info->peer_mac_addr, WLAN_MAC_ADDR_LEN, oal_nla_data(iter),
                    oal_nla_len(iter)) != EOK) {
                    oam_error_log0(0, 0, "wal_cfgvendor_fill_cancel_chba_conn_notify_msg:mac addr memcpy fail!");
                    return -OAL_EFAIL;
                }
                break;
            default:
                oam_error_log1(0, OAM_SF_ANY, "wal_cfgvendor_fill_cancel_chba_conn_notify_msg:no attr %d", attribute);
                return -OAL_EINVAL;
        }
    }
    oam_warning_log2(0, 0, "CHBA: rcv cancel conn notify. len %d, ntf id %d", len, ntf_info->op_id);
    return OAL_SUCC;
}

int32_t wal_cfgvendor_chba_cancel_conn_notify(oal_wiphy_stru *wiphy,
    oal_wireless_dev_stru *wdev, const void *data, int32_t len)
{
    wal_msg_write_stru write_msg = {0};
    int32_t ret;
    if ((wdev == NULL) || (data == NULL) || (len <= 0)) {
        oam_error_log1(0, OAM_SF_ANY, "{wal_cfgvendor_chba_cancel_conn_notify::invalid para l_len=%d.}", len);
        return -OAL_EINVAL;
    }
    if (wal_cfgvendor_fill_cancel_chba_conn_notify_msg(&write_msg, data, len) != OAL_SUCC) {
        return -OAL_EINVAL;
    }
    wal_write_msg_hdr_init(&write_msg, WLAN_CFGID_CHBA_CANCEL_CONN_NOTIFY, sizeof(hmac_chba_cancel_conn_notify_stru));
    ret = wal_send_cfg_event(wdev->netdev, WAL_MSG_TYPE_WRITE,
        WAL_MSG_WRITE_MSG_HDR_LENGTH + sizeof(hmac_chba_cancel_conn_notify_stru),
        (uint8_t*)&write_msg, OAL_FALSE, NULL);
    if (ret != OAL_SUCC) {
        oam_error_log1(0, OAM_SF_ANY, "{wal_cfgvendor_chba_cancel_conn_notify::wal_send_cfg_event fail[%d]!}", ret);
        return -OAL_EFAIL;
    }
