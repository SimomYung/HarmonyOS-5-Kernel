...
int32_t wal_regdomain_update(oal_net_device_stru *net_dev, int8_t *country)
{
    ...
    hwifi_set_country_code(country, COUNTRY_CODE_LEN);
    /* 如果新的国家码和旧国家处于一个regdomain，不刷新RF参数，只更新国家码 */
    if (hwifi_is_regdomain_changed((uint8_t *)old_pc_country, (uint8_t *)country) == OAL_TRUE) {
    ...
    if (!wal_is_alpha_upper(country[0]) || !wal_is_alpha_upper(country[1])) {
        if ((country[0] == '9') && (country[1] == '9')) {
    ...
    pst_regdom = wal_regdb_find_db(country);
    if (pst_regdom == NULL) {
        oam_warning_log0(0, OAM_SF_ANY, "{wal_regdomain_update::no regdomain db was found!}");
        return -OAL_EINVAL;
    }

    ret = wal_regdomain_update_post_event(net_dev, pst_regdom, country, OAL_TRUE);
    if (ret != OAL_SUCC) {
        return ret;
    }
...
OAL_STATIC void wal_regdomain_update_rf_param(oal_net_device_stru *net_dev, int8_t *pc_country, int8_t *old_pc_country)
{
    ...
    /* 如果新的国家码和旧国家处于一个regdomain，不刷新RF参数，只更新国家码 */
    if (hwifi_is_regdomain_changed((uint8_t *)old_pc_country, (uint8_t *)pc_country) == OAL_TRUE) {
...
int32_t wal_regdomain_update_for_dfs(oal_net_device_stru *net_dev, int8_t *country)
{
    ...
    *pst_param = wal_get_dfs_domain(country);
...
int32_t wal_regdomain_update_selfstudy_country_code(oal_net_device_stru *net_dev, int8_t *country)
{
    ...
    ret = wal_selfstdy_regdomain_update(net_dev, country);
    if (oal_unlikely(ret != OAL_SUCC)) {
    ...
    const int8_t *puc_token, uint32_t *pul_cmd_offset, uint32_t param_max_len)
{
    ...
    *pul_cmd_offset = (uint32_t)(pc_cmd_copy - pc_cmd);
    ...
    if (oal_memcmp(pc_cmd_tmp, puc_token, OAL_STRLEN(puc_token)) != 0) {
        ...
        if (memcpy_s(pc_arg, param_max_len, ac_cmd_copy + OAL_STRLEN(puc_token), arg_len) != EOK) {
...
void wal_config_mac_filter(oal_net_device_stru *net_dev, int8_t *pc_command)
{
    ...
    for (i = 0; i < mac_cnt; i++) {
        ...
        /* 5.1  检查参数是否符合MAC长度 */
        if (OAL_STRLEN(ac_parsed_command) != WLAN_MAC_ADDR_LEN << BIT_OFFSET_1) {
            oam_warning_log0(0, OAM_SF_ANY, "{wal_config_mac_filter::invalid MAC format}");
            return;
        }
        /* 6. 添加过滤设备 */
        if (wal_config_mac_filter_event_cfg(net_dev, mac_cnt, i, ac_parsed_command,
            sizeof(ac_parsed_command)) != OAL_SUCC) {
            return;
        }
    }
}
...
static uint32_t wal_misc_ioctl_copy_from_user(oal_iwreq_data_union *pst_wrqu, int8_t **pc_command)
{
    int8_t *cmd = NULL;
    uint32_t ret = OAL_SUCC;
#if defined(CONFIG_COMPAT) && defined(_PRE_HISI_HERTZ)
    struct compat_iw_point *iwp_compat = NULL;
    void __user *ptr = NULL;

    iwp_compat = (struct compat_iw_point *)&pst_wrqu->data;
    ptr = compat_ptr(iwp_compat->pointer);
#endif

    /* 1. 申请内存保存netd 下发的命令和数据 */
#if defined(CONFIG_COMPAT) && defined(_PRE_HISI_HERTZ)
    cmd = oal_memalloc((int32_t)(iwp_compat->length + 1));
#else
    cmd = oal_memalloc((int32_t)(pst_wrqu->data.length + 1));
#endif
    if (cmd == NULL) {
        oam_error_log0(0, OAM_SF_ANY, "{wal_misc_ioctl_copy_from_user: oal_memalloc fail.}");
        return OAL_FAIL;
    }
    /* 2. 拷贝netd 命令到内核态中 */
#if defined(CONFIG_COMPAT) && defined(_PRE_HISI_HERTZ)
    memset_s(cmd, (uint32_t)(iwp_compat->length + 1), 0, (uint32_t)(iwp_compat->length + 1));
    ret = oal_copy_from_user(cmd, ptr, (uint32_t)(iwp_compat->length));
#else
    memset_s(cmd, (uint32_t)(pst_wrqu->data.length + 1), 0, (uint32_t)(pst_wrqu->data.length + 1));
    ret = oal_copy_from_user(cmd, pst_wrqu->data.pointer, (uint32_t)(pst_wrqu->data.length));
#endif
    if (ret != OAL_SUCC) {
        oam_error_log0(0, OAM_SF_ANY, "{wal_misc_ioctl_copy_from_user::oal_copy_from_user: OAL_FAIL }");
        oal_free(cmd);
        return OAL_FAIL;
    }
#if defined(CONFIG_COMPAT) && defined(_PRE_HISI_HERTZ)
    cmd[iwp_compat->length] = '\0';
#else
    cmd[pst_wrqu->data.length] = '\0';
#endif
    *pc_command = cmd;
    return OAL_SUCC;
}
    ...
    oal_iw_request_info_stru *pst_info, oal_iwreq_data_union *pst_wrqu, char *pc_extra)
{
    ...
    result = wal_misc_ioctl_copy_from_user(pst_wrqu, &pc_command);
    if (result != OAL_SUCC) {
...
uint32_t wal_init_chba_netdev(oal_wiphy_stru *wiphy, const char *dev_name)
{
    ...
    if (oal_unlikely(ret != OAL_SUCC)) {
        oam_warning_log2(0, OAM_SF_ANY, "{wal_init_chba_netdev::hmac_net_register_netdev err:%d, dev_reg_stat:%d!}",
            ret, net_dev->reg_state);
