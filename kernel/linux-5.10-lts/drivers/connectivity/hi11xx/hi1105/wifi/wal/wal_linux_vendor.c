...
#ifdef _PRE_WLAN_FEATURE_ML
OAL_STATIC void wal_sta_get_mld_info(hmac_vap_stru *hmac_vap, mld_info_rpt *mld_infos)
{
    uint8_t i;
    hmac_vap_stru *link_hmac_vap = NULL;

    memset_s(mld_infos, sizeof(mld_info_rpt), 0, sizeof(mld_info_rpt));
    memcpy_s(mld_infos->sta_mld_addr, WLAN_MAC_ADDR_LEN,
        hmac_vap->mld_vap_link_info.mld_vap_manage->mld_addr, WLAN_MAC_ADDR_LEN);
    memcpy_s(mld_infos->ap_mld_addr, WLAN_MAC_ADDR_LEN,
        hmac_vap->mld_vap_link_info.mld_vap_manage->mld_bssid, WLAN_MAC_ADDR_LEN);
    for (i = 0; i < WLAN_MAX_ML_LINK_NUM; i++) {
        link_hmac_vap = (hmac_vap_stru *)hmac_vap->mld_vap_link_info.mld_vap_manage->link_vap[i];
        if (link_hmac_vap == NULL) {
            continue;
        }

        mld_infos->link_info[i].band = link_hmac_vap->st_vap_base_info.st_channel.en_band;
        mld_infos->link_info[i].ap_link_id = link_hmac_vap->mld_vap_link_info.link_ap_info.link_id;
        memcpy_s(mld_infos->link_info[i].addr, WLAN_MAC_ADDR_LEN,
            mac_mib_get_StationID(&link_hmac_vap->st_vap_base_info), WLAN_MAC_ADDR_LEN);
        memcpy_s(mld_infos->link_info[i].bssid, WLAN_MAC_ADDR_LEN,
            link_hmac_vap->st_vap_base_info.auc_bssid, WLAN_MAC_ADDR_LEN);
        mld_infos->link_info[i].assoc_succ = link_hmac_vap->mld_vap_link_info.link_ap_info.asoc_succ;
    }
}

int32_t wal_vendor_sta_get_mld_info_handle(oal_net_device_stru *net_dev, wal_wifi_priv_cmd_stru *priv_cmd, int8_t *cmd)
{
    mld_info_rpt mld_infos;
    mac_vap_stru *mac_vap = NULL;
    hmac_vap_stru *hmac_vap = NULL;

    oam_warning_log0(0, OAM_SF_CFG, "wal_vendor_sta_get_mld_info_handle::start get mld info!");
    if (priv_cmd->total_len < sizeof(mld_info_rpt)) {
        oam_error_log1(0, OAM_SF_ANY, "wal_vendor_sta_get_mld_info_handle:total_len=%d", priv_cmd->total_len);
        return OAL_SUCC;
    }
    /* 获取mac_vap */
    mac_vap = oal_net_dev_priv(net_dev);
    if (mac_vap == NULL) {
        oam_warning_log0(0, OAM_SF_CFG, "{wal_vendor_sta_get_mld_info_handle::netdevice->mac_vap is null.}");
        return OAL_SUCC;
    }

    hmac_vap = (hmac_vap_stru *)mac_res_get_hmac_vap(mac_vap->uc_vap_id);
    if (hmac_vap == NULL) {
        oam_warning_log0(0, OAM_SF_CFG, "{wal_vendor_sta_get_mld_info_handle::pst_hmac_vap is null.}");
        return OAL_SUCC;
    }

    if (hmac_vap->mld_vap_link_info.mld_vap_manage == NULL ||
        hmac_vap->mld_vap_link_info.mld_vap_manage->in_use != OAL_TRUE) {
        oam_warning_log0(0, OAM_SF_CFG, "{wal_vendor_sta_get_mld_info_handle::is not mld hmac!}");
        return -OAL_EFAIL;
    }

    wal_sta_get_mld_info(hmac_vap, &mld_infos);

    if (oal_copy_to_user(priv_cmd->puc_buf, (void *)&mld_infos, sizeof(mld_info_rpt)) != EOK) {
        oam_error_log0(0, OAM_SF_ANY, "wal_vendor_sta_get_mld_info_handle: Failed to copy ioctl_data to user !");
        return -OAL_EFAIL;
    }
    priv_cmd->used_len = sizeof(mld_info_rpt);
    oam_warning_log3(mac_vap->uc_vap_id, OAM_SF_CFG, "get_mld_info::link0_band=[%d], link0_succ=[%d], link1_succ=[%d]",
        mld_infos.link_info[0].band, mld_infos.link_info[0].assoc_succ, mld_infos.link_info[1].assoc_succ);
    return OAL_SUCC;
}

/* ioctl下发两个link的密钥和对应的link id，用于hmac_config_add_key时确认密钥是安装到哪个link的 */
int32_t wal_vendor_sta_set_mld_key_info_handle(oal_net_device_stru *net_dev, wal_wifi_priv_cmd_stru *priv_cmd,
    int8_t *cmd)
{
    wal_msg_write_stru write_msg;
    wal_msg_stru *rsp_msg = NULL;
    mac_vap_stru *mac_vap = NULL;
    hmac_vap_stru *hmac_vap = NULL;
    uint32_t skip = CMD_SET_MLD_KEY_INFO_LEN + 1;
    int8_t ret;
    uint32_t err_code;

    oam_warning_log0(0, OAM_SF_CFG, "wal_vendor_sta_set_mld_key_info_handle");

    if (wal_ioctl_judge_input_param_length(*priv_cmd, CMD_SET_MLD_KEY_INFO_LEN, sizeof(hmac_mld_key_info)) !=
        OAL_SUCC) {
        oam_warning_log0(0, OAM_SF_ANY, "{wal_vendor_sta_set_mld_key_info_handle::length is too short!}");
        return OAL_FAIL;
    }
    ...
    if (hmac_vap == NULL || hmac_vap->mld_vap_link_info.mld_vap_manage == NULL) {
        oam_warning_log0(0, OAM_SF_CFG, "{wal_vendor_sta_get_mld_info_handle::hmac_vap is null.}");
        return OAL_SUCC;
    }

    if (hmac_vap->mld_vap_link_info.mld_vap_manage->in_use != OAL_TRUE) {
        oam_warning_log0(0, OAM_SF_CFG, "{wal_vendor_sta_get_mld_info_handle::is not mld hmac!}");
        return OAL_SUCC;
    }

    /* 抛事件到wal层处理 */
    wal_write_msg_hdr_init(&write_msg, WLAN_CFGID_SET_MLD_KEY_INFO, sizeof(hmac_mld_key_info));

    /* 设置配置命令参数 */
    if (memcpy_s(write_msg.auc_value, sizeof(write_msg.auc_value), cmd + skip, sizeof(hmac_mld_key_info)) != EOK) {
        oam_warning_log0(0, OAM_SF_CFG, "{wal_vendor_sta_set_mld_key_info_handle::memcpy fail!}");
        return -OAL_EFAIL;
    }

    ret = wal_send_cfg_event(net_dev, WAL_MSG_TYPE_WRITE,
        WAL_MSG_WRITE_MSG_HDR_LENGTH + sizeof(hmac_mld_key_info), (uint8_t *)&write_msg, OAL_TRUE, &rsp_msg);
    if (ret != OAL_SUCC || rsp_msg == NULL) {
        oam_error_log1(0, OAM_SF_CFG,
            "{wal_vendor_sta_set_mld_key_info_handle::wal_send_cfg_event return err code %d!}", ret);
        return ret;
    }
    ...
    if (err_code != OAL_SUCC) {
        oam_warning_log1(0, OAM_SF_CFG, "{wal_vendor_sta_set_mld_key_info_handle::msg_resp fail[%d]!}", err_code);
        return -OAL_EFAIL;
    }
    return OAL_SUCC;
}
    ...
    hmac_user_info_stru *hmac_user_info)
{
    ...
    hmac_user_info->tx_packets = hmac_user->user_stats.tx_packets;
    hmac_user_info->tx_failed = hmac_vap->station_info.tx_failed;
    hmac_user_info->rx_packets = hmac_user->user_stats.rx_packets;
    ...
    uint32_t total_len)
{
    ...
    uint8_t *cmd_data = NULL;
    uint32_t remain_len =
        total_len > (CMD_SET_BE_BLACKLIST_LEN + 2) ? (total_len - CMD_SET_BE_BLACKLIST_LEN - 2) : 0;
    cmd_data = pc_cmd + CMD_SET_BE_BLACKLIST_LEN + 1;
    if (remain_len == 0) {
        oam_warning_log0(0, OAM_SF_11BE, "{wal_vendor_11be_wifi7_blacklist_handle::len is 0.}");
        return OAL_FAIL;
    }

    blacklist_cnt = (uint8_t)*cmd_data;
    cmd_data += 1;
    /* 命令中黑名单个数与后面的黑名单信息不匹配，返回失败 */
    ...
    ret = memcpy_s(wifi7_self_cure_info->wifi7_blacklist, blacklist_cnt * sizeof(hmac_wifi7_self_cure_blacklist_stru),
...
#ifdef _PRE_WLAN_FEATURE_ML
    if (oal_strncasecmp(p_cmd, CMD_STA_GET_MLD_INFO, CMD_STA_GET_MLD_INFO_LEN) == 0) {
        return wal_vendor_sta_get_mld_info_handle(p_net_dev, p_priv_cmd, p_cmd);
    }

    if (oal_strncasecmp(p_cmd, CMD_SET_MLD_KEY_INFO, CMD_SET_MLD_KEY_INFO_LEN) == 0) {
        return wal_vendor_sta_set_mld_key_info_handle(p_net_dev, p_priv_cmd, p_cmd);
    }
#endif
...
OAL_STATIC int32_t wal_ioctl_set_fastsleep_switch(oal_net_device_stru *pst_net_dev, int8_t *pc_command)
{
    ...
    if (!oal_strncasecmp(puc_para_str, "0", 1)) {
        ...
        wal_ioctl_set_fastsleep_switch_g_variable_assignment(pst_net_dev, auc_para_val, BYTE_OFFSET_4);
        /* 下发参数 */
        wal_ioctl_set_fast_sleep_para(pst_net_dev, auc_para_val, BYTE_OFFSET_4);
        /* 下发ps mode */
        wal_ioctl_set_sta_ps_mode(pst_net_dev, AUTO_FAST_PS);
        wal_set_fastsleep2_mode(pst_net_dev, OAL_TRUE);
