...
#define WAKEIN_IRQF    IRQF_NO_SUSPEND
#else
#define WAKEIN_IRQF    (IRQF_DISABLED | IRQF_NO_SUSPEND)
#endif
...
static int board_gpio_pins_init(struct device_node *dev_np, uint32_t *pins)
{
    ...
    uint32_t irq_flags;
    int gpio_nr;
    enum of_gpio_flags gpio_active_flag = 0;
    const tiot_kernel_pin_info *pin_infos = (const tiot_kernel_pin_info *)&g_board_pin_infos;
    /* 设置GPIO 初始pinmux. */
    for (i = 0; i < BSXX_PIN_NUM; i++) {
        gpio_nr = of_get_named_gpio_flags(dev_np, pin_infos[i].prop, 0, &gpio_active_flag);
...
#ifdef CONFIG_GPIOLIB
        printk("gpio[%d] active flags = %d.\r\n", gpio_nr, gpio_active_flag);
        gpio_active_flag = (gpio_active_flag == OF_GPIO_ACTIVE_LOW) ? GPIOF_ACTIVE_LOW : 0;
#else
        printk("GPIO_ACTIVE_LOW is not supported.\r\n");
        gpio_active_flag = 0;
#endif

        ret = gpio_request_one(gpio_nr, (pin_infos[i].gpio_flags | gpio_active_flag), NULL);
        if (ret != 0) {
            printk("gpio request one failed [%d].\r\n", gpio_nr);
            return -1;
        }
        if (pin_infos[i].gpio_flags == GPIOF_IN) {
            irq_flags = WAKEIN_IRQF;
            irq_flags |= (gpio_active_flag == GPIOF_ACTIVE_LOW) ? IRQF_TRIGGER_FALLING : IRQF_TRIGGER_RISING;
            /* 通过helper管理 irq gpio。 */
            ret = tiot_board_irq_pin_register(gpio_nr, irq_flags);
        }
        if (ret != 0) {
