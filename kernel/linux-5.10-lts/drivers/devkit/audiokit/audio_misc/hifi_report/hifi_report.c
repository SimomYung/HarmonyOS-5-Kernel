	...
	struct mb_queue *mail_handle, uint32_t mail_len)
{
	...
	hifi_priv = audio_misc_module_get_priv(MOD_NAME);
	if (!hifi_priv) 
		return;

	ret = hifi_report_parse_msg_type(rcv_msg.msg_type, hifi_priv);
	if (ret != EOK)
		return;

	ret = strncpy_s(hifi_priv->msg, sizeof(hifi_priv->msg), rcv_msg.data, sizeof(rcv_msg.data));
	if (ret != EOK)
		return;

	schedule_work(&hifi_priv->report_work);
	return;
}
