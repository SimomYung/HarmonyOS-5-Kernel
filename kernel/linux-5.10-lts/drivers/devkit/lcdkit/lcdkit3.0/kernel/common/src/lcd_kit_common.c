...
int lcd_kit_set_mipi_switch1_ctrl(int panel_id, int enable)
{
	struct lcd_kit_adapt_ops *adapt_ops = NULL;
...
int lcd_kit_set_mipi_switch2_ctrl(int panel_id, int enable)
{
	struct lcd_kit_adapt_ops *adapt_ops = NULL;
...
static int lcd_kit_svm_12pulse_cmd(int panel_id)
{
	int ret = LCD_KIT_OK;
	struct lcd_kit_adapt_ops *adapt_ops = NULL;
...
static int lcd_kit_set_dsc_switch_ctrl(int panel_id)
{
	int ret = LCD_KIT_OK;
	struct lcd_kit_adapt_ops *adapt_ops = NULL;
		...
		uint32_t event, uint32_t data, uint32_t delay)
{
	...
	case EVENT_COLOR_CALIB:
		ret = lcd_kit_color_calibration(panel_id, data);
		break;
	...
	case EVENT_EINK_WAKEUP:
		...
		break;
	case EVENT_SVM_OTA:
		lcd_kit_svm_12pulse_cmd(panel_id);
		break;
	case EVENT_MIPI_SWITCH1:
		ret = lcd_kit_set_mipi_switch1_ctrl(panel_id, data);
		break;
	case EVENT_MIPI_SWITCH2:
		ret = lcd_kit_set_mipi_switch2_ctrl(panel_id, data);
		break;
	case EVENT_DUL_DSI:
		ret = lcd_kit_dual_dsi_ctrl(panel_id, data);
		break;
	case EVENT_DSC_SWITCH:
		ret = lcd_kit_set_dsc_switch_ctrl(panel_id);
		break;
	default:
...
{
	int ret = LCD_KIT_OK;
	int grayscale_mode = GRAYSCALE_MODE_DEF;
	int table_num;
	int i;
	int j;
	struct lcd_kit_adapt_ops *adapt_ops = NULL;
