...
void lcd_kit_dsi_tx_params(struct mipi_dsi_tx_params *dsi_params,
	struct lcd_kit_dsi_panel_cmds *cmds, unsigned int dsi_index, int panel_id)
{
	...
	dsi_params->write_cmds.cmds_num = cmds->cmd_cnt;
...
void lcd_kit_dual_dsi_tx_params(struct mipi_dsi_tx_params *dsi0_params, struct mipi_dsi_tx_params *dsi1_params,
	struct lcd_kit_dsi_panel_cmds *cmds, unsigned int dsi0_index, unsigned int dsi1_index, int panel_id)
{
	...
	dsi0_params->write_cmds.cmds_num = cmds->cmd_cnt;
	...
	dsi1_params->write_cmds.cmds_num = cmds->cmd_cnt;
	...
	struct lcd_kit_dsi_panel_cmds *cmds)
{
	...
	new_mipi.connector_id = DPU_PINFO->connector_id;
	...
	unsigned char *out1, int out_len)
{
	...
	dual_mipi.connector_id = DPU_PINFO->connector_id;
	...
	for (i = 0; i < cmds->cmd_cnt; i++) {
		...
		if (lcd_kit_cmd_is_write(&dsi_cmd)) {
			...
			for (j = 0; (j < dsi_cmd.dlen) && (cnt0 <= out_len); j++) {
				out0[cnt0++] = tmp0[j];
				out1[cnt1++] = tmp1[j];
			}
		}
