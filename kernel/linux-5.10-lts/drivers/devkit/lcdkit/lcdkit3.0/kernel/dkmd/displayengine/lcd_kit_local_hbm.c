...
static void display_engine_local_hbm_delay_for_fake_off(int panel_id);
...
#define LHBM_FAKE_OFF_TOTAL_DELAY_US 30000
#define HBM_THRESHOD 255
#define HBM_MAX 500
#define MAX_ENTER_LHBM_WAIT 10
...
struct de_context_local_hbm {
	...
	struct completion enter_local_hbm_done;
};
...
struct timeval g_lhbm_start_time;
struct timeval g_lhbm_end_time;

static inline void display_engine_lhbm_do_gettimeofday(struct timeval *tv)
{
...
{
	long delay_time = 0;
	unsigned long during_time = 0;

	if (common_info->hbm.local_hbm_fake_off_delay_support != 0) {
		// this panel support fake off delay
		display_engine_lhbm_do_gettimeofday(&g_lhbm_end_time);
		during_time = g_lhbm_end_time.tv_usec - g_lhbm_start_time.tv_usec;
		delay_time = LHBM_FAKE_OFF_TOTAL_DELAY_US - during_time;
		LCD_KIT_INFO("lhbm fake off delay enter, delay_time:%d +\n", delay_time);
		if (delay_time > 0 && delay_time <= LHBM_FAKE_OFF_TOTAL_DELAY_US)
			usleep_range(delay_time, delay_time);
	}
	LCD_KIT_INFO("lhbm fake off delay exit\n");
	return;
}
...
static int display_engine_local_hbm_set_inner(int panel_id, int grayscale)
{
	...
	if (grayscale > 0) {
		reinit_completion(&g_de_context.enter_local_hbm_done);
		display_engine_lhbm_do_gettimeofday(&g_lhbm_start_time);
		...
		complete_all(&g_de_context.enter_local_hbm_done);
	} else {
...
int display_engine_local_hbm_brightness_update(unsigned int bl_level)
{
...
static void lcd_kit_parse_local_hbm_fake_off_delay_support(int panel_id, struct device_node *np)
{
	lcd_kit_parse_u32(np, "lcd-kit,local-hbm-fake-off-delay-support",
		&common_info->hbm.local_hbm_fake_off_delay_support, 0);
}
...
void lcd_kit_local_hbm_cmds_parse(int panel_id, struct device_node *np)
{
	...
	lcd_kit_parse_local_hbm_fake_off_delay_support(panel_id, np);
...
{
	LCD_KIT_INFO("+\n");
	init_completion(&g_de_context.enter_local_hbm_done);
...
{
	LCD_KIT_INFO("+\n");
	if (!wait_for_completion_timeout(&g_de_context.enter_local_hbm_done,
		msecs_to_jiffies(MAX_ENTER_LHBM_WAIT)))
		LCD_KIT_INFO("wait enter_local_hbm_done time out!\n");

	if (g_de_context.fingerprint_backlight_type == BACKLIGHT_HIGH_LEVEL) {
		display_engine_local_hbm_delay_for_fake_off(panel_id);
...
unsigned int display_engine_get_hbm_bl(int panel_id, unsigned int bl_level)
{
	...
	if (bl_src_interval <= 0 || bl_dst_interval <= 0)
		return bl_level;
