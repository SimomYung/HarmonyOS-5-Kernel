...
static int lcd_kit_send_final_safe_frm_rate(int panel_id)
{
	...
	LCD_KIT_DEBUG("final sfr:%u, last sfr:%u, force send sfr:%d, panel id:%d",
	...
	LCD_KIT_DEBUG("send safe frm rate success\n");
...
{
	lcd_kit_parse_u32(np, "lcd-kit,safe-frm-decision-by-fps-support",
		&disp_info->safe_frm_decision.fps_support, 0);
	if (disp_info->safe_frm_decision.fps_support) {
...
static void acorss_brightness_node_delay_work_handler(struct work_struct *work)
{
	...
	struct safe_frm_rate_work *sfr_work =
		container_of(work, struct safe_frm_rate_work, handle_work_brightness_change_delay_work.work);
	if (!sfr_work) {
		LCD_KIT_ERR("safe frm rate work is null\n");
		return;
	}
...
{
	LCD_KIT_DEBUG("tp touch event val:%d panel id: %d\n", val, panel_id);
	if (!disp_info->safe_frm_decision.tp_support)
		return;
	if (is_panel_id_invalid(panel_id))
		return;
