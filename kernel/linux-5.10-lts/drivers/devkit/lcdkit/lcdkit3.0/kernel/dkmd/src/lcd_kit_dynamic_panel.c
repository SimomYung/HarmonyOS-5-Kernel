	...
	struct dpu_dynamic_panel_info *dynamic_pinfo)
{
	...
	dynamic_pinfo->dfr_info.oled_info.ltpo_info.te_freq_num = te_freq.cnt;
	dynamic_pinfo->dfr_info.oled_info.ltpo_info.te_mask_num = te_mask.cnt;
	...
	struct device_node *np, struct dpu_dynamic_panel_info *dynamic_pinfo)
{
	...
	for (i = 0; i < cmd_num; i++) {
		dynamic_pinfo->dfr_info.oled_info.ltpo_info.dsi_cmds[i].cmd_num = ltpo_base_cmd[i].cmd_cnt;
	...
	struct dpu_dynamic_panel_info *dynamic_pinfo, unsigned int frame_rate, int num)
{
	...
	for (i = 0; i < cmd_num; i++) {
		if (frame_rate == fps_safe_cmd[i].val) {
			dynamic_pinfo->dfr_info.oled_info.ltpo_info.safe_frm_rates[num].dsi_cmds.cmd_num = fps_safe_cmd[i].cmds->cmd_cnt;
	...
	struct device_node *np, struct dpu_dynamic_panel_info *dynamic_pinfo)
{
	...
	dynamic_pinfo->dfr_info.oled_info.ltpo_info.safe_frm_rates_num = safe_freq.cnt;
	...
	struct device_node *np, struct dpu_dynamic_panel_info *dynamic_pinfo)
{
	...
	dynamic_pinfo->mipi.mipi_frm_rate_mode_num = disp_info->fps.panel_support_fps_list.cnt;
	for (i = 0; i < dynamic_pinfo->mipi.mipi_frm_rate_mode_num; i++) {
	...
	struct dpu_panel_info *pinfo, struct dpu_dynamic_panel_info *dynamic_pinfo)
{
	...
	dynamic_pinfo->dfr_info.oled_info.fps_sup_num = disp_info->fps.panel_support_fps_list.cnt;
