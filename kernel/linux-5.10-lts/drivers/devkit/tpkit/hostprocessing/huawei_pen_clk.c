...
static int thp_pcie_clk_ctrl(iodie_clk_out_cfg_enum status)
{
	int rc;
...
static int thp_soc_clk_enable(void)
{
...
int thp_pen_clk_enable(void)
{
...
#elif defined(CONFIG_CONTEXTHUB_IO_DIE_PCIE)
		return thp_pcie_clk_ctrl(IODIE_CLK_OUT_CMD_OPEN);
#else
	...
	return thp_soc_clk_enable();
}
EXPORT_SYMBOL(thp_pen_clk_enable);

static int thp_soc_clk_disable(void)
{
	struct pinctrl_state *state = NULL;
	int rc;
...
#elif defined(CONFIG_CONTEXTHUB_IO_DIE_PCIE)
		return thp_pcie_clk_ctrl(IODIE_CLK_OUT_CMD_CLOSE);
#else
		thp_log_err("%s: need check clk_source config\n", __func__);
		return 0;
#endif
