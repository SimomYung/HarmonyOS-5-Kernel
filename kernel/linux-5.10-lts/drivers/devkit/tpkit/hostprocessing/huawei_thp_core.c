...
#include <huawei_platform/edp/edp_ts_kit/edp_ts_kit.h>
#endif
...
#define thp_parse_dts_u32(cd, rc, node, name, value) \
	rc = of_property_read_u32(node, #name, &(value)); \
	thp_dts_parse_process_template(rc, value, &((cd)->name), \
		__func__, #name)
...
{
	for (int i = 0; i < ARRAY_SIZE(thp_execute_shb_event_table); i++) {
...
#ifdef CONFIG_HUAWEI_THP_QCOM
	value = gpiod_get_raw_value_cansleep(gpio_to_desc(cd->gpios.rst_gpio));
#else
	value = gpio_get_value(cd->gpios.rst_gpio);
#endif
	thp_log_info("%s: get rst value : %d\n", __func__, value);
	...
	struct device_node *thp_node, struct thp_core_data *cd)
{
	...
	thp_parse_dts_u32(cd, rc, thp_node, pen_supported, value);
	thp_parse_dts_u32(cd, rc, thp_node, pen_mt_enable_flag, value);
	thp_parse_dts_u32(cd, rc, thp_node, pen_change_protocol, value);
	thp_parse_dts_u32(cd, rc, thp_node, send_bt_status_to_fw, value);
	thp_parse_dts_u32(cd, rc, thp_node, send_adsorption_status_to_fw, value);
	thp_parse_dts_u32(cd, rc, thp_node, support_stylus3_plam_suppression, value);
	thp_parse_dts_u32(cd, rc, thp_node, send_stylus3_workmode_to_fw, value);
	thp_parse_dts_u32(cd, rc, thp_node, unsupported_stylus, value);
	thp_parse_dts_u32(cd, rc, thp_node, app_send_stylus3_status_to_fw, value);
	thp_parse_dts_u32(cd, rc, thp_node, support_adapter_mode, value);
	thp_parse_dts_u32(cd, rc, thp_node, support_stylus_high_hover, value);
	thp_parse_dts_u32(cd, rc, thp_node, support_stylus_extension, value);
}
	...
	struct device_node *thp_node, struct thp_core_data *cd)
{
	...
	thp_parse_dts_u32(cd, rc, thp_node, support_screen_switch, value);
#ifdef CONFIG_LIBLINUX
	thp_parse_dts_u32(cd, rc, thp_node, support_quick_fold_unfold, value);
#endif
	thp_parse_dts_u32(cd, rc, thp_node, support_ic_recovery, value);
	thp_parse_dts_u32(cd, rc, thp_node, need_check_first_screen_switch, value);

	cd->multi_panel_index = SINGLE_TOUCH_PANEL;
	thp_parse_dts_u32(cd, rc, thp_node, multi_panel_index, value);

	cd->support_multi_panel_attach = 0;
	thp_parse_dts_u32(cd, rc, thp_node, support_multi_panel_attach, value);

	cd->support_resend_status_to_daemon = 0;
	thp_parse_dts_u32(cd, rc, thp_node, support_resend_status_to_daemon, value);

	cd->support_input_location = 0;
	thp_parse_dts_u32(cd, rc, thp_node, support_input_location, value);
}
	...
	struct device_node *thp_node, struct thp_core_data *cd)
{
	...
	value = 0;
	rc = of_property_read_u32(thp_node, "use_dkmd_notify_to_sr", &value);
	if (!rc) {
		cd->use_dkmd_notify_to_sr = value;
		thp_log_info("%s: use_dkmd_notify_to_sr %u\n",
		__func__, value);
	}
	...
	struct thp_core_data *cd)
{
	...
	if (!rc) {
		...
		thp_log_info("%s:support_gesture_mode configed %u\n",
			__func__, value);
		thp_parse_dts_u32(cd, rc, thp_node, gesture_from_sensorhub, value);
	}
	thp_parse_dts_u32(cd, rc, thp_node, aod_display_support, value);
	thp_parse_dts_u32(cd, rc, thp_node, support_fingerprint_switch, value);
	thp_parse_dts_u32(cd, rc, thp_node, use_ap_gesture, value);
	thp_parse_dts_u32(cd, rc, thp_node, use_aod_power_ctrl_notify, value);
	thp_parse_dts_u32(cd, rc, thp_node, need_check_panel_from_aod, value);
#ifdef CONFIG_LIBLINUX
	thp_parse_dts_u32(cd, rc, thp_node, shb_slide_b2t_support, value);
	thp_parse_dts_u32(cd, rc, thp_node, shb_single_click_unlock_support, value);
#endif
	thp_parse_dts_u32(cd, rc, thp_node, lcd_gesture_mode_support, value);

	cd->double_click_use_ap_gesture = 0;
	thp_parse_dts_u32(cd, rc, thp_node, double_click_use_ap_gesture, value);
}
	...
	struct thp_core_data *cd)
{
	...
	thp_parse_dts_u32(cd, rc, thp_node, edit_product_in_project_id, value);
	...
	thp_parse_dts_u32(cd, rc, thp_node, projectid_from_panel_ver, value);

	cd->support_projectid_from_cmdline = 0;
	thp_parse_dts_u32(cd, rc, thp_node, support_projectid_from_cmdline, value);
	...
	struct thp_core_data *cd)
{
	...
	thp_parse_dts_u32(cd, rc, thp_node, support_vendor_ic_type, value);

	cd->need_resume_reset = 0;
	thp_parse_dts_u32(cd, rc, thp_node, need_resume_reset, value);

	cd->disable_irq_gesture_suspend = 0;
	thp_parse_dts_u32(cd, rc, thp_node, disable_irq_gesture_suspend, value);

	cd->support_dual_chip_arch = 0;
	thp_parse_dts_u32(cd, rc, thp_node, support_dual_chip_arch, value);

#if (defined(CONFIG_HUAWEI_THP_MTK) || defined(CONFIG_HUAWEI_THP_QCOM))
	value = 0;
	cd->change_spi_driving_force = 0;
	thp_parse_dts_u32(cd, rc, thp_node, change_spi_driving_force, value);

	cd->need_enable_irq_wake = 0;
	thp_parse_dts_u32(cd, rc, thp_node, need_enable_irq_wake, value);
#endif
	cd->gesture_retry_times = 20; /* defult gesture retry times:20 */
	thp_parse_dts_u32(cd, rc, thp_node, gesture_retry_times, value);

	cd->lcd_need_get_afe_status = 0;
	thp_parse_dts_u32(cd, rc, thp_node, lcd_need_get_afe_status, value);

	cd->iovdd_power_on_delay_ms = 1; /* default delay 1 ms */
	thp_parse_dts_u32(cd, rc, thp_node, iovdd_power_on_delay_ms, value);

	cd->change_vendor_name = 0;
	thp_parse_dts_u32(cd, rc, thp_node, change_vendor_name, value);

	cd->check_tp_vendor = 0;
	thp_parse_dts_u32(cd, rc, thp_node, check_tp_vendor, value);
}
	...
	struct thp_core_data *cd)
{
	...
	thp_parse_dts_u32(cd, rc, thp_node, self_control_power, value);
	thp_parse_dts_u32(cd, rc, thp_node, project_in_tp, value);
	thp_parse_dts_u32(cd, rc, thp_node, read_color_in_tp, value);
	thp_parse_dts_u32(cd, rc, thp_node, lcd_esd_event_upload, value);

	thp_parse_dts_u32(cd, rc, thp_node, supported_func_indicater, value);
	thp_parse_dts_u32(cd, rc, thp_node, use_hwlock, value);
	thp_parse_dts_u32(cd, rc, thp_node, support_shb_thp, value);
	thp_parse_dts_u32(cd, rc, thp_node, support_shb_thp_log, value);
	thp_parse_dts_u32(cd, rc, thp_node, support_shb_thp_app_switch, value);
	thp_parse_dts_u32(cd, rc, thp_node, tsa_event_to_udfp, value);
	thp_parse_dts_u32(cd, rc, thp_node, use_ap_sh_common_int, value);
	thp_parse_dts_u32(cd, rc, thp_node, delay_work_for_pm, value);
	thp_parse_dts_u32(cd, rc, thp_node, suspend_delayms_early_to_before, value);
	thp_parse_dts_u32(cd, rc, thp_node, suspend_delayms_before_to_later, value);
	thp_parse_dts_u32(cd, rc, thp_node, resume_delayms_early_to_later, value);
	thp_parse_dts_u32(cd, rc, thp_node, suspend_no_reset, value);
	thp_parse_dts_u32(cd, rc, thp_node, not_support_cover_mode, value);
	thp_parse_dts_u32(cd, rc, thp_node, support_jkm_mode, value);

	/*
	 * TDDI(TP powered by LCD) download fw in afe screen on,
	 * need wait interruptible to make sure of screen on done.
	 */
	thp_parse_dts_u32(cd, rc, thp_node, wait_afe_screen_on_support, value);
	thp_parse_dts_u32(cd, rc, thp_node, need_check_irq_status, value);

#if defined(CONFIG_TEE_TUI)
	thp_parse_dts_u32(cd, rc, thp_node, send_tui_exit_in_suspend, value);
	thp_parse_dts_u32(cd, rc, thp_node, no_need_secos_bus_init, value);
#endif
	...
	thp_parse_dts_u32(cd, rc, thp_node, support_ring_setting_switch, value);
	thp_parse_dts_u32(cd, rc, thp_node, support_extra_key_event_input, value);
#if defined(CONFIG_LIBLINUX)
	thp_parse_dts_u32(cd, rc, thp_node, support_hand_status_dev, value);
#endif
	thp_parse_dts_u32(cd, rc, thp_node, hide_product_info_en, value);
	thp_parse_dts_u32(cd, rc, thp_node, support_oem_info, value);
	thp_parse_dts_u32(cd, rc, thp_node, thp_compatible_tskit, value);
	thp_parse_dts_u32(cd, rc, thp_node, support_polymeric_chip, value);
...
static void thp_parse_spi_module_init_dts(struct thp_core_data *cd, struct device_node *thp_node)
{
	...
	cd->multi_spi = 0;
	rc = of_property_read_u32(thp_node, "multi_spi", &value);
	if (!rc) {
		cd->multi_spi = value;
		thp_log_info("%s:multi_spi parsed:%d\n",
			__func__, cd->multi_spi);
	}
	cd->hwspin_id = TP_HWSPIN_LOCK_CODE;
	rc = of_property_read_u32(thp_node, "hwspin_id", &value);
	if (!rc) {
		cd->hwspin_id = value;
		thp_log_info("%s:hwspin_id parsed:%d\n",
			__func__, cd->hwspin_id);
	}
}
...
static int thp_parse_module_init_dts(struct thp_core_data *cd, u32 panel_id)
{
	...
	rc = of_property_read_u32(thp_node, "product_frame_packet_num", &value);
	thp_dts_parse_process_template(rc, value, &(cd->product_frame_packet_num),
		__func__, "product_frame_packet_num");
	return NO_ERR;
}
