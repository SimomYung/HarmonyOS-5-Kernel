	...
	u32 panel_id, unsigned int have_pen_extension)
{
	...
	if (have_pen_extension) {
		...
		input_report_abs(input, ABS_MT_ANGLE, touch_and_time_data->twist);
	}
}
...
static int thp_mt_wrapper_ioctl_report_hand_status(unsigned long arg, u32 panel_id)
{
...
static int thp_set_pen_input_config(struct input_dev *pen_dev, u32 panel_id)
{
	...
	if (cd->support_stylus_extension)
		input_set_abs_params(pen_dev, ABS_MT_ANGLE, -65536, 65535, 0, 0);
