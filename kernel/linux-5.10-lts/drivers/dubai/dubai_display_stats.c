...
static int dubai_get_display_refresh_rate(void __user *argp)
{
	...
	refresh_rate_cmd.refresh_count.count_20hz = info.refresh_cnt[RATE_20HZ];
	...
	refresh_rate_cmd.refresh_count.count_36hz = info.refresh_cnt[RATE_36HZ];
	...
	refresh_rate_cmd.refresh_count.count_72hz = info.refresh_cnt[RATE_72HZ];
	...
	refresh_rate_cmd.refresh_count.count_144hz = info.refresh_cnt[RATE_144HZ];
	refresh_rate_cmd.total_count_time_us = info.total_count_time_us;
	// The two arrays are the same and will not overflow.
	memcpy_s(refresh_rate_cmd.block_refresh_cnt, MULTI_BLOCK_NUMS * sizeof(uint32_t), info.block_refresh_cnt,
		MULTI_BLOCK_NUMS * sizeof(uint32_t));
	refresh_rate_cmd.full_dimming_refresh_count = info.full_dimming_refresh_cnt;
#endif
