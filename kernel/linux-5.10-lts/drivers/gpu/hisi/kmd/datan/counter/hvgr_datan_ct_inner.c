...
static void hvgr_datan_ct_wait_for_config_inner(struct hvgr_ctx *ctx)
{
	...
	if (times == DUMP_WAIT_MAX_TIME)
		hvgr_debug(gdev, HVGR_DATAN, "ctx_%u config wait timeout.", ctx->id);
exit:
	hvgr_write_reg(gdev, misc_control_reg(gdev, GPU_IRQ_CLEAR), PRFCNT_SAMPLE_COMPLETED);
}
	...
	struct hvgr_ctx *ctx, struct hvgr_ct_setup *setup)
{
	...
	hvgr_info(gdev, HVGR_DATAN, "ctx_%u open config.", ctx->id);
	hvgr_mmu_flush_tlb(ctx);
	hvgr_datan_ct_open_config(gdev, ctx, &config);
	hvgr_datan_ct_wait_for_config_inner(ctx);
...
int hvgr_datan_ct_disable_inner(struct hvgr_ctx *ctx)
{
	...
	hvgr_info(gdev, HVGR_DATAN, "ctx_%u close config.", ctx->id);
...
int hvgr_datan_ct_wait_for_dump_inner(struct hvgr_ctx *ctx)
{
	...
	if (times == DUMP_WAIT_MAX_TIME)
		hvgr_debug(gdev, HVGR_DATAN, "ctx_%u dump wait timeout.", ctx->id);
exit:
	hvgr_write_reg(gdev, misc_control_reg(gdev, GPU_IRQ_CLEAR), PRFCNT_SAMPLE_COMPLETED);
