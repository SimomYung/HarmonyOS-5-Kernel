...
static bool kdatan_debug_bus_record_reg_data(struct bus_dump_resource_info *info, u64 addr,
	u64 offset)
{
	struct hvgr_device *gdev = info->gdev;
	...
	ret = snprintf_s(data, DUMP_DATA_MAX_SIZE, DUMP_DATA_MAX_SIZE - 1, RECORD_FORMAT, offset,
		val);
	if (ret < 0) {
		hvgr_err(gdev, HVGR_DATAN, "debug bus [%s] snprintf_s failed.", __func__);
		return false;
...
{
	struct hvgr_device *gdev = info->gdev;
	u64 addr = gpu_control_reg(gdev, 0);
	char data[DUMP_DATA_MAX_SIZE] = {0};
	u64 reg_offset;
	u64 base_addr = gpu_control_reg(gdev, 0);
	int ret;
	...
	for (; addr <= reg_offset; addr += (u32)sizeof(u32))
		kdatan_debug_bus_record_reg_data(info, addr, addr - base_addr);
}
...
static void datan_bus_dump_cq_control_register(struct bus_dump_resource_info *info)
{
	...
	int chn;
	int ret;
	u64 addr = cq_control_reg(gdev, CQ_ENABLE);
	u64 reg_offset = cq_control_reg(gdev, CQ_BC_SEND_MODE);
	u64 base_addr = cq_control_reg(gdev, 0);
	u64 offset;
	char data[DUMP_DATA_MAX_SIZE] = {0};
	...
	offset = cq_control_reg(gdev, CQ_IRQ_TOP) - base_addr + CQ_REG_BASE;
	kdatan_debug_bus_record_reg_data(info, cq_control_reg(gdev, CQ_IRQ_TOP), offset);

	for (; addr <= reg_offset; addr += (u32)sizeof(u32)) {
		offset = addr - base_addr + CQ_REG_BASE;
		kdatan_debug_bus_record_reg_data(info, addr, offset);
	}

	offset = cq_control_reg(gdev, CQ_SUBMIT) - base_addr + CQ_REG_BASE;
	kdatan_debug_bus_record_reg_data(info, cq_control_reg(gdev, CQ_SUBMIT), offset);
...
static void datan_bus_dump_misc_control_register(struct bus_dump_resource_info *info)
{
	...
	int ret;
	u32 i;
	u64 addr = misc_control_reg(gdev, GPU_IRQ_RAWSTAT);
	u64 reg_offset = misc_control_reg(gdev, GPU_IRQ_STATUS);
	u64 base_addr = misc_control_reg(gdev, 0);
	char data[DUMP_DATA_MAX_SIZE] = {0};
	u64 offset;
	...
	for (i = 0; i < FAULT_NUMBER; i++) {
		addr = misc_control_reg(gdev, GPU_FAULTSTATUS);
		reg_offset = misc_control_reg(gdev, GPU_FAULTDATA3);
		addr += i * 0x20;
		reg_offset += i * 0x20;
		for (; addr <= reg_offset; addr += (u32)sizeof(u32)) {
			offset = addr - base_addr;
	...
	addr = misc_control_reg(gdev, FCM_SLOT_PRESENT);
	reg_offset = misc_control_reg(gdev, FF_CONFIG_6);
	for (; addr <= reg_offset; addr += (u32)sizeof(u32)) {
		offset = addr - base_addr;
		kdatan_debug_bus_record_reg_data(info, addr, offset);
	}
...
static void datan_bus_dump_job_control_register(struct bus_dump_resource_info *info)
{
	...
	char data[DUMP_DATA_MAX_SIZE] = {0};
	int ret;
	u64 base_addr = job_control_reg(gdev, 0);
	u64 offset;
	...
	for (i = 0; i < array_size; i++) {
		...
		offset = addr - base_addr + JOB_CONTROL_BASE;
		kdatan_debug_bus_record_reg_data(info, addr, offset);
	}
...
static void datan_bus_dump_mmu_register_v2xx(struct bus_dump_resource_info *info)
{
	...
	struct hvgr_device *gdev = info->gdev;
	u64 end_addr = mmu_control_reg(gdev, MMU_CMD_DONE);
	u64 start_addr = mmu_control_reg(gdev, 0);
	u64 base_addr = mmu_control_reg(gdev, 0);
	u64 offset;

	for (; start_addr <= end_addr; start_addr += (u32)sizeof(u32)) {
		offset = start_addr - base_addr + MEMORY_MANAGEMENT_BASE;
		kdatan_debug_bus_record_reg_data(info, start_addr, offset);
	}
	...
	for (; addr_space_num < MAX_ADDR_SPACE_NUM; addr_space_num++) {
		...
		for (; start_addr <= end_addr; start_addr += FCP_REG_SIZE) {
			...
			offset = dump_addr - base_addr + MEMORY_MANAGEMENT_BASE;
			kdatan_debug_bus_record_reg_data(info, dump_addr, offset);
		}
	}
}
...
static void datan_bus_dump_fcp_cq_regs(struct bus_dump_resource_info *info)
{
	...
	u64 addr = fcp_mb_reg(gdev, FCP_MB_MCUVECTOR_ADDR_CQ);
	u64 base_addr = fcp_mb_reg(gdev, 0);
	u64 offset;

	for (; addr <= fcp_mb_reg(gdev, FCP_MB_MCU_ERROR_CQ); addr += FCP_REG_SIZE) {
		offset = addr - base_addr + FCP_BASE + FCP_MB_MCU_KEY;
		kdatan_debug_bus_record_reg_data(info, addr, offset);
	}
	...
	addr = fcp_mb_reg(gdev, FCP_MB_EDMA2MCU_CQ_MASKINT);
	offset = addr - base_addr + FCP_BASE + FCP_MB_MCU_KEY;
	kdatan_debug_bus_record_reg_data(info, addr, offset);
}
...
static void datan_bus_dump_fcp_cf_regs(struct bus_dump_resource_info *info)
{
	...
	u64 addr = fcp_mb_reg(gdev, FCP_MB_MCUVECTOR_ADDR);
	u64 base_addr = fcp_mb_reg(gdev, 0);
	u64 offset;
	...
	addr = fcp_mb_reg(gdev, FCP_MB_EDMA2MCU_CF_MASKINT);
	offset = addr - base_addr + FCP_BASE + FCP_MB_MCU_KEY;
	kdatan_debug_bus_record_reg_data(info, addr, offset);
}
...
{
	struct hvgr_device *gdev = info->gdev;
	u64 addr = fcp_mb_reg(gdev, FCP_MB_EDMA2JM_MASKINT);
	u64 base_addr = fcp_mb_reg(gdev, 0);
	u64 offset;
...
{
	struct hvgr_device *gdev = info->gdev;
	u64 addr = fcp_mb_reg(gdev, FCP_MB_HOST2FCP_MSG_VLD);
	u64 base_addr = fcp_mb_reg(gdev, 0);
	u64 offset;
...
static void datan_bus_dump_fcp_dma_regs(struct bus_dump_resource_info *info)
{
	...
	kdatan_debug_bus_record_reg_data(info, fcp_dma_reg(gdev, FCP_DMA_CH_INT_TCRAW),
		fcp_dma_reg(gdev, FCP_DMA_CH_INT_TCRAW) - base_addr + FCP_BASE + FCP_MB_MCU_KEY);
	...
	for (; dma_ch < MAX_DMA_CHANNEL_NUMBER; dma_ch++) {
		...
		for (; addr <= FCP_DMA_BUFDATA; addr += FCP_REG_SIZE) {
			...
			offset = dump_addr - base_addr + FCP_BASE + FCP_MB_MCU_KEY;
			kdatan_debug_bus_record_reg_data(info, dump_addr, offset);
		}
	}
}
	...
	for (timer_idx = 0; timer_idx < timer_number; timer_idx++) {
		...
		for (; addr <= end_addr; addr += FCP_REG_SIZE) {
			...
			kdatan_debug_bus_record_reg_data(info, dump_addr,
				dump_addr - fcp_mb_reg(gdev, 0) + FCP_BASE + FCP_MB_MCU_KEY);
		}
	}
}
...
static bool policy_cores_powered(struct hvgr_device * const gdev)
{
...
static int kdatan_bus_dump_data_to_buffer(struct bus_dump_resource_info *info)
{
	...
	if (!policy_cores_powered(gdev)) {
		hvgr_debug(gdev, HVGR_DATAN, "debugbus gpu is not ready.");
		return -1;
	}
...
void kdatan_debug_bus_start_dump(struct hvgr_device * const gdev)
{
	...
	if (hvgr_local_debugbus_check_disk()) {
		...
		if (datan_bus_dev->debugbus_cnt != 0) {
			hvgr_debug(gdev, HVGR_DATAN, "%s debugbus already Dump count %d",
		...
		buffer_cnt = HVGR_DEBUG_BUS_CNT_SEC;
		info.gdev = gdev;
		info.buffer_size = hvgr_debugbus_get_buffer_size(gdev);
		...
		if (ret != 0 || dup_ret != 0) {
			hvgr_debug(gdev, HVGR_DATAN, "%s debugbus dump fail", __func__);
		...
		hvgr_debug(gdev, HVGR_DATAN, "free space is not enough, skip");
	}
	gpu_hang_count++;
}
...
void kdatan_debug_bus_cbit_config(struct hvgr_device * const gdev)
{
	...
	/* XBI_CFG Cbit config */
	val = hvgr_read_reg(gdev, misc_control_reg(gdev, CCE_CBI_CBIT));
	val |= HVGR_XBI_CONFIG_OFFSET;
	hvgr_write_reg(gdev, misc_control_reg(gdev, CCE_CBI_CBIT), val);

	/* BTC Cbit config */
	val = hvgr_read_reg(gdev, misc_control_reg(gdev, BTC_CONFIG_4));
	val |= HVGR_BTC_CONFIG_OFFSET;
	hvgr_write_reg(gdev, misc_control_reg(gdev, BTC_CONFIG_4), val);

	/* GPC Cbit config */
	val = hvgr_read_reg(gdev, misc_control_reg(gdev, GPC_CONFIG_2));
	val |= HVGR_GPC_2_CONFIG_OFFSET;
	hvgr_write_reg(gdev, misc_control_reg(gdev, GPC_CONFIG_2), val);

	val = hvgr_read_reg(gdev, misc_control_reg(gdev, GPC_CONFIG_3));
	val |= HVGR_GPC_3_CONFIG_OFFSET;
	hvgr_write_reg(gdev, misc_control_reg(gdev, GPC_CONFIG_3), val);

	val = hvgr_read_reg(gdev, misc_control_reg(gdev, GPC_CONFIG_4));
	val |= HVGR_GPC_4_CONFIG_OFFSET;
	hvgr_write_reg(gdev, misc_control_reg(gdev, GPC_CONFIG_4), val);
}
