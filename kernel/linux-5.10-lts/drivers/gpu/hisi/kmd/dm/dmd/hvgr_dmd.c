...
#endif
		check_idx = gdev->dm_dev.dmd_msg->dmd_fault_cnt % MAX_DMD_CHECK_CNT;
		gdev->dm_dev.dmd_msg->dmd_check_ts[check_idx] = (u64)cur_ts.tv_sec;
		++gdev->dm_dev.dmd_msg->dmd_fault_cnt;
		hvgr_debug(gdev, HVGR_DM, "gpu fault happened %u times!",
			gdev->dm_dev.dmd_msg->dmd_fault_cnt);
	}
#endif
	hvgr_debug(gdev, HVGR_DM, "higpu_dmd set dmd %u", dmd_id);

	ret = strncpy_s(gdev->dm_dev.dmd_msg->dmd_log,
