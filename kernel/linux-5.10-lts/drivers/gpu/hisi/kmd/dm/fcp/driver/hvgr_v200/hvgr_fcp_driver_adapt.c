...
{
	u64 run_reg = fcp_mb_reg(gdev, FCP_MB_MCU_CPUWAIT);
	u32 run_val = 0u;
...
{
	u64 mcu_status_reg = fcp_mb_reg(gdev, FCP_MB_MCU_CPUWAIT);
	u32 val;
...
int hvgr_fcp_dma_proc(struct hvgr_device *gdev)
{
	...
	if (hvgr_pm_is_gpu_reseting(gdev)) {
		itcm_size = FCP_ITCM_SIZE;
		dtcm_size = FCP_DTCM_SIZE;
	} else {
		itcm_size = header->size_sub0;
		dtcm_size = header->size_sub1;
	}
	dtcm_offset =  FCP_ITCM_SIZE;
#else
	itcm_size = header->size_sub0;
	dtcm_size = header->size_sub1;
	dtcm_offset = header->size_sub0;
#endif

	/* 1. update itcm */
	dma_cfg.src_addr = fcp_data->gva + sizeof(*header);
	dma_cfg.dst_addr = FCP_UPGRADE_ITCM_ADDR;
	dma_cfg.size = itcm_size;
	dma_cfg.jasid = jasid;
	dma_cfg.channel = itcm_ch;
	hvgr_fcp_configure_dma(gdev, &dma_cfg);

	/* 2. update dtcm */
	dma_cfg.src_addr += dtcm_offset;
	dma_cfg.dst_addr = FCP_DTCM_START_ADDR;
	dma_cfg.size = dtcm_size;
