...
{
	u64 reg;
	u64 int_reg = fcp_dma_reg(gdev, FCP_DMA_CH_INT_TCRAW);
...
int hvgr_get_firmware_name(struct hvgr_device *gdev, char *firmware_name, uint32_t name_len)
{
	char *name = NULL;
	static const struct gpu_product_id_name_map {
		u32 id;
		char *name;
		...
		{ .id = GPU_ID2_PRODUCT_TV211,
		  .name = FCP_RISCV_TV211_FIRMWARE_NAME },
		{ .id = GPU_ID2_PRODUCT_TV350,
		  .name = FCP_RISCV_TV350_FIRMWARE_NAME },
		{ .id = GPU_ID2_PRODUCT_TV500,
		  .name = FCP_RISCV_TV500_FIRMWARE_NAME },
	};
	size_t i;
	...
	for (i = 0; i < ARRAY_SIZE(gpu_product_id_names); ++i) {
		const struct gpu_product_id_name_map *p = &gpu_product_id_names[i];
		if ((p->id & GPU_ID1_PRODUCT_MODEL) == (gpu_id & GPU_ID1_PRODUCT_MODEL)) {
			name = p->name;
			break;
		}
	}
...
static int hvgr_fcp_requst_firmware(struct hvgr_device *gdev)
{
	...
	struct hvg_fcp_header *head = NULL;
	uint64_t src_addr;
	uint64_t dst_addr;
#endif
	...
	retval = hvgr_fcp_firmware_validate(gdev,
		(const struct hvg_fcp_header *)fw_entry->data, fw_size);
	if (retval != 0) {
...
#if hvgr_version_lt(HVGR_V300)
	head = (struct hvg_fcp_header *)fw_entry->data;
	/* copy head and itcm */
	if (memcpy_s(fcp_data->kva, FCP_MEM_SIZE, fw_entry->data, sizeof(*head) + head->size_sub0) != 0) {
		hvgr_err(gdev, HVGR_DM, "fcp firmware copy fail");
		release_firmware(fw_entry);
		return -EFAULT;
	}

	dst_addr = ptr_to_u64(fcp_data->kva) + FCP_ITCM_SIZE + sizeof(*head);
	src_addr = ptr_to_u64(fw_entry->data) + sizeof(*head) + head->size_sub0;
	if (memcpy_s((void *)dst_addr, FCP_MEM_SIZE - (sizeof(*head) + head->size_sub0),
		(void *)src_addr, head->size_sub1) != 0) {
		memset_s(fcp_data->kva, FCP_ITCM_SIZE + sizeof(*head), 0, FCP_ITCM_SIZE + sizeof(*head));
		hvgr_err(gdev, HVGR_DM, "fcp firmware copy fail");
		release_firmware(fw_entry);
		return -EFAULT;
	}
...
static int hvgr_fcp_malloc_firmware_buf(struct hvgr_device *gdev)
{
	...
	(void)hvgr_hts_alloc_events_memory(gdev, ctx);
...
int hvgr_fcp_loader_task(void *data)
{
	 ...
	 */
#if hvgr_version_ge(HVGR_V350)
	if (gdev->pm_dev.pm_pwr.fast_pwr_data.fast_pwr_en_tmp) {
		/* if fast pwr enable, only need to store info, hw will loal it */
		hvgr_pm_fast_pwr_set_mcu_info(gdev);
		return retval;
	}
#endif
...
int hvgr_fcp_irq_init(struct hvgr_device * const gdev)
{
	if (hvgr_register_irq(gdev, FCP_IRQ, hvgr_fcp_irq_handler) != 0) {
		hvgr_err(gdev, HVGR_FCP, "Register fcp irq handler failed.");
		return -1;
	}
	hvgr_info(gdev, HVGR_FCP, "Register cq irq handler success.");
	return 0;
}

int hvgr_fcp_init(struct hvgr_device * const gdev)
{
	return hvgr_fcp_irq_init(gdev);
}
#else
int hvgr_fcp_init(struct hvgr_device * const gdev)
{
