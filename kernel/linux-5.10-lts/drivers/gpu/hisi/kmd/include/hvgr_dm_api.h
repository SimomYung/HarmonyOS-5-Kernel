...
#define gpu_control_reg(gdev, offset) \
	((uint64_t)((gdev)->dm_dev.dev_reg.reg_base[0]) + (uint64_t)(offset))

#define mmu_control_reg(gdev, offset) \
	((uint64_t)((gdev)->dm_dev.dev_reg.reg_base[1]) + (uint64_t)(offset))

#define misc_control_reg(gdev, offset) \
	((uint64_t)((gdev)->dm_dev.dev_reg.reg_base[2]) - HVGR_REG_BASE2_LEN + (uint64_t)(offset))

#define job_control_reg(gdev, offset) \
	((uint64_t)((gdev)->dm_dev.dev_reg.reg_base[2]) + JOB_CONTROL_BASE - HVGR_REG_BASE2_LEN + \
	(uint64_t)(offset))

#define cq_control_reg(gdev, offset) \
	((uint64_t)((gdev)->dm_dev.dev_reg.reg_base[2]) + CQ_REG_BASE - HVGR_REG_BASE2_LEN + \
	(uint64_t)(offset))

#if defined(MALEOON_2D_MCU_FEATURE) && MALEOON_2D_MCU_FEATURE != 0
#define fcp_2d_mb_reg(gdev, offset) \
