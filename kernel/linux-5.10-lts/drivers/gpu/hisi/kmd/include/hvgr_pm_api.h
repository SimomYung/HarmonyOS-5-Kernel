...
/*
 * hvgr_pm_retain_power - retain gpu power on
 * hvgr_pm_retain_power and hvgr_pm_release_gpu must appear in pairs.
 * pm_user_ref_count_add when gpu is poweron
 * Direct return when gpu is poweroff
 *
 * @param gdev - The gdev object pointer.
 *
 * @date - 2024-02-21
...
/*
 * hvgr_pm_release_power - rlease retain gpu power on
 * hvgr_pm_release_power and hvgr_pm_retain_power must appear in pairs.
 *
 * @param gdev - The gdev object pointer.
 *
 * @date - 2024-02-21
...
/*
 * hvgr_pm_try_power_off_gpu - Try power off the gpu..
 * The user_ref will not changed is this api, just try to power off the gpu, if user_ref is zero
 * and the power policy is job_demand, the gpu will be powered off, otherwise, the gpu will not be
 * powered off.
 *
 * @param gdev - The gdev object pointer.
 *
 ...
 * @date - 2021-02-03
 */
bool hvgr_pm_is_suspending(struct hvgr_device *gdev);
...
/*
 * hvgr_pm_is_gpu_reseting - Get gpu reset status, if gpu is reseting, return true,
 *                           otherwise, return false.
 * When calling this api, the power_lock must be held on.
 * @param gdev - The gdev object pointer.
 *
 * Return: true if gpu is reseting.
 *
 * @date - 2024-07-16
 */
bool hvgr_pm_is_gpu_reseting(struct hvgr_device *gdev);

#if hvgr_version_ge(HVGR_V500)
/*
 * hvgr_rtu_debugfs_init - rtu init
 *
 * @param gdev - The gdev object pointer.
 *
 * @date - 2024-8-9
 */
void hvgr_rtu_debugfs_init(struct hvgr_device * const gdev);

/*
 * hvgr_pm_rtu_power_on - set rtu power on
 *
 * @param gdev - The gdev object pointer.
 * @param cq_ctx - Queue pointer
 *
 * @date - 2024-8-9
 */
void hvgr_pm_rtu_power_on(struct hvgr_device *gdev,
	struct hvgr_cq_ctx * const cq_ctx);

/*
 * hvgr_pm_rtu_power_off - dec rtu ref cnt or set rtu power off
 *
 * @param gdev - The gdev object pointer.
 * @param cq_ctx - Queue pointer
 * @param cnt_flag - opts flag
 *
 * @date - 2024-8-9
 */
void hvgr_pm_rtu_power_off(struct hvgr_device *gdev,
	struct hvgr_cq_ctx * const cq_ctx, int cnt_flag);

/*
 * hvgr_pm_driver_update_rtu_status - set rtu power register
 *
 * @param gdev - The gdev object pointer.
 * @param pwrcmd - rtu power ops
 * @param pfunc - check rtu power status
 *
 * @date - 2024-8-9
 */
bool hvgr_pm_driver_update_rtu_status(struct hvgr_device *gdev,
	uint32_t pwrcmd, hvgr_pm_is_core_trans_end pfunc);
#endif

#if hvgr_version_eq(HVGR_V350)
void hvgr_pm_hwcnt_en(struct hvgr_device *gdev, bool enable);
#endif
#endif