	...
	u64 key, u64 num)
{
	...
	hvgr_mem_area_ref_get(ref_area);
	...
	struct page **page_array, uint32_t pages, struct hvgr_mem_area *area)
{
	...
	if (ret != 0) {
		hvgr_mem_flush_pt(ctx, va, (uint64_t)pages, WITHOUT_FLUSH_L2, level);
	...
	u64 *pfn_off, bool is_sparse_area)
{
	...
	if (!is_sparse_area && area->property != HVGR_MEM_PROPERTY_SAME_VA &&
		...
		hvgr_debug(ctx->gdev, HVGR_MEM, "ctx_%u sparsing property is %lu",
...
void hvgr_mem_sparse_term(struct hvgr_ctx *ctx)
{
	...
	if (!hvgr_kv_is_empty(&sparse_cb->sparse_rec)) {
		hvgr_debug(ctx->gdev, HVGR_MEM, "ctx_%u sparse term but rec map not empty", ctx->id);
		hvgr_mem_sparse_del_all_spnode(&sparse_cb->sparse_rec);
	}
	kfree(sparse_cb);
}

static bool hvgr_mem_sparse_check_para(struct hvgr_mem_area * const area)
{
...
static long hvgr_mem_sparse_alloc(struct hvgr_mem_area * const area)
{
	u64 i;
	...
	if (unlikely(area->page_array == NULL)) {
		hvgr_err(ctx->gdev, HVGR_MEM, "ctx_%u sparse mem alloc page array fail", ctx->id);
		return -ENOMEM;
	}
	...
	if (unlikely(area->gva == 0)) {
		...
		hvgr_err(ctx->gdev, HVGR_MEM, "ctx_%u sparse mem alloc va fail", ctx->id);
		return -ENOMEM;
	}
	/*
...
static long hvgr_mem_sparse_free(struct hvgr_mem_area * const area)
{
	struct hvgr_ctx *ctx = container_of(area->zone->pmem, struct hvgr_ctx, mem_ctx);
	...
	bool kmd_sparse_bind)
{
	...
	hvgr_mem_unmap_by_gva(ctx, sparse_addr, (uint32_t)cnt, sparse_area);
#endif
	...
	sparse_area->kmd_sparse_bind = kmd_sparse_bind;
	mutex_unlock(&ctx->mem_ctx.area_mutex);
	return 0;
...
int hvgr_mem_sparse_unbind_nolock(struct hvgr_ctx *ctx, u64 sparse_addr, u64 num)
{
	...
	sparse_area->kmd_sparse_bind = false;
	...
	for (i = 0; i < num; i++)
		sparse_area->page_array[i + sparse_off] =
			ctx->gdev->mem_dev.sparse_zero_page;
#ifdef CONFIG_HVGR_MMU_HM
	hvgr_mem_unmap_by_gva(ctx, sparse_addr, (uint32_t)num, sparse_area);
#endif
