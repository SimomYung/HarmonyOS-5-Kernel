...
static void hvgr_mmu_clear_fault(struct hvgr_device * const gdev, struct hvgr_mmu_as *mmu_as)
{
	...
	/* keep read write in the same lock */
	spin_lock_irqsave(&gdev->mem_dev.mmu_hw_lock, flags);
	(void)liblinux_ext_hvgr_mmu_clear_fault(mmu_as->no, mmu_as->fault_type);
...
static void hvgr_mmu_unmask_fault(struct hvgr_device * const gdev, struct hvgr_mmu_as *mmu_as)
{
	...
	/*
	 * keep read and write in the same lock,
	 * between liblinux_ext_hvgr_mmu_irq_mask and in hvgr_mmu_int_process.
	 */
	spin_lock_irqsave(&gdev->mem_dev.mmu_hw_lock, flags);
	(void)liblinux_ext_hvgr_mmu_unmask_fault(mmu_as->no, mmu_as->fault_type);
	spin_unlock_irqrestore(&gdev->mem_dev.mmu_hw_lock, flags);
}
#else
static void hvgr_mmu_clear_fault(struct hvgr_device * const gdev, struct hvgr_mmu_as *mmu_as)
...
#define PAGE_FAULT_INFO_LEN  512
	...
	struct hvgr_mmu_as *mmu_as, char *reason)
{
	...
	char fault_info[PAGE_FAULT_INFO_LEN] = {0};
	int ret;
	...
	ret = snprintf_s(fault_info, PAGE_FAULT_INFO_LEN, PAGE_FAULT_INFO_LEN - 1,
	...
	if (ret > 0)
		hvgr_err(gdev, HVGR_MEM, "%s", fault_info);
}
...
static void hvgr_mmu_reset_gpu(struct hvgr_ctx *ctx, struct hvgr_mmu_as *mmu_as, char *reason)
{
	...
	if (mmu_as->fault_type == MMU_BUS_FAULT) {
		...
		ret = snprintf_s(dmd_msg, MAX_DMD_LOG_LENTH, MAX_DMD_LOG_LENTH - 1,
			"%s,%s,as%u,exception_type:0x%x,source_id:0x%x,stream_id:0x%x,task:%s,%d",
			"higpu page fault", hvgr_mmu_get_access_type(mmu_as->fault_stat),
			mmu_as->no, exception_type, source_id, stream_id,
...
#else
	spin_lock_irqsave(&ctx->gdev->mem_dev.mmu_hw_lock, flags);
	ctx->gdev->mem_dev.fault_as_num--;
	spin_unlock_irqrestore(&ctx->gdev->mem_dev.mmu_hw_lock, flags);
#endif
...
#else
	hvgr_mmu_fault_clear_all_as(NULL, gdev);
#endif
}
...
static void hvgr_mmu_clear_recover_as(struct hvgr_ctx *ctx, struct hvgr_mmu_as *mmu_as,
	uint32_t pages)
{
#if hvgr_version_lt(HVGR_V300)
	uint32_t as_bits;

	as_bits = (uint32_t)mmu_page_fault_bit(mmu_as->no);
	if (mmu_as->fault_type == MMU_BUS_FAULT)
		as_bits |= mmu_bus_fault_bit(mmu_as->no);

	hvgr_mmu_fault_clear_flush_unmask(ctx, mmu_as->fault_addr, pages, as_bits);
#else
	hvgr_mmu_fault_clear_all_as(ctx, ctx->gdev);
#endif
}
	...
	struct hvgr_mem_area *fault_area)
{
	...
	hvgr_mmu_clear_recover_as(ctx, mmu_as, 0u);
}

static long hvgr_mmu_fault_add_pages(struct hvgr_ctx *ctx, struct hvgr_mmu_as *mmu_as,
	struct hvgr_mem_area *fault_area, uint64_t add_pages, uint32_t *level)
{
	long ret;
	uint64_t add_gva;
	...
	ret = hvgr_mmu_set_up(ctx, &mmu_para, level, fault_area);
	if (ret != 0) {
	...
	if (fault_area->property != HVGR_MEM_PROPERTY_JIT) {
		atomic_add((int)add_pages, &ctx->mem_ctx.used_grow_pages);
...
void hvgr_mmu_trans_fault_process(struct hvgr_ctx *ctx, struct hvgr_mmu_as *mmu_as)
{
	...
	if (hvgr_mmu_fault_add_pages(ctx, mmu_as, fault_area, add_pages, &level) == 0) {
		mutex_unlock(&ctx->mem_ctx.area_mutex);
		hvgr_mmu_clear_recover_as(ctx, mmu_as, (uint32_t)add_pages);
		return;
	}
...
#else
void hvgr_mmu_pf_worker(struct kthread_work *work)
#endif
{
	...
	mmu_as->ctx = hvgr_cq_lookup_ctx(gdev, (int)mmu_as->no);
	if (mmu_as->ctx == NULL) {
	...
	if (unlikely(mmu_as->protected_mode)) {
		hvgr_mmu_reset_gpu(mmu_as->ctx, mmu_as, "Permission failure");
		hvgr_cq_sched_release_ctx(gdev, mmu_as->ctx);
		goto out;
	}

	if (hvgr_mmu_is_trans_fault(mmu_as->fault_stat)) {
		hvgr_mmu_trans_fault_process(mmu_as->ctx, mmu_as);
	} else {
		reset_reason = hvgr_mmu_other_fault_process(mmu_as->ctx, mmu_as);
		if (reset_reason != NULL)
			hvgr_mmu_reset_gpu(mmu_as->ctx, mmu_as, reset_reason);
	}
...
#else
void hvgr_mmu_bf_worker(struct kthread_work *work)
#endif
{
	struct hvgr_mmu_as *mmu_as = NULL;
	struct hvgr_mem_dev *mem_dev = NULL;
	...
	if (hvgr_pm_get_gpu_status(gdev) != HVGR_PM_POWER_OFF) {
		(void)hvgr_mmu_hal_disable(mmu_as->ctx);
		hvgr_mmu_clear_fault(mmu_as->ctx->gdev, mmu_as);
	}

	hvgr_mmu_reset_gpu(mmu_as->ctx, mmu_as, "MMU bus fault");
	hvgr_cq_sched_release_ctx(gdev, mmu_as->ctx);
out:
	atomic_dec(&gdev->mem_dev.pending_faults);
}
...
#else
		(void)kthread_queue_work(mmu_as->fault_wq, &mmu_as->pf_work);
#endif
	} else {
		atomic_inc(&gdev->mem_dev.pending_faults);
#if (LINUX_VERSION_CODE < KERNEL_VERSION(5, 10, 0))
...
#else
		(void)kthread_queue_work(mmu_as->fault_wq, &mmu_as->bf_work);
#endif
	}
}
...
#else
	(void)liblinux_ext_hvgr_mmu_irq_mask(0);
#endif
#if hvgr_version_lt(HVGR_V300)
	spin_unlock_irqrestore(&gdev->mem_dev.mmu_hw_lock, flags);
#endif
...
#if hvgr_version_ge(HVGR_V300)
	if (unlikely(gdev->mem_dev.fault_as_num != 0u))
		hvgr_err(gdev, HVGR_MEM, "mmu fault last fault as num is %u",
			gdev->mem_dev.fault_as_num);

	gdev->mem_dev.fault_as_num = 0u;
#endif
...
#else
	(void)liblinux_ext_hvgr_mmu_irq_mask(val);
#endif
#endif
	spin_unlock_irqrestore(&gdev->mem_dev.mmu_hw_lock, flags);
}
...
#else
		mmu_as->fault_wq = kthread_create_worker(0, wq_name);
		if (mmu_as->fault_wq == NULL) {
			hvgr_err(gdev, HVGR_MEM, "mmu fault create k worker for as%u fail", i);
			return -EINVAL;
		}

		sched_set_fifo(mmu_as->fault_wq->task);
		kthread_init_work(&mmu_as->pf_work, hvgr_mmu_pf_worker);
		kthread_init_work(&mmu_as->bf_work, hvgr_mmu_bf_worker);
#endif
	}
...
#else
		kthread_flush_work(&mmu_as->pf_work);
		kthread_flush_work(&mmu_as->bf_work);
#endif
	}

#if hvgr_version_ge(HVGR_V300)
	if (unlikely(gdev->mem_dev.fault_as_num != 0u)) {
		hvgr_debug(gdev, HVGR_MEM, "mmu flush fault as num is %u",
			gdev->mem_dev.fault_as_num);
		gdev->mem_dev.fault_as_num = 0u;
	}
#endif
