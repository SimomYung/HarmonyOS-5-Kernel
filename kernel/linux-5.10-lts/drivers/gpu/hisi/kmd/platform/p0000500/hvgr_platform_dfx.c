...
static bool hvgr_dmd_need_check_nv(struct hvgr_device *gdev)
{
	...
	u32 last_idx = (gdev->dm_dev.dmd_msg->dmd_fault_cnt - 1) % MAX_DMD_CHECK_CNT;
...
static int hvgr_dmd_sh_process(void *data)
{
	...
	if (nv_data.info.bit.freq_mark == 1) {
		/* unmark, write freq_mark to 0 */
		nv_data.info.bit.freq_mark = 0;
...
void hvgr_dmd_msg_check(struct hvgr_device *gdev)
{
	...
	if (gdev->dm_dev.dmd_msg->dmd_sh_flag != 0 || gdev->dm_dev.dmd_msg->dmd_fault_cnt == 0 ||
