...
static void hvgr_pm_driver_platform_on(struct hvgr_device *gdev)
{
	...
	if (unlikely(ret)) {
		...
		gdev->time_record.buck_on =  (uint64_t)ktime_to_ns(ktime_get());
	}
...
static void hvgr_pm_driver_platform_off(struct hvgr_device *gdev)
{
	...
	if (unlikely(ret)) {
		...
		gdev->time_record.buck_off =  (uint64_t)ktime_to_ns(ktime_get());
	}
}
...
void hvgr_pm_driver_request_cycle_counter(struct hvgr_device *gdev)
{
	...
	hvgr_write_reg(gdev, misc_control_reg(gdev, GPU_COMMAND),
	...
	hvgr_pm_is_core_trans_end pfunc)
{
	...
	if (!gdev->pm_dev.pm_dfx.rtu_enable) {
		if (pwrcmd == GPC_PWRON_CMD)
			(void)hvgr_pm_driver_update_rtu_status(gdev, RTU_PWRON_CMD, hvgr_pm_is_core_ready);
		else
			(void)hvgr_pm_driver_update_rtu_status(gdev, RTU_PWROFF_CMD, hvgr_pm_is_core_off);
	}
#endif
...
static void hvgr_pm_wait_cycle_cnt_oper_done(struct hvgr_device *gdev)
{
	...
	do {
		gpu_irq = hvgr_read_reg(gdev, misc_control_reg(gdev, GPU_IRQ_RAWSTAT));
		i++;
	...
	hvgr_write_reg(gdev, misc_control_reg(gdev, GPU_IRQ_CLEAR), CYCLECOUNT_DONE);
...
{
	hvgr_write_reg(gdev, misc_control_reg(gdev, GPU_COMMAND), GPU_COMMAND_CYCLE_COUNT_START);
	hvgr_pm_wait_cycle_cnt_oper_done(gdev);
}

static void hvgr_pm_stop_cycle_count(struct hvgr_device *gdev)
{
	hvgr_write_reg(gdev, misc_control_reg(gdev, GPU_COMMAND), GPU_COMMAND_CYCLE_COUNT_STOP);
	hvgr_pm_wait_cycle_cnt_oper_done(gdev);
}
...
void hvgr_dump_registers(struct hvgr_device *gdev)
{
	...
	hvgr_debug(gdev, HVGR_PM, "  GPU_IRQ_RAWSTAT=0x%08x GPU_STATUS=0x%08x",
		hvgr_read_reg(gdev, misc_control_reg(gdev, GPU_IRQ_RAWSTAT)),
		hvgr_read_reg(gdev, gpu_control_reg(gdev, GPU_STATUS)));

	hvgr_debug(gdev, HVGR_PM, "  FCM_SLOT_IRQ_STATUS=0x%08x",
		hvgr_read_reg(gdev, job_control_reg(gdev, FCM_SLOT_IRQ_STATUS)));
	...
	hvgr_debug(gdev, HVGR_PM, "  MMU_IRQ_RAWSTAT=0x%08x GPU_FAULTSTATUS=0x%08x",
		...
		hvgr_read_reg(gdev, misc_control_reg(gdev, GPU_FAULTSTATUS)));

	hvgr_debug(gdev, HVGR_PM,
		"  GPU_IRQ_MASK=0x%08x    MMU_IRQ_MASK=0x%08x",
		hvgr_read_reg(gdev, misc_control_reg(gdev, GPU_IRQ_MASK)),
	...
	hvgr_debug(gdev, HVGR_PM, "  SHADER_CONFIG=0x%08x   L2_MMU_CONFIG=0x%08x",
		hvgr_read_reg(gdev, misc_control_reg(gdev, GPC_CONFIG_0)),
		hvgr_read_reg(
			gdev, misc_control_reg(gdev, L2_MMU_CONFIG_0)));
	hvgr_debug(gdev, HVGR_PM, "  TILER_CONFIG=0x%08x    JM_CONFIG=0x%08x",
		hvgr_read_reg(gdev, misc_control_reg(gdev, BTC_CONFIG_0)),
		hvgr_read_reg(gdev, misc_control_reg(gdev, JM_CONFIG)));

	hvgr_debug(gdev, HVGR_PM, "  GPC pwr status = 0x%08x   RJD_OS_CONFIG = 0x%08x",
		hvgr_read_reg(gdev, gpu_control_reg(gdev, GPC_PWR_STATUS)),
		hvgr_read_reg(gdev, job_control_reg(gdev, JOB_RJD_OS_CONFIG)));
...
static void hvgr_pm_wait_clean_flush_end(struct hvgr_device *gdev)
{
	...
	do {
		val = hvgr_read_reg(gdev, misc_control_reg(gdev, GPU_IRQ_RAWSTAT));
...
{
	hvgr_write_reg(gdev, misc_control_reg(gdev, GPU_COMMAND), GPU_COMMAND_CLEAN_CACHES);
...
void hvgr_pm_driver_reset_gpc_and_bl(struct hvgr_device *gdev)
{
	...
	if (!hvgr_pm_wait_gpc_poweroff_ready(gdev, true)) {
...
void hvgr_pm_driver_cbit_config_by_sw(struct hvgr_device *gdev)
{
	...
	btc_cfg_4 = hvgr_read_reg(gdev, misc_control_reg(gdev, BTC_CONFIG_4));
	btc_cfg_4 = btc_cfg_4 | BTC_CFG4_TOP_MUX_SEL_2V;
	hvgr_write_reg(gdev, misc_control_reg(gdev, BTC_CONFIG_4), btc_cfg_4);

	gpc_cfg_6 = hvgr_read_reg(gdev, misc_control_reg(gdev, GPC_CONFIG_6));
	gpc_cfg_6 |= GPC_CFG6_LSC_NONGPC_STORE_MASK_DISABLE;

	gpc_cfg_6 |= GPC_CFG6_LSC_PUSH_VTC_GTC_PROP;
	hvgr_write_reg(gdev, misc_control_reg(gdev, GPC_CONFIG_6), gpc_cfg_6);

	if (!hvgr_hw_has_issue(gdev, HVGR_HW_ISSUE_50300)) {
		if (hvgr_is_fpga(gdev)) {
			gpc_cfg_4 = hvgr_read_reg(gdev, misc_control_reg(gdev, GPC_CONFIG_4));
			gpc_cfg_4 |= (0x2AU << 8); /* mask GPC0~2's SP1 */
			hvgr_write_reg(gdev, misc_control_reg(gdev, GPC_CONFIG_4), gpc_cfg_4);
		}
	}

	if (hvgr_hw_has_issue(gdev, HVGR_HW_ISSUE_00031)) {
		gpc_cfg_0 = hvgr_read_reg(gdev, misc_control_reg(gdev, GPC_CONFIG_0));
		gpc_cfg_0 |= SHADER_CONFIG_0_ICG;
		hvgr_write_reg(gdev, misc_control_reg(gdev, GPC_CONFIG_0), gpc_cfg_0);

		gpc_cfg_2 = hvgr_read_reg(gdev, misc_control_reg(gdev, GPC_CONFIG_2));
		gpc_cfg_2 |= SHADER_CONFIG_2_ICG;
		hvgr_write_reg(gdev, misc_control_reg(gdev, GPC_CONFIG_2), gpc_cfg_2);

		gpc_cfg_5 = hvgr_read_reg(gdev, misc_control_reg(gdev, GPC_CONFIG_5));
		gpc_cfg_5 |= SHADER_CONFIG_5_ICG;
		hvgr_write_reg(gdev, misc_control_reg(gdev, GPC_CONFIG_5), gpc_cfg_5);
	}
...
int hvgr_pm_driver_clock_on_by_sw(struct hvgr_device *gdev)
{
	...
	hvgr_hts_load_event_table(gdev);
	hvgr_hts_event_unlock();
#if defined(HVGR_STATE_BUNDLE2) && HVGR_STATE_BUNDLE2 > 0
	hvgr_set_common_memctx_addr(gdev);
#endif
...
int hvgr_pm_driver_clock_off_by_sw(struct hvgr_device *gdev)
{
	...
	hvgr_hts_event_lock(gdev);
	hvgr_hts_store_event_table(gdev);
	if (hvgr_hw_has_issue(gdev, HVGR_HW_ISSUE_50210)) {
		hvgr_pm_clean_flush(gdev);
	} else {
		cores = pm_pwr->present_cores[HVGR_PM_CORE_BL];
