...
bool hvgr_pm_wait_gpc_poweroff_ready(struct hvgr_device *gdev, bool force)
{
	unused(force);
...
void hvgr_pm_driver_cbit_config(struct hvgr_device *gdev)
{
	...
	uint32_t cfg;
	cfg = hvgr_read_reg(gdev, misc_control_reg(gdev, TU_CONFIG_0));
	cfg = cfg | TU_CFG0_TCC_CLOCK;
	hvgr_write_reg(gdev, misc_control_reg(gdev, TU_CONFIG_0), cfg);
#endif
