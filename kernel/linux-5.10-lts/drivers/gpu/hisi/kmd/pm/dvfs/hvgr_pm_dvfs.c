...
static void hvgr_pm_calc_gpu_utilisation(struct hvgr_device *gdev)
{
	...
	hvgr_info(gdev, HVGR_DVFS, "busy time = %u, cl time = %u, idle time = %u.",
	...
	hvgr_info(gdev, HVGR_DVFS, "cl boost = %d,  utilisation = %d.",
...
int hvgr_pm_gpufreq_init(struct hvgr_device *gdev)
{
	...
	if (ret != 0) {
		hvgr_err(gdev, HVGR_DVFS, "Get_busytime callback register failed.");
		return ret;
	}

	ret = gpufreq_register_callback(GF_CB_VSYNC_HIT, hvgr_pm_vsync_hit);
	if (ret != 0) {
		hvgr_err(gdev, HVGR_DVFS, "Get_vsync_hit callback register failed.");
		return ret;
	}

	ret = gpufreq_register_callback(GF_CB_CL_BOOST, hvgr_pm_get_cl_boost);
	if (ret != 0) {
		hvgr_err(gdev, HVGR_DVFS, "Get_cl_boost callback register failed.");
		return ret;
	}
...
#endif
	hvgr_info(gdev, HVGR_DVFS, "Thro freq = %lu", freq);
...
void hvgr_pm_set_force_freq(struct hvgr_device *gdev, unsigned long freq, bool low)
{
	...
	hvgr_info(gdev, HVGR_DVFS, "freq = %lu, thro_freq = %lu.",
...
unsigned long hvgr_pm_trans_thro_hint_to_freq(struct hvgr_device *gdev, uint32_t thro_hint)
{
	...
	hvgr_info(gdev, HVGR_DVFS, "thro_hint = %u, thro_freq = %lu.", thro_hint, thro_freq);
...
int hvgr_pm_metrics_init(struct hvgr_device *gdev)
{
	...
	if (sysfs_create_file(&gdev->dev->kobj, &dev_attr_gpu_utilisation.attr))
		hvgr_err(gdev, HVGR_DVFS, "Create gpu_utilisation failed.");
