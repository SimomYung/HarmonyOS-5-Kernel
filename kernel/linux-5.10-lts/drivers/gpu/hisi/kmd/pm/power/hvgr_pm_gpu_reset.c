...
static void hvgr_pm_reset_submmit_proc(struct hvgr_device *gdev)
{
	...
	if (atomic_read(&pm_reset->reset_type) == GPU_RESET_TYPE_EXIT_PROTECT_MODE) {
		...
		hvgr_hts_store_event_through_register(gdev);
		/* Soft reset gpu */
	...
	gdev->time_record.crg_reset = (uint64_t)ktime_to_ns(ktime_get());
	...
	if (atomic_read(&pm_reset->reset_type) == GPU_RESET_TYPE_EXIT_PROTECT_MODE)
		(void)hvgr_fcp_load_protect_exit(gdev);
#endif
...
int hvgr_pm_gpu_reset_soft_init(struct hvgr_device *gdev)
{
	...
	if (!hvgr_is_fpga(gdev))
		...
		pm_reset->reset_delay_time = HVGR_PM_GPU_RESET_DELAY_TIME_MS_FPGA;
	return 0;
}
...
bool hvgr_pm_is_gpu_reseting(struct hvgr_device *gdev)
{
	...
	return atomic_read(&pm_reset->reset_status) != GPU_RESET_END;
}
