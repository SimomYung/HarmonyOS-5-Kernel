...
bool hvgr_pm_power_on_sync(struct hvgr_device *gdev, bool gpu_reset)
{
	...
	if (hvgr_pm_auto_gpc_is_enable(gdev)) {
		if (pm_pwr->cur_status == HVGR_PM_POWER_ON) {
			pm_pwr->cur_status = HVGR_PM_GPU_READY;
			hvgr_info(gdev, HVGR_PM, "auto gpc cur status power on, set to ready");
			goto out;
		}
	}
#endif
...
void hvgr_pm_power_off_async(struct hvgr_device *gdev)
{
	...
	hvgr_systrace_begin("%s get power_lock", __func__);
	mutex_lock(&pm_pwr->power_lock);
	hvgr_systrace_end();
	if (pm_pwr->cur_status == HVGR_PM_POWER_OFF)
		goto exit;
	...
	hvgr_datan_ct_disable_by_power(gdev, true);
#if defined(HVGR_STATE_BUNDLE2) && HVGR_STATE_BUNDLE2 > 0
	hvgr_mcu_mem_flush(gdev);
#endif
	(void)hvgr_pm_update_pwr_state(gdev, HVGR_PM_GPC_POWERING_OFF);
#if defined(HVGR_WAITFENCE_LOWPOWER) && HVGR_WAITFENCE_LOWPOWER != 0
	if ((atomic_read(&gdev->cq_dev.wait_fence_num) == 0) && hvgr_hts_is_waitqueue_empty(gdev))
#else
	if (hvgr_hts_is_waitqueue_empty(gdev))
#endif
...
int hvgr_pm_power_soft_init(struct hvgr_device *gdev)
{
	...
	mutex_init(&pm_pwr->power_ctrl_lock);
...
int hvgr_pm_reboot_gpu(struct hvgr_device *gdev)
{
	...
	if (hvgr_is_always_on(gdev)) {
		hvgr_info(gdev, HVGR_PM, "%s always on policy can reboot", __func__);
		return 0;
	}

	ret = hvgr_sch_suspend(gdev);
	if (ret != 0) {
		hvgr_err(gdev, HVGR_PM, "%s submit_allow=%d,sch_switch_kref=%d.\n", __func__,
	...
	/* wait power off done */
	time_out = jiffies + msecs_to_jiffies(500);
	do {
		wait_time = time_out - jiffies;
		ret = (int)wait_event_interruptible_timeout(pm_pwr->power_off_done_wait,
			pm_pwr->cur_status == HVGR_PM_POWER_OFF, (long)wait_time);
		if (ret == -ERESTARTSYS) {
			hvgr_debug(gdev, HVGR_PM, "%s wait ERESTARTSYS", __func__);
			udelay(20);
		}
	...
	if (ret <= 0) {
		...
		hvgr_err(gdev, HVGR_PM, "wait power off time out, ret = 0x%x", ret);
		return -1;
	}
	...
	hvgr_info(gdev, HVGR_PM, "%s done", __func__);
	return 0;
}
