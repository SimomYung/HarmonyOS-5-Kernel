...
{
	struct hvgr_pm_pwr *pm_pwr = &gdev->pm_dev.pm_pwr;
	bool gpc_force = false;

	/*
	 * If dest_state is not GPU_READY, then we must wait until gpc powered off.
	 * Before BL power off, gpc status must be powered off.
	 */
	if (dest_state != HVGR_PM_GPU_READY)
		gpc_force = true;

	/* wait for GPC powering off */
	if (!hvgr_pm_wait_gpc_poweroff_ready(gdev, gpc_force)) {
		pm_pwr->cur_status = HVGR_PM_CORE_ERROR;
		return;
	}

	if (dest_state == HVGR_PM_GPU_READY) {
		hvgr_info(gdev, HVGR_PM, "Begin unreset core, when powering off");
		if (!hvgr_pm_driver_unreset_cores(gdev))
			pm_pwr->cur_status = HVGR_PM_CORE_ERROR;
		else
			pm_pwr->cur_status = HVGR_PM_GPU_READY;
