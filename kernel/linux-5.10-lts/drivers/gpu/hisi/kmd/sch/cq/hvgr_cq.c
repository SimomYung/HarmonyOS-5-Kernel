...
long hvgr_ioctl_alloc_cq(struct hvgr_ctx * const ctx, union hvgr_ioctl_para_alloc_cq * const para)
{
	...
	cq_ctx = hvgr_cq_alloc_queue(ctx, para->in.bind_softq, para->in.all_entry_size,
		para->in.priority, para->in.rt_flag);
	if (cq_ctx == NULL) {
	...
	hvgr_ctx_ioctl_record(ctx, ktime_to_ns(ktime_get()), HVGR_IOCTL_ALLOC_CQ, cq_ctx->queue_id);
	para->out.queue_id = cq_ctx->queue_id;
	para->out.gpu_status_addr = ptr_to_u64(cq_ctx->cq_status_area->gva);
	para->out.sw_wr_context_addr = cq_ctx->sw_ctx_addr;
	para->out.hw_wr_context_addr = cq_ctx->hw_ctx_addr;
	para->out.cq_entry_addr = cq_ctx->cq_entry_base_uva;

#if hvgr_version_ge(HVGR_V500)
	datan_jobtrace_dump_sw_ctx(ctx, cq_ctx);
#endif
	mutex_unlock(&ctx->ctx_lock);
	return 0;
}
...
long hvgr_ioctl_free_cq(struct hvgr_ctx * const ctx, union hvgr_ioctl_para_free_cq * const para)
{
	...
	hvgr_ctx_ioctl_record(ctx, ktime_to_ns(ktime_get()), HVGR_IOCTL_FREE_CQ, para->in.queue_id);
...
bool hvgr_cq_ctx_is_active(struct hvgr_ctx * const ctx)
{
	uint32_t idx;
	...
	for (idx = 0; idx < ARRAY_SIZE(ctx->ctx_cq.cq_ctx); idx++) {
		if (ctx->ctx_cq.cq_ctx[idx].sw_wr_ctx == NULL)
			continue;
		if (ctx->ctx_cq.cq_ctx[idx].status != CQ_STATUS_IDLE) {
			hvgr_err(ctx->gdev, HVGR_CQ, "[ctx_sw] cq:%u retain active ctx,status:%u.",
				idx, ctx->ctx_cq.cq_ctx[idx].status);
			return true;
		}
	}
	return false;
...
void hvgr_cq_check_ctx_is_active(struct hvgr_ctx * const ctx)
{
	uint32_t idx;
	unsigned long flags;
	...
	for (idx = 0; idx < ARRAY_SIZE(ctx->ctx_cq.cq_ctx); idx++) {
		...
		if (ctx->ctx_cq.cq_ctx[idx].status != CQ_STATUS_IDLE) {
			hvgr_debug(ctx->gdev, HVGR_CQ,
				"ctx_%u, queueid_%u retain active ctx.tgid=%d pid=%d pname=%s gname=%s qstatus=%d.",
				ctx->id, ctx->ctx_cq.cq_ctx[idx].queue_id, ctx->tgid, ctx->pid, ctx->process_name,
				ctx->group_name, ctx->ctx_cq.cq_ctx[idx].status);
...
long hvgr_cq_ctx_init(struct hvgr_ctx * const ctx)
{
	...
	INIT_LIST_HEAD(&cqctl->fence_waiting_list);
	spin_lock_init(&cqctl->wait_fence_lock);
#endif
	...
	for (idx = 0; idx < CQ_TOKEN_NUMS; ++idx)
		spin_lock_init(&cqctl->callback_data_lock[idx]);

	if (hvgr_ksync_init(ctx) != 0)
		return -1;
	...
	if (hvgr_ctx_ioctl_record_init(ctx) != 0) {
		hvgr_token_term(ctx);
		hvgr_ksync_term(ctx);
		return -1;
	}
...
void hvgr_cq_ctx_term(struct hvgr_ctx * const ctx)
{
	...
	hvgr_msync_term(ctx);
	hvgr_ctx_ioctl_record_term(ctx);
	mutex_unlock(&ctx->ctx_lock);
...
static struct hvgr_sch_backend_ops g_sch_backend_ops = {
	...
	.start = hvgr_cq_start_and_schedule,
	...
	.hts_schedule = hvgr_cq_hts_schedule,
};
