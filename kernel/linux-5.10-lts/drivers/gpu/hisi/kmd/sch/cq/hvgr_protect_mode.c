...
#if hvgr_version_lt(HVGR_V300) && (defined(HVGR_FEATURE_PROTECT_MODE))
...
bool hvgr_mem_protect_jcd_pool_init(struct hvgr_device * const gdev)
{
	...
	if (!hvgr_get_prot_enhance_from_of(gdev) || !prot_enhance)
		return true;
	...
	hvgr_debug(gdev, HVGR_MEM, "%s mod_to nc.\n", __func__);
#endif
#endif
...
{
	if (!prot_enhance || gdev->mem_dev.protect_pool == NULL)
		return false;
...
void hvgr_mem_protect_jcd_term(struct hvgr_ctx * const ctx)
{
	...
	paras.va =  ctx->jcd_area->gva;
	paras.pages = ctx->jcd_area->pages;
	paras.bitmap = 0U;
	INIT_LIST_HEAD(&paras.free_list);

	(void)hvgr_mmu_take_down(ctx, &paras, ctx->jcd_area);
	...
	hvgr_mem_zone_free_gva(ctx->jcd_area->zone->pmem, ZONE_TYPE_GPU, ctx->jcd_area->gva,
		(ctx->jcd_area->max_pages << PAGE_SHIFT));
...
static uint32_t hvgr_cq_get_entry_info(struct hvgr_cq_ctx * const cqctx, struct js_info_s *js_info)
{
	...
	uint32_t get_index_hold = hvgr_get_queue_get_index(cqctx);
	...
	struct js_info_s *js_info)
{
	...
	hvgr_timer_setup(&cqctx->backup_timer_protect_mode, backup_timer_callback_protect_mode);
	(void)mod_timer(&cqctx->backup_timer_protect_mode,
		jiffies + gdev->cq_dev.reset_timeout_normal * HZ / CQ_TIME_S2MS);
}
...
static void hvgr_cq_schedule_js_in(struct hvgr_cq_ctx * const cqctx)
{
	...
	cqctx->ctx->gdev->time_record.gpu_running = (uint64_t)ktime_to_ns(ktime_get());
	hvgr_cq_mod_status_macro(cqctx, CQ_STATUS_RUNNING);
...
static void hvgr_cq_signal_hold_en_out_token(struct hvgr_cq_ctx * const cqctx)
{
	...
	uint32_t get_index_hold = (hvgr_get_queue_get_index(cqctx) + 2) % ENTRY_NUM_MAX;
	...
	hvgr_token_finish(cqctx->ctx, cmd_entry->token);
}
...
static void kmd_gpu_job_done(struct hvgr_device *gdev, int js)
{
	...
	if (jc_done != 0) {
		...
		hvgr_info(gdev, HVGR_CQ, "%s update ctx_%u, qid=%d. getindex=%d\n",
			__func__, ctx->id, cqctx->queue_id, hvgr_get_queue_get_index(cqctx) + 3);

		cqctx->sw_wr_ctx->queue_get_index = (hvgr_get_queue_get_index(cqctx) + 3) % ENTRY_NUM_MAX;
		...
		hvgr_cq_mod_status_macro(cqctx, CQ_STATUS_IDLE);
...
irqreturn_t hvgr_job_irq_handler(int irq, void *data)
{
	...
	done = hvgr_read_reg(gdev, job_control_reg(gdev, FCM_SLOT_IRQ_STATUS));
	...
	uint32_t channel_id)
{
	...
	case HVGR_CQ_ENTER_PROTECTED_CONFIG:
		...
		hvgr_write_reg(gdev, gpu_control_reg(gdev, GPU_PROT_MODE),
	...
	uint32_t channel_id)
{
	...
	case HVGR_CQ_EXIT_PROTECTED_WAIT_IDLE:
		if (!prot_enhance) {
			...
			gdev->time_record.gpu_idle = (uint64_t)ktime_to_ns(ktime_get());
		}
