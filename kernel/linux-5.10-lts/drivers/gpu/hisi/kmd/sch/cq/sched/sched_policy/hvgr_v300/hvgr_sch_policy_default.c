	...
	uint32_t *cnt_sum, uint32_t real)
{
	...
	for (idx = CQ_PRIORITY_MAX - 1; idx >= lowest_pri; idx--) {
...
#if defined(HVGR_SUPPORT_SPEC_LOW) && HVGR_SUPPORT_SPEC_LOW != 0
static struct hvgr_cq_ctx *hvgr_sch_dequeue_low_spec(struct hvgr_device * const gdev)
{
	struct hvgr_cq_dev * const gcqctx = &gdev->cq_dev;
	struct cq_pending *pending_queue = NULL;
	struct hvgr_cq_ctx *cq_ctx = NULL;
	...
	if (!list_empty(&pending_queue->pending_list)) {
		hvgr_info(gdev, HVGR_SPEC_LOW, "%s, pending list not empry", __func__);
		cq_ctx = hvgr_sch_dequque_cq_ctx(pending_queue);
	}
...
#if defined(HVGR_SUPPORT_SPEC_LOW) && HVGR_SUPPORT_SPEC_LOW != 0
	if (cq_ctx == NULL)
		cq_ctx = hvgr_sch_dequeue_low_spec(gdev);
#endif
	return cq_ctx;
}
