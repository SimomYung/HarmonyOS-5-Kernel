...
uint64_t hvgr_semaphore_get_addr(struct hvgr_ctx * const ctx, uint64_t sema_addr)
{
	...
	if (unlikely(!ctx_cq->semp_res_flag || info->semp_buffer->kva == 0 || info->semp_buffer->uva == 0 ||
		info->size == 0))
		goto exit;

	if (unlikely((sema_addr < info->semp_buffer->uva) ||
		(sema_addr >
		(info->semp_buffer->uva + info->size - CQ_SEMAPHORE_ADDR_ALIGNED_SIZE))))
		goto exit;

	ret_addr = info->semp_buffer->kva + (sema_addr - info->semp_buffer->uva);
	...
	uint64_t sema_uaddr, uint64_t sema_value)
{
	...
	if (cur_value != 0U || sema_uaddr == 0U)
		hvgr_err(ctx->gdev, HVGR_CQ,
			"%s, cur_value is 0x%llx, set value 0x%llx, sem_addr 0x%llx\n",
...
void hvgr_semaphore_set(struct hvgr_ctx * const ctx, uint64_t sema_addr, uint64_t sema_value)
{
	...
	if (unlikely((ctx == NULL) || (sema_addr == 0))) {
		pr_err("[HVGR_ERR]%s, ctx or sema_addr is null .\n", __func__);
		return;
	}
	...
	if (unlikely(!ctx_cq->semp_res_flag || info->semp_buffer->kva == 0 || info->semp_buffer->uva == 0 ||
		info->size == 0)) {
		hvgr_err(ctx->gdev, HVGR_CQ, "%s, semp_buffer has been freed .\n", __func__);
		spin_unlock_irqrestore(&ctx_cq->semp_lock, flags);
		return;
	}
	...
	union hvgr_ioctl_para_alloc_semp * const para)
{
	...
	para->out.semaphore_addr = ctx->ctx_cq.semp_info.semp_buffer->gva;
...
void hvgr_msync_term(struct hvgr_ctx * const ctx)
{
	...
	if (ctx->ctx_cq.semp_info.semp_buffer != NULL) {
		hvgr_mem_free_kernel_area(ctx, ctx->ctx_cq.semp_info.semp_buffer);
...
void hvgr_msync_event_wakeup(struct hvgr_ctx * const ctx)
{
	...
	ctx->token_process_step = TOKEN_PROCESS_WAKEUP;
}

bool hvgr_msync_event_checkdata_no_lock(struct hvgr_ctx * const ctx)
{
	struct hvgr_cq_ctx_data *ctx_cq = &ctx->ctx_cq;

	return ctx_cq->token_finish_wr != ctx_cq->token_finish_rd;
}
...
void hvgr_msync_event_enqueue(struct hvgr_ctx * const ctx, struct msync_event *event)
{
	...
	if (((event->data < CQ_TOKEN_NUMS) && (event->data != 0)) ||
		...
		ctx->token_process_step = TOKEN_PROCESS_ENQUEUE;
...
uint32_t hvgr_token_dequeue(struct hvgr_ctx * const ctx, char __user *buf, uint32_t num)
{
	...
	ctx->token_process_step = TOKEN_PROCESS_DEQUEUE;
	...
	else if (!(atomic_read(&ctx->softq_closed) != 0) && ctx->kfile != NULL)
...
#ifdef CONFIG_LIBLINUX_HVGR_POLL_OPT
void hvgr_msync_liblinux_pal_sync_wakeup(struct hvgr_ctx * const ctx)
{
	unsigned long flags;

	spin_lock_irqsave(&ctx->ctx_cq.event_lock, flags);
	if (ctx->kfile == NULL) {
		hvgr_err(ctx->gdev, HVGR_CQ, "%s ctx_%u kfile has benn freed.", __func__, ctx->id);
		goto no_file;
	}
	if (hvgr_msync_event_checkdata_no_lock(ctx) || (atomic_read(&ctx->softq_closed) != 0))
