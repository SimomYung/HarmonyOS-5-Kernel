...
static void hvgr_ksync_wait_fence_semp_record_check(struct hvgr_ctx * const ctx,
	uint64_t semp_addr, uint64_t semp_value)
{
	uint32_t idx;
	struct hvgr_cq_ctx_data *ctx_cq = &ctx->ctx_cq;

	/* find the same addr in semp record */
	for (idx = 0U; idx < HVGR_WAIT_FENCE_SEMP_RECORD_MAX; idx++)
		if (semp_addr == ctx_cq->semp_record.sem_addr[idx]) {
			hvgr_debug(ctx->gdev, HVGR_FENCE,
				"%s ctx_%u idx=%u hw_semp_value=0x%llx semp_value=0x%llx.",
				__func__, ctx->id, idx, semp_value, ctx_cq->semp_record.sem_value[idx]);
			break;
		}

	if (idx == HVGR_WAIT_FENCE_SEMP_RECORD_MAX)
		hvgr_debug(ctx->gdev, HVGR_FENCE, "%s ctx_%u can not find same semp_addr.",
			__func__, ctx->id);
}

static void hvgr_fence_wait_list_info(struct hvgr_ctx * const ctx)
{
	unsigned long flags;
	...
	if (list_empty(&ctx_cq->fence_waiting_list))
		hvgr_debug(ctx->gdev, HVGR_SCH, "%s ctx_%u fence waiting list is empty.",
			__func__, ctx->id);

	list_for_each_entry_safe(iter, tmp, &ctx_cq->fence_waiting_list, pending_item) {
		cq_ctx = iter;
		hvgr_ksync_wait_fence_semp_record_check(ctx,
			cq_ctx->wait_fence_sem_addr, cq_ctx->wait_fence_sem_value);
		hvgr_semaphore_print_value(ctx, cq_ctx->wait_fence_sem_addr,
			0U, cq_ctx->wait_fence_sem_value);
	}
	spin_unlock_irqrestore(&ctx_cq->wait_fence_lock, flags);
}

static void hvgr_cq_print_running_queue(struct hvgr_device *gdev)
{
	int idx;
	struct hvgr_cq_ctx *cq_ctx = NULL;
	...
	for (idx = 0; idx < CQ_CHANNEL_NUM; idx++) {
		if (gcqctx->channel_info[idx].running_queue != NULL) {
			...
			subchn_os = hvgr_read_reg(gdev, cq_control_reg(gdev,
				cq_bind_subchn_os(cq_ctx->channel_id)));
			hvgr_debug(gdev, HVGR_CQ,
				"ctx_%u, queueid_%u tgid=%d pid=%d chn=%u processname:%s,"
				"group:%s bind_status=%u queue_status=%u subchn_os=%u priority=%u.",
				ctx->id, cq_ctx->queue_id, ctx->tgid, ctx->pid, cq_ctx->channel_id, 
				ctx->process_name, ctx->group_name, bind_status, queue_status,
				subchn_os, cq_ctx->priority);
		}
	}
	spin_unlock_irqrestore(&gdev->cq_dev.schedule_lock, flags);
}

static void hvgr_fence_timeout_info_print(struct hvgr_ctx *ctx)
{
	hvgr_cq_print_running_queue(ctx->gdev);
	hvgr_fence_wait_list_info(ctx);
}
#endif
...
enum hrtimer_restart hvgr_fence_timeout_callback(struct hrtimer *timer)
{
	...
	if (ctx == NULL || ctx->gdev == NULL) {
		pr_err("[hvgr][HVGR_ERROR] fence time-expired");
		return HRTIMER_NORESTART;
	}

	ctx_cq = &ctx->ctx_cq;
	spin_lock_irqsave(&ctx_cq->callback_data_lock[callback->token], flags);
	if (!ctx->gdev->cq_dev.submit_allow) {
		hvgr_info(ctx->gdev, HVGR_FENCE, "CQ submit is stop when fence time-expired");
		goto out;
	}

	/* flag bit2 is wait fence, bit3 is trigger fence */
	hvgr_debug(ctx->gdev, HVGR_FENCE, "ctx_%u fence time-expired tpid=%d pid=%d flag=0x%x token=%u",
		ctx->id, ctx->tgid, ctx->pid, callback->flags, callback->cq_event.token);
	hvgr_debug(ctx->gdev, HVGR_FENCE, "%s process_name: %s, group_name: %s\n",
		 __func__, ctx->process_name, ctx->group_name);
	if (callback->fence_data != NULL && callback->fence_data->ops != NULL)
		hvgr_debug(ctx->gdev, HVGR_FENCE, "fence time-expired drv:%s, timeline:%s, seqno:%llu\n",
...
out:
	spin_unlock_irqrestore(&ctx_cq->callback_data_lock[callback->token], flags);
#if defined(HVGR_WAITFENCE_LOWPOWER) && HVGR_WAITFENCE_LOWPOWER != 0
	hvgr_fence_timeout_info_print(ctx);
#endif
	return HRTIMER_NORESTART;
}
...
static void hvgr_trigger_timer_callback(struct timer_list *timer)
{
#else
static void hvgr_trigger_timer_callback(unsigned long addr)
{
	...
	callback = container_of(timer, struct token_callback_data, trigger_timer);
	ctx = callback->ctx;

	if (ctx == NULL || ctx->gdev == NULL) {
		pr_err("[hvgr][HVGR_ERROR] trigger timer error");
		return;
	}
	...
	if (callback->fence_data == NULL) {
		hvgr_err(ctx->gdev, HVGR_FENCE,
			"ctx_%u, trigger timer token callback wait fence is NULL. token = %x\n", ctx->id,
			callback->cq_event.token);
		goto out;
	}

	fence = callback->fence_data;
	ret = dma_fence_remove_callback(fence, &callback->fence_cb);
	if (ret) {
		hvgr_debug(ctx->gdev, HVGR_FENCE,
			"%s [fence_info] proactive tigger wait fence drv:%s timeline:%s seqno:%llu %d\n",
		...
		hvgr_debug(ctx->gdev, HVGR_FENCE,
			"%s [fence_info] wait fence has signed:%s timeline:%s seqno:%llu %d\n",
			__func__, fence->ops->get_driver_name(fence),
			fence->ops->get_timeline_name(fence),
			fence->seqno, (int)ctx->tgid);
	}
...
void hvgr_callback_data_alloc_and_init(struct hvgr_ctx *ctx, uint32_t token)
{
	...
	hvgr_timer_setup(&cb_data->trigger_timer, hvgr_trigger_timer_callback);
	spin_lock_irqsave(&ctx_cq->callback_data_lock[token], flags);
	ctx_cq->token_callback[token] = cb_data;
	spin_unlock_irqrestore(&ctx_cq->callback_data_lock[token], flags);
out:
	mutex_unlock(&ctx_cq->as_lock[token]);
}
	...
	struct token_callback_data *callback)
{
	...
	uint64_t timeout_time = hvgr_platform_get_sch_fence_timeout();
	...
	struct hvgr_msync * const msync, wq_work_entry *entry)
{
	...
	spin_lock_irqsave(&ctx->ctx_cq.callback_data_lock[msync->token], flags);
	...
	if (unlikely(callback == NULL)) {
		hvgr_err(ctx->gdev, HVGR_CQ, "%s token = %u, alloc callback data failed.",
			__func__, msync->token);
		spin_unlock_irqrestore(&ctx->ctx_cq.callback_data_lock[msync->token], flags);
		return false;
	}
	...
	hvgr_datan_jt_record_hold_cnt(ctx,
		(uint32_t)atomic_read(&cq_ctx->wq_hold_count), cq_ctx->queue_id);
	hvgr_info(ctx->gdev, HVGR_CQ, "ctx_%u WQ register wait token = %u, count = %d",
...
void hvgr_token_do_wq_wait(struct hvgr_ctx *ctx, struct token_callback_data *callback)
{
	...
	spin_lock_irqsave(&ctx->ctx_cq.callback_data_lock[callback->token], flags);
	/* callback is not wq wait */
	if ((callback->flags & HVGR_TOKEN_CB_WQ_WAIT) != HVGR_TOKEN_CB_WQ_WAIT ||
		callback->cq_ctx == NULL) {
		spin_unlock_irqrestore(&ctx->ctx_cq.callback_data_lock[callback->token], flags);
		return;
	}

	cq_ctx = callback->cq_ctx;
	/* cq fault flow or kill proc. */
	if (hvgr_cq_is_queue_error(cq_ctx) || hvgr_ctx_flag(ctx, HVGR_CTX_FLAG_DYING)) {
		hvgr_debug(ctx->gdev, HVGR_CQ, "%s ctx_%u queue error(%u) or kill proc",
			__func__, ctx->id, cq_ctx->err_flags);
		callback->cq_ctx = NULL;
		callback->flags = 0;
	...
	hvgr_datan_jt_record_hold_cnt(ctx, (uint32_t)atomic_read(&cq_ctx->wq_hold_count),
		cq_ctx->queue_id);
...
/*
 * return 0       : success
 * return -EINVAL : no need do token finish.
 * return other   : need do token finish.
 */
long hvgr_token_register_trigger_fence(struct hvgr_ctx * const ctx,
	struct hvgr_msync *msync, struct dma_fence *fence, bool ai_freq_enabled)
{
	...
	hvgr_callback_data_alloc_and_init(ctx, msync->token);
	spin_lock_irqsave(&ctx_cq->callback_data_lock[msync->token], flags);
	callback = ctx_cq->token_callback[msync->token];
	if (unlikely(callback == NULL)) {
		hvgr_err(ctx->gdev, HVGR_FENCE, "%s token = %u, alloc callback data failed.",
			__func__, msync->token);
		spin_unlock_irqrestore(&ctx_cq->callback_data_lock[msync->token], flags);
		return -ENOSPC;
	}
	...
	if (((msync->check_wait_flag) && (msync->wait_token == CQ_TOKEN_INVALID)) ||
		(hvgr_cq_has_queue_error(ctx))) {
		spin_unlock_irqrestore(&ctx_cq->callback_data_lock[msync->token], flags);
		return -EPERM;
	}
...
static void hvgr_token_do_trigger_fence(struct hvgr_ctx *ctx, struct token_callback_data *callback)
{
	unsigned long flags;
	struct dma_fence *fence = NULL;
	struct hvgr_cq_ctx_data *ctx_cq = &ctx->ctx_cq;
 ...
 * return other   : need do token finish.
 */
long hvgr_token_register_wait_fence(struct hvgr_ctx * const ctx,
	...
	dma_fence_func_t callback_func)
{
	...
	if (((callback->flags & HVGR_TOKEN_CB_WAIT_FENCE) != 0) ||
		...
		hvgr_err(ctx->gdev, HVGR_FENCE,
			"%s ctx_%u, fence token %u already set wait fence\n",
			__func__, ctx->id, token);
		spin_unlock_irqrestore(&ctx_cq->callback_data_lock[token], flags);
		return -EINVAL;
	}
	...
	if (callback->fence_data == NULL) {
		/* fence callback maybe signaled after add func immediately. so set need move to this. */
		hvgr_token_set_cq_event(&callback->cq_event, msync);
		...
		if (ret == 0) {
			...
			hvgr_fence_timeout_timer_start(ctx, callback);
			if ((strcmp(fence->ops->get_driver_name(fence), HVGR_DIRVER_NAME) == 0) &&
				(strcmp(fence->ops->get_timeline_name(fence), HVGR_TIMELINE_NAME) == 0))
				(void)mod_timer(&callback->trigger_timer,
					jiffies + (uint32_t)FENCE_TRIGGER_TIMEOUT * HZ);
			spin_unlock_irqrestore(&ctx_cq->callback_data_lock[token], flags);
			return 0;
		} else if (ret == -ENOENT) {
...
static void hvgr_token_do_wait_fence(struct hvgr_ctx *ctx, struct token_callback_data *callback)
{
	unsigned long flags;
	struct dma_fence *fence = NULL;
	...
	hvgr_fence_timeout_timer_cancel(ctx, callback);
	del_timer_sync(&callback->trigger_timer);
	hvgr_ksync_remove_fence(ctx, fence, callback->cq_event.token, WAIT_FENCE_FLAG);
}

void hvgr_token_unregister_wait_fence(struct hvgr_ctx * const ctx, uint32_t token)
{
	unsigned long flags;
	struct token_callback_data *callback = NULL;
	struct hvgr_cq_ctx_data *ctx_cq = &ctx->ctx_cq;
	...
	if (callback->flags & HVGR_TOKEN_CB_WAIT_FENCE) {
		if (callback->fence_data != NULL) {
			hvgr_semaphore_set(ctx, callback->cq_event.sem_addr, callback->cq_event.sem_value);
			hvgr_info(ctx->gdev, HVGR_FENCE, "Kernel recover do wait fence set value "
				"addr 0x%llx value 0x%llx",
				callback->cq_event.sem_addr, callback->cq_event.sem_value);
	...
	uint32_t token, struct hvgr_msync *dependency)
{
	...
	spin_lock_irqsave(&ctx_cq->callback_data_lock[dependency->token], flags);
	dep_callback = ctx_cq->token_callback[dependency->token];
	if (unlikely(dep_callback == NULL)) {
		hvgr_err(ctx->gdev, HVGR_CQ, "%s token = %u, alloc callback data failed.",
			__func__, dependency->token);
		spin_unlock_irqrestore(&ctx_cq->callback_data_lock[dependency->token], flags);
		return -1;
	}
...
static void hvgr_token_do_cl_profiling(struct hvgr_ctx *ctx, struct token_callback_data *callback)
{
	unsigned long flags;
...
exit:
	...
	hvgr_msync_liblinux_pal_sync_wakeup(ctx);
#endif
}
...
void hvgr_token_clean_all(struct hvgr_ctx *ctx, bool need_free_token)
{
	...
	flush_workqueue(ctx->ctx_cq.fence_callback_wq);
#else
	kthread_flush_worker(ctx->ctx_cq.token_callback_wq);
	kthread_flush_worker(ctx->ctx_cq.fence_callback_wq);
#endif
 ...
 */
void hvgr_token_finish(struct hvgr_ctx * const ctx, uint32_t token)
{
	...
	struct hvgr_msync *msync, uint64_t policy_info, uint32_t info_num)
{
	...
	unsigned long flags;
	long ret;
	void *callback_policy_info = NULL;
	...
	hvgr_callback_data_alloc_and_init(ctx, msync->token);
	ret = hvgr_mem_sc_get_policy_info(ctx, policy_info, info_num, &callback_policy_info);
	if (ret != 0) {
	...
	callback->policy_info = callback_policy_info;
	...
	hvgr_info(ctx->gdev, HVGR_CQ, "%s ctx_%u, token %u callback->flags=0x%x", __func__,
		ctx->id, msync->token, callback->flags);
	...
	uint32_t freq_hint)
{
	...
	hvgr_callback_data_alloc_and_init(ctx, token);
	spin_lock_irqsave(&ctx_cq->callback_data_lock[token], lock_flag);
	callback = ctx_cq->token_callback[token];
	if (unlikely(callback == NULL)) {
		hvgr_err(ctx->gdev, HVGR_CQ, "%s token = %u, alloc callback data failed.",
			__func__, token);
		spin_unlock_irqrestore(&ctx_cq->callback_data_lock[token], lock_flag);
		return;
	}
	...
	union hvgr_ioctl_para_clear_cl_flags * const para)
{
	...
	ctx_cq = &ctx->ctx_cq;
	spin_lock_irqsave(&ctx_cq->callback_data_lock[para->in.token], lock_flag);
	callback = ctx_cq->token_callback[para->in.token];
	if (unlikely(callback == NULL)) {
		spin_unlock_irqrestore(&ctx_cq->callback_data_lock[para->in.token], lock_flag);
		return 0;
	}
