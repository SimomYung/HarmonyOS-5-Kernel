	...
	union hvgr_ioctl_para_bind_softq * const para)
{
	...
	hvgr_ctx_ioctl_record(ctx, ktime_to_ns(ktime_get()), HVGR_IOCTL_BIND_SOFTQ, softq_id);
	...
	union hvgr_ioctl_para_term_softq * const para)
{
	...
	mutex_lock(&ctx->ctx_lock);
	stage = para->in.stage;
	hvgr_ctx_ioctl_record(ctx, ktime_to_ns(ktime_get()), HVGR_IOCTL_TERM_SOFTQ, stage);
	if (stage == HVGR_TERM_STAGE_MESSAGE) {
		atomic_set(&ctx->softq_closed, true);
		hvgr_datan_flush_softq_trace(ctx);
	...
	unsigned int cmd, unsigned long arg)
{
	...
	case HVGR_IOCTL_HTS_CLEAR_EVENT:
		hvgr_ioctl_handle_w(ctx, HVGR_IOCTL_HTS_CLEAR_EVENT,
			hvgr_ioctl_hts_clear_event, arg, union hvgr_ioctl_para_hts_clear_event);
		break;
#endif
	default:
