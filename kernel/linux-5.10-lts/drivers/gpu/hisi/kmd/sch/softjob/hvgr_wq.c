...
struct hvgr_wq_jit_mem_data {
	uint64_t addr_ptr;
	uint64_t addr_ptr_backup;
	uint64_t request_size;
};

static hvgr_wq_opt_ret hvgr_wq_opt_alloc_jit_memory_helper(struct hvgr_cq_ctx * const
	cq_ctx, struct hvgr_wq_jit_mem_data * const jit_mem_data, uint64_t *jitaddr_ret)
{
	uint64_t jit_addr_for_usr;
	uint64_t jitaddr;
	long ret;

	jitaddr = hvgr_wq_alloc_jit_memory(cq_ctx->ctx, jit_mem_data->request_size);
	if (jitaddr == 0)
		return WQ_RET_FAIL;

	jit_addr_for_usr = jitaddr + 128;
	hvgr_info(cq_ctx->ctx->gdev, HVGR_CQ, "%s jit mem=0x%llx, mem_addr_pointer=0x%llx 0x%llx",
		__func__, jit_addr_for_usr, jit_mem_data->addr_ptr, jit_mem_data->addr_ptr_backup);
	ret = hvgr_mem_rw(cq_ctx->ctx, jit_mem_data->addr_ptr, &jit_addr_for_usr, true);
	ret += hvgr_mem_rw(cq_ctx->ctx, jit_mem_data->addr_ptr_backup, &jit_addr_for_usr, true);
	if (ret != 0) {
	...
	*jitaddr_ret = jitaddr;
	datan_jobtrace_set_value((uint64_t *)u64_to_ptr(jit_mem_data->addr_ptr), jitaddr + 128);
	...
	hvgr_datan_jobtrace(cq_ctx->ctx, JOBTRACE_TYPE_OBJ, KDATAN_JD_UPDATE_JIT_MEMORY,
		cq_ctx->ctx, jitaddr, jitaddr, jit_mem_data->request_size, cq_ctx->queue_id);

	hvgr_datan_jobtrace(cq_ctx->ctx, JOBTRACE_TYPE_OBJ, KDATAN_SET_PTR, cq_ctx->ctx,
		jitaddr,  jitaddr + 128, "jit", cq_ctx->queue_id);

	hvgr_datan_jobtrace(cq_ctx->ctx, JOBTRACE_TYPE_OBJ, KDATAN_SET_PTR, cq_ctx->ctx,
		jit_mem_data->addr_ptr, jitaddr + 128, "jit", cq_ctx->queue_id);
	...
	wq_work_opt const * const opt)
{
	...
	struct hvgr_wq_jit_mem_data jit_mem_data;
	...
	for (idx = 0; idx < num; idx++) {
		jit_mem_data.addr_ptr = para->mem_addr_pointer[idx];
		jit_mem_data.addr_ptr_backup = para->mem_addr_pointer_backup[idx];
		jit_mem_data.request_size = (uint64_t)para->mem_request_size[idx];
		ret = hvgr_wq_opt_alloc_jit_memory_helper(cq_ctx, &jit_mem_data, &jit_addr[idx]);
		if (ret != WQ_RET_PASS) {
			alloc_idx = idx;
			goto alloc_jit_fail;
		}
...
alloc_jit_fail:
	...
	hvgr_debug(cq_ctx->ctx->gdev, HVGR_CQ, "hvgr %s jit alloc mem fail, num %u",
		__func__, num);
	...
	cq_ctx, uint64_t mem_addr_pointer)
{
	...
	hvgr_info(cq_ctx->ctx->gdev, HVGR_CQ, "%s free at 0x%llx", __func__, mem_addr_pointer);
	...
	wq_work_opt const * const opt)
{
	...
	if (hvgr_mem_sparse_bind(cq_ctx->ctx, sparse_addr, mem_addr, pages, true) != 0)
		return WQ_RET_FAIL;
	...
	cq_ctx, wq_work_opt const * const opt)
{
	...
	/* Wakeup */
	hvgr_token_finish(cq_ctx->ctx, token);
	return WQ_RET_PASS;
}
...
static bool hvgr_wq_has_wait_semaphore(struct hvgr_cq_ctx * const cq_ctx)
{
	if (unlikely(hvgr_cq_is_queue_error(cq_ctx)))
		hvgr_debug(cq_ctx->ctx->gdev, HVGR_CQ, "%s ctx_%u queue err",
...
static long hvgr_wq_get_event_wait_timeout(struct hvgr_device * const gdev)
{
	return (long)msecs_to_jiffies(
		hvgr_is_fpga(gdev) ? WQ_EVENT_WAIT_FPGA_TIMEOUT : WQ_EVENT_WAIT_TIMEOUT);
}
	...
	wq_work_opt const * const opt)
{
	...
	long timeout = hvgr_wq_get_event_wait_timeout(gdev);
	...
	hvgr_systrace_begin("%s ctx_%u", __func__, cq_ctx->ctx->id);
	...
	wq_work_opt const * const opt)
{
	...
	uint64_t sem_addr;
	uint32_t token = para->token;
	long timeout = hvgr_wq_get_event_wait_timeout(ctx->gdev);
	...
	wq_work_entry * const wq_entry, uint64_t *jit_addrs, int jit_num)
{
	...
	if (ret == WQ_RET_FAIL || ret == WQ_RET_ERROR)
		hvgr_debug(cq_ctx->ctx->gdev, HVGR_CQ, "WQ running FAIL");
	return ret;
}
...
static bool hvgr_wq_has_wait_max(struct hvgr_cq_ctx * const cq_ctx)
{
	...
	if (unlikely(hvgr_cq_is_queue_error(cq_ctx)))
		hvgr_debug(gdev, HVGR_CQ, "%s ctx_%u queue err",
			__func__, cq_ctx->ctx->id);
...
static hvgr_wq_opt_ret hvgr_wq_barrier_max_wait(struct hvgr_cq_ctx * const cq_ctx)
{
	long timeout = hvgr_wq_get_event_wait_timeout(cq_ctx->ctx->gdev);
	if (wait_event_timeout(cq_ctx->ctx->gdev->wq_dev.barrier_wait_wq,
		hvgr_wq_has_wait_max(cq_ctx), timeout) == 0) {
		hvgr_debug(cq_ctx->ctx->gdev, HVGR_CQ, "ctx_%u barrier max wait timeout, count=%d",
...
static void hvgr_wq_worker(struct hvgr_cq_ctx * const cq_ctx)
{
	...
	while (!hvgr_ctx_flag(cq_ctx->ctx, HVGR_CTX_FLAG_DYING)) {
		if (unlikely(hvgr_cq_is_queue_error(cq_ctx))) {
			hvgr_debug(cq_ctx->ctx->gdev, HVGR_CQ, "%s ctx_%u, qid=%u, fault, wq quit.",
		...
		if (entry == NULL) {
			...
			cq_ctx->wq_kwq_is_running = false;
			spin_unlock_irqrestore(&cq_ctx->wq_kwq_lock, flags);
			hvgr_info(cq_ctx->ctx->gdev, HVGR_CQ, "WQ empty ctx_%u, qid=%u",
		...
		if (ret == WQ_RET_SUSPEND) {
			...
			cq_ctx->wq_kwq_is_running = false;
			spin_unlock_irqrestore(&cq_ctx->wq_kwq_lock, flags);
			hvgr_info(cq_ctx->ctx->gdev, HVGR_CQ, "WQ barrier suspend ctx_%u, qid=%u",
				cq_ctx->ctx->id, cq_ctx->queue_id);
			break;
		}
		...
		if (hvgr_softq_dequeue(cq_ctx->sq_ctx) != 0)
			hvgr_err(cq_ctx->ctx->gdev, HVGR_CQ, "SQ dequeue FAIL ctx_%u, qid=%u",
				cq_ctx->ctx->id, cq_ctx->queue_id);
	}
	cq_ctx->wq_kwq_is_running = false;
}
...
#else
static void hvgr_wq_workqueue_handle(struct kthread_work *data)
#endif
{
...
#define HVGR_QUEUE_WORK_RETRY_TIMES    3
#define HVGR_QUEUE_WORK_DELAY_TIME     5
bool hvgr_wq_start_work(struct hvgr_cq_ctx * const cq_ctx)
{
	unsigned long flags;
	bool ret = false;
	int retry = HVGR_QUEUE_WORK_RETRY_TIMES;

	if (cq_ctx->sq_ctx == NULL) {
#if hvgr_version_ge(HVGR_V300)
		hvgr_err(cq_ctx->ctx->gdev, HVGR_CQ, "ctx_%u qid=%u queue work sq_ctx null.",
			cq_ctx->ctx->id, cq_ctx->queue_id);
#endif
		return true;
	}
	...
	while (retry > 0) {
#if (LINUX_VERSION_CODE < KERNEL_VERSION(5, 10, 0))
		ret = queue_work(cq_ctx->wq_kwq, &cq_ctx->wq_kwq_handle);
#else
		ret = kthread_queue_work(cq_ctx->wq_kwq, &cq_ctx->wq_kwq_handle);
#endif
		if (ret) {
			...
			hvgr_err(cq_ctx->ctx->gdev, HVGR_CQ,
				"ctx_%u qid=%u queue work fail, retry %d.",
				cq_ctx->ctx->id, cq_ctx->queue_id, retry);
		}
		retry--;
		udelay(HVGR_QUEUE_WORK_DELAY_TIME);
	}
	spin_unlock_irqrestore(&cq_ctx->wq_kwq_lock, flags);
	return true;
}
	...
	struct hvgr_cq_ctx * const cq_ctx, uint32_t softq_id)
{
	...
	if (cq_ctx->wq_kwq == NULL) {
		hvgr_err(ctx->gdev, HVGR_CQ, "alloc workqueue hvgr_wq_kwq fail.\n");
		return -1;
	}
	INIT_WORK(&cq_ctx->wq_kwq_handle, hvgr_wq_workqueue_handle);
#else
	cq_ctx->wq_kwq = kthread_create_worker(0, "hvgr_wq_kwq");
	if (IS_ERR(cq_ctx->wq_kwq)) {
		hvgr_err(ctx->gdev, HVGR_CQ, "alloc workqueue hvgr_wq_kwq fail.\n");
		return -1;
	}
	sched_set_fifo(cq_ctx->wq_kwq->task);
	kthread_init_work(&cq_ctx->wq_kwq_handle, hvgr_wq_workqueue_handle);
#endif
	return 0;
}
...
void hvgr_wq_ctx_term(struct hvgr_cq_ctx * const cq_ctx)
{
	...
	if (cq_ctx->wq_kwq != NULL) {
		...
		kthread_destroy_worker(cq_ctx->wq_kwq);
#endif
		cq_ctx->wq_kwq = NULL;
