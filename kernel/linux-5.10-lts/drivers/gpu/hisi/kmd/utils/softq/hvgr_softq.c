	...
	uint32_t entry_size, uint32_t entry_nums)
{
	...
	hvgr_info(ctx->gdev, HVGR_CQ, "%s ctx_%u start ", __func__, ctx->id);
	...
	hvgr_info(ctx->gdev, HVGR_CQ, "%s ctx_%u queue_area->gva=0x%lx.", __func__, ctx->id, qctx->queue_area->gva);

	area = hvgr_softq_alloc_ctrl(ctx);
	if (area == NULL) {
		hvgr_err(ctx->gdev, HVGR_CQ, "%s alloc ctrl mem fail.", __func__);
		hvgr_mem_free_kernel_area(qctx->ctx, qctx->queue_area);
		qctx->queue_area = NULL;
		return false;
	}
	qctx->ctrl_area = area;
	hvgr_info(ctx->gdev, HVGR_CQ, "%s ctx_%u ctrl_area->gva=0x%lx.", __func__, ctx->id, qctx->ctrl_area->gva);
	area = hvgr_softq_alloc_user_ctrl(ctx);
	if (area == NULL) {
		hvgr_err(ctx->gdev, HVGR_CQ, "%s alloc user ctrl mem fail.", __func__);
		hvgr_mem_free_kernel_area(qctx->ctx, qctx->queue_area);
		qctx->queue_area = NULL;
		hvgr_mem_free_kernel_area(qctx->ctx, qctx->ctrl_area);
		qctx->ctrl_area = NULL;
		return false;
	}
	qctx->user_ctrl_area = area;
	hvgr_info(ctx->gdev, HVGR_CQ, "%s ctx_%u user_ctrl_area->gva=0x%lx.", __func__, ctx->id, qctx->user_ctrl_area->gva);
	return true;
}
	...
	uint32_t entry_nums, uint64_t flags)
{
	...
	qctx->ctrl->queue_base = qctx->queue_area->gva;
	qctx->ctrl->user_ctrl_addr = qctx->user_ctrl_area->gva;
	...
	if (softq_id == HVGR_INVALID_SOFTQ_ID) {
		...
		hvgr_mem_free_kernel_area(qctx->ctx, qctx->ctrl_area);
		qctx->ctrl_area = NULL;
		hvgr_mem_free_kernel_area(qctx->ctx, qctx->user_ctrl_area);
		qctx->user_ctrl_area = NULL;
		hvgr_err(ctx->gdev, HVGR_MEM, "%s ctx_%u hvgr_alloc_softq_id FAIL.", __func__, ctx->id);
		return -ENOMEM;
	}
	...
	uint64_t flags, uint64_t *uva)
{
	...
	*uva = hvgr_softq_get_ctrl(qctx);
	hvgr_info(ctx->gdev, HVGR_CQ, "%s return uva=0x%llx.", __func__, (unsigned long long)*uva);
...
void *hvgr_softq_get_entry(struct hvgr_soft_q * const qctx)
{
	...
	hvgr_info(qctx->ctx->gdev, HVGR_CQ, "%s ctx_%u alloc_idx %u curr_idx %u.",
		__func__, qctx->ctx->id, alloc_idx, curr_idx);
