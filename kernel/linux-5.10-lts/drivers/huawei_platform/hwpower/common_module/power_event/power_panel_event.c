...
#define POWER_PANEL_HALL_WINDOW_SHELL_BASE 0x9 /* bit0 & bit3 */
#define POWER_PANEL_HALL_WINDOW_SHELL_FOLD 0x8 /* bit3:1 bit0:0 */
	...
	unsigned long action, void *data)
{
	...
	status = (action >> EXT_HALL_BIT_HINGLE) & SCREEN_FULL_FOLD;
	di->fold_status = (int)status;
	if (status == di->screen_full_fold) {
	...
	/* hall bit0 and bit3 is the window shell folding status */
	status = action & POWER_PANEL_HALL_WINDOW_SHELL_BASE;
	if (status == POWER_PANEL_HALL_WINDOW_SHELL_FOLD)
		power_event_bnc_notify(POWER_BNT_HALL_EVENT, POWER_NE_WINDOW_SHELL_FOLD, NULL);
	else
		power_event_bnc_notify(POWER_BNT_HALL_EVENT, POWER_NE_WINDOW_SHELL_UNFOLD, NULL);

	hwlog_info("raw hall status=0x%x\n", action);
	return NOTIFY_OK;
}
