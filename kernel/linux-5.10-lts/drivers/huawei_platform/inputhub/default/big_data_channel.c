...
static const big_data_event_detail_t big_data_event[] = {
	...
	{ BIG_DATA_EVENT_COVER_OFFSET_INFO, 11, event_cover_type_param },
};
...
static void process_cover_big_data_ohos(uint32_t event_id, void *data, uint32_t data_len)
{
	...
	hwlog_info("[process_cover_big_data_ohos]cover big data finish in ohos");
}
...
static void iomcu_dubai_ddr_log_fetch(void)
{
	...
	ddr_request_pack ddr_static_info;
	...
	if (((count_sum * THOUSAND / interval_time) >= DDR_REQ_THRESHOLD)
		...
		for (i = 0; i < USR_NUM; i++) {
			for (j = 0; j < PC_NUM; j++) {
				hwlog_info("usr_name: %d,total_count: %d,key: 0x%x,key_num: %d,time: %d \n",
					ddr_static_info.ddr_usr[i].usr_name, ddr_static_info.ddr_usr[i].total_count,
					ddr_static_info.ddr_usr[i].pc1[j].pc_index, ddr_static_info.ddr_usr[i].pc1[j].pc_num, ddr_static_info.ddr_request_time);
