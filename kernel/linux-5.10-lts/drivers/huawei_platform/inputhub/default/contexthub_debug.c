...
static int send_cmd_formats(unsigned char cmd_tag, unsigned char cmd_type, unsigned int subtype, const char *buf, unsigned int count,
			    bool is_lock)
{
	char *buffer = NULL;
	int ret = 0;
	...
	if (CMD_CMN_CONFIG_REQ != cmd_type) {
		...
		*subcmd = subtype;
		if (count) {
			ret = memcpy_s(buffer + (sizeof(struct pkt_header) + sizeof(unsigned int)), SENSORHUB_IPC_MAX_LEN - (sizeof(struct pkt_header) + sizeof(unsigned int)), buf,
				       count);
			if (EOK != ret) {
				hwlog_err("%s memcpy buffer fail, ret[%d]\n", __func__, ret);
				kfree(buffer);
