...
static int audio_fusion_gnss_power()
{
	int ret = -EFAULT;
...
int sensor_raw_big_data_report_ohos(const struct pkt_header *head)
{
	...
	for (i = 0; i < RAW_DATA_EVENTS_NUM; i++) {
		...
		for (j = 0; j < cycle_times; j++) {
			sprintf_s(event_key_name, sizeof(event_key_name), "DATA%d", j + 1);
...
static int als_ud_bl_report(const struct pkt_header *head)
{
	...
	if (head->tag == TAG_ALS) {
		memcpy_s(&(hisi_dkmd_alsc_init.bl_param), sizeof(hisi_dkmd_alsc_init.bl_param), (head + 1), sizeof(hisi_dkmd_alsc_init.bl_param));
		mcu_notifier_queue_work(head, als_update_bl_param_handler);
	} else {
		memcpy_s(&(hisi_dkmd_alsc1_init.bl_param), sizeof(hisi_dkmd_alsc1_init.bl_param), (head + 1), sizeof(hisi_dkmd_alsc1_init.bl_param));
