...
int process_drop_report(const pkt_drop_data_req_t *head)
{
	...
	// After the restart, the drop automatically reports all-zero data once.
	if (head->data.height <= 0) {
		hwlog_err("%s para error: drop_height <= 0, not report\n", __func__);
		return -1;
	}
	...
	ret += imonitor_set_param_integer_v2(obj, "Material", (long)(head->data.material));
	ret += imonitor_set_param_integer_v2(obj, "Speed", (long)(head->data.speed));
	...
	int32_t *raw_data_offset)
{
	...
	for (i = 0; i < cycle_times; i++) {
		sprintf_s(param, sizeof(param), "DATA%d", i + 1);
		sprintf_s(value, sizeof(value), "%llu", *(uint64_t *)(raw_data + *raw_data_offset));
...
int process_drop_report_ohos(const pkt_drop_data_req_t *head)
{
	...
	hwlog_info("[sensorALG][process_drop_report_ohos]Kernel get drop type %d, initial_speed %d, height %d, angle_pitch %d, angle_roll %d, impact %d posture: %d\n",
		head->data.type, head->data.initial_speed, head->data.height, head->data.angle_pitch,
		head->data.angle_roll, head->data.material, head->data.speed);
	schedule_drop_work(head->data);
