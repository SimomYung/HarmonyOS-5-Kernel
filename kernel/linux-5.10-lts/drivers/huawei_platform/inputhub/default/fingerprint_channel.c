...
static void fingerprint_report(int value)
{
	...
	fingerprint_upload.data.event = FHB_EVENT_INPUTHUB;
	fingerprint_upload.data.length = sizeof(value);
	fingerprint_upload.data.value[0] = value;
	...
	fingerprint_type_t type, bool use_lock)
{
	...
	if (cmd == CMD_CMN_OPEN_REQ) {
		if (!really_do_enable_disable(&fp_ref_cnt, true, type))
			return 0;
		inputhub_route_clean_buffer(ROUTE_FHB_PORT);
		...
		fingerprint_report(type == FINGERPRINT_TYPE_HUB ? FHB_INPUTHUB_CANCEL_FINGER : FHB_INPUTHUB_CANCEL_CONFIG);
...
static void enable_fingerprint_when_recovery_iom3(void)
{
	...
	fingerprint_report(FHB_INPUTHUB_SHB_RECOVERY);
}
...
static int __init fingerprinthub_init(void)
{
	...
	register_iom3_recovery_notifier(REC_USR_FP, &fingerprint_recovery_notify);
	hwlog_info("%s ok\n", __func__);
