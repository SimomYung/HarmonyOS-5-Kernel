...
#define CHR_DUANLBAND_ROAM_ERROR 909009185
#define CHR_DUANLBAND_ROAM_STATISTIC 909009524
...
uint32_t get_2g_pkt_fail(void) {
	if (g_amax_stat != NULL)
		return g_amax_stat->chr_stat.pkt_2g_fail;
	return 0;
}

uint32_t get_5g_pkt_fail(void) {
	if (g_amax_stat != NULL)
		return g_amax_stat->chr_stat.pkt_2g_fail;
	return 0;
}
...
{
	(void)memset_s(&(g_amax_stat->chr_stat), sizeof(amax_chr_stat), 0, sizeof(amax_chr_stat));
	(void)memset_s(&(g_amax_stat->roam_chr_stat), sizeof(amax_roam_chr_stat), 0, sizeof(amax_roam_chr_stat));
	return;
}
...
/* 上传双频漫游异常统计 */
static void amax_dualband_roam_err_dmd_upload(amax_roam_chr_err *roam_err)
{
	amax_roam_chr_stat *roam_sta = (amax_roam_chr_stat *)get_amax_roam_stat();
	char *upload_str = NULL;
	if (roam_err == NULL) {
		amax_print(PRINT_ERROR, "[dmd]roam_err is null, do not upload\n");
		return;
	}
	upload_str = (char *)kmalloc(MAX_SINGLE_STAT_SIZE, GFP_KERNEL);
	if (upload_str == NULL) {
	...
	if (sprintf_s(upload_str, MAX_SINGLE_STAT_SIZE,
		"AMAX_DUALBAND_ROAM_ERROR: [%u, %u] ",
		roam_err->ml_reasoncode, roam_err->ml_disconnect) < 0) {
		amax_print(PRINT_ERROR, "sprintf_s failed\n");
		kfree(upload_str);
		return;
	}
	amax_print(PRINT_DEBUG, "[dmd]upload dualband roam err:%s\n", upload_str);
#ifdef CONFIG_HUAWEI_DSM
	amax_dsm_client_notify(CHR_DUANLBAND_ROAM_ERROR, upload_str);
#endif
	kfree(upload_str);
}

/* 上传双频漫游连接统计 */
static void amax_dualband_roam_stat_dmd_upload(amax_roam_chr_stat *upload_stat)
{
	char *upload_str = NULL;
	if (upload_stat == NULL) {
		amax_print(PRINT_ERROR, "[dmd]upload_stat is null, do not upload\n");
		return;
	}
	upload_str = (char *) kmalloc(MAX_SINGLE_STAT_SIZE, GFP_KERNEL);
	if (upload_str == NULL) {
	...
	if (sprintf_s(upload_str, MAX_SINGLE_STAT_SIZE,
		"AMAX_DUALBAND_ROAM_STAT: [%u, %u, %u, %u, %u, %llu, %llu, %llu] ",
		upload_stat->roam_trigger, upload_stat->roam_success, upload_stat->roam_main_first, 
		upload_stat->duration_ave, upload_stat->send_duration_ave,
		upload_stat->rx_pkt_loss, upload_stat->tx_pkt_loss[0], upload_stat->tx_pkt_loss[1]) < 0) {
		amax_print(PRINT_ERROR, "sprintf_s failed\n");
		kfree(upload_str);
		return;
	}
	amax_print(PRINT_DEBUG, "[dmd]upload dualband roam stat:%s\n", upload_str);
#ifdef CONFIG_HUAWEI_DSM
	amax_dsm_client_notify(CHR_DUANLBAND_ROAM_STATISTIC, upload_str);
#endif
	kfree(upload_str);
}
...
/* 双频漫游连接统计大数据上报 */
void amax_dualband_roam_upload(uint64_t end_tstamp)
{
	amax_roam_chr_stat *roam_sta = (amax_roam_chr_stat *)get_amax_roam_stat();
	if (roam_sta == NULL || roam_sta->roam_trigger == 0) {
		amax_print(PRINT_DEBUG, "[dmd]roam_sta is null, do not upload\n");
		return;
	}
	uint64_t ktime_interval;
	uint64_t ktime_sec;
	uint64_t start_tstamp = roam_sta->last_upload_tstamp;
	if (roam_sta->last_upload_tstamp == 0) {
		roam_sta->last_upload_tstamp = end_tstamp;
	...
	if (end_tstamp > start_tstamp) {
		...
		amax_print(PRINT_DEBUG, "[dmd]last upload time: %llu cur time: %llu duration: %llu\n", start_tstamp, end_tstamp,
			ktime_sec);
		if (ktime_sec >= g_amax_tx_dmd_trigged_time_thr) {
			amax_dualband_roam_stat_dmd_upload(roam_sta);
			(void)memset_s(roam_sta, sizeof(amax_roam_chr_stat), 0, sizeof(amax_roam_chr_stat));
			roam_sta->last_upload_tstamp = end_tstamp;
		} else {
			amax_print(PRINT_DEBUG, "[dmd]not allow to upload ,time thr is %llu", g_amax_tx_dmd_trigged_time_thr);
		}
	} else {
		roam_sta->last_upload_tstamp = end_tstamp;
		amax_print(PRINT_ERROR, "[dmd]end_tstamp %llu is smaller than start_tstamp %llu\n", end_tstamp, start_tstamp);
...
void amax_chr_update_start(void)
{
	...
	/* 更新全局上报统计 */
	roam_chr_info->roam_trigger++;
	/* 更新单次双频漫游统计 */
	roam_chr_info->single_time.roam_start_timestamp = ktime_get(); //ns
	roam_chr_info->single_time.send_start_timestamp = ktime_get();
}

void amax_chr_update_first_link_done(void)
{
	amax_roam_chr_stat *roam_chr_info = (amax_roam_chr_stat *)get_amax_roam_stat();
	if(roam_chr_info == NULL) {
		return;
	...
	/* 更新单次双频漫游统计 */
	uint64_t end_time = ktime_get(); //ns
	if (end_time > roam_chr_info->single_time.send_start_timestamp)
		roam_chr_info->single_time.send_link_dura_ns[0] = 
			end_time - roam_chr_info->single_time.send_start_timestamp;
}

void amax_chr_update_second_link_start(void)
{
	amax_roam_chr_stat *roam_chr_info = (amax_roam_chr_stat *)get_amax_roam_stat();
	if(roam_chr_info == NULL) {
		return;
	...
	/* 更新单次双频漫游统计 */
	roam_chr_info->single_time.send_start_timestamp = ktime_get(); //ns
}

void amax_print_dualband_roam_info(void)
{
	amax_roam_chr_stat *roam_chr_info = (amax_roam_chr_stat *)get_amax_roam_stat();
	if (roam_chr_info == NULL)
		return;

	amax_print(PRINT_INFO, "[amax roam] trigger:%u, success:%u, mainfirst:%u, send_t:%u ms, send_a:%u ms, roam_t=%u ms, roam_a=%u ms\n",
		roam_chr_info->roam_trigger, roam_chr_info->roam_success, 
		roam_chr_info->roam_main_first,roam_chr_info->send_duration_total, 
		roam_chr_info->send_duration_ave, roam_chr_info->duration_total,
		roam_chr_info->duration_ave);
}

void amax_chr_update_dualband_roam_done(void)
{
	amax_roam_chr_stat *roam_chr_info = (amax_roam_chr_stat *)get_amax_roam_stat();
	...
	/* 更新单次双频漫游统计 */
	if (end_time > roam_chr_info->single_time.send_start_timestamp)
		roam_chr_info->single_time.send_link_dura_ns[1] = end_time - roam_chr_info->single_time.send_start_timestamp;
	/* 更新全局上报统计 */
	roam_chr_info->roam_success++;
	if (get_first_link_band() == fn_mode)
		roam_chr_info->roam_main_first++;

	if (end_time > roam_chr_info->single_time.roam_start_timestamp)
		roam_chr_info->duration_total += 
			(end_time - roam_chr_info->single_time.roam_start_timestamp) / TIME_NS_TO_MS; //ns->ms

	roam_chr_info->send_duration_total += 
		(roam_chr_info->single_time.send_link_dura_ns[0] + roam_chr_info->single_time.send_link_dura_ns[1]) / TIME_NS_TO_MS;
	roam_chr_info->duration_ave = 
		roam_chr_info->duration_total / roam_chr_info->roam_success;
	roam_chr_info->send_duration_ave = 
		roam_chr_info->send_duration_total / roam_chr_info->roam_success;
	(void)memset_s(&(roam_chr_info->single_time), sizeof(amax_roam_single_stat), 0, sizeof(amax_roam_single_stat));
	amax_print_dualband_roam_info();
}

void amax_chr_update_dualband_roam_fail(uint32_t reason_code, uint32_t rebuild)
{
	amax_roam_chr_err *roam_chr_err = (amax_roam_chr_err *)get_amax_roam_err();
	if(roam_chr_err == NULL) {
		amax_print(PRINT_ERROR, "{amax_chr_update_dualband_roam_fail} roam_chr_err null\n");
		return;
	}
	roam_chr_err->ml_reasoncode = reason_code;
	roam_chr_err->ml_disconnect = rebuild;
	amax_dualband_roam_err_dmd_upload(roam_chr_err);
}
