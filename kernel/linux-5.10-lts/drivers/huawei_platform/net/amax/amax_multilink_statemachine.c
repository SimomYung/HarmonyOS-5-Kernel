...
static uint32_t amax_ml_first_link_roaming_start_func(amax_roam_info_stru *roam_info, amax_chan_info_struct *chan_info)
{
	...
	amax_print(PRINT_INFO, "{amax_ml_first_link_roaming_start_func} g_first_link_band = %d,  g_second_link_band = %d\n", g_first_link_band, g_second_link_band);
...
static uint32_t amax_ml_first_link_done_func(amax_roam_info_stru *roam_info, amax_chan_info_struct *chan_info)
{
	...
	if (ml_check_roam_para(roam_info)) {
	    amax_print(PRINT_ERROR, "{amax_ml_first_link_done_func} roam_info check fail, roam success band = %d\n", roam_info->ml_roam_band);
...
static uint32_t amax_ml_second_link_roam_fail_resume_func(amax_roam_info_stru *roam_info, amax_chan_info_struct *chan_info)
{
	amax_ml_roam_main_change_state(roam_info, INIT_STATUS);
	/*重新触发multilink流程*/
	amax_ml_roam_fail_process();
                                 ...
                                 amax_chan_info_struct *chan_info)
{
	...
	if (dualband_roam_info->ml_roam_msg_type == ML_ROAM_START) {
		...
		/* 上报信令类型为双频漫游成功指示 */
		if (g_roam_info.ml_roam_main_state != FIRST_LINK_ROAM_ONGOING && g_roam_info.ml_roam_main_state != SECOND_LINK_ROAM_ONGOING) {
			/*双频漫游未开始,不处理漫游成功指示 */
