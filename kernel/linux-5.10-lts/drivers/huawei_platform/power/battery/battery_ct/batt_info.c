...
int get_battery_sn_by_id(unsigned int index, char *sn_buf, unsigned int *buf_size, unsigned int *sn_len)
{
	...
	if (!sn_buf || !buf_size || !sn_len)
		return -ENXIO;

	list_for_each_entry(temp, &batt_checkers_head, node) {
		checker_data = platform_get_drvdata(temp->pdev);
		if ((!checker_data) || (checker_data->id_in_grp == index))
			break;
	}

	if (!checker_data || (checker_data->id_in_grp != index) || !checker_data->bco.get_batt_sn)
		return -ENOTSUPP;
...
{
	int i;
	/* product does not need to be inspected */
	if (checker_data->backup_id_check_level <= 0)
		return true;

	if (checker_data->backup_id_check_level >= BACKUP_ID_CHECK_MAX) {
		hwlog_err("%s sn_backup is over\n", __func__);
		return false;
	}
...
static int check_backup_id_func(struct batt_chk_rslt *result)
{
	...
	list_for_each_entry(temp, &batt_checkers_head, node) {
		...
		if (!check_backup_id_validate(checker_data, val)) {
			hwlog_err("%s sn backup id content is not equal to required para\n", __func__);
			checker_data->result.sn_status = SN_SNS_UNMATCH;
			*result = checker_data->result;
			return BACKUP_ID_CHECK_ERROR;
		}
	...
	char *buf)
{
	...
	list_for_each_entry(temp, &batt_checkers_head, node) {
		...
		ret = get_battery_type_by_id(checker_data->id_in_grp, name, name_size);
		if (ret != EOK) {
			count += snprintf_s(buf + count, PAGE_SIZE,
