...
bool btf_temp_fitted_eqr(int batt_id)
{
	...
	if ((di->batt_id >= 0) && (batt_id != di->batt_id))
		return false;
...
static int _btf_dsm_report(struct btf_dev *di, int temp_adc, int temp_fitted)
{
	...
	/* report */
	return power_dsm_report_dmd(POWER_DSM_BATTERY, POWER_DSM_ERROR_BATT_TEMP_LOW,
		dsm_buf);
}
	...
	int temp_meas)
{
	...
	for (i = 0; i < di->linear_fit_para.coef_num - 1; i++)
		ret = ((tz_data[i].temp - temp_meas) > di->btf_temp_gap) && ret;
...
static enum btf_tz_group btf_sensor_group_select(struct btf_tz_temp *tz_data,
	int tz_num_max, int invalid_temp_th, int temp_th)
{
	int i;
	int temp1, temp2;
	...
	for (i = 0; i < ARRAY_SIZE(g_tz_grp_lkup_tbl); i++) {
		...
		temp2 = btf_tz_temp_lkup(tz_data, g_tz_grp_lkup_tbl[i].tz_name2,
			tz_num_max);
		if ((temp1 < invalid_temp_th) || (temp2 < invalid_temp_th))
			return BTF_TZ_GROUP_ILLEGAL;

		if (abs(temp1 - temp2) < temp_th)
...
static int btf_do_linear_fitting(int temp_meas, int *temp_fit)
{
	...
	di->tz_group = btf_sensor_group_select(tz_data,
		di->linear_fit_para.coef_num - 1, di->btf_invalid_temp_th, di->btf_select_th);
...
static int btf_do_replace_fitting(int temp_meas, int *temp_fit)
{
	int ret;
	int temp = 0;
	struct btf_dev *di = btf_get_dev();
	...
	ret = btf_get_tz_temp(di->replace_sensor, &temp);
	if (ret) {
		if (!di->ready && di->fake_btf_by_unready) {
			hwlog_info("btf unready, return fake val\n");
			*temp_fit = di->fake_btf_val;
	...
	if (abs(temp - temp_meas) <= di->btf_temp_gap) {
		hwlog_info("fit not necessary\n");
		return -EPERM;
	}
...
static void btf_validity_check_for_bt_measured(int *temp)
{
	...
	if (*temp < di->btf_temp_lth) {
		switch (di->fitting_mode) {
		case BTF_MODE_LINEAR:
			ret = btf_do_linear_fitting(*temp, &temp_fitted);
			break;
		case BTF_MODE_REPLACE:
			ret = btf_do_replace_fitting(*temp, &temp_fitted);
			break;
		default:
...
int btf_try_fit(int temp_in, int batt_id)
{
	int ret;
...
static int btf_replace_para_parse(struct device_node *np, struct btf_dev *di)
{
	int ret;
	const char *tmp_str = NULL;

	ret = power_dts_read_string(power_dts_tag(HWLOG_TAG), np, "replace_sensor", &tmp_str);
	if (ret)
		return ret;

	return strncpy_s(di->replace_sensor, THERMAL_NAME_LENGTH - 1, tmp_str, strlen(tmp_str));
}
...
static int btf_parse_dts(struct device_node *np)
{
	...
	(void)power_dts_read_str2int(power_dts_tag(HWLOG_TAG), np,
		"btf_temp_lth", &di->btf_temp_lth, BTF_TEMP_LTH);
	(void)power_dts_read_str2int(power_dts_tag(HWLOG_TAG), np,
		"btf_temp_gap", &di->btf_temp_gap, BTF_DO_LINE_FIT_TEMP_GAP);
	(void)power_dts_read_str2int(power_dts_tag(HWLOG_TAG), np,
		"btf_select_th", &di->btf_select_th, BTF_TZ_GROUP_SELECT_TH);
	(void)power_dts_read_str2int(power_dts_tag(HWLOG_TAG), np,
		"btf_invalid_temp_th", &di->btf_invalid_temp_th, 0);
	(void)power_dts_read_str2int(power_dts_tag(HWLOG_TAG), np,
		"batt_id", &di->batt_id, -1);
	...
	case BTF_MODE_LINEAR:
		...
		break;
	case BTF_MODE_REPLACE:
		ret = btf_replace_para_parse(np, di);
		if (ret)
			return ret;
		break;
	default:
