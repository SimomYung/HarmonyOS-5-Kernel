...
static int wltx_check_abnormal_vbatt(struct wltx_dev_info *di)
{
	...
	for (i = 0; i < 3; i++) {
		...
		if (tx_open_flag && (vbatt < (int)di->abnormal_vbat)) {
...
static bool wltx_is_7_5w(struct wltx_dev_info *di)
{
	hwlog_info("%d %d\n", wltx_cap_get_cur_id(WLTRX_DRV_MAIN), wltx_get_tx_pwr(WLTRX_DRV_MAIN));
	if ((wltx_cap_get_cur_id(WLTRX_DRV_MAIN) == WLTX_HIGH_PWR_CAP) &&
...
static void wltx_handle_ask_rx_event(struct wltx_dev_info *di)
{
	...
	case RX_STATR_CHARGING:
		...
		hwlog_info("%d %d\n", di->new_hp_timeout, wltx_is_7_5w(di));
		if (di->new_hp_timeout && wltx_is_7_5w(di)) {
