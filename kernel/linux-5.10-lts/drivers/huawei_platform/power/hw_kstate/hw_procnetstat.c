...
#ifdef CONFIG_LIBLINUX
static void procnet_liblinux_get_thread_name(pid_t pid, char *comm, int common_len)
{
	int main_thread_tid = 0;
	int ret;

	ret = liblinux_pal_query_main_thread(pid, &main_thread_tid, comm, common_len);
	if (ret != 0) {
		pr_err_ratelimited("query main thread failed, ret %d\n", ret);
	}
...
static void report_stats_packet(uid_t uid, pid_t pid)
{
	...
	if (ret == 0) {
		stat_logd("hook net packet from uid=%d, pid=%d", uid, pid);
		hw_packet_cb(uid, -1);
	} else {
...
#else
	if (skb_info.uid == 0U) {
		dubai_send_app_wakeup(skb, state->hook, state->pf, skb_info.uid, skb_info.pid);
	} else {
		dubai_send_app_wakeup(skb, state->hook, state->pf, skb_info.uid, skb_info.tgid);
	}
#endif
...
static int process_stats_show(struct seq_file *m, void *v)
{
	...
	list_for_each_entry(iface_entry, &iface_stat_list, list) {
		hash_for_each(iface_entry->proc_pid_stat_table, bkt, pid_entry, node) {
			...
			rcu_read_unlock();
#ifdef CONFIG_LIBLINUX
			if (strlen(pid_entry->comm) == 0) {
				procnet_liblinux_get_thread_name(pid_entry->pid, pid_entry->comm, TASK_COMM_LEN);
			}
			if (strncpy_s(comm, TASK_COMM_LEN, pid_entry->comm, TASK_COMM_LEN - 1) != 0) {
				stat_logi("Copy failed: pid: %d, comm: %s", pid_entry->pid, pid_entry->comm);
			}
#endif
...
/* hook anco net data */
static int __net_init hook_net_init(struct net *net)
{
    int err = 0;

    if (net == &init_net) {
        // in OH net namespace
        stat_logd("Operating in the init_net namespace");
        ...
        stat_logd("Operating in a non-init_net namespace, attempting to register hooks");

        err = nf_register_net_hooks(net, netstat_nf_hooks, ARRAY_SIZE(netstat_nf_hooks));
        if (err < 0) {
            stat_loge("Failed to register netfilter hooks for non-init_net namespace with error: %d", err);
			goto nf_register_hooks_err;
        } else {
            stat_logi("Successfully registered netfilter hooks for non-init_net namespace");
        }
    }
...
static void __net_exit hook_net_exit(struct net *net) {
    if (net != &init_net) {
        nf_unregister_net_hooks(net, netstat_nf_hooks, ARRAY_SIZE(netstat_nf_hooks));
    }
}

static struct pernet_operations hook_net_ops = {
    .init = hook_net_init,
    .exit = hook_net_exit,
};
...
static int __init procnetstat_init(void)
{
	...
	err = register_pernet_subsys(&hook_net_ops);
	if (err < 0) {
		stat_loge("register_pernet_subsys err");
		goto register_hooks_err;
	}
