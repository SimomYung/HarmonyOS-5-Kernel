...
static void fd_enable_set(struct color_chip *chip, UINT8 en)
{
	...
	hwlog_info("%s = %u atime = %d\n", __func__, en, ctx->alg_ctx.itime_ms);
	fd_enable_status = get_rgb_fd_enable_status(ctx->handle);
	if (en) {
		while (ctx->alg_ctx.itime_ms != AS7341_ITIME_DEFAULT) {
			cnt += EACH_TIME_WAIT_TIME;
			hwlog_info("%s: cnt = %d ctx->alg_ctx.itime_ms = %d\n", __func__, cnt, ctx->alg_ctx.itime_ms);
			mdelay(5); // wait 5ms every time
		...
		if ((fd_enable_status & FDEN) != FDEN) {
			...
			hwlog_info("%s: fd_enable 6ms for a gain quickly ctx->alg_ctx.itime_ms = %d\n", __func__, ctx->alg_ctx.itime_ms);
		} else {
