...
static bool g_is_root_ver = false; /* true-eng,false-user */

#define DTS_UPDATE_IN_AIR_CTRL "116x_update_in_air_ctrl"
#define arry_size(a) (sizeof(a) / sizeof((a)[0]))

static int __init early_parse_buildvariant_cmdline(char *p)
{
	if (p) {
		g_is_root_ver = (strncmp(p, "eng", 3) == 0) ? true : false;
		sparklink_errmsg("buildvariant = %s\n", g_is_root_ver ? "eng" : "user");
	}
 
	return 0;
}
early_param("buildvariant", early_parse_buildvariant_cmdline);
...
#else
	if ((!g_is_root_ver && ret >= di->version_app_reg.value) || (g_is_root_ver && ret == di->version_app_reg.value))
#endif
	{
...
static void hi1162_fwupdate_work(struct work_struct *work)
{
	...
	if (!work || !g_hi1162_info || !g_hi1162_info->client)
		return;
...
late_initcall(hi1162_module_init);
