...
static void hi1162_get_acc_dev_state(u8 state)
{
	...
	if (ret < 0)
		sparklink_errmsg("snprintf_s g_wltx_acc_info_tab failed\n");
...
static int hi1162_get_acc_info(struct hi1162_device_info *di, u8 *dev_no)
{
	...
	*dev_no = acc_info[HI1162_ACC_DEV_NO];
	...
	ret = snprintf_s(g_wltx_acc_info_tab[WL_TX_ACC_INFO_NO].value, ACC_VALUE_MAX_LEN, ACC_VALUE_MAX_LEN - 1,
	...
	ret = snprintf_s(g_wltx_acc_info_tab[WL_TX_ACC_INFO_MAC].value, ACC_VALUE_MAX_LEN, ACC_VALUE_MAX_LEN - 1,
	...
	if (ret < 0)
		sparklink_errmsg("snprintf_s mac_addr failed\n");

	/* the lenth of acc model id is 3 */
	ret = snprintf_s(g_wltx_acc_info_tab[WL_TX_ACC_INFO_MODEL_ID].value,
	...
	ret = snprintf_s(g_wltx_acc_info_tab[WL_TX_ACC_INFO_SUBMODEL_ID].value, ACC_VALUE_MAX_LEN, ACC_VALUE_MAX_LEN - 1,
	...
	ret = snprintf_s(g_wltx_acc_info_tab[WL_TX_ACC_INFO_VERSION].value, ACC_VALUE_MAX_LEN, ACC_VALUE_MAX_LEN - 1,
	...
	ret = snprintf_s(g_wltx_acc_info_tab[WL_TX_ACC_INFO_BUSINESS].value, ACC_VALUE_MAX_LEN, ACC_VALUE_MAX_LEN - 1,
	...
	ret = snprintf_s(g_wltx_acc_info_tab[WL_TX_ACC_INFO_SLE].value, ACC_VALUE_MAX_LEN, ACC_VALUE_MAX_LEN - 1,
...
void hi1162_wltx_get_rx_product_type(struct hi1162_device_info *di)
{
	...
	if (memset_s(di->rx_product_type, size, 0, size) != EOK) {
		sparklink_errmsg("memset_s error\n");
		return;
	}

	for (i = 0; i < sizeof(rx_product_type); i++) {
		ret = snprintf_s(di->rx_product_type + str_len, size - str_len, size - str_len - 1, "%02x", rx_product_type[i]);
		if (ret < 0)
			sparklink_errmsg("snprintf_s failed, byte: %d\n", i);
...
void hi1162_wltx_get_tx_info_block(struct hi1162_device_info *di, u16 *info, u8 reg)
{
	...
	*info = (tx_info[0] << BYTE_LEN) | tx_info[1];
