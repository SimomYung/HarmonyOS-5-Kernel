...
rszn_uwb_power_supply g_uwb_pmu_rszn = {NULL, NULL, NULL, NULL, NULL, 0, 0, 0, 0};
...
/*-------------------------------------------------------------------------*/
static ssize_t
rszn_sync(struct rszn_data *rszn, struct spi_message *message)
{
	int status;
...
rszn_sync_write(struct rszn_data *rszn, size_t len)
{
...
static inline ssize_t
rszn_sync_read(struct rszn_data *rszn, size_t len)
{
	struct spi_transfer	t = {
...
static ssize_t
rszn_read_payload(struct rszn_data *rszn, char __user *buf, size_t count) {
...
/* Read-only message with current device setup */
static ssize_t
rszn_read(struct file *filp, char __user *buf, size_t count, loff_t *f_pos)
{
...
static ssize_t
rszn_write(struct file *filp, const char __user *buf,
...
static inline void
rszn_message_construct_set(struct rszn_data *rszn, struct spi_transfer *k_tmp,
...
static inline int
rszn_message_construct(struct rszn_data *rszn, struct spi_ioc_transfer *u_xfers,
...
static int
rszn_message(struct rszn_data *rszn,
...
rszn_get_ioc_message(unsigned int cmd, struct spi_ioc_transfer __user *u_ioc,
	unsigned *n_ioc)
{
	u32 tmp;
...
static int
rszn_ioctl_rd_wait(struct rszn_data *rszn, unsigned long arg)
{
	int			 retval = 0;
	u32			 time;
	unsigned long   flags;
...
static int
rszn_ioctl_wr_wait(struct rszn_data *rszn, unsigned long arg)
{
	int			 	retval = UWB_SUCC;
	u32			 	time;
	unsigned long   flags;
...
static int
uwb_power_manage(struct rszn_data *spidev, rszn_uwb_power_supply uwbldo, char sw)
{
	int ret = 0;
...
static int
rszn_wakeup_cycle(struct rszn_data *rszn)
{
	pr_info("###%s: %d; rszn enter #### .... \n", __func__, __LINE__);
...
static int
rszn_power(struct rszn_data *rszn, int on)
{
	pr_info("###%s: %d; rszn enter #### .... \n", __func__, __LINE__);
...
static int
rszn_reset(struct rszn_data *rszn, u32 reset_flag)
{
	pr_info("###%s: %d; rszn enter #### .... \n", __func__, __LINE__);
...
static int
rszn_ioctl_wr_power(struct rszn_data *rszn, unsigned long arg)
{
	int retval;
	u32 tmp;
...
static int
rszn_ioctl_wr_reset(struct rszn_data *rszn, unsigned long arg)
{
	int retval = 0;
	u32 tmp;
...
static int
rszn_ioctl_wr_wakeup(struct rszn_data *rszn, unsigned long arg)
{
	int retval;
	u32 tmp;
...
static int
rszn_ioctl_upgrade_forced(struct rszn_data *rszn, unsigned long arg)
{
	int			 retval = UWB_SUCC;
...
static int
rszn_ioctl_wr_bits_per_word(struct rszn_data *rszn, struct spi_device *spi, unsigned long arg)
{
	int retval;
	u32 tmp;
...
static int
rszn_ioctl_wr_mode(struct rszn_data *rszn, struct spi_device *spi, unsigned int cmd, unsigned long arg)
{
	int retval;
	u32 tmp;
...
static int
rszn_ioctl_wr_lsb_first(struct rszn_data *rszn, struct spi_device *spi, unsigned long arg)
{
	int retval;
	u32 tmp;
...
static int
rszn_ioctl_wr_speed_hz(struct rszn_data *rszn, struct spi_device *spi, unsigned long arg)
{
	int retval;
	u32 tmp;
...
static int
rszn_ioctl_default(struct rszn_data *rszn, struct spi_device *spi, unsigned int cmd, unsigned long arg)
{
	int					 	retval;
...
static int
rszn_ioctl_switch_extend(struct rszn_data *rszn, struct spi_device *spi, unsigned int cmd, unsigned long arg)
{
	int retval;
...
static int
rszn_ioctl_switch(struct rszn_data *rszn, struct spi_device *spi, unsigned int cmd, unsigned long arg)
{
	int retval;
...
static long
rszn_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	int				 retval;
	struct rszn_data	*rszn;
	struct spi_device   *spi;
...
static void
rszn_buffer_kfree(struct rszn_data *rszn)
{
	if (!rszn->users) {
		if (rszn->tx_buffer) {
...
static int
rszn_buffer_kmalloc(struct rszn_data *rszn)
{
	int status = UWB_SUCC;
...
static int
rszn_open(struct inode *inode, struct file *filp)
{
	struct rszn_data	*rszn;
	int					status = -ENXIO;
...
static int
rszn_release(struct inode *inode, struct file *filp)
{
	struct rszn_data	*rszn;
	int					dofree;
	unused(inode);
...
static int
rszn_free(struct rszn_data *rszn)
{
	pr_info("###%s: %d; rszn enter #### .... \n", __func__, __LINE__);
...
static void
rszn_data_init(struct rszn_data *rszn, struct spi_device *spi)
{
...
static int
rszn_register_device(struct rszn_data *rszn)
{
	int status;
...
static int
rszn_get_dts_config(const char *node_name, const char *prop_name, char *out_str, int out_str_len)
{
...
static int
rszn_get_dts_config_u32(const char *node_name, const char *prop_name, u32 *pvalue)
{
	struct device_node *np = NULL;
	int ret = -1;
...
static ssize_t
rszn_chip_type_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	return snprintf_s(buf, sizeof(rszn_chip_type), sizeof(rszn_chip_type) - 1, "%s", rszn_chip_type);
}

static ssize_t
rszn_chip_type_store(struct device *dev, struct device_attribute *attr,
...
static int
rszn_create_sysfs_interfaces(struct device *dev)
{
	int i;
...
static int
rszn_remove_sysfs_interfaces(struct device *dev)
{
	int i;
...
static void
rszn_set_pmu38m_div_flag(void)
{
	int ret;

	ret = rszn_get_dts_config_u32("pmu_38mclk", "pmu_38mclk", &g_pmu38m_div_flag);
	if (ret != 0) {
		g_pmu38m_div_flag = 0;
...
static void
rszn_set_2v85gpio_flag(void)
{
	int ret;

	pr_info("###%s: rszn enter running line: %d #### .... \n", __func__, __LINE__);

	ret = rszn_get_dts_config_u32("2v85gpio_enbale", "2v85gpio_enbale", &g_2v85gpio_flag);
	if (ret != 0) {
		g_2v85gpio_flag = 0;
...
static void
rszn_set_pmu_32k_clk_name(void)
{
...
static int
rszn_create_device_attr(struct rszn_data *rszn)
{
	int status = UWB_FAIL;
...
static int
rszn_device_init(struct rszn_data *rszn, struct spi_device *spi)
{
	int status = 0;
...
static int
rszn_probe(struct spi_device *spi)
{
	struct rszn_data *rszn;
	int status;
...
static int
rszn_remove(struct spi_device *spi)
{
