...
static unsigned int g_write_idx = 0;
static char processed_ocp_error[OCP_INFO_ARRAY_LEN][OCP_LDO_NUM_LEN] = {0};
...
static bool is_processed_ocp(char *ldo_num)
{
    for (unsigned int index = 0; index < array_size(processed_ocp_error); index++) {
        if (strcmp(ldo_num, processed_ocp_error[index]) == 0) {
            print_info("This ocp error had been processed earlier, so ignore!\n");
            return true;
        }
    }
    return false;
...
static void process_ocp(struct bootfail_proc_param *pparam, int i)
{
	enum bootfail_errorcode errorcode;

	format_ocp_proc_param(pparam, &proc_ocp_param.ocp_info[i]);

	errorcode = boot_fail_error(pparam);
	if (errorcode == BF_OK) {
		int ret = strncpy_s(processed_ocp_error[g_write_idx], OCP_LDO_NUM_LEN, proc_ocp_param.ocp_info[i].binfo.ldo_num,
                            strlen(proc_ocp_param.ocp_info[i].binfo.ldo_num));
		if (ret == 0) {
			g_write_idx++;
		} else {
			print_err("ocp_hisi __boot_fail_process_ocp strncpy ldo num failed!\n");
		}
	}
}
...
static void __boot_fail_process_ocp(void)
{
	...
	for (i = 0; i < (int)array_size(proc_ocp_param.ocp_info); i++) {
		...
		if ((strcmp(proc_ocp_param.ocp_info[i].binfo.ldo_num, "") != 0) &&
			(!is_processed_ocp(proc_ocp_param.ocp_info[i].binfo.ldo_num)) && (g_write_idx < OCP_INFO_ARRAY_LEN)) {
