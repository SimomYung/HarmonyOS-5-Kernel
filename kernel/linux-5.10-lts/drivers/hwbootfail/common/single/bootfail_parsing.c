...
{
	for (unsigned int index = 0; index < ARRAY_SIZE(stage_map); index++) {
		if (stage_map[index].stage == stage_no) {
			strncpy_s(stage, BF_SIZE_128 - 1, stage_map[index].stage_dec, strlen(stage_map[index].stage_dec));
			return;
		}
	}
}
...
static void send_bbox_notify(struct error_info *info, char *stage, char *errnum)
{
	...
	strncpy_s((char *)info->top_category, CATEGORY_MAX_LEN - 1, stage, strlen(stage));
	strncpy_s((char *)info->error_desc, ERROR_DESC_MAX_LEN - 1, errnum, strlen(errnum));
...
static void append_log_path(struct bootfail_log_header *bf_bootfail_info, char *logs, int space)
{
	...
	for (int i = 0; i < (sizeof(bf_bootfail_info->log_path) / sizeof(bf_bootfail_info->log_path[0])); i++) {
...
{
	for (int i = strlen(log_path) - 1; i >= 0; --i) {
		if (log_path[i] == '/')
			return log_path + i;
	}
	return log_path;
							...
							char *errno, char *stage)
{
	...
	struct bootfail_log_content *log_content = NULL;
...
int parse_shutfail()
{
	...
	struct error_info info;
	struct shutfail_info sinfo;
	struct shutfail_log_header sf_log_header;

	memset_s(&info, sizeof(struct error_info), 0, sizeof(struct error_info));
	memset_s(&sinfo, sizeof(struct shutfail_info), 0, sizeof(struct shutfail_info));
	int ret = get_shut_header((struct shutfail_log_header*)&sf_log_header);
	if (ret != 0) {
	...
	char *klog = NULL;
	sf_log_header.kernel_log_size = min(BF_SIZE_512K, sf_log_header.kernel_log_size);
	klog = (char *)vmalloc(sf_log_header.kernel_log_size);
	if (klog == NULL) {
		print_err("malloc shutfail klog buffer failed!\n");
		return -1;
	}
	rtc_time_transfer((char *)sinfo.time, sf_log_header.sinfo.rtc_time, &info);
	ret = read_part(get_sfi_dev_path(), get_sfi_dev_offset() + sizeof(struct shutfail_log_header),
					klog, sf_log_header.kernel_log_size);
	if (ret != 0) {
	...
	ret = write_file(bf_info_path, bf_info_content, len);
	if (ret < 0)
		print_err("parse_shutfail: write bootfail info file failed!\n");

	ret = write_file(klog_path, klog, sf_log_header.kernel_log_size);
	if (ret < 0)
