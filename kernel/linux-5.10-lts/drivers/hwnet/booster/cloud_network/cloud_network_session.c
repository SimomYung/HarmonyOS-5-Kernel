...
void tcp_tx_packet_infor_update(const struct sock *sk, const size_t len)
{
	...
	struct sock *fullsk = NULL;
	int uid;
	if (!sk || !cloud_network_started())
		return;
	
	fullsk = sk_to_full_sk((struct sock *)sk);
	uid = fullsk->sk_uid.val;
...
static void session_statistics_print_rx_interval_each(struct session *ss, u32 *rx_interval, int len)
{
...
static void session_statistics_print_rx_interval(struct session *ss)
{
	...
	session_statistics_print_rx_interval_each(ss, ss->rx_interval, RX_INTERVAL_NUM);
	session_statistics_print_rx_interval_each(ss, ss->rx_interval_primary, RX_INTERVAL_NUM);
	session_statistics_print_rx_interval_each(ss, ss->rx_interval_secondary, RX_INTERVAL_NUM);
	session_statistics_print_rx_interval_each(ss, ss->rx_interval_wifi, RX_INTERVAL_NUM);
	return;
}
...
void start_all_session_print(void)
{
	...
	hrtimer_start(&all_session_print_timer,
		ms_to_ktime(ALL_SESSION_PRINT_MS), HRTIMER_MODE_REL_SOFT);
	return;
}
