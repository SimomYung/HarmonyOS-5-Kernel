...
struct dc_adpt_antifake_para {
	...
	u32 adp_antifake_recheck_key_index;
...
{
	int ret, key_index, len;
	int adp_code = 0;
	char auth_buf[POWER_EVENT_NOTIFY_SIZE] = { 0 };
	struct dc_adpt_dts_info *dts_info = dc_adpt_get_dts_info(mode);
	struct dc_adpt_inherent_para *para = dc_adpt_get_inherent_para();

	if (!dts_info || !para || para->adp_antifake_rechk_flag)
		return;

	key_index = (int)dts_info->antifake_para.adp_antifake_recheck_key_index;
	if (key_index <= 0) {
		hwlog_info("%s need not recheck, key_index=%d\n", __func__, key_index);
		return;
	}
	...
	ret = adapter_auth_encrypt_start(g_prot, key_index);
	if (ret) {
	...
	struct dc_adpt_antifake_para *para)
{
	...
	(void)power_dts_read_u32(power_dts_tag(HWLOG_TAG), np,
		"adp_antifake_recheck_key_index", &para->adp_antifake_recheck_key_index, 0);
