	...
	int *vbus_limit, int *ibus_limit)
{
	...
	if ((vbat <= di->vbat_protect_th) ||
		(soc <= di->soc_protect_th)) {
		di->is_soc_or_vbat_low = true;
		*vbus_limit = di->vbus_init;
		*ibus_limit = di->ibus_init;
		return false;
	}
...
static void rchg_quit_rscp(struct reverse_charge_device *di)
{
	if (di->is_mcu_in_scp_mode) {
		rprot_ic_reset();
		di->is_rscp_enble = false;
		di->is_mcu_in_scp_mode = false;
		(void)power_msleep(DT_MSLEEP_1S, 0, NULL);
	}
}

static void rchg_recover_rscp(struct reverse_charge_device *di)
{
	if (!di->is_rscp_enble) {
		rprot_enable_rscp(RSCP_EN);
		di->is_rscp_enble = true;
	}
}
...
static void rchg_control_work(struct work_struct *work)
{
	...
	if (!di || !di->rchg_status || di->is_soc_or_vbat_low) {
		hwlog_err("%s: di is null or rchg plug out or rchg status bad\n", __func__);
		return;
	}
	...
	if (!rchg_check_battery_status(di, &vbus_limit, &ibus_limit)) {
		...
		if (di->last_icon_type == ICON_TYPE_RCHG_SUPER)
			rchg_send_icon_uevent(ICON_TYPE_RCHG_NORMAL);
		rchg_quit_rscp(di);
		goto next_loop;
	}
	rchg_recover_rscp(di);
...
static int reverse_charge_start(struct reverse_charge_device *di)
{
	...
	if (hw_pd_is_reverse_charge_enable()) {
		...
		power_icon_notify(ICON_TYPE_RCHG_SUPER);
#endif
	} else {
		...
		if (!hw_pd_is_rev_sc_plugin())
			power_icon_notify(ICON_TYPE_RCHG_NORMAL);
#endif
		return ret;
	}
...
static void rchg_vol_test_work(struct work_struct *work)
{
	...
	(void)charge_set_hiz_enable(HIZ_MODE_ENABLE);
	(void)boost_set_idle_mode(BST_NORMAL_STATE);
	(void)boost_set_vcg_on(BST_VCG_ON);
	(void)boost_set_vbus(di->write_value);
	(void)boost_set_ibus(RCHG_DBC_IBUS);
	(void)boost_ic_enable(BST_EN);
...
static void rchg_test_exit_work(struct work_struct *work)
{
	...
	(void)boost_ic_enable(BST_DIS);
	(void)boost_set_idle_mode(BST_IDLE_STATE);
	(void)charge_set_hiz_enable(HIZ_MODE_DISABLE);
	di->write_value = 0;
