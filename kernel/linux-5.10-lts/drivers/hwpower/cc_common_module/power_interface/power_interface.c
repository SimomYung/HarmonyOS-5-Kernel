...
#define POWER_IF_VTERM_DEC_MAX_TH     400
...
static int power_if_set_vterm_dec(struct power_if_ops *ops, unsigned int value)
{
	if (value > POWER_IF_VTERM_DEC_MAX_TH) {
		hwlog_info("%s vaule=%u is invalid\n", __func__, value);
		return 0;
	}

	if (!ops->set_vterm_dec)
		return 0;

	return ops->set_vterm_dec(value);
}
	...
	unsigned int type, unsigned int sysfs_type, char *value_str)
{
	...
	case POWER_IF_SYSFS_BATT_ICHG_RATIO:
		...
		break;
	case POWER_IF_SYSFS_BATT_VTERM_DEC:
		return power_if_set_vterm_dec(l_ops, value);
	case POWER_IF_SYSFS_ENABLE:
		if (l_ops->set_enable)
