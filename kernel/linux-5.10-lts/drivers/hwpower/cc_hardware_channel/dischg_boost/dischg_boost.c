...
typedef int (*switch_handler)(struct dischg_boost_context *ctx);
...
static int dischg_boost_switch_to_boost_by_boost_5v(void)
{
	return boost_5v_enable(true, BOOST_CTRL_DISCHG_BOOST);
}

static int dischg_boost_switch_to_normal_by_boost_5v(void)
{
...
static int dischg_boost_switch_to_boost(struct dischg_boost_context *ctx)
{
	...
	switch (ctx->dischg_cfg.switch_mode) {
	case DISCHG_BOOST_SWITCH_MODE_GPIO:
		ret = dischg_boost_switch_to_boost_by_gpio(&ctx->dischg_cfg);
		break;
	case DISCHG_BOOST_SWITCH_MODE_BUCKBOOST:
		ret = dischg_boost_switch_to_boost_by_buckboost(&ctx->dischg_cfg);
		break;
	case DISCHG_BOOST_SWITCH_MODE_BOOST_5V:
		ret = dischg_boost_switch_to_boost_by_boost_5v();
		break;
	defalut:
		ret = -EPERM;
		break;
...
static int dischg_boost_switch_to_normal(struct dischg_boost_context *ctx)
{
	...
	switch (ctx->dischg_cfg.switch_mode) {
	case DISCHG_BOOST_SWITCH_MODE_GPIO:
		ret = dischg_boost_switch_to_normal_by_gpio(&ctx->dischg_cfg);
		break;
	case DISCHG_BOOST_SWITCH_MODE_BUCKBOOST:
		ret = dischg_boost_switch_to_normal_by_buckboost(&ctx->dischg_cfg);
		break;
	case DISCHG_BOOST_SWITCH_MODE_BOOST_5V:
		ret = dischg_boost_switch_to_normal_by_boost_5v();
		break;
	defalut:
		ret = -EPERM;
		break;
