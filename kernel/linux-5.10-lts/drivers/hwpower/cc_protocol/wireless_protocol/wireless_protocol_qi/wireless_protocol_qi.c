...
static void hwqi_handle_fsk_tx_version(unsigned int ic_type, u8 *pkt)
{
	u16 data;
	u8 header;
	unsigned int i;
...
static void hwqi_handle_fsk_tx_sn(unsigned int ic_type, u8 *pkt)
{
	u8 header;
	unsigned int i;
...
static void hwqi_handle_fsk_packet(u8 *pkt, int len, void *dev_data)
{
	...
	case HWQI_CMD_GET_TX_VERSION:
		...
		hwqi_handle_fsk_tx_version(ic_type, pkt);
		break;
	case HWQI_CMD_GET_TX_SN:
		hwlog_info("received cmd with tx sn\n");
		hwqi_handle_fsk_tx_sn(ic_type, pkt);
		break;
	default:
		break;
	}
