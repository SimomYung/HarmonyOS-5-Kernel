	...
	struct mmc_card *oldcard)
{
	...
	pr_info("%s: mmc init begin cmd0,caps is 0x%x\n", mmc_hostname(host), host->caps);

	mmc_go_idle(host);

	pr_info("%s: mmc init processing cmd1 MMC_SEND_OP_COND\n", mmc_hostname(host));
	...
	pr_info("%s: mmc init processing cmd2 MMC_ALL_SEND_CID\n", mmc_hostname(host));
	/*
	 * Fetch CID from card.
	 */
	...
	pr_info("%s: mmc init processing cmd3 MMC_SET_RELATIVE_ADDR\n", mmc_hostname(host));
	...
	pr_info("%s: mmc init processing cmd7 MMC_SELECT_CARD\n",  mmc_hostname(host));
	/*
...
static int _mmc_suspend(struct mmc_host *host, bool is_suspend)
{
	...
	if(mmc_status_check(EMMC_USE_FLAG) == SENSOR_USE_FLAG) {
		pr_err("%s: unable execute mmc suspend because sensorhub using mmc.\n", __func__);
		goto out;
	}
#endif
...
#ifdef CONFIG_ZODIAC_SHARE_MMC
	mmc_status_mark(EMMC_AP_POWER_ATTRIBUTE, AP_DONT_USE_FLAG);
#endif
...
#ifdef CONFIG_ZODIAC_SHARE_MMC
	if(mmc_status_check(EMMC_USE_FLAG) == SENSOR_USE_FLAG) {
		pr_err("%s: dont execute mmc resume because sensorhub using mmc.\n", __func__);
		goto out;
	}
#endif
