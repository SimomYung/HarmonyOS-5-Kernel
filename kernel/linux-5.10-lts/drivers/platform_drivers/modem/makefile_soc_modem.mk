...
MODEM_DRIVER_DRV := modem_driver_drv
MODEM_DRIVER_BUILD_PATH := drivers/platform_drivers/modem
$(MODEM_DRIVER_MODULE)-y :=


# overwrite obj-m
obj-$(CONFIG_MODEM_DRIVER)	+= $(MODEM_DRIVER_MODULE).o
obj-$(CONFIG_MODEM_DRIVER) += $(MODEM_DRIVER_DRV).o
...
$(MODEM_DRIVER_DRV)-y += $(addprefix drv/, $(drv-y))
obj-y	+= $(addprefix drv/, $(drv-builtin))

# ps
-include $(srctree)/drivers/platform_drivers/modem/ps/Makefile
$(MODEM_DRIVER_MODULE)-y += $(MODEM_DRIVER_DRV).o
$(MODEM_DRIVER_MODULE)-y += $(addprefix ps/, $(ps-objs))

lib-y += $(MODEM_DRIVER_DRV)_tmp.o
$(MODEM_DRIVER_BUILD_PATH)/$(MODEM_DRIVER_DRV)_tmp.o : $(MODEM_DRIVER_BUILD_PATH)/modules.order
	cp $(MODEM_DRIVER_BUILD_PATH)/$(MODEM_DRIVER_DRV).o $(MODEM_DRIVER_BUILD_PATH)/$(MODEM_DRIVER_DRV)_tmp.o
	sed -i "/drivers\/platform_drivers\/modem\/modem_driver_drv.ko/d" $(MODEM_DRIVER_BUILD_PATH)/modules.order
