...
int tc_ns_set_tavertion_ctrl(struct ta_ctrl_info *arg, unsigned int cmd_id)
{
	...
	struct ta_ctrl_info ioctl_arg = {NULL, 0};
	struct ta_ctrl_info *ioctl_argp = &ioctl_arg;
	if (copy_from_user(ioctl_argp, arg, sizeof(*ioctl_argp))) {
		tloge("copy from user failed\n");
		return -EFAULT;
	}
	uint32_t buf_len = ioctl_argp->size;
	...
	if (copy_from_user(buf_to_tee, ioctl_argp->buffer, buf_len)) {
		tloge("copy from user failed\n");\
		mailbox_free(buf_to_tee);
		return -EFAULT;
	}
