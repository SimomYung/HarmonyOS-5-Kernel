				...
				void __user *p)
{
	...
	struct usb_interface *intf;
	int i;
	for (i = 0; (dev->actconfig != NULL) && (i < dev->actconfig->desc.bNumInterfaces); ++i) {
		intf = dev->actconfig->interface[i];
		if (check_storage_rejects_status() && (intf != NULL) && (intf->cur_altsetting != NULL) &&
			(intf->cur_altsetting->desc.bInterfaceClass == USB_CLASS_MASS_STORAGE)) {
			dev_info(&dev->dev, "%s: devpath = %s bInterfaceClass %d is not authorized for access",
				__func__, dev->devpath, intf->cur_altsetting->desc.bInterfaceClass);
			return ret;
		}
	}
#endif
