...
static void *ffs_build_sg_list(struct sg_table *sgt, size_t sz)
{
	...
	if (trace_ldk_rvh_ffs_build_sg_list_enabled()) {
		trace_ldk_rvh_ffs_build_sg_list(sgt, sz, &vaddr);
		return vaddr;
	}
#endif
...
static inline void ffs_free_buffer(struct ffs_io_data *io_data)
{
	...
	if (io_data->use_sg) {
#ifdef CONFIG_LIBLINUX
		if (trace_ldk_rvh_ffs_free_buffer_enabled()) {
			trace_ldk_rvh_ffs_free_buffer(io_data->buf, &io_data->sgt);
			return;
		}
#endif
...
static void ffs_epfiles_destroy(struct ffs_epfile *epfiles, unsigned count)
{
	...
	for (; count; --count, ++epfile) {
		...
		if (epfile->dentry) {
			...
			trace_ldk_rvh_pdentry(epfile->dentry->d_sb ,epfile->dentry);
			dput(epfile->dentry);
			epfile->dentry = NULL;
		}
	}
