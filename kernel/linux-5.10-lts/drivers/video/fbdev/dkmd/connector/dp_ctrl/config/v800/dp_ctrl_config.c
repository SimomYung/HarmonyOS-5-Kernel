...
void enable_dptx_timing_gen(struct dp_ctrl *dptx)
{
	...
	dpu_pr_info("[DP] DPTX_GCTL0 = 0x%x, dptx timing gen enabled!", reg);
}
...
static uint64_t dp_pixel_ppll7_param_calc(uint64_t pixel_clock_cur, struct ppll7_cfg_param *cfg)
{
	...
	cfg->post_div = 1;
	if (pixel_clock_cur * (uint64_t)freq_divider_list[21] < VCO_MIN_FREQ_630_OUPUT ) {
		cfg->post_div = 2;
...
#ifdef DPU_NSV_SCOUT
    dpu_pr_debug("[DP] scout default support DP");
    return false;
#else
