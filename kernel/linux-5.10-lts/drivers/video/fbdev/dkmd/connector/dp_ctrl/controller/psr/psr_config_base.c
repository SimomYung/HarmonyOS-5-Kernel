...
void dptx_send_psr2_entry_event(struct dp_ctrl *dptx, uint32_t event)
{
	...
	dpu_pr_info("send psr2 entry event = %u!", event);
}
...
int dptx_psr_set_update_status(struct dp_ctrl *dptx, bool need_update)
{
	...
	if (need_update && dptx->psr_params.is_psr_active) {
...
void dptx_psr_update_handler(struct dp_ctrl *dptx)
{
	...
	if (is_dpu_pu_support(&pinfo->base) && dptx->psr_params.need_psr2_entry_event) {
		dptx_send_psr2_entry_event(dptx, 1);
		dptx->psr_params.need_psr2_entry_event = false;
	}

	if (dptx->psr_params.is_psr_update) {
		dpu_pr_debug("[DP] PSR update start");
		(void)dptx_change_psr2_status(dptx, PSR_UPDATE);
	} else if (dptx->psr_params.is_psr2_su_update) {
		dpu_pr_debug("[DP] PSR2 SU update start");
		(void)dptx_change_psr2_status(dptx, PSR2_SU_UPDATE);
	} else {
...
static int32_t dptx_psr_entry_isr_handle(struct notifier_block *self, unsigned long action, void *data)
{
	...
	if (dptx->psr_params.is_psr_entry)
		dptx->psr_params.non_psr_frame_idx += 1;
	dpu_pr_debug("[DP] non_psr_frame_idx is %d", dptx->psr_params.non_psr_frame_idx);

	if (dptx->psr_params.is_psr_entry && dptx->psr_params.non_psr_frame_idx == 5) {
		dpu_pr_debug("[DP] [DP-IRQ] Set PSR entry");
		dptx->psr_params.is_psr_entry = false;
		(void)dptx_change_psr2_status(dptx, PSR_ENTRY);
	}
