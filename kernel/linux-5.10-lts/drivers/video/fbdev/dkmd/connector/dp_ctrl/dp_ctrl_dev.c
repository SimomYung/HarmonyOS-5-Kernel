...
void dp_send_cable_notification(struct dp_ctrl *dptx, int val)
{
	...
	if (sprintf_s(dptx->audio_name, sizeof(dptx->audio_name), "%s,device_port=%s-%d",
		dptx->audio_pre_name, vendor_name, dptx->port_id) <= 0) {
		dpu_pr_err("[DP] audio_name create failed");
		return;
	}
	dpu_pr_info("[DP] audio_name is %s", dptx->audio_name);
...
static int32_t set_dptx_display_timing(struct dkmd_connector_info *pinfo, struct edid_support_timing *user_timing)
{
	...
	dpu_pr_info("[DP] dptx set display resolution [%u * %u@%uHz] start!", user_timing->hactive, user_timing->vactive,
		user_timing->fps);
	if (dptx->dptx_check_user_timing(dptx, user_timing) != 0)
		return -EINVAL;
...
bool dp_get_enable_status(struct dkmd_connector_info *pinfo)
{
	struct dp_ctrl *dptx = NULL;

	dptx = dp_get_dptx(pinfo);
	if (!dptx || !dptx->dptx_enable || dptx->power_saving_mode) {
		dpu_pr_err("[DP] dptx is not exist or has already been off!");
		return false;
	}
