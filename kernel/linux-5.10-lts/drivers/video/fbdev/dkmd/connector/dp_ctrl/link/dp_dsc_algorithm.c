...
int dptx_dsc_initial(struct dp_ctrl *dptx)
{
	...
	if (dptx->dsc && (byte & DP_DSC_DECOMPRESSION_IS_SUPPORTED)) {
		...
		dpu_pr_info("[DP] dptx_dsc_initial dp_mode: %d, bpc=%d", dptx->connector->conn_info->base.mode, dptx->vparams.bpc);
	} else {
		dpu_pr_info("[DP] DSC disable! retval=%d", byte);
		dptx->dsc = false;
	}
	...
	if (retval)
		return retval;
	if ((dptx->fec != 0) && ((byte & DP_FEC_CAPABLE) != 0) && (dptx->connector->conn_info->base.mode == DP_MODE)) {
		dpu_pr_info("[DP] FEC enable! retval=%d", byte);
		dptx->fec = true;
	} else {
		dptx->fec = false;
	}
