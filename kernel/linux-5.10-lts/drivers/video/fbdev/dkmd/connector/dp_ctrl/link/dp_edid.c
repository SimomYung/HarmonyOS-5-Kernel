...
static void copy_edid_info_to_comp(struct dp_ctrl *dptx)
{
	...
	if (dptx->edid_info.video.dptx_timing_list != NULL) {
		list_for_each_entry_safe(dptx_timing_node, _node_, dptx->edid_info.video.dptx_timing_list, list_node) {
			...
			dpu_pr_info("[DP] copy display info to composer: h:%d, v:%d, f:%d", pinfo->base.edid_timing[i].hactive,
...
int parse_edid(struct dp_ctrl *dptx, uint16_t len)
{
	...
	dpu_pr_debug("[DP] ext_block_num is %d, len is %d", ext_block_num, len);
	...
	es_timing_num = parse_established_timing(dptx);
	dpu_pr_info("total established timing num:%d\n", es_timing_num);
...
int dptx_calc_fps(struct timing_info *dptx_timing_node, uint32_t *fps)
{
	...
	*fps = (uint32_t)(dptx_timing_node->pixel_clock * 10000 / pixels);
	dpu_pr_debug("[DP] dptx_calc_fps fps:%d, pixels:%d, pixel_clock:%d", *fps, pixels, dptx_timing_node->pixel_clock);
...
static int parse_establish_timing_description(struct dp_ctrl *dptx, const struct dtd *edid_timing)
{
	...
	dpu_pr_info("[DP] parse_establish_timing_description start");
	vid_info = &(dptx->edid_info.video);
...
static int check_exten_edid_v21(const uint8_t *exten)
{
	...
	if (dtd_total != EDID_LENGTH - 2 - 5) {
		dpu_pr_err("[DP] dtd_total err, dtd_total: %d", dtd_total);
		return -EINVAL;
	}
...
int parse_extension_v21(struct dp_ctrl *dptx, uint8_t *exten)
{
	...
	while (dts_data_block_position < dtd_total + 5 - 2 && exten[dts_data_block_position] != 0) {
		...
		if (exten[dts_data_block_position + 2] == 0x0) {
			dpu_pr_err("[DP] dts_data_block_position is end, P: %d", dts_data_block_position);
			break;
		}
		dpu_pr_info("[DP] continue dts_data_block_position: %d, size: 0x%x", dts_data_block_position,
	...
	bool is_native)
{
	...
	if (!convert_code_to_dtd(&mdtd, vesa_id, 60000, VCEA)) {
		dpu_pr_info("[DP] Invalid video mode value %d", vesa_id);
		return -EINVAL;
	}
...
int parse_monitor_limits(struct dp_ctrl *dptx, uint8_t *block)
{
	...
	dpu_pr_info("[DP] parse_monitor_limits max_pixel_clock is %d", vid_info->max_pixel_clock);
...
bool convert_code_to_dtd(struct dtd *mdtd, uint8_t code, uint32_t refresh_rate, uint8_t video_format)
{
	...
	*mdtd = handle_dtd->support_modes_dtd[code];
	dpu_pr_info("[DP] convert_code_to_dtd, pixel_clock is %d, code: %d, ha: %d, va: %d",
...
bool convert_code_to_dtd_by_user(struct dp_ctrl *dptx, struct dtd *mdtd, uint8_t *fps)
{
	...
	pinfo = dptx->connector->conn_info;
	cnt = (uint8_t)pinfo->base.edid_timing_num;
	for (i = 0; i < cnt; i++) {
		dpu_pr_debug("[DP]pinfo timing is, fps:[%d], ha:[%d], va[%d]",
			pinfo->base.edid_timing[i].fps, pinfo->base.edid_timing[i].hactive, pinfo->base.edid_timing[i].vactive);
		pixels = (pinfo->base.edid_timing[i].hactive + pinfo->base.edid_timing[i].hblanking) *
			(pinfo->base.edid_timing[i].vactive + pinfo->base.edid_timing[i].vblanking);
		if (pixels == 0) {
			dpu_pr_err("[DP] pixels cannot be zero");
			return false;
		}
		*fps = (uint8_t)(pinfo->base.edid_timing[i].fps);
		if (pinfo->base.edid_timing[i].hactive == dptx->user_timing->hactive
				&& pinfo->base.edid_timing[i].vactive == dptx->user_timing->vactive
				&& *fps <= dptx->user_timing->fps + 1 && *fps >= dptx->user_timing->fps - 1) {
			mdtd->h_active = pinfo->base.edid_timing[i].hactive;
			mdtd->h_blanking = pinfo->base.edid_timing[i].hblanking;
			mdtd->h_sync_offset = pinfo->base.edid_timing[i].hsync_offset;
			mdtd->h_sync_pulse_width = pinfo->base.edid_timing[i].hsync_pulse_width;
			mdtd->h_image_size = pinfo->base.edid_timing[i].hsize;
			mdtd->h_sync_polarity = pinfo->base.edid_timing[i].hsync_polarity;
			mdtd->v_active = pinfo->base.edid_timing[i].vactive;
			mdtd->v_blanking = pinfo->base.edid_timing[i].vblanking;
			mdtd->v_sync_offset = pinfo->base.edid_timing[i].vsync_offset;
			mdtd->v_sync_pulse_width = pinfo->base.edid_timing[i].vsync_pulse_width;
			mdtd->v_image_size = pinfo->base.edid_timing[i].vsize;
			mdtd->v_sync_polarity = pinfo->base.edid_timing[i].vsync_polarity;
			mdtd->pixel_clock = pinfo->base.edid_timing[i].pixel_clock * 10;
			mdtd->interlaced = pinfo->base.edid_timing[i].interlaced;
			mdtd->pixel_repetition_input = 0;
			break;
		}
	}
	dpu_pr_info("[DP] convert_code_to_dtd by user, pixel_clock:[%d], fps:[%d], ha:[%d], va[%d]",
		mdtd->pixel_clock, *fps, mdtd->h_active, mdtd->v_active);

	vary_vfp(mdtd, (uint8_t)i, (uint32_t)*fps);
