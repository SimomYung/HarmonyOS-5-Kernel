...
static bool dptx_check_priority_timing(struct dp_ctrl *dptx, struct timing_info *dptx_timing_node)
{
...
static int dptx_choose_edid_timing(struct dp_ctrl *dptx, bool *bsafemode)
{
	...
	list_for_each_entry_safe(dptx_timing_node, _node_, dptx->edid_info.video.dptx_timing_list, list_node) {
		...
		if (!dptx_check_priority_timing(dptx, dptx_timing_node))
			continue;
...
void dptx_link_params_reset(struct dp_ctrl *dptx)
{
	...
	dpu_pr_info("[DP] dptx_link_params_reset set fec enable:%d dsc enable %d ", dptx->fec ,dptx->dsc);
...
static int dptx_link_psr_init(struct dp_ctrl *dptx)
{
	...
	if (dptx->connector->conn_info->base.mode == EDP_MODE) {
		...
		if ((reg == DP_PSR_IS_SUPPORTED || dptx->psr_params.psr_version == DP_PSR_IS_SUPPORTED)
			...
			dpu_pr_info("[DP] psr2/psr NOT SUPPORTED, psr_version:%d\n", dptx->psr_params.psr_version);
		}
	}
	return 0;
}
...
int dptx_link_choose_timing(struct dp_ctrl *dptx, bool bsafe_mode, int edid_info_size)
{
	...
	if ((bsafe_mode) || (dptx->connector->conn_info->base.fpga_flag != 0)) {
		dpu_pr_info("[DP] bsafe_mode:%d fpga_flag : %d",bsafe_mode, dptx->connector->conn_info->base.fpga_flag);
...
static void dptx_psr2_entry(struct dp_ctrl *dptx)
{
