...
int dpu_blank_peri_handle(int blank_mode)
{
	return dpu_aod_panel_handle(blank_mode);
}
...
int dpu_sensorhub_aod_dvfs_vote(uint32_t seq_num)
{
	...
	*/
	...
	value = inp32(SOC_SCTRL_SCBAKDATA11_ADDR(sctrl_base));
	core_val = ((value >> SH_VOTE_BIT) & 0x3);
	dpu_pr_debug("[dvfs]:start vote the value is %u, vote seq num: %u\n",
		core_val, seq_num);
	dpu_dvfs_direct_vote(dpu_comp->comp.index, min(DPU_PERF_LEVEL_MAX, (core_val + 1)), true);
