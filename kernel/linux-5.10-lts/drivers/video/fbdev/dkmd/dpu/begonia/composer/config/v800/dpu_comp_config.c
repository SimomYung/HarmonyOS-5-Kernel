...
{
	struct dkmd_connector_info *pinfo = bl_ctrl->conn_info;
	...
	if (dfr_info->dfr_mode != DFR_MODE_TE_SKIP_BY_MCU && dfr_info->dfr_mode != DFR_MODE_LONGH_TE_SKIP_BY_MCU)
		return pipeline_next_ops_handle(pinfo->conn_device, pinfo, SET_BACKLIGHT, &bl_ctrl->bl_level);
	else
...
void dpu_print_vsync_connect_info(struct dpu_connector *connector) 
{
...
{
	char __iomem *dpu_base;
	char __iomem *media1_ctrl_base = dpu_comp->comp_mgr->media1_ctrl_base;
...
bool dpu_bl_check_workqueue_level(struct dkmd_connector_info *conn_info, uint32_t bl_lvl)
{
	void_unused(bl_lvl);
	void_unused(conn_info);
	return false;
}
