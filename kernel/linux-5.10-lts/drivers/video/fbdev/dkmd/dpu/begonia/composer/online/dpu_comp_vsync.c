...
uint32_t g_idle_enter_vote_level = 0;

uint32_t get_enter_idle_level(void)
{
	return g_idle_enter_vote_level;
}
...
static void dpu_comp_process_vsync_idle(struct dpu_composer *dpu_comp, struct dkmd_connector_info *pinfo)
{
	...
	if (((pinfo->vsync_ctrl_type & VSYNC_IDLE_PSR2_CMD_MODE) != 0) &&
		...
		if (!is_dp_primary_panel(&dpu_comp->comp.base)) {
			dpu_pr_warn("not dp primary panel, can not enter idle");
			return;
		}
		// check is psr2 active mode.
		if (!is_edp_cmd_mode(pinfo)) {
			dpu_pr_warn("psr not active, can not enter idle");
			return;
		}
		secure_ctrl = &dpu_comp->secure_ctrl;
		if (secure_ctrl != NULL && secure_ctrl->secure_status == TUI_SEC_RUNNING) {
			dpu_pr_warn("tui mode can not enter idle");
			return;
		}
	...
	struct dpu_vsync *vsync_ctrl, struct dkmd_connector_info *pinfo)
{
	...
	if (((dpu_idle_status & VSYNC_IDLE_PSR2_CMD_MODE) != 0) &&
		((comp_mgr->idle_func_flag & VSYNC_IDLE_PSR2_CMD_MODE) == 0)) {
			g_idle_enter_vote_level = dpu_dvfs_get_final_vote_level();
...
void dpu_comp_active_vsync(struct dpu_composer *dpu_comp)
{
	...
	if ((comp_mgr->idle_func_flag & VSYNC_IDLE_PSR2_CMD_MODE) != 0 && is_dp_primary_panel(&dpu_comp->comp.base)) {
		dpu_pr_info("psr update, exit idle mode");
		dpu_dvfs_direct_vote(dpu_comp->comp.index, g_idle_enter_vote_level, true);
		dpu_ddr_bandwidth_recovery(!is_offline_panel(&pinfo->base));
		comp_mgr->idle_func_flag &= ~VSYNC_IDLE_PSR2_CMD_MODE;
...
static void check_vsync_is_correct(uint64_t real_vsync, struct dpu_vsync *vsync_ctrl)
{
	...
	struct dpu_composer *dpu_comp = NULL;
	dpu_comp_status_t *dimming_status = NULL;
	present = (struct comp_online_present *)vsync_ctrl->dpu_comp->present_data;
	if (unlikely(present == NULL)) {
		dpu_pr_err("present is null");
		return;
	...
	dimming_status = &dpu_comp->comp_mgr->dimming_status;
	dpu_check_and_no_retval(dpu_comp_status_is_disable(dimming_status), debug, "dimming status is off");
	...
	if (present->dfr_ctrl.cur_frm_rate != 0) {
		...
		if (vsyncdiff > VSYNC_MAX_TIME_OFFSET) {
			...
			dpu_print_vsync_connect_info(connector);
		} else {
