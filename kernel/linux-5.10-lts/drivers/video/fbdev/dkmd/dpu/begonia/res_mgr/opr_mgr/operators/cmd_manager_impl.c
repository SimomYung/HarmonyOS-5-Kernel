...
{
	frame->scene_cmdlist_id = cmdlist_create_user_client(dpu_res_get_cmdlist_dev_id(),
...
{
	network->reg_cmdlist_id = cmdlist_create_user_client(dpu_res_get_cmdlist_dev_id(),
...
{
	if (unlikely((cmdlist_append_client(dpu_res_get_cmdlist_dev_id(),
...
{
	cmdlist_flush_client(dpu_res_get_cmdlist_dev_id(), (uint32_t)frame->scene_id, network->dm_cmdlist_id);

	if (network->is_last_block)
		cmdlist_flush_client(dpu_res_get_cmdlist_dev_id(), (uint32_t)frame->scene_id, frame->scene_cmdlist_id);
}
	...
	union dkmd_opr_id cur_opr_id)
{
	...
	cmd_data->data->dm_param =
		(struct dpu_dm_param *)cmdlist_get_payload_addr(dpu_res_get_cmdlist_dev_id(),
			(uint32_t)frame->scene_id, network->dm_cmdlist_id);
...
void set_frame_cmd_data(const struct dkmd_base_frame *frame, const struct dkmd_network *network)
{
	...
	for (i = 0; i < DPU_DBUF_REG_NUM; i++)
		ukmd_set_reg(dpu_res_get_cmdlist_dev_id(),
	...
	for (i = 0; i < dyn_config.config_cnt; i++)
		ukmd_set_reg(dpu_res_get_cmdlist_dev_id(), (uint32_t)frame->scene_id, network->reg_cmdlist_id,
