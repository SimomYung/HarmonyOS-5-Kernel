...
int32_t cmdlist_client_table_setup(struct cmdlist_table *cmd_table)
{
	...
	if (chash_is_valid(&cmd_table->ht)) {
		ukmd_pr_info("bkts is already initialized!");
		return ret;
	}
 ...
 * @param cmd_table cmdlist client table
 */
void cmdlist_client_table_release(struct cmdlist_table *cmd_table, uint32_t dev_id)
{
	if (unlikely(!cmd_table)) {
		ukmd_pr_err("input error, cmd_table is null pointer!\n");
		return;
 ...
 * @param cmd_table cmdlist client table
 */
void cmdlist_client_table_clear(struct cmdlist_table *cmd_table, uint32_t dev_id)
{
	if (unlikely(!cmd_table)) {
		ukmd_pr_err("input error, cmd_table is null pointer!\n");
		return;
	...
	chash_clear(&cmd_table->ht, cmdlist_client_release_node, (void *)&dev_id);
}
...
int32_t cmdlist_create_client(struct cmdlist_table *cmd_table, struct cmdlist_node_client *user_client)
{
	...
	struct cnode *list;
	struct cnode *c_node = NULL, *tmp = NULL;
	struct cmdlist_client *err_client = NULL;
	uint32_t total_scene_num;
	uint32_t cur_scene_num;
	...
	if (unlikely(!node)) {
		...
		for (i = 0; i < cmd_table->scene_num; i++) {
			list = &cmd_table->ht.bkts[i % cmd_table->ht.bkt_size];
			total_scene_num = 0;
			c_node = NULL;
			tmp = NULL;
			clist_for_each_saft(c_node, tmp, list) {
				total_scene_num++;
			}
			if (total_scene_num < 200)
				continue;
			c_node = NULL;
			tmp = NULL;
			cur_scene_num = 0;
			cmdlist_dev_id = cmdlist_get_dev_id_by_name(DPU_CMDLIST);
			clist_for_each_saft(c_node, tmp, list) {
				err_client = cnode_entry(c_node, struct cmdlist_client, hash_node);
				cur_scene_num++;
				if (cur_scene_num > total_scene_num / 2)
					continue;
				ukmd_pr_err("err_client->scene_id: %u, err_client->key %u, err_client->node_type %u",
					err_client->scene_id, err_client->key, err_client->node_type);
				cmdlist_client_release_node(c_node, (void *)&cmdlist_dev_id);
			}
		}
		return -1;
	}
	...
	ukmd_pr_debug("create node[%pK].scene[%d].mode[%d].size[%#x].prev=%pK.next=%pK.list_cmd_header=%pK.fence=%pK",
...
int32_t cmdlist_release_client_or_list(struct cmdlist_table *cmd_table, struct cmdlist_node_client *user_client)
{
	...
	if (unlikely(!header) || unlikely(!header->node)) {
		ukmd_pr_err("invalid scene_id:%u, cmdlist_id:%u!", user_client->scene_id, user_client->id);
		return -1;
	}

	if (header->node_type != SCENE_NOP_TYPE) {
		ukmd_pr_warn("--> release client<%u, %pK, %u>\n", header->key, header->node, header->node_type);
		cmdlist_client_release_node(&header->hash_node, (void *)&user_client->dev_id);
		return 0;
	}
...
static void cmdlist_hash_node_dump(struct cnode *node)
{
	...
	ukmd_pr_debug("--> dump client <%u, %pK, %u>", client->key, client->node, client->node_type);
	ukmd_pr_debug("dump node[%pK].scene[%d].mode[%d].size[%#x].prev=%pK.next=%pK.list_cmd_header=%pK.fence=%pK",
		client->node, client->node->scene_id, client->node->cmd_mode, client->node->node_size,
		client->node->cnode.prev, client->node->cnode.next,
		client->node->list_cmd_header, client->node->fence);
	cmdlist_dump_node(client->node);
