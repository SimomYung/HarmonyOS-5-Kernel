...
{
	node->list_cmd_header = (struct cmd_header *)(uintptr_t)cmdlist_mem_alloc(dev_id, size,
	...
	if ((node->node_size != 0) && (size > node->node_size)) {
		...
		cmdlist_mem_free(dev_id, node->list_cmd_header, (uint32_t)node->node_size);
	...
	ukmd_pr_debug("alloc node list_cmd_header[%#x].node_size[%zu]",
