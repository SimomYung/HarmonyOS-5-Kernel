...
{
	ukmd_pr_debug("release fence %s seqno:%d\n",
...
{
	ukmd_pr_debug("drv:%s timeline:%s seqno:%d signaled",
		fence->ops->get_driver_name(fence),
...
int32_t ukmd_acquire_fence_wait(struct dma_fence *fence, long timeout)
{
	...
	if (rc > 0) {
		ukmd_pr_debug("signaled drv:%s timeline:%s seqno:%d\n",
			fence->ops->get_driver_name(fence),
			fence->ops->get_timeline_name(fence),
			fence->seqno);
		rc = 0;
	} else if (rc == 0) {
		ukmd_pr_err("timeout drv:%s timeline:%s seqno:%d.\n",
			fence->ops->get_driver_name(fence),
			fence->ops->get_timeline_name(fence),
			fence->seqno);
		rc = -ETIMEDOUT;
...
void ukmd_acquire_fence_signal_release(struct dma_fence *fence)
{
	...
	if (!dma_fence_is_signaled_locked(fence))
		ukmd_pr_info("no signal drv:%s timeline:%s seqno:%d, but release!\n",
			fence->ops->get_driver_name(fence),
			fence->ops->get_timeline_name(fence),
			fence->seqno);
...
int32_t ukmd_acquire_fence_signal(int32_t fence_fd)
{
	...
	if (dma_fence_is_signaled_locked(fence)) {
		...
		ukmd_pr_info("signal err drv:%s timeline:%s seqno:%d\n",
			fence->ops->get_driver_name(fence),
			fence->ops->get_timeline_name(fence),
			fence->seqno);
		rc = -1;
