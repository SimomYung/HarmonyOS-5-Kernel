...
#define pd_dpm_support_voltage_up hw_pd_support_voltage_up
...
#define hw_pd_is_reverse_charge_enable() hw_pd_get_reverse_charge_state()
#endif

#ifdef CONFIG_USB_HW_F_ACM_MUSL
void hw_pd_get_raw_pdo(uint32_t *raw_pdo, size_t size);
#else
inline void hw_pd_get_raw_pdo(uint32_t *raw_pdo, size_t size)
{
}
