...
struct btf_dev {
	...
	int wldc_pwr_limit;
	int batt_id;
	char replace_sensor[THERMAL_NAME_LENGTH];
	bool ready;
	bool dmd_reported;
...
#else
static inline bool btf_temp_fitted_eqr(int batt_id)
{
	return false;
}

static inline int btf_try_fit(int temp_in, int batt_id)
{
	return temp_in;
}
