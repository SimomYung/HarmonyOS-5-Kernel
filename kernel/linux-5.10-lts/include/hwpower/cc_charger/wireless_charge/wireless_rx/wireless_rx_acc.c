...
static int wlrx_acc_parse_trx_plim_int_array(const char* tmp_str, struct wlrx_acc_trx_plim_cfg *trx_plim_cfg, int index, int col)
{
	int delimiter_cnt;
	...
	case WLRX_ACC_TRX_PLIM_TX_TYPE:
		...
		if (!(trx_plim_cfg[index].tx_type) || power_str2intArray(tmp_str, '|', trx_plim_cfg[index].tx_type, delimiter_cnt + 1))
			return -EINVAL;
	...
	case WLRX_ACC_TRX_PLIM_ADAP_TYPE:
		...
		if (!(trx_plim_cfg[index].adap_type) || power_str2intArray(tmp_str, '|', trx_plim_cfg[index].adap_type, delimiter_cnt + 1))
			return -EINVAL;
	...
	case WLRX_ACC_TRX_PLIM_SCN_ID:
		...
		if (!(trx_plim_cfg[index].scn_id) || power_str2intArray(tmp_str, '|', trx_plim_cfg[index].scn_id, delimiter_cnt + 1))
			return -EINVAL;
	...
	case WLRX_ACC_TRX_PLIM_EPT_TYPE:
		...
		if (!(trx_plim_cfg[index].ept_type) || power_str2intArray(tmp_str, '|', trx_plim_cfg[index].ept_type, delimiter_cnt + 1))
			return -EINVAL;
...
static int wlrx_acc_parse_trx_plim_int(const char* tmp_str, struct wlrx_acc_trx_plim_cfg *trx_plim_cfg, int index, int col)
{
	int tmp = 0;
