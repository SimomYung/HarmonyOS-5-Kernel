...
static void wltx_cc_time_monitor(struct wltx_cc_dev_info *di)
{
	...
	if (di->cfg.time_alarm[i].alarm.reserved > 0)
		wltx_ic_set_ilim(WLTRX_IC_MAIN, di->cfg.time_alarm[i].alarm.reserved);
}
	...
	const unsigned int delay)
{
	...
	if (!di || !cfg || ((wltx_cap_get_cur_id(WLTRX_DRV_MAIN) != WLTX_HIGH_PWR2_CAP) &&
		(wltx_cap_get_cur_id(WLTRX_DRV_MAIN) != WLTX_HIGH_PWR_CAP)))
		return;
