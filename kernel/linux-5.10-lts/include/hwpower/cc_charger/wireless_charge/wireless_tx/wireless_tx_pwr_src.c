...
static enum wltx_pwr_src wltx_set_otg_output_enable(int otg_volt)
{
	int i;
	int vout = 0;
	int vout_l = (otg_volt == OTG_VOLTAGE_9V) ? WLTX_9VOTG_VOUT_LTH : WLTX_OTG_VOUT_LTH;
	int vout_h = (otg_volt == OTG_VOLTAGE_9V) ? WLTX_9VOTG_VOUT_HTH : WLTX_OTG_VOUT_HTH;
	struct wltx_dev_info *di = wltx_get_dev_info();

	if (!di)
		return PWR_SRC_NULL;

	wltx_otg_output_set_wired_channel(true);
	charger_select_channel(CHARGER_CH_WLSIN);
	(void)charge_otg_mode_with_volt_enable(true, otg_volt, VBUS_CH_USER_WL_TX);
	...
	for (i = 0; i < 10; i++) {
		...
		if ((vout >= vout_l) && (vout < vout_h)) {
			if (di->rxsw_ctrl_ovp)
				wlps_control(WLTRX_IC_MAIN, WLPS_RX_SW, true);
			charge_pump_chip_enable(CP_TYPE_MAIN, true);
			return (otg_volt == OTG_VOLTAGE_9V) ? PWR_SRC_9VOTG : PWR_SRC_OTG;
		}
	}

fail:
	(void)charge_otg_mode_with_volt_enable(false, otg_volt, VBUS_CH_USER_WL_TX);
	charger_select_channel(CHARGER_CH_USBIN);
	wltx_otg_output_set_wired_channel(false);
	return PWR_SRC_NULL;
}

static enum wltx_pwr_src wltx_set_otg_output(bool enable, int otg_volt)
{
	int i;
	...
	return wltx_set_otg_output_enable(otg_volt);
}
...
static enum wltx_pwr_src wltx_set_otg_cp_output(bool enable)
{
	...
	if (!enable)
		return wltx_set_otg_output(false, OTG_VOLTAGE_5V);

	if (di->ps_need_ext_pwr)
		wlps_control(WLTRX_IC_MAIN, WLPS_RX_EXT_PWR, true);

	src = wltx_set_otg_output(true, OTG_VOLTAGE_5V);
	if (src != PWR_SRC_OTG)
		goto fail;
...
enum wltx_pwr_src wltx_set_pwr_src_output(bool enable, enum wltx_pwr_src src)
{
	...
	case PWR_SRC_OTG:
		return wltx_set_otg_output(enable, OTG_VOLTAGE_5V);
	case PWR_SRC_5VBST:
	...
	case PWR_SRC_9VOTG:
		return wltx_set_otg_output(enable, OTG_VOLTAGE_9V);
	case PWR_SRC_NULL:
		return PWR_SRC_NULL;
	default:
		return PWR_SRC_NA;
