...
static int bk96285_fw_check_mtp_crc(struct bk96285_dev_info *di, u8 margin, const u8 *buf)
{
	int i, ret;
	u32 data32;
	u32 read_crc = 0;
	...
	for (i = 0; i < (BK96285_CHIP_SIZE / BK96285_PGM_WORD); i++)
		data32 = bk96285_func_crc32(*(u32 *)(buf + i * BK96285_PGM_WORD), data32);
...
static int bk96285_fw_program(struct bk96285_dev_info *di, const u8 *buf, u32 len)
{
	bk96285_pgm_chip_erase(di);

	return bk96285_pgm_write(di, 0, buf, (u16)len, BK96285_PGM_CMD_WRITE_CODE);
}

static int bk96285_fw_program_mtp(struct bk96285_dev_info *di)
{
	int ret;
	u32 firmware_length = BK96285_FW_MTP_SIZE;
	const u8 *firmware_buf = g_bk96285_fw_mtp;
	...
	ret = bk96285_fw_program(di, firmware_buf, firmware_length);
	if (ret < 0) {
		hwlog_err("fw_program: fail\n");
		goto program_fail;
	}

	ret = bk96285_fw_check_mtp_crc(di, BK96285_CRC_CHECK_MARGIN0, firmware_buf);
	if (ret < 0) {
		hwlog_err("check_mtp_crc: margin 0 check crc fail\n");
		goto program_fail;
	}

	ret = bk96285_fw_check_mtp_crc(di, BK96285_CRC_CHECK_MARGIN7, firmware_buf);
	if (ret < 0) {
...
static void bk96285_fw_mtp_check_work(struct work_struct *work)
{
	...
	struct bk96285_dev_info *di = container_of(work,
		struct bk96285_dev_info, mtp_check_work.work);
	...
	mtp_dta = g_bk96285_fw_mtp;
	mtp_len = BK96285_FW_MTP_SIZE;
...
int bk96285_fw_ops_register(struct wltrx_ic_ops *ops, struct bk96285_dev_info *di)
{
	...
	di->fw_mtp.mtp_len = BK96285_FW_MTP_SIZE;
