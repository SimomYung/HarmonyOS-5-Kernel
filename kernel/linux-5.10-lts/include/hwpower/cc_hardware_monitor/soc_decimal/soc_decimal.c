...
static void soc_decimal_init_para(struct soc_decimal_dev *l_dev)
{
	if ((l_dev->info.level < 0) || (l_dev->info.level >= l_dev->para_level))
		return;
	...
	hwlog_info("init para level=%d\n", l_dev->info.level);
}

static void soc_decimal_select_para(struct soc_decimal_dev *l_dev, int type, const void *data)
{
	soc_decimal_select_para_level(l_dev, type, data);
	soc_decimal_init_para(l_dev);
}
	...
	unsigned long event, void *data)
{
	...
	case POWER_NE_SOC_DECIMAL_DC:
		soc_decimal_select_para(l_dev, SOC_DECIMAL_TYPE_DC, data);
		break;
	case POWER_NE_SOC_DECIMAL_WL_DC:
		soc_decimal_select_para(l_dev, SOC_DECIMAL_TYPE_WL_DC, data);
		break;
	case POWER_NE_CHARGING_START:
		l_dev->info.level = 0;
		hwlog_info("charge start, set level 0 first\n");
		break;
