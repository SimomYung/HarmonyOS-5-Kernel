...
static void vbus_vote_send_bms_auth_start_uevent(struct vbus_dev *l_dev)
{
	struct power_event_notify_data n_data;

	l_dev->bms_auth_state = BMS_AUTH_VOTE_STATE;
	hwlog_info("%s bms_auth_state=%d\n", __func__, l_dev->bms_auth_state);
...
static void vbus_vote_send_bms_auth_stop_uevent(struct vbus_dev *l_dev)
{
	struct power_event_notify_data n_data;

	l_dev->bms_auth_state = BMS_AUTH_NO_VOTE_STATE;
	hwlog_info("%s bms_auth_state=%d\n", __func__, l_dev->bms_auth_state);
	...
	void *data, int result, const char *client_str)
{
	...
	if (result)
		vbus_vote_send_bms_auth_start_uevent(l_dev);
	else
		vbus_vote_send_bms_auth_stop_uevent(l_dev);
...
static struct power_sysfs_attr_info vbus_sysfs_field_tbl[] = {
	...
	power_sysfs_attr_ro(vbus, 0440, VBUS_SYSFS_BMS_AUTH_STATE, bms_auth_state),
};
	...
	struct device_attribute *attr, char *buf)
{
	...
	case VBUS_SYSFS_CONNECT_STATE:
		return scnprintf(buf, PAGE_SIZE, "%d\n", l_dev->connect_state);
	case VBUS_SYSFS_BMS_AUTH_STATE:
		hwlog_info("%s bms_auth_state=%d\n", __func__, l_dev->bms_auth_state);
		return scnprintf(buf, PAGE_SIZE, "%d\n", l_dev->bms_auth_state);
	default:
		return 0;
	}
}
