...
#define EDID_BLOCK_NUM_NUM 4
#define EDID_BLOCK_LENGTH 128
struct dpu_dp_hdmi_link_info {
	uint16_t edid_len;
	uint8_t edid[EDID_BLOCK_NUM_NUM * EDID_BLOCK_LENGTH];
};
...
struct disp_dmd {
	uint32_t dmd_id;
	char *dmd_report_info;
	uint32_t info_size;
};

struct disp_dmd_list {
	struct disp_dmd *dmd_info;
	uint32_t dmd_count;
};
enum hardware_cursor_cmd {
    HARDWARE_CURSOR_UPDATE_COORDINATE = 1,
