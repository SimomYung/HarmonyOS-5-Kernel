...
#if defined(CONFIG_HUAWEI_QOS_BLKIO)
...
#if defined(CONFIG_HUAWEI_QOS_BLKIO)
DECLARE_EVENT_CLASS(bio_qos_info_class,
	TP_PROTO(struct bio *bio, int qos),

	TP_ARGS(bio, qos),

	TP_STRUCT__entry(
		__field( unsigned int,		bi_opf_vip			)
		__field( unsigned int,		bi_opf_fg			)
		__field( int,	qos			)
	),

	TP_fast_assign(
#ifdef CONFIG_MAS_BLK
		__entry->bi_opf_vip		= (bio->mas_bi_opf & REQ_VIP) ? 1 : 0;
#else
		__entry->bi_opf_vip		= 0;
#endif
		__entry->bi_opf_fg		= (bio->bi_opf & REQ_FG) ? 1 : 0;
		__entry->qos	= qos;
	),

	TP_printk("%u, %u, %d", __entry->bi_opf_vip, __entry->bi_opf_fg, __entry->qos)
);

DEFINE_EVENT(bio_qos_info_class, bio_qos_info,
	TP_PROTO(struct bio *bio, int qos),
	TP_ARGS(bio, qos)
);
#endif /* CONFIG_HUAWEI_QOS_BLKIO */
