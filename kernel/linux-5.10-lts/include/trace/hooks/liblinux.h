...
DECLARE_LDK_RESTRICTED_HOOK(ldk_rvh_too_many_workers,
			    TP_PROTO(unsigned int nr_workers, bool *need_caching),
			    TP_ARGS(nr_workers, need_caching), HOOK_ALWAYS_ON);
...
DECLARE_LDK_RESTRICTED_HOOK(ldk_vh_set_net_nsid,
			TP_PROTO(struct net *net, struct net *old_net),
			TP_ARGS(net, old_net), HOOK_ALWAYS_ON);

DECLARE_LDK_RESTRICTED_HOOK(ldk_vh_net_cookie_init,
...
DECLARE_LDK_RESTRICTED_HOOK(ldk_rvh_trace_iptable_drop,
	     TP_PROTO(unsigned int hook, const char *tablename,
		      const struct xt_table_info *private,
		      const struct ipt_entry *e, struct sk_buff *skb),
	     TP_ARGS(hook, tablename, private, e, skb), HOOK_ALWAYS_ON);
...
DECLARE_LDK_RESTRICTED_HOOK(ldk_vh_bpf_charge_memlock,
			TP_PROTO(u32 pages, int *ret),
			TP_ARGS(pages, ret), HOOK_ALWAYS_ON);

DECLARE_LDK_RESTRICTED_HOOK(ldk_vh_bpf_uncharge_memlock,
			TP_PROTO(u32 pages),
