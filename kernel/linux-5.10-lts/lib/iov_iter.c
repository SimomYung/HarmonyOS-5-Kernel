...
static int get_user_pages_fast_copy(unsigned long start, int nr_pages,
								    unsigned int gup_flags, struct page **pages)
{
	...
	for (i = 0; i < nr_pages; i++) {
		...
		if (copy_from_user(page_address(new_page), start + i * PAGE_SIZE, PAGE_SIZE)) {
			put_page(new_page);
			ret = -EFAULT;
			break;
		}
...
ssize_t iov_iter_get_pages_copy(struct iov_iter *i,
								struct page **pages, size_t maxsize, unsigned maxpages, size_t *start)
{
	...
	iterate_all_kinds(i, maxsize, v, ({
		...
		addr &= ~(PAGE_SIZE - 1);
		n = DIV_ROUND_UP(len, PAGE_SIZE);
		res = get_user_pages_fast_copy(addr, n,
				iov_iter_rw(i) != WRITE ?  FOLL_WRITE : 0,
				pages);
		if (unlikely(res <= 0))
			return res;
...
EXPORT_SYMBOL(iov_iter_get_pages_copy);
#endif
