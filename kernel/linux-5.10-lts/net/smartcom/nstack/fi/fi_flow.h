...
#define FI_MAX_ALIGN_SIZE ((FI_NETLINK_BUF_MAX_SIZE / sizeof(struct fi_periodic_rpt_msg)) * \
...
enum fi_parse {
	FI_PAYLOAD_LEN_PARSE,
	FI_PAYLOAD_SEGMENT_PARSE,
	FI_ECHO_DELAY,
	FI_TCP_SEQ_PARSE,
	FI_TCP_ACK_PARSE,
	FI_TCP_FLAGS_PARSE,
	FI_TCP_WINDOW_SIZE_PARSE,
	FI_TCP_OPTIONS_PARSE,
	FI_END_PARSE,
};
...
/* FI basic feature config from fi config file */
typedef enum {
	FI_PROTO_NULL,
	FI_TCP,
	FI_UDP,
...
extern void fi_ipv4_flow_lock(uint32_t lock_index);

extern void fi_ipv4_flow_unlock(uint32_t lock_index);

extern uint32_t fi_ipv4_flow_hash(struct fi_pkt_parse *pkt_info, int str_len, bool exchange);
...
extern void fi_ipv4_flow_clear_node(struct fi_flow_node *flow_node);
...
static inline bool fi_flow_qos_rpt_enable(struct fi_flow_node *node)
{
	return node->flow_cb.app_info->msg.qos_rpt_enable;
}
#endif /* _FI_FLOW_H */
