...
static bool mtp_hi1106_check(void)
{
    ...
    mtp_info("subchip_type %s\n", subchip_type);
	if (strcmp(subchip_type, "bisheng") == 0 || strcmp(subchip_type, "mp17c") == 0 ||
        strcmp(subchip_type, "shenkuo") == 0 || strcmp(subchip_type, "hi1161") == 0 ||
        strcmp(subchip_type, "mp12") == 0)
		return true;
	return false;
}
#endif
	...
	list_for_each_safe(q, n, clean_list) {
		...
		if (!sock_owned_by_user(sk)) {
			...
			if (!sock_flag(sk, SOCK_DEAD)) {
				mtp_sk_info(msk, "sock is not dead\n");
				sk->sk_error_report(sk);
			} else {
				mtp_sk_info(msk, "try dec sk ref %d\n", refcount_read(&sk->sk_refcnt));
				sock_put(sk);
			}
		}
		bh_unlock_sock(sk);
...
int dev_get_mac_by_inet6addr(unsigned char *addr, u32 scope, unsigned char *mac)
{
	struct net_device *dev;
	struct inet6_dev *ind = NULL;
	struct inet6_ifaddr *ina = NULL;

	rcu_read_lock();
	for_each_netdev_rcu(&init_net, dev) {
		if (dev->ifindex != scope)
			continue;
		ind = in6_dev_get(dev);
		if (!ind)
			continue;

		read_lock_bh(&ind->lock);
		list_for_each_entry(ina, &ind->addr_list, if_list) {
			if (ina->scope == IFA_LINK &&
			    memcmp(addr, ina->addr.s6_addr, sizeof(ina->addr.s6_addr)) == 0) {
				(void)memcpy_s(mac, ETH_ALEN, dev->dev_addr, ETH_ALEN);
				read_unlock_bh(&ind->lock);
				in6_dev_put(ind);
				rcu_read_unlock();
				return 0;
			}
...
static struct sk_buff *mtp_build_udp_skb(struct sock *sk)
{
	...
	if (unlikely(!skb)) {
		mtp_sk_warn(msk, "alloc_skb failed\n");
		return NULL;
	}
...
static int mtp_get_queue_id(struct sock *sk, struct net_device *dev)
{
	...
	mtp_sk_debug(mtp_sk(sk), "ndo_select_queue %d\n", queue_index);
	__kfree_skb(skb);
	return queue_index;
}

static int mtp_setup_caps(struct sock *sk, struct net_device *dev)
{
	u32 max_segs = 1;
	struct mtp_sock *msk = (struct mtp_sock *)sk;

	if (dev->mtu < MTP_HLEN_MIN + MTP_MIN_MSS) {
		mtp_sk_info(msk, "mtu %u too small\n", dev->mtu);
		return -EINVAL;
	}
	sk_dst_set(sk, NULL);
	...
	mtp_sk_info(msk, "%s tx_queue is %d\n", dev->name, msk->tx_queue);
	...
	if (strnstr(dev->name, "wlan", IFNAMSIZ) ||
		...
		if (mtp_hi1106_check()) {
			msk->is_hi1106 = true;
			mtp_sk_info(msk, "on BISHENG(hi1106) devices\n");
		}
	}
#endif
	return 0;
...
void mtp_put_port(struct sock *sk)
{
	...
	mtp_sk_info(msk, "try to release port\n");
	spin_lock_bh(&mtp_hash.lock);
	if (!msk->src_port) {
		spin_unlock_bh(&mtp_hash.lock);
		return;
...
ebegin:
	sk_nulls_for_each_rcu(sk, node, &mtp_hash.established) {
		struct mtp_sock *msk = (struct mtp_sock *)sk;
		if (skb->dev->ifindex != sk->sk_bound_dev_if)
			continue;
...
struct sock *mtp_get_cookie_sock(struct sock *sk, const struct sk_buff *skb)
{
	...
	if (mtp_check_syn_cookie(eth, mtph) != true) {
		mtp_sk_err(mtp_sk(sk), "server mtp_check_syn_cookie failed\n");
		return NULL;
	}

	if (sk_acceptq_is_full(sk)) {
		__MTP_INC_STATS(MTP_MIB_ATTEMPTFAILS);
		mtp_sk_err(mtp_sk(sk), "sk_acceptq_is_full\n");
		return NULL;
	}
	...
	if (!newsk) {
		__MTP_INC_STATS(MTP_MIB_ATTEMPTFAILS);
		mtp_sk_err(mtp_sk(sk), "sk_clone_lock failed\n");
		return NULL;
	}
	...
	if (unlikely(err)) {
		...
		mtp_sk_info(mtp_sk(newsk), "hash failed\n");
		sock_put(newsk);
	...
	mtp_sk_info(mtp_sk(newsk), "new socket connected\n");
	return newsk;
}
