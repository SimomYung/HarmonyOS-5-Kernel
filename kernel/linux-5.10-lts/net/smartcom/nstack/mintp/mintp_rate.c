...
void mtp_rate_gen(struct sock *sk, u32 delivered, struct mtp_rate_sample *rs)
{
	...
	if (!rs->is_app_limited ||
		...
		msk->msq_limit -= (msk->msq_limit >> 3);
		msq_limit_add = msk->stats.send_rate >> READ_ONCE(sk->sk_pacing_shift);
		/* in Previous half slow-start, Increase by 50%. */
		if ((msk->snd_cwnd < (msk->snd_ssthresh >> 1)) &&
		    (u32)(MTP_JIFFIERS32 - msk->snd_cwnd_stamp) < msk->rtx.rto)
			msq_limit_add += (msq_limit_add >> 1);
		msk->msq_limit += msq_limit_add;
		mtp_sk_debug(msk, "delivered %u cwnd %u ssthresh %u send_rate %lu msq_limit %u pacing_rate %u\n",
			msk->delivered, msk->snd_cwnd, msk->snd_ssthresh, msk->stats.send_rate, msk->msq_limit,
