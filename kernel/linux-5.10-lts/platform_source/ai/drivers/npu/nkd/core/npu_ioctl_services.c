...
static int npu_ffrt_load_mdc_buf(npu_model_desc_t *model_desc, uint16_t uid)
{
	...
	struct npu_hwts_static_swap_buff* ffrt_swap_buf_phy =
		(struct npu_hwts_static_swap_buff *)(uintptr_t)get_shm_phy_addr(0, NPU_FFRT_SWAP_MEM) + (2 * model_id);
	...
	int swap_buf_index, uint16_t uid)
{
	...
	struct npu_hwts_static_swap_buff* ffrt_swap_buf_index =
		(struct npu_hwts_static_swap_buff *)(uintptr_t)get_shm_virt_addr(0, NPU_FFRT_SWAP_MEM) +
			(2 * model_id) + swap_buf_index;
...
static int npu_ffrt_load_model_buf(struct npu_proc_ctx *proc_ctx, npu_model_desc_t *model_desc, uint16_t uid)
{
	int ret = 0;
...
static int npu_ffrt_load_model(struct npu_proc_ctx *proc_ctx,
	...
	ret = npu_ffrt_load_model_buf(proc_ctx, model_desc, uid);
	cond_return_error(ret != 0, -1, "load swap_buf failed");
...
int npu_ioctl_set_model_bandwidth(struct npu_proc_ctx *proc_ctx, unsigned long arg)
{
	...
	cond_return_error(proc_ctx->devid >= NPU_DEV_NUM, -1, "invalid device");
	npd = get_npd();
	cond_return_error(npd == NULL || npd->npd_send_ipc_with_power_check == NULL, -1, "npd ptr is NULL\n");
	...
	model_desc_info = npu_calc_model_desc_info(proc_ctx->devid, para.model_id);
	...
	ret = npd->npd_send_ipc_with_power_check(IPCDRV_TS_SET_MODEL_BANDWIDTH, IPCDRV_MSG_ASYNC,
