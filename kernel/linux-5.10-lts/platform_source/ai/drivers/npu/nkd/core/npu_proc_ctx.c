...
#else
		npu_drv_warn("direct svm can't unbind here, pid = %u\n", current->tgid);
#endif
	}
...
int npu_release(struct inode *inode, struct file *filep)
{
	...
	if (npu_is_proc_resource_leaks(proc_ctx) == true) {
		...
		down_write(&cur_dev_ctx->except_manager.status_lock);
		if (proc_ctx->dev_type == NPU)
			cur_dev_ctx->except_manager.status = NPU_STATUS_NORMAL;
		up_write(&cur_dev_ctx->except_manager.status_lock);
	}
...
#ifndef CONFIG_NPU_OHOS
static int npu_ffrt_set_ssid(uint32_t model_id, uint16_t ssid)
{
	...
	uint32_t model_id, uint32_t enable_state)
{
	...
	ret = npu_ffrt_set_ssid(model_id, ssid);
	cond_return_error(ret != 0, ret,
