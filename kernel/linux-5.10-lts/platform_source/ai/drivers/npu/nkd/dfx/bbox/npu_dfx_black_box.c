...
{ 
	...
	if (report != NULL && report->persist_task_id != UINT16_MAX) {
#ifdef CONFIG_NPU_FFRT
		if (report->service_type == NPU_SERVICE_TYPE_FFRT)
			// get persist stream + persist task
			npu_get_persist_task_info(0, report);
#endif
...
{
	uint8_t wptr1 = g_npu_task_dump_manager.wptr % NPU_TASK_DUMP_NUM;
	uint8_t wptr = wptr1 == 0 ? (NPU_TASK_DUMP_NUM - 1) : wptr1 - 1;
	errno_t ret = 0;
	if (error_task != NULL) {
		while (wptr != wptr1) {
