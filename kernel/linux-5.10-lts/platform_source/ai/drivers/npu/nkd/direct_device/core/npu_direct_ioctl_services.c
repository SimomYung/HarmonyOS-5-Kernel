	...
	unsigned long arg)
{
	...
	ret = npu_send_request(proc_ctx, &task);
#else
	ret = task_sched_put_request(proc_ctx,  &task);
#endif
	if (ret != 0)
		npu_drv_err("send request fail\n");
	return ret;
}
	...
	unsigned long arg)
{
	...
	npu_drv_debug("direct stop model id  %d priority id %u stream id %u task id %u\n", model_id,
		task.u.model_execute_task.priority, task.stream_id, task.task_id);
#ifndef CONFIG_NPU_SWTS
