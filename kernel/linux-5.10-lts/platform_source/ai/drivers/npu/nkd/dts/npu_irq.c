	...
	struct npu_platform_info *plat_info)
{
	...
	plat_info->dts_info.irq_smartisp_ack = platform_get_irq(pdev,
		NPU_IRQ_SMARTISP_ACK);
	cond_return_error(plat_info->dts_info.irq_smartisp_ack < 0,
		-EINVAL, "get smartisp irq fail\n");
#endif
...
u32 g_npu_irq_gic_map[][NPU_IRQ_GIC_MAP_COLUMN] = {
	// irq_type,                 gic_num
#ifdef CONFIG_NPU_SMARTISP
	{NPU_IRQ_SMARTISP_ACK,    NPU_NPU2ACPU_HW_EXP_IRQ_NS_3},
#endif
...
{
	uint32_t gic_irq_max;
#ifdef CONFIG_NPU_SMARTISP
	gic_irq_max = NPU_NPU2ACPU_HW_EXP_IRQ_NS_4;
#else
	gic_irq_max = NPU_NPU2ACPU_HW_EXP_IRQ_NS_3;
#endif
