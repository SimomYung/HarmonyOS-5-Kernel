	...
	void *data)
{
	...
	model_id = cq->u.hwts_exception_info.model_id;
	...
	if (cq->u.exception_info.aicore_bitmap) {
		for (i = 0; i < dev_ctx->ai_core_num; i++) {
			...
			if ((dev_ctx->inuse.ai_core_error_bitmap & (0x01U << i)) == 0) {
				...
				npu_rdr_exception_report(EXC_TYPE_TS_AICORE_EXCEPTION, uid);
			}
		}
	}
...
int npu_heart_beat_exception_process(const u8 *cq_slot, u32 size)
{
	...
	struct npu_heart_beat_cq *cq = NULL;
	struct dsm_client *tmp_dsm_client = NULL;
	...
	npu_rdr_taskerr_exception_report(dev_ctx, cq->exception_code, &(cq->u.hwts_exception_info));

#if defined(CONFIG_HUAWEI_DSM) || defined(CONFIG_HISYSEVENT)
#if defined(CONFIG_NPU_PM_SMC) && defined(CONFIG_NPU_DEBUG)
	tmp_dsm_client = npu_get_dsm_client();
	if (tmp_dsm_client != NULL && (dsm_client_ocuppy(tmp_dsm_client) == 0)) {
		dsm_client_record(tmp_dsm_client,
			"NPU TS EXCEPTION, ERRCODE = %u, POWER STATUS = %d\n",
			cq->exception_code, npu_pm_get_power_vote_smc_request());
		dsm_client_notify(tmp_dsm_client,
			KERN_WTD_TIMEOUT_ERR);
		npu_drv_err("[I/DSM] %s dmd report, errcode = %u, power status = %d\n",
			tmp_dsm_client->client_name, cq->exception_code,
			npu_pm_get_power_vote_smc_request());
	}
#endif
#endif
...
#endif
		npu_rdr_exception_report(cq->exception_code, uid);
	}
	...
	u64 sq_addr, u64 cq_addr)
{
	...
	cqsq.sync_type = NPU_MAILBOX_SYNC;
...
void npu_heart_beat_exit(struct npu_dev_ctx *dev_ctx)
{
	...
	if ((dev_ctx->heart_beat.sq < NPU_MAX_FUNCTIONAL_SQ_NUM) &&
		...
		cqsq.sync_type = NPU_MAILBOX_ASYNC;
		cqsq.valid = NPU_MAILBOX_MESSAGE_VALID;
		cqsq.result = 0;
