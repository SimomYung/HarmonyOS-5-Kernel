	...
	struct npu_mailbox_message *message, int *result)
{
	...
	message->process_result = 0;
	message->is_sent = 0;
	if ((header->sync_type == NPU_MAILBOX_SYNC) || (header->sync_type == NPU_MAILBOX_BOTTOM_SYNC)) {
	...
	if ((header->sync_type == NPU_MAILBOX_SYNC) || (header->sync_type == NPU_MAILBOX_BOTTOM_SYNC)) {
		...
		if (ret == 0) {
			*result = message->process_result;
			...
			header = (struct npu_mailbox_message_header *)mailbox->send_sram;
			npu_drv_err("mailbox down timeout. ret[%d], valid[0x%x], cmd_type[%u], sync_type[%u], result[%u]\n",
				ret, header->valid, header->cmd_type, header->sync_type, header->result);
		}
	} else {
		*result = 0;
	}
