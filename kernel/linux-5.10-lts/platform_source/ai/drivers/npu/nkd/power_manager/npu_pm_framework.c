...
#ifdef CONFIG_NPU_FFRT
static int npu_plat_ffts_entry_is_clear(void)
{
	struct npd_registry *npd = NULL;
	npd = get_npd();
	cond_return_error(npd == NULL, -1, "npd ptr is NULL\n");
	cond_return_error(npd->npd_ffts_entry_is_clear == NULL, -1, "npd_ffts_entry_is_clear is NULL\n");
	return npd->npd_ffts_entry_is_clear();
}
#endif
...
int npu_suspend_powerdown(struct npu_dev_ctx *dev_ctx)
{
	...
	npu_pm_dev_clear_work_mode_idle(dev_ctx, NPU_NONSEC);
