...
{
	if (rtsq_idx >= NPU_RTSQ_NUM_PER_ENTRY) {
...
int npd_dump_exception_ffts(struct ffts_exception_report_info *report)
{
	...
	if (report->exception_type >= NPU_EXCEPTION_TYPE_HWTS_TASK_EXCEPTION
		...
		npu_hwts_exception_dump(report->exception_type, (u16)report->hwts_sq_id, report->channel_id);
	} else {
...
static int npu_handle_ffts_exception_hwts(uint16_t rtsq)
{
	...
	if (acsq_id == UINT16_MAX) {
		npu_drv_warn("the rtsq %d is not swapped in\n", rtsq);
		goto disable_sq;
	}
...
static int npu_ffts_clear_entry(uint16_t task_idx, uint16_t entry_id)
{
...
static int npu_check_and_handle_rtsq(uint16_t entry_id, uint16_t rtsq_id, uint32_t rtsq_err)
{
	...
	if (rtsq_id < rtsq_low_limit || rtsq_id > rtsq_high_limit)
		npu_drv_err("the entry %d rtsq %d invalid", entry_id, rtsq_id);
	if (rtsq_err == 1) {
		npu_drv_err("the entry %d rtsq %d has err", entry_id, rtsq_id);
		if (npu_handle_ffts_exception_hwts(rtsq_id) != 0)
			return -1;
	} else {
		npu_drv_err("the entry %d need kill rtsq %d", entry_id, rtsq_id);
...
static int npu_handle_ffrt_entry(uint16_t entry_id, bool proc_block)
{
	...
	if (ctrl0.reg.rtsq0_vld == 0 && ctrl0.reg.rtsq1_vld == 0) {
		// rtsq not occupied
		if (ctrl0.reg.rtsq0_err == 0  && ctrl0.reg.rtsq1_err == 0) {
			npu_drv_err("the entry %d has no error", entry_id);
		} else if (ctrl0.reg.rtsq0_err == 1  && ctrl0.reg.rtsq1_err == 1) {
			npu_drv_err("the entry %d check fail", entry_id);
		} else {
			npu_drv_err("the entry %d mdc err", entry_id);
		}
	} else {
...
static int npu_proc_block_kill_task(u16 uid)
{
	struct npu_dev_ctx *dev_ctx = get_dev_ctx_by_id(0);
	uint64_t hwts_base = npu_hwts_get_base_addr();
	SOC_NPU_HWTS_HWTS_FFRT_ENTRY_CTRL0_UNION ctrl0 = { 0 };
	...
	for (i = 0; i < MAX_NPU_ENTRY; i++) {
		if (i != tasksync_entry) {
			...
			if (ctrl0.reg.task_vld == 1 && info2.reg.ssid == uid) {
				npu_drv_warn("ffrt entry %d has task, ctrl0 0x%x, info1 0x%x, info2 0x%x, dfx0 0x%x",
					i, ctrl0.value, info1.value, info2.value, dfx0.value);
				ret = npu_handle_ffrt_entry(i, proc_block);
				if (ret != 0)
					npu_drv_err("npu handle ffrt %d entry is not finished", i);
			}
		} else {
			continue;
		}
...
static int npu_handle_local_task_exception_for_ffts(uint16_t entry_id, uint16_t rtsq_idx, uint16_t channel_id)
{
	uint64_t hwts_base = npu_hwts_get_base_addr();
	int ret = 0;
	bool proc_block = false;
	SOC_NPU_HWTS_HWTS_FFRT_ENTRY_CTRL0_UNION ctrl0 = { 0 };
	SOC_NPU_HWTS_HWTS_FFRT_ENTRY_INFO0_UNION info0 = { 0 };
	struct npu_dev_ctx *dev_ctx = get_dev_ctx_by_id(0);

	cond_return_error(dev_ctx == NULL, NPU_EXCEPTION_HANDLED, "dev_ctx is null\n");
	cond_return_error(hwts_base == 0ULL, NPU_EXCEPTION_HANDLED, "hwts_base is NULL\n");
	...
	|| (rtsq_idx == 1 && ctrl0.reg.rtsq1_vld == 1 && ctrl0.reg.rtsq1_err == 1 && info0.reg.rtsqid_1 == channel_id)) {
		ret = npu_handle_ffrt_entry(entry_id, proc_block);
	} else {
