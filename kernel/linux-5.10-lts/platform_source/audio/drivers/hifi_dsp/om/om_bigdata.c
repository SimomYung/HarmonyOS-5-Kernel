...
#include <audio_misc/collec_audio_info/collec_audio_info.h>
#endif
...
#define BIGDATA_VOICE_QUALITY_EVENTID  916200101
#define BIGDATA_PLAY_QUALITY_EVENTID   921001032
...
struct voice_3a_dmd_om {
	unsigned int   recv_msg_type;
	unsigned short recv_msg;
	unsigned short nvid;
	unsigned short data1;    // device
	unsigned short data2;    // line
	unsigned short data[7];  // 0:pre_nvid,1~6:create\reset\set\get\apply_up\dowm errcode
};
...
#ifdef CONFIG_HISYSEVENT
static void voip_bigdata_micblk_hisysevent_report(voip_micblk_3a_om *voip_micblk_bigdata_buff)
{
	int ret = -1;
	struct hiview_hisysevent *event = NULL;
	event = hisysevent_create("AUDIO", "VOIP_MIC_BLK_REPORT", STATISTIC);
	if (event == NULL) {
		loge("voip 3a bigdata update report create hisysevent failed: VOIP_MIC_BLK_REPORT");
		return;
	}

	hisysevent_put_integer(event, "MIC_BLK_TIMES", voip_micblk_bigdata_buff->data.mic_blk_times);
	hisysevent_put_integer(event, "HS_TOP_MIC_BLK_TIMES", voip_micblk_bigdata_buff->data.top_mic_blk_times[HS_DEV]);
	hisysevent_put_integer(event, "HF_TOP_MIC_BLK_TIMES", voip_micblk_bigdata_buff->data.top_mic_blk_times[HF_DEV]);
	hisysevent_put_integer(event, "HS_RIGHT_MIC_BLK_TIMES", voip_micblk_bigdata_buff->data.right_mic_blk_times[HS_DEV]);
	hisysevent_put_integer(event, "HF_RIGHT_MIC_BLK_TIMES", voip_micblk_bigdata_buff->data.right_mic_blk_times[HF_DEV]);
	hisysevent_put_integer(event, "HS_LEFT_MIC_BLK_TIMES", voip_micblk_bigdata_buff->data.left_mic_blk_times[HS_DEV]);
	hisysevent_put_integer(event, "HF_LEFT_MIC_BLK_TIMES", voip_micblk_bigdata_buff->data.left_mic_blk_times[HF_DEV]);
	hisysevent_put_integer(event, "HS_HEAD_MIC_BLK_TIMES", voip_micblk_bigdata_buff->data.head_mic_blk_times[HS_DEV]);
	hisysevent_put_integer(event, "HF_HEAD_MIC_BLK_TIMES", voip_micblk_bigdata_buff->data.head_mic_blk_times[HF_DEV]);
	hisysevent_put_integer(event, "HS_MIC_BLK_CHANGE_NVID", voip_micblk_bigdata_buff->data.mic_blk_change_nvid[HS_DEV]);
	hisysevent_put_integer(event, "HF_MIC_BLK_CHANGE_NVID", voip_micblk_bigdata_buff->data.mic_blk_change_nvid[HF_DEV]);
	hisysevent_put_integer(event, "MIC_ERR_TIMES", voip_micblk_bigdata_buff->data.mic_err_times);
	hisysevent_put_integer(event, "HS_MIC_DIS_CHANGE_NVID", voip_micblk_bigdata_buff->data.hs_mic_dis_change_nvid);
	hisysevent_put_integer(event, "HF_MIC_DIS_CHANGE_NVID", voip_micblk_bigdata_buff->data.hf_mic_dis_change_nvid);
	hisysevent_put_integer(event, "LEFT_MIC_DIS_TIMES", voip_micblk_bigdata_buff->data.left_mic_dis_times);
	hisysevent_put_integer(event, "HEAD_MIC_DIS_TIMES", voip_micblk_bigdata_buff->data.head_mic_dis_times);
	hisysevent_put_integer(event, "TOP_MIC_RMS", voip_micblk_bigdata_buff->data.top_mic_rms);
	hisysevent_put_integer(event, "RIGHT_MIC_RMS", voip_micblk_bigdata_buff->data.right_mic_rms);
	hisysevent_put_integer(event, "LEFT_MIC_RMS", voip_micblk_bigdata_buff->data.left_mic_rms);
	hisysevent_put_integer(event, "HEAD_MIC_RMS", voip_micblk_bigdata_buff->data.head_mic_rms);
	hisysevent_put_integer(event, "MIC_BLK_RATE", voip_micblk_bigdata_buff->data.mic_blk_rate);
	hisysevent_put_integer(event, "MIC_ERR_RATE", voip_micblk_bigdata_buff->data.mic_err_rate);
	ret = hisysevent_write(event);
	if (ret < 0)
		loge("voip 3a bigdata update report hisysevent write failed: VOIP_MIC_BLK_REPORT");
	hisysevent_destroy(&event);
	logi("VOIP_MIC_BLK_REPORT hisysevent report success");
}
#endif
...
{
#if   defined(CONFIG_HISYSEVENT)
	voip_bigdata_micblk_hisysevent_report(voip_micblk_bigdata_buff);
#else
...
static void voice_quality_update_imonitor(unsigned int eventid, audio_quality_om *data)
{
#if   defined(CONFIG_HISYSEVENT)
	int ret = -1;
	struct hiview_hisysevent *event = NULL;
	...
	char adsp_chr[1024] = {'0'};
	logi("msg_type: %d, msg_id: %d, msg_len: %d", data->recv_msg_type, data->msg_id, data->msg_len);
	vqm_fill_event_buffer(adsp_chr, sizeof(adsp_chr), data->msg_id, data->msg_len, data->data);
	hisysevent_put_string(event, "VOIP_ADSP_CHR", adsp_chr);
	ret = hisysevent_write(event);
	if (ret < 0)
		loge("voip 3a bigdata update report hisysevent write failed: VOIP_MIC_BLK_REPORT");
	hisysevent_destroy(&event);
...
static void play_quality_update_imonitor(unsigned int eventid, audio_quality_om *data)
{
#if   defined(CONFIG_HISYSEVENT)
	int ret = -1;
	struct hiview_hisysevent *event = NULL;
	char adsp_quality[1024] = {'0'};
#if IS_ENABLED(CONFIG_COLLEC_AUDIO_INFO)
    struct received_info kernel;
	get_collected_audio_info(&kernel);
	memcpy_s(data->data + data->msg_len, sizeof(adsp_quality) - data->msg_len, &kernel, sizeof(kernel));
	data->msg_len += sizeof(kernel);
#endif
	event = hisysevent_create("AUDIO_DRV", "CODEC_HIFI_TIMEOUT", FAULT);
	if (event == NULL) {
		loge("play bigdata update report create hisysevent failed: PLAY_QUALITY_REPORT");
		return;
	}
	logi("msg_type: %d, msg_id: %d, msg_len: %d", data->recv_msg_type, data->msg_id, data->msg_len);
	vqm_fill_event_buffer(adsp_quality, sizeof(adsp_quality), data->msg_id, data->msg_len, data->data);
	hisysevent_put_string(event, "FAULT_PHENOMENON", adsp_quality);
	ret = hisysevent_write(event);
	if (ret < 0)
		loge("play bigdata update report hisysevent write failed: PLAY_QUALITY_REPORT");
	hisysevent_destroy(&event);
	logi("PLAY_QUALITY_REPORT hisysevent report success");
#else
	logw("imonitor and hisysevent is not supported\n");
#endif
}
...
#ifdef CONFIG_HUAWEI_DSM_AUDIO
static void voice_dmd_update_imonitor(unsigned int type, unsigned int eventid, unsigned int msg_type, struct voice_3a_dmd_om *dmd_data)
{
    logi("3a error type: %u error: %u nvid: %u device: %u line: %u pre_nv: %u errcode(0~5): %u,%u,%u,%u,%u,%u\n",
        msg_type, dmd_data->recv_msg, dmd_data->nvid, dmd_data->data1, dmd_data->data2, dmd_data->data[0],
        dmd_data->data[1], dmd_data->data[2], dmd_data->data[3], dmd_data->data[4], dmd_data->data[5], dmd_data->data[6]);
#if   defined(CONFIG_HISYSEVENT)
	int ret = -1;
	char adsp_dmd[1024] = {'0'};

	struct hiview_hisysevent *event = NULL;
	event = hisysevent_create("AUDIO_DRV", "SOC_HIFI_3A_ERROR", FAULT);
	if (event == NULL) {
		loge("dmd bigdata update report create hisysevent failed: SOC_HIFI_3A_ERROR");
		return;
	}

    ret = sprintf_s(adsp_dmd, sizeof(adsp_dmd),
        "3a error type: %u error: %u nvid: %u device: %u line: %u pre_nv: %u errcode(0~5): %u,%u,%u,%u,%u,%u\n",
        msg_type, dmd_data->recv_msg, dmd_data->nvid, dmd_data->data1, dmd_data->data2, dmd_data->data[0],
        dmd_data->data[1], dmd_data->data[2], dmd_data->data[3], dmd_data->data[4], dmd_data->data[5], dmd_data->data[6]);
    if (ret <= 0) {
        loge("dmd_data str: %s too long\n", adsp_dmd);
        return;
    }
	hisysevent_put_string(event, "CONTENT", adsp_dmd);
	ret = hisysevent_write(event);
	if (ret < 0)
		loge("dmd bigdata update report hisysevent write failed: SOC_HIFI_3A_ERROR");
	hisysevent_destroy(&event);
	logi("voice dmd hisysevent report success");
#else
	logw("imonitor and hisysevent is not supported\n");
#endif
}
...
static void socdsp_om_show_voice_3a_info(struct work_struct *work)
{
	...
	unsigned int data_len = 0;
	unsigned int socdsp_msg_type;
	struct voice_3a_dmd_om  voice_3a_dmd_om_info;
	data = (unsigned char*)kcalloc(MAIL_LEN_MAX, sizeof(char), GFP_KERNEL);
	if (data == NULL) {
		logw("calloc mem fail, len: %u\n", MAIL_LEN_MAX);
		return;
	}
	while(get_workqueue_data(SOCDSP_OM_WORK_VOICE_3A, data, &data_len) == 0) {

	socdsp_msg_type = *(unsigned int *)data;
	switch (socdsp_msg_type) {
	case SOCDSP_3A_DMD_MSG:
		if ((sizeof(voice_3a_dmd_om_info)) != data_len) {
			logw("unavailable data from socdsp, type: %u, len: %u\n",
				socdsp_msg_type, data_len);
			kfree(data);
			data = NULL;
		...
		memcpy(&voice_3a_dmd_om_info, data, sizeof(voice_3a_dmd_om_info));

#ifdef CONFIG_HUAWEI_DSM_AUDIO
        voice_dmd_update_imonitor(AUDIO_CODEC, DSM_SOC_HIFI_3A_ERROR, socdsp_msg_type, &voice_3a_dmd_om_info);
#endif
		break;
	case SOCDSP_VOICE_INFO_MSG:
		if ((sizeof(audio_quality_om)) < data_len) {
			logw("unavailable data from socdsp, type: %u, len: %u\n",
				socdsp_msg_type, data_len);
			kfree(data);
			data = NULL;
		...
		voice_quality_update_imonitor(BIGDATA_VOICE_QUALITY_EVENTID, (audio_quality_om *)data);
        break;
    case SOCDSP_VOIP_MICBLK_INFO_MSG:
        ...
        voip_blkmic_bigdata_update_imonitor(BIGDATA_VOICE_QUALITY_EVENTID, 0, (voip_micblk_3a_om *)data);
		break;
    case SOCDSP_PLAY_INFO_MSG:
		if ((sizeof(audio_quality_om)) < data_len) {
			logw("unavailable data from socdsp, type: %u, len: %u\n",
				socdsp_msg_type, data_len);
			kfree(data);
		...
		play_quality_update_imonitor(BIGDATA_PLAY_QUALITY_EVENTID, (audio_quality_om *)data);
        break;
	default:
		logi("type: %u, not support\n", socdsp_msg_type);
		break;
...
static int a2dp_offload_report_info(unsigned int event_id, struct a2dp_offload_report_info *info)
{
	...
	logi("a2dp_offload_report_hisysevent ++\n");
	...
	event = hisysevent_create("AUDIO", "A2DPOFFLOAD_BIGDATA_REPORT", STATISTIC);
	if (event == NULL) {
		loge("a2dp_offload create hisysevent failed\n");
		return -1;
	}

	hisysevent_put_integer(event, "ERR_CODE", 1);
	hisysevent_put_integer(event, "SUB_ERROR_CODE", 0);

	for (i = 0; i < INT_INFO_BUT; i++)
		hisysevent_put_integer(event, A2DP_OFFLOAD_REPORT_STR[i], info->int_info[i]);

	for (i = INT_INFO_BUT; i < STR_INFO_BUT; i++)
		hisysevent_put_string(event, A2DP_OFFLOAD_REPORT_STR[i], info->str_info[i - INT_INFO_BUT]);

	ret = hisysevent_write(event);
	if (ret < 0)
		loge("a2dp_offload report hisysevent failed\n");
