...
extern int dpu_dptx_set_aparam(uint32_t channel_num, uint32_t data_width, uint32_t sample_rate, int port_id);
...
static void set_dp_audio_params(struct snd_pcm_hw_params *params)
{
	...
	if (dp_audio_pdata->is_dp_plug_in && dp_audio_pdata->enable_set_audio_config_with_kernel) {
		dpu_dptx_set_aparam(channels, data_width, sample_rate, 0);
