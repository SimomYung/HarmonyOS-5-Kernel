...
#define USBAUDIO_WAIT_HIFI_POWER_OFF ((3 * HZ) / 5)
...
#endif
	if (is_typec_support_suspend() && rcv_msg.msg_type == USBAUDIO_CHN_MSG_DISCONNECT_RCV)
		AUDIO_LOGI("No wakeup source is required when typeC is disconnected");
	else
...
int usbaudio_nv_check(void)
{
	...
	usbaudio_mailbox_send_data(&nv_check_msg, sizeof(nv_check_msg), 0);
	retval = wait_for_completion_timeout(&g_nv_check, HIFI_USBAUDIO_NV_CHECK_TIMEOUT);
	if (retval == 0) {
		AUDIO_LOGE("usbaudio nv check timeout");
		return DSP_HANDSHAKE_TIMEOUT;
	}

	AUDIO_LOGI("usbaudio nv check intime");
	return DSP_HANDSHAKE_OK;
}
...
int usbaudio_disconnect_msg(unsigned int dsp_reset_flag)
{
	...
	/*
	 * in ohos pc, if the TypeC headset is installed, the HiFi is always running.
	 * When the system enter ULSR state, the headset is disconnected and the chip is powered off.
	 * If no delay,the HiFi cannot powered off. When the system exits ULSR, the HiFi fails powered on, causing a panic
	 */
	if (is_ulsr()) {
		mdelay(USBAUDIO_WAIT_HIFI_POWER_OFF);
		AUDIO_LOGI("delay 600ms");
	}
