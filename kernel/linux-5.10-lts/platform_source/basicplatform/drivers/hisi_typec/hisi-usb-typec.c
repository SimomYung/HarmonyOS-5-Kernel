...
static int hisi_typec_set_pdo(enum adapter_cap_type type, int mv, int ma)
{
...
static int hisi_typec_get_vendor_id(void)
{
...
static int hisi_typec_get_product_id(void)
{
	struct pd_source_cap_ext cap_ext = {0};
...
static int hisi_typec_get_version(int *fw_ver, int *hw_ver)
{
	struct pd_source_cap_ext cap_ext = {0};
...
{
	return hisi_typec_set_pdo(HISI_PD_APDO_START, mv, ma);
}

static int hisi_typec_set_pps_end(void)
{
	return hisi_typec_set_pdo(HISI_PD_APDO_END, 0, 0);
}

static int hisi_typec_get_source_apdo_cap(struct hisi_usb_apdo_info *para)
{
...
static int hisi_typec_get_pps_adapter_output(int *mv, int *ma)
{
	struct usb_typec *typec = _typec;
	int ret;
...
static int hisi_usb_pd_dp_state(void *client)
{
	struct usb_typec *typec = _typec;
	...
	return hisi_tcpm_is_during_dp(typec->tcpc_dev);
}
...
void hisi_usb_typec_get_raw_pdo(uint32_t *raw_pdo, size_t size)
{
	hw_pd_get_raw_pdo(raw_pdo, size);
}
...
static struct pps_protocol_ops hisi_typec_pps_protocol_ops = {
	.chip_name = "hisi_pd",
	.set_cap = hisi_typec_set_pps_start,
	.set_pps_end_cap = hisi_typec_set_pps_end,
	.get_temp = hisi_typec_get_temp,
	.get_output = hisi_typec_get_pps_adapter_output,
	.get_source_apdo_cap = hisi_typec_get_source_apdo_cap,
	.get_max_current = hisi_typec_get_max_current,
	.is_support_pps = hisi_typec_adapter_support_pps,
	.get_vendor_id = hisi_typec_get_vendor_id,
	.get_product_id = hisi_typec_get_product_id,
	.get_version = hisi_typec_get_version,
};
#endif
...
static struct pd_dpm_ops hisi_device_pd_dpm_ops = {
	...
	.pd_dpm_get_dp_state = hisi_usb_pd_dp_state,
