...
static DEFINE_MUTEX(smmu_cfg_lock);
...
static void nsv_pcie_common_rst_cfg(struct hi_pcie_nsv *plat)
{
	...
	bit = ((plat->pcie->ctrl_id == RC_ID0) ? HI_PCIE_VOTE_COMMON_RST_PCIE0 : HI_PCIE_VOTE_COMMON_RST_PCIE1);
	...
	if ((val & SCTRL_PCIE_RESOURCE_VOTE_RST_MASK) == 0x0) {
		hi_pcie_pr_i("pcie common rst_cfg");
...
static void nsv_pcie_common_unrst_cfg(struct hi_pcie_nsv *plat)
{
	...
	val = iodie_pcie_sctrl_reg_readl(SCTRL_PCIE_RESOURCE_VOTE_REG);
	if ((val & SCTRL_PCIE_RESOURCE_VOTE_RST_MASK) == 0x0) {
		hi_pcie_pr_i("pcie common unrst_cfg");
...
static void nsv_pcie_common_clk_cfg(struct hi_pcie_nsv *plat)
{
	...
	bit = ((plat->pcie->ctrl_id == RC_ID0) ? HI_PCIE_VOTE_COMMON_CLK_PCIE0 : HI_PCIE_VOTE_COMMON_CLK_PCIE1);
	resource_vote_lock = iodie_pcie_resource_vote_lock_get();
	...
	if ((val & SCTRL_PCIE_RESOURCE_VOTE_CLK_MASK) == 0x0) { // bit10 tcu, bit14 fsm
		hi_pcie_pr_i("pcie common clk_cfg");
...
static void nsv_pcie_common_clk_uncfg(struct hi_pcie_nsv *plat)
{
	...
	val = iodie_pcie_sctrl_reg_readl(SCTRL_PCIE_RESOURCE_VOTE_REG);
	if ((val & SCTRL_PCIE_RESOURCE_VOTE_CLK_MASK) == 0x0) { // bit10 tcu, bit14 fsm
		hi_pcie_pr_e("pcie common clk_uncfg");
...
static int nsv_pcie_fnpll_clk_cfg(struct hi_pcie_nsv *plat)
{
	...
	spin_unlock_irqrestore(resource_vote_lock, flags);
	iodie_pcie_hwspinlock_release();
	hi_pcie_pr_i("[%s]fnpll_cnt=0x%x", __func__, iodie_pcie_sctrl_reg_readl(SCTRL_PCIE_RESOURCE_VOTE_REG));
	return 0;
}
...
static bool nsv_pcie_is_pipe_clk_stable(struct hi_pcie_nsv *plat, int behavior)
{
	...
	hi_pcie_pr_i("PIPE clk is stable in (1000-%u)*100 us", time);
	return true;
}
...
static int nsv_pcie_tbu_config(struct hi_pcie_nsv *plat, u32 enable)
{
	...
	if (enable) {
		...
		if (tok_trans_gnt < PCIE_TBU_DEFALUT_TOK_TRANS) {
			hi_pcie_pr_e("tok_trans_gnt is less than setting");
			return -1;
		}
	}
...
static int nsv_pcie_enable_smmu(struct hi_pcie_nsv *plat)
{
	...
	hi_pcie_pr_i("enable smmu pwr_cnt=%d", atomic_read(nsv_pcie_smmu[cdx].tcu_pwr_cnt));

	ret = mm_smmu_nonsec_tcuctl(nsv_pcie_smmu[cdx].smmu_id);
	if (ret) {
		hi_pcie_pr_e("TCU cfg Failed");
		return -1;
	}

	if (atomic_add_return(1, nsv_pcie_smmu[cdx].tcu_pwr_cnt) == 1) {
		hi_pcie_pr_i("smmu poweron");
	...
	if ((ret != 0) && (atomic_sub_return(1, nsv_pcie_smmu[cdx].tcu_pwr_cnt) == 0)) {
		(void)mm_smmu_poweroff(plat->pcie->dev);
...
static int nsv_pcie_disable_smmu(struct hi_pcie_nsv *plat)
{
	...
	hi_pcie_pr_i("disable smmu pwr_cnt=%d", atomic_read(nsv_pcie_smmu[cdx].tcu_pwr_cnt));
	...
	ret = nsv_pcie_tbu_config(plat, DISABLE);
	if (ret)
		hi_pcie_pr_e("TBU config(disconnect) failed");

	if (atomic_sub_return(1, nsv_pcie_smmu[cdx].tcu_pwr_cnt) == 0) {
		hi_pcie_pr_i("smmu poweroff");
...
static int nsv_pcie_turn_on(struct hi_pcie *pcie)
{
	...
	if (!plat->share_smmu_support) {
		nsv_pcie_common_unrst_cfg(plat); // 2.3.3
		nsv_pcie_common_clk_cfg(plat); // 2.3.4
	}
...
static int nsv_pcie_turn_off(struct hi_pcie *pcie)
{
	...
	if (plat->share_smmu_support)
		mutex_lock(&smmu_cfg_lock);
	nsv_pcie_disable_smmu(plat);
	if (plat->share_smmu_support) {
		nsv_pcie_common_clk_uncfg(plat);
		nsv_pcie_common_rst_cfg(plat);
		mutex_unlock(&smmu_cfg_lock);
	}
...
static int nsv_pcie_parse_base_config(struct platform_device *pdev, struct hi_pcie_nsv *plat)
{
	...
	plat->share_smmu_support = device_property_read_bool(&pdev->dev, "share_smmu_support");
	hi_pcie_pr_i("share_smmu_support = %u", plat->share_smmu_support);
...
static int nsv_pcie_start_link(struct hi_pcie_port *port)
{
	struct hi_pcie_nsv *plat = (struct hi_pcie_nsv *)port->pcie->plat;

	if (nsv_pcie_phy_chk_recfg_ffe(port)) {
		hi_pcie_pr_e("phy status chk failed");
		return -1;
	}
	...
	nsv_pcie_smmu_init(plat); // @todo smmu configuration
	if (plat->share_smmu_support)
		mutex_unlock(&smmu_cfg_lock);

	hi_pcie_idle_clear_irq(port->pcie->idle);
	hi_pcie_32k_idle_init(port->pcie->idle);
	hi_pcie_idle_enable_irq(port->pcie->idle);
