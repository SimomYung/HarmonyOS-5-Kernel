...
struct opt_regs ship_mode_por_option_regs[] = {
	/* reg, mask, shift, val, before,after */
	reg_cfg(0x362, 0x1, 0, 0x1, 0, 5),
	reg_cfg(0x362, 0x1, 0, 0x0, 0, 5),

	reg_cfg(0x07a, 0xff, 0, 0x60, 0, 0),
	reg_cfg(0x900, 0xff, 0, 0x1, 0, 0),
	reg_cfg(0x901, 0xff, 0, 0x1, 0, 0),
	reg_cfg(0x902, 0xff, 0, 0x1, 0, 0),
	reg_cfg(0x903, 0xff, 0, 0x1, 0, 0),
	reg_cfg(0x904, 0xff, 0, 0x1, 0, 0),
	reg_cfg(0x905, 0xff, 0, 0x0, 0, 0),
	reg_cfg(0x906, 0xff, 0, 0x0, 0, 0),
	reg_cfg(0x908, 0xff, 0, 0x0, 0, 0),
	reg_cfg(0x90b, 0xff, 0, 0x0, 0, 0),
	reg_cfg(0x90c, 0xff, 0, 0x0, 0, 0),
	reg_cfg(0x90f, 0xff, 0, 0x5a, 0, 0),
	reg_cfg(0x91b, 0xff, 0, 0x1, 0, 0),
	reg_cfg(0x91c, 0xff, 0, 0x0, 0, 0),
	reg_cfg(0x91e, 0xff, 0, 0x1, 0, 0),
	reg_cfg(0x91f, 0xff, 0, 0x0, 0, 0),
	reg_cfg(0x252, 0xff, 0, 0x0, 0, 0),
	reg_cfg(0x253, 0xff, 0, 0x0, 0, 0),
};

struct opt_regs ship_mode_rst_option_regs[] = {
	/* reg, mask, shift, val, before,after */
	reg_cfg(0x315, 0xff, 0, 0xac, 0, 0),
	reg_cfg(0x315, 0xff, 0, 0x5a, 1, 0),
	reg_cfg(0x316, 0xff, 0, 0xac, 0, 0),
	reg_cfg(0x316, 0xff, 0, 0x5a, 1, 0),
	reg_cfg(0x313, 0xff, 0, 0xac, 0, 0),
	reg_cfg(0x313, 0xff, 0, 0x5a, 1, 0),
	reg_cfg(0x312, 0xff, 0, 0xac, 0, 0),
	reg_cfg(0x312, 0xff, 0, 0x5a, 1, 0),
	reg_cfg(0x317, 0xff, 0, 0xac, 0, 0),
	reg_cfg(0x317, 0xff, 0, 0x5a, 1, 0),
	reg_cfg(0x314, 0xff, 0, 0xac, 0, 0),
	reg_cfg(0x314, 0xff, 0, 0x5a, 1, 0),
	reg_cfg(0x31c, 0xff, 0, 0xac, 0, 0),
	reg_cfg(0x31c, 0xff, 0, 0x5a, 1, 0),
	reg_cfg(0x123, 0xff, 0, 0xac, 0, 0),
	reg_cfg(0x123, 0xff, 0, 0x5a, 1, 0),
};
...
static int scharger_get_vbus_vset(int *value)
{
	struct scharger_buck *buck = g_buck;
...
static void ovp_dmd_delay_work(struct work_struct *work)
{
	...
	scharger_dsm_report_fault_info(buff, DSM_SC0_INFO);
}
#endif
...
static int buck_init(struct charge_init_data *init_crit)
{
	...
	ret += scharger_set_terminal_voltage(buck->param_dts.default_vterm);
	ret += scharger_set_terminal_current(CHG_TERM_ICHG_150MA);
	ret += scharger_set_watchdog_timer(WATCHDOG_TIMER_50_S);
...
static int scharger_set_ship_mode_enable(bool enable)
{
	...
	if (buck->param_dts.ship_mode_rst_enable && enable) {
		scharger_inf("[%s] set rst!\n", __func__);

		opt_regs_set(ship_mode_por_option_regs,
			ARRAY_SIZE(ship_mode_por_option_regs));

		opt_regs_set(ship_mode_rst_option_regs,
			ARRAY_SIZE(ship_mode_rst_option_regs));
	}
...
static struct charge_device_ops scharger_ops = {
	...
	.get_vbus_vset = scharger_get_vbus_vset,
	...
	.check_batfet_single_ctrl = get_batfet_single_ctrl_status,
};
...
static void buck_parse_dts(struct device_node *np, struct scharger_buck *buck)
{
	...
	if (ret != 0) {
		...
		buck->param_dts.default_vterm = CHG_FAST_VCHG_4430;
	}
	...
	ret = of_property_read_u32(np, "ship_mode_rst_enable",
				(u32 *)&(buck->param_dts.ship_mode_rst_enable));
	if (ret) {
		buck->param_dts.ship_mode_rst_enable = 0;
		pr_info("get ship_mode_rst_enable invalid, do not reset in shipmode\n");
	}
	pr_info("prase_dts ship_mode_rst_enable=%d\n", buck->param_dts.ship_mode_rst_enable);
...
struct scharger_buck *scharger_v700_get_buck_device(void)
{
	return g_buck;
}
