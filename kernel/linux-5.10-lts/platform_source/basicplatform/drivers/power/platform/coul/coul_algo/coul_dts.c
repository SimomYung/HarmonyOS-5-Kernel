...
static void get_batt_fake_coul_info(struct smartstar_coul_device *di,
	const struct device_node *np)
{
	int ret;
	ret = of_property_read_u32(np, "fake_coul_enable", &di->fake_coul_enable);
	ret += of_property_read_u32(np, "fake_coul_current_enable", &di->fake_coul_current_enable);
	ret += of_property_read_u32(np, "fake_coul_max_current_th", &di->fake_coul_max_current_th);
	if (ret) {
		coul_core_err("%s get batt fake coul_info not error\n", __func__);
		di->fake_coul_enable = 0;
	} else {
		coul_core_info("%s fake coul enable=%d, fake coul curr enable=%d, fake coul max curr=%d\n",
		__func__, di->fake_coul_enable, di->fake_coul_current_enable, di->fake_coul_max_current_th);
	}
}
