...
#define ACR_MAX_BATTERY_CURRENT_MA      100
#define COUL_INVALID_MAX_COUNT          6
...
bool check_ocv_dense_area(struct smartstar_coul_device *di, int ocv_uv)
{
	int i;
	struct cap_dense_area *areas = di->batt_data->cap_dense_area;

	/* whtether to use cap dense area data from batt raw data excel */
	if (areas && areas->rows > 0) {
		for (i = 0; i < areas->rows; i++) {
			if ((di->batt_ocv > areas->start[i] * PERMILLAGE) &&
				(di->batt_ocv < areas->end[i] * PERMILLAGE))
				return TRUE;
		}
	} else {
...
{
	if (check_ocv_dense_area(di, ocv_uv))
		return TRUE;
	/* restrict [0, 4000) ocv update with cc dischage 5%(maybe 20%) fcc */
	...
	enum batt_coul_event event)
{
	...
	case BATT_COUL_DROP:
		...
		ret = call_coul_blocking_notifiers(BATT_COUL_DROP, (void *)&data);
		if ((ret != NOTIFY_DONE) && (ret != NOTIFY_OK))
...
static bool check_coul_ready_by_fifo_current(struct smartstar_coul_device *di)
{
	short i;
	int current_ua;
	int fifo_depth;
	...
	for (i = 0; i < fifo_depth; i++) {
		current_ua = di->coul_dev_ops->get_battery_cur_ua_from_fifo(di->batt_index, i);
		current_ua /= UA_PER_MA;
		if (abs(current_ua) > di->fake_coul_max_current_th) {
			invalid_cnt++;
			coul_core_err("[%s] current = %d", __func__, current_ua);
		}
	}
 
	return (invalid_cnt < COUL_INVALID_MAX_COUNT);
}

 /* check wheather coul is ready 1: OK 0:not ready */
int coul_is_ready(void *data)
{
	bool is_ready;
	...
	if (di->fake_coul_enable) {
		is_ready = check_coul_ready_by_fifo_current(di);
		if (di->fake_coul_current_enable && !is_ready) {
			coul_core_info("%s, batt_fake startup successful\n", __func__);
			return 0;
		}
	}
