...
int coul_battery_retry_temp_permille(struct smartstar_coul_device *di, enum battery_temp_user user)
{
	...
	if (di->coul_dev_ops && di->coul_dev_ops->get_bat_temp) {
		temperature = di->coul_dev_ops->get_bat_temp(di->batt_index) * PERMILLAGE;
#ifdef CONFIG_HUAWEI_BATTERY_TEMP_FITTING
		temperature = btf_try_fit(temperature, di->batt_index);
#endif
		return temperature;
	}
	...
	enum battery_temp_user user)
{
	...
	if (di->external_compensation_is) {
		if (bat_temp_get_rt_temperature_for_coul(di->batt_index, &temp)) {
			coul_core_info("%s error\n", __func__);
			return coul_battery_retry_temp_permille(di, user) / PERCENT;
		} 
		coul_core_info("%s temp %d\n", __func__, temp);
		return temp;
	}
	return coul_battery_retry_temp_permille(di, user) / PERCENT;
}
...
#ifdef CONFIG_HUAWEI_BATTERY_TEMP_FITTING
	batt_temp_fitted = btf_temp_fitted_eqr(di->batt_index);
#endif
	...
	enum battery_temp_user user)
{
	...
	if (di->external_compensation_is) {
		if (bat_temp_get_rt_temperature_for_coul(di->batt_index, &temperature)) {
			goto next;
		}
		coul_core_info("%s temperature %d\n", __func__, temperature * TENTH);
		return temperature * TENTH;
	}
