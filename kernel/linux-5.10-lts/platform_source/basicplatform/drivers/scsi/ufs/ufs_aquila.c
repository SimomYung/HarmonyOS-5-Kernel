...
#define UFS_SYSCTRL_UFS_AXUSER_CFG_OFFSET 0x300
...
#define EFUSE_MPHY_VESRION_SEL (0x1 << 23)


#define MPHY_FSM_WAIT_TIME 50000
...
#ifdef CONFIG_SCSI_UFS_AQUILA_CS
...
static const struct ufs_attr_cfg hisi_fw_config_attr[] = {
	{ 0x000080C8, 0x00000000 },
#ifdef CONFIG_SCSI_UFS_AQUILA_SCOUT
	{ 0x000080CC, 0x00000019 },
#else
	{ 0x000080CC, 0x0000001D },
#endif
	{ 0x000080D0, 0x10000000 },
#ifdef CONFIG_SCSI_UFS_AQUILA_SCOUT
	{ 0x000080D4, 0x00C00000 },
#else
	{ 0x000080D4, 0x01C00000 },
#endif
	{ 0x000080D8, 0x05400000 },
...
static uint32_t ufs_get_ufs_version(struct hufs_host *host)
{
	 return readl(host->sysctrl + SCTRL_SCBAKDATA1_MASK_OFFSET) & 0x1;
}
...
static void ufs_dfa_cpu_clk_init(struct ufs_hba *hba, struct hufs_host *host)
{
	int ret;

	/* close pll2, dfa cpu clk 964M */
	ret = clk_set_rate(host->clk_ufs_dfa_cpu, PLL2_FREQ);
	if (ret)
		dev_err(hba->dev, "clk_ufs_dfa_cpu set clk failed\n");
	ret = clk_prepare_enable(host->clk_ufs_dfa_cpu);
	if (ret)
		dev_err(hba->dev, "clk_ufs_dfa_cpu clk_prepare_enable failed\n");
	if (!is_dfa_enabled(hba)) {
		udelay(10);
		clk_disable_unprepare(host->clk_ufs_dfa_cpu);
	}
}
...
void ufs_clk_init(struct ufs_hba *hba)
{
	...
	if (is_dfa_enabled(hba) && !atomic_read(&hba->dhba.need_reset))
		return dfa_clk_init(hba);
#endif
	...
	if (host->host_id == 1) {
		...
		/* 1. config ufs1 retention dual -rail ram exit deep sleep */
		writel(UFS1_MEM_RETENTION_MASK, host->actrl + ACTRL_HSDT1_MEM_CTRL0_OFFSET);
		...
		/* 9. ufs1 need open clkin_sedes buf share with usb */
		ret = clk_prepare_enable(host->clk_ufs1phy_ref);
		if (ret)
			dev_err(hba->dev, "clk_ufs1phy_ref clk_prepare_enable failed\n");
	} else {
		/* 0. reset device first */
		...
		/* 4.0 ufs device */
		if (ufs_get_ufs_version(host)) {
			/* select pll0, ufs dfa sys clk 418M */
...
void ufs_soc_init(struct ufs_hba *hba)
{
	...
	if ((host->host_id == 0) && !is_dfa_enabled(hba)) {
		/* disable dfa clk, only enable ufs_peri_subsys bit6 */
		writel(BIT(6), acore_cfg_clk_en(host->dfa_sys_ctrl));
		/* reset dfa bit[0:5] and bit[7:8] */
		writel(0x1bf, acore_cfg_rst_req(host->dfa_sys_ctrl));
	}
#endif
...
static void unipro_h8_clk_gate_enable(struct ufs_hba *hba)
{
	u32 reg = 0;
...
static void unipro_h8_clk_gate_disable(struct ufs_hba *hba)
{
	u32 reg = 0;
...
static int hufs1_suspend(struct ufs_hba *hba, enum ufs_pm_op pm_op)
{
	int ret;
	u32 value;
	int retry = POLLING_RETRY;
	...
	clk_disable_unprepare(host->clk_ufs1phy_ref);

	/* 9. close qic ufs hic, ufs subsys, mphy clk */
	writel(GT_CLK_UFS_MPHY_SYS_HSDT1 | GT_CLK_UFS_SYS_SUBSYS_HSDT1 | GT_CLK_QIC_UFS_HCI_HSDT1,
	...
	/* step 11. reset  dfa por */
	writel(IP_RST_UFS_CRG_HSDT1 | IP_RST_UFS_SYS_SUBSYS_HSDT1 | IP_RST_QIC_UFS_HCI,
				host->hsdt1_crg + HSDT1_CRG_PERRSTEN0_OFFSET);

	/* 12. check ufs1 reset state */
	while (retry--) {
		value = readl(host->hsdt1_crg + HSDT1_CRG_PERRSTSTAT0_OFFSET);
	...
	ufs_actrl_writel(host, 40004000, ACTRL_HSDT1_MEM_CTRL0_OFFSET);

	pmic_write_reg(PMIC_CLK_IODIE_EN_ADDR(0), 0);
...
int hufs_suspend(struct ufs_hba *hba, enum ufs_pm_op pm_op)
{
	...
	if (ret)
		pr_err("%s: enable RET of dual-rail RAM, set DS =1 , SD = 0 fail ret = %d\n", __func__, ret);
...
static int hufs1_resume(struct ufs_hba *hba, enum ufs_pm_op pm_op)
{
	int ret;
	u32 value;
	int retry = POLLING_RETRY;
	...
	pmic_write_reg(PMIC_CLK_IODIE_EN_ADDR(0), 1);
...
int hufs_resume(struct ufs_hba *hba, enum ufs_pm_op pm_op)
{
	...
	if (ret)
		pr_err("%s: disable RET of dual-rail RAM, DS = 0, SD = 0 fail ret = %d\n", __func__, ret);
...
static void mphy_set_pcs_init(struct hufs_host *host)
{
...
static void mphy_set_pcs_time_escape_trunk(struct hufs_host *host)
{
...
static void mphy_hibern8_config(struct hufs_host *host)
{
	...
	mphy_pcs_writel(host, MIN_ACTIVATETIME_CAPABILITY_VAL, PCS_RX0_MIN_ACTIVATETIME_CAPABILITY);
	mphy_pcs_writel(host, MIN_ACTIVATETIME_CAPABILITY_VAL, PCS_RX1_MIN_ACTIVATETIME_CAPABILITY);
...
static void mphy_config_efuse(struct hufs_host *host)
{
	u32 reg;
	...
	/* config mphy soc version */
	if (readl(host->sysctrl + SCTRL_SCBAKDATA1_MASK_OFFSET) & (0x1 << 9)) {
		dev_err(host->hba->dev, "soc v110 found\n");
		reg = mphy_pma_readl(host, PMA_API_CSR13);
		reg |= EFUSE_MPHY_VESRION_SEL;
		mphy_pma_writel(host, reg, PMA_API_CSR13);
	}
}
...
static void mphy5_idle_set(struct ufs_hba *hba)
{
	u32 reg = 0;
	struct hufs_host *host = NULL;
...
static void mphy_config_tactivate(struct ufs_hba *hba)
{
	u32 pa_activate;
	u32 pa_granularity;
	u32 value;
...
void hufs_full_reset(struct ufs_hba *hba)
{
	...
	ufshcd_hba_stop(hba);
	msleep(200); /* 300 + 200ms sleep is enough for waiting those requests. */
	/* close dfa clk */
	if (host->host_id == 0)
		clk_disable_unprepare(host->clk_ufs_dfa_sys);
	else
	...
	struct ufs_hba *hba, struct ufs_pa_layer_attr *dev_req_params)
{
	...
	host = hba->priv;
	hba->scramble = true;
	if ((hba->manufacturer_id == UFS_VENDOR_SKHYNIX) && (hba->ufs_device_spec_version == UFS_DEVICE_SPEC_4_0))
		ufshcd_dme_set(hba, UIC_ARG_MIB(PA_SAVECONFIGTIME), 0xFF);

	/* solve nac problem on samsung for nsv/ara */
	if (hba->manufacturer_id == UFS_VENDOR_SAMSUNG) {
		...
		if (value < 0x4e) {
			value += 2;
			ufshcd_dme_set(hba, UIC_ARG_MIB(PA_TXHSG4SYNCLENGTH), value);
		}
		ufshcd_dme_get(hba, UIC_ARG_MIB(PA_TXHSG5SYNCLENGTH), &value);
		if (value < 0x4e) {
			value += 2;
			ufshcd_dme_set(hba, UIC_ARG_MIB(PA_TXHSG5SYNCLENGTH), value);
		}
	}

	/* squelch detect diff_n config on THR925 & HH3.1 for nsv/ara */
	...
	if ((hba->manufacturer_id == UFS_VENDOR_TOSHIBA) && (hba->ufs_device_spec_version == UFS_DEVICE_SPEC_4_0)) {
		/* toshiba need 0x80fc bit0 ~ bit1 set 0x1 */
...
int ufs_get_hufs_hc(struct hufs_host *host)
{
	...
	if (host->host_id == 0) {
		host->dfa_sys_ctrl = of_iomap(np, 5);
		if (!host->dfa_sys_ctrl) {
			dev_err(host->hba->dev, "dfa_sys_ctrl iomap error!\n");
			return -ENOMEM;
		}
	}

#ifdef CONFIG_DFA_MCQ
	if ((host->host_id == 0) && dfa_get_base_addr(host->hba, np))
		return -ENOMEM;
#endif
	...
	if (host->host_id == 1) {
		host->clk_ufs1phy_ref = devm_clk_get(host->hba->dev, "clk_ufs1phy_ref");
		if (IS_ERR(host->clk_ufs1phy_ref)) {
			dev_err(host->hba->dev, "clk_ufs1phy_ref not found.\n");
			return -ENOMEM;
		}
	...
	host->clk_ufs_dfa_cpu = devm_clk_get(host->hba->dev, "clk_ufs_dfa_cpu");
	if (IS_ERR(host->clk_ufs_dfa_cpu)) {
		dev_err(host->hba->dev, "clk_ufs_dfa_cpu not found.\n");
		return -ENOMEM;
	}
...
void ufshcd_enable_autodiv(struct ufs_hba *hba)
{
	...
	/* ufs_dfa_sys_div_auto_cfg = 0x1f(1/32) for ufs, 0x3f(1/64) for dfa */
	if (is_dfa_enabled(hba))
		reg |= UFS_DFA_SYS_DIV_AUTO_CFG_DFA;
	else
	...
	dev_info(hba->dev, "ufs PERI_AUTODIV1:0x%x\n", readl(host->pericrg + PERI_AUTODIV1_OFFSET));
}
