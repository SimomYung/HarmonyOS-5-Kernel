...
#define RPMB_LP_DELAY_MS 100

#ifdef CONFIG_RPMB_UFS_VERIFY
char g_rpmb_ufs_model[MAX_MODEL_LEN + 1] = {0};
#endif
...
#ifdef CONFIG_UFS_LOW_POWER_VOTE_STATUS_SYNC
	atomic_set(&hba->rpmb_lp_delayed, 1);

	while (atomic_read(&hba->rpmb_lp_delay_extend)) {
		atomic_set(&hba->rpmb_lp_delay_extend, 0);
		msleep(RPMB_LP_DELAY_MS);
	}

	atomic_set(&hba->rpmb_lp_delayed, 0);
	atomic_set(&hba->rpmb_lp_voted, 0);

	/* set ldk cmd status as false if cmd count is 0 */
...
#ifdef CONFIG_RPMB_UFS_VERIFY
	memcpy(g_rpmb_ufs_model, hba->model, MAX_MODEL_LEN);
#endif
