					  ...
					  unsigned int timeout)
{
	...
	if (is_mcq_enabled(hba))
		return __ufsmcq_queuecommand_directly(hba, cmd, timeout);
#endif
					...
					unsigned int timeout, bool eh_handle)
{
	...
	if (is_dfa_enabled(hba))
		return dfa_queuecommand_directly(hba, cmd, timeout, eh_handle);
#endif
				       ...
				       struct scsi_device *sdp, enum ufs_dev_pwr_mode pwr_mode)
{
	...
	for (i = 3,j=0; j < 4; j++,i--) {
		...
		ufshcd_compose_scsi_cmd(&cmnd, psdev, cmd, buffer, DMA_NONE,
					NULL, 0, 0);
#ifdef CONFIG_SCSI_UFS_RESET_FLUSH
		if (hba->ufshcd_reset_flush && !is_dfa_enabled(hba)) {
			ret = __ufshcd_queuecommand_directly(hba, &cmnd, 1500);
			if (ret) {
				ret = __ufshcd_queuecommand_directly(hba, &cmnd, 1500);
				dev_err(hba->dev, "%s: ufs flush retry! ret = %d\r\n", __func__, ret);
			}
		} else {
