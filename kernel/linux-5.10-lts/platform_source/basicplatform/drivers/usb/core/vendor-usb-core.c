...
void usb_notify_set_address_err(struct usb_device_info *udev_info)
{
	blocking_notifier_call_chain(&usb_fault_notifier_list, USB_SET_ADDRESS_ERR, udev_info);
}

void usb_notify_get_descriptor_err(struct usb_device_info *udev_info)
{
	blocking_notifier_call_chain(&usb_fault_notifier_list, USB_GET_DESCRIPTOR_ERR, udev_info);
}

void usb_notify_enum_dev_unknown_err(struct usb_device_info *udev_info)
{
	blocking_notifier_call_chain(&usb_fault_notifier_list, USB_ENUM_UNKNOWN_ERR, udev_info);
}

void usb_notify_transfer_err(struct usb_device_info *udev_info)
{
	blocking_notifier_call_chain(&usb_fault_notifier_list, USB_TRANSFER_ERR, udev_info);
}
...
static void usb_string_copy(char *dest, u32 dest_max_len, char *src)
{
	int ret;
	...
	(void)memset_s(dest, dest_max_len, 0, dest_max_len);
	ret = strncpy_s(dest, dest_max_len, src, dest_max_len - 1);
	if (ret != EOK)
		pr_info("copy %s name failed!\n", src);
	dest[dest_max_len - 1] = '\0';
}

static void get_usb_dev_info(struct usb_device *udev)
{
	struct usb_device_info *udev_info = &g_record.udev_info;

	udev_info->devnum = udev->devnum;
	udev_info->route = udev->route;
	udev_info->quirks = udev->quirks;
	udev_info->slot_id = udev->slot_id;
	udev_info->portnum = udev->portnum;
	udev_info->level = udev->level;
	udev_info->devaddr = udev->devaddr;
	udev_info->bus_mA = udev->bus_mA;
	udev_info->state = udev->state;
	udev_info->speed = udev->speed;

	udev_info->descriptor = udev->descriptor;

	usb_string_copy(udev_info->devpath, DEV_PATH_LEN, udev->devpath);
	usb_string_copy(udev_info->product, PORDUCT_LEN, udev->product);
	usb_string_copy(udev_info->manufacturer, MANUFEATURER_LEN, udev->manufacturer);
	usb_string_copy(udev_info->serial, SERIAL_LEN, udev->serial);

#ifdef CONFIG_USB_HIUSB
	udev_info->port_id = hiusb_find_portid_by_udev(udev);
#endif
	udev_info->busnum = udev->bus->busnum;
}
...
static void __usb_trans_err_work(struct work_struct *work)
{
	unused(work);
	struct usb_device_info *udev_info = &g_record.udev_info;

	usb_notify_transfer_err(udev_info);
}

static DECLARE_WORK(usb_trans_err_work, __usb_trans_err_work);

static void usb_transfer_err_work(struct usb_device *udev)
{
	get_usb_dev_info(udev);
	schedule_work(&usb_trans_err_work);
}
