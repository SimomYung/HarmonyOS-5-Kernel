			...
			struct platform_device **xhci)
{
	...
	host_res[1].start = (unsigned int)irq;
	host_res[1].end = (unsigned int)irq;
...
int hiusbc_host_init(struct hiusbc *hiusbc)
{
	...
	if (hiusbc->usb2_host_lpm_capable == 0)
		props[prop_idx++] = PROPERTY_ENTRY_BOOL("usb2-lpm-disable");

	if (hiusbc->host_abort_cmd_quirk)
		props[prop_idx++] = PROPERTY_ENTRY_BOOL("host_abort_cmd_quirk");

	if (hiusbc->host_ca_timeout_quirk)
		props[prop_idx++] = PROPERTY_ENTRY_BOOL("host_ca_timeout_quirk");
