...
static unsigned int calc_trbs_needed(struct hiusbc_req *hreq)
{
	...
	if (trbs_needed > 8)
		pr_debug("!!!more than 8 TRBs in the TD!!!\n");
...
static void hiusbc_noc_err_test(
	struct hiusbc_ep *hep, u64 *buf_ptr)
{
	if (hiusbc_is_test_noc_err()) {
		if (hep->is_in) {
			if(usb_endpoint_xfer_bulk(hep->ep.desc)) {
				*buf_ptr = hiusbc_get_noc_err_addr(buf_ptr);
				pr_err("buf_ptr 0x%llx", *buf_ptr);
			}
		}
	}
}
				...
				struct hiusbc_req *hreq, gfp_t gfp_flags)
{
...
#ifdef CONFIG_DFX_DEBUG_FS
	hiusbc_noc_err_test (hep, &buf_ptr);
#endif
	...
	hiusbc_dbg(HIUSBC_DEBUG_XFER,
		...
		"buf_dma = 0x%llx, length = %u, first TRB = 0x%pK (0x%llx dma),"
		" last TRB = 0x%pK (0x%llx dma)\n",
		hep->epnum,
			...
			struct hiusbc_ep *hep, int status, bool skip_last)
{
	...
	while (!list_empty(&hep->queue)) {
		hreq = list_first_entry_or_null(&hep->queue, struct hiusbc_req, queue);
			...
			struct hiusbc_ep *hep, bool new_req, gfp_t gfp_flags)
{
	...
	if (hep->stalled != 0 || hep->set_halt_pending != 0) {
		hiusbc_dbg(HIUSBC_DEBUG_ERR,
	...
	if (!hiusbc_request_to_trbs(hep, gfp_flags)) {
		 ...
		 */
		if (hiusbc->db_on_received_event == 0)
			return 0;
	}
			...
			struct hiusbc_ep *hep, bool new_req, gfp_t gfp_flags)
{
	...
	if (hep->xfer_state == HIUSBC_XFER_IDLE && hep->nrdy_received == 0) {
					...
					bool is_stall, gfp_t gfp_flags)
{
	...
	hiusbc_dbg(HIUSBC_DEBUG_CTRL,
		...
		"first TRB = %pK (0x%llx dma), last TRB = %pK (0x%llx dma).\n",
		is_stall ? "STALL" : "ACK",
		hep->epnum,
		&hreq->req,
		hreq->first_trb, hreq->trb_dma,
...
static int hiusbc_ep_queue_transfer(struct usb_ep *ep, struct hiusbc_ep *hep, struct hiusbc *hiusbc)
{
	int ret;
...
int hiusbc_ep_queue(struct usb_ep *ep, struct usb_request *req)
{
	...
	if (!hep->ep.desc || hiusbc->running == 0) {
		hiusbc_dbg(HIUSBC_DEBUG_ERR,
	...
	ret = hiusbc_ep_queue_transfer(ep, hep, hiusbc);
	if (ret)
		hiusbc_dbg(HIUSBC_DEBUG_ERR, "queue trb transfer failed\n");
...
int hiusbc_ep_dequeue(struct usb_ep *ep, struct usb_request *req)
{
	...
	if (hep->xfer_state != HIUSBC_XFER_END && hep->xfer_state != HIUSBC_XFER_IDLE) {
		ret = hiusbc_send_cmd_end_transfer(hep, false);
		if ((ret < 0) && (hep->end_xfer_pending == 0))
			return ret;
	}
