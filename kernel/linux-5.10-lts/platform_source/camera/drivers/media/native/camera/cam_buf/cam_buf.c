...
int cam_buf_sc_attach(int fd, struct systemcache_format *fmt)
{
	if (fd < 0 || !fmt) {
		cam_err("%s: fd or fmt invalid", __func__);
		return -EINVAL;
	...
	return cam_internal_sc_attach(fd, fmt);
}

int cam_buf_sc_detach(int fd)
{
	if (fd < 0) {
		cam_err("%s: fd invalid", __func__);
		return -EINVAL;
	...
	return cam_internal_sc_detach(fd);
}
...
static long cam_config(struct cam_buf_cfg *cfg)
{
	...
	case CAM_BUF_SC_ATTACH:
		cam_buf_sc_attach(cfg->fd, &cfg->systemcache_format);
		break;
	case CAM_BUF_SC_DETACH:
		cam_buf_sc_detach(cfg->fd);
		break;
#endif
	default:
