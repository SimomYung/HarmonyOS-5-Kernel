...
atomic_t g_flash_fd_use_count = ATOMIC_INIT(0);
	...
	struct v4l2_subdev_fh *fh)
{
	...
	atomic_dec(&g_flash_fd_use_count);
	cam_info("flash close use count %d", atomic_read(&g_flash_fd_use_count));
	if (atomic_read(&g_flash_fd_use_count) > 0) {
		cam_info("still someone use flash %d", atomic_read(&g_flash_fd_use_count));
		return 0;
	}

	if (atomic_read(&g_flash_fd_use_count) < 0) {
		atomic_set(&g_flash_fd_use_count, 0);
	}
	if (!flash_ctrl)
		return 0;
...
static int hw_flash_subdev_internal_open(struct v4l2_subdev *sd,
	struct v4l2_subdev_fh *fh)
{
	atomic_inc(&g_flash_fd_use_count);
	cam_info("flash open use count %d", atomic_read(&g_flash_fd_use_count));
	return 0;
}
...
static struct v4l2_subdev_internal_ops g_flash_subdev_internal_ops = {
	.open = &hw_flash_subdev_internal_open,
