...
struct hwsensor_t {
	...
	struct mutex cmd_pid_list_mtx;
	struct list_head cmd_pid_list;

	struct mutex pid_fh_map_mtx;
	struct pid_fh_info pid_fh_map[MAX_SENSOR_CLIENT];
};
...
struct cmd_pid_info {
	...
	enum power_ctrl_type cmd;
	pid_t pid;
};

static bool add_pid_fh_info(struct pid_fh_info pid_fh_map[MAX_SENSOR_CLIENT], pid_t pid, struct v4l2_subdev_fh *fh) {
	for (int i = 0; i < MAX_SENSOR_CLIENT; ++i) {
		if (pid_fh_map[i].pid == 0) {
			pid_fh_map[i].pid = pid;
			pid_fh_map[i].fh = fh;
			return true;
		}
	}
...
static pid_t get_pid_by_fh(struct pid_fh_info pid_fh_map[MAX_SENSOR_CLIENT], struct v4l2_subdev_fh *fh) {
	pid_t pid = 0;
	for (int i = 0; i < MAX_SENSOR_CLIENT; ++i)
		if (pid_fh_map[i].fh == fh) {
			pid = pid_fh_map[i].pid;
			pid_fh_map[i].pid = 0;
			return pid;
		}
	return 0;
...
static int query_cmd_count(struct list_head *cmd_pid_list, enum power_ctrl_type cmd) {
	struct cmd_pid_info *node;
	int ref_count = 0;

	list_for_each_entry(node, cmd_pid_list, list) {
		if (node->cmd == cmd) ref_count++;
	}
	return ref_count;
}

// ref count +1
static bool add_cmd_pid_to_list(const char *dev_name, struct list_head *cmd_pid_list, enum power_ctrl_type cmd,
		pid_t pid)
{
    int ref_count = 0;
	struct cmd_pid_info* node = kzalloc(sizeof(struct cmd_pid_info), GFP_ATOMIC);
	if (!node) {
		cam_err("%s alloc cmd pid info failed!", __func__);
		return false;
	}
	node->cmd = cmd;
	node->pid = pid;
	list_add(&node->list, cmd_pid_list);
	ref_count = query_cmd_count(cmd_pid_list, cmd);
	cam_info("%s: %s cmd: %d ref_count: %d", __func__, dev_name, cmd, ref_count);
	return ref_count == 1;
}

// ref count -1
static bool remove_cmd_pid_from_list(const char *dev_name, struct list_head *cmd_pid_list, enum power_ctrl_type cmd,
		pid_t pid, bool force)
{
	int ref_count = query_cmd_count(cmd_pid_list, cmd);
	char cmd_pid_list_str[256] = {};
	int pos = 0;
	int written = 0;
	if (ref_count == 0) {
		cam_info("%s: %s cmd: %d already removed", dev_name, __func__, cmd);
		return false;
	}

	struct cmd_pid_info *node, *tmp;
	list_for_each_entry_safe(node, tmp, cmd_pid_list, list) {
		if (node->pid == pid && node->cmd == cmd) {
			list_del(&node->list);
			kfree(node);
			if (!force) break;
		}
	...
	if (ref_count > 0 && force) {
		list_for_each_entry(node, cmd_pid_list, list) {
			if (node->cmd == cmd) {
				written = snprintf_s(cmd_pid_list_str + pos, sizeof(cmd_pid_list_str) - pos, sizeof(cmd_pid_list_str) - pos - 1,
						"%d, ", node->pid);
				if (written <= 0 || written + pos >= sizeof(cmd_pid_list_str))
					break;
				pos += written;
			}
		}
...
void cleanup_cmd_pid_list(struct list_head *cmd_pid_list) {
	struct cmd_pid_info *node, *tmp;
	list_for_each_entry_safe(node, tmp, cmd_pid_list, list) {
		list_del(&node->list);
		kfree(node);
...
static enum power_ctrl_type to_power_ctl_type(int cfgtype) {
	switch (cfgtype) {
...
static bool handle_power_ctl(struct hwsensor_t *s, struct sensor_cfg_data *data, long *rc, bool force_down,
		pid_t orig_pid)
{
	bool do_power = false;
	bool power_type = false;

	mutex_lock(&s->cmd_pid_list_mtx);
	switch (data->cfgtype) {
		case SEN_CONFIG_UFS_AUTO_CONTROL_OFF:
		case SEN_CONFIG_EXT_VCM_POWER_ON:
		case SEN_CONFIG_POWER_ON:
			power_type = true;
            do_power = add_cmd_pid_to_list(dev_name(&s->pdev->dev), &s->cmd_pid_list, to_power_ctl_type(data->cfgtype),
					orig_pid);
			break;
		case SEN_CONFIG_UFS_AUTO_CONTROL_ON:
		case SEN_CONFIG_EXT_VCM_POWER_OFF:
		case SEN_CONFIG_POWER_OFF:
			power_type = true;
            do_power = remove_cmd_pid_from_list(dev_name(&s->pdev->dev), &s->cmd_pid_list,
					to_power_ctl_type(data->cfgtype), orig_pid, force_down);
			break;
		case SEN_CONFIG_NCS_EXTEND_CMD:
			if (data->cfg.ncs_extend_cmd.extend_cmd == NCS_EXTEND_CMD_SET_LCD_MIPI_SWITCH_OCCUPY) {
				power_type = true;
				do_power = true;
			}
			break;
	...
	if (power_type)
		cam_info("%s: me: %d %s power type: %d %s", __func__, orig_pid, dev_name(&s->pdev->dev), data->cfgtype,
				do_power ? "do power" : "skip");

    if (do_power)
		*rc = s->intf->vtbl->config(s->intf, data);
	else if (power_type)
		*rc = 0;

	mutex_unlock(&s->cmd_pid_list_mtx);
	return power_type;
}
	...
	struct v4l2_subdev_fh *fh)
{
	...
	mutex_lock(&s->pid_fh_map_mtx);
	orig_pid = get_pid_by_fh(s->pid_fh_map, fh);
	mutex_unlock(&s->pid_fh_map_mtx);

	cdata.cfgtype = SEN_CONFIG_UFS_AUTO_CONTROL_ON;
	handle_power_ctl(s, &cdata, &power_rc, true, orig_pid);
	rc += power_rc;

	cdata.cfgtype = SEN_CONFIG_NCS_EXTEND_CMD;
	cdata.cfg.ncs_extend_cmd.extend_cmd = NCS_EXTEND_CMD_SET_LCD_MIPI_SWITCH_OCCUPY;
	cdata.cfg.ncs_extend_cmd.cmd.mipi_switch_info.mipi_status = false;
	handle_power_ctl(s, &cdata, &power_rc, true, orig_pid);
	rc += power_rc;
	...
	struct v4l2_subdev_fh *fh)
{
	...
	mutex_lock(&s->pid_fh_map_mtx);
	if (!add_pid_fh_info(s->pid_fh_map, current->tgid, fh)) {
		cam_err("%s: failed to add pid fh info", __func__);
		mutex_unlock(&s->pid_fh_map_mtx);
		return -ENOENT;
	}
	mutex_unlock(&s->pid_fh_map_mtx);
	...
	unsigned int cmd, unsigned long args)
{
	...
	case HWSENSOR_IOCTL_SENSOR_CFG: {
		...
		if (handle_power_ctl(g_s, &data, &ret, false, current->tgid))
			break;
	...
	unsigned int cmd, void *arg)
{
	...
	case HWSENSOR_IOCTL_GET_INFO:
		...
		break;
	case HWSENSOR_IOCTL_SENSOR_CFG:
		if (handle_power_ctl(s, arg, &rc, false, current->tgid))
			break;
...
int hwsensor_register(struct platform_device *pdev, hwsensor_intf_t *si)
{
	...
	mutex_init(&sensor->cmd_pid_list_mtx);
	mutex_init(&sensor->pid_fh_map_mtx);
	INIT_LIST_HEAD(&sensor->cmd_pid_list);
...
void hwsensor_unregister(struct platform_device *pdev)
{
	...
	cleanup_cmd_pid_list(&sensor->cmd_pid_list);
