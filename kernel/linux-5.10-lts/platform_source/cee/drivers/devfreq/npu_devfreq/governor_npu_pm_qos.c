...
#ifdef CONFIG_NPUFREQ_ISP_SOFT_VOTE
static struct platform_qos_request g_npu_pm_qos_req_isp;
static DEFINE_MUTEX(npufreq_mutex);
#endif
...
void isp_nputop_freq_dnlimit(unsigned long freq)
{
#ifdef CONFIG_NPUFREQ_ISP_SOFT_VOTE
	mutex_lock(&npufreq_mutex);
	if (freq == 0 && platform_qos_request_active(&g_npu_pm_qos_req_isp)) {
		platform_qos_remove_request(&g_npu_pm_qos_req_isp);
		mutex_unlock(&npufreq_mutex);
		return;
	}

	if (!platform_qos_request_active(&g_npu_pm_qos_req_isp))
		platform_qos_add_request(&g_npu_pm_qos_req_isp, g_npu_pm_qos_class_dn, freq);
	else
		platform_qos_update_request(&g_npu_pm_qos_req_isp, freq);
...
#ifdef CONFIG_NPUFREQ_ISP_SOFT_VOTE
	platform_qos_add_request(&g_npu_pm_qos_req_isp, g_npu_pm_qos_class_dn, 0);
#endif
