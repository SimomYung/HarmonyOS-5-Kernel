...
int cpu_register_iy_nvme(char *buffer, unsigned int *length)
{
	...
	u32 cluster;
	improve_yield_cpu_nv_t cpu_info;
	char *cluster_str[] = {";CPUL ", ";CPUM ", ";CPUB "};
	...
	*length = 0;
	for (cluster = CPU_LIT_CLUSTER; cluster < CPU_CLUSTER_CNT; cluster++) {
		if (cpu_info.cluster_info[cluster].bits.pv_flag == 1) {
			ret = sprintf_s(buffer + *length, CPU_IY_NV_INFO_MAX_LENGTH, "%s%s%d%s%d%s%d%s0x%x", cluster_str[cluster],
				"enable:", cpu_info.cluster_info[cluster].bits.enable_flag,
				";pv_flag:", cpu_info.cluster_info[cluster].bits.pv_flag,
				";all_freq_pv_flag:", cpu_info.cluster_info[cluster].bits.all_freq_pv_flag,
				";freq_ce_mask:", cpu_info.cluster_info[cluster].bits.freq_ce_mask);
			if (ret == -1) {
...
static bool is_cluster_improve_yield_enabled(u32 cluster)
{
	int ret = 0;
	improve_yield_cpu_nv_t cpu_info;
	...
	return cpu_info.cluster_info[cluster].bits.enable_flag == 1;
}
...
static bool cpu_improve_yield(u32 cpu)
{
	...
	if (is_rtb_intercept_phase() == 1 && is_cluster_improve_yield_enabled(cluster))
