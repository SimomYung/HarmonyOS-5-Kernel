				...
				u32 granted_power)
{
	...
	if (strncmp(instance->cdev->type, "thermal-devfreq-0", THERMAL_ACTOR_NAME_LEN) == 0) {
		actor_id = ipa_get_actor_id("gpu");
	} else if (strncmp(instance->cdev->type, "thermal-cpufreq-0", THERMAL_ACTOR_NAME_LEN) == 0) {
		actor_id = ipa_get_actor_id("cluster0");
	} else if (strncmp(instance->cdev->type, "thermal-cpufreq-1", THERMAL_ACTOR_NAME_LEN) == 0) {
		actor_id = ipa_get_actor_id("cluster1");
	} else if (strncmp(instance->cdev->type, "thermal-cpufreq-2", THERMAL_ACTOR_NAME_LEN) == 0) {
