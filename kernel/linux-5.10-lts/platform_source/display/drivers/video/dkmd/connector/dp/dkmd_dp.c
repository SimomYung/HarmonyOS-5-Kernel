...
static int dpu_dptx_handle_plugin(struct dp_ctrl *dptx, int mode, int typec_orien)
{
	...
	if (is_dp_hdmi_link_over()) {
		switch_set_state(&dptx->sdev, HOT_DP_NUM_EXCEED);
...
static int dpu_dptx_handle_plugout(struct dp_ctrl *dptx, int mode, int typec_orien)
{
	...
	dptx->dp_present->vsync_ctrl.timestamp = 0;
	dptx->dp_present->vsync_ctrl.force_wake_up = 1;
	wake_up_interruptible_all(&(dptx->dp_present->vsync_ctrl.wait));
	return 0;
}
