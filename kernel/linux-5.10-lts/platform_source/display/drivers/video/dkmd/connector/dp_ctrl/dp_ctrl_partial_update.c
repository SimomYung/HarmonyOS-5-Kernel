...
static void dp_ctrl_partial_update_set_coord(const struct dpu_connector *connector)
{
	...
	if (!dptx || !dptx->dptx_enable || dptx->power_saving_mode) {
		dpu_pr_err("[DP] port id %d not exist or has already been off!", dptx);
		return;
	}

	dptx->su_rect.top_x = dirty_rect.x;
	dptx->su_rect.top_y = dirty_rect.y;
	dptx->su_rect.bottom_x = (uint32_t)dirty_rect.x + dpu_width(dirty_rect.w);
	dptx->su_rect.bottom_y = (uint32_t)dirty_rect.y + dpu_height(dirty_rect.h);
