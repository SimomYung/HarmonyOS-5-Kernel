...
static int32_t mipi_async_tx_task_stop(struct dpu_connector *connector, const void *value)
{
	struct mipi_dsi_connector_params con_params;
	struct dkmd_connector_info *pinfo;
	void_unused(value);
	if (!connector) {
		dpu_pr_err("connector is null, error\n");
		return -1;
	}
	...
	dpu_mipi_dsi_async_tx_stop(&con_params);
	return 0;
}
...
static int32_t mipi_dsi_off(struct dkmd_connector_info *pinfo)
{
	...
	mipi_async_tx_task_stop(connector, NULL);
...
static int32_t mipi_dsi_isr_setup(struct dpu_connector *connector, const void *value)
{
	...
	isr_ctrl->unmask = ~(DSI_INT_VACT0_START | DSI_INT_VACT0_END |
		DSI_INT_FRM_END | DSI_INT_UNDER_FLOW | frm_start_bit);
...
static struct connector_ops_handle_data dsi_ops_table[] = {
	...
	{ ASYNC_TX_STOP, mipi_async_tx_task_stop },
