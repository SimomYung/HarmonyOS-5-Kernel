...
static uint32_t mipi_dsi_read(uint32_t *out, const char __iomem *dsi_base, uint32_t wait_us)
{
	uint32_t pkg_status;
	uint32_t present_time_us = 35 * 1000; /* total wait 35ms */
...
int32_t mipi_dsi_lread_reg(uint32_t *out, int out_len, struct dsi_cmd_desc *cm, uint32_t len, char *dsi_base)
{
	...
	for (i = 0; (i < (len + 3) / 4) && (i < (uint32_t)out_len); i++) {  /* read 4 bytes once */
		if (mipi_dsi_read(out, dsi_base, cm->wait) == 0) {
			ret = -1;
