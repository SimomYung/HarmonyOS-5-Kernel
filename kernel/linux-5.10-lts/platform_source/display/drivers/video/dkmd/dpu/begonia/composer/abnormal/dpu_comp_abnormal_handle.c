...
static void dpu_comp_abnormal_dump_cmdlist_dm(const struct dpu_cmdlist_frame_info *last_cmdlist_frm_info)
{
	...
	uint32_t cmdlist_id = last_cmdlist_frm_info->cmdlist_id;
	...
	ukmd_cmdlist_dump_dm(dpu_res_get_cmdlist_dev_id(), scene_id, cmdlist_id);
#endif
}
#endif
...
static void dpu_print_sdma_dbg_info(char __iomem *module_base)
{
	uint32_t i;
	...
	for (i = 0; i < OPR_SDMA_NUM; ++i) {
		...
		if ((inp32(DPU_GLB_SDMA_DBG_MONITOR4_ADDR(module_base, i)) != 0) && (g_dpu_config_data.version.info.version == DPU_ACCEL_DPUV800)) {
			g_dpu_dsm_info.hebc_err = 1;
			dpu_pr_warn("sdma hebc occured err may cause underflow: smda[%d]_monitor4[0x%x]", i, inp32(DPU_GLB_SDMA_DBG_MONITOR4_ADDR(module_base, i)));
		}
	}
}
...
void dpu_comp_abnormal_debug_dump(struct dpu_composer *dpu_comp, uint32_t scene_id)
{
	...
	dpu_pr_warn("OV:\n"
		...
		inp32(DPU_DSI_VCOUNT_ADDR(dpu_comp->comp_mgr->dpu_base)));
	udelay(20);
	dpu_pr_warn("OV:\n"
		"\tDBG_OV_YCNT0=%#x\tDBG_OV_YCNT1=%#x\tDBG_OV_YCNT2=%#x\tDBG_OV_YCNT3=%#x\tDSI_VCOUNT=%#x\n\n",
	...
	dpu_print_sdma_dbg_info(module_base);
...
static void dpu_comp_abnormal_handle_work(struct kthread_work *work)
{
	...
	*/
	...
	if (is_mipi_cmd_panel(&dpu_comp->conn_info->base)) {
		...
		dpu_pr_warn("underflow clear composer_present_timeline_resync.");
	}
...
void dpu_comp_abnormal_dump_offline(char __iomem *dpu_base)
{
	...
	if ((g_dpu_config_data.version.info.version == DPU_ACCEL_DPUV800) ||
		...
		dpu_pr_warn("OV:\n"
			...
			inp32(DPU_GLB_DBG_OV_YCNT0_ADDR(glb_base)), inp32(DPU_GLB_DBG_OV_YCNT0_ADDR(glb_base)));
		dpu_print_sdma_dbg_info(glb_base);
