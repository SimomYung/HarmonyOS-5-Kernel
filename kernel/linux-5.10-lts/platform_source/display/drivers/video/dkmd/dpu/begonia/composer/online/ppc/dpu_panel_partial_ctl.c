...
int32_t dpu_set_active_rect(uint32_t ppc_config_id)
{
	...
	for (i = 0; i < DCS_CMDLIST_PACKET_NUM; ++i) {
		priv->cmdlist_phy_addr[real_ppc_config_id + i][PPC_1ST_DSI_IDX] =
			cmdlist_get_phy_addr(dpu_res_get_cmdlist_dev_id(), priv->cmdlist_scene_id,
	...
	priv->cmdlist_phy_addr[sec_part_config_id][PPC_1ST_DSI_IDX] = cmdlist_get_phy_addr(dpu_res_get_cmdlist_dev_id(),
		priv->cmdlist_scene_id, priv->reg_cmdlist_ids[sec_part_config_id][PPC_1ST_DSI_IDX]);
	if (connector->bind_connector)
		priv->cmdlist_phy_addr[sec_part_config_id][PPC_2ND_DSI_IDX] = cmdlist_get_phy_addr(dpu_res_get_cmdlist_dev_id(),
