...
/* tui_request: 
 * 0-DPU_SEC_EXIT_ALL, 1-DPU_SEC_ENTER_LEVEL0, 2-DPU_SEC_ENTER_LEVEL1
 */
static int notify_dpu_tui_request(void *data, int tui_request)
{
	...
	if (tui_request != DPU_SEC_EXIT_ALL) {
		secure_ctrl->tui_mode = tui_request;
...
{
	dpu_comp->secure_ctrl.tui_ready_status = (tui_ready_status == DPU_SEC_EXIT_ALL) ? TUI_SEC_DISABLE : TUI_SEC_ENABLE;
}
...
static DEVICE_ATTR(dpu_secure, 0440, dpu_secure_event_show, dpu_secure_event_store);
