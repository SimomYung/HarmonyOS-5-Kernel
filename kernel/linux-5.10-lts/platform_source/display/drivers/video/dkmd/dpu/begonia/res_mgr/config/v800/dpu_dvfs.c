...
static void dpu_dvfs_peri_init(char __iomem *pmctrl_base)
{
	...
	if (g_dpu_config_data.version.info.soc_type == DPU_SOC_TYPE_ES)
		g_vote_info[4].vote_clk_div = 0b0000; /* level4 0.8  - 640M, ppll2_b, 1div  */
	else if (g_dpu_config_data.version.info.soc_type == DPU_SOC_TYPE_CS)
		g_vote_info[4].vote_clk_div = 0b0001; /* level4 0.8  - 640M, ppll2_b, 2div  */
	...
	int cmd_size, int opt_type)
{
	...
	if (vote_level >= DPU_CORE_LEVEL_ON) {
		ref_value = inp32(SOC_PMCTRL_PERI_DSS_AVS_VOL_IDX_0_ADDR(pmctrl_base));
		value = (uint32_t)inp32(SOC_PMCTRL_PERI_M1_DVS_STAT3_ADDR(pmctrl_base)) & 0xFF;
		if (value < ((ref_value >> ((vote_level - 1) * 8)) & 0xFF)) {
			dpu_pr_warn("pls check sw dvfs, vote_level %d, new dvfs level %#x, "
				"curr dvfs level %#x, soft channel stat %#x, hard channel stat %#x, dvfs ref level %#x",
				vote_level, inp32(SOC_PMCTRL_PERI_M1_DVS_STAT3_ADDR(pmctrl_base)),
				inp32(SOC_PMCTRL_PERI_M1_DVS_STAT7_ADDR(pmctrl_base)),
				inp32(SOC_PMCTRL_HW_DSS_DVFS_SF_RD_ADDR(pmctrl_base)),
				inp32(SOC_PMCTRL_HW_DSS_DVFS_HW_RD_ADDR(pmctrl_base)),
				inp32(SOC_PMCTRL_PERI_DSS_AVS_VOL_IDX_0_ADDR(pmctrl_base)));
...
static void dpu_hw_dvfs_vote_level(uint32_t vote_level, uint32_t last_level)
{
...
{
	if (vote_level == DPU_CORE_FREQ1 || vote_level == DPU_CORE_FREQ2)
		return DPU_CORE_FREQ3;
...
void dpu_dvfs_intra_process(struct intra_frame_dvfs_info *dvfs_info, uint32_t vote_level, bool inter_locked)
{
...
uint32_t dpu_get_intra_prev_vote_level(void)
{
	return 0;
}
