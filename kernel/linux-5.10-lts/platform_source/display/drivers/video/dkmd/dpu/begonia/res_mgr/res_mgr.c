...
uint32_t dpu_res_get_cmdlist_dev_id(void)
{
	...
	if (g_rm_dev->data.cmdlist_dev_id != CMDLIST_DEV_MAX_NUM)
		return g_rm_dev->data.cmdlist_dev_id;

	g_rm_dev->data.cmdlist_dev_id = cmdlist_get_dev_id_by_name(DPU_CMDLIST);
	return g_rm_dev->data.cmdlist_dev_id;
}
...
static void dpu_res_init_data(struct dpu_res_data *data)
{
	...
	data->cmdlist_dev_id = CMDLIST_DEV_MAX_NUM;
}
