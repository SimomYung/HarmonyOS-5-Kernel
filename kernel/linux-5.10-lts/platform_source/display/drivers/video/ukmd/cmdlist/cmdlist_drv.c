...
static uint32_t g_cmdlist_dev_id;

static struct cmdlist_private *cmdlist_get_priv_by_name(const char *dev_name)
{
	uint32_t dev_id;
	struct cmdlist_private *cmd_priv;

	if (unlikely(!dev_name)) {
		ukmd_pr_err("dev_name is NULL");
		return NULL;
	}

	for (dev_id = 0; dev_id < CMDLIST_DEV_MAX_NUM; ++dev_id) {
		cmd_priv = &g_cmdlist_privs[dev_id];
		if (cmd_priv->device_initialized == 0)
			continue;

		if (cmd_priv->of_dev && cmd_priv->of_dev->of_node && cmd_priv->of_dev->of_node->name)
			if (strcmp(dev_name, cmd_priv->of_dev->of_node->name) == 0)
				return cmd_priv;
	}
...
{
	struct cmdlist_private *cmd_priv = cmdlist_get_priv_by_name(dev_name);
	if (!cmd_priv)
		return CMDLIST_DEV_MAX_NUM;

	return cmd_priv->dev_id;
}
...
static int32_t cmdlist_of_device_setup(struct platform_device *pdev)
{
	...
	ukmd_pr_info("get scene_num: %d", cmd_priv->cmd_table.scene_num);

	(void)of_property_read_u32(cmd_priv->of_dev->of_node, "cmdlist_max_size", (uint32_t *)&cmd_priv->sum_pool_size);
	ukmd_pr_info("get cmdlist_max_size: %d", cmd_priv->sum_pool_size);
	 ...
	 */
	cmd_priv->memory_pool = devm_gen_pool_create(cmd_priv->of_dev, 4 /* order */, -1,
		cmd_priv->of_dev->of_node->name);
	if (IS_ERR_OR_NULL(cmd_priv->memory_pool)) {
		ukmd_pr_err("create memory pool failed %d!", PTR_ERR(cmd_priv->memory_pool));
...
static int32_t cmdlist_probe(struct platform_device *pdev)
{
	...
	ukmd_pr_info("+");
	cmd_priv = cmdlist_get_priv_by_name(pdev->dev.of_node->name);
	if (!cmd_priv) {
		if (unlikely(g_cmdlist_dev_id >= CMDLIST_DEV_MAX_NUM)) {
			ukmd_pr_err("g_cmdlist_dev_id %u exceeds array size!", g_cmdlist_dev_id);
			return -EINVAL;
		}

		dev_id = g_cmdlist_dev_id;
		cmd_priv = &g_cmdlist_privs[g_cmdlist_dev_id++];
	}
	...
	cmdlist_client_init_cmdlist_id(&cmd_priv->cmd_table);
	cmd_priv->device_initialized = 1;
	cmd_priv->dev_id = dev_id;
	INIT_LIST_HEAD(&cmd_priv->user_info_head);
	ukmd_pr_info("-");
