...
static int ffts_rdr_register_exception(void)
{
	...
	for (index = 0; index < size; index++) {
		...
		if (ret == 0) {
			ffts_drv_err("register rdr exception is failed! index = %ld, ret = %d",
				index, ret);
			return -EINTR;
		}
	}
...
static int ffts_rdr_unregister_exception(void)
{
	...
	for (index = 0; index < size; index++) {
		...
		if (ret < 0) {
			ffts_drv_err("unregister rdr exception is failed! index = %ld, ret = %d",
				index, ret);
			return -EINTR;
		}
	}
...
void ffts_rdr_exception_report(uint32_t error_code)
{
	if (ffts_rdr_exception_is_count_exceeding(error_code) == 0) {
		...
		// output to file
		ffts_dump_to_file("/data/ffts_bbox.bin", g_ffts_mntn_info.rdr_addr, g_ffts_mntn_info.bbox_addr_offset);
#else 
