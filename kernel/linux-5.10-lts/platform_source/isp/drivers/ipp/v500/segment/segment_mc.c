	...
	global_info_ipp_t *global_info)
{
	...
	for (i = 0; i < cmdlst_para->stripe_cnt;) {
		...
		if (global_info->mc_en == 1) {
			...
			global_info->mc_coordinate_addr = seg_cfg_tab->mc_cfg_tab.mc_coordinate_addr;
		}
		...
		if (mc_req->mc_stat_buff[MC_COORDINATE_PAIRS_BUFF].buffer) {
			...
			cmdlst_wr_cfg.reg_rd_addr = IPP_BASE_ADDR_MC + MC_COORDINATE_CFG_0_REG;
		...
		if (mc_req->mc_stat_buff[MC_LOW_KPT_BUFF].buffer) {
			...
			cmdlst_wr_cfg.reg_rd_addr = IPP_BASE_ADDR_MC + MC_LOW_KPT_STATUS_REG;
			loge_if(seg_ipp_set_cmdlst_wr_buf(&cmd_link_entry[i++].cmd_buf, &cmdlst_wr_cfg));
		}
	}
...
static void seg_mc_dump_req(msg_req_mc_t *mc_req)
{
	...
	logi("cfg_mode=%d, push_inliers_en=%d",
		reg_cfg->ctrl_cfg.bits.cfg_mode, reg_cfg->ctrl_cfg.bits.push_inliers_en);
	...
	logi("mc_static_cfg: matched_num_th=%d",
		reg_cfg->static_cfg.bits.matched_num_th);
	...
	memcpy_s(tmp_arr, sizeof(tmp_arr), reg_cfg->coord_pairs_cfg.coordinate_cur_ref_pairs, sizeof(tmp_arr));
	log_dump_array("cur_ref_coordinate = ", "0x%08x, ", tmp_arr, sizeof(tmp_arr)/sizeof(tmp_arr[0]), 8);
