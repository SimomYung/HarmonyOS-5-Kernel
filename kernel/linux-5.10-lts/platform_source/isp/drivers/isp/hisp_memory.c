...
static void hisp_dump_mem_info(struct hisp_mem_info_s *mem)
{
	...
	pr_debug("[%s] cpu addr=0x%x, size=0x%x\n",
...
static void hisp_free_sgt_from_size(struct hisp_mem_pool_info_list_s *list_info)
{
	...
	if (list_info->table != NULL) {
		hisp_free_dynmem_sg_table(list_info->table);
		list_info->table = NULL;
	}
}
...
static int hisp_memory_unmap_ispdma(size_t addr, size_t size)
{
	...
	if (len != size) {
		pr_err("%s: iommu_unmap failed: addr 0x%lx\n", __func__, addr);
		return -EINVAL;
	}
					  ...
					  unsigned long size)
{
	...
	if (ret) {
		pr_err("[%s] iommu unmap failed! iova.0x%lx, size.0x%x\n",
			__func__, addr, size);
		mutex_unlock(&dev->hisp_map_mutex);
		return -EINVAL;
	}
			   ...
			   struct hisp_mem_pool_info_list_s *list_info)
{
	...
	if (master != OISP_CPU) {
		...
		pr_debug("[%s] map size.0x%x, prot.0x%x, sgl.0x%p\n",
	...
	pr_debug("[%s] mapped addr.0x%lx, size.0x%x, master.0x%x\n",
		__func__, addr, size, master);
			     ...
			     struct hisp_mem_pool_info_list_s *list_info)
{
	...
	if (master != OISP_CPU) {
		...
		pr_debug("[%s] unmap addr.0x%lx, size.0x%lx, master.0x%x\n",
			__func__, addr, size, master);
				...
				struct hisp_map_info_s *info)
{
	...
	pr_debug("[%s] type.0x%x paddr.0x%x size.0x%x prot.0x%x\n", __func__,
		*(unsigned int *)(uintptr_t)&type,
		info->paddr, info->size, info->prot);
		      ...
		      struct hisp_map_info_s *info, struct hisp_mem_info_s *mem)
{
	...
	pr_debug("[%s] type[0x%x] map addr start\n",
		__func__, *((unsigned int *)(uintptr_t)&type));
				  ...
				  unsigned long long addr, unsigned int size)
{
	...
	if ((use_mempool == 0) || (addr == 0) || (size == 0)) {
		pr_err("[%s] Invalid args use_mempool.%u add.0x%x size.0x%x\n",
