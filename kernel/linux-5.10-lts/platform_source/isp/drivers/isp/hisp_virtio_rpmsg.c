			...
			u32 src, u32 dst, void *data, int len, bool wait)
{
	...
	while (!msg) {
		...
		err = (int)wait_event_interruptible_timeout(vrp->sendq,
					(msg = hisp_vr_txbuf_get(vrp)),
	...
	msg->len = cpu_to_virtio16(vrp->vdev, (unsigned short)len);
	msg->flags = 0;
	msg->src = cpu_to_virtio32(vrp->vdev, src);
	msg->dst = cpu_to_virtio32(vrp->vdev, dst);
	msg->reserved = 0;
	err = memcpy_s(msg->data, (unsigned long)len, data, (unsigned long)len);
	if (err)
			     ...
			     struct rpmsg_hdr *msg, unsigned int len)
{
	...
	dev_dbg(dev, "From: 0x%x, To: 0x%x, Len: %u, Flags: %d, Reserved: %d\n",
	...
	if (len > vrp->buf_size ||
		...
		dev_warn(dev, "inbound msg too big: (%u, %u)\n", len, msg_len);
		return -EINVAL;
	}
	...
	if (ept) {
		...
		if (hisp_msg_service_recv(msg->data, msg_len,\
				virtio32_to_cpu(vrp->vdev, msg->src)) == ISPMSG_TYPE_OTHER) {
			if (ept->cb)
				ept->cb(ept->rpdev, msg->data, (int)msg_len, ept->priv,\
					virtio32_to_cpu(vrp->vdev, msg->src));
		}
		mutex_unlock(&ept->cb_lock);
