...
static int write_sh_dump_history(union dump_info dump_info)
{
	...
	if (g_dp_regs != NULL) {
		...
		print_ret = snprintf_s(buf, sizeof(buf), sizeof(buf) - 1,
			"reason [%s], [%02u], time [%s],dump_info[0x%x], boxid[0x%x]",
			content, g_dump_index, date, dump_info.value, boxid);
		if (print_ret < 0)
