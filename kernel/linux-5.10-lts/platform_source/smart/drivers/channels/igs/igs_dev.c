...
struct kfifo g_fdul_exception_kfifo;
STATIC int g_is_fdul_self_cure_failure = 0;
#endif
...
STATIC int ioctl_read_buffer(struct file *file, unsigned int cmd, unsigned long arg);
...
#endif
STATIC int igs_pm_notify(struct notifier_block *nb, unsigned long mode, void *_unused);
...
STATIC struct igs_ioctl_ops g_igs_ioctl_ops[] = {
	...
	{IGS_IOCTL_READ_BUFFER, ioctl_read_buffer},
#ifdef CONFIG_CONTEXTHUB_IGS_20
...
struct igs_hall_notify_handle_work {
	struct work_struct worker;
	unsigned int hall_status;
};
...
struct igs_camera_with_product {
	char product_name[40];
	enum product_name product_name_id;
	unsigned int camera_para;
};

#define IGS_READ_CACHE_COUNT          5
#define IGS_SYSCAHCE_ID               8
#define IGS_IOCTL_WAIT_TMOUT          2000 /* ms */
...
static struct igs_camera_with_product  g_igs_product_info[] = {
	{"LEM", LEM, (FRONT_CAMERA << 24) | (REAR_CAMERA << 16) | (REAR_CAMERA << 8) | NO_CAMERA},
	{"Alta", ALB, (SUB_FRONT_CAMERA << 24) | (REAR_CAMERA << 16) | (FRONT_CAMERA << 8) | REAR_CAMERA},
	{"ICL", ICL, (SUB_FRONT_CAMERA << 24) | (REAR_CAMERA << 16) | (FRONT_CAMERA << 8) | REAR_CAMERA},
	{"VDE", VDE, (FRONT_CAMERA << 24) | (REAR_CAMERA << 16) | (REAR_CAMERA << 8) | NO_CAMERA}
};
...
static unsigned int g_igs_sensor_para = 0;
...
#ifdef CONFIG_CONTEXTHUB_IGS_VOLTAGE_CURE
#define NPU_SH_NUM                   529
#define NPU_SH_LEN                   1
#define FDUL_SH_NUM                  530
#define FDUL_SH_LEN                  1
#define MAX_FDUL_EXCEPTION           (5)
#define ONE_MINUTE                   60 * 1000000000LL
#define IGS_VOLTAGE_SELF_CURE        1
#define IGS_VOLTAGE_SELF_NOT_CURE    0


int igs_lite_nve_read(unsigned char *data)
{
	struct opt_nve_info_user igs_lite_nv_read_info = {0};
	int ret;

	if (data == NULL) {
		igs_log_err("igs_lite_nve_read:invalid data\n");
		return -1;
	}

	ret = strncpy_s(igs_lite_nv_read_info.nv_name, sizeof("NPU_SH"), "NPU_SH", (sizeof("NPU_SH") - 1));
	if (ret != 0) {
		igs_log_err("igs_lite_nve_read:strncpy_s fail\n");
		return -1;
	}

	igs_lite_nv_read_info.nv_number = NPU_SH_NUM;
	igs_lite_nv_read_info.valid_size = NPU_SH_LEN;
	igs_lite_nv_read_info.nv_operation = NV_READ;
 
	ret = nve_direct_access_interface(&igs_lite_nv_read_info);
	if (ret == 0) {
		*data = igs_lite_nv_read_info.nv_data[0];
		igs_log_warn("igs_lite_nve_read: nv read 0x%x success!\n",
			    igs_lite_nv_read_info.nv_data[0]);
		return 0;
	}

	igs_log_err("igs_lite_nve_read: nv read faild!\n");

	return -1;
}
 
int igs_tiny_nve_read(unsigned char *data)
{
	struct opt_nve_info_user igs_tiny_nv_read_info = {0};
	int ret;

	if (data == NULL) {
		igs_log_err("igs_tiny_nve_read:invalid data\n");
		return -1;
	}

	ret = strncpy_s(igs_tiny_nv_read_info.nv_name, sizeof("NPU_SH"), "NPU_SH", (sizeof("NPU_SH") - 1));
	if (ret != 0) {
		igs_log_err("igs_tiny_nve_read:strncpy_s fail\n");
		return -1;
	}

	igs_tiny_nv_read_info.nv_number = FDUL_SH_NUM;
	igs_tiny_nv_read_info.valid_size = FDUL_SH_LEN;
	igs_tiny_nv_read_info.nv_operation = NV_READ;

	ret = nve_direct_access_interface(&igs_tiny_nv_read_info);
	if (ret == 0) {
		if (data != NULL)
			*data = igs_tiny_nv_read_info.nv_data[0];
		igs_log_warn("igs_tiny_nve_read: nv read 0x%x success!\n",
			    igs_tiny_nv_read_info.nv_data[0]);
		return 0;
	}

	igs_log_err("igs_tiny_nve_read: nv read faild!\n");

	return -1;
}
 
int igs_tiny_nve_write(const unsigned char *data)
{
	int ret;
	struct opt_nve_info_user igs_tiny_nv_write_info = {0};

	if (data == NULL) {
		igs_log_err("igs_tiny_nve_write: data is null\n");
		return -1;
	}

	ret = strncpy_s(igs_tiny_nv_write_info.nv_name, sizeof("NPU_SH"), "NPU_SH", (sizeof("NPU_SH") - 1));
	if (ret != 0) {
		igs_log_err("igs_tiny_nve_write: strncpy_s fail\n");
		return -1;
	}

	igs_tiny_nv_write_info.nv_number = FDUL_SH_NUM;
	igs_tiny_nv_write_info.valid_size = FDUL_SH_LEN;
	igs_tiny_nv_write_info.nv_operation = NV_WRITE;

	igs_tiny_nv_write_info.nv_data[0] = *data;

	ret = nve_direct_access_interface(&igs_tiny_nv_write_info);
	if (ret == 0) {
		igs_log_warn("igs_tiny_nve_write: nv write 0x%x success!\n",
			    igs_tiny_nv_write_info.nv_data[0]);
		return 0;
	}

	igs_log_err("igs_tiny_nve_write: nv write faild!\n");
...
void igs_tiny_vlotage_cure(void)
{
	int64_t timestamp = 0;
	int64_t timestamp_out = 0;
	char tiny_exception = 0;
	char lite_exception = 0;
	struct igs_config *igs;

	if (g_is_fdul_self_cure_failure == 1)
		return;
	timestamp = get_time_stamp();
	if (kfifo_len(&g_fdul_exception_kfifo) / sizeof(int64_t) != MAX_FDUL_EXCEPTION) {
		if (kfifo_in(&g_fdul_exception_kfifo, (unsigned char *)&timestamp,
			(unsigned int)sizeof(int64_t)) == 0)
			igs_log_err("%s: kfifo_in failed\n", __func__);
	}
	if (kfifo_len(&g_fdul_exception_kfifo) / sizeof(int64_t) == MAX_FDUL_EXCEPTION) {
		if (kfifo_out(&g_fdul_exception_kfifo,\
			(unsigned char *)&timestamp_out, (unsigned int)sizeof(int64_t)) == 0)
			igs_log_err("%s: kfifo_out failed\n", __func__);
	
		if ((timestamp - timestamp_out) < ONE_MINUTE) {
			igs_tiny_nve_read(&tiny_exception);
			if (tiny_exception == IGS_VOLTAGE_SELF_CURE) {
				g_is_fdul_self_cure_failure = 1;
				tiny_exception = IGS_VOLTAGE_SELF_NOT_CURE;
				igs_tiny_nve_write(&tiny_exception);
				igs = iomcu_config_get(IOMCU_CONFIG_USR_IGS, sizeof(struct igs_config));
				igs->is_tiny_voltage_increase = IGS_VOLTAGE_SELF_NOT_CURE;
				/* DMD */
				bbox_diaginfo_record(DMD_FDUL_SH_PV_CNT, NULL, "fdul sh pv: cancel");
			} else {
				tiny_exception = IGS_VOLTAGE_SELF_CURE;
				igs_tiny_nve_write(&tiny_exception);
				igs = iomcu_config_get(IOMCU_CONFIG_USR_IGS, sizeof(struct igs_config));
				igs->is_tiny_voltage_increase = IGS_VOLTAGE_SELF_CURE;
				/* DMD */
				bbox_diaginfo_record(DMD_FDUL_SH_PV_CNT, NULL, "fdul sh pv: enable");
			}
			kfifo_reset(&g_fdul_exception_kfifo);
		}
	} else {
		igs_lite_nve_read(&lite_exception);
		if (lite_exception == IGS_VOLTAGE_SELF_CURE) {
			igs = iomcu_config_get(IOMCU_CONFIG_USR_IGS, sizeof(struct igs_config));
			igs->is_tiny_voltage_increase = IGS_VOLTAGE_SELF_CURE;
		}
	}
...
STATIC int ext_hall_data_compare(int hall_val, unsigned int comp_val)
{
	...
	bit_tmp = (int)(((unsigned int)hall_val >> comp_val) & 0x01);
	return bit_tmp;
}
...
STATIC int ioctl_fusion_en(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	igs_tiny_nve_read(&tiny_exception);
	igs = iomcu_config_get(IOMCU_CONFIG_USR_IGS, sizeof(struct igs_config));
	if (g_is_fdul_self_cure_failure == 0) {
		if ((lite_exception == IGS_VOLTAGE_SELF_CURE) || (tiny_exception == IGS_VOLTAGE_SELF_CURE))
			igs->is_tiny_voltage_increase = IGS_VOLTAGE_SELF_CURE;
		else
			igs->is_tiny_voltage_increase = IGS_VOLTAGE_SELF_NOT_CURE;
	}
...
STATIC int ioctl_igs_sensor_sets(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	igs_log_info("%s:send sensor sets, sensor_type:%d, sensor_para1:%d, sensor_para2:%d\n",
		__func__, fs.set.sensor_type, fs.set.sensor_para1, fs.set.sensor_para2);
...
static void get_product_name_from_dts(void)
{
	int i;
	...
	igs = iomcu_config_get(IOMCU_CONFIG_USR_IGS, sizeof(struct igs_config));
	igs->camera_idx1 = FRONT_CAMERA;
	igs->camera_idx2 = REAR_CAMERA;
	g_igs_sensor_para = (FRONT_CAMERA << 24) | (REAR_CAMERA << 16) | (FRONT_CAMERA << 8) | REAR_CAMERA;
	igs->product_name = BA;
	...
	if (of_property_read_string(sensorhub_node, "hisi,product_name", &product_name) != 0) {
		hwlog_err("read hisi,product_name from dts fail\n");
		return;
	}
	hwlog_info("%s: product_name is %s\n", __func__, product_name);

	for (i = 0; i < ARRAY_SIZE(g_igs_product_info); i++) {
		if (strncmp(product_name, g_igs_product_info[i].product_name, strlen(g_igs_product_info[i].product_name)) == 0) {
			g_igs_sensor_para = g_igs_product_info[i].camera_para;
			igs->product_name = g_igs_product_info[i].product_name_id;
			return;
		}
	}
...
STATIC int ioctl_read_buffer(struct file *file, unsigned int cmd, unsigned long arg)
{
#ifdef CONFIG_CONTEXTHUB_IGS_SC_READ_BUFFER
	struct igs_read_buffer_param_t ap_input = {0};
	if (copy_from_user((void *)&ap_input, (void *)((uintptr_t)arg), sizeof(ap_input))) {
		igs_log_err("[%s]copy_from_user error\n", __func__);
		return -EFAULT;
	}
	if (ap_input.user_buffer == 0) {
		igs_log_err("%s user buffer is NULL\n", __func__);
		return -EFAULT;
	}
	if (ap_input.length >= (u32)0xFFFFFFFF - ap_input.bias ||
		ap_input.length + ap_input.bias >= DDR_IGS_SCAN_CODE_SIZE) {
		igs_log_err("[%s]input bias[%u] or length[%u] error, length_limit[%u].\n",
		    __func__, ap_input.bias, ap_input.length, DDR_IGS_SCAN_CODE_SIZE);
		return -EFAULT;
	}
	void *image_addr = ioremap(DDR_IGS_SCAN_CODE_BASE_AP + ap_input.bias, ap_input.length);
	if (image_addr == NULL) {
		igs_log_err("%s failed to ioremap_nocache\n", __func__);
		iounmap(image_addr);
		return -EFAULT;
	}
	if (copy_to_user((char __user *)((uintptr_t)ap_input.user_buffer), image_addr, ap_input.length)) {
		igs_log_err("%s failed to copy to user\n", __func__);
		iounmap(image_addr);
		return -EFAULT;
	}
...
#ifdef CONFIG_ION
		ret = secmem_get_buffer(ion_info.fd,
			&table, &ion_info.id, &type);
#elif defined CONFIG_DMABUF_HEAPS
		ret = mm_dma_heap_secmem_get_buffer(ion_info.fd,
			&table, &ion_info.id, &type);
#else
		ret = -ENOTTY;
		igs_log_err("[%s] : get no buffer.\n", __func__);
#endif
	if (ret != 0) {
...
#ifdef CONFIG_CONTEXTHUB_IGS_30
static bool is_igs_ion_empty(void)
{
	bool empty = list_empty(&g_igs_priv.ion_buf_list);
	igs_log_info("[%s] is empty %d\n", __func__, empty);
	return empty;
}
#endif
...
STATIC int igs_get_shb_dmd_report(const struct pkt_header *head)
{
	...
	igs_log_info("igs get shb dmd report: dmd_id[%u], dmd_case[0x%x], length[%u], tranid[%u], para_num [%u]\n",
		       req->dmd_id, req->dmd_case, req->hd.length, req->hd.tranid, req->info_num);
	...
	if (req->info_num != dmd_log[index].dmd_para_num) {
		igs_log_err("igs get shb dmd report: info_num[%u] not equal to dmd_para_num[%u]!! req->dmd_id[%u].\n",
			      req->info_num, dmd_log[index].dmd_para_num, req->dmd_id);
		return -EFAULT;
	}

	/* req->info_num: info[] arry size */
	switch (req->info_num) {
	case 0:
...
STATIC int igs_dev_release(struct inode *inode, struct file *filep)
{
	struct read_info rd;
#ifdef CONFIG_CONTEXTHUB_IGS_20
	struct igs_ion_node *cur = NULL;
...
static void igs_hall_notifier_handler(struct work_struct *work)
{
	struct igs_config *igs = NULL;
	if (work == NULL) {
		igs_log_err("[%s] work NULL\n", __func__);
		return;
	}
	igs = iomcu_config_get(IOMCU_CONFIG_USR_IGS, sizeof(struct igs_config));

	struct igs_hall_notify_handle_work *p = container_of(work, struct igs_hall_notify_handle_work, worker);
	struct igs_set_sensor_param_t fs = { {0} };
	fs.set.sensor_type = 0;
	if (p == NULL) {
		igs_log_err("igs_hall_notify_handle_work is NULL\n");
		return;
	}

	switch (p->hall_status) {
	case HALL_UNFOLD_STATUS: /* change finished */
		fs.set.sensor_para1 = (g_igs_sensor_para >> 24) & 0xFF;
		fs.set.sensor_para2 = (g_igs_sensor_para >> 16) & 0xFF;
		break;
	case HALL_FOLD_STATUS:
		fs.set.sensor_para1 = (g_igs_sensor_para >> 8) & 0xFF;
		fs.set.sensor_para2 = (int8_t)(g_igs_sensor_para & 0xFF);
		break;
	default:
		igs_log_err("unknown action%d unknown in %s\n", p->hall_status, __func__);
		goto FREE;
	}	

	igs->camera_idx1 = fs.set.sensor_para1;
	igs->camera_idx2 = fs.set.sensor_para2;	
	...
	if (send_cmd_from_kernel(TAG_IGS, CMD_CMN_CONFIG_REQ, SUB_CMD_IGS_SENSOR_SETS,
		...
		igs_log_err("[%s]send cmd error\n", __func__);
		mutex_lock(&g_igs_priv.wklock_mutex);
		g_igs_priv.send_ipc_fail_flag = 1;
		mutex_unlock(&g_igs_priv.wklock_mutex);
	}
FREE:
	kfree(p);
...
static int igs_hall_notifier(struct notifier_block *nb, unsigned long action, void *data)
{
	struct igs_hall_notify_handle_work *notifier_work = NULL;

	notifier_work = kzalloc(sizeof(struct igs_hall_notify_handle_work), GFP_KERNEL);
	if (!notifier_work) {
		igs_log_err("[%s] alloc work error is err.\n", __func__);
		return NOTIFY_OK;
	}
	notifier_work->hall_status = (unsigned int)ext_hall_data_compare(action, EXT_HALL_BIT_HINGLE);

	INIT_WORK(&notifier_work->worker, igs_hall_notifier_handler);
	queue_work(g_igs_priv.notifier_wq, &notifier_work->worker);

	return NOTIFY_OK;
}

static struct notifier_block sensorhub_hall = {
	.notifier_call = igs_hall_notifier,
};
...
static struct notifier_block g_igs_pm_notify = {
	.notifier_call = igs_pm_notify,
	.priority = 0
};
...
STATIC int __init igs_dev_init(struct platform_device *pdev)
{
...
#ifndef CONFIG_CONTEXTHUB_IGS_30
	igs_log_info("%s: register hall\n", __func__);
	ret = ext_hall_register_notifier(&sensorhub_hall);
	if (ret != 0) {
		igs_log_err("%s: unable to register ext_hall_notifier: %d", __func__, ret);
		misc_deregister(&igs_miscdev);
		return ret;
	}
#endif
	ret = register_pm_notifier(&g_igs_pm_notify);
	if (ret != 0) {
		igs_log_err("%s: unable to register g_igs_pm_notify: %d", __func__, ret);
		misc_deregister(&igs_miscdev);
		return ret;
	}
	...
	g_igs_priv.notifier_wq = alloc_workqueue("igs_hall_notify", WQ_UNBOUND, 0);
	...
	ret = register_iom3_recovery_notifier(REC_USR_IGS, &igs_reboot_notify);
	if (ret < 0) {
	...
	g_igs_priv.send_ipc_to_shb = 0;
	g_igs_priv.send_ipc_fail_flag = 0;
...
static int igs_pm_resume(struct device *dev)
{
	...
	igs_log_info("[%s] send syscache disable ipc.\n", __func__);
...
STATIC int igs_pm_notify(struct notifier_block *nb, unsigned long mode, void *_unused)
{
	...
	fs.set.sensor_para1 = igs->camera_idx1;
	fs.set.sensor_para2 = igs->camera_idx2;
	switch (mode) {
	case PM_POST_SUSPEND: /* resume */
		igs_log_info("resume in %s\n", __func__);
		mutex_lock(&g_igs_priv.igs_mutex);
		igs_log_info("get igs_mutex in %s\n", __func__);
		if (g_igs_priv.send_ipc_fail_flag != 0) {
			igs_log_info("[%s] resend ipc.\n", __func__);
			(void)send_cmd_from_kernel(TAG_IGS, CMD_CMN_CONFIG_REQ, SUB_CMD_IGS_SENSOR_SETS,
					(char *)(&fs.set), sizeof(struct igs_set_sensor_t));
			g_igs_priv.send_ipc_fail_flag = 0;
		}
		mutex_unlock(&g_igs_priv.igs_mutex);
...
#ifdef CONFIG_CONTEXTHUB_IGS_30
static int igs_pm_freeze(struct device *dev)
{
	struct device_node *sensorhub_node = NULL;
	const char *product_name = NULL;
	char product_name_for_s4[] = "MRO";
	igs_log_info("PM:[%s] enter.\n",__func__);
	sensorhub_node = of_find_compatible_node(NULL, NULL, "hisilicon,vendor");
	if (!sensorhub_node) {
		igs_log_err("%s, can't find node sensorhub\n", __func__);
		return false;
	}
	if (of_property_read_string(sensorhub_node, "hisi,product_name", &product_name) != 0) {
		igs_log_err("read hisi,product_name from dts fail\n");
		return false;
	}
	if (strncmp(product_name, product_name_for_s4, strlen(product_name_for_s4)) == 0)
		return is_igs_ion_empty() ? false : true;
	return false;
}
#endif
...
{
	get_npu_dts_status();
	get_product_name_from_dts();
#ifdef CONFIG_CONTEXTHUB_IGS_VOLTAGE_CURE
	int ret = kfifo_alloc(&g_fdul_exception_kfifo,
		sizeof(int64_t) * MAX_FDUL_EXCEPTION, GFP_KERNEL | __GFP_ZERO);
	if (ret)
