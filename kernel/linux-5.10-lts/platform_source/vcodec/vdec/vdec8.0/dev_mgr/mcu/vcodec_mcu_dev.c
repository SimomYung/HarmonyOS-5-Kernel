...
static int32_t vcodec_mcu_alloc_buffer(struct vcodec_dev_mcu* dev)
{
	...
	if (ret) {
		dprint(PRN_ERROR, "alloc mcu exec memory failed, size:%u", VDEC_MCU_SIZE);
	...
	if (ret) {
		dprint(PRN_ERROR, "alloc msg pool memory failed, size:%u", VDEC_MSG_POOL_SIZE);
		goto msg_pool_fail;
	}
...
const struct vcodec_mcu_control_proc_tbl *vcodec_mcu_dev_get_control_proc_tbl(void)
{
	static struct vcodec_mcu_control_proc_tbl tbl[] = {
		...
		{ VCODEC_DEV_MCU_INIT_CHAN_OUTPUT_EVENT_QUEUE, sizeof(int32_t),
