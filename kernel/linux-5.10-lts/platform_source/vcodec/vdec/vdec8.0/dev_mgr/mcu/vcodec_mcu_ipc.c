...
static void ipc_handle_alloc_segbuffer(void)
{
	ipc_alloc_mem mem = { 0 };
	uint32_t iova;
    int ret;
	...
	dprint(PRN_ALWS, "alloc seg buffer spend time: %lld us", vcodec_osal_get_time_in_us() - temp);
	ipc_ack_to_mcu(IPC_MSG_TYPE_ALLOC_SEG_BUFFER, &mem, sizeof(ipc_alloc_mem));
}

static void ipc_handle_free_segbuffer(uint32_t iova)
{
	int32_t ret;

	ret = vdec_free_segbuffer(iova);
	ipc_ack_to_mcu(IPC_MSG_TYPE_FREE_SEG_BUFFER, &ret, sizeof(int32_t));
}
...
static void process_msg(recv_msg *packet)
{
	...
	case IPC_MSG_TYPE_ALLOC_SEG_BUFFER: {
		dprint(PRN_DBG, "process cmd IPC_MSG_TYPE_ALLOC_SEG_BUFFER");
		ipc_handle_alloc_segbuffer();
		break;
	}
	case IPC_MSG_TYPE_FREE_SEG_BUFFER: {
		uint32_t free_iova = *(uint32_t *)(packet->buffer);
		dprint(PRN_DBG, "process cmd IPC_MSG_TYPE_FREE_SEG_BUFFER, addr is 0x%x", free_iova);
		ipc_handle_free_segbuffer(free_iova);
		break;
	}
	default: {
