...
int32_t vcodec_pxp_alloc_syscache(struct vcodec_dev_pxp* dev)
{
	...
	ret = vdec_alloc_dma_and_map(dev->device, VDEC_PXPC_SC_SIZE, &dev->pxp_sc);
#else
	if (dev->syscache_enable)
		ret = vdec_mem_alloc_with_sc(dev->device, VDEC_PXPC_SC_SIZE, &dev->pxp_sc);
	...
	dev->buffer_info.syscache_len = VDEC_PXPC_SC_SIZE;
