...
#define DATA_QIC_DBG_START_ADDR 0x00420000
...
static void vdec_qic_irq_clear(uint32_t start_addr)
{
	...
	wr_vreg(vdec->reg_base_vir_addr, (start_addr + 0x000C), 1);
}
...
{
	vdec_qic_irq_info(QIC_CORE_DATA_TYPE, DATA_QIC_DBG_START_ADDR);
	vdec_qic_irq_clear(DATA_QIC_DBG_START_ADDR);
}
...
irqreturn_t vdec_others_isr(int irq, void *id)
{
	...
	if (vdec->qic_irq_cnt >= 10) {
		wr_vreg(vdec->reg_base_vir_addr, 0x4080B4, 0xFFFFFFFF);
