...
static const char *vctrl_cmd_name(vfmw_cid cmd)
{
	const char *name[] = {
		...
		"VFMW_CMD_IPC_RESUME",
		"VFMW_CMD_MCORE_SUSPEND",
		"VFMW_CMD_MCORE_RESUME",
		"VFMW_CID_SET_SINGLE_FRAME_MEM",
	};
	...
	vdec_ioctl_msg *vdec_msg)
{
	...
	check_return(file_handle->private_data, "file handle private data is null");

	chan_list = &handle->chanlist;
	...
	dprint(PRN_ALWS, "IPC_MSG_TYPE_VCTRL_COMMAND chanid %d cmdid %d",
		vctrl_info->chanId, vctrl_info->cmdId);
	...
	dprint(PRN_DBG, "end VDEC_IOCTL_IPC_VCTL_CMD");
...
{
	int32_t ret = -EINVAL;
	int32_t mapping_chan_id;
	vdec_entry *vdec = vdec_get_entry();
	ipc_vctrl_ioctl *vctrl_info;
	struct ipc_vctrl_msg msg;
	struct dec_file_handle *handle;
	struct vdec_chan_list *chan_list;
    ipc_vctrl_ret_mcu_to_acpu vctrl_ret = {
        .res = -1,
        .chan_id = -1,
	...
	check_return(file_handle->private_data, "file handle private data is null");
	handle = (struct dec_file_handle *)file_handle->private_data;
	chan_list = &handle->chanlist;
	...
	if (vctrl_ret.chan_id < VFMW_NO_SEC_CHAN_START || vctrl_ret.chan_id >= VFMW_CHAN_ID_MAX) {
		dprint(PRN_FATAL, "chan id error %d range is [2, 17]", vctrl_ret.chan_id);
		ret = -EFAULT;
		goto mem_error;
	}

	// chan_id 2-17 map to 0-15 shared output event queue buffer
	mapping_chan_id = vctrl_ret.chan_id - VFMW_NO_SEC_CHAN_START;
	// init output event queue of the corresponding channel
	(void)vdec->mcu_dev->control(vdec->mcu_dev,
		VCODEC_DEV_MCU_INIT_CHAN_OUTPUT_EVENT_QUEUE, &mapping_chan_id, sizeof(int32_t));

	if (vdec_chan_list_add(chan_list, vctrl_ret.chan_id)) {
		dprint(PRN_FATAL, "add chan_id % to list failed", vctrl_ret.chan_id);
		ret = -EFAULT;
		goto add_chan_list_error;
	}

	vdec->vdec_chan_state[vctrl_ret.chan_id - VFMW_NO_SEC_CHAN_START] = VFMW_CID_CREATE_CHAN;
	if (copy_to_user(vdec_msg->out, &vctrl_ret, sizeof(vctrl_ret))) {
		dprint(PRN_FATAL, "copy_to_user failed");
		ret = -EFAULT;
		goto copy_to_user_failed;
	}
	dprint(PRN_DBG, "end VDEC_IOCTL_IPC_CREATE_CHAN");
	vfree(vctrl_info);
	return ret;
copy_to_user_failed:
	vdec->vdec_chan_state[vctrl_ret.chan_id - VFMW_NO_SEC_CHAN_START] = VFMW_CID_MAX;
	(void)vdec_chan_list_remove(chan_list, (int32_t)vctrl_info->chanId);
add_chan_list_error:
	handle_add_chanlist_failed(vdec, vctrl_info, vctrl_ret.chan_id);
mem_error:
	vfree(vctrl_info);
exit:
	return ret;
...
static int32_t handle_ipc_destory_chan(struct file *file_handle, vdec_ioctl_msg *vdec_msg)
{
	...
	vdec->vdec_chan_state[vctrl_info->chanId - VFMW_NO_SEC_CHAN_START] = VFMW_CID_DESTROY_CHAN;
	if (copy_to_user(vdec_msg->out, &vctrl_ret, sizeof(vctrl_ret))) {
		dprint(PRN_FATAL, "copy_to_user failed");
		ret = -EFAULT;
		goto mem_error;
	}
	dprint(PRN_DBG, "end VDEC_IOCTL_IPC_DESTROY_CHAN");
	...
	vdec_ioctl_msg *vdec_msg)
{
	...
	if (!vdec_chan_list_get(chan_list, stm_info.chanId)) {
		dprint(PRN_FATAL, "chanid %d is ilegal", stm_info.chanId);
		return -EFAULT;
	}
	...
	dprint(PRN_DBG, "send QUEUE_MSG_TYPE_STREAM_SYNC, in size is %d", sizeof(stm_info));
	ret = vdec->mcu_dev->control(vdec->mcu_dev,
		VCODEC_DEV_MCU_SEND_STREAM_MSG, &stm_info, sizeof(stream_ioctl));
	if (ret)
		return ret;

	dprint(PRN_DBG, "end VDEC_IOCTL_IPC_CONFIG_INPUTBUFFER");
	return 0;
}
...
static int32_t vdec_get_unsupport_event(struct dec_file_handle *handle, event_output_list *output_list)
{
	...
	list_for_each_entry(chan_pos, &chan_list->chan_head, head) {
		if (vdec->vdec_chan_state[chan_pos->chan_id - VFMW_NO_SEC_CHAN_START] == VFMW_CID_CREATE_CHAN) {
			is_need_report = true;
	...
	vdec_ioctl_msg *vdec_msg)
{
	...
	dprint(PRN_DBG, "start VDEC_IOCTL_IPC_PROCESS_EVENT");
	...
	dprint(PRN_DBG, "end VDEC_IOCTL_IPC_PROCESS_EVENT, event_cnt:%d",
	...
	struct file *file_handle, vdec_ioctl_msg *vdec_msg)
{
	...
	dprint(PRN_DBG, "chanid %d", in.chan_id);
	ret = vdec->mcu_dev->control(vdec->mcu_dev,
		VCODEC_DEV_MCU_SEND_IMAGE_MSG, &in, sizeof(image_ioctl));
	if (ret)
		return ret;

	dprint(PRN_DBG, "end VDEC_IOCTL_IPC_RELEASE_IMG");
	return 0;
}
