				  ...
				  enum liblinux_iofast_page_type page_type)
{
	...
	unsigned int flags = __gfp_mask_to_page_pool_flag(gfp_mask) | LIBLINUX_PAL_GFP_CONTINUOUS
					...
					unsigned long length, void **page_array)
{
	...
	flags |= LIBLINUX_PAL_GFP_CONTINUOUS;
	if (order > 0) {
	...
	ret = lnxbase_iofast_alloc_pfn_v(order, pa_array, length, flags, &pa_num);
	dmabuf_systrace_end();
	if (ret < 0) {
		pr_err("%s: liblinux_iofast_alloc_pfn_v ret %d\n",  __func__, ret);
		goto out;
	}
