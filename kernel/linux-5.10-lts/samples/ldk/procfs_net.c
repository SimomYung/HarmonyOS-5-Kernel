...
static int proc_entries_realloc(struct proc_path_list **entries)
{
	...
	memcpy(new_path, *entries, origin_size);
	vfree(*entries);
	*entries = new_path;
	(*entries)->max_len = realloc_num;
				...
				char *name_table, struct vfsmount *proc_mnt)
{
	...
	while (node != NULL) {
		if (entries->len >= entries->max_len) {
			err = proc_entries_realloc(&entries);
			if (err < 0)
				return err;
		}
