...
void __override read_dump_end(void)
{
	/* 2M dump memory from bl31_mntn_mem to bl2_mntn_mem is always reserved in liblinux,
	 * so override read_dmp_end to avoid releasing the memory */
	pr_info("use override function to avoid releasing mntn reserved memory\n");
}
