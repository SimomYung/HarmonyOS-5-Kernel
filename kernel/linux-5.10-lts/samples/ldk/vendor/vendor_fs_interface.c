...
{
	return liblinux_pal_ksys_access(filename, mode);
}
EXPORT_SYMBOL(vendor_access);

long vendor_mkdir(const char *pathname, umode_t mode)
{
	return liblinux_pal_ksys_mkdir(pathname, mode);
}
...
{
	return liblinux_pal_ksys_rename(old_name, new_name);
}
...
long vendor_getdents64(struct file *filp, struct linux_dirent64 *buf, unsigned int count)
{
	...
	ret = liblinux_pal_ksys_readdir(internal_fp->fd, (char *)buf, (size_t)count);
