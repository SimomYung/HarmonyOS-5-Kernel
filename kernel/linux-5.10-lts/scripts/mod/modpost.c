...
static int parse_elf(struct elf_info *info, const char *filename)
{
	...
	for (i = 1; i < info->num_sections; i++) {
		...
		/* HM_KERNEL: record dynamic section to distinguish with ko */
		if (sechdrs[i].sh_type == SHT_DYNAMIC)
			info->dynamic_sec = i;
		/*
		 * HM_KERNEL: stripped dynamic libs may not have symtab section,
		 * use dynsym section instead.
		 */
		if ((sechdrs[i].sh_type == SHT_DYNSYM) &&
		    (info->symtab_start == NULL)) {
			unsigned int sh_link_idx;
			symtab_idx = i;
			info->symtab_start = (void *)hdr +
			    sechdrs[i].sh_offset;
...
#ifdef CONFIG_LIBLINUX
	/*
	 * HM_KERNEL: ignore undef symbol for shared object file with
	 * .dynamic seciton.
	 */
	if ((info->hdr->e_type == ET_DYN) && (info->dynamic_sec))
		return 1;
#endif /* CONFIG_LIBLINUX */
...
#ifdef CONFIG_LIBLINUX
	/* HM_KERNEL: treat dynamic libs as part of vmlinux */
	if (info.dynamic_sec)
		mod->is_vmlinux = 1;
#endif /* CONFIG_LIBLINUX */
