...
def generate_arg_stksz(params: List[CParameterDeclaration], is_capcall: str, is_fallback: str) \
    ...
    """
    ret = [CStatement.constant_of(f"""__stksz_state = __arch_actv_rpc_stksz_init({is_capcall}, {is_fallback});""")]

    for param in params:
        ret.append(subst(CStatement.constant_of(f"""
    ...
    ret.append(CStatement.constant_of('__stacksz = __arch_actv_rpc_stksz_done(__stksz_state);'))
    return ret
...
def __generate_actvxact_template(method: SyscallMethod) -> CFunction:
    ...
    return_type = CType.constant_of(method.direct_return_type)
    orig_parameters = list(CParameterDeclaration(p.name, CType.constant_of(p.parameter_type))
...
def __generate_actvxact_calls(method: SyscallMethod) -> List[CFunction]:
    ...
    caller_prefix = method.manager.server.call_prefix
    caller_name_postfix = f"{caller_prefix}_{method.manager_name}_{method.name}"
    orig_parameters = list(CParameterDeclaration(param.name, CType.constant_of(param.parameter_type))
                           for param in method.parameters)
    ...
    if method.method_type is MethodType.FALLBACK:
        is_fallback = '1'
        rply = [CExpression.constant_of("ptr_to_void(__ret)"), CExpression.constant_of("sizeof(*__ret)")]
    if method.slow_data:
        ...
        send_buf_sz = CExpression.constant_of("sizeof(*__ret)")
    else:
        send_buf = CIdentifierExpression("NULL")
        send_buf_sz = CExpression.constant_of("0ul")
