...
from hmdsl.common.c_biast.biast import CExpression, CTranslationUnit, CAttribute, CFunction, \
    ...
    CIdentifierExpression, CStatement, CDeclaration, CClassicalAttribute, CCastExpression
...
def __generate_internal_actv_decls(method: SyscallMethod) -> List[CVariableDeclaration]:
    ...
    call_attributes = [
        ...
        CAttribute.of("@__internal_rpccall:attr")
    ]
    capcall_attributes = [
        CAttribute.alias(f"__internal_rpccapcall_{internal_impl_postfix}"),
        CAttribute.of("@__internal_rpccapcall:attr")
    ]
...
def __generate_internal_actvcall_template(method: SyscallMethod) -> CFunction:
    ...
    actv_call_template = CFunction(
        ...
        body=CStatement.of(
            ...
            """),
        attributes=[CAttribute.of("@__internal_actvcall:attr")])

    local_args = []
    call_args = []
    for param in method.parameters:
        local_args += CVariableDeclaration.of(
            f'{param.parameter_type} @__maybe_unused:attr __actv_local_var_{param.name} = {param.name};')

        call_args.append(CExpression.of(
            f'@__ARCH_ACTV_RPC_PASS_ARG:expr({param.name}, __actv_local_var_{param.name})'))

    return subst(actv_call_template,
                 {'return_type': return_type,
                  'local_args': local_args,
                  '_rply': list(CIdentifierExpression(p) for p in rply_names),
                  'call_args': call_args},
                 True)
