...
from hmdsl.common.c_biast.biast import CExpression, CTranslationUnit, CAttribute, \
    ...
    CExpressionStatement, CReturnStatement, CMemberDesignator, CUnaryExpression, CUnaryOperator, \
    CMacroExpression
...
def __mk_actvhdlrs(server: Server) -> List[Union[str, CTranslationUnit]]:
    ...
    res.append(CTranslationUnit(CVariableDeclaration.constant_of(
        """struct compat_x3 {
...
def __mk_hdlr_template(ilp32: bool) -> CFunction:
    attributes = [CAttribute.constant_of("@__opt_nonoframe:attr"), CAttribute.constant_of("@__opt_nostkprot:attr")]
    if not ilp32:
        attributes = [CAttribute.constant_of("@__opt_nonoframe:attr")]
...
def __adjust_ilp32_args(method_parameters: List[Parameter]) -> List[CStatement]:
    ...
    for param in method_parameters:
        adjust_one = subst(
            adjust_one_template,
            {'param_type': CType.constant_of(param.parameter_type),
...
def __lsyscall_check() -> Tuple[List[CVariableDeclaration], List[CStatement]]:
    decls = CVariableDeclaration.constant_of(
...
def __get_method_info(method: Union[SyscallMethod, LsyscallMethod]) -> Tuple[str, str, List[CParameterDeclaration]]:
    ...
    parameters = list(CParameterDeclaration(p.name, CType.constant_of(p.parameter_type)) for p in method.parameters)
...
def __mk_bridge_hdlr(methods: Union[List[SyscallMethod], List[LsyscallMethod]]) -> List[CTranslationUnit]:
    ...
    for method in methods:
        ...
        append_args = __mk_rpc_recv_arg(method)
        ...
        unused_fallback = CStatement.constant_of('UNUSED(__actv_fallback_unused_1, __actv_fallback_unused_2);') \
            if method.method_type is MethodType.FALLBACK else []
        bridge_req = subst(HDLR_TEMPLATE,
                           {'rpc_unused': [], 'unused_rpc': [], 'frame': __BRIDGE_HDLR_FRAME, 'actv': __ACTV_LOCAL,
                            ...
                            'ret_type': CType.constant_of(method.direct_return_type), 'adjust_ilp32_args': [],
        ...
        if isinstance(method, SyscallMethod):
            bridge_req_cap = subst(
                HDLR_TEMPLATE,
                {'rpc_unused': CParameterDeclaration.constant_of('@__RPC_CAPCALL_HDLR_UNUSED_ARGS:decl'),
                 'actv': __ACTV_LOCAL,
                 'unused_rpc': CStatement.constant_of('UNUSED(@__RPC_CAPCALL_HDLR_UNUSED_ARGS_STRICT:expr);'),
                 ...
                 'parameters': parameters, 'ret_type': CType.constant_of(method.direct_return_type),
                 'adjust_ilp32_args': [],
                 'callee': CIdentifierExpression(f'__actvshadowhdlr_{func_call_postfix}')},
                False)
    ...
    for method in methods:
        ...
        bridge_req = subst(HDLR_TEMPLATE_ILP32,
                           {'rpc_unused': [], 'unused_rpc': [], 'frame': __BRIDGE_HDLR_FRAME, 'actv': __ACTV_LOCAL,
                            ...
                            'ret_type': CType.constant_of(method.direct_return_type),
        ...
        if isinstance(method, SyscallMethod):
            bridge_req_cap = subst(
                ...
                {'rpc_unused': CParameterDeclaration.constant_of('@__RPC_CAPCALL_HDLR_UNUSED_ARGS:decl'),
                 ...
                 'check_decl': check_decl_cap, 'stk_check': stk_check_cap, 'check_lsys': [],
                 'unused_fallback': unused_fallback, 'append_args': append_args, 'parameters': parameters,
                 'fallback_unused': fallback_unused, 'ret_type': CType.constant_of(method.direct_return_type),
    ...
    for method in methods:
        ...
        bridge_nreq = subst(HDLR_TEMPLATE,
                            {'rpc_unused': [], 'unused_rpc': [], 'frame': [], 'stksz': [],
                             ...
                             'adjust_ilp32_args': [], 'ret_type': CType.constant_of(method.direct_return_type)},
                            False)
        ...
        if isinstance(method, SyscallMethod):
            bridge_nreq_cap = subst(
                ...
                {'rpc_unused': CParameterDeclaration.constant_of('@__RPC_CAPCALL_HDLR_UNUSED_ARGS:decl'),
                 ...
                 'ret_type': CType.constant_of(method.direct_return_type), 'parameters': parameters, 'check_lsys': [],
...
def __mk_compat_hdlr(methods: Union[List[SyscallMethod], List[LsyscallMethod]]) -> List[CTranslationUnit]:
    ...
    for method in methods:
        ...
        def_args = list(CVariableDeclaration(f'_{p.name}', CType.constant_of(p.parameter_type))
                        for p in method.parameters)
        for def_arg in def_args:
            def_arg.attributes = [CAttribute.constant_of('@__maybe_unused:attr')]
        (check_decl, stk_check_stmt) = __stk_check([], 'unused', 'unused', (True, False, False)) \
            if method.stk_check else ([], [])
        if method.method_type is MethodType.FALLBACK:
            rpc_unused = CExpression.constant_of('@__RPC_ACTVCALL_HDLR_UNUSED_ARGS_ONE:expr')
            rpc_unused_cap = CExpression.constant_of('@__RPC_ACTVCALL_HDLR_UNUSED_ARGS_THREE:expr')
            passing_bool = CIdentifierExpression('false')
        else:
            rpc_unused = CExpression.constant_of('@__RPC_ACTVCALL_HDLR_UNUSED_ARGS_NONE:expr')
            rpc_unused_cap = CExpression.constant_of('@__RPC_ACTVCALL_HDLR_UNUSED_ARGS_ONE:expr')
        ...
        if isinstance(method, SyscallMethod):
            hdlr_cap_compat = subst(compat_template,
                                    {'def_args': def_args, 'fetch_args': __fetch(method.parameters),
                                     ...
                                     'callee': CIdentifierExpression(f'__actvshadowhdlr_{return_postfix}'),
                                     'ret_type': CType.constant_of(method.direct_return_type), 'check_lsys_decl': [],
                                     'actv': __ACTV_LOCAL},
                                    False)
...
def __fetch(method_parameters: List[Parameter]) -> List[CStatement]:
    # Do not subst a constant template in place
    fetch_template = CStatement.constant_of(
        """
        do{
            int fetch_sz;
            unsigned long align;
...
def __stk_check(params: List[CParameterDeclaration],
    ...
    if not is_compat:
        ...
        if is_ilp32:
            stmts.append(CStatement.constant_of(
...
def __mk_hdlr_table_template(server: Server) -> CVariableDeclaration:
    ...
    if not server.ro_after_init_postponed:
        ...
        hdlr_table_template.attributes = [CAttribute.constant_of(f"@__ro_after_init_hdlr{unique_section}:attr")]
...
def __mk_hdlr_table_inits(server: Server, offset: str) \
    ...
    inits = [
        (CDesignation.of(f'[{start} ... {offset} __{svr}_method_MIN] ='),
         CConstructorExpression.constant_of(
...
def __construct_hdlr_table_items(server: Server, is_cap: bool, items_type: str, offset: str) -> \
    ...
    rsvd_method_ref = CConstructorExpression.constant_of(
    ...
    for manager in server.managers:
        ...
        for method in mthds:
            ...
            ret.append((
                CDesignation([CIndexDesignator(
                    CIdentifierExpression(f'{offset} __{server.name}_method_{method.manager_name}_{method.name}'))]),
                __mk_constructor(method, f'p__actvbridge{cap}hdlr{union_type}_{hdlr_name}', _name)))
        ret += list((CDesignation([CIndexDesignator(CIdentifierExpression(
...
def __mk_constructor(method: Union[SyscallMethod, LsyscallMethod], _type: str, _name: str) -> CConstructorExpression:
    return CConstructorExpression(items=[(
        CDesignation(designators=[CMemberDesignator(field_name=_type)]),
        CUnaryExpression(operator=CUnaryOperator.ADDRESS, operand=CIdentifierExpression(_name))
    )])


def __mk_rpc_recv_arg(method: Union[SyscallMethod, LsyscallMethod]) -> List[CMacroExpression]:
    res = []
    for p in method.parameters:
        res.append(CMacroExpression(
            '__ARCH_ACTV_RPC_RECV_ARG',
            [CType.constant_of(p.parameter_type), CIdentifierExpression(p.name)]))
