...
def _node_embedded_code():
    ...
    code['ac']['hkids'] = NodeStr(optional=True)
    code['ac']['isolate'] = NodeStr(optional=True)
...
def _node_hook_ac_hkids():
    """
    Return a node which describes the hkids module.
    """
    hkids = _ac_common()
    hkids['enabled'].default_fn = None
    hkids['enabled'].default = False
    return hkids

def _node_hook_ac_isolate():
    """
    Return a node which describes the isolate module.
    """
    isolate = _ac_common()
    isolate['enabled'].default_fn = None
    isolate['enabled'].default = False
    return isolate
...
def _node_hook_ac():
    ...
    access_control['hkids'] = _node_hook_ac_hkids()
    access_control['isolate'] = _node_hook_ac_isolate()
...
def _node_hook_module_relation():
    mod_rlt = NodeStr(default_fn=depend_on_scheme(
        '{all:dac,mac,capability,ability,seharmony}', {
            "security context/initialization": '{all:dac,dec,mac,capability,encaps,isolate,ability,seharmony,acl,userns,yama,hkids,keyring,audit}',
            "security context/finalization": '{all:dac,dec,mac,capability,encaps,isolate,ability,seharmony,acl,userns,yama,hkids,keyring,audit}',
            "security context/update": '{all:dac,dec,mac,capability,encaps,isolate,ability,seharmony,acl,userns,yama,hkids,keyring,audit}',
