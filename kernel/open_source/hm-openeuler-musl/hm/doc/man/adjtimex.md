...
## Description
The adjtimex() function tunes system clock and returns the partial system parameters through
...
struct timex {
	...
	struct timeval time;
	                 /* Current time (read-only, except for ADJ_SETOFFSET); upon
			    return, when STA_NANO flag is set, the unit of time.tv_usec
			    is nanoseconds, otherwise its microseconds */
	...
	long calcnt;     /* PPS count of calibration intervals (read-only), it is not
			    supported currently */
	long errcnt;     /* PPS count of calibration errors (read-only), it is not
			    supported currently */
