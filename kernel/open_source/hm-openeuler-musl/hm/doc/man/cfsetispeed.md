...
- **tio**
A pointer to the termios structure to set the input baud rate.
&nbsp;
- **speed**
...
## Description
The cfsetispeed() function set the input baud rate stored in the structure pointed to by *tio* to *speed*. There be no effect on the baud rates set in the hardware until a subsequent successful call to tcsetattr() with the same termios structure. Similarly, errors resulting from attempts to set baud rates not supported by the terminal device need not be detected until the tcsetattr() function is called.

## Returns

- **0**
The input baud rate was successfully set into the termios structure.
&nbsp;
