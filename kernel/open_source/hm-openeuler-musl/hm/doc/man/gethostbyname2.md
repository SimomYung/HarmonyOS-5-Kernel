...
The gethostbyname2() function takes a string like "www.huawei.com", and returns a struct hostent which contains tons of information, including the IP address. (Other information is the official host name, a list of aliases, the address type, the length of the addresses, and the list of addresses.)

## Returns

The gethostbyname2() function returns a pointer to hostent structure or a null pointer if an error occurs. If *af* is not a valid value, a hostent whose member h_addr is not set will be returned. On error, the *h_errno* variable holds an error number. It can be printed using the functions herror() or hstrerror(). These work just like the classic *errno*, perror(), and strerror(). When non-NULL, the return value may point at static data.
&nbsp;
...
- **HOST_NOT_FOUND**
  The specified host is unknown. For example, *name* is "", host does not exist, or addr is AF_INET but the parameter *af* is AF_INET6.
  &nbsp;
...
int main(int argc, char *argv[])
{
    ...
    if ((he = gethostbyname2(argv[1]), af) == NULL) {  /* get the host info */
