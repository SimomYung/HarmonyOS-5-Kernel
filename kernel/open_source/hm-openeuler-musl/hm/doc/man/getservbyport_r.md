...
The reentrant version of getservbyport()
...
- **port**
  Service port number, given in network byte order.
  &nbsp;
- **proto**
...
getservbyport_r() is the reentrant version of getservbyport(). The caller supplies a *buf* of size *buflen* which will be filled with the returned structure servent in on success.

## Returns

On success, this function returns 0. On error, it returns one of the positive error numbers listed in errors. If record is not found, *result* will be set to NULL.
&nbsp;
