...
## Description
The function lsetxattr() will set the extended attributes to a file (file is identified according to the parameter *path* and *name*). It is very similar to setxattr(), the exception is in the case of file belongs to symbolic link. In the symbolic case, the attributes will set on the link, not the origin file that link refers to.
