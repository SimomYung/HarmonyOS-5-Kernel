...
long sysconf(int name)
{
	static const short values[] = {
		...
		[_SC_XOPEN_XPG4] = -1,
		[_SC_NZERO] = NZERO,
		[_SC_AIX_KERNEL_BITMODE] = sizeof(char *) == 4 ? 32 : 64,
	...
	/* Note: In glibc, JT_NPROCESSORS_CONF means the cpu core that is configured
	 * in systerm. JT_NPROCESSORS_ONLN means that the cpu core that is online in
	 * systerm. But in musl, those two configure call SYS_sched_getaffinity which
	 * return the cpu affinity of current tcb.
	 * So we choose to implement those according to glibc.
	 */
	case JT_NPROCESSORS_CONF & 255:
		return syscall((long)SYS_cpu_getnum, 0);
	case JT_NPROCESSORS_ONLN & 255:
		return syscall((long)SYS_cpu_getnum, 1);
