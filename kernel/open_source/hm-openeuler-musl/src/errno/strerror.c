...
hidden int __errno_valid(int e)
{
	if (EDQUOT == 1133) {
		if (e == 109)
			e = -1;
	...
	if ((unsigned int)e < (sizeof(errmsgidx) / sizeof(short))) {
		return (int)(errmsgidx[e] != 0);
	}
	return 0;
}
...
weak_alias(errmsgstr, sys_errlist);
int sys_nerr = sizeof(errmsgstr)/sizeof(char *);
