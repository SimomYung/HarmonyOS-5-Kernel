	...
	char *const argv[restrict], char *const envp[restrict])
{
	pid_t pid;
	/**
	 * Append stack from 1024 to 4096 as we need to call
	 * vsyscall instead of trapping into kernel mode, which
	 * need larger stack size.
	 */
	char stack[4096 + PATH_MAX];
	int ec=0, cs;
	struct args args;
	...
	pid = __clone(child, stack+sizeof stack,
		CLONE_VM|CLONE_VFORK|SIGCHLD, &args, NULL, NULL, NULL);
	close(args.p[1]);
	UNLOCK(__abort_lock);
