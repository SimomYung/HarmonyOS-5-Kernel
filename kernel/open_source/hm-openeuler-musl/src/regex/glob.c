...
struct glob_arg {
	...
	const char *arg_pat;
	int arg_flags;
	int (*arg_errfunc)(const char *path, int err);
	struct match **arg_tail;
	glob_t *restrict arg_g;
};
...
static int do_glob(struct glob_arg *arg)
{
	char *buf = arg->arg_buf;
	size_t pos = arg->arg_pos;
	int type = arg->arg_type;
	const char *pat = arg->arg_pat;
	int flags = arg->arg_flags;
	int (*errfunc)(const char *path, int err) = arg->arg_errfunc;
	struct match **tail = arg->arg_tail;
	glob_t *restrict g = arg->arg_g;
	...
	/* If dir functions were customized,use them instead of normal functions. */
	void (*do_closedir) (void *) = NULL;
	struct dirent *(*do_readdir) (void *) = NULL;
	void *(*do_opendir) (const char *) = NULL;
	int (*do_lstat) (const char *__restrict, struct stat *__restrict) = NULL;
	int (*do_stat) (const char *__restrict, struct stat *__restrict) = NULL;
	if (((unsigned int)flags & GLOB_ALTDIRFUNC) != 0) {
		do_closedir = g->gl_closedir;
		do_readdir = g->gl_readdir;
		do_opendir = g->gl_opendir;
		do_lstat = g->gl_lstat;
		do_stat = g->gl_stat;
	} else {
		do_closedir = (void (*)(void *))closedir;
		do_readdir = (struct dirent *(*)(void *))readdir;
		do_opendir = (void *(*)(const char *))opendir;
		do_lstat = lstat;
		do_stat = stat;
	}
	...
	if (!*pat) {
		...
		struct stat st;
		if ((flags & GLOB_MARK) && (!type||type==DT_LNK) && !do_stat(buf, &st)) {
			if (S_ISDIR(st.st_mode)) type = DT_DIR;
			else type = DT_REG;
		}
		if (!type && do_lstat(buf, &st)) {
	...
	int old_errno = errno;
	struct dirent *de;
	while (errno=0, de=do_readdir(dir)) {
		...
		struct glob_arg g_arg = {
			buf, pos+l, (int)(de->d_type), p2 ? p2 : "",
			flags, errfunc, tail, g
		};
		int r = do_glob(&g_arg);
		if (r) {
			do_closedir(dir);
...
int glob(const char *restrict pat, int flags, int (*errfunc)(const char *path, int err), glob_t *restrict g)
{
	...
	if (((unsigned)flags & GLOB_ALTDIRFUNC) != 0 && (g->gl_closedir == NULL || g->gl_readdir == NULL ||
		g->gl_opendir == NULL || g->gl_lstat == NULL || g->gl_stat == NULL)) {
		errno = EINVAL;
		return -1;
	}
