...
static struct tzfile_stat old_tzfile;
...
static inline bool is_tzfile_updated(const struct tzfile_stat *tzfile, const struct stat *st)
{
	return (tzfile->dev != st->st_dev || tzfile->ino != st->st_ino || tzfile->mtime != st->st_mtime);
}

static inline void update_tzfile(struct tzfile_stat *tzfile, struct stat *st)
{
	tzfile->dev = (st != NULL) ? st->st_dev : 0;
	tzfile->ino = (st != NULL) ? st->st_ino : 0;
	tzfile->mtime = (st != NULL) ? st->st_mtime : 0;
}

static bool check_tzfile(const char *file, struct tzfile_stat *tzfile)
{
	/* Check whether the tzfile needs to be updated. */
	struct stat st;

	if (stat(file, &st) == 0) {
		if (!is_tzfile_updated(tzfile, &st)) {
			return false;
		} else {
			update_tzfile(tzfile, &st);
			return true;
		}
	} else {
		/* cannot read stat of file, continue to refresh timezone */
		update_tzfile(tzfile, NULL);
		return true;
	}
}

/**
* description: parse TZ variable, see man 3 tzset
* TZ="NZST-12:00:00NZDT-13:00:00,M10.1.0,M3.3.0"
*
* Footer of tzfile defined in RFC8536 has the same format with TZ
*/
static void parser_tz(const char *tz)
{
	const char *s = tz;
	if (s == NULL) {
		s = __utc;
	...
	if (dst_name[0]) {
		...
		if (*s == '+' || *s == '-' || isdigit(*s) != 0) {
			...
			dst_off = (int)__timezone - H2S;
		}
	} else {
		__daylight = 0;
		dst_off = (int)__timezone;
	}
...
static void do_tzset()
{
	...
	s = getenv("TZ");
	if (s != NULL) {
		/* use TZ, reinit tzfile cache */
		update_tzfile(&old_tzfile, NULL);
		...
		* strategy makes it so free would have minimal benefit anyway. */
		...
		/* use tzfile, reinit TZ cache */
		if (old_tz != old_tz_buf) {
				free(old_tz);
				old_tz = old_tz_buf;
		}
		(void)memset(old_tz, 0, old_tz_size);

		s = "/etc/localtime";
		if (!check_tzfile(s, &old_tzfile)) {
			/* tzfile not updated, do not refresh timezone */
			return;
		}
	}
	...
	if (map) {
		...
		if (zi[map_size-1] == '\n') {
			...
			// do last conversion while no rules matched
			footer = (const unsigned char*)s + 1;
		} else {
			footer = NULL;
		}
		{ // consistent with glibc, firstly do parsing from data block define in RFC8536
			...
			for (p=types; p<abbrevs; p+=6) {
				if (!p[4] && !__tzname[0]) {
					...
					__timezone = -(long)zi_read32(p);
				}
				if (p[4] && !__tzname[1]) {
					__tzname[1] = (char *)abbrevs + p[5];
					dst_off = -(int)zi_read32(p);
					__daylight = 1;
				}
			}
...
void __secs_to_zone(long long t, int local, int *isdst, long *offset, long *oppoff, const char **zonename)
{
	...
	if (zi) {
		...
		// fallthrough in parsing the POSIX TZ-style string
		parser_tz((const char*)footer);
	}
