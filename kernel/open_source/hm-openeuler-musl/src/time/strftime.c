 ...
 * the calculation method (sizeof(long long) - 1)*log10(2) + 1*/
#define VAL_MAX_LEN (20)
#define __INT_MAX_MUL_OF_TEN (INT_MAX / 10)
#define __INT_MAX_REMAINDER_OF_TEN (INT_MAX % 10)
...
number: {
		char *p = (buf + VAL_MAX_LEN);
		int negative_number = (val < 0);
		if (negative_number) {
			val = -val;
		}
		do {
		...
		int padding = width + p - (buf + VAL_MAX_LEN);
		if (padding > 0) {
...
size_t __strftime_l(char *restrict s, size_t n, const char *restrict f, const struct tm *restrict tm, locale_t loc)
{
	...
	for (l=0; l<n; f++) {
		...
		width = 0;
		while (isdigit(*p)) {
			if (width > __INT_MAX_MUL_OF_TEN || (width == __INT_MAX_MUL_OF_TEN
				&& *p - '0' > __INT_MAX_REMAINDER_OF_TEN)) {
				width = INT_MAX;
			} else {
				width *= 10;
				width += *p - '0';
