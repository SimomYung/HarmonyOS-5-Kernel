...
static const struct sig_default_act sig_default_action[] = {
    ...
    {SIGUSR1, 0, __sig_ignore},
    {SIGSEGV, 0, __sig_core},
    {SIGUSR2, 0, __sig_ignore},
...
{
    if (sig < SIGHUP || sig > (_NSIG - 1))
        return -EINVAL;

    pthread_spin_lock(&sig_lite_lock);
    sigactq_t *sigact = __sig_find_action(sig);
    if (sigact == NULL) {
        pthread_spin_unlock(&sig_lite_lock);
        return -EINVAL;
    }
    if (oact) {
        __sig_copy_sigaction(&sigact->act, oact);
    }
    if (act == NULL) {
        pthread_spin_unlock(&sig_lite_lock);
        return 0;
    ...
    sa_sighandler_t handler = act->sa_handler;
    /* Skip sig that can not be catched */
    if (__sig_cannot_catche(sig, handler)) {
        pthread_spin_unlock(&sig_lite_lock);
        return -EINVAL;
    }
