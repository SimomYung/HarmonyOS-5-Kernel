 ...
 * @brief       Key derivation (KDF) algorithm header file
 */
...
/**
  ...
  * @brief Scrypt compliance assessment
  *
...
/**
 * @ingroup crypt_eal_kdf
 * @brief   Scrypt compliance assessment callback registration interface, which is used to check whether the module
 * status is available. After the CMVP feature is enabled, the CMVP provides the default implementation.
 * This callback is called by the CRYPT_EAL_Scrypt interface before an operation is performed, if the callback fails,
 * the CRYPT_EAL_Scrypt fails.
 *
 * @param   func     [IN] Scrypt compliance assessment callback
 *
 * @return  None
 */
...
/**
  * @ingroup crypt_eal_kdf
  * @brief Pbkdf2 compliance assessment
  *
  * @return  true, if successful.
  *          false, if failed.
  */
typedef bool (*CRYPT_EAL_Pbkdf2C2)(CRYPT_MAC_AlgId id, uint32_t saltLen, uint32_t it, uint32_t len);

/**
 * @ingroup crypt_eal_kdf
 * @brief   Pbkdf2 Callback registration interface, which is used to determine whether the module status is available.
 * After the CMVP feature is enabled, the CMVP provides the default implementation.
 * This callback is called by the CRYPT_EAL_Pbkdf2 interface before an operation is performed.
 * If the callback fails, the CRYPT_EAL_Pbkdf2 fails.
 *
 * @param   func     [IN] Pbkdf2 compliance assessment callback
 *
 * @return  None
 */
...
/**
 * @ingroup crypt_eal_kdf
 * @brief Hkdf compliance assessment
 *
 * @return  true, if successful.
 *          false, if failed.
 */
typedef bool (*CRYPT_EAL_HkdfC2)(CRYPT_MAC_AlgId id, uint32_t keyLen);

/**
 * @ingroup crypt_eal_kdf
 * @brief HKDF compliance assessment callback registration interface, which is used to check whether the module
 * status is available. After the CMVP feature is enabled, the CMVP provides the default implementation.
 * This callback is called by the CRYPT_EAL_Hkdf interface before an operation is performed, if the callback
 * fails, the CRYPT_EAL_Hkdf fails.
 *
 * @param func [IN] Hkdf compliance check callback
 *
 * @return None
 */
...
/**
  * @ingroup crypt_eal_kdf
  * @brief KDF-TLS1.2 compliance assessment
  *
  * @param id [IN] MAC algorithm ID
  *                 Any algorithm can be used in non-approved mode
  *                 Only CRYPT_MAC_HMAC_SHA256/CRYPT_MAC_HMAC_SHA384/CRYPT_MAC_HMAC_SHA512 is allowed in ISO19790 mode
  *
  * @return true, if successful.
  *         false, if failed.
  */
typedef bool (*CRYPT_EAL_KdfTls12C2)(CRYPT_MAC_AlgId id, uint32_t keyLen);

/**
 * @ingroup crypt_eal_kdf
 * @brief   KDF-TLS1.2 Compliance Assessment callback registration interface, which is used to check
 * whether algorithm parameters are compliant and whether the module status is available. After the CMVP feature
 * is enabled, the CMVP provides the default implementation.
 * The CRYPT_EAL_KdfTls12 calls this callback before performing an operation, if the callback fails,
 * the CRYPT_EAL_KdfTls12 interface fails.
 *
 * @param   func     [IN] KDF-TLS1.2 compliance assessment callback
 *
 * @return None
 */
 ...
 *
 * @return CRYPT_SUCCESS, if success.
