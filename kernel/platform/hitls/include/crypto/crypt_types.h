/**
  ...
  * @brief       General type external interface
 */
 ...
 * @ingroup crypt_types
 *
 * ML-KEM private key parameter structure
 */
 ...
 * @ingroup crypt_types
 *
 * ML-KEM public key parameter structure
 */
...
typedef struct {   /**< This parameter cannot be NULL and is determined by the underlying structure. */
    ...
    uint32_t bits; /**< Bits of para, FIPS 186-5 does not support generation and use of keys with odd bits. */
} CRYPT_RsaPara;

/**
 * @ingroup crypt_types
*
* Para structure of the DSA algorithm
*/
typedef struct {   // This parameter cannot be null, and it is determined by the underlying structure.
...
typedef struct {
    /**
    * @ingroup crypt_types
    * @brief Obtain the entropy source. If the default entropy source provided by HiTLS is not used,
    ...
    * entropy  [OUT] Indicates the obtained entropy source data. The length of the entropy source data must meet the
    *                following requirements: lenRange->min <= len <= lenRange->max.
...
int32_t (*getEntropy)(void *ctx, CRYPT_Data *entropy, uint32_t strength, CRYPT_Range *lenRange);
     ...
     *
     * ctx      [IN] Context used by the caller
     * nonce    [OUT] Obtained random number.
...
typedef enum {
    ...
    CRYPT_CTRL_SET_RSA_SALT,           /**< RSA sets the salt content and length of PSS padding.
                                            The salt value will be reset(pointer set to zero) for each signature,
                                            so the memory of salt should be managed by the caller of this option.
                                            This option should only be used for KAT and self-verification. */
    ...
    CRYPT_CTRL_SET_SM9_NO_ENCODE_H_S,   /**< SM9 set the signature value non-ASN1 code, The combination mode is H+S. */
    ...
    CRYPT_CTRL_MLKEM_SET_KEY_BITS,      /**< Set the key info of ML-KEM */
    CRYPT_CTRL_MLKEM_GET_EK_LEN,        /**< Get the encapsulation key length */
    CRYPT_CTRL_MLKEM_GET_DK_LEN,        /**< Get the decapsulation key length */
    CRYPT_CTRL_MLKEM_GET_CT_LEN,        /**< Get the ciphertext length */

    CRYPT_PKEY_CTRL_MAX = 0x3FFFFFFF
} CRYPT_PkeyCtrl;
...
/* SM9 algorithm, key type enumeration */
typedef enum {
    SM9_NOKEY = 0x00,               // The key information does not exist
    SM9_SIGN_KEY = 0x01,            // Signature
    SM9_VERIFY_KEY = 0x02,          // Verification
    SM9_COMPUTE_SHARE_KEY = 0x04,   // Shared Key Calculation
    SM9_ENC_KEY = 0x08,             // Encryption
    SM9_DEC_KEY = 0x10,             // Decryption
    SM9_KEY_GEN = 0x20,             // Key generation
} CRYPT_SM9_KeyType;

/* In SM9 algorithm, structure of the public key used by a user to set or obtain a key. */
typedef struct {
    CRYPT_Data pub;     // Main public key 1, used for signature verification Fp2.
                        // 2. Used for encrypting and decrypting Fp1.
    CRYPT_Data idA;    // Identifier ID.
    CRYPT_SM9_KeyType keyType;  // key type, which is used for signature verification.
                                // shared key calculation, and encryption and decryption.
    uint8_t hid;    // Identifier.
} CRYPT_Sm9Pub;

/* In SM9 algorithm, structure of the private key used by the SM9 algorithm user to set the private key. */
typedef struct {
    CRYPT_Data pub;     // Main public key 1. used for signature verification Fp2. 2. Used for encryption, decryption,
                        // and shared key calculation Fp1.
    CRYPT_Data prv;     // Indicates the master private key. 1. This key is used during key generation.
    CRYPT_Data d;       // User private key. 1. Used for signature verification Fp1. 2. Used for encryption
                        // and decryption and shared key calculation Fp2.
    CRYPT_Data idA;     // Identifier ID, signature can set to NULL.
    CRYPT_SM9_KeyType keyType; // Key Type
    uint8_t hid;        // Identifier, do not use this value when signing.
} CRYPT_Sm9Prv;

/* This parameter specifies the SM9 encoding format. */
typedef enum {
    CRYPT_SM9_ASN_ENCODE = 0, /**< Default mode according to GB/T 41389-2022 */
    CRYPT_SM9_NO_ENCODE_H_S, /**< Do not use encoding, The signature result output sequence is h+s. > */
} CRYPT_SM9_EncodeType;
 ...
 * @ingroup crypt_ctrl_param
 *
 * Sets and obtains the internal MAC parameters.
 */
typedef enum {
