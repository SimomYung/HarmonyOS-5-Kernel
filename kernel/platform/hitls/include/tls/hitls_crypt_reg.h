 ...
 * @brief       Algorithm related interfaces to be registered by the user
 */
...
/**
 * @ingroup hitls_crypt_reg
 * @brief   KEM:Generate a key pair based on secbits.
 *
 * @param   secbits [IN] Key security level
 *
 * @return  Key handle
 */
typedef HITLS_CRYPT_Key *(*CRYPT_GenerateHybridKeyPairCallback)(uint32_t secbits);

/**
 * @ingroup hitls_crypt_reg
 * @brief   KEM:Release the key.
 *
 * @param   key [IN] Key handle
 */
typedef void (*CRYPT_FreeHybridKeyCallback)(HITLS_CRYPT_Key *key);

/**
 * @ingroup hitls_crypt_reg
 * @brief   KEM: Extract the public key data.
 *
 * @param   key [IN] Key handle
 * @param   pubKeyBuf [OUT] Public key data
 * @param   bufLen [IN] Buffer length
 ...
 * @return 0 indicates success. Other values indicate failure.
 */
typedef int32_t (*CRYPT_HybridGetPubKeyCallback)(HITLS_CRYPT_Key *key, uint8_t *pubKeyBuf, uint32_t bufLen,
    uint32_t *pubKeyLen);

/**
 * @ingroup hitls_crypt_reg
 * @brief   KEM: Calculate the shared key and cipher key based on the peer public key.
 *
 * @param   pkey [IN/OUT] Hybrid key handle
 * @param   pubkey [IN] Public key data
 * @param   pubKeyLen [IN] Public key data length
 * @param   group [IN] Named Group enumerated value
 *
 * @return 0 indicates success. Other values indicate failure.
 */
typedef int32_t (*CRYPT_HybridKeyEncapsCallback)(HITLS_HybridExchCtx *pkey, uint8_t *pubkey, uint32_t pubKeyLen,
    HITLS_NamedGroup group);

/**
 * @ingroup hitls_crypt_reg
 * @brief   KEM: Calculate the shared key based on the local key and peer cipher text.
 *
 * @param   pkey [IN] Hybrid key handle
 * @param   cipherText [IN] Peer cipher text data
 * @param   cipherLen [IN] Cipher text length
 *
 * @return 0 indicates success. Other values indicate failure.
 */
typedef int32_t (*CRYPT_HybridKeyDecapsCallback)(HITLS_HybridExchCtx *pkey, uint8_t *cipherText, uint32_t cipherLen);
...
typedef struct {
    CRYPT_GenerateHybridKeyPairCallback genHybridKeyPair;  /**< Generating a public key pk and a private key sk */
    CRYPT_FreeHybridKeyCallback freeHybridKey;             /**< Release the context. */
    CRYPT_HybridGetPubKeyCallback getHybridPub;            /**< Get the Public Key */
    CRYPT_HybridKeyEncapsCallback encaps;                  /**< Key encapsulation algorithm. Input the public key,
                                                                   output a ciphertext and shared password. */
    CRYPT_HybridKeyDecapsCallback decaps;                  /**< Key decapsulation algorithm. Input the private key
                                                                   and ciphertext, and the shared key is output. */
} HITLS_CRYPT_HybridMethod;
...
/**
 * @brief   Register the callback function of the Hybrid.
 *
 * @param   userCryptCallBack [IN] Callback function to be registered
 *
 * @retval  HITLS_SUCCESS, if successful.
 * @retval  HITLS_NULL_INPUT, the input parameter is NULL..
 */
int32_t HITLS_CRYPT_RegisterHybridMethod(HITLS_CRYPT_HybridMethod *userCryptCallBack);
