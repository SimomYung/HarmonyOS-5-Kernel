...
#define SEPOL_BITS_PER_BYTE 8
static inline int sepol_class_map_test_bit(uint8_t *map, uint16_t classval)
{
	unsigned int byte_off = (unsigned int)(classval / SEPOL_BITS_PER_BYTE);
	unsigned int bit_off = (unsigned int)(classval % SEPOL_BITS_PER_BYTE);
	return ((map == 0) ||
		(map[byte_off] & (1u << bit_off)) != 0);
}
#endif
