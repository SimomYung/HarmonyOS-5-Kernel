...
#define SEPOL_XPERMS_ALLOWED 1
#define SEPOL_XPERMS_AUDITALLOW 2
#define SEPOL_XPERMS_DONTAUDIT 4
struct sepol_xperms_data {
	uint32_t p[8];
};

struct sepol_xperms {
	uint16_t len;
	struct sepol_xperms_data drivers;
};

struct sepol_xperms_decision {
	uint8_t used;
	uint8_t driver;
	struct sepol_xperms_data *allowed;
	struct sepol_xperms_data *auditallow;
	struct sepol_xperms_data *dontaudit;
};
#endif
