...
cd $TOPDIR/self_source/hm-network-libs

rm -rf $TOPDIR/self_source/hm-network-libs/build
mkdir $TOPDIR/self_source/hm-network-libs/build
cd $TOPDIR/self_source/hm-network-libs/build
cp $TOPDIR/self_source/hm-network-libs/.config .
...
cmake -DCMAKE_MODULE_PATH:PATH=$TOPDIR/output/aarch64/usr/share/cmake/Modules \
          ...
          $CMAKE_OPTIONS_LLVM \
          $TOPDIR/self_source/hm-network-libs/ulibs/stub
cmake --build $TOPDIR/self_source/hm-network-libs/build --verbose -- -j 32
DESTDIR="${TOPDIR}/output/aarch64" cmake --install $TOPDIR/self_source/hm-network-libs/build
