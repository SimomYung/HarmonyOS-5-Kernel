...
cd $TOPDIR/self_source/hm-security-libs

rm -rf $TOPDIR/self_source/hm-security-libs/build
mkdir -p $TOPDIR/self_source/hm-security-libs/build
cd $TOPDIR/self_source/hm-security-libs/build
cp $TOPDIR/self_source/hm-security-libs/.config .
...
cmake -DCMAKE_MODULE_PATH:PATH=$TOPDIR/output/aarch64/usr/share/cmake/Modules \
          ...
          $CMAKE_OPTIONS_LLVM \
          $TOPDIR/self_source/hm-security-libs/ulibs
cmake --build $TOPDIR/self_source/hm-security-libs/build --verbose -- -j 32
DESTDIR="${TOPDIR}/output/aarch64" cmake --install $TOPDIR/self_source/hm-security-libs/build
