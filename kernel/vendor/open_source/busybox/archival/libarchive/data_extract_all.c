...
static void FAST_FUNC xattrs_write(char const *file_name, const char *xattr_key, const char *xattr_val, size_t size)
{
	int ret;

	if (!xattr_key || !xattr_val)
		return;

	ret = lsetxattr(file_name, xattr_key, xattr_val, size, 0);
	if (ret < 0)
		bb_error_msg("failed to set xattr(%s) on file %s, errno %d", xattr_key, file_name, errno);
}

static void FAST_FUNC xattrs_set_hmmac(char const *file_name, archive_handle_t *archive_handle)
{
	if (!archive_handle->tar__hmmac_val)
		return;

	xattrs_write(file_name, XATTR_HMMAC_KEY, archive_handle->tar__hmmac_val, archive_handle->tar__hmmac_len);
	free(archive_handle->tar__hmmac_val);
	archive_handle->tar__hmmac_val = NULL;
	archive_handle->tar__hmmac_len = 0;
}

static void FAST_FUNC xattrs_set_cap(char const *file_name, archive_handle_t *archive_handle)
{
	if (!archive_handle->tar__cap_val)
		return;

	xattrs_write(file_name, XATTR_CAP_KEY, archive_handle->tar__cap_val, archive_handle->tar__cap_len);
	free(archive_handle->tar__cap_val);
	archive_handle->tar__cap_val = NULL;
	archive_handle->tar__cap_len = 0;
}
#endif
...
//TODO: what if file_header->link_target == NULL (say, corrupted tarball?)
	...
	// when ENABLE_FEATURE_TAR_RESTORE_PERMS is on, do not skip link
	{
#else
...
#else
			// Also deal with link when ENABLE_FEATURE_TAR_RESTORE_PERMS is on
			lchown(dst_name, uid, gid);
#endif
		}
...
#if ENABLE_FEATURE_HM_TAR_XATTR_SEC
	xattrs_set_hmmac(dst_name, archive_handle);
	xattrs_set_cap(dst_name, archive_handle);
#endif
