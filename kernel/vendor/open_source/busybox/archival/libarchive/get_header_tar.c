...
#define XATTR_USER_DOT_PREFIX "user."
#define XATTR_SEC_CONTEXT_PREFIX "SCHILY.xattr."
#define XATTR_HMMAC_CONTEXT XATTR_SEC_CONTEXT_PREFIX XATTR_HMMAC_KEY
#define XATTR_CAP_CONTEXT XATTR_SEC_CONTEXT_PREFIX XATTR_CAP_KEY
#endif
...
#define TAR_EXTD (ENABLE_FEATURE_TAR_GNU_EXTENSIONS || ENABLE_FEATURE_TAR_SELINUX \
		|| ENABLE_FEATURE_HM_TAR_XATTR_SEC)
#if !TAR_EXTD
...
#if ENABLE_FEATURE_HM_TAR_XATTR_SEC
/* Traverse xattrs-include, check if the keyword is matched */
static bool xattrs_kw_included(const char *key, const llist_t *includes)
{
	if (!includes)
		return strncmp(key, XATTR_USER_DOT_PREFIX, sizeof(XATTR_USER_DOT_PREFIX) - 1) == 0;

	while (includes) {
		if (fnmatch(includes->data, key, 0) == 0)
			return true;
		includes = includes->link;
	}
...
# if ENABLE_FEATURE_HM_TAR_XATTR_SEC
		if ((archive_handle->ah_flags & ARCHIVE_RESTORE_XATTRS) && !global) {
			/* Scan for hmmac contexts, via "SCHILY.xattr.security.hmmac" keyword */
			if (is_prefixed_with(value, XATTR_HMMAC_CONTEXT"=")) {
				if (xattrs_kw_included(XATTR_HMMAC_KEY, archive_handle->tar__xattrs_include)) {
					value += sizeof(XATTR_HMMAC_CONTEXT"=") - 1;
					if (archive_handle->tar__hmmac_val)
						free(archive_handle->tar__hmmac_val);
					archive_handle->tar__hmmac_val = xstrdup(value);
					archive_handle->tar__hmmac_len = (size_t)(p - value) - 1;
					continue;
				}
			}

			/* Scan for capability contexts, via "SCHILY.xattr.security.capability" keyword */
			if (is_prefixed_with(value, XATTR_CAP_CONTEXT"=")) {
				if (xattrs_kw_included(XATTR_CAP_KEY, archive_handle->tar__xattrs_include)) {
					value += sizeof(XATTR_CAP_CONTEXT"=") - 1;
					if (archive_handle->tar__cap_val)
						free(archive_handle->tar__cap_val);
					/* capability is hex value, may include \0, DO NOT use xstrdup */
					archive_handle->tar__cap_val = xmalloc((size_t)(p - value) - 1);
					if (archive_handle->tar__cap_val) {
						archive_handle->tar__cap_len = (size_t)(p - value) - 1;
						memcpy(archive_handle->tar__cap_val, value, archive_handle->tar__cap_len);
					} else {
						bb_error_msg("fail to xmalloc for capability xattr of file %s", archive_handle->file_header->name);
					}
					continue;
				}
			}
		} /* end of security xattr if */
# endif
	}
