...
//config:
//config:config FEATURE_CHMOD_SKIP_LINKS_OPTION
//config:	bool "Enable -h options"
//config:	default n
//config:   depends on CHMOD
//config:
...
//usage:       "[-Rh"IF_DESKTOP("cvf")"] MODE[,MODE]... FILE..."
...
//usage:     "\n	-R	Recurse"
//usage:     "\n	-h	Skip the soft link directly and no modifications"
//usage:	IF_DESKTOP(
...
#if ENABLE_FEATURE_CHMOD_SKIP_LINKS_OPTION
#define OPT_SKIP_LINKS (1 << 4)
#define OPT_STR     "Rh" IF_DESKTOP("vcf")
#else
		...
		struct stat *statbuf)
{
	...
	if (state->depth == 0) {
#if ENABLE_FEATURE_CHMOD_SKIP_LINKS_OPTION
		if (S_ISLNK(statbuf->st_mode) && (state->flags & OPT_SKIP_LINKS)) {
				return TRUE;
		}
#endif
...
#if ENABLE_FEATURE_CHMOD_SKIP_MODE_SET
	/* If the file permission has been set to the expected mode, return directly. */
	if ((statbuf->st_mode & 07777) == (newmode & 07777)) {
		return TRUE;
	}
#endif
...
int skip_soft_link (int flags, char **argv, char *smode) {
...
int chmod_main(int argc UNUSED_PARAM, char **argv)
{
...
#if ENABLE_FEATURE_CHMOD_SKIP_LINKS_OPTION
    		if (arg[1] && strchr(arg, 'h')) {
			flags |= OPT_SKIP_LINKS;
		}
#endif
	...
	smode = *argv++;
#if ENABLE_FEATURE_CHMOD_SKIP_LINKS_OPTION
	retval = skip_soft_link(flags, argv, smode);
#else
    do {
		if (!recursive_action(*argv,
			OPT_RECURSE,    // recurse
