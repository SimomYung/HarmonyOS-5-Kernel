...
//config:
//config:config FEATURE_LN_ROOT_LIMIT
//config:	bool "Only the root user can run the ln command."
//config:	default n
...
#if ENABLE_FEATURE_LN_ROOT_LIMIT
	uid_t euid = geteuid();
	uid_t ruid = getuid();
	if (ruid != 0 || euid != 0) {
		bb_error_msg_and_die("uid:%d is not root, the ln command cannot be used", ruid);
	}
#endif
