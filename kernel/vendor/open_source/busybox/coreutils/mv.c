...
//config:
//config:config FEATURE_MV_DEFAULT_INTERACTIVE
//config:	bool "Enable default -i"
//config:	default n
//config:	depends on MV
//config:	help
//config:	By default, users need to confirm each other when files are overwritten.
...
#define OPT_VERBOSE     ((1 << 5) * ENABLE_FEATURE_VERBOSE)
...
#if ENABLE_FEATURE_MV_DEFAULT_INTERACTIVE
	if (!(flags & OPT_FORCE)) {
		...
		if (argc != 2) {
			...
			if (flags & OPT_DESTNOTDIR) {
				if (stat(argv[0], &statbuf) == 0 && !S_ISDIR(statbuf.st_mode))
					...
					bb_error_msg("'%s' is a directory", last);
					return EXIT_FAILURE;
#else
	...
	do {
#if ENABLE_FEATURE_NOFORK_MV
		dest = concat_path_file_nofork(last, bb_get_last_path_component_strip(*argv));
		if (dest == NULL) {
			goto RET_0;
		}
#else
...
#endif
#if ENABLE_FEATURE_NOFORK_MV
				if ((copy_file(*argv, dest, copy_flag, false) >= 0)
