...
#  define OFF_FMT "ll"
# endif
# if !defined(__GLIBC__) && !defined(__UCLIBC__) && ENABLE_FEATURE_FTP_ADAPT_MUSL
...
# endif
#endif
#if defined(__GLIBC__) || !ENABLE_FEATURE_FTP_ADAPT_MUSL
/* scary. better ideas? (but do *test* them first!) */
#define OFF_T_MAX  ((off_t)~((off_t)1 << (sizeof(off_t)*8-1)))
/* Users report bionic to use 32-bit off_t even if LARGEFILE support is requested.
...
#if ENABLE_MONOTONIC_US_PRECISE
unsigned long long monotonic_us_precise(void) FAST_FUNC;
#endif
...
extern int copy_file(const char *source, const char *dest, int flags, bool isFork) FAST_FUNC;
#else
...
#define CHECK_UID_ALLOW 0
#define CHECK_UID_DENY 1
...
#if ENABLE_FEATURE_IFCONFIG_INTER_STATISTICS
int query_interface_statistics(const char *query_ifname) FAST_FUNC;
#endif
...
#if !ENABLE_FIX_CPUTIME_OVERFLOW
...
extern bool is_applet_nofork_extension(int applet_no);
extern bool is_applet_allow_signal_on(int applet_no);
extern char *malloc_readlink(const char *path) FAST_FUNC RETURNS_MALLOC;
extern char *malloc_readlink_or_warn(const char *path) FAST_FUNC RETURNS_MALLOC;
extern int print_numbered_lines_nofork(struct number_state *ns, const char *filename) FAST_FUNC;
extern char *malloc_fgetline(FILE *file) FAST_FUNC RETURNS_MALLOC;
extern char *bb_get_chunk_from_file_nofork(FILE *file, size_t *end) FAST_FUNC;
extern void add_to_ino_dev_hashtable_nofork(const struct stat *statbuf, const char *name) FAST_FUNC;
extern char *asprintf_nofork(const char *format, ...) __attribute__ ((format(printf, 1, 2))) FAST_FUNC RETURNS_MALLOC;
extern char *concat_path_file_nofork(const char *path, const char *filename) FAST_FUNC;
#endif
