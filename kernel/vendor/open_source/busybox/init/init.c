...
//config:
//config:config FIX_SECURITY_BOOT_INIT
//config:	bool "fix bug in security_boot_init"
//config:	default n
//config:	depends on INIT || LINUXRC
//config:	help
//config:	does not use single-user mode by default
//config:
//config:config FEATURE_ADD_SELINUX_LABLE
//config:	bool "add selinux label for init"
//config:	default n
//config:	depends on SELINUX
//config:	help
//config:	init process lack selinux label before restorecon, it
//config:	inherits label kernel_t from parent process. kernel_t
//config:	is inapprociate to init process, so give specific label
//config:	os_init_t to it.
//config:
//config:config FEATURE_SUPPORT_KEXEC
//config:	bool "support reboot through kexec"
//config:	default n
//config:	depends on REBOOT
//config:	help
//config:	After the user loads the kexec image,
//config:	you can use the reboot --kexec command to implement a
//config:	command similar to kexec -e, but we will reuse the reboot
//config:	process to complete the uninstallation of the file system
//config:	and the operation of the killall process.
//config:
//config:config FIX_SVR_MONIT_PROBLEM
//config:	bool "fix bug in svr_monit"
//config:	default n
//config:	depends on INIT || LINUXRC
//config:	help
//config:	the daemon process added in inittab will be infinitely redrawn
...
#include <syslog.h>
#if ENABLE_FIX_SVR_MONIT_PROBLEM
#include <sched.h> /*after vfork,schedule*/
#endif
#ifdef __linux__
# include <linux/vt.h>
...
#if ENABLE_FIX_SVR_MONIT_PROBLEM
/* This function remove the char "c" of the command.
 * Exmaple:
 * @Input@ command string may be the following format,and need to remove char ' ':
 *     /usr/sbin/syslog-ng -f /etc/syslog-ng/syslog-ng.conf -p /var/run/syslogd.pid -F
 * @Output@ /usr/sbin/syslog-ng-f/etc/syslog-ng/syslog-ng.conf-p/var/run/syslogd.pid-F
 * @Return@ output string length after cut
 */
static int strcut(char *input, char *output, char c, int length)
{
	char *p_in = NULL;
	char *p_out = NULL;
	int len = 0;
...
static int is_parent_init(char *dir_name)
{
	char tmpdir[2 * PROC_PATH_LEN] = {0};
	char *buf;
	FILE *fp = NULL;
	int size = 2 * PROC_PATH_LEN;
	int ppid = 0;
	int found = 0;

	snprintf(tmpdir, PROC_PATH_LEN, "/proc/%s/status", dir_name);

	fp = fopen(tmpdir, "r");
	if (fp == NULL) {
		message(L_CONSOLE, "fopen %s failed!", tmpdir);
		return 0;
	}
	...
	while (fgets(buf, size, fp)) {
		if (strstr(buf, "PPid:")) {
			sscanf(buf, "%s %d", tmpdir, &ppid);
			if (ppid == 1) /* Found deamon process */
				found = 1;
			break;
		}
	}
...
/*
 * scanning directory '/proc' and  getting a deamon pid with its cmdline
 */
static int get_pid(char *cmd)
{
	DIR *dp = NULL;
	struct dirent *dirp = NULL;
	char tmpdir[PROC_PATH_LEN] = {0};
	char *buf = NULL;
	char *tmpbuf = NULL;
	char *cmdline = NULL;
	int len = 0;
	...
	int rdcnt = 0;
	int found = 0;
	const char *getty = "/sbin/getty";

	if (NULL == cmd) {
		message(L_CONSOLE, "%s %d:input cmd is NULL!", __FUNCTION__, __LINE__);
		return -1;
	}

	/*When cmd is "/sbin/getty", traverse /proc will make serial dead! Who know why?*/
	if (strncmp(cmd, getty, 11) == 0)
		return 0;

	if ((dp = opendir("/proc")) == NULL) {
		message(L_CONSOLE, "%s %d:Can not open /proc directory!", __FUNCTION__, __LINE__);
		return -1;
	}

	buf = (char *)malloc(COMMAND_SIZE*3);
	if (!buf) {
		message(L_CONSOLE, "%s %d:malloc memory failed!", __FUNCTION__, __LINE__);
		return -1;
	}
	tmpbuf = &buf[COMMAND_SIZE];
	cmdline = &buf[COMMAND_SIZE*2];

	memset(cmdline, 0, COMMAND_SIZE);
	/* remove char ' ' from cmd string,because the /proc/[pid]/cmdline does not contain spaces */
	len = strcut(cmd, cmdline, ' ', strlen(cmd));
	...
	while ((dirp = readdir(dp))!= NULL ) {
		...
		if (pid <= 1)
			continue;
		snprintf(tmpdir, PROC_PATH_LEN, "/proc/%s/cmdline", dirp->d_name);

		fd = open(tmpdir, O_RDONLY);
		if (fd < 0) {
			message(L_CONSOLE, "%s %d: warning: open %s failed!", __FUNCTION__, __LINE__, tmpdir);
			continue;
		}

		memset(buf, 0, COMMAND_SIZE);
		rdcnt = read(fd, buf, COMMAND_SIZE);
		close(fd);
		if (rdcnt > 0) {
			memset(tmpbuf, 0, COMMAND_SIZE);
			strcut(buf, tmpbuf, '\0', rdcnt); /* remove char '\0' from read string, otherwise strncmp will be failed. */

			/*compare cmdline is or not equal*/
			ret = strncmp(tmpbuf, cmdline, len);
			if (ret != 0)
				continue;
 
			if (is_parent_init(dirp->d_name) == 1) {
				found = 1;
				break;
			}
		}
...
static int get_cmd_pid(char *cmd)
{
	int ret = 0;
	usleep(300000); /* 300ms, avoid cmd pid not changed when exit signal coming */
	ret = get_pid(cmd);
	if (ret >= 0) {
		return ret;
	} else {
		/* Not find cmd pid,sleep some time and find again.
		 * If find cmd's pid failed and return the error pid,
		 * init will not be able to pull up this service.
		 */
		usleep(120000); /* 120ms */
		ret = get_pid(cmd);
		if (ret < 0)
			message(L_CONSOLE, "svrmonit:\"%s\" not find right pid!", cmd);
	}
	return ret;
}
#endif
...
static void run_actions(int action_type)
{
	...
	for (a = G.init_action_list; a; a = a->next) {
		...
		if (a->action_type & (RESPAWN | ASKFIRST)) {
			...
			if (a->pid == 0) {
				a->pid = run(a);
				/* check the return pid, if it is running
				 * background, update a->pid with its real pid.
				 */
				if (a->action_type & RESPAWN) {
					/* relinquish the CPU and sleep 4ms,schedule other processes to run,
					 * fixup: can't find right pid though cmdline.
					 */
					sched_yield();
					usleep(4000);

					cmdpid = get_cmd_pid(a->command);
					if ((cmdpid > 0) && (cmdpid != a->pid))
						a->pid = cmdpid;
				}
			}
#else
...
#if ENABLE_FEATURE_SUPPORT_KEXEC
	else if (sig == SIGKEXEC) {
		m = "reboot through kexec";
		rb = RB_KEXEC;
	}
#endif
...
#if ENABLE_FEATURE_SUPPORT_KEXEC
	sigaddset(&G.delayed_sigset, SIGKEXEC); /* signal for kexec */
#endif
...
#if !ENABLE_FIX_SECURITY_BOOT_INIT
...
#if ENABLE_SELINUX
#if ENABLE_FEATURE_ADD_SELINUX_LABLE
#define INIT_PATH "/bin/busybox.nosuid"
#define INIT_CONTEXT "system_u:object_r:os_init_exec_t:s0:c0"
#endif
...
#if ENABLE_FEATURE_ADD_SELINUX_LABLE
			setfilecon(INIT_PATH, INIT_CONTEXT);
#endif
