...
//config:config FEATURE_SUPPORT_NON_ROOT_PROCS
//config:	bool "Add ability to support non root processes"
//config:	default n
//config:	help
//config:	Add prctl function to retain capability permissions
//config:	of non-root processes.
...
#if ENABLE_FEATURE_SUPPORT_NON_ROOT_PROCS
#include <sys/prctl.h>
#endif
...
//config:config FEATURE_APPLET_BLACKLIST
//config:	bool "Provide busybox command blacklist function"
//config:	default n
//config:	depends on BUSYBOX
//config:	help
//config:	With this option busybox support command blacklist
//config:	function, the blacklist configuration file is placed
//config:	in /etc/busybox-blacklist.
#if ENABLE_FEATURE_APPLET_BLACKLIST
#define MAX_APPLET_NAME_SIZE 24
#define MAX_BUF_LEN 32
static int FAST_FUNC find_from_blacklist(const char *name)
{
	int ret = -1;
	char buf[MAX_BUF_LEN] = {0};
	int length;
	bool invalid_print = false;
 
	FILE *fp = fopen_for_read("/etc/busybox-blacklist");
	if (fp == NULL)
		return ret;
 
	while(fgets(buf, MAX_BUF_LEN, fp) != NULL) {
		length = strlen(buf);
		if (length > MAX_APPLET_NAME_SIZE + 1) {
			if (!invalid_print) {
			    printf("Busybox blacklist invaild, please check\n");
			}
			invalid_print = true;
			continue;
		}
...
#if ENABLE_FEATURE_SUPPORT_NON_ROOT_PROCS
		/*
		 * Ability to support non-root processes
		 * If the busybox.nosuid is executed under non-root,
		 * the weight reduction by setuid will result in the
		 * file capability being unavailable. In order to keep
		 * the file capability available, use prctl to retain
		 *　privileges and avoid drop privileges after setuid
		 */
...
#  if NUM_APPLETS > 0
	...
	{
		int applet = find_applet_by_name(name);
#   if ENABLE_FEATURE_APPLETLIB_UID_LIMIT
		if ((applet >= 0) && (check_applet_uid_limit(name) == CHECK_UID_DENY)) {
			full_write2_str(name);
			full_write2_str(": applet is limit for current user\n");
			exit(127);
		}
#   endif
#   if ENABLE_FEATURE_APPLET_BLACKLIST
		if ((applet >= 0) && find_from_blacklist(name) != 0)
#   else
		if (applet >= 0)
#   endif
...
/* This array is used by the main ash process to determine
 * whether the nofork mechanism is enabled for each command.
 */
#if ENABLE_FEATURE_NOFORK_EXTENSION

#ifndef NUM_APPLETS
#define NUM_APPLETS 1024
#endif

static bool is_force_built_in[NUM_APPLETS] = {
#if ENABLE_FEATURE_NOFORK_STAT
	[APPLET_NO_stat] = true,
#endif
#if ENABLE_FEATURE_NOFORK_CHOWN
	[APPLET_NO_chown] = true,
#endif
#if ENABLE_FEATURE_NOFORK_DATE
	[APPLET_NO_date] = true,
#endif
#if ENABLE_FEATURE_NOFORK_CHMOD
	[APPLET_NO_chmod] = true,
#endif
#if ENABLE_FEATURE_NOFORK_LS
	[APPLET_NO_ls] = true,
#endif
#if ENABLE_FEATURE_NOFORK_MKDIR
	[APPLET_NO_mkdir] = true,
#endif
#if ENABLE_FEATURE_NOFORK_SLEEP
	[APPLET_NO_sleep] = true,
#endif
#if ENABLE_FEATURE_NOFORK_WC
	[APPLET_NO_wc] = true,
#endif
#if ENABLE_FEATURE_NOFORK_TOUCH
	[APPLET_NO_touch] = true,
#endif
#if ENABLE_FEATURE_NOFORK_CAT
	[APPLET_NO_cat] = true,
#endif
#if ENABLE_FEATURE_NOFORK_RM
	[APPLET_NO_rm] = true,
#endif
#if ENABLE_FEATURE_NOFORK_MV
	[APPLET_NO_mv] = true,
#endif
#if ENABLE_FEATURE_NOFORK_READAHEAD
	[APPLET_NO_readahead] = true,
#endif
#if ENABLE_FEATURE_NOFORK_UNAME
	[APPLET_NO_uname] = true,
#endif
};

bool is_applet_nofork_extension(int applet_no)
{
	if (applet_no < 0 || applet_no >= NUM_APPLETS)
		return false;
	return is_force_built_in[applet_no];
}

/* The sleep function is implemented to allow the processing signal
 * to interrupt execution.
 * To cancel sleep [num], press CTRL-C
 */
bool is_applet_allow_signal_on(int applet_no)
{
	return applet_no == APPLET_NO_sleep;
}
#endif
