...
int FAST_FUNC copy_file(const char *source, const char *dest, int flags, bool isFork)
#else
	...
	if (S_ISDIR(source_stat.st_mode)) {
...
#if ENABLE_FEATURE_NOFORK_MV
		if (!isFork) {
			add_to_ino_dev_hashtable_nofork(&dest_stat, NULL);
		} else {
		...
		while ((d = readdir(dp)) != NULL) {
			...
			if (new_source == NULL)
				continue;
#if ENABLE_FEATURE_NOFORK_MV
			new_dest = concat_path_file_nofork(dest, d->d_name);
			if (new_dest == NULL) {
				free(new_source);
				closedir(dp);
				return -1;
			}
			if (copy_file(new_source, new_dest, flags & ~FILEUTILS_DEREFERENCE_L0, isFork) < 0)
				retval = -1;
#else
...
#if ENABLE_FEATURE_NOFORK_MV
			if (!isFork) {
				add_to_ino_dev_hashtable_nofork(&source_stat, dest);
			} else {
