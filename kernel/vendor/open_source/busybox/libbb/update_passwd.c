...
#define member NULL
	...
	int changed_lines;
	int ret = -1; /* failure */
#if ENABLE_FEATURE_DELUSER_BUGFIX_ERR_PRINT
	bool is_initgrp = false;
#endif
...
#if ENABLE_FEATURE_DELUSER_BUGFIX_ERR_PRINT
			is_initgrp |= (strncmp(line, member, member_len) == 0 && line[member_len] == ':') ? true : false;
#endif
...
#if ENABLE_FEATURE_DELUSER_BUGFIX_ERR_PRINT
			if (ENABLE_DELGROUP && applet_name[0] == 'd' && !is_initgrp)
#else
