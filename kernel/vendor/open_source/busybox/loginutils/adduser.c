...
//config:
//config:config FEATURE_HM_SEC_USERMGT
//config:	bool "Enable hm security user management feature"
//config:	default n
//config:	depends on PAM
//config:	help
//config:	hm security user management feature, depends on pam
...
int adduser_main(int argc UNUSED_PARAM, char **argv)
{
	...
	/* fields:     2 3  4 5     6 78 */
#if ENABLE_FEATURE_HM_SEC_USERMGT
	p = xasprintf("!:%u:0:90:7:::", (unsigned)(time(NULL)) / (24*60*60));
#else
	...
	if (!(opts & OPT_DONT_MAKE_HOME)) {
		...
		if (mkdir_err == 0) {
			...
			copy_file(skel, pw.pw_dir, FILEUTILS_RECUR, true);
#else
...
#if ENABLE_FEATURE_HM_SEC_USERMGT
		/* The security baseline requires the homedir permission to be 0750 */
		 || chmod(pw.pw_dir, 00750) != 0
#else
