...
//config:
//config:config FEATURE_PASSWD_CHECK_CATEGORIES_NUMBER
//config:	bool "New passwords should contain at least two types of characters"
//config:	default n
//config:	depends on FEATURE_PASSWD_WEAK_CHECK
//config:	help
//config:	With this option new passwords containing one type of characters
//config:	is viewed as "weak" passwords.
//config:
//config:config FEATURE_PASSWD_HINT_CATEGORIES_NUMBER
//config:	bool "Show characters requirement before setting passwords"
//config:	default n
//config:	depends on FEATURE_PASSWD_CHECK_CATEGORIES_NUMBER
//config:	help
//config:	With this option user get the requirement of characters before setting
//config:	passwords.
...
#if ENABLE_FEATURE_HM_SEC_USERMGT
...
static int pam_passwd(const struct passwd *pw, uid_t myuid)
{
	int err;
	pam_handle_t *handle;
	struct pam_conv conv = {misc_conv, NULL};

	err = pam_start("passwd", pw->pw_name, &conv, &handle);
	if (err == PAM_SUCCESS) {
		err = pam_chauthtok(handle, 0);
	}
	if (err == PAM_SUCCESS) {
		bb_info_msg("password updated successfully");
	} else {
		bb_info_msg("password unchanged");
	}

	(void) pam_end(handle, err);
	return err;
}
#endif
...
#if ENABLE_FEATURE_PASSWD_HINT_CATEGORIES_NUMBER
    puts("Please use a combination of at least two types of characters");
    puts("upper or lower case letters or numbers or special characters");
#endif
