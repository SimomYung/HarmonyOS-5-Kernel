...
//config:	depends on SU
//config:
//config:config FEATURE_SU_HIDE_FAIL_REASON
//config:	bool "Do not inform user the fail reason"
//config:	default n
//config:	depends on SU
//config:
...
static int pam_check_perms(const char *name, const char *oname)
{
	pam_handle_t *pamh = NULL;
	struct pam_conv conv = {misc_conv, NULL};
	int err;

	err = pam_start ("su", name, &conv, &pamh);
	if (err == PAM_SUCCESS) {
	...
	if (err == PAM_NEW_AUTHTOK_REQD) {
		bb_info_msg("Your account has expired");
	}

	if (ENABLE_FEATURE_SU_SYSLOG) {
		syslog(LOG_NOTICE, "su: %s to %s %s", oname, name,
		(err == PAM_SUCCESS ? "succeeded" : "failed"));
	}
...
int su_main(int argc UNUSED_PARAM, char **argv)
{
	...
	if (pam_check_perms(opt_username, old_user) != PAM_SUCCESS) {
		bb_simple_error_msg_and_die("pam check user permission failed");
	}
...
#if ENABLE_FEATURE_SU_HIDE_FAIL_REASON
		bb_simple_error_msg_and_die("incorrect user name or password");
#else
