...
#ifdef __linux__
//config:config FEATURE_VLOCK_CLEAN_SCREEN
//config:	bool "clean the screen when locks"
//config:	default n
//config:	depends on VLOCK
//config:	help
//config:	Due to the security requirements of Germany CC certification,
//config:	With this option busybox support clean the screen when vlock
//config:	is enabled.
#if ENABLE_FEATURE_VLOCK_CLEAN_SCREEN
	ioctl(STDIN_FILENO, VT_GETMODE, &vtm);
#else
