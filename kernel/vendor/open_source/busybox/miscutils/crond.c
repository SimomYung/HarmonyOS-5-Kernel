...
//config:
//config:config FIX_CROND_ZOMBIE_CMD
//config:	bool "solve crond zombie no exit cmd"
//config:	default n
//config:	depends on CROND
//config:	help
//config:	Resolve the zombie process that is not launched.
...
#if ENABLE_FIX_CROND_ZOMBIE_CMD
        waitpid(pid, NULL, 0);
#endif
	/*
