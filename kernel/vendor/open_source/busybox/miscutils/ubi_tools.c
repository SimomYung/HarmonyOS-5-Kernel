...
//config:
//config:config FEATURE_UBIATTACH_SET_BADBLOCK_RATIO
//config:	bool "set badblock ratio"
//config:	default n
//config:	depends on UBIATTACH
//config:	help
//config:	with option on busybox can set max_beb_per1024 in strcuture "ubi_attach_req",
//config:	to specify the maximum number of bit errors per 1024 bytes that the UBI driver
//config:	should tolerate when scanning for UBI volumes during the attach process
...
#if ENABLE_FEATURE_UBIATTACH_SET_BADBLOCK_RATIO
	/* The variable in the "attach_req" structure is used to specify
	 * the maximum number of bearers that the UE (User Equipment)
	 * can establish per 1024 kbps of downlink data rate.
	 */
	int max_beb_per1024 = 0;
#endif
	char *vol_name;
...
#if ENABLE_FEATURE_UBIATTACH_SET_BADBLOCK_RATIO
		opts = getopt32(argv, "^" "m:+d:+n:+N:s:a:+t:b:+" "\0" "-1",
				&mtd_num, &dev_num, &vol_id,
				&vol_name, &size_bytes_str, &alignment, &type, &max_beb_per1024
		);
#else
...
//usage:     IF_FEATURE_UBIATTACH_SET_BADBLOCK_RATIO(
//usage:     "\n	-b MAX_BEB_PER1024    maximum expected bad block number per 1024 eraseblock")
	if (do_attach) {
		if (!(opts & OPTION_m))
...
#if ENABLE_FEATURE_UBIATTACH_SET_BADBLOCK_RATIO
		attach_req.max_beb_per1024 = (short)max_beb_per1024;
#endif
