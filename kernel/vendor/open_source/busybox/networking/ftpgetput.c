...
//config:
//config:config FEATURE_FTP_TIMEOUT
...
//config:
//config:config FEATURE_FTPGETPUT_SUGGEST_USE_SFTP_PRINT
//config:	bool "Print suggestion to use sftp tool"
//config:	default n
//config:	depends on FTPGET || FTPPUT
//config:
//config:config FEATURE_FTPGETPUT_GET_PASSWORD_INTERACITVELY
//config:	bool "Get password through interactive method"
//config:	default n
//config:	depends on FTPGET || FTPPUT
//config:
//config:config FEATURE_FTPGETPUT_SET_USERINFO_NULL
//config:	bool "Set user information(name and password) to null"
//config:	default n
//config:	depends on FTPGET || FTPPUT
//config:	help
//config:	with the option on, there is no anonymous user in busybox,
//config:	meeting the security requirements and adapting to the hm kernel 
//config:config FEATURE_FTP_ADAPT_MUSL
//config:	bool "add busybox-ftp to musl platform"
//config:	default n
//config:	depends on FTPGET || FTPPUT
//config:	help
//config:	add busybox-ftp to musl platform andfix compilation
//config:	for musl on 32-bit archs.
...
static void ftp_set_timeout_opts(int fd, int set_ftp_login)
{
	...
	if (setsockopt(fd, SOL_TCP, TCP_KEEPIDLE, &opt_val, sizeof(opt_val)) < 0) {
...
static int creat_tcp_connection(int timeout_option)
{
	int fd;
	fd = xsocket(lsa->u.sa.sa_family, SOCK_STREAM, 0);
	ftp_set_timeout_opts(fd, timeout_option);
	xconnect(fd, &lsa->u.sa, lsa->len);
	return fd;
}

static void keepalive_timeout_ctrl(int flags)
{
...
{
#if ENABLE_FEATURE_FTP_TIMEOUT
	control_stream = fdopen(creat_tcp_connection(SET_FTP_CSTREAM), "r+");
#else
...
#if ENABLE_FEATURE_FTP_TIMEOUT
	return creat_tcp_connection(SET_FTP_DSTREAM);
#else
...
#endif
#if ENABLE_FEATURE_FTPGETPUT_SUGGEST_USE_SFTP_PRINT
    printf("Warning: ftp is not a secure protocol, and it is recommended to use sftp. \n");
#endif
	INIT_G();
	/* Set default values */
#if ENABLE_FEATURE_FTPGETPUT_SET_USERINFO_NULL
	/* No user management in the hm kernel */
	user = "";
	password = "";
#else
	user = "anonymous";
...
#define OPTSTRING "^cvu:p:P:ts:+d:+S:+D:+k:+i:+l:+n:+""\0""-2:vv:cc"
	...
	argv += optind;
	keepalive_timeout_ctrl(flags);
#endif
#if ENABLE_FEATURE_FTPGETPUT_GET_PASSWORD_INTERACITVELY
