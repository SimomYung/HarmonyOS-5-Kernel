...
//config:
//config:config FEATURE_HTTPD_SECURE
//config:	bool "Add secure header to response"
//config:	default n
//config:	depends on HTTPD
//config:	help
//config:	Add secure header to response.
//config:
...
#if ENABLE_FEATURE_HTTPD_SECURE
	/* Add the security field to HTTPD response header. */
	len += sprintf(iobuf + len,
		"Content-Security-Policy: default-src 'self' 'unsafe-inline' 'unsafe-eval'; img-src 'self' https: data:\r\n");
	len += sprintf(iobuf + len, "X-Content-Type-Options: nosniff\r\n");
	len += sprintf(iobuf + len, "X-Download-Options: noopen\r\n");
	len += sprintf(iobuf + len, "X-Frame-Options: SAMEORIGIN\r\n");
	len += sprintf(iobuf + len, "X-XSS-Protection: 1; mode=block\r\n");
#endif
