...
//config:
//config:config HM_TELNETD_WATCHCHILD
//config:	bool "make some adaptions while set OPT_WATCHCHILD of opt"
//config:	default n
//config:	depends on TELNETD && FEATURE_TELNETD_STANDALONE
//config:	help
//config:	enable when OPT_WATCHCHILD of opt needs to be set.
//config:	If not, when bash exits, some resource won't be cleaned up in hongmeng
//config:
//config:config HM_TELNETD_NONSUPPORT_LOGIN
//config:	bool "make some adaptions while hm doesn't support login"
//config:	default n
//config:	depends on TELNETD && FEATURE_TELNETD_STANDALONE
//config:	help
//config:	enable when /bin/login not support in hongmeng
...
make_new_session(
	...
	enum { sock = 0 };
#endif
#if ENABLE_HM_TELNETD_NONSUPPORT_LOGIN
	 ...
	 * the O_NOCTTY option (see open()), it is implementation-defined whether
	 * the terminal becomes the controlling terminal of the session leader.
	 *
	 * Consider current implementation on hongmeng, the terminal not becomes
	 * the ctty automaticly so we need set it manually.
	 */
	ioctl(0, TIOCSCTTY, NULL);

	/* FIXME: not support /dev/tty, use CONSOLE=xxx as temp solution */
	setenv("CONSOLE", tty_name, 1);
#endif
	xdup2(0, 1);
	xdup2(0, 2);
...
#if ENABLE_HM_TELNETD_NONSUPPORT_LOGIN
	login_argv[0] = "/bin/bash";
	login_argv[1] = "-l";
	login_argv[2] = NULL;
	G.loginpath = "/bin/bash";
#else
...
#ifdef __hongmeng
	/* "normal" telnetd just closes ptyfd,
	 * doesn't send SIGKILL in hongmeng */
	if (ts->shell_pid > 0) {
...
#if ENABLE_HM_TELNETD_WATCHCHILD
	opt = opt | OPT_WATCHCHILD;
#endif
