...
//config:
//config:config FEATURE_D6_DHCP_REQUEST_WITH_IANA_IPADDRESS
//config:   bool "Enable the request packet sent by dhcpc6 to carry IANA_IPAddress."
//config:   default n
//config:   depends on UDHCPC6
//config:	help
//config:	To be compatible with the ONT, the client adds the IP address
//config:	to the IA of the request frame when sending the request frame.
...
static void handle_server_info(char *dev_key, const uint8_t *option, int addrs, int option_offset)
{
	char *dlist;
	...
	*new_env() = dlist = xmalloc(strlen(dev_key) + addrs * 40 - 1);
	...
	while (addrs--) {
        /* The first 4 bytes of the DNS server address option are the option code and option length. */
		dlist += sprint_nip6(dlist, option + 4 + option_offset);
        /* each information occupied 16 bits */
		option_offset += 16;
		if (addrs)
			*dlist++ = ' ';
	}
...
{
#if ENABLE_FEATURE_UDHCPC6_RFC3646 || ENABLE_FEATURE_UDHCPC6_RFC4075
 ...
 * server.
      ...
      +-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+
 */
#if ENABLE_FEATURE_UDHCPC6_CONSISTENT_IAID
static int d6_create_iaid()
{
	...
	mac_len = sizeof(client_data.client_mac)/sizeof(client_data.client_mac[0])*sizeof(uint8_t);
...
#if !ENABLE_FEATURE_D6_DHCP_REQUEST_WITH_IANA_IPADDRESS
                if (packet.d6_msg_type == D6_MSG_ADVERTISE) {
...
#if ENABLE_FEATURE_D6_DHCP_REQUEST_WITH_IANA_IPADDRESS
				/* Note: request with ip address to be compatible with DHCPv6 server of the ONT */
                if (packet.d6_msg_type == D6_MSG_ADVERTISE) {
					/* enter requesting state */
					change_listen_mode(LISTEN_RAW);
					client_data.state = REQUESTING;
