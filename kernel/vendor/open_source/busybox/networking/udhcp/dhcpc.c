 ...
 */
//config:config FEATURE_UDHCPC_VENDER_SPEC_SET_DSCP
//config:	bool "Support DHCP option 125 (VenderSpec)"
//config:	default n
//config:	depends on UDHCPC
//config:	help
//config:	With this option dhcpc supports analyse option 125
...
#if ENABLE_FEATURE_UDHCPC_VENDER_SPEC_SET_DSCP
				const char *venderInfo = NULL;
				venderInfo = udhcp_get_option(&packet, DHCP_VENDER_SPEC_INFO);
				if (venderInfo != NULL) {
					if ((strcasestr(venderInfo, "Hu") != NULL) &&
						(strcasestr(venderInfo, "uAw") != NULL) &&
						(strcasestr(venderInfo, "awE") != NULL) &&
						(strcasestr(venderInfo, "eI") != NULL)) {
						bb_info_msg("rcv venderInfo:%s to set iptables", venderInfo);
						system("iptables -t mangle -A OUTPUT -j DSCP --set-dscp 0x5");
					}
				}
#endif
