...
//config:
//config:config FEATURE_SYSCTL_REPORT_ACCESS_FAILS
//config:	bool "solve the problem that no error is reported when access fails"
//config:	default n
//config:	help
//config:	When an general user without authority sets the system
//config:	configuration, 0 is returned, but the modification is not
//config:	successful. It is unreasonable.
...
#if ENABLE_FEATURE_SYSCTL_REPORT_ACCESS_FAILS
