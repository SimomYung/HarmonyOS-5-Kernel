...
//config:config FEATURE_TOP_PER_CPU
//config:	bool "Per CPU usage display ('c' key)"
//config:	default n
//config:	depends on FEATURE_TOP_SMP_CPU
//config:	help
//config:	Allow 'c' key to switch between individual/cumulative CPU stats
...
int top_main(int argc UNUSED_PARAM, char **argv)
{
	...
	if (col & OPT_H) {
        ...
        /* show per cpu info if "-c" is configure */
#if ENABLE_FEATURE_TOP_PER_CPU
	if (col & OPT_c) {
		cpu_jif = cpu_prev_jif = NULL;
		smp_cpu_info = 1;
