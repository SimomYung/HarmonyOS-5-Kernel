...
#if ENABLE_FDISK_SUPPORT_NOINTERACTIVE_OPTIONS
/* Self-developed patches to meet user nointeractive needs.
 * The option parameter description are as follows:
 * "-p": Set partition type.
 * The corresponding variable is part_type.
 * "-i": Select the partition number(1-4). The corresponding variable is addpart_id.
 * "-f": Select the first sector of the partition. The corresponding variable is part_s_cylinders_no.
 * "-e": Select the last sector of the partition. The corresponding variable is part_e_cylinders_no.
 * "-a": Toggle a bootable flag for the partition. The corresponding variable is boot_part_no.
 * "-t": Change a partition's system id. The corresponding variable is part_sys_id.
 * "-d": Delete a partition. The corresponding variable is delpart_id.
 * All parameters are passed in with parameters when the user uses self-developed parameters.
 */
unsigned int part_type = 0xffffffff;
unsigned int addpart_id = 0xffffffff;
unsigned int part_s_cylinders_no = 0xffffffff;
unsigned int part_e_cylinders_no = 0xffffffff;
unsigned int boot_part_no = 0xffffffff;
unsigned char *part_sys_id = 0;
unsigned int delpart_id = 0xffffffff;
unsigned int silence_mode = 0;
unsigned char currcmdkey = ' ';
#endif
...
//config:
//config:config FDISK_SUPPORT_NOINTERACTIVE_OPTIONS
//config:	bool "Enable nointeractive add or del partions"
//config:	default n
//config:	depends on FDISK
//config:	help
//config:	Enabling this option allows you to add or del one partion with just
//config:	one command line string, without several command line check item one
//config:	by one ;If you make busybox publication for  MPTG Board LinuxSmp
//config:	Project, it uses fdisk like "system("fdisk ...")", then, you obviously
//config:	should leave this enabled.
...
//usage:        IF_FDISK_SUPPORT_NOINTERACTIVE_OPTIONS(
//usage:     "\n        -p TYPE         Set partition type(0:primary partition 1:extended)"
//usage:     "\n        -i NUMBER       Select the partition number(1-4)"
//usage:     "\n        -f SECTOR       Select the first sector of the partition"
//usage:     "\n        -e SECTOR       Select the last sector of the partition"
//usage:     "\n        -a NUMBER       Toggle a bootable flag for the partition"
//usage:     "\n        -t SYSID        Change a partition's system id"
//usage:     "\n        -d NUMBER       Delete a partition"
//usage:        )
...
#if ENABLE_FDISK_SUPPORT_NOINTERACTIVE_OPTIONS
#define	DEFAULT_SILENCE_MODE    1
#define OPT_SILENCE_OFFSET      0x2000
#endif
...
#if ENABLE_FDISK_SUPPORT_NOINTERACTIVE_OPTIONS
	if (silence_mode == DEFAULT_SILENCE_MODE) {
		printf("Silence mode is set. Stop entering interactive mode.\n");
		exit(127);
	}
#endif
...
#if ENABLE_FDISK_SUPPORT_NOINTERACTIVE_OPTIONS
	/*
	 * Change the system ID of a partition, corresponding
	 * to "t change a partition's system id" in fdisk.
	 */
	while (1) {
		if ((currcmdkey == 't') && (part_sys_id != 0)) {
			v = bb_strtoul(part_sys_id, NULL, 16);
		} else {
...
#if ENABLE_FDISK_SUPPORT_NOINTERACTIVE_OPTIONS
/* Obtains the partition number entered by the user. */
static sector_t get_int_from_options(sector_t low, /* minimum */
		sector_t dflt,  /* defaults */
		sector_t high, /* max */
		unsigned int save_val) /* The value in the current command line parameter */
{
	if ((save_val >= low) && (save_val <= high))
		return save_val;
...
#if ENABLE_FDISK_SUPPORT_NOINTERACTIVE_OPTIONS
	/* Perform partitioning operations based on user input.  */
	if ((currcmdkey == 'd') && (delpart_id != 0xffffffff)) {
		i = get_int_from_options(1, 1, max, delpart_id) - 1;
	} else if ((currcmdkey == 'n') && (addpart_id != 0xffffffff)) {
		if (addpart_id < 1 || addpart_id > 4) {
			printf("The value passed in -i can only be 1 or 2 or 3 or 4.\n");
			exit(1);
		}
		if (addpart_id != boot_part_no) {
			printf("The value passed in -i and -a should be the same.\n");
			exit(1);
		}
		i = get_int_from_options(1, 1, max, addpart_id) - 1;
	} else if ((currcmdkey == 'a') && (boot_part_no != 0xffffffff)) {
		if (addpart_id != boot_part_no) {
			printf("The value passed in -i and -a should be the same.\n");
			exit(1);
		}
		i = get_int_from_options(1, 1, max, boot_part_no) - 1;
	} else {
...
#if ENABLE_FDISK_SUPPORT_NOINTERACTIVE_OPTIONS
			/* Adding a partition */
			if ((currcmdkey == 'n') && (part_s_cylinders_no != 0xffffffff)) {
				if (part_s_cylinders_no >= 1 && part_s_cylinders_no <= cround(limit) &&
					(part_s_cylinders_no <= part_e_cylinders_no)) {
					start = part_s_cylinders_no;
				} else {
					printf("The first sector value passed in -f is not in the valid range\n");
					exit(1);
				}
			} else {
...
#if ENABLE_FDISK_SUPPORT_NOINTERACTIVE_OPTIONS
		/* Confirm the end position of the partition */
		if ((currcmdkey == 'n') && (part_e_cylinders_no != 0xffffffff))
			stop = get_int_from_options(cround(start), cround(limit), cround(limit), part_e_cylinders_no);
		else
...
"table first (use 'o'). This will destroy the present disk contents.");
	...
	if (!free_primary) {
		...
		while (1) {
#if ENABLE_FDISK_SUPPORT_NOINTERACTIVE_OPTIONS
			/* Create a partition based on the partition type entered by the user. */
			if ((currcmdkey == 'n') && (part_type >= 2)) {
				printf("The parameter set by '-p' is out of range, please set '0' or '1'\n");
				exit(1);
			} else if ((currcmdkey == 'n') && (part_type < 2)) {
				char parttype[2] = {'p'};
				if (extended_offset) {
					parttype[1] = 'l';
				} else {
...
#endif
#if ENABLE_FDISK_SUPPORT_NOINTERACTIVE_OPTIONS
/* Invalid user input verification */
#define IF_USED_BY_NOINTERACTIVE if((part_type != 0xffffffff)\
	|| (addpart_id != 0xffffffff)\
	|| (part_s_cylinders_no != 0xffffffff)\
	|| (part_e_cylinders_no != 0xffffffff)\
	|| (boot_part_no != 0xffffffff)\
	|| (part_sys_id != 0)\
	|| (delpart_id != 0xffffffff))
#endif
...
int fdisk_main(int argc UNUSED_PARAM, char **argv)
{
...
#if ENABLE_FDISK_SUPPORT_NOINTERACTIVE_OPTIONS
	/*
	 * c52662:opt = getopt32(argv, "b:C:H:lS:u" IF_FEATURE_FDISK_BLKSIZE("s"),
	 *                              &sector_size, &user_cylinders, &user_heads, &user_sectors);
	 *
	 * "b:C:H:lS:f:e:a:t:d:u"
	 */
	opt = getopt32(argv, "b:+C:+H:+lS:+up:+i:+f:+e:+a:+t:d:+s" IF_FEATURE_FDISK_BLKSIZE("s"),
				&sector_size, &user_cylinders, &user_heads, &user_sectors,
				&part_type, &addpart_id, &part_s_cylinders_no,
				&part_e_cylinders_no, &boot_part_no, &part_sys_id, &delpart_id);
	if (opt & OPT_SILENCE_OFFSET)
		silence_mode = DEFAULT_SILENCE_MODE;
	IF_USED_BY_NOINTERACTIVE {
		silence_mode = DEFAULT_SILENCE_MODE;
	}
#else
	...
	while (1) {
		int c;
#if ENABLE_FDISK_SUPPORT_NOINTERACTIVE_OPTIONS
		/* Parameters required for adding or deleting a partition */
		unsigned char  addpara[5] = {'n', 'a', 't', 'w', 'p'};
		unsigned char  delpara[3] = {'d', 'w', 'p'};
		unsigned int  cmdcnt = 0;
		unsigned char* cmdptr = NULL;
#endif
		bb_putchar('\n');
#if ENABLE_FDISK_SUPPORT_NOINTERACTIVE_OPTIONS
		IF_USED_BY_NOINTERACTIVE {
			cmdcnt = (delpart_id != 0xffffffff)?(sizeof(delpara)/sizeof(delpara[0])):(sizeof(addpara)/sizeof(addpara[0]));
			cmdptr = (delpart_id != 0xffffffff)?delpara:addpara;
		...
		case 'd':
			{
				int j;
#if ENABLE_FDISK_SUPPORT_NOINTERACTIVE_OPTIONS
				if(delpart_id != 0xffffffff) {
					if(delpart_id <= 0) {
						printf("The value passed in -d should be greater than 0.\n");
						exit(1);
					}
				}
#endif
