...
//config:config IPCRM_ALL_SHM
//config:	bool "ipcrm all"
//config:	default n
//config:	depends on IPCRM
//config:	help
//config:	Remove all options only support shared memory segment
//config:	-ashm Remove all memory segments after their last detach
...
#if ENABLE_IPCRM_ALL_SHM
static int do_remove_all(type_id type)
{
	int maxid, index, id;
	int ret = 0;
	...
	if ((type == SHM) || (type == ALL)) {
		...
		for (index = 0; index <= maxid; index++) {
			id = shmctl(index, SHM_STAT, &shmseg);
			if (id < 0)
				continue;
			ret |= shmctl(id, IPC_RMID, NULL);
		}
...
//usage:       "[-MQS key] [-mqs id]" IF_IPCRM_ALL_SHM(" [-ashm]")
...
//usage:     "\n	-sS	Remove semaphore"
//usage:	IF_IPCRM_ALL_SHM(
//usage:     "\n\nRemove all options only support shared memory segment\n"
//usage:     "\n	-ashm Remove all memory segments after their last detach"
//usage:	)
...
#if ENABLE_IPCRM_ALL_SHM
	while ((c = getopt(argc, argv, "q:m:s:Q:M:S:a::")) != -1) {
#else
