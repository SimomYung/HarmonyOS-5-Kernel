...
//config:config FEATURE_IDENTIFY_LOOP_NOT_EXIST
//config:	bool "Indentify loop device not exist"
//config:	default n
//config:	depends on LOSETUP
//config:	help
//config:	Indentify loop device not exist
//config:
	...
	if (opt & OPT_f) {
		...
		do {
			if (n > MAX_LOOP_NUM)
				bb_simple_error_msg_and_die("no free loop devices");
			sprintf(dev, LOOP_FORMAT, n++);
#if ENABLE_FEATURE_IDENTIFY_LOOP_NOT_EXIST
			int fd_dev = open(dev, O_RDONLY);
			if (fd_dev < 0)
				bb_simple_error_msg_and_die("no free loop devices");
			else close(fd_dev);
#endif
