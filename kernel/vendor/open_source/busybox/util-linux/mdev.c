...
//config:
//config:config FEATURE_MDEV_SUPPORT_OLD_KERNEL
//config:	bool "mdev support kernel 2.x"
//config:	default n
//config:	depends on MDEV
//config:	help
//config:	Adapt to the 2.x kernel, the node path corresponding to the
//config:	/dev device is /sys/class/block.
...
#if ENABLE_FEATURE_MDEV_SUPPORT_OLD_KERNEL
