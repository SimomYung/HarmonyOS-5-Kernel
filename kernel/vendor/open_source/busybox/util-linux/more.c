...
//config:
//config:config BUGFIX_MORE_PERCENT_OVER
//config:	bool "bugfix in more"
//config:	default n
//config:	help
//config:	fix percent value overflow in more
...
int more_main(int argc UNUSED_PARAM, char **argv)
{
 ...
 loop_top:
			if (input != 'r' && please_display_more_prompt) {
				len = printf("--More-- ");
				if (st.st_size != 0) {
#if !ENABLE_BUGFIX_MORE_PERCENT_OVER
					...
					len += printf("(%u%% of %"OFF_FMT"u bytes)",
						...
						st.st_size);
#else
					/* fix percent value overflow
					 * example: st.st_size = 130, d = 1, ftello(file) = 120
					 * percent value will overflow, 120% */
					len += printf("(%u%% of %"OFF_FMT"u bytes)",
						(int) ((float)ftello(file) / (float)st.st_size * 100),
						st.st_size);
#endif
				}
