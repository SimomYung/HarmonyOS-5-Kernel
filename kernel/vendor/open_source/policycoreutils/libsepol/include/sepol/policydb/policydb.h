...
typedef int (*class_callback_fn_t)(unsigned char *, size_t);
void class_callback_register(class_callback_fn_t fn);
void class_callback_deregister(void);
#endif

#ifdef CONFIG_HKIP_SEHARMONY_PROT
typedef void *(*sepol_malloc_fn_t)(size_t size);
typedef void *(*sepol_calloc_fn_t)(size_t num, size_t size);
typedef char *(*sepol_strdup_fn_t)(const char *s);
typedef void (*sepol_free_fn_t)(void *ptr);

void prmem_callback_register(
	sepol_malloc_fn_t malloc_fn, sepol_calloc_fn_t calloc_fn, sepol_strdup_fn_t strdup_fn, sepol_free_fn_t free_fn);
