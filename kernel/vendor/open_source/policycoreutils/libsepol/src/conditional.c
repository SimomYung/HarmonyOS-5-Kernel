...
int cond_init_bool_indexes(policydb_t * p)
{
	...
	p->bool_val_to_struct = (cond_bool_datum_t **)
	    sepol_malloc(p->p_bools.nprim * sizeof(cond_bool_datum_t *));
#else
	p->bool_val_to_struct = (cond_bool_datum_t **)
...
#ifdef CONFIG_SEHARMONY_POLICY_SHARE
	booldatum = sepol_malloc(sizeof(cond_bool_datum_t));
#else
...
extern uint8_t *sepol_class_map;
#endif
...
#ifdef CONFIG_SEHARMONY_POLICY_SPLIT
	if (!sepol_class_map_test_bit(sepol_class_map, k->target_class))
		return 0;
#endif
...
#ifdef CONFIG_SEHARMONY_POLICY_SHARE
	list = sepol_malloc(sizeof(cond_av_list_t));
#else
...
#ifdef CONFIG_SEHARMONY_POLICY_SHARE
		expr = sepol_malloc(sizeof(cond_expr_t));
#else
...
int cond_read_list(policydb_t * p, cond_list_t ** list, void *fp)
{
	...
	for (i = 0; i < len; i++) {
#ifdef CONFIG_SEHARMONY_POLICY_SHARE
		node = sepol_malloc(sizeof(cond_node_t));
#else
...
#ifdef CONFIG_SEHARMONY
void cond_compute_xperms(avtab_t *ctab, avtab_key_t *key,
			 struct sepol_xperms_decision *xpermd)
{
	avtab_ptr_t node = NULL;
	if (ctab == NULL || key == NULL || xpermd == NULL) {
		return;
	}
	for (node = avtab_search_node(ctab, key);
	     node != NULL;
	     node = avtab_search_node_next(node, key->specified)) {
