...
uint32_t is_register_cb = false;

static sepol_malloc_fn_t sepol_malloc_fn = NULL;
static sepol_calloc_fn_t sepol_calloc_fn = NULL;
static sepol_strdup_fn_t sepol_strdup_fn = NULL;
static sepol_free_fn_t sepol_free_fn = NULL;

void prmem_callback_register(sepol_malloc_fn_t malloc_fn, sepol_calloc_fn_t calloc_fn, sepol_strdup_fn_t strdup_fn, sepol_free_fn_t free_fn)
{
	sepol_malloc_fn = malloc_fn;
	sepol_calloc_fn = calloc_fn;
	sepol_strdup_fn = strdup_fn;
	sepol_free_fn = free_fn;
	is_register_cb = true;
}

void prmem_callback_deregister(void)
{
	sepol_malloc_fn = NULL;
	sepol_calloc_fn = NULL;
	sepol_strdup_fn = NULL;
	sepol_free_fn = NULL;
	is_register_cb = false;
}
#endif

void *sepol_malloc(size_t size)
{
#ifdef CONFIG_HKIP_SEHARMONY_PROT
	if (is_register_cb)
		return sepol_malloc_fn(size);
#endif
	return malloc(size);
}

void *sepol_calloc(size_t num, size_t size)
{
#ifdef CONFIG_HKIP_SEHARMONY_PROT
	if (is_register_cb)
		return sepol_calloc_fn(num, size);
#endif
	return calloc(num, size);
}

char *sepol_strdup(const char *s)
{
#ifdef CONFIG_HKIP_SEHARMONY_PROT
	if (is_register_cb)
		return sepol_strdup_fn(s);
#endif
	return strdup(s);
}

void sepol_free(void *p)
{
#ifdef CONFIG_HKIP_SEHARMONY_PROT
	if (is_register_cb)
		sepol_free_fn(p);
...
static inline void try_free(void *p)
{
#ifdef CONFIG_HKIP_SEHARMONY_PROT
	if (!is_register_cb)
#endif
...
#ifdef CONFIG_HKIP_SEHARMONY_PROT
	x->types.protectable = true;
	x->negset.protectable = true;
#endif
...
#ifdef CONFIG_HKIP_SEHARMONY_PROT
	x->dominates.protectable = true;
	x->cache.protectable = true;
	x->roles.protectable = true;
#endif
...
static int roles_init(policydb_t * p)
{
	...
	role = sepol_calloc(1, sizeof(role_datum_t));
	if (!role) {
		rc = -ENOMEM;
		goto out;
	}
	key = sepol_malloc(strlen(OBJECT_R) + 1);
	if (!key) {
		rc = -ENOMEM;
		goto out_free_role;
	}
...
int policydb_init(policydb_t * p)
{
	...
	p->filename_trans = hashtab_create(HASHTAB_FILENAMETR, (1 << 10));
#else
...
#ifdef CONFIG_SEHARMONY_POLICY_SHARE
	p->range_tr = hashtab_create(HASHTAB_RANGETR, 256);
#else
...
#ifdef CONFIG_HKIP_SEHARMONY_PROT
	p->policycaps.protectable = true;
	p->permissive_map.protectable = true;
#endif
...
{
	try_free(p->p_common_val_to_name);
	p->p_common_val_to_name = (char **)
	    sepol_calloc(p->p_commons.nprim, sizeof(char *));
	...
	try_free(p->class_val_to_struct);
	p->class_val_to_struct = (class_datum_t **)
	    sepol_calloc(p->p_classes.nprim, sizeof(class_datum_t *));
	if (!p->class_val_to_struct)
		return -1;

	try_free(p->p_class_val_to_name);
	p->p_class_val_to_name = (char **)
	    sepol_calloc(p->p_classes.nprim, sizeof(char *));
...
int policydb_index_bools(policydb_t * p)
{
	...
	p->p_bool_val_to_name = (char **)
	    sepol_calloc(p->p_bools.nprim, sizeof(char *));
...
static int policydb_index_decls(sepol_handle_t * handle, policydb_t * p)
{
	...
	p->decl_val_to_struct =
	    sepol_calloc(num_decls, sizeof(*(p->decl_val_to_struct)));
#else
	p->decl_val_to_struct =
			  ...
			  policydb_t * p, unsigned verbose)
{
	...
	try_free(p->role_val_to_struct);
	p->role_val_to_struct = (role_datum_t **)
	    sepol_calloc(p->p_roles.nprim, sizeof(role_datum_t *));
	if (!p->role_val_to_struct)
		return -1;

	try_free(p->user_val_to_struct);
	p->user_val_to_struct = (user_datum_t **)
	    sepol_calloc(p->p_users.nprim, sizeof(user_datum_t *));
	if (!p->user_val_to_struct)
		return -1;

	try_free(p->type_val_to_struct);
	p->type_val_to_struct = (type_datum_t **)
	    sepol_calloc(p->p_types.nprim, sizeof(type_datum_t *));
	...
	for (i = SYM_ROLES; i < SYM_NUM; i++) {
		try_free(p->sym_val_to_name[i]);
		p->sym_val_to_name[i] = NULL;
		if (p->symtab[i].nprim) {
			p->sym_val_to_name[i] = (char **)
			    sepol_calloc(p->symtab[i].nprim, sizeof(char *));
		  ...
		  uint32_t scope, uint32_t avrule_decl_id, uint32_t * value)
{
	...
	if (scope_datum == NULL) {
		hashtab_key_t key2 = sepol_strdup((char *)key);
		if (!key2)
			return -ENOMEM;
		if ((scope_datum = sepol_malloc(sizeof(*scope_datum))) == NULL) {
			try_free(key2);
			return -ENOMEM;
		}
...
int type_set_cpy(type_set_t * dst, const type_set_t * src)
{
	...
	dst->flags = src->flags;
#ifdef CONFIG_HKIP_SEHARMONY_PROT
	dst->types.protectable = true;
#endif
	if (ebitmap_cpy(&dst->types, &src->types))
		return -1;
#ifdef CONFIG_HKIP_SEHARMONY_PROT
	dst->negset.protectable = true;
#endif
...
#ifdef CONFIG_HKIP_SEHARMONY_PROT
	t->types.protectable = true;
#endif
	if (ebitmap_read(&t->types, fp))
		return -1;
#ifdef CONFIG_HKIP_SEHARMONY_PROT
	t->negset.protectable = true;
#endif
...
#ifdef CONFIG_HKIP_SEHARMONY_PROT
	r->level[0].cat.protectable = true;
#endif
...
#ifdef CONFIG_HKIP_SEHARMONY_PROT
	r->level[1].cat.protectable = true;
#endif
	if (items > 1) {
					  ...
					  struct policy_file *fp)
{
	...
	for (i = 0; i < ncat; i++) {
#ifdef CONFIG_SEHARMONY_POLICY_SHARE
		cat = (mls_semantic_cat_t *) sepol_malloc(sizeof(mls_semantic_cat_t));
#else
	...
	ebitmap_for_each_bit(&l->cat, cnode, i) {
		if (ebitmap_node_get_bit(cnode, i)) {
			...
			open_cat = (mls_semantic_cat_t *)
			    sepol_malloc(sizeof(mls_semantic_cat_t));
#else
			open_cat = (mls_semantic_cat_t *)
		     ...
		     struct policy_file *fp, uint32_t nprim)
{
	...
	perdatum = sepol_calloc(1, sizeof(perm_datum_t));
	if (!perdatum)
		return -1;
...
static int common_read(policydb_t * p, hashtab_t h, struct policy_file *fp)
{
	...
	comdatum = sepol_calloc(1, sizeof(common_datum_t));
	if (!comdatum)
		return -1;
			    ...
			    int allowxtarget, struct policy_file *fp)
{
	...
	lc = NULL;
	for (i = 0; i < ncons; i++) {
		c = sepol_calloc(1, sizeof(constraint_node_t));
		if (!c)
			return -1;
		...
		depth = -1;
		for (j = 0; j < nexpr; j++) {
			e = sepol_malloc(sizeof(constraint_expr_t));
			if (!e)
				return -1;
...
static int class_read(policydb_t * p, hashtab_t h, struct policy_file *fp)
{
	...
	cladatum = (class_datum_t *) sepol_calloc(1, sizeof(class_datum_t));
	if (!cladatum)
		return -1;
	...
	if (len2) {
		cladatum->comkey = sepol_malloc(len2 + 1);
		if (!cladatum->comkey)
			goto bad;
...
static int role_read(policydb_t * p, hashtab_t h, struct policy_file *fp)
{
...
#ifdef CONFIG_HKIP_SEHARMONY_PROT
	role->dominates.protectable = true;
#endif
	...
	if (p->policy_type == POLICY_KERN) {
#ifdef CONFIG_HKIP_SEHARMONY_PROT
		role->types.types.protectable = true;
#endif
...
static int type_read(policydb_t * p, hashtab_t h, struct policy_file *fp)
{
	...
	typdatum = sepol_calloc(1, sizeof(type_datum_t));
	if (!typdatum)
		return -1;
	...
	if (p->policy_type != POLICY_KERN) {
#ifdef CONFIG_HKIP_SEHARMONY_PROT
		typdatum->types.protectable = true;
#endif
...
static int role_trans_read(policydb_t *p, struct policy_file *fp)
{
	...
	ltr = NULL;
	for (i = 0; i < nel; i++) {
		tr = sepol_calloc(1, sizeof(struct role_trans));
		if (!tr) {
			return -1;
		}
		if (ltr) {
...
static int role_allow_read(role_allow_t ** r, struct policy_file *fp)
{
	...
	lra = NULL;
	for (i = 0; i < nel; i++) {
		ra = sepol_calloc(1, sizeof(struct role_allow));
		if (!ra) {
			return -1;
		}
		if (lra) {
			      ...
			      uint32_t *present_otype)
{
	...
	if (!datum) {
		datum = sepol_malloc(sizeof(*datum));
		if (!datum)
			return SEPOL_ENOMEM;

#ifdef CONFIG_HKIP_SEHARMONY_PROT
		datum->stypes.protectable = true;
#endif
...
static int filename_trans_read_one_compat(policydb_t *p, struct policy_file *fp)
{
	...
	name = sepol_calloc(len + 1, sizeof(*name));
	if (!name)
		return -1;
...
#ifdef CONFIG_HKIP_SEHARMONY_PROT
	stypes.protectable = true;
	otypes.protectable = true;
#endif
	ebitmap_init(&stypes);
	ebitmap_init(&otypes);
	...
	policydb_t *p, struct policy_file *fp)
{
	...
	for (i = 0; i < info->ocon_num; i++) {
		...
		for (j = 0; j < nel; j++) {
#ifdef CONFIG_SEHARMONY_POLICY_SHARE
			c = sepol_calloc(1, sizeof(ocontext_t));
#else
...
#ifdef CONFIG_SEHARMONY_POLICY_SHARE
				c->u.ibendport.dev_name = sepol_malloc(len + 1);
#else
...
static int genfs_read(policydb_t * p, struct policy_file *fp)
{
	...
	for (i = 0; i < nel; i++) {
		...
		if (zero_or_saturated(len))
			goto bad;
		newgenfs = sepol_calloc(1, sizeof(genfs_t));
		if (!newgenfs)
			goto bad;
		newgenfs->fstype = sepol_malloc(len + 1);
		if (!newgenfs->fstype) {
			try_free(newgenfs);
			goto bad;
		}
		...
		for (j = 0; j < nel2; j++) {
#ifdef CONFIG_SEHARMONY_POLICY_SHARE
			newc = sepol_calloc(1, sizeof(ocontext_t));
#else
			...
			if (zero_or_saturated(len))
				goto bad;
#ifdef CONFIG_SEHARMONY_POLICY_SHARE
			newc->u.name = sepol_malloc(len + 1);
#else
...
static int user_read(policydb_t * p, hashtab_t h, struct policy_file *fp)
{
	...
	usrdatum = sepol_calloc(1, sizeof(user_datum_t));
	if (!usrdatum)
		return -1;
	...
	if (p->policy_type == POLICY_KERN) {
#ifdef CONFIG_HKIP_SEHARMONY_PROT
		usrdatum->roles.roles.protectable = true;
#endif
...
#ifdef CONFIG_SEHARMONY_POLICY_SHARE
	usrdatum->cache.protectable = true;
#endif
		     ...
		     struct policy_file *fp)
{
	...
	levdatum = sepol_malloc(sizeof(level_datum_t));
	if (!levdatum)
		return -1;
	...
	levdatum->level = sepol_malloc(sizeof(mls_level_t));
		    ...
		    struct policy_file *fp)
{
	...
	catdatum = sepol_malloc(sizeof(cat_datum_t));
	if (!catdatum)
		return -1;
...
static avrule_t *avrule_read(policydb_t * p, struct policy_file *fp)
{
...
#ifdef CONFIG_SEHARMONY_POLICY_SHARE
	avrule = (avrule_t *) sepol_malloc(sizeof(avrule_t));
#else
	...
	for (i = 0; i < len; i++) {
#ifdef CONFIG_SEHARMONY_POLICY_SHARE
		cur = (class_perm_node_t *) sepol_malloc(sizeof(class_perm_node_t));
#else
...
#ifdef CONFIG_SEHARMONY_POLICY_SHARE
		avrule->xperms = sepol_calloc(1, sizeof(*avrule->xperms));
#else
...
static int range_read(policydb_t * p, struct policy_file *fp)
{
	...
	nel = le32_to_cpu(buf[0]);
	for (i = 0; i < nel; i++) {
		rt = sepol_calloc(1, sizeof(range_trans_t));
		if (!rt)
			return -1;
		rc = next_entry(buf, fp, (sizeof(uint32_t) * 2));
		if (rc < 0)
...
#ifdef CONFIG_SEHARMONY_POLICY_SHARE
		rtr = sepol_malloc(sizeof(range_trans_rule_t));
#else
				...
				struct policy_file *fp)
{
	...
	ltr = NULL;
	for (i = 0; i < nel; i++) {
		tr = sepol_malloc(sizeof(role_trans_rule_t));
		if (!tr) {
			return -1;
		}
...
static int role_allow_rule_read(role_allow_rule_t ** r, struct policy_file *fp)
{
	...
	lra = NULL;
	for (i = 0; i < nel; i++) {
		ra = sepol_malloc(sizeof(role_allow_rule_t));
		if (!ra) {
			return -1;
		}
				 ...
				 struct policy_file *fp)
{
	...
	for (i = 0; i < nel; i++) {
#ifdef CONFIG_SEHARMONY_POLICY_SHARE
		rt = sepol_malloc(sizeof(range_trans_rule_t));
#else
			    ...
			    unsigned int num_scope_syms, struct policy_file *fp)
{
...
#ifdef CONFIG_HKIP_SEHARMONY_PROT
		scope_index->scope[i].protectable = true;
#endif
	...
	if ((scope_index->class_perms_map =
	     sepol_calloc(scope_index->class_perms_len,
...
#ifdef CONFIG_HKIP_SEHARMONY_PROT
		((ebitmap_t *)(scope_index->class_perms_map + i))->protectable = true;
#endif
			     ...
			     struct policy_file *fp)
{
	...
	while (num_blocks > 0) {
		...
		uint32_t num_decls;
#ifdef CONFIG_SEHARMONY_POLICY_SHARE
		if ((curblock = sepol_calloc(1, sizeof(*curblock))) == NULL) {
#else
...
#ifdef CONFIG_SEHARMONY_POLICY_SHARE
	if ((scope = sepol_calloc(1, sizeof(*scope))) == NULL) {
#else
...
#ifdef CONFIG_SEHARMONY_POLICY_SHARE
	if ((scope->decl_ids =
	     sepol_malloc(scope->decl_ids_len * sizeof(uint32_t))) == NULL) {
#else
	if ((scope->decl_ids =
...
static class_callback_fn_t class_callback_fn = NULL;

uint8_t *sepol_class_map = NULL;
static void class_map_init(size_t nprim)
{
	if (class_callback_fn != NULL) {
		sepol_class_map = calloc((nprim / BITS_PER_BYTE + 1), sizeof(uint8_t));
		if (sepol_class_map != NULL) {
			if (class_callback_fn(sepol_class_map, nprim) != SEPOL_OK) {
				free(sepol_class_map);
				sepol_class_map = NULL;
			}
		}
	}
}

static void class_map_fini(void)
{
	if (sepol_class_map != NULL) {
		free(sepol_class_map);
		sepol_class_map = NULL;
...
void class_callback_register(class_callback_fn_t fn)
{
	class_callback_fn = fn;
}

void class_callback_deregister(void)
{
	class_callback_fn = NULL;
}
...
int policydb_read(policydb_t * p, struct policy_file *fp, unsigned verbose)
{
	...
	if (p->policy_type == POLICY_MOD) {
		...
		if (zero_or_saturated(len))
			goto bad;
#ifdef CONFIG_SEHARMONY_POLICY_SHARE
		if ((p->name = sepol_malloc(len + 1)) == NULL) {
#else
		...
		if (zero_or_saturated(len))
			goto bad;
#ifdef CONFIG_SEHARMONY_POLICY_SHARE
		if ((p->version = sepol_malloc(len + 1)) == NULL) {
#else
	...
	if (policy_type == POLICY_KERN) {
#ifdef CONFIG_SEHARMONY_POLICY_SPLIT
		class_map_init(p->p_classes.nprim);
#endif
	...
	if (policy_type == POLICY_KERN) {
		p->type_attr_map = sepol_malloc(p->p_types.nprim * sizeof(ebitmap_t));
		p->attr_type_map = sepol_malloc(p->p_types.nprim * sizeof(ebitmap_t));
		if (!p->type_attr_map || !p->attr_type_map)
			goto bad;
		for (i = 0; i < p->p_types.nprim; i++) {
#ifdef CONFIG_HKIP_SEHARMONY_PROT
			p->type_attr_map[i].protectable = true;
			p->attr_type_map[i].protectable = true;
#endif
