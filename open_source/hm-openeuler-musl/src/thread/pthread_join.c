...
static int __pthread_timedjoin_np(pthread_t t, void **res, const struct timespec *at)
{
	...
	while ((state = t->detach_state) && r != ETIMEDOUT && r != EINVAL) {
		/**
		 * according to linux manpage, pthread_join() should return EINVAL
		 * instead of crash if the target thread is not a joinable thread.
		 */
		if (state >= DT_DETACHED) return EINVAL;
