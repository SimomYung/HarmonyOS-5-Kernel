...
#define MAPS_PATH_MAX_LEN 128
#define MAPS_PATH_PREFIX "/var/run/hpe/maps/"
#define MAPS_INFO_MAX_LEN 4096
	...
	if (strchr(name, '/')) {
		...
		if (fd == -1) {
			if (!sys_path) {
				...
				fd = open(etc_ldso_path, O_RDONLY);
				if (fd>=0) {
					size_t n = 0;
			...
			if (!sys_path || sys_path[0] == 0) sys_path = "/usr/local/hpe:/ramfs/lib:/usr/lib:/lib:/usr/local/lib";
...
void do_dump_maps_info(void)
{
    int pid = getpid();

    char path[MAPS_PATH_MAX_LEN] = {0};
    int ret = sprintf(path, MAPS_PATH_PREFIX"%d_maps", pid);
    if (ret < 0) {
        return;
    }

    char maps[MAPS_INFO_MAX_LEN] = {0};
    int pos = 0;
    for (struct dso *p = head; p != NULL && pos < MAPS_INFO_MAX_LEN; p = p->next) {
        ret = sprintf(maps + pos, "%s -o %p\n", p->name, p->base);
        if (ret < 0) {
            return;
        }
        pos += ret;
    ...
    int fd = open(path, O_CREAT | O_WRONLY | O_TRUNC | O_APPEND, 0640);
    if (fd < 0) {
        return;
    }

    (void)write(fd, maps, pos);
    close(fd);
}
