 ...
 * @brief       MAC algorithm interface, including HMAC and CMAC
 */
...
/**
 * @ingroup crypt_eal_mac
 * @brief  Callback registration interface for MAC Compliance checking, which is used to check whether algorithm
 * parameters are compliant and whether the module status is available.
 *     After the CMVP feature is enabled, the CMVP provides the default implementation, the EAL hash interface calls
 * this callback before performing an operation.
 *     If the callback fails, CRYPT_EAL_MacNewCtx/CRYPT_EAL_MacInit/CRYPT_EAL_MacUpdate, CRYPT_EAL_MacFinal,
 * and CRYPT_EAL_MacReinit interfaces fail.
 *     This verification does not take effect in non-approved mode. Only CRYPT_MAC_HMAC_SHA1/CRYPT_MAC_HMAC_SHA224/
 * CRYPT_MAC_HMAC_SHA256/CRYPT_MAC_HMAC_SHA384/CRYPT_MAC_HMAC_SHA512/CRYPT_MAC_CMAC_AES128/CRYPT_MAC_CMAC_AES192/
 * CRYPT_MAC_CMAC_AES256 is allowed in ISO19790 mode.
 *
 * @param   func     [IN] MAC Compliance checking callback
 *
 * @return  None
 */
 ...
 * @param   id [IN] MAC algorithm ID
 *
 * @return CRYPT_EAL_MacCtx Pointer.
...
/**
 * @ingroup crypt_eal_mac
 * @brief MAC algorithm initialize the context, which is used after the CRYPT_EAL_MacNewCtx interface is called.
 ...
 * Doesn't need call the init interface again for initialization, it is equivalent to the combination
 ...
 * @param   ctx [IN] MAC context
 * @return  The MAC length corresponding to the context.
 */
 ...
 * Currently, only two operation codes are supported: CRYPT_CTRL_SET_IV and CRYPT_CTRL_SET_TAGLEN.
