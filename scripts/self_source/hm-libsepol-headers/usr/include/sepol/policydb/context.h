...
static inline int mls_context_cpy_high(context_struct_t *dst, context_struct_t *src)
{
	...
	dst->range.level[0].cat.protectable = true;
#endif
...
#ifdef CONFIG_HKIP_SEHARMONY_PROT
	dst->range.level[1].cat.protectable = true;
#endif
...
static inline int mls_context_cpy_high_nohkip(context_struct_t *dst, context_struct_t *src)
{
	int rc;
