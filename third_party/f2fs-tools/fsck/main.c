...
extern void update_currcp_crc(struct f2fs_sb_info *sbi);
...
void fsck_usage()
{
	...
	MSG(0, "  --convert=f2fs/hmfs convert f2fs between hmfs\n");
	exit(1);
}
	...
	if (!strcmp("fsck.f2fs", prog)) {
		...
		while ((option = getopt_long(argc, argv, option_string,
			...
			case 4:
				...
				break;
			case 5:
				c.convert = parse_convert_config(optarg);
				break;
			case 'a':
				c.auto_fix = 1;
...
static int do_fsck(struct f2fs_sb_info *sbi)
{
	...
	if (c.fs_magic != sbi->raw_super->magic) {
		c.fs_magic = sbi->raw_super->magic;
		update_currcp_crc(sbi);
	}
