...
extern void hmfs_enable_large_nat_bitmap(struct f2fs_sb_info *sbi, unsigned long long total_size,
			...
			struct f2fs_super_block *new_sb, unsigned int offset)
{
	...
	*((__le32 *)((unsigned char *)new_cp + get_cp(checksum_offset))) =
	...
	crc = f2fs_cal_crc32(c.fs_magic, cp, CP_CHKSUM_OFFSET);
...
static int f2fs_resize_grow(struct f2fs_sb_info *sbi)
{
    ...
    if (get_newsb(main_blkaddr) < get_sb(main_blkaddr)) {
        MSG(0, "Info: No Need resize grow, new_main_blkaddr:[%u], old_main_blkaddr:[%u]\n",
            get_newsb(main_blkaddr), get_sb(main_blkaddr));
        return 0;
    }
...
int f2fs_resize(struct f2fs_sb_info *sbi)
{
	...
	if ((c.target_sectors * c.sector_size >> get_sb(log_blocksize)) < get_sb(block_count))
		if (!c.safe_resize) {
			...
			hmfs_enable_large_nat_bitmap(sbi, target_size, cur_size);
