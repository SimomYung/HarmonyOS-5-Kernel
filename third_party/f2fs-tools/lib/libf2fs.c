...
{
	uint32_t cal_crc = 0;
	// only check crc, do not care magic
	struct f2fs_super_block *sb = (struct f2fs_super_block *)buf;
	cal_crc = f2fs_cal_crc32(sb->magic, buf, len);
...
__u32 f2fs_checkpoint_chksum(struct f2fs_checkpoint *cp)
{
	...
	chksum = f2fs_cal_crc32(c.fs_magic, cp, chksum_ofs);
