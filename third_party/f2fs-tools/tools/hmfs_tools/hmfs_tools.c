...
bool is_valid_hmfs_magic(unsigned long hmfs_magic)
{
	if (hmfs_magic != HMFS_SUPER_MAGIC) {
		MSG(0, "HMFS Magic Mismatch, valid(0x%x) - read(0x%lx)\n",
...
void hmfs_enable_large_nat_bitmap(struct f2fs_sb_info *sbi, unsigned long long total_size, unsigned long long cur_size)
{
