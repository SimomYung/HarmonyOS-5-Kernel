...
#if ENABLE_FEATURE_ZCAT_PRODUCE_ZOMBIE_PROCS
static void handle_zcat_sigchld(int sig)
{
	pid_t pid;
...
#if ENABLE_FEATURE_ZCAT_PRODUCE_ZOMBIE_PROCS
	if ((option_mask32 & BBUNPK_SEAMLESS_MAGIC) != 0)
		signal(SIGCHLD, handle_zcat_sigchld);
#endif
 ...
 err:
		...
		if (!(option_mask32 & BBUNPK_SEAMLESS_MAGIC)) {
			...
			if (status < 0) {
				...
				/* Use the command "gzip -cdf" in cracklib-format.
				 * If the input data is not in a format recognized by gzip,
				 * it will report an error while fully functional gzip will
				 * directly output the input data.
			 	 * Copy the input data without change to the standard output:
				 * The if statement ensures that the input data is directly output
				 * when executing "gzip -cdf" to process uncompressed data */
				if ((option_mask32 & (BBUNPK_OPT_FORCE | BBUNPK_OPT_STDOUT | BBUNPK_OPT_DECOMPRESS))
				&& !expected_ext) {
...
//config:
//config:config FEATURE_ZCAT_PRODUCE_ZOMBIE_PROCS
//config:	bool "solve zcat will produce zombie process problem"
//config:	default n
//config:	depends on ZCAT
//config:	help
//config:	When using zcat * or zcat multiple compressed packages, because
//config:	the parent process does not reclaim the child process resources,
//config:	the child process will generate zombie processes during the
//config:	execution of zcat.
