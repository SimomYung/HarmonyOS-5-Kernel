...
//config:
//config: config FEATURE_TAR_RESTORE_PERMS
//config:	bool "Enhance restoring ownerships and permissions and mtime"
//config:	default n
//config:	depends on TAR && FEATURE_TAR_LONG_OPTIONS
//config:	help
//config:	With this option busybox supports restoring
//config:	ownerships(*default for root*),
//config:	permissions(default, apply umask for both files and *links*) and
//config:	mtime(for both files and *links*) when FEATURE_TAR_NOPRESERVE_TIME
//config:	is on when extracting files.
//config:
//config: config FEATURE_HM_TAR_XATTR_SEC
//config:	bool "Enhance restoreing security xattrs"
//config:	default n
//config:	depends on TAR && FEATURE_TAR_LONG_OPTIONS
//config:	help
//config:	With this option busybox supports restoring
//config:	security xattrs(eg: security.hmmac) when extracting tar files.
//config:
//config: config FEATURE_TAR_IGNORE_FAILED_READ
//config:	bool "Enable -I option to ignore damaged files when tar files."
//config:	default n
//config:	depends on TAR
//config:	help
//config:	with this option busybox supports getting the tar files
//config:	even though there are damaged target in filelist.
//config:
...
#if ENABLE_FEATURE_TAR_IGNORE_FAILED_READ
#define IGNORE_FAILED_READ 1
/*
 * After the tar command is invoked, the system exits,
 * which does not affect subsequent tar commands.
 */
static int g_tar_ignore_failed_read = 0;
#endif
...
#if ENABLE_FEATURE_TAR_IGNORE_FAILED_READ
static int ignoreDamgedFileToTarball(struct TarBallInfo *tbInfo,
        struct stat *statbuf, const char *fileName,
		int fileFd, off_t off_old)
{
	off_t off_new = 0;
	if (g_tar_ignore_failed_read == IGNORE_FAILED_READ) {
		off_t retSize;
		retSize = bb_copyfd_size(fileFd, tbInfo->tarFd, statbuf->st_size);
		if (retSize != statbuf->st_size) {
			off_new = lseek(tbInfo->tarFd, 0x0, SEEK_CUR);
			(void)lseek(tbInfo->tarFd, (off_old - off_new), SEEK_END);
			printf("tar warning: \"%s\" skiped!\n", fileName);
			return SKIP;
		}
	} else {
		bb_copyfd_exact_size(fileFd, tbInfo->tarFd, statbuf->st_size);
	}
	return TRUE;
}
#endif
...
# if ENABLE_FEATURE_TAR_IGNORE_FAILED_READ
	/* Record the file pointer in the tarball before a new file is added. */
	off_t off_old = 0;
	if (g_tar_ignore_failed_read == IGNORE_FAILED_READ) {
		off_old = lseek(tbInfo->tarFd, 0x0, SEEK_CUR);
	}
# endif
...
# if ENABLE_FEATURE_TAR_IGNORE_FAILED_READ
		/* When the Ignore Bad Files option is turned on,
		 * if a file is read corrupt, the file is not read into the tarball.
		 */
		if (ignoreDamgedFileToTarball(tbInfo, statbuf, fileName, inputFileFd, off_old) == SKIP) {
			return SKIP;
		}
# else
...
//usage:	IF_FEATURE_TAR_RESTORE_PERMS("p")
//usage:	"vokO] "
...
//usage:	IF_FEATURE_TAR_RESTORE_PERMS(
//usage:     "\n	-p	Restore file permissions(except links, default for root)"
//usage:	)
...
//usage:	)
//usage:	)
//usage:	IF_FEATURE_HM_TAR_XATTR_SEC(
...
//usage:	IF_NOT_FEATURE_TAR_RESTORE_PERMS(
...
#if !ENABLE_FEATURE_TAR_RESTORE_PERMS
	OPTBIT_NOPRESERVE_PERM,
#endif
	OPTBIT_OVERWRITE,
#if ENABLE_FEATURE_TAR_IGNORE_FAILED_READ
	OPTBIT_IGNORE_FAILED_READ,
#endif
#endif
#if ENABLE_FEATURE_HM_TAR_XATTR_SEC
	IF_FEATURE_TAR_FROM(OPTBIT_EXCLUDE,                  )
	IF_FEATURE_HM_TAR_XATTR_SEC(OPTBIT_XATTRS,         )
	IF_FEATURE_HM_TAR_XATTR_SEC(OPTBIT_XATTRS_INCLUDE, )
#endif
...
#if ENABLE_FEATURE_HM_TAR_XATTR_SEC
	OPT_EXCLUDE          = IF_FEATURE_TAR_FROM((1 << OPTBIT_EXCLUDE                )) + 0, // exclude
	OPT_XATTRS = IF_FEATURE_HM_TAR_XATTR_SEC((1 << OPTBIT_XATTRS                 )) + 0, // xattrs
	OPT_XATTRS_INCLUDE = IF_FEATURE_HM_TAR_XATTR_SEC((OPTBIT_XATTRS_INCLUDE      )) + 0, // xattrs-include
#endif
...
# endif
# if ENABLE_FEATURE_TAR_IGNORE_FAILED_READ
	"ignore-failed-read\0"	No_argument       "I"
# endif
#if ENABLE_FEATURE_HM_TAR_XATTR_SEC
	"xattrs\0"              No_argument       "\xe0"
	"xattrs-include\0"      Required_argument "\xe1"
#endif
	;
...
int tar_main(int argc UNUSED_PARAM, char **argv)
{
	...
	opt = GETOPT32(argv, "^"
...
#if ENABLE_FEATURE_TAR_IGNORE_FAILED_READ
		IF_FEATURE_TAR_CREATE("I") // ignore-failed-read
#endif
		"\0"
		"tt:vv:" // count -t,-v
#if ENABLE_FEATURE_TAR_LONG_OPTIONS && ENABLE_FEATURE_TAR_FROM
...
#endif
		IF_FEATURE_HM_TAR_XATTR_SEC("\xe1::") // --xattrs-include mask
		...
		, &excludes // --exclude
#endif
		IF_FEATURE_HM_TAR_XATTR_SEC(, &tar_handle->tar__xattrs_include) // --xattrs-include MASK
...
# undef showopt
	...
	if (opt & OPT_NOPRESERVE_PERM)
		tar_handle->ah_flags |= ARCHIVE_DONT_RESTORE_PERM;
#else
	/* If not root and not with -p option */
	if ((getuid() != 0) && !(opt & OPT_P))
		tar_handle->ah_flags |= ARCHIVE_DONT_RESTORE_PERM;
#endif
...
# if ENABLE_FEATURE_TAR_IGNORE_FAILED_READ
	if (opt & OPTBIT_IGNORE_FAILED_READ)
		g_tar_ignore_failed_read = 1;
# endif
#if ENABLE_FEATURE_TAR_FROM
