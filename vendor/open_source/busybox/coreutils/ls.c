...
static NOINLINE unsigned display_single(const struct dnode *dn)
{
	...
	if (opt & OPT_l)
		if (S_ISLNK(dn->dn_mode))
			...
			lpath = malloc_readlink_or_warn(dn->fullname);
#else
...
#if ENABLE_FEATURE_NOFORK_LS
		void *ptr = malloc(sizeof(*cur));
		if (ptr == NULL) {
			bb_simple_perror_msg("malloc in ls fails because of exhausted memory\n");
			G.exit_code = EXIT_FAILURE;
			return NULL;
		}
		memset(ptr, 0, sizeof(*cur));
		cur = ptr;
#else
...
#if ENABLE_FEATURE_NOFORK_LS
		void *ptr = malloc(num * sizeof(struct dnode *));
		if (ptr == NULL) {
			bb_simple_perror_msg("malloc in ls fails because of exhausted memory\n");
			return NULL;
		}
		memset(ptr, 0, num * sizeof(struct dnode *));
		return ptr;
#else	
...
#if ENABLE_FEATURE_NOFORK_LS
		fullname = concat_path_file_nofork(path, entry->d_name);
		if (fullname == NULL) {
			closedir(dir);
			return NULL;
		}
