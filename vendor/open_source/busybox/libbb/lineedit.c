...
#if ENABLE_FEATURE_GETIP_FROM_UTMP
#include <utmp.h>
#else
#include <utmpx.h>
#endif
...
# if ENABLE_FEATURE_BUILDIN_CMD_MATCH
	...
	if ((type == FIND_EXE_ONLY) && (strchr(command, '/') == NULL)) {
		int buildin_cmd_num;
		char **buildin_cmd = get_builtin_command_name_list(&buildin_cmd_num);
		if (buildin_cmd != NULL) {
			for (int idx = 0; idx < buildin_cmd_num; idx++) {
				if (strncmp(basecmd, buildin_cmd[idx], baselen) == 0) {
					add_match(xstrdup(buildin_cmd[idx]));
				}
			}
			free(buildin_cmd);
		}
...
#if ENABLE_FEATURE_SYSLOG_CMD_LOG
uint8_t exitstatus_addr;
bool show_ip = true;
#define STDIN_PATH "/proc/self/fd/0"
#define HISTLEN (ENABLE_FEATURE_EDITING ? CONFIG_FEATURE_EDITING_MAX_LEN : 1024) /* Keep the same with IBUFSIZE */
static char historycmd[HISTLEN];

#define DEV_DIR_WITH_TRAILING_SLASH "/dev/"
#define DEV_DIR_LEN (sizeof(DEV_DIR_WITH_TRAILING_SLASH) - 1)

static bool can_get_user_ip(struct utmpx **ut)
{
	char *tty_name = NULL;
	bool flag = false;
	...
	if (strncmp(tty_name, DEV_DIR_WITH_TRAILING_SLASH, DEV_DIR_LEN) == 0)
		tty_name += DEV_DIR_LEN;

	setutxent();
	while ((*ut = getutxent()) != NULL) {
		if ((strcmp(tty_name, (*ut)->ut_line) == 0) &&
			(strlen((*ut)->ut_host) > 0)) {
			flag = true;
			break;
		}
...
static void cmd_log(int retval)
{
	char buf[HISTLEN] = {0};
	ssize_t ret;
	...
	ret = readlink(STDIN_PATH, buf, (HISTLEN - 1));
	if (ret == -1)
		strncpy(buf, "NULLTTY", strlen("NULLTTY"));

	openlog("cmd_log", LOG_PID, LOG_USER);

	syslog(LOG_INFO, "%s:%s:%u:%s:%s:%s:%d:%s (%s)",
		getlogin(),"audit_current_user", geteuid(), "history_command",
		historycmd, "return_value", retval, buf,
		show_ip == true ? (can_get_user_ip(&ut) ?
		ut->ut_host : "NULL") : "NULL");

	closelog();
	memset(historycmd, 0, HISTLEN);
}
#endif
...
#if ENABLE_FEATURE_SYSLOG_CMD_LOG
	/* Copy command line to global historycmd buffer */
	strncpy(historycmd, command, HISTLEN - 1);
	historycmd[HISTLEN - 1] = '\0';
#endif
	INIT_S();
