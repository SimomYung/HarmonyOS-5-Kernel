...
//config:config FEATURE_IFCONFIG_INTER_STATISTICS
//config:	bool "Enable option \"-S\""
//config:	default n
//config:	depends on IFCONFIG
//config:	help
//config:	The -S parameter is provided to query the specified network device
//config:	for NIC and driver-specific statistics

//applet:IF_IFCONFIG(APPLET(ifconfig, BB_DIR_SBIN, BB_SUID_DROP))

//kbuild:lib-$(CONFIG_IFCONFIG) += ifconfig.o interface.o
//kbuild:lib-$(CONFIG_FEATURE_IFCONFIG_INTER_STATISTICS) += query_interface_statistics.o

//usage:#define ifconfig_trivial_usage
//usage:	IF_FEATURE_IFCONFIG_STATUS("[-a]") IF_FEATURE_IFCONFIG_INTER_STATISTICS("[-S]") " [IFACE] [ADDRESS]"
...
//usage:       "	[up|down] ...\n"
//usage:	IF_FEATURE_IFCONFIG_INTER_STATISTICS(
//usage:     "\n"
//usage:       "Query statistics of network interface\n"
//usage:     "\n"
//usage:       "	[-S IFACE]")
...
int ifconfig_main(int argc UNUSED_PARAM, char **argv)
{
	...
	IF_FEATURE_IFCONFIG_INTER_STATISTICS(char *query_ifname);
	...
	if (argv[0] && argv[0][0] == '-' && argv[0][1] == 'S' && !argv[0][2]) {
		if (!argv[1])
			bb_simple_error_msg_and_die("please specify one interface");
		/* get ifname */
		query_ifname = argv[1];
		if (strlen(query_ifname) >= IFNAMSIZ)
			bb_simple_error_msg_and_die("Device name is too long");
 
		/* query interface's statistics */
		return query_interface_statistics(query_ifname);
	}
#endif
