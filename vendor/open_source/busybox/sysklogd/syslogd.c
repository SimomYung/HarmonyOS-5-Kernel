...
//config:
//config:config FEATURE_SYSLOG_CMD_LOG
//config:	bool "Turn On CMD LOG"
//config:	default n
//config:	depends on SYSLOGD || SHELL_ASH
//config:	help
//config:	Turn On record CMD LOG
//config:
//config:config FEATURE_GETIP_FROM_UTMP
//config:	bool "get ip from utmp instead of utmpx"
//config:	default n
//config:	depends on FEATURE_SYSLOG_CMD_LOG
//config:	help
//config:	With this option production, using UCLIBC, can get IP
//config:	from utmp instead of utmpx when record the commandline log
//config:
...
//config:
//config:config FEATURE_SYSLOGD_YEAR_IN_TIMESTAMPS
//config:	bool "Include year in timestamps"
//config:	default n
//config:	depends on SYSLOGD
//config:	help
//config:	Includes year in timestamp when timestamps are added.
...
#if ENABLE_FEATURE_SYSLOGD_YEAR_IN_TIMESTAMPS
	time(&now);
	char *cur_year = ctime(&now) + 20; /* year */
	cur_year[4] = '\0';
#endif
...
#if ENABLE_FEATURE_SYSLOGD_YEAR_IN_TIMESTAMPS
		sprintf(G.printbuf, "%s %s %d %s\n", timestamp, cur_year, getuid(), msg);
#else
		sprintf(G.printbuf, "%s %d %s\n", timestamp, getuid(), msg);
#endif
#else
#if ENABLE_FEATURE_SYSLOGD_YEAR_IN_TIMESTAMPS
		sprintf(G.printbuf, "%s %s %s\n", timestamp, cur_year, msg);
#else
...
#if ENABLE_FEATURE_HM_SEC_USERMGT
#if ENABLE_FEATURE_SYSLOGD_YEAR_IN_TIMESTAMPS
		sprintf(G.printbuf, "%s %s %d %.64s %s %s\n", timestamp, cur_year, getuid(), G.hostname, res, msg);
#else
		sprintf(G.printbuf, "%s %d %.64s %s %s\n", timestamp, getuid(), G.hostname, res, msg);
#endif
#else
#if ENABLE_FEATURE_SYSLOGD_YEAR_IN_TIMESTAMPS
		sprintf(G.printbuf, "%s %s %.64s %s %s\n", timestamp, cur_year, G.hostname, res, msg);
#else
