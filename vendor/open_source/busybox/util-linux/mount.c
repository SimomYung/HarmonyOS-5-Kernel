...
//config:
//config:config FEATURE_REMOVE_MOUNT_OPTION_NOFAIL
//config:	depends on MOUNT
//config:	bool "Do not send nofail option to mount system call function"
//config:	default n
//config:	help
//config:	With this option, busybox will remove nofail option
//config:	before passing it to the mount system call function
...
#if ENABLE_FEATURE_REMOVE_MOUNT_OPTION_NOFAIL
// options format is: a,b,c
static void remove_option(char *options, const char *opt)
{
	if (options == NULL || opt == NULL)
		return;

	char *opt_str = strstr(options, opt);
	if (opt_str == NULL)
		return;

	size_t opt_len = strlen(opt);
	size_t pos = opt_str - options;    // opt position
	if (*(opt_str + opt_len) == ',') { // a,b,c remove a or b
		char *remain = opt_str + opt_len + 1;
		size_t remain_len = strlen(remain);
		strcpy(options + pos, remain);
		pos += remain_len;
		options[pos] = '\0';
	} else {
		// a remove a
...
#if ENABLE_FEATURE_REMOVE_MOUNT_OPTION_NOFAIL
	if (unrecognized)
		remove_option(*unrecognized, "nofail");
#endif
