...
static bool exfat_node_detached(struct exfat_node* node)
{
	return node->parent == NULL && node->child == NULL &&
	       node->prev == NULL && node->next == NULL;
}

void exfat_put_free_node(struct exfat* ef, struct exfat_node* node)
{
	char buffer[EXFAT_UTF8_NAME_BUFFER_MAX] = {0};
	...
	if (node->references < 0) {
		...
		if (node->is_unlinked) {
			if (!exfat_node_detached(node)) {
				exfat_get_name(node, buffer);
				exfat_bug("unlinked '%s' is not detached from tree", buffer);
			}
			free(node);
		}
	}
