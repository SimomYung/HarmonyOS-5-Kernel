...
LIBMTP_mtpdevice_t *LIBMTP_Open_Raw_Device_Uncached(LIBMTP_raw_device_t *rawdevice)
{
  ...
  if (ptp_getdeviceinfo(current_params,
    ...
    PTPPropertyValue propval;
    char *str = "OpenHarmony/1.1.21 MTPClassDriver/1.1.21";
    propval.str = str;
    if (ptp_setdevicepropvalue(current_params, PTP_DPC_MTP_SessionInitiatorInfo, &propval, PTP_DTC_STR) != PTP_RC_OK) {
        LIBMTP_ERROR("LIBMTP WARNING: Unable to set device prop value");
    }
...
#define MAX_GET_HANDLES_RETRIES		3
#define RETRY_INTERVAL		700000
...
LIBMTP_object_handles_t *LIBMTP_Get_Object_Handles(LIBMTP_mtpdevice_t *device, uint32_t const storage,
  uint32_t const parent)
{
  if (!device || device->cached) {
    LIBMTP_ERROR("Invalid or cached device");
    return NULL;
  }

  PTPParams *params = (PTPParams *) device->params;
  PTPObjectHandles currentHandles;
  LIBMTP_object_handles_t *retHandles = NULL;
  uint32_t storageid;
  uint16_t ret;
  int attempts = 0;
  ...
  } while (attempts < MAX_GET_HANDLES_RETRIES);

  if (ret != PTP_RC_OK) {
    LIBMTP_ERROR("Get object handles failed");
    return NULL;
  }
  ...
  retHandles = (LIBMTP_object_handles_t *) malloc(sizeof(LIBMTP_object_handles_t));
  if (retHandles == NULL) {
    free(currentHandles.Handler);
    return NULL;
  }

  retHandles->num = currentHandles.n;
  retHandles->handler = (uint32_t*) malloc(retHandles->num * sizeof(uint32_t));
  if (retHandles->handler == NULL) {
    free(retHandles);
    free(currentHandles.Handler);
    return NULL;
  }
  memcpy_s(retHandles->handler, retHandles->num * sizeof(uint32_t),
           currentHandles.Handler, currentHandles.n * sizeof(uint32_t));
  free(currentHandles.Handler);
                                                ...
                                                uint32_t count)
{
  ...
  if (!device || !handles || !handles->handler || handles->offset >= handles->num) {
    LIBMTP_ERROR("Invalid parameters");
    return NULL;
  }

  uint32_t endNum = (handles->num < handles->offset + count) ? handles->num : handles->offset + count;
  for (i = handles->offset; i < endNum; i++) {
    LIBMTP_file_t *file = LIBMTP_Get_Filemetadata(device, handles->handler[i]);
    if (file == NULL) {
      continue;
    }
    // Add track to a list that will be returned afterwards.
			 ...
			 void const * const data)
{
  ...
  LIBMTP_file_t *file_metadata = LIBMTP_Get_Filemetadata(device, id);
  if (file_metadata != NULL) {
      unsigned long expected_size = file_metadata->filesize;
      ptp_set_block_size(expected_size);
      LIBMTP_destroy_file_t(file_metadata);
  } else {
      ptp_set_block_size(0);
  }
			       ...
			       void const * const data)
{
  ...
  if (fstat(fd, &file_stat) == -1) {
      add_error_to_errorstack(device, LIBMTP_ERROR_GENERAL, "Failed to get file size");
      close(fd);
      return -1;
  }
  unsigned long file_size = file_stat.st_size;
  ptp_set_block_size(file_size);
