...
#define LIBMTP_FILETYPE_IS_AUDIO(a)                                                                                   \
    ((a) == LIBMTP_FILETYPE_WAV || (a) == LIBMTP_FILETYPE_MP3 || (a) == LIBMTP_FILETYPE_MP2 || \
     (a) == LIBMTP_FILETYPE_WMA || (a) == LIBMTP_FILETYPE_OGG || (a) == LIBMTP_FILETYPE_FLAC || \
     (a) == LIBMTP_FILETYPE_AAC || (a) == LIBMTP_FILETYPE_M4A || (a) == LIBMTP_FILETYPE_AUDIBLE || \
...
#define LIBMTP_FILETYPE_IS_VIDEO(a)                                                                                   \
    ((a) == LIBMTP_FILETYPE_WMV || (a) == LIBMTP_FILETYPE_AVI || (a) == LIBMTP_FILETYPE_MPEG || \
...
#define LIBMTP_FILETYPE_IS_IMAGE(a)                                                                                   \
    ((a) == LIBMTP_FILETYPE_JPEG || (a) == LIBMTP_FILETYPE_JFIF || (a) == LIBMTP_FILETYPE_TIFF || \
     (a) == LIBMTP_FILETYPE_BMP || (a) == LIBMTP_FILETYPE_GIF || (a) == LIBMTP_FILETYPE_PICT || \
     (a) == LIBMTP_FILETYPE_PNG || (a) == LIBMTP_FILETYPE_JP2 || (a) == LIBMTP_FILETYPE_JPX || \
 ...
 */
#define LIBMTP_FILETYPE_IS_ADDRESSBOOK(a) ((a) == LIBMTP_FILETYPE_VCARD2 || (a) == LIBMTP_FILETYPE_VCARD3)
 
/**
 *  \def LIBMTP_FILETYPE_IS_CALENDAR
 *  Calendar and Appointment filetype test
 */
#define LIBMTP_FILETYPE_IS_CALENDAR(a) ((a) == LIBMTP_FILETYPE_VCALENDAR1 || (a) == LIBMTP_FILETYPE_VCALENDAR2)
...
typedef struct LIBMTP_object_handles_struct LIBMTP_object_handles_t;     /**< @see LIBMTP_object_handles_t */
...
struct LIBMTP_object_handles_struct {
	uint32_t num;
    uint32_t offset;
	uint32_t *handler;
};
...
int LIBMTP_check_is_mtp_device(uint8_t deviceClass, uint16_t idVendor, uint16_t idProduct);
...
int LIBMTP_Get_Allowed_Property_Values(LIBMTP_mtpdevice_t *, LIBMTP_property_t const, LIBMTP_filetype_t const,
...
LIBMTP_file_t *LIBMTP_Get_Filelisting_With_Callback(LIBMTP_mtpdevice_t *, LIBMTP_progressfunc_t const,
                                                    void const *const);
...
LIBMTP_object_handles_t *LIBMTP_Get_Object_Handles(LIBMTP_mtpdevice_t *device, uint32_t const storage,
                                                   uint32_t const parent);
LIBMTP_file_t *LIBMTP_Get_Patial_Files_Metadata(LIBMTP_mtpdevice_t *device, LIBMTP_object_handles_t *handles,
                                                uint32_t count);
...
int LIBMTP_Get_File_To_File(LIBMTP_mtpdevice_t *, uint32_t, char const *const, LIBMTP_progressfunc_t const,
                            void const *const);
int LIBMTP_Get_File_To_File_Descriptor(LIBMTP_mtpdevice_t *, uint32_t const, int const, LIBMTP_progressfunc_t const,
                                       void const *const);
...
int LIBMTP_Send_File_From_File(LIBMTP_mtpdevice_t *, char const *const, LIBMTP_file_t *const,
                               LIBMTP_progressfunc_t const, void const *const);
int LIBMTP_Send_File_From_File_Descriptor(LIBMTP_mtpdevice_t *, int const, LIBMTP_file_t *const,
                                          LIBMTP_progressfunc_t const, void const *const);
int LIBMTP_Send_File_From_Handler(LIBMTP_mtpdevice_t *, MTPDataGetFunc, void *, LIBMTP_file_t *const,
                                  LIBMTP_progressfunc_t const, void const *const);
...
LIBMTP_track_t *LIBMTP_Get_Tracklisting_With_Callback(LIBMTP_mtpdevice_t *, LIBMTP_progressfunc_t const,
                                                      void const *const);
...
int LIBMTP_Get_Track_To_File(LIBMTP_mtpdevice_t *, uint32_t, char const *const, LIBMTP_progressfunc_t const,
                             void const *const);
int LIBMTP_Get_Track_To_File_Descriptor(LIBMTP_mtpdevice_t *, uint32_t const, int const, LIBMTP_progressfunc_t const,
                                        void const *const);
int LIBMTP_Get_Track_To_Handler(LIBMTP_mtpdevice_t *, uint32_t const, MTPDataPutFunc, void *,
                                LIBMTP_progressfunc_t const, void const *const);
int LIBMTP_Send_Track_From_File(LIBMTP_mtpdevice_t *, char const *const, LIBMTP_track_t *const,
                                LIBMTP_progressfunc_t const, void const *const);
int LIBMTP_Send_Track_From_File_Descriptor(LIBMTP_mtpdevice_t *, int const, LIBMTP_track_t *const,
                                           LIBMTP_progressfunc_t const, void const *const);
int LIBMTP_Send_Track_From_Handler(LIBMTP_mtpdevice_t *, MTPDataGetFunc, void *, LIBMTP_track_t *const,
                                   LIBMTP_progressfunc_t const, void const *const);
