...
static int probe_device_descriptor(libusb_device *dev, FILE *dumpfile)
{
   ...
   */
  if (desc.bDeviceClass == LIBUSB_CLASS_VENDOR_SPEC || desc.bDeviceClass == LIBUSB_CLASS_PER_INTERFACE ||
    ...
    if (!(buf[0x12] == 'M' && buf[0x13] == 'T' && buf[0x14] == 'P') && 
     !(buf[0x12] == 'P' && buf[0x13] == 'T' && buf[0x14] == 'P') ) {
      libusb_close(devh);
      return 0;
    }
			      ...
			      int * numdevs)
{
  ...
  while (dev != NULL) {
    ...
    if (!device_known) {
      ...
      char *vendorName = "MTP Device";
      char *productName = "MTP device (ID1)";
      retdevs[i].device_entry.vendor = vendorName;
      retdevs[i].device_entry.product = productName;
    }
...
extern unsigned long context_block_size_1 = 0x1e00;
extern unsigned long context_block_size_2 = 0x200;
extern unsigned long context_block_size = 0x2000;

void ptp_set_block_size(unsigned long file_size) {
    if (file_size > 200 * 1024 * 1024) {
        context_block_size_1 = 0x80000;
        context_block_size_2 = 0x200;
    } else {
        context_block_size_1 = 0x1e00;
        context_block_size_2 = 0x200;
    }
    context_block_size = context_block_size_1 + context_block_size_2;
}
  ...
  while (curread < size) {
    ...
    do {
          ...
          if (ret == LIBUSB_ERROR_TIMEOUT) {
              LIBMTP_ERROR("SXQ ptp_read_func timeout");
              libusb_clear_halt(ptp_usb->handle, ptp_usb->inep);
          }
        } while (ret == LIBUSB_ERROR_TIMEOUT && ++retry_count < MAX_RETRIES);
 ...
 *
 */
static short ptp_read_cancel_func(
    PTPParams* params,
    uint32_t transactionid
) {
...
{
    if (!(deviceClass == LIBUSB_CLASS_PER_INTERFACE || deviceClass == LIBUSB_CLASS_COMM ||
          deviceClass == LIBUSB_CLASS_PTP || deviceClass == 0xEF || deviceClass == LIBUSB_CLASS_VENDOR_SPEC)) {
        return 0;
    }
    int i;
    for (i = 0; i < mtp_device_table_size; i++) {
        if (idVendor == mtp_device_table[i].vendor_id && idProduct == mtp_device_table[i].product_id) {
            return 1;
        }
    }
    return 0;
