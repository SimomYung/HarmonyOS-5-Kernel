		  ...
		  avtab_datum_t * datum)
{
	...
	newnode = (avtab_ptr_t) sepol_malloc(sizeof(struct avtab_node));
	if (newnode == NULL)
		return NULL;
	...
	if (key->specified & AVTAB_XPERMS) {
		xperms = sepol_calloc(1, sizeof(avtab_extended_perms_t));
		if (xperms == NULL) {
			sepol_free(newnode);
			return NULL;
		}
...
#ifdef CONFIG_SEHARMONY_POLICY_SPLIT
	if (!sepol_class_map_test_bit(sepol_class_map, key->target_class))
		return SEPOL_OK;
#endif
...
int avtab_alloc(avtab_t *h, uint32_t nrules)
{
	...
	h->htable = sepol_calloc(nslot, sizeof(avtab_ptr_t));
	if (!h->htable)
		return -1;
out:
	h->nel = 0;
