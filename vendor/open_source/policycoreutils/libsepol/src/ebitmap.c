...
extern uint32_t is_register_cb;

static inline ebitmap_node_t *try_alloc(uint32_t protectable)
{
	if(protectable && is_register_cb)
		return (ebitmap_node_t *) sepol_malloc(sizeof(ebitmap_node_t));
	else
		return (ebitmap_node_t *) malloc(sizeof(ebitmap_node_t));
}

static inline void try_free(uint32_t protectable, ebitmap_node_t *n)
{
	if (!protectable || !is_register_cb)
		free(n);
}
...
void ebitmap_destroy_nohkip(ebitmap_t * e)
{
	ebitmap_node_t *n, *temp;
