...
static inline int mls_range_set(context_struct_t * context, mls_range_t * range)
{
	...
	for (l = 0; l < 2; l++) {
		...
		context->range.level[l].cat.protectable = true;
#endif
...
static inline int mls_range_set_nohkip(context_struct_t * context, mls_range_t * range)
{
	int l, rc = 0;
...
int mls_compute_sid_nohkip(const policydb_t * policydb,
		    context_struct_t * scontext,
		    context_struct_t * tcontext,
		    sepol_security_class_t tclass,
		    uint32_t specified, context_struct_t * newcontext)
{
	...
	case AVTAB_TRANSITION:
		...
		r = hashtab_search(policydb->range_tr, (hashtab_key_t) &rtr);
		if (r)
			return mls_range_set_nohkip(newcontext, r);
		...
		case DEFAULT_SOURCE_LOW:
			return mls_context_cpy_low(newcontext, scontext);
		case DEFAULT_SOURCE_HIGH:
			return mls_context_cpy_high_nohkip(newcontext, scontext);
		case DEFAULT_SOURCE_LOW_HIGH:
			return mls_context_cpy(newcontext, scontext);
		case DEFAULT_TARGET_LOW:
			return mls_context_cpy_low(newcontext, tcontext);
		case DEFAULT_TARGET_HIGH:
			return mls_context_cpy_high_nohkip(newcontext, tcontext);
		case DEFAULT_TARGET_LOW_HIGH:
			return mls_context_cpy(newcontext, tcontext);
		case DEFAULT_GLBLUB:
			return mls_context_glblub(newcontext, scontext, tcontext);
