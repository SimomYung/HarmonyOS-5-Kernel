...
void sepol_compute_xperms_drivers(struct sepol_xperms *xperms, avtab_ptr_t node)
{
	unsigned int i;

	if (node->datum.xperms->specified == AVTAB_XPERMS_IOCTLDRIVER) {
		for (i = 0; i < ARRAY_SIZE(xperms->drivers.p); i++)
			xperms->drivers.p[i] |= node->datum.xperms->perms[i];
	} else if (node->datum.xperms->specified == AVTAB_XPERMS_IOCTLFUNCTION) {
		xperm_set(node->datum.xperms->driver, xperms->drivers.p);
	}
...
int context_compute_av_norbuf(context_struct_t * scontext,
			      context_struct_t * tcontext,
			      sepol_security_class_t tclass,
			      sepol_access_vector_t requested,
			      struct sepol_av_decision *avd,
			      unsigned int *reason,
			      struct sepol_xperms *xperms)
{
	constraint_node_t *constraint;
		...
		ebitmap_for_each_positive_bit(tattr, tnode, j) {
			...
			for (node =
				...
				if (node->key.specified == AVTAB_ALLOWED)
					...
					sepol_compute_xperms_drivers(xperms, node);
			}
...
int
services_compute_xpermd(struct sepol_xperms_decision *xpermd, avtab_ptr_t node)
{
	unsigned int i;

	if (node->datum.xperms->specified == AVTAB_XPERMS_IOCTLFUNCTION) {
		if (xpermd->driver != node->datum.xperms->driver)
			return -EINVAL;
	} else if (node->datum.xperms->specified == AVTAB_XPERMS_IOCTLDRIVER) {
		if (!xperm_test(xpermd->driver, node->datum.xperms->perms))
			return -EINVAL;
	} else {
		return -EINVAL;
	}
	...
	if (node->key.specified == AVTAB_XPERMS_ALLOWED) {
		xpermd->used |= SEPOL_XPERMS_ALLOWED;
		if (node->datum.xperms->specified == AVTAB_XPERMS_IOCTLDRIVER) {
		...
		if (node->datum.xperms->specified == AVTAB_XPERMS_IOCTLFUNCTION) {
			for (i = 0; i < ARRAY_SIZE(xpermd->allowed->p); i++)
				xpermd->allowed->p[i] |=
					node->datum.xperms->perms[i];
		}
	} else if (node->key.specified == AVTAB_XPERMS_AUDITALLOW) {
		xpermd->used |= SEPOL_XPERMS_AUDITALLOW;
		if (node->datum.xperms->specified == AVTAB_XPERMS_IOCTLDRIVER) {
		...
		xpermd->used |= SEPOL_XPERMS_DONTAUDIT;
		if (node->datum.xperms->specified == AVTAB_XPERMS_IOCTLDRIVER) {
...
int context_compute_xpermsd(context_struct_t * scontext,
			    context_struct_t * tcontext,
			    sepol_security_class_t tclass,
			    struct sepol_xperms_decision *xpermd)
{
	constraint_node_t *constraint;
	struct role_allow *ra;
	avtab_key_t avkey;
	...
	if (scontext == NULL || tcontext == NULL || xpermd == NULL) {
		ERR(NULL, "invalid param");
		return -EINVAL;
	}
	...
	ebitmap_for_each_positive_bit(sattr, snode, i) {
		ebitmap_for_each_positive_bit(tattr, tnode, j) {
			...
			for (node = avtab_search_node(&policydb->te_avtab, &avkey);
				...
				services_compute_xpermd(xpermd, node);
			}
				     ...
				     unsigned int flags)
{
	...
	ebitmap_for_each_positive_bit(sattr, snode, i) {
		ebitmap_for_each_positive_bit(tattr, tnode, j) {
			...
			/* Check conditional av table for additional permissions */
#ifdef CONFIG_SEHARMONY
			cond_compute_av(&policydb->te_cond_avtab, &avkey, avd, NULL);
#else
			cond_compute_av(&policydb->te_cond_avtab, &avkey, avd);
#endif
		}
